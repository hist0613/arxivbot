...
### Prioritized Generative Replay (https://arxiv.org/abs/2410.18082)
- **What's New**: 본 연구는 생성 모델을 이용한 에이전트 메모리의 우선 순위 매개변수를 제안합니다. 이를 통해 에이전트는 온라인 경험을 효율적으로 포착하고, 이를 기반으로 할 수 있는 더 잘 일반화된 경험을 생성하는 것이 가능해집니다.

- **Technical Details**: 제안된 접근법은 두 가지 주요 이점을 제공합니다. 첫째, 과거 경험의 밀도를 높여 온라인에서 관찰된 데이터 분포를 초과하는 새로운 훈련 데이터를 생성할 수 있습니다. 둘째, '관련성 함수(relevance functions)'라는 패밀리를 통해 에이전트가 수집한 경험의 더 유용한 부분으로 생성된 데이터를 유도합니다. 이 접근법을 조건부 확산 모델(conditional diffusion model)과 호기심 기반 메트릭(curiosity-based metrics) 등의 간단한 관련성 함수로 구체화하여 실험을 수행하였습니다.

- **Performance Highlights**: 실험 결과, 본 방법은 상태(state) 기반 및 픽셀(pixel) 기반 도메인 모두에서 성능을 일관되게 개선하였으며, 샘플 효율성(sample efficiency) 역시 높아졌습니다. 또한, 저자는 본 접근법이 생성된 전이의 다양성을 높이고 과적합(overfitting)을 줄이는 방식으로 작동함을 보여주었습니다.
...
### ProFL: Performative Robust Optimal Federated Learning (https://arxiv.org/abs/2410.18075)
Comments:
          27 pages with Appendix, 18 figures. The paper has been submitted and is currently under review

- **What's New**: 이 논문에서는 Performative robust optimal Federated Learning (ProFL)이라는 새로운 알고리즘을 제안합니다. ProFL은 노이즈와 오염된 데이터를 고려하여 performative optimal (수행적 최적점)을 찾을 수 있도록 설계되었습니다.

- **Technical Details**: ProFL은 Polyak-Lojasiewicz (PL) 조건 하에서의 수렴 분석을 제공합니다. 이를 통해 비볼록 (non-convex) 목표 함수에 대해서도 수렴성을 가지며, 기존의 중앙 집중식 방법이 가진 한계를 극복합니다.

- **Performance Highlights**: 다양한 데이터셋에서의 실험을 통해 ProFL이 복잡한 분포 변화에 더 resistant (강인함)을 보이며, 효과적으로 performative optimal (수행적 최적점)을 찾는 능력의 우수성을 입증했습니다.
...
### The Double-Edged Sword of Behavioral Responses in Strategic Classification: Theory and User Studies (https://arxiv.org/abs/2410.18066)
- **What's New**: 이 논문은 알고리즘적 결정 시스템에 대한 인간의 행동 편향을 고려하여 전략적 분류 모델을 제안합니다. 기존 연구들이 완벽하게 합리적인 에이전트 모델을 가정하는 반면, 본 연구는 인간의 인지적 편향이 알고리즘에 대한 반응에 미치는 영향을 탐구합니다.

- **Technical Details**: 본 연구에서는 에이전트가 분류기의 출력에서 특정 기능의 중요성을 잘못 인식하는 방식으로 행동 편향을 나타내는 전략적 분류 모델을 제안합니다. 이는 '게임 이론적 모델링'을 활용하여 에이전트들이 알고리즘의 기능을 과대 또는 과소 평가하는 상황을 분석합니다. 이 과정에서 에이전트가 특정 기능에 대해 과잉 또는 과소 투자하는 조건을 식별합니다.

- **Performance Highlights**: 사용자 연구를 통해 활동적인 AI 결정을 지원하는 시스템에서 인간이 범하는 인지적 편향을 실증적으로 확인했습니다. 특히, 가장 중요한 기능의 중요성을 과소평가하고 least 중요한 feature의 중요성을 과대평가하는 경향을 보였습니다. 또한 기능 복잡성이 증가할수록 이 경향이 더욱 두드러진다는 것을 발견했습니다.



### POD-Attention: Unlocking Full Prefill-Decode Overlap for Faster LLM Inferenc (https://arxiv.org/abs/2410.18038)
- **What's New**: 본 논문에서는 GPU에서 하이브리드 배치(hybrid batching)에서 주목적(attention) 계산을 효율적으로 처리하기 위한 POD-Attention이라는 새로운 커널을 소개합니다. 이 방법은 CPU와 메모리 대역폭을 최적화하여 여러 요청의 prefill과 decode 작업을 동시에 실행할 수 있도록 설계되었습니다.

- **Technical Details**: POD-Attention은 GPU의 리소스를 효율적으로 관리하기 위해 일관된 CTA(협력 스레드 배열) 병합을 통해 prefill과 decode 작업을 동시에 계산합니다. 이는 기존의 FlashAttention 커널을 기반으로 하여, compute와 memory 자원을 요청에 따라 조정하여 주목적 계산을 최대 75% 빠르게 수행합니다.

- **Performance Highlights**: POD-Attention을 Sarathi-Serve와 통합한 결과, 오프라인 추론에서 LLM 서비스 처리량이 최대 22% 증가하고, time-to-first-token(TTFT) 및 request execution latency가 개선되었습니다. 온라인 추론에서도 전체적인 성능 향상이 관찰되었습니다.



### Inferring stability properties of chaotic systems on autoencoders' latent spaces (https://arxiv.org/abs/2410.18003)
- **What's New**: 이번 연구에서는 CAE-ESN(Convolutional Autoencoder-Echo State Network) 모델을 활용하여 고차원 혼돈 시스템의 안정성 특성을 유추하는 방법을 제안하였습니다. 이 모델은 비선형 잠재 공간(latent space)에서 시스템의 동역학을 예측하면서도, 안정성을 평가하는 미지의 특성을 탐구합니다.

- **Technical Details**: 연구의 핵심은 Kuramoto-Sivashinsky (KS) 방정식이라는 전형적인 편미분방정식을 기반으로 하여 혼돈 시스템의 안정성을 분석하는 것입니다. 저자들은 CAE를 사용하여 비선형 축소 차원 표현을 만들고, 이를 바탕으로 ESN을 통해 기존의 데이터를 낮은 차원의 잠재 공간에서 표현합니다. Lyapunov 지수(Lyapunov exponents)와 공변 Lyapunov 벡터(covariant Lyapunov vectors)를 이용하여 잠재 공간에서의 안정성 특성을 유추합니다.

- **Performance Highlights**: 연구는 고차원 혼돈 시스템에서 낮은 차원으로의 매핑이 가능하다는 점과, CAE-ESN이 동역학과 안정성 특성을 동시에 유추할 수 있음을 보여줍니다. 이는 고차원 데이터의 안정성 분석에 대한 새로운 기회를 열어줍니다.



### Estimating the Spectral Moments of the Kernel Integral Operator from Finite Sample Matrices (https://arxiv.org/abs/2410.17998)
- **What's New**: 본 논문에서는 유한한 표본 행렬에서 커널 적분 연산자의 스펙트럼 모멘트에 대한 비편향 추정값을 제공하는 새로운 알고리즘을 소개합니다. 이 방법은 동적 프로그래밍(dynamic programming)을 기반으로 하여, 신경망(neural network)의 학습된 표현의 기하학을 이해하는 데 유용성과 강건성을 보여줍니다.

- **Technical Details**: 새로 제안된 알고리즘은 유한한 측정 행렬에서 스펙트럼 모멘트를 추정하기 위해 비반복 순환의 적절한 곱을 평균화하는 방법을 사용합니다. 이 절차는 계산적으로 효율적이며, 행렬의 크기와 모멘트의 차수에 대해 다항식(polynomial) 시간 복잡도를 가집니다.

- **Performance Highlights**: 우리의 방법은 방사 기저 함수(radial basis function, RBF) 커널 연산자에 대해 정확성을 입증하였으며, 이론적인 스펙트럼 및 다른 추정 방법과의 직접 비교를 통해 그 성능을 강조합니다. 또한, 신경망의 학습 동역학을 분석하는데 사용될 수 있음을 보여주고, 다양한 너비의 신경망들이 어떻게 커널 연산자에 의해 관련될 수 있는지를 강조합니다.
...
### POMDP-Driven Cognitive Massive MIMO Radar: Joint Target Detection-Tracking In Unknown Disturbances (https://arxiv.org/abs/2410.17967)
Comments:
          The paper has been submitted to ieee Transactions on radar systems

- **What's New**: 이 연구는 Partially Observable Markov Decision Process (POMDP) 프레임워크를 활용하여 복잡한 환경에서의 목표물 추적 및 탐지 성능을 향상시키는 방법을 제안합니다.

- **Technical Details**: POMDP 설정에서 레이더 시스템은 지능형 에이전트로 간주되며 지속적으로 주변 환경을 감지하고, 탐지 확률 $(P_D)$를 극대화하며 목표물의 위치와 속도 추정을 개선하는 행동을 최적화합니다. 제안된 접근 방식은 기본적으로 전통적인 range-azimuth-elevation 모델보다 더 일반적인 관측 모델에 의존하며, 노이즈 통계에 대한 사전 지식 없이 작동하는 온라인 알고리즘을 사용합니다.

- **Performance Highlights**: 시뮬레이션 결과, POMDP 기반 알고리즘이 최신 연구에서 다룬 State-Action-Reward-State-Action (SARSA) 기반 방법보다 유의미한 성능 향상을 보였습니다.
...
### Generalized Resubstitution for Regression Error Estimation (https://arxiv.org/abs/2410.17948)
- **What's New**: 이 논문에서는 회귀를 위한 일반화된 재대체 오차 추정량(generalized resubstitution error estimators)을 제안합니다. 각 추정량은 경험적 확률 측정(empirical probability measures) 및 손실 함수(loss function)에 대한 선택에 따라 다릅니다.

- **Technical Details**: 일반화된 재대체 오차 추정량은 임의의 경험적 확률 측정에 따라 손실 함수의 기대값으로 정의되며, 특히 이 논문에서는 이차 손실 함수(quadratic loss function)에 대한 회귀 문제에 적용됩니다. 제안된 추정량의 일관성을 위해 여러 가지 충분 조건이 정립되었습니다.

- **Performance Highlights**: 세부 실험 결과를 통해 제안된 추정량은 유한 샘플에서의 편향(bias) 및 분산(variance) 측면에서 우수한 성능을 보여주며, R 코드도 제공됩니다.



### Spiking Graph Neural Network on Riemannian Manifolds (https://arxiv.org/abs/2410.17941)
Comments:
          Accepted by NeurIPS 2024, 30 pages

- **What's New**: 본 논문에서는 리만 기하학(Riemannian geometry)에 기반한 새로운 스파이킹 그래프 신경망(Spiking Graph Neural Network, GNN)인 Manifold-valued Spiking GNN (MSG)를 제안합니다. 기존의 스파이킹 GNNs가 유클리드 공간(Euclidean space)에서 작동하는 문제를 해결하고, 구조적 기하학을 고려한 유망한 접근 방식을 제공하는 것이 특징입니다.

- **Technical Details**: MSG는 지오데식 완전한 매니폴드(geodesically complete manifolds)에서 새로운 스파이킹 뉴런을 설계하였으며, 여기서 스파이크에 대한 역전파(Back-Propagation-Through-Time, BPTT)를 대체하는 매니폴드를 통한 미분(Differentiation via Manifold)이 도입되었습니다. MSG는 매니폴드 보통 미분 방정식(Ordinary Differential Equation, ODE)을 근사하는 방식으로, 각 레이어가 매니폴드의 차트를 생성하고, 반복적 계산 없이 그래디언트를 전파할 수 있습니다.

- **Performance Highlights**: MSG는 기존의 스파이킹 GNNs보다 우수한 성능을 보여주며, 에너지 효율성 또한 기존의 GNNs보다 뛰어납니다. 실험 결과는 MSG의 효과성과 에너지 효율성이 검증되었습니다.
...
### regAL: Python Package for Active Learning of Regression Problems (https://arxiv.org/abs/2410.17917)
- **What's New**: 이 논문은 회귀 문제를 위한 새로운 Python 패키지 regAL을 소개합니다. 이 패키지는 사용자가 다양한 active learning 전략을 평가할 수 있도록 설계되었습니다.

- **Technical Details**: regAL은 사용자가 최소한의 입력으로 (label이 있는) 데이터셋을 제공하는 것만으로도 다양한 샘플 선택 방법을 비교할 수 있도록 합니다. 내부적으로 Gaussian process regressor와 RBF 커널을 기본으로 사용하지만, 다른 모델 아키텍처로 사용자 설정이 가능합니다.

- **Performance Highlights**: 이 패키지를 사용하면 사용자는 더 이상 여러 시도 실험을 수행할 필요 없이 데이터셋에 가장 적합한 active learning 방법을 평가할 수 있습니다. regAL은 학습된 모델을 사용해 unlabeled 데이터에 대해 active learning을 수행할 수 있게 해줍니다.



### Deep learning for model correction of dynamical systems with data scarcity (https://arxiv.org/abs/2410.17913)
- **What's New**: 본 논문은 제한된 고신뢰도(high-fidelity) 데이터 세트를 사용하여 기존의 동적 시스템 모델을 보정하는 딥러닝 프레임워크를 제시합니다. 일반적으로 낮은 신뢰도(low-fidelity) 모델은 동작을 어느 정도 잘 캡처하지만, 모델의 본질적인 한계로 인해 고해상도를 갖지 못합니다. 이 연구는 고신뢰도 데이터가 극히 적을 때 데이터를 이용한 모델링 방법의 한계를 극복하는 데 초점을 맞춥니다.

- **Technical Details**: 제안된 방법은 기존의 낮은 신뢰도 모델을 근사하는 딥 신경망(DNN) 모델을 찾고, 이를 통해 제한된 고신뢰도 데이터를 사용하여 전이 학습(transfer learning, TL)으로 DNN 모델을 보정합니다. 이 접근법은 모델 보정 용어의 특정 형태를 가정하지 않고, TL을 통해 낮은 신뢰도 모델에 대한 내재적 보정을 제공합니다.

- **Performance Highlights**: 제안된 방법이 효과적임을 입증하기 위해 다양한 수치 예제가 제공됩니다. 이 방법은 고신뢰도 데이터가 제한적일 때도 동적 시스템의 예측 정확성을 개선할 수 있음을 보여줍니다.
...
### Scalable Offline Reinforcement Learning for Mean Field Games (https://arxiv.org/abs/2410.17898)
Comments:
          Submitted to AAMAS

- **What's New**: 본 논문에서는 Offline Munchausen Mirror Descent (Off-MMD)라는 새로운 mean-field RL 알고리즘을 제안합니다. Off-MMD는 기존의 online 상호작용이나 시스템 동역학에 의존하지 않고 순전히 offline 데이터만을 사용하여 mean-field 게임에서의 균형 정책을 근사화합니다.

- **Technical Details**: Off-MMD는 반복적인 mirror descent 및 importance sampling 기법을 활용하며, 고정된 데이터셋에서 mean-field 분포를 추정합니다. 또한, 최적 Q-value의 과대 평가 문제를 해결하기 위한 오프라인 강화 학습 기법을 통합하여 데이터 커버리지가 제한된 경우에도 신뢰할 수 있는 정책 학습을 보장합니다.

- **Performance Highlights**: Off-MMD는 복잡한 환경에서 확장 가능하며, crowd exploration 또는 navigation과 같은 벤치마크 작업에서 강력한 성능을 발휘하는 것을 보여줍니다. 실험을 통해 낮은 품질의 데이터셋에 대한 강인성을 입증하고, 하이퍼파라미터 선택에 대한 민감도를 조사했습니다.



### Identifiable Representation and Model Learning for Latent Dynamic Systems (https://arxiv.org/abs/2410.17882)
- **What's New**: 이 논문은 고유(tangible)한 표현(Representation) 및 모델 학습 문제를 다루며, 동적 시스템(Dynamic Systems)의 고유한 표현을 학습할 수 있는 새로운 방식을 제시합니다. 특히, 제어 정준형(control canonical forms)에 영감을 받은 유도 편향(inductive bias)을 통해, 복잡한 잠재 변수 관계를 식별할 수 있음을 보입니다.

- **Technical Details**: 이 연구에서는 선형(linear) 및 아핀 비선형(affine nonlinear) 잠재 동적 시스템을 고려하여 표현과 모델을 식별할 수 있는 가능성을 입증합니다. 잠재 동적 시스템의 명확한 메커니즘 덕분에, 시스템을 단순 변형에 따라 모델링할 수 있습니다. 또한 이 방법은 고차(high-order) 동적 시스템에 대해서도 확장 가능합니다.

- **Performance Highlights**: 학습된 표현은 스케일링(scaling)에 따른 고유성을 가지며, 단일 입력 선형 시스템(single-input linear systems)에서는 시스템 행렬의 계수(coefficients)가 진실 값(truth-value)과 일치 합니다. 다중 입력 선형 시스템 및 아핀 비선형 시스템의 경우에는, 계수 또는 계수 함수가 사소한 변형에 따라 식별됩니다.



### AdaRankGrad: Adaptive Gradient-Rank and Moments for Memory-Efficient LLMs Training and Fine-Tuning (https://arxiv.org/abs/2410.17881)
- **What's New**: 이 논문에서는 대규모 언어 모델(LLM) 훈련 시 메모리 요구사항을 최적화하고 성능을 개선할 수 있는 새로운 방법을 소개합니다. 이 새로운 접근법은 훈련이 진행됨에 따라 예상되는 레이어 그래디언트의 계수가 점진적으로 감소한다는 현상에 영감을 받았습니다.

- **Technical Details**: 이 접근법은 Adam 최적화 단계에서 그래디언트의 계수를 적응적으로 줄이는 방식으로, 효율적인 온라인-업데이트(low-rank projections) 규칙을 사용하여 진행됩니다. 이를 위해 무작위 SVD 스킴을 이용하여 투영 행렬을 효율적으로 찾고, 전체 매개변수 조정(full-parameter fine-tuning)을 가능하게 합니다.

- **Performance Highlights**: 이 기술은 기존의 최신 방법들과 비교해 훈련 중 전체 메모리 요구사항을 유의미하게 줄이면서도 사전 훈련(pretraining) 및 미세 조정(fine-tuning) 시 모델 성능을 개선합니다.



### Relaxed Equivariance via Multitask Learning (https://arxiv.org/abs/2410.17878)
- **What's New**: 본 논문에서는 REMUL이라는 훈련 절차를 제안합니다. REMUL은 multitask learning을 사용하여 접근 가능한 equivariance를 학습할 수 있도록 합니다. 이 방식은 기존의 제약된 모델 대신 비제한 모델이 근사적인 대칭성을 학습할 수 있도록 지원합니다.

- **Technical Details**: REMUL은 비제한 모델에 대해 equivariance를 학습하는 방법으로, 가중치를 조정하여 모델의 대칭성 표현 수준을 조절합니다. 이 과정은 Transformers와 Graph Neural Networks (GNNs)의 성능을 비교 평가하며, 저비용으로 더욱 유연한 대칭성 모델을 가능하게 합니다.

- **Performance Highlights**: REMUL을 적용한 모델은 equivariant baselines과 비교하여 inference에서 최대 10배, training에서는 2.5배 더 빠른 성능을 보여줍니다. 이는 선형 변환기에 비해 더 나은 최적화 경량 솔루션을 제공함을 나타냅니다.



### Population stratification for prediction of mortality in post-AKI patients (https://arxiv.org/abs/2410.17865)
- **What's New**: 이 연구는 급성 신 손상(AKI) 환자의 임상 후속 관리를 지원하는 도구를 개발하였으며, 이는 환자 그룹의 분류와 예측 모델을 포함하여 환자의 사망 위험을 평가하는 자동화된 절차에 중점을 두었습니다.

- **Technical Details**: 본 연구는 Clustering Aware Classification 기법에 영감을 받아 환자 그룹의 유사성을 기준으로 그룹화하고 이 과정을 통해 예측 향상 및 새로운 환자의 정확한 할당을 달성합니다. 최종 예측 모델은 각 그룹의 Receiver Operating Characteristic curve의 면적 합(AUROC)을 최적화하여 설정되었습니다.

- **Performance Highlights**: 예측의 정확성은 테스트 데이터 세트를 사용하여 평가되었으며, GAM(Generalized Additive Models)을 사용하여 예측 모델의 신뢰성을 높였습니다. 본 연구는 AKI 후 환자의 병원에서 퇴원 후 90일 이내 사망 위험을 예측하며, 이는 임상 의사결정의 지원 지표로 활용될 수 있습니다.
...
### Is the GPU Half-Empty or Half-Full? Practical Scheduling Techniques for LLMs (https://arxiv.org/abs/2410.17840)
Comments:
          12 pages, 11 figures

- **What's New**: 이 논문에서는 대형 언어 모델(LLM)을 위한 새로운 스케줄링 기법인 LARRY와 SAL를 도입하여 현재 기술들을 능가하는 성능을 보이고 있습니다. 이 두 가지 기법은 구현이 용이하며 실질적인 성능 향상을 제공합니다.

- **Technical Details**: LLM의 요청 처리에서 스케줄링 기술을 개선하기 위해, LARRY(Load-Adaptive Request Reordering for Low Latency)는 요청의 예상 리소스 소비와 현재 시스템 부하에 따라 요청을 재배치합니다. SAL(Server-Aware Load Balancer)은 토큰 인식 부하 분산기로, 기본 메트릭만으로도 작동합니다. 이들은 모두 기존 시스템에 쉽게 적용할 수 있습니다.

- **Performance Highlights**: LARRY와 SAL는 각기 20줄, 30줄의 코드로 구현되었으며, 현재의 스케줄링 기법들을 초월하는 성능을 나타내고 있습니다. 실무 워크로드 추적에서 더욱 향상된 성능을 보여주며, 다양한 LLM 시스템에 적용 가능하여 운영 비용을 감소시킬 수 있습니다.



### Optimal Streaming Algorithms for Multi-Armed Bandits (https://arxiv.org/abs/2410.17835)
Comments:
          24pages

- **What's New**: 이 논문은 제한된 메모리 내에서 도착하는 스트리밍 모델 하의 최적 팔(arm) 식별(best arm identification, BAI) 문제의 두 가지 변형을 연구합니다. 이는 스트리밍 ε-top-k arms identification 문제와 스트리밍 BAI 문제를 포함합니다.

- **Technical Details**: 첫 번째로, 단일 크기의 메모리를 사용하여 k개의 팔을 식별하는 알고리즘을 제안하며, k에 관계없이 최적의 샘플 복잡도 O(n/ε² log(k/δ))를 달성합니다. 두 번째로, 최대 보상을 가진 팔을 식별하기 위한 단일 팔 메모리 알고리즘을 제시하고, O(log(Δ₂⁻¹)) 패스를 통해 instance-dependent 최적 샘플 복잡도를 달성합니다.

- **Performance Highlights**: 제안된 알고리즘은 단일 패스를 통해 최적 샘플 복잡도를 달성하며, 많은 응용에서의 실용성을 높이고 있습니다. 기존 알고리즘과 비교하여 메모리 효율과 정확성을 동시에 강화했습니다.
...
### A Comprehensive Analysis on the Learning Curve in Kernel Ridge Regression (https://arxiv.org/abs/2410.17796)
- **What's New**: 이 논문은 Kernel Ridge Regression (KRR)의 학습 곡선을 최소한의 가정 하에 포괄적으로 연구했습니다. 주요 기여는 세 가지로 나뉘며, 1) 커널의 주요 특성 분석, 2) Gaussian Equivalent Property (GEP)의 타당성 입증, 3) 다양한 설정에서 더 나은 경계를 도출하는 것입니다.

- **Technical Details**: KRR의 학습 곡선은 샘플 크기가 무한대로 성장할 때 테스트 오류가 어떻게 감소하는지에 대한 것입니다. 주목할 점은 eigen-decay, 고유 함수의 특성 및 커널의 매끄러움을 포함한 여러 주요 커널 특성에 집중하는 것입니다.

- **Performance Highlights**: 본 연구는 KRR의 테스트 오류에 대한 새로운 한계를 제시하며, 독립적 (Gaussian) 및 종속적 (커널) 특성이 특정 조건 하에 유사한 일반화 성능을 가진다는 점 및 커널 매끄러움과 일반화 성능 간의 관계를 밝혀냈습니다.
...
### Locating Information in Large Language Models via Random Matrix Theory (https://arxiv.org/abs/2410.17770)
Comments:
          17 pages, 14 figures

- **What's New**: 대형 언어 모델(LLMs)의 내부 작동 방식을 이해하기 위해 사전 훈련된 변환기 모델(BERT와 Llama)의 가중치 행렬을 무작위 행렬 이론(Random Matrix Theory, RMT)을 사용하여 분석하였습니다. 이 연구는 훈련 후의 가중치 변화와 RMT 예측의 차이를 보여줌으로써 모델 내에서 학습된 구조를 찾는 방법을 제시합니다.

- **Technical Details**: 가중치 행렬의 고유 값 분해(Singular Value Decomposition, SVD)를 통해 BERT 및 Llama 모델의 학습 동역학을 분석하였습니다. RMT를 기반으로 가중치 행렬의 분포가 특정 타겟과 다른 지역을 찾는 데 도움을 주며, 이 지역이 특성 학습을 나타냄을 확인하였습니다. 특히, 훈련된 BERT 모델에 작은 고유 값들을 제거할 경우 성능에 미치는 영향을 조사하였습니다.

- **Performance Highlights**: 작은 고유 값은 모델의 능력 향상에 중요한 역할을 하며, 이를 제거하는 것은 이미 정렬된 변환기 모델의 기능성에 악영향을 줄 수 있음을 발견했습니다. 따라서, 이러한 결과는 LLM의 미세 조정(fine-tuning) 과정에서 작은 고유 값의 중요성을 강조합니다.



### Faster Language Models with Better Multi-Token Prediction Using Tensor Decomposition (https://arxiv.org/abs/2410.17765)
- **What's New**: 이번 논문에서는 트랜스포머(Transformer) 모델을 위한 multi-token(다중 토큰) 예측을 향상시키기 위한 새로운 모델을 제안합니다. 샘플링 효율성을 높이면서도 정확도를 유지하는 것이 목표입니다. 최근 연구에서 여러 heads를 이용해 다음 토큰의 확률을 예측하는 방식을 rank-1 canonical tensor decomposition과 연결하고, 이를 일반화하여 rank-r canonical probability decomposition을 통해 개선된 모델이 다중 토큰을 동시에 예측할 수 있도록 합니다.

- **Technical Details**: 제안된 방식은 rank-r CP (Canonical Polyadic) 텐서 분해를 기반으로 하며, 각 토큰 간의 의존성을 고려하여 미래 토큰의 결합 확률 분포를 더 정확하게 근사합니다. 기존의 multi-token 예측 방식이 가진 독립성 가정을 완화하여, 전문가 모델의 혼합(mixture of experts)으로 해석될 수 있도록 합니다.이는 효율적인 학습을 위한 기존 기법을 활용할 수 있게 해줍니다.

- **Performance Highlights**: 제안된 방법은 텍스트 및 코드 생성 작업에서 추론 속도를 유의미하게 향상시켰으며, self-speculative decoding 패러다임 내에서 특히 유용합니다. 여러 모델 크기와 학습 epoch에 걸쳐 효과성을 유지하여 강 robustness와 scalability(확장성)를 강조합니다.
...
### Anomaly Resilient Temporal QoS Prediction using Hypergraph Convoluted Transformer Network (https://arxiv.org/abs/2410.17762)
Comments:
          16 pages, 12 figures

- **What's New**: 본 논문에서는 QoS(서비스 품질) 예측의 어려움을 해결하기 위해 실시간, 신뢰성 기반의 프레임워크를 소개합니다. 이 프레임워크는 Hypergraph Convoluted Transformer Network (HCTN)라는 최첨단 딥 아키텍처를 사용하여 데이터 희소성을 효과적으로 해결합니다.

- **Technical Details**: HCTN은 그래프의 하이퍼 엣지를 통해서 복잡한 고차 상관관계를 캡처하는 하이퍼그래프 구조와 그래프 컨볼루션을 결합합니다. 이와 함께 멀티헤드 어텐션과 병렬 1D 컨볼루션 레이어 및 완전 연결된 밀집 블록을 활용하여 세부적이고 전반적인 동적 패턴을 포착합니다.

- **Performance Highlights**: HCTN은 대규모 WSDREAM-2 데이터셋에서 응답 시간(response time)과 처리량(throughput) 측면에서 최첨단의 성능을 보였습니다.



### Topology meets Machine Learning: An Introduction using the Euler Characteristic Transform (https://arxiv.org/abs/2410.17760)
- **What's New**: 본 논문은 기계 학습에서 위상수학(topology) 개념이 어떻게 연구를 풍부하게 할 수 있는지를 논의합니다. Euler Characteristic Transform (ECT)을 예로 들어, 점 구름(point clouds), 그래프(graphs), 그리고 메시(meshes)를 분석하기 위한 더 효율적인 모델을 만드는 다양한 사용 사례를 제시합니다.

- **Technical Details**: 위상학적 딥러닝(topological deep learning)이라는 새로운 분야에서, 점 집합 위상(point-set topology), 대수적 위상(algebraic topology), 그리고 미분 위상(differential topology) 기법을 사용하여 데이터의 부드러운 성질을 분석하고 효율적인 방법을 제시합니다. 이는 기존 알고리즘을 그래프에서 셀 복합체(cell complexes)로 확장하는 데 기여하고 있습니다.

- **Performance Highlights**: 기계 학습 알고리즘은 많은 예를 통해 적절한 함수를 학습하여, 새로운 예제에 대한 예측을 개선할 수 있게 됩니다. 딥러닝(deep learning) 모델은 특히 이미지 인식(computer vision) 분야에서 혁신을 가져왔으며, 적절한 데이터와 매개변수를 통해 매우 높은 예측 능력을 보여줍니다.
...
### Optimizing Load Scheduling in Power Grids Using Reinforcement Learning and Markov Decision Processes (https://arxiv.org/abs/2410.17696)
- **What's New**: 본 논문에서는 전통적인 최적화 방법이 겪는 한계를 극복하기 위해 마르코프 결정 프로세스(Markov Decision Process, MDP) 프레임워크를 활용한 강화 학습(Reinforcement Learning, RL) 접근 방법을 제안합니다.

- **Technical Details**: MDP는 전력망 상태를 나타내는 상태 공간(state space), 발전기 조정 및 저장 관리와 같은 제어 작업을 포함하는 행동 공간(action space), 경제적 효율성과 시스템 신뢰성을 균형 잡는 보상 함수(reward function)로 정의됩니다. 여기서 기본 Q-Learning부터 더 발전된 Deep Q-Networks (DQN) 및 Actor-Critic 방법에 이르기까지 다양한 RL 알고리즘의 적용을 탐구합니다.

- **Performance Highlights**: 제안된 접근 방식은 시뮬레이션된 전력망 환경에서 평가되어 스케줄링 효율을 향상시키고 변동하는 수요 패턴에 적응할 수 있는 잠재력을 보여주며, RL 기반 방법이 실시간 부하 스케줄링을 위한 강력하고 확장 가능한 솔루션을 제공함을 입증합니다.



### Towards Active Participant-Centric Vertical Federated Learning: Some Representations May Be All You Need (https://arxiv.org/abs/2410.17648)
- **What's New**: 본 연구에서는 Vertical Federated Learning (VFL)의 새로운 접근방식을 제안합니다. Active Participant-Centric VFL (APC-VFL)은 단 한 번의 통신 라운드만을 요구하며, 이는 VFL 분야에서의 대규모 통신 비용 절감을 가능하게 합니다.

- **Technical Details**: APC-VFL은 비지도 표현 학습(unsupervised representation learning)과 지식 증류(knowledge distillation)를 통합하여, 기존의 VFL 방식과 유사한 정확도를 달성합니다. 이 방법은 고전적인 환경에서 수직 분할 학습(vertical split learning)을 기반으로 하며, 통신 라운드를 최대 4200배 줄이는 성과를 가져옵니다.

- **Performance Highlights**: APC-VFL은 비연합 로컬 모델(non-federated local models) 및 기존의 VFL 제안인 VFedTrans와 비교해도 우수한 성능을 보이며, 협업 학습을 위한 효율적이고 유연한 솔루션을 제공합니다.



### Entity-based Reinforcement Learning for Autonomous Cyber Defenc (https://arxiv.org/abs/2410.17647)
Comments:
          Material to appear in the proceedings of the 1st International Workshop on Autonomous Cybersecurity at ACM CCS 2024

- **What's New**: 본 논문은 자율 사이버 방어 분야에서 에이전트가 다양한 네트워크 토폴로지와 구성에서 일반화할 수 있는 능력을 보장하는 문제를 다루고 있습니다. 특히, 엔티티 기반 강화 학습(Entity-based Reinforcement Learning) 방법론을 적용하여 에이전트의 관찰 및 행동 공간을 디스크리트 엔티티의 집합으로 분해했습니다. 이를 통해 Transformer 기반의 정책을 훈련하여 다양한 네트워크 토폴로지에서 일반화 가능성을 테스트했습니다.

- **Technical Details**: 본 연구에서는 Yawning Titan 사이버 보안 시뮬레이션 환경에서 엔티티 기반 RL을 활용하였으며, 에이전트의 관찰 공간은 환경 인스턴스에 따라 가변적입니다. 각 노드는 하나의 엔티티로 처리되며, Entity-Gym 프레임워크를 통해 다양한 환경에 원활히 적용될 수 있는 기반을 마련했습니다. 에이전트는 Proximal Policy Optimization (PPO) 알고리즘을 통해 10, 20, 40 개 노드의 네트워크 구조에서 훈련되었습니다.

- **Performance Highlights**: 실험 결과, 엔티티 기반 정책이 MLP 기반 정책보다 훈련 중 더 나은 성능을 보였으며, 특히 랜덤 모드에서 MLP 정책이 학습하는 데 어려움을 겪었습니다. 또한, 엔티티 기반 정책은 훈련 중에 보지 않은 네트워크 크기에서도 유사한 성능을 발휘하는 것으로 나타나, 물리적으로 다르게 배치된 네트워크에서도 제로샷(zero-shot) 일반화가 가능함을 입증했습니다.



### Feature Learning in Attention Mechanisms Is More Compact and Stable Than in Convolution (https://arxiv.org/abs/2410.17628)
- **What's New**: 이번 연구에서는 Attention(어텐션) 메커니즘과 Convolution(합성곱) 기반 모델의 특성 학습 다이나믹스를 직접 비교하였습니다. Wasserstein 거리와 covering numbers에 대한 Lipschitz 연속성을 비교함으로써, 어텐션이 데이터를 더 간결하고 안정적으로 처리함을 발견하였습니다.

- **Technical Details**: 본 논문에서는 Attention과 Convolution의 평균장 영역(mean-field regime)을 탐구하며, Wasserstein-Lipschitz 조건을 비교하여 어텐션 메커니즘이 입력 데이터 분포의 변동에 더 강한 내성을 지닌 것을 보여주었습니다. 또한, 실험을 통해 Topological Data Analysis(TDA)를 이용하여 두 모델의 feature learning 안정성을 확인하였습니다. Vision Transformers(비전 트랜스포머)와 Residual Networks(잔차 네트워크)를 포함한 다양한 아키텍처에 대한 분석도 수행하였습니다.

- **Performance Highlights**: 어텐션 레이어에서의 특성 학습이 합성곱 레이어보다 더 안정적이며, 낮은 내재적 차원 공간에서 발생하는 경향을 실험적으로 입증하였습니다. 비전 트랜스포머는 높은 출력 분산을 보이지만 그 특성 학습의 안정성은 잔차 네트워크보다 더 뛰어난 결과를 보였습니다.



### Incremental Learning of Affordances using Markov Logic Networks (https://arxiv.org/abs/2410.17624)
Comments:
          accepted at IEEE IRC 2024

- **What's New**: 로봇이 주변 환경을 이해하는 데 도움을 주는 affordances(어포던스)를 캡처하는 새로운 MLN Cumulative Learning Algorithm (MLN-CLA)을 제안합니다. 이 알고리즘은 새롭게 수집된 정보를 기존 지식에 통합하여 지식을 누적적으로 학습할 수 있는 기능을 제공합니다.

- **Technical Details**: Markov Logic Networks (MLN)을 활용해 기계 학습 모델에서 어포던스를 학습하며, MLN-CLA는 새로운 관계를 유지하고 기존 지식만 업데이트하여 재학습을 피합니다. 이 방법은 기존의 지식 기반을 관리하기 위해 지식 카테고리와 지식 목록 개념을 도입합니다.

- **Performance Highlights**: MLN-CLA는 누적 학습(cumulative learning) 및 zero-shot affordance inference에서 뛰어난 성능을 보여주며, 강력한 벤치마크를 능가하는 효과적인 알고리즘임을 입증했습니다.



### Self-Supervised Graph Neural Networks for Enhanced Feature Extraction in Heterogeneous Information Networks (https://arxiv.org/abs/2410.17617)
- **What's New**: 이 연구에서는 인터넷의 급속한 발전으로 인해 발생하는 복잡한 그래프 데이터 처리에 그래프 신경망(GNNs)의 응용과 도전을 탐구합니다. 기존의 GNN 방법들이 그래프의 초기 구조와 속성 정보에 지나치게 의존하는 문제를 극복하고자 자기 지도 학습(self-supervised learning) 프레임워크 하에서 새로운 모델을 제안합니다.

- **Technical Details**: 제안된 모델은 그래프의 여러 속성과 노드의 다양한 추가 정보를 유연하게 결합할 수 있는 구조를 가지고 있습니다. 이러한 접근은 그래프 데이터에서 깊이 있는 특징을 더 잘 발굴할 수 있도록 돕습니다. 자기 감독 메커니즘(self-supervisory mechanism)을 도입하여, 기존 모델이 그래프 데이터의 다양성과 복잡성에 더 잘 적응하도록 개선하는 것을 목표로 합니다.

- **Performance Highlights**: 이 연구에 의해 개발된 모델이 기존 모델의 성능을 향상시킬 것으로 예상되며, 다양한 그래프 데이터에서의 적응력과 전반적인 성능을 개선하는 데 중요한 기여를 할 것으로 보입니다.



### A Kernel Perspective on Distillation-based Collaborative Learning (https://arxiv.org/abs/2410.17592)
Comments:
          Accepted to NeurIPS 2024

- **What's New**: 이번 연구는 비공식적 데이터나 모델을 직접 공유하지 않고도 비모수적 협업 학습 알고리즘의 (거의) minimax 최적성을 증명하는 첫 번째 이론적 결과를 제공합니다. 이 결과는 통계적으로 이질적인 환경에서도 적용될 수 있습니다.

- **Technical Details**: 논문에서는 Kernel Regression(커널 회귀) 관점에서 기존의 distillation 기반 알고리즘을 분석하였으며, DCL-KR이라는 비모수적 알고리즘을 통해 협업 학습 환경에서의 성능을 향상시킵니다. 각 지역 AI 모델의 특징 커널을 조정하기 위한 DCL-NN 알고리즘을 제안하였습니다.

- **Performance Highlights**: DCL-KR은 중앙 집중식 모델과 동등한 성능을 달성하며, DCL-NN은 기존의 DCL 프레임워크에 비해 대부분의 환경에서 현저히 더 나은 성과를 보였습니다.
...
### Adversarial Domain Adaptation for Metal Cutting Sound Detection: Leveraging Abundant Lab Data for Scarce Industry Data (https://arxiv.org/abs/2410.17574)
Comments:
          8 pages, 3 figures, 3 tables, First two named Authors have equal contribution (Co-first author)

- **What's New**: 본 연구는 밀링(milling) 공정에서의 절삭 상태 모니터링을 위한 혁신적인 적대적 도메인 적응(adversarial domain adaptation) 방법을 제안합니다. 기존의 절삭 소리 감지 기법을 개선하기 위해 실험실에서 수집한 데이터를 활용하여 산업 데이터의 부족 문제를 해결하는 모델을 개발하였습니다.

- **Technical Details**: 연구에서는 lab 데이터에 적합한 두 개의 잠재 공간(latent spaces)을 구성하여 도메인 독립적 표현(domain-independent representations)을 학습하는 방식을 채택하였습니다. 두 가지 적대적 학습(adversarial learning) 메커니즘을 분석하여, 판별기(discriminator)가 각기 다른 설정에서 적대자(adversary) 및 평가자(critic)로 작용하도록 하여, 도메인 불변 및 도메인 내재적 특징(domain-ingrained features)을 효과적으로 학습합니다.

- **Performance Highlights**: 제안된 모델은 세 가지 서로 다른 센서를 통해 수집한 데이터를 기반으로 실험을 진행하였고, 산업 환경에서의 라벨링 태스크(labeling tasks)에서 multi-layer perceptron(WAN) 기반의 전통적 도메인 적응 모델들보다 우수한 성능을 보였습니다. 정확도는 각각 92%, 82%, 85%에 도달하여, 모델의 효과성을 입증하였습니다.



### Securing Federated Learning Against Novel and Classic Backdoor Threats During Foundation Model Integration (https://arxiv.org/abs/2410.17573)
- **What's New**: 본 논문은 Federated Learning (FL) 환경에서 Foundation Model (FM) 통합으로 인한 새로운 백도어 공격 메커니즘을 제시하고, 기존 백도어 방어 방법의 한계를 극복할 수 있는 데이터 없는 방어 전략을 제안합니다.

- **Technical Details**: 제안된 방어 전략은 모델 집계 중 서버에서 숨겨진 특성 공간의 비정상적 활성화를 제한하여 효과적으로 공격을 완화하는 방법입니다. 이 활성화 제약은 생성된 합성 데이터와 함께 최적화하며 모델 성능에 거의 영향을 미치지 않도록 구성되었습니다.

- **Performance Highlights**: 광범위한 실험 결과는 제안된 방법이 기존 방어 기법보다 높은 성능을 발휘함을 보여주며, 새로운 및 기존의 백도어 공격에 대해 효과적으로 대응할 수 있음을 입증하였습니다.
...
### DisenGCD: A Meta Multigraph-assisted Disentangled Graph Learning Framework for Cognitive Diagnosis (https://arxiv.org/abs/2410.17564)
Comments:
          21 pages, Accepted by NeurIPS 2024 as a poster

- **What's New**: 본 논문에서는 기존의 인지 진단(cognitive diagnosis, CD) 방법들의 한계를 극복하기 위해 메타 멀티 그래프 기반의 분리된 그래프 학습 프레임워크인 DisenGCD를 제안합니다. 이 프레임워크는 학생-문제-개념의 상호작용을 모델링하기 위해 세 가지의 분리된 그래프를 사용하여 각각의 표현을 학습합니다.

- **Technical Details**: DisenGCD는 학생-문제-개념의 상호작용 그래프, 문제-개념 관계 그래프, 개념 종속 그래프의 세 가지 그래프에서 세 가지 유형의 표현을 학습합니다. 학생 표현은 상호작용 그래프에서 메타 멀티 그래프 학습 모듈을 통해 학습되며, 이는 여러 학습 가능 경로를 통해 현재 학생의 잠재 표현이 저차원 문제 잠재 표현에 접근할 수 있도록 합니다. 문제 및 개념 표현은 관계 그래프와 종속 그래프에서 그래프 주의(module attention) 네트워크를 통해 학습됩니다.

- **Performance Highlights**: 실험 결과, DisenGCD는 최신 CD 방법들보다 성능과 견고성에서 우수한 결과를 보였으며, 분리된 학습 프레임워크와 메타 멀티 그래프 모듈의 효과를 입증하였습니다.



### Multimodal Information Bottleneck for Deep Reinforcement Learning with Multiple Sensors (https://arxiv.org/abs/2410.17551)
Comments:
          31 pages

- **What's New**: 이 논문에서는 강화 학습(RL)을 이용한 로봇 제어에서 여러 감각 모달리티(multiple sensory modalities)의 정보를 압축하여 정책 학습에 유용한 공동 표현(joint representations)을 학습하는 멀티 모달 정보 병목(multimodal information bottleneck) 모델을 제안합니다.

- **Technical Details**: 제안된 MIB 모델은 원시 멀티모달 관찰(raw multimodal observations)의 예측 정보를 유지하면서 정책 학습에 관련된 정보만을 압축하고 필터링하여, 상호 보완 정보를 융합하는 압축된 공동 표현을 학습합니다. MIB 목표의 상한을 최소화하여 계산적으로 유용한 최적화를 수행합니다. 실험은 여러 고차원 보행(task)에서 수행되었습니다.

- **Performance Highlights**: 제안된 방법은 샘플 효율성이 개선되고 본 적 없는 화이트 노이즈에 대한 제로샷(zero-shot) 견고성이 우수하다는 것을 보입니다. 또한, egocentric 이미지와 비례감각(proprioceptive) 정보를 함께 활용할 때 정책 학습 성능이 단일 모달리티를 사용할 때보다 더욱 향상됩니다.



### Predicting 30-Day Hospital Readmission in Medicare Patients: Insights from an LSTM Deep Learning Mod (https://arxiv.org/abs/2410.17545)
Comments:
          5 pages, 1 table, 5 figures, Accepted by 2024 3rd International Conference on Cloud Computing, Big Data Application and Software Engineering(CBASE 2024), the final version will be published on on IEEE Conference proceeding

- **What's New**: 이번 연구에서는 LSTM 네트워크를 활용하여 Medicare 수혜자의 병원 재입원 문제를 분석하였습니다. 이는 미국 의료 시스템에 있어 의료 운영과 환자 케어 결과 측면에서 중대한 문제로 여겨집니다.

- **Technical Details**: 연구는 입원 수준 데이터(admission-level data), 입원 환자의 의학적 이력(inpatient medical history), 환자 인구 통계(patient demography)에서 선택된 변수를 사용하여 특징 공학(feature engineering)을 수행했습니다. LSTM 모델은 입원 수준 및 환자 수준 데이터의 시간적 동력을 포착하도록 설계되었습니다. MIMIC 데이터셋을 대상으로 한 사례 연구에서, LSTM 모델은 로지스틱 회귀(logistic regression) 기준선을 초과하여 정확하게 재입원을 예측하였습니다.

- **Performance Highlights**: 주요 특징으로는 Charlson Comorbidity Index, 병원에 머문 기간(hospital length of stay), 과거 6개월 동안의 병원 입원 횟수가 있었습니다. 인구 통계 변수는 덜 영향력이 있었습니다. 이 연구는 LSTM 네트워크가 Medicare 환자 재입원 예측을 개선하는 보다 유망한 접근 방식을 제공한다고 제안합니다.
...
### GDDA: Semantic OOD Detection on Graphs under Covariate Shift via Score-Based Diffusion Models (https://arxiv.org/abs/2410.17526)
Comments:
          4 pages, 6 figures

- **What's New**: 이 논문에서는 Graph Neural Networks (GNNs)를 사용한 out-of-distribution (OOD) 검출 문제를 다루고 있으며, 특히 semantic shift와 covariate shift가 동시에 나타나는 경우에 대한 새로운 도전 과제를 제시합니다. 기존의 연구는 주로 한 종류의 분포 변화에만 초점을 맞추었던 반면, 이 연구에서는 두 가지 변화를 모두 고려하여 graph-level semantic OOD 검출 문제를 제안합니다.

- **Technical Details**: 제안된 새로운 프레임워크인 Graph Disentangled Diffusion Augmentation (GDDA)는 두 단계로 구성됩니다. 첫 번째 단계는 그래프 표현을 domain-invariant semantic factors와 domain-specific style factors로 분리하는 것이고, 두 번째 단계에서는 training semantic과 style 공간 외부의 잠재적 요인을 생성하는 score-based generative diffusion model을 활용합니다. 또한, pseudo-InD 및 pseudo-OOD 그래프 표현을 생성하여 energy 기반의 semantic OOD 탐지기의 효과성을 향상시킵니다.

- **Performance Highlights**: 세 가지 벤치마크 데이터셋에서의 광범위한 실험 결과, 제안한 GDDA 접근법이 기존의 최첨단 방법들을 능가하는 성능을 보였으며, OOD 검출에서의 새로운 가능성을 제시합니다.
...
- **What's New**: 이 논문은 대형 언어 모델(LLM)에서 효과적인 unlearning 메커니즘이 필요한 상황에서 모델의 weight와 LLM unlearning 사이의 관계를 체계적으로 탐구합니다. 이를 통해 WAGLE(Weight Attribution-Guided LLM Unlearning) 방법을 설계하여 weights의 '영향(influence)'과 데이터를 잊거나 기억하는 사이의 상호작용을 밝혀냅니다.

- **Technical Details**: WAGLE 방법은 데이터를 잊고 유지하는 과정에서 LLM unlearning을 전략적으로 안내하는 방식으로, 다양한 unlearning 방법과 작업에서 원치 않는 내용을 지우는 동시에 원래 작업의 성능을 유지할 수 있도록 합니다. 이 방법은 기존의 LLM unlearning 알고리즘과 쉽게 호환됩니다. 또한, MMT와 같은 최적화 방법 및 최근의 변형된 방법을 기반으로 한 다양한 unlearning 벤치마크와 평가 메트릭에서 그 효과를 입증합니다.

- **Performance Highlights**: WAGLE은 gradient difference 및 negative preference optimization과 같은 여러 LLM unlearning 방법에서 unlearning 성능을 향상시키며, Zephyr-7b-beta 및 Llama2-7b 모델에서 실험을 수행하였습니다. 이 연구는 LLM unlearning을 향상시키는 데 있어 중요한 weights의 영향을 정량적으로 식별하는 원리를 제공하는 첫 번째 연구로 기록될 것입니다.
...
### DROP: Distributional and Regular Optimism and Pessimism for Reinforcement Learning (https://arxiv.org/abs/2410.17473)
Comments:
          16 pages, 11 figures

- **What's New**: 이 논문은 긍정적인 학습 (optimism)과 부정적인 학습 (pessimism)을 통합한 이론적으로 기반을 둔 새로운 모델 DROP (distributional and regular optimism and pessimism)을 제안하고 있습니다.

- **Technical Details**: DROP은 제어 (control) 개념을 바탕으로 하여 TD 오류 (temporal difference error)에 대한 비대칭적인 학습률을 사용하는 기존의 휴리스틱 모델을 개선합니다. 이 과정에서 각각의 신경세포 모델은 가치 함수 (value function)를 통합하여 정치 (policy)를 개선하는 방식으로 작동합니다.

- **Performance Highlights**: 기존의 휴리스틱 모델이 낮은 학습 성능을 보인 반면, DROP은 모든 동적 작업에서 우수한 성능을 나타내며 높은 일반성을 보여주었습니다.
...
- **What's New**: 이 논문은 다양한 조건에서 동작하는 대규모 이질적 학습 에이전트들의 진화 과정에 대한 새로운 통찰력을 제공합니다. 다중 에이전트 강화 학습(MARL) 환경에서, 저자들은 Replicator Dynamic과 유사한 멀티 에이전트 정책 경량화(Policy Gradient) 방법을 보여주고, Opponent-Learning Awareness를 활용하여 진화 시뮬레이션을 가능하게 합니다.

- **Technical Details**: 논문은 몰티-암드 밴딧 문헌에서 흔히 사용되는 가정하에, 나이브한 멀티 에이전트 정책 경량화가 Replicator Dynamic과 수학적으로 정렬될 수 있음을 보여줍니다. 이어서, 두 가지 방법(멀티 에이전트 정책 경량화 및 Opponent-Learning Awareness)의 빠른 병렬화 가능한 구현을 도출해 내고, 이를 통해 200,000명의 학습 에이전트를 대상으로 하는 인구 시뮬레이션을 수행합니다.

- **Performance Highlights**: 저자들은 전통적인 게임인 Hawk-Dove, Stag-Hunt, Rock-Paper-Scissors에서 200,000명의 에이전트의 진화 과정을 시뮬레이션하여 나이브한 학습 전략과 Opponent-Learning Awareness가 진화 역학에 미치는 다양한 영향을 입증합니다. 각 게임은 Opponent-Learning Awareness가 진화에 미치는 독특한 영향을 강조합니다.
...
### Guaranteeing Conservation Laws with Projection in Physics-Informed Neural Networks (https://arxiv.org/abs/2410.17445)
Comments:
          Accepted to NeurIPS 2024 Workshop on Data-driven and Differentiable Simulations, Surrogates, and Solvers

- **What's New**: 본 연구에서는 PINN(Proposed-informed Neural Networks)의 한계를 극복하기 위해 새로운 모델인 PINN-Proj를 제안했습니다. PINN-Proj는 보존법칙을 보장하기 위한 새로운 프로젝션 기법을 사용하여 물리적 시스템을 더 정확하게 모델링할 수 있게 합니다.

- **Technical Details**: PINN-Proj는 물리 방정식의 출력을 보존법칙이 위반되지 않는 함수 공간으로 투영하여 모델의 출력을 조정합니다. 이러한 투영은 학습 및 테스트 과정에서 모두 적용되어 PINN의 예측이 항상 보존법칙을 준수하도록 합니다.

- **Performance Highlights**: PINN-Proj는 모멘텀 보존 측면에서 PINN에 비해 약 3~4배의 예측 오차 감소를 보였고, 세 가지 물리 PDE 데이터셋에서 상태 예측 작업에서도 경미하게 더 나은 성능을 발휘했습니다.



### Detecting Adversarial Examples (https://arxiv.org/abs/2410.17442)
- **What's New**: 이 논문에서는 Deep Neural Networks (DNNs)에 대한 새로운 범용 (universal) 및 경량 (lightweight) 방어 방법인 Layer Regression (LR)을 제안합니다. LR은 DNN의 내부 레이어 출력의 변화를 분석하여 적대적 예시 (adversarial examples)를 탐지합니다.

- **Technical Details**: LR은 DNN의 초기 및 최종 레이어 간의 영향을 비교하여 적대적 샘플이 미치는 변화를 회귀 (regression) 분석합니다. 이를 통해 높은 탐지 성능을 달성하며, 이미지, 비디오, 오디오 등 다양한 데이터 도메인에 적용할 수 있습니다.

- **Performance Highlights**: LR은 672개의 다양한 공격-데이터셋-모델-방어 조합에 대한 실험에서 평균 97.6%의 탐지 성능을 보이며, 현재까지 가장 높은 성능을 기록하고 있습니다. 기존 방어 방법들 중에서는 82.9%가 가장 높은 성능입니다. 또한 LR은 기존 방어 방법보다 속도가 수 배 빨라 실시간 공격 감지에 적합합니다.
...
### Meta Stackelberg Game: Robust Federated Learning against Adaptive and Mixed Poisoning Attacks (https://arxiv.org/abs/2410.17431)
Comments:
          This work has been submitted to the IEEE for possible publication

- **What's New**: 본 논문에서는 적응형 공격 및 혼합 공격에 효과적으로 대응할 수 있는 새로운 방어 메커니즘인 메타 스택엘버그(meta-Stackelberg) 방어 프레임워크를 제안합니다. 이를 하여 Bayesian Stackelberg Markov 게임 모델을 기반으로 한 새로운 접근법을 보여줍니다.

- **Technical Details**: 제안된 메타 스택엘버그 방어는 두 가지 주요 단계로 구성됩니다: 사전 훈련(pre-training)과 온라인 적응(online adaptation)이다. 사전 훈련은 다양한 공격 유형을 시뮬레이션하여 방어 정책을 오프라인에서 구축하는 단계를 포함하며, 온라인 적응 단계에서는 실제 공격에 대한 즉각적인 피드백을 바탕으로 방어 정책을 조정합니다. 이 과정에서 메타-강화학습(meta-reinforcement learning) 기법을 사용하여 상호작용 데이터에서 배우게 됩니다.

- **Performance Highlights**: 실험 결과, 메타 스택엘버그 프레임워크는 강력한 모델 오염(model poisoning) 및 백도어(Backdoor) 공격에 대해 우수한 성능을 보여주었으며, 다수의 불확실하고 알려지지 않은 유형의 공격에 효과적으로 대응할 수 있는 능력을 입증하였습니다.



### Uncovering RL Integration in SSL Loss: Objective-Specific Implications for Data-Efficient RL (https://arxiv.org/abs/2410.17428)
Comments:
          Under Review AISTATS 2025,accepted to Neurips 2024 SSL Workshop

- **What's New**: 본 연구는 SPR(자기 예측 표현) 프레임워크 내에서 SSL(자기 감독 학습) 목표 수정의 효과를 조사하며, 특히 단말 상태 마스킹과 우선 재생 가중치와 같은 특정 조정을 중점적으로 다룹니다. 이러한 수정은 RL(강화 학습) 알고리즘에 따라 다르며, 전반적으로 성능에 미치는 영향을 평가하고, 이러한 조정을 수용하지 않는 Barlow Twins 및 VICReg와 같은 다른 SSL 목표를 탐색합니다.

- **Technical Details**: 이 연구에서는 Atari 100k 벤치마크에서 SPR의 6개 변형을 평가하며, 수정된 버전과 수정되지 않은 버전 모두를 포함합니다. 또한 이 목표의 성능을 DeepMind Control Suite에서 테스트하였으며, 해당 환경에서는 이러한 수정이 적용되지 않습니다. 특히, SPR에서 SSL 수정이 SPR 성능에 미치는 영향이 analysed 되며, 수정 제거 시 IQM에서 18% 감소하는 결과를 보여줍니다.

- **Performance Highlights**: 우리의 연구 결과에 따르면, SSL 목표 수정이 SPR 성능에 상당한 영향을 미치며, 우선 재생 가중치가 가장 영향력 있는 요소로 나타났습니다. Barlow Twins는 원래 SPR의 성과와 비교할 만한 결과를 보여주었으며, VICReg의 성과는 우선 재생 가중치와 유사했습니다. DeepMind Control Suite에서는 VICReg이 최고 성과를 내었으며, 이는 특정 도메인에 맞춘 알고리즘이 다른 문제 집합에서 일관되게 우수하지 않을 수 있다는 것을 강조합니다.
...
### Cooperative Multi-Agent Constrained Stochastic Linear Bandits (https://arxiv.org/abs/2410.17382)
- **What's New**: 이번 연구에서는 $N$개의 에이전트가 상호작용하여 자신의 집합적인 후회를 최소화하고 예상 비용을 특정 임계값 \( \tau \) 아래로 유지하는 협력적 다중 에이전트 스토캐스틱 선형 밴딧 문제를 탐구합니다. 각 에이전트는 고유한 보상 및 비용 매개변수를 가진 선형 밴딧 문제를 경험하며, 목표는 이러한 매개변수의 평균값에 해당하는 최적의 행동을 결정하는 것입니다.

- **Technical Details**: 본 연구에서 제안하는 분산 알고리즘 MA-OPLB는 가속 동의 방법을 활용하여 네트워크 전체에서 평균 보상 및 비용을 근사하며, 에이전트들은 이웃과 정보를 교환합니다. 연구 결과는 해당 알고리즘의 후회 경계가 \( \mathcal{O}\left(\frac{d}{\tau-c_0}\frac{\log(NT)^2}{\sqrt{N}}\sqrt{\frac{T}{\log(1/|\lambda_2|)}}\right) \)에 비례함을 보여줍니다.

- **Performance Highlights**: MA-OPLB 알고리즘은 다양한 네트워크 구조에서 실험적으로 성능을 검증하였으며, 통신 제약 조건 하에서도 탐색과 활용의 균형을 잘 맞추는 것으로 나타났습니다.
...
### Hierarchical Multi-agent Reinforcement Learning for Cyber Network Defens (https://arxiv.org/abs/2410.17351)
Comments:
          9 pages, 7 figures, AAMAS preprint

- **What's New**: 최근 mult-agent reinforcement learning (MARL)의 발전이 복잡한 실제 작업을 해결할 기회를 제공했습니다. 특히 사이버 보안 분야에서의 응용 가능성에 주목하고 있습니다. 본 연구는 고유한 사이버 방어 체계를 구축하기 위해 새로운 MARL 전략을 탐구합니다.

- **Technical Details**: 우리는 Proximal Policy Optimization (PPO) 기반의 계층적 구조를 제안하여 사이버 방어 작업을 네트워크 조사 및 호스트 복구와 같은 특정 하위 작업으로 분해합니다. 각 하위 작업에 대해 도메인 전문지식이 강화된 PPO를 사용하여 하위 정책을 훈련시키고, 이러한 하위 정책을 조정하는 마스터 방어 정책을 훈련시키게 됩니다.

- **Performance Highlights**: 우리는 CybORG Cage 4 환경을 활용하여 실험을 수행하였으며, 우리의 계층적 학습 접근 방식이 수렴 속도와 에피소드 수익, 그리고 네트워크 내의 청정 머신 비율, 정확도 및 복구 시 잘못된 긍정 비율과 같은 여러 사이버 보안 관련 메트릭에서 뛰어난 성능을 달성함을 보여주었습니다.



### Enhancing Robustness and Efficiency of Least Square Twin SVM via Granular Computing (https://arxiv.org/abs/2410.17338)
- **What's New**: 이번 논문에서는 최소제곱 쌍지원벡터 머신(Least Squares Twin Support Vector Machine, LSTSVM)의 단점을 보완하기 위해 강력한 그라눌러 볼 LSTSVM(Granular Ball Least Squares Twin Support Vector Machine, GBLSTSVM)을 제안하고, 이 모델의 대규모 버전인 LS-GBLSTSVM을 개발한다. GBLSTSVM은 원래 데이터 포인트 대신 그라눌러 볼을 사용하여 훈련되고, 이는 훈련 시간을 크게 단축시키며, 노이즈와 이상치에 대한 저항력이 강화된다.

- **Technical Details**: GBLSTSVM은 그라눌러 볼(granular ball)을 입력으로 사용하여 비평행 분리 초평면을 구성하는데, 이때 비선형 방정식계(system of linear equations)를 해결한다. 각 그라눌러 볼의 중심은 볼 안의 중요한 데이터 포인트 정보를 캡슐화한다. 또한, LS-GBLSTSVM은 최적화 문제의 주형(primal form)에서 정규화 항을 포함하여 SRM 원칙을 적용함으로써 매트릭스 역산(matrix inversion)의 필요를 없앴다.

- **Performance Highlights**: 실험을 통해 UCI, KEEL, NDC 벤치마크 데이터셋에서 GBLSTSVM과 LS-GBLSTSVM이 기존 모델보다 일관되게 뛰어난 성능을 보여주며, 노이즈 및 이상치가 존재하는 데이터셋에서도 효과적으로 관리할 수 있다는 것이 입증되었다.



### Computing Optimal Regularizers for Online Linear Optimization (https://arxiv.org/abs/2410.17336)
- **What's New**: 이번 논문에서는 Online Linear Optimization (OLO) 문제에서 Follow-the-Regularized-Leader (FTRL) 알고리즘을 사용하여, 주어진 정규화 함수와 관련하여 하한 경계를 강화한 알고리즘을 제안합니다. 이 알고리즘은 주어진 convex(볼록) 및 symmetric(대칭) 액션 세트와 손실 세트에 대한 정규화기를 출력하며, 이를 통해 최적의 후회 경계를 구현할 수 있다는 것을 증명합니다.

- **Technical Details**: 주요 기술적 내용으로, FTRL 알고리즘이 입력으로 convex 및 symmetric 액션 세트와 손실 세트를 받아, 이를 바탕으로 정규화를 수행하여 최적의 성능을 보장합니다. 이 알고리즘은 OLO 인스턴스의 차원 d에 대한 지수적인 전처리 시간과 공간을 필요로 하며, 이후 온라인으로 효율적으로 실행될 수 있습니다.

- **Performance Highlights**: 제안된 알고리즘은 임의의 convex, symmetric 액션 세트와 손실 세트에 대해, 최적의 학습 알고리즘과 상수 요인 이내의 후회를 도출할 수 있으며, 이는 Srebro et al.(2011)의 결과를 강화하는 성과로 평가됩니다. 이 연구는 OLO에서의 후회 경계 최적화를 보다 잘 이해하는 데 기여할 것입니다.



### Improving Insurance Catastrophic Data with Resampling and GAN Methods (https://arxiv.org/abs/2410.17294)
- **What's New**: 이 논문은 보험업계에서 중요한 재해(재난) 이벤트 데이터를 향상시키기 위한 세 가지 방법, 즉 bootstrap, bootknife, GAN(generative adversarial networks) 알고리즘을 제안하고 비교합니다.

- **Technical Details**: bootstrap과 bootknife는 고전적인 리샘플링 방법이며, GAN은 최신의 생성 모델 개발 기법으로 다양한 분야에서 사용됩니다. 이 방법들은 실제 데이터와 수치적 실험을 통해 평균 제곱 오차(Mean Squared Error, MSE)와 평균 절대 오차(Mean Absolute Error, MAE) 기준으로 비교됩니다.

- **Performance Highlights**: 제안된 방법들은 재해 데이터를 기반으로 각 모델의 적합성을 평가하며, 전문가의 의견을 바탕으로 불확실성을 고려한 추정을 제안합니다. 이로 인해 보험 계약 체결 및 가격 책정에 있어 보다 나은 결정을 지원할 수 있습니다.



### An Effective Theory of Bias Amplification (https://arxiv.org/abs/2410.17263)
- **What's New**: 이 논문에서는 머신 러닝 모델이 데이터에 존재하는 편향을 포착하고 확대할 수 있다는 문제를 다룹니다. 특히, ridge regression을 통해 모델 설계 선택과 데이터 분포 속성이 어떻게 편향에 기여하는지를 이론적으로 깊이 이해하고 평가할 수 있는 새로운 이론을 제시합니다.

- **Technical Details**: 이 연구는 ridge regression을 기반으로 한 정밀한 분석 이론을 제공합니다. 무작위 투사(random projections)가 있는 경우와 없는 경우를 모두 포함하여 신경망(neural networks)을 단순화된 환경에서 모델링합니다. 이 이론은 머신 러닝 편향(bias)에 대한 통합적이고 엄격한 설명을 제공하며, 편향 확대(bias amplification)와 소수 집단(bias in minority groups) 편향 현상을 다양한 특성과 매개변수(regime)에서 분석합니다.

- **Performance Highlights**: 본 이론은 최적의 정규화 패널티(regularization penalty) 또는 훈련 시간(training time)을 통해 편향 확대를 피할 수 있는 가능성을 보여줍니다. 또한, 매개변수화(parameterization)가 증가하더라도 집단 간의 테스트 오류(test error) 차이가 사라지지 않는 근본적인 차이가 존재할 수 있다는 점을 강조합니다. 또한, 우리의 이론적 예측은 문헌에서 보고된 여러 실증적 관찰(empirical observations)과 일치합니다.



### Inference with K-means (https://arxiv.org/abs/2410.17256)
Comments:
          15 pages

- **What's New**: 이 논문의 핵심 목표는 k-means 알고리즘을 활용한 추론 방안을 새롭게 제안하는 것입니다. 논문에서는 온라인 균형 k-means 접근 방식을 통해 클러스터링된 데이터에서 마지막 구성 요소의 예측을 다루며, 이를 통해 k군집의 정확성과 신뢰성을 높이는 방법을 탐구합니다.

- **Technical Details**: k-means 알고리즘은 반복적인 클러스터링 방법으로, 초기 k개의 중심을 임의로 할당하고, 각 데이터 포인트를 가장 가까운 중심에 할당한 후, 할당된 점의 평균을 기반으로 중심을 업데이트합니다. 이 과정은 수렴할 때까지 지속됩니다. 온라인 균형 k-means는 신속하게 데이터가 스트리밍되는 환경에서 클러스터링을 수행하며, 균형을 고려하여 클러스터의 크기를 조정합니다.

- **Performance Highlights**: 연구 결과에 따르면, 클러스터의 수가 증가할수록 오류가 감소하는 경향이 있으며, 더 많은 데이터 포인트를 할당하는 것이 추론 오류를 크게 개선하지는 않는 것으로 나타났습니다. 또한, 학습 과정에서 손실을 줄이는 것이 전체 추론 오류에 큰 영향을 미치지 않는다는 사실도 밝혀졌습니다. 이러한 내용을 바탕으로 다중 클러스터 데이터를 통해 더 나은 예측을 할 수 있는 전문적인 추론 기술이 필요함을 강조하고 있습니다.
...
### metasnf: Meta Clustering with Similarity Network Fusion in R (https://arxiv.org/abs/2410.17976)
Comments:
          72 pages, 22 figures, submitted to Journal of Statistical Software

- **What's New**: metasnf는 사용자가 클러스터 솔루션의 폭넓은 공간에서 효율적으로 검색할 수 있도록 하는 메타 클러스터링 방법을 적용할 수 있게 해주는 R 패키지입니다.

- **Technical Details**: 이 패키지는 유사성 네트워크 융합(SNF) 기반의 클러스터링 워크플로우에 적용됩니다. SNF는 생물 의학적 하위 유형 발견에 일반적으로 사용되는 다중 모달 데이터 통합 알고리즘입니다. 패키지에는 클러스터 시각화, 특성화 및 검증을 지원하는 기능도 포함되어 있습니다.

- **Performance Highlights**: 이 패키지는 연구자들이 질의의 품질의 맥락 무관적 측정이 아닌 맥락 특정 유틸리티에 의해 안내되는 SNF 유도 클러스터 솔루션을 식별하는 데 도움을 줄 수 있습니다.



### Optical Generative Models (https://arxiv.org/abs/2410.17970)
Comments:
          24 Pages, 9 Figures

- **What's New**: 이 논문은 확산 모델(diffusion models)에 영감을 받은 광학 생성 모델(optical generative models)을 소개합니다. 이 모델은 낮은 복잡도의 디지털 인코더를 사용하여 무작위 노이즈를 빛 패턴으로 변환하여 원하는 데이터 분포에 대한 생성 시드를 만듭니다.

- **Technical Details**: 본 모델은 조합 훈련된 자유 공간 기반의 재구성 디코더(free-space-based reconfigurable decoder)를 통해 이 생성 시드를 광학적으로 처리하여 새로운 이미지를 생성합니다. 이 과정에서 새로운 이미지를 합성할 때 컴퓨팅 파워를 소모하지 않으며, 조명 전력(illumination power)과 얕은 인코더를 통한 무작위 시드 생성(random seed generation) 외에는 에너지를 절약할 수 있습니다.

- **Performance Highlights**: 우리는 MNIST, Fashion MNIST, Butterflies-100, Celeb-A 데이터셋에 대한 데이터 분포를 따르면서 손글씨 숫자, 패션 제품, 나비, 인간 얼굴의 단색 및 다채로운 새로운 이미지를 광학적으로 생성하는 데 성공했습니다. 결과적으로 디지털 신경망 기반 생성 모델과 비교해도 전반적으로 유사한 성능을 보였습니다.
...
### Theoretically Grounded Pruning of Large Ground Sets for Constrained, Discrete Optimization (https://arxiv.org/abs/2410.17945)
- **What's New**: 이 연구에서는 거대한 조합 최적화 문제의 구성 요소 중 불필요하거나 중복된 요소를 신속하게 제거하기 위해 경량화된 pruning 알고리즘인 QuickPrune을 개발했습니다. 이는 다양한 예제에서 90% 이상의 원소를 효율적으로 제거할 수 있음을 보여줍니다.

- **Technical Details**: QuickPrune 알고리즘은 주어진 예제에 대한 이론적 보장을 제공하면서 동시에 큰 예제 집합을 단일 패스로 처리합니다. 이 알고리즘은 예산 범위 내에서 최적 솔루션의 일부가 아닌 원소들을 제거하는 것을 목표로 하며, 그 결과로 생성된 pruned ground set의 크기와 최적 가치 보 retained에 대한 보장을 제공합니다.

- **Performance Highlights**: 실제 데이터셋을 바탕으로 한 광범위한 실험을 통해 QuickPrune 알고리즘은 기존의 클래식 및 머신러닝 기반 pruning 휴리스틱보다 더 나은 성능을 보였습니다. 또한, 이 알고리즘은 최적 솔루션의 가치의 비율을 유지하면서, 많은 양의 불필요한 데이터를 제거하는 데 성공했습니다.
...
### Semi-Implicit Functional Gradient Flow (https://arxiv.org/abs/2410.17935)
Comments:
          31 pages, 12 figures

- **What's New**: 이 논문에서는 파티클 기반 변별 추론 방법인 Semi-Implicit Functional Gradient Flow (SIFG)를 소개합니다. SIFG는 변별 분포를 근사하기 위해 편향된 파티클을 사용하는 기능적 기울기 방법으로, 기존 방법의 제한된 탐색 능력을 개선합니다.

- **Technical Details**: SIFG는 과도한 계산을 요구하지 않으며, 거짓 대응(denoising score matching)을 통해 기능적 기울기 흐름(Wasserstein gradient flow)을 추정합니다. 또한, 알고리즘의 탐색 능력을 높이기 위해 Gaussian 노이즈를 주입하여 다양한 샘플을 생성할 수 있습니다.

- **Performance Highlights**: 실험 결과, SIFG는 시뮬레이션 및 실제 데이터 문제에서 기존 방법들에 비해 높은 효과성과 효율성을 보여주었습니다.



### Retrieving snow depth distribution by downscaling ERA5 Reanalysis with ICESat-2 laser altimetry (https://arxiv.org/abs/2410.17934)
- **What's New**: 본 연구는 ICESat-2 위성 레이저 알티미터로부터 수집된 낮은 밀도의 눈 깊이 데이터를 사용하여 월별 눈 깊이 그리드 맵을 생성하는 새로운 방법론을 제시합니다.

- **Technical Details**: ICESat-2에서의 눈 표면 고도 측정 결합, 기후 재분석 데이터(ERA5 Land) 사용, 회귀 모델 적용을 통해 미세 스케일(10 m)에서의 눈 깊이 추정 및 축소 기법을 개발하였습니다. 눈 깊이를 수치화하기 위해 서브그리드 변동성(subgrid variability)을 활용합니다.

- **Performance Highlights**: 눈 깊이 예측의 R2값은 0.74에서 0.88로 나타났으며, Nor웨이 하르당에르비다 지역의 중간 규모(100 m x 500 m)에서의 검증 결과입니다. 이 방법은 지역특정 보정이 필요하지만 다른 고산 지대에서도 적용 가능하며, 수업가를 통해 기존의 눈 조사 결과를 시간적으로 및 넓은 지역으로 보급하는 데 유용합니다.
...
### Non-intrusive Speech Quality Assessment with Diffusion Models Trained on Clean Speech (https://arxiv.org/abs/2410.17834)
- **What's New**: 본 연구는 클린 음성을 사용하여 훈련된 조건 없는 확산 모델을 통해 비지도 방식으로 음성 품질을 평가하는 새로운 접근 방식을 제안합니다. 기존의 오디오 퀄리티 측정 중 비참여 방식 평가의 문제를 해결하고자 합니다.

- **Technical Details**: 우리의 방식은 클린 음성 데이터로 훈련된 확산 모델을 기반으로 하며, 이 모델이 학습한 클린 데이터 분포를 바탕으로 입력 음성이 얼마나 잘 일치하는지를 파악합니다. ODE(Ordinary Differential Equation)를 이용하여 확률 밀도를 계산하며, 최종 가우시안 분포에서 낮은 품질의 샘플은 낮은 가능도로 매핑됩니다.

- **Performance Highlights**: 제안한 로그-가능도(log-likelihood) 기반의 음성 품질 추정 방법은 POLQA, SI-SDR 및 SNR과 같은 전통적인 측정 지표와 높은 상관성을 보이며, 기존 연구보다 더 우수한 성능을 발휘했습니다.
...
### Can Uncertainty Quantification Enable Better Learning-based Index Tuning? (https://arxiv.org/abs/2410.17748)
Comments:
          14 pages, 11 figures

- **What's New**: 본 논문에서는 학습 기반 모델의 결과에서 불확실성을 정량화하는 새로운 접근 방식을 도입하여 전통적인 방법과 학습 기반 방법의 장점을 결합하여 신뢰할 수 있는 인덱스 튜닝을 가능하게 하는 Beauty라는 불확실성 인식 프레임워크를 제안합니다.

- **Technical Details**: 제안된 방법은 AutoEncoder와 Monte Carlo Dropout을 통합한 하이브리드 불확실성 정량화 방법을 통해 이루어집니다. 이 방법은 기존의 Benefit Estimation (BE) 작업들에 적합하게 조정되어, 신뢰성 높은 인덱스 추천을 가능하게 합니다.

- **Performance Highlights**: 우리는 16개의 서로 다른 모델에 대해 실험을 수행한 결과, 기존의 불확실성 정량화 방법에 비해 우수한 성능을 보였으며, 6개의 데이터 세트에서 인덱스 튜닝 테스트를 수행한 결과 최악의 상황을 제거하고 최선의 상황을 세 배 이상 증가시켰습니다.
...
### Exploring structure diversity in atomic resolution microscopy with graph neural networks (https://arxiv.org/abs/2410.17631)
- **What's New**: 딥러닝(deep learning)의 발전은 원자 해상도 미세 그래프(micrographs)의 고처리량(high-throughput) 분석에 큰 기회를 제공합니다. 새로운 연구에서는 고정 크기의 이미지 패치에서 훈련된 모델의 비효율성과 유연성을 극복하고자, 원자 구조와 그래프의 유사성에서 영감을 얻어 에퀴바리언트 그래프 신경망(equivariant graph neural network, EGNN)을 기반으로 한 몇 샷 학습(few-shot learning) 프레임워크를 제안합니다.

- **Technical Details**: 이 연구에서 제안하는 EGNN은 다양한 원자 구성을 포함한 미세 그래프를 분석하는 데 있어 강력한 안정성과 함께 이미지 기반 모델보다 세 배의 계산 파라미터(computing parameters)를 줄이는 데 성공했습니다. 특히, 다양한 격자 변형을 가진 결함 집합의 원자 배치(line of vacancy) 분석에 있어 더욱 두드러진 성과를 보입니다.

- **Performance Highlights**: EGNN 프레임워크는 이미지에서 원자 규모의 구조적 특징을 정량적이고 간단하게 추출하는 데 용이하며, 전자빔 조사 아래에서 결함 선의 자기 조립 역학을 통계적으로 드러내는 데 기여합니다. 또한, 다양한 구성의 이미지를 처리하기 위해 단일 구조 인식을 위한 EGNN 서브 모델을 통합하여 작업 체인(task chain) 형태로 처리하게 되어, 수소 발생 반응을 위한 우수한 전기 촉매 특성을 가진 신규 도핑 구성(configurations)을 발견하는 결과를 가져왔습니다.
...
### Predicting Company Growth by Econophysics informed Machine Learning (https://arxiv.org/abs/2410.17587)
Comments:
          18 pages, 12 figures

- **What's New**: 최근에 발표된 이 논문에서는 머신 러닝 기반의 회사 성장 예측 프레임워크를 제안합니다. 기존의 모델들은 이론에 과도하게 의존하거나 시간적 예측 기법에만 의존하여 해석 가능성을 간과하는 경우가 많았습니다.

- **Technical Details**: 이 모델은 econophysics 모델을 통합하여 회사 성장의 내재적 메커니즘과 무작위적 요인 및 개별 결정에 의한 변동성을 캡처합니다. 시간적 예측 기법을 활용하여 기본 성장 및 변동 요인에 대한 명시적인 모델링을 통해 해석 가능성을 높였습니다.

- **Performance Highlights**: 본 모델은 기존의 시간적 예측 기법만을 사용하는 방법들에 비해 뛰어난 예측 성능을 보여주며, 특히 장기 예측 작업에서 그 장점이 더욱 두드러집니다.
...
- **What's New**: LEADS (Lightweight Embedded Assisted Driving System)는 전기차의 발전과 대학 및 고등학교의 포뮬러 레이스 증가에 맞춰 설계된 전문 솔루션입니다. 이는 설계 및 제작의 기준이 낮아진 현상에서 팀들이 복잡한 상업용 차량의 아키텍처를 사용하는 경향에 대응하기 위해 개발되었습니다.

- **Technical Details**: LEADS는 계측(instrumentation), 제어(control), 분석(analysis) 시스템의 효율성을 개선하고 개발을 단순화하는데 중점을 둡니다. 이 시스템은 아마추어 애플리케이션에 적합하도록 경량화된 구조를 가지고 있으며, 복잡한 기존 솔루션을 대체할 수 있습니다.

- **Performance Highlights**: LEADS는 작고 단순한 시스템으로, 대학 및 고등학교 팀들이 쉽게 채택할 수 있도록 해주며, 이는 전반적인 레이스 성능 향상과 직접적인 연결이 됩니다.



### Univariate Conditional Variational Autoencoder for Morphogenic Patterns Design in Frontal Polymerization-Based Manufacturing (https://arxiv.org/abs/2410.17518)
- **What's New**: 본 논문에서는 프론탈 폴리머화(frontal polymerization, FP)를 기반으로 한 복합적인 패턴의 역설계(inverse design)를 위한 새로운 확률적 생성 모델인 단변량 조건부 변량 오토인코더(univariate conditional variational autoencoder, UcVAE)를 제안합니다. 이 UcVAE는 설계 공간만 인코딩하여 학습 파라미터 수를 대폭 줄이고 훈련 시간을 단축하면서도 유사한 성능을 유지합니다.

- **Technical Details**: UcVAE는 설계 공간(process conditions)과 설계 목표(pattern image) 간의 관계에서 고차원 설계 목표를 저차원 설계 공간으로 매핑하는 방식을 사용합니다. 프론탈 폴리머화 과정에서 열화학 반응 모델과 FEM(유한요소법)에 기반하여 패턴 이미지를 생성하는 과정을 설명하며, 기존의 cVAE와 비교하여 학습 효율성을 높입니다. UcVAE는 이론적으로 프론탈 폴리머화의 열-화학적 반응 확산 방정식을 통해 생성된 패턴의 복잡성을 다룰 수 있도록 설계되었습니다.

- **Performance Highlights**: 훈련된 UcVAE 모델은 시뮬레이션에서 생성된 패턴 이미지를 기반으로 높은 정밀도의 복합 패턴을 생성하는 다양한 공정 조건 솔루션을 제공합니다. UcVAE는 디자인 공간이 저차원일 때도, 패턴 이미지가 고차원인 경우에서 효과적으로 작동하여, 향후 프론탈 폴리머화 기반의 설계에서 유용한 도구로 자리 잡을 것입니다.
...
### Multi-modal Machine Learning Analysis of X-ray Absorption Near-Edge Spectra and Pair Distribution Functions: Performance and Interpretability towards Experimental Design (https://arxiv.org/abs/2410.17467)
- **What's New**: 본 논문에서는 다양한 스펙트럼 정보(X-ray absorption near-edge spectra (XANES)와 atomic pair distribution functions (PDFs))를 결합하여 전이금속 산화물의 국소 구조와 화학에 대한 정보를 추출하는 해석 가능한 머신러닝(ML) 기법을 소개합니다.

- **Technical Details**: XANES와 PDFs의 정보를 통합하여 전이금속 양이온의 산화 상태(charge state), 배위 수(coordination number), 평균 가장 가까운 이웃 결합 길이(mean nearest-neighbor bond length)를 추출하기 위해 랜덤 포레스트(random forest) 모델을 훈련했습니다. 이 과정에서 각각 스펙트럼에 대한 상대적인 기여를 분석했습니다.

- **Performance Highlights**: XANES-only 모델이 모든 작업에서 PDF-only 모델보다 뛰어난 성능을 보였으며, XANES와 PDF가 결합된 입력을 사용할 경우에도 XANES의 정보가 주도를 차지하는 경우가 많았습니다. 그러나 종 별 차별 PDF(dPDFs)를 입력으로 사용할 경우 성능 차이가 작아졌습니다. 이 결과는 XANES가 구조적 정보를 풍부하게 포함하고 있음을 강조하며, 향후 구조 탐사(probe)로서의 발전 가능성을 시사합니다.



### Bauplan: zero-copy, scale-up FaaS for data pipelines (https://arxiv.org/abs/2410.17465)
Comments:
          Accepted for the 10th International Workshop on Serverless Computing (pre-print)

- **What's New**: 본 논문에서는 데이터 응용 프로그램을 위해 설계된 새로운 서버리스(FaaS) 프로그래밍 모델과 런타임인 bauplan을 소개합니다. 이 모델은 데이터 파이프라인의 복잡성을 다루는 데 최적화되어 있습니다.

- **Technical Details**: bauplan은 Declarative API를 제공하여 사용자들이 Functional Directed Acyclic Graphs (DAGs)를 선언적으로 정의하고 이를 클라우드 기반 작업자에서 효율적으로 실행할 수 있게 합니다. 또한, 모든 중간 데이터는 Arrow 테이블 형식으로 저장되며, 이는 데이터 이동 시 직렬화 비용을 최소화합니다.

- **Performance Highlights**: bauplan은 일반적인 FaaS 플랫폼보다 데이터 파이프라인 워크로드에 대해 뛰어난 성능과 개발자 경험을 제공합니다. 예를 들어, bauplan의 빌드 프로세스는 AWS Lambda보다 15배, Snowpark보다 7배 빠릅니다.



### Scalable Implicit Graphon Learning (https://arxiv.org/abs/2410.17464)
- **What's New**: 본 논문에서는 Scalable Implicit Graphon Learning (SIGL)이라는 새로운 방법을 제안합니다. SIGL은 새로운 그래프 모델을 학습하기 위해 implicit neural representations (INRs)와 graph neural networks (GNNs)를 결합하여 다양한 크기의 그래프에서 그래프론을 추정하는 스케일러블한 방법입니다.

- **Technical Details**: SIGL은 관측된 그래프를 기반으로 그래프론을 추정하며, 고정된 해상도의 매트릭스에 의존하지 않고 임의의 해상도로 연속적인 그래프론을 학습할 수 있습니다. GNN을 통해 노드 정렬을 개선하고, 반면 MSE를 최적화하여 실제 노드 순서를 추정합니다. 이를 통해 대규모 그래프에서도 계산 효율적이며 스케일링이 용이한 방식으로 문제를 해결합니다.

- **Performance Highlights**: SIGL은 합성 및 실제 그래프에서 기존 방법들과 비교하여 월등한 성능을 보여줍니다. 특히 대규모 그래프에서의 스케일링 문제를 효과적으로 해결하며, 그래프 데이터 증강(graph data augmentation) 작업에서 성능을 향상시키는 데 기여합니다. SIGL의 실험 결과는 그래프의 구조적 속성을 효과적으로 포착하고, 실무 적용에서 작업 성능을 높일 수 있음을 보여줍니다.



### Graph Neural Network-Accelerated Network-Reconfigured Optimal Power Flow (https://arxiv.org/abs/2410.17460)
- **What's New**: 이 논문은 전력망 최적화 문제인 Optimal Power Flow (OPF)를 해결하기 위해 기계 학습(Machine Learning) 기반의 그래프 신경망(Graph Neural Network, GNN) 접근 방식을 제안합니다.

- **Technical Details**: 이 접근 방식은 복잡한 Mixed-Integer Linear Programming (MIP) 선형 변환 과정을 포함하여 네트워크 재구성이 이루어지는 NR-OPF 문제를 빠르게 해결하기 위해 GNN 모델을 오프라인에서 학습시킵니다. 또한, GNN의 모델 크기 및 훈련 시간을 줄이고 정확도를 높이기 위한 오프라인 Pre-ML 필터 레이어와 GNN 예측 값을 분석하여 높은 신뢰도의 NR 솔루션 서브셋을 선택하는 빠른 온라인 Post-ML 선택 레이어를 제안합니다.

- **Performance Highlights**: 사례 연구에서는 제안된 GNN 가속화 NR-OPF 방법이 제안된 Pre-ML 및 Post-ML 레이어와 함께 탁월한 성능을 보여주었다.



### Real-time experiment-theory closed-loop interaction for autonomous materials scienc (https://arxiv.org/abs/2410.17430)
- **What's New**: 이 논문에서는 Autonomous Materials Search Engine (AMASE)를 소개합니다. AMASE는 로봇 과학을 이용하여 실험과 이론 예측의 자율적이고 지속적인 순환 상호작용을 수행하여 재료 탐사에 나섭니다.

- **Technical Details**: AMASE는 온도-조성 (temperature-composition) 상도 중 하나인 eutectic phase diagram을 신속하게 매핑하는 데 적용되었습니다. 이 시스템은 열처리 (thermal processing) 및 얇은 필름의 조성 상계선 (compositional phase boundaries) 실험적 결정과 Gibbs 자유 에너지 (Gibbs free energies)의 최소화를 통해 실시간으로 상도 예측을 업데이트 합니다.

- **Performance Highlights**: AMASE는 Sn-Bi 이진 얇은 필름 시스템의 eutectic phase diagram을 자율적으로 정확하게 결정했습니다. 이는 전체 조성-온도 상 공간의 단지 일부를 아우르는 자가 유도 캠페인을 통해 이루어졌고, 실험 횟수를 6배 감소시키는 성과를 달성했습니다. 이 연구는 인간의 개입 없이 실시간, 자율적이며 반복적인 실험과 이론의 상호작용이 가능함을 최초로 보여주었습니다.
...
### Learning Graph Filters for Structure-Function Coupling based Hub Node Identification (https://arxiv.org/abs/2410.17410)
Comments:
          13 pages, 4 figures

- **What's New**: 이 논문은 그래프 신호 처리(Graph Signal Processing, GSP) 기반의 새로운 허브 노드 탐지 프레임워크 GraFHub를 제안합니다. 기존의 방법들은 기능적 연결성 네트워크만 활용했으나, 제안된 방법은 구조적 연결성과 기능적 활성화를 모두 고려합니다.

- **Technical Details**: GraFHub는 허브 노드가 희소(sparse)하고 이웃에 비해 높은 활성화 수준을 가지며, 비허브 노드의 활성화는 그래프 기반 필터의 출력으로 모델링될 수 있다는 가정을 기반으로 합니다. 계층화된 그래프 필터를 학습하여 허브 노드를 식별하며, 참가 계수(participation coefficient)를 사용하여 연결 허브를 추가적으로 식별합니다.

- **Performance Highlights**: 제안된 프레임워크는 Human Connectome Project(HCP)의 안정 상태 fMRI 데이터와 시뮬레이션된 데이터에서 평가되었습니다. GraFHub는 기존의 최첨단 허브 노드 탐지 방법들과 비교하여 유의미한 성과를 위에 제시된 두 가지 점에서 달성하였습니다.
...
### Characterizing Robocalls with Multiple Vantage Points (https://arxiv.org/abs/2410.17361)
Comments:
          Accepted for publication at the 46th IEEE Symposium on Security and Privacy, 2025

- **What's New**: 이번 연구는 여러 독립적으로 운영되는 데이터 출처를 사용하여 전화 스팸과 관련된 문제를 분석하였습니다. 5년 이상의 데이터를 가진 음성 허니팟, 공적 집행 데이터 및 소비자 불만 조사를 포함한 다양한 출처의 데이터를 융합하여 전화 스팸의 현황과 효과를 평가합니다.

- **Technical Details**: 연구진은 약 300만 건의 음성 통화에서 오디오 및 전사(transcripts) 데이터를 비교하여 비선형적인 방법론적 과제를 해결합니다. 이를 통해 다양한 관점을 비교하고, 캠페인 식별을 위한 오디오 클러스터링 기법을 개발하였습니다. STIR/SHAKEN 인증 프로토콜의 채택에 대한 첫 분석을 포함하며, 상호작용이 있는 불법 전화가 증가할 수 있는 가능성에 대해 논의합니다.

- **Performance Highlights**: 연구 결과, 원치 않는 전화는 서서히 감소하고 있지만 여전히 불만 및 통화량은 높게 유지되고 있습니다. STIR/SHAKEN 인증에 적응한 로보콜러도 존재하며, 상호작용적인 음성 허니팟을 구현할 경우 수집된 통화 콘텐츠가 60% 증가하는 등 개선된 성과가 관찰되었습니다.
...
### Error estimates between SGD with momentum and underdamped Langevin diffusion (https://arxiv.org/abs/2410.17297)
- **What's New**: 이번 논문에서는 확률적 경사 하강법(Stochastic Gradient Descent, SGD)에서 모멘텀(momentum) 기법의 새로운 통계적 관계를 제시하고 있습니다. 특히, 모멘텀을 갖춘 확률적 경사 하강법이 언더댐프드 랜주빈 확산(underdamped Langevin diffusion)과 밀접한 관련이 있다는 점을 밝히고 있습니다.

- **Technical Details**: 이 논문은 1-워서스타인 거리(1-Wasserstein distance)와 전체 변동 거리(total variation distance)를 활용하여 모멘텀을 포함한 확률적 경사 하강법과 언더댐프드 랜주빈 확산 간의 오류 추정(quantitative error estimate)을 수립합니다.

- **Performance Highlights**: 이 연구는 두 방법 간의 정량적 관계를 규명하여, 확률적 경사 하강법의 효율성과 효과성을 수치적으로 평가할 수 있는 기초 자료를 제공합니다.



### A Fusion-Driven Approach of Attention-Based CNN-BiLSTM for Protein Family Classification -- ProFamN (https://arxiv.org/abs/2410.17293)
Comments:
          It is the authors original Work

- **What's New**: 본 연구에서는 단백질 서열 분류를 위한 차별화된 모델인 ProFamNet을 소개합니다. 이 모델은 1D-CNN, BiLSTM, attention 메커니즘의 융합을 통해 공간적 특징 추출과 장기 의존성, 문맥 인식을 결합합니다.

- **Technical Details**: ProFamNet은 1D-CNN(1차원 합성곱 신경망), BiLSTM(양방향 장기 단기 기억 모델), attention 메커니즘을 결합하여 단백질 가족 분류를 수행합니다. 이 모델은 450,953개의 파라미터와 1.72MB의 컴팩트한 크기를 가지며, 기존 최고 성능 모델을 능가하는 성능을 자랑합니다.

- **Performance Highlights**: ProFamNet은 98.30%의 F1 점수를 달성하였고, 이는 97.67%의 F1 점수를 기록한 기존 모델보다 우수합니다. 또한 훈련 에폭수는 25로, 30이었던 기존 모델보다 적은 시간을 소요해 훈련되었습니다.



### Disease Outbreak Detection and Forecasting: A Review of Methods and Data Sources (https://arxiv.org/abs/2410.17290)
Comments:
          40 pages, 2 figures

- **What's New**: 이 연구는 2015년부터 2022년까지 발표된 연구들을 중심으로 현재의 전염병 감시 시스템, 특히 소셜 미디어 데이터를 활용한 방법론을 리뷰합니다. 전통적인 데이터 소스와 인터넷 기반 데이터 소스를 통합하여 전염병 감지 및 예측의 효율성을 높이기 위해 다양한 기법들이 발전해 왔음을 강조합니다.

- **Technical Details**: 연구에서는 기술적인 기법으로 탄력적 통계학적 접근과 머신러닝 (machine learning)을 포함한 시계열 (time series) 분석 방법을 사용하여 전염병의 발생과 전파를 감지하고 예측하는 방법들을 다룹니다. 데이터 출처로는 WHO, 보건부 기록, 클리닉 자료와 더불어 소셜 미디어 (social media)를 포함한 온라인 데이터가 사용됩니다.

- **Performance Highlights**: 소셜 미디어 데이터를 이용한 전염병 탐지 및 예측 방법은 전통적인 데이터 소스의 한계를 극복합니다. 예를 들어, Google Flu Trends와 같은 프로젝트는 인플루엔자 발생 초기 단계의 신속한 탐지를 가능하게 하여 공중 보건 대응을 촉진합니다. 또한, Twitter 데이터를 사용하는 질병 감시 시스템은 실시간으로 신흥 전염병을 모니터링 할 수 있어 자원의 효율적인 배분을 지원합니다.
...
### Behavior Matters: An Alternative Perspective on Promoting Responsible Data Scienc (https://arxiv.org/abs/2410.17273)
Comments:
          23 pages, 4 figures, to be published in CSCW 2025

- **What's New**: 이 논문에서는 데이터 과학과 행동 변화 이론을 접목하여 책임 있는 데이터 과학을 촉진하는 새로운 관점을 제시합니다. 기존의 기술적 해결책과 함께 데이터 과학 실무자의 행동 책임을 강조하고 있습니다.

- **Technical Details**: 논문은 데이터 과학 파이프라인에서의 사회적 불평등을 완화하기 위한 행동 변화 개입(interventions)의 필요성을 언급하며, 인지 심리학(cognitive psychology)의 이론을 데이터 과학(workflow) 지식 및 윤리 가이드라인(ethics guidelines)과 통합하는 방법을 제안합니다.

- **Performance Highlights**: 데이터 과학 프로젝트가 사회에 미치는 영향을 반영하여 책임 있는 데이터 과학 관행을 통해 사회적 선을 촉진해야 한다는 점을 강조하였으며, 행동 변화 이론을 활용한 구체적인 사례를 통해 긍정적 사회 결과를 주도하기 위한 데이터 과학자의 행동 변화를 장려하는 방안을 제시하였습니다.
...
### MOFFlow: Flow Matching for Structure Prediction of Metal-Organic Frameworks (https://arxiv.org/abs/2410.17270)
Comments:
          10 pages, 6 figures

- **What's New**: 이번 연구에서는 MOF 구조 예측을 위해 설계된 최초의 딥 생성 모델인 MOFFlow를 소개합니다. 기존의 방법들은 MOF 구조의 복잡성으로 인해 어려움을 겪었던 반면, MOFFlow는 금속 노드와 유기 링크를 강체로 다루어 문제의 차원을 줄입니다.

- **Technical Details**: MOFFlow는 리만 유량 매칭( Riemannian flow matching) 프레임워크를 기반으로 하여, MOF의 금속 노드와 유기 링크의 회전 및 변환을 예측합니다. 이 모델은 $SE(3)$ 공간에서 작동하며, MOF 컴포넌트의 회전 및 전이 역학을 캡처하는 데 효과적입니다.

- **Performance Highlights**: MOFFlow는 324,426개의 MOF 구조로 구성된 데이터셋에서 실험을 수행한 결과, 보이지 않는 MOF 구조에서 31.69%의 적합도(match rate)를 달성했습니다. 기존의 방법들은 computationally expensive함에도 불구하고 거의 적합도를 보여주지 못했습니다.
...
### Radio Map Prediction from Aerial Images and Application to Coverage Optimization (https://arxiv.org/abs/2410.17264)
Comments:
          12 pages, 8 Figures, This work has been submitted to the IEEE for possible publication. arXiv admin note: substantial text overlap with arXiv:2402.00878

- **What's New**: 최근 연구에서 심층 학습(Deep Learning) 알고리즘을 활용하여 도시 통신 네트워크에서 신호 손실(Path Loss)을 예측하려고 하고 있습니다. 본 논문에서는 공중 사진(Aerial Images)과 보조적인 높이 정보와 함께 컨볼루션 신경망(Convolutional Neural Network, CNN)을 사용하여 무선 신호의 손실을 예측합니다. 이 접근법은 환경 객체의 명시적 분류에 의존하지 않으며, 접근성을 높이고 측정 비용을 절감할 수 있는 가능성을 제시합니다.

- **Technical Details**: 본 연구에서는 CNN을 사용하여 공중 사진 또는 높이 데이터를 기반으로 한 불완전한 환경 정보를 통해 라디오 맵(Radio Map)을 예측하는 문제를 다루었습니다. 새로운 네트워크 아키텍처가 제안되었고, 기존의 SOTA(State-of-the-Art) 모델을 능가하는 성능을 보였습니다. 모델은 미분 가능(Differentiable)하여 다양한 네트워크 최적화 알고리즘에 통합할 수 있는 장점이 있습니다.

- **Performance Highlights**: 훈련된 모델은 빠르고 정확한 예측을 가능하게 하며, 그 성능은 실시간 애플리케이션에 적합합니다. 새로운 데이터셋이 공개되어 실제 환경에 대한 3D 정보와 올바른 높이 및 형태 정보를 포함한 공중 사진을 제공하며, 향후 네트워크 최적화를 위한 기초 자료로 사용될 수 있습니다.
...
### Large Language Models for Knowledge-Free Network Management: Feasibility Study and Opportunities (https://arxiv.org/abs/2410.17259)
- **What's New**: 이 논문은 지식 기반 알고리즘이 아닌, 지식 없는 최적화 방법을 기반으로 하는 새로운 네트워크 관리 패러다임을 제안합니다. 이는 대규모 언어 모델(LLMs)을 활용하여 최적화를 수행하며, 시스템에 대한 사전 지식 없이도 다양한 네트워크 관리 문제에 적응할 수 있는 능력을 지닙니다.

- **Technical Details**: 지식 없는 최적화는 고차원적인 결정 과정을 필요로 하는 하이퍼-지능형 블랙박스 최적화기를 요구합니다. 본 논문에서는 GPT-3.5-Turbo를 사용하여 자원 관리 문제를 해결하며, 다수의 LLMs를 결합하여 효율적인 자원 할당 문제 해결을 시도합니다. 제안된 프레임워크에서는 수학적 모델이나 추가 훈련 없이 최적화를 수행할 수 있도록 설계되었습니다.

- **Performance Highlights**: 수치 결과는 지식 없는 LLM 최적화기가 기존의 지식 기반 최적화 알고리즘과 유사한 성능을 달성할 수 있음을 확인시켜 줍니다. 본 연구에서 제안된 방법은 전통적인 LLM 최적화기 기술보다 우수한 성능을 보이며, 미래의 다양한 연구 기회와 도전 과제가 제시됩니다.
...
### Annotation-Free MIDI-to-Audio Synthesis via Concatenative Synthesis and Generative Refinemen (https://arxiv.org/abs/2410.16785)
Comments:
          Work in progress; 7 pages, 2 figures, 1 table

- **What's New**: CoSaRef는 MIDI-오디오 짝 데이터셋 없이 MIDI-오디오 합성을 수행할 수 있는 새로운 메서드를 제안합니다. 이 방법은 합성된 오디오를 실제적인 트랙으로 정제하기 위해 확산 기반 깊은 생성 모델을 사용합니다.

- **Technical Details**: CoSaRef는 두 가지 모듈로 구성되어 있습니다: concatenative sampler와 generative refinement module. concatenative sampler는 MIDI 스코어를 입력으로 받아 미리 녹음된 샘플 라이브러리에서 해당 노트 샘플을 찾고, 이로부터 합성 트랙을 만듭니다. 이후 generative refinement module에서 GAN 모델을 이용하여 합성된 오디오를 실제적인 오디오로 변환합니다.

- **Performance Highlights**: 실험 결과, CoSaRef는 다양한 트랙 생성 및 음색 조절이 가능한 능력을 보여주며, 기존 방법들과 비교할 때도 유사한 수준의 표현력을 발휘했습니다.

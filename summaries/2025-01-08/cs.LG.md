...
### A Survey on Federated Learning in Human Sensing (https://arxiv.org/abs/2501.04000)
- **What's New**: 이번 연구에서는 Federated Learning (FL)을 Human Sensing 분야에 적용한 최근 동향을 종합적으로 분석하고, FL 접근법에 대한 분류 체계와 여덟 가지 차원에서의 평가 방법론을 제시합니다. 기존의 연구들은 여러 분야에서 FL의 유용성을 입증했으나, Human Sensing에서의 구체적인 과제들은 상대적으로 덜 탐구되었습니다. 본 조사는 FL이 실제 Human Sensing 작업에서 얼마나 효과적으로 기능할 수 있는지를 탐구하며, 이러한 연구 방향이 왜 중요한지를 논의합니다.

- **Technical Details**: FL 시스템은 사용자 개인정보를 중앙 서버로 전송하지 않고, 클라이언트 측에서 모델 파라미터만 교환하여 개인의 프라이버시 문제를 해결하려고 합니다. FL은 시간과 통신 비용을 절감하는 동시에 더 나은 개인화된 서비스를 제공할 수 있는 가능성을 지니고 있습니다. 그러나 FL은 시스템의 이질성, 제한된 라벨 데이터, 센서 데이터의 주관적인 도메인 이동과 같은 도전 과제를 여전히 안고 있습니다.

- **Performance Highlights**: 본 조사에서는 FL이 Human Sensing에서 다루어야 할 다섯 가지의 주요 연구 측면을 강조하고, 향후 연구가 필요한 개방 과제를 정리합니다. FL이 적용된 여러 연구의 결과를 분석하여, 이 분야의 성과와 한계를 명확히 하고, 실제 적용에서의 유용성을 평가하기 위해 여덟 가지 차원을 바탕으로 FL 시스템을 평가합니다. 이 연구를 통해 FL 연구자들이 Human Sensing의 복잡한 현실을 해결하는 솔루션을 개발하는 데 필요한 기반 자료를 제공합니다.



### WAPTS: A Weighted Allocation Probability Adjusted Thompson Sampling Algorithm for High-Dimensional and Sparse Experiment Settings (https://arxiv.org/abs/2501.03999)
- **What's New**: 본 연구에서는 비디오 콘텐츠 광고와 같은 사용자 참여를 위한 실험 디자인의 효율성을 높이기 위해 다중 군대 문제(multi-arm bandit problem) 접근 방식을 적용합니다. 기존 Contextual Bandit 프레임워크의 한계를 극복하기 위해, 동적 가중치 매개변수를 사용하는 Weighted Allocation Probability Adjusted Thompson Sampling (WAPTS) 알고리즘을 도입하였습니다. 이는 데이터가 부족한 환경에서도 더 효과적으로 개입을 최적화할 수 있도록 설계되었습니다.

- **Technical Details**: WAPTS 알고리즘은 Contextual Thompson Sampling (CTS) 방법을 기반으로 하며, 각 개입의 할당 확률을 동적으로 조정할 수 있는 가중치 매개변수를 도입합니다. 이 매개변수는 각 실험의 성과에 따라 자동으로 조정되어, 데이터가 부족할 때에도 최적의 팔(arm)을 빠르게 찾아내는 것을 목표로 합니다. 알고리즘은 Bayesian 업데이트 프로세스를 사용하여 성공 및 실패 수에 따른 후행 확률을 조정합니다.

- **Performance Highlights**: 다양한 팔의 수와 효과 크기에 대해 WAPTS의 성능을 실험적으로 검증하였습니다. 전통적인 방법에 비해 WAPTS는 더 빠르게 최적의 개입을 선택할 수 있으며, 데이터 희소성 문제를 효과적으로 해결합니다. 이러한 성과는 교육 환경에서 학생들의 학습 효과를 극대화하는 데 기여할 것으로 기대됩니다.
...
### A precise asymptotic analysis of learning diffusion models: theory and insights (https://arxiv.org/abs/2501.03937)
- **What's New**: 이 논문에서는 두 개의 레이어로 이루어진 오토인코더(auto-encoder)를 활용하여 고차원 대상으로부터 흐름 기반의 생성 모델(generative model)을 학습하는 문제를 다룹니다. 특히, 저차원 다양체(low-dimensional manifold) 구조를 기반으로 한 샘플의 분포를 낮은 차원으로 투영(projection)하는 방법을 제시하고, 훈련 샘플의 수에 따라 발생할 수 있는 문제를 분석합니다. 또한, 생성된 합성 데이터(synthetic data)로 모델을 재훈련할 때 발생할 수 있는 모드 붕괴(mode collapse) 문제를 논의합니다.

- **Technical Details**: 이 연구에서는 생성적 이동(generative transport) 아이디어를 바탕으로 고차원 벡터 공간에서 특정 밀도(ρ𝜌)를 샘플링하는 문제를 간결하게 설명합니다. 특히, stochastic interpolant 모델을 사용하여 가우시안 샘플에서 시작하여 Stochastic Differential Equation (SDE)를 통해 진행하는 방법을 제시합니다. 이 모델은 다양한 손실 함수(loss function)와 조건부 기대값(conditional expectation)을 사용하여 샘플을 복원하는 과정에서 데이터로부터 직접 학습할 수 있는 경로를 제공합니다.

- **Performance Highlights**: 실험적으로, 제안된 모델은 화난 소음(noisy data)에서 샘플을 효율적으로 복원할 수 있는 행동을 보여주며, 특히 저차원 공간에서의 분포 특성을 잘 유지하고 있습니다. 또한, DAE(de-noising auto-encoder)가 시간(t)를 입력으로 받지 않아도 우수한 성능을 보이는 것을 확인했습니다. 이러한 결과들은 복잡한 시간 인코딩(time encoding) 없이도 충분한 성능을 나타낼 수 있음을 시사합니다.
...
### Stochastically Constrained Best Arm Identification with Thompson Sampling (https://arxiv.org/abs/2501.03877)
Comments:
          30 pages, 12 figures, 1 table

- **What's New**: 이 연구에서는 확률적 제약 하에서 최적 무장 식별(Best Arm Identification, BAI) 문제를 다룹니다. 가장 중요한 성능 지표를 최적화하고 이를 제약 조건으로 설정하여 arm을 식별하는 방법인 BFAI(Best Feasible Arm Identification)를 제안합니다. 기존의 Thompson Sampling(TS)의 아이디어를 확장한 첫 번째 시도로, 이 알고리즘의 새로운 접근 방식을 소개합니다.

- **Technical Details**: 제안된 방법인 BFAI-TS는 고정된 예산 설정에서 비모수가 아닌 보상 벡터를 다루며, 후보가 아닌 arm을 선택할 확률을 조절하는 파라미터 β를 도입합니다. 이로써 TS의 일반적인 방식에 개선된 수렴 특성을 보장합니다. 알고리즘의 이론적 기반으로 어림잡기/후방 수렴 속도에 대한 분석을 수행하여, 각 arm이 충분히 샘플링됨을 입증하였습니다.

- **Performance Highlights**: BFAI-TS는 여러 합성 데이터셋 및 현실적인 데이터셋에서 성능을 평가한 결과, β의 적절한 설정으로 인해 비교 알고리즘들에 비해 높은 성능을 발휘합니다. 또한, BFAI-TS는 후방 확률이 빠르게 0으로 수렴함을 보여주어 알고리즘의 신뢰성을 강조합니다. 이론적 및 실험적 분석을 통해 알고리즘의 강력한 성능을 입증하였습니다.



### Truthful mechanisms for linear bandit games with private contexts (https://arxiv.org/abs/2501.03865)
Comments:
          To appear at AAMAS 2025

- **What's New**: 이 논문은 개인의 컨텍스트를 비공식적으로 보고하는 문제를 다루며, 기존의 알고리즘들이 이 문제를 어떻게 대처할 수 없는지를 보여줍니다. 다양한 의료 및 추천 시스템에서 등장하는 이 문제를 처음으로 탐구합니다. 연구진은 이를 통해 전통적인 알고리즘들이 어떻게 비정직한 보고 (misreporting)로 인해 성능 저하를 겪는지를 분석합니다.

- **Technical Details**: 논문은 Bayesian contextual linear bandit 모델을 기반으로 하여 K개의 팔(arms)과 각 팔에 할당된 숨겨진 파라미터를 정의합니다. 시스템은 새로운 에이전트가 개인적인 컨텍스트를 보고하면, K개의 팔 중 하나를 선택하게 됩니다. 기존의 알고리즘은 에이전트들이 개인 정보를 왜곡할 경우에 대해 비효율적으로 작동하여 높은 후회(regret)를 초래합니다.

- **Performance Highlights**: 새로운 메커니즘은 Thompson 샘플링 알고리즘을 바탕으로 하여, 에이전트들이 자신의 컨텍스트를 왜곡할 유인을 차단하도록 설계되었습니다. 실험 결과, 이 메커니즘은 O(ln T) 수준의 빈도수 기반 후회(regret)를 달성하며, 다양한 상황에서 높은 성능을 나타냅니다. 이 연구는 개인의 선택을 최적화하고 병원 임상 시험 및 추천 시스템과 같은 실제 문제를 해결하는 데 기여할 것으로 기대됩니다.



### Symmetry and Generalisation in Machine Learning (https://arxiv.org/abs/2501.03858)
Comments:
          PhD Thesis

- **What's New**: 이번 연구에서는 감독 학습에서 불변성(invariance)과 공변성(equivariance)이 일반화(generalisation)에 미치는 영향을 분석합니다. 비공변적인(predictor) 예측자에 대해, 모든 회귀(regression) 문제에서 테스트 위험이 엄밀히 낮은 공변적 예측자가 존재함을 수학적으로 증명했습니다. 이는 불변성 또는 공변성과 같은 대칭(symmetry)이 유용한 귀납적 편향(inductive bias)이라는 것을 시사합니다.

- **Technical Details**: 연구에서는 평균화 연산자(averaging operator)의 관점을 통해, 불변성 및 공변성을 조사합니다. 특정 모델들은 주어진 입력-출력 쌍의 모집단에서 학습했을 때, 예측(test risk)을 더 잘 줄일 수 있는 성질이 있습니다. 이 연구는 무작위 설계(random design) 최소 제곱(minimum squares) 및 커널 릿지 회귀(kernel ridge regression) 상황에 이를 적용하여, 그룹(group), 모델 및 데이터의 특성을 통해 테스트 위험의 감소를 구체적으로 설명합니다.

- **Performance Highlights**: 실험적으로, 공변적 모델들은 무한한 불변 모델보다 더 우수한 일반화 성능을 발휘합니다. 이 연구는 예측자(predictor)의 특성과 학습 알고리즘의 성능을 비교하는 데 중점을 두며, 다양한 문제에 있어 대칭 원리를 이용한 모델이 특히 긍정적인 결과를 보이는 경우를 지적합니다. 연구자는 이러한 기법이 양자 화학(quantum chemistry) 및 의료 영상(medical imaging) 등 다양한 분야에서 어떻게 활용될 수 있는지를 논의하며, 향후 연구 방향도 제시하였습니다.



### Machine learning applications in archaeological practices: a review (https://arxiv.org/abs/2501.03840)
- **What's New**: 최근 고고학 분야에서 인공지능(AI)과 머신러닝(machine learning) 기술의 활용이 급증하고 있습니다. 1997년부터 2022년까지 발표된 135개의 연구를 분석한 결과, 2019년 이후 관련 논문 수가 크게 증가한 것을 확인했습니다. 하지만 머신러닝의 일부 기술은 여전히 오해와 불명확한 요구사항으로 인해 제대로 활용되지 못하고 있습니다.

- **Technical Details**: 이번 연구에서는 머신러닝의 다양한 방법론을 포괄적으로 살펴보고, 자동 구조 감지와 유물 분류 등 주요 작업을 도출했습니다. 분석 결과, 클러스터링(clustering) 및 비지도 학습(unsupervised learning) 방법은 지도 학습(supervised learning) 모델에 비해 상대적으로 낮은 비율을 보였습니다. 또한, 인공신경망(artificial neural networks)과 앙상블 학습(ensemble learning)이 전체 모델의 3분의 2를 차지했습니다.

- **Performance Highlights**: 연구에 따르면, 머신러닝은 대량의 다변량 데이터 분석에 유용한 도구로 자리잡고 있습니다. 그러나 그 잠재력을 극대화하기 위해서는 명확하게 정의된 방법론과 협력적 실천이 중요합니다. 더욱이, 머신러닝을 활용한 고고학 연구의 성과와 윤리적 고려사항에 대한 논의는 여전히 활발히 진행 중입니다.
...
### Vision Transformer Neural Architecture Search for Out-of-Distribution Generalization: Benchmark and Insights (https://arxiv.org/abs/2501.03782)
Comments:
          Accepted in NeurIPS 2024

- **What's New**: 이 논문에서는 ViT 아키텍처의 Out-of-Distribution (OoD) 일반화에 중점을 둔 최초의 체계적인 벤치마크인 OoD-ViT-NAS를 소개합니다. 이 벤치마크는 다양한 연산 예산을 가진 3000개의 ViT 아키텍처를 포함하며, 8개의 일반적인 OoD 데이터셋에서 평가하였습니다. 연구진은 이를 통해 ViT 아키텍처 디자인이 OoD 일반화와 상관관계가 있음을 밝혀냈고, 기존의 ID 성능 최적화가 OoD 정확도를 예측하는 데 부적합하다는 것을 제시했습니다.

- **Technical Details**: ViT 아키텍처에 대한 기존 연구는 In-Distribution (ID) 정확도를 극대화하는 데 중점을 두었으나, OoD 일반화에 대한 연구는 부족했습니다. 이 논문은 One-Shot NAS 방식을 통하여 AutoFormer 기반의 다양한 서브 아키텍처를 샘플링하여 벤치마크를 구축하였습니다. 이 방식은 고성능의 ViT 아키텍처를 효율적으로 확보할 수 있게 해줍니다.

- **Performance Highlights**: OoD-ViT-NAS 벤치마크를 통해, 아키텍처 디자인이 OoD 정확도에 미치는 중요한 영향을 보여주었습니다. 특정 아키텍처 특성이 인지한 대로 OoD 일반화를 개선할 수 있으며, 임베딩 차원을 증가시키는 것이 전반적으로 성능을 향상시킨다는 것을 발견했습니다. 논문에서는 일부 OoD 시나리오에서 11.85%의 정확도 향상을 기록하여, ViT 아키텍처가 OoD 연구에서 얼마나 중요한지 다시 한번 강조하였습니다.
...
### A Multimodal Lightweight Approach to Fault Diagnosis of Induction Motors in High-Dimensional Datas (https://arxiv.org/abs/2501.03746)
- **What's New**: 본 논문은 유도 전동기의 (IM) Broken Rotor Bar (BRB) 결함 진단을 위한 ShuffleNetV2라는 경량 딥러닝 모델을 적용한 대규모 데이터 세트를 사용하여 새로운 방법을 제시합니다. 기존 연구의 한계점은 소규모 데이터에 기반하여 훈련되어 과적합(overfitting) 문제를 겪는 것이었습니다. 본 연구는 57,500개의 스펙트럼 이미지를 활용하여 정확하고 효과적인 결함 진단 개선을 목표로 하고 있습니다.

- **Technical Details**: 이 연구는 현재 및 진동 신호 데이터를 사용하여 BRB 결함의 진단을 위해 ShuffleNetV2 모델을 구현하였습니다. 데이터 전처리 과정으로 Short-Time Fourier Transform (STFT)을 사용하여 훈련 및 테스트에 사용할 스펙트럼 이미지를 생성했습니다. 전체 57,500개의 이미지 중 훈련에는 47,500, 테스트에는 10,000장이 사용되었으며, Fast Fourier Transform (FFT)을 통해 결함에 따른 하모닉 사이드밴드를 시각화했습니다.

- **Performance Highlights**: ShuffleNetV2 모델은 스펙트럼 이미지의 98.856%를 정확하게 분류하는 뛰어난 성능을 보였습니다. 경량 모델로서 높은 정확성을 유지하면서도 계산 비용이 적어 효율적인 결함 진단 시스템 개발의 기반을 마련하였습니다. 본 논문의 결과는 다양한 ML 및 DL 모델의 성능과 효율성에 대한 통찰을 제공하며, 산업 환경에서의 유도 전동기 결함 진단 시스템 개발에 기여할 것으로 예상됩니다.



### Deep Networks are Reproducing Kernel Chains (https://arxiv.org/abs/2501.03697)
Comments:
          25 pages, 3 figures

- **What's New**: 이번 논문은 깊은 신경망의 적절한 함수 공간을 정의하는 과제에 대해 새로운 프레임워크인 체인 RKBS(cRKBS)를 제안합니다. 기존의 RKBS에서 함수 대신 커널을 구성함으로써 RKBS의 장점을 보존하면서도 깊은 신경망의 고유한 특성을 해결합니다. 이를 통해, 모든 깊은 신경망 함수는 신경 cRKBS 함수이고, 유한 데이터 세트에서 정의된 모든 신경 cRKBS 함수는 깊은 신경망에 대응한다는 것을 증명했습니다.

- **Technical Details**: 신경 cRKBS는 커널 체인을 통해 구축되며, 이는 심층 네트워크의 숨겨진 레이어를 직접 맞추는 방식으로 구성됩니다. 이 접근 방식은 각 레이어의 뉴런 수가 데이터 포인트 수보다 많지 않도록 설정하여 경험적 위험 최소화 문제에 대한 희소한 솔루션을 제공합니다. 깊은 신경망과 그에 대응하는 공간들은 세 가지 주요 범주로 그룹화될 수 있으며, 이를 통해 깊은 네트워크의 질문을 해결하기 위한 기초 연구를 개관합니다.

- **Performance Highlights**: 이 논문에서 제안하는 cRKBS는 특히 원하는 속성을 보존하고 심층 네트워크 구조의 제약 조건을 최소화하며 경험적 위험을 효과적으로 감소시키는 데 도움을 줍니다. 이는 신경망의 표현 능력을 높이며, 각 레이어의 뉴런 수를 효율적으로 제한할 수 있는 가능성을 제시합니다. 새로운 이론적 결과와 함께 신경망을 해석할 수 있는 새로운 방법론을 제공하여 머신 러닝 분야에서의 깊은 신경망 연구를 더욱 발전시킬 것으로 기대됩니다.
...
- **What's New**: 이번 연구에서는 딥 러닝(DL) 모델이 표 형식 데이터 회귀 작업에서 더 나은 성과를 내도록 돕기 위해 여러 데이터 보강(data augmentation, DA) 방법을 제안하고 평가했습니다. 기존의 머신 러닝(ML) 모델이 DL 모델보다 우수한 경우가 많음을 감안할 때, 이 연구는 DL 모델의 잠재력을 극대화하는 방법에 대한 중요한 통찰력을 제공합니다. 특히, 여러 가지 DA 전략을 비교하여 고급 보강 방법이 여러 데이터 세트와 회귀 작업에서 DL 모델 성능을 유의미하게 향상시킨다는 것을 입증했습니다.

- **Technical Details**: 이 연구에서는 DL 모델의 성능을 개선하기 위해 데이터 보강 기술을 적용했습니다. 기존 관측치를 단순히 복제하거나 노이즈를 추가하는 것부터 시작하여, 데이터의 근본 통계적 관계를 보존하는 고급 DA 전략까지 다양한 방법이 비교되었습니다. 연구는 30개의 여러 데이터 세트를 통해 자동 딥 러닝(AutoDL) 프레임워크인 AutoKeras, H2O, AutoGluon을 사용하여 이러한 DA 전략의 효과성을 rigorously 검증했습니다.

- **Performance Highlights**: 결과적으로, DA 적용 없이 DL을 수행하는 기본 모델과 비교했을 때 평균적으로 10% 이상의 성능 향상이 기록되었습니다. 연구에서는 새로운 DA 기술을 통해 DL 모델의 회귀 작업에서의 성과가 개선될 수 있음을 보여주었으며, 이는 실용적인 응용에서의 더 넓은 채택과 성능 향상에 기여할 것으로 기대됩니다.
...
### Coupled Hierarchical Structure Learning using Tree-Wasserstein Distanc (https://arxiv.org/abs/2501.03627)
- **What's New**: 이 논문에서는 데이터 샘플과 피처 간의 잠재적 계층 구조의 유기적 결합 구조를 탐구하는 Coupled Hierarchical Structure Learning (CoHSL) 방법을 소개합니다. 기존의 학습 방법들은 샘플이나 피처 중 하나의 계층 구조에만 초점을 맞췄으나, 이 연구는 두 가지를 동시에 고려하여 TWD(트리-워셔인 거리)를 통해 이를 해결하고자 합니다. 이 방법은 특히 고차원 데이터에서 계산 효율성을 유지하며, Hyperbolic Graph Convolutional Networks(HGCN)와 통합하여 성능을 개선할 수 있음을 보여줍니다.

- **Technical Details**: CoHSL은 세 가지 주요 구성 요소로 이루어져 있습니다. 먼저, 데이터 행렬에서 샘플(행)과 피처(열)를 각각 나타내는 트리를 구성합니다. 이어서 트리-워셔인 거리(TWD)를 사용하여 두 가지 모드 간의 거리를 계산하며, 각 모드의 정보를 서로 보완하는 방식으로 트리를 세분화합니다. 마지막으로, 두 모드 간의 트리 추론과 TWD 계산을 반복적으로 수행하여 샘플과 피처의 계층 구조를 개선합니다.

- **Performance Highlights**: 이 연구에서는 CoHSL 방법이 여러 워드-문서 데이터와 단일 세포 RNA 시퀀싱 데이터셋에서 기존의 방법들보다 우수한 성능을 보인다고 보고합니다. 특히, 링크 예측 및 노드 분류 작업에서 HGCN에 통합했을 때 성능 개선 효과가 뚜렷하게 나타났습니다. 이는 CoHSL 방법이 데이터의 내재된 계층적 구조를 보다 효과적으로 학습할 수 있게 해주기 때문입니다.
...
### AADNet: Exploring EEG Spatiotemporal Information for Fast and Accurate Orientation and Timbre Detection of Auditory Attention Based on A Cue-Masked Paradigm (https://arxiv.org/abs/2501.03571)
- **What's New**: 이 연구에서는 시뮬레이션된 실제 환경에서의 청각 주의력 디코딩을 위한 새로운 패러다임을 제안했습니다. 'cue-masked auditory attention paradigm'을 도입하여 실험 전 정보 누출을 방지했으며, 이를 통해 더 현실적인 데이터 수집을 가능하게 하였습니다. 또한, 새로운 딥러닝 모델인 AADNet을 발표하여 EEG 신호의 시공간 정보를 활용해 높은 디코딩 정확도를 달성했습니다.

- **Technical Details**: AADNet 모델은 짧은 EEG 신호 시간 창을 통해 청각 주의력의 방향(Orientation Attention, OA)과 음색(Timbre Attention, TA) 정보를 디코딩합니다. 0.5초의 EEG 창을 사용하여 OA에서 93.46%, TA에서 91.09%의 평균 정확도를 기록했습니다. 이 연구는 기존의 5가지 방법보다 우수한 성능을 보였으며, 원래 오디오 소스를 알 필요 없이도 주의력을 측정할 수 있음을 입증했습니다.

- **Performance Highlights**: 시험 결과, AADNet은 빠르고 정확하게 청각 주의력의 방향과 음색을 감지할 수 있습니다. 이 연구는 신경 기반 청각 보조기구와 같은 실시간 다중 특성 청각 주의력 디코딩을 위한 가능성을 보였습니다. 이러한 기술의 발전은 청각 기구의 활용도를 높이고, 다양한 환경에서의 청취 경험을 개선하는 데 기여할 것으로 기대됩니다.



### Advanced Tutorial: Label-Efficient Two-Sample Tests (https://arxiv.org/abs/2501.03568)
- **What's New**: 이번 논문에서는 label-costly 환경에서 두 샘플 테스트(two-sample testing)의 개념을 확장하여, 머신 러닝의 액티브 러닝(active learning)과 유사한 시나리오를 다룹니다. 분석가는 두 샘플에서 많은 특징을 가지고 있지만, 이러한 특징의 샘플 멤버십(sample membership)이나 라벨(label)을 결정하는데 드는 비용이 높은 상황에 초점을 맞추고 있습니다. 이를 통해 통계적 유효성과 높은 테스트 파워(testing power)를 유지하면서 라벨 효율적인 두 샘플 테스트의 실제 응용 사례를 논의합니다.

- **Technical Details**: 전통적인 두 샘플 테스트는 두 샘플이 동일한 분포(null hypothesis)에서 유래했는지 또는 서로 다른 분포(alternative hypothesis)에서 유래했는지를 평가합니다. 이 연구에서는 특징(feature)과 해당 라벨이 동시에 사용 가능하다는 기존 가정 대신, 측정값(measurements)은 쉽게 접근 가능하나 그룹(group)에 대한 정보는 드물고 획득하기 어려운 설정을 소개합니다. 이는 머신 러닝 커뮤니티에서 액티브 러닝과 유사한 문제로, 최적의 라벨 쿼리(strategy) 방안을 개발하여 통계적 유효성을 유지하고 높은 테스트 파워를 보장하는 것을 목표로 합니다.

- **Performance Highlights**: 라벨 효율적인 두 샘플 테스트의 수행 가능성에 대해 논의하며, 라벨 쿼리 수를 최소화하면서도 전통적인 두 샘플 테스트와 유사한 성능을 달성할 수 있음을 보입니다. 이를 위한 효율적인 라벨 쿼리 기법인 bimodal query를 도입하고, 배치(batch) 및 순차적(sequential) 두 샘플 테스트를 통해 성능을 평가합니다. 또한, 아밀로이드 축적과 디지털 바이오마커를 비교하는 등의 실제 예제를 통해 연구의 필요성을 강조합니다.
...
### Multi-Source Urban Traffic Flow Forecasting with Drone and Loop Detector Data (https://arxiv.org/abs/2501.03492)
- **What's New**: 최근의 교통 예측 연구는 주로 단일 데이터 모달리티인 루프 감지기에 집중되어 있었습니다. 하지만, 인공지능(AI)과 드론 기술의 발전으로 도시 트래픽에 대한 고효율, 정확, 유연한 공중 관측이 가능해졌습니다. 이 논문은 드론과 루프 감지기 데이터를 동시에 활용하여 다중 출처의 트래픽 속도를 예측하는 문제를 다루고 있으며, 도시의 대규모 네트워크를 대상으로 한 정확한 다중 센서 모빌리티 관측소를 구축하는 방안을 제시합니다.

- **Technical Details**: 이 논문에서는 HiMSNet라는 그래프 기반 모델을 제안하여 여러 데이터 모달리티를 통합하고 시공간 상관관계를 학습합니다. 특히, 세그먼트 수준의 속도 예측은 지역 속도 예측보다 더 어려운 도전 과제가 될 수 있으며, 혼잡도가 높고 교통 동역학이 급변하는 시나리오에서 더욱 그렇습니다. 드론과 루프 감지기 데이터를 활용하면 센서의 커버리지가 낮고 노이즈가 많은 경우에도 예측 정확도를 높일 수 있음을 보여줍니다.

- **Performance Highlights**: 실제 도시 도로 네트워크에서 차량 궤적을 기반으로 한 시뮬레이션 연구는 교통 예측 및 모니터링에 드론을 통합하는 것의 가치를 강조합니다. HiMSNet은 SimBarca라는 다중 출처 데이터셋을 활용하여 세그먼트 수준과 지역 속도 예측을 평가합니다. 이 연구는 드론 및 루프 감지기에서 취합된 데이터를 활용한 도시 교통 예측을 위한 새로운 패러다임을 제시합니다.



### Entropy-Guided Attention for Private LLMs (https://arxiv.org/abs/2501.03489)
Comments:
          The 6th AAAI Workshop on Privacy-Preserving Artificial Intelligence (PPAI), 2025. arXiv admin note: substantial text overlap with arXiv:2410.13060

- **What's New**: 이 논문은 개인 정보 보호(PI)를 위한 비공식적 인프런스 기술에 대한 새로운 접근 방법을 제시합니다. 특히 전자 파일의 암호화된 데이터에서 연산을 수행할 때 비선형성의 역할을 분석하고, 이를 최적화하기 위한 정보 이론적 프레임워크를 도입합니다. Shannon의 엔트로피를 활용하여 비선형성의 중요성을 두 가지 차원에서 밝혀내며, 이는 학습 안정성 유지와 주의 헤드 다양성을 위한 것임을 설명합니다.

- **Technical Details**: 저자들은 비선형성의 제거가 두 가지 주요 실패 모드를 촉발하는 것을 발견했습니다: 더 깊은 레이어에서의 '엔트로피 붕괴'(entropy collapse)와 초기 레이어에서의 '엔트로피 과부하'(entropic overload). 이를 해결하기 위해 엔트로피 중심의 주의 메커니즘과 새로운 엔트로피 규제 기법을 제안하며, PI 친화적인 레이어 정규화 대안을 탐색하여 엔트로피 붕괴를 방지하는 방법을 모색합니다. 이러한 방식으로 정보 이론과 아키텍처 설계 간의 간극을 해소하고, 효율적인 PI 아키텍처 개발을 위한 엔트로피 역학을 원칙적으로 도출합니다.

- **Performance Highlights**: 논문에서는 GPT-2 모델의 256 토큰 입력과 18 레이어 구조에서 비선형 연산과 FLOPs에 대한 구체적인 비교를 통해 지연 시간(latency)과 통신 절약 성과를 제시합니다. 결과적으로 엔트로피 기반 접근 방식이 학습 안정성과 토큰 선택의 불확실성을 제어하는 데 유용하다는 점을 강조하며, 특히 수학적 추론과 관련된 토큰 수준 성능 향상에 중요한 기여를 한다고 설명합니다.
...
### A study on performance limitations in Federated Learning (https://arxiv.org/abs/2501.03477)
Comments:
          archive 2021 work

- **What's New**: 이 논문은 Federated Learning (FL)이라는 새로운 기계 학습 패러다임의 발전을 다룹니다. FL은 개인 정보 보호에 대한 우려가 증가하고 데이터 접근이 자유로워짐에 따라 발전하였으며, Google에 의해 2016년에 처음 소개되었습니다. FL은 분산 기계 학습의 여러 아이디어를 차용하지만, 모바일 기기에서 모델이 학습됨으로 인해 발생하는 다양한 과제를 다룹니다.

- **Technical Details**: FL은 다양한 도전 과제가 동반되며, 특히 통신 병목 현상(communication bottleneck)과 비독립적이고 동일 분포가 아닌 데이터(Non IID-ness)의 문제를 중점적으로 연구하고 있습니다. 이러한 문제들은 기본 모델을 통해 특성화하고, 모델의 성능을 평가하는 방식으로 다루어집니다. 또한, 성능 향상을 위한 논의도 포함되어 있습니다.

- **Performance Highlights**: FL은 여러 분야에서 활발하게 연구되고 있으며, 최적화 알고리즘 및 개인화된 프라이버시 보호 모델과 같은 주제들을 포함합니다. 하지만 여전히 여러 가지 도전과제들이 존재하며, 이 연구는 이러한 문제를 해결하기 위한 첫 걸음을 제시하고 있습니다. 성능 평가를 통해 모델의 효율성을 높이기 위한 새로운 통찰을 제공합니다.
...
### Optimizing Value of Learning in Task-Oriented Federated Meta-Learning Systems (https://arxiv.org/abs/2501.03448)
- **What's New**: 최근 페더레이티드 러닝(Federated Learning, FL)의 한계점이 크게 부각되고 있으며, 이 논문에서는 이를 개선하기 위한 페더레이티드 메타 러닝(Federated Meta-Learning, FML) 프레임워크를 제안합니다. 일반적인 FL이 모든 참여자에게 공통된 글로벌 모델을 제공하는 반면, FML은 장치들이 서버로부터 공유된 메타 모델을 받고 이를 기반으로 로컬 모델을 조정할 수 있습니다. 또한, 새로운 메트릭인 Learning Value(VoL)를 도입하여 각 장치의 개별 학습 필요성을 평가하고, 특정 작업 요건을 유지하면서 공정성을 고려합니다.

- **Technical Details**: 논문에서는 비정형 다중접속(Non-Orthogonal Multiple Access, NOMA) 네트워크를 기반으로 FML 프레임워크를 제안하며, 기계 학습에서의 TLW(Task Level Weight) 개념을 도입하여 다양한 작업의 중요도를 평가합니다. 문제는 TLW 기반의 VoL을 극대화하는 방향으로 설정되며, 이는 비선형 혼합 정수 프로그램(Non-Convex Mixed-Integer Non-Linear Programming, MINLP) 문제로 정의됩니다. 문제를 해결하기 위해 매개변수화된 심층 Q-네트워크(Parameterized Deep Q-Network, PDQN) 알고리즘을 개발하여 이산 및 연속 변수를 동시에 처리합니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 프레임워크는 베이스라인 기법에 비해 성능이 현저히 향상된 것으로 나타났습니다. 새로운 VoL 및 TLW 메트릭을 활용한 접근 방식은 장치별 요구를 효과적으로 반영할 수 있어, 보다 개인화된 솔루션을 제공하며 통신 효율성을 개선하는 것으로 평가됩니다. 이는 FML을 통한 분산 학습을 통해 다양한 작업에 최적화된 해법을 제시함으로써 기존의 한계를 뛰어넘을 가능성을 보여줍니다.



### Physics-Constrained Generative Artificial Intelligence for Rapid Takeoff Trajectory Design (https://arxiv.org/abs/2501.03445)
Comments:
          Conference version with 10 pages and 7 figures

- **What's New**: 이 논문은 전통적인 다학제 분석 및 최적화(MDAO)의 고비용과 물리적 제약에 어려움을 겪는 대체로서 물리 제약이 있는 생성적 적대 신경망(physicsGAN)을 제안합니다. physicsGAN은 eVTOL 항공기의 이륙 제어 프로필을 지능적으로 매개변수화하여 원래 설계 공간을 실행 가능한 공간으로 변환하게 됩니다. 이 프레임워크의 향상된 효율성과 정확성이 본 연구의 주요 기여 중 하나입니다.

- **Technical Details**: 제안된 physicsGAN 모델은 높은 차원의 비선형 최적화 문제에서 물리적 제약을 만족하는 이륙 궤적을 생성하는 데 초점을 맞춥니다. 이 모델은 공급망 데이터와 GAN을 기반으로 한 서베이 모델에 의해 제공되는 데이터를 학습하여 제약이 위반될 경우 페널티를 부여받습니다. 이를 통해 원래의 설계 요구 공간을 제약이 만족되는 실행 가능한 공간으로 변환하는 것이 가능해집니다.

- **Performance Highlights**: 제안된 설계 프레임워크는 Airbus A3 Vahana 드론의 이륙 궤적 설계에서 99.6%의 정확도를 달성했으며, 2.2초 만에 계산을 완료하여 기존 방법보다 약 200배 빠른 성능을 보여줍니다. physicsGAN을 기반으로 한 최적화는 데이터 기반 GAN 최적화보다 10배 이상 빠르며, 실제 작업에서 발생할 수 있는 비실행 가능 지역에 갇히는 문제도 발생하지 않습니다. 결과적으로, physicsGAN 기반의 설계 프레임워크는 효율성, 최적성 및 실행 가능성 면에서 뛰어난 성능을 보입니다.



### Mixture-of-Experts Graph Transformers for Interpretable Particle Collision Detection (https://arxiv.org/abs/2501.03432)
- **What's New**: 이 논문에서는 그래프 변환기 모델(Graph Transformer)과 전문가 혼합(Mixture-of-Expert) 층을 결합하여 고도의 예측 성능을 가지면서도 해석 가능성이 내장된 모델을 제안합니다. 그래프를 통한 충돌 데이터를 분석할 때, 이러한 접근 방식이 기계 학습 모델의 결정 과정을 더욱 투명하게 만들어 준다는 점에서 중요한 발전입니다. 주목할 만한 점은 이 모델이 물리학적으로 의미 있는 피처와 예측 간의 연관성을 명확히 할 수 있는 능력입니다.

- **Technical Details**: 모델은 그래프 신경망(Graph Neural Network)과 트랜스포머 아키텍처를 바탕으로 하여, 주의 맵(attention maps)을 활용하여 그래프 구조에서의 관계를 시각화하는 능력을 가집니다. 표준 피드 포워드 레이어를 Mixture of Experts(MoE) 레이어로 대체하여, 전문화된 "전문가"들이 개별 데이터 하위 집합을 처리하도록 하여 다양한 데이터에 보다 적합한 예측을 가능하게 합니다. 이러한 디자인은 모델의 설명 가능성을 크게 향상시키며, 물리 법칙과 일치하는 결정 경로를 추적할 수 있습니다.

- **Performance Highlights**: ATLAS 실험에서의 시뮬레이션된 이벤트를 대상으로 세부 테스트를 진행하였으며, 희귀한 초대칭 신호(event)와 표준 모델 배경(background)을 구분하는 데 있어 경쟁력 있는 분류 정확도를 달성했습니다. 특히, 모델은 예측의 해석 가능성을 제공하여 잘 알려진 물리학 개념과 일치함을 보여주었습니다. 이는 고에너지 물리 데이터 분석에 있어 AI 기반 접근법의 신뢰성을 높이는 중요한 결과로, 신뢰성 있는 도구로서의 가능성을 입증합니다.
...
### Low-Order Flow Reconstruction and Uncertainty Quantification in Disturbed Aerodynamics Using Sparse Pressure Measurements (https://arxiv.org/abs/2501.03406)
- **What's New**: 본 논문에서는 희소하고 잡음이 많은 표면 압력 측정값으로부터 저차의 돌풍( gust ) 경과 유동장(flow field) 및 양력 계수(lift coefficient)를 재구성하는 새로운 머신러닝 프레임워크를 제시합니다. 센서가 돌풍-항공익 항작용(gust-airfoil interaction)에 대한 시간에 따라 변화하는 반응을 면밀히 조사하여 최적의 센서 배치에 대한 귀중한 통찰을 제공합니다. 또한, 확률 회귀(pr probabilistic regression) 전략을 구현하여 딥러닝(predictions) 예측의 불확실성을 모델링합니다.

- **Technical Details**: 이 연구에서는 Monte Carlo dropout 기법을 사용하여 모델의 신뢰도를 나타내는 인식 불확실성(epistemic uncertainty)과 측정의 잡음으로 인해 발생하는 우연적 불확실성(alectoric uncertainty)을 모델링합니다. 본 연구의 방법론은 고급 데이터 압축 기술을 활용하여 입력 표면 압력 측정 값을 저차원 잠재 공간으로 매핑(maps)하여 유동장(flow fields)의 효율적 재구성을 지원합니다. 마지막으로, 학습된 손실 감소방법을 추가하여 측정 잡음을 처리하며, 이러한 복합적인 접근법은 예측의 신뢰도를 높입니다.

- **Performance Highlights**: 연구 결과는 극한의 공기역학적 조건에서도 공기 역학적 거동(aerodynamic behavior)을 정확하게 예측하는 이중 불확실성 정량화 군 전략의 효능을 보여줍니다. 또한 제안된 접근법은 실제 응용에서 온라인 센서 기반 흐름 추정(flow estimation)을 개선할 수 있는 잠재력을 강조합니다. 이와 같은 불확실성 정량화는 공기역학적 성능의 신뢰성을 증가시키고, 복잡한 고차원 데이터에 대한 예측의 정확성을 향상시킵니다.
...
### Detecting Defective Wafers Via Modular Networks (https://arxiv.org/abs/2501.03368)
- **What's New**: 본 연구에서는 반도체 제조 공정 내 센서의 사용 가능성이 증가함에 따라 결함이 있는 웨이퍼를 데이터 기반 모델로 감지할 수 있는 가능성을 제시합니다. 기존 모델들은 다양한 웨이퍼 제품의 다양한 공정 간의 상관관계에 대한 주의를 기울이지 않았으며, 일반화(generalizability) 문제에 직면해 있습니다. 이를 해결하기 위해, 우리는 제조 공정의 구조를 반영한 모듈형 네트워크(modular network, MN)를 제안합니다.

- **Technical Details**: 모듈형 네트워크는 시계열(stage-wise) 데이터셋을 사용하여 훈련되며, KQI(주요 품질 지표, key quality indicators) 예측을 위한 단계를 모듈 조합으로 분해합니다. 이 과정은 반도체 제조의 조합적(compositional) 특성을 시뮬레이션하여 다양한 웨이퍼 유형과 제조 공정 간의 결함 감지를 향상시킵니다. 연구 결과, 이 디자인이 해석 가능한 인터페이스를 제공하여 실제 응용 가능성을 높임을 확인했습니다.

- **Performance Highlights**: 광범위한 실험을 통해 제안된 접근 방식의 유용성이 입증되었습니다. 특히, 우리의 모듈형 네트워크가 다양한 웨이퍼 제품군에 걸쳐 더 나은 결함 감지를 이끌어낸다는 점이 중요합니다. 이는 물리적 측정 단계 유지 관리의 자본 및 인건비를 크게 줄이는 데 기여할 수 있습니다.
...
### Method of data forward generation with partial differential equations for machine learning modeling in fluid mechanics (https://arxiv.org/abs/2501.03300)
- **What's New**: 이 연구는 유체역학에서 인공지능(AI)의 활용을 위한 중요한 고충 중 하나인 고충실도 데이터(High-Fidelity Data)를 효과적으로 생성하는 새로운 방법을 제안합니다. 특히, 이 방법은 부분 미분 방정식(Partial Differential Equations, PDEs)을 사용하여 데이터 쌍을 구성하고, 이를 통해 물리 법칙에 따른 데이터 생성을 가능하게 합니다. 이는 기존의 데이터 확보가 어렵거나 비싼 문제를 해결할 수 있는 혁신적인 접근으로 보입니다.

- **Technical Details**: 제안된 방법은 두 가지 주요 기법을 포함합니다. 첫째, PDE의 해를 생성하기 위해 임의의 필드(Gaussian Random Field, GRF)나 물리적 법칙을 따르는 방법을 사용합니다. 둘째, Poisson 신경망(Poisson Neural Network, Poisson-NN)과 웨이블릿 변환 합성곱 신경망(Wavelet Transform Convolutional Neural Network, WTCNN)을 적용하여 비압축성 Navier-Stokes 방정식을 해결하는 방식이 사용되었습니다. 이 과정에서 해결해야 할 경계값과 초기 조건은 PDE를 만족하도록 계산됩니다.

- **Performance Highlights**: 실험 결과에 따르면, DNS(Direct Numerical Simulation) 데이터 없이도 생성된 데이터로 Poisson-NN과 WTCNN을 훈련할 수 있으며, 두 모델 모두 우수한 일반화(Generalization) 및 정확도(Accuracy)를 보여줍니다. 특히 물리적 법칙에 따른 데이터는 GRF를 따르는 데이터보다 수렴 속도(Convergence Rate)와 일반화 성능의 향상을 실현하며, 전체적인 성능을 크게 개선하는 결과를 나타냅니다.
...
### Adaptive Pruning of Pretrained Transformer via Differential Inclusions (https://arxiv.org/abs/2501.03289)
- **What's New**: 이 논문에서는 사전 훈련된 트랜스포머(Transformer) 모델의 효율적인 압축을 위한 새로운 방법, SPP(Solution Path Pruning)를 제안합니다. SPP는 최적화 문제의 해의 경로(solution path)를 통해 다양한 희소성 수준을 가진 트랜스포머 가중치 패밀리를 생성할 수 있습니다. 기존의 고정된 압축 비율을 사용하는 방법들과는 달리, SPP는 단일 검색 단계에서 원하는 비율로 모델을 압축할 수 있는 유연성을 제공합니다.

- **Technical Details**: SPP는 쿼리-키 쌍, 값-투영 쌍, 순차적 선형 레이어 등 쌍 모듈을 세분화하여 저랭크 압축(low-rank compression)을 적용합니다. 이 프로세스는 트랜스포머의 출력 구조를 유지하면서 내부 상태에서 구조적 압축이 가능합니다. 또한, SPP는 반복적인 형태로 구현되며, 이를 통해 모델의 다양한 압축 비율을 신속하게 얻을 수 있습니다.

- **Performance Highlights**: 연구에서는 ImageNet 데이터셋과 CLIP 모델을 활용하여 SPP의 효과를 입증했습니다. SPP를 적용하여 계산 비용을 크게 줄이면서도 예측 정확도를 유지하는 데 성공했습니다. 다양한 트랜스포머 백본을 실험한 결과, SPP의 방법론이 뛰어난 성능을 나타내며, 다양한 희소성 수준을 동시에 지원하는 점이 특히 강조되었습니다.



### Inverse Design of Optimal Stern Shape with Convolutional Neural Network-based Pressure Distribution (https://arxiv.org/abs/2501.03286)
- **What's New**: 본 논문에서는 후미 형상 설계를 위한 새로운 접근 방식을 제안합니다. 딥러닝(deep learning)을 활용하여 압력 분포를 기반으로 후미 모양을 추정하는 역설계(逆設計, inverse design) 알고리즘을 도입하였습니다. 이는 전통적인 방식인 계산 유체 역학(computational fluid dynamics) 계산 및 모델 실험을 대체할 수 있습니다.

- **Technical Details**: 압력 분포를 윤곽선 형태로 표현하고 이를 기반으로 특징을 추출하기 위해 합성곱 신경망(convolutional neural network)을 사용하였습니다. 또한, 다중 작업 학습(multi-task learning) 모델을 활용하여 후미의 다양한 단면을 추정하는 데 초점을 맞추었습니다. 이를 통해 B-spline의 제어점을 추정하고, 실제 및 변환된 오프셋을 비교하여 후미 형상을 간접적으로 추정합니다.

- **Performance Highlights**: 제안된 알고리즘은 후미 형상에 대한 성능이 검증되었습니다. 실제 결과와 모델에서 도출한 결과를 비교하여 설계 요구 사항에 부합하는 성능을 달성하였습니다. 본 연구의 결과는 후미의 효율성을 향상시키기 위한 새로운 기법으로 활용될 수 있을 것입니다.



### Sensorformer: Cross-patch attention with global-patch compression is effective for high-dimensional multivariate time series forecasting (https://arxiv.org/abs/2501.03284)
Comments:
          18 pages, 15 figures

- **What's New**: 최근 다변량 시계열 예측(Multivariate Time Series Forecasting) 분야에서 Sensorformer라는 새로운 Transformer 기반 모델이 제안되었습니다. 이 모델은 글로벌 패치 정보(Global Patch Information)를 압축하여 변수 간(cross-variable) 및 시간 간(cross-time) 종속성을 동시에 추출하는 방식으로, 기존 방법들이 해결하지 못한 문제를 개선하였습니다. Sensorformer는 동적인 인과 관계(Dynamic Causal Relationships) 속에서 변수 간의 올바른 상관관계를 효과적으로 포착할 수 있습니다.

- **Technical Details**: Sensorformer는 전통적인 Channel-Dependent Transformer 방법과 달리, 패치 압축 및 self-attention 기법을 통해 인과 관계를 보다 효율적으로 처리합니다. 이 접근 방식은 계산 복잡도(Computational Complexity)를 $O(D^2 	imes Patch
um^2 	imes d	ext{model})$에서 $O(D^2 	imes Patch
um 	imes d	ext{model})$로 줄여, 성능을 극대화합니다. 이 구조적 개선에 따라서, Sensorformer는 변동하는 인과 시간 지연(Dynamic Time Lags)에 강한 내성을 가지고 있습니다.

- **Performance Highlights**: Sensorformer는 9개의 주요 실제 다변량 시계열 예측 데이터셋에서 기존 모델들과 비교하여 우수한 성능을 입증했습니다. 복잡한 데이터에서도 뛰어난 정확성을 보이며, 높은 수준의 효율성을 제공합니다. 이 모델의 구현 및 주요 결과를 재현하기 위한 스크립트는 공개되어 있어, 연구자들이 쉽게 접근할 수 있습니다.
...
### mFabric: An Efficient and Scalable Fabric for Mixture-of-Experts Training (https://arxiv.org/abs/2501.03905)
Comments:
          Corresponding authors: zhizhenz@mit.edu (Z. Zhong), kaichen@cse.this http URL (K. Chen)

- **What's New**: 이번 논문에서는 Mixture-of-Expert (MoE) 모델의 분산 학습 과정 중에 토폴로지 재구성을 가능하게 하는 새로운 시스템인 mFabric을 제안합니다. MoE 모델은 각 토큰에 대해 선택적으로 다양한 전문가(Experts)를 활성화하여 동적 통신을 생성하지만, 기존의 GPU 상호 연결 구조는 이러한 동적 패턴을 처리하기 어렵습니다. mFabric은 이러한 한계를 극복하고, 효율적인 학습을 위해 고대역폭 도메인 아키텍처를 설계하였습니다.

- **Technical Details**: mFabric의 핵심은 밀리세컨드 단위의 광회로 스위칭(Optical Circuit Switching, OCS)을 사용하여 기존 전기 상호 연결 위에 구축된 고대역폭의 지역 재구성 가능한 도메인입니다. 이를 통해 mFabric은 MoE 레이어 내에서의 모든-to-모든(all-to-all) 전송의 반예측 가능성을 활용하여 트래픽 모니터를 구현하고, 요구량에 근거하여 재구성된 OCS 토폴로지를 생성하는 알고리즘을 적용합니다. 마지막으로, mFabric은 전기 상호 연결 및 OCS를 통해 이동 경로를 관리하는 맞춤형 집합 통신 관리자를 구현하였습니다.

- **Performance Highlights**: mFabric은 32개의 NVIDIA A100 GPU와 함께 구축된 프로토타입을 통해 세 가지 최첨단 MoE 모델을 성공적으로 학습하였습니다. 대규모 패킷 수준 시뮬레이션 결과, mFabric은 기존의 전기 구조와 비교할 때 유사한 훈련 속도를 유지하면서, 100Gbps 및 400Gbps 링크에서 1.5배에서 2.3배까지 비용 효율성을 향상시킵니다. 또한, mFabric은 TopoOpt 보다 2.5배 높은 성능을 보이며, 클러스터 크기를 30,000 GPU 이상까지 확장할 수 있는 Scalability를 갖추고 있습니다.
...
### Leveraging time and parameters for nonlinear model reduction methods (https://arxiv.org/abs/2501.03853)
- **What's New**: 이번 연구에서는 느리게 감소하는 Kolmogorov $n$-widths를 가진 문제들을 위한 모델 차원 축소 (model order reduction, MOR) 방법을 다룹니다. 특정 파동과 같은 문제를 해결하기 위해 비선형 투영 (nonlinear projections)을 적용하였으며, 이는 많은 경우 자동 인코더 (autoencoders)를 통해 수치적으로 실현됩니다. 이러한 접근법의 주요 혁신점은, 차원 축소된 시스템의 훈련 데이터 확장을 통해 비선형 인코더를 선형 인코더로 대체할 수 있음을 보여준 점입니다.

- **Technical Details**: 제안된 방법은 비선형 인코더와 비선형 디코더로 구성된 자동 인코더의 구조를 활용합니다. 하지만, 이 시스템에서 훈련해야 할 하이퍼파라미터 (hyperparameters)의 수가 많아 훈련 비용이 증가하는 문제가 있었습니다. 연구에서는 시스템을 확대하여 훈련 데이터를 생성함으로써 선형 인코더를 사용하였고, 이로 인해 하이퍼파라미터 수를 대폭 줄일 수 있었습니다.

- **Performance Highlights**: 제안된 방법은 정확도를 유지하면서도 하이퍼파라미터 수를 절반으로 줄이는 데 성공하였습니다. 따라서, 차원 축소가 필요한 문제에서 보다 효율적인 훈련을 가능하게 하여, 모델의 성능을 향상시키고 훈련 시간을 단축시킬 수 있는 가능성을 보여 줍니다. 이 접근법은 향후 다양한 MOR 문제에 적용될 수 있으며, 응용 가능성이 큽니다.
...
### Class-Balance Bias in Regularized Regression (https://arxiv.org/abs/2501.03821)
Comments:
          27 pages, 21 figures

- **What's New**: 이 논문은 다양한 정규화(normalization) 방법이 회귀모델에 미치는 효과를 연구하여, lasso, ridge 및 elastic net 회귀와 같은 정규화 방법이 데이터의 다른 특성과 어떻게 상호작용하는지를 보여줍니다. 특히 이 연구는 이진(i.e., binary) 특성이 불균형하게 나타날 때 정규화 선택이 회귀 계수에 미치는 영향을 강조합니다. 이러한 내용은 지난 연구에서 다루어지지 않았던 부분으로, 특정 정규화 방법의 선택이 모델 성능에 미치는 심각한 영향을 제시하는 첫 번째 시도로 평가될 수 있습니다.

- **Technical Details**: 본 논문에서는 특성 행렬을 정규화(normalize)하기 위한 다양한 기법들을 소개합니다. 정규화는 일반적으로 중심화(centering)와 스케일링(scaling) 과정을 포함하며, 이는 알고리즘의 성능을 개선하는 데 필수적입니다. 본 문서에서는 이진 및 일반 특성에 대해 각 정규화 방법이 회귀 계수에 미치는 직접적인 영향을 다루고, 이를 통해 정규화 과정의 다양한 측면을 조명합니다.

- **Performance Highlights**: 연구 결과, lasso 및 ridge 회귀는 이진 특성의 분산에 따라 스케일링을 적용함으로써 모델의 성능을 향상시킬 수 있음을 보여줍니다. 그러나 이러한 방법은 높은 분산을 유발할 수 있는 단점도 있습니다. Elastic net의 경우, 특성을 스케일링하는 대신 패널티 가중치를 조정하는 방법으로 유사한 효과를 얻을 수 있음을 발견했습니다. 이 논문은 이러한 접근법들이 혼합 데이터(mixed data) 케이스에서도 어떻게 적용될 수 있는지를 논의하며, 초기 결과도 포함하고 있습니다.
...
### Detecting Neurocognitive Disorders through Analyses of Topic Evolution and Cross-modal Consistency in Visual-Stimulated Narratives (https://arxiv.org/abs/2501.03727)
Comments:
          12 pages, 8 figures

- **What's New**: 이 논문은 뇌인지장애(NCD) 조기 탐지를 위한 새로운 접근법을 소개합니다. 기존의 언어 분석 방법은 주로 미세구조(microstructure)에 중점을 두었지만, 본 연구는 이야기의 전반적인 구조인 대구조(macrostructure)를 포괄적으로 분석합니다. 구체적으로, 동적 주제 모델(Dynamic Topic Models, DTM)과 텍스트-이미지 시계열 정렬 모델(Text-Image Temporal Alignment Model, TITAN)을 활용하여 NCD를 탐지하는 효율적인 방법론을 제시합니다.

- **Technical Details**: 연구는 CU-MARVEL Rabbit Story 코퍼스를 기반으로 758명의 노인의 스토리텔링 기록을 분석하였습니다. DTM 기반 접근법은 주제 일관성을 DTM 데이터셋을 통해 평가하며, TITAN 모델은 시각적 자극과 내러티브의 일관성을 평가합니다. DTM과 TITAN 모델은 각각 F1 점수 0.61 및 0.72, AUC 0.78 및 0.81로, 기존의 미세구조 및 대구조 피쳐 세트를 초과하는 성능을 기록했습니다.

- **Performance Highlights**: 티탄 모델은 NCD 탐지에 있어 가장 높은 성능을 달성하며, 동적 대구조 모델링 접근법이 NCD 탐지에 효과적임을 보여줍니다. 본 연구는 내러티브 불일치 식별을 통해 인지 장애를 드러내고, 언어 사용 및 의사소통 능력에 미치는 영향을 탐구하여 실질적인 초기 탐지 도구로서의 잠재력을 제시합니다. 다양한 실험을 통해 제안된 접근법의 유효성을 입증하며, 향후 연구의 방향성을 제안합니다.
...
### Run-and-tumble chemotaxis using reinforcement learning (https://arxiv.org/abs/2501.03687)
- **What's New**: 이 논문에서는 박테리아 화학주성(chemotaxis)의 현상에서 영감을 받아 강화 학습(reinforcement learning, RL) 알고리즘을 구현하였습니다. 에이전트는 다양한 형태의 화학 농도 구배가 있는 환경에서 일차원적으로 이동하며, 지속적인 방향으로의 운동 또는 방향 전환을 선택할 수 있습니다. 이 연구의 핵심 질문은 서로 다른 유형의 유인체(또는 자극물) 환경에서 어떤 RL 전략이 최선인지를 조사하는 것입니다.

- **Technical Details**: 연구 팀은 에이전트가 런-앤-텀블(run-and-tumble) 모션을 수행하는 RL 전략을 수립하였습니다. 에이전트는 최근의 궤적에 기반하여 각 액션에 대해 비용을 부여하며, 탐사(exploration)와 활용(exploitation) 사이의 균형을 최적화하여 성능을 극대화해야 합니다. 에이전트는 'Q-learning' 방법을 사용하여 자신의 경험을 바탕으로 최적의 행동을 학습하게 됩니다.

- **Performance Highlights**: 연구는 에이전트가 자극물 농도 피크에 얼마나 효과적으로 국소화(localize)할 수 있는지를 평가함으로써 RL 알고리즘의 효율성을 측정하였습니다. 두 가지 종류의 자극물 농도 프로필을 고려한 결과, 탐사 및 학습 파라미터의 최적 선택이 에이전트의 장기적인 행동에 큰 영향을 미친다는 것을 발견했습니다. 이 논문은 



### Imitation Learning of MPC with Neural Networks: Error Guarantees and Sparsification (https://arxiv.org/abs/2501.03671)
- **What's New**: 이 논문에서는 신경망(neural networks)을 활용한 모방 모델 예측 제어기(imitation model predictive controllers)의 근사 오차를 경계짓는 프레임워크를 제시합니다. Lipschitz 속성을 활용하여, 데이터셋 디자인을 안내하는 경계를 파생하고, 이를 통해 근사 오차를 선택한 한계 내에서 유지할 수 있도록 합니다. 안정적인 신경망 제어기를 설계하고 성능 보장을 제공하는 방법에 대해 논의하며, 기존의 강건 모델 예측 제어 방식과 데이터 생성을 결합합니다.

- **Technical Details**: 비선형 시스템을 다루며, 최적 제어 문제(optimal control problem)에서의 반복적인 솔루션 제공 을 통해 제어됩니다. 여기서는 입력에 대한 상태 전환을 정의하는 함수 f를 사용하여 시스템의 동작을 모델링합니다. 네트워크 학습에 있어 샘플 밀도를 줄이는 방식으로 네트워크 교육 조정을 도입하였고, 비선형 프로그램의 민감도(sensitivities)를 활용하여 데이터셋 증강(dataset augmentation)을 구현했습니다.

- **Performance Highlights**: 제안된 방식을 통해 예제에서는 강화된 예측 능력을 검증했으며, 구분된 폐쇄 루프(closed-loop) 행동이 원래의 모델 예측 제어기와 더욱 밀접하게 일치함을 보여주었습니다. 이 결과는 특히 안전이 중요한 응용 분야에서의 약간의 오차가 큰 영향을 미칠 수 있는 경우에 큰 의미가 있습니다. 실험에서는 실제 시뮬레이션 환경에서 개선된 성능을 확인하였습니다.



### Hybrid Machine Learning Model with a Constrained Action Space for Trajectory Prediction (https://arxiv.org/abs/2501.03666)
Comments:
          Submitted to 2025 IEEE Intelligent Vehicles Symposium (IV)

- **What's New**: 이 논문은 자율주행의 발전을 위한 필수 요소인 궤적 예측의 새로운 접근 방식을 제시합니다. 기존의 end-to-end deep learning 모델들은 차량의 동적 제한을 고려하지 않아 비현실적인 예측을 초래하곤 했습니다. 저자들은 깊은 학습(deep learning) 모델과 운동 모델(kinematic motion model)을 결합한 하이브리드 모델을 소개하여, 물리적으로 실행 가능한 궤적을 생성할 수 있도록 하였습니다.

- **Technical Details**: 제안된 모델은 두 가지 주요 구성 요소로 나뉘어 있습니다: 현대적인 깊은 학습 모델과 전통적인 운동 모델입니다. 깊은 학습 모델은 교통 참가자에 대한 시나리오 정보를 처리하여 중간 액션 스페이스를 생성합니다. 이때, 전문가 지식을 바탕으로 물리적 제약을 학습 목표에 포함시켜 액션 스페이스를 제한하고, 이후 운동 모델이 이러한 중간 액션 스페이스를 기반으로 고유한 궤적을 예측합니다.

- **Performance Highlights**: 실험 결과는 공공 데이터베이스인 Argoverse에서 수행되었으며, 다양한 벤치마크 비교와 ablation 연구를 통해 제안된 모델이 현실적인 주행 행동을 생성하는 데 성공적임을 입증했습니다. 이러한 결과는 안전성 및 강인성을 증가시키며, 딥러닝 방법의 신뢰성을 강화하는 데 기여합니다.
...
### Transfer Learning for Deep-Unfolded Combinatorial Optimization Solver with Quantum Annealer (https://arxiv.org/abs/2501.03518)
Comments:
          8 pages, 6 figures

- **What's New**: 이번 논문에서는 고전-양자 전이 학습(classical-quantum transfer learning) 전략을 통해 훈련 가능한 샘플링 기반 COP 솔버(trainable sampling-based COP solver)에서 양자 어닐링(Quantum Annealing, QA)을 활용하는 접근 방식을 제안했습니다. 기존의 샘플링 방식에서는 고전적 샘플러(classical sampler) 사용으로 인해 학습 비용이 발생하는 반면, 제안된 방법은 교훈을 통해 훈련된 파라미터를 QA에 적용하여 성능을 향상시킨다는 점에서 큰 개선점을 나타냅니다.

- **Technical Details**: 이 연구는 Ohzeki 방법을 기반으로 한 훈련 가능한 양자 COP 솔버를 개발하였으며, 그 과정에서 심층 전개(deep unfolding) 기법을 통한 문제 해결의 효율을 극대화하고자 했습니다. 또한 훈련 과정에서 고전적 메트로폴리스-헤이스팅스(Metropolis-Hastings, MH) 알고리즘을 활용하여 파라미터를 최적화하고, 그 이후에 양자 어닐링을 통한 솔빙을 수행하는 방법론이 소개되었습니다. 이러한 접근은 기존 알골리즘의 느린 수렴 속도 문제를 해결하면서도 훈련 비용을 최소한으로 줄이는 데 중점을 두었습니다.

- **Performance Highlights**: 수치 실험 결과, 제안한 고전-양자 전이 학습 전략을 통해 훈련 가능한 양자 COP 솔버가 기존 솔버보다 수렴 속도 및 실행 시간을 개선할 수 있음을 확인했습니다. 특히, 심층 전개 방식으로 조정된 OHzeki 방법(DUOM)의 성능이 확인되었으며, 이 방법이 양자 어닐링이 결합된 학습 전략에서도 더욱 신뢰할 수 있는 성과를 이끌어낼 수 있음을 나타냈습니다. 최종적으로, 이 연구는 권장된 학습 알고리즘이 더 큰 COP 문제에 대한 실행 가능성을 높인데 기여할 것을 기대하고 있습니다.
...
### Structure-Preference Enabled Graph Embedding Generation under Differential Privacy (https://arxiv.org/abs/2501.03451)
Comments:
          Accepted by ICDE 25

- **What's New**: 이번 논문에서는 SE-PrivGEmb라는 새로운 그래프 임베딩 생성 방법을 제안합니다. 이 방법은 차별적 개인 정보 보호(differential privacy) 환경에서 구조 선호(settings)을 지원하는 첫 번째 방법으로, 데이터 유틸리티를 실질적으로 보존할 수 있습니다. 고유의 노이즈 내성 기법을 도입하여 민감도를 감소시키고, 다양한 구조 선호를 만족시키는 능력을 갖추고 있습니다.

- **Technical Details**: 본 연구에서는 그래프 G=(V,E)와 노드 근접성을 정의하며, 이 관점에서 스킵-그램 모델(skip-gram)을 활용합니다. 기존 연구에서 노드의 차수와 관련된 구조적 특성만을 보존했던 것과 달리, 우리의 방법은 노드 간 관계를 이론적으로 증명하여 보다 강력한 해석력을 제공합니다. 특히, 고유의 노이즈 내성 기법을 통해 고감도에 따른 유틸리티 저하를 효과적으로 완화합니다.

- **Performance Highlights**: 다양한 대규모 네트워크(예: 사회적 네트워크, 인용 네트워크)에서 실시된 실험 결과, SE-PrivGEmb는 구조적 동등성(structural equivalence) 및 링크 예측(link prediction) 과제에서 기존의 최신 기술들을 초월하는 성능을 보여주었습니다. 본 논문은 이론적 분석과 더불어 실험적 검증을 통해 제안하는 방법의 우수성을 입증합니다.
...
- **What's New**: 이번 연구는 Benjamini-Hochberg (BH) 절차가 다중 검정에서 어떻게 작동하는지를 자세히 분석하며, 특히 이 방법의 적대적 강인성을 평가합니다. 불리한 조건 하에서도 BH 절차가 어떻게 실패할 수 있는지를 설명하고, 간단하고 쉽게 구현 가능한 적대적 테스트 방 perturbation 알고리즘을 제시하며, 이를 통한 계산 실험을 진행합니다. 이러한 연구는 안전 및 보안이 중요한 컨텍스트에서 BH의 FDR(거짓 발견율) 조절 기능이 얼마나 쉽게 깨질 수 있는지를 보여줍니다.

- **Technical Details**: 연구에서는 BH 절차를 ``balls into bins'' 과정으로 재구성하고, 정보 이론적 접근을 통해 비비대칭 하한(non-asymptotic lower bounds)을 도출하는 방법을 설명합니다. BH 절차는 독립적으로 생성된 p-value들에 대해 FDR을 사용자가 정의한 임계값 이하로 유지하는 알고리즘이지만, 이 절차는 적대적 상황에서는 강인성이 부족할 수 있음을 보여줍니다. 논문에서는 다양한 통계적 방법론의 취약성과 함께 대규모 컨텍스트에서의 발생하는 문제점을 다룹니다.

- **Performance Highlights**: 연구 결과에 따르면, BH 절차는 특정 조건에서 매우 소수의 테스트 점수 변동(하나의 경우에도)으로 사실상 무너질 수 있습니다. 적대적 조정에 대한 기대값에 대한 비비대칭 보장을 제공하며, 기존 FDR 조절 방안에 대한 신뢰성을 재고하게 만드는 중요한 발견을 합니다. 이러한 발견은 머신러닝 및 통계적 방법론의 안정성에 실질적인 영향을 미칠 수 있습니다.
...
### The Artificial Scientist -- in-transit Machine Learning of Plasma Simulations (https://arxiv.org/abs/2501.03383)
Comments:
          12 pages, 9 figures

- **What's New**: 이 논문에서는 HPC(고성능 컴퓨팅) 클러스터의 크기가 증가하고 페타바이트의 데이터를 생성하는 대규모 시뮬레이션으로 인한 데이터 저장 및 IO 문제를 해결하기 위한 새로운 스트리밍 워크플로우를 소개합니다. 이 워크플로우는 시뮬레이션 데이터를 파일 시스템을 우회하여 직접 머신러닝(ML) 프레임워크로 전송함으로써 기존의 제약을 극복합니다. 데이터는 전이 중에 비동기적으로 변환되어 모델 훈련에 활용되며, 사용자는 응용 프로그램 출력을 조정하지 않아도 자동으로 데이터 처리할 수 있습니다.

- **Technical Details**: 키워드 중 하나인 Kelvin-Helmholtz instability(KHI)의 입자-격자 시뮬레이션을 GPU 가속으로 수행합니다. 이 시뮬레이션 과정을 통해 비정상적인 프로세스에서 발생할 수 있는 catastrophic forgetting을 피하기 위해 experience replay 기법을 적용합니다. 연구에서는 Frontier exascale 시스템에 적합하게 포팅 및 스케일링할 때 직면하게 되는 다양한 도전 과제를 상세히 설명합니다.

- **Performance Highlights**: 종합적으로 데이터 전송 속도를 극대화하고 메모리 사용을 효율적으로 관리하는 방법을 보여줍니다. 관측된 방사선과 입자 역학 간의 상관관계를 파악하여 KHI와 관련된 입자 역학을 비지도 방식으로 식별합니다. 연구의 결과는 대규모 시뮬레이션과 복잡한 과학적 데이터 이해를 위한 새로운 솔루션 방향을 제시하는 데 가치가 있습니다.
...
### Data integrity vs. inference accuracy in large AIS datasets (https://arxiv.org/abs/2501.03358)
Comments:
          10 pages, 6 figures, conference: The International Conference on Business and Digital Technology (ICBDT2024)

- **What's New**: 본 논문은 자동 선박 식별 시스템(Automatic Ship Identification Systems, AIS)의 데이터 무결성(data integrity)이 대규모 AIS 데이터셋에서 분류 정확도(classification accuracy)에 미치는 영향을 분석합니다. 특히, AIS 시스템이 해상 안전(maritime safety) 및 환경 보호(environmental protection)에 필수적인 데이터를 제공하는 방법을 다룹니다. 이를 통해 데이터의 신뢰성(reliability)을 향상시키기 위한 몇 가지 오류 검출(error detection) 및 수정(correction) 방법이 소개됩니다.

- **Technical Details**: 분석에서는 AIS 데이터셋에서의 데이터 무결성을 보장하기 위한 다양한 기법이 포함되어 있습니다. 이러한 기법들은 데이터 검증(data verification) 기술을 포함하여, 데이터 오류를 찾아내고 수정하는 방법을 제공합니다. 논문에서 적용된 방법론은 운영 효율성(operational efficiency)을 높이는 데 기여할 수 있도록 설계되었습니다.

- **Performance Highlights**: AIS 데이터의 무결성을 향상시키는 결과는 추론의 품질(inference quality)을 상당히 개선함을 보여줍니다. 이 개선은 해상에서의 안전(safety)과 운영 효율성을 직접적으로 향상시키는 결과를 가져옵니다. 따라서, AIS 시스템의 신뢰성 강화를 위한 전략이 실제로 중요하다는 점이 강조됩니다.
...
- **What's New**: 이번 연구는 Spiking Neural Networks (SNNs)와 Federated Learning (FL)의 융합을 다룹니다. 특히, 비잔틴 공격(Byzantine attacks)과 대역폭 제한(bandwidth limitation)에 대한 연구가 부족한 상황에서 이 두 문제에 대한 해결책을 제시합니다. 저자들은 SNN의 에너지 효율성을 강조하며 FL-SNNs가 FL-ANNs 대비 더 뛰어난 통신 효율성을 보여준다는 점을 밝혔습니다.

- **Technical Details**: SNN은 생물학적 뉴런을 모방하여 에너지 효율적인 머신러닝 시스템을 제공합니다. 이 시스템은 event-driven 방식으로 작동하여 실시간 로컬 처리와 시간적 데이터 저장 기능을 포함하고 있습니다. FL-SNNs의 경우, Top-k sparsification을 통해 대역폭 사용을 줄이고 비잔틴 공격에 대한 내구성을 높일 수 있습니다. 연구자로서는 주로 비관측(non-omniscient) 비잔틴 적대자들에 대해 SNN의 강점을 실험했습니다.

- **Performance Highlights**: FL-SNNs 모델은 MinMax 공격 하에서 약 40%의 정확도 향상을 보여주었습니다. 이 결과는 FL-ANNs에 비해 FL-SNNs가 비잔틴 공격에 더 강하다는 것을 입증합니다. 또한, Top-k sparsification을 이용해 FL 시스템의 대역폭 제한을 극복하며, 통신 효율성을 향상시키는 것이 가능하다는 점을 강조합니다.
...
### LiLMaps: Learnable Implicit Language Maps (https://arxiv.org/abs/2501.03304)
- **What's New**: 이번 연구에서는 자연어 명령을 기반으로 환경을 탐색할 수 있도록 도와주는 Learnable implicit Language Maps (LiLMaps)를 제안합니다. LiLMaps는 환경 내 언어 표현을 지속적으로 배우며, 임의의 객체가 등장했을 때도 효과적으로 언어 지도를 최적화할 수 있는 기능을 제공합니다. 연구팀은 이 새로운 접근 방식을 통해 기존 SLAM (Simultaneous Localization and Mapping) 시스템과 통합하여 사용 가능한 기술적 향상을 목표로 하고 있습니다.

- **Technical Details**: LiLMaps는 언어 인코더를 통해 RGB 이미지를 기반으로 언어 기능을 추출하고, 적응형 언어 디코더 최적화 전략을 활용하여 기존 관찰된 기능을 유지하면서 새로운 언어 기능에 적응합니다. 또한, 불일치한 시각-언어 측정을 조정하기 위해 측정 업데이트 전략을 사용합니다. 이 과정은 옥트리 구조를 사용하여 공간 정보를 효율적으로 인코딩하고, 지속적인 환경 탐색 중에도 품질 높은 언어 지도를 제공할 수 있습니다.

- **Performance Highlights**: 실험 결과, LiLMaps는 다양한 관측이 점진적으로 발생할 때도 안정적으로 비전-언어 환경 탐색을 가능하게 하며, 성능 향상을 입증했습니다. 기존 시스템과 비교했을 때 LiLMaps는 다양한 환경에서 빠르고 유연하게 언어 기반 탐색을 수행할 수 있는 장점을 가지고 있습니다. 이 접근 방식은 특히 로봇의 자율성과 대화형 인간-로봇 상호 작용을 높이는 데 기여할 것입니다.
...
### DenseGNN: universal and scalable deeper graph neural networks for high-performance property prediction in crystals and molecules (https://arxiv.org/abs/2501.03278)
Comments:
...
- **What's New**: 이번 논문에서는 고급 온도 및 자재 예측을 위한 DenseGNN을 제안합니다. 이 모델은 Dense Connectivity Network (DCN), Hierarchical Node-Edge-Graph Residual Networks (HRN), 그리고 Local Structure Order Parameters Embedding (LOPE) 기술을 활용하여 기존의 Graph Neural Networks (GNNs)가 직면한 여러 문제를 해결합니다.

- **Technical Details**: DenseGNN은 GNN의 훈련 비용을 절감하고 도메인 적응 문제를 완화하며, 과도한 스무딩(over-smoothing) 문제를 해결합니다. 이 모델은 atomic embeddings(원자 임베딩)을 최적화하고 계산 비용을 줄임으로써 더 깊은 아키텍처를 가능하게 합니다.

- **Performance Highlights**: DenseGNN은 JARVIS-DFT, Materials Project, QM9와 같은 데이터셋에서 최첨단 성능을 달성했습니다. 이 모델은 GIN, Schnet, Hamnet과 같은 기존 모델들의 성능을 개선하여 결정 구조(distinction of crystal structures)에서 다른 GNN들을 초월하고 X-ray diffraction 방법의 정확도에 근접합니다.
...
### Efficacy of Full-Packet Encryption in Mitigating Protocol Detection for Evasive Virtual Private Networks (https://arxiv.org/abs/2412.17352)
Comments:
          6 pages, 3 figures, target conference undecided

- **What's New**: 이 논문에서는 최신의 회피형 가상 사설망(VPN) 프로토콜인 Aggressive Circumvention of Censorship (ACC)와 머신러닝 기반 분류 모델을 사용하여 전체 패킷 암호화(full-packet encryption) 방식을 구체적으로 분석하였습니다. ACC 프로토콜은 다양한 회피형 VPN의 전략을 융합하여 제작된 프로토콜로, 가장 일반적인 네트워크 트래픽과 구별되는 성능이 강조되었습니다. 하지만 ACC는 머신러닝 모델에 의해 비교적 쉽게 탐지될 수 있음이 밝혀졌습니다.

- **Technical Details**: 논문에서는 libpcap을 통해 수집된 약 400만 개의 패킷과 ACC 프로토콜을 기반으로 한 암호화된 패킷을 대상으로 스크립트를 사용하여 데이터를 처리하였습니다. ACC는 3단계 캡슐화 모델을 사용하여 네트워크 전송을 위해 패킷을 준비하며, 마지막 단계에서 GCM-SIV 모드의 AES로 패킷을 암호화합니다. 이후, 8개의 머신러닝 분류 알고리즘을 활용하여 ACC 패킷과 무작위 패킷 또는 일반 네트워크 트래픽 간의 구별 가능성을 평가하였습니다.

- **Performance Highlights**: 테스트 결과 ACC 프로토콜은 무작위 잡음과 비교할 때 머신러닝 모델에 의해 잘 탐지되지 않았지만, 일반 네트워크 트래픽의 스트림 내에서는 상대적으로 쉽게 탐지되었습니다. 이를 통해 ACC 프로토콜이 국가 차원의 검열자들이 사용하는 기존 기술에 저항력이 있다는 점은 인정되지만, 패킷 기반의 프로토콜 식별에는 여전히 취약함이 있는 것으로 나타났습니다. 이 연구는 미래에 FPE 모델을 향상시킬 수 있는 방향을 제시하고 있습니다.
...
- **What's New**: 본 논문에서는 M명의 클라이언트가 협력하여 차별적 프라이버시(differential privacy)를 보장하며 스토캐스틱 볼록 최적화(stochastic convex optimization)를 수행하는 문제를 다룹니다. 새로운 하한을 설정하고 Vaidya의 plane cutting 방법을 기반으로 한 새로운 알고리즘을 제시하여, DL(SCO)에서의 정확도, 통신 비용 및 프라이버시 간의 균형을 명확하게 이해합니다. 이 작업은 분산 환경에서의 DP-SCO의 특성을 완전히 파악하는 데 기여합니다.

- **Technical Details**: M명의 클라이언트는 각자의 지역 데이터 세트를 사용하여 볼록 함수의 손실을 최소화하는 알고리즘을 설계해야 하며, 이 과정에서 통신 비용과 데이터 프라이버시 간의 균형을 고려해야 합니다. 본 연구에서는 통신 비용을 측정할 때 전달되는 비트 수를 평균하여 알고리즘의 하한을 유도했습니다. 이는 분산 최적화 알고리즘의 상호작용이 줄어들면 정확도 및 프라이버시도 저하될 수 있음을 시사합니다.

- **Performance Highlights**: 알고리즘의 성능 하한인 Ω(d²MN⋅CC)의 도출은 분산 최적화 알고리즘의 통신 복잡성을 명확히 구분하는 데 중요한 기초 자료로 제공됩니다. 본 작업은 DP-SCO 알고리즘의 정확도는 Ω(d²MN⋅min{CC, dNεDP²}) 이상이어야 함을 제안하며, 이는 차별적 프라이버시를 충족하는 최적의 알고리즘의 설계에 중요한 통찰력을 제공합니다. 분산 DP-SCO의 정확도, 프라이버시 및 통신 비용 간의 균형을 이끌어내는 매력적인 패러다임을 제시합니다.



### Multimodal Machine Learning Can Predict Videoconference Fluidity and Enjoymen (https://arxiv.org/abs/2501.03190)
Comments:
          ICASSP 2025

- **What's New**: 이번 연구는 비디오 회의 중 부정적인 경험의 순간을 예측하기 위해 다중 모달 머신 러닝을 활용하였습니다. 특히, 자연스러운 상호작용을 모방하는 비디오 회의의 효과를 평가하여, 낮은 대화 유창성(fluidity)과 낮은 흥미(enjoyment) 및 대화 이벤트(이탈, 중단, 공백)를 구분할 수 있는 모델을 개발했습니다.

- **Technical Details**: 이 연구는 RoomReader 데이터세트에서 채집한 비디오 클립을 분석하였습니다. 오디오 임베딩(audio embeddings), 얼굴 행동(facial actions), 신체 움직임(body motion) 등의 기능을 모델 훈련에 사용하였으며, 최고의 모델은 비디오 회의 세션에서 ROC-AUC 0.87을 기록했습니다. 각 기능 도메인이 모델 성능에 미치는 기여도와 사전 이벤트 기능의 예측 능력 등이 주요 연구 질문으로 설정되었습니다.

- **Performance Highlights**: 연구 결과에 따르면, 다중 모달 오디오-비디오 신호가 주관적인 대화 결과를 효과적으로 예측할 수 있음이 입증되었습니다. 특히, 뒷채널(backchanneling) 이벤트가 가장 긍정적으로 평가된 반면, 중단(interruption)과 공백(gap)은 대화 유창성에 부정적인 영향을 미쳤습니다. 이러한 발견은 비디오 회의의 경험 개선 및 부정적 경험의 식별과 연구에 기여할 것입니다.



### Scalable Forward-Forward Algorithm (https://arxiv.org/abs/2501.03176)
- **What's New**: 본 논문에서는 backpropagation을 필요로 하지 않는 Scalable Forward-Forward (SFF) 알고리즘을 제안합니다. 이 알고리즘은 각 레이어를 독립적으로 학습시킬 수 있으며, 메모리 효율성이 높고 모듈성이 뛰어납니다. SFF는 MobileNetV3와 ResNet18 같은 현대적인 합성곱 신경망 아키텍처에 적용되며, convolutional layer에 대한 손실 계산 방식을 새롭게 도입했습니다.

- **Technical Details**: SFF는 Channel-wise Competitive Learning Forward-Forward 알고리즘(CwC)에 기반하여, 긍정적 및 부정적 데이터를 단일 전방 패스에서 처리하는 방식을 사용합니다. 이를 통해 레이어 간의 교차 업데이트 없이 각 레이어의 파라미터를 독립적으로 조정할 수 있으며, 깊은 CNN 구조에서도 효과적으로 적용할 수 있습니다. 또한, 새로운 손실 함수는 log-sum-exp 스무딩 단계를 도입하여 훈련 안정성을 향상시킵니다.

- **Performance Highlights**: 작은 데이터셋과 전이 학습에 대한 실험 결과, SFF가 전통적인 backpropagation보다 우수한 성능을 보였으며, 특히 데이터가 적은 상황에서 더욱 두드러진 성능을 발휘했습니다. 또한, 하이브리드 블록 방식의 FF와 Backpropagation을 결합하여 블록 내에서만 backpropagation을 적용함으로써, 전통적인 backpropagation 성능을 초과하는 결과를 도출했습니다.



### Deep-Relative-Trust-Based Diffusion for Decentralized Deep Learning (https://arxiv.org/abs/2501.03162)
- **What's New**: 본 논문에서는 분산 학습(decoupled learning) 전략을 통해 여러 에이전트가 중앙 집계 없이 지역 데이터에서 효율적으로 학습할 수 있는 방법을 제안합니다. 기존의 분산 학습 방법은 주로 매개변수(parameter) 공간의 합의를 유도하는 평균화(aggregating) 메커니즘에 의존하고 있습니다. 그러나 저자들은 딥 신경망(deep neural networks)에서는 출력(output)의 합의를 유도하는 것이 더 적합하다고 주장합니다. 이를 기반으로 "DRT diffusion"라는 새로운 알고리즘을 개발하였습니다.

- **Technical Details**: 본 연구에서 제안하는 DRT diffusion 알고리즘은 딥 상대 신뢰(deep relative trust)라는 유사성 측정을 활용하여 기존의 확산 알고리즘에 대해 페널티 기반의 변형을 적용합니다. 각 에이전트는 동일한 네트워크 아키텍처를 갖고 있으며 매개변수의 지역 모델을 유지하면서도 출력이 유사하도록 유도됩니다. 이 알고리즘의 수렴성을 이론적으로 분석하고, 희소(sparse) 토폴로지에서 이미지 분류 작업에서의 일반화 성능 향상에 대한 수치적 결과도 제공합니다.

- **Performance Highlights**: 실험 결과, DRT diffusion 알고리즘은 기존의 분산 최적화 방법들보다 더 나은 일반화 성능을 보였습니다. 이는 고차원 매개변수 공간에서 각 에이전트가 독립적으로 작업하더라도, 함수 공간(function space)에서의 합의가 달성되기 때문입니다. 또한 네트워크가 과잉 매개화(over-parameterized) 상태에서도 효과적인 학습이 가능해졌음을 보여줍니다. 이로 인해, 분산 학습 분야에서의 연구가 더욱 발전할 것으로 기대됩니다.
...
### Communication Bounds for the Distributed Experts Problem (https://arxiv.org/abs/2501.03132)
Comments:
          38th Conference on Neural Information Processing Systems (NeurIPS 2024)

- **What's New**: 이번 연구에서는 전문가 문제(expert problem)를 분산 설정(distributed setting)에서 다루고 있습니다. 특히, 여러 서버(server)에서 전문가의 비용(cost)을 집계해야 하는 상황을 고려하고 있습니다. 다양한 통신 모델(communication models)인 메시지 전달 모델(message-passing model)과 방송 모델(broadcast model)뿐만 아니라, 전문가 비용을 합산(summing)하거나 $	ext{ℓ}_p$ 노름($	ext{ℓ}_p$ norm)으로 취하는 여러 집계 함수(aggregation functions)를 분석합니다.

- **Technical Details**: 연구에서 제안한 프로토콜(protocol)은 통신 효율성(communication efficiency)을 고려하여, 강력한 적대자(adversary)가 입력을 적응적으로 선택하더라도 거의 최적의 후회(regret)를 달성합니다. 또한, 조건부 하한(conditional lower bound)을 제시하여 이러한 프로토콜의 통신량이 거의 최적임을 보여줍니다. 우리 연구는 프로그램 구현과 실험을 통해 제안한 프로토콜의 유효성을 입증하고 있습니다.

- **Performance Highlights**: HPO-B 벤치마크(HPO-B benchmarks)에서 실험적인 성과(empirical savings)를 확인하며, 제안한 프로토콜이 실제 적용에서 효과적임을 보여주었습니다. 또한, 이 프로토콜들이 수행하는 작업의 질과 효율성은 다른 접근 방식에 비해 우수합니다.



### Learning DAGs and Root Causes from Time-Series Data (https://arxiv.org/abs/2501.03130)
Comments:
          25 pages, 9 figures, conference preprint

- **What's New**: 본 논문에서는 시간 시계열 데이터에서 소수의 근본 원인(root causes)으로부터 유도된 비순환 방향 그래프(directed acyclic graphs, DAGs)를 학습하는 새로운 방법인 DAG-TFRC를 소개합니다. 특히, 시간 지연된 의존성과 즉각적인 의존성을 모두 표현하는 DAG를 학습하는 데 중점을 두며, 근본 원인의 위치와 시간을 발견하는 것도 목표로 합니다. 실험 결과, DAG-TFRC는 이전 연구들보다 뛰어난 정확도와 실행 시간 성능을 보이며, 수천 개의 노드로 스케일링 가능합니다.

- **Technical Details**: 시간 시계열 데이터를 다루기 위해 구조 벡터 자기회귀 모델(structural vector autoregression, SVAR)을 사용합니다. SVAR는 즉각적인 의존성과 지연된 의존성을 포함하는 선형 관계를 사용하여 노드 간의 상관관계를 모델링합니다. 본 연구는 SVAR의 몇 가지 근본 원인 가정 하에 시간 시계열 데이터를 대상으로 가중치가 부여된 윈도우 그래프를 학습하며, 이는 과거 데이터로부터 현재 데이터로 어떻게 유도되는지를 명확히 합니다.

- **Performance Highlights**: DAG-TFRC는 합성 데이터에서 근본 원인이 소수인 경우 쓰리리즘의 성능이 우수하며, 주식 시장의 실제 데이터를 사용하여 근본 원인 가정의 타당성을 보여줍니다. 특히 S&P 500 데이터 분석을 통해 주식들을 섹터별로 클러스터링하고, 주요 주가 변동을 근본 원인으로 성공적으로 식별하였습니다. 이러한 결과는 DAG-TFRC가 경제 및 금융 데이터 분석에 효과적으로 활용될 수 있음을 입증합니다.
...
### Balancing Efficiency and Expressiveness: Subgraph GNNs with Walk-Based Centrality (https://arxiv.org/abs/2501.03113)
Comments:
          33 pages, 8 figures

- **What's New**: 이번 논문에서는 서브그래프 GNN(Subgraph GNNs)과 구조적 인코딩(Structural Encodings, SEs)의 두 가지 주요 GNN 확장을 결합한 효율적이고 표현력이 뛰어난 접근 방식을 제안합니다. 이 접근 방식은 걸음 기반 중심성을 활용하여 SE의 강력한 형태로 작용하며 동시에 서브그래프 선택 전략으로 활용됩니다. 중심성 기반 샘플링의 효과를 강조하며, 이를 통해 서브그래프 GNN의 계산 부담을 크게 줄일 수 있음을 보여줍니다.

- **Technical Details**: 연구에서 제안하는 방법인 HyMN(Hybrid Marking Network)은 서브그래프 GNN과 구조적 인코딩이 협력하여 MPNN의 표현력 한계를 극복하고 서브그래프 GNN을 더 큰 그래프에 적용할 수 있도록 합니다. 특히 걸음 기반의 노드 중심성 지표를 사용하여 서브그래프의 중요성을 평가하고, 이 지표를 이용해 선택된 서브그래프의 노드 기능을 증강합니다. 이 과정에서 우리가 제안하는 구조적 중심성(Centrality-based Structural Encodings, CSEs)은 모델의 표현력을 향상시키는데 기여합니다.

- **Performance Highlights**: 실험 결과, HyMN은 기존 서브그래프 샘플링 접근 방식보다 우수한 성능을 보여주며, 전체 서브그래프 GNN과 비교하여도 경쟁력을 갖춥니다. 논문은 HyMN이 다른 최신 GNN 모델보다 낮은 계산 비용과 더 빠른 실행 시간을 가지고 있다고 주장합니다. 편리하게 서브그래프를 샘플링함으로써, 하이브리드 모델은 합리적이며 실용적인 성능을 발휘함을 실험적으로 증명했습니다.



### Qinco2: Vector Compression and Search with Improved Implicit Neural Codebooks (https://arxiv.org/abs/2501.03078)
- **What's New**: 이번 논문에서는 QINCo2라는 새로운 방법론을 통해 이전의 QINCo를 확장하고 개선하였습니다. 특히, 코드워드 사전 선택과 빔 검색을 활용한 벡터 인코딩 개선, 코드워드 쌍을 활용한 빠른 근사 디코더 도입, 그리고 훈련 절차 및 네트워크 아키텍처 최적화가 포함됩니다. 이러한 접근 방식은 기존 방법들보다 더 낮은 왜곡(distorion)으로 높은 성과를 달성할 수 있도록 돕습니다.

- **Technical Details**: QINCo2는 여러 코드북을 사용하는 다중 코드북 양자화(multi-codebook quantization) 기술을 발전시킵니다. 벡터 인코딩 과정에서 코드워드 사전 선택을 통해 기능 평가 수를 줄여 RQ 중심을 업데이트하며, 빔 검색을 통해 양자화 오류를 줄입니다. 또한, 코드 간의 의존성을 고려하여 각 단계에서 코드 쌍을 사용하는 근사 디코더를 통해 대규모 검색 시 필요한 단기 목록을 설정합니다.

- **Performance Highlights**: QINCo2는 BigANN 데이터셋에서 16바이트 벡터 압축의 MSE(Mean Squared Error)에서 34%를 개선하여 0.32×10⁻⁴에서 0.18×10⁻⁴로 감소시켰습니다. 또한, Deep1M 데이터셋에서 8바이트 압축의 최근접 이웃 검색 정확도를 24% 향상시켜 36.3%에서 45.1%로 증가시켰습니다. 이러한 결과는 QINCo2의 우수한 성능을 입증합니다.
...
### Probably Correct Optimal Stable Matching for Two-Sided Markets Under Uncertainty (https://arxiv.org/abs/2501.03018)
Comments:
          This paper was accepted to International Conference on Autonomous Agents and Multiagent Systems (AAMAS 2025)

- **What's New**: 이 논문은 안정적인 결혼 모델의 학습 문제를 다루며, 시장의 왼쪽에 대한 선호가 알려지지 않은 경우를 중점적으로 분석합니다. 특히, 제안된 알고리즘을 통해 최적의 안정 매칭을 빠르게 식별하기 위한 탐색(exploration) 문제로 접근하고 있습니다. 이를 통해 온라인 플랫폼이 에이전트들을 매칭하고 얻은 노이즈가 포함된 피드백을 사용하여 선호 정보를 효율적으로 수집하는 방법에 대한 기초적인 이해를 제공합니다.

- **Technical Details**: 이 연구에서는 안정적 결혼 모델의 중앙 집중화된 버전을 다루고, 에이전트의 불확실한 선호에 대한 최적의 안정 매칭을 효율적으로 찾는 방법에 중점을 두었습니다. 저자들은 Probably Correct Optimal Stable Matchings(PAC)라는 개념을 도입하고, 이를 도출하기 위한 여러 밴디트 알고리즘을 제안합니다. 또한, 균일 샘플링, 행동 제거 알고리즘, 적응 샘플링과 같은 다양한 전략을 통해 알고리즘의 성능을 향상시키는 방법을 탐구합니다.

- **Performance Highlights**: 제안된 알고리즘들은 여러 합성 데이터 세트를 대상으로 실험 분석을 수행하여 최적의 안정 매칭 식별 성능을 평가합니다. 특히, 균일 샘플링 알고리즘은 높은 확률로 최적의 안정 매칭을 생성할 수 있음을 보였으며, 샘플 복잡도에 대한 경계를 제공합니다. 이러한 기법들은 불확실성이 존재하는 양면 시장에서 더 나은 매칭을 제공하는 데 중요한 기여를 할 것으로 기대됩니다.



### Convexity in ReLU Neural Networks: beyond ICNNs? (https://arxiv.org/abs/2501.03017)
- **What's New**: 이 논문은 Convex Neural Networks의 표현력을 탐구하는 데 중점을 두고 있습니다. 특히, ReLU 신경망의 국소 볼록성(Local Convexity)을 판별할 수 있는 충분하고 필요한 조건을 제시합니다. 첫 번째로, 1-hidden-layer 전 구조에 대해서는 ICNNs가 모든 볼록 함수에 대한 최적 솔루션임을 증명하고, 2-hidden-layer 이상의 경우, ICNNs 조건을 만족하지 않는 새로운 네트워크를 소개합니다.

- **Technical Details**: 이 논문에서는 ReLU 신경망을 활용하여 연속 조각별 선형 함수(Continuous PieceWise Linear, CPWL)를 구현하고 이를 통해 볼록 함수의 표현이 가능하다는 점을 강조합니다. 특히, 다양한 신경망 아키텍처에 대해 볼록성의 수학적 특성을 정량화하여 ReLU 신경망의 계층적 구성이 볼록성을 유지하는 방법을 제안합니다. 이러한 처리 방식을 통해, 현대적 기능을 포함한 심층 신경망 아키텍처에서도 볼록성을 판별하는 이론적 조건을 수립합니다.

- **Performance Highlights**: 최고의 결과를 보여주는 ICNN의 사용에도 불구하고, 이 연구는 ReLU 신경망이 더 나은 표현력을 발휘할 수 있도록 받아들여지지 않는 구조적 제약이 있음을 강조합니다. 특히, 새로운 프로시저를 통해 많은 수의 아핀 영역을 가진 ReLU 신경망의 볼록성을 정확하게 검증할 수 있습니다. 이러한 방법을 통해 볼록 함수의 다양한 특성을 실험적으로 검증하고, 성능 향상을 위한 기틀을 마련하고 있습니다.
...
### LOHA: Direct Graph Spectral Contrastive Learning Between Low-pass and High-pass Views (https://arxiv.org/abs/2501.02969)
Comments:
          Accepted at AAAI2025

- **What's New**: 본 논문에서는 다양성 속 조화를 수용하여 저역 통과 필터(‘low-pass filter’)와 고역 통과 필터(‘high-pass filter’)의 두 개의 뚜렷한 시각을 최적화하여 효과적인 자가 지도 대조 프레임워크(‘self-supervised contrastive framework’)인 LOHA를 제안합니다. LOHA는 동일한 노드를 위한 공통 특징을 탐색하지 못했던 기존 방법들과 달리, 두 필터가 제공하는 뚜렷한 특성을 활용해 그래프 특성 일관성을 강화합니다. 저역 통과 시각과 고역 통과 시각에서 전파 신호를 복합 특성으로 고려하여 노드를 보다 잘 식별할 수 있도록 합니다.

- **Technical Details**: 그래프 데이터는 𝒢=(𝒱,ℰ)로 정의되며, 여기서 𝒱는 노드 집합이고 ℰ는 노드 간의 간선 집합을 나타냅니다. 각 노드에 대한 특성 행렬 𝐗∈ℝN×F와 인접 행렬 𝐀∈ℝN×N는 네트워크 구조의 대칭성을 포함합니다. 특히, LOHA는 스펙트럼 신호 경향(‘spectral signal trend’)이라는 새로운 고차원 특성을 제안하여, 이는 변화하는 필터에 영향을 덜 받으면서 원래 특성 차이에 집중합니다. 이는 대조 학습의 기반으로 활용되며, 노드의 특징 일관성과 인식을 개선하는 데 기여합니다.

- **Performance Highlights**: LOHA는 9개의 실제 데이터셋에서 평균 2.8%의 성능 향상을 달성하며, 일부 이질적 그래프 데이터셋에서는 완전 감독 모델보다도 우수한 성능을 보였습니다. 이 연구 결과는 LOHA가 다양한 그래프 구조에서 스펙트럴 그래프 신경망(‘spectral GNNs’)의 효율성을 향상시키는 잠재력을 강조합니다. 또한, 다양한 실험과 세밀한 배제 연구(ablation study)를 통해 이론적 발견의 효과성을 입증하였습니다.



### MSA-CNN: A Lightweight Multi-Scale CNN with Attention for Sleep Stage Classification (https://arxiv.org/abs/2501.02949)
Comments:
          10 pages, 6 figures, journal paper

- **What's New**: 이번 연구는 자동 수면 단계 분류 분야에서 기계 학습 기반 신호 분석의 최신 발전을 다루고 있습니다. 특히, Multi-Scale and Attention Convolutional Neural Network (MSA-CNN) 라는 경량 아키텍처를 소개하여, 약 10,000개의 매개변수(parameter)를 가지고도 효과적인 성능을 발휘할 수 있음을 보여줍니다.

- **Technical Details**: MSA-CNN은 보완적 풀링(complementary pooling)을 사용하여 중복 필터 매개변수(redundant filter parameters) 및 밀집 합성곱(dense convolutions)을 제거하는 새로운 다중 스케일 모듈을 활용합니다. 또한, 시간적(feature extraction) 및 공간적(feature extraction) 특성 추출을 분리하고, 비용 효율적인 글로벌 공간 합성곱(global spatial convolutions)을 사용하여 모델 복잡성을 추가로 줄였습니다.

- **Performance Highlights**: 큰 MSA-CNN 모델은 세 가지 공공 데이터셋에서 아홉 개의 최신 기준 모델(baseline models)을 상대로 정확도 및 Cohen's kappa 측면에서 모든 데이터셋에서 우수한 성능을 기록했습니다. 이번 평가에서는 반복 교차 검증 및 기준 모델의 재평가를 통해, 매개변수 수가 현저히 적음에도 불구하고 우수한 성능을 입증하였습니다.
...
- **What's New**: 이 논문은 TabPFN의 새로운 회귀 변형 모델인 TabPFN-TS를 소개합니다. 이 모델은 최소한의 파인튜닝으로 시계열 예측을 수행하는 능력이 있습니다. TabPFN-TS는 단순한 특징 엔지니어링과 결합하여 강력한 예측 성능을 달성하는 간단한 접근 방식을 제안합니다.

- **Technical Details**: TabPFN-TS는 1100만 개의 매개변수를 사용하며, 유사한 크기의 모델인 Chronos-Mini를 초과하는 성능을 보여줍니다. 또한, 6500만 개의 매개변수를 가진 Chronos-Large와 대등하거나 약간 우수한 성능을 기록합니다. 이 방법론의 주요 강점은 대규모 학습 데이터셋 없이 인공 데이터만을 기반으로 사전 학습(pre-training) 한다는 점입니다.

- **Performance Highlights**: TabPFN-TS는 탁월한 예측 성능을 발휘하며, 기존의 모델보다 간단하고 효율적입니다. 특히 데이터셋 오염의 위험 없이도 이전 모델들과 비교하여 뛰어난 성능을 입증했습니다. 이러한 혁신은 다양한 예측 작업에 대한 새로운 가능성을 열어줍니다.



### Self-Attention as a Parametric Endofunctor: A Categorical Framework for Transformer Architectures (https://arxiv.org/abs/2501.02931)
- **What's New**: 이번 연구는 Self-Attention 메커니즘의 수학적 기초를 정리하고, 이러한 메커니즘이 카테고리 대수 (categorical algebra)를 통해 형식화될 수 있음을 보여줍니다. 연구자들은 Self-Attention의 선형 구성 요소를 대상으로 파라메트릭 엔도펑터 (parametric endofunctor)로 정의하여, query, key, value맵이 단일 카테고리 프레임워크로 통합될 수 있음을 명시합니다. 이를 통해 Self-Attention의 여러 레이어가 이 엔도펑터 위에 자유 모나드 (free monad)가 구성되는 것과 연결된다는 것을 증명하였습니다.

- **Technical Details**: 연구에서는 Self-Attention의 선형 부분을 $	extbf{Para}(	extbf{Vect})$의 파라메트릭 엔도펑터로 형식화 하였습니다. 적절한 2-카테고리 설정을 통해 다층 Self-Attention의 구조를 해석할 수 있으며, 위치 인코딩 (positional encodings)은 임베딩 공간에서 모노이드 작용 (monoid actions)으로 다루어 지고 있습니다. 연구자들은 표준적인 사인 곡선 인코딩이 비가산적이지만, 여전히 위치 보존 함수 (position-preserving functors)에서의 보편적 속성을 만족함을 보여주었습니다.

- **Performance Highlights**: Self-Attention의 선형 부분은 입력 토큰의 순열에 대한 자연적인 동등성 (equivariance) 속성을 보여주며, 그룹 작용 (group actions)과 연결시켜 네트워크의 구성을 해석합니다. 연구는 매커니즘 해석 가능성과 형식적 카테고리 구조를 연결하는 튼튼한 다리를 제공하여, Self-Attention 메커니즘의 기초에 대한 새로운 수학적 도구를 제시합니다. 이러한 접근은 다양한 심층 학습 이론의 패러다임을 통합할 수 있는 기초를 마련하고 있으며, 향후의 연구와 카테고리 이론 방법의 광범위한 수용을 촉진할 것으로 기대됩니다.
...
- **What's New**: 이 논문에서는 스토캐스틱 밴딧( stochastic bandit) 문제에서 하이퍼파라미터( hyperparameter)를 효과적으로 설정하기 위한 데이터 기반 접근 방식을 제안합니다. 기존의 알고리즘은 하이퍼파라미터 튜닝의 어려움 때문에 효율적인 성능을 발휘하지 못하는 경우가 많습니다. 특히, 저자들은 여러 밴딧 문제에서 수집된 오프라인 데이터를 활용하여 새로운 작업의 하이퍼파라미터를 설정하는 방법을 연구하고 있습니다. 본 연구는 이론적 한계와 실제 문제 해결을 조화롭게 접근하고자 합니다.

- **Technical Details**: 연구팀은 스토캐스틱 밴딧 문제의 하이퍼파라미터에 대한 효과적인 튜닝을 위해 두 가지 중요 요소를 고려합니다: 인터-태스크 샘플 복잡도( inter-task sample complexity)와 인트라-태스크 샘플 복잡도( intra-task sample complexity). 이들은 각각 과거 데이터에서 필요한 문제 인스턴스의 수와 각 작업에서 수집된 데이터를 의미합니다. 저자들은 유도된 이론적 경계를 바탕으로 기존의 여러 알고리즘에서 하이퍼파라미터의 효과성을 검증하며, 이전 하이퍼파라미터를 새로운 문제에 효과적으로 전이하는 방법을 제시합니다.

- **Performance Highlights**: 실험 결과, 오프라인 문제에서의 하이퍼파라미터 지식을 온라인 학습에서 효과적으로 전이할 수 있음을 보여줍니다. 또한, 기존의 UCB 및 LinUCB 알고리즘에서의 하이퍼파라미터 조정에 대한 성능이 향상될 뿐만 아니라, GP-UCB 알고리즘에서도 유용한 결과를 도출했습니다. 이 연구는 하이퍼파라미터 튜닝 문제에서 발생할 수 있는 정보이론적 한계를 극복할 수 있는 새로운 방안을 제시하며, 향후 스토캐스틱 밴딧 최적화의 방향성을 제시합니다.
...
### Conditional Mutual Information Based Diffusion Posterior Sampling for Solving Inverse Problems (https://arxiv.org/abs/2501.02880)
- **What's New**: 본 논문에서는 정보 이론적 접근법을 통해 확산 모델(diffusion models, DMs)의 역문제(inverse problems) 해결 능력을 향상시키는 방법을 제시합니다. 특히, 조건부 상호 정보(conditional mutual information, CMI)를 극대화함으로써, 생성된 중간 신호가 최종 복원 신호가 측정값을 최대한 많이 보존하도록 보장합니다. 이 접근법은 최근 다른 방법들과 통합될 수 있으며, 성능을 질적으로나 양적으로 향상시킬 수 있음을 증명합니다.

- **Technical Details**: 확산 모델은 랜덤 노이즈를 데이터로 점진적으로 변환하는 생성적 프레임워크입니다. 본 연구는 이러한 모델이 선형 역문제 해결에 활용될 수 있음을 보여줍니다. 기계학습을 통해 추정된 확률 분포의 로그 밀도(log-density) 기울기와 측정값에 대한 가능도(posterior) 기울기를 결합하여 역문제에 대한 샘플링을 가능하게 합니다. 그러나 가능도는 분석적으로 다루기 어려워 근사를 통해 접근해야 하고, 이로 인해 생성되는 이미지 품질이 저하될 수 있다는 문제를 해결하고자 합니다.

- **Performance Highlights**: 제안된 방법은 정보 이론적 접근법을 통해 각 역단계에서 조건부 상호 정보를 극대화하여 생성되는 중간 신호가 최종 복원 신호의 품질을 높입니다. 이 방식은 최근의 다른 접근 방법들과 쉽게 통합될 수 있으며, 이를 통해 이미지 생성의 질적, 양적 성능이 개선됩니다. 따라서 본 연구는 역문제 해결을 위한 DMs의 효과성을 강조하며, 다양한 분야에 적용할 수 있는 가능성을 제시합니다.
...
### Randomly Sampled Language Reasoning Problems Reveal Limits of LLMs (https://arxiv.org/abs/2501.02825)
Comments:
          8 pages, 3 figures, 2 tables

- **What's New**: 이번 연구에서는 LLM(대형 언어 모델)이 새로운 언어 구조를 이해할 수 있는지를 평가하는 기준을 제시합니다. 연구팀은 기존의 데이터 세트에서 보지 못한 완전히 새로운 언어를 사용하여 모델의 언어 이해 능력을 측정하려고 합니다. 결과적으로, 모델들은 기본적인 n-그램 모델보다 언어 인식 및 합성 작업에서 부족한 성능을 보였습니다.

- **Technical Details**: 연구는 정해진 유한 오토마타(Deterministic Finite Automata, DFA)에 의해 인식되는 언어 집합을 구성하여 이를 통해 모델을 평가합니다. 3 상태 DFA와 같은 간단한 언어 작업을 사용하여 LLM의 언어 패턴 인식 능력을 검증하였습니다. 이러한 방식으로 모델이 언어 구조를 일반화하는 능력을 파악하고자 합니다.

- **Performance Highlights**: 대형 언어 모델은 알려진 언어의 구조를 인식하고 reasoning 할 수 있는 능력이 제한적임을 발견했습니다. LLM은 기대 이하의 성능을 나타냈으며, 특히 훈련 데이터와 충분히 구별되는 언어에서 더 큰 어려움을 보였습니다. 실험 결과는 LLM이 새로운 언어 문제에 대해 일반화할 수 없음을 보여줍니다.



### DarkFarseer: Inductive Spatio-temporal Kriging via Hidden Style Enhancement and Sparsity-Noise Mitigation (https://arxiv.org/abs/2501.02808)
Comments:
          TKDE (Under Review)

- **What's New**: 이번 논문에서는 Inductive Spatio-Temporal Kriging (ISK) 방법의 한계를 극복하기 위해 DarkFarseer라는 새로운 프레임워크를 제안합니다. 여기에는 가상 센서에 대한 표현을 향상시키기 위한 Neighbor Hidden Style Enhancement, 가상 노드의 패턴과 지역 패턴 간의 연관성을 구축하는 Virtual-Component Contrastive Learning, 그리고 노이즈 연결을 줄여주는 Similarity-Based Graph Denoising Strategy가 포함됩니다. 이는 기존 ISK 방법들과 비교해 훨씬 더 뛰어난 성능을 보입니다.

- **Technical Details**: DarkFarseer의 주요 구성 요소인 Neighbor Hidden Style Enhancement(NHSE) 모듈은 가상 노드의 표현을 강화하기 위해 시간적 특성과 공간적 관계를 별도로 처리하는 방식을 사용합니다. 또한, Virtual-Component Contrastive Learning(VCCL)은 희소한 공간적 관계의 문제를 해결하기 위해 지역 정보를 활용하며, Similarity-Based Graph Denoising Strategy(SGDS)는 이웃 노드와 가상 노드 간의 관계에서 노이즈를 줄이는 데 초점을 맞춥니다. 이러한 기술들은 ISK에서 가상 노드의 표현을 극대화하는 데 기여합니다.

- **Performance Highlights**: 대규모 실험을 통해 DarkFarseer는 기존 ISK 방법들보다 현저하게 우수한 성능을 발휘하고 있음을 입증했습니다. 특히, 다양한 데이터셋에서 이전의 최첨단 성능(SOTA)을 넘어서는 결과를 보여주며, 실험을 통해 제안된 기법들이 효과적임을 강조합니다. 이러한 결과들은 가상 센서 네트워크에서의 활용 가능성을 더욱 넓혀줍니다.



### From Dense to Sparse: Event Response for Enhanced Residential Load Forecasting (https://arxiv.org/abs/2501.02781)
Comments:
          12 pages and 6 figures. Accepted for publication by IEEE Transactions on Instrumentation and Measurement

- **What's New**: 최근 연구는 가전제품 간의 이벤트 관련 연관성을 활용하여 주거용 전력 수요 예측을 개선하는 새로운 접근법인 ERKG(Event-Response Knowledge Guided)를 제안합니다. 이 방법은 다양한 가전제품의 전력 사용 이벤트를 추정하여, 주거용 전력 소비 행동을 보다 정확하게 모델링합니다. 기존의 로드 전력 예측 모델의 성능을 향상시키기 위해, ERKG는 이벤트 반응 정보를 활용하여 학습의 집중도를 높입니다.

- **Technical Details**: ERKG는 지식 추출 및 안내 기능으로 구성되며, 가전제품의 작동 상태를 기반으로 이벤트를 예측하는 다변량 상태 예측기를 제안합니다. 이 모델은 이벤트 관련 희소 지식을 학습하기 위해 가전제품의 상태 변화를 구체적으로 모델링하며, RLF 모델에 이벤트 반응 기반 지침을 융합하여 전력 소비 패턴 학습을 개선합니다. ERKG는 기존 모델의 구조와 추론 과정을 변경하지 않고도 플러그인 모듈로 유연하게 작동할 수 있습니다.

- **Performance Highlights**: 세 가지 데이터 세트에서의 실험 결과, ERKG는 기존의 최첨단 전력 소비 예측 모델에 비해 평균 절대 오차(MAE)를 8% 이상 개선하는 효과를 보였습니다. 이러한 성과는 ERKG가 기존 모델의 성능을 실질적으로 향상시킬 수 있다는 것을 보여줍니다. 연구 결과는 RLF 분야에서의 효율성과 정확성을 높이는 데 기여할 것으로 기대됩니다.



### Learn A Flexible Exploration Model for Parameterized Action Markov Decision Processes (https://arxiv.org/abs/2501.02774)
- **What's New**: 이 논문에서는 Hybrid Action Model을 통해 강화 학습의 효율성을 높이기 위해 새로운 모델 기반 강화 학습(MBRL) 알고리즘인 FLEXplore를 제안합니다. FLEXplore는 Parameterized Action Markov Decision Processes (PAMDPs) 하에서 작동하며, 기존의 MBRL 알고리즘의 한계를 극복하기 위해 다이나믹스 손실 함수 및 보상 스무딩 과정을 특별히 설계하였습니다. 또한, 상태와 하이브리드 액션 간의 상호 정보를 최대화하여 탐색 효율성을 향상시키는 방식에 주목합니다.

- **Technical Details**: FLEXplore는 데이터 수집의 비효율성을 극복하기 위해 파라미터화된 액션을 조건으로 하는 동역학 모델을 학습합니다. 이 모델은 수정된 Model Predictive Path Integral 제어 방식에 따라 운영되며, Wasserstein Metric을 활용하여 동역학과 환경 간의 관계를 명확히 합니다. 특히, FIXplore는 다양한 하이브리드 액션 공간에서 손실 함수를 기반으로 유연성을 갖춘 동역학 모델을 학습하여 탐색 성능을 개선합니다.

- **Performance Highlights**: 실험 결과 FLEXplore는 다양한 표준 벤치마크에서 탁월한 학습 효율성과 비대칭적인 성능을 보였으며, 특히 이전의 모델들에 비해 샘플 효율성이 우수하다는 것을 입증하였습니다. FLEXplore는 회피 과정을 통해 낮은 보상 회피 위험을 줄이면서 효과적인 상태 학습을 가능하게 하여, 롤아웃 궤적의 후회 값을 감소시키는 성과를 보였습니다.
...
- **What's New**: 이 논문은 Transformer 기반의 시계열 예측(Time Series Forecasting) 모델의 성능을 향상시키기 위한 새로운 주목(attention) 메커니즘인 Sequence Complementor를 제안합니다. 시계열 데이터에서 표현의 다양성과 표현력을 높이는 것이 모델 성능에 결정적이라는 연구 결과를 바탕으로, 고유한 정보 이론적 보장을 통해 이 메커니즘의 타당성을 입증하였습니다.

- **Technical Details**: Sequence Complementor는 입력 시퀀스에 학습 가능한 보완 시퀀스를 추가하여, 자기 주의를 통해 입력에서 추가 정보를 찾는 데 도움을 줍니다. 이를 통해, 학습된 시퀀스 표현의 엔트로피와 평균 제곱 오차(MSE) 사이의 거의 선형적 관계를 발견하였고, 다양한 표현이 더 나은 성능을 보임을 강조하였습니다. 또한, 이 논문은 보완 시퀀스를 다양하게 만들기 위해 다각화 손실(diversification loss) 개념을 추가하였습니다.

- **Performance Highlights**: 제안된 방법은 장기 및 단기 예측에서 최근의 최첨단 방법들에 비해 탁월한 성능을 입증하였습니다. 다양한 시계열 예측 작업에서 실행된 광범위한 경험적 평가 결과, 모델이 더 풍부하고 일반화 가능한 표현을 학습하도록 돕는 데 성공했다는 것을 보여주고 있습니다. 이는 시계열 예측 작업의 효과를 높이는 데 중요한 기여를 하였습니다.
...
### Learning Stochastic Nonlinear Dynamics with Embedded Latent Transfer Operators (https://arxiv.org/abs/2501.02721)
Comments:
          This submission includes a supplementary file providing additional details. It also contains a code directory (code/) for the experiments. Both are included within the TeX source package

- **What's New**: 본 논문에서는 확률론적 비선형 동적 시스템의 연산자 기반 잠재 마르코프 표현을 제안합니다. 잠재 상태의 확률적 진화는 재생 커널 힐베르트 공간 내에서 전송 연산자(transfer operator)로 설명되며, 이를 학습하는 스펙트럴 방법(spectral method)을 개발합니다. 또한, 비선형 시스템에서의 순차 상태 추정(sequential state-estimation)과 동역학의 연산자 기반 고유 모드 분해(eigen-mode decomposition)를 일반화하는 문제도 다룹니다.

- **Technical Details**: 잠재 상태의 진화는 전송 연산자에 의해 기술되며, 이 연산자는 재생 커널을 통해 학습될 수 있습니다. 연구에서 다루는 모델은 비선형 확률 과정에 대한 일반화된 상태-공간 표현을 제안하며, 이는 기존의 비선형 필터링 알고리즘을 확장합니다. 스펙트럼 방법을 사용하여 동적 모드 분해(dynamic mode decomposition, DMD) 및 순차 상태 추정 절차를 포함한 여러 기법들이 소개됩니다.

- **Performance Highlights**: 실험적으로 합성 및 실제 데이터를 사용하여 제안된 방법의 성능을 입증합니다. 순차 상태 추정 및 모드 분해의 정확성이 향상됨을 보여주며, 동적 시스템의 복잡한 역학을 보다 효과적으로 모델링할 수 있는 가능성을 제시합니다. 이 방법은 전송 연산자 및 재생 커널을 이용한 데이터 분석 분야에 기여할 것으로 기대됩니다.
...
### Knowledge Distillation with Adapted Weigh (https://arxiv.org/abs/2501.02705)
- **What's New**: 본 논문에서는 Knowledge Distillation with Adaptive Influence Weight (KD-AIF)라는 새로운 프레임워크를 제안합니다. 이 프레임워크는 Robust Statistics에서 도출된 영향 함수(influence function)를 활용하여 훈련 데이터에 가중치를 부여함으로써 모델 성능을 개선합니다. 이는 기존의 지식 전이 방식을 뛰어넘어 성능 저하 없이 더 효율적인 모델 배포를 가능하게 합니다.

- **Technical Details**: KD-AIF 프레임워크는 Sustainability(지속 가능성), Accuracy(정확성), Fairness(공정성), Explainability(설명 가능성)라는 네 가지 주요 SAFE 원칙을 기반으로 합니다. 이를 통해 훈련 데이터의 중요도를 평가하고, 이에 따라 학생 모델의 학습 효율성을 높입니다. KD-AIF는 특히 온라인 지식 증류 기법을 이용하여 교사와 학생 모델을 동시에 학습시키며, 이는 성능 개선을 목표로 합니다.

- **Performance Highlights**: KD-AIF는 CIFAR-100, CIFAR-10-4k, SVHN-1k 및 GLUE 데이터셋과 같은 여러 벤치마크에서 기존 지식 증류 기법과 비교하여 우수한 성능을 나타냅니다. 이 접근법은 특히 반감독 학습(semi-supervised learning)에서도 뛰어난 성능을 보이며, 해석 가능성을 제공함으로써 모델의 투명성과 적용성을 증가시킵니다. 실험 결과 KD-AIF는 다수의 메트릭에서 효과성과 강인성을 입증하였습니다.
...
- **What's New**: 본 논문에서는 딥 뉴럴 네트워크(DNN)의 백도어 기반 워터마킹(schema) 기술의 저항성에 대한 새로운 평가와 개선 아이디어를 제안합니다. 특히 파인튜닝(fine-tuning) 과정에서 워터마크의 지속성을 분석하고, 트리거 세트(trigger set)를 노출하지 않고 워터마크를 복원하는 데이터 기반(data-driven) 접근 방식을 개발하였습니다. 이러한 방법을 통해 기존 기술의 한계를 극복하려는 노력을 강조하고 있습니다.

- **Technical Details**: 저자들은 파인튜닝 중 모델 파라미터(parameter)가 크게 변화하지 않을 경우, 훈련 데이터(training data)만으로 워터마크를 복원할 수 있음을 보여주었습니다. 다양한 트리거 샘플(trigger sample) 유형에 따라 복원된 정확도는 100%에 이를 수 있습니다. 더불어, 손실 경관(loss landscape) 시각화를 통해 복원 과정이 어떻게 작동하는지를 탐구하고 있습니다.

- **Performance Highlights**: 실험 결과는 파인튜닝 후에도 워터마크가 효과적으로 복원될 수 있음을 시사합니다. 본 연구는 워터마크 소실(watermark vanishing)을 완화하기 위해 파인튜닝 단계에서 훈련 데이터를 도입하는 아이디어를 추가로 제안합니다. 이러한 발견은 DNN에서 지적 재산권(IP) 보호를 위한 신뢰할 수 있는 방법으로서 큰 의의를 가지고 있습니다.



### Exploring a Datasets Statistical Effect Size Impact on Model Performance, and Data Sample-Size Sufficiency (https://arxiv.org/abs/2501.02673)
- **What's New**: 이 논문은 데이터셋의 적정성을 평가하는 방식에 대한 실험 결과를 보고하며, 기본적인 기술 통계적 측정이 데이터셋 훈련의 효과성을 예측할 수 있는지를 검토합니다. 연구자들은 효과 크기(effect size)가 모델 성능에 미치는 영향을 분석하며, 효과 크기가 결정 요인으로 작용할 것이라는 가정을 가지고 실험을 수행하였습니다. 결과적으로, 효과 크기는 데이터셋의 적합성이나 모델 성능 관점을 예측하는 데 효과적이지 않음을 보여주었습니다.

- **Technical Details**: 연구에서 사용된 데이터셋은 48,842개의 성인 인구 조사 데이터 샘플로 구성되어 있으며, 이 데이터는 UCI Machine Learning Repository에서 가져온 것입니다. 이 실험은 로지스틱 회귀(Logistic Regression), 결정 트리(Decision Tree), 랜덤 포레스트(Random Forest), 신경망(Neural Network) 등 4개의 머신러닝 모델을 활용해 수행되었습니다. 여러 실험을 통해 다양한 특성과 데이터 셋을 비교하며, 효과 크기를 계산하는 과정도 포함되어 있습니다.

- **Performance Highlights**: 실험 결과, 효과 크기가 모델 성능에 거의 예측적인 가치가 없음을 밝히며, R2 값이 이에 대한 변동성이 거의 없음을 나타냅니다. 데이터셋의 충분성을 결정하는 것은 특정 모델과 데이터 셋의 다른 요소들에 달려있을 가능성이 있으며, 따라서 머신러닝 모델 성능의 예측을 위한 다른 요인들이 필요합니다. 최종적으로, 데이터 수집 및 실험 설계와 관련하여 추가적인 연구가 필요함을 강조하고 있습니다.



### Incentive-Compatible Federated Learning with Stackelberg Game Modeling (https://arxiv.org/abs/2501.02662)
- **What's New**: 본 논문에서는 Federated Learning(FL) 분야의 새로운 접근 방식인 FLamma를 제안합니다. FLamma는 클라이언트의 상이한 자원과 역량을 고려하여 시스템의 공정성을 개선하기 위해 설계된 adaptive gamma 기반의 Stackelberg 게임 이론을 활용합니다. 이 프레임워크는 서버와 클라이언트 간의 역동적인 상호작용을 모델링함으로써, 공정한 기여를 유도하고 클라이언트의 참여를 확실히 함으로써 모델의 성능을 향상시키는 데 초점을 맞추고 있습니다.

- **Technical Details**: FLamma는 클라이언트가 자원의 효율적인 배분을 고려하여 로컬 에폭(local epochs)의 수를 최적화하는 가운데, 서버가 기여도를 조절하는 감쇠 인자를 동적으로 조정합니다. 이 과정에서 사용되는 새로운 유틸리티 함수는 시간에 따라 클라이언트 기여의 영향을 조정하여 트레이닝 시간의 공정한 배분을 촉진합니다. 또한, FLamma는 클라이언트의 기여와 Engagement의 변동성을 고려하여 자원 할당의 균형과 시스템의 효율성을 모두 달성하도록 설계되었습니다.

- **Performance Highlights**: FLamma는 기존 q-FFL에 비해 120.93%의 정확도 개선을 달성하였으며, FedAvg에 비해 편차(variance)를 99.09% 감소시켰습니다. 이러한 성과는 FLamma가 정확도와 공정성을 동시에 개선하는 데 매우 효과적임을 보여줍니다. 실험 결과, FLamma는 공정한 정확도 분포를 이루면서도 강력한 글로벌 모델 성능을 유지하며 전통적인 FL 방법들을 능가하는 결과를 나타냅니다.



### A New Interpretation of the Certainty-Equivalence Approach for PAC Reinforcement Learning with a Generative Mod (https://arxiv.org/abs/2501.02652)
Comments:
          14 pages, excluding references and appendices. Total of 28 pages

- **What's New**: 이 논문은 강화 학습(Reinforcement Learning, RL)에서의 확실성 동등성 원칙(Certainty-Equivalence Method, CEM)에 대한 새로운 관점을 제공합니다. CEM은 관측된 전이들로부터 최적 정책을 계산하는데 활용되는 방법으로, 본 연구는 CEM을 궤적 트리 방법(Trajectory Tree Method, TTM)과 연결하여 새로운 분석 템플릿을 제시합니다. 이를 통해 CEM의 샘플 복잡도(sample complexity) 상한에 대한 간단하고 새로운 증명들이 도출됩니다.

- **Technical Details**: CEM은 에이전트가 비관측 MDP(Markov Decision Process)와 상호작용하며 모델의 최대 우도 추정치(MLE)인 $\,\widehat{M}$를 기반으로 합니다. 이 연구는 비정상(non-stationary) 및 정상(stationary) MDP에 대한 CEM-NS라는 알고리즘을 분석하고, 각 (state, action, time-step) 삼중 조합에 대해 별도의 전이 확률 분포를 추정하는 방법을 제시합니다. 여기서 도출된 샘플 요구량은 O(|S||A|H^3𝜖^2log(1/δ) + |S|^2|A|H^4log|A|𝜖^2)입니다.

- **Performance Highlights**: 새롭게 제시된 상한은 Li et al.(2023)에서 보여준 상한보다 소규모의 실수 확률($\,\delta$) 하의 경우에서 H에 의해 더 세밀합니다. 또한, 이 연구는 보상에 대한 가정 완화를 통해 기존 문헌에서의 일반적인 가정보다 약한 조건에서도 적용 가능함을 보여줍니다. 따라서 CEM은 비정상 MDP에 대한 최소 최대-최적성을(minimax-optimality)를 가지며, 얻어진 새로운 샘플 복잡도의 상한은 RL 응용에 실질적 기여를 할 것으로 기대됩니다.
...
### HALO: Hadamard-Assisted Lossless Optimization for Efficient Low-Precision LLM Training and Fine-Tuning (https://arxiv.org/abs/2501.02625)
Comments:
          10 pages, 8 figures

- **What's New**: 새로운 연구 결과로 HALO라는 양자화 인식 훈련 기법을 소개합니다. HALO는 Transformer 모델의 구조를 수정하여 낮은 정밀도(FP8, INT8 또는 FP6)에서 훈련할 수 있도록 합니다. 이 방법은 전방 및 후방 패스에서 모든 대규모 행렬 곱셈을 낮은 정밀도로 수행할 수 있도록 설계되었습니다.

- **Technical Details**: HALO의 핵심은 Hadamard 변환을 전략적으로 배치하여 저정밀도 계산 중에 발생하는 이상값(outlier)을 줄이고, Fully Sharded Data Parallel (FSDP) 통신을 위한 지원을 포함합니다. 데이터를 처리하는 동안의 각 데이터 타입의 민감도를 분석함으로써, Hadamard 변환의 수와 배치를 조정하여 HALO의 여러 레벨을 정의합니다. 이 과정은 정확성과 효율성과 관련된 기본 알고리즘 기여를 포함합니다.

- **Performance Highlights**: HALO는 Llama-family 모델에 적용되었으며, 다양한 작업에서 완전 정밀도에 근접하는 결과를 달성했습니다. RTX 4090 GPU에서 전체 미세 조정(end-to-end fine-tuning) 시 1.31배 속도 향상을 기록하며, FP8에서 손실 없는 미세 조정을 수행할 수 있음을 보여줍니다. 이는 이전 방법보다 더 우수한 정확성과 성능 균형을 제공합니다.
...
### Chameleon2++: An Efficient Chameleon2 Clustering with Approximate Nearest Neighbors (https://arxiv.org/abs/2501.02612)
Comments:
          29 Pages, 15 Figures, 12 Tables

- **What's New**: 이 논문은 기존 Chameleon2 알고리즘의 성능과 관련된 두 가지 주요 실패를 분석하고 개선 사항을 제안합니다. 첫째, Chameleon2의 시간 복잡도가 실제로 $O(n^2)$가 아닌 $O(n^2\log{n})$임을 보여줍니다. 둘째, 알고리즘의 매개변수 값이 제공되지 않아 재현성(replicability)의 문제가 있었으며, 이를 해결하기 위해 필요한 매개변수 값을 자세히 나열하였습니다.

- **Technical Details**: Chameleon2 알고리즘은 네 가지 주요 단계로 구성되어 있습니다: 그래프 생성, 그래프 파티셔닝, 파티션 정제 및 병합입니다. 원래의 k-nearest neighbor (k-NN) 검색을 정밀한 k-NN 검색에서 근사 k-NN 검색으로 변경하여 계산 복잡도를 $O(n\log{n})$로 줄였습니다. 이 과정에서 Annoy, FLANN 및 NMSLIB와 같은 알고리즘을 조정하여 Chameleon2 클러스터링 프레임워크에 통합하였습니다.

- **Performance Highlights**: 실험 결과 제안된 Chameleon2++ 알고리즘은 이전의 Chameleon2 알고리즘보다 더 효율적이고 강건하며 계산적으로 최적화되었음을 보여줍니다. 또한, 기존의 정확한 k-NN 검색 대신 근사 k-NN 검색을 사용하였음에도 불구하고 성능 저하가 없음을 확인하였습니다. 여러 표준 벤치마크 데이터셋에서 실험 평가가 이루어졌습니다.
...
### Efficient Graph Condensation via Gaussian Process (https://arxiv.org/abs/2501.02565)
- **What's New**: 이번 논문은 GNN의 큰 데이터 세트에 대한 계산 비효율성을 해결하기 위해, Graph Condensation via Gaussian Process (GCGP)라는 새로운 접근법을 제안합니다. 이 방법은 Gaussian Process(GP)를 활용하여, 기존 GNN이 요구하는 반복적이고 자원 집약적인 훈련 과정을 없앱니다. GCGP는 기능 값 간의 의존성을 캡처하기 위해 특수한 공분산 함수(covariance function)를 파생하여, 구조적 정보를 통합하여 입력 노드의 수용 범위를 확장합니다.

- **Technical Details**: GCGP는 압축된 그래프를 관측값으로 사용하여 예측의 후행 분포를 추정합니다. 이러한 접근 방식은 GNN 모델이 필요로 하는 복잡한 반복 절차를 제거하여 계산 효율성을 크게 향상시킵니다. 또한, 이 논문은 그래프 구조에서의 이산적(digital) 정보 최적화를 위한 Concrete random variables를 사용하여 이진 인접 행렬(bipartite adjacency matrix)을 연속적으로 근사화합니다. 이 완화 과정을 통해 그래프 구조의 미분 가능성이 확보되어, 그래디언트 기반 최적화 기법의 적용이 가능해집니다.

- **Performance Highlights**: 실험 결과, GCGP 방법은 대규모 그래프 데이터를 효율적으로 압축하면서 예측 성능을 유지하는 데 성공했습니다. 제안한 방법은 노드 분류 정확도(node classification accuracy) 측정에서 우수한 압축 품질을 보여주며, 높은 계산 효율성을 입증했습니다. GCGP는 다양한 GNN 아키텍처와의 실험을 통해 효율성과 적응성을 입증하며, 그래프 데이터 압축의 품질 및 효율성 문제를 효과적으로 해결하는 데 기여하고 있습니다.
...
### An Analysis Framework for Understanding Deep Neural Networks Based on Network Dynamics (https://arxiv.org/abs/2501.02436)
Comments:
          12 pages, 10 figures

- **What's New**: 이 논문에서는 딥 러닝의 기초가 되는 메커니즘을 이해하기 위한 새로운 분석 프레임워크를 제안합니다. 이 프레임워크는 뉴런을 두 가지 모드로 분류하고, 이를 통해 깊이 있는 신경망이 정보를 극대화하는 방법을 밝힙니다. 이는 학습 샘플의 매력 구역(attraction basin)을 도입하여 DNN의 일반화 능력을 특징짓는 데 도움을 줍니다. 이러한 분석은 '평탄 최소값 효과(flat minima effect)'와 같은 DNN의 근본적인 행동을 설명하는 데 기여합니다.

- **Technical Details**: 논문에서 제안한 프레임워크는 완전 연결된 DNN과 잔여 연결(residual connection)을 사용하는 DNN 모델을 포함합니다. 각 뉴런의 전송 함수는 선형 또는 비선형의 Tanh-type으로 구성되어 정보를 인코딩하는 두 가지 모드, 즉 순서를 보존하는 모드(OPM)와 순서를 변경하는 모드(NPM)로 나누어 설명합니다. OPM은 입력의 순서를 보존하여 정보를 최대화하는 반면, NPM은 비선형 처리를 통해 입력 순서를 변경하여 더 자유롭게 정보를 추출할 수 있습니다. 이 두 모드의 최적의 조합이 효과적인 신경망 설계에 필수적임을 강조합니다.

- **Performance Highlights**: 매력 구역의 크기는 모델의 일반화 능력과 전체 네트워크 구조의 견고성을 측정하는 데 유용한 지표로 사용됩니다. 설계된 DNN 모델이 입력 데이터의 변형에 대해 100% 학습 정확도를 달성한 후, 무작위 노이즈 샘플에 대한 평균 정확도를 평가하여 매력 구역의 크기를 확인했습니다. 이러한 매력 구역의 크기는 DNN의 깊이와 넓이에 따라 다르게 나타나며, 'flat minima effect'와의 강한 상관관계를 통해 DNN의 특성을 이해하는 데 중요한 역할을 합니다.
...
### Tensor-GaLore: Memory-Efficient Training via Gradient Tensor Decomposition (https://arxiv.org/abs/2501.02379)
- **What's New**: Tensor-GaLore는 고차원 텐서 가중치를 이용한 신경망의 효율적인 훈련을 위한 새로운 방법론입니다. 이 방법은 복잡한 다차원 관계를 포착하는 텐서 매개변수화 계층을 활용하며, 특히 부분 미분 방정식(PDE) 해결에 중요한 역할을 하는 Fourier Neural Operators(FNOs)의 효과성을 보여줍니다. 본 연구는 기존의 GaLore 접근 방식과 비교하여 메모리 사용량을 최대 75%까지 줄일 수 있음을 입증하였습니다.

- **Technical Details**: Tensor-GaLore는 고차원 텐서 매개변수 공간에서 직접 작업하며, 경량화된 그라디언트 계산을 통해 메모리 요구사항을 줄입니다. Tucker 분해를 사용하여 그라디언트 텐서를 저계수(Subspace)로 프로젝션하고, 이 과정에서 구조적 다차원성을 보존하여 신경 연산자(Neural Operators) 훈련의 효율을 높입니다. 이러한 접근방법은 후행 과정을 통해 원래의 고차원 구조를 유지하며, 주로 과학적 모델링에서 발생하는 문제를 해결하는 데 중점을 둡니다.

- **Performance Highlights**: Tensor-GaLore는 Navier-Stokes 방정식과 같은 다양한 PDE 작업에 대해 다양한 실험을 통해 성능을 입증하였습니다. 특히, 1024x1024 해상도에서 진행한 실험에서 최적화 상태에 대해 최대 75%의 메모리 절약을 이루어냈습니다. 또한, Darcy 흐름, Burgers’ 방정식 및 전자기파 전파와 같은 다른 중요한 PDE들에 대해서도 Tensor-GaLore의 효율성을 검증하였습니다.



### A ghost mechanism: An analytical model of abrupt learning (https://arxiv.org/abs/2501.02378)
- **What's New**: 본 논문에서는 전통적인 신경망에서 관찰되는 '갑작스러운 학습' 현상을 분석합니다. 연구자들은 지연 활성화 작업에서 최소한의 동적 시스템을 도입하고, 이 시스템이 범주 분기(bifurcation) 없이도 어떻게 갑작스러운 학습을 나타낼 수 있는지를 보여줍니다. 이를 통해 문화적 포인트(ghost points)가 학습 동태를 불안정하게 만드는 중요한 요인임을 입증합니다.

- **Technical Details**: 저자들은 toy model을 사용하여, 모델의 학습 속도와 손실 함수의 구조를 분석합니다. r이 긍정적일 때, 손실 함수는 안정된 영역(no-learning zone)과 진동하는 최소값(oscillatory minimum)으로 나뉘며, 특정 학습 속도 이하에서 학습을 저지하게 됩니다. 이 toy model은 심플한 쿼드러틱(quadratic) 형태를 가진 것을 기반으로 하여, 타임티커(timestamps) t에 따른 모델 출력의 변화를 수학적으로 다룹니다.

- **Performance Highlights**: 실험적으로 저자들은 RNN(recurrent neural networks)에서 갑작스러운 학습이 발생하기 전에 문화적 포인트가 나타난다는 것을 확인했습니다. 또한, 낮추는 방법과 추가적인 파라미터를 통해 더욱 안정적인 학습 경로를 얻을 수 있음을 보여주었습니다. 이 연구는 고전적인 학습 동태를 안정화하기 위해 의도적인 불확실성과 중복성의 역할을 강조합니다.



### BADTV: Unveiling Backdoor Threats in Third-Party Task Vectors (https://arxiv.org/abs/2501.02373)
- **What's New**: 이 논문에서는 대규모 사전 훈련 모델에서 작업 산술(task arithmetic) 사용의 새로운 취약점, 즉 백도어 공격(backdoor attacks)에 대한 보안 문제를 탐구하고 있습니다. 특히, 작업 벡터(task vectors, TVs)가 악의적인 사용자에 의해 어떻게 조작될 수 있는지를 분석하며, 이를 통해 모델 무결성과 보안이 심각하게 위협받고 있음을 입증합니다. 저자들은 BadTV라는 새로운 공격 방법론을 제시하여, 작업 학습(task learning), 망각(forgetting), 유사성(analogies) 작업 시에도 효과적으로 작동할 수 있도록 설계되었습니다. 이러한 작업의 취약성을 이해하고 개선하기 위한 필요성을 강조하고 있습니다.

- **Technical Details**: BadTV는 작업 벡터에서 백도어를 구성하는 혁신적인 방법으로, 기존의 단일 백도어 공격과 달리 복합 백도어(composite backdoors)를 삽입하여 작동합니다. 이러한 방식은 각 작업에서 학습이나 망각 중에 백도어가 활성화될 수 있도록 하며, 두 백도어가 서로 간섭하지 않도록 설계되어 있습니다. 이 연구는 현재의 방어 메커니즘들이 BadTV를 식별하거나 완화할 수 없다는 사실을 보여주며, 이를 해결하기 위한 강력한 방어 메커니즘의 개발이 절실하다고 말합니다.

- **Performance Highlights**: 실험 결과, BadTV는 다양한 시나리오에서 거의 완벽에 가까운 공격 성공률을 기록했습니다. 전통적인 방법들이 실패한 것과는 달리, BadTV는 임베디드 TV가 포함된 모델에서도 높은 공격 성공률(Attack Success Rate, ASR)을 유지하며 이를 통해 실질적인 위협을 입증했습니다. 저자들은 BadTV의 효과적인 구현 및 기존 방어 기법의 한계를 강조하고, 이를 통해 향후 머신러닝 생태계에서의 보안 경각심을 높이고 있습니다.



### Predicting two-dimensional spatiotemporal chaotic patterns with optimized high-dimensional hybrid reservoir computing (https://arxiv.org/abs/2501.02369)
- **What's New**: 이 논문은 고차원 동적 시스템 예측을 위한 하이브리드 Reservoir Computing (RC) 접근 방식을 제안하고 있습니다. 특히, 두 차원에서의 시공간적 패턴 예측에 대한 새로운 포맷을 확장했습니다. 복잡한 시스템을 예측할 수 있는 능력을 향상시킬 수 있는 방법으로, 지식을 기반으로 한 모델(KBM)를 통합하는 하이브리드 기법을 설명합니다.

- **Technical Details**: 하이브리드 RC는 입력 하이브리드(IH), 출력 하이브리드(OH), 전체 하이브리드(FH) 세 가지 접근 방식을 포함하며, 각 방법에 따라 시뮬레이션 된 고차원 동적 데이터를 예측합니다. 본 연구에서는 로컬 상태(LS) 접근 방식을 활용하여 차원 저주(curse of dimensionality)를 극복하였습니다. 바클리 모델을 사용하여 심장 조직에서의 혼돈 전기파 전파를 시뮬레이션하였고, 시간 차분 방법으로 Eular를 적용하여 시간 통합을 수행했습니다.

- **Performance Highlights**: 세 가지 하이브리드 접근 방식 모두 저장소만 사용할 때보다 더 나은 성능을 보였으며, 특히 하이브리드 RC의 출력 하이브리드(OH) 방식이 높아진 예측 성능과 해석 가능성으로 선호되었습니다. 그러나 큰 저장소의 경우 OH 성능이 낮아지고, 모든 응용에 대해 세 가지 설정 테스트를 통해 최적의 성능과 CPU 요구사항 사이의 균형을 찾는 것이 권장됩니다.
...
### Easing Optimization Paths: a Circuit Perspectiv (https://arxiv.org/abs/2501.02362)
Comments:
          Accepted at ICASSP 2025

- **What's New**: 이 논문은 대규모 인공지능 시스템 훈련에 대한 이해를 높이기 위해 회로(circuit) 관점을 제안합니다. 기계적 해석 가능성(mechanistic interpretability)에서 유래된 이 접근법은 훈련 동역학(training dynamics)을 최적화하여 연산 비용을 절감하고 시스템의 유해한 행동을 감소시키는 데 기여할 수 있습니다. 저자들은 회로가 신경망의 학습 및 성능 최적화에 미치는 영향을 탐구하고 있습니다.

- **Technical Details**: 인공 신경망은 상호 연결된 뉴런의 복잡한 전기 회로로 구성된 인간 뇌에서 영감을 받았습니다. 논문에서는 회로의 정의와 이들이 신경망 훈련 동작에서 어떻게 유용한지를 설명합니다. 구체적으로, 그래디언트 강하(gradient descent)가 유용한 회로를 강화하고 다른 회로를 제거하는 메커니즘을 탐구하며, 간단한 문제를 통해 회로의 동작을 연구합니다.

- **Performance Highlights**: 저자들은 회로 관점이 신경망의 훈련 동역학을 이해하는 새로운 방법을 제공하며, 효과적인 학습을 위한 커리큘럼 디자인(curriculum design)을 통해 유용한 하위 회로(sub-circuits)를 촉진할 수 있다고 주장합니다. 이 연구는 Transformer 아키텍처를 중심으로 구성되며, 이를 통해 대규모 언어 모델의 성능을 최적화할 수 있는 가능성을 보여줍니다. 제안된 방법은 복잡한 작업을 중간 추론 단계로 나누어 해결할 수 있는 가능성을 제공합니다.



### When is the Computation of a Feature Attribution Method Tractable? (https://arxiv.org/abs/2501.02356)
Comments:
          8 pages in row format

- **What's New**: 이번 연구에서는 SHAP 외의 다양한 power index의 계산 복잡성을 분석하고, 효율적으로 계산 가능한 조건을 제시합니다. 이를 통해 feature attribution methods에서 덜 접한 Bernoulli power indices를 소개하며, 이들의 계산이 상수 개수의 기대값 평가로 단순화될 수 있음을 보여줍니다. 이 연구는 모델 예측의 설명 가능성을 높이는 데 기여할 것입니다.

- **Technical Details**: 연구의 주요 초점은 cooperative game theory의 power index 계산을 다루는 것입니다. 먼저, SHAP 및 Banzhaf 값을 포함한 간단한 power index들에 대해 논의하며, 이들을 통해 각 feature가 모델 예측에 미치는 기여도를 평가합니다. 일반적으로 이들 power index의 복잡성을 이해하는 것은 머신러닝 모델의 예측을 이해하는 데 핵심적입니다.

- **Performance Highlights**: SHAP scores 계산의 효율성을 보여줌으로써, 특정 모델 클래스에서 기대값이 다항 시간 내에 계산 가능한 경우 SHAP scores 또한 효율적으로 계산될 수 있음을 확인했습니다. 그러나 SHAP 외의 power index의 계산 복잡도는 잘 알려져 있지 않으며, 이 연구는 그 계산 가능성을 정량화하는 데 중요한 기여를 할 것으로 보입니다.



### Reweighting Improves Conditional Risk Bounds (https://arxiv.org/abs/2501.02353)
Comments:
          33 pages

- **What's New**: 이번 연구에서는 weighted empirical risk minimization (weighted ERM) 방법론을 탐구합니다. 일반적인 'balanceable' Bernstein 조건을 통해 데이터 종속적인 가중치를 추가로 고려함으로써 ERM보다 우수한 성능을 발휘할 수 있음을 보여주었습니다. 특히, 특정 서브 영역에서의 매개변수 조정이 중요한 역할을 하는 것을 강조하고 있습니다.

- **Technical Details**: 실험적 위험 최소화(ERM)는 머신러닝에서 중요한 역할을 하며, 주어진 샘플 집합에 대해 타겟 가설을 추정하는데 사용됩니다. 데이터 종속적인 가중치 함수 ω(⋅)를 통해 조건부 초과 위험을 개선할 수 있으며, 이를 통해 결과의 신뢰성을 높일 수 있습니다. 이 연구에서는 Bernstein 유형의 조건을 활용하여 효율적인 경량화된 방법론을 제시하고 있습니다.

- **Performance Highlights**: 실험 결과, 개선된 weighted ERM 추정기가 표준 ERM에 비해 높은 신뢰도의 영역에서 저조한 초과 위험을 제시하였습니다. 특히, 선택적 학습 설정과 유사한 경우에서 성능이 중요하게 나타났으며, 데이터 의존적인 상수 용어가 오류 경계에서 우수성을 나타냅니다. 이러한 방법론은 다양한 머신러닝 설정에서 실질적인 응용 가능성을 보여줍니다.
...
### Digital Twin Calibration with Model-Based Reinforcement Learning (https://arxiv.org/abs/2501.02205)
Comments:
          28 pages, 6 figures

- **What's New**: 이 논문은 복잡한 비선형 동적 시스템의 효과적인 제어를 위해 디지털 트윈(Digital Twin) 보정(calibration)을 모델 기반 강화 학습(Model-based Reinforcement Learning)에 통합한 새로운 방법론인 Actor-Simulator를 제안합니다. 기존의 모델 기반 제어 방법은 제한적인 구조적 가정(linear state transitions)을 요구하며 모델의 파라미터 불확실성을 고려하지 않는데, 이러한 문제들이 바이오 제약 제조와 같은 산업에서는 특히 중요합니다. 이러한 방법은 효율적인 제어 정책(optimal control policy) 검색을 통해 모델 오차(model error)를 줄이고 최적의 정책으로 수렴하는 것을 입증하였습니다.

- **Technical Details**: 이 연구에서는 디지털 트윈이 한정된 수의 보정 파라미터로 정의되는 상황을 고려하고, 이 파라미터들은 물리적 시스템의 실험 데이터를 기반으로 설정됩니다. 판별할 수 없는 파라미터를 포함하는 비선형 상태 전이 모델을 사용하여 물리적 시스템과 디지털 시스템을 무한 지평 마르코프 결정 과정(Markov Decision Processes)으로 모델링하고, 정책 함수(optimal policy function)를 학습합니다. Actor-Simulator 프레임워크는 디지털 트윈 보정, 데이터 수집 및 정책 최적화를 교대로 수행하며, 불확실성을 정량화하는 새로운 불확실성 함수가 중요한 요소입니다.

- **Performance Highlights**: 제안된 방법은 바이오 제약 제조와 같은 복잡하고 비선형 동적 시스템에 대한 광범위한 실험을 통해 기존의 최첨단 Gaussian Process 기반 방법 및 무작위 정책(random policy)에 비해 현저한 성능 향상을 보여주었습니다. 이 연구는 정량화된 불확실성이 모델 기반 강화 학습 목적을 어떻게 영향을 주는지를 명확히 하고, 최적의 정책 탐색 과정에서 가장 유의미한 데이터 수집이 이루어질 수 있도록 협업을 통해 성능 개선을 이룬 것을 강조합니다. 결과적으로, 이 새로운 접근법은 모델 파라미터 추정과 정책 최적화 모두에서 입증된 이점을 제공합니다.
...
### On LLM-Enhanced Mixed-Type Data Imputation with High-Order Message Passing (https://arxiv.org/abs/2501.02191)
- **What's New**: 이 논문에서는 LLM(대형 언어 모델)과 고차 메시지 전달(high-order message passing)을 활용한 통합적인 결측값 대체 프레임워크인 UnIMP를 제안합니다. UnIMP는 수치형, 범주형 및 텍스트 데이터가 혼합된 데이터를 다룰 수 있도록 설계되었습니다. 특히, 테이블을 모델링하기 위해 셀 중심 하이퍼그래프(cell-oriented hypergraph)를 도입하고, 전역-local 정보 및 고차 관계를 집계하는 Bidirectional High-order Message-Passing(BiHMP) 네트워크를 제안합니다.

- **Technical Details**: UnIMP는 BiHMP와 LLM의 능력을 효과적으로 연계하기 위한 Xfusion을 도입하여, LLM을 위한 어댑터로 작용합니다. 모델 훈련은 사전 훈련(pre-training)과 미세 조정(fine-tuning) 파이프라인을 따르며, 두 가지 최적화 방법인 청크 기술(chunking technique)과 점진적 마스킹 기술(progressive masking technique)을 통합합니다. 이 접근 방식은 10개의 실제 데이터셋에 대한 이론적 증명과 실험적 결과를 통해 UnIMP의 우수성을 강조합니다.

- **Performance Highlights**: UnIMP는 기존 방법들에 비해 결측값 대체에서 더 높은 정확도를 나타내며, 다양한 데이터 유형을 처리하는 데 있어 우수한 성능을 발휘합니다. 특히, LLM 기반 방법들이 숫자 데이터에 대해 만족스럽지 못한 퍼포먼스를 보이는 반면, UnIMP는 이러한 단점을 극복하고 상관관계 및 패턴을 효과적으로 학습할 수 있는 능력을 가지고 있습니다. 따라서, 실제 적용 가능성이 높은 데이터 품질 향상 기술로 자리 잡을 수 있습니다.
...
### How Your Location Relates to Health: Variable Importance and Interpretable Machine Learning for Environmental and Sociodemographic Data (https://arxiv.org/abs/2501.02111)
Comments:
          AAAI

- **What's New**: 이 연구에서는 환경적 및 사회인구학적 요인들이 건강 결과에 미치는 영향을 분석하기 위해 새로운 MEDSAT (Medical and Socio-Demographic Analysis Tool) 데이터셋을 활용합니다. 이는 영국의 건강, 환경, 사회인구학적 정보를 통합한 유일무이한 데이터로, 연구자들이 다양한 건강 결과에 대한 더 정밀한 분석을 수행 가능하게 합니다. 연구는 Generalized Additive Models (GAMs) 및 Multiscale Geographically Weighted Regression (MGWR)이라는 기계학습 프레임워크를 통해 지역 및 글로벌 변수를 해석하는 접근법을 채택합니다.

- **Technical Details**: 이 연구는 변동성이 큰 환경 데이터와 사회인구학 정보를 포함하는 포괄적인 데이터셋을 바탕으로 건강 결과에 대한 예측 변수를 조명합니다. 데이터 처리에는 Knockoff 기법과 SHAP, LOCO 등의 해석 가능성 향상 기술이 포함되어 있으며, GAM과 MGWR을 통해 지역 간 및 시간에 따른 관계 변화를 분석합니다. 이를 통해 NO2와 같은 글로벌 변수가 천식, 고혈압, 불안과 언제나 연관있음을 보여줍니다.

- **Performance Highlights**: 연구 결과 NO2가 천식, 고혈압 및 불안에 대한 글로벌 예측 변수로 작용하는 것을 발견했습니다. 또한, 직업, 결혼 및 식생과 관련된 다양한 특정 예측 변수들이 드러났고, COVID-19 기간 동안 공기 오염 및 태양 복사와 같은 지역적 변화를 확인했습니다. 이 연구는 공공 건강 정책 향상을 위한 실행 가능한 통찰력을 제공하며, 해석 가능한 기계 학습 모델의 통합 필요성을 강조합니다.
...
- **What's New**: 이 논문에서는 Distributional Reinforcement Learning (DRL) 프레임워크를 활용하여 더 넓은 범위의 static Spectral Risk Measures (SRM)를 최적화하는 새로운 DRL 알고리즘을 제안합니다. 기존의 접근법들이 고정된 위험 측정값을 사용하여 지나치게 보수적인 정책을 초래하는 단점을 보완합니다. 또한, 학습된 정책의 해석을 명확히 하기 위해 수익의 분포와 static coherent risk measures의 분해를 활용합니다.

- **Technical Details**: 제안된 알고리즘은 수렴 보장을 가지고 있으며, DRL 프레임워크 내에서 SRM 객체와 일치하는 정책을 학습합니다. 이 알고리즘은 static CVaR 위험 측정에 국한되지 않고 더 폭넓은 위험 측정 방법을 통합합니다. DRL 결정 과정에서 위험 민감도를 포함하여 의사결정의 복잡성을 감소시킵니다.

- **Performance Highlights**: 다양한 실험을 통해, 제안된 모델이 SRM 목표에 부합하는 정책을 학습하고, 기존의 위험 중립 및 위험 민감 DRL 모델에 비해 우수한 성능을 보임을 입증했습니다. 특히, 금융 및 의료 분야와 같은 복잡한 환경에서의 적용 가능성이 높습니다.



### Counterfactual Explanation for Auto-Encoder Based Time-Series Anomaly Detection (https://arxiv.org/abs/2501.02069)
Comments:
          8 pages, 6 figures, 6 tables, conference proceeding

- **What's New**: 이번 연구에서는 복잡한 전자기계 시스템의 이상 탐지에서 결정 과정의 불투명성을 해결하기 위한 방법을 제시합니다. 선택된 기능과 반사실적 설명(counterfactual explanation)을 이용하여 모델 출력을 이해할 수 있는 맥락을 제공합니다. 제안된 접근법은 SKAB 벤치마크 데이터셋과 산업 시간 시계열 데이터셋에서 테스트되었으며, 그 결과 더욱 신뢰할 수 있고 해석 가능한 이상 탐지 모델을 구현할 수 있는 가능성을 보여주었습니다.

- **Technical Details**: Auto-Encoder(AE)는 입력 데이터를 낮은 차원의 잠재 표현(latent representation)으로 줄이는 비지도 학습 모델입니다. 이 연구에서는 AE의 구조를 활용하여 시간 시계열 데이터를 처리하는 합성곱 기반(convolution-based) 아키텍처를 사용합니다. 특히, 이상 탐지의 경우 정상 데이터를 잠재공간으로 인코딩하고 복원하는 방식으로 적용되며, 복원 오류가 특정 기준을 초과하는 데이터를 이상으로 간주합니다.

- **Performance Highlights**: 실험 결과는 제안된 반사실적 설명 방법이 모델의 의사 결정 과정을 보다 심도 있게 설명할 수 있음을 보여줍니다. 전통적인 방법에 비해 더 적은 신호로도 의미 있는 통찰을 제공하며, 이는 분석과 진단 과정에서 유용성을 증가시킵니다. 이러한 방향은 AI 기반 이상 탐지의 해석 가능성과 신뢰성을 높이는 데 기여할 것으로 기대됩니다.



### Active Learning Enables Extrapolation in Molecular Generative Models (https://arxiv.org/abs/2501.02059)
- **What's New**: 이 논문에서는 생성 모델(generative models)이 최적화된 물질(properties)을 발견하는 데의 가능성에도 불구하고, 기존 훈련 과정에서 본 물질들과 비교하여 더 좋은 합성 가능한 분자를 제안하지 못하는 문제를 다룹니다. 핵심 한계는 분자 생성 과정이 아니라 분자 특성을 예측하는 모델의 일반화(generalization) 능력이 부족하다는 점을 발견했습니다.

- **Technical Details**: 연구진은 능동 학습(active-learning) 구조를 통해 분자 생성 파이프라인을 구성하고, 양자 화학 시뮬레이션(quantum chemical simulations)의 피드백을 통해 분자 생성 모델을 반복적으로 개선하는 방법을 제시합니다. 이 닫힌 루프(closed-loop) 접근 방식은 새로운 화학 공간에 대한 일반화 능력을 향상시키는 데 중점을 두고 있습니다.

- **Performance Highlights**: 기타 생성 모델 접근법들과 비교했을 때, 본 연구의 능동 학습 접근법은 훈련 데이터 범위를 넘어 0.44 표준 편차까지 extrapolate할 수 있는 분자를 생성합니다. 또한, 모델의 분류 정확도(out-of-distribution classification accuracy)가 79% 향상되었으며, 열역학적 안정성(thermodynamic stability) 정보를 기반으로 생성된 안정한 분자의 비율이 다음으로 좋은 모델보다 3.5배 높습니다.
...
- **What's New**: 이번 연구는 NLP 분야의 설명 가능한 인공지능(XAI)에서 적대적 공격(adversarial attacks)을 살펴보며, 특히 Lime과 같은 지역 대리 모델(local surrogate methods)의 취약성을 분석하는 데 중점을 두었습니다. 이러한 공격은 원래 입력의 의미와 구조는 유사한 상태에서 생성된 설명이 조작되는 현상을 다룹니다. XAI가 의사 결정 또는 법적 조치의 기반으로 사용될 때, 이는 특히 우려스러운 문제로, 이에 대한 충분한 탐구가 부족한 상황입니다.

- **Technical Details**: 이 연구는 텍스트 기반의 순위 리스트에서 사용될 수 있는 다양한 유사도 측정 방법(similarity measures)을 조사하여, 이들 각각의 적합성을 비교 분석합니다. 유사도 측정 방법의 선택이 적대적 검색 과정(adversarial search process)에 미치는 중요성을 강조하며, 적절하지 않은 유사도 측정은 XAI 방법의 안정성 예측을 왜곡할 수 있음을 보여줍니다. 최종적으로, 동의어 가중치를 포함한 새로운 유사도 측정 방법을 제안하며, 이는 텍스트 기반 데이터에 대한 보다 정확한 안정성 평가를 가능하게 합니다.

- **Performance Highlights**: 연구 결과, 많은 전통적인 유사도 측정 방법들이 지나치게 민감하여 XAI 방법의 안정성에 대한 잘못된 추정을 초래할 수 있음을 발견했습니다. 새로운 유사도 측정 방법이 기존의 방법보다 더 나은 결과를 제공하여 적대적 본질을 보다 정교하게 이해할 수 있도록 돕는다는 것을 입증했습니다. 이는 향후 XAI의 발전에 중요한 기여를 할 것으로 기대됩니다.
...
### Information Subtraction: Learning Representations for Conditional Entropy (https://arxiv.org/abs/2501.02012)
- **What's New**: 이 논문은 기존의 조건부 대비 샘플링 방법을 넘어 연속 변수에 대한 정보 대표화를 위해 Information Subtraction(정보 차감) 프레임워크를 도입합니다. 이 프레임워크는 원하는 정보를 유지하면서 바람직하지 않은 정보를 제거할 수 있는 대표성을 생성하는 구조로, 생성 기반 아키텍처를 사용하여 정보 항목을 최적화합니다. 이를 통해 우리는 연속 변수 간의 복잡한 관계를 보다 효과적으로 설명할 수 있습니다.

- **Technical Details**: 정보 차감 프레임워크는 생성기 신경망과 두 개의 판별기로 구성되어 있으며, 첫 번째 판별기로부터 얻은 정보 추정치를 극대화하고 두 번째 판별기로부터의 추정치를 최소화하여 작동합니다. 이 과정에서 조건부 엔트로피와 조건부 상호정보 등을 효과적으로 대표할 수 있으며, 연속 입력에 대한 조건부 표현을 생성할 수 있습니다. 또한, 이 구조는 특정 정보를 차감함으로써 다양한 관계를 독립적으로 표현할 수 있습니다.

- **Performance Highlights**: 이 연구 결과는 Information Subtraction 프레임워크가 공정한 학습(fair learning) 및 도메인 일반화(domain generalization)에 효과적임을 보여줍니다. 실험을 통해 제안된 구조가 민감한 정보와 도메인 특정 정보를 잘 제거하면서 좋은 성능을 발휘함을 입증하였으며, 이는 다양한 변수 간의 관계를 설명하는 데 유용합니다. 논문에는 본 연구의 코드가 제공되어 있어, 연구자들이 이를 바탕으로 후속 작업을 진행할 수 있습니다.



### Explainable Neural Networks with Guarantees: A Sparse Estimation Approach (https://arxiv.org/abs/2501.02010)
- **What's New**: 이 논문은 예측력(predictive power)과 해석 가능성(interpretability)을 조화롭게 결합할 수 있는 새로운 접근 방안을 소개합니다. 저자들은 SparXnet이라는 모델을 제안하며, 이는 단일 1차원 입력 특성에 대해 여러 학습 가능한 함수를 적용하여 파생된 희소한 특성 집합(sparse set of features)의 선형 조합(linear combination)으로 설계되었습니다. 이 모델은 복잡한 관계를 자동으로 학습하며, 중요한 특성을 선택할 수 있는 능력을 가지고 있습니다.

- **Technical Details**: SparXnet의 구조는 다차원 입력의 복잡한 관계를 선형 조합을 통해 설명 가능하게 만듭니다. 본 논문에서 제시된 이론적인 분석은 선택된 특성의 수에 비례하여 선형(linear)하게 일반화 경계(generalization bounds)를 제시하며, 입력 특성의 수에는 로그(logarithmic)적 한계를 두고 있습니다. 또한, 샘플 복잡성(sample complexity)이 매개변수나 구조적 세부 사항에 의존하지 않도록 완화된 조건을 제공합니다.

- **Performance Highlights**: SparXnet은 합성 및 실제 데이터 셋에 대한 광범위한 실험을 통해 선택된 중요한 특성을 유지하면서도 예측 성능(predictive performance)을 비교 가능하게 유지한다는 것을 보여줍니다. 모델은 예측 정확도를 손상시키지 않으면서도 직관적으로 설명 가능하다는 장점을 가지고 있습니다. 이러한 연구는 희소(sparse)하고 해석 가능(explainable)한 신경망에 대한 향후 연구에 새로운 길을 열어줄 것입니다.
...
### HMM-LSTM Fusion Model for Economic Forecasting (https://arxiv.org/abs/2501.02002)
Comments:
          33 pages, 18 figures

- **What's New**: 이 논문은 Hidden Markov Models (HMM)와 Long Short-Term Memory (LSTM) 신경망을 경제 예측에 적용하는 혁신적인 접근 방식을 탐구하며, 특히 CPI 인플레이션율을 예측하는 데 초점을 맞추고 있습니다. HMM에서 유도된 숨겨진 상태와 평균을 LSTM 모델링에 추가적인 특성으로 통합하여 모델의 해석 가능성과 예측 성능을 향상시키고자 하였습니다. 연구에서는 데이터 수집 및 전처리 과정을 시작으로 HMM을 구현하여 경제적 조건을 나타내는 숨겨진 상태를 식별하였습니다.

- **Technical Details**: 연구는 다양한 경제적 요인을 반영하여 CPI 인플레이션율을 예측하는 LSTM 모델을 구축했습니다. HMM을 통해 얻은 숨겨진 상태 및 평균을 LSTM 특성으로 추가하여 다양한 경제적 추세를 파악하고 이러한 데이터를 기반으로 한 예측의 정확성을 높였습니다. Integrated Gradients를 사용하여 LSTM의 해석 가능성을 높였으며, 이를 통해 예측 결과에 반영된 경제 역학에 대한 통찰을 제공하였습니다.

- **Performance Highlights**: 연구 결과, HMM에서 유도된 데이터를 포함한 LSTM 모델이 예측 정확성을 향상시켰으며, 이는 복잡한 시간적 패턴을 포착하고 변동성이 큰 경제적 조건의 영향을 완화하는 데 특히 효과적이었습니다. 저자들은 전통 경제 모델링과 고급 머신러닝 기술 간의 간극을 메우는 방안을 제시하여 경제 예측을 더 견고하고 통찰력 있게 만들었습니다. 이러한 접근법은 정책 입안자, 분석가 및 연구자들에게 실질적인 이익을 제공합니다.



### Communication Efficient Cooperative Edge AI via Event-Triggered Computation Offloading (https://arxiv.org/abs/2501.02001)
Comments:
          13 pages, 11 figures

- **What's New**: 본 연구는 6세대(6G) 모바일 네트워크에서 드문 사건에 대한 효율적인 처리에 중점을 둔 이벤트 기반 엣지 추론 프레임워크를 제안합니다. 이 프레임워크는 이중 임계값 및 다중 종료 아키텍처를 활용하여 드문 사건을 조기에 로컬에서 추론하고, 복잡한 사건은 엣지 서버로 전송하여 세분화된 분류를 가능하게 합니다. 이를 통해 임베디드 AI 시스템이 자원을 효율적으로 사용하면서도 신속한 응답을 제공할 수 있습니다.

- **Technical Details**: 이 연구에서 제안된 시스템은 이벤트 감지, 적응형 추론, 동적 오프로드 기술을 통합하여 통신과 계산 리소스가 제약된 상황에서도 드문 사건 분류 정확도를 극대화하는 것을 목표로 합니다. 이 시스템은 동적 이탈 조건을 활용하는 이중 임계값 아키텍처를 특징으로 하며, 높은 신뢰도의 사건은 조기에 추론을 수행하고 불확실한 사건은 서버로 오프로드하여 더욱 정밀하게 분석합니다. 최적화를 위해 비볼록(non-convex) 문제를 강한 볼록(strongly convex) 문제로 변환하여 처리합니다.

- **Performance Highlights**: 실험을 통해 제안된 이벤트 기반 협력 추론 시스템은 실제 의료 데이터셋을 활용하여 기존의 엣지 추론 접근 방식보다 뛰어난 드문 사건 분류 정확도를 달성함을 보여주었습니다. 또한 통신 오버헤드를 효과적으로 줄여서 자원의 효율적인 배분을 가능하게 합니다. 이러한 결과는 임무 수행에 있어 중요한 응용 프로그램에서 특히 유용하게 작용할 수 있습니다.



### Towards Sustainable Large Language Model Serving (https://arxiv.org/abs/2501.01990)
- **What's New**: 본 연구에서는 탄소 배출 관점에서 대규모 언어 모델(LLM)을 분석하고, 운영 및 포함된 탄소 배출을 논의하여 지속 가능한 LLM 서비스 체계를 위한 기초를 마련합니다. LLaMA 모델의 파라미터 수에 따라 성능과 에너지를 분석하고, 최신 Nvidia GPU와 구형 GPU를 활용하여 실제 환경에서의 탄소 배출을 모델링합니다. 이 연구는 LLM 서비스에 대한 탄소 배출 이해를 보다 세분화된 수준에서 제시하며 새로운 연구 방향을 제시합니다.

- **Technical Details**: LLM의 성능 및 에너지는 1B, 3B, 7B 파라미터를 가진 LLaMA 모델을 대상으로 하며, 두 가지 Nvidia GPU 유형인 RTX6000 Ada와 T4에서 실험합니다. 운영 탄소 배출은 에너지 소비 및 세 가지 전력망 지역의 탄소 강도(CI)를 바탕으로 분석하였고, 포함된 탄소 배출은 칩 면적과 메모리 크기를 이용해서 모델링했습니다. 본 연구는 LLM 서비스의 수행 성능 및 에너지 소비를 평가하여 환경 영향에 대한 깊은 통찰을 제공합니다.

- **Performance Highlights**: T4는 특정 요청(예: 배치 크기 1)에 대해 RTX6000 Ada보다 에너지 효율이 더 높다는 사실이 밝혀졌습니다. 파라미터 크기와 배치 크기에 따른 최적의 구성은 에너지 효율성 최대화를 위해 복잡한 절충을 요함을 보여 주며, 전략적으로 T4와 같은 구형 GPU를 사용할 경우, 총 탄소 배출량을 효과적으로 줄일 수 있음을 시사합니다. 연구 결과, 특정 GPU 타입에서 가장 높은 처리량을 달성하는 구성은 항상 최고의 에너지 효율을 보장하지 않음을 강조합니다.
...
### Noise-Robust Target-Speaker Voice Activity Detection Through Self-Supervised Pretraining (https://arxiv.org/abs/2501.03184)
Comments:
...
- **What's New**: 이 논문에서는 TS-VAD(Target-Speaker Voice Activity Detection) 성능을 향상시키기 위해 새로운 Self-Supervised Learning(SSL) 프레임워크인 Denoising Autoregressive Predictive Coding (DN-APC)를 제안합니다. 기존의 모델들이 지도 학습 데이터에 의존했던 반면, DL-APC는 레이블이 없는 데이터로 사전 훈련을 진행하여 환경 변화를 일반화하는 데 중점을 두었습니다. 실험 결과, DN-APC는 다양한 잡음 조건에서도 약 2%의 성능 개선을 보여주었고, FiLM conditioning이 가장 뛰어난 성능을 보였습니다.

- **Technical Details**: TS-VAD는 타겟 스피커의 음성을 감지하는 작업으로, 신호 처리의 진보와 심층 신경망(DNN)의 사용이 이 작업의 많은 발전을 이끌어냈습니다. TS-VAD 모델은 보통 타겟 스피커의 특징 정보를 활용하여 맞춤형 출력을 생성하는 방식으로 작동합니다. 특히, 논문에서는 causal Conformer 기반 아키텍처를 업데이트하여 LSTM 기반 모델보다 성능을 개선하였습니다.

- **Performance Highlights**: 실험 결과, DN-APC로 사전 훈련된 TS-VAD 모델은 청정 환경에서도 뛰어난 성능을 보였으며, 노이즈 환경에서도 보다 강건하게 작동하는 것으로 나타났습니다. 또한, 다양한 스피커 conditioning 방법이 TS-VAD 성능에 미치는 영향을 분석했으며, 사전 훈련된 모델이 노이즈에 대한 더 나은 클래식 분리를 이루는 것을 보여주었습니다. 이 연구는 SSL이 TS-VAD 모델에서 어떻게 성능을 개선할 수 있는지를 명확하게 나타냅니다.
...
### Slim multi-scale convolutional autoencoder-based reduced-order models for interpretable features of a complex dynamical system (https://arxiv.org/abs/2501.03070)
- **What's New**: 최근 몇 년간, 데이터 기반의 딥러닝 모델들이 난류 동적 시스템 분석에 대한 관심이 증가하고 있습니다. 본 논문에서는 기존의 Convolutional Autoencoders (CAEs)보다 해석 가능성이 우수한 새로운 CAE 접근 방식을 제안합니다. 이 방법은 시스템에 대한 사전 지식 없이 데이터로부터 비선형 변환을 배우면서도, 높은 차원의 유체 흐름 데이터에 대한 해석성을 제공합니다.

- **Technical Details**: 제안된 CAE 방법은 기존 CAE 아키텍처에 소규모 수정으로 쉽게 통합될 수 있으며, 훈련 프로세스에서 소수의 변경만을 필요로 합니다. 이 모델은 Proper Orthogonal Decomposition (POD)뿐만 아니라 두 가지 다른 해석 가능한 CAE 기법과 비교되었습니다. 연구에서는 다양한 복잡성의 세 가지 실험적 난류 레일리-베나르 대류 데이터셋을 적용하였으며, 제안된 방법은 경량화되었고 훈련이 용이하여 64 모드 기준으로 POD보다 최대 6.4%의 상대적 재구성 성능 향상을 달성했습니다.

- **Performance Highlights**: 제안된 방법은 POD와 유사한 해석 가능한 특징을 제공하며, 기존 CAE 접근 방식보다 자원 소모가 훨씬 적고 2% 미만의 매개변수만을 사용합니다. 이 접근 방식은 해석 가능성을 재구성 성능과 거래하는 기존 방법들과 달리 두 가지 모두를 달성할 수 있도록 설계되었습니다. 결과적으로 이 방법은 사용자가 후속 분석을 위해 필요에 따라 더 작은 수의 잠재적 특징을 선택할 수 있는 유연성을 제공합니다.
...
### Group Shapley with Robust Significance Testing and Its Application to Bond Recovery Rate Prediction (https://arxiv.org/abs/2501.03041)
- **What's New**: 이번 연구에서는 전통적인 개별 수준의 Shapley value 프레임워크를 확장하여 그룹 Shapley 값을 제안합니다. 이 방법은 경제 및 비즈니스 데이터에서 자주 나타나는 예측 변수의 구조적 특성을 다루며, 그룹의 중요성을 평가하는 데 유용합니다. 특히, 세 가지 누적 모멘트 카이 제곱 근사를 기반으로 한 유의성 검정 절차를 개발하고, 이를 통해 그룹 Shapley 값의 비대칭 속성을 확립했습니다.

- **Technical Details**: 제안된 Tree Group SHAP 방법은 의사결정 트리 구조를 활용하여 그룹 Shapley 값을 계산합니다. 기존 방법의 통계적 한계를 해결하기 위해 세 가지 누적 모멘트 카이 제곱 근사 방식의 유의성 검정 절차를 발전시켜 다양한 데이터 조건에서도 강력한 성능을 보여줍니다. 특히, 희소하거나 왜곡된 분포 및 작은 샘플 크기의 경우에도 검정을 수행할 수 있습니다.

- **Performance Highlights**: 시뮬레이션 연구에서 제안된 테스트 통계량은 다양한 시나리오에서 기존 방법을 능가하는 성능을 보였습니다. 정상 상태에서 27개 구성을 평가한 결과, 24개 구성에서 우수한 성능을 나타내었으며, 분포가 정상적이지 않은 경우에도 모두 경쟁 방법에 비해 우수한 성능을 유지했습니다. 이 연구는 금융 분야의 Explainable AI 발전에 대한 기여를 확인하며, 그룹 Shapley 가치는 기능 그룹 중요성을 보다 공정하게 할당함을 보여주었습니다.
...
### NeuroPMD: Neural Fields for Density Estimation on Product Manifolds (https://arxiv.org/abs/2501.02994)
- **What's New**: 본 연구는 제품 리만 다양체(product Riemannian manifold) 도메인에서 밀도 추정(density estimation)을 위한 새로운 심층 신경망(deep neural network) 방법론을 제안합니다. 기존의 밀도 추정 방법들의 한계를 극복하기 위해, 이 네트워크는 밀도 함수의 파라미터화(parameterization)를 직접 수행하고 최대 우도(Maximum Likelihood) 프레임워크를 사용하여 훈련됩니다. 이는 고차원 제품 다양체 도메인에서 발생하는 문제를 효과적으로 처리하여 전통적인 커널 밀도 추정기(kernel density estimator)보다 뛰어난 성능을 보여줍니다.

- **Technical Details**: 제안한 방법은 고차원(manifold) 데이터에서 밀도 추정을 위해 깊은 신경망을 활용하며, 특이하게도 매니폴드 미분 연산자를 사용하여 패널티 용어(penalty term)를 형성합니다. 이 방법은 전통적인 비모수(nonparametric) 방법과 비교하여 계산 복잡성을 대폭 줄이고, 특히 큰 표본 크기(large sample size)와 고차원(high-dimensional) 설정에서의 통계적 도전 과제를 극복합니다. 심층 신경망을 기반으로 한 밀도 추정기는 대량의 데이터 포인트에 빠르게 예측을 수행할 수 있습니다.

- **Performance Highlights**: 광범위한 시뮬레이션과 실제 뇌 구조 연결성 데이터(brain structural connectivity data)에 대한 응용을 통해 제안한 방법이 기존 대안들보다 명확한 이점을 가진다는 것을 보여주었습니다. 특히 제안된 모델이 고해상도 신경 이미징(neuroimaging) 기술을 활용하여 얻은 데이터에서 더 우수한 밀도 추정 성능을 발휘한다는 것이 입증되었습니다. 또한, 이 방법은 계산의 효율성을 높이며, 기존의 연결성 분석 방법들이 손실시키는 고해상도 정보를 최대한 활용할 수 있는 가능성을 엿봅니다.



### Classifier Weighted Mixture models (https://arxiv.org/abs/2501.02989)
- **What's New**: 이 논문에서는 표준 혼합 확률 모델의 확장을 제안하고 있습니다. 기존의 고정 혼합 가중치를 분류기를 사용하여 정의된 기능적 가중치로 대체하는 방식입니다. Classifier Weighted Mixtures는 혼합 구성 요소의 수를 늘리거나 복잡성을 증가시키지 않으면서도 밀도 평가(density evaluation)와 샘플링(sampling)을 용이하게 하고 변별적 추정(variational estimation) 문제에서의 표현력을 향상시킵니다.

- **Technical Details**: 제안한 모델은 기존의 혼합 모델에 비해 보다 유연한 기법을 제공하며, 이로 인해 더 나은 성능을 기대할 수 있습니다. 분류기를 기반으로 한 가중치 정의는 각 데이터 포인트의 특성을 보다 명확하게 반영할 수 있게 해줍니다. 이 논문은 구체적인 수학적 정의와 함께 이러한 접근 방식의 구현을 상세히 설명하고 있습니다.

- **Performance Highlights**: 모델의 성능은 여러 실험을 통해 입증되었습니다. 기존 모델과 비교하여 유사한 또는 더 나은 성능을 보여주면서, 가중치의 유연성 덕분에 데이터의 다양한 특성을 효과적으로 포착할 수 있다는 것을 보였습니다. 특히, 변별적 추정에서의 성능 향상이 돋보이며, 이는 실용적인 응용 가능성을 나타냅니다.
...
### A Point Process Model for Optimizing Repeated Personalized Action Delivery to Users (https://arxiv.org/abs/2501.02961)
Comments:
          13 pages

- **What's New**: 이 논문은 온라인 광고 시스템에서 사용자-광고주 상호작용에 영감을 받아 인과 추론(causal inference) 문제가 새로운 형식으로 정립되었다는 점이 신선합니다. 특히, 이전 관측을 바탕으로 사용자의 행동을 개인화(personalization)할 수 있는 방식과 시스템의 다중 개입 가능성에 대한 논의가 포함되어 있습니다. 이 접근법을 통해 신뢰성 있는 인과 관계 모델링이 가능해질 것으로 기대됩니다.

- **Technical Details**: 제안된 모델은 사용자의 행동을 관찰하고, 그에 따른 행동 조치를 취하는 과정에서 정보를 업데이트하는 Bayes 방법론을 사용합니다. 세 가지 주요 부분이 있습니다: 첫 번째는 초기 관측을 기록하는 것이며, 두 번째는 행동 조치가 이루어진 후의 관측을 업데이트하고, 세 번째는 사용자에게 전달된 행동이 서로 간섭하지 않는다는 SUTVA 가정입니다. 이 모델은 통계적 분석을 통해 과거 데이터가 매개변수의 특정 값에 대한 지지를 얼마나 잘 제공하는지를 평가하는 데 사용할 수 있습니다.

- **Performance Highlights**: 이 연구는 Bayesian 결정 이론을 인과적 문제에 적용하며, 다중 사용자를 대상으로 개인화된 행동을 지속적으로 제공하는 방안을 제시합니다. 제시된 알고리즘은 기존의 Reinforce 알고리즘을 기반으로 하여 예측된 유틸리티를 극대화하여 사용자 행동에 적절하게 대응할 수 있는 방법론을 제시합니다. 이는 기존 강화 학습(reinforcement learning) 틀을 넘어서는 접근법으로, 사용자의 특성과 시스템의 반응을 고려한 실용적인 솔루션으로서 주목받을 것입니다.
...
### Improved Approximation Algorithms for Low-Rank Problems Using Semidefinite Optimization (https://arxiv.org/abs/2501.02942)
Comments:
          30 pages, 5 figures, plus references and appendices

- **What's New**: 이 연구에서는 Goemans-Williamson 알고리즘에 영감을 받아 저랭크 최적화 문제에 대한 유사한 relax-then-sample 전략을 제안합니다. 구체적으로, 정칙 제약이 있는 이차 최적화 문제에 대해 반정의(semi-definite) 이완과 무작위 반올림(randomized rounding) 기법을 도출하여 근사 최적 해를 얻습니다. 이들은 Max-Cut 문제의 Bluprint을 모방한 것입니다.

- **Technical Details**: 연구에서는 먼저 정칙 제약이 있는 이차 최적화 문제에 대한 반정의 이완(semi-definite relaxation)을 제시합니다. 이어서 저랭크 최적화 문제를 위한 새로운 반정의 이완을 개발하였고, 이전 연구들보다 더 강력하고 폭넓게 적용 가능한 방법론을 제안했습니다. 더욱이, 대규모 반정의 매트릭스가 결정 변수로 도입되었으나, 최적 해를 변경하지 않고 대부분의 블록을 생략할 수 있어 접근 방식의 확장성을 향상시킵니다.

- **Performance Highlights**: 여러 사례를 통해 연구에서는 매트릭스 완성(matrix completion), 기저 추구(basis pursuit), 축소된 랭크 회귀(reduced-rank regression) 문제에서 이완의 크기를 줄이는 방법을 제시합니다. 실험적으로도 이차 최적화 문제 및 매트릭스 완성 문제에서 이완 및 샘플링 기법의 효과성과 확장성을 수치적으로 보여주고 있습니다. 이러한 결과는 저랭크 최적화 문제에 대한 새로운 알고리즘 설계의 가능성을 제시합니다.



### A Bayesian Approach for Discovering Time- Delayed Differential Equation from Data (https://arxiv.org/abs/2501.02934)
- **What's New**: BayTiDe(배이지안 타임 딜레이드 미분 방정식 발견 방법)는 데이터에서 TDDE(Time-Delayed Differential Equations)를 효과적으로 추론할 수 있는 새로운 접근법입니다. 기존의 방법들이 큰 시간 지연에 대해 저조한 성능을 보이는 반면, BayTiDe는 입력 데이터의 해상도에 비례하여 정확성을 유지하면서도 대규모의 시간 지연을 처리할 수 있습니다. 이 알고리즘은 베이지안 추론(Bayesian inference)과 희소성 유도( sparsity-promoting ) 프리오르(prior)를 결합하여 지연 미분 방정식을 식별하는 능력을 제공합니다.

- **Technical Details**: BayTiDe는 베이지안 접근 방식을 채택하여 불확실성을 정량화할 수 있는 장점을 가지고 있습니다. 이렇게 함으로써, 단순한 점 추정치뿐 아니라 매개변수의 사후분포(posterior distribution)도 제공하며 이는 모델의 신뢰성을 평가하는 데 중요한 역할을 합니다. 또한, 이 방법은 데이터가 소음(noise) 또는 불완전한 경우에도 강인한 성능을 유지하며 다양한 동적 행동을 갖는 시스템에도 적응할 수 있는 특성을 가지고 있습니다.

- **Performance Highlights**: BayTiDe는 불량한 데이터에도 큰 저항력을 보이며, 처음 조건이 훈련 세트와 크게 다를 경우에도 정확한 예측을 제공합니다. 이 알고리즘은 두 가지 상이한 안정성 특성을 지닌 시스템에서도 효과적으로 작동할 수 있으며, 주기 시스템과 비주기 시스템 모두에서 방정식을 성공적으로 발견할 수 있습니다. 이러한 특성 덕분에 BayTiDe는 여러 실세계 응용에 적합한 솔루션으로 자리 잡을 것이 기대됩니다.
...
- **What's New**: 이 논문은 화학 조성(Chemical Composition)만을 바탕으로 전자 밴드 갭(Band Gap)을 예측하는 간단한 머신러닝(Machine Learning) 모델을 소개합니다. 이전 연구에서 제시된 통계적 모델링을 바탕으로, 혼합 랜덤 변수(Mixed Random Variable) 모델링 접근법을 사용하여 예측 문제를 재구성했습니다. 모델은 각 화학 원소에 대한 하나의 파라미터(Parameter)를 포함하고, 이 파라미터의 가중 평균을 계산하여 밴드 갭을 예측하는 형태입니다.

- **Technical Details**: 모델은 각 원소의 가중 평균을 계산하고, 그 결과가 0보다 작지 않도록 ReLU(Rectified Linear Unit) 함수를 적용합니다. 이는 전자 밴드 갭이 연속 랜덤 변수가 아닌 비연속적인 성격을 가지고 있다는 점을 반영한 설계입니다. 따라서 이 모델은 화학적 해석 가능성(Chemical Interpretability)을 제공하며, 각 원소의 파라미터가 큰 경우 해당 물질의 밴드 갭이 클 가능성이 높음을 의미합니다.

- **Performance Highlights**: 이 모델은 전통적인 DFT(밀도함수 이론, Density Functional Theory)나 GNN(그래프 신경망, Graph Neural Networks)보다 계산 비용이 적고, 구조 정보 없이도 작동할 수 있다는 장점이 있습니다. 이로 인해 다양한 물질의 밴드 갭을 예측할 수 있어, 특히 실험 또는 계산이 어렵거나 비싼 경우에 유용합니다. 연구 결과는 다수의 화학 원소를 고려한 모델이 정밀하면서도 해석 가능한 예측을 가능하게 함을 보여줍니다.
...
- **What's New**: 이 논문은 딥 강화 학습(Deep Reinforcement Learning, RL)을 기반으로 하는 제어기를 통해 로봇의 민첩성과 정교한 조작 능력을 향상시킨 사례를 소개합니다. NVIDIA Isaac Sim과 같은 현대의 툴 및 시뮬레이션 플랫폼을 활용하여 이동 로봇의 지역 계획(local planning) 및 장애물 회피(obstacle avoidance)를 시연합니다. 특히 외부 감각(exteroception) 접근 방식을 표준화 및 재현 가능하게 만드는 데 집중하고, 사용자 정의 로봇이 시뮬레이션 환경에서 훈련될 수 있는 기본 프레임워크를 제공합니다.

- **Technical Details**: 이 연구는 Isaac 시뮬레이터에서 훈련된 RL 정책의 제로 샷 전이(zero-shot transferability)를 Demonstrate하며, 로봇 운영 체제(Robot Operating System, ROS) 내에서 최신 내비게이션 스택(Nav2)과 비교하여 종단 간(end-to-end) 정책을 벤치마킹합니다. 또한, 다양한 시뮬레이션 로봇을 가지고 수행된 테스트를 통하여 학습된 정책의 일반화 가능성을 보여줍니다. 이 과정에서 Gazebo 테스트 및 ROS 2를 활용해 여러 단계의 로봇을 위한 실시간 추론을 지원합니다.

- **Performance Highlights**: 최종 벤치마크 결과는 Nav2와 비교할 수 있는 성능을 보여주며, 사용자 정의 로봇 플랫폼을 위한 최첨단 종단 간 지역 계획기(deep local planners)의 빠른 배포 가능성을 확인합니다. 더 나아가, 상태(state) 및 행동(action) 공간이나 작업 정의(task definitions)를 확장함으로써 더욱 복잡한 임무를 수행할 수 있는 가능성을 열어줍니다. 이 논문은 Isaac Sim 훈련 및 Gazebo 테스트를 통한 RL 정책을 배치하는 데 고려해야 할 중요한 단계 및 측면을 소개합니다.
...
### ParetoLens: A Visual Analytics Framework for Exploring Solution Sets of Multi-objective Evolutionary Algorithms (https://arxiv.org/abs/2501.02857)
Comments:
          Accepted by IEEE Computational Intelligence Magazine

- **What's New**: 이번 논문에서는 다목적 진화 알고리즘(Multi-Objective Evolutionary Algorithms, MOEAs)에서 파생된 해결책들의 검토 및 탐색을 개선하기 위한 시각 분석 프레임워크인 ParetoLens를 소개합니다. ParetoLens는 모듈화되고 알고리즘에 구애받지 않는 설계를 바탕으로 하여 다양한 MOEA를 지원할 수 있는 다각적 시각화를 제공하여 의사결정 공간과 목표 공간에서의 솔루션 분포를 면밀히 검토할 수 있게 합니다. 이 프레임워크는 고정된 형식의 정적 시각화의 한계를 극복하고, 보다 유연하고 심층적인 분석을 촉진합니다.

- **Technical Details**: ParetoLens는 MOEAs가 생성한 솔루션 세트를 조사하고 탐색하기 위해 최적화된 시각화 도구로, 데이터 분포의 직관적인 시각화를 제공합니다. 모듈화된 디자인을 통해, 사용자는 실시간 상호작용을 통해 솔루션 세트 내의 중요한 패턴을 더 잘 이해하고 탐색할 수 있습니다. 이 프레임워크는 다차원 분석과 데이터 필터링과 같은 반복적이고 다면적인 접근 방식을 허용하여 복잡한 데이터셋의 해석과 탐색에서의 유용성을 극대화합니다.

- **Performance Highlights**: 이 프레임워크의 유용성은 사례 연구 및 전문가 인터뷰를 통해 평가되었으며, 다양한 MOEA와 벤치마킹 테스트 문제에 대한 실질적인 분석을 통해 복잡한 패턴을 발견하고 다목적 최적화 솔루션 세트에 대한 깊은 이해를 돕는 가능성을 보여주었습니다. ParetoLens는 웹 기반 구현으로 높은 사용자 친화성과 상호작용성을 지원하며, 연구자들이 데이터 분석 과정에서 활용할 수 있는 효과적인 도구로 나타났습니다.
...
### RAHN: A Reputation Based Hourglass Network for Web Service QoS Prediction (https://arxiv.org/abs/2501.02843)
Comments:
          4 pages,3 figures

- **What's New**: 이 논문에서는 웹 서비스 추천에 있어 품질(QoS) 예측의 효율성과 정확성을 높이기 위한 새로운 방법론을 제안합니다. 구체적으로, 명성과 딥러닝(Deep Learning) 기술을 결합한 RAHN 네트워크를 개발하였으며, 여기에는 Reputation Calculation Module (RCM), Latent Feature Extraction Module (LFEM), QoS Prediction Hourglass Network (QPHN)이라는 세 가지 모듈이 포함됩니다. 이러한 모듈은 부정확한 사용자 및 서비스로부터의 영향을 받을 수 있는 QoS 정보를 개선하는 데 중점을 두었습니다.

- **Technical Details**: RAHN 네트워크는 RCM을 통해 사용자와 서비스의 평판을 계산하고, LFEM은 알려진 정보로부터 잠재 특성을 추출하여 초기 특성 벡터를 형성합니다. QPHN은 Attention Mechanism을 사용하여 다양한 규모의 잠재 특성 벡터를 집계하며, 여러 번 쌓아서 최종 잠재 특성 벡터를 예측에 사용합니다. 이를 통해 RAHN은 웹 서비스 추천의 신뢰성과 정확성을 높일 수 있는 잠재력을 발휘합니다.

- **Performance Highlights**: 실험 결과, RAHN의 Mean Absolute Error (MAE)와 Root Mean Square Error (RMSE)는 기존의 여섯 가지 기본 방법들보다 더 낮은 수치를 기록하였습니다. 이는 기존 연구와 비교하여 RAHN이 효과적으로 QoS 예측을 수행할 수 있음을 입증한 것입니다. 분석된 데이터는 실제 QoS 데이터로 구성된 대규모 데이터셋으로, 신뢰성 있는 결과를 제공하고 있습니다.
...
### Proteomic Learning of Gamma-Aminobutyric Acid (GABA) Receptor-Mediated Anesthesia (https://arxiv.org/abs/2501.02824)
- **What's New**: 이 연구는 GABA 수용체를 표적으로 하여 새로운 마취제를 개발하는 데 있어 혁신적인 접근을 제시합니다. 기존의 마취제의 부작용을 줄이고 효과를 개선하기 위해 약 4,000개 이상의 단백질과 150만 개 이상의 특정 화합물을 대상으로 하는 단백질-단백질 상호작용 네트워크를 분석했습니다. 이러한 기술을 통해 마취제의 선택성을 높이고 안전성을 향상시킬 수 있는 가능성을 열었습니다.

- **Technical Details**: 이 연구는 24개의 GABA 수용체 아형에 대한 단백질 상호작용 네트워크 구성과 이를 통한 마취제 후보 물질 추출을 중심으로 이루어졌습니다. 머신러닝 알고리즘을 활용하여 180,000개 이상의 약물 후보를 평가했으며, 구조 최적화와 ADMET 특성 분석을 통해 효과적인 마취 후보 물질을 선별했습니다. 특징 벡터는 변환기(transformer) 모델과 오토인코더(autoencoder)를 통해 생성되었습니다.

- **Performance Highlights**: 이 연구에서 개발된 플랫폼은 약물의 안정성과 효능을 극대화하며 새로운 마취제 개발을 가속화할 것으로 기대됩니다. GABRA5 수용체를 목표로 한 후보 물질들에 대한 종합적인 스크리닝이 이루어졌으며, FDA 승인 약물 및 기존 의약품의 부작용 최적화를 통해 안전한 마취제 개발의 가능성을 제시했습니다. 전반적으로, 이 접근 방식은 새로운 마취 약물의 설계 및 고도화에 기여할 것으로 예상됩니다.



### Analogue Forecast System for Daily Precipitation Prediction Using Autoencoder Feature Extraction: Application in Hong Kong (https://arxiv.org/abs/2501.02814)
Comments:
          16 pages, 10 figures

- **What's New**: 홍콩 기상청의 탁월한 예측 능력을 가진 Analogue Forecast System (AFS)이 기계 학습을 이용해 강화되었습니다. 이 개선된 AFS는 유럽 중기 기상 예보 센터(ECMWF)의 결정론적 모델 출력과 유사한 기상 패턴을 가진 과거 사례를 기반으로 예측하는 방식을 제공합니다. 특히, 깊이 학습 기술인 autoencoder를 사용하여 더욱 효과적인 데이터를 처리합니다.

- **Technical Details**: 강화된 AFS는 ECMWF Reanalysis (ERA5)의 다섯 번째 세대 데이터셋을 활용하여 더 높은 해상도의 기상 요소를 포함하고 있습니다. 이 시스템은 네 가지 주요 단계로 이루어집니다: (1) 그리드화된 ERA5 및 ECMWF 모델 예측의 전처리, (2) 사전 훈련된 autoencoder에 의한 특징 추출, (3) 과거 사례를 기반으로 한 최적화된 특징 가중치 적용, (4) 최고 유사 사례를 바탕으로 한 가중치 앙상블을 이용한 최종 강수 클래스 계산입니다.

- **Performance Highlights**: 강화된 AFS는 2019년부터 2022년까지의 검증 기간 동안 기존 AFS보다 일관되게 우수한 성능을 보여주었습니다. 특히, 강우량이 많은 사례를 포착하는 데 있어 뛰어난 효과를 나타냈습니다. 이 연구는 홍콩의 강수 예측을 지원하는 데 있어 개선된 AFS의 장점과 한계에 대해 논의합니다.



### GraphDART: Graph Distillation for Efficient Advanced Persistent Threat Detection (https://arxiv.org/abs/2501.02796)
Comments:
          "This work has been submitted to the IEEE for possible publication."

- **What's New**: 사이버 물리적 사회 시스템(CPSSs)의 보안 문제는 최근 몇 년간 증가하고 있는 정교한 위협, 특히 Advanced Persistent Threats (APTs)로 인해 더욱 복잡해지고 있습니다. 본 논문에서는 GraphDART라는 새로운 모듈형 프레임워크를 소개하여, 원천 그래프를 압축된 정보 표현으로 변환하여 효과적인 이상 탐지를 가능하게 합니다. 이 프레임워크는 다양한 그래프 증류 기술을 활용해 필수적인 구조적 및 맥락적 정보를 보존한 채로 대규모 그래프를 간소화합니다.

- **Technical Details**: GraphDART는 그래프 증류(Graph Distillation) 기반으로 설계되어 APT 탐지에서 대규모 프로보넌스 그래프의 크기를 크게 줄입니다. 기존의 그래프 신경망(GNN) 방법들보다 더 효율적으로 그래프를 처리할 수 있으며, 이 과정에서 계산 오버헤드를 상당히 줄입니다. GraphDART는 백그라운드에서 발생하는 모든 이벤트를 포괄적으로 추적하기 위해 고유한 방법론으로 정의된 증류 과정을 통해 생성된 그래프를 사용합니다.

- **Performance Highlights**: 실험 결과, GraphDART는 기존의 최신 기술들을 능가하거나 동등한 성능을 달성하였으며, 도출된 그래프는 원래 프로보넌스 그래프보다 5% 크기 이하로 축소되었습니다. 이로 인해 APT 탐지의 효율성과 효과성을 동시에 향상시키는 결과를 보여주었습니다. 또한, GraphDART는 연관된 사이버 물리적 사회 시스템에서 악성 활동을 감지하는 데 있어 강력한 성능을 입증했습니다.
...
### Orthogonal greedy algorithm for linear operator learning with shallow neural network (https://arxiv.org/abs/2501.02791)
- **What's New**: 이번 논문에서는 Orthogonal Greedy Algorithm (OGA)을 이용해 얕은 신경망을 통해 선형 연산자 학습에 대한 새로운 접근 방식을 제안합니다. OGA는 커널 함수(kernal function)의 추정 및 편미분 방정식(Partial Differential Equations, PDE) 문제 해결에 있어서 최적의 근사 속도를 보장하며, 이를 통해 알고리즘의 효용성을 입증합니다.

- **Technical Details**: 새로운 반내적(semi-inner product) 구조를 도입하여 OGA 프레임워크를 설계하고, 이론적으로 수렴 속도를 분석했습니다. OGA는 데이터로부터 그린 함수(Green's function)를 근사하는 데 사용되며, 다양한 차원의 연산자 학습 문제를 다룹니다. 더불어, 점별 커널 추정(point-wise kernel estimation)을 통해 근사 성능을 더욱 향상시켰습니다.

- **Performance Highlights**: 제안된 OGA와 PW-OGA 방법은 기존의 모델들에 비해 정확성의 차원이 증가하여 우수한 성과를 보입니다. 특히, 1D에서 3D에 이르는 여러 연산자 학습 문제에서 적용되어 이론적으로 최적의 근사 속도를 달성하였습니다. 이 결과는 커널 추정 및 선형 PDE 문제에서의 OGA의 잠재적 가능성을 증명합니다.
...
### Beyond $\mathcal{O}(\sqrt{T})$ Regret: Decoupling Learning and Decision-making in Online Linear Programming (https://arxiv.org/abs/2501.02761)
Comments:
          Extension of conference submission this https URL

- **What's New**: 이번 논문에서는 온라인 선형계획법(Online Linear Programming)에서 보다 향상된 성능을 갖는 알고리즘의 개발을 다루고 있습니다. 특히, 기존의 첫 번째 차수(first-order) 방법이 보이는 평균적 손실(regret) $	extmath{O(oot{T})$ 보다 우수한 결과를 도출하기 위한 새로운 프레임워크를 제시합니다. 이를 통해 연속 연속적 지원(continuous support) 및 유한 지원(finite support) 설정에서 이전의 비매끄러움(non-degeneracy) 가정 이외의 조건 아래서도 $	extmath{o(oot{T})$와 $	extmath{O(	extroy{log} T)}$ 손실을 달성할 수 있음을 처음으로 입증하였습니다.

- **Technical Details**: 이 논문에서 제시된 알고리즘은 LP의 이중 문제(dual problem)가 특정 오류 경계 조건(error bound conditions)을 만족할 때의 성능 향상에 초점을 맞추고 있습니다. 독립 랜덤 변수(random variables)와 비정상(primal non-degenerate) 및 이중(non-degenerate) 문제를 고려하여, 고유한 해(unique solution)와 조건부 집합(conditional sets)에서의 특성을 탐구합니다. 이와 더불어 데이터와 최적 기준을 설정하여 효율성을 높이는 방법론을 개발하고 있습니다.

- **Performance Highlights**: 제안된 방법은 기존 최고의 성과(state-of-the-art)와 비교하여 상당한 개선을 보여주며, 연속적과 이산적인 조건 모두에서 손실의 경계를 줄이는 데 성공하였습니다. 이는 순차적 의사결정(sequential decision-making)에 대한 새로운 통찰력을 제공하여, 다양한 실제 문제에서의 적용 가능성을 높이고 있음을 나타냅니다. 체계적인 엄밀함을 바탕으로 하여 제안된 알고리즘은 향후 연구와 실제 문제 해결에 큰 기여를 할 것으로 예상됩니다.



### CHAT: Beyond Contrastive Graph Transformer for Link Prediction in Heterogeneous Networks (https://arxiv.org/abs/2501.02760)
- **What's New**: 본 논문에서는 이종 네트워크(link prediction in heterogeneous networks)에서 링크 예측(link prediction)의 복잡성을 해결하기 위해 Contrastive Heterogeneous grAph Transformer (CHAT)을 제안합니다. CHAT은 사전 정의된 메타 경로(meta-paths) 없이도 관심 있는 노드를 선택적으로 유지하는 샘플링 기반의 그래프 변환기(graph transformer) 기법을 도입합니다. 이 방법은 이종 네트워크에서 링크 예측을 위해 특별히 설계된 이중 측면 손실 함수(dual-faceted loss functions)를 사용하여 노드 시퀀스(node sequences)와 그 상호 연결을 고충실도로 인코딩합니다.

- **Technical Details**: CHAT은 이종 네트워크에서 링크 예측(link prediction)을 위한 집중화된 그래프 무작위워크 샘플링 기술(concentrated graph random-walk sampling technique)을 채택하여 관심 있는 노드를 선택하고, 샘플링된 시퀀스를 생성합니다. 이후 연결-aware transformer를 이용하여 샘플링된 시퀀스에서 노드와 연결을 인코딩합니다. 이 과정에서 연결-aware transformer는 연결된 노드 간의 근접성을 강화하고, 링크 예측을 위한 주석 대조 손실(supervised contrastive link prediction loss)과 관측 확률 손실(observation probability loss)로 지도 학습을 수행합니다.

- **Performance Highlights**: CHAT는 세 가지 실험적 약물-대상 상호작용(drug-target interaction) 데이터 세트를 사용하여 평가되었으며, 기존의 벤치마크와 DTI 예측에 특화된 최신 기술들보다 우수한 성능을 보였습니다. 이러한 결과는 CHAT이 복잡한 이종 네트워크에서의 링크 예측 문제를 효과적으로 해결할 수 있음을 입증합니다. CHAT의 혁신적인 접근 방식은 기존 방법들이 직면한 문제들을 해결하는 데 기여하고, 링크 예측 분야에 새로운 가능성을 제시합니다.
...
### Improving Quantum Machine Learning via Heat-Bath Algorithmic Cooling (https://arxiv.org/abs/2501.02687)
Comments:
          17 pages, 7 figures

- **What's New**: 본 연구는 양자 열역학에 기초한 새로운 접근 방식을 통해 양자 기계 학습(QML)에서 샘플 효율성을 극대화하는 방법을 제안합니다. 양자 감독 학습을 열역학적 냉각 과정으로 개념화하였으며, 이를 통해 훈련 및 예측 단계에서 샘플 효율성을 향상시키는 양자 냉장고 프로토콜을 개발하였습니다. 이 기술은 Grover 반복이나 양자 위상 추정과 같은 복잡한 연산 없이도 동작할 수 있도록 설계되었습니다.

- **Technical Details**: 제안하는 프로토콜은 엔트로피 압축과 열화 과정을 교대로 수행하여 큐빗의 엔트로피를 감소시키고, 지배적인 바이어스 방향으로 극성을 증가시킵니다. 제안된 양자 냉장고 프로토콜(Bidirectional Quantum Refrigerator, BQR)은 초기 극성 값에 대한 사전 지식 없이 작동하며, 서로 다른 두 고정점 중 하나로 자연스럽게 유도됩니다. 이로 인해 분류 점수 및 기울기를 추정하기 위해 필요한 측정 수가 크게 줄어들어 QML의 전반적인 계산 비용이 감소합니다.

- **Performance Highlights**: 이 연구에서 제안된 방법은 이론적 분석 및 수치적 분석을 통해 분류 점수 추정 및 기울기 측정에 필요한 측정 수를 유의미하게 줄이는 것으로 나타났습니다. 또한, 엔트로피 압축을 통한 효율적인 구현을 위해 k-local 압축 유니타리의 실용적인 적용을 제공합니다. 결과적으로, QML 알고리즘은 노이즈가 있는 중간 규모의 양자 장치에서 보다 실용적이고 효율적으로 작동할 수 있습니다.
...
### Re-examining Granger Causality from Causal Bayesian Networks Perspectiv (https://arxiv.org/abs/2501.02672)
- **What's New**: 이번 연구는 Granger 인과관계(Granger Causality, GC)에 대해 적절한 인과적 해석을 부여함으로써 GC의 정당성을 개선했습니다. 연구는 Reichenbach의 공통 원인 원칙(Common Cause Principles, RCCPs)과 인과적 베이지안 네트워크(Causal Bayesian Networks, CBNs)의 관점에서 GC를 분석하였습니다. 이러한 새로운 접근법을 통해 GC는 특정 가정 하에 적절한 인과적 해석을 가능하게 한다는 것을 이론적으로 및 그래픽적으로 입증했습니다.

- **Technical Details**: Granger causality는 시간적 데이터에서 인과 관계를 추론할 수 있는 계산 도구로, 관측 데이터에서 인과 구조 학습(Causal Structure Learning, CSL)을 통해 인과 관계를 파악하는 데 활용됩니다. Pearl의 인과 프레임워크를 채택하여 두 변수 간의 인과 관계를 정의하고, 이론적 기초를 통해 GC를 개정하여 새로운 알고리즘을 발전시켰습니다. 이 연구에서는 사고를 통해 CBN 관점에서 GC를 조건부(Conditional) 독립성 테스트로 해석하는 등 새로운 방법론을 제안하였습니다.

- **Performance Highlights**: 이 연구의 주요 기여는 GC 프레임워크에 인과적 추론을 부여하고, 이를 통해 시뮬레이션 데이터에서 만족스러운 결과를 얻었다는 것입니다. 다양한 기존 연구에서 GC의 한계와 비인과적 프레임워크로서의 비판이 있었으나, 본 연구는 이러한 의견을 극복하고 인과적 공개성을 증명했습니다. 연구 결과는 보다 신뢰할 수 있는 시간 시계열 데이터에서의 인과 관계 분석 방법을 제시함으로써, 복잡한 시스템 이해에 기여할 것입니다.
...
### LWFNet: Coherent Doppler Wind Lidar-Based Network for Wind Field Retrieva (https://arxiv.org/abs/2501.02613)
Comments:
          13 pages, 7 figures

- **What's New**: 이 연구에서는 전통적인 방법에 의존하지 않고, Lidar 기반의 Wind Field (WF) 회수 신경망인 LWFNet을 제안합니다. 이 모델은 Transformer와 Kolmogorov-Arnold 네트워크를 기반으로 하며, 기존의 바람 회수 알고리즘에서 파생된 대상만을 사용하여 훈련됩니다. 특히, LWFNet은 고해상도 바람 필드 탐지에서 최대 범위를 확장하고 더 높은 정확도를 달성하는 것을 목표로 합니다.

- **Technical Details**: LWFNet 모델은 전통적인 바람 회수 알고리즘을 통해 습득한 데이터와 라디오 sonding (radiosonde) 측정을 이용해 테스트 결과를 평가하는 구조입니다. 기존의 유사 연구와는 달리, Mie 산란(Mie scattering)이 발생하는 고도에서의 신호 강도 부족 문제를 해결하는 데 초점을 맞추고 있습니다. 연구에서는 LWFNet이 레이저 신호의 회수 정확성 향상과 함께, 기존 SOTA 모델과 비교했을 때 우수한 성능을 발휘하는 것을 보여줍니다.

- **Performance Highlights**: 실험 결과, LWFNet은 라이다 기반의 바람 필드 회수에서 놀라운 성능을 보여주며, 특히 높은 정밀도로 정확한 바람 데이터를 제공합니다. 'Super-accuracy'(슈퍼 정확도)라는 새로운 개념이 도입되어, 라벨된 대상으로부터 뛰어난 성능을 발휘하고 있음을 강조합니다. 이러한 성과는 고해상도 바람 탐지 분야의 미래 연구에 중요한 이정표가 되며, 심층 학습 모델의 발전에도 기여할 것으로 기대됩니다.
...
### Transformers Simulate MLE for Sequence Generation in Bayesian Networks (https://arxiv.org/abs/2501.02547)
Comments:
          51 pages, 17 figures, 5 tables

- **What's New**: 본 논문은 변환기(transformers)의 이론적 능력에 대해 심도 있는 분석을 제공합니다. 특히, 베이지안 네트워크(Bayesian network)를 기반으로 하는 독립적인 시퀀스를 생성하여 최대 우도 추정(maximum likelihood estimation)을 수행하는 변환기 모델의 존재를 증명하였습니다. 이러한 접근 방식은 변환기가 시퀀스 생성에서 복잡한 확률 모델을 학습할 수 있는 잠재력을 보여줍니다.

- **Technical Details**: 변환기는 조건부 확률(conditional probabilities)의 최대 우도 추정을 위해 ‘문맥 데이터셋(contextual dataset)’을 입력으로 사용하며, 이를 통해 새로운 샘플을 자가 회귀적으로 생성할 수 있습니다. 논문에서는 변환기가 이러한 복잡한 과제를 수행할 수 있는 간단한 모델이 존재함을 이론적으로 입증하고, 다양한 베이지안 네트워크 구조(확률적 체인, 트리 등)에 대해 실험을 진행하였습니다. 이를 통해 변환기가 모델을 학습하는 다양한 환경에서도 효과적으로 작동함을 보여주었습니다.

- **Performance Highlights**: 실험 결과, 변환기는 주어진 조건부 확률을 기반으로 새로운 변수를 샘플링할 수 있는 능력을 가지고 있음을 확인하였습니다. 베이지안 네트워크가 다양한 구조를 가지더라도, 변환기의 사전 훈련(pre-training)을 통해 효과적인 조건부 확률 추정을 수행할 수 있습니다. 또한 실제 데이터에서의 실험 결과는 이러한 주장을 더욱 강화해 주어, 변환기가 실제 환경에서도 우수한 성능을 발휘함을 입증하였습니다.



### Unified Guidance for Geometry-Conditioned Molecular Generation (https://arxiv.org/abs/2501.02526)
Comments:
          38th Conference on Neural Information Processing Systems (NeurIPS)

- **What's New**: UniGuide는 무조건적(conditional) 확산 모델을 위한 제어된 기하학적 지침을 제공하는 혁신적인 프레임워크입니다. 이 방법은 추가 학습이나 네트워크 없이 추론 동안 유연한 조건지를 가능하게 하여 약물 발견의 여러 작업에 효율적으로 적용할 수 있습니다. UniGuide는 구조 기반(structure-based), 조각 기반(fragment-based), 리간드 기반(ligand-based) 약물 설계에 유리한 성능을 보여 경쟁 모델보다 앞서는 잠재력을 입증하였습니다.

- **Technical Details**: UniGuide는 기하학적 조건을 통합하여 분자 생성을 유도하는 방법으로, 복잡한 기하학적 조건을 확산 모델의 구성 공간과 연계를 통해 자기 유도(self-guidance)를 가능하게 합니다. 이 방법은 기존의 조건부(diffusion) 모델이 주로 특정 작업에 제약되는 단점을 극복하며, 복잡한 기하학적 조건을 포괄적으로 처리할 수 있는 특별한 기능을 제공합니다. UniGuide는 훈련 데이터의 부족에도 불구하고 다양한 약물 발견 작업을 수행할 수 있는 가능성을 가지고 있습니다.

- **Performance Highlights**: UniGuide는 맞춤형 모델과 동등하거나 더 나은 성능을 보여줍니다. 이 방법은 비조건적 생성의 발전에 초점을 맞추어 여러 응용 프로그램에 직접적으로 이득을 제공합니다. 양질의 데이터가 부족한 생물학적 환경에서도 작업을 최소한의 데이터로 처리할 수 있어 실제적이고 유용한 방식으로 약물 설계에 적용될 수 있습니다.
...
### Predicting Vulnerability to Malware Using Machine Learning Models: A Study on Microsoft Windows Machines (https://arxiv.org/abs/2501.02493)
- **What's New**: 이 연구는 증가하는 사이버 위협 속에서 효과적인 악성코드 탐지 전략의 필요성을 다루고 있으며, Microsoft Windows Defender에서 수집한 대규모 데이터셋을 활용한 기계 학습(Machine Learning, ML) 기법을 이용하여 악성코드를 탐지합니다. 본 연구의 목표는 특정 머신의 조건에 기반하여 악성코드 취약성을 정확히 예측하는 고급 ML 모델을 개발하는 것입니다. 전통적인 서명 기반 탐지 방식에서 벗어나 역사적 데이터와 혁신적인 특징 엔지니어링을 통합하여 탐지 능력을 향상시키고 있습니다.

- **Technical Details**: 연구의 핵심은 Microsoft Windows Defender에서 수집된 대규모의 실제 데이터셋을 활용해 악성코드 탐지를 위한 ML 모델을 구축하는 것입니다. 전통적인 서명 기반 탐지 시스템 대신 동적 행동 분석에 중점을 둠으로써 다양한 특징을 분석하여 악성코드의 기초적인 지표를 식별합니다. 이를 통해 기업 환경에 적합한 모델을 제안하여 방어 전략을 보다 능동적으로 개선할 수 있습니다.

- **Performance Highlights**: 본 연구는 여러 기여를 포함하고 있으며, 첫째로 ML 알고리즘을 활용한 기존 악성코드 탐지 기술의 발전을 가져왔습니다. 둘째, 대규모 실제 데이터셋을 통해 연구 결과의 적합성을 보장하고 있습니다. 셋째, 특징 분석의 중요성을 강조하여 악성코드 감염의 주요 지표를 규명하고, 넷째, 기업 환경에 적합한 모델을 제안하여 새로운 위협으로부터 광범위한 네트워크를 보호할 수 있는 방안을 제공합니다.
...
### IRIS: A Bayesian Approach for Image Reconstruction in Radio Interferometry with expressive Score-Based priors (https://arxiv.org/abs/2501.02473)
Comments:
          17 pages, 8 figures, submitted to the Astrophysical Journal

- **What's New**: 이 논문에서는 전통적인 radio interferometry imaging 알고리즘과 비교하여, Score-based 모델을 이용한 새로운 방법론인 IRIS(Imaging for Radio Interferometry with Score-based models)를 제안합니다. 이 방법은 ALMA의 DSHARP 조사에서 얻은 시각화 데이터를 기반으로 하여 protoplanetary disks의 이미지를 추론하는 데에 Gaussian likelihood를 사용합니다. 과거의 더 전통적인 기법들에 비해 이 모델은 유의미한 posterior samples를 생성할 수 있음을 보여줍니다.

- **Technical Details**: Interferometry는 여러 안테나에서 수신된 신호를 결합하여 개별 단일 접시 망원경보다 높은 해상도로 우주를 탐색하는 기술입니다. 이 연구에서는 Bayes 추론을 위해 score-based generative models를 사용하여 radio interferometric imaging에 대한 새로운 접근 방식을 제시하고 있습니다. 이 과정에서, ALMA의 protoplanetary disk 사진에 대한 관찰 데이터를 활용하여, visibilities로부터 sky surface brightness를 복구하는 방법에 대해 설명합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 기존의 CLEAN 및 MEM 알고리즘보다 더 정확한 posterior samples를 생성하는 것을 보여줍니다. 또한, 시뮬레이션 테스트를 통해 이 접근 방식은 정확하고 공정한 결과를 제공함을 입증하였습니다. 따라서, IRIS는 radio astronomy 분야에서 새로운 가능성을 열어줄 것으로 기대됩니다.
...
### Transfer learning via Regularized Linear Discriminant Analysis (https://arxiv.org/abs/2501.02411)
- **What's New**: 이번 논문에서는 Linear Discriminant Analysis (LDA) 기법을 통해 높은 차원의 예측 변수와 소표본 문제를 해결하기 위해, Transfer Learning을 활용한 새로운 방법을 제안합니다. 보조 모델(source model)로부터의 데이터를 활용하여 목표 모델(target model)의 분류 성능을 향상시키는 접근 방식을 제시합니다. 특히, 정규화된 랜덤 효과를 통한 LDA 방법론을 사용하여, 각 모델에서의 추정치를 가중합으로 결합하는 방식을 설명합니다.

- **Technical Details**: 제안된 TL-RDA 방식은 두 클래스 사이의 공변량(mean)의 차이를 무작위량(δ, delta)으로 가정하여, 보조 및 목표 모집단에서의 naive RDA 추정치를 가중합하여 Bayes 최적 예측기를 추정합니다. 이 방법론은 고차원 설정에서의 가중치와 분류 오류율을 유도하며, $p/n ightarrow 	ext{γ}$의 형태를 갖습니다. 다양한 가중치 평가 전략이 논의되며, 무작위 행렬 이론(random matrix theory)을 기반으로 가중치의 기하학적 해석이 제공됩니다.

- **Performance Highlights**: 실험 결과로는 10년 심혈관 질환 위험 분류에 대한 프로테오믹스(Proteomics) 기반의 분석이 포함되며, 제안된 방식이 성능 향상에 효과적임을 보여줍니다. 특히, g의 다양한 가중치를 사용하여 TL-RDA 방법의 성능이 향상된다는 것을 실증적으로 입증하였습니다. 마지막으로, 다양한 가중치 선택에 대한 지침이 제공되어 실질적인 적용을 가능하게 합니다.
...
### On The Causal Network Of Face-selective Regions In Human Brain During Movie Watching (https://arxiv.org/abs/2501.02333)
- **What's New**: 본 연구는 DAGMA라는 새로운 인과 발견 방법을 사용하여 뇌의 얼굴 선택 네트워크의 인과 구조를 조사합니다. 이전의 연구에서는 얼굴 영역의 상호작용이 얼마나 중요한지를 파악하지 못했으나, 이번 연구를 통해 그 관계를 보다 명확히 이해하고자 합니다. 우리는 자연 영상 자극을 사용하여 인과 네트워크를 추출하고, 얼굴이 포함된 프레임이 그러한 네트워크에 미치는 영향을 분석했습니다.

- **Technical Details**: 연구에서는 fMRI 데이터와 특정 얼굴 선택적인 영역을 대상으로 DAGMA와 GOLEM 같은 그래픽 모델들을 비교 분석했습니다. Granger causality, Bayesian Networks, 그리고 GOLEM과 DAGMA의 인과 구조 학습 방법도 다루어졌습니다. 우리의 접근 방식은 대규모 연결망에서 숨겨진 인과 구조를 보다 정확히 식별할 수 있도록 설계되었습니다.

- **Performance Highlights**: DAGMA 방법은 인간의 뇌에서 얼굴 선택 네트워크의 인과 구조를 매우 효과적으로 추출하는 데 성공했으며, 영화 시청 과제에서 수집된 실제 fMRI 데이터를 통해 그 효과를 입증했습니다. 또한, 얼굴을 포함한 영화 프레임이 네트워크 내 인과 연결의 수와 강도에 미치는 인과적 영향을 보여주며, 피질하(subcortical) 영역의 필수적인 역할을 강조했습니다.



### Analysis of Fluorescence Telescope Data Using Machine Learning Methods (https://arxiv.org/abs/2501.02311)
Comments:
          12 pages; to be published in the proceedings of the 38th Russian Cosmic Ray Conference (2024)

- **What's New**: 이 논문은 고에너지 우주선(UHECR) 연구를 위한 형광 망원경(Fluorescence Telescopes, FTs)의 활용과 기계 학습(machine learning) 방법을 소개합니다. EUSO-TA라는 소형 망원경 데이터에 기반하여, 대기 샤워의 트랙 인식과 주요 입자의 에너지 및 도착 방향을 재구성하는 방법을 모색했습니다. 이 접근 방법을 다른 형광 망원경에서 활용할 수 있는 기회에 대해서도 논의하고 있습니다.

- **Technical Details**: EUSO-TA는 1m 직경의 두 개의 프레넬 렌즈를 갖춘 굴절형 망원경으로, 총 시야각(FoV)은 10.5°×10.5°입니다. 데이터 세트를 구축하기 위해 CONEX와 EUSO-Off 코드를 사용하여 5–100 EeV의 에너지 범위에서 생성된 대기 샤워(EASs)를 시뮬레이션했습니다. 모델 데이터에 대해 기계 학습 및 신경망(neural networks) 기법을 적용하여 트랙 인식과 재구성을 수행했습니다.

- **Performance Highlights**: 클래식한 기계 학습 방법인 기울기 부스팅(gradient boosting)을 통해 트랙 분할 모델을 개발하였으며, 이는 픽셀에 대한 이진 분류 문제를 해결합니다. 이 방법은 모델 아키텍처 설계나 데이터 정규화가 필요 없으며, 훈련 및 추론 시 그래픽 가속기가 필요 없는 장점을 가지고 있습니다. 결과적으로 128개의 프레임에서 약 295,000개의 훈련 포인트를 생성하였고, 트랙이 포함된 프레임의 경우 픽셀의 1-10%만 사용하는 방식으로 모델을 구축했습니다.
...
### Beyond Log-Concavity and Score Regularity: Improved Convergence Bounds for Score-Based Generative Models in W2-distanc (https://arxiv.org/abs/2501.02298)
- **What's New**: 본 논문에서는 Score-based Generative Models (SGMs)의 \\mathcal{W}_2-수렴 (\\mathcal{W}_2-convergence)에 대한 새로운 분석 프레임워크를 제시합니다. 기존의 엄격한 가정 대신, 약한 로그-볼록성 (weak log-concavity) 및 정규화 성질을 활용하여 데이터 분포의 수렴 과정을 규명합니다. 또한, Ornstein-Uhlenbeck (OU) 프로세스의 특성을 통해 시간이 지남에 따라 데이터 분포의 로그-볼록성으로의 진화를 설명하고 있습니다.

- **Technical Details**: SGMs는 샘플을 가우시안 노이즈로 변형하여 목표 분포로부터 샘플링하는 모델로, 이번 연구는 기존의 로그-볼록성 및 점수 정규성 (score regularity)에 대한 가정을 완화합니다. 논문에서는 해밀턴-자코비-벨만 (Hamilton-Jacobi-Bellman) 방정식에 대한 PDE 기반 분석을 통해 수렴 과정을 수치적으로 정량화합니다. 특히, 시간 반전 OU 프로세스의 드리프트 (drift)를 통해 볼록성의 동역학을 표현하고 있습니다.

- **Performance Highlights**: 연구에서는 가우시안 혼합 모델에 대한 명시적인 계산을 통해 제안된 프레임워크의 넓은 적용 가능성을 보여줍니다. 기존의 정규성 조건이 없이도 효율적으로 데이터 생성을 가능하게 하며, 이를 통해 다양한 복잡한 데이터 분포에 대한 SGM의 활용 가능성을 높입니다. 또한, 이러한 접근 방식은 컴퓨터 비전 및 자연어 생성 등의 다양한 분야에서도 유용할 것으로 기대됩니다.
...
### Robust Multi-Dimensional Scaling via Accelerated Alternating Projections (https://arxiv.org/abs/2501.02208)
- **What's New**: 본 논문은 감염된 거리로부터 점 위치를 재구성하는 견고한 다차원 스케일링(robust multi-dimensional scaling, RMDS) 문제에 대해 다룹니다. 전통적인 MDS 이론과 Robust Principal Component Analysis (RPCA)에 대한 최신 연구에서 영감을 받아, 제안한 알고리즘은 교대 프로젝션(Alternating Projection) 기반이며, 접선 공간 프로젝션(tangent space projection) 기법으로 가속화됩니다. 실험 결과, 제안한 방법이 기존 방법들보다 뛰어난 성능을 보이는 것을 확인했습니다.

- **Technical Details**: 이 연구에서는 결합된 최적화 방법을 적용하여, 일반화된 거리 행렬(EDM)에서 아웃라이어를 처리합니다. 관측된 거리 행렬 D는 실제 거리 행렬 D⋆와 아웃라이어 매트릭스 S의 합으로 표현됩니다. 제안하는 알고리즘의 강점은 아웃라이어가 충분히 희소한 경우, 재구성된 점들의 선형 수렴성을 보장할 수 있다는 것입니다.

- **Performance Highlights**: Numerical experiments에서 제안한 알고리즘은 아웃라이어만 있는 설정에서 성능을 입증하며, 노이즈와 아웃라이어가 동시에 존재하는 상황에서도 기존 최첨단 솔버들과 비교하여 우수한 장점을 보였습니다. 특히, 희소 아웃라이어의 양과 원래 점들의 특성 간의 관계도 명확히 설명되었습니다.
...
### Majorization-Minimization Dual Stagewise Algorithm for Generalized Lasso (https://arxiv.org/abs/2501.02197)
- **What's New**: 이 논문에서는 일반화된 라쏘( generalized lasso) 문제의 솔루션 경로를 효과적으로 추적하는 새로운 최적화 알고리즘인 MM-DUST(majorization-minimization dual stagewise) 알고리즘을 제안합니다. 이 알고리즘은 다양한 볼록 손실 함수에 대해 조정 가능하고, 비선형 및 비가우시안 모델에도 적용할 수 있는 장점을 가지고 있습니다. MM-DUST 알고리즘은 큰 데이터셋에서도 계산 효율성을 극대화하면서도 통계적 정확성을 유지할 수 있는 방법론을 제시합니다.

- **Technical Details**: MM-DUST 알고리즘은 세 가지 주요 개념에 기반을 두고 있습니다. 첫째, majorization-minimization 기법을 사용하여 복잡한 손실 함수에 대한 근사 목표 함수를 구성합니다. 둘째, 문제를 직접 해결하기보다는 이중 문제(dual problem)에 초점을 맞춰 박스 제약(box-constrained) convex 문제로 변환합니다. 마지막으로 스테이지-wise 하강 전략을 활용하여 이중 변수들을 업데이트하고, 각 업데이트에서 적절한 학습률(learning rate)을 설정하여 계산 효율성을 높입니다.

- **Performance Highlights**: 본 연구에서는 MM-DUST 알고리즘의 계산 복잡성을 분석하고 근사된 솔루션 경로의 균일 수렴(uniform convergence)을 확립했습니다. 시뮬레이션 연구와 정규화 로지스틱 회귀 및 콕스 모델에 대한 실제 적용 사례를 통해 제안한 접근법의 효과성을 입증하였습니다. 이로 인해 일반화된 라쏘 문제에 대한 새로운 경로 알고리즘이 기존보다 더 나은 성능을 발휘할 것으로 기대됩니다.
...
### SMDP-Based Dynamic Batching for Improving Responsiveness and Energy Efficiency of Batch Services (https://arxiv.org/abs/2501.02181)
Comments:
          Accepted by IEEE Transactions on Parallel and Distributed Systems (TPDS)

- **What's New**: 이 논문에서는 온라인 서비스 제공에 있어 응답 시간과 에너지 효율성 간의 균형을 맞추기 위한 동적 배치(dynamic batching) 기법을 제안합니다. 특히, 배치 크기가 서비스 시간에 의존하는 배치 서비스 큐(batch service queue) 모델을 기반으로 하여, 평균 응답 시간과 평균 전력 소비의 가중 합을 최소화하는 문제를 다루고 있습니다. 연구팀은 주어진 비즈니스 환경에 적합한 최적의 동적 배치 정책을 도출하는 방법을 제안하며, 기존 이론에 비해 63.5%의 공간 복잡도와 98%의 시간 복잡도를 감소시킬 수 있음을 확인하였습니다.

- **Technical Details**: 제안된 시스템은 패러렐(Parallel) 컴퓨팅 프로세서에서 배치 크기 결정에 대한 동적 정책을 개발하기 위한 반 마르코프 결정 과정(semi-Markov decision process, SMDP)으로 모델링되었습니다. 이 과정에서 요청의 도착이 포아송(Poisson) 분포를 따른다고 가정하고, 서비스 시간의 분포는 임의로 설정됩니다. 연구 결과, 배치 크기를 최적화하기 위해 새로운 유한 상태 근사와 모델 이산화(discretization), 그리고 상대 가치 반복(relative value iteration) 기법을 도입하여 근사 최적 정책을 도출하였습니다.

- **Performance Highlights**: 수치 결과에 따르면, SMDP 기반 배치 정책은 다양한 매개변수 설정하에서 다른 벤치마크와 비교하여 가장 낮은 평균 비용을 달성하였습니다. 또한, 제안된 동적 배치 기법은 서로 다른 트래픽 강도에 적응하는 능력을 가지고 있으며, 응답 시간과 전력 소비 간의 균형을 잘 유지할 수 있음을 보여주었습니다. 이러한 유연성은 특히 ML(as-a-Service) 플랫폼에서 중요한데, 이는 실시간으로 들어오는 요청을 효과적으로 처리하기 위한 근본적인 전략으로 제시됩니다.



### Molecule-dynamic-based Aging Clock and Aging Roadmap Forecast with Sundia (https://arxiv.org/abs/2501.02176)
- **What's New**: Sundial이라는 새로운 프레임워크가 도입되었으며, 이는 분자 동역학을 통해 노화 과정을 모델링합니다. 이 시스템은 인구 수준의 노화 과정과 개인 수준의 상대적인 노화 순서를 모두 포착합니다. 또한, Sundial은 생물학적 나이를 편향 없이 추정하고 노화 로드맵을 예측할 수 있게 해줍니다.

- **Technical Details**: Sundial은 확산 필드(diffusion field)를 이용하여 분자 동역학(molecular dynamics)을 모델링하며, 이는 개인의 노화 특성을 분석하는 데 유용합니다. 이 프레임워크는 슈퍼바이즈드 에이징 클락(supervised aging clocks)에서 발견된 편향을 극복할 수 있도록 설계되었습니다. 이를 통해 노화 속도가 빠른 개인이 더 높은 질병 위험을 보이는 경향이 있다고 밝혔습니다.

- **Performance Highlights**: Sundial은 개인 수준의 분석을 통해 나이 및 성별 별 노화 역학(age- and sex-specific aging dynamics)을 탐구할 새로운 기회를 열어줍니다. 또한, 건강한 노화 경로(faster yet healthy aging paths)에 대한 연구에 기여할 가능성이 큽니다.
...
- **What's New**: 이 논문은 ResNet-50 이미지 분류 모델의 FGSM(Fast Gradient Sign Method) 공격과 악성 페이로드 주입 같은 두 가지 주요 보안 위협에 대한 복원력을 분석합니다. 초기에는 클린 이미지에서 53.33%의 정확도를 기록했지만, FGSM 공격을 받을 때 전체 정확도는 변화가 없으나 잘못된 예측에 대한 자신감이 증대되었습니다. 또한, 93.33%의 샘플에서 페이로드 주입이 성공적으로 이루어져, 시각적 품질을 저하시키지 않고도 모델의 예측을 조작할 수 있는 방법이 드러났습니다.

- **Technical Details**: 본 연구에서는 ResNet-50이라는 50층의 심층 잔차 네트워크를 사용하여 이미지 분류의 적대적 공격을 탐구합니다. FGSM 공격은 손실 함수의 입력 데이터에 대한 기울기를 활용하여 적대적 예제를 생성하는 화이트박스 공격입니다. 연구는 사이버 보안과 적대적 공격 간의 교차점을 탐구하며, 악성 페이로드와 적대적 소음을 결합한 공격에 대한 모델의 복원력을 평가하려고 합니다.

- **Performance Highlights**: 결과적으로, 이 연구는 인기 있는 사전 훈련된 모델인 ResNet50이 적대적 조건에서 심각한 취약성을 보일 수 있음을 강조합니다. 작은 변형에도 불구하고 모델의 정확도가 크게 훼손될 수 있으며, 페이로드 주입 공격의 성공률 또한 측정되었습니다. 이는 얼굴 인식, 자율 주행 시스템, 접근 제어 메커니즘과 같은 보안이 중요한 실제 응용 분야에서의 잠재적 위험을 부각시킵니다.
...
- **What's New**: 이번 논문은 Maass forms에 대한 데이터를 과학적으로 조사하고 있으며, Fourier 계수를 평균화하는 과정을 통해 Fricke sign에 대한 새로운 패턴을 발견했습니다. 이 연구는 파라미터를 추가하여 패턴을 강조하였고, 기계 학습을 통해 Fricke sign의 예측 정확도를 96% 이상으로 달성했습니다. 또한, Hejhal의 알고리즘에 의해 제공된 추정값과의 비교를 통해 예측의 타당성을 입증했습니다.

- **Technical Details**: 연구에서 사용된 기계 학습 기법은 Linear Discriminant Analysis (LDA)와 신경망(neural networks)이며, 35,416개의 Maass form 데이터베이스를 활용했습니다. LDA를 통해 Fricke sign을 예측하는 과정에서, 짝수와 홀수 패러티에 대해 각각 96%와 94%의 정확도를 기록했습니다. Fricke sign이 알려지지 않은 경우에도 기계 학습 모델을 적용하여 예측 값을 도출하고, 이들 예측을 Hejhal 알고리즘의 추정값과 비교하여 신뢰성을 검증했습니다.

- **Performance Highlights**: 연구 결과는 기존 방법들과 크게 다르며, 데이터셋 전체를 고려한 기계 학습 도구를 활용하여 Fricke sign을 예측했습니다. 예측 결과는 Hejhal의 알고리즘과 약 95%의 일치를 보였으며, 네트워크 기반의 신경망도 LDA와 비슷한 결과를 도출했습니다. 향후 연구에서는 기계 학습 결과를 해석하고, Fricke sign 예측에 주요한 특징들을 규명하여 더 깊이 있는 이해를 시도할 예정입니다.



### Laws of thermodynamics for exponential families (https://arxiv.org/abs/2501.02071)
- **What's New**: 이 논문에서는 일반 지수 패밀리(exponential families) 관점에서 열역학의 법칙을 개발했습니다. 학습 문제를 최대 엔트로피(max-entropy)와 통계 역학(statistical mechanics) 문제에 맞춰 풀면서, 열역학의 결과를 학습 시나리오에 적용할 수 있습니다. 이러한 연장은 지수 패밀리가 어떻게 열역학 및 학습 평형 상태를 특성화하는지를 보여주어, AI와 통계에서의 분포 변화(distribution shift)를 정량화하고 해결하는 데 넓은 함의를 가집니다.

- **Technical Details**: 열역학 법칙을 일반 모델링 환경에서 공식적으로 도출함으로써, 우리는 물리 시스템에 적합하게 진술된 경우 열역학의 잘 알려진 법칙을 회복할 수 있습니다. 이 법칙의 일반 형식에는 지수 패밀리 분포가 포함되어 있으며, 이는 기존의 물리 연구와 우리의 모델링 시나리오를 연결합니다. 여기서 로스 손실(log loss)은 에너지로 해석되며, 데이터셋에 대한 관측을 통해 모델이 설명됩니다.

- **Performance Highlights**: 이 논문은 AI 및 통계 모델링에서 전통적인 열역학 도구를 적용할 수 있는 가능성을 열어줍니다. 통계 물리(statistical physics)의 결과는 지수 패밀리와 같은 효율적이고 실용적인 구조로 발전하여, 강력한 예측 모델을 제공하고 있습니다. 이를 통해 우리는 샘플의 분포를 관측하는 가능성을 자연스럽게 도출함으로써, 정보 이론(information theory)과 통계 이론(statistical theory) 간의 깊은 연관성을 이해할 수 있습니다.
...
### Modeling COVID-19 spread in the USA using metapopulation SIR models coupled with graph convolutional neural networks (https://arxiv.org/abs/2501.02043)
- **What's New**: 최근에 Graph Convolutional Networks (GCNs)는 데이터 집약적인 문제 해결에 큰 가능성을 보였습니다. 특히, GCN을 활용하여 Susceptible-Infected-Recovered (SIR) 모델의 예측 개선에 대한 연구가 이루어졌습니다. 일본의 precinct 수준 데이터에서는 GCN-SIR 하이브리드 접근 방식이 기존 방법론보다 우수한 성능을 보였으며, 본 연구에서는 이를 미국 대륙의 데이터에 적용하여 이동 패턴과 정책 반응에 따라 조정했습니다.

- **Technical Details**: 모델링 커뮤니티에서 감염병 확산을 설명하기 위해 compartment 모델인 SIR 모델이 널리 사용됩니다. SIR 모델은 감수성(Susceptible), 감염(Infected), 회복(Recovered) 세 가지 상태를 고려합니다. 이 연구에서는 메타인구(metapopulation) 구조와 GCN 방법론을 결합하여 COVID-19의 확산 예측을 수행하며, 모델의 예측 정확도를 검토합니다. 이를 통해 GCN-SIR 모델의 장점과 단점을 논의하며 질병 동역학 모델링에 대한 잠재력을 탐색합니다.

- **Performance Highlights**: GCN-SIR 모델은 COVID-19 확산 예측에 있어서 표준 SIR 모델과의 비교를 통해 다양한 시간적 변화에서 더 높은 예측 정확도를 보여주었습니다. 모델은 미국 내 다양한 하위 인구 집단을 분석하며, 전체 재생산 번호를 추정하는 데 기여하였습니다. 그러나 메타 인구 모델의 계산 복잡성과 더 많은 파라미터 추정이 필요하다는 점이 도전 과제로 남아 있습니다.
...
### Follow The Approximate Sparse Leader for No-Regret Online Sparse Linear Approximation (https://arxiv.org/abs/2501.00799)
Comments:
          12 pages, 5 figures, corrected title, added proof of a lemma in appendix

- **What's New**: 해당 논문에서는 '온라인 희소 선형 근사(Online Sparse Linear Approximation)' 문제에 대해 다루고 있습니다. 이러한 문제는 의료 시험, 웹 캐싱 및 자원 할당과 같은 다양한 응용 프로그램에서 발생하며, 이는 주어진 측정 행렬의 열을 조합하여 측정 벡터를 선형으로 근사하는 것입니다. 새롭게 제안된 방법인 'Follow-The-Approximate-Sparse-Leader'는 이 문제를 해결하기 위해 설계된 효율적인 온라인 메타 정책으로, 이론적인 분석과 실제 시뮬레이션을 통해 그 유효성이 입증되었습니다.

- **Technical Details**: 희소 선형 근사란 주어진 측정 벡터를 가장 잘 선형 근사할 수 있는 특정 개수의 열을 찾는 문제를 의미합니다. 본 논문에서는 온라인 정책의 성능을 평가하기 위한 정적 후회(static regret) 지표를 정의하고, 제안된 메타 정책인 FTASL의 정적 후회에 대한 상한을 확립했습니다. FTASL은 측정 시퀀스의 구조에 따라 로그 형식에서 제곱근 형식에 이르는 다양한 후회 보장을 제공할 수 있습니다.

- **Performance Highlights**: 제안된 FTASL 메타 정책은 이론적인 분석과 수치 시뮬레이션을 통해 실제로 그 효용성을 입증하였습니다. 이는 희소 복구 알고리즘을 활용하여 온라인 문제를 효과적으로 해결하는 데 기여합니다. 연구 결과는 기존의 오프라인 문제와 비교할 때 적은 후회를 달성할 수 있음을 보여주며, 다양한 실제 응용에 효과적으로 적용될 수 있음을 시사합니다.



### Residual connections provably mitigate oversmoothing in graph neural networks (https://arxiv.org/abs/2501.00762)
- **What's New**: 이 논문은 그래프 신경망(Graph Neural Networks, GNNs)에서 발생하는 오버스무딩(oversmoothing) 현상에 대한 이론적 분석을 제공합니다. 특히, 잔차 연결(residual connection)의 유무에 따른 심층 GNN의 비대칭 오버스무딩 비율을 조사하였습니다. 이결과들은 일반적인 경량성 기법들이 오버스무딩을 완화할 수 있음을 보여줍니다.

- **Technical Details**: 분석은 곱셈성 에르고딕 정리(multiplicative ergodic theorem)에 토대를 두고 있으며, 두 가지 주요 GNN 아키텍처, 즉 메시지 전달(graph message passing) 메커니즘의 MP-GNN 아키텍처에 대해 연구가 진행되었습니다. 특히, GCNs(Graph Convolutional Networks)와 GATs(Graph Attention Networks)를 포함한 두 가지 구조에 대한 이해를 깊게 하였습니다.

- **Performance Highlights**: 수치 실험을 통해 이론적 발견이 실증적으로 강력히 뒷받침된다는 것을 보여주었습니다. 연구결과는 MP-GNN이 심층 구조에서도 각 정점의 독립성을 유지할 수 있는 가능성을 제시하며, 이를 통해 복잡하고 대규모의 응용에서도 성능을 향상시킬 수 있음을 시사합니다.

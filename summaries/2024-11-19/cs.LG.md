...
### Competing Bandits in Decentralized Large Contextual Matching Markets (https://arxiv.org/abs/2411.11794)
- **What's New**: 이번 논문은 동적이고 비대칭적인 매칭 시장에서 에이전트들이 자원 제약 하에 안정적인 매칭을 이루기 위해 학습하는 방법을 탐구합니다. 기존 학습 알고리즘들이 비효율적이라는 점에서 착안하여 새로운 알고리즘을 제시하고 있습니다.

- **Technical Details**: 이 논문에서는 multi-agent(다중 에이전트) 환경에서 context-based(맥락 기반) 매칭을 모델링하고, 각 에이전트의 선호도가 시간의 흐름에 따라 변화하는 동적 매칭 시장을 다룹니다. 알고리즘의 성능은 linear contextual bandit framework(선형 맥락 밴딧 프레임워크)를 기반으로 하며, 에이전트의 획득 보상은 알려진 feature vector(특징 벡터)와 에이전트 특정 파라미터의 선형 함수로 나타낼 수 있습니다.

- **Performance Highlights**: 제안된 알고리즘은 무작위 샘플링으로 추정된 우상한 보상 평균에 기초하여 비례적으로 로그 스케일의 regret(후회)를 달성하며, K(arms의 개수)에 독립적으로 스케일링합니다. 이로 인해 새로운 알고리즘이 대규모 자원 제한 시장에서도 효율적으로 작동함을 보여줍니다.



### A Potential Game Perspective in Federated Learning (https://arxiv.org/abs/2411.11793)
- **What's New**: 연합 학습(Federated Learning, FL)에서의 클라이언트의 독립적인 훈련 노력을 게임 이론 기반 모형을 통해 탐구하며, 각 클라이언트의 보상과 훈련 노력의 상관관계를 분석합니다.

- **Technical Details**: 제안된 모형은 Nash 균형(Nash Equilibrium, NE)의 존재성을 증명하고, 동질적인 환경에서의 고유성을 조사하며, 최적 보상 요소를 통해 클라이언트의 훈련 노력이 크게 향상될 수 있음을 보여줍니다. FL 게임에서 최적 보상 요소는 서버의 최적 선택으로 식별됩니다.

- **Performance Highlights**: 현실 세계의 FL 시나리오에 적용된 결과, 특정 NE로부터 도출된 훈련 노력이 효율성을 입증하여, 클라이언트들이 자발적이고 효과적으로 기여할 수 있도록 유도하는 능력을 보여줍니다.



### LLM-IE: A Python Package for Generative Information Extraction with Large Language Models (https://arxiv.org/abs/2411.11779)
- **What's New**: 이 논문에서는 생물 의학 정보 추출을 위한 새로운 Python 패키지인 LLM-IE를 개발했다고 발표했습니다. 이 패키지는 정보 추출 파이프라인을 구축하는 데 필요한 다양한 기능을 제공합니다.

- **Technical Details**: LLM-IE는 named entity recognition, entity attribute extraction, 그리고 relation extraction 작업을 지원합니다. 독창적인 점은 스키마 정의(schema definition) 및 프롬프트 설계(prompt design)를 위한 상호작용 LLM 에이전트를 포함하고 있다는 것입니다. i2b2 데이터 세트에서 벤치마킹을 수행하고 시스템 평가를 진행했습니다.

- **Performance Highlights**: 문장 기반 프롬프트 알고리즘이 최상의 성능을 보였으나 더 긴 추론 시간(inference time)이 필요했습니다. 시스템 평가는 직관적인 시각화를 제공하였습니다.



### Freezing of Gait Detection Using Gramian Angular Fields and Federated Learning from Wearable Sensors (https://arxiv.org/abs/2411.11764)
- **What's New**: FOGSense는 파킨슨병(PD) 환자의 자발적으로 움직이는 조건에서 FOG(Freezing of Gait) 감지를 위한 혁신적인 시스템입니다. 기존 방법의 한계를 극복하고 GAF(Gramian Angular Field) 변환과 분산식 딥 러닝(federated deep learning)을 활용하여 시간 및 공간의 보행 패턴을 효과적으로 포착합니다.

- **Technical Details**: FOGSense 시스템은 한 축의 단일 가속도계를 사용하는 최소화된 접근 방식을 채택하여 성능과 신뢰성을 동시에 향상시킵니다. 이를 통해 CNN(Convolutional Neural Network)을 사용하여 실시간 FOG 감지를 실행하고, 분산된 학습 프레임워크를 통해 스마트폰과의 효율적인 동기화를 가능하게 합니다. 특히, GAF는 가속도 데이터로부터 2차원 표현을 생성하여 FOG 감지 정확도를 높여줍니다.

- **Performance Highlights**: FOGSense는 기존 최첨단 방법에 비해 F1-score에서 22.2% 개선을 달성하고, 단일 축 가속도계를 비교할 때 10.4%의 정확도 향상을 보여줍니다. 또한, 복수 센서 시스템에 비해 실패 포인트를 줄이며, 결측값에 대한 강인성을 입증하여 장기적인 모니터링에 효과적입니다.



### Mapping out the Space of Human Feedback for Reinforcement Learning: A Conceptual Framework (https://arxiv.org/abs/2411.11761)
- **What's New**: 이 연구는 Human feedback (인간 피드백)과 Reinforcement Learning (강화 학습) 사이의 간극을 메우기 위해, 상호작용 학습 시나리오에서 인간 피드백의 공통된 이해를 개발하는 데 중점을 두고 있습니다.

- **Technical Details**: 본 연구에서는 인간 피드백에 의한 보상 기반 학습을 위한 피드백 유형의 분류법(taxonomy)을 도입하였습니다. 이 분류법은 아홉 가지 주요 차원에 기반하여 인간 중심, 인터페이스 중심 및 모델 중심의 측면을 통합합니다. 또한, 인간 피드백의 질을 판단하는 일곱 가지 지표(quality metrics)를 정의하고, 이는 피드백을 표현하는 인간의 능력과 에이전트의 학습 능력 모두에 영향을 미칩니다.

- **Performance Highlights**: 우리는 본 연구에서 개발한 피드백 분류법과 질적 기준을 바탕으로 인간 피드백에서 학습하는 시스템의 요구사항과 설계 선택을 도출하였으며, 기존의 상호작용 기계 학습과 관련된 작업들과 연결 지었습니다. 이 과정에서 기존 연구의 격차를 식별하고 미래 연구 기회를 제시합니다. 우리는 데이터 기반(co-adaptive modeling) 및 다양한 상호작용 메카닉(interaction mechanics)을 활용한 강화 학습의 잠재력을 완전히 끌어내기 위한 학제 간 협업을 촉구합니다.
...
- **What's New**: 이번 논문에서는 BitMoD라는 알고리즘-하드웨어 공동 설계 솔루션을 제안하여, 대형 언어 모델(LLM)의 메모리 사용량을 줄이고 배포 가능성을 향상시킵니다. 이 솔루션은 낮은 중량 정밀도에서 LLM 가속화를 가능하게 하며, 새로운 데이터 타입을 도입하여 모델의 정확도를 유지하면서도 정밀도를 4비트 및 3비트로 양자화할 수 있습니다.

- **Technical Details**: BitMoD는 알고리즘 측면에서 미세 분할된 데이터 타입 적응을 통해 특정 수치 데이터 타입을 사용하여 128개 가중치 그룹을 양자화합니다. 하드웨어 측면에서는 비트 직렬 처리 요소를 사용하여 다양한 숫자 정밀도와 데이터 타입을 지원하며, 두 가지 주요 혁신을 포함합니다: 1) 다양한 가중치를 처리하기 위한 통합 표현을 사용하여 하드웨어 비용을 줄입니다. 2) 최소 하드웨어 오버헤드로 그룹 합계를 재조정하는 비트 직렬 복원 단위를 채택합니다.

- **Performance Highlights**: BitMoD는 6개의 대표적인 LLM에 대한 평가에서 기존 LLM 양자화 및 가속화 방법들보다 우수한 성능을 보였으며, 차별적 작업의 경우 평균 0.5%의 정확도 손실로 LLM 가중치를 4비트로 양자화할 수 있었습니다. 생성 작업에서는 3비트로 양자화하면서도 이전 LLM 양자화 방식보다 더 좋은 perplexity를 달성했습니다. BitMoD는 기존 가속기 대비 평균 1.69배 및 1.48배의 속도를 향상시켰습니다.
...
### FLMarket: Enabling Privacy-preserved Pre-training Data Pricing for Federated Learning (https://arxiv.org/abs/2411.11713)
- **What's New**: 본 논문에서는 Federated Learning (FL) 데이터 시장을 위한 혁신적인 가격 책정 메커니즘인 FLMarket를 제안합니다. FLMarket는 프리-트레이닝 가격 책정에 대한 새로운 접근법을 제시하여 데이터 제공자를 유인하고 FL 작업에 참여하도록 장려합니다.

- **Technical Details**: FLMarket는 두 단계의 경매 기반 가격 책정 메커니즘을 결합하여 공정성과 인센티브를 보장합니다. 첫 번째 단계에서, 클라이언트의 통계 정보를 바탕으로 초기 가격이 결정되며, 두 번째 단계에서는 예산 제약 기반 가격 책정 메커니즘을 통해 보상 가격이 할당됩니다. 또한, 비밀 정보를 보호하기 위한 프라이버시 보호 프로토콜이 설계되었습니다.

- **Performance Highlights**: 실험을 통해 FLMarket는 기존 최첨단 방법들에 비해 10% 이상의 높은 정확도를 달성했으며, 자체 인-트레이닝 기준에 비해서도 2% 이상의 정확도 향상과 3배의 실행 속도 향상을 보였습니다.



### Robust Reinforcement Learning under Diffusion Models for Data with Jumps (https://arxiv.org/abs/2411.11697)
- **What's New**: 본 논문에서는 강화 학습(Reinforcement Learning, RL) 분야에서 연속시간 설정에서의 가치 함수 추정의 어려움을 해결하기 위해 Mean-Square Bipower Variation Error (MSBVE) 알고리즘을 제안합니다. 기존의 Mean-Square TD Error (MSTDE) 알고리즘의 한계를 분석하고, 점프(jump) 성분을 포함한 상태 동역학을 효과적으로 처리하는 방안을 모색합니다.

- **Technical Details**: 강화 학습의 상태 공간 및 행동 공간은 각각 𝒮와 𝒜로 나타내며, 환경이 제공하는 현재 상태는 Xt ∈ 𝒮입니다. 에이전트는 정책(π)에 따라 행동(At)을 선택하며, 이에 대한 보상(Rt)은 r(t, Xt, At)에 의해 계산됩니다. 본 논문에서는 주로 확률적 미분 방정식(stochastic differential equations, SDEs)에서의 상태 동역학을 다루며, 특히 점프 성분이 있는 경우에 초점을 맞추고 있습니다.

- **Performance Highlights**: 시뮬레이션 결과에 따르면, MSBVE 알고리즘은 점프 프로세스가 있는 복잡한 환경에서도 가치 함수를 정확하게 추정하여 MSTDE 알고리즘의 성능을 초월함을 입증합니다. 이는 다양한 확률적 잡음이 있는 상황에서도 더 나은 수렴을 보장하며, 강화 학습 알고리즘의 견고성을 높이는 데 기여합니다.
...
### Efficient and Robust Continual Graph Learning for Graph Classification in Biology (https://arxiv.org/abs/2411.11668)
- **What's New**: 이번 연구에서는 Graph Classification을 위한 Perturbed and Sparsified Continual Graph Learning (PSCGL) 프레임워크를 제안합니다. PSCGL은 생물학적 데이터셋에 중점을 두어, 기존의 그래프 신경망(GNN)의 단점을 보완하고, 그래프 데이터의 지속적인 학습을 지원합니다.

- **Technical Details**: PSCGL은 중요한 데이터 포인트를 식별하기 위한 perturbed sampling 전략을 도입하며, 그래프의 스토리지 요구를 줄이기 위해 motif 기반 그래프 sparsification 기법을 사용합니다. 또한, PSCGL은 그래프 백도어 공격(graph backdoor attacks)을 방어하는 기능을 내장하고 있습니다. 이는 생물학적 맥락에서 필수적입니다.

- **Performance Highlights**: 실험 결과, PSCGL은 Enzymes와 Aromaticity 데이터셋에서 기존 방법들보다 우수한 성능을 보여주며, 지속적인 그래프 학습의 효과성을 입증하였습니다. PSCGL은 다양한 태스크 간의 지식을 유지하면서도 효율성과 강건성을 향상시킵니다.
...
### Feature Selection for Network Intrusion Detection (https://arxiv.org/abs/2411.11603)
- **What's New**: 본 논문에서는 네트워크 침입 탐지(Network Intrusion Detection, NID)를 위한 새로운 정보 이론 기반의 방법인 Feature Selection for Network Intrusion Detection (FSNID)을 제안합니다. FSNID는 비정보적인 특징을 제외하면서, 신경망(neural network)을 사용하여 기능 근사(function approximation)를 기반으로 합니다. 이 방법은 모든 특징의 중요도를 평가해 최적의 특징 집합을 선택합니다.

- **Technical Details**: FSNID는 전송 엔트로피(Transfer Entropy, TE) 계산을 기반으로 한 기능 선택 방법으로, 공격 벡터 레이블로 전달되는 엔트로피를 근사(calculate)합니다. 재발(returning)하는 레이어(recurrent layer)를 통합하여 시간적 종속성(temporal dependencies)을 포함시키고, 이전 이터레이션(iteration)에서의 관계를 반영할 수 있습니다. 이러한 접근 방식은 필요한 기능의 수를 더 줄여주며, NID 성능을 저하시키지 않습니다.

- **Performance Highlights**: 실험 결과, FSNID는 기능 공간(feature space)을 상당히 줄이면서도 NID 성능을 유지하는 데 성공했습니다. 모델이 처리해야 할 데이터의 양을 줄이는 것이 비용 효율성을 높이고, 네트워크 침입 탐지의 정확성을 향상시키는 데 기여합니다.



### Generative Spatio-temporal GraphNet for Transonic Wing Pressure Distribution Forecasting (https://arxiv.org/abs/2411.11592)
- **What's New**: 이 연구에서는 비정상(transonic) 날개 압력 분포를 예측하기 위한 새로운 프레임워크를 제안하며, 이는 autoencoder 아키텍처와 graph convolutional networks(GCNs), graph-based temporal layers를 통합하여 시간 의존성을 모델링합니다.

- **Technical Details**: 제안된 GST GraphNet 프레임워크는 GCN 기반의 autoencoder 아키텍처와 시간 예측 레이어를 통합해 날개 압력 분포를 효과적으로 모델링합니다. 고차원 압력 분포 데이터를 저차원 잠재 공간으로 압축하고, 이를 기반으로 그래프 기반의 시간 예측 레이어를 활용하여 미래의 압력을 예측합니다.

- **Performance Highlights**: Benchmark Super Critical Wing 테스트 케이스에 적용하여 계산 유체 역학(cfd)과 유사한 정확도를 달성하며, 예측 시간을 크게 단축하는 등 비정상 현상에 대한 공기역학적 분석에 효율적인 솔루션을 제공합니다.



### SeqProFT: Applying LoRA Finetuning for Sequence-only Protein Property Predictions (https://arxiv.org/abs/2411.11530)
- **What's New**: 본 연구에서는 LoRA (Low-Rank Adaptation) 방법을 활용하여 ESM-2 모델을 단백질 특성 예측 작업에 맞춰 끝에서 끝으로 파인튜닝하는 새로운 접근법을 제시합니다. 이는 단백질 서열 정보만을 사용하며 다중 헤드 어텐션 메커니즘을 통합하여 서열 특징과 접촉 맵 정보를 결합합니다.

- **Technical Details**: ESM-2 모델은 트랜스포머 기반의 대규모 언어 모델로, 아미노산 서열로부터 직접 단백질 구조를 예측하기 위해 설계되었습니다. 본 연구에서는 LoRA를 사용하여 ESM-2를 파인튜닝하고, 다운스트림 네트워크를 처음부터 학습시키는 방식을 채택하였습니다. 특히, 각 서열 예측 시 주어진 접촉 정보를 고려할 수 있도록 다중 헤드 어텐션 메커니즘을 추가하였습니다.

- **Performance Highlights**: 실험 결과, 파인튜닝된 모델은 회귀 및 분류 작업에서 우수한 성능을 보여주었으며, LoRA의 도입으로 정확성과 성능이 개선되었습니다. 또한, 다양한 하이퍼파라미터에 따라 빠른 수렴 속도를 기록했습니다.



### Efficient Sample-optimal Learning of Gaussian Tree Models via Sample-optimal Testing of Gaussian Mutual Information (https://arxiv.org/abs/2411.11516)
Comments:
          47 pages, 16 figures, abstract shortened as per arXiv criteria

- **What's New**: 이번 연구에서는 고차원 Gaussian 분포의 학습 및 구조 추정 문제를 다루고 있습니다. 이는 이전의 연구들이 주로 이산 분포에 초점을 맞추었던 것과는 다르게, 연속형 분포에 대한 새로운 알고리즘을 제시합니다.

- **Technical Details**: 연구진은 Gaussian 랜덤 변수의 조건부 상호 정보량을 테스트하는 알고리즘을 설계했습니다. 이 알고리즘은 두 Gaussian 랜덤 변수가 독립적인지 또는 조건부 상호 정보량이 최소 ε(ε ∈ (0,1)) 이상인지 여부를 판별합니다. 차별화된 점은 이 알고리즘이 O(ε^{-1}) 샘플을 사용하여 근사 최적성을 제공한다는 것입니다.

- **Performance Highlights**: 연구진의 실험 결과는 이론적 수렴 경계와 잘 일치하며, n-변수 Gaussian 트리 모델의 ε-근사 구조 학습 알고리즘이 Θ(nε^{-1}) 샘플을 사용하고 이 또한 근사 최적임을 입증했습니다. 반면, 트리 구조가 알려지지 않은 경우에는 Θ(n²ε^{-2}) 샘플이 필요하다는 점도 명시되었습니다.
...
- **What's New**: 이 논문은 기존의 물리 기반 모델과 데이터 기반 모델을 결합하는 일반화된 접근법을 제안하며, 물리 지식이 제한적인 상황에서도 효과적으로 작동할 수 있는 새로운 Physics Encoded Residual Neural Network (PERNN) 아키텍처를 소개한다.

- **Technical Details**: PERNN은 피드 포워드 레이어로 구성된 학습 블록과 수학 연산자로서의 물리 블록을 결합하여 모델링을 수행한다. 중간에 잔여(residual) 블록이 포함되어 안정적인 그래디언트 흐름을 유지하며, 이로 인해 모델은 물리 시스템의 기하학적 및 운동학적 측면을 준수하도록 학습한다.

- **Performance Highlights**: 제안된 방법은 기본 로봇 운동 모델과 자율주행 차량의 복잡한 스티어링 모델을 포함한 두 가지 응용 프로그램에서 테스트되었으며, 기존의 신경망 기반 방법과 최신 Physics Informed Machine Learning 방법보다 뛰어난 성능을 보였다.



### Graph Artificial Intelligence for Quantifying Compatibility Mechanisms in Traditional Chinese Medicin (https://arxiv.org/abs/2411.11474)
Comments:
          10 pages, 5 figures. Includes open-source dataset and code for reproducibility

- **What's New**: 이 연구에서는 전통 중의학(Traditional Chinese Medicine, TCM)의 복잡한 상호작용을 이해하기 위해 그래프 인공지능(graph artificial intelligence)을 적용하여 다차원 지식 그래프를 개발했습니다. 이는 전통 TCM 이론과 현대 생의학 과학을 연결하는 새로운 접근 방식입니다.

- **Technical Details**: 연구에서는 주요 TCM 용어와 한약재(chinese herbal pieces, CHP)를 처리하기 위해 특징 엔지니어링(feature engineering)과 임베딩(embedding)을 사용했습니다. 약리적 속성은 가상 노드로 도입되었고, 그래프 신경망(graph neural network)과 주의 메커니즘(attention mechanisms)을 활용하여 6,080개의 한약 조제(chinese herbal formulas, CHF)를 모델링하고 분석했습니다.

- **Performance Highlights**: 이 방법은 COVID-19 관리에 설계된 215개의 CHF를 사용하여 CHP의 역할을 정량적으로 평가했습니다. 해석 가능한 모델, 오픈 소스 데이터 및 코드가 제공되어 TCM 이론의 발전과 약물 발견에 강력한 도구를 제공합니다.



### Physics meets Topology: Physics-informed topological neural networks for learning rigid body dynamics (https://arxiv.org/abs/2411.11467)
Comments:
          17 pages, 9 figures

- **What's New**: 이 논문에서는 기존 그래프 기반 방법의 제한점을 해결하는 새로운 프레임워크인 'Higher-Order topological Physics-informed Network'(HOPNet)를 소개합니다. HOPNet은 정적 데이터에서 벗어나 동적 물체의 기하학과 상호작용을 높은 차원의 위상적 구조로 모델링 할 수 있는 기능을 갖추고 있습니다.

- **Technical Details**: HOPNet은 고차원 위상적 그룹을 활용하여 복잡한 동적 물체 기하학을 모델링하고 물리 법칙이 반영된 위상 신경 아키텍처를 통해 강체 역학을 학습합니다. 또한, HOPNet의 메시지 전송 방식은 뉴턴의 물리 법칙에 기반하여 정의되며, 이는 물체 간의 상호작용을 더 정확하게 수학적으로 표현하게 해줍니다.

- **Performance Highlights**: HOPNet은 복잡한 모양과 다중 물체 충돌을 포함하는 다양한 시나리오에서 높은 정확도를 보여주며, 긴 롤아웃에서도 우수한 성능을 유지합니다. 이 프레임워크는 가시적이지 않은 시나리오에서의 일반화 능력 또한 뛰어나며, 다양한 가설적 조건에 대한 엄격한 탐색을 가능하게 합니다.
...
### Upside-Down Reinforcement Learning for More Interpretable Optimal Contro (https://arxiv.org/abs/2411.11457)
- **What's New**: 본 연구는 Upside-Down Reinforcement Learning (UDRL) 프레임워크 내에서 Neural Networks (NNs) 외에 다른 함수 근사 알고리즘이 사용될 수 있는지 조사합니다.

- **Technical Details**: UDRL은 상태와 원하는 명령으로부터 행동을 예측하는 방법을 배우기 위한 새로운 학습 패러다임으로, 이는 Supervised Learning 문제로 구성됩니다. 본 논문에서는 Random Forests와 Extremely Randomized Trees와 같은 트리 기반 방법들이 NNs와 유사한 성능을 나타나는지를 실험하였습니다.

- **Performance Highlights**: 여러 인기 있는 최적 제어 벤치마크에서 수행된 실험 결과, 트리 기반 방법들이 NNs와 동등한 성능을 보이며, 추가적으로 더 해석 가능한 정책을 생성한다는 점에서 안전하고 견고한 RL로 나아가는 기반이 됩니다.
...
### Temporal and Spatial Reservoir Ensembling Techniques for Liquid State Machines (https://arxiv.org/abs/2411.11414)
- **What's New**: 본 논문에서는 Reservoir Computing의 일종인 Liquid State Machines (LSM)에서 성능 향상을 위한 두 가지 새로운 앙상블 기법인 Multi-Length Scale Reservoir Ensemble (MuLRE)와 Temporal Excitation Partitioned Reservoir Ensemble (TEPRE)를 제안합니다.

- **Technical Details**: Reservoir Computing은 입력(Input), Reservoir, 출력(Readout) 층으로 구성된 얕은 네트워크 모델입니다. LSM은 spiking neurons로 구성되어 있고, 시간에 따라 변하는 복잡한 동적 행동을 표현할 수 있습니다. 우리 연구에서는 NMNIST, SHD, DVSGesture와 같은 표준 neuromorphic 데이터셋에서 앙상블 기법을 평가하였습니다.

- **Performance Highlights**: N-MNIST 데이터셋에서 3600개의 뉴런으로 구성된 LSM 모델로 98.1%의 테스트 정확도를 달성했으며, SHD 데이터셋에서는 77.8%의 정확도를 기록하여 기존 LSM 기반 접근 방식을 초월하였습니다.
...
- **What's New**: 본 연구에서는 대규모 언어 모델(LLM)의 권위에 대한 편향(bias)을 새롭게 제시하며, 이 편향이 LLM 생성 콘텐츠의 품질을 향상시킬 수 있는 동시에 잠재적 취약점을 만들어 유해한 콘텐츠를 생성할 위험을 증가시킬 수 있다고 강조합니다.

- **Technical Details**: DarkCite라는 새로운 jailbreak 공격 방법을 소개합니다. DarkCite는 특정 위험 유형에 맞는 최적의 권위 인용 유형을 일치시키고, 유해한 지침과 관련된 권위 있는 인용을 생성함으로써 공격의 효과성을 높입니다. 또한, 이 방법은 LLM의 내재적 권위 편향을 활용하여 재빠르고 효율적으로 공격 프롬프트를 생성합니다.

- **Performance Highlights**: 실험 결과, DarkCite는 기존 방법보다 높은 공격 성공률을 보여줍니다. 예를 들어, LLama-2 모델의 경우 DarkCite가 76%의 공격 성공률을 기록했으며, 기존의 다른 방법들과 비교하여 훨씬 적은 수의 쿼리로도 효과적인 공격을 가능하게 했습니다. 이 연구는 또한 필터 우회를 방지하기 위한 진정성 및 유해성 검증 방어 전략을 제안하며, 평균 방어 통과율(DPR)을 11%에서 74%로 향상시켰습니다.
...
- **What's New**: 이 논문에서는 고급 모방 학습 알고리즘을 엣지 디바이스에 배치하기 위한 파이프라인을 제안합니다. 이 파이프라인은 효율적인 모델 압축 방법과 실용적인 비동기 병렬 방법인 TEDA(Temporal Ensemble with Dropped Actions)를 활용하여 작업의 매끄러움을 높입니다.

- **Technical Details**: 논문의 핵심은 모델 압축 과정으로, 입력 형태 통일(Input Shape Unification) 및 대칭 양자화(Symmetric Quantization)를 통해 모델의 메모리 요구사항과 계산 요구사항을 줄이는 것입니다. 또한, TEDA 기법을 통해 예측이 수행되는 동안 행동을 병렬로 실행함으로써 연산 지연을 최소화합니다.

- **Performance Highlights**: 제안된 파이프라인을 통해 대규모 모방 학습 모델이 서버에서 훈련된 후 엣지 장치에 배포되어 다양한 조작 작업을 성공적으로 수행했습니다. 이는 자원 제한 환경에서 고급 AI 알고리즘을 효과적으로 운영할 수 있는 가능성을 보여줍니다.
...
### Graph Neural Networks on Graph Databases (https://arxiv.org/abs/2411.11375)
Comments:
          14 pages, 8 figures

- **What's New**: 본 연구에서는 그래프 데이터베이스(Graph DB)를 활용하여 그래프 신경망(GNN)을 보다 효율적으로 학습하는 새로운 방법을 제시합니다. 이는 메모리에 최소한의 데이터만 로드하고 쿼리 엔진을 이용해 샘플링 하는 방식으로, GNN의 메모리 효율성을 크게 개선합니다.

- **Technical Details**: 우리는 GNN 학습을 위해 Graph DB를 중심으로 기능을 통합합니다. 이를 통해 이웃 샘플링(neighbor sampling)과 특성 추출(feature retrieval) 같은 그래프 ML 작업을 DB 쿼리로 표현합니다. GNN의 교육 과정에서 데이터 전송을 줄이고 메모리 사용량을 최적화할 수 있습니다.

- **Performance Highlights**: 실험 결과, 우리의 접근 방식은 단일 머신 및 분산 훈련 모두에서 자원 효율성 면에서 여러 이점을 보여주었습니다. 이 방법은 GNN의 스케일링을 위한 새로운 가능성을 열어주는 것뿐만 아니라 그래프 DB에 대한 새로운 응용분야를 제공합니다.
...
### Zero-Shot Load Forecasting with Large Language Models (https://arxiv.org/abs/2411.11350)
Comments:
          21 pages,5 figures

- **What's New**: 이번 논문은 데이터가 부족한 상황에서도 효과적으로 로드(Load) 예측을 수행할 수 있는 새로운 접근 방식을 제안합니다. 특히, 자연어 처리에서 성공적으로 사용된 사전 학습된 언어 모델(LLM)을 활용하여, Chronos 모델이라는 진화된 프레임워크를 통해 제로샷(zero-shot) 로드 예측을 가능하게 합니다.

- **Technical Details**: Chronos 모델은 방대한 사전 학습된 지식을 이용해 데이터가 부족한 환경에서도 정밀한 로드 예측을 수행합니다. 이 모델은 특정 로드 데이터셋에 맞춰 조정되거나 파인튜닝(fine-tuning)되지 않았음에도 불구하고, 다섯 개의 실제 데이터셋을 통한 시뮬레이션 결과에서 아홉 개의 주요 베이스라인 모델에 비해 현저히 우수한 성능을 나타냅니다. 예측 기간은 1시간에서 48시간까지 다양합니다.

- **Performance Highlights**: Chronos 모델은 평균 제곱근 오차(RMSE), 연속 순위 확률 점수(CRPS), 그리고 분위수 점수(QS)에서 각각 약 7.34%-84.30%, 19.63%-60.06%, 그리고 22.83%-54.49% 개선된 성능을 보였습니다. 이러한 결과는 Chronos 모델의 우수성과 유연성을 부각시킵니다.



### A Hybrid Loss Framework for Decomposition-based Time Series Forecasting Methods: Balancing Global and Component Errors (https://arxiv.org/abs/2411.11340)
- **What's New**: 이번 연구에서는 시계열 예측을 위한 새로운 하이브리드 손실 프레임워크를 제안합니다. 이 프레임워크는 기존의 전체 손실을 유지하면서 각 서브 시리즈에 대한 컴포넌트 손실을 통합하여 학습의 효율성을 높입니다.

- **Technical Details**: 제안된 하이브리드 손실 프레임워크는 전체 손실(global loss)과 각 서브 시리즈에 대한 손실(component losses)을 결합하여 두 개의 최적화(min-max optimization) 문제로 구성됩니다. 이 방법은 각 컴포넌트 손실의 가중치를 동적으로 조정하여 더 중요한 서브 시리즈에 중점을 두게 합니다.

- **Performance Highlights**: 여러 시계열 데이터셋에서 실험한 결과, 기존 방법에 비해 평균 0.5-2%의 성능 향상을 보여주었으며, 모델 아키텍처에 대한 수정 없이도 이루어졌습니다.



### Enhancing Decision Transformer with Diffusion-Based Trajectory Branch Generation (https://arxiv.org/abs/2411.11327)
- **What's New**: 오프라인 강화 학습(Offline Reinforcement Learning, RL)에서 sub-optimal한 경로(trajectories)에 수렴하는 문제를 해결하기 위해, 확산 기반의 경로 분기 생성(Diffusion-Based Trajectory Branch Generation, BG) 방법을 제안합니다. 이 방법은 기존 데이터셋의 경로를 확장하여 더 나은 경로로 이동할 수 있는 가능성을 제공합니다.

- **Technical Details**: Decision Transformer (DT)는 과거 경로 및 return-to-go (RTG)에 조건적으로 행동의 가능성을 최대화하는 transformer 아키텍처를 사용합니다. BG는 데이터셋 내의 경로 세그먼트를 기반으로 경로 분기를 생성하며, Trajectory Value Function (TVF)을 사용해 높은 보상을 유도하는 랜덤한 경로 확장을 제공합니다. 생성된 분기는 DT가 sub-optimal 경로로부터 분기하여 더 나은 정책을 학습할 수 있는 기회를 제공합니다.

- **Performance Highlights**: BG를 사용한 DT는 D4RL benchmark에서 최신 sequence modeling 방법보다 뛰어난 성능을 보이며, sub-optimal 경로에서 최적 경로로 전환하는 것이 더 어려운 작업에서 성능이 특히 향상됩니다.



### Cuvis.Ai: An Open-Source, Low-Code Software Ecosystem for Hyperspectral Processing and Classification (https://arxiv.org/abs/2411.11324)
Comments:
          5 pages, 2024 14th Workshop on Hyperspectral Imaging and Signal Processing: Evolution in Remote Sensing (WHISPERS)

- **What's New**: 이번 논문에서는 오픈소스(low-code) 소프트웨어 생태계인 cuvis.ai를 소개합니다. 이 시스템은 하이퍼 스펙트럼 데이터의 수집, 전처리 및 모델 교육을 지원하며, 사용자가 머신러닝(ML) 라이브러리를 쉽게 활용할 수 있도록 설계되었습니다.

- **Technical Details**: cuvis.ai는 Python으로 작성되었으며, 머신러닝 알고리즘(예: scikit-learn, PyTorch)에 대한 래퍼를 제공합니다. 이 구조는 노드-기반의 유향 비순환 그래프(directed acyclic graph)를 사용하여 데이터 수집에서 최종 클래스 할당까지의 모든 단계를 처리합니다. 사용자 정의 모델은 직렬화(serialization) 기능을 제공하여 연구 커뮤니티 내에서의 공유를 용이하게 합니다.

- **Performance Highlights**: cuvis.ai는 새롭고 다양한 모델을 확장 가능하게 만들 수 있는 인터페이스를 제공하며, 하이퍼 스펙트럼 데이터에 대한 최신 머신러닝 기법들의 적용을 가능하게 합니다. 전체 데이터 로딩, 전처리 및 분류 과정이 단 11 라인의 코드로 구현될 수 있어 사용자에게 높은 유연성과 간결함을 제공합니다.



### A Review on Machine Unlearning (https://arxiv.org/abs/2411.11315)
- **What's New**: 이번 논문에서는 사용자의 개인 정보 보호와 관련된 법적 요구사항이 증가하는 상황에서 머신러닝 모델의 보안과 개인 정보 보호 문제를 깊이 있게 분석한다. 특히, GDPR(General Data Protection Regulation)의 17조 '잊혀질 권리'에 따라 데이터가 삭제 요청 시 머신러닝 모델이 이를 고려하여 재훈련해야 한다는 점에 주목한다.

- **Technical Details**: 이 논문은 머신러닝에서 사용자의 개인 정보 보호를 위한 여러 접근법, 특히 머신 언러닝(machine unlearning)의 개념을 다룬다. 머신 언러닝은 데이터 삭제 후 모델을 재훈련하여 특정 데이터를 잊도록 하는 기술로, 두 가지 방법이 존재한다: 완전 언러닝(Exact Unlearning)과 근사 언러닝(Approximate Unlearning).

- **Performance Highlights**: 논문은 머신 언러닝 기술이 기존의 재훈련 방식보다 더 효율적이라는 점을 강조하며, 현재 진행되고 있는 연구 결과들과 데이터 라인리지(data lineage) 맥락에서의 논의를 통해 향후 연구 과제를 제시한다. 이를 통해 머신러닝 모델의 개인정보 유출 및 공격으로부터의 보호를 강화할 수 있는 방향을 모색한다.



### Toward Personalized Federated Node Classification in One-shot Communication (https://arxiv.org/abs/2411.11304)
Comments:
          Work in progress

- **What's New**: 본 논문은 그래프 데이터에 맞춘 최초의 One-shot Personalized Federated Graph Learning (OFGFL) 방법을 제안합니다. 이는 Secure Aggregation 체계와 호환되며, 노드 분류를 위해 설계되었습니다.

- **Technical Details**: OFGFL에서는 클래스 별 특성 분포의 통계를 추정하고 집계하여 서버에서 글로벌 의사 그래프(global pseudo-graph)를 생성하여 글로벌 그래프 모델을 학습합니다. 기존의 개인화된 FGL 방법들이 개인화된 모델이 편향되어 있으며, 소수 집단에 대한 일반화 능력을 무시한다는 점을 새롭게 발견하였습니다. 이를 해결하기 위해, 개인 정보를 활용하는 2단계 개인화 학습 방법을 제안합니다.

- **Performance Highlights**: 여러 설정과 다양한 파티션에서 8개의 다중 스케일 그래프 데이터셋을 대상으로 한 포괄적인 실험 결과, 본 연구의 방법이 기존 최첨단 방법들보다 우수한 성능을 발휘함을 입증했습니다.
...
- **What's New**: 이 연구에서는 언어 모델(Language Model) 안전성을 향상시키기 위해 spars한 자동인코더(sparse autoencoder)를 활용한 feature steering 방안을 제안하고 있습니다. 특히, 모델 가중치를 업데이트하지 않고도 모델 활성화를 조작하여 특정 행동(예: 응답 거부)을 유도하는 방법을 모색합니다.

- **Technical Details**: 연구에서는 Phi-3 Mini를 대상으로 spars한 자동인코더(SAE)를 훈련하여, 응답 거부 행동을 매개하는 특성을 식별합니다. 이를 통해, 특정 특성의 활성화 값을 수동으로 조절(클램핑(clamping))하여 모델의 응답을 외부 프롬프트에 대해 조정합니다. 이 과정은 단일 턴(single-turn) 벤치마크에서 안전하지 않은 프롬프트에 대한 거부율을 증가시킴으로써, 모델의 견고성을 향상시킵니다.

- **Performance Highlights**: feature steering을 통해 안전하지 않은 프롬프트에 대한 모델의 거부율이 향상되었습니다. 그러나, 안전한 프롬프트에 대한 과도한 거부(over-refusal) 및 사실적 회상(factual recall)과 추론(reasoning) 성능의 저하가 관찰되었습니다. 따라서, feature steering은 언어 모델 안전성을 높이는 잠재력이 있으나, 성능 저하 문제를 해결하기 위한 추가 연구가 필요합니다.



### SADDE: Semi-supervised Anomaly Detection with Dependable Explanations (https://arxiv.org/abs/2411.11293)
- **What's New**: 이 논문에서는 SADDE라는 새로운 프레임워크를 소개합니다. 이 프레임워크는 반지도 학습을 통해 네트워크 이상 탐지 과정이 해석 가능하도록 하고, 해석 결과의 신뢰성을 높이는 동시에 레이블이 없는 샘플에 대해 신뢰도가 높은 유사 레이블(pseudo labels)을 할당합니다.

- **Technical Details**: SADDE는 Global-local Knowledge Association Mechanism (KAM)과 Two-stage Semi-supervised Learning System (ToS)의 두 가지 주요 구성 요소로 이루어져 있습니다. KAM은 보안 분석가가 모델 예측의 근본 원인을 이해할 수 있도록 도와주며, ToS는 효과적인 유사 레이블링 프로세스를 제공하여 반지도 학습을 지원합니다.

- **Performance Highlights**: SADDE는 두 개의 대표적인 네트워크 이상 탐지 데이터세트(ISCXTor2016 및 CIC-DoHBrw-2020)에서 기존 방법들과 비교하여 평가되었습니다. 실험 결과, SADDE는 신뢰할 수 있는 해석을 제공하면서 반지도 네트워크 이상 탐지 시스템의 성능을 향상시키는 것으로 나타났습니다.



### Dual-Frequency Filtering Self-aware Graph Neural Networks for Homophilic and Heterophilic Graphs (https://arxiv.org/abs/2411.11284)
Comments:
          11pages,17figures

- **What's New**: 본 논문에서는 Dual-Frequency Filtering Self-aware Graph Neural Networks (DFGNN)을 제안하여 topological(위상) 및 attribute(속성) 간의 간섭 문제를 해결하고 heterophilic(이질적인) 그래프에서의 high-frequency(고주파) 정보 손실을 방지합니다.

- **Technical Details**: DFGNN은 low-pass(저주파) 및 high-pass(고주파) 필터를 통합하여 그래프 데이터로부터 부드러운 표현과 세밀한 특징을 추출합니다. 주파수에 특화된 제약을 통해 각 주파수 대역에서의 노이즈와 중복을 최소화하며, 동적으로 필터링 비율을 조정하여 homophilic(동질적인) 및 heterophilic 그래프에 적응합니다.

- **Performance Highlights**: 많은 벤치마크 데이터셋에 대한 실험 결과, DFGNN은 최신 방법들보다 뛰어난 분류 성능을 보여 주며, 이는 homophilic 및 heterophilic 그래프 처리에 효과적임을 강조합니다.
...
### Effective Predictive Modeling for Emergency Department Visits and Evaluating Exogenous Variables Impact: Using Explainable Meta-learning Gradient Boosting (https://arxiv.org/abs/2411.11275)
- **What's New**: 이 연구에서는 Emergency Department (ED) 방문 예측을 위한 새로운 Meta-learning Gradient Booster (Meta-ED) 접근 방식을 소개합니다. 이 방법은 23년 동안 Canberra Hospital에서 수집된 다양한 exogenous variables를 활용하여 정확한 방문 예측을 목표로 합니다.

- **Technical Details**: Meta-ED는 Catboost, Random Forest, Extra Tree 및 lightGBoost의 네 개의 기초 학습자와 Multi-Layer Perceptron (MLP)로 구성된 상위 학습자를 결합하여 다양한 기반 모델(sub-learners)의 독특한 역량을 활용합니다. 이를 통해 모델의 정확도를 극대화합니다.

- **Performance Highlights**: Meta-ED 모델은 23개 다른 모델과의 비교 분석을 통해 85.7%의 정확도를 보여주며(95% CI; 85.4%, 86.0%) 10개의 평가 메트릭에서 뛰어난 성능을 입증했습니다. 기존 기법들과 비교했을 때, Meta-ED는 XGBoost, Random Forest, AdaBoost, LightGBoost, Extra Tree에 대해 각각 58.6%, 106.3%, 22.3%, 7.0%, 15.7%의 정확도 향상을 보여주었습니다. 날씨 관련 데이터를 포함하면 방문자 수 예측의 정확도가 3.25% 개선됩니다.



### GROOT: Effective Design of Biological Sequences with Limited Experimental Data (https://arxiv.org/abs/2411.11265)
- **What's New**: 이번 연구에서는 GROOT라는 새로운 프레임워크를 소개하여, 제한된 라벨 데이터를 사용할 때의 문제를 해결합니다. GROOT는 훈련된 잠재 표현 주위에 샘플링된 이웃에 대한 유사 레이블을 생성하고, 레이블 전파(Label Propagation) 기법으로 이러한 유사 레이블을 정제하여 최적화를 이끄는 데 도움을 줍니다.

- **Technical Details**: GROOT는 기존 데이터를 기반으로 합성 샘플을 생성하며, 건강한 표본 공간을 구축하고 노이즈에 강한 예측 모델을 훈련하는 데 중점을 둡니다. 이 모델은 그래프 기반의 매끄럽게 한 피트니스(fitness) 모델을 학습하여 수치 최적화 과정에 활용됩니다.

- **Performance Highlights**: GROOT는 GFP 및 AAV라는 두 가지 단백질 디자인 벤치마크에서 기존 방법들을 능가하며, 100개 미만의 라벨 훈련 시퀀스가 있는 극단적인 상황에서도 안정적인 성능을 보여 줍니다. 특히 GFP에서는 6666배의 피트니스 향상을, AAV에서는 1.31배 더 나은 결과를 기록하였습니다.



### Graph Retention Networks for Dynamic Graphs (https://arxiv.org/abs/2411.11259)
- **What's New**: 이번 연구에서는 동적 그래프를 위한 통합 아키텍처인 Graph Retention Network (GRN)을 제안합니다. GRN은 동적 그래프 데이터에 대한 retention의 핵심 계산 방식을 확장하여 병렬 학습, O(1) 저비용 추론, 장기 배치 학습을 가능하게 합니다.

- **Technical Details**: GRN은 그래프 retention을 핵심 계산 메커니즘으로 하며 세 가지 계산 패러다임을 제공합니다: 병렬 패러다임은 훈련 병렬성을 허용하고, 반복 패러다임은 저비용 추론을 가능하게 하며, 청크 단위 패러다임은 장기 배치 훈련을 지원합니다.

- **Performance Highlights**: GRN은 다양한 벤치마크 데이터셋에서 우수한 성능을 보여주며, 훈련 지연 시간을 줄이고 GPU 메모리 소비를 감소시키며, 기준 모델에 비해 최대 86.7배의 추론 처리량 향상을 달성했습니다.



### Progressive Generalization Risk Reduction for Data-Efficient Causal Effect Estimation (https://arxiv.org/abs/2411.11256)
Comments:
          Accepted by KDD'25

- **What's New**: 본 논문은 causal effect estimation (CEE) 방법이 기존의 고정된 레이블 데이터 세트를 가정하지 않고, 초기에는 소량의 레이블이 있는 데이터 샘플로 시작하며, 트레이닝 과정에서 점진적으로 더 많은 레이블 샘플을 수집하는 실제적인 설정을 다룹니다.

- **Technical Details**: 모델 불완전성 문제를 해결하기 위해 'Model Agnostic Causal Active Learning (MACAL)' 알고리즘을 제안하며, 이 알고리즘은 레이블 획득을 위한 샘플 선택 기준을 개선하는 데 초점을 맞추고 있습니다. 이론적으로, 레이블의 정보적인 위험 상한을 줄이는 방식으로 접근하며, 치료 그룹 간 샘플의 다양성과 유사성을 고려합니다.

- **Performance Highlights**: MACAL 알고리즘은 여러 CEE 모델과 다양한 데이터 세트에서 테스트되었으며, 모든 비교에서 성능 향상을 입증했습니다.
...
### Mirror Descent on Reproducing Kernel Banach Spaces (https://arxiv.org/abs/2411.11242)
Comments:
          42 pages, 3 figures

- **What's New**: 이 논문은 reproducing kernel Banach spaces (RKBS)에서의 학습 문제를 다루며, 이론적 및 실용적인 알고리즘적 결과를 제공하여 근사화(approximation)와 최적화(optimization) 오류 간의 간극을 메우는 것을 목표로 합니다.

- **Technical Details**: 제안된 알고리즘은 mirror descent (MDA)를 기반으로 하며, Banach 공간의 이중 공간(dual space)에서 기울기 단계를 수행합니다. 이 알고리즘은 RKBS에서의 일반적인 최소화 문제를 해결하는 데 초점을 맞추며, 이를 통해 선형 수렴률(linear convergence rate)을 달성할 수 있는 조건을 규명하고 있습니다.

- **Performance Highlights**: 이 연구는 Banach 공간의 도구를 활용하여 알고리즘의 수렴 특성을 분석하고 있어, 실제 적용에서의 성능 향상을 기대할 수 있습니다.



### Reliable Learning of Halfspaces under Gaussian Marginals (https://arxiv.org/abs/2411.11238)
- **What's New**: 이 논문에서는 Kalai et al. (2012)의 신뢰할 수 있는 agnostic 모델에서 Gaussian halfspaces의 신뢰할 수 있는 학습을 위한 새로운 알고리즘을 제안합니다. 이 알고리즘은 샘플 및 계산 복잡성을 줄이는 데 중점을 두고 있습니다.

- **Technical Details**: 제안된 알고리즘은 다음과 같은 샘플 및 계산 복잡성을 갖습니다: $$d^{O(	ext{log}(	ext{min}igrac{1}{	ext{α}},rac{1}{	ext{ε}}))}	ext{min}(2^{	ext{log}(1/	ext{ε})^{O(	ext{log}(1/	ext{α}))}},2^{	ext{poly}(1/	ext{ε})})$$. 여기서 $	ext{ε}$는 초과 오류(excess error), $	ext{α}$는 최적 halfspace의 편향(bias)을 나타냅니다.

- **Performance Highlights**: 우리는 통계 질의에 대한 하한선을 만족하여 알고리즘에서 $d^{	ext{Ω}(	ext{log}(1/	ext{α}))}$ 의존성이 최적임을 나타냅니다. 결과적으로, 신뢰할 수 있는 agnostic 학습과 표준 agnostic 학습 간의 강한 계산적 분리를 의미합니다.



### Don't Be So Positive: Negative Step Sizes in Second-Order Methods (https://arxiv.org/abs/2411.11224)
- **What's New**: 이 논문에서는 현대 머신러닝에서 널리 활용되지 않는 2차 방법의 새로운 최적화 방향인 음의 스텝 사이즈(negative step sizes)를 도입하여, 신경망 훈련에서 효율성을 극대화할 수 있는 가능성을 제시합니다.

- **Technical Details**: 본 논문은 음의 스텝 사이즈를 사용하는 것이 2차 방법과 유사한 최적화 알고리즘에서 유용하다는 주장을 합니다. 특히, 너무 일반적인 조건 하에서 상승 방향을 생성하는 방법이 우르프(Wolfe) 선탐색(Wolfe line search)과 결합되면 전역 수렴(global convergence)을 보장할 수 있음을 증명했습니다.

- **Performance Highlights**: 실험 결과, 음의 스텝 사이즈를 사용한 경우가 일반적인 헤세 행렬 수정(hessian modification) 방법보다 더 효과적이라는 점을 보여주었습니다. 특히, SR1(대칭 순위 1) 방법과 음의 스텝 사이즈의 조합이 신경망 훈련에서 간과되었던 긍정적인 방법임을 나타냈습니다.
...
- **What's New**: 본 논문은 Graph Neural Networks (GNNs)로부터 훈련된 그래프를 도용하는 새로운 문제를 조사합니다. 특히, 프라이빗 데이터에서 훈련된 GNN 모델의 매개변수를 활용하여 원본 훈련 그래프를 재구성하는 방법을 제안합니다.

- **Technical Details**: 제안된 GraphSteal 프레임워크는 그래프 디퓨전 모델(Graph Diffusion Model)과 디퓨전 노이즈 최적화(Diffusion Noise Optimization) 알고리즘을 사용하여 목표 훈련 데이터와 유사한 고품질 그래프를 생성합니다. GNN 모델의 매개변수를 활용하여 후보 그래프 세트에서 훈련 그래프를 식별할 수 있는 선택 방법을 제시합니다.

- **Performance Highlights**: 실제 데이터셋에서 수행된 실험을 통해, GraphSteal 프레임워크가 훈련된 GNN으로부터 개인 훈련 그래프를 정확하게 재구성하는 데 효과적임을 입증하였습니다.
...
- **What's New**: 이 논문은 메타-강화학습(meta-RL) 에이전트의 성능을 향상시키기 위해 불균형한 수익 스케일 문제를 해결하는 새로운 방법을 제안합니다. 저자들은 변형(변형된) 태스크에 대한 적응을 목표로 하는 RL 에이전트를 위한 Transformer 기반 정책을 평가합니다.

- **Technical Details**: 본 연구에서는 강화학습에 기반한 정책이 서로 다른 태스크들이 가질 수 있는 다양한 보상(scale of returns) 스케일에 의해 어려움을 겪는다는 점에 주목합니다. 저자들은 actor와 critic의 목적을 분류(classification) 문제로 변환하고, 이를 통해 스케일에 독립적이고 유연한 업데이트를 가능하게 합니다. 대규모 실험을 통해 Meta-World ML45 및 Multi-Game Atari 등 다양한 환경에서 이 접근 방식의 효과를 증명하였습니다.

- **Performance Highlights**: 저자들은 제안된 방법이 온라인 다중 태스크 적응과 메모리 문제에서 중대한 진전을 이룰 수 있음을 보여줍니다. 특히, 수익의 스케일을 고려한 업데이트 변환이 강화학습의 경쟁력과 일반화 능력을 크게 향상시킨다고 강조하였습니다.



### Infinite Width Limits of Self Supervised Neural Networks (https://arxiv.org/abs/2411.11176)
- **What's New**: 본 논문에서는 NTK(Neural Tangent Kernel)와 자기 지도 학습(self-supervised learning) 간의 간극을 메우고, Barlow Twins loss 하에서 훈련된 두 층 신경망의 NTK가 어떻게 동작하는지를 분석합니다.

- **Technical Details**: 이 연구에서는 두 층 신경망에 대한 Barlow Twins loss를 적용한 경우, 네트워크의 폭이 무한대에 가까워질 때 NTK가 상수값으로 수렴함을 증명합니다. 기존의 NTK 관련 연구와는 다른 분석 기법을 사용하며, 이는 독립적인 흥미를 불러일으킬 수 있습니다.

- **Performance Highlights**: 이 연구는 넓은 신경망의 자기 지도 학습을 이해하기 위한 고전적인 커널 이론(kernel theory)의 사용을 처음으로 엄격하게 정당화하며, 커널화된 Barlow Twins의 일반화 오차 구간을 도출하고 이를 유한 폭의 신경망과 연결합니다.



### Learning the Sherrington-Kirkpatrick Model Even at Low Temperatur (https://arxiv.org/abs/2411.11174)
- **What's New**: 익명 그래픽 모델(undirected graphical model) 또는 마르코프 랜덤 필드(Markov Random Field, MRF)에서 매개변수를 학습하는 문제를 다루며, 무작위로 선택된 엣지 가중치 환경에서도 문제를 해결할 수 있는 알고리즘을 제시합니다. 특히, 이 논문은 KLIVANS와 MEKA의 곱셈 가중치 업데이트(multiplicative-weight update) 알고리즘을 통해 Ising 모델에서의 매개변수 학습을 다룹니다.

- **Technical Details**: 논문에서는 인버스 온도(inverse temperature) β에 대해 \(\beta \leq \sqrt{\log n}\)인 경우에 대해 폴리노미얼 타임(polynomial time) 내에 매개변수를 학습할 수 있음을 보여줍니다. 이 결과는 Sherrington-Kirkpatrick(SK) 모델을 포함한 여러 고차 MRF로도 확장되며, 특히 이전 연구에서 발견되지 않았던 고온 영역(high-temperature regime)에서도 개선된 성과를 냅니다.

- **Performance Highlights**: 이 연구의 결과는 Sparsitron 알고리즘이 \(\exp(O(\beta^{2} + \beta \log n))\cdot poly(n)\)의 시간 복잡도 내에 매개변수 회수(parameter recovery)가 가능함을 입증하며, 이는 β>=1(저온 영역)에서는 확인되지 않았던 효율적인 학습이 가능함을 보여줍니다.
...
### Federated Learning for UAV-Based Spectrum Sensing: Enhancing Accuracy Through SNR-Weighted Model Aggregation (https://arxiv.org/abs/2411.11159)
- **What's New**: 이 논문에서는 무인 항공기(UAV) 네트워크에서 스펙트럼 감지 작업을 위한 연합 학습(federated learning, FL) 기반 방법을 제안합니다. 이 방법은 UAV 네트워크의 분산된 특성과 제한된 컴퓨터 용량을 고려합니다.

- **Technical Details**: 제안된 방법은 스펙트럼 감지를 위한 경량 컨볼루션 신경망(lightweight CNN)을 활용하며, 3D 환경에서 신호의 실수 및 허수 구성 요소를 최적화하여 처리할 수 있습니다. 또한, 신호 대 잡음비(signal-to-noise ratio, SNR)를 고려한 연합 집계 방법인 FedSNR을 통해 전 세계 모델을 획득합니다.

- **Performance Highlights**: 수치 결과에 따르면, 제안된 아키텍처 및 집계 방법은 기존의 방법들보다 우수한 성능을 보여 스펙트럼 감지 정확도를 향상시킵니다.



### From Primes to Paths: Enabling Fast Multi-Relational Graph Analysis (https://arxiv.org/abs/2411.11149)
Comments:
          35 pages: 28 main, 7 appendix; 6 figures. Submitted to ECML PKDD 2025 Journal Track for Data Mining and Knowledge Discovery. For the code accompanying the paper see this http URL . For a demo app on relation prediction on HetioNet using BoP representations see this http URL

- **What's New**: 이 연구에서는 Prime Adjacency Matrices (PAMs) 프레임워크를 확장하여 다중 관계 네트워크를 보다 효율적으로 표현하고 분석하는 방법을 개발했습니다. 특히, 손실이 없는 알고리즘을 도입하여 다중 세부 정보가 포함된 multi-hop 행렬들을 계산할 수 있게 했으며, Bag of Paths (BoP)라는 기능 추출 방법론을 제안했습니다.

- **Technical Details**: PAM 프레임워크는 각각의 관계 유형을 고유한 소수(prime number)로 매핑하여 모든 관계를 단일 인접 행렬로 압축할 수 있도록 설계되었습니다. 이 방식은 그래프의 모든 정보를 포함하며 multi-hop 관계를 빠르게 추출하는 데 용이합니다. BoP представляет собой метод извлечения признаков, который использует эффективные операции матриц для генерации интерпретируемых и универсальных векторов признаков для различных задач аналитики.

- **Performance Highlights**: 다양한 토대 작업에서 BoP 기반 모델이 일반적으로 사용되는 신경망 모델들과 비교해 빠른 속도와 해석 가능성을 제공하면서 유사한 성능을 발휘함을 보였습니다. 특히, GraphBLAS를 활용한 PAM 프레임워크의 구현은 더 나은 계산 효율성을 제공하며, Python 모듈로 접근 가능하여 추가 실험이 용이해졌습니다.
...
### Generating medical screening questionnaires through analysis of social media data (https://arxiv.org/abs/2411.11048)
- **What's New**: 이 연구에서는 사회적 미디어 게시글을 기반으로 의료 조건에 대한 스크리닝 질문지를 생성하는 가능성을 탐구합니다. 전통적인 방법이 아닌 사용자의 경험을 바탕으로 한 질문지를 제공하여 진단 과정에서 실질적으로 도움을 줄 수 있는 방법을 제시하고 있습니다.

- **Technical Details**: 이번 연구는 Reddit에서 여러 사용자들의 게시글을 수집하여 스크리닝 질문지를 생성하는 과정으로, 사용자의 경험을 분석하여 결정 나무(decision tree)를 훈련시키고, 이를 통해 두 집단(질병 집단과 대조군) 간의 차이를 구분하는 결정 규칙을 생성합니다. 이 과정에서 Word Mover's를 통해 증상 간 거리를 측정하고, 집합적 군집화(agglomerative clustering)를 통해 의미가 유사한 증상들을 그룹화합니다.

- **Performance Highlights**: 연구 결과, 생리적 상태에 대한 평균 Pearson 상관계수는 자궁내막증(endometriosis) 0.58, 루푸스(lupus) 0.40, 통풍(gout) 0.27로 확인되었습니다. 이는 스크리닝 질문지 생성 과정이 부분적으로는 자동화 가능함을 나타냅니다. 그러나 수집된 질문지가 증상의 맥락, 지속 시간, 시점을 포착하는 데 한계가 있음을 지적합니다.
...
### FedUHB: Accelerating Federated Unlearning via Polyak Heavy Ball Method (https://arxiv.org/abs/2411.11039)
- **What's New**: 본 논문은 federated learning (FL) 환경에서 "right to be forgotten" 요구사항을 충족하기 위해 고안된 새로운 접근 방식인 FedUHB를 제안합니다. FedUHB는 특정 데이터의 모델에 대한 영향을 정확히 제거하는 방법으로, Polyak heavy ball 최적화 기술을 활용합니다.

- **Technical Details**: FedUHB는 exact unlearning을 위해 first-order method인 Polyak heavy ball 최적화를 사용하며, unlearning 과정 종료를 최적화하는 동적 중단 메커니즘을 도입합니다. 이를 통해 unlearning 효율성을 높이고 계산 자원과 통신 자원을 절약할 수 있습니다.

- **Performance Highlights**: 실험 결과, FedUHB는 unlearning 효율성을 향상시킬 뿐만 아니라, unlearning 후에도 견고한 모델 성능을 유지합니다. 동적 중단 메커니즘은 unlearning 반복 횟수를 효과적으로 줄여줍니다.



### EfQAT: An Efficient Framework for Quantization-Aware Training (https://arxiv.org/abs/2411.11038)
Comments:
          12 pages, 5 figures

- **What's New**: 이번 논문에서는 Quantization-aware Training (QAT)와 Post-training Quantization (PTQ)의 이점을 결합한 새로운 방법, EfQAT를 제안합니다.

- **Technical Details**: EfQAT는 사전 훈련된 모델에 PTQ를 적용한 후, 가장 중요한 네트워크 파라미터만 업데이트하고 나머지는 고정하여 역전파를 가속화합니다. 이 방식은 QAT의 효과를 유지하면서도 더욱 효율적입니다. EfQAT는 Convolutional Neural Networks (CNNs)와 Transformer 기반 모델에서 성능을 검증했습니다.

- **Performance Highlights**: EfQAT는 PTQ 기법보다 3% 높은 정확도를 기록하며, 여러 GPU에서 1.44-1.64배의 역전파 속도 향상을 보여줍니다. ResNet-50에서 ImageNet 데이터셋을 사용하여 검증하였고, BERTbase 모델에서는 SQuAD에서 6 F1 향상을 달성했습니다.



### Training a Label-Noise-Resistant GNN with Reduced Complexity (https://arxiv.org/abs/2411.11020)
- **What's New**: 본 연구는 Label Ensemble Graph Neural Network (LEGNN)이라는 새로운 방법을 소개하여 그래프의 label noise(레이블 노이즈)에 대한 강건한 GNN 훈련을 위한 저복잡도 접근법을 제안합니다. LEGNN은 node classification with label noise (NCLN)를 label ensemble task로 재정의하여 신뢰할 수 있는 단일 레이블을 구축하는 대신 여러 개의 정보성 레이블을 수집하여 알고리즘의 복잡성을 줄입니다.

- **Technical Details**: LEGNN은 두 단계로 구성된 과정으로 운영됩니다. 첫 번째 단계에서는 각 노드에 대해 무작위 이웃 마스크를 적용하여 높은 확률의 레이블 집합을 모읍니다. 두 번째 단계에서는 이러한 높은 확률 레이블 정보를 활용하여 모델 학습을 수행하는 partial label learning(부분 레이블 학습) 기반 전략을 사용합니다. 이 과정은 label noise의 영향을 줄이는데 도움을 주며, 최종적으로 노이즈가 포함된 레이블 집합을 균형 있게 모아서 신뢰성을 높입니다.

- **Performance Highlights**: 6개의 데이터셋에 대한 광범위한 실험을 통해 LEGNN은 뛰어난 성능을 발휘하며 효율성을 보장합니다. 또한, 100,000개 이상의 노드와 1,000,000개의 엣지를 가진 데이터셋에서 좋은 확장성을 보입니다.



### Towards a framework on tabular synthetic data generation: a minimalist approach: theory, use cases, and limitations (https://arxiv.org/abs/2411.10982)
- **What's New**: 본 논문은 합성 표 데이터 생성에 대한 미니멀리스트 접근 방식을 제안하고 연구하였습니다. 주된 구조는 비지도학습 기법인 SparsePCA 인코더와 구조화된 데이터 회귀 및 분류 작업에서 SOTA를 기록하고 있는 XGBoost 디코더로 구성됩니다. 이 방법은 비선형성을 처리하기 위해 클러스터링 단계를 포함하고 있습니다.

- **Technical Details**: 이 방법은 고차원 시뮬레이션된 신용 점수 데이터에 적용되어 실제 금융 응용 프로그램과 유사한 결과를 보여주었습니다. 여기서는 정보 압축을 통해 필요한 잠재적 표현을 구축하고 그로부터 확률적 샘플링 방식을 활용하여 합성 데이터를 생성합니다. 본 연구는 SparsePCA와 XGBoost의 결합 방식을 사용하는데, 이는 데이터 생성 과정에서 해석 가능성을 보장하며 추가적인 튜닝이 필요 없습니다.

- **Performance Highlights**: 케이스 스터디 결과는 이 방법이 모델의 강건성 테스트에 있어 원시 및 분위수 변동의 대안이 될 수 있음을 암시합니다. 제안된 SparsePCA+XGBoost 프레임워크는 간단하면서도 높은 해석 가능성을 제공하고, 데이터 생성 과정에서 오버피팅을 줄이는 장점을 가집니다.
...
### Efficient, Low-Regret, Online Reinforcement Learning for Linear MDPs (https://arxiv.org/abs/2411.10906)
Comments:
          27 pages, 9 figures

- **What's New**: 본 연구에서는 LSVI-UCB 알고리즘의 두 가지 변형을 제안하여, 공간과 시간 사용을 줄이면서도 서브리니어 (sublinear) 회귀를 유지하는 방법을 탐구합니다.

- **Technical Details**: 제안된 알고리즘은 LSVI-UCB-Fixed와 LSVI-UCB-Adaptive로, 전자는 고정된 학습 간격을 유지하며 작업 공간을 재설정하여 공간과 회귀 간의 균형을 잡고, 후자는 더 빨리 계산할 수 있는 측정 기준을 기준으로 학습 간격의 길이를 조정합니다.

- **Performance Highlights**: 실험 결과, 두 알고리즘 모두 낮은 공간 사용과 잔여 손실(regret)을 기록하며, 다양한 선형 MDP에 대해 효과적으로 작동하는 것을 보였습니다.
...
### Neuc-MDS: Non-Euclidean Multidimensional Scaling Through Bilinear Forms (https://arxiv.org/abs/2411.10889)
Comments:
          Accepted to 38th Conference on Neural Information Processing Systems (NeurIPS 2024)

- **What's New**: 본 논문에서는 Non-Euclidean-MDS(Neuc-MDS)를 소개합니다. 이는 고전적인 다차원 척도법(Multidimensional Scaling, MDS)의 확장으로, 비유클리드(Non-Euclidean) 및 비메트릭(non-metric) 입력을 수용합니다.

- **Technical Details**: Neuc-MDS는 비유클리드 데이터의 내적의 일반화를 통해 대칭 쌍선형 형태(symmetric bilinear forms)를 사용하여 비유클리드 공간에서의 STRESS를 최소화하는 알고리즘입니다. 이 알고리즘은 비유클리드 거리 행렬의 불일치 그램 행렬(dissimilarity Gram matrix)의 고유값(eigenvalues)을 최적화합니다.

- **Performance Highlights**: Neuc-MDS는 기존의 MDS를 대체하여 STRESS와 평균 왜곡(average distortion) 측면에서 업무 성능을 크게 향상시키고, 차원 수에 따른 역설(Dimensionality Paradox) 문제를 완전히 해결합니다. 실제 다양한 데이터셋에 대한 평가 결과, Neuc-MDS 및 Neuc-MDS+가 이전 기준보다 우수한 성능을 보였습니다.
...
### A Data-Efficient Sequential Learning Framework for Melt Pool Defect Classification in Laser Powder Bed Fusion (https://arxiv.org/abs/2411.10822)
- **What's New**: 이 연구는 Metal Additive Manufacturing (MAM)에서 중요한 melt pool 결함 분류를 위한 새로운 Sequential Learning (SL) 프레임워크인 SL-RF+ (Sequentially Learned Random Forest with Enhanced Sampling)를 제안합니다. 이는 적은 데이터 환경에서 데이터 효율성과 모델 정확성을 극대화하도록 설계되었습니다.

- **Technical Details**: SL-RF+는 RF 분류기를 사용하여 Least Confidence Sampling (LCS)과 Sobol sequence 기반의 합성 샘플링을 결합하여 정보성이 가장 높은 샘플을 반복적으로 선택하고, 최소한의 레이블된 데이터를 통해 모델의 결정 경계를 정제합니다. 이 프레임워크는 melt pool 결함을 식별하는 데 강력한 성능을 보여줍니다.

- **Performance Highlights**: SL-RF+는 정확도, 정밀도, 재현율 및 F1 점수를 포함한 주요 성능 지표에서 전통적인 머신 러닝 모델을 초과하여 데이터를 거의 사용하지 않고도 melt pool 결함을 식별하는 데 탁월한 강인성을 보였습니다.



### GeomCLIP: Contrastive Geometry-Text Pre-training for Molecules (https://arxiv.org/abs/2411.10821)
Comments:
          BIBM 2024

- **What's New**: 이 연구는 분자 구조와 생물 의학 텍스트를 기반으로 하는 다중 모드(multi-modal) 표현 학습을 통해 GeomCLIP 프레임워크를 제안합니다. 이는 기존의 3D 기하학적 정보와 생물 의학 텍스트를 통합한 새로운 접근 방식을 통해 분자의 특성을 효과적으로 모델링하고, PubChem3D라는 데이터셋을 통해 실질적인 실험을 진행하였습니다.

- **Technical Details**: GeomCLIP은 3D 기하학 인코더와 텍스트 인코더를 포함하며, 두 가지 유형의 작업인 다중 모드 표현 정렬(multi-modal representation alignment) 및 단일 모드 노이즈 제거 사전 학습(unimodal denoising pretraining)을 설계합니다. 이를 통해 3D 기하학 인코더와 텍스트 정보를 정렬하고, 결과적으로 원래의 표현력을 유지하도록 합니다.

- **Performance Highlights**: 실험 결과 GeomCLIP는 분자 특성 예측, 제로 샷 텍스트-분자 검색(zero-shot text-molecule retrieval), 3D 분자 캡셔닝(3D molecule captioning) 등 다양한 작업에서 효과적이라는 것을 보여주었습니다.



### An Oversampling-enhanced Multi-class Imbalanced Classification Framework for Patient Health Status Prediction Using Patient-reported Outcomes (https://arxiv.org/abs/2411.10819)
Comments:
          10 pages, 12 figures, 4 tables

- **What's New**: 이번 연구는 방사선 치료를 받는 암 환자로부터 직접 수집된 Patient-reported outcomes (PROs)를 활용하여 환자의 건강 상태 및 증상 예측을 위한 다양한 머신러닝 기법을 탐구합니다. 특히, 여러 종류의 머신러닝 분류기를 사용하여 다중 클래스 불균형 문제를 해결하려는 접근이 주목받고 있습니다.

- **Technical Details**: 연구에서는 Random Forest (RF), XGBoost, Gradient Boosting (GB), Support Vector Machine (SVM), Multi-Layer Perceptron with Bagging (MLP-Bagging), Logistic Regression (LR) 총 6개의 고급 머신러닝 분류기를 배치하여 세 가지 주요 암 유형(두경부암, 전립선암, 유방암)의 환자 결과를 예측합니다. 클래스 불균형 문제를 해결하기 위해 오버샘플링 전략을 사용하여, 이웃 샘플의 보간(interpolations)을 통해 소수 클래스의 샘플 수를 증대시키는 방법을 적용합니다.

- **Performance Highlights**: 실험 결과에 따르면 RF와 XGB 방법이 포괄적인 일반화 성능을 달성하였으며, 이를 통해 방사선 치료 후 결과를 경증, 중간, 중증으로 분류하는 데 효과적임을 보여주었습니다. 특히, weighted AUC 및 자세한 혼동 행렬(confusion matrices)을 통해 모델의 효과성과 임상적 활용 가능성이 강조되었습니다.



### Conformation Generation using Transformer Flows (https://arxiv.org/abs/2411.10817)
Comments:
          Technical Report. Code available at this https URL

- **What's New**: ConfFlow 모델은 그래프 기반의 딥러닝 네트워크를 활용하여 대규모 분자의 3D 구조를 빠르고 정확하게 생성할 수 있도록 개발되었습니다. 기존 방법과 달리, ConfFlow는 물리적 제약을 명시적으로 강제하지 않고 좌표 공간에서 직접 샘플링을 수행하여 유연성을 극대화합니다.

- **Technical Details**: ConfFlow는 transformer 네트워크를 기반으로 하며, 3D 포인트 클라우드 처리를 위한 point transformer 아키텍처를 활용합니다. 모델은 연속 정규화 흐름(Continuous Normalizing Flow, CNF) 프레임워크 내에서 원자 좌표의 반복적 소정을 통해 작동하며, 이는 MD 시뮬레이션의 힘 필드 업데이트와 유사하게 진행됩니다.

- **Performance Highlights**: ConfFlow는 GEOM 벤치마크에서 대규모 및 소규모 분자에 대해 시행된 실험에서 기존의 학습 기반 방법들에 비해 최대 40% 향상된 정확도를 보였습니다. 이는 분자의 3D 구조를 더 안정적이고 신뢰할 수 있는 방식으로 예측할 수 있음을 의미합니다.



### Stable Continual Reinforcement Learning via Diffusion-based Trajectory Replay (https://arxiv.org/abs/2411.10809)
Comments:
          10 pages, 3 figures, 1 table, inclusion at ICLR 2024 Workshop on Generative Models for Decision Making

- **What's New**: 이번 논문에서는 지속적인 Reinforcement Learning (RL) 환경에서 발생하는 catastrophic forgetting 문제를 해결하기 위한 새로운 알고리즘 DISTR (Diffusion-based Trajectory Replay)를 제안합니다. 이는 diffusion model을 사용하여 이전 작업의 고수익 경로 분포를 기억하고 새로운 작업 학습 중에 이를 활용하는 방식입니다.

- **Technical Details**: DISTR 알고리즘은 결정 정책과 diffusion model의 두 모듈로 구성됩니다. 결정 정책은 새로운 작업을 해결하는 방법을 학습하고, diffusion model은 경험한 작업의 skilled trajectories를 메모리화 합니다. 우리의 방법은 주요 작업의 skilled trajectories를 우선적으로 리플레이하는 메커니즘을 도입하여 데이터 재생의 비효율성을 줄입니다.

- **Performance Highlights**: 실험 결과, DISTR은 평균 성공률에서 기존의 다양한 지속적 RL 기준선들을 초과하는 성과를 보였으며, stability와 plasticity 간의 균형을 잘 유지한 것으로 나타났습니다. 또한, diffusion model이 trajectory distribution을 잘 재구성하는 능력을 보여주었습니다.



### On Reductions and Representations of Learning Problems in Euclidean Spaces (https://arxiv.org/abs/2411.10784)
- **What's New**: 본 논문에서는 Euclidean 공간에서 입력을 표현하고, 이산적인 0/1 분류 손실(discrete 0/1 classification loss)을 실수값 대리 손실(real-valued surrogate loss)로 대체하여, 분류 작업(classification tasks)을 확률적 최적화(stochastic optimization)로 축소하는 예측 알고리즘의 표현력을 조사합니다.

- **Technical Details**: 우리는 VC 차원(VC dimension)이 있는 개념 클래스(concept class)를 Stochastic Convex Optimization (SCO) 문제로 축소하는 데 필요한 최소 Euclidean 차원 D에 대한 경계를 설정합니다. Borsuk-Ulam 정리를 일반화하는 방법을 개발하였으며, 이는 고전적인 위상적 접근과 볼록성(convexity) 고려를 결합한 것입니다. 이 결과는 Kamath, Montasser, Srebro(COLT 2020)에서 제기된 미해결 질문을 해결합니다.

- **Performance Highlights**: 우리는 확률적 최적화(SCO) 문제로 축소할 때 필요로 하는 최소 차원을 설명하며, 무작위성(randomness)을 활용할 수 있는 자연적인 분류 작업을 제시하여, 무작위 초기화(random initialization)와 같은 기술을 통해 훨씬 작은 차원에서 표현할 수 있는 방법을 보여줍니다.



### Steam Turbine Anomaly Detection: An Unsupervised Learning Approach Using Enhanced Long Short-Term Memory Variational Autoencoder (https://arxiv.org/abs/2411.10765)
- **What's New**: 이번 연구에서는 증기 터빈의 정확한 이상 탐지를 위해 Enhanced Long Short-Term Memory Variational Autoencoder (ELSTMVAE-DAF-GMM) 모델을 제안하였습니다. 이 모델은 고차원의 시간-행렬 데이터에서 저차원 단계 공간으로의 변환을 통해 더 나은 탐지 성능을 제공합니다.

- **Technical Details**: 제안된 ELSTMVAE-DAF-GMM 모델은 LSTM (Long Short-Term Memory)과 VAE (Variational Autoencoder)를 통합하여 높은 차원의 데이터를 처리합니다. 훈련 과정에서 DAE-LOF (Deep Autoencoder-Local Outlier Factor) 샘플 선택 메커니즘을 사용하여 내재된 이상을 제거하며, 새로운 깊은 고급 기능(DAF)은 LSTMVAE 모델의 잠재 임베딩과 재구성 불일치를 결합해 구조화된 단계 공간 내에서 데이터 표현을 개선합니다.

- **Performance Highlights**: 산업 증기 터빈의 실제 운영 데이터를 활용한 실험에서, 제안된 모델은 기존 방법에 비해 높은 정확도를 유지하며 낮은 오탐률로 뛰어난 이상 탐지 결과를 나타냈습니다.



### ML$^2$Tuner: Efficient Code Tuning via Multi-Level Machine Learning Models (https://arxiv.org/abs/2411.10764)
Comments:
          Accepted in NeurIPS 2024 workshop on Machine Learning for Systems, 12 pages, 5 figures

- **What's New**: 이 논문에서는 복잡한 딥러닝 모델을 위한 효율적인 튜닝 기법인 ML$^2$Tuner를 제안합니다. ML$^2$Tuner는 유효성 예측 모델을 포함하여 잘못된 구성(configurations)을 필터링하고 성능 예측 모델을 통해 실행 시간을 단축합니다.

- **Technical Details**: ML$^2$Tuner는 다단계 기계학습(Machine Learning) 튜닝 기법으로, 유효성 예측 모델(Model V)과 성능 예측 모델(Model A)을 결합합니다. 이 과정에서 숨겨진 특징(hidden features)을 활용하여 최적 성능을 예측하는 방식입니다. 이 시스템은 딥러닝 가속기(DL accelerators)에서 유효한 설정만을 평가하여 잘못된 프로파일링 시도를 평균 60.8% 감소시킵니다.

- **Performance Highlights**: 실험 결과, ML$^2$Tuner는 TVM 방법과 비교해 ResNet18의 모든 레이어에서 동등한 성능 개선을 이끌어내며, 필요 샘플 수를 12.3%만 사용하여 성능을 개선하였습니다.
...
### On-device Anomaly Detection in Conveyor Belt Operations (https://arxiv.org/abs/2411.10729)
Comments:
          Preprint submitted to IEEE Transactions on Instrumentation and Measurement

- **What's New**: 이 연구에서는 Mining 4.0 분야에 적용되는 두 가지 독특한 패턴 인식 접근 방식을 제안하여 실시간 이상 감지를 수행합니다. 이는 Feature extraction, threshold-based cycle detection, 및 Tiny machine-learning classification을 결합하여 안전하고 효율적인 운영을 지원합니다.

- **Technical Details**: 제안된 방법은 두 개의 저전력 마이크로컨트롤러에서 구현되어 있으며, 추론 시 에너지 소비가 각각 13.3 및 20.6 μJ로 나타났습니다. 첫 번째 접근 방식은 F1-score가 97.3% 및 80.2%로 첫 번째 데이터셋에서 가장 높은 성능을 달성하며, 두 번째 접근 방식은 두 번째 데이터셋에서 각각 91.3% 및 67.9%를 기록했습니다.

- **Performance Highlights**: 두 가지 접근 방식 모두 F1-score 측면에서 최신 기술보다 뛰어난 성능을 보였으며, 제안된 솔루션은 기계적 고장 원인을 감지하고, 예방 정비를 지원하며, 생산 주기를 최적화하는 데 유용한 통찰력을 제공합니다.



### Multi Scale Graph Neural Network for Alzheimer's Diseas (https://arxiv.org/abs/2411.10720)
Comments:
          Findings paper presented at Machine Learning for Health (ML4H) symposium 2024, December 15-16, 2024, Vancouver, Canada, 9 pages

- **What's New**: 이 연구에서는 Alzheimer’s disease (AD)의 복잡한 신경퇴행성 과정을 이해하기 위해 다중 스케일 그래프 신경망(GNN) 모델인 ALZ PINNACLE을 개발하였습니다. 이 모델은 AD 진행 과정의 생물학적 맥락을 반영하여 신경 세포, 단백질, 세포 유형 간의 상호작용을 신뢰성 있게 학습합니다.

- **Technical Details**: ALZ PINNACLE은 단백질-단백질 상호작용(protein-protein interaction, PPI) 그래프와 세포 타입의 관계를 모델링하기 위해 설계되었습니다. 연구에서는 14,951개의 단백질, 206,850개의 단백질 간 상호작용, 7개 세포 타입 및 48개 세포 하위 유형을 활용하여 네트워크를 구축하였습니다. 또한, APOE 유전자의 임베딩을 분석하여 AD와 관련된 세포 유형 내에서의 역할을 예측했습니다.

- **Performance Highlights**: ALZ PINNACLE 모델은 AD 유전적 위험 인자 및 세포 유형들의 관계를 예측하는 데 뛰어난 성능을 보였습니다. APOE의 임베딩은 미세아교세포, 신경세포, CD8 세포 간의 높은 유사성을 보여주어 APOE의 다중 세포 타입에서의 공통적인 역할을 시사합니다. 이러한 결과는 ALZ PINNACLE이 AD 신경생물학에 대한 새로운 통찰력을 제공할 수 있는 유용한 프레임워크임을 제시합니다.



### FlowScope: Enhancing Decision Making by Time Series Forecasting based on Prediction Optimization using HybridFlow Forecast Framework (https://arxiv.org/abs/2411.10716)
Comments:
          12 pages and 6 figures

- **What's New**: FlowScope는 전통적인 통계 모델(ARIMA, SARIMA, ETS)과 딥러닝 모델(LSTM)을 결합하여 시간 시계열 예측을 위한 통합 프레임워크를 제공하는 혁신적인 접근 방식입니다. 이는 시간이 지남에 따라 데이터의 복잡한 패턴을 분석하고 예측할 수 있는 플랫폼을 목표로 합니다.

- **Technical Details**: FlowScope는 다양한 시계열 데이터의 특징(추세, 계절성, 사이클)을 효과적으로 포착할 수 있는 하이브리드 학습 방법론을 기반으로 구성되었습니다. ARIMA 모델은 선형 데이터에 적합하고, SARIMA 모델은 계절 변동을 잘 반영하며, ETS 모델은 추세를 포착하고 오류를 수정하는 데 강점을 보입니다. LSTM 모델은 복잡한 패턴 인식에 적합합니다.

- **Performance Highlights**: FlowScope는 다양한 예측 방법의 조합을 통해 사용자가 데이터 기반의 정보를 활용하여 장기 전략 및 최적화를 할 수 있도록 지원합니다. 이는 기업의 의사결정을 향상시키고, 예측의 정확성을 극대화합니다.
...
### Hybrid Attention Model Using Feature Decomposition and Knowledge Distillation for Glucose Forecasting (https://arxiv.org/abs/2411.10703)
- **What's New**: 이번 연구는 GlucoNet이라는 AI 기반 센서 시스템을 제안하여, 행동 및 생리 건강을 지속적으로 모니터링하고 혈당 패턴을 robust하게 예측하는 가능성을 탐구하고 있습니다. 특히 VMD(Variational Mode Decomposition) 기술을 활용하여 데이터의 주파수 성분을 세분화하고, LSTM과 Transformer 모델을 결합하여 혈당 예측의 정확성을 향상시키고자 했습니다.

- **Technical Details**: GlucoNet은 Multimodal attention model로, 혈당 수치 예측을 위해 LSTM과 Transformer을 결합함으로써 저주파 및 고주파 신호의 특징을 활용합니다. 이 모델은 VMD를 사용하여 혈당 데이터를 분해하고, 효과적인 인슐린 및 탄수화물 값과 결합하여 더욱 정확한 예측을 도모합니다. 또한 Knowledge Distillation을 통해 모델의 복잡성을 줄이고, 실시간 운영을 위해 최적화되었습니다.

- **Performance Highlights**: GlucoNet은 T1-Diabetes 환자 12명의 데이터를 분석하여 RMSE에서 60% 개선과 파라미터 수에서 21% 감소를 달성했습니다. 이러한 개선은 GlucoNet이 실제 당뇨병 예방 및 관리에서 신뢰할 수 있는 도구로 자리 잡을 수 있는 잠재력을 강조합니다.
...
### How to Defend Against Large-scale Model Poisoning Attacks in Federated Learning: A Vertical Solution (https://arxiv.org/abs/2411.10673)
- **What's New**: 본 논문은 Federated Learning(FL) 시스템에 있어 모델 오염 공격(model poisoning attack)에 대한 방어를 개선하기 위한 새로운 접근 방식, VERT를 제안합니다. 기존의 방어 방법은 수평적(solution) 방식을 기반으로 하여, 사용자의 모든 그래디언트를 사용해 최적의 집합을 찾는 방식이었으나, 이 방법은 대규모(>50%) 오염 공격에는 실패합니다. VERT는 역사적 그래디언트 정보를 이용하여 사용자 그래디언트를 예측하고, 실제 사용자 그래디언트와의 유사성을 비교하여 최적의 집합을 선택합니다.

- **Technical Details**: VERT는 통신 라운드(communication round)를 수직 축(vertical axis)으로 사용하며, 과거(역사적) 그래디언트 정보를 사용하여 사용자의 그래디언트를 예측합니다. 또한, 저차원 벡터 프로젝터(low dimensional vector projector)를 설계하여 사용자 그래디언트를 계산 복잡성을 줄여서 처리합니다. 이러한 구조를 통해 VERT는 대규모 모델 오염 공격에 견고한 방어 메커니즘을 제공합니다.

- **Performance Highlights**: VERT는 다양한 FL 시나리오에서 실험을 통해 80% 이상의 대규모 모델 오염에 대해 효율적이고 확장 가능한 방어 효과를 보여줍니다. 실험 결과, VERT는 IID 및 non-IID 데이터 분포에서 모두 큰 성능을 발휘하며, 공격자의 정보나 능력에 대한 비현실적인 가정을 필요로 하지 않습니다.
...
- **What's New**: 이번 연구는 결핵(TB) 환자의 치료 예측을 개인화된 모델을 통해 개선하는 새로운 방법론을 제시합니다. 특히 HIV, 당뇨병, 빈혈 등의 동반 질환이 환자 치료에 미치는 복잡한 영향을 분석하기 위해 "contextualized modeling"이라는 다중 작업 학습 접근법을 사용합니다.

- **Technical Details**: 연구진은 TB Portals 데이터셋을 활용하여 3,000명 이상의 TB 환자에 대한 동반 질환, 치료 방법 및 환자 결과 간의 상호작용을 구조적으로 파악했습니다. 이 모델에서 개인별 치료 효과를 정량화하기 위해 각 환자의 특이한 분포를 사용하는 개인화된 모델을 정의하였고, 이 과정에서 심층 학습을 통해 환자의 맥락을 모델링합니다.

- **Performance Highlights**: 연구의 결과, 빈혈, 발병 연령, HIV가 치료 효능에 미치는 영향을 식별하였으며, 이 개인화된 모델은 환자 특정 통찰을 제공하여 다양한 환자 집단에서 예측 정확도를 향상시킬 것으로 기대됩니다. 이는 새로운 개인 맞춤 치료 접근 방식에 기여할 것입니다.



### Drift-Resilient TabPFN: In-Context Learning Temporal Distribution Shifts on Tabular Data (https://arxiv.org/abs/2411.10634)
Comments:
          Accepted at the 38th Conference on Neural Information Processing Systems (NeurIPS 2024)

- **What's New**: 본 논문에서는 Drift-Resilient TabPFN이라는 새로운 접근 방식을 제안합니다. 이는 기존의 머신 러닝 모델이 동일하고 독립적인 데이터 개수를 요구하는 가정을 무시하고, 변칙적인 분포 변화를 다루기 위해 설계되었습니다.

- **Technical Details**: Drift-Resilient TabPFN은 In-Context Learning을 기반으로 한 Prior-Data Fitted Network로, 전체 훈련 데이터를 입력으로 받아 하나의 forward pass(전방 전파)에서 테스트 세트에 대한 예측을 수행합니다. 이 네트워크는 구조적 인과 모델(Structural Causal Models, SCM)에서 파생된 합성 데이터셋에 대한 Bayesian inference(베이esian 추론)를 근사하도록 학습합니다.

- **Performance Highlights**: 이 방법은 18개의 합성 및 실제 데이터셋에 대한 종합적인 평가에서 XGB, CatBoost 및 TabPFN을 포함한 여러 기준 대비 큰 성능 향상을 보여주었습니다. 정확도는 0.688에서 0.744로, ROC AUC는 0.786에서 0.832로 개선되었으며, 강한 보정을 유지합니다.



### KAT to KANs: A Review of Kolmogorov-Arnold Networks and the Neural Leap Forward (https://arxiv.org/abs/2411.10622)
- **What's New**: 이번 논문은 Kolmogorov-Arnold Networks (KANs)에 대한 심도 있는 탐구를 통해 'curse of dimensionality'에 영향을 받지 않는 새로운 네트워크 구조를 제안합니다. KANs는 기계 학습 커뮤니티에서 주목받고 있으며, 이를 통한 성능 향상 가능성을 보여줍니다.

- **Technical Details**: KANs의 핵심 구성 요소는 Kolmogorov-Arnold representation theorem과 B-spline을 통한 함수 근사입니다. KAT는 고차원 연속 함수를 단일 변수 함수의 조합으로 표현할 수 있음을 보여주며, KANs는 이를 통해 높은 차원에서도 높은 성능을 유지할 수 있습니다.

- **Performance Highlights**: KANs는 다양한 분야에서 적용성을 보여주고 있으며, 예를 들어 시계열 분석 및 컴퓨터 비전에서 기존 MLP 아키텍처를 초월하는 성능을 기록하였습니다. 또한, KANs는 데이터 양을 크게 줄이면서도 신뢰성 높은 모델 생성을 가능하게 합니다.



### Electrical Load Forecasting in Smart Grid: A Personalized Federated Learning Approach (https://arxiv.org/abs/2411.10619)
Comments:
          This paper has been accepted by the IEEE Consumer Communications \& Networking Conference (CCNC), Jan. 2025

- **What's New**: 이 연구는 스마트 그드(Smart Grid)의 전력 수요 예측을 위해 개인화된 연합학습(PFL) 방법을 제안합니다. 이 접근 방법은 비독립적이고 동일 분포가 아닌(non-IID) 계량 데이터 환경에서 로드 예측의 효율성을 높이도록 설계되었습니다.

- **Technical Details**: 개인화된 연합학습(PFL)에서는 메타 학습(meta-learning)을 사용하여 각 클라이언트에 대한 학습률을 조정하고, 각 클라이언트가 가진 데이터의 품질에 따라 맞춤형 모델을 구축합니다. 이로 인해 클라이언트의 처리 용량, 데이터 크기 및 배치 크기에 따라 각각의 로드 예측이 향상됩니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안한 PFL 방법은 기존의 기계 학습(ML) 및 연합 학습(FL) 방법보다 더 높은 로드 예측 정확도를 보였습니다. 특히, 다양한 속성을 가진 데이터에 대해 효율적으로 대처할 수 있는 능력이 입증되었습니다.
...
- **What's New**: BioNeMo Framework는 컴퓨터 생물학 및 화학 AI 모델의 훈련을 지원하기 위한 개방형 프레임워크로, 256개의 GPU에서 30억 개 매개변수를 가진 BERT 기반의 pLM을 4.2일 만에 훈련할 수 있는 성능을 보여주며, 커뮤니티 기여를 통해 구성 요소를 통합할 수 있는 모듈식 디자인을 특징으로 합니다.

- **Technical Details**: BioNeMo Framework는 NVIDIA NeMo 및 Megatron-LM 위에 구축된 오픈 소스 파이썬 패키지 모음으로, 단백질 시퀀스 샘플링과 단일 세포 데이터를 위한 고성능 데이터 로더를 포함하고 있습니다. bionemo-core, bionemo-llm 및 bionemo-geometric과 같은 서브 프로젝트로 구성되어 있으며, 각 서브 패키지는 데이터 처리 및 모델 빌딩을 위한 필수 요소를 제공합니다.

- **Performance Highlights**: BioNeMo Framework는 ESM-2 모델을 33층, 6억 5천만 개 매개변수로 설정했을 때, 기본 Hugging Face Transformers 라이브러리에 비해 1.47배 더 빠른 훈련 속도를 기록했습니다. 또한 A100 GPU에서 62.3%의 모델 FLOPS 활용도를 달성하여, H100에서는 약 49.1%로 감소합니다.
...
### Debias-CLR: A Contrastive Learning Based Debiasing Method for Algorithmic Fairness in Healthcare Applications (https://arxiv.org/abs/2411.10544)
Comments:
...
- **What's New**: 본 논문은 인공지능 기반 예측 모델의 인구 통계학적 편향 (demographic bias)을 줄이기 위한 새로운 방법론인 Debias-CLR을 제안합니다.

- **Technical Details**: Debias-CLR은 Clinical BERT와 LSTM autoencoders를 활용하여 심부전 환자의 임상 노트에서 특징 추출 (feature extraction)을 진행합니다. 이 방법은 성별 및 인종에 따라 다른 예측 결과를 내놓는 문제를 해결하기 위해 대조 학습 (contrastive learning) 프레임워크를 개발하였습니다.

- **Performance Highlights**: Debias-CLR은 성별과 인종에 대한 편향을 감소시키면서, 환자의 병원 체류 기간 예측과 같은 다운스트림 작업에서 예측 정확도를 유지하는 데 성공했습니다. 이는 건강 불균형을 줄이는데 기여할 수 있는 공정한 모델임을 의미합니다.
...
- **What's New**: 이번 연구는 Prompted LLM(대형 언어 모델)의 개인 정보 유출 위험을 최초로 분석하고 이를 Membership Inference Attack(MIA)으로 세밀하게 조사했습니다. 연구 결과는 Prompted LLM이 프라이빗 데이터의 공개 위험이 상당하다는 것을 보여줍니다.

- **Technical Details**: 이 논문에서는 LLM을 가지고 극소수의 예제 기반의 프롬프트를 사용하여 다운스트림 과제를 수행하는 동안 발생할 수 있는 개인 정보 유출 위험을 분석합니다. 연구진은 MIA를 통해 프롬프트 데이터의 멤버십을 효과적으로 추론할 수 있는 방법을 통해 개인 정보 요리를 강조했습니다. 이를 통해 Prompted 모델이 Fine-tuned 모델보다 5배 더 취약하다는 것을 발견했습니다.

- **Performance Highlights**: Prompt Ensembling 기법을 통해 MIA의 성공률을 무작위 추측에 가깝게 낮출 수 있음을 입증하였습니다. Avg-Ens와 Vote-Ens 두 가지 구체적인 방법을 평가하며, 이러한 방법으로 MIA 성공률을 효과적으로 줄일 수 있음을 보여주었습니다.
...
- **What's New**: 이번 연구에서는 Diffusion Transformers (DiT) 모델의 추론 과정을 가속화하기 위해 SmoothCache라는 새로운 모델 불문 감면기법을 도입하였습니다. 이 방법은 인접한 diffusion timestep 간의 레이어 출력 사이의 높은 유사성을 활용하여, 추론 도중 주요 특성을 재사용하며 전반적인 속도를 개선합니다.

- **Technical Details**: SmoothCache는 작은 보정 세트에서 분석한 레이어별 표현 오류를 기반으로 하여, 다양한 denoising 과정의 단계에서 최적의 캐싱 강도를 적응적으로 결정합니다. 이 기법은 모델 고유한 가정이나 훈련 없이도 적용 가능하여, 다양한 DiT 아키텍처에서 성능을 높인다는 특징이 있습니다.

- **Performance Highlights**: 실험 결과, SmoothCache는 이미지 생성, 텍스트-비디오 변환 및 텍스트-오디오 생성 분야에서 8%에서 71%까지의 속도 향상을 보이며, 생성 품질을 유지하거나 개선시키는 성과를 낳았습니다. 이는 DiT 모델의 실시간 응용 가능성을 높이고, 강력한 DiT 모델의 접근성을 확대할 기회를 제공합니다.
...
### Physics-Informed Neural Networks for Electrical Circuit Analysis: Applications in Dielectric Material Modeling (https://arxiv.org/abs/2411.10483)
- **What's New**: 본 논문은 물리 법칙을 머신 러닝(Machine Learning) 모델에 직접 통합하여 학습 데이터의 필요성을 줄이는 Physics-Informed Neural Networks(PINNs)의 적용 가능성과 제한점에 대해 탐구합니다. 특히, DeepXDE 프레임워크를 활용하여 고전압 직류(HVDC) 시스템에서의 유전체 성질 관련 문제를 다루고 있습니다.

- **Technical Details**: 논문은 PINNs를 사용하여 전방 문제(forward problem)와 역방향 문제(inverse problem)를 해결하는 과정에서 RC 회로 모델을 이용해 유전체 자재의 동적 거동을 분석합니다. PINNs는 자동 미분(Automatic Differentiation)을 통해 미분 방정식(Partial Differential Equations, PDEs)과 그 경계 조건을 통합하여 다루며, 이를 통해 물리적으로 제약된 모델링을 수행합니다.

- **Performance Highlights**: 연구 결과, PINNs를 적용하여 전류 흐름을 예측하는 데 높은 정확성과 안정성을 달성했습니다. 특히, 로그 변환(ln(I))을 사용하여 데이터가 드문 경우에서도 예측 안정성을 크게 향상시켰습니다. 그러나 더 복잡한 시스템 처리에서는 저항(resistance)과 정전 용량(capacitance) 추출에 대한 도전 과제가 존재함을 나타내었습니다.



### Boolean-aware Boolean Circuit Classification: A Comprehensive Study on Graph Neural Network (https://arxiv.org/abs/2411.10481)
- **What's New**: 이번 논문에서는 Boolean 회로의 동적 그래프 구조와 정적 기능을 다루며, 새로운 'Boolean-aware' 속성에 기반한 매칭 동등 클래스에 대해 정의합니다. 이를 통해 Boolean 회로의 변환 가능성을 논의합니다.

- **Technical Details**: 제안된 연구 프레임워크는 그래프 신경망(Graph Neural Network, GNN)을 기반으로 하여 Boolean-aware 기능이 Boolean 회로 분류에 미치는 주요 요인을 분석합니다. 이 방법은 그래프 구조와 기능을 고려하여 회로를 분류하는 동시에, 기능 기반의 회로 분류 문제를 해결하려는 향후 연구 방향을 제시합니다.

- **Performance Highlights**: 실험 결과는 제안된 분석의 유효성을 입증하며, Boolean-aware 회로 분류 문제 해결을 위한 방향성과 기회를 보여주었습니다. 논문 승인 후 코드와 데이터셋이 공개될 예정입니다.



### Challenges in the Differential Classification of Individual Diagnoses from Co-Occurring Autism and ADHD Using Survey Data (https://arxiv.org/abs/2411.10479)
- **What's New**: 이 연구는 자폐증(Autism)과 주의력 결핍 과다 활동성 장애(ADHD)를 구별하기 위한 새로운 머신러닝 모델을 개발하였습니다. 이 모델은 행동 특징을 기반으로 자동화된 임상 결정 지원 시스템에 활용될 수 있습니다.

- **Technical Details**: 이 연구에서는 National Survey of Children's Health의 데이터를 사용하여 행동 특징 추출을 위한 핵심 기능 집합을 식별하였습니다. 이진 분류 태스크에서는 자폐증 또는 ADHD와 비진단을 구별하는 모델이 92% 이상의 민감도(sensitivity)와 94% 이상의 특이도(specificity)를 기록하였고, 4-way 분류 태스크에서는 자폐증, ADHD, 두 가지 모두, 또는 없음으로 구분하는 모델이 각각 65% 이상의 민감도와 66% 이상의 특이도를 나타냈습니다.

- **Performance Highlights**: 이진 모델은 성능이 양호했으나, 자폐증과 ADHD의 차별적 분류에서 상대적으로 낮은 성능은 발달 지연에 대한 임상 결정 지원 도구에서의 특이성 문제가 계속 존재함을 강조합니다. 그럼에도 불구하고, 이 연구는 전통적으로 임상 목적으로 사용되지 않았던 행동 질문지를 활용하여 소아 발달 지연을 위한 디지털 스크리닝 평가를 지원할 가능성을 보여줍니다.
...
### Constrained composite Bayesian optimization for rational synthesis of polymeric particles (https://arxiv.org/abs/2411.10471)
- **What's New**: 이 연구는 제약 조건 및 복합 Bayesian 최적화(Constrained and Composite Bayesian Optimization, CCBO)를 통합하여 효율적인 목표 값 최적화를 달성한 최초의 사례를 제공합니다. 이는 검증된 실험 데이터가 제한적인 상황에서도 가능하게 하였습니다.

- **Technical Details**: CCBO는 검증되지 않은 상태에서 실험의 가능성을 고려한 최적화 방법으로, electrospraying이라는 모사 문제를 통해 나노 제조 과정에서 입자 생산의 목표 크기를 전략적으로 조정했습니다. 이 방법은 전통적인 Bayesian 최적화 기법보다 더 나은 결정을 제공하였으며, 인간 전문가와 비교할 수 있는 결과를 도출했습니다.

- **Performance Highlights**: CCBO는 최소한의 초기 데이터와 알려지지 않은 실험 제약 조건 하에서도 4회의 반복을 통해 설계 목표에 도달하였습니다. 실험 결과, CCBO는 전기 분무(electrospraying)를 통해 직경 300 nm 및 3.0 μm의 poly(lactic-co-glycolic acid) (PLGA) 입자를 합리적으로 합성할 수 있음을 입증하였습니다.



### Pairwise Markov Chains for Volatility Forecasting (https://arxiv.org/abs/2411.11838)
Comments:
          14 pages, 9 figures

- **What's New**: 본 논문에서는 Pairwise Markov Chain (PMC)을 활용한 예측 알고리즘을 새롭게 제안합니다. PMC는 전통적인 Hidden Markov Model (HMM)을 확장한 확률적 그래픽 모델로, 특히 연속 값 예측에 대한 활용도가 낮았습니다. 본 연구에서는 PMC의 기능을 최대한 활용할 수 있는 방법론을 제시합니다.

- **Technical Details**: 제안된 알고리즘은 PMC를 통해 숨겨진 상태(hidden states)를 도입하고, 이를 통해 비정상성(nonlinearity)을 모델에 효율적으로 추가할 수 있도록 합니다. 이 모델은 기존의 예측 모델을 확장할 수 있는 잠재력을 가지고 있으며, 실시간으로 상태를 갱신하며 예측精度를 개선합니다. 또한, PMC는 다양한 시계열 예측 모델에 통합하여 동적 조정을 가능하게 합니다.

- **Performance Highlights**: 주요 테스트 결과, PMC 알고리즘은 전통적인 GARCH(1,1) 모델 및 Feedforward Neural Networks (FNN)와 비교하여 변동성 예측에서 우수한 성능을 보였습니다. 여러 통계적 시나리오에서 PMC의 도입이 예측 모델의 성능을 현저하게 개선함을 입증하였습니다.
...
- **What's New**: 최근 연구에서는 Kolmogorov-Arnold networks (KAN)라는 해석 가능한 신경망 구조를 사용하여 비선형 동적 시스템의 방정식 구조를 발견하는 새로운 프레임워크를 제안했습니다. 이 프레임워크는 신뢰할 수 있는 시스템 모델을 위한 효율적이고 해석 가능한 방법을 제공합니다.

- **Technical Details**: 프레임워크는 세 가지 주요 요소로 구성됩니다: i) SRDD (Sequentially Regularized Derivatives) 알고리즘을 통한 데이터의 노이즈 제거, ii) KAN을 사용하여 방정식 구조 식별 및 비선형 함수 제안, iii) PISF (Physics-Informed Spline Fitting) 알고리즘을 통한 과잉 함수 걸러내기. KAN은 다층 퍼셉트론(MLP)과는 달리 가중치가 고정된 것이 아니라 학습 가능한 활성화 함수를 사용합니다.

- **Performance Highlights**: 제안된 방법은 강제 듀핑 오실레이터, 반더 폴 오실레이터, 그리고 버거 방정식 등의 시스템에서 true equation으로 수렴하는데 성공했습니다. Bouc-Wen 모델에 대해서는 허용 가능한 히스테리시스 반응을 캡처하는 근사 모델을 제공하여 긍정적인 성과를 보였습니다.



### Parallelly Tempered Generative Adversarial Networks (https://arxiv.org/abs/2411.11786)
- **What's New**: 본 논문에서는 기존의 GAN 훈련 구조의 불안정성 문제를 해결하기 위해, 새로운 GAN 훈련 프레임워크를 제안합니다. 이 프레임워크는 오목 보간(convex interpolation)을 통해 생성된 일련의 tempered distributions를 활용하여 모드 붕괴(mode collapse)로 인한 훈련 문제를 완화합니다.

- **Technical Details**: 새로운 GAN 목적 함수는 생성기가 모든 tempered distributions를 동시에 학습할 수 있도록 하며, 이는 통계학(statistics)에서의 병렬 온도 조절(parallel tempering) 개념과 유사합니다. 모드 붕괴와 다중 양상(multimodality) 문제를 해결하기 위해 gradient들의 분산을 줄이는 접근 방식이 포함됩니다.

- **Performance Highlights**: 시뮬레이션 연구를 통해 제안된 방법이 기존의 인기 있는 훈련 전략보다 이미지 및 표 형식 데이터 합성에서 우수한 성능을 보이는 것을 입증하였습니다. 또한, 신뢰성 높은 AI 분야에서의 공정한 합성 데이터 생성에 대한 향상된 변형도 제안하였습니다.
...
### Debiased Regression for Root-N-Consistent Conditional Mean Estimation (https://arxiv.org/abs/2411.11748)
- **What's New**: 본 연구는 고차원 및 비모수 회귀 추정기를 위한 디바이싱(debiasing) 방법을 소개합니다. 이 방법은 기존의 비모수 추정기에 바이어스 교정 항을 추가하여 $\sqrt{n}$-일관성을 달성하도록 설계되었습니다.

- **Technical Details**: 비모수 회귀 방법은 데이터 기반 방식으로 회귀 함수 추정을 가능하게 하며, 이 과정에서 최소한의 가정을 요구합니다. 그러나 일반적으로 이러한 방법은 $\sqrt{n}$-일관성을 달성하지 못하며, 특히 머신 러닝 분야에서도 이러한 비일관성을 보입니다. 연구진은 원래의 비모수 추정기를 기반으로 조건부 기대 잔차를 추정하고 이를 바이어스 감소 항으로 통합하는 방법을 제안합니다. 이 접근법은 원래 추정기와 조건부 기대 잔차 추정기가 완화된 수렴 속도 조건을 만족하는 한 모델에 구애받지 않습니다.

- **Performance Highlights**: 제안된 추정기는 $\sqrt{n}$-일관성과 점근적 정상성(asymptotic normality)을 달성하며, 추정 정확도 향상과 신뢰 구간(confidence intervals) 구성의 간소화를 제공합니다.
...
### Learning Differentiable Surrogate Losses for Structured Prediction (https://arxiv.org/abs/2411.11682)
- **What's New**: 이 연구에서는 새로운 구조화된 손실 함수(structured loss function)를 제안하고, 이 손실 함수를 신경망(neural networks)을 통해 학습하도록 하는 새로운 프레임워크인 Explicit Loss Embedding (ELE)을 소개합니다. 이 방법은 Contrastive Learning을 통해 출력 훈련 데이터로부터 직접 손실 함수를 학습하는 방식을 적용합니다.

- **Technical Details**: ELE는 기존 구조화 예측 문제를 해결하기 위해 신경망에 의해 매개변수화된 구조화된 손실 함수를 학습하고, 이를 서포트 회귀(surrogate regression) 문제에 적용합니다. 이 프레임워크에서는 정규화된 특성 공간에서 제곱 유클리드 거리(squared Euclidean distance)를 기반으로 한 손실 함수가 사용되며, 이 과정에서 손실 함수의 미분 가능성(differentiability)을 보장합니다.

- **Performance Highlights**: 실험 결과, 이 방법은 텍스트-그래프 예측(task) 문제에서 기존의 미리 정의된 손실 함수를 사용하는 방법들과 유사하거나 더 나은 성능을 보여주었습니다. 특히 그래프 객체(graph objects)에 대한 디코딩 전략을 통해 새로운 구조를 예측할 수 있는 가능성을 탐색하였습니다.
...
- **What's New**: 이 논문은 고에너지 입자 충돌기에서 실시간 데이터 처리와 수집을 위한 기계 학습(ML)의 하드웨어 구현을 분석합니다. ML 알고리즘의 FPGA(Field Programmable Gate Array) 기반 최적화와 두 가지 합성 프레임워크인 SLAC Neural Network Library (SNL)와 hls4ml을 비교하여 성능과 자원 사용의 트레이드오프를 평가합니다.

- **Technical Details**: 기계 학습 알고리즘은 소프트웨어 라이브러리에서 하드웨어 설명 언어로 변환되어 FPGA에 배치될 수 있는 하드웨어 버전으로 합성됩니다. 이 과정에서 데이터 입력 속도, HLS(High-Level Synthesis)의 영향 및 리소스 사용을 고려하여 최적화된 네트워크 레이턴시를 달성하는 것이 중요합니다. 논문은 특히 LHC 및 미래의 원형 충돌기(Future Circular Collider)와 같은 환경에서 실시간, 자원 제약이 있는 환경에 적합한 하드웨어와 소프트웨어 구성을 선택하는 데 필요한 가이드를 제공합니다.

- **Performance Highlights**: SLAC Neural Network Library(SNL)과 hls4ml의 비교 분석 결과는 각각의 접근 방식의 강점과 한계를 강조하며, 실시간 ML 모델에 대한 요구 사항을 충족하기 위한 다양한 모델 크기의 실현 가능성을 조사합니다. 이러한 결과는 특히 고에너지 물리학 및 복잡한 사건의 실시간 데이터 분석에서 FPGA 솔루션을 사용하는 데 중요한 통찰을 제공합니다.
...
### On the physics of nested Markov models: a generalized probabilistic theory perspectiv (https://arxiv.org/abs/2411.11614)
Comments:
          21 pages, 5 figures, 5 tables; Comments are welcome!

- **What's New**: 본 논문은 인과 그래프(causal graph)가 주어진 조건에서 잠재 변수(latent variables)를 정량화하는 데 어려움이 있는 Bayesian 네트워크를 우회하기 위해, nested Markov 모델(nested Markov model)의 새로운 해석을 제공합니다. 이 연구는 일반화된 확률론(generalized probabilistic theory) 관점을 통해 기존의 모델을 재조명합니다.

- **Technical Details**: 저자는 nested Markov 모델의 동등성 제약 사항들이 이론(independently theory)과 관계없이 유효하다는 것을 증명합니다. 그러나, 이 모델은 일반적으로 상대성 원칙(relativity principles)과 온건한 확률 규칙(mild probabilistic rules)만을 따르는 수정된 물리 이론에서도 실행 가능한 분포가 아닌 것을 보여줍니다. 이를 통해 저자들은 고차원 Bell 유형 인과 구조에서 유도된 유효한 분포를 정의하는 새로운 인과 모델을 확립합니다.

- **Performance Highlights**: 이 새로운 모델은 상대성 원칙에 의해 발생하는 불평등 제약(inequality constraints) 및 고차원 조건 독립성(high-dimensional conditional independences)을 드러냅니다. 또한, 일반화된 확률론 체계에서 도입된 상태와 측정에 대한 제한이 새로운 인과 모델을 넘어선 추가적인 불평등 제약을 발생시킬 수 있음을 발견하였습니다. 이 결과는 Bayesian 네트워크, 일반화된 확률론, 그리고 nested Markov 모델 간의 분포 집합 간의 엄격한 차이를 보여주는 새로운 인과 구조를 발견하는 것으로 이어졌습니다.



### Robust Causal Analysis of Linear Cyclic Systems With Hidden Confounders (https://arxiv.org/abs/2411.11590)
Comments:
          18 pages, 2 figures

- **What's New**: 이 논문은 복잡한 시스템을 이해하고 분석하기 위한 인과성(causality) 접근 방식을 다루고 있으며, 특히 순환(cyclic) 모델과 숨겨진 혼란 변수(hidden confounders)에 대한 저항성을 높이기 위해 LLC(Linear system with Latent confounders and Cycles) 알고리즘을 확장합니다. 또한, 이 과정에서 원천 코드를 공개하여 재현 가능성(reproducibility)을 높이고 추가 연구를 촉진합니다.

- **Technical Details**: LLC는 숨겨진 혼란 변수와 순환 인과 관계를 처리할 수 있는 몇 안 되는 알고리즘 중 하나로, 개입(intervention) 데이터를 입력으로 받아 전체 선형 모델을 반환합니다. 이 논문은 원래 LLC 알고리즘의 저항성 속성을 이론적으로 분석하고, 저항성을 강화하기 위한 알고리즘의 확장도 제시합니다.

- **Performance Highlights**: 저항성 분석 및 확장된 LLC 알고리즘의 성능 평가를 통해 기존 알고리즘보다 더 강력한 성능을 제공할 수 있습니다. 다양한 테스트에서 노이즈(noise)나 아웃라이어(outlier)에 대한 저항성이 확인되었습니다.
...
### GNN-Based Code Annotation Logic for Establishing Security Boundaries in C Cod (https://arxiv.org/abs/2411.11567)
Comments:
          Submitted to the IEEE Symposium on Security and Privacy 2025

- **What's New**: 이 논문에서는 Trusted Execution Environments (TEEs)와 같은 보안상의 요구사항을 충족하기 위해 Code Annotation Logic (CAL)이라는 새로운 도구를 제안하는데, 이 도구는 자동으로 보안 민감한 코드를 식별하여 격리할 수 있도록 지원합니다.

- **Technical Details**: CAL은 그래프 기반 접근 방식을 통해 코드베이스를 분석하며, 새로운 특성 구성과 커스텀 그래프 신경망 모델을 사용하여 어떤 코드 부분이 격리되어야 하는지를 정확하게 판단합니다. CAL은 보안 민감한 코드의 정의, 포괄적인 소스 파일 데이터세트의 구축 및 레이블링, 특성 중심의 그래프 기반 데이터 준비 파이프라인을 포함한 기여를 합니다.

- **Performance Highlights**: CAL은 보안 민감한 코드 식별에서 86.05%의 recall, 81.56%의 F1 스코어, 91.59%의 식별률을 기록하여 뛰어난 성능을 보여주었습니다. 이 결과는 개발자가 TEE를 효율적으로 사용할 수 있도록 하여 애플리케이션의 공격 벡터를 감소시킵니다.



### Data-driven model reconstruction for nonlinear wave dynamics (https://arxiv.org/abs/2411.11556)
Comments:
          6 pages, 5 figures

- **What's New**: 이 논문은 복잡한 파동 매체에서 광파의 비선형 진화를 해석할 수 있는 머신 러닝 프레임워크를 제시합니다. 전통적인 딥 러닝 방법의 해석 가능성의 부족을 극복하고, 스파스 회귀(sparse regression)를 사용하여 미세한 격자 모델을 단순한 연속 모델로 전환하여 파동 패킷의 동역학을 예측할 수 있습니다.

- **Technical Details**: 저자들은 비선형 광학에서 시달림을 받는 honeycomb 격자에서 valley-Hall 도메인 벽의 파동 패킷 동역학을 연구합니다. 제안된 방법은 초기 데이터 기반에서 피드백을 통해 비선형 복잡성을 저하시켜 간단하고 효과적인 PDE 모델을 도출합니다.

- **Performance Highlights**: 이 연구에서 복원된 방정식은 선형 분산 및 비선형 효과(자기 경사 및 자기 집중)를 정확하게 재현합니다. 제안된 방법은 기존의 비대칭 해석적 방법론에서 보기 드문 효율성을 보이며, 다양한 상위 토폴로지 재료에서 복잡한 상호작용 동역학을 탐구하는 데 유용한 기법으로 자리매김할 것으로 기대됩니다.
...
### Hierarchical-Graph-Structured Edge Partition Models for Learning Evolving Community Structur (https://arxiv.org/abs/2411.11536)
- **What's New**: 본 연구에서는 동적 네트워크 내의 진화하는 잠재 커뮤니티를 포착하기 위한 새로운 동적 네트워크 모델을 제안합니다. 모델은 포아송-감마(edge partition) 모델을 사용하여 각 정점의 동적 엣지를 분해하며, 각 정점을 하나 이상의 잠재 커뮤니티에 할당합니다. 이는 네트워크의 상호작용을 계층적 전이 커널(hierarchical transition kernel)을 통해 모델링하며, 시간이 지남에 따라 커뮤니티의 진화를 추적하는 기능을 제공합니다.

- **Technical Details**: 제안된 모델은 비부정적인 정점-커뮤니티 멤버십(vertex-community memberships)을 사용하는 포아송-감마(edge partition) 모델을 기반으로 하고 있으며, 계층적 전이 구조를 통해 잠재 커뮤니티 간의 상호작용을 정의합니다. 이 모델은 시간 의존적 변수를 사용하여 커뮤니티의 상태를 특정 시점에서 나타내며, 그래프 구조를 강제하여 더욱 해석 및 설명 가능성을 강화합니다.

- **Performance Highlights**: 다양한 실제 동적 네트워크 데이터셋에 대한 실험 결과, 제안된 모델은 링크 예측(link prediction)과 커뮤니티 탐지(community detection)에서 이전의 최첨단(dynamic network models) 방법보다 월등한 성능을 보여주었습니다. 또한, 추정된 네트워크 매개변수는 모델의 설명 가능성을 잘 나타내었습니다.
...
- **What's New**: 이 논문은 사용자가 LLM(대형 언어 모델)과 상호작용할 때의 개인 정보 보호 문제를 다루며, 사용자 단에서 소형 언어 모델(SLM)을 활용하여 텍스트 위생 처리의 영향을 선제적으로 평가하는 아키텍처를 제안합니다. 이를 통해 무의미한 결과를 방지하고 자원의 낭비를 줄일 수 있습니다.

- **Technical Details**: 논문에서는 차별적 프라이버시(Differential Privacy)를 활용한 텍스트 위생 처리 방법에 대해 다루며, 이는 텍스트 데이터에 노이즈를 추가해 개인정보를 보호하는 기법입니다. 특히, 사전 훈련된 SLM을 통해 위생 처리된 텍스트가 LLM에 적합한지 평가할 수 있는 방법론이 소개되었습니다. SLM은 사용자 단에서 실제로 실행되며, 시스템의 입력 데이터가 특정 처리에 의해 의미가 손실되는지 여부를 알릴 수 있습니다.

- **Performance Highlights**: 실험 결과, 채택된 차별적 프라이버시 기법은 위생 처리된 텍스트에 대해 LLM이 유의미한 결과를 산출하지 못하게 하는 심각한 문제를 나타냈습니다. 특히, 길이와 노이즈 수준에 따라 결과의 변동성이 커지며, 공개적으로 이용 가능한 데이터셋을 기준으로 한 요약 작업에서 매우 낮은 유용성을 입증했습니다. 이는 사용자와 제공자 모두에 대한 비용과 자원 낭비 문제를 심각하게 부각시킵니다.
...
### A Modular Open Source Framework for Genomic Variant Calling (https://arxiv.org/abs/2411.11513)
- **What's New**: 본 논문은 DeepVariant를 DeepChem 프레임워크에 통합하여 변이 호출(variant calling) 파이프라인을 개선하고, 이를 통해 정확성과 신뢰성을 높이는 새로운 접근 방식을 제시합니다. 이 작업은 open-source 환경 내에서 진행됩니다.

- **Technical Details**: 변이 호출 파이프라인은 sequencing reads의 재정렬(realignment), 후보 변이 탐지(candidates variant detection), pileup 이미지 생성(pileup image generation) 등을 포함하며, 변이 분류(variant classification)는 수정된 Inception v3 모델을 사용합니다. 이 통합은 모듈형(featurizers)으로 설계되어 DeepChem의 확장성을 높입니다.

- **Performance Highlights**: DeepChem에 통합된 DeepVariant는 높은 정확도를 자랑하며, 기존의 DeepVariant보다 코드 라인이 약 1,500로 줄어들어, 연구자들이 실험 요구 사항에 맞게 더 쉽게 수정하고 확장할 수 있도록 했습니다. 또한, 고속 GPU 가속 및 Tensors 연산을 활용하여 성능을 최적화하였습니다.
...
### PALMS: Parallel Adaptive Lasso with Multi-directional Signals for Latent Networks Reconstruction (https://arxiv.org/abs/2411.11464)
Comments:
          48 pages

- **What's New**: 이 논문에서는 대규모 네트워크 구조를 추측하기 위한 새로운 분산 및 병렬 컴퓨팅 프레임워크인 PALMS(Parallel Adaptive Lasso with Multi-directional Signals)를 제안합니다. 이 방법은 종래의 네트워크 재구성 방법과 비교하여 더욱 효율적이고 적용 가능성을 높입니다.

- **Technical Details**: PALMS는 덴시티 패널 데이터(density panel data) 및 분산 알고리즘(distributed algorithms)을 통해 네트워크 구조를 효과적으로 추정합니다. 수학적으로, PALMS는 네트워크 동적 모델을 기반으로 설정된 최적화 문제를 해결하며, 이를 통해 기존 연결의 해석적(statistical) 성질을 유지합니다.

- **Performance Highlights**: 실험 결과, PALMS는 다양한 대규모 네트워크 구조에서 뛰어난 성능을 보여주었으며, 컴퓨팅 시간은 줄이면서 비슷한 정확도를 달성했습니다. 이 논문은 이론적 성질의 유지(consistency, asymptotic normality)를 보장합니다.
...
### Extended Neural Contractive Dynamical Systems: On Multiple Tasks and Riemannian Safety Regions (https://arxiv.org/abs/2411.11405)
Comments:
          arXiv admin note: substantial text overlap with arXiv:2401.09352

- **What's New**: 이번 논문에서는 Neural Contractive Dynamical Systems (NCDS)이란 신경망 아키텍처를 제안하며, 이는 자율 로봇의 안정성 보장을 제공합니다. 기존 연구에서 해결되지 않았던 여러 문제에 대해 심층적인 설명과 다양한 개선점들을 제시합니다.

- **Technical Details**: NCDS는 안정성을 보장하는 계약성(Contractive) 특성을 가지며, 이를 통해 학습-시연(Learning-from-Demonstrations) 접근 방식에서 안정성 보장을 쉽게 제공합니다. 또한, 여러 작업을 처리하기 위한 조건부 변형(Conditional Variant)과 불확실성을 기반으로 한 잠재적 장애물 회피(Uncertainty-driven Latent Obstacle Avoidance) 접근 방식을 도입했습니다.

- **Performance Highlights**: 실험 결과, 개발된 시스템은 일반적인 신경망의 유연성을 가지면서도 자율 로봇에 필요한 안정성 보장을 제공합니다.
...
### Modeling Multivariable High-resolution 3D Urban Microclimate Using Localized Fourier Neural Operator (https://arxiv.org/abs/2411.11348)
- **What's New**: 이 논문에서는 도시 미세 기후 예측을 위해 새로운 로컬 푸리에 신경 연산자(Local-FNO)를 제안합니다. 이 모델은 로컬 학습, 기하학적 인코딩, 패치 중첩을 이용하여 3차원 멀티 변수를 효과적으로 예측합니다.

- **Technical Details**: Local-FNO는 60초 동안 빠르게 변화하는 난류를 정확하게 예측하며, 평균 0.35 m/s의 속도 오차와 0.30 °C의 온도 오차를 기록합니다. 또한 이 모델은 2 km x 2 km 영역에서 10 m 해상도로 난류 패턴을 해석할 수 있습니다.

- **Performance Highlights**: Local-FNO는 단일 32 GB GPU에서 CFD 솔버보다 거의 50배 빠른 속도로 1억 5천만 개의 특성 차원을 처리할 수 있으며, 예측 오차를 23.9% 줄이고 난류 변동 상관성을 47.3% 개선했습니다.



### Accelerating spherical K-means clustering for large-scale sparse document data (https://arxiv.org/abs/2411.11300)
Comments:
          28 pages, 23 figures

- **What's New**: 이 논문은 대규모 고차원 희소 문서 데이터 세트를 위한 가속된 구형 K-평균 군집화 알고리즘을 제안합니다. 이 알고리즘은 현대 컴퓨터 시스템의 CPU에서의 성능 저하 요인을 억제하는 아키텍처 친화적 방식으로 작동합니다.

- **Technical Details**: 제안된 알고리즘은 'architecture-friendly manner (AFM)'을 활용하며, 유니버설 특성 (universal characteristics, UCs)을 이용하여 유사도 계산에 필요한 곱셈 횟수를 최소화합니다. 또, 두 개의 구조적 파라미터를 도입하여 고빈도 (high-df) 용어와 높은 평균 특성 값이 포함된 특정 지역을 추출하고, 이를 통해 효율적 가지치기를 합니다.

- **Performance Highlights**: 실험을 통해 제안된 알고리즘이 최신 기술을 사용하는 알고리즘에 비해 대규모 문서 처리에 있어 속도 성능이 우수함을 입증하였습니다.



### Coupled Integral PINN for conservation law (https://arxiv.org/abs/2411.11276)
- **What's New**: 본 논문에서는 Coupled Integrated PINN(물리정보신경망)의 새로운 접근법을 제시하며, 이는 비선형 보존법칙을 다룰 때의 한계를 극복하고 충격파를 모델링하는 데 효과적입니다.

- **Technical Details**: 비선형 보존법칙에 대한 약한 형태(Weak Form)를 해석하고, 정방향 문제(Foward Problem) 해결을 위한 두 개의 신경망을 사용하는 방법론을 제안합니다. 이를 통해 기존의 수치적 접근법들의 복잡함을 피하고, 정방향 해의 적분형태를 기반으로 한 신경망을 이용합니다.

- **Performance Highlights**: Burgers 방정식, Buckley-Leverett 방정식 및 Euler 시스템을 포함한 다양한 예제를 통해 제안된 Integrated PINN이 기존의 vanilla PINN보다 더 높은 성능을 발휘하며, 보존법칙을 정확하게 다루는 데 강력한 일반화 능력을 보여줍니다.



### ACE2: Accurately learning subseasonal to decadal atmospheric variability and forced responses (https://arxiv.org/abs/2411.11268)
Comments:
          31 pages, 23 figures

- **What's New**: 이번 연구에서는 ACE2(Ai2 Climate Emulator version 2)를 소개하며, 이 모델은 지난 80년간 대기의 변동성을 재현할 수 있는 기계 학습 기반의 기후 에뮬레이터입니다. ACE2는 450M 파라미터를 가진 자가 회귀(autoregressive) 시스템으로, 6시간의 시간 해상도와 1°의 수평 해상도를 제공합니다.

- **Technical Details**: ACE2는 총 8개의 수직 레이어를 활용하며, 전 세계의 건조 공기 질량과 수분을 정확히 보존합니다. 사용자는 지정한 해양 표면 온도(SST) 경계 조건 아래 무한히 많은 시간 스텝을 안정적으로 진행할 수 있으며, 하루에 약 1500년의 시뮬레이션을 처리할 수 있는 처리량을 가지고 있습니다.

- **Performance Highlights**: ACE2는 엘니뇨 변동성에 대한 대기의 반응을 정확하게 재현하고, 지난 80년간의 온도 글로벌 트렌드도 충실히 반영합니다. 특히, ACE2는 트로피컬 사이클론과 매든-줄리안 진동은 물론, 다른 기후 현상들의 추적 성능 또한 우수합니다.
...
### Data Driven Automatic Electrical Machine Preliminary Design with Artificial Intelligence Expert Guidanc (https://arxiv.org/abs/2411.11221)
- **What's New**: 이번 논문은 전통적인 설계 방식과는 달리 인공지능(AI) 기반 전문가 데이터베이스를 활용한 전기 기계 설계(EMD) 프레임워크를 제안합니다. 특히, 이 프레임워크는 사용자의 요청 사양에 따라 직접적으로 예비 설계를 제공합니다.

- **Technical Details**: 기계 길이와 직경을 포함한 기본 설계 변수를 스윕(sweep)하여 2D 유한 요소(FE) 기계 모델을 사용하여 초기 데이터를 생성합니다. 이 데이터는 Metamodel of Optimal Prognosis (MOP) 기반의 대체 모델을 교육하는 데 사용됩니다. 이 대체 모델은 설계 변수를 핵심 성능 지표(KPI)와 매핑합니다.

- **Performance Highlights**: 설계 사례로 30kVA WRSG가 검증되었으며, 데이터베이스에서 선택된 설계 No.1138은 기존의 설계 방식보다 월등히 높은 전력 밀도인 2.21 kVA/kg를 5초 만에 달성했습니다. 전통적인 방법에서는 2.02 kVA/kg를 얻기 위해 수일이 소요되었습니다.
...
### Countering Backdoor Attacks in Image Recognition: A Survey and Evaluation of Mitigation Strategies (https://arxiv.org/abs/2411.11200)
- **What's New**: 이 논문은 이미지 인식에서의 백도어 공격(backdoor attack)을 방어하기 위한 기존의 완화 전략에 대한 포괄적인 분석을 제공합니다. 저자들은 16가지 최첨단 접근법을 8가지 다양한 백도어 공격에 대해 평가하고, 각 접근법의 이론적 근거, 실제 효능, 제한점 등을 심도 있게 논의합니다.

- **Technical Details**: 연구에서는 3개의 데이터셋, 4개의 모델 아키텍처, 3개의 오염 비율을 이용해 총 122,236개의 실험을 수행했습니다. 다양한 기존 방법들을 비교하고, 백도어 공격에 대한 방어의 효과성을 평가했습니다. 각 방어 전략의 성능은 상당히 달라지며, 최신 방법들이 전체 성능이나 일관성을 크게 개선하지 못했다는 점을 강조합니다.

- **Performance Highlights**: 대부분의 방어 방법들이 일정 수준의 보호를 제공하지만, 기존의 두 가지 주요 방법에 비해 발전이 없는 경향을 보입니다. 실험 결과, 접근법들이 실질적인 개선 없이 다양한 설정에서 일관성을 결여하고 있는 것으로 나타났습니다.
...
### Accelerating Quantum Emitter Characterization with Latent Neural Ordinary Differential Equations (https://arxiv.org/abs/2411.11191)
- **What's New**: 이번 연구에서는 깊은 신경망 모델을 사용하여 소량의 잡음이 있는 상관 함수로부터 전체와 잡음 없는 PCFS 실험을 예측할 수 있는 잠재적 신경 일반 미분 방정식(latent neural ordinary differential equation, NODE) 모델을 제안했습니다. 이 모델은 실험적 데이터 수집 시간을 약 20배 빠르게 줄이는 데 기여할 수 있습니다.

- **Technical Details**: NODE는 측정된 광자 상관 함수를 초기값 문제로 인코딩하고, 이를 다양한 간섭기 지연 시간에서 전파할 수 있는 능력을 갖추고 있습니다. 또한 추가적인 푸리에 손실 항(Fourier loss term)을 도입하여 시간 영역에서의 진동성 행동을 학습합니다. 

- **Performance Highlights**: 본 연구에서 제안한 방법은 10개의 잡음이 있는 광자 상관 함수로부터 최대 200개의 단계 위치에 해당하는 전체 de-noised 간섭 이미지를 추출할 수 있어, 실험적 데이터 수집 시간을 약 3시간에서 10분으로 줄일 수 있음을 보여줍니다.



### Mixing Neural Networks and Exponential Moving Averages for Predicting Wireless Links Behavior (https://arxiv.org/abs/2411.11185)
Comments:
          preprint, 6 pages, 2024

- **What's New**: 이 연구는 Wi-Fi 링크 품질 예측을 위한 두 가지 신경망 모델을 도입하고, 기존 방법론에 비해 더 높은 정확성을 보여줍니다. 이를 통해 산업 환경에서의 통신 성능 최적화를 위한 새로운 가능성을 제시합니다.

- **Technical Details**: 본 연구에서는 Neural Network for Wireless Channel Quality Prediction (NN-WCQP)이라는 새로운 접근 방식을 제안합니다. 이 접근 방식은 신경망(NN)과 무한 임펄스 응답(IIR) 저역 통과 필터를 결합하여 링크 품질을 예측합니다. 필터는 서로 다른 평활 계수를 사용하여 지수 이동 평균(EMA)을 계산하고, 이를 NN에 입력하여 현재의 링크 품질 예측을 생성합니다.

- **Performance Highlights**: 실험 결과 NN-WCQP는 전통적인 이동 평균 기반 방법보다 일관되게 우수한 성능을 보였으며, 환경 변화에 대한 견고성과 정확성이 크게 향상되었습니다. 또한, 본 알고리즘은 관리 가능한 계산 복잡도를 유지하면서 높은 확장성을 가능하게 합니다.
...
### Robust Defense Against Extreme Grid Events Using Dual-Policy Reinforcement Learning Agents (https://arxiv.org/abs/2411.11180)
Comments:
          6 pages, 5 figures, submitted to the 2025 Texas Power and Energy Conference (TPEC)

- **What's New**: 이번 연구는 전력망 운영을 위한 강화학습(RL) 에이전트의 새로운 구성을 제안 하였습니다. 특히 PPO(Proximal Policy Optimization) 알고리즘과 그래프 신경망(GNN)의 통합을 통해 다양한 기후 및 사이버 공격 시나리오에 대한 안정성 평가를 진행합니다.

- **Technical Details**: 연구에서는 Grid2Op 프레임워크를 활용하여 에이전트를 교육하고, PPO 알고리즘으로 얻은 보상 기능을 통해 에이전트의 성능을 측정합니다. 이 모델은 N-k 비상 사태 평가에 대한 새로운 접근 방식을 제공하여, 복잡한 전력망 환경에서의 에이전트의 의사 결정을 지원합니다.

- **Performance Highlights**: RL 에이전트는 다양한 비상 상황에서의 전력망 안정성 평가에 있어 보다 유연하고 효율적인 접근을 가능하게 합니다. 에이전트의 성능은 보상 기능을 통해 수치적으로 표현되며, 기존의 전통적인 방법보다 뛰어난 대응 능력을 보여줍니다.
...
### Variational Bayesian Bow tie Neural Networks with Shrinkag (https://arxiv.org/abs/2411.11132)
- **What's New**: 본 논문은 Bayesian 신경망(Bayesian Neural Networks, BNNs)의 제한 사항을 해결하기 위해, 특정한 아키텍처에서의 의존성을 완화하고 Polya-Gamma 데이터 증강 기법을 적용한 새로운 'Bow Tie Neural Network' 모델을 제안합니다. 이는 깊은 신경망의 불확실성을 정량화하는데 있어 혁신적인 접근법을 제공합니다.

- **Technical Details**: 제안된 모델은 원래의 feed-forward rectified neural network를 변형한 것으로, stochastic relaxation을 통해 조건부 선형 및 가우시안 모델을 형성합니다. 이 모델은 sparsity-promoting priors를 활용하여 데이터를 기반으로 한 아키텍처 설계를 지원하고 있으며, 변분 추정(Variational Inference) 알고리즘을 도입하여 계층 간 독립성 및 분포적 가정 없이 근사합니다.

- **Performance Highlights**: 다양한 회귀 작업을 포함한 전통적인 테스트에서 제안된 방법은 기존의 Bayesian 알고리즘들과 비교하여 경쟁력 있는 성능을 보여줍니다. 특히, 여러 개의 변분 추정 알고리즘을 동시에 실행하여 불확실성 추정의 정확성을 높이는 방식에 관한 성과가 두드러집니다.



### An Investigation of Offline Reinforcement Learning in Factorisable Action Spaces (https://arxiv.org/abs/2411.11088)
Comments:
          Published in Transactions on Machine Learning Research (11/2024)

- **What's New**: 이 연구는 factorisable action spaces에 대한 offline reinforcement learning (RL)의 초기 분석을 진행합니다. 기존의 연구들이 주로 continuous 또는 소규모 discrete action spaces에 집중했던 것에 반해, 이 연구는 사실성 있는 행동을 기반으로 한 factorised 접근 방식을 제안하고 이러한 접근법의 유효성을 실증적으로 평가합니다.

- **Technical Details**: Value-decomposition을 기반으로 한 factorised 행동 공간에서 offline RL을 수행하며, 다양한 품질과 복잡성을 가진 데이터셋을 활용하여 여러 가지 offline 기법을 평가합니다. MDP (Markov Decision Process)를 정의하고, reward function, 환경 동역학, 할인 인자 등을 활용하여 기존의 Q-learning과 actor-critic 방법론에서 발생하는 오추정 편향을 완화하기 위한 방법을 모색합니다.

- **Performance Highlights**: 이 연구는 factorisable action spaces에서 RL을 수행할 때의 장점을 강조하며, 기존의 atomic action 표현보다 성능이 향상된다는 것을 보여줍니다. 연구를 통해 얻어진 데이터셋은 공개되어 있어 향후 연구자들이 이를 활용할 수 있도록 지원하고 있으며, 코드도 공개됩니다.



### Spectral Subspace Clustering for Attributed Graphs (https://arxiv.org/abs/2411.11074)
Comments:
          15 pages. Full version of the paper accepted to KDD 2025

- **What's New**: 이번 논문에서는 SCAG(Subspace Clustering for Attributed Graphs) 계산을 위한 두 가지 효율적인 알고리즘, S2CAG와 M-S2CAG를 소개합니다. 이들은 기존 연구에서의 계산 비용 문제와 그래프의 구조 및 속성 데이터를 효과적으로 활용하지 못하는 한계를 극복하기 위한 것입니다.

- **Technical Details**: S2CAG는 새로운 목적 함수와 두 가지 비가역적 제약 조건을 포함한 정제된 표현 모델을 기반으로 SCAG를 공식화합니다. 또한, 이론적으로 기반이 있는 문제 변환과 잘 설계된 적응 전략을 토대로 효율적인 선형 시간 최적화 솔버를 개발하였습니다. M-S2CAG는 모듈성을 최대화하는 설계를 통해 S2CAG와의 이론적 연결성을 강조합니다.

- **Performance Highlights**: S2CAG와 M-S2CAG는 8개의 벤치마크 데이터셋에서 17개의 경쟁 알고리즘과 비교하여 모든 기준선보다 우수한 클러스터링 품질을 보였으며, 높은 효율성을 유지합니다.



### Efficient Federated Unlearning with Adaptive Differential Privacy Preservation (https://arxiv.org/abs/2411.11044)
- **What's New**: 본 연구는 Federated Unlearning (FU)의 새로운 접근 방식인 FedADP를 제안합니다. 이 방법은 unlearning의 효율성과 개인 정보 보호를 동시에 달성하기 위해 적응형 차별적 개인 정보 보호 메커니즘을 도입하였습니다.

- **Technical Details**: FedADP는 budget allocation을 위한 지수 방정식을 사용하여 클라이언트가 공유하는 업데이트에서 민감한 정보를 보호하기 위해 Gaussian 노이즈를 추가하도록 유도합니다. 또한, 역사적 정보를 효율적으로 선택하는 이중 레이어 선택 전략을 사용하여 통신 및 저장 비용을 줄입니다.

- **Performance Highlights**: 실험 결과, FedADP는 다양한 데이터셋과 공격 시나리오에서 높은 정확성 및 공격에 대한 강한 저항력을 유지하는 것으로 나타났습니다. 이 방식은 unlearning의 효율성과 개인 정보 보호 간의 균형을 효과적으로 관리하는 것으로 검증되었습니다.



### Beyond Normal: Learning Spatial Density Models of Node Mobility (https://arxiv.org/abs/2411.10997)
- **What's New**: 본 논문은 두 차원 지형에서 이동하는 모바일 노드의 정적 밀도 함수의 모델링을 다룹니다. 이를 통해 다양한 네트워크 디자인과 최적화 문제에 기여할 수 있습니다. 특히, Möbius 분포를 이용한 밀도 모델링의 가능성을 탐구하며, 기하학적 대칭성을 유지하면서도 다양한 변화에 적응할 수 있는 유연성을 제공합니다.

- **Technical Details**: Möbius distributions를 이용한 혼합 밀도 네트워크(Mixture Density Networks, MDNs)를 적용하여, Gaussian 혼합 모델(Gaussian mixture models)과 비교합니다. Gaussian mixtures는 밀도를 근사하는 데 널리 사용되지만, 일반적으로 적절한 튜닝이 필요하다는 것을 강조합니다. 본 연구는 밀도 모델링에서 Möbius distributions의 우위를 설명하면서, 학습 과정에서의 취약성 또한 언급합니다.

- **Performance Highlights**: Möbius distributions를 사용하여, 밀도 모델의 성능이 Gaussian에 비해 해석 가능성이 높다는 것이 입증됩니다. 본 연구는 소규모 훈련 데이터 세트로도 높은 품질의 모델을 생성할 수 있음을 보여줌으로써, 자원의 배치에 대한 최적화 문제 해결에 기여할 것으로 기대됩니다.



### Program Evaluation with Remotely Sensed Outcomes (https://arxiv.org/abs/2411.10959)
- **What's New**: 최근 연구에서 전통적인 프로그램 평가 방식인 설문 조사 대신에 원거리 감지 변수(remotely sensed variables, RSVs)를 사용하여 치료 효과를 추정하는 방법이 제안되었습니다. 이 방법은 기존의 방법이 RS가 결과 변수인 후처리(post-outcome) 변수일 때 편향된 추정을 초래한다는 것을 증명합니다.

- **Technical Details**: 연구는 RSV가 있으며 치료 상태가 포함된 실험 샘플과 RSV 및 결과 레이블이 포함된 관찰 샘플이 주어졌을 때, 조건부 분포(conditional distribution)가 두 샘플에서 동일하다는 가정 하에 치료 효과를 비모수적으로 식별합니다. 이러한 방법론에서는 치료와 결과 간의 관계를 일관되게 추정할 필요가 없으며, 기계 학습(machine learning) 기술을 사용할 수 있습니다.

- **Performance Highlights**: 인도에서 대규모 공공 프로그램의 효능을 재평가하는 과정에서, 우리는 위성 이미지를 통해 이 프로그램이 지역 소비와 빈곤에 미치는 영향을 추정하고, 기존의 방법에 비해 연구 비용을 약 300만 달러 절감할 수 있었습니다. 대조적으로, 기존의 대리(substitute) 접근 방식을 사용할 경우 평균 제곱 오차(mean squared error)와 추정된 치료 효과의 편향이 50% 증가할 수 있음을 보여주었습니다.
...
### Constructing accurate machine-learned potentials and performing highly efficient atomistic simulations to predict structural and thermal properties (https://arxiv.org/abs/2411.10911)
- **What's New**: 이 연구에서는 Cu7P6S6 화합물의 열전기적 응용 가능성에 주목하며, ab initio 분자 동역학 (AIMD) 시뮬레이션에서 생성된 데이터셋을 기반으로 신경 진화 잠재력 (NEP)을 도입하였습니다.

- **Technical Details**: 이 논문에서는 기계 학습 (ML) 잠재력이 복잡한 원자 시스템을 모델링하는 데 있어 혁신적인 기여를 하고 있다고 설명하며, 전통적인 양자역학적 접근 방식을 초월하는 고정밀 과정의 필요성에 대해 논의합니다. 특히, moment tensor potential (MTP)과 NEP 두 가지 고급 ML 잠재력을 사용하여 Cu7P6S6의 구조적 성질을 체계적으로 조사하였습니다.

- **Performance Highlights**: MTP는 약간 더 높은 정확도를 제공하지만 NEP는 약 41배 빠른 계산 속도를 달성하여 정확성과 효율성 간의 절충을 강조합니다. 이 발견은 Cu 이온의 빠른 확산 및 동력학에 대한 미시적인 통찰력을 제공하여 Cu 기반 고체 전해질의 앞으로의 연구를 위한 길을 닦았습니다.



### Watermarking Generative Categorical Data (https://arxiv.org/abs/2411.10898)
- **What's New**: 본 논문에서는 생성 된 범주형 데이터에 대한 새로운 통계적 수중 인식 시스템을 제안합니다. 이 방법은 데이터 분포를 두 개의 구성 요소로 나누고, 하나의 구성 요소를 다른 구성 요소와의 결정론적 관계에 기반하여 수정함으로써 수중을 분포 수준에서 삽입하는 방식을 채택합니다.

- **Technical Details**: 제안된 시스템은 정량적 거리를 측정하고, 역 디코딩 알고리즘을 사용해 수중의 존재를 확인하며, 이는 원래 데이터 분포와의 총 변동 거리(total variation distance)를 측정하여 구성됩니다. 이 방법은 기존의 방법들과 달리 통계적 분포 관점에서 수중을 검증할 수 있도록 하기 위해 설계되었습니다.

- **Performance Highlights**: 이 연구의 효과는 이론적 분석과 경험적 검증을 통해 입증되었으며, 생성된 데이터가 재생성되더라도 삽입된 수중을 신뢰성 있게 감지할 수 있음을 보여줍니다. 이로 인해 현대의 합성 데이터 생성 방식에 매우 적합한 방법입니다.
...
### Building Interpretable Climate Emulators for Economics (https://arxiv.org/abs/2411.10768)
- **What's New**: 이 논문은 효율적이고 해석 가능한 탄소 순환 모형(carbon-cycle emulator, CCE)을 개발하기 위한 프레임워크를 제안합니다. 이는 통합 평가 모델(Integrated Assessment Models, IAMs)의 기후 모형의 일환으로써, 경제학자들이 선진 기후 과학에 정확하게 보정된 CCE를 맞춤형으로 구축할 수 있도록 지원합니다.

- **Technical Details**: 우리는 간단한 다중 저수지 선형 박스 모델 CCE를 제안합니다. 이 모델은 핵심 물리량을 보존하며 사용 사례에 맞게 조정할 수 있습니다. 3가지 CCE 모델이 제시되며, 각 모델은 다르게 설계되어 있습니다: DICE-2016을 복제한 3SR 모델, 육상 생물권을 포함한 4PR 모델, 그리고 용적 변화와 같은 동적 육지 사용 변화를 고려하는 4PR-X 모델이 있습니다. 이 CCE는 Earth System Models (ESMs) 또는 기타 기후 데이터 출처에 대해 사용자 맞춤 조정을 진행하는 데 필요합니다.

- **Performance Highlights**: 4PR-X 모델은 대기 중 탄소 농도와 평균 온도에 대해 더 나은 표현력을 제공하여, 지구 온난화와 정책 결정에 대한 중요한 통찰을 제공합니다. 연구 결과는 3SR 및 4PR 모델이 유사한 정책 관련 시뮬레이션 결과를 내지만, 4PR-X 모델은 대기 중 탄소 양 및 온도가 현저히 높게 나타나며, 이는 완화 노력 증가와 더 높은 사회적 비용(Cost of Carbon, SCC)의 필요성을 강조합니다.
...
### Wireless Resource Allocation with Collaborative Distributed and Centralized DRL under Control Channel Attacks (https://arxiv.org/abs/2411.10702)
Comments:
          This work has been submitted to the IEEE for possible publication

- **What's New**: 이 논문에서는 무선 자원 할당 문제를 다루고 있으며, 사이버-물리 시스템(CPS)에서 자원 할당 명령을 전달하는 제어 채널이 서비스 거부(DoS) 공격에 노출되는 상황을 고려합니다. 이를 위해, 협력적인 분산 및 중앙 집중식(CDC) 자원 할당의 새로운 개념을 제안합니다.

- **Technical Details**: CDC 자원 할당 정책을 최적화하기 위해, 새로운 CDC-딥 강화 학습(DRL) 알고리즘을 개발하였으며, 이는 기존의 DRL 프레임워크가 중앙 집중식 또는 분산 결정 문제 중 하나만을 모델링하는 점과 차별화됩니다. 이 시스템은 N개의 센서와 서버(원격 추정기)로 구성되며, 자원 할당 명령을 센서에게 전달하는 방송 제어 채널을 포함합니다.

- **Performance Highlights**: CDC-DRL 알고리즘은 최신 중앙 집중식 및 분산 DRL 기준을 훨씬 초월하는 성능을 보여주며, 소규모 및 대규모 시스템에서 원격 추정 오류를 각각 평균 13.0% 및 52.6% 감소시켰습니다.
...
### Series Expansion of Probability of Correct Selection for Improved Finite Budget Allocation in Ranking and Selection (https://arxiv.org/abs/2411.10695)
- **What's New**: 이번 논문에서는 Ranking and Selection (R&S) 문제의 유한 샘플 성능을 향상시키기 위해 Probability of Correct Selection (PCS) 개선을 위한 Bahadur-Rao 타입 확장을 개발했습니다. 전통적인 접근법은 큰 편차를 근사하는 방식이지만, 유한 샘플 상황에서는 정확성이 부족할 수 있습니다. 우리는 제한된 시뮬레이션 예산 하에서 PCS 근사를 개선하여 최적의 샘플링 비율과 예산에 의존하는 최적 조건을 더 정확하게 특성화했습니다.

- **Technical Details**: 논문에서는 유한 예산 할당 정책(Finite Budget Allocation, FCBA)을 제안하며, 이를 통해 최적 조건을 순차적으로 추정하고 샘플링 비율을 조정하는 방법을 설명합니다. 기존의 방법론과 달리, FCBA 정책은 여러 샘플링 비율을 조화롭게 균형을 이루도록 설계되었습니다. 또한, 논문에서는 낮은 신뢰도 시나리오에서의 비선형 PCS 행동을 처리하기 위한 세분화된 확장과 맞춤형 할당 전략을 제시합니다.

- **Performance Highlights**: FCBA 정책은 수치 예제를 통해 기존 방법들과 비교했을 때 PCS 성능이 우수함을 보여줍니다. 더욱이, 제안된 가까운 형식의 확장은 확률 밀도 함수(Probability Density Function, PDF)의 약한 경계 조건을 활용하여 유한 샘플 상황에서의 샘플링 성능을 현저하게 향상시키는 데 기여함을 확인했습니다.
...
### Deep Learning-Based Image Compression for Wireless Communications: Impacts on Reliability,Throughput, and Latency (https://arxiv.org/abs/2411.10650)
- **What's New**: 이 논문은 동적 채널 조건에서 효과적인 이미지 전송을 위한 학습 이미지 압축(learned image compression, LIC) 기반 아키텍처를 위한 적응형 및 점진적 파이프라인을 제시합니다. 특히, 하이퍼프라이어 모델과 벡터 양자화 생성적 적대 신경망(Vector Quantized Generative Adversarial Network, VQGAN) 두 가지 최신 모델을 조사하며, 두 모델의 점진적 버전을 제안하여 불완전한 채널 조건에서 이미지의 부분 전송 및 디코딩을 가능하게 합니다.

- **Technical Details**: 하이퍼프라이어 모델은 병목 지점에서 무손실 압축을 통해 탁월한 압축 성능을 달성하지만 비트 오류에 취약합니다. 반면 VQGAN 디코더는 채널 코딩이 없더라도 강력한 이미지 재구성 기능을 보여줍니다. 두 모델의 점진적 버전은 초기 저품질 이미지의 즉각적인 처리를 가능하게 하며, 채널 조건이 개선됨에 따라 이미지 품질을 점진적으로 향상시킬 수 있습니다.

- **Performance Highlights**: Kodak 고해상도 이미지 데이터셋을 사용한 평가 결과, 점진적 전송 프레임워크가 비점진적 방법에 비해 신뢰성과 지연 시간을 효과적으로 향상시키는 것으로 나타났습니다. 특히, 점진적 하이퍼프라이어 모델이 모든 신호 대 잡음 비율(Signal-to-Noise Ratio, SNR)에서 99.9 백분위 대기 시간이 가장 적은 것으로 확인되었으며, 낮은 SNR 시나리오에서 높은 처리량을 달성했습니다.
...
### To Shuffle or not to Shuffle: Auditing DP-SGD with Shuffling (https://arxiv.org/abs/2411.10614)
- **What's New**: 본 논문에서는 Differentially Private Stochastic Gradient Descent (DP-SGD)에서 Poisson sub-sampling 대신 shuffling을 사용하는 것이 이론적인 프라이버시 보증에 영향을 미친다는 것을 확인하고자 하며, 새로운 DP 감사 절차를 도입하여 이를 분석하였다.

- **Technical Details**: DP-SGD는 훈련 데이터를 배치로 처리하며, 각 단계에서 Poisson sub-sampling을 사용해 배치를 선택하는 전통적인 방법이 있다. 그러나 shuffling을 사용하여 배치를 선택하는 것이 일반화되었으나, 이에 대한 이론적 보증이 부족하다. 본 연구에서 제안한 Batched Gaussian Mechanism (BGM)이라는 간소화된 DP-SGD를 통해 shuffling의 영향을 감사하였다.

- **Performance Highlights**: 실험 결과, shuffling을 사용하여 훈련한 DP-SGD 모델에서는 최대 4배 더 높은 프라이버시 유출이 발생하며, 다양한 데이터 세트와 모델 아키텍처에서 프라이버시 유출 정도를 평가하였다. 또한, 두 가지 shuffling 변형인 부분 shuffling과 배치-후-쉐플을 조사하여 더 큰 프라이버시 유출을 발견하였다.
...
### Learning Quantitative Automata Modulo Theories (https://arxiv.org/abs/2411.10601)
Comments:
          30 pages, 13 figures, 1 table

- **What's New**: QUINTIC는 기존의 L-star 알고리즘의 변형을 사용하여 입력-출력 예제를 명시적으로 수집하는 대신, 제약 조건을 기반으로 결정론적 양적 오토마타를 학습하는 새로운 접근 방식을 제시합니다. 이로 인해 귀찮은 입력-출력 쌍의 수집을 피할 수 있습니다.

- **Technical Details**: QUINTIC는 현재 이용 가능한 제약 집합과 가정된 선호 모델, 양적 오토마타 클래스에 이론을 적용하여 유효한 오토마타를 유도합니다. 이 과정에서 반복적인 심화(iterative deepening)를 통해 검색의 완전성과 결과의 최소성을 보장합니다.

- **Performance Highlights**: QUINTIC는 합산, 할인 합산, 곱, 분류와 같은 다양한 양적 오토마타를 학습하는데 효과적임을 입증하는 여러 실험 결과를 보였습니다.
...
### User-wise Perturbations for User Identity Protection in EEG-Based BCIs (https://arxiv.org/abs/2411.10469)
- **What's New**: 이번 연구에서는 사용자별로 추가된 perturbation(섭동)이 EEG 신호에서 사용자 신원 정보를 학습 불가능하게 만들 수 있음을 처음으로 보여줍니다. 제안된 네 가지 유형의 사용자별 개인 정보 보호 섭동 기법(랜덤 노이즈, 합성 노이즈, 오차 최소화 노이즈, 오차 최대화 노이즈)을 통해 BCI 작업 정보에는 영향을 미치지 않고 사용자 신원 정보를 효과적으로 숨길 수 있습니다.

- **Technical Details**: 제안된 섭동 방법은 EEG 훈련 데이터에 추가되어 사용자 신원 정보가 학습되지 않도록 하며, 다양한 신경망 분류기와 전통적인 기계 학습 모델을 사용한 실험에서 이 방법의 강인성과 실용성을 입증하였습니다. 각 유형의 섭동은 모델이 EEG 데이터의 사용자 신원을 구분할 수 없도록 설계되었습니다.

- **Performance Highlights**: 여섯 개의 EEG 데이터셋을 이용한 실험에서, 제안된 섭동을 적용한 모델은 테스트 단계에서 사용자 신원 정보 구분이 불가능함을 보였고, BCI 작업 분류기는 성능 저하 없이 원본 테스트 데이터에서 거의 동일한 성능을 유지했습니다. 이는 효과적인 개인 정보 보호 기법으로서의 가능성을 높이며 미래의 연구 방향을 제시합니다.



### Neural decoding from stereotactic EEG: accounting for electrode variability across subjects (https://arxiv.org/abs/2411.10458)
Comments:
          Accepted for publication at the 38th Conference on Neural Information Processing Systems (NeurIPS 2024)

- **What's New**: 이 논문에서는 sEEG(대칭체 전기 뇌파 검사) 데이터를 사용하여 여러 피험자 간의 행동을 해독하기 위한 새로운 훈련 프레임워크와 아키텍처, seegnificant를 소개합니다. 기존 방법들이 각 피험자의 이질성으로 인해 데이터 통합에서 어려움을 겪는 가운데, 이 모델은 여러 피험자의 데이터를 통합하여 효과적인 spatiotemporal neural representations를 추출할 수 있습니다.

- **Technical Details**: seegnificant는 전극의 신경 활동을 CNN(합성곱 신경망)을 통해 토큰화하고, 시간 차원에서 self-attention(자체 주의)을 사용하여 장기적인 시간 의존성을 추출합니다. 3D 위치 정보를 라벨링하여 전극 차원에서 또 다른 self-attention을 수행하여 효과적인 spatiotemporal 신경 표현을 도출합니다. 이 모델은 21명의 피험자 데이터를 기반으로 설계되었으며, 사용된 데이터는 3600회의 행동 실험에서의 sEEG 기록으로, 다양한 recording session(녹음 세션)을 포함합니다.

- **Performance Highlights**: 이 모델은 행동 실험에서 피험자들의 반응 시간을 신경 데이터만으로 해독할 수 있음을 보여주었으며, 기존 피험자 데이터를 통한 사전 훈련된 신경 표현을 새로운 피험자에게 적은 학습 예제로 전이할 수 있는 능력을 입증했습니다. 이를 통해 sEEG 데이터 통합을 위한 확장 가능한 접근법을 제시하며, 교차 피험자 일반화 가능성을 열어줍니다.

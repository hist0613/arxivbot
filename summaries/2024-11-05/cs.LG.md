...
### Dimension-free Private Mean Estimation for Anisotropic Distributions (https://arxiv.org/abs/2411.00775)
- **What's New**: 이 논문에서는 고차원 평균 추정 문제에서 차별적 개인 정보 보호(differential privacy, DP)를 유지하면서도 샘플 수에 대한 차원 의존성을 줄일 수 있는 새로운 알고리즘을 제안합니다. 기존의 추정기는 대칭 분포에 대해 최적이었으나, 비대칭(subgaussian) 데이터에서는 효과적이지 않았습니다. 이를 해결하기 위해, 저자들은 비대칭 구조를 가진 신호에 적합한 추정기를 개발하였습니다.

- **Technical Details**: 제안된 알고리즘은 특정한 이중 샘플 수 조건 하에 동작하며, 각 데이터 샘플이 알려진 공분산(proxies) Σ를 가질 때, 정확한 평균 추정치 
\hat{\mu}이 존재합니다. 이 알고리즘은 차원에 무관한 샘플 복잡성을 가지며, 특히 비대칭 잠재(variable) 데이터에 최적화되었습니다. 제안된 추정기는 수학적으로 최적의 샘플 복잡도를 달성하며, 이전 알고리즘의 충분한 샘플 수의 차원 의존성을 극복합니다.

- **Performance Highlights**: 제안된 알고리즘의 성능은 차원과 무관하게 샘플 복잡도를 감소시킬 수 있는 특징이 있으며, 이에 따라 고차원의 데이터에서도 비대칭 분포를 효과적으로 처리할 수 있게 됩니다. 샘플 수 요구사항은 기존 대칭 추정기로 설정된 것에 비해 크게 줄어들며, 이것은 실제 데이터에 기반한 설정에서 평균 추정을 할 수 있는 효율성을 증가시킵니다.



### Minibatch Optimal Transport and Perplexity Bound Estimation in Discrete Flow Matching (https://arxiv.org/abs/2411.00759)
- **What's New**: 최근 연구에서는 비연속 데이터 배포에 대한 모델링이 중요한 과제가 되고 있으며, 특히 Autoregressive 모델에 비해 Continuous Diffusion 및 Flow 모델이 많이 사용되고 있습니다. 이 논문은 Discrete Flow Matching이라는 새로운 프레임워크를 제공하여 비연속 데이터와의 경쟁력 있는 성능을 보여줍니다.

- **Technical Details**: 이 연구에서는 dynamic-optimal-transport-like 최적화 목표를 제안하여 비연속 흐름에서 상태 전이를 최소화하고, Kantorovich 형식으로 이를 도출합니다. 또한 Discrete Flow 모델이 가지는 확률 밀도 추정의 문제를 해결하기 위해, 혼란도(perplexity)의 상한을 제시하고 이를 바탕으로 평가 및 비교 가능성을 높였습니다.

- **Performance Highlights**: 이 연구는 군집 최적화(minibatch strategy)를 통해 효율적인 학습을 지원하며, 기존의 Continuous Flow에서 가져온 결과들을 Discrete Flow로 전이 가능함을 보입니다. 특히, 모델의 성능 평가가 가능해져서, 다양한 접근법과의 비교를 통해 본 모델의 강점을 더욱 부각할 수 있게 되었습니다.



### Hierarchical Transformer for Electrocardiogram Diagnosis (https://arxiv.org/abs/2411.00755)
Comments:
          5 pages,3 figures,under review by ISBI 2025

- **What's New**: 이 논문은 ECG 신호 분석을 위한 새로운 계층적 Transformer 아키텍처를 제안하며, 다수의 단계로 모델을 분할하여 임베딩의 공간적 크기를 평가함으로써 추가적인 다운샘플링 전략이나 복잡한 주의 메커니즘을 필요로 하지 않는다.

- **Technical Details**: 모델은 6층의 CNN(Convolutional Neural Network) 인코더를 활용하여 ECG 신호에서 특징을 추출하고, Depth-wise convolution를 이용해 여러 개의 리드 간의 관계를 보존한다. 분류 토큰(CLS token)을 사용하여 정보 집계를 수행하고, 가벼운 attention gate 메커니즘으로 리드 간의 연관성을 학습한다.

- **Performance Highlights**: 제안된 모델은 다양한 임베딩 네트워크와 입력 크기에 유연하게 적응하며, ECG 신호 분석에서 Transformers의 해석 가능성을 향상시킨다. 실험 결과는 다중 스케일 처리와 리드 특화 정보의 활용이 ECG 분석에서의 이전 Transformer 기반 방법들과 중요한 차별점을 시각적으로 보여준다.



### Private, Augmentation-Robust and Task-Agnostic Data Valuation Approach for Data Marketplac (https://arxiv.org/abs/2411.00745)
- **What's New**: 본 논문은 PriArTa라는 혁신적인 task-agnostic 데이터 가치 평가 방법을 소개합니다. 이 방법은 구매자의 기존 데이터셋과 판매자의 데이터셋 간의 분포 거리(computing the distance)를 계산하여 구매자가 새로운 데이터가 자신의 데이터셋을 얼마나 효과적으로 향상시킬 수 있는지 판단할 수 있게 합니다.

- **Technical Details**: PriArTa는 통신 효율성이 높아, 구매자가 각 판매자에게서 전체 데이터셋에 접근하지 않고 평가를 할 수 있도록 합니다. 대신, 구매자는 판매자에게 특정 전처리(preprocessing)를 수행하고 결과를 전달할 것을 요청합니다. 이러한 정보를 사용하여 구매자는 데이터셋을 평가하고, 더불어 판매자의 데이터셋의 프라이버시를 보호하면서 점수를 계산할 수 있습니다. PriArTa는 또한 일반적인 데이터 변환(transformation)에 강건성을 유지하여 일관된 가치 평가를 보장합니다.

- **Performance Highlights**: 실제 이미지 데이터셋에 대한 실험을 통해 PriArTa의 효과성을 демонстрация하였으며, 프라이버시를 보호하면서 효과적으로 데이터 가치를 평가할 수 있음을 증명했습니다.
...
### Exploring Multi-Modality Dynamics: Insights and Challenges in Multimodal Fusion for Biomedical Tasks (https://arxiv.org/abs/2411.00725)
- **What's New**: 이번 논문은 Han et al.(2022)에서 제안한 MM dynamics 접근법을 재조명하며, 생물 의학 분류 작업을 위한 다중 모드 융합(multi-modal fusion)에 대한 한계를 확인한 후 이를 보완하여 이미지 데이터에 적용한 Image MM dynamics를 개발했습니다.

- **Technical Details**: MM dynamics 알고리즘은 feature-level과 modality-level 정보의 통합을 통해 동적으로 모드를 융합하여 분류 성능을 개선합니다. 본 연구에서는 feature informativeness를 그림 데이터에 확장하여 U-net을 이용한 패치 수준의 feature informativeness 추정을 도입했습니다.

- **Performance Highlights**: 기존 방법들과 비교했을 때 Image MM dynamics는 정량적 성능에서는 향상을 보이지 않았으나, 질적 분석에서 유망한 결과를 보여주었습니다. 동적 융합 방법이 여러 데이터 세트에서 state-of-the-art 결과를 달성하는 데 유효하다는 주장을 검토하며, 각 샘플에 대한 modality의 중요성을 다르게 추정하였다.



### Token-level Proximal Policy Optimization for Query Generation (https://arxiv.org/abs/2411.00722)
Comments:
          10 pages

- **What's New**: 본 논문에서는 웹 검색 쿼리 생성에 대한 새로운 접근법인 Token-level Proximal Policy Optimization (TPPO)을 제안합니다. 이 방법은 LLMs의 쿼리 생성 성능을 향상시키기 위해 fine-tuning을 통해 설계되었습니다.

- **Technical Details**: TPPO는 Reinforcement Learning from AI Feedback (RLAIF) 패러다임 기반으로, token-level reward model과 token-level proximal policy optimization 모듈을 포함합니다. 기존의 sparse reward 문제를 해결하기 위해 토큰 단위로 보상을 부여합니다.

- **Performance Highlights**: TPPO는 공개 데이터셋과 산업 데이터셋에서 2%-4%의 쿼리 생성 관련성 향상을 보였고, PPO와 비교 시 2%-8% 더 높은 승률을 보였습니다. 훈련 과정에서 TPPO는 더 나은 수렴성을 나타내며, 실제 어플리케이션에서도 성공적으로 배포되었습니다.



### PedSleepMAE: Generative Model for Multimodal Pediatric Sleep Signals (https://arxiv.org/abs/2411.00718)
- **What's New**: 이번 논문은 Pediatric Sleep Masked Autoencoder (PedSleepMAE)를 소개합니다. 이는 다채널 소아 수면 신호(EEG, 호흡 신호, EOG, EMG 등)를 활용하는 최초의 일반 목적의 생성 모델로, 수면 스코어링 및 다양한 수면 이벤트 감지에 있어 지도 학습 모델과 비슷한 성능을 보여줍니다.

- **Technical Details**: PedSleepMAE는 masked autoencoder 기반의 딥러닝 모델로, 2,379개의 PSG를 활용하여 16개의 채널을 처리합니다. SSL(Semi-Supervised Learning) 방식을 사용하여, 대량의 비지도 데이터에서 유용한 임베딩을 학습하고 여러 하위 작업에 활용됩니다. UMAP을 사용하여 7680차원 임베딩의 진단 정보를 평가하였습니다.

- **Performance Highlights**: PedSleepMAE는 수면 이벤트 감지 및 수면 스코어링에서 높은 성능을 보이며, 소아 수면 질환에 대한 진단 및 예측 가능성을 높입니다. 또한, 임베딩을 통한 수면 생체 지표의 가설 생성 및 결측 데이터 보완 등 다양한 응용 가능성을 보여줍니다.
...
### Wasserstein Flow Matching: Generative modeling over families of distributions (https://arxiv.org/abs/2411.00698)
Comments:
          24 pages, 10 figures

- **What's New**: 본 논문에서는 Wasserstein flow matching(WFM)이라는 새로운 generative modeling 기법을 제안합니다. 이는 전통적인 flow matching 기법을 보강하여 데이터셋의 해당 기하학을 반영할 수 있는 방법을 제공합니다.

- **Technical Details**: WFM은 Wasserstein 기하학의 리만 기하학적 성질을 활용하여, 확률 분포들의 집합에서 흐름을 학습합니다. 이 알고리즘은 (entropic) optimal transport 이론과 우리의 신경망 아키텍처에서 주목(attention) 메커니즘을 활용합니다.

- **Performance Highlights**: WFM을 통해 가우시안 분포 간의 생성 모델링과 시각적 transcriptomics 데이터에서 세포 미세환경을 합성하는 방법을 보여주었습니다. 성능은 기존의 방법들과 비교했을 때 더 강력한 샘플을 생성하며, 고차원 및 가변 크기의 point-cloud 상황에서도 성공적인 결과를 보였습니다.
...
### Explainable few-shot learning workflow for detecting invasive and exotic tree species (https://arxiv.org/abs/2411.00684)
- **What's New**: 본 연구는 브라질 아틀란틱 포레스트에서 침입종 및 외래종 나무를 탐지하기 위한 설명 가능한 few-shot learning (소수 샷 학습) 워크플로우를 제안합니다. 이 방법은 제한된 라벨 데이터로도 효과적인 분류를 가능하게 하며, 결과에 대한 시각적 설명을 제공합니다.

- **Technical Details**: 제안된 워크플로우는 Siamese network (시암 네트워크)와 설명 가능한 인공지능(Explainable AI, XAI)을 통합하여 구성됩니다. MobileNet과 같은 경량 백본을 사용하여 3-shot learning(3샷 학습)에서 F1-score (F1 점수) 0.86을 달성했으며, 기존의 얕은 CNN 모델보다 성능이 우수합니다.

- **Performance Highlights**: 제안된 접근법은 적은 데이터 환경에서도 새로운 나무 종을 식별하는 데 효과적임을 보여줍니다. 설명 메트릭스(예: 정확성, 연속성, 대비성)와 시각적 사례를 통해 예측 결과에 대한 통찰을 더욱 제공합니다. 이 연구는 AI 및 UAV (무인 항공기)를 활용한 산림 관리와 생물 다양성 보전에 새로운 가능성을 열어줍니다.
...
### Rethinking Node Representation Interpretation through Relation Coherenc (https://arxiv.org/abs/2411.00653)
- **What's New**: 이번 논문에서는 Node Representation의 해석을 위한 새로운 방법인 Node Coherence Rate for Representation Interpretation (NCI)을 제안하였습니다. NCI는 노드 관계가 얼마나 잘 포착되었는지를 정량화하고, 기존의 해석 방법에 대한 정확성을 평가하는 새로운 알고리즘(IME)을 함께 도입합니다.

- **Technical Details**: NCI는 노드 임베딩이 특정 노드 관계를 얼마나 잘 포착하는지를 평가하여, 노드 간의 유사성을 기반으로 임베딩간의 거리와 일치성을 측정합니다. IME는 모든 해석 방법의 정확도를 평가하기 위한 프로세스로 발전되었습니다. 이 연구에서는 6개의 실제 그래프에서 NCI를 적용하여 해석 오류를 평균 39% 감소시키는 결과를 얻었습니다.

- **Performance Highlights**: NCI를 사용하여 다수의 그래프 기반 방법이 생성한 노드 표현의 질을 평가하였으며, IME와 NCI를 통해 정확도가 향상된 해석 결과를 제공하였습니다. 이 방법은 다양한 다운스트림 작업을 위한 고품질 피처 선택에 기여할 수 있음을 보여주었습니다.



### Variational Neural Stochastic Differential Equations with Change Points (https://arxiv.org/abs/2411.00635)
- **What's New**: 본 연구에서는 신경 확률 미분 방정식(neural stochastic differential equations, neural SDEs)을 사용하여 시계열 데이터에서 변화점(change points)을 모델링하는 새로운 접근 방식을 제안합니다.

- **Technical Details**: 변형 오토인코더(variational autoencoder, VAE) 프레임워크를 기반으로 한 새로운 모델 형식과 훈련 절차를 제안하며, 기존 알고리즘들과 달리 초기 상태에 대해 가우시안 사전(Gaussian prior)만 필요로 하고 전체 프로세스에 대한 Wiener 프로세스(Wiener process) 사전은 필요하지 않습니다. 이 모델은 변화점 추정과 모델링을 위한 두 가지 방법론을 개발하며, 반복 알고리즘을 통해 신경 SDE 파라미터와 변화점을 동시에 업데이트합니다.

- **Performance Highlights**: 제안한 모델은 고전적인 매개변수적 SDE(parametric SDE)로부터 실제 데이터셋에 이르기까지 효과적으로 모델링할 수 있는 능력을 보여주며, 변화 분포(distribution shifts)가 있는 데이터셋에서도 우수한 성능을 발휘합니다.



### Fast and scalable Wasserstein-1 neural optimal transport solver for single-cell perturbation prediction (https://arxiv.org/abs/2411.00614)
- **What's New**: 본 논문에서는 Wasserstein-1 ($W_1$) 이론을 기반으로 한 새로운 neural optimal transport solver를 제안합니다. 이 솔버는 기존의 $W_2$ 솔버의 복잡한 min-max 최적화 문제를 단일 1-Lipschitz 함수에 대한 최적화 문제로 단순화하여, 속도가 훨씬 빨라지는 장점을 가집니다.

- **Technical Details**: $W_1$ dual formulation을 통해 우리는 transport direction을 찾고, adversarial training을 통해 transport step size를 결정하여 transport map을 복구합니다. 두 단계의 학습 과정이 있으며, GroupSort neural network를 사용하여 1-Lipschitz Kantorovich potential을 매개변수화합니다. 이 과정은 transport direction을 고정한 후, step size function을 DNN으로 매개변수화하여 추가적인 discriminator를 통해 학습하게 됩니다.

- **Performance Highlights**: 제안된 $W_1$ neural optimal transport solver는 2D 데이터셋에서 $W_2$ OT 솔버와 비슷한 성능을 보이며, 실제 single-cell perturbation 데이터셋에서는 $W_2$ OT 솔버와 동등하거나 더 나은 성과를 달성합니다. 또한 25에서 45배의 속도 향상과 고차원 작업에서 더 나은 확장성을 제공하며, 고변동성이 있는 single-cell RNA-seq 데이터셋에 직접 적용할 수 있습니다.



### Provably and Practically Efficient Adversarial Imitation Learning with General Function Approximation (https://arxiv.org/abs/2411.00610)
Comments:
...
- **What's New**: 본 논문에서는 일반 함수 근사를 사용하는 온라인 적대적 모방 학습(adversarial imitation learning, AIL)의 이론적 기초를 탐구하며, OPT-AIL이라는 새로운 방법론을 소개합니다. 이 방법은 보상 함수에 대한 온라인 최적화 및 Q-value 함수의 낙관적 벨만 오차 최소화에 중점을 두고 있습니다.

- **Technical Details**: OPT-AIL은 두 가지 주요 목표를 최소화하는 데 초점을 맞춥니다. 보상 회수를 위해 온라인 최적화 문제를 무시하도록 설계하였으며, 정책 학습을 위해서는 낙관적 벨만 오류를 최소화하여 Q-value 함수를 유추하고 해당 정책을 도출합니다. 이론적으로는 OPT-AIL이 거의 전문가 수준의 정책 학습을 위한 다항식 샘플 복잡도 및 상호작용 복잡도를 달성함을 증명하였습니다.

- **Performance Highlights**: 실험적 연구에 따르면, OPT-AIL은 여러 도전 과제에서 이전의 최첨단 딥 AIL 방법보다 뛰어난 성능을 발휘하였습니다. 이는 이 알고리즘이 실제 적용에 용이함을 입증합니다.
...
### Improving self-training under distribution shifts via anchored confidence with theoretical guarantees (https://arxiv.org/abs/2411.00586)
Comments:
          NeurIPS 2024

- **What's New**: 본 논문에서 저자들은 분포 변화(distribution shifts)를 고려하여 self-training의 성능을 향상시키기 위한 새로운 접근법인 Anchored Confidence(AnCon)를 제안합니다. 이 방법은 높은 신뢰도의 예측을 기반으로 한 시간적 일관성(temporal consistency)을 활용하여, 기존의 복잡한 계산 없이 잘못된 pseudo label을 수정할 수 있습니다.

- **Technical Details**: AnCon은 예측 불확실성을 고려하여 예측의 가중치를 부여하는 일반화된 시간적 앙상블(generalized temporal ensemble)을 구성합니다. 그 후, 이 앙상블을 label smoothing에서 스무딩 벡터로 사용하여 noisy pseudo labels를 매끄럽게 합니다. 이론적으로, AnCon은 점근적으로 정당화되며, label smoothing 기법은 self-training의 최적성 갭을 줄일 수 있습니다.

- **Performance Highlights**: 실험 결과, AnCon은 다양한 분포 변화 시나리오에서 self-training 성능을 8%에서 16%까지 일관되게 향상시켰으며, 추가적인 계산 오버헤드 없이도 사용 가능합니다. 또한, AnCon은 향상된 보정 성능(calibration performance)과 하이퍼파라미터 선택에 대한 강인성을 보여주었습니다.
...
### Active Preference-based Learning for Multi-dimensional Personalization (https://arxiv.org/abs/2411.00524)
- **What's New**: 이번 논문에서는 사용자 선호를 평가하는 새로운 능동적 다목적 개인화 프레임워크를 제안합니다. 이 프레임워크는 비교 피드백을 활용하여 사용자 맞춤형 AI 모델 응답을 생성하도록 설계되었습니다.

- **Technical Details**: 본 연구에서는 Bayesian inference를 활용하여 다수의 목표에 대한 사용자 선호를 업데이트합니다. 사용자의 피드백을 기반으로 후행 확률 분포를 조정하며, 이 과정에서 취득 함수(acquisition function)을 사용하여 최적의 쿼리를 선택합니다. 노이즈를 처리하기 위한 파라미터도 도입되어 사용자 피드백의 정확성을 향상시킵니다.

- **Performance Highlights**: 이 연구는 언어 생성 작업에 대한 실험을 통해 제안된 접근법의 피드백 효율성과 효과성을 입증하였으며, 이론적 분석과 함께 다양한 모델 반응을 개인화하는 데 기여하고 있습니다.



### Analyzing Multimodal Integration in the Variational Autoencoder from an Information-Theoretic Perspectiv (https://arxiv.org/abs/2411.00522)
- **What's New**: 본 논문에서는 다중 모드 학습(Multimodal Learning)을 위한 새로운 접근 방식을 제안하며, 특히 다중 모드 변분 오토인코더(Multimodal Variational Autoencoder, VAE)의 통합을 분석하는 방법을 제시합니다.

- **Technical Details**: 다중 모드 VAE는 다양한 입력 모드(visual, auditory, tactile 등)를 통합하기 위해 설계된 구조로, 각 입력을 독립적인 네트워크를 통해 인코딩한 후 라텐트 공간(Latent Space)에서 결합합니다. 이 연구에서는 통계적 정보 측정 방식인 KL-divergence를 사용하여 다양한 모드의 정보가 입력 재구성에 미치는 영향을 평가합니다.

- **Performance Highlights**: 비교 분석을 통해 다양한 라텐트 손실의 가중치 조정 전략이 다중 모드 통합 성능에 미치는 효과를 실험하여 확인하였습니다. 이 연구 결과는 로봇의 인간 개발 단계를 모사하는 학습에 도움을 줄 수 있습니다.



### Outlier-Oriented Poisoning Attack: A Grey-box Approach to Disturb Decision Boundaries by Perturbing Outliers in Multiclass Learning (https://arxiv.org/abs/2411.00519)
- **What's New**: 본 논문에서는 Outlier-Oriented Poisoning (OOP) 공격이라는 새로운 형태의 데이터 중독 공격을 제안합니다. 이 공격은 결정 경계로부터 가장 멀리 떨어진 샘플의 레이블을 조작하는 방식으로 진행되며, 다양한 머신러닝 알고리즘에 미치는 부정적인 영향을 다루고 있습니다.

- **Technical Details**: 실험은 IRIS, MNIST, ISIC의 세 가지 공개 데이터셋을 사용하여 수행되었습니다. OOP 공격은 5%에서 25% 범위의 다양한 데이터 중독 수준에서 분석되며, 정확도, 정밀도(precision), 재현율(recall), F1 점수, 위양성률(false positive rate) 등의 변화를 평가하였습니다. K-최근접 이웃(KNN)과 가우시안 나이브 베이즈(GNB) 알고리즘이 가장 큰 영향을 받았으며, 반면 결정 트리(Decision Trees)와 랜덤 포레스트(Random Forest)는 상대적으로 저항력이 강했습니다.

- **Performance Highlights**: IRIS 데이터셋에서 15%의 중독 수준으로 KNN의 정확도가 22.81% 감소하고 GNB는 56.07% 감소했습니다. 반면, 결정 트리는 12.28%, 랜덤 포레스트는 17.52%의 정확도 감소를 보였습니다. 모델의 클래스 수가 증가할수록 성능 저하가 줄어드는 상관관계도 발견되었습니다.



### Zero-shot Generalization in Inventory Management: Train, then Estimate and Decid (https://arxiv.org/abs/2411.00515)
- **What's New**: 이 논문에서는 깊은 강화 학습 (Deep Reinforcement Learning, DRL)을 실제 재고 관리에 적용할 때의 도전 과제를 다루고, 제로샷 일반화(Zero-shot Generalization, ZSG)를 통해 일반적으로 유능한 에이전트(Generally Capable Agents, GCA)를 훈련하는 새로운 프레임워크인 TED(Train, then Estimate and Decide)를 제안합니다.

- **Technical Details**: TED 프레임워크는 세 가지 단계로 구성됩니다: 다양한 문제 인스턴스에서 GCA를 훈련한 후, 배포 중 문제 매개변수(Parameters)를 지속적으로 추정하며, 추정된 값을 바탕으로 결정을 내립니다. 또한, 제안된 Super-Markov Decision Process(Super-MDP) 모델은 기존의 Markov Decision Processes (MDPs)를 일반화하여 다양한 재고 문제 인스턴스를 통일적으로 처리할 수 있게 합니다.

- **Performance Highlights**: GC-LSN(Generically Capable Lost Sales Network)은 전통적인 정책들과 비교하여 문제 매개변수가 알려졌을 때 지속적으로 우수한 성능을 보였습니다. 초기 요구사항 및 리드 타임 분포가 불확실한 경우에도 TED 프레임워크 하에서 온라인 학습 방법과 비교하여 우수한 실증 성능을 기록했습니다.



### Label Cluster Chains for Multi-Label Classification (https://arxiv.org/abs/2411.00514)
Comments:
          26 pages, 11 figures, 5 tables

- **What's New**: 본 논문에서는 다양한 다중 레이블 분류(multi-label classification) 방법 중 Ensemble of Classifier Chains (ECC)를 개선하기 위해 레이블 공간 내에서 상관관계를 가진 레이블 클러스터를 분할하여 연결하는 새로운 방법인 Label Cluster Chains for Multi-Label Classification (LCC-ML)을 제안합니다.

- **Technical Details**: LCC-ML은 레이블 클러스터를 기반으로 하여 각 클러스터에서 다중 레이블 Random Forest을 기본 분류기로 활용합니다. 학습 단계에서는 클러스터의 실제 레이블을 피처로 사용하고, 테스트 단계에서는 예측된 레이블을 기반으로 다음 클러스터에 대한 피처를 생성합니다. 이 방법은 ECC의 기존 구조를 개선하여 높은 차원을 처리하고 레이블 간의 상관관계를 더 잘 활용합니다.

- **Performance Highlights**: LCC-ML은 14개의 다중 레이블 벤치마크 데이터셋에서 실험했으며, ECC와 비교하여 더 나은 성능을 보였습니다. 이러한 결과는 레이블 클러스터를 통한 학습 및 연결이 분류기의 예측력을 향상시킬 수 있음을 보여줍니다.



### Exploring the Precise Dynamics of Single-Layer GAN Models: Leveraging Multi-Feature Discriminators for High-Dimensional Subspace Learning (https://arxiv.org/abs/2411.00498)
Comments:
          Accepted for NeurIPS 2024, 16 pages, 7 figures

- **What's New**: 이 연구에서는 single-layer GAN 모델의 훈련 동역학을 subspace learning의 관점에서 탐구하며, 이를 새로운 접근법으로 재구성합니다. 특히 sequential feature learning을 넘어 non-sequential 시나리오에 집중하여 inter-feature interactions의 중요성을 강조합니다.

- **Technical Details**: 훈련 동역학을 이해하기 위해 gradient-based learning algorithms를 continuous-time stochastic process로 변환하며, 이 모델의 가중치 동역학은 stochastic differential equation으로 모형화됩니다. 이러한 두 개의 방정식을 통해 GAN의 훈련 수렴 행동을 분석합니다.

- **Performance Highlights**: 이 연구 결과에 따르면, GAN 모델이 Oja의 방법보다 더 의미 있는 특징을 학습하며, 특히 MNIST와 Olivetti Faces 같은 실제 데이터셋에서 강력한 성능을 보여줍니다. 또한, powerful discriminator를 사용했을 때 훈련 속도가 빨라지고 성능이 개선되는 것을 확인했습니다.
...
### Unlocking Your Sales Insights: Advanced XGBoost Forecasting Models for Amazon Products (https://arxiv.org/abs/2411.00460)
- **What's New**: 본 연구는 Amazon 플랫폼에서 소비자 전자제품의 판매 예측을 위해 XGBoost 모델을 활용하는 솔루션을 제안합니다.

- **Technical Details**: 판매량(sales volume) 데이터 대신 판매 범위(sales range) 값을 사용하여 모델의 정확성을 향상시켰습니다. XGBoost 모델의 예측 성능은 전통적인 모델들보다 우수하다는 것을 보여줍니다.

- **Performance Highlights**: 기존의 판매량 예측 방식으로는 불만족스러운 결과를 얻었으나, 판매 범위를 사용함으로써 만족스러운 정확도를 달성했습니다.



### Diffusion Models as Network Optimizers: Explorations and Analysis (https://arxiv.org/abs/2411.00453)
- **What's New**: 본 논문에서는 최신 네트워크 최적화 연구의 동향을 살펴보고, 생성적 확산 모델(Generative Diffusion Models, GDMs)의 활용 가능성을 제시합니다. 특히 이러한 모델들이 기존의 최적화 방법들보다 이유 있는 이점을 제공하는지를 이론적으로 증명합니다.

- **Technical Details**: GDM은 노이즈를 추가하는 전진 과정과 이 노이즈를 제거하는 역진 과정으로 이루어집니다. 이 과정은 확률 분포를 효과적으로 수렴시키기 위해 Fokker-Planck 및 Langevin dynamics를 활용합니다. GDM을 사용하여 주어진 입력에 대한 고품질 솔루션 분포를 학습하고, 이 분포에서 샘플링하여 최적의 해를 근사합니다.

- **Performance Highlights**: 이 연구는 세 가지 도전적인 네트워크 최적화 문제에 대해 GDMs의 성능을 검증했습니다. 다양한 모델 구성 및 GDMs의 원리를 탐구한 결과, 생성된 솔루션의 수렴이 가능하며, 최적 솔루션에 근접할 수 있음을 보여주었습니다.
...
### A KAN-based Interpretable Framework for Process-Informed Prediction of Global Warming Potentia (https://arxiv.org/abs/2411.00426)
- **What's New**: 이 연구에서는 전통적인 GWP(지구온난화 잠재력) 예측 모델이 주로 분자 구조에 의존하는 한계를 극복하기 위해 분자 설명자(molecular descriptors)와 프로세스 관련 정보를 통합한 새로운 GWP 예측 모델을 제안합니다.

- **Technical Details**: 모델은 MACCS keys 및 Mordred 설명자(mordred descriptors)와 프로세스 정보(프로세스 제목, 설명 및 위치)를 결합하여 작성되었습니다. DNN(deep neural network) 모델을 사용하여 Mordred 설명자와 프로세스 정보로 테스트 데이터에서 R-squared 값 86%를 달성했으며, 이는 이전 기준인 61% 보다 25% 향상된 결과입니다. 또한 Kolmogorov-Arnold Network (KAN)를 통해 GWP 예측을 위한 상징적 공식(symbolic formula)을 도출하여 투명한 해석을 제공했습니다.

- **Performance Highlights**: 모델은 데이터가 밀집된 범위에서 신뢰성 있게 작동하며, 높은 GWP 값에서는 불확실성이 증가하는 경향을 보였습니다. 이 분석을 통해 사용자는 예측 불확실성을 효과적으로 관리할 수 있으며, 지속 가능성 평가에서 유용한 도구로 활용할 수 있습니다.
...
### Black-Box Forgetting (https://arxiv.org/abs/2411.00409)
Comments:
          NeurIPS 2024

- **What's New**: 본 논문에서는 Black-Box Forgetting이라는 새로운 문제를 제안하고, 블랙 박스 모델을 위한 선택적 망각(selective forgetting) 접근 방식을 제안합니다. 이는 기계 학습 모델이 특정 클래스를 인식하지 못하도록 조정하는 작업입니다.

- **Technical Details**: 제안된 방법은 파라미터 및 그라디언트에 접근할 수 없는 블랙 박스 모델을 위한 것으로, 입력 텍스트 프롬프트를 최적화하여 특정 클래스를 잊도록 유도합니다. 이를 위해 고차원 최적화의 난이도를 완화하기 위해 Latent Context Sharing (LCS)이라는 방법론을 도입하여 여러 토큰 간의 공통 저차원 잠재 컴포넌트를 공유합니다.

- **Performance Highlights**: 실험 결과, 네 개의 표준 벤치마크 데이터셋에서 제안된 방법이 기존의 합리적인 기준선보다 우수한 성능을 보이고, zero-shot CLIP을 개선하는 데 성공했습니다.



### MoD: A Distribution-Based Approach for Merging Large Language Models (https://arxiv.org/abs/2411.00406)
- **What's New**: 이 논문에서는 기존의 LLM(대형 언어 모델)을 무게 기반의 방법이 아닌 확률 분포(output probability distribution)에 직접 적용하여 병합하는 새로운 방법인 Mixture of Distributions (MoD)를 제안한다. 이 방법은 각 모델의 전문적인 능력을 유지하면서도 효율적인 지식 공유를 가능하게 한다.

- **Technical Details**: MoD 방법은 모델 매개변수에 대한 혼합 분포를 구축하여 모델을 병합한다. 이를 통해 각 모델의 강점을 보호하고, 재앙적 망각(catastrophic forgetting)의 위험을 줄인다. Qwen2.5 모델을 활용한 실험을 통해, MoD가 전통적인 병합 기법들에 비해 매우 우수한 성능을 발휘한다는 것을 입증하였다.

- **Performance Highlights**: MoD는 다양한 수학적 추론 벤치마크에서 기존의 모델 병합 기술을 비롯한 여러 기준에 대해 우수한 성능을 보였으며, 다중 작업(multitask) 시나리오에서 특히 두드러진 결과를 나타냈다. 이 방법을 통해 LLM의 비용이 비싼 훈련 없이도 효율적인 작업 수행이 가능해졌다.



### Fast Adaptation with Kernel and Gradient based Meta Leaning (https://arxiv.org/abs/2411.00404)
Comments:
          12 pages(with reference), 2 figures, 4 tables

- **What's New**: 이 논문은 Model Agnostic Meta Learning (MAML)의 inner loop와 outer loop를 개선하기 위한 두 가지 알고리즘을 제안합니다. 특히, 함수 공간에서 최적화 문제를 재정의하고, 메타-학습자의 손실 가중치를 동적으로 조정하는 방법을 통해 교육 및 추론 단계에서의 수렴을 최적화합니다.

- **Technical Details**: 첫 번째 알고리즘은 클로즈드 폼 솔루션(closed-form solutions)을 사용하여 모델을 업데이트하고, 두 번째 알고리즘은 각 작업의 손실에 가중치를 부여하여 메타-학습자의 학습을 조정합니다. 이들은 MAML의 안정성과 계산 비효율성을 개선합니다.

- **Performance Highlights**: 이 연구는 기존 메타-학습 방법들과 비교하여 더욱 효율적인 few-shot learning과 빠른 작업 적응 방식을 제안하며, 메타-학습의 새로운 패러다임을 확립하기 위한 기초를 마련합니다.
...
### Preventing Model Collapse in Deep Canonical Correlation Analysis by Noise Regularization (https://arxiv.org/abs/2411.00383)
Comments:
          Accepted by NeurIPS 2024 as a poster

- **What's New**: 본 논문에서는 Multi-View Representation Learning (MVRL)에서 Deep Canonical Correlation Analysis (DCCA) 기반 방법의 모델 붕괴(model collapse) 문제를 해결하기 위해 NR-DCCA라는 새로운 방법론을 제안합니다.

- **Technical Details**: NR-DCCA는 새로운 노이즈 정규화(noise regularization) 접근 방식을 통해 DCCA 기반 방법의 모델 붕괴를 예방합니다. 이 연구는 Correlation Invariant Property (CIP)가 모델 붕괴를 방지하는 핵심 요소임을 이론적으로 분석하였으며, 신경망이 이러한 속성을 가지도록 강요하는 노이즈 정규화 기법을 개발하였습니다.

- **Performance Highlights**: 개발된 NR-DCCA는 합성 데이터(synthetic datasets) 및 실제 데이터(real-world datasets) 모두에서 안정적이고 일관되게 기준선(baselines) 방법들을 능가하는 성능을 보였습니다.



### Communication Learning in Multi-Agent Systems from Graph Modeling Perspectiv (https://arxiv.org/abs/2411.00382)
Comments:
          Extension of the corresponding ICLR edition: arXiv:2405.08550

- **What's New**: 본 연구에서는 여러 지능형 에이전트 간의 협업을 향상시키기 위해 새로운 통신 아키텍처인 CommFormer를 제안합니다. 이 방법은 통신 그래프를 학습 가능한 구조로 개념화하고 이를 이중 최적화 프로세스로 해결합니다.

- **Technical Details**: CommFormer는 그래프 표현의 연속 완화와 주의(attention) 유닛을 활용하여 통신 그래프를 효율적으로 최적화합니다. 에이전트의 관찰 정보에 기초하여 동적 결정을 내릴 수 있는 시간 게이팅 메커니즘을 도입하여 의사결정 효율성을 향상시킵니다.

- **Performance Highlights**: 다양한 협력 작업에서 CommFormer는 강력한 기준선 모델들을 일관되게 초과 달성하며, 모든 에이전트 간의 무제한 정보 공유 접근 방식과 비슷한 성능을 가지고 있어 동적 접근 방식의 효율성을 강조합니다.
...
- **What's New**: 이번 논문에서는 중앙 서버 없이 분산 데이터셋을 사용하여 글로벌 모델을 학습하는 탈중앙화 학습(decentralized learning)에서 데이터 배포의 이질성(heterogeneity) 문제를 해결하기 위한 새로운 알고리즘 ROSS를 제안합니다.

- **Technical Details**: ROSS는 Shapley 값(Shapley values)에 기반한 강력한 분산 확률적 학습 알고리즘입니다. 각 에이전트는 이웃의 데이터셋에 대한 지역 모델의 도함수(gradient)를 수집하고, 이를 통해 자신의 모델을 업데이트합니다. 이 업데이트 과정에서 Shapley 값에 의해 측정된 기여도에 따라 도함수를 가중치를 부여하여 지속적인(converging) 방식으로 모델을 개선합니다.

- **Performance Highlights**: 광범위한 실험 결과에 따르면, ROSS 알고리즘은 다양한 데이터 도전 과제에 직면했을 때 기존의 최첨단 알고리즘보다 수렴(convergence) 속도와 예측 정확도(prediction accuracy) 면에서 뚜렷한 장점을 보여주었습니다.



### Hierarchical Preference Optimization: Learning to achieve goals via feasible subgoals prediction (https://arxiv.org/abs/2411.00361)
- **What's New**: 이번 연구에서는 Hierarchical Preference Optimization (HPO)라는 새로운 접근 방식을 소개합니다. 이는 복잡한 로봇 제어 작업에서 비정상성과 시행 불가능한 하위 목표 생성 문제를 해결하기 위한 계층적 강화 학습(HRL) 방법입니다.

- **Technical Details**: HPO는 최대 엔트로피 강화 학습(maximum entropy reinforcement learning)과 토큰 수준의 Direct Preference Optimization (DPO)을 결합하여 새로운 정책을 학습합니다. HPO는 HRL을 이중 최적화 문제(bi-level optimization problem)로 정식화하고, 기본적인 정규화된 DPO 형식(primitive-regularized DPO formulation)으로 변환하여 유효한 하위 목표 생성이 가능하도록 합니다.

- **Performance Highlights**: HPO는 도전적인 로봇 내비게이션 및 조작 작업에서 기존 방법에 비해 최대 35%의 성능 향상을 보여줍니다. 실험 결과는 HPO가 HRL에서 비정상성과 시행 불가능한 하위 목표 생성을 완화할 수 있는 능력을 입증합니다.
...
- **What's New**: 이번 논문은 사전 훈련된 확산 모델을 이용하여 노이즈가 포함된 선형 역문제를 해결하기 위한 효율적인 알고리즘을 제안합니다. 특히, 기존의 denoising diffusion implicit models (DDIM)을 확장한 constrained diffusion implicit models (CDIM)을 설명하며, 궁극적인 출력에 제약을 가하도록 확산 업데이트를 수정합니다.

- **Technical Details**: 우리의 연구는 CDIM을 통해 노이즈가 없는 경우에 정확한 관측값 복원을 달성하며, 노이즈가 있는 경우에는 Kullback-Leibler (KL) 발산을 최적화하여 잔여 노이즈 분포의 정확한 제약을 확보합니다. 이 접근법은 Gaussian 노이즈를 넘어서는 일반적인 노이즈 모델에 대해 효과적으로 작동합니다.

- **Performance Highlights**: CDIM은 기존의 조건부 확산 방법보다 10배에서 50배 더 빠른 추론 속도를 제공하며, 슈퍼 해상도, 노이즈 제거, 회복(inpainting), 블러 제거, 3D 포인트 클라우드 재구성과 같은 다양한 문제에 대해 뛰어난 성능을 시연했습니다.
...
- **What's New**: 이번 연구는 Kolmogorov-Arnold Networks (KAN)의 한계를 극복하기 위해 Fourier series를 활용한 KAN-AD(Time Series Anomaly Detection) 방법을 제안합니다. 이 방법은 전세계적인 패턴을 강조하여 로컬 이상(이상치)의 영향을 줄이는 데 중점을 두었습니다.

- **Technical Details**: KAN-AD는 기존의 KAN이 spline 함수를 사용하여 단일 변수 함수(univariate functions)를 최적화하는 방식에서 벗어나, Fourier series를 활용하여 여러 복잡한 시계열 패턴을 처리합니다. 이로 인해 학습 과정이 더 제어 가능해지고, 로컬 이상에 대한 내성이 향상됩니다.

- **Performance Highlights**: 실험 결과, KAN-AD는 기존 최첨단 방법 대비 15% 높은 정확도를 달성했으며, 추론 속도는 55배 향상되었습니다.



### Efficient Model Compression for Bayesian Neural Networks (https://arxiv.org/abs/2411.00273)
- **What's New**: 본 논문에서는 Bayesian 모델 선택 원칙을 심층 학습(Deep Learning) 설정에서 모방하는 새로운 전략을 제안합니다. 체계적으로 파라미터를 제거하면서도 정확도를 유지하는 방법인 파라미터 프루닝(parameter pruning)을 Bayesian 접근법에 통합하여 더 나은 일반화 성능을 달성했습니다.

- **Technical Details**: 우리는 완전 연결된 Bayesian 신경망(Bayesian Neural Network, BNN)에서 spike-and-slab priors를 사용하여 각 노드의 posterior inclusion probability를 얻습니다. 이러한 확률을 사용하여 여러 시뮬레이션 데이터와 실제 데이터에 대해 특징 선택(feature selection) 및 프루닝을 수행합니다. Bayesian 접근은 불확실성 추정(uncertainty estimation)뿐만 아니라 회귀 계수 및 복잡성 파라미터의 동시 추정을 가능하게 합니다.

- **Performance Highlights**: 우리는 모든 실험에서 프루닝된 모델이 더 나은 일반화 성능을 보이는 것을 발견했습니다. 이는 과도한 파라미터의 문제를 해결하고, 적은 메모리와 컴퓨팅 자원으로도 안정적인 예측을 가능하게 합니다.



### Unsupervised Feature Selection Algorithm Based on Graph Filtering and Self-representation (https://arxiv.org/abs/2411.00270)
Comments:
          in Chinese language

- **What's New**: 본 논문에서는 기존 방법들이 데이터의 고차원 이웃 정보(higher-order neighborhood information)를 충분히 반영하지 못하는 문제를 해결하기 위해, 그래프 필터링(graph filtering) 및 자기 표현(self-representation) 기반의 비지도(feature selection) 방법을 제안합니다.

- **Technical Details**: 제안된 알고리즘에서는 먼저 데이터에 고차원 그래프 필터(higher-order graph filter)를 적용하여 부드러운 표현(smooth representation)을 얻습니다. 이어서, 고차원 그래프 정보를 통합하는 정규화기(regularizer)를 설계하여 자기 표현 행렬(self-representation matrix)을 학습합니다. 이후 l2,1 노름을 사용하여 오류 항(error term)과 특징 선택 행렬(feature selection matrix)을 재구성하여 모델의 강인성(robustness) 및 행 희소성(row sparsity)을 향상시킵니다. 마지막으로, 반복(iterative) 알고리즘을 적용하여 제안된 목표 함수를 효과적으로 해결합니다.

- **Performance Highlights**: 시뮬레이션 실험을 통해 제안된 알고리즘의 유효성을 검증하였으며, 특히 판별(feature discrimination) 기능 선택에서 이전 방법들보다 더 뛰어난 성능을 보였습니다.



### Clustering ensemble algorithm with high-order consistency learning (https://arxiv.org/abs/2411.00268)
Comments:
          in Chinese language

- **What's New**: 이 연구는 Clustering Ensemble에서의 고차 일관성 학습(HCLCE) 알고리즘을 제안하여, 기본 클러스터의 품질 차이에 의한 문제를 해결하고자 합니다.

- **Technical Details**: HCLCE 알고리즘은 다양한 차원에서 데이터 간의 연결을 표현하기 위한 고차 정보 융합 알고리즘입니다. 먼저, 각 고차 정보를 새롭게 구조화된 일관성 행렬(consistency matrix)로 융합하고, 이후 여러 일관성 행렬을 통합하여 최종적으로 일관된 결과를 얻습니다.

- **Performance Highlights**: 실험 결과, HCLCE 알고리즘은 Locally Weighted Evidence Accumulation (LWEA) 알고리즘과 비교했을 때, 클러스터링 정확도(clustering accuracy)가 평균 7.22% 향상되었으며, Normalized Mutual Information (NMI)는 평균 9.19% 개선되었습니다.
...
- **What's New**: 본 논문은 NeurIPS Datasets and Benchmarks 트랙에 발표된 데이터셋을 체계적으로 검토하며, 데이터셋 관리의 현재 상황과 문제점을 파악했습니다.

- **Technical Details**: 검토는 데이터셋 출처(provenance), 분포(distribution), 윤리적 공개(ethical disclosure), 라이센스(licensing)의 네 가지 주요 측면에 중점을 두었습니다. 연구 결과, 데이터셋 출처가 불확실한 경우가 많고, 데이터셋 호스팅을 위한 다양한 사이트가 존재하나 구조화된 메타데이터(metadata)와 버전 관리(version control)를 제공하는 곳은 극히 적다는 점을 밝혔습니다.

- **Performance Highlights**: 데이터셋 관리의 일관성 부족이 기본적인 긴장(tension)과 윤리적 문제를 진단하고 해결하는 데 큰 장애가 되고 있음을 보여주며, 표준화된 데이터 인프라의 필요성을 강조합니다.
...
### Space for Improvement: Navigating the Design Space for Federated Learning in Satellite Constellations (https://arxiv.org/abs/2411.00263)
- **What's New**: 이 논문에서는 우주에서의 연합 학습(federated learning, FL) 알고리즘의 기존 방법들을 새로운 방식으로 발전시키고, 이를 통해 위성 집합체(constellation)의 특성에 맞춘 FL 구현을 제안하고 있습니다. 특히, AutoFLSat라는 새로운 FL 알고리즘을 소개하여 모델 학습 시간을 12.5%에서 37.5%까지 단축시켰습니다.

- **Technical Details**: FLySTacK 플랫폼은 위성 궤도의 테스팅과 통신 프로토콜을 위한 시뮬레이션을 제공하며, 사전 개발된 FL 알고리즘을 우주 환경에 적합하도록 변형하는 방법을 제시합니다. AutoFLSat는 계층적 FL의 원칙에 기반하여 완전 자율적인 FL 훈련 프레임워크로, 위성 하드웨어의 제약을 인식하며 최적의 성능을 제공합니다.

- **Performance Highlights**: FLySTacK을 사용하여 위성 배치 크기, 클러스터 수와 같은 다양한 구성 조건에서의 알고리즘 성능을 철저히 평가하였으며, 새로운 FL 알고리즘의 성능은 선도적인 기존 방법 대비 12.5%에서 37.5%의 훈련 시간 절감을 보여주었습니다.



### Enhancing Diversity in Bayesian Deep Learning via Hyperspherical Energy Minimization of CKA (https://arxiv.org/abs/2411.00259)
Comments:
          NeurIPS 2024

- **What's New**: 이 논문에서는 Bayesian deep learning에서 네트워크를 비교하기 위한 새로운 유사성 메트릭으로 Centered Kernel Alignment (CKA)를 도입하고, 이를 최적화 목표로 사용하여 다양성을 갖춘 앙상블 및 하이퍼네트워크를 생성하는 방법을 제안합니다.

- **Technical Details**: CKA는 네트워크의 피처 커널(feature kernels) 간의 유사성을 측정하는 방법으로, 하이퍼스피어(hypersphere) 위에 커널을 투사합니다. 본 논문에서는 CKA를 기반으로 한 손실 항을 추가하고, Hyperspherical Energy (HE) 최소화를 통해 앙상블 신경망을 더 고르게 분포시키는 방법을 발전시킵니다. 또한, CKA 기반의 피처 커널을 활용하여 인위적으로 생성된 아웃라이어(outlier) 예제에 대해 피처 반발(term) 항을 도출합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 다양한 앙상블 및 하이퍼네트워크에서 불확실성 정량화(uncertainty quantification)에서 기존 방법들보다显著하게 성능을 향상시키며, MNIST, CIFAR, TinyImageNet와 같은 데이터셋에서 전반적인 예측 정확도를 유지하고 있습니다.
...
- **What's New**: 본 논문에서는 Li-ion 배터리의 상태(State of Health, SOH)를 예측하기 위해 SambaMixer라는 새로운 구조의 상태 공간 모델(Structured State Space Model, SSM)을 제안합니다. 이 모델은 다변량 시간 신호를 처리하기 위해 설계된 MambaMixer 아키텍처에 기반하고 있습니다.

- **Technical Details**: SambaMixer 모델은 시간 신호의 일정한 길이를 보장하는 새로운 앵커 기반 재샘플링 방법을 도입하고, 예측을 샘플 시간 및 사이클 시간 차이에 조건화하여 성능을 개선하기 위해 위치 인코딩(Positional Encoding)을 적용합니다. 이 모델은 NASA의 배터리 방전 데이터셋을 사용한 평가에서 기존의 최신 기술보다 우수한 성능을 보입니다.

- **Performance Highlights**: 우리의 모델은 Li-ion 배터리의 SOH를 높은 정확성과 강 robustness으로 예측할 수 있음을 입증하였습니다.
...
### MEDS-Tab: Automated tabularization and baseline methods for MEDS datasets (https://arxiv.org/abs/2411.00200)
- **What's New**: 이 연구에서는 MEDS(의료 사건 데이터 표준) 프레임워크의 발전을 통해 의료 기록 데이터의 표준화를 개선하고, 다양한 슈퍼바이즈드 러닝 작업을 위한 고품질의 기초 모델을 자동으로 생성하는 시스템인 MEDS-Tab을 소개합니다. 이는 연구자들이 대규모의 EHR 데이터를 자동으로 특징화하고 정형화할 수 있게 해줍니다.

- **Technical Details**: MEDS-Tab은 비정형 시계열 데이터를 정형 데이터로 전환하고, 여러 수천 개의 개별 특징 및 수억 개의 임상 이벤트를 처리하는 동시에, XGBoost를 사용하여 고급 기초 모델을 생성합니다. 이 시스템은 고유한 데이터 파이프라인을 필요로 하지 않으며, MEDS 표준을 통해 데이터의 구조를 일관되게 유지합니다.

- **Performance Highlights**: MEDS-Tab을 사용함으로써 연구자들은 다양한 EHR 데이터세트에서 신뢰할 수 있는 기초 예측 결과를 빠르게 생성할 수 있으며, 이는 데이터 전처리 및 모델 교육에 소요되는 시간을 크게 줄여줍니다. 이 시스템은 대규모 데이터셋에서도 뛰어난 성능을 발휘하며, 연구자들이 쉽게 재현 가능한 방식으로 기초 모델을 제작할 수 있도록 돕습니다.



### Kernel Operator-Theoretic Bayesian Filter for Nonlinear Dynamical Systems (https://arxiv.org/abs/2411.00198)
- **What's New**: 최근 Koopman operator 이론에 대한 관심이 높아지는 가운데, 본 논문에서는 데이터 기반 비선형 동적 시스템을 모델링하기 위한 기능적 베이esian 관점을 바탕으로 한 머신러닝 대안을 제안합니다. 이 접근은 일반적으로 선형 연산자로 구성된 무한 차원 공간에서 직접 수행됩니다.

- **Technical Details**: 비선형 동적 시스템을 무한 차원 관찰 가능한 공간으로 변환하여 시간에 따른 변화를 선형적으로 분석할 수 있게 하는 Koopman operator의 원리를 활용합니다. 여기서 reproducing kernel Hilbert space (RKHS) 이론을 통해 비선형 동역학을 선형 임베딩을 통해 처리할 수 있는 가능성을 제공합니다. 이론적으로 Gaussian 커널은 연속적인 목표 함수의 근사성을 보장합니다.

- **Performance Highlights**: Gaussian 커널의 빠른 감쇠 특성 덕분에, 논문에서 제안하는 기능적 베이esian 필터(FBF)를 통해 작은 차원에서도 뛰어난 근사 결과를 도출할 수 있음을 보입니다. 또한, 제안된 방식은 기존의 유한 차원 Koopman 분해 알고리즘보다 우수한 성능을 보여줍니다.
...
### APEBench: A Benchmark for Autoregressive Neural Emulators of PDEs (https://arxiv.org/abs/2411.00180)
Comments:
          Accepted at Neurips 2024. The code is available at this https URL and APEBench can be installed via "pip install apebench"

- **What's New**: APEBench(Autoregressive PDE Emulator Benchmark)는 부분 미분 방정식(PDE)을 해결하기 위한 자가 회귀 신경 에뮬레이터를 평가하기 위한 포괄적인 벤치마크 모음을 소개합니다. 이 벤치마크는 JAX를 기반으로 하며, 효율적인 pseudo-spectral 방법을 사용하여 1D, 2D, 3D에서 46개의 다양한 PDE를 지원하는 차별화된 시뮬레이션 프레임워크를 제공합니다.

- **Technical Details**: APEBench는 학습된 에뮬레이터의 체계적인 분석과 비교를 용이하게 하고, 고유한 PDE 동역학 식별자를 도입하여 전통적인 수치 방법의 안정성 기준과 직접 연결됩니다. 통합된 pseudo-spectral solver는 데이터 생성과 훈련 중 네트워크가 동적으로 상호작용할 수 있는 차별화된 솔버 역할을 수행합니다. APEBench는 다양한 신경 아키텍처를 평가할 수 있으며, 시간적 일반화를 이해하기 위한 rollout metrics를 강조합니다.

- **Performance Highlights**: 여러 실험에서 신경 에뮬레이터와 수치 시뮬레이터 간의 유사성을 강조하며, 전통적인 수치 기법보다 더 나은 성능을 보여줄 수 있도록 설계되었습니다. 이 벤치마크는 새로운 아키텍처와 훈련 방법론에 쉽고 빠르게 적응할 수 있는 레시피를 포함하고 있으며, 신속한 데이터 재생성이 가능하여 고정 데이터셋의 배포 필요성을 없애고 물리학의 기본 수정이 가능하게 합니다.



### What Makes An Expert? Reviewing How ML Researchers Define "Expert" (https://arxiv.org/abs/2411.00179)
- **What's New**: 이번 연구에서는 112개의 학술 출판물을 분석하여 '전문가'(expert)와 '전문성'(expertise)의 정의가 명확하지 않음을 강조하며, 머신러닝(ML) 시스템 개발에서 전문가의 역할을 탐구하고 있습니다. 연구 결과, 공식 교육이나 전문 인증 외의 지식 형태가 거의 찾혀지지 않는 문제를 지적하고 있습니다.

- **Technical Details**: ML 연구는 일반적으로 전문성을 정의하지 않으며, 데이터 주석(data annotation)과 같은 명시적인 지식 활용에 편중되어 있습니다. 연구는 ML 개발에 참여하는 전문가의 소외 및 기술적 능력 저하(deskilling) 문제를 다루고, 전문 지식이 고전적이고 규범적 사유에 의해 제한된다는 점을 언급합니다.

- **Performance Highlights**: 연구는 머신러닝 시스템 개발에서 더 넓은 범위의 전문성을 인정하고, 균형 잡힌 참여를 통해 책임 있는 AI 개발을 위한 사회적 관계를 고려할 필요가 있음을 강조합니다.



### EARL-BO: Reinforcement Learning for Multi-Step Lookahead, High-Dimensional Bayesian Optimization (https://arxiv.org/abs/2411.00171)
Comments:
          14 pages

- **What's New**: 이 논문은 고차원 블랙박스 최적화 문제에서 다단계 전망 다루는 새로운 강화 학습 기반의 베이지안 최적화 프레임워크인 EARL-BO(Encoder Augmented RL for Bayesian Optimization)를 제안합니다. 이를 통해 스케일링(Scalability)과 의사결정 품질을 향상시키는 데 중점을 두었습니다.

- **Technical Details**: 제안된 EARL-BO 프레임워크는 Attention-DeepSets 인코더를 사용하여 강화 학습(RL) 에이전트가 지식을 상태로 표현하고, off-policy 학습을 통해 초기 훈련을 가속화합니다. 또한, 인코더-RL 조합을 활용하여 다중 작업 및 미세 조정(end-to-end learning) 절차를 제안하며, GP(가우시안 프로세스) 기반의 가상 환경에서 RL 알고리즘이 전체 프로세스를 최적에 가깝게 해결할 수 있도록 설계되었습니다.

- **Performance Highlights**: 이 방식은 기존의 다단계 전망 및 고차원 최적화 방법들에 비해 합성 벤치마크 함수 및 실제 하이퍼파라미터 최적화 문제에서 성능 향상을 입증했습니다.
...
### Mutual Information Preserving Neural Network Pruning (https://arxiv.org/abs/2411.00147)
- **What's New**: 새로운 연구에서는 Mutual Information Preserving Pruning (MIPP) 기법을 도입하여 신경망(Neural Network) 및 합성곱 신경망(Convolutional Neural Network)에서 노드 간의 상호 정보(Mutual Information)를 유지하면서 효과적으로 가지치기를 수행한다.

- **Technical Details**: MIPP는 인접 레이어의 활성화 간의 상호 정보를 유지하는 기법으로, 기존의 top-k 방법 대신 transfer entropy redundancy criterion (TERC)을 사용하여 노드를 동적으로 가지치기한다. 이 방법은 실시간으로 노드 간 상호작용을 고려하며, 각 레이어마다 가지치기 비율(Pruning Ratios)을 설정하여 레이어 붕괴를 방지한다.

- **Performance Highlights**: 포괄적 실험 평가를 통해 MIPP는 사전 훈련된 ResNet50을 포함한 다양한 비전 모델에서 최신 가지치기 기법보다 우수한 성능을 보여주었다.



### Learning local discrete features in explainable-by-design convolutional neural networks (https://arxiv.org/abs/2411.00139)
- **What's New**: 본 연구에서는 성능과 설명 가능성 간의 trade-off를 극복하기 위한 설명 가능한 설계(Explainable-by-design) 컨볼루션 신경망(CNN) 프레임워크를 소개합니다. ExplaiNet 모델은 고정밀 CNN과 공간 상호작용을 표현하는 explainer 확률 그래프를 포함하고 있습니다.

- **Technical Details**: ExplaiNet은 측면 억제(lateral inhibition) 메커니즘에 기반한 CNN 모델로, 로컬 불연속 특징(Local Discrete Feature, LDF) 벡터를 사용하여 뉴런의 활성화 강도에 따라 적대 뉴런의 색인을 순서대로 저장합니다. EM 기반의 시퀀스 동기 모티프 발견 방법인 EXTREME을 통해 설명의 간결함을 증가시키고, 중간 이미지 표현에 대한 불연속 특징 모티프 행렬을 제공합니다.

- **Performance Highlights**: 다양한 작은 이미지 벤치마크 데이터셋에서 실험한 결과, 제안된 예측기가 기존 아키텍처와 동일한 수준의 성능을 보이며, MNIST 분류 작업에서 단일 모델 기준으로 최신 성능과 비교 가능한 결과를 달성했습니다.



### LLM-Inference-Bench: Inference Benchmarking of Large Language Models on AI Accelerators (https://arxiv.org/abs/2411.00136)
- **What's New**: 이번 논문에서는 LLM-Inference-Bench라는 포괄적인 벤치마킹 툴킷을 소개하여 LLM의 하드웨어 추론 성능을 평가합니다. 이 연구는 Nvidia 및 AMD GPU는 물론 Intel Habana와 SambaNova 같은 전문 AI 가속기를 포함한 다양한 하드웨어 플랫폼에서 검토되었습니다.

- **Technical Details**: LLM-Inference-Bench는 LLaMA, Mistral, Qwen와 같은 다양한 LLM 구조를 사용하여 7B 및 70B 파라미터 모델의 성능을 분석합니다. 이 연구에서는 vLLM, TensorRT-LLM, llama.cpp 및 Deepspeed-MII와 같은 다양한 추론 프레임워크의 성능을 조사합니다. 주요 성능 메트릭으로는 throughput과 전력 소비가 포함됩니다.

- **Performance Highlights**: 벤치마킹 결과는 다양한 모델, 하드웨어 플랫폼 및 추론 프레임워크의 강점과 한계를 보여줍니다. 또한, 최적의 성능을 위한 환경 구성을 파악할 수 있도록 인터랙티브 대시보드를 제공합니다.
...
### Derivative-Free Optimization via Finite Difference Approximation: An Experimental Study (https://arxiv.org/abs/2411.00112)
- **What's New**: 이 논문은 Derivative-free optimization (DFO)에서 batch 기반 FD 추정기를 이용한 경량한 최적화 접근법의 효과를 보여줍니다.

- **Technical Details**: DFO는 노이즈가 섞인 함수 평가만을 사용할 수 있는 복잡한 최적화 문제에 중요합니다. 기존 Kiefer-Wolfowitz (KW)와 Simultaneous Perturbation Stochastic Approximation (SPSA) 알고리즘은 각각 두 샘플을 사용하여 경량한 최적화를 하지만, 정확도가 떨어지는 경향이 있습니다. 반면, Correlation-Induced FD (Cor-FD) 방법을 통해 보다 정확한 기울기 추정이 가능합니다.

- **Performance Highlights**: 실험 결과, batch 기반 FD 추정기를 사용하는 Cor-CFD가 KW 및 SPSA 알고리즘보다 일반적으로 더 나은 성능을 보이는 것으로 나타났습니다. 특히, 고차원 문제에서 빠른 수렴 속도를 보여주었습니다.



### Lagrangian neural networks for nonholonomic mechanics (https://arxiv.org/abs/2411.00110)
- **What's New**: 이번 논문에서는 Lagrangian Neural Networks (LNN)를 비홀로노믹(nonholonomic) 제약을 가진 기계 시스템에 적용하는 새로운 접근 방식을 제시합니다. 이를 통해 해당 제약조건을 학습에 포함시킴으로써 궤적(trajectory) 추정의 정확도 향상과 에너지 보존을 증진시키는 성과를 보였습니다.

- **Technical Details**: LNN은 기계 시스템의 Lagrangian을 매개변수화하여 에너지를 거의 보존하는 궤적을 예측하는 강력한 도구입니다. 이 연구에서는 비홀로노믹 제약을 가진 기계 시스템에 LNN 기술을 적응시켜 학습 중 제약 조건을 고려했으며, 이는 Euler-Lagrange 방정식의 벡터화된 형태를 채택하여 시스템의 동역학을 추출하는 방식을 포함합니다.

- **Performance Highlights**: LNN의 적용은 비홀로노믹 제약이 있는 기계 시스템에서 궤적 추정의 정확성을 개선하고, 제약 조건 준수 및 에너지 행동을 향상시켰습니다. 이 방식은 기존의 비제약(unconstrained) 시스템에 비해 더 나은 성능을 보여주었습니다.



### Label Noise: Ignorance Is Bliss (https://arxiv.org/abs/2411.00079)
- **What's New**: 본 논문은 다중 클래스 인스턴스 의존성 레이블 노이즈(multi-class, instance-dependent label noise) 하에서 학습을 위한 새로운 이론적 프레임워크를 제시합니다. 이 프레임워크는 레이블 노이즈를 도메인 적응(domain adaptation) 문제로 변환하며, 특히 사후 변화(posterior drift) 하의 도메인 적응 문제로 설정합니다. 사용된 새로운 개념인 상대 신호 강도(relative signal strength, RSS)는 노이즈에서 클린(posterior)으로의 전이 가능성을 정량화하는 척도를 제공합니다.

- **Technical Details**: 상대 신호 강도(RSS)를 사용하여 노이즈 문제의 난이도를 특성화하고, 초과 위험(excess risk)의 거의 일치하는 상한 및 하한 경계를 Establish합니다. 또한, 샘플 크기(n)가 증가함에 따라 상한과 하한이 0에 수렴하도록 보장하는 분포 가정을 식별합니다. 놀랍게도 분할 제거 경험적 위험 최소화(Noise Ignorant Empirical Risk Minimization, NI-ERM) 원칙은 (거의) minimax 최적입니다.

- **Performance Highlights**: NI-ERM을 사용하여 self-supervised 특징 추출기 위에 선형 분류기를 적합시킴으로써 CIFAR-N 데이터 챌린지에서 최첨단 성능을 달성했습니다.



### $\boldsymbol{\mu}\mathbf{P^2}$: Effective Sharpness Aware Minimization Requires Layerwise Perturbation Scaling (https://arxiv.org/abs/2411.00075)
- **What's New**: Sharpness Aware Minimization (SAM)의 새로운 스케일링 동작을 분석하고, 신경망의 무한 너비 한계에서 SAM의 효과적인 적용 방식을 제시했습니다. 특히, 모든 레이어에서 특성 학습과 유효한 섭동을 달성하는 새로운 매개변수화를 제안합니다.

- **Technical Details**: SAM의 무한 너비 한계에 대해 Tensor Programs 프레임워크를 사용하여 연구했습니다. 실험 결과, Maximal Update and Perturbation Parameterization ($\mu$P$^2$)이 hyperparameter transfer를 통해 모든 레이어에서 효과적인 섭동을 가능하게 함을 보여주었습니다.

- **Performance Highlights**: $	extit{Maximal Update and Perturbation Parameterization} ($	extit{	extbf{μ}}P^2$)$은 MLP, ResNets, Vision Transformers를 포함한 다양한 모델에서 실험적으로 검증되었으며, 다양한 모델 스케일에 걸쳐 학습률과 섭동 반경의 최적 지점을 성공적으로 전이하는 첫 번째 매개변수화로 확인되었습니다.
...
### Modern, Efficient, and Differentiable Transport Equation Models using JAX: Applications to Population Balance Equations (https://arxiv.org/abs/2411.00742)
- **What's New**: 이 논문은 Population Balance Equation (PBE) 모델이 약물 개발 프로세스를 자동화하고 단축시킬 수 있는 가능성을 다루고 있습니다. 특히, 실험적인 추측 작업을 없애기 위해 Hybrid 모델을 활용하여 신경망과 물리적 PBE 모델을 통합한다고 설명합니다.

- **Technical Details**: 논문에서는 JAX라는 최신 라이브러리를 사용하여 효율적이고 미분 가능한 PBE 알고리즘의 구현을 소개합니다. 이를 통해 기존에 널리 사용되던 소프트웨어에 비해 PBE 시뮬레이션에서 최대 300배의 속도 향상을 보여줍니다. 또한, 이는 대규모 학습 가능한 데이터 기반 모델 통합을 가능하게 하며, 기존 접근법보다 40배 더 빠른 최적화를 제공합니다.

- **Performance Highlights**: 이 연구의 하이라이트는 JAX 기반의 PBE 솔버가 빠르고 효율적이라는 것입니다. 기존의 한계와 병목 현상을 제거하여 SciML(Scientific Machine Learning) 통합의 기반을 제공하며, PBE 모델링의 전통적인 문제 해결을 향상시킵니다.
...
### Toward Automated Algorithm Design: A Survey and Practical Guide to Meta-Black-Box-Optimization (https://arxiv.org/abs/2411.00625)
- **What's New**: 이번 연구에서는 Meta-Black-Box-Optimization (MetaBBO)가 진화적 계산 (Evolutionary Computation, EC) 커뮤니티 내에서 자동화된 알고리즘 설계를 지원하는 새로운 접근 방법으로 소개됩니다. 이 논문은 MetaBBO의 최근 발전을 종합적으로 리뷰하며 체계적인 기초를 제공합니다.

- **Technical Details**: MetaBBO는 메타 학습 (Meta-learning) 접근법을 통해 Black-Box Optimization (BBO) 알고리즘의 최적화 성능을 향상시키고, 알고리즘 선택, 알고리즘 구성, 솔루션 조작, 알고리즘 생성 등 다양한 알고리즘 설계 과제를 포괄하는 체계적 분류를 제공합니다. 강화 학습 (Reinforcement Learning), 지도 학습 (Supervised Learning), 신경 진화 (Neuroevolution) 등 여러 학습 방법론이 소개됩니다.

- **Performance Highlights**: MetaBBO 방법에 대한 종합적인 성능 평가가 수행되며, 이를 통해 일반화 및 학습 효율성을 높이는 핵심 설계 요소가 도출됩니다. 최종적으로 MetaBBO 분야의 미래 방향성에 대한 비전이 제시됩니다.
...
### Nonparametric estimation of Hawkes processes with RKHSs (https://arxiv.org/abs/2411.00621)
- **What's New**: 이 논문은 비모수적(nonparametric) 추정 방법을 통해 비선형 다변량 하우크스 프로세스(nonlinear multivariate Hawkes processes)를 다룹니다. 상호작용 함수는 재생 커널 힐베르트 공간(RKHS)에 속하는 것으로 가정되며, 이는 복잡한 상호작용 효과를 표현하는 데 유용합니다. 특히 신경 과학(neuroscience)에서의 응용에 중점을 두고 있으며, 신경 세포의 리프랙토리(period) 및 흥분 및 억제 효과를 모델링하는데 관심을 가집니다.

- **Technical Details**: 논문은 ReLU 함수에 의해 정류된 조건부 강도를 고려하고, 대표 정리에 대한 결과를 제시합니다. 또한, ReLU 함수와 적분 연산자(integral operator)에 대한 두 가지 간단한 근사를 활용한 추정 방법을 제안합니다. 이를 통해 근사 경계(approximation bound)를 제공하며, 이러한 근사가 통계적으로 무시할 수 있는 영향을 미친다는 것을 정당화합니다.

- **Performance Highlights**: 수치 실험 결과는 제안한 추정 방법이 기존의 비모수적 추정 기법에 비해 우수한 성능을 보이는 것과 더불어, 신경 세포 응용에 적합하다는 것을 확인했습니다.



### Apriori_Goal algorithm for constructing association rules for a database with a given classification (https://arxiv.org/abs/2411.00615)
- **What's New**: 이 연구는 Apriori_Goal 알고리즘을 제안하여 주어진 분류에 대한 연관 규칙을 효율적으로 구축하는 방법을 소개합니다. 이 알고리즘은 데이터베이스의 특성과 레코드 인코딩 방법의 특정 사항에 연관된 특징을 가지고 있습니다.

- **Technical Details**: Apriori_Goal 알고리즘은 관계형 데이터베이스에서 다양한 특성을 가진 열을 처리합니다. 이 알고리즘은 레코드를 이진 속성으로 변환하고, 각 레코드를 하나의 정수로 인코딩하여 기본 연산의 효율적 구현을 가능하게 합니다. 이러한 인코딩 방식은 정보를 압축할 뿐만 아니라, 규칙 특성 계산에 필요한 연산을 정수에 대한 논리 연산으로 신속하게 수행할 수 있게 합니다.

- **Performance Highlights**: 제안된 Apriori_Goal 알고리즘은 의료 데이터베이스와 같은 실제 데이터베이스에서 분류 문제를 해결하는 데 효과적입니다. 알고리즘은 연관 규칙을 효율적으로 추출할 수 있으며, 정수 기반의 인코딩 덕분에 계산 집약적인 작업을 거의 즉시 수행할 수 있습니다.



### Small coresets via negative dependence: DPPs, linear statistics, and concentration (https://arxiv.org/abs/2411.00611)
Comments:
          Accepted at NeurIPS 2024 (Spotlight Paper). Authors are listed in alphabetical order

- **What's New**: 이 논문에서는 DPP(Determinantal Point Process)를 기반으로 한 coreset이 독립 샘플링에 기반한 coreset보다 기본적으로 더 작은 집합임을 증명합니다. 이는 DPP를 사용하여 랜덤화된 coreset을 구축하는데 대한 이론적 및 경험적 지원을 제공합니다.

- **Technical Details**: DPP의 핵심적인 특징은 조정 가능한 부정적 의존성을 가진 점의 무작위 구성입니다. 우리는 coreset 손실을 (무작위) coreset의 선형 통계로 이해하며, DPP의 선형 통계에 대한 집중 현상(concentration phenomena) 문제와 연결합니다. 이 과정에서 우리의 결과는 비대칭 kernel을 포함하여 최첨단을 넘어선 효과적인 집중 불평등(effective concentration inequalities)을 제공합니다.

- **Performance Highlights**: DPP를 활용한 coreset 구축 방법은 벡터 값의 목적 함수(objective functions)에 대한 coreset 문제도 다룰 수 있는 장점을 지니고 있어, coreset 문헌에서의 새로운 기여를 보여줍니다.
...
### Enhancing Adaptive Mixed-Criticality Scheduling with Deep Reinforcement Learning (https://arxiv.org/abs/2411.00572)
Comments:
          Version submitted to RTNS 2024, on 17/08/2024 (with some typos fixed)

- **What's New**: 이번 논문에서는 Adaptive Mixed-Criticality (AMC) 스케줄링 알고리즘을 개선하기 위해 심층 강화 학습(deep reinforcement learning) 접근 방식을 제안하고 있습니다. 기존 AMC는 중요도가 낮은 작업의 예산을 초과할 경우, 낮은 중요도의 작업을 삭제하는 단점이 있습니다. 새로운 방법론은 Deep-Q Network를 기반으로 하여, 작업의 예산을 조정하여 예산 초과를 방지합니다.

- **Technical Details**: 제안된 방법에서는 DRL 에이전트가 오프라인에서 훈련되어 런타임에 작업의 낮은 중요도 예산을 조정합니다. 변경된 예산으로 인해 스케줄링 유효성을 떨어뜨리지 않으며, AMC-rtb의 조건을 충족합니다. 에이전트는 시스템의 낮은 우선 순위로 실행되어 HI 모드로 전환된 후에도 예산을 조정할 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 DQN-agent는 LO-중요도 및 HI-중요도 작업의 예산 초과를 50% 이상 줄일 수 있는 것으로 나타났습니다. 이는 실제 자동차 애플리케이션에서의 작업 부하를 시뮬레이션하여 평가하였습니다.



### Constrained Sampling with Primal-Dual Langevin Monte Carlo (https://arxiv.org/abs/2411.00568)
Comments:
          39 pages, 14 figures. Published at NeurIPS 2024

- **What's New**: 본 논문은 정규화 상수(normalization constant)까지 알려진 확률 분포에서 일반 비선형 함수의 기댓값(expected values)을 제약조건으로 하여 샘플링하는 문제를 고려합니다. Bayes 추론(Bayesian inference)에서 반사실적 시나리오(counterfactual scenarios) 평가나 예측 공정성(prediction fairness) 보장을 위해 필요합니다.

- **Technical Details**: 표준 가정인 (강) 볼록성(strong convexity) 및 로그-소벨 불평등(log-Sobolev inequalities)을 바탕으로, 본 연구는 Wasserstein 공간에서의 경량 기법을 사용하여 부모-쌍대 Langevin Monte Carlo 알고리즘(PD-LMC)을 제안합니다. 이 알고리즘은 목표 분포를 제약하고 동시에 샘플링을 수행합니다. PD-LMC의 수렴(convergence)을 분석하며, 제약 조건의 함수에 대해 더 광범위한 클래스의 보장을 제공합니다.

- **Performance Highlights**: 실험을 통해 PD-LMC의 유효성을 보여주며, Kullback-Leibler divergence와 Wasserstein 거리를 기준으로 한 하위 선형(sublinear) 수렴 속도를 입증합니다. 이를 통해 샘플링 생성에서 성능 개선이 이루어짐을 강조합니다.



### PatternBoost: Constructions in Mathematics with a Little Help from AI (https://arxiv.org/abs/2411.00566)
Comments:
          32 pages

- **What's New**: PatternBoost라는 새로운 방법론이 소개되었습니다. 이 방법론은 수학에서 흥미로운 구조를 찾기 위해 유연한 접근 방식을 제공합니다. 이 알고리즘은 '로컬' 단계와 '글로벌' 단계를 번갈아 수행하여 최적의 수학적 구조를 생성합니다.

- **Technical Details**: PatternBoost는 고전적인 탐색 알고리즘을 이용하여 초기 많은 구조를 생성한 후, 최상의 구조를 바탕으로 transformer neural network를 훈련시키는 방법입니다. 이 과정에서 transformer는 더 나은 시작점을 제공하여 로컬 탐색의 결과를 향상시키는데 기여합니다.

- **Performance Highlights**: PatternBoost는 여러 오랜 문제에 대한 최선의 해결책을 발견하는데 성공하였으며, 30년 이상 미해결된 가설의 반례를 구축하는 등 놀라운 성과를 보여주었습니다. 다양한 문제에 적용 가능하며, 전반적으로 인상적인 성능을 자랑합니다.
...
### DeepSeq2: Enhanced Sequential Circuit Learning with Disentangled Representations (https://arxiv.org/abs/2411.00530)
- **What's New**: 새로운 연구인 DeepSeq2는 순차적으로 회로를 표현하는 새로운 프레임워크로, 구조, 기능 및 순차적 동작의 세 가지 임베딩 공간으로 회로를 매핑함으로써 고유한 복잡성을 포착합니다.

- **Technical Details**: DeepSeq2는 Directed Acyclic Graph Neural Network (DAG-GNN)를 활용하여 실행 시간을 대폭 줄이고 모델 확장성을 향상시킵니다. 또한, 전환 동작을 효과적으로 포착하는 독특한 감독 메커니즘을 통합합니다.

- **Performance Highlights**: DeepSeq2는 전력 추정 및 신뢰성 분석에서 이전 작업보다 우수한 성능을 보이며, 순차 회로 표현 학습에서 새로운 기준을 설정합니다.
...
### Dirichlet process mixtures of block $g$ priors for model selection and prediction in linear models (https://arxiv.org/abs/2411.00471)
- **What's New**: 이번 연구는 직교 프로세스 혼합 모형(Dirichlet process mixtures)과 블록 $g$ prior의 새로운 조합을 제안하여 선형 모형의 모델 선택(model selection) 및 예측(prediction) 문제를 해결합니다. 이 방법은 데이터를 기반으로 한 파라미터 블록에 대한 차별적 수축(differential shrinkage)을 가능하게 하며, 예측 변수 간의 상관 구조를 완전히 반영합니다.

- **Technical Details**: 연구에서는 전통적인 $g$ prior을 확장한 블록 $g$ prior를 직교 프로세스 혼합 형태로 적용합니다. 이 접근법은 모델 선택 및 연속 수축 priors에 대한 문헌을 연결하며, 특히 Som et al.(2016)이 강조한 조건부린들리 패러독스(conditional Lindley paradox)를 피할 수 있습니다. posterior 추정을 위한 Markov chain Monte Carlo (MCMC) 알고리즘을 개발하였으며, 최소한의 튜닝(tuning)만으로도 작동합니다.

- **Performance Highlights**: 작은 수의 큰 효과를 보이는 경우에도 이 방법은 작은 그러나 중요한 효과를 감지하는 데 더 높은 검출력을 제공합니다. 이는 잘못된 발견(false discoveries)의 발생률이 최소한 증가하는 상황에서 이루어집니다. 다양한 실제 데이터 및 시뮬레이션 데이터셋에 대한 성능 평가를 통해 이 방법의 유효성을 입증하였습니다.



### The learned range test method for the inverse inclusion problem (https://arxiv.org/abs/2411.00463)
Comments:
          25 pages, 12 figures

- **What's New**: 이번 연구에서는 Cauchy 데이터로부터 포함체를 재구성하는 역문제를 해결하기 위한 새로운 방법, 즉 LRT (Learned Range Test)를 제안합니다. 이 방법은 전통적인 range test (RT) 방법을 심층 학습(Deep Learning)과 통합하여 더 높은 정확도로 재구성을 수행할 수 있도록 합니다.

- **Technical Details**: 제안된 LRT는 특정 아키텍처를 갖는 신경망(Neural Network)으로 작성될 수 있으며, 감독 학습(Supervised Learning) 방식을 통해 가중치(weights)를 학습합니다. 이는 사전 훈련된(classifier) 분류기와 결합되어 경계로부터의 거리(difference)에 따라 포함체를 구분합니다.

- **Performance Highlights**: 다양한 수치 실험(numerical simulations)을 통해 LRT 방법이 다각형 포함체를 정확하고 안정적으로 재구성하는 것을 확인하였습니다. LRT는 표준 RT 방법이나 완전 연결 심층 신경망(end-to-end fully connected deep neural network)과 비교했을 때 훨씬 우수한 성능을 보입니다.
...
- **What's New**: 본 연구에서는 Lorentz-Equivariant Geometric Algebra Transformer (L-GATr)가 대형 강입자 충돌기(LHC)에서 다양한 머신러닝 작업에서 최첨단 성능을 발휘함을 보여줍니다. L-GATr는 시공간에 대한 기하학적 대수를 사용하여 데이터를 표현하고, Lorentz 변환에 대해 동등성을 유지합니다.

- **Technical Details**: L-GATr의 구조는 다재다능하고 확장 가능한 transformer로, 필요한 경우 대칭성을 깨트릴 수 있습니다. 이 아키텍처는 기하학적 대수(GA)를 기반으로 하며, Lorentz 동등성을 갖춘 새로운 네트워크 레이어로 일반화되었습니다. 네트워크에서는 회귀(regression), 분류(classification), 생성(generative) 네트워크에 적합한 내적 또는 잠재 표현을 제공합니다.

- **Performance Highlights**: L-GATr는 조사된 세 가지 LHC 작업에 대해 이전 아키텍처에 비해 상당한 성능 향상을 보였습니다. 구체적으로, L-GATr는 진폭 회귀(amplitude regression) 및 제트 분류(jet classification)에서 그 능력을 입증하며, 다중 클래스 태깅(multi-class tagging)과 사전 훈련(pre-training)을 통해 성능을 개선하였습니다.
...
### Inference-to-complete: A High-performance and Programmable Data-plane Co-processor for Neural-network-driven Traffic Analysis (https://arxiv.org/abs/2411.00408)
Comments:
          Under review

- **What's New**: Kaleidoscope는 데이터 평면에서 높은 성능을 발휘하면서도 다양한 신경망(Neural Networks, NN) 모델을 지원할 수 있는 유연한 협력 프로세서입니다.

- **Technical Details**: Kaleidoscope는 세 가지 주요 기술을 바탕으로 발전했습니다: (1) 프로그래머블 런-투-컴플리션(Run-to-Completion, RTC) 가속기를 통한 유연한 추론, (2) 엘리펀트 및 마우스 흐름에 맞춤화된 고성능 추론 엔진 개발, (3) 원시 바이트(raw-bytes) 기반 신경망을 통해 데이터 평면에 대한 의존성을 최소화.

- **Performance Highlights**: Kaleidoscope는 6개의 NN 모델에서 256-352 ns의 추론 지연(latency)과 100 Gbps의 처리량(throughput)을 달성하여 유연성과 높은 성능을 보여줍니다. 28nm ASIC 라이브러리를 기반으로 할 때, 1.6 Tbps의 처리량을 기록하였으며, 전체 데이터 평면의 성능에는 미미한 영향을 미쳤습니다.



### HAVER: Instance-Dependent Error Bounds for Maximum Mean Estimation and Applications to Q-Learning (https://arxiv.org/abs/2411.00405)
- **What's New**: 본 논문에서는 K개 분포 중에서 가장 큰 평균을 추정하는 문제를 다룹니다. 새로운 알고리즘 HAVER (Head AVERaging)을 제안하고, 기존 알고리즘들과의 비교를 통해 그 성능을 분석하였습니다.

- **Technical Details**: HAVER 알고리즘은 샘플에서 최대 평균을 추정하는 방법으로, 기존 알고리즘들의 성능 한계를 극복합니다. 본 논문에서 제안된 HAVER는 최적의 분포를 알고 있는 오라클(oracle)의 샘플 평균과 같도록 추정하는 성능을 보여줍니다.

- **Performance Highlights**: HAVER는 특히 많은 분포가 최적의 분포 근처에 있을 때 오라클보다 더 좋은 성능을 보여주는 점이 주목할 만합니다. 실험에서는 bandits와 Q-learning 시나리오에 대해 HAVER가 기초 방법들을 초월하는 성과를 확인하였습니다.



### Towards Building Secure UAV Navigation with FHE-aware Knowledge Distillation (https://arxiv.org/abs/2411.00403)
Comments:
          arXiv admin note: text overlap with arXiv:2404.17225

- **What's New**: 본 연구는 자율 비행체(UAV)의 내비게이션 중 경로의 프라이버시를 보호하기 위한 혁신적인 접근 방식을 제안합니다. Reinforcement Learning (RL)과 Fully Homomorphic Encryption (FHE)의 통합을 통해 보안성을 높이면서도 컴퓨팅 오버헤드를 줄이는 방법을 탐구하고 있습니다.

- **Technical Details**: 우리의 프레임워크는 Knowledge Distillation 기법을 활용하여 FHE의 지연(latency)을 완화하고, 네트워크를 압축함으로써 성능 저하 없이 18배의 속도 향상을 이루었습니다. R-squared 점수는 원래 모델의 0.9631에 비해 0.9499로 나타났습니다. 이는 보안된 UAV 카메라 피드의 실시간 처리를 가능하게 합니다.

- **Performance Highlights**: 이 접근 방식은 비행체 내비게이션 작업에 있어 암호화된 데이터를 처리하는 가능성을 강조하며, 성능 효율성과 적시 처리를 동시에 달성하였습니다. 이를 통해 민감한 환경에서 자율 UAV를 배치할 수 있는 길이 열렸습니다.
...
### Towards Data Valuation via Asymmetric Data Shapley (https://arxiv.org/abs/2411.00388)
- **What's New**: 본 논문은 비대칭 데이터 샤플리(Asymmetric Data Shapley)라는 새로운 프레임워크를 도입하여 데이터 가치 평가의 한계를 극복하고, 다양한 데이터 구조를 반영한 데이터를 평가할 수 있는 방법을 제시합니다.

- **Technical Details**: 이 연구는 전통적인 데이터 샤플리 프레임워크의 대칭 공리를 완화하고, 주어진 데이터셋의 구조를 고려한 weighted systems와 ordered partitions를 도입하여 데이터 점의 기여도를 평가합니다. 비대칭 데이터 샤플리를 위해 몬테 카를로(Monte Carlo) 방법과 KNN 근사 알고리즘을 개발했습니다.

- **Performance Highlights**: 다양한 머신러닝 작업 및 데이터 마켓(데이터 거래소) 문맥에서 비대칭 데이터 샤플리 프레임워크의 유용성을 보여주며, 증강 데이터의 가치, 순차 데이터 평가 및 공정한 보상 분배 문제를 해결하는 데 효과적임을 입증했습니다.



### DeepCore: Simple Fingerprint Construction for Differentiating Homologous and Piracy Models (https://arxiv.org/abs/2411.00380)
Comments:
          9 pages

- **What's New**: DeepCore라는 새로운 방법을 제안하여, 모델의 클래시피케이션 신뢰도가 모델의 결정 경계에서 얼마나 떨어져 있는지와 긍정적으로 상관관계가 있음을 발견하였습니다. 이 방법은 높은 신뢰도의 샘플을 생성하여, 불법 복제 모델을 효과적으로 식별할 수 있습니다.

- **Technical Details**: DeepCore는 높은 신뢰도의 샘플인 core points를 생성하여 모델의 지문을 만들며, 불법 모델과 동등 모델 간의 행동 차이를 활용하여 불법 모델을 식별합니다. 세 가지 모델 식별 방법(L1 distance-based, Cos distance-based, clustering-based)을 설계하였습니다.

- **Performance Highlights**: DeepCore는 다양한 불법 모델 식별에서 우수한 성능을 보이며, 놓친 식별률(Missed Identification Rate, MIR)과 잘못된 식별률(False Identification Rate, FIR)이 0%를 기록하여 최신 기법들을 초월한 성과를 달성하였습니다.



### A Machine Learning Driven Website Platform and Browser Extension for Real-time Scoring and Fraud Detection for Website Legitimacy Verification and Consumer Protection (https://arxiv.org/abs/2411.00368)
Comments:
          Journal of Multidisciplinary Engineering Science and Technology (JMEST) 2024

- **What's New**: 이 논문은 머신 러닝(Machine Learning) 기반의 웹사이트 플랫폼과 브라우저 확장 기능을 소개합니다. 이는 웹사이트 합법성 검증과 소비자 보호를 위한 실시간 위험 점수 및 사기 탐지를 제공하여 온라인 보안을 신속하게 강화합니다.

- **Technical Details**: 플랫폼은 웹사이트 행동, 네트워크 트래픽(network traffic), 사용자 상호작용을 분석하여 잠재적인 위협이 감지될 경우 즉각적인 피드백과 경고를 제공합니다. 사용자 친화적인 브라우저 확장 기능으로 통합되어 사용자들이 안전하게 웹을 탐색할 수 있도록 지원합니다.

- **Performance Highlights**: 이 시스템의 실시간 기능은 전자 상거래(e-commerce) 및 일상적인 브라우징에서 매우 중요하며, 신속하고 실행 가능한 통찰력으로 재정적 손실, 신원 도용(identity theft), 악성 사이트에 대한 노출을 방지합니다. 이 해결책은 온라인 소비자 보호를 위한 실용적이며 신속하게 작동하는 도구로서, 사용자 보호 및 디지털 거래에서의 신뢰 유지를 위한 중요한 역할을 할 수 있습니다.
...
### Unified theory of upper confidence bound policies for bandit problems targeting total reward, maximal reward, and mor (https://arxiv.org/abs/2411.00339)
- **What's New**: 이번 연구는 고전적인 총 보상 밴딧 문제와 최대 보상(max bandit) 문제에서 상한 신뢰 구간(Upper Confidence Bound, UCB) 정책이 순서 최적성(order optimality)을 달성하는 통합 조건을 명확히 했습니다.

- **Technical Details**: UCB 정책을 적용하기 위해, 'oracle quantity'라는 개념을 도입했습니다. 이는 최고의 값을 가진 팔(arm)을 식별하며, 이를 통해 oracle quantity의 가장 높은 UCB를 가진 팔을 선택하는 것으로 UCB 정책을 통합적으로 정의합니다. 이 설정에서, 기존의 후회(regret)를 대체하여 실패의 수를 핵심 측정지표로 사용하여 최적성 분석을 수행할 수 있습니다.

- **Performance Highlights**: 본 연구에서 제안된 PIUCB 알고리즘은 최대 개선 확률(Probability of Improvement, PI)을 최대화하는 팔을 선택하는 알고리즘으로, 실험에서 MaxSearch 알고리즘과 비교했을 때 유사하거나 더 나은 성능을 나타냈습니다. 이는 새로운 정책을 특정 oracle quantity에 맞춰 개발할 수 있는 잠재력을 제시합니다.



### Coherent Hierarchical Probabilistic Forecasting of Electric Vehicle Charging Demand (https://arxiv.org/abs/2411.00337)
Comments:
          Paper accepted for IEEE Transactions on Industrial Applications. Personal use of this material is permitted. Permission from Elsevier must be obtained for all other uses

- **What's New**: 전기차(EV)의 채널 수요 예측을 위한 새로운 심층 학습 기반의 프레임워크가 제안되었습니다. 이 프레임워크는 전통적인 최적화 기반 방법의 한계를 넘어서 다수의 EV 충전소(EVCS)의 확률적 모델링 문제를 계층적 방식으로 해결합니다.

- **Technical Details**: 이 연구는 부분 입력 볼록 신경망(Partial Input Convex Neural Network, PICNN)을 활용하여 채널 수요의 조건부 분포를 예측합니다. 또한, 차별 가능한 볼록 최적화 층(Differentiable Convex Optimization Layers, DCL)을 사용하여 샘플링된 시나리오를 조정하고 계층적 제약에 부합하는 일관된 시나리오를 생성합니다. 이러한 접근 방식은 기존의 수렴 방법에 비해 예측 결과의 가중치 행렬을 더 잘 학습할 수 있습니다.

- **Performance Highlights**: 실제 EV 충전 데이터를 기반으로 한 수치 실험을 통해 제안된 방법의 효과성을 입증했습니다. 이 연구는 EVCS 운영자에게 예측 일관성을 제공하며 전력 구매, 에너지 저장 시스템 운영 등을 통해 효율성을 크게 향상시킬 수 있습니다.



### In-situ Self-optimization of Quantum Dot Emission for Lasers by Machine-Learning Assisted Epitaxy (https://arxiv.org/abs/2411.00332)
Comments:
          5 figures

- **What's New**: 본 연구에서는 전통적인 방법에서 벗어나 Machine Learning (ML)을 이용하여 InAs/GaAs 양자점 (Quantum Dots, QDs)의 광 발광 (Photoluminescence, PL) 최적화를 실현했습니다. 이는 반도체 레이저의 활성 영역을 위해 중요한 기술입니다.

- **Technical Details**: 연구팀은 in-situ 반사 고에너지 전자 회절 (Reflection High-Energy Electron Diffraction, RHEED)과 ML을 결합하여, QDs의 표면 재구성과 PL 간의 상관관계를 분석했습니다. RHEED 데이터를 입력으로 하는 경량 ResNet-GLAM 모델이 적용되어 실시간으로 광 성능을 식별하고, 성장 파라미터의 동적 최적화를 가능하게 했습니다.

- **Performance Highlights**: 최적화된 InAs QDs는 GaAs 기판 위에서 PL 강도가 3.2배 증가하고, 풀 폭의 반 최대치 (Full Width at Half Maximum, FWHM)가 36.69 meV에서 28.17 meV로 감소했습니다. 최종적으로, 자동화된 in-situ 자기 최적화 레이저가 1240 nm에서 전기 펌프 연속파 (Continuous-Wave) 작동을 달성하며, 실온에서 낮은 임계 전류 150 A/cm2를 기록했습니다.



### How many classifiers do we need? (https://arxiv.org/abs/2411.00328)
- **What's New**: 이 논문에서는 여러 모델의 예측을 결합하여 정확성을 향상시키는 앙상블(ensemble) 기법을 다루고 있습니다. 특히, 모델 간의 불일치(disagreement)와 새로운 개념인 극성(polarization)이 어떻게 성능 향상에 기여하는지에 대한 분석을 제공합니다.

- **Technical Details**: 저자들은 극성(polarization)의 상한을 도출하고, 대부분의 보간 신경망 모델이 4/3-극화(polarized)된다고 주장하는 신경 극성 법칙(neural polarization law)을 제안합니다. 또한, 다수 투표 방식에서의 분류기(classifier)의 오류를 제한된 엔트로피 조건 하에서 분석하고, 불일치가 목표와 선형으로 상관관계가 있음을 보여줍니다.

- **Performance Highlights**: 이론적 결과와 함께 다양한 이미지 분류 작업에서의 실험 결과를 통해 새롭게 제안한 경계가 기존 경계보다 상당히 개선된 성능을 보임을 입증하였습니다. 다양한 신경망 아키텍처와 데이터셋의 조합에서도 예측이 매우 정확하다고 확인되었습니다.



### Forecasting Mortality in the Middle-Aged and Older Population of England: A 1D-CNN Approach (https://arxiv.org/abs/2411.00317)
- **What's New**: 이번 연구는 전통적으로 표 형태의 데이터에 효과적이지 않은 것으로 알려진 Convolutional Neural Networks (CNN)을 Longitudinal 데이터에 적용하여 사망률을 예측하는 방법을 제시합니다. 특히, 영어 노인 연구(ELSA) 데이터를 기반으로 1D-CNN을 사용하여 사회적, 건강적 요인을 고려한 새로운 접근 방식을 탐구합니다.

- **Technical Details**: 연구자는 ELSA 데이터 세트에서 감지를 통해 CNN을 적용했습니다. 1D-CNN은 시간에 따라 정보가 변화하는 Longitudinal 데이터 분석에 적합하며, 기존의 GLMs 및 GLMMs 모델과 비교할 수 있는 기계 학습 모델의 가능성을 보여줍니다. 연구에 따르면, 데이터가 불균형한 경우 소수 클래스의 과대표집이 예측력을 향상시킬 수 있으며, swish 비선형 함수가 다른 활성화 함수보다 우수한 결과를 보였습니다.

- **Performance Highlights**: 실험 결과, 1D-CNN을 통한 사망률 예측에서 swish 활성화 함수가 가장 우수한 성능을 발휘했으며, 이는 전통적인 회귀 모델과의 비교에서도 유의미한 개선점을 demostrated했습니다. Longitudinal 데이터를 처리하기 위해 CNN을 적용하는 최초의 사례로, 향후 연구에 큰 기여를 할 것으로 기대됩니다.
...
### Analysis of ELSA COVID-19 Substudy response rate using machine learning algorithms (https://arxiv.org/abs/2411.00297)
- **What's New**: 이번 연구는 COVID-19 팬데믹 동안 진행된 English Longitudinal Study of Ageing (ELSA) COVID-19 Substudy의 비응답자를 예측하기 위한 새로운 접근 방식을 제안합니다. 특히, 머신 러닝 (Machine Learning) 알고리즘을 사용하여 비응답자를 예측하고 영향을 미치는 요인을 파악하는 데 중점을 두었습니다.

- **Technical Details**: 이 연구는 K-nearest neighbours (KNN), random forest (RF), AdaBoost, logistic regression, neural networks (NN), support vector classifier (SVC)와 같은 머신 러닝 알고리즘을 활용하여 2차 웨이브에서의 비응답을 예측합니다. RF 모델이 균형 정확도에서 가장 우수한 성능을 보였으며, KNN은 정밀도와 테스트 정확도에서 두각을 나타냈고, 로지스틱 회귀는 수신자 조작 특성 곡선 (ROC) 아래 면적 (AUC)에서 가장 좋은 성과를 보였습니다.

- **Performance Highlights**: 연구 결과, 가장 효과적인 모델인 RF는 비응답 예측의 정밀도와 균형 정확도 면에서 우수한 성과를 낳았습니다. 비응답자 예측에 대한 머신 러닝 접근 방식의 적합성을 보여줌으로써, 미래의 통계 조사에서 비효율성을 줄이는 데 기여할 수 있습니다.
...
### Minimum Empirical Divergence for Sub-Gaussian Linear Bandits (https://arxiv.org/abs/2411.00229)
- **What's New**: 새로운 알고리즘 LinMED(Linear Minimum Empirical Divergence)는 다중 팔 밴딧(multi-armed bandit)을 위해 최초 설계된 MED 알고리즘의 선형 확장이다.

- **Technical Details**: LinMED는 팔 샘플링 확률(arm sampling probabilities)을 폐쇄형(closed-form)으로 계산할 수 있는 랜덤화된 알고리즘(randomized algorithm)이다. 이는 유명한 랜덤화된 알고리즘인 linear Thompson sampling과 다르다. LinMED는 저희가 증명한 바에 따르면, $d\sqrt{n}$의 거의 최적(regret bound)의 손실을 가지며, 여기서 $d$는 차원(dimension), $n$은 시간 범위(time horizon)을 의미한다. LinMED는 또한 $rac{d^2}{\Delta} \left(\log^2(n)\right)\log(\log(n))$의 문제 의존적(regret) 손실을 가지며, 여기서 $\\Delta$는 최소한의 비최적성 간격을 나타낸다.

- **Performance Highlights**: 실험 연구에 따르면, LinMED는 최신 알고리즘과 경쟁력이 있는 성능을 보인다.
...
### BOMP: Bin-Optimized Motion Planning (https://arxiv.org/abs/2411.00221)
- **What's New**: 새로운 논문에서는 Bin-Optimized Motion Planning(BOMP)라는 혁신적인 모션 계획 프레임워크를 제시하며, 이는 산업용 로봇이 깊은 빈에서 박스를 빠르게 픽 앤 플레이스 할 수 있도록 돕는 것을 목표로 합니다.

- **Technical Details**: BOMP는 로봇 팔의 기구학(kinematics), 작동 한계(actuation limits), 잡은 박스의 크기, 빈 환경의 높이 맵(height map)을 고려하여 최적화된 경로를 빠르게 생성합니다. 이 과정에서 딥 뉴럴 네트워크(deep neural network)를 사용하며, 25,000개의 시뮬레이션 장면을 바탕으로 훈련되어 초기 경로를 예측합니다.

- **Performance Highlights**: BOMP는 표준 Up-Over-Down 알고리즘에 비해 36% 빠르고, 기본 샘플링 기반 계획자에 비해 최대 58% 빠른 충돌이 없는 경로를 생성하며, 79%의 높은 성공률을 자랑합니다. 실험에서는 96개의 시뮬레이션 환경과 15개의 실제 환경에서 성능을 검증했습니다.



### Inclusive KL Minimization: A Wasserstein-Fisher-Rao Gradient Flow Perspectiv (https://arxiv.org/abs/2411.00214)
- **What's New**: 본 논문은 inclusive KL divergence 최적화를 위한 일반적인 근사 패러다임이 PDE(편미분 방정식) 해석 이론에 기반하여 구축될 수 있음을 보여줍니다. 또한, 기존의 여러 학습 알고리즘을 이 패러다임의 특정한 구현으로 해석할 수 있음을 밝혔습니다.

- **Technical Details**: 이 논문에서는 Wasserstein-Fisher-Rao gradient flow의 이론적 기초를 제공하고, inclusive KL divergence를 최소화하는 과정에서 발생하는 다양한 새로운 결과를 설명합니다. 또한 Fisher-Rao 흐름의 발견을 통해 MMD(최소 이동 거리) 문제에 대한 새로운 통찰을 제시합니다.

- **Performance Highlights**: 기존의 학습 알고리즘과 sampling 알고리즘이 inclusive KL inference framework 내에서 통합적으로 설명될 수 있으며, 이는 Bayesian 통계 및 generative modeling의 기초 이론에 진전을 가져올 것으로 기대됩니다. 또한 이 연구는 기존 알고리즘을 이해하기 위한 새로운 방향성을 제시합니다.



### Learning Mixtures of Unknown Causal Interventions (https://arxiv.org/abs/2411.00213)
- **What's New**: 이 논문은 다양한 과학 분야에서 변수 간의 인과 관계를 이해하는 데 중요한 역할을 하는 개입(intervention)의 효과를 다루며, 특히 선형 구조 방정식 모델(Linear SEM)에서의 잡음이 있는 혼합 데이터(mixed data) 분리 문제를 제기합니다.

- **Technical Details**: 잡음이 있는 개입 분포에서 수집된 mixed observational data를 분리하기 위한 효과적인 알고리즘을 제시합니다. 이 접근 방식은 Gaussian additive noise가 있는 Linear-SEM 모델을 사용하며, 개입의 다양성과 프로퍼티를 활용해 혼합 분포의 각 구성 요소를 효율적으로 복구할 수 있습니다.

- **Performance Highlights**: 샘플의 수가 증가함에 따라 각 개입 분포의 복구 오류가 0에 가까워짐을 보여줍니다. 또한, 시뮬레이션 결과를 통해 혼합 파라미터를 복구하고, 알려지지 않은 개입 대상을 식별하며, 높은 정확도로 인과 그래프를 학습할 수 있음을 입증합니다.
...
### Machine Learning Framework for Audio-Based Content Evaluation using MFCC, Chroma, Spectral Contrast, and Temporal Feature Engineering (https://arxiv.org/abs/2411.00195)
Comments:
          6 pages, 6 figures

- **What's New**: 이번 연구에서는 오디오 콘텐츠 간의 유사성을 평가하고 감정 점수를 예측하는 머신러닝 프레임워크를 제시합니다. YouTube에서 음악 커버 샘플과 원곡 오디오, 사용자 댓글로부터 유도된 감정 점수를 포함하는 데이터셋을 구축했습니다.

- **Technical Details**: 오디오 신호를 30초 간격으로 구분하고 상대적으로 높은 차원의 특징 표현을 추출하기 위해 Mel-Frequency Cepstral Coefficients (MFCC), Chroma, Spectral Contrast 및 Temporal 특성을 사용하였습니다. 이 특징을 활용하여 0-100 점 스케일로 감정 점수를 예측하는 회귀 모델을 훈련하였고, RMSE (root mean square error) 값을 각각 3.420, 5.482, 2.783, 4.212로 달성했습니다.

- **Performance Highlights**: 제안된 모델은 절대 차이 메트릭스 기반의 기준 모델에 비해 개선된 성능을 보여주었으며, 오디오 감정과 유사성을 포착하는 데 있어 머신러닝의 잠재력을 입증했습니다. 이 연구 결과는 미디어 분석에 대한 AI 응용 프로그램을 위한 적응 가능한 프레임워크를 제공합니다.
...
### Residual Deep Gaussian Processes on Manifolds (https://arxiv.org/abs/2411.00161)
- **What's New**: 본 논문에서는 Riemannian manifolds에 대한 실용적인 deep Gaussian process 모델을 제안합니다. 이는 residual neural networks와 유사한 접근 방식을 갖고 있습니다.

- **Technical Details**: 제안된 모델은 manifold-to-manifold hidden layers와 임의의 마지막 layer를 사용하여 manifold 및 scalar-valued functions, 그리고 vector fields를 모델링할 수 있습니다. 이런 모델은 shallow Gaussian processes로는 너무 복잡해 보이는 데이터에 적합합니다.

- **Performance Highlights**: 제안된 모델은 낮은 고도에서 발생하는 복잡한 비정상 패턴에 대한 예측 품질과 불확실성 보정에서 큰 성능 향상을 보여주며, 과적합에 강한 견고성을 유지합니다. 또한, Bayesian optimisation 문제에 대한 적용을 통해 로보틱스에서 동기를 부여받은 구체적인 사례에서 최적화 과정의 후반 단계에서 상당한 개선을 나타냅니다.
...
### Enhancing Brain Source Reconstruction through Physics-Informed 3D Neural Networks (https://arxiv.org/abs/2411.00143)
Comments:
          Under Review in IEEE Transactions on Medical Imaging

- **What's New**: EEG 신호로부터 뇌 소스를 재구성하는 데 있어, 전통적인 방법과 딥러닝 기술을 통합한 새로운 하이브리드 방법 3D-PIUNet를 제안합니다. 이 방법은 물리학 기반의 초기 추정값을 사용하여 뇌의 절차적 구조를 활용하며, 3D 볼륨으로서 뇌를 바라보아 공간 의존성을 학습합니다.

- **Technical Details**: 3D-PIUNet는 가짜 역행렬( pseudo inverse)을 통해 측정값을 소스 공간으로 매핑하며, 3D convolutional U-Net을 사용하여 각 공간 패턴을 학습함으로써 솔루션을 세밀하게 조정합니다. 이 모델은 다양한 이론적 배경과 개념을 결합하여 M/EEG의 비선형 문제를 해결할 수 있는 잠재력을 보여줍니다.

- **Performance Highlights**: 3D-PIUNet는 기존의 전통적인 방법과 딥러닝 기법에 비해 소스 로컬리제이션(source localization)의 정확도가 현저하게 향상되었음을 입증합니다. 특히, 노이즈가 있는 환경에서도 안정적인 재구성을 이루며, 실제 EEG 데이터를 사용한 실험에서도 비교적 정확하게 시각 피질을 식별하고 예상되는 시간적 행동을 재구성합니다.
...
### Soft Condorcet Optimization for Ranking of General Agents (https://arxiv.org/abs/2411.00119)
- **What's New**: 이 논문에서는 Soft Condorcet Optimization (SCO)이라는 새로운 랭킹 기법을 제안합니다. 이 기법은 에이전트들의 성능을 다양한 작업에서 비교하기 위해 각 에이전트의 비교를 최소한의 실수로 예측하는 최적 랭킹을 계산합니다.

- **Technical Details**: SCO는 에이전트에 숫자 값을 할당하고, 이를 파라미터 벡터로 간주하며, 평가 데이터에 따라 업데이트됩니다. 이 랭킹 시스템은 고전적인 Elo 시스템의 제한점을 극복하며, sigmoid loss를 사용하여 Kemeny-Young 투표 방법의 근사값을 제공합니다. 이 방법은 최대 가능도 추정(MLE) 방식에 기반하여 각 에이전트의 랭킹을 구합니다.

- **Performance Highlights**: SCO 랭킹은 59% 이상의 데이터가 결여된 상태에서도 정확한 근사치를 제공하며, 31,049개의 인간 플레이어로 이루어진 Diplomacy 게임에서 엘로 및 다른 방법들보다 더 나은 성능을 보여주었습니다.



### A Geometric Framework for Understanding Memorization in Generative Models (https://arxiv.org/abs/2411.00113)
Comments:
          10 pages, 7 figures

- **What's New**: 본 논문에서는 manifold memorization hypothesis (MMH)를 제안하여, generative 모델의 메모리화 현상을 기하학적 관점에서 이해하려고 합니다. 이는 메모리화가 모델이 학습한 manifold의 차원과 관련이 있다는 새로운 통찰을 제공합니다.

- **Technical Details**: MMH는 데이터의 진실한 manifold와 모델이 학습한 manifold의 차원 간의 관계를 분석합니다. 이 프레임워크를 통해 데이터 포인트가 얼마나 '메모리화'되어 있는지의 형식적 기준을 제공하고, 메모리화된 데이터를 과적합 기반 메모리화와 데이터 분포 기반 메모리화로 두 가지 범주로 나눕니다.

- **Performance Highlights**: MMH를 사용하여 synthetic data와 Stable Diffusion 규모의 이미지 데이터셋을 통한 실험적 검증을 진행했습니다. 모델의 local intrinsic dimension (LID)이 메모리화 예측에 강력한 상관관계를 가지고 있으며, 메모리화를 방지하기 위한 새로운 접근 방식도 제시하였습니다.
...
### First, Learn What You Don't Know: Active Information Gathering for Driving at the Limits of Handling (https://arxiv.org/abs/2411.00107)
- **What's New**: 본 연구에서는 Bayesian 메타-러닝 (meta-learning) 기반의 모델 예측 제어 (MPC) 프레임워크를 제안합니다. 이 프레임워크는 빠른 온라인 적응을 가능하게 하는 표현력 있는 차량 동역학 모델을 포함하며, 불확실성 추정치를 사용하여 정보 기반 데이터 수집을 유도하고 모델을 빠르게 개선합니다.

- **Technical Details**: 제안된 차량 동역학 모델은 물리 기반 모델과 마지막 층 Bayesian 메타-러닝 모델을 결합하여 비선형 동역학을 효율적으로 식별할 수 있도록 설계되었습니다. 이 모델은 신뢰성 있는 제어를 위해 온라인에서 빠르게 적응할 수 있으며, 데이터 수집 과정에서 제약 조건을 준수합니다. 차량 상태는 곡선 좌표계에서 표현되며, 이 모델은 Gaussian 분포를 따르는 방해 요소를 포함하여 차량의 동적 특성을 설명합니다.

- **Performance Highlights**: 실험 결과, 이 프레임워크는 동적 드리프트 조작에서 신뢰할 수 있는 제어를 가능하게 하며, 온라인 적응만으로는 안정성 한계에서 차량의 무사한 조작이 부족할 수 있음을 보여주었습니다. 또한, 능동적인 데이터 수집이 신뢰할 수 있는 성능을 달성하는 데 크게 기여함을 확인했습니다.



### A Universal Quantum Computer From Relativistic Motion (https://arxiv.org/abs/2411.00105)
Comments:
          5 pages + appendices, 1 figure - revtex4-2

- **What's New**: 이 논문에서는 변별적 양자 회로 접근 방식을 사용하여 상대론적 양자 컴퓨팅 아키텍처를 명시적으로 구축했습니다. 논문의 결과는 보편적인 양자 컴퓨팅을 가능하게 합니다.

- **Technical Details**: 변별적 양자 회로는 조절 가능한 단일 큐비트 회전과 서로 얽히게 하는 게이트로 구성되어 있으며, 이는 연속적으로 구현됩니다. 단일 큐비트 회전은 큐비트의 궤적의 적절한 시간 간격에 의해 매개변수화되며, 이는 시공간에서의 상대론적 운동을 변화시킴으로써 조절될 수 있습니다. 얽힘 층은 큐비트 간의 직접적인 결합이 아닌 상대론적 양자 장을 통해 매개됩니다.

- **Performance Highlights**: 모든 얽힘 연산이 효과적으로 유니터리하게 유지되는 매개변수 범위를 보여주는 채널 충실도에 대한 하한을 도출했습니다. 또한, 상대론적 큐비트를 활용하여 양자 푸리에 변환을 명시적으로 구현하는 방법을 고려하였습니다.
...
### Solving the 2D Advection-Diffusion Equation using Fixed-Depth Symbolic Regression and Symbolic Differentiation without Expression Trees (https://arxiv.org/abs/2411.00011)
Comments:
          13 pages, 2 figures, 16 equations

- **What's New**: 이 논문은 고정 깊이 기호 회귀(fixed-depth symbolic regression)와 기호 미분(symbolic differentiation)을 이용하여 2D 대류-확산 방정식(2D advection-diffusion equation)을 푸는 새로운 방법을 제시합니다. 이 방법은 명시적 표현 트리(expression trees)를 사용하지 않고, 두 가지 서로 다른 초기 및 경계 조건을 적용하여 정확성을 입증했습니다.

- **Technical Details**: 기호 회귀는 다차원 피쳐 벡터(x→)를 바탕으로 스칼라 레이블(y)을 예측하는 기법입니다. 이 과정에서 다양한 연산자(unary operators, binary operators)와 특성(feature)들이 조합되어 모델이 형성됩니다. 또한, 기호 미분은 체인(rule), 곱(rule) 및 몫(rule)을 사용해 해석적으로 도함수를 계산합니다. 이 논문에서 제안하는 방법은 메모리 할당을 최소화하고 최적의 성능을 고려하여 배열 기반(array-based) 접근 방식을 통해 수행됩니다.

- **Performance Highlights**: 이 방법은 두 가지 경우에 대해 적용되어 대류-확산 방정식의 근사 해를 효율적으로 찾는 능력을 시연하였으며, 향후 복잡한 시스템에의 적용 가능성을 제시합니다. 또한 기존 기법들에 비해 계산 성능이 개선될 가능성이 있습니다.



### RapidDock: Unlocking Proteome-scale Molecular Docking (https://arxiv.org/abs/2411.00004)
- **What's New**: RapidDock라는 새로운 효율적인 transformer 기반 모델을 통해 빠르고 정확한 분자 도킹(molecular docking) 방법을 제안합니다. 이는 기존의 도킹 도구들보다 속도가 최소 100배 빠르며, 정확도에 대한 타협 없이 수행됩니다.

- **Technical Details**: RapidDock는 3D 구조의 상대적 거리 임베딩(relative distance embeddings)과 주의(attention) 매트릭스를 활용하며, 단백질 접힘(protein folding)에 대한 사전 훈련(pre-training)을 포함하고 있습니다. 또한 분자 대칭에 불변인(custom loss function) 손실 함수를 적용하여 도킹 성능을 향상시킵니다.

- **Performance Highlights**: Posebusters와 DockGen 벤치마크에서 각각 52.1%와 44.0%의 성공률을 기록하며, 단일 GPU에서 평균 추론 시간은 0.04초입니다. 이는 RapidDock의 대규모 도킹 연구에 대한 잠재력을 강조합니다.

...
### On the Crucial Role of Initialization for Matrix Factorization (https://arxiv.org/abs/2410.18965)
- **What's New**: 이 연구는 전통적인 low-rank (저차원) matrix factorization (행렬 분해) 문제에 대한 새로운 통찰을 제시하며, 초기화(initialization)가 이러한 비볼록(nonconvex)하고 비smooth(nonsmooth) 최적화의 수렴 속도에 미치는 중요한 역할을 강조합니다.

- **Technical Details**: Nystrom 초기화 방식을 도입하여 Scaled Gradient Descent (ScaledGD)의 글로벌 수렴을 크게 개선하였고, 이는 대칭 및 비대칭 matrix factorization 작업 모두에 적용됩니다. 특히, Nystrom 초기화를 적용한 ScaledGD는 기존에 선형 수렴 속도만 알려졌던 경우에서도 제곱 수렴(quadratic convergence)에 도달함을 증명하였습니다. 본 연구는 또한 이 초기화를 low-rank adapters (LoRA)에 확장시켰으며, 이는 foundation models(기초 모델)의 finetuning (세부 조정)에 자주 사용됩니다.

- **Performance Highlights**: NoRA(LoRA에 Nystrom 초기화를 적용한 방식)는 다양한 downstream tasks (하위 작업) 및 모델 스케일(1B에서 7B 파라미터까지)에서 우수한 성능을 보여주었습니다.
...
### Learning Structured Compressed Sensing with Automatic Resource Allocation (https://arxiv.org/abs/2410.18954)
Comments:
          Unsupervised Learning, Information Theory, Compressed Sensing, Subsampling

- **What's New**: 이 논문에서는 Structured COmpressed Sensing with Automatic Resource Allocation (SCOSARA)를 소개합니다. SCOSARA는 정보 이론 기반의 비지도 학습 전략을 사용하여 샘플링 차원에 따라 샘플을 자동으로 분배하면서 Fisher 정보(content)를 극대화합니다.

- **Technical Details**: SCOSARA는 q차원에서 데이터 수집을 수행하며, 각 차원은 자동으로 학습된 샘플 수에 따라 샘플링됩니다. Fisher Information Matrix (FIM)을 활용하여, 샘플링 매트릭스를 효율적으로 설계하며, Cramér-Rao Bound (CRB)를 통해 성능을 평가합니다.

- **Performance Highlights**: 시뮬레이션 결과, SCOSARA는 최신 머신 러닝 기반 알고리즘 및 탐욕 검색 알고리즘보다 낮은 Cramér-Rao Bound 값을 기록하며, 학습 가능한 매개변수, 계산 복잡도, 메모리 요구사항 면에서도 우수한 성능을 보입니다.



### Adjusted Overfitting Regression (https://arxiv.org/abs/2410.18950)
- **What's New**: 이 논문에서는 "distance-based regression"(거리 기반 회귀)이라는 새로운 형태의 회귀 분석을 소개합니다. 이 기법은 과적합(overfitting) 및 과소적합(underfitting) 문제를 조정할 수 있는 방법입니다.

- **Technical Details**: 이 회귀 기법은 false patterns(잘못된 패턴)을 찾는 것을 방지하여 더 정확한 예측을 가능하게 합니다. 이 방법은 데이터 간의 거리 정보에 기반하여 회귀 분석의 결과를 개선하는 데 중점을 둡니다.

- **Performance Highlights**: 제안된 회귀 기법을 특정 데이터 집합에 적용하여 실제 가치를 입증하며, 추가적인 최적화 방법도 제시합니다.



### LoRANN: Low-Rank Matrix Factorization for Approximate Nearest Neighbor Search (https://arxiv.org/abs/2410.18926)
Comments:
          Accepted to NeurIPS 2024

- **What's New**: 이번 논문은 클러스터링 기반의 Approximate Nearest Neighbor (ANN) 검색을 위한 새로운 감독된 스코어 계산 방법인 Reduced-Rank Regression (RRR)을 제안합니다.

- **Technical Details**: 이 방법은 쿼리 포인트와 클러스터 포인트 간의 유사성을 추정하는 다변량 회귀 문제로 모델링됩니다. RRR은 8비트 정수 계산을 사용하여 쿼리 대기 시간 및 메모리 소비를 개선하며, 기존의 Product Quantization (PQ) 방식보다 간단하고 효율적입니다.

- **Performance Highlights**: 실험 결과, RRR은 PQ보다 더 빠른 쿼리 속도 및 낮은 메모리 사용량을 보입니다. 또한, 새롭게 제안된 LoRANN 라이브러리는 GPU에서의 효율적 구현을 제공하며, 기존의 클러스터링 기반 라이브러리인 Faiss 및 ScaNN보다 뛰어난 성능을 보여주며, 대다수 데이터 세트에서 90% 이하의 리콜 수준에서도 주요 그래프 기반 라이브러리 GLASS보다 빠른 쿼리 시간을 기록했습니다.



### Using Parametric PINNs for Predicting Internal and External Turbulent Flows (https://arxiv.org/abs/2410.18917)
Comments:
          To be presented at the Data-driven and Differentiable Simulations, Surrogates, and Solvers (D3S3) Workshop at NeurIPS'2024

- **What's New**: 본 논문에서는 물리 기반 신경망(Physics-Informed Neural Networks, PINNs)을 사용하여 난류 유동 예측을 위한 매개변수 대체 모델(parametric surrogate models)을 개발하는 새로운 방법론을 소개합니다. 이 연구는 내부 및 외부 유동의 난류 변수를 예측하는 데 특화된 RANS-PINN 프레임워크의 사용을 강조합니다.

- **Technical Details**: CFD(Calculational Fluid Dynamics) 시뮬레이션을 최적화하기 위해 k-ε 난류 모델을 PINN 기반 프레임워크에 통합하여 사용하며, 정확한 수렴을 보장하기 위해 새로운 샘플링 방식을 도입합니다. 또한, RANS(Formulation of Reynolds-Averaged Navier-Stokes) 방정식을 활용하여 다양한 유동 문제에 대한 예측 정확성을 높입니다.

- **Performance Highlights**: 제안된 방법은 여러 유동 형상에서 Reynolds 수에 관계없이 속도 및 압력 필드를 정확히 예측할 수 있는 능력을 보여주며,와결합한 착시적 균형을 통해 다양한 영역에서의 기여도를 고려하여 훈련의 수렴성을 높이는 데 성공했습니다.



### ArterialNet: Reconstructing Arterial Blood Pressure Waveform with Wearable Pulsatile Signals, a Cohort-Aware Approach (https://arxiv.org/abs/2410.18895)
- **What's New**: 새로운 연구에서 Continuous arterial blood pressure (ABP) 모니터링의 비침습적 접근 방식을 제안하는 ArterialNet 모델이 소개되었습니다. 기존의 비침습적 방법들이 개인 차이에 민감했던 문제를 해결하기 위한 혁신적인 방법입니다.

- **Technical Details**: ArterialNet은 일반화된 pulsatile-to-ABP 신호 변환 및 개인 맞춤형 특성 추출을 하이브리드 손실 함수와 정규화(regularization) 기술을 사용하여 통합합니다. MIMIC-III 데이터셋을 사용하여 검증했으며, root mean square error (RMSE) 5.41 mmHg를 기록했습니다.

- **Performance Highlights**: 원격 건강 시나리오에서 ArterialNet은 7.99 mmHg의 RMSE로 ABP를 재구성하며 뛰어난 성능을 보여주었습니다. 심박수(SBP) 및 이완기 혈압(DBP) 추정에서 개인 간 변동성이 크게 감소되었습니다.



### Meta-Learning with Heterogeneous Tasks (https://arxiv.org/abs/2410.18894)
Comments:
          13 pages, 4 figures

- **What's New**: 이 논문에서는 이질적인(task heterogeneous) 작업을 효과적으로 관리하기 위한 새로운 메타 학습 방법인 Heterogeneous Tasks Robust Meta-learning (HeTRoM)을 소개합니다. 기존 메타 학습 방법들이 모든 작업이 동등하게 중요하다는 가정을 바탕으로 하는 반면, HeTRoM은 작업의 난이도와 데이터 품질이 이질적인 실제 환경에 맞춰 작업 수준의 목표를 설정합니다.

- **Technical Details**: HeTRoM의 핵심 구성 요소는 새로운 순위 기반(task-level) 손실 함수입니다. 이 손실 함수는 메타 훈련 단계에서 동적으로 작업을 선택할 수 있도록 설계되어 있으며, 너무 쉬운 작업이나 노이즈가 있는 작업, 아웃라이어(outlier) 작업의 영향을 줄이는 데 기여합니다. 새로운 손실 함수는 로그 정규 분포(log-normal distribution) 모델을 기반으로 하며, 스토캐스틱 경량(stochastic gradient) 알고리즘을 통해 효율적으로 최적화됩니다.

- **Performance Highlights**: 실험 결과, HeTRoM은 다양한 작업 설정에 적응할 수 있는 유연성을 제공하며, 메타 학습기의 전반적인 성능을 향상시키는 것으로 나타났습니다. 여러 기준 데이터셋에 대한 실험 평가를 통해 HeTRoM의 효과성과 강인성을 입증하였습니다.



### A Riemannian Framework for Learning Reduced-order Lagrangian Dynamics (https://arxiv.org/abs/2410.18868)
Comments:
          28 pages, 16 figures

- **What's New**: 본 연구는 물리적 일관성을 인덕티브 바이어스로 하여 딥 뉴럴 네트워크(DNN)가 비선형 동적 모델을 학습하는 데 있어 더 나은 일반화 능력과 데이터 효율성을 보일 수 있음을 보여줍니다. 특히, 제안된 기하학적 네트워크 아키텍처는 원래의 고차원 시스템 동작을 정확히 설명하는 물리적으로 일관된 축소차원(dynamic) 매개변수를 학습합니다.

- **Technical Details**: 제안된 접근법은 제약 조건을 포함한 오토인코더(autoencoder)와 라그랑지안 신경망(LNN)을 활용해, 물리적으로 일관된 저차원 동역학을 학습하기 위한 잠재적 임베디드 서브매니폴드를 학습합니다. 이 과정에서 파라미터의 내재적 기하학을 고려하여 LNN의 양수정의층(positive-definite layers)과 리만 최적화(Riemannian optimization)를 통해 파라미터와 가중치를 추론합니다. 또한, 축소된 에너지가 방출되지 않도록 하는 물리적으로 그럴듯한 모델을 추론합니다.

- **Performance Highlights**: 효율적인 축소차원 동역학을 학습하여 강체 및 변형 가능한 고차원 시스템인 진자, 로프, 얇은 천의 동작을 시간적으로 더 정확하게 예측하는 결과를 나타냈습니다.



### FedSPD: A Soft-clustering Approach for Personalized Decentralized Federated Learning (https://arxiv.org/abs/2410.18862)
- **What's New**: 본 논문은 개인화된 연합 학습 (Personalized Federated Learning) 알고리즘인 FedSPD를 제안합니다. FedSPD는 분산된 환경에서 클라이언트 간 직접적으로 모델 교환을 가능하게 하여, 개인화된 모델을 지속적으로 학습하고, 각 클라이언트의 데이터 분포에 맞춰 성능을 향상시키는 데 중점을 두고 있습니다.

- **Technical Details**: FedSPD 알고리즘은 클러스터 기반의 프레임워크를 활용하여 서로 다른 데이터 클러스터에 대한 합의 모델을 학습합니다. 이 과정에서 각 클라이언트는 클러스터 모델을 개별적으로 학습하고, 개별 데이터 혼합에 맞춰 개인화된 업데이트를 수행할 수 있습니다. 또한 FedSPD는 낮은 연결성 네트워크에서도 정확한 모델 학습을 보장합니다.

- **Performance Highlights**: 실제 데이터셋에서 진행된 실험 결과, FedSPD는 기존의 개인화 연합 학습 알고리즘의 여러 분산 변형들을 능가하는 성능을 보였으며, 특히 낮은 연결성 네트워크 환경에서도 뛰어난 성능을 유지하는 것으로 나타났습니다.
...
### MazeNet: An Accurate, Fast, and Scalable Deep Learning Solution for Steiner Minimum Trees (https://arxiv.org/abs/2410.18832)
Comments:
          15 pages, 15 figures. Submitted to ICLR 2025

- **What's New**: 이 논문은 OARSMT(Obstacle Avoiding Rectilinear Steiner Minimum Tree) 문제를 해결하기 위해 MazeNet이라는 딥러닝 기반 방법을 제안합니다. 기존의 정확한 알고리즘들이 많은 터미널 수와 함께 길게 확장되지 못하는 문제를 해결하고자 합니다. MazeNet은 OARSMT 문제를 미로 해결 과제로 재구성하여 RCNN(재귀 신경망)을 사용하여 해결합니다.

- **Technical Details**: OARSMT 문제는 주어진 여러 개의 터미널을 수직 및 수평 선으로 연결하는 최단 경로를 찾는 것으로 정의되며, 장애물을 피해야 합니다. 이 문제를 해결하기 위해 MazeNet은 다양한 사이즈와 터미널 수를 가진 미로를 해결할 수 있도록 설계되었으며, 11×11 크기와 최대 8개의 터미널까지의 미로에 대한 결과를 제시합니다. 또한, 별도의 검색 기반 알고리즘을 통해 정확한 해를 인식하는 과정을 추가하여 효율성 및 정확성을 높였습니다.

- **Performance Highlights**: MazeNet은 다양한 실험에서 100%의 정확도를 기록했으며, 전통적인 정확한 알고리즘에 비해 실행 시간을 크게 단축하였습니다. 또한, 최신 근사 알고리즘보다 더 많은 터미널을 처리할 수 있는 능력을 보여줍니다.
...
- **What's New**: PointPatchRL (PPRL)은 point clouds(점군)를 활용한 새로운 강화 학습 방법입니다. 기존의 단순한 인코더 구조를 넘어, 원시 point cloud 데이터에서 작업 관련 기능을 직접적으로 학습할 수 있는 패칭 기반 접근 방식을 제안합니다.

- **Technical Details**: PPRL은 point clouds를 겹치는 패치로 나누고 이를 토크나이징한 뒤 Transformer 인코더로 처리하는 방식으로 구현됩니다. 이는 원시 point cloud 데이터를 사용하는데 최적화된 구조로, 이미지 기반 RL보다 더 많은 기하학적 정보를 활용할 수 있습니다. 추가적으로, masked reconstruction을 사용하여 latent representation에서 geometric 정보를 향상시킵니다.

- **Performance Highlights**: PPRL은 복잡한 조작 작업에서 상태에 따른 이미지 관측치를 사용한 기존 강력한 모델 프리 및 모델 기반 기준선보다 뛰어난 성능을 보여줍니다.특히, 변화하는 물체 기하학과 이동하는 카메라가 있는 환경에서도 성능을 개선하는데 성공하였습니다.
...
- **What's New**: 본 논문에서는 denoising diffusion probabilistic model (DDPM) 이 저차원의 구조를 자동으로 활용하여 샘플링 속도를 빠르게 할 수 있는 방법을 제시했습니다. 기존 연구를 보강하고, DDPM의 반복 복잡도가 저차원성에 비례하여 최적적으로 조정될 수 있음을 밝혔습니다.

- **Technical Details**: DDPM의 업데이트 규칙은 적절히 매개변수화된 Stochastic Differential Equation (SDE)을 이산화(discretization)하여 실행한 것과 동등하다는 주목할 만한 관찰을 기반으로 하고 있습니다. 논문의 주요 성과는 저차원 데이터 분포에 대해 O(k/ε²) 단계만 필요하다는 점이며, 이는 KL divergence로 분포의 차이를 측정할 때 최적의 복잡도를 나타냅니다.

- **Performance Highlights**: 이론적으로 DDPM는 저차원 데이터 구조에서 O(k/ε²) 단계로 ε²-정확도(ε²-accuracy)를 달성할 수 있으며, 초기 추정값 없이도 이론을 확장할 수 있습니다. 또한, 저차원 지원을 통해 샘플링 과정을 가속화한다는 이론적 근거를 제공합니다.
...
- **What's New**: 이번 연구에서는 Retrieval-Augmented Time series Diffusion 모델(RATD)을 제안하여, 기존의 시간 시계열 확산 모델의 불안정한 성능을 해결하고자 합니다. RATD는 두 가지 주요 구성 요소로 이루어져 있으며, 하나는 임베딩 기반 검색 프로세스, 다른 하나는 참조 기반 확산 모델입니다.

- **Technical Details**: RATD는 역사적 시간 시계열을 기반으로 가장 관련성이 높은 시계열을 검색하고, 이를 통해 잡음 제거 과정에서 의미 있는 샘플을 활용합니다. 이 과정은 두 가지 주요 기능을 통해 진행됩니다: 첫 번째는 k개의 이웃 샘플을 검색하여 참조로 활용하는 것입니다; 두 번째는 참조를 통해 잡음 제거(denoising) 과정을 유도하는 것입니다.

- **Performance Highlights**: 다양한 데이터셋에서 실시된 실험 결과, RATD는 기존의 시간 시계열 확산 모델보다 성능이 우수하거나 동등함을 보여주었으며, 특히 복잡한 예측 작업에서 효과적인 결과를 도출하였습니다.



### Exploiting Interpretable Capabilities with Concept-Enhanced Diffusion and Prototype Networks (https://arxiv.org/abs/2410.18705)
Comments:
          Interpretable AI: Past, Present and Future Workshop at NeurIPS 2024

- **What's New**: 본 논문에서는 Concept-Guided Conditional Diffusion과 Concept-Guided Prototype Networks를 제안하여 기존 모델에 개념 정보를 통합함으로써 머신 러닝의 해석 가능성을 극대화하고 새로운 연구 방향을 열었다.

- **Technical Details**: 개념 기반(machine learning methods) 기법을 통해 Interpretability를 높이는 방법을 개발하였다. 특히, conditional diffusion models를 이용하여 개념 정보를 활용한 시각적 표현을 생성하고, Prototypical Part Networks를 통해 해석 가능한 개념 예측을 수행하는 구조로 구성되어 있다.

- **Performance Highlights**: 이러한 방법들을 실제 데이터셋에 적용함으로써 해석 가능한 결과를 도출하였다. Concept-Guided Conditional Diffusion은 개념 지식을 바탕으로 샘플을 생성하는 방식으로, Concept-Guided Prototype Networks은 트리의 프로토타입을 구축하여 예측을 수행함으로써 해석 가능성을 높였다.



### BATON: Enhancing Batch-wise Inference Efficiency for Large Language Models via Dynamic Re-batching (https://arxiv.org/abs/2410.18701)
- **What's New**: BATON은 대형 언어 모델(LLM)의 효율적인 배치 방식 추론 기법으로, 처리 배치를 동적으로 조정하여 거의 제로에 가까운 유휴(computation) 계산을 달성하면서 추가 자원 소비 없이 새로운 쿼리를 삽입할 수 있도록 설계되었습니다.

- **Technical Details**: BATON은 다음의 두 가지 주요 방식을 사용합니다: 1) 새로운 쿼리와 처리 배치에 관련된 벡터의 차원을 정렬하기 위해 패딩 작업을 수행하여 텐서를 형성하고, 추론의 정확성을 보장하기 위한 새로운 attention mask를 생성합니다. 2) 새로운 쿼리의 Keys와 Values를 처리 배치의 KV_Cache에 삽입하여 유휴 계산을 제거합니다.

- **Performance Highlights**: 실험 결과, BATON은 최신 솔루션인 Orca보다 최대 1.75배의 쿼리 처리 성능 향상을 보였습니다.



### Hierarchical Multimodal LLMs with Semantic Space Alignment for Enhanced Time Series Classification (https://arxiv.org/abs/2410.18686)
- **What's New**: HiTime는 다변량 시간 시계열 분류(Multivariate Time Series Classification, MTSC)를 위해 체계적으로 시간 정보를 LLMs(대형 언어 모델)에 통합하는 새로운 접근 방식을 제안합니다. 기존 접근법에서 보이지 않았던 동적 시간적 정보의 중요성을 강조합니다.

- **Technical Details**: HiTime는 데이터 특정 및 작업 특정 임베딩을 사용하여 다양한 시간 시계열 특성을 캡처하는 계층적 특징 인코더(hierarchical feature encoder)를 활용합니다. 또한, 시간 시계열 데이터와 텍스트 간 의미 공간 정합을 위한 이중 뷰 대조 정합 모듈(dual-view contrastive alignment module)을 도입해 두 가지 모달리티에서의 임베딩을 연결합니다. 하이브리드 프롬프트 전략(hybrid prompting strategy)을 채택하여 사전 훈련된 LLM을 효율적으로 튜닝(tuning)합니다.

- **Performance Highlights**: 광범위한 벤치마크 데이터셋을 이용한 실험 결과 HiTime이 기존의 경쟁적 기준 방법에 비해 시간 시계열 분류 정확도를 크게 개선하는 것을 보여줍니다. 이는 동적 시간적 특징을 효과적으로 통합하고 의미적 정합을 보장한 덕분입니다.



### Homomorphism Counts as Structural Encodings for Graph Learning (https://arxiv.org/abs/2410.18676)
- **What's New**: 이번 연구에서는 그래프 구조를 효과적으로 인코딩하기 위해 새로운 인코딩 방식인 motif structural encoding (MoSE)를 제안합니다. MoSE는 그래프 동형사상(homomorphisms)의 수를 기반으로 하여 더 강력하고 유연한 구조적 인코딩 프레임워크를 제공합니다.

- **Technical Details**: MoSE는 그래프 동형사상의 수를 이용하여 그래프 구조를 캡처하는 인코딩 방식입니다. 이 방법은 기존의 random-walk structural encoding (RWSE)와 비교할 때 표현력에서 유의미한 이점을 가지며, 1-WL(Weisfeiler Leman) 계층에만 제한되지 않습니다. MoSE는 MPNNs 및 Transformer와 함께 사용할 수 있는 강력한 대안입니다.

- **Performance Highlights**: MoSE는 다양한 아키텍처에서 잘 알려진 다른 위치적 및 구조적 인코딩을 초월하여 우수한 성능을 보여주며, 널리 연구된 분자 속성 예측 데이터셋에서 최첨단(State-of-the-Art) 성과를 달성했습니다.
...
- **What's New**: 본 논문은 시간 창(time window)으로 전처리된 흐름 데이터에서 작동하는 네트워크 침입 탐지 시스템( Network Intrusion Detection System, NIDS)을 위한 신경망(neural networks) 시스템을 제시합니다. 기존의 깊은 패킷 검사를 필요로 하지 않으며, 대부분의 NIDS 데이터셋에서 쉽게 얻을 수 있는 11개의 특징만을 요구합니다. 특히, Kalmogorov-Arnold Network (KAN)에서 영감을 받은 트레인 가능한 활성화 함수(trainable activation functions)를 사용하여 보다 간단한 네트워크 구조로 높은 정확도를 달성하는 방안을 제안하고 있습니다.

- **Technical Details**: 이 연구에서는 CICIDS2017 데이터셋 및 본 연구의 일환으로 수집된 맞춤형 데이터셋을 사용하여 NIDS의 일반화 능력을 연구합니다. 제안된 방법은 최소 20개의 입력 특징(feature)을 사용하여 99% 이상의 훈련 정확도를 기록하고, 네트워크 아키텍처 및 활성화 함수에 대한 다양한 연구가 포함되어 있습니다. 데이터셋을 교체할 경우 성능 지표(performance metrics)가 크게 감소하며 이는 서로 다른 데이터셋에서 같은 유형의 흐름의 서명(signature) 차이에 기인합니다.

- **Performance Highlights**: 본 연구의 결과, NIDS의 일반화 정확도가 일부 신경망에서 매우 불안정하고 무작위 초기화 매개변수(random initialization parameters)에 민감하다는 것을 보여줍니다. 또한, 매개변수가 적고 잘 조정된 활성화 함수를 가진 신경망이 더 안정적이며 높은 정확성을 달성하는 것으로 나타났습니다.



### Learning dissipative Hamiltonian dynamics with reproducing kernel Hilbert spaces and random Fourier features (https://arxiv.org/abs/2410.18656)
- **What's New**: 이 논문에서는 제한적이고 noisy 데이터셋에서 소산적인 Hamiltonian 동역학을 학습할 수 있는 새로운 방법을 제안합니다. 이 방법은 Helmholtz 분해를 사용하여 대칭적(vector field)과 소산적(vector field)인 두 개의 벡터 필드의 합으로 배운다는 점에서 차별화됩니다.

- **Technical Details**: Helmholtz decomposition을 활용해 대칭적(vector field)과 소산적(vector field)인 두 벡터 필드를 학습하며, 각각의 벡터 필드는 대칭적인(kernel)과 회전이 없는(curl-free kernel) reproducing kernel Hilbert space(KH)에서 정의됩니다. Random Fourier features를 사용하여 kernel을 근사하여 최적화 문제의 차원을 줄이는 방법을 적용하고 있습니다.

- **Performance Highlights**: 두 개의 소산적 Hamiltonian 시스템에 대해 시뮬레이션을 통해 이 방법의 성능을 검증하였으며, Gaussian separable kernel을 사용하는 방법과 비교하여 예측 정확도가 상당히 향상됨을 보여주었습니다.
...
### Understanding Players as if They Are Talking to the Game in a Customized Language: A Pilot Study (https://arxiv.org/abs/2410.18605)
Comments:
          published in Workshop on Customizable NLP at EMNLP 2024

- **What's New**: 이 파일럿 연구는 게임 이벤트 시퀀스를 사용자 정의된 자연어로 취급하여 언어 모델(Language Models, LMs)의 적용 가능성을 탐구합니다. 인기 모바일 게임인 Candy Crush Saga의 원시 이벤트 데이터를 텍스트 시퀀스로 변환하여 Longformer 모델을 사전 훈련하는 과정을 다루고 있습니다.

- **Technical Details**: 플레이어의 행동은 시간 순서에 따른 이벤트 시퀀스를 생성하며, 이는 사용자의 기기에서 로컬로 기록된 후 중앙 게임 서버로 전송됩니다. 이벤트 데이터는 JSON 형식으로 수집되며, 이를 LM에 적합하게 변환하기 위해 비효율적인 데이터를 제거하고, 도메인 지식에 따른 피처를 카테고리형 변수로 조정합니다. Longformer 모델은 긴 텍스트 입력을 처리하도록 설계되어 있으며, 슬라이딩 윈도우 관심 메커니즘을 사용하여 지역적 맥락과 전역적 주의를 결합합니다.

- **Performance Highlights**: 자기 지도(self-supervised) LMs는 기본 진실 레이블(ground-truth labels)에 의존하지 않고 게임 디자인과 개인화를 개선하는 잠재력을 보여주었습니다. 파일럿 연구의 결과는 플레이어 세분화를 효과적으로 식별하며, 100회의 훈련 후 모델의 성능은 크게 향상되었습니다.



### Differential Informed Auto-Encoder (https://arxiv.org/abs/2410.18593)
- **What's New**: 이번 논문에서는 원본 데이터의 내적 구조를 얻기 위해 미분 방정식을 이용해 훈련된 인코더와 원본 데이터 도메인에서 새로운 데이터를 생성하기 위한 디코더를 제안합니다. 이 과정에서 물리 정보가 포함된 신경망(Physics-Informed Neural Network, PINN)을 활용하여 데이터 생성 과정을 수행합니다.

- **Technical Details**: 단일 변수 함수 u(t)에 대해 일반적인 형태의 2차 미분 방정식을 정의하고, 이를 해결하기 위해 로컬 PCA(Principal Component Analysis)를 이용하여 데이터의 미분 값을 계산합니다. 또한, 모델을 훈련시키기 위해 FCN(Fully Connected Network)을 생성하며 데이터 포인트에서 0을 출력하게끔 학습합니다.

- **Performance Highlights**: 제안된 방법은 낮은 차원의 다양체에서 미분 관계를 제공하여 더 일반화된 데이터 구조를 학습할 수 있게 하며, 실험 결과도 시각적으로 나타내어 효율성과 정확성을 입증했습니다.



### Knowledge Distillation Using Frontier Open-source LLMs: Generalizability and the Role of Synthetic Data (https://arxiv.org/abs/2410.18588)
Comments:
          25 pages, 4 figures

- **What's New**: 다양한 NLP(자연어 처리) 작업을 수행할 수 있는 Llama-3.1-405B-Instruct와 같은 대규모 언어 모델(LLMs)의 지식 증류(knowledge distillation) 방법론을 탐구하였으며, 이를 통해 8B 및 70B 모델의 성능을 유의미하게 향상시킬 수 있는 방법을 제시했습니다.

- **Technical Details**: 이 연구에서는 Llama-3.1-405B-Instruct를 교사 모델로, Llama-3.1-8B-Instruct 및 Llama-3.1-70B-Instruct를 학생 모델로 하여 지식 증류의 유효성을 검증했습니다. 교사 모델이 생성하는 합성 데이터(synthetic data)를 통해 학생 모델의 정확도를 크게 개선했으며, 제로샷(zero-shot) 정확도를 일부 데이터셋에서 초과하는 결과를 얻었습니다. 이 방법론은 특정 작업에 맞춤화된 평가 기준(기준 기반의 정확성 검증)과 함께 고품질 합성 데이터의 중요성을 강조합니다.

- **Performance Highlights**: Llama-3.1-8B 및 70B 학생 모델은 지식 증류 과정을 통해 교사 모델의 추론 능력을 내재화하였으며(standard fine-tuning으로 수행), 모든 평가 실험에 대한 좋은 성능을 보여주었습니다. 합성 데이터의 질이 증류의 효과성 평가에 핵심적인 역할을 했으며, 다양한 벤치마크 데이터셋에서 향상된 성능을 확인했습니다.



### Benchmarking Graph Learning for Drug-Drug Interaction Prediction (https://arxiv.org/abs/2410.18583)
- **What's New**: 이번 연구에서는 약물 간 상호작용(DDI) 예측을 위해 그래프 학습 기반의 새로운 벤치마크를 제안합니다. 현재의 DDI 예측 방법들에 대한 평가의 통합적 비교가 부족한 점, 현실적인 시나리오에서의 평가 공백, 그리고 사이드 정보의 활용 부족 등 기존의 문헌에서 해결되지 않은 한계들을 다루고자 하였습니다.

- **Technical Details**: 우리는 DDI 그래프를 𝒢DDI={𝒱DDI,ℰDDI,ℛDDI}로 정의하며, 여기서 𝒱DDI는 약물 노드의 집합, ℰDDI는 약물 간 상호작용을 나타내는 엣지 집합을 의미합니다. DDI 데이터는 (u,r,v) 형태의 트리플렛으로 구성됩니다. DDI 예측에서의 목표는 주어진 약물 쌍 (u,v)에 대한 상호작용 타입 r를 결정하는 것입니다. 추가적으로, 약물 지문(drug fingerprints)과 생물 의학 네트워크(biomedical networks)를 사이드 정보로 활용하였습니다.

- **Performance Highlights**: 실험 결과에 따르면, 충분한 데이터가 주어질 경우, 더 표현력이 뛰어난 방법들이 다양한 방식에서 우수한 성능을 보입니다. 다만, 기존의 약물 간 상호작용 예측을 위한 방법들은 새로운 약물이 포함된 설정에서 잘 작동하지 않으며, 사이드 정보의 활용이 성능 향상에 기여합니다. 생물 의학 네트워크를 적절히 활용하는 것이 DDI 예측 성능을 향상시킬 수 있다는 점을 발견했습니다.
...
### Graph Pre-Training Models Are Strong Anomaly Detectors (https://arxiv.org/abs/2410.18487)
- **What's New**: 이 연구에서는 Graph Anomaly Detection (GAD)에서 그래프 사전 학습 모델이 강력한 이상 탐지기로 작용할 수 있음을 보여줍니다. 특히, 제한된 감독 상태에서 기존 최첨단(end-to-end) 모델보다 우수한 성능을 발휘한다는 점에 주목하고 있습니다.

- **Technical Details**: 그래프 사전 학습 모델은 두 단계의 학습 패러다임을 따르며, 자가 감독(self-supervised) 목표를 통해 비지도 그래프 데이터에서 보편적인 지식을 습득한 후, 이를 특정 하위 작업에 적용합니다. 이 연구는 이러한 사전 학습 과정이 2-hop 커뮤니티를 넘어 존재하는 하위 표현된 이상 탐지 능력을 향상시킨다는 점을 강조합니다.

- **Performance Highlights**: 사전 학습 모델은 다양한 실제 데이터 세트에서 일관되게 경쟁력 있는 성능을 보였으며, 특히 저밀도 그래프 및 제한된 감독 조건에서 significant하게 뛰어난 성능을 보여주었습니다. 또한, 기존의 세련된 아키텍처나 고급 분류기보다도 앞서는 경향을 보였습니다.



### Classifier Clustering and Feature Alignment for Federated Learning under Distributed Concept Drif (https://arxiv.org/abs/2410.18478)
- **What's New**: 이번 연구에서는 federated learning(FL)에서의 데이터 이질성과 분산 개념 drift가 동시 존재할 때의 문제를 해결하기 위해 새로운 프레임워크인 FedCCFA를 제안합니다. FedCCFA는 local classifier의 클러스터링 및 feature alignment를 통해 데이터의 불일치를 줄이고, 분산된 개념 드리프트에 적응할 수 있도록 설계되었습니다.

- **Technical Details**: FedCCFA는 class-level로 local classifiers를 클러스터링하고, 클러스터링 결과에 따라 clustered feature anchors를 생성합니다. 이 방법은 클라이언트의 feature space를 label distribution의 entropy를 기반으로 적응적으로 맞추어 주며, 데이터 이질성으로 인한 일관성 결여 문제를 완화합니다. 또한, 클라이언트의 feature alignment를 위한 적응형 weight를 도입하여 주 작업 학습이 방해받지 않도록 합니다.

- **Performance Highlights**: 실험 결과, FedCCFA는 다양한 개념 드리프트 설정 하에 기존 방법들보다 현저히 우수한 성능을 보이며, 특히 정확도와 gradient norm에서 개선된 결과를 입증합니다.



### What If the Input is Expanded in OOD Detection? (https://arxiv.org/abs/2410.18472)
Comments:
          accepted by NeurIPS 2024

- **What's New**: 새로운 연구에서는 입력 공간에 다양한 일반적인 손상을 적용하여 OOD(Out-of-distribution) 데이터와 ID(In-distribution) 데이터의 신뢰도 변화를 관찰했습니다. 이를 통해 OOD 감지의 정확도를 높일 수 있는 새로운 방법을 제안했습니다.

- **Technical Details**: 본 연구에서는 Confidence aVerage (CoVer)라는 새로운 점수 방법을 도입했습니다. CoVer는 서로 다른 손상된 입력 및 원본 입력에서 얻은 점수를 단순히 평균하여 OOD 데이터의 동적 변화를 포착할 수 있게 해줍니다. 이 방법은 OOD와 ID 분포를 더욱 구별 가능하게 만듭니다.

- **Performance Highlights**: CoVer는 기존 방법들과 비교하여 뛰어난 성능을 나타냈으며, 단일 모달 및 다중 모달 네트워크 모두에 호환됩니다. 포괄적인 평가를 통해 CoVer의 유효성과 호환성이 확인되었습니다.
...
### Doubly Non-Central Beta Matrix Factorization for Stable Dimensionality Reduction of Bounded Support Matrix Data (https://arxiv.org/abs/2410.18425)
Comments:
          33 pages, 18 figures

- **What's New**: 본 논문은 제한된 지원(bounded support)을 가진 행렬의 해석 가능하고 계산 효율적인 분해 방법을 개발하는 문제를 제안합니다. 이는 대규모 DNA 메틸화 연구 및 다양한 다른 맥락에서 발견됩니다.

- **Technical Details**: 저자들은 데이터 행렬을 Tucker 표현 방식으로 분해하며, 구성 요소 요인 행렬의 열 수에 대한 제약이 없습니다. Tucker 분해를 해결하기 위한 계산 효율적인 샘플링 알고리즘을 개발하였으며, 이 방법은 예측 가능성(predictability), 계산 가능성(computability), 안정성(stability)을 기준으로 성능을 평가합니다.

- **Performance Highlights**: 우리의 방법은 held-out 예측과 계산 복잡성 측면에서 다른 최첨단 접근 방식과 유사한 성능을 보였으나, 하이퍼 파라미터 변경에 대한 안정성 측면에서는 현저히 더 뛰어난 성능을 나타냈습니다. 이러한 향상된 안정성은 DNA 메틸화 분석과 같은 과학적 가설 생성을 위한 응용에서 결과에 대한 신뢰성을 높입니다.



### A Causal Graph-Enhanced Gaussian Process Regression for Modeling Engine-out NOx (https://arxiv.org/abs/2410.18424)
- **What's New**: 본 논문에서는 Gaussian process regression을 사용하여 엔진에서 배출되는 질소산화물(NOx) 예측을 위한 확률적 모델을 개발하고 검증합니다. 이는 기존의 결정론적 모델과는 달리 불확실성을 포착하는 데 중점을 두고 있습니다.

- **Technical Details**: 우리는 세 가지 변형된 Gaussian process 모델을 사용하였습니다: 첫 번째는 입력 창을 가진 표준 radial basis function 커널, 두 번째는 시계열 의존성을 포착하는 깊은 커널을 적용한 convolutional neural network, 세 번째는 그래프 합성곱 네트워크를 통해 유도된 인과 그래프로 깊은 커널을 풍부하게 하였습니다. 이 인과 그래프는 학습 과정에 물리학적 지식을 내장합니다.

- **Performance Highlights**: 모델을 가상 ECM 센서와 비교한 결과, 입력 창과 깊은 커널 구조를 사용하였을 때 예측 성능이 개선된 것으로 나타났습니다. 특히 깊은 커널에 인과 그래프를 포함시킴으로써 추가적인 성능 향상을 달성하였습니다. 이러한 결과는 다양한 검증 데이터셋을 통해 corroborated(확인됨)되었습니다.



### SkiLD: Unsupervised Skill Discovery Guided by Factor Interactions (https://arxiv.org/abs/2410.18416)
- **What's New**: 이 논문에서는 Skill Discovery from Local Dependencies (Skild)라는 새로운 기법을 제안하여, 복잡한 환경에서 다양한 상태 간 상호작용을 유도하는 기술을 학습하는 방식을 소개합니다. 기존의 방법들이 상태 다양성을 강조하는 반면, Skild는 자원 없는 보상 환경에서 agent가 보다 유용한 기술을 자동으로 발굴할 수 있도록 합니다.

- **Technical Details**: Skild는 상태 요인 분해(state factorization)를 활용하여 환경 내에서 다양한 상호작용을 유도하는 기술을 학습하는 새로운 목표를 설정합니다. 이 과정에서 agent는 능동적으로 특정 상호작용을 유도하는 방법을 발견하고, 이를 통해 향후 과제를 해결하기 위한 복합적인 기술을 학습합니다.

- **Performance Highlights**: Skild는 다양한 다운스트림 작업을 평가하는 실험에서 기존의 비지도 강화 학습 방법들과 비교하여 우수한 성능을 보였습니다. 특히, 현실적인 가정용 로봇 환경에서 명확한 의미를 지닌 기술을 성공적으로 학습하여 높은 성과를 나타냈습니다.



### Enhancing Feature-Specific Data Protection via Bayesian Coordinate Differential Privacy (https://arxiv.org/abs/2410.18404)
- **What's New**: 본 논문에서는 Local Differential Privacy (LDP)의 한계를 극복하기 위해 Bayesian Coordinate Differential Privacy (BCDP)라는 새로운 프레임워크를 제안합니다. BCDP는 각 데이터 특징(feature)별로 개인의 민감도에 맞춰 차별화된 개인 정보 보호를 가능하게 하여, 하위 작업의 성능을 향상시킵니다.

- **Technical Details**: BCDP는 Bayesian 기법을 활용하여 특정 데이터 feature에 대한 개인 정보 보호 수준을 조정합니다. 이 프레임워크는 원래 LDP의 구조를 보완하며, 각 feature별로 다른 프라이버시 수요를 반영합니다. BCDP는 특정 feature에 대한 추론 확률이 개인 기계의 결과 관찰 전후로 거의 변경되지 않도록 보장하며, 이는 각 feature별로 다양한 프라이버시 수준을 구현할 수 있도록 합니다. 이 과정에서 다양한 통계적 접근 방식과 연결됩니다.

- **Performance Highlights**: BCDP를 기반으로 한 알고리즘은 전통적인 LDP 기반 알고리즘보다 개선된 정확성을 보여줍니다. 특히 다변량 평균 추정 및 일반 최소 제곱 회귀 문제에서 BCDP 제약 조건을 더욱 강화한 결과 보안성을 유지하면서도 성능이 향상됨을 실험적으로 입증했습니다.



### Low-Rank Tensor Learning by Generalized Nonconvex Regularization (https://arxiv.org/abs/2410.18402)
- **What's New**: 본 논문에서는 관찰된 훈련 샘플이 적고 저랭크( low-rank) 구조를 가지는 텐서(tensor)를 학습하는 문제를 연구합니다. 저자들은 변환된 텐서 핵 노름( transformed tensor nuclear norm)을 기반으로 하는 비볼록(nonconvex) 모델을 제안하고, 이를 통해 저랭크를 효과적으로 탐색할 수 있는 방법론을 제공합니다.

- **Technical Details**: 제안된 모델은 텐서의 모든 전면 슬라이스의 특이값(singular values)에 비볼록 함수를 적용하여 저랭크성을 특성화합니다. 비볼록 모델은 손실 함수의 제한된 강한 볼록성 하에 기준점(stationary point)과 실제 텐서 간의 오류 경계를 설정합니다. 결과적으로, 두 개의 볼록 함수의 차이로 비볼록 함수를 재구성하고, 이를 해결하기 위해 근접 대수화-최소화( proximal majorization-minimization, PMM) 알고리즘을 설계했습니다.

- **Performance Highlights**: 제안된 방법은 텐서 완성과 이진 분류 문제에서 다른 최신 방법들에 비해 우수한 성능을 나타내며, 수치 실험을 통해 그 효과성을 입증했습니다.



### Revisiting Differentiable Structure Learning: Inconsistency of $\ell_1$ Penalty and Beyond (https://arxiv.org/abs/2410.18396)
- **What's New**: 최근의 발전에 따라, 차별화 가능한 구조 학습 방법은 방향 비순환 그래프(directed acyclic graphs, DAG)를 학습하는 조합 문제를 연속 최적화 문제로 프레임화하였습니다. 이 연구에서는 차별화 가능한 구조 학습 방법의 제한점에 대해 집중적으로 조사하며, 진정한 구조가 마르코프 동등성 클래스로 확인될 수 있는 설정에서의 문제를 다룹니다.

- **Technical Details**: 이 논문에서는 ℓ1-패널리저(penalized likelihood)의 일관성이 없음을 입증하고, 이를 해결하기 위해 하드 비순환성 제약(hard acyclicity constraint)에 기반한 ℓ0-패널리저를 사용하는 하이브리드 방법을 개발했습니다. 구체적으로, 도출된 도덕적 그래프(moral graph)를 활용하여 최적화 문제의 검색 공간을 제한하여 비선형성 문제를 완화합니다.

- **Performance Highlights**: 제안된 CALM 방법은 데이터 표준화 전후 모두에서 실험 결과로 향상된 성능을 보이며, 향후 차별화 가능한 구조 학습의 발전에 신뢰성 있는 경로를 제공합니다. 여러 실험을 통해 1-패널리저로 인한 일관성 문제를 해결하고 진정한 구조에 가까운 솔루션을 도출함을 보였습니다.



### A contrastive-learning approach for auditory attention detection (https://arxiv.org/abs/2410.18395)
- **What's New**: 이 연구에서는 Electroencephalogram (EEG) 신호를 활용하여 노이즈가 많은 환경에서의 청각적 주의(Auditory Attention Detection, AAD) 문제를 효과적으로 해결하기 위해 자가 지도 학습(self-supervised learning) 기법을 적용한 새로운 방법론을 제안합니다. 이는 제한된 데이터로 수행되는 머신 러닝 문제에서 더 강력한 해결책을 제시하는 것을 목표로 합니다.

- **Technical Details**: 우리의 방법은 자가 지도 학습을 기반으로 하여, 주의 깊게 선택한 음성 신호의 잠재 표현(latent representation)과 이에 상응하는 EEG 신호 간의 차이를 최소화하는 것을 목표로 합니다. 특히, 두 개의 CMAA(Cross-modal Attention-based Auditory Attention) 인코더를 사용하여 각 인코더의 표현 간의 유사성을 최대화하고, 분류 손실을 최소화하는 새로운 손실 함수를 설계하였습니다. EEG 신호의 전처리 과정으로는 Common Spatial Pattern (CSP)을 적용하여 서로 다른 카테고리 사이의 분산을 극대화하였습니다.

- **Performance Highlights**: 이 방법은 이전의 방법들과 비교하여 검증 세트에서 최첨단의 성능을 달성하였습니다. 특히, CNN 기반 방법과 비교할 때, 약 15%의 평균 정확도 향상이 있음을 보여주었으며, 실험 결과 자가 증강(data augmentation)과 차별화된 표현의 중요성이 입증되었습니다.



### Faster Algorithms for User-Level Private Stochastic Convex Optimization (https://arxiv.org/abs/2410.18391)
Comments:
          NeurIPS 2024

- **What's New**: 이번 논문은 사용자 수준 차별 프라이버시(user-level differential privacy, DP) 제약 하에서 비공식적인 확률적 볼록 최적화(private stochastic convex optimization, SCO)를 연구했습니다. 기존의 알고리즘들은 대규모 머신 러닝 시나리오에서 현실적이지 않으며, 본 논문에서는 이를 완화하는 새로운 알고리즘을 제시하였습니다.

- **Technical Details**: 사용자 수준 DP는 각 사용자의 전체 훈련 데이터의 포함 또는 제외가 모델 출력에 미치는 영향을 최소화하여 강력한 개인 정보 보호를 제공합니다. 본 연구에서는 특정 제약이 없는 상태에서 사용자 수준 DP의 상태-of-the-art 초과 위험(excess risk) 및 실행 시간(runtime) 보장을 제공하는 새로운 알고리즘을 개발하였습니다. 3가지 알고리즘을 제안했으며, 각각은 선형 시간 알고리즘 및 비선형 손실에 대한 최적의 초과 위험을 달성합니다.

- **Performance Highlights**: 제안된 첫 번째 알고리즘은 부드러운 가정 하에서 상태-of-the-art 초과 위험을 달성하며 선형 시간으로 작동합니다. 두 번째 알고리즘은 임의의 부드러운 손실에 대해 최적의 초과 위험을 약 (mn)^(9/8) 그래디언트 계산을 요구합니다. 세 번째 알고리즘은 비부드러운 손실에 대해 n^(11/8) m^(5/4)의 그래디언트 계산에서 최적의 초과 위험을 제공합니다.



### Harnessing PU Learning for Enhanced Cloud-based DDoS Detection: A Comparative Analysis (https://arxiv.org/abs/2410.18380)
Comments:
          9 pages, 5 figures

- **What's New**: 이 논문은 클라우드 환경에서의 분산 서비스 거부 공격(DDoS) 탐지를 향상시키기 위해 Positive-Unlabeled (PU) 학습의 적용 가능성을 탐구하고 있습니다. 연구진은 $	exttt{BCCC-cPacket-Cloud-DDoS-2024}$ 데이터셋을 활용하여 XGBoost, Random Forest, Support Vector Machine, Naïve Bayes를 포함한 네 가지 머신러닝 알고리즘으로 PU 학습을 구현했습니다.

- **Technical Details**: PU 학습은 긍정적인 인스턴스 집합과 레이블이 없는 데이터 풀을 활용하여 이상 탐지를 수행하는 반감독 머신러닝 기법입니다. 본 연구에서는 클라우드 보안의 맥락에서 PU 학습의 유효성을 조사하고, DDoS 공격 식별에 있어 다양한 머신러닝 방법의 영향을 평가하였습니다. PU 학습은 이상 트래픽을 식별하고 잠재적인 보안 위협을 효과적으로 플래그할 수 있는 모델을 구축하는 데 초점을 맞췄습니다.

- **Performance Highlights**: XGBoost와 Random Forest는 98% 이상의 F1 점수를 달성하였으며, PU 학습은 레이블이 제한된 데이터가 있는 시나리오에서 유용하다는 것을 시사합니다. 이를 통해 적법한 트래픽 증가와 사이버 공격을 구별하는 데 있어 PU 학습의 잠재력을 강조하고 있습니다.



### Delta: A Cloud-assisted Data Enrichment Framework for On-Device Continual Learning (https://arxiv.org/abs/2410.18378)
- **What's New**: 본 연구에서는 모바일 응용 프로그램에서의 데이터 부족 문제를 해결하기 위한 새로운 접근법인 Delta 프레임워크를 제안합니다. Delta는 클라우드 데이터와 디바이스 데이터를 결합하여 모델 성능을 개선합니다.

- **Technical Details**: Delta는 디바이스 측과 클라우드 측의 데이터 풍부화 문제를 분해하는 디렉토리 데이터셋을 도입하여 민감한 데이터를 공유하지 않고도 문제를 해결합니다. 또한, 희소한 사용자 데이터를 효과적으로 처리하는 소프트 데이터 매칭 전략과 클라우드 서버에 가장 적합한 데이터 샘플링 기법을 제안합니다.

- **Performance Highlights**: Delta는 시각, IMU, 오디오 및 텍스트 작업에서 각각 15.1%, 12.4%, 1.1%, 5.6% 모델 정확도를 향상시키고, 연합 학습(federated CL)과 비교하여 통신 비용을 90% 이상 감소시켰습니다.



### Multi-objective Optimization in CPU Design Space Exploration: Attention is All You Need (https://arxiv.org/abs/2410.18368)
- **What's New**: 이번 연구에서는 AttentionDSE라는 새로운 Design Space Exploration(DSE) 프레임워크를 소개합니다. AttentionDSE는 attention mechanism을 활용하여 마이크로 아키텍처 파라미터와 성능 예측 간의 직접적인 연결을 수립하여 성능 모델의 예측 정확도와 해석 가능성을 향상시킵니다.

- **Technical Details**: AttentionDSE는 디자인 변경에 효과적으로 적응할 수 있는 동적인 가중치 조정을 통해 설계 공간 내에서의 핵심 파라미터를 신속하게 식별합니다. 이는 Attention-aware Bottleneck Analysis(ABA) 알고리즘과 결합되어 병목 현상을 목적 지향적이고 신속하게 찾아내는 데 기여합니다. 이 프레임워크는 DSE를 시퀀스 예측 문제로 변환하고, Perception-Driven Attention(PDA) 메커니즘을 도입하여 관련 없는 파라미터의 계산 오버헤드를 최소화합니다.

- **Performance Highlights**: 장비 SPEC 2017 기반 실험에서 AttentionDSE는 탐색 시간을 80% 이상 단축하였고, 최신 DSE 프레임워크와 비교하여 Pareto Hypervolume(PHV)를 3.9% 향상시켰습니다. 또한, 마이크로 아키텍처 파라미터 수가 증가함에 따라 AttentionDSE는 안정적인 예측 정확도를 유지하며 훈련 및 추론 시간을 크게 늘리지 않고 효율성을 발휘합니다.



### Assessing Alcohol Use Disorder: Insights from Lifestyle, Background, and Family History with Machine Learning Techniques (https://arxiv.org/abs/2410.18354)
- **What's New**: 이번 연구는 Alcohol Use Disorder (AUD) 발생 위험에 대한 라이프스타일, 개인 배경 및 가족 이력의 영향을 분석한 첫 번째 연구로, 6,016명의 참가자 데이터를 바탕으로 한 결정 트리(Decision Trees), 랜덤 포레스트(Random Forests), 나이브 베이즈(Naive Bayes)와 같은 머신 러닝 기법을 활용하여 AUD의 주요 결정 요인을 규명했습니다.

- **Technical Details**: 이 연구는 All of Us Research Program의 설문 데이터를 사용하여 AUD 상태, 라이프스타일, 개인 배경 및 가족 이력에 대한 정보를 추출했습니다. 주로 연간 소득, 오락용 약물 사용, 거주 기간, 성별, 결혼 상태, 교육 수준 및 AUD의 가족 이력 등이 AUD의 결정 요인으로 파악되었습니다. 머신 러닝 기법을 통해 참가자의 AUD 발생 가능성을 예측하였으며, 그 중 랜덤 포레스트가 82%의 정확도를 기록했습니다.

- **Performance Highlights**: 결과적으로, 이 연구는 부모, 의료 전문가 및 교육자들이 AUD 위험을 줄이기 위한 전략을 개발하는 데 필요한 통찰력을 제공하며, 조기 개입 및 목표 예방 노력을 가능하게 합니다. 연구 결과는 AUD 예방을 위한 건강한 라이프스타일 및 환경 수정의 중요성을 강조합니다.



### Precision Soil Quality Analysis Using Transformer-based Data Fusion Strategies: A Systematic Review (https://arxiv.org/abs/2410.18353)
Comments:
          14 pages, 9 figures, 4 tables, Journal

- **What's New**: 이 리뷰는 농업 원격 감지(RS)에서의 Transformer 기반 데이터 융합 기술의 최근 발전을 탐구하며, 특히 토양 분석에 주목합니다.

- **Technical Details**: 체계적이고 데이터 기반의 접근 방식을 활용하여, Transformer's는 2022년 이후 기존의 딥 러닝(deep learning) 및 머신 러닝(machine learning) 방법보다 현저히 우수한 성능을 보였음을 입증하였습니다. 토양 수분 예측 및 토양 요소 분석을 포함한 다변량(multivariate) 토양 데이터의 정확성을 크게 향상시켰습니다.

- **Performance Highlights**: Transformer 기반 모델은 예측 성능이 92%에서 97%로 나타나며, 농업 생산성을 최적화하고 지속 가능한 농업 관행을 보장하는 데 있어 토양 상태의 중요성을 강조합니다.



### FedBaF: Federated Learning Aggregation Biased by a Foundation Mod (https://arxiv.org/abs/2410.18352)
- **What's New**: 본 논문에서는 Foundation Model을 클라이언트와 공유하지 않고도 Federated Learning (FL)에서 효과적으로 활용할 수 있는 새로운 방법인 Federated Learning Aggregation Biased by a Foundation Model (FedBaF)를 소개합니다. FedBaF는 모델 훈련 과정에서 기밀성을 유지하면서도 추가적인 훈련 성능 향상을 도모할 수 있는 혁신적인 접근 방식입니다.

- **Technical Details**: FedBaF는 전통적인 FL 방법보다 클라이언트의 데이터를 직접 액세스하지 않고, 서버 측에서 Foundation Model의 가중치를 동적으로 통합하여 FL 모델의 성능을 극대화합니다. 이 방법은 비독립적이거나 동일하게 분포되지 않은(non-IID) 데이터 환경에서도 효과적으로 모델의 수렴성을 높이며, 모델의 기밀성과 무결성을 보장합니다.

- **Performance Highlights**: 실험 결과, FedBaF는 전통적인 가중치 초기화 방식에 비해 최대 11.4% (IID 환경) 및 최대 15.8% (non-IID 환경) 향상된 테스트 정확도를 보여주었으며, Transformer 기반 언어 모델에서 perplexity를 39.2%까지 감소시켰습니다. 공격적 상황에서도 FedBaF는 19.4%에서 64.7%까지 성능을 향상시켰습니다.
...
### Augmenting Training Data with Vector-Quantized Variational Autoencoder for Classifying RF Signals (https://arxiv.org/abs/2410.18283)
Comments:
          IEEE Milcom 2024

- **What's New**: 이 논문에서는 RF 신호 분류의 성능을 향상시키기 위해 Vector-Quantized Variational Autoencoder (VQ-VAE)를 사용하는 새로운 방법론을 제시합니다. VQ-VAE를 통해 훈련 데이터의 다양성과 신뢰성을 증가시키며, 특히 저 신호 대 잡음 비율(SNR) 조건에서도 효과적인 성과를 보여줍니다.

- **Technical Details**: 제안된 방법론은 VQ-VAE 모델을 활용하여 고품질의 합성 RF 신호를 생성합니다. 이는 RF 통신 신호의 복잡한 변화를 포착하여 훈련 데이터셋의 다양성을 높입니다. 본 연구는 ResNet 기반의 분류기와 함께 훈련 중 잠재 공간에 잡음을 추가하여 성능을 극대화합니다.

- **Performance Highlights**: 실험 결과, VQ-VAE로 생성된 데이터를 포함했을 때, 기초 모델에 비해 분류 정확도가 4.06% 향상되었으며, 저 SNR(예: -10 dB) 조건에서는 15.86%의 개선을 보였습니다. 이는 신호 분류기의 일반화 및 강건성을 증대시키는 결과를 가져왔습니다.



### Hamiltonian Matching for Symplectic Neural Integrators (https://arxiv.org/abs/2410.18262)
Comments:
          NeurReps 2024

- **What's New**: 이 연구에서는 Hamilton의 운동 방정식을 기초로 한 기계 학습 알고리즘인 SympFlow를 제안합니다. SympFlow는 시간에 의존하는 Hamiltonian 함수를 정확하게 표현하는 흐름 맵의 연속적인 조합으로 구성된 신경망 기반의 신플렉틱 적분기입니다.

- **Technical Details**: SympFlow는 복잡한 Hamiltonian 시스템을 처리하기 위해 고안된 신경망 아키텍처로, 시간에 따라 조정되는 Hamiltonian을 기반으로 합니다. 이 아키텍처는 Hamiltonian 일치를 정의하는 목적 함수를 통해 네트워크를 학습합니다. 또한, SympFlow는 정확한 흐름 맵에 대한 근사치를 제공하며, 기하학적 수치적 적분의 원리를 바탕으로 설계되었습니다.

- **Performance Highlights**: Numerical 실험을 통해, SympFlow는 기존의 시간 단계 신플렉틱 적분기와 유사한 에너지 보존 행동을 나타내며, 긴 시간 동안의 시스템 발전에서도 유망한 결과를 보였습니다.
...
### Dreaming Learning (https://arxiv.org/abs/2410.18156)
Comments:
          Accepted at the NeurIPS 2024 workshop on Intrinsically Motivated Open-ended Learning

- **What's New**: 딥러닝 시스템에 새로운 정보를 통합하는 것은 여전히 도전적인 문제입니다. 본 연구에서는 Stuart Kauffman의 개념인 Adjacent Possible을 기반으로 한 훈련 알고리즘을 제안합니다. 이 알고리즘은 신경망이 통계적 특성이 다른 데이터 시퀀스를 원활하게 수용하고 통합할 수 있도록 돕습니다. 이를 통해 비정상적인 사건에 대한 신경망의 반응성을 향상시키는 데 중점을 두고 있습니다.

- **Technical Details**: 제안된 'Dreaming Learning' 알고리즘은 신경망이 기존 출력 확률 분포에서 샘플링하여 새로운 합성 시퀀스를 생성하고, 이를 통해 학습을 반복하는 방법입니다. 이 과정에서는 Gibbs sampling을 사용하여 샘플링 온도 T에 따라 출력 분포를 조절하며, 비정상적인 데이터 변화에 대응해 신경망의 학습을 최적화합니다.

- **Performance Highlights**: 실험 결과, Markov 체인에서 Paradigm shift가 발생하는 경우 자동 상관관계가 약 29% 향상되었으며, 손실 수렴 속도가 약 100% 향상되는 성능을 보여주었습니다. 이러한 성과는 비정상적인 시계열 데이터의 학습에 있어 Dreaming Learning의 우수성을 입증합니다.
...
### Efficient Adaptive Federated Optimization (https://arxiv.org/abs/2410.18117)
- **What's New**: 본 논문에서는 연합 학습(federated learning)에서의 적응형 최적화(adaptive optimization)의 중요성에 대해 논의하며, 특히 서버와 클라이언트 측 동시에 적응성이 필요한 이유를 강조합니다. 새로운 효율적인 적응형 알고리즘 $FedAda^2$를 소개합니다.

- **Technical Details**: $FedAda^2$는 대규모 교차 장치(cross-device) 연합 환경을 위해 특별히 설계되었습니다. 이 알고리즘은 서버와 클라이언트 간의 preconditioners 전송을 피하여 통신 효율성을 최적화합니다. 또한, 클라이언트 측에서 메모리 효율적인 적응형 최적화기를 활용하여 장치 메모리 소비를 줄입니다.

- **Performance Highlights**: 이론적으로, $FedAda^2$는 일반 비볼록 목적 함수에 대해 자원 집약적인 대안들과 동일한 수렴 속도를 달성합니다. 실험적으로는 이미지 및 텍스트 데이터셋에서의 joint adaptivity의 이점과 $FedAda^2$의 효과성을 보여줍니다.
...
### Tuning-free coreset Markov chain Monte Carlo (https://arxiv.org/abs/2410.18973)
- **What's New**: 이 논문에서는 Bayesian coreset(Bayesian 코어셋)의 학습률을 조정할 필요 없이 학습할 수 있는 Hot-start Distance over Gradient(Hot DoG)라는 새로운 방법을 제안합니다. 이는 이전의 Coreset MCMC 알고리듬의 한계를 극복하는 데 중점을 두고 있습니다.

- **Technical Details**: Bayesian coreset는 데이터셋의 작은 가중치 집합으로, inference(추론) 중 전체 데이터셋을 대체하여 계산 비용을 줄이는 데 사용됩니다. 기존의 Coreset MCMC 방식은 stochastic gradient optimization(확률적 경량 최적화) 기법을 사용하여 코어셋 가중치를 학습하였으나, 이는 학습률의 민감성 문제로 인해 성능 저하가 발생했습니다. Hot DoG는 이러한 학습률 조정 없이도 성능을 보장할 수 있는 새로운 최적화 절차입니다.

- **Performance Highlights**: Empirical results(경험적 결과)에서 Hot DoG는 다양한 모델 및 데이터셋에서 최적화된 ADAM에 비해 경쟁력 있는 성과를 보여주었으며, 다른 학습률 기반의 방법들에 비해 월등한 품질의 posterior approximation(후방 근사)을 제공합니다.
...
### Learning to Look: Seeking Information for Decision Making via Policy Factorization (https://arxiv.org/abs/2410.18964)
Comments:
          Project Website: this https URL

- **What's New**: 이번 연구에서는 로봇 조작 작업에서 정보 탐색 행동을 통합한 새로운 문제 유형인 분리된 Contextual Markov Decision Processes(CMDP)를 제안하고, 정보를 찾는 정책(information-seeking policy)과 그 정보를 활용하는 정책(information-receiving policy)을 기반으로 한 이중 정책 솔루션(DISaM)을 소개합니다.

- **Technical Details**: DISaM은 정보 탐색 에이전트(IS)와 정보 수신 에이전트(IR)라는 두 가지 협력적 정책을 활용하여 정보를 수집하고 조작 작업을 달성합니다. 이 두 정책은 각각의 행동 및 관찰 공간에 따라 자연스럽게 분리됩니다. DISaM은 POMDP(Partially Observable Markov Decision Processes)의 변형을 기반으로 하며, 정보 탐색과 조작 행동을 조화롭게 조정할 수 있습니다.

- **Performance Highlights**: DISaM은 5개의 조작 작업에서 정보 탐색 행동을 효과적으로 Demonstrate합니다. 시뮬레이션과 실제 환경에서 기존 방법들보다 월등한 성능을 보여주며, 특히 여러 단계에 걸쳐 정보 탐색기와 행동기 간의 전환을 효율적으로 수행합니다.
...
### A Random Matrix Theory Perspective on the Spectrum of Learned Features and Asymptotic Generalization Capabilities (https://arxiv.org/abs/2410.18938)
- **What's New**: 이 논문에서는 완전히 연결된 두 층 신경망이 단일이지만 강력한 Gradient Descent 단계 후에 어떤 방식으로 목표 함수에 적응하는지를 무작위 행렬 분석을 통해 탐구합니다. 이 연구는 훈련이 Feature Learning에 미치는 영향을 명확하고 수학적으로 설명하였으며, 일반화 오류에 대한 이론적 기초를 제시합니다.

- **Technical Details**: 제안된 모델에서는 대칭성(spiked random feature model)과 같은 경우에 대해 Feature의 경험적 공분산 행렬(feature empirical covariance matrix)에 대한 결정론적 등가(deterministic equivalent) 설명을 파생합니다. 이를 통해 훈련의 영향을 수학적으로 분석하고, Feature Spectrum의 꼬리(tails) 수정의 메커니즘을 제시합니다.

- **Performance Highlights**: 결과적으로, 이 논문은 신경망이 높은 차원 데이터로부터 저차원 Feature를 효과적으로 추출하는 방법에 대한 수학적 패러다임을 제시하며, Feature Learning이 두 층 신경망의 일반화 성능을 개선하는 방법을 설명합니다. 실험적으로도 확인된 바와 같이, 강력한 학습률(maximal learning rate)에서의 훈련이 신경망 성능에 긍정적인 영향을 미친다는 점에서 독립적인 기술적 관심을 끌고 있습니다.



### AutoStep: Locally adaptive involutive MCMC (https://arxiv.org/abs/2410.18929)
- **What's New**: 새로운 AutoStep MCMC 방법은 각 반복에서 대상 분포의 지역 기하학에 적합한 스텝 사이즈를 선택합니다. 이에 따라 복잡한 다중 스케일 목표를 처리하는 데 있어 기존의 MCMC 방법보다 더 효과적으로 동작합니다.

- **Technical Details**: AutoStep MCMC는 확률 분포 π에 대해 π-불변성과 병렬성, 불가피성 등의 성질을 보이며, 대상 분포 π와 제안 분포의 약한 가정 하에 작동합니다. 이 방법은 국지적으로 적응적인 스텝 사이즈 선택을 통해 기존의 MCMC 방법의 한계를 극복합니다.

- **Performance Highlights**: 실험 결과에 따르면, AutoStep MCMC는 다양한 도전적인 대상 분포에서 단위 비용당 효과적인 샘플 크기 측면에서 최첨단 방법과 경쟁력이 있습니다. 이는 제안된 스텝 사이즈 선택 설계의 효과를 잘 보여주는 결과입니다.



### Learning $k$-body Hamiltonians via compressed sensing (https://arxiv.org/abs/2410.18928)
Comments:
          30+12 pages, 1 figure

- **What's New**: 이번 연구에서는 $M$개의 미지의 Pauli 항을 포함한 $k$-체계 Hamiltonian을 학습하는 문제를 다룹니다. 새로운 프로토콜을 통해 비례 정확도 $oldsymbol{	ext{epsilon}}$로 Hamiltonian을 학습할 수 있으며, 총 진화 시간은 ${oldsymbol{	ext{O}}}(M^{1/2+1/p}/oldsymbol{	ext{epsilon}})$로 표현됩니다. 이 연구는 또한 기존 연구와 달리 기하학적 지역성과 완화된 지역성이 필요하지 않음을 강조합니다.

- **Technical Details**: 제안된 학습 프로토콜은 단일 큐비트 제어 작업 및 GHZ 상태 초기 상태를 사용하고, 비적응형이며, SPAM(errors) 오차에 강인하고, $M$과 $k$를 사전에 정확히 알지 못하더라도 잘 작동합니다. 또한, 논문에서는 컴프레스드 센싱(compressed sensing) 이론을 적용하여 Hamiltonian 내의 $M$ 항들을 효율적으로 식별하는 방법을 제공합니다. 이론적으로, $oldsymbol{	ext{l}}^1$ 및 $oldsymbol{	ext{l}}^2$ 오류 메트릭의 운영적 해석을 논의합니다.

- **Performance Highlights**: 새로운 프로토콜은 단일 양자 제어를 통해 효과적으로 Hamiltonian을 학습하며, chính xác한 상호작용에 대한 이해가 필요한 양자 시뮬레이션 및 양자 오류 수정 등의 과제를 향상시킵니다. 더불어 기하학적 지역성 요구 조건 없이도 Hamiltonian을 성공적으로 학습할 수 있는 가능성을 보여줍니다.



### Optimizing Edge Offloading Decisions for Object Detection (https://arxiv.org/abs/2410.18919)
Comments:
          SEC 2024

- **What's New**: 최근 기계 학습과 하드웨어의 발전으로 실시간 객체 탐지가 가능한 내장 장치가 등장하였습니다. 논문에서는 내장 장치가 지역적인 감지가 부족할 때 더 강력한 edge 서버에 탐지를 오프로드(Offload)하는 옵션을 고려하고 있으며, 이를 위해 이미지를 효율적으로 선택하는 방법을 제안합니다.

- **Technical Details**: 제안된 새로운 보상 메트릭인 ORIC(Object detection Reward per Image within Context)는 오프로드한 개별 이미지에서 발생하는 탐지 정확도의 향상을 정량화합니다. ORIC는 약한 모델의 결과만을 사용하여 예측할 수 있는 효율적인 방법으로 내장 장치에서 실행 가능하도록 설계되어 있습니다.

- **Performance Highlights**: ORIC은 기존 방법들에 비해 탐지 정확도 향상에서 효과적이며, 오프로드되는 이미지의 비율이 적을 때에도 성능을 높이는 것으로 나타났습니다. 여러 객체 탐지 모델과 데이터셋을 사용하여 평가 하였으며 결과는 공개된 데이터셋과 실제 edge 컴퓨팅 테스트베드를 통해 검증되었습니다.



### MissNODAG: Differentiable Cyclic Causal Graph Learning from Incomplete Data (https://arxiv.org/abs/2410.18918)
- **What's New**: 제안된 MissNODAG는 부분적으로 관측된 데이터에서 사이클(cycle) 원인 그래프(causal graph)와 결손 메커니즘(missingness mechanism)을 학습하기 위한 차별화된(differentiable) 프레임워크입니다. 이는 표준 알고리즘이 해결하기 어려운 피드백 루프(feedback loops)와 불완전한 데이터 문제를 해결합니다.

- **Technical Details**: MissNODAG는 추가 노이즈 모델(additive noise model)과 기댓값 최대화(expectation-maximization, EM) 절차를 통합하여 결측값을 보간(imputing)하고 관측된 데이터 우도(likelihood)를 최적화(optimizing)하는 방식을 채택합니다. 이 방법은 선형(structural equation models) 및 비선형(non-linear) 모델 모두에 적용 가능합니다.

- **Performance Highlights**: MissNODAG는 합성 실험(synthetic experiments) 및 실제 유전자 교란 데이터(real-world gene perturbation data) 애플리케이션을 통해 검증되었으며, 부분적으로 결측된 간섭(interventional) 데이터에서 최첨단(imputation techniques) 기법과의 결합으로 성능 향상을 입증했습니다.



### Testing Support Size More Efficiently Than Learning Histograms (https://arxiv.org/abs/2410.18915)
Comments:
          40 pages

- **What's New**: 이 논문에서는 알려진 확률 분포 p에 대해 두 가지 문제를 다루며, 샘플 개수와 관련하여 더 효율적인 알고리즘을 제시합니다. 특히, 지원 크기가 n 이하인지 여부를 판단하기 위한 샘플 수를 감소시키는 방법을 소개합니다.

- **Technical Details**: 첫 번째 문제는 확률 분포 p의 지원이 n 개의 요소에 한정되는지, 또는 ε-멀리 떨어져 있는지를 검증하는 것입니다. 이를 위해 O(n/(ε log n) log(1/ε))의 샘플만으로 테스트를 수행할 수 있으며, 두 번째 문제에서는 이전 작업보다 더 큰 하한을 제공하면서 지원 크기에 대한 더 나은 솔루션을 제시합니다. 이 논문에서는 Chebyshev 다항식 근사를 사용하여 이론을 뒷받침합니다.

- **Performance Highlights**: 비교적 효율적인 알고리즘을 제공하며, 지원 크기를 결정짓는 문제와 관련하여 기존 연구보다 나은 성능을 보여줍니다. 특히, 필요한 샘플 수를 줄이는 데 성공하여, ε-멀리 떨어져 있는 분포에 대해 더욱 정확한 예측이 가능합니다.
...
### Modulated Adaptive Fourier Neural Operators for Temporal Interpolation of Weather Forecasts (https://arxiv.org/abs/2410.18904)
Comments:
          8 pages, 5 figures

- **What's New**: 기후와 날씨 데이터의 제한된 시간 해상도를 개선하기 위한 새로운 보간(interpolation) 모델을 소개합니다. 이 모델은 적응형 푸리에 신경 연산자(Adaptive Fourier Neural Operator, AFNO)를 수정한 모듈레이션 모듈(MODAFNO)을 기반으로 하며, 각 시간 단계 사이의 대기 상태를 재구성합니다.

- **Technical Details**: 모델은 2개의 시간 지점 사이의 대기 상태를 예측하는 문제로 정의됩니다. ModAFNO 계층은 시간에 따른 변화를 반영할 수 있는 학습된 스케일-시프트(scale-shift) 연산을 적용하여 입력 데이터에서 1시간 해상도의 중간 시간을 생성합니다. 이를 통해 6시간 간격에서 얻은 입력 데이터를 바탕으로 거의 indistinguishable한 1시간 해상도의 결과를 도출할 수 있습니다.

- **Performance Highlights**: ModAFNO 기반 보간 모델은 중간 단계 재구성을 위한 RMSE 손실을 약 50% 감소시키며, 허리케인과 열파와 같은 극단적인 날씨 이벤트의 통계를 6시간 해상도 데이터보다 뛰어나게 재현하는 능력을 보여줍니다.
...
### Exploring the Universe with SNAD: Anomaly Detection in Astronomy (https://arxiv.org/abs/2410.18875)
Comments:
          14 pages, 4 figures

- **What's New**: SNAD 프로젝트는 대규모 천문 조사에서의 천문학적 비정상 탐지를 위해 활성 학습(active learning)과 기타 머신 러닝(machine learning) 알고리즘을 사용하는 데 중점을 두고 있습니다. 이 논문은 SNAD 프로젝트의 개요와 여러 해에 걸친 전진과 성과를 요약합니다.

- **Technical Details**: SNAD 프로젝트의 주요 목표는 인간의 전문성과 현대 머신 러닝 기술을 결합하여 주로 광도(photometrical) 특징을 통해 비정상 천문학적 물체를 식별하는 것입니다. 이를 위해 팀은 여러 AL 및 AD 알고리즘을 결합하여 전문가의 추가 정보를 이용해 비정상 물체를 라벨링합니다. 데이터 세트는 Open Supernova Catalog과 Zwicky Transient Facility를 포함하며, Isolation Forest, Local Outlier Factor, Gaussian Mixture Models 및 one-class Support Vector Machines와 같은 다양한 비정상 탐지 알고리즘을 실험하였습니다.

- **Performance Highlights**: SNAD 팀은 5년 동안 천문학적 비정상 탐지에 집중하며, 여러 혁신적인 알고리즘과 기술을 개발해왔습니다. 초기 데이터 세트에서 1999개의 광도 곡선(light curves)을 활용하여 여러 특이 및 초고휘도 초신성(supernova)을 발견했고, ZTF 조사 결과에서는 225만 건의 물체 중에서 비정상 물체 탐색을 진행하여 고무적인 결과를 얻었습니다.
...
### Omics-driven hybrid dynamic modeling of bioprocesses with uncertainty estimation (https://arxiv.org/abs/2410.18864)
- **What's New**: 이 연구는 다중 스케일 생물학 시스템의 동적 모델링을 촉진하는 머신 러닝 도구들을 통합한 'omics-driven modeling pipeline'을 제안합니다. 특히, 랜덤 포레스트(random forests)와 순열 특성 중요성을 사용하여 omics 데이터셋에서 주요 특징을 선별하고, 이를 기반으로 하이브리드 모델을 구축합니다.

- **Technical Details**: 이 프레임워크는 고차원 단백질체 데이터셋을 사용하여 핵심 세포 내 단백질을 식별하고, 가우시안 프로세스(Gaussian processes)를 통해 동적 모델의 주요 매개변수를 추정합니다. 또한, 랜덤 포레스트를 통해 차원 축소를 수행하고, 이후 심층 신경망(deep neural networks)이나 중간 데이터셋의 경우 가우시안 프로세스를 사용하여 연속적이고 미분 가능한 함수에 대한 훈련을 수행합니다.

- **Performance Highlights**: 이 모델은 다양한 단백질 체 프로필 하에서 효모 계통의 동적 행동을 포착하며, 하이브리드 모델의 예측 불확실성을 추정할 수 있는 가능성을 보여줍니다. 제안된 프레임워크는 다른 omics 데이터 통합에도 적용 가능하며, 대규모 데이터셋 처리를 위한 대체 머신 러닝 방식에 대해서도 활용 가능합니다.



### Bilinear Sequence Regression: A Model for Learning from Long Sequences of High-dimensional Tokens (https://arxiv.org/abs/2410.18858)
- **What's New**: 본 논문에서는 Bilinear Sequence Regression (BSR) 모델이라는 새로운 분석 가능 모델을 도입하고, 긴 고차원 시퀀스 데이터로부터의 감독 학습(supervised learning)에 대한 연구를 시작합니다. 이는 현대 AI에서 중요한 기초 모델로 자리잡을 수 있을 것입니다.

- **Technical Details**: BSR 모델은 고차원 벡터인 토큰으로 이루어진 긴 시퀀스를 처리하며, 통계 물리학(statistical physics) 기법을 통해 모델의 베이즈 최적 일반화 오류(Bayes-optimal generalization error)를 계산합니다. 이 연구는 전통적인 방법론으로는 파악하기 힘든 심화된 특성들을 제공할 수 있습니다.

- **Performance Highlights**: BSR 모델을 통해 최적 학습(optimal learning)과 단순 선형 회귀(linear regression) 기반 학습을 비교하면서 시퀀스 토큰을 벡터화해 학습하는 것의 이점을 수량화합니다. 또한 BSR 모델 내의 경량화된 경량하강(optimal shortcut) 알고리즘의 성능 또한 주목할 만합니다.
...
### High-dimensional Analysis of Knowledge Distillation: Weak-to-Strong Generalization and Scaling Laws (https://arxiv.org/abs/2410.18837)
- **What's New**: 본 연구는 knowledge distillation 과정의 정확한 특성을 ridgeless 고차원 회귀 상황에서 모델 변화(model shift)와 분포 변화(distribution shift) 두 가지 조건 하에 규명합니다.

- **Technical Details**: 이 논문에서는 대상 모델(target model)의 위험(risk)을 샘플 크기와 데이터 분포에 따라 비 비대칭적(non-asymptotic) 경계로 특성화하고, 최적의 대리 모델(optimal surrogate model) 형태를 식별합니다. 실험은 두 단계의 경험적 위험 최소화(empirical risk minimization, ERM)를 통해 수행됩니다.

- **Performance Highlights**: 실험 결과, 약한 레이블을 사용한 W2S(weak-to-strong) 훈련이 동일한 데이터 예산 하에서 강한 레이블을 사용하는 훈련을 초과할 수 있으나, 데이터 스케일링 법칙(data scaling law)에 개선을 가져오지 못함을 확인했습니다.
...
### Language-Agnostic Modeling of Source Reliability on Wikipedia (https://arxiv.org/abs/2410.18803)
- **What's New**: 최근 몇 년간 내용 검증(reliable content verification)이 허위정보(disinformation)와의 전투를 위해 필수적으로 요구되고 있습니다. 이 논문에서는 여러 언어 버전의 Wikipedia에서 출처의 신뢰성을 평가하기 위해 설계된 언어 독립적인 모델(language-agnostic model)을 제안합니다.

- **Technical Details**: 이 모델은 Wikipedia의 다양한 주제(예: Climate Change, COVID-19, History 등)에 대해 출처의 신뢰성을 평가하며, 편집 활동 데이터(edit activity data)를 활용합니다. 모델은 영어 및 고자원 언어(high-resource languages)에서 약 0.80의 F1 Macro 점수를 달성했으며, 중간 자원(mid-resource languages)에서는 0.65를 기록했습니다. 모델은 도메인 사용의 영속성(permanence)이 가장 예측 가능한 특징 중 하나임을 보여줍니다.

- **Performance Highlights**: 이 작업은 다양한 언어 커뮤니티에서 사용자 생성 콘텐츠의 신뢰성을 보장하는 데 기여합니다. 또한, 고자원 언어의 모델을 적응시켜 성능을 향상시킬 수 있음을 입증했습니다.
...
### Adapting MLOps for Diverse In-Network Intelligence in 6G Era: Challenges and Solutions (https://arxiv.org/abs/2410.18793)
Comments:
          7 pages, 5 figures. This paper has been submitted to IEEE for possible publication

- **What's New**: 본 논문은 6G 네트워크에서 인공지능(AI) 및 머신 러닝(ML) 기술을 무선 시스템에 원활하게 통합하는 방법을 제시합니다. 특히, 다양한 학습 패러다임과 네트워크 이질성의 문제를 해결하기 위해 강화 학습 운영(RLOps), 연합 학습 운영(FedOps), 생성 AI 운영(GenOps)이라는 세 가지 운영 파이프라인을 정의하여 AI-네이티브 6G 네트워크의 대규모 배치를 촉진합니다.

- **Technical Details**: ML 운영(MLOps)은 데이터 엔지니어링, AI/ML 모델, CI/CD 파이프라인의 원칙에 기반하여 AI/ML 모델의 생애 주기를 관리하는 방법론입니다. 본 논문에서는 RAN(무선 접속 네트워크) 수준에서 MLOps를 구현하여, RLOps는 Open RAN에서의 지능형 관리 및 오케스트레이션을, FedOps는 대규모 IoT 장치 및 엣지 인텔리전스를 위한 솔루션을 제공하며, GenOps는 네트워크에서 생성 모델을 통합하는 프로세스를 다룹니다.

- **Performance Highlights**: 이 연구는 다가오는 6G 네트워크에서 AI/ML 모델의 효율적인 관리와 배치를 위한 새로운 접근 방식을 제시하며, 그에 따른 성능 개선과 서비스 품질 향상을 기대합니다.



### Fully Stochastic Primal-dual Gradient Algorithm for Non-convex Optimization on Random Graphs (https://arxiv.org/abs/2410.18774)
Comments:
          15 pages, 7 figures

- **What's New**: 본 논문에서는 비동기식 분산 최적화 문제를 해결하기 위한 첫 번째 Fully Stochastic Primal Dual Gradient Algorithm (FSPDA)를 제안합니다. 이 알고리즘은 연속적인 정적 그래프 및 동기적 업데이트에 의존하지 않고도 수렴하는 특성을 지니고 있습니다.

- **Technical Details**: FSPDA는 무작위 무방향 그래프에서 비차단(non-blocking) 통신 및 지역적(local) 확률적(stochastic) 그래디언트 업데이트를 통해 비동기적 분산 절차를 가능하게 합니다. 이 알고리즘은 여러 지역적 그래디언트 스텝을 허용하여 정적(stationarity)에 대한 수렴 속도를 가속화하고, 확률적(primal-dual) 업데이트를 통해 합의된(consensual) 해를 찾습니다.

- **Performance Highlights**: FSPDA는 평활(smooth)하고 비볼록(non-convex) 목적 함수에 대해 $	ext{O}(	ext{σ}/	ext{√{nT}})$-정상 솔루션으로 수렴함을 보이며, 데이터 이질성(data heterogeneity)을 가정하지 않습니다. 실험 결과, FSPDA는 기존의 최신(state-of-the-art) 알고리즘과 비슷한 성능을 보입니다.
...
### Remote Detection of Applications for Improved Beam Tracking in mmWave/sub-THz 5G/6G Systems (https://arxiv.org/abs/2410.18637)
- **What's New**: 이번 연구에서는 5G/6G 시스템에서의 비음파(beam) 추적 기능의 최적화를 위한 새로운 방법을 제안했습니다. 특히, 수신 신호 강도 패턴을 기반으로 사용자 장치에 현재 사용 중인 애플리케이션을 원격으로 감지할 수 있는 기술을 개발했습니다.

- **Technical Details**: 연구팀은 서브-THz 대역인 156 GHz에서 다단계 측정 캠페인을 실시하여 비디오 통화, 동영상 시청, VR 시청 및 게임과 같은 인기 스마트폰 애플리케이션에서의 수신 신호 강도 추적치를 수집했습니다. 이를 통해 Mann-Whitney 검정 및 다양한 머신러닝(ML) 분류 기법을 적용하여 애플리케이션을 원격으로 분류하는 방법을 제안했습니다.

- **Performance Highlights**: Mann-Whitney 검정은 애플리케이션 클래스를 0.95의 신뢰도로 구별할 수 있으며, 애플리케이션 초기화 후 약 1초 이내에 클래스 감지 지연을 유도했습니다. 랜덤 포레스트 분류기는 수신 신호 강도 메트릭만으로 80% 이상의 정확도로 빠른 및 느린 미세 이동성을 가진 애플리케이션을 구분할 수 있었습니다. 특정 애플리케이션 감지의 정확도는 60%에 불과했습니다.
...
### Evolutionary Dispersal of Ecological Species via Multi-Agent Deep Reinforcement Learning (https://arxiv.org/abs/2410.18621)
- **What's New**: 본 연구에서는 생태계 내에서 종의 동태를 이해하기 위해 starvation-driven diffusion (SDD) 모델을 활용하여 자원 조건에 따른 종의 분산을 설명하고, multi-agent reinforcement learning (MARL)과 deep Q-networks (DQN)을 적용하여 단일 종 및 포식자-피식자 상호 작용을 시뮬레이션합니다. 또한, SDD 기반의 보상을 포함하여 종의 분산 메커니즘에 대한 통찰력을 제공합니다.

- **Technical Details**: SDD 모델은 비선형 확산을 통해 생물학적 종의 분산을 설명합니다. 이 모델은 자원의 상대적인 풍부함과 부족함을 기반으로 하여 종의 이동 방식을 조정합니다. MARL 접근법은 대규모 에이전트를 통해 포식자-피식자 관계와 같은 복잡한 상호 작용을 효율적으로 모델링합니다. 공학적으로는 DQN 알고리즘이 사용되어 에이전트의 정책 학습을 지원하고 있습니다.

- **Performance Highlights**: 시뮬레이션 결과는 MARL이 제공하는 진화적 분산 전략이 SDD 기반 이론과 일치함을 보여줍니다. 포식자의 생존 가능성이 증가하며, SDD가 생태적 조건에서 종의 적합성을 향상시키는 것을 확인할 수 있었습니다. 이러한 결과는 전통적인 수학 모델과의 비교를 통해 생태계 내 종의 진화적 분산에 대한 중요한 통찰력을 제공합니다.
...
### Optimal Equivariant Architectures from the Symmetries of Matrix-Element Likelihoods (https://arxiv.org/abs/2410.18553)
Comments:
          31 pages, 6 figures, 3 tables

- **What's New**: 이번 논문에서는 고에너지 물리학 데이터 분석에 Matrix-Element Method (MEM)를 새로운 방식으로 적용하여 대칭성(equivariant) 심층 신경망 아키텍처를 제안합니다. 이 두 가지 접근법의 결합으로 더 효과적인 사건 분류가 가능해지는 점이 강조됩니다.

- **Technical Details**: 저자들은 롤렌츠 불변성과 순열 불변성을 포함한 긴축 부스트-대칭 메시지 전달 신경망 아키텍처를 제안하고, MEM의 이론적 지식을 활용하여 최적의 대칭성을 네트워크 구조에 통합합니다. 이 연구는 Neyman-Pearson 레마를 이용하여 분류 문제에 적합한 대칭군(symmetry group) 선택을 탐구합니다.

- **Performance Highlights**: MEM에서 영감을 받은 아키텍처는 네 개의 바텀 쿼크로 다이-힉스(decay) 사건을 QCD 배경으로부터 구별하는 데 있어 최첨단 성능을 달성하였으며, 이는 샘플 및 파라미터 효율성을 크게 향상시킵니다.
...
### Assured Automatic Programming via Large Language Models (https://arxiv.org/abs/2410.18494)
- **What's New**: AI 기반 코딩 엔진을 활용하여 자연어 요구사항을 실행 가능한 코드로 변환하는 것이 가능해졌지만, 생성된 코드는 신뢰성이 떨어질 수 있습니다. 본 연구에서는 프로그래머의 의도를 발견하고 의도에 부합하는 코드를 생성하여 이를 검증하는 방법을 제시합니다.

- **Technical Details**: 본 연구에서 제시한 접근 방식인 'program-proof co-evolution'은 LLM(대규모 언어 모델)에서 생성된 코드, 논리적 명세(logical specification), 테스트를 포함하는 튜플을 수리(repair)하는 데 초점을 맞추고 있습니다. 코드, 명세, 테스트 간의 일관성을 점진적으로 개선하여 사용자의 의도를 명확히 하고, 최종적으로 이는 자연어로 번역되어 개발자가 검토할 수 있도록 합니다.

- **Performance Highlights**: 본 연구의 접근 방식으로 발견된 명확한 의도는 Dafny 프로그래밍 언어의 데이터셋에서 verifiable(검증 가능한) 자동 생성 프로그램 비율을 증가시키는 데 기여함을 보여줍니다.



### Evolving Voices Based on Temporal Poisson Factorisation (https://arxiv.org/abs/2410.18486)
Comments:
          main paper: 19 pages (2 single figures, 3 double figures, 3 tables) appendix: 9 pages (3 quadruple figures, 1 table) references: 3 pages

- **What's New**: 이 논문에서는 30년 이상의 정치 연설 데이터를 분석하기 위해 시간과 주제를 함께 변화하는 새로운 모델인 Temporal Poisson Factorisation (TPF) 모델을 제안합니다.

- **Technical Details**: TPF 모델은 Poisson factorisation 모델을 확장하여 시간에 따른 변화가 있는 문서-용어 행렬(DTM)을 생성합니다. 이 모델은 auto-regressive (자기 회귀) 구조와 random walk (무작위 보행) 파라미터화를 포함하여 시간에 따른 의존성을 모델링합니다. 변분 추정을 통한 계량적 추정이 이루어지며, 문서 배치의 자동 미분을 사용합니다.

- **Performance Highlights**: TPF 모델은 미국 상원에서의 36년간의 정치 담론에 적용되어, auto-regressive 모델과 random walk의 성능을 비교하고, 독립적인 단변량 vs. 다변량 변분 분포의 효과를 분석하며, 주제의 변화와 그 유행의 변화를 효과적으로 모니터링할 수 있음을 시사합니다.
...
### Structure Language Models for Protein Conformation Generation (https://arxiv.org/abs/2410.18403)
Comments:
          Preprint. Under Review

- **What's New**: 본 논문에서는 Structure Language Modeling (SLM)을 소개하며, 이는 단백질 구조를 효율적으로 생성하는 새로운 프레임워크입니다. 기존의 방법들이 확산 과정에 의존했던 것에 반해, SLM은 단백질 구조를 잠재 공간(latent space)에 인코딩하고 조건부 언어 모델링을 적용하여 다양한 구성을 탐색합니다.

- **Technical Details**: SLM은 이산 변분 오토인코더를 사용하여 단백질 구조를 간결한 잠재 공간으로 인코딩하며, 후속 조건부 언어 모델링에서는 아미노산 유형을 맥락으로 사용해 시퀀스 특이적 구조 분포를 찾습니다. 제안된 모델 ESMDiff는 ESM3에서 마스킹된 확산 방식으로 미세 조정된 구조 언어 모델입니다.

- **Performance Highlights**: SLM은 다양한 구성 생성을 위해 여러 시나리오에서 검증되었으며, 기존 방법보다 20-100배 빠른 속도를 달성했습니다. 이러한 성과는 향후 단백질 구성 샘플링 문제를 해결하는 새로운 연구 길을 여는 기회를 제공합니다.
...
### Unified Microphone Conversion: Many-to-Many Device Mapping via Feature-wise Linear Modulation (https://arxiv.org/abs/2410.18322)
Comments:
          Currently under review for ICASSP 2025

- **What's New**: 본 연구에서는 Unified Microphone Conversion을 소개하며, 이는 기기 변동성에 대비한 사운드 이벤트 분류(sound event classification) 시스템의 회복력을 향상시키기 위한 통합 생성 프레임워크입니다. 이전 연구의 한계를 극복하기 위해, 우리는 주파수 응답 정보를 사용하여 생성기 네트워크를 조건화하여 기기 간 매핑을 구현합니다.

- **Technical Details**: 이 접근법은 각 기기 쌍에 대해 별도의 모델을 요구하는 CycleGAN의 한계를 극복했습니다. 우리 프레임워크는 CycleGAN의 비숙련(training)을 활용하여 오디오 녹음에서 기기 특성을 시뮬레이션하고, Frequency Response 관련 정보를 Feature-wise Linear Modulation(FiLM)을 통해 통합하여 확장성을 크게 향상시킵니다.

- **Performance Highlights**: 실험 결과, 우리의 방법이 기존 최고 수준의 방법보다 2.6% 더 우수하며, 매크로 평균 F1 스코어에서 변동성을 0.8% 감소시켰습니다.
...
### Stabilizing black-box model selection with the inflated argmax (https://arxiv.org/abs/2410.18268)
- **What's New**: 이 논문은 데이터에 따라 후보 모델 집합에서 모델을 선택하는 프로세스를 매우 안정적으로 만드는 새로운 접근 방식을 제시합니다. 기존의 LASSO 및 SINDy와 같은 방법들이 주어진 데이터의 작은 변화에 의해 매우 불안정하다는 문제점을 해결하고자 하였습니다.

- **Technical Details**: 제안한 방법은 bagging과 'inflated' argmax 연산을 결합하여 모델 선택 프로세스를 안정화합니다. 이 방식은 데이터에 잘 맞는 소수의 모델을 선택하며, 학습 점을 제거하더라도 초기 선택된 모델 집합과 겹치는 특성을 보입니다. 이론적 보장을 개발하고, 강한 상관관계를 가진 변수들이 포함된 시뮬레이션과 Lotka-Volterra 모델 문제에 적용하여 방법의 효과를 입증하였습니다.

- **Performance Highlights**: 제안한 방법은 두 가지 실험 환경 모두에서 안정적이고 콤팩트한 모델 컬렉션을 제공합니다. 기존의 다양한 벤치마크를 초월하여 성능을 향상시키는 결과를 보였습니다.
...
### Assessment of Developmental Dysgraphia Utilising a Display Tab (https://arxiv.org/abs/2410.18230)
Comments:
          16 pages

- **What's New**: 본 논문은 전통적인 방법 대신, 화면 디스플레이 태블릿을 통한 온라인 필기(processing handwriting)에 대한 발전을 제시합니다. 이 방법은 아이들이 스스로 사용할 수 있는 접근성을 개선합니다.

- **Technical Details**: 144명의 초등학교 3, 4학년 아동을 대상으로 Handwriting Proficiency Screening Questionnaires for Children (HPSQ C)를 사용하여 필기 능력을 평가했습니다. 머신러닝 모델 중 gradient-boosting 알고리즘을 사용하여 DD 진단의 정확도를 83.6%까지 향상시켰습니다. 기계 학습 기반의 분석이 DD 증상 분석에 필수적임을 강조합니다.

- **Performance Highlights**: 이 연구는 DD 아동이 공중에서 필기를 할 때 평균적으로 더 긴 시간과 더 많은 펜 높이와 같은 특성을 보임을 발견했습니다. 전체 HPSQ C 점수의 최소 오차는 10.34%로 평가되었습니다. 이는 DD 진단을 위한 유망한 접근법을 나타냅니다.
...
### Melody Construction for Persian lyrics using LSTM recurrent neural networks (https://arxiv.org/abs/2410.18203)
- **What's New**: 이 논문에서는 페르시아 가사를 입력으로 하는 자동 멜로디 작성을 연구했습니다. 가사 음절과 멜로디 간의 음운론적 상관관계가 존재한다고 가정하고, seq2seq(neural sequence-to-sequence) 신경망을 개발하여 페르시아 노래에서 병렬 음절과 음표 시퀀스에 대한 훈련을 통해 새로운 음절 시퀀스에 대해 즐거운 멜로디를 제안합니다.

- **Technical Details**: 본 연구의 방법은 기계 번역(machine translation) 방법을 기반으로 하며, 가사와 음악을 두 가지 언어로 간주합니다. LSTM(Long Short-Term Memory) 셀을 사용하는 시퀀스-투-시퀀스 모델을 통해 멜로디를 생성합니다. 주된 구조는 임베딩(embedding), 인코딩(encoding), 디코딩(decoding) 및 어텐션(attention) 레이어로 구성되어 있습니다.

- **Performance Highlights**: 훈련된 모델의 멜로디는 5점 만점에 3.005점을 기록하였으며, 동일 가사에 대해 사람에 의해 제작된 멜로디는 평균 4.078점을 기록했습니다. 170명 이상의 응답자를 대상으로 한 오디오 설문조사를 통해 평가가 진행되었습니다.
...
### Stochastic gradient descent in high dimensions for multi-spiked tensor PCA (https://arxiv.org/abs/2410.18162)
Comments:
          58 pages, 10 figures. This is part of our manuscript arXiv:2408.06401

- **What's New**: 본 논문은 다중 스파이크 텐서 모델에 대한 온라인 확률적 경량화 기법인 SGD의 고차원 동역학을 연구한 결과를 제시합니다. 다중 텐서 주성분 분석(PCA) 문제에서 발생하는 이 모델은 알려지지 않은 신호 벡터를 효과적으로 복구하기 위한 샘플 수와 신호 대 잡음 비율(SNR)을 결정하는 데 초점을 맞추고 있습니다.

- **Technical Details**: 연구 결과에 따르면, 다중 스파이크는 단계적 제거(sequential elimination)를 통해 복구되며, 이는 상관관계가 임계 값을 초과할 때 발생합니다. 모든 스파이크를 완전히 복구하기 위해 필요한 샘플 수는 $N^{p-2}$에 비례하며, 이 결과는 1계급(rank-one) 사례에서 발견된 알고리즘적 임계값과 일치합니다. 또한, 고차원의 동적 시스템을 저차원으로 분석하여 알고리즘의 경로를 이해할 수 있습니다.

- **Performance Highlights**: 본 연구는 SNR이 충분히 분리된 경우 모든 스파이크를 정확히 복구할 수 있음을 보여줍니다. 반면, 동일한 SNR의 경우 스파이크가 구성하는 하위 공간이 복구됩니다.
...
### Music102: An $D_{12}$-equivariant transformer for chord progression accompanimen (https://arxiv.org/abs/2410.18151)
Comments:
          10 pages, 3 figures

- **What's New**: 이번 연구에서는 Music101 프로토타입을 기반으로 한 고급 모델 Music102를 제안합니다. D12-equivariant transformer를 통해 코드 진행을 개선하는 데 중점을 두었습니다. 음악의 대칭성을 활용하여 멜로디와 코드 시퀀스에서의 동등성을 유지하는 방법을 보여줍니다.

- **Technical Details**: Music102는 전통적인 음악 이론과 그룹 이론에 기초하여 설계되었습니다. 이 모델은 트랜스포머 아키텍처를 이용하여 음악의 대칭성(예: 전이 및 반사 작업)을 통합하며, POP909 데이터셋을 사용하여 학습을 진행했습니다. Music102는 이전의 Music101보다 더 적은 파라미터로도 더 나은 성능을 보였습니다.

- **Performance Highlights**: 평가 결과, Music102는 가중 손실(weighted loss) 및 정확도(exact accuracy) 측면에서 Music101에 비해 유의미한 개선을 보였습니다. 이는 자기 주의 메커니즘과 층 정규화가 음악의 이해 및 생성에서 유연하고 효과적으로 작용할 수 있음을 나타냅니다.



### Using Platt's scaling for calibration after undersampling -- limitations and how to address them (https://arxiv.org/abs/2410.18144)
- **What's New**: 이 연구는 Platt의 스케일링(Platt's scaling)을 언더샘플링(undersampling) 이후 모델 보정(calibration)으로 사용하는 것의 유효성에 대한 첫 번째 상세한 연구 결과를 제시합니다.

- **Technical Details**: 기존의 언더샘플링 방식에서는 다수 클래스의 일부를 유지함으로써 신뢰도 높은 훈련 데이터셋을 만든 후, 보정 메서드로는 Platt's scaling을 활용합니다. 본 연구에서는 언더샘플링 후 Platt의 스케일링을 사용할 때 요구되는 전제 조건을 수학적으로 분석하고 시뮬레이션 연구 및 사례 연구를 통해 검토하였습니다. 제안된 수정된 Platt's scaling는 일반화가속모델(logistic generalized additive model)을 기반으로 하여, 모델의 예측값에 로그리트를 적용하여 훈련됩니다.

- **Performance Highlights**: 연구 결과, Platt의 스케일링 사용 시 언더샘플링을 고려할 때 신중하게 접근해야 함을 강조하였고, 수정된 Platt의 스케일링이 검토된 모든 설정에서 우수한 성능을 보였습니다.
...
### Generative Design of Functional Metal Complexes Utilizing the Internal Knowledge of Large Language Models (https://arxiv.org/abs/2410.18136)
- **What's New**: 이번 논문은 지도 학습(Fine-tuning) 없이 대규모 언어 모델(Large Language Models, LLMs)을 활용하여 전이 금속 복합체(Transition Metal Complexes, TMCs)의 진화 최적화 과정에 통합한 LLM-EO(LLM-Evolutionary Optimization) 방법론을 제안합니다.

- **Technical Details**: LLM-EO는 단일 목표 및 다중 목표 최적화를 위해 적용되며, LLM의 사전 훈련(pretraining) 동안 획득한 화학 지식을 활용하여 전통적인 유전 알고리즘(Genetic Algorithms, GAs)보다 우수한 성능을 발휘합니다. 137만 개의 TMCs 공간 중 200개의 후보만으로 상위 20개 TMC 중 8개를 식별했습니다.

- **Performance Highlights**: LLM-EO는 복잡한 수학적 공식화 없이 자연어를 사용한 프롬프트 엔지니어링(prompt engineering)을 통해 다중 목표 최적화에 비할 데 없는 유연성을 제공합니다. 또한, 새로운 리간드와 TMC를 제안하여 효율적인 최적화와 분자 생성의 이점을 결합합니다.
...
### OWPCP: A Deep Learning Model to Predict Octanol-Water Partition Coefficien (https://arxiv.org/abs/2410.18118)
Comments:
          Preprint

- **What's New**: 이번 논문에서는 화학 화합물의 물리화학적 (physicochemical) 특성을 측정하기 위한 최신 딥러닝 모델인 OWPCP를 제안합니다. 이 모델은 다양한 화학 구조를 포함하는 26,254개의 화합물에서 logP(옥탄올-물 분배 계수)를 계산하는 데 사용됩니다.

- **Technical Details**: OWPCP는 Morgan fingerprints와 MACCS keys를 입력 특징으로 활용하며, Keras Tuner와 Hyperband 알고리즘을 통해 하이퍼파라미터 최적화를 실행했습니다. 이 방법은 logP 값과 화합물 특성 간의 연결성을 분석하여, 다양한 분자량(molecular weight)과 극성 표면적(polar surface area)을 가진 화합물들의 특성을 포괄했습니다.

- **Performance Highlights**: OWPCP는 테스트 세트에서 MAE=0.247을 달성하며, 기존의 모든 DL 모델을 초과하는 뛰어난 성능을 보였습니다. 특히, 실험적 데이터를 요구하지 않고 효율적으로 logP를 계산할 수 있어, 초기 단계의 약물 발견 과정에서 매우 유용하다는 점이 주목할 만합니다.
...
### Scalable Co-Clustering for Large-Scale Data through Dynamic Partitioning and Hierarchical Merging (https://arxiv.org/abs/2410.18113)
Comments:
          8 pages, 2 figures

- **What's New**: 이 논문은 기존의 co-clustering 방법들이 큰 규모의 데이터에서 겪는 확장성 문제를 해결하기 위해 새로운 대규모 co-clustering 방법을 제안합니다.

- **Technical Details**: 제안된 방법은 먼저 큰 행렬을 더 작은 하위 행렬(submatrix)로 분할하는 대형 행렬 분할 알고리즘(large matrix partitioning algorithm)을 사용하여 병렬 co-clustering을 가능하게 합니다. 이 과정에서 확률적 모델(probabilistic model)을 사용하여 하위 행렬의 구성을 최적화하며, 계산 효율성과 분석 깊이를 균형 있게 조절합니다. 또한, 계층적 co-cluster 병합 알고리즘(hierarchical co-cluster merging algorithm)을 제안하여 하위 행렬에서 co-cluster를 효율적으로 식별하고 병합합니다.

- **Performance Highlights**: 실험 결과, 밀집 행렬에서 약 83%의 계산 시간 단축과 희소 행렬(sparse matrix)에서 최대 30%의 단축을 달성하여 제안된 방법의 효과성과 효율성을 입증했습니다.



### OPTIMA: Optimized Policy for Intelligent Multi-Agent Systems Enables Coordination-Aware Autonomous Vehicles (https://arxiv.org/abs/2410.18112)
- **What's New**: 이 논문은 연결된 자율주행차량(CAV) 간의 협력 강화를 위한 새로운 분산 강화학습 프레임워크인 OPTIMA를 소개합니다. OPTIMA는 환경 상호작용의 데이터 샘플링과 다중 에이전트 강화학습 알고리즘을 교차하여 CAV의 협력을 최적화합니다.

- **Technical Details**: OPTIMA는 분산 강화학습 기법을 활용하여 자율주행 시스템의 확장성을 개선하며, 복잡한 교통 시나리오의 안전성과 효율성을 원활하게 처리할 수 있도록 설계되었습니다. 이 프레임워크는 Decentralized Partially Observable Markov Decision Processes (Dec-POMDPs)로 문제를 공식화하여 각 에이전트의 관찰 및 행동을 명확히 정의합니다.

- **Performance Highlights**: OPTIMA는 극도로 도전적인 다중 에이전트 협력 시나리오에서 100% 성공률을 달성하였으며, 기존의 단순화된 가정에 의존하지 않고 복잡한 환경에서 자율주행 연구의 새로운 기준을 설정했습니다.
...
### A Case Study of Next Portfolio Prediction for Mutual Funds (https://arxiv.org/abs/2410.18098)
- **What's New**: 상호펀드의 미래 포트폴리오 할당 예측을 Next Novel Basket Recommendation (NNBR) 작업으로 정의하여 새로운 아이템을 예측하는 새로운 접근 방식을 제안합니다.

- **Technical Details**: 다양한 추천 시스템 모델을 평가하기 위해 SEC, Open FIGI 및 Yahoo Finance에서 공공 데이터를 기반으로 포괄적인 벤치마크 데이터셋을 생성하였습니다. 이 연구는 추천 시스템의 특정 특성을 고려하고, autoencoder 기반 접근법이 새로운 아이템 예측에서 우수한 성능을 보임을 발견했습니다.

- **Performance Highlights**: 상태에서의 NBR 모델이 단순한 휴리스틱에 의해 성능이 초과되는 결과를 도출했습니다. 특히 NNBR 작업에서 새로운 아이템을 예측하는 하위 작업에서 autoencoder 기반 접근법이 다른 모든 모델을 능가하는 성능을 기록했습니다.

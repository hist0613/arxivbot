...
### Coordinate ascent neural Kalman-MLE for state estimation (https://arxiv.org/abs/2511.01855)
- **What's New**: 본 논문에서는 최대 우도 추정(maximum likelihood estimation, MLE)을 이용한 동적 상태 추정(dynamic state estimation) 모델 학습을 위한 새로운 좌표 상승 알고리즘(coordinate ascent algorithm)을 제안합니다. 특히, 이 알고리즘은 동적 및 측정 모델을 가우시안(Gaussian)으로 가정하며, 동적 및 측정 함수와 노이즈 공분산 행렬(noise covariance matrices)을 모델링하는 신경망 파라미터를 학습합니다. 최종적으로 학습된 모델은 비선형 칼만 필터(non-linear Kalman filter)를 활용하여 테스트 단계에서 상태를 추정하는 데 사용됩니다.

- **Technical Details**: 논문에서는 동적 및 측정 함수와 이들의 노이즈 공분산이 알려지지 않은 Bayesian 필터링 문제를 다룹니다. 목표 상태(target state)와 측정의 관계는 각각의 전이 밀도(transition density) 및 측정 밀도(measurement density)를 통해 표현됩니다. 특히, 동적 함수와 측정 함수를 신경망(neural network)으로 모델링하고, 좌표 상승 MLE 알고리즘이 이를 반복적으로 최적화하여 학습합니다.

- **Performance Highlights**: 제안된 CAN-Kalman-MLE 알고리즘은 알려진/알려지지 않은 동적/측정 함수의 조합과 노이즈 공분산을 학습하는 다양한 시나리오에서 실험을 통해 그 성능을 입증합니다. 알고리즘의 결과는 KalmanNet 및 UKF와 비교되었으며, 각 경우에 대해 1200개의 시뮬레이션 경로가 생성되어 학습 및 테스트 시퀀스로 나누어 사용되었습니다. 실험 결과는 CAN-Kalman-MLE가 KalmanNet에 비해 정확도 및 훈련 시간 면에서 우수함을 보여주었습니다.



### Bridging Lifelong and Multi-Task Representation Learning via Algorithm and Complexity Measur (https://arxiv.org/abs/2511.01847)
- **What's New**: 본 연구에서는 라이프롱(생애 동안의) 학습에서 공통 데이터를 통한 표현(Representation) 학습의 일반화된 프레임워크를 제안합니다. 기존의 멀티태스크 학습이나 메타 학습과는 다르게, 제안된 알고리즘은 온라인(online) 방식으로 부분 정보를 지속적으로 수집하는 능력을 활용합니다. 특히, 제안된 알고리즘은 멀티태스크 경험적 위험 최소화(multi-task empirical risk minimization)를 서브루틴으로 사용하여, 새로운 복잡도 개념인 태스크 회피 차원(task-eluder dimension)을 기반으로 샘플 복잡도 경계를 확립합니다.

- **Technical Details**: 연구에서는 특정 함수를 갖는 예측기(predicator)인 ft∘h를 다루며, 여기서 h는 공유 표현(shared representation)이고 ft는 태스크 특화된 예측 레이어(prediction layer)입니다. 이 알고리즘은 두 개의 모듈화된 서브루틴인 소수 샷 속성 테스트(few-shot property test)와 멀티태스크 경험적 위험 최소화를 포함합니다. 이러한 구조를 통해 라이프롱 학습과 현실 세계의 멀티태스크 학습의 연결을 제시하고 이론적 보장을 제공하여 알고리즘의 성능과 샘플, 공간 복잡성을 분석합니다.

- **Performance Highlights**: 제안된 알고리즘은 노이즈가 있는 회귀(regression) 및 분류(classification) 태스크에서의 적용 사례를 통해 실용성을 입증하며 실험 결과를 제시합니다. 또한, 태스크 회피 차원이 작을 때 라이프롱 학습에서의 표현 전이의 이점이 더욱 두드러짐을 확인하였습니다. 이 결과는 다양한 학습 문제에 널리 적용될 수 있으며, 라이프롱 학습을 더 효과적으로 활용할 수 있는 기초를 제공합니다.
...
- **What's New**: 본 연구는 Kolmogorov-Arnold Networks (KANs)와 설명 가능한 머신러닝(explainable machine learning)을 결합하여 Red River Basin의 10개 저수지에서 Reservoir Water Temperature (RWT) 역학의 주요 변수를 밝혔습니다. 10,000개 이상의 깊이 기반 온도 프로파일을 사용하여 Random Forest(RF), Extreme Gradient Boosting(XGBoost), Multilayer Perceptron(MLP)과 같은 모델을 활용하여 뛰어난 예측 성능을 보였습니다. 이 연구는 예측과 이해를 통합한 새로운 접근 방식을 제공합니다.

- **Technical Details**: 연구에서는 SHAP(SHapley Additive exPlanations)를 이용하여 기상 변수인 공기 온도, 깊이, 바람, 저수지 부피의 기여도를 정량화했습니다. 또한, KANs를 사용하여 RWT를 기호적으로 근사하는 새로운 방정식을 개발하여 단순성과 정확성의 균형을 강조했습니다. R^2 = 0.84에서 R^2 = 0.92로 성능이 개선되었지만, 다섯 개의 예측 변수를 초과하면 성능 향상이 둔화되었습니다.

- **Performance Highlights**: 상당한 비선형 행동이 반영된 결과 방정식은 선형 및 합리적 형태를 크게 포함하며 해석 가능성을 유지합니다. depth는 일관되게 중요하나 보조적인 예측 변수로 나타났고, 강수량의 영향은 제한적입니다. 이 연구는 KANs와 설명 가능한 머신러닝 접근 방식이 예측 정확성과 해석력을 동시에 확보함으로써 저수지 열역학의 이해를 증진시킬 수 있음을 보여줍니다.
...
- **What's New**: 본 연구에서는 Sparse Autoencoders (SAEs)가 언어 모델에서 발생하는 시간적 구조를 효과적으로 포착하지 못한다는 점을 지적하며, Temporal Feature Analysis라는 새로운 해석 가능성 목표를 제시합니다. 이 방법은 주어진 시간에서의 표현을 예측 가능한 구성 요소와 잔여 구성 요소로 분해하여 텍스트의 흐름을 더 잘 이해할 수 있도록 합니다. 이러한 접근법은 언어 모델의 동적 표현에서 발생하는 비정상성을 처리하는 데 중점을 두고 있습니다.

- **Technical Details**: Temporal Feature Analysis는 예측 가능한 구성 요소를 문맥으로부터 유도하고, 문맥에 의해 설명되지 않는 새로운 정보를 포착하는 잔여 구성 요소를 구성합니다. 실험에서는 Gemma-2-2B와 Llama-3.1-8B 모델에서 10K 개의 활성화 데이터를 사용하여 언어 모델 표현의 동적 특성을 분석했습니다. 본 연구에서는 U-statistic을 사용해 LLM(대형 언어 모델) 표현의 유효 차원을 측정하고, 자기상관을 통해 시간에 따른 표현의 변화를 추적합니다.

- **Performance Highlights**: 이 연구에서 제안한 Temporal Feature Analyzers는 기존의 SAEs보다 훨씬 더 뛰어난 성능을 발휘하며, 가드닝 경로 문장(parse garden path sentences)을 정확하게 해석하고, 사건 경계를 올바르게 식별하는 등의 성과를 기록했습니다. 이는 시간에 따라 이동하는 정보와 느리게 변화하는 추상적 정보를 명확하게 구분하는 데 효과적임을 보여주며, Robust 한 해석 가능성 도구 설계를 위한 귀납적 편향의 필요성을 강조합니다.
...
### Towards Multi-Fidelity Scaling Laws of Neural Surrogates in CFD (https://arxiv.org/abs/2511.01830)
- **What's New**: 이번 연구에서는 과학 기계 학습에 있어 데이터의 정확도(fidelity)와 비용(cost) 간의 균형을 다루며, 전통적인 스케일링 법칙(classical scaling laws)을 재정식화하였습니다. 특히, 데이터셋 축을 계산 예산(compute budget)과 데이터셋 구성(dataset composition)으로 분해하여 최적의 training set 구성을 찾는 방법론을 제시합니다. 이는 다양한 높은 정확도(high-fidelity) 및 낮은 정확도(low-fidelity) 샘플의 조합을 통해 모델 성능을 극대화하는 기반이 됩니다.

- **Technical Details**: 이 연구는 Reynolds-Averaged Navier-Stokes (RANS) 시뮬레이션을 활용하여 두 가지 신뢰도 수준 사이에서의 데이터 생성 비용 및 성능 향상 간의 상관관계를 분석합니다. 각 신뢰도 수준은 경계층 처리를 달리하여 설정되어 있으며, 고비용의 고신뢰도(고해상도) 시뮬레이션에 비해 저비용의 저신뢰도(저해상도) 시뮬레이션의 활용 가능성을 제시합니다. 연구는 Computational Fluid Dynamics (CFD) 데이터셋을 기반으로 하여 성능 스케일링을 평가하고, 데이터 구성을 최적화하는 방법을 검토합니다.

- **Performance Highlights**: 실험 결과에 따르면, 예산에 따라 최적의 신뢰도 혼합 비율이 다르며, 이는 모델의 성능에 중대한 영향을 미칩니다. 또한, 각기 다른 데이터셋 조합에서 모델 성능이 어떻게 변화하는지를 관찰함으로써, 다중 신뢰도(multi-fidelity) 신경 대리 모델(neural surrogate)의 데이터셋 생성에 대한 실용적인 고려사항을 도출했습니다. 이 연구는 과학 기계 학습에서 데이터 생성의 비용 효율성을 극대화하기 위한 최초의 실증 연구로 자리 잡고 있습니다.
...
### No-rank Tensor Decomposition Using Metric Learning (https://arxiv.org/abs/2511.01816)
- **What's New**: 이 논문에서는 전통적인 텐서 분해(tensor decomposition) 기법의 한계를 극복하기 위해 메트릭 학습(metric learning)에 기반한 새로운 '무순위(no-rank)' 텐서 분해 프레임워크를 제안합니다. 기존의 재구성(reconstruction) 목표를 대체하여 유사성 기반 최적화(similarity-based optimization)를 활용합니다. 이를 통해 데이터 기반의 임베딩(embeddings)을 학습하고, 거리(distance)가 직접적으로 의미론적 유사성(semantic similarity)을 반영하는 특성 공간을 만듭니다.

- **Technical Details**: 제안된 프레임워크는 다양성(diversity) 및 균일성(uniformity) 규제를 고려한 트리플렛 손실(triplet loss)을 최적화하여 고차원 데이터의 의미 있는 구조를 포착합니다. 기존 텐서 분해 방법들은 미리 정의된 순위(rank) 매개변수에 의존해야 했으나, 제안된 기법은 이들의 한계를 극복하고 데이터에 따라 적응 가능한 텐서 분해를 가능하게 합니다. 또한, 이 방법은 텐서 분석을 위한 새로운 패러다임으로 메트릭 학습을 강조하였습니다.

- **Performance Highlights**: 다양한 도메인(얼굴 인식, 뇌 연결 분석 등)에서 기존의 PCA, t-SNE, UMAP 및 텐서 분해 기법(CP, Tucker)보다 우수한 성능을 보여주었습니다. 실험 결과, 클러스터링 메트릭에서 큰 개선(실루엣 점수(Silhouette Score), 다비스-볼딘 지수(Davies--Bouldin Index) 등)을 확인하였으며, 메트릭 학습이 전반적인 클래스 분리를 최적화하는 동시에 의미론적 관계에 맞춰 로컬 기하학을 변형한다는 근본적인 트레이드오프를 드러냈습니다. 개별적 데이터가 적은 환경에서도 우수한 성능을 보이며, 이는 기존의 transformer 기반 방법보다 효율적인 대안이 됩니다.



### Dynamic Reconstruction of Ultrasound-Derived Flow Fields With Physics-Informed Neural Fields (https://arxiv.org/abs/2511.01804)
Comments:
          29 pages, 18 figures

- **What's New**: 이 논문은 초음파 데이터에서 희박하고 잡음이 섞인 혈류를 추정하기 위해 물리 정보 기반의 신경장(reinforcement)을 활용한 모델을 제안합니다. 이 모델은 다중 스케일 푸리에 특성 인코딩(multi-scale Fourier Feature encoding)을 포함하여, 고유한 지상 진리 감독 없이 혈류를 효과적으로 추정할 수 있도록 설계되었습니다. 또한, 이 연구는 물리법칙에 따라 혈류를 재건하는 것을 목표로 하며, 기존의 방법들과 차별화됩니다.

- **Technical Details**: 제안된 모델은, 희박한 초음파 데이터에서도 혈류 속도를 정확하게 추정할 수 있도록 물리 기반 신경망(PINNs)의 자동 미분 기능을 활용합니다. 이를 통해 유동의 동적 법칙을 따르는 혈류 필드를 복원할 수 있고, 기존의 이미지 기반 접근법에서 벌어지는 비물리적 환상의 가능성을 줄입니다. 또한, 신경필드(neural fields)는 이미지가 지속적 시공간 도메인으로 처리되도록 하여 공간적 및 시간적 해상도의 변화를 지원합니다.

- **Performance Highlights**: 모델은 합성 데이터와 실제 데이터 모두에서 일관되게 낮은 평균 제곱 오차(mean squared error)를 기록하며, 참고 혈류 필드(flow fields)와 유효 혈류 속도 측정치와 비교하여 검증되었습니다. 이 연구는 정형화된 데이터에서의 성공적인 적용을 넘어 실제 초음파 이미징의 복잡성에 적합한 신경 필드 기술을 적용하려고 하며, 심혈관 진단 및 수술 절차 평가에서 유효한 진단 도구로서의 가능성을 보여줍니다.



### Bayesian Coreset Optimization for Personalized Federated Learning (https://arxiv.org/abs/2511.01800)
Comments:
          9 pages, 5 figures, ICLR 2024

- **What's New**: 본 논문에서는 Federated Learning의 새로운 개선 방안을 제안합니다. $	ext{CoreSet-PFedBayes}$라는 개인화된 coreset 가중치 연합 학습 설정을 통해, 각 클라이언트의 데이터 전체 대신 대표적인 데이터 포인트만을 사용하여 중앙 서버에 업데이트를 전송합니다. 이 방법은 계산 자원을 절약하고 전반적인 일반화 오차를 줄이는데 기여합니다.

- **Technical Details**: 이 연구는 Bayesian coreset 최적화를 Federated Learning에 통합하는 새로운 아키텍처를 제안합니다. 클라이언트 별로 개인화된 coreset 가중치를 사용하는 다수의 새로운 목적 함수도 제안되며, 이로 인해 일반화 오류를 최적화합니다. 이론적 분석 결과, 제공된 접근 방식은 로그 범위 내의 수렴 속도를 달성합니다.

- **Performance Highlights**: 다양한 벤치마크 데이터셋에서 진행된 실험 결과, 제안된 방법은 임의 샘플링 방식에 비해 우수한 성과를 보여주었습니다. 의료 데이터셋에 대한 실험에서도 기존의 하위 모듈 최적화 기반 접근 방안보다 성능 향상이 있음을 입증하였습니다. 또한, 클라이언트 측에서 적은 양의 데이터로도 높은 모델 정확도를 달성할 수 있음을 나타냅니다.
...
### HyperNQ: A Hypergraph Neural Network Decoder for Quantum LDPC Codes (https://arxiv.org/abs/2511.01741)
Comments:
          6 pages, 4 figures, Submitted to the IEEE International Conference on Communications (ICC 2026). Preprint version

- **What's New**: 이번 연구에서는 Quantum Low-Density Parity-Check (QLDPC) 코드를 디코딩하기 위한 Hypergraph Neural Network (HGNN) 기반의 새로운 프레임워크인 HyperNQ를 제안합니다. HyperNQ는 고차 안정자 제약조건을 포착할 수 있는 구조를 사용하여, 기존의 디코더보다 더 높은 성능을 보이는 것을 목표로 합니다. 특히, HyperNQ는 짧은 사이클 문제 및 느린 수렴 문제를 해결하여 오류율을 현저히 개선합니다.

- **Technical Details**: HyperNQ는 하이퍼엣지를 활용하여 노드 간의 복잡한 관계를 모델링합니다. 이러한 구조는 다수의 큐비트 안정자 제약을 효과적으로 표현할 수 있게 해줍니다. 또한, 이번 연구에서는 두 단계의 메시지 패싱 (Message Passing) 메커니즘을 사용하여 노드와 하이퍼엣지 간의 피처 전파, 집계 및 업데이트를 수행합니다.

- **Performance Highlights**: HyperNQ는 실험적으로 기존의 클래식 및 머신러닝 기반 디코더보다 최대 84% 개선된 논리적 오류율 (Logical Error Rate, LER)을 달성했습니다. 이는 오류율 임계점인 pseudo-threshold 이상의 영역에서도 높은 성능을 발휘한다는 것을 나타냅니다. HyperNQ는 QLDPC 코드에 대한 디코딩 성능을 크게 향상시킴으로써 양자 오차 수정의 가능성을 넓혔습니다.



### Game-theoretic distributed learning of generative models for heterogeneous data collections (https://arxiv.org/abs/2511.01740)
Comments:
...
- **What's New**: 이번 연구에서는 분산 학습에서의 불균형한(local models and data) 문제를 해결하기 위해 생성 모델(generative models) 기반의 새로운 접근법을 제안합니다. 파라미터를 공유하는 대신, 합성 데이터(synthetic data)를 교환하는 방식을 통해 모델을 다룰 수 있는 '블랙 박스(black box)'로 취급하게 됩니다. 특히, 반지도 학습(semi-supervised learning)이 가능한 경우, 서로 다른 확률 공간에서의 로컬 모델을 활용할 수 있게 되어 이질적인 데이터 모달리티를 다룰 수 있습니다.

- **Technical Details**: 연구에서는 게임 이론(game theory)의 원리를 바탕으로 로컬 모델의 학습을 협력 게임(cooperative game)으로 형식화했습니다. 여기에서 각 모델은 독립적으로 데이터로부터 학습하여 합성 데이터를 생성하는 능력을 가집니다. 기본적으로, 이 접근법은 로컬 모델이 서로에 대해 아는 것이 없어야 하며, 다른 모델들이 생성한 데이터만 관찰해야 한다는 제약 조건을 갖습니다.

- **Performance Highlights**: 제안된 방법은 표준 비전 데이터셋(vision datasets)에서 이미지 분류(image classification) 및 조건부 생성(conditional generation)을 위해 테스트되어 그 효과를 입증하였습니다. 결과적으로, 조합된 교육 데이터를 통해 각 모델이 최적화된다는 점에서 협력적인 학습 구조의 필요성을 강조하고, 이질적인 데이터 처리에 있어 강력한 가능성을 열어주고 있음을 확인했습니다.



### Edge AI in Highly Volatile Environments: Is Fairness Worth the Accuracy Trade-off? (https://arxiv.org/abs/2511.01737)
Comments:
          Presented at IEEE FLTA 2025

- **What's New**: 본 논문은 Federated Learning (FL)에서 모델 정확성과 공정성 간의 근본적인 균형을 조사하였다. 특히, 변동성이 큰 엣지 환경에서의 클라이언트 선택 알고리즘의 공정성이 미치는 영향을 분석하고, 기존의 랜덤 및 탐욕적(greedy) 방법과 비교하였다. 연구 결과를 통해 공정한 클라이언트 선택이 훈련 시간을 느리게 만들 수 있다는 점을 제시하여, FL의 공정성-성능 및 공정성-속도 무역을 조명하고 있다.

- **Technical Details**: 이 연구는 FL에서 클라이언트 선택 알고리즘이 자원의 가용성 및 클라이언트의 이질성 문제에 어떻게 대응하는지를 살펴보았다. 3개의 벤치마크 데이터셋(CIFAR10, FashionMNIST, EMNIST)을 통해, 기존의 탐욕적 및 랜덤 선택 전략과 공정성을 중시하는 RBFF 및 RBCSF 알고리즘의 성능을 비교하였다. 이 과정에서 각 데이터셋의 분포가 IID 및 비 IID(Non-IID)인지에 따라 다른 시나리오를 설정하여 성능을 평가하였다.

- **Performance Highlights**: 실험 결과는 공정한 클라이언트 선택 알고리즘이 훈련 성능과 속도 간의 균형을 이룰 수 있음을 보여주었다. 탐욕적 방식은 훈련 시간을 25% 개선할 수 있었으나 공정성 면에서는 손해를 보였다. 반면 임의의 전략은 높은 공정성과 경쟁력 있는 성능을 제공했으나 훈련 시간이 증가하는 경향이 있었다. 공정한 선택 전략은 세 가지 요소를 동시에 고려할 때 각각의 성능에서 제한을 받을 수도 있다는 점을 발견하였다.



### Collaborative Large Language Model Inference via Resource-Aware Parallel Speculative Decoding (https://arxiv.org/abs/2511.01695)
- **What's New**: 이번 논문에서는 모바일 엣지 컴퓨팅(MEC) 환경에서 대규모 언어 모델(LLM)의 큰 수요에 발맞추어 효율적인 솔루션을 제안합니다. 특히, 저자들은 경량 드래프트 모델과 강력한 타겟 모델 간의 토큰 생성 파티셔닝을 통한 'Speculative Decoding' 방식을 적용합니다. 이 연구는 사용자 연관(Association) 및 자원 할당(Resource Allocation)을 동시에 최적화하는 통합 프레임워크를 최초로 제안합니다.

- **Technical Details**: 제안된 방법은 다중 에이전트 심층 강화 학습(Multi-Agent Deep Reinforcement Learning) 알고리즘을 사용하여 사용자 연관 및 자원 할당(UARA) 문제를 해결합니다. 이를 통해 비동기 지연(asynchronous delays) 및 통신 오버헤드(communication overhead)를 줄이면서 효율적인 병렬 Speculative Decoding을 지원합니다. 실험은 Sionna 시뮬레이터를 이용하여 현실적인 조건 하에서 수행되었습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 종단 간 지연(end-to-end latency)을 최대 28.0%, 평균 23.7%까지 줄이는 성과를 보였습니다. 이러한 개선은 추론 정확성을 저하시키지 않으면서 실현되어 MEC 시스템에서 확장 가능하고 낮은 지연의 LLM 서비스를 가능하게 합니다.
...
### Cross-Treatment Effect Estimation for Multi-Category, Multi-Valued Causal Inference via Dynamic Neural Masking (https://arxiv.org/abs/2511.01641)
- **What's New**: 이번 논문에서는 복합적인 다중 카테고리 및 다중 값 치료 효과 추정을 위한 새로운 네트워크 아키텍처인 XTNet을 제안합니다. 기존 방법론들이 이진 또는 단일 유형 치료에 국한되어 있고, 복잡한 간섭 효과를 모델링하는데 있어 한계를 지니고 있음을 지적합니다. XTNet은 동적 마스킹 메커니즘을 활용하여 제한적인 구조적 가정 없이 치료 상호작용을 캡처하는 교차 효과 추정 모듈을 도입합니다.

- **Technical Details**: XTNet 아키텍처는 기본적인 치료 효과를 설정하기 위한 BasicNet, 교차 카테고리 상호작용을 명시적으로 모델링 하기 위한 EffectNet, 치료 특화 매개변수 마스크를 생성하여 확장성을 강화하는 MaskNet의 세 가지 주요 혁신을 포함합니다. 또한, 우리는 다중 카테고리 및 다중 값 치료 시나리오에 적합한 평가 지표인 MCMV-AUCC를 제안하여 치료 비용과 상호작용 효과를 반영합니다. XTNet은 이러한 복잡한 설정에서 효과적으로 치료 비용과 한계 수익을 통합하여 낮은 기대 지표 오차를 달성할 수 있음을 보였습니다.

- **Performance Highlights**: XTNet은 합성 및 실제 데이터셋에서 광범위한 실험을 통해 최첨단 기준선 모델들보다 항상 더 높은 효과 추정 품질과 순위 정확도를 달성했습니다. 실제 A/B 테스트 결과에서도 XTNet의 효과성을 입증하며 그 성능을 강화했습니다. 본 연구의 주요 기여로는 다중 카테고리 및 다중 값 치료 효과 추정 문제를 처음으로 체계적으로 정리하고, XTNet을 통해 개별 치료 조합 별 모델 없이도 효과적인 추정이 가능하다는 점이 강조됩니다.
...
### Estimation of Toeplitz Covariance Matrices using Overparameterized Gradient Descen (https://arxiv.org/abs/2511.01605)
- **What's New**: 이번 논문은 Toeplitz 구조 하의 공분산(matrix covariance) 추정에 대한 새로운 접근법을 제안합니다. 심도 있는 딥러닝의 발전은 단순한 Gradient Descent (GD) 기법이 과도하게 매개변수화된 모델에 강력한 성능을 보여준다는 점에서 저자들은 이를 중요한 동기로 삼았습니다. 새로운 매개변수화 기법을 통해 모델의 성능을 향상시키고 GD를 활용하여 정확한 공분산 추정을 목표로 합니다.

- **Technical Details**: 저자들은 $P \times P$ 공분산을 학습 가능한 매개변수를 가진 K개의 복소수 사인파의 합으로 모델링하고 GD를 통해 최적화합니다. 이 방법은 매개변수 K가 P일 경우 최적 해에 수렴할 수 없지만, K를 $2P$ 또는 $4P$로 설정할 경우 무작위 초기화에서의 전역 수렴(global convergence)을 보장합니다. 또한 진폭(amplitudes)과 주파수(frequencies)에 대해 별도의 학습률을 두어 GD의 수렴을 가속화하는 변형을 제안합니다.

- **Performance Highlights**: 대표적인 방법들과 비교하였을 때, 과도하게 매개변수화된 GD는 정확도에서 동등하거나 우수한 성능을 보여주었습니다. 특히 GD가 진폭을 고정하고 주파수만 최적화할 경우, 최적화 경관(landscape)이 양호하여 모든 정적 포인트가 진짜 공분산을 회복하는 것으로 나타났습니다. 이러한 결과는 간단하면서도 확장 가능한 접근법으로 통계 신호 처리의 여러 응용 분야에서 유용할 것으로 기대됩니다.



### Defining Energy Indicators for Impact Identification on Aerospace Composites: A Physics-Informed Machine Learning Perspectiv (https://arxiv.org/abs/2511.01592)
- **What's New**: 이번 연구에서는 저속 충격이 항공 우주 복합재에서 유발할 수 있는 내부 손상을 감지하기 위한 에너지 예측 방법론의 한계를 극복하는 물리 지식 기반 머신 러닝 (Physics-Informed Machine Learning, PIML) 프레임워크를 제안합니다. 이 프레임워크는 객체의 동적 행동을 반영하는 물리적으로 동기를 부여 받은 신호 특징을 활용해 입력 공간을 최적화합니다. 이 방법은 데이터를 더욱 해석 가능하게 만들고, 실용성을 높이며, 모델의 성능을 향상시킵니다.

- **Technical Details**: 프레임워크는 일관된 특징 추출 및 선택 과정을 통해 물리적으로 의미 있는 에너지 감지 특성의 집합을 생성합니다. 특징은 시간, 주파수 및 시간-주파수 영역에서 추출되어 구조적 반응의 보완 측면을 포착합니다. 연구자는 실험적 데이터를 기반으로 하는 완전 연결 신경망 (Fully Connected Neural Network, FCNN)을 훈련시키고 검증함으로써 이 최적화된 입력 공간을 적용합니다.

- **Performance Highlights**: 연구 결과, 새로운 접근 방식은 기존의 시간 시계열 기법 및 데이터 기반 모델보다 에너지 예측 정확도를 세 배 향상시켜, 저속 충격 시나리오에 대한 에너지 예측을 보다 신뢰할 수 있도록 합니다. 이 모델은 불완전한 측정 조건에서도 강력한 성능을 보이며, 충격 에너지를 효과적으로 추정할 수 있는 기반을 제공합니다.
...
### Gated Fusion Enhanced Multi-Scale Hierarchical Graph Convolutional Network for Stock Movement Prediction (https://arxiv.org/abs/2511.01570)
- **What's New**: 이 연구에서는 다중 규모 Graph Neural Networks (GNNs)의 한계를 극복하기 위해 Multi-Scale Hierarchical Graph Fusion Network (MS-HGFN)을 제안합니다. MS-HGFN은 각 주식의 intra-attribute 패턴과 inter-attribute features를 동적으로 학습하여 시간적 경향성을 포괄적으로 캡처합니다. 또한, 상단-하향 게이팅(gating) 접근 방식을 통해 중요 coarse- 및 fine-grained features를 통합하면서 예측 성능을 향상시킵니다.

- **Technical Details**: MS-HGFN은 계층적 Graph Neural Network 모듈과 게이티드 퓨전 모듈로 구성됩니다. 계층적 GNN 모듈은 각 시간 규모에서 주식의 속성 간의 내재적 상호작용 패턴을 캡처하는 학습 가능한 행렬을 통합합니다. 게이티드 퓨전 모듈은 시간이 작은 규모에서 필수적인 coarse- 및 fine-grained features를 유지하기 위해 게이팅 메커니즘을 적용합니다.

- **Performance Highlights**: 미국과 중국 주식 시장의 실제 데이터셋을 사용한 실험 결과, MS-HGFN은 기존 모델보다 최대 1.4%의 예측 정확도를 향상시켰으며 수익률 시뮬레이션에서 안정성을 높였습니다. 이 연구는 다양한 주식 간의 동적 공간-시간 의존성을 효과적으로 감지하고 있으며, 이러한 접근 방식이 거시적 및 미시적 트렌드를 포착할 수 있음을 강조합니다.
...
### Efficient Curvature-aware Graph Network (https://arxiv.org/abs/2511.01443)
- **What's New**: 이번 연구에서는 그래프 신경망(Graph Neural Networks, GNNs)의 성능을 향상시키기 위한 새로운 그래프 곡률 측정법인 'Effective Resistance Curvature'를 제안합니다. 이는 노드 쌍 간의 실제 저항을 이용하여 메시지 전파의 용이성을 정량화함으로써 기존의 Ollivier-Ricci 곡률보다 계산 효율성을 극대화합니다. 제안된 곡률은 비교 가능한 기하학적 표현력을 유지하면서도 대규모 그래프 데이터셋에서의 적용 가능성을 높입니다.

- **Technical Details**: 이 연구에서는 GNN의 메시지 전달 과정을 전기 회로와 유사하게 모형화하여, 노드 간의 연결성을 반영하는 곡률을 효과적으로 계산합니다. 기존의 Ollivier-Ricci 곡률에 비해 시간 복잡도를 현저히 줄인 'Effective Resistance Curvature'의 이론적 분석을 제공하며, 이 두 곡률이 그래프에서 노드 분포 구조를 일관되게 설명한다는 사실을 밝혀냅니다.

- **Performance Highlights**: 다양한 GNN 작업에 대해 실시된 광범위한 실험에서는 제안된 방법이 Ollivier-Ricci 곡률과 유사한 성능을 보이면서도 계산 부담을 크게 줄였음을 보여줍니다. 이러한 결과는 GNN의 그래프 표현 학습에 있어 새로운 효율적인 접근법이 될 것으로 기대됩니다.



### The Curvature Rate λ: A Scalar Measure of Input-Space Sharpness in Neural Networks (https://arxiv.org/abs/2511.01438)
Comments:
          14 pages

- **What's New**: 본 논문은 경도(Curvature)가 신경망의 일반화, 강건성(robustness), 그리고 작은 입력 변동에 대한 반응에서 어떻게 영향을 미치는지를 새롭게 정의합니다. 기존의 경도 측정 지표는 매개변수 공간에서 정의되며, 계산 비용이 크고 해석이 어려운 반면, 본 논문에서는 입력 공간에서 직접 정의되는 스칼라 경도 측정 값 λ를 소개합니다. 이 방법은 신경망의 결정 경계에서 고주파 구조의 출현을 추적할 수 있으며, 경도의 변화를 예측 가능하게 합니다.

- **Technical Details**: 경도 값 λ는 고차 입력 미분의 기하급수적 성장률에 따라 정의됩니다. 이는 미분 동역학을 기반으로 하며, 신경망과 고전적인 분석 함수 간의 유사성을 보여줍니다. λ는 고차 미분의 노름을 통해 추정되며, 이는 전체 훈련 중에 예측 가능한 방식으로 진화합니다. 이를 통해 최적화 과정에서 경도를 제어할 수 있는 간단한 정규화기법인 Curvature Rate Regularization (CRR)을 도입합니다.

- **Performance Highlights**: CRR은 Sharpness-Aware Minimization (SAM)과 비슷한 정확도를 유지하면서도 입력 공간의 기하학을 더 평평하게 만들고 신뢰도 조정(calibration)을 향상시킵니다. 실험 결과 경도 λ는 훈련 과정에서 예측 가능한 방식으로 진화하며, 이를 통해 경도를 제어할 수 있어 학습 모델의 동작 및 특성을 효과적으로 형상화할 수 있습니다. 본 연구는 분석 및 심층 학습 간의 개념적 연계를 제공하여 신경망에서의 기능적 매끄러움을 설명합니다.



### CG-FKAN: Compressed-Grid Federated Kolmogorov-Arnold Networks for Communication Constrained Environmen (https://arxiv.org/abs/2511.01433)
Comments:
          5 pages

- **What's New**: 본 논문에서는 압축-그리드 연합 KAN(CG-FKAN)을 제안하여, KAN의 그리드 확장으로 인해 발생하는 통신 오버헤드를 줄입니다. CG-FKAN은 통신 예산 하에 필수 계수만 전송하도록 설계되었습니다. 실험 결과, CG-FKAN은 통신 제약 조건 하에서 고정 그리드 KAN보다 최대 13.6% 낮은 RMSE를 달성하는 것을 보여주었습니다.

- **Technical Details**: 연합 학습(FL) 시스템은 NN 개의 클라이언트와 중앙 서버로 구성됩니다. 각 클라이언트는 자신의 개인 데이터셋을 소유하며, 서버는 클라이언트 데이터에 직접 접근하지 않고 모델 파라미터를 집계합니다. 본 연구에서는 KAN을 활용하여 각 클라이언트의 지역 모델을 업데이트하고, 이 과정에서 그리드 크기를 증가시키는 미세 조정 전략을 채택합니다.

- **Performance Highlights**: CG-FKAN은 KAN 기반 FL의 업링크 통신 비용을 축소하는 데 초점을 맞추고 있습니다. 제안된 방법은 고정된 업링크 예산 내에서 핵심 계수만 선택하여 데이터 전송을 최소화합니다. 이러한 접근 방식은 통신 비용을 효율적으로 저감시키면서도 유의미한 모델 성능을 유지하는데 기여합니다.



### Leveraging Compact Satellite Embeddings and Graph Neural Networks for Large-Scale Poverty Mapping (https://arxiv.org/abs/2511.01408)
- **What's New**: 이번 연구에서는 Sub-Saharan 아프리카의 군집 수준의 부를 예측하기 위해 저차원 AlphaEarth 위성 임베딩을 활용한 그래프 기반 접근 방식을 제안합니다. 기존의 Demographic and Health Surveys (DHS) 데이터에 따라 군집의 좌표가 익명화되는 문제를 해결하며, 이로 인해 공간적 불확실성이 추가되는 문제도 함께 해결하고자 하였습니다. 이 연구는 DHS 데이터셋을 사용하여 부 예측의 일반화를 향상시키는 데 중요한 진전을 이룹니다.

- **Technical Details**: 모델 개발 및 평가 시 2017년부터 2023년 사이에 수행된 37개의 DHS 설문조사 데이터를 활용하였습니다. DHS 종료 클러스터의 좌표는 최대 2km(도시 지역) 또는 10km(농촌 지역) 만큼 무작위로 이동되어 공간적 노이즈를 유발하며, 이를 고려하여 'fuzzy label' 손실 함수를 도입했습니다. 이 손실 함수는 일정한 확률에 의해 추천 거주지의 가능성을 가중치를 두어 확산시킴으로써 모델의 신뢰성을 개선해줍니다.

- **Performance Highlights**: 모델은 cross-validation을 통해 평가되었으며, 공간 구조와 비 라벨 장소를 포함한 모델이 기존의 이미지 기반 모델보다 약간 더 높은 정확도를 보였습니다. 특히 Ego Graphs 모델이 가장 우수한 성능을 나타내어, 가까운 군집으로부터의 정보 전파가 유용하다는 것을 보여주었습니다. 그러나 fuzzy label을 사용하는 모델은 성능이 떨어지며, 이는 DHS 데이터에 의존한 위치 정규화가 여전히 더 신뢰할 수 있는 감독 방법일 수 있음을 시사합니다.



### Memory-Efficient Training with In-Place FFT Implementation (https://arxiv.org/abs/2511.01385)
Comments:
          Accepted at NeurIPS 2025. Presents a real-domain in-place FFT (rdFFT) operator for memory-efficient fine-tuning of large language models

- **What's New**: rdFFT는 기존 FFT 구현의 한계를 극복하는 새로운 방식으로, 메모리 공간 일관성을 유지하는 최초의 실수 도메인 완전 인플레이스 FFT 프레임워크입니다. 이 방법은 중간 캐시 사용을 완전히 제거함으로써, 메모리 비용 절감에 기여합니다. 또한 bfloat16 데이터 타입을 지원하여 현대 신경망 모델과의 호환성을 높입니다.

- **Technical Details**: rdFFT는 주파수 도메인에서의 대칭성과 공액 특성을 활용하여 암묵적인 복소수 인코딩 방식을 설계하였습니다. 이는 보조 메모리 버퍼의 필요 없이 인플레이스 계산을 가능하게 하며, 현대 딥러닝 워크플로우에 원활하게 통합될 수 있도록 합니다. FFT와 IFFT 계산을 정리하기 위해 고안된 새로운 메모리 레이아웃과 나비(Butterfly) 실행 구조도 포함됩니다.

- **Performance Highlights**: 여러 자연어 처리(NLU) 작업에 대한 실험 결과, rdFFT는 훈련 중 메모리 비용을 효과적으로 줄이며, 실세계 모델에서의 성능 또한 입증하였습니다. 중간 텐서 계산에 대한 제로 메모리 할당을 달성하여, 메모리 제약이 있는 환경에서도 효율적인 성능을 보입니다. 이 방법은 주파수 도메인에서의 경량화된 적응을 위한 유망한 방향성을 제시합니다.



### Protecting the Neural Networks against FGSM Attack Using Machine Unlearning (https://arxiv.org/abs/2511.01377)
Comments:
          7 pages, 9 figures, 1 table

- **What's New**: 본 논문에서는 머신 러닝 모델의 취약점을 해결하기 위한 새로운 방법으로 '머신 언러닝'을 제안합니다. 이 기술은 적대적 공격(adversarial attacks)인 FGSM(Fast Gradient Sign Method)에 대한 방어를 목표로 합니다. 기존 데이터에서 추가된 변동(perturbations)을 제거하고 원본 데이터로 모델을 재교육하는 방식으로, LeNet 신경망 구조에 적용하여 그 효과를 검증합니다.

- **Technical Details**: 머신 언러닝(machine unlearning) 기술은 훈련 데이터셋에서 특정 데이터 포인트를 제거함으로써 모델의 견고성을 개선하려는 접근입니다. FGSM 공격은 입력 데이터에 작은 변화를 주어 모델이 잘못 분류하도록 만드는 기법입니다. 본 연구에서는 LeNet 네트워크 아키텍처에 언러닝 기법을 적용하는 방법을 다룹니다.

- **Performance Highlights**: 실험 결과, FGSM 공격에 대해 LeNet 네트워크의 견고성이 현저히 향상됨을 확인했습니다. 머신 언러닝 기법을 활용함으로써 모델이 공격에 더 강해질 수 있는 가능성을 보여줍니다. 이러한 결과는 다양한 이미지 분류(Task) 분야에서 적대적 공격에 대한 방어 가능성을 열어줍니다.



### Learning Intractable Multimodal Policies with Reparameterization and Diversity Regularization (https://arxiv.org/abs/2511.01374)
Comments:
          NeurIPS 2025

- **What's New**: 전통적인 심층 강화 학습(RL) 알고리즘은 결정적 또는 단일 모드의 Gaussian(actor)을 사용하여 복잡한 다중 모드 결정 분포를 표현할 수 없다는 한계를 가지고 있습니다. 이 논문에서는 이러한 문제를 해결하기 위해 기존의 비관계 다중 모드 액터를 재구성하고 정책 기울기를 통한 최적화가 가능하다는 점을 증명합니다. 또한, 결정 확률을 명시적으로 요구하지 않는 거리 기반의 다양성 정규화를 제안하여 정책 다양성을 향상시킵니다.

- **Technical Details**: 기존의 비관계적 다중 모드 액터를 스토케스틱-매핑 액터(stochastic-mapping actors)로 재정의하고, 정책 기울기를 재매개화 기법(reparameterization trick)을 통해 직접 최적화할 수 있음을 제시합니다. 이는 비관계적 스토케스틱 액터를 최적화할 수 있는 일반적인 방법론을 제공합니다. 또한, 거리를 기반으로 한 다양성 정규화를 사용하여 기대 수익과 결정 다양성을 동시에 최적화하는 학습 프레임워크를 제안합니다.

- **Performance Highlights**: 논문은 다중 목표 달성과 생성적 RL 두 가지 분야에서 다중 모드 정책의 장점을 입증합니다. MuJoCo 벤치마크에서도 경쟁력 있는 성능을 보여줍니다. 특히, 제안된 알고리즘은 몇 번의 샷에서도 견고하게 작동하며, 이는 다중 모드 정책과 새로운 접근 방식의 이점을 명확히 합니다.



### Verifiable Split Learning via zk-SNARKs (https://arxiv.org/abs/2511.01356)
Comments:
          Submitted to CAI'26 (IEEE Conference on Artificial Intelligence 2026)

- **What's New**: 이번 논문에서는 분산 학습(split learning)에 zk-SNARK 증명을 통합하여 계산의 정확성과 검증 가능성을 보장하는 새로운 프레임워크를 제안합니다. 기존의 분산 학습은 각 참여자가 계산의 올바름을 확인할 수 없어서 신뢰의 문제가 발생하였습니다. 논문에서는 클라이언트와 서버가 각자의 연산 부분에 zk-SNARK 증명을 추가함으로써 이러한 신뢰 문제를 해결할 수 있음을 보여줍니다. 블록체인의 업데이트 기록 기능과 비교하여, zk-SNARK가 제공하는 검증 가능성과 정확성이 강조됩니다.

- **Technical Details**: 분산 학습에서는 신경망이 클라이언트 측과 서버 측 두 부분으로 나뉘어 있습니다. 클라이언트는 자신의 데이터로 연산을 수행하고, 중간 활성 값(activation)을 서버에 전달하여 서버에서 이어서 학습하도록 합니다. 기존의 시스템에서는 이러한 과정이 올바르게 진행되었는지 검증할 방법이 없었으나, zk-SNARK를 통해 각 단계의 정확성을 검증할 수 있습니다. 논문에 따르면, zk-SNARKs는 컴퓨팅 비용을 적게 들이면서도 각 연산 단계의 무결성을 보장합니다.

- **Performance Highlights**: 실험 결과, 제안된 zk-SNARK 기반의 분산 학습 프레임워크는 블록체인 기반 시스템에 비해 더 낮은 계산 비용으로 검증 가능성을 확보할 수 있음을 확인했습니다. zk-SNARK가 적용된 시스템은 각 처리 단계에서의 정확성을 확인할 수 있는 반면, 블록체인 시스템은 업데이트 사항을 기록하더라도 계산의 올바름을 보장하지 못합니다. 이러한 차별점은 고위험 분야, 예를 들어 의료 및 금융 분야에서 신뢰성을 요구하는 경우에 있어 중요한 의미를 갖습니다.



### MiniFool - Physics-Constraint-Aware Minimizer-Based Adversarial Attacks in Deep Neural Networks (https://arxiv.org/abs/2511.01352)
Comments:
          Submitted to Computing and Software for Big Science

- **What's New**: 이 논문에서는 MiniFool이라는 새로운 알고리즘을 제안합니다. 이 알고리즘은 물리학에서 영감을 받은 적대적 공격(adversarial attacks)을 통해 신경망 기반의 분류 작업을 테스트하기 위해 개발되었습니다. IceCube Neutrino Observatory에서의 천문학적 타우 중성미자 검색을 위해 초기 개발된 이후, 다른 과학 분야의 데이터에도 적용되었습니다.

- **Technical Details**: MiniFool 알고리즘은 목표 점수(target score)와 입력의 변화 요구를 균형 있게 조절하는 비용 함수(cost function)를 최소화하는 방식으로 동작합니다. 이 알고리즘은 데이터의 실험적 불확실성을 기반으로 한 통계 및 물리적 해석을 제공하며, 이를 통해 신경망의 견고성을 평가할 수 있습니다. 알고리즘의 성공률은 분류를 변경하기 위해 요구되는 공격의 변형이 데이터 불확실성에 비해 과도해지는지 여부에 따라 판단됩니다.

- **Performance Highlights**: MiniFool 알고리즘은 MNIST 데이터 세트에 대한 초기 테스트와 IceCube에서의 타우 중성미자 탐지, Large Hadron Collider의 CMS 공개 데이터를 포함한 다양한 실험적 데이터의 분류에 적용되어 그 일반성을 보여주었습니다. 이 논문은 결과에 대한 논의로 끝나며, MiniFool의 적용이 다른 입자 물리학 사례에서도 가능함을 강조합니다.



### Diffusion-Based Solver for CNF Placement on the Cloud-Continuum (https://arxiv.org/abs/2511.01343)
Comments:
          7 pages, 7 figures. Presented at PE-WASUN'25 (IEEE International Symposium on Performance Evaluation of Wireless Ad Hoc, Sensor, and Ubiquitous Networks)

- **What's New**: 본 연구에서는 클라우드 네이티브 네트워크 기능(CNF) 배치를 위한 새로운 이론적 프레임워크를 제안합니다. 이 접근 방식은 배치를 생성 그래프로 재구성하여, CNF-클라우드 할당 행렬의 노이즈를 반복적으로 정제하는 그래프 신경망(GNN) 디노이저(denoiser)를 사용합니다. 이 모델은 제약 조건을 직접 손실 함수에 통합하여 실현 가능한 솔루션 공간을 학습할 수 있도록 설계되었습니다.

- **Technical Details**: 연구에서는 여러 클라우드에서 CNF 및 서비스 기능 체인(SFC)을 배치할 때의 제약 조건을 형식화하는 시스템 모델을 설명합니다. 클라우드는 독립적으로 관리되지만, 그래프 구조로 연결되어 있어 서로 간에 경로가 존재합니다. CNF는 서로 분리할 수 없는 원자화된 컴퓨팅 작업으로 정의되며, SFC는 이러한 CNF들의 부분적으로 정렬된 순서로 구성됩니다.

- **Performance Highlights**: 제안된 모델은 다양한 토폴로지에서 광범위한 평가를 수행한 결과, 기존의 혼합 정수 비선형 프로그래밍(MINLP) 솔버에 비해 수치적으로 실현 가능한 솔루션을 훨씬 빠른 추론 속도로 제공함을 입증하였습니다. 확산 모델 기반 생성 모델은 제약 조건이 있는 네트워크 임베딩 문제에 대한 잠재력을 보여 주며, 분산 CNF의 실용적이고 확장 가능한 오케스트레이션에 기여할 수 있는 가능성을 제시합니다.
...
### Koopman-based Prediction of Connectivity for Flying Ad Hoc Networks (https://arxiv.org/abs/2511.01286)
- **What's New**: 이번 논문은 기존의 정적 무선 환경에서의 머신러닝(Machine Learning) 기술을 넘어, 동적 환경인 비행 무선 애드혹 네트워크(FANETs)에서 UAV(무인 항공기) 궤적 동력을 모델링하기 위한 데이터 기반의 쿠프만(Koopman) 접근 방식을 탐구합니다. 이를 통해 UAV 간의 신뢰할 수 있는 통신을 보장하기 위한 예측 신호 대 간섭 및 잡음 비율(SINR) 예측이 가능해집니다. 이 접근 방식은 중앙 집중형 및 분산형 두 가지로 나뉘어 수행됩니다.

- **Technical Details**: 쿠프만 연산자 이론은 비선형 동적 시스템의 진화를 선형화하는 강력한 프레임워크입니다. 논문에서는 UAV 동력을 비선형 동적 시스템으로 모델링하고, 관찰 가능한 측정 기능을 사용하여 이를 쿠프만 연산자로 변환하여 시간이 지남에 따라 예측합니다. 또한, 비유클리드 데이터인 그래프 신호를 처리하기 위한 데이터 기반 접근 방식의 변형도 제시합니다.

- **Performance Highlights**: 모의 실험을 통해 제안한 두 가지 접근 방식의 효과성을 입증했습니다. 중앙 집중형 접근에서는 중앙 단위가 모든 UAV의 SINR을 예측하여 네트워크의 전반적인 연결 상태를 파악하고, 분산형 접근에서는 각 UAV가 독립적으로 미래의 SINR을 예측하여 자율적인 의사 결정을 가능하게 합니다. 이러한 예측 능력은 FANET 내의 격리 사건 예측에 있어 중요한 통찰력을 제공합니다.



### Lyapunov Stability Learning with Nonlinear Control via Inductive Biases (https://arxiv.org/abs/2511.01283)
Comments:
          Accepted by IEEE Robio 2025

- **What's New**: 이 논문에서는 비선형 동적 시스템에서 안정성을 보장하기 위해 Yeni 누적 후 입출력을 활용하는 새로운 접근법을 제안합니다. 기존의 learner-verifier 프레임워크에서 Lyapunov 조건을 복잡한 제약으로 처리할 때 글로벌 컨버전스를 달성하기 어려웠으나, 이 방법론은 Lyapunov 조건을 귀납적 편향으로 다루어 효율적인 최적화가 가능합니다. 또한, end-to-end 방식으로 CLF와 컨트롤러 학습을 가능하게 하여 안정적인 최적화 과정을 제공합니다.

- **Technical Details**: 우리는 비선형 제어-어피넌트 리프치츠 연속 동적 시스템을 고려하여 주어진 안정점 주변에서 제어 Lyapunov 함수(CLFS)의 조건을 정의했습니다. 논문에서는 Lyapunov 조건을 귀납적 편향으로 활용하여 Neural CLF 및 컨트롤러를 설계하며, 이러한 설계는 적은 제약으로 높은 수렴률과 더 넓은 매력 지역(ROA)을 달성하게 합니다. 이를 통해 CLF와 컨트롤러를 동시에 최적화하며, 깊은 학습 라이브러리만으로 검증 단계를 수행할 수 있습니다.

- **Performance Highlights**: 제안된 방법은 다양한 실험 케이스에서 기존 방법들과 비교하여 더 높은 수렴률과 더 넓은 매력 지역(ROA)을 달성하였습니다. 또한, 기존 방법이 학습 중 성공률이 감소하는 이유를 철저하게 분석하고 있으며, 이는 더욱 견고한 수렴을 가능하게 합니다. 본 방법은 복잡한 최적화 목표를 다루는 데 있어 강력한 성능을 발휘하며, 기존의 다른 방법들과 비교했을 때 더 나은 성과를 보입니다.



### Identification of Capture Phases in Nanopore Protein Sequencing Data Using a Deep Learning Mod (https://arxiv.org/abs/2511.01277)
- **What's New**: 이 연구에서는 단백질 캡처(critical capture) 및 전이(translocation)와 같은 중요한 분자 단계를 탐지하는 경량의 1차원 합성곱 신경망(1D CNN)을 개발했습니다. 기존 수동으로 이루어지던 캡처 단계의 식별을 자동화함으로써 분석 시간을 며칠에서 30분 이내로 단축할 수 있습니다. 이는 나노포어(nanopore) 단백질 시퀀싱의 실험적 용이성을 향상시키는 중요한 발전으로 평가됩니다.

- **Technical Details**: 모델의 이름은 CaptureNet-Deep으로, down-sampled 신호(window)에서 캡처 단계를 감지하도록 훈련되었습니다. CNN-LSTM(롱 쇼트-텀 메모리) 하이브리드, 히스토그램 기반 분류기 및 기타 CNN 변형들과 비교 평가한 결과 F1 점수 0.94와 정밀도 93.39%를 기록했습니다. 이 연구는 또한 저지연(inference) 추론을 지원하며, 옥스포드 나노포어의 실험 대시보드에 통합되어 있습니다.

- **Performance Highlights**: CaptureNet-Deep은 효율적이고 실시간(real-time) 캡처 탐지를 가능하게 하며, 간단하고 해석 가능(interpretable)한 아키텍처를 사용합니다. 이는 ML(머신 러닝) 모델이 시퀀싱 작업 흐름에서 더 넓은 역할을 할 수 있음을 시사합니다. 본 연구의 결과는 현대의 나노포어 단백질 서열 분석에 큰 기여를 하게 될 것입니다.
...
### A Spatio-Temporal Online Robust Tensor Recovery Approach for Streaming Traffic Data Imputation (https://arxiv.org/abs/2511.01267)
- **What's New**: 본 연구에서는 Intelligent Transportation Systems (ITS)에서 트래픽 데이터 회복 문제를 스트리밍 프레임워크 내에서 재구성하고, 글로벌 스페이셜-템포럴 상관관계(global spatio-temporal correlations)와 로컬 일관성(local consistency)을 동시에 활용하는 새로운 온라인 강건 텐서 회복 알고리즘을 제안합니다. 이 알고리즘은 대규모 시나리오에서 높은 회복 정확도를 달성하며, 계산 효율성을 크게 개선합니다. 특히, 결측치와 이상치를 동시에 처리할 수 있어 다양한 결측 패턴에 강한 적응성을 보여줍니다.

- **Technical Details**: 기존 배치 기반 방법들은 많은 컴퓨팅 및 저장 자원을 요구하며 ITS의 확장성에 제한을 두었습니다. 본 연구에서는 데이터를 수집할 때 지속적으로 생성되는 대규모 센서 네트워크에서의 온라인 처리 방법을 통해 이를 극복하고자 합니다. 제안된 방법은 글로발 스페이셜-템포럴 상관관계와 로컬 일관성을 통합하여 데이터를 회복하며, 실시간으로 데이터를 수집하고 처리받을 수 있는 능력을 갖추고 있습니다.

- **Performance Highlights**: 세 가지 실제 트래픽 데이터셋에 대한 실험 결과, 제안된 접근 방식은 최신 배치 기반 방법에 비해 최대 3배 이상의 계산 효율성을 보이며 높은 회복 정확도를 달성했습니다. 이는 복잡한 시나리오에서 저 샘플링 비율과 구조적 결측 패턴을 포함한 다양한 환경에서도 뛰어난 성능을 발휘함을 나타냅니다. 따라서 이 방법은 현대 ITS 애플리케이션에서 효율적인 실시간 데이터 관리의 필요성에 적합합니다.



### KAT-GNN: A Knowledge-Augmented Temporal Graph Neural Network for Risk Prediction in Electronic Health Records (https://arxiv.org/abs/2511.01249)
Comments:
          10 pages, 3 figures

- **What's New**: 이 연구는 전자 건강 기록(EHRs)을 사용한 임상 위험 예측을 위한 KAT-GNN(지식 증강 시계열 그래프 신경망)라는 새로운 그래프 기반 프레임워크를 제안합니다. KAT-GNN은 환자 그래프를 생성하고, 두 가지 지식 출처를 통해 그래프를 풍부하게 하여 다양한 임상 작업에 대한 효과적이고 일반화 가능한 접근 방식을 제공합니다.

- **Technical Details**: KAT-GNN은 임상 지식과 시계열 동적 요소를 통합하여 위험 예측 성능을 향상시킵니다. 방법론적으로, 이 프레임워크는 EHR에서 추출한 모달리티별 환자 그래프를 구축하고, SNOMED CT에서 유래한 온톨로지 기반 엣지 및 EHR에서 추출된 공존 우선 순위를 통해 그래프를 보강합니다. 그런 다음 시간 인식 변환기를 통해 그래프에서 후속 환자 표현의 장기적 동태를 캡처합니다.

- **Performance Highlights**: KAT-GNN은 세 가지 데이터 세트에서 위험 예측을 수행하고 최첨단 성능을 기록합니다. 관상동맥 질환 예측에서 0.9269의 AUROC를 달성하였으며, MIMIC-III와 MIMIC-IV 데이터 세트에서의 병원 내 사망 예측에 있어서도 각각 0.9230 및 0.8849의 AUROC를 기록하였습니다. 이러한 결과는 KAT-GNN이 기존의 GRASP 및 RETAIN과 같은 신뢰할 수 있는 기초선 모델을 지속적으로 초과 달성한 것을 증명합니다.



### A Saddle Point Remedy: Power of Variable Elimination in Non-convex Optimization (https://arxiv.org/abs/2511.01234)
- **What's New**: 이 논문에서는 머신러닝의 대규모 비볼록( non-convex) 최적화에서 주요 장애물로 여겨지는 saddle points에 대한 새로운 해석을 제공합니다. 기존의 variable elimination 알고리즘인 Variable Projection (VarPro) 등을 활용하여 비로소 이 복잡한 에너지 구조를 효과적으로 탐색하는 이유를 기하학적 관점에서 명확히 설명합니다. 이를 통해 최적화 문제의 국소 최대값이 원래 문제의 saddle points와 직접적으로 연관되어 있음을 밝혀냈습니다.

- **Technical Details**: 논문에서는 Hessian inertia와 Schur complement에 기반한 엄밀한 분석을 통해 variable elimination이 최적화 문제의 critical point 구조를 재편성함을 증명하였습니다. 결과적으로 variable elimination을 통해 복잡한 최적화 경관을 간소화하고 saddle points를 제거하여 성능을 향상시킬 수 있음을 보여줍니다. 이 기법은 비볼록 행렬 분해 문제와 같은 일반적인 문제에 적용되어 다양한 신경망에서 검증되었습니다.

- **Performance Highlights**: 이 연구에서 제안된 방법은 깊은 Residual Networks의 훈련 시 안정성과 수렴성 모두에서 극적인 향상을 보여주었습니다. 특히, Rosenbrock 함수와 같은 실용적인 예시를 통해 변수 제거 방법이 어떻게 이러한 문제에서 더 나은 최솟값을 찾는 데 효과적인지를 보여주여, 기존 접근법에 비해 더 높은 품질의 해를 제공하는 것으로 나타났습니다. 이러한 결과는 향후 더욱 강력하고 효율적인 최적화 알고리즘 설계에 중요한 기초가 될 것입니다.



### WindMiL: Equivariant Graph Learning for Wind Loading Prediction (https://arxiv.org/abs/2511.01226)
- **What's New**: 본 논문에서는 WindMiL이라는 새로운 기계 학습 프레임워크를 소개합니다. 이 프레임워크는 시스템적으로 생성된 데이터셋과 대칭을 인식하는 그래프 신경망(GNN)을 결합하여 저층 건물의 바람 하중 예측을 개선합니다. WindMiL은 기존의 비싼 풍동 실험이나 대규모 에디 시뮬레이션(LES)의 한계를 극복하고, 구속된 대칭과 물리적 일관성을 유지하는 모델을 제공합니다.

- **Technical Details**: WindMiL은 저층 건물의 지붕 형상과 바람 방향을 다양화하여 462개의 사례를 시뮬레이션한 대규모 데이터셋을 생성합니다. 이를 위해 반사 대칭성을 확보한 그래프 신경망을 개발하여 물리적으로 일관된 예측을 보장합니다. 이 모델은 표면 압력 계수의 평균과 표준 편차에서 높은 정확도를 달성하며(RMSE $	extless$ 0.02), 반사 테스트 평가에서도 96	ext{%} 이상의 적중률을 유지합니다.

- **Performance Highlights**: WindMiL은 기존의 비대칭적 기반 모델에 비해 대칭 기하학에서 오류를 10	ext{%} 이상 줄이는 성과를 보였습니다. 추가로, WindMiL은 건물의 바람 하중 예측에서 효율적이고 확장 가능한 솔루션을 제공하며, 데이터 기반이고 대칭 인식이 가능하다는 점에서 도시 설계의 지속 가능성을 높일 수 있습니다.



### Optimizing Electric Vehicle Charging Station Placement Using Reinforcement Learning and Agent-Based Simulations (https://arxiv.org/abs/2511.01218)
Comments:
          Under Review

- **What's New**: 본 논문에서는 전기차(Electric Vehicles, EV)의 급속한 성장에 따라 최적의 충전소 위치를 결정할 수 있는 새로운 프레임워크를 제안합니다. 기존의 결정론적 보상 시스템의 한계를 극복하기 위해, 심층 강화 학습(Deep Reinforcement Learning, DRL)과 에이전트 기반 시뮬레이션(Agent-Based Simulation)을 통합하여 충전 수요를 실시간으로 추정할 수 있도록 했습니다. 특히, 하이브리드 보상 함수와 듀얼 Q-네트워크를 활용하여 충전소 위치 선택 및 포트 구성을 최적화합니다.

- **Technical Details**: 제안된 프레임워크는 GAMA 모델을 이용해 전기차의 이동과 충전 수요를 시뮬레이션하는 시스템을 포함합니다. 이 시스템은 환경이 시간에 따라 동적으로 변화하는 것을 반영할 수 있어, 실제 상황에서의 사용자 수요와 교통 흐름을 효과적으로 모델링할 수 있습니다. 실험에서는 베트남 하노이를 사례로 하여 평균 대기 시간을 기존 상태에 비해 53.28% 단축하는 성과를 달성했습니다.

- **Performance Highlights**: 제안된 방법은 하노이에서의 실증 연구를 통해 전통적인 방법들보다 더 나은 성능을 보였습니다. 특히, 정적인 기준선을 초과하여 자원의 효율적인 할당과 더불어 사용자 경험을 개선하는 데 유용성이 입증되었습니다. 이 연구는 EV 인프라 계획을 위한 확장 가능하고 적응 가능한 솔루션을 제공합니다.
...
### Transmitter Identification and Protocol Categorization in Shared Spectrum via Multi-Task RF Classification at the Network Edg (https://arxiv.org/abs/2511.01198)
- **What's New**: 이 연구에서는 미래의 증가하는 무선 수요를 충족하기 위한 스펙트럼 공유의 필요성을 강조하며, 강력한 송신기 식별 및 프로토콜 분류 프레임워크를 제안합니다. 제안된 방법은 무선 신호를 다중 작업으로 분류하여 동일 주파수 대역에서 동작하는 여러 송신 프로토콜(예: 4G LTE, 5G NR, IEEE 802.11a)을 식별할 수 있도록 디자인되었습니다. 이 방법은 CNN(Convolutional Neural Network)을 활용하여 중복 신호 특성과 환경 변동성을 효과적으로 처리할 수 있도록 만들어졌습니다.

- **Technical Details**: 제안된 방법은 다중 채널 입력 전략을 사용하여 의미 있는 신호 특성을 추출하고, 이를 통해 프로토콜 분류에서 90%, 송신 기지국 분류에서 100%, 연합 분류 작업에서 92%의 놀라운 정확도를 달성했습니다. RF 데이터는 POWDER 플랫폼을 사용하여 수집되었습니다. 이 방법은 자원이 제한된 네트워크 엣지에서 사용할 수 있는 가벼운 CNN 기반 솔루션으로 설계되었습니다.

- **Performance Highlights**: 이 연구의 성과는 스펙트럼 모니터링, 관리 및 보안을 향상시키는 데 중요한 잠재력을 보여줍니다. 특히, 송신 기지국 및 프로토콜을 동시에 분석하여 중복 신호 특징으로 인한 복잡성을 해결하며, 기존 방법의 한계를 극복하는 데 성공했습니다. 이러한 결과는 스펙트럼 사용 정책을 엄격히 준수하지 않는 사용자나 악의적 사용자를 식별할 수 있는 가능성을 제공합니다.



### Analyzing the Power of Chain of Thought through Memorization Capabilities (https://arxiv.org/abs/2511.01190)
- **What's New**: 이 논문은 체인 오브 사고(Chain of Thought, CoT)가 대규모 언어 모델(LLMs)의 수학적 추론 문제 해결 능력을 향상시킨다는 기존 연구를 바탕으로, CoT의 능력에 대한 더 깊은 분석을 제공합니다. 특히, CoT가 모든 추론 작업에 대한 변환기(transformer)의 능력을 확장하는지에 대한 질문에 부정적인 답을 제시했습니다. 고정 정밀도 변환기의 기억 능력을 평가하고, CoT가 있는 경우와 없는 경우에 대한 조건을 명확히 했습니다.

- **Technical Details**: 논문에서는 고정 정밀도 변환기와 CoT의 결합이 유한 추론 데이터셋을 기억하기 위한 필요 충분 조건을 제공합니다. 이 조건들이 서로를 함의하지 않음을 보여주고, CoT가 없거나 있는 경우 메모리 저장을 위한 매개변수의 하한과 상한을 제시합니다. 이러한 매개변수는 모든 경우에 대해 Θ~(N)입니다.

- **Performance Highlights**: 연구 결과, CoT는 특정 작업에서 성능 향상을 가져오지만, 일부 추론 작업에서는 CoT가 변환기의 추론 능력을 향상시키지 않는다는 사실이 밝혀졌습니다. 또한 CoT 변환기가 유한 데이터셋 일부를 기억할 수 없다는 최초의 결과를 제공하며, 이는 CoT가 선택한 일부 무한 데이터셋에서도 마찬가지라는 것을 나타냅니다. 이론적 인사이트 외에도, 메모리 변환기의 매개변수 수에 대한 타이트한 경계는 실제 응용에 유용한 가이드를 제공합니다.



### A Comparative Study of Model Adaptation Strategies for Multi-Treatment Uplift Modeling (https://arxiv.org/abs/2511.01185)
- **What's New**: 최근 개별화된 치료 효과 추정에 있어 Uplift modeling이 중요한 기법으로 자리 잡고 있으며, 다중 치료 시나리오에서의 효과 모델링이 실질적인 응용 분야에서 핵심적 역할을 하고 있습니다. 본 연구는 기존의 이진 치료 기술을 다중 치료에 맞춰 조정한 여러 모델들을 두 가지 유형, 즉 Structure Adaptation과 Feature Adaptation으로 분류합니다. 더불어, 우리는 함수 근사 정리에 기반한 새로운 알고리즘인 Orthogonal Function Adaptation(OFA)를 제안합니다.

- **Technical Details**: 논문에서는 Neyman-Rubin 잠재적 결과 프레임워크를 기반으로 다중 치료의 적응을 논의하며, 모델의 구조를 조정하여 치료 변수와 특징 정보를 결합하는 두 가지 방법을 설명합니다. OFA는 윌러스트라스 근사 정리를 이용한 다중 치료 모델링 방법으로, 다양한 데이터 조건에서 모델 일반화를 개선할 수 있도록 설계되었습니다. 이 방법은 최소한의 계산적 오버헤드로 모델의 성능을 향상시키며, 각 잠재 함수에 대한 계수를 생성하는 방식을 채택하고 있습니다.

- **Performance Highlights**: 실험을 통해 기존의 여러 적응 기법들과 OFA의 성능을 비교하였으며, OFA가 다양한 데이터 특성 하에서 가장 높은 성능과 강건성을 보여주었음을 입증했습니다. 또한, OFA가 여러 실제 데이터셋 및 합성 데이터셋에 걸쳐 일관되게 우수한 결과를 나타냈다는 점이 강조되었습니다. 전체적으로 OFA는 혼합 데이터와 상관작용 요인을 고려하는 데 있어 현저한 개선을 보여주는 것으로 나타났습니다.
...
### Regularization Implies balancedness in the deep linear network (https://arxiv.org/abs/2511.01137)
Comments:
          18 pages, 3 figures

- **What's New**: 이 논문은 Deep Linear Network (DLN)의 기하학적 불변 이론(Geometric Invariant Theory, GIT)을 이용하여 심층 신경망의 훈련 동역학을 분석합니다. Kempf-Ness 정리를 통해 L² 정규화가 balanced manifold에서 최소화되며, 이 두 가지 기울기 흐름으로 훈련이 이루어짐을 밝히고 있습니다. 이러한 접근은 신경망 훈련에서 발생하는 hidden convexity를 설명하며, 딥러닝과 선형 시스템 이론에서의 balancedness(균형 상태)에 대한 공통의 수학적 틀을 제공합니다.

- **Technical Details**: 논문에서는 balancedness를 위한 최소 원리를 연구하며, fiber와 balanced variety라는 두 가지 기하학적 구조를 대비하여 hidden convexity를 드러냅니다. 특히, fiber는 end-to-end 매트릭스에 대한 대수적 다양체이며, balanced variety는 균형 잡힌 파라미터 표현이 가능한 매트릭스의 모임으로 정의됩니다. L² 정규화는 Occam's razor(옥컴의 면도날)의 형태로 해석되며, balancedness 개념은 선형 시스템 이론과 딥러닝 이론 간의 통합을 통해 설명됩니다.

- **Performance Highlights**: DLN에서의 regularization은 가장 간단한 매개변수 표현을 생성하도록 설계되어 있으며, 이는 Occam's razor와 유사한 역할을 수행합니다. 연구 결과, balancedness는 선형 시스템 이론 및 딥러닝에서 다양한 이점을 제공하며, 이러한 통합적 접근은 모델 축소 및 베이지안 원칙으로 해석될 수 있습니다. 이로 인해 기계 학습 모델의 효과적 적용 및 개선이 가능해질 것입니다.
...
- **What's New**: 이 논문에서는 Online bilevel optimization (OBO) 문제를 위한 새로운 접근 방식을 제시합니다. 기존 OBO 방법들이 사용하는 	extit{window-smoothed} 회귀 최소화는 함수가 빠르게 변하는 경우에 시스템 성능을 정확하게 반영할 수 없습니다. 제안된 방법은 새로운 탐색 방향을 활용하여 서브선형(stochastic bilevel regret) OBO 알고리즘을 구현합니다.

- **Technical Details**: 우리는 첫 번째 및 제로 순서(zeroth-order) 확률적 OBO 알고리즘을 개발하여 window smoothing 없이 OBO를 해결합니다. 이 프레임워크는 하이퍼그래디언트 추정에서 오라클(oracle) 의존도를 줄이고, 선형 시스템 해와 함께 내부 및 외부 변수를 업데이트하며, Hessian, Jacobian, 그리고 그래디언트의 ZO 기반 추정을 사용합니다. 이를 통해 효율성을 크게 향상시킵니다.

- **Performance Highlights**: 논문에서 제안하는 방법은 온라인 파라메트릭 손실 조정과 블랙박스 적대적 공격과 같은 실험에서 그 유효성을 입증하였습니다. 실험 결과, 새로운 접근 방식이 기존 방법보다 더 나은 성능을 발휘함을 보여줍니다. 이는 OBO 문제의 다양한 실제 적용 가능성을 강조합니다.



### One model to solve them all: 2BSDE families via neural operators (https://arxiv.org/abs/2511.01125)
- **What's New**: 본 논문에서는 전통적인 신경 연산자 모델의 완화된 생성적 변종을 제안합니다. 이 모델은 Kolmogorov--Arnold 네트워크를 활용하여 랜덤 종단 시간(random terminal time)을 가진 무한한 패밀리의 2차역 확률 미분 방정식(2BSDEs)을 해결합니다. 첫 번째 주요 결과는 다양한 2BSDE 패밀리와 관련된 솔루션 연산자가 적절한 신경 연산자 모델에 의해 근사 가능함을 보여줍니다.

- **Technical Details**: 각 (g, f₀) 쌍에 대해, 제안된 방법은 적절한 파라미터 수의 다항식(m polynomial)으로 신경 연산자 근사를 요구하는 구조적 하위 클래스의 2BSDE 패밀리를 식별합니다. 이 과정에서 Sobolev 공간 W^{1,p}의 특정 부분 집합을 활용하며, 각 2BSDE는 일반적으로 비선형 쌍 대칭 연산자(pseudo-development)와 무관하게 정의됩니다. 이 결과는 특히 최적 제어(optimal control) 및 수리 재무(mathematical finance) 분야에 응용될 수 있습니다.

- **Performance Highlights**: 제안된 신경 연산자 모델은 기존 모델을 확장하여 2BSDEs에 대한 유니버설 근사 이론(universal approximation theorem)을 설립하였고, 이를 통해 실제 애플리케이션에서 수치적 안정성을 확보합니다. 결과적으로, 이 논문은 복잡한 비선형 편미분방정식(PDE) 문제를 효과적으로 해결할 수 있는 새로운 방법론을 제시하여 신경망이 확률론적 시스템에서의 효용을 향상시킵니다.
...
### AI Progress Should Be Measured by Capability-Per-Resource, Not Scale Alone: A Framework for Gradient-Guided Resource Allocation in LLMs (https://arxiv.org/abs/2511.01077)
Comments:
          9 pages (main) + appendix, 3 figures. Accepted at NeurIPS 2025 (Position Paper Track), submission #491. OpenReview: this https URL

- **What's New**: 이번 논문은 AI 연구에서 지배적인 "스케일링 근본주의(Scaling Fundamentalism)"에 도전장을 내밀며, 모델 크기와 계산 능력의 무제한 성장이 환경에 미치는 영향을 강조합니다. 저자들은 LLM 개발이 단순한 능력이 아닌 자원 대비 능력(ability-per-resource)으로 재편되어야 한다고 주장하며, 이를 위한 이론적 프레임워크를 제시합니다. 구체적으로, 기울기(gradient) 영향을 기반으로 한 자원 배분 결정을 통해 AI 생태계 전반의 효율성을 크게 향상시킬 수 있음을 보여줍니다.

- **Technical Details**: 이론적 기초를 바탕으로, 저자들은 두 가지 주요 차원에서의 자원 효율성을 보여줍니다: 1. 파라미터 효율성(parameter efficiency) - 실제 파워 법칙(power-law) 분포 하에서 가장 영향력 있는 파라미터만 업데이트하는 것이 전체 파라미터 조정보다 자원 대비 성능을 높일 수 있다는 것을 증명합니다. 2. 데이터 효율성(data efficiency) - 모든 훈련 데이터가 동일하게 학습에 기여하지 않으며, 고-gradient 훈련 데이터 샘플을 선별해 선택적으로 조정하는 방법을 제안합니다. 이와 더불어, gradient blueprints로 알려진 메타데이터를 통해 특정 태스크에 가장 중요한 파라미터를 식별할 수 있습니다.

- **Performance Highlights**: 저자들은 최근의 연구 결과를 통해 3가지 주요 발견을 제시합니다: 1. 트랜스포머 모델에서 기울기 분포가 파워 로우 패턴을 따르며, 적은 수의 파라미터가 비정상적으로 큰 영향을 미친다는 사실. 2. 간단한 gradient norms가 복잡한 영향 측정치를 효과적으로 근사할 수 있으며, 두 번째 차수 계산을 요구하지 않습니다. 3. 선택적 파라미터 업데이트를 통해 메모리 절약이 가능해 질문으로 최적화 상태를 관리할 수 있다는 점입니다. 이러한 접근 방식은 AI 능력에 대한 접근성을 민주화하고 환경적 영향을 줄이는데 기여합니다.



### Happiness as a Measure of Fairness (https://arxiv.org/abs/2511.01069)
- **What's New**: 이 논문에서는 행복(happiness) 개념을 기반으로 한 새로운 공정성(fairness) 프레임워크를 제안합니다. 이러한 접근 방식은 결정 결과로부터 각 집단이 얻는 효용(utility)을 측정함으로써, 더욱 인간 중심적인 방법을 제공합니다. 또한, 이 방법은 수학적으로도 엄밀하며, 최적의 공정한 후처리(post-processing) 전략을 계산하기 위해 필요한 것은 단지 선형 프로그램(linear program)을 해결하는 것뿐입니다.

- **Technical Details**: 제안된 공정성 프레임워크는 효율적이고 확장 가능한 방식으로 기존의 최적화 도구와 호환됩니다. 이 방법은 여러 잘 알려진 공정성 정의를 통합하고 확장하는 기능을 가지고 있으며, 공정성을 직관적인 렌즈를 통해 포착합니다. 이러한 접근 방식은 데이터 분석이나 의사결정 지원 시스템에서 유용할 수 있습니다.

- **Performance Highlights**: 실험 결과는 다양한 시나리오에서 제안된 프레임워크의 실제적 강점을 강조합니다. 특히, 이 방법이 공정성 유지와 동시에 최적의 결과를 창출하는 데 유리함을 입증합니다. 이를 통해 공정성을 고려한 머신 러닝 모델의 디자인에 중요한 기여를 할 수 있다는 점을 보여주고 있습니다.
...
- **What's New**: 이 연구에서는 COLET 데이터 세트를 활용하여 인지 작업량(cognitive workload)에 대한 분류를 수행하는 새로운 방법론을 제안합니다. 눈 추적 데이터의 시계열(time-series) 특징 생성을 위한 윈도우 기반(window-based) 접근 방식을 채택하여, 기계 학습 및 딥 러닝을 이용한 분류 작업을 수행합니다. 연구 결과에 따르면, 딥 러닝 모델, 특히 테이블 구조(tabular architecture)가 전통적인 기계 학습 방법보다 우수한 성능을 보였습니다.

- **Technical Details**: COLET 데이터 세트는 동기화된 눈 추적 및 작업 수행 데이터로 구성되어 있으며, 세 가지 수준의 인지 작업량(Low, Medium, High)으로 분류할 수 있습니다. 연구에서는 TabNet이라는 딥 러닝 아키텍처를 활용하여, 희소 마스크(sparsity-mask)를 가진 피처(feature) 제공과 같은 기능을 통해 주요 특성의 패턴을 포착합니다. 또한, 각 눈 추적 데이터의 연속 스트림을 윈도우 단위로 분할하여 인지 부하의 변화를 감지하는 데 중점을 두었습니다.

- **Performance Highlights**: 연구에서는 딥 러닝 모델이 전통적인 기계 학습 모델에 비해 더 높은 정확도와 F1-score를 달성하는 성과를 보였습니다. 특히, 윈도우 기반 접근 방식이 실시간 인지 작업량 평가에 효과적임을 보여주었습니다. 이는 복잡하고 동적인 작업 환경에서의 인지 부하를 잘 감지할 수 있게 해줍니다.



### MedEqualizer: A Framework Investigating Bias in Synthetic Medical Data and Mitigation via Augmentation (https://arxiv.org/abs/2511.01054)
- **What's New**: 이 연구에서는 MIMIC-III 데이터셋을 사용하여 여러 Generative Adversarial Network (GAN) 기반 모델이 생성한 합성 데이터의 공정성을 평가합니다. 중요한 인구통계학적 특성에 대한 대리성을 중점적으로 점검하고, 합성 데이터에서 특정 하위 그룹이 과대 또는 과소 대표되는 문제를 확인합니다. 이를 해결하기 위해 MedEqualizer라는 모델 무관한 증강 프레임워크를 소개하여 다양한 하위 그룹의 대표성을 높이는 방안을 모색합니다.

- **Technical Details**: 이 연구에서 비교 검토된 여러 모델은 MedGAN, HealthGAN, Conditional Tabular GAN (CTGAN) 등입니다. MedEqualizer는 합성 데이터 생성을 위해 기본 데이터 배포에 미치는 영향을 최소화하면서도 공정성을 대폭 개선하는 방법을 제시합니다. 특히, 자동 인코더와 GAN 구조를 결합하여 고차원 이산 환자 기록을 효과적으로 생성할 수 있도록 설계되었습니다.

- **Performance Highlights**: MedEqualizer는 결과적으로 생성된 합성 데이터의 인구 통계학적 균형을 효율적으로 향상시켜, 보다 공정하고 대리적인 의료 데이터 합성을 가능하게 합니다. 연구 결과는 합성 데이터를 활용한 의료 연구 및 결정-making 프로세스에서 신뢰성과 유용성을 높이는 데 기여할 것입니다. 이러한 접근 방식은 다양한 의료 데이터 생성 시나리오와의 통합이 가능하도록 설계되었습니다.



### SARIMAX-Based Power Outage Prediction During Extreme Weather Events (https://arxiv.org/abs/2511.01017)
Comments:
          12 pages, 3 figures. This paper presents the solution of Team 12 for the 2025 INFORMS Data Mining Society Data Challenge. The open-source code is available at: this https URL

- **What's New**: 이번 연구는 극단적인 기상 현상 중 단기 정전 예측 시스템을 개발하는 것을 목표로 하고 있습니다. 미시간 카운티의 시간별 정전 데이터와 날씨 특성 데이터를 활용하여 SARIMAX 기반의 예측 모델을 구현하였습니다. 이 모델은 정전 패턴의 시간적 의존성을 포착하기 위해 다양한 피처를 통합하고, 강화된 특성 공학을 통해 데이터를 처리합니다.

- **Technical Details**: 연구는 두 단계의 피처 엔지니어링 파이프라인을 통해 정전 예측을 위한 필수적인 37개의 날씨 특성을 도출하였습니다. 첫 번째 단계에서는 15개의 제로 분산 피처와 10개의 의미 불명 피처를 제거하고, 결측값 보간 및 이상값 수정을 수행했습니다. 두 번째 단계에서는 군집 분석과 주성분 분석(PCA)을 통해 84개의 날씨 특성을 37개의 대표적인 피처로 압축하여 최종 피처 세트를 구성했습니다.

- **Performance Highlights**: 모델은 단기(24시간) 및 중기(48시간) 예측에 대해 각각 최적화되었으며, RMSE는 177.2로, 기존 방안에 비해 8.4% 개선된 성능을 보였습니다. 이 연구는 강력한 최적화 전략과 피처 공학의 효과성을 확인하는 성과를 거두었습니다. 따라서 극단적인 기상 상황 시 전력 정전 예측의 신뢰성이 향상되었습니다.



### What's the next frontier for Data-centric AI? Data Savvy Agents (https://arxiv.org/abs/2511.01015)
Comments:
          Presented at ICLR 2025 Data-FM. Seedat & Liu contributed equally

- **What's New**: 최근 AI 에이전트는 자율적으로 소통하고 협력하며 다양한 도구를 사용하는 능력을 보여주고 있습니다. 그러나 데이터 처리를 어떻게 수행하는지에 대한 연구는 아직 미흡합니다. 이 논문은 AI 에이전트의 설계에서 데이터 지능(data-savvy) 능력을 최우선으로 삼아야 한다고 주장합니다. 특히, 데이터 수집, 고급 데이터 처리, 상호작용 테스트 데이터 생성, 지속적인 적응 등 네 가지 핵심 능력을 제안합니다.

- **Technical Details**: 이 논문에서는 데이터 중심 AI의 다음 단계로, 데이터 지능 에이전트의 네 가지 주요 기능을 정의합니다. 이러한 기능은 각각의 난이도에 따라 연구 방향이 분류되며, "쉬운" 부분은 기존 기계 학습 연구와 밀접하게 연관되어 있습니다. "중간" 부분은 논리적 사고와 데이터 처리 문제를 더 심도 있게 다루고, "어려운" 부분은 인간 전문가 및 일반 AI 시스템과 협력할 수 있는 에이전트를 개발하는 데 중점을 두고 있습니다.

- **Performance Highlights**: 데이터 지능 에이전트는 능동적으로 데이터를 수집하고 처리하여 다양한 환경과 요구 사항에 적응할 수 있습니다. 논문은 데이터 유용성, 비용, 개인 정보 보호 문제의 균형을 맞추고, 에러 발견 및 성능 저하에 대한 원인 규명 등의 문제를 강조합니다. 이러한 접근법은 AI가 정적 데이터 세트에 의존하지 않고, 실시간으로 데이터 처리 및 적응을 수행할 수 있는 기회를 제공합니다.



### Equality Graph Assisted Symbolic Regression (https://arxiv.org/abs/2511.01009)
- **What's New**: 이번 논문에서는 symbolic regression (SR)을 위한 새로운 검색 알고리즘, SymRegg를 제안합니다. 이전 연구에서 발견된 불필요한 계산을 피하고 탐색의 효율성을 높이기 위해 equality graph (e-graph) 구조를 활용하여 동등한 표현식을 압축 저장합니다. SymRegg는 기존의 genetic programming (GP) 알고리즘과는 달리 인구 기반 접근 방식을 사용하지 않으며, 최소한의 하이퍼파라미터만으로도 일관된 정확성을 유지합니다.

- **Technical Details**: SymRegg 알고리즘은 e-graph에서 선택한 표현식 샘플에서 솔루션을 왜곡하여 새로운 표현식을 생성하고, 그 표현식이 방문되지 않은 경우 이를 e-graph에 삽입하며, 동등한 형태를 생성하는 방식으로 진행됩니다. 이 과정에서 표현식의 등가 관계를 관리하는 equality saturation 알고리즘을 결합하여 중복 평가를 피하고 상황에 맞는 솔루션 탐색을 최적화 합니다. 논문에서는 e-graph를 통해 표현식의 동등성을 탐지하고, 이를 통해 중복되는 표현식의 방문을 줄이는 기술적 접근을 논의합니다.

- **Performance Highlights**: 실험 결과 SymRegg는 다양한 데이터 세트에서 GP를 초월하는 효율적인 검색 성능을 보여줍니다. SymRegg는 우수한 품질의 가설을 찾기 위한 탐색을 효율적으로 만드는 동시에 불필요한 컴퓨팅 자원을 절약했습니다. 이 알고리즘은 단순한 사용 경험을 제공하면서 진행 중인 연구에서 효과적인 탐색 공간 탐색을 가능하게 합니다.



### None To Optima in Few Shots: Bayesian Optimization with MDP Priors (https://arxiv.org/abs/2511.01006)
- **What's New**: 이 논문에서는 Procedure-inFormed Bayesian Optimization (ProfBO) 알고리즘을 소개합니다. ProfBO는 매우 적은 수의 함수 평가로 블랙박스 최적화를 해결할 수 있도록 설계된 방법입니다. Markov Decision Process (MDP) priors를 활용하여 관련 소스 작업의 최적화 경로를 모델링하며, 이를 통해 과거의 평가 데이터를 효과적으로 이용합니다.

- **Technical Details**: ProfBO는 Prior-Fitted Neural Network (PFN)를 채택하여 MDP priors를 매핑하고, Model-Agnostic Meta-Learning (MAML)으로 새로운 목표 작업에 빠르게 적응합니다. 이 알고리즘은 기존의 Gaussian Process (GP) 모델보다 훨씬 적은 평가로도 최적화 성능이 두드러지는 특징을 갖습니다. 결과적으로, ProfBO는 저비용의 평가로도 고품질 솔루션을 도출할 수 있습니다.

- **Performance Highlights**: 실제 Covid 및 Cancer 벤치마크에서 ProfBO는 기존 최첨단 방법보다 일관되게 우수한 성능을 보여줍니다. 실험에서는 20회 미만의 평가로도 높은 품질의 솔루션을 제공하여 실제 적용 가능성을 높였습니다. 이 연구는 최소한의 평가로 과학적 발견을 가속할 수 있는 혁신적인 접근 방식을 제시합니다.



### Hydra: Dual Exponentiated Memory for Multivariate Time Series Analysis (https://arxiv.org/abs/2511.00989)
- **What's New**: 최근 다변량 시계열 모델링의 중요성이 증가하고 있으며, 이를 통해 헬스케어, 금융, 에너지 관리 등 다양한 분야에서 활용되고 있습니다. 기존의 모델들은 시계열의 시간적 동역학을 포착하는 데 한계가 있어 새로운 접근 방식이 필요했습니다. 이를 해결하기 위해, 본 논문에서는 시간 및 변량 차원을 모두 고려한 2차원 메타 인컨텍스트 메모리 모듈인 Hydra를 제안합니다.

- **Technical Details**: Hydra는 각 단계에서 시간과 변량에 걸쳐 2차원 순환을 수행하는 혁신적인 구조로 설계되었습니다. 이러한 구조는 과거 데이터의 예외적인 패턴을 메모리에서 우선 수집하여 메모리의 희소성을 보장합니다. 또한 새로운 2D 청크 훈련 알고리즘을 통해 효율성을 극대화하여 훈련 과정에서 발생할 수 있는 문제를 해결합니다.

- **Performance Highlights**: 다양한 시계열 예측, 분류 및 이상 탐지 과제를 포함한 실험 결과에서 Hydra는 최신 기술 대비 우수한 성능을 보였습니다. 특유의 설계로 인해 Hydra는 보다 정확하게 시계열의 특성을 학습할 수 있으며, 기존 모델에 비해 월등한 성능 향상을 보여주었습니다.



### Balanced Multimodal Learning via Mutual Information (https://arxiv.org/abs/2511.00987)
- **What's New**: 이번 연구에서는 모달리티 불균형(modality imbalance) 문제를 해결하기 위한 새로운 융합 프레임워크를 제안합니다. 제안된 프레임워크는 상호 정보(mutual information)를 활용하여 다양한 데이터모달리티 간의 상호작용을 정량화합니다. 또한, 크로스 모달 지식 증류(cross-modal knowledge distillation)과 멀티태스크 스타일의 학습 방식을 도입하여 불균형을 완화하고 전반적인 모델 성능을 개선합니다.

- **Technical Details**: 제안된 통합 프레임워크는 수정된 그래프 컨볼루션 네트워크(r-GCN)를 사용하여 각 모달리티를 효과적으로 인코딩합니다. 이 구조는 크로스 샘플 간의 유사성을 활용하여 특징을 통합하고, 기준에 따라 모달리티의 학습 기여도를 조정하는 멀티태스크 스타일의 최적화 절차를 포함합니다. 이어지는 학습 단계에서, 성능 메트릭에 기반하여 각 모달리티의 기여도를 동적으로 조정하면서 효과적으로 모달리티 불균형 문제를 해결합니다.

- **Performance Highlights**: 본 연구는 유방암(브레스트 캔서) 분류를 위해 여러 데이터셋을 활용하여 실험을 진행하였으며, 제안된 방법이 전통적인 방법보다 더 나은 성과를 보임을 입증했습니다. 특히, 결합된 그래프 인코더와 상호 정보 기반의 지식 증류 방식은 모달리티 불균형을 완화하고 주요 성능 지표인 macro-F1을 향상시키는 데 기여했습니다. 또한, 소규모 샘플과 낮은 정보의 채널에서도 견고한 성능을 유지하는 것을 확인하였습니다.



### Modeling Microenvironment Trajectories on Spatial Transcriptomics with NicheFlow (https://arxiv.org/abs/2511.00977)
Comments:
          37 pages, 15 figures, to appear in NeurIPS 2025

- **What's New**: NicheFlow는 연속적인 공간 슬라이드에서 세포 미세환경의 시간적 궤적을 추론하는 흐름 기반의 생성 모델로, 지역 세포 이웃을 포인트 클라우드(point cloud)로 표현하고 세포 상태의 발전과 공간 좌표를 최적 수송(optimal transport) 및 변분 흐름 매칭(Variational Flow Matching) 기법을 통해 공동 모델링합니다. 이 모델은 다양한 공간적 시간적 데이터 세트에서 세포 유형 구성 및 공간 구조를 복구해내며, 배아 개발부터 뇌 발전까지 모든 단계를 아우릅니다.

- **Technical Details**: NicheFlow는 세포 니치(niche)의 시 공간적 동역학을 학습하는 생성을 기반으로 한 모델로, 다양한 공간 단면에서 세포 이웃의 역학을 단일 셀 궤적이 아닌 응집력 있는 단위로 직접 모델링합니다. 이 과정에서는 공간 회복(spatial reconstruction)과 생물학적 충실성(biofidelity)을 위한 분포적 가정으로 정제된 손실을 사용한 분리된 변분 흐름 매칭(factorized Variational Flow Matching) 방식을 결합하고, 주어진 미세 환경의 변화를 효과적으로 모델링하여 공간적 세부 사항을 반영합니다.

- **Performance Highlights**: 우리의 접근 방식은 배아, 뇌 개발 및 노화 데이터 세트에서 세포 유형의 조직 및 공간 구조를 복구하는 데 있어 기존의 방법들을 지속적으로 초월하며 우수한 성능을 보여줍니다. NicheFlow는 구조화된 고차원 및 변동적인 크기의 공간 도메인에서의 동역학을 원칙적으로 학습할 수 있도록 하여, 생물학 외 다른 시공간 모델링 영역과 유사한 도전 과제를 해결할 수 있는 강력한 프레임워크를 제공합니다.
...
### Motion-Robust Multimodal Fusion of PPG and Accelerometer Signals for Three-Class Heart Rhythm Classification (https://arxiv.org/abs/2511.00949)
Comments:
...
- **What's New**: 이번 연구에서 새로운 모델인 RhythmiNet을 소개합니다. RhythmiNet은 residual neural network를 기반으로 하며, PPG(photoplethysmography)와 ACC(accelerometer) 신호를 통합하여 심장 리듬을 분류합니다. 이 모델은 Atrial Fibrillation (AF), sinus rhythm (SR), 그리고 기타(arrhythmias)로 나누어 세 가지 클래스를 식별할 수 있습니다. 또한, 다양한 운동 조건에서의 강건성을 평가하기 위해 motion intensity를 기반으로 테스트 데이터를 계층화하였습니다.

- **Technical Details**: RhythmiNet은 경량형 residual neural network로 다음 네 가지 주요 구성 요소로 이루어져 있습니다: (1) convolutional stem, (2) residual backbone, (3) temporal attention module, 그리고 (4) 최종 분류 헤드입니다. 이 모델은 30초 기간의 PPG와 3축 ACC 데이터를 동시에 입력으로 받습니다. 또한, Squeeze-and-Excitation (SE) 블록을 통해 채널별 가중치를 조절하고, temporal attention module을 통해 장기적인 시간적 의존성을 모델링합니다.

- **Performance Highlights**: RhythmiNet은 PPG 단일 모델과 HRV(feature) 기반의 로지스틱 회귀 모델을 초과하는 성능을 보여주었습니다. 특히, RhythmiNet은 PPG 전용 기준선에 비해 macro-AUC에서 4.3% 개선된 결과를 얻었으며, 손수 제작한 HRV feature를 사용한 로지스틱 회귀 모델보다 12% 더 향상된 성과를 보였습니다. 이러한 결과는 노이즈가 가득한 실제 임상 데이터에서 다중 모드 융합(multimodal fusion)과 주의 기반 학습(attention-based learning)의 이점을 강조합니다.



### Transformers as Intrinsic Optimizers: Forward Inference through the Energy Princip (https://arxiv.org/abs/2511.00907)
- **What's New**: 본 논문은 Transformer 모델에 대한 새로운 에너지 기반 프레임워크를 제시합니다. 이 프레임워크는 글로벌 에너지 $F^*$, 에너지 함수 $E_i$, 그리고 최적화 알고리즘으로 구성됩니다. 기존의 softmax attention이 Helmholtz 자유 에너지의 최소화를 통해 유도될 수 있음을 보여주며, 이는 잔여 연결(residual connections)을 통해 점진적으로 최적화가 이루어짐을 설명합니다.

- **Technical Details**: 본 프레임워크는 softmax attention을 Helmholtz 자유 에너지를 사용하여 설명할 수 있으며, 이를 통해 다양한 attention 구조의 새로운 변형도 가능하게 됩니다. 에너지 형태를 조정하여 linear attention을 통합하고, multi-head attention의 경우 여러 저차원 서브스페이스의 평균 Helmholtz 자유 에너지를 최적화하는 방식으로 확대됩니다. 각기 다른 GD 알고리즘에서 영감을 받아 새로운 attention 구조를 제안하며, 이는 Momentum-based GD, Nesterov Accelerated Gradient(NAG) 및 Newton 방안으로 발전됩니다.

- **Performance Highlights**: 실험 결과는 에너지 기반 프레임워크가 attention 구조를 설계하는 데 있어 잠재력을 지닌다는 것을 입증합니다. 새로운 attention 구조는 다양한 방식으로 토큰의 응답을 개선할 수 있도록 설계되어 있으며, 이로 인해 네트워크가 더욱 효율적이고 효과적으로 작동할 수 있습니다. 에너지 기반 프레임워크를 통해 다양한 모델 아키텍처의 이해도를 높이고, 성능을 향상시킬 수 있는 가능성을 보여줍니다.



### Random Spiking Neural Networks are Stable and Spectrally Simp (https://arxiv.org/abs/2511.00904)
- **What's New**: 이번 연구에서는 이산 시간의 Leaky Integrate-and-Fire (LIF) 스파이킹 신경망(SNN) 모델의 안정성 및 강건성에 대한 새로운 통찰을 제공합니다. 스파이킹 신경망이 기존 인공 신경망(ANN)보다 에너지 효율성이 높지만 이론적 기초가 부족한 문제를 해결하고자 했습니다. 특히, Boolean 함수 분석을 통해 입력 변동이 출력에 미치는 영향을 정량화하였습니다.

- **Technical Details**: 이 연구는 무작위 매개변수 초기화에 대한 LIF SNN 분류기의 안정성을 수량화하는 한계를 설정하여, 이 분류기가 평균적으로 안정적이라는 것을 보여줍니다. 입력이 {-1, 1}^n 범위에 있을 때, 최대 𝒪(√n) 좌표의 변동이 분류기 출력에 영향을 미치지 않음을 입증합니다. 또한, ‘spectral simplicity’라는 개념을 도입하여, LIF SNN가 주파수 스펙트럼이 저주파 성분에 집중되는 경향이 있음을 분석하였습니다.

- **Performance Highlights**: 실험 결과에 따르면, 훈련된 LIF SNN 네트워크는 실제로 안정성을 유지하며, 얕은 구조와 깊은 구조 모두 노이즈에 대해 강건한 특성을 보였습니다. 이러한 안정성은 훈련 과정을 통해 평균적으로 증가하는 경향이 관찰되었습니다. 이 연구는 스파이킹 신경망의 이론적 이해를 심화시킬 수 있는 기초를 마련하며, 미래의 알고리즘 및 하드웨어 설계에 귀중한 문헌으로 기여할 것으로 기대됩니다.
...
### SpEx: A Spectral Approach to Explainable Clustering (https://arxiv.org/abs/2511.00885)
Comments:
          NeurIPS 2025

- **What's New**: 이 논문에서는 스펙트럴 그래프 분할(Spectral Graph Partitioning)을 기반으로 하는 새로운 설명 가능한 클러스터링 방법을 제안합니다. 이전의 연구들은 특정 클러스터링 목표에 대해 설명 가능성의 가격을 최소화하는 것에 집중했지만, 이 연구는 어떠한 클러스터링에도 일반적으로 적용 가능한 설명 트리를 맞추는 방법의 부족을 해결하고자 합니다. 이 접근법은 주어진 비설명 가능한 클러스터링이나 데이터셋 자체에 설명 트리를 직접 맞출 수 있도록 설계되었습니다.

- **Technical Details**: 제안된 방법은 SpEx-Clique 및 SpEx-kNN이라는 두 가지 알고리즘을 포함합니다. SpEx-Clique는 각 클러스터 위에 클리크를 포함하는 그래프를 사용하여 주어진 클러스터링을 설명 가능한 클러스터링으로 변환합니다. SpEx-kNN은 주어진 데이터셋에 직접 구축된 그래프를 사용하여 참조 클러스터링 없이 설명 가능한 클러스터링을 계산합니다. 이를 통해 기존의 방법들과 비교하여 제안된 접근법이 더 유연하고 범용적인 방법임을 입증합니다.

- **Performance Highlights**: 실험 결과 제안된 방법이 다양한 데이터셋에서 기존 방식들과 비교하여 우수한 성능을 보임을 보여줍니다. 특히, 이전의 알고리즘들이 가지는 한계에도 불구하고, 제안된 방법은 복잡한 데이터 구조에 대해서도 효과적으로 설명 가능한 클러스터링을 생성할 수 있습니다. 이는 더 나은 알고리즘 성능을 위한 새로운 기초를 제공하며, 설명 가능성이 요구되는 머신러닝 응용에 대한 새로운 가능성을 제시합니다.
...
### Training with Fewer Bits: Unlocking Edge LLMs Training with Stochastic Rounding (https://arxiv.org/abs/2511.00874)
- **What's New**: LLM(대형 언어 모델) 교육은 많은 계산 및 메모리 자원을 요구합니다. 이 논문에서는 양자화 훈련이 계산 및 메모리 효율성을 높이는 방법을 제시하지만, 양자화 소음이 수렴을 방해할 수 있다는 문제를 다룹니다. Stochastic Rounding(SR)은 이러한 소음을 관리할 수 있는 유망한 대안으로, 미니 배치 Stochastic Gradient Descent(SGD)와의 관계를 탐구합니다.

- **Technical Details**: 이 연구에서는 미니 배치 SGD에서 SR과 배치 크기의 상호작용을 이론적으로 및 실험적으로 분석합니다. 연구 결과에서, 배치 크기가 증가하면 백프로파게이션 동안의 정밀도 감소를 보완할 수 있음을 보여줍니다. 특히 가중치 및 활성화의 양자화가 경량화되면서 발생하는 기울기 분산의 영향을 다르게 나타냅니다.

- **Performance Highlights**: 실험 결과는 SR이 RTN(가장 가까운 값으로 반올림)보다 우수하다는 것을 보여줍니다. 1비트 정밀도의 감소가 배치 크기의 약 두 배 증가에 의해 보완될 수 있으며, 이는 유사한 수렴 동작을 유지하는 데 도움이 됩니다. 이 결과는 LLM을 에지 디바이스에서보다 효율적으로 교육할 수 있는 실질적인 가이드라인을 제공합니다.



### FlexiCache: Leveraging Temporal Stability of Attention Heads for Efficient KV Cache Managemen (https://arxiv.org/abs/2511.00868)
- **What's New**: 최근의 연구에서는 Large Language Models (LLM)의 처리 성능과 메모리 효율성을 높이기 위한 새로운 시스템인 FlexiCache를 소개합니다. FlexiCache는 KV heads의 시간적 안정성을 활용하여 GPU 메모리 사용량과 컴퓨팅 오버헤드를 줄이면서도 정확성을 유지하는 방법론입니다. 이를 통해 특정한 KV heads에 대해 메모리 관리 방식을 세분화하여 다양한 요청 상황에서 성능을 개선합니다.

- **Technical Details**: FlexiCache는 KV heads를 안정적인 것과 불안정적인 것으로 분류하여 상대적으로 안정적인 heads에 대해서는 최상위 K 페이지만 GPU 메모리에 유지하고 나머지는 호스트 메모리로 오프로드합니다. 불안정한 heads의 경우, 모든 KV 페이지를 GPU 메모리에 유지하여 메모리 접근 횟수를 줄이게 설계되었습니다. 또한, 안정적인 heads에 대해서는 주기적으로 페이지 재정렬을 실행하여 최신 정보를 유지합니다.

- **Performance Highlights**: FlexiCache는 vLLM 위에 구현되어 32k 이상의 긴 컨텍스트 요청에 대해 GPU 메모리 사용량을 최대 70% 줄이고, 오프라인 서비스의 처리량을 1.38-1.55배 향상시키며, 온라인 서비스 중 토큰 대기 시간을 1.6-2.1배 줄이는 성과를 보여줍니다. 전반적으로, FlexiCache는 긴 컨텍스트 및 긴 생성 시나리오에서도 모델의 정확성을 유지하며 뛰어난 성능을 발휘합니다.



### Identifying Slug Formation in Oil Well Pipelines: A Use Case from Industrial Analytics (https://arxiv.org/abs/2511.00851)
Comments:
...
- **What's New**: 이 논문에서는 석유 및 가스 파이프라인에서 발생하는 슬러그 형성을 실시간으로 검출할 수 있는 상호작용 애플리케이션을 소개합니다. 기존의 검출 방법들이 오프라인 기반이며 도메인 전문 지식이 필요한 점을 고려하여, 이 시스템은 직관적인 사용자 인터페이스를 통해 데이터 기반의 슬러그 검출을 가능하게 합니다.

- **Technical Details**: 이 시스템은 데이터 탐색(data exploration) 및 라벨링(labeling), 다양한 분류기(classifier)를 이용한 구성 가능한 모델 훈련과 평가, 시간 데이터(time-series) 오버레이를 활용한 분류 결과 시각화(visualization) 및 슬러그 이벤트 검출 시 지속성(persistence) 기반의 경고를 생성하는 실시간 추론 모듈(real-time inference module)으로 구성되어 있습니다.

- **Performance Highlights**: 이 데모는 라벨링된 CSV 업로드에서부터 보지 못한 데이터셋에 대한 실시간 추론에 이르기까지 원활한(workflows) 흐름을 지원합니다. 이 도구는 연구 프로토타입뿐만 아니라 실제 산업 응용 측면에서도 큰 가치를 지니며, 시계열 결함 진단(time-series fault diagnosis) 작업에 대한 넓은 적용 가능성을 제공합니다.
...
### Equilibrium Policy Generalization: A Reinforcement Learning Framework for Cross-Graph Zero-Shot Generalization in Pursuit-Evasion Games (https://arxiv.org/abs/2511.00811)
- **What's New**: 이 논문은 Equilibrium Policy Generalization (EPG) 프레임워크를 제안하여 적대적 게임인 추격-도주 게임(PEG)에서 다양한 그래프 구조에 대해 일반화된 정책을 학습하도록 합니다. EPG는 추적자와 회피자 측 모두에 적용 가능하며, 특정 그래프에 대한 균형 정책을 기반으로 다양한 그래프에서의 제로 샷(zero-shot) 성능을 보장합니다. 이는 이 도메인에서 처음으로 나타나는 일반화 가능성에 대한 두 가지 특성으로, 실시간 응용의 가능성을 높입니다.

- **Technical Details**: EPG 프레임워크는 서로 다른 그래프 구조에서 RL(강화학습) 정책을 훈련하여 각 그래프에 대한 균형 정책을 제공합니다. 이를 위해 다이나믹 프로그래밍(DP) 알고리즘을 사용하여 단일 그래프에서 순수 전략인 Nash 균형을 효율적으로 생성하는 오라클을 구성합니다. 또한 추적자 수에 따라 확장 가능성을 보장하기 위해 그룹화 메커니즘과 시퀀스 모델을 설계하여 정책 분해를 수행합니다.

- **Performance Highlights**: 실험 결과, EPG 프레임워크는 교차 그래프 PEG 훈련을 위한 균형 안내와 거리 특성을 활용하여 다양한 보이지 않는 현실 세계의 그래프에서 요구되는 제로 샷 성능을 보장합니다. 그리고 그래프의 출구에 대한 균형 휴리스틱을 사용하여 훈련할 경우, 일반화된 추적자 정책이 최첨단 PEG 방법의 파인 튜닝된 정책과 비슷한 성능을 발휘합니다.
...
### Effective Series Decomposition and Components Learning for Time Series Generation (https://arxiv.org/abs/2511.00747)
Comments:
          Accepted at IEEE International Conference on Data Mining (ICDM 2025). Camera-ready version to appear

- **What's New**: 이번 연구에서는 시간 시계열 생성(Time series generation)을 위한 새로운 접근법인 Seasonal-Trend Diffusion(STDiffusion)을 제안합니다. 기존의 방법들은 의미 있는 트렌드와 계절 패턴 생성을 해석 가능하게 분해하는 방법을 효과적으로 활용하지 못했던 반면, STDiffusion은 diffusion probabilistic models와 고급의 학습 가능한 시계열 분해 기법을 통합하여 해석성을 높였습니다. 이 방법은 트렌드와 계절 학습을 별도의 블록으로 분리하는 구조를 가지며, 이는 모델의 해석 가능성을 여러 스케일에서 개선합니다.

- **Technical Details**: STDiffusion 프레임워크는 멀티 레이어 퍼셉트론(Multi-Layer Perceptron, MLP) 구조를 통해 트렌드를 포착하고, 적응형 웨이브릿 디스틸레이션(adaptive wavelet distillation)을 통해 계절 요소의 다중 해상도 학습을 개선합니다. 이 방법은 생성된 구성 요소 간의 내부 일관성을 높이고 의미 있는 상관관계를 보존할 수 있도록 설계된 포괄적 수정 메커니즘을 포함합니다. 이를 통해 실세계 데이터의 복잡한 의존성을 더 잘 설명할 수 있으며, 높은 차원 설정에서도 신호 붕괴를 예방합니다.

- **Performance Highlights**: STDiffusion는 8개의 실제 데이터셋을 사용한 경험적 연구에서 최신 성능을 달성했습니다. 이 모델은 다중 창의 긴 시계열 생성을 위해 확장되어 신뢰할 수 있는 결과를 제공하며, 그 강력함과 다재다능성을 입증했습니다. STDiffusion은 기존의 생성 모델들이 직면한 문제들을 효과적으로 해결하며, 시간 시계열 생성에서 차별화된 성능을 나타냅니다.



### Enhancing Heavy Rain Nowcasting with Multimodal Data: Integrating Radar and Satellite Observations (https://arxiv.org/abs/2511.00716)
Comments:
          accepted to ICMLA 2025

- **What's New**: 이번 연구는 독일에서의 도시 홍수를 유발하는 강우 예측의 정확성을 향상시키기 위한 위성 및 레이더 데이터 융합의 효과를 평가합니다. 특히, 2001년부터 2018년까지 기록된 시간당 강한 강우 이벤트의 17.3%만이 레인 게이지로 포착된 사실을 강조하며, 레이더와 위성 이미지를 결합한 다중 모달(nowcasting) 모델의 개발을 통해 예측 정확성을 획기적으로 향상시킬 수 있음을 보여줍니다.

- **Technical Details**: 연구는 다중 모달(nowcasting) 모델을 개발하여 레이더와 위성 이미지를 결합하여 5, 15, 30분의 단기 강우 예측을 수행합니다. 레이더 데이터는 독일 기상청(DWD)에서 제공되며, 고해상도 레이더 조합을 사용하여 1km의 공간 해상도로 5분마다 데이터를 수집합니다. 위성 데이터는 EUMETSAT의 SEVIRI 레벨 1.5 신속 스캔 제품을 활용하여 5분마다 11개의 스펙트럼 밴드를 제공합니다.

- **Performance Highlights**: 다중 모달 모델은 레이더 전용 모델보다 강우 예측에서 4%의 Critical Success Index 향상을 보였고, 특히 강한 강우에 대해서는 예측 정확도가 크게 개선되었습니다. 2021년 노르트라인베스트팔렌(North Rhine-Westphalia)에서 발생한 심각한 홍수 사건을 분석하여, 다중 모달 모델이 레이더 전용 모델보다 더 상세하고 정확한 예측을 제공함을 입증하였습니다. 이러한 개선된 정확성은 적시의 신뢰할 수 있는 경고를 가능하게 하여 생명을 구하는 데 기여할 수 있습니다.
...
### Investigating the Robustness of Knowledge Tracing Models in the Presence of Student Concept Drif (https://arxiv.org/abs/2511.00704)
Comments:
          10 pages, 6 figures

- **What's New**: 이 논문은Knowledge Tracing (KT) 모델이 온라인 학습 플랫폼(OLP)에서의 학생 행동에 어떻게 영향을 받는지를 조사합니다. 이는 변화하는 학생 집단 및 개념 드리프트(concept drift)가 KT 모델의 성능에 미치는 영향을 분석하는 새로운 시도를 포함합니다. 연구 결과, Bayesian Knowledge Tracing (BKT) 모델이 가장 안정적인 성능을 나타내며, 더 복잡한 모델들은 예측력이 더 빠르게 저하된다는 점을 발견했습니다.

- **Technical Details**: KT는 교육 데이터 마이닝(EDM) 분야의 기초적인 문제로, 이전 성과를 기반으로 학생의 응답 정확성을 예측하는 이진 분류 문제입니다. 이 연구에서는 Bayesian Knowledge Tracing, Performance Factors Analysis, Deep Knowledge Tracing 및 Self-Attentive Knowledge Tracing의 네 가지 모델을 분석합니다. 각 모델은 특정한 교육적 요건과 학생 상호작용 데이터를 통해 평가되며, 시간적 맥락을 고려하여 모델의 성능을 비교합니다.

- **Performance Highlights**: 연구 결과, 모든 KT 모델이 개념 드리프트에 의해 성능 저하를 겪을 수 있으며, 특히 BKT 모델이 최신 데이터에서도 가장 뛰어난 안정성을 보였습니다. 반면, 복잡한 주의(attention) 기반 모델들은 상대적으로 예측력이 급격히 감소하는 경향을 보였습니다. 본 연구는 다세대의 학생 상호작용 데이터를 공개하여, 다른 연구자들이 자가 평가할 수 있는 기회를 제공합니다.



### Privacy-Aware Time Series Synthesis via Public Knowledge Distillation (https://arxiv.org/abs/2511.00700)
Comments:
          Published on Transactions on Machine Learning Research (TMLR)

- **What's New**: 이 논문에서는 공공 데이터의 이질성을 활용하여 개인 정보를 보호하면서도 유용한 시계열 데이터를 생성하는 새로운 프레임워크 Pub2Priv를 소개합니다. 기존의 개인 정보 보호 기술들이 너무 많은 사적인 메타데이터에 의존하는 경향이 있었던 반면, 이 연구는 관련 있는 공적 메타데이터를 사용하여 개인 정보 보호와 유용성 간의 균형을 맞추는 방법을 제시합니다. 또한, 우리는 DP-SGD(차별적 개인 정보 보호 확률적 경량 하강법)를 사용하여 모델을 훈련하여 개인 정보를 효과적으로 보호합니다.

- **Technical Details**: Pub2Priv는 시계열의 특징을 캡처하는 자가 주의(self-attention) 메커니즘을 사용하여 다차원 공공 메타데이터를 임베딩으로 변환합니다. 이러한 임베딩은 확산 모델(diffusion model)에 대한 조건적 입력으로 사용되어 사전 훈련된 변환기(transformer)를 통해 생성된 합성 개인 데이터를 만듭니다. 이 연구에서는 합성 데이터의 식별 가능성을 평가하기 위한 실용적인 메트릭을 제안하여 프레임워크의 개인 정보를 평가합니다.

- **Performance Highlights**: 실험 결과에 따르면, Pub2Priv는 금융, 에너지 및 상품 거래와 같은 다양한 도메인에서 개인 정보 보호와 유용성 간의 균형을 개선하는 데 있어 기존의 최신 기법들을 지속적으로 초월하는 성과를 보였습니다. 이 연구의 접근법은 공공 컨텍스트 메타데이터를 활용함으로써 시계열 생성의 품질을 향상시키며, 개인 사용자의 패턴을 보호하면서도 유용한 데이터를 생성할 수 있는 가능성을 제시합니다.



### Inference-Time Chain-of-Thought Pruning with Latent Informativeness Signals (https://arxiv.org/abs/2511.00699)
- **What's New**: 이 논문에서는 대규모 언어 모델(LLMs)의 추론 품질을 향상시키기 위해 KL-조정 가지치기 경로 알고리즘(KAPPA)을 제안합니다. KAPPA는 Kullback-Leibler 발산, 신뢰도 및 엔트로피를 포함하는 원칙적인 점수 함수를 사용하여 불리한 경로를 조기에 제거합니다. 이 방법으로 메모리와 토큰 사용량을 상당히 줄이면서 정확도를 유지할 수 있습니다.

- **Technical Details**: KAPPA는 세 가지 주요 단계로 진행됩니다: 초안 단계(Draft Phase), 점수 매기기 단계(Scoring Phase) 및 가지치기 단계(Gating Phase)입니다. 초안 단계에서는 여러 후보 경로를 병렬로 생성하고, 점수 매기기 단계에서는 정보 이득, 신뢰도 및 엔트로피를 결합하여 각 경로의 점수를 평가합니다. 가지치기 단계에서는 잘못된 경로를 제거하면서 최종적으로 하나의 경로를 선택하여 완전 생성합니다.

- **Performance Highlights**: 실험을 통해 KAPPA는 MATH500 데이터셋에서 72.2%의 성능을 달성하고, 표준 Best-of-N(BoN) 방법과 비교하여 토큰 생성을 최대 97.3% 감소시켰습니다. KAPPA는 작은 모델에서 안정된 성능을 유지하며, 최고 메모리를 약 60% 감소시키는 결과를 나타냈습니다.



### Sensitivity Analysis for Climate Science with Generative Flow Models (https://arxiv.org/abs/2511.00663)
- **What's New**: 이 연구는 기후 과학에서 민감도 분석(sensitivity analysis)의 효용성을 향상시키기 위해 생성 모델(generative models)에서의 기울기(gradient) 계산에 대해 새로운 접근 방식을 제안합니다. 기존의 물리적 모델을 사용하는 방법이 계산 및 개발 시간 면에서 매우 비효율적이라는 점을 해결하고자 합니다. 특히, cBottle이라는 모델을 사용하여 해수면 온도에 대한 민감도 분석을 진행하면서, 이를 통해 계산 비용을 크게 줄일 수 있음을 보여줍니다.

- **Technical Details**: 논문에서는 생성 흐름 모델(generative flow models), 특히 확산 모델(difusion models)을 사용하여 기후 데이터의 민감도 분석을 수행합니다. 제안하는 방법은 인접 상태(adjoint state) 방법을 사용하여 모델의 기울기를 계산하고, 이를 통해 주어진 입력 변수(input variable)와 출력 값(output value) 간의 관계를 분석합니다. 또한, 모델 예측의 일관성을 검증하는 새로운 방법론을 통해, 기울기가 얼마나 신뢰할 수 있는지를 정량적으로 평가할 수 있도록 합니다.

- **Performance Highlights**: 기존의 물리적 모델에서 소요되는 수 주의 계산 시간을 GPU를 활용하여 몇 시간으로 단축시키는 방식으로 민감도 분석의 효율성을 크게 개선하였습니다. 결과적으로 이 연구는 최신 AI 기반 모델이 기후 과학의 핵심 작업을 간소화하는 데 어떻게 기여할 수 있는지를 보여주는 초기 증거를 제공합니다. 실험 결과는 생성 모델이 대기 상태를 성공적으로 재현하는 동시에 기울기를 신뢰성 있게 제공한다는 것을 나타냅니다.
...
- **What's New**: 본 논문에서는 데이터 공유 없이 분산된 클라이언트 간의 협력적 모델 훈련을 지원하는 Federated Learning (FL)의 한계를 극복하기 위해 FedRevive라는 프레임워크를 제안합니다. 이 프레임워크는 비동기적 환경에서도 기존의 글로벌 모델과 더불어 지식 증류(data-free knowledge distillation, DFKD) 기법을 활용해 오래된 클라이언트 업데이트를 효과적으로 재활용합니다. 이를 통해 대규모 비동기 FL에서도 스케일러빌리티를 유지할 수 있는 방법을 제시합니다.

- **Technical Details**: FedRevive 프레임워크는 서버 측 경량 DFKD 프로세스와 파라미터 공간 집계를 통합하여, 구식 클라이언트 모델에서 현재 글로벌 모델로 지식을 전이합니다. 이 과정에서 메타 학습된 생성기가 가짜 샘플을 합성하여 다중 교사(distillation) 방법을 가능하게 하며, 하이브리드 집계 기법을 통해 일반적인 업데이트와 DFKD 업데이트를 결합하여 구식 업데이트의 영향을 완화합니다. 이러한 방식은 비동기 FL의 특성을 보존하면서도 모델 품질을 개선합니다.

- **Performance Highlights**: 다양한 비전 및 텍스트 벤치마크에서의 실험 결과에 따르면, FedRevive는 비동기 기준선에 비해 최대 32.1% 더 빠른 훈련 속도와 21.5% 높은 최종 정확도를 달성하였습니다. 이러한 결과는 FedRevive가 오래된 클라이언트 업데이트에서 유용한 정보를 추출하고, 이를 효과적으로 활용하여 모델 품질을 유지하면서도 비동기 FL의 장점을 살릴 수 있음을 보여줍니다.



### Diluting Restricted Boltzmann Machines (https://arxiv.org/abs/2511.00648)
- **What's New**: 이번 논문은 인공지능의 최신 발전이 대규모 신경망에 의존하는 경향이 있으며, 그 결과로 발생하는 계산 및 환경 비용에 대한 우려를 다룹니다. 특히, Restricted Boltzmann Machines (RBMs)가 극단적인 가지치기(pruning) 조건에서도 우수한 성능을 유지할 수 있는지를 분석했습니다. 연구 결과에 따르면, 최대 80%의 연결이 제거된 후에도 고품질의 생성 성능을 달성할 수 있지만, 재학습(retraining) 후 잃어버린 성능을 완전히 회복하는 것은 불가능함을 확인했습니다.

- **Technical Details**: RBMs는 Markov random field 구조를 갖고 있으며, 두 개의 계층으로 구성된 bipartite graph로 나타납니다. 여기서 가시 neuron(visible neurons)과 은닉 neuron(hidden neurons)의 상호작용을 통해 데이터 간의 의존성을 포착합니다. 학습 과정에서는 모델의 파라미터를 조정하여 훈련 데이터의 가능성을 극대화하며, 일반적으로 사용되는 기법으로 Contrastive Divergence (CD) 및 Persistent Contrastive Divergence (PCD)가 있습니다.

- **Performance Highlights**: 연구에서는 MNIST 데이터셋을 사용하는 단일 계층 RBMs를 학습시키고, 다양한 가지치기 조건 하에서 생성 품질을 평가했습니다. 생성 품질 측정의 어려움으로 인해, Second Moment Error (E(2)), Adversarial Accuracy Indicator Error (EAAI)와 같은 세 가지 다른 메트릭을 사용하여 결과를 분석했습니다. 이 과정에서, 초기 학습 조건이 네트워크 성능에 미치는 지속적인 영향을 강조하며, 효율적인 신경망 아키텍처 개발에 대한 실용적인 통찰을 제공합니다.



### Stochastic Shortest Path with Sparse Adversarial Costs (https://arxiv.org/abs/2511.00637)
- **What's New**: 이 연구는 제거된 상태-행동 쌍에서 발생하는 비용이 적은 상황에서의 적대적 비확률 경로 문제(Adversarial Stochastic Shortest Path, SSP) 분석을 다룬다. 연구진은 상태-행동 공간이 사실상 작을 때 귀납적 보상을 개선하기 위해 새로운 ℓ_r 정규화 기법을 제안한다. 이 새로운 접근법은 최소화 최대 보상이 상태-행동 공간의 크기(SA)에 대한 로그 의존도가 아닌 비어 있는 쌍 수(M)에 따라 결정되도록 하여 최적의 성능을 보인다.

- **Technical Details**: 이 논문에서는 기존의 온라인 미러 하강법(Online Mirror Descent, OMD)과 음의 엔트로피 정규화를 활용하여 상태-행동 공간의 크기 SA에 따라 비선형적으로 보상이 발생하는 경향을 연구한다. 제안된 ℓ_r 정규화는 비용 함수의 희소성에 적응할 수 있으며, M에 대한 로그적 보상을 달성한다. 다양한 설정에서의 성능 개선을 위한 새로운 아이디어를 제시하고, 포괄적인 성과를 이루어 내기 위해 다수의 이론적 경계와 하한을 설정하여 결과를 명확히 한다.

- **Performance Highlights**: 희소성이 존재하는 경우, 제안된 정규화 기법은 기존 OMD 방식보다 성능이 뚜렷하게 향상되는 것으로 나타났다. 특히, 희소성이 있는 문제에서는 보상이 M에 대한 로그 의존도로 개선되며, 기존 방법에 비해 훨씬 효율적인 결과를 제공한다. 또한, 알려지지 않은 전이 조건에서 희소성이 도움이 되지 않음을 보이며, 이러한 상황에서의 적대적 SSP 문제에 대한 깊은 통찰을 제시하였다.
...
### Gaining Momentum: Uncovering Hidden Scoring Dynamics in Hockey through Deep Neural Sequencing and Causal Modeling (https://arxiv.org/abs/2511.00615)
Comments:
          5 Pages, 4 Figures, 2 Tables

- **What's New**: 이번 논문에서는 프로 아이스하키에서 공격 동력(Offensive Momentum)과 득점 가능성(expected goals, xG)을 정량화하고 향상시키기 위한 통합된 데이터 기반 프레임워크를 제시합니다. 541,000개의 NHL 사건 기록을 포함하는 Sportlogiq 데이터셋을 활용하여, 여러 단계로 구성된 파이프라인을 통해 인사이트를 제공합니다.

- **Technical Details**: 제안된 프레임워크는 (1) 로지스틱 회귀를 통한 해석 가능한 모멘텀 가중치 부여, (2) 그래디언트 부스팅 결정 트리를 이용한 비선형 xG 추정, (3) LSTM(Long Short-Term Memory) 네트워크를 활용한 시간적 시퀀스 모델링, (4) 주성분 분석(PCA)을 통한 공간 형성 발견, 그리고 K-Means 클러스터링을 통한 표준화된 선수 좌표 기법, (5) X-Learner 인과 추정기를 활용한 평균 처리 효과(ATE) 산출의 다섯 가지 단계로 구성됩니다.

- **Performance Highlights**: 결과적으로 ATE는 0.12로 나타났으며(95% 신뢰구간: 0.05-0.17, p < 1e-50), 이는 득점 가능성이 15% 향상되는 것으로 해석됩니다. 이러한 결과는 전략적으로 구조화된 시퀀스와 밀집된 형성이 공격 성능을 유의미하게 상승시키는 것을 입증합니다.
...
- **What's New**: 이번 논문에서는 시스템 생물학과 같은 복잡한 동적 시스템의 모델 방정식을 데이터에 기반하여 발견하는 새로운 방법인 Kernel-based Sindy (KB-Sindy) 알고리즘을 제안합니다. KB-Sindy는 기존의 Sindy 알고리즘의 파라메트릭 (parametric) 추정 방법에 비파라메트릭 (nonparametric) 기법을 결합하여 복잡한 비선형성을 효과적으로 캡처할 수 있습니다. 이는 함수 형태에 대한 사전 지식 없이도 가능하며, 생물학적 과정을 해석할 수 있는 모델을 추출할 수 있습니다.

- **Technical Details**: KB-Sindy는 두 가지 요소를 혼합하여 동적 시스템을 모델링 하는데, 하나는 희소성 제약에 의해 조절되는 파라메트릭 요소이고, 다른 하나는 커널 기반의 비파라메트릭 요소입니다. 이는 임의의 함수 형태를 사전 지정하지 않고도 비선형적 관계를 포착할 수 있도록 도와줍니다. 기존의 Sindy 알고리즘이 제한적으로 비선형성을 추정하는 반면, KB-Sindy는 시스템의 구조가 미리 알려지지 않은 경우에도 효과적으로 작동할 수 있습니다.

- **Performance Highlights**: 논문에서는 KB-Sindy가 Lorenz 방정식, 유전자 조절, 시공간 칼슘 신호 및 로지스틱 맵을 비롯한 다양한 복잡한 생물학적 현상들의 모델링에 성공적으로 적용되었다고 보고합니다. 이러한 사례들은 KB-Sindy가 데이터로부터 모델 방정식을 직접 발견할 수 있는 능력을 보여줍니다. KB-Sindy의 접근법은 시스템 생물학 분야에서 비선형 모델을 추론할 수 있는 새로운 가능성을 제공합니다.



### Bayesian Network Structure Discovery Using Large Language Models (https://arxiv.org/abs/2511.00574)
- **What's New**: 이 논문에서는 기존의 데이터 기반 학습 접근 방식의 한계를 극복하기 위해, 대형 언어 모델(LLMs)을 중심에 두는 새로운 베이지안 네트워크 구조 발견 프레임워크를 제안합니다. 데이터가 없을 때는 PromptBN을 통해 메타데이터를 활용하여 확률적 관계를 효율적으로 발견하고, 데이터가 있을 때는 ReActBN을 통해 Bayesian Information Criterion(BIC)과 같은 구조 평가 점수를 통합하여 초기 그래프를 반복적으로 개선합니다. LLM을 핵심으로 삼는 이 접근 방식은 구조 학습 프로세스에서 LLM의 활발한 참여를 유지합니다.

- **Technical Details**: 제안된 프레임워크는 데이터 기반(estimation) 및 점수 기반(refinement) 두 가지 설정을 모두 포함합니다. PromptBN 단계에서, 메타 프롬프트 기법을 사용하여 LLM에 질문함으로써 유효한 구조를 신속하게 추출합니다. ReActBN 단계에서는 관측 데이터가 사용 가능할 때, 구조 점수를 오프라인으로 계산하고 ReAct 스타일의 에이전트적(workflow) 작업 흐름에 통합하여 LLM이 초기 그래프를 감독하고 개선할 수 있도록 합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 기존의 LLM 기반 알고리즘 및 전통적인 데이터 기반 알고리즘에 비해 significantly 향상된 성능을 보였습니다. 특히, 데이터가 부족하거나 전혀 없는 경우에 강력한 성능을 발휘하며, 이 모든 과정은 O(1) 복잡도로 가능하다는 장점을 가지고 있습니다. 실험에 사용된 새로운 데이터셋에서의 평가 결과, 본 프레임워크는 재교육이나 미세 조정 없이도 일반화된 결과를 나타냈습니다.
...
### Variational Autoencoder for Calibration: A New Approach (https://arxiv.org/abs/2511.00475)
Comments:
          6 pages, 5 figures

- **What's New**: 본 논문에서는 센서 보정을 위한 새로운 변이 오토인코더(Variational Autoencoder, VAE) 구현을 제시합니다. 제안된 VAE는 잠재 공간(latent space)을 보정(output)으로 훈련시켜 센서 데이터의 보정에 사용될 수 있습니다. 기존의 다중 센서 가스 데이터셋을 활용하여 이 접근 방식의 개념 증명을 보여주며, VAE가 오토인코더의 기능을 유지하며 동시에 보정 모델로 작용할 수 있음을 확인했습니다.

- **Technical Details**: 변이 오토인코더는 입력 데이터를 더 낮은 차원의 잠재 공간으로 매핑하는 인코더 네트워크와, 이 잠재 공간을 원래 입력 데이터로 다시 매핑하는 디코더 네트워크로 구성됩니다. 이 논문에서는 잠재 공간 샘플링을 보정(output)으로 사용하는 제약된 형태의 VAE를 사용하여 센서 특성을 포착하려고 합니다. 모든 모델은 스토캐스틱 역전파(stochastic backpropagation)를 통해 확률 분포로 매핑된 잠재 공간의 특성을 학습합니다.

- **Performance Highlights**: 제안된 VAE는 입력 데이터와 관련된 통계적으로 유사한 출력을 생성하는 능력을 보여주며, 이는 보정된 데이터 및 재구성된 출력 모두에 해당합니다. 실험 결과 Mean Absolute Error (MAE), R2, Kullback-Leibler Divergence (KLD) 등의 다양한 지표를 사용하여 모델 성능을 철저히 평가하고, 이로써 센서 보정의 효용성을 입증합니다. 이 연구는 ML을 활용한 센서 보정 방법의 새로운 가능성을 탐구하며, 미래 연구에 대한 확장 가능성을 제시합니다.
...
### Deep Learning Approach to Anomaly Detection in Enterprise ETL Processes with Autoencoders (https://arxiv.org/abs/2511.00462)
- **What's New**: 본 논문에서는 기업 수준의 ETL(data Extraction, Transformation, Loading) 데이터 스트림에서 발생하는 이상(Anomaly) 현상들을 다루기 위한 딥 오토인코더(Deep Autoencoders) 기반의 이상 탐지 방법을 제안합니다. ETL 과정에서 발생할 수 있는 여러 종류의 이상, 예를 들어 지연(Delay), 누락(Missing values), 중복 로드(Duplicate loading), 그리고 갑작스러운 비정상 변화(Sudden abnormal changes)를 분석하였습니다. 이 연구는 안정적이고 사용할 수 있는 입력을 보장하기 위해 데이터 표준화(Data standardization) 및 특징 모델링(Feature modeling)을 적용합니다.

- **Technical Details**: 제안된 방법은 인코더-디코더(Encoder-Decoder) 구조를 활용하여 고차원 입력을 잠재 표현(Latent representations)으로 압축하고 이를 재구성(Reconstruct)하는 방식으로 작동합니다. 재구성 오차(Reconstruction error)를 사용하여 이상 수준을 측정하며, 잠재 공간(Latent space)에서의 정규화 제약(Regularization constraints)을 통해 특징의 희소성(Feature sparsity) 및 분포 학습(Distribution learning)을 향상시킵니다. 이러한 기술적 요소들은 복잡한 데이터 스트림에서도 강인성을 높이는 데 기여합니다.

- **Performance Highlights**: 다양한 하이퍼파라미터 설정(Hyperparameter settings) 및 환경 변화(Environmental changes), 데이터 특성(Data characteristics) 하에서 체계적인 분석을 수행한 결과, 제안된 방법이 AUC, ACC, Precision, Recall 등에서 우수한 성과를 달성함을 보여줍니다. 이 연구의 결과는 딥 오토인코더 기반의 탐지 메커니즘이 기업 수준의 ETL 데이터 스트림에서 잠재적 분포 패턴을 효과적으로 캡처하고 다양한 이상을 정확히 식별하여, 기업의 데이터 처리 및 지능 분석에 신뢰할 수 있는 지원을 제공할 수 있음을 입증합니다.
...
- **What's New**: 본 논문에서는 비선형 Korteweg-de Vries (KdV) 방정식을 해결하기 위한 구조 보존 Physics-Informed Neural Network (SP-PINN) 프레임워크를 소개합니다. 기존의 PINNs는 장기 통합 과정에서 물리적 불변량을 보존하지 못하는 한계가 있었으나, 제안된 방법은 질량과 해밀턴 에너지를 손실 함수에 직접 포함하여 물리적으로 일관되고 에너지가 안정적인 진화를 보장합니다. 또한, sinusoidal activation functions를 사용하여 KdV 솔리톤의 발산성을 보다 정확하게 포착합니다.

- **Technical Details**: SP-PINN은 KdV 방정식의 Hamiltonian 구조를 유지하며, 수치적 안정성과 수렴 효율성을 개선하기 위해 conservation constraints를 통합합니다. 네트워크는 fully-connected feedforward neural network로 구성되며, 입력으로 시간 t와 공간 x를 받아 scalar field인 u(t,x)를 출력합니다. 저비용의 L-BFGS 최적화를 통해 sinusoidal activations와 결합하여 진화하는 물리적 변수를 보다 잘 반영합니다.

- **Performance Highlights**: 제안한 SP-PINN 모델은 단일 솔리톤 전파, 두 개 솔리톤의 상호작용 및 비선형 발산 붕괴와 같은 사례 연구를 통해 KdV 역학의 전형적인 행동을 재현합니다. 미세 조정 없이도 수렴이 가속화되고 장기적인 안정성이 향상되었으며, 에너지가 일관된 PINNs를 제공하여 Hamiltonian PDE 문제 해결에 있어서 높은 효율성을 증명합니다.



### Tree Training: Accelerating Agentic LLMs Training via Shared Prefix Reus (https://arxiv.org/abs/2511.00413)
- **What's New**: 이번 논문은 'Tree Training'이라는 새로운 훈련 패러다임을 제안합니다. 이는 여러 경로를 통해 분 branching되는 agentic LLM의 상호작용 과정에서 발생하는 비효율성을 해결하기 위한 것입니다. Tree Training은 공유하는 접두어(shared prefix)를 단 한번 계산하고, 계산된 결과를 관련된 경로에서 재사용함으로써 계산 효율성을 크게 향상시킵니다.

- **Technical Details**: Tree Training은 두 가지 주요 구성 요소로 이루어져 있습니다: (i) Tree Packing은 trajectory 내에서 공유된 계산을 효율적으로 재사용하여 메모리 제약 하에서도 불필요한 계산을 제거합니다. (ii) Gradient Restoration은 재사용된 접두어들이 올바른 기울기 전파(gradient propagation)를 유지하도록 보장하는 방법입니다. 이러한 구조는 메모리 제약과 기울기 정확성 문제를 해결하기 위한 것입니다.

- **Performance Highlights**: 여러 오픈 소스 모델을 통해 수행한 실험 결과, 총 훈련 시간이 최대 3.9배 줄어드는 것을 보여주었습니다. 특히 agentic LLM의 지도 학습(SFT) 및 강화 학습(RL) 훈련이 더욱 효율적으로 이루어질 수 있음을 입증했습니다. 최종 모델 품질을 유지하면서도 훈련의 전체 처리량이 최대 2.85배 향상되었습니다.
...
### A systematic evaluation of uncertainty quantification techniques in deep learning: a case study in photoplethysmography signal analysis (https://arxiv.org/abs/2511.00301)
- **What's New**: 이 논문은 심전도 및 혈압 예측을 위한 의료 시간 시리즈 데이터에 대한 새로운 불확실성 정량화(UQ) 기술 여덟 가지를 적용하여 모델의 신뢰성을 평가하는 데 중점을 두었습니다. 기존의 모델들이 실제 환경에서 적용될 때의 성능 저하 문제를 극복하기 위해, 논문에서는 불확실성을 측정하는 다양한 방법론을 비교하였습니다. 특히, 특정 예측 작업에 대해 신뢰성을 높이기 위한 저자들의 접근 방식이 주목받고 있습니다.

- **Technical Details**: 논문에서는 Bayesian inference를 포함한 전통적인 UQ 접근 방식의 한계를 언급하며, Monte Carlo Dropout (MCD), Deep Ensembles (DE) 등 최신 불확실성 정량화 기법을 구현하였습니다. 각 기법은 이론적 배경에 따라 신뢰성을 평가하기 위한 다양한 전략으로 나뉘며, 이를 통해 모델의 예측 성능 및 불확실성의 질을 종합적으로 평가할 수 있는 프레임워크가 구성되었습니다. 유의할 점은, 각 UQ 기법마다 출력 유형이 다르기 때문에 이를 비교하기 위한 변환 방안을 제시하였습니다.

- **Performance Highlights**: 실험을 통해 모델의 예측 정확도 및 각 기법의 불확실성을 평가한 결과, 특정 작업에 따른 최적의 UQ 기법은 다양하게 나타났습니다. 지역 보정(local calibration) 및 적응성(adaptivity) 평가가 신뢰성 문제에 대한 실용적인 통찰을 제공함을 발견하였으며, 이는 통상적으로 사용되는 글로벌 신뢰성 지표로는 얻을 수 없는 정보입니다. 연구 결과, 감지된 불확실성은 환자의 작은 수에 대한 신뢰도를 개선하고, 가능한 한 높은 예측 성능을 유지하는 데 중요한 역할을 한다는 점을 강조하고 있습니다.
...
### Improving the Robustness of Control of Chaotic Convective Flows with Domain-Informed Reinforcement Learning (https://arxiv.org/abs/2511.00272)
- **What's New**: 이 논문에서는 Rayleigh-Bénard Convection (RBC)라는 전형적인 대류 모델을 사용하여 혼돈 상태의 유체 흐름에 대한 강화 학습(RL) 제어 방법의 실제 적용 가능성을 향상시키고자 합니다. 전통적인 제어 방법은 혼돈 상태 시스템을 안정화하는 데 어려움을 겪고 있지만, RL 기반의 제어는 뜨거운 유체의 상승과 차가운 유체의 하강을 포함한 대류 흐름을 안정화하는 데 도움이 될 수 있습니다. 우리의 접근 방식에서는 도메인 정보(domain-informed)를 활용하여 전체적인 유체 흐름을 개선할 수 있는 강화 학습 에이전트를 제안합니다.

- **Technical Details**: Rayleigh-Bénard Convection은 두 개의 판 사이의 온도 차로 인해 발생하는 대류 흐름에서의 열 전달 모델로, 이 논문에서는 물리적 방정식을 기반으로 한 경계 조건과 상태 변수를 설정하였습니다. RB 현상은 낮은 Rayleigh 수에서 안정된 상태로 시작하여, 임계 값을 지나 혼돈으로 전이되는 양상을 보입니다. 이 모델은 Nusselt 수를 사용하여 열 전달의 강도를 측정하며, 이를 통해 대류 제어의 수학적 기반을 확립합니다.

- **Performance Highlights**: 도메인 정보를 포함한 RL 접근 방식은 라미나(층류) 흐름 상태에서 최대 33%까지 열 전달을 감소시켰고, 혼돈 상태에서도 10%의 성공적인 감소를 달성했습니다. 이러한 결과는 기존의 전통적 제어 방법보다 훨씬 우수하며, 특히 빠른 수렴성과 다양한 흐름 상태에서도 일반화 능력을 보였습니다. 이번 연구는 강화 학습 기반의 혼돈 흐름 제어를 실용적으로 적용할 수 있는 방향으로 크게 기여할 것으로 기대됩니다.



### X-TRACK: Physics-Aware xLSTM for Realistic Vehicle Trajectory Prediction (https://arxiv.org/abs/2511.00266)
- **What's New**: 최근 확장된 장단기 메모리(Extended Long Short Term Memory, xLSTM) 아키텍처의 발전은 기존의 장단기 메모리(Long Short Term Memory, LSTM) 네트워크의 한계를 극복했습니다. 본 연구에서는 xLSTM을 기반으로 한 차량 경로 예측 프레임워크 X-TRAJ와 물리적 제약을 통합한 X-TRACK을 제안합니다. 이러한 모델들은 차량의 동역학을 고려하여 보다 현실적이고 실행 가능한 경로를 생성합니다.

- **Technical Details**: xLSTM은 메모리 구조를 개선하여 장기적인 시간 의존성을 모델링하는 데 있어 기존 LSTM보다 우수한 성능을 보입니다. 기존 LSTM의 제약을 극복하기 위해 sLSTM과 mLSTM의 두 가지 구성 요소를 도입하였으며, 이는 메모리 혼합을 통한 효율적인 정보 저장 및 복구를 가능하게 합니다. 물리 기반의 운동 모델을 통합함으로써, xLSTM 기반의 예측 아키텍처는 차량의 역학에 따른 경로 예측을 수행합니다.

- **Performance Highlights**: X-TRACK 모델은 공개된 highD 및 NGSIM 데이터 세트에서 평가되어 기존 최신 모델들을 능가하는 성능을 보여주었습니다. 이 모델은 차량의 동역학을 고려하여 예측된 경로의 물리적 현실성을 보장함으로써, 정확한 예측과 안전한 운행을 위한 기초를 마련합니다. 이는 자율주행 차량의 경로 예측 정확도를 크게 향상시키는 데 기여할 것입니다.



### A Tight Lower Bound for Non-stochastic Multi-armed Bandits with Expert Advic (https://arxiv.org/abs/2511.00257)
- **What's New**: 본 논문에서는 비확률적 멀티암 밴디트(multi-armed bandit) 문제와 전문가 조언(expert advice)에 대한 최적의 최소 최대(expected minimax) 손실을 규명합니다. 저자들은 Kale (2014)의 상한(bound)과 일치하는 하한을 증명하여, 최적의 손실이 Θ(√(T K log(N/K)))임을 보여줍니다. 여기서 K는 암의 수, N은 전문가의 수, T는 시간 지평선(time horizon)입니다.

- **Technical Details**: 논문에서는 각 라운드에서 각 전문가가 제공하는 조언과 손실을 관찰하는 방식에 대해 기술합니다. 학습자는 손실의 차이를 최소화하는 것을 목표로 하며, 이 과정에서 적대적인 상황이 고려됩니다. 특히, 전문가의 조언과 손실은 분수적으로 허용되나, 하한 증명을 위해 간단한 이산(discrete) 버전으로 진행됩니다. 또한, 여러 연구에서 제안된 알고리즘 및 분석을 통해 최적의 손실을 도출하는 과정이 설명됩니다.

- **Performance Highlights**: 이 연구의 성과로는, 하한을 Θ(√(TK log(N/K)))로 구명한 점이 주목할 만합니다. 이전 연구들에 비해, 이 하한은 최적 손실에 대한 상한과 더불어 일치하게 되며, 이는 기존의 연구 결과들보다 향상된 것입니다. 저자들은 이러한 결과가 학습자의 행동을 제한하지 않으며, 일반적인 멀티암 밴디트 상황에서 적용될 수 있음을 확인하였습니다.
...
### Iterative Foundation Model Fine-Tuning on Multiple Rewards (https://arxiv.org/abs/2511.00220)
Comments:
          Accepted to NeurIPS 2025

- **What's New**: 이 논문은 여러 보상 신호를 사용하여 파운데이션 모델을 미세 조정하는 새로운 강화 학습(RL) 기반 방법을 제안합니다. 기존의 단일 보상 신호 최적화 방식이 아니라, 다양한 평가 기준을 활용하여 최적화할 수 있는 가능성을 보여줍니다. 또한 다중 보상을 통한 RL 미세 조정의 성능을 이론적으로 분석하여, 알고리즘의 성능에 대한 통찰을 제공합니다.

- **Technical Details**: 저자들은 각 목표에 대해 모델을 별도로 미세 조정하는 방식으로 전문가 정책 네트워크를 생성합니다. 그러나 이 미세 조정 과정은 독립적으로 수행되지 않고, 작은 단계로 나누어 반복적으로 진행됩니다. 학습한 전문가 정책은 합쳐져서 새로운 단일 정책으로 사용되며, 이는 보상 결합 및 전문가 정책 병합 접근 방식의 일반화로 해석됩니다.

- **Performance Highlights**: 다양한 도메인(텍스트 생성, 생물학적 서열, 소분자 생성)에 걸쳐 실험을 통해 제안된 알고리즘의 효과를 입증하였습니다. 실험 결과는 기존의 최첨단 방법들과 비교했을 때 유의미한 성능 향상을 보여주었으며, 특히 다중 목표를 다루는 데 있어 새로운 가능성을 제공하고 있습니다.
...
### Diffusion LLMs are Natural Adversaries for any LLM (https://arxiv.org/abs/2511.00203)
- **What's New**: 이 논문은 자원 집약적인 적대적 프롬프트 최적화 문제를 효율적이고 분산된 추론(Task)으로 변환하는 혁신적인 프레임워크를 소개합니다. 특히, 미리 훈련된 비자기회귀(Generative) LLM인 Diffusion LLMs가 프롬프트-응답 쌍의 조인트 분포를 모델링하여 프롬프트 검색을 위한 강력한 대리(Proxy) 역할을 할 수 있다는 점을 강조합니다. 이를 통해 고비용의 인스턴스별 최적화를 요구하지 않고도, 적은 수의 병렬 샘플로 효과적인 프롬프트를 생성할 수 있습니다.

- **Technical Details**: 제안된 방법은 적대적 프롬프트를 찾는 비용이 많이 드는 최적화 문제를 효율적인 추론(Task)으로 재구성합니다. 논문에서는 비자기회귀(Non-Autoregressive) 방식으로 사전 훈련된 생성 LLM인 Diffusion LLM을 활용하여 가능성 있는 프롬프트-응답 쌍에 대해 강력한 생성 사전 모델을 제공한다고 설명합니다. 이를 통해 프롬프트의 조건부 생성을 가능하게 하여 적대적 공격을 효율적으로 생성할 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 낮은 당혹감(Perplexity)을 가진 다양하고 효과적인 적대적 프롬프트를 생성함을 보여줍니다. 이러한 프롬프트는 robust하게 훈련된 상용 LLM을 포함하여 다양한 블랙박스 모델에 강한 전이 가능성을 보여줍니다. 기존 공격 방법들의 비용을 크게 줄이면서도 높은 성과를 달성하는 것으로 나타나, 새로운 적대적 프롬프트 생성 방식으로서 유망한 결과를 제공합니다.
...
### PDE-SHARP: PDE Solver Hybrids Through Analysis & Refinement Passes (https://arxiv.org/abs/2511.00183)
- **What's New**: PDE-SHARP는 고비용 과학적 계산을 저렴한 LLM 추론으로 대체하여 PDE 솔버의 정확도를 높이고 계산 비용을 60-75% 절감하는 혁신적인 프레임워크입니다. 이 시스템은 세 가지 단계인 분석(Analysis), 생성(Genesis), 및 합성(Synthesis)을 통해 작동하며, 이는 효율적인 솔버 생성을 위한 독창적인 접근을 제공합니다. 기존 접근 방식과 비교할 때, PDE-SHARP는 평균 13회의 솔버 평가로 최적 솔루션을 생성할 수 있으며, 이는 30회 이상의 평가가 필요한 전통적인 방법들에 비해 월등합니다.

- **Technical Details**: PDE-SHARP는 수학적 체인-오브-생각(Chain-of-Thought) 분석, 솔버 생성 및 LLM 심사자가 솔버 선택과 실행을 반복하여 성능 피드백을 제공하는 방법론으로 구성됩니다. 분석 단계에서는 PDE를 구조화된 질문을 통해 분석하고, 생성 단계에서는 실행 없이 솔버 후보를 만듭니다. 마지막으로 합성 단계에서는 LLM이 각 라운드에서 솔버를 선택하고 실행하여 피드백에 따라 개선하는 협력적 대회를 포함합니다.

- **Performance Highlights**: 실험 결과에 따르면, PDE-SHARP는 기존 방법들보다 평균 4배 더 적은 평가로 우수한 성능을 보여주었고, 디버깅 반복 수를 63-67% 줄여 버그 없는 실행을 달성했습니다. 또한 수치 수렴 성능도 개선되었으며, 다양한 LLM 모델에 대해 더 일관된 성능을 보이는특징이 있습니다. 이는 곧 PDE-SHARP가 연구 시나리오에 따라 유연하게 솔버를 개선할 수 있는 가능성을 보여줍니다.
...
### Study on Supply Chain Finance Decision-Making Model and Enterprise Economic Performance Prediction Based on Deep Reinforcement Learning (https://arxiv.org/abs/2511.00166)
Comments:
          9 pages, 3 figures

- **What's New**: 이 논문에서는 백엔드 중앙 집중식 중복 공급망의 의사 결정 및 계획 효율성을 개선하기 위해 딥 러닝(ddeep learning)과 지능형 입자 군집 최적화(intelligent particle swarm optimization)를 통합한 의사 결정 모델을 제안합니다. 공급망 네트워크를 위한 분산 노드 배치 모델과 최적 계획 경로를 구축하였으며, 최신 기술들을 활용하여 개선된 시스템을 목표로 하고 있습니다.

- **Technical Details**: 본 연구는 역사적 데이터에서 특징(feature)을 추출하기 위해 합성곱 신경망(convolutional neural networks)과 고차 통계적 특징을 캡처하기 위한 선형 프로그래밍(linear programming)을 활용합니다. 모델은 퍼지 연관 규칙 스케줄링(fuzzy association rule scheduling)과 심층 강화 학습(deep reinforcement learning)으로 최적화되며, 신경망(neural networks)은 동적인 변화에 적합하도록 설계되었습니다. '딥 러닝 특징 추출 - 지능형 입자 군집 최적화'의 혼합 메커니즘이 글로벌 최적화(global optimization)를 안내하고 적응형 제어를 위한 최적 결정을 선택합니다.

- **Performance Highlights**: 시뮬레이션 결과, 자원 소모가 감소하고 공간 계획(spatial planning)이 향상되었습니다. 동적인 환경에서 실시간 의사 결정 조정(real-time decision adjustment), 배급 경로 최적화(distribution path optimization) 및 강건한 지능형 제어(robust intelligent control)가 개선된 것으로 나타났습니다. 이는 공급망 관리의 효율성을 크게 향상시키는 것으로 평가됩니다.
...
### Physiologically Active Vegetation Reverses Its Cooling Effect in Humid Urban Climates (https://arxiv.org/abs/2511.00134)
Comments:
          27 pages, 5 figures

- **What's New**: 이 연구는 식생 구조와 기능이 열지수(Heat Index, HI)에 미치는 영향을 정량화하여 도시 온도와 습도의 상호 작용을 밝히고 있습니다. 138개의 인도 도시에서 열지수를 분석하고 기후별 적정 식생 전략의 정량적인 기준을 제공하여 열 스트레스 완화를 위한 정책을 제안합니다. 그리고, 녹지가 기후 재난 특히 격렬한 더위에 대한 도시의 대응을 어떻게 변화시키는지에 대한 새로운 통찰을 제공합니다.

- **Technical Details**: 연구는 SHapley Additive Explanations (SHAP)와 Accumulated Local Effects (ALE)를 통합한 해석 가능한 머신러닝 프레임워크를 사용하여 열지수를 재구성합니다. 식물의 특징인 Leaf Area Index (LAI), Enhanced Vegetation Index (EVI), Fraction of Absorbed Photosynthetically Active Radiation (fPAR)가 주요 변수로 활용되며, 이들은 도시 형태와 함께 열지수에 대한 상관관계를 분석합니다. 1km 해상도의 데이터 세트를 활용하여 다양한 식생 환경에서의 기온과 습도 간의 상호작용을 시각화합니다.

- **Performance Highlights**: 결과에 따르면, 식생 특징이 열지수의 공간적 변동성을 설명하는 데 주요한 역할을 하며, 특징적으로 수분활용 비율(fPAR)이 반건조 기후 도시에 가장 큰 영향을 미치는 것으로 나타났습니다. 인도 도시에서 HI는 시간적 및 공간적으로 다양성이 존재하며, 높은 밀도의 도시 중심부는 열지수가 상대적으로 계속해서 높은 기조를 유지하는 반면, 반도시 대역은 기후 변화의 영향을 더 강하게 받습니다. 이러한 결과들은 기후별 도시 재생 정책 설계의 기반이 될 수 있음을 보여줍니다.
...
### A Comparative Analysis of LLM Adaptation: SFT, LoRA, and ICL in Data-Scarce Scenarios (https://arxiv.org/abs/2511.00130)
- **What's New**: 이 논문은 데이터 부족 상황에서 Supervised Finetuning (SFT), LoRA, 그리고 In-Context Learning (ICL)의 비교 분석을 제시합니다. LoRA는 새로운 기술을 효과적으로 주입하면서도 기존 모델의 일반 지식에 미치는 영향을 최소화하는 강점을 보입니다. 반면, SFT는 기술 습득에 뛰어나지만, 재앙적 망각(catastrophic forgetting)에 매우 취약하다는 단점이 있습니다. 이 논문은 최적의 LLM(Large Language Model) 적응 전략을 선택하는 데 도움이 되는 실용적인 프레임워크를 제공합니다.

- **Technical Details**: 논문은 SFT, LoRA, ICL을 중심으로 각 기법의 장점과 단점을 시스템atically 비교합니다. LoRA는 예전 레이어에서의 수정 덕분에 안정적이며, 훈련의 초기에 설정된 모드 위에서 작동하도록 설계되었습니다. 실험은 일정한 설정(예: 학습 속도 0.001, LoRA 랭크 4)을 기준으로 진행되며, 32k 토큰까지 데이터의 영향을 비교합니다. 모델의 고유한 능력을 유지하면서 새로운 정보를 학습할 수 있는 설정을 찾는 것이 주요 목표입니다.

- **Performance Highlights**: ICL은 즉각적인 작업 습득에 매력적이지만, 복잡한 기술 작업에서의 성능은 부족할 수 있습니다. LoRA는 신뢰성을 가지고 일정한 성능을 유지하면서도, SFT는 고난이도의 전문 능력을 이끌어내는 데 뛰어난 점을 보여주지만 재앙적 망각으로 이어질 수 있습니다. 논문은 다양한 작업을 통해 모델이 어떻게 학습하고 기억력을 잃는지를 분석하고, LoRA가 기술 습득에서의 균형 잡기에 가장 효과적이라는 것을 강조합니다.
...
### Analysis of Line Break prediction models for detecting defensive breakthrough in footba (https://arxiv.org/abs/2511.00121)
Comments:
          14 pages, 8 figures

- **What's New**: 이번 연구에서는 2023 J1 리그 시즌의 이벤트 및 추적 데이터를 활용하여 Line Break의 발생 확률을 예측하는 머신 러닝 모델을 개발했습니다. 기존 연구들은 주로 슈팅이나 골 기회에 집중된 반면, 본 연구에서는 공격 팀의 수비 라인을 돌파하는 방식에 초점을 맞추었습니다. 모델은 선수의 위치, 속도, 공간 배열 등 189개의 특성을 포함하며, XGBoost 분류기를 사용하여 높은 예측 정확도를 달성했습니다.

- **Technical Details**: 제안된 모델은 AUC 0.982 및 Brier score 0.015의 높은 성능을 보여주며 Line Break 발생에 기여하는 요인들, 즉 공격 선수의 속도와 수비 라인의 간격, 공격 선수들의 공간 분포 등을 SHAP 분석을 통해 밝혔습니다. 머신 러닝, 통계 모델링 및 하이브리드 기법을 결합하여 전술적 동력을 정량적으로 이해하는 데 기여하고, 선수의 움직임과 공의 위치를 대조적으로 분석할 수 있는 기반을 제공합니다.

- **Performance Highlights**: 모델은 Line Break를 예측하는 데 있어 고도의 신뢰도를 보여주지만, F1 score가 0.150으로 낮아 긍정적인 Line Break 사례를 놓치는 경향이 있음을 지적합니다. 비록 모델이 정확한 분류를 달성했으나, 소수 클래스에 대한 탐지가 어려운 이유와 데이터 세트의 클래스 불균형이 이러한 문제를 야기했음을 알 수 있습니다. 향후 개선 작업을 통해 긍정적인 Line Break 탐지 능력을 강화할 필요성이 강조됩니다.
...
### MeixnerNet: Adaptive and Robust Spectral Graph Neural Networks with Discrete Orthogonal Polynomials (https://arxiv.org/abs/2511.00113)
- **What's New**: 이번 논문에서는 MeixnerNet이라는 새로운 스펙트럴 GNN 아키텍처를 소개합니다. 이 아키텍처는 연속적인 정의로 인해 그래프의 이산적 구조와의 불일치를 해소하기 위해 이산 직교 다항식인 Meixner 다항식을 사용합니다. 모델은 다항식의 두 주요 매개변수인 beta와 c를 학습 가능하게 하여 특정 그래프의 스펙트럴 특성에 따라 다항식 기초를 적응시킬 수 있도록 합니다.

- **Technical Details**: MeixnerNet은 그래프 스펙트럴 컨볼루션을 정의하고, 이산 직교 다항식을 사용하여 그래프 필터링을 수행합니다. Meixner 다항식은 세 개의 상관관계로 정의되며, 이 매개변수들을 학습 가능하게 하여 네트워크가 최적의 다항식 기초를 찾도록 합니다. 또한, Laplacian scaling과 basis LayerNorm으로 구성된 안정화 기법을 도입하여 훈련의 수치적 불안정성을 극복합니다.

- **Performance Highlights**: 실험적으로 MeixnerNet은 최적의 K=2 설정에서 ChebyNet 대비 경쟁력 있는 성능을 보여주며, 성능이 흔들리지 않는다는 점에서 특히 우수합니다. ChebyNet이 K의 하이퍼파라미터에 민감한 반면, MeixnerNet은 이러한 변동에 대해 훨씬 더 견고한 성능을 발휘합니다.
...
### flowengineR: A Modular and Extensible Framework for Fair and Reproducible Workflow Design in R (https://arxiv.org/abs/2511.00079)
Comments:
          27 pages, 7 figures, 1 table

- **What's New**: flowengineR는 기계 학습 파이프라인을 위한 재현 가능한 알고리즘적 워크플로우를 구축하기 위해 설계된 R 패키지입니다. 알고리즘 공정성이 빠르게 발전하는 분야에 의해 동기를 부여받았으며, 새로운 메트릭, 완화 전략, 기계 학습 방법이 지속적으로 출현하는 문제를 해결하고자 합니다. 기존 도구는 단일 개입에만 집중하거나 재현성과 확장성을 부차적인 고려사항으로 삼는 반면, flowengineR은 이를 핵심 설계 원칙으로 제정하여 해결합니다.

- **Technical Details**: flowengineR은 데이터 분할, 실행, 전처리, 훈련, 프로세스 중 처리, 후처리, 평가 및 보고에 대한 표준화된 엔진의 통합 아키텍처를 도입합니다. 각 엔진은 하나의 방법론적 작업을 캡슐화하면서 경량 인터페이스를 통해 소통하여 워크플로우의 투명성, 감사 가능성 및 쉽게 extensible 함을 보장합니다. 이 패키지는 R로 구현되었지만 CWL, YAWL, KNIME와 같은 워크플로우 언어 및 R 프레임워크 (BatchJobs, batchtools)에서의 아이디어를 바탕으로 하고 있습니다.

- **Performance Highlights**: flowengineR은 공정성 방법을 서로 교환 가능한 엔진으로 구조화함으로써 연구자들이 모델링 파이프라인에서 개입을 통합하고 비교하며 평가할 수 있도록 합니다. 또한 이 아키텍처는 설명 가능성, 강건성, 및 컴플라이언스 메트릭에도 적용될 수 있어, 핵심 수정 없이 다양한 워크플로우 맥락에서 재현성, 투명성 및 확장성이 필수적인 기반 구조를 제공합니다.



### Bridging Vision, Language, and Mathematics: Pictographic Character Reconstruction with Bézier Curves (https://arxiv.org/abs/2511.00076)
- **What's New**: 비전-언어 모델(VLMs)은 이미지의 고수준 의미 설명과 매핑에서 강력한 성능을 보여왔으나, 시각적 형태의 기하학적 구조 해석 능력은 상대적으로 미흡하였습니다. 이러한 기회를 활용하여 우리는 그림 문자(pictographic characters)를 대상으로 VLM의 이해를 기하학적 프로그램을 통해 재구성할 수 있는지 연구하였습니다. 특히, 현대 한자만으로 훈련된 모델이 고대 갑골문(Oracle Bone Script)을 제로샷(zero-shot) 환경에서 재구성할 수 있다는 중요한 발견을 하였습니다.

- **Technical Details**: 기존 VLM들은 픽셀 패턴과 언어 토큰 간의 표면 수준의 상관관계에 의존하는 경향이 있습니다. 이를 극복하기 위해 우리는 문자 재구성을 프로그램 합성(program synthesis) 문제로 정의하였고, 목표는 Bézier 곡선으로 구성된 실행 가능 프로그램을 생성하는 것입니다. 모델은 ‘비주얼 디컴파일러(visual decompiler)’로 작용하여 픽셀 기반 출력에서 기저의 생성 프로그램을 유추하게 됩니다. 이를 통해 모델은 문자 형식의 기하학적 속성을 학습하는 데 초점을 맞추게 됩니다.

- **Performance Highlights**: 모델은 강력한 제로샷 기준선인 GPT-4o를 초월하는 성능을 보였습니다. SFT(Supervised Fine-tuning) 훈련 방식에서 모델은 갑골문 평가에서 0.568의 점수를 기록하여 GPT-4o보다 15.9% 높은 성과를 보였습니다. 이는 모델이 현대 문자에서 고대 문자로의 횡단 일반화(cross-script generalization)를 성공적으로 이룬 것을 나타내며, 기하학적 표현을 효과적으로 학습하였음을 강력하게 지원합니다.



### Wavelet-Based Feature Extraction and Unsupervised Clustering for Parity Detection: A Feature Engineering Perspectiv (https://arxiv.org/abs/2511.00071)
Comments:
          8 pages, 2 figures. Code: this http URL

- **What's New**: 이 논문은 홀수와 짝수를 결정하는 고전적인 패리티 검출 문제에 대해 의도적으로 오버엔지니어링된 접근 방법을 탐구합니다. Wavelet 기반의 특징 추출과 비지도 클러스터링을 결합하여 이 문제를 해결합니다. 전통적인 모듈러 산술에 의존하는 대신, 정수를 Wavelet 도메인 표현으로 변환하여 이로부터 다중 스케일 통계적 특징을 추출하고 k-means 알고리즘으로 클러스터링합니다.

- **Technical Details**: 이 연구에서는 정수 시퀀스를 Wavelet 변환과 k-means 클러스터링을 사용하여 패리티 예측의 타당성을 평가합니다. 각 정수는 이진 시퀀스로 변환된 후, 다단계 Discrete Wavelet Transform(DWT)을 적용받습니다. Haar Wavelet이 가장 높은 분리 성능을 보였고, 각 분해 단계에서 에너지, L2 노름, 평균 절댓값(MAV)과 같은 세 가지 통계적 특징을 계산하여 k-means 클러스터링을 수행합니다.

- **Performance Highlights**: 제안된 방법은 10,001개의 정수 데이터셋에서 69.67%의 분류 정확도를 기록하며, 이는 무작위 기준선인 50%를 크게 초과합니다. 이 성과는 Wavelet 기반의 특징이 홀수와 짝수 이진 시퀀스 간의 잠재적 구조적 차이를 포착할 수 있음을 나타냅니다. 결과적으로, 전통적인 신호 처리 기법이 비연속적인 분류 문제에서도 유용할 수 있음을 보여줍니다.
...
### Token-Regulated Group Relative Policy Optimization for Stable Reinforcement Learning in Large Language Models (https://arxiv.org/abs/2511.00066)
- **What's New**: 이번 연구에서는 Token-Regulated Group Relative Policy Optimization (TR-GRPO)을 도입하여 Reinforcement Learning with Verifiable Rewards (RLVR)에서 토큰의 기여도를 조절하는 방법을 제안합니다. 기존의 Group Relative Policy Optimization (GRPO) 방법의 한계를 극복하여 저확률 토큰의 과도한 그라디언트 업데이트 문제를 해결하고, 고확률 토큰의 중요성을 부각시킵니다. TR-GRPO는 토큰의 확률에 기반한 가중치를 부여하여 훈련의 안정성을 높였습니다.

- **Technical Details**: 대부분의 대형 언어 모델 (LLM)은 transformer 디코더 아키텍처를 사용하며, 각 토큰은 유한한 어휘에서 선택됩니다. RLVR의 맥락에서, TR-GRPO는 고확률 토큰에 긴밀하게 연관된 가중치를 도입하여 훈련 중 저확률 토큰의 기여를 줄이고, 고확률 토큰의 기여를 강화합니다. 그라디언트 업데이트는 각 토큰의 모델 확률과 밀접한 연관이 있으며, 기본 목표는 최적화 문제를 해결하는 것입니다.

- **Performance Highlights**: TR-GRPO는 기존 GRPO보다 RLVR 작업 전반에서 일관되게 성능이 우수한 결과를 나타냅니다. 특히 K&K 논리 퍼즐 기준에서 정확도가 0.39에서 0.63으로 상승하였으며, 에이전틱 다중 단계 질문 응답에서는 정확도가 13.84에서 27.29로 거의 두 배 증가했습니다. TR-GRPO는 개선된 정확성 외에도 더 부드러운 그라디언트를 보여주며, 안정적인 훈련 동역학을 확립했습니다.
...
- **What's New**: EVINGCA (Evolving Variance-Informed Nonparametric Graph Construction Algorithm)는 클러스터 형성을 인접 이웃 그래프에서의 적응적이고 진화하는 과정으로 모델링하는 새로운 밀도-분산 기반 클러스터링 알고리즘입니다. 이 알고리즘은 고정된 밀도 임계값 대신 지역 통계 피드백을 사용하여 클러스터 확장을 수행합니다. EVINGCA는 낮은 차원 및 높은 차원의 다양한 데이터셋에서 기존 알고리즘에 비해 경쟁적인 성능을 보여주며, 평균적으로 로그-선형 복잡도를 가집니다.

- **Technical Details**: EVINGCA는 각 클러스터에 대해 실행 평균(μ)과 평균 절대 편차(Δ)를 유지하여 지역 밀도와 변동성을 나타냅니다. 이 알고리즘은 두 가지 필터(Level 1, L1과 Level 2, L2)를 사용하여 클러스터 성장 과정을 제어하고, L1 필터는 지역 밀도를 기반으로 하며, L2 필터는 형상 보존을 강조합니다. 전통적인 DBSCAN과 달리 EVINGCA는 고정 반경 대신 분산 기반의 허용 오차를 사용하여 클러스터 확장을 제어합니다.

- **Performance Highlights**: EVINGCA는 다양한 합성 및 실제 데이터셋에서 성능을 평가했으며, 기존의 밀도 기반 방법의 한계를 극복하면서 더 높은 확장성을 유지합니다. 특히, 이 알고리즘은 데이터 포인트 간의 작은 거리 문제를 해결하기 위해 매개변수를 조정하는 대신 내장된 수학적 휴리스틱을 사용하여 눈에 띄는 클러스터 생성을 도와줍니다. 또한, 소규모 클러스터를 더 큰 인접 클러스터로 재배치하여 클러스터의 품질을 향상시키는 방법을 제시하고 있습니다.
...
### Feature-Guided Analysis of Neural Networks: A Replication Study (https://arxiv.org/abs/2511.00052)
- **What's New**: 이 논문은 Feature-Guided Analysis (FGA)의 유용성을 평가하기 위해 MNIST 및 LSC 데이터셋을 사용하는 새로운 방법론을 제시합니다. 기존의 연구에 비해 높은 precision과 recall을 기록하여 FGA의 효과성을 입증하고, 신경망 아키텍처와 학습, feature 선택이 FGA의 효과성에 미치는 영향을 분석합니다. 이러한 연구는 산업적 맥락에서 FGA의 적용 가능성을 제시합니다.

- **Technical Details**: FGA는 심층 신경망(DNN)의 내부에서 관련된 뉴런을 식별하고 이들과 관련된 규칙을 추출하는 알고리즘입니다. 이 과정에서 DNN의 피드를 통해 뉴런의 활성화 값을 분석하고, 이를 기반으로 결정 트리를 생성하여 규칙을 도출합니다. 결정 트리는 뉴런 활성화의 조건에 따라 feature의 존재 여부를 판단하는데 활용됩니다.

- **Performance Highlights**: FGA는 MNIST 및 LSC 데이터셋을 활용한 실험에서 기존 문헌의 결과보다 높은 정확도를 보였습니다. 신경망의 선택과 학습, feature 선택이 FGA의 recall에 미치는 영향이 크며, 이러한 요소들이 precision에 미치는 영향은 미미합니다. 본 연구는 FGA의 재현성과 일반화 가능성을 높이며, 소프트웨어 엔지니어링에서도 유용하게 활용될 수 있는 규칙을 제공합니다.
...
### ReLaX-Net: Reusing Layers for Parameter-Efficient Physical Neural Networks (https://arxiv.org/abs/2511.00044)
- **What's New**: 이번 논문에서는 Physical Neural Networks (PNNs)의 성능을 향상시키기 위한 Reuse of Layers for eXpanding a Neural Network (ReLaX-Net) 아키텍처를 제안합니다. ReLaX-Net은 레이어 간 시간 분할(time-multiplexing) 방식을 통해 효과적인 네트워크 심도를 증가시키고, 적은 파라미터로도 효율성을 극대화합니다. 기존 PNN에 빠른 스위칭 요소를 추가하는 것만으로 쉽게 통합할 수 있습니다.

- **Technical Details**: PNN는 전통적인 디지털 신경망과는 달리 메모리와 연산을 분리하지 않는 방식으로 운영될 수 있습니다. ReLaX-Net은 레이어의 재사용(Reuse) 및 시간 분할 방식을 통해 여러 계산 단계에서 사용할 수 있습니다. 이는 기존 PNN의 파라미터 수를 효과적으로 늘리지 않고도 성능을 향상시킬 수 있는 방법입니다.

- **Performance Highlights**: ReLaX-Net은 이미지 분류(image classification)와 자연어 처리(natural language processing) 과제에서 수치 실험을 통해 검증됐습니다. 기존 PNN에 대해 소폭의 수정만으로도 계산 성능을 개선할 수 있음을 보여주었습니다. ReLaX-Nets는 같은 수의 파라미터를 가진 전통적인 RNN 및 DNN의 성능을 초과하는 경향을 보였습니다.
...
- **What's New**: 이 연구에서는 Physics-Informed Neural Networks (PINNs) 방법론의 예측 능력을 검증합니다. 전통적인 수치 방법들이 다루기 어려운 고강도, 충격, 불규칙한 영역 등 다양한 문제에서 PINNs가 효과적이라는 점을 강조합니다. 이러한 연구는 PINNs의 가능성을 넓히고, 기존 방법의 한계점을 지적하는 데 기여합니다.

- **Technical Details**: PINNs는 고전적인 ODE 문제를 제어된 테스트 베드로 사용하여 정확도, 훈련 효율성, 일반화 능력을 평가합니다. 여러 벤치마크 문제의 존재성과 고유성 분석 후, PINNs 방법론은 이들 모델 시스템에서 검증됩니다. 손실 함수의 구성 요소인 데이터 손실(data loss), 초기 조건 손실(initial condition loss), 잔여 손실(residual loss)의 적절한 균형이 필요합니다.

- **Performance Highlights**: 복잡한 문제의 수렴을 위해 하이퍼파라미터의 체계적인 조정이 필수적입니다. 네트워크 깊이, 층 너비, 활성화 함수, 학습률, 최적화 알고리즘 등 다양한 요소가 정확한 솔루션을 달성하는 데 중요한 역할을 합니다. 또한 사전 지식의 통합과 네트워크 아키텍처에 대한 강한 제약을 부여함으로써 PINNs의 예측 능력이 크게 향상된다는 결과를 제시합니다.
...
### Neural Architecture Search for global multi-step Forecasting of Energy Production Time Series (https://arxiv.org/abs/2511.00035)
- **What's New**: 이 연구에서는 동적 에너지 부문에서 에너지 생성의 단기 예측을 위한 효율적인 신경망 아키텍처 탐색(NAS) 기반 프레임워크를 설계했습니다. 이 프레임워크는 복잡한 방법의 수동 구성 없이도 예측 성능과 일반화 기능의 균형을 이룹니다. 에너지 시계열 데이터의 고유 패턴을 포착할 수 있는 효율적인 구성요소로만 구성된 탐색 공간을 도입하였습니다.

- **Technical Details**: 제안된 프레임워크는 시간적 맥락에서 성능 일반화를 고려하는 새로운 목표 함수를 정의합니다. 고차원 탐색 공간의 다양한 영역을 최대한 탐험하는 기능도 포함되어 있습니다. 이 접근 방식은 에너지 생산 시계열 데이터에서 경량 아키텍처의 앙상블을 활용하여 기존의 Transformer 같은 최신 기술보다 우수한 성과를 보여줍니다.

- **Performance Highlights**: NAS로 발견된 경량 아키텍처 앙상블이 효율성과 정확성 면에서 최신 기법 및 사전 훈련된 예측 모델들을 능가하는 결과를 기록했습니다. 이는 에너지 생성의 단기 예측에서 시간이 중시되는 효율성과 예측 정확성을 동시에 달성할 수 있는 가능성을 보여줍니다.
...
### Proximal Regret and Proximal Correlated Equilibria: A New Tractable Solution Concept for Online Learning and Games (https://arxiv.org/abs/2511.01852)
Comments:
...
- **What's New**: 이번 논문은 proximal regret라는 새로운 회귀 개념을 소개합니다. 이는 기존의 external regret과 swap regret 사이에 위치하며, 모든 플레이어가 no-proximal-regret 알고리즘을 사용할 경우, 경향적으로 proximal correlated equilibria (PCE)로 수렴함을 보여줍니다. 이에 따라, 고전적인 Online Gradient Descent (GD) 알고리즘이 O(√T)라는 최적 경과를 이루는 것을 제안하고, 이는 GD가 external regret보다 더 강력한 회귀 개념을 최소화한다는 것을 드러냅니다.

- **Technical Details**: 논문은 온라인 학습과 게임 이론에서의 여러 개념을 통합하는 framework를 제시합니다. 특히, convex games에 대한 효율적인 균형 계산 및 학습에 관한 문제를 다루며, Φ-regret를 통해 external regret와 swap regret를 연결짓습니다. φ-다양한 전략 수정을 통해 Φ-equilibria로의 수렴을 정의하며, 알고리즘이 효율적일 경우 T=poly(1/ε, d) 라운드 내에서 총 평균 회귀가 ε에 달할 수 있음을 설명합니다.

- **Performance Highlights**: Gradient Descent는 온라인 학습에서 empirically 우수한 성과를 보이는 반면, 새로운 proximal regret를 통해 더욱 효율적인 균형 계산이 가능하다는 새로운 시각을 제공합니다. 본 논문은 또한 Mirror Descent 및 Optimistic Gradient Descent 분석으로 일반 convex games에서 더 빠른 수렴 속도를 보이며, 기존 알고리즘과의 비교를 통해 성능 차별성을 입증합니다.
...
### Disciplined Biconvex Programming (https://arxiv.org/abs/2511.01813)
- **What's New**: DBCP(Disciplined Biconvex Programming)라는 새로운 모델링 프레임워크가 제안되었습니다. 이 프레임워크는 biconvex 최적화 문제를 명시하고 해결하는 데 도움을 줍니다. DBCP는 사용자가 적은 수의 문법 규칙에 따라 자연스럽게 문제를 지정할 수 있도록 하여, biconvex 문제를 쉽게 다룰 수 있게 해줍니다.

- **Technical Details**: DBCP는 biconvex 최적화 문제를 DCP(Disciplined Convex Programming)개념에 확장한 것입니다. 사용자가 문제 설명을 DBCP 문법 규칙에 맞게 입력하면, 문제는 자동으로 convex 서브문제로 분할되고 변환됩니다. 이 과정에서 맞춤형 ACS(Alternate Convex Search) 솔버가 생성되어 적용됩니다.

- **Performance Highlights**: 이러한 접근 방식은 사용자가 convex 최적화에 대한 전문 지식 없이 biconvex 문제를 신속하게 실험하고 프로토타입할 수 있도록 도움을 줍니다. DBCP는 Python 패키지인 dbcp로 구현되어 있으며, CVXPY라는 유명한 DSL의 확장입니다. 사용자는 biconvex 문제의 변수 분할 및 해결 과정을 인식하지 못하더라도 유용한 솔루션을 얻을 수 있습니다.



### Hybrid Neural Network-Based Indoor Localisation System for Mobile Robots Using CSI Data in a Robotics Simulator (https://arxiv.org/abs/2511.01797)
Comments:
          13 pages, 7 figures. Conference paper (ROBOVIS 2025)

- **What's New**: 본 논문에서는 Massive MIMO 시스템의 Channel State Information (CSI)을 활용하여 모바일 로봇의 위치를 추정하기 위한 하이브리드 신경망 모델을 제안합니다. 기존 CSI 데이터를 활용하여 합성 이미지로 변환한 후, Convolutional Neural Network (CNN)와 Multilayer Perceptron (MLP)을 결합하여 2D 로봇 위치를 추정하는 하이브리드 신경망(HyNN)을 구성하였습니다.

- **Technical Details**: 하이브리드 신경망은 CNN 잔가지와 MLP 잔가지로 나누어져 있으며, 각 잔가지에서 특징을 추출하고 데이터를 처리하는 방식으로 설계되었습니다. TINTO 도구를 사용하여 CSI 데이터를 2D 합성 이미지로 변환하고, CNN는 합성 이미지에서 특징을 학습하며, MLP는 원시 CSI 데이터를 처리합니다. 최종적으로 두 잔가지의 출력을 결합하여 로봇의 위치를 예측합니다.

- **Performance Highlights**: 제안된 HyNN 모델은 복잡한 환경에서의 모바일 로봇의 실내 위치 추정 및 내비게이션에서 높은 정확도를 달성하는 가능성을 보여줍니다. 로봇 운영 체제(ROS)와의 통합을 통해 다양한 테스트 케이스를 평가할 수 있으며, Kalman 필터와 같은 상태 추정기를 통해 로봇의 정확한 위치를 분석할 수 있는 유용성을 제공합니다.



### ADNAC: Audio Denoiser using Neural Audio Codec (https://arxiv.org/abs/2511.01773)
Comments:
          Accepted and presented at the 13th International Conference on Speech Technology and Human-Computer Dialogue (SpeD), Cluj-Napoca, Romania, October 19-22, 2025. 4 pages, 1 figure. IEEE Catalog Number: CFP2555H-USB, ISBN: 979-8-3315-7485-7

- **What's New**: 이 논문에서는 음악의 잡음 제거를 위한 최첨단 신경 오디오 코덱(neural audio codec)인 Descript Audio Codec(DAC)을 적응하는 개념 증명(proof-of-concept)을 제시합니다. 기존의 U-Net과 같은 전통적인 아키텍처의 한계를 극복하고자 다양한 소스에서 맞춤형으로 합성한 대규모 데이터셋에 대한 모델 훈련을 진행했습니다. 이를 통해 고음질의 생성적 오디오 복원이 가능함을 보여주고자 합니다.

- **Technical Details**: 이 연구는 다중 목표 손실 함수(multi objective loss function)를 기반으로 훈련을 진행하며, 시간 영역(time-domain), 스펙트럼(spectral), 신호 수준(signal-level) 충실도 메트릭을 결합하여 사용합니다. 이러한 접근은 오디오 데이터의 다양한 특성을 고려하여 더 효과적인 잡음 제거가 가능하도록 합니다. 또한, 맞춤형 데이터셋을 사용함으로써 기존의 데이터셋에 없었던 다양성을 포함시킵니다.

- **Performance Highlights**: 논문은 DAC 모델이 기존의 전통적인 오디오 처리 기술보다 뛰어난 성능을 발휘함을 입증합니다. 특히, 높은 충실도의 음악 복원이 가능하며, 이는 실제 응용 프로그램에서의 사용 가능성을 극대화합니다. 결론적으로, 이 연구는 고음질 오디오 복원을 위한 새로운 가능성을 제시하며, 향후 연구에 중요한 기초 자료가 될 것입니다.
...
- **What's New**: 이 논문에서는 새로운 시스템 클래스인 semantic query processing engines에 대한 벤치마크를 제시합니다. 이 시스템은 최신 대형 언어 모델(large language models, LLMs)의 생성(generative) 및 추론(reasoning) 능력을 기반으로 합니다. 이들은 자연어 지시에 의해 구성된 semantic operator를 활용하여 SQL을 확장하며, 사용자가 다중 모달 데이터에 대해 다양한 작업을 수행할 수 있도록 합니다.

- **Technical Details**: 제안된 벤치마크는 세 가지 주요 차원인 시나리오(scenarios), 모달리티(modalities), 및 오퍼레이터(operators)에 걸쳐 다양성을 제공합니다. 시나리오는 영화 리뷰 분석에서부터 의료 질문 답변까지 다양하며, 데이터 모달리티는 이미지, 오디오, 텍스트를 포함합니다. 쿼리는 semantic filters, joins, mappings, ranking, classification operators와 같은 다양한 오퍼레이터를 포함합니다.

- **Performance Highlights**: 이 벤치마크는 세 개의 학술 시스템(LOTUS, Palimpzest, ThalamusDB)과 하나의 산업 시스템(Google BigQuery)에서 평가되었습니다. 이 결과는 지속적으로 발전하고 있는 시스템의 스냅샷을 반영하지만, 현재의 강점과 약점에 대한 중요한 통찰을 제공합니다. 이를 통해 향후 연구의 가능성 있는 방향을 조명합니다.
...
### Making Interpretable Discoveries from Unstructured Data: A High-Dimensional Multiple Hypothesis Testing Approach (https://arxiv.org/abs/2511.01680)
- **What's New**: 이 논문은 불구속 데이터(unstructured data)에서 발견(discovery)을 추출하기 위한 일반적이고 유연한 통계적 프레임워크를 제안합니다. 이 프레임워크는 머신 러닝 해석 가능성(machine learning interpretability) 문헌에서 최근의 방법을 활용하여 불구속 데이터 포인트를 고차원적이고 희소(sparse)하며 해석 가능한 개념 사전(dictionaries of concepts)으로 매핑합니다. 연구자는 이 사전을 기반으로 통계적 추론(statistical inference)을 수행하고, 연구자의 개입 없이 원하는 결과를 발견할 수 있는 통계 절차를 사용합니다.

- **Technical Details**: 제안된 프레임워크는 불구속 데이터 셋을 고차원, 희소, 이진 벡터(binary vectors)인 사전 특징 벡터(dictionary feature vectors)로 변환합니다. 그런 다음, 이 데이터에서 저널래이트 성(statistics)을 계산하고, 선택적 추론(selective inference) 절차를 통해 가설 검정을 수행합니다. 이러한 방법은 고차원 데이터 세트에서의 오류율을 관리하는 데 특히 유용하며, 연구자가 필요로 하는 가설을 경량하게 선택할 수 있는 유연성을 제공합니다.

- **Performance Highlights**: 본 프레임워크는 연구자의 자유도가 적기 때문에, 분석 중 데이터 조작(data mining)으로 인한 편향을 최소화할 수 있는 장점이 있습니다. 또한 구현 비용이 낮고, 연구자 부담이 적어 빠르고 손쉽게 재현할 수 있습니다. 이 논문은 최근의 설명적 및 인과 분석의 여러 사례를 통해 이 프레임워크의 응용 가능성을 보여 줍니다.



### Panther: A Cost-Effective Privacy-Preserving Framework for GNN Training and Inference Services in Cloud Environments (https://arxiv.org/abs/2511.01654)
Comments:
          Accepted for publication in IEEE Transactions on Services Computing (TSC)

- **What's New**: 이 논문은 GNN(그래프 신경망)의 개인 정보 보호와 클라우드 환경에서의 비용 효율성을 동시에 해결하는 Panther 프레임워크를 소개합니다. Panther는 기존의 동기식 접근 방식과 다른 비동기식 집계 모델을 통해 GNN 모델의 학습과 추론 서비스를 위한 안전한 통신을 제공합니다.

- **Technical Details**: Panther는 네 개의 파티 간 안전한 배열 접근 프로토콜을 실행하기 위해 비동기식 방식으로 설계되었습니다. 이 프레임워크는 노드의 이웃 정보를 랜덤으로 패딩하는 기법을 사용하여 인접 행렬 계산의 효율성을 높이고, 동기식 인접 상태 집계에서 발생하는 대기 시간을 제거합니다.

- **Performance Highlights**: 평가 결과, Panther는 GNN 학습 및 추론 과정을 각각 평균 75.28% 및 82.80% 단축시켰고, 통신 오버헤드도 평균 52.61% 및 50.26% 감소시켰습니다. 결과적으로 GNN의 클라우드 교육 및 추론 코스트를 평균 55.05% 및 59.00% 절감하는 효과를 얻었습니다.
...
### Partial Trace-Class Bayesian Neural Networks (https://arxiv.org/abs/2511.01628)
Comments:
          10 pages, 4 figures

- **What's New**: Bayesian Neural Networks (BNNs)는 딥러닝에서 고기능적인 불확실성 정량화를 가능하게 해주는 도구이지만, 종종 높은 계산 비용을 동반합니다. 본 논문에서는 Partial Trace-Class Bayesian Neural Networks (PaTraC BNNs)이라는 세 가지 혁신적인 아키텍처를 제안하여, 표준 BNN들과 비슷한 수준의 불확실성 정량화가 가능하면서도 훨씬 적은 Bayesian 매개변수를 사용하도록 합니다. 이러한 접근법은 신뢰할 수 있고 강력하며 확장 가능한 불확실성 정량화를 지원합니다.

- **Technical Details**: PaTraC BNNs는 사용되는 신경망 매개변수의 자연스러운 순서를 유도하며, 이는 Bayesian 매개변수를 선택하는 노드 기반 절차로 이어집니다. 이 논문에서는 세 가지 구조의 PaTraC BNNs를 정의하고, 각 구조의 효과를 수치적 연구를 통해 평가합니다. 특히, 이 연구는 PaTraC BNN이 관심 있는 목표 함수의 불확실성을 정확하게 정량화할 수 있는 능력에 초점을 맞춥니다.

- **Performance Highlights**: 제안된 PaTraC BNNs는 계산적 효율성과 메모리 요구량에서 기존 BNN들보다 빠르고 유리하다고 주장합니다. 논문에 포함된 수치 시뮬레이션 및 실제 데이터 세트를 통한 연구는 이러한 성능 이점을 확인하며, PaTraC BNN의 신뢰할 수 있는 불확실성 정량화 기능을 강조합니다. 이는 Bayesian 모델을 활용하여 신경망의 예측 부정확성을 줄이는 데 기여합니다.



### L2T-Tune:LLM-Guided Hybrid Database Tuning with LHS and TD3 (https://arxiv.org/abs/2511.01602)
- **What's New**: 이번 연구는 L2T-Tune이라는 새로운 하이브리드 데이터베이스 튜닝 프레임워크를 제안합니다. 이 프레임워크는 LLM(large language model) 지침을 활용하며, 세 개의 단계로 구성되어 있습니다: 첫 번째 단계는 균일한 샘플을 생성하여 공유 풀에 로깅하는 준비 단계이고, 두 번째 단계는 LLM을 통해 튜닝 힌트를 채굴하는 과정이며, 세 번째 단계는 TD3(Delayed Deep Deterministic Policy Gradient) 알고리즘을 사용하여 최종 튜닝을 수행합니다. L2T-Tune은 다양한 워크로드에 대해 평균 37.1%, TPC-C에서는 최대 73%의 성능 개선을 보였습니다.

- **Technical Details**: L2T-Tune은 데이터베이스 구성요소인 'knobs'(조정 가능한 매개변수)의 차원 수를 줄이고 효율성을 높히기 위해 랜덤 포레스트(Random Forest) 및 주성분 분석(Principal Components Analysis, PCA)을 활용합니다. 이 과정에서는 MySQL 상태를 나타내는 63개의 내부 매트릭스를 사용하여 TD3 모델에 대한 액터의 동작을 안내합니다. 또한, 온라인 튜닝 단계에서 메모리나 디스크가 변경되면 단계 1에서 초기화한 균일 샘플을 활용하여 신속하게 기반을 재설정하고, 짧은 TD3 로컬 튜닝을 실행하는 세미-전이 방식이 사용됩니다.

- **Performance Highlights**: L2T-Tune은 기존의 강화 학습 모델과 비교할 때 오프라인 튜닝 단계에서 빠른 수렴성을 보여 주며, 단일 서버에서 최적의 구성을 신속하게 달성합니다. 온라인 튜닝 단계에서는 최적의 성과를 달성하는 데 30단계만 필요합니다. 연구 결과, 우리 방법은 TPC-C 벤치마크에서 최대 73%의 성능 개선을 보이며, 기존의 최첨단 메소드보다 월등한 튜닝 성능을 입증하였습니다.
...
- **What's New**: 이번 연구에서는 'gtars-tokenizers'라는 고성능 라이브러리를 소개합니다. 이 라이브러리는 유전체 간격(genomic intervals)을 사전 정의된 특성 집합으로 매핑하여, 자연어 처리(natural language processing)에서의 텍스트 토큰화(tokenization)와 유사한 방식으로 작동합니다. Rust로 구축된 이 라이브러리는 Python, R 및 WebAssembly와의 바인딩을 제공하며, 최신 머신 러닝 프레임워크와의 통합이 용이합니다.

- **Technical Details**: gtars-tokenizers는 두 가지 중첩(overlap) 방법을 구현하며, 각각 이진 간격 트리 검색(binary interval tree search, BITS)과 증강된 간격 목록(Augmented Interval List, AIList)을 사용합니다. 이 도구는 다양한 프로그래밍 환경에 통합될 수 있도록 Python, R 및 CLI 인터페이스를 지원합니다. 또한 사용자들이 기존의 ML 워크플로우에 쉽게 통합할 수 있도록 최적화되어 있습니다.

- **Performance Highlights**: gtars-tokenizers는 대규모 데이터셋 처리에서 뛰어난 효율성을 발휘하며, 기존 도구들보다 2-3배 빠릅니다. 벤치마크 결과, 이 라이브러리는 Hugging Face와 호환되어 널리 사용되는 ML 프레임워크와 쉽게 통합될 수 있습니다. 이러한 특성 덕분에 서로 다른 실험 결과를 보다 쉽게 비교하고 결합할 수 있는 기반을 제공합니다.



### Learning what to say and how precisely: Efficient Communication via Differentiable Discrete Communication Learning (https://arxiv.org/abs/2511.01554)
Comments:
          30 pages, 12 figures, 6 tables

- **What's New**: 이 논문은 Multi-Agent Reinforcement Learning (MARL)에서 효과적인 커뮤니케이션을 위해 Differentiable Discrete Communication Learning (DDCL) 프레임워크를 확장하여 비구속 신호를 지원하는 새로운 방법을 제안합니다. 이를 통해 각 에이전트가 메시지의 정밀도를 동적으로 조정할 수 있도록 하여 통신 대역폭을 효율적으로 활용하기를 목표로 합니다. 기존의 복잡한 구조 대신 범용적으로 적용할 수 있는 플러그 앤 플레이 방식으로 DDCL을 변형하였습니다.

- **Technical Details**: 연구에서는 Multi-Agent 시스템을 Partsially Observable Markov Game (POMG)으로 모델링 하여, 각 에이전트가 고유한 부분 관찰을 기반으로 행동을 선택하고, 타 에이전트와 메시지를 교환하며 동작하는 과정을 설명합니다. DDCL은 스토캐스틱 양자화 과정을 활용하여 실수 벡터를 다양한 길이의 비트 문자열로 매핑하므로, 에이전트는 자신의 관측치에 따라 메시지의 정밀도를 조정할 수 있습니다. 또한, 훈련 간 고정밀 메시지 대신 비트 수준에서 메시지의 정밀도를 최적화하는 방법을 제시합니다.

- **Performance Highlights**: 저자들은 새로운 DDCL 변형이 네 가지 최신 MARL 알고리즘에 통합될 수 있음을 입증했습니다. 이 방식은 통신 대역폭을 10배 이상 줄이면서도 작업 성능을 유지하거나 끌어올릴 수 있음을 보여줍니다. 마지막으로, 단순한 Transformer 기반 정책이 복잡한 구조를 사용한 통신 성능을 초과할 수 있음을 통해, 가향(구축)된 통신 메커니즘의 필요성에 의문을 제기합니다.



### Deep Learning Prediction of Beam Coherence Time for Near-FieldTeraHertz Networks (https://arxiv.org/abs/2511.01491)
Comments:
          IEEE Wireless Communication Letters (accepted October 2025)

- **What's New**: 이 논문에서는 테라헤르츠(THz) 통신을 위한 새로운 빔 응집 시간(beam coherence time)을 제안하고, 이를 통해 모바일 네트워크에서 빔 업데이트 속도를 획기적으로 줄이는 방법을 소개합니다. 제안된 방법은 단순한 피드포워드 신경망(feedforward neural network)을 기반으로 하여, 최소한의 오버헤드로 빔 포밍을 조정하도록 설계되었습니다.

- **Technical Details**: 제안된 시스템 모델은 AP와 단일 안테나를 가진 이동 사용자 장치(UE) 간의 THz 통신 링크를 고려하여, 균일 선형 배열(uniform linear array) 구조와 구형파 깨달음(spherical wave assumptions)을 활용합니다. 이 모델은 기존의 평면파 가정(plane wave assumption)을 넘어서, 이동성과 채널 변화를 반영한 빔 포밍을 위한 동적 예측을 가능하게 합니다.

- **Performance Highlights**: 수치적 결과는 제안된 접근 방식이 NF 기반 THz 네트워크에서 더 높은 데이터 속도를 가능하게 하며, 특히 고속 이동 상황에서도 통신 성능을 유지할 수 있음을 보여줍니다. 따라서 빔 업데이트의 오버헤드가 매우 높아지는 경우에도(예: 데이터 전송 속도 0) 이상적인 상황에 가까운 데이터 전송을 가능하게 합니다.



### Quantum Blackwell's Ordering and Differential Privacy (https://arxiv.org/abs/2511.01467)
Comments:
          46 pages, 3 figures

- **What's New**: 이 논문에서는 양자 가설 테스트(quantum hypothesis testing) 및 블랙웰의 정렬(Blackwell's ordering)을 기반으로 한 양자 차별 개인 정보 보호(quantum differential privacy, QDP) 프레임워크를 개발하였습니다. 이 접근 방식은 가설 테스트 발산(hypothesis testing divergences)을 통해 (ε,δ)-QDP를 특징짓고, 개인 정보 보호 제약 하에서 가장 유용한 양자 상태 쌍을 식별합니다. 또한 양자 학습 알고리즘의 안정성을 분석하며, δ>0인 경우에 대해 고전적인 결과를 일반화하는 연구가 포함되어 있습니다.

- **Technical Details**: 이 프레임워크는 QDP 메커니즘을 탐구하기 위해 구축되었으며, 블랙웰의 정보를 비교하는 개념과 관련이 있습니다. 이 논문에서는 두 개의 양자 상태 쌍이 (ε,δ)-DP를 만족할 때의 안정성에 관한 내용을 제시하고, 완전 양성 추적 보존 매핑(completely positive trace preserving map)을 통해 이들 상태 간의 관계를 밝힙니다. 또한, 양자 피셔 정보(quantum Fisher information)에 대한 타이트한 경계를 도출하였고, 이는 QDP 하에서의 개인화된 양자 매개변수 추정(quantum parameter estimation) 분석에 활용됩니다.

- **Performance Highlights**: 연구 결과, 제안된 QDP 메커니즘의 효율성을 증명하였으며, 정보 추출 및 개인 정보 보호 간의 균형을 이해하는 데 중요한 통찰을 제공합니다. δ=0인 경우와 비교할 때, δ>0의 경우에도 안정성을 보장하는 새로운 경계를 설정했습니다. 마지막으로, 하키스틱 발산(hockey-stick divergence)에 대한 차별적으로 개인적인 양자 채널의 근사 최적 축소 경계를 확립했습니다.



### Split-Flows: Measure Transport and Information Loss Across Molecular Resolutions (https://arxiv.org/abs/2511.01464)
- **What's New**: 이번 논문에서는 split-flows라는 새로운 flow-based 접근 방식을 소개합니다. 이 방법은 backmapping을 해상도 간의 연속적인 시간 측정 수송으로 재해석하여, coarse-grained 모델의 세부 사항을 효과적으로 복원할 수 있게 합니다. 또한, split-flows는 해상도 간의 확률적 연결을 직접적으로 구축하여, atomistic 구조를 조건부로 샘플링할 수 있는 가능성을 제공합니다.

- **Technical Details**: split-flows는 다양한 해상도 간의 구성 밀도를 연결할 수 있는 확률적 링크를 정의합니다. 이는 coarse-grained 모델의 정보 손실을 계산할 수 있는 가능성을 열어줍니다. 특히, mapping entropy를 효과적으로 계산할 수 있는 첫 번째 방법을 제시하여 정보 손실을 체계적으로 평가할 수 있게 합니다. 정량화된 mapping entropy는 coarse-grained 모델의 정확성을 평가하는 데 중요한 역할을 합니다.

- **Performance Highlights**: 이 연구에서는 chignolin, 인지질 이중층, 그리고 알라닌 다이펩타이드와 같은 다양한 생체 분자 시스템에 split-flows를 적용하여, 정확한 backmapping을 성공적으로 수행했습니다. 그 결과, split-flows가 coarse-grained 모델의 시스템적 평가 및 정확한 복원을 위한 원칙적인 프레임워크임을 입증했습니다. 이 연구는 다양한 생체 시스템에 걸쳐 split-flows의 응용 가능성을 보여주며, molecular modeling 분야에서의 진전을 이끌 것으로 기대됩니다.
...
### RobustVLA: Robustness-Aware Reinforcement Post-Training for Vision-Language-Action Models (https://arxiv.org/abs/2511.01331)
- **What's New**: 이 논문에서는 RobustVLA라는 경량의 온라인 RL (Reinforcement Learning) 후처리 방법을 소개합니다. 이 방법은 VLA (Vision-Language-Action) 모델의 환경 변화에 대한 강건성을 향상시키기 위해 설계되었습니다. VLA 모델이 Out-of-Distribution (OOD) 시나리오에서 신뢰성 있게 일반화하지 못하는 문제를 해결하기 위해, 환경 불확실성에 대한 강건성을 체계적으로 분석했습니다.

- **Technical Details**: RobustVLA는 두 가지 중요한 정규화 방법을 사용합니다: Jacobian 정규화와 smoothness 정규화입니다. Jacobian 정규화는 관측 소음에 대한 민감도를 완화하고, smoothness 정규화는 행동 변화에 따라 정책의 안정성을 향상시킵니다. 이러한 정규화 기법은 VLA 모델의 의사결정 과정에서 환경 변동에 대한 민감도를 명시적으로 제어하여 신뢰할 수 있는 정책을 보장합니다.

- **Performance Highlights**: 다양한 로봇 환경에서의 광범위한 실험을 통해 RobustVLA가 기존의 최신 VLA 모델들보다 환경 불확실성과 변동성에 대해 우수한 저항성을 보여 주었습니다. 이 결과는 VLA 모델의 안정성과 신뢰성을 향상시키기 위한 강건성을 인지한 RL 후처리의 중요성을 강조합니다. RobustVLA를 통해 실시간 상호작용에서 로봇의 성능이 크게 향상될 수 있습니다.



### Black-Box Differentially Private Nonparametric Confidence Intervals Under Minimal Assumptions (https://arxiv.org/abs/2511.01303)
- **What's New**: 본 연구에서는 차별적 개인정보 보호(differential privacy)를 유지하면서 비모수 신뢰 구간(confidence interval)을 구성하는 새로운 프레임워크를 소개합니다. 이 방법은 데이터를 반복적으로 서브샘플링하고, 각 서브샘플에 대해 비공개 추정기를 적용한 후, 결과값의 경험적 누적분포함수(empirical CDF)를 신뢰 구간으로 변환합니다. 제안된 방법은 기존의 비모수 신뢰 구간 방식보다 더 일반적이며, 경험적 증거를 통해 기존 최첨단 알고리즘들과 비교할 때 우수한 성능을 보입니다.

- **Technical Details**: 이 프레임워크는 기존의 차별적 개인정보 보호 방식의 한계를 극복하기 위해, 첫 번째 단계로 전체 데이터셋에 대해 통계량(statistic)을 계산하고, 두 번째 단계로 크기가 m인 서브셋을 샘플링하여 T개의 통계량을 얻습니다. 이러한 접근 방식은 경험적 CDF를 구성하여 신뢰 구간을 형성하는 데 사용되며, 이 CDF는 샘플 크기가 증가할수록 비공식 통계량의 분포에 수렴합니다. 결과적으로 제안된 신뢰 구간은 점근적(asymptotic)으로 유효하고 견고하며 비공식 방법과 동등한 수준에 도달할 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 소규모 샘플 크기에서도 안정적이며 신뢰할 수 있는 성능을 보입니다. 특히, 기존 부트스트랩(bootstrapping) 방법보다 더 완화된 가정 하에서도 정확한 신뢰 구간을 생성할 수 있습니다. 또한, 계산한 신뢰 구간은 기존 비공식 방법들보다 우수한 정확성을 입증하며, 다양한 통계량에 대한 비공개 데이터 분석에 적합하다는 것을 보여줍니다.



### Optimal Attention Temperature Enhances In-Context Learning under Distribution Shif (https://arxiv.org/abs/2511.01292)
Comments:
          26 pages, 6 figures

- **What's New**: 본 논문은 프리트레인된 Transformers에서 주의 온도(attention temperature)가 in-context learning (ICL) 성능에 미치는 영향을 이론적으로 및 경험적으로 분석합니다. 최근 연구에서는 softmax의 주의 온도를 조정하면 Transformer의 성능이 향상될 수 있음을 시사하지만, 분포 이동(distribution shift) 상황에서의 주의 온도의 역할은 전혀 탐구되지 않았습니다. 이 연구는 주의 온도를 최적화함으로써 ICL의 일반화 오류를 최소화할 수 있는 접근법을 제시합니다.

- **Technical Details**: 본 연구에서 사용된 'linearized softmax' 프레임워크를 통해, 입력 공분산(input covariance) 또는 레이블 노이즈(label noise)가 ICL에 미치는 부정적 영향을 증명하고 이에 따라 최적의 주의 온도 값을 도출하였습니다. 주의 온도 τ가 softmax 출력의 분산을 조절하고, 특정 입력에 대한 주의 가중치(selectivity)의 간섭을 관리하는 중요한 파라미터임을 강조하고 있습니다. 이를 통해, 주의 온도의 적절한 설정이 ICL의 성능 개선에 매우 중요한 요소로 작용한다는 것을 확인할 수 있습니다.

- **Performance Highlights**: 연구 결과는 주의 온도가 프리트레인된 Transformers의 ICL 성능을 개선하는 강력하고 원칙적인 메커니즘임을 입증했습니다. 선형 회귀(linear regression) 작업과 GPT-2 및 LLaMA2-7B와 같은 대규모 모델을 활용한 질문-응답(많은 QA) 벤치마크 실험을 통해 이 예측을 검증하였습니다. 이 논문은 주의 온도 조절이 다양한 ICL 작업에서 성능을 향상시킬 수 있는 단순한 방법임을 보여줍니다.
...
### Stability of the Kim--Milman flow map (https://arxiv.org/abs/2511.01154)
- **What's New**: 이번 논문에서는 Kim-Milman 흐름 맵(heat flow map)의 안정성을 목표 측정(target measure)의 변화에 대해 분석했습니다. 최근 머신 러닝 분야에서 'probability flow ODE'라는 명칭으로 주목받고 있는 이 맵은 생성 모델링(Generative Modeling) 작업에 효과적임이 입증되었습니다. 이 연구는 전통적인 Wasserstein 거리 대신 상대적인 Fisher 정보(relative Fisher information)에 대해 안정성(stability)이 유지됨을 보여줍니다.

- **Technical Details**: 연구에서 다루는 Kim-Milman 흐름 맵은 동적으로 정의된 전송 맵(trnasport map)입니다. 이 흐름 맵은 표준 다차원 가우시안 분포(standard Gaussian distribution)를 기반으로 하여 목표 측정이 변할 때의 안정성 특성을 연구합니다. 주어진 수식 FI(ν || μ)를 통해 상대적인 Fisher 정보가 두 목표 측정 간의 다양성을 측정하는 기준으로 사용됩니다.

- **Performance Highlights**: 논문의 주요 성과는 Kim-Milman 흐름 맵이 강하게 로그-볼록(log-concave)한 측정의 섭동(perturbation)이나 asymptotically positive convexity profile을 가진 측정에 대해 안정성을 보여 준다는 점입니다. 이러한 결과는 기존의 엔트로픽 최적 전송 맵(entropic optimal transport maps)으로는 다루지 못했던 경우로, 새로운 안정성 결과를 제공합니다. 이는 실제 생성 모델링에서의 활용 가능성을 열어 줄 수 있습니다.
...
### Generative Machine Learning Models for the Deconvolution of Charge Carrier Dynamics in Organic Photovoltaic Cells (https://arxiv.org/abs/2511.01118)
- **What's New**: 이번 연구에서는 유기 태양광 장치의 효율성과 안정성에 중요한 charge carrier dynamics(전하 운반체 동역학)를 모델링하기 위해 -LLODE(eta-Linearly Decoded Latent Ordinary Differential Equations)라는 머신러닝 프레임워크를 소개합니다. 기존의 분석 방법으로는 어려운 이 동역학 분석이 가능해졌습니다. 이 모델은 P3HT:PCBM 셀의 시간에 따라 변하는 전하 추출 측정값에서 동역학을 분리하고 재구성하는 데 중점을 두고 있습니다.

- **Technical Details**: 모델은 압축된 지수 감쇠(compressed exponential decay)로 잘 설명되는 기본 전하 운반체 행동을 분석하도록 고안되었습니다. 또한, 학습한 해석 가능한 잠재 공간(latent space)은 실험 측정 조건의 보간 및 외삽을 포함하여 시뮬레이션을 가능하게 합니다. 이는 기존 데이터를 기반으로 새로운 실험 조건에서의 예측을 위해 유용한 도구가 됩니다.

- **Performance Highlights**: 이 연구의 결과는 유기 태양광 장치 연구에 있어 중요한 예측 도구를 제공합니다. 이를 통해 장치 연구 및 최적화를 지원하여 태양 전지 성능 개선에 기여할 수 있는 가능성을 보여줍니다. 향후 이러한 기술은 더 효율적인 전지 설계 및 개발에 필수적인 역할을 할 것으로 기대됩니다.



### SLAP: Shortcut Learning for Abstract Planning (https://arxiv.org/abs/2511.01107)
- **What's New**: 본 연구에서는 기존 TAMP(options)를 활용하여 새로운 옵션을 자동으로 발견하는 방법, 즉 Shortcut Learning for Abstract Planning(SLAP)을 제안합니다. 이 방법은 모델 프리 강화학습(model-free reinforcement learning, RL)을 사용하여 TAMP에서 유도된 추상 계획 그래프에서 단축 경로를 학습합니다. SLAP은 기존의 계획 방식보다 더 짧은 솔루션과 높은 작업 성공률을 달성할 수 있도록 돕습니다.

- **Technical Details**: SLAP의 핵심 아이디어는 기존의 기술(high-level skill) 구조를 활용하여 새로운 기술을 학습하는 것입니다. SLAP는 추상 상태 간의 유망한 단축 연결(shortcut connections)을 식별하며, 목표 기반 보상을 통해 RL 옵션 학습 환경을 생성합니다. 이를 통해 SLAP는 새로운 옵션을 생성하여 짧은 계획을 생성하는 데 활용할 수 있습니다.

- **Performance Highlights**: 네 가지 로봇 환경에서 SLAP의 성능을 평가한 결과, SLAP는 기존의 수평 및 계층적 RL 모델보다 높은 성공률을 유지하면서 더 짧은 실행 시간을 달성했습니다. SLAP에 의해 발견된 단축 경로의 수는 훈련 시간 동안 증가하며, 이는 출력 계획 길이의 개선으로 이어졌습니다. SLAP는 새로운 물체 수에 대한 작업에도 일반화할 수 있는 능력을 보여, 로봇 계획에 혁신적인 변화를 선도하고 있습니다.



### Hyper Hawkes Processes: Interpretable Models of Marked Temporal Point Processes (https://arxiv.org/abs/2511.01096)
- **What's New**: 본 논문에서는 인과관계를 잘 설명하면서도 우수한 예측 성능을 유지하는 새로운 MTPP 모델인 하이퍼 호크스 프로세스(hyper Hawkes process, HHP)를 소개합니다. 기존의 호크스 프로세스를 확장하여 잠재 공간(latent space)으로의 차원 확장을 통해 표현력을 높였고, 하이퍼네트워크(hypernetwork)를 도입하여 시간과 데이터 의존적인 역학(dynamics)을 허용합니다.

- **Technical Details**: HHP는 고전적인 호크스 프로세스의 구조를 유지하면서도 조건부 선형성과 조각별 선형성을 보유합니다. 이러한 확장은 모델의 유연성을 높이고, MTPP가 다루는 다양한 벤치마크 과제에서 최첨단(performance) 기술을 달성하게 합니다. 모델의 예측 생성 과정을 투명하게 분석할 수 있는 방법도 제공합니다.

- **Performance Highlights**: HHP는 여러 벤치마크 태스크와 메트릭에서 최첨단 성능을 기록하고 있습니다. 특히, 원래 호크스 프로세스의 구조를 활용함으로써 예측 생성 방식에 대한 투명성을 제공해 연구자들이 결과를 분석할 수 있도록 합니다.
...
- **What's New**: 이 논문은 변이 추론(Variational Inference, VI)의 새로운 대칭 기반 보장을 제공하며, 특히 위치-스케일(location-scale) 분포군을 다룹니다. VI는 목표 밀도인 $p$를 해결 가능한 분포 군인 $Q$ 내에서 최적의 근사인 $q^*$로 근사하는 방법입니다. 저자들은 VI가 어떻게 특정 대칭을 가진 밀도에 대해 평균(mean)과 상관 행렬(correlation matrix)을 회복할 수 있는지를 확장합니다. 이 보장은 대칭성이 부족한 경우에도 유효합니다.

- **Technical Details**: 논문에서는 다양한 종류의 차이(divergence)를 통해 VI의 동작을 확장합니다. 일반적으로 VI는 역 쿨백-라이블러 다이버전스(reverse Kullback-Leibler divergence)를 최소화하여 최적의 근사를 찾습니다. 하지만 여기서는 다른 종류의 다이버전스도 허용하는 영역에서 더 넓은 확장의 가능성을 보여줍니다. 특히, 대칭성을 부분적으로 가진 밀도에 대해 VI가 회복할 수 있는 평균과 상관관계에 대한 보장을 제공합니다.

- **Performance Highlights**: 이 연구는 여러 실험적 환경에서 이론적 결과를 검증합니다. VI 방법의 정확성이 보장되는 조건을 명시하며, 특히 베이지안 계층 모델에서 자주 발생하는 기하학적 특정에 주목합니다. 결과적으로, 대칭성이 있는 경우 다양한 다이버전스 기준으로 얻어진 근사값이 동일한 통계적 속성을 회복할 수 있음을 보여줍니다.



### Binary perceptron computational gap -- a parametric fl RDT view (https://arxiv.org/abs/2511.01037)
- **What's New**: 최근 연구에 따르면 비대칭 바이너리 퍼셉트론(ABP)은 두 단계 전이 제약 밀도 임계값이 등장하는 통계-컴퓨팅 격차(statistical-computational gap)를 나타내는 것으로 보인다. 이 임계값은 (i) 저장 메모리로 작동하는 성공/실패를 결정하는 만족 가능성 임계값(satisfiability threshold) $	extit{α}_c$와 (ii) 저장 메모리로 작동할 수 있도록 ABP의 가중치를 효율적으로 결정할 수 있는 알고리즘 임계값(algorithmic threshold) $	extit{α}_a$로 구성된다. 이러한 맥락에서 완전히 들어 올린 랜덤 이중성 이론(fully lifted random duality theory, fl RDT)의 특정 매개변수 활용을 고려하고 ABP의 알고리즘적 의미를 연구한다.

- **Technical Details**: 논문의 주요 발견은 fl RDT의 리프팅 수준을 진행하면서 나타나는 중요한 구조적 매개변수 변화이다. 첫 두 수준에서 $	extit{c}$ 수열은 자연 감소형(natural decreasing type)으로 나타나며, 이러한 현상이 상위 수준에서 변화되는 것은 $	extit{α}_c$와 $	extit{α}_a$의 변화를 연결한다. 두 번째 수준에서 비판적 제약 밀도 $	extit{α}=	extit{α}_c	ext{≈0.8331}$에 대한 구체적인 수치가 주어진다. 상위 수준으로 진행하면서 이 추정치는 감소하고 다섯 번째 수준에서는 $	extit{α} 	ext{≈0.7764}$로 수렴의 양호한 수준을 관찰한다.

- **Performance Highlights**: 유의미한 점은 얻어진 제약 밀도 추정치가 클러스터링 분해(clustering defragmentation)의 범위인 $	extit{α}	ext{∈(0.77,0.78)}$와 상당한 일치를 보인다는 것이다. 이는 최근에 유사한 유형의 임계값에 근접하는 효율적인 알고리즘의 존재가 입증된 부정 호프필드 모델(negative Hopfield model)의 특성과도 밀접하게 연결된다. 따라서 ABP에 대한 알고리즘 효율성에 관한 이해를 심화시키는 결과를 도출하며, 이러한 발견은 AI 분야에서의 퍼셉트론 연구의 발전에 중요한 기여를 할 것이다.
...
### Transformer-Based Decoding in Concatenated Coding Schemes Under Synchronization Errors (https://arxiv.org/abs/2511.00999)
Comments:
          16 pages, 19 figures, a shortened version was published in the ISIT 2025 conference

- **What's New**: 이번 연구에서는 삽입, 삭제 및 대체로 손상된 여러 개의 노이즈가 있는 복사본으로부터 코드워드를 재구성하는 문제를 다룹니다. 기존의 BCJR 알고리즘은 노이즈 복사본의 수가 증가함에 따라 지수적으로 확장되기 때문에, 약 4개 복사본 이상에서는 재구성이 불가능했습니다. 이를 해결하기 위해, 우리는 BCJRFormer라는 트랜스포머(Transformer) 기반의 새로운 내부 디코더를 제안합니다.

- **Technical Details**: BCJRFormer는 기존 BCJR 알고리즘과 유사한 오류 비율을 제공하며, 노이즈 복사본의 수에 대해 제곱적으로 확장됩니다. 이는 DNA 데이터 스토리지와 같은 상황에서 여러 번의 DNA 스트랜드 읽기에서 효과적입니다. 또한, Belief Propagation 외부 디코더를 트랜스포머 기반 디코더로 교체하여 오류 비율을 낮추고, 효율적인 end-to-end 파이프라인을 구현합니다.

- **Performance Highlights**: BCJRFormer는 단일 메시지 전송에 대한 바이너리 및 쿼터너리 마커 코드에서 중요한 성능 개선을 보여줍니다. 더 나아가, 우리는 ConvBCJRFormer라는 새로운 크로스 어텐딩(transformer architecture) 아키텍처를 제안하여 컨볼루션 코드워드를 디코딩할 수 있는 첫걸음을 마련했습니다. 이러한 발전은 더 일반적인 선형 코드 클래스의 상호 및 외부 디코딩을 향한 초기 단계로 볼 수 있습니다.
...
### Towards Channel Charting Enhancement with Non-Reconfigurable Intelligent Surfaces (https://arxiv.org/abs/2511.00919)
- **What's New**: 이 논문에서는 슬기로운 전파 환경(Smart Radio Environment) 내에서 완전히 수동적인 전자기 스킨(Electromagnetic Skins, EMSs)을 설계하여 채널 차팅(Channel Charting, CC)을 개선하는 방법을 탐구합니다. 기존의 재구성 가능한 지능형 표면(Reconfigurable Intelligent Surfaces, RISs)의 최적화 방법과는 달리, EMS는 정적이며 피드백 없이 작동하여 위치 정밀도를 높이는 데 도움을 줍니다. 이번 연구는 반 감독 학습을 활용하여 획득한 결과의 일관성을 검증하고, 신뢰도와 연속성을 개선하는 정량적 기준을 통해 최악의 사용자에게 맞춘 정적 EMS 위상 프로필을 설계했습니다.

- **Technical Details**: 제안된 EMS 단계는 두 가지로 나누어집니다: 학습 단계에서는 레이블이 있는 테스트 포인트에 대한 채널 상태 정보(Channel State Information, CSI)를 수집하고, 추론 단계에서는 학습된 저차원 다양체를 사용하여 미지의 사용자 위치를 추정합니다. 이 연구는 t-분포 확률적 이웃 임베딩(t-SNE)과 오토인코더(Autoencoder, AE) 기반의 반감독 채널 차팅 모델을 통합하여 분석합니다. 또한, 신호 대 잡음비(Signal-to-Noise Ratio, SNR)와 채널 비대칭성의 균형이 위치 정밀도를 향상시키는 데 중요함을 수치적으로 입증했습니다.

- **Performance Highlights**: 시뮬레이션 결과, 최적화된 EMS 구성이 90번째 백분위수 위치 오차를 50미터에서 25미터 이하로 줄이며, 도전적인 사용자 위치에서의 신뢰성(Trustworthiness, TW)과 연속성(Continuity, CT)을 크게 향상시킵니다. 이러한 개선은 다양한 알고리즘 선택과 감독 수준에서도 일관되게 나타났습니다. 이 연구의 결과는 EMS의 물리적 최적화가 채널 차팅 애플리케이션에 있어 실제 가치가 있음을 보여줍니다.
...
- **What's New**: 이 논문에서는 랜덤 피쳐 신경망(RFNN)을 이용하여 고차원 편미분 방정식(PDE)의 솔루션을 구하는 새로운 방법인 HEATNET을 제안합니다. HEATNET는 열 연산자의 기본해(Green's function)에서 파생된 랜덤화된 열 커널을 사용하여 편미분 방정식의 솔루션에 대한 편향 없는 보편 근사기를 제공합니다. 또한, HEATNET은 고차원 문제의 특성을 고려하여 효율적으로 확장 가능하다는 점을 강조합니다.

- **Technical Details**: HEATNET은 고차원에서의 편미분 방정식 솔루션을 매우 정확하게 근사합니다. 이 신경망 구조는 열 방정식과 관련된 확률적 전이 밀도를 계산하는 방식을 바탕으로 하며, 이를 통해 미분 연산자와 관련된 약한 형태의 솔루션을 생성합니다. HEATNET은 특히 샘플링을 통해 차원의 저주를 극복할 수 있는 방법론이 포함되어 있으며, 중요 샘플링 기법을 사용하여 학습의 효율성을 높입니다.

- **Performance Highlights**: 평가 결과, HEATNET은 500차원에 대해 $1.0E-05$에서 $1.0E-07$까지의 근사 오차를 보였고, 1,000차원에서 2,000차원까지는 $1.0E-04$에서 $1.0E-03$ 범위의 오차를 기록했습니다. 이는 HEATNET이 적은 수의 특징(최대 15,000개)으로도 높은 정확성을 달성함을 보여줍니다. 이 모델은 일반적인 수치적 방법들보다 뛰어난 성능을 지니고 있습니다.
...
### Perturbations in the Orthogonal Complement Subspace for Efficient Out-of-Distribution Detection (https://arxiv.org/abs/2511.00849)
- **What's New**: 이 논문에서는 OOD(Out-of-Distribution) 샘플을 효과적으로 감지하기 위한 새로운 경량화된 방법인 P-OCS(Orthogonal Complement Subspace의 Perturbations)를 제안합니다. 기존의 방법들처럼 높은 차원에서 ID(In-Distribution)와 OOD 샘플을 구별하는 대신, P-OCS는 주어진 피처의 주축에 대해 외적 보완(subspace)에서의 단일 변동만을 이용하기 때문에 더 효과적으로 섬세한 불일치를 포착합니다. 이 방법은 기존의 모델 재훈련이나 OOD 데이터 접근 없이도 뛰어난 성능을 보여줍니다.

- **Technical Details**: P-OCS 방법은 ID 피처의 주축에서 계산된 외적 보완에서 이루어지는 단일 단계의 변동으로 OOD를 감지합니다. 펜얼티미닛 레이어에서 특징을 계산하고, 이 특징의 변동을 정량적으로 측정하여 OOD 점수를 도출합니다. 모델의 복잡성은 𝒪(T⋅d²)로, 이는 외적 보완 공간에서의 행렬 곱셈에 의해 지배됩니다.

- **Performance Highlights**: 여러 아키텍처와 데이터셋을 통해 P-OCS는 최신 OOD 감지 성능을 달성하였고, 적은 계산 비용이 소요됨에도 불구하고 모델 구조 변경 없이도 높은 검출력을 보였습니다. 특히, 근접 OOD(near-OOD)와 원거리 OOD(far-OOD) 모두에서 효과적인 성능 향상을 입증하여 기존 방법들과 비교해 우수한 결과를 나타냈습니다.



### Real-Time Learning of Predictive Dynamic Obstacle Models for Robotic Motion Planning (https://arxiv.org/abs/2511.00814)
Comments:
          10 pages, 6 figures, submitted to IEEE International Conference on Robotics and Automation (ICRA) 2025

- **What's New**: 이 논문은 실시간으로 불완전하고 노이즈가 많은 데이터를 이용해 타 단말의 움직임을 예측할 수 있는 비선형 예측 모델을 학습하는 방법을 제안합니다. 제안된 프레임워크는 수정된 슬라이딩 윈도우 핸켈 다이나믹 모드 분해(Hankel-DMD)를 사용하여 이러한 동역학을 제거하고 예측하는 기능을 가지고 있습니다. 이를 통해 안정적이고 안전한 실시간 제어 프레임워크에 통합할 수 있는 단기 예측 및 노이즈 제거가 가능합니다.

- **Technical Details**: 본 논문에서는 Cadzow 프로젝션을 통해 구조화된 저등급 일관성을 유지하며, Singular-Value Hard Thresholding (SVHT)을 사용하여 효과적인 랭크를 추정합니다. 새로운 예측 모델은 시간에 따라 변화하는 핸켈-DMD 리프트 선형 예측기를 기반으로 하며, 노이즈 분산을 추적할 수 있는 신호를 제공합니다. 이 과정에서 제안된 방법은 고가우시안 및 강한 꼬리 분포의 노이즈 하에서도 검증되었습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 안정적인 분산 인식 노이즈 제거와 짧은 시간 예측 성능을 보여주며, 이는 실제 제어 및 계획 작업에 적합합니다. 특히, 이 방법은 동적 크레인 테스트베드에서 실험적으로 입증되었으며, 알고리즘은 스트리밍 관측값에 기반하여 실시간으로 예측 모델을 업데이트할 수 있습니다. 전반적으로 이 연구는 복잡한 비선형 동작 예측을 보다 실용적으로 접근하는 새로운 가능성을 제공합니다.
...
- **What's New**: 이번 연구에서는 AReaL-Hex라는 새로운 비동기 강화 학습(RL) 훈련 시스템을 소개합니다. 이 시스템은 이질적인 GPU 클러스터에서의 RL 훈련 작업의 엔드 투 엔드 처리량과 비용 효율성을 극대화하도록 설계되었습니다. 특히, AReaL-Hex는 비동기 RL 훈련의 다단계 프로세스를 효과적으로 관리하여, 각 단계의 자원을 최적화하는 것을 목표로 합니다.

- **Technical Details**: AReaL-Hex는 두 단계의 스케줄링 알고리즘을 사용합니다. 첫 단계에서는 제한된 자원 예산 하에 각 단계별 병렬화 전략과 작업 할당을 결정하기 위해 혼합 정수 선형 프로그래밍(MILP)을 사용합니다. 두 번째 단계에서는 그래프 파티셔닝을 통해 이질적인 장치 간 자원 할당을 최적화하여 엔드 투 엔드 처리량을 극대화합니다.

- **Performance Highlights**: AReaL-Hex는 동일한 훈련 예산을 유지하면서도 최대 1.50배 높은 훈련 처리량을 달성하며, 동일한 처리량을 달성하는 경우 훈련 비용을 최대 1.46배 줄이는 성과를 보였습니다. 이러한 결과는 이질성 인식 스케줄링이 비동기 RL 훈련의 실질적인 이점을 제공함을 보여줍니다.
...
- **What's New**: CELEC는 대규모 언어 모델(LLM)을 활용한 자동화된 전자 건강 기록(EHR) 데이터 추출 및 분석 프레임워크입니다. 이 시스템은 자연어 질문을 SQL 쿼리로 변환하여 데이터 접근성을 높이고 연구자들이 EHR 데이터베이스를 쉽게 탐색할 수 있도록 지원합니다. CELEC는 개인정보 보호 프로토콜을 엄격히 준수하여 LLM이 데이터베이스 메타데이터에만 접근하게 하고, 환자 데이터를 외부 서비스와 공유하지 않도록 설계되었습니다.

- **Technical Details**: CELEC은 데이터베이스 스키마 정보를 포함한 프롬프트 전략을 통해 자연어(NL) 질문을 SQL 쿼리로 변환합니다. 이 과정에서 소수 샘플 데모와 사고 과정(Chain-of-Thought, CoT) 논리를 결합하여 정확성과 강인성을 향상시킵니다. 한 번 데이터가 추출되면, CELEC은 즉시 시각화를 생성하여 사용자가 데이터를 쉽게 탐색할 수 있게 도와줍니다.

- **Performance Highlights**: 대조군 EHRSQL 벤치마크에서 CELEC은 이전 시스템과 유사한 실행 정확성을 달성하면서도 낮은 지연시간과 비용 효율성을 유지합니다. 각 구성 요소가 성능에 미치는 영향을 검증하는 압축 연구도 수행되었으며, 특히 소수 샘플 데모가 중요한 역할을 한다는 점이 밝혀졌습니다. 연구자들은 CELEC을 통해 EHR 데이터베이스를 직접 쿼리하고 초급 분석을 수행함으로써 업무 흐름을 간소화하고 생의학적 발견을 가속화할 수 있습니다.



### A Framework Based on Graph Cellular Automata for Similarity Evaluation in Urban Spatial Networks (https://arxiv.org/abs/2511.00768)
- **What's New**: 이 논문에서는 도시의 공간 네트워크에서 유사성을 측정하기 위한 새로운 방법론인 GCA-Sim을 제안합니다. 기존의 유사성 측정 방법들은 공간 네트워크에 적합하지 않아 효과적으로 구별하기 어렵다는 문제점을 해결하고자 합니다. GCA-Sim은 그래프 셀룰러 오토마타(graph cellular automata)를 기반으로 하여 정보 진화 과정의 여러 단계에서 기록된 값 분포의 발산을 통해 유사성을 측정합니다.

- **Technical Details**: GCA-Sim은 네트워크 신호 간의 차이를 증폭시키는 전파 규칙을 통해 '네트워크 공명(network resonance)'이라는 개념을 도입합니다. 개선된 미분 가능 논리 게이트 네트워크를 사용하여 여러 서브모델을 학습하며, 각 서브모델은 네트워크 공명을 유도합니다. 이 프레임워크의 서브모델들은 50개의 도시 및 50개의 구역 수준 도로 네트워크에서 클러스터링 성능으로 평가되었으며, 실루엣 점수(Silhouette score)가 0.9 이상으로 기존 방법들을 능가합니다.

- **Performance Highlights**: GCA-Sim의 최상위 서브모델을 통해 계획된 거리 네트워크는 자생적으로 성장한 거리 네트워크보다 내부 동질성이 낮다는 것을 관찰했습니다. 또한, 다양한 도메인에서의 형태학적 범주가 유사한 중요성을 가지고 기여하며, 기본적인 위상적 신호로서의 차수(degree)는 반복마다 토지 가치와 관련 변수를 점점 더 잘 반영하게 됩니다.



### Trust Region-Based Bayesian Optimisation to Discover Diverse Solutions (https://arxiv.org/abs/2511.00750)
- **What's New**: 이 연구에서는 다양한 차원의 블랙박스 문제를 위한 트러스트 지역 기반의 베이지안 최적화 알고리즘의 효과를 탐구하고, TuRBO1을 확장한 divTuRBO1을 제안합니다. divTuRBO1은 참조 솔루션 집합에 대해 주어진 거리 한계를 유지하면서 최적 솔루션을 찾습니다. 이 알고리즘은 다양한 솔루션을 찾기 위해 divTuRBO1을 순차적으로 또는 교차적으로 실행하는 두 가지 접근 방식을 제안합니다.

- **Technical Details**: 베이지안 최적화(Bayesian Optimisation, BO)는 확률적 대리 모델과 획득 함수(acquisition function)를 통해 진행되며, 대리 모델은 가우시안 프로세스 모델로 구성되어 비싼 블랙박스 함수의 평가를 줄입니다. 각 반복에서 선택된 새로운 데이터 포인트에 기반하여 대리 모델이 훈련됩니다. 트러스트 지역(trust region)은 이러한 대리 모델의 예측을 통해 탐색 범위를 제한하며, 알고리즘의 적응성을 높입니다.

- **Performance Highlights**: 실험 결과에서 제안한 알고리즘들은 특히 큰 차원에서 강력한 성능을 보였습니다. divTuRBO1과 ROBOT 알고리즘 사이의 비교를 통해 연구된 개선 사항들이 실제 문제에서 유의미한 성과를 내는 것을 입증하였습니다. 평가 예산이 제한적일 때에도 제안된 방법들이 우수한 결과를 보여줍니다.



### Correspondence Between Ising Machines and Neural Networks (https://arxiv.org/abs/2511.00746)
Comments:
          22 pages, 4 figures

- **What's New**: 이번 논문에서는 Ising 모델을 활용한 계산을 기존의 저온에서의 기초 상태(ground state) 기반의 처리에서 스핀 평균(spin averages)을 통한 새로운 접근 방식으로 확장합니다. 이는 높은 온도에서도 Ising 시스템이 실제적으로 작동하도록 하여, 소음(tolerance to noise)을 고려한 확률적 계산을 가능하게 합니다. 또한, Ising 디바이스와 신경망(neural networks) 간의 체계적인 대응 관계를 정립하여, 훈련된 피드포워드 신경망(feed-forward neural networks)을 Ising 하드웨어에서 실행할 수 있는 간단한 방법을 소개합니다.

- **Technical Details**: Ising 회로(Ising circuits)는 고온에서의 시간 평균을 통해 관측되는 방식으로 설계되며, 전통적인 모델과는 달리 단일 상태의 샘플링 대신에 사용됩니다. 이 시스템은 스핀(spin)으로 이루어진 유한 집합을 다루며, 이는 특정 상태 공간을 가집니다. 또한, 해당 시스템은 쿼드라틱 해밀토니안(quadratic Hamiltonian)을 통해 구성되며, 이 해밀토니안의 매개변수는 훈련된 신경망과 매핑되어 신경망처럼 작동하도록 설계되었습니다.

- **Performance Highlights**: 이 논문에서 제시된 주요 결과는 이진 입력과 출력을 가지는 훈련된 신경망의 매개변수를 Ising 해밀토니안의 계수에 매핑하면, 결과적으로 생성된 Ising 시스템이 신경망처럼 행동한다는 것입니다. 이 방식은 저온에서의 깊은(sign networks) 신경망부터 고온에서의 얕은(tanh networks) 신경망까지 다양한 기능을 효과적으로 수행할 수 있음을 보여줍니다. 실험 결과는 2비트 정수 곱셈에 대한 성능을 포함하여, Ising 시스템의 행동을 구체적으로 검증하고 있습니다.
...
- **What's New**: 이 논문은 대규모 언어 모델(LLMs)을 활용하여 시뮬레이션 최적화(Simulation Optimization, SO) 알고리즘의 설계를 자동화하는 SOCRATES라는 새로운 절차를 제안합니다. 기존의 일반 목적 SO 방법을 탈피하여, LLM을 이용해 맞춤형 최적화 절차를 적용하며, 이를 통해 복잡하고 비용이 많이 드는 스토캐스틱 시스템의 최적화를 실현하고자 합니다. 이 방식은 특히 고도의 복잡한 플로우가 존재하는 시스템의 최적화에서 그 유용성을 발휘합니다.

- **Technical Details**: SOCRATES의 접근 방식은 두 단계로 나뉘어 있습니다. 첫 번째 단계에서는 LLM을 통해 시스템의 텍스트 설명에서 인과 구조를 추론하여 디지털 복제 모델인 Operational AI Replicas (OARs)를 학습합니다. 두 번째 단계에서는 이 복제 모델을 시험대로 삼아 여러 기본 SO 알고리즘을 평가하고, 최종 알고리즘을 계속해서 검토 및 수정하는 메타-최적화 과정을 거칩니다. 이러한 이중 단계를 통해 최종 알고리즘은 문제의 구조에 맞춰 조정됩니다.

- **Performance Highlights**: SOCRATES는 샘플 효율성을 높이면서 비교적 낮은 비용으로 최적화 문제를 해결하는 데 강점을 보입니다. Adaptive feedback loop을 통해, 실제 시스템에서 예상치 못한 성과 편차가 생기면 즉시 최적화 전략을 수정할 수 있습니다. 이러한 특징들은 SOCRATES를 통해 복잡한 최적화 문제에 대한 효과적이고 적응 가능한 솔루션을 제공하는 기반을 마련합니다.
...
### Filtered Neural Galerkin model reduction schemes for efficient propagation of initial condition uncertainties in digital twins (https://arxiv.org/abs/2511.00670)
- **What's New**: 본 연구에서는 디지털 트윈(Digital Twin)에서의 불확실성 정량화(UQ)가 데이터를 넘어서 신뢰할 수 있는 예측을 가능하게 한다고 강조합니다. 특히 초기 조건의 불확실성을 비선형 축소 모델을 통해 전파하는 필터링된 축소 모델링 접근법을 제안합니다. 이는 몬테 카를로(Monte Carlo)와 같은 전통적인 집합 기반 방법의 높은 계산 비용을 피하면서도 비용 효율적입니다.

- **Technical Details**: 필터링된 축소 모델은 Neural Galerkin 스킴(Neural Galerkin schemes)을 기반으로 하며, 사전 학습된 신경망(pre-trained neural networks)을 사용하여 비선형 근사를 제공합니다. 이 모델은 초기 조건의 불확실성에 의해 유도된 축소 해의 분포의 평균 및 공분산을 전파합니다. Neural Galerkin 다이나믹스를 선형화하여 Gaussian 필터링과 유사한 구조로 닫는 방식으로 평균 및 공분산 dynamics를 도출합니다.

- **Performance Highlights**: 실험 결과, 필터링된 Neural Galerkin 스킴은 기존의 집합 기반 불확실성 전파 방법에 비해 1차 정밀도 이상의 속도 향상을 보여주었습니다. 시간 통합(Tim Integration)에서 발생하는 계산 비용은 축소 차원에 비례하여 증가하였고, 기존 방법 대비 1차 정밀도 이상의 비용 절감을 달성했습니다. 이를 통해 비선형 문제에서도 효과적인 불확실성 정량화를 가능하게 합니다.
...
- **What's New**: 이 논문에서는 새로운 코드 최적화 접근법인 ComPilot을 소개합니다. ComPilot은 대형 언어 모델(LLMs)과 컴파일러 간의 피드백 루프를 통해 최적화 과정을 안내하는 실험적 프레임워크입니다. 이 시스템은 LLM이 제안한 변형을 컴파일러에 전달하고, 그 결과를 바탕으로 LLM이 최적화 전략을 정교하게 개선하는 방식을 채택합니다.

- **Technical Details**: ComPilot은 루프 변형 제안의 피드백 루프를 통해 작동하며, 여기서 LLM은 변형의 합법성 및 성능을 컴파일러에 제출합니다. 이 시스템은 Tiramisu 컴파일러를 활용하여 성능 영향을 측정하며, LLM은 이전의 성공과 실패에서 학습하여 최적화 과정을 거듭할 수 있습니다. 이러한 접근 방식은 LLM의 고급 표현력을 이용해 최적화 공간을 탐색하면서도 컴파일러의 엄격한 합법성 검사를 통해 코드 생성을 유도합니다.

- **Performance Highlights**: ComPilot의 실험 결과, PolyBench 벤치마크를 통해 원래 코드에 비해 평균 3.54배의 속도 향상을 달성했습니다. 특히 특정 벤치마크에서는 100배 이상의 속도 향상도 발견되었습니다. ComPilot은 기존의 Pluto 폴리헤드럴 최적화 도구보다도 뛰어난 성능을 보이며, LLM이 주도하는 최적화 접근법의 가능성을 보여주었습니다.
...
### Accuracy estimation of neural networks by extreme value theory (https://arxiv.org/abs/2511.00490)
- **What's New**: 본 논문에서는 신경망(neural networks)의 오차(error)를 정량화하기 위해 극값 이론(extreme value theory)을 적용하는 새로운 접근 방식을 제안합니다. 일반적으로 신경망의 오차는 유한한 테스트 셋에서의 최대 오차(maximal error)와 비교할 때 현저하게 큰 값을 가질 수 있습니다. 이러한 큰 오차 값을 통계적으로 다루기 위해, 해당 오차가 특정 임계값(threshold) 이상일 때의 분포가 일반화된 파레토 분포(generalized Pareto distribution)로 근사된다고 제안합니다.

- **Technical Details**: 극값 이론을 적용하여, 신경망의 오차 ℰ에 대해 특정 임계값 u를 초과하는 경우의 확률 P(ℰ>x)와 기대값 𝔼[ℰ−u∣ℰ>u]를 추정할 수 있습니다. 논문에서는 특히 파레토 분포의 형태 매개변수(shape parameter)인 γ를 추정하는 새로운 방법을 제시합니다. 이 방법은 γ<0의 조건을 만족하면서 오차 ℰ의 특성을 보다 정확하게 반영할 수 있도록 설계되었습니다.

- **Performance Highlights**: 재무(financial) 분야에서의 응용을 통해 제안된 방법의 유용성을 확인합니다. 신경망을 사용하여 금융 계약(price of the contract)의 가격 책정을 위한 오차 ℰ를 효과적으로 정량화할 수 있으며, 이는 금융 기관들이 미가격(mispricing) 위험을 보다 명확히 이해하는 데 도움을 줄 수 있습니다. 임계값 u에 대해 P(ℰ>x)와 𝔼[ℰ−u∣ℰ>u]의 계산은 금융 계약의 정확한 평가에 중요한 역할을 합니다.
...
### Trust-Region Methods with Low-Fidelity Objective Models (https://arxiv.org/abs/2511.00434)
Comments:
          Submitted to the Proceedings of Domain Decomposition Methods in Science and Engineering XXIX

- **What's New**: 본 논문은 Magical Trust Region (MTR) 프레임워크에 기반한 두 가지 다중 신뢰 영역 방법을 소개합니다. 기존의 신뢰 영역 단계를 보완하는 보조 방향을 통해 수렴 속도를 향상시키는 새로운 접근법을 제시합니다. 첫 번째 방법인 Sketched Trust-Region (STR)은 데이터 행렬을 스케치하여 고차원 문제를 저차원으로 축소하는 방식이며, 두 번째 방법인 SVD Trust-Region (SVDTR)은 데이터의 중요한 분산 방향을 캡처하는 절단된 특이값 분해를 통해 보조 방향을 정의합니다.

- **Technical Details**: STR은 신뢰 영역 서브 문제의 차원을 줄이기 위해 랜덤 스케치 행렬을 사용하여 보조 방향을 구성하고, SVDTR은 데이터 행렬의 주된 특이벡터로부터 보조 방향을 형성합니다. 두 방법 모두 첫 번째 단계로 고신뢰성 절차를 통해 단계를 계산하고, 다음에 저신뢰성 방향을 찾아 최종 업데이트를 결정합니다. 두 방법의 공통적인 점은 서로 다른 방식의 데이터 축소와 최적화 기법을 통해 효율적인 학습을 목표로 한다는 것입니다.

- **Performance Highlights**: NUMERICAL EXAMPLES를 통해 STR 및 SVDTR 방법이 기존의 고차원 신뢰 영역 방법에 비해 뛰어난 성능을 발휘함을 보여주었습니다. 수행된 평가에서는 다양한 예제 데이터셋을 사용하여 성능을 비교하였고, 실험 결과는 두 방법이 신뢰 영역 알고리즘의 수렴 속도를 향상시킬 수 있음을 나타냅니다. 최적화의 효율성을 높임으로써 데이터 기반 응용 프로그램에 더 적합한 솔루션을 제공할 수 있음을 시사합니다.
...
### A Streaming Sparse Cholesky Method for Derivative-Informed Gaussian Process Surrogates Within Digital Twin Applications (https://arxiv.org/abs/2511.00366)
- **What's New**: 이 논문에서는 디지털 트윈(Digital Twin, DT)의 정확도를 향상시키기 위해 파생 데이터(derivative data)를 포함한 가우시안 프로세스(Gaussian Process, GP) 모델을 확장하는 방법을 제시합니다. 이로 인해 실제 자산의 운영 데이터를 동적으로 업데이트하여 실시간 예측이 가능해졌습니다. 파생 데이터를 포함하는 것은 공분산 행렬(covariance matrix) 차원의 증가라는 단점이 있지만, 희소 가우시안 프로세스(sparse GP) 근사화를 통해 이 문제를 해결합니다.

- **Technical Details**: 가우시안 프로세스는 서그릿 모델링(surrogate modeling)에서 중요한 역할을 하는 수학적 도구이며, 이 논문에서는 파생 정보를 통합하여 모델의 정확도를 높이는 방법을 제시합니다. 파생 정보는 고차원 및 다중 물리학(multiphysics) 상황에서 로컬 민감도를 효과적으로 포착하는 데 사용됩니다. 이를 위해 희소 촤르스키 인수분해(sparse Cholesky factorization) 방법의 확장을 통해 파생 증가 공분산 구조를 처리합니다.

- **Performance Highlights**: 수치 실험을 통해 파생이 강화된 희소 GP 방법이 새로운 데이터를 동적으로 추가함에 따라 예측 정확도가 향상됨을 입증하였습니다. 이 방법은 항공기 구조물의 피로 균열 성장 모델링에 적용되어 결과적으로 높은 신뢰성과 비용 효과를 보여주었습니다. 이 연구는 디지털 트윈 적용에 있어 효율적이고 적응성 있는 GP 서그릿의 차별화된 기초를 제공합니다.
...
Comments:
          11 pages, 5 figures, Under review in an IEEE Transactions journal

- **What's New**: 이 논문은 IoMT 장치에 대한 악성 코드 탐지를 위한 새로운 Split Learning (SL) 기반 프레임워크를 제안합니다. 이 방법은 고객과 엣지 서버 간에 신경망 훈련을 분할하여 리소스가 제한된 클라이언트의 계산 부담을 줄이고 데이터 프라이버시를 보장합니다. SL 기법을 통해 IoT 악성 코드 탐지의 효율성과 보안을 개선하고, 전통적인 Federated Learning 방법의 한계를 극복하고자 합니다.

- **Technical Details**: 제안된 SL 프레임워크는 딥 CNN(Convolutional Neural Networks) 모델을 기반으로 하며, 훈련 과정에서 이미지 기반 악성 코드 데이터셋을 사용합니다. 이 방법은 고객 측에서 이미지 데이터를 처리하고 활성화를 서버로 전송하는 방식으로 진행됩니다. 서버는 이 활성화를 통해 예측을 생성하고 손실을 계산한 후, 경량화된 모델을 지속적으로 업데이트하는 과정을 반복합니다.

- **Performance Highlights**: 실험 결과, 제안된 SL 프레임워크는 기존의 Federated Learning 기법에 비해 정확도(6.35% 향상), F1 점수(5.03% 향상), 높은 수렴 속도(14.96% 향상), 자원 소모(33.83% 감소) 면에서 우수한 성능을 보여줬습니다. 이러한 결과는 SL이 차세대 IoT 보안을 위한 확장 가능하고 안전한 패러다임이 될 잠재력을 지니고 있음을 입증합니다.
...
### NaturalVoices: A Large-Scale, Spontaneous and Emotional Podcast Dataset for Voice Conversion (https://arxiv.org/abs/2511.00256)
Comments:
          Under review for IEEE Transactions on Affective Computing

- **What's New**: 이 논문은 감정 인식 음성 변환을 위해 설계된 최초의 대규모 자발적 팟캐스트 데이터셋인 NaturalVoices를 소개합니다. 총 5,049시간의 자발적인 팟캐스트 녹음이 포함되어 있으며, 감정, 음질, 원고, 화자 정체성, 소리 이벤트에 대한 자동 주석이 제공됩니다. 이 데이터셋은 수천 명의 화자에서 감정의 다양성을 포착하여 실제 음성 변환(VC)과 감정 음성 변환(EVC) 모델을 개발하는 데 기여할 수 있는 잠재력을 지니고 있습니다.

- **Technical Details**: NaturalVoices는 자동화된 처리 파이프라인을 통해 다중 수준의 주석을 생성하며, 이는 화자 성질, 감정 레이블, 음질 메트릭 등을 포함하여 연구자들이 특정 과제를 위한 맞춤 집합체를 구성할 수 있도록 돕습니다. 팟캐스트의 원시 형식은 VC 연구에 바로 활용하기에 적합하지 않지만, 다양한 토픽과 자연스러운 감정 변화를 잘 포착하고 있습니다. 이 구성 요소들은 개방형으로 제공되어, 연구자들이 직접 데이터셋을 사용할 수 있게 합니다.

- **Performance Highlights**: NaturalVoices를 활용한 실험 결과, 이는 자연스럽고 감정 표현이 풍부한 음성을 생성하는 데 도움이 되며, 질적 개선이 실현되는 것을 보여주었습니다. 그러나, 현재의 모델들은 이 데이터셋의 규모와 변동성을 완전히 활용하지 못하고 있어, 보다 강력하고 표현력이 뛰어난 모델의 필요성을 강조합니다. 이 연구는 음성 변환 분야를 진전시키기 위한 중요한 기회와 도전 과제를 제공합니다.



### Gradient Boosted Mixed Models: Flexible Joint Estimation of Mean and Variance Components for Clustered Data (https://arxiv.org/abs/2511.00217)
- **What's New**: 이 논문에서는 Gradient Boosted Mixed Models (GBMixed)라는 새로운 알고리즘과 프레임워크를 소개합니다. GBMixed는 선형 혼합 모델의 제약을 벗어나 비모수적(mean estimation) 방법을 사용하여 군집화된(clustering) 데이터의 평균 및 분산 성분을 동시에 추정할 수 있습니다. 이 접근법은 비모수적 모델의 장점과 통계적 추론(statistical inference) 능력을 결합하여 예측 정확도를 높입니다.

- **Technical Details**: GBMixed의 주요 혁신은 기본 학습자(base learner)를 통해 랜덤 효과(random effects) 및 잔차 분산(residual variance)을 모델링하는 방법입니다. 이 알고리즘은 최대 우도 기반의 경량 학습방식으로 업데이트하여 평균과 분산을 동시에 추정합니다. 이는 서로 다른 군집(cluster) 및 집단(population) 수준의 예측을 조정할 수 있는 공변량 의존적(covariate-dependent) 축소를 가능하게 합니다.

- **Performance Highlights**: 시뮬레이션 연구와 실제 사례 연구를 통해 GBMixed는 전통적인 선형 혼합 모델이나 비모수적 방법에 비해 더욱 정확한 예측 성능을 달성하고, 분산 성분을 정확하게 회복하며, 신뢰할 수 있는 불확실성 정량화(uncertainty quantification)를 제공합니다. 이러한 효과는 특히 군집화된 데이터에서 의미 있는 가치가 있으며, 이 프레임워크는 이질적인 치료 효과(heterogeneous treatment effects)의 추정에도 활용될 수 있습니다.



### Transfer learning discovery of molecular modulators for perovskite solar cells (https://arxiv.org/abs/2511.00204)
- **What's New**: 본 논문에서는 페로브스카이트 태양전지(PSCs)에 대한 효과적인 분자 조절제(molecular modulator)의 발견이 중요함을 강조합니다. 전통적인 실험적인 탐색 방법이 비효율적이고 비용이 많이 드는 반면, 기계 학습(ML)을 통해 신속한 소재 발견이 가능하다는 점을 피력하고 있습니다. 특히, 사전 훈련된 심층 신경망(deep neural networks)을 활용한 화학 정보 기반의 전이 학습(transfer learning) 프레임워크를 적용하여 높은 정확도를 달성했습니다.

- **Technical Details**: 제안된 프레임워크는 다양한 분자 표현(molecular representations)의 체계적인 벤치마킹을 통해 구성되었으며, 이를 통해 79,043개의 상업적으로 이용 가능한 분자에 대한 저비용과 고처리량 가상 스크리닝(virtual screening)을 수행할 수 있었습니다. 또한, 우리가 개발한 프레임워크는 해석 가능성(interpretability) 기술을 이용해 학습된 화학 표현을 시각화하고, 실험적으로 조절제와 페로브스카이트 간의 상호작용을 특성화할 수 있도록 지원합니다.

- **Performance Highlights**: 프레임워크에 의해 식별된 상위 분자 조절제들은 실험적으로 검증되었고, PSC에서 26.91%라는 놀라운 성능 향상을 기록했습니다. 이는 페로브스카이트 태양전지의 에너지 변환 효율(power conversion efficiency, PCE)을 획기적으로 개선하는 결과로, 향후 연구에 있어 중요한 이정표가 될 것입니다.



### Position: Vibe Coding Needs Vibe Reasoning: Improving Vibe Coding with Formal Verification (https://arxiv.org/abs/2511.00202)
Comments:
...
- **What's New**: 최근 'Vibe coding'이라는 소프트웨어 개발 패러다임이 급격히 인기를 끌고 있습니다. 이 방법론은 대규모 언어 모델(LLM)과의 대화를 통해 코드를 작성하는 것이지만, 개발자들은 기술 부채(technical debt), 보안 문제, 코드의 변동성(code churn) 등 주요 한계에 직면하고 있다고 보고합니다. LLM이 사용자 명령을 우선시함에 따라 이러한 문제들이 발생하고 있으며, 이는 개발자가 모순을 해결하는 데 실패하게 만듭니다.

- **Technical Details**: 이 논문에서는 LLM과의 상호작용 과정에서 발생하는 문제를 이해하고, 이를 해결하기 위한 방법으로 형식적 방법(formal methods)의 도입을 제안합니다. Vibe Reasoning 시스템은 애플리케이션에 특화된 검증 목표(autoformalization)를 자동으로 정리하고, 최소한의 효과적인 기법을 사용하여 이를 검증하며, 개발자가 원하는 대로 사전 피드백을 제공합니다. 기존의 접근 방식과 달리, 이 시스템은 핵심 불변량(critical invariants)만을 반복적으로 검토하는 방식을 취합니다.

- **Performance Highlights**: Vibe coding 과정에서 LLM은 많은 사용자 피드백을 수용할 수 있지만, 효율적으로 피드백을 생성하기 위해서는 개발자의 반복적인 테스트와 점검이 필요합니다. 이를 위해 형식적 방법을 통해 시스템 전반의 불변량과 의존성을 모델링하고 검증함으로써, LLM이 생성하는 코드의 보안성과 정확성을 향상시킬 수 있습니다. 이러한 접근은 LLM이 필수적인 정적 분석(static analysis) 기법을 수용하도록 도와주며, 소프트웨어 개발 프로세스를 더욱 신뢰성 있게 만듭니다.



### Reducing Robotic Upper-Limb Assessment Time While Maintaining Precision: A Time Series Foundation Model Approach (https://arxiv.org/abs/2511.00193)
- **What's New**: 이번 연구에서는 Kinarm 로봇에서의 Visually Guided Reaching (VGR) 평가를 단축하기 위해 시간 시리즈 기반의 예측 모형을 활용하는 새로운 접근 방식을 제안합니다. 기존에는 40–64회의 반복이 필요했지만, 본 연구를 통해 8회의 기록된 시도와 예측을 결합하는 방식으로 평가 시간을 크게 줄일 수 있음을 보여주었습니다. 이를 통해, 특히 심한 뇌졸중 환자에게 있어서 로봇 평가 세션이 4-5분에서 1분으로 감소하게 됩니다.

- **Technical Details**: 연구는 461명의 뇌졸중 환자와 599명의 대조군 참가자들로부터 수집된 VGR 속도 신호를 분석했습니다. ARIMA, MOMENT, Chronos 모델을 사용하여 70%의 참가자 데이터를 학습하고, 일부 기록되지 않은 시도를 예측하는 알고리즘을 적용했습니다. 예측된 시도와 재조합된 네 가지 운동적 특성을 통합하여 신뢰성 평가 (ICC(2,1))를 실시하였고, 이 과정에서 Chronos 모델이 뛰어난 성능을 보였습니다.

- **Performance Highlights**: Chronos 모델이 8회의 기록된 시도와 예측조합으로 0.90 이상의 ICC 신뢰도를 재구성하여, 24-28회의 완전 기록과 유사한 성과를 달성했습니다. 통계적으로 MOMENT 모델도 중간 정도의 개선을 보여주었으며, ARIMA의 성과는 미비했습니다. 이 연구는 합성된 시도가 기존의 VGR 평가 기능의 신뢰성을 크게 저해하지 않고 시도 수를 대체할 수 있음을 확인하였습니다.
...
- **What's New**: 이번 논문에서는 QLC 3D NAND 플래시 메모리의 신뢰성을 향상시키기 위해 페이지 데이터 어레인지먼트(PDA) 방법을 제안합니다. PDA는 LSTM(장기 단기 메모리) 신경망을 사용하여 인접한 메모리 셀 간의 전하 밀도 차이를 최소화하고, 데이터 배치를 최적화하여 Lateral Charge Migration(LCM)의 영향을 줄입니다. 이를 통해 기존의 WBVM이나 DVDS 방식보다 더 효율적인 데이터 전송을 가능하게 합니다.

- **Technical Details**: 논문에서는 PDA-LSTM 모델이 입력된 페이지 데이터 패턴으로부터 데이터 배치 확률 행렬을 계산한다고 설명합니다. 이 행렬을 통해 LCM으로 인해 발생할 수 있는 글로벌 영향을 최소화합니다. 또한 LSTM 네트워크의 출력 행렬로부터 비 반복적인 서열 생성 확률 행렬로 변환하여 정렬 과정을 지원합니다. LCM의 효과를 평가하기 위해 설계된 점수 매트릭스는 훈련 과정에서만 필요하며 추론 과정에서는 필요하지 않습니다.

- **Performance Highlights**: PDA-LSTM 모델은 기존 데이터 정렬 전략과 비교하여 평균 비트 오류율(BER)을 80.4% 감소시킬 수 있음을 입증했습니다. WBVM 및 DVDS 방식과 비교했을 때 각각 18.4% 및 15.2%의 개선된 성능을 보여줍니다. 추가적으로 PDA-LSTM은 데이터 이동을 기록하기 위한 별도의 플래그 비트가 필요하지 않아 효율성을 높입니다.
...
### On the Fundamental Limitations of Decentralized Learnable Reward Shaping in Cooperative Multi-Agent Reinforcement Learning (https://arxiv.org/abs/2511.00034)
Comments:
          8 pages, 5 figures, 2 tables

- **What's New**: 이번 연구에서는 분산된 강화 학습 환경에서 개별 에이전트가 개인화된 보상을 학습해 최적의 행동을 찾는 DMARL-RSA의 잠재력을 탐구합니다. DMARL-RSA는 협동적 내비게이션 작업에서 성능을 평가하며, 중앙 집중식 방법과 비교해 -24.20 +/- 0.09의 평균 보상을 기록했습니다. 연구 결과, DMARL-RSA는 독립 학습 방식에 비해 미약한 성능을 보였으며, 이는 분산 보상 샤핑이 협동적 조정의 근본적인 한계를 극복하지 못함을 보여줍니다.

- **Technical Details**: 연구에서는 MAPPO, DMARL-RSA, IPPO의 세 가지 접근법을 사용하여 다중 에이전트 조정 문제를 평가했습니다. MAPPO는 중앙 집중식 훈련을 통해 전역 상태 정보를 활용하여 정책 업데이트를 효율적으로 수행하는 반면, DMARL-RSA는 각 에이전트가 독립적으로 보상을 학습하는 완전한 분산 방식입니다. DMARL-RSA의 보상 네트워크는 세 층 구조로 구성되어 있으며, 각 에이전트는 지역 관찰만을 기반으로 개인화된 피드백을 생성합니다.

- **Performance Highlights**: DMARL-RSA는 중앙 집중식 MAPPO에 비해 성능이 크게 떨어지는 -24.20의 평균 보상을 기록했습니다. 반면 IPPO는 -23.19로 유사한 성능을 보여주어, 복잡한 보상 형상이 기본적인 협동 조정의 한계를 극복하지 못하는 것을 시사합니다. 흥미롭게도, DMARL-RSA는 높은 랜드마크 커버리지(0.888 +/- 0.029)를 기록했으나, 중앙 집중식 MAPPO의 전반적인 성능(0.273 +/- 0.008)에 비해 부족한 결과를 나타냈습니다.
...
### On the Structure of Floating-Point Noise in Batch-Invariant GPU Matrix Multiplication (https://arxiv.org/abs/2511.00025)
- **What's New**: 이번 논문에서는 GPU에서 수행한 행렬 곱셈(matmul) 연산에 대한 부동 소수점 비결정성(floating-point non-determinism)과 관련하여 기존의 가정인 독립적이고 동일하게 분포된(i.i.d.) 가우시안 소음 모델을 실증적으로 검토하였습니다. 연구 결과, 이 모델이 실제 GPU의 동작을 설명하지 못한다는 것을 보여주었고, 부동 소수점 오류가 구조적이고 상관관계가 있음을 발견하였습니다. 이러한 발견은 깊은 학습의 신뢰성을 분석하는 데 중요한 원칙을 제공합니다.

- **Technical Details**: 부동 소수점 연산은 결합 법칙을 따르지 않기 때문에(GPUs의 동작에서), 동일한 계산이라도 실행 순서와 커널 구현에 따라 결과가 미세하게 달라질 수 있습니다. 연구진은 단일 입력 벡터와 배치에 포함된 동일한 입력을 사용하는 경우의 행렬 곱셈 결과를 비교하여 이러한 현상을 명시적으로 테스트하였습니다. 부동 소수점 오차가 오프 대각선(off-diagonal) 용어에서 약 50%의 전체 오류 분산을 차지하고 있다는 사실을 발견하였습니다.

- **Performance Highlights**: 이 연구의 주요 결과는 10,000회의 실험에서 예측된 후회(flip) 비율이 0.00%라는 점으로, 이는 가정된 모델이 실제 불안정성을 크게 과대 평가하고 있음을 보여줍니다. 따라서 비결정적 조건에서도 깊은 학습 모델이 예측적으로 안정성을 유지할 수 있다는 것을 확인했습니다. 이번 연구는 대규모 모델에서의 결정적 추론의 이론적 한계와 신뢰성 문제를 재조명합니다.
...
- **What's New**: 이번 연구는 심리생리학적 테스트를 사용하여 인지 연령(cognitive age)을 예측하는 새로운 방법을 소개합니다. 연구자들은 반응 시간(reaction time), 단기 기억(short-term memory), 언어 기능(verbal functions)과 색 및 공간 지각(color and spatial perception)을 포함한 다양한 인지 기능을 측정하기 위한 심리 테스트를 실시했습니다. 이러한 테스트 결과를 바탕으로 머신러닝 알고리즘을 학습시켜 인지 연령을 예측하는 회귀 작업(regression task)을 수행했습니다. 이러한 발견은 모바일 장치를 통한 원격 스크리닝(remote screening)에서 인지 노화(cognitive aging)를 진단하고 모니터링하는 데 기여합니다.

- **Technical Details**: 데이터 수집 과정에서 참가자들은 온라인 데이터 수집 시스템을 통해 설문지와 심리생리학적 테스트를 진행했습니다. 테스트는 정해진 순서로 진행되며 개인 데이터는 수집되지 않아 익명으로 유지됩니다. 주어진 시간 내에 모든 테스트를 완료하도록 설계된 이 시스템은 각 테스트에 대한 반응 시간과 답변을 데이터베이스에 저장합니다. 연구의 예측 모델은 Random Forest, Gradient Boosting, Support Vector Regression(SVR) 등 다양한 회귀 알고리즘을 사용하여 최고의 성과를 나타내는 알고리즘을 선택했습니다.

- **Performance Highlights**: 연구 데이터는 44개의 변수를 포함하고 있으며, 성별에 약간의 불균형이 존재하였고, 평균 연령은 34세로 나타났습니다. 변수의 분포는 Z-score 정규화를 실시하여 아웃라이어(outlier)를 처리했으며, 윈소리제이션(winsorization) 절차를 통해 분석 결과에 미치는 극단 값의 영향을 줄였습니다. 이 모든 단계는 데이터의 신뢰성을 높이기 위해 수행되었으며, 전체적으로 안정적인 예측 기반을 제공하는 모델을 구축하는 데 기여했습니다.



### Matrix Phylogeny: Compact Spectral Fingerprints for Trap-Robust Preconditioner Selection (https://arxiv.org/abs/2511.00012)
Comments:
          16 Pages

- **What's New**: 이 연구에서 제안하는 Compact Spectral Fingerprints (CSF) 및 Adaptive Spectral Fingerprints (ASF)는 행렬의 가족 수준에서의 특성을 파악하는 저차원, eigendecomposition-free (고유분해 없는) 설명자입니다. 이 설명자는 Hutchinson 스케치를 통해 추정된 Chebyshev trace moments를 사용하여 구성됩니다. 이들은 변환 및 유사성에 불변하며, 전역 스케일링에 강인성을 발휘합니다. 여러 테스트에서 보인 바에 따르면, 매우 적은 수의 모멘트만으로도 효과적인 군집화 (clustering)가 가능합니다.

- **Technical Details**: 각 행렬 A는 일반적으로 대칭 행렬로 가정하며, CSF/ASF를 통해 비슷한 가족으로 그룹화할 수 있습니다. 이 방법은 spectral radius (스펙트럼 반경)와 트레이스(trace) 정보를 사용하여 행렬을 스케일링하고, Chebyshev 다항식을 활용하여 고유 벡터나 스펙트럼 전체를 계산하지 않고도 설명자를 만듭니다. CSF는 고정된 차원 K = {3, 5}를 사용하며, ASF는 데이터에 기반 한 중지 규칙을 통해 동적으로 차원 K를 선택합니다.

- **Performance Highlights**: 실험 결과에 따르면, CSF-K=3 및 K=5에서 ARI (Adjusted Rand Index)는 1.0을 기록하며, 매우 높은 군집화 품질을 나타내었습니다. 다른 강력한 대안들과 비교했을 때, CSF-K=5는 더 적은 수의 특징(최대 K=10 versus 64/9153)을 활용하면서도 정확도를 맞추거나 초과하는 성능을 보였습니다. 실용적인 Trap->Recommendation 파이프라인을 통해, 이 설명자는 자동 선형 변환 (preconditioner selection)에서 높은 정확성을 제공합니다.



### Group-Equivariant Diffusion Models for Lattice Field Theory (https://arxiv.org/abs/2510.26081)
Comments:
          45 pages, 12 figures

- **What's New**: 이번 연구에서는 최근 확산 모델(diffusion models)을 채택하여 격자 양자 장 이론(lattice quantum field theories, LQFT)의 확률 분포를 샘플링하는 새로운 방법론을 제시하고 있습니다. 이 방법은 전통적인 MCMC(Markov Chain Monte Carlo) 기법에 비해 더 높은 샘플 품질을 제공합니다. 또한, 기존의 일반적인 점수 네트워크(score networks)보다 더 우수한 성능을 나타내는 대칭 인식 모델을 개발했습니다.

- **Technical Details**: 연구에서 제안된 확산 모델은 다양한 그룹 변환에 대해 등가인 점수 네트워크(score networks)를 훈련시키는 것을 포함합니다. 특히, 이들 네트워크는 글로벌 Z_2 반사, 로컬 U(1) 회전 및 주기적 변환(tanslation)에 대해 대칭을 유지합니다. 점수 정맥 안정성(score matching)의 새로운 목표 함수를 통해 학습된 점수 네트워크의 품질을 향상시키는 방법도 함께 소개됩니다.

- **Performance Highlights**: 실험을 통해 제안된 대칭 인식 모델들이 더 일반적인 점수 네트워크보다 샘플 품질, 표현력(expressivity), 효과적인 샘플 크기(effective sample size)에서 뛰어남을 입증했습니다. 즉, 이 접근 방식은 격자 양자 장 이론에서의 샘플 생성 및 'critical slowing down' 문제를 극복하는 데 있어 유용한 기여를 할 수 있습니다.

...
- **What's New**: 이 논문에서는 AI 기반 수학 보조 도구의 훈련과 평가에 사용되는 데이터셋들이 여러 가지 결함을 가지고 있음을 강조합니다. 과거의 데이터셋들은 주로 초급 대학 수준의 수학에 국한되어 있으며, 이론 증명 기반의 포괄적인 평가를 어렵게 만드는 여러 문제들을 포함하고 있습니다. 따라서 미래의 AI 수학 보조 도구들의 능력을 향상시키기 위해서는 데이터셋 설계와 평가 기준에서 패러다임 전환이 필요하다고 주장하고 있습니다.

- **Technical Details**: 논문에서는 LLMs (Large Language Models)를 포함한 데이터셋의 설계 개선을 위한 방법으로, G. Pólya가 1949년에 제안한 "motivated proof" 개념을 소개합니다. 이는 더 나은 증명 학습 신호를 제공할 수 있는 데이터셋의 청사진 역할을 하며, 수학적 연구 실행의 복잡성을 LLMs가 훈련할 수 있는 형태로 전환하는 것을 목표로 합니다. 또한 새로운 수학적 프로세스를 반영하는 워크플로우 및 과제를 데이터셋에 포함하는 것을 제안하고 있습니다.

- **Performance Highlights**: 현대 AI 수학 도구들은 다양한 연구에서 괄목할만한 성과를 보이고 있으며, 특히 LLMs가 수학적 작업에서 지원해줄 가능성을 보여주고 있습니다. AlphaGeometry와 Numina와 같은 시스템은 국제 수학 올림피아드 수준의 문제를 해결할 수 있는 모델을 제시하고 있으며, GPT-4는 경우에 따라 대학생 수준의 성능을 나타냈습니다. 그러나 이러한 혁신에도 불구하고, 기존 데이터셋은 전문화된 모델만 지원하는 경향이 있으며, LLMs의 활용성을 저해하는 여러 문제들이 여전히 존재합니다.



### Rethinking Uncertainty Estimation in Natural Language Generation (https://arxiv.org/abs/2412.15176)
- **What's New**: 본 논문에서는 언어 모델의 생성 텍스트 신뢰성을 평가하기 위한 새로운 불확실성 추정 방법인 G-NLL을 제안합니다. G-NLL은 'greedy decoding'을 통해 생성된 단일 출력 시퀀스에서 신뢰성 있는 추정값을 도출할 수 있는 장점을 가지고 있습니다. 기존의 불확실성 추정 방법들이 여러 출력 시퀀스를 생성하고 분석해야 했던 것과는 대조적으로, G-NLL은 계산 비용을 크게 줄이고 이론적 엄밀함을 유지합니다.

- **Technical Details**: G-NLL은 가장 가능성이 높은 출력 시퀀스의 음의 로그-우도(negative log-likelihood)를 사용하여 불확실성을 측정합니다. 이 측정 방식은 기존에 비해 많은 출력 시퀀스를 생성할 필요가 없어 계산 효율성을 증대시킵니다. 본 연구에서는 G-NLL을 이용하여 자연어 생성(NLG) 분야에서 불확실성 추정의 이론적 기초를 확립하고, 특히 올바른 점수 규칙(proper scoring rules) 프레임워크를 기반으로 이론적인 동기를 제공합니다.

- **Performance Highlights**: G-NLL은 다양한 언어 모델과 작업에서 최첨단 성능을 달성하며, 기존의 불확실성 측정 방법들과 비교했을 때 효과적으로 성능을 초과할 수 있음을 실험적으로 입증합니다. 이 방법은 이론적 rigor를 유지하면서도 자연어 생성 분야에 있어 불확실성 추정의 효율적이고 실용적인 솔루션을 제공합니다. G-NLL은 향후 연구에 대한 강력한 기준점 역할을 하며, 실제 애플리케이션에서 널리 채택될 가능성을 제공합니다.
...
### DroughtSet: Understanding Drought Through Spatial-Temporal Learning (https://arxiv.org/abs/2412.15075)
Comments:
          Accepted by AAAI25

- **What's New**: 이번 연구에서는 새로운 데이터셋인 DroughtSet를 소개합니다. 이 데이터셋은 미국 본토(CONUS) 지역의 여러 원격 감지(Remote Sensing) 및 재분석(Reanalysis) 데이터셋에서 수집된 기후, 물리적, 생태 조건 및 세 가지 가뭄 지수를 포함하고 있습니다. DroughtSet는 가뭄 예측 모델의 강력한 벤치마크로 활용될 수 있어, 향후 연구와 딥러닝 기반 기후 과학 연구에 기여할 것으로 기대됩니다.

- **Technical Details**: DroughtSet는 2003년부터 2013년까지의 주 단위 기후 관련 데이터를 포함하며, 지리적 해상도를 4 km로 통일하여 모든 변수를 재샘플링했습니다. 데이터는 585×1386 픽셀의 그리드로 표현되며, 벡터 데이터 형태로 제공됩니다. 이 데이터셋은 정적(static) 및 동적(dynamic) 변수뿐만 아니라, 세 가지 가뭄 유형(토양 수분 가뭄, 생태 수문학적 가뭄, 생태학적 가뭄)에 대해 정규화된 예측 변수들을 포함합니다.

- **Performance Highlights**: SPDrought라는 다중 작업 공간-시간 모델을 통해 가뭄 예측의 정확성을 향상시키며, сосед(Neighboring) 지역의 영향을 고려하여 예측합니다. 통합 기울기(Integrated Gradient) 방법을 통해 딥러닝 모델의 해석 가능성과 예측 변수의 중요성을 평가합니다. 이러한 결과는 가뭄의 기계론적 이해 및 기존 지구 시스템 모델에서의 시뮬레이션을 향상시키는데 기여할 수 있습니다.
...
### Diffusion priors for Bayesian 3D reconstruction from incomplete measurements (https://arxiv.org/abs/2412.14897)
- **What's New**: 이번 연구에서는 Bayesian 프레임워크 내에서 실험적 데이터를 결합하여 확산 모델(diffusion model)을 프라이어(prior)로 활용하는 방법을 탐구합니다. 특히, cryo-electron microscopy (cryo-EM) 이미지를 통해 biomolecular assemblies의 3D 구조를 재구성하는 데에 초점을 맞추고 있습니다. 새로운 방법론은 일반적인 프라이어가 아닌, PDB에서 얻은 세부 정보를 포함한 확산 모델을 사용하여 더 높은 fidelity를 제공합니다.

- **Technical Details**: 연구에서는 3D 포인트 클라우드(point cloud)를 활용하여 가정용 물품이나 단백질 및 핵산으로 구성된 생물 분자 복합체와 같은 3D 물체를 표현합니다. 저속 해상도가 있는 불완전하고 노이즈가 포함된 실험 데이터를 통합하여 확산 기반 후처리 샘플링(difusion-based posterior sampling, DPS)을 통해 3D 구조를 재구성합니다. 이 과정은 최적화된 확산 스케줄(diffusion schedule)과 노이즈 주입(noise injection)을 포함하여 샘플 품질과 실행 시간을 개선합니다.

- **Performance Highlights**: 이 방법은 ShapeNet 및 PDB의 복잡하고 다양한 3D 포인트 클라우드 데이터세트에서 높은 신뢰성과 유연성을 입증합니다. 이 연구는 2D 투영에서 3D 구조를 재구성하는 문제를 해결하며, 표면 정보뿐만 아니라 전체 볼륨에서 밀도에 대한 정보를 제공하기 때문에 기존의 접근법과 차별화됩니다. 결과적으로 연구진은 매우 희소한 관측에서 3D 구조를 성공적으로 재구성하는 데에 성공하였습니다.



### Hierarchical Subspaces of Policies for Continual Offline Reinforcement Learning (https://arxiv.org/abs/2412.14865)
- **What's New**: 이번 연구에서는 HILOW라는 새로운 프레임워크를 소개합니다. 이는 Continual Reinforcement Learning의 과제를 해결하기 위해 설계되었으며, 특히 오프라인 탐색 환경에서의 지속적인 학습을 지원합니다. HILOW는 계층적 정책 서브공간을 이용하여 새로운 작업에 유연하고 효율적으로 적응할 수 있게 해줍니다.

- **Technical Details**: HILOW는 여러 작업을 처리하는 데 있어 지식을 보존하면서도 새로운 작업에 적응할 수 있도록 설계된 구조를 가지고 있습니다. 이 구조는 계층적(policy) 된 접근 방식을 사용하여 기존의 기술을 최대한 반영합니다. 연구진은 MuJoCo 미로 환경과 비디오 게임 시뮬레이션에서 HILOW의 효과성을 입증했습니다.

- **Performance Highlights**: 실험 결과, HILOW는 고전적인 지속 학습(metric) 지표에 따라 뛰어난 성능과 적응성을 보여주었습니다. 특히 기억 사용(memory usage) 측면에서 경쟁력 있는 성능을 기록하였습니다. 이는 실제로 수집된 데이터로부터 지속적으로 학습할 수 있는 응용 분야에 매우 유망한 프레임워크를 제공합니다.



### Entropy Regularized Task Representation Learning for Offline Meta-Reinforcement Learning (https://arxiv.org/abs/2412.14834)
Comments:
          7 Pages, Accepted at AAAI 2025

- **What's New**: 이번 논문에서는 Offline Meta-Reinforcement Learning (OMRL)에서 에이전트가 새로운 작업에 적응할 수 있는 능력을 개선하기 위한 새로운 방법인 Entropy Regularized Task Representation Learning (er-trl)을 제안합니다. 이 접근법은 기존의 context-based 방법들이 가진 분포 불일치 (distribution mismatch) 문제를 해결하려고 합니다. OMRL의 핵심 아이디어는 에이전트가 오프라인 데이터에서 작업을 학습할 때 행동 정책에 의존하지 않도록 하는 것입니다.

- **Technical Details**: OMRL은 다양한 작업의 오프라인 데이터를 활용하여 에이전트가 새로운 작업에서 잘 일반화할 수 있도록 합니다. 이 논문에서는 context encoding 방법을 사용하여 과거의 상태-행동-보상 전이로부터 작업에 대한 표현을 학습합니다. 하지만 기존 방법은 훈련 시 사용한 context와 테스트 시의 context 간의 분포 차이로 인해 성능이 저하되는 한계가 있습니다.

- **Performance Highlights**: 제안된 er-trl 메소드는 MuJoCo 환경에서 기존 기법들과 비교하여 작업 표현이 보다 정확하게 작업을 나타내는데 성공하였으며, 이로 인해 분포 내 (in-distribution) 및 분포 외 (out-of-distribution) 작업에서 높은 성능을 발휘했습니다. 에이전트의 작업 표현이 행동 정책과 독립적이도록 만드는 것이 mutual information을 최소화하여 가능하게 하였으며, 이러한 방식으로 새로운 작업에 대한 적응력이 크게 향상되었습니다.
...
### Extending TWIG: Zero-Shot Predictive Hyperparameter Selection for KGEs based on Graph Structur (https://arxiv.org/abs/2412.14801)
- **What's New**: 최근 Knowledge Graphs (KGs)에 대한 연구에서 Topologically-Weighted Intelligence Generation (TWIG) 모델이 제안되었습니다. 이 모델은 KGE (Knowledge Graph Embedding) 성능을 예측하는 새로운 체계를 제공하여, 각 요소가 어떻게 관계되는지를 모델링하는 데 도움을 줍니다. 특히, TWIG는 이전 연구를 확장하여 KGE 모델 ComplEx의 출력을 여러 KG에서 동시에 시뮬레이션할 수 있는 능력을 평가합니다.

- **Technical Details**: KG는 정보의 노드와 엣지로 구성된 그래프 기반 데이터베이스입니다. KGE 모델은 KG의 정보를 바탕으로 새로운 사실을 예측하는데 사용되며, 이는 'link prediction' 작업으로 불립니다. TWIG 모델은 KG의 구조와 KGE 성능을 공통적인 분석 프레임워크로 이해하는 데 기여하며, 수치 분석을 통해 KGE의 전반적인 성능을 예측하는 데 성공적인 결과를 보여주었습니다.

- **Performance Highlights**: TWIG는 다양한 하이퍼파라미터 설정과 범위의 KGs에 대한 KGE 성능을 요약할 수 있는 능력을 가집니다. 또한, TWIG는 이전에 보지 못한 KG에서 하이퍼파라미터 성능을 성공적으로 예측할 수 있음을 입증하였습니다. 이를 통해 앞으로 TWIG와 유사한 방법으로 KGE 모델에 대한 최적의 하이퍼파라미터 선택을 선행적으로 결정할 수 있는 가능성을 제안합니다.



### A parametric algorithm is optimal for non-parametric regression of smooth functions (https://arxiv.org/abs/2412.14744)
- **What's New**: 이번 논문에서는 일반 함수 $f:[-1,1]^d\to \mathbb R$에 대한 회귀 문제를 다루고, 학습자가 전체 도메인에서 균일한 오류 경계(uniform error bound)를 달성하기 위해 훈련 점을 선택하는 방법을 제시합니다. 특히, 새로운 알고리즘인 PADUA를 소개하며, 이는 기존 비모수(nonparametric) 회귀 기법과는 다르게 샘플 복잡도(sample complexity)에서 최적의 성능을 보입니다. 또한 PADUA는 예측 단계에서 공간 복잡도(space complexity)에서도 최적의 특성을 유지합니다.

- **Technical Details**: PADUA는 함수의 부드러움(degree of smoothness)에 전적으로 의존하는 샘플 복잡도 경계(sample complexity bound)를 제시합니다. 주기적 경계(periodicity at the domain boundaries)를 가정하였으며, 높은 확률로 모든 문제 파라미터에 걸쳐 성능 보장을 제공합니다. 이는 기존의 비모수 기법과 차별화된 점으로, 주로 함수가 가진 특성을 활용하여 효율적인 학습을 가능하게 합니다.

- **Performance Highlights**: PADUA는 실제 오디오 데이터에서 유래된 함수에 대한 수치 실험을 통해 검증되었습니다. 이 알고리즘은 최신 기법들과 비교했을 때 유사한 성능을 보이며, 계산 시간(computational time)은 훨씬 적은 양만 요구합니다. 이를 통해 PADUA가 기존 방법에 비해 효율성이 뛰어난 것으로 입증되었습니다.
...
- **What's New**: 이 논문은 Graph Neural Networks (GNNs)가 입력의 노이즈에 취약하다는 문제를 다루고 있습니다. 저자는 노이즈에 가장 민감한 노드를 식별하기 위해 적대적 강건성 평가 기법을 사용한 새로운 접근 방식을 제안합니다. 이를 통해 특수하게 선택된 노드를 포함한 훈련 세트를 구성하고, 이를 사용하여 Graph Convolutional Network (GCN)을 훈련시키는 방법을 소개합니다.

- **Technical Details**: 논문은 GNN의 작동 원리를 설명하며, 그래프 구조와 노드 특징을 이용하여 정보를 집계하는 방식에 대해 상세히 설명합니다. 특히, 적대적 공격은 모델의 예측을 조작하기 위해 의도적으로 구성된 변형으로, 이러한 공격에 대한 방어 기법이 소개됩니다. 저자는 새로운 특징 표현을 학습하기 위해 KNN 그래프를 생성하고, 노드의 강건성을 평가하기 위해 고유치(eigenvalue) 계산을 이용합니다.

- **Performance Highlights**: 실험 결과는 제안된 방법이 GCN의 성능을 획기적으로 향상시킨다는 것을 보여줍니다. 노이즈에 취약한 샘플을 중심으로 훈련한 결과, GNN의 분류 성능이 크게 개선되었습니다. 이러한 결과는 GNN의 강건성을 높이기 위해 비강건 샘플에 집중하는 것이 중요하다는 것을 강조합니다.



### Generative AI for Banks: Benchmarks and Algorithms for Synthetic Financial Transaction Data (https://arxiv.org/abs/2412.14730)
Comments:
          Presented at the 34th Workshop on Information Technologies and Systems (WITS 2024)

- **What's New**: 이번 연구에서는 은행 부문에서의 데이터 민감성과 규제 제약으로 인한 딥 러닝의 도전 과제에 대해 언급하고, 생성적 AI가 해결책이 될 수 있음을 제시합니다. 이는 합성 금융 거래 데이터를 생성하기 위한 효과적인 알고리즘을 식별하고 평가하는 과정을 포함합니다.

- **Technical Details**: 연구에서는 Conditional Tabular Generative Adversarial Networks (CTGAN), DoppelGANger (DGAN), Wasserstein GAN, Financial Diffusion (FinDiff), Tabular Variational AutoEncoders (TVAE) 등 다섯 가지 주요 모델을 다루었습니다. 이들 알고리즘은 충실도(fidelity), 합성 품질(synthesis quality), 효율성(efficiency), 프라이버시(privacy), 그래프 구조(graph structure)의 다섯 가지 기준을 바탕으로 평가되었습니다.

- **Performance Highlights**: 모든 알고리즘이 실제 데이터의 그래프 구조를 완벽히 복제하지는 못하지만, 각 모델은 특정 영역에서 우수한 성능을 보였습니다. 특히 DGAN은 프라이버시가 중요한 작업에 적합하며, FinDiff와 TVAE는 데이터 복제 및 증강에서 뛰어난 성능을 발휘합니다. CTGAN은 모든 기준에서 균형 잡힌 결과를 보여주어, 일반적인 애플리케이션에서 중간 정도의 프라이버시 우려에 적합하게 사용될 수 있는 것으로 평가되었습니다.



### FROC: Building Fair ROC from a Trained Classifier (https://arxiv.org/abs/2412.14724)
Comments:
          51 pages, The 39th Annual AAAI Conference on Artificial Intelligence

- **What's New**: 이 논문은 binary protected groups(이진 보호 그룹)에서 공정한 확률적 이진 분류의 문제를 다룹니다. 제안된 접근법은 불공정한 분류기의 출력을 조정하여 ε1-Equalized ROC(ε1-균일화된 ROC) 조건을 만족하는 공정한 확률적 분류기를 만드는 것입니다. 이를 위해 새로운 post-processing 알고리즘인 FROC을 소개하며, 이는 기존의 모델 재학습 없이도 실행 가능합니다.

- **Technical Details**: 저자들은 ROC(Receiver Operating Characteristic) 곡선의 쿼리 모델을 도입하여 각 보호 그룹에 대한 곡선을 근사화합니다. 제안된 접근법에서는 주어진 ROC 곡선 내에서 ε 거리를 유지하며, AUC(Area Under the Curve)를 최소한의 손실로 변환하는 방법을 제시합니다. 이 과정에서 FROC 알고리즘을 통해 확률적 분류기를 설계하고 이론적으로 최적 보장을 증명합니다.

- **Performance Highlights**: FROC 알고리즘은 다수의 실제 데이터셋에서 평가되었으며, 성능이 기존의 불공정한 분류기보다 우수함을 입증하였습니다. 논문에서는 AUC 손실을 최소화하면서 ε1-Equalized ROC를 달성하는 방법을 제시하고, 이를 통해 언급된 다양한 공정성 기준을 만족 set할 수 있습니다.



### A Comprehensive Forecasting Framework based on Multi-Stage Hierarchical Forecasting Reconciliation and Adjustmen (https://arxiv.org/abs/2412.14718)
Comments:
          Published in 2024 IEEE International Conference on Big Data (BigData)

- **What's New**: 이번 연구에서는 Walmart의 광고 제품에 대한 수요 예측을 위한 포괄적인 시스템인 "Multi-Stage Hierarchical Forecasting Reconciliation and Adjustment (Multi-Stage HiFoReAd)" 프레임워크를 제안합니다. 기존의 계층적 예측 방법들이 직면한 계층별 인과관계와 계절성의 손실 문제를 해결하기 위해, 이 프레임워크는 여러 모델의 성과를 조합하고 최적화하는 접근 방식을 취합니다.

- **Technical Details**: 프레임워크는 여러 단계를 통해 예측 성능을 향상시킵니다. 첫 번째 단계는 Top-Down 예측과 'harmonic alignment'를 활용하여 계층을 개선하고, 두 번째 단계에서는 MinTrace 알고리즘으로 상위 수준 예측을 정렬합니다. 마지막 단계에서는 'harmonic alignment'와 'stratified scaling'을 통해 모든 계층에서 정확하고 일관된 예측을 달성합니다.

- **Performance Highlights**: Walmart 내부의 광고 수요 데이터셋과 3개의 공개 데이터셋에서 실험을 수행한 결과, Cross-validation 세트에서의 평균 절대 백분율 오류가 3%에서 40%로 개선되었습니다. 또한, 제안한 방법은 모든 계층에서 일관된 예측을 제공하며, Walmart의 광고, 판매 그리고 운영팀에 의해 실제로 활용되고 있습니다.



### Computing Gram Matrix for SMILES Strings using RDKFingerprint and Sinkhorn-Knopp Algorithm (https://arxiv.org/abs/2412.14717)
- **What's New**: 이 논문은 SMILES (Simplified Molecular Input Line Entry System) 문자열을 사용하여 분자 구조를 인코딩 및 분석하기 위한 새로운 커널 기반 접근 방식을 제안합니다. 특히, Sinkhorn-Knopp 알고리즘을 이용하여 커널 매트릭스를 계산하고, 커널 주성분 분석 (PCA)을 통해 차원 축소를 수행합니다. 이 방법론은 분자 구조의 저차원 임베딩을 통해 분류 및 회귀 분석을 가능하게 합니다.

- **Technical Details**: 제안된 방법은 RDKit 라이브러리를 활용하여 SMILES 문자열을 분자 그래프로 변환하고, RDKFingerprint 함수를 통해 각 분자의 지문을 생성합니다. 그 후, 쌍별 커널 함수(pairwise kernels function)를 사용해 거리 매트릭스를 계산하고, 최적 운송 매트릭스를 이용해 최종 커널 매트릭스를 생성합니다. 이 커널 PCA를 통해 분자 구조의 내재적 속성을 포착하며, 이는 분류 및 회귀 분석에 적합한 저차원 임베딩을 만들어냅니다.

- **Performance Highlights**: 제안된 방법은 약물 하위 분류 예측(분류 작업)과 용해도 AlogPS 예측(회귀 작업)에서 기존의 여러 기반 방법들에 비해 우수한 성능을 보였습니다. 이 연구는 약물 발견 및 분자 설계 분야에 커다란 영향을 미칠 수 있는 가능성을 제시하며, 더 나아가 분자 성질 예측에 있어 가장 효과적인 방법을 찾는 데 기여할 수 있습니다.
...
- **What's New**: 이 논문에서는 HARP라는 새로운 방법을 소개합니다. HARP는 기존 연구들보다도 대담한 파라미터 pruning에 훨씬 더 효과적으로 고안되었습니다. 특히, 이 방법은 각 layer에 따라 최적화된 파라미터 pruning을 학습합니다.

- **Technical Details**: HARP는 네트워크 전체를 고려하여 압축 전략(compression strategy)을 학습합니다. 이는 파라미터 수와 어떤 파라미터를 pruning할지를 결정하는 global 적인 접근방식입니다. 모델을 동적으로 정규화(dynamic regularization)하여 fine-tuning하며, 초기에는 robust성을 우선으로 하고 이후에 압축률(target compression rate)을 목표로 전환합니다.

- **Performance Highlights**: 이 방법을 사용하면 네트워크 원래 크기에서 99%를 축소해도, 사전 훈련된 모델의 자연적인 정확성과 adversarial robustness를 유지할 수 있습니다. 또한, layer간 비균일한 압축(non-uniform compression)이 성능에 미치는 중요한 영향을 관찰하였습니다.



### ReMoE: Fully Differentiable Mixture-of-Experts with ReLU Routing (https://arxiv.org/abs/2412.14711)
- **What's New**: 이 논문에서는 Sparsely activated Mixture-of-Experts (MoE) 모델의 한계를 극복하기 위해 ReMoE라는 새로운 아키텍처를 제안합니다. ReMoE는 기존 TopK 라우팅 대신 ReLU를 사용하여 완전하게 미분 가능한 구조로 설계되었습니다. 이로 인해 MoE 모델이 더 효율적으로 운영될 수 있으며, 성능과 확장성이 향상됩니다.

- **Technical Details**: ReMoE는 기존의 TopK 라우팅 방식에서 발생하는 비연속성을 해결하기 위해 ReLU 라우팅을 사용합니다. ReLU 함수는 전문가의 활성 상태를 매끄럽게 제어할 수 있으며, 이는 전문가의 활성 수를 동적으로 조정할 수 있게 합니다. 또한 로드 밸런싱을 위한 정규화를 추가하여 전문가 간의 균형을 유지하면서도 sparsity를 유지하도록 합니다.

- **Performance Highlights**: 실험 결과, ReMoE는 다양한 모델 크기와 전문가 수에서 기존의 TopK 라우팅 MoE보다 일관되게 뛰어난 성능을 보였습니다. ReMoE는 전문가 수가 증가할수록 성능 향상이 더욱 뚜렷하게 나타나며, 이는 기존 MoE 아키텍처를 초월하는 우수한 확장성을 보여줍니다.
...
- **What's New**: 이번 논문에서는 새로운 LResNet 방법론을 제안하여 하이퍼볼릭( hyperbolic) 잔여 신경망의 한계를 극복하려고 합니다. LResNet은 로렌츠( Lorentz) 모델을 기반으로 하여 가중된 로렌츠 중심점을 이용하여 효율적인 잔여 연결(residual connections) 통합을 지원합니다. 기존의 방법들에 비해 구조적 안정성과 효율성을 높이는 데 기여합니다.

- **Technical Details**: LResNet은 하이퍼볼릭 기하학에서 유래된 로렌츠 모델을 활용하여, 기존의 하이퍼볼릭 잔여 네트워크의 복잡성과 수치적 불안정성을 줄이고 있습니다. 이 방법론은 잔여 연결을 통해 네트워크의 계층적 표현 능력을 유지하면서도, 다양한 아키텍처, 예를 들어 CNNs(Convolutional Neural Networks), GNNs(Graph Neural Networks) 및 그래프 트랜스포머(graph Transformers) 등에서 일반적으로 적용 가능한 프레임워크로 설계되었습니다.

- **Performance Highlights**: 실험 결과, LResNet은 그래프 및 비전 작업에서 기존의 유클리드(Euclidean) 및 하이퍼볼릭 대안들에 비해 뛰어난 성능과 강건성을 보여주었습니다. 이 연구는 하이퍼볼릭 임베딩 공간에서 더 표현력이 뛰어난 신경망 구축의 가능성을 제시하며, 데이터 분석 및 이미지 이해와 같은 여러 응용 분야에 기여할 것으로 기대됩니다.
...
### Trainable Adaptive Activation Function Structure (TAAFS) Enhances Neural Network Force Field Performance with Only Dozens of Additional Parameters (https://arxiv.org/abs/2412.14655)
- **What's New**: 본 논문은 Trainable Adaptive Activation Function Structure (TAAFS)를 도입하여 비선형 활성화에 대한 다양한 수학적 공식을 선택함으로써 신경망 강체(NNFFs)의 정확성을 향상시키는 방법을 제시합니다. 이는 네트워크의 매개변수를 크게 증가시키지 않고도 가능하며, 각종 신경망 모델에 TAAFS를 통합하여 정확도 개선을 관찰했습니다. 결과적으로, 주요 모델에서 약 10%의 정확도 향상을 이끌어냈습니다.

- **Technical Details**: TAAFS는 KAN의 개념에서 영감을 받아, 고정된 변환과 조합에 의존하지 않고 비선형 계산을 위한 수학적 공식을 자유롭게 선택할 수 있도록 해줍니다. 이 구조는 신경망에서의 표현력을 높이며, 다양한 네트워크 모델에 적용할 수 있는 유연성을 제공합니다. 특히 TAAFS는 수치 데이터와 선택된 수학적 수식에 따라 활성화 함수의 형태를 동적으로 조정하는 특징이 있습니다.

- **Performance Highlights**: 본 연구에서 적용된 TAAFS는 DP, ANI2, chgnet과 같은 모델에서 10% 이상의 정확도 개선을 이루었으며, 매개변수 수의 증가는 미미하여 전체 매개변수의 0.2% 미만에 불과했습니다. 이는 실험 결과에서 구체적으로 나타나며, 신경망 강체의 성능 향상에 실질적인 기여를 하였습니다. 실험 결과는 5장에서 자세히 제공됩니다.
...
### Continuous latent representations for modeling precipitation with deep learning (https://arxiv.org/abs/2412.14620)
- **What's New**: 이번 연구는 강수 데이터의 시뮬레이션과 통계적 처리에서 발생하는 여러 가지 어려움을 해결하기 위해, 심층 학습(Deep Learning)을 사용하여 매끄럽고 시공간적으로 연속적인 변수를 생성하는 방법을 탐구합니다. 특히, 정규 분포를 따르는 '가상 강수(pseudo-precipitation, PP)'라는 대안을 개발하고, 이를 통해 1도(약 100 km)에서 0.25도(약 25 km)로의 다운스케일링을 수행해 그 적용 가능성을 평가합니다.

- **Technical Details**: PP는 총 강수(total precipitation)와 수직적 통합 수분 발산(Vertically Integrated Moisture Divergence, VIMD)의 결합을 통해 생성된 매끄러운 가우시안 필드입니다. 이는 강수의 비정상적 및 비선형적 특성을 해결하고, 만족스러운 통계적 처리를 위해 가우시안 분포로 변환됩니다. 우리의 모델은 모든 레이어에서 완전히 연결된 인코더-디코더 구조를 활용하여 PP를 생성하며, 양자 손실(quantile loss)을 적용하여 PP의 분포를 표준 정규 분포와 정렬합니다.

- **Performance Highlights**: 유럽에 대한 다운스케일링 모델의 성능 평가 결과, 모형이 미세 구조를 잘 포착하고 큰 구조를 보존하는 것을 보여주었습니다. 결과는 ERA5 데이터 세트와 강한 일치를 보였으며, 특히 극단적인 강수 사건의 예측에서 높은 정확성을 달성했습니다. 이러한 접근법은 다운스케일링, 디바이아이징(debiasing) 및 예측(forecasting) 작업에서도 적용할 수 있는 유용한 방법론이 될 것입니다.
...
- **What's New**: 이 논문에서는 LLM(Large Language Model)의 양자화(Quantization) 원칙에 대한 종합적인 분석을 제시하고, MixLLM을 통해 새로운 혼합 정밀도 양자화 최적화 공간을 탐색합니다. MixLLM은 모델의 서로 다른 출력 특징이 가지는 중요성에 따라 비트 폭을 다르게 설정하여 효율적인 메모리 소비와 높은 정확도를 달성합니다. 이로 인해 모델의 출력 가치에 따른 전역적인 중요성 식별을 통해 보다 효과적인 양자화를 제공합니다.

- **Technical Details**: MixLLM에서는 출력 특징의 전역적인 중요성을 고려하여 고유의 비트 폭을 적용하여 가중치 양자화를 진행합니다. 8비트는 높은 중요성을 가진 출력 특징에, 4비트는 그 외의 특징에 할당하여 시스템 설계를 보다 간편하게 합니다. 또한, 데이터를 효과적으로 변환하고 빠른 int8 Tensor Core 사용을 가능하게 하는 2단계 비양자화 과정을 설계하여 시스템 효율성을 크게 향상시킵니다.

- **Performance Highlights**: 대규모 실험 결과, MixLLM은 기존의 4비트 양자화 알고리즘을 초월하며, 오직 10%의 비트 증가로 PPL(Perplexity) 증가를 0.5에서 0.2로 상당히 줄여 Llama 3.1 70B 모델에 대해 뛰어난 정확도를 기록했습니다. 평균적으로 MMLU-Pro는 세 가지 인기 모델의 SOTA 대비 0.93 향상되었습니다. MixLLM은 이러한 탁월한 정확도와 함께 뛰어난 시스템 효율성을 달성했습니다.
...
### Single-Loop Federated Actor-Critic across Heterogeneous Environments (https://arxiv.org/abs/2412.14555)
Comments:
          Extended version of paper accepted at AAAI'25

- **What's New**: 본 논문에서는 이질적인 환경에서 여러 에이전트가 협력하여 학습할 수 있는 Single-loop Federated Actor Critic (SFAC) 알고리즘을 제안합니다. 이 접근법은 두 단계 구조를 통해 에이전트 간의 상호작용을 최적화하며, 기존 연합 강화 학습(FRL) 방법들과 차별화됩니다. 또한, SFAC의 수렴 오차에 대한 경계를 제공하여 이질적인 환경에서의 효과적인 학습이 가능함을 입증합니다.

- **Technical Details**: SFAC는 이질적인 환경에서 에이전트들이 두 단계의 연합 구조를 통해 액터-비평자(actor-critic) 학습을 수행하도록 설계되었습니다. 이 과정에서 에이전트의 비평자는 지역 가치 함수 모델을 집계하여 정책 평가를 수행하고, 액터는 지역 정책 그래디언트를 집계하여 정책 개선을 진행합니다. 이러한 구조는 비평자와 액터 간의 강한 상호작용을 통해 수렴성 분석에 새로운 도전 과제를 제공합니다.

- **Performance Highlights**: SFAC는 수치 실험을 통해 일반적인 RL 벤치마크에서 평가될 예정이며, 이를 통해 이 알고리즘의 효과성을 입증합니다. SFAC의 수렴 오차는 환경 이질성에 비례하여 0으로 수렴하며, 연합을 통해 샘플 복잡도 선형적 속도를 경험할 수 있음을 보였습니다. 이러한 성과는 실제 환경에서 적용 가능한 강화 학습의 새로운 가능성을 열어줍니다.



### Transformer models are gauge invariant: A mathematical connection between AI and particle physics (https://arxiv.org/abs/2412.14543)
Comments:
          10 pages, 2 figures, 1 table

- **What's New**: 이 논문은 입자 물리학에서의 기초적인 힘과 관련된 대칭성인 gauge invariance에 대한 논의를 통해 transformer 아키텍처의 새로운 특성을 밝힙니다. 저자는 데이터 모델링의 과정에서 transformer가 gauge invariance를 완전히 제거하지 않았음을 보여주고, 이는 모델의 불필요한 파라미터를 식별하고 이를 최적화하는 데 중요한 역할을 합니다. 이러한 발견은 신경망 아키텍처의 효율성을 높이는 데 기여할 수 있습니다.

- **Technical Details**: 논문의 두 번째 부분에서 확인된 대칭성은 모델의 특정 파라미터 조합에서 flat한 방향이 존재함을 보여줍니다. 이러한 flat한 방향은 추가적인 유용한 계산 경로 없이도 각종 연산 비용을 절감할 수 있습니다. 이로 인해 예측 단계에서 필요로 하는 연산의 양이 줄어들게 되며, 결과적으로 비용 절감 효과를 기대할 수 있습니다. 특히, layer normalization은 토큰을 특정한 형태의 단위 구에 제한하여, 해당 구를 따라 변형될 수 있는 대칭성을 형성합니다.

- **Performance Highlights**: 이 연구를 통해 transformer 모델에 내재된 대칭성을 이해하게 되면, 아키텍처의 설계를 더욱 효율적으로 만들어 준다는 점에서 성능 향상의 가능성을 제공합니다. 저자는 이 대칭성이 transformer의 각 구성 요소에 대해 적용 가능하다고 주장하며, 이를 통해 파라미터 최적화 과정에서 연산량을 줄이면서도 최적의 해결책에 이르는 경로는 보존할 수 있음을 강조합니다. 전체적인 최적화 과정에서 초래되는 효율적인 성능 개선은 최신 모델의 훈련 비용이 10억 달러에 접근하는 가운데, 연산시간 및 리소스를 절감하는 데 중요한 의미를 가집니다.
...
### ST-ReP: Learning Predictive Representations Efficiently for Spatial-Temporal Forecasting (https://arxiv.org/abs/2412.14537)
Comments:
          13 pages, 7 pages. Accepted by AAAI2025

- **What's New**: 이 논문에서는 공간-시간 예측(spatial-temporal forecasting)을 위한 새로운 경량 표현 학습 모델인 ST-ReP를 제안합니다. ST-ReP는 현재 값 복원(reconstruction)과 미래 값 예측(prediction)을 통합하여 사전 학습(pre-training) 프레임워크로 활용합니다. 특히, 모델의 예측 능력을 향상시키기 위해 다중 시간대(multi-time scale) 분석을 손실 함수에 포함시켰습니다. 실험 결과, 제안된 모델이 기존 사전 학습 기반 베이스라인보다 더 나은 성능을 보여주었습니다.

- **Technical Details**: ST-ReP는 복원 기반(self-supervised) 모델로, 변수 간의 공간적 상관관계를 명시적으로 고려하여 표현을 학습합니다. 제안된 공간-시간 인코더는 압축(compression), 추출(extraction), 복원(decompression) 과정을 통해 공간-시간 관계를 모델링합니다. 그리고 손실 함수는 다중 스케일의 시간 정보를 캡처하도록 설계되어, 기존의 계산 비용을 증가시키지 않고도 모델이 시간 상관관계를 이해하도록 돕습니다. 이러한 새로운 접근을 통해 ST-ReP는 효율적으로 의미가 풍부한 공간-시간 표현을 학습할 수 있습니다.

- **Performance Highlights**: ST-ReP는 다양한 분야에서 수집된 6개의 공간-시간 데이터셋에서 실험을 수행하였고, 그 결과 기존의 고급 자기 지도 학습(baseline) 방법들보다 더 높은 다운스트림 예측 정확도를 달성하였습니다. 모델은 또한 더 낮은 차원의 표현을 학습하면서 뛰어난 확장성(scalability)을 보여주었습니다. 이러한 특징들은 ST-ReP가 실제 활용 가능성이 뛰어난 모델임을 입증하고 있습니다.



### Leveraging Time Series Categorization and Temporal Fusion Transformers to Improve Cryptocurrency Price Forecasting (https://arxiv.org/abs/2412.14529)
- **What's New**: 본 연구는 암호화폐 포트폴리오 관리를 위한 최적 자산 선택을 돕기 위해 유사한 행동을 보이는 하위 시계열을 분류하여 예측 정확도를 높이는 접근 방식을 제안합니다. 각 하위 시계열 카테고리에서 Transformer 기반의 딥러닝 모델을 통해 다음 데이터 포인트를 예측하며, 데이터의 부족 문제를 해결하기 위해 다른 암호화폐의 시계열 데이터를 결합하는 방법을 채택합니다.

- **Technical Details**: 이 연구에서는 시간적 패턴과 종속성을 포착하기 위해 Temporal Fusion Transformer (TFT) 아키텍처를 기반으로 하여 유사한 하위 시계열로 데이터를 분리하고, 각 카테고리에 따른 예측 모델을 학습시킵니다. Markov 체인을 활용하여 마지막 관찰된 하위 시계열의 카테고리를 찾은 후 다음 카테고리를 예측하고, 해당 카테고리에 적합한 Transformer 모델을 사용하여 암호화폐 가치를 예측합니다.

- **Performance Highlights**: 본 연구에서 제안한 하위 시계열 분리는 전통적인 딥러닝 모델인 LSTM과 간단한 TFT 모델에 비해 예측 정확도를 개선하여 알고리즘 거래의 수익성을 증가시켰습니다. 또한, 하위 시계열의 반복적인 이벤트를 통해 모델의 학습을 최적화함으로써 성능을 극대화하는 결과를 도출하였습니다.



### Knowledge Distillation in RNN-Attention Models for Early Prediction of Student Performanc (https://arxiv.org/abs/2412.14526)
Comments:
          Published in Proceedings of The 40th ACM/SIGAPP Symposium on Applied Computing (SAC '25), see this https URL

- **What's New**: 본 연구는 교육 데이터 마이닝(EDM) 분야에서 조기 예측을 통해 위험에 처한 학생들을 식별하는 새로운 접근 방식을 제안합니다. RNN-Attention-KD(지식 증류) 프레임워크를 도입하여 학습 활동 데이터의 시계열을 효과적으로 분석합니다. 이 모델은 신경망의 특정 강점을 활용하여, 위험에 처한 학생들을 조기에 식별할 수 있도록 지원합니다.

- **Technical Details**: RNN-Attention-KD 프레임워크는 순환 신경망(Recurrent Neural Networks, RNN)을 기반으로 하며, 주의 메커니즘(attention mechanism)을 통해 학생들의 성과를 시간적 간격에서 예측합니다. 또한, 지식 증류(Knowledge Distillation, KD)를 적용하여 시간이 압축된 데이터로 조기 예측을 가능케 합니다. 이 과정에서 큰 모델으로부터 작은 모델로 지식을 전이하여 예측 성능을 향상시킵니다.

- **Performance Highlights**: 이 연구의 결과, RNN-Attention-KD 모델은 전통적인 신경망 모델에 비해 회상(recall)과 F1 점수에서 우수한 성능을 보였습니다. 예를 들어, 대학 강좌의 4개 데이터 세트에서 1-3주 및 1-6주 동안 각각 0.49 및 0.51, 0.51 및 0.61의 회상과 F1 점수를 기록했습니다. 더불어, 다양한 지식 전이 방법(지식 증류 목표)의 기여에 대한 연구도 진행되어, 성과 예측 성능을 향상시키는 데 효과적임을 발견했습니다.
...
### A hybrid framework for effective and efficient machine unlearning (https://arxiv.org/abs/2412.14505)
Comments:
          14 pages, 5 figures, accepted by CSE2024

- **What's New**: 최근 기계 비학습(Machine Unlearning, MU)이 제안되어 사용자의 개인 정보 보호 문제를 해결하기 위해 이미 학습된 모델 매개변수에서 삭제된 샘플의 흔적을 제거하는 방법을 다루고 있습니다. 전통적인 방법인 처음부터 다시 학습하는 것과 달리 본 논문에서는 정확한 MU와 근사 MU의 하이브리드 전략을 제시하여 효율성과 정확성을 동시에 개선하고자 합니다. 특정 조건 아래에서 경량의 재학습 기법을 사용하고, 필요 시 현재의 매개변수를 직접 수정하여 비학습을 수행하는 방식을 제안합니다.

- **Technical Details**: 제안된 하이브리드 전략은 두 가지 다른 방법인 부분 재학습 전략(Partial Retraining Strategy, PRS)과 직접 매개변수 업데이트 전략(Direct Parameters Update Strategy, DPUS)을 기반으로 합니다. 데이터에 대한 영향을 최소화하기 위해 미리 보존된 매개변수를 불러와 특정 샘플을 제거한 후 재학습 과정을 진행합니다. 반면, 재학습 비용이 지나치게 클 경우에는 기존 매개변수로부터 삭제된 데이터의 영향을 뺀 후 직접 업데이트를 수행하여 비학습을 진행합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법을 통해 다양한 데이터셋에서 비학습 효율성을 1.5배에서 8배까지 개선하면서도 모델 정확도를 유지할 수 있음을 보였습니다. 본 연구는 또한 멤버십 추론 공격을 통해 비학습 방법의 효과성을 검증하였으며, 법적 제약사항에 부합하는 기계 학습 서비스 제공을 위한 새로운 방향을 제시합니다.
...
### Graph-Structured Topic Modeling for Documents with Spatial or Covariate Dependencies (https://arxiv.org/abs/2412.14477)
- **What's New**: 이 논문은 주제 모델링(topic modeling)에서 문서 수준의 메타데이터(document-level metadata)를 통합하여 주제 혼합 추정(topic mixture estimation)을 개선하는 새로운 접근법을 제안합니다. 기존의 베이지안(Bayesian) 방법에서의 계산 복잡성과 이론적 보장이 부족한 문제를 해결하기 위해, 확률론적 잠재 의미 색인(probabilistic latent semantic indexing, pLSI) 방식을 확장합니다. 이 과정에서 문서를 노드(node)로, 문서 간 유사성을 그래프 형태로 나타내며, 새로운 추정기를 제안합니다.

- **Technical Details**: 제안된 방법은 문서 간 유사성을 반영하여 유사한 문서가 비슷한 주제 혼합 비율을 공유하도록 장려하는 빠른 그래프 정규화(iterative singular value decomposition, SVD) 기반의 추정기를 사용합니다. 또한, 고확률 경계를 도출하는 방식으로 추정 오류를 특성화하고, 정규화 매개변수를 최적화하기 위한 특수한 교차 검증(method) 방법을 개발하여 수학적 근거를 제시합니다. 이러한 설정을 통해 고차원 데이터의 구조를 효과적으로 표현할 수 있습니다.

- **Performance Highlights**: 모델은 합성 데이터셋(synthetic datasets)과 세 가지 실제 데이터 소스를 사용한 포괄적인 실험을 통해 검증되었으며, 기존의 베이지안 방법들에 비해 성능 향상 및 빠른 추론 속도를 보여줍니다. 특히, 주제 모델링에서 빠른 추정과 함께 높은 정확성을 달성하며 문서 간의 공통된 구조를 잘 포착하는 특징이 있습니다. 이로 인해, 암 미세환경(tumor microenvironments), 마이크로바이옴 연구(microbiome studies), 짧은 문서 분석 등의 다양한 응용 분야에서 유용성이 입증되었습니다.



### Benign Overfitting in Out-of-Distribution Generalization of Linear Models (https://arxiv.org/abs/2412.14474)
Comments:
          58 pages, 1 figure

- **What's New**: 이번 논문에서는 benign overfitting에 대한 이해를 OOD(Out-of-Distribution) 환경에서 확장하려고 한다. 기존 연구들은 주로 in-distribution 환경에 중점을 두었던 반면, 본 연구는 covariate shift가 있는 상황에서 over-parameterized(과도 파라미터화된) 선형 모델의 기본 설정을 검토한다. 이를 통해 OOD 환경에서도 benign overfitting이 발생하는 조건을 명확히 제시하였다.

- **Technical Details**: 연구에서는 ridge regression을 통한 OOD에서의 benign overfitting을 비대칭적인 보장을 제시하고, 목표 공분산(target covariance)가 특정 구조적 조건을 만족할 때 발생한다고 증명하였다. 이 과정에서, source와 target 공분산과 관련된 여러 중요한 양을 규명하였으며, 이들이 OOD 일반화 성능에 미치는 영향을 분석하였다. 주목할 점은, 우리의 결과가 이전의 in-distribution 및 under-parameterized OOD 보장을 회복하는 데 성공했음을 보여준다.

- **Performance Highlights**: 이 논문은 standard ridge regression이 OOD에서 $O(1/	ext{sqrt}(n))$의 느린 통계적 비율을 달성하는 반면, Principal Component Regression(PCR)은 빠른 비율인 $O(1/n)$을 보장한다는 이론적 결과를 제시한다. 이러한 성과는 다양한 target 공분산 행렬에 대해 일반화될 수 있는 가능성을 보여주며, 요약하자면 OOD 환경에서도 모델의 성능을 개선할 수 있는 중요한 지침을 제공한다.
...
### Balanced Gradient Sample Retrieval for Enhanced Knowledge Retention in Proxy-based Continual Learning (https://arxiv.org/abs/2412.14430)
- **What's New**: 이 논문에서는 지속적인 학습(continual learning) 중 발생하는 치명적인 망각(catastrophic forgetting) 문제를 해결하기 위한 새로운 샘플 검색 전략을 제안합니다. 이 전략은 메모리 버퍼로부터의 그레이디언트 간섭 샘플(gradient-conflicting samples)과 그레이디언트 정렬 샘플(gradient-aligned samples)을 활용하여 과거 작업에 대한 지식을 효과적으로 유지하는 것을 목표로 합니다. 이 접근법은 샘플 간의 다양성을 높이고 파라미터 공간 내의 정렬을 개선하여 지식 유지 및 대리 불확실성(proxy drift)을 줄이는 데 기여합니다.

- **Technical Details**: 제안된 방법론은 감정 대조 학습(supervised contrastive learning) 프레임워크를 기반으로 하며, 메모리 버퍼에서의 샘플 검색 전략을 정립합니다. 이 방법은 과거 작업의 지식을 보존하기 위해 gradients의 정렬(re-aligning)과 간섭(interference) 문제를 동시에 고려합니다. 이론적 분석을 통해 그레이디언트 정렬 인스턴스는 작업 간의 안정적인 임베딩을 찾는 데 유용하며, 그레이디언트 간섭 인스턴스는 과거 지식을 보존하고 치명적 망각을 피하는 데 도움이 된다는 것을 보여줍니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 단일 샘플 유형 또는 무작위 샘플 검색에 의존하는 기존 방법들보다 뛰어난 성능을 보여줍니다. 컴퓨터 비전 분야에서의 일반적인 지속적인 학습 벤치마크 실험을 통해 치명적 망각을 감소시키는 한편 새로운 작업에 대한 경쟁적인 정확도를 유지함을 입증했습니다. 이로 인해 메모리 기반 지속적 학습 알고리즘 중 최첨단 성능을 가진 방법으로 자리매김할 가능성이 높습니다.
...
### Stealing That Free Lunch: Exposing the Limits of Dyna-Style Reinforcement Learning (https://arxiv.org/abs/2412.14312)
- **What's New**: 이 논문은 Dyna-style off-policy model-based reinforcement learning (DMBRL) 알고리즘의 성능 격차를 조사합니다. 특히, 동일한 물리 엔진을 사용하는 OpenAI Gym과 DeepMind Control Suite (DMC) 환경 간의 성능 차이를 밝혔습니다. DMBRL 알고리즘이 OpenAI Gym에서는 잘 작동하지만 DMC에서는 성능이 급락하는 현상을 발견했습니다.

- **Technical Details**: DMBRL 알고리즘은 상태 전환 데이터를 합성 생성하여 off-policy 강화 학습의 샘플 효율성을 높이는 데 초점을 맞춘 기술 모음입니다. 그러나 DMC와 같은 특정 환경에서는 합성 롤아웃(synthetic rollouts) 추가로 인해 오히려 성능이 저하되는 결과를 보였습니다. 이는 환경 간 성능 평가에서의 일관성이 부족하다는 것을 나타내며, 모델 기반 강화 학습에서 중요한 도전 과제를 드러냅니다.

- **Performance Highlights**: DMBRL 알고리즘의 성능 저하 문제는 모델 기반 강화 학습의 기본 원리를 이해하는 데 기여합니다. 특정 현대 기술들로 인해 다양한 환경에서 기대한 만큼의 성능 향상이 이루어지지 않음을 보여줍니다. 종합적으로, 다양한 벤치마크에서 성능을 평가할 때에는 '보험'이 없다는 점이 강조되었습니다.



### The Multiplex Classification Framework: optimizing multi-label classifiers through problem transformation, ontology engineering, and model ensembling (https://arxiv.org/abs/2412.14299)
Comments:
          43 pages, 15 figures, submitted to Applied Ontology

- **What's New**: 이 논문은 Multiplex Classification Framework라는 새로운 접근 방식을 소개합니다. 이 프레임워크는 문제 변환(problem transformation), 온톨로지 엔지니어링(ontology engineering), 그리고 모델 앙상블(model ensembling)을 통합하여 복잡한 분류 문제를 해결하는 데 초점을 맞추고 있습니다. 기존의 분류 모델보다 더 나은 적응성과 클래스 불균형(class imbalance) 관리 방법을 제공하는 점에서 차별화됩니다.

- **Technical Details**: Multiplex Classification Framework는 여러 클래스(class)와 논리적 제약 조건(logical constraints)을 지원하며, 클래스 불균형 문제를 혁신적으로 처리하는 방법을 제시합니다. 또한, 신뢰도 임계값(confidence threshold) 선택을 없애고 모듈화된 구조를 제공합니다. 이 방법은 복잡한 분류 문제에 대한 깊은 이해와 함께 온톨로지 엔지니어링에 대한 경험이 필요한 한편, 여러 모델을 훈련해야 하므로 절차가 복잡해질 수 있다는 제한사항이 있습니다.

- **Performance Highlights**: 실험 결과에 따르면, Multiplex 접근 방식은 기존의 분류 모델에 비해 전체 F1 점수에서 최대 10% 향상을 보였습니다. 특히, 클래스 수가 많고 클래스 불균형이 두드러진 분류 문제에서 성능 개선이 두드러졌습니다. 이러한 결과는 복잡한 분류 문제를 다루는 연구자와 실무자들에게 유용한 도구를 제공할 수 있음을 시사합니다.



### Distributionally Robust Policy Learning under Concept Drifts (https://arxiv.org/abs/2412.14297)
- **What's New**: 이번 연구는 개념 변화(concept drift)에 따른 강건한 정책 학습(robust policy learning) 문제를 다루며, 기존의 방법들이 고려하지 않았던 조건부 관계의 변화를 포함합니다. 이전의 접근 방식은 공동 분포(joint distribution)의 변화를 고려하여 다소 보수적일 수 있었던 반면, 이 연구에서는 조건부 보상의 분포만이 변화하는 상황을 명확히 합니다. 이를 통해 정책 평가와 학습 모두에서 성능을 개선할 수 있는 새로운 알고리즘을 제안했습니다.

- **Technical Details**: 이 논문은 개념 변화 하에 평균 보상을 극대화하는 정책을 찾기 위한 방법을 제시합니다. 특히, 주어진 정책에 대해 두 번 강건한(dual-robust) 추정기를 제공하고, 이 추정기가 느린 수렴 속도로도 대칭 정규 분포(asymptotic normal)를 만족하는 성질을 주장합니다. 또한, 정책 클래스 (policy class) Π 내에서 정책 가치를 최대화하는 알고리즘을 제안하며, 하한(bound)과 상한(upper bound)을 모두 제공해 알고리즘의 최적성을 확립합니다.

- **Performance Highlights**: 제안된 방법은 숫자적 연구에서 기존 기준에 비해 상당한 성능 개선을 보여줍니다. 특히, 정책 학습 측면에서 고정확도와 관련된 하락률(regret/suboptimality gap)을 공간 복잡성(policy class complexity)과 샘플 수(sample size)로부터 파생된 κ(Π)/√n 형태로 제시합니다. 결과적으로, 제공된 알고리즘은 원래의 최적 정책과 비교했을 때, 고확률(low probability)로 우수한 성능을 입증했습니다.



### FedSTaS: Client Stratification and Client Level Sampling for Efficient Federated Learning (https://arxiv.org/abs/2412.14226)
Comments:
          6 pages, 3 figures, to be submitted to ICML

- **What's New**: 이번 논문에서는 Federated Stratification and Sampling (FedSTaS)라는 새로운 클라이언트 및 데이터 레벨 샘플링 방법을 제안합니다. FedSTaS는 기존의 FedSTS와 FedSampling에서 영감을 받아, 각 연도마다 클라이언트의 압축된 그래디언트에 따라 클라이언트를 계층화하고 최적의 Neyman 할당을 통해 샘플링합니다. 이 방법은 데이터 레벨 샘플링과 로컬 차별 개인 정보 보호(Local Differential Privacy, LDP)를 통해 개인 정보 보호를 더욱 강화합니다.

- **Technical Details**: FedSTaS는 FedSampling의 데이터 샘플링 방법과 FedSTS의 압축 그래디언트 클러스터링을 통합하여, 클라이언트 샘플링의 효율성을 높이는데 중점을 두고 있습니다. FL의 목표는 N개의 클라이언트가 공동으로 글로벌 모델을 학습하는 것이며, 각 클라이언트는 자신의 데이터에 대한 로컬 학습을 수행한 후 업데이트된 파라미터를 서버에 반환합니다. FedSTaS는 이러한 로컬 업데이트의 편향성을 줄이고, 훈련 효율성을 향상시키는 데 도움을 줍니다.

- **Performance Highlights**: 세 가지 데이터셋에 대한 실험 결과, FedSTaS는 고정된 학습 라운드 내에서 기존의 FedSTS보다 더 높은 정확도를 달성했습니다. 이는 독립적이고 동일한 데이터(IID) 및 비IID 데이터를 모두 포함하여 나타났습니다. FedSTaS는 새로운 샘플링 방법이 FL의 효율성과 정확도를 개선하는데 효과적임을 보여줍니다.



### Towards Precise Prediction Uncertainty in GNNs: Refining GNNs with Topology-grouping Strategy (https://arxiv.org/abs/2412.14223)
Comments:
          Accepted at AAAI 2025

- **What's New**: 최근 그래프 신경망(Graph Neural Networks, GNNs)의 발전으로 인해 모델 예측의 보정(calibration)의 중요성이 강조되고 있습니다. 본 논문에서는 이웃의 예측 유사성(neighborhood prediction similarity)이 보정의 중요한 요소로 인식되고 있음을 바탕으로, 기존 GNN 보정 방법들의 한계를 설명하고 이를 해결하기 위한 새로운 방법인 Simi-Mailbox를 제안합니다. Simi-Mailbox는 이웃 유사성과 신뢰도(confidence)에 기반하여 노드를 그룹화하고, 그룹별로 온도 조절을 통해 보다 세밀한 보정을 가능하게 합니다.

- **Technical Details**: 이 논문은 Simi-Mailbox라는 새로운 보정 방법을 제안하여, 유사한 이웃 예측을 가지는 노드라도 신뢰도가 다를 경우 보정 오류가 다르게 나타날 수 있음을 지적합니다. 이는 기존의 단일 보정 접근 방식이 모든 경우에 적용될 수 없음을 보여주며, 그로 인해 하위 최적의 보정 결과가 발생할 수 있음을 강조합니다. Simi-Mailbox는 각 노드 그룹에 대해 특화된 온도를 조절하여 예측의 정확성을 높이고, 네트워크의 근접성이나 연결성과는 무관하게 작동하도록 설계되었습니다.

- **Performance Highlights**: Simi-Mailbox는 다양한 GNN 아키텍처와 데이터셋에서 효과성을 입증하여, 보정되지 않은 GNN 예측과 비교해 최대 13.79%의 오류 감소를 달성했습니다. 이에 따라, Simi-Mailbox는 GNN 보정에 있어 더 세밀한 조정이 가능하다는 점에서 과거 연구들에 비해 두드러진 성과를 보여줍니다. 이는 기존의 GNN 보정 방법들의 한계를 극복할 수 있는 가능성을 제시합니다.
...
### STRAP: Robot Sub-Trajectory Retrieval for Augmented Policy Learning (https://arxiv.org/abs/2412.15182)
Comments:
          Project website at this https URL

- **What's New**: 로봇 학습 분야에서, STRAP라는 새로운 기법이 소개되었습니다. STRAP는 많은 로봇 학습 데이터셋을 효과적으로 활용하기 위해, 테스트 시간에 실시간으로 관련 데이터를 검색 및 훈련합니다. 이는 기존의 다-task 전문가 정책보다, 특정 작업에 더 적합한 접근 방식을 제공하며, 일반적인 정책들의 성능 저하 문제를 해결하는 데 초점을 맞추고 있습니다.

- **Technical Details**: STRAP는 'sub'-trajectory의 유사성을 기반으로 데이터 검색을 수행합니다. 즉, 전반적인 궤적 전체 대신, 필요한 행동의 하위 궤적을 검색하여 학습 상황에 맞춘 정책 훈련을 가능하게 합니다. 이 과정에서 DINOv2 같은 기초 모델의 특징을 활용하여, 다양한 작업 간의 공통 요소를 포착하며, 동적 시간 왜곡(dynamic time warping) 기법을 통해 궤적의 길이에 관계없이 유사성을 계산합니다.

- **Performance Highlights**: 실험 결과, STRAP는 기존 검색 알고리즘 및 다-task 학습 방법들을 초월하는 성능을 입증하였습니다. STRAP는 대규모 오프라인 데이터셋에서도 확장 가능하며, 실제 시연 몇 번으로도 견고한 제어 정책을 학습할 수 있는 능력을 보여줍니다. 이는 로봇이 동적인 환경에서 유연하게 적응할 수 있도록 도와줍니다.



### HPC-Coder-V2: Studying Code LLMs Across Low-Resource Parallel Languages (https://arxiv.org/abs/2412.15178)
- **What's New**: 이번 연구에서는 고성능 컴퓨팅(High Performance Computing, HPC) 분야에 특화된 대형 언어 모델(Large Language Model, LLM)에 대한 심층 연구를 수행했습니다. 기존의 LLM 기반 코딩 도구들이 일반 프로그래밍 작업에는 성공적이지만, HPC와 같은 전문 분야에서는 성능이 저조하다는 문제를 다루고 있습니다. 연구 결과, HPC에 최적화된 LLM을 미세 조정하여 기존의 오픈 소스 코드 LLM 중 평행 코드 생성에서 최고의 성능을 발휘하는 모델을 개발했습니다.

- **Technical Details**: 이 연구는 HPC LLM을 미세 조정(fine-tuning)하는 다양한 축(axes)에서의 과제를 이해하고 해결하려는 노력을 포함하고 있습니다. 이전의 연구에서 HPC에 특화된 모델들이 탐색되었지만, LLM들이 평행 코드(parallel code)를 생성하는 데 어려움을 겪는 이유를 파악하는 데 중점을 두었습니다. 본 연구는 이러한 문제를 규명하고, 이를 해결하기 위한 접근 방식을 제안합니다.

- **Performance Highlights**: 최종적으로, 연구팀은 최적화된 HPC LLM이 현재까지의 평행 코드 생성에 대한 오픈 소스 모델 중 가장 높은 성능을 보여준다고 평가했습니다. 이러한 성과는 HPC 작업에서 LLM의 응용 가능성을 높이는 중요한 진전을 나타냅니다. 이로 인해 HPC 분야의 소프트웨어 개발에 있어 LLM의 혜택을 실현하는 데 큰 기여를 할 것으로 기대됩니다.
...
### Tests for model misspecification in simulation-based inference: from local distortions to global model checks (https://arxiv.org/abs/2412.15100)
Comments:
          11 pages, 5 figures. Code available on github (NoemiAM/mist) at this https URL

- **What's New**: 본 연구에서는 모델 미스스펙피케이션 분석을 위한 포괄적인 시뮬레이션 기반 프레임워크를 제시하고 있습니다. 특히, 왜곡 기반의 테스트를 통해 다양한 모델 불일치를 평가할 수 있는 탄탄하고 유연한 기초를 마련하여 과학적 모델 개발을 향상시키고자 합니다. 본 연구는 또한 고전적인 기법과의 명확한 연결을 수행하고, 실무자에게 유용한 자가 보정 훈련 알고리즘을 도입합니다.

- **Technical Details**: 이 프레임워크는 데이터 증강을 통한 고용량 가설 테스트 기법을 기반으로 하여, 다양한 각도에서 모델의 변동성을 탐색합니다. 제안된 시스템은 이상 탐지(anomaly detection) 및 모델 검증(model validation) 같은 고전적인 기법과 일치할 수 있는 가능성을 포함하고 있으며, 관측 소음에 맞추어 적절한 왜곡을 찾기 위한 과정을 간소화합니다. 모델 미스스펙피케이션 검출 전략과 기존 기법 간의 관계도 충실히 다루어집니다.

- **Performance Highlights**: 프레임워크의 성능은 다양한 시나리오에서 검증되었으며, 특히 실제 중력파 데이터(GW150914)에서 기법을 적용한 실제 사례를 통해 중요한 연결고리를 보여줍니다. 연구 결과는 기존의 통계적 결과와 잘 연결되며, 직관적인 예시를 제공함으로써 제안된 접근법의 유효성을 강조합니다. 이를 통해 보다 효과적으로 모델 미스스펙피케이션 문제를 해결할 수 있는 가능성을 제시하고 있습니다.
...
### From Point to probabilistic gradient boosting for claim frequency and severity prediction (https://arxiv.org/abs/2412.14916)
Comments:
          26 pages, 4 figures, 26 tables, 7 algorithms

- **What's New**: 본 논문에서는 전통적인 일반화 선형 모델(Generalized Linear Models, GLMs)에 비해 보험 위험 예측에서 더 나은 성능을 보이는 다양한 그래디언트 부스팅 머신(Gradient Boosting Machine, GBM) 알고리즘을 종합적으로 비교分析하였습니다. 특히, GBM, XGBoost, LightGBM, CatBoost 등 다양한 포인트(Point) 및 확률적(Probabilistic) GBM 알고리즘을 통일된 표기법으로 제시하고, 이들의 성능을 공정하게 분석하여 기존 기술의 발전을 잘 보여주고 있습니다. 또한, 확률적 예측의 필요성을 강조하면서 보험 모델링에서 불확실성 평가의 중요성을 제기합니다.

- **Technical Details**: GBM 알고리즘은 결정 트리를 통해 약한 학습기(Weak Learners)를 결합해 점진적으로 예측 성능을 향상시키는 기법으로, 이는 경량화된 계산 방법(XGBoost와 LightGBM)을 통해 성능과 효율을 크게 개선합니다. 이 논문에서는 5개의 공개 데이터 세트를 통해 청구 빈도와 정도에 대한 성능을 비교하였으며, 다양한 고유 카테고리 변수를 포함하는 경우에도 알고리즘의 적합성을 분석합니다. 또한, 포인트 예측에서 관측값별 예측 분포로의 이동을 설명하며, 다양한 위험 노출에 대한 처리 방안을 제시합니다.

- **Performance Highlights**: LightGBM과 XGBoostLSS가 계산 효율성 면에서 두각을 나타냈으며, EGBM은 기존 블랙박스 알고리즘에 비해 해석 가능성 측면에서 매우 경쟁적인 예측 성능을 기록하였습니다. 논문에서는 모델 적합성과 예측 정확성 간에 상충이 없음을 발견하였으며, 모든 알고리즘은 유사한 예측 성능을 달성했으나 CatBoost는 고차원 카테고리 변수가 포함된 데이터 셋에서 소폭의 성능 향상을 보여주었습니다. 이와 같은 비교 연구는 기존 알고리즘 이해에 기여하며 향후 보험 데이터 분석 분야에서의 활용 가능성을 제시합니다.
...
### Surrogate-assisted multi-objective design of complex multibody systems (https://arxiv.org/abs/2412.14854)
Comments:
          arXiv admin note: text overlap with arXiv:2412.01566

- **What's New**: 이 논문에서는 다목적 최적화(Multi-Objective Optimization) 문제를 해결하기 위한 새로운 접근 방식을 제안하고 있습니다. 이를 위해 서그릿 모델(Surrogate Model), 즉 다소 비싼 모델 평가로부터 만들어진 근사 모델을 활용하여 최적의 해를 구하는 방법을 모색하고 있습니다. 특히 다체계(multibody systems)의 경우 계산량이 방대하므로, 이를 해결하기 위한 상호작용적 접근법을 소개합니다.

- **Technical Details**: 연구에서는 다체계 시스템을 수학적으로 모델링하기 위해 오일러-뉴턴 공식(Newton-Euler Formalism)을 사용합니다. 즉, 질량 행렬(Mass Matrix), 일반화된 힘 벡터(Generalized Forces) 등을 포함하는 미분 방정식을 통해 동역학 행동을 분석합니다. 또한, 적절하게 선택된 점에서 얻은 데이터로 서그릿 사례를 다수 훈련하고, 이를 통해 더 높은 품질의 해답을 얻을 수 있는 방법을 설명합니다.

- **Performance Highlights**: 이 논문의 실험 결과는 NSGA-II 알고리즘과 신경망 서그릿 모델을 활용한 페어토 집합(Pareto Set)의 상호 작용적 연결이 최적의 해답을 찾기 위한 가장 효과적인 방법임을 보여줍니다. 다양한 샘플링 전략과 다목적 진화 알고리즘을 비교하여, 계산 효율성과 솔루션 품질 측면에서 가장 유망한 접근 방식을 식별하였습니다. 이러한 결과는 다체계 최적화의 발전에 기여할 것으로 기대됩니다.
...
### Opportunities and limitations of explaining quantum machine learning (https://arxiv.org/abs/2412.14753)
Comments:
          16+16 pages, 3+4 figures

- **What's New**: 본 연구에서는 양자 기계 학습 모델의 설명 가능성에 대한 주요 연구 방향을 제시합니다. 기존의 연구 부족에 대한 인식을 바탕으로, 양자 기계 학습의 개념을 소개하고 우리가 최초로 제안하는 두 가지 설명 기법도 시연합니다. 이 논문의 주요 초점은 양자 기계 학습 모델이 AI의 신뢰성을 높일 수 있는 방법을 탐색하는 것입니다.

- **Technical Details**: 양자 기계 학습(QML)에서의 설명 가능성은 기존의 설명 가능한 AI(XAI)와는 현저히 다릅니다. 본 논문에서는 파라미터화된 양자 회로(parametrized quantum circuits, PQCs)의 원리를 소개하며, 양자 상태가 확률 분포의 일반화로 어떻게 작용하는지를 설명합니다. 양자 상태는 비단순한 특정 행렬로 표현되며, 측정 시 확률 분포를 생성하여 통계적 정보를 저장합니다.

- **Performance Highlights**: 양자 기계 학습 모델은 고도로 구조화된 문제를 다룰 때 고전적 알고리즘보다 뛰어난 성능을 발휘할 수 있는 잠재력이 있습니다. 연구 결과는 양자 이해 가능성과 관련된 기법을 개선하기 위한 기회를 제공하며, 여기에 대한 여러 가지 새로운 방법을 비교할 예정입니다. 본 연구의 목적은 양자 기계 학습의 신뢰성과 이해 가능성을 증진시키는 것입니다.



### Deep Learning Based Recalibration of SDSS and DESI BAO Alleviates Hubble and Clustering Tensions (https://arxiv.org/abs/2412.14750)
Comments:
          5 pages, 2 figures, 2 tables. Comments are welcome

- **What's New**: 이번 연구는 Baryon Acoustic Oscillations (BAO) 데이터의 보정을 위한 새로운 방법을 제시합니다. 전통적인 방법이 우주론적 모델에 의존하는 반면, 우리는 심층 학습(deep learning) 기술을 사용하여 모델 독립적으로 소리 수평선(sound horizon)을 추정하는 접근 방식을 채택했습니다. 이를 통해 SDSS와 DESI의 두 BAO 데이터 세트를 재보정하여 $	extbackslashLambda CDM$ 우주론적 매개변수에 미치는 영향을 분석했습니다.

- **Technical Details**: 본 연구에서는 초우주(early universe) 관측 데이터를 기반으로 하여 소리 수평선 $r_d$를 추정하는 대신, 심층 학습 기술을 통해 BAO 데이터의 독자적인 재보정을 수행했습니다. 이러한 방법은 기존의 우주론적 모델에 대한 의존성을 줄이며, Hubble 매개변수($H_0$)와 클러스터링 매개변수($S_8$) 간의 긴장을 상당히 감소시키는 결과를 가져왔습니다.

- **Performance Highlights**: 재보정된 데이터 세트 각각에 대해 Hubble과 클러스터링 긴장도가 크게 줄어드는 것이 관찰되었습니다. 또한 몇몇 다른 매개변수에서의 중간 정도의 변화는 데이터 기반 접근 방식의 추가 탐색이 필요함을 시사합니다.



### Active Inference and Human--Computer Interaction (https://arxiv.org/abs/2412.14741)
- **What's New**: 이번 논문에서는 Active Inference (AIF) 이론이 인간-컴퓨터 상호작용(HCI)의 설계와 분석 방식에 어떻게 기여할 수 있는지를 다루고 있습니다. AIF는 내부적 확률적 생성 모델을 가진 에이전트가 환경의 숨겨진 상태에 대한 신념을 인코딩하여 예측되는 놀라움을 최소화하려는 행동을 기반으로 한 이론입니다. 이 연구는 AIF를 통해 HCI 이론과 설계를 통합하려고 하며, 덜 예측 가능한 사용자 경험을 개선할 수 있는 새로운 도구를 제공할 것으로 기대합니다.

- **Technical Details**: Active Inference 이론은 생물학적 또는 인공 에이전트가 환경의 숨겨진 상태로부터 감각을 이끌어내는 방식에 대한 믿음을 인코딩하는 내부 생성 모델을 가정합니다. 이론은 Bayesian inference, probabilistic programming, 그리고 동적 시스템을 통합하여 상호작용 설계의 상호 제어 과정을 모델링합니다. AIF는 자가 조직화 시스템의 자유 에너지를 최소화하는 원칙을 기반으로 하며, 이를 통해 설계 결정이 특정 사용자 및 맥락에 맞춰 조정될 수 있도록 지원합니다.

- **Performance Highlights**: AIF는 HCI의 온라인 및 오프라인 분석 방법론에 중대한 영향을 미치며, 사용자 선호를 반영하여 신속하게 적응할 수 있는 상호작용 시스템을 설계하는 데 기여할 수 있습니다. 이 이론은 의도, 자유, 그리고 회복력 문제에 대한 새로운 관점을 제공하고 상호작용 엔지니어링을 위한 실용적인 기법을 제안합니다. 그러나 AIF 시스템 구현 과정에서 모델링, 계산적 문제 및 소프트웨어 개발의 한계와 같은 실질적인 도전 과제가 존재함을 강조합니다.



### On the Use of Deep Learning Models for Semantic Clone Detection (https://arxiv.org/abs/2412.14739)
Comments:
          Accepted at the 40th IEEE International Conference on Software Maintenance and Evolution (ICSME 2024)

- **What's New**: 이 논문에서는 코드 클론(duplicate code fragments) 감지를 위한 다섯 개의 최신 모델을 평가하는 새로운 접근 방식을 제안합니다. 특히 GPTCloneBench라는 새 데이터셋을 포함하여 다양한 벤치마크 데이터셋을 활용해 모델의 성능을 비교합니다. 기존 데이터셋의 한계를 극복하기 위해 차세대 클론 탐지 기술이 어떻게 서로 다르게 성능을 발휘하는지를 조사하고 있습니다.

- **Technical Details**: 코드 클론 감지 모델은 주로 심층 학습(deep learning)을 기반으로 하고 있으며, 이러한 모델은 주로 BigCloneBench 데이터셋에서 평가되고 있습니다. 그러나 이 데이터셋은 클래스 불균형(class imbalance) 문제와 의미 클론(semantic clone)의 부족으로 그 효용성이 떨어집니다. 따라서, 이 연구에서는 ASTNN, GMN, CodeBERT와 같은 단일 언어 모델을 평가하고, C4, CLCDSA와 같은 교차 언어 모델과의 성능을 비교하여 구체적인 코드 표현 및 네트워크 아키텍처에 따른 모델의 이해도를 분석합니다.

- **Performance Highlights**: 단일 언어 모델들은 BigCloneBench에서 높은 F1 점수를 보이지만, SemanticCloneBench에서는 성능 차이가 최대 20%에 이릅니다. 흥미롭게도, C4 교차 언어 모델은 SemanticCloneBench에서 다른 모델에 비해 약 7% 더 나은 성능을 보이며, BigCloneBench와 GPTCloneBench에서도 비슷한 성능을 발휘하고 있습니다. 또한, 변형 기반의 데이터셋에서 C4 모델은 단일 언어 모델보다 강력한 성능을 보여주며, 단일 언어 모델들은 성능이 매우 변동적입니다.
...
### Taming the Memory Beast: Strategies for Reliable ML Training on Kubernetes (https://arxiv.org/abs/2412.14701)
Comments:
          4 pages

- **What's New**: 본 연구는 Kubernetes가 기계 학습(ML) 훈련에 있어 메모리 관리를 어떻게 처리하는지를 구체적으로 설명합니다. 특히 GPU 메모리와 임시 저장소(ephemeral storage)에 대한 특별한 주목을 두어 메모리 요청(memory requests), 제한(limit), 서비스 품질(QoS) 클래스 및 퇴거 정책(eviction policies)을 다룹니다. 또한, 메모리 누수(memory leaks)나 임시 볼륨 소진(ephemeral volume exhaustion)과 같은 일반적인 함정에 대해서도 논의합니다.

- **Technical Details**: Kubernetes는 ML 워크로드의 메모리 관리에서 다양한 기술적 세부사항을 포함하고 있습니다. 메모리 요청과 제한은 QoS 클래스에 따라 다르며, 퇴거 정책은 자원의 제약이 있는 상황에서 메모리의 안정적인 사용을 보장하기 위해 중요합니다. 본 논문은 이러한 프로세스를 깊이 있게 탐구하며, GPU 메모리에 대한 구체적인 관리 방법도 제시합니다.

- **Performance Highlights**: 연구결과, 최적의 메모리 관리를 통해 ML 모델 훈련의 성능을 크게 향상시킬 수 있다는 것을 보여줍니다. 안정적이고 확장 가능한 메모리 활용을 위한 우수 사례(best practices)를 제공하여, ML 실무자들이 메모리 부족(out-of-memory) 이벤트를 방지하고 고성능 ML 훈련 파이프라인을 구축하는 데 기여할 수 있도록 합니다.
...
### Permutation recovery of spikes in noisy high-dimensional tensor estimation (https://arxiv.org/abs/2412.14650)
Comments:
          29 pages, 2 figures. arXiv admin note: substantial text overlap with arXiv:2408.06401

- **What's New**: 이번 연구는 높은 차원에서의 gradient flow 다이나믹스를 다루며, noisy Gaussian tensor 관찰을 통해 $r$ 개의 알려지지 않은 신호 벡터(스파이크)를 추정하는 문제를 다루고 있습니다. 이 연구는 최대 가능도 추정(maximum likelihood estimation, MLE) 절차를 최적화하는 과정에서 발생하는 고도로 비볼록(random function) 난제를 분석합니다. 특히, 신호 대 잡음비(signal-to-noise ratio, SNR) 간의 구분에 대한 가정 없이 gradient flow가 모든 스파이크를 효율적으로 회복할 수 있는 샘플 복잡도를 결정합니다.

- **Technical Details**: 스파이크 텐서 모델의 다중 스파이크(case) 설정을 위해, 최소한의 간단한 조건에서, 회복 과정의 기본 원리를 규명합니다. 연구는 $p \geq 3$, $r \geq 1$의 정수를 가지며, $M$ 개의 독립 동등 분포(i.i.d.) 관측을 통해 얻어지는 경우에서 $r$개의 신호 벡터를 추정하는 문제를 수학적으로 정형화합니다. 이 분석은 초기 조건이 알고리즘에 의해 회복된 스파이크의 순열을 결정하게 되는 방식으로 저차원 동적 시스템으로의 정량적 축소에 중점을 둡니다.

- **Performance Highlights**: 우리의 결과는 gradient flow의 경우 구분 조건 없이 MLE의 최적화 다이나믹스를 완전히 규명할 수 있음을 시사합니다. 이는 스파이크와 관련된 회복 문제를 더 간단하게 풀 수 있는 새로운 가능성을 보여주며, 기존의 Langevin 다이나믹스 결과와 포괄적으로 연결되는 중요한 발견입니다. 특히, 랜덤한 고차원 동적 시스템의 특성을 이해함으로써 더 복잡하고 예기치 않은 행동을 보일 수 있는 저차원 동적 시스템이 형성될 수 있음을 제안합니다.
...
### Fast inverse lithography based on a model-driven block stacking convolutional neural network (https://arxiv.org/abs/2412.14599)
Comments:
          21 pages, 7 figures

- **What's New**: 이번 논문에서는 Optical Proximity Correction (OPC) 분야의 새로운 접근 방식을 제안합니다. 전통적인 픽셀 기반 OPC 방법들의 제조 도전 과제를 해결하기 위해, 블록 스태킹 심층 학습 프레임워크를 활용하여 효율적으로 마스크를 생성하는 방법을 소개합니다.

- **Technical Details**: 제안된 방법은 벡터 리소그래피 모델링(vector lithography modelling)에 기초하여, 대규모 레이블이 부착된 데이터셋의 필요성을 제거하며 훈련 과정을 간소화합니다. 또한, wave function collapse 알고리즘을 사용하여 표적 패턴의 다양성을 증대시키며, 무작위로 다양한 마스크 패턴을 생성하여 마스크 기법의 범위를 크게 확장합니다.

- **Performance Highlights**: 수치 실험 결과는 제안된 엔드 투 엔드 접근 방식이 고급 OPC 리소그래피 맥락 내에서 마스크 복잡성을 효과적으로 관리하는 우수한 능력을 강조합니다. 이러한 발전은 실제 제조 환경에서 OPC 기술의 적용 가능성과 경제성을 향상시킬 것으로 기대됩니다.
...
### Accelerated Patient-Specific Calibration via Differentiable Hemodynamics Simulations (https://arxiv.org/abs/2412.14572)
- **What's New**: 이 연구는 개인화된 진단 절차를 위한 차별화된 0D-1D Navier-Stokes 감소 모델 기반 솔버를 제안합니다. 기존의 비효율적이거나 해석이 어려운 방법 대신, 이 접근 방식은 매개변수 추정(parameter inference) 및 민감도 분석(sensitivity analysis)의 효율성을 높입니다. 특히, 데이터로부터 충분한 정보를 추출하여 개인에게 특화된 모델링을 가능하게 합니다.

- **Technical Details**: 이 방법은 환자 고유의 혈관 컴플라이언스(vascular compliance) 및 이미지(imging)를 통해 측정할 수 있는 환자 기하학과 같은 매개변수를 고려합니다. JAX 라이브러리를 사용하여 작성된 혈류역학 솔버는 고속 실행과 더불어 변별성을 제공하며, 이는 모델이 제약을 받지 않고 병렬화(parallelization)할 수 있게 해줍니다. 이러한 접근은 데이터 병렬화를 통해 여러 환자에 대해 동시에 실행될 수 있습니다.

- **Performance Highlights**: 제안된 솔버는 여러 기하학적 환경에서의 기존 프로세스와 비교하여 성능이 검증되었습니다. 이 솔버를 통해 솔류션의 해석 가능성을 유지하면서도 매개변수 추정의 속도를 대폭 향상시킬 수 있습니다. 이를 통해 개인 맞춤형 의학 분야에서 진단과 치료의 정확성을 높일 수 있는 가능성을 제시합니다.
...
### Statistical Undersampling with Mutual Information and Support Points (https://arxiv.org/abs/2412.14527)
- **What's New**: 본 연구는 머신러닝에서의 클래스 불균형 문제를 해결하기 위해 두 가지 새로운 언더샘플링 기법을 제안합니다. 이 방법은 상호 정보 기반의 계층적 단순 무작위 샘플링(mutual information-based stratified simple random sampling)과 지원점 최적화(support points optimization)를 통해, 정보 손실을 최소화하면서 대표적인 데이터 선택에 우선순위를 둡니다. 다수의 실험 결과, 제안된 기법은 기존의 언더샘플링 기법보다 더 높은 균형 분류 정확도(balanced classification accuracy)를 기록하며, 통계적 개념과 머신러닝의 융합 가능성을 보여줍니다.

- **Technical Details**: 연구에서 상호 정보(mutual information)란 한 확률 변수의 정보를 다른 확률 변수를 통해 얻는 양을 의미합니다. 이를 통해 데이터 포인트의 그룹을 만들고, 유사한 데이터를 그룹화하여 계층적 단순 무작위 샘플링(SRS)을 수행하는 새로운 방법론이 소개됩니다. 본 연구는 K-평균 클러스터링(K-Means clustering)을 통해 각 데이터 포인트의 상호 정보를 계산하고 이를 기반으로 샘플링 과정을 최적화함으로써, 기존 언더샘플링 기법의 한계를 극복하고자 합니다.

- **Performance Highlights**: 제안된 두 가지 언더샘플링 기법은 기존 기법들과 비교하여 클래스 불균형 문제가 있는 데이터셋에서 개선된 성능을 보였습니다. 특히, 상호 정보 및 지원점을 활용한 접근 방식은 소수 클래스 인식의 향상을 가져오며, 정보 손실을 최소화하여 처리 효율성을 유지합니다. 이 연구는 특히 의료 진단 및 사기 탐지 같은 고위험 분야에서 보다 정확한 예측을 가능하게 합니다.



### Dynamic User Interface Generation for Enhanced Human-Computer Interaction Using Variational Autoencoders (https://arxiv.org/abs/2412.14521)
- **What's New**: 이 연구에서는 변이 오토인코더(variational autoencoder, VAE) 모델에 기반한 지능형 사용자 인터페이스(User Interaction Interface) 생성 및 최적화를 위한 새로운 접근 방식을 제시합니다. 기존의 전통적인 인터페이스 디자인 방법론이 다양성과 개인화의 요구를 충족하는 데 한계를 겪는 상황에서, HCI(Human-Computer Interaction)의 중요성이 강조되고 있습니다. 본 연구는 RICO 데이터셋을 활용하여 사용자 미적 감각과 상호작용 습관에 맞는 인터페이스를 생성하고 시뮬레이션하는 데 중점을 두고 있습니다.

- **Technical Details**: 본 연구는 VAE 모델을 활용하여 실시간 사용자 행동 데이터를 통합하여 사용자 인터페이스를 동적으로 개선하고 최적화합니다. 이를 통해 인터페이스의 사용성을 높이고, HCI의 중요성을 강조합니다. 특히 연구는 오토인코더(Autoencoder, AE), 생성적 적대 신경망(Generative Adversarial Networks, GAN), 조건부 GAN(Conditional GAN, cGAN), 깊은 신념망(Deep Belief Network, DBN) 및 VAE-GAN과 같은 다른 방법들과 비교하여 VAE 기반 접근 방식의 우수성을 제공합니다.

- **Performance Highlights**: 실험 결과에 따르면, VAE 기반 접근 방식이 인터페이스 생성의 품질과 정확성을 크게 향상시키는 것으로 나타났습니다. 이러한 성과는 자동화된 인터페이스 생성과 사용자 경험 최적화에 대한 강력한 기술적 솔루션을 제공하여 HCI에 대한 귀중한 통찰력을 제시합니다. 이 결과는 지능형 기술의 발전에 따라 점점 더 중요해지는 사용자 인터페이스 디자인에 대한 새로운 비전을 제공합니다.



### Guided Diffusion Model for Sensor Data Obfuscation (https://arxiv.org/abs/2412.14499)
- **What's New**: IoT(Internet of Things) 장치에 의해 수집된 센서 데이터는 주변 개인에 대한 상세 정보를 담고 있지만, 이러한 데이터의 공유는 개인의 사생활을 침해할 수 있는 잠재적 위험이 있습니다. 이 논문에서는 프라이버시를 보호하기 위해 Generative 모델을 활용한 새로운 데이터 난독화 기법인 PrivDiffuser를 제안합니다. PrivDiffuser는 데이터의 유용성과 프라이버시를 균형 있게 유지하면서 개인 정보를 가릴 수 있는 뛰어난 기술입니다.

- **Technical Details**: PrivDiffuser는 Denoising Diffusion 모델을 기반으로 하여 센서 데이터의 공공 속성과 개인 속성 간의 정보 얽힘을 줄이기 위해 상호 정보 기반의 정규화를 적용합니다. 이 과정에서 생성된 데이터는 원래 데이터와 유사한 속성을 가짐으로써 기존의 애플리케이션과 원활하게 통합될 수 있습니다. 개별 사용자들은 모델을 재훈련하지 않고도 다양한 프라이버시 요구에 맞춰 자신의 개인 속성을 보호할 수 있습니다.

- **Performance Highlights**: 실제 데이터 세트에서 PrivDiffuser의 성능을 평가한 결과, 기존의 최첨단 GAN 기반 난독화 모델에 비해 프라이버시와 유용성 모두에서 더 나은 결과를 보여주었습니다. 구체적으로, 유용성 손실을 최대 1.81% 감소시키고, 프라이버시 손실을 최대 3.42% 감소시켰습니다. 이는 PrivDiffuser가 인공지능 기반의 새로운 솔루션으로서 IoT 시스템에서의 프라이버시 보호에 중요한 기여를 할 수 있음을 의미합니다.
...
### MAIDS: Malicious Agent Identification-based Data Security Model for Cloud Environments (https://arxiv.org/abs/2412.14490)
Comments:
          28 pages, 10 figures

- **What's New**: 이 논문은 클라우드 환경에서의 데이터 보호를 위해 MAIDS (Malicious Agent Identification-based Data Security) 모델을 제안합니다. 이 모델은 XGBoost 기계 학습(classification algorithm)을 활용하여 데이터 공유 및 저장 중 악의적인 접근을 사전에 예방합니다. 기존의 접근 방식과 달리, MAIDS는 데이터 유출 후 악의적인 에이전트를 식별하는 것이 아닌, 데이터 접근에 대한 자격을 검토하여 미리 악의적인 에이전트를 식별하는 방식으로 보호합니다.

- **Technical Details**: MAIDS 모델은 XGBoost 분류 알고리즘을 사용하여 클라우드 시스템 내 여러 참여 엔티티 간의 데이터 할당 및 통신을 안전하게 보호합니다. 이 모델은 온라인 데이터 통신이나 거래와 관련된 여러 보안 매개 변수(security parameters)를 탐색하고 계산합니다. 또한, 이 논문에서는 XGBoost 분류기를 기반으로 한 악의적인 에이전트 예측(XC-MAP) 유닛을 개발하기 위한 보안 중심의 지식 데이터베이스를 만들어 제공합니다.

- **Performance Highlights**: 제안된 MAIDS 모델은 허가된 에이전트에게만 데이터 접근을 부여하여 의도적 또는 비의도적인 데이터 유출로부터 데이터를 보호하는 포괄적인 솔루션을 제공합니다. 에이전트의 행동을 평가하고 악의적인 에이전트를 예측하여 데이터 접근을 관리함으로써, 클라우드 환경에서의 데이터 보안을 보다 강력하게 강화합니다. 이 모형은 데이터 보호와 악의적인 에이전트 예측을 효과적으로 결합하여, 데이터 공유의 안전성을 높이는데 기여합니다.
...
### Fingerprinting Codes Meet Geometry: Improved Lower Bounds for Private Query Release and Adaptive Data Analysis (https://arxiv.org/abs/2412.14396)
Comments:
          Abstract slightly shortened to meet the arXiv requirement; 50 Pages and 1 Figure

- **What's New**: 이 논문에서는 differential privacy(차별적인 개인 정보 보호)에서 fingerprinting codes(지문 인식 코드)의 일반적인 하한을 증명할 수 있는 새로운 프레임워크를 제안합니다. 이는 쿼리 세트의 기하학적 성질에 맞게 기법을 조정할 수 있는 가능성을 제공합니다. 특히, 이 프레임워크는 이전에 알려진 하한을 개선하여 적응형 카운팅 쿼리에 대한 샘플 복잡도를 명확하게 보여줍니다.

- **Technical Details**: 이 연구에서는 fingerprinting codes를 활용하여 차별적 개인 정보 보호 하의 카운팅 쿼리 문제를 해결하는 새로운 추상적 프레임워크를 제시합니다. 이 기법의 두 가지 주요 기술적 요소는 임의의 점 집합에 대한 분포 가족을 정의하기 위한 exponential tilt(지수적 기울기)를 사용하는 것과, 적절한 '스코어' 조절을 위해 Stokes' theorem을 활용하는 것입니다. 이러한 접근 방식은 이전의 많은 연구들이 직육면체에서 '파라미터 벡터'를 가져오는 것과 달리 보다 일반화된 구조를 제공합니다.

- **Performance Highlights**: 새로운 프레임워크를 통해, Q개의 적응형 카운팅 쿼리에 대해 정확성 α을 달성하려면 최소 Ω(√log |𝓧| * log Q / α^3)의 샘플이 필요하다는 것을 보여줍니다. 이 하한은 기존의 upper bounds에 일치하며, 기존에 알려진 하한인 log Q / α^2와 min(√Q, √log |𝓧|)/α^2를 크게 개선합니다. 또한 이 프레임워크는 약한 차별적 개인 정보 보호 아래에서 임의의 0-1 쿼리 집합에 대한 샘플 복잡도를 정량화합니다.



### Nemesis: Noise-randomized Encryption with Modular Efficiency and Secure Integration in Machine Learning Systems (https://arxiv.org/abs/2412.14392)
- **What's New**: 이번 연구에서는 Fully Homomorphic Encryption (FHE) 기반 시스템의 효율성을 높이기 위해 	extit{Nemesis}라는 프레임워크를 제안합니다. Nemesis는 기존 Rache의 캐싱 메커니즘을 발전시켜, 다수의 평문 구조를 지원하며, 높은 성능을 유지하면서도 보안성을 손상시키지 않는 연산이 가능합니다. 또한, 다양한 공개 데이터 세트에서 성능을 평가하여, 개인정보 보호 기술의 보급을 촉진할 수 있는 방법을 제시했습니다.

- **Technical Details**: Nemesis 프레임워크는 사전 계산(precomputation), 재구성(reconstruction), 무작위화(randomization)의 세 가지 핵심 구성 요소를 기반으로 설계되었습니다. 사전 계산 단계에서는 재사용할 수 있는 암호화된 스칼라 값을 생성하여 중복 계산을 최소화하며, 재구성 과정에서는 다수의 평문을 다항식으로 효율적으로 인코딩하여 조작할 수 있도록 합니다. 무작위화 과정은 암호문에 통제된 노이즈를 추가하여 보안성과 효율성을 균형 있게 제공합니다.

- **Performance Highlights**: Nemesis는 다양한 데이터 세트인 MNIST, FashionMNIST, CIFAR-10에서 광범위한 실험을 통해 성능을 평가했습니다. 실험 결과는 Nemesis가 FHE 기반 머신 러닝 시스템의 계산 오버헤드를 크게 줄여, 개인정보 보호 기술의 보다 광범위한 채택을 위한 기반을 마련함을 보여주었습니다. 이를 통해 Nemesis는 실제 응용 프로그램에서 FHE의 효율성을 크게 향상시키는 가능성을 제시합니다.
...
### Scaling Deep Learning Training with MPMD Pipeline Parallelism (https://arxiv.org/abs/2412.14374)
- **What's New**: JaxPP는 대규모 딥러닝 모델의 훈련을 효과적으로 확장할 수 있는 새로운 시스템으로, 유연한 pipeline parallelism을 구현합니다. 이 시스템은 gradient accumulation을 위해 사용자 정의 pipeline 일정을 쉽게 구현할 수 있는 프로그래밍 모델을 제공합니다. JaxPP는 작업을 클러스터 노드에 자동으로 분배하고, 노드 간의 통신을 자동으로 유추하여 하드웨어 활용도를 최대 $1.11	imes$까지 개선합니다.

- **Technical Details**: JaxPP는 GSPMD 프로그래밍 모델을 기반으로 하여, 사용자 정의 스케줄을 통해 pipeline parallelism을 통합할 수 있는 새로운 접근 방식을 도입합니다. 이 시스템은 사용자에게 (비연속적인) 통신 처리를 자동으로 관리하도록∙ 설계되어 있으며, 작업 그래프를 통해 배포된 장치에서 작업을 스케줄링하고 자원 관리를 수행합니다. 또한, JaxPP는 고속 링크가 필요하지 않은 환경에서도 효율적으로 동작할 수 있도록 설계되었습니다.

- **Performance Highlights**: JaxPP는 기존의 SPMD 모델과 비교하여 파이프라인 스케줄에서 후보로 제안된 다양한 MPMD 분산 데이터 흐름을 지원합니다. 이를 통해 데이터와 파이프라인 병렬성을 결합한 최적의 훈련 구성을 제공하며, 실용적인 대규모 훈련 벤치마크에서 성능을 기준으로 기존의 최신 시스템들과 비교하여 그 강점을 입증하였습니다. JaxPP는 사용자 정의 가능한 스케줄을 통해 다양한 병렬 전략을 효과적으로 적용할 수 있는 가능성을 제시합니다.
...
### Implementing TD3 to train a Neural Network to fly a Quadcopter through an FPV Ga (https://arxiv.org/abs/2412.14367)
- **What's New**: 이 연구에서는 Twin Delayed Deep Deterministic Policy Gradient (TD3) 알고리즘을 활용하여 쿼드콥터의 속도 제어기를 훈련시키는 새로운 접근 방식을 제안합니다. 쿼드콥터는 FPV 드론 레이싱 게이트를 통과하는 것을 목표로 하며, 훈련된 정책은 실제 환경에서 시연됩니다. 이는 기존의 기존 제어 방식들이 복잡한 환경에서의 성능 저하를 보완해 줄 수 있습니다.

- **Technical Details**: 논문에서는 Gymnasium을 기반으로 쿼드콥터의 동작을 시뮬레이션하는 환경을 구축하였습니다. 여기서 쿼드콥터는 X, Y, Z 위치와 Yaw, 속도 등의 변수를 입력으로 받아 최적의 속도 명령을 출력합니다. 스토캐스틱 환경 속성에 따라 불확실성을 모델링하기 위해 특정 확률 분포에서 샘플링하여 제어기가 훈련됩니다.

- **Performance Highlights**: 훈련된 정책은 현실 세계의 쿼드콥터에 배포하여 테스트되었으며, 조종기가 가상 게이트를 성공적으로 통과하는 성과를 보였습니다. 이 연구 결과는 드론 비행 관련 작업을 수행하는 데 있어 Deep Reinforcement Learning의 정밀한 적용 사례로 주목받고 있습니다. 향후 연구는 다양한 환경에서의 성능을 개선하기 위한 알고리즘 최적화를 포함할 수 있습니다.



### On the Robustness of Spectral Algorithms for Semirandom Stochastic Block Models (https://arxiv.org/abs/2412.14315)
Comments:
          45 pages. NeurIPS 2024

- **What's New**: 이 논문은 그래프 이분할 문제( graph bisection problem)에 대한 스펙트럴 클러스터링(spectral clustering) 알고리즘의 강건성을 다루고 있습니다. 스펙트럴 알고리즘은 특정 확률 모델에서 클러스터 구조를 복구할 수 있지만, 모델 오분류에 대해 강건하지 않은 것으로 알려져 있습니다. 저자들은 세미랜덤 적대자(semi-random adversaries)의 영향을 고려하여 이 알고리즘의 강건성을 연구하고, 이를 통해 비정상화된 라플라시안(unnormalized Laplacian)을 사용하여 강한 일관성을 달성할 수 있는 조건을 식별합니다.

- **Technical Details**: 그래프 G의 라플라시안과 관련된 두 번째 가장 작은 고유값(eigenvalue)에 해당하는 고유벡터를 사용하여 커뮤니티 할당을 예측하는 스펙트럴 클러스터링 알고리즘에 대한 연구가 진행되었습니다. 세미랜덤 적대자들은 클러스터 내부에 엣지를 추가할 수 있으며, 이러한 방식으로 모델 사양을 변경할 수 있습니다. 논문에서는 비정상화된 라플라시안이 네트워크의 진정한 클러스터 구조를 정확하게 회복하는 조건을 제시하는 한편, 정규화된 라플라시안을 사용할 경우 클러스터에서 상수 비율의 오류를 생성하는 문제도 다루고 있습니다.

- **Performance Highlights**: 수치 실험(numerical experiments)을 통해 이론적인 결과를 보완하고 확인할 수 있었습니다. 비정상화된 라플라시안을 사용할 경우, 기존의 스펙트럴 클러스터링 알고리즘보다 강한 일관성을 보이는 특정 세미랜덤 적대자 클래스가 식별되었습니다. 반면, 정규화된 라플라시안을 사용할 경우에는 예측 정확성이 감소하는 현상이 관찰되었습니다.



### Reinforcement Learning from Automatic Feedback for High-Quality Unit Test Generation (https://arxiv.org/abs/2412.14308)
Comments:
          Accepted to DeepTest 2025 (ICSE Workshop). arXiv admin note: text overlap with arXiv:2310.02368

- **What's New**: 이 논문에서는 Reinforcement Learning from Static Quality Metrics (RLSQM)이라는 새로운 방법론을 제안합니다. 기존의 Large Language Models (LLMs)에서 발생하는 테스트 품질 문제를 해결하기 위해, 정적 분석을 기반으로 한 품질 지표를 활용하여 고품질의 유닛 테스트를 생성할 수 있도록 합니다. 실험 결과, RL 최적화된 Codex 모델은 LLM의 기본 모델보다 최대 23%까지 품질 지표가 향상되었음을 보여주었습니다.

- **Technical Details**: 훈련 절차에서는 데이터셋을 SFT, RM, PM 훈련을 위해 세 부분으로 나누며, RM 훈련을 위해 10,000개의 프롬프트/완료 쌍을 무작위 샘플링했습니다. 정책 모델의 과적합을 방지하기 위한 검증 절차가 중요한데, 메소드의 초점에 따라 생성된 테스트를 비교하여 품질을 평가합니다. RM 훈련 데이터의 불균형 문제를 해결하기 위해 대다수 품질 속성을 무작위로 언더샘플링하여 최종 테스트 품질을 개선합니다.

- **Performance Highlights**: RLSQM은 Best Practices 및 Test Smells의 훈련에서 가장 뚜렷한 향상을 보였으며, Assertions의 빈도가 92.8%에 달하고 Conditional/Exception 테스트는 0.16%까지 감소했습니다. RL 조정 후 모든 RLSQM 모델의 구문적 정확성은 97-99.5%에 도달하였으며, 테스트 품질 향상에서도 큰 성과를 나타냈습니다. 모델 간 상관관계로 인해 특정 속성을 최적화할 때 다른 속성에 대한 성능 저하가 발생할 수 있지만, 본 연구는 향후 이러한 문제를 다룰 가능성을 열어줍니다.
...
Comments:
          Accepted to ICSE 2025 research track. Camera-ready version

- **What's New**: 이 논문은 DeepVulGuard라는 새로운 IDE 통합 툴의 첫 경험적 연구를 소개하며, 소프트웨어 개발자들이 실제 프로젝트에서 이 툴을 사용한 결과를 제시합니다. DeepVulGuard는 코드의 보안 취약점을 탐지하고 수리할 수 있는 최신 모델에 기반한 도구로, 마이크로소프트의 17명의 전문 개발자들이 사용한 경험을 포함합니다. 이 연구는 모델의 사용 가능성과 현업에서의 효용성을 깊이 있게 평가하는 데 중점을 두고 있습니다.

- **Technical Details**: DeepVulGuard는 Visual Studio Code (VSCode)와 통합된 확장 프로그램으로, CodeBERT와 GPT-4와 같은 최신 딥러닝 모델을 활용하여 취약점 탐지 및 수정 작업을 수행합니다. 연구 참가자들은 총 24개 프로젝트에 대해 코드 파일 6.9천 개, 170개의 알림, 50개의 수정 제안을 생성하여 실질적인 데이터를 안전하게 수집하였습니다. 이 툴은 탐지 작업에서 80%의 정밀도, 32%의 재현율, 46%의 F1 점수를 달성했습니다.

- **Performance Highlights**: 사용자 연구에서 59%의 참가자가 DeepVulGuard의 향후 사용에 관심을 표명했지만, 높은 허위 경고와 부적절한 수정 제안 등 여러 문제로 인해 실용성이 제한적임을 발견했습니다. 특히 75%의 제안된 수정이 사용자의 코드 베이스에 맞지 않아 수정 없이 사용하기 어려웠습니다. 툴은 인터랙티브한 채팅 방법을 제공하여 개발자가 더 적합한 수정 방향으로 안내할 수 있는 가능성을 보여주었습니다.
...
### Projected gradient methods for nonconvex and stochastic optimization: new complexities and auto-conditioned stepsizes (https://arxiv.org/abs/2412.14291)
- **What's New**: 본 논문에서는 함수가 부드럽지만 반드시 볼록(Convex)하지 않을 수 있는 문제를 해결하기 위한 새로운 projected gradient (PG) 방법을 제시합니다. 전통적인 '바닐라' PG 방법의 최적 반복 복잡도(Iteration Complexity)를 제공하고, 자동 조건화(auto-conditioned)된 변형인 AC-PG를 개발하여 Lipschitz 상수(Lipschitz constant)의 입력 없이도 동일한 복잡도를 달성합니다. 이 접근법은 이전 반복에서 수집된 정보를 바탕으로 Lipschitz 상수를 추정하며, 이의 과소 추정에서 발생하는 오류를 적절히 제어할 수 있음을 보여줍니다.

- **Technical Details**: 이 연구는 경량의 비선형 최적화 문제를 다루며, 특히 일반 비볼록(nonconvex) 최적화 문제에 적합한 첫 번째 방법들을 제안합니다. PG 방법은 확률적 설정(Stochastic setting)으로 확장되어, stochastic projected gradient (SPG) 방법과 분산 저감(stochastic gradient) 방법인 VR-SPG를 제안합니다. 이러한 방법들은 다양한 oracle 설정(oracle settings)에서 새로운 복잡도 경계를 이루며, 첫 번째 주문 정보(First-order information)를 활용합니다.

- **Performance Highlights**: 연구진은 두 가지 stochastic PG 방법에 대한 자동 조건부 단계 크기 정책(auto-conditioned stepsize policies)을 제시하고, 이의 수렴 보장(Convergence guarantees)을 확인합니다. 제안된 방법들은 현상적인 성능 변화를 보여주며, 기존 방법들보다 더 향상된 복잡도를 제공하여 실제 비선형 프로그래밍 문제를 더 효과적으로 해결할 수 있는 가능성을 보여줍니다. 이를 통해 볼록성과 비볼록성을 통합적으로 처리할 수 있는 새로운 접근법이 제시되었습니다.
...
### Detecting Dark Patterns in User Interfaces Using Logistic Regression and Bag-of-Words Representation (https://arxiv.org/abs/2412.14187)
- **What's New**: 이 논문은 사용자 인터페이스에서의 다크 패턴(dark patterns)을 탐지하기 위한 새로운 접근 방식을 제안합니다. 기존의 방식들과는 달리, 이 방법은 로지스틱 회귀(logistic regression)와 단어 가방(bag-of-words) 표현을 사용하여 데이터의 다양한 표본을 수집하고 분석하는 데 중점을 두었습니다. 이를 통해 사용자의 행동을 조작하는 설계 관행을 명확히 드러내고, 이러한 패턴의 탐지 및 완화가 어떻게 투명성과 신뢰를 증진시키는지에 대한 통찰을 제공합니다.

- **Technical Details**: 제안된 방법론은 사용자 인터페이스 텍스트 샘플의 다양성을 고려하여 데이터 세트를 수집하는 것부터 시작합니다. 수집된 데이터는 전처리(preprocessing), 텍스트 특징 추출의 단계가 포함되며, 여기서 단어 가방 표현을 활용합니다. 이후 로지스틱 회귀 모델을 훈련시키고 정확도(accuracy), 정밀도(precision), 재현율(recall), F1-score, ROC 곡선 아래 면적(AUC)과 같은 다양한 성과 지표를 사용하여 평가하게 됩니다.

- **Performance Highlights**: 실험 결과는 제안된 접근 방식이 다크 패턴을 정확하게 식별하는 데 효과적이라는 것을 보여줍니다. 이 모델은 데이터 세트 구성 및 모델 매개변수의 변화에 대해 높은 예측 성능과 강인성을 보였습니다. 이러한 연구에서 얻어진 통찰력은 디지털 환경에서의 윤리적 디자인 관행을 촉진하고 사용자 권리를 보호하기 위한 실용적인 시사점을 제공합니다.
...
### On Convex Optimal Value Functions For POSGs (https://arxiv.org/abs/2311.09459)
Comments:
          Currently under review at JAIR

- **What's New**: 본 논문은 Multi-agent planning과 reinforcement learning에서 발생할 수 있는 문제를 다룹니다. 특히, 에이전트들이 상태를 서로 볼 수 없거나 소통하지 못하는 경우에 대한 새로운 접근법을 제시합니다. 이를 통해 Partially Observable Stochastic Games (POSGs)의 수학적 구조를 개선하고자 하며, 신뢰할 수 있는 제3자의 관점에서 모형을 재구성합니다.

- **Technical Details**: 이 연구는 원래의 POSG를 에이전트를 대신하여 동시에 계획하는 신뢰할 수 있는 제3자의 시각에서 재편성합니다. 이를 통해 상태는 occupancy states로 전환되며, 이는 세계의 숨겨진 상태에 대한 후방 확률 분포(posterior probability distributions)로 해석됩니다. 또한, 이 논문은 zero-sum, common-payoff 및 Stackelberg POSGs에서 최적의 상태 가치 함수(optimal state-value function)가 점근적인 basis에서 볼록 함수(convex function)임을 증명합니다.

- **Performance Highlights**: 연구 결과, 제안된 접근법을 통해 POSGs에 대한 계획과 강화 학습 알고리즘의 효율성이 향상됨을 보여주었습니다. 이는 에이전트들이 보다 높은 성능을 발휘할 수 있도록 돕는다는 점에서 의미가 있습니다. 따라서, 이 방법론은 Multi-agent 시스템에서의 실제 적용 가능성을 높이며, 향후 연구의 길을 제시합니다.
...
- **What's New**: 본 논문에서는 Target-Domain Structural Smoothing (TDSS)라는 새로운 접근 방식을 도입하여 비지도 그래프 도메인 적응(UGDA)의 문제를 해결합니다. 이는 기존 접근 방식들이 간과한 구조적 변화에 대한 민감성을 반영하여, GNN(Graph Neural Networks)의 구조적 분포 이동을 완화합니다. TDSS는 target graph에 직접적으로 구조적 스무딩을 적용하여 노드 표현의 일관성을 확보하고, 복잡한 구조에 대한 적응능력을 한층 강화합니다.

- **Technical Details**: TDSS는 이웃 샘플링(neighborhood sampling) 전략과 결합하여 구조적 스무딩을 행합니다. 이는 노드 간의 주요 차이를 유지하며, 과도한 스무딩(over-smoothing)의 위험을 줄입니다. 이 방법은 모델의 일관성을 높이고, GNN의 로컬 구조 변동을 완화하여 노드 임베딩(node embeddings)의 효과적인 전이를 보장합니다.

- **Performance Highlights**: 세 가지 실제 데이터셋에서의 실험 결과, TDSS는 최신의 최상위 기준치를 능가하며 여섯 가지 전이 시나리오에서 모든 일관된 성능 향상을 보여줍니다. TDSS는 간단하면서도 기존의 프레임워크에 쉽게 통합될 수 있는 효과적인 방법으로, 다양한 그래프 설정에서 그 적응성과 효율성을 입증했습니다.

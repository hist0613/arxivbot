...
### Task Arithmetic Through The Lens Of One-Shot Federated Learning (https://arxiv.org/abs/2411.18607)
- **What's New**: 이 논문에서는 Task Arithmetic와 Federated Learning(FedAvg) 간의 수학적 동등성을 제시하여, Task Arithmetic의 이론적 이해를 확장하는 새로운 관점을 제공합니다. Federated Learning은 데이터를 교환하지 않고 여러 장치에서 공동으로 모델을 학습하는 분산 머신러닝 패러다임입니다. 이러한 관점을 통해 Task Arithmetic의 성능을 발휘할 수 있는 두 가지 주요 요인인 데이터 이질성과 훈련 이질성에 대한 분석을 제시합니다.

- **Technical Details**: Task Arithmetic는 여러 모델의 능력을 단순한 산술 연산을 통해 통합하는 기법입니다. 각 세부 모델의 성능 향상을 위해 사전 훈련된 모델 파라미터와 파인 튜닝된 모델 파라미터 간의 차이를 기반으로 하는 작업 벡터를 생성합니다. 이 작업 벡터들을 합치면 다중 작업에서 성능을 높일 수 있는 방향이 형성됩니다. 이 연구에서는 Task Arithmetic와 FedAvg 간의 상관관계를 정립하고, 이를 통해 알고리즘을 최적화하는 방법을 제시합니다.

- **Performance Highlights**: 실험 결과, Federated Learning 알고리즘을 Task Arithmetic에 적용했을 때 모델 통합 성능이 현저히 개선됨을 확인하였습니다. 특히, 데이터 이질성과 훈련 이질성 문제를 해결하기 위한 알고리즘을 설계하여 Task Arithmetic의 효용성을 높였습니다. 이러한 결과들은 Task Arithmetic와 Federated Learning의 통합이 실용적 방법론에도 크게 기여할 수 있음을 보여줍니다.
...
### Pruning Deep Convolutional Neural Network Using Conditional Mutual Information (https://arxiv.org/abs/2411.18578)
- **What's New**: 이 연구에서는 Convolutional Neural Networks (CNNs)에서 모델 사이즈를 줄이면서도 정확도를 유지할 수 있는 Structured Filter-Pruning 접근법을 제안합니다. 본 방법은 각 레이어에서 가장 정보를 많이 가진 특징을 식별하고 선택적으로 보존하고, Conditional Mutual Information (CMI) 값을 활용하여 특징 맵의 중요도를 평가합니다. 새로운 CMI 형식을 도입해 레이어 간의 정보를 공유하면서, 보다 효율적인 알고리즘으로 모델을 대부분 비어 있는 쪽으로 정리할 수 있게 하였습니다.

- **Technical Details**: 제안된 방법은 Renyi {
 \alpha}-order entropy 수치적 방법을 사용하여 CMI 값을 계산하고, 이를 통해 필터 지우기 기준을 설정하여 네트워크를 정리합니다. CMI 값이 중요하지 않은 특징을 줄이는 컷오프 포인트를 결정하는 다양한 전략이 개발되었습니다. 이러한 방법은 모델 사이즈를 줄이면서도 정확도를 유지하는 데 집중하고 있으며, VGG16 아키텍처에서 CIFAR-10 데이터셋을 활용한 테스트에서 26.83%의 파라미터 감소와 36.15%의 필터 감소를 달성했습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 테스트 정확도에서 0.32%의 최소 감소로 모델의 성능을 유지하였습니다. 이는 필터 수를 1/3 이상 줄이면서도 부담 없는 정확도를 자랑합니다. 연구팀은 이러한 방식이 자원 제약이 있는 하드웨어에서도 효과적일 것이라고 기대하고 있습니다.



### Concentration of Cumulative Reward in Markov Decision Processes (https://arxiv.org/abs/2411.18551)
Comments:
          60 pages

- **What's New**: 이 논문에서는 마르코프 결정 프로세스(Markov Decision Processes, MDPs)에서 누적 보상의 집중성 특성을 조사합니다. 특히, 무한 수평 평균 보상, 무한 수평 할인 보상 및 유한 수평 설정에서의 집중성을 기술하는 통합 접근 방식을 제안합니다. 또한 두 가지 주요 함의를 분석하며, 학습 정책에 대한 후회(regret) 정의의 개념적 차이를 명확히 합니다.

- **Technical Details**: 논문에서는 비대칭 및 비대칭 집중 결과를 포함하여 LLN(법칙), CLT(중심 극한 정리), LIL(반복 로그의 법칙)과 같은 결과를 다룹니다. 독창적인 마르틴게일 분해 기법을 바탕으로 보상의 통계적 분포를 분석하며, Azuma-Hoeffding 유형의 부등식과 비대칭 반복 로그법칙의 적용을 포함한 비대칭 집중 경계도 제시합니다. 이러한 경계는 MDP에서의 누적 보상에 대한 분석을 가능하게 합니다.

- **Performance Highlights**: 이 연구의 주요 기여 중 하나는 유한 수평 설정에서의 누적 보상에 대한 비대칭 집중 특성을 확립하고, 이는 고전적 결과에 대한 보강으로 작용합니다. 두 개의 정량적 후회 정의 간의 차이를 조사하여, 학습 알고리즘의 후회가 시간의 함수로서 어떻게 상호 관계를 가지는지를 입증합니다. 이 연구 결과를 통해 강화 학습 알고리즘의 성능을 개선하는 데 중요한 통찰을 제공할 수 있습니다.



### Perturbation Ontology based Graph Attention Networks (https://arxiv.org/abs/2411.18520)
- **What's New**: 이번 논문에서는 복잡한 그래프에서 의미론적 기본 요소로 온톨로지(ontology)를 도입하여 기존의 메타 경로 기반(meta-path-based) 및 인접 행렬 기반(adjacent matrix-based) 접근 방식을 통합한 Perturbation Ontology-based Graph Attention Networks (POGAT)라는 새로운 방법론을 제안합니다. POGAT는 온톨로지 서브그래프(ontology subgraphs)와 고급 자기 지도 학습(self-supervised learning) 패러다임을 결합하여 보다 깊은 맥락적 이해를 도모합니다.

- **Technical Details**: POGAT의 핵심 혁신은 모델이 미세한 맥락적 특징을 더 철저하게 탐색할 수 있도록 강화된 동질적인 변형(perturbation) 방식을 채택하여 강력한 부정 샘플(negative samples)을 생성하는 데 있습니다. 또한, 온톨로지 기반의 HIN(heterogeneous information networks) 표현 학습을 통해 온톨로지 서브그래프를 활용하여 노드의 맥락 표현을 개선하도록 설계되어 있습니다.

- **Performance Highlights**: 실험 결과 POGAT는 링크 예측(link prediction)과 노드 분류(node classification) 과제에서 최첨단 성능을 보여주며, F1-score에서 최대 10.78%의 혁신적인 향상을 달성했습니다. 또한, Micro-F1 점수에서 12.01%의 향상률을 기록하며 기존 기법들을 능가했습니다.
...
### SPTTE: A Spatiotemporal Probabilistic Framework for Travel Time Estimation (https://arxiv.org/abs/2411.18484)
- **What's New**: 이번 논문에서는 정확한 여행 시간 추정을 위한 새로운 프레임워크 SPTTE(Spatiotemporal Probability model for Travel Time Estimation)를 제안합니다. 기존 연구의 한계를 극복하기 위해 SPTTE는 다중 여행 시간 분포의 변화를 모델링하며, 이를 공간적 시간적 불확실성 문제로 설정합니다. 이 모델은 RNN 기반의 템포럴 가우시안 프로세스를 활용하여 드문 관찰을 정규화하고 시간적 종속성을 캡처합니다.

- **Technical Details**: SPTTE는 드문 관찰 문제를 해결하기 위해 공간적 시간적 확률론적 프레임워크를 채택하여 다중 여행 시간의 공동 분포를 모델링합니다. 이를 위해 RNN 기반의 가우시안 프로세스 파라미터화를 사용하여 여행 시간 분포의 시간적 의존성을 포착하며, 비정상적인 여행 분포로 인한 불신뢰 학습을 수정하기 위한 사전 기반의 이질성 스무딩 전략도 개발했습니다. 이러한 접근 방식은 정적 모델인 ProbETA의 한계를 극복하는 데 기여합니다.

- **Performance Highlights**: 실제 데이터 세트에 대한 평가 결과 SPTTE는 기존의 결정론적 및 확률적 방법보다 10.13% 이상 향상된 성능을 보여주었습니다. 또한, ablation study와 시각적 분석을 통해 모델 구성 요소의 효과가 확인되었습니다. 연구 결과는 고속도로와 같은 복잡하고 변화하는 교통 환경에서 더 정확하고 맥락에 맞는 예측을 가능하게 합니다.



### What do physics-informed DeepONets learn? Understanding and improving training for scientific computing applications (https://arxiv.org/abs/2411.18459)
- **What's New**: 본 연구에서는 물리 정보를 활용한 딥 오퍼레이터 네트워크(Physics-informed DeepONets)의 학습 내용을 분석하고, 추출된 기저 함수의 보편성을 평가하여 스펙트럴 방법을 통한 모델 축소 가능성을 입증하고자 합니다. 또한, 파라미터간의 전이 학습(transfer learning) 접근 방식을 제안하여 동일한 PDE의 파라미터들 사이 및 서로 관련된 다른 PDE 간의 학습 성능을 개선합니다.

- **Technical Details**: 이 논문에서는 시간에 의존하는 PDE와 초기 및 경계 조건을 고려하며, 연속적이고 비선형적인 솔루션 연산자(solution operator)를 학습하는 목표를 가지고 있습니다. 이를 위해 DeepONet 구조를 활용하며, 네트워크는 입력 함수 공간과 출력 함수의 도메인을 인코딩하는 두 개의 깊은 신경망(DNN)으로 구성됩니다. 자동 미분(automatic differentiation)을 이용하여 물리적 법칙을 강제하는 훈련 절차를 수립하고 있으며, 이는 기존의 훈련 방식보다 효율적인 결과를 도출합니다.

- **Performance Highlights**: 제안된 전이 학습 접근 방식은 여러 PDE에서 효과적으로 학습할 수 있도록 도와주며, 이로 인해 오류 감소가 크게 이루어졌습니다. 딥 오퍼레이터 네트워크는 보다 효과적인 기저 함수 학습을 통해 PDE의 솔루션 표현 능력을 향상시킵니다. 결과적으로 물리 정보를 활용한 딥 오퍼레이터 네트워크가 전통적인 과학 컴퓨팅 방법을 개선할 잠재력이 있음을 입증하고 있습니다.
...
### Advancements in Myocardial Infarction Detection and Classification Using Wearable Devices: A Comprehensive Review (https://arxiv.org/abs/2411.18451)
- **What's New**: 심근경색증(가슴 통증)은 심장으로 가는 혈류가 제한되어 발생하는 심각한 건강 상태입니다. 이 논문은 지속적인 ECG 모니터링을 통한 조기 발견의 중요성을 강조하며, 웨어러블 장치에서의 MI 분류 방법론의 발전을 다룹니다. 기존 기술들인 morphology filtering과 wavelet decomposition뿐 아니라, 최신 Convolutional Neural Networks(CNNs) 방식과 VLSI 기반 방법들을 포함한 다양한 분석을 제공합니다.

- **Technical Details**: 심근경색증의 탐지는 feature extraction 과정에서 중요한 역할을 하며, 생리학적 신호로부터 적절한 패턴이나 특성을 식별하는 작업을 포함합니다. 기본적인 신호 처리 접근법인 morphological filtering, FIR band-pass filtering과 같은 방법들이 적용되며, CNN과 Binary CNN이 도입되어 더 효율적인 feature extraction을 이끌어냅니다. 또한 wavelet decomposition을 활용하여 신호에서 더 많은 정보를 추출하는 방법론과, 신호를 이미지로 변환하는 등의 혁신적인 전략이 제안되었습니다.

- **Performance Highlights**: 여러 연구가 제안된 방법들의 성능을 평가하였고, 한 연구에서는 에너지 소비를 3배 줄이면서 90%의 정확도를 달성했습니다. 다른 연구에서는 90.29%의 평균 정확도, 90.41%의 민감도, 90.16%의 특이성을 기록했습니다. 각 연구는 K-fold 교차 검증을 적용하여 모델의 신뢰도를 높였으며, 이를 통해 웨어러블 심근경색 탐지 장치의 신뢰성과 유용성을 향상시키는 데 기여하고 있습니다.
...
### An End-to-End Smart Predict-then-Optimize Framework for Vehicle Relocation Problems in Large-Scale Vehicle Crowd Sensing (https://arxiv.org/abs/2411.18432)
Comments:
          31 pages, 12 figures

- **What's New**: 이 논문에서는 차량 군집 감시(VCS) 문제를 해결하기 위해 새롭고 혁신적인 Smart Predict-then-Optimize (SPO) 프레임워크를 개발했습니다. 기존의 예측-최적화(PTO) 접근 방식의 한계를 극복하기 위해 예측과 최적화를 하나의 딥 러닝 구조 내에서 통합하는 방식을 사용했습니다. 이 SPO 프레임워크는 차량의 재배치 문제를 제약된 이차 프로그래밍(Quadratic Programming, QP)으로 공식화하여 차량 분포와 목표 감지 분포 간의 격차를 최소화하는 것을 목표로 합니다.

- **Technical Details**: 이 방법론에서는 차량 재배치 문제를 QP로 공식화하고, Alternating Direction Method of Multipliers (ADMM) 기반의 새로운 언롤링(unrolling) 접근법을 적용하여 QP 레이어의 기울기를 계산합니다. 이 과정은 복잡한 네트워크 내에서의 역전파와 기울기 기반 최적화를 용이하게 만들며, 최종적으로 전체 프레임워크가 작업에 특화된 매칭 발산(matching divergence)을 최소화하는 방식으로 훈련됩니다. 또한, 이 방법은 차별화 과정에서의 계산 효율성을 향상시키려는 의도를 가지고 있습니다.

- **Performance Highlights**: 제안된 SPO 프레임워크는 홍콩의 실제 택시 데이터셋을 통해 그 효율성을 검증하였습니다. 이 프레임워크는 불확실성을 동반한 의사결정 문제를 해결하는 새로운 개념을 제시하며, 지능형 교통 시스템(intelligent transportation systems)에서의 응용 가능성을 크게 확장시킬 잠재력을 보여줍니다. 아울러, 차량 감시의 지속적인 데이터 수집을 최적화함으로써, 향후 교통 관리 및 인프라 모니터링 같은 다양한 분야에서의 활용이 기대됩니다.
...
### Streamlining Prediction in Bayesian Deep Learning (https://arxiv.org/abs/2411.18425)
- **What's New**: 이번 연구는 Bayesian Deep Learning (BDL)에서 예측을 간소화하는 새로운 방법을 제시합니다. Monte Carlo integration이 여전히 표준인 반면, 부스터의 효율적인 계산에 대한 연구는 상대적으로 부족했습니다. 이 연구는 샘플링 없이 단일 forward pass를 통해 예측을 수행하는 방법을 탐구합니다.

- **Technical Details**: 연구팀은 activation function의 지역 선형화(local linearisation)와 선형 계층에서의 지역 Gaussian 근사(local Gaussian approximations)를 사용하여 posterior predictive distribution의 근사를 분석적으로 계산합니다. 이 기술은 Multi-Layer Perceptron (MLP)와 ViT, GPT-2와 같은 transformer 모델에서 적용되었습니다.

- **Performance Highlights**: 회귀(regression)와 분류(classification) 작업에서 성능을 평가하여 새로운 접근법의 효과를 입증하였습니다. 이러한 방법은 BDL의 예측 능력을 향상시키는 데 기여할 것으로 기대됩니다.



### FastSwitch: Optimizing Context Switching Efficiency in Fairness-aware Large Language Model Serving (https://arxiv.org/abs/2411.18424)
- **What's New**: 이 논문에서는 FastSwitch라는 공정성을 고려한 새로운 LLM 제공 시스템을 제안합니다. 기존 시스템들이 성능을 최대화하기 위해 너무 많은 자원을 할당하면서 발생하는 오버헤드를 해결하는 데 초점을 맞추었습니다. FastSwitch는 기존 KV 캐시 메모리 할당 정책에 맞추면서도 컨텍스트 스위칭 오버헤드를 줄이고, 사용자 요청의 공정성을 높입니다.

- **Technical Details**: FastSwitch는 I/O 인식을 통해 KV 캐시 관리 전략을 강화하며, 동적 블록 그룹 관리자를 사용해 대역폭과 커널 배치 오버헤드를 최소화합니다. 또한, 다중 스레딩 스왑 관리자를 통해 KV 캐시 전송을 비동기적으로 관리하여 컨텍스트 스위칭 시의 지연을 최소화합니다. 이를 통해 여러 사용자들이 동시에 요청할 때 응답성을 보장하면서 전체 자원 활용을 효율적으로 유지합니다.

- **Performance Highlights**: FastSwitch는 기존의 최첨단 LLM 제공 시스템인 vLLM과 비교하여 TTFT에서 최대 1.4-5.8배, TBT에서는 최대 11.2배의 속도 향상을 이끌어냈습니다. 또한 전송량(Throughput)에서도 최대 1.44배 증가시켰습니다. 이러한 성과는 LLM 서비스의 공정성과 확장성을 크게 개선합니다.



### When does a bridge become an aeroplane? (https://arxiv.org/abs/2411.18406)
Comments:
          Conference proceedings paper for ISMA, Sept. 2024

- **What's New**: 이번 연구에서는 이질적인 구조 간 정보 이전의 문제를 해결하기 위해 중간 구조를 활용하는 새로운 접근법을 제안합니다. 연구진은 '다리(bridge)'와 '비행기(aeroplane)'를 포함한 간단한 모형을 사용하여 중간 구조 변형의 가능성을 시연하고 있습니다. 이러한 기법은 재료 특성과 기하학적 매개 변수를 조정하여 한 구조를 연속적으로 다른 구조로 변형할 수 있는 원리를 기반으로 합니다.

- **Technical Details**: 연구는 기하학적 접근 방법을 사용하여 비유클리드(non-Euclidean) 데이터에서의 이전을 다루고 있습니다. 특히, Grassmannian manifold 상의 PCA를 통해 소스와 타깃 도메인 간의 기하학적 및 통계적 속성의 변화를 통합하는 geodesic flow kernel (GFK) 개념이 도입됩니다. 이러한 방법은 간단한 두 지지 다리 간의 전이에서 뛰어난 성능을 보였으며, 현재 연구는 이러한 접근 방식을 비행기와 다리 간의 전달 학습에 적용하고 있습니다.

- **Performance Highlights**: 실험 결과, 기존의 직접적인 손상 레이블 전이는 낮은 성능을 보인 반면, 다수의 중간 구조를 통한 전이는 탁월한 결과를 보여주었습니다. 연구진은 支持 벡터 머신(Support Vector Machines, SVM)을 이용하여 정상 조건 정렬(normal-condition alignment) 방법으로 분류를 수행하였고, 이는 기하급수적 흐름_KERNEL(geodesic flow kernel)을 통해 더욱 향상되었습니다. 이 연구는 이질적인 시스템 사이에서도 긍정적인 정보 이전이 가능함을 보여주고 있습니다.
...
- **What's New**: 본 논문에서는 재훈련 없이 비전 네트워크 추론 비용을 줄이기 위한 원샷 포스트 트레이닝 프루닝(franning) 프레임워크인 SNOWS를 제안합니다. 기존의 원샷 프루닝 방법들은 레이어 단위의 재구성 오차를 최소화하는 데 초점을 맞추고 있어, 심층 네트워크의 비선형 활성화 저장을 고려하지 않았습니다. 그러나 SNOWS는 더 글로벌한 재구성 목표를 최적화하여 네트워크 손실에 대한 더 나은 프록시를 제공합니다. 이러한 접근은 비선형 최적화 문제를 제시하지만, 특화된 2차 최적화 프레임워크를 사용하여 효율적으로 해결할 수 있습니다.

- **Technical Details**: SNOWS는 해시안 행렬을 계산하거나 저장할 필요 없이 정확한 뉴턴 하강 단계(Newton descent steps)를 계산하는 헤시안 프리 최적화(Hessian-free optimization)를 사용하여 효율적인 2차 최적화를 수행합니다. 이 프레임워크는 기존의 글로벌 또는 재훈련 기반 방법들과는 다른 접근 방식으로, 단일 GPU에서 대규모 CNN과 비전 변환기(ViT)를 간편하게 프루닝할 수 있도록 설계되었습니다. SNOWS의 주요 특징은 기존의 프루닝 방식보다 더 큰 메모리 효율성을 제공하는 특수화된 컨주게이트 그래디언트(Conjugate Gradient, CG) 방법을 사용하는 점입니다.

- **Performance Highlights**: SNOWS는 ResNet50을 1:4:1 sparsity로 프루닝하면서 CIFAR-100에서 1.5%의 정확도 감소를 보였고, ImageNet-1k에서는 9.4% 감소를 기록했습니다. 이는 기존의 원샷 N:M 프루닝 방법들이 각각 7.6%와 29.42%의 정확도 감소를 보인 것에 비해 상당히 우수한 성능입니다. 또한, SNOWS는 MobileNet을 70% sparsity로 프루닝하면서 5.6%의 정확도 감소로 성공하였는데, 이전 가장 좋은 방법이 12.55%의 감소를 나타내는 것을 고려할 때 경쟁력이 확인되었습니다.
...
### Large Models Enabled Ubiquitous Wireless Sensing (https://arxiv.org/abs/2411.18277)
Comments:
          8 pages, 11 figures

- **What's New**: 이번 연구는 5G 통신 기술의 발전에 따라 MIMO-OFDM 시스템 내에서 공간 채널 상태 정보(CSI) 예측을 위한 언어 모델 활용을 탐구합니다. 기존의 데이터 기반 접근법으로의 전환을 강조하며, 실시간 환경 정보를 이용한 새로운 공간 CSI 예측 프레임워크를 제안하였습니다. 실험 결과는 제안된 접근법이 효과적임을 보여주며, 무선 네트워크 관리에 혁신적인 전략을 제공할 수 있는 가능성을 제시합니다.

- **Technical Details**: 이 보고서에서는 무선 채널 이론, 채널 모델링, 대규모 MIMO 및 OFDM 기초를 포함하여 공간 CSI 예측을 위한 기초 개념을 제시합니다. 또한, 'LLM4CP' 프레임워크를 통해 시계열 예측 실험의 반복을 다루며, 대규모 모델을 사용한 공간 CSI 예측 연구를 탐구합니다. 이러한 방법론은 기존의 피드백 기반 채널 추정에서 예측 기반 관리로의 발전을 반영하며, 데이터 기반 접근법의 효율성을 조사합니다.

- **Performance Highlights**: 제안된 프레임워크는 높은 정확도를 가지고 있으며, 기존의 전통적인 CSI 예측 방법과 비교하여 개선된 성능을 보입니다. 여러 실험을 통해 무선 환경의 복잡한 특성을 효과적으로 포착할 수 있는 가능성을 보여줍니다. 미래의 5G 네트워크에서의 활용 가능성을 포함하여, 연구 결과는 차세대 통신 시스템의 성능을 크게 향상시킬 잠재력을 가지고 있습니다.



### Dynamic Retail Pricing via Q-Learning -- A Reinforcement Learning Framework for Enhanced Revenue Managemen (https://arxiv.org/abs/2411.18261)
Comments:
...
- **What's New**: 이 연구에서는 전통적인 가격 책정 방법인 static demand models와는 달리, Q-Learning 알고리즘을 활용한 강화학습(RL) 프레임워크의 적용을 탐구합니다. RL 방법론은 시장 동향에 더욱 유연하게 대응하며, 소비자 행동의 실시간 변화를 다룰 수 있는 효과적인 전략을 제시합니다. 이 연구는 경제적 의사결정에 AI를 적용하는 가능성을 강조하며, 다양한 상업 분야에서 데이터 기반의 정교한 가격 모델 개발의 길을 열었습니다.

- **Technical Details**: 이 논문에서는 소매 산업에서 동적 가격 책정 전략을 최적화하기 위해 Q-Learning 알고리즘을 적용했습니다. 이를 위해 기초 수요(base demand), 기초 가격(base price), 가격 탄력성(price elasticity) 및 운영 비용을 포함한 가격 환경을 구축하였으며, 이 요인들은 시장 수요 변화에 대응하는 전략의 효과에 직접적인 영향을 미칩니다. Q-Learning 에이전트는 각 상태에서 행동의 가치를 학습하며, 보상 함수는 수익을 극대화하는 방향으로 설계되었습니다.

- **Performance Highlights**: 모델의 성능 분석 결과, 강화학습 접근법은 전통적 최적화 방법에 비해 상당한 수요 최적화를 달성했습니다. 예를 들어, 삼성 49” 4K Q6F 모델의 경우, RL은 최적 가격인 $820.3에서 101.5단위의 수요를 달성했으며, 전통적 방법에서는 $509.5에서 260.1단위에 그쳤습니다. 이러한 결과는 강화학습이 시장 변화에 민첩하게 적응하여 수익을 극대화하는 데 효과적이라는 것을 명확히 보여줍니다.



### Transfer Learning for Deep Learning-based Prediction of Lattice Thermal Conductivity (https://arxiv.org/abs/2411.18259)
- **What's New**: 이 연구에서는 딥러닝 모델인 ParAIsite의 성능을 향상시키기 위해 transfer learning을 활용하는 두 단계의 접근 방식을 제안합니다. 기계 학습 모델은 적은 퀄리티의 데이터에서 발생하는 한계를 극복하기 위해 대규모 비정확 데이터로 사전 학습됩니다. 이후, 고품질의 소규모 데이터셋에서 재조정하여 더욱 정확한 예측을 수행합니다. 이러한 방법은 저열전도율(reduced thermal conductivity) 물질 탐색의 가능성을 열어줍니다.

- **Technical Details**: 연구는 다양한 데이터셋을 활용하여 ParAIsite 모델의 훈련을 진행하며, 특히 AGL 모델에 기초한 저정밀 데이터셋에서 사전 훈련을 실시합니다. 각 데이터셋은 특정 기계적 구조를 지닌 재료들로, 이 과정에서 anharmonic lattice dynamics의 원리를 바탕으로 계산된 데이터가 중요합니다. TensorFlow와 같은 딥러닝 프레임워크를 통해 구현된 MLP(multilayer perceptron) 구조는 기존의 MEGNet 모델과 연결되어 훈련됩니다.

- **Performance Highlights**: 모델의 최종 성능은 기존의 기계 학습 모델들과 비교했을 때 유의미한 개선을 보여주며, low-quality 데이터셋에서 사전 훈련된 후 high-quality 데이터셋에서 재훈련 되었을 때 더 뛰어난 일반화 능력을 발휘합니다. 성능 평가에서 MAPE(mean absolute percentage error)가 기준으로 사용되며, 여러 차례의 검증을 통해 안정적인 결과를 달성하였습니다. 이로 인해, 다양한 재료 데이터베이스에서의 저열전도율 물질 탐색이 가능해질 것으로 기대됩니다.



### Active partitioning: inverting the paradigm of active learning (https://arxiv.org/abs/2411.18254)
- **What's New**: 이 연구에서는 데이터를 다양한 기능 패턴으로 분할하는 새로운 알고리즘인 'active partitioning'을 제안합니다. 이 알고리즘은 경쟁을 통해 모델이 최상의 예측을 제출하도록 유도하여 데이터에서 특수한 패턴을 학습합니다. 각 모델은 자신의 강점을 살려 학습하며, 이를 통해 데이터 포인트에 대한 분할을 실시하게 됩니다.

- **Technical Details**: 제안된 알고리즘은 여러 모델 간의 경쟁을 기반으로 하며, 각 모델은 최적의 예측을 위해 데이터 포인트를 제출하고, 가장 우수한 예측을 한 모델이 해당 데이터 포인트에 대해 훈련을 받게 됩니다. 이 과정은 여러 차례 반복되며, 최종적으로 모델 간의 예측 결과에 따라 데이터 세트가 서로 다른 분할로 나뉘게 됩니다. 이 알고리즘의 결과는 경계로 저장되며, 각 경계는 Support Vector Machine (SVM)을 통해 표현됩니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘은 모듈 방식으로 구성된 모델이 단일 모델에 비해 현저한 성능 향상을 보여주었습니다. 20개 이상의 회귀 문제에서 54%까지 손실을 줄이는 성과를 거두었으며, 이는 partitioning 알고리즘의 유용성을 입증합니다.
...
### Semantic Edge Computing and Semantic Communications in 6G Networks: A Unifying Survey and Research Challenges (https://arxiv.org/abs/2411.18199)
Comments:
          Submitted to ACM Computing Surveys (CSUR)

- **What's New**: 이번 논문은 Semantic Edge Computing (SEC)과 Semantic Communications (SemCom)의 통합적인 시각을 제시하여, 두 분야 간의 연구 문제와 최신 기술 동향을 정리합니다. 기존 연구는 각각 독립적으로 발전해왔으나, 본 연구는 이 두 분야의 유사성과 차별성을 조명하여 상호 발전의 필요성을 강조합니다. SEC는 정보 전달 효율성을 높이는 데 중점을 두고, SemCom은 채널 왜곡을 보완하기 위해 심층 신경망(Deep Neural Networks, DNN)을 활용하여 시맨틱 정보를 전송합니다.

- **Technical Details**: SEC는 모바일 장치와 엣지 서버 간의 DNN 처리를 분산하여 실행하며, DNN의 각 부분을 장치의 컴퓨팅 및 네트워킹 제약에 맞추어 나누는 방식입니다. 반면 SemCom은 시맨틱 정보 및 채널 코딩을 위해 DNN을 적용하여, 원시 입력 메시지로부터 정보를 추출하고 전송 과정에서 발생하는 왜곡을 고려하는 모델입니다. 이들 시스템은 데이터 전송 효율성을 극대화하고, 서로 다른 집중 처리를 통해 성능을 저하시키지 않으며, 저지연 데이터 전송을 목표로 합니다.

- **Performance Highlights**: SEC와 SemCom 간의 협업은 두 영역 모두에서 참여 장치 간의 계산과 통신 부하를 최적화함으로써 성능을 크게 향상시킬 수 있음을 보여줍니다. 특히, SEC는 라티언트 표현의 크기를 압축하여 전송하는 방법으로 통신 지연을 감소시키고, SemCom은 채널 특성을 고려하여 정보를 더 견고하게 처리하여 전송 효율을 높이는 데 기여합니다. 본 연구에서는 SEC와 SemCom의 통합을 통해, 두 분야의 연구자들이 서로의 혁신적인 기술을 채택하여 보다 나은 성과를 도출할 수 있는 발판을 마련하고자 합니다.



### Scalable Multi-Objective Reinforcement Learning with Fairness Guarantees using Lorenz Dominanc (https://arxiv.org/abs/2411.18195)
Comments:
          29 pages

- **What's New**: 본 논문은 복수의 목표를 최적화하는 Multi-Objective Reinforcement Learning (MORL)에서 공정성을 반영하는 알고리즘을 제안합니다. 특히, 다양한 목표가 포함될 때 공정한 보상 분배를 실현하기 위해 Lorenz dominance를 이용한 방법론을 도입했으며, 이를 통해 높은 차원의 목표 공간에서도 뛰어난 확장성을 보여줍니다. 이를 통해 Xi'an과 Amsterdam의 두 대도시에서 실험을 수행하여 공정한 정책 세트를 성공적으로 발견하였습니다.

- **Technical Details**: MORL은 각 목표에 대해 별도의 보상 함수를 도입하여, 최적의 정책 후보 집합을 생성하고 개인의 선호에 따라 정책을 선택할 수 있게 합니다. 제안된 λ-Lorenz dominance는 의사 결정을 위한 공정성 제약을 조정할 수 있는 유연성을 제공합니다. 또한, Lorenz Conditioned Networks (LCN)라는 새로운 알고리즘을 도입하여 다목적 환경의 최적화를 가능하게 하였고, 이는 실제 도시의 교통망 계획에 적용되었습니다.

- **Performance Highlights**: LCN은 기존의 다목적 접근법보다 우수한 성능을 보여주며, 특히 고차원 목표 공간에서 보다 효과적으로 정의될 수 있는 공정한 정책 세트를 생성합니다. 이 알고리즘은 고차원 문제에서도 뛰어난 확장성을 보여주며, 제작된 새로운 대규모 다목적 환경을 통해 그 가능성을 실증하였습니다. 연구 결과는 GitHub에 공개되어 접근할 수 있습니다.



### Machine Unlearning reveals that the Gender-based Violence Victim Condition can be detected from Speech in a Speaker-Agnostic Setting (https://arxiv.org/abs/2411.18177)
- **What's New**: 이번 연구는 성 기반 폭력(Gender-Based Violence, GBV)이 여성의 정신 건강에 미치는 영향이라는 중요한 문제를 다룹니다. 기존의 인공지능(AI) 음성 기술이 정신 건강 평가에 유용하지만, 학습에 사용되지 않은 데이터에 대한 성능 저하 문제가 발생합니다. 본 연구는 다양한 화자에 대해 일반화 가능한 강력한 AI 모델 개발에 중점을 두며, 성별 폭력 피해자 상태(GBVVC) 탐지에 대한 새로운 접근법을 제시합니다.

- **Technical Details**: 연구에서는 고급 딥러닝(Deep Learning) 모델과 도메인-적대적 훈련(Domain-Adversarial Training) 기술을 활용하여 화자의 특성이 결과에 미치는 영향을 최소화합니다. 우리가 달성한 26.95% 상대적 감소는 화자 식별 능력에서의 감소를 의미하며, GBVVC 탐지의 정확도는 6.37% 향상되었습니다. 이러한 기술적 접근은 구별 가능한 파라언징기(paralinguistic) 바이오마커에 초점을 맞춰 분석하며, 주제 특정 특성의 영향을 줄이는 데 기여합니다.

- **Performance Highlights**: 모델의 예측 결과는 사전 임상 PTSD(Post-Traumatic Stress Disorder) 증상과 중간 정도의 상관관계를 보이며, 이는 GBV와 정신 건강 간의 연결을 강조합니다. 본 연구는 AI 기반 도구가 정신 건강 전문의가 사회적 문제를 다루는 데 어떻게 기여할 수 있는지를 보여주며, 향후 연구에 대한 유망한 기초를 제공합니다.



### A Machine Learning-based Framework towards Assessment of Decision-Makers' Biases (https://arxiv.org/abs/2411.18122)
- **What's New**: 이번 연구에서는 금표준 레이블(gold standard labels)이 제한된 상황에서 인간 결정 편향(human decision bias)을 평가할 수 있는 머신러닝 기반 프레임워크를 제안합니다. 기존의 방법론에 비해 우수성을 입증하는 이론적 보장(theoretical guarantees)과 실증적 증거(empirical evidence)를 제공합니다.

- **Technical Details**: 이 방법론은 결정의 품질을 고려하는 편향 측정 지표를 개발하며, 역사적 데이터와 소규모 금표준 레이블을 활용하여 인간 결정의 편향을 평가합니다. 특히, 오류의 유형(예: false positives, false negatives)에 따른 집단 간 불균형을 특정하여 분석합니다.

- **Performance Highlights**: 제안된 방법은 다양한 데이터 도메인과 인적 편향 유형을 포함한 여러 설정에서 기존의 대안과 비교하여 지속적으로 우수한 성과를 나타냅니다. 이 접근법은 기업의 의사 결정 개선 및 알고리즘 편향 완화에 기여할 수 있는 큰 잠재력을 지니고 있습니다.
...
### ORIS: Online Active Learning Using Reinforcement Learning-based Inclusive Sampling for Robust Streaming Analytics System (https://arxiv.org/abs/2411.18060)
Comments:
          To appear in 2024 IEEE International Conference on Big Data (IEEE BigData 2024)

- **What's New**: 본 논문에서는 온라인 액티브 러닝(online active learning)에 기반한 ORIS 방법을 제안하여, 제출된 문서의 라벨링에서 인간 오류를 최소화하고 기계 학습(ML) 모델 성능을 향상시키는 전략을 소개합니다. 기계 메모리 감퇴 행동을 모사하는 오라클 주석가를 사용하여 라벨링 오류를 통제하는 프레임워크를 진행하며, 기존 방식 대비 높은 성능 개선치를 제공합니다. 실험 결과, Twitter와 Reddit을 대상으로 한 감정 인식 작업에서 각각 최대 38.3% 및 44.2%의 인간 라벨링 성능 향상이 관찰되었습니다.

- **Technical Details**: ORIS는 강화를 통한 포함 샘플링(Reinforcement learning-based Inclusive Sampling)을 활용하여 데이터 스트림의 도입 문서를 효율적으로 샘플링하는 데 초점을 맞추고 있습니다. 독창적인 Deep Q-Network 기반 전략을 사용하여 라벨링 오류를 최소화하고 ML 모델의 품질을 높이기 위한 새로운 상태 표현과 보상 기능을 도입합니다. 다양한 실험을 통해 ORIS 방식은 기존의 전통적인 접근 방식들보다 높은 라벨링 성능과 ML 성능의 개선율을 보여줍니다.

- **Performance Highlights**: ORIS 방법은 두 가지 플랫폼에서 감정 인식 작업을 수행하며, Twitter에서는 55.7%의 기계 성능 향상을 보였습니다. Reddit에서는 무려 70.1%의 기계 성능 향상을 기록하며, 라벨링 작업의 효율성을 극대화하는 데 성공했습니다. 이러한 결과는 ORIS의 효과적인 샘플링 전략이 기존의 라벨링 방법보다 더욱 우수하다는 것을 입증하고 있습니다.



### FAMES: Fast Approximate Multiplier Substitution for Mixed-Precision Quantized DNNs--Down to 2 Bits! (https://arxiv.org/abs/2411.18055)
- **What's New**: 본 연구에서는 낮은 비트폭의 양자화된 DNN 모델에 효과적으로 근사 곱셈기(AppMul)를 적용할 수 있는 가능성을 탐구합니다. 이를 통해 제안된 FAMES 방법은 기존의 연구와 달리 비트폭 2비트에서 평균 28.67%의 에너지 절약을 달성하며 정확도 손실을 1% 이내로 유지합니다. 또한 FAMES는 기존의 유전자 알고리즘 기반 방법보다 최대 300배 빠른 성능을 보여줍니다.

- **Technical Details**: FAMES는 혼합 정밀도 DNN을 위한 빠른 근사 곱셈기 대체 방법으로, 두 가지 주요 기술을 활용합니다. 첫 번째는 AppMul의 오류를 모델링하기 위한 새로운 카운팅 매트릭스를 제안하여 FAMES의 핵심 기반을 형성합니다. 두 번째는 테일러 확장을 기반으로 하여, DNN 모델에서 AppMul의 손실 변동을 효율적으로 추정하는 방법을 소개합니다.

- **Performance Highlights**: FAMES는 비트폭이 2비트인 양자화된 DNN 모델에서 근사 곱셈기를 성공적으로 적용합니다. 실험 결과, 기존 양자화된 모델 대비 평균 28.67%의 에너지 절약과 1% 미만의 정확도 손실을 달성하였습니다. 이는 에너지 효율성 뿐만 아니라 높은 성능을 동시에 가능하게 합니다.
...
### Diffeomorphic Latent Neural Operator Learning for Data-Efficient Predictions of Solutions to Partial Differential Equations (https://arxiv.org/abs/2411.18014)
- **What's New**: 이 논문은 다양한 기하학적 배경을 가진 데이터 샘플을 취득하거나 시뮬레이션하는 것이 불가능한 경우에도 PDE(Partial Differential Equations) 해법 연산자를 학습하는 새로운 접근 방식을 제시합니다. 연구진은 잠재적(neural latent) 신경 연산자를 사용하여 여러 기하학적/공간 범위에서 맵핑된 두 개의 해(final fields)만으로도 일반화된 모델을 학습할 수 있음을 보여줍니다. 이 과정에서 미분 연산자의 성질을 보존하는 것이 데이터 요구량을 현저히 줄일 수 있음을 강조합니다.

- **Technical Details**: 신경 연산자(neural operators)는 비선형 연속 연산자를 근사하는 데 설계된 신경망 구조입니다. 이들은 무한차원 함수 공간에서의 매핑(mapping)을 통해 입력 함수와 PDE 솔루션 간의 관계를 모델링할 수 있습니다. 논문에서는 2D Laplacian을 통해 다양한 맵핑이 신경 연산자의 성과에 미치는 영향을 분석하고, 수학적 성질을 보존하는 맵핑을 사용하는 것이 데이터 효율성을 높이고 정확한 학습을 가능하게 함을 시연합니다.

- **Performance Highlights**: 실험 결과, 다양한 맵핑 방법을 통한 해석을 통해 변환된 PDE 솔루션의 표현 방식과 학습 효율성 간의 관계를 명확히 하였습니다. 이 연구는 고충실도 수치 시뮬레이션 및 신경 연산자 학습을 위한 새로운 프레임워크를 제공하며, 효율적 학습을 위한 맵핑 설계의 필요성을 강조합니다. 최종적으로, 이런 접근 방식은 데이터 요구량을 최소화하고 컴퓨테이션 비용을 줄이는 데 기여할 수 있음을 시사합니다.
...
### Optimized Tradeoffs for Private Prediction with Majority Ensembling (https://arxiv.org/abs/2411.17965)
Comments:
          57 pages, 10 figures. Proceedings of Transactions on Machine Learning Research (TMLR), November 2024

- **What's New**: 이번 논문에서는 $(meta,	ext{delta})$-차분 프라이버시(differential privacy)를 갖는 K개의 알고리즘으로부터 다수결을 계산하는 문제를 다룬다. 이 논문에서는 데이터 종속적 난수 응답 다수결(DaRRM) 알고리즘을 소개하며, 이는 데이터에 따라 결정되는 노이즈 함수를 통해 개인 정보 보호와 유용성을 최적화하는 방법을 제안한다. 또한, 무한한 프라이버시 제약을 다항식 형태로 축소하는 새로운 구조적 결과를 통해 유틸리티를 극대화할 수 있는 방법을 제시한다.

- **Technical Details**: DaRRM 알고리즘은 전통적인 서브샘플링(subsampling) 및 무작위 응답(randomized response) 방법보다 효과적인 개인 정보 보호와 유용성의 절충을 가능하게 한다. 이 알고리즘은 각 모델의 출력에 로컬 프라이버시를 부여한 뒤, 서버에 전송하는 방식을 사용하여 중앙 집중식 프라이버시 보장을 도출한다. 해당 연구에서는 $(meta,	ext{delta})$-차분 프라이버시를 갖는 메커니즘이 주어진 데이터 집합에 따라 움직이는 다수결 기능을 효율적으로 평가할 수 있는 방법을 제안하고 있다.

- **Performance Highlights**: 실험 결과 DaRRM 알고리즘은 특정 상황에서 기존의 일반적인 기법들에 비해 2배의 프라이버시 이득을 제공하는 것으로 나타났다. 최적화된 노이즈 함수 $eta$를 사용하는 DaRRM 프레임워크는 여러 기준선에 비해 상당한 유틸리티 증가를 보여주었다. 특히, 이미지 분류와 같은 개인 정보 보호가 요구되는 작업에서 강력한 성과를 입증하여, 다수결을 통한 프라이버시 제약 유틸리티 최적화의 효과를 강조하고 있다.



### ESS-ReduNet: Enhancing Subspace Separability of ReduNet via Dynamic Expansion with Bayesian Inferenc (https://arxiv.org/abs/2411.17961)
- **What's New**: 이번 논문에서는 ReduNet의 한계를 극복하기 위해 ESS-ReduNet이라는 새로운 프레임워크를 제안합니다. 이 모델은 최대 코딩 속도 감소(maximal coding rate reduction) 원칙을 활용하여 데이터 샘플을 저차원 선형 판별 특징 표현으로 변환합니다. ESS-ReduNet은 학습 샘플 간의 서브스페이스(subspace) 분리 가능성을 향상시키고, Bayesian inference를 통해 더 정확한 피처 업데이트를 지향합니다.

- **Technical Details**: ESS-ReduNet은 네트워크 파라미터의 기하학적 해석에 기반하여 서브스페이스의 분리를 최적화합니다. 학습 과정에서 레이블 지식을 Bayesian 방식으로 통합하여 서브스페이스의 분리를 강화합니다. 또한, 고려해야 할 보조 기준으로 조건 수(condition number)를 사용하여 네트워크 훈련의 효율성과 품질을 제고하고, 불필요한 연산 자원 소모를 피합니다.

- **Performance Highlights**: 실험을 통해 ESS-ReduNet은 ReduNet에 비해 수렴 속도가 10배 향상된다고 보고되었습니다. 특히 ESR 데이터셋에서 ESS-ReduNet의 변환된 피처는 SVM 정확도가 47% 증가하는 성과를 나타냈습니다. HAR 및 Covertype 데이터셋에서도 각각 37%의 정확도 향상을 보여줌으로써, 본 연구의 효과를 입증합니다.
...
### Multi-Label Bayesian Active Learning with Inter-Label Relationships (https://arxiv.org/abs/2411.17941)
- **What's New**: 이번 논문에서는 다중 레이블 능동 학습(multi-label active learning)에서 레이블 상관관계(label correlation)를 효과적으로 다루기 위해 새로운 전략인 CRAB를 제안합니다. 기존의 연구와 비교했을 때, CRAB는 양의 상관 행렬과 음의 상관 행렬을 점진적으로 업데이트하여 레이블 공간 내에서의 관계를 포괄적으로 분석합니다. 이 방법은 불균형 데이터 문제(data imbalance)를 해결하는 데에도 주목하며, 데이터의 개별적인 불확실성(uncertainty)뿐 아니라 전체적인 불확실성을 평가할 수 있게 합니다.

- **Technical Details**: CRAB 모델은 다중 레이블 분류에서 레이블 간의 상관관계를 정확하게 포착하기 위해 이진 분류(binary classification) 방식과는 다르게, 긍정적 및 부정적 상관 행렬을 기반으로 합니다. 베타 스코어링 규칙(beta scoring rules)을 통해 데이터 불균형을 해결하고, 샘플의 가장 정보량이 많은 인스턴스를 선택하는 기대 손실 감소(expected loss reduction) 프레임워크를 통합하고 있습니다. 이를 통해, 레이블 간의 비대칭 및 독립적인 관계를 다룰 수 있습니다.

- **Performance Highlights**: 네 가지 실제 데이터셋을 기반으로 한 실험 결과, CRAB는 기존의 여러 방법들보다 일관되게 신뢰할 수 있고 우수한 성능을 보였습니다. 데이터의 고유한 상관관계를 활용하여 수집된 데이터(instance)의 정보량을 극대화하고, 분류 성능을 높였습니다. 이러한 결과는 다중 레이블 능동 학습 분야에서 새로운 프레임워크의 가능성을 보여줍니다.
...
### RankMap: Priority-Aware Multi-DNN Manager for Heterogeneous Embedded Devices (https://arxiv.org/abs/2411.17867)
Comments:
...
- **What's New**: 이 논문에서는 RankMap이라는 새로운 우선 순위 인식 관리자를 소개합니다. 이 관리자는 이종 임베디드 장치에서 다중 DNN 작업을 위한 관리 시스템으로 설계되었습니다. RankMap은 다중 DNN 매핑의 솔루션 공간을 확률론적 공간 탐색과 성능 추정기를 결합하여 해결합니다.

- **Technical Details**: RankMap은 각 DNN 층의 계산 프로파일을 학습하고 이를 파이프라인 스테이지로 그룹화하여 가용한 컴퓨팅 구성 요소에 매핑합니다. 이 접근 방법은 자원 경쟁을 줄이고 시스템의 처리량을 향상시키며, 각 DNN이 동시에 실행되는 동안 다른 DNNs의 자원을 고갈시키지 않도록 합니다. 또한 RankMap은 다중 작업 주목 기반 CNN을 사용하여 다양한 다중 DNN 시나리오의 처리량을 추정합니다.

- **Performance Highlights**: 실험 결과, RankMap은 기존 방법에 비해 평균 3.6배 높은 처리량을 달성하고, 다중 DNN 작업에서 첫 번째 DNN의 우선 순위에 따라 57.5배 효율적으로 우선 순위를 개선합니다. 이러한 성능 향상은 DNN들이 동시에 실행될 때에도 자원이 부족하지 않도록 보장합니다.



### Distributed Sign Momentum with Local Steps for Training Transformers (https://arxiv.org/abs/2411.17866)
Comments:
          23 pages, 21 figures

- **What's New**: 이 논문에서는 대규모 분산 학습의 효율성을 높이기 위해 새로운 통신 효율적인 sign momentum 방법을 제안합니다. 제안된 방법은 다양한 기본 최적화 기법을 로컬 업데이트에 사용할 수 있으며, 로컬 단계에서 누적된 차이를 이용해 글로벌 업데이트에서 momentum을 생성합니다. 이 접근법은 Transformer 모델을 사전 훈련할 때 다른 분산 방법들보다 성능이 우수하다는 것을 보여줍니다.

- **Technical Details**: 분산 최적화 문제를 다루며, n (italic_n) 개의 워커(worker)에서의 supervised learning 모델 F_{i} (F_{i})와 데이터 샘플 (𝝃(i) (𝝬) 을 정의합니다. 로컬 업데이트 방법의 성과는 SCAFFOLD, BMUF, SlowMo와 같은 기존 기술들에 의해 강화되어 왔습니다. 본 작업에서는 sign momentum 방법을 활용하여 로컬 업데이트 환경에서도 효과적인 학습이 가능함을 입증합니다.

- **Performance Highlights**: 제안된 알고리즘은 다양한 크기의 GPT-2 모델을 사전 훈련하는 데 적용되었으며, 기존의 로컬 업데이트 방법들보다 일관되게 개선된 성능을 보였습니다. 특히, 단일 워커 환경에서 제안된 알고리즘은 AdamW보다도 뛰어난 성능을 나타냈습니다. 이로 인해 분산 학습에서의 효율성 및 성능이 크게 향상된 결과를 얻었습니다.
...
### Integrating Machine Learning and Quantum Circuits for Proton Affinity Predictions (https://arxiv.org/abs/2411.17856)
- **What's New**: 이 연구에서는 복잡한 유기 분자의 프로톤 결합 위치를 신속하게 식별하기 위한 효율적인 프로톤 친화도(proton affinity, PA) 예측 방법을 개발하였습니다. 기존의 질량 분석(mass spectrometry) 및 ab initio 계산 방법 대신, 머신러닝(machine learning, ML) 모델을 사용하여 PA를 예측하는 혁신적인 접근을 제시합니다. 이 방법은 자원 소모가 적고 시간도 절약할 수 있는 장점이 있습니다.

- **Technical Details**: 연구진은 186개의 포괄적인 설명자(descriptor)를 활용하여 모델을 구축하였으며, R2 값 0.96과 평균 절대 오차(mean absolute error, MAE) 2.47 kcal/mol을 달성하여 실험적인 불확실성과 유사한 예측 성능을 보여주었습니다. 더욱이, 고전 신경망(classical neural network)을 위한 특징 인코더(feature encoder)로 양자 회로(quantum circuits)를 설계하여 융합된 양자-고전 모델의 효과를 평가하였습니다. 이 두 가지 접근법을 비교함으로써 하이브리드 모델의 신뢰성을 확인할 수 있었습니다.

- **Performance Highlights**: 하이브리드 모델은 노이즈 없는 시뮬레이터와 실제 양자 하드웨어 모두에서 동일한 축소된 설명자 세트를 사용한 전통적인 ML 모델과 비교하여 일관된 성능을 보였습니다. 이는 양자 기계 학습(quantum machine learning)이 PA 예측의 정확성과 효율성을 높이는 데 잠재력이 있음을 나타냅니다. 이러한 결과는 복잡한 유기 분자의 구조 예측에 있어 새로운 길을 열어줄 것으로 기대됩니다.



### Rock the KASBA: Blazingly Fast and Accurate Time Series Clustering (https://arxiv.org/abs/2411.17838)
- **What's New**: 이번 논문에서는 시계열 클러스터링(time series clustering, TSCL)의 새로운 알고리즘인 KASBA(k-means Accelerated Stochastic subgradient Barycentre Average)를 소개합니다. KASBA는 Move-Split-Merge(MSM) 거리 개념을 모든 클러스터링 단계에 적용하고, 랜덤화된 확률적 서브그래디언트 하강법을 이용해 중심점을 찾습니다. 이 알고리즘은 게다가 클러스터링 성능과 실행 시간을 효과적으로 개선할 수 있도록 설계되었습니다.

- **Technical Details**: KASBA는 기존의 k-means 클러스터링 알고리즘과 통합되어, 탄력적 거리(elastic distance)를 모든 단계에 적용합니다. 중심점 생성에서 barycentre averaging을 이용하며, 삼각 부등식(triangle inequality)를 활용하여 거리 계산 횟수를 줄이는 방식을 채택합니다. 이 알고리즘은 다양한 시계열 데이터에 적용 가능하며, 기존 알고리즘보다 더 나은 성능을 제공합니다.

- **Performance Highlights**: KASBA는 기존의 TSCL 알고리즘들과 비교하여 우수한 클러스터링 성능을 보여주었으며, 실행 시간 역시 획기적으로 개선되었습니다. 실험을 통해 KASBA가 최신 알고리즘의 성능을 초과하는 결과를 보인다는 것을 입증했습니다. 특히 KASBA의 구성 요소와 매개변수에 대한 기여도를 평가하기 위한 탈착 연구가 진행되어, 이 알고리즘의 효율성에 대한 깊은 통찰을 제공합니다.



### Adaptive Client Selection with Personalization for Communication Efficient Federated Learning (https://arxiv.org/abs/2411.17833)
- **What's New**: 이번 논문은 Federated Learning (FL)에서 모델 훈련을 위한 통신 및 계산 비용을 줄이는 ACSP-FL(Adaptive Client Selection with Personalization for communication efficient Federated Learning)이라는 새로운 방법론을 소개합니다. ACSP-FL은 기계 학습 모델 훈련을 위한 클라이언트를 동적으로 선택하여 전체 통신 비용을 효과적으로 감소시키며, 모델 개인화를 통해 클라이언트의 성능을 향상시킬 수 있는 전략을 사용합니다. 연구 결과, ACSP-FL은 기존의 방법에 비해 통신 비용을 최대 95%까지 줄이고, 데이터가 비독립적이고 비동일하게 분 배된 경우에도 효율적인 수렴을 달성합니다.

- **Technical Details**: ACSP-FL은 클라이언트 선택 및 부분 모델 공유를 통해 FL 환경에서의 통신 효율을 높입니다. 이 방법은 클라이언트의 글로벌 모델 성과에 따라 적응적으로 클라이언트를 선택하여 훈련 단계에서 통신 및 계산 오버헤드를 줄입니다. 부분 모델 공유를 활용하여 클라이언트에서 서버로의 업링크와 서버에서 클라이언트로의 다운링크에서 공유 모델의 크기를 감소시키며, 성능 저하 없이 모델의 일부 층만을 공유하여 각 클라이언트의 성능을 유지합니다.

- **Performance Highlights**: 펀칭된 성능 평가 결과에 따르면 ACSP-FL은 통신 비용을 최대 90%까지 줄이며, 다양한 데이터셋에 대해 기존의 FL 방법들과 비교했을 때 통신 효율을 크게 향상시킵니다. 이 연구는 FL 환경에서 클라이언트 선택 및 개인화 지원을 결합한 새로운 접근 방식을 통해 모델 훈련의 속도와 효율성을 개선함을 보여줍니다. ACSP-FL의 능력은 다양한 성능 메트릭을 통해 검증되었으며, 이는 FL 기술의 발전에 기여하는 중요한 성과로 자리잡고 있습니다.
...
### Scalable iterative pruning of large language and vision models using block coordinate descen (https://arxiv.org/abs/2411.17796)
Comments:
          16 pages, 6 figures, 5 tables

- **What's New**: 이 논문에서는 Combinatorial Brain Surgeon (CBS)를 기반으로 한 새로운 신경망 pruning 기법인 'iterative Combinatorial Brain Surgeon' (iCBS)을 소개합니다. iCBS는 블록 좌표 하강법(block coordinate descent)을 통해 네트워크 가중치의 부분 집합에 대해 반복적으로 최적화 문제를 해결하며, 이렇게 함으로써 매우 큰 모델에 대한 확장성을 제공합니다. 이 방법은 대형 언어 모델(LLMs)에서도 효과적으로 적용될 수 있음을 보여주고, 기존의 방법인 Wanda보다 향상된 성능 지표를 획득할 수 있음을 입증하였습니다.

- **Technical Details**: iCBS는 가중치를 효율적으로 제거하고 성능을 최적화하는 데 필요한 연산 요구 사항을 줄이기 위해 각 단계에서 작은 문제를 해결합니다. 이 과정에서 Hessian의 업데이트와 각 블록의 그래디언트를 통합합니다. 또한, 초기 pruning 및 블록별 가중치 선택에 가중치 점수 방법(weight-scoring methods)을 도입하여 연산 자원을 효과적으로 사용할 수 있도록 합니다. 마지막으로, Tabu search를 활용하여 사이클을 피하고 탐색을 촉진합니다.

- **Performance Highlights**: 제안된 iCBS 방법은 Mistral과 DeiT와 같은 대규모 모델에 적용했을 때, Wanda와 같은 기존의 방법보다 높은 성능 지표를 달성했습니다. 또한, 성능 개선은 전체 가중치의 소규모만을 최적화하여 이루어지므로, 연산 병목 현상은 블록 솔버 실행 시간에 의해 제한됩니다. 이 방식은 하드웨어 가속기(예: 양자 컴퓨터)의 사용으로 크게 향상될 수 있으며, 이론적으로는 양자 최적화 문제로 변환할 수 있습니다.
...
### MTS-UNMixers: Multivariate Time Series Forecasting via Channel-Time Dual Unmixing (https://arxiv.org/abs/2411.17770)
- **What's New**: 이 논문에서는 다변량 시계열 예측을 위한 새로운 모델인 MTS-UNMixer를 제안합니다. 이 모델은 시계열 데이터를 시간과 채널 차원에서 중요한 기초와 계수를 분해하여 명확한 매핑을 제공합니다. MTS-UNMixer는 기존 모델들이 직면한 시계열 데이터의 혼합 문제와 비선형 관계를 효과적으로 해결합니다.

- **Technical Details**: MTS-UNMixer는 두 가지 주요 요소, 즉 시간 기반의 Mamba 네트워크와 비대칭 Mamba 네트워크를 사용하여 채널 상의 상관 관계를 모델링합니다. 이 모델은 기초 신호와 계수 매트릭스를 공유함으로써 역사적 시퀀스와 미래 시퀀스 간의 관계를 명시적으로 모델링 할 수 있게 합니다. 특히, 채널 차원에서는 비인과 관계가 존재하므로 이중 방향 Mamba 네트워크를 이용하여 변수 간의 상호 작용을 효과적으로 캡처합니다.

- **Performance Highlights**: 실험 결과, MTS-UNMixer는 여러 벤치마크 데이터셋에서 기존 방법들과 비교해 월등한 성능을 보여주었습니다. 이 모델은 시계열 데이터의 비선형 causal dependencies를 정확히 추출하여 예측의 신뢰성을 높이고, 물리적으로 해석 가능한 모델을 제공합니다. 이러한 성능 향상은 시간 차원에서의 장기 의존성 확보와 채널 차원에서의 독립적인 특성 분해에 기인합니다.



### Integrating Dual Prototypes for Task-Wise Adaption in Pre-Trained Model-Based Class-Incremental Learning (https://arxiv.org/abs/2411.17766)
Comments:
          8 pages,6 figures,2 tables

- **What's New**: 본 논문은 PTM 기반의 클래스 증분 학습(Class-Incremental Learning, CIL)을 위한 새로운 모델인 Dual Prototype Network for Task-wise Adaption (DPTA)을 제안합니다. DPTA는 각 증분 학습 작업마다 별도의 어댑터 모듈을 구축하여 PTM을 미세 조정하고, 중심 적응 손실(center-adapt loss)을 통해 클래스 간 구분을 강화합니다. 이러한 접근 방식은 개별 작업에서 더 높은 예측 정확성을 유지하며, 적절한 어댑터 모듈을 선택하여 테스트 샘플의 분류 성능을 향상시키는 데 기여합니다.

- **Technical Details**: DPTA는 기존의 PTM에서 원시 프로토타입(raw prototypes)을 구축하고 이를 기반으로 소프트 예측을 수행하여 각 샘플에 대한 최적 탑-K(top-K) 예측을 수행합니다. 이 과정에서 원시 프로토타입을 통해 관련된 작업 인덱스를 유추하고, 그 결과에 따라 각 업무에 적절한 어댑터 모듈을 선택합니다. 또한, 중심 적응 손실을 이용해 미세 조정된 프로토타입들이 클래스 중심으로 집합되고 클래스 간 거리(distance)를 증가시키는 특징이 있습니다.

- **Performance Highlights**: 여러 벤치마크 데이터셋에서 수행한 실험 결과, 제안된 DPTA 모델은 최신 기법들 대비 최첨단 성능을 달성했습니다. 본 연구는 다양한 증분 학습 데이터셋에 대해 테스트되었으며, DPTA의 분류 정확성을 향상시키기 위한 분석 또한 포함하고 있습니다. 향후 본 논문에서 소개된 코드는 오픈소스로 공개될 예정입니다.
...
### Surveying the space of descriptions of a composite system with machine learning (https://arxiv.org/abs/2411.18579)
Comments:
          Code here: this https URL

- **What's New**: 이 논문에서는 다변량 정보 이론을 활용하여 복합 시스템의 구성 요소들이 어떻게 연결되어 있는지를 분석하는 새로운 접근 방식이 제시됩니다. 기존의 분석 방법은 주로 이산(subsystem) 부분의 특성을 기반으로 하여 다소 정형화된 경향이 있었습니다. 반면, 저자들은 연속적인 설명 공간을 연구하여 시스템의 조직 구조를 이해하는 데 초점을 맞춥니다.

- **Technical Details**: 제안된 프레임워크는 각 구성 요소에 대해 전달되는 특정 정보를 포함하는 설명을 최적화하는 기계 학습(machine learning) 접근 방식을 사용합니다. 설명 공간은 구성 요소의 손실 압축(lossy compression) 방식과 동등하며, 정보 이론적 양(total correlation 및 O-information 등)을 극대화하는 설명을 추구합니다. 이 방식을 통해 약한 시스템에서의 변화를 개별 요소에 기반해서 파악할 수 있습니다.

- **Performance Highlights**: 저자들은 스핀 시스템(spin systems) 및 스도쿠(Sudoku) 판, 자연어의 문자 시퀀스 등을 사용하여 실제 사례 연구(case studies)를 수행합니다. 이를 통해 극단적인 설명(extremal descriptions)을 식별하고 시스템 전반의 변동이 개별 구성 요소에서 어떻게 나타나는지를 밝혀냈습니다. 기계 학습을 통합함으로써 복합 확률 변수(composite random variables)의 분석을 세부적으로 다룰 수 있는 새로운 방법론이 열리게 되었습니다.
...
### Living off the Analyst: Harvesting Features from Yara Rules for Malware Detection (https://arxiv.org/abs/2411.18516)
Comments:
          To appear in BigData'24 CyberHunt 2024

- **What's New**: 이 연구에서는 악성 행위자가 사용하는 'land off the land' 전략을 반영하여, 악성코드 탐지 능력을 향상시키기 위해 기존의 YARA 규칙을 재활용할 수 있는 방법을 모색합니다. YARA 규칙을 활용하여 공개되지 않은 서브 서명을 추출하고 이를 통해 악성 샘플과 일반 샘플을 더 효과적으로 구별하는 특성 세트를 구성하였습니다.

- **Technical Details**: 접합된 서브 서명은 YARA 규칙에서 인간이 작성한 서명을 기반으로 하며, 이로 인해 특정 악성코드 집합 또는 기능을 식별할 수 있습니다. 연구는 EMBER 2018 데이터셋을 사용하여 기존의 전통적인 특성들을 초월하는 가치를 더할 수 있는지를 실험적으로 입증하였습니다. 추가된 서브 서명은 특색 있고 고유하며, 동시에 자주 발생하는 특성과 결합되어 지배적인 파워-로우( power-law ) 행동을 보였습니다.

- **Performance Highlights**: 기대한 바와는 달리, 특성들이 고유한 악성코드 집합에만 한정되지 않고 실제로 유용하다는 것을 보여주었습니다. 연구는 두 가지 목적을 가진 서브 서명(예: 가상 머신 환경 탐지)이나 광범위한 일반 특성(예: DLL 가져오기)도 발견하였습니다. 이는 수집된 특성이 다양한 맥락에서 적용될 수 있음을 나타냅니다.
...
### Multiple Choice Learning for Efficient Speech Separation with Many Speakers (https://arxiv.org/abs/2411.18497)
- **What's New**: 최근 연구에서는 음성 분리 모델을 교육하는 데 있어 Permutation Invariant Training (PIT) 대신 Multiple Choice Learning (MCL) 프레임워크를 사용하는 것을 제안하였습니다. MCL은 불확실한 작업을 처리하기 위해 설계된 접근법으로, 이 방법이 PIT의 성능을 능가하며 계산에 있어서도 이점이 있음을 실험적으로 증명하였습니다. 이는 가변적인 수의 화자를 처리하거나 비지도 학습 환경에서도 사용할 수 있는 가능성을 열어줍니다.

- **Technical Details**: MCL은 음성 분리 문제를 불확실한 작업으로 재구성하여, 다수의 예측을 제공함으로써 분리된 음성 신호의 추정값을 생성합니다. 모델은 n명의 개별 화자에서 오디오 신호를 입력 받아 y^=(y^1,…,y^n) 형태의 분리된 신호를 생성합니다. MCL은 기존의 PIT 기법보다 더 나은 계산 효율성을 제공하며, 훈련 시간과 성능 측면에서도 경쟁력을 갖습니다.

- **Performance Highlights**: WSJ0-mix 및 LibriMix benchmark에서 MCL의 성능을 입증한 결과, 2명 및 3명의 화자가 포함된 혼합 신호에서 효과적인 성능을 보였습니다. 새로운 평가 지표인 AUC-SDR을 도입하여 분리된 소스 간의 일관성을 평가하며, 특히 다수의 화자를 분리할 때 더욱 중요하게 작용합니다. 이러한 결과는 MCL을 다수 화자 환경에서도 유효한 음성 분리 방법으로 자리잡게 할 것입니다.
...
### Machine learning-based classification for Single Photon Space Debris Light Curves (https://arxiv.org/abs/2411.18231)
- **What's New**: 최근 위성 궤도에서 인위적인 잔해(debris)의 수가 증가하면서 활발한 위성 미션에 대한 충돌 위험이 커지고 있습니다. 이러한 잔해를 특성화하는 것은 매우 중요하며, 이는 모양, 자세 상태, 회전 상태와 같은 정보를 포함할 수 있는 Light Curves (LCs)를 통해 가능합니다. 본 연구는 Single Photon Space Debris LCs를 머신 러닝(ML) 프레임워크를 활용하여 분류하는 데 초점을 맞추고 있습니다.

- **Technical Details**: 본 연구에서는 IWF SPARC(Single Photon Light Curve And Laser Ranging Catalogue)에서 수집된 6500개 이상의 LCs를 사용합니다. 머신 러닝 모델로는 k-Nearest Neighbour (k-NN), Random Forest (RDF), XGBoost (XGB), Convolutional Neural Network (CNN) 분류기를 사용하여 LC의 성능을 비교합니다. 데이터에서 직접 분류하는 대신 자동화된 파이프라인을 통해 특징을 추출한 뒤 모델을 적용합니다.

- **Performance Highlights**: Space Debris LCs의 분류 정확도는 최대 90.7%에 달했습니다. 자동 추출된 특징을 사용했을 때 분류기의 성능이 개선되었으며, 이는 기존의 방법보다도 더 나은 결과를 보였습니다. 이 연구는 Single Photon LC의 유용성을 입증하며 향후 우주 쓰레기 분류와 특성화에 기여할 것으로 기대됩니다.
...
- **What's New**: 이번 논문에서는 Large Language Models (LLMs)을 활용한 공격 탐지 기능의 생성을 평가하고 개선하기 위한 새로운 접근법을 제안합니다. 이 연구는 Retrieval Augmented Generation (RAG)과 Self-Ranking 기술을 통합하여 LLM의 출력 강 robustness을 향상시키는 것을 목표로 합니다. 이를 통해 공격 탐지 성능이 크게 개선되었으며, 실제 사용 사례에서 LLM이 생성한 코드의 한계를 극복할 수 있는 기초를 제공합니다.

- **Technical Details**: 제안한 접근법은 LLM의 비결정적 특징을 활용하여 여러 가지 대안 출력을 생성하고 이를 평가하는 Self-Ranking 메커니즘을 포함합니다. 또한, RAG를 통해 외부 지식원을 통합하여 공격 탐지 기능의 정확성과 신뢰성을 높이는 방법을 적용했습니다. 이 연구는 웹 보안에서 자주 발생하는 Cross-Site Scripting (XSS) 및 SQL Injection (SQLi) 공격에 중점을 두고, 9개의 LLM을 평가하여 성과를 입증했습니다.

- **Performance Highlights**: 실험 결과에 따르면, RAG 통합은 XSS와 SQLi 탐지에서 각각 최대 71%와 43%의 F2-Score 개선을 가져왔습니다. 이는 전통적인 몇 샷 예제에 비해 상당히 높은 성능 향상입니다. 또한, LLM 기반 접근법은 주어진 훈련 세트가 없어도 RAG 소스가 존재하는 한 유용하게 적용될 수 있다는 점에서도 큰 장점을 가지고 있습니다.
...
### A Runtime-Adaptive Transformer Neural Network Accelerator on FPGAs (https://arxiv.org/abs/2411.18148)
Comments:
          arXiv admin note: text overlap with arXiv:2409.14023

- **What's New**: 이 논문은 Transformer Neural Networks (TNN)을 위해 FPGA에서 밀집 행렬 계산을 위한 런타임 적응형 가속기인 ADAPTOR를 소개합니다. 이 가속기는 처리 요소와 온칩 메모리의 활용도를 향상시켜 병렬성을 높이고 레이턴시를 줄입니다. ADAPTOR는 FPGA 플랫폼에 자원을 분산시키는 효율적인 행렬 타일링(matrix tiling)을 통합하고, 계산 효율성과 이동성을 위해 완전히 양자화되었습니다.

- **Technical Details**: ADAPTOR는 DSP 리소스의 최대 병렬 사용을 극대화하기 위해 HLS 도구를 사용하여 설계되었습니다. DSP48 슬라이스에 의해 계산이 수행되며, 필요한 데이터는 BRAM, LUTRAM 또는 처리 요소 내 레지스터에 저장됩니다. 이 아키텍처는 멀티헤드 어텐션 레이어와 피드포워드 네트워크 레이어의 가중치 행렬에 대해 효율적인 타일링 전략을 적용하여 다양한 TNN 모델에 즉시 배포할 수 있습니다.

- **Performance Highlights**: Xilinx Alveo U55C 데이터 센터 카드 및 VC707와 ZCU102와 같은 임베디드 플랫폼에서 평가한 결과, ADAPTOR 설계는 NVIDIA K80 GPU 및 i7-8700K CPU에 비해 각각 1.2$	imes$ 및 2.87$	imes$ 더 전력 효율적임을 보여주었습니다. 또한, 기존의 FPGA 기반 가속기들과 비교하여 1.7에서 2.25$	imes$의 속도 향상을 달성했습니다.
...
### The Bigger the Better? Accurate Molecular Potential Energy Surfaces from Minimalist Neural Networks (https://arxiv.org/abs/2411.18121)
- **What's New**: 이 논문에서는 KerNN이라는 새로운 방식의 Potential Energy Surface (PES) 표현 기법을 도입하였습니다. KerNN은 kernel과 neural network를 결합하여, 최신 neural network PES에 비해 학습 가능한 파라미터의 수를 상당히 줄임으로써 훈련 및 평가 시간을 몇 배나 단축시킵니다. 또한, 커널을 특징으로 사용하여 학습 데이터의 범위를 초과한 예측 능력을 향상시켜, NN 기반 PES의 일반적인 문제를 해결합니다.

- **Technical Details**: KerNN은 하나의 입력층, 두 개의 숨겨진 층, 그리고 하나의 출력층으로 구성된 소규모 feed-forward NN입니다. 분자 설명자는 1차원 역방향 거듭제곱 재생 커널을 사용하며, 이는 기준 구조와 쿼리 구조 간의 원자 간 거리 사이의 유사성을 측정하는 데 효과적입니다. 시스템의 총 잠재 에너지가 KerNN의 출력으로 제공되며, 힘은 역모드 자동 미분을 통해 계산됩니다.

- **Performance Highlights**: KerNN은 spectroscopy와 반응 역학 분야에서 뛰어난 성능을 보이며, 테스트 세트 통계 및 관측 가능성에서 우수한 결과를 보여줍니다. 특히, H2CO에 대한 훈련에서 비대칭 및 대칭 설명자를 활용하여 다양한 데이터 세트 크기에서도 유의미한 성과를 얻었으며, dipole moments를 예측하는 데도 응용되었습니다.
...
### Using different sources of ground truths and transfer learning to improve the generalization of photometric redshift estimation (https://arxiv.org/abs/2411.18054)
Comments:
          10 pages, 4 figures, 2 tables, accepted to NeurIPS 2024 Workshop ML4PS

- **What's New**: 이번 연구에서는 galaxy redshift 예측을 개선하기 위한 새로운 방법을 제시합니다. 이는 photometry와 spectroscopy에서 얻은 각각의 ground truth를 결합하여 redshift 모델의 일반화 가능성을 높이려는 시도를 포함합니다. 기존의 방식은 정확하지만 sample의 범위가 제한된 spectroscopic redshift에 의존해왔습니다. 연구진은 COSMOS2020 조사를 기반으로 한 TransferZ 데이터세트를 사용하였습니다.

- **Technical Details**: TransferZ는 35개의 이미징 필터에서 얻은 photometric redshift 추정치를 포함하는 데이터세트로, 다양한 galaxy 종류와 색상을 아우릅니다. 연구진은 기본 신경망을 TransferZ에서 훈련한 다음, 더욱 정확한 spectroscopic redshift를 가진 GalaxiesML 데이터세트에서 transfer learning을 통해 성능을 개선했습니다. 또한, TransferZ와 GalaxiesML 데이터세트를 결합하여 신경망을 훈련시켜 bias, RMS 오차, catastrophic outlier 비율을 개선하였습니다. 전반적으로 이 방식은 성능을 일반화하는 데 기여하고 있습니다.

- **Performance Highlights**: TransferZ와 GalaxiesML을 결합한 방법은 GalaxiesML에서 5배의 bias 감소와 1.5배의 RMS 오류 감소를 보여주었습니다. 하지만 TransferZ 데이터에서 평가할 경우, RMS와 bias에서 성능 저하를 발견했습니다. 이러한 결과는 우리가 얻은 방법이 우주론적 요구를 충족할 수 있음을 나타냅니다. 이 연구는 데이터 품질의 중요성을 강조하며, 다양한 galaxy 샘플에서 redshift 예측의 일반화 가능성을 평가하고 있습니다.



### Generative Semantic Communication for Joint Image Transmission and Segmentation (https://arxiv.org/abs/2411.18005)
Comments:
          6 pages, 7 figures

- **What's New**: 이 논문에서는 기존의 단일 작업 재구성에서 벗어나 이미지 재구성과 분할 작업을 지원하는 새로운 생성적 수의적 통신 시스템을 제안합니다. 이 시스템은 송신기와 수신기 모두에 수학적 지식 기반(semantic knowledge base, KBs)을 구축하여 작동하며, 각 KB는 소스 KB와 작업 KB로 구성됩니다. 이 접근법은 여러 작업에 대한 모델의 적응성과 일반화를 고려하여, 데이터 전송의 효율성을 크게 개선합니다.

- **Technical Details**: 제안하는 시스템은 송신기의 소스 KB가 계층 구조의 Swin-Transformer를 통해 입력 이미지에서 다중 레벨의 특징을 추출하고, 수신기의 소스 KB는 계층적 잔차 블록(hierarchical residual blocks)을 사용하여 작업별 지식을 생성합니다. 각 작업 KB는 작업 요구사항에 따라 기능 선택을 용이하게 하고, 이를 통해 원활한 정보 전송을 보장합니다. JSCC(합동 원천 및 채널) 인코더와 두 개의 작업별 JSCC 디코더를 통해 이미지 재구성과 분할 작업을 수행합니다.

- **Performance Highlights**: 실험 결과, 제안한 다중 작업 생성적 수의적 통신 시스템은 피크 신호 대 잡음 비율(peak signal-to-noise ratio, PSNR)과 분할 정확도(accuracy)에 있어 이전의 단일 작업 통신 시스템들을 능가하는 성능을 보였습니다. 특히 생성적 확산 모델이 이미지 재구성 작업을 위한 JSCC 디코더를 구축하는 데 사용되어, 개선된 성능 결과에 기여하였습니다. 전반적으로, 이 연구는 멀티태스크 환경에서의 통신 효율성을 향상시키는 새로운 가능성을 제시합니다.
...
### On the ERM Principle in Meta-Learning (https://arxiv.org/abs/2411.17898)
Comments:
          20 pages

- **What's New**: 이번 논문에서는 메타학습(meta-learning) 프레임워크를 통해 여러 작업(task)과 해당 작업의 예시 예제 수(m) 변화에 따른 예상 오차를 평가하는 새로운 연구를 제시합니다. 특히, 레이블된 예시(n)를 통해 훈련된 전통적인 감독 학습(supervised learning) 알고리즘을 기반으로, 메타 학습에서 학습 곡선(learning curve)이 다차원 학습 표면(learning surface)으로 확장되는 방식을 설명합니다. 이런 확장은 In-context learning, hypernetworks 등 여러 현대 문제에 적용됩니다. 저자들은 m이나 n이 무한대로 증가할 때 메타-경험적 리스크 최소화(meta-Empirical Risk Minimizers)의 분포 비자유(distribution-free) 학습 표면의 특성을 규명하였습니다.

- **Technical Details**: 메타 학습에서는 n개의 작업(task)에 걸쳐 학습하는데, 각 작업에서는 m개의 예시를 사용합니다. 이 논문은 메타 분포(meta-distribution)에서 독립적으로 샘플링된 도메인 문제를 고려하고, 주어진 수의 클래스가 유한한 경우의 메타 학습 가능성을 논의합니다. 특히 메타-ERM(메타-경험적 리스크 최소화) 알고리즘의 학습 표면을 통해, 특정 작업 당 필요한 예시 수와 오차를 연관 짓는 조건을 제시합니다. 이를 통해, 적은 수의 예시로도 성공적인 학습이 가능함을 보여주는 다양한 경우를 분석합니다.

- **Performance Highlights**: 이 연구는 메타 학습에서의 이상적인 도메인 수(n)와 예시 수(m)에 따른 학습 성능을 깊이 있게 조명합니다. 특정 메타-가설 클래스에서의 에러를 근본적으로 제어하기 위한 m의 극한 동작을 명확히 하여, 유한한 예시 수로도 에러를 줄일 수 있는 수학적 조건을 도출했습니다. 따라서 연구 결과들은 메타 학습의 효율성과 유연성을 높이는데 기여하며, 동시에 향후 연구 방향에 대한 기초 자료를 제공합니다.
...
### Rate-Informed Discovery via Bayesian Adaptive Multifidelity Sampling (https://arxiv.org/abs/2411.17826)
Comments:
          Published at CoRL 2024: this https URL

- **What's New**: 이 논문에서는 자율주행차(AV)의 안전성을 확보하기 위한 새로운 방법으로 Bayesian adaptive multifidelity sampling (BAMS)을 제안합니다. BAMS는 적응형 Bayesian 샘플링을 활용하여 성능을 효율적으로 추정하고 잠재적인 실패 사례를 발견합니다. 이를 통해 전통적인 방법으로는 발견하기 어려운 새로운 시나리오를 파악할 수 있습니다.

- **Technical Details**: BAMS는 AV 안전성을 평가하기 위해 발생률 추정을 효율적으로 수행하며, 이를 위해 다양한 질의의 시뮬레이션 매개변수 분포 P를 고려합니다. 또한, 나이브 Monte Carlo 방식은 비효율적이기 때문에, IS 기법을 활용하여 성능 지표의 낮은 지역에 높은 확률로 집중하여 샘플링 합니다. BAMS는 저비용의 저충실도 시뮬레이터와 고충실도 시뮬레이터를 결합하여 효율적으로 성능 개선에 필요한 피드백을 제공합니다.

- **Performance Highlights**: 실제 AV 데이터를 사용한 결과, BAMS는 Monte Carlo (MC) 및 Importance Sampling (IS) 기법에 비해 10배 더 많은 문제를 발견했습니다. 또한, 성능 추정의 분산은 MC 및 IS보다 각각 15배와 6배 더 좁은 것으로 나타났습니다. 이러한 성과는 AV 개발 및 안전성 평가에서 BAMS의 효율적인 가능성을 보여줍니다.
...
### New Test-Time Scenario for Biosignal: Concept and Its Approach (https://arxiv.org/abs/2411.17785)
Comments:
          Findings paper presented at Machine Learning for Health (ML4H) symposium 2024, December 15-16, 2024, Vancouver, Canada, 6 pages

- **What's New**: 이 논문은 Online Test-Time Adaptation (OTTA)이라는 새로운 개념을 도입하여, 혈압을 예측하는 의료 분야의 실제 환경에서 모델을 강화하는 방법을 제안합니다. OTTA는 테스트 중에 라벨이 없는 데이터로 사전 훈련된 모델을 업데이트함으로써 모델의 견고함을 높이는 데 중요한 역할을 합니다. 이 새로운 테스트 시간 시나리오는 주기적인 보정이 가능한 비확정적인 표시 데이터를 통한 모델의 적응을 가능하게 합니다.

- **Technical Details**: 제안된 프레임워크는 이중 큐 버퍼와 가중 배치 샘플링을 활용하여 라벨이 있는 및 없는 데이터에서 균형 잡힌 학습을 지원합니다. 이를 통해 혈압 예측을 위한 다양하고 연속적인 생체 신호에서 모델의 적응성과 정확성을 높입니다. 실험에서는 PulseDB 데이터세트를 활용하여 다양한 라벨 주입 빈도 조건을 시뮬레이션하고, 동적 환경에서의 모델 성능을 평가합니다.

- **Performance Highlights**: 실험 결과, 주기적으로 라벨을 주입하고 초기 라벨 데이터셋 크기를 늘리면 혈압 예측의 모델 적응성과 정확도가 향상되는 것으로 나타났습니다. 특히, 버퍼 크기와 샘플링 전략의 절충점이 성능에 미치는 영향을 강조하며, 실시간으로 데이터를 처리하는 능력과 예측 정확도를 동시에 확보할 수 있음을 보여줍니다.



### KACDP: A Highly Interpretable Credit Default Prediction Mod (https://arxiv.org/abs/2411.17783)
- **What's New**: 이 논문에서는 금융 분야에서 개인 신용 리스크 예측을 위해 Kolmogorov-Arnold Networks (KANs) 기반의 새로운 방법을 소개합니다. 기존 방법들이 높은 차원 및 비선형 데이터 처리에서 해석 가능성과 투명성 부족이라는 문제를 가지는 반면, KANs는 학습 가능한 활성화 함수와 비선형 가중치 없이 고차원의 복잡한 데이터를 효과적으로 처리할 수 있는 잠재력을 가지고 있습니다.

- **Technical Details**: KANs는 Kolmogorov-Arnold 표현 정리에 기반한 신경망 아키텍처로, 다차원 함수를 단일 변수를 결합한 형태로 표현합니다. 이는 강력한 표현 능력을 유지하면서도 높은 해석 가능성을 제공합니다. 특히 KANs는 재무 데이터에 대해 유연하게 적응할 수 있으며, 이는 금융 산업이 요구하는 모델의 투명성과 해석 가능성에 부합합니다.

- **Performance Highlights**: KACDP 모델은 ROC_AUC 및 F1 값과 같은 주요 성능 지표에서 기존의 통제 모델 및 기계 학습 모델보다 우수한 성능을 보입니다. 실험 결과를 통해 KACDP 모델의 특징, 성능 향상, 설명 가능성을 명확히 제시하며, 이는 금융 기관의 의사 결정에 효과적인 도구로 작용할 것으로 기대됩니다.



### MetaGraphLoc: A Graph-based Meta-learning Scheme for Indoor Localization via Sensor Fusion (https://arxiv.org/abs/2411.17781)
- **What's New**: MetaGraphLoc는 실내 위치 확인의 정확성을 높이기 위해 인간 내장형 센서 데이터와 무선 신호 강도 지표(RSSI) 측정을 융합한 새로운 시스템입니다. 이 시스템은 그래프 신경망(GNN)과 메타 학습(meta-learning)을 활용하여 환경 변화에 빠르게 적응할 수 있는 능력을 갖추고 있습니다. 이를 통해 기존의 방법들이 부족했던 정밀도를 향상시킬 수 있습니다.

- **Technical Details**: MetaGraphLoc는 데이터를 융합하기 위해 새로운 GNN 아키텍처를 활용하며, 동적인 엣지 구성(dynamic edge construction, DEC) 기법을 통해 엑세스 포인트와 데이터 패턴 간의 공간 관계를 효과적으로 포착합니다. 이 프레임워크는 제한된 데이터 수집으로도 새로운 환경에 적응이 가능하게 하는 메타 학습 기법을 포함하고 있습니다. 이를 통해 데이터 수집 노력과 보정을 상당히 줄이면서도 높은 정확도를 유지할 수 있습니다.

- **Performance Highlights**: MetaGraphLoc의 데이터 융합 기술로 인한 위치 확인 오차는 15.92% 감소하며, GNN이 기존의 딥 뉴럴 네트워크(DNN)에 비해 최대 30.89% 더 나은 성능을 보였습니다. 이러한 결과는 실내 네비게이션 및 IoT 네트워크에 적합한 혁신적인 솔루션으로서 MetaGraphLoc의 가능성을 한층 더 부각시킵니다.
...
### Deciphering Acoustic Emission with Machine Learning (https://arxiv.org/abs/2411.17755)
- **What's New**: 이번 연구에서는 기계 학습(Machine Learning)을 활용하여 미세한 구조에서 발생하는 전위(Dislocation) 눈사태의 세부 정보를 음향 방출(Acoustic Emission) 데이터만으로 추론할 수 있는 방법을 제안합니다. 이를 통해 미세 기둥 압축 실험(micropillar compression test)에서 발생하는 힘-시간 응답(force-time response)을 예측하고, 개별 변형 사건의 크기도 예측할 수 있게 됩니다. 이러한 접근 방식은 음향 데이터를 통해 보다 심층적인 이해를 가능하게 합니다.

- **Technical Details**: 연구에서는 음향 방출(AE) 신호를 기반으로 전위 눈사태의 미세한 세부 사항을 추론하는 데 중점을 두고 있습니다. AE 신호는 저장된 탄성 에너지의 방출로 유도된 탄성파로, 재료 내부의 비가시적인 눈사태 활동의 지표로 작용합니다. 기계 학습 모델은 다양한 설명 변수를 사용하여 이러한 데이터를 처리하고 예측의 중요성을 분석합니다.

- **Performance Highlights**: 본 연구에서의 기계 학습 접근법은 음향 방출 신호와 힘 간의 관계를 효과적으로 모델링함으로써 예측력이 뛰어난 성과를 나타냈습니다. 전위 눈사태의 시간적 위치와 개별 변형 사건의 크기를 높은 정밀도로 예측할 수 있었으며, 이 방법은 다른 샘플 크기로도 이전 가능한 가능성을 보였습니다. 이는 기계 학습의 잠재력을 통한 여러 고급 응용 프로그램으로의 확장을 제시합니다.



### Path Loss Prediction Using Deep Learning (https://arxiv.org/abs/2411.17752)
Comments:
          5 pages, 5 figures, 3 tables

- **What's New**: 본 논문에서는 고해상도 장애물 높이 맵으로부터 자동으로 특징 추출을 수행하는 경로 특정(path-specific) 경로 손실(prediction loss) 예측 방법을 제안합니다. 이 방법은 Convolutional Neural Networks (CNNs)를 사용하여 특성을 추출하며, 도출된 장애물 메트릭스 없이 다양한 환경에서 낮은 예측 오류를 달성합니다. 특히, 기존의 경로 손실 모델들보다 더 포함된 정보를 바탕으로한 개선된 예측 성능을 나타냅니다.

- **Technical Details**: RF 측정 데이터와 지역적 고도/클러스터 데이터 두 가지 종류의 데이터를 사용하여 경로 손실 모델을 훈련합니다. 데이터는 영국의 OFCOM으로부터 수집된 820만 개의 측정치와 여러 도시의 디지털 고도 모델을 기반으로 하며, 이 모델은 두 가지 차원(1D 및 2D)의 표면 고도 데이터를 포함합니다. 본 연구는 두 가지 차원의 고해상도 클러스터 맵을 사용하여 깊은 신경망(deep neural networks)을 통해 경로 손실을 직접 예측합니다.

- **Performance Highlights**: 본 연구에서 제시한 경로 손실 모델은 기존 모델에 비해 낮은 RMSE(평균 제곱근 오차)를 도출하여, 특히 비 가시선(non-LOS) 환경에서의 예측 정확도를 향상시킵니다. 이 방법은 다양한 링크 거리와 무선 주파수에서 활용될 수 있으며, 고차원 공간 특징을 신뢰성 있게 추출하여 경로 손실 예측의 일반화 가능성을 증진합니다. 향후 연구에서는 이 접근법을 통해 더욱 다양한 환경에서 시험할 수 있는 가능성을 보여줍니다.



### Deployment of ARX Models for Thermal Forecasting in Power Electronics Boards Using WBG Semiconductors (https://arxiv.org/abs/2411.17748)
Comments:
          in French language. Conf{é}rence des jeunes chercheurs en g{é}nie {é}lectrique, CNRS; GDR SEEDS, Jun 2024, Le croisic, France

- **What's New**: 이번 연구는 Wide Bandgap (WBG) 반도체의 열 관리 문제를 다루고 있으며, ARX 파라메트릭 모델의 사용을 강조합니다. 이 모델은 부품 두께 차이나 재료의 물리적 특성에 대한 깊은 이해가 필요 없이 온도를 정확하게 예측할 수 있습니다. 실험 측정에만 의존함으로써, 기존의 FEM 시뮬레이션 및 전통적인 열 모델에 대한 신뢰할 수 있는 대안으로 자리잡고 있습니다.

- **Technical Details**: 제안된 ARX 파라메트릭 모델은 시스템 식별을 크게 단순화하면서도 높은 결과 정확도를 보장합니다. 이러한 모델은 실험 데이터를 기반으로 하여 열 관리 시스템을 최적화하는 데 효과적입니다. WBG 반도체의 특성과 그에 따른 열 전이 문제를 해결하는 데 통찰력을 제공합니다.

- **Performance Highlights**: ARX 모델은 기존의 방법들보다 더 쉬운 접근성과 높은 정확성을 제공하여, 열 관리의 복잡성을 줄입니다. 연구 결과는 실험적 증거를 기반으로 하며, 이러한 접근 방식이 실제로 시스템 성능을 향상시키는 기회를 제공한다는 점에서 중요한 의미를 갖습니다.



### Comparison of Tiny Machine Learning Techniques for Embedded Acoustic Emission Analysis (https://arxiv.org/abs/2411.17733)
Comments:
          Conference Presentations (Accepted) at IEEE 10th World Forum on Internet of Things. "this https URL

- **What's New**: 이 논문은 기계 학습 접근 방식과 다양한 입력 데이터 형식을 비교하여 음향 방출(AE) 신호 분류를 수행합니다. AE 신호는 구조적 건강 모니터링에서 유망한 모니터링 기법으로, 손상 메커니즘에 따라 다양한 AE 신호를 분류하는 데 기계 학습이 효과적인 데이터 분석 방법으로 입증되었습니다. 그러나 어떤 접근 방식이 선호되는지에 대한 연구가 부족했으며, 이 연구는 IoT 시스템에 최적화된 모델 배포를 목표로 합니다.

- **Technical Details**: 논문에서는 음향 방출 신호에서 특징(feature)을 추출하고 선택하며, 다양한 입력 데이터 시나리오에 대해 최적화된 신경망(neural network) 모델을 설계합니다. 모델 배포를 위해 선택된 하드웨어는 Arduino Nano 33 BLE Sense로, 저전력 nRF52840 마이크로컨트롤러를 기반으로 하고 있습니다. 신호 분류에 있어 15,000개의 AE 이벤트로 구성된 공용 데이터셋을 사용하고 있으며, 각각의 손상 유형에 대해 균형 잡힌 데이터를 제공합니다.

- **Performance Highlights**: 모든 모델은 99% 이상의 높은 분류 정확도를 달성할 수 있지만, 임베디드 특징 추출 방식은 계산 비용이 많이 듭니다. 반면, 원시 AE 신호를 입력으로 사용하는 모델은 빠른 처리 속도를 보이며, 가장 낮은 에너지 소비를 나타냅니다. 그러나 이러한 접근 방식은 더 큰 메모리 요구 사항을 동반하게 됩니다.
...
### Analytic Continuation by Feature Learning (https://arxiv.org/abs/2411.17728)
Comments:
          8 pages, 9 figures

- **What's New**: 새로운 연구에서 Feature Learning Network (FL-net)을 제안하여, 전통적인 방법보다 스펙트럼 함수 예측 정확도를 최소 20% 향상시켰습니다. 이 네트워크 아키텍처는 특성(Features)을 학습하여 분광 밀도와 시간 응답을 효과적으로 추출하는 데 초점을 맞추었습니다.

- **Technical Details**: FL-net은 G(iωₙ)에서 A(Ω)로 직접 매핑하는 대신, 데이터셋에서 특성을 먼저 학습한 후, 이러한 특성과 스펙트럼 함수를 연결하는 과정을 포함합니다. 숨겨진 차원(Hidden dimensionality)의 증가가 정확도 저하를 초래할 수 있으며, 이를 통해 모델의 강건성(Robustness)을 분석하는 방법론도 제시됩니다.

- **Performance Highlights**: FL-net은 개별 피크 모델링에서의 신뢰성을 높이고, 네트워크의 강건성을 평가하여 다양한 데이터셋에서 성능을 극대화합니다. 이 연구는 신경망의 효율성을 기반으로 하여 복잡한 스펙트럼 예측 문제를 해결하는 데 중요한 통찰을 제공합니다.



### EQNN: Enhanced Quantum Neural Network (https://arxiv.org/abs/2411.17726)
Comments:
          in Chinese language

- **What's New**: 이번 연구에서는 향상된 양자 신경망(Enhanced Quantum Neural Network, EQNN)을 제안합니다. EQNN은 양자 컴퓨팅에 더 적합한 값 범위로 입력 변수를 매핑하기 위해 설계된 향상된 특성 맵(Enhanced Feature Map, EFM)을 포함하고 있습니다. 이 연구는 모바일 데이터 사용 예측을 사례로 하여 사용자에게 적합한 요금제를 추천하는 방안을 탐구합니다.

- **Technical Details**: 양자 신경망(QNN)은 입력을 양자 회로에 매핑하기 위해 특성 맵(Feature Map, FM)을 사용하고, 변동 모델(variational models)을 통해 매개변수 값을 조정하여 회귀(regression) 및 분류(classification) 작업에 활용됩니다. 그러나 특정 응용 문제에 적합한 FM 설계는 상당한 도전 과제가 됩니다.

- **Performance Highlights**: 제안된 EQNN은 현재 주류 QNN과 비교되었습니다. 실험 결과, EQNN은 더 적은 양자 논리 게이트와 함께 더 높은 정확도를 달성하며, 다양한 최적화 알고리즘 하에서 최적의 솔루션으로 더 빠르게 수렴하는 성능을 나타냈습니다.



### Automatic EEG Independent Component Classification Using ICLabel in Python (https://arxiv.org/abs/2411.17721)
- **What's New**: 이 논문에서는 EEG 데이터 처리에 있어 가장 널리 사용되는 소프트웨어인 EEGLAB의 플러그인 기능인 ICLabel에 대해 설명합니다. 전통적인 MATLAB 기반 ICLabel의 한계를 극복하고, Python 버전의 ICLabel을 개발하여 다양한 플랫폼에서의 호환성을 향상시켰습니다. 기존 ICLabel의 신경망 구조가 열린 소스 Octave 해석기와 호환되지 않아 발생했던 문제를 해결하였습니다.

- **Technical Details**: EEGLAB의 ICLabel은 Independent Component Analysis (ICA)를 이용해 EEG 데이터를 분해한 후, 결과적으로 생성된 독립 구성 요소(Independent Components, ICs)를 분류하는 중요한 도구입니다. 연구팀은 14명의 피험자 데이터를 기반으로 ICLabel의 MATLAB과 Python 구현을 비교하였으며, ICLabel은 7개의 구성 요소 클래스에 대한 분류 가능성을 반환합니다. 실험에서 Python과 MATLAB 간의 IC 분류 결과는 거의 동일하며, 분류 비율 차이는 0.001% 이하로 매우 미세하게 나타났습니다.

- **Performance Highlights**: 이 연구를 통해 개발된 Python 버전의 ICLabel은 MATLAB 버전과의 사실상 동일한 성능을 보여주었습니다. 이는 클라우드 처리 환경이나 호환되지 않는 플랫폼에서의 EEG 데이터 처리가 더욱 용이해질 것임을 의미합니다. 이와 같은 발전은 신경망 분석 및 EEG 연구의 효율성을 높일 수 있는 기회를 제공합니다.
...
### Comprehensive Methodology for Sample Augmentation in EEG Biomarker Studies for Alzheimers Risk Classification (https://arxiv.org/abs/2411.17717)
Comments:
          20 pages, 7 figures, 2 tables

- **What's New**: 이번 연구는 알츠하이머병(AD)의 위험 분류를 개선하기 위해 EEG(뇌전도) 신호 처리를 통합하는 새로운 접근 방식을 제안합니다. 저자들은 데이터베이스에서 수집한 샘플을 통합하고 표본 크기를 늘리는 방법을 모색했습니다. 이 방법론은 알츠하이머병 위험 검사에 대한 신뢰성을 높이는 것을 목표로 하고 있습니다.

- **Technical Details**: 본 연구에서는 EEG 사전 처리(EEG preprocessing), 특징 추출(feature extraction), 조정(harmonization), 성향 점수 매칭(propensity score matching, PSM) 등의 기술을 사용하여 건강한 비보유자와 무증상 E280A 변이 보유자 간의 균형을 맞추었습니다. 네 개의 데이터베이스에서 수집한 데이터를 조정하여 연령과 성별과 같은 공변량을 유지하며 사이트 효과를 보정했습니다. 또한, 2:1, 5:1, 10:1의 PSM 비율을 적용하여 샘플 크기가 모델 성능에 미치는 영향을 평가했습니다.

- **Performance Highlights**: PSM을 통해 샘플 크기를 균형 있게 조정한 결과, 분류 정확도가 0.92에서 0.96 사이로 나타났습니다. 이 접근 방식은 제한된 샘플로도 정확한 위험 식별을 가능하게 합니다. 데이터 처리 및 조정 기술의 통합은 알츠하이머병 위험 분류의 정확성을 개선하며, 다른 신경 퇴행성 질환에도 적용할 수 있는 가능성을 제시합니다.
...
### Quantity versus Diversity: Influence of Data on Detecting EEG Pathology with Advanced ML Models (https://arxiv.org/abs/2411.17709)
Comments:
          20 pages, 17 figures

- **What's New**: 이번 연구는 EEG 병리 탐지에서 다양한 기계 학습 모델의 성능에 대한 데이터 양과 다양성의 영향을 조사합니다. 연구팀은 Temple University Hospital의 2,993개 레코딩과 Elmiko Biosignals sp. z o.o.의 55,787개 레코딩으로 구성된 두 개의 EEG 데이터 세트를 활용했습니다. 후자의 데이터 세트는 39개 병원에서 수집된 다양한 환자 데이터를 포함하고 있으며, 이는 공개적으로 사용 가능한 가장 큰 EEG 코퍼스입니다.

- **Technical Details**: 연구에서는 간단한 기계 학습 모델과 고급 모델을 EEG 데이터의 양 및 다양성에 따라 대조하였습니다. TUH 데이터 세트와 Elmiko에서 제공한 대규모 이질적 데이터 세트를 사용했습니다. 모델링에는 수작업으로 특징을 이용하는 비신경 접근법과 원시 EEG 신호에서 특징을 추출하는 신경망이 포함됩니다. 특히, 주의 메커니즘(attention mechanism)이나 트랜스포머 아키텍처(transformer architecture)를 사용하는 네트워크도 포함되어 있습니다.

- **Performance Highlights**: 분석 결과, 작은 데이터 세트에서 일관된 데이터가 다양한 모델이 높은 정확도를 달성하는 데 기여한다는 사실이 확인되었습니다. 그러나 병리 상태, 기록 프로토콜 및 라벨 기준의 변동성은 성능 저하를 야기할 수 있습니다. 특히, 주의 메커니즘에 기반한 신경망은 데이터 다양성을 보완하는 데 효과적임을 보여주었습니다. 이 연구는 ML 아키텍처 설계와 의료 데이터 세트 수집 개선에 기여할 것으로 기대됩니다.
...
### Probabilistic Forecasting of Radiation Exposure for Spacefligh (https://arxiv.org/abs/2411.17703)
- **What's New**: 이 논문에서는 NASA의 Artemis 1 임무의 일환으로 발사된 BioSentinel 위성의 다중 모드 시계열 데이터를 활용하여 BLEO에서의 방사선 노출을 예측하는 머신러닝 접근법을 제안합니다. 특히, 전세계 태양 이미지를 사용하여 방사선 노출을 예측하는 것은 이번 연구가 처음입니다. 이번 연구의 목적은 예측 불확실성을 제공하여 우주 비행사들의 의사결정을 개선하는 것입니다.

- **Technical Details**: 우리는 Solar Dynamics Observatory(SDO)에서 측정한 태양의 다양한 극자외선 파장에서의 이미지를 포함하여 GOES 위성의 X선 플럭스 측정치와 BioSentinel의 방사선 측정치를 통합하여 모델을 구축했습니다. 모델은 SDO 이미지를 인코딩하는 합성곱 신경망(CNN)과 방사선 예측을 위한 시계열 입력을 처리하는 구조로 구성되어 있습니다. 예측 성과는 SPE 이벤트의 발생 전후 방사선 패턴을 정확히 예측하는 능력에 기반하고 있습니다.

- **Performance Highlights**: 모델은 SPE 이벤트로 인한 방사선 증가의 발생 시점을 예측하며, 이벤트 후 방사선 감소 프로필을 정확하게 예측하는 능력을 보여줍니다. 이를 통해 NASA의 기존 방사선 모니터링 도구와 통합하여 우주 비행사들의 안전성을 증대시킬 수 있는 가능성을 제시합니다. 또한, 방사선 노출을 예측하는 주목할 만한 혁신적인 접근으로, 향후 태양계 다양한 지역의 방사선 수준을 예측하기 위한 연구로 확장될 수 있는 잠재력을 지니고 있습니다.
...
- **What's New**: 본 연구에서는 심전도(ECG) 데이터를 사용하여 심장 부정맥을 탐지하기 위한 새로운 접근 방식을 제시합니다. 다양한 방식으로 긍정 샘플을 정의하고 이 방법들의 성능을 비교하여 최상의 결과를 도출하는 방법을 모색합니다. 특히, 환자 고유성과 무관한 정보를 활용한 표현 학습이 부정맥 탐지에서 더 정확하다는 점을 확인하였습니다.

- **Technical Details**: 이 연구의 접근 방식은 환자 특정 및 환자 비특정 샘플 정의를 포함하여 두 가지 주요 단계로 구성됩니다. 심전도의 표현을 contrastive learning으로 학습한 후, 다운스트림 분류 작업으로 각 표현의 품질을 평가합니다. 특히, 세 가지 주요 부정맥 클래스와 하나의 정상 심장 상태 클래스를 분류하는 작업에 초점을 맞추었습니다.

- **Performance Highlights**: Contrastive learning을 활용한 결과, 모든 방법이 기존 감독된 기법보다 우수한 성능을 발휘했습니다. 환자 무관한 표현은 기존의 환자 특정 표현보다 더 높은 정확도를 보이였으며, 자동으로 추출된 ECG 파형 속성 기반의 새로운 매칭 전략이 효과적인 것으로 나타났습니다. 이를 통해 라벨이 없는 ECG 데이터에서 효과적으로 표현을 학습할 수 있음을 보여주었습니다.



### When Is Heterogeneity Actionable for Personalization? (https://arxiv.org/abs/2411.16552)
- **What's New**: 이번 논문에서는 A/B 테스트의 최적 정책을 넘어서는 개인화 정책 및 타겟팅 전략을 개발하여 효과성을 극대화할 수 있는지에 대한 통계 모델을 제시합니다. 연구자들은 '적용 가능 이질성'(actionable heterogeneity)이라는 개념을 도입하여 개인화가 최적 정책보다 더 나은 성과를 나타낼 수 있는 조건을 규명하고 있습니다. 이 모델은 여러 인구 통계학적 변수에 기반한 맞춤형 개입을 통해 개인화의 이점을 설명합니다.

- **Technical Details**: 적용 가능 이질성을 정량화하기 위해, 연구자들은 세 가지 인구 통계학적 파라미터인 '처치 내 이질성'(within-treatment heterogeneity), '처치 간 상관관계'(cross-treatment correlation), 그리고 '평균 효과의 변동성'(variation in average responses)을 고려한 통계 모델을 개발했습니다. 이 모델은 A/B 테스트에서 서로 다른 개입의 효과 간 교차 상호작용을 시각화하고 이질성을 평가하여 개인화의 잠재적 이점을 예측할 수 있습니다.

- **Performance Highlights**: 연구팀은 두 개의 대규모 필드 실험을 통해 다섯 가지 개인화 접근법을 적용해 보았으며, 첫 번째 실험에서는 18%의 성과 향상을 얻었고 두 번째 실험에서는 4%의 보수적인 성과 향상을 기록했습니다. 이 모델을 통해 처리 내 이질성의 차이가 개인화의 이점에 큰 영향을 미친다는 것을 발견하였고, 처치 간 상관관계를 줄이는 것이 개인화의 이점을 더욱 증가시킬 수 있는 잠재력이 크다는 것을 제시했습니다.

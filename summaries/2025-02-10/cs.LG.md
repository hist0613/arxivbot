...
### In-context denoising with one-layer transformers: connections between attention and associative memory retrieva (https://arxiv.org/abs/2502.05164)
- **What's New**: 본 논문에서는 "in-context denoising"라는 작업을 소개하며, 이는 attention 기반 아키텍처와 밀집 연상 기억 네트워크(대칭적 Hopfield 네트워크) 간의 연관성을 정제하는 데 중점을 둡니다. Bayesian 구조를 사용하여 특정한 제한된 denoising 문제를 셀프-주목 기법인 single-layer transformer를 통해 최적해를 찾을 수 있음을 이론적으로 및 경험적으로 보여줍니다. 이 연구는 연상 기억 모델이 in-context learning의 연구에서 얼마나 중요한지를 입증합니다.

- **Technical Details**: in-context denoising 문제는 L개의 예제를 바탕으로 denoising을 수행하는 구조로, 문맥(context)에서 제시된 noisy sample을 바탕으로 원본 데이터를 추정하는 과정입니다. 각각의 훈련된 attention layer는 단 하나의 gradient descent 업데이트를 수행하는데, 이 때 문맥 토큰이 연상 기억의 역할을 하며, 쿼리 토큰이 초기 상태로 작용합니다. 이 방식은 최근 연구에서 transformers가 gradient descent를 내부적으로 구현하는 모습을 관찰한 것과 연결됩니다.

- **Performance Highlights**: 단일 attention head를 가진 single-layer transformers가 특정 denoising 문제를 최적으로 해결할 수 있는 충분한 표현력을 가지고 있음이 입증되었습니다. 기존의 무작위 가중치로부터 시작한 훈련은 Bayes 최적 예측기를 복구할 수 있음을 경험적으로 보여주었습니다. 또한, 훈련된 attention layers는 밀집 연상 기억 네트워크에 매핑되어, 표준 기억 검색의 패러다임을 넘어서는 새로운 in-context learning 작업을 해결할 수 있음을 시사합니다.
...
### Efficient distributional regression trees learning algorithms for calibrated non-parametric probabilistic forecasts (https://arxiv.org/abs/2502.05157)
- **What's New**: 본 논문에서는 WIS (weighted interval score) 또는 CRPS (continuous ranked probability score) 손실 함수를 사용하는 확률적 회귀 트리를 학습하기 위한 새로운 알고리즘을 소개합니다. 이러한 알고리즘들은 계산 효율성을 높이기 위해 min-max heaps, weight-balanced binary trees, 그리고 Fenwick trees와 같은 기존 데이터 구조를 적절히 활용합니다. 각적 모델이 아닌 non-parametric 모델을 이용해 출력 변수의 조건부 누적 분포 함수(cumulative distribution function)를 직접 학습하는 접근법이 더욱 정밀한 예측을 가능하게 합니다.

- **Technical Details**: 확률적 회귀 트리는 분포적 회귀(distributional regression)와 quantile 회귀(quantile regression)를 결합하여 동시에 여러 quantile을 추정하는 방법을 제공합니다. WIS는 전통적인 회귀 방법들과 달리 response variable의 전체 조건부 분포를 추정할 수 있는 장점을 가집니다. 본 연구는 WIS와 CRPS 손실 함수의 최적화를 통해 강력한 예측 성능을 유지하면서도 해석 가능성을 갖춘 회귀 모델의 필요성을 강조합니다.

- **Performance Highlights**: 수치 실험을 통해 제안된 방법의 성능이 기존 대안 접근법들과 경쟁력이 있음을 입증했습니다. 확률적 회귀 트리는 해석 가능하고, 의료 분야와 같은 고위험 응용 분야에서 모델의 결정을 이해하는 데 중요한 장점을 제공합니다. 연구 결과, 제안하는 알고리즘은 그룹 조건부 커버리지 보장을 달성하는 데 특히 적합하다는 것을 보여주었습니다.



### Deep Dynamic Probabilistic Canonical Correlation Analysis (https://arxiv.org/abs/2502.05155)
Comments:
          accepted by ICASSP-25, code is available at \url{this https URL}

- **What's New**: 이번 논문에서는 비선형 동적 시스템을 분석하기 위한 Deep Dynamic Probabilistic Canonical Correlation Analysis (D2PCCA) 모델을 제안합니다. D2PCCA는 딥 러닝(deep learning)과 확률적 모델링(probabilistic modeling)을 결합하여 동적인 데이터 분석을 지원하며, Canonical Correlation Analysis (CCA)의 확장된 버전입니다. 이 모델은 KL annealing 및 normalizing flows 와 같은 기법을 통해 개선된 수렴성을 제공합니다.

- **Technical Details**: D2PCCA는 비선형(latent dynamics) 잠재 동력을 포착하고, 다수의 관측 변수(observed variables)에 자연스럽게 확장될 수 있는 기능을 가지고 있습니다. 이 모델은 시퀀셜 데이터 세트에 대한 사전 지식을 인코딩하는 데 유용하며, 시스템 동력학에 대한 확률적 이해를 제공합니다. 또한, D2PCCA는 KL annealing 기법을 통해 학습 과정을 촉진하고, posterior approximation을 더욱 유연하게 만들어 줍니다.

- **Performance Highlights**: 실제 금융 데이터셋에서의 실험 검증을 통해 D2PCCA의 효과성이 입증되었습니다. 이 모델은 잠재 동역학(latent dynamics)를 효과적으로 포착할 수 있으며, 다양한 데이터셋에 대한 적용 가능성을 보여줍니다. D2PCCA는 전통적인 방법에 비해 더 나은 성능을 발휘하며, 복잡한 비선형 동적 시스템 분석에 적합한 유용한 도구로 자리매김하고 있습니다.



### From Restless to Contextual: A Thresholding Bandit Approach to Improve Finite-horizon Performanc (https://arxiv.org/abs/2502.05145)
- **What's New**: 이번 연구에서는 классический контекстуальный бандит (contextual bandits) 문제를 확장하여 온라인 무한대 대리인 (restless bandits) 문제를 도입하였습니다. 이 모델은 상태 전이 (state transitions)와 예산 제약 (budget constraints)을 포함하여 각 에이전트를 마르코프 결정 과정 (Markov Decision Process, MDP)으로 나타냅니다. 새로운 접근 방법으로, 예산이 있는 임계값 설정 (budgeted thresholding contextual bandit)으로 문제를 재구성하여 학습의 용이성을 꾀하였습니다.

- **Technical Details**: 이 연구에서는 동적 결정 (dynamic decision-making)을 위한 프레임워크를 제공하면서, 각 에이전트의 MDP를 학습하는 데 필요한 복잡성을 줄였습니다. 구체적으로, 저자는 균등 결정을 통해 각 시점에서의 사용자 이점이 미리 정의된 임계값 을 초과하는지를 파악하는 알고리즘을 제안했습니다. 최초로 제안된 알고리즘인 LGRT(하향 신뢰 구간 안내 랜덤화 임계값 알고리즘)는 이론적으로 최적의 성능을 보임을 입증하였습니다.

- **Performance Highlights**: 제안된 알고리즘은 기존 온라인 무한대 대리인 방법들보다 유의미한 성과 향상을 보여주었습니다. 구체적으로, 새로운 알고리즘은 유한 기간 (finite-horizon) 설정에서 누적 보상을 극대화하는 데 크게 기여하는 것으로 나타났습니다. 실험 결과는 새로운 방법이 실제 문제에 적용가능성을 갖춘다는 점을 잘 보여줍니다.



### Data-Parallel Neural Network Training via Nonlinearly Preconditioned Trust-Region Method (https://arxiv.org/abs/2502.05133)
Comments:
          8 pages, 6 figures

- **What's New**: 이 논문에서는 딥 뉴럴 네트워크(DNN) 훈련을 위한 Additively Preconditioned Trust-Region Strategy (APTS)의 변형을 제안합니다. APTS 방법은 데이터 병렬(data-parallel) 접근 방식을 활용하여 비선형 최적화(nonlinear optimization) 전략에 적용되는 비선형 전처리기(preconditioner)를 구성합니다. 이는 일반적인 확률적 경량 하강법(Stochastic Gradient Descent, SGD)이나 Adaptive Moment Estimation(Adam)과의 차별점으로, 각 반복(iteration)에서 단계 크기를 암묵적으로 조정하여 비싼 하이퍼파라미터 튜닝(hyperparameter tuning)을 제거합니다.

- **Technical Details**: 기계 학습(ML)에서, APTS는 각의 네트워크를 병렬로 훈련시키는 구조를 가집니다. 이는 데이터 병렬 방식의 표준 방법과 비교할 때, 동기화 단계와 관련된 통신 비용을 줄여줍니다. 이 방법은 또한 비선형 최적화 문제를 해결하기 위해 설계되었으며, 전통적인 Trust-Region(TR) 방법의 글로벌 수렴(global convergence) 특성을 갖추고 있습니다.

- **Performance Highlights**: MNIST와 CIFAR-10 데이터셋을 사용하여 제안된 APTS의 성능을 입증하였습니다. 결과적으로, APTS는 SGD 및 Adam과 유사한 검증 정확도를 달성하면서도 병렬 훈련을 지원하고 비싼 하이퍼파라미터 튜닝의 필요성을 없앱니다. 이러한 특성은 모델 훈련의 효율성을 크게 향상시키는 것으로 보여집니다.



### SpecTUS: Spectral Translator for Unknown Structures annotation from EI-MS spectra (https://arxiv.org/abs/2502.05114)
- **What's New**: 이번 논문에서 제안하는 SpecTUS(Spectral Translator for Unknown Structures)는 저해상도 가스 크로마토그래피 전자 이온화 질량 스펙트럼(GC-EI-MS)으로부터 소분자의 구조를 주석 달기 위한 딥 뉴럴 모델입니다. 이 모델은 스펙트럼을 직접 2D 구조 표현으로 변환하는 	extit{de novo} 방식으로 작동하며, 기존 스펙트럼 라이브러리에 존재하지 않는 화합물 분석에 특히 유용합니다. 평가 결과, SpecTUS는 기존 데이터베이스 검색 방식들보다 더 높은 성능을 보여주었습니다.

- **Technical Details**: 실험은 NIST20 데이터셋을 사용하여 SpecTUS의 성능을 평가하였으며, 여기서 SpecTUS 훈련 세트는 232,035개의 스펙트럼으로 구성됩니다. 데이터베이스 검색 방식으로는 Simple Similarity Search (SSS), Hybrid Similarity Search (HSS), Best Database Candidate (BDC) 방법 등을 사용하였습니다. SpecTUS는 이들 방식들보다 잘 일반화되어 있으며, 특히 참고 데이터베이스에 없는 화합물 식별에서 두드러진 성과를 보였습니다.

- **Performance Highlights**: 정밀한 결정이 없이 제안하는 하나의 후보에서 43%의 화합물을 완벽하게 재구성할 수 있었고, 10개의 후보를 제안할 경우에는 65%의 재구성을 달성할 수 있었습니다. 이러한 결과는 현재의 데이터베이스 검색 방식들이 발전할 필요가 있음을 보여줍니다. SpecTUS는 전문가 검토가 필요한 다수의 후보를 제시할 수 있어, 작업의 효율성과 정확성을 서로 균형 잡을 수 있는 가능성을 제공합니다.



### Graph Contrastive Learning for Connectome Classification (https://arxiv.org/abs/2502.05109)
Comments:
          Submitted to EMBC '25

- **What's New**: 최근 비침습적 뇌 활동 측정 기법의 발전으로 인해 그래프 신호 처리(Graph Signal Processing, GSP)를 통한 구조적 및 기능적 뇌 네트워크 연구가 주목받고 있습니다. 이 연구는 그래프 표현 학습의 영역에서 감독 대조 학습(Supervised Contrastive Learning) 방법을 탐색하며, 같은 레이블을 공유하는 주체들을 그룹화하고 다른 레이블을 가진 주체들을 구별하는 그래프 수준 벡터 표현을 생성하는 것을 목표로 합니다. 데이터 증강(Data Augmentation) 기법을 통해 여성-남성 분류 작업에서 최첨단 성능을 달성했습니다.

- **Technical Details**: 이 연구는 뇌의 기능적 연결성과 구조적 연결성을 동시에 고려하는 그래프 신경망(Graph Neural Network, GNN) 인코더-디코더 아키텍처를 사용합니다. 입력으로는 구조적 연결 행렬과 기능적 연결 행렬이 사용되며, GCN(Graph Convolutional Network)을 통한 저차원 노드 임베딩 생성을 통해 그래프 구조에 대한 분석을 수행합니다. 특히, 대조 학습을 활용해 의미 있는 관계를 포착한 표현을 학습하고 있습니다.

- **Performance Highlights**: 연구 결과, 제안한 대조 학습 방법이 보다 전통적인 그래프 분류 프레임워크와 비교했을 때 성능 향상을 보여주었습니다. HCP 데이터셋을 기반으로 한 성능 평가에서 최첨단 기법들과 비교하여 경쟁력 있는 분류 성능을 입증하였고, 이로써 뇌 기능과 연결성에 대한 깊은 이해를 더할 수 있는 가능성을 제시합니다.
...
### Discrepancies are Virtue: Weak-to-Strong Generalization through Lens of Intrinsic Dimension (https://arxiv.org/abs/2502.05075)
- **What's New**: 이번 논문에서는 Weak-to-Strong (W2S) 일반화의 현상을 조사합니다. 이 방법론은 강력한 학생 모델이 약한 교사 모델이 생성한 pseudo-label을 기반으로 훈련됨에도 불구하고 종종 성능 상에서 약한 교사를 초월하는데, 이는 낮은 내재적 차원 속에서 Fine-tuning (FT)이 발생하기 때문입니다. W2S 모델이 약한 교사의 성능을 초월하는 이유를 분석하기 위해, 연구팀은 Variance reduction의 관점에서 리지 리그레션 설정을 통해 W2S를 분석합니다.

- **Technical Details**: 연구에서는 두 개의 저차원 서브스페이스인 \(\mathcal{V}_s\)와 \(\mathcal{V}_w\)를 고려하며, 이 서브스페이스는 각 모델에서 관련 기능을 캡슐화합니다. 또한, 강력한 모델과 약한 모델의 내재적 차원 \(d_s, d_w\)와 근사 오류 \(ρ_s, ρ_w\)를 측정하여 모델의 용량 차이를 공식화합니다. 이러한 분석은 학생-교사 상관관계를 중심으로 구축되며, W2S 일반화 오류를 분산(variance)과 편향(bias)으로 분해하여 설명합니다.

- **Performance Highlights**: 실험 결과 W2S 알고리즘은 합성 회귀 문제 및 실제 비전 작업에서 약한 모델의 성능을 초과하는 결과를 보여주었습니다. W2S 성능이 강조되는 조건은 강력한 학생 모델이 더욱 낮은 내재적 차원을 가지면서, 충분한 수의 pseudo-label을 가질 때 발생합니다. 이는 모델 간의 차이에 따른 분산 감소 효과로 해석되며, 이로 인해 W2S가 일반화에서 효과적으로 작용함을 확인할 수 있었습니다.
...
- **What's New**: 이 연구에서는 섬유 직물 구조의 투과율을 예측하기 위한 혼합 머신러닝 기반의 스케일 브리징(scale-bridging) 프레임워크를 소개합니다. 기존의 서브레이트 모델(surrogate models)과 물리정보 신경망(physics-informed neural networks, PINNs)을 통합하여 미세 및 중간 스케일에서의 정확한 투과율 예측을 가능하게 합니다. 이 접근법은 여러 스케일 간의 데이터를 효과적으로 통합하며, 특히 계산 비용과 예측 신뢰성을 균형 있게 조정합니다.

- **Technical Details**: 이 연구에서는 네 가지 스케일 브리징 방법론을 평가하였습니다: 단일 스케일 방법(Single Scale Method, SSM), 간단 업스케일링 방법(Simple Upscaling Method, SUM), 스케일 브리징 방법(Scale-Bridging Method, SBM), 완전 해석 모델(Fully Resolved Model, FRM). SSM은 미세 스케일 투과율을 무시하여 FRM 모델에 비해 최대 150%의 차이를 보였고, SBM은 세그먼트 기반의 미세 스케일 투과율 할당을 통해 예측 정확성을 크게 향상시켰습니다. 또한, PINNs를 통합한 이중 스케일 솔버는 데이터 부족 문제를 극복할 수 있는 가능성을 보여주며, 물리 정보 기반의 정규화(physics-informed regularization)를 통해 예측 성능을 추가로 개선할 수 있습니다.

- **Performance Highlights**: SBM은 약 45분의 계산 시간 안에 미세 스케일과 중간 스케일에서 거의 동일한 값을 달성하여 효율성을 강조했습니다. 반면, FRM은 가장 높은 정확성을 제공하지만, SSM에 비해 최대 270배의 계산 시간을 요구했습니다. 이 연구는 섬유 복합재 제조 분야에서의 향후 응용 가능성을 염두에 두고, 계산 비용과 신뢰성을 최적화하는 예측 모델링을 발전시키는 데 기여하고 있습니다.
...
### Leveraging a Simulator for Learning Causal Representations from Post-Treatment Covariates for CATE (https://arxiv.org/abs/2502.05037)
Comments:
          Accepted at TMLR-25

- **What's New**: 이번 논문은 치료 효과 추정(Treatment effect estimation)에서 포스트 처리 상황에서의 조건부 평균 치료 효과(Conditional Average Treatment Effect, CATE)를 추정하는 방법을 다룹니다. 기존의 방법들은 치료 배정 전에 수집된 공변량을 사용했지만, 본 연구에서는 공변량과 결과물이 치료 후에 수집되는 상황을 분석합니다. 연구 결과, 포스트 치료 공변량은 CATE의 비식별성(unidentifiability)을 유발하며, 이를 회복하기 위해 치료 독립적인 인과 표현(causal representations)을 학습해야 한다고 주장합니다.

- **Technical Details**: 이 논문은 관찰 데이터에서 인과 표현을 학습하기 위해 대조학습(contrastive learning)을 사용합니다. 대조학습은 반사실적(supervision) 정보가 있는 경우 유용합니다. 이 연구는 시뮬레이터(simulator)를 사용하여 합성적인 반사실적(supervision)을 제공하는 방법을 분석하며, 다양한 베이스라인의 CATE 오차를 검토하여 그 한계를 강조합니다. 또한, 실제와 시뮬레이터 분포에서의 공동 훈련으로부터 CATE 오차를 특성화하는 일반화 경계를 제공합니다.

- **Performance Highlights**: SimPONet이라는 새로운 방법을 제안하며, 이 방법의 손실 함수는 일반화 경계에서 영감을 받았습니다. SimPONet은 CATE 작업과 관련된 시뮬레이터의 영향을 조정하여 학습 목표에 맞춰 조정하는 방법을 보여줍니다. 여러 데이터 생성 과정(Directed Graphical Processes, DGP)을 통해 실험을 진행하며, 실제-시뮬레이터 분포 간의 격차를 체계적으로 변화시켜 SimPONet의 효능을 평가했습니다. 이 연구는 SimPONet이 기존 CATE 베이스라인에 비해 우수한 성능을 나타낸다고 결론을 내립니다.



### News about Global North considered Truthful! The Geo-political Veracity Gradient in Global South News (https://arxiv.org/abs/2502.05032)
- **What's New**: 이 논문은 글로벌 남반부(Global South)의 뉴스에서 발견되는 중요한 특성인 지역 정치적 진실도 경도(geo-political veracity gradient)에 대해 다룹니다. 연구 결과에 따르면, 글로벌 남반부에서 나오는 뉴스는 글로벌 북반부(Global North) 주제를 다룰 경우 더 진실할 가능성이 높습니다. 이는 해당 지역과 다른 지역에 관한 왜곡된 뉴스 생산에 대한 경제적 동기의 상대적 결여에 기인할 수 있습니다.

- **Technical Details**: 이 논문의 핵심은 '지역 정치적 진실도 경도'라는 패턴을 공식적으로 제안하는 것입니다. 저자들은 미디어의 정치 경제적 요인과 데이터셋의 실증적 분석을 통해 이 경도를 정량적으로 설명합니다. 또한 AI 기반의 가짜 뉴스 탐지 모델이 다른 지역에서 어떻게 적용될 수 있는지를 실증적으로 분석하여 이 경도를 반영할 필요성을 강조합니다.

- **Performance Highlights**: 연구팀은 다양한 데이터셋을 통해 각각의 지역에서의 진실성이 가짜 뉴스 감지 모델에 미치는 영향을 조사했습니다. 그 결과, 글로벌 북반부 뉴스에 대한 글로벌 남반부의 뉴스는 상대적으로 더 높은 정확도를 보이는 경향이 있다는 것을 발견했습니다. 이는 AI 모델이 특정 지역에 최적화된 데이터에 기반하여 작동하기 때문에 발생하는 현상입니다.



### Analog and Multi-modal Manufacturing Datasets Acquired on the Future Factories Platform V2 (https://arxiv.org/abs/2502.05020)
- **What's New**: 본 논문은 2024년 8월 13일 사우스캐롤라이나 대학교의 Future Factories Lab에서 8시간 동안 지속적으로 운영된 제조 조립 라인에서 수집된 두 개의 산업 수준 데이터셋을 소개합니다. 이 데이터셋은 산업 표준을 준수하며, 통신 프로토콜, 액추에이터(Actuator), 제어 메커니즘(Control Mechanism), 변환기(Transducer), 센서(Sensor), 카메라(Camera)에 대한 정보가 포함되어 있습니다. 또한, 이 데이터는 통합 및 외부 센서를 활용하여 수집되었습니다.

- **Technical Details**: 데이터 수집 과정에서 액추에이터에 내장된 센서와 외부에 설치된 기기를 포함한 다양한 센서를 이용했습니다. 또한 고성능 카메라를 통해 운영의 주요 요소를 기록했습니다. 본 데이터셋은 (1) 시계열 아날로그(Time-Series Analog) 데이터셋과 (2) 동기화된 시스템 데이터와 이미지를 포함한 다중 모달 시계열(Multi-Modal Time-Series) 데이터셋으로 구성되어 있으며, 이러한 데이터셋은 물리적 제조 환경을 재현할 필요 없이 새로운 알고리즘을 테스트할 수 있는 플랫폼을 제공합니다.

- **Performance Highlights**: 이 데이터셋은 오픈 소스로 제공되어 인공지능 모델 훈련을 용이하게 돕고, 연구를 간소화하는 데 기여합니다. 연구자들이 다양한 응용 프로그램 및 프로젝트에 즉시 사용할 수 있는 종합적이고 준비된 자원을 제공하여 향후 제조 공정을 발전시키는 데 있어 중요한 역할을 할 것으로 기대됩니다. 이전 실험에서는 30시간 동안의 지속 운영이 진행되었고, 모든 이상 현상이 기록되어 유지보수 절차가 적용되었습니다.



### $O(\sqrt{T})$ Static Regret and Instance Dependent Constraint Violation for Constrained Online Convex Optimization (https://arxiv.org/abs/2502.05019)
- **What's New**: 이 논문은 표준 온라인 볼록 최적화(Online Convex Optimization, OCO) 프레임워크의 제약 버전인 COCO를 다룬다. COCO에서는 매 라운드마다 학습자가 행동을 선택한 후에 볼록 비용 함수와 볼록 제약 함수가 공개된다. 제안된 알고리즘은 정적 손실(static regret)과 누적 제약 위반(cumulative constraint violation, CCV)을 동시 최소화할 수 있다.

- **Technical Details**: 제안된 알고리즘은 정적 손실 $O(	ext{sqrt}(T))$과 CCV를 $	ext{min}igackslash{	ext{CV}, O(	ext{sqrt}(T)	ext{log}(T))igackslash}$로 보장한다. $	ext{CV}$는 제약 세트 간의 거리, 제약 세트의 형태, 행동 공간의 차원 및 행동 공간의 직경에 따라 달라진다. 특히, 제약 세트의 특정 경우에서는 $	ext{CV}=O(1)$이 된다.

- **Performance Highlights**: 새로운 CCV 결과는 기존의 보편적인 $O(	ext{sqrt}(T))$ 정적 손실 및 $O(	ext{sqrt}(T)	ext{log}(T))$ CCV와 비교할 때 인스턴스 의존적(instance dependent)이다. 이는 제약 세트의 기하학적 특성을 활용하여 도출된 결과로, 사용자에게 더 향상된 실시간 성능을 제공할 수 있는 가능성을 보여준다.



### Seasonal Station-Keeping of Short Duration High Altitude Balloons using Deep Reinforcement Learning (https://arxiv.org/abs/2502.05014)
- **What's New**: 이번 연구에서는 고고도 풍선의 위치 유지를 위한 경로 계획 문제를 Deep Q-Learning(DQN)을 활용하여 해결하고자 하였습니다. 합성 풍력 예측을 통해 풍선이 겪는 실제와 유사한 바람의 변화를 적용하여 이를 시뮬레이션 환경에서 훈련하고 평가하였습니다. 각 계절에 따른 풍속 차이를 독립적으로 분류할 수 있는 Forecast Score 알고리즘을 도입하여, 각 계절의 성과를 비교하였습니다.

- **Technical Details**: 본 연구의 DQN 알고리즘은 고고도 풍선이 바람을 이용해 위치 유지를 할 수 있도록 훈련되었습니다. 다양한 바람의 고도 및 지역 특성을 반영하기 위해, 150 x 150 x 10 km의 시뮬레이션 환경을 설정했습니다. DQN은 상태-행동 쌍으로부터 가장 높은 보상을 극대화할 수 있는 정책을 학습하기 위해 수십만 개의 에피소드를 설계하여 훈련되었습니다.

- **Performance Highlights**: 훈련된 DQN HAB 에이전트는 다양한 계절에 따라 달라지는 풍속을 이용해 높은 성과를 보였습니다. 특히, TWR50(50km 이내의 비행 시간)과 같은 메트릭스를 통해 성공적인 위치 유지를 시뮬레이션했습니다. 연구 결과, DQN을 적용한 HAB의 성과는 실제 바람 예측과 밀접하게 연결되어 있음을 보여주었습니다.



### Learning the Language of NVMe Streams for Ransomware Detection (https://arxiv.org/abs/2502.05011)
Comments:
          25 pages, 8 figures

- **What's New**: 이번 연구에서는 NVMe command sequences 내에서 랜섬웨어 활동을 탐지하기 위해 언어 모델링 기술을 적용하였습니다. 두 가지 유형의 transformer 기반 모델을 설계하고 훈련했습니다: Command-Level Transformer (CLT)와 Patch-Level Transformer (PLT)입니다. CLT는 개별 명령이 랜섬웨어에 의해 시작되었는지를 판단하고, PLT는 명령 패치 내에서 랜섬웨어에 의해 접근된 데이터 양을 예측합니다.

- **Technical Details**: 제안된 모델들에 대한 디자인, 토큰화(tokenization) 및 임베딩(embedding) 방식이 포함되어 있습니다. CLT는 in-context token classification 방법을 사용하여 랜섬웨어에 의한 명령을 식별하며, PLT는 명령 패치 내에서 데이터 접근량을 예측합니다. 연구에서 이러한 접근법이 기존의 tabular 방식에 비해 상당히 향상된 결과를 보여줍니다.

- **Performance Highlights**: 이 모델들은 검출 누락률에서 최대 24% 향상, 데이터 손실 방지에서 66% 향상, 랜섬웨어에 의해 접근된 데이터 식별에서 84% 향상을 이루었습니다. 이는 기존의 최첨단 기법과 비교했을 때 눈에 띄는 성과로, 랜섬웨어 탐지 분야에서 더 우수한 성능을 입증하였습니다.



### QuEST: Stable Training of LLMs with 1-Bit Weights and Activations (https://arxiv.org/abs/2502.05003)
- **What's New**: 이 논문은 QuEST라는 새로운 Quantization-Aware Training (QAT) 방법을 제안합니다. QuEST는 모델 전체에서 4비트 이하의 가중치와 활성화로 트레이닝할 수 있으며, 특히 1비트에서의 안정적인 훈련을 가능하게 합니다. 이 방법은 기존의 8비트 최적화보다 더 나은 정확성을 제공하면서도 모델 사이즈를 줄이는 데 도움을 줍니다.

- **Technical Details**: QuEST는 두 가지 주요 요소로 QAT 방법을 개선합니다. 첫 번째는 Hadamard normalization을 이용한 정확하고 빠른 가중치와 활성화의 양자화입니다. 두 번째는 불확실한 경량화 경량화로 과감한 신뢰도 추정기를 제안하여 낮은 정밀도 가중치와 활성화의 불규칙성을 최소화합니다.

- **Performance Highlights**: 실험 결과, QuEST는 Llama 계열 모델에서 4비트 가중치 및 활성화로 BF16 모델과 비교해 약 4배 더 높은 정확도를 보여주었습니다. 이러한 성능은 QuEST가 다양한 하드웨어 지원 정밀도에서 안정성을 제공하며, 효율적으로 실행할 수 있음을 나타냅니다.
...
### Enhancing Pre-Trained Decision Transformers with Prompt-Tuning Bandits (https://arxiv.org/abs/2502.04979)
- **What's New**: 이번 논문에서는 오프라인 데이터셋을 활용한 강화 학습(Reinforcement Learning; RL) 모델인 Prompting Decision Transformer (PDT)의 한계를 개선하기 위해, 고성능 임프롬프트(prompt)를 동적으로 생성하는 확장 가능한 밴딧(bandit) 기반의 프롬프트 튜닝 방법을 도입했습니다. 기존 PDT는 임프롬프트를 전문가의 데이터셋에서 무작위로 샘플링했지만, 이는 정보 비대칭(informativeness)으로 인해 성능을 저하시킬 수 있음을 인식했습니다. 또한, 본 연구는 사전 훈련된 Transformer의 기본 구조를 변경하지 않고도 다운스트림(task-specific) 작업 성능을 획기적으로 향상시킬 수 있는 방법을 제시합니다.

- **Technical Details**: 오프라인 다중 작업 강화 학습(Multi-task Reinforcement Learning; RL) 문제는 주어진 훈련 작업 세트 T_{train}을 해결하는 것을 목표로 하며, 각 작업에 대해 Markov Decision Process (MDP)의 트라젝토리(trajectory) 데이터를 사용합니다. 각 작업에 대한 정책 π는 지정된 작업의 정보를 포함하는 보조 입력 ψ를 사용하여 최적의 행동을 결정합니다. 본 연구에서는 기존 PDT에 비해 더 높은 성능을 발휘하는 능동적인 임프롬프트 선택 방법을 통해 오프라인 데이터에서의 정책 학습을 최적화합니다.

- **Performance Highlights**: 실험 결과, 제안된 밴딧 기반 프롬프트 튜닝 방법이 다양한 벤치마크 작업 및 새로 설계된 다중 작업 환경에서 성능 향상을 보여주었습니다. 특히, 초기 프리트레이닝(pre-training) 단계와 작업 별 온라인 적응(task-specific online adaptation) 사이의 간극을 효과적으로 메우는 모델을 개발함으로써, 다양한 작업의 일반화 능력을 크게 개선했습니다. 이로 인해 PDT의 작업 구별 능력이 향상되었음을 실험적으로 입증했습니다.



### DE-PADA: Personalized Augmentation and Domain Adaptation for ECG Biometrics Across Physiological States (https://arxiv.org/abs/2502.04973)
- **What's New**: 최근 연구는 Electrocardiogram (ECG) 기반 생체 인식의 가능성에 주목하고 있으며, 이는 생체 신호의 독특한 형태와 생명성 검출 능력 덕분입니다. 본 논문에서 제안된 DE-PADA는 다양한 생리적 상태에 대한 강건성을 높이기 위해 개발된 Dual Expert 모델입니다. 이 모델은 운동 후 상태에서의 사용자 식별 성능 갭을 줄이기 위해 개인화된 증강과 도메인 적응을 통합한 혁신적인 접근 방식을 사용합니다.

- **Technical Details**: DE-PADA 모델은 주로 안정 상태 데이터에서 훈련되며, 심박수에 민감한 ST 구간과 PQRS 간섭을 활용하여 ECG의 고유한 특성을 반영합니다. 이 모델은 특정 T-wave 변동성을 시뮬레이션하여 개인별 요구에 맞는 데이터 증강을 진행합니다. 또한, 다양한 생리적 조건에서의 일반화 능력을 향상시키기 위해 보조 데이터를 활용하는 도메인 적응 방식을 적용합니다.

- **Performance Highlights**: UofTDB(Electrocardiogram Database) 실험 결과, DE-PADA는 초기 회복 단계에서 26.75%, 후기 회복 단계에서 11.72%의 상대적 식별률 향상을 보였습니다. 또한, DE-PADA는 앉은 자세에서 98.12%의 높은 식별률을 유지하여, 다양한 생리적 상태에서 ECG 기반 생체 인식 시스템의 강건성을 강화하는 데 기여하고 있습니다.
...
- **What's New**: 이번 논문에서는 여러 작업에 특화된 모델들을 하나의 다중 작업 모델로 통합하는 '모델 병합'(model merging) 기술에 대해 다룹니다. 기존의 병합 모델과 단일 작업 모델 간의 성능 차이를 줄이기 위해 '이소트로픽 병합'(isotropic merging) 프레임워크를 제안하며, 이를 통해 모델 간의 정렬(alignment)을 향상시키는 방식입니다. 특히, 사전 훈련된 모델의 weight update matrices인 '작업 행렬'(task matrices)의 주요 특성을 연구하고, 효과적인 병합을 위한 기준을 마련하였습니다.

- **Technical Details**: 이 논문에서는 다수의 작업에 대한 효과적인 병합을 위한 작업 행렬의 정렬을 측정하기 위해 'Subspace Alignment Ratio'를 사용합니다. Singular Value Decomposition(SVD)을 활용하여, 작업 행렬의 상위 고유 벡터가 스팬하는 서브스페이스 간의 유사도를 정량화하며, 이를 통해 병합된 모델의 성능 향상과 강한 상관관계를 발견합니다. 또한, 이소트로픽 구조를 적용하여 병합된 행렬의 고유값 스펙트럼을 균일하게 만들고, 고유 방향의 정렬을 개선하여 성능을 높이는 기술에 대해 기술하고 있습니다.

- **Performance Highlights**: 제안된 병합 기법은 다양한 작업 조합과 모델 크기에 걸쳐 최첨단 성능을 달성했습니다. 특히, Iso-C와 Iso-CTS 방법론은 최적의 성능을 보이며, 대규모 모델과 많은 작업에 대해 성공적으로 확장 가능한 방법론을 제시합니다. 이 연구는 모델 병합의 역동성을 이해하는 데 기여하며, 추가적인 훈련 없이도 효과적인 모델 병합 방법을 제공합니다.
...
### Generative-enhanced optimization for knapsack problems: an industry-relevant study (https://arxiv.org/abs/2502.04928)
- **What's New**: 이번 연구에서는 지능형 최적화 문제를 해결하기 위한 새로운 접근법으로서 텐서 네트워크(tensor networks, TN) 및 대칭 텐서 네트워크(symmetric tensor networks, STN)를 기반으로 한 TN-GEO 방법론을 소개합니다. 이 방법론은 생성 모델(generative modeling)을 활용하여 최적화 문제의 특정 제약 조건에 대해 유효한 해를 효율적으로 샘플링하는 전략입니다.

- **Technical Details**: 연구의 핵심은 다중 배낭 문제(multi-knapsack problem)를 대상으로 TN-GEO 및 STN-GEO의 적용 가능성을 점검하는 것입니다. 또한, TN 및 STN-GEO 방법론을 실제로 사용할 수 있도록 practitioners에게 제시하고 있으며, 하이퍼파라미터(hyper-parameters)와 스케일링 행동의 의존성을 분석합니다.

- **Performance Highlights**: 60개의 다양한 문제 인스턴스를 기준으로 벤치마킹을 수행하였고, TN-GEO와 STN-GEO 방식이 시뮬레이티드 어닐링(simulated annealing)과 유사한 품질의 결과를 산출하는 것을 발견했습니다. 이를 통해 제안된 방법론이 산업 강의 문제 해결에 효과적임을 강조했습니다.
...
### On the Power of Heuristics in Temporal Graphs (https://arxiv.org/abs/2502.04910)
- **What's New**: 이 논문은 동적 그래프 데이터셋의 최신성과 인기와 같은 강력한 시간적 패턴을 활용하는 단순 휴리스틱(heuristic) 방법이 최신 신경망 모델(state-of-the-art)과 동등하거나 더 나은 성과를 낼 수 있음을 보여줍니다. 새로운 메트릭(metrics)을 도입하여 다양한 데이터셋에서 이러한 패턴이 미치는 영향을 정량화하고 있으며, BenchTemp 및 Temporal Graph Benchmark에서 최첨단 성능 결과를 달성했습니다. 이러한 결과는 동적 그래프 모델의 발전을 촉진하기 위한 공정한 비교를 가능하게 하는 평가 기준의 중요성을 강조합니다.

- **Technical Details**: 동적 그래프는 시간이 지나면서 노드와 엣지가 지속적으로 추가, 제거 및 업데이트되는 관계를 모델링합니다. 링크 예측(link prediction)은 이러한 그래프 분석에서 중요한 작업으로, 미래의 노드 연결을 예측하는 데 초점을 맞춥니다. 최근의 연구들은 복잡한 신경망 아키텍처를 활용하여 동적 그래프 작업을 처리하고 있으나, 본 논문에서는 단순한 메모리 기반 휴리스틱이 효과적임을 보여줍니다.

- **Performance Highlights**: 다양한 벤치마크 데이터셋에 대한 분석 결과, 최신성(Recency) 기반의 링크 예측 휴리스틱이 가장 효과적이라는 것이 밝혀졌습니다. 이는 많은 실제 네트워크에서 최근에 활동적인 노드가 지속적으로 연결될 가능성이 높기 때문입니다. 또한, 인기(Popularity) 기반 휴리스틱은 추가 연결을 유도하는 '부자도 더욱 부유해지는(Rich-get-richer)' 동적을 효과적으로 반영하여 성과를 향상시킵니다.
...
### Deep Learning Models for Physical Layer Communications (https://arxiv.org/abs/2502.04895)
Comments:
          PhD Thesis

- **What's New**: 이 논문은 데이터와 컴퓨팅 자원의 증가로 인해 머신 러닝(Machine Learning, ML) 기법이 엔지니어링 분야에서 성공적으로 도입되고 있다는 점을 강조합니다. 특히, 심층 학습(Deep Learning, DL) 알고리즘이 물리적 현상의 하향식(top-down) 설명이 부족할 때 뛰어난 성능을 발휘한다는 것을 보여줍니다. 또한, 통신 공학 분야에서 최근 데이터 기반 학습 모델로의 전환이 이루어지고 있음을 언급합니다.

- **Technical Details**: 논문은 채널 용량(channel capacity) 및 최적 인코딩-디코딩 스킴(optimal coding-decoding schemes)과 같은 전통적인 문제를 ML 용어로 수학적으로 정식화합니다. 이를 통해 임의의 통신 매체에 대해 새로운 DL 패러다임을 활용하여 근본적인 문제들을 해결하고자 합니다. 또한 DL 모델을 훈련시키기 위해 필요한 아키텍처(architecture), 알고리즘(algorithm), 코드(code)를 설계하고 개발합니다.

- **Performance Highlights**: 마지막으로, 연구자는 오랜 기간 동안 존재해온 통신 분야의 문제들에 대한 새로운 솔루션을 제안합니다. 이러한 접근 방식은 채널 모델링과 물리 계층 설계 등 다양한 분야에서 제공할 수 있는 최적 전략이 부족한 상황에서 매우 중요합니다. 이는 나아가 통신 기술의 발전에 기여할 것으로 예상됩니다.



### A Foundational Brain Dynamics Model via Stochastic Optimal Contro (https://arxiv.org/abs/2502.04892)
Comments:
          The first two authors contributed equally

- **What's New**: 이 논문에서는 Stochastic Optimal Control (SOC)과 Amortized Inference를 활용한 뇌 동역학을 위한 기초 모델인 Brain Dynamics with Optimal control (BDO)를 소개합니다. BDO는 fMRI 신호의 복잡하고 잡음이 많은 특성을 잘 처리할 수 있는 연속-이산 상태 공간 모델을 특징으로 하며, 컴퓨팅 제한을 해결하기 위한 근사화 전략을 구현하고 있습니다. 이러한 접근법은 확장 가능성과 효율성을 제공하면서도 향상된 전이 가능성을 자랑합니다.

- **Technical Details**: 이 연구는 SOC 이론을 기반으로 한 연속-이산 SSM을 도입하여 효과적인 전이 표현을 추출하고, Amortized Inference를 통해 효율적이고 대규모 데이터에서의 학습을 가능하게 합니다. 연구팀은 최신 기계 학습 기술을 활용하여 전통적인 SSM의 한계를 극복하고, Time-Series 데이터를 처리하는 데 있어 더 나은 성능을 자랑합니다. 또한, 이 접근 방식은 SSL의 원리를 통합하여 데이터의 시간적 의존성을 올바르게 포착합니다.

- **Performance Highlights**: BDO 모델은 UKB와 같은 대규모 데이터셋에서 사전 학습을 통해 인구 통계 예측, 특성 분석, 질병 진단 및 예후 등 다양한 다운스트림 작업에서 최신 성능을 달성합니다. HCP-A, ABIDE, ADHD200와 같은 외부 데이터셋에서도 우수한 능력과 다양한 인구 및 임상 분포에 대한 저항력을 입증하였습니다. 이러한 성과는 BDO가 뇌 동역학 모델링에 있어서 새로운 표준을 설정하고 있음을 보여줍니다.



### GNNs Getting ComFy: Community and Feature Similarity Guided Rewiring (https://arxiv.org/abs/2502.04891)
Comments:
          Accepted at ICLR 2025

- **What's New**: 이 논문에서는 그래프 재구성을 통해 spectral gap을 극대화하는 것이 메시지 전달 그래프 신경망(GNN)의 성능을 향상시킬 수 있다는 기존 연구에 더하여, spectral gap을 최소화하는 것이 일반화(generalization)에도 도움이 될 수 있음을 보여줍니다.

- **Technical Details**: 저자들은 stochastic block models의 맥락에서 GNN이 재구성을 통해 어떻게 이점을 얻는지 분석합니다. 특히, spectral gap 최적화는 커뮤니티 강도에 영향을 미치며, 커뮤니티 구조가 노드 레이블과 일치할 때 성능을 향상시킵니다. 이를 바탕으로, 저자들은 커뮤니티 구조, 노드 레이블 및 이들의 정렬을 명시적으로 목표로 하는 세 가지 재구성 전략을 제안합니다.

- **Performance Highlights**: 저자들은 제안된 세 가지 전략, 즉 커뮤니티 구조 기반 재구성(ComMa), 특성 유사성 기반 재구성(FeaSt), 하이브리드 접근법(ComFy)의 효과성을 폭넓은 실험을 통해 입증하였습니다. 각 전략은 성능을 개선하면서도 커뮤니티 구조와 레이블 간의 정렬을 최적화하는 데 중점을 두고 있음을 알고리즘적으로 확인시켰습니다.



### Exploit Gradient Skewness to Circumvent Byzantine Defenses for Federated Learning (https://arxiv.org/abs/2502.04890)
- **What's New**: 이 논문에서는 Federated Learning(FL)의 Byzantine 공격에 대한 새로운 취약점을 다룹니다. 기존의 방어 기법들이 다수의 gradient를 신뢰할 수 있는 것으로 간주하는 경향이 있지만, 이 연구에서 제시된 gradient skew 현상이 이러한 신뢰를 무너뜨린다는 점을 강조합니다. 이 현상은 다양 한 데이터로 인해 발생하며, 기존의 방어 시스템이 Byzantine gradient를 진짜 gradient와 혼동하게 만듭니다.

- **Technical Details**: 이 논문은 STRIKE라는 새로운 공격 방법을 제안합니다. STRIKE는 먼저 skewed gradients를 탐색한 후, 이러한 skewed gradients 내에 Byzantine gradients를 구성합니다. 실험은 세 가지 벤치마크 데이터셋에서 수행되었으며, 이 새로운 공격 방법의 효과성이 검증되었습니다.

- **Performance Highlights**: 기존의 Byzantine 방어 방법들이 gradient skew 현상을 무시하는 경향이 있다는 것을 밝혔습니다. 이 연구는 Byzantine 공격의 공격성을 강화하는 새로운 시각을 제공하며, FL의 성능에 대한 중요한 통찰을 제공합니다. 따라서 gradient skew를 이용한 공격에 대한 새로운 방어 메커니즘이 필요하다는 것을 시사합니다.
...
- **What's New**: 이번 연구에서는 협력적 학습(Collaborative Learning, CL)에서 기여에 비례하는 공정한 보상 할당을 위한 새로운 방법을 제안합니다. 이 방법은 슬리머블 신경망(slimmable neural networks)의 개념을 활용하여, 참여자 각자의 기여도에 맞춰 모델의 폭을 동적으로 조절하여 성능을 반영합니다. 기여도 평가와 보상 할당 메커니즘에서 겪는 문제를 해결하면서도, 협력적 공정성을 유지할 수 있는 방법론을 개발했습니다.

- **Technical Details**: 공정한 보상 할당은 기여도를 측정하고 보상을 배분하는 두 가지 기본 문제에 의존합니다. 연구진은 각 참여자의 기여에 따라 모델의 폭을 조정하는 후처리 보상 할당 알고리즘과 함께, 훈련 시 모델 보상 할당을 가능하게 하는 방법을 제안하였습니다. TEEs(Trusted Execution Environments)를 활용하여 개인 정보 보호와 공정성을 극대화하며 기여도에 따른 다양한 하위 모델(subnetwork)을 할당합니다.

- **Performance Highlights**: 이 연구는 다양한 데이터셋과 아키텍처에서 포괄적인 실험을 통해 제안한 방법의 유효성을 입증하였습니다. 자연스러운 성능 감소에도 불구하고 참여자 기여에 비례하는 모델 보상을 통해 높은 성능과 협력적 공정성을 동시에 달성할 수 있음을 demonstrated하였습니다. 또한, 실제 애플리케이션에서의 적용 가능성에 대한 기초를 마련하여 미래의 연구에 기여할 것으로 기대됩니다.



### Memory Capacity of Nonlinear Recurrent Networks: Is it Informative? (https://arxiv.org/abs/2502.04832)
Comments:
          8 pages, 1 figure

- **What's New**: 이번 논문은 선형 순환 신경망(RNN)의 메모리 용량(MC) 개념을 재조명하며, 비선형 RNN의 MC가 입력 프로세스 스케일에 따라 임의의 값을 가질 수 있음을 보여줍니다. 이전 연구([Ball 24])에서 제안된 선형 RNN 메모리 용량 정의가 품질 비교에 있어서 실질적으로 결점이 있다는 점을 강조합니다. 이러한 발견은 MC 정의의 유용성에 대한 의문을 제기하며, 다른 메모리 용량 개념이 필요하다는 요구를 나타냅니다.

- **Technical Details**: 메모리 용량((T)MC) 개념은 RNN이 과거 입력을 재생산하는 능력을 측정하는 데 사용되어 왔습니다. 논문에서는 비선형 RNN의 경우에도 MC 정의가 입력 스케일에 대해 불변하지 않다는 점을 기술합니다. 이로 인해 MC는 문헌에서 알려진 경계 내의 임의의 값을 가진다고 결론짓습니다.

- **Performance Highlights**: 기존 연구에서는 MC가 네트워크 상태가 이전의 독립적이고 동일하게 분포된(i.i.d.) 입력에 대한 정보를 가지고 있는 능력을 정량화한다고 설명되었습니다. 그러나 이번 발견은 메모리 용량 정의가 신경망의 성능을 성취하는 데 실질적인 가치가 없음을 보여주며, 향후 연구에서는 메모리 용량을 다른 방식으로 정의할 필요가 있음을 암시합니다.
...
### Harnessing omnipresent oscillator networks as computational resourc (https://arxiv.org/abs/2502.04818)
- **What's New**: 이 연구는 자연에서 발견되는 진동 특성을 활용하여 진동자 네트워크를 정보 처리 시스템으로 변환하는 보편적인 프레임워크를 제시합니다. 특히, Kuramoto 모델을 기반으로 한 reservoir computing 프레임워크를 통해 외부 입력에 대한 처리와 기억이 가능해짐을 보여줍니다. 연구에서는 비선형 목표 시스템을 통해 Kuramoto 모델을 강제하고, 훈련된 피드백 루프를 사용하여 목표 시스템을 모방하는 방법을 탐구하였습니다.

- **Technical Details**: Kuramoto 모델의 상호 작용 함수는 Γjk(θj−θk)=KNsin(θj−θk) 형태로 정의되며, 이는 네트워크의 성능을 최적화하는 데 중요한 역할을 합니다. 본 논문에서는 초기값 문제를 정의하여 시스템을 평가하고, 훈련 과정에서 훈련 신호를 파라미터로 조정하여 최적의 Wout를 찾습니다. 이러한 접근은 자연의 진동자 네트워크를 이용한 다양한 정보 처리 가능성을 탐구하는 기초를 마련합니다.

- **Performance Highlights**: 모델이 구현된 결과, Kuramoto 모델의 성능 특성이 계승되었으며, 모든 노드 간의 결합이 선형 시간 내에 이루어지는 성능을 보여주었습니다. 또한, 진동자 네트워크의 학습 능력은 Kuramoto 모델의 차수 매개변수로 설명이 가능하다는 것을 입증하였습니다. 이러한 발견은 진동자 네트워크를 새로운 정보 처리 시스템 클래스로 활용할 수 있는 가능성을 제시합니다.



### Describing Nonstationary Data Streams in Frequency Domain (https://arxiv.org/abs/2502.04813)
- **What's New**: 이 논문에서는 데이터 스트림 특성을 설명하는 새로운 도구인 Frequency Filtering Metadescriptor(FFM)를 소개합니다. 이 도구는 데이터 조각의 특성 벡터에서 유용한 주파수 성분을 검색하여 데이터 개념의 변화를 효과적으로 분석하고 시각화할 수 있도록 설계되었습니다. 또한, 이 방법은 기존의 주요 기법과 비교하여 데이터 스트림의 포스트 호크 개념 식별 작업에서 우수한 성능을 보입니다.

- **Technical Details**: FFM 방법은 Fast Fourier Transform을 활용하여 높은 차원의 데이터에서 유의미한 주파수를 추출합니다. 이 접근법은 데이터 스트림 안의 비정상성을 처리하는 데 큰 장점을 제공하며, 데이터 조각의 메타 설명을 생성할 수 있습니다. 특히, 데이터 스트림의 주파수 도메인에서의 일반화 능력은 복잡한 특징 종속성을 효과적으로 캡처하고 데이터의 의미를 유지합니다.

- **Performance Highlights**: 제안된 방법은 k-means 알고리즘을 이용한 비지도 개념 식별 작업에서 실험적으로 평가되었습니다. 또한, 다양한 최신 기법 및 기준 메타 설명 접근법과 비교하여 효과적임을 증명했습니다. 이 연구는 실세계 데이터 스트림에 대한 개념 식별을 수행할 수 있는 현실적인 응용 가능성을 보여주며, 이 과정에서 데이터 청크의 메타 설명 공간에서의 개념 소속도 시각화됩니다.



### Humans Co-exist, So Must Embodied Artificial Agents (https://arxiv.org/abs/2502.04809)
- **What's New**: 본 논문에서는 생물학과 설계 이론에서 영감을 받아 존재 기반 인공지능(embodied artificial agents)의 상호작용을 위한 새로운 패러다임을 제시합니다. 이들은 인간과의 의미 있는 장기 상호작용을 위한 전제 조건으로, 환경 내에서 서로 공존하며 지속적으로 적응하고 진화할 수 있는 능력을 강조합니다. 이러한 관점은 기계 학습 커뮤니티에 기여할 핵심 연구 방향을 제안하고 있습니다.

- **Technical Details**: 현재의 인공지능 시스템은 대규모 모델, 대규모 데이터셋, 컴퓨팅 집약적인 오프라인 학습 프로세스 덕분에 큰 성장을 이루었습니다. 하지만 현존하는 존재 기반 인공지능 시스템은 고립된 환경에서의 놀라운 성과에도 불구하고, 복잡하고 변화하는 실제 세계에서 지속적으로 인간과 상호작용하는 데 어려움을 겪고 있습니다. 따라서 이들은 동적인 상황에 잘 적응할 수 있는 새로운 설계 원칙이 필요합니다.

- **Performance Highlights**: 최근의 진행 상황에도 불구하고, 존재 기반 인공지능은 스스로 고정된 지식과 환경 변화에 대한 무관심으로 인해 문화적 및 기술적 부작용을 초래할 수 있는 위험이 있습니다. 이와 같은 맥락에서, 본 논문은 기술이 발전함에 따라 인간의 기대와 니즈가 변화할 수 있음을 언급하고, 공존할 수 있는 지능형 시스템이 필요하다고 강조합니다.
...
### An Extended Benchmarking of Multi-Agent Reinforcement Learning Algorithms in Complex Fully Cooperative Tasks (https://arxiv.org/abs/2502.04773)
- **What's New**: 이 논문은 Multi-Agent Reinforcement Learning (MARL)의 평가에서 체계적인 다양성이 부족한 점을 지적하고, 이를 해결하기 위해 기존 벤치마크를 확장할 필요성을 강조합니다. 특히, 협력적 MARL 알고리즘의 평가가 주로 SMAC 및 GRF와 같은 팀 게임 시나리오에 국한되어 있다는 점을 비판하며, 실제 협력 작업에 필요한 다양한 요인을 고려하지 못하고 있다는 점에 주목합니다. 이에 따라 복잡한 벤치마크에서 개선된 평가를 위해 여러 MARL 알고리즘을 비교 분석합니다.

- **Technical Details**: 저자는 협력적 MARL을 Dec-POMDP로 모델링하고, 상태공간, 행동공간, 보상 함수 및 관찰 함수를 사용하여 구체적으로 정의합니다. 알고리즘은 부분 관찰 상황에서 작동하며, 각 에이전트는 자신만의 정책을 기반으로 작동합니다. 논문은 Common Type Decentralized (CTDE) 접근법의 중요성을 언급하며, 이러한 접근이 주어진 환경에서 효과적으로 가치 함수 조건화를 가능하게 한다고 설명합니다.

- **Performance Highlights**: 저자의 분석 결과, SMAC 및 GRF에서 최첨단으로 평가되던 다수의 알고리즘이 완전 협력 벤치마크에서 표준 MARL 기준 성능에 미치지 못할 수 있음을 보여주었습니다. 새로운 오픈소스 PyMARLzoo+를 통해 기존 (E)PyMARL 라이브러리를 확장하여, PettingZoo의 모든 벤치마크와의 통합이 원활하도록 지원합니다. 이로써 기존 MARL 평가의 한계를 극복하고 통합적인 평가를 위한 기초자료를 제공하고자 합니다.
...
### Learning Universal Multi-level Market Irrationality Factors to Improve Stock Return Forecasting (https://arxiv.org/abs/2502.04737)
Comments:
          KDD2025

- **What's New**: 최근 딥러닝(Deep Learning)과 정량 거래(Quantitative Trading)의 조화가 이뤄지면서 주식 투자에서 큰 성공을 거두었습니다. 본 논문은 시장 비합리성(Market Irrationality)을 반영한 새로운 모델인 UMI를 제안하며, 이는 개별 주식과 전체 시장 수준의 비합리적 행동을 학습합니다. UMI 모델은 주식 가격의 합리적인 추정치를 생성하여 실제 가격과 비교함으로써 주식 수준의 비합리성을 성과 지표로 사용합니다.

- **Technical Details**: UMI 모델은 주식 가격의 비합리성을 설명하기 위해 동전확률 회귀(Cointegration) 주의 메커니즘을 채택합니다. 이를 통해 주식의 각 가격에 대한 합리적인 가격을 구성하고 이 가격과 실제 가격의 차이를 주식 수준 비합리성의 지표로 활용합니다. 또한, 서브 마켓 비교학습(Sub-market Comparative Learning)과 시장 동시성 예측(Market Synchronism Prediction)이라는 자가 지도 학습(Self-supervised Learning) 방식으로 시장 수준의 비합리성을 시장 표현 벡터에 통합합니다.

- **Performance Highlights**: UMI 모델은 미국과 중국의 주식 시장에서 광범위한 실험을 통해 예측 정확도 및 투자 이익을 입증하였습니다. 본 연구는 기존의 딥러닝 기반 주식 예측 모델에 시장 비합리적 행동을 최초로 통합한 결과, 다양한 예측 모델의 성능을 향상시키는 보편적 효과를 확인했습니다.
...
### Nearly Tight Bounds for Cross-Learning Contextual Bandits with Graphical Feedback (https://arxiv.org/abs/2502.04678)
- **What's New**: 이 논문은 그래픽 피드백을 갖춘 크로스-러닝 맥락 밴딧 문제에 대한 새로운 알고리즘을 제시하여, Stochastic 상황에서도 O~(√αT)만의 후회를 달성할 수 있음을 증명합니다. 이는 2024년 Han et al.의 연구가 제시한 문제를 해결하면서, 이전 연구들이 불가능하다고 주장했던 알고리즘의 존재 여부에 대한 질문에 답합니다. 크로스-러닝 구조를 활용하여 확률적 맥락에서도 어떠한 상황에서도 후회 한계를 최소화할 수 있는 가능성을 보여줍니다.

- **Technical Details**: 맥락 밴딧 문제 및 그래픽 피드백을 갖춘 밴딧 문제는 전통적인 밴딧 문제를 확장하는 두 가지 일반적인 설정입니다. 이 논문은 이러한 설정에 대해 O~(√αT) 후회 경계를 달성할 수 있는 알고리즘을 제안하며, 알고리즘은 강하게 관찰 가능한 피드백 그래프 G와 확률적 맥락 분포를 사용하는 경우에 성능을 발휘합니다. 주요 기술적 기법은 Stochastic 특성과 적대적 밴딧 기법의 혼합 사용에 기반하고 있습니다.

- **Performance Highlights**: 제안된 알고리즘은 O~(√αT) 후회 경계를 달성하며, 이는 모든 맥락의 수와 독립적으로 이루어지기 때문에 전통적인 밴딧의 최소 최대 후회와 일치합니다. 이전 연구에서는 O~(min(αMT, KT)) 후회로 성능 제한이 있었으나, 본 연구는 이보다 더 강력한 성능을 보여줍니다. 해당 알고리즘은 그래픽 피드백 구조와 크로스-러닝 구조를 효과적으로 통합함으로써 더 나은 성능을 달성한 것입니다.
...
### Implicit Bias of SignGD and Adam on Multiclass Separable Data (https://arxiv.org/abs/2502.04664)
- **What's New**: 본 논문에서는 다중 클래스 선형 분리 가능한 데이터에 대한 교차 엔트로피 손실(CE loss)의 최소화 과정에서 Adam과 Sign Gradient Descent(SignGD)의 암묵적 편향을 규명하였습니다. 기존의 연구들은 주로 이진 분류에 집중하였으나, 본 연구는 다중 클래스 문제에 대한 실질적인 분석을 제공합니다. 연구진은 softmax 함수의 특성을 활용하여 다중 클래스 설정에서 직접적으로 손실 분석을 수행하였고, 이는 기존의 이진 분류로 축소하는 방식과는 차별화됩니다.

- **Technical Details**: 우리는 다중 클래스 분리 가능한 데이터에 대해 SignGD의 반복(iterates)이 분류기 행렬의 max-norm에 대해 마진을 최대화하는 솔루션으로 수렴함을 보였습니다. 또한, 동일한 설정 하에 Adam의 반복도 max-norm에 대한 마진을 최대화하며, 이때의 수렴 속도는 각각 𝒪(1/t^{1/2}) 및 𝒪(1/t^{1/3})입니다. 이러한 결과는 다중 클래스 분류에 대한 기존 연구의 빈틈을 메우며, 다양한 p-norm에 대해 일반화된 결과를 제공합니다.

- **Performance Highlights**: 본 연구에서는 Adam과 SignGD의 결과를 실험적으로 검증하였으며, 이론적으로 예측된 암묵적 편향 결과가 확인되었습니다. Adam의 경우 다중 클래스 설정에서도 안정성 항 상관없이 예측과 일치하는 경향을 보였으며, SignGD와 Adam 모두 max-norm 마진을 선호하는 솔루션을 찾는 것으로 나타났습니다. 이러한 발견은 현대의 딥러닝 활용에서 암묵적 편향의 실제적인 중요성을 강조합니다.
...
- **What's New**: 본 논문은 강화 학습(Reinforcement Learning, RL)에서 정책 평가(policy evaluation) 문제를 다룹니다. 기존 논문들은 잘 동작하는 보상 분포를 가정하고 TD(Temporal Difference) 학습 알고리즘에 대한 유한 시간 수렴 보장을 연구하였으나, 이는 현실 세계의 복잡한 환경에서는 항상 유효하지 않을 수 있습니다. 이에 따라 저자들은 적대적 강인성(adversarial robustness) 관점에서 정책 평가 문제를 재조명하였습니다.

- **Technical Details**: 논문에서는 허버(Huber)로 오염된 보상 모델을 고려합니다. 이 모델에서는 적대자가 소량의 확률 $\\epsilon$로 각 보상 샘플을 임의로 손상시킬 수 있습니다. 저자들은 기존의 TD 알고리즘이 이러한 오염된 보상 아래에서 임의의 가치 함수(value function)로 수렴할 수 있음을 보여주고, Robust-TD라는 새로운 알고리즘을 개발하여 유한 시간 보장이 기존의 TD와 유사하다는 것을 증명합니다.

- **Performance Highlights**: Robust-TD 알고리즘은 기존 TD 알고리즘의 선형 함수 근사(linear function approximation)에 대한 수렴 보장과 일치하며, 이러한 추가적인 오염으로 인한 항은 $O(\epsilon)$입니다. 저자들은 이러한 결과와 함께 minimax 하한(minimax lower bound)을 제시하여, 오염으로 인한 항이 불가피하다는 것을 밝힙니다. 이는 적대적 강인성을 가진 확률 근사 기법에 대한 첫 번째 연구 결과로서 중요한 기여를 합니다.
...
### Overcoming Fake Solutions in Semi-Dual Neural Optimal Transport: A Smoothing Approach for Learning the Optimal Transport Plan (https://arxiv.org/abs/2502.04583)
Comments:
          18 pages, 10 figures

- **What's New**: 본 연구에서는 Optimal Transport (OT) 맵 학습의 수렴 문제를 다룹니다. 많은 semi-dual Neural OT 방법들이 실제로는 올바른 OT 맵을 전송하지 못하는 가짜 솔루션(faked solutions)을 만들어냅니다. 우리는 Semi-dual Neural OT의 최대-최소(max-min) 솔루션이 진정한 OT 맵을 회복할 수 있는 충분한 조건을 도출하였으며, 이는 기존의 연구에서 분석되지 않았던 측면입니다.

- **Technical Details**: 우리는 Optimal Transport Plan (OTP)이라는 새로운 방법을 제안하여 OT 맵과 최적 운송 계획(optimal transport plan)을 동시에 학습합니다. 이 모델은 Hausdorff 차원이 d-1 이하인 측정 가능한 집합에 긍정적인 질량을 두지 않는 분포에 대한 충분한 조건을 기반으로 합니다. OTP는 기존 솔루션에서 발생하는 가짜 솔루션 문제를 제거함으로써 OT 문제를 올바르게 해결합니다.

- **Performance Highlights**: 실험 결과, OTP 모델은 기존 방법들이 실패하는 응용에서 최적의 운송 맵(optimal transport map)을 정확하게 회복했으며, 이미지 간 변환(image-to-image translation) 작업에서 현재의 OT 기반 모델들을 능가하는 성능을 보였습니다. 특히, 이 모델은 하나에서 여러 개로의 변환과 같은 확률적 운송 맵을 학습할 수 있어, 결정론적 OT 맵이 존재하지 않는 경우에도 유용합니다.
...
### Learning Semantics-aware Search Operators for Genetic Programming (https://arxiv.org/abs/2502.04568)
Comments:
          Submitted to GECCO 2025

- **What's New**: 이 연구에서는 test-based program synthesis의 rugged fitness landscapes 문제를 해결하기 위해 semantics-aware search operator인 NEON을 제안합니다. 기존의 genetic programming 알고리즘에서 발생하는 fitness value의 비효율성을 극복하고자 하며, 이 새로운 접근 방식은 그래프 신경망(Graph Neural Network)을 활용합니다. NEON은 최적의 candidate programs를 탐색하는 데 있어 더욱 향상된 성능을 제공합니다.

- **Technical Details**: NEON은 부모 프로그램의 다양한 수정 가능성을 예측하여, 후속 세대에서 자식 솔루션의 품질을 향상시킵니다. 이 알고리즘은 기존의 추상적인 search operators를 초월하여 domain-specific language(DSL)와 문제 인스턴스의 세부 사항을 활용하도록 설계되었습니다. 또한, GNN을 통해 개별 DSL 지침의 동작적 및 의미적 특성을 인지하고 이를 바탕으로 후보 프로그램을 평가합니다.

- **Performance Highlights**: NEON은 다양한 symbolic regression benchmarks에서 기존의 tree-based genetic programming과 비교하여 현저히 높은 성능을 보여주었습니다. 실험 결과는 NEON이 기존의 방법보다 더 효과적으로 후보 솔루션을 탐색하고, 질 높은 결과를 도출할 수 있음을 나타냈습니다. 연구의 경쟁력 있는 결과와 함께 NEON이 가지는 잠재적 가치가 강조되고 있습니다.



### Private Federated Learning In Real World Application -- A Case Study (https://arxiv.org/abs/2502.04565)
- **What's New**: 본 논문에서는 엣지(Edge) 장치에서 개인 프라이버시를 보장하며 머신러닝 모델을 훈련하는 Private Federated Learning (PFL) 구현을 제시합니다. 이 프레임워크는 사용자 데이터를 각 장치에 유지하면서도 중앙 서버로 모델 업데이트만 전송하여 개인 정보 보호를 구현합니다. 또한, 주의(attention) 메커니즘을 포함한 신경망을 이용한 앱 선택 모델 아키텍처에 대해서도 상세히 설명합니다.

- **Technical Details**: PFL은 데이터 프라이버시를 보장하기 위해 훈련 중 사용자 데이터를 장치 내부에 유지합니다. 본 연구는 앱 선택을 위한 예측 모델을 훈련하기 위해 HPFL(계층적 개인화 연합 학습) 프레임워크를 적용했습니다. 각 장치는 자신의 데이터에 기반하여 모델 업데이트를 계산하고 이 업데이트를 중앙 서버에 전송하여 글로벌 모델을 보정합니다.

- **Performance Highlights**: 이 연구에서는 오프라인 시뮬레이션과 장치 훈련을 통해 접근 방식의 실현 가능성을 입증했습니다. PFL을 통해 사용자 행동 변화를 반영하여 앱 선택 모델의 정확도를 높일 수 있으며, 이는 개인 데이터의 프라이버시를 유지하면서도 개인화된 서비스를 제공하는 데 많은 응용 가능성을 보입니다.
...
### Mixture of neural operator experts for learning boundary conditions and model selection (https://arxiv.org/abs/2502.04562)
- **What's New**: 이 연구에서는 기존의 비주기적 경계 조건을 다루는 데 한계가 있는 방법 대신, 수치적 방법에서의 볼륨 페널리제이션(volume penalization)과 기계 학습의 전문가 혼합(Mixture of Experts, MoE)에서 영감을 받은 새로운 경계 조건 부여 접근법을 제안합니다. 이 방법은 경쟁하는 전문가들을 도입하여 모델 선택이 가능하게 하며, 모달 연산자 회귀(Modal Operator Regression for Physics, MOR-Physics)를 통해 방정식이 정의된 영역에서 비선형 연산자를 복원합니다. 또한, 제안한 접근법으로 대형 소용돌이 시뮬레이션(LES) 모델을 학습하며, 배이즈 변분 추론(Bayesian variational inference)을 통해 예측치를 추출합니다.

- **Technical Details**: 이 연구에서 소개한 POU-MOR-Physics 모델은 공간적으로 구분된 MoE와 Fourier 기반 MOR-Physics 신경 연산자를 결합하여 경계 조건을 효과적으로 학습합니다. POU(Partition of Unity)에 의해 결정되는 전문가의 가중치를 통해 비연속성과 비주기적 전환에 대해 서로 다른 전문가를 선택할 수 있습니다. 또한, 학습 데이터를 보완하기 위해 미분 방정식 솔버를 모델에 포함시키고, 불확실성 정량화(Uncertainty Quantification, UQ)를 도입하여 훈련 데이터 외의 예측값 범위를 제공합니다.

- **Performance Highlights**: 본 연구의 LES 모델은 최첨단 연구인 Wang et al. [2024]의 모델에 비해 더 높은 레이놀즈 수(Re=1000)를 지원하며, 전문가 혼합 기법을 통해 더 해석 가능한 접근을 제공합니다. 제안한 방법을 통해 좌우 경계 및 비주기적 조건에 적합한 적합 모델을 개발하였으며, 최신 데이터를 기반으로한 높은 품질의 예측을 가능하게 합니다. 마지막으로, 본 모델은 데이터를 보강하는 효과를 낼 수 있으며, 불확실성을 보다 효과적으로 정량화합니다.



### Speeding up Speculative Decoding via Approximate Verification (https://arxiv.org/abs/2502.04557)
- **What's New**: 최근에 제안된 Speculative Decoding(SD) 기법은 대형 언어 모델(LLMs)의 추론 속도를 높이는 기술이다. SD는 더 작은 드래프트 LLM을 사용해 자동 회귀적으로 토큰을 생성하고 이를 바탕으로 큰 타겟 LLM이 통계적 일관성을 검증하는 방식으로 작동한다. 하지만 주기적으로 타겟 LLM을 호출하는 것은 여전히 높은 지연(latency)을 초래한다. 본 논문은 이를 해결하기 위해 SPRINTER를 제안하며, 저복잡도의 검증기를 활용해 드래프트 LLM에서 생성된 토큰이 타겟 LLM에 의해 수용될지를 예측한다.

- **Technical Details**: SPRINTER는 드래프트 LLM이 생성하는 토큰에 대한 작성을 수용하는지 여부를 예측하는 저복잡도의 검증기를 이용한다. 이를 통해, SPRINTER는 전체 작업을 연속적으로 검증함으로써 단일 호출로 드래프트 토큰의 검증을 수행하고, 타겟 LLM의 호출 횟수를 줄임으로써 지연을 감소시킨다. 이 방식은 드래프트 모델에 의해 생성된 토큰의 분포가 타겟 모델의 분포와 반드시 일치할 필요가 없다는 점에서 더 높은 속도를 달성할 수 있다. 또한, مؤخراً 발표된 연구들과의 일치성도 보여준다.

- **Performance Highlights**: SPRNTER는 여러 데이터셋 및 모델 쌍에서 성능을 평가하였으며, 지연 감소와 높은 품질의 생성 결과를 모두 달성하였다. 예를 들어, Wiki-Summaries 데이터셋에서는 SPRINTER가 SD보다 1.7배 빠른 지연을 달성하고, 8.3배 적은 연산량(flops)을 소모하면서도 높은 품질의 응답을 생성할 수 있었다. GPT2-Small과 GPT2-XL을 드래프트/타겟 모델로 활용하는 경우 이러한 성능이 입증되었다.



### Mechanisms of Projective Composition of Diffusion Models (https://arxiv.org/abs/2502.04549)
Comments:
          9 pages, 7 figures. The first two authors contributed equally

- **What's New**: 본 연구는 확산 모델(difussion models)에서 구성(composition)의 이론적 기초를 탐구하며, 특히 out-of-distribution extrapolation과 length-generalization에 중점을 두고 있습니다. 기존 연구들은 선형 점수 조합(linear score combination)을 통해 분포를 구성하여 promising한 결과를 얻었다고 밝혔으나, 이러한 구성의 작동 원리에 대한 이론적 이해는 불완전해왔습니다. 이 논문은 프로젝트 구성을 정의하고, 선형 점수 조합이 이 구성을 어떻게 달성할 수 있는지, 또한 그 과정에서의 실패 조건을 탐구합니다.

- **Technical Details**: 연구팀은 개별 확산 모델의 점수를 선형적으로 결합해 구성하는 방법에 집중하고 있습니다. 예를 들어, 개인 개 사진을 학습한 모델과 유화 스타일 모델을 결합하여 자신의 개가 그려진 유화를 생성하는 경우를 생각해 볼 수 있습니다. 이 과정에서 각 모델의 학습 데이터에는 해당 구성물에 대한 정보가 포함되어 있지 않기 때문에 out-of-distribution 이미지를 성공적으로 생성할 수 있는 이론적 조건을 분석하고 있습니다.

- **Performance Highlights**: 이 연구는 OOD 일반화(generalization) 기능을 중점적으로 다루며, 서로 다른 모델이 조합될 때 각 모델이 OOD 생성을 보장하지 않더라도 유용한 결과를 생성할 수 있도록 하는 가능성을 탐구합니다. 길이 일반화(length-generalization)라는 특정 요구 사항을 통해 여러 개념을 조합할 수 있는 능력의 이론적 기반을 구축하고 있습니다. 여러 개념을 결합할 때 적절한 점수 조합을 사용하면, 기대하는 결과를 생성할 수 있다는 것이 실험적으로 입증된 바 있습니다.
...
### Discovering Physics Laws of Dynamical Systems via Invariant Function Learning (https://arxiv.org/abs/2502.04495)
- **What's New**: 이 논문은 다양한 환경에서의 고유 동역학(intrinsic dynamics)을 탐색하면서 환경 특유의 메커니즘을 우회하는 새로운 접근 방식을 제안합니다. 기존의 연구와는 달리, 함수 계수를 넘어서는 다양한 함수 형태가 포함된 복잡한 환경 변화를 다루었습니다. 예를 들어, 진자의 자연 운동을 관찰함으로써 그 동역학을 발견하는 방식을 시연했습니다.

- **Technical Details**: 고유 함수 학습(invariant function learning) 과제를 인과 분석(causal analysis) 기반으로 접근합니다. 본 연구에서는 인과 그래프(causal graph)를 제안하고, 환경 특유의 동역학에서 고유한 함수를 분리하는 인코더-디코더 하이퍼네트워크(encoder-decoder hypernetwork)를 설계했습니다. 이 방법은 환경과 추출된 고유 함수 간의 독립성을 보장하는 정보 기반 원칙을 통해 고유 함수를 발견하는 것을 목표로 합니다.

- **Performance Highlights**: 세 가지 ODE 시스템에 대한 메타 학습(meta-learning) 및 고유 학습(invariant learning) 기준선과의 정량적 비교 결과, 제안된 방법의 효과성과 효율성을 입증했습니다. 또한, 기호 회귀(symbolic regression) 결과는 이 프레임워크가 본질적인 법칙을 밝혀내는 능력이 있음을 강조합니다.



### Provable Sample-Efficient Transfer Learning Conditional Diffusion Models via Representation Learning (https://arxiv.org/abs/2502.04491)
- **What's New**: 이 논문에서는 전이 학습 (transfer learning) 이론을 기반으로 하여 조건부 확산 모델 (conditional diffusion models, CDMs)의 샘플 효율성을 높이는 방법을 다룬다. 기존의 CDMs는 많은 데이터가 필요하지만, 사용 가능한 데이터가 적은 실용적인 상황에서 효과적인 개선 방안을 제공한다. 연구자들은 조건을 공유하는 저차원 표현 (low-dimensional representation)을 활용하여 목표 작업의 샘플 복잡성을 크게 줄일 수 있음을 보여준다.

- **Technical Details**: 논문에서는 특히 조건 인코더 (condition encoder)를 학습하는 데 필요한 샘플 복잡성을 감소시키기 위한 일반화 보장을 제시한다. 이 과정에서 네트워크의 스코어 (score)만 학습하는 접근 방식을 통해 전이 학습의 가능성을 보여준다. 또한, 다양한 실제 애플리케이션에서의 통계적 보장을 다루며, 전이 학습 기반의 조건부 확산 모델이 어떻게 적용될 수 있는지에 대한 이론적 기초를 구축한다.

- **Performance Highlights**: 연구 결과는 작은 데이터 환경에서도 CDMs의 성능을 획기적으로 향상시킬 수 있음을 나타낸다. 제안된 이론적 접근 방식을 통해 조건부 분포 추정 작업에서 제공되는 정확도와 샘플 복잡성을 감소시킬 수 있으며, 실제로 수치 실험을 통해 이러한 결과를 검증하였다. 특히, 모델 전이 학습 방식은 이미지 생성, 강화 학습, 역 문제와 같은 다양한 분야에서 적용 가능성이 높다.
...
### Iterative Importance Fine-tuning of Diffusion Models (https://arxiv.org/abs/2502.04468)
- **What's New**: 이번 연구는 diffusion 모델을 활용하여 조건부 샘플링을 보다 효율적으로 수행할 수 있는 self-supervised 알고리즘을 도입합니다. 연구의 핵심은 $h$-transform을 추정하여 diffusion 모델을 미세 조정(fine-tuning)하면서, path-based importance weights를 사용한 합성 데이터셋으로 iterative refinement 을 수행하는 것입니다.

- **Technical Details**: 제안된 방법은 주어진 데이터에서 효과적으로 posterior distribution에서 샘플링하는 기술적 도전을 해결합니다. 특히,  text-to-image diffusion 모델에 대해 class-conditional sampling과 reward fine-tuning을 적용하여 그 효과를 입증합니다.

- **Performance Highlights**: 이 프레임워크는 다양한 downstream tasks에서의 성능을 개선하는 데 큰 도움이 됩니다. 실험 결과, 제안된 방법이 기존 방법들보다 우수한 샘플링 결과를 도출함을 보여주었으며, 이는 diffusion model의 유용성을 더욱 강조합니다.
...
### Learning low-dimensional representations of ensemble forecast fields using autoencoder-based methods (https://arxiv.org/abs/2502.04409)
- **What's New**: 본 논문은 기상 예측을 위한 복잡한 앙상블 데이터의 차원 축소를 위한 새로운 방법론을 제안합니다. 기존의 차원 축소 기법은 결정론적 입력에 맞춰 설계되어 앙상블 예측 데이터를 처리하기에 적합하지 않았습니다. 저자들은 각 앙상블 구성원에 대한 배급 기반 표현을 학습하는 두 가지 대안을 제시하여, 확률적 특성을 유지하면서 저차원 표현을 생성하는 방법을 제안합니다.

- **Technical Details**: 첫 번째 접근 방식은 표준 차원 축소 기법(PCA 등)을 사용하여 각 앙상블 구성원에 대해 저차원 표현을 생성하고, 이를 공동 파라메트릭 분포 모델로 통합하는 것입니다. 두 번째 접근 방식은 변별 오토인코더(VAE)를 이용하여 앙상블 구성원을 한 번에 인코딩하여 확률적 표현을 제공합니다. 이러한 방법들은 앙상블의 공간적 및 통계적 특성을 보존하며, 예측 필드를 확률적으로 재구성하는 데 기여합니다.

- **Performance Highlights**: 저자들은 유럽 전역의 10년간 온도 및 풍속 예측 데이터를 활용한 사례 연구를 통해 두 방법을 평가하고 비교합니다. 연구 결과, 제안된 두 가지 접근 방식 모두 앙상블 예측 필드의 주요 특성을 잘 보존하며, 확률적 재구성을 가능하게 함을 보여주었습니다. 이 연구는 기상 예측 및 관련 응용 프로그램에서의 데이터 처리 효율성을 개선하는 데 중요한 기여를 할 것으로 기대됩니다.
...
### XMTC: Explainable Early Classification of Multivariate Time Series in Reach-to-Grasp Hand Kinematics (https://arxiv.org/abs/2502.04398)
- **What's New**: 본 논문에서는 사용자 손의 제스처에 대한 의도를 조기에 예측하기 위해 비동기 다변량 시계열 데이터의 분류 결과를 예측하는 새로운 접근 방식을 제시합니다. 특히, 다양한 분류기를 통합하고 가중치를 두는 앙상블(ensemble) 접근법을 사용하여 정확성과 투명성을 모두 강화하고 있습니다. XMTC 도구는 예측 결과 분석을 위한 시각화 기능을 포함하여 예측의 해석 가능성을 개선합니다.

- **Technical Details**: 제안된 XMTC 프레임워크는 비동기 다변량 시계열의 조기 예측을 위한 모형으로, 다양한 분류기의 가중 합성을 통해 기능을 확장합니다. Diverse Representation Canonical Interval Forest (DrCIF)라는 간격 기반 분류기를 활용함으로써 데이터의 왜곡을 최소화하고 있습니다. 이 모형은 랜덤으로 선택된 예측 간격에서 여러 특성을 추출하고, 통계적 특성들을 결합하여 구성된 앙상블 시계열 트리를 기반으로 합니다.

- **Performance Highlights**: XMTC는 다양한 실제 HCI 데이터 시나리오에서 적용되었으며, 조기 예측을 통해 높은 분류 정확성을 달성할 수 있음을 보여줍니다. 예측의 품질과 시간적 정확도 간의 최적의 균형을 발견하기 위해 여러 시각적 도구를 사용하고 있습니다. 연구 결과는 손/제스처 기반 인터페이스 개발에 있어 유용한 통찰을 제공하며, 예측 알고리즘을 활용하여 지연 시간을 고려하고 미래 행동을 예상하는 데 기여할 수 있습니다.
...
### Meta Audiobox Aesthetics: Unified Automatic Quality Assessment for Speech, Music, and Sound (https://arxiv.org/abs/2502.05139)
Comments:
          Repository: this https URL Website: this https URL

- **What's New**: 이 논문은 오디오 미적 평가의 주관적 특성 때문에 직면한 어려움을 해결하기 위해 자동화된 시스템의 필요성을 강조하고 있습니다. 기존 방법은 인간 청취자에게 의존하여 불일치와 리소스 요구가 크기 때문에, 인간 개입 없이 오디오 미학을 예측할 수 있는 새로운 접근법을 제안합니다. 이 연구에서는 새로운 주석 가이드라인을 도입하여 인간 청취 관점을 네 가지 축으로 분해하여 제공하고, 이를 통해 전혀 참조가 필요 없는 비침습적이며 개별 항목 예측 모델을 개발했습니다.

- **Technical Details**: 이 연구의 핵심은 오디오 미학 데이터 수집을 위한 새로운 주석 축인 AES 점수를 도입하는 것입니다. 각 축은 생산 품질(Production Quality), 생산 복잡성(Production Complexity), 콘텐츠 즐거움(Content Enjoyment), 콘텐츠 유용성(Content Usefulness)으로 구성되어 있습니다. 연구팀은 이들 축을 기반으로 하여 비침습적인 오디오 미학 예측 모델인 Audiobox-Aesthetics를 훈련하였으며, 158명의 고품질 평가자를 선정하고 총 500시간, 97,000개의 오디오 샘플을 주석 처리했습니다.

- **Performance Highlights**: 제안된 AES 예측 모델은 인간 평균 의견 점수(MOS) 및 기존 방법과 비교하여 유사하거나 우수한 성능을 보임을 입증했습니다. 이 연구는 오디오 미학 분야의 발전에 기여할 뿐만 아니라, 오픈 소스 모델 및 데이터셋을 제공하여 향후 연구 및 벤치마킹을 용이하게 하고 있습니다. 제공되는 코드와 사전 훈련된 모델은 이를 활용하려는 연구자들에게 큰 도움이 될 것입니다.



### Distinguishing Cause from Effect with Causal Velocity Models (https://arxiv.org/abs/2502.05122)
- **What's New**: 이 논문에서는 이원 구조 인과 모델(Bivariate structural causal models, SCM)을 새롭게 파라미터화 하여 원인 변수를 동적 시스템의 시간으로 보고 인과 방향성을 추론할 수 있는 방법을 제시합니다. 이러한 접근은 초기 값 문제의 해를 통해 반사실적 곡선을 정의하며, 이 과정에서 측정 이동(Measure transport) 이론을 활용하여 SCM과 점수 함수(Score function) 사이의 독특한 대응 관계를 얻습니다. 이 연구는 인과 탐색에서 기존의 가정들을 넘어서는 새로운 메서드를 개발하며, 특히 소음 분포에 대한 가정 없이도 유용합니다.

- **Technical Details**: SCM을 동적 시스템으로 보고 인과적인 속도(Causal velocity)를 통해 파라미터를 설정하는 프레임워크를 개발합니다. 이 방식은 점수 기능(Score function)에서 직접적으로 반사실적 예측을 회복 할 수 있도록 하며, 비모수적 추정이 가능하다는 장점을 가집니다. 논문은 점수 함수의 정확한 추정이 가능할 경우, 기존의 최첨단 모델들보다 우수한 성능을 보일 수 있음을 증명합니다. 또한, 여기에 도달하기 위한 다양한 수학적 정당성과 통계적 일관성을 정립합니다.

- **Performance Highlights**: 이 방법은 시뮬레이션 및 벤치마크 실험에서 ANM(Additive Noise Model) 및 LSNM(Location-Scale Noise Model) 모델들이 실패하는 상황에서도 인과 방향성을 정확하게 복원할 수 있음을 보여주었습니다. 특히, 점수 추정이 정확히 이루어질수록 모델의 유연성이 증가하여 더 복잡한 데이터 분포에서도 높은 성능을 발휘합니다. 연구진은 코드 및 실험 결과를 GitHub에 공개하여 결과의 재현성을 지원합니다.
...
- **What's New**: 이번 연구에서는 머신 러닝 기법을 적용하여 Feynman 적분의 integration-by-parts (IBP) 축소를 위한 새로운 휴리스틱을 탐색합니다. 기존의 기술들은 주로 휴리스틱 접근 방식에 의존하며 성능이 크게 영향을 받습니다. 연구팀은 LLM(대형 언어 모델)을 기반으로 한 genetic programming 변형인 funsearch를 이용하여 잠재적 접근 방식을 탐색하였습니다.

- **Technical Details**: 연구에서는 IBP 정체성과 Feynman 적분을 다루며, 유전자 알고리즘을 통한 해결책 탐색이 강조됩니다. 그 중에서도, genetic programming을 이용한 자기 최적화 방법을 적용하였고, 이로 인해 기존 방법보다 빠른 수렴이 가능했습니다. 특히, funsearch를 통해 도출된 솔루션은 기존 최첨단 솔루션과 비교하여 개선된 결과를 도출했습니다.

- **Performance Highlights**: 실험 결과, 연구진은 funsearch 기법이 제안한 새로운 휴리스틱들이 기존 IBP 축소 코드가 사용하는 전략들과 비교했을 때 유의미한 성과를 올렸음을 보여주었습니다. 특히 강화형 genetic programming 접근법은 같은 최적 솔루션을 30세대 만에 찾아내 신속한 해결책을 제공했습니다. 연구팀은 이를 통해 미래의 다양한 문제 해결을 위한 접근 방법으로서의 가능성을 제시하고 있습니다.



### Optimizing Wireless Resource Management and Synchronization in Digital Twin Networks (https://arxiv.org/abs/2502.05116)
Comments:
          12 pages, 6 figures

- **What's New**: 본 논문에서는 물리적 네트워크와 그 디지털 네트워크 쌍둥이(Digital Network Twin, DNT) 간의 정확한 동기화를 조사합니다. DNT는 물리적 네트워크의 가상 표현으로, 기지국(Base Station, BS)이 사용자에게 제한된 스펙트럼 자원을 할당하면서 DNT를 생성하기 위해 필요한 정보를 클라우드 서버로 전송하는 구조입니다. 이 연구는 BS가 필요한 정보를 모든 시간 슬롯마다 전송할 필요가 없음을 고려하여 스펙트럼 자원을 절약할 수 있는 방안을 제안합니다.

- **Technical Details**: DNT는 고급 통신 기술을 이용하여 물리적 객체와 그 쌍둥이 간의 정보 공유를 가능하게 하며, 본 연구에서는 GRU(Gate Recurrent Unit)와 가치 분해 네트워크(Value Decomposition Network, VDN)를 기반으로 한 방법을 제안합니다. 각 BS는 물리적 네트워크 정보를 클라우드 서버에 전송할지 여부를 결정하고, DNT와의 동기화를 최적화하기 위해 스펙트럼 자원 할당 정책을 결정합니다. 이 최적화 문제의 목표는 모든 사용자의 데이터 전송률을 극대화하고 물리적 네트워크와 DNT 간의 비동기화를 최소화하는 것입니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 GRU 및 VDN 기반 알고리즘이 데이터 전송률의 가중합과 DNT와 물리적 네트워크 상태의 유사성을 최대 28.96% 향상시킨 것으로 나타났습니다. 이는 기존 GRU와 독립적인 Q 학습(IQL)을 조합한 방법과 비교할 때 더욱 개선된 결과입니다. 연구는 DNT와 물리적 네트워크 간의 동기화 최적화와 네트워크 성능 향상에 기여할 것으로 기대됩니다.



### 3DMolFormer: A Dual-channel Framework for Structure-based Drug Discovery (https://arxiv.org/abs/2502.05107)
Comments:
          Accepted by ICLR 2025

- **What's New**: 이번 논문에서는 구조 기반 약물 발견(Structure-based drug discovery, SBDD) 작업에 대한 통합된 변환기 기반 프레임워크인 3DMolFormer를 제안합니다. 3DMolFormer는 단백질-리간드 도킹(protein-ligand docking)과 3D 약물 디자인(pocket-aware 3D drug design)이라는 두 가지 핵심 과제를 동시에 다룰 수 있는 능력을 가지고 있습니다. 기존의 접근 방식들이 각 작업의 이중성을 효과적으로 활용하지 못했던 문제를 해결하기 위해, 도킹 기능을 약물 디자인 과정에서 활용합니다.

- **Technical Details**: 3DMolFormer는 병렬 시퀀스 형식을 통해 3D 단백질 포켓과 소분자 리간드의 복합체를 나타냅니다. 이 구조는 이산 토큰(discrete tokens)과 연속 숫자(continuous numbers)로 구성되어 있으며, 이에 맞는 이중 채널 변환기 모델을 설계하여 3D 정보 모델링의 도전 과제를 극복합니다. 대규모 혼합 데이터셋에서의 사전 훈련(pre-training)과 두 작업에 적합하게 조정된 감독 및 강화 학습(fine-tuning) 방법을 통해 데이터 제한을 완화하고 있습니다.

- **Performance Highlights**: 실험 결과에 따르면, 3DMolFormer는 단백질-리간드 도킹 작업에서 기존의 모든 검색 기반 및 심층 학습 도킹 기준을 초월하여 예측 정확성을 크게 개선했습니다. 또한 3D 약물 디자인에서는, 설계된 복합 보상 함수를 통해 약물 후보를 생성하여 바인딩 친화성(binding affinity), 약물 유사성(drug-likeness), 합성 가능성(synthesizability) 기준을 모두 만족시켰습니다. 이는 구조 기반 약물 발견 분야에서 3DMolFormer의 뛰어난 성과를 입증합니다.



### Non-linear Quantum Monte Carlo (https://arxiv.org/abs/2502.05094)
Comments:
          30 pages

- **What's New**: 이 논문에서는 확률 분포의 비선형(non-linear) 기능에 대한 추정에서 양자 컴퓨터가 제공하는 정량적 이점에 대해 분석합니다. 기존의 몬테카를로 방법에 비해 제안된 알고리즘은 선형 리니어 기능에 대해 퀀텀 속도 향상을 달성하는 것에 그치지 않고, 보다 넓은 범위의 비선형 추정 문제에 대해서도 유사한 속도 향상을 이루고자 합니다. 이 연구는 깊은 조건부 기대값(nested conditional expectations) 및 확률적 최적화(stochastic optimization) 문제를 포함하여 여러 비선형 문제를 다룹니다.

- **Technical Details**: 제안된 양자 내부-양자 몬테카를로 알고리즘은 비선형 추정 문제들을 해결하기 위해 특별히 설계된 멀티레벨 몬테카를로(multilevel Monte Carlo) 근사를 사용합니다. 개선된 성능을 보장하는 key innovation은 양자 컴퓨팅에 최적화된 새로운 일련의 근사치 계산 방식입니다. 알고리즘의 효율성은 아난(An) 외의 연구에서 소개된 양자 멀티레벨 몬테카를로의 직접 적용을 넘어서는 것임을 보여줍니다.

- **Performance Highlights**: 이 알고리즘은 독립적이고 동일하게 분포된(i.i.d.) 샘플이 필요한 수를 감소시켜, 비선형 기능 θ(ℙ)의 추정에서 최적의 효율성을 나타냅니다. 본 연구는 추가적 오차를 ε로 제어했을 때 샘플 수가 𝒪(1/ε^2)으로 감소함을 확인시킵니다. 이로써, 양자 몬테카를로 접근 방식이 고전적인 방법에 비해 처리 속도가 두 배 빠르다는 점이 강조됩니다.



### Two-Point Deterministic Equivalence for Stochastic Gradient Dynamics in Linear Models (https://arxiv.org/abs/2502.05074)
- **What's New**: 이번 논문에서는 랜덤 행렬의 resolvent의 두 점 함수에 대한 새로운 결정론적 동등성을 도출했습니다. 이 결과를 바탕으로, 확률적 경량 하강법(stochastic gradient descent)으로 학습된 다양한 고차원 선형 모델의 성능을 통합적으로 유도합니다. 이 연구는 고차원 선형 회귀, 커널 회귀 및 랜덤 피처 모델을 포함하여 기존의 비대칭적 결과와 새로운 결과를 포함합니다.

- **Technical Details**: 논문에서는 랜덤 피처 모델의 학습 동역학을 분석하기 위한 선형 모델을 설정합니다. 주목할 점은 SGD가 محدود된 데이터와 특징들, 그리고 SGD 소음의 상호작용을 모델링할 수 있도록 해주는 새로운 결정론적 동등성을 도출한 것입니다. 또한, S-transform과 자유확률의 연결성을 활용해 학습 손실의 비대칭적 표현을 얻었습니다.

- **Performance Highlights**: 그림 5와 6에서는 선형 회귀 모델과 랜덤 피처 모델에 대한 성능을 비교합니다. 이 성능 분석을 통해 이전 문헌에서 다뤄진 분포 외(out-of-distribution) 결과를 복원하고, 동역학의 설정으로 확장했습니다. 특히, 두 점 결정론적 동등성의 관계를 통해 자유확률의 S-transform에 대한 새로운 해석을 제공하며, 이는 응답 함수로 해석될 수 있습니다.



### Noise Sensitivity of Hierarchical Functions and Deep Learning Lower Bounds in General Product Measures (https://arxiv.org/abs/2502.05073)
Comments:
          17 pages

- **What's New**: 이 논문은 심층 학습(deep learning) 분야에서의 두 가지 연구 흐름을 결합하는 것을 목표로 하고 있습니다. 특히, 함수의 계층 구조(hierarchical structure)와 노이즈 민감성(noise sensitivity)의 관계를 탐구합니다. 이 연구는 비선형 함수(non-linear functions)의 반복 조합을 통해 생성된 함수들이 일반적인 곱 측정(product measures) 하에서 노이즈 민감함을 증명하는 데 중점을 두고 있습니다.

- **Technical Details**: 연구에서는 데이터 생성 과정(data generative processes)이나 학습된 함수(learned functions)의 계층적 속성이 심층 학습의 효과에 기여하는 방식을 조사합니다. 특히, 다중 선형(multilinear) 함수의 경우를 다루며, 이들이 비정형 함수의 분해를 통해 구축되었을 때 노이즈 민감성(noise sensitivity)을 가질 수 있음을 보여줍니다. 이러한 결과는 모든 형태의 곱 측정(product measures)에 유효하며, 심층 신경망(deep neural networks)에서의 경량 학습(complexity bounds)을 설정할 수 있습니다.

- **Performance Highlights**: 본 논문은 비선형 함수의 조합이 노이즈 민감성을 지닌다는 것을 처음으로 보이며, 이를 통해 경량 학습의 하한을 도출할 수 있음을 시사합니다. 특히, 푸리에 변환(Fourier expansion)에서의 계층 구조(hierarchical structure)가 경량 학습의 수렴 속도(convergence speed)에 미치는 영향 또한 언급되었습니다. 이러한 발견은 향후 심층 학습의 효과를 높이는 데 있어 중요한 기여를 할 것으로 기대됩니다.
...
### Near-Optimal Online Learning for Multi-Agent Submodular Coordination: Tight Approximation and Communication Efficiency (https://arxiv.org/abs/2502.05028)
Comments:
          Accepted to ICLR 2025

- **What's New**: 본 논문에서는 다수의 에이전트를 협력하여 하위 모듈화 함수(submodular functions)를 최대화하는 새로운 알고리즘인 MA-OSMA와 MA-OSEA를 제안합니다. 기존 OSG 알고리즘의 한계를 극복하기 위해 제안된 알고리즘은 완전 연결 통신 그래프에 대한 엄격한 요구 사항을 완화하고 연속 최적화(continuous optimization) 기법을 활용합니다. 또한, 새로운 대체 그래디언트(surrogate gradient)를 사용하여 최적이 아닌 정지점을 피할 수 있습니다.

- **Technical Details**: MA-OSMA는 다중 선형 확장을 활용하여 하위 모듈러 최대화 문제를 연속 최적화로 변환합니다. 이 과정에서는 분포가 혼합된 KL 발산(KL divergence)을 사용하여 계산 집약적인 투영 연산(projection operation)을 제거합니다. MA-OSMA와 MA-OSEA 두 알고리즘 모두 O(√(CT/T(1-β)))의 후회 경계를 제공하며 기존 OSG 알고리즘보다 크게 개선된 성능을 보여줍니다.

- **Performance Highlights**: 제안된 알고리즘은 시뮬레이션 기반의 다중 목표 추적(multi-target tracking) 사례를 통해 효과성을 입증하였습니다. 알고리즘은 각 에이전트가 자신의 행동과 다른 에이전트의 행동을 평가하여 마진 이득(marginal gains)을 계산하는 방식을 사용합니다. 이러한 접근 방식은 에이전트의 관찰 범위를 줄여주며, 분산 최적화 분야의 합의 기법(consensus techniques)을 수용하여 효과적인 협력을 가능하게 합니다.
...
- **What's New**: 본 연구는 생존 분석(survival analysis) 모델에서 딥러닝을 활용하면서도 해석 가능성을 높이기 위한 새로운 프레임워크를 제안합니다. 기존의 LIME이나 SHAP과 같은 방법에 생존 신경망(survival neural networks)용 기법을 접목시켜, 개인화된 의학에서 딥러닝 모델의 해석 가능성을 극대화하고자 합니다. 새로운 기법 GradSHAP(t)는 기존 방법들에 비해 계산 속도와 정확도를 모두 향상시키는 데 기여합니다.

- **Technical Details**: 제안된 방법론은 지역적(local) 및 전역적(global) 예측 해석을 제공하는 여러 기법을 통합하여 이론적 가정을 체계적으로 평가합니다. 특히, 시간적 차원을 포함한 시각화를 통해 생존 데이터에서의 해석 가능성을 강조하며, gradient 기반 기법을 사용하여 기능적 함수의 결과에 대한 시간을 기반으로 한 설명을 구현합니다. GradSHAP(t)는 SurvSHAP(t)와 비교했을 때 효과적인 성과를 보여주며, 프로세스의 주요 특징을 잘 포착합니다.

- **Performance Highlights**: 실험 결과, 제안된 GradSHAP(t) 방법은 기존의 SurvSHAP(t) 및 SurvLIME보다 더 나은 성능을 보여주었으며, 특히 빠른 계산 속도와 정확도의 균형을 잘 맞추었습니다. 또한, 다중 모달 입력을 가진 의료 데이터에 적용했을 때, 예측과 관련된 특성과 그 시간적 변동성을 잘 포착할 수 있었음을 입증했습니다. 본 연구는 생존 분석을 위한 새로운 해석 가능성 방식으로 의학 연구와 임상 의사결정에 중요한 기여를 할 것으로 기대됩니다.



### Towards Smarter Sensing: 2D Clutter Mitigation in RL-Driven Cognitive MIMO Radar (https://arxiv.org/abs/2502.04967)
Comments:
          6 pages, 8 figures. Submitted to EuCNC 2025

- **What's New**: 이 논문은 6세대(6G) 네트워크를 위한 통합 감지 및 통신에 대한 관심이 높아지는 가운데, 강화 학습(rl)을 통해 향상된 인지 다중 입력-다중 출력(mimo) 레이더 시스템을 제시합니다. 이 시스템은 동적 환경에서의 신뢰성 있는 다중 목표 탐지를 목표로 한다.

- **Technical Details**: 계획형 배열(planar array) 구성을 사용하는 이 시스템은 전달된 파형(transmitted waveforms) 및 빔포밍(bmforming) 패턴을 조정하여 알려지지 않은 2차원(2D) 왜곡이 있는 상황에서도 탐지 성능을 최적화합니다. 로버스트 월드 유형 감지기(Wald-type detector)와 SARSA 기반의 강화 학습 알고리즘을 통합하여 복잡한 잡음 환경에 적응할 수 있도록 설계되었습니다.

- **Performance Highlights**: 시뮬레이션 결과, 이 시스템은 특히 클러터 환경에 의해 가려진 낮은 신호 대 잡음비(SNR) 목표에 대해 전방향 방법(omnidirectional methods)보다 탐지 확률(detection probability)에서 상당한 개선을 보였습니다.
...
### Does Unsupervised Domain Adaptation Improve the Robustness of Amortized Bayesian Inference? A Systematic Evaluation (https://arxiv.org/abs/2502.04949)
- **What's New**: 본 연구는 시뮬레이션 기반 추론 방법인 신경 암호화 베이지안 추론(ABI)의 견고성을 높이기 위해 비지도 도메인 적응(UDA) 접근법을 체계적으로 평가합니다. 특히, 다양한 도메인 불일치 시나리오에서 UDA 방법을 테스트하여 특정 오류 유형의 중요성을 강조합니다. 이 결과는 기존 연구에서 다루지 않았던 모델 사전 미스스펙에 관한 문제를 명확히 드러냅니다.

- **Technical Details**: 연구에서는 도메인 적대적 신경망(DANN)을 신경 후방 추정(NPE)용으로 조정하고, 강력한 ABI을 위한 유용성을 평가합니다. 또한, 다른 미스스펙 소스에 따라 견고한 방법들을 분류하여 이론적으로 그 강점과 한계를 평가합니다. 시뮬레이션 기반 추론(SBI) 메소드를 통해 학습된 조건부 분포 q(𝜽|𝒙)를 활용하여 복잡한 데이터에서도 신속한 추론이 가능하도록 합니다.

- **Performance Highlights**: UDA 기반 ABI 방법은 다양한 미스스펙 시나리오에서 평가돼 그 유용성이 입증되었습니다. 실험 결과, 도메인 간의 요약 공간을 정렬함으로써 모델링되지 않은 현상이나 노이즈의 영향을 효과적으로 완화할 수 있음을 보여줍니다. 그러나 사전 미스스펙 상황에서 동일한 정렬 메커니즘이 실패를 일으킬 수 있다는 점이 중요하게 지적되며, 이는 실제 활용에 중대한 영향을 미칠 수 있습니다.
...
### Explainable and externally validated machine learning for neuropsychiatric diagnosis via electrocardiograms (https://arxiv.org/abs/2502.04918)
Comments:
          9 pages, 2 figures, source code under this https URL

- **What's New**: 이번 연구는 심전도(ECG) 분석을 통해 신경정신 질환을 식별하는 새로운 가능성을 제시했습니다. ECG의 변화를 통해 신경정신 질환의 진단, 치료 모니터링 및 위험 분류에서 중요한 바이오마커(biomarker)가 될 수 있음을 강조합니다. 이 연구는 기계 학습 모델을 사용하여 다양한 환자 집단을 대상으로 신경정신적 상태를 예측하는 데 초점을 맞췄습니다.

- **Technical Details**: 본 연구에서는 MIMIC-IV-ECG 데이터베이스에서 데이터를 수집하여 심전도 특징과 인구 통계학적 정보를 활용했습니다. 급성기환자 데이터(ACD)와 외부 검증을 위해 ECG-VIEW-II 데이터베이스와 함께 AUROC 점수로 성과를 평가했습니다. 모델 해석을 강화하기 위해 Shapley 값을 사용하여 각 ECG 특징의 기여도를 분석합니다.

- **Performance Highlights**: 모델 성능은 신경계 및 정신과 조건 모두에서 높은 예측 성과를 보였습니다. 알츠하이머(G30)의 경우 내부 AUROC이 0.813, 외부 AUROC이 0.868에 달했습니다. 또한, 비지정 치매(F03)의 경우 내부 AUROC이 0.849, 외부 AUROC이 0.862로 확인되어, 신경정신 질환 진단에 있어 ECG의 유용성을 강조합니다.



### Scalable and consistent embedding of probability measures into Hilbert spaces via measure quantization (https://arxiv.org/abs/2502.04907)
- **What's New**: 이 연구에서는 확률 측정을 데이터로 사용하는 통계적 학습에 초점을 맞추고 있습니다. 기존에 널리 사용되는 방법들은 데이터를 Hilbert 공간에 매핑하는 것이지만, 이러한 과정은 대규모 데이터 처리에서 높은 계산 비용으로 인해 실용적이지 못합니다. 이 논문은 양적 및 평균 측정을 기반으로 한 두 가지 접근법을 제안하며, 이러한 방법들이 안정적인 통계적 성질을 유지하면서도 계산 비용을 낮출 수 있음을 입증합니다.

- **Technical Details**: 연구진은 K-quantization 단계를 도입하여 입력 확률 측정을 상대적으로 적은 수의 점으로 근사화하는 방법을 사용합니다. 첫 번째 방법은 각 입력 측정의 최적 양적화에 기반하고, 두 번째 방법은 평균 측정의 양적화에 의존합니다. 각 방법의 성능을 평가하기 위해 Wasserstein distance를 이용하여 확률 측정의 수렴 성질을 분석합니다.

- **Performance Highlights**: Numerical experiments에서 제안된 방법들이 기존의 LOT(Linearized Optimal Transport) 및 KME(Kernel Mean Embedding) 방법들에 비해 더 큰 데이터 세트에서 낮은 계산 비용으로도 효과적으로 작동함을 보여주었습니다. 이 연구는 대규모 데이터 처리의 가능성을 제시하며, 실제 데이터 분석에 즉시 적용될 수 있는 범위를 넓힙니다.



### On the Difficulty of Constructing a Robust and Publicly-Detectable Watermark (https://arxiv.org/abs/2502.04901)
- **What's New**: 이번 연구는 공개적으로 감지 가능한 수단을 통해 수탁 화상의 출처(provenance)를 생성하는 이론적 한계를 조사합니다. C2PA와 같은 메타데이터 기반 접근법은 변조 불가능성과 공개 감지가 가능하지만, 현재의 어떤 시스템도 강건함(robustness), 변조 불가능성(unforgeability) 및 공개 감지(public-detectability)를 모두 결합하지 못합니다. 이 논문에서는 이러한 조합이 이론적으로 가능하다는 것을 정의하고 존재성을 확립하였으나, 현재의 딥러닝 능력으로는 특정 구성 요소를 구축하기 어렵다는 점을 발견했습니다.

- **Technical Details**: 이 논문은 강건한 수탁 화상이 어떤 속성을 가져야 하는지를 다룹니다. 즉, cryptographic unforgeability와 robustness to accidental stripping을 포함한 공개 감지가 포함되어야 하며, 품질을 유지하는 것이 필요합니다. 연구진은 이러한 요구사항을 충족하는 수탁 시스템의 구성을 위해 cryptographic signatures와 robust embeddings를 활용해야 한다고 주장합니다.

- **Performance Highlights**: 주요 기여는 공개 감지 가능하나 제한된 강건성을 가진 수탁 기법을 제안한 것입니다. 연구진은 adversarial 공격에 취약한 최첨단 이미지 임베딩 모델의 문제점을 분석했지만, 인간의 시각을 더 잘 포착하는 미래의 모델은 본질적인 adversarial 강건성을 갖게 돼 robust하고 공개적으로 감지 가능한 수탁을 가능하게 할 것으로 기대하고 있습니다.



### Any-stepsize Gradient Descent for Separable Data under Fenchel--Young Losses (https://arxiv.org/abs/2502.04889)
- **What's New**: 이번 연구에서는 gradient descent (GD) 최적화 알고리즘이 자가 경계(self-bounding) 속성을 가지지 않는 다양한 손실 함수에서 임의의 스텝 사이즈로 수렴할 수 있음을 발견하였다. 특히, Fenchel–Young 손실 프레임워크를 통해 GL의 수렴 성질을 분석하고, Tsallis 엔트로피와 R{é}nyi 엔트로피의 놀라운 수렴 속도를 보였다. 자가 경계 속성이 없는 손실 함수에서도 GD의 수렴 가능성이 있음을 제안하는 중요한 전환점이 된다.

- **Technical Details**: 본 연구는 손실 함수의 자가 경계 속성이 GD의 수렴에 미치는 영향을 탐구하며, Fenchel–Young 손실 클래스를 중심으로 기초 이론을 쌓아간다. 주목할 점은, 임의의 스텝 사이즈 η>0 선택 시 GD가 Fenchel–Young 손실에 대해 수렴한다는 점이다. 이를 통해, 전통적인 수렴 분석을 넘어, 손실 함수의 분리 마진(separation margin)이 GD 성능에 미치는 영향도 명확히 하였다.

- **Performance Highlights**: Tsallis 엔트로피의 경우 GD 수렴 속도가 T=Ω(ε^{-1/2})를 달성하였고, R{é}nyi 엔트로피는 그보다 더 나은 속도인 T=Ω(ε^{-1/3})를 기록하였다. 이러한 성능은 손실 함수의 분리 마진 때문에 가능하다고 주장하며, 기존의 자가 경계 속성에 의존하지 않고도 GD가 효과적으로 작동할 수 있음을 시사한다. 즉, 다양한 손실 함수에 대한 GD의 능력을 확장하기 위한 강력한 고려 사항으로 자리잡았다.
...
- **What's New**: 이번 연구에서는 학습 알고리즘이 점점 더 플랫폼에서 의존하게 됨에 따라, 집단이 플랫폼에 미치는 영향력을 조정하는 방법을 연구합니다. 연구자들은 집단이 데이터를 변경하고 확산하는 과정에서 수행해야 할 계산을 이해하는 것이 중요하다는 점을 강조합니다. 특히, 집단은 행동에 나서기 전에 미리 플랫폼에 대한 영향력을 평가해야 합니다. 연구자들은 이와 관련한 새로운 프레임워크를 개발하여 집단이 구현 가능한 조정 알고리즘을 설계할 수 있도록 합니다.

- **Technical Details**: 새로운 프레임워크는 집단이 플랫폼에 미치는 영향을 파악하기 위해 통계적 추론(statistical inference) 방법을 제공합니다. 이 프레임워크는 세 가지 주요 목표, 즉 신호 심기(signal planting), 신호 지우기(signal erasing), 그리고 신호 철회(signal unplanting)를 달성하기 위한 전략을 도출합니다. 연구자들은 이러한 목표를 위해 집단이 사용할 수 있는 다양한 전략을 탐구하고, 이들 전략의 효과에 대한 이론적 보장을 제시합니다. 또한, 각 특성(feature)의 저항과 공격 강도의 관계를 이용하여 데이터 오염(data poisoning) 경계에 대한 새로운 해석을 제공합니다.

- **Performance Highlights**: 실험 결과, 집단의 효과는 플랫폼과 상호작용하는 에이전트의 수에 비례하여 증가합니다. 특히, 집단의 절대적인 크기가 클수록 더 좋은 통계적 추정을 가능하게 하여 최적의 전략을 더욱 정확하게 추론할 수 있습니다. 이러한 결과는 더 큰 플랫폼이 집단 행동에 더 취약하다는 것을 시사하며, 연구자들은 이론적 결과를 검증하기 위해 합성 데이터셋을 구축했습니다.
...
### Advancing Wasserstein Convergence Analysis of Score-Based Models: Insights from Discretization and Second-Order Acceleration (https://arxiv.org/abs/2502.04849)
- **What's New**: 본 연구는 score-based diffusion models의 이론적 기초를 탐구하는 새로운 접급을 제시합니다. Wasserstein 수렴 분석에 초점을 맞추어, 다양한 discretization schemes(이산화 기법)의 영향을 조사하고 있습니다. 이러한 기법은 Euler discretization, exponential integrators, midpoint randomization methods를 포함합니다.

- **Technical Details**: 이 논문에서는 이러한 이산 근사치들의 정량적 비교를 통해 수렴 행동에 대한 이해를 높이고 있습니다. 또한 Hessian 정보가 제공되는 상황을 탐색하며, 지역 선형화 방법(local linearization method)을 기반으로 한 가속 샘플러(accelerated sampler)를 제안합니다. 이를 통해 수렴 속도를 개선하고자 하는 본 연구의 접근 방법이 강조됩니다.

- **Performance Highlights**: Hessian 기반 접근 방식은 표준 rate인 $\tilde{\mathcal{O}}(\frac{1}{\varepsilon^2})$에 비해 $\tilde{\mathcal{O}}(\frac{1}{\varepsilon})$로 더 빠른 수렴 속도를 달성한 것으로 나타났습니다. 이 연구는 score-based diffusion 모델의 성능을 상당히 개선하여, 효율적인 generative modeling을 위한 중요한 기여를 하고 있습니다.



### Coherent Local Explanations for Mathematical Optimization (https://arxiv.org/abs/2502.04840)
- **What's New**: 이번 연구에서는 수학적 최적화를 위한 Coherent Local Explanations for Mathematical Optimization(CLEMO)라는 새로운 접근법을 제안합니다. CLEMO는 최적화 모델의 다양한 구성 요소에 대한 설명을 제공하며, 모델의 구조와 일관성이 있는 설명을 제시합니다. 기존의 분석 방법들이 최적화 문제의 구조를 고려하지 않아 신뢰할 수 없는 결과를 초래했던 문제를 해결하고자 합니다.

- **Technical Details**: CLEMO는 샘플링 기반의 방법으로, 최적화 모델의 목표 값 및 의사 결정 변수에 대한 설명을 제공합니다. 이 접근법은 설명 모델의 일관성을 평가하기 위해 정규화 기법을 설계하였으며, 모든 정확한 알고리즘 및 휴리스틱 알고리즘에 적용 가능하다고 주장합니다. CLEMO는 다양한 최적화 문제에 대해 정확성, 해석 가능성, 일관성 및 안정성을 조사하여 그 효과를 검증하였습니다.

- **Performance Highlights**: 실험은 최단 경로 문제, 배낭 문제 및 차량 경로 문제를 포함한 여러 유명한 최적화 문제에 대해 수행되었습니다. CLEMO는 각 문제에 대해 모델의 구조와 일관된 설명을 제공함으로써 결정 메이커의 이해를 돕고, 그 결과로 설명의 신뢰성과 안정성을 높였습니다. 이러한 성과는 최적화 모델의 결정 사항을 명확하게 전달하는 데 기여합니다.
...
### Robust Conformal Outlier Detection under Contaminated Reference Data (https://arxiv.org/abs/2502.04807)
- **What's New**: 이 논문은 분류 불가능한 (distribution-free) 통계적 보장을 제공하는 변동 예측(conformal prediction) 프레임워크를 통한 아웃라이어(Outlier) 탐지 문제를 다룬다. 특히, 레퍼런스 데이터세트에 아웃라이어가 혼합된 경우, 이로 인해 발생하는 문제점과 그 해결책을 제시한다. 저자들은 데이터 세트의 오염이 변동 예측 방법의 유효성에 미치는 영향을 분석하고, 이 과정에서 다루는 기술적으로 중요한 요소들을 강조한다.

- **Technical Details**: 연구에서 제안된 방법은 제한된 라벨링 예산과 아웃라이어 탐지 모델을 활용하여, 오염된 레퍼런스 세트에서 아웃라이어로 의심되는 데이터 포인트를 선별적으로 주석을 붙이는 방식을 포함한다. 이를 통해 데이터의 품질을 향상시키고, typ-I 오류율 상승의 위험을 줄일 수 있다. 본 논문에서는 이론적 분석을 통해 이러한 방법이 유효한지를 검증하며, 아웃라이어 탐지 모델을 통해 얻은 결과가 실질적 효과를 낼 수 있음을 보인다.

- **Performance Highlights**: 실제 데이터셋에서 수행된 실험은 오염된 데이터에서의 변동 예측 방법이 보수적인 경향이 있음을 보여주고 있으며, 제안된 데이터 정제 전략이 유의미한 탐지력을 향상시키는 것을 입증하고 있다. 특히, 타겟 typ-I 오류율이 낮고 오염된 세트 내 아웃라이어 수가 적을 때, 탐지 성능이 더욱 개선됨을 나타낸다. 재현 가능한 실험을 위한 소프트웨어는 GitHub에 제공된다.
...
- **What's New**: 이번 논문에서는 Lipschitz 연속 Hessian을 가진 비볼록 함수의 $\'\epsilon$-정적 점을 찾는 문제를 다루고, 현재 및 이전 기울기로부터 구성된 새로운 클래스의 정규화를 포함하는 이차 정규화 Newton 방법을 제안합니다. 이 방법은 부정적인 곡률 모니터를 가진 선형 켤레 경량 접근 방식을 활용하여 정규화된 Newton 방정식을 해결합니다. 특히, 이 알고리즘은 Hessian의 Lipschitz 상수에 대한 사전 지식이 필요하지 않으며, 전역 복잡도에서 $O(\epsilon^{-\frac{3}{2}}) + \tilde O(1)$을 달성합니다.

- **Technical Details**: 이 방법은 로컬 수렴 속도가 이차적인 특성을 갖는 유사 뉴턴 방법이며, 강한 볼록성이 없는 경우에도 활용될 수 있습니다. 주어진 벡터 공간에 대하여 최적의 방향으로 유도된 경량 지원 방식인 Conjugate Gradient (CG) 방법에 의존하고, 각 CG 반복은 Hessian-벡터 곱을 요구합니다. 특히, 이 알고리즘은 비볼록 함수에서 발생하는 문제를 겪는 정규화된 하위 문제에 대한 적절한 수정이 필요합니다.

- **Performance Highlights**: 초기 수치 결과는 제안된 알고리즘이 경쟁력이 있음을 나타냅니다. 이 논문에서 제안한 방법은 전역 수렴을 보장하고 네트워크에서 높은 효율성을 보여주며, 적응형 알고리즘의 특성을 지니고 있습니다. 이러한 성과는 비볼록 최적화 문제에 대한 유의미한 진전을 나타내며 향후 연구에 발판이 될 것입니다.



### $t$-Testing the Waters: Empirically Validating Assumptions for Reliable A/B-Testing (https://arxiv.org/abs/2502.04793)
- **What's New**: 이번 연구는 A/B 테스트에서의 신뢰성을 높이기 위한 새로운 방법론을 제안합니다. 특히, 통계적 가설 검정(framework)에서 p-value의 분포가 균일해야 한다는 가정 하에, 실제 데이터를 기반으로 한 A/A 테스트를 통해 이를 검증합니다. 이를 통해 A/B 테스트의 정확성을 보장하고 Type-I 오류를 줄일 수 있는 실용적인 프레임워크를 제공합니다.

- **Technical Details**: 연구에서는 A/B 테스트의 성과를 평가하기 위해, 전자 신뢰구간(confidence interval, CI)과 p-value의 분포를 확인하는 방법론을 적용합니다. 중심극한정리(Central Limit Theorem, CLT)로부터 유도된 이 방법은, A/A 테스트를 통해 반복 샘플링한 p-value가 균일 분포를 이루는지 검증하여 정상성(normality) 가정을 평가합니다. 결과적으로, CIs가 참 ATE를 포함하는지 여부를 판단할 수 있는 신뢰성을 확보할 수 있습니다.

- **Performance Highlights**: 제안된 접근법을 통해 연구는 기존 A/B 테스트에서의 낮은 신뢰성 문제를 보완할 수 있는 새로운 관점을 제공합니다. 이 방법은 통계적 rigor(정확성)를 높여 A/B 테스트의 신뢰성 있는 추정치(estimands)를 도출할 수 있도록 돕습니다. 이를 통해 실무에서 자주 발생하는 잘못된 긍정 오류를 검출하고, 향후 데이터 분석의 정확성을 향상시킬 수 있는 가능성을 보여줍니다.



### Efficient Evaluation of Quantization-Effects in Neural Codecs (https://arxiv.org/abs/2502.04770)
- **What's New**: 이 논문은 신경 코덱(neural codec)의 효율적인 평가 프레임워크를 제안합니다. 또한, 훈련 요구사항과 신뢰할 수 있는 메트릭의 부족으로 인해 비용과 시간이 많이 소요되는 양자화(quantization)의 효과를 평가하기 위한 접근 방식을 제공합니다.

- **Technical Details**: 논문에서 제안하는 방법은 정의된 비트 수와 저복잡도 neural encoder/decoder를 사용하여 시뮬레이션된 데이터를 활용합니다. 이를 통해 대규모 네트워크에서의 비선형(non-linear) 동작을 모방할 수 있으며, 훈련 시간과 컴퓨팅 요구사항을 크게 줄이는 데 기여합니다.

- **Performance Highlights**: 본 연구는 내부 신경 오디오 코덱(neural audio codec) 및 최신의 descript-audio-codec과 비교하여 제안된 방법의 유효성을 검증하였습니다. 특히, straight-through estimator를 기반으로 한 훈련 안정화(modification)의 효과를 강조하며, 신경 코덱의 서로 다른 행동을 발견할 수 있는 기회를 제공합니다.



### Shapley Value Approximation Based on k-Additive Games (https://arxiv.org/abs/2502.04763)
- **What's New**: 이번 연구에서는 Shapley 값을 근사화하기 위한 새로운 방법인 SVA_kADD를 제안합니다. 이 방법은 k-additive surrogate 게임을 적합시킴으로써 Shapley 값을 정확하게 도출하고, 이를 통해 원래의 공정한 분배 문제를 해결하는 데 필요한 근사 값을 제공합니다. 이러한 접근 방식은 기존의 공정 분배 문제 해결 방법의 한계를 극복할 수 있는 잠재력을 보여줍니다.

- **Technical Details**: SVA_kADD 방법은 k-additivity 개념에 기반하여 구성된 대체 게임을 사용합니다. 이 방법은 주어진 게임의 파라미터화 된 가치 함수 구조를 활용하여 최적 값을 제공하며, Shapley 값의 수학적 유효성을 이론적으로 증명합니다. 이 방법은 특정 값 함수의 구조적 특성에 의존하지 않기 때문에 다양한 협력 게임에 적용될 수 있습니다.

- **Performance Highlights**: 연구는 SVA_kADD의 효능을 다양한 설명 과제를 통한 경험적 비교를 통해 평가하며, 기존의 대안 방법들과 비교하고 있습니다. 실험 결과, 제안된 방법이 지역적 및 전역적 설명을 모두 처리할 수 있는 모델 불변성(mold-agnostic) 특성을 가지고 있음을 확인하였으며, Shapley 값을 근사하는 데 있어 뛰어난 성능을 보여줍니다.



### Differential Privacy of Quantum and Quantum-Inspired-Classical Recommendation Algorithms (https://arxiv.org/abs/2502.04758)
Comments:
          17 pages, 6 figures in total(including appendix)

- **What's New**: 이 논문에서는 양자 추천 알고리즘(Quantum Recommendation Algorithm)과 양자 영감을 받은 고전 추천 알고리즘(Quantum-Inspired Classical Recommendation Algorithm)의 차별적 개인정보보호(Differential Privacy, DP) 특성을 분석하였습니다. 특히, 양자 추천 알고리즘은 외부 잡음이 필요 없는 자기 수렴적 메커니즘으로, 전통적 DP 방법과는 다르게 작동함을 발견했습니다. 이 외에도, SVD(특이값 분해)와 저랭크 행렬 근사 문제에 적합한 새로운 섭동 방법(Perturbation Method)을 도입했습니다.

- **Technical Details**: 본 연구에서는 $ig(	ilde{	ext{O}}ig(rac{1}{n}ig), 	ilde{	ext{O}}ig(rac{1}{	ext{min}ig
ewline igackslash{m,nigackslash}}ig)ig)$-DP라는 결과를 통해 양자 추천 알고리즘과 양자 영감을 받은 고전 추천 알고리즘의 식별성을 확보했습니다. 논문에서 제안하는 새로운 섭동 방법을 통해 이 두 알고리즘의 개인정보 보호 성능과 관련된 수치적 결과를 도출하였습니다. 이 방법들은 전통적인 개인 정보 보호 메커니즘과는 달리, 아예 외부의 잡음 없이 개인정보를 보호할 수 있는 특성을 보여주고 있습니다.

- **Performance Highlights**: 양자 추천 알고리즘은 고전적인 접근 방식보다 더 나은 개인정보 보호 잠재력을 가지고 있으며 적대자(Adversary)에 대한 저항력도 더 강하다는 것이 밝혀졌습니다. 두 알고리즘 모두 비슷한 특성을 가지고 있지만, 양자 알고리즘이 특정 조건 하에서 더 효과적인 개인 정보 보호 메커니즘으로 작용한다는 점이 중요합니다. 본 연구는 양자 컴퓨팅과 차별적 개인정보 보호의 융합을 탐구함으로써 기존 연구의 공백을 메우고, 향후 간섭이 없는 데이터 관리 방식의 가능성을 제시합니다.
...
### Tighter sparse variational Gaussian processes (https://arxiv.org/abs/2502.04750)
- **What's New**: 본 논문은 inducing points를 기반으로 한 희소 변분 가우시안 프로세스(SVGP)의 새로운 변형을 제안합니다. 기존의 표준 가정에서 조건부 변분 후분포가 사전분포와 일치해야 한다는 제약을 완화하여, 더욱 효율적인 변분 근사를 증명합니다. 이 혁신적인 접근법은 훈련 지점에서 사전분포보다 작은 분산을 가진 조건부 후분포를 적용하는 것입니다.

- **Technical Details**: 가우시안 프로세스(GP) 모델에서는 데이터의 관측값에 대한 조건부 후분포가 주어지면, 이를 통해 회귀 또는 분류 작업을 수행합니다. 본 연구는 사전 분포를 기준으로 한 조건부 접근을 수정해 새로운 낮은 경계(lower bound)를 제시하며, 이 과정에서 추가적인 변분 매개변수가 필요 없습니다. 이는 스토캐스틱 미니 배치 최적화(stochastic mini-batch optimization)와 비가우시안 가능도(non-Gaussian likelihood)에도 쉽게 확장될 수 있습니다.

- **Performance Highlights**: 제안된 접근법은 다양한 회귀, 분류, 잠재 변수 모형(latent variable model) 작업의 예측 성능을 지속적으로 향상시키며, 표준 희소 변분 GP와 비교하여 동일한 계산 비용으로 더 나은 성능을 보여줍니다. 실험 결과에 따르면, 극대화된 로그 주변 가능도(log marginal likelihood estimates)에서도 개선된 결과를 확인할 수 있습니다. 이러한 방법은 모든 인기 있는 GP 패키지에서 구현될 예정입니다.
...
### PhyloVAE: Unsupervised Learning of Phylogenetic Trees via Variational Autoencoders (https://arxiv.org/abs/2502.04730)
Comments:
          ICLR 2025. 22 pages, 14 figures

- **What's New**: 이 논문에서는 고유한 속성을 가지는 phylogenetic tree(계통수) 구조를 분석하기 위해 phylogenetic variational autoencoders (PhyloVAEs)를 도입했습니다. PhyloVAE는 비지도 학습(unsupervised learning) 틀 안에서 tree topologies(나무 구조)의 표현 학습과 생성 모델링을 동시에 처리할 수 있는 혁신적인 방법입니다. 기존의 거리 기반 방법들과 비교하여 더 높은 분해능과 효율성을 제공합니다.

- **Technical Details**: PhyloVAEs는 autoregressive tree topology generation(자기 회귀 트리 구조 생성)에서 영감을 받은 효율적인 인코딩 메커니즘을 이용하여 깊은 잠재 변수 생성 모델을 개발합니다. 해당 모델은 learnable topological features(학습 가능한 토폴로지 특징)를 활용하여 latent variable(잠재 변수)의 후방 분포를 추론합니다. 이렇게 함으로써, PhyloVAE는 tree topology distribution(트리 구조 분포)을 높은 해상도로 시각화할 수 있는 잠재 공간 표현을 제공합니다.

- **Performance Highlights**: 많은 실험에서 PhyloVAE는 robust representation learning capabilities(강력한 표현 학습 능력)과 phylogenetic tree topologies(계통수 구조)의 빠른 생성을 입증하였습니다. 이 모델은 기존의 autoregressive 모델보다 훨씬 빠른 학습 및 생성 속도를 가지며, 다양한 형태의 트리 구조를 효과적으로 구별할 수 있는 능력을 지니고 있습니다. PhyloVAE는 phylogenetic tree topologies의 표현 학습을 위한 최초의 심층 모델 프레임워크로 자리 잡고 있습니다.



### Singing Voice Conversion with Accompaniment Using Self-Supervised Representation-Based Melody Features (https://arxiv.org/abs/2502.04722)
Comments:
          Accepted by ICASSP2025

- **What's New**: 본 논문에서는 배경 음악(BGM)이 포함된 오디오에서 멜로디를 보다 정확하게 추출하기 위해 새로운 자가 지도 학습(self-supervised learning, SSL) 기반의 멜로디 추출 방법을 제안합니다. 이는 기존의 음성 변환(SVC) 모델의 성능을 크게 개선할 수 있는 가능성을 보여줍니다. 실험 결과, 제안된 모델은 잡음 있는 환경에서도 멜로디 정확성과 자연성이 향상된 것을 입증했습니다.

- **Technical Details**: 제안된 SVC 모델은 ASR(aAutomatic Speech Recognition) 모델과 멜로디 추출기, 인코더-디코더 구조, 여러 종류의 판별기, 그리고 HiFi-GAN 보코더(vocoder)로 구성됩니다. 각 음성 데이터에서 멜로디 특성과 내용 특성을 분리해 추출하고, 이러한 특성들을 FFT 블록을 통해 처리하여 Mel-spectrogram을 생성합니다. 이 과정에서 HuBERT와 WavLM 같은 두 가지 SSL 모델을 활용하여 우수한 멜로디 피처 추출을 달성합니다.

- **Performance Highlights**: 실험 결과, 제안된 SVC 모델은 기존 방법들에 비해 멜로디 정확도가 크게 향상되고, 주관적 및 객관적 평가에서 더 높은 유사성과 자연성을 보였습니다. 노이즈가 있는 환경에서도 SVC 성능이 향상되어, 다양한 음악적 배경에 대한 강건성을 입증했습니다. 이러한 결과는 실제 응용 및 사용자의 편의성을 크게 개선할 것으로 기대됩니다.



### Symbolic Regression of Data-Driven Reduced Order Model Closures for Under-Resolved, Convection-Dominated Flows (https://arxiv.org/abs/2502.04703)
- **What's New**: 본 연구에서는 데이터 기반의 ROM(Reduced Order Models) 폐쇄 모델을 제안하며, 심볼릭 회귀(symbolic regression) 접근 방식을 활용하여 기존 방법의 장점은 살리고 단점을 제거하고자 하였습니다. 이 새로운 SR-ROM 폐쇄 모델은 해석 가능하고 간결하며, 복잡한 동역학을 정확하게 모델링할 수 있는 능력을 가지고 있습니다. 이로 인해 새로운 SR-ROM 모델은 기존의 구조적 모델 또는 머신 러닝 기반 모델보다 더 높은 정확성과 안정성을 제공합니다.

- **Technical Details**: 연구에서는 두 가지 주요 데이터 기반 ROM 폐쇄 모델 접근 방식을 비교합니다. 첫 번째는 간단한 수학적 구조를 가지는 구조적 접근 방식인 반면, 두 번째는 신경망을 활용한 머신 러닝 기반 접근 방식입니다. 새로운 SR_ROM 폐쇄 모델은 기존 두 접근 방식의 장점을 통합하며, 모델 파라미터 수를 줄여 과적합(overfitting)을 방지하고 일반화(generalization) 능력을 향상시킵니다.

- **Performance Highlights**: 실험 결과, 제안된 SR-ROM 폐쇄 모델이 구조적 및 머신 러닝 기반 ROM 폐쇄 모델에 비해 더욱 정확하고 강건한 성능을 보임을 확인하였습니다. 이를 위해 원통 주위의 흐름, 그리고 상단 구동 구멍 흐름을 포함한 두 가지 테스트 문제를 사용하여 비교 분석하였습니다. 이러한 결과는 새로운 접근 방식이 실제 터뷸런트 흐름 문제에서 유용할 것임을 시사합니다.



### A Meta-learner for Heterogeneous Effects in Difference-in-Differences (https://arxiv.org/abs/2502.04699)
- **What's New**: 이 논문은 이질적인 치료 효과를 추정하기 위한 새로운 방법론을 제안합니다. 우리는 Difference-in-Differences (DiD) 프레임워크를 활용해 Conditional Average Treatment Effect on the Treated (CATT)를 추정하는 이중 강건(meta-learner) 접근법을 개발하였습니다. 이 방법은 보조 모델의 추정 오류에 강건하며, 다양한 변수 집합에 대한 유연한 추정이 가능합니다. 특히, 이 연구는 고차원 공변량 조건화에 대한 새로운 해석을 제공합니다.

- **Technical Details**: 우리는 패널 데이터에서 이질적인 치료 효과를 이해하기 위해 다변수 조건에서 CATT를 추정하는 새로운 이중 강건 추정 프레임워크를 제시합니다. Neyman orthogonality를 활용함으로써, 보조 모델의 추정 직후 발생할 수 있는 견고함을 보장합니다. 또한, 기계 학습을 통해 복잡한 작업을 학습하는 과정에서의 기법들을 결합하여, 자주 발생하는 실증적 문제들에 대한 해결책을 제공합니다. 우리의 방법은 이질적인 치료 효과와 조건부 함수적 접근법들을 통합하여 일반화된 프레임워크를 개발하였습니다.

- **Performance Highlights**: 제안한 메타-러너는 기존의 기본선들과 비교했을 때 성능이 우수함을 실험을 통해 증명하였습니다. 특히, 실제 사례 연구에서 최저임금 인상에 대한 청소년 취업의 효과를 분석한 결과, 카운티 인구가 치료 효과의 크기에 중요한 역할을 한다는 것을 확인했습니다. 이 연구는 기존 문헌에서 강조되지 않았던 이질성의 차원과 패턴을 자동으로 식별하는 시스템을 보여주었습니다. 또한, 비표본 검증 프로세스를 통해 확인된 이질성 패턴은 통계적으로 유의미한 결과를 나타냈습니다.
...
### Optimistic Algorithms for Adaptive Estimation of the Average Treatment Effec (https://arxiv.org/abs/2502.04673)
Comments:
          15 pages, 2 Figures

- **What's New**: 이 논문에서는 평균 치료 효과(ATE) 추정과 인과 추론의 발전을 위한 새로운 알고리즘, Optimistic Policy Tracking (OPTrack)을 제안합니다. 기존 연구들은 비대칭적 성능 보장에 한계를 가지고 있었으나, 이 연구는 AIPW 추정기를 기반으로 하여 ATE 추정의 비대칭적 관점을 탐구합니다. 결과적으로, 이 논문은 기존의 비효율적인 방법들에 비해 이론적 및 경험적으로 우수한 성능을 보여줍니다.

- **Technical Details**: 기존의 ATE 추정 방식은 주로 asymptotic (비대칭적) 환경을 고려했으나, 본 연구는 finite-sample (유한 표본) 관점을 취합니다. OPTrack 알고리즘은 적응 샘플링 프로시저를 활용하여 ATE 추정을 수행하며, 초기에는 더 과도하게 샘플링한 후 차차 감축함으로써 ATE의 더 나은 추정을 가능하게 합니다. 이 과정에서 알고리즘의 설계 원칙을 다루고, 이전의 연구가 간과했던 중요한 측면을 조명합니다.

- **Performance Highlights**: 본 논문에서 제안된 OPTrack 알고리즘은 작은 표본 환경에서 특히 개선된 성능을 보였으며, 기존 알고리즘들과 비교해 우수성을 입증되었습니다. 시뮬레이션 결과, 이론적 개선이 실제 결과로 이어져 랜덤화된 임상 시험 같은 응용에 중요한 기여를 할 수 있음을 확인했습니다. OPTrack은 기존 방법들과 비교할 때 이론적 성능과 경험적 성능 모두에서 실질적인 이점을 갖습니다.
...
### Machine-Learning Interatomic Potentials for Long-Range Systems (https://arxiv.org/abs/2502.04668)
Comments:
          7 pages, 4 figures

- **What's New**: 이번 논문에서는 기존의 머신 러닝 기반 간섭 포텐셜의 한계를 극복하기 위해 Sum-of-Gaussians Neural Network (SOG-Net)를 제안합니다. SOG-Net은 기계 학습(force field)에서 장거리 상호작용을 통합하는 경량화되고 다재다능한 프레임워크입니다. 이는 짧은 범위와 긴 범위의 상호작용을 연결하는 라텐트 변수 학습 네트워크(latent-variable learning network)를 사용하여 혁신적인 접근 방식을 제공합니다.

- **Technical Details**: SOG-Net은 효율적인 포리에 컨볼루션 층(Fourier convolution layer)을 통합하여 장거리 효과를 포함합니다. 여러 컨볼루션 층에서 가우스 곱의 합(sum-of-Gaussian multipliers)을 학습함으로써 다양한 장거리 감쇠 행동을 적응적으로 캡처할 수 있습니다. 또한, 비균일 빠른 푸리에 변환(non-uniform fast Fourier transforms)을 사용하여 훈련 및 시뮬레이션 과정에서 거의 선형(computational complexity) 성능을 유지할 수 있습니다.

- **Performance Highlights**: SOG-Net은 광범위한 장거리 시스템에서 효과적인 수행 결과를 보여줍니다. 이 새로운 프레임워크는 기존의 간섭 포텐셜이 놓치기 쉬운 장거리 상호작용을 모델링할 수 있는 가능성을 열어줍니다. 또한, 계산 비용을 최소화하면서도 고품질의 시뮬레이션을 가능하게 합니다.



### End-to-End Learning Framework for Solving Non-Markovian Optimal Contro (https://arxiv.org/abs/2502.04649)
- **What's New**: 이 논문은 기존의 정수 차수 미적분이 가져오는 한계를 극복하고 실세계 시스템에서 발생하는 장기 의존성과 메모리 효과를 포착하기 위한 새로운 접근 방식을 제시합니다. 특히, 유리 차수 시스템에 대해 최적 제어를 위한 선형 이차 조절기(Linear Quadratic Regulator, LQR)를 구현하고 이를 위한 데이터를 기반으로 한 딥러닝 프레임워크인 FOLOC을 개발했습니다. 이 연구는 FOLTI 시스템에 대한 새로운 시스템 식별 방법 및 강력한 제어 전략을 제안하여 실제 복잡한 문제 해결에 기여할 수 있는 가능성을 모색합니다.

- **Technical Details**: FOLTI 시스템은 유리 차수 미적분을 통해 고전적인 정수 차수 모델을 확장하여 비국소적인 의존성과 역사에 따른 동역학을 포착할 수 있도록 합니다. 본 연구에서는 FOLTI 시스템 매개변수 추정을 위한 새로운 접근법과 함께, LQR를 기반으로 한 최적 제어 정책을 도출하기 위해 최소자승 최적화 및 라그랑주 승수를 활용합니다. 이 프레임워크는 시스템 식별과 최적 제어를 결합하여 복잡한 동적인 시나리오에서도 강력하게 작동할 수 있도록 설계되었습니다.

- **Performance Highlights**: FOLOC은 기존의 가우시안 노이즈 가정을 배제하고 비가우시안 노이즈 및 실제 불확실성에서 견고한 성능을 보여줍니다. 실험 결과는 FOLOC이 복잡한 유리 차수 시스템의 동작을 정확히 근사할 수 있으며, 다양한 노이즈 분포 및 시스템 복잡성에 대한 확장 가능성이 있다는 것을 증명합니다. 이 연구는 실제 시간 제어 작업에서 Computational Efficiency를 확보하면서 관련 데이터에 기반한 최적 제어 문제의 해결을 가능하게 합니다.



### LATTEO: A Framework to Support Learning Asynchronously Tempered with Trusted Execution and Obfuscation (https://arxiv.org/abs/2502.04601)
- **What's New**: 이번 논문은 비동기 연합 학습(asynchronous federated learning, AsyncFL)의 개인 정보 보호 취약점을 보여줍니다. 이 연구는 독창적인 데이터 재구성 공격(data reconstruction attack)을 통해 비동기 설정의 그라디언트 업데이트가 민감한 클라이언트 데이터를 복구하는 데 어떻게 악용될 수 있는지를 설명합니다. 또한, 비동기 FL에서의 개인 정보 보호를 보장하기 위한 새로운 프레임워크인 LATTEO를 제안하며, 이 프레임워크는 Gradient Obfuscation 메커니즘과 Trusted Execution Environments(TEE)를 결합하고 있습니다.

- **Technical Details**: LATTEO 프레임워크는 비동기 연합 학습의 개인 정보 및 확장성 문제를 해결하기 위해 세 가지 구성 요소로 구성됩니다. 첫 번째는 지역 그라디언트 업데이트를 사적인 직교 분포(orthogonal distribution)와 얽어 개인 정보를 보호하는 그라디언트 난독화 기법입니다. 두 번째는 신뢰할 수 있는 집계 서비스가 네트워크 에지에서 신속하게 배포될 수 있도록 하는 안전한 집계 서비스 메쉬입니다. 마지막으로, LATTEO는 클라이언트가 다양한 공급자의 집계 서비스를 암시적으로 검증할 수 있도록 하는 데이터 중심의 인증 메커니즘을 제공합니다.

- **Performance Highlights**: LATTEO의 평가 결과는 우리 그라디언트 난독화 메커니즘이 원래 비동기 FL(vanilla AsyncFL)과의 모델 유틸리티를 1% 이내로 유지하면서 데이터 재구성 위험을 크게 줄인다는 것을 보여줍니다. 구조적 유사성 지수(structural similarity index)를 85% 낮추고 재구성 오류를 400% 증가시키며, 또한 고부하 클라이언트 환경에서 평균 클라이언트 지연 시간을 기존 RA-TLS와 비교하여 최대 1500% 개선합니다.
...
- **What's New**: 이번 연구는 비대칭 표본법(importance sampling)의 고전적인 방법을 개선하기 위한 새로운 접근 방식을 제안합니다. 특히, 번역 역 확산 샘플러(reverse diffusion samplers)를 기반으로 한 새로운 정규화 상수 추정 알고리즘을 개발하여 기존 방법의 한계를 극복하고자 하였습니다. 또한, 비대칭적인 조건을 완화하면서 비대칭 추정기(oracle complexity)를 구체적으로 분석하여 이론적 근거를 제시하고 있습니다.

- **Technical Details**: 연구에서는 정규화 상수 Z를 추정하기 위한 비대칭 정량적 복잡성(oracle complexity) \( \widetilde{O}\left(\frac{d\beta^2\mathcal{A}^2}{\varepsilon^4}\right) \)을 도출하였습니다. 이때, \( \beta \)는 함수 V의 매끄러움(smoothness)을 나타내고, \( \mathcal{A} \)는 목표 분포(\( \pi \))와 다루기 쉬운 기준 분포(reference distribution) 간의 확률 측정의 행동(action)을 나타냅니다. Girsanov 정리와 최적 운송(optimal transport)을 활용하여 대칭 또는 비대칭적 가정 없이도 분포를 분석할 수 있습니다.

- **Performance Highlights**: 이 연구는 비대칭 표본법과 정열적 중요 샘플링(annealed importance sampling, AIS)의 성능에 대한 비대칭적 분석을 통해 이론적인 매우 고급의 정량적 안전 경계(quantitative complexity guarantees)를 제공합니다. 특히, 비대칭 조건을 만족하지 않는 다양한 목표 분포에 대한 복잡성을 이해하는 데 중요한 진전을 이루었습니다. 이는 정규화 상수 추정 문제의 해결에 있어 기존의 방법들보다 더 신뢰할 수 있는 근거를 제공하게 됩니다.
...
### Sparsity-Based Interpolation of External, Internal and Swap Regr (https://arxiv.org/abs/2502.04543)
Comments:
          Equal contribution, alphabetical order

- **What's New**: 이번 논문은 온라인 학습에서의 전문가 문제(expert problem)를 다루며, 성능 지표(performance metrics)의 보간(interpolation)을 $	heta$-regret 최소화(minimization)를 통해 연구합니다. 이 방법은 알고리즘의 성능을 특정 행동 수정 규칙($	heta$)에 대한 후회를 통해 측정합니다. 연구진은 $d$명의 전문가와 총 $T	ext{(T	ext{ is much larger than } d)}$라운드에서 작동하는 단일 알고리즘을 제시하여, 기존 결과를 개선하는 새로운 경계를 확립하였습니다.

- **Technical Details**: 논문에서는 $	ilde Oigg(	ext{min}iggigg{	ext{sqrt}(d-d_{	heta}^{	ext{uniform}} + 1), 	ext{sqrt}(d-d_{	heta}^{	ext{self}})iggigg{)}	ext{sqrt}(T)igg)$의 인스턴스 적응형 $	heta$-regret 경계(instance-adaptive $	heta$-regret bound)를 달성합니다. 여기서 $d_{	heta}^{	ext{uniform}}$은 $	heta$에 의해 동일하게 수정된 전문가의 최대 수이며, $d_{	heta}^{	ext{self}}$는 $	heta$이 스스로 수정한 전문가의 수를 나타냅니다. 이러한 계산을 통해 다양한 경우에 대한 최적 결과를 도출하면서, 고전적인 확률 행렬에서 $	heta$-regret 최소화(proportional judgment minimization)로의 변환을 기반으로 합니다.

- **Performance Highlights**: 이 연구는 기존의 최적 외부 후회 경계($O(	ext{sqrt}(T	ext{ log } d))$), 내부 후회 경계($	ilde O(	ext{sqrt}(T))$), 최악의 경우 스왑 후회 경계($	ilde O(	ext{sqrt}(dT))$)를 복원하는 성과를 거두었습니다. 또한, 제안된 알고리즘은 $	heta$ 외부 후회보다 더 단순한 설정에 해당하는 최적의 분위수 후회 경계(optimal quantile regret bound)도 달성합니다. 이 결과를 통해 온라인 선형 회귀(online linear regression)에 필요한 희소성을 활용하여 학습 과정을 개선시켰습니다.
...
### GenVC: Self-Supervised Zero-Shot Voice Conversion (https://arxiv.org/abs/2502.04519)
- **What's New**: 이 논문에서는 GenVC라는 새로운 zero-shot voice conversion 모델을 제안합니다. GenVC는 외부 감독 모델에 의존하지 않고도 화자 특성과 언어적 내용을 분리하는 것을 가능하게 합니다. 이 모델은 self-supervised 방식으로 학습되어, 다양한 대규모 비라벨 데이터셋에서 효율적인 훈련이 가능합니다.

- **Technical Details**: GenVC는 오디오를 이산적 토큰으로 변환하는 모듈과 생성적 Transformer 기반의 언어 모델을 통합합니다. 이 아키텍처는 각 단계에서 손실을 최소화하면서 진행되며, 토큰 생성, 생성적 모델링 및 고해상도 오디오 복원을 포함합니다. 이 모델은 화자의 스타일을 유지하며 언어적 내용을 변형할 수 있어, 저음 질감 및 언어 내용 간의 변환이 가능합니다.

- **Performance Highlights**: 실험 결과, GenVC는 최첨단 화자 유사성을 달성하면서도 자연스러움을 유지합니다. 이 모델의 자기회귀 생성 방식은 변환된 음성이 원본 발화의 시간적 구조에서 벗어나도록 하여, 개인정보 보호에 매우 효과적입니다. GenVC는 원본 발화의 억양과 화자 특성을 최소화하며, 이로 인해 개인 정보 보호의 효율성이 향상됩니다.
...
### The ML Supply Chain in the Era of Software 2.0: Lessons Learned from Hugging Fac (https://arxiv.org/abs/2502.04484)
- **What's New**: 이번 논문은 머신 러닝(Machine Learning) 시스템의 공급망(supply chain) 분석을 통해 Hugging Face 플랫폼에서 모델과 데이터셋 간의 관계를 평가합니다. 연구자들은 760,460개의 모델과 175,000개의 데이터셋을 분석하여 문서화 현황과 라이선스 문제를 조사하며, 이를 통해 컴플라이언스(compliance)와 보안(security) 관점에서의 문제들도 강조합니다. 이 연구는 ML 생태계에서의 공급망 관리를 강화하기 위한 다양한 제안 사항을 포함하고 있습니다.

- **Technical Details**: ML 공급망은 유연하고 복잡한 구조로, 다양한 단계와 구성 요소가 포함됩니다. 모델의 생성, 데이터로의 전환, 훈련 데이터셋의 컴파일, ML 모델의 설계 및 훈련과 같은 여러 단계를 포함합니다. 또한, fine-tuning이나 quantization과 같은 모델의 재사용이나 적응 또한 새로운 복잡성을 초래합니다. 일반적인 소프트웨어의 공급망과는 달리, ML 공급망은 모델 간의 의존성 및 훈련 데이터셋과의 관계가 뒤얽혀 있어 더욱 복잡합니다.

- **Performance Highlights**: Hugging Face 플랫폼에서의 모델 문서화 상태를 평가하고, 예시를 통해 발생하는 문제점과 개선 사항을 제안합니다. 본 연구는 또한 현재의 라이선스 환경과 관련된 다양한 문제들을 분석하며, 이를 바탕으로 ML 모델과 데이터셋 간의 상호작용을 파악합니다. 연구 결과는 ML 모델과 데이터셋의 효과적인 관리 및 자동화된 불일치 검토(validation)가 필요하다는 인사이트를 제공합니다.
...
### Identifying Flaky Tests in Quantum Code: A Machine Learning Approach (https://arxiv.org/abs/2502.04471)
Comments:
          8 pages, 1 figure, accepted by Q-SANER 2025

- **What's New**: 이 연구에서는 양자 소프트웨어의 불안정성을 자동으로 감지하는 새로운 머신 러닝 플랫폼을 제시합니다. 특히, 양자 시스템의 고유한 특징인 원주율성(indeterminacy)로 인해 발생하는 flaky 테스트 문제를 다루고 있습니다. 다양한 머신 러닝 모델을 활용해, 기존의 연구 논문에서 다루어지지 않았던 양자 flaky 테스트의 개념을 증명하고 그 결과를 개선하려는 방안을 탐구합니다.

- **Technical Details**: 제안된 방법은 eXtreme Gradient Boosting (XGB), Decision Tree (DT), Random Forest (RF), K-Nearest Neighbors (KNN), Support Vector Machine (SVM) 등 여러 머신 러닝 모델을 학습하고 평가하여 flaky 테스트를 감지합니다. 데이터는 여러 양자 소프트웨어 저장소에서 수집된 flaky 테스트와 non-flaky 테스트를 포함하며, 이는 양자 테스트의 신뢰성과 효율성을 향상시키는 데 기여하고자 합니다. 비기술적 요소인 환경 설정으로 인한 노이즈는 제외하여 최적의 데이터셋을 구성합니다.

- **Performance Highlights**: 양자 flaky 테스트 탐지에서 XGB와 DT 모델이 F1 점수 및 Matthews Correlation Coefficient에서 우수한 성과를 보였습니다. 연구 결과는 46개의 독특한 flaky 테스트가 확인되었으며, 이들 각각은 코드 변경 없이도 불안정한 결과를 가져옵니다. 향후 연구는 데이터셋의 확장과 다양한 유형의 flaky 테스트 감지를 위한 전략 개발에 중점을 둘 계획입니다.
...
### Analysis of Diffusion Models for Manifold Data (https://arxiv.org/abs/2502.04339)
- **What's New**: 이번 연구에서는 generative diffusion models의 시간 반전 동역학(time reversed dynamics)을 분석합니다. 이 모델들은 많은 차원(dimension)과 기하급수적으로 많은 샘플(sample)을 사용하는 경우, 서로 다른 동적 모드(dynamic regimes) 사이의 전이를 경험하는 것으로 알려져 있습니다. 특히, 저차원 가우시안 혼합(mixture of lower dimensional Gaussians)이 고차원 공간에 임베딩된 경우를 분석하였습니다.

- **Technical Details**: 우리는 분석 가능한 매니폴드 모델(analytically tractable manifold model)을 이용하여 전이를 계산하였습니다. 매니폴드 대 주변 공간 차원 비율(manifold-to-ambient space dimensions)과 데이터 모델(data model)의 다른 특성을 기반으로 speciation과 collapse 전이 시간을 산출했습니다. 이 과정에서 Generalized Linear Models의 상호 정보(mutual information) 또는 자유 에너지(free energy)의 정확한 공식을 중요한 도구로 사용하였습니다.

- **Performance Highlights**: 이 연구는 고차원 데이터에서 generative diffusion models의 동적 행동을 이해하는 데 기여합니다. 또한, 매니폴드와 주변 공간의 차원 비율에 따라 모델의 전이 시간을 계산함으로써 이론적 및 실험적 통찰을 제공합니다. 이 분석은 향후 다른 모델에 대한 적용 가능성을 열어줄 것으로 기대됩니다.



### Discrete GCBF Proximal Policy Optimization for Multi-agent Safe Optimal Contro (https://arxiv.org/abs/2502.03640)
Comments:
          31 pages, 15 figures, accepted by the thirteenth International Conference on Learning Representations (ICLR 2025)

- **What's New**: 본 논문에서는 다중 에이전트 시스템(Multi-Agent Systems, MAS)의 안전하고 고성능 정책을 동시에 학습하는 새로운 프레임워크인 DGPPO(Distributed Graph CBF Proximal Policy Optimization)를 제안합니다. 이 방법은 이산 그래프 CBF(Discrete Graph Control Barrier Functions)와 안전한 분산 정책을 결합하여 알려지지 않은 이산 시간 동역학 및 제한된 감지 환경을 고려합니다. 이러한 새로운 접근 방식은 고성능 정책 없이는 해결하기 어려운 문제를 다룹니다.

- **Technical Details**: DGPPO는 고성능 및 안전성을 확보하기 위해 이산 시간의 분산 제어 장벽 함수(Control Barrier Function, CBF)와 강화 학습(Reinforcement Learning, RL) 기술을 통합합니다. 특히, DGCBF(Discrete Graph Control Barrier Function)가 사용되며, 이는 변화하는 이웃 환경과 입력 제약 조건을 고려하여 안전을 보장합니다. 기존 연구들과 달리, DGPPO는 알려진 동역학 모델이나 기준 성능 정책 없이도 작동할 수 있는 기능을 갖추고 있습니다.

- **Performance Highlights**: DGPPO는 3개의 서로 다른 시뮬레이션 엔진에서 수행된 다중 에이전트 작업들을 통해 효과성이 검증되었습니다. 실험 결과, DGPPO는 기존의 방법들과 비교하여 높은 작업 성능과 안전률을 달성하며, 모든 환경에서 동일한 하이퍼파라미터 설정으로도 성능 저하 없이 안정적으로 작동하는 것으로 나타났습니다. 특히, 거의 100%의 안전률을 유지하면서도 저비용 솔루션을 제공했습니다.



### High-Dimensional Bayesian Optimization Using Both Random and Supervised Embeddings (https://arxiv.org/abs/2502.00854)
- **What's New**: 이 논문은 고차원 최적화 문제를 효과적으로 해결하기 위해 고안된 Efficient Global Optimization coupled with Random and Supervised Embedding (EGORSE)라는 새로운 베이지안 최적화 방법을 소개합니다. 기존의 베이지안 최적화(BO) 기법이 낮은 차원 최적화 문제에 주로 사용되어 온 한계를 극복하기 위해, 작은 차원의 선형 임베딩 서브스페이스를 통합하여 최적화를 수행합니다. 이 방법은 무작위 및 감독된 선형 임베딩을 조합하여 적응형 방식으로 진행됩니다.

- **Technical Details**: EGORSE는 고차원 베이지안 최적화(HDBO)의 일종으로, Gaussian Process (GP)를 사용하여 서브모델을 구축하고 디자인 공간 내의 최적 해를 추정합니다. EGORSE의 중요한 기능 중 하나는 선형 임베딩을 통해 최적화 문제를 단순화하고, 이를 통해 계산 효율성을 증가시키는 점입니다. 또한, 이 방법은 비선형 최적화 문제에서도 효과적일 수 있는 가능성을 보여줍니다.

- **Performance Highlights**: EGORSE는 10개에서 600개의 설계 변수를 가진 여러 학술적 예제에 대해 기존의 최고 성능 방법들과 비교되었습니다. 결과적으로 EGORSE는 CPU 시간과 고비용 블랙 박스 시뮬레이션 호출 횟수를 최소화하면서도 높은 차원의 최적화 문제를 해결할 수 있는 높은 잠재력을 보였습니다. 이러한 성과는 특히 MDAO(Multidisciplinary Design Analysis and Optimization) 문제에 적합한 해결책을 제공할 수 있음을 시사합니다.



### Real-Time Privacy Risk Measurement with Privacy Tokens for Gradient Leakag (https://arxiv.org/abs/2502.02913)
Comments:
...
- **What's New**: 이번 논문은 깊은 학습 모델이 개인정보가 민감한 분야에 배포됨에 따라 발생하는 개인정보 보호 문제를 해결하기 위한 새로운 접근 방식을 제안합니다. 기존의 방법들이 훈련 후 공격 시뮬레이션에 의존하는 반면, 제안된 'privacy tokens'는 훈련 중 생성된 그래디언트로부터 직접 유도되어 개인정보 유출을 실시간으로 측정할 수 있습니다. 또한 Mutual Information (MI)를 활용해 훈련 데이터와 그래디언트 간의 관계를 정량화하여 개인정보 위험을 보다 명확하게 평가할 수 있는 방법을 제공합니다.

- **Technical Details**: 논문에서는 'privacy tokens'의 개념을 도입하고, 이를 통해 훈련 중 데이터와 그래디언트 간의 상관관계를 정량화하는 새로운 프레임워크를 제안합니다. 이 프레임워크는 Mutual Information Neural Estimator (MINE)를 활용하여 훈련 데이터와 그래디언트 사이의 상호 정보를 근사하고, 고차원 데이터에 대한 확장 가능성을 확보합니다. 이러한 방법은 훈련 과정에서 지속적으로 실시간 개인정보 모니터링을 가능하게 하여 프라이버시 관리를 사전에 수행할 수 있도록 합니다.

- **Performance Highlights**: 연구 결과는 privacy tokens와 Mutual Information이 개인정보 위험을 파악하고 정량화하는 데 효과적임을 입증했습니다. 다양한 실험을 통해 모델 아키텍처, 데이터 세트 특성, 훈련 구성 등이 개인정보 위험에 미치는 영향을 분석하였으며, 이를 통해 깊은 학습 시스템에서의 자료 보호 전략을 강화할 수 있는 귀중한 통찰을 제공합니다. 이러한 접근 방식은 민감한 애플리케이션에 깊은 학습 모델을 더 안전하게 배치할 수 있도록 기여할 것으로 기대됩니다.

...
- **What's New**: 이 논문은 이산(discrete) 공간에서의 확산 모델(diffusion models)의 샘플링 효율성(sampling efficiency)을 연구하고 있으며, 특히 연속 시간 마르코프 체인(CTMC) 형식에 기반한 $	au$-leaping 방식의 샘플러(sampler)에 주목하고 있습니다. 이 논문은 Kullback-Leibler (KL) 발산에서 $	au$-leaping 알고리즘이 목표 분포의 차원에 따라 선형 의존성을 제거하는 방법을 제시합니다. 또한, 구조화된 데이터에 대해서는 효율성을 확보할 수 있는 새로운 샘플러를 소개합니다.

- **Technical Details**: 우리는 uniform 이산 확산에 대해 $	au$-leaping 알고리즘이 $O(d/	ilde{	au})$의 반복 복잡도를 가짐을 증명합니다. 이때 $d$는 목표 분포의 환경 차원(ambient dimension)이며, 이 연구는 vocab size $S$에 대한 선형 의존성을 없앴습니다. 또한 마스킹 이산 확산의 경우, 샘플러의 수렴 속도가 효과적인 총 상관(effective total correlation)에 의해 지배됨을 보여주며, 이는 적절한 구조적 데이터에서 서브선형(sublinear) 또는 상수값을 가질 수 있습니다.

- **Performance Highlights**: 본 논문에서 제안된 샘플러는 추가적인 지식 없이도 저차원 구조에 적응할 수 있으며, 다양한 실제 사례에 대해 썩 좋은 수렴 속도를 보입니다. 이러한 사례에는 히든 마르코프 모델(hidden Markov models), 이미지 데이터(image data), 랜덤 그래프(random graphs)가 포함됩니다. 이 연구는 점수 추정(score estimator)의 경계성이나 부드러움에 대한 가정을 요구하지 않으며, 점수 엔트로피 손실(score entropy loss)에 대한 제어만을 필요로 합니다.



### PDE foundation models are skillful AI weather emulators for the Martian atmospher (https://arxiv.org/abs/2602.15004)
- **What's New**: 이 연구는 다양한 부분 미분 방정식(Partial Differential Equations, PDE)에 대한 수치 솔루션으로 사전 훈련된 AI 기초 모델을 활용하여, 화성 대기의 예측을 위한 기상 모의 모델(weather emulator)을 정교하게 조정할 수 있음을 보여줍니다. Poseidon PDE 기초 모델을 기반으로 2차원에서 3차원으로 확장하는 새로운 방법을 개발하였고, 희소한 초기 조건에서도 모델의 성능을 조사했습니다. 이를 통해 AI 모델이 실제 복잡한 문제를 해결하는 데 사용할 수 있는 가능성을 입증하였습니다.

- **Technical Details**: 연구에서는 약 34GB의 훈련 데이터를 사용하여 화성의 4개 년도의 대기 데이터를 기반으로 모델을 훈련하였습니다. 평균 13 GPU 시간의 계산 예산을 활용하였으며, 34.4%의 성능 향상을 보이는 결과를 얻었습니다. 또한 2D 데이터를 기반으로 사전 훈련된 모델을 3D 공간으로 확장하는 기법을 도입하였고, 희소 입력 데이터에서도 이 결과가 유효하다는 점을 보여주었습니다.

- **Performance Highlights**: PDE-FMs (PDE Foundation Models)의 조합이 실제 데이터가 부족하거나 적절한 계산 예산이 없는 실세계 문제에 대한 해결책이 될 수 있음을 입증하였습니다. 이 연구는 기상 물리학 분야에도 PDE 기초 모델 접근 방식이 적합함을 보여주는 첫 사례로, 기후 예측 및 대기 역학의 다양한 하위 문제들에 대해 활용 가능성을 제시하였습니다.



### Boundary Point Jailbreaking of Black-Box LLMs (https://arxiv.org/abs/2602.15001)
- **What's New**: 이번 논문에서는 Boundary Point Jailbreaking (BPJ)이라는 새로운 자동화된 jailbreak 공격 방법을 소개합니다. 기존의 공격 방법들은 화이트/그레이 박스 가정을 기반으로 했지만, BPJ는 완전한 블랙박스 시스템으로 단 하나의 정보(클래시파이어가 상호작용을 플래그하는지 여부)만을 사용하여 공격합니다. 이 방법은 산업에서 배포된 강력한 방어 수단을 회피합니다.

- **Technical Details**: BPJ는 공격을 최적화하는 데 있어 주요 난제를 해결하며, 공격 수정의 효과를 평가하는 새로운 접근법을 취합니다. 특정 해로운 문자열에 대한 공격을 직접 학습하기보다는, 이를 중간 공격 대상들의 커리큘럼으로 변환하고 공격 강도의 작은 변화를 가장 잘 감지할 수 있는 평가 포인트를 능동적으로 선택합니다. 이와 같은 방식을 통해 BPJ는 비인간 공격 시드에 의존하지 않고도 GPT-5의 입력 클래스파이어에 대한 글로벌 jailbreak을 개발하는 데 성공했습니다.

- **Performance Highlights**: BPJ는 개별 상호작용에서 방어하기 어려운 성격을 가지지만, 최적화 과정에서 많은 플래그를 발생시킵니다. 이는 효과적인 방어를 위해 단일 상호작용 방법과 배치 수준 모니터링을 보완해야 한다는 점을 시사합니다. BPJ의 소개는 자동화된 공격 알고리즘의 새로운 가능성을 여는 중요한 진전을 나타내며, 앞으로의 연구에 매우 중요한 영향을 미칠 것입니다.
...
- **What's New**: 최근 발표된 COrAL 논문은 다중 모달 (multimodal) 학습의 효율성을 높이기 위한 새로운 프레임워크를 소개합니다. COrAL은 중복 (redundant), 고유 (unique), 그리고 시너지 (synergistic) 정보를 명확하게 모델링하면서 이들을 동시에 보존하는 구조를 가지고 있습니다. 이 프레임워크는 두 경로 인코딩 (dual-path encoding) 아키텍처와 직교성 제약 (orthogonality constraints)을 적용하여 정보의 세분화를 극대화하고, 각 모달의 해석을 명확히 하여 성능 향상을 추구합니다.

- **Technical Details**: COrAL의 독창성은 비대칭 마스킹 (asymmetric masking) 기법에 있습니다. 이 기법은 특정 모달에 대해 보완적인 뷰 전용 패턴을 도입하여 모델이 중복 정보에 의존하지 않고 교차 모달 의존성을 추론하도록 유도합니다. 이를 통해 서로 다른 모달 간의 상호작용을 효과적으로 모델링할 수 있으며, 효과적인 정보 분리를 통해 심층 학습의 주체가 될 수 있습니다.

- **Performance Highlights**: 실험 결과 COrAL은 최근 다중 모달 성능 기준을 균형 있게 유지하면서도 중복 및 고유 성분을 잘 회복하는 모습을 보였습니다. 여러 synthetic 벤치마크와 MultiBench 데이터셋에서 일관되게 성능을 발휘하며, 반복 실행 간 성능의 분산이 낮아 안정성을 보장합니다. 이러한 결과는 COrAL이 오히려 보다 안정적이고 신뢰할 수 있으며 포괄적인 임베딩을 제공하는 데 기여할 수 있음을 시사합니다.



### MacroGuide: Topological Guidance for Macrocycle Generation (https://arxiv.org/abs/2602.14977)
- **What's New**: 이 연구에서는 MacroGuide라는 새로운 방법을 소개합니다. MacroGuide는 분자 생성 모델을 대상으로 하여 매크로사이클(행렬 구조를 가진 분자)의 생성을 안내하는 기법입니다. 기존의 깊은 생성 모델에서는 매크로사이클 생성에 대한 성과가 미비하였으나, MacroGuide를 통해 생성률을 1%에서 99%로 향상시킬 수 있었습니다.

- **Technical Details**: Diffusion 모델은 데이터 생성 프로세스를 풀어내는 접근 방식으로, 원래의 샘플에 점진적으로 노이즈를 가하여 최종적으로 정규 분포의 잡음을 생성합니다. MacroGuide는 Persistent Homology를 활용하여 매크로사이클 생성을 위한 topological guidance를 제공합니다. 이 방식은 훈련이 필요 없으며, 경량화되어 다양한 diffusion 기반 모델에 적용 가능하다는 장점이 있습니다.

- **Performance Highlights**: 매크로사이클 생성을 위한 MacroGuide의 적용 결과, 실험적으로 99%의 높은 생성률을 달성했으며, 화학적 유효성, 다양성, PoseBusters 체크와 같은 주요 품질 지표에서도 최신 기술을 초과하거나 일치하는 성과를 보였습니다. 이러한 결과는 매크로사이클을 생성하는 것에 있어 깊은 생성 모델링의 중요한 단계를 의미합니다.



### Use What You Know: Causal Foundation Models with Partial Graphs (https://arxiv.org/abs/2602.14972)
- **What's New**: Causal Foundation Models (CFMs)의 최근 개발은 인과 추론과 발견을 하나의 단계로 통합하는 더 통합된 접근 방식을 제공하고 있습니다. 그러나, 현재 CFMs는 도메인 지식을 통합하는 것을 허용하지 않아 예측의 정확도가 떨어질 수 있습니다. 본 논문에서는 인과 정보에 CFMs를 조건화할 수 있는 방법을 소개하여 이 문제를 해결합니다.

- **Technical Details**: 본 논문에서는 세 가지 시나리오에 대해 CFMs를 조건화하는 기술을 연구합니다. (a) 모든 인과 변수가 알려진 경우, (b) 일부 변수가 부분적으로 알려진 경우, (c) 전혀 인과 관계가 없는 경우입니다. 이러한 모든 경우를 단일 모델에서 표현할 수 있도록 하고, 학습 가능한 주의(attention) 편향을 통해 부분 인과 정보를 효과적으로 활용하는 방법을 제시합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법이 일반 목적의 CFM이 특정 인과 구조에 대해 훈련된 전문 모델의 성능과 일치하는 데 기여함을 보여주었습니다. 부분적으로 알려진 인과 정보를 조건화하는 것이 예측의 정확도를 상당히 향상시킨다는 결과가 나타났습니다. 이에 따라 CFMs가 데이터 기반 방식으로 인과 질의를 효과적으로 처리할 수 있는 방향으로 나아가고 있습니다.
...
- **What's New**: 이번 연구에서는 여러 목표를 동시에 만족하는 예측기를 학습하는 문제를 다룹니다. 특히, 데이터 분포가 시간이 지남에 따라 변화하는 온라인 환경에서의 다중 목표 학습(multi-objective learning) 방법론을 제안합니다. 기존 알고리즘이 데이터 분포 변화에 적응하지 못하는 한계를 극복하기 위해, 적응형 온라인 알고리즘을 통합하여 로컬 적응성을 향상시킨 새로운 접근 방식을 소개합니다.

- **Technical Details**: 제안된 알고리즘은 여러 목표를 만족하는 예측기를 생성하며, 이를 통해 모든 로컬 시간 간격에 걸쳐 목표를 거의 만족시킬 수 있습니다. 특히, Fixed Share 방법을 사용하여 고정된 타겟 너비를 가진 모든 간격에 대해 적응성을 보장하는 메타 알고리즘을 제공합니다. 다양한 적응형 방법의 성능을 평가하기 위한 광범위한 실험도 진행하였으며, 전력 수요 예측 및 재범 예측에서 기존의 방법보다 탁월한 성능을 나타냈습니다.

- **Performance Highlights**: 실험 결과, 제안한 로컬 적응형 알고리즘이 기존 방법에 비해 일관되게 뛰어난 성능을 보였습니다. 특히, 온도 그룹에 따라 다르게 예측할 수 있는 다중 정확성(multiaccuracy) 문제를 해결하는 데 있어 효율적이었습니다. 전반적으로, 제안한 알고리즘은 다양한 상황에서 고른 예측 정확도를 유지하며, 기존의 비적응형 알고리즘에 비해 월등히 낮은 오류율을 기록했습니다.



### Variance-Reduced $(\varepsilon,δ)-$Unlearning using Forget Set Gradients (https://arxiv.org/abs/2602.14938)
- **What's New**: 이 논문에서는 머신 언러닝의 새로운 접근 방식을 제안합니다. 저자들은 Variance-Reduced Unlearning (VRU) 알고리즘을 소개하며, 이는 훈련된 모델에서 데이터 세트를 제거할 때 더 나은 효과를 보장하는 $(
epsilon,	ext{ }	ext{ }
epsilon,	ext{ }	ext{ }
epsilon,	ext{ }	ext{ }
epsilon,	ext{ }	ext{ }
epsilon,	ext{ }	ext{ }
epsilon,	ext{ }	ext{ }(	ext{1st-order})$ 문맥에선 최초로 forget set gradients를 업데이트 규칙에 포함합니다.

- **Technical Details**: VRU는 확률적 목적함수를 탐색하기 위해 gradient ascent를 사용하며, 기존의 방법들과 차별화되는 점은 forget set이 포함된다는 것입니다. 이 알고리즘은 충분히 구체화된 성능을 보이며, 기존 방법들보다 높은 수렴 속도를 제공합니다. 또한, VRU는 낮은 오류 영역에서, forget set을 무시하는 다른 첫 번째 순위 방법들에 비해 우수한 성능을 보임을 증명합니다.

- **Performance Highlights**: 저자는 VRU의 성능을 다양한 기준선과 비교하여 입증합니다. 이 알고리즘은 최신 인증된 언러닝 방법들보다 일관된 성과를 보이며, forget set을 활용하는 경험적 기준선보다도 우수한 성능을 자랑합니다. 따라서 VRU는 머신 언러닝에서 근본적인 데이터 삭제의 효과성을 높이는 데 기여할 것으로 보입니다.
...
### Coverage Guarantees for Pseudo-Calibrated Conformal Prediction under Distribution Shif (https://arxiv.org/abs/2602.14913)
Comments:
          Under review. 6 pages, 2 figures, 1 table

- **What's New**: 이 논문은 변환 예측(conformal prediction, CP)에서 데이터 분포의 변화에 따른 성능 저하를 해결하기 위해, 제한된 레이블 조건부 공변량 이동 모델 아래에서 의사 교정(pseudo-calibration)의 활용을 분석합니다. 기존의 CP 방법들은 소스(교정) 도메인과 타겟(테스트) 도메인 간의 교환 가능성 가정에 의존하며, 이 가정이 데이터 분포에서 깨질 경우 보장된 마진 커버리지(marginal coverage)가 실패할 수 있습니다. 이에 따라, 소스 도메인의 손실과 이동의 Wasserstein 거리의 관점에서 타겟 커버리지를 하한으로 유도하는 방법을 제시합니다.

- **Technical Details**: 이 논문에서는 도메인 적응(domain adaptation) 이론을 활용하여 클래시파이어(Classifier)의 손실과 Wasserstein 이동 측정을 기반으로 타겟 도메인에서의 커버리지 보장을 유도합니다. 특히, 다중 클래스(multi-class) 설정에서 의사 교정 집합을 설계할 때, 경량 매개변수(slack parameter)를 이용하여 변환 경량 기준을 부풀리는 방법을 제안합니다. 마지막으로, 클래시파이어의 불확실성(classifier uncertainty) 수준에 따라 굳은 의사 라벨과 무작위 라벨 간의 보간을 수행하는 소스 조정 의사 교정 알고리즘(source-tuned pseudo-calibration algorithm)을 제안합니다.

- **Performance Highlights**: 수치 실험을 통해, 제안된 경계가 경험적 행동을 잘 추적하며, 소스 조정 방식이 데이터 분포 변화 하에서도 타겟 커버리지 저하 문제를 완화하는 것을 보여줍니다. 또한, 이 방식은 비트리비얼 예측 집합 크기를 유지하면서도 CP 방식의 보수성을 줄이는 데 기여합니다. 본 연구는 데이터 분포 변화에 따른 라벨이 없는 다중 클래스 CP의 분석을 위한 새로운 기초를 제공합니다.
...
- **What's New**: 이 연구에서는 대규모 심층 학습 모델의 압축에 대한 새로운 관점을 제시합니다. 저자들은 훈련된 모델의 파라미터가 무작위 초기화보다 알고리즘적 복잡성(algorithmic complexity)이 낮으며, 이러한 구조가 압축 기술을 통해 모델 압축에 활용될 수 있다고 가정합니다. 특히, $	extit{Mosaic-of-Motifs}$ (MoMos)라는 새로운 방법론을 도입하여 알고리즘적으로 더 간단한 모델 파라미터화를 제안합니다.

- **Technical Details**: MoMos는 파라미터를 블록으로 나누고, 각 블록이 재사용 가능한 모티프(motif) 집합에서 선택되도록 제약하는 방식입니다. 이러한 제한을 통해 모델 파라미터의 구조를 단순화시켜 알고리즘적 복잡성을 줄이는 방법론을 제시합니다. 저자들은 Kolmogorov 복잡성을 근사하여 모델의 학습된 가중치가 알고리즘적으로 규칙적 알고리즘적으로 더 압축 가능한 파라미터화로 변한다는 것을 실증적으로 입증합니다.

- **Performance Highlights**: 모델 압축은 지난 수년간 활발히 연구되어 왔으나, 이 연구는 학습된 파라미터의 알고리즘적 복잡성에 대한 보장을 제공하는 첫 번째 연구입니다. 저자들은 여러 실험을 통해 MoMos가 제한되지 않은 모델들과 유사한 성능을 유지하면서도 알고리즘적으로 간단한 모델을 생성할 수 있음을 보여줍니다. 이러한 결과는 모형의 메모리 및 컴퓨팅 효율성을 크게 향상시킬 수 있는 가능성을 시사합니다.
...
- **What's New**: 이 논문은 클러스터링 알고리즘의 외적 평가(extrinsic evaluation)의 필요성을 강조하며, 겹치는 클러스터(overlapping clusters) 및 아웃라이어(outliers)를 포함한 클러스터링을 비교할 수 있는 실용적인 유사도 측정 방식을 정의합니다. 기존의 비교 방법들이 이러한 특성을 고려하지 못하는 문제를 해결하여 더 나은 클러스터링 성능을 평가할 수 있도록 합니다. 실험을 통해 제안된 측정 방식이 일반적으로 발생하는 편향(bias) 영향에서 자유롭다는 것을 보여줍니다.

- **Technical Details**: 제안된 유사도 측정 방식은 클러스터(t) 사이의 비교를 위해 F∗ 점수(F∗ score)를 사용하며, 이 점수는 Jaccard 지수(Jaccard index)와 동등한 성질을 갖습니다. 이 방법은 검색의 비대칭성(asymmetric similarity)를 도입하여 클러스터 간의 최대 유사성을 평가하고, 클러스터가 '실제로 존재하는' 매칭을 찾는 능력을 판단하는 데 유용하게 사용될 수 있습니다. 이러한 접근법은 클러스터의 일관성을 유지하면서도 아웃라이어를 포함할 수 있는 데이터셋에서도 효과적으로 작동합니다.

- **Performance Highlights**: 실험 결과, 제안된 유사도 측정 방식은 기존 방법과 비교할 때 더 신뢰할 수 있는 평가 결과를 도출합니다. 기존의 Omega index나 oNMI와 같은 측정 방식들보다 안정적이고 직관적인 비교 결과를 제공합니다. 또한, 이 방법은 클러스터링의 효과적인 성능 분석을 가능하게 하여 데이터 분석 및 머신러닝 분야에서 활용될 수 있는 가능성을 보여줍니다.
...
- **What's New**: 이 논문은 전통적인 신경망(Neural Networks)이 훈련 데이터의 볼록 경계(convex hull) 밖에서 안정적으로 일반화하는 데 한계를 가지고 있음을 강조합니다. 저자들은 PDE(Partial Differential Equations) 문제에 대한 형식적으로 검증된 신경망 해결 방법을 구축함으로써 이러한 한계를 극복할 수 있음을 보여줍니다. 이를 통해 신뢰할 수 있는 근사값과 안정적인 추정치를 제공하고, 확장 가능한 경계에서의 오류를 제어할 수 있는 방법을 제시하였습니다.

- **Technical Details**: BEACONS(경계 오류, 대수적으로 조합 가능한 신경망 해결기)는 PDE 해의 근사를 위해 설계된 새로운 프레임워크로, 신경망 아키텍처의 구성 원리를 대수적으로 조합하여 심층 구조를 형성합니다. 이 접근 방식은 characteristic 메서드를 통해 PDE 해의 해석적 성질을 미리 예측하고, 신경망 근사의 오류 가치를 rigorously 관리하는 방법론을 제공합니다. 또한, 단일 은닉층을 가진 얕은 신경망 아키텍처로도 신뢰할 수 있는 경계의 오류를 증명할 수 있습니다.

- **Performance Highlights**: 저자들은 BEACONS 프레임워크가 1D와 2D에서 다양한 선형 및 비선형 PDE 문제에 효과적으로 적용될 수 있음을 시연하였습니다. 특히, 비정상적인 함수나 충격파와 같은 고도로 불연속적인 함수를 조합하여 L^inf 오류를 효과적으로 억제하는 새로운 접근 방식을 통해, 훈련 데이터 범위를 넘어서는 신뢰할 수 있는 해결책을 모색합니다. BEACONS는 기존의 PINN 접근 방식에 비해 다양한 이점들이 있는 것으로 논의됩니다.
...
### Interactionless Inverse Reinforcement Learning: A Data-Centric Framework for Durable Alignmen (https://arxiv.org/abs/2602.14844)
Comments:
          Accepted for the AAMAS 2026 Blue Sky Ideas track

- **What's New**: AI alignment의 중요성이 증가하고 있지만, 현재의 접근법들은 안전 목표와 에이전트의 정책을 얽히게 하는 구조적 결함이 있다. 연구진은 이 문제를 해결하기 위해 Interactionless Inverse Reinforcement Learning (IIRL)을 제안하여 정책 최적화와 보상 모델 학습을 분리하고, 이는 검사 가능하고 수정 가능한 보상 모델을 생성한다. 또한 Alignment Flywheel이라는 인프라를 도입하여 자동 감사와 개선을 통해 보상 모델을 강화하는 반복적인 주기를 구축한다.

- **Technical Details**: IIRL Paradigm은 AI alignment를 불안정한 예술에서 엄격한 공학 관행으로 변화시키는 기초적인 전환을 나타낸다. 기존의 Inverse Reinforcement Learning (IRL) 방식은 보상 발견과 정책 최적화를 결합하지만, IIRL은 이 프로세스를 구조적으로 분리한다. 이를 통해 에이전트의 특정 정책이나 아키텍처에 의존하지 않는, 내구성 있고 검사 가능하며 수정 가능한 보상 모델을 직접 유도하는 데이터 중심의 문제로 재구성한다.

- **Performance Highlights**: IIRL에 의해 생성된 보상 아티팩트는 안정성과 해석 가능성을 보장하며, Alignment Flywheel은 전문가 피드백을 활용하여 통제 가능성과 윤리성을 보장한다. 최종적으로, 이 구조는 FATE의 사회적 명령(공정성, 책임, 투명성, 윤리)을 지원하는 기술적 RICE 원칙을 충족시킨다. 이러한 아키텍처는 유연한 도구 상자를 제공하여 즉각적인 구현을 가능하게 하며, 영구적인 alignment를 정의하는 로드맵을 제시한다.
...
- **What's New**: 이 논문에서는 다원적 정보 소스를 사용하는 상황에서 효율적인 최적화를 위한 Multi-Source Causal Bayesian Optimization (MSCBO) 알고리즘을 제안합니다. MSCBO는 전통적인 Bayesian Optimization (BO)과 Causal Bayesian Optimization (CBO)의 장점을 통합하여 복잡한 고차원 문제에서 계산 효율성을 높이고 성과를 개선합니다. 기존 MSBO가 가정하는 독립적인 변수 관계의 한계를 극복하여 인과적 관계를 활용하는 새로운 접근법입니다.

- **Technical Details**: MSCBO는 다원적 정보 소스의 특성과 인과적 정보 간의 시너지를 창출하기 위해 고안되었습니다. 인과정보를 통해 개입(set of interventions)을 최적화하고 비용-효율성(cost-effectiveness)을 고려하여 적절한 소스를 선택할 수 있도록 설계되었습니다. 이를 통해 최적화 문제를 해결하는 과정에서 변수 간의 의존성을 효과적으로 다루고, 실험 및 시뮬레이션의 번거로움을 줄일 수 있습니다.

- **Performance Highlights**: MSCBO는 합성 데이터 및 실제 데이터셋을 대상으로 기존의 MSBO 알고리즘과 비교하여 성능을 평가했습니다. 그 결과, MSCBO는 소음(noise) 수준이 다양한 상황에서도 일관된 우수한 성능을 나타냈습니다. 이 연구는 인과적 최적화 기법의 통합이 다원적 문제에서 수렴 속도와 효율성을 현저히 개선할 수 있음을 보여줍니다.
...
- **What's New**: 이번 연구에서는 최적화 알고리즘의 안정성을 조사하며 비선형(Nonlinear) 항들의 영향을 명확히 연구합니다. 특히, 다변량(multivariate) 설정에서 경량 안정 동작의 정확한 기준을 도출했습니다. 기존의 선형화(linearization) 기법이 전반적인 비선형 행동을 올바르게 반영하지 못할 수 있음을 보여주고, 이는 새로운 인사이트를 제공합니다.

- **Technical Details**: 연구는 고차 미분(high-order derivatives)을 포함하여 GD(Gradient Descent)에서의 안정적 진동(stable oscillations)을 설명하는 조건을 제시합니다. 또한 SGD(Stochastic Gradient Descent)의 경우, 단일 배치(single batch) 불안정성에도 불구하고 비선형 동역학(nonlinear dynamics)이 기대값에서 발산할 수 있음을 입증합니다. 이는 선형 분석이 제안하는 평균적 효과와는 달리, 불안정하게 진동하는 단일 배치에 의해 안정성이 좌우될 수 있다는 것을 의미합니다.

- **Performance Highlights**: 연구 결과, 모든 배치가 선형적으로 안정성(linearly stable)을 가져도 SGD의 비선형 동역학은 기대값에서 안정적일 수 있다는 점을 증명했습니다. 이러한 발견은 안정성과 수렴(convergence) 특성의 이해를 더욱 깊게 할 뿐만 아니라, 최적화 알고리즘 설계에 유용한 지침을 제공할 수 있습니다.
...
- **What's New**: 이 논문은 조합 경매의 우승자 결정 문제(Winner Determination Problem, WDP)에 대한 새로운 접근 방식을 제안합니다. 기존의 방법들은 대부분 빠른 탐욕적 휴리스틱에서 어떤 인스턴스가 실패할지를 예측하는 데 한계가 있었습니다. 이 연구는 인스턴스의 하드니스(hardness)를 예측하여 적절한 알고리즘 선택을 가능하게 하며, 이로 인해 최적성 격차(optimality gap)를 줄일 수 있습니다.

- **Technical Details**: 저자들은 20차원의 구조적 특징 벡터를 설계하고, 이를 기반으로 한 경량화된 MLP 하드니스 분류기를 훈련시켰습니다. 이 분류기는 탐욕적인 최적성 격차를 평균 절대 오차(mean absolute error, MAE) 0.033으로 예측하였고, Pearson 상관계수 0.937 및 94.7%의 이진 분류 정확도를 기록했습니다. 어려운 인스턴스(탐욕적 방법이 실패할 것으로 예측되는 인스턴스)에 대해 이질적인 GNN 전문 모델을 배포하여 최적성 격차를 약 0%로 줄였습니다.

- **Performance Highlights**: 하이브리드 할당기(allocator)는 하드니스 분류기와 GNN, 탐욕적 솔버를 결합하여 혼합 분포에 대해 0.51%의 전체 격차를 달성하였습니다. CATS 기준에서의 평가 결과는 GNN이 Gurobi 같은 정확한 ILP 솔버를 능가하지 못함을 보여주었고, 이러한 결과는 더 비싼 솔버를 배치할 시점을 예측하는 것이 더 현실적임을 강조합니다. 논문에서는 코드와 데이터를 공개하여 재현성을 확보할 수 있도록 하였습니다.
...
- **What's New**: 본 논문에서는 미분 방정식을 해결하기 위해 파라미터 최소화(neural architecture)를 제안합니다. 이 접근방법은 Horner 인수로 인코딩된 다항식(hypothesis class)을 사용하여 적은 수의 학습 가능한 계수만으로 미분 방정식의 해를 제공합니다. 초기 조건은 낮은 차수의 다항식(fixed low-order polynomial) 자유도로 인해 정확히 삽입되며, 이에 따라 훈련은 단지 미분 방정식 레지듀얼(residual)을 일치시키는 데 집중됩니다.

- **Technical Details**: 제안된 모델은 Horner 형태의 다항식으로 구성된 신경망 아키텍처로, 이를 통해 미분 방정식의 연속적이고 미분 가능한 해를 표현합니다. 초기 조건은 모델 구조에 직접 내장되어 탈락 계수(penalty weights)의 민감도를 줄이고 훈련 신뢰성을 개선합니다. 우리는 또한 여러 개의 작은 Horner 모델을 서브 인터벌에서 훈련하여 조각별(spline-like) 연장을 도입하여 근사 오차(appoximation error)를 줄였습니다.

- **Performance Highlights**: Horner 네트워크는 수십 개의 파라미터로 해와 그 파생(derivatives)을 정확하게 일치시키며, 동일한 훈련 환경에서 작은 다층 퍼셉트론(MLP)과 사인 기반 모델을 초과하는 성능을 보여주었습니다. 이 연구는 자원 효율적인 과학 모델링을 위한 실용적인 정확도-파라미터(trade-off)를 입증하며, 적은 수의 학습 가능한 파라미터로도 높은 정확도(a high accuracy)를 달성한다는 점에서 주목할 만합니다.
...
- **What's New**: 이번 논문에서는 실용적 데이터 및 컴퓨터 제약 내에서 매개변수 효율적인 미세 조정 디자인 공간을 체계적으로 조사하고 D2-LoRA를 제안합니다. 이 방법은 한 작업당 5,000개의 훈련 샘플과 두 번의 에포크를 사용하여 8개 질문 응답 및 독서 이해 벤치마크에서 평균 76.4%의 정확도를 달성합니다. D2-LoRA는 추론 시 알지브라적 병합 가능성을 유지하여 거의 정확한 수치 동등성을 보장합니다.

- **Technical Details**: D2-LoRA는 서명된 저순위 잔여 업데이트와 가감 성분을 결합한 방법으로, 훈련 중 각 열을 원래 노름에 가깝게 유지하는 열 방향 투영을 포함합니다. 훈련 후에는 어댑터가 단일 가중치 행렬로 병합되어 추가적인 추론 지연이 발생하지 않습니다. 이 방식은 작은 데이터의 저순위 범위에서의 효과적인 적응을 위한 두 가지 핵심 요구 사항을 만족하며, 이를 통해 D2-LoRA는 LoRA보다 평균 정확도를 2.2%p 높여주고 있습니다.

- **Performance Highlights**: D2-LoRA는 8개의 QA/RC 벤치마크와 두 가지 생성 작업에서 엄격한 훈련 예산 내에서도 +2.2%p의 성능 향상을 보여주며, 36% 낮은 손실 변동성을 유지합니다. 또한 D2-LoRA는 모든 작업에서 DoRA와 동등하거나 우수한 성능을 보이며, 총 평가 처리량을 약 1.91배 회복합니다. 훈련 오버헤드는 19%로 DoRA와 유사하지만, 입력 시퀀스가 길어짐에 따라 감소합니다.



### Unbiased Approximate Vector-Jacobian Products for Efficient Backpropagation (https://arxiv.org/abs/2602.14701)
- **What's New**: 본 연구에서는 깊은 신경망(deep neural networks) 훈련의 계산 비용(computational cost) 및 메모리 비용(memory cost)을 줄이는 방법을 소개합니다. 우리가 제안하는 접근 방식은 역전파(backpropagation) 과정에서 정확한 벡터-야코비안 제품(vector-jacobian products)을 무작위화(randomized)된 편견 없는(unbiased) 근사값으로 대체하는 것입니다.

- **Technical Details**: 이론적 분석을 통해 목표 정밀도(target precision)를 달성하기 위해 필요한 에폭(epoch)의 수와 각 에폭에서의 비용 감소(cost reduction) 간의 트레이드오프(trade-off)를 도출합니다. 또한 희소성(sparsity constraint) 제약 하에서 최소 분산(minimal variance)의 바람직한 최적 특성을 갖는 특정 편견 없는 추정치를 확인합니다.

- **Performance Highlights**: 다층 퍼셉트론(multi-layer perceptrons), BagNet 및 Visual Transformers 아키텍처에 대한 심층 실험을 수행하여 우리의 이론적 결과를 검증했습니다. 이 실험들은 우리가 제안한 편견 없는 무작위 역전파 방법이 깊은 학습(deep learning)의 비용을 감소시킬 수 있는 가능성을 확인합니다.
...
- **What's New**: 이 논문에서는 대형 언어 모델(LLM)의 지시 선택(Instruction Selection)에 대한 명확한 프레임워크를 제시합니다. 기존 방법들이 다양한 설계 선택 및 기초선 부족으로 인해 단편화되고 불투명했던 현상을 해결하고자 합니다. 데이터 표현(data representation)과 선택 알고리즘(selection algorithm) 두 가지 핵심 요소를 분리하여 체계적으로 분석함으로써, LLM의 파인튜닝에서의 데이터 선택(data selection) 방법론에 대한 중요한 통찰을 제공합니다.

- **Technical Details**: 이 연구에서 제안된 프레임워크는 두 개의 핵심 요소로 구성되어 있습니다. 첫째, 데이터 표현은 후보 풀(candidates pool)과 쿼리 세트(query set)로부터 예제 간의 유사성(similarity)을 제공하여 성능을 예측하는 역할을 합니다. 둘째, 선택 알고리즘은 이러한 데이터 표현을 기반으로 후보 풀에서 예제를 선택하여 훈련 예산(training budget)을 최소화하는 방식입니다. 여기에 더해 여러 선택 알고리즘을 근사 거리 최소화(approximate distance minimization)로 통합하는 새로운 이론적 관점을 개발하였습니다.

- **Performance Highlights**: 실험 결과, 그레디언트 기반 데이터 표현(gradient-based representations)이 쿼리 세트와의 유사성이 특정 데이터셋 및 모델에서 성능을 예측하는 데 가장 효과적이라는 점이 확인되었습니다. 또한, 예산이 적을 때는 탐욕적 순환 선택(greedy round-robin)이 가장 좋은 성능을 보이며, 큰 예산에서는 최적 수송(optimal transport) 방법이 더 나은 성능을 낼 수 있음을 발견했습니다. 흥미롭게도 무작위로 샘플링된 하위 집합이 현재의 인기 있는 선택 방법보다 성능이 비슷하거나 더 뛰어난 경우가 많아, 현행 데이터 선택 관련 관행의 취약점을 드러내었습니다.
...
- **What's New**: 본 논문에서는 Fourier 공간(Fourier space)에서의 gradient enhancement의 확장을 통해 pseudo-differential enhanced physics-informed neural networks (PINNs)를 제안합니다. PINNs의 PDE 잔여(residual)를 더 높은 미분 차수로 취급하여 훈련을 향상시키는 기법이 새로운 방식으로 소개되었으며, 이는 Fourier 변환을 통해 수행됩니다. 이 방법론은 학습의 신뢰도를 높이는 데 도움을 주며, 특히 저 샘플 수에서 효과적입니다.

- **Technical Details**: 정확한 훈련을 위해 Fourier 공간에서의 미분을 활용합니다. 이는 적절한 감소 조건을 갖춘 함수에 대해 Fourier 파수(wavenumber)로 곱하는 과정을 통해 이루어집니다. 따라서, 우리의 접근법은 상대적으로 단순한 선형 미분 연산자를 사용하여 PINNs의 PDE 잔여와 상호작용하며, 이를 통해 신속하고 효율적인 계산이 가능합니다. 이러한 방식은 또한 분수 미분(fractional derivatives)에도 적합합니다.

- **Performance Highlights**: 우리의 방법은 처리 속도와 효율성이 높으며, 종종 기존의 PINN 대비 수치적 오차(numerical error)를 더 빠른 훈련 반복(iterations) 내에 달성합니다. 이로 인해, 저 콜로케이션 설정(low collocation settings)에서도 학습의 정체(plateaus)를 극복할 수 있는 잠재력이 있습니다. 추가적으로, Monte Carlo 기법을 통해 비유클리드(Non-Euclidean) 도메인에서도 높은 유용성을 보여줍니다.



### An Embarrassingly Simple Way to Optimize Orthogonal Matrices at Sca (https://arxiv.org/abs/2602.14656)
Comments:
          23 pages, 10 figures, in review

- **What's New**: 이 연구는 강인하고 확률적인 머신 러닝에서 일반적으로 사용되는 orthogonality constraints(직교성 제약 조건)의 최적화 문제를 해결합니다. 기존의 최적화 알고리즘들이 계산적으로 비싸고 수백 또는 수천 개의 제약 조건을 처리하는 데 한계가 있었으나, Landing 알고리즘이 이를 일부 해결했습니다. 하지만 이 알고리즘은 orthogonality를 일시적으로 완화해야 했습니다. 본 연구에서는 Landing의 아이디어를 개선하여 현대의 adaptive optimizers(적응형 최적화 기법)와 결합하였습니다.

- **Technical Details**: 제안된 알고리즘 POGO는 GPU 친화적이며 단지 5개의 matrix products(행렬 곱셈)로 구성되어 있습니다. 이 알고리즘은 실제로 항상 orthogonality를 유지합니다. POGO는 새로운 hyperparameters(하이퍼파라미터)의 필요성을 크게 줄이며, 효율적인 최적화를 제공하는 데 집중하고 있습니다. 또한, POGO는 최신의 adaptive optimizer와 비교할 때 실행 속도가 매우 빠릅니다.

- **Performance Highlights**: 여러 어려운 벤치마크에서 POGO는 최신 최적화 기법들을 크게 능가하는 성능을 보여줍니다. POGO는 수천 개의 orthogonal matrices(직교 행렬)를 몇 분 안에 최적화할 수 있으며, 상대적으로 다른 방법들은 몇 시간 이상 소요되는 경우가 많습니다. 따라서 POGO는 머신 러닝에서 orthogonality constraints를 대규모로 활용할 수 있는 이정표를 제시합니다. POGO의 PyTorch 구현은 공개적으로 제공됩니다.
...
Comments:
          To appear in ICLR 2026, code: this https URL

- **What's New**: 본 논문은 Concept Bottleneck Models (CBMs)의 해석 가능성을 향상시키기 위해 명시적인 Information Bottleneck regularizer를 도입하고, 이는 인간이 이해할 수 있는 개념 레이어를 통해 의사결정을 경유하도록 합니다. 이전의 CBMs는 정확도 저하와 개념 유출 문제로 인해 신뢰성을 잃는 경향이 있었으나, 본 연구에서는 이 새로운 정규화 기법이 그러한 문제를 해결할 수 있음을 보여줍니다.

- **Technical Details**: Information Bottleneck regularizer는 $I(X;C)$를 페널티를 부여하여 줄이는 동시에 $I(C;Y)$에서 과제 관련 정보를 보존하도록 설계되었습니다. 이는 최소한의 충분한 개념 표현(minimal-sufficient concept representations)을 유도합니다. 연구에서는 두 가지 실용 변형(variational objective 및 entropy-based surrogate)을 제안하며, 이러한 기법을 표준 CBM 교육에 통합할 수 있습니다.

- **Performance Highlights**: 여섯 가지 CBM 모델군(families)과 세 가지 벤치마크를 통해 평가된 결과, 정보 기반 정규화(IB-regularized) 모델들이 기존의 vanilla 모델들과 비교하여 일관되게 우수한 성능을 보였습니다. 정보 평면 분석(information-plane analyses) 또한 이론적 근거에 맞는 행동을 지원하며, 최소한의 충분한 개념 병목을 강제하는 것이 예측 능력과 개념 수준 개입의 신뢰성을 모두 향상시킬 수 있음을 입증했습니다.
...
- **What's New**: 최근 알고리즘적 재현성(algorithmic replicability)을 다룬 이 연구는 기계 학습 실험의 재현 가능성 문제를 해결하기 위한 새로운 접근법을 제시합니다. 저자들은 제약이 있는 다중팔 밴딧(constrained MAB) 문제에서 재현 가능한 온라인 학습 알고리즘을 설계하여 그 성공적인 실행을 보여줍니다. 특히, 이 알고리즘들은 고전적인 MAB 문제의 재현 가능성과 비재현 가능 알고리즘 간의 성능 차이가 거의 없는 것을 입증합니다.

- **Technical Details**: 제안된 알고리즘 DEBORA는 동적 에포크 기반 접근 방식을 활용하여 재현 가능한 UCB 스타일의 알고리즘입니다. 이는 에포크 이전에 정해진 고정된 방식이 아닌, 선행적으로 선택된 행동을 에포크의 모든 라운드에 걸쳐 적용하여 학습을 진행합니다. DEBORA의 변형인 DEBORA-S 및 DEBORA-H 알고리즘은 각각 소프트 및 하드 제약 설정에서 작동하며, 두 알고리즘 모두 T에 따라 ~(T)과 같은 성능을 보장합니다.

- **Performance Highlights**: DEBORA 알고리즘의 성능은 기존 비재현 가능한 알고리즘의 성능과 비슷할 뿐만 아니라, 재현 가능성의 추가 비용이 거의 없음을 보여줍니다. 특히, 소프트 및 하드 제약 설정에서 제안된 알고리즘은 기존 최첨단 비재현 가능한 알고리즘과 동등한 수준의 성능을 발휘하여, 제약이 있는 MAB 문제에서의 재현성 있는 학습의 가능성을 열었습니다.



### RNM-TD3: N:M Semi-structured Sparse Reinforcement Learning From Scratch (https://arxiv.org/abs/2602.14578)
- **What's New**: 이 논문은 강화학습에서 N:M 구조적 스파시티(N:M structured sparsity)를 처음으로 연구하여 압축, 성능, 하드웨어 효율성 간의 균형을 제공합니다. 제안된 RNM-TD3 알고리즘은 비정책 강화학습(Off-policy RL)에서 모든 네트워크에 대해 N:M 스파시티를 강제하여 하드웨어 가속기와의 호환성을 유지합니다. 실험 결과, RNM-TD3는 50%에서 75% 스파시티 상태에서도 밀집 모델보다 더 나은 성능을 보여줍니다.

- **Technical Details**: 이 연구는 N:M 구조적 스파시티를 이용하여 강화학습에서의 스파스 훈련을 구현합니다. 네트워크의 매개변수 중 많은 부분이 제로일 때 스파스하다고 간주되며, 현재까지 제안된 방법들은 비구조적 스파시티에 의존했습니다. 이는 하드웨어 가속 기회를 제한하는 불규칙한 계산 패턴을 초래했으나, RNM-TD3는 일관된 N:M 방식으로 스파시티 패턴을 유지하도록 설계되었습니다.

- **Performance Highlights**: 체계적인 실험 결과, RNM-TD3는 특정 환경에서 밀집 모델보다 최대 14%의 성능 향상을 달성하였습니다. 또한, 87.5% 스파시티에서도 경쟁력을 유지하며 잠재적인 훈련 속도 향상을 가능하게 합니다. 이러한 성과는 N:M 스파시티 사용 시 계산 효율성을 극대화함으로써 이루어진 것입니다.



### DCTracks: An Open Dataset for Machine Learning-Based Drift Chamber Track Reconstruction (https://arxiv.org/abs/2602.14571)
- **What's New**: 이번 연구에서는 단일 및 이중 트랙 드리프트 챔버 이벤트의 몬테 카를로(MC) 데이터셋을 소개하여 머신 러닝(ML) 기반 트랙 재구성을 발전시키고자 합니다. 연구를 통해 표준화되고 비교 가능한 평가를 위해 트랙 재구성에 특화된 메트릭을 정의하며, 기존 전통적 트랙 재구성 알고리즘 및 그래프 신경망(GNNs) 방법에 대한 결과를 보고합니다. 이러한 접근 방식은 향후 연구를 위한 엄격하고 재현 가능한 검증을 촉진합니다.

- **Technical Details**: 정밀 측정을 위한 실험에서 높은 에너지 물리 실험 및 고정밀 검출기와 데이터 분석이 필요합니다. 특히, ML 접근법, 특히 GNN을 사용하여 트랙 파라미터를 직접 재구성할 수 있는 가능성을 탐색합니다. 기존의 전통적인 알고리즘과 GNN 기반의 ML 트랙 재구성 모델을 비교하고, 데이터셋의 신뢰성과 평가 메트릭의 효과성을 입증합니다.

- **Performance Highlights**: 이번 데이터셋은 드리프트 챔버 특성을 충실히 포착하고 있으며, 트랙 재구성 방법의 발전을 위한 필수 자원입니다. 연구 팀은 다양한 유형의 이벤트를 생성하고, MC 진실 정보를 통해 사건 레벨 선택 및 트랙 레벨 선택을 수행하여 질 높은 데이터셋을 구축하였습니다. 이는 고정밀 물리 실험을 위한 ML 방법의 발전을 가속화하기 위한 중요한 발판이 될 것입니다.
...
- **What's New**: 이번 연구는 제한된 다중팔 밴디트(multi-armed bandit, MAB) 문제의 새로운 접근 방식을 제시합니다. 기존 연구에서 제약 조건이 적대적일 경우, 후회(regret)와 제약 위반의 동시 최적 보장이 어려웠으나, 본 논문에서는 확률적 제약을 가지면서도 손실이 임의로 변동할 수 있는 상황에서 최적의 알고리즘을 제공합니다. 특히, 새로운 알고리즘은 전반적인 제약 모형에 대한 이해를 향상시키는 데 기여합니다.

- **Technical Details**: 저자들은 제약 위반을 측정하기 위해 비상태성(non-stationarity) 수준을 설정하여 이며 C가 제약이 얼마나 비상태적인지를 나타냅니다. 알고리즘은 전피드백(full feedback)과 밴디트 피드백(bandit feedback) 상황에 따라 다르게 작동하며, 각 경우에 대해 제약과 손실의 변동을 반영한 성능 보장을 제공합니다. 특히, 완전한 피드백에서 제안된 알고리즘은 $	ilde{	ext{O}}(	ext{에 대한 새로운 결과를 도출합니다. $C$는 비상태성의 정도를 나타냅니다.

- **Performance Highlights**: 저자들은 알고리즘이 전피드백 하에 $	ilde{	ext{O}}(	ext{T} + C)$의 후회와 제약 위반을 달성한다고 주장합니다. 효율적인 최적화 절차를 통해 매 라운드에서 실질적인 결정을 내릴 수 있도록 하여, 동적 환경에서도 안정적인 성능을 보여줍니다. 또한, 밴디트 피드백 상황에서 제안된 알고리즘은 $	ilde{	ext{O}}(T^{	ext{최대} rac{1}{2}, eta} + C)$의 긍정적 위반과 $	ilde{	ext{O}}(T^{eta} + CT^{1-eta})$의 후회를 달성하여 유연한 대처가 가능합니다.
...
- **What's New**: 이 논문에서는 변환기(transformers)를 활용하여 이차 프로그램(quadratic programs)을 해결하는 새로운 접근법을 탐구합니다. 특히 공분산 행렬(covariance matrices)을 포함하는 의사결정 문제에서 이 능력이 어떻게 유용한지를 중점적으로 살펴봅니다. 저자들은 선형 어텐션 메커니즘(linear attention mechanism)을 통해 제약 조건 없는 이차 프로그램을 효과적으로 해결할 수 있음을 입증하였으며, 이는 행렬 변수를 토큰화(tokenizing)하고 그래디언트 하강법(gradient descent) 반복을 모방하여 수행됩니다.

- **Technical Details**: 본 연구에서 제시하는 Time2Decide는 공분산 정보를 명시적으로 활용하여 시간 시계열 모델(time-series foundation model, TSFM)을 향상시키는 방법입니다. 이 방법은 공분산 구조를 포함하는 토큰을 입력으로 추가하여 모델이 역사적 맥락에서 의사결정으로의 엔드 투 엔드(end-to-end) 매핑을 학습하도록 합니다. 실험 결과 Time2Decide는 기존 TSFM 모델보다 일관되게 우수한 성능을 보였으며, 전통적인 예측-최적화(predict-then-optimize, PtO) 접근법보다도 뛰어난 결과를 나타냅니다.

- **Performance Highlights**: Time2Decide는 포트폴리오 최적화(portfolio optimization) 문제에서 강력한 성능을 보이며, 이는 공분산 정보가 포함되어 있어 최적의 결정을 내리는 데 도움을 줍니다. 특히, 이 두 단계 접근법은 예측이 불확실하거나 포트폴리오 전환 제약이 느슨할 때에도 유효하였습니다. 논문에서 제시된 Proposition 5.1을 통해서도 이 모델이 두 단계 최적화 과정을 명확히 능가하는 것이 증명되었습니다.



### Divine Benevolence is an $x^2$: GLUs scale asymptotically faster than MLPs (https://arxiv.org/abs/2602.14495)
- **What's New**: 이 논문은 전통적인 함수 근사 이론의 관점에서 LLMs에 있어 GLU(Gated Linear Unit)의 성공을 새롭게 해석합니다. GLU는 전통적인 MLP(Multi-layer Perceptron)와 비교하여 함수 근사에서 더 빠른 비율의 스케일링을 보여주는데, 이는 GLU가 조각선형(의 유사한) 함수들을 형성함으로써 가능해졌습니다. 이 연구는 검증된 경험적 발견을 기반으로 GLU의 스케일링 특징을 수학적으로 입증합니다.

- **Technical Details**: 연구는 GLU와 MLP의 함수 근사를 통해 스케일링 기울기를 유도하고, GLU의 경우 L(P) ∝ P^{-3}, MLP는 L(P) ∝ P^{-2}로 표현됨을 밝혔습니다. 이로 인해 GLU가 상당히 더 높은 성능을 가지는 점이 강조됩니다. 이 연구는 GLU를 통해 제안된 매개변수 구성을 기반으로 1D 함수 근사 문제에 대한 실험적 검증을 포함하고 있습니다.

- **Performance Highlights**: 1D 함수 근사에서 MLP는 오차가 1/n^2의 비율로 줄어드는 반면, GLU는 1/n^3으로 감소합니다. 이는 GLU가 매개변수 수를 두 배로 증가시킬 경우, 오차를 여덟 배 줄일 수 있음을 의미합니다. 이러한 결과는 GLU의 구성 요소가 더욱 높은 근사 능력을 제공함을 시사하며, 향후 대규모 모델 설계에 있어 새로운 가능성을 열어줍니다.
...
### One Good Source is All You Need: Near-Optimal Regret for Bandits under Heterogeneous Nois (https://arxiv.org/abs/2602.14474)
- **What's New**: 본 논문에서는 $M$개의 이질적 데이터 소스를 활용한 $K$-armed Multiarmed Bandit (MAB) 문제를 다루고 있습니다. 각 데이터 소스는 서로 다른 노이즈 분산을 가지고 있으며, 이를 통해 학습자는 최적의 소스를 동적으로 선택하며 보상을 최소화하는 것을 목표로 합니다. 새로운 알고리즘인 Source-Optimistic Adaptive Regret minimization (SOAR)은 높은 분산의 소스를 효과적으로 제거하고, 최적의 데이터 소스와 팔(arm)을 동시에 식별하는 균형 잡힌 접근 방식을 제공합니다.

- **Technical Details**: SOAR 알고리즘은 ‘균형 잡힌 최소-최대 LCB-UCB 접근법’을 적용하여 각 팔의 보상에 대한 상한 신뢰 구간(Upper Confidence Bound, UCB)과 소스의 분산에 대한 하한 신뢰 구간(Lower Confidence Bound, LCB)을 결합합니다. 분석 결과, SOAR는 $	ilde{O}ig({oldsymbol{eta}}^2rac{	ext{log}	ext{T}}{	ext{gap}}ig)$와 같은 조건부 후회를 달성하며, 이는 최적의 소스 분산에 대한 사전 지식이 있는 오라클 MAB과 유사한 성능을 보입니다. 즉, 각 팔(i)과 소스(j)를 선택하여 관측되는 노이즈 보상을 통해 정보를 수집합니다.

- **Performance Highlights**: 실험 결과, SOAR 알고리즘은 다양한 합성 문제 및 실제 문제인 MovieLens 25M 데이터셋에서 기존의 기준선보다 우수한 성능을 보였습니다. 특히, 평균 분산이 높은 비효율적인 소스를 적시에 제거함으로써 후회를 줄이는 데 성공했습니다. SOAR는 복잡한 데이터 환경에서 최적 결과를 도출할 수 있는 가능성을 보여줍니다.



### LACONIC: Length-Aware Constrained Reinforcement Learning for LLM (https://arxiv.org/abs/2602.14468)
- **What's New**: 이번 논문에서는 LACONIC을 제안하여 길이 제한(length control)을 강화하는 동시에 태스크 성과(task performance)를 극대화하는 새로운 RL 기반 접근 방식을 소개합니다. 기존의 접근 방식은 고정된 휴리스틱 보상 방식을 사용하여 태스크 목적과의 불일치 문제를 초래했습니다. LACONIC은 평균 토큰 예산을 반영하여 길이에 따른 비용을 조정함으로써 이러한 문제를 해결합니다.

- **Technical Details**: LACONIC은 기본-구상(dual) 알고리즘으로, 태스크 보상과 길이 비용을 조합하여 학습 신호를 생성합니다. 각 응답에 대해 토큰 예산의 위반 정도에 따라 비용을 부여하며, 학습 중에 이 비율을 조정합니다. 이 방법은 정책 모델을 업데이트 할 때 고전적인 RL 튜닝을 확장하여, 응답 길이에 대한 제약을 효과적으로 적용합니다.

- **Performance Highlights**: LACONIC을 적용한 모델은 기존 길이 제어 기준선에 비해 응답 길이를 50% 이상 줄이면서도 pass@1 성능을 보존하거나 개선하는 성과를 보여주었습니다. 일반 지식 및 다국어 벤치마크에서도 44% 적은 토큰으로 Out-of-domain 성능을 유지했습니다. 이 방법은 표준 RL 튜닝과 통합이 가능하며, 최소한의 배포 오버헤드를 요구합니다.
...
- **What's New**: 본 논문에서는 전통적인 인과 발견( causal discovery ) 방법의 한계를 극복하기 위해 TLVD라는 새로운 인과 모델링 프레임워크를 제안합니다. TLVD는 대규모 언어 모델(LLM)과 전통적 인과 발견 알고리즘(TCDA)의 메타데이터 기반 추론 능력을 통합하여 잠재 변수를 추론하고 그 의미를 명확하게 모델링합니다. 이 방법은 특히 'traceable latent variable discovery'에 중점을 두며, 여러 웹 기반 데이터 소스에서 증거를 탐색하여 추론된 잠재 변수의 추적 가능성을 높입니다.

- **Technical Details**: TLVD는 세 가지 주요 단계로 구성됩니다: (i) 잠재 인과 그래프 구조 식별, (ii) 멀티 에이전트 협업을 통한 잠재 변수 식별, (iii) 잠재 변수의 검증. 각 단계에서 LLM의 배경 지식을 활용하지만, core identifiability(핵심 식별 가능성) 보장은 여전히 전통적인 잠재 변수 발견 알고리즘에 의존합니다. 이 과정에서 게임 이론을 이용하여 여러 에이전트들이 효율적으로 협력하여 잠재 변수를 추론할 수 있도록 설계되었습니다.

- **Performance Highlights**: 논문에서 TLVD는 세 가지 대규모 데이터 세트에서 실험을 수행하였으며, 기존의 최첨단 방법들과 비교하여 평균적으로 Acc에서 32.67%, CAcc에서 62.21%, ECit에서 26.72%의 향상을 보였습니다. 이러한 결과는 TLVD의 효과성과 신뢰성을 뒷받침하며, 실제 의료 데이터와 일반 벤치마크 데이터 세트 모두에서 우수한 성능을 발휘하였음을 보여줍니다. TLVD는 인과 구조 학습에서 잠재 변수의 추적 가능성을 개선하는 데 기여할 것으로 기대됩니다.
...
- **What's New**: 이 논문에서는 하이드로카본 탐사(모사) 과정에서 머신러닝 기반의 설명 가능한 인공지능(Explainable AI, XAI) 방법인 LIME와 SHAP의 한계점을 지적하고, 두 방법에서 도출된 피처 순위가 서로 다를 수 있음을 설명합니다. 이러한 문제를 해결하기 위해, 본 연구는 필요성과 충분성(necessity and sufficiency) 개념을 바탕으로 한 통합 프레임워크를 제안하여 더 신뢰할 수 있는 설명을 제공합니다.

- **Technical Details**: 딥러닝 기반의 예측을 위한 지표로 사용되는 DHIs(Direct Hydrocarbon Indicators)는 지하에서 하이드로카본이 존재할 가능성을 나타내는 비정상적인 지진 진폭 패턴입니다. LIME과 SHAP은 이러한 피처들의 중요도를 순위 매기는 방법으로, 각기 다른 내용을 통해 예측을 설명합니다. 본 연구에서는 LIME과 SHAP의 피처 순위가 다를 수 있음을 보이며, 이들 방법의 결과를 필요성과 충분성을 평가하는 방식으로 향상시키는 방안을 제공합니다.

- **Performance Highlights**: 제안된 통합 프레임워크를 통해 DHI 데이터셋에서 다양한 피처의 중요성을 비교하고, 각각의 필요성과 충분성 점수를 계산합니다. 이를 통해 머신러닝 모델이 잘못된 데이터를 처리하는 능력을 평가하고, 하이드로카본 지표 데이터에 대해 어떤 XAI 모듈이 최상의 결과를 내는지를 분석할 수 있습니다.
...
- **What's New**: 본 논문은 동적 환경에서 다중 클래스 온라인 퍼지 분류기(multi-class online fuzzy classifier)를 제안합니다. 기존의 퍼지 분류기는 두 클래스 문제에만 적용되었지만, 이 연구에서는 다중 클래스 문제를 해결하기 위해 여러 개의 이진 퍼지 분류기를 결합하는 방법을 탐구합니다. 동적 데이터에서의 성능을 평가하기 위해 수치 실험(numerical experiments)을 수행하고 여러 벤치마크 데이터셋을 사용하였습니다.

- **Technical Details**: 퍼지 분류기는 인간 사용자가 사전에 결정한 퍼지 집합을 기반으로 하는 일련의 퍼지 규칙으로 구성됩니다. 이 논문에서는 이진 퍼지 분류기가 입력으로 실수 값을 받아 해당 클래스의 예측 결과를 제공하는 방법을 상세히 설명합니다. 설정된 퍼지 규칙에 따라, 입력 패턴의 멤버십 값과 결과 값을 결합하여 클래스를 예측하는 분류 규칙을 적용합니다.

- **Performance Highlights**: 논문에서 제안한 다중 클래스 온라인 퍼지 분류기는 여러 실험을 통해 전통적인 이진 퍼지 분류기보다 더 나은 성능을 보였습니다. 다중 클래스 문제를 해결하기 위한 One-vs-the-Rest (OvR) 및 One-vs-One (OvO) 방식도 제시되어, 각 방법의 특징과 작동 방식을 간략히 설명합니다. 결과적으로, 동적 환경에서의 학습 알고리즘을 통해 변화하는 데이터를 효과적으로 처리할 수 있는 가능성을 보여주었습니다.
...
- **What's New**: 이 논문은 형식적 시간 논리(specification)가 인공지능 강화 학습(reinforcement learning, RL)에 어떻게 안전성과 견고성을 더할 수 있는지를 탐구합니다. AeroBench F-16 시뮬레이션 벤치마크를 사용하여, Proximal Policy Optimization(PPO) 에이전트를 훈련하고 엔진 스로틀(throttle)과 명령한 비행 속도를 조절합니다. 온라인 conformal prediction을 기반으로 한 conformal STL shield를 제안하여 RL 에이전트의 행동을 필터링합니다. 이를 통해 전통적인 방법보다 더 강력한 견고성 보장을 제공하는 결과를 도출하였습니다.

- **Technical Details**: 강화 학습(RL)은 순차적 의사 결정을 마르코프 결정 과정(Markov decision process, MDP)으로 모델링합니다. Proximal Policy Optimization(PPO) 알고리즘을 활용하여, 스테이트와 액션 공간을 기반으로 한 네트워크를 통해 데이터를 배워 제어 정책을 최적화합니다. Signal Temporal Logic(STL)은 의사결정 과정에서 시간 의존성을 고려하여 그 특성을 정량적으로 모니터링합니다. 논문에서는 이론적인 안전성을 보장하기 위해 conformal prediction(CP)과 STL을 접목하여 실시간 모니터링 시스템을 구현하였습니다.

- **Performance Highlights**: 실험 결과 conformal shield가 STL 만족도를 유지하면서 기본 성능에 가까운 결과를 유지하는 것으로 나타났습니다. 고유식과 같은 극단적인 조건에서도 견고성을 제공하며, 기존의 전통적인 방법에 비해 보다 정량적인 안전성을 보장합니다. 이러한 결과는 데이터 주도型 RL 제어와 형식적 사양 모니터링을 이용하여 자율 비행 제어의 신뢰성을 상당히 개선할 수 있음을 보여줍니다.
...
- **What's New**: 본 논문은 트랜스포머 모델의 신뢰성에 대한 포괄적인 분석을 제공합니다. 이는 자연어 처리(NLP), 컴퓨터 비전, 의학, 로봇 공학 등 안전-critical 응용 분야에서 그들의 활용이 증가함에 따라 더욱 필요해졌습니다. 논문은 트랜스포머 모델의 신뢰도를 평가하는 데 있어 해석가능성, 설명 가능성, 적대적 공격에 대한 강인성, 공정성 및 개인 정보 보호를 포함하는 핵심 요소를 다룹니다.

- **Technical Details**: 트랜스포머 아키텍처는 시퀀스 모델링 시 주의 메커니즘(self-attention mechanism)을 사용하여 혁신적인 결과를 도출합니다. 이 모델들은 뛰어난 성능에도 불구하고 내부 작동 방식이 불투명하여 신뢰 구축에 도전 과제를 안겨줍니다. 해석가능성(Interpretability) 및 설명 가능성(Explainability) 연구는 모델의 판단 근거를 해명하여 사용자 신뢰를 향상시키는 데 집중하고 있으며, 이러한 연구는 안전-critical 환경에서의 실용적 문제를 해결하는 데 중요한 역할을 합니다.

- **Performance Highlights**: 이 논문은 트랜스포머 모델이 기존 기준에서 그 성능을 초월했음을 강조합니다. 그러나 높은 성능에도 불구하고 이 모델들이 극복해야 할 구조적 취약점과 불공정성을 드러냅니다. 해독 가능성 요구를 충족하면서도 신뢰할 수 있는 AI 시스템으로서의 역할을 유지하는 방향으로 연구가 진행되고 있음을 시사합니다.
...
- **What's New**: 이번 논문에서는 Radial-VCReg라는 새로운 자가 지도 학습 방법을 제안합니다. 이 방법은 VCReg에 Radial Gaussianization 손실을 추가하여, 피쳐 노름을 Chi 분포에 정렬하는 데 중점을 둡니다. 이를 통해 고차원 Gaussian의 정의 속성을 이용해 정보를 극대화하는데 어려움이 없도록 하였습니다. 이 논문은 Radial-VCReg가 기존 방법보다 더 넓은 클래스의 분포를 정규성으로 변환한다는 것을 이론적으로 입증하였습니다.

- **Technical Details**: Radial-VCReg는 피쳐의 방사형 분포를 명시적으로 정규화함으로써 자가 지도 학습의 성능을 높입니다. 이 과정에서 Kullback–Leibler divergence의 추정량을 활용하여 경험적 반경 분포와 진짜 Chi 분포 간의 차이를 최소화합니다. 기존의 VCReg는 공분산( covariance) 정규화를 사용하는 반면, Radial-VCReg는 Radial Gaussianization을 통해 고차원 자료의 정보 최대화를 달성합니다.

- **Performance Highlights**: Radial-VCReg는 실험적으로 기존 VCReg와 비교하여 샘플 분포를 표준 정규 분포에 더 가깝게 만드는 성능 개선을 보였습니다. 또한 실제 이미지 데이터셋에 대해서도 일관된 성능 향상을 이끌어냈습니다. 이러한 결과는 Radial-VCReg가 더 다양하고 정보가 풍부한 표현을 생성하는 데 기여하고 있음을 시사합니다.
...
- **What's New**: 이번 연구에서는 Robust Multi-Task Boosting using Clustering and Local Ensembling (RMB-CLE)이라는 새로운 Multi-Task Learning (MTL) 프레임워크를 제안합니다. RMB-CLE는 에러 기반의 작업 클러스터링과 로컬 앙상블을 통합하여 부정적 이전(negative transfer)을 방지하는 혁신적인 방법론을 제공하며, 각 작업 간의 유사성을 직접적으로 도출합니다. 이는 기능적 불일치와 환산 불가능한 노이즈로 리스크를 분해할 수 있는 이론적 기반을 제공합니다.

- **Technical Details**: RMB-CLE는 에러 기반 유사성 개념을 사용하여 작업 간의 관계를 기반으로 작업을 그룹화합니다. 이를 위해 적응형 계층적 클러스터링을 수행하고, 클러스터 내에서는 로컬 앙상블 모델을 적용하여 강력한 지식 공유가 이루어지면서도 작업별 패턴이 보존됩니다. 이 방식은 기존의 고정 클러스터나 수작업 유사성 지표에 의존하는 방법들과는 달리 동적인 구조를 제공합니다.

- **Performance Highlights**: 실험 결과, RMB-CLE는 합성 데이터에서의 정확한 클러스터 회복 능력을 보여주었으며, 다양한 실제 및 합성 기준에서 기존의 멀티 태스크 방법, 단일 태스크 방법, 풀링 기반 앙상블 방법을 일관되게 초월하는 성능을 기록했습니다. 이러한 결과는 RMB-CLE가 단순한 클러스터링과 부스팅의 조합이 아니라 강력한 멀티태스크 학습을 위한 새로운 기초를 확립하는 일반적이고 확장 가능한 프레임워크임을 입증합니다.
...
Comments:
          34 pages, accepted by ICLR 2026 as a conference paper

- **What's New**: 이번 연구에서는 Batch Size Scheduling (BSS)의 이론적 기초를 탐구하고, Li et al. (2025a)에서 제안된 Functional Scaling Law (FSL) 프레임워크를 통해 이를 분석하는 방법을 제시합니다. BSS의 최적 설정이 작업의 난이도에 따라 어떻게 달라지는지를 밝혀냅니다. 쉽게 해결되는 작업의 경우에는 배치 크기가 지속적으로 증가하는 최적의 스케줄이 나타나고, 어려운 작업의 경우에는 훈련 후반에야 배치 크기를 크게 전환합니다.

- **Technical Details**: 제 연구에서 제안된 FSL 프레임워크는 정적 데이터 예산 하에서 최적 BSS를 특징짓습니다. 'Catch-up effect'라는 동적 메커니즘을 통해 작은 배치에서 큰 배치로 전환된 후 손실이 빠르게 정렬된다는 점을 밝혀냈습니다. 이 메커니즘은 누적된 그래디언트 노이즈의 빠른 망각으로 인해 발생하며, 작업의 난이도에 따라 Catch-up 속도가 결정됩니다.

- **Performance Highlights**: 광범위한 LLM (Large Language Model) 사전 학습 실험 결과, late-switch 스케줄이 상수 배치 및 초기 전환 스케줄에 비해 높은 성능을 기록했습니다. 실험에서는 최대 1.1B 파라미터와 1T 토큰을 사용하는 Dense 및 MoE 아키텍처를 다루어, 모든 환경에서 late-switch 스케줄이 consistently outperforming 하였습니다.



### TS-Haystack: A Multi-Scale Retrieval Benchmark for Time Series Language Models (https://arxiv.org/abs/2602.14200)
Comments:
          18 pages, 13 figures, 10 tables (main paper: 5 pages, 3 figures, 2 tables)

- **What's New**: 이 논문은 TS-Haystack라는 새로운 장기 맥락 검색 벤치마크를 소개합니다. 이 벤치마크는 짧은 활동 구간(needles)을 최대 2시간의 실제 가속도계 데이터(haystacks)로 삽입하여 구성됩니다. 이를 통해 모델이 긴 맥락에서 검색과 추론을 수행할 수 있도록 지원합니다. 또한, 논문에서는 기존 TSLM 아키텍처의 한계가 긴 맥락에서의 정확한 시간적 표현과 효율성 간의 본질적 균형을 필요로 한다고 주장합니다.

- **Technical Details**: TS-Haystack는 10개의 작업 유형과 4개의 카테고리로 구성된 탐색 벤치마크로, 긴 맥락의 검색 성능을 평가할 수 있는 기준을 제공합니다.이 벤치마크는 Capture24 데이터셋을 기반으로 하며, 연구자들은 두 가지 TSLM 아키텍처인 OpenTSLM-Flamingo와 ITFormer를 사용하여 테스트했습니다. 또한, 랜덤 샘플링 및 코사인 혼합 기술을 사용하여 데이터 삽입 프로토콜을 구현했습니다.

- **Performance Highlights**: 연구 결과, LSTM 아키텍처는 176배의 압축 비율에서도 분류 정확성을 유지하거나 향상시켰으나, 검색 성능은 맥락 길이에 따라 떨어졌습니다. 이는 TSLM이 긴 맥락에서 시간적으로 국한된 정보를 잃어버리고 있음을 보여줍니다. 이 결과는 맥락 길이를 계산 복잡성과 분리하는 아키텍처 디자인의 중요성을 강조합니다.
...
### When Benchmarks Lie: Evaluating Malicious Prompt Classifiers Under True Distribution Shif (https://arxiv.org/abs/2602.14161)
- **What's New**: 본 연구는 LLM(대규모 언어 모델) 기반 에이전트의 프롬프트 주입(prompt injection) 및 탈옥(jailbreak) 공격 탐지를 위한 연구로, 18가지 다양한 벤치마크 데이터셋을 사용해 포괄적으로 분석합니다. 기존의 평가 프로토콜은 데이터셋의 동일한 출처에서 훈련 및 테스트 샘플을 사용하여 성능을 과대 평가하는 문제를 지적하며, 데이타셋을 통째로 제외하는 Leave-One-Dataset-Out (LODO) 평가 방식을 제안합니다.

- **Technical Details**: 연구에서 제안한 LODO 평가는 특정 데이터셋을 통째로 제외하고 프롬프트 공격 분류기를 평가함으로써, 표준 평가와의 AUC 차이가 평균 8%에 이른다는 것을 보여줍니다. 또한, Sparse Auto-Encoder (SAE) 특징 계수를 분석하여, 최상위 특징의 28%가 데이터셋에 의존하는 단축키임을 발견했습니다. 다양한 생산 시스템(PromptGuard 2, LlamaGuard)과 LLM-as-judge 접근 방식을 비교하여, 간접 공격에 대한 탐지가 7-37%에 불과하다는 것을 밝혔습니다.

- **Performance Highlights**: LODO 안정적인 SAE 특징들은 분류기 결정에 대한 신뢰할 수 있는 설명을 제공하며, 데이터셋 아티팩트를 필터링하여 설명 가능한 특징을 드러냅니다. 연구 결과는 LODO가 프롬프트 공격 탐지에 적합한 평가 프로토콜로 자리 잡을 수 있도록 하며, 분류기 일반화 이해를 위한 진단 도구들을 제공합니다. 추가적으로, 에이전트 기반 타겟 공격을 평가할 수 없는 기존 시스템들의 한계도 드러났습니다.
...
- **What's New**: 본 연구에서는 Sparse Mixture-of-Experts (MoE) 모델의 전반적인 효율성을 높이기 위한 두 가지 새로운 정규화 손실 함수를 제안합니다. 이는 전문가 간의 중복을 줄이고 라우팅 효율성을 향상시켜 MoE의 전문성을 강화하는 데 목표를 두고 있습니다. 이러한 개선은 모델 아키텍처나 라우터를 변경하지 않고도 이루어집니다.

- **Technical Details**: 연구에서 제안하는 첫 번째 손실인 intra-layer specialization loss는 동일한 토큰에 대해 서로 다른 전문가의 SwiGLU 활성화 간 코사인 유사성을 처벌하여 중복 지식을 억제합니다. 두 번째는 cross-layer coupling loss로, 인접한 층 간의 전문가 경로를 명확히 하며 라우팅 확률을 최적화하여 라우팅 명확성을 높입니다. 이 두 손실은 기존의 load-balancing loss와는 독립적이며, DeepSeekMoE 및 기본 top-k MoE 아키텍처와의 호환성이 보장됩니다.

- **Performance Highlights**: 실험 결과, 제안된 손실 함수들은 다양한 예비 학습(pre-training), 세부 조정(fine-tuning) 및 제로샷 평가(zero-shot evaluation) 환경에서 일관된 성능 향상을 보여줍니다. 예를 들어, load-balancing에 추가했을 때, perplexity는 평균 2.7% 감소하는 성과를 보였고, 전체 파라미터 튜닝 시 HumanEval pass@1도 약 3.66 포인트 향상되었습니다. 이러한 성과는 더 낮은 활성화 중복과 명확한 cross-layer 전문가 경로를 통해 이루어졌습니다.
...
- **What's New**: 본 연구에서는 Quadratic Program (QP)을 통한 미분(differentiation)에서의 한계를 극복하기 위해 dXPP라는 새로운 프레임워크를 제안합니다. 기존의 KKT(Karush-Kuhn-Tucker) 시스템을 통해 미분하는 방식은 계산 비용과 수치적 안정성이 부족한 경우가 많았습니다. dXPP는 QP 해결(computation)과 미분 과정을 분리하여, 더욱 효율적이고 강인한 방법을 제공하는 점에서 새로운 기여를 합니다.

- **Technical Details**: dXPP는 penalized 문제를 통해 제약 조건을 목적 함수에 통합시키고, 역전파 과정에서는 더 작은 크기의 선형 시스템(linear system)을 풀도록 간소화합니다. 이 프레임워크는 특히 KKT 시스템의 명시적 미분을 피할 수 있으며, 원시 변수(primal variables)에 대해 대칭 양의 정부호(symmetric positive definite) 선형 시스템을 해결하는 과정을 요구합니다. 이를 통해 dXPP는 대규모 문제에서의 계산 효율성을 크게 개선합니다.

- **Performance Highlights**: 다양한 기준(baselines)과 실제 포트폴리오 최적화(real-world portfolio optimization) 작업에 대해 dXPP를 평가한 결과, 기존의 KKT 기반 미분 방법에 비해 꾸준히 우수한 성능을 보였습니다. 특히 대규모 문제에서의 처리 속도가 크게 향상되었으며, dXPP는 QP 미분에서 최신 성능(state-of-the-art) 기준을 달성했습니다.
...
- **What's New**: 이 논문은 Sparse Autoencoders (SAEs)의 효용성과 한계를 심도 있게 평가합니다. SAEs는 인간이 해석할 수 있는 특징으로 신경망의 활성화를 분해하는 도구로 주목받고 있으며, 최근에는 다양한 아키텍처가 제안되었습니다. 그러나 SAEs가 진정한 의미 있는 특징을 회복하지 못한다는 부정적인 결과들이 증가하고 있습니다.

- **Technical Details**: SAEs는 고차원 활성화 벡터를 희소한 특징의 선형 조합으로 근사화하는 방식으로 동작합니다. 활성화 패턴을 이해하는 데 도움을 주기 위해, 이 논문은 세 가지 간단한 기준선을 제안하며, 이 기준선을 통해 SAEs의 학습된 특징을 평가합니다. 이를 통해 SAEs가 원하는 구조를 회복하지 못한다는 점을 강조했습니다.

- **Performance Highlights**: 다수의 SAE 아키텍처를 실험한 결과, 제안된 기준선들이 완전 훈련된 SAEs와 비슷한 해석 가능성을 보였습니다. 특히, 해석 가능성(0.87 대 0.90) 및 희소성 탐색(0.69 대 0.72)에서도 비슷한 성능을 보였으며, SAEs가 의미 있는 특징을 학습한다는 주장을 도전합니다. 이러한 발견은 SAEs의 현재 평가 방법이 불충분할 수 있음을 시사합니다.
...
- **What's New**: 이번 연구는 비압축성 유동을 포토리포지토리 구조를 통과하고 주위에서 동시에 예측하는 문제에 대한 첫 체계적 평가를 제공합니다. 이를 위해 Physics Informed PointNets (PIPN)와 Physics Informed Geometry Aware Neural Operator (PI-GANO) 아키텍처를 활용하였으며, 각기 다른 기하학에서 재훈련 필요 없이 예측을 가능하게 합니다. 또, 3D 문제를 다루는 PI-GANO 아키텍처의 확장 및 복잡하고 불규칙한 기하를 포함한 실험도 포함되어 있습니다.

- **Technical Details**: 제안된 모델은 Navier-Stokes 및 Darcy-Forchheimer 방정식을 기본으로 하여, 서로 다른 경계 조건과 기하학적 변형에 대해 일반화를 평가합니다. 연구에서는 OpenFOAM을 사용하여 2D 덕트와 3D 바람 방지 시나리오에 대한 데이터 세트를 생성하였으며, 각 분야에서 운동 방정식을 적용합니다. 특히, 내부의 흐름과 외부의 유동 서로의 상호작용을 정량화하여 복잡한 물리적 상황을 모델링합니다.

- **Performance Highlights**: 연구 결과는 보았거나 보지 못한 경우 모두에서 낮은 속도 및 압력 오류를 보여주었으며, 와이크 구조가 정확하게 재현되었습니다. 주로 급격한 경계 및 큰 기울기가 있는 영역에서 성능 저하가 발생했지만, 이 모델은 복잡한 기하학에서 필요한 계산을 효율적으로 수행할 수 있는 잠재력을 가지고 있습니다.
...
- **What's New**: 본 논문은 무한차원의 Hilbert 공간에서의 Neural Optimal Transport(Neural OT)를 연구합니다. 기존의 Semidual Neural OT 방법이 종종 표적 분포(target distribution)를 정확하게 캡처하지 못하는 문제를 분석하고, 정규 측도를 통한 해결책을 제시합니다. 또한, Gaussian smoothing 전략을 통해 잘 정립된 해결책을 제공하며, 이론적으로 Monge 맵의 고유성을 입증하고, smoothing의 성공은 covariance operator의 kernel에 엄격하게 의존한다는 것을 확립합니다.

- **Technical Details**: Optimal Transport(OT) 문제는 source measure μ와 target measure ν 간의 변환을 탐구하는 수학적 프레임워크입니다. 본 연구에서는 semi-dual Neural OT 방법론을 제안하며, 이 방법이 정규 조건 하에서 유일한 최적 transport map을 도출함을 입증합니다. 이를 위해 Gaussian smoothing 전략에 기반한 정규화 방식을 사용하여, 데이터 매니폴드의 특성에 맞춰 noise를 주입해야 한다고 주장합니다.

- **Performance Highlights**: 실험 결과, synthtic functional data와 time-series datasets에서 제안된 접근 방식이 spurious solutions를 효과적으로 억제하며 기존의 성능 기준들을 초월함을 보여줍니다. 또한, 실제 고차원 시간 시리즈 보간(imputation) 벤치마크에서 state-of-the-art 성능을 달성함으로써 이론적 발견의 실제 유용성을 검증합니다. 본 연구는 무한차원 공간에서의 최적 coupling 회복을 위한 이론적으로 건전한 방법으로 Gaussian smoothing을 확립합니다.
...
- **What's New**: 이번 논문에서는 에너지 수확(energy harvesting) 기술을 분산형 연합 학습(decentralized federated learning, DFL)에 적용하여 에지 디바이스가 환경 에너지를 수집하고 지속 가능하게 운영할 수 있도록 제안합니다. 이는 배터리 소모 문제를 해결하는 데 초점을 맞추고 있으며, 필요한 감시 없이 디바이스 간의 모델 교환이 이루어지는 구조입니다.

- **Technical Details**: 저자들은 다중 에이전트 마르코프 결정 프로세스(multi-agent Markov decision process, MDP) 프레임워크를 통해 DFL과 에너지 수확 디바이스를 위한 최적 스케줄링 및 전송 전략을 도출합니다. 제안된 알고리즘은 두 홉 이내의 이웃 디바이스로부터의 지역 상태 정보만을 활용하여 통신 오버헤드와 계산 복잡성을 상당히 줄입니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘은 기존 벤치마크 방법에 비해 우수한 테스트 정확도를 달성하며, 배터리 용량이 학습 성능에 미치는 영향을 조사합니다. 이로 인해 제안된 DFL 프레임워크가 실제 데이터셋에서 효과적임을 입증하게 됩니다.
...
- **What's New**: 이 연구에서는 언어 모델의 길이 일반화(length generalization) 능력을 개선하기 위해 새로운 위치 인코딩(position encoding) 전략인 Random Float Sampling (RFS)을 도입합니다. RFS는 미리 정해진 이산 집합(discrete set)에서 위치 인덱스를 선택하는 대신, 무작위로 샘플링된 연속값을 사용하여 훈련 중 다양한 인덱스에 노출되도록 합니다. 이를 통해 훈련 과정에서 제공되지 않았던 길이에 대해서도 OOD(out-of-distribution) 문제를 피할 수 있습니다.

- **Technical Details**: RFS는 훈련 중에 주어진 길이 n의 시퀀스에 대해 [0,1) 범위에서 독립적으로 샘플링 된 n개의 값을 정렬하여 위치 인덱스로 사용합니다. 이 방법은 인퍼런스(inference) 단계에서도 공통 범위의 고르게 배치된 부분으로 인덱스를 선택하며, 이를 통해 훈련 중 샘플링의 무작위성에 의존하지 않게 됩니다. RFS는 기존의 위치 인코딩 방식과 비교할 때 상대적인 토큰 간 거리 기반으로 주의 메커니즘(attention mechanism)을 학습할 수 있게 해 줍니다.

- **Performance Highlights**: 실험 결과, RFS는 길이가 두 배인 입력에서 약 80%의 정확도를 유지하는 반면, 기존 방법(NoPE)에서는 1.5배 길이의 입력에서 약 20%의 정확도에 그쳤습니다. 또한, RFS는 미지의 길이에 대한 제로 샷(Zero-shot) 상식 추론(common sense reasoning) 과제에서도 강력한 성능을 보여주며, 이는 기존 방식의 성능과 비교할 때 유사한 결과를 나타냅니다.
...
- **What's New**: 이번 연구는 기후 회복력(climate resilience)과 지속 가능성(sustainability)을 위한 결정 요망 주간부터 월 간 계획 영역에서 필요로 하는 Subseasonal-to-seasonal (S2S) 예보의 마지막 단계, 즉 과학적 예보를 신뢰할 수 있는 실행 가능한 기후 서비스로 전환하는 과정을 주목합니다. 연구자들은 S2SServiceBench라는 멀티 모달(multi-modal) 벤치마크를 소개하여 이 능력을 평가할 수 있는 새로운 도구를 제공합니다.

- **Technical Details**: S2SServiceBench는 운영 기후 서비스 시스템에서 선별된 150개 이상의 전문가 선정 사례를 포함하며, 농업, 재해, 에너지, 금융, 건강 및 운송 등 여섯 가지 응용 분야를 아우릅니다. 각 사례는 세 가지 서비스 수준에서 구현되며, 총 약 500개의 작업과 1,000개 이상의 평가 항목을 생성하여 기후 회복력 및 지속 가능성 응용 프로그램을 다룹니다.

- **Performance Highlights**: 이 벤치마크를 활용하여 최첨단 멀티모달 대형 언어 모델(MLLMs)과 에이전트의 성능을 분석하였으며, S2S 서비스 이해 및 추론에서 지속적인 도전과제들이 드러났습니다. 구체적으로 실행 가능한 신호 이해(actionable signal comprehension), 불확실성을 실행 가능한 인계로 전환하는 문제, 동적 위험에 대한 안정적이고 증거 기반의 분석 및 계획에서의 어려움을 강조하며, 향후 기후 서비스 에이전트를 구축하기 위한 실행 가능한 지침을 제시합니다.
...
- **What's New**: 이번 논문에서는 Koopman 기반의 신경망 프레임워크인 KoopGen을 도입합니다. KoopGen은 상태 의존적(co-dependent)으로 설계된 Koopman 생성자를 통해 비선형 역학을 모델링합니다. 이로 인해 보존적이며 비가역적인 동역학을 구분할 수 있는 구조를 제공하고, 학습 과정에서 정확한 연산자 이론적 제약을 강화합니다. 결과적으로 다양한 복잡성을 가진 시스템을 아우르는 통합된 프레임워크를 제시합니다.

- **Technical Details**: Koopman 생성자(Koopman generator)는 Cartesian 분해를 활용하여 비선형 동역학을 선형적 관점으로 해석합니다. 이는 비선형 시스템의 스펙트럼 분석을 가능하게 하고, 정확한 해석 가능 모드 구조를 제공합니다. KoopGen은 기존의 데이터 기반 방법들이 가진 한계를 극복하기 위해 고안되었으며, 상태 의존적인 연산자(parameterization)를 도입하여 연속 스펙트럼을 효과적으로 모델링합니다.

- **Performance Highlights**: KoopGen은 비선형 오실레이터, 고차원 카오스 및 시공간 동역학 등 다양한 시스템에서 그 효과를 입증했습니다. 실험 결과, KoopGen은 기존 방법에 비해 장기 예측 정확도와 표현 품질에서 우수함을 보여주었습니다. 또한 강력한 해석 가능성과 물리적 충실성을 바탕으로 높은 차원의 시스템에도 확대 적용할 수 있는 가능성을 제시합니다.
...
- **What's New**: 이 논문에서는 고정된 단계 크기(stepsize) $\alpha$에 대해 비대칭적 확률 보정(steady-state)의 근사오차를 분석하여 명시적인 비점근적(Non-asymptotic) 범위를 제시합니다. 기존 연구들은 $\alpha \downarrow 0$일 때 정규분포(Gaussian random vector)로의 약한 수렴을 보였으나, 그에 대한 유용한 오차 경계는 없었습니다. 본 논문은 Wasserstein 거리(Wasserstein distance)를 통해 고정된 $\alpha$에서 이를 구체적으로 증명합니다.

- **Technical Details**: 연구에서는 드리프트(drift)와 잡음(noise)에 대한 순간(moment) 조건을 갖춘 일반적인 이론으로 시작하여, 중심 축척(centred-scaled) 정적 상태와 적절한 정규 분포 간의 Wasserstein 거리를 제한합니다. 이를 위해 i.i.d. 및 Markovian 잡음 모델 모두를 다루며, 대표적인 세 가지 SA 설정의 이론을 실증적으로 적용하였습니다: (1) 매끄럽고 강하게 볼록한 목표에 대한 확률 경량하강법(Stochastic Gradient Descent, SGD), (2) 선형 SA, (3) 수축적 비선형 SA입니다.

- **Performance Highlights**: 작은 $\alpha$에 대해 Wasserstein 거리의 차수 $\alpha^{1/2}\log(1/\alpha)$에 의존하는 구체적인 경계치를 도출했습니다. 또한, Berry–Esseen 타입의 비균일 경계에서 정적 상태의 꼬리 확률이 정규 꼬리에 비해 어떻게 변하는지를 비교했으며, 체계적인 오차 항을 도출하였습니다. 강하게 볼록하지 않은 일반적인 목적 함수에 대해서도 SGD의 같은 분석을 적용하고, 올바른 스케일링 하에서 비정규(Gibbs) 한계 법칙을 확인하고, 이에 대한 Wasserstein 오차 경계를 제공합니다.
...
### QuRL: Efficient Reinforcement Learning with Quantized Rollou (https://arxiv.org/abs/2602.13953)
Comments:
          Accepted to ICLR 2026

- **What's New**: 본 연구에서는 훈련 효율성을 높이기 위해 Quantized Reinforcement Learning (QuRL)이라는 새로운 접근 방식을 제안합니다. QuRL은 양자화된(actor) 연산을 통해 rollout을 가속화하며, 이는 전통적인 LLM 훈련에서 자주 발생하는 효율성 병목 현상을 해결하는 데 기여합니다. 또한 Adaptive Clipping Range (ACR)와 Update-Aware Quantization (UAQ) 기법을 도입하여 훈련 안정성과 업데이트의 성능을 향상시킵니다.

- **Technical Details**: QuRL에서는 정책 비율 정책 비율(Policy Ratio)의 변화를 기반으로 클리핑 비율을 동적으로 조정하는 Adaptive Clipping Range (ACR)를 제안합니다. 그리고 일반적인 RL 업데이트 과정에서 발생하는 양자화 문제를 해결하기 위해 업데이트 인식 양자화(Update-Aware Quantization)를 사용합니다. 이를 통해 훈련 중 가중치 변화가 양자화 오류 보다 크도록 보장하여 훈련 동역학에 더 잘 통합되도록 합니다.

- **Performance Highlights**: 실험 결과, QuRL은 7B, 14B 및 32B 모델에서 20%에서 80%까지의 처리량 향상을 달성했습니다. 특히, INT8 QuRL은 DeepScaleR 벤치마크에서 평균 55.5%의 정확도를 기록하며 기존 성능을 1.7% 초과하는 성과를 보였습니다. 이러한 결과는 QuRL의 효율성과 효과성을 강하게 입증합니다.
...
- **What's New**: 이 논문에서는 자동화된 머신러닝(Automated Machine Learning, AutoML)의 한계를 극복하고, 코드 기반의 모듈형 및 검증 가능한 아키텍처 패러다임으로 전환하는 새로운 다중 에이전트 프레임워크인 iML을 소개합니다. iML은 코드 지침 기획(Code-Guided Planning), 코드 모듈 구현(Code-Modular Implementation), 그리고 코드 검증 통합(Code-Verifiable Integration)이라는 세 가지 주요 아이디어로 구성되어 있습니다. 이는 각 구성 요소의 자율적인 경험적 형상화를 기반으로 하여 환각(logic hallucination)을 제거하고, 복잡한(real-world) 데이터 과학 작업을 처리할 수 있도록 설계되었습니다.

- **Technical Details**: 기존의 AutoML 시스템은 일반적으로 복잡한 프로세스에 소요되는 막대한 인적 자원과 기술적 전문성을 요구합니다. iML에서는 각 프로세스를 독립적으로 처리할 수 있는 전문화된 에이전트를 도입하여 데이터 전처리와 모델링을 엄격한 인터페이스 계약에 의해 분리합니다. 또한, 매 단계의 코드를 실제 실행하여 동적 계약 검증을 수행함으로써 환각된 논리를 방지하고, 자기 수정(self-correcting) 기능을 통해 물리적 실행 가능성을 보장합니다.

- **Performance Highlights**: iML은 다양한 실제 Kaggle 대회를 포함한 MLE-BENCH 및 새로 소개된 iML-BENCH에서 실험을 통해 기존의 최첨단 자동화된 에이전트들보다 우수한 성능을 보여주었습니다. MLE-BENCH에서 iML은 85%의 유효 제출율과 45%의 메달 획득률을 달성하며, iML-BENCH에서는 38%에서 163% 더 우수한 평균 표준화 성능 점수(APS)를 기록했습니다. 이 결과들은 iML이 확률적 생성과 신뢰할 수 있는 엔지니어링 사이의 간극을 메우는 가능성을 보여줍니다.
...
- **What's New**: 본 연구는 여러 개의 다중 모드(예: EEG, EOG, ECG, SpO₂)에서의 수면 신호를 일관되게 모델링하는 것의 복잡성을 해결하기 위해 	exttt{sleep2vec}이라는 새로운 기초 모델을 제안합니다. 이 모델의 차별점은 교차 모드 정렬을 통해 다양한 수면 생리 신호의 공동 표현을 학습한다는 것입니다. 또한, 42,249개의 녹음을 사용한 대비적(pre-trained) 훈련을 통해 기초 모델의 성능을 비약적으로 향상시켰습니다. 이 연구는 수면 의학에서 기기와 신호의 이질성을 효과적으로 극복하는 방법을 제시하고 있습니다.

- **Technical Details**: 	exttt{sleep2vec}은 다양한 생리적 데이터에 대한 통합된 다중 모드 전처리 프레임워크를 사용하여 만들어졌습니다. 이 모델은 DEMOGRAPHY, AGE, SITE & HISTORY-aware InfoNCE(objective)를 적용하여, 인구통계적 메타데이터를 기반으로 부정 샘플의 가중치를 동적으로 조정합니다. 이러한 방식은 코호트 특유의 단점을 완화하고 수신기 간의 일반화를 개선하는 데 기여합니다. 이 연구는 수면 신호에 대한 앙상블 모델링을 통해 robust한 표현 학습의 필요성을 강조합니다.

- **Performance Highlights**: 수면 스테이징 및 임상 결과 평가에서, 	exttt{sleep2vec}은 강력한 기준선과 비교하여 지속적으로 더 우수한 성능을 보였습니다. 이 모델은 사용 가능한 모드 중 어떤 것을 선택하더라도 안정성을 유지하고 있으며, 실제 센서 누락 상황에서도 견고한 성능을 제공합니다. 연구 결과는 다양한 모드의 통합과 체계적인 확장이 결합되어 실세계의 수면 생리 신호에 대해 label-efficient하고 일반 목적의 모델링이 가능하다는 것을 입증하고 있습니다.



### Testing For Distribution Shifts with Conditional Conformal Test Martingales (https://arxiv.org/abs/2602.13848)
- **What's New**: 본 연구에서는 고정된 기준조건 설정에서 작동할 수 있는 적합 테스트 마팅게일(conformal test martingales, CTMs)을 활용하여 임의의 분포 전이(detection arbitrary distribution shifts)를 감지하는 연속 테스트를 제안합니다. 기존 CTM 탐지기는 테스트 시간 오염(test-time contamination)의 문제에 시달리며, 이는 분포 전이가 발생한 후에 새로운 관찰값이 참조 세트에 들어와 과거 데이터에 대한 증거를 희석합니다. 반면, 본 방법은 고정된 무효 참조 데이터셋을 통해 새로운 샘플을 비교하여 오염을 피합니다.

- **Technical Details**: 주요 기술적 기여는 고정된 무효 참조 데이터에 대해 여전히 유효한 마팅게일 구성입니다. 이를 통해 통계적 추정 오차를 고려하고, 유한한 참조 세트에 의해 유도된 참조 분포에 대한 오류를 명시적으로 컴퓨팅합니다. 이 과정은 항상 유효한 type-I 오류 제어와, 비대칭적인 분포 전이를 감지하기 위한 경계가 있는 검출 지연을 보장합니다.

- **Performance Highlights**: 실제로, 저희 방법은 기존 표준 CTM보다 더 빠른 속도로 분포 전이를 감지하여 강력하고 신뢰할 수 있는 분포 전이 탐지기를 제공합니다. 실험 결과에서 저희 방법의 후반 변칙 검지는 기존 CTM p-값의 점차 감소하는 증거와는 대조적으로 여전히 높은 값을 유지하여 효과를 입증합니다.
...
- **What's New**: 이 논문에서는 시간 시계열 이상 탐지(Time Series Anomaly Detection, TSAD)를 에이전틱(agentic) 구조를 통해 순차적인 의사결정 과정으로 재구성하는 'AnomaMind' 프레임워크를 제안합니다. 기존의 방법들은 고정된 피쳐(input) 입력을 사용하여 이상 탐지를 단순한 예측 작업으로 다루었으나, AnomaMind는 점차적(localize)으로 이상 구간을 식별하고 자가 성찰(self-reflection) 과정을 통해 결정을 정제합니다.

- **Technical Details**: AnomaMind는 다단계에서 진단 증거를 구축하고 이상 결정을 정제하는 구조화된 에이전틱 워크플로우를 통해 작동합니다. 이 프레임워크는 다중 턴(multi-turn) 상호작용을 통해 자주적 특징(feature) 준비를 증강시키며, 하이브리드 추론(hybrid inference) 메커니즘을 통해 일반 목적 모델이 도구 호출(tool invocation)과 자기 반영을 처리하도록 구성되었습니다.

- **Performance Highlights**: 다양한 시간 시계열 이상 탐지 설정에서 수행된 광범위한 실험 결과, AnomaMind는 일관되게 이상 탐지 성능을 향상시키는 것으로 나타났습니다. 이러한 결과는 에이전틱(tool-augmented) 추론 방식이 TSAD에 있어 효과적임을 검증합니다.



### Fast Physics-Driven Untrained Network for Highly Nonlinear Inverse Scattering Problems (https://arxiv.org/abs/2602.13805)
- **What's New**: 본 논문에서는, 고차원 공간 최적화의 한계를 극복하기 위해, 물리가 주도하는 푸리에 스펙트럼 (Fourier-Spectral) 해결책을 제안합니다. 이 방법은 스펙트럼 도메인 차원 축소를 통해 서브 초 (sub-second) 재구성을 달성합니다. 특히, 유도 전류를 잘라진 푸리에 기저를 사용하여 전파되며, 최적화가 고주파 데이터를 무시하고 저주파 매개변수 공간에 제한되도록 합니다.

- **Technical Details**: 이 연구에서 제안하는 PDF 솔버는 비선형성을 완화하는 수축 적분 방정식 (CIE)와 스펙트럼 유도 감쇠를 수정하는 대비 보상 연산자 (CCO)를 통합합니다. 이러한 방법들은 기존 유닛리스 신경망 (UNNs)보다 100배 빠른 성능을 발휘하며, 인근 산란체 간의 경계 선명도를 향상시키기 위해 브리지 억제 손실 (bridge-suppressing loss)을 설계했습니다. 이는 복잡한 잡음 및 안테나 불확실성 하에서도 강력한 성능을 보장합니다.

- **Performance Highlights**: 수치적 및 실험적 결과들은 제안된 접근 방식의 뛰어난 성능을 보여줍니다. 특히, 제안된 PDF 솔버는 기존 UNNs에 비해 100배의 속도 향상을 이루며, 이는 실시간 마이크로파 이미징 응용에 중요한 돌파구를 제공합니다. 높은 대조를 가진 오브젝트에 대해서도 안정성을 유지하며, 재구성 정확도를 극대화하는 데 기여합니다.



### Cast-R1: Learning Tool-Augmented Sequential Decision Policies for Time Series Forecasting (https://arxiv.org/abs/2602.13802)
- **What's New**: 이 연구에서는 
Cast-R1라는 새로운 학습 기반 시계열 예측 프레임워크를 제안합니다. 이 프레임워크는 기존의 모델 중심 접근 방식에서 벗어나 예측을 순차적 의사 결정을 통한 문제로 재구성하며, 메모리 기반 상태 관리 메커니즘을 도입하여 의사결정에 필요한 정보를 유지합니다. 이를 통해 Cast-R1은 경험적 데이터와 맥락 정보를 활용하여 장기적인 추론을 지원하고 예측을 보정할 수 있는 고급 기능을 제공합니다.

- **Technical Details**: Cast-R1는 예측 작업을 위한 도구 확장형 에이전트 워크플로우를 통해 작동합니다. 이 워크플로우는 기능 추출, 진단 분석 및 예측 모델 호출과 같은 재사용 가능한 기능을 제공하는 모듈형 툴킷을 갖춘 상태에서 수행됩니다. 학습 과정은 두 단계의 학습 전략을 채택하며, 감독된 미세 조정(supervised fine-tuning)과 다중 턴 강화 학습(multi-turn reinforcement learning)을 결합하여 장기적인 예측 정책을 훈련합니다.

- **Performance Highlights**: 다양한 실세계 시계열 데이터셋에서의 광범위한 실험을 통해 
Cast-R1의 효과성을 입증하였습니다. 이 프레임워크는 기존의 방법들과 비교했을 때 예측 정확도를 높이고, 동적 환경 변화에 잘 대응할 수 있는 능력을 展示합니다. 궁극적으로, 이 연구는 에이전틱(paradigms for agentic paradigms) 접근 방식을 통한 시계열 모델링의 추가적인 탐색 방향을 제시하고 있습니다.
...
- **What's New**: MEMTS (Memory for Time Series)는 시간 시계열 예측을 위한 경량의 플러그 앤 플레이 방법으로, 도메인 적응을 위한 효과적인 방안을 제시합니다. 이 방법은 기존의 Domain-Adaptive Pretraining (DAPT)나 Retrieval-Augmented Generation (RAG) 방식의 한계를 극복하는데 중점을 두고 있습니다. 여기서 핵심 요소는 도메인 특화된 시계열 역학을 내재화하는 Knowledge Persistence Module (KPM)입니다.

- **Technical Details**: MEMTS는 도메인 특정의 반복적인 계절 패턴과 트렌드를 학습 가능한 잠재 프로토타입으로 내재화합니다. KPM은 과거 패턴을 캡처하는 Context Encoder와 다양한 미래 예측을 생성하는 Future-Separable Decoder로 구성되어 있습니다. 이 과정에서, 설정-매칭 순열 손실을 이용하여 모드 붕괴를 방지하고, 고정된 TSFM 예측과 메모리 파생 후보를 융합하는 Adaptive Fusion 모듈을 통해 정밀한 시간 단계별 수정을 제공합니다.

- **Performance Highlights**: MEMTS는 다양한 데이터 세트에서 실시된 광범위한 실험을 통해 뛰어난 성능을 입증했습니다. 이 방법은 기존의 SOTA 기준을 지속적으로 초과하며, 낮은 배포 오버헤드로 높은 효율성을 자랑합니다. 또한 MEMTS는 원래 TSFM의 변경 없이 추가할 수 있어 실시간 예측 시나리오의 확장 가능성을 크게 향상시킵니다.



### On Representation Redundancy in Large-Scale Instruction Tuning Data Selection (https://arxiv.org/abs/2602.13773)
- **What's New**: 이 논문에서는 대형 언어 모델(LLM) 훈련에 있어서 데이터 품질이 중요한 요소임을 강조합니다. 연구자들은 적은 양의 고품질 데이터셋에서 훈련된 모델이 더 큰 노이즈가 있는 데이터셋보다 성능이 우수하다는 사실을 알려왔지만, 산업 규모의 데이터 선택에 대한 체계적인 방법은 아직 충분히 탐구되지 않았습니다. 이 연구에서는 의미적 표현 유사성을 바탕으로 한 지시 튜닝 데이터 선택의 중요성을 연구하며, 최신 LLM 인코더의 한계점과 이로 인해 생기는 중복성을 언급합니다.

- **Technical Details**: 이 논문은 새로운 데이터 선택 프레임워크인 압축 표현 데이터 선택(Compressed Representation Data Selection, CRDS)를 제안합니다. CRDS-R 변형에서는 라데마허 무작위 투영(Rademacher random projection) 후에 트랜스포머 숨겨진 층 표현을 연결하는 방법을 사용하며, CRDS-W 변형에서는 화이트닝(whitening) 기반의 차원 축소 기법을 도입하여 표현 품질을 개선합니다. 실험 결과 이 두 변형이 데이터 품질을 크게 향상시켜 현 상태의 다른 방법들보다 우수한 성능을 보여주었습니다.

- **Performance Highlights**: CRDS-W 방식은 SFT 데이터의 3.5%만 사용하여도 네 가지 데이터셋에서 평균 0.71% 더 나은 성능을 발휘하며 전체 데이터셋을 사용하는 것보다 우수한 결과를 나타냅니다. 이는 LLM의 지시 튜닝 프로세스의 효율성을 극대화하고, 엄청난 양의 산업 데이터에서도 효과적으로 데이터 선택을 수행할 수 있는 가능성을 보여줍니다. 이러한 결과는 더 나아가 LLM 훈련 파이프라인의 후속 단계의 효과성에도 긍정적인 영향을 미칠 것으로 기대됩니다.
...
- **What's New**: 본 논문은 행렬-벡터 곱(MVP) 오라클 하에서의 행렬 없는 고유분해(matrix-free eigendecomposition)에 대한 연구를 다룹니다. 기존의 확률적 근사법들은 고정된 스텝 크기를 사용하여 자주 수렴성을 해치며, 업데이트가 소실되는 방식으로 속도를 저하시킵니다. 저자들은 이산 쌍괄호 흐름(discrete double-bracket flow)을 도입하여 이소트로픽 변화에 불변하는 생성을 제공함으로써, 고유분해 문제를 해결하는 새로운 방법을 제안합니다.

- **Technical Details**: 논문에서 제안된 방법은 생성기(generator)인 Ω(M)=[A,D]를 이용하여, 알고리즘이 고유분해를 수행하는 동안 이소트로픽 노이즈를 보이지 않게 합니다. 이 방식은 또한 트레이스가 없는 공분산 C_e에 의존하는 최대 안정적 스텝 크기 η_{max}를 도출합니다. 결과적으로, 제안된 방법은 고전적 알고리즘에서의 한계를 극복하고 더욱 효과적인 수렴을 보장합니다.

- **Performance Highlights**: 결과는 입력-상태 안정성(input-to-state stability) 분석에 따라 글로벌 수렴을 마련하며, 샘플 복잡성(sample complexity)은 O(‖C_e‖_2^2 / (Δ^2 ε))로 스케일링됩니다. 제안된 기법은 명시적인 Givens-회전 구조를 통해 O(log(1/ζ))의 탈락 속도를 제공합니다. 이는 섭동된 그래디언트 하강과 비교하여 더 나은 성능을 약속합니다.
...
- **What's New**: 본 논문은 산업 열전력 시스템의 데이터 기반 최적화를 위한 완전한 기계 학습 기반의 이층 최적화 프레임워크를 제시합니다. 특히, 인공지능(AI)과 디지털화 시대에 운영 데이터가 풍부해진 점을 활용하여, 인공신경망(ANN) 모델을 통해 상위 및 하위 목표 함수의 근사화를 통해 계층적으로 최적화를 수행할 수 있음을 보여줍니다. 이 연구는 ANN-KKT 프레임워크가 기존의 KKT 조건을 활용하면서도 더 빠르고 효율적임을 강조합니다.

- **Technical Details**: 이 연구에서 제안된 ANN-KKT 프레임워크는 피드포워드 ANN 모델을 사용하여 이층 최적화 문제의 목표 함수를 대체합니다. 하위 수준 문제는 KKT 조건으로 대체되어 이층 문제를 단일 수준 최적화 문제로 변환합니다. ANN 모델을 통합하여 전력 생산 관리를 최적화하고, 실제로 660 MW 석탄 발전소 및 395 MW 가스 터빈 시스템에서 사례 연구를 통해 이 프레임워크의 효용성을 검증합니다.

- **Performance Highlights**: ANN-KKT 프레임워크는 660 MW 석탄 발전소에서 583 MW, 395 MW 가스 터빈 시스템에서 402 MW의 전력을 생성하며, 각각의 최적 터빈 열 효율은 7337 kJ/kWh 및 7542 kJ/kWh입니다. 이 프레임워크는 0.22에서 0.88초의 짧은 계산 시간을 요구하여 산업 작업에서의 요구에 적합함을 강조합니다. 또한, 이 방법은 다양한 시나리오에서 열 효율성을 극대화하면서 운영 변수의 불확실성을 고려한 견고한 운영 공간을 정의하는 데도 사용됩니다.



### HBVLA: Pushing 1-Bit Post-Training Quantization for Vision-Language-Action Models (https://arxiv.org/abs/2602.13710)
- **What's New**: 이번 연구에서는 Vision-Language-Action (VLA) 모델의 효율성을 높이기 위한 새로운 프레임워크인 HBVLA를 제안합니다. HBVLA는 VLA에 특화된 이진화(binarization) 방법을 사용하여 1비트 정밀도로 가중치(Weights)를 낮추면서도 모델의 성능을 최대한 유지하는 것을 목표로 합니다. 이 방법은 자원 제한이 있는 로봇 및 엣지 플랫폼에서의 적용 가능성을 높입니다.

- **Technical Details**: HBVLA는 정책 인지(HB) 향상된 해시안(Hessian)을 통해 주요 가중치를 식별하고, 낮은 엔트로피 중간 상태를 유도하기 위해 희소 직교 변환을 사용합니다. 또한, 중요 표시 구간(Haar domain)에서 군 단위의 1비트 정량화를(quantization) 통해 두 가지 가중치 집합을 처리하여 성능 손실을 최소화합니다. 이러한 접근법은 VLAs의 행동 특성을 유지하는 데 중점을 두고 설계되었습니다.

- **Performance Highlights**: 평가 결과 HBVLA는 LIBERO에서 92.2%, SimplerEnv에서 93.6%의 성능을 달성하여 기존의 이진화 방법들과 비교하여 상당히 우수한 성능을 보여주었습니다. 실제 세계의 평가에서도 HBVLA는 완전 정밀도 모델과 비교하여 성공률의 작은 감소만을 초래하여, 자원이 제한된 하드웨어에서의 견고한 배치를 입증했습니다. 이러한 연구는 VLA의 초저비트 정량화(quantization)의 실용적인 기반을 제공합니다.
...
- **What's New**: 본 논문에서는 일반적인 오프라인 함수 근사를 기반으로 하는 최초의 정책 최적화 알고리즘 OPO-CMDP를 소개합니다. 이 알고리즘은 확률적 Contextual Markov Decision Process (CMDP)에 적용되며, 최적의 의존성을 가지는 첫 번째 후회 경계를 제공합니다. 제안된 알고리즘은 CMDP 문제를 해결하는 보다 자연스럽고 계산적으로 우수하며 이론적으로 근사 최적의 경로를 제공합니다.

- **Technical Details**: OPO-CMDP는 표준 오프라인 회귀 오라클(oracle)만을 사용하여 CMDP의 일반적인 오프라인 함수 근사에 근거한 최초의 정책 최적화 알고리즘입니다. 본 논문에서 제안된 알고리즘은 후회 경계를 O~(H^4 \sqrt{T|S||A| \log(|\mathcal{F}||\mathcal{P}|)})로 증명하며, 이는 상태와 행동 공간에 대한 의존성을 이전 연구보다 개선했습니다.

- **Performance Highlights**: OPO-CMDP는 높은 확률의 후회 경계를 제공하며, 이는 기존 최첨단 알고리즘(Lolipop; Qian et al., 2024)보다 우수합니다. 해당 알고리즘은 CMDP의 프레임워크 내에서 실용적이고 높은 차원의 신경망을 훈련할 수 있는 기초를 제공합니다. 또한 STC (Stochastic Contextual Learning)는 A/B 테스트 및 추천 시스템과 같은 다양한 실제 응용에서 정책 최적화의 강력한 선택으로 자리 잡을 것입니다.



### Optimal Regret for Policy Optimization in Contextual Bandits (https://arxiv.org/abs/2602.13700)
- **What's New**: 이 논문은 일반적인 오프라인 함수 추정(offline function approximation)으로 스토캐스틱 컨텍스추얼 다중 무장 도박(Stochastic Contextual Multi-Armed Bandit, CMAB) 문제에 적용되는 정책 최적화 방법의 처음으로 높은 확률의 최적 후회 경계를 제시합니다. 제안된 알고리즘은 효율적일 뿐만 아니라 $	ilde{O}(	ext{sqrt}(K|	ext{A}|	ext{log}|	ext{F}|))$의 최적 후회 경계를 달성합니다. 이 결과는 정책 최적화 방법이 이론적으로 입증된 최적 후회 경계를 달성할 수 있음을 보여줍니다.

- **Technical Details**: 정책 최적화(Policy Optimization, PO) 방법은 강화 학습(Reinforcement Learning, RL)에서 실용적인 기술로, 많은 작업에서 인상적인 성과를 보입니다. CMAB는 외부 요인이 선택된 전략의 결과에 영향을 미치는 온라인 의사 결정 시나리오를 모델링하여 소비자 추천 시스템 및 헬스케어와 같은 다양한 분야에 응용됩니다. 각 라운드에서 에이전트는 현재 컨텍스트에 따라 행동을 선택하고, 최적 행동 선택 전략은 컨텍스트에 따라 다르게 결정되므로, 후회를 최소화하는 것이 목표입니다.

- **Performance Highlights**: 본 논문은 스토캐스틱 CMAB에서 후회를 최소화하는 효율적인 알고리즘을 제안하며, 기존 연구에서는 rigorously provable(엄격하게 입증된) 알고리즘이 부족했습니다. 제안된 알고리즘은 오프라인 회귀 오라클(offline regression oracle)을 통해 관찰된 손실 데이터셋에 가장 잘 맞는 후보 함수를 반환받아 높은 성능을 발휘하며, 전통적인 방법과 비교하여 정책 업데이트의 안정성을 높입니다. 유명한 IGW와 UCB 기반 방법과 비교하여 제안된 방법이 더 부드럽고 안정적인 정책 조정을 제공하는 것을 확인했습니다.



### Attention Head Entropy of LLMs Predicts Answer Correctness (https://arxiv.org/abs/2602.13699)
- **What's New**: 이번 연구에서는 Head Entropy라는 새로운 방법을 소개하여 대규모 언어 모델(LLMs)의 답변 정확성을 예측합니다. 이 방법은 모델 내부의 주의 패턴의 분포를 측정하여 생성되는 답변의 정확성을 예측하는 데 사용됩니다. 이전의 방법들은 답변의 정확성을 평가하지 못하고 다양한 도메인에서 일반화하는 능력이 부족한 문제를 해결하고자 하였습니다.

- **Technical Details**: Head Entropy는 희소 로지스틱 회귀를 사용하여 각 주의 헤드의 2-Rényi 엔트로피를 기반으로 정확성을 평가합니다. 연구는 5개의 지침 조정된 LLM과 3개의 QA 데이터셋을 활용하여 수행되었으며, 각 세팅의 컨텍스트 가용성, 추론의 복잡성, 도메인 특이성과 같은 다양한 요소를 고려하였습니다. 이 과정에서 Head Entropy는 모든 5개의 기준선을 초과하거나 동등한 성능을 발휘했습니다.

- **Performance Highlights**: Head Entropy는 기준선보다 평균 +8.5% AUROC의 성능 향상으로 도메인 간 일반화 능력을 입증했습니다. 또한 질문 토큰만으로도 생성 이전에 예측적인 신호를 제공하여, 평균 +17.7% AUROC를 기록했습니다. 이는 모델이 방향성이 약할 때가 아닌 경우, 즉 훈련 데이터에 없는 경우 발생하는 불안정한 주의 패턴과 관련이 있다는 점에서 중요한 의미를 갖습니다.



### Physics Aware Neural Networks: Denoising for Magnetic Navigation (https://arxiv.org/abs/2602.13690)
- **What's New**: 본 논문은 GPS 신호의 부족한 상황에서 대안이 될 수 있는 자기이상 내비게이션의 발전을 제안합니다. 기존의 내비게이션 시스템의 한계를 극복하기 위해 두 가지 물리 기반 제약 조건인 발산-자유 벡터 장(divergence-free vector field)과 E(3)-등가성을 E(3)-equivariance를 도입합니다. 이를 통해 기계적 일관성을 유지하며, 예측의 정확성을 향상시킵니다.

- **Technical Details**: 제안된 접근법은 자기장 데이터를 추출하면서 물리적 법칙을 준수하도록 설계되었습니다. 이 시스템은 자속을 벡터 포텐셜(A)로 매개변수화하며, 자기장 B는 A의 컬(curl)로 정의됩니다. 또한, 연속적인 시간 동역학을 활용하여 시간에 따른 자기 데이터의 변화를 효과적으로 모델링합니다.

- **Performance Highlights**: 실험 결과, 제안된 제약 조건을 적용함으로써 기존의 심층 학습 방법보다 예측 정확성과 물리적 신뢰성이 크게 향상됨을 보여주었습니다. 논문의 결과는 새로운 신경망 아키텍처와 함께 사용되며, 자기장 데이터를 세밀하게 조절하여 다양한 환경에서 탐색 성능을 극대화합니다.
...
- **What's New**: 이번 논문에서는 사전 훈련 모델(pre-trained models, PTMs)을 사용하는 클래스 점진적 학습(class-incremental learning, CIL)의 한계를 파악하고 이를 개선하는 새로운 접근법인 VILA를 제안합니다. 기존의 분석적 학습(analytic learning) 방식이 직면한 오류 누적 및 특징 불일치 문제를 해결하기 위해, VILA는 이중 분기 프레임워크를 통해 특징의 유연성과 고정된 의미 체계를 융합합니다. 이를 통해 기존 방법의 효율성을 유지하면서도 안정성을 크게 향상시킬 수 있습니다.

- **Technical Details**: VILA 프레임워크는 두 가지 주요 전략을 사용합니다. 첫째, 기하학적 보정을 통해 작업에 적합한 유연한 특징과 고정된 보편적 의미 앵커(semantic anchor)를 통합하여 특징 수준에서 융합합니다. 둘째, 교차 모드 사전(cross-modal priors)을 활용하여 의사 결정 수준에서 예측 편향을 수정하여 분석적 학습의 효율성을 극대화합니다.

- **Performance Highlights**: VILA는 8개 벤치마크에서 광범위한 실험을 거쳐 일관되게 우수한 성능을 발휘하며, 특히 미세한 구별 작업(fine-grained tasks) 및 긴 시퀀스 시나리오에서 두드러진 결과를 보입니다. 이러한 성능은 고충실도(prediction fidelity) 예측과 분석적 학습의 간단함을 조화시킨 결과입니다.
...
- **What's New**: 이 논문은 안전이 중요한 응용 분야에서 신뢰성 보장을 강화하기 위한 새로운 프레임워크인 최적화된 확률 동등 리스크 예측 집합(Optimized Certainty Equivalent Risk Prediction Sets, OCE-RCPS)을 소개합니다. 기존의 리스크 제어 예측 집합(RCPS)과 달리, OCE-RCPS는 꼬리 행동 및 최악의 경우 시나리오를 Capture하여 신뢰성을 높입니다. 제안된 방법론은 사용자 지정 리스크 허용 수준에 따라 보장된 높은 확률을 제공합니다.

- **Technical Details**: OCE-RCPS는 기본적으로 상한 신뢰 경계(Upper Confidence Bounds, UCBs)를 활용하여 일반 최적화된 확률 동등(OCE) 리스크 측정의 예측 집합 파라미터를 식별합니다. 이 시스템은 조건부 가치-at-risk(Conditional Value-at-Risk, CVaR)와 엔트로픽 리스크(Entropic Risk)와 같은 리스크 측정에 대해 높은 확률 보장을 제공하여, 고위험 애플리케이션에 적합합니다.

- **Performance Highlights**: 의료 이미지 분할 작업에서 실시된 실험 결과, OCE-RCPS는 다양한 리스크 측정 및 신뢰성 구성에서 목표 만족률을 지속적으로 충족하는 것으로 나타났습니다. 이는 OCE-RCPS가 실제 응용에서 신뢰할 수 있는 결정을 내릴 수 있도록 함을 보여줍니다. 반면에 기존의 OCE-CRC는 이러한 확률적 보장을 제공하지 못하는 것으로 평가되었습니다.



### Zero-Order Optimization for LLM Fine-Tuning via Learnable Direction Sampling (https://arxiv.org/abs/2602.13659)
- **What's New**: 이 논문에서는 제로-오더(Zero-order, ZO) 방법을 개선한 새로운 프레임워크를 제안하고 있습니다. 기존 ZO 방법들은 높은 차원에서의 변동성과 낮은 효율성으로 인해 문제에 제약이 있었으나, 제안된 방법은 샘플링 분포를 학습 가능한 정책으로 다루어 이전 반복에서 정보를 효율적으로 활용하게 됩니다. 이를 통해 메모리 요구 사항을 줄이고, 대규모 언어 모델(LLM) 미세 조정의 가능성을 높이는 성과를 목적으로 합니다.

- **Technical Details**: 이 연구는 방향 샘플링의 변동성을 줄이는 데 목표를 두고 있으며, 이를 위해 정책 기반의 ZO 프레임워크를 개발했습니다. 학습 가능한 정책을 통해 방향 샘플링을 최적화하고, 이론적인 분석을 통해 수렴 경계에서 차원(d)에 대한 명시적인 의존성을 완화하는 결과를 보였습니다. 이러한 접근법은 메모리 효율성을 제공하면서도 샘플링 분포의 품질을 향상시켜 기울기 정보의 질을 높입니다.

- **Performance Highlights**: 실험적으로, 이 방법은 기존 ZO 기준에 비해 значительно 개선된 성능을 나타냈습니다. LLM 미세 조정의 어려운 벤치마크에서 의미 있는 성과를 검증하였으며, 제안된 방법의 신뢰성을 입증하였습니다. 이러한 연구 결과는 적응형 방향 샘플링이 대규모 ZO 미세 조정에 유용한 경로가 될 수 있음을 시사합니다.
...
- **What's New**: 이번 연구에서는 기존의 time series chain (TSC) 개념을 확장하여 이종 시계열 간 또는 중단된 시계열에서 예상치 못한 발전 동향을 식별하기 위해 Joint Time Series Chain (JTSC)이라는 새로운 정의를 도입했습니다. JTSC는 두 개별 하지만 관련된 시계열 간의 독특한 변화 패턴을 인식할 수 있도록 설계되었습니다.

- **Technical Details**: JTSC의 정의는 시간 시리즈 내의 간극이나 중단으로 인해 발생할 수 있는 강건성 문제를 완화하는 데 중점을 두고 있습니다. 이 연구는 상대적으로 안정적인 기준 시계열(reference time series)과 관심 대상인 시계열(target time series) 간의 이상한 변화 경향을 식별하는 것을 목표로 합니다.

- **Performance Highlights**: 연구 결과, JTSC 알고리즘은 광범위한 실험을 통해 기존 TSC 방법보다 현저히 우수한 성능을 보여주었습니다. 또한 이 방법은 Intel의 실제 제조 응용 프로그램에서 센서 상태 모니터링 및 정기적 또는 비상 유지보수에 유용성을 입증하였습니다.



### Optimization-Free Graph Embedding via Distributional Kernel for Community Detection (https://arxiv.org/abs/2602.13634)
- **What's New**: 이 논문에서는 Neighborhood Aggregation Strategy (NAS) 방법의 한계를 극복하기 위한 새로운 방법인 weighted distribution-aware kernel을 제안합니다. 기존의 NAS 방법이 과도한 smoothing 문제에 취약하다는 점을 지적하고, 노드의 분포 및 노드도(degree)라는 두 가지 중요한 특성을 고려하여 해결하고자 했습니다. 이 방법은 최적화 없이도 노드의 구별 가능성을 유지하며, 커뮤니티 탐지 성능을 크게 향상시킬 수 있습니다.

- **Technical Details**: 제안된 방법은 두 가지 주요 분포 특성을 통합하여 노드 임베딩을 수행합니다. 첫 번째는 입력 공간에서 고밀도의 노드 벡터가 존재하는 지역의 분포이고, 두 번째는 노드의 차수 분포입니다. mWDK(multilevel Weighted Distributional Kernel)은 이러한 특성을 고려하여 임베딩 품질을 개선하는 기능을 합니다. 이 방식은 특히 고차 (high-degree) 및 저차 (low-degree) 노드 간의 정보 집계 속도 차이를 줄이는 데 도움을 줍니다.

- **Performance Highlights**: 실험 결과, 제안된 방법이 기존의 그래프 임베딩 방법, 심지어 딥러닝 기반 방법들보다 더 나은 커뮤니티 탐지 결과를 달성하는 것을 보였습니다. 특히, 스펙트럴 클러스터링을 통해 커뮤니티 탐지 성능이 뚜렷하게 향상되었습니다. 이 연구는 커뮤니티 탐지 분야에서 기존의 문제를 해결하는 중요한 기여를 하고 있습니다.



### Benchmark Leakage Trap: Can We Trust LLM-based Recommendation? (https://arxiv.org/abs/2602.13626)
- **What's New**: 이 논문은 대형 언어 모델(LLMs)을 기반으로 한 추천 시스템에서의 벤치마크 데이터 유출 문제를 처음으로 식별하고 조사합니다. LLMs는 사전 훈련이나 미세 조정 중 벤치마크 데이터셋에 노출되어, 모델 성능이 인위적으로 부풀려지는 현상을 초래합니다. 이 연구는 다양한 데이터 유출 시나리오를 시뮬레이션하여 LLM 기반 추천 시스템의 진정한 성능에 미치는 영향을 파악하고자 합니다.

- **Technical Details**: 연구팀은 사용자-아이템 상호작용의 도메인 내(in-domain) 및 도메인 외(out-of-domain) 데이터를 포함한 혼합 유출 데이터셋을 구축하여, 지저분한 LLM(dirty LLM)으로 미세 조정하였습니다. Low-Rank Adaptation (LoRA) 기법을 통해 훈련된 LLM을 기반으로 하여 추천 성능에 대한 영향력을 평가했습니다. 모델 아키텍처, 유출 구성, 외부 데이터 출처 등의 다양한 변수를 분석하였습니다.

- **Performance Highlights**: 실험 결과, 인-domain 유출은 모델 성능을 인위적으로 증가시키는 반면, out-of-domain 유출은 추천 정확도를 낮추는 경향이 있음을 확인했습니다. 이러한 복합적인 효과는 LLM 기반 추천 시스템의 평가 방식에 도전 과제가 될 수 있음을 시사합니다. 연구자들은 이상적인 성능 평가와 진정한 사용자 선호를 이해하기 위해 데이터 유출의 영향을 고려해야 한다고 강조합니다.



### Interpretable clustering via optimal multiway-split decision trees (https://arxiv.org/abs/2602.13586)
- **What's New**: 이 논문에서는 기존의 클러스터링 방법에 대한 새로운 접근 방식을 제안합니다. 기존의 모델들이 이진 결정 트리(binary decision trees)를 사용하면서 발생하는 높은 계산 비용 및 해석의 어려움을 해결하기 위해, 최적의 다중 분할 결정 트리(optimal multiway-split decision trees)를 기반으로 한 클러스터링 방법을 개발했습니다. 이 방법은 0-1 정수선형 최적화 문제로 공식화되어 기존 방법보다 더 효율적으로 최적화할 수 있습니다.

- **Technical Details**: 제안된 방법은 연속 변수를 이산화하기 위해 1차원 K-평균 알고리즘을 통합하여 데이터 기반의 유연한 분기를 허용합니다. 이를 통해 각 클러스터가 표현하는 구조를 명확하게 설명할 수 있는 다중 분할 결정 트리를 생성합니다. 이 과정에서 클러스터 내부의 응집력과 클러스터 간의 분리도를 목표로 포함하며, 다양한 평가 지표가 유연하게 적용될 수 있습니다.

- **Performance Highlights**: 공개된 실제 데이터셋에 대한 광범위한 수치 실험 결과, 제안된 방법이 기존의 방법들(ICOT, CART-Hybrid, PCT)보다 클러스터링 정확도 및 해석 능력에서 우수하다는 것을 확인했습니다. 이로 인해, 제안된 방법은 깊이 있는 해석 가능성을 가지고 있으면서도 경쟁력 있는 성능을 유지합니다. 따라서 이 방법은 클러스터링 분야의 발전에 기여할 것으로 기대됩니다.



### Scenario-Adaptive MU-MIMO OFDM Semantic Communication With Asymmetric Neural Network (https://arxiv.org/abs/2602.13557)
- **What's New**: 본 논문에서는 다중 사용자 환경에서의 의미론적 통신(Semantic Communication, SemCom)을 위한 새로운 프레임워크를 제안합니다. 이 프레임워크는 다중 사용자 다중 입출력(MU-MIMO) 시스템에 특화된 비대칭 구조를 포함하고 있으며, 이를 통해 사용자의 전반적인 효율성을 개선하려고 합니다. 또한, 기존 깊은 결합 소스-채널 코딩(Deep Joint Source-Channel Coding, DJSCC) 방식이 다중 사용자 시나리오에서 성능이 포화되는 문제를 해결하려고 합니다.

- **Technical Details**: 제안된 시스템 모델은 다중 사용자 MIMO(MU-MIMO) 환경을 기반으로 하며, 베이스 스테이션(BS)은 Nm개의 송신 안테나를 사용하여 Nk개의 사용자 장비(UE)에 서비스를 제공합니다. 논문에서는 OFDM 자원 격자를 포함한 모델을 통해 4차원 텐서 형태로 채널 계수를 나타냅니다. 또한, 차세대 신경망 아키텍처를 사용하여 사용자와 채널 상태 정보를 적절히 반영한 다중 사용자 환경에서의 송신 효과성을 극대화하도록 설계되었습니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 프레임워크가 기존 DJSCC 및 별도 소스-채널 코딩(Separate Source-Channel Coding, SSCC) 방식에 비해 피크 신호 대 잡음 비율(PSNR) 및 분류 정확도에서 월등한 성능을 보임을 확인했습니다. 특히 낮은 신호 대 잡음 비율(Signal-to-Noise Ratio, SNR) 환경에서도 낮은 지연 시간 및 연산 비용을 유지하면서 효과적인 성능 향상을 나타냈습니다. 이를 통해 제안된 방식이 실질적이고 복잡한 멀티 사용자 통신 환경에서도 우수한 성과를 낼 수 있음을 입증했습니다.



### Out-of-Support Generalisation via Weight Space Sequence Modelling (https://arxiv.org/abs/2602.13550)
Comments:
          8 pages, 2 figures, 1 table, 1 algorithm

- **What's New**: 이번 논문은 out-of-support (OoS) 일반화 문제를 해결하는 새로운 WeightCaster 프레임워크를 소개합니다. 이 프레임워크는 훈련 세트를 동심원으로 나누고, 각 원을 시간 단계로 매핑하여 시퀀스 모델링 문제로 재구성합니다. 이를 통해 명시적인 유도 편향 없이도 해석 가능하고 불확실성을 고려한 예측이 가능합니다.

- **Technical Details**: WeightCaster는 입력 도메인을 메트릭 공간으로 정의하고, 앵커 포인트를 선택하여 hyperspherical shells(하이퍼 구형 쉘)로 나누는 도메인 분해를 수행합니다. 이후 각 쉘의 데이터 포인트에 대한 가중치를 예측하는 시퀀스 모델을 학습합니다. 이 과정에서 훈련 데이터와 테스트 데이터 모두에 대한 불확실성 추정이 가능하도록 확률적 프레임워크를 도입합니다.

- **Performance Highlights**: 실험 결과, WeightCaster는 합성 코사인 데이터셋 및 실제 공기질 센서 데이터에서 뛰어난 성능을 보이며, 기존의 최첨단 기법과 비교하여 경쟁력 있는 또는 우수한 결과를 제공합니다. 이로 인해 안전-critical 응용 분야에서 인공지능의 신뢰성 강화에 기여할 수 있는 중요한 의미를 지니고 있습니다.
...
- **What's New**: 이 논문에서는 차원 축소(dimension reduction) 기법으로서 곱셈이 없는 요소 선택(element selection)을 위한 빠른 알고리즘을 제안합니다. 기존의 주성분 분석(PCA)과 같은 방법들은 많은 곱셈을 필요로 하여 리소스가 제한된 시스템에서 병목 현상을 초래할 수 있습니다. 반면, 요소 선택은 필요한 요소를 직접 선택하는 방식으로 계산 비용을 크게 줄입니다. 특히 이 알고리즘은 선형 회귀(linear regression)의 최소 평균 제곱 오차(mean squared error)를 통해 최적의 요소 집합을 평가합니다.

- **Technical Details**: 제안된 방법은 NN 차원의 데이터를 KK 차원으로 축소하기 위해 KK 개의 요소를 선택하는 접근 방식입니다. NK 는 1000 차원에서 100 차원으로 축소할 때 10^5 개의 곱셈을 수행해야 하지만, 요소 선택은 이러한 곱셈 없이 메모리 접근으로만 이루어집니다. 선택된 요소 집합의 품질은 회귀 분석을 통해 측정되며, 목표(target) 벡터와의 최소 제곱 오차를 미니마이즈(mimize)하여 최적의 요소 집합을 찾습니다.

- **Performance Highlights**: MNIST 손글씨 이미지 데이터세트를 이용한 실험 결과, 제안된 방법이 효과적임을 입증하였습니다. 요소 선택 방식은 특히 오디오 머신 러닝 시스템과 같은 저복잡도의 응용 분야에서도 좋은 성능을 보입니다. 최적화 과정에서 매트릭스 역학을 활용한 빠른 스왑 기반(local search) 최적화 알고리즘을 통해 계산량을 줄여 실시간 적용 가능성을 높였습니다.



### QuaRK: A Quantum Reservoir Kernel for Time Series Learning (https://arxiv.org/abs/2602.13531)
- **What's New**: 이 논문에서는 QuaRK라는 새로운 프레임워크를 소개합니다. 이 프레임워크는 하드웨어에 적합한 양자 리저버(quantum reservoir) 기능화 기법과 커널 기반의 리드아웃(readout) 방식이 결합되어 있습니다. 양자 리저버가 점들을 연속적으로 주입하여 효율적으로 측정된 k-로컬 관측값을 통해 집약된 특징 벡터(feature vector)를 생성합니다.

- **Technical Details**: QuaRK의 작동 방식은 classical shadow tomography 방식으로 측정된 관측값을 바탕으로 하여, 클래식한 커널 기반 리드아웃이 명시적인 정규화 및 빠른 최적화를 통해 목표 매핑(target mapping)을 학습하는 것입니다. 이 과정에서 회로의 폭(circuit width)과 깊이(depth), 측정 예산(measurement budget) 등 명확한 계산 조절 수단이 제공됩니다. 커널 방법의 유연성을 유지하면서 비선형 시계열 함수(temporal functionals) 모델링이 가능하고, 고차원 데이터에도 확장이 가능합니다.

- **Performance Highlights**: QuaRK의 성능을 검증하기 위해 실험을 수행하였으며, 합성 베타 혼합(beta-mixing) 시계열 작업에서의 예측된 보간(interpolation) 및 일반화(generalization) 행동을 시각적으로 보여주었습니다. 이 연구는 시계열 데이터의 의존성을 고려한 학습 이론적인 일반화 보장도 제공하며, 설계 및 자원 선택이 유한 샘플 성능(finite-sample performance)과 어떻게 연결되는지에 대한 원칙적 지침을 제시합니다.
...
- **What's New**: 이번 연구에서는 비선형 동적 시스템에서 고정된 고객 모델을 가진 클라이언트 간의 시간적 의존성을 학습하기 위한 연합 학습(federated learning) 프레임워크를 제안합니다. 중앙 서버는 의사소통된 잠재 상태를 기반으로 Graph Attention Network(GAT)를 사용하여 클라이언트 간의 동적 상호작용을 학습합니다. 이 접근법은 클라이언트 간 상호작용을 해석하기 위해 Jacobian-주의 분석을 포함하여, 분산 비선형 시스템에서 최초의 해석 가능한 특성을 제공합니다.

- **Technical Details**: 제안된 프레임워크에서는 각 클라이언트가 고차원 지역 관측값을 비선형 상태 공간 모델을 사용하여 저차원 잠재 상태로 매핑합니다. 그 후, 중앙 서버는 의사소통된 잠재 상태를 입력으로 받아 GAT를 통해 상태 전이 모델을 학습합니다. 또한, 주의 계수와 Jacobian 블록 간의 분석적 관계를 도출하여 학습된 클라이언트 간의 의존성을 원칙적으로 해석할 수 있도록 합니다.

- **Performance Highlights**: 본 연구는 강제 시스템에서 제안된 프레임워크의 해석 가능성, 수렴성, 확장성 및 프라이버시-유틸리티 균형을 제어한 합성 실험을 통해 검증하였습니다. 실제 산업 데이터셋에 대한 벤치마크를 통해 분산 기본값과 비슷한 성능을 나타내어 제안된 방식의 유효성을 확인했습니다.
...
### Text Has Curvatur (https://arxiv.org/abs/2602.13418)
- **What's New**: 이 논문은 텍스트가 내재적인 곡률의 특징을 가지고 있다는 주장을 바탕으로 하며, 이를 탐지하고 정의할 수 있는 방법을 제안합니다. 특히, Texture라는 텍스트 고유의 단어 수준 곡률 신호를 도입하여 언어의 의미 구성을 직관적으로 측정할 수 있도록 합니다. 이는 기존의 비유클리드 기하학적 접근 방식과는 달리, 텍스트 자체의 곡률을 정의하고 활용할 수 있는 새로운 패러다임을 제시합니다.

- **Technical Details**: 텍스트의 곡률을 정의하기 위해, 논문은 먼저 왼쪽과 오른쪽 문맥의 신념을 비교하여 Schrödinger bridge를 통해 곡률 필드를 생성합니다. 이 곡률 필드는 문맥이 의미를 집중시키는 곳에서는 양수(positive), 경쟁적인 대안으로 팬아웃(fan-out)되는 곳에서는 음수(negative)로 정의됩니다. 또한, 두 가지 고전적인 플랫니스(null hypotheses) 조건을 사용하여 자연 텍스트의 두 가지 증명으로 비평면 구조를 확인하고, 이를 통해 텍스트의 곡률을 성공적으로 측정합니다.

- **Performance Highlights**: Texture는 인퍼런스 타임에서 리소스를 효과적으로 할당하고 문맥을 가이드하는 데 활용되는 제어 신호로 작동합니다. 연구진은 CurvPrune과 CurvFlag라는 두 가지 유틸리티를 도입하여 고곡률 영역에 프롬프트 예산을 할당하고, 지역 문맥이 부족한 경우에는 검색을 트리거합니다. 이러한 방법들은 굽어진 구조를 측정함으로써 기존의 LLM 훈련을 대체할 수 있는 실용적인 방법론을 제공합니다.
...
- **What's New**: 최근 기후 모델링 분야에서 LUCIE라는 데이터 기반 기후 지표 예측 모델이 소개되었습니다. 이 모델은 물리적으로 일관된 경량화된 기후 에뮬레이터로, 고해상도 예측을 제공하기 위해 '수퍼 해상도' 프레임워크를 도입하였습니다. 이 연구는 LUCIE의 낮은 해상도 예측을 25km 해상도로 변환해 지역 기후 연구에 필요한 더 자세한 데이터를 생성하는 방법론을 제안합니다.

- **Technical Details**: LUCIE 모델은 Spherical Fourier Neural Operator (SFNO) 아키텍처를 기반으로 하며, 이는 기후 통계치인 기후 평균과 계절 변동성을 정확히 재현할 수 있는 능력을 가지고 있습니다. 그러나 이 모델의 원래 해상도는 약 300km로 지역적 영향 평가에는 부족합니다. 이를 해결하기 위해, 본 연구에서는 조건부 및 포스터리어 샘플링 프레임워크를 활용한 확률적 확산 기반 생성 모델을 결합하여 LUCIE 출력을 고해상도로 다운스케일할 수 있는 딥러닝 기반 프레임워크를 개발했습니다.

- **Performance Highlights**: 제안된 방법은 LUCIE로부터의 조잡한 동적 특성을 보존하면서도 약 28km 해상도로 세밀한 기후 통계치를 생성하는 능력을 보여주었습니다. 실제 데이터에 대한 엄격한 검증을 통해 충분한 성능을 입증하였고, 다양한 품질 측정을 사용하여 방법의 효과를 평가하였습니다. 이 연구는 지역 기후 계획 및 적응 노력을 위한 기초 데이터를 제공함으로써 기후 모델링의 새로운 가능성을 제시합니다.



### Why is Normalization Preferred? A Worst-Case Complexity Theory for Stochastically Preconditioned SGD under Heavy-Tailed Nois (https://arxiv.org/abs/2602.13413)
- **What's New**: 본 논문에서는 heavy-tailed noise 환경 하에서 stochastic preconditioned stochastic gradient descent (SPSGD)와 그 가속화 변형의 최악의 경우 복잡도 이론을 개발하였습니다. 널리 사용되는 적응 방법들인 Adam, RMSProp, Shampoo를 포함하는 설정에서의 수렴성을 분석하였으며, 이러한 분석이 SGD 훈련의 안정성을 높이는 새로운 방법론을 제시합니다.

- **Technical Details**: 우리는 stochastic gradient noise가 유한한 $p$-th moment를 가지며, 어떤 $p 	extin{ }in (1,2]$일 때, $T$ 반복 후의 수렴을 측정합니다. 이 연구에서 clipping과 normalization은 heavy-tailed noise 하에서 SGD 훈련 안정성을 위한 병행 도구로 작용하는데, stochastically preconditioned 설정에서는 그 성능의 차이가 있음을 보여주었습니다. 특히, normalization은 첫 번째 순서 stationary point에 대한 수렴 속도를 보장합니다.

- **Performance Highlights**: 정확한 문제 매개변수가 알려진 경우 수렴 속도는 $	ext{O}(T^{-rac{p-1}{3p-2}})$이며, 알려지지 않은 경우는 $	ext{O}(T^{-rac{p-1}{2p}})$입니다. 이 결과는 normalized SGD에 대한 최적의 속도에 해당하며, clipping은 통계적 의존성으로 인해 최악의 경우 수렴에 실패할 수 있음을 입증하였습니다. 이러한 결과들은 대규모 모델 훈련에서 normalization이 clipping보다 선호되는 이유에 대한 이론적 설명을 제공합니다.
...
- **What's New**: 이 논문에서는 높은 농도와 세포 생존율을 동시에 달성할 수 있는 cryoprotectant agent (CPA) 칵테일을 효율적으로 설계하기 위한 데이터 효율적 접근 방식을 제시합니다. 기존의 실험방법에서는 전문가의 직관이나 반복적인 실험이 필요했으나, 본 연구는 다목적 Bayesian 최적화를 기반으로 한 활성 학습(Active Learning) 루프를 통해 이 과정을 가속화합니다. 이 방법을 통해, CPA의 농도와 생존율 사이의 균형을 효율적으로 탐색하고, 높은 품질의 솔루션을 빠르게 발견할 수 있습니다.

- **Technical Details**: 제안된 프레임워크는 CPA 칵테일의 최적화를 위해 고속 스크리닝(high-throughput screening) 기법과 다목적 Bayesian 최적화를 통합합니다. 주어진 초기 세트로부터 확률적 대리 모델(probabilistic surrogate models)을 학습하여 각 조합의 농도와 생존율을 예측하고, 확인할 수 있는 불확실성을 정량화합니다. 이후 이 모델을 사용하는 활성 학습 루프가 새로운 실험을 선택하며, 높은 Pareto 개선을 최대화하여 실험 절차의 효율성을 높입니다.

- **Performance Highlights**: 본 연구에서 실험적으로 확인된 결과에 따르면, 제안된 접근 방식은 무작위 샘플링(random sampling) 및 다른 기준선에 비해 우수한 성능을 보여줍니다. CPA 농도와 세포 생존율의 Pareto-optimal 솔루션을 찾는 데 필요한 실험 수를 크게 줄이면서도, 9.5%와 4.5%의 개선을 이끌어냈습니다. 또한, 기존의 최첨단 방법에 비해 약 30%의 평가 수만으로도 비교 가능한 성능을 얻을 수 있었고, 이로 인해 약 10주에 달하는 실험 시간이 절약되었습니다.



### The Speed-up Factor: A Quantitative Multi-Iteration Active Learning Performance Metric (https://arxiv.org/abs/2602.13359)
- **What's New**: 이 연구는 Active Learning (AL)의 반복적인 평가 과정에 대해 적합한 성능 메트릭이 부족하다는 점을 지적하였습니다. 특히, 다양한 쿼리 방법(Question Methods, QMs)의 성과를 비교하기 위한 새로운 메트릭인 speed-up factor를 도입하여 AL 성능을 정량적으로 평가할 수 있는 틀을 마련했습니다. 이 속성은 무작위 샘플링 성과에 도달하기 위해 필요한 샘플의 비율을 나타냅니다.

- **Technical Details**: 논문은 향후 AL 평가를 위한 실험 설정을 정리하고, 8년간의 AL 평가 문헌을 포괄적으로 검토하였습니다. speed-up factor의 도입을 통해 4개의 다양한 데이터셋과 7개의 쿼리 방법을 이용해 실제 실험을 수행하였으며, 이 결과를 기존의 AL 성능 메트릭과 비교하여 그 안정성을 입증했습니다. AL의 핵심 목표는 최적화된 쿼리 선정으로 데이터 주석 비용을 최소화하여 학습 모델의 성능을 극대화하는 것입니다.

- **Performance Highlights**: 실험 결과, speed-up factor는 무작위 샘플링 성과를 reproducing하는 데 필요한 최소 샘플을 효과적으로 포착하며, 여러 반복(iterations)에서 뛰어난 안정성을 보여주었습니다. 기존의 AL 메트릭보다 더 많은 정보와 신뢰성을 제공하여, AL 연구에 실질적으로 기여할 수 있음을 확인하였습니다. 속도 향상 요소는 AL 연구가 나아가야 할 방향에 대한 새로운 통찰력을 제공할 것으로 기대됩니다.
...
Comments:
          20 pages, 13 figures, 4 tables. Submitted to AAS journals. Comments welcome

- **What's New**: 이 연구는 LAMOST 저해상도 스펙트럼(LRS)에서 DESI 중해상도 스펙트럼(MRS)으로의 이전을 통해 별 스펙트럼 분석에서의 교차 조사 일반화 문제를 조사합니다. 연구팀은 간단한 다층 퍼셉트론(MLP) 모델을 사용하여 사전 학습한 MLP를 LAMOST LRS에 대해 학습하고 이를 DESI 별 스펙트럼에 적용하여 조정합니다. 특히, 이 논문은 LRS와 DESI 스펙트럼 간의 차이를 직접적으로 학습하는 방법을 중점적으로 다룹니다.

- **Technical Details**: MLP 모델은 스펙트럼 정보를 별 매개변수로 매핑하도록 설계되었습니다. 입력으로는 연속 정규화된 스펙트럼 또는 스펙트럼 임베딩을 사용하여, 400-560 nm 범위의 1,462 로그 간격의 파장 구간을 제한합니다. MLP 모델은 여러 개의 완전 연결층과 ReLU 활성화를 포함하고 있으며, 최종 출력층에서 목표 매개변수를 예측합니다. 모델 아키텍처는 입력 차원에 따라 [input_dim,1024,512,64,1]로 설계되며, LRS 플럭스에 직접 적용할 경우 약 206만 개의 가중치 파라미터를 가집니다.

- **Performance Highlights**: 실험 결과, LAMOST LRS에서 사전 학습된 MLP는 미세 조정 없이도 강력한 성능을 보이며, DESI 스펙트럼으로의 약간의 미세 조정이 결과를 더욱 개선합니다. 금속 농도 추정 ([Fe/H])의 경우, 트랜스포머 기반 모델의 임베딩이 금속이 풍부한 영역에서 유리하지만, 금속이 부족한 영역에서는 MLP에서 직접 학습된 성능이 더 우수합니다. 이는 별 매개변수의 이동을 위한 최적의 미세 조정 전략이 특정 별 매개변수에 따라 달라짐을 보여줍니다.
...
- **What's New**: 이 논문에서는 로봇이 과거 관측 값을 기억해야 하는 여러 작업에서 발생하는 한계를 다루고 있습니다. 기존의 로봇 정책들이 현재 관측치에만 의존하는 경향이 있으며, 이로 인해 실제 상황에서의 적용 가능성이 제한된다고 주장합니다. 연구자들은 이러한 문제의 원인이 훈련 시 과거 기록에 대한 제한된 커버리지에 있으며, 이에 대한 해결책으로 Big Picture Policies (BPP)를 제안합니다.

- **Technical Details**: BPP는 비전-언어 모델에 의해 감지된 최소한의 의미 있는 키프레임에 기초해 조건화되는 접근 방법입니다. 이 방법은 다양한 롤아웃을 작업 관련 사건의 압축된 집합에 투영하여 훈련과 배포 간의 분포 변화를 상당히 줄이는 데 성공했습니다. 정책 아키텍처는 표준 ResNet34 이미지 인코더를 사용하며, 각 카메라 뷰에 대해 다른 가중치를 설정하였습니다.

- **Performance Highlights**: BPP는 4가지 실제 조작 작업과 3가지 시뮬레이션 작업에서 평가되었고, 각각의 작업은 역사 조건화가 필요합니다. 실제 평가에서 BPP는 최상의 비교 결과보다 70% 높은 성공률을 기록하며, 이 방식이 기존 기술보다 효과적임을 보여줍니다. 이 논문은 로봇의 실제 조작 성과를 향상시키기 위한 새로운 접근 방식을 제시하고 있습니다.
...
- **What's New**: 이 논문에서는 Quantum Gaussian Processes (QGPs)의 성능을 향상시키기 위한 Distributed Quantum Gaussian Process (DQGP) 방법을 제안합니다. DQGP는 다중 에이전트 환경에서 모델링 능력과 확장성을 증가시키기 위해 고안된 방법으로, 데이터를 복잡한 고차원 힐베르트 공간에 임베딩하여 기존의 제약을 극복하려고 합니다. 또한, 논문에서는 DR-ADMM 알고리즘을 사용해 지역 에이전트 모델을 글로벌 모델로 통합하는 방법을 다룹니다.

- **Technical Details**: 본 연구에서는 Gaussian Process(GP) 모델의 훈련 및 분포를 다중 에이전트를 통해 수행하고, 이를 위한 비유클리드 최적화 문제를 해결하기 위해 Distributed Riemannian Alternating Direction Method of Multipliers (DR-ADMM) 알고리즘을 개발했습니다. 또한, 진화하는 양자 컴퓨팅 기술을 활용하여 양자 커널을 이용한 GP 모델을 설계하였습니다. 이러한 접근 방식을 통해 모델의 표현력과 연산 효율성을 동시에 확보하고자 합니다.

- **Performance Highlights**: 제안된 DQGP 방법은 NASA의 Shuttle Radar Topography Mission의 비정상적 고도 데이터 세트 및 양자 GP가 생성한 합성 데이터 세트를 사용하여 평가되었습니다. 초기 실험 결과, 기존의 고전적인 방법들에 비해 DGGP가 향상된 성능을 나타내었으며, 양자 하드웨어에서의 연산 속도 향상 가능성도 제시하고 있습니다.
...
- **What's New**: 이번 논문에서는 DMTS-NC 접근법을 제안하며, 비보존력(non conservative forces)을 활용하여 원자 분자 동역학 시뮬레이션의 속도를 더욱 증가시킵니다. 기존의 DMTS 모델을 발전시켜, 간소화된 모델을 통해 비보존력을 생성하고 특히 높은 정확도를 유지함과 동시에 시뮬레이션의 안정성을 제공하는 것이 특징입니다. 또한, 이 방법은 추가적인 파인튜닝 없이 쉽게 구현할 수 있으며, 모든 신경망 포텐셜(neural network potential)에 적용 가능합니다.

- **Technical Details**: DMTS-NC는 두 단계의 역방향 참조 시스템 전파기 알고리즘(RESPA)을 사용하여 정확한 보존적 잠재력(conservative potential)을 간소화된 표현과 결합합니다. 이 과정에서 물리적 사전 정보인 회전에 대한 등가성(equivariance)과 원자력(component) 요소의 상쇄를 강제하는 설계를 통해 안정성을 보장합니다. 모델의 성능 향상을 위해 수소 질량 재분배(Hydrogen Mass Repartitioning)와 같은 안정화 기법이 사용됩니다.

- **Performance Highlights**: DMTS-NC는 기존의 비보존 모델과 비교하여 15-30% 더 빠른 계산 속도를 기록했습니다. 평균 절대 오차(mean absolute error)가 1 kcal/mol/Å에 가까운 성능을 보여주어, 다양한 분자 시스템에 대해 더욱 정밀하고 안정적인 시뮬레이션이 가능하게 되었습니다. 이 방법은 최적화된 내부 단계에서의 계산 비용을 줄여, 더 긴 시간 스텝을 통해 시뮬레이션의 속도와 강인성을 개선합니다.



### Gradient Networks for Universal Magnetic Modeling of Synchronous Machines (https://arxiv.org/abs/2602.14947)
- **What's New**: 이번 논문에서는 포화 가능한 동기 기계의 동적 모델링을 위한 물리 기반(physics-informed) 신경망 접근법을 소개합니다. 이 아키텍처는 기본 기계 방정식에 직접적으로 gradient networks(기울기 네트워크)를 통합하여 비선형 및 결합된 전자기 응집 관계를 정확하게 모델링합니다. 자기장 에너지의 기울기를 학습함으로써, 모델은 에너지 균형(energy balance)을 자연스럽게 만족합니다.

- **Technical Details**: 이 논문은 고전적인 기계 모델링의 한계를 해결하기 위해 포화되는 동기 기계의 전자기영역을 그라디언트 네트워크와 결합하는 물리 정보 기반 모델링 프레임워크를 제안합니다. 스테이터 전류와 전자기 토크는 스칼라 필드 에너지 함수의 기울기로 직접적으로 모델링되며, 이는 물리적 일관성을 보장합니다. 또한 몬토네 기울기 네트워크를 사용하여 필드 에너지가 볼록(convex)한지 확인하고, 이를 통해 플럭스 연결과 전류 간의 유일하고 가역적인 관계를 마련하였습니다.

- **Performance Highlights**: 제안된 접근법은 5.6 kW 영구자석(PM) 동기 저항 기계의 측정 및 유한요소법(FEM) 데이터셋을 사용하여 검증되었습니다. 결과적으로 제한된 훈련 데이터로도 정확하고 물리적으로 일관된 모델이 생성됨을 입증하였으며, 이는 제어 응용 분야에서 필요한 강력한 모델 역전 및 최적 경로 생성에 기여합니다.



### Fault Detection in Electrical Distribution System using Autoencoders (https://arxiv.org/abs/2602.14939)
- **What's New**: 본 논문은 전기 파워 시스템에서의 고장 탐지(anomaly detection) 방법으로 딥 오토인코더(deep autoencoder)를 도입합니다. 기존 방법들이 줄어든 데이터로 인해 실용화에 어려움을 겪고 있는 반면, 제안된 방법은 적은 매개변수로 빠른 훈련 시간을 자랑합니다. 또한, 다양한 고장 전류 및 전압의 탐지와 분류를 통해 높은 정확도를 달성했습니다.

- **Technical Details**: 제안된 모델은 Convolutional Autoencoders (CAE)를 기반으로 하여 특성 차원 감소를 수행하며, 입력 데이터의 재구성을 통해 고장을 탐지합니다. 훈련 과정에서는 정상 시계열 데이터(normal time-series data)로 모델을 설정하고, 테스트 동안 허용된 재구성 오차를 초과하는 경우 해당 신호를 고장으로 간주합니다. 이 방식은 특정 고장이 발생한 신호의 동적 특성을 신속하게 분류하고자 하는 요구에 적합합니다.

- **Performance Highlights**: 이 연구에서 제안된 방법은 시뮬레이션 및 공개 데이터셋을 사용하여 각각 97.62%와 99.92%의 정확도로 고장을 탐지하였습니다. 이는 전통적인 머신 러닝 모델과 비교해 많은 개선을 이룬 결과로, 신뢰성 있는 고장 탐지 및 분류가 가능함을 보여줍니다. 이러한 성능은 효율적인 전기 배전 시스템(EDS)의 운영에 중요한 기여를 할 것으로 기대됩니다.



### Activation-Space Uncertainty Quantification for Pretrained Networks (https://arxiv.org/abs/2602.14934)
- **What's New**: 본 논문에서는 사후 (post-hoc) 방법론으로서 Gaussian Process Activations (GAPA)를 도입합니다. 이는 베이지안 모델링을 가중치에서 활성화(activations)로 전환하여 변형된 예측을 보존하면서도 예측 불확실성을 측정할 수 있게 합니다. 특히 GAPA는 사전 훈련된 모델의 Frozen Backbone의 포인트 예측을 유지하며, 폐쇄형 에피스틱 변동성을 활성화 공간에서 제공합니다.

- **Technical Details**: GAPA는 훈련 시 활성화 데이터를 캐시(cached)하여 희소 변동 유도점(sparse variational inducing-point) 근사를 활용합니다. 이를 통해 k-최근접이웃(k-nearest-neighbor) 서브셋 조정을 결합하여 샘플링, 역전파(backpropagation), 또는 2차 정보가 없는 결정론적 단일 패스 불확실성 전파가 가능합니다. 이 모듈은 다양한 현대 신경망 구조에 쉽게 통합될 수 있습니다.

- **Performance Highlights**: GAPA의 성능은 회귀 (regression), 분류 (classification), 이미지 분할 (image segmentation), 언어 모델링 (language modeling) 등의 태스크에서 강력한 사후 기준 (post-hoc baselines)를 초월하거나 대등한 결과를 보입니다. 또한, GAPA는 효율적인 테스트 시간을 유지하면서 정확한 보정(calibration) 및 OOD(out-of-distribution) 탐지 성능을 향상시키는 데 기여합니다.
...
- **What's New**: 이 논문은 Prometheus 프레임워크를 확장하여 2D 고전 시스템에서 3D 고전 및 양자 다체 시스템으로의 비지도 학습적인 상전이 발견을 다룹니다. 이 연구는 고차원에서의 확장성과 양자 플럭츄에이션(quantum fluctuations)과의 일반화 문제를 해결합니다. 3D Ising 모델에서의 실험 결과는 문헌 값과 0.01% 이내로 일치하는 비판적 온도를 확인했으며, 이는 VAE 접근법을 통한 비지도 학습이 효과적임을 입증합니다.

- **Technical Details**: 고전 시스템에서는 3D 컨볼루션 VAE 아키텍처를 개발하였고, 이는 최대 32^3 격자에서 부피(spin configurations)를 처리합니다. 비판적 온도는 문헌 값보다 오차가 0.01% 이내인 결과를 보여 주며, 비판적 지수도 평균 70% 이상의 정확도로 추출되었습니다. 양자 시스템에 대해서는 Q-VAE 아키텍처를 개발하여 복소수 파동함수를 사용하고, 정확도 2%로 양자 비판적 점을 탐지하였습니다.

- **Performance Highlights**: 이 연구는 고전 및 양자 시스템에서 비지도 학습을 통해 다양한 비판적 행동을 식별할 수 있는 능력을 보여줍니다. 3D 결과는 온도 정확도가 0.01%에 평행하고, 지수 정확도는 70-75%를 기록했습니다. 양자 결과는 복잡한 동적 스케일링을 자동으로 인식하고, 이는 비지도 학습이 전이점 발견 이상으로 비판적 행동의 질적으로 다른 유형을 식별할 수 있음을 입증합니다.
...
- **What's New**: 본 연구는 해양 구조물의 형상 최적화에 있어 기존의 adjoint 기반 방법을 개선하기 위해 기계 학습(ML) 지원 최적화 프레임워크를 제안합니다. 이 프레임워크는 Voith Schneider Propeller(VSP)의 효과를 반영하기 위해 Conditional Variational Autoencoder(CVAE) 기반의 서그레이트 모델을 활용하여, 수치적 저항을 최소화하는 디자인을 도출하여 8% 이상의 저항 감소를 달성합니다.

- **Technical Details**: 제안된 방법은 시간에 따라 변화하는 흐름을 정밀하게 모사하는 대신, VSP로 인해 유도된 시간 평균 흐름 필드를 재현하는 ML 모델을 학습하여 진행됩니다. 특히, 11개의 데이터 레이블을 설정하여 Cruise speed와 같은 운전 매개변수를 포함, 다양한 기하학적 구성을 처리할 수 있도록 모델을 조정합니다. 이는 기존의 adjoint 최적화 과정에 nonlinear한 유속을 도입하여 강력한 결합 처리를 가능하게 하고, 메쉬의 유연성을 높입니다.

- **Performance Highlights**: 제안한 ML 기반 모델은 하이퍼파라미터 최적화와 관련하여 효과적으로 연산 비용을 절감하면서도 높은 정확도를 유지하는 것으로 입증되었습니다. 실험 결과, VSP의 영향을 정확히 반영한 최적 디자인은 초기 설계보다 훨씬 높은 성능을 보이는 것으로 나타났습니다. 이는 조정된 메타 그리드를 통해 ML 기반 유속이 CFD 메쉬로 잘 전이되기 때문입니다.
...
- **What's New**: 이번 연구에서는 비대칭 연결성을 허용하여 발전적 드리프트와 확산을 표현할 수 있는 연속 지연 리커런트 신경망(RNN)을 제안합니다. 연구진은 이러한 네트워크가 비선형 및 비평형적인 동역학을 포괄할 수 있음을 검증하며, 이를 통해 RNN이 카오틱 어트랙터와 같은 복잡한 특성을 표현할 수 있는 가능성을 열었습니다. 이로 인해, 신경 계산의 메커니즘을 이해하는 데 중요한 발전을 이루게 되었습니다.

- **Technical Details**: 연구진은 드리프트-확산 매칭(Drift-Diffusion Matching) 프레임워크를 통해 임의의 확률적 동역학 시스템(SDE)을 저차원의 잠재 서브스페이스로 표현하는 방법을 제시합니다. 이 RNN 모델은 비대칭 연결성을 통해, 입력 기반 전환 및 비평형 전류로 인해 자율적으로 전이되는 동작을 구현하고, 이러한 동작이 연상 및 순차적 기억 모델로 해석될 수 있다고 주장합니다. 이론적으로, 비대칭 연결성을 가진 신경 집단은 넓은 범위의 동적 시스템을 인코딩할 수 있는 능력을 가지고 있음을 보여줍니다.

- **Performance Highlights**: 연구 결과에 따르면, 대형 RNN이 비대칭 연결성을 통해 임의 동역학을 인코딩할 수 있으며, 이는 기억 회복 능력을 높일 수 있음을 시사합니다. 예를 들어, 에너지 잠재력의 기울기 또는 비평형 정지 확산을 통해 어트랙터 간의 자율적 순환이 가능하다는 점에서 연구는 RNN의 기능성을 확장합니다. 이러한 결과는 신경 과학과 비평형 통계 역학의 통합적인 이해를 도울 수 있는 기초를 제공합니다.
...
- **What's New**: 본 연구에서는 크리티컬 지역(critical regions)에서 고체의 동시 원자론적 시뮬레이션과 연속체 모델링을 결합한 새로운 하이브리드 방법을 제안합니다. 이 방법은 다양한 크기의 준 원자(quasi-atoms)로 구성된 복합 매체를 사용하여 상호작용 잠재력(interaction potential) 매개변수를 최적화하며, 전통적인 방법보다 계산 효율성이 뛰어납니다. 이를 통해 LAMMPS 같은 표준 소프트웨어 패키지를 보완하여 분자 동역학(molecular dynamics) 시뮬레이션의 정확도를 유지하면서도 계산 속도를 크게 개선하였습니다.

- **Technical Details**: 우리의 하이브리드 MD-메조스케일 접근법은 파이썬 최적화 프로그램을 통해 구현되었습니다. 크리티컬 지역의 경우 완전 원자적 설명을 사용하고, 나머지 지역은 '준 원자'로 구성하여 모델링을 진행합니다. 준 원자는 많은 원자를 포함하는 효과적인 입자로, 매질의 동적 분산 노드로 작용하며, 이를 통해 원자 수준의 정확도와 연속체 수준의 효율성을 동시에 달성할 수 있습니다.

- **Performance Highlights**: 제안된 방법을 사용하여 다양한 크기가 있는 입자의 충돌을 시뮬레이션한 결과, 새로운 하이브리드 접근법이 전체 원자 시뮬레이션과 비교해 정확성, 유효성, 그리고 압도적인 계산 속도의 우수성을 보였습니다. 특히, 다른 하이브리드 접근법인 AtC 방법과 비교했을 때, 우리의 방법이 계산 속도와 구현의 용이성 면에서 현저히 뛰어나는 것을 확인하였습니다.
...
- **What's New**: 이번 논문에서는 RF-GPT라는 새로운 라디오 주파수 언어 모델(RFLM)을 소개합니다. 이 모델은 멀티모달 LLM의 비주얼 인코더를 활용하여 RF 스펙트로그램을 처리하고 이해할 수 있도록 설계되었습니다. 기존의 LLM 기반 접근 방식이 텍스트 및 구조화된 데이터에 국한되어 있다면, RF-GPT는 RF 데이터와 고수준 추론 간의 간극을 메우는 것을 목표로 합니다.

- **Technical Details**: RF-GPT는 복잡한 In-phase/Quadrature (IQ) 파형을 시간-주파수 스펙트로그램으로 변환한 후, 이 데이터를 사전 훈련된 비주얼 인코더를 통해 인코딩합니다. 이 과정에서 생성된 RF 토큰은 단독 디코더 LLM에 주입되어 RF 기반의 응답, 설명 및 구조화된 출력을 생성합니다. 모델 훈련은 완전한 합성 RF 코퍼스를 기준으로 감독된 미세 조정을 수행하며, 다양한 무선 기술에 대한 넓은 대역 장면을 생성합니다.

- **Performance Highlights**: RF-GPT는 넓은 대역 변조 분류, 중첩 분석, 무선 기술 인식, WLAN 사용자 수 계산, 5G NR 정보 추출과 같은 기준에서 강력한 다중 작업 성능을 보여주었습니다. 반면 일반 목적의 비주얼-언어 모델(VLM)은 RF 기반 과제에서 대부분 실패하였습니다. 이로 인해 RF-GPT는 다양한 RF 작업을 다루는 데 있어 뛰어난 성능을 입증하였습니다.
...
- **What's New**: 이 연구에서는 단백질 서열(Protein Sequences)의 효과적인 표현이 기계 학습 기반의 단백질 설계에 있어 핵심 요소로 인식되고 있음을 강조합니다. 특히, 기존의 단백질 생체공학(Bioengineering)에서 서열 표현에 대한 체계적인 비교 연구가 부족하다는 점을 지적하며, 현재 연구에서는 ProtBERT와 ESM2 임베딩 변형 여러 가지를 평가합니다.

- **Technical Details**: 연구는 아데노 바이러스 캡시드(Adeno-associated Virus Capsid)를 사례로 하여, 서열 수준 표현(Sequence-level Representations)과 아미노산 수준 임베딩(Amino Acid-level Embeddings)의 성능을 비교합니다. 사전 조정(Fine-tuning) 이전에는 아미노산 임베딩이 우수한 예측 성능을 보였지만, 서열 수준 표현은 비지도 학습(Unsupervised Settings)에서는 더 효과적이라는 결과를 나타냅니다.

- **Performance Highlights**: 특히, 임베딩이 태스크 특정 레이블(Task-specific Labels)로 세밀하게 조정될 때 최적의 성능이 발휘됩니다. 연구 결과는 생체공학 연구에서 보통 탐색되는 것보다 서열 변동(Sequence Variation)의 범위가 더 커야 의미 있는 신호 변화가 발생한다는 점을 시사하며, 희소(mutations)가 분포하거나 국부화된 지역에서 세밀한 조정의 필요성을 강조합니다.
...
### SA-SSL-MOS: Self-supervised Learning MOS Prediction with Spectral Augmentation for Generalized Multi-Rate Speech Assessmen (https://arxiv.org/abs/2602.14785)
Comments:
          Accepted at ICASSP 2026

- **What's New**: 이 논문에서는 서로 다른 샘플링 주파수(16-48 kHz)의 다중 속도 음성 품질 평가(Multi-rate Speech Quality Assessment, SQA)를 위한 새로운 방법인 SA-SSL-MOS를 제안합니다. 특히 고주파 정보(High-frequency Information)를 포함하여 SSL(자기지도학습) 모델의 한계를 극복하려고 합니다. 또, 제한된 다중 속도 데이터셋을 잘 활용할 수 있도록 두 단계 학습 방식을 도입했습니다.

- **Technical Details**: SA-SSL-MOS는 두 개의 병렬 경로를 통해 음성을 처리합니다: 첫 번째 경로는 16 kHz 음성을 사용하여 SSL 기반의 특징을 추출하고, 두 번째 경로는 48 kHz로 업샘플링하여 스펙트로그램(Spectrogram) 특징을 추출합니다. 두 경로에서 얻은 벡터 표현을 합쳐서 최종 MOS 점수를 예측합니다. 이러한 접근 방식은 고주파 정보를 보존하며, MOS 예측을 위한 Gaussian posterior 모델링을 사용하고, Gaussian negative log-likelihood (GNLL) 손실을 최적화합니다.

- **Performance Highlights**: 제안된 SA-SSL-MOS는 색다른 외부 데이터셋에서 평가 시 더 나은 일반화 성능을 발휘하며, 다양한 샘플링 속도에서 훈련된 모델들은 각기 다른 조건에서도 효과적으로 작동합니다. 실험 결과, 특히 고주파 정보를 포함하는 것이 음성 품질 예측의 정확도를 높이는 데 중요하며, 제한된 데이터셋에서도 두 단계 학습 전략이 일반화 성능을 크게 향상시키는 것을 보여줍니다.
...
- **What's New**: 이 논문에서는 제어, 역 문제, 불확실성 정량화와 관련된 매개변수 의존 부분 미분 방정식(parameter-dependent partial differential equations)의 보간(interpolation)을 기반으로 한 저차 모델링(reduced-order modeling) 프레임워크를 개발합니다. 물리적 영역에서의 해는 유한 요소 방법(finite element methods)으로 이산화(discretized)되며, 매개변수에 대한 의존성은 별도로 근사화됩니다. 이 프레임워크의 강점은 저차 및 고차 매개변수 공간에서의 정확한 추정(error estimates)을 제공한다는 점입니다.

- **Technical Details**: 본 연구의 핵심은 2차원 또는 3차원 볼록 다각형(convex polygonal) 공간 영역에서의 슈뢰딩거 유형(PDE)을 다루며, Dirichlet 경계 조건에 따라 파라미터화 된 문제를 설정합니다. 연구자들은 저차 매개변수 공간에서는 고전적 보간이 대수적 수렴률을 제공하고, 고차 매개변수 공간의 경우 Extreme Learning Machine(ELM) 대리 모델을 사용하며 명시적인 근사 및 안정성 가정 하에 오차 경계를 도출합니다. 또한, 파라미터 공간에서는 텐서 곱(tensor product) 또는 신경망 기반의 보간을 활용합니다.

- **Performance Highlights**: 논문에서 제안하는 프레임워크는 정량적 광음향 단층 촬영(quantitative photoacoustic tomography) 문제에 적용되어, 잠재적(potential) 및 매개변수 재구성 오차 추정치를 도출하며, 표준 접근 방식에 비해 상당한 계산 절약을 보여줍니다. 특히 저차 및 고차 매개변수 공간 모두에서 일관된 FEM 기반 전략과 보간 접근법을 제시하여, 매개변수 간의 상호 작용을 정량화할 수 있는 엄격한 정규성 결과를 제공합니다. 따라서 이 연구는 역 문제 해결 시 용이성과 효과성을 극대화하는데 기여할 것으로 기대됩니다.
...
- **What's New**: 이번 연구에서는 양자 분류기의 성능을 분석하며, 특히 소음이 존재하는 상황에서의 지역 제한 측정(locality-constrained measurements) 하의 클래스 정보(class information)의 접근 가능성을 다루고 있다. 저자들은 이진 양자 분류(binary quantum classification)를 지역 제한 양자 상태 구별(constrained quantum state discrimination)로 공식화하고, 최대한의 편향(bias)이 달성될 수 있는 지역 제한 구별성(locality-restricted distinguishability) 측정 기준을 도입하였다. 나아가, $n$-큐비트 시스템에서 독립적인 탈상과 노이즈(depolarizing noise)에 의해 영향을 받는 실험 결과를 통해 이론 예측의 정확성을 입증하였다.

- **Technical Details**: 양자 기계 학습(Quantum Machine Learning, QML)을 위한 새로운 측정 기반 프레임워크를 제안하며, 신호 수평(Signal Horizon) 개념을 도입한다. 이 개념은 클래스 정보가 여전히 존재하더라도, k-지역(truncated k-local) 측정에 제한된 관측자는 이 정보를 접근할 수 없는 경계(noise–locality boundary)를 설정한다. 또한, Pauli 접근 가능 진폭(Pauli-accessible amplitude) $A_k(p)$를 정의하여, 독립적인 탈상 노이즈 하에서 최적의 $k$-지역 Pauli 측정으로 도달할 수 있는 최대 분류 정확도를 예측한다.

- **Performance Highlights**: 숫자적으로, 네 큐비트를 사용한 인코딩의 경우, 실험적 정확도(empirical accuracy)와 예측 간의 정량적 일치를 보여주었다. 특정 임계값에서 $k$-지역 분류기들이 무작위 추측(random guessing)과 구별되지 않게 되는 작업적인 붕괴(operational breakdown) 한계를 발견하였다. 이러한 결과는 NISQ(노이즈 간섭 중간 규모 양자) 환경에서 강력한 양자 기계 학습 시스템 설계를 위한 중요한 교훈을 제공한다.
...
- **What's New**: 본 연구에서는 양자 저수지 컴퓨터(QRCs)의 성능을 높이기 위해 최적의 측정 연산자(optimal measurement operators)를 찾는 것이 얼마나 중요한지를 강조한다. 저자는 커널 능선 회귀(kernel ridge regression) 프레임워크를 통해 상태 비의존(stateful) 학습 방식인 양자 극한 학습 기계(QELMs)와 메모리 의존형 QRC의 훈련 방식으로 이 문제를 정식화한다. 이 접근방식은 주어진 저수지와 훈련 데이터셋을 위해 예측 오류를 최소화하는 최적의 측정 연산자를 제공한다고 주장한다.

- **Technical Details**: 연구에서는 N-큐비트 모델을 고려하여 특징 맵(feature map)을 통해 입력 데이터를 양자 상태로 인코딩한다. QRC의 고전적인 출력은 선택된 측정 연산자 세트에 따라 정의되며, 이러한 측정 연산자는 최적화를 통해 결정된다. 본 논문에서는 측정 연산자 선택과 관련된 다양한 방법론을 제시하고, QRCs의 경우에는 이러한 연산자들을 적절히 선택해야 최적의 성능을 이끌어낼 수 있음을 강조한다.

- **Performance Highlights**: 최적의 측정 연산자를 활용함으로써 이미지 분류 및 시계열 예측과 같은 작업에서 QRC의 성능이 체계적으로 개선됨을 보인다. 이러한 접근 방식은 고전적 저수지 컴퓨팅에서도 사용되는 커널화 기법과 유사하며 다양한 양자 기계 학습 모델에 응용될 수 있다. 연구 결과, 최적의 측정 연산자를 통해 QRCs의 정확도를 높이는 것이 가능하다는 것을 수치 실험을 통해 검증하였다.
...
- **What's New**: 이번 논문에서는 다상 매체(multi-phase media)에서의 역 문제(inverse problems) 및 설계(inverse design)에 대한 새로운 접근법인 GenPANIS를 제안합니다. 이 모델은 미세 구조를 보존하면서도 연속적인 잠재(latent) 표현을 통해 그래디언트 기반 추론을 가능하게 합니다. GenPANIS는 단일 아키텍처 안에서 양방향 추론(forward prediction과 inverse recovery)을 지원하며, 이는 기존의 비선형 경량 모델보다 효율적입니다.

- **Technical Details**: GenPANIS는 물질 필드를 연속적인 잠재 공간으로 변환하여 미세 구조가 정확히 복원되도록 합니다. 이 모델은 물리적으로 적합한 해를 생성하는 이음새 있는 코사인 PDE 솔버(coarse-grained PDE solver)를 갖추고 있으며, 이는 governing equations의 수학적 구조를 유지합니다. 또한, learnable normalizing flow prior를 통해 복잡한 posterior 구조를 잡아낼 수 있습니다.

- **Performance Highlights**: Darcy 유동(Darcy flow)과 헬름홀츠 방정식(Helmholtz equations)을 기반으로 한 실험에서 GenPANIS는 드문 경우에서의 정확한 재구성을 성공적으로 수행했습니다. 이 모델은 10배에서 100배 더 적은 매개변수로 최신 기술보다 더 나은 성능을 보여주며, 원거리 조건과 미세 구조 통계의 변동에도 잘 일반화됩니다. 또한, 불확실성 정량화(uncertainty quantification)에서 원칙적 기준을 제공하여 더욱 신뢰할 수 있는 결과를 연결합니다.



### Quantum Reservoir Computing with Neutral Atoms on a Small, Complex, Medical Datas (https://arxiv.org/abs/2602.14641)
- **What's New**: 이 연구는 바이오마커의 임상 결과 예측을 위한 새로운 접근 방식으로 양자 저장소 컴퓨팅(Quantum Reservoir Computing, QRC)을 탐구합니다. 기존의 기계 학습 방법들은 비선형 관계와 상관된 특징들의 존재로 인해 도전에 직면해 있으며, QRC는 이러한 문제를 해결하기 위한 대안으로 제안됩니다. 특히, 노이즈가 없는 시뮬레이션과 중성 원자 리드버그 프로세서 'Aquila'에서 실행된 하드웨어를 사용하여 성능을 평가합니다.

- **Technical Details**: 연구에서는 바이오마커 데이터로 이진 분류 작업을 수행하고, 전통적인 기계 학습 모델 6개와 SHAP를 사용하여 특징 서브셋을 생성했습니다. 모델에서는 두 종류의 데이터를 사용하여 QRC와 전통적인 방법을 비교하여, 각각의 최적 특징 수로 평가했습니다. QRC의 하드웨어 실행은 다양한 데이터 분할에 걸쳐 더 높은 평균 테스트 정확도와 로버스트함을 보여줍니다.

- **Performance Highlights**: QRC는 기존 모델과 비교할 때 트레이닝 정확도가 더 높고 분할 간 변동성이 큽니다. 이는 과적합과 일관된 결과입니다. 하드웨어 실행의 효과는 통계적으로 유의미한 개선을 보여줍니다. QRC의 하드웨어 실행은 평균으로의 압축과 상호 정보의 점진적 감소 같은 구조적 변환을 적용하여, 결과적으로 데이터 처리에서 더 안정적인 성능을 제공합니다.
...
Comments:
          13 pages, 3 figures, mODa14

- **What's New**: 이 논문에서는 subset selection 문제를 처음으로 kernel discrepancy와 관련하여 NP-hard임을 보여주고 있습니다. 기존의 L∞ star discrepancy와는 다른 관점에서 이 문제의 복잡성을 분석하였으며, Bayesian Optimization을 통해 이 문제를 효율적으로 해결할 수 있는 방법을 제안합니다. 이를 통해 딥 임베딩 커널을 활용한 새로운 접근 방식을 소개하고 있습니다.

- **Technical Details**: 논문에서는 커널 기반의 subset selection 문제를 다루고 있으며, 여기서 최대 평균 불일치(maximum mean discrepancy) 개념을 이용합니다. 먼저 subset selection 문제의 일반적인 NP-hard성을 보였고, 이를 바탕으로 Bayesian 최적화 알고리즘을 제안합니다. 이 방법은 주어진 집합에서 정보 손실을 최소화하며 대표성을 극대화하는 방향으로 개발되었습니다.

- **Performance Highlights**: 제안된 Bayesian Optimization 알고리즘의 성능은 여러 실험을 통해 입증되었습니다. 다양한 디자인 기준에 대해 광범위하게 적용가능한 프레임워크를 제공하며, 결과적으로 기존의 방법들에 비해 보다 효과적으로 discrepancy measures를 최소화하는 것을 목표로 합니다. 이 연구는 머신러닝, 로보틱스, 컴퓨터 그래픽스 등 다양한 분야에서 활용될 수 있는 가능성을 보여줍니다.
...
- **What's New**: 이 논문은 두 가지 로그-오목(log-concave) 샘플링 문제인 제약 샘플링(constrained sampling)과 복합 샘플링(composite sampling)을 다룹니다. 저자들은 에피그래프(epigraph) 변환을 사용하여 두 문제를 거 lifted된 공간에서 거의 균등한 샘플링으로 줄이는 방법을 제안합니다. 이를 통해 제약 조건을 처리하면서도 혼합(mixing)을 유지할 수 있는 새로운 접근 방식을 제시하고 있습니다.

- **Technical Details**: 논문에서 사용된 proximal sampler는 로그-오목 대상에서의 샘플링을 위한 편향이 없는 방법으로, optimization에서의 proximal point method와 밀접한 관련이 있습니다. 구성된 샘플링 알고리즘은 원래 RGO(restricted Gaussian oracle)에 기초하지만, 이는 구현하기 어려운 경우가 많습니다. 하지만 이 논문에서는 다양한 oracle 모델에 따라 RGO를 효율적으로 구현할 수 있는 방법을 제시하고 있습니다.

- **Performance Highlights**: 제약 샘플링과 복합 샘플링 모두에 대해, 저자들은 Rényi 및 χ² 발산(divergence)으로 측정되는 혼합 시간 경계(mixing time bounds)를 설정했습니다. 해당 연구는 기존의 프로젝션(projection), 반사(reflection), 장벽 함수(barrier functions) 또는 미러 맵(mirror maps)에 의존하지 않고, 최소한의 oracle 접근 방식을 통해 가능하다는 점에서 실용적이고 편향이 없는 샘플러를 만들어냈습니다. 이는 다양한 샘플링 상황에서 활용 가능성을 높이는 결과입니다.
...
- **What's New**: 본 연구에서는 Gaussian Process Thompson Sampling (GP-TS)을 sequential decision-making에 대해 다루며, 기존 연구에서 사용하는 domain discretization에 의존하지 않고 fractional Gaussian process posteriors를 기반으로 하는 frequentist regret 분석을 제공합니다. 본 연구에서 제시한 방법론은 기존의 GP-TS 이론에서 나타나는 variance inflation을 fractional posterior에 대한 Thompson Sampling으로 해석할 수 있음을 보여줍니다.

- **Technical Details**: 저자들은 정보 얻음 매개변수(Information gain parameter)인 $gamma_t$와 posterior contraction rate인 $epsilon_t$에 따라 표현된 kernel-agnostic regret bound를 도출하였습니다. 또한, Gaussian process prior가 특정 조건을 충족할 때 $epsilon_t$를 제어할 수 있는 방법을 주목합니다. 연구 결과, 제시된 일반적인 bound의 특별한 경우로는 squared exponential kernel에 대한 $tilde{mathcal{O}}(T^{rac{1}{2}})$와 Matérn-$nu$ kernel에 대해 $tilde{mathcal{O}}(T^{rac{2
u+3d}{2(2
u+d)}})$ 등이 포함됩니다.

- **Performance Highlights**: 제시된 분석은 GP-TS를 위한 통합되고 이산화(discretization-free)된 regret framework를 제공하며, 여러 kernel 클래스에 폭넓게 적용됩니다. 특히, rational quadratic kernel에 대해서도 $tilde{mathcal{O}}(T^{rac{2
u+3d}{2(2
u+d)}})$의 제약을 확립합니다. 이러한 결과는 GP-TS의 성능을 개선하고, 다양한 상황에서 활용 가능한 기초 자료를 제공합니다.
...
- **What's New**: 이번 논문에서 제안하는 CAIRO (Calibrate After Initial Rank Ordering) 프레임워크는 회귀(regression)과 순위学习에서 발생하는 목표 간 관계를 분리하여 두 단계로 나누어 해결하려는 시도를 하고 있습니다. 첫 번째 단계에서는 스케일 불변(ranking loss)을 최소화하여 점수를 학습하고, 두 번째 단계에서는 이소토닉 회귀(isotonic regression)를 통해 목표 스케일을 복구합니다. 이를 통해 모델은 아웃라이어(outlier)나 비대칭 노이즈(heavy-tailed noise)에 대해 더 견고한 성능을 보일 수 있습니다.

- **Technical Details**: CAIRO의 첫 번째 단계는 최적의 순서를 학습하기 위해 순위 손실(ranking loss)을 최소화합니다. 두 번째 단계는 점당 손실(pointwise loss)을 최소화하여 스케일 복구를 진행합니다. 이론적으로, CAIRO는 인구 집단 수준에서 진정한 조건부 기대값을 목표로 하고 있으며, 이 단계적 학습 방법은 아웃라이어로부터의 보호 협력할 수 있는 장점을 제공합니다.

- **Performance Highlights**: CAIRO는 표준 회귀 목표에서 수익성이 낮은 지역에서는 상당한 성과를 보이며, 특히 표 형태의 데이터셋에서 최첨단 트리 앙상블(tree ensembles)과 경쟁하며, 심지어 이질적 노이즈(heteroskedastic noise)가 있는 상황에서도 분명한 성과 차이를 보여줍니다. 전반적으로 CAIRO는 회귀 문제에서의 순위 방법과 모델 성능을 개선할 수 있는 중요한 진전을 보여줍니다.
...
- **What's New**: 이 논문에서는 저비용 및 에너지 효율적인 다자 간 계산 (Multi-party Computation, MPC) 추론을 위한 새로운 최적화 기법인 저랭크 분해 (Low-Rank Decomposition, LRD)를 제안합니다. ML 모델의 선형 계층에서 저랭크 분해를 적용함으로써 큰 행렬 곱셈을 두 개의 작은 행렬 곱셈으로 대체하여 성능을 개선하려고 합니다. 그러나 단순한 저랭크 분해는 예상보다 더 많은 통신과 계산 오버헤드를 초래하여 성능 향상의 이점을 상쇄할 위험이 있습니다.

- **Technical Details**: 저자들은 저랭크 분해를 개선하기 위해 두 가지 최적화 기법인 트렉케이션 생략 (truncation skipping)과 선형 계층 연결 (linear layer concatenation)을 제안합니다. 트렉케이션 생략은 두 번째 저랭크 행렬 곱셈 후에만 트렁케이션 단계를 수행하도록 수정하여 추가적인 오버헤드를 줄입니다. 선형 계층 연결은 두 개의 연속적인 행렬 곱셈을 파이프라인 방식으로 수행하여 통신 지연을 숨기고 전체적인 MPC의 효율성을 향상시키는 데 기여합니다.

- **Performance Highlights**: 실험 결과, n-PC 및 3-PC 프로토콜에서 각각 25% 및 33%의 처리 속도 향상이 관측되었으며, GPU 에너지 소비는 최대 52% 절감되고 오프라인 단계 지연은 88% 감소하는 것으로 나타났습니다. 이로써 저자들은 LRD-MPC가 MPC 환경에서 ML의 적용 가능성을 크게 확장할 수 있음을 주장합니다.
...
- **What's New**: 이 논문은 AdaptManip이라는 자율적인 휴머노이드 로봇을 위한 통합 내비게이션, 물체 운반 및 배달 기능을 수행하는 프레임워크를 소개합니다. 이전의 모방 학습(imitation learning) 기반 접근 방식과 달리, AdaptManip은 인간의 시연이나 원격 제어 데이터 없이 강화 학습(reinforcement learning)을 사용하여 견고한 loco-manipulation 정책을 훈련하는 것을 목표로 합니다. 이 시스템은 실시간으로 조작된 물체를 추적할 수 있는 객체 상태 추정기(object state estimator)와 견고한 보행을 위한 전체 몸체 정책(whole-body base policy)으로 구성됩니다.

- **Technical Details**: AdaptManip 프레임워크는 세 가지 상호 연결된 컴포넌트로 구성되어 있습니다. 첫 번째는 제한된 시야(field-of-view)와 장애물이 있을 때도 물체를 추적하는 반복적 객체 상태 추정기입니다. 두 번째는 안정적인 물체 운반을 위해 잔여 조작 제어(residual manipulation control)가 포함된 전체 몸체 정책이며, 세 번째는 드리프트에 강한 로컬라이제이션을 제공하는 LiDAR 기반의 로봇 위치 추정기(robot global position estimator)입니다. 모든 컴포넌트는 강화 학습을 통해 시뮬레이션에서 훈련되며, 실제 하드웨어에 제로샷(zero-shot) 방식으로 배포됩니다.

- **Performance Highlights**: 실험 결과에 따르면, AdaptManip은 적응성(adaptability) 및 전체 성공률(overall success rate) 측면에서 모방 학습 기반 접근 방식을 포함한 기본 방법들보다 훨씬 뛰어난 성능을 나타냅니다. 정확한 물체 상태 추정(object state estimation)은 장애물이 있을 때도 조작 성능을 향상시키는 데 기여합니다. 또한, 우리는 완전 자율적인 실제 내비게이션, 물체 운반 및 배달 기능을 가진 휴머노이드 로봇의 사례를 보여줍니다.
...
- **What's New**: 이 논문에서는 log-concave 샘플링을 위한 높은 정확도 보장이 가능한 방법을 제시합니다. 특히, stochastic gradients의 tail behavior가 subexponential일 때, 해당 메소드는 polylog(1/δ) 쿼리와 시간 내에 δ-정확한 샘플을 생성할 수 있음을 보여주고 있습니다. 이 연구는 기존의 convex optimization 문제에서 stochasticity로 인해 쿼리 복잡도가 poly(1/δ)로 나빠지는 것과는 대조적입니다. 또한, light-tailed stochastic gradients의 필요성을 정보 이론적 관점에서 설명합니다.

- **Technical Details**: 이 논문은 stochastic gradient oracle에 접근하여 log-concave density에서 샘플링하는 문제를 다룹니다. stochastic gradients가 unbiased하고 light tails를 가지면, polylog(1/δ) 쿼리를 사용하여 δ-정확한 샘플을 생성할 수 있음을 증명합니다. 특히, 대칭성 없는 상관관계가 있는 우도에서는 minimax-optimal 쿼리 복잡도가 Θ(1/δ)임을 보여줍니다. 이러한 탐색 및 추출이 가능한 새로운 샘플러를 개발하였으며, 각종 stochastic noise 환경에서도 높은 정확도를 달성할 수 있음을 시사합니다.

- **Performance Highlights**: 이 연구의 주요 발견 중 하나는 light-tailed stochastic gradients가 높은 정확도의 sampling에 필수적이며 충분하다는 것입니다. 연구 결과는 α-strongly log-concave 및 β-log-smooth 기저 분포에 대해 δ-정확한 샘플을 생성하는 데 필요한 쿼리 수를 최적화하였다. 또한, stochastic gradient noise의 영향을 받더라도 정확도는 크게 감소하지 않으며, 이는 샘플링 기법의 강력성을 보여줍니다. 종합적으로, 이 연구는 샘플링의 정확성을 높이기 위한 새로운 경로를 제시합니다.
...
### Floe: Federated Specialization for Real-Time LLM-SLM Inferenc (https://arxiv.org/abs/2602.14302)
Comments:
          Accepted by IEEE Transactions on Parallel and Distributed Systems

- **What's New**: 이번 논문에서는 Floe라는 하이브리드 연합 학습 프레임워크를 제안합니다. 이 프레임워크는 실시간 응답이 중요한 자원 제약 환경에서 로우 지연(low-latency) 및 프라이버시 보호 추론을 지원합니다. Floe는 클라우드 기반의 블랙 박스 LLM과 경량의 작은 언어 모델(SLM)을 결합하여 개인 데이터와 미세 조정(fine-tuning)을 장치에서 유지하면서 클라우드 LLM이 일반 지식을 제공하는 방식입니다.

- **Technical Details**: Floe는 두 가지 단계로 운영됩니다: 미세 조정 및 추론입니다. 미세 조정 단계에서는 Low-Rank Adaptation (LoRA) 어댑터를 사용하여 다양한 훈련 데이터와 장치 참여를 증진합니다. 또한, 하드웨어 및 실행 중 이질성에 대비해 이질성 인식 적응형 LoRA 어댑터를 도입하고, 파라미터가 없는 Mixture-of-Experts (MoE) 라우터를 통해 데이터 이질성을 완화합니다.

- **Performance Highlights**: 실험 결과에 따르면, Floe는 사용자 프라이버시와 개인화(user privacy and personalization)를 개선하고, 기존 방법들에 비해 모델 성능 및 실시간 제약 하의 추론 지연을 크게 줄이는 효과를 보였습니다. 이러한 결과는 Floe가 자원 제약 환경에서 우수한 성능을 발휘할 수 있음을 시사합니다.
...
- **What's New**: MILD는 다중 의도를 가진 네트워크에서 발생할 수 있는 고장이 여러 의도 간의 KPI(주요 성과 지표) 저하를 유발할 때, 원인 의도를 식별하고 사전 예방적 알림을 제공하는 혁신적인 프레임워크입니다. 기존의 반응적 접근 방식 대신, MILD는 고정된 시계열 범위 내에서 실패를 예측하고 원인 의도를 판단합니다. 이 시스템은 GPT와 같은 Teacher-Augmented Mixture-of-Experts (MoE) 구조를 사용하여 학습하며, KPI 위험 점수를 조정합니다.

- **Technical Details**: MILD는 고정 범위 내에서 의도 기반 속성 변화에 대한 예측을 수행하며, 각 의도에 대해 조정된 위험 점수를 출력합니다. 이 프레임워크는 KPI 피처에서 시작하여, 각 의도에 대한 위험 점수 및 원인 분포를 생성합니다. 모델 훈련은 Kullback-Leibler (KL) 발산 기반의 프로세스를 통해 진행되며, 확인된 원인과 여러 의도의 상관(symptomatic) 증상 간의 관계를 추적합니다.

- **Performance Highlights**: MILD는 비선형 고장 및 공진 상황에서 기존 방법보다 3.8%에서 92.5% 더 긴 정비 리드 타임을 제공하며, 의도 수준의 원인 식별 정확도를 평균 9.4%에서 45.8% 향상시켰습니다. 또한, 각 경고에 대한 KPI 설명을 통해 보다 효과적인 진단을 가능하게 하여 운영 효율성을 크게 향상시킵니다. MILD는 이러한 성능 개선을 통해 다중 의도를 고려한 네트워크의 개선을 지원합니다.



### Fast Compute for ML Optimization (https://arxiv.org/abs/2602.14280)
- **What's New**: 이번 연구에서는 분산-평균 스케일 혼합 표현을 허용하는 손실을 최적화하는 방법을 제시합니다. 제안된 Scale Mixture EM (SM-EM) 알고리즘은 사용자 지정 학습률과 모멘텀 스케줄을 제거하고, 잠재 변수가 관측 및 매개변수 가중치를 결정하는 방식으로 작동합니다. 이는 Adam 알고리즘의 두 번째 모멘트 스케일링과 유사하나 모델로부터 유도됩니다.

- **Technical Details**: SM-EM 알고리즘은 EM(Expectation-Maximization) 알고리즘을 통해 확장된 표현에 적용되어 잠재 변수(ω, λ)가 단순히 관찰 가중치와 매개변수 가중치를 나타냅니다. 이 알고리즘은 데이터 보강(data-augmentation) 프레임워크를 사용하여, 경량의 최적화 업데이트를 수행하며, 모멘텀 및 학습률 조정을 완전히 제거합니다. 최적화 스케줄을 조정하지 않고도 Adam보다 낮은 손실을 초과합니다.

- **Performance Highlights**: SM-EM은 합성의학적 로지스틱 회귀 테스트에서 Nesterov 가속을 적용하여 Adam보다 최대 13배 더 낮은 최종 손실을 달성했습니다. 또한 40포인트 정규화 경로에서 통계량을 공유하여 동일한 Adam 조정 프로토콜에 비해 10배 더 빠른 실행 시간을 기록했습니다. 이 방법은 반복마다 O(p3) 선형 해결과 관련된 계산의 병목 현상을 해결하는 방법으로 Halton의 몬테카를로 방법을 사용하여 효율성을 더욱 높입니다.
...
Comments:
          To be presented at IEEE International Conference on Communications (ICC 2026): Symposium on Next Generation Multiple Access. 6 pages, 3 algorithms, 3 figures

- **What's New**: 최근에 제안된 Pinching Antenna Systems (PASSs)는 유연한 안테나 기술로서, 저비용의 pinching 요소를 유전체 waveguide에 부착하여 구현됩니다. 이러한 시스템은 직접적인 링크를 우회하여 무선 채널의 대규모 효과를 효과적으로 보완할 수 있습니다. 본 연구는 PASSs의 OTA-FL(over-the-air federated learning)에서의 잠재적 이점을 탐구하며, 에너지 소비를 최소화하는 알고리즘적 접근법을 개발하였습니다.

- **Technical Details**: PASS를 사용하는 분산 학습 시스템에서, 단일 안테나 장치들이 단일 매개변수 서버(PS)와 통신합니다. PS는 PASS 수신 송신기를 장착하며, PIN 요소를 수용하는 유전체 waveguide를 통해 신호를 송출합니다. 이 시스템은 모델 집합(aggregation)을 위해 AirComp(무선 송신) 기술을 활용하며, 에너지 효율성을 높이기 위해 PASS 매개변수를 최적화합니다.

- **Performance Highlights**: PASS가 지원하는 OTA-FL 시스템은 전통적인 MIMO 서버와 비교하여 모델 집합의 에너지를 획기적으로 감소시켰습니다. 수치 실험을 통해 PASS는 대규모 효과를 효율적으로 보상하여 다음 세대 무선 시스템의 배포 학습에 적합한 기술로 자리매김하고 있습니다. PASS는 단일 radio-frequency 체인을 필요로 하며, 그 비용과 하드웨어적인 측면에서 경제적 이점을 제공합니다.
...
- **What's New**: 이번 논문에서는 Personalized Federated Learning (PFL)에서의 모델의 교환 가능성을 부각하는 새로운 방법인 Progressive Personalized Federated Ensembles (PPFE)를 제안합니다. PFL은 클라이언트 전용 모델을 통해 통계적 이질성을 감소시키는 데 초점을 맞추고 있으며, 기존 방법들보다 더 나은 일반화 성능을 제공합니다. 또한, 이 방법은 모델의 복잡성을 글로벌 표현을 유지하면서 조절할 수 있는 혁신적인 구조를 제공합니다.

- **Technical Details**: PPFE는 각 클라이언트에 대해 T개의 개별 모델을 훈련하면서 점진적으로 개인화의 정도를 조절하는 부스팅(Boosting) 스타일의 프레임워크를 활용합니다. 모델은 이전 단계에서 발생한 잔차 오류를 재가중치화하여 후속 모델에서 직접적으로 목표화함으로써 학습됩니다. 이러한 메커니즘을 통해, 모델의 복잡성을 제한하고 오버피팅(overfitting)을 방지하며, 개인화의 효과를 극대화할 수 있습니다.

- **Performance Highlights**: 저자들은 EMNIST, CIFAR-10/100 및 Sent140과 같은 데이터셋에서 PPFE의 성능을 검증하였고, 이 방법이 현재 최첨단 PFL 방법보다 항상 우수한 성능을 발휘함을 보여주었습니다. PPFE는 특히 비동질 데이터 분포에서의 성능 향상을 증명했으며, 기존의 모델보다 잔여 오류를 효과적으로 수정할 수 있는 능력을 가지고 있습니다.
...
- **What's New**: 본 논문에서는 인간의 글쓰기와 구별이 불가능한 텍스트를 생성하는 대형 언어 모델들에 대한 신뢰할 수 있는 출처 추적의 필요성을 강조합니다. 기존의 방법들이 긴 메시지를 처리하면서 텍스트 품질과 워터마크 강도를 동시에 유지하는 데 어려움을 겪는 반면, 본 연구는 MC$^2$Mark라는 새로운 다중 비트 워터마킹 프레임워크를 제안합니다.

- **Technical Details**: MC$^2$Mark의 핵심 기술 아이디어는 Multi-Channel Colored Reweighting 입니다. 이 방법은 구조화된 토큰 재가중치를 사용하여 비트를 인코딩하면서도 토큰 분포의 편향을 없애고, Multi-Layer Sequential Reweighting을 활용하여 워터마크 신호를 강화합니다. 또한 메시지 복구를 위한 증거 축적 감지기를 포함하여 신뢰할 수 있는 임베딩 및 디코딩을 지원합니다.

- **Performance Highlights**: 실험 결과, MC$^2$Mark는 이전의 다중 비트 워터마킹 방법들에 비해 감지 가능성과 강인성이 향상되었으며, 생성 품질을 유지하고 있습니다. 짧은 메시지의 경우 거의 완벽한 정확도를 달성했으며, 긴 메시지의 경우 두 번째로 좋은 방법에 비해 거의 30% 향상된 성능을 보여주었습니다.



### Why Self-Training Helps and Hurts: Denoising vs. Signal Forgetting (https://arxiv.org/abs/2602.14029)
Comments:
          8 pages main, 29 pages in total

- **What's New**: 본 연구는 반복적 자기 훈련(self-training)이 과다 매개변수화된 선형 회귀 모델에서 어떻게 작용하는지를 분석하였습니다. 이 절차는 초기 추정기가 노이즈가 포함된 레이블로 훈련되고, 이후 각 반복(iteration)에서 이전 모델에서 생성된 무노이즈의 의사 레이블(pseudo-labels)을 사용하여 새로운 공변량(covariates)으로 훈련됩니다. 연구 결과, 예측 위험(prediction risk)과 효과적인 노이즈에 대한 결정론적 동등 회귀(deterministic-equivalent recursions)를 도출하였으며, 이는 고차원 환경에서의 모델 훈련 효과를 시사합니다.

- **Technical Details**: 이 연구에서 사용된 고차원 선형 모델은 훈련 데이터 𝒟₀를 통해 정의됩니다. 회귀 추정량(estimators)은 비모수적 예측 위험을 기반으로 평가되며, 이는 독립적인 테스트 포인트에 대한 예측 오차로 정의됩니다. 특히, 라지 추정량(ridge estimator)과 비 라지 추정량(ridgeless estimator)의 차이점이 명확히 설명되며, 회귀의 한계 상황에서 두 추정량의 연관성을 다룹니다.

- **Performance Highlights**: 실험은 이론을 검증하고, 반복적인 자기 훈련 과정에서 예측 성능의 감소와 증가 사이의 균형을 보여줍니다. 이 연구는 또한 이상적 중단 시점(optimal stopping time)을 추정하기 위한 반복화된 일반화 교차 검증(iterated generalized cross-validation) 기준을 제안합니다. 실험 결과는 이론이 제시하는 예측 노이즈와 포기(signal forgetting) 간의 상충 관계가 실제 데이터에서도 존재함을 확인시켜 줍니다.



### Computable Bernstein Certificates for Cross-Fitted Clipped Covariance Estimation (https://arxiv.org/abs/2602.14020)
- **What's New**: 본 연구에서는 heavy-tailed 샘플을 이용한 연산자 노름(operator norm) 공분산(covariance) 추정 방법을 제안합니다. 기존의 유클리드 노름 클리핑(Euclidean norm clipping) 방법은 클리핑 수준에 대한 신뢰성이 떨어지기 때문에, 데이터 기반의 조정 방법인 MinUpper를 통해 더 적절한 클리핑 수준을 자동으로 선택합니다. 이 방법은 멀쩡하지 않은 관측치(outliers)의 영향을 최소화하는 동시에 공분산의 정확도를 향상시킵니다.

- **Technical Details**: 연구에서는 교차 적합(cross-fitting) 기법을 사용하여 클리핑 반경을 추정하고, 멀쩡한 설명 변수에서만 이 클리핑을 적용하는 방안을 설계했습니다. 이렇게 함으로써, Clipped covariance 추정치의 안정성과 신뢰성을 동시에 높일 수 있었으며, 특히 스파이크(spiked) 모델을 통해 고차원 PCA 및 팩터 모델에 적합한 성능을 발휘했습니다. 제안된 방법은 Bernstain-type deviation certificates로 무작위 오류와 클리핑 비이용(bias) 간의 균형을 잘 맞춥니다.

- **Performance Highlights**: 대규모 실험을 통해 contaminated spiked-covariance 모델에서 제안된 방법이 안정적인 성능을 보이는 것을 확인했습니다. 다양한 시나리오에서도 경쟁력 있는 정확도를 자랑하며, 특히 클리핑 수준을 데이터 기반으로 조정할 수 있는 점에서 큰 장점을 가집니다. 이 연구는 높은 차원의 데이터 분석에 대한 중요한 통찰을 제공합니다.
...
- **What's New**: 본 논문은 대형 언어 모델(LLMs)의 미세 조정(fine-tuning) 이론적 기초를 발전시키기 위해 통계적 프레임워크를 제시합니다. 구체적으로, 주목(attention) 기반의 Neural Tangent Kernel (NTK) 이론을 활용하여 미세 조정의 수렴 보장을 제공하고, 샘플 크기에 따른 수렴 속도가 경험적 커널 행렬의 고유값 감소율과 밀접한 관계가 있음을 보여줍니다. 이는 미세 조정이 왜 몇 번의 에폭(epoch)만으로도 높은 성능을 발휘하는지에 대한 새로운 통찰을 제공합니다.

- **Technical Details**: 본 연구는 미세 조정에 대한 NTK 이론의 클래식한 확장을 비무작위(pretrained) 초기화에 적용하여, 변환기(transformer) 기반 네트워크에 대한 새로운 이해를 제공합니다. 실험을 통해 현대적인 대형 언어 모델에 적용하여 우리 이론의 경험적 증거를 제시합니다. 특히, 다중 헤드(mult- head) 주목 메커니즘을 포함하는 네트워크에 대한 안정성과 수렴성 분석을 정의하고, 이를 통해 특정 숨겨진 층에 구조를 추가할 수 있는 경우에 대한 이론적 관점을 제공합니다.

- **Performance Highlights**: 현대 대형 언어 모델인 GPT-Neo에 대한 실험 결과는 우리 이론적 통찰을 뒷받침하는 경험적 증거를 제공합니다. 또한, 우리의 이론적 프레임워크가 실제 LLM 미세 조정 관행과 어떻게 연결되는지를 논의하며, 다른 작업을 위한 작업 벡터(task vectors) 설명에 대한 이론적 근거를 제공합니다. 이러한 발견은 LLMs의 효과적인 활용을 위한 중요한 기초를 제공합니다.
...
- **What's New**: 본 연구에서 소개하는 voice2mode는 대규모 자기 지도 음성 모델에서 추출한 임베딩을 활용하여 네 가지 노래 발성 모드(숨소리, 중성, 플로우, 눌린)을 분류하는 방법론입니다. 이전의 수작업 특징(feature) 분석이나 특정 작업에 맞춘 신경망에 비해, 이 연구는 음성 기반 기초 모델의 전이 가능성을 검증합니다. 실험 결과, HuBERT 임베딩을 활용했을 때 약 95%의 정확도로 기존 방법들보다 12-15% 높은 성능을 기록했습니다.

- **Technical Details**: voice2mode는 HuBERT와 두 가지 wav2vec2 변형 모델에서 레이어별 표현을 추출하는 방법을 기반으로 하며, 전역 시간 풀링(global temporal pooling)을 통해 이들을 간단한 분류기(SVM, XGBoost)와 결합하여 발성 모드를 분류합니다. 연구는 763개의 지속된 모음 녹음을 포함하는 공개 소프라노 데이터셋을 이용하여 진행되었습니다. 특히, 전형적인 스펙트럼 기반 기준(스펙트로그램, 멜 스펙트로그램, MFCC)과 비교해 자가 지도 학습(self-supervised learning) 모델의 특징이 뚜렷한 우위를 보였습니다.

- **Performance Highlights**: HuBERT 모델에서 추출한 임베딩은 특히 정확도가 높아, SVM을 사용하여 약 95%의 정확도를 달성했습니다. 이는 기존 최상의 전통적 기준인 80%에 비해 상당한 개선을 보여줍니다. 연구에서 발견된 바와 같이, 모델의 하위 레이어는 발화 세부 사항을 보다 잘 포착하여 더 높은 분류 정확도를 보였으며, 이는 향후 발성 모드 분류에 데이터 기반 특징을 채택하는 것의 중요성을 시사합니다.
...
- **What's New**: 이번 논문에서는 Stochastic Approximation (SA) 알고리즘의 유한 시간에서 Gaussian 근사와의 분포적 편차를 정량화하는 새로운 경계를 확립합니다. 기존의 연구에서는 SA의 비대칭 통계적 성질에 대한 탐구가 부족했으나, 본 연구를 통해 비대칭 경계와 Wasserstein 거리의 세밀한 비대칭 속성을 구체적으로 제시합니다. 특히, 다양한 스텝 사이즈에 대해 유한 시간의 오차를 구하는 방법을 추가적으로 제공합니다.

- **Technical Details**: 접근 방식으로는 Stein의 방법을 활용하여 SA의 분포 동역학을 분석하며, 행렬 가중 합을 포함한 i.i.d. 랜덤 변수의 제어를 위해 DOUG(Discrete O-U with Generalized noise) 프로세스를 도입합니다. 본 논문에서 다루는 DOUG는 비대칭 행렬을 허용하며, 데이터 샘플링 문헌과의 연결성을 통해 선형 드리프트를 식별하고 이를 바탕으로 SA의 유한 시간 행동을 연구합니다. 우리는 이러한 기법을 통해 비대칭적 오차 경계를 얻는 데 중점을 둡니다.

- **Performance Highlights**: SA의 유한 시간 성능을 분석한 결과, 도출한 경계는 Wasserstein-1 거리에서의 수렴률과 함께 명확한 장점을 보여줍니다. 추가적으로, 이 방법론은 기존 SA 알고리즘의 더 많은 응용 가능성을 제시하며, 미세한 오차를 정량화하여 샘플링 연구 분야와는 별개로도 흥미로움을 제공합니다. 이는 SA 알고리즘의 효율성과 신뢰성을 동시에 강화하는데 기여할 것으로 기대됩니다.
...
### Ensemble-Conditional Gaussian Processes (Ens-CGP): Representation, Geometry, and Inferenc (https://arxiv.org/abs/2602.13871)
Comments:
...
- **What's New**: 이 논문에서는 앙상블 조건부 가우시안 프로세스(Ensemble-Conditional Gaussian Processes, Ens-CGP)를 소개합니다. Ens-CGP는 앙상블 기반 추론을 중심으로 하는 유한 차원 프레임워크로, 조건부 가우시안 법칙을 기반으로 하여 가우시안 프로세스, RKHS 회귀, 이차 프로그램 및 칼만 업데이트를 통합합니다.

- **Technical Details**: 이 연구에서는 가우시안 우선분포가 상태 벡터에 미치는 영향을 분석하고, 이를 통해 조건부 가우시안 분포(CGP)의 포스터리어 분포를 유도합니다. CGP는 칼만 필터와 같은 동적 모델과 관계없이 정의될 수 있으며, 가우시안 모델의 임의의 조건부 법칙으로 확장될 수 있습니다. 또한 Ens-CGP는 저차(rank) 프라이어를 이용한 앙상블 구성으로 얻어진 조건부 가우시안 법칙으로, 발생된 불확실성의 전파를 명확하게 이해할 수 있게 합니다.

- **Performance Highlights**: Ens-CGP는 다양한 역문제 및 학습 문제에 대한 공통적인 표현적 백본으로 작용합니다. 아울러, 이는 기후 다운스케일링, 수치 모델링과 같은 여러 분야에서 확장성을 지닌 기법으로 자리 잡을 수 있습니다. 최종적으로, 논문은 고전적인 결과들을 다루고 이를 유한 차원 설정으로 귀결하여, 앙상블이 조건부 가우시안 프레임워크에 잘 통합될 수 있음을 보여줍니다.
...
- **What's New**: 이 논문에서는 결측값(missing data)이 있는 데이터에서 학습할 때 발생하는 예측 편향과 일반화 저하 문제를 해결하기 위해 새로운 세 가지 채널 진화 활성화 함수(Three-Channel Evolved Activations, 3C-EA)를 제안합니다. 기존의 고정 스칼라 활성화 함수는 결측값 지표(missingness indicators)나 신뢰도(confidence scores)를 고려하지 않으며, 이는 예측 정확도에 부정적인 영향을 미칩니다. 3C-EA는 유전 프로그래밍(Genetic Programming)을 통해 진화된 다변량(multivariate) 활성화 함수를 사용하여 데이터 품질을 반영한 처리 방식으로 문제를 해결합니다.

- **Technical Details**: 제안된 3C-EA는 세 가지 채널로 구성됩니다: (i) 특성 값(feature value) x, (ii) 결측값 지표 m, (iii) 신뢰도 점수 c로 이루어진 멀티변량 활성화 함수 f(x, m, c)를 통해 동작합니다. 또한, ChannelProp라는 알고리즘을 통해 신뢰성 정보를 선형 레이어를 따라 결정적으로 전파하여 네트워크 전반에 걸쳐 결측 및 신뢰성 값을 유지합니다. 이는 네트워크의 입력 레이어 이후에도 활성화가 신뢰도에 맞춰 조정될 수 있도록 보장합니다.

- **Performance Highlights**: 실험 결과, 3C-EA와 ChannelProp를 사용하면 데이터의 결측성과 신뢰도 정보를 통합함으로써 결측값이 있는 상태에서 분류 성능(classification performance)이 향상됩니다. 여러 데이터셋에서 (MCAR, MAR, MNAR) 결측률을 다양하게 조정하여 테스트한 결과, 고정 활성화 함수(ReLU, Swish 등)와 비교했을 때 성능이 개선되었음을 확인했습니다. 이러한 결과는 데이터 품질 정보가 활성화 함수 탐색에 통합될 때 measurable한 성능 향상을 가져올 수 있음을 입증합니다.



### Causally constrained reduced-order neural models of complex turbulent dynamical systems (https://arxiv.org/abs/2602.13847)
- **What's New**: 이번 연구는 기후 역학을 포함한 복잡한 난류 시스템의 데이터 기반 모델링에 대한 새로운 접근 방식을 제안한다. 신경망 모델에 원인 구조를 부여하는 기법을 통해, 외부 강제력에 대한 반응 능력을 향상시키는 방법을 탐구하였다. 이 프레임워크는 응답 이론(response theory)과 점수 매칭(score matching)에 기반하여 비인과적(noncausal) 의존성을 억제하는 방식으로, 다양한 실제 시스템에 적용될 수 있다.

- **Technical Details**: 이 연구에서는 Fluctuation–Dissipation Theorem (FDT)을 활용하여 다변량 마르코프 시스템의 인과적 연결 xj→xk 를 추론하고, 이를 기반으로 신경망 모델을 학습하는 두 단계로 구성된 프레임워크를 제안한다. 정형화된 데이터를 통해 직접적인 인과관계를 다룰 수 있는 방법을 제공함으로써 모델의 정확성을 높인다. 스코어 함수(score function)를 데이터에서 직접 추정하며, 이를 통해 시스템의 반응을 계산할 수 있는 방법론을 제시하였다.

- **Performance Highlights**: Stochastic Charney-DeVore 모델을 활용하여 제안된 방법의 효과를 실증적으로 보여주었다. 결과적으로, 인과적 제약을 적용한 신경망 모델은 약한 및 강한 외부 강제력에 대한 반응을 효과적으로 포착할 수 있었고, 정적이고 비방해적인 데이터만으로 훈련을 진행했다. 이는 기후 모델링 등 복합적인 난류 시스템의 정밀한 응답 예측에 있어 중요한 기초자료로서 역할을 할 것으로 기대된다.
...
- **What's New**: 최근 물리 정보 신경망(Physics-Informed Neural Networks, PINNs)이 과학 기계 학습(SciML) 분야에서 혁신적인 접근법으로 주목받고 있습니다. PINNs는 손실 함수(loss function)에 부드러운 제약 조건(Soft Constraints)을 포함하여 물리 법칙이 정의된 편미분 방정식(Partial Differential Equations, PDE)을 신경망의 훈련 과정에 직접 통합합니다. 이 연구는 선형 피에조일렉트릭(piezoelectricity)을 모델링하는 1차원 전자-탄성계(system)에 PINNs를 적용하여, PINNs가 결합된 시간 의존성 PDE 시스템의 효과적인 메쉬 프리 솔버(mesh free solver)로 작용할 수 있음을 입증합니다.

- **Technical Details**: 이 연구에서는 Voigt 응력-전하 표기법(Stress-Charge Notation)을 사용하여 정의된 1차원 전자-탄성계 시스템을 고려합니다. 여기서 기계적 변위(mechanical displacement)와 전기적 포텐셜(electric potential)은 본질적으로 연결되어 있으며, 물리적 매개변수로는 밀도(ρ), 탄성 강도(cE), 피에조electric 결합 계수(e33), 그리고 유전율(εS)이 포함됩니다. 신경망은 공간-시간 좌표(x,t)를 기계적 변위(u)와 전기적 포텐셜(φ)로 매핑하는 완전 연결된(feedforward) 구조를 채택하며, 총 약 100,000개의 학습 가능한 매개변수를 포함합니다.

- **Performance Highlights**: PINN 모델은 변위(displacement)와 전기적 포텐셜(electric potential)에 대해 각각 2.34%, 4.87%의 전반적인 상대 L2 오차를 달성했습니다. 이 결과는 PINNs가 결합된 전자-탄성계의 응답을 재현하는 데 능숙하다는 것을 보여줍니다. 그러나 오류 누적(error accumulation) 및 결합된 고유값 시스템에서의 강성(stiffness)과 같은 문제는 여전히 해결해야 할 과제로 남아 있습니다.
...
- **What's New**: 이번 논문에서는 NeuroMambaLLM이라는 새로운 프레임워크를 제안하고, 이를 통해 동적 뇌 신호와 대형 언어 모델(LLM)을 통합합니다. 이 방법은 고정된 상관 그래프 대신 적응형 잠재 연결성을 사용하여 뇌의 기능적 연결을 동적으로 학습하며, 이는 자폐증과 같은 신경발달 장애의 분석에 중요한 역할을 한다고 주장합니다. 또한, LLM을 활용한 의미 있는 텍스트 보고서 생성 기능을 포함하여 진단 분류와 언어 기반 추론을 동시에 수행할 수 있게 됩니다.

- **Technical Details**: NeuroMambaLLM은 (1) 동적 잠재 그래프 추론, (2) 선택적 주(state) 공간 템포럴 모델링, (3) 대형 언어 모델과의 교차 모달 정렬의 세 가지 단계로 구성되어 있습니다. 이 프레임워크는 원시 혈중 산소 농도 의존(BOLD) 신호로부터 시간에 따라 변하는 기능적 연결성을 직접 학습하며, Mamba 아키텍처를 채택하여 긴 시간 시퀀스를 효율적으로 처리합니다. Mamba는 각 시간 단계에서 관련 정보를 선택적으로 추출하여 선형 시간 복잡도로 신호를 처리할 수 있게 도와줍니다.

- **Performance Highlights**: 제안된 방법은 동적 뇌 그래프를 대형 언어 모델(LLaMA-3-8B)의 임베딩 공간으로 투영하여 의미론적 추론을 수행합니다. 최종적으로, NeuroMambaLLM은 자폐증의 진단 정확성을 높여주면서 동적 fMRI 패턴을 분석하고 임상적으로 의미 있는 텍스트 보고서를 생성할 수 있도록 합니다. 이에 따라 임상적 결정 과정에서 이 시스템은 높은 해석 가능성과 contextual reasoning을 제공합니다.
...
### LEAD-Drift: Real-time and Explainable Intent Drift Detection by Learning a Data-Driven Risk Scor (https://arxiv.org/abs/2602.13672)
Comments:
          Copyright 2026 IEEE. Accepted for publication in IEEE ICC 2026. This is a author version. 6 pages

- **What's New**: 이번 연구에서는 Intent-Based Networking (IBN)의 가장 큰 문제인 'intent drift'를 다루는 새로운 프레임워크인 LEAD-Drift를 제안합니다. LEAD-Drift는 프로액티브 실패 예방을 위해 실시간으로 intent drift를 탐지하는 시스템으로, 이를 통해 네트워크의 신뢰성과 성능 저하를 방지함으로써 운영 효율을 높입니다. 이 프레임워크는 경량화된 신경망을 활용해 미래 리스크 점수를 예측하며, 이는 Exponential Moving Average (EMA)와 통계적으로 조정된 임계값을 통해 실시간 경고를 생성합니다.

- **Technical Details**: LEAD-Drift는 역사적인 데이터로부터 실패의 전조를 학습하는 오프라인 훈련 단계와 실시간 네트워크 데이터를 분석하여 알림을 생성하는 온라인 추론 단계를 포함하는 두 단계로 구성됩니다. 이 과정에서 설정된 목표는 경고 시간이 최대화되는 한편, 잘못된 경고의 수를 최소화하여 시스템의 흠결을 실시간으로 탐지하는 것입니다. 이 절차는 Neyman-Pearson 스타일의 제한 프로그램으로 임계값 선택을 최적화하는 형태로 구성됩니다.

- **Performance Highlights**: LEAD-Drift는 실험을 통해 평균 53.6분의 경고 시간과 0.81일당 잘못된 경고 수를 기록하며, 이는 기존의 거리 기반 접근 방식에 비해 현저히 향상된 성능을 나타냅니다. 또한, LEAD-Drift는 무게 기반 KPI 휴리스틱과 비교하여 경고 노이즈를 80.2% 감소시켜, 프로액티브 네트워크 보장을 위한 매우 효과적이고 해석 가능한 솔루션으로 자리잡고 있습니다.
...
- **What's New**: 이 논문에서는 지역적 차분 개인 정보 보호(local differential privacy) 하에서 분포 변화의 탐지를 목표로 하는 파라메트릭(change-point detection) 변화를 연구합니다. 비개인 정보 보호 설정에서는, 마틴게일(martingale) 방법론을 통해 일반화된 로그 우도 비율 테스트에 기반한 변화 탐지 알고리즘의 유한 샘플(finite-sample) 정확도 보장을 향상시킵니다.

- **Technical Details**: 개인 정보 보호 설정에서는 랜덤 응답(randomized response) 및 이진 메커니즘(binary mechanisms)에 기반한 두 가지 지역적 차분 개인 정보 보호 알고리즘을 제안하고 그 이론적 성능을 분석합니다. 이러한 알고리즘의 탐지 정확도에 대한 경계를 도출하고, 경험적 평가(empirical evaluation)를 통해 결과를 검증합니다.

- **Performance Highlights**: 우리의 결과는 변화 탐지에서 지역적 차분 개인 정보 보호의 통계적 비용(statistical cost)을 규명하며, 개인 정보 보호가 비개인 정보 보호 기준에 비해 성능을 어떻게 저하시키는지를 보여줍니다. 또한, 강력한 데이터 처리 불평등(strong data processing inequalities, SDPI)에 대한 구조적 결과를 확립하며, Rényi 발산(Rényi divergences) 및 그 대칭 변형(Jeffreys-Rényi divergences)에 대한 SDPI 계수(coefficients)가 이진 입력 분포에 의해 달성됨을 증명합니다.
...
Comments:
          44 pages, 13 figures. To be submitted to CMAME

- **What's New**: 이번 연구는 데이터 기반의 방정식 발견(data-driven equation discovery)을 사용하여 제한이 없는 최적화 문제의 연속적 수학적 동역학을 모델링하고 예측하도록 하였습니다. 기울기(gradient) 계산과 같은 비싼 함수를 피하기 위해, 최적화 변수에 대한 궤적 데이터를 활용하여 기울기 하강법, 뉴턴 방법, ADAM 최적화와 관련된 연속적 동역학을 학습하였습니다. 이 연구에서는 Learned Gradient Flow (LGF) 최적화기를 소개하며, 사용자가 정의한 구간에서 다항식 차수(polynomial order)의 대체 모델을 구축할 수 있는 기능을 갖추고 있습니다.

- **Technical Details**: LGF 최적화기는 높은 차원의 공간 또는 축소된 차원의 공간에서 최적화 프로세스 중에 사용자 정의 간격에서 면밀한 대상으로의 근사 모델을 구성합니다. 우리는 엔지니어링 역학과 과학 기계 학습의 여러 표준 문제에 대한 결과를 통해 이 접근법의 효능을 입증하였습니다. 기울기 흐름(gradient flow)을 사용하면, 최적화 궤적의 중요한 특징을 포착하면서 목표 함수와 그 기울기(gradient)를 비싼 계산 없이 피할 수 있습니다.

- **Performance Highlights**: 우리가 제안한 방법은 여러 역문제(inverse problems), 구조적 절대 최적화(structural topology optimization) 및 다양한 이산화(discretizations)를 포함하는 여러 문제에 대해 수행되었습니다. 연구 결과는 학습된 기울기 흐름이 수렴을 상당히 가속화할 수 있음을 시사하며, 이는 비싼 함수 평가를 피함으로써 이루어집니다.
...
- **What's New**: 이번 연구에서는 변분 불등식(variational inequalities, VIs) 해결을 통해 더 넓은 의미에서 최적화(optimization)에 대한 접근을 다룹니다. 특히, 함수의 최소화(minimization), 쌍극(minimax) 문제, 내시균형(Nash equilibrium) 문제와 같은 다양한 문제들을 포함하는 이 범주에 대해서 논의합니다. 저자들은 유한 합 표현(finite-sum representation)과 두 단계 계층 구조(two-level hierarchical structure)에서의 주요 도전을 해결했습니다.

- **Technical Details**: 이 연구에서는 매끄러운 연산자(smooth operators)의 유한 합 표현을 갖는 계층적 변분 불등식을 다룹니다. 이를 통해 저자들은 유클리드(Euclidean) 및 브레그만(Bregman) 설정에서 계층적 VIs의 해(solution) 접근을 위한 분산 감소 확률적 알고리즘(variance-reduced stochastic algorithms)의 수렴률(convergence rates)과 복잡도(complexity) 진술을 최초로 증명하였습니다.

- **Performance Highlights**: 연구 결과는 계층적 변분 불등식 문제에 대한 해결 방안을 제시하며, 특히 새로운 알고리즘의 성능을 강조합니다. 저자들은 이러한 알고리즘이 실제 응용 문제에 효과적으로 적용될 수 있음을 입증하였습니다. 이는 특정 성능 지표를 기반으로 하여 알고리즘의 효율성을 높이는 새로운 가능성을 열어줄 것입니다.



### AsyncVLA: An Asynchronous VLA for Fast and Robust Navigation on the Edg (https://arxiv.org/abs/2602.13476)
Comments:
          13 pages, 9 figures, 2 tables

- **What's New**: 이 논문에서는 AsyncVLA라는 비동기 제어 프레임워크를 제안하여 로봇 제어에 있어 고비용의 대형 로봇 기초 모델의 사용을 지원하고자 합니다. 이 프레임워크는 의미론적 사고(semantic reasoning)와 반응적 실행(reactive execution)을 분리하여, 원격 작업장에서 대형 모델의 고수준 지침을 제공하면서도 가벼운 Edge Adapter가 실시간으로 행동을 수정합니다. 이를 통해 제어 루프에서 발생하는 지연 문제를 해결하고, 로봇의 반응성을 높이는 데 중점을 두었습니다.

- **Technical Details**: AsyncVLA는 비대칭적인 제어 시스템을 구축하여 큰 규모의 로봇 기초 모델과 경량의 Edge Adapter 간의 비동기적 상호작용을 가능하게 합니다. Edge Adapter는 76M 파라미터를 가지며, 실시간 센서를 활용하여 지연된 고수준 지침을 통합합니다. 본 논문에서는 동적 상호작용을 우선적으로 고려하는 훈련 전략과 새로운 경로 재가중 전략을 통해 로봇의 성능을 높이기 위한 방법을 모색했습니다.

- **Performance Highlights**: 실제 환경에서 진행된 테스트 결과, AsyncVLA는 최대 6초의 통신 지연이 있는 상황에서도 40% 이상의 성공률을 기록해 기존 최첨단 기법들을 초월하는 성능을 자랑합니다. 특히, 동적인 내비게이션 시나리오에서 적용했을 때, AsyncVLA는 지연에도 불구하고 일관된 성과를 보여주며, 대형 모델의 의미론적 지능과 엣지 로보틱스에서 요구되는 반응성을 효과적으로 결합했습니다.
...
- **What's New**: 본 논문에서는 그래프 신경망(Graph Neural Networks, GNNs)을 활용하여 복잡한 이질적 시스템의 동적인 활동을 해석 가능한 표현으로 분해하는 방법을 제시합니다. 특히, 각종 뉴런의 연결 구조, 유형, 신호 기능, 외부 자극을 동시에 밝혀내는 데 성공했습니다. 전통적인 기계학습 접근법인 순환신경망(recurrent neural networks)과 transformers와는 달리, 우리의 방법은 신뢰할 수 있는 신경 활동 예측과 메커니즘의 해석 가능성을 동시에 제공합니다.

- **Technical Details**: 이 연구는 뉴럴 어셈블리의 활동을 그래프 형태로 모델링하여, 각 뉴런에 해당하는 노드와 시냅스 연결을 나타내는 엣지를 생성합니다. GNN은 뉴런 간 상호작용과 업데이트 규칙을 함께 학습하며, 잠재적 외부 자극을 암묵적으로 나타냅니다. 실험은 1,000개 및 8,000개의 뉴런을 가진 시스템에서 진행되었으며, 다양한 매개변수를 조정하여 자가 결합(self-coupling) 문제를 시험합니다.

- **Performance Highlights**: 훈련된 GNN은 정확한 연결성 행렬을 복원하는 데 성공했으며, 예측 성능을 통해 동적 시스템의 특성을 효과적으로 나타냈습니다. 또한, 다양한 뉴런 유형에 대한 클러스터링 정확도는 100%에 근접하였고, 새로운 초기 활동 상태에 대한 일반화 능력도 우수했습니다. 이는 GNN이 이질적 뉴런 집합의 동적 특성을 잘 캡처할 수 있음을 보여줍니다.
...
- **What's New**: 이번 연구는 클라우드 시스템에서의 이상 탐지(anomaly detection)의 성능을 평가하기 위해 다양한 딥 러닝 모델(GRU, TCN, Transformer, TSMixer)과 전통적인 Isolation Forest를 포함하여 비교 분석하였습니다. 최대 10만 개의 특성을 포함한 네 가지 텔레메트리 데이터셋을 사용하여 각 모델의 견고성, 확장성 및 구조적 차이에 대한 민감도를 평가했습니다. 연구는 통일된 훈련 및 평가 파이프라인을 통해 이루어졌으며, 이는 이상 탐지 시스템의 재현 가능성과 배포 인식 평가를 지원하는 데 기여합니다.

- **Technical Details**: 이상 탐지 모델은 Numenta Anomaly Benchmark, Microsoft Cloud Monitoring dataset, Exathlon dataset, IBM Console dataset 등 다양한 데이터셋을 기반으로 평가되었습니다. 데이터셋은 구조, 차원, 라벨링 전략에서 서로 다르며, 이를 통해 각 모델의 적합성을 분석하였습니다. 이러한 평가 과정은 NAB 스타일의 메트릭을 활용하여 조기 탐지 행동을 포착하고, 모델 성능을 고르게 비교할 수 있게 해줍니다.

- **Performance Highlights**: 연구 결과에 따르면, 이상 탐지 성능은 모델 아키텍처뿐만 아니라 캘리브레이션 안정성(calibration stability) 및 특성 공간의 기하학(feature-space geometry)에 의해 크게 좌우됩니다. 특히, 몇몇 모델은 분포 변화에 적응하지 못하여 탐지 정확도에서 부정적인 영향을 미치는 경향이 있음을 발견했습니다. 연구진은 이와 관련된 위험을 강조하며, 이상 탐지 시스템의 실용적인 평가 프레임워크를 제안합니다.
...
- **What's New**: 이 논문은 AI 모델의 의사결정에서 도덕 인지는 중요한 요소임에도 불구하고 여전히 불충분하게 탐구되고 있음을 강조합니다. 도덕적 능력을 AI 모델에 통합하기 위한 기술들의 비교 분석과 윤리를 수학적으로 이산화하는 새로운 방법론을 제시하였습니다. 또한 논문은 Expected Moral Shortfall (EMS)이라는 개념을 도입하여 AI 모델의 성능 향상을 도모합니다.

- **Technical Details**: 도덕적 AI 시스템과 윤리적으로 능력 있는 AI 시스템의 차이를 명확히 하였으며, 후자는 윤리적 이론을 적용하여 결정을 내리는 명시적 윤리적 에이전트라고 정의합니다. 연구에서 다루는 이론적 틀로는 결과론적 윤리(consequentialism), 의무론(deontology), 덕 윤리(virtue ethics)를 포함하며, 각 이론이 도덕적 결정을 내리는 데 어떻게 적용될 수 있는지를 논의합니다. 각 이론의 맥락에 따른 가중치를 조정하여 도덕적 판단을 정량화할 수 있는 방법도 제안하고 있습니다.

- **Performance Highlights**: 해당 연구는 두 개의 실제 데이터셋에서 제안된 이론을 실험하여 AI 모델의 다양한 목표 간의 균형을 조사합니다. Expected Moral Shortfall (EMS)이라는 지표를 통해 도덕적 실패를 최소화하면서 AI 모델의 성능을 극대화하는 방법에 성공적으로 접근하였습니다. 이를 통해 AI 모델이 사회적 결정에서 도덕성을 효과적으로 고려할 수 있는 가능성을 제시하였습니다.
...
Comments:
          13 pages. Submmited for possible publication

- **What's New**: 본 논문은 불완전한 수동 도청자 채널 상태 정보(CSI)를 고려한 스택형 지능형 메타서페이스(SIM) 기반의 물리적 레이어 보안(PLS)을 위한 최초의 하이브리드 양자 강화 학습(QRL) 프레임워크를 제안합니다. 기존 연구들과는 달리, 도청자의 채널 정보를 완벽히 알고 있다고 가정하지 않고 CSI의 불확실성을 명확히 모델링하여 장기 평균 보안 비율(ASR)을 최적화하는 문제를 다룹니다.

- **Technical Details**: 이 논문에서는 최소한의 전송 전력 할당 및 SIM 단계 전환의 조합을 통해 ASR을 극대화하기 위해 정량적 근접 정책 최적화(Q-PPO) 알고리즘을 개발했습니다. 이 알고리즘은 매개변수화된 양자 회로를 포함하여 고차원 지속적 행동 공간에서의 정책 표현 능력과 탐색 효율성을 높입니다. 또한, Q-PPO는 전송 전력 및 서비스 품질 제약을 고려하여 SIM의 작동을 최적화합니다.

- **Performance Highlights**: 제안된 Q-PPO 프레임워크는 높은 차원의 SIM 최적화에 있어 기존의 DRL 기반 방법들과 비교해 약 15% 높은 보안 비율을 달성하고, 30% 더 빠른 수렴 속도를 보였습니다. 이는 불완전한 도청자 채널 상태 정보를 고려한 시뮬레이션에서 Q-PPO가 기존 방법들보다 지속적으로 우수한 성능을 발휘함을 보여줍니다. 이 연구 결과는 SIM을 지원하는 무선 네트워크에서 Q-PPO가 강력한 최적화 패러다임으로 자리매김할 수 있음을 확인합니다.

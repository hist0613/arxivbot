...
### Unsupervised Parameter-free Outlier Detection using HDBSCAN* Outlier Profiles (https://arxiv.org/abs/2411.08867)
Comments:
          Accepted at IEEE International Conference on Big Data, IEEE BigData 2024

- **What's New**: 본 연구에서는 GLOSH(Global-Local Outlier Scores based on Hierarchies) 방법을 사용하여 최적의 minpts 값을 찾기 위한 비지도 학습 전략을 제안합니다. 이를 통해 GLOSH 점수의 범위를 활용하여 데이터 세트에서 최상의 이상치(outlier)를 식별할 수 있는 값을 찾습니다.

- **Technical Details**: GLOSH는 HDBSCAN* 클러스터링 방법론에 기반한 비지도형 이상치 탐지 방법입니다. 데이터 포인트의 밀도와 HDBSCAN* 계층 내에서 해당 데이터 포인트가 위치한 지역의 최고 밀도를 비교하여 이상치 점수를 추정하며, 다양한 minpts 값에 따라 GLOSH 점수가 달라지는 패턴을 통해 최적의 minpts 값을 선택합니다.

- **Performance Highlights**: 실험 결과, 본 연구에서 제안한 전략은 minpts 값과 임계값(threshold)을 자동으로 찾아 GLOSH를 사용한 이상치 탐지 결과에서 최적 또는 근접한 성능을 보임을 확인하였습니다.
...
- **What's New**: LLMStinger는 Large Language Models(모델)에서 악의적인 접미사를 자동으로 생성하는 새로운 접근 방식을 소개합니다. 기존의 복잡한 프롬프트 엔지니어링이나 화이트박스 접근 대신, LLMStinger는 강화 학습( reinforcement learning)을 활용하여 공격자 LLM을 미세 조정하며, HarmBench 벤치마크에서의 기존 공격을 기반으로 새로운 접미사를 생성합니다.

- **Technical Details**: LLMStinger는 공격자 LLM이 유해 질문과 공개적으로 사용 가능한 접미사 7개를 입력받아 유사한 새 접미사를 생성하도록 훈련됩니다. 생성된 접미사는 유해 질문에 추가되어 피해자 모델에 전달되며, 성공 여부는 판단 모델에 의해 평가됩니다. 강화 학습 루프는 수상 신호를 활용하여 공격 성공률을 향상시키기 위해 공격자 LLM을 미세 조정합니다. 사용된 알고리즘은 Proximal Policy Optimization(PPO)입니다.

- **Performance Highlights**: LLMStinger는 15개의 최신 공격 방법 중 모든 모델에서 공격 성공률(ASR)에서 우수한 성과를 보였습니다. 특히 LLaMA2-7B-chat에서는 +57.2%의 개선을 기록했으며, Claude 2 모델에서는 +50.3% 증가했습니다. GPT-3.5 및 Gemma-2B-it 모델에서도 각각 94.97%와 99.4%의 ASR를 달성하였습니다.



### Learning Gaussian Multi-Index Models with Gradient Flow: Time Complexity and Directional Convergenc (https://arxiv.org/abs/2411.08798)
Comments:
          21 pages, 6 figures, under review by AISTATS 2025

- **What's New**: 이 논문은 고차원 정규 분포 데이터에서 다중 인덱스 함수(multi-index function)를 근사하기 위해 상관 손실(correlation loss)을 사용하는 신경망 모델의 기울기 흐름(gradient flow) 동역학을 분석합니다. 이 연구는 다중 인덱스 함수의 경우에도 신경망들이 인덱스 벡터에 수렴하는지 또는 최적이 아닌 해결책에서 머무르는지를 다룹니다.

- **Technical Details**: 여기서 다중 인덱스 함수 $f^*(x) = \\sum_{j=1}^k \\sigma^*(v_j^T x)$로 정의되며, $v_1, \dots, v_k$는 단위 벡터입니다. 이 연구에서는 신경 네트워크가 \( k \\log k \) 수의 뉴런을 사용할 때 무작위 초기화(random initialization)를 통해 높은 확률로 인덱스 벡터를 찾아내는 것을 보장합니다. 또한 인덱스 벡터 간의 점곱이 특정 임계값을 초과하면 상관 손실이 실패하는 경향이 있다는 것을 입증합니다.

- **Performance Highlights**: 수치 시뮬레이션 결과, 상관 손실과 적절한 오버파라미터화(overparameterization)를 통해 뉴런이 거의 직교하는 인덱스 벡터를 성공적으로 학습할 수 있는 것으로 나타났습니다. 또한 인덱스 벡터가 직교할 때, 고정점의 완전한 특성화가 제공되어 뉴런이 인덱스 벡터에 가까워진다고 증명합니다.



### Locally Private Sampling with Public Data (https://arxiv.org/abs/2411.08791)
- **What's New**: 본 연구에서는 사용자가 개인 데이터와 공공 데이터를 모두 활용할 수 있는 로컬 차별 사생활 보호(Local Differential Privacy, LDP) 샘플링 프레임워크를 제안합니다. 이는 사용자 각각이 두 개의 분포(p: 개인 데이터, q: 공공 데이터)를 가지고 있다는 가정하에, 주어진 프라이버시와 유용성의 트레이드오프를 고려하여 개인 샘플을 생성하는 메커니즘을 설계합니다.

- **Technical Details**: 제안된 방법은 최악의 경우 입력 분포에 대한 최적 목표 분포를 식별함으로써 성능을 향상시킵니다. 또한, 모든 f-divergence에 대해 보편적으로 최적의 샘플러가 존재함을 입증하며, 알고리즘은 간단한 닫힌 형태를 가집니다. 핵심 성질로는 ε-LDP를 만족하고, 공공 데이터(q)에 대해 영향을 주지 않는 것입니다.

- **Performance Highlights**: 실험을 통해 제안한 미니맥스 최적 샘플러가 기존의 LDP 샘플러보다 월등한 성능을 보임을 입증하였으며, 이는 합성 및 실제 데이터 세트 모두에서 비교Benchmarks against Husain et al.의 방법보다 우수한 결과를 나타냅니다.
...
### Mapping Methane -- The Impact of Dairy Farm Practices on Emissions Through Satellite Data and Machine Learning (https://arxiv.org/abs/2411.08766)
Comments:
          16 pages, 5 figures

- **What's New**: 이 연구는 동부 캐나다의 dairy farm (우유 생산 농장) 특성과 위성 관측에 의해 유도된 methane concentration (메탄 농도) 간의 상관관계를 조사합니다.

- **Technical Details**: 2020년 1월부터 2022년 12월까지 수집된 11개의 dairy farms 데이터를 활용했습니다. Sentinel-5P 위성의 메탄 데이터와 herd genetics (유전자), feeding practices (사료 관리), management strategies (경영 전략) 등 farm-level attributes (농장 수준 특성)를 통합했습니다. 초기 분석에서 메탄 농도와의 유의미한 상관관계가 발견되어 Variance Inflation Factor (VIF)와 Principal Component Analysis (PCA)를 적용하여 다중공선성을 해결하고 모델 안정성을 향상시켰습니다. 이후 Random Forest와 Neural Networks를 활용하여 feature importance (특성 중요도)를 평가하고 메탄 배출량을 예측했습니다.

- **Performance Highlights**: 결과에 따르면, 단백질 비율에 대한 Estimated Breeding Value (EBV)와 메탄 농도 사이에 강한 음의 상관관계가 나타났습니다. 이는 높은 우유 단백질 함량을 위한 유전적 선택이 배출량 감소를 위한 효과적인 전략이 될 수 있음을 시사합니다. 대기 수송 모델과 위성 데이터의 통합을 통해 배출량 추정치가 더욱 정교해졌으며, 정확성과 공간 해상도 또한 크게 향상되었습니다. 이 연구는 고급 위성 모니터링, 머신러닝 기법, 대기 모델링이 dairy sector (유제품 산업) 내 메탄 배출 평가를 개선하는 데 있어 중요한 역할을 할 수 있음을 강조합니다.
...
### ScaleNet: Scale Invariance Learning in Directed Graphs (https://arxiv.org/abs/2411.08758)
Comments:
...
- **What's New**: 본 연구는 그래프 신경망(GNN)에서 노드 분류를 위한 새로운 접근법으로 'scaled ego-graphs'라는 개념을 도입합니다. 기존의 ego-graph를 다수의 방향성이 있는 엣지를 포함하는 형태로 확장하여, 이미지 분류에서의 스케일 불변성과 유사한 방식으로 GNN의 노드 분류 정확도를 개선하고자 합니다.

- **Technical Details**: 이 연구에서 제안된 'scaled ego-graphs'는 전통적인 ego-graph 구조를 일반화하여, 단일 방향 엣지를 다수의 방향 엣지로 대체합니다. 또한, ScaleNet이라는 아키텍처를 통해 방향 정보가 포함된 다중 스케일 특징을 활용하며, 이로 인해 대칭 그래프 및 비대칭 그래프 모두에서 높은 성능을 발휘할 수 있습니다. 모델은 배치 정규화를 포함한 여러 추가 구성 요소들로 성능을 향상시킵니다.

- **Performance Highlights**: ScaleNet은 7개의 벤치마크 데이터셋 중 5개(4개의 동질 그래프와 1개의 비동질 그래프)에 대해 최첨단 결과를 달성했으며, 나머지 2개 데이터셋에서도 상위 성능을 보였습니다. 이 연구는 다양한 그래프 유형에 걸쳐 노드 분류 성능을 향상시키는 통합 프레임워크를 제공하여 그래프 학습의 중요한 발전을 나타냅니다.



### Bayesian Comparisons Between Representations (https://arxiv.org/abs/2411.08739)
- **What's New**: 이번 논문에서는 Bayesian 통계 기반의 새로운 방법을 통해 신경망의 표현을 비교하는 방법을 제시합니다. 특히, prior predictive distribution의 total variation distance나 Jensen-Shannon distance를 사용하여 표현을 비교하는 것을 제안합니다.

- **Technical Details**: 이 연구에서 제안하는 메트릭은 주로 ridge 정규화된 선형 읽기 모델에 기반하며, 이는 predictive distribution을 정규 분포로 변환합니다. 새로운 메트릭은 예측 분포 간의 거리 측정을 통해 각 표현이 선형 읽기를 기반으로 얼마나 잘 구분될 수 있는지를 평가합니다. 또한, 이 메트릭은 representations의 linear kernel matrix에만 의존하며, 다른 기존 메트릭들과 효과적인 연결을 보여줍니다.

- **Performance Highlights**: 제안된 새로운 메트릭은 ImageNet-1k에서 훈련된 딥 뉴럴 네트워크에 적용됐으며, 기존 메트릭들과 비교할 때 더 진지한 평가를 제공하고, 다양한 랜덤 이미지 샘플 간의 변동성이 적습니다. 이를 통해 표현 비교를 위한 도구킷을 효과적으로 확장할 수 있음을 보여줍니다.



### Balancing Speed and Stability: The Trade-offs of FP8 vs. BF16 Training in LLMs (https://arxiv.org/abs/2411.08719)
Comments:
          2 pages,extended abstract

- **What's New**: Llama 3 시리즈는 4050억 개의 파라미터를 갖춘 모델을 소개하며, FP8 포맷을 통한 훈련 최적화 가능성을 탐색합니다.

- **Technical Details**: 본 연구는 Megatron-LM과 NVIDIA의 H100 GPU를 활용하여 BF16 및 FP8 정밀도로 LLM을 훈련하고, 3D 병렬성(3D parallelism) 기술을 적용하였습니다. AdamW 옵티마이저와 cosine learning rate scheduler을 사용했습니다.

- **Performance Highlights**: FP8을 사용한 경우, 모델 성능 저하 없이 훈련 시간이 크게 단축될 수 있음을 시사했습니다. 일본어 및 영어 작업에 대한 평가 결과도 제시되었습니다.
...
### FedSub: Introducing class-aware Subnetworks Fusion to Enhance Personalized Federated Learning in Ubiquitous Systems (https://arxiv.org/abs/2411.08699)
Comments:
          Submitted to Proceedings of the ACM on Interactive, Mobile, Wearable and Ubiquitous Technologies (IMWUT)

- **What's New**: 이 논문에서는 개인화된 연합 학습(Personalized Federated Learning) 문제를 해결하기 위한 새로운 접근법인 FedSub을 제안합니다. FedSub은 클래스를 고려한 프로토타입(class-aware prototypes)과 서브네트워크(subnetworks)를 활용하여 사용자 맞춤형 모델 업데이트를 제공합니다.

- **Technical Details**: FedSub은 사용자 데이터의 간결한 표현인 프로토타입을 클러스터링하여 유사성을 파악합니다. 또한, 각 라벨에 필요한 모델 컴포넌트를 로컬에서 추출한 후 클러스터에 맞게 서버에서 결합하여 사용자 개별 데이터 패턴에 최적화된 업데이트를 생성합니다. 이는 비독립적이며 동일하게 분포되지 않은(non-IID) 데이터 환경에서 효과적입니다.

- **Performance Highlights**: 세 가지 실제 사용 사례(인간 활동 인식(HAR), 스트레스 감지, 수면 감지)에서 FedSub의 성능을 검증하였으며, 기존의 최첨단 방법과 비교해 분류 정확도와 모델 수렴 속도가 크게 향상되었습니다. 이러한 결과는 고도의 데이터 변동성과 장치 이질성을 특징으로 하는 환경에서 개인화된 접근의 중요성을 강조합니다.



### Measuring similarity between embedding spaces using induced neighborhood graphs (https://arxiv.org/abs/2411.08687)
- **What's New**: 본 논문에서는 Nearest Neighbor Graph Similarity (NNGS)라는 새로운 메트릭을 통해 쌍의 임베딩(embedding) 표현 간의 유사성을 측정하는 방법을 제안합니다.

- **Technical Details**: NNGS는 각 표현의 이웃 그래프(nearest-neighbors induced graphs) 간의 구조적 유사성을 기반으로 하며, 다양한 거리 메트릭과 이웃 크기를 기반으로 공간을 비교할 수 있도록 구성될 수 있습니다.

- **Performance Highlights**: GloVe 임베딩을 이용한 유추 과제와 CIFAR-100 데이터셋에서 CLIP 임베딩을 이용한 제로 샷(zero-shot) 분류 실험을 통해, 제안된 메트릭이 성능 차이를 설명하는 데 도움이 되며, 유추와 제로 샷 분류에서의 정확도는 임베딩 유사성과 연관성이 있음을 보여줍니다.



### UniMat: Unifying Materials Embeddings through Multi-modal Learning (https://arxiv.org/abs/2411.08664)
- **What's New**: 이번 연구에서는 재료 과학의 다양한 변조체(모달리티)를 통합하기 위한 최신 멀티모달 AI 기술인 'UniMat'을 도입합니다. UniMat은 XRD 패턴, 구조 그래프 및 성분 정보를 통합하여 재료의 특성을 효과적으로 예측할 수 있게 합니다.

- **Technical Details**: 이 연구에서 사용한 기법은 Graph Neural Networks (GNNs)와 Convolutional Neural Networks (CNNs)로, GNN은 구조 그래프를 인코딩하고, CNN은 XRD 패턴을 분석하여 중요한 피크를 학습합니다. 서로 다른 모달리티의 임베딩을 정렬하기 위해 대조 손실 함수(contrastive loss)를 사용하여 동일한 재료와 다른 재료 간의 유사성을 극대화합니다.

- **Performance Highlights**: XRD 패턴과 화학 조성을 융합하고 정렬함으로써 단일 모달리티 모델과 비교하여 예측 정확도가 향상되었습니다. 연구에서 제안하는 방법은 구조 그래프 데이터에 강력히 의존하는 모델과 동일한 수준의 퍼포먼스를 달성하였습니다.



### Accelerating Quasi-Static Time Series Simulations with Foundation Models (https://arxiv.org/abs/2411.08652)
Comments:
          Equal contributors: A.P. and F.M.; Lead contact: A.P

- **What's New**: 최근 제안된 Grid Foundation Model (GridFM)을 활용하여 Neural Power Flow Solver의 경제성을 개선하는 방법을 제시합니다. GridFM은 다양한 전력망 운영 및 계획 작업에 대한 기초 모델로, 최소한의 파인튜닝만으로 적용할 수 있습니다.

- **Technical Details**: Quasi-static time series (QSTS) 시뮬레이션은 전력망의 안정적인 전력 흐름을 식별하기 위한 고급 시뮬레이션 기술입니다. 이 과정에서 Neural Power Flow Solver는 비선형 방정식을 반복적으로 풀이하는 대신, 입력을 직접 출력 공간에 매핑하여, 보다 빠르고 효율적인 문제 해결을 가능하게 합니다. 특히 그래프 신경망 (Graph Neural Networks, GNN)을 기반으로 한 접근 방식은 전통적인 방법보다 3-4배 더 빠른 성능을 보여줍니다.

- **Performance Highlights**: 이 논문에서는 IEEE 테스트 케이스에서 118개의 버스까지의 전통적인 솔버와 비교하여 3-4배의 성능 향상을 달성했음을 강조합니다. Neural Power Flow Solver는 전통적인 방식에서 발생하는 수치적 불안정성 문제를 해결하며, 대규모 및 복잡한 전력망에서도 유효하게 작동할 가능성을 보여줍니다.
...
### Gaussian Mixture Models Based Augmentation Enhances GNN Generalization (https://arxiv.org/abs/2411.08638)
- **What's New**: 이 연구는 그래프 신경망(Graph Neural Networks, GNNs)의 일반화 능력을 향상시키기 위한 새로운 데이터 증강 기술인 GMM-GDA(Gaussian Mixture Model-based Graph Data Augmentation)를 소개합니다. 이 기술은 이론적 틀을 통해 데이터 증강의 효과를 분석하며, 기존 방법들과 비교하여 뛰어난 성능을 보입니다.

- **Technical Details**: 제안된 GMM-GDA는 Gaussian Mixture Models를 활용하여 그래프의 최종 은닉 표현에서 효율적으로 데이터를 증강합니다. 이 방법은 Expectation-Maximization (EM) 알고리즘을 적용하여 GMM을 훈련하고, 이를 통해 새로운 그래프 표현을 샘플링하여 훈련 데이터의 다양성을 증가시킵니다.

- **Performance Highlights**: 이 연구의 결과, GMM-GDA는 기존의 데이터 증강 기술들보다 일반화 성능이 우수하며, 시간 복잡도 또한 개선되어 실제 응용에서 높은 적용 가능성을 지닙니다.



### Robot See, Robot Do: Imitation Reward for Noisy Financial Environments (https://arxiv.org/abs/2411.08637)
- **What's New**: 이 논문은 강화 학습(Reinforcement Learning, RL) 프레임워크에서의 재무 자산 거래에 적용되는 새로운 보상 함수(reward function)를 도입합니다. 이 보상 함수는 모방 학습(imitation learning)을 활용하여 전문가의 피드백을 통합하는 방식으로, 재무 시장의 높은 변동성과 저신호 대 잡음 비율(noise ratio)을 극복하려고 합니다.

- **Technical Details**: 이 연구는 모방 학습 기법을 활용하여 전문가의 행동([expert's actions])을 따르는 보상 함수 설계를 제안합니다. 이를 통해 에이전트는 자신의 경험(강화 피드백) 뿐만 아니라 전문가의 행동(모방 피드백)에서도 배울 수 있습니다. 논문에서는 마르코프 의사 결정 과정(Markov Decision Process, MDP)으로 환경을 정의하고, 기법을 적용하여 무형 모델 프레임워크(model-free RL framework)에서 보상 신호의 확률적 노이즈를 처리합니다.

- **Performance Highlights**: 실험 결과, 제안한 방법이 전통적 벤치마크 및 단순 강화 피드백으로 훈련된 RL 에이전트보다 재무 성과 관련 메트릭에서 유의미한 향상을 보였음을 보여줍니다. 이는 모방 학습의 효과가 강화 학습의 정책 학습(policy learning)을 더 견고하게 만든다는 것을 시사합니다.



### Hopfield-Fenchel-Young Networks: A Unified Framework for Associative Memory Retrieva (https://arxiv.org/abs/2411.08590)
Comments:
          49 pages, 14 figures. arXiv admin note: text overlap with arXiv:2402.13725

- **What's New**: 본 논문에서는 Hopfield 모델과 그것의 현대적 변형들을 확대하는 Hopfield-Fenchel-Young 네트워크라는 통합 프레임워크를 소개합니다. 이 프레임워크는 에너지 함수의 폭넓은 패밀리로 일반화되며, 기존의 Hopfield 네트워크의 한계를 극복할 수 있는 가능성을 제시합니다.

- **Technical Details**: 우리는 Fenchel-Young 손실 함수의 차이로 표현되는 에너지 함수를 고려합니다. 이 에너지는 Hopfield 스코어링 메커니즘과 후처리 변환을 정의하며, Tsallis와 norm 엔트로피를 활용하여 희소한 업데이트 규칙을 도출합니다. 이 프레임워크는 기존 Hopfield 네트워크의 전통적인 및 현대적 모델을 통합하는 데 기여하며, 에너지 최소화 관점에서 널리 사용되는 후처리 변환들을 설명합니다.

- **Performance Highlights**: 실험 결과는 자유 및 순차 기억 회상 작업을 포함한 다양한 메모리 회상 작업에서의 Hopfield-Fenchel-Young 네트워크의 효과를 입증합니다. 시뮬레이션 데이터, 이미지 검색, 다중 인스턴스 학습 및 텍스트 합리화 실험에서 우리의 방법론의 유용성이 확인되었습니다.
...
### Learning Locally Adaptive Metrics that Enhance Structural Representation with $\texttt{LAMINAR}$ (https://arxiv.org/abs/2411.08557)
Comments:
          Accepted to the NeurIPS 2024 Machine Learning and the Physical Sciences workshop. 6 pages, 6 figures

- **What's New**: 이 논문에서는 구조화된 데이터 세트를 개선하기 위해 더 정보적인 거리 메트릭을 생성하는 비지도 기계 학습 파이프라인인 \texttt{LAMINAR}를 소개합니다. 기존의 유클리드 메트릭이 복잡한 데이터 세트의 구조를 정확히 포착하지 못할 경우가 많아, \texttt{LAMINAR}는 Riemannian manifold를 사용하여 데이터 공간에서 카테고리 적인 메트릭을 사용자에게 사전 지정할 필요 없이 정의합니다.

- **Technical Details**: \texttt{LAMINAR}는 비가역 네트워크를 사용하여 국부적 적응 메트릭을 찾고, 입력 데이터 세트의 두 점 사이의 구조적으로 대표적인 거리를 계산합니다. 이 과정은 데이터의 국소성을 유지하면서 d차원 데이터를 단위 구의 균일 분포로 변환하는 것에서 시작됩니다. 이어서, pseudo-cdf에서 유클리드 k-최근접 이웃 그래프를 정의하여 구조적으로 대표적인 이웃 그리고 거리 메트릭을 계산합니다.

- **Performance Highlights**: \texttt{LAMINAR}의 출력은 유클리드 메트릭과 비교하여 구조화된 데이터 세트에서 우수한 성능을 보여주며, 이는 더 많은 정보를 함축한 밀도 기반 거리 메트릭의 중요성을 증명하는 결과로 나타납니다.
...
### Graph Neural Networks in Supply Chain Analytics and Optimization: Concepts, Perspectives, Dataset and Benchmarks (https://arxiv.org/abs/2411.08550)
Comments:
          27 Pages. Extended journal version of SupplyGraph (arXiv:2401.15299). In Review

- **What's New**: 본 논문은 Graph Neural Networks (GNNs)의 공급망 관리(Supply Chain Management, SCM)에 대한 응용 가능성을 탐구합니다. 공급망이 본질적으로 그래프와 유사하게 구성되어 있어 GNN 방법론이 복잡한 문제를 최적화하고 해결하는 데 유리합니다. 또한, 세계적 FMCG 기업의 다각적 실세계 벤치마크 데이터셋을 제시하여 GNN의 성능을 벤치마킹합니다.

- **Technical Details**: 이 연구는 공급망을 그래프 구조로 연결하여 GNN의 효과적인 응용을 위한 구체적인 수식과 예제를 제시합니다. GNN 기반 모델은 회귀(regression), 분류(classification), 탐지(detection), 이상 탐지(anomaly detection) 등 여러 공급망 분석 작업에서 기존의 통계적 머신러닝 및 딥러닝 모델보다 10-40% 뛰어난 성능을 보였습니다. 여기에는 동형(homogeneous) 및 이형(heterogeneous) 그래프에서의 성과도 포함됩니다.

- **Performance Highlights**: GNN 기반 모델은 공급망에서 다양한 작업을 수행하며, 통계적 머신러닝 및 다른 딥러닝 모델과 비교하여 일관되게 우수한 성능을 보였습니다. 특히, 회귀 작업에서 10-30%, 분류 및 탐지 작업에서 10-30%, 이상 탐지 작업에서는 15-40% 더 나은 결과를 기록했습니다. 이 연구는 GNN을 활용한 공급망 분석 및 모델링의 기초를 다집니다.
...
### Properties of fairness measures in the context of varying class imbalance and protected group ratios (https://arxiv.org/abs/2411.08425)
- **What's New**: 본 논문은 보호 그룹(protected group) 비율의 변화와 클래스 불균형(class imbalance)이 공정성(fairness) 조치에 미치는 영향을 분석합니다. 기존의 불공정성 평가 방법들이 데이터셋에 의존하고 있었던 반면, 이 연구는 데이터셋 독립적인 일반 공정성 속성을 제안합니다.

- **Technical Details**: 연구에서는 6개의 인기 있는 그룹 공정성 측정을 위한 확률 질량 함수(probability mass functions)를 분석하였습니다. 이 논문은 클래스 불균형 비율이 변동할 때 최적의 공정성 성취 확률이 어떻게 변화하는지를 측정하였습니다. 또한, 공정성 조치가 실생활 작업에서 분류기의 공정성과 어떻게 관련되는지를 설명합니다.

- **Performance Highlights**: 연구 결과, Equal Opportunity와 Positive Predictive Parity처럼 특정 공정성 조치들은 Accuracy Equality보다 클래스 불균형에 더 민감하다는 것을 보여주었습니다. 이러한 발견은 연구자와 실무자들이 분류 문제에 적합한 공정성 측정을 선택하는 데 도움을 줄 수 있습니다.
...
### Federated Graph Learning with Graphless Clients (https://arxiv.org/abs/2411.08374)
Comments:
          Accepted by Transactions on Machine Learning Research (TMLR)

- **What's New**: 본 논문에서는 그래프 데이터가 없는 클라이언트가 포함된 Federated Graph Learning (FGL)의 새로운 문제를 해결하기 위한 프레임워크인 FedGLS를 제안합니다. 이는 다양한 클라이언트가 각자의 그래프 데이터를 보유하지만 모든 클라이언트가 그래프 구조를 갖지 못할 경우 공동으로 머신러닝 모델을 훈련하는 것을 목표로 합니다.

- **Technical Details**: FedGLS는 각 그래프 없는 클라이언트에서 지역 그래프 구조를 학습할 수 있는 로컬 그래프 학습기를 구현하고, 다른 클라이언트로부터 구조 지식을 전송받으며 지역 훈련 중에 GNN 모델과 피처 인코더를 통해 구조 지식을 유지합니다. 피처 인코더는 지식 증류(knowledge distillation) 기법을 통해 GNN 모델의 출력을 근사화합니다. 중앙 서버가 클라이언트로부터 모은 로컬 파라미터를 통해 전세계적 파라미터를 생성하게 됩니다.

- **Performance Highlights**: 광범위한 실험 결과에서 제안된 FedGLS가 다섯 개의 기준선(baselines)보다 우수한 성능을 보였으며, 특히 그래프 없는 클라이언트와 협력하여 GNN 모델을 성공적으로 훈련할 수 있음을 입증했습니다.



### Coverage Analysis for Digital Cousin Selection -- Improving Multi-Environment Q-Learning (https://arxiv.org/abs/2411.08360)
Comments:
          This work has been submitted to the IEEE for possible publication

- **What's New**: 본 논문에서는 MEMQ(multi-environment mixed Q-learning) 알고리즘의 데이터 커버리지 조건을 최적화하기 위한 포괄적인 확률론적 분석을 수행합니다. 이를 통해 MEMQ 알고리즘의 성능 개선과 효율적인 환경 비교 방법을 제시합니다.

- **Technical Details**: MEMQ 알고리즘은 여러 독립적인 Q-learning 알고리즘을 사용하는 다중 환경에서 학습하며, 다양한 환경에서의 커버리지 계수(coverage coefficient) 분석을 통해 최적의 성능을 확보합니다. 우리의 접근법은 MEMQ 알고리즘의 Q-function을 업데이트하는 동시에, 서로 다른 환경의 유틸리티를 비교하는 간단한 방법을 제시합니다.

- **Performance Highlights**: 우리의 새로운 CC 기반 MEMQ 알고리즘은 평균 정책 오류(Average Policy Error, APE)를 65% 줄이고, 전체 검색(Exhaustive Search)보다 95% 빠르며, 기존의 최첨단 강화 학습(Reinforcement Learning) 알고리즘보다 60% 낮은 APE를 달성합니다.



### Neural Conjugate Flows: Physics-informed architectures with flow structur (https://arxiv.org/abs/2411.08326)
- **What's New**: Neural Conjugate Flows (NCF)는 정확한 flow 구조를 갖춘 신경망 아키텍처의 한 종류로, 이는 연속 군에 자연스럽게 동형이며 일반적인 미분 방정식 (ODE)의 흐름에 대한 범용 근사자입니다. 이 구조는 다루기 쉬운 방식으로 토폴로지적 (topological) 특성을 강제할 수 있습니다.

- **Technical Details**: NCF는 topological conjugation의 역동적 시스템 프레임워크를 활용하여 미분 방정식의 솔루션 그룹 구조를 정확히 재현합니다. NCF는 초기 조건과 시간 인과성을 자동으로 준수하며, 자신의 내부 흐름 연산자를 적절히 선택함으로써 ODE의 여러 topological 특성을 강제할 수 있습니다. 이 신경망은 병렬 방식으로 닫힌 형태로 계산될 수 있어 학습 속도가 기존의 flow 기반 아키텍처보다 최대 5배 빠릅니다.

- **Performance Highlights**: NCF는 미분 방정식의 잠재적 역학을 추정하고 외삽하는 데 있어 기존의 물리 정보 신경망 (PINNs)에 비해 뛰어난 성능을 보여주며, 더욱 효율적인 계산 성과를 달성하는 데 기여합니다.



### Conditional Variable Flow Matching: Transforming Conditional Densities with Amortized Conditional Optimal Transpor (https://arxiv.org/abs/2411.08314)
- **What's New**: 논문에서는 Conditional Variable Flow Matching (CVFM)이라는 새로운 프레임워크를 제안합니다. 이는 연속 조건부 변수에서 조건부 분포를 변형하는 흐름을 배울 수 있게 하여, 비정렬된 관측 샘플로부터 시스템의 동역학을 예측하도록 합니다.

- **Technical Details**: CVFM은 두 개의 조건부 흐름, 조건부 Wasserstein 거리, 그리고 조건부 optimal transport을 가능하게 하는 커널을 사용하여 기존의 지속적인 정규화 흐름(CNF) 및 확률적 동역학을 확장합니다. 이 방법은 표본 데이터를 쌍으로 필요로 하지 않으며, 조건부 변수에 대한 여유 있는 훈련을 지원합니다.

- **Performance Highlights**: CVFM은 여러 도전 과제를 통해 입증되었으며, 특히 재료의 내부 구조의 시간적 진화를 모델링하고, 이미지 간 도메인 변환에서 기존 방법들보다 성능이 향상되었습니다.



### SDDBench: A Benchmark for Synthesizable Drug Design (https://arxiv.org/abs/2411.08306)
- **What's New**: 본 연구에서는 약물 디자인 생성 모델과 관련된 새로운 접근법인 데이터 기반의 파라미터를 도입하여 화합물의 합성 가능성을 평가하는 방법을 제시합니다. 우리는 'round-trip score'라는 새로운 메트릭을 통해 합성 경로의 실현 가능성을 직접적으로 평가합니다.

- **Technical Details**: 연구에서는 생성된 분자의 합성 경로 예측을 위해 데이터 기반의 회고합성 계획 도구를 활용합니다. 해당 방법은 기존의 반응 데이터에 학습된 계산 모델을 통해 합성 경로를 예측한 후, 반응 예측 모델을 사용하여 이 경로의 실현 가능성을 평가합니다.

- **Performance Highlights**: 교차 검증을 통해 제안된 round-trip score는 기존 생성 모델들과 비교해 높은 성공률을 보여주었으며, 실현 가능한 합성 경로를 가진 분자들에서 높은 점수를 기록했습니다. 이는 제안된 메트릭이 계산적으로 생성된 분자의 실제 합성 가능성을 평가하는 데 효과적임을 강조합니다.
...
### Least Squares Training of Quadratic Convolutional Neural Networks with Applications to System Theory (https://arxiv.org/abs/2411.08267)
- **What's New**: 본 논문은 2층의 convolutional neural network (CNN) 훈련을 위한 least squares formulation을 제안합니다. 이 방법은 2-norm loss function을 사용하고 정규화 항이 없는 설정에서 전역 최적 해를 찾을 수 있는 해석적 표현을 제공합니다.

- **Technical Details**: 제안된 2층 quadratic convolutional neural network (CQNN)의 훈련은 비슷한 QNN 문제로 변환하여 수행되며, 이로 인해 학습해야 할 가중치의 수가 현저히 감소합니다. 이 방식은 안전-critical 시스템에서의 입력 변화에 대한 출력 민감성을 분석할 수 있게 하여, 시스템 이론에서 유효한 도구로 자리매김할 수 있습니다.

- **Performance Highlights**: least squares CQNN은 기존의 훈련 방법에 비해 훈련 시간을 크게 단축시키면서도 예측 정확도의 최소한의 타협을 유지했습니다. 이 방식은 시스템 식별 문제와 GPS 위치 추정 문제에도 응용되었습니다.
...
### NVCiM-PT: An NVCiM-assisted Prompt Tuning Framework for Edge LLMs (https://arxiv.org/abs/2411.08244)
Comments:
          Accepted by DATE 2025

- **What's New**: 본 연구는 edge LLM에 최적화된 prompt tuning (PT) 프레임워크를 제안합니다. 특히, 비휘발성 메모리 내 컴퓨팅(non-volatile computing-in-memory, NVCiM) 아키텍처의 장점을 활용하여 edge LLM의 파라미터 조정을 혁신적으로 개선하는 방법을 소개합니다.

- **Technical Details**: 제안된 NVCiM 지원 PT 프레임워크는 행렬-행렬 곱셈(matrix-matrix multiplication)으로 핵심 연산을 축소하고, 이를 NVCiM에서 인-사이트(in-situ) 계산을 통해 가속화합니다. 이 방식은 기존의 복잡한 신경망 모델과 정교한 훈련 방법을 필요로 하지 않습니다.

- **Performance Highlights**: 본 논문은 edge LLM의 PT 성능 향상을 위해 NVCiM을 처음으로 활용한 연구로, 자원 제약이 있는 환경에서 효과적으로 사용자 데이터에 대한 지속적인 파인튜닝을 가능하게 합니다.



### Imitation Learning from Observations: An Autoregressive Mixture of Experts Approach (https://arxiv.org/abs/2411.08232)
- **What's New**: 이 논문은 관찰로부터의 모방 학습(imitation learning)에서 새로운 접근 방식을 제안합니다. 오토회귀 혼합 전문가 모델(autoregressive mixture of experts model)을 사용하여 기본 정책을 적합시킵니다.

- **Technical Details**: 이 모델의 매개변수는 두 단계의 프레임워크를 통해 학습됩니다. 첫 번째 단계에서는 제어 입력 시퀀스를 추정하여 문제의 복잡성을 감소시키고, 두 번째 단계에서는 추정된 제어 입력 시퀀스를 사용해 정규화된 최대 우도 추정(maximum-likelihood estimation) 문제를 해결하여 정책을 학습합니다. 또한, 라이프눈 안정성(Lyapunov stability) 제약을 통합하여 다단계 예측의 정확성을 보장합니다.

- **Performance Highlights**: 제안된 프레임워크는 인간 시연에서 수집된 두 개의 자율주행 데이터셋을 통해 검증되었으며, 복잡한 비선형 동역학을 모델링하는 데 실용성을 입증하였습니다.



### Joint Diffusion models in Continual Learning (https://arxiv.org/abs/2411.08224)
- **What's New**: JDCL(Join Diffusion Continual Learning)를 도입하여, 기존의 생성적 리허설(generative rehearsal) 기법을 새로운 joint diffusion 모델과 결합하여 지속적인 학습 문제를 해결하는 방법을 제안합니다.

- **Technical Details**: 지속적인 학습에 있어, JDCL은 생성 모델과 분류 모델을 통합한 공동의 파라미터화(joint parametrization)를 사용하여 기존 데이터에서 발생하는 지식을 효율적으로 전이할 수 있도록 설계되었습니다. 이 모델은 UNet을 사용하여 숨겨진 특징(latent features)을 추출하고, 이를 분류 작업에 활용합니다.

- **Performance Highlights**: JDCL은 다양한 기준 벤치마크에서 최근의 생성적 리허설 기법을 초월하는 성능을 보였으며, 반지도 학습(semi-supervised learning) 설정에서도 경쟁력 있는 성능을 나타냈습니다. 특히, 버퍼 기반의 리허설 기술에 비해 뛰어난 성능을 강조하고 있습니다.
...
### Multi-Agent Stochastic Bandits Robust to Adversarial Corruptions (https://arxiv.org/abs/2411.08167)
- **What's New**: 이 논문은 적대적 오염(adversarial corruption)이 있는 이질적(multi-agent heterogeneous) 다중무장 밴디트(multi-armed bandit) 문제를 연구합니다. 우리는 모든 에이전트가 서로 공유한 오염된 보상 정보를 바탕으로 최적의 누적 보상을 극대화하는 알고리즘을 제안합니다.

- **Technical Details**: 우리가 제안한 DRAA 알고리즘은 에이전트가 독립적으로 결정을 내리면서 협력하는 fully distributed하고 robust한 방법입니다. 이 알고리즘은 epoch 기반(epoch-based)으로 작동하며, 각 에이전트는 자신이 접근할 수 있는 arm을 선택하고, 오염의 영향에 대해서도 강건성을 보입니다.

- **Performance Highlights**: DRAA는 총 오염 수준 C에 대해 O((L / Lmin) C)와 O(log(T) K / Δmin)의 후회(regret) 경계를 달성합니다. 이 알고리즘은 단일 에이전트 MAB 및 동질적(homogeneous) MA2B로의 축소에서도 성능을 개선합니다.



### Tackling Polysemanticity with Neuron Embeddings (https://arxiv.org/abs/2411.08166)
- **What's New**: 이 논문에서는 neuron embeddings(뉴런 임베딩)를 소개하여 다의성을(polysemanticity) 해결하는 새로운 방법을 제안합니다. 이 방법은 뉴런의 고유 데이터셋 예제에서의 의미적 행동을 식별하여 해석을 더 쉽게 만듭니다. GPT2-small 모델에 적용하고 결과를 탐색하기 위한 UI를 제공합니다.

- **Technical Details**: Neuron embeddings는 모델의 내부 표현과 가중치를 기반으로 계산되며, 이는 domain(도메인)과 architecture(아키텍처)에 구애받지 않습니다. 각 뉴런의 입력에 따라 뉴런이 응답하는 정보를 포착하며, 이것은 뉴런의 데이터셋 예제를 클러스터링하여 그 행동을 구성 요소로 나누는 데 도움이 됩니다. 이 과정에서 neuron embedding을 사용하여 다의성의 정도를 측정할 수 있습니다.

- **Performance Highlights**: Neuron embeddings를 이용해 Sparse Auto-Encoders (SAEs)의 품질을 평가할 수 있는 새로운 메트릭스를 제공하며, MNIST 데이터셋에서 간단한 MLP 모델을 훈련한 결과, 이 메트릭스를 SAE 손실에 포함시켜 monosemanticity(단의미성)를 증가시키고, 활성화되지 않는 뉴런의 수를 크게 줄일 수 있음을 보여줍니다.
...
### Impactful Bit-Flip Search on Full-precision Models (https://arxiv.org/abs/2411.08133)
- **What's New**: 본 연구는 Neural Network의 성능을 저하시킬 수 있는 Bit-Flip Attack (BFA)에 대해 집중적으로 다루며, Impactful Bit-Flip Search (IBS)라는 새로운 기법을 제안하여, 주요 비트를 효율적으로 식별하고 전환하는 방법을 제시합니다.

- **Technical Details**: IBS 방법은 부동 소수점 수치가 있는 완전 정밀도 네트워크에서의 비트 변동을 탐지하고 전환할 수 있는 효과적인 접근 방식을 제공합니다. 또한, Weight-Stealth 기법은 모델의 파라미터를 전략적으로 수정하여, 원래 분포 내에서 Float 값이 유지되도록 하여 단순한 범위 검사를 회피합니다.

- **Performance Highlights**: 실험 결과, IBS는 기존의 비트 변동 탐색 방법에 비해 성능 저하를 극대화 시키면서도 비트 변동을 최소화하는 데 있어 효율적임을 입증하였습니다. 기존 메소드와 비교했을 때, 특정 비트의 전환이 전체 네트워크 성능에 미치는 영향을 더 정확하게 평가할 수 있었습니다.
...
### Equitable Length of Stay Prediction for Patients with Learning Disabilities and Multiple Long-term Conditions Using Machine Learning (https://arxiv.org/abs/2411.08048)
Comments:
          13 pages of article with 9 figures; Supplementary material follows after article with 27 pages and 27 figures

- **What's New**: 이번 연구는 웨일스에서 학습 장애(learning disabilities, LD)를 가진 환자 9,618명의 병원 입원 데이터를 분석하고, 전자 건강 기록(electronic health records, EHR)을 사용하여 병원에서의 체류 기간(length of hospital stay, LOS)을 예측하기 위한 기계 학습 모델을 개발했습니다. 이 연구에서는 성별, 인종 집단 간의 모델 성능 차이를 완화하기 위한 편향 완화 알고리즘도 적용했습니다.

- **Technical Details**: 연구는 랜덤 포레스트(random forest, RF) 모델을 사용하여 체류 기간을 예측했으며, AUC(Area Under the Curve) 값은 남성 0.759, 여성 0.756에 달했습니다. 이 모델은 39개의 장기 질환(long-term conditions, LTCs)과 관련된 데이터를 분석하였으며, 병원 입원 기록 62,243건을 활용했습니다. 또한, 발견된 두 가지 편향 완화 알고리즘인 임계값 최적화(threshold optimization)와 확장된 기울기를 이용한 감소 알고리즘(reductions algorithm)을 비교하였습니다.

- **Performance Highlights**: 랜덤 포레스트 모델의 균형 정확도(balanced accuracy)는 남성 0.690, 여성 0.689로 나타났으며, 남성 세 그룹에서의 허위 양성율(false positive rate)과 균형 정확도에서 임계값 최적화 알고리즘이 우수한 성능을 보였습니다. 연구는 EHR 데이터를 사용하여 시장 내 다양한 그룹에 대한 공정한 예측을 가능하게 하는 기계 학습 모델의 잠재력을 입증했습니다.
...
### Oblique Bayesian additive regression trees (https://arxiv.org/abs/2411.08849)
- **What's New**: 이번 연구에서는 Bayesian Additive Regression Trees (BART)의 새로운 접근인 obliqueBART를 개발했습니다. 이 모델은 데이터에 적응하는 결정 규칙을 통해 feature space를 무작위 hyperplane를 따라 분할합니다. 이로 인해 axis-aligned BART보다 더 나은 예측 성능을 보이는 경우가 있음을 입증했습니다.

- **Technical Details**: obliqueBART는 oblique decision rules를 적용하여 결정 트리가 feature space를 임의의 hyperplane을 따라 분할하도록 합니다. 기존의 axis-aligned BART 모델과 비교하여, obliqueBART는 예측 정확도에서 경쟁력을 발휘하며 때때로 더 우수한 성능을 보입니다. 이 연구에서 사용된 데이터셋은 synthetic 및 real-world benchmark 데이터로 다양합니다.

- **Performance Highlights**: 비교 실험 결과, obliqueBART는 axis-aligned BART 및 일반적인 tree ensemble 방법들에 비해 예측 성능에서 유의미하게 개선된 결과를 보여주었습니다. 특히 복잡한 경계선을 정확하게 복원하는 데 필요한 깊이가 더 얕아지는 경향이 있으며, 이는 예측의 정확도 향상으로 이어집니다.
...
### Model agnostic local variable importance for locally dependent relationships (https://arxiv.org/abs/2411.08821)
- **What's New**: 본 논문에서는 머신러닝 모델 결과를 해석하기 위한 새로운 방법론인 CLIQUE(Conditional Local Importance by QUantile Expectations)를 제안합니다. CLIQUE는 현재 사용되고 있는 지역 변수 중요도 측정 방법의 한계를 극복하고, 지역적으로 의존적인 관계를 정확히 반영하며, 다중 클래스 분류(multiclass classification) 문제에도 적용 가능하다는 점에서 혁신적입니다.

- **Technical Details**: CLIQUE는 전역 순열(pemutation) 기술을 지역적 방법으로 변환하여 개별 교차 검증(cross-validation) 오류를 다루고, 무작위 순열 대신 quantile 교체를 사용합니다. 이러한 점에서 CLIQUE는 예측(prediction) 대신 오류(error)를 기반으로 중요도를 산출하여 다중 클래스 문제에서도 손쉽게 적용할 수 있습니다.

- **Performance Highlights**: 시뮬레이션 및 실제 사례 연구에서 CLIQUE는 지역적으로 의존적인 정보를 강조하며, 응답에 영향을 미치지 않는 변수에 대해서는 중요도를 거의 영(near zero)으로 산출하는 것을 보여주었습니다. 이로써 기존 방법들에서 발생할 수 있는 잘못된 해석을 줄이는 데 기여합니다.
...
### FinRobot: AI Agent for Equity Research and Valuation with Large Language Models (https://arxiv.org/abs/2411.08804)
Comments:
          The 1st Workshop on LLMs and Generative AI for Finance, ICAIF 2024

- **What's New**: FinRobot은 주식 연구를 위해 특별히 설계된 최초의 AI 에이전트 프레임워크입니다. 이 시스템은 다중 에이전트 Chain of Thought (CoT) 시스템을 사용하여 양적 및 질적 분석을 결합하여 인간 분석가의 종합적인 추론을 에뮬레이션합니다.

- **Technical Details**: FinRobot은 데이터 수집, 재무 개념 해석, 보고서 작성의 세 가지 전문 에이전트(Data-CoT Agent, Concept-CoT Agent, Thesis-CoT Agent)로 구성되어 있습니다. 이 구조를 통해 포괄적이고 즉각적인 재무 분석을 제공합니다.

- **Performance Highlights**: FinRobot은 기존 자동화 연구 도구와 비교하여 대형 증권 회사 및 기본 연구 공급자가 제공하는 통찰력과 동등하거나 더 나은 수준의 분석을 제공합니다. 실시간 데이터 파이프라인을 통해 최신의 관련 정보를 제공하며, 리스크 평가의 현실성을 통해 금융 분석의 정확성과 실용성을 향상시킵니다.



### Deep Learning Accelerated Quantum Transport Simulations in Nanoelectronics: From Break Junctions to Field-Effect Transistors (https://arxiv.org/abs/2411.08800)
Comments:
          10 pages, 4 figures

- **What's New**: 이번 논문에서는 정확성과 계산 효율성의 절충을 극복하기 위해 딥 러닝 기반의 tight-binding Hamiltonian 모델인 DeePTB를 비평형 Green의 함수(NEGF) 방법과 결합한 새로운 프레임워크인 DeePTB-NEGF를 제안합니다. 이를 통해 나노 전자 소자에 대한 효율적인 quantum transport 계산이 가능해졌습니다.

- **Technical Details**: DeePTB-NEGF 프레임워크는 첫 번째 원칙의 정확성을 유지하면서도 SCF(iterations)를 피할 수 있는 혁신적인 접근 방식입니다. DeePTB는 환경에 따라 달라지는 Slater-Koster tight-binding Hamiltonians와 DFT Hamiltonians을 예측하는 두 가지 보완 전략을 제공합니다. NEGF 방법론을 통해 전이 스펙트럼(T(E))과 같은 다양한 transport properties를 정확히 계산할 수 있습니다.

- **Performance Highlights**: 프레임워크의 유용성은 두 가지 응용 사례—금속 접촉 및 단일 분자 접합에서의 break junction 시스템과 카본 나노튜브 전계 효과 트랜지스터(CNT-FETs) 시뮬레이션—를 통해 입증되었습니다. 두 경우 모두 DFT-NEGF 계산 및 실험 결과와의 뛰어난 일치를 보였으며, 나노 전자 소자에서의 고처리량 quantum transport 계산을 위한 새로운 가능성을 제시하고 있습니다.



### Optimal Oblivious Subspace Embeddings with Near-optimal Sparsity (https://arxiv.org/abs/2411.08773)
- **What's New**: 이번 연구에서는 최적 차원 $m=	heta(d/	heta^2)$을 가지며, 스파르시티(sparsity)가 $	ilde O(1/	heta)$인 새로운 oblivious subspace embedding을 제안했습니다. 이는 Nelson과 Nguyen의 추측을 거의 충족시키며, 이전의 스파르시티 경계인 $	ilde O(1/	heta^6)$보다 향상된 결과입니다.

- **Technical Details**: 제안된 oblivious subspace embedding은 모든 d차원 서브스페이스의 벡터의 norm을 $1 	imes 	heta$ 범위 내에서 보존하도록 설계된 랜덤한 $m 	imes n$ 행렬 $	ext{Pi}$입니다. 또한, 우리는 비-oblivious(non-oblivious) 설정을 확장하여, 독립 칼럼을 가진 Leverage Score Sparsified embeddings의 새로운 가족을 제안합니다.

- **Performance Highlights**: 제안된 접근 방식은 매트릭스 근사(matrix approximation) 및 회귀(regression) 작업을 위한 빠른 런타임을 제공합니다. 분석에서는 균등성과 관련된 경계를 제공하기 위해 새로운 방법론을 개발하였으며, 이 과정에서 isotropic 랜덤 행렬의 에지 유니버설리티 오류를 바운딩하기 위한 cumulant 방법을 사용했습니다.



### Energy Dissipation Preserving Physics Informed Neural Network for Allen-Cahn Equations (https://arxiv.org/abs/2411.08760)
- **What's New**: 이 논문은 Allen-Cahn 방정식의 수치적 해법을 다루며, 이를 위해 Physics-Informed Neural Network (PINN)을 활용합니다. 특히, 저자들은 PINN의 학습 능력을 향상시키기 위해 Allen-Cahn 방정식의 에너지 소산 성질을 손실 함수에 패널티 항으로 포함했습니다. 또한, 초기 조건이 랜덤할 때 학습 과정을 용이하게 하기 위해 Fourier 급수를 이용해 연속적인 초기 랜덤 조건을 활용했습니다.

- **Technical Details**: 본 연구에서는 일정 및 퇴화 이동성(degenerate mobility)을 가진 Allen-Cahn 방정식을 검토하며, 폴리노미얼(polynomial)과 로그(logarithmic) 에너지 함수형을 사용합니다. 이 연구는 하나의 공간 차원에서뿐만 아니라 두 개 및 세 개의 공간 차원에서 운동 방정식과 아드벡티브 항을 포함합니다. 전통적인 수치 해석의 적응형 방법이 PINN에 통합되어 제안된 방법의 효과성을 향상시킵니다.

- **Performance Highlights**: 수치적 결과는 이산 에너지의 일관된 감소를 보여주며, 상 분리(phase separation) 및 메타스테이블(metastability)과 같은 현상도 나타났습니다. 제안된 PINN은 에너지 소산을 보장하며 Allen-Cahn 방정식의 동역학 학습을 보다 정확하게 수행하는 데 핵심적인 역할을 하고 있습니다.
...
### Optimal Transport-Based Displacement Interpolation with Data Augmentation for Reduced Order Modeling of Nonlinear Dynamical Systems (https://arxiv.org/abs/2411.08750)
- **What's New**: 이 논문에서는 비선형 동역학을 모델링하기 위한 새로운 저차원 모델(Reduced-Order Model, ROM)을 제안합니다. 본 연구는 Optimal Transport(OT) 이론과 변위 보간(displacement interpolation)을 활용하여 복잡한 시스템의 동역학을 개선합니다.

- **Technical Details**: 제안된 방식은 OT 원칙에 기반한 데이터 증강(data augmentation) 전략을 통해 한정된 데이터 환경에서도 효과적으로 작동합니다. 이 방법론은 데이터 간의 지오데식 경로를 따라 보간된 솔루션을 생성하며, 가상의 시간-물리적 맵을 통해 연속적인 해의 표현을 제공합니다. 이를 통해 시간적 스케일을 더 세밀하게 구성할 수 있습니다.

- **Performance Highlights**: 대기 중형 메조 스케일 벤치마크에서 제안된 방법론의 효과를 입증하였으며, 복잡한 시스템의 행동 예측에서 정확도와 효율성이 향상되었습니다. 이는 컴퓨테이셔널 물리학 및 엔지니어링 분야에서의 광범위한 적용 가능성을 시사합니다.



### Recommender systems and reinforcement learning for building control and occupant interaction: A text-mining driven review of scientific literatur (https://arxiv.org/abs/2411.08734)
- **What's New**: 본 연구는 건물 제어 및 거주자 상호작용에서 추천 시스템(Recommendation Systems)과 강화 학습(Reinforcement Learning)이 어떻게 적용되는지를 탐구하였으며, 이 과정에서 약 27,000개의 논문을 분석하여 실내 환경 최적화와 에너지 효율성 향상에서의 새로운 가능성을 강조합니다.

- **Technical Details**: 텍스트 마이닝(Text Mining)과 자연어 처리(Natural Language Processing, NLP) 기술을 통해, 추천 알고리즘, 데이터 입력, 개입/목표 및 플랫폼 간의 관계를 파악하기 위한 체계적인 6단계 프로세스가 마련되었습니다. 이 연구는 데이터 기반 기술을 활용하여 방대한 문헌 데이터를 효과적으로 분석하는 것을 목표로 합니다.

- **Performance Highlights**: 추천 시스템과 강화 학습의 광범위한 적용에도 불구하고, 실내 환경 최적화 및 에너지 효율성 향상에 대한 사용은 제한적입니다. 고급 기계 학습 기술이 필요하며, 혁신 영역으로는 예측 유지보수, 건물 관련 제품 추천, 수면 및 생산성 향상을 위한 환경 최적화 등이 제안되었습니다.
...
### Towards Secure Intelligent O-RAN Architecture: Vulnerabilities, Threats and Promising Technical Solutions using LLMs (https://arxiv.org/abs/2411.08640)
Comments:
          10 pages

- **What's New**: 최근 O-RAN(개방형 무선 접속망) 아키텍처의 보안 위협을 심층적으로 분석하고, 이를 위한 새로운 기술적 접근을 제안합니다. 특히 제로 트러스트(Zero Trust), 이동 목표 방어(Moving Target Defense, MTD), 블록체인, 대규모 언어 모델(Large Language Models, LLM) 등을 활용하여 O-RAN의 보안을 강화하는 방안을 모색합니다.

- **Technical Details**: O-RAN 아키텍처는 RAN의 가상화와 분리, API(Open Interface) 개발 및 AI/ML 기술을 이용하여 지능형 RAN 레이어를 개발합니다. 또한, 다양한 서비스 요청을 지원하는 네트워크 슬라이싱을 통해 RAN 성능을 향상시키며, CIA(Confidentiality, Integrity, Availability) 측면에서 보안 위협을 분석하고 이러한 위협을 완화하기 위한 기술 적용 방안을 제안합니다. 특히 LLM의 설명 가능한 AI(XAI) 기능을 적용해 시스템의 취약점을 탐지하는 방법을 설명합니다.

- **Performance Highlights**: MTD 기술을 활용하여 O-RAN 아키텍처에서 동적 네트워크 슬라이스 수용 제어를 위한 심층 강화 학습(Deep Reinforcement Learning, DRL) 방법의 강인성을 증대시키는 효과를 수치적으로 입증했습니다. 이 연구는 O-RAN 보안 강화를 위한 새로운 솔루션을 제시하며, 시스템의 보안을 한층 강화하는 길을 제시합니다.



### Deep Generative Demand Learning for Newsvendor and Pricing (https://arxiv.org/abs/2411.08631)
Comments:
          30 pages, 6 figures

- **What's New**: 이번 논문은 feature-based newsvendor 문제에서 가격과 맥락적 특성이 수요에 미치는 영향을 고려한 데이터 기반 재고 및 가격 결정 방식을 제안합니다. 기존의 이론적인 가정 없이 수요 분포를 모델링하고, 가격 변동성이 있는 상황에서의 의사 결정을 위해 조건부 심층 생성 모델(cDGMs)을 활용합니다.

- **Technical Details**: 조건부 심층 생성 모델(cDGMs)은 가격과 맥락적 특성에 따라 조건화된 probabilistic demand 토대를 학습하고 이를 통해 정확한 이익 추정을 가능하게 합니다. 이 모델은 재고와 가격 최적화를 동시에 고려하며, latent variable 기반의 cDGMs(예: VAEs, GANs)를 채택하여 실용적으로 구현됩니다.

- **Performance Highlights**: 광범위한 시뮬레이션 및 실제 사례 연구를 통해 제안된 방법이 전통적인 접근법보다 뛰어난 성능을 보임을 입증했습니다. 특히, 텍스트 특성을 포함하면 이익이 증가하는 효과가 있음을 확인했습니다.
...
### Grammarization-Based Grasping with Deep Multi-Autoencoder Latent Space Exploration by Reinforcement Learning Agen (https://arxiv.org/abs/2411.08566)
Comments:
          Submitted for review at IEEE ICRA 2025

- **What's New**: 본 논문에서는 로봇이 비구조적인 환경에서 물체를 효과적으로 잡기 위한 새로운 프레임워크를 제안합니다. 이 방법은 다양한 물체의 기하학 및 물리적 특성을 압축하여 공통 잠재 공간(common latent space)에서 표현하도록 하며, 이를 위해 세 가지 오토인코더(autoencoder)를 활용합니다.

- **Technical Details**: 프레임워크는 세 개의 오토인코더를 사용하여 목표(target) 물체, 그리퍼(gripper), 그리고 이들 간의 잠재 표현(latent representations)을 융합합니다. 강화 학습(RL) 에이전트는 객체의 명시적 재구성이 없어도 높은 품질의 그립을 탐색합니다. 또한 PoWER 알고리즘을 통해 보상 가중치 잠재 공간에서의 정책 업데이트를 수행합니다.

- **Performance Highlights**: 다양한 물체에 대한 실험에서 우리의 시스템은 minimum computational overhead인 상황에서도 높은 성공률을 기록하였고, 시뮬레이션 실험에서 RL 에이전트의 적응성이 35% 이상 향상되었습니다.
...
### Quantifying Qualitative Insights: Leveraging LLMs to Market Predic (https://arxiv.org/abs/2411.08404)
Comments:
          7 pages, 4 figures

- **What's New**: 최근 대규모 언어 모델(LLMs)의 발전은 수치 및 텍스트 데이터를 통합하여 금융 분석을 변혁할 잠재력을 지니고 있습니다. 하지만 다양한 정보(정보를 융합하는 과정에서의 충분한 맥락 부족과 LLM이 생성하는 질적 출력의 효용 측정 어려움)가 이를 저해하고 있습니다. 본 연구는 증권사 일일 리포트를 활용해 질 높은 맥락 정보를 생성하고, 이를 텍스트 기반 주요 요소와 수치 데이터로 분할하여 통합하는 방법을 제시합니다.

- **Technical Details**: 자본시장 정보는 LLM의 금융 추론을 위한 맥락을 제공하는 데 사용됩니다. 본 연구에서는 가장 조회 수가 높은 증권사 리포트의 상위 3개를 선정하여 사용하며, 이는 개인 투자자들에게 유용합니다. 이후 LLM의 텍스트 생성에서 생성 결과의 확률 기반 토큰 선택을 거치며, 샘플링 프로세스에서 온도를 조절하여 다양성을 조절합니다. 본 연구는 또한 질적 통찰력을 정량적 결과로 변환하며,  리커트 척도(Likert scale)를 통해 점수를 할당하는 새로운 프로프트를 개발했습니다.

- **Performance Highlights**: 실험 결과, LLM은 시장 예측에서 시계열 모델보다 우수한 성능을 보였습니다. 그러나 출력의 일관성 부족과 설명 가능성 제한과 같은 도전과제들이 여전히 존재합니다.
...
### Communication Efficient Decentralization for Smoothed Online Convex Optimization (https://arxiv.org/abs/2411.08355)
Comments:
          39 pages

- **What's New**: 본 연구는 다중 에이전트 Smoothed Online Convex Optimization (SOCO) 문제를 다루며, 첫 번째로 비중심화 알고리즘을 제안합니다.

- **Technical Details**: 각 에이전트는 강한 볼록성(strongly convex) 비용 함수와 시간을 고려한 '스위칭 비용' 및 공간적 '비유사성 비용'을 최적화해야 합니다. 제안된 알고리즘은 주변 정보만을 사용하여 동작 가능하며, 통신 그래프가 동적으로 변화할 때도 성능을 유지합니다.

- **Performance Highlights**: 제안된 알고리즘은 경쟁 비율(competitive ratio)에서 근사 최적성을 달성하며, 에이전트 수에 로그 방식으로만 의존하여 대규모 시스템 확장성(scalability)이 보장됩니다.
...
### Learning-Augmented Algorithms for Online Concave Packing and Convex Covering Problems (https://arxiv.org/abs/2411.08332)
Comments:
          38 pages. In submission

- **What's New**: 이 논문에서는 머신 러닝을 활용하여 전통적인 알고리즘의 성능을 향상시키기 위한 학습-증강 알고리즘(learning-augmented algorithms)의 새로운 프레임워크를 제안합니다. 온라인 포장(maximization) 및 온라인 커버(minimization) 문제의 두 가지 기초적인 최적화 설정을 다루며, 과거 연구를 확장하고 일반화합니다.

- **Technical Details**: 제안된 알고리즘은 온라인 포장 및 커버 최적화 문제를 위한 것으로, 각각 오목(Concave) 및 볼록(Convex) 목표를 가진 문제에서 기존의 알고리즘들보다 우수한 성능을 보입니다. 특히, 오목 목표에 대해서는 '전환(switching)' 전략을 사용하며, 볼록 목표에 대해서는 기본-이중(primal-dual) 기반의 프레임워크를 제안하여 많은 새로운 응용 분야에 적용할 수 있도록 합니다.

- **Performance Highlights**: 우리의 알고리즘은 제공된 조언이 정확할 경우 기존의 불가능성 결과를 극복하고, 조언이 부정확하더라도 고전적인 온라인 알고리즘과 비교해 유사한 성능을 유지하는 것을 보였습니다. 다양한 온라인 최적화 문제에 적용하여 실제 활용 가능성을 보여주었습니다.
...
### SynapsNet: Enhancing Neuronal Population Dynamics Modeling via Learning Functional Connectivity (https://arxiv.org/abs/2411.08221)
- **What's New**: 본 논문은 SynapsNet이라는 새로운 딥러닝 프레임워크를 소개하여 신경 집단의 동적 모델링과 기능적 연결성을 효과적으로 설명합니다. SynapsNet은 생물학적으로 현실적인 접근 방식을 통해 각 신경세포가 보내고 받는 전류를 모델링하며, 동적 연결성을 통해 인과관계를 이해하게 합니다.

- **Technical Details**: SynapsNet은 각 신경세포의 과거 활동, 연결된 신경세포로부터의 입력 전류, 신경세포의 내재적 속성, 행동 데이터를 통합하여 미래의 신경 활동을 예측합니다. 이 모델은 호지킨-헉슬리 방정식을 기반으로 하여 신경 활동을 예측하며, 기능적 연결성(fFunctional Connectivity)을 통해 신경세포 간의 상호작용을 포착합니다.

- **Performance Highlights**: SynapsNet은 공개 데이터셋에서 마우스의 피질 활동을 사용하여 수행된 실험에서, 기존의 모델들을 지속적으로 초월하는 성능을 보여주었습니다. 실제 및 합성 데이터에서도 기능적 연결성을 정확하게 학습하여 신경세포 간의 예측적 상호작용을 효과적으로 노출했습니다.
...
### Fault Localization in Deep Learning-based Software: A System-level Approach (https://arxiv.org/abs/2411.08172)
- **What's New**: FL4Deep는 딥러닝 (Deep Learning, DL) 기반 시스템에서 발생할 수 있는 결함을 효율적으로 탐지하고 위치를 파악하기 위해 전체 DL 개발 파이프라인을 고려한 시스템 수준의 결함 위치 찾기 접근 방식을 제안합니다.

- **Technical Details**: FL4Deep는 정적 및 동적 정보를 추출하여 Knowledge Graph (KG)를 구성하고, 이를 통해 딥러닝 모델의 학습에 사용된 데이터셋 특성 및 하이퍼파라미터와 같은 시스템 특징을 표현합니다. 이 방법은 과거의 여러 결함 위치 찾기 기법들을 결합하여 전체 DL 파이프라인에 걸쳐 결함을 목표로 합니다.

- **Performance Highlights**: FL4Deep는 100개의 결함을 가진 DL 스크립트 샘플을 평가한 결과, 이전의 4가지 접근 방식보다 정확도 면에서 더 우수한 성능을 보였으며, 특히 데이터 문제(84%), 훈련과 배포 간 불일치하는 라이브러리(100%), 손실 함수(69%) 관련 결함에 대해 3가지 DL 관련 결함의 정확도에서 개선 효과를 나타냈습니다.



### Emergent field theories from neural networks (https://arxiv.org/abs/2411.08138)
Comments:
          17 pages

- **What's New**: 본 연구는 해밀토니안 시스템(Hamiltonian systems)과 신경망 기반 학습 시스템(neural network-based learning systems) 간의 이중성 관계(duality relation)를 확립했습니다. 또한, 해밀턴-자코비 방정식(Hamilton-Jacobi equations)과 신경망의 활성화 동역학(activation dynamics) 및 학습 동역학(learning dynamics) 사이의 상관관계를 보여주었습니다.

- **Technical Details**: 이 연구는 인공 신경망을 복잡한 동적 시스템으로 이해하고, 활성화 동역학과 학습 동역학이 해밀토니안 시스템의 동적 시스템과 어떻게 등가성(equivalence)을 갖는지를 탐구합니다. 클라인-고든(Klein-Gordon) 필드에 대한 모델링에서, 대칭(weight tensor)이 요구되며, 디랙(Dirac) 필드의 경우 비대칭(anti-symmetric) 텐서 팩터가 필요함을 보여줍니다.

- **Performance Highlights**: 이 논문의 주요 성과는 여러 물리적 시스템을 신경망의 활성화 및 학습 동역학을 통해 모델링할 수 있음을 입증한 것입니다. 이를 통해 디랙 방정식이 학습 시스템에서 어떻게 유도될 수 있는지를 설명하고, 해밀턴 변환에 대해 공변성(covariant)을 유지하는 방법을 제시했습니다.
...
### A Tale of Two Cities: Pessimism and Opportunism in Offline Dynamic Pricing (https://arxiv.org/abs/2411.08126)
- **What's New**: 이번 논문은 데이터 커버리지 가정 없이 오프라인 다이나믹 프라이싱(online dynamic pricing) 전략을 연구합니다. 이는 최적 가격이 오프라인 데이터에 관찰되지 않을 수 있는 경우를 허용하여, 기존의 접근 방식에서 발생할 수 있는 수익 감소 문제를 해결하고자 합니다.

- **Technical Details**: 이 연구에서는 수요 매개변수에 대한 부분적 식별 경계(partial identification bound)를 설정하여 관찰되지 않은 가격을 기반으로 최적 가격 전략을 찾습니다. 또한 비관적(pessimistic) 및 기회주의적(opportunistic) 전략을 포함하여 추정된 정책을 도출합니다. 이는 수요 모델의 고유한 단조성(monotonicity) 속성을 활용한 것입니다.

- **Performance Highlights**: 이론적으로 제안된 두 전략에 대해 비율 최적의 유한 샘플 후회 보장을 제공하며, 실험적으로 새로운 방법들이 우수한 성능을 보임을 입증합니다. 이는 오프라인 데이터가 부족한 환경에서도 효과적인 가격 결정 방법을 제시하여 기업의 지속 가능한 성장과 수익성 향상에 기여할 수 있습니다.



### Explainable Deep Learning Framework for SERS Bio-quantification (https://arxiv.org/abs/2411.08082)
- **What's New**: 본 연구는 SERS(Surface-enhanced Raman spectroscopy) 기반의 생체 지표(biomarker) 정량화 프레임워크를 개발하여, 생화학적 소음의 영향을 줄이고 모델 설명 가능성을 높이는 방법을 제시합니다. 이는 SERS 데이터의 질을 개선하고, 딥 러닝(deep learning) 모델을 이용한 더 정밀한 정량화를 가능하게 합니다.

- **Technical Details**: 682개의 SERS 스펙트럼을 사용하여 소변(media) 내 세로토닌(serotonin) 정량화를 평가하였으며, denoising autoencoder를 통해 스펙트럼을 강화하고 CNN(convolutional neural networks) 및 비전 트랜스포머(vision transformers)를 통해 생체 지표의 정량화에 사용하였습니다. 또한, CRIME(context representative interpretable model explanations) 방법을 통해 모델의 예측 맥락을 설명하였습니다.

- **Performance Highlights**: 세로토닌 정량화는 denoised 스펙트럼을 사용한 CNN 모델을 활용하여 가장 효과적이었고, 평균 절대 오차(mean absolute error)는 0.15 μM, 평균 백분율 오차(mean percentage error)는 4.67%로 나타났습니다. CRIME 방법은 CNN 모델이 6개의 예측 맥락을 제시하며, 이 중 3개는 세로토닌과 관련이 있음이 밝혀졌습니다.



### LoRA-BERT: a Natural Language Processing Model for Robust and Accurate Prediction of long non-coding RNAs (https://arxiv.org/abs/2411.08073)
- **What's New**: 이번 연구에서는 Long non-coding RNA(lncRNA)의 효과적인 분류를 위해 새로운 모델인 LoRA-BERT를 소개합니다. 이전 모델들과의 비교에서 LoRA-BERT는 더 높은 정확성과 효율성을 보여주었으며, 특히 사람(human)과 쥐(mouse)에서의 lncRNA 및 mRNA 예측이 우수하다는 결과를 보였습니다.

- **Technical Details**: LoRA-BERT는 Transformer 아키텍처를 기반으로 하여 주의(attention) 메커니즘을 활용하여 전체 입력 시퀀스의 문맥 정보를 포착합니다. 이를 통해 lncRNA와 mRNA의 구별을 위한 k-mer 패턴과 Open Reading Frame(ORF)과 같은 기능을 사용하여 모델의 성능을 향상시킵니다.

- **Performance Highlights**: LoRA-BERT는 기존의 순차 예측 도구들과 비교했을 때 정확성이 뛰어나며, lncRNA와 mRNA를 모두 포함하여 사람(human)과 쥐(mouse)에서 최첨단 성능을 기록했습니다. 연구 결과는 lncRNA와 mRNA의 특성을 이해하고, 관련 질병의 탐지에 기여할 가능성을 제시합니다.



### Modeling variable guide efficiency in pooled CRISPR screens with ContrastiveVI+ (https://arxiv.org/abs/2411.08072)
Comments:
          15 pages, 4 figures, will be included in the AIDrugX workshop in Neurips 2024

- **What's New**: 이번 연구에서는 CRISPR-Cas9을 이용한 유전자 스크리닝 데이터에서 유전적 방해(induced perturbation)로 인한 변화를 효과적으로 분석하는 새로운 모델, ContrastiveVI+를 제안합니다.

- **Technical Details**: ContrastiveVI+는 컨트롤 샘플로부터 불필요한 변수를 분리하고 유전자 변형의 영향을 받았는지 여부를 추론할 수 있는 생성적 모델링 프레임워크입니다. 이 모델은 세 가지 대규모 Perturb-seq 데이터셋에 적용되어 기존 방법보다 유전자 방해에 의해 유도된 변화를 더 효과적으로 회수합니다.

- **Performance Highlights**: ContrastiveVI+는 Perturb-seq 데이터셋에서 방해가 예상된 셀을 식별하는 데 성공하여, 기능적 결과를 회피한 셀을 더 잘 구분할 수 있음을 보였습니다. 이는 기존의 기법들보다 더 나은 성능을 나타냅니다.



### Intelligent Green Efficiency for Intrusion Detection (https://arxiv.org/abs/2411.08069)
Comments:
          16 pages, 9 tables, FPS 2024 conference

- **What's New**: 이 논문은 인공지능(AI)의 환경적 영향에 대한 우려가 증가함에 따라 Green AI의 필요성을 강조하며, 네트워크 침입 탐지(Network Intrusion Detection) 및 사이버 공격 분류 작업을 중심으로 서로 다른 프로그래밍 언어 및 특성 선택(Feature Selection, FS) 방법을 평가합니다.

- **Technical Details**: 다섯 가지 머신러닝 모델(랜덤 포레스트, XGBoost, LightGBM, 다층 퍼셉트론, 장단기 메모리)과 네 가지 프로그래밍 언어(Python, Java, R, Rust), 세 가지 특성 선택 방법(정보 이득, 재귀적 특성 제거, 카이제곱)을 사용하여 실험이 진행됩니다. 필터, 래퍼, 임베디드 방법으로 FS를 평가하며, 각 방법의 특징과 장단회에 대해 서술합니다. 또한 프로그래밍 언어의 선택이 에너지 사용과 계산 속도에 미치는 영향을 분석합니다.

- **Performance Highlights**: FS는 AI 모델의 계산 효율성을 높이는 데 중요한 역할을 하며, 정확도 손실 없이 효율성을 개선할 수 있음을 보여줍니다. Python과 R 언어가 AI 라이브러리 환경이 풍부하여 이점이 있다는 점도 강조됩니다. 이 논문은 AI 시스템을 보다 지속 가능하고 효율적으로 설계하는 데 유용한 통찰력을 제공합니다.
...
### Mobility-based Traffic Forecasting in a Multimodal Transport System (https://arxiv.org/abs/2411.08052)
Comments:
          17 pages, 18 figures

- **What's New**: 세네갈의 인구 이동 데이터를 기반으로 다중 교통 수단 네트워크에서의 교통 예측 가능성을 연구하였다. 이 연구는 머신러닝을 활용하여 도로 혼잡의 영향을 예측하고자 하며, 이전의 연구들에서 부족했던 패턴 인식 기법을 통해 트래픽 예측의 정확도를 높이고자 한다.

- **Technical Details**: 모델링은 Prophet(time series forecasting) 기반으로 진행되며, trend, seasonality 및 holidays의 세 가지 구성 요소로 나뉘어 있다. 데이터 분석에서 CDR 파일 내의 패턴 인식을 향상시켜 혼잡 예측을 위한 알고리즘 개선이 이루어지며, 다수의 알고리즘과 통계적 기법이 활용된다. 또한, 포화 성장 모델과 구간선형 모델을 통해 인구 이동의 성장 추세를 모델링한다.

- **Performance Highlights**: 연구 결과, 예측된 교통량은 다수의 혼잡 구간과 관련이 있으며, 이는 세네갈의 도시 이동 관리 정책 수립에 실질적 기여를 할 것으로 기대된다. 특히, 패턴 기반 분석이 실시간으로 도시 교통 흐름을 더욱 효과적으로 관리할 수 있는 기반이 될 것이다.



### Stochastic Reconstruction of Gappy Lagrangian Turbulent Signals by Conditional Diffusion Models (https://arxiv.org/abs/2410.23971)
- **What's New**: 본 논문에서는 난류(turbulent flow)에 의해 수동적으로 전이되는 작은 물체의 궤적에 따라 누락된 공간 및 속도 데이터를 재구성하기 위한 확률적(stochastic) 방법을 제시합니다. 최근 제안된 데이터 기반 기계 학습 기법인 conditional generative diffusion models(C-DMs)를 활용하였으며, 3D 비트의 동질적(isotropic) 터뷸런스와 NOAA 및 Global Drifter Program의 2D 궤적 문제에 대해 해결책을 제공합니다.

- **Technical Details**: C-DM은 두 가지 주요 과정인 전방(forward) 및 후방(backward) 프로세스를 포함합니다. 전방 과정에서는 초기 현실화 및 분산 스케줄을 사용하여 데이터 간의 상관 관계를 점진적으로 파괴하면서 부드럽게 전이됩니다. 후방 과정에서는 외부 측정값을 기반으로 노이즈를 줄이는 조건부 확률을 모델링합니다. 여기서 신경망(neural network)을 통해 각 단계의 평균 및 공분산을 예측합니다.

- **Performance Highlights**: 본 방법은 비가우시안(non-Gaussian) 및 간헐적(intermittent) 특성을 보존하면서 속도 신호를 정확하게 재구성하는 데 성공하였으며, 점 검출(pointwise reconstruction) 및 통계적 표현(statistical expressivity) 측면에서 Gaussian 프로세스 회귀(Gaussian process regressions)에 비해 우수한 정확도를 보여주었습니다. 다양한 라그랑지안(Lagrangian) 문제에 대한 가능성 있는 응용도 제시합니다.
...
### SPDIM: Source-Free Unsupervised Conditional and Label Shift Adaptation in EEG (https://arxiv.org/abs/2411.07249)
- **What's New**: 이번 연구에서는 비라벨 보정 데이터가 없는 전기생리학(EEG) 기반 신경기술의 일반화를 효과적으로 지원하기 위해 새로운 기하학적 심층 학습 프레임워크를 제안합니다. 기존의 리만 기하학 기반 통계 정렬 방법이 레이블 변화에 대한 일반화를 저해하는 점을 보완하기 위해, SPDIM이라는 매니폴드 최적화 전략을 도입하였습니다.

- **Technical Details**: 제안된 SPDIM(Symmetric Positive Definite Information Maximization)은 정보 극대화 원리를 활용하여 각 타겟 도메인에 대해 하나의 SPD-매니폴드 제약 파라미터를 학습합니다. 이는 기존의 라벨 이동 문제를 처리할 수 있는 능력을 가지고 있으며, 특정 마진 및 조건부 분포 이동에 대한 보상을 제공합니다. 연구 시뮬레이션에서 SPDIM은 제안된 생성 모델 아래에서 이동을 보상할 수 있음을 입증했습니다.

- **Performance Highlights**: SPDIM은 공개된 EEG 기반 뇌-컴퓨터 인터페이스와 수면 단계 데이터셋을 활용하여 검증되었으며, 기존 접근 방식보다 우수한 성능을 보였습니다. 이 연구는 EEG 기반 BCI의 도메인 일반화 문제를 해결하기 위한 획기적인 기여를 제공합니다.

...
### Learning Correlated Reward Models: Statistical Barriers and Opportunities (https://arxiv.org/abs/2510.15839)
- **What's New**: 이번 논문에서는 Random Utility Models (RUMs)에서 발생하는 Independence of Irrelevant Alternatives (IIA) 가정의 한계를 분석합니다. 기존 방식은 모든 인간 선호를 단일한 유틸리티 함수로 축소하여 사용해왔으나, 이는 복잡한 인간의 선호를 국소화시키는 데 있어 부족함을 드러냅니다. 이 논문은 상관된 probit 모델을 통해 선호 데이터를 수집하는 새로운 접근 방식을 제안하고, 이를 통해 RUMs의 효율적인 학습이 가능함을 보여줍니다.

- **Technical Details**: 연구에서는 기본적으로 상관된 probit 모델을 다루며, 데이터 수집은 관찰되지 않은 유틸리티(X)를 최대화하는 방식으로 이루어집니다. RUMs는 사용자의 선호를 모델링하며, 파라미터 μ와 공분산 행렬 Σ를 복원하여 보다 정밀한 인간 선호 표현을 목표로 합니다. 특히, 3-way preference 데이터를 활용하여 기존의 pairwise preference 데이터에서 발생하는 한계를 극복할 수 있음을 증명했습니다.

- **Performance Highlights**: 연구 결과, best-of-three preference 데이터는 통계적, 계산적 보장 측면에서 필수적이며 충분하며, 이를 통해 상관된 유틸리티 모델을 효과적으로 학습할 수 있음을 입증하였습니다. 여러 실제 데이터셋에서 검증한 결과, 인간 선호의 개인화가 향상되었고, 통계적으로 최적의 성능을 보였습니다. 이러한 결과는 고차원 선호 데이터의 필요성과 효용을 강조하며 향후 연구 방향에 대한 중요한 통찰을 제공합니다.



### Transfer Orthology Networks (https://arxiv.org/abs/2510.15837)
Comments:
          4 pages

- **What's New**: 이번 연구에서는 종 간 전이 학습을 위해 설계된 새로운 신경망 아키텍처인 Transfer Orthology Networks (TRON)을 제안합니다. TRON은 종 간의 정교한 지식 전달을 위해 이분 그래프( bipartite graph) 형태로 나타낸 동형(orthologous) 관계를 활용합니다. 이 아키텍처는 기계학습 기반의 변환 층을 사용하여 소스 종의 유전자 발현 데이터에서 타겟 종의 유전자 공간으로 지식을 효율적으로 전이하는 방법을 제공합니다.

- **Technical Details**: TRON의 핵심은 이분 그래프 BB로, 이는 소스 종과 타겟 종 간의 동형 관계를 인코딩합니다. 연구에서는 reciprocal best hits (RBHs)를 활용하여 이 그래프를 구성하며, RBHs는 두 종의 유전자 집합 간의 모든 비교를 통해 식별됩니다. 변환 층에서 사용되는 가중치 행렬 Wc는 단지 동형 유전자 쌍 간의 연결만 고려하여 변환된 유전자 발현 벡터를 계산하도록 설계되었습니다.

- **Performance Highlights**: TRON은 생물학적 근거에 기반한 해석 가능한 접근 방식을 제공하여 기존의 전이 학습 방법들을 개선합니다. 연구팀은 현재 TRON 아키텍처의 실험적 검증을 위해 종 간의 전사체 및 표현형 데이터를 수집하는 과정에 있습니다. TRON의 학습된 가중치는 기능적 동형을 해석하는 데 중요한 통찰력을 제공할 수 있으며, 이는 다양한 생물학적 맥락의 질병 연구 및 유전자 분석에 유용할 것입니다.



### FIDDLE: Reinforcement Learning for Quantum Fidelity Enhancemen (https://arxiv.org/abs/2510.15833)
- **What's New**: 이 논문에서는 프로세스 충실도 프로세스의 신뢰성을 직접 극대화하는 FIDDLE이라는 혁신적인 학습 프레임워크를 제안합니다. FIDDLE은 Gaussian Process 기반의 서리게이트 모델과 강화 학습 모듈로 구성되어 있습니다. 이 접근법은 전통적인 회로 깊이나 게이트 수와 같은 간접 메트릭에 의존하는 방법들을 능가하여 양자 회로의 프로세스 충실도를 직접 개선하는 최초의 방법입니다.

- **Technical Details**: 제안된 FIDDLE 프레임워크는 두 가지 주요 모듈로 구성됩니다. 첫 번째 모듈은 한정된 학습 샘플로 프로세스 충실도를 추정하는 Gaussian Process 기반의 서리게이트 모델입니다. 두 번째 모듈은 서리게이트 모델이 예측한 프로세스 충실도 값에 기반하여 최적의 게이트 시퀀스를 식별하기 위해 강화 학습을 사용할 수 있도록 합니다.

- **Performance Highlights**: FIDDLE의 성능은 최신 신뢰도 추정 기법 및 라우팅 최적화 방법과 비교하여 체계적으로 평가되었습니다. 실험 결과, 제안된 서리게이트 모델이 기존의 학습 기술보다 더 나은 충실도 추정을 제공하며, FIDDLE 프레임워크가 다양한 노이즈 모델에서 양자 회로의 프로세스 충실도를 획기적으로 개선함을 보여줍니다.
...
### Cavity Duplexer Tuning with 1d Resnet-like Neural Networks (https://arxiv.org/abs/2510.15796)
- **What's New**: 이 논문은 많은 조정 나사를 가진 캐비티 듀플렉서를 튜닝하기 위한 머신러닝 방법을 제시합니다. 기존의 강화학습 방식은 문제를 해결하기에 적합하지 않다고 판단하여, 감독학습(Supervised Learning) 설정으로 작업을 재구성했습니다. 제안된 신경망 아키텍처는 1D ResNet-like 백본을 포함하며, S-매개변수와 관련된 추가 정보를 처리합니다.

- **Technical Details**: 이 연구는 듀플렉서의 주파수 응답을 나타내는 S-파라미터(S-parameters)를 최적화하는 것을 목표로 합니다. 세 가지 S-커브(S-curves)인 S11, S21 및 S31을 사용하여 1300개의 포인트를 수집합니다. 튜닝 과정은 필터 인클로저의 조정 나사를 회전하여 이루어지며, 이를 위해 다수의 나사 회전 값을 예측해야 합니다.

- **Performance Highlights**: 저자들은 강화학습의 단점을 극복하기 위해 신경망 모델을 사용하여 듀플렉서를 효과적으로 튜닝하는 방법을 개발했습니다. 튜닝 품질은 S11-커브와 기준선 간의 면적으로 측정되며, 이 논문에서 제안한 방법은 이전의 Updater-Actor 스킴보다 더 나은 결과를 보여줍니다.
...
### SAMix: Calibrated and Accurate Continual Learning via Sphere-Adaptive Mixup and Neural Collaps (https://arxiv.org/abs/2510.15751)
- **What's New**: 이번 연구는 지속적인 학습(Continual Learning, CL) 시스템에서 중요한 요소인 네트워크 보정(calibration)을 개선하는 새로운 방법, Sphere-Adaptive Mixup (SAMix)를 제안합니다. SAMix는 신경 붕괴(Neural Collapse) 기반 모델의 기능적 특징 공간을 고려하여 자동으로 혼합 프로토타입을 생성함으로써 성능을 향상시키고 과도한 자신감(overconfidence)을 줄이며 정확성을 향상시킵니다. 이는 기존 CL 모델의 교정 성능을 넘어서는 중요한 발전으로 평가됩니다.

- **Technical Details**: SAMix는 고정된 ETF(Equiangular Tight Frame) 프로토타입의 구형 선형 보간(spherical linear interpolation)을 활용하여 적응형 혼합 프로토타입을 생성합니다. 이를 통해 모델의 표현 학습을 강화하고 결정 경계를 부드럽게 만들어 수행합니다. 이 연구는 다양한 NC(Nural Collapse) 기반 방법들을 비교 분석하며, 보정과 성능을 균형있게 맞추는 능력을 평가합니다.

- **Performance Highlights**: SAMix는 광범위한 실험을 통해 현저한 성능 향상을 보여주며, 기존의 최첨단(SOTA) 지속 학습 방법들을 초월한 결과를 달성하였습니다. 이 방법은 과제 간 정확도를 증대시키고 예측의 전반적인 신뢰도를 향상시키면서, CL 시스템에서 중요한 기반 기술로 자리잡을 가능성을 보여줍니다.
...
Comments:
          14 pages, 6 figures, 5 tables. Code available at:this https URL

- **What's New**: 이 논문은 Finite Element Analysis (FEA)의 대안으로서 그래프 신경망(Graph Neural Networks, GNNs)과 3D U-Net을 평가합니다. 특히, 해당 분야에서의 딥러닝 모델의 활용 가능성을 탐구하며 물리 법칙을 적용한 Physics-Informed Neural Network (PINN) 프레임워크를 소개합니다. 또한 커리큘럼 학습 전략을 통해 훈련의 안정성을 높이는 중요성을 강조합니다.

- **Technical Details**: 연구는 Navier-Cauchy 방정식에 의해 규제되는 PINN 프레임워크를 사용하여 FEA를 대체하기 위한 모델을 학습합니다. GNN과 U-Net을 비교하여, GNN이 U-Net보다 월등한 성능을 보이는 것을 입증하였습니다. 또한, Message Passing Neural Networks (MPNN)와 Graph Transformers 모델이 가장 높은 정확도를 기록하며, PINN이 포함된 MPNN은 практич적인 솔루션을 제공합니다.

- **Performance Highlights**: GNN의 최악의 성능을 보인 GCN 프레임워크는 상대 L2 오차가 8.7%였으며, U-Net 중 가장 좋은 성능은 13.0%의 점수를 기록했습니다. MPNN과 Graph Transformers는 각각 3.5%와 2.6%의 상대 L2 점수를 달성하여 최고의 정확도를 보였으며, PINN을 포함함으로써 최대 11.3%의 오차 감소를 이루어냈습니다. 그러나 Graph Transformer는 두 번째 모델인 MPNN PINN에 비해 37.5% 느린 추론 속도를 가집니다.
...
- **What's New**: 이번 연구는 범용적 적대적 변동(Universal Adversarial Perturbations, UAP)의 효율성을 높이는 새로운 접근 방식을 제안합니다. 기존의 UAP 방법들은 특정 도메인 제약을 무시했지만, 본 논문에서는 다양한 도메인에 적용 가능한 제약 기반 적대적 변동(Constrained Adversarial Perturbation, CAP)을 도입합니다. CAP는 복잡하고 다양한 중요성을 가진 제약을 동시에 적용할 수 있는 새로운 최적화 문제로 정의되며, 이를 통해 실제 적용 가능성을 높입니다.

- **Technical Details**: CAP는 여러 제약 조건을 준수하는 적대적 변동 생성을 위한 최소-최대(min-max) 최적화 문제로 형식화됩니다. 이 접근법은 Lagrangian 승수법을 적용하여 제약 위반에 대한 명시적인 패널티를 부여하며, 복잡한 고차원 제약하에서도 최적화를 지원합니다. CAP 알고리즘은 그래디언트 기반의 교대 최적화 전략을 사용하여 해결되며, 이를 통해 현대의 GPU 아키텍처를 활용하여 실행 시간을 대폭 줄이고 scalability를 개선합니다.

- **Performance Highlights**: CAP는 대출 처리, IT 네트워크 트래픽 분석, 의료 진단 및 사이버 물리 시스템 등 다양한 도메인에서 평가되었습니다. 실험 결과 CAP는 기존 방법들보다 평균 2.90%에서 47.90%까지 공격 성공률을 향상시키며, 런타임은 최소 45배 감소하는 성과를 보였습니다. 또한, CAP는 개인화된 적대적 변동을 생성하기 위해 설계된 CAPx 버전으로도 동일하게 뛰어난 성능을 발휘합니다.
...
### WARP-LUTs - Walsh-Assisted Relaxation for Probabilistic Look Up Tables (https://arxiv.org/abs/2510.15655)
Comments:
          Preprint. Under review

- **What's New**: 이번 연구는 Walsh-Assisted Relaxation for Probabilistic Look-Up Tables (WARP-LUTs)라는 새로운 경량 모델을 제안합니다. 이 방법은 논리 게이트 조합을 학습하는 데 필요한 트레이너블 파라미터 수를 대폭 줄이며, 기존의 Differentiable Logic Gate Networks (DLGNs)에 비해 훈련 속도를 크게 개선합니다. WARP-LUTs는 CIFAR-10 데이터셋에서 DLGNs보다 빠른 수렴을 보여줍니다.

- **Technical Details**: WARP-LUTs는 Walsh-Hadamard 분해의 완화(relaxation)를 바탕으로 하여 효율적인 파라미터화(parameterization)를 제공합니다. 이 방식은 연속적인 입력에 대해 일반화된 Hadamard 계수를 사용할 수 있게 해주며, 이를 통해 이진 함수의 무한한 조합을 표현할 수 있는 가능성을 열어줍니다. 제안된 방식은 Gumbel 재파라미터화 방법을 통해 훈련과 추론 간의 정렬을 개선하고, 손실 경관을 매끄럽게 하여 빠른 수렴을 유도합니다.

- **Performance Highlights**: WARP-LUTs는 기존의 DLGNs에 비해 실험을 통해 정확성을 유지하면서도 뛰어난 학습 속도를 자랑합니다. 이 시스템은 현대 FPGA에서의 효율적인 배포 가능성을 제시하며, 높은 입력 수를 가지는 논리 블록으로 확장될 수 있는 잠재력을 가지고 있습니다. 따라서, 이 연구는 실시간 과학 응용 분야에서 매우 효율적인 머신러닝 모델의 활용 가능성을 여는 중요한 기초가 될 것입니다.



### Fast and Compact Tsetlin Machine Inference on CPUs Using Instruction-Level Optimization (https://arxiv.org/abs/2510.15653)
- **What's New**: Tsetlin Machine (TM)은 CPU와 같은 자원 제약 장치에서 빠른 추론 속도를 제공하는 머신러닝 모델입니다. 이 논문은 비트 연산을 활용하여 TM의 효율적인 소프트웨어 구현을 제안하며, 추론 속도를 더욱 향상시키기 위한 조기 종료 메커니즘을 도입했습니다. 이를 통해 필요 없는 연산을 피하고, 조기 종료 가능성을 극대화하기 위한 리터럴 재정렬 전략을 제공합니다.

- **Technical Details**: TM의 핵심은 전통적인 산술 연산 대신 논리 기반의 연산에 완전히 의존한다는 점입니다. 논문에서는 왼쪽에서 오른쪽으로 비트 연산을 수행하여 차이 비트가 0인 경우 조기 종료를 적용합니다. 이러한 방식은 TM의 이론적인 본질에 더욱 부합하며, FPGA나 ASIC와 같은 하드웨어 구현에 대해 논리적인 일관성을 보장합니다.

- **Performance Highlights**: ARM 프로세서를 사용한 gem5 시뮬레이터를 통해 최적화된 구현이 기존의 정수 기반 TM 구현에 비해 추론 시간을 최대 96.71% 줄이는 성과를 보였습니다. 실험 결과, 리터럴 재정렬 전략을 통해 조기 종료 가능성을 높임으로써 모델의 해석 가능성과 성능을 모두 개선할 수 있음을 보여주고 있습니다.



### Deep Neural ODE Operator Networks for PDEs (https://arxiv.org/abs/2510.15651)
- **What's New**: 이 논문은 부분 미분 방정식(PDEs)을 해결하기 위한 효율적인 대리 모델을 개발하기 위한 새로운 접근인 심층 신경 일반 미분 방정식(ODE) 연산자 네트워크 프레임워크인 NODE-ONet을 소개합니다. 기존 방법들은 PDE에 내재된 도메인 지식을 간과하여 시간 역학을 캡처하고 훈련 시간 프레임을 넘어서 일반화 문제에 어려움을 겪고 있습니다. NODE-ONet은 인코더-디코더 아키텍처를 채택하여 입력 함수를 공간적으로 이산화하고, 잠재적인 시간 역학을 포착하며, 물리적 공간에서 솔루션을 재구성합니다.

- **Technical Details**: NODE-ONet은 세 가지 핵심 구성 요소로 이루어집니다: 인코더는 입력 파라미터 집합을 잠재 공간으로 임베딩하며, 'physics-encoded' NODE는 잠재 변수의 동역학을 근사하여 복잡성을 상당히 줄어듭니다. 이러한 아키텍처는 PDE의 비선형 종속성과 초기값, 경계 조건 등 기존의 방정식에서 알려진 파라미터도 포함하여 설계됩니다. 후속적으로, 디코더는 학습된 잠재 동역학을 기반으로 PDE의 솔루션을 재구성합니다.

- **Performance Highlights**: 비선형 확산-반응 및 나비에-스토크스 방정식에 대한 수치 실험을 통해 NODE-ONet의 높은 정확성과 계산 효율성이 입증되었습니다. 또한, 다양한 인코더와 디코더를 수용할 수 있는 유연성 및 관련 PDE 계열을 넘어 일반화할 수 있는 능력을 강조합니다. 이러한 특성은 NODE-ONet을 과학적 머신 러닝을 위한 확장 가능하고 물리적으로 인코딩된 도구로 강조합니다.



### Decentralized Parameter-Free Online Learning (https://arxiv.org/abs/2510.15644)
- **What's New**: 본 논문은 네트워크 회귀 보장을 갖춘 최초의 파라미터 없는 분산 온라인 학습 알고리즘을 제안합니다. 이 알고리즘은 하이퍼파라미터 조정 없이 서브리니어 회귀를 달성하며, 다중 에이전트의 동전 베팅과 분산 온라인 학습을 연결하는 새로운 '베팅 함수' 형식을 도입합니다. 이를 통해 알고리즘의 분석을 단순화하고, 분산 센싱, 최적화 및 협업 머신러닝(application) 응용에 적용할 수 있는 가능성을 제시합니다.

- **Technical Details**: 제안된 알고리즘은 각 에이전트가 개인화된 베팅 게임을 수행하여 총 손실을 최소화하도록 설계되었습니다. 각 에이전트는 과거 손실에 대한 지식을 바탕으로 다음 행동을 결정하고, 이 결정은 코인 베팅 프레임워크에서 유래한 파라미터 없는 전략을 사용합니다. 에이전트들은 각 라운드 후 이웃 에이전트와 내부 상태를 교환하여 정보를 확산시키고 네트워크 전반에 걸쳐 합의(consensus)를 도모합니다.

- **Performance Highlights**: 분석 결과 이 알고리즘은 서브리니어 회귀를 달성하며, 중앙 집중식 방법과 비교할 때 동등한 성능을 보이는 것을 검증했습니다. 네트워크의 연결성이나 구조에 관계없이 평균 지역 회귀가 최적의 성능을 유지함을 보여주며, 이는 노드 간의 통신이 불가능한 경우에도 적용됩니다. 본 알고리즘은 효율적인 운영을 가능하게 하며, 분산 네트워크 환경에서의 실험을 통해 그 유효성을 확인하였습니다.
...
### GRATING: Low-Latency and Memory-Efficient Semantic Selection on Devic (https://arxiv.org/abs/2510.15620)
- **What's New**: 이 논문은 현재 사용되는 on-device AI 서비스에서 중요한 상위 K(K) 선택의 성능 병목 문제를 분석하고, 기존 최적화 방법들이 이러한 작업에 실패하는 이유를 체계적으로 설명합니다. 또한, 시퀀스 수준의 희소성(sequence-level sparsity)과 I/O-계산 오버랩(I/O-computation overlap)의 두 가지 잘 활용되지 않은 기회를 밝혀냈습니다. 최적의 결과를 위해 GRATING이라는 훈련이 필요 없는 시스템을 설계하고 구현하였습니다.

- **Technical Details**: GRATING은 진보적인 클러스터 프루닝(progressive cluster pruning)과 청크 단위 실행(chunked execution)과 같은 기법을 통해 크로스 인코더 크기 조정의 효율적인 배포를 가능하게 합니다. 이 시스템은 먼저 모든 후보를 단일 배치로 처리하면서 메모리에 대한 경량화를 유지하여 중간 텐서 크기를 조절할 수 있도록 하고, 모든 후보에 대한 전역적인 시각(global view)을 유지합니다. 이러한 구조는 계산 지연 시간을 줄이고 메모리 사용량을 감소시켜, 실제 디바이스 운영의 효율을 크게 향상시킵니다.

- **Performance Highlights**: GRATING은 마이크로 벤치마크(microbenchmark)에서 최대 89.0%의 지연 시간(latency) 감소를 달성하고, 피크 메모리(peak memory)는 최대 94.9%까지 줄였습니다. 또한, 세 가지 실제 on-device AI 응용 관련 성능 평가에서 11.6%에서 51.0%까지의 지연 시간 감소와 18.6%에서 77.8%까지의 피크 메모리 절약을 보였습니다. 이러한 결과들은 on-device AI 응용에서의 실제 적용 가능성과 효율성을 크게 증대시킵니다.



### Attn-JGNN: Attention Enhanced Join-Graph Neural Networks (https://arxiv.org/abs/2510.15583)
- **What's New**: 이 논문에서는 Attention Enhanced Join-Graph Neural Networks(Attn-JGNN) 모델을 제안하여 #SAT 문제를 해결합니다. 이 모델은 Iterative Join Graph Propagation (IJGP) 알고리즘에서 영감을 받아 CNF 공식을 조인 그래프로 인코딩하고 반복적인 메시지 전달을 수행하여 해결 정확성을 크게 개선합니다. 특히 조인 그래프 내 및 클러스터 간에 주의 메커니즘을 적용하여 핵심 변수와 클러스터에 더욱 집중하며, 중복 계산을 줄입니다.

- **Technical Details**: 개념적으로, Attn-JGNN은 IJGP를 기반으로 하며, 그래프 신경망(GNN)을 통해 전이 구조를 매개변수화하고 메시지 업데이트를 주의 메커니즘으로 시뮬레이션합니다. 이 모델은 클러스터의 메시지 전달 방식에 계층적 구조를 도입하여 효율성을 개선하고, 만족된 절을 우선시하고 제약을 위반한 변수 할당을 벌점으로 처리하는 제약 인식 모듈을 도입하여 성능을 극대화합니다. 또한 동적 주의 메커니즘을 적용하여 훈련 속도를 증가시키고 리소스 소비를 줄입니다.

- **Performance Highlights**: 실험 결과는 BIRD 및 SATLIB 벤치마크 데이터셋에서 Attn-JGNN이 NSNet 및 BPGAT 대비 각각 31%와 45% 향상된 솔루션 정확성을 보여주었습니다. Attn-JGNN 모델은 전통적인 전파 알고리즘이 갖는 그래프 구조의 한계를 극복하며, 주의 메커니즘과 함께 동작하여 계산 효율성을 극대화하는 데 성공했습니다. 이러한 성과는 #SAT 문제 해결에 있어 새로운 접근법을 제공하여 다양한 응용 프로그램에서 활용될 수 있음을 보여줍니다.



### On the Neural Feature Ansatz for Deep Neural Networks (https://arxiv.org/abs/2510.15563)
- **What's New**: 본 논문에서는 Neural Feature Ansatz (NFA)의 깊이 의존성을 보여줍니다. NFA의 지수 $B1$를 깊이에 따라 $B1 = 1/L$로 확장하여 두 개 이상의 레이어를 갖는 신경망에 적용했습니다. 또한, 비균형 초기화의 경우에도 weight decay가 적용될 때 NFA가 점근적으로 유지된다는 것을 증명했습니다.

- **Technical Details**: 이 연구는 주로 gradient flow dynamics와 균형 초기화 하에 훈련된 심층 선형 신경망을 통해 NFA의 역할을 분석합니다. 두 레이어 이상의 신경망에 대해 NFA가 성립하는 지수를 $B1 = 1/L$로 확장하였으며, 비선형 활성화 함수가 있는 신경망에서는 NFA가 항상 성립하지 않음을 보여주는 반례를 제시했습니다. 다양한 최적화 알고리즘과 초기화 기법을 통해 이론적 결과를 실증적으로 검증했습니다.

- **Performance Highlights**: 모델의 깊이에 따라 NFA의 지수가 변하는 것을 증명하였으며, 이는 신경망의 성능에 큰 영향을 미칩니다. 수치 실험을 통해 여러 최적화 방법에서 NFA의 효과를 확인했으며, 특히 multi-index 함수 학습 시 NFA가 유도하는 차원 축소 메커니즘을 조사했습니다. 이러한 결과는 심층 신경망의 특성 추출 및 일반화 능력에 대한 이해를 심화하는 데 기여할 것입니다.



### Doubly Robust Estimation of Causal Effects in Strategic Equilibrium Systems (https://arxiv.org/abs/2510.15555)
- **What's New**: 본 논문은 전략적 환경에서의 인과 추론을 위한 새로운 프레임워크인 전략적 이중 견고 추정기(Strategic Doubly Robust, SDR)를 소개합니다. SDR은 전략적 행동에 의해 발생하는 내생적 처리 할당(endogenous treatment assignment)을 해결 개념으로 삼으며, 전략적 고려 사항을 포함하면서 이중 견고성을 유지합니다. 이론적 분석 결과 SDR은 전략적 비혼잡성(strategic unconfoundedness) 하에서 일관성과 점근적 정규성을 보장합니다.

- **Technical Details**: SDR 추정기는 전통적인 이중 견고 추정 개념을 전략적 행동에 맞게 확장하여, 에이전트가 처리 할당에 전략적으로 응답할 때에도 일관된 인과 효과 추정이 가능하도록 합니다. 기본적인 통찰은 전략적 행동이 내생성을 생성하지만, 전략적 평형(strategic equilibrium) 자체가 식별을 위한 추가 구조를 제공한다는 것입니다. SDR은 혹시라도 전술 모델(기대(payoff) 함수)가 잘못 명시되더라도 견고성을 유지합니다.

- **Performance Highlights**: 실험적 평가 결과 SDR은 기본 방법에 비해 7.6%에서 29.3%의 편향 감소를 달성하며 에이전트 집단 규모에 대한 견고한 확장을 유지합니다. 이는 SDR이 에이전트가 전략적으로 반응하는 개입에 대해 신뢰할 수 있는 인과 추론을 제공할 수 있는 원칙적인 접근 방식을 제공함을 의미합니다.
...
### Compressive Modeling and Visualization of Multivariate Scientific Data using Implicit Neural Representation (https://arxiv.org/abs/2510.15535)
Comments:
          Accepted for publication in 16th Indian Conference on Computer Vision, Graphics and Image Processing (ICVGIP 2025)

- **What's New**: 이 연구는 수십에서 수백 개의 변수를 포함하는 다변량 데이터셋에 대한 압축 신경 표현(compressed neural representations)을 개발한 혁신적인 접근을 소개합니다. 이 기술은 하나의 네트워크를 통해 모든 데이터 변수를 동시에 학습하게 하여, 데이터 압축의 새로운 패러다임을 제시합니다. 특히, 다변량 데이터 분석을 위한 다양한 과제를 수행하기에 적합한 고품질의 복원 데이터를 제공할 수 있는 가능성을 보여줍니다.

- **Technical Details**: 저자는 잔여 SIREN 아키텍처(residual SIREN architecture)를 활용하여 다변량 데이터를 압축하는 방법을 사용합니다. 이 방식은 데이터 변수 간의 매개변수 공유를 통해 높은 압축 비율을 달성할 수 있도록 합니다. 연구는 변수 간의 종속성과 복원 데이터 품질이 보존되는지를 종합적으로 평가하며, 이를 다른 데이터 감소 방법과 비교합니다.

- **Performance Highlights**: 연구 결과, 압축 신경 표현이 복잡한 다변량 과학 데이터의 패턴을 효과적으로 캡처할 수 있으며, 많은 변수와 복잡한 의존성을 포함하는 데이터 세트에서 성능이 우수하다는 것을 입증하였습니다. 여러 테스트에서 시각화 품질, 데이터를 복원할 때의 정확도, 종속성 정보를 보존하는 정도 등에서 기존의 경쟁 기술들보다 뛰어난 성능을 나타냅니다.
...
### Theoretical Refinement of CLIP by Utilizing Linear Structure of Optimal Similarity (https://arxiv.org/abs/2510.15508)
- **What's New**: 이번 연구에서는 다중 모달 대비 사전 훈련 프레임워크인 CLIP의 유사성 계산 메커니즘을 개선하는 방법을 제안합니다. 기존 이론 연구에서는 두 모달리티 간 최적의 유사성 메트릭이 점별 상호 정보(pointwise mutual information, PMI)와 일치해야 한다고 보여주었습니다. 그러나 현재 CLIP 구현은 PMI의 선형 구조를 충분히 활용하지 못하고 있습니다. 이에 따라 우리는 이 구조를 활용하는 KME-CLIP을 제안합니다.

- **Technical Details**: KME-CLIP은 재생산 커널 힐버트 공간(reproducing kernel Hilbert space)에서 내적(inner product)을 통해 PMI에 대한 근사를 개선합니다. 이 방법은 이미지 및 텍스트 인코더로부터의 임베딩을 긍정적 함수로 표현하여 이들의 내적의 로그를 유사성 메트릭으로 활용합니다. 우리는 이 메트릭이 임의의 정확도로 PMI를 근사할 수 있음을 이론적으로 입증합니다. 또한, CLIP은 특정 조건 하에서 PMI를 근사하는 데 한계가 있다는 것을 보입니다.

- **Performance Highlights**: 실험 결과, CC3M 및 CC12M 데이터셋을 사용하여 KME-CLIP이 CC3M, MSCOCO, Flickr30K 등 여러 검색 과제에서 CLIP보다 일관되게 뛰어난 성능을 기록했음을 보였습니다. 또한 ImageNet, CIFAR-100을 포함해 10개 이상의 다양한 데이터셋에서 제로샷(zero-shot) 및 선형 분류(linear classification) 정확성을 평가한 결과, KME-CLIP이 대부분의 과제에서 우수한 성능을 보였습니다.
...
- **What's New**: 이 논문에서는 할당 편향(assignment bias) 하에서의 반사실적 예측을 연구하고, 적대적 훈련 없이 치료-공변량 종속성을 제거하는 수학적으로 기반이 있는 정보 이론적 접근법을 제안합니다. 이 연구는 반사실적-사실적 리스크 갭을 상호 정보(mutual information)와 연결하는 경계를 시작으로, 결과를 예측할 수 있는 확률적 표현 Z를 학습하면서 I(Z; T)를 최소화합니다. 이로 인해 보다 안정적이고 증명된 훈련 기준을 갖춘 변별적인 변별 목표를 도출합니다.

- **Technical Details**: 정보 이론적 접근법을 통해, 우리는 치료 선택에 대한 공변량이 전달하는 정보의 양을 조절하는 확률적 표현을 학습하여, 치료 노출을 드러내는 특징을 억제합니다. 이 과정에서, 우리는 정적(static) 모델을 정적 정보 이론적 반사실적 추정기(Static Information-theoretic Counterfactual Estimator, SICE)로, 동적(dynamic) 확장을 동적 정보 이론적 반사실적 추정기(Dynamic Information-theoretic Counterfactual Estimator, DICE)로 각각 지칭합니다. 이러한 접근법은 시간에 따라 변하는 공변량을 사용하는 동적 경우로 원활하게 확장됩니다.

- **Performance Highlights**: 실험 결과, 우리는 통제된 수치 시뮬레이션과 실제 임상 데이터셋에서 제안한 방법을 평가하였으며, 최신 최첨단 방법들과 비교했습니다. Likelihood, counterfactual error 및 정책 평가와 같은 metrics 전반에 걸쳐, 우리의 접근법은 훈련 불안정성과 적대적 기법의 조정 부담을 피하면서도 유리한 성능을 보여주었습니다.
...
### Particle Dynamics for Latent-Variable Energy-Based Models (https://arxiv.org/abs/2510.15447)
- **What's New**: 본 논문에서는 잠재 변수 에너지 기반 모델(LVEBMs)을 제안하여 관측 데이터와 잠재 변수의 쌍에 대한 단일 정규화된 에너지를 할당합니다. 이를 통해 표현력이 풍부한 생성 모델링을 제공하며 숨겨진 구조를 포착합니다. 최대 우도(maximum-likelihood) 학습을 잠재 및 조합 다양체에 대한 새들 문제(saddle problem)로 재구성하고, 내적 업데이트는 결합된 Wasserstein 그래디언트 흐름으로 간주합니다.

- **Technical Details**: LVEBMs는 잠재 변수를 명시적으로 다루어 원인 분리를 가능하게 하고, 에너지 기반으로 파라미터화된 조합 Gibbs 밀도 하에서 쌍을 점수화합니다. 제안된 알고리즘은 오버댐프드 랑주베인 업데이트를 사용하여 직접적으로 Wasserstein 흐름을 실현하며, 이는 KL 발산 및 Wasserstein-2 거리에서의 수렴성을 보장합니다. 이 방법은 별도의 판별기(discriminator)나 보조 네트워크(auxiliary networks) 없이 작동하며, 단순하고 원리적인 학습 프레임워크를 제공합니다.

- **Performance Highlights**: 제안된 LV-EBM 방법은 수치 물리 시스템의 근사치에 대해 평가되었으며, 유사한 접근 방식들과 경쟁력 있는 성과를 보였습니다. 이 알고리즘은 잠재 상태를 통해 제어 가능성을 높이고, 각 예제에 대한 조건부 잠재 이동 및 조합 부정 샘플 Refresh를 통해 안정성을 유지합니다. 실험 결과는 혼합 및 조건부 진단을 포함하여 다양한 시뮬레이션에서 유망한 성능을 나타냅니다.
...
### Safe, Efficient, and Robust Reinforcement Learning for Ranking and Diffusion Models (https://arxiv.org/abs/2510.15429)
Comments:
          PhD Thesis of Shashank Gupta defended at the University of Amsterdam on October 13th 2025

- **What's New**: 이 논문은 Reinforcement Learning (RL) 방법들이 안전하고, 샘플 효율적이며, 강인하게 설계될 수 있는 방안을 다룹니다. 특히, 컨텍스트 기반의 Bandit RL 관점에서 안전한 배포 및 최소 위험을 고려한 새로운 방법론을 제안합니다. 이 연구는 순위 시스템 및 추천 알고리즘에 대한 두 가지 주요 응용 분야에 초점을 맞추고 있습니다.

- **Technical Details**: 첫 번째 섹션에서는 클릭 상호작용 데이터를 기반으로 샘플링 편향을 수정하기 위한 Counterfactual Learning to Rank (CLTR)에 대한 이론과 알고리즘을 개발했습니다. 특히, 이론적 일반화 경계를 도출하여, 새로운 정책이 기존 생산 정책보다 적어도 좋게 보장할 수 있는 방법에 대해 설명합니다. 두 번째 섹션에서는 단일 행동 Bandits에서 오프 정책 추정기를 통합하는 방안을 다룹니다.

- **Performance Highlights**: 논문에서 제안된 Exposure-based Counterfactual Risk Minimization (CRM) 방법은 당시 데이터가 적을 때 초기 성과 저하를 방지할 수 있도록 설계되었습니다. LOOP 알고리즘을 통해 PPO 수준의 샘플 효율성을 달성하며, 텍스트 속성과 더욱 일치하는 생성물을 만들어내는 성과를 보여줍니다. 실험 결과는 이 방법이 실질적인 사용자 경험을 저해하지 않으면서 고성능을 유지할 수 있음을 입증하였습니다.



### ParaFormer: Shallow Parallel Transformers with Progressive Approximation (https://arxiv.org/abs/2510.15425)
- **What's New**: 본 논문은 전통적인 '더 깊은 것이 더 좋다'는 철학에 도전하며, ParaFormer라는 얕은 Transformer 아키텍처를 제안합니다. ParaFormer는 진정한 병렬성(true parallelism)을 구현하여 모델의 깊이를 줄이면서도 성능 저하 없이 효율적으로 동작하도록 설계되었습니다. 우리는 표준 Transformer를 폐쇄형 함수 근사화기로 구성하여 이전 층들과의 협업을 중요시하며, 이를 통해 더 깊지 않아도 우수한 성능을 제공할 수 있음을 보여줍니다.

- **Technical Details**: ParaFormer의 핵심은 층을 병렬 분기로 구성하고, 알고리즘 수준에서 층 간 협업을 보장하는 것입니다. 각 분기는 동일한 입력을 공유하며, 무거운 교차 분기 복사를 줄여 효율성을 높입니다. 이 아키텍처를 통해 성능 최적화의 기초를 닦으며, 기존의 Transformer 아키텍처를 새로운 시각으로 분석할 수 있는 기틀을 제공합니다.

- **Performance Highlights**: 실험 결과, ParaFormer는 ViT와 같은 표준 Transformer를 초과하는 성능을 보여주고 모델 압축에서 최대 15.07배 향상을 지원합니다. Multi-GPU 배포에서 ParaFormer는 FairScale과 같은 기존의 병렬 샘플링 솔루션보다 3.30배 빠른 성능을 나타내어, 자원 제약 조건에서도 유용하게 활용될 수 있음을 입증합니다. 이는 Universal Approximation Theorem에 기반한 폐쇄형 공식화로 얻어진 개선 사항들입니다.



### Online Kernel Dynamic Mode Decomposition for Streaming Time Series Forecasting with Adaptive Windowing (https://arxiv.org/abs/2510.15404)
- **What's New**: 이 연구에서는 실시간 데이터 스트리밍으로부터 예측하는 데 필요한 도전과제를 다루고, 기존의 방법들 간의 정확도, 적응성 및 효율성을 비교합니다. 새로운 방법인 WORK-DMD (Windowed Online Random Kernel Dynamic Mode Decomposition)을 소개하여, 고정된 계산 비용을 유지하면서 비선형 동역학을 효과적으로 캡처합니다. WORK-DMD는 Sherman-Morrison 업데이트를 통해 현재 데이터만을 이용해 지속적으로 적응할 수 있도록 설계되었습니다.

- **Technical Details**: WORK-DMD는 Random Fourier Features와 온라인 Dynamic Mode Decomposition을 결합하여 동적인 시스템의 비선형 동작을 모델링합니다. 이 방법은 긴 훈련이나 역사적 데이터 저장의 필요성을 제거하고, 최신 데이터의 창을 활용하여 예측을 수행합니다. 기존의 DMD 기술에 비해 비선형 동역학을 더 잘 캡처하고, 상태 기반에서 강력한 예측 성능을 유지하는 것이 특징입니다.

- **Performance Highlights**: 다양한 벤치마크 데이터 세트를 통해 WORK-DMD는 최신 온라인 예측 기법들보다 높은 정확도를 기록하였으며, 데이터에 대한 단일 패스로 강력한 성능을 발휘했습니다. 실험 결과는 kernel 평가와 적응 행렬 업데이트의 결합이 최소한의 데이터 요구사항으로도 높은 예측 성능을 달성할 수 있음을 보여주고 있습니다. 이는 스트리밍 예측 응용에서 깊은 학습을 위한 실질적인 대안이 될 수 있습니다.



### Geometric Mixture Models for Electrolyte Conductivity Prediction (https://arxiv.org/abs/2510.15403)
- **What's New**: 이번 연구는 전해질 시스템에서 이온 전도도의 정확한 예측을 위해 CALiSol 및 DiffMix와 같은 전해질 데이터셋을 재구성하고 보강하였습니다. 새로운 기하학적 그래프 표현을 도입하여 각 분자의 기하학적 구조와 분자 간의 상호작용을 강화하는 방법을 제시합니다. 이를 통해 제안된 GeoMix 프레임워크는 Set-SE(3) 등가성을 유지하면서 혼합 시스템의 모델링에 있어 진전을 이룩하였습니다.

- **Technical Details**: GeoMix는 Geometric Interaction Network (GIN)을 중심으로 구성되어 있으며, 이는 분자 간 기하학적 메시지 패싱을 위한 등가 모듈입니다. 각 분자에 대해 로컬 좌표계로 정렬하고, 서로 다른 로컬 프레임의 원자 쌍을 정렬하기 위해 학습 가능한 좌표 변환을 적용합니다. 이러한 구조적 접근은 분자 간의 기하학적 상호작용을 효과적으로 모델링할 수 있게 합니다.

- **Performance Highlights**: 실험 결과 GeoMix는 다양한 기본 모델(MLPs, GNNs, 기하학적 GNNs 등)보다 지속적으로 우수한 성능을 보였습니다. 특히, 전해질 전도도 예측에 있어 기존 방법론의 한계를 극복하고, 정확한 물성 예측을 위한 교차-분자 기하학적 상호작용과 등가 메시지 패싱의 중요성을 보여줍니다. 이 연구는 또한 전해질 연구를 위한 새로운 기준을 세우고, 혼합 시스템 모델링을 위한 일반적인 기하학적 학습 프레임워크를 제공합니다.



### Iterative Refinement of Flow Policies in Probability Space for Online Reinforcement Learning (https://arxiv.org/abs/2510.15388)
- **What's New**: 본 논문에서는 Stepwise Flow Policy (SWFP) 프레임워크를 소개합니다. 이 프레임워크는 기존의 behavior cloning 정책들이 분포성 변화에 취약하다는 점을 개선하기 위한 방법으로, 고정 단계의 유일러 방법을 통해 흐름 일치 추론 과정을 이산화하여 구성됩니다. SWFP는 전체 흐름을 근접 분포 간의 작은 변환으로 분해하여 각 단계를 Jordan-Kinderlehrer-Otto (JKO) 원칙에 맞춰 진행함으로써 안정적인 온라인 적응을 보장합니다.

- **Technical Details**: SWFP는 사전 훈련된 유동 정책을 미세 조정하기 위해 작은 흐름 블록의 연속을 통한 효율적인 알고리즘입니다. 이 방법은 각 단계에서 정책 업데이트를 이전 이터레이트 근처로 유지하도록 정규화하여 불안정을 줄이면서 효율적인 최적화를 가능하게 합니다. JKO 원칙과 결합됨으로써, 이 프레임워크는 제도적 안정성을 위한 이론적 보장을 제공합니다.

- **Performance Highlights**: SWFP는 다양한 로봇 제어 벤치마크에서 안정성, 효율성, 우수한 적응 성능을 입증한 실험 결과를 통해 명확한 이점을 보여줍니다. 이 알고리즘은 사전 훈련된 흐름 정책을 통해 안정적으로 미세 조정할 수 있으며, 계산 및 메모리 비용이 대폭 절감되는 장점을 가집니다. 또한, Wasserstein 신뢰 지역을 기반으로 한 이론적 보증을 유지하면서 고차원 연속 작용 설정에서도 실용성이 뛰어난 성능을 발휘합니다.
...
### Sequence Modeling with Spectral Mean Flows (https://arxiv.org/abs/2510.15366)
Comments:
          30 pages, 9 figures

- **What's New**: 이번 논문에서는 신경망을 사용한 시퀀스 모델링의 새로운 접근법을 제안합니다. 전통적인 방법에서는 각 단계에서 비선형 동적 시스템을 모델링하기 위해 랜덤 네트워크를 사용하는 반면, 본 연구는 힐베르트 공간에서 확률 분포를 선형 변환으로 다룰 수 있는 오퍼레이터 이론을 기반으로 합니다. 이 접근법을 통해 숨겨진 마르코프 모델(HMM)의 시퀀스 분포를 텐서 형태로 제시하고, 더 나아가 계산적으로 효율적인 신경망 아키텍처를 제안합니다.

- **Technical Details**: 이를 위해, 논문은 시퀀스 전체의 분포를 텐서 제품 힐베르트 공간의 요소로 임베드(Embed)하며, 최대 평균 불일치(MMD) 경량 흐름(Maximum Mean Discrepancy Gradient Flow)을 통해 강력한 생성적 프로세스를 정의합니다. 또한 이 논문에서는 고차원 텐서와 표준 MMD 흐름의 느린 수렴 문제를 해결하기 위해 스펙트럴 평균 흐름(Spectral Mean Flows)이라는 새로운 알고리즘을 제안합니다. 이 방법은 선형 연산자의 스펙트럴 분해를 활용하여 시퀀스의 분포 임베딩을 효율적으로 계산합니다.

- **Performance Highlights**: 연구 결과, 제안된 스펙트럴 평균 흐름은 다양한 시계열(time-series) 데이터셋에서 경쟁력 있는 성능을 보였습니다. 실험은 인공 데이터(synthetic data)와 실제 시계열 모델링 데이터셋을 대상으로 진행되었으며, 전반적으로 시뮬레이션 없는 학습과 빠른 샘플링 수렴을 가능하게 하여 높은 효율성을 나타냈습니다. 코드는 공개되어 있어 후속 연구 및 실용적 응용이 용이합니다.
...
- **What's New**: 이 논문은 여러 개의 그래프 신경망(GNN) 공격에 동시에 대응할 수 있는 통합 방안을 제안합니다. 기존의 방어 기술들은 일반적으로 단일 공격 유형에 집중하고 있어, 다양한 위협에 대처하는 데 한계가 있었습니다. 이에 따라, Mixture of Experts (MoE) 아키텍처를 활용하여 백도어, 엣지 조작, 노드 주입 공격에 대응할 수 있는 확장 가능하고 통일된 프레임워크인 RGMoE를 개발했습니다.

- **Technical Details**: RGMoE는 전문가들 각각이 다양한 이웃 구조에 집중하도록 장려하는 MI 기반의 로직 다양성 손실을 도입합니다. 또한, 역량이 있는 라우터를 도입하여 공격받은 노드를 해당 공격에 강한 전문가에게 동적으로 라우팅합니다. 이를 통해 다수의 그래프 적대적 공격에 대해 효과적으로 방어하면서도 적정한 추론 시간 비용을 유지할 수 있습니다.

- **Performance Highlights**: 다양한 적대적 설정 하에서 수행된 실험 결과, RGMoE는 기존 방법들보다 일관되게 뛰어난 내구성을 보여주었습니다. 논문에서는 백도어, 조작 및 노드 주입 공격에 대한 방어의 효율성을 통해, 제안된 프레임워크가 그래프 적대적 공격에 대한 효과적인 방어 수단이 될 수 있음을 입증하였습니다.



### On the Generalization Properties of Learning the Random Feature Models with Learnable Activation Functions (https://arxiv.org/abs/2510.15327)
- **What's New**: 이 논문은 최근 제안된 커널 방법, 즉 Learnable Activation Functions (RFLAF)와 관련된 일반화 특성을 연구합니다. 데이터 의존 샘플링 방식을 적용하여 RFLAF 학습에 필요한 특성의 수에 대한 가장 뚜렷한 경계를 제공합니다. 이 연구는 회귀 및 분류 작업 양쪽 모두에서 RFLAF를 학습하기 위해 필요로 하는 특성 수를 개선하였습니다.

- **Technical Details**: 무한 폭의 랜덤 피처 모델을 학습하는 데 초점을 맞추며, 다양한 활성화 함수와 해당 조건을 고찰합니다. Lipschitz 지속성을 가진 함수의 정규 분포를 가정하고, RFLAF의 무작위 특성의 수가 표본 크기에 따라 개선된 경계를 도출합니다. 레버리지 가중치 샘플링 방법을 통해 MSE 손실 시 최적의 경계를 제공하며, 이를 위한 알고리즘도 제안합니다.

- **Performance Highlights**: 실험 결과, 가중치가 적용된 RFLAF가 단순 샘플링을 통해 구성된 RFLAF에 비해 현저히 적은 수의 특성으로 동일한 성능을 달성하는 것을 확인하였습니다. 이는 제안된 이론과 방법론의 유효성을 검증합니다. 또한, 관련 작업에 대한 논의는 부록 A에서 이루어집니다.



### DFCA: Decentralized Federated Clustering Algorithm (https://arxiv.org/abs/2510.15300)
- **What's New**: 이번 논문에서는 클러스터링된 연합 학습(Clustered Federated Learning)의 한계점을 극복하기 위해 DFCA(Decentralized Federated Clustering Algorithm)를 제안합니다. 기존의 기법들은 중앙 서버에 의존하여 모델 업데이트를 조정하는 반면, DFCA는 중앙 조정 없이도 클라이언트들이 협력하여 클러스터별 모델을 학습할 수 있도록 해줍니다. 이 접근 방식은 통신 효율성을 높이며, 클러스터링 성능을 유지합니다.

- **Technical Details**: DFCA는 클러스터 할당, 로컬 업데이트, 그리고 분산 집계라는 세 가지 단계로 구성되어 있습니다. 각 클라이언트는 클러스터 c(i)에 할당되어, 자신의 로컬 데이터에 대해 경량의 경량화(gradient descent)를 수행하며, 커뮤니케이션 단계에서 이웃 클라이언트들과 모델을 교환합니다. 이를 통해 모든 클라이언트가 훈련된 모델을 보유하게 됩니다.

- **Performance Highlights**: 여러 데이터셋을 대상으로 한 실험에서 DFCA는 기존 중앙 집중식 IFCA와 비슷한 정확도를 보였으며, 다른 분산 알고리즘들보다 뛰어난 성능을 발휘했습니다. Sparse connectivity 상황에서도 그 강인성과 실용성을 보여주었으며, 이는 실제 분산 네트워크에서의 활용 가능성을 크게 증가시킵니다.
...
### Small Ensemble-based Data Assimilation: A Machine Learning-Enhanced Data Assimilation Method with Limited Ensemble Siz (https://arxiv.org/abs/2510.15284)
- **What's New**: 이 연구에서는 데이터 동화(data assimilation, DA) 접근법에 대한 새로운 기계 학습 기반 방법을 제안합니다. 전통적인 앙상블 칼만 필터(Ensemble Kalman Filter, EnKF)와 완전 연결 신경망(Fully Connected Neural Network, FCNN)을 결합하여 소규모 앙상블을 사용하더라도 분석 정확도를 높일 수 있는 방법론을 제시합니다. 이 방법은 제한된 앙상블 크기로 인해 발생할 수 있는 성능 저하를 완화합니다.

- **Technical Details**: 제안된 방법은 EnKF를 기반으로 하며, 초기 앙상블 상태를 생성한 후 FCNN을 활용하여 이러한 상태의 보정 항을 학습합니다. 이 과정은 예측 단계와 분석 단계를 포함하며, FCNN은 예측된 평균과 측정값을 통합하여 더 정확한 분석 결과를 제공합니다. FCNN의 입력 변수로는 예측된 앙상블 상태와 이전 분석 상태 등이 포함됩니다.

- **Performance Highlights**: 수치 실험 결과, 제안된 EnKF-FCNN 방법이 동일한 앙상블 크기를 사용했을 때 전통적인 EnKF보다 높은 정확도를 보인다는 것이 일관되게 입증되었습니다. 추가적인 계산 비용은 거의 발생하지 않으면서도 다양한 모델과 결합하여 적용 가능성을 보여주는 장점이 있습니다. 이러한 유연성 덕분에 이 방법은 여러 분야에 적응할 수 있는 잠재력을 가지고 있습니다.
...
### Semi-Supervised Regression with Heteroscedastic Pseudo-Labels (https://arxiv.org/abs/2510.15266)
Comments:
          Accepted by NeurIPS 2025

- **What's New**: 이 논문은 준지도 학습(SSL)의 한 분야인 준지도 회귀(SSR)에서의 pseudo-labeling 적용에 대해 다룹니다. 기존의 연구들은 classification 태스크에서 pseudo-labeling의 적용이 일반적이었으나, SSR에서는 연속적인 출력과 이종 노이즈로 인한 pseudo-label의 신뢰성 평가가 어렵습니다. 본 연구에서는 불확실성(uncertainty)을 고려한 pseudo-labeling 프레임워크를 제안하여, pseudo-label의 영향을 동적으로 조정합니다.

- **Technical Details**: 제안된 방법은 라벨이 있는 데이터와 없는 데이터 모두에서 경험적 리스크(를 최소화하면서, 불확실성 추정치를 최적화하여 라벨이 있는 데이터에 대한 일반화를 개선합니다. 이 두 가지 작업을 동시에 수행하기 위해 bi-level 최적화(bi-level optimization) 접근 방식을 사용합니다. 이 과정에서 pseudo-label의 신뢰성을 개선하고, 과적합(overfitting)의 위험을 줄입니다.

- **Performance Highlights**: 다양한 benchmark SSR 데이터셋에 대한 실험을 통해 제안된 방법이 기존 방법들보다 더 나은 성능과 강인성을 보임을 입증했습니다. 특히, 불확실성을 기반으로 한 pseudo-labeling이 회귀 모델의 일반화를 현저히 향상시킬 수 있음을 확인했습니다. 이 연구는 이론적 분석을 통해도 제안된 방법이 과적합 문제를 효과적으로 해소하는 이유에 대한 인사이트를 제공합니다.



### Causal Time Series Modeling of Supraglacial Lake Evolution in Greenland under Distribution Shif (https://arxiv.org/abs/2510.15265)
Comments:
          Accepted as full paper in ICMLA 2025 (Special Session 1: Deep Learning and Applications)

- **What's New**: RIC-TSC(Regionally-Informed Causal Time Series Classification) 프레임워크는 시간적 모델링에 지연 인지(causal discovery)를 통합하여 예측 정확성과 과학적 해석 가능성을 동시에 향상시키는 새로운 접근 방식을 제안합니다. 이 연구는 그린란드에서의 표층 호수(evolution of supraglacial lakes)의 진화를 이해하기 위해 멀티모달 위성 데이터와 기후 재분석을 활용했습니다. 전통적인 상관 분석 접근법의 한계를 극복하기 위해 인과 모델을 채택하고 있습니다.

- **Technical Details**: RIC-TSC 프레임워크는 J-PCMCI+(Joint PCMCI+)를 사용하여 그린란드의 1000개 수기 레이블 호수에 대한 인과 그래프를 추정하고 지연 변수(lagged predictors)를 도출합니다. 이 지연에 대한 인과 예측자는 MiniROCKET을 통한 특성 추출 후 RidgeClassifier를 통해 시퀀스 분류에 사용됩니다. 이를 통해 모델의 해석 가능성과 속도, 변별력의 균형을 맞추어 복잡한 신경망의 오버헤드를 피합니다.

- **Performance Highlights**: 이 연구는 두 개의 대비되는 융해 시즌(2018-2019)에서 수작업으로 라벨링된 1000개의 호수의 균형 잡힌 벤치마크에서 인과 모델이 상관 기반 기준선보다 최대 12.59% 높은 정확도를 달성함을 보여줍니다. 인과 특성이 분포 이동 상황에서 모델의 일반화를 크게 향상시켜, 동적 지구 표면 과정의 메커니즘 기반 모델로의 발전 가능성을 제시합니다.
...
### Spatiotemporal Transformers for Predicting Avian Disease Risk from Migration Trajectories (https://arxiv.org/abs/2510.15254)
- **What's New**: 이번 연구는 조류 질병 발생 예측을 위한 Transformer 기반 프레임워크를 제시합니다. 이는 야생동물 보존(wildlife conservation)과 공공 건강(public health)에 중요한 역할을 합니다. 다양한 데이터셋인 GPS 추적 데이터, 세계 동물 건강 기구(WOAH)의 발생 기록, 그리고 지리적 맥락이 포함됩니다.

- **Technical Details**: 모델은 H3 계층적 지리 공간 인코딩(geospatial encoding)을 활용하여 공간적 패턴을 포착합니다. 조류 움직임 시퀀스로부터 시공간 종속성(spatiotemporal dependencies)을 학습하여, 도착지에서의 질병 위험을 추정합니다. 다양한 소스의 데이터를 통합하여 보다 정교한 예측을 가능하게 하였습니다.

- **Performance Highlights**: 모델은 검증용 테스트 세트에서 매우 강력한 예측 성능을 보였습니다. 정확도(accuracy) 0.9821, ROC 곡선 면적(AUC) 0.9803, 평균 정밀도(AP) 0.9299, F1 점수(F1-score) 0.8836을 달성하며 최적의 임계값에서 우수한 성과를 기록했습니다. 이러한 결과는 조류 질병 감시를 위한 조기 경고 시스템의 가능성을 강조합니다.



### Dual-Weighted Reinforcement Learning for Generative Preference Modeling (https://arxiv.org/abs/2510.15242)
- **What's New**: 이 논문은 Dual-Weighted Reinforcement Learning (DWRL)이라는 새로운 프레임워크를 제안합니다. DWRL은 chain-of-thought (CoT) 추론을 Bradley-Terry (BT) 모델과 통합하여 선호 모델링에 대한 새로운 접근 방식을 제공합니다. 이 방법은 인간의 선호와 불일치하는 훈련 데이터를 강조하는 비대칭 가중치와 유망한 생각을 촉진하는 조건부 선호 점수를 사용하여 훈련을 진행합니다.

- **Technical Details**: DWRL은 모델이 여러 가지 응답을 고려할 수 있도록 하여 인간의 선호를 이렇게 모델링되는 데이터 쌍에 대해 더욱 정교하게 점수를 매길 수 있게 합니다. DWRL은 BT 모델의 최대 우도 목표를 두 개의 보완 가중치로 근사화하는데, 이것은 인스턴스별 및 집단별 점수를 모두 고려하여 훈련을 최적화합니다. 이는 코트 훈련이 선호 모델링의 귀납적 편향을 보존하면서 보다 일반적인 비검증 가능 작업에 적용되도록 합니다.

- **Performance Highlights**: 다양한 벤치마크 및 모델 스케일(Llama3 및 Qwen2.5)에서 DWRL은 기존의 생성적 선호 모델(Generative Preference Models, GPM) 및 스칼라 모델에 비해 일관되게 우수한 성능을 보여주었습니다. 또한, DWRL을 통해 생성된 생각은 일관되고 해석 가능하여 후속 작업(Downstream Tasks)에 효과적으로 활용될 수 있습니다. 이는 DWRL이 검증 가능한 작업을 넘어서는 인과관계 강화된 선호 학습의 일반적인 프레임워크로 자리매김할 수 있는 기회를 제공합니다.
...
### Stress-Aware Learning under KL Drift via Trust-Decayed Mirror Descen (https://arxiv.org/abs/2510.15222)
- **What's New**: 본 연구에서는 분포 변화(distribution drift) 하에서의 순차적 의사결정(sequential decision-making)을 조사합니다. 엔트로피 정규화(entropy-regularized)된 신뢰 감소(trust-decay) 메커니즘을 제안하고, 이를 통해 스트레스 인지(stress-aware) 방식으로 신뢰 업데이트를 수행합니다. 결과적으로, 신뢰 감소 거울 하강법(trust-decayed mirror descent, TD-MD)은 KL 제어 하에 동적 회귀(dynamic regret) O~(T)tilde{O}(	ext{sqrt}(T))을 달성하는 방법론을 제시합니다.

- **Technical Details**: 순차적 의사결정 과정에서 학습자는 각 라운드 t에서 선택한 x_t에 따라 자연(Environment)에서 z_t를 뽑고 손실 ℓ(x_t; z_t)을 겪습니다. 각 손실의 기대값을 정의하고, 동적 회귀(dyamic regret)를 성능 지표로 삼습니다. 이 연구에서는 KL 발산(KL divergence)을 통해 비정상(non-stationarity)을 정량화하며, 스트레스 벡터(σ_t)가 손실 변동을 제어하는 데 중요한 역할을 함을 나타냅니다.

- **Performance Highlights**: 신뢰 감소 메커니즘은 비정상적인 시퀀스에서 O(1) 손실을 달성하며, 매번의 업데이트에서 지능적으로 스트레스 강도를 조정할 수 있습니다. 메커니즘은 강한 확장성 및 스트레스 변화(stress variation), 분산 최적화(distributed optimization) 등에 대한 일반화된 해법을 제공합니다. 결론적으로, 연구는 동적 회귀 분석, 분포적 강건성(distributional robustness) 및 KL 정규화된 제어를 통합하여 스트레스 적응형 학습을 위한 이론적 기초를 제공합니다.



### Integrating Product Coefficients for Improved 3D LiDAR Data Classification (Part II) (https://arxiv.org/abs/2510.15219)
Comments:
          16 pages, 6 figures, 5 tables

- **What's New**: 이 연구는 이전의 LiDAR (Light Detection and Ranging) 포인트 클라우드 분류 개선에 관한 연구를 확장하여 product coefficients를 도입합니다. 이러한 measure-theoretic descriptor는 기존의 공간 LiDAR 특성 이외의 정보로 데이터의 질을 향상시킵니다. Autoencoder representation을 활용하여 PCA 기반의 이전 모델보다 일관된 성능 향상을 보입니다. 또한, product coefficients를 단계적으로 추가할 때 클래스 분리와 전체 정확도가 향상된다는 명확한 경향성을 발견했습니다.

- **Technical Details**: 본 연구에서 product coefficients는 LiDAR 데이터의 지역 구조 정보를 보완하는 measure-theoretic descriptor입니다. 이들은 각 점 주변의 dyadic neighborhood에 대해 계산되며, 원래의 공간 좌표 이상의 기하학적 변동성을 포착합니다. Autoencoder는 PyTorch로 구현되어 있으며, 두 개의 은닉층을 통해 학습됩니다. 이를 통해 원래의 특성과 product coefficients로 이루어진 강화된 feature space에서 컴팩트한 표현을 직접 학습합니다.

- **Performance Highlights**: 자동 인코더와 product coefficients의 결합은 이전 PCA 기반 접근법보다 일관되게 높은 분류 정확도와 F1-macro 점수를 기록합니다. 이 결과는 복잡한 feature 의존성을 포착하고, PCA와 같은 선형 변환보다 중복을 더 효과적으로 줄이는 비선형 표현 학습의 장점을 강조합니다. 실험 결과는 기존의 모델과 비교하여 데이터의 정확성과 신뢰성을 향상시키는데 기여합니다.



### Machine Learning for Early Detection of Meningitis: Stacked Ensemble Learning with EHR data (https://arxiv.org/abs/2510.15218)
- **What's New**: 이 연구에서는 MIMIC-III 데이터베이스의 214명의 수막염 환자와 46,303명의 비수막염 환자를 사용하여 새로운 데이터 처리 방법을 선보였습니다. 주요 ICD 코드와 성별 등의 임상적으로 중요한 특징을 선택하여 진단의 정확성을 높이고자 했습니다. 이러한 특징들은 모델링 성능을 크게 향상시키는 데 기여했습니다.

- **Technical Details**: 데이터 전처리 과정에는 ICD 기반의 집단 선택, 코드의 one-hot encoding, 두 단계의 특징 선택 과정이 포함됩니다. Random Forest, LightGBM, Deep Neural Networks (DNN) 등 세 가지 모델을 훈련하여 앙상블 학습의 기본 모델로 사용하였고, 이 기본 모델의 출력을 메타 모델(Logistic Regression)로 집계하여 더욱 정교한 진단 도구를 개발했습니다.

- **Performance Highlights**: 최종적으로 Testing Set 1에서 AUC 0.9637, Testing Set 2에서 AUC 0.9472를 달성하여 모델의 우수한 성과를 입증하였습니다. 이 연구는 실제 응급실의 진단 과정을 시뮬레이션하여 수막염 진단의 도전적인 상황을 설정하였으며, 향후 AI 기반 진단 접근 방식에 대한 가능성을 제시합니다.



### Reflections from Research Roundtables at the Conference on Health, Inference, and Learning (CHIL) 2025 (https://arxiv.org/abs/2510.15217)
- **What's New**: CHIL 2025, 제6회 건강, 추론 및 학습 컨퍼런스는 AHLI가 주최하며 2025년 6월 25일부터 27일까지 미국 캘리포니아 대학교 버클리에서 개최되었습니다. 이번 회의에서는 기계 학습과 건강 관리의 융합을 논의하기 위한 연구 원탁회의가 개최되었습니다. 각 원탁세션은 전문가들이 이끌며 혁신적인 아이디어 탐구와 긴급한 도전과제 토론이 이루어졌습니다.

- **Technical Details**: 원탁회의는 '설명 가능성, 해석 가능성 및 투명성', '불확실성, 편향 및 공정성', '인과관계', '도메인 적응', '기초 모델', '소규모 의료 데이터 학습', '다중 모달 방법' 및 '확장 가능하고 번역 가능한 의료 솔루션'과 같은 8개의 주제에 대해 진행되었습니다. 각 세션은 전문가들이 제시한 가이딩 프롬프트에 따라 진행되었으며, 최종 주제는 생명 윤리를 포함한 여러 주요 사항들이 정리되었습니다.

- **Performance Highlights**: 설명 가능 AI의 필요성이 더욱 부각되면서, 의료 분야에서의 AI 통합의 도전과제들이 강조되었습니다. 특히, 기존 기술 한계 외에도, 임상 시스템과의 통합 작업에서 발생하는 여러 문제들이 주요 논의거리가 되었습니다. 성공적인 AI 도입은 기술적 성과보다 신뢰성과 사용 용이성에 더 크게 의존함을 인식할 필요가 있습니다.
...
### An Advanced Two-Stage Model with High Sensitivity and Generalizability for Prediction of Hip Fracture Risk Using Multiple Datasets (https://arxiv.org/abs/2510.15179)
Comments:
...
- **What's New**: 이번 연구에서는 노인에서 고관절 골절의 조기 리스크 평가를 위한 새로운 접근 방식인 두 단계 모델을 제안합니다. 기존의 DXA T-score와 FRAX 도구는 민감도가 떨어져 고위험군 환자를 놓치는 경우가 많았습니다. 이에 따라, 이 모델은 임상 정보와 이미징 정보를 통합하여 예측 정확성을 향상시키고자 합니다.

- **Technical Details**: 제안된 두 단계 모델은 첫 단계(Step 1, Screening)에서 임상 및 인구통계학적 변수들을 활용하여 기초 위험도를 추정합니다. 두 번째 단계(Step 2, Imaging)에서는 DXA에서 유도된 특징들을 포함시켜 예측을 정교화합니다. 이 모델은 Osteoporotic Fractures in Men Study (MrOS), Osteoporotic Fractures (SOF), 및 UK Biobank의 데이터를 통해 개발 및 검증되었습니다.

- **Performance Highlights**: 모델은 내부 및 외부 테스트를 통해 엄격하게 검증되었으며, 다양한 집단에서 일관된 성능을 보여주었습니다. T-score 및 FRAX와 비교했을 때, 두 단계 모델은 더 높은 민감도를 기록하여 놓치는 사례를 줄였으며, 비용 효율적이고 개인 맞춤형 접근법으로 고관절 골절 위험 조기 평가를 가능하게 합니다.



### Finding geodesics with the Deep Ritz method (https://arxiv.org/abs/2510.15177)
- **What's New**: 이 논문은 지오데식 문제(geodesic problems)의 간단한 기하학과 자연적인 비선형성 덕분에 Deep Ritz 방법(Deep Ritz method)이 특히 유리하다고 주장합니다. 연구팀은 경로 계획(path planning)과 광학(optics), 고체 역학(solid mechanics)에서의 세 가지 예제를 통해 이 주장을 뒷받침합니다. 이 연구의 목표는 지오데식 문제에 대한 포괄적인 연구를 제공하는 것이 아니라 Deep Ritz 방법의 잠재적인 적용 가능성을 탐구하는 것입니다.

- **Technical Details**: 지오데식은 두 상태를 연결하는 곡선으로, 문제에 따라 정의된 거리, 비용 또는 에너지를 최소화합니다. 이 연구에서는 지오데식 문제를 정적 경계값 문제(static boundary value problem)로 구성하여 경계 조건을 자동으로 부과하고, 주어진 목표 기능(objecive functional)을 직접 최소화하는 방법을 제안합니다. 이 접근 방식은 기존의 유도-라그랑주 방정식을 푸는 대신, 지오데식의 변분적 구조를 강조하여 보다 실질적으로 다루고 있습니다.

- **Performance Highlights**: 제안된 전략은 세 가지 예제 문제에 대해 간단하고 효과적으로 적용되었습니다. 지오데식 문제를 신경망(neural network)을 통해 이산화하여 경계 조건을 자동으로 부과하는 방식으로, Deep Ritz 방법을 효과적으로 적용할 수 있음을 보여주었습니다. 이러한 연구는 SciML 커뮤니티 내에서 지오데식 проблем을 다루는 최초의 탐색으로, 향후 연구의 유망한 방향성을 제시합니다.



### A simple mean field model of feature learning (https://arxiv.org/abs/2510.15174)
- **What's New**: 이번 연구는 두 층 비선형 신경망이 확률적 경량 동역학(SGLD)으로 훈련되는 Bayesian posterior에 대한 자기 일관된 평균장(MF) 이론을 도출합니다. 기존 이론들은 무한 폭의 한계에서 유효하지만 유한 폭에서는 특성 학습(FL) 메커니즘을 포착하지 못하는 한계를 가지고 있습니다. 연구에서는 입력 특성 선택(IFS)을 포함시켜 이론에 새롭고 중요한 메커니즘을 추가하여 일반화 오류를 정확하게 예측합니다.

- **Technical Details**: 이 연구는 통계 물리학의 기법을 사용하여 SGLD를 통해 훈련된 두 층 비선형 네트워크에 대한 MF 이론을 도출합니다. 이 MF 모델은 유한 폭 상황에서 FL의 출발을 대칭 파괴 상전이로 예측하며, 데이터의 적합한 방향으로의 가중치 정렬을 특성으로 합니다. 기본 MF 이론은 입력 차원에서의 𝒪​(d) 패널티를 제거하여 FL이 차원의 저주를 극복할 수 있는 메커니즘을 제공합니다.

- **Performance Highlights**: MF-ARD 모델은 SGLD로 훈련된 네트워크의 일반화 오류를 양적 예측할 수 있는 능력을 보여줍니다. 이는 다양한 데이터셋 크기와 잡음 수준에 대해 균형 잡힌 예측력을 갖습니다. 연구는 MF-ARD가 FL 메커니즘을 효과적으로 포착하면서도 평균장 이론의 단순성 및 효율성을 유지한다는 점을 강조합니다.



### Policy Transfer Ensures Fast Learning for Continuous-Time LQR with Entropy Regularization (https://arxiv.org/abs/2510.15165)
- **What's New**: 본 논문은 강화 학습(Reinforcement Learning, RL)에서 정책 전이(Policy Transfer)라는 전이 학습(Transfer Learning, TL) 접근 방식을 제안하며, 이론적으로 연속시간 선형 이차 조절기(Linear Quadratic Regulators, LQRs)에서의 유효성을 입증합니다. 특히, 원래 알고리즘의 수렴 속도를 유지하면서, 한 LQR에 대해 최적의 정책이 인접한 LQR에 대해 근접 최적의 초기화로 작용함을 밝혀냈습니다. 이로써 기존의 이론적 틈새를 메우고, 이산적으로부터 연속적인 상황으로의 접근을 확장하는 데 기여합니다.

- **Technical Details**: 논문에서 제안하는 연속시간 LQR을 위한 새로운 정책 학습 알고리즘은 글로벌 선형 수렴(Global Linear Convergence)과 로컬 초선형 수렴(Local Super-Linear Convergence)을 동시에 달성합니다. 정책 전이는 유사한 RL 작업 간에 지식을 활용하여 초기 정책을 효과적으로 설정할 수 있는 메커니즘을 제공하며, 수학적으로는 최적 정책의 가우시안 구조 및 관련 리카티 방정식의 강건성을 활용합니다. 이는 연속시간 환경에서 RL의 전이 학습을 이론적으로 분석하는 첫 시도로, 복잡한 제어 문제에 대한 중요한 기초를 마련합니다.

- **Performance Highlights**: 연구 결과는 연속시간 RL에서의 전이 학습의 이론적 보장과 알고리즘적 이점을 모두 입증하여, 학습 시간과 계산 비용을 효과적으로 줄이는 데 기여합니다. 또한, 정책 전이는 추천 알고리즘에서의 성능 향상을 이끌어내며, 특정 연속시간 점수 기반 확산 모델의 안정성을 유도하는 부가적인 성과도 있습니다. 이는 강화 학습 분야의 효율성을 높이는 중요한 기여가 될 것으로 예상됩니다.



### Predicting the Unpredictable: Reproducible BiLSTM Forecasting of Incident Counts in the Global Terrorism Database (GTD) (https://arxiv.org/abs/2510.15136)
Comments:
          12 pages, 5 figures, 2 tables. Code & reproducibility: this https URL Data/ethics: GTD used under research-only terms; no raw GTD is redistributed

- **What's New**: 이번 연구에서는 Global Terrorism Database (GTD, 1970-2016)를 활용하여 주간 테러 사건 수의 단기 예측을 새로운 방식으로 분석하였습니다. Bidirectional LSTM (BiLSTM)을 통해 고전적인 방법(계절적 naive, 선형/ARIMA)과 LSTM-Attention의 성능을 비교했습니다. 이 연구는 기존 방법들을 능가하는 결과를 제공하며, 투명하고 재현 가능한 파이프라인을 구축하였습니다.

- **Technical Details**: 모델 평가는 고정 시간 기반 분할 방식으로 진행되었으며, BiLSTM은 RMSE 6.38을 기록하여 LSTM-Attention의 9.19보다 30.6% 우수한 성과를 보였습니다. 연구에서는 다양한 임시 기억(temporal memory), 훈련 이력(training-history length), 공간적 차원(spatial grain), 그리고 피쳐 그룹(feature groups) 등을 변경하여 성능을 분석했습니다. 긴 역사적 데이터에서 훈련된 모델이 가장 잘 일반화되며, 중간 크기의 과거 데이터(20-30주)가 강력한 맥락을 제공하기 때문에 필수적이라고 밝혔습니다.

- **Performance Highlights**: BiLSTM 모델은 MAE와 MAPE에서도 기존 모델들보다 일관된 향상을 보여주었습니다. 특히 짧은 기간 구조(lagged counts 및 rolling statistics)가 가장 큰 기여를 했으며, 지리적 및 인명 피해(feature)도 추가적인 성과 향상에 도움을 주었습니다. 연구진은 코드와 구성, 결과 테이블을 공개하여 연구의 투명성을 강조하였으며, GTD 라이센스를 문서화한 데이터/윤리 성명을 제공합니다.



### A Simple Method for PMF Estimation on Large Supports (https://arxiv.org/abs/2510.15132)
- **What's New**: 이번 연구에서는 다중 모드(multi-modal)와 두꺼운 꼬리(heavy-tailed)를 가진 확률 질량 함수(Probabilistic Mass Function, PMF)의 비모수(nonparametric) 추정 방법을 제안합니다. 주요 아이디어는 경험적 PMF를 선 그래프(signal on a line graph)로 간주하고 데이터 종속적인 저주파 필터(low-pass filter)를 적용하는 것입니다. 이 방법은 PMF를 낮은 차원의 부분공간으로 투영하여 부드럽고 다중 모드의 추정을 생성하고 노이즈를 억제하는 데 초점을 맞추고 있습니다.

- **Technical Details**: 제안된 방법은 대칭 삼중 대각선 연산자(symmetric tri-diagonal operator)를 구성하고, 경험적 PMF로부터 유도된 대각행렬을 perturbation(섭동)합니다. 그런 다음 가장 작은 고유값에 해당하는 고유벡터를 계산하고, 이 저차원 부분공간에 경험적 PMF를 투영하여 부드러운 추정치를 형성합니다. 이러한 방법은 빠른 계산과 저하 성능을 보장하며, 최소한의 조정으로 실행될 수 있습니다.

- **Performance Highlights**: 실험에서는 제안된 방법이 로그 스플라인(logspline) 및 가우시안 커널 밀도 추정기(Gaussian KDE)와 비교해 좋은 성능을 보이며, 다수의 샘플에서 robust한 결과를 나타냅니다. 또한 데이터 기반 변수 선택 규칙을 제공하여 자동화된 파이프라인이나 탐색 분석에 적합하게 구현될 수 있습니다. 하지만 이 방법은 간헐적인 실패 모드도 존재하여 더욱 신뢰성 있는 결과를 추구해야 함을 시사합니다.
...
- **What's New**: 이 연구는 기계 환기(MV)와 보조 치료 결정이 환자 결과에 미치는 영향을 이해하기 위해 기계 환기 결정의 디지털 트윈을 개발하는 새로운 프레임워크를 제시합니다. 이는 다양한 환자-환기 시스템으로부터의 데이터를 분석하여 최적의 개인화된 MV 관리 방안을 제시할 수 있는 기회를 제공합니다. 진화적 게임 이론(EGT)을 사용하여 숨쉬기 행동을 분석하고 환자와 기계의 상호작용을 보다 깊이 이해할 수 있는 기초를 마련합니다.

- **Technical Details**: 본 연구는 J6(공동 환자-환기-돌봄 시스템) 관점에서 MV 관리에서 발생하는 데이터의 분석 방법을 제시합니다. EGT를 통해 숨쉬기 행동의 패턴을 분석하고, 이를 통해 강화 학습(Reinforcement Learning) 기법을 적용할 수 있는 수량적 전처리를 도출합니다. 여기서 목표는 폐 손상(ventilator-induced lung injury; VILI)을 최소화하고 최적의 가스 교환을 위한 MV 결정을 분석하는 것입니다.

- **Performance Highlights**: EGT 기반 프로세스는 합성 데이터를 통해 분석적으로 검증되었으며, 실제 중환자실(ICU) 데이터에서 발생하는 복잡성을 탐구할 수 있는 기초를 제공합니다. 각 개인의 MV 결정이 환자 결과에 미치는 영향을 시간 의존적으로 정량화할 수 있도록 하는 로드맵을 제공합니다. 이로 인해 MV 관리에서의 의사 결정 최적화를 위한 길이 열리게 됩니다.
...
### Online Correlation Clustering: Simultaneously Optimizing All $\ell_p$-norms (https://arxiv.org/abs/2510.15076)
Comments:
          66 pages

- **What's New**: 이 논문은 온라인 설정에서의 상관 클러스터링을 위한 새로운 알고리즘을 제시하고 있습니다. 연구의 초점은 다양한 $	heta_p$-norm(예: $	heta_1$-norm과 $	heta_	ext{infinity}$-norm)에서의 공정성을 보장하면서 동시에 높은 경쟁력을 유지하는 클러스터링을 생성하는 것입니다. 개발된 알고리즘은 샘플을 기반으로 하여 모든 $	heta_p$-norm에 대해 $O(	ext{log}^4 n)$ 경쟁력을 가지고 있습니다.

- **Technical Details**: 제안된 알고리즘은 온라인-with-a-sample (AOS) 모델에서 작동하며, 입력 데이터의 일정 비율을 샘플로 제공받습니다. 이 알고리즘은 클러스터 일관성을 유지하면서도 각 에지의 레이블에 따라 클러스터를 나누며, $	heta_	ext{infinity}$-norm을 최적화하는 데 강력한 성능을 보입니다. $	heta_1$-norm에서의 예상 경쟁성 비율은 $O(1)$로 유지됩니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘은 $	heta_p$-norm의 모든 경우에서 강력한 근사 보장을 달성했습니다. 특히, 전체 데이터세트의 적은 비율인 $O(1/	heta^8 	ext{log}^4 n)$와 $O(1/	heta^6 	ext{log} n)$에 대해, 각각 $θ_p$와 $θ_	ext{infinity}$의 최적비용 $	ext{OPT}_p$와 $	ext{OPT}_{	ext{infinity}}$로 표현되는 경쟁력을 달성한 것으로 나타났습니다.



### Physics-informed data-driven machine health monitoring for two-photon lithography (https://arxiv.org/abs/2510.15075)
- **What's New**: 이 논문은 두 광자 리소그래피(두 광자 리소그래피는 TPL) 시스템의 기계 건강을 모니터링하기 위한 새로운 접근 방식을 제안합니다. 현재의 유지보수 방법은 경험에 의존하는 경향이 있어 비효율성을 초래할 수 있습니다. 제안된 세 가지 방법은 다양한 생산 시나리오에서 기계 상태를 정확하게 모니터링할 수 있도록 하여 기존의 문제점을 해결할 수 있습니다.

- **Technical Details**: 제안된 방법은 물리 정보에 기반한 예측 모델과 통계적 접근 방식을 통합하여, 서로 다른 데이터 가용성 수준을 가진 여러 생산 시나리오에 적용할 수 있습니다. 첫 번째 방법은 동일한 설계와 공정 매개변수 하에서 구조의 기하학적 차이를 모니터링하며, 두 번째 방법은 Hotelling의 T2 테스트를 이용하여 새로운 설계에 대해서도 모니터링을 확장합니다. 세 번째 방법은 기계 상태의 일반적인 특성을 모니터링하며, bootstrap 재표집 방식을 활용합니다.

- **Performance Highlights**: 모든 테스트 시나리오에서 제안된 방법들은 높은 정확성을 보여주며, 엑셀런트한 효과성과 robust함을 입증합니다. 예를 들어, 첫 번째 방법은 100%의 정확성을 달성했으며, 두 번째 방법은 서로 다른 기계 조건 하에서도 각각 83.33%와 86.11%의 분류 정확성을 보였습니다. 이러한 결과들은 TPL 시스템에 대한 조건 기반 유지보수의 중요한 정진을 나타냅니다.
...
### Learn to Change the World: Multi-level Reinforcement Learning with Model-Changing Actions (https://arxiv.org/abs/2510.15056)
- **What's New**: 이 논문에서는 전통적인 강화 학습(RL) 모델에서 한 걸음 더 나아가 에이전트가 스스로 MDP 모델을 변경할 수 있는 행동을 수행할 수 있는 새로운 파라다임을 소개합니다. 새로운 구조인 다층 구성 가능 시간 가변 마르코프 결정 프로세스(MCTVMDP)는 시간에 따라 변화하는 환경에서 에이전트의 보상을 극대화하는 새로운 가능성을 제시합니다. 여기서는 MDP의 전이 함수가 고정되지 않고, 상위 레벨에서 변경 가능한 모델 변경 행동을 통해 조정됩니다.

- **Technical Details**: MCTVMDP는 두 가지 주요 구성 요소로 나뉘며, 상위 레벨 MDP에서 구성 정책을 최적화하고 하위 레벨 MDP에서 원시 행동 정책을 최적화하여 장기적인 기대 보상을 동시에 개선하는 것을 목표로 합니다. 다양한 예를 통해 환경을 모델링하고 정책을 탐색하는 능력을 보여주고 있습니다. 특히, 이 논문은 다층 MDP의 구성 가능성을 다루며, 하위 레벨 전이 커널이 상위 레벨 MDP의 상태가 되는 독특한 접근 방식을 마련합니다.

- **Performance Highlights**: 알고리즘은 볼록 최적화 형식과 다층 가치 반복(multilevel value iteration)을 포함하여 다층 구성 가능 RL 문제를 해결하기 위한 기법을 제안하였습니다. 제안된 알고리즘에 대한 이론적 성능 보장이 제공되며, 수치 실험 결과는 학습을 통해 RL 환경을 구성하거나 개선하는 효과를 입증합니다. 이러한 새로운 접근법을 통해 에이전트의 보상을 직접적으로 증가시킬 수 있는 가능성이 열립니다.
...
### IQNN-CS: Interpretable Quantum Neural Network for Credit Scoring (https://arxiv.org/abs/2510.15044)
Comments:
          Accepted for oral presentation at QUEST-IS'25. To appear in Springer proceedings

- **What's New**: 이 논문에서는 IQNN-CS라는 다중 클래스 신용 위험 분류를 위한 해석 가능한 양자 신경망 프레임워크를 제안합니다. 기존의 양자 기계 학습(QML) 모델이 투명성을 중시하지 않았던 반면, IQNN-CS는 해석 가능성을 중요한 설계 요소로 삼고 있습니다. 해당 프레임워크는 구성이 간단하면서도 안정적인 훈련 역학을 보여주는 등 여러 현실 세계 신용 데이터 세트에서 평가되었습니다.

- **Technical Details**: IQNN-CS는 클래스 불균형을 해결하기 위해 두 개의 벤치마크 신용 데이터 세트에 다양한 전처리 기법을 적용합니다. 이 과정에서는 PCA를 사용하여 입력 데이터를 저차원으로 변환하고, 양자 하드웨어에 적합한 형식으로 만들어 과정 전반에 걸쳐 완전 연결 계층과 양자 회로를 조합하여 예측을 수행합니다. 또한, Inter-Class Attribution Alignment(ICA)라는 새로운 메트릭을 도입하여 모델의 추론 방식에 대한 구조적 분석을 가능하게 합니다.

- **Performance Highlights**: IQNN-CS는 두 가지 실제 신용 데이터 세트에서 평가되었으며, 안정적인 훈련 다이내믹스와 경쟁력 있는 예측 성능, 그리고 향상된 해석 가능성을 보여줍니다. 이러한 결과는 금융 의사결정에서 투명하고 책임 있는 양자 기계 학습 모델로의 실질적인 경로를 나타냅니다. 특히, 해당 모델은 규제가 있는 금융 환경에 배치 가능성을 강조하면서, 다중 클래스 분류의 해석 가능성 요구를 충족하는 데 중점을 두고 있습니다.



### AlignFlow: Improving Flow-based Generative Models with Semi-Discrete Optimal Transpor (https://arxiv.org/abs/2510.15038)
Comments:
          Submitted for peer review on Sep 24, 2025. Note: chairs and reviewers can see and bid on our submission since Sep 28, 2025

- **What's New**: 이 논문은 AlignFlow라는 새로운 접근 방식을 소개하며, 이는 Semi-Discrete Optimal Transport (SDOT)를 활용하여 흐름 기반 생성 모델(Flow-based Generative Models, FGMs) 훈련을 개선합니다. AlignFlow는 잡음 분포(noise distribution)와 데이터 포인트(data points) 간의 최적 정렬을 수립하여 약속된 수렴(convergence)을 보장합니다. 이 방법은 기존의 방법들과는 달리 높은 차원의 대규모 데이터셋에서도 효과적으로 확장 가능합니다.

- **Technical Details**: AlignFlow는 잡음 공간(noise space)을 Laguerre 셀(Laguerre cells)로 분할하여 각 셀을 대응하는 데이터 포인트에 맵핑합니다. SDOT는 결정론적 맵(deterministic map)을 제공하여 각 잡음 샘플이 특정 데이터 포인트와 일관되게 연결되도록 보장합니다. 이 방식은 이전의 디스크리트 최적 수송(discrete Optimal Transport) 방식들이 직면했던 차원의 저주(curse of dimensionality)를 피할 수 있게 해주며, FGMs가 고차원 데이터셋에서 효과적으로 작동하도록 합니다.

- **Performance Highlights**: AlignFlow는 기존의 FGMs에 쉽게 통합될 수 있는 플러그 앤 플레이(plug-and-play) 방법이며, 다양한 최신 SOTA(FGMs)와 결합하여 성능 향상을 이끌어냅니다. SDOT 맵의 계산 추가 비용은 실제로 1% 미만으로 매우 적어, 전체 시스템의 효율성을 해치지 않습니다. 테스트 결과, AlignFlow를 적용했을 때 FGMs의 성능이 크게 향상되었음을 보여주며, 이는 코드가 온라인으로 제공됨에 따라 연구자들에게 접근성을 높여줍니다.
...
### ES-C51: Expected Sarsa Based C51 Distributional Reinforcement Learning Algorithm (https://arxiv.org/abs/2510.15006)
- **What's New**: 이 논문에서는 전통적인 가치 기반 강화 학습(Reinforcement Learning, RL) 알고리즘의 문제점을 해결하기 위해 C51 알고리즘의 수정된 버전인 ES-C51을 제안합니다. 기존 C51은 최적의 Q-값을 선택하기 위해 탐욕적인 방법을 사용하지만, 유사한 보상을 가지는 여러 행동이 있을 경우 안정성이 떨어질 수 있습니다. ES-C51은 이 문제를 해결하기 위해 Expected Sarsa 업데이트를 적용하여 모든 가능한 행동에서 정보를 결합하는 소프트맥스 계산을 사용합니다.

- **Technical Details**: RL에서는 에이전트가 각 행동에 대한 기대 보상만을 추정하는 반면, Distributional RL(DRL)은 가능한 보상의 전체 확률 분포를 추정합니다. C51은 고전적인 Q-learning 방식을 기반으로 하여 이산 행동 공간에서 사용되며, 이 분포는 탐욕적인 벨만 업데이트를 통해 학습됩니다. 그러나 여러 행동이 유사한 기대 보상을 가질 경우 분포의 불안정성이 발생할 수 있으며, 이는 ES-C51이 해결하고자 하는 문제입니다.

- **Performance Highlights**: ES-C51 알고리즘은 다양한 환경에서 검증된 결과, QL-C51 알고리즘보다 우수한 성과를 보였습니다. 에이전트는 우수한 정책을 학습할 수 있으며, 이러한 성과는 Gym의 고전적인 제어 환경과 Atari-10 게임에서 실험을 통해 분석되었습니다. ES-C51은 성능 향상뿐만 아니라 시간 효율성 비교에서도 긍정적인 평가를 받았습니다.
...
### Extending Load Forecasting from Zonal Aggregates to Individual Nodes for Transmission System Operators (https://arxiv.org/abs/2510.14983)
Comments:
          Collaborative Research, Stanford University and Midcontinent Independent System Operator

- **What's New**: 이번 연구는 전력 시스템 운용자(Transmission System Operator, TSO)의 개별 노드에 대한 고해상도 전력 수요 예측을 위한 새로운 다단계 예측 시스템을 설계한 것입니다. 기존의 존(zonal) 단위 예측에서 개별 노드(Nodal) 예측으로의 확장을 가능하게 하여, TSO가 불확실한 전력 수요에 보다 효과적으로 대응할 수 있도록 합니다. 연구는 TSO 데이터셋을 활용하여 해석 가능하고 확장 가능한 예측 모델을 개발하고 평가합니다.

- **Technical Details**: 연구는 Midcontinent Independent System Operator (MISO)에서 제공하는 방대한 데이터셋을 사용하여 시간 단위의 순전력 수요를 측정하고, 이를 기반으로 다단계 예측 시스템을 구현합니다. 모델의 설계는 TSO 운영자의 요구를 충족하기 위해 계층적 조정 방법과 데이터 풀링 접근 방식을 포함합니다. 이 과정에서 발생하는 이질성과 변동성을 해결하기 위한 여러 모델 학습 패러다임을 평가합니다.

- **Performance Highlights**: 결과적으로, 연구는 존 단위 예측의 정확성과 해석 가능성을 향상시키며, 노드 단위 예측에 대해서도 상당한 개선을 보여줍니다. TSO 운용자는 향상된 정확도로 예측을 조정할 수 있으며, 오류의 출처를 보다 명확하게 진단할 수 있는 도구를 제공합니다. 다단계 예측 시스템을 통해 TSO는 예측 조정의 신뢰성과 정확성을 극대화할 수 있습니다.
...
### Blackwell's Approachability for Sequential Conformal Inferenc (https://arxiv.org/abs/2510.15824)
Comments:
          25 pages, 0 figures

- **What's New**: 이 논문은 비거래 가능한 환경에서의 conformal inference에 대한 새로운 접근법을 탐구하고 있습니다. 기존의 Adaptive Conformal Inference (ACI) 방법을 반복적인 두 플레이어 벡터 값 유한 게임으로 재구성하여 도달할 수 있는 coverage와 efficiency 간의 tradeoff를 명확히 하였습니다. 이 논문은 Blackwell의 접근 가능성 이론을 활용하여 이러한 목표를 달성하려는 캘리브레이션 기반의 전략을 설계하였습니다.

- **Technical Details**: 저자들은 sequential conformal inference를 두 개의 플레이어가 반복적으로 참여하는 게임으로 모델링하였습니다. 이 접근은 기존의 방법에서 암시된 목표를 명확히 하고 ACI 분석의 여러 불일치성을 교정합니다. Blackwell의 접근 가능성 이론을 통해, 도달 가능한 coverage-efficacy 쌍의 집합이 기하학적으로 특성화될 수 있음을 보여줍니다.

- **Performance Highlights**: 이 알고리즘은 이론적인 보장을 강하게 제공하며, 실제적으로도 효율성을 보여주지만, 계산의 부담이 실제 배치를 제한할 수 있습니다. 특히, 기댓값 기반 접근 방식은 ACI의 전통적인 conformal prediction의 효율성 보장을 회복하면서도, 완전히 적대적인 환경에서도 coverage를 유지하는 유연성을 제공합니다. 이러한 결과들은 비거래성의 영역에서도 유용한 framework를 제공하며, worst-case 유효성과 실용적 효율성 간의 간극을 메웁니다.



### Error analysis of a compositional score-based algorithm for simulation-based inferenc (https://arxiv.org/abs/2510.15817)
- **What's New**: 본 논문은 시뮬레이션 기반 추정(SBI)에서 개별 관측치를 통해 합성 점수(compositional score)를 생성하여 매개변수를 추정하는 GAUSS 알고리즘을 연구합니다. 기존의 연구에서 미비했던, 관측치가 늘어날 때 발생하는 오류의 축적 효과를 이론적으로 분석하였습니다. 이를 통해 개별 점수 오류와 관측치 수에 따른 평균 제곱오차(Mean Squared Error, MSE)의 상한을 제시합니다.

- **Technical Details**: 시뮬레이션 기반 추정(SBI)은 확률적 모델의 매개변수를 추정하기 위한 틀로, 주로 조건부 점수 기반 모델링(Conditional Score-Based Modeling) 방식을 이용합니다. GAUSS 알고리즘은 개별 후방 점수(posteroir scores)를 집계하여 합성 점수를 생성하며, 이 과정에서 Gaussian 분포를 기반으로 하는 가정이 필요합니다. 본 논문에서는 최종적으로 제안된 식을 통해 MSE의 상한을 도출하고 이에 영향을 미치는 요인을 분석합니다.

- **Performance Highlights**: 이론적 발견은 다수의 관측치를 통해 매개변수를 더 정확하게 추정할 수 있음을 시사합니다. 특히, 합성 점수의 오류를 제어함으로써 다중 관측 후방 분포에서의 샘플링 품질을 보장할 수 있습니다. 본 연구는 SBI 분야에서의 샘플링 정확도 향상을 위한 여러 새로운 방향성을 제공하게 될 것입니다.



### On Universality of Deep Equivariant Networks (https://arxiv.org/abs/2510.15814)
Comments:
          Preprint. 22 pages

- **What's New**: 이 논문은 에퀴바리언트(neural networks에서 대칭(symmetry)를 인코딩하는 방식) 신경망의 보편성 결과를 보강하는 새로운 접근법을 제시합니다. 특히, 분리 제약 조건(activity separation constraint)을 두고 불변(invariant) 네트워크에 대한 보편성 정리를 수립하였고, 에퀴바리언트 네트워크에 대한 새로운 기준인 entry-wise separability를 도입하였습니다. 이로 인해 보편성을 달성하기 위해 깊이(depth)와 판독층(readout layers)이 결정적인 역할을 한다는 점을 명확히 하였습니다.

- **Technical Details**: 불변 네트워크에 대한 보편성 정리에 따르면, 완전 연결(reachable)된 판독층을 추가함으로써 모든 연속 함수(continuous functions)를 근사할 수 있습니다. 또한, 에퀴바리언트 네트워크에서는 표준 분리(separability) 개념이 불충분하다는 것을 확인하며, entry-wise separability라는 새로운 기준을 통해 이를 해결합니다. 이를 통해 에퀴바리언트 네트워크가 깊이가 충분하거나 특정 판독층을 장착할 경우 보편성에 도달할 수 있다는 사실을 보여줍니다.

- **Performance Highlights**: 저자들은 깊이와 판독층이 보편성을 이루는 데 중요한 요소로 작용한다는 것을 통계적으로 입증하였습니다. 또한, 이 연구는 기존의 얕은 모델에서의 보편성 실패 사례와 연관지어 새로운 통합된 관점을 제공합니다. 즉, 이 연구는 다양한 불변 및 에퀴바리언트 아키텍처에서의 보편성을 명확히 하고, 참고할 수 있는 결과들을 발표함으로써 해당 분야의 기초를 다지고 있습니다.



### DexCanvas: Bridging Human Demonstrations and Robot Learning for Dexterous Manipulation (https://arxiv.org/abs/2510.15786)
- **What's New**: DexCanvas는 7000시간의 인간 조작 데이터로 구성된 대규모 하이브리드 실-합성 데이터셋을 소개합니다. 이 데이터셋은 70시간의 실제 인간 시연에서 출발하였으며, 21가지 기본 조작 유형에 맞춰 조직됩니다. DexCanvas는 대규모 실제 시연, 체계적인 기술 범위, 물리적으로 검증된 접촉 주석을 결합한 첫 번째 조작 데이터셋입니다.

- **Technical Details**: DexCanvas는 다변화된 인간 시연을 원료로 사용하여 로봇 시스템을 학습할 수 있는 기반을 제공합니다. 이 데이터셋은 동기화된 다중 보기 RGB-D 이미지와 고정밀 모캡(motion capture) 및 물리적으로 일관된 힘 프로파일을 포함합니다. 실제-합성 파이프라인에서는 강화 학습(reinforcement learning)을 통해 MANO 손을 제어하는 정책을 학습하여 인간 시연을 재현하고 접촉력을 발견합니다.

- **Performance Highlights**: 이번 데이터셋은 로봇 조작 학습, 접촉이 풍부한 제어, 다양한 손 형태에 대한 기술 이전 연구를 촉진할 수 있습니다. 데이터셋의 모든 부분은 정확한 힘과 접촉 주석을 제공하여 기존 데이터셋의 중요한 공백을 메우는 데 기여합니다. DexCanvas는 높은 수준의 정확성과 활용성을 바탕으로 로봇 손기술 발전에 필요한 중요한 자원을 제공합니다.



### Enhanced Renewable Energy Forecasting using Context-Aware Conformal Prediction (https://arxiv.org/abs/2510.15780)
- **What's New**: 이 논문에서는 전력 시스템의 확률론적 예측을 보정하기 위한 새로운 접근 방식인 Context-Aware Conformal Prediction(CACP)을 제안합니다. CACP는 물리적으로 관련된 특징을 활용하여 보정 샘플에 더 높은 비중을 부여함으로써 더욱 신뢰할 수 있는 예측을 제공합니다. 이는 특히 재생 가능 에너지의 다양성과 변동성을 고려한 것으로, 기존 방법들과의 차별점을 보여줍니다.

- **Technical Details**: CACP는 기존의 Conformal Prediction (CP) 기법을 바탕으로 하여, 예측 모델의 출력물 보정 능력을 강화하는 혁신적인 무게 매김 방식입니다. 이 방법은 예측 시점에서 목표 조건과 가장 유사한 샘플에 더 높은 중요도를 부여하고, 이를 통해 모델의 변동성을 효과적으로 정량화합니다. 연구에서는 여러 미국의 전력 시스템에 대한 대규모 데이터 세트를 사용하여, CACP의 성능을 입증했습니다.

- **Performance Highlights**: CACP는 기존 모델에 비해 예측 신뢰도와 강건성을 크게 향상시키는 결과를 보여주었습니다. 특히 시스템 수준에서 보정을 약 12% 향상시키고, 사이트 수준에서 최대 55%의 개선을 달성했습니다. 이런 성과는 CACP가 효율적인 예측 간격을 유지하면서도 더욱 정확한 예측을 가능하게 한다는 것을 강조합니다.
...
- **What's New**: 본 논문에서는 [1m슬릿 클라이언트(slit-client)[22m 프레임워크를 제안하여, 비동기적으로 그래디언트와 커브처(curvature)를 별도로 계산하는 새로운 접근 방식을 구축했습니다. 이 방법은 지연과 불완전한 해시안(Hessian) 업데이트를 반영하여 Lazy Hessian 방법이 요구하는 수동 조정을 피할 수 있게 합니다. 이 연구는 비동기적 커브처 처리가 현대의 두 번째 최적화 방법에 있어 강력한 디자인 원칙이 될 수 있음을 보여줍니다.

- **Technical Details**: 최적화 문제를 다룰 때, 본 모델은 두 개의 계산 에이전트(gradient client와 curvature client)가 병렬로 작동하도록 설정됩니다. 그래디언트 클라이언트는 현재 반복에서 정확한 그래디언트를 계산하고, 커브처 클라이언트는 해시안의 근사값을 계산하되 지연을 수반할 수 있습니다. 지연 [1mτ[22m는 해시안 계산에 필요한 시간과 관련이 있으며, 이는 고차원 문제에서 매우 큰 값을 가질 수 있어 전체 성능에 실질적인 영향을 미칩니다.

- **Performance Highlights**: 실험 결과, 비동기적 커브처는 일반적인 Lazy Hessian 기준선과 비교해 일관되게 더 나은 성능을 보였으며, 정확한 커브처와 근사 커브처 모두에서 강력한 수렴 속도를 달성했습니다. 특히, 최적화 시간_profile 분석에서 해시안의 분해 비용이 주요 비용임을 확인하였으며, 우리 방법의 성능 개선은 이 비용을 성공적으로 겹치게 함으로써 도출되었습니다.
...
### Disentanglement of Sources in a Multi-Stream Variational Autoencoder (https://arxiv.org/abs/2510.15669)
- **What's New**: 이번 연구에서는 Variational Autoencoders (VAEs)을 활용한 새로운 접근 방식을 소개합니다. 전통적인 VAE는 연속적인 latent space를 사용하여 disentangled representations을 추구하지만, 여기서는 discrete latents를 이용하여 여러 소스의 VAE 표현을 결합하는 방법을 탐구합니다. 이를 통해 다중 소스 VAE (MS-VAE) 방식을 정의하고, 그 추론 및 학습 방정식을 유도하며, 수치적으로 그 기능성을 조사합니다.

- **Technical Details**: MS-VAE의 생성 모델 pΘ(x→)은 각 소스에 대한 인코더와 디코더의 가중치 Wk를 포함하여 구성됩니다. 본 연구에서는 데이터 x→(1:N)와 이산 latent s1:K, 연속 latent zk를 사용하여 MS-VAE의 확률적 모델을 명시합니다. 학습은 접근적 최대 가능도 접근법을 기반으로 하여 ELBO(변분하한)를 최적화하고, 모든 DNN의 가중치 및 바이어스를 포함하는 모델 매개변수 Θ를 설정합니다. 이를 통해 KL-divergence와 같은 통계적 개념을 활용해 파라미터 최적화를 수행합니다.

- **Performance Highlights**: 실험 결과, MS-VAE는 수동으로 쓰여진 숫자와 혼합된 음원에 대한 소스를 효과적으로 분리하는 능력을 보였습니다. 특히, speaker diarization 작업에서 잃어버린 화자의 비율이 현저히 낮아졌습니다. 또한 다양한 감독 및 훈련 데이터 양에 걸쳐 이 접근 방식의 유연성이 강조되었습니다.



### Bayesian Inference for PDE-based Inverse Problems using the Optimization of a Discrete Loss (https://arxiv.org/abs/2510.15664)
- **What's New**: 이번 연구에서는 최적화된 이산 손실 함수(ODIL)의 베이지안 확장인 B-ODIL을 소개합니다. 이 방법은 PDE 손실을 사전 지식으로 통합하고 관측 데이터를 설명하는 가능성과 결합하여 역문제를 해결합니다. B-ODIL은 확률적 불확실성을 정량화하면서 PDE 기반 역문제를 추론할 수 있는 능력을 보여주고 있습니다.

- **Technical Details**: B-ODIL은 PDE 또는 ODE에 의해 지배되는 복잡한 시스템에서 파라미터와 상태를 추정하는 문제를 다룹니다. 이 방법은 관측 데이터와 모델의 불확실성을 같이 고려하여 손실을 최소화하는 방식으로 동작합니다. 또한, 다양한 샘플링 기법과 근사를 사용하여 사후 분포를 추정함으로써 계산적 부담을 경감합니다.

- **Performance Highlights**: B-ODIL은 여러 복잡성 수준을 테스트한 벤치마크에서 우수한 성능을 발휘하였습니다. 예를 들어, 비선형 2차원 PDE 상태를 재구성하는 능력에서 B-ODIL은 진리의 불확실성 범위 내에 실제 결과를 포함하는 것을 보여주었습니다. 또한, MRI 스캔을 이용해 환자의 뇌에서 종양 세포의 농도를 추정하는 데에도 효과적임을 입증하였습니다.
...
- **What's New**: 본 논문은 이종 하드웨어 클러스터에서 머신러닝 워크로드를 관리하기 위한 학습 기반 아키텍처를 제안합니다. 새로운 하드웨어로의 업그레이드가 늘 불가능한 상황에서, 기존의 다양한 세대 자원을 지속 가능하게 활용하는 것이 필수적입니다. 이 시스템은 들어오는 훈련 또는 추론 요청에 자원을 할당하면서 에너지 소비를 최소화하고 성능 요구 사항을 충족하도록 설계되었습니다.

- **Technical Details**: 시스템은 두 개의 신경망을 사용합니다. 첫 번째 네트워크는 새로운 모델이 다양한 하드웨어 유형을 어떻게 활용할지를 초기 추정하는 역할을 하며, 두 번째 네트워크는 실제 성능을 모니터링하여 예측을 보완합니다. GOGH는 GPU 할당 문제를 정수 선형 프로그래밍(ILP) 모델로 공식화하여 처리하며, 이를 통해 에너지 효율 극대화와 최소 성능 보장을 통합하여 고려합니다.

- **Performance Highlights**: 실험 결과, GOGH는 공인된 데이터 세트를 통해 스케줄링 효율성과 예측 정확성을 모두 개선한 것으로 입증되었습니다. 특히, GOGH는 역사적 성능 데이터를 활용하여 다양한 GPU 유형에서의 작업 처리량을 예측하며, 더 나은 스케줄링 결정을 가능하게 합니다. 이러한 방식으로 GOGH는 딥러닝 클러스터에서 자원 할당 결정을 더욱 효과적으로 수행할 수 있게 됩니다.
...
### Stochastic Optimization with Random Search (https://arxiv.org/abs/2510.15610)
- **What's New**: 본 논문에서는 확률적 최적화(random search for stochastic optimization)를 재조명하고, 노이즈가 포함된 함수 평가만 가능할 때 이 방법이 이전보다 약한 매끄러움 가정 하에서도 작동함을 보여줍니다. 또한, 더 강한 가정이 향상된 보장을 가능하게 한점을 강조합니다. 새로운 변형인 분산 감소(random search) 방법을 제안하였으며, 여러 샘플을 활용하여 더 빠른 수렴을 이루는 방법을 설계하였습니다.

- **Technical Details**: 이 연구는 상대적으로 단순한 변환 불변성 속성(translation invariance property)을 바탕으로 하여 노이즈를 균형 있게 조절하고 분산을 줄이는 방법으로 분석합니다. 이전 연구에서 제안된 미니배치 STP(Minibatch STP) 방법과 비교할 때, 해당 연구는 Noise 또는 서브 샘플링된 목표 추정치와 효과적으로 작동하는 확률적 변형을 개발하는 데 중점을 두었습니다. 이를 통해 일반적으로 입력되는 여러 데이터 포인트에 대한 유한 합을 다루는 대규모 기계 학습 환경에서도 적용될 수 있습니다.

- **Performance Highlights**: 무작위 탐색(Random Search) 방법은 최적의 𝒪(d/ε4) 성능을 달성하며, 이는 기존의 기울기 추정 기반 방법과 동등한 성능을 나타냅니다. 본 연구는 평균 함수가 (L0, L1)-매끄럽다는 약한 가정 하에서도 여전히 𝒪(d3/ε6) 속도를 회복할 수 있음을 보여줍니다. 이러한 성과는 확률적, 유한합, 인간 피드백(human-in-the-loop) 환경에서 무작위 탐색 방법의 첫 통합 분석을 제공합니다.



### Kernel-Based Evaluation of Conditional Biological Sequence Models (https://arxiv.org/abs/2510.15601)
Comments:
          29 pages

- **What's New**: 본 연구에서는 조건부 시퀀스 모델의 설계 평가 및 하이퍼파라미터 조정을 위한 커널 기반 도구 세트를 제안합니다. 이 도구의 핵심은 Augmented Conditional Maximum Mean Discrepancy (ACMMD)라는 새로운 불일치 척도입니다. ACMMD는 모델이 샘플링 가능할 경우, 데이터를 통해 편향되지 않게 추정될 수 있으며, 절대 모델 적합도를 정량화하고 가설 검정에 통합될 수 있습니다.

- **Technical Details**: 조건부 시퀀스 모델은 현재 머신러닝의 주요 모델 클래스 중 하나이며, 자연어 생성에서 생물 의학 응용까지 다양한 분야의 오랜 문제들을 해결하는 데 기여하고 있습니다. 본 연구에서 제안한 ACMMD는 조건부 확률 분포 간의 불일치를 측정하는 새로운 방법론으로, 샘플을 사용하여 추정할 수 있습니다. 조건부 정확도 평가 문제를 다루고 있으며, 추정된 모델 Q|Q_{|}의 정확성과 신뢰성을 정량화할 수 있는 방법을 제시합니다.

- **Performance Highlights**: ProteinMPNN이라는 인기 있는 단백질 설계 모델을 분석함으로써 제안된 방법의 유용성을 입증하였습니다. 실험을 통해 여러 단백질 계열에서 ProteinMPNN이 데이터에 적합하지 않다는 가설을 기각할 수 있었으며, 모델의 온도 하이퍼파라미터를 조정하여 더 나은 적합도를 달성할 수 있었습니다. 이 연구는 모델 평가의 가능성을 극대화하는 새로운 접근 방식을 제공합니다.
...
### Geometric Convergence Analysis of Variational Inference via Bregman Divergences (https://arxiv.org/abs/2510.15548)
Comments:
          14 pages, 4 figures

- **What's New**: 이 논문에서는 Variational Inference (VI)의 수렴 분석을 위한 새로운 이론적 프레임워크를 제시합니다. 이를 통해 비구조적으로 보이는 문제를 파악하고, ELBO (Evidence Lower Bound)의 경량 복잡성을 지니고 있음에도 불구하고 수렴을 보장하는 강인한 구조를 제공합니다. 또한 Bregman divergence를 활용하여 최적화 환경을 기하학적으로 분석할 수 있는 방법을 제안하고 있습니다.

- **Technical Details**: 연구는 음의 ELBO를 Bregman divergence로 표현하며, 이는 log-partition function에 대한 기하학적 분석을 가능하게 합니다. 이러한 기법을 통해 다양한 파라미터 공간에서의 수렴 속도와 하나의 지점에서의 PL-type 불평등을 도출할 수 있었습니다. 이 접근법으로는 GRADIENT DESCENT 알고리즘이 상수 및 감쇠 단계 크기를 포함한 경우에 대해 큰 성능 향상을 보여줍니다.

- **Performance Highlights**: 분석을 통해 자연 기울기(Natural Gradient) 방법의 수렴 보장과 함께 특정 geometric 구조의 존재를 확인하였습니다. 이러한 조건 하에, VI의 일반적인 수렴률을 정량화할 수 있는 이론적 결과를 도출하였으며, 기존의 비선형 최적화 이론을 반영한 개선된 알고리즘 성능도 보였습니다. 또한, Fisher 정보 행렬의 스펙트럼 특성을 이용하여 규제를 구축함으로써 더욱 정밀한 수렴 속도 보장을 확립합니다.
...
### SpikeFit: Towards Optimal Deployment of Spiking Networks on Neuromorphic Hardwar (https://arxiv.org/abs/2510.15542)
Comments:
          13 pages, 2 figures. Work in progress

- **What's New**: 이 논문은 Spiking Neural Networks (SNNs)를 위한 새로운 훈련 방법인 SpikeFit을 소개합니다. SpikeFit은 neuromorphic hardware의 엄격한 요구 사항을 고려하여 효율적인 추론을 가능하게 합니다. 기존 방법들이 해결하지 못했던 여러 요구 사항을 동시에 충족시키며, 높은 네트워크 압축 효율을 달성합니다.

- **Technical Details**: SpikeFit은 허용되는 가중치의 이산 값을 학습 가능한 파라미터로 보고, 저정밀도(2-비트, 4-비트 등) 모델 가중치의 최적화된 Clusterization-Aware Training (CAT)을 허용합니다. 또한, 새로운 하드웨어 친화적인 Fisher Spike Contribution (FSC) 가지치기 방법을 포함하여, SNN의 압축에 있어 선행 기술보다 더 뛰어난 성능을 보여줍니다.

- **Performance Highlights**: SpikeFit 방법은 특히 4개의 고유한 시냅스 가중치 값만을 사용할 때도 우수한 성능을 발휘하는 것으로 나타났습니다. 실험에서는 에너지 사용량을 최소화하면서 보다 광범위한 neuromorphic 하드웨어 요구 사항을 충족했습니다. 기존 SNN 압축 방법들과 비교해도 우수한 성능을 보임을 확인했습니다.
...
### AI and analytics in sports: Leveraging BERTopic to map the past and chart the futur (https://arxiv.org/abs/2510.15487)
Comments:
...
- **What's New**: 이 연구는 인공지능(AI), 분석(analytics) 및 스포츠의交差점에서의 학술 문헌을 체계적으로 분석하는 것을 목표로 하고 있습니다. 특히, 2002년부터 2024년까지 발표된 204개의 저널 기사를 식별하여 AI와 분석의 활용에 대한 통찰을 제공합니다. BERTopic이라는 새로운 주제 모델링 기법을 활용하여 스포츠 연구의 잠재 구조를 도출합니다.

- **Technical Details**: 이 연구는 체계적 문헌 검토(Systematic Literature Review, SLR)를 사용하여 PRISMA(Preferred Reporting Items for Systematic Reviews and Meta-Analysis) 프로토콜을 따릅니다. 연구에서는 성과 모델링(performance modelling), 신체 및 정신 건강(physical and mental health), 소셜 미디어 감성 분석(social media sentiment analysis), 전술 추적(tactical tracking) 등의 주요 주제를 도출하여 각각의 상대적 중요성과 주요 연구를 분석합니다.

- **Performance Highlights**: 이 연구는 AI와 분석이 스포츠에 미치는 변혁적인 영향력에 대한 통찰을 제공하여 학계와 스포츠 관리자를 위한 유용한 정보를 창출합니다. 연구에서 제시된 통계 및 주제는 향후 연구의 유망한 경로를 delineate하며, 학술적 이해와 분야의 방법론적 도구를 한층 더 발전시킵니다.



### Online Policy Learning via a Self-Normalized Maximal Inequality (https://arxiv.org/abs/2510.15483)
- **What's New**: 이번 논문에서는 적응적 실험에서 발생하는 종속 데이터를 고려하여 martingale empirical processes에 대한 self-normalized maximal inequality를 개발하였습니다. 이로 인해 표본 분산 패널리제이션(sample-variance penalization) 절차를 제안하여 경험적 손실과 표본 분산을 조화롭게 조절할 수 있습니다. 이는 적응적으로 수집된 정책 학습에 적용 가능하며, 새로운 분산 규제 비관적 오프 정책 학습 목표를 도출합니다.

- **Technical Details**: 제안된 접근법은 총 세 가지 기여로 구성됩니다: (i) martingale empirical processes에 대한 새로운 self-normalized maximal inequality를 수립하여 i.i.d.의 범위를 넘어 표본 분산 패널리제이션을 확장합니다; (ii) adaptively 수집된 bandit 피드백을 기반으로 분산 적응 초과 위험 보증을 유도합니다; (iii) 모든 과거 데이터를 재사용하고, 매개변수적(parametric) 및 비매개변수적(nonparametric) 영역 간에 부드럽게 보장하는 온라인 알고리즘을 개발합니다.

- **Performance Highlights**: 우리는 실험을 통해 제안한 방법이 기존 방식에 비해 안정성과 성능을 일관되게 개선함을 입증했습니다. 특히 제한된 탐색이나 높은 적응성이 있는 환경에서 그 효과가 더욱 두드러집니다. 이 논문은 Bernstein 스타일의 분산 패널리제이션과 적응적, 순차적 정책 업데이트를 통합하는 첫 번째 프레임워크를 제공합니다.
...
### Nonlinear Dimensionality Reduction Techniques for Bayesian Optimization (https://arxiv.org/abs/2510.15435)
Comments:
          34 pages including appendixes, 8 figures. Keywords: global optimisation, dimensionality reduction techniques, Bayesian methods, Variational Autoencoders

- **What's New**: 이 논문에서는 한계가 있는 고차원 Bayesian Optimization (BO)의 확장성 문제를 해결하기 위해 Nonlinear Dimensionality Reduction 기술을 적용합니다. 구체적으로, Latent-Space BO (LSBO)를 통해 저차원 최적화 문제로 전환하며, Variational Autoencoders (VAEs)를 이용해 구조적 잠재 매니폴드를 학습합니다. 우리는 또한 Sequential Domain Reduction (SDR) 방법을 통해 탐색 효율성을 향상시키고자 하며, 이는 LSBO와 결합하여 보다 나은 최적화를 이끕니다.

- **Technical Details**: 이 연구에서는 Variational Autoencoders (VAEs)를 사용하여 데이터로부터 비선형 임베딩을 학습하는 방법론을 제시합니다. VAEs는 Encoder-Decoder 아키텍처를 통해 원래의 고차원 정보를 저차원 잠재 공간으로 변환하며, 각각의 반복마다 SDR을 통해 효율적인 탐색을 촉진합니다. 특정 구현 변화가 기존의 분자 생성 작업에 맞춰져 있었던 것을 보고하며 더 폭넓은 최적화 문제를 위해 알고리즘을 재구성하였습니다.

- **Performance Highlights**: 실험 결과, 우리는 다양한 벤치마크 작업에서 향상된 최적화 품질을 관찰했습니다. VAEs 기반의 차원 축소 방식이 랜덤 임베딩보다 뛰어난 성능을 보여주며, SDR과 VAEs의 결합이 고차원에서의 BO 성능을 크게 개선하는 데 기여함을 확인했습니다. 논문의 코드는 reproducibility (재현성)가 가능하도록 제공되며, 이는 연구자들에게 새로운 최적화 접근법에 대한 실습 기회를 제공합니다.
...
### Information Theory in Open-world Machine Learning Foundations, Frameworks, and Future Direction (https://arxiv.org/abs/2510.15422)
- **What's New**: 이 논문은 Open World Machine Learning (OWML)의 정보 이론적 접근 방식을 포괄적으로 검토하고 있습니다. 기존의 연구들을 세 가지 주요 축으로 통합하며, 안전한 미지 식별과 새로운 개념 형성, 그리고 지속적인 학습이 어떻게 이루어지는지를 설명하고자 합니다. 정보 이론의 핵심 개념인 엔트로피(Entropy), 상호 정보(Mutual Information), 쿨백-라이블러 발산(Kullback-Leibler Divergence)을 활용하여, 학습의 적응성과 리스크 관리를 수학적으로 설명하는 방법을 제시합니다.

- **Technical Details**: 정보 이론은 불확실성(uncertainty)과 정보 흐름(information flow)을 정량화하는 수학적 프레임워크를 제공합니다. 엔트로피는 데이터의 불확실성을 나타내는 도구로, 상호 정보는 두 변수 간의 의존성을 측정합니다. 쿨백-라이블러 발산은 확률 분포 간의 차이를 나타내며, 이들 세 가지 개념은 현대 머신러닝 시스템에서 지식과 학습 동역학을 측정하는 기초가 됩니다. 이러한 개념을 바탕으로 OWML 환경에서의 정보 흐름을 분석할 수 있습니다.

- **Performance Highlights**: OWML의 발전은 지식 유지와 미지 식별 간의 균형이 상호 정보의 최적화로 해석될 수 있다는 점에 주목합니다. 최근 연구들은 정보 이론이 OWML의 이해 및 공식화에 필요한 이론적 기초를 제공할 수 있음을 시사하고 있으며, 이는 모델의 안전한 인식, 미지의 노출 리스크 제어와 같은 문제를 해결하는 데 기여할 수 있습니다. 향후 정보 리스크 정량화, 동적 상호 정보 한계 개발 등 여러 해결해야 할 문제들을 제시하며 OWML의 신뢰성을 높이는 방향으로 나아갈 것을 강조합니다.
...
### Recursive Inference for Heterogeneous Multi-Output GP State-Space Models with Arbitrary Moment Matching (https://arxiv.org/abs/2510.15390)
- **What's New**: 이 논문은 시스템 동역학 학습을 위해 개선된 Gaussian Process State-Space Model (GPSSM) 프레임워크를 제안합니다. 이를 통해 다중 출력 전이 함수 학습과 최첨단 베이지안 필터 기법을 incorporat하여 온라인 학습의 유연성과 정확성을 크게 향상시켰습니다. 이 연구의 주요 기여는 이질적인 다중 출력 커널 설계 및 고유한 inducing-point 관리 알고리즘을 통해 얻어진 컴퓨팅 효율성입니다.

- **Technical Details**: 제안된 프레임워크는 이질적인 다중 출력 커널을 통해 각각의 출력 차원에서 서로 다른 입력 변수, 커널 함수 및 하이퍼 파라미터를 사용할 수 있게 하여 표현력을 개선합니다. 또한, 새로운 inducing-point 관리 전략을 통해 각 출력 차원에 대해 독립적인 선택과 중복 포인트 제거가 가능하여 계산 효율성과 수치적 안정성을 높였습니다. GPSSM에 대한 통합 추론 프레임워크는 EKF(Extended Kalman Filter) 및 UKF(Unscented Kalman Filter)와 같은 일반적인 모멘트 일치 기법을 지원합니다.

- **Performance Highlights**: 제안하는 방법은 SOTA (State-of-the-Art) 오프라인 GPSSM과 비교할 때 1/100의 실행 시간으로 동일한 정확도를 달성하며, SOTA 온라인 GPSSM보다 약 70% 높은 정확도를 보여줍니다. 이 연구는 합성 데이터와 실제 데이터 세트를 바탕으로 평가되었으며, 특히 하이퍼슨 비행체 및 쿼드로터 동역학 학습에서 뛰어난 성능을 입증했습니다.
...
### TranSimHub:A Unified Air-Ground Simulation Platform for Multi-Modal Perception and Decision-Making (https://arxiv.org/abs/2510.15365)
Comments:
          9 pages, 4 figures

- **What's New**: TranSimHub는 공중-지상 협업 지능을 위한 통합 시뮬레이션 플랫폼으로, 다양한 시각 및 환경 모델을 지원하여 복잡한 도시에 적합한 지능형 교통 관리를 가능케 합니다. 기존의 고립된 시뮬레이터들과 달리, TranSimHub는 RGB, 깊이(Depth), 및 의미론적 분할(Semantic Segmentation) 모드에서 동기화된 다중 뷰 렌더링을 제공하여 지상 및 공중 시점 간의 일관된 인식을 확보합니다. 이 플랫폼은 정보 교환을 지원하고 사용자 정의가 가능한 시나리오 생성을 통해 다양한 조건에서 인식 및 의사 결정 연구를 촉진합니다.

- **Technical Details**: TranSimHub는 환경 제공자 계층, 시뮬레이션 및 제어 계층, 통합 인터페이스 계층의 세 가지 계층으로 구성되어 있습니다. 이 플랫폼은 OpenStreetMap에서 가져온 정적 요소와 SUMO와 같은 엔진을 통해 제어되는 동적 요소를 포함하여 사용자 정의가 가능한 3D 세계를 모델링합니다. 그리고 리인포스먼트 러닝(RL)과 같은 제어 프레임워크와의 통합을 지원하여 복합적인 상호작용을 관리하며, Blender를 사용하여 고화질 시각화를 제공합니다.

- **Performance Highlights**: TranSimHub는 다중 뷰 및 다중 모드의 데이터를 생성하는 용이성을 제공하여 공중-지상 협업 시스템에서의 인식, 융합 및 의사 결정 연구의 효과를 극대화합니다. 사용자들은 환경 설정에 따라 다양한 날씨 변화, 동적 이벤트, 응급 차량을 추가하여 모델의 견고성과 안전성을 평가하는 데 유용한 시나리오를 생성할 수 있습니다. 이러한 기능들은 복잡한 교통 환경에서 모델의 인터프리터 및 인과 일반화를 체계적으로 분석하는 데 필수적입니다.
...
### Singularity-free dynamical invariants-based quantum contro (https://arxiv.org/abs/2510.15340)
- **What's New**: 본 논문에서는 비마르코프(open quantum systems) 시스템에서 단일 큐비트 상태 준비를 위한 일반화된 불변 기반 프로토콜을 제안합니다. 기존 연구들은 대부분 고립 시스템에 초점을 맞추었거나 마르코프적 잡음만을 다루어 왔지만, 본 연구는 일반적인 잡음 조건 하에서도 상태 준비를 가능하게 합니다. 이를 통해 노이즈를 고려한 제어 합성을 위한 첫 번째 단계로 제한된 제어 펄스를 생성하고, 두 번째 단계에서 최적의 펄스를 식별하여 잡음의 영향을 최소화합니다.

- **Technical Details**: 제어 Hamiltonian Hctrl(t)은 큐비트의 시간에 따른 상태 변화를 조절하는데 사용되며, Hnoise(t)은 시스템에 영향을 미치는 잡음을 나타냅니다. 논문에서 제안하는 프로토콜은 두 가지 설정을 고려합니다: (i) 잡음 특성이 알려진 경우, 이를 통해 잡음의 영향을 줄이기 위한 분석적 비용 함수를 사용하고, (ii) 잡음이 알려지지 않은 경우에는 머신러닝(ML) 모듈을 제어 루프에 통합합니다. 이러한 접근 방식은 콤플렉스한 잡음 환경에 적응할 수 있도록 설계되었습니다.

- **Performance Highlights**: 수치 시뮬레이션 결과, 제안된 프로토콜은 다양한 큐비트 목표에 대해 높은 충실도의 상태 준비를 달성하였습니다. 또한, 생성된 제어 필드는 부드럽고 하드웨어에서 사용할 수 있는 형태로 나타납니다. 이는 비마르코프 동역학을 보이는 노이즈 시스템에서도 적용 가능하며, 기존의 GRAPE와 같은 경량 최적화 기법에 비해 보다 강건하고 확장성이 있는 접근 방식을 제공합니다.



### Transfer Learning for Benign Overfitting in High-Dimensional Linear Regression (https://arxiv.org/abs/2510.15337)
Comments:
          42 pages, 4 figures, 2 tables, 1 algorithm; camera-ready version accepted at NeurIPS 2025 (Spotlight)

- **What's New**: 이 연구에서는 전이 학습(transfer learning)과 최소-$	ext{ℓ}_2$-노름 보간기(minimum-$	ext{ℓ}_2$-norm interpolator, MNI)의 접점을 탐구하여 두 가지 구성 요소의 통합 방안을 제안합니다. 새로운 전이 MNI 접근법을 통해 타겟 전용 MNI를 향상시킬 수 있는 방법을 분석해 이들의 비대칭적 초과 위험(non-asymptotic excess risk)을 정의합니다. 특히, 이 연구는 서로 다른 데이터 출처를 활용하여 전이 학습의 이익을 달성할 수 있는 조건을 규명하고 있습니다.

- **Technical Details**: 연구는 두 단계의 Transfer MNI 접근법을 제안하며, 첫 번째 단계에서 소스 전용 MNI를 사전 훈련(pre-train)하고 두 번째 단계에서 타겟 데이터를 보간(interpolate)하여 미세 조정(fine-tune)합니다. 또한, 모델 이동(model shift)과 공변량 이동(covariate shift) 조건 하에서 Transfer MNI가 언제 타겟 전용 MNI보다 성능이 더 우수한지를 식별합니다. 데이터 기반 프로세스를 개발하여 긍정적인 전이를 유도하는 정보 출처를 식별하고, 이를 통해 다수의 유익한 Transfer MNI를 집계하는 앙상블 방법을 도입합니다.

- **Performance Highlights**: 유한 샘플 실험은 모델 및 데이터 이질성에 대한 방법의 강인성을 입증하였으며, 이전 방법들과 비교하여 탁월한 성능을 보여주었습니다. 특히, 기존 pooled-MNI와 SGD 기반 전이 방법보다 우수한 결과를 확인하였고, 고차원 선형 회귀 분야에서의 성과를 도출했습니다. 이 연구는 전이 학습이 고차원 회귀에서의 일반화 성능을 더욱 향상시킬 수 있다는 실질적인 이점을 강조합니다.
...
### Foresighted Online Policy Optimization with Interferenc (https://arxiv.org/abs/2510.15273)
- **What's New**: 본 논문에서는 정수적 결정-making 과정에서 발생하는 interference를 고려한 foresighted online policy with interference(이하 FRONT)을 제안합니다. 기존의 contextual bandit 방법들이 개별 행동만을 고려한 것과 달리, FRONT은 이전 행동이 이후 결과에 미치는 장기적인 영향을 반영하여 최적의 성과를 달성합니다. 이는 온라인 환경에서의 탐색(exploration)과 착취(exploitation)를 동시에 관리하여, 정책의 단기 성과뿐만 아니라 장기적인 보상을 최대화하기 위한 새로운 접근 방식을 제공합니다.

- **Technical Details**: 제안된 FRONT 방법은 이질적인 처리 효과와 동질적인 interference 효과를 고려한 online additive outcome 모델을 기반으로 합니다. 이 모델은 각 결정 단계에서 현재 행동의 장기 효과를 반영하기 위해 확장된 exposure mapping 기능을 도입합니다. 또한, 두 단계의 의존 구조를 관리하기 위해 ϵ-Greedy 전략과 force-pull trigger를 활용하여 탐색을 수행하고, 온라인 추정기와 매개변수 추론 절차를 개발했습니다.

- **Performance Highlights**: 존재하는 두 가지 정의의 regret을 고려했을 때, FRONT은 두 경우 모두 sublinear한 regret을 달성하는 것으로 나타났습니다. 이는 FRONT이 제안한 정책이 최적의 정책과 비교하여 생성하는 누적 차이를 줄이는 데 매우 효과적임을 의미합니다. 실제 도시 호텔 수익에 대한 응용을 통해 FRONT의 효과성을 입증하였으며, 이론적 결과와 실험적 결과 모두에서 의미 있는 성과를 보여줍니다.



### Minimisation of Submodular Functions Using Gaussian Zeroth-Order Random Oracles (https://arxiv.org/abs/2510.15257)
- **What's New**: 이 논문에서는 서브모듈러 함수(submodular function)의 최소화(minimization) 문제를 다루고, 해당 문제에 제로스 오더(zero-order) 방법을 적용합니다. 이 방법은 가우시안 스무딩(Gaussian smoothing) 랜덤 오라클(oracle)을 활용하여 스무딩된 함수의 그래디언트를 추정하는 데 초점을 맞추었으며, 알고리즘이 오프라인 환경에서 글로벌 $	ext{epsilon}$ 근사 솔루션(global $	ext{epsilon}$-approximate solution)으로 수렴한다는 것을 증명하였습니다.

- **Technical Details**: 우리는 알고리즘이 정적 회귀(static regret) 측면에서 하난-일관성(Hannan-consistent)하며, 동적 회귀(dynamic regret)가 $O(	ext{sqrt}(N P_N^*)$ 이라는 점을 강조합니다. $N$은 반복(iterations)의 수, $P_N^*$는 최적화 문제의 경로 길이(path length)를 나타냅니다. 또한 복잡도 분석(complexity analysis)과 하이퍼파라미터 선택(hyperparameter selection)도 모든 경우에 대해 제공하였습니다.

- **Performance Highlights**: 가우시안 스무딩 제로스 오더 방법은 기대값 기대에 따라 $O(n^2 	ext{epsilon}^{-2})$ 함수 호출(function calls)만에 convergence에 성공합니다. 온라인 환경에서도 정적 회귀와 동적 회귀 모두에 대한 알고리즘의 성능이 향상되었으며, 평균 회귀가 반복 횟수가 증가함에 따라 사라짐을 보여주었습니다. 이 연구는 SFM 문제의 해결을 위한 중요한 발전을 나타내며, 미래 연구 방향에 대한 논의도 포함되어 있습니다.
...
### How to Sell High-Dimensional Data Optimally (https://arxiv.org/abs/2510.15214)
- **What's New**: 이 논문은 대규모 독점 데이터 판매 문제를 다루고 있으며, 판매자가 구매자의 유틸리티를 극대화하는 결정을 내릴 수 있도록 돕는 정보를 어떻게 가격 책정할 수 있는지를 설명합니다. 판매자는 구매자가 더 정확한 정보를 바탕으로 더 나은 결정을 내리게 할 경우 추가 정보를 제공하고 그에 대한 대가를 받을 수 있습니다. 본 연구에서 제안된 알고리즘은 주어진 샘플을 통해 상태 공간에 의존하지 않는 수익 극대화를 이룰 수 있는 메뉴를 생성합니다.

- **Technical Details**: 본 연구는 구매자의 개인 선호를 고려하여 통계적 실험을 공급하는 메뉴를 설계하는 문제로, 데이터의 차원이 증가할수록 상태 공간이 지수적으로 성장한다는 점에서 중요한 과제가 있습니다. 제안된 알고리즘은 샘플링 접근만으로도 근사 최적 메뉴를 생성할 수 있으며, 이 과정은 행동의 수에 대해 다항적으로 의존합니다. 특히, 고차원 가우시안 데이터를 다루는 특별한 경우를 분석하며, 스칼라 가우시안 실험을 고려하는 것이 수익 극대화에 충분하다는 점을 명확히 하였습니다.

- **Performance Highlights**: 제안된 알고리즘은 구매자의 신념 분포에 따라 샘플을 수집하여 높은 확률로 근사 최적 메뉴와 가격을 계산하며, 이에 대한 시간 복잡도와 샘플 복잡도는 상태 공간의 크기에 독립적입니다. 본 연구는 다양한 데이터 제품을 제공하여 이질적 구매자로부터 수익을 추출하는 데 있어 중요한 질적 특성을 보여줍니다. 또한, 결정론적 데이터 제품의 사용이 '인위적인' 무작위성을 도입하지 않으면서도 효과적이며 수익 극대화를 이루는 조건을 제시합니다.



### HyperAIRI: a plug-and-play algorithm for precise hyperspectral image reconstruction in radio interferometry (https://arxiv.org/abs/2510.15198)
Comments:
          18 pages, 10 figures, submitted to MNRAS

- **What's New**: 이 논문에서는 AIRI 알고리즘의 하이퍼스펙트럴 확장인 HyperAIRI를 도입합니다. HyperAIRI는 다이나믹 레인지에 맞춘 여러 학습된 denoiser를 사용하여 스펙트럼 채널의 이미지를 동시에 처리하는 기능을 제공합니다. 이를 통해 기존 알고리즘의 한계를 극복하고, 고해상도 및 하이 다이나믹 레인지 이미지를 효과적으로 생성할 수 있습니다.

- **Technical Details**: HyperAIRI 모델은 각 스펙트럼 채널의 현재 이미지 추정치를 기반으로 하여 두 이웃 채널의 추정치와 스펙트럼 인덱스 맵을 입력으로 받아들입니다. 이 모델의 수렴을 보장하기 위해 Jacobian 정규화를 적용하며, 다양한 다이나믹 레인지를 수용하기 위해 사전 훈련된 denoiser를 배열하여 사용합니다. Hyper-uSARA는 HyperSARA의 최적화 기반 변형으로, 스펙트럼 채널 전반에 걸쳐 조인트 스파시티를 촉진하는 구조를 가집니다.

- **Performance Highlights**: HyperAIRI는 시뮬레이션된 데이터와 실제 관측데이터에서 Hyper-uSARA, CLEAN의 하이퍼스펙트럴 변형, 그리고 단일 색상 이미징 알고리즘인 AIRI 및 uSARA와 비교하여 뛰어난 성능을 보여줍니다. AIRI 알고리즘의 장점을 살리면서도, HyperAIRI는 스펙트럼 채널 간의 상관관계를 이용하여 고정밀 이미지 복원을 가능하게 합니다. 이로 인해 대규모 관측에서의 사용 가능성을 높이고 있습니다.



### OCR-APT: Reconstructing APT Stories from Audit Logs using Subgraph Anomaly Detection and LLMs (https://arxiv.org/abs/2510.15188)
- **What's New**: 이 논문에서는 고급 지속적 위협(Advanced Persistent Threat, APT)을 탐지하고 인간과 같은 공격 이야기를 재구성할 수 있는 OCR-APT 시스템을 소개합니다. 기존 시스템은 선형 로그 표현에 의존하여 발견하지 못했던 관계를 드러내는 Provenance graphs를 활용하여 더 나은 탐지를 목표로 합니다. OCR-APT는 Graph Neural Networks (GNNs)와 대형 언어 모델(Large Language Models, LLMs)을 결합하여 더욱 정교한 탐지 및 해석 가능한 공격 보고서를 생성합니다.

- **Technical Details**: OCR-APT는 두 가지 주요 컴포넌트로 구성되어 있습니다: GNN 기반의 서브그래프 이상 탐지기와 LLM 기반의 공격 조사기입니다. 서브그래프 이상 탐지기는 관계형 그래프 합성곱 신경망(Relational Graph Convolutional Network, RGCN)과 일급 SVM(One-Class SVM)을 통합하여 이상 행동을 탐지합니다. 이후 탐지된 이상 노드는 구조적 및 행동적 일관성을 바탕으로 그룹화되며, 각 서브그래프는 고유한 특성에 따라 점수를 받아 필터링됩니다.

- **Performance Highlights**: 실험 결과, OCR-APT는 현재 최고 수준의 시스템들에 비해 이상 탐지 정확도와 알림 해석 가능성에서 일관되게 우수한 성능을 보여줍니다. 평균 F1-score가 0.96에 이르며, NODLINK(0.248)와 FLASH(0.945)를 초과하여 서브그래프 기반 탐지의 새로운 진전을 이룹니다. OCR-APT는 또한 간결하고 인간이 이해할 수 있는 공격 보고서를 생성하여 조사 시간을 단축하고 경고 분류를 효율적으로 개선합니다.



### Beyond PCA: Manifold Dimension Estimation via Local Graph Structur (https://arxiv.org/abs/2510.15141)
- **What's New**: 본 논문에서는 국소 주 성분 분석(Local PCA)을 개선한 곡률 조정 PCA(CA-PCA)를 통해 다변량 구조에 대한 보다 정밀한 차원 추정(framework) 방법론을 제안합니다. 새로운 프레임워크는 PCA를 회귀(regression) 기법과 통합하여 매니폴드의 국소 그래프 구조를 포착합니다. 이 연구는 특히 향상된 차원 추정을 위한 두 가지 대표적인 추정기인 사각 임베딩(quadratic embedding)과 총 최소 제곱(total least squares)을 소개합니다.

- **Technical Details**: 논문에서는 매니폴드 가설을 바탕으로 하여 데이터가 고차원 공간 내의 저차원 매니폴드에 위치한다고 가정합니다. 기존의 전통적인 기법들은 주로 평면 가정(flatness assumption)을 기반으로 하고 있으며, 이는 데이터의 국소 지역을 고차원 선형 공간으로 근사화합니다. 이에 반해 CA-PCA는 로컬 PCA 절차에 곡률 정보를 명시적으로 통합하여 더 많은 정보를 갖춘 차원 추정 방법을 제공합니다.

- **Performance Highlights**: 실험 결과 두 가지 새로운 추정기인 QE와 TLS가 최신 기법들과 비교해도 경쟁력 있는 성능을 보이며, 특히 샘플 사이즈가 작거나 비선형적으로 매핑된 경우에 종종 더 우수한 성능을 나타낸다는 것을 보여줍니다. 이러한 성과는 현장 적용 가능성을 높이며, 기계 학습 분야에서의 효과적인 데이터 표현을 지원할 것으로 기대됩니다.
...
### Polarization based direction of arrival estimation using a radio interferometric array (https://arxiv.org/abs/2510.15116)
- **What's New**: 이번 연구에서는 기존의 전파 간섭 배열(radio interferometric array)을 사용하여 DOA(방향 도착) 추정을 수행하는 새로운 방법을 제안합니다. 이 방법은 이미지 생성 없이 저렴한 비용으로 신호의 방향을 빠르게 추정하는 데 적합합니다. 특히, 이 기법은 이동 수신기 및 제한된 데이터로도 효율적으로 사용될 수 있으며, 안정적인 방해 신호(RFI) 완화 후단계에서도 유용합니다.

- **Technical Details**: 제안된 방법은 하나의 지배적인 소스만 존재한다고 가정하고, 상호 연관된 두 수신기로부터 수집한 데이터를 사용하여 DOA를 추정합니다. 이 과정에서 ESPRIT(Estimation of Signal Parameters via Rotational Invariance) 알고리즘을 활용하여 각 베이스라인의 위상 각도(phase angle)를 추정합니다. 각각의 서브 배열(sub-array)의 위상을 통합하여 최종적으로 2D DOA를 추정하는 비선형 비용 함수(minimization of a non-linear cost function)를 최소화하는 방식으로 작동합니다.

- **Performance Highlights**: 이 논문에서 제안된 방법은 기존의 DOA 추정 기법에 비해 컴퓨팅 비용이 훨씬 적고, 임의의 배열 형상에서도 작동 가능합니다. 또한, 전체 데이터 대신 일부 데이터만으로도 신뢰성 있는 결과를 도출할 수 있어, 온라인 RFI 탐지와 결합하여 실시간 처리에 최적화된 성능을 나타냅니다. 시뮬레이션 결과를 통해 실제 두 개의 전파 간섭 배열에서의 효과성을 입증하며, 저비용으로도 높은 정확도의 방향 추정을 가능하게 합니다.
...
- **What's New**: 이번 연구에서는 Large Language Models (LLMs)의 훈련 과정에서 발생할 수 있는 잠재적 Backdoor 공격에 대한 검증 프로토콜인 Proof-of-Training Steps (PoTS)를 제안합니다. 기존의 검증 방법이 LLM의 규모와 복잡성 때문에 비효율적이라는 점을 강조하며, PoTS는 훈련 단계와 검증 단계를 교대로 실행함으로써 조기 공격 탐지를 가능하게 합니다. 이 프로토콜은 외부 감사인이 LLM 개발자가 공표한 훈련 요리법을 준수했는지를 확인할 수 있는 점에서도 혁신적입니다.

- **Technical Details**: PoTS는 LLM의 훈련 과정에서 의도적으로 삽입된 잘 알려지지 않은 백도어 공격을 감지하기 위해 언어 모델 헤드 (Language Modeling Head, LM-Head)의 민감도를 분석합니다. 기존의 방법들과는 달리, LLM의 최종 레이어만을 검토하여도 효과적으로 백도어를 탐지할 수 있음을 보여줍니다. 이로 인해 검증 비용이 최대 70%까지 절감되며 공격 성공률 (Attack Success Rate, ASR)의 감소를 도모할 수 있습니다.

- **Performance Highlights**: 연구 결과에 따르면, PoTS는 훈련 데이터의 10%에서 발생하는 백도어 공격을 효과적으로 탐지할 수 있으며, 검증 단계는 훈련 단계보다 3배 빠르다는 장점을 가지고 있습니다. 또한, 모델의 레이어 수를 조절하여 탐지 신뢰도를 높일 수 있는 가능성도 제시합니다. 이러한 성과는 LLM 개발의 책임성과 보안을 향상시키는 데 중요한 기여를 할 것으로 기대됩니다.
...
### The Minimax Lower Bound of Kernel Stein Discrepancy Estimation (https://arxiv.org/abs/2510.15058)
- **What's New**: 이번 연구에서는 커널 스테인 불일치(Kernel Stein Discrepancy, KSD) 추정의 최적 경계가 n^{-1/2}임을 수립하였으며, 이는 기존 KSD 추정기의 최적성을 확인합니다. KSD 추정은 Langevin-Stein 연산자를 활용하여 고차원 공간에서도 유효함을 보여주며, 이로 인해 KSD 추정의 어려움이 차원에 따라 지수적으로 증가할 수 있음을 시사합니다. 또한 일반 도메인에서 KSD 추정의 최적성을 주목적으로 한 두 번째 결과도 제시하며, 상이한 증명 전략을 통해 도출되었습니다.

- **Technical Details**: KSD는 알려진 타겟 분포와 샘플링 분포 사이의 불일치를 정량화하기 위한 도구로, RKHS(Reproducing Kernel Hilbert Spaces)를 통해 확장 가능성이 높습니다. 이전의 연구들은 KSD 추정기들이 n의 제곱근 경향에 따라 수렴하는 것으로 알려져 있었으나, 본 연구는 최소값 하한선을 직관적으로 설정하였습니다. KSD의 의미론적 측면을 바탕으로 하는 것은 이론적인 가정을 기반으로 하기 때문에, 기존 연구들과 차별화된 요인으로 작용합니다.

- **Performance Highlights**: KSD는 다양한 데이터 과학 및 통계적 문제 해결에 널리 사용되며, 특히 모델 검증, 상호 작용 테스트 및 모델 설명 가능성과 같은 분야에서 효과적인 성과를 보이고 있습니다. 연구 결과는 KSD 추정의 성능을 더욱 구체적으로 설정할 수 있는 유용한 기준을 제공합니다. 이를 통해 KSD의 적용 범위를 더욱 넓힐 수 있는 가능성을 제시하고 있습니다.
...
### The Tree-SNE Tree Exists (https://arxiv.org/abs/2510.15014)
- **What's New**: 이번 연구는 MNIST 데이터셋과 같은 고차원 데이터의 클러스터링과 시각화의 문제를 강화하기 위해 t-SNE(tree-SNE) 알고리즘을 제안합니다. 기존의 t-SNE 방법은 데이터의 scale 문제를 간과하고 있으며, 이 연구는 t-SNE의 하향식 확장을 통해 계층적 클러스터 구조를 생성하는 새로운 접근법을 제공합니다. 이를 통해 데이터의 다양한 시각적 클러스터를 보다 효과적으로 탐색할 수 있는 가능성을 제시합니다.

- **Technical Details**: t-SNE는 비선형 차원 축소 방법으로, 클러스터를 형성하는 방식으로 데이터 구조를 시각화합니다. 본 논문에서 제안하는 tree-SNE는 t-SNE의 기본 아이디어를 확장하여 클러스터 내에서 더욱 세부적인 정보를 제공하는 계층적 구조를 생성합니다. 각 layer는 데이터 포인트의 매끄러운 궤적을 추적하며, 데이터 간의 유사성을 기반으로하여 재구성이 가능합니다.

- **Performance Highlights**: 위의 구조를 통해 tree-SNE는 고차원 데이터에서의 세밀한 클러스터링을 가능하게 하며, 특히 MNIST 데이터셋의 예를 통해 클러스터가 다양한 쓰기 스타일로 더욱 세분화됨을 보여줍니다. 각 클러스터의 평균 이미지를 사용하여 클러스터의 구성을 시각적으로 파악할 수 있으며, 이는 다양한 데이터셋에서 구조를 탐색하는데 있어 유용성을 극대화합니다.



### Reliable data clustering with Bayesian community detection (https://arxiv.org/abs/2510.15013)
- **What's New**: 이번 연구에서는 neuroscience, genomics, systems biology, 그리고 ecology와 같은 다양한 분야에서 신뢰할 수 있는 클러스터를 탐지하기 위해 Bayesian community detection 방법을 활용한 새로운 접근법을 제공합니다. 기존의 클러스터링 방법들은 노이즈에 취약하였으며, 데이터 처리 단계에서 발생하는 임의의 선택이 문제를 초래했습니다. 본 연구는 두 가지 Bayesian 방법인 Degree-Corrected Stochastic Block Model과 Regularized Map Equation을 사용하여 이러한 문제를 해결하고 신뢰성을 높였습니다.

- **Technical Details**: 연구자는 Minimum Description Length 원칙에 기반하여 모델 선택의 원칙화된 방법을 탐색합니다. 새로운 접근법은 클러스터링 및 희소화 과정을 통합하여, 임의의 임계값 설정 없이 직접적으로 클러스터를 탐지합니다. Pearson 상관관계 외에도 상관계수와 유사도 측정들을 포괄하는 방법으로, 주어진 데이터가 적거나 노이즈가 많을 때도 정확성을 높일 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법이 기존의 방법보다 더 정확하고 노이즈 저항성을 가지고 있으며, 특히 제한된 데이터 환경에서도 우수한 성능을 발휘함을 보여주었습니다. Regularized Map Equation은 Allen Human Brain Atlas에서 유전자 공발현 데이터에 대해 더욱 견고한 유전자 모듈을 발견했습니다. 이러한 결과는 고차원의 데이터를 다루는 다양한 분야에서 모듈 구조를 발견할 수 있는 강력한 프레임워크로서의 가능성을 제시합니다.
...
### Estimand framework and intercurrent events handling for clinical trials with time-to-event outcomes (https://arxiv.org/abs/2510.15000)
- **What's New**: 이 논문은 ICH E9(R1) 가이드라인의 한계를 보완하여 시간-사건(time-to-event, TTE) 결과에 대한 estimand를 정의하고 intercurrent events (ICEs) 의 처리를 논의합니다. 특히, 이 논문은 ICE 처리 전략으로 기존의 다섯 가지 방법 외에 경쟁위험(competing-risk) 전략을 제안합니다. 이로 인해 논문의 독창성은 세 가지 측면에서 두드러지며, 이는 잠재적 결과(potential outcomes) 측면에서 정의된 estimands, 시간 의존적 공변량(time-dependent covariates) 활용 방법, 그리고 효율적인 추정기(efficient estimators)에 대한 논의를 포함합니다.

- **Technical Details**: 본 논문에서는 TTE 결과를 다루기 위한 ICE 처리의 복잡성을 살펴보며 여러 측면에서 문제를 제기합니다. 예를 들어, 검열(censoring) 처리, 시간 의존적 공변량 활용, 그리고 근본 가정의 이탈에 대한 민감도 분석 수행 등입니다. 이를 해결하기 위해, 검열을 결측 데이터(missing data)의 특별한 경우로 간주하고, 다양한 검열 메커니즘을 분류하여 다룹니다. 또한, 시간 의존적 치료 변수(time-dependent treatment variable)의 데이터를 수집하는 방법도 포함합니다.

- **Performance Highlights**: 이 논문은 TTE 결과를 다루는 새로운 ICE 처리 전략을 제안함으로써 ICH E9(R1) 가이드라인의 부족한 부분을 보완할 수 있는 가능성을 제공합니다. 특히, 경쟁위험 전략의 도입은 다각적인 사건 처리에 기여하며, 이로 인해 연구 결과의 신뢰성 및 해석 가능성이 증가할 것으로 기대됩니다. TTE 연구에서 발생할 수 있는 다양한 사건을 구별하고 보다 효율적으로 대처하기 위한 방법론이 제시됩니다.
...
### Incentive-Based Federated Learning: Architectural Elements and Future Directions (https://arxiv.org/abs/2510.14208)
Comments:
          24 pages, 5 figures, chapter for edited book (Federated Learning: Foundations and Applications)

- **What's New**: 최근의 연구에서는 연합 학습(Federated Learning, FL) 시스템의 참여 난제(participation dilemma)를 해결하기 위한 인센티브 메커니즘 설계를 다루고 있습니다. 참여자들이 개인적인 이득을 추구하기 때문에 이러한 문제는 데이터를 공유하지 않고 협력적으로 모델 학습을 하는 데 큰 도전 과제가 됩니다. 이번 논문은 경제학과 게임 이론의 기본 개념을 연합 학습에 적용하여 이 문제를 해결할 수 있는 방법을 제시합니다.

- **Technical Details**: 연합 학습은 중앙 서버에 데이터를 공유하지 않고도 모델을 학습할 수 있도록 하는 새로운 방식입니다. 이 연구에서는 중앙 집중식 연합 학습(Centralized Federated Learning, CFL)과 분산형 연합 학습(Decentralized Federated Learning, DFL)의 구조를 분석하고, 인센티브 기반 연합 학습(Incentive-Based Federated Learning, IBFL) 아키텍처를 제안합니다. 이 아키텍처는 기여 평가, 인센티브 계산, 보상 분배 및 신뢰 추적 레이어로 구성되어 참여자들의 지속 가능한 참여를 보장하려고 합니다.

- **Performance Highlights**: 인센티브 메커니즘은 연합 학습 시스템의 성공적인 운영을 위한 필수 요소로 떠오르고 있으며, 데이터의 프라이버시를 보호하면서 참여자의 기여도를 평가할 수 있는 다양한 접근법을 통해 시스템의 효과성을 높이는 데 기여할 수 있습니다. 연구는 기존의 경제 모델과 게임 이론을 결합하여 합리적인 참여를 유도하고, 데이터 품질을 향상시키는데 필요한 다양한 전략을 제시합니다. 이러한 전략의 성공적인 도입은 의료, 금융, 스마트 시티 등의 현업 적용을 가능하게 하여, 연합 학습 생태계의 지속 가능성과 공정성을 높이는 데 중요한 역할을 할 것입니다.

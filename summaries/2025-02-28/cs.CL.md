...
### Machine Learning-Based Cloud Computing Compliance Process Automation (https://arxiv.org/abs/2502.16344)
- **What's New**: 본 논문은 클라우드 컴퓨팅의 준수(compliance) 관리에서 발생하는 많은 문제를 해결하기 위해 머신러닝 기반의 새로운 프레임워크를 제안합니다. 기존의 수작업 리뷰 프로세스는 점점 더 비효율적이므로, 자동화된 방법이 필요하다는 점을 강조합니다. 이 프레임워크는 GDPR 및 ISO 27001과 같은 진화하는 규정 요구사항을 충족하기 위해 설계되었습니다.

- **Technical Details**: 제안된 프레임워크는 다양한 머신러닝 기술을 통합하여 구성됩니다. BERT 기반 문서 처리(94.5% 정확도), One-Class SVM을 이용한 이상 탐지(anomaly detection)(88.7% 정확도), 시퀀셜 컴플라이언스 데이터 분석을 위한 개선된 CNN-LSTM 아키텍처(90.2% 정확도)를 포함합니다. 이러한 기술들은 단순히 수작업 리뷰를 대체하는 것을 넘어, 준수 프로세스를 자동화하는 데 기여합니다.

- **Performance Highlights**: 구현 결과는 준수 프로세스의 소요 시간을 7일에서 1.5일로 단축시키고, 정확도를 78%에서 93%로 향상시켰습니다. 이 프레임워크는 또한 수작업 노력을 73.3% 감소시켰습니다. 주요 증권사에서의 실시간 배포 결과, 일일 800,000건의 거래를 처리하며 리스크 식별에서 94.2%의 정확도를 보였습니다.

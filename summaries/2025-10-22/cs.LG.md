...
### A Hybrid Enumeration Framework for Optimal Counterfactual Generation in Post-Acute COVID-19 Heart Failur (https://arxiv.org/abs/2510.18841)
- **What's New**: 이 논문은 COVID-19 후유증인 PASC(포스트-급성 합병증 증후군)에 대한 개인별 리스크 추정 및 개입 분석을 위해 이론적 프레임워크인 counterfactual inference(반사실 추론)를 제시합니다. 기존 심장병(HF) 환자들을 대상으로 데이터 기반 접근법을 통해 PASC 관련 HF 입원의 행동 유도 경로를 탐색합니다. 이 새로운 프레임워크는 정규화된 예측 모델링과 반사실 검색을 통합하여 개별적인 예측 및 개입 효과를 이해할 수 있게 합니다.

- **Technical Details**: 정량적 분석에는 Mass General Brigham 의료 체계의 전자 건강 기록(EHR)을 활용한 연구가 포함되었습니다. PASC 케이스와 비 PASC 대조군을 비교하기 위해 8개의 병원에서 85,000명 이상의 환자 데이터를 분석했습니다. Temporal Learning with Dynamic Range(TLDR) 기술을 통해 EHR 타임라인에서 임상적으로 중요한 특징을 추출하고, 모델 훈련은 다섯 번의 교차 검증을 통해 이루어졌습니다.

- **Performance Highlights**: 모델은 2,700명 이상의 COVID-19 확진자에 대한 분석을 통해 0.88의 AUROC를 기록하며 강력한 구별 성능을 보였습니다. 개인 맞춤형 counterfactuals를 생성하여 동반 질병 패턴이나 치료 인자를 수정함으로써 예측 결과를 어떻게 변경할 수 있는지를 정량화하는데 성공했습니다. 이러한 접근 방식은 개인화된 추론과 복잡한 생물의학 시스템 내에서의 인과 이해를 발전시킬 수 있는 가능성을 보여줍니다.
...
### BO4Mob: Bayesian Optimization Benchmarks for High-Dimensional Urban Mobility Problem (https://arxiv.org/abs/2510.18824)
- **What's New**: 본 논문에서 제안하는 [1mBO4Mob[0m은 대규모 도시 도로망에서 출발지-목적지(OD) 여행 수요 추정의 도전 과제를 해결하기 위해 설계된 새로운 벤치마크 프레임워크입니다. 제한된 교통 센서 데이터로 OD 여행 수요를 추정하는 것은 어려운 역최적화 문제로, 이 프레임워크는 로드맵과 최적화 방법의 진전을 지원합니다. 이 벤치마크는 실제 샌호세(San Jose) 도로망을 기반으로 한 5가지 시나리오를 포괄하며, 입력 차원은 최대 10,100으로 확장됩니다.

- **Technical Details**: BO4Mob은 비선형 및 확률적 동역학을 통합하는 고해상도 오픈소스 교통 시뮬레이션을 활용하여 현실적인 교통 시나리오를 구현합니다. 이는 시뮬레이터가 깊이 있는 수요 및 공급 행동을 자세히 설명하여 복잡한 시스템을 효과적으로 다룰 수 있도록 돕습니다. 최적화 작업은 종종 계산 비용이 많이 드는 블랙박스 최적화로 분류되며, 이 문제를 해결하기 위해 샘플 효율적인 베이지안 최적화(Bayesian Optimization) 기술이 도입되었습니다.

- **Performance Highlights**: 논문은 세 가지 최첨단 BO 알고리즘과 두 개의 비-BO 기준을 평가하여 BO4Mob의 유용성을 실증합니다. 이 벤치마크는 도시 이동성 모델 설계에 적용될 수 있는 확장 가능한 최적화 알고리즘의 개발을 지원합니다. 코드와 문서화는 공개되어 reproducibility(재현 가능성) 및 확장성을 촉진하는 데 기여합니다.



### Search Self-play: Pushing the Frontier of Agent Capability without Supervision (https://arxiv.org/abs/2510.18821)
- **What's New**: 본 논문에서는 강화 학습과 검증 가능한 보상(Reinforcement Learning with Verifiable Rewards, RLVR)의 결합을 통해 LLM(대규모 언어 모델) 에이전트를 훈련하는 새로운 접근법을 제시합니다. 기존의 RLVR 방법은 지식 기반 구축과 인간의 수작업 노력을 요구했지만, Self-Play Training(자기 대국 훈련) 방법을 통해 이 문제를 해결하고자 합니다. 제안된 접근법에서는 LLM이 질문 제안자와 문제 해결자로 동시에 활동하여 훈련 과제를 스스로 생성하고 해결하는 방식으로 반복 진행됩니다.

- **Technical Details**: 연구에서는 Search Self-play(SSP) 게임을 설계하여, LLM이 두 가지 역할을 겸하며 진화합니다. 질문 제안자는 검증 가능한 답변과 점진적인 난이도를 갖춘 깊은 검색 쿼리를 생성하며, 문제 해결자는 생성된 질문에 대해 다단계 추론을 통해 답변을 시도합니다. 생성된 모든 쿼리의 정확성을 검증하기 위해 검색 결과를 외부 자료로 수집하고, Retrieval-Augmented Generation (RAG)을 사용하여 답변 예측이 가능한지를 확인합니다.

- **Performance Highlights**: 실험 결과, 제안된 SSP 모델은 다양한 벤치마크에서 일관되게 성능을 개선하는 것으로 나타났습니다. 또한, SSP는 기존의 감독 없이도 LLM의 검색, 추론 및 자기 검증 능력을 체계적으로 향상시킵니다. 이 연구는 자율적으로 고품질 훈련 과제를 생성하고 해결할 수 있는 훈련 방법론을 제시하여, LLM 에이전트 훈련의 확장 가능성을 높였습니다.
...
### A Unified Perspective on Optimization in Machine Learning and Neuroscience: From Gradient Descent to Neural Adaptation (https://arxiv.org/abs/2510.18812)
- **What's New**: 이번 리뷰 논문은 반복 최적화(iterative optimization) 방법이 AI 및 적응 시스템을 이해하는 중요한 프레임워크로서, 고전 이론(classic theory)과 신경망(neural networks) 훈련, 생물학적 학습을 연계하고 있습니다. 전통적인 경량화된 기법인 제로 오더(zeroth-order, ZO) 최적화는 함수 평가와 무작위성에만 의존하며, 생물학적 학습에 특히 적합하다는 점이 강조됩니다. 이 논문은 다양한 최적화 접근법을 분류하고, 신경망 훈련의 독특한 도전에 어떻게 적응하는지를 탐구합니다.

- **Technical Details**: 최적화는 복잡한 시스템의 효과적인 구성(configurations)을 찾는 데 필수적이며, 이는 목적 함수(objective function)를 최소화하는 것과 관련이 있습니다. 경량화된 제로 오더 최적화 방법은 미분 가능한 구성 요소가 아닌 경우와 블랙박스 시스템을 최적화할 때 유용하며, 이러한 방법들은 샘플 효율(sample efficiency)은 낮지만 현대 ZO 기술이 BP와 경쟁할 수 있는 성능을 달성할 수 있음을 보여줍니다. 이 논문은 최적화 방법을 도함수 정보를 사용하는 방법과 사용하지 않는 방법으로 나누어 논의합니다.

- **Performance Highlights**: 이 논문에서는 ZO 최적화 패러다임이 생물학적 학습의 기초 원칙과 연결되고, 뇌의 내재적인 노이즈를 계산 자원으로 활용하는 방식을 제안합니다. 이러한 접근은 자연 지능(natural intelligence)의 이해를 높이고, 에너지 효율적이고 빠른 AI 시스템 설계에 큰 영향을 미칠 수 있습니다. 최적화 접근법의 서로 다른 장점과 제한점을 논의하며, 생물학적 학습과 인공지능에 대한 통합적 통찰력을 제공합니다.



### When LRP Diverges from Leave-One-Out in Transformers (https://arxiv.org/abs/2510.18810)
Comments:
          BlackboxNLP @ EMNLP 2025

- **What's New**: 이 논문에서는 현대 Transformer에서 Layer-Wise Relevance Propagation (LRP)의 공리적 타당성이 충분히 검토되지 않았음을 지적합니다. 특히, 최근의 AttnLRP에서 사용되는 이항 전파 규칙이 구현 불변성 원칙을 위반한다는 것을 증명합니다. 또한, CP-LRP의 중요성을 재조명하고 소프트맥스 계층을 우회하여 관련성을 전파함으로써 Leave-One-Out (LOO)와의 정렬을 개선할 수 있음을 발견했습니다.

- **Technical Details**: 기존 연구들은 LRP가 일반적으로 구현 불변성 원리를 위반한다고 주장했으며, 최근의 CP-LRP 및 AttnLRP에서도 이 결함이 여전히 존재하는지 확인되지 않았습니다. 본 연구는 AttnLRP의 이항 계층 전파 규칙이 구현 불변성 원칙을 위반한다는 것을 정량적으로 입증합니다. 우리는 LRP의 적절한 적용을 위해 소프트맥스 계층을 우회하고 주목(weights)을 상수로 처리하는 접근 방식을 제안합니다.

- **Performance Highlights**: 실험 결과, CP-LRP가 AttnLRP보다 LOO와 더 잘 일치한다는 것을 보여주었습니다. 중후반 계층에서는 CP-LRP 방법이 LOO 정렬을 개선하는 데 더 효율적이라는 사실을 발견했습니다. 이러한 결과는 현대 Transformer 구조에서 LRP의 적용이 어떻게 최적화될 수 있는지를 시사합니다.



### On Biologically Plausible Learning in Continuous Tim (https://arxiv.org/abs/2510.18808)
- **What's New**: 이 연구에서는 생물학적으로 그럴듯한 학습 알고리즘을 통합한 연속 시간 신경망 모델을 제안합니다. 기존의 알고리즘 모델들이 이산형 업데이트와 학습 및 추론 단계의 분리에 의존하는 반면, 제안하는 모델은 이러한 단계를 통합하여 생물학적 시간 척도에서 신경망이 안정적으로 학습할 수 있도록 합니다. 이 모델은 입력 신호와 오류 신호의 시간적 중첩이 학습의 주된 요인임을 강조합니다.

- **Technical Details**: 모델은 연속 시간의 동적 시스템으로, 뉴런의 상태와 시냅스 가중치가 coupled first-order differential equations에 따라 진화합니다. 각 뉴런은 feedforward drive와 오류 drive를 받아들이고, 지역적인 두 신호 규칙에 따라 시냅스를 업데이트합니다. 학습과 추론은 동시에 발생하여 전역 클락이나 단계 분리가 필요하지 않으며, 이는 실제 생물학적 시스템에서의 시간 초과에 대한 연구를 가능하게 합니다.

- **Performance Highlights**: 시뮬레이션 결과, 이 모델은 생물학적인 시간 척도에서 효과적으로 학습할 수 있으며, 시냅스 가소성의 시간 척도가 자극 지속 시간의 1~2 배를 초과해야 한다는 정량적 제약을 도출합니다. 이는 수초 범위의 기능적 가소성 창을 설정하며, 이러한 조건들이 생물학적 회로에서 오류 기반 학습의 필수적인 요소임을 시사합니다. 결과적으로, KP, FA, DFA와 같은 알고리즘이 연속 시간에서 생물학적으로 실현 가능하다는 것을 보여줍니다.



### Stick-Breaking Embedded Topic Model with Continuous Optimal Transport for Online Analysis of Document Streams (https://arxiv.org/abs/2510.18786)
Comments:
          Under review

- **What's New**: 이번 논문은 온라인 토픽 모델링의 새로운 접근법인 SB-SETM을 소개합니다. 기존의 Embedded Topic Model (ETM)을 확장하여 데이터 스트림을 처리하는 혁신적인 모델로, 문서 배치의 연속적인 부분을 결합하여 작동합니다. SB-SETM은 문서당 주제 분포를 위한 절단된 스틱-브레이킹 구조를 활용해 각 시점에서 데이터에서 활성 주제의 수를 자동으로 추정합니다.

- **Technical Details**: SB-SETM은 두 가지 주요 기술적 기법을 도입합니다. 첫 번째는 데이터에 적응 가능한 주제 활성화를 가능하게 하는 문서 수준의 절단된 스틱-브레이킹 구조입니다. 두 번째는 고차원 잠재 주제 공간에 적합화된 최적 운송의 연속적 형태를 활용하여 주제 임베딩을 통합하는 병합 전략입니다.

- **Performance Highlights**: 실험 결과 SB-SETM은 시뮬레이션된 시나리오에서 기존 방식을 능가하는 성능을 보여주었습니다. 2022-2023년 사이 러시아-우크라이나 전쟁에 대한 뉴스 기사를 대상으로 한 실제 데이터셋에서도 주제 빈도 피크가 주요 사건과 일치함을 나타냈습니다.



### CAGE: Curvature-Aware Gradient Estimation For Accurate Quantization-Aware Training (https://arxiv.org/abs/2510.18784)
- **What's New**: 이번 논문에서는 저비트 양자화 인지 학습(Quantization-Aware Training, QAT)의 정확성을 크게 향상시킬 수 있는 새로운 방법인 CAGE(Curvature-Aware Gradient Estimation)를 소개합니다. CAGE는 양자화로 인한 손실 증가를 상쇄하기 위해 곡률 인지 보정을 사용하여 기존의 곧바로 통과하는 추정기(Straight-Through Estimator, STE) 기법을 보강합니다. 이 방법은 양자화 제약을 감안한 다목적 최적화 관점에서 유도된 것으로, 손실 최소화와 양자화 오류를 동시에 고려합니다.

- **Technical Details**: CAGE는 양자화된 최적 모델을 향해 파라미터를 업데이트하기 위한 경량 기법으로, 기존의 STE 기법에 곡률 인지 보정 항을 추가하여 손실 증가를 보다 효과적으로 관리합니다. 이 방법은 손실 표면의 지역 곡률 정보를 활용하여 이론적으로 강력한 수렴 보장을 제공하며, 실제 구현에서는 Adam과 같은 적응형 최적화 방법의 통계를 활용할 수 있다. CAGE는 기존의 QAT 기법들이 가지는 비제어 가능성과 느린 수렴 속도를 개선하였습니다.

- **Performance Highlights**: CAGE는 Llama 스타일의 800M 파라미터 모델의 사전 학습 실험을 통해 W4A4(4비트 가중치 및 4비트 활성화) 설정에서 양자화로 인한 손실 증가의 10% 이상을 회복하는 성과를 보였습니다. 이는 현대의 이상치 완화 기법과 비교하여 의미 있는 개선을 나타냅니다. 우리의 결과는 곡률 인지 경량 보정이 저비트 양자화 모델과 전체 정밀도 모델 간의 성능 격차를 줄이는 중요한 단계임을 보여줍니다.



### Enhancing Fractional Gradient Descent with Learned Optimizers (https://arxiv.org/abs/2510.18783)
- **What's New**: 이번 연구에서는 Learning to Optimize Caputo Fractional Gradient Descent (L2O-CFGD)라는 혁신적인 방법을 제안하여, Caputo FGD의 하이퍼파라미터를 동적으로 조정하는 메타 학습 접근법을 통합했습니다. 이를 통해 고정된 하이퍼파라미터를 사용하는 전통적인 CFGD보다 최적화 성능을 개선하며, 일부 작업에서는 완전 블랙박스 메타 학습 최적화기와 유사한 성능을 달성하고 있습니다.

- **Technical Details**: L2O-CFGD는 Caputo 미분의 독특한 성질을 활용하여 비볼록(neural network training과 같은)한 최적화 문제에서 효율적으로 하이퍼파라미터를 조정합니다. Caputo 미분은 고전적인 미분의 일반화로, 비국소적(nonlocal) 성질을 허용하기 때문에 전통적인 최소점을 찾는 데 어려움을 겪습니다. 이 연구에서는 Caputo 미분의 이론적 배경과 함께 최적화 알고리즘에 대한 업데이트 규칙을 제공합니다.

- **Performance Highlights**: L2O-CFGD는 다양한 볼록(convex) 및 비볼록(non-convex) 작업에서 기존 CFGD보다 더 빠른 최적화 성능을 보여주며, 예측 가능한 행동을 나타냅니다. 또한 하이퍼파라미터의 역사적 의존성을 활용하여 연구자들이 고성능 하이퍼파라미터를 식별하고 이론적 이해를 깊게 하는 데 도움을 주는 강력한 도구가 될 수 있습니다. 이러한 결과들은 L2O-CFGD가 분수 미분을 기반으로 한 최적화 방법과 학습 기반 방법 간의 시너지를 처음으로 시도한 것임을 보여줍니다.



### Improving the Generation and Evaluation of Synthetic Data for Downstream Medical Causal Inferenc (https://arxiv.org/abs/2510.18768)
- **What's New**: 이 논문은 의료 개입의 개발 및 평가를 위한 인과 추론에 초점을 맞추고 있습니다. 실제 의료 데이터에 대한 접근이 규제 장벽으로 인해 어려운 상황에서, 합성 데이터(synthetic data)의 중요성이 부각되고 있습니다. 연구자들은 기존의 생성 모델이 치료(treatment)에 집중한 하류 인과 추론(causal inference) 과제가 가지는 특정한 문제를 고려하지 않음을 지적하며, 이를 해결하기 위한 새로운 평가 지표와 생성 방법을 제시합니다.

- **Technical Details**: 저자들은 치료를 포함한 합성 데이터가 충족해야 하는 조건(desiderata) 세 가지를 정의합니다: (i) 공변량 분포(covariate distribution)의 보존, (ii) 치료 할당 메커니즘(treatment assignment mechanism)의 유지, (iii) 결과 생성 메커니즘(outcome generation mechanism)의 최적화입니다. STEAM(Synthetic data for Treatment Effect Analysis in Medicine)은 이러한 조건을 최적화하면서 치료가 포함된 데이터 생성 과정을 모사하는 새로운 합성 데이터 생성 방법입니다.

- **Performance Highlights**: 적험 결과에 따르면, STEAM은 기존의 생성 모델과 비교할 때 모든 평가 지표에서 최첨단 성능을 달성했습니다. 특히 실제 데이터 생성 프로세스(data-generating process)의 복잡성이 증가함에 따라 그 효과가 두드러지며, 고차원 시나리오에서도 뛰어난 결과를 보였습니다. 이를 통해 연구자들은 의학적 분석에서의 합성 데이터의 사용 가능성을 크게 확장할 수 있습니다.
...
### OmniCast: A Masked Latent Diffusion Model for Weather Forecasting Across Time Scales (https://arxiv.org/abs/2510.18707)
Comments:
          Neural Information Processing Systems (NeurIPS 2025)

- **What's New**: 이번 연구에서는 OmniCast라는 새로운 확률적 모델이 제안되었습니다. OmniCast는 모든 시간 스케일에서의 기상 예측을 통합하는 스케일 가능한 (scalable) 모델로, VAE(Variational Autoencoder)와 확산 기반의 Transformer 모델을 포함하고 있습니다. 이를 통해 기존 방법들의 제한 사항을 극복하고, 특히 중기 예측 및 S2S(subseasonal-to-seasonal) 타임스케일에서의 성능을 개선하였습니다.

- **Technical Details**: OmniCast는 두 단계의 학습 과정을 따릅니다. 첫 번째 단계에서는 원시 기상 데이터를 연속적이고 저차원(latent space)으로 압축하는 VAE 모델을 학습하며, 두 번째 단계에서는 조건부 토큰을 기반으로 미래의 잠재 토큰 시퀀스의 분포를 모델링하는 Transformer를 학습합니다. 이 과정에서 무작위로 미래 토큰을 마스킹(masking)하여 Transformer가 이를 추정하도록 훈련하며, 최종적으로는 반복적인 프로세스를 통해 전체 미래 토큰 시퀀스를 생성합니다.

- **Performance Highlights**: OmniCast는 ChaosBench 벤치마크에서 S2S 예측의 주요 기상 변수에 대해 최첨단 성능을 기록하였습니다. 이 모델은 기존의 확률적 방법들에 비해 10배에서 20배 빠른 속도로 예측을 수행할 수 있으며, 정확도, 물리 기반 및 확률적 지표에서 우수한 성능을 보였습니다. 또한, OmniCast는 최대 100년까지 안정적인 예측 롤아웃(rollouts)을 생성할 수 있음을 보여주었습니다.



### Reinforcement Learning with Imperfect Transition Predictions: A Bellman-Jensen Approach (https://arxiv.org/abs/2510.18687)
- **What's New**: 이 논문은 전통적인 강화 학습(RL)에서 벗어나 다단계 예측(Multi-step predictions)을 통한 의사결정의 이점을 다룹니다. 기존의 MDP(Markov decision process) 기반 모델에서는 단일 단계 전이 모델만을 사용하였으나, 실제 적용에서는 여러 단계의 예측이 필요합니다. 저자들은 이러한 예측을 효과적으로 처리하기 위한 새로운 기법 세 가지를 제안합니다.

- **Technical Details**: 우선, Bayesian value function를 제안하여 예측 인식 정책(예측을 반영한 의사 결정을 효율적으로 나타내는 방법)을 정의합니다. 또한, Bellman-Jensen Gap 분석을 통해 불완전한 예측의 가치를 나타낼 수 있도록 하였습니다. 마지막으로, BOLA(Bayesian Offline Learning with Online Adaptation)라는 두 단계 모델 기반 RL 알고리즘을 도입하여 오프라인 Bayesian 가치 학습과 실시간 예측에 대한 경량화된 온라인 적응을 분리합니다.

- **Performance Highlights**: 저자들은 BOLA가 불완전한 예측 하에서도 샘플 효율성을 유지한다는 것을 증명하였습니다. 이를 통해 이론과 알고리즘을 합성 MDP와 실제 풍력 에너지 저장 제어 문제에 대해 검증하였습니다. 이러한 접근 방식은 다차원 예측을 활용하여 의사결정의 질을 높일 수 있는 가능성을 보여줍니다.



### Learning Task-Agnostic Representations through Multi-Teacher Distillation (https://arxiv.org/abs/2510.18680)
Comments:
          NeurIPS-2025

- **What's New**: 본 논문에서는 다양한 입력에 대해 효과적인 표현을 생성하기 위한 무관한 작업을 기반으로 한 새로운 프레임워크를 제안합니다. 이전의 multi-teacher distillation 방식은 특정 작업에 맞춰져 있었던 반면, 본 방법론은 'majority vote' 기반의 목적 함수를 사용하여 작업에 구애받지 않는 distillation loss를 생성합니다. 이러한 접근 방식은 특정 레이블이나 선행 지식에 대한 의존도를 줄이는 효과가 있습니다.

- **Technical Details**: 이 연구에서는 다양한 embedding 모델과 그에 따른 다양한 embedding 공간에 대해 주목합니다. 10-layer GINE 신경망을 사용하여 분자 모델링에서 더 나은 성능을 나타내는 과정을 설명하고 있습니다. teacher-specific heads를 활용하여 각 teacher의 지식을 학생 모델에 전달하도록 설계하였으며, 다양한 깊이의 kernels을 사용하여 그 성능을 평가하였습니다.

- **Performance Highlights**: 실험 결과 GINE 아키텍처가 다른 아키텍처들보다 더 낮은 훈련 손실과 빠른 수렴 속도를 보여주었습니다. 특히 작은 데이터셋(250k)에서 교수의 다양성을 활용하여 더 유용한 표현을 학습할 수 있음을 보여줍니다. 결과적으로 regression 작업에서 student-250k 모델이 student-2M 모델보다 우수한 성능을 보였습니다.
...
### Learning Time-Varying Turn-Taking Behavior in Group Conversations (https://arxiv.org/abs/2510.18649)
- **What's New**: 우리는 개별 특성과 과거 발언 행동을 기반으로 그룹 대화에서 턴 테이킹(turn-taking) 패턴을 예측하기 위한 유연한 확률적 모델을 제안합니다. 많은 기존 대화 역학 모델들은 단일 그룹을 넘어서는 통찰력을 제공하지 않으며, 보편적인 방식으로 발언 행동을 설명하려는 시도는 모든 그룹에 적합하지 않을 수 있습니다. 이러한 문제를 해결하기 위해, 우리는 개별 특성과 과거 발언 행동을 바탕으로 모든 그룹에서 개인 간의 발언 턴을 예측하는 모델을 발전시켰습니다.

- **Technical Details**: 이 모델은 개인의 성격 특성(personality traits)과 과거 발언 행동을 기반으로 하여, 각 개인이 마지막으로 발언한 시점에 따라 발언 경향이 어떻게 변화하는지를 학습할 수 있는 새로운 능력을 제공합니다. 우리는 이 모델을 합성(synthetic) 및 실제(real-world) 대화 데이터에 적용하여 제안된 접근 방식을 검증하고 실제 그룹 상호작용을 특성화하였습니다. 이를 통해 모델의 유연성과 적용 가능성이 입증되었습니다.

- **Performance Highlights**: 우리의 결과는 기존 행동 모델들이 항상 현실적이지 않을 수 있음을 보여주며, 데이터 기반(data-driven)이며 이론적으로 근거가 있는 접근 방식을 강화하는 동기를 제공합니다. 이 연구는 그룹 대화에서 개인의 발언 패턴 예측이 어떻게 이뤄질 수 있는지를 제시하며, 향후 그룹 대화의 구조와 역학을 이해하는 데 기여할 것으로 기대됩니다.



### Informed Learning for Estimating Drought Stress at Fine-Scale Resolution Enables Accurate Yield Prediction (https://arxiv.org/abs/2510.18648)
- **What's New**: 이 연구는 농업 생산성과 식량 안보를 위한 중요한 요소로서 물 부족과 수확량 감소 잠재력을 평가합니다. 신뢰할 수 있는 모델을 개발하기 위해, 물 가용성에 따라 수확량을 정의하고 물 부족에 대한 민감성과 가뭄 스트레스를 예측합니다. 본 연구는 물리적 원리를 준수하면서도 성능이 뛰어난 기계 학습 모델과 해석 가능한 시뮬레이션 모델의 장점을 결합합니다.

- **Technical Details**: 작물 수확량은 수력 부족에 대한 함수로서 공식화되며, 이는 시간적 물 부족과 실제 에바포트랜스피레이션(actual evapotranspiration)과 연결됩니다. 새로운 물리 기반 손실 함수가 제안되어 모델의 물리적 일관성을 강화합니다. 이 모델은 다중분광 위성 이미지(multispectral satellite imagery), 기상 데이터(meteorological data) 및 고해상도 수확량 데이터(yield data)를 활용하여 구현됩니다.

- **Performance Highlights**: 이 연구의 방법론은 LSTM 및 Transformers와 같은 최신 모델을 능가하며, 결정계수($R^2$-score)가 최대 0.82에 달하여 높은 예측 정확성을 보여줍니다. 이는 정책 입안자 및 농민에게 변화하는 기후 조건에서 보다 탄력적인 농업을 구축하기 위한 결정 지원을 제공합니다.



### Optimality and NP-Hardness of Transformers in Learning Markovian Dynamical Functions (https://arxiv.org/abs/2510.18638)
Comments:
          NeurIPS 2025

- **What's New**: 이번 논문에서는 Transformer 아키텍처가 동적 함수 모델링에서 어떻게 In-Context Learning (ICL)을 표현하는지를 연구했습니다. 특히 Markovian 동적 함수를 통한 학습을 다루며, 단일 레이어 선형 자기 주의 모델에 대한 글로벌 최소화 해를 제공합니다. 저자들은 또한 단일 레이어 모델에서 구조적 동적 함수를 나타내는 데 있어 NP-hard 문제를 증명하며, 이는 해당 모델의 한계를 보여줍니다.

- **Technical Details**: 저자들은 Markovian 동적 함수에 대한 ICL을 이해하기 위한 분석 프레임워크를 설정했습니다. 이 연구는 길이-2 체인에서의 손실 경관의 글로벌 최소를 특징화하였고, 이를 통해 Markovian 동역학에 최적 솔루션이 어떻게 적응하는지를 밝혔습니다. 또한, 다층 선형 변환기의 역량을 깊이 있는 최적화 문제로 설명하며, 제곱 손실을 최소화하면서 다수의 목표들을 동시에 최적화하는 과정을 탐구했습니다.

- **Performance Highlights**: 이 연구는 단일 레이어 LSA 모델이 Markovian 동적 함수를 학습하는 데 있어 나타나는 구조적 제약을 강조하고 있습니다. 수치적으로 검증된 결과는 다층 변환기가 어떻게 다수의 객관적인 목표를 최적화할 수 있는지를 보여줍니다. 이러한 발견은 ICL에 대한 이론적 기반을 확장시키고, transformer의 표현력에 대한 새로운 통찰력을 제공합니다.



### Hardness of Learning Regular Languages in the Next Symbol Prediction Setting (https://arxiv.org/abs/2510.18634)
Comments:
          7 pages

- **What's New**: 본 논문에서는 Next Symbol Prediction (NSP) 설정에서 언어의 학습 가능성을 연구합니다. 학습자는 목표 언어의 긍정적인 예제와 각 접두사에 대한 풍부한 감독 정보를 받습니다. 이 연구는 기존의 neural sequence 모델을 분석하는 데 사용되었던 NSP 접근법의 새롭고 중요한 측면을 조명합니다.

- **Technical Details**: 우리는 DFA(Deterministic Finite Automaton)와 불리언 공식(Boolean formulas)과 같은 개념 클래스의 학습 가능성을 분석하고, NSP 감독이 정규 언어 학습의 주요 계산 장벽을 제거하지 않음을 증명합니다. 논문에서는 NSP 설정에서도 학습이 계산적으로 어렵다는 것을 입증하며, 이는 알고리즘의 하드니스(hardness) 결과에 대한 축소를 통해 이루어집니다.

- **Performance Highlights**: 본 연구 결과는 NSP 라벨이 일부 이점을 제공할 수 있지만, 전반적으로 계산적 어려움을 회피하지 못한다는 것을 보여줍니다. NSP 설정에서의 폴리노미얼 시간 학습이 불가능하다는 점을 강조하며, 이는 이 분야의 기존 연구에 중요한 영향을 미칠 것입니다.
...
### Unrolled-SINDy: A Stable Explicit Method for Non linear PDE Discovery from Sparsely Sampled Data (https://arxiv.org/abs/2510.18611)
Comments:
          56 pages, 12 figures, 39 tables

- **What's New**: 이 논문은 데이터 샘플링이 드문 실제 문제들을 다루기 위해 Unrolled-SINDy라는 새로운 방법론을 소개합니다. 이 방법은 시간 단계 크기를 샘플링 속도와 분리하여 PDE 발견의 안정성을 높이는 데 기여합니다. 이를 통해 원래의 SINDy 최적화 문제에서의 대규모 국소 절단 오류 때문에 미니마이저로 나타나지 않는 방정식 파라미터를 회수할 수 있습니다.

- **Technical Details**: Unrolled-SINDy는 K번의 수치적 단계를 언롤링하는 방식으로, 두 가지 관측치 사이의 솔루션에 대한 반복적 추정치를 평가하여 라이브러리 항을 계산합니다. 이 방법은 작은 시간 단계의 장점을 활용하여 국소 절단 오류를 낮춥니다. 기존의 SINDy 접근 방식과는 달리, Unrolled-SINDy는 명시적 수치 방법에 기반하여 방정식을 폐쇄형으로 식별할 수 있는 장점을 지닙니다.

- **Performance Highlights**: 실험 결과, Unrolled-SINDy는 전통적인 SINDy와 최신 노이즈 강건 iNeural-SINDy에 비해 더욱 강력하게 물리적 법칙을 회복할 수 있음을 보여주었습니다. 큰 시간 단계에서도 문제를 해결할 수 있는 능력을 입증하며, 이는 현재의 방법들이 접근할 수 없는 까다로운 문제들입니다. 또한 Unrolled-SINDy는 신경망 기반의 PDE 발견 방법에도 적용될 수 있어 널리 활용될 수 있는 가능성을 지니고 있습니다.



### Robustness Verification of Graph Neural Networks Via Lightweight Satisfiability Testing (https://arxiv.org/abs/2510.18591)
- **What's New**: 이 논문은 그래프 신경망(Graph Neural Networks, GNNs)의 적대적 공격(adversarial attacks)에 대한 강건성(robustness) 검증을 개선하는 새로운 접근 방식을 제안합니다. 기존의 강력한 솔버(solvers)를 사용한 방법 대신 효율적인 부분 솔버(partial solvers)를 활용하여 구조적 강건성을 높일 수 있음을 보여줍니다. 이는 GNN의 적대적 견고성 문제를 해결하기 위해 대칭적 제약을 형성하는 기존 접근 방식에 대한 혁신적인 대안으로 작용할 수 있습니다.

- **Technical Details**: 연구에서는 방향 그래프(directed graphs)와 비정상 그래프(incomplete graphs) 개념을 사용하여 GNN의 성능을 분석합니다. 비정상 그래프는 정상 에지(normal edges), 미지의 에지(unknown edges), 비에지(non-edges)로 구성된 그래프를 정의하며, 그래프의 이웃(neighbors) 및 그래프 구조에 대한 작은 변화를 분석하는 데 초점을 맞춥니다. 새로운 부분 오라클(partial oracles)을 도입하여 제약 조건(constraints)의 만족 가능성을 효율적으로 판단하며, 이는 기존의 일반-purpose 솔버보다 성능이 높습니다.

- **Performance Highlights**: 제안된 도구 RobLight는 다양한 GNN 변형 및 데이터 세트에서 평가되었으며, 기존의 최첨단 기법에 비해 상당한 성능 개선을 보여주었습니다. 연구 결과, RobLight는 적대적 공격 상황에서도 그래프의 강건성을 훨씬 더 효율적으로 검증할 수 있음을 입증하였습니다. 이는 그래프 학습 문제의 Robustness Verification 연구에 중요한 기여를 할 것으로 기대됩니다.



### HeFS: Helper-Enhanced Feature Selection via Pareto-Optimized Genetic Search (https://arxiv.org/abs/2510.18575)
- **What's New**: 본 논문에서는 NP-hard 문제인 Feature selection(특징 선택)에서 새로운 방법론인 HeFS(Helper-Enhanced Feature Selection) 프레임워크를 소개합니다. 기존의 휴리스틱(heuristic) 또는 탐욕적(greedy) 전략의 한계를 극복하기 위해 이 방법론을 개발하였습니다. HeFS는 잔여 특징 공간에서 Helper Set(보조 집합)을 찾아내어 원래의 특징 집합을 보완합니다.

- **Technical Details**: HeFS는 유전 알고리즘(genetic algorithm)의 편향 초기화(biased initialization) 방식과 비율 유도 변이(ratio-guided mutation) 메커니즘을 활용합니다. 이와 함께 Pareto 기반 다중 목표 최적화(multi-objective optimization)를 적용하여 예측 정확도와 특징 보완성을 동시에 극대화합니다. 이러한 기술적 접근은 복잡한 상호 의존적 특징 관계를 효과적으로 파악할 수 있도록 합니다.

- **Performance Highlights**: 18개의 벤치마크 데이터셋에서 실시된 실험 결과, HeFS는 놓치기 쉬운 유용한 특징들을 지속적으로 파악하며 최첨단 방법들보다 우수한 성능을 기록하였습니다. 특히 위암 분류(gastric cancer classification), 약물 독성 예측(drug toxicity prediction), 컴퓨터 과학 애플리케이션 등 어려운 분야에서도 높은 성과를 보였습니다. 해당 코드와 데이터셋은 제공된 URL에서 확인할 수 있습니다.
...
### Partial VOROS: A Cost-aware Performance Metric for Binary Classifiers with Precision and Capacity Constraints (https://arxiv.org/abs/2510.18520)
- **What's New**: 이번 연구에서는 전통적인 ROC 곡선 분석의 한계를 극복하기 위한 새로운 지표인 Partial VOROS를 제안합니다. 이 연구는 병원에서의 및 위험 감지 알림 시스템에서 기준을 충족해야 하는 분류기들이 어떻게 ROC 공간에서의 유효 지역으로 표현될 수 있는지 보여줍니다. 또한, 실질적인 오류 비용이 비대칭적일 때 이를 고려한 평가 방식을 개발했습니다.

- **Technical Details**: 연구에서는 비용 인식(cost-aware) 분석을 통해, 병원 상황에 맞는 정밀도(precision) 및 용량(capacity) 제약을 조정합니다. 이를 기반으로 Partial VOROS라는 새로운 메트릭을 도입하여 다양한 분류기의 비용 및 성능 제약을 효율적으로 평가하도록 합니다. ROC 공간의 유효 지역을 정의하고, 해당 영역에서의 성능 지표를 활용한 구체적인 수식도 제공합니다.

- **Performance Highlights**: MIMIC-IV 데이터셋을 통한 실험에서 Partial VOROS가 병원 경고 시스템에서 분류기를 순위 매기는 데 다른 대안보다 우수함을 입증했습니다. 이 방법은 특히 높은 오류 비용이 발생하는 상황에서 실질적으로 중요한 성능 평가를 가능하게 하여, 의료 진단의 정확성을 높이는 데 기여할 것으로 예상됩니다.



### Alibaba International E-commerce Product Search Competition DILAB Team Technical Repor (https://arxiv.org/abs/2510.18499)
Comments:
          CIKM Alibaba E-commerce Search Challenge 2025

- **What's New**: 이번 연구에서는 DILAB 팀이 개발한 다국어 전자상거래 검색 시스템을 소개합니다. 이 시스템은 최종 리더보드에서 5위에 올라, 전체 점수 0.8819를 기록하며 안정적이고 높은 성능을 보여주었습니다. 다양한 평가 지표에서 우수한 결과를 입증하였습니다.

- **Technical Details**: 다국어 쿼리-아이템 이해를 위한 문제를 해결하기 위해, 데이터 정제(data refinement), 경량 전처리(lightweight preprocessing), 적응형 모델링(adaptive modeling)을 통합한 다단계 파이프라인을 설계했습니다. 데이터 정제 단계에서는 데이터셋의 일관성과 카테고리 커버리지를 향상시켰고, 언어 태깅(language tagging) 및 노이즈 필터링(noise filtering)을 통해 입력 질을 개선했습니다.

- **Performance Highlights**: 모델링 단계에서는 여러 아키텍처와 미세 조정(fine-tuning) 전략을 탐색하였으며, 하이퍼파라미터를 최적화하여 쿼리-카테고리(QC)와 쿼리-아이템(QI) 작업 간 성능 균형을 맞췄습니다. 제안된 프레임워크는 언어와 도메인에 걸쳐 강력성과 적응성을 보여주며, 다국어 검색 시스템을 위한 체계적인 데이터 큐레이션(data curation)과 반복 평가(iterative evaluation)의 효과를 강조합니다.



### Learning to Navigate Under Imperfect Perception: Conformalised Segmentation for Safe Reinforcement Learning (https://arxiv.org/abs/2510.18485)
- **What's New**: COPPOL은 안전 비판 환경에서의 항해를 위한 새로운 접근법으로, 위험 인식을 및 불확실성 처리를 통합한 방법입니다. 이 접근법은 기존의 위험 감지 방법들이 완벽한 감지를 가정하는 것을 극복하고, 불완전한 샘플 환경에서 안전성을 보장하는 분포 자유화 원리를 적용합니다. COPPOL을 통해 칼리브레이트된 위험 지도와 하위 정책 학습을 가능하게 하고, 이는 안전성과 효율성을 동시에 유지할 수 있게 합니다.

- **Technical Details**: COPPOL는 세그멘테이션 Deep Neural Network (DNN)를 사용하여 위험 점수를 산출하고 이를 칼리브레이션하여 유한 샘플 보증을 가진 세트 값 위험 지도(set-valued hazard maps)로 변환합니다. 이 결과로 생성된 위험 지도는 로봇 경로 계획을 위한 위험 인식 비용 필드를 유도합니다. 이 방법론은 통계적으로 보장된 위험 범위를 통해 RL 에이전트가 실제 안전한 정책을 학습하도록 돕습니다.

- **Performance Highlights**: COPPOL을 통한 평가 결과는 기존 비교 기준 대비 위험 감지 커버리지를 6배 이상 향상시켰음을 보여줍니다. 특히, 일부 베이스라인에서는 불안전 지역의 탐지 정확도가 90%에 달하며, RL 에이전트는 위험 지역에서의 체류 시간을 약 50% 줄이고 안전한 지역과의 거리를 2~3배 더 유지하는 등의 성과를 보였습니다. 이를 통해 COPPOL이 비효율성을 제거하고 안전한 경로를 만드는 데 기여했다는 것을 입증할 수 있었습니다.
...
- **What's New**: 이번 연구에서는 Uncertain Safety Critic (USC)라는 새로운 접근 방식을 도입합니다. 이 기법은 안전 비평가의 학습에 불확실성 인식 모듈화를 통합하여 안전성을 강화하면서도 정책의 성능을 유지합니다. USC는 불확실하고 비용이 높은 영역에서 보수성을 집중시키고 안전한 영역에서는 뚜렷한 경량을 유지함으로써 효과적인 보상-안전 거래를 가능하게 합니다. 기존의 안전 비평가들 사이에서 발생하던 그라디언트 붕괴 문제를 해결하여 안전성과 성능 간의 균형을 획기적으로 개선합니다.

- **Technical Details**: USC는 파라미터 공간의 불확실성을 명시적으로 고려하여 안전 비평가의 과도한 보수성을 줄입니다. 기존의 비평가들이 리스크 경관을 불균형적으로 평평하게 하거나 부풀리는 것과 달리, USC는 비평가의 인지적 불확실성에 비례하여 보수성을 조절하여 정보 그라디언트를 보존합니다. 또한, USC는 불확실성 기반 목표와 불확실성 개선 절차를 활용하여 주어진 상태-행동 공간에서의 잘 표현되지 않은 영역을 보강합니다. 이를 통해 USC는 더 날카롭고 신뢰할 수 있는 비용 맵을 생성하여 정책이 확장 가능하고 정확한 보상-비용 거래를 수행할 수 있게 합니다.

- **Performance Highlights**: USC는 기존 방법과 비교하여 안전 위반을 약 40% 감소시키면서 경쟁력 있는 보상을 유지합니다. 또한, 예측된 비용 그라디언트와 실제 그라디언트 간의 오류를 약 83% 줄여, 보상 극대화(성능)와 제약 충족(안전) 간의 미세한 거래를 가능하게 합니다. USC는 실험적 결과와 이론적 분석을 통해 자신의 효과를 충분히 입증했으며, 여러 상황에서 작업 복잡성에 효과적으로 적응할 수 있는 보상을 달성했습니다.
...
### Learning Boltzmann Generators via Constrained Mass Transpor (https://arxiv.org/abs/2510.18460)
- **What's New**: 이번 논문에서는 Constrained Mass Transport (CMT)라는 새로운 변별적 프레임워크를 제안하여 볼츠만 생성기(Boltzmann Generators)로부터 고차원 및 다중 모드의 비정규화된 확률 분포로부터 효율적으로 샘플링하는 문제를 해결합니다. 이 방법은 KL 발산과 엔트로피 감소를 제한하여 샘플링 과정에서 모드 붕괴를 피하고, 물질 이동(teleportation) 문제를 감소시킵니다. 저자들은 이후 표준 BG 기준 테스트와 에너지 평가에 기초한 ELIL 테트라펩타이드라는 대규모 시스템에서 CMT의 성능을 입증합니다.

- **Technical Details**: CMT는 KL 발산과 엔트로피 감소에 대한 제약 조건을 두어 선형적인 밀도 변화(transportation) 과정을 가능하게 합니다. 이 방법론은 이전의 샘플링 기법과는 다르게, 반복 최적화 기법으로 최적의 밀도를 업데이트하면서 기존 밀도와의 근접성을 유지합니다. CMT는 Trust Region과 혼합 제약을 통해 샘플링 문제를 해결하며, 이는 매 단계에서 확률 밀도를 제어할 수 있는 추가적인 자유도를 제공합니다.

- **Performance Highlights**: CMT는 기존의 최첨단 방법들보다 2.5배 높은 유효 샘플 크기를 달성하며, 특히 MD 샘플에 의존하지 않고도 안정적인 결과를 보여줍니다. 저자들은 엄청난 성능 개선을 보여주며, ELIL 테트라펩타이드 문제를 제시하여 에너지 평가만으로 학습이 가능한 가장 큰 시스템을 연구하게 되었다고 강조합니다. 이와 같은 성과는 CMT가 고차원 샘플링 문제에 있어 실용적인 솔루션을 제공할 수 있다는 점을 보여줍니다.



### Provable Generalization Bounds for Deep Neural Networks with Adaptive Regularization (https://arxiv.org/abs/2510.18410)
Comments:
          8 pages

- **What's New**: 최근의 연구는 심층 신경망(DNN)에서의 과적합(overfitting) 문제를 해결하기 위한 새로운 정규화 방법인 Momentum-Adaptive Gradient Dropout (MAGDrop)을 제안하고 있습니다. 이전의 정적 정규화 기법들이 가지는 한계를 극복하기 위해 MAGDrop은 현재의 그래디언트(gradient)와 누적된 모멘텀(momentum)에 기반하여 드롭아웃(dropout) 비율을 동적으로 조정합니다. 이러한 혁신적인 기법은 DNN의 비선형 최적화(non-convex optimization)에서의 안정성을 향상시킵니다.

- **Technical Details**: MAGDrop은 활성화(activation)에서 정규화를 수행하며, 현재 그래디언트의 노름(norm)과 최적화 알고리즘으로부터 축적된 모멘텀을 통해 드롭아웃 비율을 조정합니다. 전통적인 드롭아웃 방식과는 달리, MAGDrop은 안정적이고 정보성 높은 특징을 우선시하여 비선형 손실 경관에서의 과적합을 줄입니다. 수학적으로, MAGDrop의 적응형 개념을 반영한 PAC-Bayes 일반화 경계(generalization bound)를 도출하였으며, 이를 통해 20% 더 날카로운 일반화 보장을 제공합니다.

- **Performance Highlights**: 실험 결과, MAGDrop은 MNIST와 CIFAR-10 데이터셋에서 테스트 정확도에서 표준 드롭아웃 및 Adaptive Gradient Regularization(AGR)과 비교하여 1-2% 향상된 성능을 보여주었습니다. CIFAR-10에서는 일반화 격차(generalization gap)가 7.14% 이하로, MNIST에서는 0.48% 이하로 유지되었습니다. 이러한 성과는 MAGDrop이 DNN의 일반화를 향상시킬 수 있는 강력한 프레임워크임을 시사합니다.
...
### Approximation Rates of Shallow Neural Networks: Barron Spaces, Activation Functions and Optimality Analysis (https://arxiv.org/abs/2510.18388)
- **What's New**: 이 논문은 지수 함수의 거듭제곱으로 구성된 활성화 함수가 있는 얕은 신경망의 근사 특성을 조사합니다. 특히, Barron 함수 공간 내에서 근사 속도가 차원과 함수의 매끄러움에 어떻게 의존하는지를 강조합니다. ReLU$^{k}$ 활성화 함수의 근사 속도를 분석하고, 1-벡터 한계(coefficient)를 초과하는 최적 속도를 확인했습니다.

- **Technical Details**: 연구에서는 Barron 함수 공간 및 Sobolev 공간의 함수에 대한 최적 근사 속도를 정립하였습니다. 특히, 소위 '차원의 저주(curse of dimensionality)'를 확인한 것이 주목할 만합니다. Barron 공간에서 다항식 감소 조건을 가진 활성화 함수의 속성을 분석하고, d차원에서의 근사 속도가 어떻게 변하는지에 대한 예제로 다루었습니다.

- **Performance Highlights**: 이 논문은 얕은 신경망의 근사 능력의 한계를 명확히 찾아내며, 활성화 함수 및 네트워크 구조의 선택에 대한 통찰력을 제공합니다. 얻어진 결과는 신경망에서 최적의 매개변수 설정을 통해 성능을 개선할 가능성을 제시합니다.



### Training Diverse Graph Experts for Ensembles: A Systematic Empirical Study (https://arxiv.org/abs/2510.18370)
- **What's New**: 이 연구는 GNN 앙상블을 위한 전문가 다양화 기술의 체계적인 경험적 연구를 첫 번째로 소개합니다. 20가지의 다양화 전략을 평가하고, 14개의 노드 분류 벤치마크에 걸쳐 200개 이상의 앙상블 변형을 구성하고 분석합니다. 이를 통해 전문가 다양성, 상호 보완성 및 앙상블 성능 평가에 대한 통찰을 제공합니다.

- **Technical Details**: 연구에서는 두 단계의 훈련-병합 방식의 Mixture-of-Experts (MoE) 설정을 채택하였습니다. GNN 전문가들은 독립적으로 훈련되며, 각 전문가의 로짓(logit)을 집계하기 위해 별도의 게이팅 모델이 훈련됩니다. 마지막으로, 리샘플링(random re-initialization), 하이퍼파라미터 조정(hyperparameter tuning), 설계 변형(architectural variation), 방향성 모델링(directionality modeling), 훈련 데이터 파티셔닝(training data partitioning) 등 다양한 기술을 조합하여 최적의 조합을 찾습니다.

- **Performance Highlights**: 연구 결과는 전문가 간의 더 큰 다양성이 일반적으로 향상된 상호 보완성과 향상된 앙상블 성능으로 이어짐을 보여줍니다. 또한, 최대 3명의 전문가와 단일 다양화 방법을 사용하여 앙상블은 강력한 개별 모델에 비해 0.7%~3.9%의 노드 분류 정확도 개선을 달성하였습니다. 이러한 결과는 GNN 성능을 일관되게 향상시키는데 기여하는 원칙적인 전문가 다양화의 가능성을 보여줍니다.



### Towards Unsupervised Open-Set Graph Domain Adaptation via Dual Reprogramming (https://arxiv.org/abs/2510.18363)
Comments:
          Accepted by NeurIPS 2025

- **What's New**: 이 연구는 비지도(Open-set) 그래프 도메인 적응 문제를 탐구하며, 기존의 방법들과는 달리 알려진 클래스뿐만 아니라 이전에 보지 못한 노드 유형을 식별할 수 있도록 하는 새로운 프레임워크 GraphRTA를 제안합니다. 기존의 보편적인 그래프 도메인 적응 모델들은 보통 닫힌 집합(closed-set) 환경에서만 작동하였으나, 실제 데이터에서는 새로운 클래스가 출현할 수 있어 실용성이 떨어지는 점을 지적합니다. 특히 이 프레임워크는 그래프 거리 조정과 모델 매개변수 정제를 통해 지식 전이 과정에 효과를 봅니다.

- **Technical Details**: GraphRTA는 그래프 구조 및 노드 특징을 재프로그래밍(reprogramming)하여 이미 알려진 클래스와 알려지지 않은 클래스 간의 더 나은 분리성을 확립합니다. 모델 측면에서도 특정 도메인 매개변수를 정제하여 원본 그래프에 대해 편향을 줄입니다. 또한, 알려지지 않은 클래스에 대한 추가 차원을 클래스 분류기에 추가하여 오픈 셋 인식(open-set recognition)에서 수동으로 설정된 임계값(threshold)을 제거하는 혁신을 포함합니다.

- **Performance Highlights**: 다양한 공공 데이터 세트에 대한 실험 결과는 GraphRTA가 최신의 상태(state)-of-the-art 기준선들과 비교하여 우수한 성능을 발휘할 수 있음을 보여줍니다. 이 연구는 그래프 도메인 적응 문제에서 더 나은 일반화 능력을 작동할 수 있도록 하여 다양한 응용에 적합할 수 있음을 입증합니다. GraphRTA의 성과는 특히 비지도 오픈 셋 그래프 도메인 적응 문제를 해결하는 데 중요한 기여를 합니다.



### Learning to Flow from Generative Pretext Tasks for Neural Architecture Encoding (https://arxiv.org/abs/2510.18360)
Comments:
          Published as a conference paper at NeurIPS 2025

- **What's New**: 이 논문은 딥러닝 모델의 성능을 예측하기 위한 새로운 전훈련(pre-training) 방법인 FGP(Flow-based Generative Pre-training)를 제안합니다. FGP는 복잡한 모델 구조를 요구하지 않고도 신경 아키텍처의 정보 흐름을 포착할 수 있도록 인코더를 훈련시킵니다. 이는 전통적인 훈련 방법보다 효율적으로 아키텍처를 검색할 수 있는 가능성을 제공합니다.

- **Technical Details**: 기존의 흐름 기반 인코더는 정보 흐름을 캡처하기 위해 복잡한 모델 구조를 사용해야 했으며, 이로 인해 처리 시간이 길어지는 도전 과제가 있었습니다. FGP는 인코더가 신경 아키텍처의 흐름 대리(representation)를 재구성하도록 훈련하여 정보 흐름을 효과적으로 학습할 수 있게 합니다. 이를 통해 인코더는 아키텍처의 정보를 보다 빠르고 정확하게 캡처할 수 있습니다.

- **Performance Highlights**: 실험 결과, FGP는 동일한 인코더를 감독 학습만으로 훈련한 경우에 비해 최대 106%까지 Precision-1% 성능을 향상시켰습니다. 또한, FGP는 여러 다운스트림 작업에서 베이스라인 전훈련 방법에 비해 성능 우위를 보였으며, 특히 성능 예측 실험에서 27개 설정 중 23개에서 우수한 결과를 보여줍니다.
...
### Computable universal online learning (https://arxiv.org/abs/2510.18352)
Comments:
          Accepted for presentation at NeurIPS 2025

- **What's New**: 이번 논문은 머신 러닝 이론에서 학습이 가능한 시점을 이해하는 데 중점을 둡니다. 특히, Bousquet et al. (2021)에 의해 특성화된 보편적 온라인 학습(universal online learning) 모델을 통해 학습이 실제 컴퓨터 프로그램으로 구현될 수 있는지를 다룹니다. 이 논문은 예상치 못한 오류(finite number of mistakes)를 줄이는 전략을 요구하며, 학습자가 어떤 가설 클래스가 computable learning을 보장하는지를 연구합니다.

- **Technical Details**: 이 논문에서는 R​E​RRER 클래스가 보편적으로 온라인 학습 가능하지만 computable learner에 의해 학습될 수 없는 사례를 제시합니다. 따라서, 비록 관련된 클래스가 computable 성격을 가진 경우에도 학습 가능성과 관련하여 심오한 통찰을 제공합니다. 또한, proper universal online learning의 변형도 함께 검토하며, 그 가능성을 명확히 설명합니다.

- **Performance Highlights**: 주요 결과로는, agnostic learning 변형에서 Adversary가 가설을 선택하는데 제한이 없을 때 sublinear expected regret를 보장하는 것을 반증하는 이론을 제시합니다. 본 연구는 computability constraints를 도입하여 학습 클래스가 computably learnable한 지를 탐색했으며, 이는 기존 이론에 대한 보다 실질적인 관점을 제공합니다.



### Why Policy Gradient Algorithms Work for Undiscounted Total-Reward MDPs (https://arxiv.org/abs/2510.18340)
- **What's New**: 이번 연구에서는 정책 그라디언트 방법(policy gradient method)의 수렴(convergence) 특성을 무가선(total-reward) 무한 지평선(infinite-horizon) MDP에 적용하였습니다. 이는 기존 이론이 $eta < 1$인 상황에만 적용될 수 있었던 것과 대조적입니다. 새로운 접근 방식으로, 정책이 모든 행동에 양의 확률을 부여할 때 MDP 상태를 순환(recurrent) 및 비순환(transient) 상태로 구분할 수 있음을 발견하였습니다.

- **Technical Details**: 연구에서는 고전적인 상태 방문 측정값(classical state visitation measure)이 $eta = 1$일 경우 잘 정의되지 않을 수 있음을 언급하고, 이를 대체할 새로운 개념인 전이 방문 측정(transient visitation measure)을 제안하였습니다. 전이 방문 측정을 통해, projected policy gradient 및 natural policy gradient 알고리즘의 수렴 보장을 확립하였습니다. 이 분석은 딥 RL 모델에 일반적인 softmax 출력 레이어를 사용하는 정책 세트의 특성을 활용하여 이루어졌습니다.

- **Performance Highlights**: 정책 그라디언트 방법에 대한 최근 연구는 무가선 설정에서 크게 부족하였습니다. 반면 이번 연구는 무가선 기대 총 보상 infinite-horizon MDP에 대한 수렴을 rigorously 확립했습니다. 특히, 제안된 새로운 측정 방식 덕분에, 기존의 제한을 극복하고 더욱 발전된 RL 모델에 적용할 수 있는 기반을 마련하였습니다.
...
### Uncertainty Estimation by Flexible Evidential Deep Learning (https://arxiv.org/abs/2510.18322)
Comments:
          NeurIPS 2025

- **What's New**: 이번 연구에서는 유연한 증거 심층학습(flexible evidential deep learning, \mathcal{F}-EDL) 방법을 제안하여, 기존의 EDL의 한계를 극복하고 더 유연한 Dirichlet 분포를 클래스 확률에 대해 예측하게 함으로써 불확실성을 개선했습니다. 이 새로운 접근 방식은 예측의 일반화 가능성을 증대시키고, 복잡한 상황에서도 신뢰성을 높입니다.

- **Technical Details**: \mathcal{F}-EDL은 Dirichlet 분포의 일반화인 유연한 Dirichlet (FD) 분포를 통해 더 표현력이 뛰어난 불확실성 모델링을 가능하게 합니다. 이를 위해, 클래스 확률에 대한 FD 분포 예측을 수행하며, 적절한 목적 함수를 도입하여 불확실성 인식 분류를 수행합니다. 이 모델은 또한 학습된 데이터를 바탕으로 데이터에 의존하는 혼합 가중치를 사용하여 EDL과 소프트맥스 예측을 적응적으로 결합하는 혼합 모델 기능을 구현합니다.

- **Performance Highlights**: 저자들은 \mathcal{F}-EDL이 다양한 UQ 관련 다운스트림 작업에서 최첨단 성능을 꾸준히 달성하며, 신뢰성 있는 결과를 보여주는 것을 실험적으로 입증했습니다. 특히, 노이즈가 있는 ID 데이터 및 다른 분포 이동 시나리오에서도 이 모델이 우수한 일반화 능력을 보여주며, 다중 모드 불확실성을 효과적으로 포착했다고 강조하고 있습니다.
...
### Towards Identifiability of Hierarchical Temporal Causal Representation Learning (https://arxiv.org/abs/2510.18310)
- **What's New**: 본 연구는 시간 시계열 데이터의 계층적 잠재 역학을 모델링하는 새로운 방법론인 Causally Hierarchical Latent Dynamic (CHiLD) 프레임워크를 제안합니다. 기존의 방법들이 단일 타임스텝 관측 변수에서 계층적 잠재 변수의 결합 분포를 회복하지 못하는 한계를 지적하고, 세 개의 조건부 독립 관측값이 결합 분포를 고유하게 결정할 수 있다는 점을 발견하였습니다. 이 접근법은 다층 잠재 변수를 식별하기 위해 시간적 맥락의 관측 변수를 활용합니다.

- **Technical Details**: CHiLD 프레임워크는 다층 잠재 변수를 식별하기 위해 시간적 맥락 관측 변수를 사용하여 필연적으로 발생하는 희소성을 통해 잠재 변수를 식별하는 과정으로 구성됩니다. 연구진은 변분 추론(variational inference)을 기반으로 하여 시간 시계열 생성 모델을 개발하였습니다. 이 모델은 맥락 인코더(contextual encoder)를 포함하여 다층 잠재 변수를 재구성하고, 계층적 잠재 역학의 독립적 잡음 조건을 부과하기 위해 정규화 흐름 기반의 계층적 사전 네트워크(normalizing flow-based hierarchical prior networks)를 활용합니다.

- **Performance Highlights**: 실험적 평가를 통해 다층 잠재 변수 식별의 유효성을 입증하였으며, 합성 및 실제 데이터셋에서 CHiLD의 효과를 검증했습니다. 연구의 정량적 및 정성적 결과는 CHiLD가 시간 시계열 생성에서 품질과 조절 가능성 측면에서 우수함을 보여줍니다. 이러한 결과는 계층적 잠재 역학을 모델링하는 데 있어 CHiLD 접근법의 효과성을 강조합니다.



### Physics-Informed Parametric Bandits for Beam Alignment in mmWave Communications (https://arxiv.org/abs/2510.18299)
- **What's New**: 이번 논문에서는 밀리미터파(mmWave) 통신에서 해결하기 어려운 빔 정렬 및 추적 문제를 새로운 알고리즘으로 접근합니다. 제안한 두 가지 알고리즘인 pretc와 prgreedy는 mmWave 채널의 희소 다중 경로 성질을 활용하여 효율성을 높였습니다. 이 알고리즘은 기존의 전통적인 밴딧 알고리즘이 가지는 비효율성을 보완하고, 실제 환경에서의 채널 변동에 대응하도록 설계되었습니다.

- **Technical Details**: 제안된 pretc 알고리즘은 임의의 탐색 단계로 시작하여, 추정된 보상 함수에 따라 최적의 빔을 선택합니다. 반면에 prgreedy는 온라인 방식으로 이러한 추정을 수행하며, 현재의 추정에 따라 가장 좋은 빔을 선택합니다. 두 알고리즘 모두 유동적인 환경 속에서의 빔 추적에도 쉽게 적응할 수 있습니다.

- **Performance Highlights**: DeepMIMO 데이터셋과 DeepSense6G 실제 데이터셋을 통해 실험한 결과, 제안한 알고리즘은 다양한 채널 환경에서 기존 방법들을 능가하는 성능을 보이며 일반화 가능성과 강건함을 보여줍니다. 특히, 모바일 환경에서의 실험은 알고리즘이 이동성과 차단에 빠르게 적응할 수 있음을 강조합니다.



### Online Time Series Forecasting with Theoretical Guarantees (https://arxiv.org/abs/2510.18281)
- **What's New**: 이번 논문에서는 온라인 시계열 예측을 다루며, 이는 시간에 따라 알려지지 않은 분포 변화가 발생하는 상황을 포함합니다. 저자는 이 문제를 해결하기 위해 이러한 분포 변화를 다루는 이론적 프레임워크인 TOT(Theoretical framework for Online Time-series forecasting)를 제안합니다. 이 프레임워크는 예측자가 잠재 변수(latent variables)를 공급받을 때 Bayes 위험을 감소시킬 수 있다는 이론적인 보장을 제공합니다.

- **Technical Details**: 저자들은 잠재 변수를 사용하여 예측의 오차를 줄이고, 이를 위해 단순히 네 개의 인접한 관측값을 사용하여 이러한 변수와 그 인과적 역학을 식별할 수 있다는 점을 보여줍니다. 논문에서 제안하는 새로운 모델-불가지론적(agnostic) 청사진은 시간 디코더(temporal decoder)와 두 개의 독립적인 노이즈 추정기를 사용하여 설계됩니다. 이 접근법은 관측 변수의 분포와 잠재 변수의 인과 추론을 모델링하는 데 도움을 줍니다.

- **Performance Highlights**: 실험 결과는 이론적 주장을 지지하며, 다른 여러 기준에 대한 플러그인(plug-in) 구현들이 일반적인 개선을 보여줍니다. 이러한 결과는 현실 세계의 응용 프로그램에서 효과적임을 강조합니다. 저자들은 시계열 예측 성능을 향상시키기 위한 명시적인 이론적 지침을 제공하고 있으며, 그 효과성은 여러 벤치마크 테스트에서 확인되었습니다.
...
### Learning with Dual-level Noisy Correspondence for Multi-modal Entity Alignmen (https://arxiv.org/abs/2510.18240)
Comments:
          30 pages, 12 figures

- **What's New**: 이번 논문에서는 다중 모달 엔티티 정렬(Multi-Modal Entity Alignment, MMEA) 분야에서 활용되고 있는 모델의 새로운 문제점인 이중 수준 노이즈 대응(Dual-level Noisy Correspondence, DNC)을 규명하고 이를 다루기 위한 새로운 방법인 RULE을 제안합니다. DNC는 엔티티 속성과 엔티티 간, 속성 간의 대응 관계에서 발생하는 잘못된 정렬을 의미하며, 이는 실제 MMKG에서는 전문가의 주석에 의존하기 때문에 빈번히 발생합니다. 이 논문은 이러한 노이즈가 MMEA의 성능에 미치는 부정적인 영향을 분석하고, 이를 해결하기 위한 접근법을 제시합니다.

- **Technical Details**: RULE은 두 가지 원칙에 기반하여 엔티티와 그래프 간의 대응 관계의 신뢰성을 평가합니다. 이를 통해 엔티티 속성 융합 시 발생할 수 있는 노이즈의 부정적인 영향을 줄이고, 그래프 간 불일치를 제거하는 과정에서도 노이즈에 대한 과적합을 방지합니다. 이 외에도 RULE은 테스트 시간에 이러한 속성 간 연관성을 추적하는 모듈을 포함하여 더욱 높은 정밀도의 엔티티 식별을 가능하게 합니다.

- **Performance Highlights**: 다양한 벤치마크에서 실시한 실험 결과, RULE은 기존의 최첨단 방법들과 비교하여 DNC 문제를 극복하며 우수한 성능을 보였습니다. 특히, 노이즈에 대한 Robustness를 제공하여 MMEA 작업에서의 정확도를 크게 향상시켰습니다. 이러한 결과는 RULE이 존재하는 방법 중에서 테스트 시간에 강인성을 최초로 개선하는 방법 중 하나일 수 있음을 시사합니다.



### Fostering the Ecosystem of AI for Social Impact Requires Expanding and Strengthening Evaluation Standards (https://arxiv.org/abs/2510.18238)
Comments:
          Accepted at NeurIPS 2025

- **What's New**: AI/ML을 통한 사회적 영향(AISI)에 대한 연구가 최근 증가하고 있으며, 이러한 흐름에 맞춰 리뷰 기준이 정교화되고 있습니다. 그러나 현재 기준은 배치(Deployment)와 ML 방법론 혁신을 동시에 이루는 프로젝트에만 초점을 맞추고 있어, 연구 생태계의 지속 가능성을 해치는 경향이 있습니다. 연구자와 리뷰어가 배치 외에 사회적 영향을 포괄적으로 이해하고 평가할 필요가 있습니다.

- **Technical Details**: AISI는 적용 문제를 해결하고 사회에 긍정적인 영향을 미치는 것을 주요 목표로 하는 연구 흐름입니다. 기존의 ML 연구와는 달리, AISI는 다양한 외부 파트너와의 협력이 요구됩니다. 그러나 현재의 연구 생태계에서는 '임팩트(Impact)'의 정의가 명확하지 않고, 배치에만 중점을 두어 다른 중요한 기여들을 간과하는 시각이 존재합니다.

- **Performance Highlights**: 연구자들은 기술적으로 새로운 방법론을 개발하지 않더라도 ML의 실제 사용을 최적화하는 기여를 인식해야 하며, 이로 인해 파트너 조직의 기술 능력이 강화됩니다. 배치된 시스템의 평가는 보다 철저히 이루어져야 하며, 이를 통해 AISI의 전반적인 생태계가 개선될 수 있습니다. 마지막으로, 실제 적용 사례를 통한 연구는 많은 연구자들에게 귀중한 학습 기회를 제공합니다.



### ACTG-ARL: Differentially Private Conditional Text Generation with RL-Boosted Contro (https://arxiv.org/abs/2510.18232)
- **What's New**: 이번 연구는 Differential Privacy (DP) 하에서 고품질 합성 텍스트 생성을 위한 계층적 프레임워크를 소개합니다. 이 프레임워크는 DP 합성 텍스트 생성을 두 개의 하위 작업인 특징 학습(feature learning)과 조건부 텍스트 생성(conditional text generation)으로 분해합니다. 이를 통해 생성을 단순화하고 최적화된 구성을 통해 효과적으로 합성 텍스트를 생성하는 확인 된 방법론을 제시하고 있습니다.

- **Technical Details**: 제안하는 방법론 ACTG (Attribute-Conditioned Text Generation)은 풍부한 표 형식 스키마를 특징으로 하며, 특별히 설계된 DP 테이블 합성기와 DP 미세 조정된 조건부 생성기를 함께 사용합니다. 또한, 사후 훈련 방법인 Anchored RL (ARL)을 통합하여 합성기가 조건에 따라 더욱 잘 학습하고 지시를 따를 수 있도록 개선하였습니다. ARL은 강화 학습(reinforcement learning) 기법을 활용하여 좋은 성능을 유지하고, 최상의 데이터를 기반으로 감독형 미세 조정(supervised fine-tuning)을 적용하여 보상 해킹(reward hacking)을 방지합니다.

- **Performance Highlights**: ACTG-ARL은 실제 데이터셋에서 최첨단 성능을 발휘하며, 기존 연구보다 DP 합성 텍스트의 품질을 20% 향상시키고 속성 분포 매칭(attribute distribution matching)에서는 50%의 개선을 보여줍니다. 이는 다양한 실제 응용 프로그램을 위한 세밀하고 제어 가능한 생성을 가능하게 합니다. 따라서 본 연구는 개인 정보 보호 보장을 유지하면서도 고품질 합성 텍스트 생성을 위한 새로운 경지를 열었습니다.



### Towards Fast LLM Fine-tuning through Zeroth-Order Optimization with Projected Gradient-Aligned Perturbations (https://arxiv.org/abs/2510.18228)
Comments:
          10 pages, 5 figures

- **What's New**: 이 논문에서는 P-GAP을 제안하여 제로 차원 최적화(zero-order optimization) 기법을 통해 대규모 언어 모델(LLM)의 파인튜닝(fine-tuning) 속도를 개선합니다. 기존의 ZO 방법들이 겪는 높은 그래디언트 추정의 분산 문제를 해결하기 위해, P-GAP은 저차원 그래디언트 공간에서 프로젝션 그래디언트 방향으로의 섭동을 정렬하여 이를 수행합니다. 이 접근 방식은 섭동된 매개변수의 수를 줄이고 분산을 감소시켜 LLM 파인튜닝의 수렴 속도를 가속화합니다.

- **Technical Details**: P-GAP의 핵심 아이디어는 저차원 그래디언트 공간을 추정하고 섭동을 그 방향으로 정렬하는 것입니다. 구체적으로, 그래디언트의 방향과 일치하도록 섭동을 조정하여 그래디언트 추정의 분산을 줄입니다. 이러한 방법 덕분에 P-GAP은 훈련iterations을 최대 81% 줄이고, GPU 사용 시간을 약 70% 절감할 수 있습니다. 전체적으로 P-GAP은 파라미터 효율적인 업데이트 방식을 통해 LLM의 자원 효율성을 높입니다.

- **Performance Highlights**: P-GAP을 이용한 실험 결과, 분류 작업에서 기준 모델 대비 최대 6%의 정확도 증가, 생성 작업에서는 최대 12%의 정확도 증가를 기록하였습니다. 또한, 훈련 속도는 5.2배 빨라지고 총 훈련 시간은 61분 이상 단축되었습니다. 이러한 결과는 P-GAP이 빠르고 확장 가능하며 자원 효율적인 ZO LLM 파인튜닝을 가능하게 한다는 것을 보여줍니다.



### Joint Optimization of Cooperation Efficiency and Communication Covertness for Target Detection with AUVs (https://arxiv.org/abs/2510.18225)
- **What's New**: 이번 논문은 자율 수중 차량(AUVs)을 활용한 수중 협력 목표 탐지에 대한 연구를 진행하며, 협력 효율성과 통신 은밀성 간의 중요한 균형을 다룹니다. 해당 연구에서는 공동 궤적 및 전력 제어 최적화 문제를 공식화하고, 이를 해결하기 위한 혁신적인 계층적 행동 관리 프레임워크를 제시합니다. 계층적 공식화에 따라, 매크로 수준에서는 마스터 AUV가 에이전트 선택 과정을 마르코프 결정 과정으로 모델링하며, 전략적 작업 할당에 대해 proximal policy optimization 알고리즘을 적용합니다.

- **Technical Details**: 각 에이전트의 분산 의사결정은 부분적으로 관측 가능한 마르코프 결정 과정으로 모델링되며, 여기에서 다중 에이전트 proximal policy optimization 알고리즘을 통해 로컬 관측에 따라 동적으로 궤적과 전송 전력을 조정합니다. 중앙 집중화 교육과 분산 실행 패러다임 하에서, 우리의 목표 탐지 프레임워크는 에너지 및 이동성 제약을 충족하면서 적응형 비밀 협력을 가능하게 합니다. 제안된 시스템은 고려된 신호 및 작업, 에너지 소비를 포괄적으로 모델링하여 다수 AUV의 효율적이고 안전한 운영을 위한 이론적 통찰 및 실질적 솔루션을 제공합니다.

- **Performance Highlights**: 논문은 수중 비밀 통신 작업의 실행을 위한 상당한 시사점을 제공하며, 특히 매크로 및 마이크로 수준에서 에이전트 할당과 궤적 제어를 공동으로 최적화하는 계층적 multi-agent proximal policy optimization (HMAPPO) 프레임워크를 개발하여 효과적인 협력 능력을 극대화하는 것을 목표로 하고 있습니다. 실험 결과는 제안된 설계의 수렴성, 확장성 및 협력과 비밀성이 조화를 이루는 성능에 대한 트레이드오프를 보여줍니다.



### Ensemble based Closed-Loop Optimal Control using Physics-Informed Neural Networks (https://arxiv.org/abs/2510.18195)
- **What's New**: 이 논문에서는 Hamilton-Jacobi-Bellman (HJB) 방정수를 해결하는 새로운 멀티스테이지 앙상블 프레임워크를 제안합니다. 이전의 PINN 기반 접근 방식은 훈련 중 HJB 시행을 안정화하기 위해 안정화 기법에 의존했으나, 본 연구는 안정화 항 없이 비선형 시스템 제어를 가능하게 합니다. 또한, 단일 학습 제어 신호 또는 앙상블 제어 신호 정책을 통해 제어할 수 있는 방식이 제시됩니다.

- **Technical Details**: 이 연구는 시간 불변의 연속 비선형 시스템을 위한 최적 비용을 배우기 위해 HJB 방정식을 해결하는 멀티스테이지 앙상블 프레임워크를 도입합니다. HJB 방정식은 여러 단계의 제어 신호를 통해 비선형 시스템을 효과적으로 제어할 수 있도록 돕습니다. 또한, 기존의 PINN 방법에서는 경계조건 손실 컴포넌트를 제거하여 훈련이 가능합니다.

- **Performance Highlights**: 실험을 통해, 제안된 방법이 닫힌 루프 제어 시나리오에서 안정적인 성능을 보여줍니다. 노이즈가 있는 시스템 상태와 다양한 초기 조건에서도 좋은 성능을 유지하며, 아울러 앙상블 알고리즘을 통한 적응형 제어 정책을 개발하여 아웃라이어 제어 신호를 제외하면서도 평균 신호로 복원하는 방식을 제시합니다.
...
### Nash Policy Gradient: A Policy Gradient Method with Iteratively Refined Regularization for Finding Nash Equilibria (https://arxiv.org/abs/2510.18183)
- **What's New**: 이번 논문에서는 Nash 균형(Nash equilibrium)을 찾는 새로운 접근 방식에 대해 논의합니다. 특히, 기존의 규제 기반 방법들과는 달리 규제 강도를 감소시키지 않고 큰 값을 유지함으로써 안정성을 확보하고, 반복적인 참조 정책(referecne policy) 개선을 통해 수렴에 도달하는 방법을 제안합니다. 이론적으로는 두 플레이어 제로섬 게임(zero-sum games)에서 Nash 균형에 정확히 수렴함을 보장하는 주장을 제시합니다.

- **Technical Details**: Nash Policy Gradient (NashPG)라는 새로운 알고리즘을 개발하였으며, 이는 기존의 정책 기울기 방법들(policy gradient methods)의 일반화를 유지하면서 현재와 참조 정책만을 고려하여 작동합니다. 이 알고리즘은 각 에이전트가 다른 에이전트에 대한 고려 없이 자신의 정책을 독립적으로 업데이트할 수 있도록 하며, 경량화되어 현재 정책과 참조 정책만 관리하면 됩니다. 이 iterative-refinement 과정은 정책의 엄격한 단조 개선을 보장합니다.

- **Performance Highlights**: NashPG는 고전적인 벤치마크 게임에서 기존의 모델 프리 방법들과 비교해 비슷하거나 더 낮은 exploitability를 달성하였으며, Battleship와 No-Limit Texas Hold'em과 같은 대규모 도메인에서도 높은 성능을 보였습니다. 특히 NashPG이 지속적으로 향상된 Elo rating을 달성하는 것을 보여 주며, 대규모 실제 환경에서도 효과적으로 확장성을 갖추고 있음을 입증합니다.



### Rethinking PCA Through Duality (https://arxiv.org/abs/2510.18130)
Comments:
          NeurIPS 2025 poster

- **What's New**: 최근 자기 주의(self-attention)와 커널 주성분 분석(kernel PCA) 간의 연관성을 바탕으로, 주성분 분석(PCA)의 기본을 재검토하는 연구가 진행되었습니다. 이 연구에서는 오목-볼록 차이(difference-of-convex, DC) 프레임워크를 활용하여 여러 가지 새로운 공식들을 제시하고 새로운 이론적 통찰을 제공합니다.

- **Technical Details**: 연구에서는 PCA와 유사한 문제의 커널화 가능성(kernelizability) 및 샘플 외(out-of-sample) 적용성에 대해 자세히 설명합니다. 또한, 고전적인 QR 알고리즘과 연결된 동시 반복이 오목-볼록 알고리즘(difference-of-convex algorithm, DCA)의 일례라는 점을 밝히며, 이를 통해 이 기존 방법론에 대한 최적화 관점을 제공합니다.

- **Performance Highlights**: 새롭게 제안된 PCA 알고리즘과 최신 기술들을 실험적으로 비교하고, 강건한 변형의 커널화 가능한 이중 공식(kernelizable dual formulation)을 소개하여 복원 오류의 $l_1$ 편차를 최소화하는 방안을 제시합니다.



### HyperDiffusionFields (HyDiF): Diffusion-Guided Hypernetworks for Learning Implicit Molecular Neural Fields (https://arxiv.org/abs/2510.18122)
- **What's New**: 이 논문에서는 3D 분자 구조를 연속적인 필드로 모델링하는 HyperDiffusionFields (HyDiF)라는 새로운 프레임워크를 소개합니다. 이 접근 방식의 핵심은 특정 원자의 방향으로 공간의 어느 점을 매핑하는 분자 방향 필드(Molecular Directional Field, MDF)입니다. 우리는 분자별 신경 임플리시트 필드(Molecular Neural Fields, MNFs)를 사용하여 MDF를 나타내고, 공유 하이퍼네트워크를 통해 분자 간의 일반화를 촉진합니다.

- **Technical Details**: HyDiF는 밀도 있는 연속 표현을 사용하여 공간의 임의의 점에서 특정 원자의 방향을 나타냅니다. 이 필드는 분자 구조를 명확하게 이해하는 데 중요한 역할을 하며, 신경망을 통해 학습되는 역방향 노이즈 제거 모델로 훈련되어, 연속적인 영역에서의 샘플링을 가능하게 합니다. 또한, HyDiF는 υol기 사용자의 요구에 따라 구조에 기반한 생성을 지원하는 마스크 드퓨전 메커니즘을 자연스럽게 확장합니다.

- **Performance Highlights**: 제안한 모델은 분자 특성 예측을 위한 공간적인 세부 특성 추출 기능을 제공하며, 이로 인해 기존의 그래프나 포인트 클라우드 방식으로는 달성하기 어려운 정밀도를 자랑합니다. 나아가, HyDiF는 단백질과 같은 대형 생체 분자에 대한 확장성을 보여주어, 다양한 크기와 복잡성을 가진 분자 모델링을 다룰 수 있는 유연성을 나타냅니다. 이 연구는 분자 모델링에 있어 새로운 방향을 제시하며, 분할된 고정된 표현에서 연속적이고 생성적인 필드 기반 모델로의 전환을 강조합니다.



### Efficient Long-context Language Model Training by Core Attention Disaggregation (https://arxiv.org/abs/2510.18121)
- **What's New**: 이번 논문에서는 core attention disaggregation (CAD)라는 기법을 제안하고, 이는 큰 언어 모델의 긴 문맥 훈련을 개선하는 데 중요한 역할을 합니다. CAD는 core attention 계산(softmax(QK^T)V)을 모델의 나머지 부분과 분리하여 별개의 장치 풀에서 실행함으로써 효과를 발휘합니다. 이 기법은 긴 문맥 처리에서 발생하는 부하 불균형과 지연 문제를 해결할 수 있는 방법으로 주목받고 있습니다.

- **Technical Details**: CAD는 두 가지 주요 관찰에 기반합니다. 첫째, core attention은 무상태(stateless)로, 훈련 가능한 파라미터가 없으며 최소한의 전이 데이터만을 사용합니다. 둘째, core attention이 조합(composable) 가능하다는 점입니다. 이는 token 수준에서 파티셔닝을 통해 각 shard가 문서의 서로 다른 파티션에서 독립적으로 계산될 수 있음을 뜻합니다. 이 두 가지 특성은 CAD의 효과적인 수행을 가능하게 합니다.

- **Performance Highlights**: DistCA라는 시스템에 구현된 CAD는 512개의 H200 GPU와 최대 512k 토큰의 문맥 길이를 사용하는 작업에서 1.35배 향상된 훈련 처리량을 보여줍니다. CAD는 부하 균형을 거의 완벽하게 유지하는 동시에 메모리 균형을 이룰 수 있습니다. 또한, 기존 시스템에 비해 데이터 및 파이프라인 병렬화에서의 느림 문제를 상당히 줄이며, 효율적인 통신과 계산 병합을 통해 메모리 사용량을 줄였습니다.
...
- **What's New**: 본 논문에서는 Rectified Flows가 ODE 벡터 필드(ODE vector fields)를 학습하고, 이로 인해 출처(source)와 목표(target) 분포 간의 직선 경로를 통해 거의 한 단계 추론이 가능하다는 점을 강조합니다. 그러나 이 직선 경로 목표가 기본적인 실패 모드를 숨기고 있다는 사실을 보여줍니다. 특히 결정론적 훈련(deterministic training) 하에서는 낮은 그래디언트 분산(low gradient variance)으로 인해 임의의 훈련 쌍을 암기(memorization)하게 만들 수 있습니다.

- **Technical Details**: 이 논문은 기본적인 정보 전달 방법인 Gaussian-to-Gaussian 전송(transport)을 분석합니다. 두 가지 훈련 모드(확률적(stochastic)과 결정론적(deterministic)) 하에서 손실(loss) 그래디언트 분산을 활용하여 최적화가 각 설정에서 어떤 벡터 필드를 선호하는지를 규명합니다. 우리의 이론적 연구를 통해 훈련 간 간섭(intersection)이 있는 상황에서도 기억하는 벡터 필드가 존재하고, 직선 경로 최적화 목표가 이 잘 정의되지 않은 필드로 수렴한다는 사실을 입증했습니다.

- **Performance Highlights**: 본 조사 결과는 CelebA 데이터셋을 통해 실증적으로 검증되었습니다. 결정론적 간섭(interpolants)은 암기를 유도하고, 작은 노이즈(injection of small noise)를 주입하면 일반화(generalization)가 회복된다는 것을 확인하였습니다. 특히, 이 논문은 기존 반복 생성 모델의 근본적인 한계를 드러내고, 그래디언트 분산이 학습된 벡터 필드의 비최적성을 드러내는 방법에 대한 통찰력을 제공합니다.
...
### Provably Optimal Reinforcement Learning under Safety Filtering (https://arxiv.org/abs/2510.18082)
Comments:
          17 pages, 3 figures

- **What's New**: 최근 강화 학습(RL)의 발전으로 더욱 복잡한 작업에서의 활용 가능성이 높아졌으나, 정식 안전 보장 부족이 안전-critical 환경에서의 응용에 제한이 되고 있습니다. 본 논문은 안전 필터를 활용하여 RL 정책을 보강하는 일반적인 접근 방식을 분석합니다. 연구진은 안전성과 성능 간의 상충관계가 내재화되어 있지 않음을 증명하며, 보다 관대한 안전 필터를 적용해도 비대칭 성능 저하가 발생하지 않는다는 새로운 이론적 결과를 제공합니다.

- **Technical Details**: 이 논문은 안전-critical Markov 결정 프로세스(SC-MDP)를 통해 RL 안전성을 형식화했습니다. 이 과정에서 주목해야 할 점은 모든 조치가 안전한 효과를 가져올 수 있도록 설계된 필터링된 MDP를 정의하는 것입니다. 주요 정리에서는 필터링된 MDP에서의 학습이 안전하게 이루어지고, RL의 전통적인 수렴 특성이 필터링된 MDP에 적용될 수 있음을 보여주며, 필터를 통한 최적 정책이 SC-MDP 내 가장 안전한 정책과 동일한 성능을 달성함을 명시합니다.

- **Performance Highlights**: 안전 체계의 이론을 Safety Gymnasium 벤치마크에서 검증함으로써 다양한 작업과 제약 조건에서 성과를 확인했습니다. 실험 결과, 훈련 과정에서 안전 위반이 전혀 발생하지 않았으며, 최종 성과는 비필터 기준선을 초과하거나 일치했습니다. 이러한 결과는 안전 필터링이 학습 효율성을 개선하는 동시에 성능 최적화와의 완전한 분리를 가능하게 한다는 점을 부각시킵니다.
...
### MEG-GPT: A transformer-based foundation model for magnetoencephalography data (https://arxiv.org/abs/2510.18080)
- **What's New**: MEG-GPT는 대규모 MEG 데이터에서 복잡한 시공간(dynamic spatiotemporal) 패턴을 학습하는 transformer 기반의 foundation model입니다. 이는 시간 주의(time-attention) 메커니즘과 다음 시간 점 예측(next time-point prediction)을 활용합니다. 또한, 연속 MEG 데이터를 위한 새로운 데이터 기반(token-based) 토크나이저(tokeniser)가 도입되어 높은 시계열 해상도를 유지하면서 데이터를 변환합니다.

- **Technical Details**: MEG-GPT는 대규모 MEG 데이터 세트(Cam-CAN)로부터 추출한 뇌 영역의 시간 경과(tokenized time-courses)가 훈련에 사용됩니다. 이 모델은 전이적 사건(transient events)과 개체 간 변동성(population variability)을 포함한 현실적(spatial-spectral) 신호를 생성할 수 있습니다. 기존의 분석 방법론이 시간 및 공간 평균화를 사용해 정보 손실을 초래한 반면, MEG-GPT는 이를 방지하여 그렇게 구축되었습니다.

- **Performance Highlights**: MEG-GPT는 전통적인 기본선(baseline) 방법과 비교하여 주제 간(zero-shot) 일반화 정확도를 크게 향상시켰습니다. 예를 들어, 주제 간 정확도는 0.41에서 0.49로, 그리고 세션 간 정확도는 0.54에서 0.59로 개선되었습니다. 또한, 이 모델은 소규모 레이블 데이터셋에서 효율적으로 세부 조정(fine-tuning)할 수 있어, 교차 주제 디코딩(cross-subject decoding) 시나리오에서도 성능을 높입니다.



### Batch Distillation Data for Developing Machine Learning Anomaly Detection Methods (https://arxiv.org/abs/2510.18075)
- **What's New**: 본 연구는 화학 공정에서의 이상 탐지(anomaly detection, AD) 발전을 위한 실험실 규모의 배치 증류 공장을 설정하고 조작하여, 다양한 운전 조건 및 혼합물에서 총 119개의 실험을 실시하여 포괄적인 실험 데이터베이스를 생성하였다. 이를 통해 고급 기계 학습 기반의 AD 방법을 훈련시키기 위한 데이터를 제공하고 있으며, 이 데이터베이스는 센서 및 액추에이터의 시계열 데이터를 포함하고 있다. 또한, 온라인 벤치탑 NMR 분광법 및 비디오, 오디오 기록을 통해 수집된 비전통적인 데이터 소스도 포함되어 있다.

- **Technical Details**: 연구에서 사용된 배치 증류 설비는 Iludest LM 2/S 유리 플랜트를 수정한 것으로, 두 개의 열전달 장치와 세 개의 구조화된 실험실 포장으로 구성되어 있다. 이 증류기와 리보일러는 다양한 센서와 제어 장비를 갖추고 있으며, 센서, 관측, 샘플, 액추에이터(SOSA) 온톨로지에 따라 분류되어 있다. 실험의 모든 과정과 발생된 이상에 대한 정보가 포함된 구조화된 데이터베이스가 생성되었고, 연구자들은 이를 통해 ML 기반 AD 방법론을 발전시키고자 한다.

- **Performance Highlights**: 이 새로운 데이터베이스는 ML 기반 AD 방법의 개발을 위한 토대를 마련하며, 이상 발생의 원인에 대한 정보를 포함하여 해석 가능하고 설명 가능한 ML 접근법 및 이상 완화 방법의 개발을 가능하게 한다. 연구자들은 실험 데이터를 공개적으로 제공함으로써 ML 기반의 AD 발전을 위한 협력을 촉진하고 있으며, 향후 관련 데이터는 추가적으로 확장 및 다양화될 예정이다.
...
Comments:
          Accepted at the 39th Conference on Neural Information Processing Systems (NeurIPS 2025) Workshop: Data on the Brain & Mind

- **What's New**: 이 논문은 신경 활동 예측(neural activity forecasting) 분야에서 프로바빌리스틱 딥러닝 모델(probalistic deep learning models)을 체계적으로 평가한 내용을 다룹니다. 특히, 마우스 피질에서 기록된 자발적인 신경 활동을 대상으로 하여, 총 8개의 모델을 고전적 통계 모델(statistical models) 및 베이스라인(baseline) 방법들과 비교했습니다. 연구 결과, 여러 딥러닝 모델이 전통적인 접근 방식보다 일관되게 우수한 성능을 보여주었고, 최상위 모델은 1.5초 미래까지 유용한 예측을 생성할 수 있음을 입증했습니다.

- **Technical Details**: 저자들은 12개의 단변량 확률적 예측 모델과 2개의 베이스라인 방법을 사용하여 자발적인 신경 활동을 평가했습니다. 이 데이터는 35Hz에서 와이드 필드 칼슘 이미징(widefield calcium imaging)을 통해 5개의 실험 세션에서 수집되었습니다. 모델에는 고전적 통계 모델뿐만 아니라, DeepAR, DLinear, Temporal Fusion Transformer, PatchTST, Time-series Dense Encoder, WaveNet과 같은 다양한 딥러닝 기반 모델이 포함되어 있으며, 또한 최신 타임 시리즈 파운데이션 모델인 Chronos와 Moirai도 평가되었습니다.

- **Performance Highlights**: 결과는 딥러닝 모델들이 고전적 접근 방식에 비해 자발적 신경 활동 예측에서 일관되게 더 좋은 성능을 보였음을 보여줍니다. 특히, 최상의 예측 모델은 1.5초 미래에 대한 유익한 예측을 제공하며, 이는 신경 활동의 내재된 시간 구조(intrinsic temporal structure)를 탐구하고 향후 제어 응용(application) 가능성을 열어줍니다. 이 연구 결과는 신경 기반 조절 시스템의 개발 및 치료적 뇌 자극 패러다임의 발전에 기여할 것으로 기대됩니다.



### Attention-Guided Deep Adversarial Temporal Subspace Clustering (A-DATSC) Model for multivariate spatiotemporal data (https://arxiv.org/abs/2510.18004)
Comments:
          9 pages, under review submitted to ICLR 2025

- **What's New**: 이 논문에서는 깊은 서브스페이스 클러스터링(Deep Subspace Clustering) 모델로서 A-DATSC(Attention-Guided Deep Adversarial Temporal Subspace Clustering)를 제안합니다. 이 모델은 데이터의 시공간적 복잡성을 다루기 위해 심층 서브스페이스 클러스터링 생성기와 품질 검증 이분기를 결합하여 클러스터링 품질을 감독합니다. A-DATSC는 기존 방법들이 겪고 있는 제약 사항들을 해결하며, 특히 4D 시공간 데이터에 대한 적용성을 강조합니다.

- **Technical Details**: A-DATSC 모델은 U-Net 아키텍처에서 영감을 받은 깊은 오토인코더를 포함하고 있습니다. 이 생성기는 TimeDistributed ConvLSTM2D 층을 쌓아 올려 공간적 및 시간적 무결성을 유지합니다. 그래프 주의(transformer) 기반의 자기 표현 네트워크는 지역 공간 관계와 글로벌 의존성을 캡처하고, 짧고 긴 거리의 상관성을 모두 모델링합니다.

- **Performance Highlights**: 세 가지 실제 다변량 시공간 데이터셋에서 수행한 실험 결과, A-DATSC는 최신 깊은 서브스페이스 클러스터링 모델과 비교하여 훨씬 우수한 클러스터링 성능을 달성했습니다. 이 모델은 시공간적 패턴을 포착하고 클러스터링의 품질을 향상시키는 데 크게 기여함으로써, 기존의 방법들이 가지던 한계를 극복합니다.
...
### Shock-Aware Physics-Guided Fusion-DeepONet Operator for Rarefied Micro-Nozzle Flows (https://arxiv.org/abs/2510.17887)
- **What's New**: 이 논문에서는 희귀한 미세 노즐 흐름을 위한 빠르고 정밀한 surrogate 모델을 구축하기 위한 통합된 물리적 정보 기반 딥러닝 프레임워크를 소개합니다. 이 프레임워크는 파라미터 의존성을 포착하기 위한 Fusion DeepONet 연산자 학습 아키텍처와 충격 정렬 좌표계를 포함하는 물리 유도 피처 공간, 고밀도 영역을 강조하는 2단계 커리큘럼 전략으로 구성됩니다. 이러한 혁신적인 접근법을 통해 미세 노즐 흐름의 복잡성을 잘 처리할 수 있도록 설계되었습니다.

- **Technical Details**: 이 프레임워크는 얇은 진공 상태와 함께 발생하는 복잡한 흐름 현상을 정확하게 모델링하는 것을 목표로 합니다. 특히, Knudsen 수(Kn)가 흐름의 다양한 상태를 나타내곤 하며, 충격파의 생성 및 장소와 강도에 민감한 노즐 압력 비율(NPR)과 같은 변수를 포착할 수 있습니다. DeepONet 아키텍처는 입력 파라미터로부터 해 해결함수를 예측하는 연산자 학습 문제로 접근하여, 이론적으로 정당화된 구조를 가지고 있습니다.

- **Performance Highlights**: 이 연구에서 제안한 프레임워크는 기존의 DSMC 방법에 비해 월등히 높은 정확도를 유지하면서도 계산 속도를 크게 향상시킬 수 있습니다. 또한, 기존의 PINNs 방식에서 발생할 수 있는 데이터 부족 문제에도 효과적으로 대응하고, 흐름의 비선형 특성을 잘 대표할 수 있는 능력을 보입니다. 최종적으로 이 연구는 미세 노즐 설계와 최적화를 위한 효율적인 시뮬레이션 방법론과 고급 기능 근사 모델의 활용 가능성을 보여줍니다.
...
### From Noise to Laws: Regularized Time-Series Forecasting via Denoised Dynamic Graphs (https://arxiv.org/abs/2510.17817)
- **What's New**: 이번 논문에서는 PRISM이라는 새로운 모델을 제안합니다. PRISM은 스코어 기반의 확산(Score-based diffusion) 전처리를 통해 입력 시퀀스의 노이즈를 제거하고 로컬 패턴을 잘 포착합니다. 또한, 데이터 기반으로 발전하는 동적 그래프를 구성하여 시계열 간 상호작용을 효과적으로 모델링하며, 물리적 규정을 적용하여 예측의 해석 가능성을 증대시킵니다.

- **Technical Details**: PRISM은 (a) 스코어 기반의 확산을 활용하여 노이즈를 억제한 후 특징을 추출하며, (b) 시간에 따라 변화하는 데이터 기반의 동적 그래프를 구성하여 Bidirectional message passing을 가능하게 합니다. (c) 물리적 규정을 통해 훈련 과정에서 도메인 독립적이고 해석 가능한 제약을 추가하였으며, 이를 통해 예측 정확성과 안정성을 높였습니다.

- **Performance Highlights**: PRISM은 여섯 개의 표준 벤치마크에서 일관된 SOTA(State Of The Art) 성능을 달성하였으며, MSE(Mean Squared Error) 및 MAE(Mean Absolute Error)에서 강력한 개선을 보였습니다. 또한, 이론적으로도 모델의 안정성과 효과를 입증하였으며, 이전 Transformer, GNN, 또는 확산 모델의 한계를 극복하고 있습니다.
...
### SO(3)-invariant PCA with application to molecular data (https://arxiv.org/abs/2510.18827)
- **What's New**: 본 논문에서는 주성분 분석(Principal Component Analysis, PCA)을 3차원 체적 데이터(3D volumetric datasets)에 적용하는 새로운 방법을 제시합니다. 기존 PCA는 고차원 데이터의 차원 축소와 잡음 제거에 유용하지만, 데이터의 방향성이 불확실한 경우 처리하기 어려운 점이 있었습니다. 이 논문은 SO(3)-불변(BGInvariance) PCA를 개발하여 명시적인 데이터 보강 없이 모든 회전을 암묵적으로 고려할 수 있도록 합니다.

- **Technical Details**: 논문에서는 구형 조화 함수(ball harmonics)와 Wigner D-행렬의 선형 조합을 사용하여 3D 데이터에 대한 표현을 효율적으로 구성하는 방법을 설명합니다. 특히, SO(3)-불변 공분산 행렬(covariance matrix)을 계산하여 이를 바탕으로 주성분을 추출합니다. 이 과정은 고차원 공간에서의 차원 축소를 목표로 하며, 처리 복잡성을 크게 감소시킵니다.

- **Performance Highlights**: 개발된 방법은 실제 분자 데이터셋(molecular datasets)에 적용되어 그 효과iveness를 입증하였습니다. 특이하게, 이 방법은 고차원의 큰 구조를 재구성하는 데 필요한 새로운 가능성을 열어줍니다. 단일 입자 크라이오 전자현미경(single-particle cryo-electron microscopy)과 같은 구조 생물학적 응용 분야에서의 강력한 성능을 기대할 수 있습니다.
...
### A Frequentist Statistical Introduction to Variational Inference, Autoencoders, and Diffusion Models (https://arxiv.org/abs/2510.18777)
Comments:
          This is an introduction paper. 28 pages, 2 figures

- **What's New**: 본 논문은 Variational Inference (VI)와 Variational Autoencoders (VAEs), Denoising Diffusion Models (DDMs)에 대한 이해를 증진시키기 위해 전통적인 Frequentist 관점에서 접근합니다. 이 논문은 기존의 통계학적 접근이 아닌 Frequentist 원칙에 기반한 기술을 소개함으로써 VAEs와 DDMs의 개념을 통계학자들에게 더 쉽게 접근할 수 있도록 합니다. 또한, Classical Expectation-Maximization (EM) 알고리즘을 기반으로 VI의 이론을 전개하며, 이는 현대 생성 AI 모델과 고전적 통계적 추론을 연결하는 가교 역할을 합니다.

- **Technical Details**: 논문은 첫 번째 섹션에서 Frequentist 잠재변수 모델의 기초를 설명하고, 이와 관련하여 Expectation-Maximization (EM) 알고리즘을 리뷰합니다. EM 알고리즘의 두 가지 주요 변형인 Monte Carlo EM (MCEM) 알고리즘과 정규화된 Q-function에 중점을 두어 VI로의 전환을 자연스럽게 이끌어냅니다. 다음으로 SEction 3에서는 EM 알고리즘의 E-step의 근사방법으로서 VI를 소개하고 증거 하한 (ELBO)을 정규화된 Q-function의 변형으로 설명합니다.

- **Performance Highlights**: 결국 논문은 Denoising Diffusion Model (DDM)을 이론적으로 수립하면서 이 프레임워크의 심층적이고 계층적 확장을 다룹니다. DDM은 생성과정에서의 포워드(variational) 및 리버스(generative) 프로세스를 포함하며, 이의 실용적 성공을 위한 핵심 요소로서 단순화된 노이즈 예측 객체를 도출합니다. 논문에서 제시하는 이러한 접근법은 VI가 대규모 딥러닝 모델에 어떻게 효과적으로 적용될 수 있는지를 보여줍니다.



### Analyse comparative d'algorithmes de restauration en architecture dépliée pour des signaux chromatographiques parcimonieux (https://arxiv.org/abs/2510.18760)
Comments:
          4 pages, in French, GRETSI Symposium on Signal and Image Processing, Strasbourg, France, August 2025

- **What's New**: 이 연구는 데이터 복원(data restoration) 기술의 발전을 다루고 있으며, 기존의 반복 최적화(iterative optimization) 방법과 최근의 심층 학습(deep learning) 기법의 조화를 보여줍니다. 특히, 물리-화학적 신호 특징에 맞춰서 세 가지 아키텍처를 비교하는 연구를 진행하였습니다. 이를 통해 예측된 신호의 해석성과 실용성을 높이는 방향으로 진행되고 있습니다.

- **Technical Details**: 연구에서는 비대칭(spesymmetric) 형태의 피크(pique)를 가진 합성 신호 모델을 사용하여 시뮬레이션하였습니다. 이 모델은 Gauss의 잡음(additive gaussian noise)과 주변의 흐림 효과(blur)를 포함하여, 파라미터화된 모델을 통해 발생하는 피크의 특성을 분석합니다. 이 과정에서 2000개의 샘플 및 여러 가지 피크와 잡음 파라미터를 조정하여 총 7개의 데이터베이스를 테스트하였습니다.

- **Performance Highlights**: 성능 평가에서는 전통적인 MSE(Mean Squared Error)와 SNR(Signal-to-Noise Ratio) 지표 외에, HAL(Height-Area-Location) 지표를 도입하여 보다 신뢰성 있는 평가를 시도했습니다. 이 지표들은 복원된 신호가 화학적으로 유의미하게 전송되도록 보여주며, 복원 품질이 높은 알고리즘을 비교 분석합니다. 결과적으로 이 연구는 복원된 데이터가 화학적 특성을 잘 반영한다는 것을 입증하며, Pytorch로 작성된 도구 상자를 공개하여 재현성을 보장합니다.



### Symbolic Emulators for Cosmology: Accelerating Cosmological Analyses Without Sacrificing Precision (https://arxiv.org/abs/2510.18749)
Comments:
          22 pages, 6 figures. Invited contribution for the Royal Society Philosophical Transactions A special issue "Symbolic regression in the physical sciences"

- **What's New**: 이 논문은 우주론(cosmology)에서 심볼릭 에뮬레이터(symbolic emulator)를 확장하여 현재 우주론 분석에 필요한 넓은 매개변수 범위를 커버할 수 있도록 발전시켰습니다. 이전의 심볼릭 에뮬레이터들은 비교적 좁은 범위에 국한되었으나, 본 연구에서는 정확도를 유지하면서(0.001%, 0.05% 이내) 보다 넓은 범위를 커버하기 위해 하이퍼지오메트릭 함수( hypergeometric function)에 대한 근사를 도입했습니다.

- **Technical Details**: 저자는 ΛCDM 모형의 매개변수로 현재 바리온 및 총 물질의 밀도(Ωb, Ωm)와 허블 상수(H0) 등을 사용하여 심볼릭 에뮬레이터의 범위를 확장했습니다. 이 연구는 유전 알고리즘(genetic programming)을 기반으로 한 심볼릭 회귀(symbolic regression)를 활용하여 분석 모델을 개발하며, 이는 자동화된 방식으로 분석할 수 있는 혁신적인 방법입니다. 이러한 모델은 고차원 샘플링 알고리즘의 효율성을 개선할 수 있는 특성을 가지고 있습니다.

- **Performance Highlights**: 이 심볼릭 에뮬레이터는 다크 에너지 탐사(Dark Energy Survey)와 유사한 3x2pt 분석에 통합되었고, 그 결과 기존의 수치적 방법으로 얻은 우주론적 제약과 일관된 결과를 보여주었습니다. 또한 이 에뮬레이터는 속도와 메모리 사용량에서 현저한 개선을 제공하여 스케일 수 있는 가능성을 입증하였습니다. 저자들은 이 접근법이 미래의 우주론 분석에 매우 유용할 것임을 강조하고 있습니다.



### Diffusion Buffer for Online Generative Speech Enhancemen (https://arxiv.org/abs/2510.18744)
- **What's New**: 이 논문은 온라인 음성 향상(Online Speech Enhancement) 분야에서 새로운 접근 방식인 Diffusion Buffer를 소개합니다. 이 모델은 한 번의 신경망 호출만으로 입력 신호 프레임을 향상시킬 수 있도록 설계되었으며, 이는 소비자용 GPU에서도 온라인 방식으로 작동할 수 있습니다. 특히, Diffusion Buffer는 물리적 시간과 확산 시간 단계를 정렬하여 과거 프레임의 노이즈를 점진적으로 제거합니다.

- **Technical Details**: Diffusion Buffer는 고유한 신경망 아키텍처인 2D convolutional UNet을 사용하여 Enhance된 출력을 생성합니다. 이 모델의 핵심은 데이터 예측 손실(Data Prediction loss)을 적용하여 알고리즘 지연 시간과 품질 간의 유연한 조정을 가능하게 한다는 점입니다. 이를 통해 알고리즘 지연 시간이 320-960ms에서 32-176ms로 줄어들고 정확도가 향상되었습니다.

- **Performance Highlights**: 이 연구는 Diffusion Buffer가 이전의 예측 기반 모델보다 새로운 노이즈 음성 데이터를 처리하는 데 더 우수한 성능을 발휘함을 보여줍니다. 논문에서 제시된 실험 결과는 Diffusion Buffer의 개선 사항이 성능을 향상시킬 뿐만 아니라, 낮은 사양의 GPU에서도 원활하게 온라인으로 작동되는 것을 확인했습니다. 이는 음성 통화 및 실시간 상호작용 플랫폼에 적합한 솔루션이 될 것입니다.
...
### Differentially Private E-Values (https://arxiv.org/abs/2510.18654)
- **What's New**: 이 논문에서는 민감한 데이터를 포함하는 실제 응용 프로그램에서 E-value의 안전한 사용을 보장하기 위한 새로운 프레임워크를 제안합니다. 제안된 방법은 비공식적인 E-value를 차별적 개인 정보 보호(Differential Privacy, DP)를 만족하는 E-value로 변환합니다. 본 연구는 또한 통계적 유효성을 유지하면서 개인 정보 보호를 보장하는 새로운 바이어스 곱셈 노이즈 메커니즘을 개발하였습니다.

- **Technical Details**: 차별적 개인 정보 보호(DP) 기술은 데이터셋의 정보 공개에 따른 개인 정보 손실을 제어하기 위한 프레임워크입니다. 본 연구에서는 기본적인 DP 메커니즘이 통계적으로 유효한 E-value를 생성하는 데 충분하지 않음을 보이고, 이를 해결하기 위해 새로운 바이어스 곱셈 노이즈 메커니즘을 도입합니다. 따라서 비공식적인 E-value의 전환을 통해 개인 정보 보호와 통계적 유효성을 동시에 달성할 수 있는 방법을 제공합니다.

- **Performance Highlights**: 우리는 당사의 방법을 통해 세 가지 실제 작업에서 효과를 입증하였습니다: 당뇨병 유병률에 대한 E-value 기반 신뢰 구간, 모델의 연속 위험 모니터링, 피싱 공격 탐지를 위한 E-형식 예측입니다. 실험 결과, 제안된 방법은 기존의 비공식 E-value와 유사한 통계적 파워를 유지하면서도 철저한 개인 정보 보호를 보장합니다. 이는 다양한 E-value 기반 추론 절차에 쉽게 적용할 수 있는 가능성을 열어줍니다.
...
### A Compositional Paradigm for Foundation Models: Towards Smarter Robotic Agents (https://arxiv.org/abs/2510.18608)
- **What's New**: 이번 연구는 Foundation Models (FMs)의 발전을 기반으로, Continual Learning (CL)과 Compositionality 원칙을 적용하여 더 유연하고 효율적인 AI 솔루션을 개발하는 것을 제안합니다. FMs는 강력한 성능을 보이지만, 실시간 변화하는 환경에 적응하는 데에는 어려움을 겪고 있습니다. 이 연구는 이러한 문제를 해결하기 위해 model architectures의 개선을 시도하고 있습니다.

- **Technical Details**: Transformer 기반 모델이 다양한 작업에 적용되고 있으며, 특히 로봇 제어 분야에서도 큰 성과를 보이고 있습니다. 연구자들은 Open X-Embodiment 데이터셋을 활용하여 22종의 로봇에서 정보를 통합하고 다양한 작업을 수행하기 위한 벤치마크와 시뮬레이터를 개발했습니다. 특히 Vision Language Action (VLA) 모델이 로봇의 성능을 향상시키는 데 기여하고 있습니다.

- **Performance Highlights**: 연구의 초기 실험 결과는 이미지 분류 및 로봇 조작 작업에서 높은 성능을 보여주고 있습니다. 특정도에서 강화된 모델들은 기존의 복잡한 모델들보다 더 낮은 계산 비용으로 더욱 높은 성공률을 달성했습니다. 이러한 결과는 간단한 아키텍처와 적은 계산 자원으로도 효과적인 능력을 가져올 수 있음을 시사합니다.



### Channel-Aware Vector Quantization for Robust Semantic Communication on Discrete Channels (https://arxiv.org/abs/2510.18604)
Comments:
          12 pages, 8 figures

- **What's New**: 이번 논문에서는 최신 디지털 통신 인프라와의 호환성을 강화하는 채널 인식 벡터 양자화(CAVQ) 알고리즘을 도입하여 의미 기반 통신의 한계를 극복했습니다. 제안된 VQJSCC 프레임워크는 의미적 특징을 변조 별자리( constellation symbols)로 매핑하고, 채널 전이 확률을 통합하여 양자화 과정에서 더욱 최적화된 대응을 구현합니다. 이를 통해 데이터 통신에서 발생하는 디지털 클리프 효과를 줄이고, 다양한 변조 방안에서 우수한 재구성 품질을 달성합니다.

- **Technical Details**: 제안된 VQJSCC 프레임워크는 이산 메모리 없는 채널(DMC) 상에서 세 가지 주요 특징을 중심으로 설계되었습니다. 첫째, 코드북 간 조정 메커니즘을 통해 코드북 순서와 변조 순서 간의 불일치를 처리하는 멀티 코드북 알고리즘을 도입합니다. 둘째, CAVQ는 전송 오류를 최소화하기 위해 특정 손실 함수를 설계하여, 오류 확률이 높은 코드워드를 의미 공간에 적절히 집결시킵니다. 이러한 구조는 기존의 비트 기반 통신 방식을 새로운 패러다임으로 변화시키는 데 기여합니다.

- **Performance Highlights**: 실험 결과, VQJSCC는 디지털 통신에서 흔히 발생하는 클리프 효과를 효과적으로 완화하고, 최첨단 조합 코딩 변조 방식보다도 피크 신호 대 잡음 비율(PSNR) 및 계산 효율성 측면에서 우수한 성능을 보여줍니다. 제안된 알고리즘은 혼동되기 쉬운 변조 기호를 의미적으로 유사한 코드워드와 연관짓는 독특한 코드북 분포를 생성합니다. 이러한 접근 방식은 모든 코드워드가 효과적으로 정보 표현에 기여하여, 원주율보다 더 높은 소스 엔트로피를 달성하게 합니다.
...
- **What's New**: 이번 연구에서는 전통적인 유전체 연관 연구에서의 여러 테스트 수정(multiple testing correction) 접근이 통계적 유의미성(statistical significance)에 의존하는 대신, 낮은 표본력(underpowered cohorts)으로 인해 발생하는 한계를 극복하기 위한 새로운 방법론을 제시합니다. 특히, TCGA 유방암 데이터 세트를 사용하여 기존 방법론의 실패를 확인하고, 대안적 접근 방식의 필요성을 강조합니다.

- **Technical Details**: 이 연구에서 개발된 다섯 가지 기준의 계산적 프레임워크는 원인 추론(causal inference) 기법인 역확률 가중치(inverse probability weighting) 및 이중 견고성 추정(doubly robust estimation)과 생물학적 검증(orthogonal biological validation)을 통합합니다. 이를 통해 유방암 분석에서 표본의 낮은 사건 발생률이 초래하는 통계적 제한을 해결하게 됩니다.

- **Performance Highlights**: 연구 결과는 표본의 낮은 힘에 의해 전통적인 Cox+FDR 방법이 0개의 유전자를 발견하지 못한 반면, 제안된 프레임워크가 RYR2와 KMT2C 유전자를 식별하는 데 성공했음을 보여줍니다. RYR2는 명목상의 유의미성에도 불구하고(false positive) 실제 암 기능을 지니지 않은 것이 확인되었으며, KMT2C는 한계적인 유의미성을 보였지만 강한 생물학적 증거를 바탕으로 추가 검증이 필요한 복합 후보로 제시되었습니다.



### Interval Prediction of Annual Average Daily Traffic on Local Roads via Quantile Random Forest with High-Dimensional Spatial Data (https://arxiv.org/abs/2510.18548)
- **What's New**: 이 연구는 연간 평균 일일 교통량(AADT) 예측을 위한 새로운 접근법인 인터벌 예측(interval prediction)을 도입하여 예측 불확실성을 명확히 정량화합니다. 기존의 모델들이 단일 예측값만 산출하는 것과는 달리, 제안한 방식은 최소값과 최대값으로 경계가 설정된 AADT 예측 구간을 제공합니다. 이는 데이터의 누락이 발생하기 쉬운 마이너 도로에 대한 보다 장치적인 통찰을 제공합니다.

- **Technical Details**: 연구는 양적 랜덤 포레스트(Quantile Random Forest, QRF) 모델을 주성분 분석(Principal Component Analysis, PCA)과 결합하여 AADT 예측 구간을 구성합니다. 모델은 888개의 지리적 특성, 접근성 지표, 인프라 특성 등을 포함하는 고차원 특성 공간에서 훈련됩니다. Bayesian 최적화 및 확률적 검색을 통한 하이퍼파라미터 조정을 실시하며, Winkler Score를 사용해 인터벌 성능을 평가합니다.

- **Performance Highlights**: 제안된 방법은 2021년 잉글랜드와 웨일즈의 2,000개 이상의 마이너 도로 데이터에 대해 88.22%의 인터벌 커버리지 확률을 달성했습니다. 평균 너비는 0.23으로, Winkler Score는 7,468.47에 달하는 성과를 보였습니다. 이로 인해 AADT 추정의 정확성과 해석 가능성이 향상되어 보다 견고한 교통 계획을 지원합니다.
...
- **What's New**: 이번 연구는 신경 데이터의 이질성을 활용한 자가 지도 학습(self-supervised learning, SSL) 방법인 POYO-SSL을 제안합니다. 이는 예측 가능한 신경 세포에 대한 미리 학습(pretraining)을 수행한 후, 예측 불가능한 세포를 위한 미세 조정(fine-tuning)을 실시하여 신경 데이터에서의 일관된 활동 패턴을 식별하는 데 도움을 줍니다. 연구 결과 POYO-SSL을 통해 Allen Brain Observatory 데이터셋에서 약 12-13%의 성능 개선을 달성하였으며, 모델 크기 증가에 따른 안정적으로 빠른 확장을 보여주고 있습니다.

- **Technical Details**: POYO-SSL은 고차 통계(예: 왜도(skewness) 및 첨도(kurtosis)를 활용하여 신경 세포의 예측 가능성을 평가합니다. 이 연구는 주요 억제 신경 세포와 조절성 신경 세포를 포함하는 예측 가능한 하위 집합을 선별하여 신경 데이터의 이질성을 자산으로 활용하는 데이터 다이어트를 제안합니다. 이러한 방식은 신경 세포에서 일반적인 패턴을 학습하는 데 필요한 통계적 규칙성을 극대화합니다.

- **Performance Highlights**: 연구를 통해 POYO-SSL이 기존의 동등 모델들보다 모델 용량 증가 시 안정성을 유지하며, 신경 회로 동역학의 기초 모델을 형성하는 가능성을 보여줍니다. 이 방법은 전통적인 방법들이 성능의 정체점에 도달하는 데 비해 데이터 효율성을 1.98배 향상시키고, 높은 정확도의 영상 재구성을 직접 신경 기록으로부터 가능하게 합니다.
...
### A machine learning approach to automation and uncertainty evaluation for self-validating thermocouples (https://arxiv.org/abs/2510.18411)
Comments:
          6 pages, 7 figures. TEMPERATURE: ITS MEASUREMENT AND CONTROL IN SCIENCE AND INDUSTRY, VOLUME 9: Proceedings of the Tenth International Temperature Symposium 3-7 April 2023 Anaheim, USA

- **What's New**: 이번 연구에서는 전통적인 thermocouple의 보정 드리프트 문제를 해결하기 위해 새로운 기계 학습 접근법을 소개합니다. 이 방법은 thermocouple의 measurement junction 근처에 위치한 phase-change cell을 활용하여 melting point를 자동으로 인식하고 측정합니다. 이를 통해 melting plateau의 특징적인 형태를 식별하고, 사람이 개입하지 않고도 melting 시작점을 정량화할 수 있게 됩니다.

- **Technical Details**: 이 연구에서 사용하는 고정점(fixed point)은 특정 melting temperature를 가진 금속 주괴를 포함한 도가니다. thermocouple은 이 고정점이 melting temperature를 지난 경우에 발생하는 plateau를 통해 교정이 가능합니다. 기계 학습 알고리즘은 melting plateau의 형상을 인식하고, 이를 기반으로 melting이 시작되는 시점을 정확히 계산합니다.

- **Performance Highlights**: CCPI Europe에서 제공한 테스트 데이터에 대한 결과는 melting plateau 탐지에서 100%의 정확도를 보여주었습니다. 또한, 보정 드리프트 예측에 대한 cross-validated R2 값이 0.99에 달하여 매우 높은 정확성을 입증했습니다.
...
- **What's New**: 이 연구에서는 전극 심전도 (electrocardiography) 분야에 맞춰 공개된 모델을 세밀하게 조정하여 의료에서의 활용 가능성을 탐구했습니다. 기존의 일반 목적 LLM과 비교하여 도메인 적합화 (domain adaptation)와 정확한 평가 방법론을 구현하여 데이터를 활용했습니다.

- **Technical Details**: 연구는 Llama 3.1 모델을 도메인 특화된 문헌에 맞춰 미세 조정 (finetuning)하고, 정보 검색을 보강하는 RAG 접근법과 결합하여 성능을 비교했습니다. 다양한 평가 방법론, 예를 들어 객관식 평가와 자동 텍스트 유사성 메트릭을 활용하여 모델의 성능을 측정했습니다.

- **Performance Highlights**: 결과적으로, 미세 조정된 Llama 3.1 70B 모델이 여러 평가에서 우수한 성능을 보였으며, Claude 3.7보다 약간 뒤처진 성과를 냈습니다. 그러나 복잡한 쿼리에 대한 평가에서는 Claude 3.7와 RAG가 더 높은 점수를 받았습니다. 이러한 결과는 도메인 특화된 모델이 상용 모델과 견주어 경쟁력을 갖추고 있음을 보여줍니다.



### Parametrising the Inhomogeneity Inducing Capacity of a Training Set, and its Impact on Supervised Learning (https://arxiv.org/abs/2510.18332)
- **What's New**: 본 논문에서는 학습 데이터셋의 속성을 매개변수화(parametrisation)하는 새로운 개념인 ``inhomogeneity parameter''를 소개합니다. 이 매개변수는 변수 쌍 간의 관계를 모델링하는 기능에 대해 비균일한 상관 구조가 필요함을 나타냅니다. 기존의 데이터 비정상성(non-stationarity) 개념과는 달리, 이 매개변수는 데이터셋의 특정 성질을 명확히 드러내게 됩니다.

- **Technical Details**: 우리는 이 ``inhomogeneity parameter''를 작은 데이터셋부터 대규모 데이터셋까지 쉽게 계산할 수 있음을 보여 주며, 여러 공개 데이터셋을 통해 이러한 계산을 시연하였습니다. 또한, 확률적 Gaussian Process 기반 학습 접근법을 사용하여 비제로(non-zero) inhomogeneity parameter를 가진 훈련 세트는 비정상적인(non-stationary) 모델링 프로세스를 요구하는 것을 증명합니다. 이로 인해 모델링 대상 함수에 대한 처리 과정이 필수적임을 강조합니다.

- **Performance Highlights**: 실세계의 다변량(multivariate) 함수를 학습한 결과, 테스트 입력에서 예측의 품질과 신뢰성이 훈련 데이터의 inhomogeneity parameter에 의해 영향을 받는다는 것을 확인했습니다. 이는 데이터셋의 특성에 따라 모델의 예측 능력이 달라질 수 있음을 시사합니다. 이 연구는 데이터 분석(data analysis) 및 모델링 과정에서 inhomogeneity parameter의 중요성을 강조합니다.
...
- **What's New**: 본 연구는 heterogeneous (이기종) HPC 아키텍처 내의 GPU 대규모 트레이스를 효율적으로 처리할 수 있는 엔드 투 엔드 병렬 성능 분석 프레임워크를 제안합니다. 이 프레임워크는 트레이스 데이터를 동시에 처리하고 causal graph (인과 그래프) 방법 및 병렬 조정 차트를 활용하여 실행 흐름 내의 성능 변동성 및 의존성을 노출합니다. 실험 결과, 이 시스템이 여러 대규모 GPU 트레이스의 분석에서 67%의 확장성을 개선함을 보여주었습니다.

- **Technical Details**: GPU 컴퓨팅에서 커널은 수천 개의 스레드에 의해 병렬로 실행되는 사용자 정의 함수입니다. 각각의 커널 실행은 스레드가 조직되고 스케줄되는 방식으로 그리드-블록 구성에 따라 정의되며, 성능 지표는 리소스 경쟁 및 하드웨어 변동성으로 인해 변동할 수 있습니다. 저자는 GPU 트레이스를 분석하기 위해 고정 길이의 시간 세그먼트인 bins로 나누고, 각 bin의 성능 통계를 계산하여 변동성을 정량화합니다.

- **Performance Highlights**: 제안된 프레임워크는 여러 GPU 트레이스 데이터셋을 동시에 분석하여 거의 선형적인 확장성을 달성합니다. 각각의 데이터셋은 독립적인 처리 경로를 가지고 있어 하나의 트레이스에서의 작업이 다른 트레이스 분석에 지연을 초래하지 않도록 설계되었습니다. 이러한 효율적인 수평 확장 기능 덕분에 증가하는 분석 부하에 대한 성능을 유지할 수 있습니다.
...
### The Bias-Variance Tradeoff in Data-Driven Optimization: A Local Misspecification Perspectiv (https://arxiv.org/abs/2510.18215)
- **What's New**: 이 논문에서는 데이터 기반 확률적 최적화(data-driven stochastic optimization) 방식 중에서 Sample Average Approximation(SAA), Estimation-then-Optimize(ETO), Integrated Estimation-Optimization(IEO) 방법의 상대적 성능을 보다 세분화하여 분석합니다. 특히, 이 연구는 국소적 오류(local misspecification) 설정 하에서 랜덤 변수의 분포를 모델링하여 오류가 적은 경우를 다룹니다. 통계학적 접근 방식인 contiguity theory를 활용하여 SAA, IEO, ETO 간의 bias-variance tradeoff를 실증적으로 보여줍니다.

- **Technical Details**: 논문에서는 데이터 노이즈 수준에 비해 모델 오류의 심각성이 큰 경우 SAA가 IEO보다 우수하다는 점을 확인했습니다. 반면, 오류 수준이 낮을 때는 그 반대의 성능 순서가 나타났습니다. SAA는 bias 측면에서 가장 우수하고 ETO는 variance에서 가장 좋은 성능을 발휘하는 등, 이 세 가지 방법 사이의 tradeoff가 드러납니다.

- **Performance Highlights**: 주요 발견 중 하나는 모델 오류가 데이터 노이즈와 유사한 경우, 즉 'balanced case'에서 각 접근 방식의 성능 순서가 상이해진다는 것입니다. 이 논문은 모델 오류 방향에 따라 bias가 어떻게 영향을 받는지를 명확하게 설명하는 공식도 제공합니다. 또한, ETO와 IEO의 비정상성을 확인하고, SAA가 bias 측면에서 항상 가장 우수하다는 점을 강조하며, 모델 오류가 최종 결정을 미치는 영향을 해명합니다.
...
- **What's New**: 이 논문은 기존의 Retrieval-Augmented Generation (RAG) 방법의 한계를 극복하기 위해 RESCUE라는 새로운 프레임워크를 제안합니다. RESCUE는 하이브리드 지식 기반 구축 방법과 계층적 다면적 검색 방식을 통합하여 보안 코드 생성을 위한 효율성을 높입니다. 이를 통해 LLM의 코드 생성 능력을 유지하면서도 보안 문제가 있는 코드를 방지하는 데 도움을 줍니다.

- **Technical Details**: 이 프레임워크는 LLM 보조 클러스터-요약(distillation) 방법과 정적 프로그램 슬라이스(program slicing) 기법을 결합한 하이브리드 지식 기반 구축 접근 방식을 사용합니다. 또한, 보안 요구사항을 충족하기 위해 API 패턴, 취약성 원인 분석 및 코드와 같은 세 가지 보안 측면을 분석하는 계층적 다면적 검색 전략을 설계합니다. 이러한 방법론을 통해 관련성 높은 정보를 정확하게 검색하여 코드 생성에 기여할 수 있습니다.

- **Performance Highlights**: RESCUE는 네 가지 벤치마크를 사용하여 평가되었으며, 기존의 다섯 가지 최첨단 보안 코드 생성 방법과 비교된 결과, 평균 4.8%의 SecurePass@1 지표 향상을 기록하며 새로운 성능 기준을 수립했습니다. 또한, RESCUE는 원래 모델이 생성하는 기능적으로 올바른 코드의 98.7%를 유지합니다. 이러한 결과는 RESCUE의 각 구성 요소의 효과성을 엄격히 검증한 심층 분석 및 제거 연구(ablation study)를 통해 확인되었습니다.
...
Comments:
          Paper submitted to ICASSP2026

- **What's New**: 이번 논문에서는 오디오 녹음에서 피아노 다이나믹을 추정하는 효율적인 멀티태스킹 네트워크를 제안합니다. 이 네트워크는 동적 수준, 변화 지점, 비트 및 다운비트를 공동으로 예측하며, 공유된 잠재 표현(latent representation)에서 작동합니다. Bark_scale 특정의 음량을 입력 특성으로 사용하여 모델 크기를 저감시킴으로써 긴 시퀀스 입력을 가능하게 하였습니다. 이러한 접근 방식은 피아노 다이나믹 추정의 새로운 기준을 설정하며, 대규모의 자원 효율적인 음악 표현 분석에 기여할 수 있는 도구를 제공합니다.

- **Technical Details**: 피아노 다이나믹의 추정을 위해 Bark-scale 특정의 음량(BSSL)이라는 입력 특성을 사용합니다. BSSL은 인간의 청각 인식을 기반으로 하며, 다양한 음역대에서의 적용 가능성이 입증되었습니다. 이 네트워크는 다중 시간 해상도를 요구하는 동적 추정 문제를 해결하기 위해 다중 스케일 네트워크를 채택했습니다. 최종적으로 다중 게이트 전문가 혼합(MMoE) 모듈이 도입되어 각 작업마다 적합한 특성 조합을 생성합니다.

- **Performance Highlights**: 제안된 모델은 공개된 MazurkaBL 데이터셋에서 모든 작업에 대해 최신 기술인 SOTA(State-of-the-Art) 결과를 달성했습니다. 이 모델은 비트 정보는 제공되지만 다이나믹 마크가 부족한 악보를 풍부하게 하는 데 주요한 응용으로 활용될 수 있습니다. 또한, 자동 음악 표기 시스템(AMT)과의 결합을 통해 기존 악보 없이도 완전하게 작동할 수 있는 유연한 도구임을 입증하였습니다.
...
- **What's New**: 최근 자동으로 개별 특성(covariates)을 기반으로 하는 치료 결정 정책을 학습하는 시스템에 대한 관심이 급증하고 있습니다. 이 논문은 현재의 정책 평가 방법들이 겪고 있는 'winner’s curse' 문제를 해결하기 위해 정책 최적화 과정을 수정하는 새로운 전략인 inference-aware policy optimization을 제안합니다. 이 방법은 예측된 목표 값뿐만 아니라 학습된 정책이 기존 정책보다 통계적으로 유의미하게 우수할 가능성도 고려하여 정책 최적화를 수행합니다.

- **Technical Details**: Inference-aware policy optimization 문제를 형식화하고 이를 해결하기 위한 수학적 특성을 개발합니다. 특정한 정책 평가 방법론으로는 inverse propensity weighting (IPW)을 사용하며, 이는 학습된 정책이 수집된 데이터 하에서 재weighted된 시험 결과를 평가하는 방법입니다. 우리가 목표로 하는 것은 예상되는 목표 값과 통계적 유의미성 간의 균형을 맞추는 정책을 설계하는 것입니다.

- **Performance Highlights**: 정책 최적화 알고리즘을 시뮬레이션을 통해 구현하고, 기존의 접근법과 비교하여 파레토 경계(Pareto frontier)를 효과적으로 계산할 수 있음을 보여줍니다. 특히, 우리는 학습된 정책이 기존 정책에 비해 작은 개선을 달성하면서도 통계적 유의미성을 유지할 수 있도록 최적화되었습니다. 이러한 접근법은 causal model의 확장을 기반으로 하여, 치료 효과의 평균(treatment effect)을 비교하는 정책 평가 문제를 다루고 있습니다.
...
### Generalization Below the Edge of Stability: The Role of Data Geometry (https://arxiv.org/abs/2510.18120)
Comments:
          Under Review. Comments welcome!

- **What's New**: 본 논문은 과잉 파라미터화된 신경망에서 일반화(generalization)에 대한 이론적 탐구를 다루고 있습니다. 특히, 데이터 기하학(data geometry)이 이러한 암묵적 편향(implicit bias)을 어떻게 제어하는지 설명하며, 두 층 구조의 ReLU 네트워크에서 안정성의 경계(edge of stability) 아래에서 훈련될 때의 결과를 제시합니다. 논문은 데이터 차원에 따라 적응하는 일반화 경계를 도출하고, 확률 질량이 구(구체)의 경계에 집중될수록 성능이 저하된다는 것을 보여줍니다.

- **Technical Details**: 이 연구는 훈련 데이터의 기하학적 속성에 따라 신경망의 효과적인 용량(effective capacity)이 결정된다는 주장을 합니다. 과잉 파라미터화된 두 층 ReLU 네트워크의 학습에서, 데이터의 저차원(n-dimensional) 특성에 대한 적응한 일반화 경계를 증명하였고, 특히 데이터가 저차원적으로 분포할 때의 일반화 성능이 향상된다는 결과를 도출했습니다. 또한, 데이터가 구의 경계에 집중될 경우 일반화 성능이 저하됨을 이론적으로 분석하였습니다.

- **Performance Highlights**: 고차원 데이터의 경우, 데이터의 재분포에 따른 일반화 성능 변화 양상을 분석하였고, 이론적으로 도출된 경계값은 기존의 경험적 결과와 일치함을 보였습니다. 높은 shatterability(파괴 가능성)이 데이터 기하학에 따라 암묵적 정규화(implicit regularization)를 강화한다고 주장하며, 이는 학습 전략에 직접적인 영향을 미친다고 강조합니다. 이를 통해 gradient descent의 학습 방식과 일반화 간의 관계를 비로소 명확히 이해할 수 있는 기초를 마련하였습니다.



### PrivaDE: Privacy-preserving Data Evaluation for Blockchain-based Data Marketplaces (https://arxiv.org/abs/2510.18109)
- **What's New**: 이 논문에서는 프라이버시 보호를 위한 데이터 유틸리티 점수 생성 및 선택의 암호화 프로토콜인 PrivaDE를 제시합니다. PrivaDE는 블록체인을 활용하여 악의적인 보안 보장과 강력한 프라이버시 보호를 보장하는 실용적 디자인을 가지고 있습니다. 이 접근법은 기존의 데이터 평가 프로토콜보다 진화된 방법으로, 기계 학습 데이터셋의 효율적인 평가를 가능하게 합니다.

- **Technical Details**: PrivaDE는 데이터 소유자(DO)와 모델 소유자(MO)가 데이터 유틸리티 점수 ϕ(M, D)를 공동으로 계산할 수 있도록 설계된 보안 프로토콜입니다. 이 프로토콜은 수시로 발생할 수 있는 악의적인 행위를 고려하여 개발되었으며, 최소한의 신뢰할 수 있는 제3자(TTP) 개입만을 요구합니다. 암호학적 기술과 다양한 효율성 기법을 통합하여 실제적 수준의 실행 시간을 달성하고, 평가 점수는 데이터셋의 대표성을 가지도록 설계되었습니다.

- **Performance Highlights**: PrivaDE는 온라인에서 15분 이내의 시간 안에 데이터 평가를 수행할 수 있는 효율적인 성능을 보여줍니다. 연구 결과, 다양한 모델 아키텍처와 실제 데이터셋에 대해 강력하고 효율적인 데이터 유틸리티 점수 산출이 가능함을 입증했습니다. 이 시스템은 공정하고 자동화된 데이터 마켓플레이스의 기초를 제공하며, 특히 시뮬레이션 기반 보안 증명과 실험적 검증을 통해 실제적인 응용 가능성을 강조합니다.
...
### Arbitrated Indirect Treatment Comparisons (https://arxiv.org/abs/2510.18071)
- **What's New**: 이 논문에서는 Health Technology Assessment (HTA)에서의 Matching-Adjusted Indirect Comparison (MAIC) 및 새로운 방법인 arbitrated indirect treatment comparisons를 소개합니다. MAIC의 역설(‘MAIC paradox’)이란 동일한 데이터 분석을 통해 서로 다른 결론에 도달하는 현상으로, 이는 주로 각 스폰서가 서로 다른 인구집단을 목표로 설정하기 때문입니다. 이러한 불일치를 해결하기 위해, 제안된 방법들은 공통 목표 인구집단을 정의하고 그에 따른 치료 효과를 추정하는 데 중점을 두고 있습니다.

- **Technical Details**: 이 논문은 MAIC와 Simulated Treatment Comparison (STC) 같은 Population-Adjusted Indirect Comparisons (PAIC)의 이론적 배경을 설명합니다. 두 가지 PAIC 유형인 anchored PAIC와 unanchored PAIC를 다루며, 특히 anchored PAIC에 초점을 맞추고 있습니다. MAIC 역설을 해결하기 위해 새로운 방법론이 제안되며, 이를 위해 중재자가 필요하다는 점이 강조됩니다. 이 중재자는 AC 및 BC 시험에서 공통적인 인구집단(overlap population)을 기준으로 MAIC를 수행할 것을 보장하는 역할을 합니다.

- **Performance Highlights**: 제안된 방법은 치료 효과를 제로로 나타내는 일관된 결과를 도출하여 MAIC 역설 문제를 해결합니다. Jiang et al. (2025)의 예제에서, 두 스폰서가 서로 다른 치료 효과를 주장하는 상황이 명시되어 있으나, 중재자가 개입하여 공통 목표 인구집단을 기준으로 할 경우 치료 효과에 차이가 없음을 입증합니다. 따라서, 이 논문은 하위 인구를 올바르게 정의함으로써 MAIC를 더욱 신뢰할 수 있는 방법으로 발전시킬 수 있는 가능성을 보여줍니다.



### Fast Agnostic Learners in the Plan (https://arxiv.org/abs/2510.18057)
- **What's New**: 이번 논문에서는 기하학적 개념 클래스에 대한 agnostic learning의 계산 효율성을 조사합니다. 특히, 삼각형 및 다각형과 같은 기본 기하학적 구조에 대한 학습 알고리즘의 샘플 복잡도(sample complexity)와 시간 복잡도(time complexity)를 최적화하였습니다. 새로운 알고리즘은 기존의 방법보다 뛰어난 성능을 보여, eg. 삼각형의 경우 시간 복잡도가 𝜏(𝜖<sup>-6</sup>)로 개선되었습니다.

- **Technical Details**: 연구는 2차원 평면에서 삼각형, 볼록 다각형(2~5변), 그리고 정사각형 내 볼록 집합에 대한 proper agnostic learner를 개발했습니다. 특히, 볼록 다각형의 경우 및 일반적인 볼록 집합의 agnostic learning에 대한 이론적 한계를 탐구하며, VC 차원이 무한인 개념 클래스에 대해서는 어떤 학습이 불가능하다는 점도 강조합니다. 이를 통해 넓은 범위의 응용 분야—예를 들면 이미지 인식 및 신호 처리 등—에서 이론적 기초를 제공하고자 합니다.

- **Performance Highlights**: 제안된 proper agnostic learner는 샘플 복잡도와 시간 복잡도 간의 연관성을 명확하게 보여줍니다. 새로운 알고리즘은 각 개념 클래스에 대해 이전의 시간 복잡도 최고치를 크게 개선하였고, 특이하게도 agnostic learning에서 샘플 복잡도와 시간 복잡도 간의 정체성에 대한 근본적인 질문을 제기합니다. 또한, 우리의 연구 결과는 기하학적 데이터의 처리 및 분류에 대한 효율적인 해결책을 제공합니다.
...
### Transformer Redesign for Late Fusion of Audio-Text Features on Ultra-Low-Power Edge Hardwar (https://arxiv.org/abs/2510.18036)
- **What's New**: 이 연구는 경량 AI 아키텍처 개발을 통해 사람의 의사소통을 분석하여 사회적 문제를 탐지하는 데 필요한 감정 인식 시스템의 경량화에 중점을 둡니다. 기존의 클라우드 기반 인프라에 의존하지 않고도 저비용의 마이크로컨트롤러에서 실시간으로 감정을 인식할 수 있는 시스템을 제시합니다. 이 시스템은 키워드 탐지와 스펙트로그램 음향 분석을 결합하여 미래의 폭력 탐지 작업으로 쉽게 확장할 수 있습니다.

- **Technical Details**: 제안된 시스템은 Edge TPU에 최적화된 후처리 아키텍처를 사용하여 음향 및 언어적 특징을 결합합니다. 이 시스템은 양자화된 변환기 기반 음향 모델과 DSResNet-SE 네트워크에서 동결된 키워드 임베딩을 통합하여 1.8MB 메모리 예산 내에서 21-23ms의 지연 시간으로 실시간 추론을 가능하게 합니다. MicroFrontend와 MLTK를 사용하여 훈련과 배포 간의 스펙트로그램 정렬을 보장합니다.

- **Performance Highlights**: 이 연구는 IEMOCAP 샘플에 대해 unimodal 기준선 대비 6.3%의 매크로 F1 점수를 개선하여 실질적인 성능 향상을 입증합니다. 제안된 다중 수준 융합 모델은 감정 표현과 언어적 지표를 효과적으로 감지할 수 있으며, 이러한 민감성은 실시간 감정 추론에 중요한 역할을 합니다. 이 시스템은 저비용 자원 제한이 있는 MCU에서도 정밀한 감정 추론을 가능하게 합니다.
...
### QINNs: Quantum-Informed Neural Networks (https://arxiv.org/abs/2510.17984)
Comments:
          20 pages, 9 figures

- **What's New**: 이번 논문에서는 classical 모델에 양자 정보 개념을 통합하는 quantum-informed neural networks (QINNs)를 제안합니다. 특히, 논문에서는 각 입자를 qubit으로 인코딩하고 Quantum Fisher Information Matrix (QFIM)을 사용하여 입자 상관관계를 요약하는 실질적인 접근 방식을 다룹니다. 이 방법은 jet tagging에 적용되어 모델의 표현력과 유연성을 증대시키며, QFIM이 물리적 기대와 일치하는 전혀 다른 패턴을 드러냅니다.

- **Technical Details**: QINN은 event 정보를 양자 상태로 인코딩하고, QFIM과 같은 양자 관측 가능성을 통해 classical 모델에서 양자 구조를 주입하는 방법으로 정의됩니다. 여기서 QFIM은 입자의 동적 민감도를 측정하는 기본적인 수단으로, 1P1Q (one-particle-one-qubit) 표현을 통해 입자 간의 상관관계를 자연스럽게 인코딩합니다. 이 논문에서는 QFIM을 사용하여 여러 jet 구성 요소의 동적인 정보를 나타내는 데 집중하고, 이를 통해 jet의 물리적 속성을 해석할 수 있는 방법을 제시합니다.

- **Performance Highlights**: QFIM을 이용한 jet tagging은 기존의 방법보다 개선된 분별력을 보여주며, 효과적으로 QCD와 top jets를 구분하는 데 기여합니다. 특히, QINN의 결과는 딥러닝 모델의 훈련을 가속화하고 최적화 역학을 개선하는 데 중요한 역할을 합니다. 이 연구는 QINN이 jet 물리학에 양자 정보를 통합하는 실용적이면서도 스케일이 가능한 경로를 제공한다는 것을 입증합니다.
...
### Learning Time-Varying Graphs from Incomplete Graph Signals (https://arxiv.org/abs/2510.17903)
- **What's New**: 이 논문은 부분적으로 관찰된 그래프 신호에서 시간에 따라 변하는 네트워크 토폴로지를 공동으로 추론하고 누락된 데이터를 보완하는 문제를 다룹니다. 이를 위해 그래프 라플라시안 행렬의 시퀀스를 회복하는 통합 비선형 최적화 프레임워크를 제안합니다. 전통적인 방법과 달리, 우리의 접근법은 그래프와 신호 도메인 사이에 쌍방향 정보 흐름을 용이하게 하여 특히 높은 결측 데이터 비율에서 향상된 강인성을 제공합니다.

- **Technical Details**: 제안된 방법에서는 시퀀스의 라플라시안에 대해 융합 라쏘(fused-lasso) 유형의 정규화를 도입하여 노이즈로 인한 잘못된 변화를 방지하면서 점진적인 톱올로지 변화를 허용합니다. 이 비선형 최적화 문제를 해결하기 위해 효율적인 교차 방향 방법(ADMM) 알고리즘을 개발하였으며, 해당 구조를 활용하여 두 가지 서브문제를 폐쇄형 해(closd-form solutions)로 해결하게 됩니다.

- **Performance Highlights**: 폭넓은 수치 실험 결과는 제안된 방법이 최첨단 기준선을 능가하며, 빠른 수렴 속도와 함께 그래프 학습과 신호 회복의 공동 정확성을 크게 향상시킬 수 있음을 보여줍니다. 이 논문은 제안된 ADMM 기반 알고리즘의 수렴 보장을 수립하고, 그래프 추정기가 일관성을 가지기 위해 필요한 최소 샘플 크기를 정확히 묘사하는 비비대칭 통계적 분석을 제공합니다.



### TritonRL: Training LLMs to Think and Code Triton Without Cheating (https://arxiv.org/abs/2510.17891)
- **What's New**: 이 논문에서는 TritonRL이라는 Triton 커널 생성을 위한 도메인 특화된 대형 언어 모델(LLM)을 소개합니다. TritonRL은 새로운 훈련 프레임워크를 통해 견고하고 자동화된 커널 합성을 가능하게 하며, 이를 통해 기존 모듈을 대체할 수 있는 높은 품질의 Triton 커널을 생성할 수 있습니다. 또한, 보상 해킹(reward hacking) 문제를 효과적으로 해결하였으며, 다양한 검증 기법을 통해 우수한 성능을 달성하였습니다.

- **Technical Details**: TritonRL의 훈련 파이프라인은 두 가지 주요 단계로 구성되어 있습니다. 첫 단계에서는 감독 하에 세밀화된 훈련(Supervised Fine-Tuning, SFT)을 통해 Triton 구문과 커널 최적화 능력을 학습합니다. 두 번째 단계는 강화 학습(Reinforcement Learning, RL)을 도입하여 견고하고 검증 가능한 보상을 통해 모델을 더욱 정교하게 조정합니다. 이 과정에서 계층적 보상 분해(Hierarchical Reward Decomposition)와 데이터 혼합(Data Mixing) 기법을 활용하여 코드 품질과 일반화 능력을 향상시킵니다.

- **Performance Highlights**: TritonRL의 실험 결과는 KernelBench에서 다루어지며, 기존의 Triton 특화 모델들을 초월하는 정확성과 속도를 기록하고 있습니다. 이 모델은 8B 규모의 LLM으로, 특히 Triton 프로그래밍에 특화되어 있어 시간 절약과 성능 개선을 동시에 이룹니다. 대규모 모델(32B 이상)에 비해 효율성과 정확성을 획기적으로 개선했으며, 이는 상업적 및 오픈 소스 모델 간의 진정한 성능 차이를 명확히 보여주고 있습니다.



### Graphical model for tensor factorization by sparse sampling (https://arxiv.org/abs/2510.17886)
Comments:
          75 pages, 26 figures

- **What's New**: 이 논문에서는 텐서( tensor ) 구성 요소의 희소 측정(sparse measurements)을 기반으로 한 텐서 분해(tensor factorization)를 고찰합니다. 무작위 그래프(random graph)로 상호작용의 기초가 되는 그래프를 설계하여, 상당한 데이터가 누락된 상황에서도 유용성을 극대화합니다. 이는 사회 네트워크 서비스에서 많이 사용되는 추천 시스템( recommendation systems )에 적용 가능성이 큽니다.

- **Technical Details**: 저자들은 고차원 제한(high dimensional limit)에서의 통계적 추론(statistical inference)을 고려하며, 이를 '조밀한 제한(dense limit)'이라고 명명합니다. 조밀한 제한에서는 그래프가 크고 조밀하지만 완전 연결되지 않은 경우에 대한 이론적 통찰을 제공합니다. 메시지 전파(message-passing) 알고리즘을 구축하고, 베이즈 최적 교사-학생 설정(Bayes optimal teacher-student setting)에서 테스트를 수행합니다.

- **Performance Highlights**: 조밀한 제한에서의 성능을 검토하기 위해 복제 이론(replica theory)을 개발하였으며, 이 이론은 조밀한 제한에서 정확해집니다. 이를 통해 통계적 추론의 성능(performance of statistical inference)을 분석하며, 제안된 알고리즘의 효과를 평가합니다.
...
### Three-dimensional inversion of gravity data using implicit neural representations (https://arxiv.org/abs/2510.17876)
Comments:
          10 Pages, 5 figures

- **What's New**: 이번 논문에서는 중력 데이터의 3차원 역해를 위해 암시적 신경 표현(implicit neural representation, INR)을 사용하는 새로운 기계 학습 접근 방식을 제안합니다. 이 방법은 전통적인 메쉬나 불연속화 없이, 공간 좌표를 연속적인 밀도 필드로 매핑하는 깊은 신경망을 직접 훈련시킵니다. 포지셔널 인코딩(positional encoding)은 네트워크가 날카로운 대비와 짧은 파장 특징을 더 잘 포착할 수 있도록 합니다. 이 연구는 합성 예제를 통해 방사형 근원 및 경사 블록 모델을 사용하여 설정된 접근 방식을 검증하였습니다.

- **Technical Details**: 제안된 접근 방식은 중력 변화를 층상으로 표현하며, 전통적인 기법의 잔여물(grid-aligned artifacts) 문제를 해결합니다. 중력 데이터로부터 밀도 필드를 역해하는 프레임워크는 물리 기반의 전방 모델 손실(physics-based forward-model loss)을 통해 직접 학습됩니다. 각 공간 좌표는 포지셔널 인코딩을 통해 네트워크에 입력되어, 주파수의 왜곡을 줄이고 날카로운 계면을 잘 표현하도록 합니다. 이렇게 구성된 모델은 깊이 무게 부여 없이도 지질학적으로 신뢰할 수 있는 구조 및 경계선을 재구성하는 데 필요한 유연성을 제공합니다.

- **Performance Highlights**: 신경망을 통해 복원된 밀도 필드는 기존의 격자 기반 접근 방식에 비해 적은 매개변수 수로 더 정밀성을 제공합니다. 또한, 제안된 접근 방식은 깊이 기반의 정규화 없이는 수정된 지질학적 대칭성을 가능하게 하여 대규모의 해양 및 육상의 지구 물리학적 역해를 유도할 수 있습니다. 이 연구 결과는 암시적 표현이 대규모 지구 물리학적 문제에서 유용할 수 있음을 보여주며, 다른 지구 물리적 방법 및 멀티피직스 역해에 일반화 될 가능성이 있습니다.



### Mixed Monotonicity Reachability Analysis of Neural ODE: A Trade-Off Between Tightness and Efficiency (https://arxiv.org/abs/2510.17859)
Comments:
          27 pages, 11 figures

- **What's New**: 이 논문에서는 Neural Ordinary Differential Equations (neural ODE)이 복잡한 동적 시스템의 행동을 모델링하는 데 강력하지만, 그 검증이 제한된 도달 분석 도구로 인해 도전적이라는 점을 소개합니다. 새로운 인터벌 기반 접근법을 통해 continuous-time mixed monotonicity 기법을 활용하여 neural ODE 도달 집합에 대한 과잉 근사(over-approximation)를 계산하는 방법을 제안합니다. 이 방법은 초기 집합의 기하학적 구조를 이용하여 효율적인 경계 전파를 보장합니다.

- **Technical Details**: 제안된 방법은 neural ODE의 동적 시스템을 혼합 단조 시스템(mixed monotone system)으로 포함시켜, TIRA에서 단일 단계, 점진적, 경계 기반 접근법을 활용한 인터벌 기반 도달 가능성 분석(interval-based reachability analysis)을 구현합니다. CORA의 zonotopes 및 NNV2.0 star set 표현과 비교하여 사운드하고 계산 효율적인 과잉 근사를 제공합니다. 이 과정에서 효율성과 타이트함(tightness) 간의 균형을 이루며, 특히 고차원, 실시간 및 안전-critical 응용 분야에 적합합니다.

- **Performance Highlights**: 이 방법은 spiral system과 fixed-point attractor system의 두 가지 수치 예제를 통해 설명됩니다. 혼합 단조성을 neural ODE 도달 분석에 적용함으로써, 형태가 대칭적이고 기하학적으로 단순한 인터벌 박스(interval boxes)를 활용한 경량화된 형식 분석(lightweight formal analysis)을 가능하게 합니다. 이러한 새로운 접근 방식은 neural representation의 대칭성과 기하학적 일치를 기반으로 한 확장 가능한 검증을 위한 새로운 경로를 열어줍니다.
...
### Neural networks for neurocomputing circuits: a computational study of tolerance to noise and activation function non-uniformity when machine learning materials properties (https://arxiv.org/abs/2510.17849)
- **What's New**: 이 연구는 기계 학습(Machine Learning)과 관련된 고Throughput 및 저전력 소비 애플리케이션에서 사용하는 전용 아날로그 신경 컴퓨팅 회로에 대한 내용을 다룹니다. 특히, 신경망(Neural Networks) 구현 시 회로 소음(circuit noise) 및 뉴론 활성화 함수(Neuron Activation Function)의 비균일성 문제를 분석합니다. 소재 정보학(materials informatics) 애플리케이션을 통해 기계 학습의 적용 예를 제시합니다.

- **Technical Details**: 연구는 신경망의 구조와 훈련 방식에 따라 회로 소음 및 활성화 함수 불균일성이 미치는 영향을 정량적으로 분석합니다. 구체적으로, 반응성이 높은 기계 학습 문제로 페리 응축된 탄화수소의 형성 에너지와 가장 낮은 에너지 이성질체의 관계, 이중 페롭스카이트의 형성 에너지 및 밴드 갭을 다룹니다. 또한 QM9 데이터셋의 분자들의 제로포인트 진동 에너지도 포함됩니다.

- **Performance Highlights**: 신경망은 일반적으로 낮은 소음 감내력을 가지며, 소음 수준이 증가함에 따라 모델의 정확성이 빠르게 저하됩니다. 단일 은닉층 신경망과 최적보다 큰 크기를 가진 신경망이 다소 더 높은 소음 감내력을 보입니다. 특히, 과적합(overfitting)이 적은 모델이 더 높은 소음 감내성을 가지며, 활성화 함수의 불균일성 문제는 NAF의 실제 형태를 사용하여 신경망을 재훈련함으로써 완화될 수 있음을 입증합니다.
...
### Covariance Matrix Construction with Preprocessing-Based Spatial Sampling for Robust Adaptive Beamforming (https://arxiv.org/abs/2510.17823)
Comments:
          13 figures, 14 pages

- **What's New**: 본 연구는 제어 벡터(steering vector, SV) 추정 불일치를 해결하고 데이터 공분산(matrix) 복원 문제를 처리하기 위해 효율적이고 강인한 적응형 빔포밍(adaptive beamforming) 기법을 제안합니다. 특히, 간섭원(interfering sources)의 도착 방향(direction-of-arrival, DoA)을 이용 가능한 샘플을 통해 추정하고, 적응적으로 간섭 신호의 각도 영역을 계산합니다. 이와 함께, 기존의 잘 알려진 일반 선형 조합 알고리즘을 활용하여 사전 처리 기반 공간 샘플링(preprocessing-based spatial sampling, PPBSS)을 통해 간섭-잡음 공분산(interference-plus-noise covariance, IPNC) 행렬을 복원합니다.

- **Technical Details**: 연구에서 제안하는 방법은 사전 처리 기반 공간 샘플링을 통해 간섭원들의 각도 도착 방향을 초기 추정하고, 신호의 관심(signal-of-interest, SOI) 영역에 대응하는 공분산 행렬을 구성하여 SOI의 SV를 계산합니다. 제안된 방법의 핵심은 영행렬의 평균 제곱 오차(mean squared error, MSE)를 이용하여 보다 정확한 IPNC 행렬 복원을 시도하며, 이 과정에서 간섭 신호의 전력과 SV 추정을 피합니다. 이와 같은 접근 방식은 모델 오류에 대한 강인성을 향상시키는 동시에 복잡성을 줄여줍니다.

- **Performance Highlights**: 제안한 PPBSS 방법은 잘 알려진 기존 기법들과 비교하여 높은 신호 대 잡음 비(signal-to-interference-plus-noise ratio, SINR)를 기록하며, 다양한 불일치 시나리오에서 성능을 평가합니다. 이 연구는 동적으로 간섭 신호의 수와 불확실한 각도 영역을 계산할 수 있는 알고리즘을 통하여 실시간 적응이 가능하도록 했습니다. 또한, 전력 기반 알고리즘을 통해 SOI의 공분산 행렬의 주 고유값과 고유벡터를 효율적으로 계산하여 빔포밍 성능을 개선합니다.



### CLARAE: Clarity Preserving Reconstruction AutoEncoder for Denoising and Rhythm Classification of Intracardiac Electrograms (https://arxiv.org/abs/2510.17821)
- **What's New**: 본 논문에서는 CLARAE(CLArity-preserving Reconstruction AutoEncoder)라는 새로운 인코더-디코더 모델을 도입합니다. 이 모델은 고충실도 재구성과 64차원의 압축된 잠재 표현을 통해 심장 전기생리학에서 실시간 분석의 한계를 극복하고자 합니다. CLARAE는 파형 형태를 유지하고, 재구성 아티팩트를 완화하며, 해석 가능한 임베딩을 생성하는 세 가지 원칙을 기반으로 설계되었습니다.

- **Technical Details**: CLARAE는 필터링과 밀접한 관련이 있는 고차원 EGM 데이터를 다루는데 최적화되어 있으며, 다층 전극 방식의 진단 도구로부터 획득된 EGM 신호를 사용합니다. 이 모델은 495,731개의 EGM 세그먼트를 평가하여 파형 형태 유지 및 아티팩트 감소 등에서 우수한 성과를 나타냈습니다. 데이터는 스페인 마드리드의 다양한 환자들로부터 수집되었으며, 기존의 화이트박스 모델에 비해 더 나은 성능을 보였습니다.

- **Performance Highlights**: CLARAE는 모든 리듬 타입에서 0.97 이상의 F1 점수를 달성하여 리듬 분류 작업에서 뛰어난 성능을 입증했습니다. 또한, 노이즈 제거 작업에서도 일관되게 최상위 성능을 기록하며, 전개된 잠재 공간이 리듬에 따라 뚜렷하게 클러스터링되는 것을 확인하였습니다. 추가적으로, 웹 기반의 인터랙티브 애플리케이션을 통해 사용자들이 실시간으로 모델을 탐색하고 평가할 수 있는 플랫폼을 제공하여 접근성과 재현성을 향상시킵니다.



### Single-Snapshot Gridless 2D-DoA Estimation for UCAs: A Joint Optimization Approach (https://arxiv.org/abs/2510.17818)
- **What's New**: 이 논문은 단일 데이터 스냅샷에서의 균일 원형 배열(Uniform Circular Array, UCA)을 이용한 2차원(2D) 도착 방향(DOA) 추정 문제를 다룹니다. 기존의 그리드리스(gridless) 방법들이 높은 계산 비용이나 낮은 강인성으로 인해 실패하는 가운데, 우리는 데이터 충실성(data fidelity)과 변환 강인성(transformation robustness)을 통합한 새로운 최적화 프레임워크를 제안합니다. 이는 비정확한 증강 라그랑주 방법(Inexact Augmented Lagrangian Method, iALM)을 사용하여 해결되며, 이는 반정방형 프로그래밍(Semidefinite Programming, SDP)의 필요성을 완전히 회피합니다.

- **Technical Details**: UCA는 전체 360도 방위각 커버리지를 제공하기 때문에 2D-DOA 추정에 유리한 선택입니다. 본 연구에서는 KK개의 비상관(narrow-band) 신호가 저 멀리에서 UCA에 입사한다고 가정하고, 각 신호의 도착 각을 azimuth- elevation 쌍으로 정의합니다. 우리의 접근 방식은 복잡한 UCA 매니폴드를 간단한 분리 가능 구조의 가상 배열 매니폴드로 변환하는데 중점을 두고 있으며, 이를 위해 선형 변환 행렬(Linear Transformation Matrix) 𝐓를 사용합니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 iALM 프레임워크는 도전적인 단일 스냅샷 환경에서도 강력하고 높은 해상도의 2D-DOA 추정 결과를 제공합니다. 특히 제안된 방법은 낮은 신호 대 잡음비(low SNR) 상황에서도 우수한 정확성과 해상도를 달성하였음을 보여줍니다. 이 연구는 기존 기준 기술들과 비교하여 현저한 성능 향상을 입증하며, 실제 배열 신호 처리 애플리케이션에 대한 효과성을 설정합니다.



### Exploring Complexity Changes in Diseased ECG Signals for Enhanced Classification (https://arxiv.org/abs/2510.17810)
Comments:
          Version submitted to NODYCON 2025

- **What's New**: 이번 연구에서는 심장 질환과 관련된 ECG의 비선형성을 분석하여 질병 감지에 대한 기여를 탐구했습니다. PTB-XL 데이터셋을 활용하여, lead II ECG에서 비선형 지표와 교차 채널 데이터를 추출하고 이를 머신 러닝 모델에 통합했습니다. 이에 따라 질병 있는 사람과 건강한 사람의 ECG 데이터 간의 유의미한 차이를 발견하였습니다.

- **Technical Details**: 연구에 사용된 PTB-XL ECG 데이터베이스는 21,837개의 12리드 ECG 데이터를 포함하고 있으며, 다양한 복잡성 지표를 계산했습니다. 여기에는 Higuchi Fractal Dimension (HD), Approximate Entropy (ApEn) 등 비선형 지표와, Spearman 상관계수 및 상호정보와 같은 교차 시간 시리즈 지표가 포함됩니다. 이러한 지표는 건강한 그룹과 질병 그룹 간의 주요 차이를 파악하는 데 사용되었습니다.

- **Performance Highlights**: 연구 결과, 머신 러닝 모델에 복잡성 특성을 포함함으로써 분류 정확도가 크게 향상되었습니다. baseline 모델에서 0.86의 AUC에서 비선형 지표 포함 후 0.87로 증가하고, 교차 시간 시리즈 지표까지 포함했을 때 0.90에 도달했습니다. 이는 ECG의 복잡성 지표가 심장 질환 분류에 실질적인 기여를 할 수 있음을 보여줍니다.



### In-Process Monitoring of Gear Power Honing Using Vibration Signal Analysis and Machine Learning (https://arxiv.org/abs/2510.17809)
Comments:
          20 pages, 17 figures, 3 tables, 33 references

- **What's New**: 이 연구에서는 전통적인 품질 관리 전략이 가지는 한계를 극복하기 위해 새로운 데이터 기반 프레임워크를 제안합니다. 기존의 품질 관리가 공정 후 검사 및 통계적 공정 제어(Statistical Process Control, SPC)에 의존하는 반면, 본 연구는 가공 중 모니터링을 가능하게 합니다. 이를 통해 진동 신호 분석(vibration signal analysis)과 머신러닝(machine learning)을 사용하여 실시간으로 결함을 감지할 수 있는 방법을 제공합니다.

- **Technical Details**: 제안된 방법론은 가속도계(accelerometer)를 이용한 지속적인 데이터 수집과 시간-주파수 신호 분석(time-frequency signal analysis)을 포함합니다. 세 가지 서브스페이스 학습(subspace learning) 방법을 조사하며, 각각 (1) 차원 축소를 위한 주성분 분석(Principal Component Analysis, PCA), (2) PCA와 선형 판별 분석(Linear Discriminant Analysis, LDA)을 결합한 두 단계 프레임워크, (3) 정규화(Regularization)를 포함한 비상관 다중 선형 판별 분석(Uncorrelated Multilinear Discriminant Analysis, R-UMLDA) 방식을 사용하여 특징을 추출합니다. 이 특징들은 지지 벡터 기계(Support Vector Machine, SVM) 분류기에 입력되어 네 가지 기어 품질 범주를 예측하는 데 사용됩니다.

- **Performance Highlights**: 본 연구에서 제안한 프레임워크는 실제 산업 환경에서 기어 파워 호닝(power honing) 작업 중 수집된 실험 데이터를 바탕으로 높은 분류 정확도(100%에 달하는)를 달성하였습니다. 기어는 철저한 기하학적 검사와 조립된 기어박스의 테스트 벤치 결과를 통해 네 개의 서로 다른 품질 범주로 분류됩니다. 제안된 접근법은 프로세스 역학(process dynamics)과 상관관계가 있는 해석 가능한 스펙트럼 특징을 제공하여 실시간 모니터링 및 예측 유지보수 시스템에 쉽게 통합될 수 있는 장점을 가집니다.



### MEET-Sepsis: Multi-Endogenous-View Enhanced Time-Series Representation Learning for Early Sepsis Prediction (https://arxiv.org/abs/2510.15985)
Comments:
          Accepted to PRICAI 2025

- **What's New**: 이번 논문은 세균혈증(Sepsis) 예측을 위한 MEET-Sepsis라는 새로운 프레임워크를 제안합니다. 이 방법은 Multi-Endogenous-view Representation Enhancement (MERE) 메커니즘과 Cascaded Dual-convolution Time-series Attention (CDTA) 모듈을 결합하여 세균혈증의 초기 패턴을 효과적으로 포착합니다. 기존의 방법들보다 약 20%의 ICU 모니터링 시간만으로 경쟁력 있는 예측 정확도를 달성함으로써, 적시 개입의 중요성을 강조합니다.

- **Technical Details**: MEET-Sepsis는 세 가지 주요 모듈로 구성됩니다: MERE 모듈은 특징 수준의 합성을 통해 다중 내부 뷰를 생성하여 비선형 특징 결합 학습을 촉진합니다. CDTA 모듈은 다양한 크기의 커널을 사용하여 시간적 특징을 추출하며, 자가 주의 메커니즘을 통해 멀티-스케일 종속성을 포착합니다. 마지막으로 앙상블 예측 모듈은 특징 공간을 계층적으로 나누어 예측 정확도 및 해석력을 향상시킵니다.

- **Performance Highlights**: MEET-Sepsis는 XGBoost와 같은 기존 방법들과 비교하여 뛰어난 예측 성능을 보여줍니다. 이 모델은 초기 세균혈증 예측에서 특히 높은 정확도를 보이며, 의료 분야에서의 안전성을 높입니다. 또한, MERE 및 CDTA의 조합을 통해 예측의 신뢰성이 향상되어 조기 개입의 여지를 크게 증가시킵니다.

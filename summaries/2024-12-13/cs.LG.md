...
- **What's New**: 최근의 latent-space 모니터링 기술이 LLM 공격에 대한 방어수단으로 가능성을 보이고 있습니다. 이러한 방어 기법들은 유해한 활성화를 탐지하여 원치 않는 행동으로 이어지기 전에 위험 요소를 식별하려 합니다. 그러나 본 논문에서는 모델이 불완전한 latent 상태를 이용해 유해한 행동을 수행할 수 있는지에 대한 의문을 다루고 있습니다.

- **Technical Details**: 논문은 obfuscated activations (숨겨진 활성화)라는 개념을 소개하며, 이는 latent-space 모니터를 속이면서 원하지 않는 행동을 생성하는 활성화입니다. 다양한 기법이 존재하는데, sparse autoencoders, representation probing, latent OOD detection 등과 같은 기존의 방법들이 이러한 공격에 취약하다는 사실을 보여 줍니다. Obfuscation attacks는 여러 공격 벡터를 통해 불량한 행동을 유발하며, 이는 다양한 latent 공간에서 광범위하게 나타납니다.

- **Performance Highlights**: 저자들은 특정 모델 행동을 최적화하면서 latent-space 모니터를 속이는 obfuscation attacks의 사례 연구를 통해 유해성 모니터링, 백도어 탐지, sparse autoencoders에 대해 연구합니다. 특히, 복잡한 작업에서는 obfuscation이 모델 성능을 저하시킨다는 점을 강조하며, 이는 공격자의 목표와 inconspicuous activations 간의 트레이드오프가 존재함을 시사합니다. 따라서, 이 연구는 neural activations의 유연성을 부각시키며, 기존 방어 기법에 대한 도전 과제가 있음을 보여줍니다.
...
### Capturing the Temporal Dependence of Training Data Influenc (https://arxiv.org/abs/2412.09538)
Comments:
          Correspondence to Jiachen T. Wang and Ruoxi Jia

- **What's New**: 본 논문에서는 전통적인 데이터 영향 추정 방법은 훈련 데이터의 순서에 무관하다고 가정하지만, 현대의 훈련 패러다임은 이러한 가정을 위배하는 경우가 많다고 지적합니다. 특히, 모델의 최적화 궤적(optimization trajectory)에 따라 데이터의 영향력을 평가하는 새로운 개념인 trajectory-specific leave-one-out (LOO) 영향력을 제안합니다. 이를 통해 특정 훈련 단계에서 데이터 포인트를 제거했을 때의 영향을 정량화할 수 있는 방법을 제시합니다.

- **Technical Details**: 제안된 데이터 가치 임베딩(data value embedding) 기술은 훈련 데이터를 효율적으로 표현하는 방법으로, 이러한 임베딩을 통해 LOO 영향을 간단한 점곱(dot-product) 연산으로 근사할 수 있습니다. 이 과정에서, 자료의 순서와 모델이 훈련 중 어떻게 개선되는지를 반영하여, 복잡한 계산 없이 실시간으로 데이터 영향력을 평가할 수 있게 합니다. 특히, 같은 데이터 포인트의 값이 훈련 과정을 통해 어떻게 변하는지를 추적할 수 있는 영향 체크포인팅(influence checkpointing) 기법이 도입되었습니다.

- **Performance Highlights**: 데이터 가치 임베딩을 통해 모델 훈련의 동역학(dynamics)에 대한 새로운 통찰을 얻었습니다. 우리는 데이터의 영향력이 '높은 영향(high influence)', '낮은 영향(low influence)', '점진적으로 증가하는 영향'의 세 가지 양상으로 구분된 것을 확인했습니다. 특히, 초기와 후반의 높은 영향 구역에서만 데이터를 선택하는 온라인 데이터 선택 방법이 전체 훈련 기간 동안 수행하는 것과 유사한 성능 개선을 이룰 수 있음을 입증하였습니다.



### Early Detection of At-Risk Students Using Machine Learning (https://arxiv.org/abs/2412.09483)
- **What's New**: 이 연구는 인공지능 기법을 사용해 위험에 처한 학생들을 파악하는 새로운 접근법을 제시합니다. Fall 2023에 수집된 참여도(engagement), 인구통계(demographics), 성과(performance) 데이터를 기반으로 하여, 학생 유지 및 중퇴 문제를 해결하기 위한 시스템을 구축하고자 합니다. 전통적인 메트릭과 함께 간과되었던 행동적 요소를 포함해 교육 격차를 해소하고, 학생 성과를 향상시키는 것을 목표로 합니다.

- **Technical Details**: 연구는 데이터 전처리(pre-processing) 단계를 통해 목표 변수(target variable)를 설정하고 학생 정보를 익명화하며, 결측치(missing data)를 관리하고 가장 중요한 특성(features)을 식별합니다. 다양한 데이터 유형이 포함된 데이터셋을 가지고 이 연구는 Support Vector Machines (SVM), Naive Bayes, K-nearest neighbors (KNN), Decision Trees, Logistic Regression, Random Forest와 같은 여러 머신러닝 모델을 고려합니다. 이들 모델은 위험에 처한 학생들을 예측하고 학기 중 학생 성과가 가장 취약한 시점을 식별합니다.

- **Performance Highlights**: 모든 알고리즘이 위험에 처한 학생 예측에서 유의미한 결과를 도출하는 것으로 나타났습니다. 특히 Naive Bayes 모델이 전체적으로 가장 우수한 성능을 보였습니다. 모델의 신뢰성을 확보하기 위해 train test split와 k-fold cross-validation과 같은 검증 기법(validation techniques)을 사용할 예정입니다.



### Bayesian Optimization via Continual Variational Last Layer Training (https://arxiv.org/abs/2412.09477)
- **What's New**: 이 논문은 Bayesian 최적화(Bayesian Optimization, BO)에 대한 새로운 접근 방식을 제시합니다. Gaussian Process (GP)와 Bayesian Neural Networks (BNNs)의 강점을 결합한 Variational Bayesian Last Layer (VBLL) 신경망을 사용하여 복잡한 입력 상관관계를 가진 문제에서 높은 성능을 보여줍니다. 특히, BNNs가 일반적으로 성능이 좋지 않은 문제에서도 경쟁력 있는 성능을 발휘합니다.

- **Technical Details**: 기존 GP는 데이터 포인트 간의 유의미한 상관관계를 포착하기 위해 맞춤형 커널을 필요로 하며, 이러한 커널을 디자인하는 것이 복잡하고 비용이 많이 들 수 있습니다. 새로운 VBLL 모델은 효율적인 온라인 훈련 알고리즘을 개발하며, 조건화(conditioning)와 최적화(optimization)를 상호 연동하여 수행합니다. 이 모델은 GP와 BNN의 장점을 모두 활용할 수 있도록 설계되었습니다.

- **Performance Highlights**: VBLL 네트워크는 BNN과 GPs보다 복잡한 입력 상관관계를 가진 작업에서 유의미한 성능 향상을 보입니다. 특히, 여러 다른 기준선 모델을 초월하는 성능을 보여주며, 기존의 잘 조정된 GP와 동일한 수준의 성능을 유지하는 데 성공합니다. 이 연구 결과는 VBLL이 다양한 문제 유형에 적용 가능한 강력한 대안임을 강조합니다.



### A Novel Ensemble-Based Deep Learning Model with Explainable AI for Accurate Kidney Disease Diagnosis (https://arxiv.org/abs/2412.09472)
- **What's New**: 이번 연구에서는 만성 신장 질환(Chronic Kidney Disease, CKD)을 조기에 탐지하기 위한 최첨단 전이 학습(transfer learning) 모델의 활용을 다룹니다. 공공 데이터셋을 활용하여 여러 최신 모델의 성능을 평가하였으며, 이는 만성 신장 질환 진단의 접근을 혁신적으로 변화시킬 수 있는 가능성을 보여줍니다.

- **Technical Details**: 사용된 모델에는 EfficientNetV2, InceptionNetV2, MobileNetV2, 그리고 Vision Transformer(ViT)가 포함되어 있습니다. 각 모델의 성능은 세심하게 평가되었으며, MobileNetV2의 경우 90%를 초과하는 정확도와 ViT에서 91.5%의 정확도를 달성하였습니다. 또한, 개별 모델들을 앙상블(ensemble) 모델링 기법으로 통합하여 96%의 높은 정확도를 기록했습니다.

- **Performance Highlights**: 이 연구의 결과는 CKD의 조기 탐지에서 뛰어난 성능을 보이며, 결국 임상 결과(clinical outcomes)를 개선하는 데 크게 기여할 수 있습니다. 기계 학습(machine learning)이 복잡한 의료 문제를 해결하는 데 중대한 역할을 할 것임을 강조하며, 의료 분야에서의 활용 가능성을 제시합니다.



### Neural Network Symmetrisation in Concrete Settings (https://arxiv.org/abs/2412.09469)
- **What's New**: 최근 Cornish (2024)는 마르코프 카테고리의 추상 개념에서 신경망 심리화(neural network symmetrisation)에 대한 일반 이론을 제시했습니다. 이 연구는 결정론적 함수와 마르코프 커널의 심리화에 대한 구체적인 함의를 제공합니다. 특히 논문은 다양한 심리화 기법을 새로운 맥락에서 규명하며, 확률 신경망에 대한 기존의 연구를 확장하는 데 기여합니다.

- **Technical Details**: 신경망 f:X→Y가 특정 그룹의 작용에 대해 동형성(equivariant)을 보장하는 것이 유용합니다. 기존 문헌에서는 실패한 신경망을 동형성 있는 형태로 변환하는 다양한 심리화 기법을 고려해왔습니다. 예를 들어, Janossy pooling와 같은 접근 방식이 소개되었으며, Cornish는 이러한 기법들을 포함하여 새로운 개념을 제공하고 있습니다.

- **Performance Highlights**: Cornish의 연구 결과는 비단순형 신경망의 심리화에 대해 복잡한 상황을 한층 더 통일된 방식으로 다룰 수 있는 프레임워크를 제공합니다. 기존의 기법들은 이제 특별한 사례로 일관된 이론적 기반을 갖추게 되었으며, 이를 통해 심리화 절차가 명확해졌습니다. 이 논문은 마르코프 카테고리의 개념을 접목하여 확률적 사고를 보다 직관적이고 규범적으로 설명할 수 있는 기회를 제공합니다.
...
### Search Strategy Generation for Branch and Bound Using Genetic Programming (https://arxiv.org/abs/2412.09444)
Comments:
          Accepted at AAAI 2025

- **What's New**: 이 논문은 지능형 결정을 내리면서도 계산 비용을 최소화할 수 있는 새로운 기계 학습 접근법인 GP2S (Genetic Programming for Search Strategy)를 도입합니다. 기존의 신경망 기반 접근방식과 수작업으로 작성된 휴리스틱(heuristics)과 비교하여, GP2S는 보다 일관된 성능을 제공하는 것을 목표로 합니다. 이 방법은 B&B 탐색 전략을 자동으로 생성할 수 있는 기능을 갖추고 있습니다.

- **Technical Details**: GP2S는 노드 및 문제 데이터와 관련된 정보에 기초하여 노드 점수 함수(node scoring functions)를 정의합니다. 이후 이 점수에 따라 노드를 우선순위로 정렬하여 탐색하는 최적화된 휴리스틱을 구사합니다. 유전자 프로그래밍(genetic programming) 알고리즘을 사용하여 여러 SS 휴리스틱의 공간을 탐색하며, 훈련 세트에서 최고의 성능을 보이는 정책을 선택합니다.

- **Performance Highlights**: GP2S는 SCIP 솔버의 표준 방법과 비교했을 때, 훈련 세트와 유사한 인스턴스 및 더 큰 인스턴스에서 평균 11.3%의 속도 향상을 달성하면서 우수한 성과를 보입니다. 또한 MIPLIB 2017 데이터셋을 테스트했을 때, 10개 중 7개 경우에서 SCIP보다 뛰어난 성능을 기록했습니다. 이는 GP2S가 다양한 실험에서 적절한 해결책의 개수나 최적값 간의 간격에서 우위를 차지할 수 있음을 시사합니다.
...
### Mixture of neural fields for heterogeneous reconstruction in cryo-EM (https://arxiv.org/abs/2412.09420)
- **What's New**: 이번 연구에서는 cryo-electron microscopy (cryo-EM) 데이터에서 구성을 갖는 휘발성과 연속적인 이질성(compositional and conformational heterogeneity)을 동시에 분석할 수 있는 Hydra라는 새로운 방법을 제안합니다. Hydra는 K개의 신경 필드(neural fields)를 사용하여, 복잡한 샘플을 다루는데 있어 기존 방법들의 한계를 극복하며, 신규 가능성을 제시합니다. 이 방법은 합성 데이터셋뿐만 아니라 세포 용해물에서 얻은 실험 데이터셋에서도 유용성을 입증했습니다.

- **Technical Details**: Hydra는 새로운 likelihood-based loss function을 사용하여 orientations, conformations, class assignments를 동시에 최적화합니다. 이는 기존의 순차적 접근 방식의 문제점을 회피하며, 강한 조합 이질성(compositional heterogeneity)을 처리할 수 있습니다. 주요 기여로는 ab initio heterogeneous reconstruction을 위한 신경 필드 모델의 개발과, 다양한 단백질 복합체를 재구성할 수 있다는 점이 포함됩니다.

- **Performance Highlights**: 실험 결과, Hydra는 동적 변화를 가진 단백질 구조의 3D 재구성을 높은 정확도로 지원합니다. 세포 용해물의 혼합 샘플에서 세 가지 조합 상태(compositional states)를 단일 패스(single pass)로 드러내는 등의 성과를 보였으며, 이는 cryo-EM 방법론의 적용 가능성을 넓히는 중요한 발전이라 할 수 있습니다.



### A Geometry-Aware Message Passing Neural Network for Modeling Aerodynamics over Airfoils (https://arxiv.org/abs/2412.09399)
- **What's New**: 이번 연구에서는 공기역학의 수치적 모델링에서 기하학적 구조의 중요성을 강조합니다. 저자는 공기foil의 형상을 효과적으로 통합하기 위해 메시지 전달(message passing) 방식을 제안하여, 수치 해석의 비용을 줄이고 신경망 기반의 모든 데이터를 활용하는 방법을 탐구합니다. 이를 통해 GeoMPNN이라는 데이터 기반 기계 학습 프레임워크를 개발하였고, NeurIPS 2024 ML4CFD 대회에서 우수한 성적을 거두었습니다.

- **Technical Details**: 이 연구는 압축되지 않는 흐름(incompressible flows)을 모델링하며, 공기foil의 기하학적 형상에 초점을 맞춘 메시지 전달 신경망 구조를 도입합니다. 저자는 메시지 전달 과정을 통해 공기foil 표면에서의 기하학적 표현을 잠재 그래프(latent graph) 형태로 얻고, 이를 모든 연결점(collocation points)에 전파합니다. 특히, 공기foil의 선단(leading edge) 및 미단(trailing edge) 좌표계를 사용하여 동적 환경의 구분을 가능하게 하고, 동적 표현을 더 강화하기 위해 혼합 극-데카르트 좌표체계를 사용합니다.

- **Performance Highlights**: GeoMPNN은 비표준적인 메쉬에서의 학습을 가능하게 하고, 이를 통해 테스트 시 메쉬의 분포 변화(distribution shifts)를 피할 수 있는 방법을 제시합니다. 연구는 AirfRANS 데이터셋을 기반으로 하여 모델의 성능을 입증하였으며, 특히 다양한 기하학적 조건에서 공기foil의 공력(coefficients) 예측에서 뛰어난 효율성을 보였습니다. 이러한 결과는 실험을 통해 검증되었으며, 학습된 모델은 공기foil 설계를 가속화하는 데에 중요한 역할을 할 것으로 기대됩니다.
...
### Hybrid variable spiking graph neural networks for energy-efficient scientific machine learning (https://arxiv.org/abs/2412.09379)
- **What's New**: 본 논문에서는 Hybrid Variable Spiking Graph Neural Networks (HVS-GNNs)를 제안하여 에너지 소비를 줄이면서 회귀(regression) 작업을 효과적으로 수행할 수 있는 방법을 소개합니다. HVS-GNNs는 Variable Spiking Neurons (VSNs)를 활용하여 희소한 통신(sparse communication)을 촉진하고 에너지 예산을 줄이는 데 기여합니다. 이를 통해 복잡한 GNN 아키텍처에서 발생하는 에너지 문제를 해결하고 가변적인 신경망 구조를 통해 데이터 기반의 계산 메커니즘을 개선하고자 합니다.

- **Technical Details**: HVS-GNNs는 기존의 GNN 아키텍처에 VSNs를 통합하여 설계되었습니다. VSNs는 인공 뉴런의 동역학(dynamics)과 Leaky Integrate-and-Fire (LIF) 스파이킹 뉴런을 결합하여 연속적인 활성화와 희소한 통신의 이점을 동시에 제공합니다. HVS-GNNs는 낮은 에너지 소비를 목표로 하며, 여러 종류의 GNN을 최소한의 노력으로 하이브리드 변형으로 변환할 수 있는 유연성을 가지고 있습니다.

- **Performance Highlights**: 제안된 HVS-GNNs는 다양한 스케일의 계산 기계학(Computational Mechanics) 문제를 다루는 세 가지 예제에서 검증되었습니다. 이 연구는 회귀 작업에서의 성능이 LIF 뉴런과 기존의 인공 뉴런을 사용하는 GNN 아키텍처와 비교될 때 우수함을 보여주며, 에너지 사용의 관점에서도 효율성을 입증합니다. HVS-GNNs의 스파이킹 활동은 에너지 소비를 나타내는 척도로도 활용되며, 미래의 데이터 기반 계산 기계학 분야에서의 가능성을 시사합니다.



### A comprehensive interpretable machine learning framework for Mild Cognitive Impairment and Alzheimer's disease diagnosis (https://arxiv.org/abs/2412.09376)
Comments:
          This preprint has not been peer-reviewed yet but has been submitted to a journal

- **What's New**: 이번 연구에서는 Mild Cognitive Impairment (MCI)와 Alzheimer’s disease (AD) 진단의 해석 가능성을 높이기 위한 머신러닝 프레임워크를 도입하였습니다. 이를 위해 뇌 MRI의 볼륨 측정치를 포함한 데이터를 사용했으며, 기존의 클래스 불균형 문제를 앙상블 학습 접근법을 통해 해결하고, 해석 가능성 향상을 위한 다양한 기법들을 결합했습니다. 이것은 의료 전문가들이 MCI 및 AD의 병리학적 기전을 이해하는 데 도움을 줄 수 있습니다.

- **Technical Details**: 연구는 다양한 해석 가능성 기법을 활용하여 MCI 및 AD 진단을 위한 정확한 예측 모델을 개발하는 방법론적 프레임워크를 제안합니다. 이 프레임워크는 145개의 해부학적 영역에서의 MRI 볼륨 측정값과 54개의 AD 관련 단일 염기 다형성과 같은 데이터 모달리티를 결합하여 분석하였습니다. 또한 실제 인과성을 기반으로 한 기법을 이용하여 특징의 필요성과 충분성을 평가함으로써 해석 가능성 기법의 견고함을 검증합니다.

- **Performance Highlights**: 제안된 모델은 87.5%의 균형 정확도와 90.8%의 F1-score를 달성하였습니다. 여러 분류기 성능을 비교한 결과, OVO 접근 방식이 OVA보다 높은 성능을 보였고, Bagging 방법을 활용한 경우 더욱 향상된 분류 성능을 얻었습니다. 결과적으로, 이 연구는 MCI 및 AD 진단에 있어 해석 가능하고 신뢰할 수 있는 머신러닝 모델의 필요성을 강조합니다.
...
### Dynamic Prompt Allocation and Tuning for Continual Test-Time Adaptation (https://arxiv.org/abs/2412.09308)
Comments:
          21 pages, 5 figures, and 6 tables

- **What's New**: 최근에는 CTTA(Continual Test-Time Adaptation)가 등장하여 사전 학습된 소스 모델이 지속적으로 진화하는 타겟 분포에 적응할 수 있도록 돕고 있습니다. 본 논문에서는 도메인 특화 프롬프트를 도입하여 모델이 각 타겟 도메인에 맞게 적응하도록 안내합니다. 또, PAINT(동적 프롬프트 할당 및 튜닝) 방법을 제안하여 알려진 도메인과 탐색되지 않은 도메인을 동적으로 구별하고, 적절한 프롬프트를 선택하거나 새롭게 할당합니다.

- **Technical Details**: 기존 방법들은 주로 단일 공유 모델을 사용하여 다양한 타겟 도메인에 적응하는데, 이로 인해 심각한 도메인 간 간섭이 발생합니다. PAINT 방법은 메모리 버퍼에 도메인 특화 프롬프트를 저장하고, 데이터가 알려진 도메인에서 오고 있는지 또는 탐색되지 않은 도메인에서 오는지를 결정하기 위해 쿼리 메커니즘을 사용합니다. 이 방법은 상호 정보 최대화와 구조적 정규화를 활용하여 프롬프트 튜닝을 수행합니다.

- **Performance Highlights**: 세 가지 벤치마크 데이터셋에서 수행된 실험 결과는 PAINT 방법이 CTTA에서 효과적임을 입증하며, 이전 방식들에 비해 최첨단 성능을 달성했습니다. 이는 도메인 특화 프롬프트로 인한 도메인 간 간섭을 크게 줄이며, 모델이 여러 도메인에서 얻은 지식을 잊지 않도록 돕습니다. 이를 통해 지속적으로 변화하는 환경에서도 강력한 일반화 능력을 유지할 수 있습니다.



### Transfer Learning of RSSI to Improve Indoor Localisation Performanc (https://arxiv.org/abs/2412.09292)
- **What's New**: 본 연구에서는 Conditional Generative Adversarial Networks (ConGAN)를 기반으로 한 데이터 증강 기법과 전이 학습 프레임워크(T-ConGAN)를 제안합니다. 이를 통해 서로 다른 주택 간에도 Bluetooth Low Energy (BLE) 신호 강도(RSSI) 정보를 공유할 수 있으며, 이는 헬스케어 모니터링 시스템의 성능과 확장성을 향상시킵니다. 이 방법은 각 주택에서 주석 데이터 수집에 대한 필요를 줄여주며, 특히 계단이나 외부 공간과 같은 도전적인 지역에서의 실내 로컬라이제이션 성능을 크게 개선합니다.

- **Technical Details**: ConGAN은 특정 입력 변수에 조건화된 합성 데이터를 생성하는 생성적 신경망으로, 실제 환경의 패턴과 일치하는 현실적인 RSSI 데이터를 생성하는 데 효과적입니다. 연구진은 전이 학습을 활용하여 여러 주택의 일반적인 RSSI 특성을 학습한 후, 특정 주택에 맞춤화된 RSSI 신호를 생성합니다. 이러한 방법은 서로 다른 실험 프로토콜에서도 RSSI 데이터의 전이 가능성을 입증하며, 수집된 데이터가 제한적인 실제 환경에서 성능을 향상시킵니다.

- **Performance Highlights**: 이 연구에서 제안한 T-ConGAN은 실내 로컬라이제이션에서 매크로 F1 점수를 최대 12.2% 향상시켰습니다. 특히 계단 및 외부 공간과 같은 어려운 지역에서는 51%의 성능 향상이 이루어졌습니다. 이러한 결과는 헬스케어 모니터링 시스템의 강건성을 크게 강화하며, 더욱 나은 결과를 제공합니다.



### Optimising TinyML with Quantization and Distillation of Transformer and Mamba Models for Indoor Localisation on Edge Devices (https://arxiv.org/abs/2412.09289)
- **What's New**: 이 논문은 자원이 제한된 엣지 디바이스에서 실행될 수 있는 소형 효율적인 기계 학습 모델(TinyML)을 제안합니다. 기존의 실내 위치 추정 방식은 낮은 전력의 웨어러블 기기에서 수집된 데이터를 중앙 서버에서 처리하는 데 의존하지만, 이 연구는 이러한 과정을 엣지 디바이스로 이전함으로써 배터리 수명 연장, 개인 정보 보호 향상, 지연 시간 감소와 같은 이점을 제시합니다. 특히, 내구성이 뛰어난 저전력 MCU에 이를 배포하는 가능성을 모색합니다.

- **Technical Details**: 본 연구는 모델 압축 기법으로 양자화(quantization)와 지식 증류(knowledge distillation)를 활용하여 모델 크기를 줄이면서도 높은 예측 성능을 유지하는 데 중점을 두었습니다. 제안하는 모델은 최신 상태의 transformer 기반 모델을 바탕으로 하며, 더 작은 Mamba 기반 아키텍처를 대안으로 제안합니다. 실험 결과, 양자화된 transformer 모델은 64KB RAM 제한 내에서 높은 정확도를 유지하며, Mamba 모델은 32KB RAM에서도 좋은 성능을 보여줍니다.

- **Performance Highlights**: 이 연구의 TinyML 모델은 저전력 MCU에 적합하게 설계되어 있으며, 실시간 위치 추정을 위한 혁신적 솔루션을 제공합니다. 이 기술은 환자 모니터링과 같은 헬스케어 분야에서 정확하고 실시간 위치 데이터를 제공함으로써 혁신을 가져올 가능성이 있습니다. 데이터 처리의 지역화를 통해 전력 소비를 최소화하고, 데이터 개인 보호를 강화하면서도 운영 비용을 줄이는 솔루션을 제시합니다.
...
### Single-View Graph Contrastive Learning with Soft Neighborhood Awareness (https://arxiv.org/abs/2412.09261)
Comments:
          Accepted by AAAI2025; full version including appendix

- **What's New**: 본 논문에서는 그래프 대조 학습(Graph Contrastive Learning, GCL)의 전통적인 교차 뷰 대조 방식에 대한 의존도를 줄이기 위해 단일 뷰 그래프 대조 학습 프레임워크인 SIGNA를 제안합니다. SIGNA는 소프트 이웃 인식(soft neighborhood awareness)을 활용하여 이웃 간의 관계를 고려한 대조 효과를 극대화합니다. 이를 통해 이 논문은 GCL 분야에서의 새로운 접근 방식을 모색합니다.

- **Technical Details**: SIGNA는 드롭 아웃(dropout)을 통해 구조적으로 관계가 있으면서도 랜덤하게 노이즈가 추가된 임베딩 쌍을 생성하며, 이는 잠재적인 양성 샘플로 사용됩니다. 각 에포크(epoch)에서는 이웃의 역할이 양성에서 음성으로 전환되며, 이로 인해 확률적 이웃 대조 학습(probabilistic neighborhood contrastive learning) 효과가 발생합니다. 또한, 정상화된 젠센-샤논 발산(Normalized Jensen-Shannon divergence, Norm-JSD) 추정기를 제안하여 대조 학습의 효과를 높입니다.

- **Performance Highlights**: SIGNA는 전이 학습(transductive learning), 유도 노드 분류(inductive node classification), 노드 클러스터링(node clustering) 등 다양한 노드 수준 작업에서 기존 방법들보다 최대 21.74% 성능을 향상시켰습니다. 특히, 소프트 이웃 인식을 통해 SIGNA는 MLP를 인코더로 사용하여 GCN 기반 인코더보다 109배에서 331배 더 빠른 추론 속도를 구현합니다. 이 논문은 SIGNA가 클래스 간 분리와 클래스 내 집합 간의 균형을 성공적으로 유지하고 있다는 것을 실험과 시각화를 통해 입증합니다.



### When Can Memorization Improve Fairness? (https://arxiv.org/abs/2412.09254)
- **What's New**: 이번 연구에서는 다중 클래스 분류 문제에서 메모리 제어(memorization)가 부가적인 공정성 메트릭(additive fairness metrics)인 statistical parity, equal opportunity, equalized odds에 미치는 영향을 분석합니다. 저자는 메모리 데이터셋의 그룹 구성과 레이블 분포를 바탕으로 메모리 제어를 통해 발생하는 편향(bias)을 정량화하고, 공정한 분류기를 위해 필요한 메모리 데이터셋의 최적 구성을 수학적으로 명시합니다.

- **Technical Details**: 해당 연구는 확률 이론을 기반으로 하여 공정성 측정 기준인 statistical parity, equal opportunity, equalized odds를 정립하고, 메모리 데이터셋의 비율과 구성에 따른 편향을 분석합니다. 연구 결과는 메모리 데이터셋이 공정성을 보장하기 위해 필요로 하는 확률 질량(total probability mass)에 대한 상한과 하한을 제공합니다. 각 공정성 메트릭에 대한 메모리 데이터셋의 구성을 선형 방정식 시스템을 통해 설명합니다.

- **Performance Highlights**: 이 논문의 결과는 다양한 실제 상황에서 공정성을 보장하는 데 활용될 수 있습니다. 메모리 변화가 모델 성능에 미치는 영향을 분석하고, 특정 그룹 간의 공정성 요구를 충족하기 위해 필요한 데이터셋의 비율을 제시합니다. 이를 통해 메모리 제어가 불공정성을 감추는 데 어떻게 악용될 수 있는지를 이해함으로써, 실질적인 데이터 처리 및 모델 설계의 효과를 극대화할 수 있습니다.



### GeLoRA: Geometric Adaptive Ranks For Efficient LoRA Fine-tuning (https://arxiv.org/abs/2412.09250)
- **What's New**: Geometric Low-Rank Adaptation (GeLoRA)는 저조차(Low-Rank) 적응 방법의 새로운 프레임워크로, 숨겨진 상태 표현의 내재 차원(intrinsic dimensionality)을 계산하여 LoRA의 최적 순위를 선택합니다. GeLoRA는 각 레이어의 입력과 출력 표현의 내재 차원에 따라 동적으로 순위를 조정하며, 모든 모델 매개변수가 미세 조정에 동일한 영향을 미치지 않음을 인식합니다. 이러한 접근 방식은 성능과 효율성 간의 균형을 더욱 정교하게 잡을 수 있게 해줍니다.

- **Technical Details**: GeLoRA는 데이터 표현의 내재 차원과 언어 모델 내의 파라미터 업데이트의 순위(ranks) 간의 관계를 통계적으로 조사하여 최적 순위의 하한(lower bound)을 도출합니다. 이론적으로, hidden representations의 기하학적 속성을 활용하여 전통적인 LoRA의 정확한 대체 방안을 마련하고 있습니다. Two Nearest Neighbors (TwoNN) 방법을 사용하여 각 hidden state의 내재 차원을 추정하는 방식을 채택합니다.

- **Performance Highlights**: 여러 작업에서의 경험적 검증 결과, GeLoRA는 동일한 파라미터 예산 내에서 최근의 벤치마크를 일관되게 초과하는 성능을 보여줍니다. GeLoRA는 자원 효율성과 모델 표현력 간의 최적의 균형을 제공하며, 이는 채택된 프레임워크의 실용성을 강조합니다. 이 연구는 PEFT(Parameters-Efficient Fine-Tuning) 방법의 발전을 촉진하고, LLMs의 fine-tuning 과정에서의 다양성을 높이는 데 기여합니다.



### Uplift modeling with continuous treatments: A predict-then-optimize approach (https://arxiv.org/abs/2412.09232)
- **What's New**: 이 논문에서는 uplift modeling(UM)에서 연속적 치료를 허용하는 predict-then-optimize 프레임워크를 제시합니다. 기존의 UM 접근법은 주로 이진 치료에 집중하였으나, 이 연구는 치료 용량(dose)이라는 새로운 개념을 도입하여 연속적 치료를 효과적으로 처리하는 방법을 탐구합니다. 이를 통해 의사결정자는 자원 가용성을 고려하면서도 효율적으로 치료 용량을 할당할 수 있습니다.

- **Technical Details**: 연구의 첫 번째 단계에서는 인과 기계 학습 기술을 사용하여 conditional average dose responses (CADRs)를 추정합니다. 두 번째 단계에서는 정수 선형 프로그래밍(integer linear programming, ILP)을 활용하여 연속적 치료의 할당 문제를 다룹니다. 이 접근법은 공정성(fairness) 고려 사항과 같은 추가 제약 조건을 포함하고, 비용 민감성을 고려한 목표 함수 수정의 가능성을 제공합니다.

- **Performance Highlights**: 실험 결과는 다양한 CADR 추정기를 비교하고 정책 가치와 공정성 간의 균형을 보여줍니다. 특히, 실험을 통해 공정성 제약 또는 비용 민감 목표를 포함할 경우 정책 성과에 미치는 영향을 입증하였습니다. 이러한 프레임워크는 의료, 금융, 인사 관리와 같은 다양한 분야에서 응용될 수 있는 유연성과 장점을 제공합니다.



### A Brief Discussion on KPI Development in Public Administration (https://arxiv.org/abs/2412.09142)
- **What's New**: 본 연구에서는 공공 행정(PA) 분야의 성과 평가를 위한 키 성과 지표(KPIs) 구축을 위한 혁신적인 프레임워크를 제안합니다. 랜덤 포레스트 알고리즘(Random Forest algorithms)과 변수 중요성 분석(variable importance analysis)을 활용하여, PA 성과에 크게 영향을 미치는 주요 변수를 식별합니다. 이 연구는 머신러닝 기술의 적용을 통해 PA 성과를 개선하고자 하며, 공공 행정의 더 민첩하고 결과 중심적인 접근 방식을 촉진하려고 합니다.

- **Technical Details**: 이론적 배경 섹션에서는 키 성과 지표(KPIs)와 랜덤 포레스트(Random Forest)에 대한 기본 개념을 설명합니다. KPI는 조직의 목표를 구체적인 측정값으로 전환하기 위해 필요한 중요한 지표들의 집합입니다. 랜덤 포레스트는 분류(classification) 및 회귀(regression) 작업을 위한 앙상블 학습 기술로서, 다수의 결정 트리를 구축하여 예측의 평균을 출력하는 형식을 사용합니다. 이 모델은 변수 중요성을 평가하여 KPI를 최적화하는 데 중요한 통찰력을 제공합니다.

- **Performance Highlights**: 이 연구는 공공 행정에서 KPI 정의 및 개발의 중요성을 강조하고 있습니다. 최신 기술을 통해 KPIs를 식별하고 개선할 수 있는 잠재력을 보여 주며, 기존 연구에 비해 더 체계적이고 데이터 기반의 KPI 관리 시스템을 제공하는 방법을 제시합니다. 연구 결과는 PA 프로세스의 평가 및 최적화에 실질적이고 의미 있는 통찰력을 제공합니다.



### MMD-OPT : Maximum Mean Discrepancy Based Sample Efficient Collision Risk Minimization for Autonomous Driving (https://arxiv.org/abs/2412.09121)
- **What's New**: 본 논문에서는 MMD-OPT라는 새로운 방법론을 제안합니다. 이는 동적 장애물의 예측 분포에 따라 충돌 위험을 최소화하기 위한 샘플 효율적인 접근 방식입니다. MMD-OPT는 재생 커널 힐베르트 공간(Reproducing Kernel Hilbert Space, RKHS)에서의 분포 임베딩과 최대 평균 불일치(Maximum Mean Discrepancy, MMD) 개념을 기반으로 하여, 이러한 두 가지 개념이 충돌 위험 추정의 샘플 효율성을 높이는 데 어떻게 활용될 수 있는지를 보여줍니다.

- **Technical Details**: MMD-OPT는 깊은 생성 모델과 같은 복잡한 예측 분포에서 샘플을 추출하여 충돌 위험 비용을 효율적으로 추정합니다. 논문은 RKHS에서 두 분포의 차이를 MMD를 이용하여 계산하는 방법을 소개합니다. 또한, 저자는 비계층 최적화 문제를 제안하여 불필요한 예측 샘플을 줄이면서도 충돌 확률을 외밀하게 포착할 수 있도록 합니다.

- **Performance Highlights**: 제안된 MMD-OPT는 기존의 Conditional Value at Risk (CVaR) 기반 방법들과 비교했을 때 샘플 수가 적어도 더 안전한 경로를 생성하는 성능을 보였습니다. 특히, 예측 분포가 Gaussian 형태에서 크게 벗어났을 때 그 차이는 더욱 두드러졌습니다. 최종적으로, MMD-OPT는 실시간 환경에서도 충분히 빠른 계산 성능을 보장합니다.



### The Utility and Complexity of In- and Out-of-Distribution Machine Unlearning (https://arxiv.org/abs/2412.09119)
- **What's New**: 이번 논문에서는 machine unlearning의 fundamental utility, time, and space complexity trade-offs를 체계적으로 분석합니다. 특히, 데이터 삭제 요청에 대해 이전 연구에서는 다루지 않은 이론적 간극을 메우며, differential privacy와 유사한 rigorous certification을 제공합니다. 이를 통해 in-distribution forget data에 대한 단순하면서도 일반적인 절차를 제시하여 tight한 unlearning-utility-complexity trade-off를 달성합니다.

- **Technical Details**: 이 연구는 in-distribution과 out-of-distribution forget data의 두 가지 시나리오를 중점적으로 분석합니다. 특히, in-distribution forget data에 대해서는 output perturbation과 함께 empirical risk minimization 방법이 효과적이라는 점을 강조합니다. 반면, out-of-distribution forget data에 대해서는 unlearning 시간 복잡성이 retraining보다 클 수 있음을 보여주며, 이를 해결하기 위한 로버스트하고 효율적인 gradient descent 변형 알고리즘을 제안합니다.

- **Performance Highlights**: 제안된 알고리즘은 고정된 유틸리티와 계산 예산 하에 삭제 가능한 샘플 수를 정량화하며, near-linear한 시간과 공간 복잡도로 데이터를 unlearn할 수 있는 능력을 인증합니다. 이러한 알고리즘은 특히 OOD 및 손상된 데이터에 대해 견고성을 가지고 있어, 실제 데이터 삭제 요청 시 발생할 수 있는 다양한 공격으로부터 방어할 수 있도록 설계되었습니다. 이는 machine unlearning을 실제 응용에서 신뢰할 수 있는 도구로 만드는 데 중요한 기여를 합니다.



### An Algorithm-Centered Approach To Model Streaming Data (https://arxiv.org/abs/2412.09118)
Comments:
          This manuscript is currently under review at the Symposium on Intelligent Data Analysis (IDA 2025)

- **What's New**: 이 논문은 기존의 오프라인 머신러닝 이론에 비해 드리프트(concept drift) 상황에서의 머신러닝 이론이 부족하다는 점을 강조합니다. 저자들은 데이터의 시간을 기준으로 하는 새로운 프레임워크를 제안하여 알고리즘 관점에서 드리프트를 정의하고자 합니다. 이 프레임워크는 시간 포인트(time points) 대신 시간 창(window)을 활용하여 보다 실용적인 접근 방식을 제공합니다.

- **Technical Details**: 전통적인 배치(batch) 머신러닝에서 데이터는 고정된 분포에서 i.i.d.로 추출됩니다. 반면, 스트리밍 스트림(stream learning)에서는 무한한 데이터 포인트(Xi)가 시간이 지남에 따라 도착하고, 이는 슬라이딩 윈도우(sliding window) 기법을 통해 모델을 업데이트하는 방식으로 처리됩니다. 저자들은 이러한 시간 창 접근 방식이 기존의 시간 점(time point) 정의와 유사한 정보 구조를 가진다는 점을 수학적으로 분석합니다.

- **Performance Highlights**: 이 연구에서는 새로운 프레임워크를 통해 시간 창 기반의 접근 방식과 시간 점 기반의 접근 방식을 비교하고 두 방법의 이론적 및 알고리즘적 관계를 명확히 합니다. 또한, 이들 간의 연결성을 보여주는 실험 결과와 이를 기반으로 한 중요한 인프라스트럭처 분야의 응용 사례를 소개합니다. 학문적 기여뿐만 아니라 실무적 적용 가능성에도 집중하고 있습니다.



### How to Re-enable PDE Loss for Physical Systems Modeling Under Partial Observation (https://arxiv.org/abs/2412.09116)
Comments:
          Accepted by AAAI2025

- **What's New**: 이번 연구에서 제안된 RPLPO(Partial Observation하의 PDE Loss 재활용) 프레임워크는 물리 시스템 모델링에서의 일반화 성능을 향상시키기 위해 PDE Loss를 효과적으로 재활용하는 접근 방식을 제공합니다. 일반적으로 데이터 수집이 어려운 실제 환경에서는 센서의 제한으로 인해 부분 관측만 가능하기 때문에 PDE Loss의 활용이 어려운 상황들이 많습니다. 그러나 RPLPO는 높은 해상도의 상태를 재구성하고 시스템 전이를 동시에 제약하여 모델의 예측 능력을 강화할 수 있도록 설계되었습니다.

- **Technical Details**: RPLPO는 주로 인코딩 모듈과 전이 모듈로 구성되어 있습니다. 인코딩 모듈은 부분 관측을 기반으로 높은 해상도의 상태를 재구성하며, 전이 모듈은 미래 상태를 예측하는 역할을 합니다. 본 연구는 데이터 손실과 PDE Loss가 함께 사용되는 베이스 훈련 기간과 라벨이 없는 데이터를 활용하는 반지도 학습 방식의 두 단계 미세 조정 과정을 통해 모델의 일반화를 향상시키도록 설정되었습니다.

- **Performance Highlights**: RPLPO는 다양한 물리 시스템을 대표하는 다섯 가지 PDE 다이내믹스를 통해 성능이 입증되었습니다. 실험 결과, 본 프레임워크를 사용한 모델은 단일 단계 및 다단계 예측에서 미래의 부분 관측 상태를 예측하는 일반화 능력이 상당히 향상되었습니다. 또한, RPLPO는 다양한 데이터 수, 드문 관측, 불규칙한 데이터 및 노이즈가 있는 상황에서도 효과적이며, 구현과 하이퍼파라미터의 강인성도 증명되었습니다.



### Integrated trucks assignment and scheduling problem with mixed service mode docks: A Q-learning based adaptive large neighborhood search algorithm (https://arxiv.org/abs/2412.09090)
Comments:
          29 pages, 12 figures, 15 tables

- **What's New**: 이 논문은 혼합 서비스 모드(Mixed Service Mode, MSM) 도크의 위치와 수를 사전 결정하는 기존 연구를 보완하기 위해 도크 모드 결정, 트럭 배정 및 일정 계획을 통합한 새로운 모델을 제안합니다. 이 모델은 Q-러닝 기반 적응형 대규모 이웃 탐색(Q-Learning-based Adaptive Large Neighborhood Search, Q-ALNS) 알고리즘을 활용하여 도크 모드를 동적으로 조정합니다. 이로 인해 다양한 트럭 수요에 따라 도크 모드를 적절히 분배할 수 있는 가능성을 제시합니다.

- **Technical Details**: 본 연구에서 제안된 Q-ALNS 알고리즘은 국부적 탐색 연산자(destroy and repair local search operators)와 변동 연산자(perturbation operators)를 사용하여 트럭 배정과 일정을 효과적으로 해결합니다. 운영자 필터링(Operator Filtering, OF)과 적응형 운영자 선택(Adaptive Operator Selection, AOS) 프로세스를 통해 성능을 극대화합니다. Q-러닝을 통해 과거 운영자 성과를 분석하고 미래 이익을 고려하여 최적의 운영자를 선택할 수 있습니다.

- **Performance Highlights**: Q-ALNS 알고리즘은 벤치마크 알고리즘보다 우수한 최적성 간격(optimality gap) 및 파레토 전선(Pareto front) 발견에서 일관되게 성능을 향상시킵니다. 연구 결과에 따르면, 기존의 사전 결정된 서비스 모드에 비해 평균 지연시간(average tardiness)과 작업 완료 시간(makespan)이 낮아지며, 다양한 요구에 대한 적응성이 뛰어난 것으로 나타났습니다. 이는 실질적인 창고 운영에 있어 MSM 도크 채택의 중요성을 강조합니다.



### Neural Networks for Threshold Dynamics Reconstruction (https://arxiv.org/abs/2412.09079)
Comments:
          Key words: threshold dynamics, cellular automaton, inverse problems, convolutional neural networks, deep learning

- **What's New**: 이 논문에서는 비디오 데이터에서 전선(Front)의 진화를 위한 임계역학(threshold dynamics)을 모델링하고 학습하기 위해 Merriman-Bence-Osher (MBO) 알고리즘과 세포 자동(cellular automaton)에서 영감을 받은 두 가지 합성곱 신경망(CNN) 아키텍처를 소개합니다. 첫 번째 모델인 단일 역학(single-dynamics) MBO 네트워크는 각 입력 비디오에 대해 특정 커널과 임계값을 학습하지만, 새로운 역학에 적응하지 않습니다. 두 번째 메타 학습(meta-learning) MBO 네트워크는 다양한 임계역학을 포괄적으로 일반화하여 입력에 따라 매개변수를 조정합니다.

- **Technical Details**: MBO 알고리즘은 평균 곡률(mean curvature)과 같은 정상 속도(normal velocity)를 가진 경계가 이동하는 세트의 시간 진화를 추적하는 계산 방법을 제공합니다. 이 알고리즘은 선형 작업(heat diffusion)과 비선형 작업(thresholding)을 반복적으로 적용하는 시간 이산(thresholding scheme) 기반입니다. 또한, MBO 알고리즘은 임계값 연산(thresholding operation)을 적용하여 특성 함수(characteristic function)을 통해 전선의 진화를 서술하는 수학적 구조를 제시합니다.

- **Performance Highlights**: 두 모델은 합성 및 실제 비디오 데이터(예: 얼음 녹는 과정, 화재 전선 전파)에 대해 평가되었으며, 성능 지표는 심지어 소음이 있는 조건에서도 진화하는 경계를 효과적으로 재구성하고 외삽(extrapolation)하는 데 성공했다는 것을 보여줍니다. 실험 결과는 다양한 합성 및 실제 역학에 걸쳐 두 네트워크의 강인성을 강조합니다.



### Multi-view Clustering via Unified Multi-kernel Learning and Matrix Factorization (https://arxiv.org/abs/2412.09065)
- **What's New**: 이 논문에서는 다중-뷰 클러스터링의 새로운 접근 방식인 MVC-UMKLMF를 제안합니다. 이 방법은 multi-kernel learning과 matrix factorization을 통합하여 클러스터링 구조의 정확성을 향상시키고, 기존의 비효율성을 줄이기 위해 시도되었습니다. MVC-UMKLMF는 각 뷰의 정규 직교 제약을 제거하고 합의 행렬에 대한 정규 직교 제약을 부여하여 최종 클러스터링 구조의 정확성을 높입니다.

- **Technical Details**: MVC-UMKLMF는 다중-뷰 데이터 매트릭스를 다중 커널 함수를 통해 매핑하여 비선형 데이터를 처리합니다. 이 방법은 비부정성 제약을 제거하고 개별 뷰의 기초 행렬에 대한 정규 직교 제약을 제거하여 클러스터링 구조 학습의 유연성을 증대시킵니다. 제안된 알고리즘은 세 단계의 최적화 절차를 포함하며, 각 단계에서 목표 함수의 단조 감소를 보장합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 널리 사용되는 실제 데이터 세트에서 효과성을 입증했습니다. 기존의 multi-kernel clustering(MKC) 방법과 비교할 때, MVC-UMKLMF는 계산 복잡성을 줄이는 동시에 클러스터링 정확성을 유지하는 성과를 보였습니다. 이는 연구자들에게 다중-뷰 데이터를 분석하는 데 있어 구현 효율성을 제공하는 중요한 발전입니다.



### Go With the Flow: Fast Diffusion for Gaussian Mixture Models (https://arxiv.org/abs/2412.09059)
- **What's New**: 이 논문에서는 기존의 Schrödinger Bridges(SBs) 계산 방식이 다소 비용이 많이 드는 점을 해결하기 위해, 다이나믹 시스템의 분포를 하나의 Gaussian Mixture Model(GMM)에서 다른 하나로 전환하는 최적의 정책의 해석적 파라메터화를 제안합니다. 이 방법은 비선형 최적화 기법에 의존하는 대신, 각 혼합물의 구성 요소 수에 비례하여 선형적으로 확장되는 저차원 선형 프로그래밍 문제의 해결로 정책을 근사할 수 있습니다.

- **Technical Details**: 기존의 EOT와 SB 문제는 다차원 응용에서 관심이 증가하고 있으며, 이를 해결하기 위한 다양한 알고리즘이 개발되었습니다. 이 논문에서는 특히 가변적인 Linear Time-Varying 시스템과 같은 더 일반적인 동적 시스템에서도 더 잘 일반화될 수 있는 방법을 보여줍니다. 이 접근법은 오토인코더의 잠재 공간에서 이미지 간 변환과 다양한 예시 등의 저차원 문제에서 그 잠재력을 발휘합니다.

- **Performance Highlights**: 비교한 결과, 제안한 접근법은 경계 분포가 혼합 모델일 때 최첨단 방법들보다 더 우수한 성능을 보였습니다. 또한 이 방법은 거의 훈련이 필요 없는 특성을 가지고 있어, 실제 적용 시 컴퓨팅 비용을 크게 절감할 수 있는 가능성이 있습니다.



### Safe Active Learning for Gaussian Differential Equations (https://arxiv.org/abs/2412.09053)
- **What's New**: 이 논문은 안전하고 자동화된 데이터 수집을 위한 새로운 알고리즘인 Safe Active Learning (SAL) GPODE를 제안합니다. 이 알고리즘은 JSON과 같은 다차원 데이터에 대한 효율적인 수집을 가능하게 하며, 안전성을 고려한 측정 후보의 정보를 평가하는 새로운 안전성 기능도 포함하고 있습니다. SAL GPODE는 기존의 비활성 측정 방법에 비해 성능이 우수하다는 것을 실험적으로 입증하였습니다.

- **Technical Details**: SAL GPODE 알고리즘의 핵심은 새로운 데이터를 최대한 활용하기 위한 제한된 최적화 문제입니다. 이 최적화 문제는 데이터 수집이 안전한지 여부에 대한 제약 조건을 포함합니다. 모델은 d차원 상태로 이루어진 동적 시스템을 기반으로 하며, 시스템의 일련의 시계열 데이터를 사용하여 발전합니다. 이 데이터는 노이즈를 포함하며 모델링 목적에 적합하게 처리됩니다.

- **Performance Highlights**: 논문에서는 SAL GPODE 알고리즘이 두 가지 관련 예시를 통해 기존의 비활성 데이터 측정 방법보다 뛰어난 성능을 발휘함을 보여주었습니다. SAL GPODE를 통해 수집된 데이터는 동적 시스템의 모델 성능을 크게 향상시키며, 효율적인 데이터 활용으로 시간과 비용을 절감하는 데 기여할 수 있습니다. 이러한 성능 개선은 알고리즘이 안전성과 효율성을 동시에 고려하고 있기 때문입니다.



### Beyond Confusion: A Fine-grained Dialectical Examination of Human Activity Recognition Benchmark Datasets (https://arxiv.org/abs/2412.09037)
- **What's New**: 이 연구는 인간 활동 인식을 위한 머신 러닝 알고리즘의 진행 상황을 다루며, 특히 부정적 샘플 분석의 중요성을 강조합니다. 최근 트랜스포머 모델이 HAR 데이터셋에 적용되었지만, 한계가 있음을 지적하고, 현재의 접근 방식이 직면한 문제를 분명히 합니다. 여섯 개의 인기 있는 HAR 벤치마크 데이터셋을 세밀하게 검사한 결과, 상태-of-the-art (소위 최신 기술)의 ML 방법으로 정체된 분류 작업이 확인되었습니다.

- **Technical Details**: 연구는 PAMAP2 및 Opportunity 등 여러 공개 데이터셋에서 센서 기반 인간 활동 인식(HAR)에 대한 머신 러닝(ML) 방법의 발전을 분석하였습니다. 특히, 제안된 여러 모델이 단순한 통계 메트릭치고는 성능이 미비하음을 발견했습니다. 연구는 잘못된 분류의 교차점(Intersect of False Classification, IFC)을 통해 분류 불가능한 데이터의 특정 구간을 규명하였습니다.

- **Performance Highlights**: 기술적으로, 이 연구는 부정확한 주석, 녹화 과정에서의 불규칙성 및 전환 구간의 잘못된 정렬 문제를 파악하여 결정적인 기여를 하였습니다. 연구진은 이러한 문제들을 정량화하고 설명하는 데 중점을 두었으며, 향후 데이터 수집에서의 개선 가능성을 제시하였습니다. 특히, 주석 모호성을 수치적으로 평가하고 데이터셋 수정에 필요한 분류 마스크를 제공하는 것이 큰 의의가 있습니다.



### Pulling the Carpet Below the Learner's Feet: Genetic Algorithm To Learn Ensemble Machine Learning Model During Concept Drif (https://arxiv.org/abs/2412.09035)
- **What's New**: 이 연구는 기계 학습(ML) 모델이 동적 환경에서 직면하는 개념 변화(concept drift, CD)를 해결하기 위해 유전 알고리즘(genetic algorithms, GAs)을 적용하는 방법을 탐구합니다. 연구진은 글로벌 ML 모델과 CD 탐지기가 결합된 새로운 두 단계 집합 ML 모델을 제안하며, 이는 각각 CD 탐지기를 갖춘 ML 파이프라인 모델 집단의 집합체로 작동합니다. 이 연구는 CD 특성과 그로 인한 문제를 해결하기 위한 기존 연구를 보완하는 새로운 시도를 담고 있습니다.

- **Technical Details**: 유전 알고리즘은 자연 선택과 유전 원리를 기반으로 한 탐색 및 최적화 기법으로, 대규모 솔루션 공간을 효율적으로 탐색하여 최적의 솔루션을 찾는 데 도움을 줍니다. ML의 영역에서 GAs는 강력한 ML 파이프라인을 찾거나 심볼릭 회귀 문제를 해결하는 데 활용됩니다. 본 연구에서 제안하는 모델은 GA 알고리즘을 이용하여 글로벌 ML 모델과 CD 탐지기를 결합하여 각각 자율적으로 데이터 집합을 조정하고 학습하는 방식으로 설계되었습니다.

- **Performance Highlights**: 대규모 합성 데이터셋을 분석한 결과, 제안된 모델은 단일 ML 파이프라인과 CD 알고리즘 대비 우수한 성능을 보였습니다. 특히 CD 특성을 알 수 없는 환경에서 더욱 두드러진 성능 향상을 나타냈습니다. 이 연구는 GAs를 활용한 집합 ML 및 CD 모델의 잠재력을 강조하며, 복잡한 CD 사건을 처리하는 데 있어 유용성을 제시하고 있습니다.
...
### Learning and Current Prediction of PMSM Drive via Differential Neural Networks (https://arxiv.org/abs/2412.09028)
- **What's New**: 이번 연구에서는 지속적인 시간의 동적 시스템을 모델링하기 위해 차별 신경망(differential neural networks, DNNs)을 이용한 새로운 접근 방식을 제시합니다. 특히, 영구자석 동기 모터(permanent magnet synchronous motors, PMSMs)의 전류 궤적을 예측하는 데 초점을 맞추고 있습니다. 다양한 하중 교란 및 무부하 조건에서의 실험을 통해 이 방법의 효과를 검증하였으며, 복잡한 동적 데이터의 내재된 동역학 학습에 대한 귀중한 통찰을 제공합니다.

- **Technical Details**: 연구에서 제안한 DNN 모델은 PMSM의 원래 시스템을 효과적으로 재구축하고 단기 및 장기 예측 능력을 갖추고 있습니다. 이 모델은 또한 누적 불확실성에서 발생하는 파라미터 오류의 영향을 고려하여 시스템 성능에 대한 영향을 분석합니다. 최적의 동적 수학 모델은 다양한 파라미터에 대한 불확실성을 포함하여 작성되며, 이는 DNN 예측의 정확성을 높이는 데 기여합니다.

- **Performance Highlights**: 실험 결과는 제안된 방법이 원래 시스템을 정확하게 복원하고 강력한 예측 능력을 보여준다고 나타났습니다. DNN은 다양한 하중 조건 하에서 우수한 예측 성능을 보였으나, 하중의 종류에 따라 예측 궤적의 변동이 있었습니다. 이러한 차이는 하중의 복잡성 수준에 따라 발생하며, 예측 손실 또한 그에 따라 달라졌습니다.



### Training Physical Neural Networks for Analog In-Memory Computing (https://arxiv.org/abs/2412.09010)
Comments:
          53 pages, 20 figures

- **What's New**: 이 논문은 전통적인 딥러닝 가속기에서 발생하는 von Neumann 병목 현상을 완화하기 위한 인메모리 컴퓨팅 (IMC) 아키텍처를 탐구합니다. 기존의 IMC는 아날로그 회로로 구현되었으며, 이로 인해 하드웨어의 비이상적 특성이 주요한 도전 과제가 되었습니다. 이 연구에서는 물리적 신경망 (PNNs)을 통해 IMC의 물리적 모델을 구성하고, 새로운 훈련 기법인 가분화 가능한 스파이크 시간 이산화 (differentiable spike-time discretization) 방식을 도입하여 훈련의 효율성을 높입니다.

- **Technical Details**: PNNs는 막 전위에 대한 시냅스 전류의 의존성을 해결하기 위해 설계되었습니다. 이 모델은 스파이킹 신경망(spiking neural networks)과 수학적으로 동등하며, IMC 회로의 비선형적 동작을 정확하게 포착합니다. IMC 회로와 관련된 복잡한 아날로그 동역학을 간결하게 모델링함으로써, 기존의 상향식(top-down) 접근 방식에서 발생하는 문제를 해결하고, 하드웨어 비이상성을 통합할 수 있는 가능성을 제시합니다.

- **Performance Highlights**: 연구 결과 IMC 회로는 교차 배열(crossbar array) 구조로 설계되었으며, 전하 영역(charge domain) IMC의 접근 방식을采用하여 에너지 효율성을 극대화했습니다. 후 처리(layout) 시뮬레이션 결과, 이 하향식 접근 방식에서의 모델링 오류는 기존 상향식 접근 방식보다 10배 이상 감소했습니다. 이러한 하드웨어 비이상적인 특성이 모델의 학습 성능을 향상시키는 데 어떻게 기여했는지를 명확히 보여줍니다.



### A physics-informed transformer neural operator for learning generalized solutions of initial boundary value problems (https://arxiv.org/abs/2412.09009)
Comments:
          29 pages, 11 figures, 4 tables

- **What's New**: 이 논문에서는 초기 경계 값 문제(initial boundary value problems; IBVPs)에 대해 새로운 물리 기반 변환기 신경 연산자(Physics-Informed Transformer Neural Operator; PINTO)를 개발하였습니다. 기존의 신경 연산자들은 새로운 초기 및 경계 조건에서 재교육을 요구하는 반면, PINTO 모델은 물리 손실(physics loss)만을 사용하여 시뮬레이션 없이 효율적으로 일반화할 수 있습니다. 이를 통해 공학 응용 분야에서 중요한 방정식들을 더 잘 시뮬레이션할 수 있게 됩니다.

- **Technical Details**: PINTO는 새로운 반복적 커널 적분 연산자 단위를 도입하여 cross-attention을 활용하여 PDE의 해결 영역 포인트를 초기/경계 조건 인식 표현 벡터로 변환합니다. 이러한 구조는 초기 및 경계 조건이 보이지 않는 새로운 시나리오에 대해서도 효율적으로 해결 함수를 학습할 수 있게 합니다. 본 논문에서는 유체 흐름을 모델링하는 여러 방정식들(advective, Burgers, Navier-Stokes 방정식)에 적용한 결과를 보여줍니다.

- **Performance Highlights**: 실험 결과, PINTO는 기존의 다른 물리 기반 연산자 학습 방법들보다 상대적인 오차가 1/5에서 1/3 정도로 낮아 뛰어난 성능을 발휘함을 보였습니다. 뿐만 아니라, 훈련 데이터에 포함되지 않은 시간 단계에서도 advection 및 Burgers 방정식을 정확하게 해결할 수 있습니다. 이를 통해 PINTO 모델이 향후 공학 및 자연 시스템의 시뮬레이션에 큰 도움이 될 것으로 기대됩니다.
...
### Stochastic Learning of Non-Conjugate Variational Posterior for Image Classification (https://arxiv.org/abs/2412.08951)
- **What's New**: 이 논문에서는 대규모 Bayesian 비모수 모델(BNP) 학습의 한계를 극복하기 위해 새로운 접근 방식을 제안합니다. 이를 위해 최근 제안된 변이 최대화-최대화(VMM) 학습기를 기반으로 하여 비자기화 추정기(non-conjugate posterior)에서 대규모 학습을 가능하게 합니다. 기존의 확률 변동 추론(SVI)과 달리, 이 방법은 변이 포스터리어(expectation of variational posterior)의 폐쇄형 표현을 요구하지 않습니다. 연구자는 VMM의 학습 성능을 MIT67 및 SUN397와 같은 대규모 클래스 데이터셋에서 비교하여 입증했습니다.

- **Technical Details**: 제안된 모델인 Pitman-Yor 프로세스 일반화 가우시안 혼합 모델(PYPM)은 새로운 BNP 모델로, 자극적(posteriors for conjugate, non-conjugate) 및 이산적인 포스터리어를 포함하는 특별한 속성을 가집니다. 이 모델은 변이 포스터리어 예측을 위해 MAP 추정을 사용하며, Adam 최적화 기법에서 영감을 받아 경량화된 스텝 사이즈를 통해 경량화된 학습을 제공합니다. 이렇게 얻은 학습 알고리즘은 전형적인 변이 베이즈 방식으로 동작하며, PYPM의 모든 숨겨진 변수를 반복적으로 학습합니다.

- **Performance Highlights**: 제안된 VMM 학습기는 ResNet18 특성과 잘 호환되어 MIT67 및 SUN397과 같은 대규모 클래스 데이터셋에서 뛰어난 성과를 보여줍니다. 연구 결과, 최소한 현재 수준의 최첨단 방법과 동일한 수준의 클러스터링 성과를 달성하거나 이보다 우수한 결과를 도출하였습니다. 이러한 성과는 기존의 변이 베이즈 및 몬테 카를로 메서드에 비해 계산 비용을 크게 줄일 수 있는 가능성을 보여줍니다.
...
### Federated Foundation Models on Heterogeneous Time Series (https://arxiv.org/abs/2412.08906)
Comments:
          Accepted by Main Track in AAAI'25

- **What's New**: 이 논문은 시계열 기초 모델(Foundation Models)을 훈련하기 위한 새로운 연합 학습(federated learning) 접근 방식인 FFTS를 제안합니다. FFTS는 독립적인 클라이언트로 데이터를 보유하는 조직을 취급하여 데이터의 고유 특성을 유지하는 로컬 모델을 훈련합니다. 이 모델은 중앙 집중식 데이터 통합 없이도 다양한 도메인에서 시계열 데이터를 효과적으로 분석할 수 있는 기회를 제공합니다.

- **Technical Details**: FFTS에서는 각 클라이언트가 고유의 데이터 세트를 기반으로 로컬 모델을 학습하고, 중앙 서버에서 이 모델들을 통합하여 최종 TSFM(Time Series Foundation Model)을 생성합니다. 모델 아키텍처에서는 표준 인코더-온리 Transformer를 사용하며, 동적 경향 인식 모듈을 도입하여 다양한 시계열 간 유사 패턴을 식별합니다. 최적화 과정에서는 균일한 마스킹 전략을 활용하여 도메인 특정 지식을 암기하는 것을 방지하고, 이질적인 지식 정렬 전략을 통해 글로벌 모델의 편향을 줄입니다.

- **Performance Highlights**: 이 논문에서 제안된 FFTS는 다양한 벤치마크 데이터 세트에서 강력한 제로/소수 샷 일반화(capability)를 보여주며, 예측, 데이터 보완, 이상 탐지와 같은 시계열 분석 작업에서 최신의 특정 작업 모델보다 우수한 성능을 나타냅니다. FFTS는 민감한 데이터 보호 문제를 해결하면서도 여러 도메인에서 시계열 데이터를 효과적으로 처리할 수 있다는 점에서 의미 있는 발전을 제시합니다.
...
### Lexico: Extreme KV Cache Compression via Sparse Coding over Universal Dictionaries (https://arxiv.org/abs/2412.08890)
Comments:
          18 pages, 7 figures

- **What's New**: 이 논문은 Lexico라는 새로운 KV 캐시 압축 방법을 제안합니다. 이 방법은 약 4,000개의 원자로 구성된 보편적인 사전을 활용하여 희소 코딩(sparse coding)을 사용합니다. 이는 다양한 입력 프롬프트(prompt), 작업(task), 모델에 대해 효율적인 압축이 가능함을 보여줍니다.

- **Technical Details**: Lexico는 삼 단계로 구성되며, 여기에는 사전 훈련(dictionary pretraining), 희소 분해(sparse decomposition), 경량 희소 계수(lightweight sparse coefficients)가 포함됩니다. 이 방법은 WikiText-103를 기반으로 사전을 훈련하며, 메모리 사용량이 일정하여 배치 크기(batch size)에 영향을 받지 않습니다. 또한, 2비트 양자화(quantization)보다 더 나은 압축을 이룰 수 있습니다.

- **Performance Highlights**: 실험 결과, Lexico는 GSM8K 및 LongBench와 같은 복잡한 언어 작업에서 원래 성능의 90-95%를 유지하면서 KV 캐시 메모리의 15-25%만 사용합니다. 이전의 압축 방법이 할 수 없는 낮은 메모리 환경에서도 높은 정확도를 유지하며 최대 1.7배의 압축률을 달성합니다.



### FAWAC: Feasibility Informed Advantage Weighted Regression for Persistent Safety in Offline Reinforcement Learning (https://arxiv.org/abs/2412.08880)
- **What's New**: 이 논문은 오프라인 데이터를 사용해 정책(policy)을 학습하면서 안전성 제약을 соблюдать하는 방법인 Safe offline reinforcement learning에 대해 다룹니다. 새로운 방법인 Feasibility Informed Advantage Weighted Actor-Critic (FAWAC)를 도입하여, 제약이 있는 마르코프 결정 과정(constrained Markov decision processes, CMDPs)에서의 안전성을 우선시합니다. FAWAC는 비모수적(policy optimization) 정책 공간에서의 안전한 정책 업데이트를 가능하게 하며, 파라메트릭(parametric) 공간으로의 투영을 통해 학습합니다.

- **Technical Details**: FAWAC는 Advantage Weighted Regression (AWR)에 비용-장점(cost-advantage) 항을 통합하였으며, 이를 통해 성과를 극대화하면서도 안전성 제약을 준수합니다. 이 방법은 특히 오프라인 데이터에서 파생된 타당성 조건(feasibility conditions)을 활용하여 정책 최적화를 국한합니다. FAWAC는 기존의 안전 장치들을 넘어서는 관점을 제공하며, 보다 도전적인 유혹 데이터셋이 포함된 문제를 효과적으로 해결할 수 있는 전략을 제안합니다.

- **Performance Highlights**: 실험적 평가 결과, FAWAC는 기존 표준 벤치마크에서 강력한 성과를 달성하였습니다. 이 방법은 정적(static) 데이터셋을 기반으로 학습한 정책에서 안전성과 성과를 효과적으로 조화시키는 능력을 보여주었습니다. FAWAC를 통해 안전성 및 성과 사이의 균형을 맞출 수 있는 유망한 접근법임을 입증하였습니다.
...
### Words of War: Exploring the Presidential Rhetorical Arsenal with Deep Learning (https://arxiv.org/abs/2412.08868)
- **What's New**: 이 연구는 미국 대통령의 연설에서 전쟁 발발의 징후를 식별하기 위해 딥러닝 기술을 적용하는 새로운 접근 방식을 제시합니다. 기존의 단순한 접근을 넘어, 다양한 머신러닝 기법을 통해 대통령의 수사적 언어의 미묘한 뉘앙스와 패턴을 해독하려고 합니다. 연구는 정치적 언어와 역사적 사건 간의 복잡한 상호작용을 탐색하며, Clausewitz의 전쟁 이론을 현대 기술 혁신과 결합하고자 합니다.

- **Technical Details**: 연구 데이터는 Kaggle에서 수집하였고, 미국 의회의 연구 서비스에서 제공하는 정보를 바탕으로 전쟁 발발일을 파악했습니다. 데이터 정제 후, 964개의 관측치가 있으며, 클래스 불균형 문제를 해결하기 위해 SMOTE 기법을 사용했습니다. BERT와 같은 선행 학습된 모델을 활용하여 텍스트 데이터를 처리하고, 여러 모델 구조를 실험하여 성과를 평가하였습니다.

- **Performance Highlights**: MLP 모델은 10회의 학습 에포크 후 0.7 이상의 정확도를 기록했습니다. LSTM을 포함한 RNN 모델은 10회 학습에서 정확도가 0.9를 넘겼고, Attention이 추가된 LSTM 모델은 성능이 향상되었습니다. 최종적으로 BERT 모델은 데이터셋에 맞춰 조정되어 가장 높은 성능을 발휘하며, 훈련 과정에서 스스로 중요한 텍스트 요소의 가중치를 조정했습니다.
...
- **What's New**: 본 논문에서는 Graph Neural Networks (GNNs)의 전통적인 이웃(neighborhood) 개념을 확장하여 'cover'라는 개념을 도입하고 있습니다. 이를 통해 새로운 메시지 전달(message passing) 전략을 개발할 수 있는 기반을 제시합니다. Grothendieck Graph Neural Networks (GGNN) 프레임워크를 통해 다양한 graph의 형태를 알바직 표현(algebraic representation)으로 변환하여, GNN 모델의 설계 범위를 넓힌다는 점에서 혁신적입니다.

- **Technical Details**: GGNN 프레임워크는 그래프를 모노이드(monoid)와 같은 대수적 구조 바탕으로 표현합니다. 이 구조는 directed subgraphs에 의해 생성된 cover의 집합으로, 이러한 cover는 matriz 형태로 변환되어 GNN의 메시지를 더욱 효과적으로 전달합니다.  또한, category theory에서의 sieve 개념을 활용하여 Sieve Neural Networks (SNN)라는 새로운 GNN 모델을 설계했습니다.

- **Performance Highlights**: SNN 모델은 GNN의 표현력을 검증하기 위한 벤치마크에서 우수한 성능을 입증하였습니다. GGNN 프레임워크의 유연성을 통해 다양한 그래프 아키텍처를 생성할 수 있으며, 이는 GNN 모델의 robust성과 적용성을 증대시키는 데 기여합니다. 결과적으로, 본 연구는 GNN의 설계 및 구현에서 새로운 관점을 제시하고 있습니다.



### Disentangling impact of capacity, objective, batchsize, estimators, and step-size on flow VI (https://arxiv.org/abs/2412.08824)
- **What's New**: 이 논문은 Normalizing flow 기반 변분 추론 (flow VI)의 성능을 개선하기 위한 단계별 분석을 진행합니다. 여러 알고리즘적 선택이 VI의 성능에 미치는 영향을 구분하기 위해 중요한 요소인 용량(capacity), 목표(objective), 경량 추정기(gradient estimators), 배치 크기(batchsize), 단계 크기(step-size)를 조사합니다. 이러한 분석을 통해 최적화 문제를 해결하고 표준 HMC 방법을 초월하는 성과를 제시합니다.

- **Technical Details**: Normalizing flow는 변분 가족을 형성하기 위해 일련의 가역적 변형(Invertible transformations)을 구성하여 Posterior distribution을 근사합니다. 이 논문에서는 다양한 프로그래밍 접근 방식을 활용하여 ELBO를 최대화하는 규칙을 소개하며, Normalizing flow의 변환을 위한 Diffeomorphism T를 이용한 밀도 함수에 대한 명확한 수식을 제공합니다. 이 과정에서 Jacobian의 행렬식(determinant)을 효율적으로 계산할 수 있도록 설계된 neural-network 기반 변형을 사용합니다.

- **Performance Highlights**: 연구 결과, high-capacity flow를 사용하면 복잡한 목표(objective) 없이도 우수한 성능을 얻을 수 있으며, 대규모 배치 크기(batchsize)와 결합할 때 최적화 성능을 극대화할 수 있음을 보여주었습니다. 또한, step-size의 적절한 범위 내 유지가 수렴에 매우 중요함을 확인했으며, 이로 인해 단순한 레시피로도 HMC 방법을 초월하는 결과를 얻을 수 있음을 나타냅니다. 최종적으로, 제안된 방법은 어려운 타겟에 대해 적은 평가 횟수로 우수한 결과를 달성합니다.
...
- **What's New**: 이번 논문에서는 수학 추론(math reasoning)에 대한 평가 데이터셋 'HARP(Human Annotated Reasoning Problems)'를 소개합니다. HARP는 미국 국가 수학 대회에서 가져온 5,409개의 문제로 구성되어 있으며, 이중 4,780개는 자동으로 검증 가능한 정답을 포함하고 있습니다. 논문에서는 프론티어 모델들의 성능을 평가하고, 특히 가장 어려운 문제에서 낮은 정확도를 나타내는 경향을 보였다고 보고합니다.

- **Technical Details**: HARP 데이터셋은 A(J)HSME, AMC, AIME, USA(J)MO와 같은 수학 대회에서 수집된 문제들로, 데이터셋의 주요 구성 요소는 4,780개의 단답형 문제입니다. 각 문제에는 원본 대회, 연도, 번호와 같은 메타데이터가 포함되고, 평균 두 개의 인간 작성 정답이 제공됩니다. 이 연구는 기존 데이터셋과의 중복 분석을 통해 HARP가 독창성을 유지하고 있다는 점을 강조합니다.

- **Performance Highlights**: HARP 데이터셋은 최신 모델들이 해결하는 데 어려움을 겪는 고난이도 문제를 포함하고 있습니다. 대표적으로 o1-mini와 Gemini 1.5 Pro 모델의 평균 정확도는 각각 41.1%와 9.6%에 그치며, 이는 수학 추론에서 모델의 한계를 나타냅니다. 논문에서는 다양한 모델의 성능을 평가하고, 특히 더 어려운 문제에 대해 추론 시간(inference-time) 컴퓨팅을 스케일링하는 경향을 분석했습니다.



### Test-Time Alignment via Hypothesis Reweighting (https://arxiv.org/abs/2412.08812)
Comments:
          Preprint

- **What's New**: 이 논문은 대규모 pretrained 모델이 미지정 작업에 어려움을 겪는 문제를 다루고 있습니다. 사용자 선호가 다양하고 상충할 수 있는 상황에서도 모델의 동적 적응을 요구하는 챗봇의 예를 통해 설명합니다. 제안된 HyRe 방법은 소량의 라벨된 예제를 사용하여 테스트 시에 앙상블 모델을 동적으로 재가중하여 사용자 의도에 맞추도록 트레이닝하는 혁신적인 기법입니다.

- **Technical Details**: HyRe는 여러 개의 예측 헤드를 갖는 하나의 신경망을 활용하여 다양한 기능을 표현하는 효율적인 앙상블을 구축합니다. 테스트 시간 동안 소량의 타겟 분포로부터의 예제를 기준으로 각 모델의 성능을 평가하고, 이를 바탕으로 동적으로 가중치를 조정합니다. 이 방법은 대규모 pretrained 모델을 위해 확장 가능하며, 단일 모델 미세 조정과 유사한 계산 비용을 요구합니다.

- **Performance Highlights**: HyRe 방법은 5개의 선호 쌍만으로 18개의 평가 분포에서 이전의 최첨단 모델보다 더 나은 정확도를 기록하였습니다. 특히, 개인화 설정에서 HyRe는 2B 파라미터 규모의 모델에서 평균 20%의 정확도 향상을 달성했습니다. 이러한 결과는 테스트 시간에 적은 라벨된 데이터로도 작업 미지정 문제를 효과적으로 해결할 수 있음을 시사합니다.



### Latent Safety-Constrained Policy Approach for Safe Offline Reinforcement Learning (https://arxiv.org/abs/2412.08794)
- **What's New**: 본 논문은 안전한 오프라인 강화 학습(safe offline reinforcement learning)에서 정책을 개발하는 새로운 방법론을 소개합니다. 기존 방법들이 안전 제약과 성능 간의 균형을 맞추는 데 어려움을 겪는 반면, 저자들은 Conditional Variational Autoencoders를 사용하여 보수적으로 안전한 정책을 학습하는 접근 방식을 제안합니다. 후속적으로, 이 방법은 제약된 보상-반환 극대화(constrained reward-return maximization) 문제로 구성되어 있습니다.

- **Technical Details**: 저자들은 라텐트 안전 제약을 추론하는 과정에서 보상-어드밴티지 가중 회귀(reward-Advantage Weighted Regression) 목표를 사용하여 인코더를 훈련시킵니다. 이 과정은 정책의 성능 및 샘플 복잡성에 대한 이론적 분석에 의해 뒷받침됩니다. 이론적 기초는 학습된 정책의 안전성과 성과의 상한을 제시하며, 이로 인해 효율적인 학습이 가능합니다.

- **Performance Highlights**: 포괄적인 실험 결과, 제안된 접근 방식은 기존 방법들보다 더 높은 누적 보상(cumulative reward)을 최적화하면서도 안전성을 유지하는 것으로 나타났습니다. 특히, 자율 주행 시나리오와 같은 도전적인 벤치마크 데이터셋에서 효과적인 성과를 보였습니다. 추가적인 시각화를 통해 이 방법의 효과와 기본 메커니즘에 대한 더 깊은 이해를 제공합니다.



### Bayesian optimized deep ensemble for uncertainty quantification of deep neural networks: a system safety case study on sodium fast reactor thermal stratification modeling (https://arxiv.org/abs/2412.08776)
- **What's New**: 본 연구에서는 Bayesian Optimization (BO)와 Deep Ensembles (DE)를 결합한 새로운 방법인 BODE를 제안합니다. 이 방법은 Deep Neural Networks (DNNs)의 예측 정확도와 불확실성 정량화(UQ)를 향상시키기 위해 개발되었습니다. BODE는 sodium fast reactor의 thermal stratification 모델링을 위한 eddy viscosity 예측 사례에서 성능을 평가합니다. 기존의 수동 조정된 앙상블에 비해 BODE는 노이즈가 없는 환경에서 총 불확실성을 약 4배 낮게 추정합니다.

- **Technical Details**: BODE는 Dense Connected Convolutional Neural Network (DCNN)를 사용하여 Computational Fluid Dynamics (CFD) 데이터에서 eddy viscosity를 예측하는 데 적용되었습니다. BODE는 알레아토릭 불확실성을 거의 제로에 가깝게 추정하는 반면, 기존 앙상블은 알레아토릭 불확실성을 과대 추정하는 경향이 있습니다. 이 연구는 DNN에 대한 하이퍼파라미터 최적화와 같은 다양한 기법들을 활용하여 불확실성을 줄이고 예측 성능을 향상시킵니다.

- **Performance Highlights**: BODE는 Gaussian 노이즈가 포함된 데이터에서 예측의 정확성과 불확실성을 잘 정량화합니다. 노이즈 표준편차가 5%와 10%로 설정된 경우에도 데이터 세트에 잘 맞추어 불확실성을 정확히 추정합니다. 결과적으로 BODE는 데이터 기반 모델에서 불확실성을 효과적으로 줄이고 예측을 향상시키는 유연한 접근 방식을 제공합니다.
...
### DeepNose: An Equivariant Convolutional Neural Network Predictive Of Human Olfactory Percepts (https://arxiv.org/abs/2412.08747)
Comments:
          8 pages, 8 figures, to be appear in the proceedings of Asilomar Conference on Signals, Systems, and Computers (ACSSC 2024)

- **What's New**: 본 논문에서는 후각 시스템이 분자 인식을 위해 어떻게 3D 공간 필터로 작용하는지를 탐구하고 있습니다. 연구진은 컨볼루션 신경망(Convolutional Neural Network, CNN)을 사용하여 사람의 후각 인식(prediction)을 예측하도록 훈련시키며, 이를 통해 새로운 후각 인식 모델인 DeepNose를 개발했습니다. 이 모델은 분자의 방향에 상관없이 일정한 반응을 보여줍니다.

- **Technical Details**: DeepNose 네트워크는 생물학적 시스템과 정렬되도록 설계되어 서로 다른 입체 이성질체(stereoisomers)에 대해 뚜렷한 인식 특성을 예측할 수 있습니다. 이 네트워크는 여러 분자를 동시에 처리할 수 있는 아키텍처를 채택하고 있어, 혼합된 냄새의 인식 질(perceptual quality)을 유추할 수 있는 기능을 가지고 있습니다. 또한, 각종 의미적 데이터셋(semantic datasets)을 통해 훈련 받아 다양한 후각 데이터셋에 대한 고충실도 예측을 달성하고 있습니다.

- **Performance Highlights**: DeepNose는 후각 인식에 필요한 분자 특성을 식별할 수 있으며, 이는 후각 품질(odor quality)을 결정짓는 분자 특징(molecular features)을 파악하는 데 도움을 줍니다. 또한, 이 모델은 특정 후각 묘사(perceptual descriptors)에 기여하는 분자 특징을 식별하는 능력이 뛰어납니다. 최종적으로, DeepNose는 3D 분자 형태를 활용하여 고품질 후각 예측을 생성하는 가능성을 제시합니다.
...
### Wait-Less Offline Tuning and Re-solving for Online Decision Making (https://arxiv.org/abs/2412.09594)
- **What's New**: 이 논문에서는 온라인 선형 프로그래밍(OLP)의 최신 알고리즘을 개선하여 수익 관리 및 자원 할당에 대한 효율성을 극대화하는 새로운 알고리즘을 제안합니다. 기존의 LP 기반 방법과 첫 번째 순서 방법의 장점을 결합하여, 정해진 주기 $f$마다 LP 하위 문제를 재해결하고 최적의 이중 가격을 이용해 온라인 의사결정을 지원합니다. 이러한 혁신적인 접근 방식은 OLP의 새로운 가능성을 열고, 결정 과정을 효율적으로 간소화합니다.

- **Technical Details**: 제안된 알고리즘은 $	ext{O}(	ext{log}(T/f) + 	ext{sqrt}(f))$의 회귀(regret) 성능을 보이며, LP 기반 방법의 우수한 보장성과 첫 번째 순서 방법의 계산 효율성 간의 균형을 유지하는 'wait-less' 온라인 결정 과정을 제공합니다. 논문은 앵커링(anchoring)에 대한 가정들과 더불어, LP 기반 및 첫 번째 순서 방법에 대한 기초 분석을 통해 알고리즘의 이론적 기초를 제공합니다. 또한, 지원하는 수학적 프레임워크를 통합하여 모든 OLP 방법의 고유한 특성을 도출합니다.

- **Performance Highlights**: 제안된 알고리즘은 대규모 응용 분야에서의 효율성을 극대화하는 동시에, 계산 비용을 줄이고 성능을 향상시키기 위한 전략을 제공합니다. 실험 결과, 최신 듀얼 가격 접근 방식이 서브선형 수렴(sublinear convergence)을 달성하는 것으로 나타났으며, 이는 온라인 결정을 더욱 안정적이고 빠르게 만들어줍니다. 이러한 성능 개선은 특히 재무 및 리소스 관리 웹 애플리케이션에서 실질적인 이점을 제공합니다.
...
### Improving the Reliability of Cable Broadband Networks via Proactive Network Maintenanc (https://arxiv.org/abs/2412.09564)
Comments:
...
- **What's New**: 이번 연구에서는 CableMon이라는 새로운 시스템을 제안하여 Proactive Network Maintenance (PNM) 데이터를 사용하여 케이블 브로드밴드 네트워크의 신뢰성을 향상시키고 있습니다. 기존의 공공 도구들이 높은 허위 긍정률로 문제를 겪는 반면, CableMon은 머신 러닝 기술을 활용하여 오류를 더 정확하게 탐지할 수 있도록 돕습니다. 이 시스템은 케이블 인터넷 서비스 제공업체(ISP)가 장애를 감지하고 진단하는 데 중점을 두고 설계되었습니다.

- **Technical Details**: CableMon은 통계 모델을 사용하여 시간 시리즈 데이터로부터 특징을 생성하고 고객의 문제 티켓을 활용하여 이상/장애 임계값을 추론합니다. 또한 비지도 학습 모델을 통해 유사한 이상 패턴을 공유하는 케이블 장비 그룹을 형성하여 케이블 네트워크 내에서 발생하는 장애와 가입자의 장소에서 발생하는 장애를 구별할 수 있습니다. 이를 통해 신속한 수리 인력 배정이 가능하게 됩니다.

- **Performance Highlights**: CableMon의 성과를 평가하기 위해 8개월의 PNM 데이터와 고객 문제 티켓을 분석한 결과, CableMon이 탐지한 장애의 81.9%가 상대적으로 정확하게 고객 문제 티켓으로 이어졌습니다. 반면, 기존의 AnonISP 도구는 10.0%, Comcast 도구는 23.5%만이 고객 티켓으로 이어졌습니다. 또한, CableMon은 네트워크 관련 티켓의 23.0%를 예측했으며, 이는 장애 발생의 긴급성을 좀 더 잘 반영하고 있다는 것을 나타냅니다.



### Experimental Machine Learning with Classical and Quantum Data via NMR Quantum Kernels (https://arxiv.org/abs/2412.09557)
Comments:
          8 pages, 5 figures

- **What's New**: 이 논문에서는 고차원 공간으로 데이터를 매핑하는 커널 방법(Kernel methods)을 퀀텀 컴퓨팅(Quantum computing)과 결합하여 퀀텀 커널(quantum kernel) 방법을 실험적으로 구현하였습니다. 연구팀은 10 큐비트(10-qubit) 스타 토폴로지 스타 시스템을 사용하여 데이터 종속 유니타리 변환(data-dependent unitary transformations)을 통해 고전 데이터를 인코딩하고, 일차 회귀(one-dimensional regression) 및 이차 분류(two-dimensional classification) 작업을 수행했습니다.

- **Technical Details**: 본 연구에서는 퀀텀 커널 방법을 액체 상태의 핵 자기 공명(NMR) 레지스터에서 구현하였으며, 기존의 고전적인 커널 방법을 확장하여 비모수적(non-parametrized) 연산자 입력을 처리할 수 있도록 설계되었습니다. 논문에서는 커널 함수(kernel function)가 이러한 양자 상태(quantum states) 간의 겹침(overlap)을 계산하는 과정을 설명하며, 이를 통해 양자 데이터(quantum data) 및 양자 작업(quantum tasks)을 처리하는 방법론을 제시하였습니다.

- **Performance Highlights**: 연구에서 제안한 양자 커널은 얽힘(entanglement)이 있는 유니타리와 비얽힘(unentangled) 유니타리를 분류하는 데 성공적인 결과를 보였습니다. 이 결과들은 양자 커널이 고전적 및 양자 기계 학습 작업(classical and quantum machine learning tasks) 모두에서 강력한 능력을 발휘함을 보여줍니다. 앞으로의 연구에서는 보다 복잡한 양자 컴퓨팅 응용 프로그램을 위한 길을 열 것으로 기대됩니다.



### Enhancing Convergence of Decentralized Gradient Tracking under the KL Property (https://arxiv.org/abs/2412.09556)
Comments:
          25 pages, 4 figures

- **What's New**: 이번 연구에서는 비선형(nonconvex) 및 비매끄러운(nonsmooth) 최적화 문제의 분산 해결 방법에 대해 다루었습니다. 특히, SONATA라는 분산 그래디언트 추적 기반 알고리즘의 수렴 속도를 첫 번째로 포괄적으로 특성화했습니다. 저자들은 Kurdyka-Łojasiewicz (KL) 속성을 통해 제시된 여러 비선형 문제에 대한 수렴 보장을 제공합니다.

- **Technical Details**: 논문에서는 각 에이전트가 자체 비용 함수를 가지고 있는 비중첩 그래프 네트워크에서의 다중 에이전트 최적화 문제를 고려합니다. 각 에이전트의 비용 함수는 Li-smooth 하며, 대리자이자 공유 가능한 비매끄러운(convex extended-value) 함수도 정의되고, 이러한 구조가 해결책에 제약을 가합니다. 연구자는 KL 속성을 활용하여 SONATA 알고리즘의 수렴 속도를 구체적으로 분석하였습니다.

- **Performance Highlights**: 숫자적 결과는 이론적 발견을 검증하며, KL 속성을 가진 비선형 문제에서 SONATA의 응용으로 강력한 수렴 속도를 보였습니다. 본 연구는 에르되시-레니(Erdos-Renyi) 그래프 구조를 사용하여 LASSO 문제와 PCA 문제에서의 수렴을 실험했습니다. 그 결과 SONATA의 반복이 수렴하는 방식이 이론적으로 뒷받침되지 않던 점을 발전시키며 새로운 통찰을 제공합니다.
...
### GainAdaptor: Learning Quadrupedal Locomotion with Dual Actors for Adaptable and Energy-Efficient Walking on Various Terrains (https://arxiv.org/abs/2412.09520)
Comments:
          8 pages, 6 figures

- **What's New**: 본 논문에서는 GainAdaptor라는 새로운 적응형 이득(control gain) 조절 프레임워크를 제안합니다. 이 시스템은 다리 로봇이 다양한 지형에서 에너지 효율성을 높이면서 적응성을 향상시키기 위한 이득 조절을 자동으로 수행합니다. 이중 행위자(dyad actor) 알고리즘을 활용하여 지면 조건에 따라 PD 이득을 동적으로 조정합니다. 또한, 이 프레임워크는 다양한 환경에서 로봇의 이동성을 시험하는 실험을 통해 그 효과성을 입증하였습니다.

- **Technical Details**: GainAdaptor 프레임워크는 두 가지 주요 행위자(actor)를 통한 PD 이득 동적 조정 메커니즘을 포함합니다. 첫 번째 행위자는 로봇의 관절에 대한 비례(P) 및 미분(D) 이득을 조정하여 지형에 대한 적응성을 높이고, 두 번째 행위자는 이러한 최적화된 값으로 로봇의 실체적 움직임을 조정합니다. 이 구조는 환경의 변화에 따라 움직임을 안정적이고 효율적으로 제어할 수 있도록 하며, 초기 PD 이득 값(예: P=28, D=0.7)을 설정하여 다양한 지형에서 반응성을 확보합니다.

- **Performance Highlights**: 실험 결과, GainAdaptor는 다양한 지형에서 로봇의 이동 성능을 향상시켰음을 보여줍니다. 토크 소비와 전력을 최소화하면서 안정적인 이동을 가능하게 하여 전반적인 에너지 효율성을 증가시킵니다. 이 시스템은 배터리 효율성 개선과 작동 시간을 연장함으로써, 극복해야 할 도전 과제를 해결하는 데 기여합니다.



### Loss function to optimise signal significance in particle physics (https://arxiv.org/abs/2412.09500)
Comments:
          9 pages, 4 figures. Appeared in the Machine Learning for Physical Sciences (ML4PS) workshop in NeurIPS 2024 conference

- **What's New**: 이번 연구에서는 입자 물리학에서 사용되는 중요성 메트릭을 직접 최적화하기 위한 대리 손실 함수(surrogate loss)를 구축했습니다. 이 손실 함수는 간단한 이벤트 분류 작업에서 선형 모델을 통해 평가되었으며, 관련된 프로세스의 교차 섹션(cross sections) 변화에 따라 결정 경계가 변하는 것을 보여주었습니다. 특히, 새로운 손실로 훈련된 모델이 교차 엔트로피 손실로 훈련된 모델과 비교했을 때 유사한 중요성 값에서 더 높은 신호 효율성을 보임으로써 입자 물리학에서의 민감도를 향상시킬 가능성을 제시합니다.

- **Technical Details**: 입자 물리학 실험에서 신호와 배경을 구분하기 위해 다변량 분류기(multivariate classifiers)가 필수적이며, 본 연구에서는 전통적인 교차 엔트로피 손실(BCE) 대신 새로운 손실 함수를 통해 실험적 민감도를 직접 높이는 것을 목표로 합니다. Z 점수(Z score)는 사건 집합에 대한 함수이기 때문에, 우리는 Lovász 확장을 통해 이를 최대화하는 대리 손실 함수를 정의하고, 이 손실을 사용하여 선형 모델에 기초한 사건 분류 작업에서 성능을 평가하였습니다.

- **Performance Highlights**: 실험 결과, 새로운 대리 손실 함수를 사용한 모델이 기존 접근 방법에 비해 더욱 향상된 신호 분류 능력을 보였습니다. 또한, 이러한 접근은 LHC와 같은 대형 입자 충돌기에서 새로운 물리 현상을 탐색하는 과정에서 보다 높은 민감도를 제공할 수 있는 가능성을 보여줍니다. 이 연구는 입자 물리학 연구에서 머신 러닝 기법의 효과적인 활용을 위한 중요한 기초 자료가 될 것입니다.
...
- **What's New**: 이번 연구에서는 기계 학습(ML) 기법을 활용하여 플러그인 하이브리드 전기차(PHEVs)의 친환경 교통 시스템을 위한 혁신적인 제어 시스템을 소개합니다. 이 시스템은 전기 모드에서의 에너지 사용을 예측하고 다양한 작동 모드에 따라 전력 할당을 최적화합니다. 엄격한 운전 조건에서 성능 평가를 통해 새로운 제어 방식이 기존 시스템에 비해 효율성을 크게 향상시킨 것으로 나타났습니다.

- **Technical Details**: 제어 시스템은 퍼지 논리(fuzzy logic) 의사결정 과정을 기반으로 작동하며, 순수 전기 모드에서의 효율성을 극대화합니다. 20-kWh 배터리 팩의 80% 활용으로 약 84km의 전기 주행 범위를 달성하였습니다. 또한, WLTC(Worldwide Harmonized Light Vehicles Test Cycle) 주행 사이클에서는 연료 소비량이 100km당 2.86L로 줄어들어, 가솔린 동등 연료 소비량이 20% 감소했습니다.

- **Performance Highlights**: 차량 성능 평가는 다양한 속도 조건 하에 시행되었으며, 저속에서는 충전을, 고속에서는 방전하는 효과적인 에너지 관리를 보여주었습니다. 초기 배터리 충전 수준은 성능에 큰 영향을 미치며, 90% 충전으로 지속적인 전기 모드 운전을 가능하게 하여 연료 소비를 2L/100km까지 감소시켰습니다. 실제 주행 패턴 분석 결과, 짧고 느린 주행 사이클에서 전기 추진이 우선시되어 연료 소비가 낮아지는 경향을 보였고, 반대로 긴 빠른 사이클에서는 내연기관 사용이 증가하는 것을 확인했습니다.
...
### Finite-PINN: A Physics-Informed Neural Network Architecture for Solving Solid Mechanics Problems with General Geometries (https://arxiv.org/abs/2412.09453)
- **What's New**: 이 연구는 PINN(Physics-Informed Neural Networks)을 사용하여 고체 역학 문제를 해결할 때의 두 가지 주요 도전 과제를 식별합니다. 첫 번째 과제는 PINN 모델이 무한한 영역에서 솔루션을 생성하는 데 반해, 고체 구조의 경우 유한한 경계가 일반적이라는 점입니다. 두 번째 과제는 고체 구조에서 자주 발생하는 복잡한 기하학을 다루기에 충분하지 않은 유클리드 공간(Euclidean space)을 이용하는 것입니다. 이를 해결하기 위해 이 논문은 PINN의 구현을 최대한 보존하면서 일반 고체 역학 문제를 해결하기 위한 새로운 아키텍처인 Finite-PINN 모델을 제안합니다.

- **Technical Details**: Finite-PINN 모델은 응력 및 변위 필드의 근사를 분리하고, 솔루션 공간을 기존의 유클리드 공간에서 유클리드-위상적 접합 공간(Euclidean-topological joint space)으로 변환하는 독특한 아키텍처를 갖추고 있습니다. 이 모델은 PINN의 원래 구조에서 많은 부분을 유지하면서도 고체 역학 문제에 적용할 때의 복잡함을 해결하도록 설계되었습니다. 여러 사례 연구를 통해 Finite-PINN 모델이 2D와 3D 환경에서 모두 원활히 작동하면서 다양한 문제 유형에 대해 만족스러운 결과를 제공함을 보여주고 있습니다.

- **Performance Highlights**: Finite-PINN 모델은 특히 연구가 충분히 이루어지지 않은 일반 고체 역학 문제에 대한 유망한 도구로 보여집니다. 모델은 고체 역학 내에서의 비선형ity 및 복잡한 기하학을 효과적으로 처리할 수 있는 능력을 갖추고 있습니다. 기존의 PINN 모델로는 어려움이 있었던 다양한 문제들에 대해, Finite-PINN은 좀 더 안정적이고 유연하게 접근할 수 있는 가능성을 제공합니다.



### Data Efficient Prediction of excited-state properties using Quantum Neural Networks (https://arxiv.org/abs/2412.09423)
Comments:
          10 + 4 pages, 7 + 3 figures

- **What's New**: 이 논문에서는 분자 바닥 상태(ground state)로부터 여러 형태의 기하학적 배열에서 여 상태(excited states) 속성을 예측하는 양자 기계 학습(quantum machine learning) 모델을 제안합니다. 이 모델은 대칭 불변 양자 신경망(symmetry-invariant quantum neural network)과 전통적인 신경망(classical neural network)으로 구성되어 있으며, 적은 양의 학습 데이터로도 높은 정확도를 제공합니다. 제안된 방법은 완전한 NISQ(노이즈가 존재하는 중간 규모 양자) 시스템에서 호환 가능하며, 필요한 측정 수를 줄입니다.

- **Technical Details**: 모델은 단순한 측정 관측량을 사용하여 분자의 바닥 상태에서 필요한 속성을 추출하며, 이는 양자 컴퓨터에서 효율적인 계산을 가능하게 합니다. 본 연구에서는 3가지 분자에 대해 여 상태 전이 에너지(transition energies)와 전이 쌍극자 모멘트(transition dipole moments)를 예측하여 성능을 평가합니다. 제안된 양자 신경망(QNN)은 바닥 상태의 대칭을 고려하여 구축되었으며, 상당히 적은 학습 데이터 포인트만으로 여 상태 속성을 예측할 수 있습니다.

- **Performance Highlights**: 실험 결과, 바닥 상태를 추가 특성으로 사용할 때 예측 품질이 크게 개선되었으며, 특히 저데이터 환경에서 효과적임을 입증했습니다. 제안된 방법은 여러 기존의 고전적 모델보다 여러 경우에서 더 나은 성능을 발휘했습니다. 연구를 통해 고전적 방법과 양자 기계 학습 방법이 결합될 때의 이점이 입증되었습니다.
...
### Opinion de-polarization of social networks with GNNs (https://arxiv.org/abs/2412.09404)
- **What's New**: 이 연구는 두 개의 에코 챔버가 공존하는 네트워크에서 의견 극단화를 줄이기 위한 방법을 탐구합니다. 사용자가 중립적인 의견을 채택할 경우 네트워크의 극단화가 감소한다는 사실을 확인했습니다. 이를 바탕으로 K명의 사용자가 중도적 입장을 채택했을 때 극단화를 최소화할 수 있는 효과적인 알고리즘을 제안하고 있습니다.

- **Technical Details**: 사회 네트워크는 그래프 G(V,E)로 표현되며, 각 노드는 내부 의견과 외부 의견을 갖습니다. Friedkin과 Johnsen의 의견 형성 모델을 채택하여, 각 노드의 외부 의견은 연결된 다른 노드의 의견과 내부 의견의 타협으로 업데이트됩니다. 이 모델은 네트워크의 극단화를 수치적으로 정량화할 수 있는 장점을 제공합니다.

- **Performance Highlights**: 제안된 알고리즘은 Graph Neural Network를 활용하여 대규모 그래프에서도 효과적으로 작동합니다. 기존의 연구들과 비교하여 실행 가능성을 확보하며, K명의 사용자를 선택하여 네트워크 극단화를 최적화하는 데 초점을 맞추고 있습니다. 이를 통해 소셜 미디어 플랫폼에서의 극단화 문제를 해결할 수 있는 새로운 접근법을 제시합니다.
...
### Distribution free uncertainty quantification in neuroscience-inspired deep operators (https://arxiv.org/abs/2412.09369)
- **What's New**: 이 논문에서는 에너지 효율적인 딥러닝 알고리즘의 필요성을 강조하며, Spiking Neural Networks (SNNs)와 Uncertainty Quantification (UQ)을 결합한 새로운 Conformalized Randomized Prior Operator (CRP-O) 프레임워크를 제안합니다. 특히 Gaussian Process Regression을 활용하여 zero-shot super-resolution을 가능하게 합니다. 이 프레임워크는 Variable Spiking Wavelet Neural Operator (VSWNO)와 통합되어, 예측의 신뢰성을 높이며, 다양한 응용 프로그램에서의 실용 가능성을 보여줍니다.

- **Technical Details**: CRP-O 프레임워크는 Randomized Prior (RP) 네트워크와 Split Conformal Prediction (SCP)을 이용하여 기존 및 스파이킹 신경 연산자의 불확실성을 정량화합니다. 초기 불확실성 추정치는 RP 네트워크를 통해 얻어지고, 이후 SCP를 통해 보정되어 Conditional Coverage (CC)의 조건을 만족합니다. 이 프레임워크는 두 개의 데이터 세트, 즉 훈련 데이터 세트와 보정 데이터 세트를 필요로 하여 데이터 부족 문제를 해결하는 데 기여합니다.

- **Performance Highlights**: CRP-VSWNO를 사용한 결과는 기존의 RP-VSWNO, Quantile WNO (Q-WNO), Conformalized Quantile WNO (CQ-WNO)보다 유의미한 개선을 보여주었습니다. 논문에서 다룬 네 가지 예제는 CRP-O 프레임워크의 적용 가능성과 성능을 뒷받침하며, 제안된 접근법이 실제 응용 프로그램에서의 활용 가능성을 강조합니다. 이러한 발견은 스파이킹 신경망의 에너지 효율적인 학습을 기대할 수 있는 새로운 방향을 제시합니다.



### Diffusion Predictive Control with Constraints (https://arxiv.org/abs/2412.09342)
Comments:
          Code: this https URL. 14 pages, 3 figures, 3 tables

- **What's New**: 최근 로봇 공학에서 정책 학습을 위한 Diffusion 모델의 활용이 증가하고 있습니다. 그러나 Diffusion 정책은 본질적으로 Stochastic하며, 훈련된 데이터에 나타나지 않는 새로운 제약 조건을 처리하는 데 제한이 있습니다. 이를 해결하기 위해, 우리는 제약이 있는 Diffusion Predictive Control(DPCC) 알고리즘을 제안합니다.

- **Technical Details**: DPCC는 상태 및 행동 제약을 명시적으로 적용할 수 있는 확장된 방법으로, 기존의 훈련 데이터에서 벗어날 수 있는 제약 조건을 처리하도록 설계되었습니다. 이 알고리즘은 제약 강화(constraint tightening)와 모델 기반 투영을 통합하여 훈련된 궤적의 Noise를 제거하는 과정에 적용합니다. 결과적으로, DPCC는 동적으로 실현 가능하고 제약 조건을 충족하며 목표를 달성하는 궤적을 생성합니다.

- **Performance Highlights**: 시뮬레이션된 로봇 매니퓰레이터를 통해 DPCC는 새로운 테스트 조건을 충족하는데 있어 기존의 방법들보다 우수한 성능을 보였습니다. 이 연구는 Diffusion 기반 제어 정책의 최첨단을 발전시키고 안정적인 환경에서 새로운 제약 조건을 만족시키도록 설계되었습니다.
...
### Score and Distribution Matching Policy: Advanced Accelerated Visuomotor Policies via Matched Distillation (https://arxiv.org/abs/2412.09265)
Comments:
          17 pages

- **What's New**: 이번 연구에서는 Score and Distribution Matching Policy (SDM Policy)를 제안하여 기존의 diffusion-based 정책을 단일 단계 생성기로 변환함으로써 효율성을 높이고, 실제 테스트에서 6배의 추론 속도를 기록하며 최신의 액션 품질을 유지할 수 있음을 보여줍니다. 또한, 안정성을 위해 고정된 교사 모델과 적대적 훈련을 위한 고정되지 않은 교사 모델을 통합한 이중 교사 메커니즘을 도입하여 모델의 견고성과 타겟 분포에 대한 정렬을 효과적으로 향상시킵니다.

- **Technical Details**: SDM Policy는 두 단계의 최적화 프로세스를 기반으로 하며, 첫 번째 단계인 점수 매칭(score matching)은 생성된 액션이 실제 액션 분포와 밀접하게 일치하도록 보장합니다. 두 번째 단계는 분포 매칭(distribution matching)으로, KL 발산(KL divergence)을 최소화하여 일관성을 확보합니다. 이러한 접근 방식은 기존의 반복적인 diffusion 프로세스와는 달리 단일 단계에서 고품질 액션을 직접적으로 생성할 수 있도록 하여 추론 시간을 대폭 단축합니다.

- **Performance Highlights**: SDM Policy는 57개 작업의 시뮬레이션 벤치마크에서 평가되었으며, 표준 diffusion 정책 대비 6배의 추론 속도를 달성하고, 동시에 최신의 액션 품질을 유지하는 성과를 보여주었습니다. 이러한 결과는 고주파 로보틱 작업을 위한 효율적이고 신뢰성 있는 프레임워크를 제공하며, 다양한 로봇 작업에서의 적용 가능성을 나타냅니다.
...
- **What's New**: 본 논문은 음성 프라이버시 보호를 위한 새로운 접근 방식인 적대적(Adversarial) 방해물의 가역성(Reversibility)을 연구합니다. 기존 연구들이 원래 음성을 완전히 복원하는 데 실패했던 점을 개선하기 위해, 방해물 생성기와 제거 모듈을 동시에 훈련시키는 공동 훈련(Joint Training) 프레임워크를 제안합니다. 이를 통해 원래 음성을 효과적으로 복원할 수 있는 가능성을 보여줍니다.

- **Technical Details**: 논문에서 제안하는 방법은 대칭 주의 기반 인코더-디코더(Symmetric Saliency-Based Encoder-Decoder, SSED) 모델을 활용하여 적대적 방해물을 생성합니다. 방해물 제거 모듈은 방해물이 추가된 음성을 통해 방해물을 추정하고 이를 원래 음성에서 제거하여 복원합니다. 이러한 방식은 파라미터의 조정 없이도 새로운 형태의 음성 복원을 가능하게 합니다.

- **Performance Highlights**: 실험 결과, LibriSpeech 데이터셋을 사용한 평가에서, 생성된 적대적 방해물이 효과적으로 예측되었고, 복원된 음성의 품질이 자동 음성 인식(Automatic Speech Recognition, ASR) 및 화자 검증에서 우수한 성능을 보였습니다. 복원된 음성의 성능은 기존 음성의 특성과 일관되며, 보안 및 포렌식 분석에 유용할 것으로 기대됩니다.



### Dimensionality Reduction Techniques for Global Bayesian Optimisation (https://arxiv.org/abs/2412.09183)
Comments:
          Accepted at NeurIPS 2024 Workshop OPT for ML: Optimization for Machine Learning (Submission Number:67)

- **What's New**: 이 논문에서는 Latent Space Bayesian Optimisation (LSBO)이라는 새로운 접근 방식을 탐구합니다. 기존의 LSBO 방법이 (linear) random projections를 사용한 것과 달리, 이 연구는 Variational Autoencoders (VAEs)를 적용하여 복잡한 데이터 구조를 다루고 차원 축소(Dimensionality Reduction, DR) 작업을 확장합니다. 또한, 알고리즘을 넓은 최적화 목표를 위해 재구성하고, Gaussian Processes에서 Matérn-$\frac{5}{2}$ 커널을 사용하는 내용을 포함합니다.

- **Technical Details**: Bayesian Optimisation (BO)의 두 가지 기본 요소는 GP prior와 acquisition function입니다. 데이터셋의 값은 Gaussian Random Vector (GRV)로 모델링되며, Matérn-5/2 커널을 사용하여 각 점의 함수값을 예측합니다. 이 연구에서는 Sequential Domain Reduction (SDR)을 BO에 통합하여 효율성을 개선하고, BoTorch 프레임워크 내에서 GPU 기반 계산을 활용합니다.

- **Performance Highlights**: 구조화된 잠재 매니폴드가 BO의 성능을 향상시키는 것으로 나타났습니다. 이 알고리즘은 VAE 재훈련과 깊은 거리 손실(deep metric loss)를 통해 최적화 과정에서 유리한 결과를 도출하며, 기존의 BO 기법과 SDR을 조합하여 성능을 비교하고 있습니다. 전반적으로, 새로운 BO-VAE 알고리즘이 저차원 문제에서 REMBO 방법과의 비교를 통해 효과적인 최적화 성능을 보여주고 있습니다.



### $(\epsilon, \delta)$-Differentially Private Partial Least Squares Regression (https://arxiv.org/abs/2412.09164)
Comments:
          14 pages, 5 figure

- **What's New**: 이 연구는 데이터 프라이버시가 점점 더 중요해지는 현대 사회에서 Partial Least Squares (PLS) 회귀 모델을 위한 새로운 프라이버시 보호 알고리즘인 $(\,oldsymbol{ϵ}, \, oldsymbol{δ})$-differentially private PLS (edPLS)를 제안합니다. 기존의 PLS 모델은 프라이버시 보장이된 방식으로 작동하지 않아 훈련 데이터가 공격에 취약했습니다. edPLS는 이러한 문제를 해결하기 위해 가우시안 노이즈 추가 메커니즘을 통합하여 모델의 출력에 노이즈를 추가합니다.

- **Technical Details**: edPLS 알고리즘은 PLS의 네 가지 주요 함수인 가중치(weight), 점수(score), $X$-로드 (loadings), 및 $Y$-로드에 조정된 가우시안 노이즈를 추가하는 방식으로 동작합니다. 노이즈 분산은 각 함수의 전역 민감도(global sensitivity)를 바탕으로 결정되며, $(ϵ, δ)$-differential privacy 프레임워크에 따라 프라이버시 손실을 관리합니다. 이를 통해 edPLS는 훈련 데이터의 특정 변동성을 회복하려는 프라이버시 공격을 무력화하는 효과를 보입니다.

- **Performance Highlights**: NIR 옥수수 벤치마크 데이터 세트에 대한 적용 실험 결과, edPLS는 프라이버시 레벨이 강할지라도(예: $ϵ=1$), 적절한 전처리가 이루어질 경우 예측의 평균 제곱 근오차(RMSEP)를 경쟁력 있는 수준으로 유지할 수 있음을 보여줍니다. 이러한 연구 결과는 edPLS가 프라이버시 보호 다변량 캘리브레이션과 그에 따른 프라이버시-유용성(trade-off) 분석에서 실질적인 유용성을 갖고 있음을 강조합니다.
...
### Student-Informed Teacher Training (https://arxiv.org/abs/2412.09149)
- **What's New**: 이 논문에서는 특권(privileged) 교사의 도움을 받아 모방 학습(imitation learning)이 효율적으로 수행될 수 있는 새로운 프레임워크를 제안합니다. 교사는 특정 작업 정보에 기반하여 학습되고, 학생은 제한된 관측을 통해 교사의 행동을 예측하려고 합니다. 연구의 핵심은 학생의 부분 관측으로 인한 행동 모방의 어려움을 해결하는 것입니다.

- **Technical Details**: 단일 정책을 학습하는 대신, 교사와 학생 정책을 공동으로 학습하는 과정을 통해 정보 비대칭(information asymmetry)을 극복합니다. 교사의 보상 함수는 교사와 학생 간의 행동 차이를 벌칙 항으로 포함하여 학생이 모방할 수 있는 행동을 학습하도록 유도합니다. 또한, 교사가 환경과 상호작용하는 동안 학생의 능력을 동적으로 예측하여 고차원 데이터 처리의 필요성을 줄입니다.

- **Performance Highlights**: 제안된 방법은 미로 탐색 및 복잡한 비전 기반 쿼드로터 비행(task) 등 다양한 작업에서 효과적으로 작동하였습니다. 교사는 학생의 제한된 관측능력을 고려하여 행동을 수정함으로써 자율주행, 조작 등 여러 환경에서 성공률을 향상시켰습니다. 이로 인해 학생의 성과가 크게 향상되었으며, 모든 환경에서 학생과 교사 간의 성과 격차가 줄어드는 결과를 보여주었습니다.
...
### Motor Imagery Classification for Asynchronous EEG-Based Brain-Computer Interfaces (https://arxiv.org/abs/2412.09006)
- **What's New**: 최근 연구에서는 Motor Imagery (MI) 기반 비침습적 뇌-컴퓨터 인터페이스 (BCI) 시스템이 빠르게 발전하고 있습니다. 본 논문의 핵심은 비동기식 BCI를 위한 sliding window prescreening and classification (SWPC) 접근법을 제안하여, 고정된 트리거 없이 사용자의 MI를 효과적으로 탐지하는 방법을 소개합니다. SWPC는 두 가지 모듈로 구성되어 있으며, MI와 안정 상태를 구분하는 데 중점을 두고 있습니다.

- **Technical Details**: SWPC는 preprocess와 classification의 두 가지 주요 모듈을 포함하여 설계되었습니다. 첫 번째 모듈은 supervised learning과 self-supervised learning(SSL)을 통해 훈련된 EEGNet을 사용하여 MI를 안정 상태에서 구별합니다. 두 번째 모듈은 각 MI 시험을 분류하고, 두 모듈 모두 피쳐 추출기의 성능을 향상시킬 수 있도록 훈련됩니다.

- **Performance Highlights**: SWPC의 효과는 네 가지 MI 데이터 세트에서 수행된 실험을 통해 검증되었습니다. 결과적으로 SWPC는 항상 평균 분류 정확도에서 최고의 성능을 달성하였으며, 각 데이터 세트의 기존 최첨단 방법들보다 약 2% 더 우수한 성능을 보였습니다. 이 연구에서 제안된 방법은 향후 비동기식 BCI 분야의 발전에 기여할 것으로 기대됩니다.



### Stellar parameter prediction and spectral simulation using machine learning (https://arxiv.org/abs/2412.09002)
Comments:
          Accepted for publication in Astronomy & Astrophysics

- **What's New**: 이번 연구는 ESO의 HARPS 기기의 전체 데이터 역사에 기계 학습(ML)을 적용하여 관측된 객체의 물리적 특성을 복구하고 스펙트럼을 시뮬레이션하는 데 중점을 두었습니다. 연구진은 시뮬레이션 데이터 사용, 훈련 데이터 양 변화, 네트워크 아키텍처 및 학습 패러다임의 영향을 체계적으로 조사하여 결과의 정확도와 신뢰성을 높였습니다. 특히, 자동 인코더 프레임워크 내에서 지도 학습(Supervised Learning)과 비지도 학습(Unsupervised Learning) 기술을 융합하여 효과적인 예측과 스펙트럼 생성을 달성했습니다.

- **Technical Details**: 연구진은 HARPS 데이터에 대해 표준 및 변분 자동 인코더(Variational Autoencoders)를 훈련시켜 스펙트럼 매개변수를 예측하고 스펙트럼을 생성했습니다. 이 모델은 효과적인 온도에 대한 평균 예측 오차가 약 50 K로 대부분의 천체물리학적 응용에 적합합니다. 데이터를 처리하는데 필요한 시간은 CPU에서 779.6 ms, GPU에서 3.97 ms로, 대규모 분광 조사 및 대규모 아카이브 연구에 유용합니다.

- **Performance Highlights**: 모델은 금속 성분([M/H])과 표면 중력(log g)을 각각 약 0.03 dex와 0.04 dex의 정확도로 예측하는 데 성공했습니다. 이러한 성과는 천체 물리학 연구에서 널리 활용될 수 있음을 강조하며, 기존 방법들에 비해 정확성을 유지하면서도 현저하게 줄어든 계산 시간을 제공하여 분광 분석의 범위와 효율성을 향상시켰습니다.



### Predicting Emergency Department Visits for Patients with Type II Diabetes (https://arxiv.org/abs/2412.08984)
Comments:
...
- **What's New**: 이 연구는 제2형 당뇨병(T2D) 환자의 응급실(ED) 방문을 예측하기 위해 머신러닝(ML) 기법을 활용하여 예측 모델을 개발하고 검증하는 것을 목적으로 합니다. 연구 팀은 2017년부터 2021년까지의 환자 데이터를 사용하여 34,151명의 T2D 환자에서 총 703,065회의 ED 방문 데이터를 수집하였습니다. 이 과정에서 전자 건강 기록(EMR) 데이터와 사회적 결정 요인(SDoH)을 통합하여 ML 예측 모델을 구축하였습니다.

- **Technical Details**: 연구에서는 다양한 머신러닝 알고리즘인 CatBoost, Ensemble Learning, K-nearest Neighbors (KNN), Support Vector Classification (SVC), Random Forest, Extreme Gradient Boosting (XGBoost)를 사용했습니다. 각 모델은 10배 교차 검증을 통해 ED 방문 위험을 예측하였으며, Random Forest와 XGBoost의 ROC 곡선 값은 각각 0.82로 나타났습니다. 이러한 모델은 T2D 환자에서 ED 방문 위험을 예측하는 신뢰할 수 있는 도구로 평가되었습니다.

- **Performance Highlights**: 본 연구에서 개발된 모델은 T2D 환자의 ED 방문 예측에 있어 우수한 성능을 보였으며, 그중 Ensemble Learning과 Random Forest 모델이 특히 높은 정확도로 환자의 ED 방문 위험을 구분하고 조정할 수 있는 능력을 지니고 있는 것으로 나타났습니다. 모델들은 미래 ED 수요를 추정하고, 임상의들이 ED 이용과 관련된 주요 요소를 파악하여 조기 개입을 가능하게 할 수 있도록 돕습니다. 가장 중요한 다섯 가지 예측 요소는 연령, 방문 간 차이, 방문 간격, 복부 및 골반 통증, 소득의 극단적 집중 지수(ICE)입니다.
...
- **What's New**: 이 논문은 딥러닝(Deep Learning) 모델의 보안 도전에 중점을 두고 있으며, 적대적 공격(adversarial attacks), 데이터 오염(data poisoning), 모델 도난(model theft) 및 개인 정보 유출(privy leakage)과 같은 취약점을 조사합니다. 이러한 취약점이 모델의 무결성과 기밀성에 미치는 영향을 구체적으로 설명하며, 적대적 훈련(adversarial training)과 차등 개인 정보 보호(differential privacy)와 같은 방어 방법도 논의합니다.

- **Technical Details**: 딥러닝은 인공신경망(neural network)을 사용하여 데이터를 효율적으로 처리합니다. 여러 계층의 뉴런(nerons)으로 구성되며, 각 뉴런은 입력값에 가중치(weights)를 곱한 후 활성화 함수(activation function)를 적용하여 출력을 생성합니다. 그러한 뉴런 구조는 복잡한 패턴을 인식하고 더 나은 예측을 가능하게 합니다. 논문은 파이썬(Python)과 파이토치(PyTorch) 같은 도구를 활용하여 모델을 구축하는 방법도 제안합니다.

- **Performance Highlights**: 딥러닝 모델은 특히 대규모 데이터셋에서 더 나은 성능을 발휘하며, 이로 인해 다양한 분야에서 널리 사용되고 있습니다. 그러나 복잡한 애플리케이션에 의존도가 높아짐에 따라 이러한 모델의 보안성 보장을 위한 노력이 필요합니다. 이를 통해 결정적인 실패나 취약성으로 인한 위험을 줄일 수 있으며, 이는 미래에 더 신뢰할 수 있는 딥러닝 시스템을 만들기 위한 중요한 요소로 대두되고 있습니다.



### Belted and Ensembled Neural Network for Linear and Nonlinear Sufficient Dimension Reduction (https://arxiv.org/abs/2412.08961)
Comments:
          35 pages, 5 figures, 2 tables

- **What's New**: 본 논문에서는 선형 및 비선형 차원 축소를 모두 수용할 수 있는 통합적이고 유연하며 구현이 용이한 차원 축소(Sufficient Dimension Reduction, SDR) 프레임워크를 제안합니다. 이 통합 프레임워크는 좁은 잠재층과 응답의 변환 가족을 포함하는 Belted and Ensembled Neural Network (BENN)라는 특별히 구조화된 신경망을 사용하여 구현됩니다. 이 접근법은 조건부 분포와 조건부 평균에 대한 추정 작업을 동시에 처리할 수 있는 장점을 가지고 있습니다.

- **Technical Details**: BENN 구조에서는 '벨트'라고 불리는 좁은 잠재층을 신경망의 다양한 층에 전략적으로 배치하여 선형 또는 비선형 차원 축소를 달성할 수 있습니다. 또한 적절한 변환 가족을 선택함으로써 조건부 분포 또는 조건부 평균에 대한 차원 축소를 이룰 수 있습니다. 이 방법은 전통적인 SDR 추정기가 가지고 있는 행렬 역전 문제를 회피하여 계산 속도를 빠르게 하여 대규모 데이터의 수용성을 높입니다.

- **Performance Highlights**: 제안된 알고리즘은 기존의 SDR 방법들과 비교하여 우수한 수렴 속도를 보여줍니다. 두 개의 데이터 사례에 적용한 결과, 제안된 방법이 높은 정확도와 적시성을 겸비하고 있음을 확인했습니다. 이러한 특성 덕분에 대규모 데이터 세트에서 효과적으로 사용될 수 있는 가능성을 제시합니다.



### Interpreting Graphic Notation with MusicLDM: An AI Improvisation of Cornelius Cardew's Treatis (https://arxiv.org/abs/2412.08944)
- **What's New**: 이 연구는 Cornelius Cardew의 Treatise에서 영감을 받은 음악 작곡 및 즉흥 연주를 위한 새로운 방법을 제시합니다. OpenAI의 ChatGPT를 활용하여 Treatise의 추상 시각 요소를 해석하고, 이를 서술적 텍스트 프롬프트로 변환하여 MusicLDM이라는 음악 생성용 잠재적 확산 모델에 입력합니다. 본 연구는 AI가 시각적 자극을 소리로 변환하고 현대 및 실험 음악 작곡에서의 창작 가능성을 확장할 수 있는 방법을 보여줍니다.

- **Technical Details**: 이 모델은 ChatGPT를 사용하여 Treatise의 시각 요소를 텍스트로 변환하며, 이러한 텍스트 프롬프트는 MusicLDM에서 음악 생성에 사용됩니다. MusicLDM은 Denoising Diffusion Probabilistic Models(DDPMs)를 기반으로 하며, 미리 훈련된 CLAP 인코더를 사용하여 시각과 음성을 공유하는 잠재 공간 내에서 벡터를 인코딩합니다. 이 과정에서 'outpainting' 기법이 적용되어 AI 생성 음악의 구간을 겹쳐 매끄럽고 응집력 있는 작곡을 만들어냅니다.

- **Performance Highlights**: 이 연구는 AI가 그래픽 악보를 해석하여 음악으로 변환하는 가능성을 보여줍니다. 결과적으로 생성된 음악은 시각적 요소와 음향적 표현 간의 다중 모달 관계를 탐색한 새로운 통찰력을 제공합니다. 향후 연구에서는 기존의 ChatGPT 대신 CLIP과 같은 미리 훈련된 시각적 모델을 사용하여 더 높은 제어 가능성을 제공할 계획입니다.
...
### Multi-objective Combinatorial Methodology for Nuclear Reactor Site Assessment: A Case Study for the United States (https://arxiv.org/abs/2412.08878)
Comments:
          29 pages, 8 Tables, 12 figures

- **What's New**: 이 연구는 미국 내 30,000개 이상의 원자력 발전소(NPP) 후보지를 평가하기 위해 새로운 다목적 최적화 방법론을 제시합니다. 기존의 사이트 특성에 대한 사전 결정된 가중치에 따른 편향을 줄이기 위해, 각 사이트는 세부적인 조합 분석을 기반으로 한 성능 기반 점수를 부여받습니다. 이 데이터베이스를 통해 머신 러닝을 활용한 신경망 모델을 학습시키고, 미국 전역에서 신속하게 원자력 부지의 적합성을 예측할 수 있게 됩니다.

- **Technical Details**: 이러한 연구는 복잡한 다목적 최적화 문제를 해결하기 위한 전형적인 방법인 Genetic Algorithm (GA) 를 활용합니다. 특히, 비지배 정렬 기법을 사용하여 Pareto 프론트를 형성하고, 상이한 목적 간의 균형을 고려하여 최적의 사이트를 도출합니다. 논문에서는 이전 산업 활동으로 인한 오염 여부를 평가할 수 있는 Brownfield 사이트 및 기존 석탄 화력 발전소 위치의 재활용을 통한 NPP 자리 선정 가능성을 다룹니다.

- **Performance Highlights**: 이 방법론을 통해 원자력 발전소 건설에 필요한 자본 비용을 기존의 청정 에너지 발전원보다 최대 35%까지 줄일 수 있습니다. 기존의 무작위 선정이나 주관적 가중치 부여 방식에 대한 한계를 극복하고, 다목적 최적화 기법을 통해 보다 공정하고 객관적인 평가를 가능하게 합니다. 미국 내 237개의 운영 중인 및 157개의 대기중인 석탄 화력 발전소가 원자력 시설에 적합하다는 점을 통해, 이 연구는 미국의 지속 가능한 에너지 목표를 달성하는 데 기여할 것으로 기대됩니다.



### Beyond Reweighting: On the Predictive Role of Covariate Shift in Effect Generalization (https://arxiv.org/abs/2412.08869)
- **What's New**: 이 논문에서는 기존의 covariate shift(공변량 변화)에 대한 가정이 통계적 추론의 일반화에 적합하지 않다는 것을 보여줍니다. 새로운 접근법으로, 관찰된 변수에서 관찰되지 않은 변수의 조건부 변화를 예측하는 covariate shift의 역할을 강조합니다. 이를 통해, 불확실한 조건부 변화의 크기를 예측할 수 있는 실증적인 증거를 제시하며, 효율적이고 믿을 수 있는 불확실성 정량화 방법을 제안합니다.

- **Technical Details**: 저자들은 680개의 연구와 65개 사이트를 분석하여 covariate shift와 조건부 shift 간의 관계를 정량화하는 제안된 표준화된, 'pivot' 측정을 활용하여 두 가지 큰 규모의 다중 사이트 재현 프로젝트의 데이터를 사용했습니다. 관찰된 covariate shift는 조건부 shift의 크기를 제한할 수 있으며, 이 관계를 활용하여 효과적인 일반화를 수행할 수 있음이 입증됩니다. 이는 기존 방법론들이 주로 최악의 조건부 변화만을 고려하는 것과 대조적입니다.

- **Performance Highlights**: 제안된 새로운 예측 역할에 기반한 예측 구간은 신뢰할 수 있는 커버리지를 유지하며, 기존 방법들보다 주목할 만하게 좁은 구간을 제공합니다. 이는 고정된 최악의 조건부 변화를 가정하는 기존 방법들이 적절한 커버리지를 달성하더라도 지나치게 넓은 구간 문제를 안고 있다는 점에서 두드러집니다. 최종적으로, 이 논문은 분포 변화, 일반화 가능성 및 외부 유효성 문제를 새로운 관점에서 접근할 수 있는 기초가 됩니다.
...
### Emulating the Global Change Analysis Model with Deep Learning (https://arxiv.org/abs/2412.08850)
Comments:
          Presented at Tackling Climate Change with Machine Learning, NeurIPS 2024

- **What's New**: 이 논문은 Global Change Analysis Model (GCAM)의 예측 및 계산 효율성을 향상시키기 위해 딥 러닝 기반의 고충실도 에뮬레이터를 소개합니다. GCAM의 복잡한 인간-지구 시스템 간 상호작용을 효율적으로 모사할 수 있는 이 에뮬레이터는 기존의 마른 구현보다 약 1000배 빠르게 작동합니다. 이 접근 방식은 다양한 입력 값과 경로를 탐색하는데 필요한 GCAM 실행 수를 줄이는 데 기여합니다.

- **Technical Details**: 이 연구에서는 풍력 및 태양열 에너지 생산과 연계하여 GCAM의 다양한 입력 값을 탐색하기 위해 신경망(neural network)을 훈련시킵니다. 44개의 출력 변수를 예측하며, 22,528개의 값을 생성하는 구조로, 이는 32개 지역과 특정 기간에 걸쳐 변동합니다. 또한, 입력 값 범위를 확장하기 위해 Latin hypercube와 finite-diff 샘플링 전략을 적용하여 4,096개의 구성 데이터를 수집하고 이를 훈련 세트와 검증 세트로 나누어 사용합니다.

- **Performance Highlights**: 에뮬레이터의 예측은 $R^2$ 점수로 0.998이라는 높은 정확도를 보이며, 입력-출력 민감도에 대해서는 0.812의 점수를 기록했습니다. 이를 통해 GCAM의 복잡한 다섯 가지 영역의 상호작용과 모델의 민감도를 더욱 깊이 이해할 수 있는 기반을 제공할 수 있습니다. 이 연구는 GCAM을 통한 미래 시나리오 탐색을 위한 새로운 접근 방법으로서 의미가 있습니다.
...
### On the Precise Asymptotics and Refined Regret of the Variance-Aware UCB Algorithm (https://arxiv.org/abs/2412.08843)
- **What's New**: 이번 논문에서는 Multi-Armed Bandit (MAB) 문제를 위해 UCB-V 알고리즘의 동작을 연구하였습니다. UCB-V는 전통적인 UCB 알고리즘에 변동성(variance) 추정치를 결합하여 의사결정을 진행하는 변형입니다. 연구 결과, UCB-V는 기존 UCB와 달리 불안정성을 띄며, 팔당기는 비율(arm-pulling rate)이 항상 비대칭적으로 결정되지 않을 수 있음을 보였습니다.

- **Technical Details**: MAB 문제는 탐색(exploration)과 활용(exploitation) 간의 균형을 잡는 것이 핵심입니다. 특히, K-팔 MAB에서 학습자는 K개의 팔 각각에 대해 보상 분포를 고려하며, 각 시간 단계에서 팔을 선택하고 보상을 받습니다. UCB-V 알고리즘은 변동성을 고려한 보상 추정치를 기반으로 팔당기는 비율을 결정증가시키나, 이로 인해 비교적 큰 변동성을 보입니다.

- **Performance Highlights**: UCB-V 알고리즘은 높은 확률 범위에서 팔당기는 비율에 대한 비대칭적 바운드를 제공하며, 이전에 알려지지 않았던 정밀한 후회(regret) 바운드를 달성할 수 있음을 보여주었습니다. 이는 복잡한 변동성 인식 알고리즘에 비해 나은 성과를 나타내며, 최적의 팔 선택에 관한 안정성 분석을 제공합니다. 결과적으로, UCB-V는 고급 온라인 의사결정 알고리즘의 새로운 기준이 될 수 있습니다.
...
### Efficient Gravitational Wave Parameter Estimation via Knowledge Distillation: A ResNet1D-IAF Approach (https://arxiv.org/abs/2412.08672)
Comments:
          7 pages, 4 figures, 2 tables

- **What's New**: 중력파 천문학 분야는 2015년 첫 직접 감지 이후 빠르게 발전하고 있으며, 이벤트 수의 증가로 인해 파라미터 추정 및 모델 업데이트를 위한 효율적인 방법이 요구되고 있다. 이 연구는 지식 증류(knowledge distillation) 기법을 활용하여 중력파 분석에서 컴퓨팅 효율성을 향상시키는 새로운 접근법을 제시한다.

- **Technical Details**: 본 연구는 ResNet1D와 Inverse Autoregressive Flow (IAF) 아키텍처를 결합한 프레임워크를 개발하였다. 복잡한 teacher 모델로부터 보다 경량인 student 모델로 지식을 전달하여, 파라미터 수를 43% 줄이면서도 유효성 손실(validation loss) 성능이 teacher 모델보다 더 높다는 결과를 보였다.

- **Performance Highlights**: 학생 모델은 최적 구성(40,100,0.75)에서 3.70의 유효성 손실을 달성하였으며, inference 시간을 35% 줄였다. 이는 실시간 이벤트 처리를 위한 중력파 데이터 분석에서 중요한 개선점을 제공하며, 다중 메신저 천문학에서의 신속한 파라미터 추정이 가능하게 한다.
...
### GeoConformal prediction: a model-agnostic framework of measuring the uncertainty of spatial prediction (https://arxiv.org/abs/2412.08661)
- **What's New**: 본 논문에서는 새로운 불확실성 평가 방법인 GeoConformal Prediction을 제안합니다. 이 방법은 기존의 conformal prediction에 지리적 가중치를 도입하여 공간 예측의 신뢰성을 높이는 데 기여합니다. GeoConformal은 공간 회귀 및 공간 보간의 두 가지 클래식한 공간 예측 사례에 적용되어 그 유효성을 검증하였습니다. 이 연구는 향후 GeoAI 모델 설계에 있어 중요한 통찰력을 제공할 가능성을 지니고 있습니다.

- **Technical Details**: GeoConformal Prediction은 공간 모델 간의 공변량 변화 문제를 해결하기 위해 훈련 데이터와 테스트 데이터 간의 유사성을 측정하는 새로운 접근 방식을 사용합니다. 이러한 유사성은 지리적 가중치로 표현되어 다양한 위치에서 공간적으로 변동하는 불확실성을 추정할 수 있게 합니다. 이는 기존 Kriging과 비교하여 더 나은 예측 불확실성을 바탕으로 신뢰할 수 있는 예측을 가능하게 합니다. 연구에서 XGBoost와 공간 회귀 및 보간 기법이 결합되어 GeoConformal의 성능을 평가하였습니다.

- **Performance Highlights**: GeoConformal Prediction의 적용 결과, 보장율(coverage rate)은 93.67%에 달해 기존의 Bootstrap 방법의 최대 68.33%를 초과했습니다. 또한, GeoConformal에서 추정된 불확실성은 Kriging의 분산과 밀접하게 일치하는 것으로 나타났습니다. 이 연구 결과는 AI 모델에 지역적(feature) 요소를 포함함으로써 예측 불확실성을 크게 줄일 수 있다는 것을 시사합니다. 이러한 발견은 신뢰성 높은 공간 예측 프레임워크의 개발에 기여할 것입니다.
...
### Capacitive Touch Sensor Modeling With a Physics-informed Neural Network and Maxwell's Equations (https://arxiv.org/abs/2412.08650)
Comments:
          ESM'2024 (The 38th annual European Simulation and Modelling Conference)

- **What's New**: 이 논문에서는 새로운 접근 방식으로 Physics-Informed Neural Network (PINN)을 기반으로 한 서 surrogate 모델을 소개하여 용량식 터치 센서의 설계 과정을 가속화합니다. PINN 모델은 손가락과 용량 센서 간의 상호작용을 설명하는 전기 정적 방정식을 해결하려고 합니다. 이러한 모델은 입력으로 3D 도메인에서의 공간 좌표 및 손가락 거리를 포함하여, 신경망의 손실 함수에 전기 정적 방정식을 직접 통합하여 기본 물리학을 캡처합니다.

- **Technical Details**: PINN은 기존의 유한 요소 방법 (Finite Element Method, FEM) 대신에 물리 방정식을 신경망 구조에 통합하여 연속적인 해를 제공한다는 이점을 가집니다. 이 연구에서는 단일 배열 용량 센서를 기준으로 전기장 분포, 전기 잠재력 및 전하 밀도를 예측하기 위해 신경망을 훈련합니다. 전기 정적 방정식의 전자 정적 근사를 사용하며, 정해진 거리에서 발생하는 자료로 초기 추정을 만들고, 물리학적 잔차 손실을 보완하여 모델의 훈련 품질을 보장합니다.

- **Performance Highlights**: 제안된 PINN 모델은 다양한 실험 설정에 대한 추론을 수초 내에 수행할 수 있도록 설계되었습니다. 훈련된 모델은 이전에 보지 못한 테스트 케이스에서 평가되었으며, PINN의 유효성 결과는 용량 터치 센서의 개발과 설계 최적화를 가속화하는 데 상당한 잠재력을 보여주었습니다. 이 방법은 다양한 조건에서 신뢰할 수 있는 수치해석을 가능하게 하여 장기적인 성능 연구에 기초를 마련합니다.



### Multi-modal Representation Learning Enables Accurate Protein Function Prediction in Low-Data Setting (https://arxiv.org/abs/2412.08649)
- **What's New**: 이번 연구에서는 HOPER(HOlistic ProtEin Representation)라는 새로운 멀티모달 학습 프레임워크를 제안합니다. 이는 데이터가 부족한 환경에서 단백질 기능 예측(protein function prediction, PFP)을 향상시키도록 설계되었습니다. HOPER는 단백질 서열, 생물의학 텍스트 및 단백질-단백질 상호작용(protein-protein interaction, PPI) 네트워크의 세 가지 서로 다른 모달리티를 통합하여 포괄적인 단백질 표현을 생성합니다.

- **Technical Details**: HOPER는 오토인코더(autoencoders)를 활용하여 통합된 임베딩을 생성하고, 이를 전이 학습(transfer learning)을 통해 PFP 작업에 적용합니다. 기존의 방법들보다 모든 Gene Ontology 카테고리에서 성능이 우수한 결과를 보여줍니다. 또한 HOPER는 폐선암에서 새로운 면역회피 단백질을 식별하여 잠재적인 치료 표적에 대한 통찰력을 제공합니다.

- **Performance Highlights**: HOPER는 주어진 데이터 부족 문제를 극복하고 생물학 연구에서 데이터 제한 문제를 해결하는 데 효과적임을 보여주었습니다. 이 연구 결과는 HOPER가보다 정확하고 확장 가능한 단백질 기능 예측을 가능하게 할 수 있음을 강조합니다. 소스 코드 및 데이터 세트는 제공된 링크에서 사용 가능합니다.

...
### PRIMUS: Pretraining IMU Encoders with Multimodal Self-Supervision (https://arxiv.org/abs/2411.15127)
Comments:
          Also presented under the title "PRIMUS: Pretraining IMU Encoders with Multimodal and Self-Supervised Learning" at NeurIPS 2024 TSALM Workshop (Time Series in the Age of Large Models)

- **What's New**: 본 논문은 PRIMUS라는 새로운 방법을 제안하여 IMU (Inertial Measurement Unit) 인코더의 프리트레이닝(pretraining)을 수행합니다. 특히, 라벨이 부족한 데이터 환경에서 자가 지도(self-supervised) 및 다중 모달(multimodal) 학습 손실을 결합하여 인코더가 긴밀한 표상을 학습하도록 돕습니다. 이전의 연구들에서 자주 간과된 IMU 데이터의 사전 학습 가능성을 탐구하는 데 중점을 두었습니다.

- **Technical Details**: PRIMUS는 세 가지 손실 함수를 기반으로 작동합니다: 자가 감독 손실(ℒSS), 다중 모달 손실(ℒMM), 이웃 손실(ℒNN). ℒSS는 센서 위치 변화에 대한 불변성을 보장하고, ℒMM은 IMU 데이터와 텍스트 및 비디오 표현을 정렬시키며, ℒNN은 대표 공간 내에서 가장 가까운 예제를 활용하여 대조적 학습을 강화합니다. 이러한 구조는 모바일 및 웨어러블 장치에서의 효율적인 배포를 목표로 하여 설계되었습니다.

- **Performance Highlights**: PRIMUS는 500개 미만의 라벨이 부착된 샘플로도 최고 15% 향상된 성능을 보여주었습니다. 이는 기존의 다중 모달 및 자가 지도 학습 방법과 비교할 때 뛰어난 결과입니다. 또한, PRIMUS는 다양한 사용 사례에 대한 적응성을 높이며, 연구 커뮤니티에 기여하기 위해 코드와 사전 훈련된 IMU 인코더를 공개할 예정입니다.
...
### Effective Littlestone Dimension (https://arxiv.org/abs/2411.15109)
Comments:
          12 pages

- **What's New**: Delle Rose et al. (COLT'23)은 Vapnik-Chervonenkis(VC) 차원의 효율적인 버전을 소개하며, 이는 총 컴퓨팅 가능한 학습자로서의 부적절한 PAC 학습을 특성화한다고 보여주었다. 본 논문에서는 Littlestone 차원의 유사한 효과화(effective version)를 도입하고 연구하였다. 여기서 유한한 효과적인 Littlestone 차원은 계산 가능한 온라인 학습의 필요 조건이지만, 충분 조건은 아님을 입증하였다.

- **Technical Details**: Littlestone 차원은 주어진 가설 클래스의 최대 깊이 d를 정의하며, 이 차원에서 열리는 트리의 모든 가지가 해당 클래스에 의해 실현 가능해야 한다. 본 논문에서는 Turing 기계가 깊이 d+1인 Littlestone 트리를 받았을 때 실현 불가능한 가지를 지시할 수 있는 최소 d를 정의하여 유효한 Littlestone 차원을 제안한다. 또한, 최적의 실수 비는 효율적 Littlestone 차원과 동일하며, 제한된 잔실수 개수를 허용할 경우에 한정된다.

- **Performance Highlights**: 유한한 효과적인 Littlestone 차원을 가진 클래스는 모두 계산 가능한 함수로 구성된다. 특히, 효과적인 Littlestone 차원이 1인 클래스는 최대 1회의 실수로 모든 계산 가능한 온라인 학습자를 허용하는 특성을 가진다. 이는 선행 연구에서도 유사한 조합적 특성의 실패가 관찰되었음을 시사하며, PAC 학습에서도 유사한 결과가 논의되었다.



### AttriBoT: A Bag of Tricks for Efficiently Approximating Leave-One-Out Context Attribution (https://arxiv.org/abs/2411.15102)
Comments:
          29 pages, 11 figures

- **What's New**: 이 논문에서는 대형 언어 모델(Large Language Models, LLMs)의 맥락에 대한 기여도를 효율적으로 계산하기 위한 새로운 기법인 AttriBoT를 제안합니다. AttriBoT는 LOO(Leave-One-Out) 오류의 근사치를 신속하게 계산하는 다양한 기법을 포함하여, 캐시된 활성화(cached activations)를 활용하여 중복된 연산을 방지하고, 계층적 기여도 평가(hierarchical attribution)를 통해 계산량을 줄입니다. 이렇게 하여 AttriBoT는 기존 방법들보다 300배 이상 빨라져 실용적인 응용이 가능해집니다.

- **Technical Details**: AttriBoT는 전통적인 LOO 오류 측정 방법의 효율성을 높이는 여러 기술을 활용합니다. 이 방법은 주어진 맥락에서 텍스트 조각을 제거했을 때 LLM의 응답 가능성의 변화를 측정하는 마찰적 기법입니다. LOO 기여도 점수를 계산하기 위한 대부분의 연산은 불필요한 부분이 많아 캐싱(caching)된 정보를 이용해 계산 부하를 크게 줄일 수 있습니다. 이렇게 구축된 AttriBoT는 실험적으로 다양한 모델에서 LOO 오류의 신뢰성을 높이면서도 계산 비용을 대폭 절감할 수 있음을 보여줍니다.

- **Performance Highlights**: 실험에서 AttriBoT는 공개된 질문 응답 세팅에서 LOO 기여도 계산 비용을 300배 이상 줄일 수 있음을 입증했습니다. 또한, AttriBoT의 기법은 모델의 LOO 오류에 대한 정확도를 높이면서도 계산 속도가 현저하게 개선되는 결과를 보였습니다. 이러한 성과는 LLM의 맥락 기여도 계산을 더욱 원활하게 만들어, 실제 적용 가능한 폭을 넓히는 데 기여할 것으로 기대됩니다.



### What You See is Not What You Get: Neural Partial Differential Equations and The Illusion of Learning (https://arxiv.org/abs/2411.15101)
- **What's New**: 이 논문에서는Differentiable Programming(DP)을 활용한 NeuralPDE가 실제 물리적 해석 가능한지에 대한 심도 있는 검토를 제공합니다. NeuralPDE는 높은 품질의 PDE 시뮬레이션을 훈련 데이터로 활용하여 물리적 모델링을 수행하는데, 이 과정에서 발생하는 특정한 왜곡과 편향이 모델의 일반화 능력에 영향을 미치고 있음을 발견하였습니다. 특히, 학습 데이터셋이 가진 정보에 따라 NeuralPDE의 해석 가능성에 의문을 제기합니다.

- **Technical Details**: NeuralPDE 모델은 Taylor Series의 절단 오차와 관련된 왜곡을 학습하며, 이는 모델이 초기에 설정된 조건에 의해 제한받을 수 있음을 보여줍니다. 이 연구에서는 결정적인 수치 거동과 절단 오차의 관계를 분석하고, 조사된 신뢰도와 일반화 능력에 대해 논의합니다. DP 접근법을 통해 독립적인 변수를 포함시켜 다양한 물리적 현상을 더욱 정확하게 모델링할 수 있는 가능성도 제시합니다.

- **Performance Highlights**: 이 논문은 NeuralPDE가 예측 정확도에서 특정 조건에서만 효과적일 수 있다고 주장합니다. 특히, 동일한 수치 계획을 사용하는 경우에만 훈련 데이터 세트의 절단 오차와 일치할 때 적절한 일반화가 가능하다고 밝힙니다. 이러한 발견은 고차원의 복잡한 PDE 문제에 대한 NeuralPDE의 적합성을 재고할 필요가 있음을 나타냅니다.
...
### On Multi-Agent Inverse Reinforcement Learning (https://arxiv.org/abs/2411.15046)
Comments:
          Currently under review

- **What's New**: 이 논문에서는 여러 에이전트가 참여하는 시스템에서의 Inverse Reinforcement Learning (IRL) 프레임워크를 확장하여 Nash Equilibrium (NE) 정책을 따르는 에이전트를 관찰하는 방법을 제시합니다. 이론적으로 NE 전문가의 행동을 설명하는 효용 함수의 집합을 조사하고, 전환 동역학(transitional dynamics)과 전문가 행동의 추정 과정에서의 오류가 회수된 보상에 미치는 영향을 분석합니다. 추가적으로, 다중 에이전트 IRL 문제에 대한 샘플 복잡성(sample complexity) 분석을 최초로 제공하며 이론적 결과에 대한 수치 평가도 수행합니다.

- **Technical Details**: 본 연구에서는 Q-함수, 가치 함수(value function) 및 보상 함수에 대한 수학적 표현을 위한 추가 표기법을 도입하였습니다. 이는 Lin et al. (2018)의 결과를 기반으로 하여 다중 에이전트 환경에 조정되었습니다. 모든 상태에서의 보상을 매트릭스 형식으로 나타내고, 에이전트의 행동을 기술하기 위해 적절한 조합으로 이들 매트릭스를 결합합니다. 전환 행렬(transition matrix)은 공동 행동(joint actions)에 따라 달라지며, 이로 인해 가치 함수와 Q-값 함수가 특정 차원의 벡터로 표현됩니다.

- **Performance Highlights**: 선행 연구들과의 비교를 통해 MEARL (Multi-Agent Reinforcement Learning) 문제의 샘플 복잡성이 에이전트 수에 대해 지수적으로 증가한다는 것을 실증합니다. Daskalakis et al. (2009)의 초기 연구와 Chen et al. (2007)의 결과를 인용하여 Nash 균형을 찾는 문제의 복잡성에 대해 논의합니다. 또한, Liu et al. (2020)에 의해 제안된 Optimistic Nash Value Iteration 알고리즘이 기존의 모델 기반 접근법보다 더 나은 성능을 보임을 시사하며, 이 모든 결과는 효용 함수의 추정 및 보상 회수에 대한 우리의 분석과 잘 맞아떨어집니다.



### Safe Multi-Agent Reinforcement Learning with Convergence to Generalized Nash Equilibrium (https://arxiv.org/abs/2411.15036)
- **What's New**: 이번 연구에서는 multi-agent reinforcement learning (MARL) 시스템에서 안전성을 보장하는 새로운 이론적 프레임워크를 제안합니다. 이 프레임워크는 에이전트가 방문하는 모든 상태에서 안전 요구 사항을 충족하도록 하고, 제어 이론의 개념을 사용하여 feasible regions를 정의합니다. 또한, 각 에이전트는 안전과 성능을 모두 고려하여 Nash equilibrium에 수렴하도록 하는 새로운 알고리즘을 관리합니다.

- **Technical Details**: 연구된 방법은 controlled invariant set (CIS)의 개념을 사용하여, 각 에이전트가 안전성을 확보하고 최적의 성능을 보장할 수 있도록 위험을 평가합니다. 이 방법론은 multi-agent dual policy iteration 알고리즘을 사용하며, 이는 공동 작업에서 상태별 제약 조건을 충족하면서도 더 나은 성과를 낼 수 있도록 유도합니다. 특히, 각 에이전트는 자신의 목표를 최적화하는 동시에, 협력하여 안전할 수 있는 행동 공간을 식별하게 됩니다.

- **Performance Highlights**: 제안된 Multi-Agent Dual Actor-Critic (MADAC) 알고리즘은 기존의 안전 MARL 방법들에 비해 월등한 성과를 보였습니다. 실험 결과 MADAC은 훨씬 높은 보상을 제공하며, 제약 조건 위반을 줄이는 데 효과적임을 입증했습니다. 이 알고리즘은 깊은 강화 학습(deep RL) 맥락에서 제안된 반복 방식에 근거하여 실용적인 시스템에서도 안전성을 보장합니다.



### On the Linear Speedup of Personalized Federated Reinforcement Learning with Shared Representations (https://arxiv.org/abs/2411.15014)
- **What's New**: 이 논문에서는 여러 에이전트가 지역 경로(local trajectories)를 공유하지 않고도 정책(policy)을 공동으로 학습할 수 있게 해주는 연합 강화 학습(FedRL)의 발전을 소개합니다. 기존의 FedRL 알고리즘이 서로 다른 에이전트에서 수행할 때 성능이 저하되는 문제를 해결하기 위해, 개인화된(Federated reinforcement learning) FedRL 프레임워크인 PFedRL을 제안합니다. 이 프레임워크는 이질적인 환경에서 공유될 수 있는 공통 구조를 활용합니다.

- **Technical Details**: PFedRL 알고리즘은 PFedRL-Rep라는 클래스를 통해 협력적으로 공유되는 특징(feature) 표현을 학습하고, 각 에이전트의 지역 환경에 맞춰 개인화된(agent-specific) 가중치 벡터를 학습합니다. 특히 PFedTD-Rep는 비선형 표현을 기반으로 하는 TD학습의 특정 인스턴스이며, 연구자들은 PFedRL 설정에서 에이전트 수에 따라 선형 수렴 속도 향상을 처음으로 증명하였습니다. 이 과정에서 PFedTD-Rep이 연합된 두 배율 확률적 근사(federated two-timescale stochastic approximation)의 예시임을 보여줍니다.

- **Performance Highlights**: 실험 결과, PFedTD-Rep은 이질적인 환경에서 학습 개선뿐만 아니라 새로운 환경에 대한 일반화(generalization)도 향상시킨다는 것을 보여줍니다. 또한, DQN(Deep Q-Networks)을 기반으로 하는 제어 설정으로의 확장을 통해 성능이 더욱 강화되었습니다. 이를 통해 PFedRL 접근 방식이 다양한 에이전트 환경에서 효과적으로 작동할 수 있음을 확인하였습니다.



### Adaptive Group Robust Ensemble Knowledge Distillation (https://arxiv.org/abs/2411.14984)
Comments:
          Workshop Algorithmic Fairness through the Lens of Metrics and Evaluation at NeurIPS 2024

- **What's New**: 이번 연구에서는 복합적인 teacher 모델에서 상대적으로 '단순한' student 모델로 지식이 증류(distillation)되는 과정에서 발생하는 성능 불균형 문제에 대한 새로운 접근 방식인 Adaptive Group Robust Ensemble Knowledge Distillation(AGRE-KD)을 제안합니다. 기존의 ensemble deep learning 방법이 가장 낮은 성능을 보이는 하위 집단의 성능을 개선할 수 있다는 연구 결과가 있었지만, 지식 증류 과정에서 이점이 유지되는지는 불확실하였습니다. 필요한 경우 알려지지 않은 저조한 하위 집단에 유리한 지식을 수신할 수 있도록 설계된 AGRE-KD는 부정확한 모델을 활용하여 성능을 개선합니다.

- **Technical Details**: AGRE-KD 방법은 biased model의 gradient direction을 이용하여 teacher 모델의 출력 집합 과정을 유도합니다. 이 방법은 student 모델이 teacher로부터 비뚤어진 지식을 학습하지 않도록 보장합니다. 여러 teacher 모델로부터 지식을 증류할 때, ensemble knowledge distillation이 발생할 수 있는 성능 불균형 문제를 해결하는 데 중점을 두고 있습니다. 본 연구에서는 last-layer retraining을 통해 teacher 모델에서 발생하는 부정확한 상관관계를 완화하고, 최종적으로 student 모델이 개선된 표현을 학습할 수 있도록 하는 방법론을 탐구합니다.

- **Performance Highlights**: 제안된 AGRE-KD 방법은 다양한 데이터 세트에서 기존의 ensemble distillation 기술보다 우수한 성능을 나타내며, 다수결에 기반한 고전적인 모델 앙상블보다도 뛰어난 결과를 보여주었습니다. 연구 결과에 따르면, ensembles self-distillation을 사용하여 동등한 용량의 모델들 사이에서 성능 불균형을 줄일 수 있음을 확인했습니다. 또한, 제안된 방법은 gradient 공간에서 작동하여 student 모델의 성능 향상과 최악의 그룹 오류를 최소화하는 데 기여하고 있습니다.
...
- **What's New**: 이 논문은 레거시 코드의 문서화를 생성하기 위해 대형 언어 모델(LLMs)의 활용을 조사합니다. MUMPS와 IBM 메인프레임 어셈블리 언어 코드(Assembly Language Code, ALC) 두 개의 데이터 세트를 사용하고 있습니다. 특히, 우리는 코드 주석 생성을 위한 프로그래밍 전략과 그 완전성, 가독성, 유용성 및 환각(completeness, readability, usefulness, and hallucination) 평가를 위한 루브릭(rubric)을 제안합니다.

- **Technical Details**: 우리는 LLM을 사용하여 줄별 코드 주석을 생성하는 새로운 프롬프트 전략을 개발하였으며, 이는 불완전한 주석 생성을 방지합니다. 또한, 주석의 품질을 평가하기 위해 다양한 자동화된 수치(자동화된 메트릭)을 사용하고 있습니다. 특히, 정적 코드 복잡성 메트릭(cyclomatic complexity, Halstead complexity), LLM 자체의 처리 시간(runtime metrics) 및 일반적인 기준 기반 메트릭(BLEU, ROUGE)을 포함합니다.

- **Performance Highlights**: MUMPS 및 ALC 용 LLM 생성 주석은 일반적으로 환각이 없고, 완전하며 가독성이 높고 유용한 것으로 나타났습니다. 그러나 주석 품질을 예측하거나 측정할 수 있는 강력한 자동화된 메트릭은 존재하지 않았습니다. 이는 현재 자동화된 측정이 레거시 시스템의 LLM 생성 문서화에 대한 평가 및 개선이 필요하다는 점을 강조합니다.
...
- **What's New**: 병원 혈액은행에서 혈소판 폐기물을 줄이기 위한 노력은 기존의 주문 정책에 중점을 두어왔습니다. 하지만, 가장 오래된 단위를 우선적으로 발급하는 일반적인 관행이 사용되지 않은 단위가 반환되는 상황에서는 최적이 아닐 수 있습니다. 본 연구에서는 기계 학습(ML)을 활용한 새로운 발급 정책을 제안하여 만료 전에 반환된 단위가 재발급될 가능성을 증대시키고자 합니다.

- **Technical Details**: 우리는 17,297개의 혈소판 요청을 통해 훈련된 ML 모델을 개발했습니다. 이 모델의 성능은 9,353개의 별도 요청 세트에서 AUROC 0.74로 평가되었습니다. 이 연구에서는 전체 혈액은행 운영을 시뮬레이션 하여, 반환의 가능성을 고려한 발급 결정을 지원하는 ML 방법을 적용했습니다.

- **Performance Highlights**: 시뮬레이션을 통해 제안된 Youngest Unit for Predicted Returns(YUPR) 발급 정책을 사용할 경우 폐기물이 14% 줄어드는 것으로 추정되었습니다. 파트너 병원에서는 우리 접근 방식을 채택할 가능성을 고려하고 있으며, 특히 반환율이 높은 병원에 유리할 것으로 기대됩니다. 이렇게 ML모델을 통해 실질적인 운영 개선이 이룰 수 있음을 보여주고 있습니다.
...
### Predictive Modeling For Real-Time Personalized Health Monitoring in Muscular Dystrophy Managemen (https://arxiv.org/abs/2411.14923)
- **What's New**: 이 논문에서는 근육이형성증(Muscular Dystrophy, MD)을 효과적으로 관리하기 위한 새로운 IoT(Internet of Things) 기반 시스템을 제안합니다. 이 시스템은 환자의 건강 상태를 실시간으로 모니터링할 수 있는 다차원적(remote, multi-dimensional) 접근 방식을 통해 이루어집니다. 기존 방법들이 실행 가능한(data-driven) 데이터를 실시간으로 제공하지 못했던 점에서 큰 진전을 이루고 있습니다.

- **Technical Details**: 제안된 시스템은 데이터 수집과 전송을 위한 IoT 기술을 활용합니다. 이를 통해 의사들은 환자의 질병 진행 상황 및 건강 상태를 깊이 있는(insightful) 정보로 받아볼 수 있습니다. 기술 기반의 접근은 치료 전략을 향상시키고, 환자 스스로 자신의 상태를 더 잘 관리할 수 있도록 지원합니다.

- **Performance Highlights**: 이 연구는 환자 관리의 향상을 위한 기술적 접근이 얼마나 중요한지를 강조합니다. IoT 시스템을 통해 의료 제공자들은 더 많은 신뢰를 가지고 의사 결정을 내릴 수 있게 됩니다. 결과적으로, 이 시스템은 근육이형성증 환자들에게 보다 나은 치료와 관리 방안을 제시하는 데 기여할 것으로 기대됩니다.



### Exploring Kolmogorov-Arnold Networks for Interpretable Time Series Classification (https://arxiv.org/abs/2411.14904)
- **What's New**: 이번 연구에서는 Kolmogorov-Arnold Networks (KANs) 아키텍처를 시간 시계열 분류에 적용하여, 아키텍처 전이, 하이퍼파라미터 최적화, 복잡성 및 해석 가능성의 장점을 종합적으로 탐색하고자 합니다. KANs는 기존의 다층 퍼셉트론(MLPs) 대비 우수한 성능과 해석 가능성을 제공하는 대안으로 부각되고 있습니다. 본 논문에서는 UCR 벤치마크 데이터셋에서 KAN 아키텍처의 효과를 평가하며, 실질적인 기여를 목표로 합니다.

- **Technical Details**: KAN 아키텍처가 시간 시계열 분류에서 어떻게 활용되는지를 분석하기 위해, 다양한 모델 구조와 하이퍼파라미터의 영향을 연구합니다. 특히, Efficient KAN을 활용하여 MLP와 비교하며, 성능 안정성, 학습 효율성 및 구조의 영향을 분석합니다. 또한, SHAP 분석을 통한 KAN 모델의 해석 가능성에 대해서도 깊이 있는 고찰을 진행합니다.

- **Performance Highlights**: 연구 결과, Efficient KAN은 MLP보다 더 나은 성능과 계산 효율성을 보여주며, 특히 낮은 학습률에서 구조적 안정성이 두드러집니다. KAN은 HIVE-COTE2와 같은 최첨단 모델에 비해 경쟁력 있는 정확도를 유지하면서도, 더 작은 아키텍처와 빠른 학습 시간으로 성능과 투명성의 균형을 증명합니다. 이러한 결과는 KAN 모델의 해석 가능성이 의사 결정을 투명하게 지원하는 데 기여함을 보여줍니다.



### Ex Uno Pluria: Insights on Ensembling in Low Precision Number Systems (https://arxiv.org/abs/2411.14860)
Comments:
          NeurIPS 2024

- **What's New**: 이 논문은 로우 프리시전( low precision) 앙상블 기법을 제안하며, 이는 단일 모델에서 파생된 여러 모델을 활용하여 앙상블을 구성하는 새로운 접근을 보여줍니다. 특히, 로우 프리시전 앙상블 방식을 통해 기존의 앙상블 기법 대비 일반화 성능을 향상시킬 수 있음을 실험적으로 입증하고 있습니다. 이는 변별력 있는 다양한 앙상블 멤버를 수집하는 데 성공하였으며, 대형 모델들에 적합한 해결책으로 자리 잡을 potential을 내포하고 있습니다.

- **Technical Details**: 본 연구에서는 FP32, FP16, BF16과 같은 다양한 정밀도 수치 시스템을 사용하여 신경망의 가중치, 활성화 및 기울기를 표현하는 저정밀도 최적화 방법을 탐구합니다. 앙상블 멤버의 수집 과정에서 스토캐스틱 라운딩(stochastic rounding) 기술을 적용하여 가중치 업데이트의 반올림 문제를 해결합니다. 또한, 베이지안 접근법과 샘플링 기법에 대한 비교 분석을 통해 로우 프리시전 앙상블이 기존 방식과 달리 앙상블 다양성을 효과적으로 제공한 것을 강조합니다.

- **Performance Highlights**: 실험 결과, 로우 프리시전 앙상블 방식은 다운스트림 데이터에 대한 추가 학습 없이도 사전 훈련된 대형 모델의 성능을 개선하는 데 기여했습니다. 이 방식은 기억 사용량을 줄이는 동시에 성능 향상을 도모할 수 있는 유망한 접근 방안으로, 대형 모델의 앙상블 문제에서 확장성을 해결할 potential을 가지고 있습니다. 특히, 기존의 저정밀도 수치에서 발생하는 양자화 오류가 다양성을 향상시키는 원천으로 활용될 수 있음을 보여줍니다.



### Applications of fractional calculus in learned optimization (https://arxiv.org/abs/2411.14855)
Comments:
          NeurIPS Workshop on Optimization for Machine Learning

- **What's New**: 이 논문에서는 전통적인 gradient descent 방법을 확장할 수 있는 fractional gradient descent의 가능성을 탐구하고 있습니다. 기존의 파라미터 조정 문제를 해결하기 위해, 신경망을 통해 gradient의 차수를 예측하는 방법을 제안합니다. 이를 통해 복잡한 최적화 문제에서의 성능 개선을 목표로 하고 있습니다.

- **Technical Details**: 제안된 방법은 상태 벡터로 매개변수화된 고전적 함수 집합을 기반으로 학습하는 것으로 시작합니다. 신경망은 현재 상태, 정규화된 Gradient, Gradient의 크기 및 Fourier Features를 입력으로 받아 fractional derivative의 차수와 업데이트 단계의 크기를 출력합니다. 이 과정에서 Taylor expansion을 사용하여 D^α를 근사합니다.

- **Performance Highlights**: 이 방법은 기존의 단순한 gradient descent 방법보다 비선형적이고 복잡한 최적화 문제에서 더 빠른 수렴 속도를 보여줍니다. 연구 결과는 fractional gradient descent가 특정 유형의 문제에서 더 나은 성능을 발휘할 수 있음을 입증하고, 향후 최적화 기술에 큰 영향을 미칠 것으로 기대됩니다.



### Gradient Masking All-at-Once: Ensemble Everything Everywhere Is Not Robus (https://arxiv.org/abs/2411.14834)
- **What's New**: 본 연구는 "Ensemble Everything Everywhere"라는 새로운 적대적 방어 기법에 대해 논의합니다. 이 기법은 여러 이미지 해상도에서 모델의 중간 표현을 앙상블(ensemble)하여 하나의 강력한 분류를 생성합니다. 초기 결과에 따르면, CIFAR-10에서 72%의 강인한 정확도를 보여주었지만, 우리는 이 방어가 적대적 공격에 대해 강건하지 않다는 것을 보여줍니다.

- **Technical Details**: 이 방어 기법은 여러 이미지 해상도를 사용하여 입력 이미지에 무작위 변환을 적용하고, 최종적으로 여러 내부 활성화 층을 앙상블하여 강한 예측을 실시합니다. 특히, 해당 방법에서는 
- ℓ∞-정규화 공격 (
- ℓ∞-norm threat model)
- 다양한 무작위 변환들을 적용하여 robust accuracy을 높이고자 합니다. 하지만 본 연구에서는 이 방어가 그라디언트 마스킹(gradient masking) 문제에 시달리고 있다는 것을 보여줍니다.

- **Performance Highlights**: 기존 방어 기법의 robust accuracy는 CIFAR-100에서 48%에서 1%로, CIFAR-10에서는 62%에서 4%로 급락했습니다. 이러한 결과는 해당 방어 기법이 효과적인 적대적 방어가 아니며, 현재의 평가 방식이 상당한 그라디언트 마스킹의 영향을 받고 있음을 나타냅니다. 연구 결과는 향후 적대적 방어 기술의 발전 방향에 중요한 시사점을 제공합니다.
...
### Segmenting Action-Value Functions Over Time-Scales in SARSA using TD($\Delta$) (https://arxiv.org/abs/2411.14783)
Comments:
          17 pages. arXiv admin note: text overlap with arXiv:2411.14019

- **What's New**: 이 연구에서는 기존의 SARSA 알고리즘에 대한 개선책으로서 TD(Δ) 방법이 제안되었습니다. TD(Δ) 기반의 SARSA(Δ)는 다양한 할인 인자를 기반으로 되어 있어 학습 과정의 안정성과 효율성을 크게 향상시킵니다. 특히, 긴 시간 지평선에서 최적의 정책을 학습하는 능력을 극대화합니다.

- **Technical Details**: SARSA는 에이전트가 상태-행동 전환을 경험하면서 업데이트되는 on-policy RL 알고리즘입니다. 일반적으로 사용되는 할인 인자는 γ로 표시되며, SARSA(Δ)는 여러 개의 delta 추정기를 사용하여 action-value 함수를 분해합니다. 각 추정기는 서로 다른 할인 인자와 연결되며, 이를 통해 에이전트가 장기적인 결과에 영향을 미치는 행동을 학습할 수 있도록 합니다.

- **Performance Highlights**: 실험 결과, 제안된 SARSA(Δ)는 기존의 TD 학습 방식과 비교하여 더 빠른 수렴 속도와 높은 성능을 보였습니다. 특히, 밀집 보상의 환경에서 SARSA(Δ)가 SARSA의 기존 버전에 비해 유리한 성과를 나타냄을 확인하였습니다. 이로써 SARSA(Δ)는 복잡한 환경에서 효과적인 도구로 자리잡을 가능성을 보여주고 있습니다.
...
- **What's New**: 이 논문은 공정한 대비 학습(fair contrastive learning)을 위한 새로운 방법론을 제안합니다. 새로운 주의(attention) 메커니즘을 채택하여 편향을 유발하는 상호작용을 모델링하고, 이에 따라 더 공정하고 의미 있는 임베딩 공간을 학습할 수 있도록 합니다. 특히, 주의 메커니즘은 모델을 혼란스럽게 하는 부정적 샘플을 피하고 의미 있는 표현을 학습하는 데 도움이 되는 편향 감소 샘플에 집중합니다.

- **Technical Details**: 이 방법은 Fairness-Aware (FARE) 주의 메커니즘을 활용하여 편향을 줄이는 샘플에 집중하면서 모델을 혼란스럽게 하는 샘플을 회피합니다. 지역 민감 해싱(locality-sensitive hashing)을 통해 극단적인 편향 샘플을 버리는 Sparse Fairness-Aware (SparseFARE) 주의 방법도 제안합니다. FARE의 훈련을 위해 Fair Attention-Contrastive (FAREContrast) 손실을 도출하여 부정 샘플을 주의 메커니즘의 출력으로 표현하고, 부정 샘플의 유사도 점수를 학습된 주의 점수로 조건화합니다.

- **Performance Highlights**: FARE는 기존 방법들과 비교하여 학습된 표현에서 더 많은 편향을 제거하면서도 하류 정확도를 유지하는 데 큰 효과를 보입니다. 실험을 통해 제안된 방법론이 하류 작업에 필요한 정확도를 저하시키지 않으면서도 편향을 효과적으로 줄임을 입증하였습니다. 또한, FARE는 계산 복잡도가 낮아 효율적으로 작동합니다.
...
### FLARE: FP-Less PTQ and Low-ENOB ADC Based AMS-PiM for Error-Resilient, Fast, and Efficient Transformer Acceleration (https://arxiv.org/abs/2411.14733)
- **What's New**: 이번 논문에서는 첨단 Analog-Mixed-Signal Process-in-Memory (AMS-PiM) 아키텍처를 사용하여 전통적인 Transformer 모델의 학습 효율성을 크게 향상시키는 RAP 아키텍처를 제안합니다. RAP는 Dequantization-Quantization (DQ-Q) 과정 없애고, 효율적인 비선형 처리와 저-ENOB ADC 기반의 희소 Matrix Vector 곱셈 기법을 도입하여 영역/에너지 효율성과 계산 속도를 개선합니다. 실험 결과, RAP는 기존 GPU와 전통적인 PiM 아키텍처보다 에너지 효율성, 지연시간 및 정확성에서 뛰어난 성능을 보였습니다.

- **Technical Details**: RAP 아키텍처는 Quantization-Aware Training (QAT)과 Post-Training Quantization (PTQ) 기술을 통해 부하를 줄이고, 기존 아키텍처의 한계를 극복합니다. 특히, RAP는 floating-point units (FPUs) 및 DQ-Q 과정을 제거하여 시스템 복잡성을 감소시키고, 저-ENOB ADC의 사용을 통해 데이터 전송 중 발생할 수 있는 오류를 예방합니다. 또한, RAP는 Sparse Matrix Vector multiplication을 활용하여 데이터 처리의 속도와 정확성을 높이고, 학습 과정을 단순화합니다.

- **Performance Highlights**: RAP 아키텍처는 Transformer 모델의 추론 시간의 병목 현상을 해결하며, 에너지 효율 및 계산 속도를 동시에 향상시킵니다. 저-ENOB ADC 기반의 설계는 고속, 정확하고 효율적인 희소 GEMV 연산을 가능하게 하며, MRAM-SRAM 혼합 AMS-PiM 배열 내에서 6-σ 구간의 신뢰성을 보장합니다. 이에 따라 RAP는 첨단 변환 씨스템의 효율적 배포를 위한 스케일러블한 솔루션을 제공합니다.



### K-GBS3FCM -- KNN Graph-Based Safe Semi-Supervised Fuzzy C-Means (https://arxiv.org/abs/2411.14728)
Comments:
          10 pages

- **What's New**: 본 논문에서는 K-Nearest Neighbors (KNN) 기반의 안전 인식 반자동 퍼지 C-평균 알고리즘(K-GBS3FCM)을 제안합니다. 이 알고리즘은 레이블이 붙은 데이터와 레이블이 없는 데이터 간의 이웃 관계를 동적으로 평가하여 클러스터링의 정확성을 높이는 것을 목표로 합니다. 또한, 레이블이 있는 데이터의 영향력을 조정하는 메커니즘을 통해 잘못된 레이블의 부작용을 최소화합니다.

- **Technical Details**: K-GBS3FCM은 KNN 알고리즘을 활용해 레이블이 있는 데이터 포인트와 레이블이 없는 데이터 포인트의 이웃 관계를 파악하고, 이를 바탕으로 레이블의 안정성을 평가합니다. 이 알고리즘은 밀도 기반으로 pUN(Nearest Unlabeled Neighbors) 값을 동적으로 조정하여 각 레이블의 일관성을 평가하며, 평균 일관성이 특정 임계값을 초과할 경우 추가 가중치가 활성화됩니다. 이러한 접근 방식은 클러스터링 과정에서 데이터 포인트 간의 본질적인 관계를 효과적으로 포착합니다.

- **Performance Highlights**: 다양한 벤치마크 데이터셋을 이용한 실험 결과, K-GBS3FCM은 56개 테스트 설정 중 64%에서 다른 반자동 및 전통적인 비지도 학습 방법보다 높은 클러스터링 정확성을 기록했습니다. 이 연구는 KNN과 같은 그래프 기반 접근 방식을 통합하여 보다 발전된 클러스터링 알고리즘을 개발할 수 있는 가능성을 보여주며, 레이블이 있는 데이터와 레이블이 없는 데이터 모두를 활용하는 분야에서 중요한 응용 가능성을 제공합니다.



### Attributed Graph Clustering via Generalized Quaternion Representation Learning (https://arxiv.org/abs/2411.14727)
- **What's New**: 본 논문에서는 복잡한 데이터 클러스터링을 위한 새로운 방법으로 일반화된 쿼터니온 표현 학습(Generalized Quaternion Representation Learning, QRL) 모델을 제안합니다. 이 모델은 쿼터니온 연산을 인코더에 도입하여, 더 깊은 네트워크와 더 많은 매개변수를 사용하지 않고도 효율적인 구조화된 특징 표현 학습을 가능하게 합니다. 특히, 사전 지정된 클러스터 수 $k$ 없이도 클러스터링 친화적인 표현을 얻기 위한 일반화된 그래프 클러스터링 목표를 통합하여, 다양한 클러스터링 시나리오에 적응 가능한 특징을 제공합니다.

- **Technical Details**: 제안된 GCGQ는 클러스터링 작업을 위한 전통적인 그래프 자동 인코더 구조를 기반으로 하며, 임의 차원의 그래프 데이터에 적합하도록 네 개의 기능 구성 요소에 자연스럽게 연결되는 쿼터니온 연산을 사용하는 것이 특징입니다. 각 속성 뷰는 하나의 실수 및 세 개의 허수 부분으로 변환되며, 이 과정에서 노드 설명 정보를 최대한 보존하여 정보 과잉 지배 효과를 완화합니다. 이러한 설계는 얕고 넓은 네트워크 아키텍처를 가능하게 하여 과도한 스무딩 문제를 회피하면서도 표현 능력을 유지합니다.

- **Performance Highlights**: 광범위한 실험 결과에서 GCGQ는 기존 방법에 비해 우수한 성능을 보였으며, 다양한 실험에서는 유의미한 테스트와 변별력을 갖춘 특징 표현을 통해 클러스터링 성능이 향상됨을 보여주었습니다. GCGQ는 클러스터 수를 미리 지정하지 않고도 다양한 클러스터링 작업에 적합하도록 보편적인 표현을 제공할 수 있으며, 이는 실제 응용 및 데이터 분포 이해에 매우 중요합니다.



### Enhancing Molecular Design through Graph-based Topological Reinforcement Learning (https://arxiv.org/abs/2411.14726)
- **What's New**: 이 연구는 약물 설계에서 중요한 약물과 유사한 분자의 생성을 가속화하기 위해 Graph-based Topological Reinforcement Learning (GraphTRL) 방법을 제안합니다. GraphTRL은 화학적 및 구조적 데이터를 통합하여 분자 생성을 개선합니다. 기존의 강화 학습( RL ) 방법이 구조 정보를 간과한 반면, 이 연구는 이러한 정보를 활용하여 약물 발견 과정을 가속화할 수 있는 잠재력을 지니고 있습니다.

- **Technical Details**: GraphTRL은 Multiscale Weighted Colored Graphs (MWCG)와 지속적 동질성(persistent homology)을 결합하여 분자의 구조적 정보를 포착합니다. 각 SMILES 표현에 대해 RDKit 패키지를 사용하여 원자쌍의 원자 좌표를 추출하고 MWCG를 구성합니다. 이러한 구조적 데이터는 분자 지문(molecular fingerprints)과 결합되어 RL 알고리즘에 대한 포괄적인 상태 표현을 생성합니다.

- **Performance Highlights**: GraphTRL은 기존의 방법들과 비교할 때 결합 친화도 예측에서 우수한 성능을 보여주었습니다. 이는 약물 발견 프로세스를 보다 효율적이고 효과적으로 만들어 줄 가능성을 제시합니다. 이러한 성과는 강화 학습과 고급 그래프 이론 및 위상 기술의 통합을 통해 가능해졌습니다.
...
### EV-PINN: A Physics-Informed Neural Network for Predicting Electric Vehicle Dynamics (https://arxiv.org/abs/2411.14691)
Comments:
...
- **What's New**: 이 논문에서는 EV의 동적 매개변수(예: Aerodynamic drag, rolling resistance)를 예측하는 EV-PINN이라는 새로운 Physics-Informed Neural Network 접근법을 제시합니다. 이 방법은 EV의 비선형 동적 특성에 일반화하여 주행 중 순간 배터리 전력과 누적 에너지 소비를 예측할 수 있습니다. 탐지 장비 없이 실제 주행 조건에서 배터리 사용량을 예측할 수 있는 가능성을 보여줍니다.

- **Technical Details**: EV-PINN은 자동 미분(automatic differentiation)을 활용하여 모터 효율, 회생제동 효율, 차량 질량, 공기 저항 계수(coefficient of aerodynamic drag), 구름 저항 계수(coefficient of rolling resistance)와 같은 실제 매개변수를 학습합니다. 본 연구는 Tesla Model 3 Long Range 및 Tesla Model S에서 수집된 15분 및 35분의 인 situ 배터리 로그 데이터를 활용하여 검증되었습니다. inputs로는 차량 속도와 시간만을 사용합니다.

- **Performance Highlights**: 모델은 0.002195와 0.002292의 검증 손실(validation loss)을 기록하며 높은 정확성과 동적 일반화(generalization)를 달성합니다. 이 결과는 추가 센서 없이 실제 운전 조건에서 매개변수를 추정하고 배터리 사용량을 예측하는 EV-PINN의 효과를 입증합니다.



### Self-Supervised Learning for Ordered Three-Dimensional Structures (https://arxiv.org/abs/2411.14680)
Comments:
          Version as submitted to the Learning on Graphs Conference 2022, with small clarifying edits

- **What's New**: 이 연구에서는 인간의 개입 없이 라벨링된 데이터가 적은 상태에서도 강력한 일반화 능력을 갖춘 대규모 언어 모델을 활용하여 기하학적 작업을 수행할 수 있는 새로운 접근 방식을 제안합니다. Self-supervised learning (SSL)을 통해 비이상적인 물질 구조에서 질서를 정량화하고 조립체를 구별하는 작업을 훈련하는 모델을 개발했습니다. 이는 주로 기하학적 대수에 기반한 회전 및 순열 동등 신경망을 구축하여 이루어졌습니다. 다양한 복합 재료의 자가 조립 시스템을 더 잘 이해할 수 있는 가능성을 열었습니다.

- **Technical Details**: 이 연구에서 사용된 데이터는 두 가지 유형으로 나뉩니다: 거의 이상적인 3D 벌크 결정 구조 및 분자 역학 시뮬레이션의 조립체. 벌크 결정 구조는 AFLOW 데이터베이스에서 단일 성분 유닛 셀을 선택하고 임의의 노이즈를 추가하여 생성되었습니다. 분자 동역학 시뮬레이션은 HOOMD-blue를 활용하여 수행되어 연속적인 온도 변화에 따른 입자의 상호작용을 분석합니다. 이 작업에서는 геометрия 선형 대수와 주의 메커니즘을 조합한 기하학적 대수 주의 레이어를 통해 데이터 전파의 동등성을 제어하는 기법을 활용했습니다.

- **Performance Highlights**: 제안된 SSL 작업은 기존의 지표들과 비교하여 재료 과학에서 질서를 정량화하는 데 뛰어난 성능을 나타냈습니다. 특히, 서로 다른 두 결정 구조에서 결정화 이전의 액체 상태와 같은 고도로 무질서한 시스템들 간의 구조를 구별하는 능력이 강조되었습니다. 우리는 이러한 작업을 통해 자가 조립 구조를 분석할 수 있는 잠재력을 향상시킬 수 있음을 입증했습니다. 또한, 다른 자가 감독 작업 간의 전이 학습을 수행하여 모델이 습득한 정보의 범위를 설명합니다.



### Recursive Gaussian Process State Space Mod (https://arxiv.org/abs/2411.14679)
- **What's New**: 본 논문은 Gaussian Process State-Space Model (GPSSM)에 대한 새로운 개선점을 제안합니다. 특히, 제안된 방법은 비최적의 정보가 주어지는 온라인 학습을 위한 적응형 접근 방식을 포함합니다. 이를 통해 다양한 운영 도메인과 GP 하이퍼파라미터에 적응 가능성을 제공합니다.

- **Technical Details**: 이 연구는 먼저, 시스템 상태와 GP 모델 간의 공동 분포를 위한 베이지안 업데이트 방정식을 도출하는 1차 선형화 기법을 사용합니다. 이후 유용한 기준에 기반한 유도 포인트 선택 알고리즘을 개발하여 온라인 하이퍼파라미터 최적화도 지원합니다. 이로 인해, 경량화된 학습이 가능해집니다.

- **Performance Highlights**: 자기 생성 및 실제 데이터셋에서의 포괄적인 평가를 통해, 제안된 방법이 기존의 온라인 GPSSM 기법에 비해 뛰어난 정확도, 계산 효율성 및 적응력을 제공함을 보여주었습니다. 이러한 성능은 특히 비정상적 변화에 대응하는 시스템 모델링에서 유의미한 영향을 미칩니다.



### Multiset Transformer: Advancing Representation Learning in Persistence Diagrams (https://arxiv.org/abs/2411.14662)
- **What's New**: 본 논문에서는 persistence diagram representation learning을 개선하기 위한 새로운 접근 방식으로 Multiset Transformer를 제안합니다. 이 모델은 multisets를 입력으로 사용하기 위해 특별히 설계된 attention 메커니즘을 활용하는 최초의 신경망입니다. 또한 변환 불변성(permutation invariance)에 대한 엄격한 이론적 보장을 제공합니다.

- **Technical Details**: Multiset Transformer 아키텍처는 multiset-enhanced attentions와 pool-decomposition 스킴을 통합하여, equivariant 레이어 간에 multiplicities를 보존할 수 있도록 설계되었습니다. 이런 방식은 multiplicities를 완전히 활용할 수 있게 하며, Set Transformer와 비교해 계산(computational) 및 공간(spatial) 복잡성을 크게 줄이는 효과를 제공합니다. 추가적으로, 클러스터링(clustering)을 전처리 단계로 활용하면 복잡성을 더욱 최소화할 수 있어 Set Transformer에는 없는 장점이 있습니다.

- **Performance Highlights**: 실험 결과, Multiset Transformer는 persistence diagram representation learning 분야에서 기존의 신경망 방법들보다 우수한 성능을 보이는 것으로 확인되었습니다. 이러한 성과는 우리의 접근 방식이 persistence diagram 표현에서 학습의 질을 높일 수 있는 가능성을 제시함을 의미합니다.



### VQalAttent: a Transparent Speech Generation Pipeline based on Transformer-learned VQ-VAE Latent Spac (https://arxiv.org/abs/2411.14642)
- **What's New**: 본 논문에서는 VQalAttent라는 경량 모델을 소개하고 있으며, 이는 조정 가능한 성능과 해석 가능성을 제공하면서 고품질 음성을 효율적으로 생성하는 것을 목표로 하고 있습니다. AudioMNIST 데이터셋을 활용하여, 이 방법은 두 단계의 아키텍처를 통해 음성의 약속(mimicking)을 생성하는 방식을 제안합니다. 음성을 생성하는 데 있어 계산 리소스가 적게 필요하다는 것이 이 모델의 큰 장점입니다.

- **Technical Details**: VQalAttent 모델은 벡터 양자화된 오토인코더(VQ-VAE)와 디코더 전용 트랜스포머로 구성되어 있습니다. VQ-VAE는 오디오 스펙트로그램을 이산 잠재 표현으로 압축하고, 트랜스포머는 이러한 잠재 표현의 확률 모델을 학습하는 역할을 수행합니다. 이 모델을 통해 우리는 깊이 있는 유사 음성을 생성할 수 있으며, 이러한 생성 결과의 통계적 속성과 지각 품질을 해석할 수 있습니다.

- **Performance Highlights**: VQalAttent는 제한된 계산 자원으로도 이해 가능한 음성을 생성할 수 있는 능력을 보여주었습니다. 또한, 모델의 모듈성과 투명성 덕분에 분석 결과를 응용하기 쉬워졌으며, 이는 복잡한 합성 모델에 대한 통찰력을 제공하는 데 도움이 됩니다. 이 연구는 음성 합성의 복잡한 구조를 이해하고 정제하는 데 중요한 기여를 할 것으로 기대됩니다.
...
### Active Learning-Based Optimization of Hydroelectric Turbine Startup to Minimize Fatigue Damag (https://arxiv.org/abs/2411.14618)
- **What's New**: 이번 연구에서는 수력 발전 유닛(HGU)의 기동 매개변수를 최적화하기 위한 혁신적인 자동화 접근법이 제안됩니다. 이는 제한된 기동 시퀀스의 예산으로 가능하며, 가상 응력 센서와 동적 시뮬레이션을 결합하여 이루어집니다. 실험을 통해 단 7개의 측정된 시퀀스를 이용하여 최적의 기동 시퀀스를 식별하는 데 성공하였습니다.

- **Technical Details**: 연구의 핵심은 개발된 알고리즘이 기동 시퀀스에 따른 터빈의 스트레스를 최소화하는 것에 초점을 맞추고 있다는 점입니다. 제안된 방법론은 능동 학습(active learning) 프레임워크와 블랙 박스 최적화(black-box optimization) 기법을 융합하여, HGU의 동적 특성과 함께 적용되었습니다. 이러한 기법은 실제 현장에서의 실험 캠페인 중에 검증되었습니다.

- **Performance Highlights**: 결과적으로, 제안된 알고리즘은 기존 기동 시퀀스에 비해 최대 응력 사이클 진폭을 42% 줄이는 성과를 보였습니다. 이는 HGU의 기동 최적화에 있어 매우 효율적이며, 향후 수력 발전소의 운영 수명을 연장할 수 있는 가능성을 제시합니다. 이러한 효과는 특히 증가하는 기동 사이클 빈도에 맞춰 안정성과 신뢰성을 높이는 데 기여할 것입니다.
...
### Efficient Spatio-Temporal Signal Recognition on Edge Devices Using PointLCA-N (https://arxiv.org/abs/2411.14585)
Comments:
          arXiv admin note: text overlap with arXiv:2411.00140

- **What's New**: 이 논문은 PointNet의 특징 추출 기능과 메모리 내(computing in-memory) 처리 및 에너지 효율성을 갖춘 신경모방(neuromorphic) 시스템을 결합한 새로운 접근 방식을 제안합니다. 이 방식은 공간-시간(spatio-temporal) 신호 인식 문제를 해결하기 위해 개발된 PointLCA-Net알고리즘을 포함하고 있습니다. PointLCA-Net은 해석 및 훈련 동안 다른 방법들에 비해 에너지 부담이 상당히 낮으며 고정확도의 인식을 달성합니다.

- **Technical Details**: 제안된 방법은 두 단계로 이루어져 있습니다. 첫 번째 단계에서 PointNet은 공간-시간 신호로부터 특징(feature)을 추출하고, 이러한 특징은 비휘발성 메모리 요소의 크로스바 어레이에 저장됩니다. 두 번째 단계에서는 이 특징들이 단일층 스파이킹 뉴럴 네트워크(SNN) 인코더-디코더에 의해 처리됩니다. 이 SNN은 Locally Competitive Algorithm (LCA)을 사용하여 인코딩 및 분류를 효율적으로 수행합니다.

- **Performance Highlights**: PointLCA-Net은 에지 디바이스(edge devices)에서 공간-시간 데이터에 대한 높은 인식 정확도를 달성하며, 훈련과 추론 중에 필요한 계산 리소스 및 에너지 효율성을 낮추도록 최적화되어 있습니다. 이러한 결과는 복잡한 데이터에 대한 실시간 처리에서 전통적인 컴퓨팅 시스템의 한계를 극복하고, 에너지 제약이 있는 환경에서의 신경 네트워크 아키텍처 배치 가능성을 높입니다.
...
### End-to-End Convolutional Activation Anomaly Analysis for Anomaly Detection (https://arxiv.org/abs/2411.14509)
- **What's New**: 본 논문에서는 End-to-end Convolutional Activation Anomaly Analysis (E2E-CA$^3$)라는 새로운 접근법을 제안합니다. 이는 기존 A$^3$ 이상 탐지 기법을 더욱 확장한 것으로, CNN(Convolutional Neural Network) 기반의 구조를 이용하여 이미지 및 표 형태 데이터 모두에 자연스럽게 적용할 수 있습니다. 경량 아키텍처에도 불구하고, MNIST, CIFAR-10 및 KDDcup99과 같은 일반 데이터셋에서 매우 뛰어난 이상 탐지 성능을 보여줍니다.

- **Technical Details**: 제안된 방법은 두 개의 신경망으로 구성됩니다: 하나는 Convolutional Variational Autoencoder(타겟 네트워크)로, 일반적인 데이터만을 사용하여 훈련됩니다. 다른 하나는 CNN 기반의 보조 네트워크(알람 네트워크)로, 타겟 네트워크의 활성화를 입력으로 받아 이진 분류를 수행합니다. 이 두 네트워크는 동시에 훈련되어 활성화 정보와 재구성 손실을 결합하여 함께 학습함으로써 이상 탐지의 정확성을 높입니다.

- **Performance Highlights**: E2E-CA$^3$ 방법은 기존 A$^3$ 접근법 및 유사한 복잡도의 다른 최첨단 방법들과 비교했을 때 매우 긍정적인 결과를 보였습니다. 경량화된 아키텍처에도 불구하고 성능이 우수하며, 다양한 비정상 데이터 탐지 작업에서 적용 가능성을 제시합니다. 이를 통해 이상 탐지 분야에서의 기술적 진전을 도모할 수 있을 것으로 기대됩니다.
...
### Why you don't overfit, and don't need Bayes if you only train for one epoch (https://arxiv.org/abs/2411.14478)
- **What's New**: 이 논문에서는 데이터가 풍부한 환경에서 최대 우도(maximum likelihood) 훈련이 실제 데이터 생성 과정(true DGP) 손실을 최적화함을 보여줍니다. 특히, 단 한 번의 에폭(epoch)에서 모델을 훈련하는 설정에서도 동일한 목표를 최적화할 수 있으며, 이는 베이지안 추론(Bayesian inference)과 같은 결과를 가져온다고 주장합니다. 이는 LLMs와 같은 최신 모델에서 베이지안 또는 관련 방법이 중요하지 않은 이유를 설명합니다.

- **Technical Details**: 소개 부분에서는 초기 딥러닝 시대의 작은 데이터셋과 많은 에폭 훈련에 대해 설명하며, 이로 인해 Bayesian 신경망(Bayesian neural networks)과 앙상블(ensembles) 방법이 과적합(overfitting)을 완화하는데 사용되었다고 말합니다. 그러나 최근에는 대규모 데이터셋을 사용하고 단일 에폭 훈련만으로도 과적합 문제가 줄어들고 있으며, 이로 인해 베이지안 방법이 LLM 사전 훈련에서는 드물게 사용됩니다. 베이지안 추론은 실제 데이터 생성 과정 아래에서 기대 로그우도를 최소화하는 것으로 이해할 수 있습니다.

- **Performance Highlights**: 결과 부분에서는 최대 우도 훈련이 왜 제한된 데이터에서 과적합되는지를 논의합니다. 실제 데이터 생성 과정에 기반한 손실을 최적화하는 것이 가능함을 보이며, 데이터 세트가 충분하고 단일 에폭에서 훈련이 가능할 때 표준 최대 우도 훈련이 매우 효과적이라는 점을 강조합니다. 따라서 gradient descent 알고리즘으로도 성공적인 최적화를 수행할 수 있다는 것을 제시하고 있으며, 이는 LLM 사전 훈련에서 사용되는 표준 목표와 일치합니다.
...
### Activation Functions for "A Feedforward Unitary Equivariant Neural Network" (https://arxiv.org/abs/2411.14462)
- **What's New**: 이번 논문에서는 기존 연구에서 소개된 feedforward unitary equivariant neural network의 활성화 함수들을 하나의 일반적인 형태로 일반화하였습니다. 새로운 활성화 함수는 소수의 잔여(residue)를 가진 softsign 함수, 항등(identity) 함수, 그리고 Leaky ReLU 함수를 포함하여 다양성을 높였습니다. 이는 신경망 설계에 더 큰 유연성을 제공하며, 기존의 특정 활성화 함수의 한계를 극복할 수 있습니다.

- **Technical Details**: 제안된 활성화 함수는 unitary equivariance를 유지하면서도 넓은 클래스의 함수들을 나타낼 수 있는 기능적 형태로 확장되었습니다. 이러한 일반화는 다양한 종류의 활성화 함수가 범용적으로 적용될 수 있도록 하여 신경망의 구조적 설계를 가능하게 합니다. 이로 인해 응용의 폭이 넓어지고, 다양한 문제에 대한 최적화가 가능해집니다.

- **Performance Highlights**: 이 연구는 활성화 함수의 일반화가 신경망의 성능을 어떻게 향상시킬 수 있는지를 보여주고 있습니다. 새로운 방식으로 구성된 활성화 함수는 함께 사용될 경우, 신경망의 등가성(equivariance) 특성을 더욱 잘 유지하며, 다양한 응용 프로그램에서 유용할 수 있습니다. 이는 최종적으로 기존 시스템의 한계를 극복하는 데 기여할 것으로 기대됩니다.



### Guiding Reinforcement Learning Using Uncertainty-Aware Large Language Models (https://arxiv.org/abs/2411.14457)
Comments:
          8 pages, 7 figures

- **What's New**: 본 논문은 대규모 환경에서의 강화 학습(Reinforcement Learning, RL) 훈련을 보완하기 위해 적절히 조정된 Large Language Models(LLMs)의 사용을 제안합니다. Monte Carlo Dropout을 사용하여 LLM의 조정된 가이드를 제공하고, 불확실성을 평가하여 RL 정책의 효과를 극대화할 수 있는 새로운 형태의 정책 shaping 방법을 소개합니다. 이를 통해 RL 훈련의 신뢰성을 확보하고 LLM의 조언을 보다 견고하게 만듭니다.

- **Technical Details**: 이 연구에서는 LLM의 예측 변동성을 평가하기 위해 Monte Carlo Dropout을 활용하여 조정된 가이드를 제공합니다. 또한, RL 정책을 동적 모델 평균 엔트로피(dynamc model average entropy)에 기반하여 LLM의 영향을 유도된 불확실성에 따라 조절하는 방법을 개발하였습니다. 이러한 방법은 RL 에이전트가 보다 신뢰할 수 있는 LLM 가이드를 기반으로 robust한 훈련을 받을 수 있도록 해줍니다.

- **Performance Highlights**: 실험 결과, 제안된 방법이 비조정된 LLM, 비유도 RL 및 다양한 shaping 정책을 사용하는 조정된 LLM보다 우수한 성능을 보임을 확인하였습니다. 또한, 평균 엔트로피를 활용한 불확실성 추정 방법이 잘못된 유도의 높은 불확실성을 반영하는 데 효과적임을 보여주었습니다. 이 연구는 fine-tuned LLM의 지속적인 과신 문제를 강조하며, 순차적 의사결정 문제에서의 효과적인 교정의 중요성을 보여줍니다.
...
- **What's New**: 본 연구는 물리 정보 신경망(Physics-Informed Neural Networks, PINNs)에서 학습 가능한 활성화 함수(learnable activation functions)를 사용하는 방법을 조사하고, 고정된 활성화와 학습 가능한 활성화를 갖는 전통적인 다층 퍼셉트론(Multilayer Perceptrons, MLPs)과 학습 가능한 기저 함수(learnable basis functions)를 사용하는 콜모고로프-아놀드 네트워크(Kolmogorov-Arnold Networks, KANs)의 효용성을 비교합니다. 실험 결과는 네트워크의 학습 효율성, 수렴 속도 및 테스트 정확성을 조화롭게 설계하는 데 대한 통찰력을 제공합니다.

- **Technical Details**: PINNs는 물리 법칙을 학습 과정에 직접적으로 통합하여 PDE 문제를 해결하는 효과적인 방법으로 부상했습니다. PINNs는 손실 함수의 일환으로 지배 방정식과 경계 조건을 포함하여 제한된 데이터로도 높은 정확도를 제공하는 데이터 효율적인 접근 방식을 제시합니다. 본 연구에서는 다양한 활성화 및 기저 함수가 MLP와 KAN의 수렴 행동 및 스펙트럴 바이어스(spectral bias)에 미치는 영향을 평가합니다.

- **Performance Highlights**: KAN은 스펙트럴 바이어스가 감소하여 고주파 구성 요소를 효과적으로 포착할 수 있는 특징을 보이며, 이는 다중 스케일 특성을 정확하게 모델링하는 데 필수적입니다. MLP와 KAN을 포함한 다양한 PDE 문제에 대한 적용을 탐구하고, 활성화 함수와 기저 함수의 선택에 따라 네트워크가 PDE 해의 중요한 특징을 포착하는 능력을 평가합니다. 본 연구에서 제시된 소스 코드는 재현성을 높이고 미래의 탐색을 촉진하기 위해 공개되어 있습니다.
...
### Linear convergence of proximal descent schemes on the Wasserstein spac (https://arxiv.org/abs/2411.15067)
Comments:
          28 pages

- **What's New**: 본 연구에서는 Wasserstein 공간에서 엔트로피를 정규화한 함수 최적화 문제를 다루며, Jordan-Kinderlehrer-Otto(JKO) 방식의 근사적 내리막 기법을 제안합니다. 이러한 방법은 일반적인 기하학적 볼록성의 의존성을 완화하고, 선형 수렴성을 확립합니다. 특히, 엔트로피 샌드위치 레마(entropy sandwich lemma)와 로그 소베레프 부등식(logarithmic Sobolev inequality)을 활용하여 EVI의 이산 시간 적응 필요성을 회피합니다.

- **Technical Details**: 연구에서는 Wasserstein 공간 (𝒫2⁢(ℝd), 𝒲2)에서 평탄한 볼록 함수의 엔트로피 정규화된 최적화를 목표로 합니다. 본 방법은 이산 시간 스텝 계획(discrete-time stepping schemes)의 관점에서 접근하며, proximal point, prox-linear, proximal gradient 같은 여러 방법을 제안합니다. 특히, 조닝 내에 있는 변화를 통해 각 단계에서 상대 피셔 정보(relative Fisher information)를 정의할 수 있음을 증명하며, 즉각적 엔트로피는 바이사키스가 다르지 않기 때문에 특수한 소베레프 정규성(sobolev regularity)을 유지합니다.

- **Performance Highlights**: 이러한 최적화 알고리즘은 데이터 과학 및 기계학습 분야의 다양한 응용 사례에서 유용하게 사용될 수 있습니다. 특히, 두 층 신경망 훈련, 생성 모델링, 강화 학습과 같은 작업에서 성능을 발휘합니다. 본 연구에서 제안된 JKO 기반 방법은 파라미터 조정 없이 선형적으로 수렴하며, 최적 해(minimizer)로의 수렴 보장을 제공합니다.
...
### A New Way: Kronecker-Factored Approximate Curvature Deep Hedging and its Benefits (https://arxiv.org/abs/2411.15002)
Comments:
          16 pages, 5 figures

- **What's New**: 이 논문은 Deep Hedging 프레임워크의 계산 효율성을 향상시키기 위해 Kronecker-Factored Approximate Curvature (K-FAC) 최적화를 통합했습니다. 최근 연구에서는 Deep Hedging이 기존 리스크 관리 전략에 대한 데이터 기반 대안으로 자리 잡고 있으나, 신경망 훈련 시 발생하는 계산적 부담이 큰 문제로 남아 있었습니다. 제안된 아키텍처는 Long Short-Term Memory (LSTM) 네트워크를 K-FAC 2차 최적화와 결합하여 순차적인 금융 데이터와 곡률 추정의 문제를 해결합니다.

- **Technical Details**: 이 연구에서 사용되는 Deep Hedging 프레임워크는 순환 신경망 (RNN) 아키텍처를 기반으로 하며, K-FAC 최적화기를 통합하여 훈련 방법론을 개선하고자 합니다. K-FAC 알고리즘은 Fisher Information Matrix의 효율적인 근사를 제공하여 대규모 네트워크에서 2차 최적화 방법의 실행을 가능하게 합니다. 이를 통해 신경망 훈련의 효율성을 높이고 모델 성능을 개선할 수 있는 기회를 제시합니다.

- **Performance Highlights**: 실험을 통해 K-FAC 구현이 수렴 동적 및 헤징 효율성에서 현저한 개선을 보여준 것을 설명합니다. K-FAC을 적용한 모델은 Adam 최적화와 비교할 때 거래 비용이 78.3% 감소하고, 손익 변동성이 34.4% 줄어드는 결과를 도출했습니다. 또한, K-FAC 강화 모델의 Sharpe 비율은 0.0401로, 기준 모델의 -0.0025에 비해 우수한 리스크 조정 성과를 나타냈습니다.
...
### CardioLab: Laboratory Values Estimation and Monitoring from Electrocardiogram Signals -- A Multimodal Deep Learning Approach (https://arxiv.org/abs/2411.14886)
Comments:
          7 pages, 1 figure, code under this https URL

- **What's New**: 이번 연구는 ECG 데이터와 비슷한 임상 데이터를 통해 실시간으로 실험실 값의 이상을 추정 및 모니터링할 수 있는 가능성을 처음으로 보여줍니다. 이 접근법은 비침습적이며 비용 효율적인 기존 실험실 검사에 대한 보완책이 될 수 있습니다. 연구 결과는 환자의 모니터링과 조기 개입을 강화할 수 있는 중요한 함의를 가지고 있습니다.

- **Technical Details**: MIMIC-IV 데이터셋을 활용하여 MULTIMODAL deep-learning 모델을 개발하였습니다. 12-lead ECG, 인구통계학적 데이터, 생체 신호, 그리고 바이탈 사인 데이터를 통합하여 정상 및 비정상 실험실 값을 예측하는 이 모델은 강력한 예측 성능을 보였으며, 총 23개 이상의 실험실 값에 대해 AUROC 점수 0.70 이상을 기록했습니다.

- **Performance Highlights**: 모델은 여러 생리학적 카테고리(심장, 신장, 혈액학, 신진대사 등)의 이상 값을 정확하게 예측할 수 있었습니다. 예를 들어, NTproBNP 레벨 353 pg/mL 이상을 0.882의 AUROC로 추정하였고, 모니터링 작업에서는 120분 후에 Urea nitrogen 레벨을 0.854로 예측하였습니다. 이러한 성과는 실험실 값 이상 예측의 신뢰성을 높이는 데 기여할 수 있습니다.
...
### Iterative Reweighted Framework Based Algorithms for Sparse Linear Regression with Generalized Elastic Net Penalty (https://arxiv.org/abs/2411.14875)
- **What's New**: 이번 논문에서는 일반화된 탄력적 네트워크 모델(Generalized Elastic Net Model)을 제안하며, ${\ell}_r$-norm을 손실 함수에 도입하여 다양한 노이즈 형태를 수용합니다. 이를 통해 ${\ell}_q$-norm(여기서 0 < q < 1)으로 탄력적 네트워크 패널티를 대체하여 회귀 성능을 개선할 수 있음을 보여줍니다. 또한 응용을 위한 두 가지 효율적인 알고리즘을 개발하여 실제 데이터를 통한 실험 결과를 제시합니다.

- **Technical Details**: 제안된 모델에서 우리는 비제로(nonzero) 항목의 하한(computable lower bounds)을 이론적으로 확립합니다. 알고리즘 구현을 위해, 첫 번째 알고리즘은 ADMM(Alternating Direction Method of Multipliers)을 사용하며, 두 번째 알고리즘은 PMM(Proximal Majorization-Minimization) 방법을 이용하고, 하위 문제는 SNN(Semismooth Newton Method)으로 해결됩니다. 이러한 기술들은 기존의 통계적 방법들보다 더욱 다양한 노이즈 상황에서의 회귀 성능을 나타냅니다.

- **Performance Highlights**: 상세한 수치 실험을 통해 제안된 두 알고리즘은 모두 우수한 성능을 보여주었습니다. 특히 PMM-SSN이 ADMM보다 효율적임을 보여주었으며, 이러한 성과는 고차원 희소 회귀 문제에서 더 나은 성능을 나타냅니다. 실험 결과는 다양한 데이터셋에서 좋은 결과를 달성했습니다.
...
### Bayesian dynamic mode decomposition for real-time ship motion digital twinning (https://arxiv.org/abs/2411.14839)
- **What's New**: 이 논문에서는 선박의 움직임의 nowcasting을 위한 예측 도구로서 Bayesian 방식을 적용한 Hankel 동적 모드 분해 방법을 제안합니다. 이 알고리즘은 디지털 트윈을 위한 요건을 모두 충족하며, 물리 시스템에서 수집된 데이터로 모델을 조정하고 실시간 예측을 제공합니다. 특히, 데이터의 양이 제한된 상황에서도 높은 신뢰도를 가진 예측을 가능하게 합니다.

- **Technical Details**: Hankel-DMD(혹은 Augmented DMD)는 시간 지연 복사본을 상태로 확장하는 고전적인 DMD의 변형으로, 선박의 움직임 변수를 정상 분포의 불확실한 변수로 정의하는 Bayesian 공식화가 적용됩니다. 이 알고리즘은 새로운 데이터를 지속적으로 통합하여 시스템의 근사치를 업데이트하며, 선박의 진행 상황을 실시간으로 예측할 수 있도록 설계되었습니다. 또한, 예측 과정에서 모드 확장을 통해 매우 빠른 평가가 가능합니다.

- **Performance Highlights**: 논문에서는 Fr = 0.33에서 비정상적인 파도 상태 7에서 5415M 모델의 코스 유지를 위해 세 가지 CFD 솔버로부터 수집된 데이터를 사용하여 예측 결과를 제시합니다. Bayesian 방식을 통해 개선된 결정론적 예측이 이루어졌으며, 예측 정확도는 최대 다섯 번의 파도 접촉 주기까지 유지됨을 보여주었습니다. 또한 예측된 불확실성과 예측 정확도 간의 관계도 발견되었습니다.
...
### Cosmological Analysis with Calibrated Neural Quantile Estimation and Approximate Simulators (https://arxiv.org/abs/2411.14748)
Comments:
          5+4 pages, 5+3 figures, to be submitted, comments are welcome

- **What's New**: 이번 연구에서는 Neural Quantile Estimation (NQE)이라는 새로운 Simulation-Based Inference (SBI) 방법을 소개합니다. 이 방법은 많은 수의 근사 시뮬레이션을 기반으로 훈련하고 소수의 고충실도 시뮬레이션을 통해 보정을 수행하여, 우주의 대규모 구조 분석에서 정보를 추출하는 데 필요한 계산 비용을 크게 줄이고 있습니다. NQE는 비편향의 posterior를 보장하며, 근사 시뮬레이션의 정확도가 적절할 경우 거의 최적의 제약력을 달성할 수 있습니다.

- **Technical Details**: NQE는 관측 데이터에서 모델 파라미터 𝜽 (bold_italic_θ)를 직접 추론할 수 있는 SBI 방법입니다. 주어진 시뮬레이션 데이터셋 (𝜽,𝐱) (bold_italic_θ, bold_x)에 대해, NQE는 공동 posterior를 1차원 조건부 posterior로 분리하고, 각 조건부 posterior의 분위수를 신경망을 이용해 최적화합니다. 이러한 방식으로 NQE는 Normalizing Flows (NF)를 사용하지 않아 더 간단한 네트워크 구조를 유지합니다.

- **Performance Highlights**: NQE를 사용하여 훈련한 결과, 고충실도 Particle-Particle (PP) 시뮬레이션에서 얻은 posterior에 필적하는 결과를 제공하면서도 계산 비용을 현저히 낮출 수 있었습니다. 또한, NQE는 2차원 암흑 물질 밀도 맵을 통해 우주론적 파라미터를 추론할 수 있는 능력을 입증했습니다. 이 알고리즘은 대규모 데이터셋을 효과적으로 처리하며, 결과적으로 정확한 추론을 가능하게 합니다.
...
### Can GNNs Learn Link Heuristics? A Concise Review and Evaluation of Link Prediction Methods (https://arxiv.org/abs/2411.14711)
- **What's New**: 본 논문은 Graph Neural Networks (GNNs)를 활용한 링크 예측(link prediction)의 성능을 향상시킬 수 있는 새로운 통찰력을 제공한다. 특히, GNN이 두 노드 간의 구조적 정보, 특히 공통 이웃의 수를 효과적으로 학습하지 못한다는 점에 주목하였다. 연구는 학습 가능한 노드 임베딩(trainable node embeddings)을 도입함으로써 GNN 기반 링크 예측 모델의 성능을 크게 향상시킬 수 있음을 보인다.

- **Technical Details**: 이 연구에서는 GNN이 공통 이웃 수와 관련된 쌍 특정(structural information) 구조적 정보를 효과적으로 학습할 수 있는지에 대해 검토하였다. 특히, GNN의 대표적인 이웃 집계(neighborhood aggregation) 방식은 각 노드의 이웃 수를 반영하지 않는 집합 기반 풀링(set-based pooling) 특성으로 인해, 노드 표현(node representation)이 공통 이웃의 수와 관련된 정보를학습하기 어려움을 강조하였다. 또한, 실험을 통해 GCN과 GAT에서 학습 가능한 노드 임베딩을 활용 시 성능 향상을 볼 수 있었다.

- **Performance Highlights**: 연구 결과, GNN 기반 링크 예측 모델에서 훈련 가능한 노드 임베딩을 통합하면 성능이 향상되며, 그래프가 밀집할수록 그 향상이 더욱 두드러진다는 것을 발견하였다. 밀집 그래프에서는 각 노드가 더 많은 이웃 노드와 연결되어, 링크 샘플 간의 관계를 더 잘 기억할 수 있는 기회를 제공한다. 마지막으로, 기존 링크 예측 방법의 한계를 진단하여 향후 보다 강력한 알고리즘 개발을 위한 기초 자료를 제공한다.
...
### A Data-Driven Pool Strategy for Price-Makers Under Imperfect Information (https://arxiv.org/abs/2411.14694)
Comments:
...
- **What's New**: 이 논문은 불완전한 정보에서 가격 결정자의 수영장 전략(pool strategy)에 대한 연구를 다룹니다. 전력 시스템의 필수 전송 매개변수를 이용할 수 없는 상황에서, 가격 결정자는 역사적 정보를 사용하여 시장 결과를 추정해야 합니다. 이 연구는 rim 다중 매개변수 선형 프로그래밍(rim-MPLP) 이론을 사용하여 경제 배치 모델을 분석하고 있습니다.

- **Technical Details**: 본 논문에서는 제너레이션 컴퍼니(GenCo)의 입찰 곡선과 시스템 패턴을 매핑하기 위해 서포트 벡터 머신(SVM)을 기반으로 한 다중 클래스 분류 모델을 훈련합니다. 이 모델은 가격 결정자의 의사 결정 프레임워크에 통합되며, 경량 최적화 기술을 사용하여 결정 문제를 해결합니다. 특히, 주요 제약 조건에 대한 GenCo의 입찰 영향력을 학습하여 시장 결과를 데이터 기반 방식으로 모델링합니다.

- **Performance Highlights**: 제안된 방법의 성능은 IEEE 30-bus 시스템, 일리노이 주의 인공 200-bus 시스템, 그리고 사우스 캐롤라이나의 인공 500-bus 시스템에서 검증되었습니다. 이는 제안된 접근 방식이 실제 시장에서의 적용 가능성을 높이는 데이터 중심 모델링을 통해 가격 결정자의 결정을 도울 수 있음을 시사합니다.
...
### Exploring the Use of Machine Learning Weather Models in Data Assimilation (https://arxiv.org/abs/2411.14677)
- **What's New**: 본 연구는 기계 학습(ML) 모델의 기상 예측 시스템에서의 활용 가능성을 평가합니다. 특히 GraphCast와 NeuralGCM 모델의 접선 선형(tangent linear, TL) 및 수반(adjoint, AD) 모델을 데이터 동화(data assimilation, DA) 시스템에 통합할 수 있는지 분석합니다. 이 연구는 4차원 변동(4DVar) DA 프레임워크에의 적용 가능성을 탐구하며, ML 모델이 기존의 수치 기상 예측(numerical weather prediction, NWP) 모델에 비해 어떻게 개선될 수 있는지를 논의합니다.

- **Technical Details**: GraphCast는 그래프 신경망(graph neural network, GNN)을 사용하여 기상 예측을 수행하며, 이는 비정형 격자 구조를 효과적으로 처리할 수 있는 장점이 있습니다. 반면, NeuralGCM은 전통적인 동적 코어와 기계 학습 기반의 물리적 프로세스 매개변수를 통합하여 복잡한 서브 그리드(scale) 현상을 잘 표현합니다. 이 연구에서는 두 모델의 TL 및 AD를 개발하고, 이를 통해 4DVar DA 시스템에 적합한지 평가하기 위해 각 모델의 물리적 일관성(physical consistency)과 신뢰성(reliability)을 비교합니다.

- **Performance Highlights**: GraphCast와 NeuralGCM의 수반 결과는 MPAS-A 모델과 약간의 유사성을 보이지만, 다양한 수직 수준에서 비물리적 노이즈(unphysical noise)가 발생하는 문제가 있습니다. 이 연구는 이러한 문제들이 작동 가능한 DA 시스템에 통합하는 데 위험 요소가 될 수 있음을 강조하며, ML 모델의 TL/AD의 물리적 일관성을 확보하는 것이 기상 예측의 정확도 향상에 필수적임을 보여줍니다. 따라서, 제기된 문제를 해결하는 것은 기상 예측의 효율성과 정확도를 높이는 데 꼭 필요합니다.



### Brain-Computer Interfaces for Emotional Regulation in Patients with Various Disorders (https://arxiv.org/abs/2411.14666)
- **What's New**: 이 연구는 EEG 기반의 Brain-Computer Interface (BCI)를 이용하여 신경학적 및 생리학적 장애가 있는 개인의 감정 조절을 향상시키는 응용 가능성을 탐구합니다. 특히, 새로운 신경망 알고리즘을 개발하여 EEG 데이터를 이해하고 감정 상태를 인식 및 조절하는 데 중점을 두고 있습니다.

- **Technical Details**: 이 실험에서는 open-Neuro로부터 EEG 기반 감정 데이터를 수집하고, 데이터 수정을 위한 새로운 기법을 활용하여 환자의 신경 패턴과 감정 변화를 반영한 데이터셋을 생성합니다. 이 알고리즘은 감정 상태를 높은 정확도로 분류할 수 있어, EEG 기반 BCI의 실용성을 보여줍니다.

- **Performance Highlights**: 데이터 분석 결과, 이 알고리즘은 감정 상태 분류에서 높은 정확도를 달성하며, 감정 인식을 돕고 이를 조절하는 데 귀중한 도구가 될 수 있음을 시사합니다. 향후 연구에서는 신경 장애를 가진 환자의 데이터 수집을 통해 샘플 다양성을 개선할 필요가 있습니다.



### Double Machine Learning for Adaptive Causal Representation in High-Dimensional Data (https://arxiv.org/abs/2411.14665)
- **What's New**: 이 논문은 관찰 데이터를 기반으로 한 적응형 인과Representation Learning을 제안하며, 효율적인 샘플 분할 기술이 반파라메트릭(establishing equation) 추정 프레임워크와 통합되어 있습니다.

- **Technical Details**: 샘플 분할 기법으로는 에너지 거리(energy distance)를 기반으로 한 지원 포인트 샘플 분할(SPSS)이 사용되며, 이는 인과 추론에서의 이중 머신 학습(Double Machine Learning, DML)에 최적화된 접근 방식을 제공합니다. 기존의 무작위 분할(random splitting) 방식과 달리, SPSS는 전체 원시 데이터의 최적 대표 지점을 선택하여 제공합니다.

- **Performance Highlights**: SSS를 사용한 딥러닝(DL) 및 하이브리드 슈퍼러너(SL) 방식은 SVM보다 계산 효율성과 추정 품질 면에서 우수한 성과를 보여주었습니다. 이 연구에서는 SVM과 DL, SDL의 비교 연구를 수행하였으며, Chernozhukov et al. (2018)의 기준 결과와의 비교를 통해 이점이 강조되었습니다.



### Sparsifying Suprema of Gaussian Processes (https://arxiv.org/abs/2411.14664)
Comments:
          30 pages

- **What's New**: 이번 연구에서는 중심 가우시안 프로세스의 suprema에 대한 차원 독립적인 희소화 결과를 제시합니다. 이 결과는 무한 집합을 포함할 수 있는 벡터 집합 T에 대해 O(ε(1)) 크기의 부분 집합 S를 찾아내어, S의 요소들과 관련된 실수 값 c들이 원래 Gaussian 프로세스의 suprema를 ε-근사할 수 있음을 보여줍니다. 이러한 결과는 T의 크기나 그 주위 차원 n과는 독립적입니다.

- **Technical Details**: 연구에서 소개된 중심 가우시안 프로세스는 무한 개의 랜덤 변수들에 대한 조인트 분포로, 이들 각 변수의 유한 부분 집합이 다변량 가우시안 분포를 따른다고 정의됩니다. 본 연구는 Talagrand의 majorizing measures theorem에 의존하여 이론적인 근거를 마련하였습니다. 이를 통해 도출된 희소화 결과는 에 길이 기준으로 원래 Gaussian 프로세스에 대한 높은 정확도의 근사값을 제공합니다.

- **Performance Highlights**: 이 논문이 제시하는 희소화 결과는 또한 정Convex sets로의 응용 가능성을 나타내며, 다수의 halfspaces의 교차로를 간소화할 수 있습니다. 특히, 원점에서 거리 O(1)인 halfspaces의 교차는 ε-근접 (N(0,I_n) 하에서) 하여 O(ε(1))개의 halfspaces로 표현될 수 있음을 입증합니다. 이는 비관적인 학습(agnostic learning) 및 허용 성질 테스트(tolerant property testing)와 같은 실제 응용으로 이어질 수 있습니다.
...
### ACE-Net: AutofoCus-Enhanced Convolutional Network for Field Imperfection Estimation with application to high b-value spiral Diffusion MRI (https://arxiv.org/abs/2411.14630)
Comments:
          8 pages, 5 figures, submitted to International Society for Magnetic Resonance in Medicine 32th Scientific Meeting, 2025

- **What's New**: 이번 연구에서는 B0 비균일성과 확산 부호화로 인해 발생하는 전류로 인한 자기장 변화를 자동으로 추정하기 위한 데이터 기반 접근 방식을 개발했습니다. 이는 autofocus metrics와 딥러닝을 결합하고, 예상되는 자기장 결함의 콤팩트한 기저 표현을 활용하여 이루어졌습니다. 이 방법은 높은 b-값을 가지는 단일 촬영 스파이럴 확산 MRI에 적용되었습니다.

- **Technical Details**: 연구에서 제안된 접근법은 자기장 결함을 정확하게 추정하는 데 사용되며, 이를 통해 얻어진 B0 및 전류의 정확한 추정 결과는 고품질 이미지 재구성을 가능하게 합니다. 추가적인 외부 보정(calibration) 없이도 우수한 성능을 발휘하여, 빠른 이미지 인코딩 방식에 유용하게 적용됩니다.

- **Performance Highlights**: 결과적으로, 개발된 방법은 고해상도 이미지를 효과적으로 재구성하여 이미지 아티팩트(artifacts)를 최소화했습니다. 특히, 스파이럴, EPI 및 3D-cones와 같은 이미지 인코딩 기술에서 발생할 수 있는 문제점을 성공적으로 해결했습니다.
...
- **What's New**: 본 연구에서 제안하는 CodeSAM은 소스 코드의 다양한 코드-뷰를 변형된 self-attention 기법을 통해 transformer 모델에 효과적으로 주입할 수 있는 확장 가능한 프레임워크입니다. 이를 통해 기존 모델의 입력 토큰 시퀀스를 변경하지 않고 코드-뷰 정보를 통합할 수 있습니다. CodeSAM은 코드 조각과 연결된 토큰만을 모델이 주목할 수 있도록 하여, 코드 간의 의미론적 정보를 보강합니다.

- **Technical Details**: CodeSAM의 핵심 기법은 self-attention 메커니즘을 수정하여 커스터마이즈된 코드-뷰에 기반한 attention mask A′를 생성하는 것입니다. 이 과정에서 원래의 attention matrix A를 Hadamard 곱을 이용해 A′와 결합하여, 코드-뷰의 정보가 풍부하게 내포된 학습 방식을 구현합니다. 이런 방식은 입력 시퀀스에 영향을 주지 않으며, 이는 코드-뷰를 평면화하여 추가하는 기존 방법들의 장점을 대체합니다.

- **Performance Highlights**: CodeSAM은 코드 클론 탐지, 의미론적 코드 검색 및 프로그램 분류 등의 실제 SE 작업에서 개선된 성능을 보였습니다. 실험 결과, GraphCodeBERT 및 CodeBERT와 같은 기존 소형 언어 모델들에 비해, 단일 코드-뷰 또는 여러 코드-뷰의 조합 사용 시 세 가지 작업 모두에서 성능 향상을 보여주었습니다. 이러한 결과는 CodeSAM과 같은 기법이 자원 제한 설정에서도 효율적인 코드 SLM을 개발할 수 있음을 시사합니다.



### On Linear Convergence in Smooth Convex-Concave Bilinearly-Coupled Saddle-Point Optimization: Lower Bounds and Optimal Algorithms (https://arxiv.org/abs/2411.14601)
- **What's New**: 이 논문에서는 smooth convex-concave bilinearly-coupled saddle-point 문제를 재조명합니다. 특히, 각 함수 $f(x)$와 $g(y)$가 아핀(affine) 또는 강한 볼록(strongly convex)일 때, 최적의 알고리즘과 더불어 기울기 평가(gradient evaluations) 및 행렬-벡터 곱(matrix-vector multiplications)의 하한(bound)을 제시합니다. 또한, 더 넓은 범위의 문제 클래스에서 선형 수렴(linear convergence)이 가능하다는 점에서 차별화됩니다.

- **Technical Details**: 저자들은 기울기 평가와 행렬-벡터 곱의 복잡성을 분리하는 알고리즘을 개발하였으며, 이는 이론적으로 가장 우수한 성능을 제공합니다. 논문은 smooth non-strongly convex 함수가 포함된 경우에도 적용 가능한 첫 번째 하한 경계(lower complexity bounds)를 제시하고, 기존 문헌의 결과들을 통합합니다. 이러한 발전으로, 다양한 형태의 saddle-point 문제에 대해 효율적으로 해결할 수 있는 길이 열렸습니다.

- **Performance Highlights**: 알고리즘은 선형 수렴이 가능하며, 이는 문제를 해결하는데 필요한 반복(iterations) 수가 $	heta(	ext{log}(1/	ext{ε}))$에 비례함을 의미합니다. 저자들은 이론적 성능이 이전의 결과들과 비교하여 우수하다는 것을 확인했습니다. 이러한 성과는 동시에 최적의 기울기 평가 및 행렬-벡터 곱 복잡성을 달성하는 데 기여하였습니다.
...
### Deep operator network models for predicting post-burn contraction (https://arxiv.org/abs/2411.14555)
- **What's New**: 이 연구는 화상 후 상처 변화를 예측하기 위해 딥 오퍼레이터 네트워크(DeepONet)를 활용하는 새로운 접근 방식을 제안합니다. 전통적인 수치 모델에 비해 128배에서 235배의 속도 향상을 이루면서, $R^2$ 점수 0.99를 기록해 높은 예측 정확도를 입증합니다. 이 모델은 다양한 상처 형태에 대한 예측을 가능하게 하여 임상 치료 계획에 기여할 것으로 기대됩니다.

- **Technical Details**: 본 연구에서는 생물학적 및 기계적 구성 요소를 포함하는 일반적인 변형 탄성 모델(morphoelastic model)을 사용하여 화상 후 수축을 수학적으로 모델링합니다. 모델은 섬유아세포(fibroblasts), 근섬유아세포(myofibroblasts), 신호 분자(signaling molecule), 콜라겐(collagen) 등의 생물학적 요소와 피부 기계적 변위를 나타내는 변수를 다룹니다. 특히, 딥 오퍼레이터 네트워크는 상대적으로 작은 데이터셋으로부터 효율적으로 연산자를 학습할 수 있도록 설계되었습니다.

- **Performance Highlights**: 딥 오퍼레이터 네트워크는 1년 이상의 장기 예측을 가능하게 하며, 기존 수치 모델에 비해 연산 속도를 비약적으로 향상시킵니다. 이 모델의 결과는 임상 환경에서의 실제 응용 가능성을 뒷받침하며, 화상 후 피부 변화를 더 효과적으로 예측할 수 있는 능력을 보여줍니다. 연구 결과는 향후 화상 치료 및 재생의학 분야에 중대한 영향을 미칠 것으로 보입니다.
...
### Detecting Distributed Denial of Service Attacks Using Logistic Regression and SVM Methods (https://arxiv.org/abs/2411.14512)
- **What's New**: 이 논문은 DDoS 공격을 탐지하고 이를 분류하는 방법에 대한 최신 접근 방식을 제시합니다. 특히, HTTP flood, UDP flood와 같은 다양한 DDoS 공격 유형을 효과적으로 식별하는 것이 핵심입니다. DDoS 공격을 정상 트래픽과 차별화하는 기술적 관점을 통해 네트워크 보안의 주요 이슈를 해결하고자 합니다.

- **Technical Details**: 연구에서는 인터넷에서 수집한 표준 데이터셋을 활용하여 여러 네트워크 관련 속성과 DDoS 공격 클래스 이름을 포함합니다. 두 가지 기계 학습 접근법인 SVM (Support Vector Machine)과 Logistic Regression을 적용하여 DDoS 공격을 탐지하고 분류합니다. 이들 기법은 정확도(accuracy), 정밀도(precision), 재현율(recall) 측면에서 비교 분석됩니다.

- **Performance Highlights**: 두 기법 모두 98.65%의 분류 정확도를 달성하여 같은 데이터셋을 사용한 이전의 실험들 중 가장 높은 정확도를 기록했습니다. 이 결과는 DDoS 공격 탐지에서 기계 학습의 유용성을 강조하며, 향후 연구 방향에 대한 귀중한 통찰을 제공합니다.
...
### Past, Present, and Future of Sensor-based Human Activity Recognition using Wearables: A Surveying Tutorial on a Still Challenging Task (https://arxiv.org/abs/2411.14452)
- **What's New**: 이 논문은 wearable sensor 기반의 Human Activity Recognition (HAR) 분야에서의 진전을 다룹니다. 특히, 기존의 수작업으로 개발된 기능들을 넘어, foundational models에서의 세계 지식을 통합하여 새로운 패러다임의 변화를 예고합니다. HAR 시스템 개발을 위한 실제적인 튜토리얼을 제공하여 연구자와 실무자 모두에게 도움을 주고자 합니다.

- **Technical Details**: 저자들은 HAR의 기원부터 현재 상태까지를 면밀히 조사하며, 활동 인식 문제를 해결하기 위한 다양한 방법론을 소개합니다. 이 과정에서 sensor data의 증가와 연구 기법의 복잡성에도 불구하고, 성능이 여전히 부족한 상황을 지적합니다. 따라서, 사용자가 HAR 시스템을 개발할 수 있도록 하는 실습적 튜토리얼이 포함되어 있습니다.

- **Performance Highlights**: HAR 커뮤니티는 새로운 패러다임 변화에 직면하고 있으며, 향후 더 많은 발전을 기대하고 있습니다. 이 논문은 초보자와 전문가 모두를 위한 활동 인식 문제 해결을 위한 방법을 정리한 자료를 제공합니다. 결론적으로, HAR 분야에서의 지속적인 연구와 개발이 필요하다는 것을 강조하고 있습니다.
...
### Rising Rested Bandits: Lower Bounds and Efficient Algorithms (https://arxiv.org/abs/2411.14446)
Comments:
          62 pages. arXiv admin note: substantial text overlap with arXiv:2212.03798

- **What's New**: 이번 논문은 확률적 멀티 암드 밴딧(stochastic Multi-Armed Bandits, MABs) 분야에서 연구된 것으로, 선택한 옵션에 의해 제공받는 피드백만으로 온라인 학습이 가능한 방법을 다룹니다. 특히, 팔(arm)의 기대 보상이 단조롭게 증가하고 오목한(rested) 경우에 대해 다루고 있습니다.

- **Technical Details**: 저자들은 후회(regret) 최소화 문제의 내재적인 샘플 복잡도(sample complexity)를 다루기 위해 적절한 후회 하한(bound)을 도출하였습니다. 이를 바탕으로 $	extit{R-ed-UCB}$라는 알고리즘을 설계하였으며, 이 알고리즘은 인스턴스의 특성과 특정 상황 하에 $	ilde{	ext{O}}(T^{rac{2}{3}})$ 형태의 후회 경계를 제공합니다.

- **Performance Highlights**: 저자들은 여러 가지 합성적으로 생성된(task) 문제와 실제 데이터셋에 대한 온라인 모델 선택 문제를 통해 자율적으로 개발한 알고리즘을 최첨단(non-stationary) MAB 방법들과 비교 empirically 하였습니다. 실험 결과는 제안된 알고리즘의 효과성을 보여주고 있습니다.
...
- **What's New**: 이 논문은 제조업에서 기계 고장을 실시간으로 예측하기 위한 새로운 데이터 기반 솔루션인 Transformer Quantile Regression Neural Networks (TQRNNs)를 소개합니다. 이 모델은 기계 시스템의 고장을 정확하게 식별할 수 있는 고급 예측 유지보수 모델 개발을 목표로 하며, 두 단계 접근 방식을 통해 수행됩니다. TQRNNs는 수정된 quantile 회귀 신경망과 연결된 transformer 네트워크를 활용하여 이상값을 분리하고 정확한 분류를 촉진합니다.

- **Technical Details**: TQRNN은 43개의 센서로부터 데이터를 처리하는 입력 블록으로 시작하며, 10×43개의 Quantile Regression Neural Networks (QRNNs)를 포함합니다. 2차 QRNNs 레이어가 초기 결과를 정제하고 최종 transformer 네트워크가 데이터에서 가능한 이상을 식별합니다. 이 모델은 실제 음료 제조 환경에서 시험되었으며, 기계 고장 예측을 위해 1시간의 리드 타임을 가진 70.84%의 정확도를 기록했습니다.

- **Performance Highlights**: TQRNNs를 활용한 예측 유지보수는 기계 고장을 효과적으로 식별할 수 있으며, 고품질 생산을 증가시키는 데 기여했습니다. 제품 수율을 78.38%에서 89.62%로 개선하는 결과를 보였습니다. 이를 통해 불시 정지 시간을 최소화하고 생산 효율성을 극대화하며 운영 안정성을 보장할 수 있는 가능성이 제시되었습니다.

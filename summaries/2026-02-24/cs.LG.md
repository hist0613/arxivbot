...
- **What's New**: 현재 강화 학습 목표는 주로 기대 보상을 최대화하는 데 초점을 맞추고 있습니다. 이에 따라 우세한 보상 신호에 과적합(overfitting)하여 대체 및 유효한 추론 경로를 간과하게 됩니다. 새로운 기법인 Learning Advantage Distributions (LAD)을 도입하여 보상 최대화 대신 장점 유도 분포를 학습하는 프레임워크를 제공합니다.

- **Technical Details**: LAD는 정책이 기대 이득을 극대화하는 대신 장점 유도 분포(advantage-induced distribution)를 일치시키도록 훈련하는 기법으로 구성됩니다. 이 기법은 정책 업데이트와 장점 기반 목표 분포 간의 동등성을 정립하여, 정책 유도 및 장점 유도 분포 사이의 $f$-다이버전스를 최소화하는 실용적인 LAD 목표를 도출합니다. 이를 통해 높은 장점 응답에 대한 가능성을 증가시키고 과도한 확신을 억제하는 그래디언트 업데이트를 생성합니다.

- **Performance Highlights**: LAD는 통제된 밴딧 설정에서 다변량(multi-modal) 장점 분포를 충실하게 재현하여 이론적 공식을 검증했습니다. 여러 LLM 백본을 기반으로 한 수학 및 코드 추론 작업에서 LAD는 GRPO, 엔트로피 조정 기준 및 FlowRL 대비 정확성과 생성적 다양성을 일관되게 개선하는 결과를 보였습니다. 이러한 발견은 복잡한 추론 환경에서 LLM의 RL 기반 후속 훈련을 위한 원칙적이고 효과적인 훈련 목표로 LAD의 유효성을 입증합니다.



### Adaptation to Intrinsic Dependence in Diffusion Language Models (https://arxiv.org/abs/2602.20126)
- **What's New**: 이 논문은 Diffusion Language Models (DLMs)의 새로운 unmasking schedule을 제시하며, 이러한 일정이 대상 데이터 분포의 (알 수 없는) 의존 구조에끔 맞춰 최적화되도록 설계되었습니다. 기존의 결정론적 방식과는 달리, 제안된 방법은 반복마다 공개되는 토큰의 수를 무작위화하여, 추론(인퍼런스) 품질을 향상시킵니다. 또한, Kullback-Leibler (KL) 발산을 통해 샘플 수렴 속도를 보장하고, 상대적으로 낮은 복잡성을 가진 분포에서 샘플링을 가속화합니다.

- **Technical Details**: 논문에서 소개한 unmasking schedule은 특정한 매개변수 선택에 따라 동작하며, 이에 대한 수렴 보장은 K(반복 횟수), TC(총 상관관계), DTC(이중 총 상관관계)와 같은 데이터 분포의 지표를 통해 측정됩니다. 이러한 지표들은 데이터 내 재귀 의존성을 반영하며, 제안된 방법에서는 K가 L(토큰 시퀀스 길이)보다 작은 병렬 샘플링 환경에서 적용될 수 있습니다. 기존 방법들의 이론적 한계를 극복하며, 모든 토큰을 한 번에 드러내는 것과 한 번에 하나씩 드러내는 과정의 오류를 비교 분석합니다.

- **Performance Highlights**: DLM이 내재된 데이터 구조에 적응할 수 있는 가능성을 보여주며, 무작위화된 unmasking 크기가 추론 일정 설계에서 갖는 이점을 조명합니다. 새로운 접근 방식은 샘플링 효율성을 개선하여, 특히 긴 시퀀스 생성 시에 발생하는 오류를 줄이며 성능을 향상시킵니다. 이러한 연구 결과는 DLM의 샘플링 전략을 개선하는 중요한 방향성을 제안합니다.



### Reliable Abstention under Adversarial Injections: Tight Lower Bounds and New Upper Bounds (https://arxiv.org/abs/2602.20111)
- **What's New**: 이 논문에서는 Goel et al. (2024a)가 제안한 적대적 주입 모델(adversarial injection model)에서의 온라인 학습을 연구합니다. 이 모델은 주로 i.i.d(독립 동분포)로부터 라벨이 매겨진 예제가 추출되는 반면, 적대적으로 선택된 예제가 간헐적으로 포함되는 상황을 다룹니다. 논문은 확실한 타겟 개념(target concept)에 기반하여 라벨이 항상 일관성을 유지하는 클린 라벨(setting) 설정에서의 학습자(learner)의 오류 측정을 검토합니다.

- **Technical Details**: 이 연구에서는 결합 오류(combined error)를 최소화하기 위한 잠재 기반 프레임워크(potential-based framework)를 도입합니다. 이를 통해 '강력한 증인(robust witnesses)'을 활용하여 예측을 인증할 수 있는 소규모 라벨 예제들의 집합을 정의합니다. 두 가지 조합론적 차원을 사용하여 이 프레임워크를 구현하며, 이를 통해 포함된 클래스에 따라 결합 오류의 상한을 도출합니다.

- **Performance Highlights**: 반응이 없는 데이터에서의 오류를 제어하면서, VC 차원 1의 경우에 대해 예측된 기대 결합 오류가 Ω(√T) 이하임을 증명하여 정보 범위 사이의 실제 격차를 보여줍니다. 또한, 𝓗(halfspaces) in ℝ²의 경우에는 증명된 크기 차원 3에 대해 최초의 배급 비 의존상한(bound)인 O~(T^{2/3})에 도달했습니다. 이로 인해 클린 라벨 공격에 대한 내구성이 없다는 전제하에 민감도 높고 분포 불가지론(distribution-agnostic) 측면에서 별개의 성과를 이뤘습니다.
...
- **What's New**: 본 연구에서는 신경망 훈련을 선형 시스템으로 대체하는 커널 방법을 개발하여 생성 모델링을 위한 새로운 방식을 제안합니다. 생성적 확산 과정의 드리프트(드리프트)는 데이터로부터 계산 가능한 시스템에서 해결되는 벡터로 근사됩니다. 이 접근 방식은 다양한 기능 맵을 통합할 수 있어 훈련이 필요 없는 모델 조합을 가능하게 합니다.

- **Technical Details**: 연구에서는 차원 d에 독립적인 P개의 특징들을 사용하여 드리프트와 스코어 추정을 정량적으로 조정합니다. 최적의 확산 계수 D_t^*는 Girsanov 정리를 통해 최소화된 경로 KL 경계를 기반으로 하여 주어집니다. 제안된 커널 접근법은 불확실성을 다루기 위해서 적절한 적분기를 포함하고 있으며, 다양한 기능 맵과 결합할 수 있도록 설계되었습니다.

- **Performance Highlights**: 이 모델은 금융 시계열, 난류, 이미지 생성 등의 다양한 응용 분야에서 성능을 입증하였습니다. 훈련 없이 다른 아키텍처 및 훈련 단계의 모델을 조합하여 효율적인 데이터 생성이 가능하다는 점에서 실용성이 높습니다. 본 연구의 결과는 Moment-Guided Diffusion과 상호 보완적이며, 더 나아가서 생성 모델의 오류를 최소화하는 데 기여합니다.



### A Theory of How Pretraining Shapes Inductive Bias in Fine-Tuning (https://arxiv.org/abs/2602.20062)
- **What's New**: 본 논문은 사각 선형 네트워크를 사용하여 사전 훈련(pretraining)과 미세 조정(fine-tuning)의 파이프라인을 분석하고, 초기화(initialization) 선택이 기능 재사용(feature reuse) 및 개선(refinement)에 미치는 영향을 이론적으로 설명합니다. 기존의 연구들은 기능 학습(feature learning)에 대한 이해가 부족했으나, 이 연구는 초기화의 스케일이 미세 조정에서 기능 학습의 접속성을 어떻게 결정하는지를 분석합니다. 이를 통해 미세 조정 과정에서 최적의 초기화가 필요하다는 점을 강조하며, 여러 층에서 초기화의 상대적인 스케일이 중요한 역할을 한다고 주장합니다.

- **Technical Details**: 논문에서는 초기화 매개변수 및 작업 통계(task statistics)에 따라 일반화 오류(generalization error)가 어떻게 달라지는지에 대한 정확한 표현식을 도출합니다. 저자들은 기능 학습과 재사용을 지원하는 4가지 미세 조정(regime)을 정의하며, 특히 초기화 스케일이 작은 경우는 기능 재사용 및 개선을 효과적으로 가능하게 한다고 설명합니다. 이론적인 결과는 CIFAR-100 데이터셋에서 학습된 비선형 네트워크에도 적용될 수 있음을 경험적으로 입증하였습니다.

- **Performance Highlights**: 연구 결과는 다양한 초기화 선택이 미세 조정 일반화에 중요한 영향을 미친다는 것을 보여줍니다. 새로운 타겟 작업에 대해 사전 훈련된 기능을 재사용하는 것이 결국 일반화를 향상시키는 상황에서, 여러 미세 조정 레짐에 따라 초기화의 상대적인 스케일이 질적으로 다른 결과를 낳는다는 사실이 강조되었습니다. 저자들은 초기화의 상대적 스케일에 따라 기능 학습이 지속적으로 이루어질 수 있는지를 명확히 규명하였습니다.
...
- **What's New**: 이 논문에서는 Vision Transformers(ViTs)의 새로운 텐서 기반 프레임워크를 제안합니다. 논문의 핵심은 Tensor Cosine Product(c-product)를 활용하여 효율적인 attention 메커니즘을 제공하고 이미지 데이터의 구조적 특징을 표현하는 것입니다. 이 방법은 고전적인 ViT 모델의 한계를 극복하고 계산 및 메모리 비용을 줄일 수 있습니다.

- **Technical Details**: 제안된 접근 방식은 텐서값 표현(tensor-valued representations) 위에서 직접 작업하고 코사인 변환 기반 텐서 곱을 취하여 공간 구조를 보존합니다. 새로운 Cproduct 기반 Vision Transformer 아키텍처(TCP-ViT)를 설계하여 모든 선형 구성 요소에서 일관된 1/C 매개변수 감소를 달성합니다. 이러한 텐서 코사인 곱은 멀티선형 변환 영역에서 이미지 패치 간의 상호작용을 계산하는 구조화된 attention 메커니즘을 가능하게 합니다.

- **Performance Highlights**: 수치 실험을 통해 제안된 방법이 표준 분류(classification) 및 세분화(segmentation) 벤치마크에서 경쟁력 있는 정확도를 유지하면서도 균일한 1/C 매개변수 감소를 달성함을 입증하였습니다. 또한, 이 방법은 자원 제한 환경에서도 더 나은 확장성과 실용성을 제공합니다. 실험을 통해 제안된 접근 방식이 효율적이고 경쟁력 있는 결과를 생성함을 보여줍니다.



### Discrete Diffusion Models Exploit Asymmetry to Solve Lookahead Planning Tasks (https://arxiv.org/abs/2602.19980)
- **What's New**: 이번 연구에서는 Autoregressive (AR) 모델과 Non-Autoregressive (NAR) 모델의 차이점을 분석하여 향후 계획적(Task Planning) 작업을 위한 더 나은 방법론을 제시하고 있습니다. 특히, Star-Path 문제를 활용하여 모델의 의사 결정 과정을 그래프 탐색(Graph Traversal) 문제로 변환하여 다각적으로 접근합니다. 이러한 접근 방식은 각 모델이 어떻게 문제를 해결하는지에 대한 통찰력을 제공합니다.

- **Technical Details**: STAR-Path 과제는 그래프 설명, 소스-타겟 정점 쌍, 그리고 이를 연결하는 경로의 세 가지 요소로 구성됩니다. 각 STAR-Graph는 중심 정점에서 여러 개의 팔로 연결되는 형태로 설계되어 있으며, 해당 구조를 통해 AR 모델과 NAR 모델의 학습 메커니즘 차이를 비교할 수 있습니다. NAR 모델은 과거의 복잡한 경로를 회피하여 간단한 역방향 디코딩 전략을 통해 문제를 해결하는 방식으로 진화합니다.

- **Performance Highlights**: 모델의 성능 측정 결과, AR 및 NAR 모델 모두 충분한 학습 예제를 제공받았을 때 Star-Path 작업에서 완벽한 정확도를 달성할 수 있는 것으로 나타났습니다. 그러나 NAR 모델은 AR 모델에 비해 학습 예제 요구량이 기하급수적으로 적고, 더 적은 구조로도 성능을 발휘할 수 있어 훈련 복잡성을 줄이는 데 유리합니다. 이러한 결과는 향후 더 복잡한 문제 해결을 위한 시스템 설계에 기여할 것으로 보입니다.



### Unlearning Noise in PINNs: A Selective Pruning Framework for PDE Inverse Problems (https://arxiv.org/abs/2602.19967)
- **What's New**: 이번 논문에서는 P-PINN이라는 선택적 프루닝 프레임워크를 제안하여 오염된 데이터의 영향을 제거하는 데 중점을 두었습니다. 기존의 PINN이 관측 노이즈에 취약한 문제를 해결하기 위해, P-PINN은 신뢰할 수 있는 데이터와 오염된 데이터를 구분하는 새로운 데이터 지표를 도입합니다. 이 연구는 물리적 제약을 고려한 최적화 문제를 해결하고, 기존 방식보다 발달된 접근 방식을 제공합니다.

- **Technical Details**: P-PINN은 조인트 잔여-데이터 충실도 지표를 기반으로 데이터를 신뢰성 있는 집합과 오염된 집합으로 분할합니다. 그런 다음, 방향성 활성화 차이를 기반으로 하는 신경망 중요도 점수를 도입하여 오염된 데이터에 의해 주로 영향받는 뉴런을 파악합니다. 이 프레임워크는 단계적으로 노이즈에 민감한 뉴런을 제거하고, 남은 데이터에 대해서는 파인 튜닝을 수행하여 안정성을 높입니다.

- **Performance Highlights**: P-PINN은 다양한 PDE 벤치마크에서 기존의 PINN과 비교해 최대 96.6%의 상대 오류 감소를 달성하며, 노이즈 환경에서도 안정성과 정확성이 크게 향상됩니다. 이 결과는 노이즈 오염이 있는 상태에서 물리적 학습의 신뢰성을 높이는 데 기여할 수 있는 활성화 수준의 프루닝 메커니즘이 가능함을 시사합니다.
...
- **What's New**: 이번 논문에서는 강화 학습에서 관찰에서 불필요한 정보를 제거하는 방법이 일반적인데, 이러한 방법들이 보이지 않는 작업에 대한 일반화 능력을 간과하는 문제를 다룹니다. 저자들은 attention 기반 정책 네트워크 내에서 attention weights와 통합된 학습된 마스킹 함수(mask)를 사용하여 새로운 상태에 더 잘 일반화되는 정보 제거 방법을 제안합니다. 이 방법은 Procgen 벤치마크에서 일반적인 PPO 및 기존 마스킹 접근법과 비교하여 정책의 일반화를 상당히 개선함을 보여줍니다.

- **Technical Details**: 정보 제거를 통해 정책 일반화를 평가하기 위한 MDP 집합을 정의하고, 각 작업(m_{c})은 상태, 행동, 전이 확률, 보상 함수 및 시작 상태 분포 등을 포함합니다. 본 연구에서는 Procgen 벤치마크를 사용하여 200개의 고유한 레벨에서 훈련한 정책을 나머지 테스트 MDP에 대해 평가합니다. 방금 제안한 마스킹 관계적 정책 네트워크는 상태 변수 간 불필요한 관계를 무시하게 학습함으로써, 보이지 않는 상태에 대한 일반화를 더욱 향상시킵니다.

- **Performance Highlights**: 이번 연구는 sparse dependent policies를 활용하여 다양한 환경에서 정책의 일반화 성능을 크게 향상시킬 수 있음을 입증합니다. 실험 결과, 제안된 sparse 관계적 정책을 사용하였을 때 Procgen 벤치마크 작업에서 상당한 일반화 개선이 있었음을 확인하였습니다. 이는 특히 인식 가능한 상태 변수의 중요성을 시각화하는 방법 또한 제시하였으며, 학습된 정책의 설명 가능성을 높이는 데 기여합니다.
...
- **What's New**: 이 논문은 Sparse Mixture-of-Experts (SMoE) 아키텍처의 효율성을 대폭 향상하기 위한 새로운 방법인 Replicate-and-Quantize (R&Q)를 제안합니다. 현재 SMoE 모델이 직면하고 있는 큰 문제인 전문가 간의 부하 불균형을 해결하고자, 이 방법은 훈련 없이도 동적으로 부하를 재조정할 수 있는 기반을 제공합니다. 특히, 이 연구는 전문가 선택 빈도와 중요성 간의 불일치에 관한 통찰력을 제공합니다.

- **Technical Details**: R&Q는 SMoE 아키텍처 내에서 전문가의 병렬 처리 용량을 증가시키기 위해 크게 활성화된 전문가들을 복제하고, 중요도가 낮은 전문가들을 양자화하여 메모리 예산을 유지하는 방법을 사용합니다. 연구에서 제안하는 Load-Imbalance Score (LIS)는 전문가가 올바르게 활용되고 있는지를 평가하기 위해 두 가지 기준을 비교합니다. 실험을 통해 이 접근 방식이 부하 불균형을 1.4배 줄이면서 유의미한 정확성을 유지함을 보여줍니다.

- **Performance Highlights**: 본 연구의 실험 결과, R&Q 방법을 적용한 SMoE 모델은 부하 불균형이 1.4배 감소하고 정확성은 +/-0.6% 이내로 유지되었습니다. 이로 인해 추론의 예측 가능성과 효율성이 높아지는 효과가 나타났습니다. 전반적으로 이 연구는 최신 LLM 배포에 있어 추론 효율성을 개선할 수 있는 잠재력을 보여줍니다.
...
- **What's New**: 이 논문은 오프라인 강화 학습(offline reinforcement learning, RL) 분야에서의 새로운 접근 방식인 Uncertainty-Aware Rank-One MIMO Q Network 프레임워크를 제안합니다. 이는 기존 방법들이 가지고 있던 OOD(out-of-distribution) 데이터 이용의 보수성, 부정확한 데이터 특성화, 그리고 높은 계산 복잡도를 개선하기 위한 것입니다. 이 새로운 프레임워크는 OOD 데이터의 잠재력을 최대한 활용하여 정확성과 효율성을 높이는 것을 목표로 하고 있습니다.

- **Technical Details**: 논문은 Rank-One MIMO Q 네트워크 아키텍처를 통해 불확실성의 양적 측정을 활용하는 방법을 제시합니다. 이 네트워크는 여러 개의 입력을 처리하고 동시에 여러 출력을 생성할 수 있는 기능을 분산적으로 결합하여 단일 네트워크와 유사한 비용으로 앙상블 네트워크의 가능성을 제공합니다. 또한 이 방법은 Q 값의 하한 신뢰 구간(lower confidence bound, LCB)을 극대화하는 훈련 손실을 이용하여 OOD 데이터를 효과적으로 활용합니다.

- **Performance Highlights**: D4RL 벤치마크에서 포괄적인 실험이 수행되었으며, 제안된 프레임워크는 최첨단 성능(state-of-the-art performance)을 달성하면서도 계산적으로 효율적임을 보여줍니다. 이 연구는 오프라인 RL에 최적화된 효율적이고 안정적인 앙상블 기법의 중요성을 강조하며, 향후 연구를 위한 흥미로운 질문들을 제기합니다.
...
- **What's New**: 이 논문은 미세구조(microstructure) 진화 예측을 위한 딥러닝 기반의 새로운 프레임워크를 제안합니다. 전통적인 phase-field 모델은 높은 신뢰성을 제공하지만 복잡한 미분 방정식을 해결해야 하므로 계산 비용이 많이 드는 반면, 본 연구는 이를 해결하기 위해 고안되었습니다. 제안된 방법은 자기지도 학습(self-supervised learning)을 통해 훈련된 완전 합성곱(spatial-temporal convolutional) 모델을 사용하여 미세구조 과정을 시뮬레이션한 순차 이미지를 통해 고속 예측을 가능하게 합니다.

- **Technical Details**: 본 연구에서는 SimVPv2 아키텍처에 기반한 완전 합성곱(spatiotemporal) 학습 프레임워크를 사용합니다. 이 모델은 비순환(non-recurrent)이며 다단계 예측을 단일 전방 패스에서 효율적으로 수행할 수 있습니다. 네트워크는 grain growth 및 spinodal decomposition에 대한 phase-field 시뮬레이션 시퀀스를 사용하여 훈련되어 물리적 동역학을 효과적으로 학습하며, 미세구조의 단기 및 장기 특성을 포착할 수 있습니다.

- **Performance Highlights**: 실험 결과는 제안된 모델이 단기 지역 전이와 장기 통계적 특성을 정확하게 포착하면서도 높은 계산 효율성을 유지함을 보여줍니다. 전통적인 순환 신경망(recurrent neural networks)과 비교했을 때, 우리가 제안한 완전 합성곱 아키텍처는 더 긴 예측 수평에 대해 개선된 확장성을 나타내며 다양한 구성에 적용 가능합니다. 이러한 결과는 PDE 기반 시뮬레이션의 효과적이고 확장 가능한 보완책으로서 완전 합성곱(spatiotemporal) 학습의 잠재력을 뒷받침합니다.



### De novo molecular structure elucidation from mass spectra via flow matching (https://arxiv.org/abs/2602.19912)
Comments:
          13-page preprint, 4 figures, 1 table

- **What's New**: 이번 연구에서는 MSFlow라는 새로운 머신러닝 모델을 개발하여 질량 분석기(mass spectrometry)로부터의 스펙트럼을 분자 구조(molecular structure)로 효과적으로 변환할 수 있는 방법을 제시합니다. 두 단계의 인코더-디코더 구조를 갖춘 이 모델은 작은 분자의 구조 명확화(task of structure elucidation)에서 최첨단 성능을 달성하였습니다. 이는 생물학적 통찰, 새로운 대사물질(metabolite) 발견, 그리고 여러 화학 연구 분야의 발전에 기여할 것으로 기대됩니다.

- **Technical Details**: MSFlow는 두 단계로 구성된 구조로, 첫 번째 단계에서 질량 스펙트럼을 연속적인 화학적으로 유용한 임베딩 공간으로 인코드(encode)합니다. 이를 위해, 공식 제약(transformer model)을 받는 모델을 사용하며, 두 번째 단계에서는 디코더 모델이 수렵한 임베딩을 바탕으로 분자 형태를 재구성합니다. 이 과정에서 분자 서술자(molecular descriptors)의 정보 보존이 중요한 역할을 하며, 이를 통해 MSFlow가 미래 연구에 활용될 수 있는 근거를 마련하였습니다.

- **Performance Highlights**: MSFlow는 질량 스펙트럼을 분자 표현으로 정확히 변환하는 데 있어 최대 45%의 성과를 보이며, 이는 기존 최첨단 기술에 비해 최대 14배 향상된 결과입니다. 이러한 향상된 성능은 합성 데이터와 실제 데이터 간의 커다란 격차를 줄일 뿐 아니라, 알려지지 않은 분자 구조의 주석(annotation)을 가능하게 하여 대사물질 및 생화학적 경로 해석에 기여할 것입니다. 또한, MSFlow는 GitHub에서 비상업적 사용자를 위해 공개되어 연구 생태계를 지원할 예정입니다.
...
- **What's New**: 이번 논문에서는 랜덤 방향 확률 근사(random direction stochastic approximation; RDSA)를 활용한 일반화된 해시안 추정기(generalized Hessian estimators)의 가족을 제시합니다. 이러한 추정기는 시끄러운 함수 측정값만을 활용하여 최적화 문제를 다룹니다. 특히, 더 많은 함수 측정을 사용하는 경우 추정기의 낮은 차수 편향이 나타나는 것을 보여주며, 비대칭 비편향( asymptotic unbiasedness)의 특성을 입증합니다.

- **Technical Details**: 사용된 알고리즘은 추정값을 개선하기 위해 여러 함수 측정을 통한 해시안 추정을 포함합니다. 무작위 방향 확률 근사는 단지 두 번의 함수 측정을 통해 높은 정확도의 경향을 유지할 수 있습니다. 이 논문에서는 RDSA 기반의 적응 뉴턴(stochastic Newton) 방법을 분석하여, 함수 측정 횟수를 소폭 증가시킴으로써 추정의 편향을 줄이는 방법론을 제안합니다.

- **Performance Highlights**: 제안된 해시안 추정기는 이론적인 분석과 수치적 실험을 통해 입증된 성능을 보여줍니다. 일반화된 해시안 추정기와 일반화된 기울기 추정기를 통합한 확률 뉴턴 방법이 1차 정상점(first-order stationary points)으로ASYMP의 수렴을 보이는 것을 밝혀냈습니다. 또한, кубический регуляризованный 뉴턴 방법의 변형을 통해, saddle 포인트에서의 탈출 가능성을 논의하며, 수렴의 비대칭적 경계를 제시합니다.



### I Dropped a Neural N (https://arxiv.org/abs/2602.19845)
- **What's New**: 이번 논문은 심층 잔차 네트워크(Deep Residual Network)에 대해 흥미로운 퍼즐을 소개합니다. 연구자들은 96개의 레이어를 무작위로 섞은 후 이를 다시 정렬하는 문제에 도전했습니다. 주어진 레이어와 훈련 데이터셋을 사용하여 원래 구조를 복구할 수 있는지를 탐구합니다. 이 문제는 민감한 매칭(pairing)과 정렬(ordering)으로 세분화되어 있다.

- **Technical Details**: 문제는 48개의 블록으로 이루어진 잔차 네트워크에서 입력과 출력 프로젝션을 매칭하는 것과 재조립된 블록의 순서를 정하는 것으로 나뉩니다. 전체 검색 공간은 48!의 제곱으로 약 10^122에 달해 탐색이 불가능합니다. 연구진은 동적 동형성(dynamic isometry)과 같은 안정 조건을 이용하여 올바르게 쌍을 이룬 레이어가 부정 대각 구조를 가지게끔 할 수 있음을 보여주었습니다.

- **Performance Highlights**: 최종적으로, 제안된 방법으로 정확한 블록 쌍과 올바른 순서를 복구하는 데 성공했습니다. 이 바탕으로 쌍을 형성하는 과정에서 다루기 쉬운 대각 지배 비율을 사용합니다. 또한, 정렬 과정은 데이터가 없는 경우에도 효과적으로 수행되었습니다.
...
Comments:
          Paper was accepted at the 34th European Symposium on Artificial Neural Networks, Computational Intelligence and Machine Learning --- ESANN 2026

- **What's New**: 이 논문에서는 개념 변화(concept drift) 문제에 대한 새로운 접근법을 제안합니다. 기존의 방법들은 고차원, 낮은 시그널 환경에서 성능 저하가 발생하는데 반해, 저자는 conformal prediction을 통해 보다 효과적인 drift localization 방법을 제시합니다. 이는 이미지 데이터셋을 활용하여 성능을 검증하였으며, 기존 방법들의 한계를 강화된 통계적 검정을 통해 해결하고자 합니다.

- **Technical Details**: 기존 분석 방법들은 대개 지역적 통계 테스트(local statistical testing) 기반으로 진행되어, 고차원 데이터의 경우 저조한 성능을 보입니다. 이 논문에서는 conformal prediction을 기반으로 하여, 전 세계적인 테스트(global testing scheme)를 추구합니다. 특히, 조건부 엔트로피(conditional entropy)를 최대화하여 드리프트 여부를 식별하는 접근법을 사용하며, 이 과정에서 구성이 다른 점(time point)을 배제할 수 있는 확률적 기법을 평가합니다.

- **Performance Highlights**: 저자들은 제안된 방법이 다양한 이미지 데이터셋에서 state-of-the-art 접근법보다 우수한 성과를 거두었음을 입증하였습니다. 이 방법은 드리프트가 발생한 샘플의 정확한 찾기가 가능하여, 다양한 머신러닝 환경에서의 적용 가능성을 강조합니다. 이 연구는 데이터의 변화에 적응하는 머신러닝 시스템의 효율성을 높일 수 있는 기초 자료를 제공합니다.



### Stop Preaching and Start Practising Data Frugality for Responsible Development of AI (https://arxiv.org/abs/2602.19789)
- **What's New**: 이번 논문은 머신러닝(ML) 커뮤니티가 책임감 있는 인공지능(AI) 개발을 위해 데이터 절약(data frugality)을 실천해야 한다고 주장합니다. 기존에는 데이터의 양이 증가하는 것이 성과와 연관되어 있었지만, 이는 환경에 미치는 영향을 고려하지 않은 전통적인 관점입니다. 데이터 절약 접근법을 통해 학습 효율을 극대화하고, 훈련 에너지를 줄이며 정확성을 유지할 수 있다는 실증적 증거가 제시됩니다.

- **Technical Details**: 데이터 절약이란 불필요하거나 중복된 데이터를 제거해 각 샘플로부터 최대한의 학습 효율을 달성하는 것을 의미합니다. 코어셋(selection) 기술을 이용하면, 작은 크기지만 대표성을 가진 데이터셋을 만들어 시뮬레이션 훈련 시 에너지 소비를 크게 줄이며 정확성을 유지할 수 있습니다. 논문에서는 데이터의 환경적 비용을 명확히 계산하고, 데이터 라이프사이클 전반에 걸쳐 영향을 미치는 다양한 요인들을 분석합니다.

- **Performance Highlights**: 저자들은 코어셋 방법을 통해 수치적으로 훈련 에너지를 줄이는 데 성공하였고, 데이터셋의 편향도를 줄이는 효과도 입증했습니다. 이와 함께 데이터 절약 방식을 채택하는 것이 단순히 환경적 이점뿐만 아니라, 접근성을 높이고 재현성을 강화하는 데에도 기여한다는 점을 강조합니다. 최종적으로, 연구 결과는 ML 커뮤니티가 데이터 절약을 실천하여 심층 학습의 책임 있는 발전을 이룰 수 있도록 여러 가지 실행 가능 추천 사항도 제시합니다.
...
- **What's New**: 이 논문은 기존 meta-learning 기법의 한계를 극복하기 위해, 사전 조건화를 통해 인과적 요소를 고려한 Bayesian meta-learning 방법을 제안합니다. 이를 통해 다양한 데이터 생성 메커니즘을 갖는 새로운 задачи에 성공적으로 적응할 수 있는 능력을 향상시킵니다. 특히, 이 접근법은 소스 및 타겟 작업 간의 인과적 유사도를 평가하여 부정적인 전달(negative transfer)을 줄이는 것을 목표로 합니다.

- **Technical Details**: 제안된 방법은 각 작업을 인과적 작업 임베딩 공간(latent causal task embedding space)으로 매핑하여, 기계적 유사성에 기반한 전이(transfer)를 가능하게 합니다. 맥락에서는 전문가의 유사도 판단을 사용하여 새로운 타겟 작업의 임베딩을 추론하고, 이를 통해 작업에 적합한 사전(task-adaptive prior)을 생성하여 부정적인 전달을 완화합니다. 이러한 기법은 구체적인 의료 예측 시나리오에서도 적용될 수 있습니다.

- **Performance Highlights**: 이 연구에서 제안된 방법은 고전적인 meta-learning 기법과 비교했을 때, 부정적 이전(negative transfer)을 줄이고, 데이터 분포의 변화(distribution shift)에 대한 적응력을 개선하는 실험 결과를 보여줍니다. 종합된 실험은 인과 임베딩이 실제 임상 메커니즘과 일치함을 확인하며, 의료 데이터를 활용한 대규모 실험에서도 그 효과성을 입증하고 있습니다.



### Unsupervised Anomaly Detection in NSL-KDD Using $β$-VAE: A Latent Space and Reconstruction Error Approach (https://arxiv.org/abs/2602.19785)
- **What's New**: 이 논문은 운영 기술(Operational Technology, OT)과 정보 기술(Information Technology, IT)의 통합이 가속화됨에 따라 침입 탐지 시스템(IDS)의 필요성이 증가하고 있음을 강조합니다. 본 연구에서는 NSL-KDD 데이터셋을 활용하여 네트워크 트래픽의 이상 탐지(anomaly detection)를 위한 비지도(unsupervised) 접근 방식을 탐구합니다.

- **Technical Details**: 연구에서는 두 가지 방법을 제시합니다. 첫 번째는 테스트 샘플과 훈련 데이터의 투사(projection) 간의 거리를 측정하여 잠재 공간(latent space)의 구조를 활용하는 것이고, 두 번째는 전통적인 이상 탐지 메트릭으로 복원 오차(reconstruction error)를 이용하는 것입니다. 이러한 방법들을 비교함으로써 비지도 환경에서의 장단점을 더 명확히 파악합니다.

- **Performance Highlights**: 실험 결과에 따르면, 잠재 공간을 활용한 방법이 분류 작업에서 효과적임을 보여줍니다. 이 연구 결과는 비지도 학습을 통한 네트워크 이상 탐지의 가능성과 잠재 공간 활용의 중요성을 강조합니다.
...
- **What's New**: 이 논문은 Mendelian Randomization (MR) 연구에서 환경 간 불변성(cross-environment invariance)을 활용하여 유전자 도구(instrument)의 잠재적 외생 요인을 회복하는 새로운 표현 학습 프레임워크를 제안하고 있습니다. 기존의 MR 방식은 관찰된 혼란 변수로 인해 핵심 가정이 위반되는 경우가 많았지만, 본 연구에서는 다중 환경 데이터의 활용을 통해 이러한 문제를 해결하고자 합니다.

- **Technical Details**: 논문에서는 유전자 변수가 인구 구조 또는 정열적 교배에 의해 혼란을 겪는 문제를 다루며, 이는 MR의 교환 가능성 가정(exchangeability assumption)을 위반합니다. 저자들은 여러 환경에서 수집된 데이터를 이용하여, 환경 변화에 불변한 혼란된 유전자 도구의 구성 요소를 식별하는 방법을 제시하였습니다. 이와 같은 환경 간 분리 가능성을 통해 유효한 도구 변수의 회복을 이론적으로 보장하고 있습니다.

- **Performance Highlights**: 저자들은 아울러 다양한 실험을 통해 제안된 방법이 환경적 혼란 하에서의 바이어스(bias) 교정 효과를 효과적으로 드러냅니다. 구체적으로, 시뮬레이션과 반합성 실험을 통해 이 프레임워크의 유효성을 입증하고, 다운스트림 인과 추정에서의 유효성과 효율성 효과를 분석하였습니다. 연구 결과는 MR 분야에서 환경적 혼란을 극복하는 데 기여할 것으로 기대됩니다.
...
- **What's New**: 이번 연구는 머신러닝에서 흔히 발생하는 알고리즘 언롤링(Algorithm Unrolling)의 비대칭적 행동 및 초기 도출 오류에 대한 원인을 설명합니다. 이는 일반적으로 미분을 통해 구한 자코비안(Jacobian)이 초기에는 실제 자코비안과 차이가 나는 '언롤링의 저주(Curse of Unrolling)' 현상으로 불립니다. 저자는 초기 도함수 계산을 잘라내는(truncation) 방식이 이러한 문제를 완화할 수 있음을 보여줍니다.

- **Technical Details**: 연구는 이중 최적화(bilevel optimization) 문제에 대한 고정점(fixed point) 해석을 기반으로 하며, 해당 문제는 실질적으로 다수의 반복적 알고리즘에서 발생하는 최적화 문제로 귀결됩니다. 이를 해결하기 위해서는 외부 목표의 기울기를(parameter gradient) 계산하고, 이후 적절한 조건 하에서 미분을 통해 자코비안을 계산합니다. 논문에서 제시된 이론적 결과는 대표적인 예시들을 통한 수치 실험으로 뒷받침됩니다.

- **Performance Highlights**: 저자는 미적분(evaluation) 과정에서 초기 반복을 잘라내는 전략이 메모리 요구사항을 줄이면서 언롤링의 저주를 완화할 수 있음을 입증했습니다. 또한, 바이레벨 최적화에서의 워밍 스타트(warm-start) 접근법이 이러한 자르기(truncation)의 암묵적 형태를 자연스럽게 유도함을 제안하며, 이를 통해 더욱 실용적인 해결책을 제공하고 있습니다.
...
- **What's New**: 이번 연구에서는 PaReGTA라는 LLM(대규모 언어 모델) 기반의 인코딩 프레임워크를 제안합니다. 이 프레임워크는 longitudinal EHR(전자 건강 기록) 이벤트를 방문 수준의 템플릿 텍스트로 변환하며, 가벼운 대비 미세 조정을 통해 도메인 적합화된 방문 임베딩을 학습합니다. 또한, 하이브리드 temporal pooling 방식을 사용해 최근성과 전반적으로 유의미한 방문을 캡처하는 고정 차원의 환자 표현으로 집계합니다.

- **Technical Details**: PaReGTA는 구조화된 EHR 이벤트를 템플릿 방문 수준 문장으로 변환하며, 이 과정에서 임베딩 모델을 대비 objective를 통해 미세 조정합니다. 그런 다음 방문 임베딩을 시간적으로 인식한 pooling 방식으로 환자 수준의 표현으로 집계합니다. 이 방식은 세트 및 계층적으로 구조화된 시퀀스를 학습하기 위한 기본 원칙에 뿌리를 두고 있으며, LLM 기반의 인코더에 맞춘 PaReGTA-RSS(Representation Shift Score)를 통해 해석 가능성을 높입니다.

- **Performance Highlights**: PaReGTA는 All of Us 연구 프로그램의 39,088명의 편두통 환자 데이터를 통해 희소 기준선에 비해 우수한 편두통 유형 분류 성능을 입증했습니다. 기존의 심층 시퀀스 모델은 우리 집단에서 불안정성을 보였던 반면, PaReGTA는 데이터가 제한된 상황에서도 안정적인 성능을 발휘했습니다. 이 모델은 미래의 EHR 특화 문장 임베딩 모델과 호환 가능하여 데이터 제한이 있는 코호트에서도 뛰어난 성능을 기대할 수 있습니다.
...
- **What's New**: 이 논문에서는 양자 커널 방법(Quantum Kernel Methods)의 노이즈 노출에 대한 이해를 심화시키기 위해 Spectral Phase Encoding (SPE)이라는 하이브리드 구조를 제안합니다. 이 혁신적인 접근법은 불연속 푸리에 변환(Discrete Fourier Transform) 전단과 대각선 위상 전이(phase-only embedding)를 결합하여 양자 맵의 기하학에 정렬된 임베딩을 수행합니다. 이를 통해 양자 데이터 처리 과정에서 발생하는 노이즈에 대한 내성을 평가하고, 기존의 양자 방법들 간의 성능 차이를 정량적으로 비교합니다.

- **Technical Details**: SPE는 양자 게이트의 대각선 구성과 불연속 푸리에 변환(DFT)을 결합하여 자동적으로 주파수 도메인 정보를 발휘하도록 설계되었습니다. 상대 위상 정보를 인코딩하는 대각선 양자 게이트의 효율성 덕분에, SPE는 입력 데이터의 구조적 성질을 녹여서 특성 공간 필터링을 수행할 수 있습니다. 실험적으로, 우리는 QK-DFT가 다른 양자 변형(QK-PCA, QK-RP) 및 고전적인 SVM 베이스라인과 어떻게 비교되는지 분석하여 성능 저하를 정량적으로 측정합니다.

- **Performance Highlights**: 결과적으로 DFT 기반의 전처리가 추가 노이즈가 증가함에 따라 가장 적은 성능 저하율을 보였습니다. 고전적인 방법들과 비교했을 때, QK-DFT는 선형 SVM과 유사한 저하율을 보이며, RBF SVM보다 더 안정적인 성능을 나타냈습니다. 하드웨어 실험에서도 SPE가 단계 간의 겹침 추정에서 실행 가능하고 수치적으로 안정적임을 확인하였습니다.



### Evaluating the Impact of Data Anonymization on Image Retrieva (https://arxiv.org/abs/2602.19641)
Comments:
          Submitted to IEEE Access

- **What's New**: 개인정보 보호 규정(GDPR)의 중요성이 증가함에 따라 시각적 데이터의 익명화(anonymization)가 여러 기관에서 점점 더 중요해지고 있습니다. 하지만 익명화가 시각적 특징에 의존하는 컴퓨터 비전 시스템의 성능에 부정적인 영향을 미칠 수 있다는 문제가 있습니다. 본 논문은 특정 사례인 DOKIQ 프로젝트를 통해 이러한 익명화의 영향을 시스템적으로 평가하고, 익명화 후 검색 결과가 익명화 전과 최대한 일치하도록 하는 평가 프레임워크를 제안합니다.

- **Technical Details**: DINOv2 모델을 기반으로 하여 두 개의 공개 데이터셋과 DOKIQ 내부 데이터셋을 이용해 익명화의 영향을 체계적으로 평가하였습니다. 본 연구에서는 세 가지 익명화 방법, 네 가지 익명화 정도, 그리고 네 가지 훈련 전략을 활용하여 실험을 수행하였습니다. 결과적으로 원본 데이터로 훈련된 모델이 익명화 후 가장 유사한 검색 결과를 생성하는 경향을 보였고, 이는 CBIR 시스템의 성능 저하를 이해하는 데 중요한 통찰을 제공합니다.

- **Performance Highlights**: 본 논문의 실험 결과, 익명화로 인해 발생하는 검색 편향이 나타났으며, 이는 원본 데이터로 훈련한 모델이 타 모델보다 성능이 우수함을 알 수 있게 해주었습니다. 또한, 익명화된 데이터로 훈련하는 것이 성능 저하를 완화할 수 있는지를 탐구한 결과, 성능 유지에 도움을 줄 수 있는 방법들을 제시하고 있습니다. 마지막으로, 제공된 실용적인 권고사항들은 미래의 개인정보 보호를 준수하는 CBIR 시스템 개발에 중요한 가이드라인을 제공합니다.
...
- **What's New**: 이 논문은 이산 확산 언어 모델(dLLMs)의 평가 절차에서 발생하는 문제를 다룹니다. 기존의 측정기준들이 오토회귀 모델(ARMs)과 다르게 작동하여, 두 가지 오류를 혼합하는 문제를 해결하기 위해 샘플러 중심 오라클 평가 프레임워크를 도입합니다. 이 접근법은 학습된 디노이저를 정확한 숨겨진 마르코프 모델(Hidden Markov Model) 후방 베이시안으로 대체하여 샘플러 유도 오류를 분리하고 비교 가능하도록 만듭니다.

- **Technical Details**: dLLMs는 여러 위치에서의 병렬 업데이트가 가능하여 효율적인 추론을 지원하는 모델 입니다. 이러한 모델은 파라미터화 방식이 다르지만, 공통적으로 두 단계의 생성 구조를 기반으로 하여 진행됩니다. 연구에서는 각 확산 단계에서 토큰을 마스킹하고 그 과정을 역으로 수행하는 방법을 설명하며, 샘플러와 디노이저 간의 역할을 명확히 구분합니다. 이 논문은 샘플러 유도 오류를 고립하여 평가할 수 있는 제어된 프레임워크를 소개합니다.

- **Performance Highlights**: 제안된 평가 프레임워크를 활용하여, 적은 단계를 가진 이산 확산 샘플러들이 배포적으로 정확하지 않음을 확인했습니다. 연구 결과에 따르면, 불리한 샘플링 환경에서도 표면 수준의 품질이 높은 결과를 얻을 수 있으며, 이는 전이 KL(다른 생성 간의 상대적 분포 차이)나 엔트로피 감소, n-그램 다양성이 저하된 경우에 해당합니다. Likelihood 기반 메트릭(NLL, GenPPL)은 편향 측정에서 개선될 수 있지만, 전이 수준의 오류에 대해서는 MAUVE가 무관한 가능성을 보입니다.
...
- **What's New**: 본 논문에서는 혼합 데이터 샘플링(MIDAS) 회귀를 위한 새로운 좌표 상승 변분 추론(Coordinate Ascent Variational Inference, CAVI) 알고리즘을 개발하였습니다. 이 모델은 임팩트 계수와 가중 함수 매개변수를 분리하면서 정규화 제약을 통해 이분형 구조를 생성합니다. 이러한 구조로 인해 전통적인 Hamiltonian Monte Carlo 샘플러가 신뢰성을 잃지만, CAVI는 조건부 결합성을 유지하여 장점을 발휘합니다.

- **Technical Details**: CAVI 알고리즘의 각 변분 업데이트는 회귀 계수에 대해 가우시안 분포, 오차 분산에 대해 역 감마(Inverse-Gamma) 분포를 통해 닫힌 형태의 해(solution)를 제공합니다. 이 알고리즘은 블록 간의 불확실성을 2차 모멘트를 통해 전파하며, 단순한 대입 방법과의 차별성을 갖습니다. 또한, 고급 블록 기브스 샘플러와의 비교에서 CAVI는 변화하는 매개변수 구조에도 불구하고 더 나은 성능을 발휘합니다.

- **Performance Highlights**: 21개의 데이터 생성 구성에서 Monte Carlo 연구를 수행한 결과, CAVI는 블록 기브스 샘플러의 기준과 거의 동일한 후방 평균을 생성하면서도 107배에서 1,772배까지의 속도를 향상시켰습니다. 가중 함수 매개변수의 커버리지는 모든 구성에서 92% 이상을 유지하며, 임팩트 계수의 신뢰 구간은 예측자 수 증가에 따라 하락하는 특성을 보였습니다. 마지막으로, CAVI는 매월(estimation) 추정 작업을 10밀리세컨드 이내에 완료하며, S&P 500 일일 수익률의 변동성을 예측하는 실험에서도 유사한 결과를 보여 주었습니다.



### ISO-Bench: Can Coding Agents Optimize Real-World Inference Workloads? (https://arxiv.org/abs/2602.19594)
- **What's New**: 이 논문에서는 ISO-Bench라는 새로운 벤치마크를 소개하여 코딩 에이전트의 실제 추론 최적화 작업에 대한 성능을 테스트합니다. ISO-Bench는 vLLM과 SGLang에서 가져온 54개의 작업을 포함하고, 에이전트가 최적화 패치를 생성해 전문가의 솔루션과 비교하도록 요구합니다. 기존 벤치마크가 실행 기반 메트릭에 중점을 두었다면, 이 연구는 하드 메트릭과 소프트 메트릭을 결합하여 코드 변경의 실제 의도를 포착하고자 합니다.

- **Technical Details**: ISO-Bench는 LLM 기반의 코딩 에이전트를 평가하기 위한 두 가지 주요 메트릭을 적용합니다. 하드 메트릭은 각 프로젝트의 벤치마킹 도구를 사용하여 실행 성능을 측정하고, 소프트 메트릭은 에이전트가 최적화 대상을 올바르게 식별했는지를 평가하는 데 LLM을 활용합니다. 이렇게 결합된 메트릭을 통해 에이전트의 진정한 최적화 능력과 우연한 개선을 구분할 수 있습니다.

- **Performance Highlights**: 에이전트를 평가한 결과, 모든 코드베이스에서 지배적인 에이전트는 없음을 발견했습니다. 많은 에이전트들이 올바른 병목현상을 식별하지만, 작동 가능한 솔루션을 실행하는 데 실패했습니다. 또한, 동일한 모델을 사용하는 에이전트들 간에도 성능 차이가 크게 나타나, 스캐폴딩(scaffolding)의 중요성이 강조되었습니다.
...
- **What's New**: 이번 연구에서는 Advantage-based Adversarial Transformer (AAT)라는 새로운 방법을 제안합니다. AAT는 기존 방법들이 해결하지 못했던 다양한 시점 간의 종속성을 효과적으로 포착하여 더욱 강력한 시간적 상관성을 지닌 적대적 예제를 생성합니다. 이를 통해 적대적 공격 성능을 개선하며, 다양한 실험을 통해 기존의 공격 기법을 초월하는 성능을 보여줍니다.

- **Technical Details**: AAT는 다중 스케일 인과적 자기 주의(multi-scale causal self-attention, MSCSA) 메커니즘을 활용하여, 이력적 정보를 역동적으로 캡처합니다. 또한, 가중치 대안 메커니즘(weighted advantage mechanism)을 도입하여 특정 상태에서의 퍼지션(purturbation)의 효과를 정량화하고, 높은 성능의 적대적 예제를 생성하도록 유도합니다. 이러한 두 가지 메커니즘은 공격의 효율성을 높이고, 누적 보상을 줄이는 방식으로 작동합니다.

- **Performance Highlights**: 상세한 실험 결과에 따르면, AAT는 Atari, DeepMind Control Suite 및 Google Football 과제에서 기존의 주류 적대적 공격 기준선보다 약 3% 더 나은 성능을 발휘합니다. 또한 AAT는 단일 단계에서 효과적인 공격을 생성함으로써 적대적 예제 생성의 효율성을 현저하게 향상시킵니다.
...
Comments:
          18 pages, 5 tables. Code and data available at this https URL

- **What's New**: Leap+Verify는 신경망 훈련의 속도를 높이기 위해 훈련을 세 가지 동적 레짐(chaotic, transition, stable)으로 분할하는 새로운 프레임워크를 소개합니다. 이 프레임워크는 과거의 weight trajectory를 기반으로 미래의 모델 weight를 예측하고, 이를 held-out 손실 기준에 대해 검증함으로써 예측의 정확성을 확보합니다. 이러한 'verify-then-accept' 메커니즘은 효율성을 극대화하며, 이전의 방법들과 달리 예측을 조건부로 수행하여 계산 자원을 절약합니다.

- **Technical Details**: Leap+Verify는 'speculative execution' 개념을 기반으로 하며, activation-space cosine similarity를 사용하여 동적 레짐을 식별합니다. 세 가지 레짐 각각에서 분석적 weight 예측기(모멘텀, 선형, 이차 보간법)가 훈련 단계 K만큼 미래의 모델 파라미터를 예측하고, 이를 held-out 손실 기준과 비교하여 수용 여부를 결정합니다. 이 과정에서 특정 레짐에서의 예측 가능성이 결정적으로 중요하다는 사실을 발견하였습니다.

- **Performance Highlights**: Leap+Verify는 GPT-2 124M 및 Qwen 2.5-1.5B 모델에서 훈련 동안 안정적인 레짐에서 24%에서 37%의 엄격한 수용률을 기록했습니다. 그러나 모멘텀 기반 예측은 모든 모델에서 실패하며, 실제 손실보다 최대 10,000배 높은 예측 손실을 생성합니다. 결과적으로, 이러한 발견은 모멘텀을 이용한 예측의 한계를 드러내고, 프레임워크의 유용성을 입증합니다.



### The Sample Complexity of Replicable Realizable PAC Learning (https://arxiv.org/abs/2602.19552)
- **What's New**: 이 논문에서는 복제 가능한 학습(PAC learning) 문제를 다룹니다. 특히 도전적인 학습 문제를 설계하고, 이 가설 클래스(Hypothesis class) 크기에 따라 $(	ext{log}|H|)^{3/2}$에 근접한 샘플 복잡도 하한(bound)을 제시합니다. 이 근거를 위해 특정 Cayley 그래프를 정의하고, 그래프의 인접 행렬(Adjacency matrix)의 스펙트럼 특성을 분석합니다.

- **Technical Details**: 복제 가능한 알고리즘은 Impagliazzo 외가 제안한 이론적 속성으로, 새로운 훈련 데이터에서도 알고리즘의 출력을 재현할 수 있는 가능성을 포착합니다. 구체적으로, 알고리즘 𝒜는 ρ-replicable 하며, 이는 동일한 기본 분포에서 샘플을 받으면 높은 확률로 동일한 분류기를 출력한다는 것을 의미합니다. 이 연구에서는 실현 가능한 설정에서의 PAC 학습의 샘플 복잡도를 분석합니다.

- **Performance Highlights**: 본 논문의 주요 성과는 유한 가설 클래스에 대한 복제 가능한 실현 가능한 PAC 학습의 하한을 제시한 것입니다. 특정 조건 하에서, 알고리즘 𝒜가 ρ-replicable PAC 학습자가 되기 위해 필요한 레이블이 있는 샘플의 수에 대한 하한을 정의합니다. 이 결과는 비복제 가능 설정에서 알려진 하한보다 더 강한 의존성을 보여줍니다.
...
- **What's New**: 이번 연구에서는 교통 안전에서의 충돌 분류 모델을 평가하기 위한 새로운 접근 방식을 제시합니다. 과거의 정확도, F1, AUC 메트릭스가 미치는 제약을 극복하기 위해 랜덤 행렬 이론(Random Matrix Theory, RMT)과 중ㅇ 타격 자기 정규화(Heavy-Tailed Self-Regularization, HTSR)를 기반으로 한 스펙트럼 진단 프레임워크를 도입합니다.

- **Technical Details**: 이 프레임워크는 다양한 머신러닝 모델 유형을 아우르며, BERT/ALBERT/Qwen2.5의 가중치 행렬(weight matrices), XGBoost/Random Forest의 아웃 오브 폴드 증가 행렬(out-of-fold increment matrices), 로지스틱 회귀의 경험적 헤세안(empirical Hessians), 결정 트리의 유도 친화도 행렬(induced affinity matrices), KNN의 그래프 라플라시안(Graph Laplacians)을 포함합니다. 연구는 이owa DOT의 두 개의 충돌 분류 과업에서 9개의 모델 패밀리를 평가합니다.

- **Performance Highlights**: 모델의 정규화 상태를 나타내는 지표인 파워-로우 지수(α)는 잘 정규화된 모델이 평균 2.87 (± 0.34) 범위 내의 값을 유지함을 보여주며, 과적합된 모델은 α < 2를 보입니다. 전문가 의견과의 강한 순위 상관관계(Spearman ρ = 0.89, p < 0.001) 또한 스펙트럼 품질이 전문가의 추론과 일치한 모델 동작을 포착함을 시사합니다. 논문에서는 α 기반의 조기 중지 기준과 스펙트럼 모델 선택 프로토콜을 제안하며, 이 두 가지를 F1 크로스 검증 기준선에 대해 유효성을 검증하였습니다.
...
- **What's New**: 본 논문에서는 데이터 혼합(data mixing) 문제를 다루며, 고전적인 이중 최적화(bilevel optimization) 접근 방식의 제한점을 분석합니다. 특히, 실용적인 그리드 접근 방식인 T=1이 최적의 혼합을 찾지 못할 수 있음을 보이고, 더 긴 전망(horizon)을 가진 파라미터 업데이트가 성능 향상에 효과적임을 증명합니다.

- **Technical Details**: 이 논문은 T의 유한한 갯수 내에서 데이터 혼합 문제에 대한 수렴 행동을 깊이 분석합니다. 고정된 파라미터 업데이트 예산 N을 가정하고, 각 도메인 손실이 강한 볼록성을 가질 때 최적의 T는 Θ(log N)와 Θ(√(N log N))으로 스케일 한다고 증명합니다. 이론적 결과는 개념 증명 실험으로 보완됩니다.

- **Performance Highlights**: 이론적 결과에 따라 T의 중간 값이 실제 성능에서 가장 좋은 결과를 야기함을 보여줍니다. 이를 통해 데이터 혼합법과 관련된 기존의 그리드 방식이 갖는 한계를 심도 깊게 실험적으로 입증했습니다. 이 연구는 데이터 혼합에서 그리드 전략이 체계적인 편향을 유발한다는 것을 명확히 하고, 더 나은 혼합 전략을 제안하는 기반을 마련합니다.
...
### PIS: A Physics-Informed System for Accurate State Partitioning of $Aβ_{42}$ Protein Trajectories (https://arxiv.org/abs/2602.19444)
- **What's New**: 이 논문은 알츠하이머 병의 병리적 메커니즘을 이해하기 위해 $\beta$-아밀로이드 ($A\beta_{42}$)의 상태 변화를 모델링하는 새로운 방법 PIS(Physics-Informed System)를 소개합니다. 기존의 딥러닝 모델들이 단백질의 미세한 상태 전이를 포착하는 데 어려움을 겪는 문제를 해결하기 위해 물리적 제약을 통합하는 방식을 채택했습니다. PIS는 생물학적 연구자들에게 해석 가능한 물리적 지표를 제공하는 강력한 분석 도구 세트를 선보이며, 이를 통해 고차원 결과 검증과 물리적 특성의 동적 모니터링이 가능합니다.

- **Technical Details**: PIS는 A​β42 단백질의 껍질 상태를 정확하게 분할하기 위해 사전 계산된 물리적 기준(Radius of Gyration, Rg 및 Solvent Accessible Surface Area, SASA)을 통합하여 성능을 향상시킵니다. 이 시스템은 11,751개의 TIP3P 물 분자를 포함한 A​β42 MD(분자 역학) 데이터세트를 사용하여 성능 평가를 실시했습니다. PIS는 노드와 엣지를 통해 단백질의 구조를 그래프로 모델링하고, 주의 메커니즘을 통해 중요한 상호작용 영역을 캡처하는 기능들을 포함하고 있습니다.

- **Performance Highlights**: PIS는 VAMP-2 및 VAMP-E 메트릭으로 평가할 때 우수한 성능을 보였으며, 외부 벤치마크 모델들과 비교하여 뛰어난 결과를 나타냈습니다. 이 시스템은 다양한 물리적 변수를 실시간으로 모니터링하고 직관적인 도구를 제공하여 사용자에게 단백질의 복잡한 접힘 경로를 이해하는 데 필수적인 정보를 제공합니다. 이를 통해 연구자들은 A​β42의 단백질 껍질 변화를 보다 정확하게 분석할 수 있게 됩니다.
...
Comments:
          12 pages, 1 figure, 4 tables, 1 algorithm; submitted to Designing DeFi workshop (this https URL)

- **What's New**: 이 논문은 분산 거래소에서 집중된 유동성 공급이 기본적인 Impulse Control 문제를 나타낸다고 주장합니다. 유동성 제공자(LP)는 가격 범위에 집중하여 수수료 수익을 극대화하는 것과 리밸런싱의 마찰 비용(가스 요금 및 슬리피지 포함)을 최소화하는 것 사이에서 비효율적인 거래를 경험합니다. 본 논문에서는 유동성 관리를 최적 제어 문제로 공식화하고 해당 Hamilton-Jacobi-Bellman 준 변분부등식을 도출했습니다.

- **Technical Details**: 제안된 RAmmStein은 Ornstein-Uhlenbeck 과정의 평균 회귀 속도(theta)를 포함한 여러 특성을 모델의 입력으로 활용하는 딥 강화 학습 방법입니다. RAmmStein은 상태 공간을 행동 영역과 비행동 영역으로 분리하는 방식으로 학습합니다. 또한 Coinbase의 6.8M 이상의 트레이드 데이터를 사용하여 평가하며, 이로 인해 실제 시장 상황에서의 유효성을 입증합니다.

- **Performance Highlights**: RAmmStein은 수동 및 공격적인 전략에 비해 0.72%의 우수한 순 ROI를 달성했습니다. 특히 이 에이전트는 리밸런싱 빈도를 67% 줄이면서 88%의 활성 시간을 유지했습니다. 본 연구 결과는 레짐 인식 된 게으름이 자본 효율성을 크게 개선함을 보여주며, 운영 비용으로 인해 손실될 수 있는 수익을 보존하는 데 기여합니다.
...
- **What's New**: 이 논문에서는 산업 자산(client)들 간의 상호 의존성을 효과적으로 다룰 수 있는 연합 학습(federated learning) 프레임워크를 제안합니다. 이는 개별 클라이언트가 자신만의 고유 데이터를 유지하면서도 서로 간의 제어(input) 변화가 출력(output)에 미치는 영향을 예측할 수 있도록 하는 방식입니다. 저자는 기존의 중앙 집중식 데이터 접근 방식의 한계를 극복하기 위해, 각 클라이언트가 복잡한 고차원 데이터를 저차원 잠재 상태(latent states)로 변환하고, 중앙 서버가 전체 상태 전이(state transition) 및 입력(input) 구조를 추정하는 구조를 제시합니다.

- **Technical Details**: 이 연구는 선형 상태 공간 시스템(linear state-space systems)의 원리를 기반으로 합니다. 각 클라이언트는 자율 회귀(autoregressive) 시간 시계열(dynamic time series) 동태를 분리하여 저차원 잠재 상태로 맵핑하며, 중앙 서버는 직접적인 원시 데이터(raw data) 접근 없이 클라이언트 간의 상관 관계를 추정합니다. 이러한 구조 덕분에 클라이언트는 자신의 제어가 변경되었을 때 출력이 어떻게 바뀔지를 예측할 수 있습니다. 또한, 제안된 알고리즘은 정보 보호를 위해 차별적 개인 정보 보호(differential privacy) 보장을 제공합니다.

- **Performance Highlights**: 실험 결과, 이 프레임워크는 복잡한 산업 제어 시스템에 대한 고도로 정확한 상반 사실(counterfactual reasoning) 추론을 가능하게 하며, 중앙 집중식 오라클(centralized oracle)에 대한 수렴(convergence)을 입증합니다. 이 방법은 스케일러빌리티(scalability) 또한 갖추고 있어, 실제 산업 데이터셋에 적용 가능성을 보여줍니다. 저자들은 기존의 다차원 센서 데이터 처리의 제약을 극복하고, 효율적으로 상호 의존성을 분석하는 방법을 제시함으로써 새로운 패러다임을 제시합니다.



### LEVDA: Latent Ensemble Variational Data Assimilation via Differentiable Dynamics (https://arxiv.org/abs/2602.19406)
- **What's New**: 이번 논문에서는 Latent Ensemble Variational Data Assimilation (LEVDA)라는 새로운 방법론을 제안합니다. LEVDA는 훈련된 differentiable neural dynamics surrogate의 저차원 latent 공간에서 작동하는 ensemble-space variational smoother로, 일반적으로 필요한 adjoint code나 auxiliary observation-to-latent encoders 없이 상태와 알 수 없는 매개변수를 동시에 통합합니다. 기존 방법들과 달리 LEVDA는 시간 및 공간에서 연속적인 전체 differentiable 성질을 활용하여 불규칙한 샘플링을 자연스럽게 처리합니다. 이로 인해 LEVDA는 하나의 안정적인 모델에 대한 가시적인 다차원 동적 일관성을 제공합니다.

- **Technical Details**: LEVDA는 ensemble variational data assimilation 기법을 통해 미리 학습된 동적 surrogate의 latent 공간에서 4DEnVar 스타일로 최적화를 수행합니다. 이를 통해 동적 모델을 활용하여 관측 데이터의 비적합성을 직접 계산하며, 간접적으로 관측값과 latent의 변환을 학습할 필요가 없습니다. LEVDA는 관측지의 불규칙성과 시계열 데이터의 변동성을 수용하며, 커다란 계산적 효율성을 발휘합니다. 이 방법은 기존의 전통적인 variational 방법들이 요구했던 반복적인 비선형 모델 실행을 극복하는 점에서 중요합니다.

- **Performance Highlights**: LEVDA는 세 가지 도전적인 지구 물리학 벤치마크에서 성능을 평가했으며, 극심한 관측 희소성 하에서도 최신 latent filtering 기준과 동등하거나 이를 초과하는 정확도를 보여주었습니다. 또한, 불확실성 정량화에 있어 믿을 만한 결과를 제공하여, 기존의 전체 상태 4DEnVar 방법과 비교해 상당한 계산 효율성을 달성했습니다. LEVDA는 통합 예측 정확성을 크게 향상시킴과 동시에 실용적인 이점을 제공합니다.
...
- **What's New**: 이 논문은 매트릭스 인수 분해 모델에서 학습된 임베딩의 코사인 유사성이 대각선 "게이지" 행렬 $D$에 의해 임의적으로 변경될 수 있다는 기존의 주장을 분석합니다. 저자들은 코사인 유사성을 사용하는 것이 신뢰할 수 없다는 결론은 오해라고 주장하며, 이는 훈련 목표가 각도 구조를 제한하지 않는 경우의 부작용이라고 설명합니다.

- **Technical Details**: 이 논문에서는 코사인 거리와 유클리드 거리 간의 정확한 관계를 형식을 갖추어 설명합니다. 특히, 임베딩이 단위 구면 $	ext{S}^{d-1}$에 제한되면 $D$-행렬의 모호성이 사라지고 코사인 거리는 유클리드 거리의 제곱의 절반으로 줄어듭니다. 저자들은 이론적인 증명을 통해 이와 같은 기하학적 정당성을 제공합니다.

- **Performance Highlights**: 코사인 거리와 유클리드 거리 간의 일관성 덕분에 두 거리 메트릭을 사용하는 k-NN(nearest neighbors) 방식에서 동일한 이웃 집합이 반환됩니다. 이 결과는 임베딩이 정규화된 경우 두 거리 메트릭이 동등하게 작용함을 시사합니다. 이 연구는 코사인 유사성을 사용하는 관행에 대한 중요한 통찰을 제공합니다.
...
- **What's New**: 이번 연구에서는 SpIking GrapH predicTive coding(SIGHT)를 새로운 프레임워크로 제안하여 그래프 OOD(Out-Of-Distribution) 일반화 문제 해결을 목표로 합니다. SIGHT는 스파이킹 신경망 동역학과 예측 코딩을 결합하여 불확실성을 추정하는 혁신적인 방법을 제공합니다. 이를 통해 예측 불확실성을 효과적으로 격리하고 해석 가능성을 향상시킵니다.

- **Technical Details**: SIGHT는 두 가지 주요 구성 요소로 이루어져 있습니다. 첫째, 예측 코딩 모듈은 추론 과정에 직접 통합되어 불확실성을 내부 오류 수정 경로에서 유도합니다. 둘째, 스파이킹 오류 전파 메커니즘은 각 추론 단계에서 모델이 갈등 정보를 조화시키기 위해 힘들어하는 지점을 명확하게 드러내는 해석 가능한 내부 신호를 제공합니다.

- **Performance Highlights**: 다양한 그래프 벤치마크 및 OOD 시나리오에서 SIGHT는 예측 정확성, 불확실성 추정 및 해석 가능성을 일관되게 향상시키며, GNN과 통합할 때 우수한 OOD 일반화 능력을 발휘합니다. 이러한 결과는 SIGHT가 실질적인 AI 응용 분야에서 신뢰성 있는 예측을 도출하는 데 기여할 수 있음을 보여줍니다.
...
- **What's New**: 이번 연구에서는 정책 지연이 있는 강화 학습(Reinforcement Learning, RL) 알고리즘을 다루며, 이를 수정할 필요 없도록 설계된 새로운 오프-정책 RL 알고리즘인 최적 이점 기반 정책 최적화(Optimal Advantage-based Policy Optimization, OAPL)를 제안합니다. OAPL은 오프-정책 데이터를 사용하여, 기존의 중요도 샘플링(Importance Sampling, IS) 방식과는 다른 접근 방식으로, 교육 정책이 추론 정책으로부터 너무 멀리 가지 않도록 KL 정규화(KL-regularization)를 활용합니다. 이 연구의 결과는 OAPL이 기존의 방법들보다 성능을 향상시킬 수 있음을 입증합니다.

- **Technical Details**: 오프-정책 RL 알고리즘인 OAPL은 훈련된 모델과 추론 엔진 간의 불일치를 KL-정규화된 RL 문제로 다루며, 이를 통해 훈련과 추론 간의 정책 갭을 줄입니다. OAPL은 가벼운 동기화 절차를 통해 훈련을 수행하고, 최대 400개의 그래디언트 업데이트까지 처리할 수 있음을 보여줍니다. 이는 기존 방법들보다 더욱 효과적인 훈련 경험을 제공합니다.

- **Performance Highlights**: OAPL 알고리즘은 경쟁 수학 벤치마크에서 중요도 샘플링을 사용하는 GRPO보다 우수한 성능을 보였으며, LiveCodeBench에서 공개된 코딩 모델인 DeepCoder와 유사한 성능을 발휘하면서도 훈련 과정에서 사용하는 생성 수량이 3배 적었습니다. 또한, OAPL을 통해 Pass@k 메트릭 하에서 테스트 시간 스케일링 성능이 크게 개선되었음을 보여주었습니다.
...
- **What's New**: 이번 연구에서는 H-GRAMA(Heterogeneous Graph Routing and Message Alignment)라는 혁신적인 프레임워크를 도입하여 그래프 신경망(GNN) 간의 모델 머징을 다루고 있습니다. 이 프레임워크는 매개변수 공간에서 작업 공간으로 머징을 이동시킴으로써 이질적인 아키텍처 간의 통합을 가능하게 합니다. 특히, 각 GNN 모델의 전문성을 유지하면서 정보 손실 없이 단일 네트워크로 운영할 수 있는 방법을 제시하고 있습니다. 이 접근법은 재훈련 없이도 다양한 GNN 아키텍처 간의 조합을 지원하여, inference 속도는 1.2배에서 1.9배 향상됩니다.

- **Technical Details**: 연구에서는 Universal Message Passing Mixture(UMPM)라는 공유 연산자 패밀리를 정의하여 다양한 GNN 레이어를 공통의 기능 언어로 표현합니다. 이를 통해 서로 다른 GNN 아키텍처 간의 처리 및 융합이 가능해집니다. 또한, 불일치 문제를 해결하기 위해 비선형 Centered Kernel Alignment(CKA)를 사용하여 각 레이어의 심층 비율을 정렬하고, Procrustes 맵을 통해 매개변수를 공유 좌표계로 이동시킵니다. 이 과정은 서로 다른 깊이와 너비를 가진 모델 간의 일치를 달성하기 위한 중요한 단계입니다.

- **Performance Highlights**: H-GRAMA는 이질적인 GNN 모델들을 조합하는 데 있어 성능을 유지하면서도 훈련 과정이 필요 없는 접근 방식을 제공합니다. 실험 결과, 기존의 전통적인 방법들보다 성능 저하 없이 안정적으로 통합이 이루어짐을 보여줍니다. 또한, 메시지 통계 조정 단계를 포함하여 결합된 모델의 메시지 분포를 안정화하며, 이는 향후 GNN 재사용 연구에서도 중요한 기초 자료가 될 것입니다.



### Partial Soft-Matching Distance for Neural Representational Comparison with Partial Unit Correspondenc (https://arxiv.org/abs/2602.19331)
- **What's New**: 이 논문은 neural representations의 비교를 위해 기존의 representational similarity metrics의 한계를 극복하는 새로운 접근법인 partial soft-matching distance를 제안합니다. 이 방법은 모든 신경 단위를 일치시킬 필요 없이 일부 신경 단위가 일치하지 않을 수 있도록 하여 더 견고하고 실용적인 대응을 제공합니다. 특히,상이 맞지 않는 신경 단위를 필터링함으로써 신뢰도 높은 분석을 가능하게합니다. 이는 뇌 영상(fMRI) 데이터와 심층 신경망(DNN)에 적용되어 성능 개선을 보여줍니다.

- **Technical Details**: 이 연구에서 소개된 partial soft-matching distance는 부분 최적 운송(partial optimal transport) 문제를 확장하여 다양한 신경 집단을 비교할 수 있도록 설계되었습니다. 이 방법은 각 신경 집단 각각에 대해 tuning curve를 생성하여 신경 단위를 정의하고, 이를 이용해 각 신경 집단 간의 최적 운송 비용을 계산합니다. 상대적으로 견고한 분석을 가능하게 하기 위해 일부 신경 단위가 일치하지 않아도 분석할 수 있는 장점을 제공합니다.

- **Performance Highlights**: 시뮬레이션 결과, partial soft-matching distance는 노이즈와 아웃라이어의 영향을 받는 상황에서도 올바른 일치를 유지하며 모델 식별의 정확성을 높였습니다. fMRI 데이터에서는 신뢰도가 낮은 복셀을 자동으로 제외하고, 동일한 뇌 영역 간의 정렬에서 기존의 소프트 매칭보다 높은 정밀도를 달성했습니다. 이 연구는 신경 representations의 비교를 위한 원칙적이고 실용적인 방법론을 제공하며, 특히 부분 대응하에서의 분석에 적합합니다.
...
- **What's New**: 이번 논문에서는 CTS-Bench라는 벤치마크 스위트를 소개하여, 그래프 학습 기법과 클럭 트리 합성(Clock Tree Synthesis, CTS) 관련 학습 목표 간의 트레이드오프를 체계적으로 평가합니다. CTS-Bench는 4,860개의 물리적 설계 솔루션을 제공하며, 원시 게이트 수준(netlist)과 클러스터된 그래프 표현을 제공합니다. 이를 통해 그래프 클러스터링이 CTS 모델링에 미치는 영향을 명확히 분석할 수 있으며, 학습 지원 기술 개발을 위한 기초를 마련합니다.

- **Technical Details**: CTS-Bench는 OpenLane, OpenROAD, TritonCTS와 Sky130 PDK를 활용하여 구축된 파이프라인으로, 다양한 메모리 제약을 두고 모델 성능 변화를 벤치마킹합니다. 이 과정에서 두 개의 그래프 표현(정밀한 원시 그래프(Original Graph)와 물리적 정보를 반영한 클러스터 그래프(Clustered Graph))을 생성하여 모델 정밀도와 하드웨어 효율 간의 트레이드오프를 평가합니다. 또한, 논문에서는 회로 설계와 클럭 트리 합성 간의 관계를 정량화하기 위한 새로운 평가 지표도 소개합니다.

- **Performance Highlights**: 그래프 클러스터링을 통해 GPU 메모리 사용량을 최대 17.2배 줄이고, 훈련 시간을 최대 3배 단축할 수 있지만, 클럭 분배 모델링에 필요한 구조적 정보를 제거하여 종종 부정적인 $R^2$ 점수를 초래합니다. CTS-Bench는 실제 물리적 설계 제약 하에서 GNN 아키텍처 및 가속기 성능을 벤치마킹하여 시스템 차원의 비용을 체계적으로 평가하는 기능을 제공합니다. 이러한 결과는 클러스터링 기술이 CTS 학습 목표에 미치는 영향을 명확히 설명하며, 향후 물리적 설계의 머신러닝 기반 접근 방식을 개선하는 데 기여할 것입니다.
...
### AdsorbFlow: energy-conditioned flow matching enables fast and realistic adsorbate placemen (https://arxiv.org/abs/2602.19289)
- **What's New**: 이 논문에서는 AdsorbFlow라는 결정론적 생성 모델을 도입하여 촉매 표면에서의 저에너지 흡착 기하학을 식별하는 문제를 해결합니다. 이는 기존의 조건부 노이즈 제거 확산(Conditional Denoising Diffusion) 방법보다 훨씬 적은 반복 단계로 더 빠르고 정확하게 흡착제를 배치할 수 있도록 합니다. AdsorbFlow는 에너지 조건 벡터 필드를 학습하고 기존 방법들과 비교해 20배 적은 생성 단계를 요구합니다.

- **Technical Details**: AdsorbFlow는 조건부 흐름 매칭(Conditional Flow Matching)을 통해 고정된 시공간에서의 흡착제의 이동과 회전을 모델링합니다. 에너지는 분류기 없는 가이드(conditioning) 방법을 사용하여 입력되며, ODE(Ordinary Differential Equation)를 통해 샘플링이 이루어집니다. 이 접근법은 Stochastic Generative Sampling 대신 Deterministic Transport를 사용하여 흡착제 위치를 결정합니다.

- **Performance Highlights**: AdsorbFlow는 OC20-Dense 데이터셋에서 EquiformerV2 백본을 사용하여 61.4% SR@10, 34.1% SR@1의 성공률을 달성했습니다. 이는 AdsorbDiff와 AdsorbML의 성과를 초과하며 모든 평가 수준에서 최고 성능을 보입니다. 50개의 분포 외 시스템에서도 58% SR@10을 유지하며, 머신 러닝 강제 필드(MLFF)와 DFT 간의 차이는 4% 포인트에 불과합니다.
...
- **What's New**: 본 논문은 물리 정보 신경망(Physics-Informed Neural Networks, PINNs)과 같은 신경망 아키텍처에서의 스펙트럼 바이어스(spectral bias) 현상을 체계적으로 조사한 결과를 제시합니다. 스펙트럼 바이어스란 저주파수(low-frequency) 성분을 학습하는 데 신경망이 고주파수(high-frequency) 성분보다 더 빠르게 반응하는 경향을 말합니다. 이 연구는 네트워크 아키텍처(Network Architecture), 활성 함수(Activation Functions), 손실 디자인(Loss Design), 최적화 전략(Optimization Strategy)이 스펙트럼 바이어스에 미치는 복합적인 역할을 강조합니다.

- **Technical Details**: 스펙트럼 바이어스의 양적 분석을 위해, 주파수 해결 오류 메트릭(frequency-resolved error metrics) 및 고차 통계적 모멘트(higher-order statistical moments)를 사용하여 일반화된 분석을 수행합니다. 이 과정에서 다양한 PDE(Partial Differential Equations) 문제들을 사용하여 스펙트럼 바이어스가 단순한 표현적이지 않고 본질적으로 동적인 것임을 보여줍니다. 특히, 2차 최적화 방법이 스펙트럼 학습 순서를 크게 변경함으로써 모든 PDE 유형에 대해 고주파수 모드를 더 빠르고 정확하게 복원할 수 있게 합니다.

- **Performance Highlights**: 실험을 통해, PINNs와 PIKAN(Physics-Informed KANs) 모두를 포함한 다양한 경우에 대해 최적화 방법과 네트워크 아키텍처가 스펙트럼 바이어스에 미치는 영향을 분석했습니다. 특히, DeepONet, FNO(로 Fourier Neural Operator)와 같은 신경 연산자(Neural Operators)에서 다양한 아키텍처와 손실 함수를 비교하여 고주파수 문제 해결에서의 성능을 확인했습니다. 이 결과들은 스펙트럼 바이어스 완화 전략들이 물리 기반 신경망 학습을 향상시킬 수 있음을 보여줍니다.
...
- **What's New**: 이 논문에서는 X-ANFIS라는 새로운 대안적 이원 목표 경량 최적화 기법을 제안합니다. 이는 설명 가능한 적응형 신경 퍼지 추론 시스템을 달성하기 위해 고안된 방법으로, 정확도와 설명가능성의 목표를 독립적으로 최적화합니다. Cauchy 멤버십 함수가 안정적인 훈련을 위해 사용되며, 경량 최적화가 성능 목표와 설명가능성 목표를 번갈아 가며 조정합니다.

- **Technical Details**: 기존 퍼지 시스템의 정확도-설명가능성 간의 균형 문제를 해결하기 위해 비선형성의 멤버십 함수와 함께 진화적 다목적 최적화(MOO) 기법이 사용되었지만, 계산 비용이 높습니다. X-ANFIS는 교대 경량 최적화를 통해 이러한 문제를 해결하며, 9개의 UCI 회귀 데이터셋에서 약 5,000회 실험을 통해 유효성을 검증했습니다. 이 시스템은 정확성을 유지하면서도 경쟁력 있는 예측 정확도를 보여 주며 MOO 파레토 프론트의 볼록 외부에서도 솔루션을 회복할 수 있습니다.

- **Performance Highlights**: 실험 결과, ANFIS, X-ANFIS 및 가중 합계 MO-ANFIS 모델들은 각각 유사한 조건 하에서 훈련되었으며, 이로 인해 모델들의 초기화 민감도와 성능 변화가 관찰되었습니다. X-ANFIS는 이전 모델들보다 더 우수한 성능을 기록하며, 설명 가능성과 예측 정확성을 균형 있게 처리할 수 있는 가능성을 보여 줍니다. 이를 통해 연구자들은 지속적으로 해석 가능한 퍼지 시스템 구축을 위한 방향을 찾을 수 있게 되었습니다.
...
- **What's New**: 최근의 연구는 사전 학습된 모델에서 지식을 제거하는 데 주력하고 있지만, 단순히 제거된 지식이 다양한 방식으로 회복될 수 있음을 보여줍니다. 기존의 방법들은 해석 가능성이 낮아 지식의 지속성과 회복 여부를 이해하는 데 어려움을 겪었습니다. 이를 해결하기 위해, 저자는 조합적 해석 가능성(Combinatorial Interpretability)이라는 새로운 프레임워크를 도입하였으며, 두 겹의 신경망을 기반으로 모델의 가중치에 인코딩된 지식을 직접 검사할 수 있도록 합니다.

- **Technical Details**: 조합적 해석 가능성 프레임워크는 주요한 제약으로 표현 얽힘(representation entanglement)과 다의성(polysemanticity)을 지적합니다. 한 뉴런의 활성화가 여러 개의 무관한 개념을 인코딩할 수 있으며, 이러한 문제는 의미적 개념이 유심히 정의되지 않은 상황에서 더욱 복잡해집니다. 이 프레임워크에서, 저자들은 두 층 신경망을 사용하여 논리적 조항(logical clause)을 학습하는 실험을 진행하였습니다. 앞층은 긍정적인 뉴런과 부정적인 뉴런을 통해 논리적 조항을 인식하고 억압하는 역할을 수행합니다.

- **Performance Highlights**: 저자들은 세 가지 전통적인 무효화(unlearning) 기법을 사용하여 회복 시간을 분석하였으며, 이를 통해 특정 개념을 잃은 후에도 잔재 지식이 회복될 수 있는 경로를 제시했습니다. 특히, Gradient Ascent와 Task Vector 기반의 방법은 무관한 데이터로도 손쉽게 회복될 수 있음을 발견했습니다. 초기 개념이 잘 지워지지 않았음을 시사하며, 이후의 샘플링 또는 조정에서도 중요한 성과를 내고 있습니다.
...
- **What's New**: 이 논문에서는 강화 학습(Reinforcement Learning, RL)에서 검증 가능한 보상(Verifiable Rewards)으로 작은 오픈-웨이트 언어 모델을 개선하기 위한 훈련 데이터를 생성하는 새로운 방법을 소개합니다. 기존 데이터 생성 방법이 모델의 능력에 적응하지 않는 고정된 수정사항에 의존하는 반면, 본 연구는 기호적 문제 공간(Symbolic Problem Space)에서 수정을 수행하여 문제 구조를 세밀하게 제어할 수 있습니다. 이 방법은 문제의 복잡성을 조정하고, 자동으로 사실 기반 솔루션(Ground-Truth Solutions)을 생성하여 모델의 사고 능력을 개선합니다.

- **Technical Details**: 본 연구에서는 각 문제를 기호 변수(Symbolic Variables)와 제약조건(Constraints)으로 나타내어 문제를 기호적 표현으로 변환합니다. 이어서 학습된 프롬프트(Prompt)를 적용하여 기호적 표현을 수정하고, 학생 모델의 출력 결과를 피드백으로 사용하여 이러한 프롬프트를 적응적으로 학습합니다. 이러한 닫힌 루프(closed-loop) 프레임워크는 문제 난이도를 세밀하게 조정할 수 있으며, 기존의 개방형 루프(open-loop) 방식과 비교하여 효과적인 데이터를 생성할 수 있습니다.

- **Performance Highlights**: 실험 결과, RL 기반의 훈련 데이터 생성 방법이 PPO(Proximal Policy Optimization) 또는 GRPO(Generalized Reinforcement Learning with Proximal Optimality)를 사용하여 기존의 시드 데이터(Seed Data)만으로 훈련한 것에 비해 모델의 수학 문제 해결 능력을 향상시키는 데 기여한다는 것이 입증되었습니다. 또한, 본 접근법은 다양성(Diversity)을 높이는 결과를 나타내며, 이는 문제 출제 과정에서 기호적 표현을 통해 이루어집니다. 이러한 특징은 더 나은 학습 성과로 이어지며, 특히 작은 모델이 더 큰 모델에 근접한 성능을 보여줄 수 있도록 합니다.



### Online Realizable Regression and Applications for ReLU Networks (https://arxiv.org/abs/2602.19172)
- **What's New**: 이 논문에서는 온라인 회귀와 온라인 분류의 상이한 행동 양상에 대해 논의합니다. 특히, 실현 가능한 온라인 회귀 문제에서 경계가 없는 누적 손실 보장(horizon-free cumulative loss)이 어떻게 성립되는지를 탐구하며, 손실 함수가 대략적인 삼각 부등식(approximate triangle inequality)을 만족하는 경우에 한정을 둡니다. 기존의 연구에서 추상적인 차원인 스케일드 리틀스톤(Scaled Littlestone) 차원에 의해 최소 최대(realizable minimax) 누적 손실이 특징지어지지만, 이를 구체적으로 분석하는 것이 어렵다는 점을 밝혔습니다.

- **Technical Details**: 우리는 입력 공간 𝒳(𝓧), 레이블 공간 𝒴(𝒴) 및 가설 클래스 ℋ(ℋ)를 정의하고, 손실 함수 ℓ(𝓎,𝔂) 를 통해 온라인 학습 모델의 설정을 명시합니다. 특히, $	ext{Lipschitz}$ 회귀 문제에서 다룰 경우, 입력 샘플을 기반으로 한 손실 누적의 상한을 제공하기 위한 새로운 잠재적 방법(potential method)을 제안합니다. 이런 방법의 핵심은 $	ext{Dudley}$ 엔트로피 적분으로, 이는 가설 클래스의 덮기 수(covering numbers)와 관련이 있습니다.

- **Performance Highlights**: 모델이 제시한 방법을 통해 우리는 리틀스톤 차원이 어떻게 최대화되는지를 보여주고, 복잡도 측정 및 경계 보장(upper bounds)을 제공하는 데 기여하였습니다. 특정 가설 클래스를 통한 사례 연구에서, 리제뉴 클래스를 포함한 제약 조건 하에서도 재귀적(regression) 행동과 분류(classification)의 차별성을 확인하였으며, 우리는 $k$-ReLU 네트워크를 통해 누적 손실을 정량적으로 평가했습니다. 최종적으로, 이 연구는 온라인 회귀 모델의 실현 가능성에 대한 다양한 경계를 설정하는 데 중요한 기초를 마련합니다.
...
- **What's New**: 이 논문에서는 고차 마르코프 체인(task)을 소개하며, 변환기(transformers)가 통계적으로 유의미한 다양한 과거 위치에서 정보를 통합하는 방법을 조사합니다. 연구결과는 변환기들이 이 작업을 점진적으로 학습하며, 초기 경쟁적 학습 상태에서 통계적으로 지배적인 패턴으로 수렴하는 경향이 있음을 보여줍니다. 이후 협력적 학습 단계로의 전환을 확인하였으며, 이를 통해 정보의 중복 복사가 중요한 역할을 함을 밝혔습니다.

- **Technical Details**: 변환기 구조를 단순화된 미분 방정식을 사용하여 모델링하였고, 이를 통해 학습 동역학을 설명합니다. 연구에서는 민감한 attention 패턴이 변환기에서 점진적 학습을 촉진하는 주요 요소라는 점도 강조하며, 이들은 단일 self-attention 레이어에서 작동합니다. 경우에 따라 더 복잡한 in-context 학습 설정과 비교하여 단순한 설정에서 이러한 패턴의 중요성을 고립하였습니다.

- **Performance Highlights**: 모델은 훈련 세트의 크기에 따라 다양한 attention 패턴을 학습하며, 작은 데이터 세트에서는 가장 중요한 위치에서만 복사하는 방식으로 학습합니다. 초기에 학습을 중지하는 것이 일반화(regularization) 효과를 발휘하며, 이는 저데이터 상황에서 샘플 복잡성(sample complexity) 이점을 나타냅니다. 이 결과는 자연어 처리의 일반화 및 알고리즘적 추론에서의 복잡한 행동의 출현에 대한 이론적 기초를 제공합니다.
...
- **What's New**: 이 논문에서는 사전(prior)와 가능도(likelihood) 지정 오류를 명시적으로 다루기 위해 Huber 스타일의 오염 세트를 활용한 강건한 베이지안(random feature, RF) 회귀 모델을 제안합니다. 전통적인 RF 모델에 대해 여러 가지 오염 요인을 추가하여 포스터리어(posterior) 예측 분포의 신뢰 구간을 보존할 수 있는 경향성을 드러냅니다. 새로운 개념인 불확정성의 최고 밀도 영역(Imprecise Highest Density Region, IHDR)을 도입하여 더욱 신뢰할 수 있는 예측 구간을 제시합니다.

- **Technical Details**: 기존 RF 모델에서는 Gaussian prior와 likelihood에 의존하였으나, 이 연구에서는 오염된 credal 세트를 사용하여 불확정성의 영향을 보다 세밀하게 관리합니다. 특히, ε-오염된 사전 세트와 η-오염된 가능도 세트를 사용하여 모델링 초기 가정을 다각적으로 시험하는 접근법을 채택합니다. 우리는 베이지안 민감도 분석을 활용하여 예측 밀도, 불확실성 영역 및 분산 구조의 변화를 관찰합니다.

- **Performance Highlights**: 제안된 모델은 이론적으로에서 훈련된 RF 모델의 예측 불확실성이 수치적으로 계산 가능한 방법으로 제시됨으로써 신뢰성을 향상시킬 수 있는 가능성을 보여줍니다. 높은 차원에서의 비율적 성장 규칙에서도 일관된 성능을 보여주며, 특히 고전적인 RF 모델의 쌍곡선 성질을 유지합니다. 우리의 예측 구간과 분산 경계를 통해 구체적인 사례 연구를 진행함으로써 이론적 기여를 실증적으로 확인했습니다.
...
- **What's New**: 이 논문에서는 적대적 예제(adversarial examples)를 생성하는 과정을 최적화 문제(optimization problem)로 재구성하였습니다. 연구진은 기존의 sign-based optimizer인 I-FGSM 및 MI-FGSM의 한계점을 지적하며, 특히 비수렴(non-convergence) 및 불안정성(instability) 문제에 초점을 맞추고 있습니다. 이들은 적대적 공격의 전이 가능성(transferability)에 부정적인 영향을 미친다고 주장합니다.

- **Technical Details**: 저자들은 계단형(step-size scheduling) 사용이 비수렴 및 불안정의 원인 중 하나라고 주장하며, 이를 해결하기 위해 Monotonically Decreasing Coordinate-wise Step-sizes (MDCS)를 갖춘 새로운 공격 알고리즘을 제안합니다. MDCS-MI 알고리즘은 이론적으로 최적의 수렴 속도(convergence rate)인 $O(1/\sqrt{T})$를 달성하며, 여기서 $T$는 반복(iteration) 수를 의미합니다. 이와 더불어, sign-based 최적화 방법의 적대적 공격에서의 유효성을 높이기 위한 전반적인 방법론을 제공합니다.

- **Performance Highlights**: 연구진은 이미지 분류(image classification) 및 크로스 모달 검색(cross-modal retrieval) 작업에 대한 광범위한 실험을 통해, 제안한 MDCS-MI 방법이 전이 가능성을 현저히 개선하고 안정성을 높인다는 것을 입증하였습니다. 기존의 최첨단(sign-based) 방법과 비교하여 공격의 효과 및 신뢰성을 높이고 있음을 보여줍니다. 이러한 접근법은 공격 성공률(attack success rate)을 개선하는 데 기여합니다.



### RKHS Representation of Algebraic Convolutional Filters with Integral Operators (https://arxiv.org/abs/2602.19094)
- **What's New**: 본 논문은 적분 연산자들이 신호 처리(signal processing)에서 중요한 역할을 하며, 연속 네트워크 모델(continuous network models)인 그래프온(graphons)에서 고전적인 컨볼루션(convolution) 및 필터링(filtering)의 기초가 됨을 강조합니다. 이 연구는 더불어, 적분 연산자와 재생 커널 힐벡 공간(reproducing kernel Hilbert spaces, RKHS) 간의 연관성을 탐구하며, 이를 통한 새로운 컨볼루셔널 신호 모델을 제공합니다.

- **Technical Details**: 저자들은 적분 연산자의 범위가 자연스럽게 RKHS 컨볼루셔널 신호 모델을 유도함을 보여주고, 이 모델의 재생 커널(reproducing kernels)은 연산자 기호의 박스 곱(box product)에 의해 결정된다고 설명합니다. 폴리노미얼 필터링(polynomial filtering)과 적분 연산자의 관계를 분석하며, 이 과정이 반복된 박스 곱(iterated box products)에 상응함을 입증하여 단위 커널 대수(unital kernel algebra)를 형성합니다.

- **Performance Highlights**: 결과적으로, 이 연구는 그래프온 신호 처리에서 고유 분해(eigendecompositions)와 RKHS 표현 간의 정밀한 관계를 Establish하며, 이는 지향성 그래프온(directed graphons)으로 자연스럽게 확장될 수 있습니다. 최적 필터(optimal filters)에 대한 연구 결과, 주파수 영역이 원래 신호의 부분집합일 때, 주어진 규제된 학습 문제에 대한 유한차원 RKHS 표현을 제공한다고 설명하며, 이는 적분 연산자 기반 신경망 아키텍처에서 학습 가능한 필터의 기초를 제공합니다.



### TimeRadar: A Domain-Rotatable Foundation Model for Time Series Anomaly Detection (https://arxiv.org/abs/2602.19068)
- **What's New**: 이번 논문에서는 새로운 시계열 이상 탐지(Time Series Anomaly Detection, TSAD) 모델인 TimeRadar를 소개하고 있습니다. 기존의 시계열 기반 모델들이 정규 패턴을 학습하는 데 초점을 맞추었던 반면, TimeRadar는 비정상 패턴을 효과적으로 식별할 수 있는 프랙셔널(time-frequency) 도메인을 활용합니다. 이 모델에서는 데이터 의존적인 프랙셔널 시간-주파수 표현을 통해 정규 신호와 비정상 신호를 적응적으로 구분하는 것을 목표로 하고 있습니다.

- **Technical Details**: TimeRadar 모델은 새로운 구성 요소인 Fractionally Modulated Time-Frequency Reconstruction (FTFRecon)와 Contextual Deviation Learning (CDL)을 포함하고 있습니다. FTFRecon은 시계열을 연속 시간과 주파수 도메인에서 가장 뚜렷한 각도로 회전시켜 데이터 재구성을 최적화합니다. CDL은 입력의 패치 간 편차를 모델링하여 정상 샘플 간의 집합성을 촉진하고 이상치와의 간격을 넓혀 비정상성을 더욱 잘 포착합니다.

- **Performance Highlights**: 여덟 개의 인기 있는 TSAD 벤치마크에서 수행된 광범위한 실험을 통해 TimeRadar는 기존의 TSFM 기반 방법 및 전통적인 TSAD 기법보다 월등한 성능을 보였습니다. 특히, 제로샷(zero-shot) 및 몇 샷(few-shot) 추론 설정에서 일관되게 우수한 결과를 도출해냈습니다. 이를 통해 TimeRadar는 다양한 데이터 세트에서 비정상 패턴을 효과적으로 탐지할 수 있는 능력을 입증하였습니다.
...
- **What's New**: 이 논문은 비가역적인(intransitive) 선호를 처리하는 데 중점을 두고 있으며, 그 결과로 새로운 게임 이론적 솔루션인 Maximum Entropy Blackwell Winner (MaxEntBW)를 제안합니다. 이 접근법은 다중 목표의 비가역적 선호 하에서도 잘 정의되어 있으며, 선호 정제를 위한 기존의 방법들과는 차별화된 점이 있습니다. 또한, PROSPER라는 효율적인 알고리즘을 도입하여 기존의 방법들이 필요로 하는 스칼라화 과정 없이 다중 목표를 직접 다룰 수 있도록 합니다.

- **Technical Details**: 제안된 PROSPER 알고리즘은 다중 목표 LLM 피드백을 기반으로 선호 정제를 수행하는 데 사용되며, MINIMAX 정리에 의존하지 않고도 MaxEntBW를 효율적으로 계산할 수 있도록 설계되었습니다. 알고리즘은 단일 플레이어 최적화 문제로 축소할 수 있으며, 이를 통해 LLM 심사자로부터 더 정확한 피드백을 얻는 것이 가능합니다. 이로 인해, PROSPER는 훈련된 모델 체크포인트를 7B와 3B 파라미터 스케일로 제공할 수 있음을 보여줍니다.

- **Performance Highlights**: PROSPER는 지시 사항을 따르는 것과 일반적인 채팅 벤치마크를 기준으로 하여 고려된 모든 기준선을 초월하는 성능을 보였습니다. 다중 목표의 LLM-as-a-Judge 피드백을 통해 실험적으로 검증된 결과, 선호가 비가역적인 문제를 해결하는 데 있어 기존의 방법보다 상당한 개선을 나타냈습니다. 이는 특히 LLM 모델의 품질이 직접적으로 향상되는 데 기여합니다.
...
- **What's New**: 이 논문에서는 Empirical Risk Minimization (ERM)의 이론적 복잡성을 실제 비트 수준 모델인 \(\mathsf{ERM}_{\text{bit}}\)에서 분석합니다. 이러한 모델에서 신경망의 활성화 함수에 따른 두 가지 극단적인 복잡성이 발견되었습니다. 깊은 신경망의 비트 복잡성이 높아 \(\#P\)-Hard 문제로 분류되고, 반면에 ReLU와 같은 조각 선형 활성화 함수의 경우, 해결이 상대적으로 간단하여 \(\mathsf{NP}\)-완전 문제에 속하는 것으로 나타났습니다.

- **Technical Details**: 이 연구는 제한된 정밀도 하드웨어에서의 신경망 훈련의 복잡성을 분석합니다. \(\mathsf{ERM}_{\text{bit}}\) 모델은 신경망 아키텍처와 데이터셋이 유한한 비트로 인코딩되어 있으며, 각 파라미터의 길이가 다항식으로 제한됩니다. 다항식 차수 이상의 비선형 다항식 활성화 함수에 대해 \(\mathsf{ERM}_{\text{bit}}\)는 \(\#P\)-Hard 문제로 분류되는 반면, 조각 선형 활성화 함수는 다항 시간 내에 확인할 수 있는 경향이 있습니다.

- **Performance Highlights**: 이 연구의 결과는 유한 정밀도 제약이 학습 가능성의 근본적인 결정 요인임을 보여줍니다. 깊은 네트워크에서 비선형 다항식 활성화 함수의 경우 복잡성이 높아지는 반면, ReLU와 같은 조각 선형 활성화 함수에서는 효율적인 훈련이 가능하다는 점이 확인되었습니다. 이러한 차이는 모델의 성능에 큰 영향을 미치며, 이는 실제 딥러닝 아키텍처가 조각 선형 활성화 함수를 선호하게 하는 이유이기도 합니다.



### Conditionally Site-Independent Neural Evolution of Antibody Sequences (https://arxiv.org/abs/2602.18982)
Comments:
          24 pages, 14 figures. Currently under review

- **What's New**: 이번 연구에서는 CoSiNE이라는 새로운 모델을 소개하며, 이는 항체(antibody) 결합 친화성(affinity)을 효과적으로 최적화하는 혁신적인 방법론을 제공합니다. CoSiNE는 연속 시간 마르코프 체인(continuous-time Markov chain)을 기반으로 하여 깊은 신경망(deep neural network)을 통해 항체의 진화 과정을 모델링합니다. 이 모델은 선택(selection)과 상황 의존적인 체세포 고돌연변이(somatic hypermutation)를 명확히 분리하여, 변이 효과의 예측에서 기존의 모델을 능가하는 성능을 입증합니다.

- **Technical Details**: CoSiNE는 항체의 성질을 보존하면서 필수적인 상관 관계를 수학적으로 표현할 수 있는 깊은 신경망을 활용합니다. 본 논문에서 제안된 수식은 CoSiNE이 처리할 수 있는 후속 점 돌연변이(sequential point mutation) 과정의 근사치를 제공하며, 이 과정은 가지 길이에 대해 제곱 오차 경계(quadratic error bound)를 가지고 있습니다. 또한, CoSiNE는 약 10만 개의 클론 트리에 적합하여 클론 확장을 시뮬레이션하는 기법으로 기계적 샘플링 방법을 활용합니다.

- **Performance Highlights**: CoSiNE 모델은 제로샷 변이 효과 예측(zero-shot variant effect prediction)과 인 실리코(in silico) 항체 최적화 작업에서 최신 언어 모델을 초월하는 성능을 보여주었습니다. 연구에서는 CoSiNE가 항체 적합도를 추론하는 데 어떻게 선택과 변이의 영향을 분리할 수 있는지 설명하며, 이를 통해 도출된 항체의 기능적 특성을 조절하는 방법도 제안합니다. 이러한 연구 성과는 항체 공학 분야에서 새로운 모델링 접근 방식의 가능성을 시사합니다.
...
- **What's New**: 이 논문에서 저자들은 Incremental TNP (incTNP)를 통해 Transformer Neural Processes (TNP)의 한계를 극복합니다. incTNP는 연속적인 데이터 스트림을 처리할 수 있는 능력을 갖추고 있으며, 기존 TNP가 갖지 못한 저비용의 점진적 업데이트를 지원합니다. 이는 대규모 언어 모델(Large Language Models)에서 영감을 받은 개념을 활용하여 구현되었습니다.

- **Technical Details**: incTNP는 causal masking을 적용하여 self-attention 메커니즘을 개선하며, Key-Value (KV) 캐싱을 활용하여 선형 시간 복잡도로 문맥(context)을 효율적으로 업데이트합니다. 또한, dense autoregressive training(incTNP-Seq)을 도입하여, 전체 문맥 크기에서 학습을 분산시킴으로써 보다 효율적인 데이터 학습이 가능하도록 합니다. 이로 인해 incTNP는 높은 주파수 업데이트와 실시간 스트리밍 시나리오에서 즉각적으로 활용될 수 있는 정보를 제공하게 됩니다.

- **Performance Highlights**: 실험 결과, incTNP는 기존의 TNP보다 비교 가능하거나 더 높은 예측 성능을 보이는 것으로 나타났으며, 특히 재정의된 implicit Bayesianness 지표를 통해 예측 일관성에서도 매우 유사한 성능을 보여주었습니다. incTNP는 빠른 업데이트 속도를 자랑하면서도 예측 품질을 유지하는 모델로서, 대규모 실험을 통해 그 신뢰성을 입증하였습니다.
...
- **What's New**: 본 연구는 Transformer 모델의 내부 중복성을 줄이기 위한 새로운 접근법인 ‘대칭 축소(symmetry reduction)’를 제안합니다. 기존의 방법들이 대칭을 명시적으로 깨뜨리는 방식이라면, 우리는 대칭을 유지하며 불필요한 자유도를 제거하는 방식입니다. 이 프레임워크는 모델의 표현 및 최적화 동역학을 불변 관계량(relational quantities) 관점에서 재정립하여 중복 매개변수를 자연스럽게 줄일 수 있도록 합니다.

- **Technical Details**: 대칭 축소 접근법은 Transformer의 내부 동작을 단일 좌표 표현(determined by coordinates)이 아니라 관계적 불변량(relational invariant) 기반으로 재구성합니다. 이는 주어진 입력에 대해 모델의 행동이 동일하게 유지되는 불변성을 통해 이루어집니다. 특히, 이 연구는 내부 표현의 프레임을 ‘관계적 불변성(relational invariance)’으로 ‘드레싱(dressing)’하여, ‘대칭 없는(parameter-free)’, 적합한 구조를 형성하도록 제안합니다.

- **Performance Highlights**: 이 프레임워크는 무작위적인 좌표 표현 대신 의미 있는 관계적 자유도에 따라 학습을 진행할 수 있는 개념적 및 수학적 틀을 구축하는 것을 목표로 합니다. 추가적으로, 이 방식은 최적화의 중복성을 줄이고 내부 메커니즘을 명확히 하여 더욱 해석 가능하고 불변적인 아키텍처의 방향성을 열어줄 것으로 기대됩니다.



### Exponential Convergence of (Stochastic) Gradient Descent for Separable Logistic Regression (https://arxiv.org/abs/2602.18946)
- **What's New**: 이 논문은 gradient descent와 stochastic gradient descent의 안정성과 가속화 사이의 관계를 재조명합니다. 연구자들은 기존의 불안정한 영역에서 이루어지는 가속화의 개념을 넘어서, 단순하고 비적응적인 증가하는 step-size 스케줄을 활용하여 안정적인 최적화 과정에서도 기하급수적인 수렴을 이룰 수 있음을 입증했습니다. 이로 인해 대규모 기계 학습 모델에서의 실제 성과와 이론 간의 간극을 메우는 새로운 통찰을 제공합니다.

- **Technical Details**: 잇따라 제시된 기본 이론에 따르면, separation logistic regression 문제에서 gradient descent는 플랫폼의 안정적인 영역 내에서 수명시간에 구애받지 않는 기하급수적 수렴을 달성하는 간단한 고정 표준 크기 단계 전략을 사용할 수 있습니다. stochastic gradient descent 또한 관찰된 손실 값을 기반으로 하는 경량의 적응형 스텝 크기 규칙을 통해 거듭되는 수렴 속도를 보장합니다. 이러한 방법들은 기존의 복잡한 학습률 조정법에 비해 실용적이고 간단합니다.

- **Performance Highlights**: 제안된 방법은 기존의 폴리노미얼 수렴 보증을 초과하는 빠른 속도를 보여주며, 즉각적인 수렴을 통해 안정성과 효율을 온전히 유지함을 입증했습니다. 빠른 수렴 속도는 두 개의 주요 섹션에서 다루어지며, 이는 고정 단계 크기 스케줄을 통한 gradient descent와 적응형 단계 크기 규칙을 사용하는 stochastic gradient descent에 대한 실험 결과에서도 명확하게 드러났습니다. 이러한 점은 기계 학습의 실용적 응용 가능성을 크게 향상시키는 요소가 될 것으로 보입니다.



### LoMime: Query-Efficient Membership Inference using Model Extraction in Label-Only Settings (https://arxiv.org/abs/2602.18934)
- **What's New**: 본 논문에서는 전이(inference) 및 모델 추출(model extraction)의 개념을 활용하여 비용 효율적인 라벨 전용 회원 추정 공격(label-only membership inference attacks, MIAs) 프레임워크를 제안합니다. 기존의 MIAs는 비현실적인 가정들에 의존하며, 이는 방어 기법에 취약합니다. 새로운 접근법은 목표 모델을 질의하는 대신, 고충실도 대체 모델을 한 번의 질의를 통해 추출하여 반복 질의를 제거합니다. 이로 인해 총 질의 비용이 크게 감소합니다.

- **Technical Details**: 제안된 방법은 목표 모델 MM에서 정보를 직접 추출하기보다, 최초에 효율적인 샘플링 전략을 통해 고충실도 대체 모델 SS를 추출하는 데 초점을 두었습니다. 이 대체 모델은 무한한 샘플에서 오프라인으로 회원 추정이 가능하도록 하여 대규모 평가를 용이하게 합니다. 연구를 통해 우리는 신뢰도 점수나 공개 데이터셋에 대한 의존성을 최소화하면서 회원 정보 유출 행동을 유지하는 모델을 성공적으로 추출할 수 있음을 보여주었습니다.

- **Performance Highlights**: 제안된 프레임워크는 평가에 약 1%의 훈련 샘플을 소모하여 기존의 라벨 전용 MIA와 유사한 성능을 발휘합니다. 벤치마크 데이터셋에서 회원 추정 정확도 및 AUC는 목표 모델과 5-11% 이내의 유사성을 가지며, 많은 경우 눈에 띄는 성능 저하 없이 기법을 수행합니다. 또한, 강력한 차등 프라이버시를 제외한 방어 기법에 대해 우리의 공격이 여전히 효과적임을 입증했습니다.
...
- **What's New**: 본 논문에서는 인공지능(AI) 모델의 성능을 인류 전체를 기준으로 조정하고, 이를 통해 공통된 인간 기준의 척도에서 AI의 성능을 보고하는 새로운 프레임워크를 제안합니다. 시멘틱 스케일링의 방법을 활용하여 다양한 능력에 대한 다단계 척도를 구축하고, 이를 교육 및 추론 기준에 해당하는 공개 데이터로 보정합니다. 이 새로운 접근 방식은 AI 모델의 "인간 수준(human level)" 인식의 변별력을 높이는 데 기여할 것입니다.

- **Technical Details**: 여기서 제안하는 프레임워크는 다양한 능력(추론, 이해, 지식 등)을 다루며, 각 수준이 전 세계 인구의 성공 확률을 나타내는 로가리즘 스케일을 기반으로 설정됩니다. 대규모 언어 모델(LLM)을 사용하여 다양한 인구 통계를 기준으로 샘플 간 정보를 추정하고, 데이터 그룹 슬라이싱(group slicing)과 후속 층화(post-stratification)를 통해 다양한 매핑의 질을 평가합니다. 이 방법은 인간 모집단을 기반으로 AI와 인간 간의 비교를 보다 의미 있게 만들어 줍니다.

- **Performance Highlights**: 최신 프레임워크는 다양한 AI 모델의 성능을 전체 인간 인구를 기준으로 새로운 방식으로 측정할 수 있는 가능성을 보여줍니다. 예를 들어, 여러 과목을 다루는 다수의 벤치마크에서 최첨단 AI 모델이 평균적으로 90% 이상의 점수를 기록하고, 특정 전문 과제에서는 인간 전문가를 초과하는 성과를 거두고 있습니다. 이러한 발전은 AI가 특정 과제에서 어떻게 인간 성과를 초과하거나 미달하는지를 보다 정밀하게 평가할 수 있는 기준을 제공합니다.



### SLDP: Semi-Local Differential Privacy for Density-Adaptive Analytics (https://arxiv.org/abs/2602.18910)
- **What's New**: 이번 연구에서는 Semi-Local Differential Privacy (SLDP)라는 새로운 개인 정보 보호 프레임워크를 제안합니다. SLDP는 각 사용자에게 지역 밀도를 기반으로 개인 정보 보호 영역을 할당하고, 이 영역 내에서 점의 이동 가능성을 통해 인접성을 정의합니다. 이 프레임워크는 개인 정보 보호 비용을 세분화 반복 횟수와 분리하여 높은 해상도의 그리드를 제공하며, 추가적인 개인 정보 보호 예산 비용 없이도 사용 가능합니다.

- **Technical Details**: SLDP는 사용자의 구별 가능성 집합을 데이터 종속 개인 정보 보호 영역으로 제한함으로써 개인 정보 보호 보장을 담당합니다. 제안된 상호작용 프로토콜은 로컬에서 무작위화된 신호를 이용해 데이터를 개인 정보 보호 방식으로 수집하고, 공공 채널을 통해 개인 정보 보호 영역을 추정합니다. 이 프로토콜은 (ε,δ)-DP를 만족하며, 반복적 세분화 횟수에 따라 누적 개인 정보 손실이 증가하지 않는다는 중요한 결과를 보여줍니다.

- **Performance Highlights**: 실험적으로, SLDP는 합성 데이터와 실세계 데이터 세트에서 평균 추정, 분류, 공간 범위 쿼리 작업에서 표준 LDP보다 더 나은 성능을 보이며 중앙 집중식 DP와 경쟁할 만한 성능을 입증했습니다. 연구 결과, SLDP 프레임워크는 데이터 밀도를 인식하는 세분화를 통해 더 향상된 유용성을 제공합니다.
...
- **What's New**: 이 논문에서는 벡터 값 및 조건부 밀도 예측을 대상으로 하여 일반적인 발산( divergence) 아래에서 집합 적용의 안정성을 조사하고 있으며, 이를 통해 약한 보장을 강한 보장으로 강화하는 조건을 규명하고 있습니다. 새로운 개념인 ($\alpha,\beta$)-boostability를 성문화하여 기하학적 중앙값 집합이 다양한 발산을 위해 이 성질을 충족함을 보여줍니다. 또한, 논문에서는 일반적인 구조적 예측을 지원하는 Boosting 프레임워크인 GeoMedBoost를 제안하고 있습니다.

- **Technical Details**: 구조적 예측을 위한 GeoMedBoost 프레임워크는 약한 예측자 조건을 만족해야 하며, 기하학적 집합 방법을 사용하여 예측 오류를 제어합니다. ($\alpha,\beta$)-boostability는 집합 적용의 속성이며, 약한 예측자의 성과와 최종 예측 오류 간의 명시적 경계를 제공합니다. 이 프레임워크는 고전적인 알고리즘인 MedBoost, AdaBoost, SAMME를 특수한 경우로 복원하며, 구조적 예측에 대한 통합적인 기하학적 관점을 제공합니다.

- **Performance Highlights**: 이 논문은 ℓ1 및 ℓ2 거리에서의 벡터 값 예측의 정확한 boostability를 특성화하고, 조건부 밀도 추정에서의 boostability 결과를 제시합니다. 특히, KL 발산은 기하학적 중앙값 집합을 통해 직접적으로 boostable하지 않지만, Hellinger 거리와의 관계를 활용하여 간접적으로 처리할 수 있음을 보여주었습니다. 이러한 결과는 확률적 예측 및 가능도 기반 학습에서 KL 위험 아래에서도 부스트가 가능하다는 것을 입증합니다.
...
- **What's New**: 이 논문은 Bayes-adaptive Markov decision processes에서 학습과 계획을 위한 변분(variational) 프레임워크를 제안합니다. 이 프레임워크는 변분 신념 학습(variational belief learning), 순차 몬테카를로 계획(sequential Monte-Carlo planning), 메타 강화 학습(meta-reinforcement learning)을 통합하여 탐색(exploration)과 활용(exploitation) 간의 최적 균형을 이룹니다. 새롭게 제안된 방법인 VariBASeD는 단일 GPU 환경에서 작동하며, 계획 예산(planning budgets)에 대한 스케일링(scaling)에 있어 유리한 성능을 보여줍니다.

- **Technical Details**: 변분 프레임워크는 Bayes-optimal agent의 데이터 효율성을 극대화하는 문제에 중점을 둡니다. 이 방법은 기존의 고비용인 훈련 과정을 개선하기 위해 설계되었습니다. VariBASeD는 특히 샘플(Sample) 및 런타임 효율성(runtime-efficiency)에서 이전 방법에 비해 성능을 향상시킵니다.

- **Performance Highlights**: 논문의 주된 성과는 VariBASeD 방법이 더 큰 계획 예산에서도 효율적으로 작동한다는 점입니다. 실험 결과, 제안된 방법이 기존의 방법에 비해 샘플 수와 실행 시간 모두에서 효과적으로 개선되었음을 입증하였습니다. 이는 강화 학습 분야에서 탐색과 활용의 최적 균형을 더욱 쉽게 달성할 수 있도록 도와줍니다.
...
- **What's New**: 이 논문은 강화 학습(Reinforcement Learning, RL)에서 작업의 복잡성을 측정하는 방법에 대해 새로운 분석을 제공합니다. 특히 기존의 도식에서 정립된 지표들 외에도 비도식 지표의 필요성을 강조하고 있습니다. 이를 통해 실험적인 로봇 조작(setup)에서의 복잡성 평가가 여전히 미세하다는 점을 밝혀내었습니다.

- **Technical Details**: 연구진은 우연한 정책의 성능을 통계적으로 분석하는 Random Weight Guessing (RWG)과 정보 이론적 지표인 Policy Information Capacity (PIC), Policy-Optimal Information Capacity (POIC)를 평가합니다. 두 가지 설정에서 두 개의 로봇 팔을 비교할 때, PIC는 두 링크 로봇 팔이 단일 링크보다 더 쉽다고 제안하지만, 이는 실제 상황과는 다릅니다. 이러한 이론적 지표는 RL의 기본 이해와 모순되는 결과를 보입니다.

- **Performance Highlights**: PIC와 POIC는 희소 보상(sparse rewards)과 조밀 보상(dense rewards) 설정에서 서로 다른 결과를 나타내어 논문에서 제시된 로봇 조작과 관련된 실험 결과와 불일치를 보입니다. 이러한 발견들은 RWG 기반의 지표를 넘어서는 새로운 지표 개발의 필요성을 강조하며, 비도식 RL에서 작업의 복잡성을 보다 신뢰성 있게 포착할 수 있는 방향으로의 연구를 촉구합니다.
...
- **What's New**: 이 논문에서는 그래프 고유 변환(Graph Fourier Transform, GFT)을 기반으로 한 새로운 스펙트럴 그래프 신경망(GNN) 모델 L2G-Net(로컬 투 글로벌 네트워크)을 소개합니다. 기존의 스펙트럴 방법들은 고유 기저(eigenbasis) 계산의 비용과 스펙트럴 표현의 정점 도메인(locality lack)에서 문제를 겪고 있었으나, L2G-Net은 이러한 한계를 극복합니다. 이 새로운 모델은 하위 그래프(subgraphs)에서 작용하는 연산자로 GFT를 분해하고 Cauchy 행렬들을 조합하여 작동합니다.

- **Technical Details**: L2G-Net은 GFT의 새로운 분해 방식을 활용하여 하위 그래프의 스펙트럴 표현을 처리한 후, 이를 구조화된 행렬(structured matrices)을 통해 결합합니다. 이 프로세스는 모든 고유 분해(eigendecompositions)를 피하며, 그래프 토폴로지(graph topology)를 이용하여 노드 수에 대해 이차 복잡도(quadratic complexity)로 분해를 구성합니다. 여기서 복잡도는 하위 그래프 인터페이스 크기에 의해 조정됩니다.

- **Performance Highlights**: 비교 실험에서 L2G-Net은 비국소(non-local) 의존성을 강조하는 벤치마크에서 기존의 스펙트럴 기법들을 능가하며, 적은 수의 학습 가능한 매개변수로 최신 기술과 경쟁력을 가지는 결과를 보여주었습니다. 이 모델은 비지역적 의존성 모델링에서 특히 강력한 성능을 발휘하여 GNN 분야에서의 가능성을 제시합니다.
...
- **What's New**: 이 논문은 Spectral Generator Neural Operator (SGNO)를 제안하여 시간 종속 부분 미분 방정식(PDE)의 예측 안정성을 향상시키고, 긴 예측 기간에 대한 신뢰성을 확보하는 방법을 다루고 있습니다. SGNO는 선형 및 비선형 동역학을 분리하여 안정적인 오토회귀(autoregressive) 롤아웃을 생성합니다. 이 방식은 FGNO의 설계를 통해 성과를 보여주며 특정 설계 요소의 역할을 실험적으로 증명했습니다.

- **Technical Details**: SGNO는 Fourier 공간에서 학습된 대각선 생성기(diagonal generator)를 사용하는 지수 시간 차분(exponential time differencing) 업데이트를 통해 선형 업데이트를 수행합니다. 생성기의 실수 부분이 항상 비양수로 제한되어 선형 동역학이 증폭되지 않도록 제어합니다. 비선형 동역학 부분에서는 각 Fourier 모드 내에서 채널 혼합과 게이팅(gating)된 힘을 추가하여 비선형 업데이트를 제어합니다.

- **Performance Highlights**: SGNO는 APEBench 벤치마크를 통해 1D, 2D 및 3D PDE 문제에서 우수한 성능을 보이며 긴 기간의 오차를 줄이고 안정적인 롤아웃 길이를 늘린 것으로 나타났습니다. 설계 요소인 생성기 제약과 게이팅, 필터링의 역할이 실험적으로 확인되었습니다. SGNO는 강력한 신경 연산자(neural operator) 기법들과 비교했을 때 안정성과 성능 면에서 두각을 나타냅니다.
...
- **What's New**: 이번 논문에서는 Latent Dirichlet Allocation (LDA)의 한계를 극복하기 위한 새로운 프레임워크인 Latent Dirichlet-Tree Allocation (LDTA)를 제안합니다. LDTA는 Dirichlet prior를 임의의 Dirichlet-Tree (DT) 분포로 대체하여 주제 비율에 대한 표현력을 확장합니다. 이 모델은 LDA의 생성 구조를 유지하면서도 트리 구조의 priors를 가능하게 하여 주제 간의 복잡한 상관관계를 캡처할 수 있도록 합니다.

- **Technical Details**: LDTA는 Mean-Field Variational Inference (MFVI)와 Expectation Propagation (EP)이라는 두 가지 추론 방법론을 통해 효율적인 불확실성 추정을 수행합니다. 이 두 방법은이론적인 개발을 통해 벡터화된 형태로 구현되며, GPU 가속이 가능한 방식으로 처리할 수 있습니다. 이를 통해 LDTA 모델은 복잡한 Dirichlet-Tree 구조에서도 신속한 추론을 가능하게 합니다.

- **Performance Highlights**: LDTA 프레임워크는 LDA보다 모델링 용량이 상당히 확장되며, 대규모 데이터셋에서도 효과적으로 작동합니다. 논문의 벡터화된 구현은 기존 스칼라 또는 루프 기반 구현에 비해 수행 시간을 대폭 줄이면서 최신 데이터셋의 스케일에도 적응할 수 있도록 만들어졌습니다. 다양한 Dirichlet-Tree priors의 실험을 통해 LDTA의 성능을 검증하고 비교하는 과정을 진행합니다.
...
- **What's New**: 이번 연구에서는 그래프 이상 탐지 (Graph Anomaly Detection; GAD) 분야에서 기존의 데이터셋 특정 훈련법 대신 여러 미지의 데이터셋에서 재훈련 없이도 이상을 탐지할 수 있는 새로운 일반화 GAD 패러다임을 제안합니다. 이 연구는 `ARC`와 `ARC_zero`라는 두 가지 메소드를 도입하여, 각각 few-shot 및 zero-shot 상황에서 효과적으로 이상을 탐지할 수 있도록 설계되었습니다. 이러한 접근법은 대규모 데이터 및 데이터 접근이 제한된 상황에서도 유용하게 적용할 수 있어 실용적인 가치가 높습니다.

- **Technical Details**: 제안된 GAD 방법인 ARC는 세 가지 핵심 모듈로 구성되어 있습니다. 첫 번째는 특성 정렬 (Feature Alignment) 모듈로, 데이터셋 간의 특성을 통합하고 정렬합니다. 두 번째는 잔차 GNN 인코더 (Residual GNN Encoder)로, 데이터셋 중립적인 이상 표현을 캡처하며, 마지막 세 번째는 적은 수의 정상 샘플을 이용하여 이상 점수를 계산하는 교차 주의 인-context 학습 모듈입니다. ARC_zero는 레이블이 없는 경우에도 대표적인 유사 정상 노드를 선택하여 이상 탐지를 수행할 수 있도록 설계되었습니다.

- **Performance Highlights**: 실제 데이터셋 17개에 대한 광범위한 실험 결과, ARC와 ARC_zero 모두 이상 탐지에서 뛰어난 성능을 보이며 강력한 일반화 능력을 지니고 있음을 입증하였습니다. 특히, few-shot 및 zero-shot 시나리오에서도 효율적으로 작동하여 기존의 GAD 방법들이 가지는 일반적인 한계를 뛰어넘는 것을 보여주었습니다. 이로써 두 가지 방법 모두 현실적인 응용 가능성을 높이며, GAD 연구의 새로운 방향성을 제시하고 있습니다.
...
- **What's New**: 이번 논문에서는 3D 라디오 맵 데이터 생성을 위한 효율적인 방법과 모델 훈련scheme을 제안하는 RadioGen3D 프레임워크를 소개합니다. 기존의 딥러닝 접근법들이 2D 환경에 제한되어 있는 것과 달리, 본 연구는 3D 신호 전파 특징을 포착할 수 있는 새로운 방법론을 제공합니다. 이를 위해 새로운 데이터 합성 방법과 조건부 생성적 적대 신경망(conditional generative adversarial network, cGAN)을 통한 훈련 방식을 구현했습니다.

- **Technical Details**: RadioGen3D는 파라메트릭(target) 모델을 활용하여 2D 레이 트레이싱 및 3D 채널 페이딩 특성을 포착합니다. 이 모델은 최소한의 실측치를 기반으로 현실적인 계수 조합을 도출하며, 이는 대규모 합성 데이터셋인 Radio3DMix 구축에 활용됩니다. 또한, cGAN을 통해 다양한 입력 피처 조합을 바탕으로 3D U-Net 모델을 훈련시켜, 뛰어난 라디오 맵 추정 정확도와 속도를 제공합니다.

- **Performance Highlights**: 실험 결과에 따르면, RadioGen3D는 기존 모든 기준선 모델 대비 추정 정확도와 속도에서 뛰어난 성능을 보였습니다. 또한, 데이터셋의 규모를 활용한 정교한 튜닝 실험을 통해 강력한 일반화 능력도 검증되었습니다. 이러한 결과는 RadioGen3D가 환경 인지 기반의 3D 라디오 맵 추정에 있어 뛰어난 잠재력을 지닌 것을 보여줍니다.
...
- **What's New**: 이번 논문에서는 Physical-Conditioned World Model Attack (PhysCond-WMA)이라는 새로운 공격 기법을 제안합니다. 이는 도로 모델링에서 HDMap 임베딩 및 3D 박스 특징과 같은 물리적 조건 채널을 교란하여 의미적, 논리적 또는 결정 수준의 왜곡을 유도하면서 지각적 음악 품질을 유지하는 첫 번째 화이트박스 공격입니다. 물리적 조건에 대한 공격은 고유한 단점이 있으며, 선행 연구들이 다루지 않았던 보안 취약점들을 최초로 밝혀내었습니다.

- **Technical Details**: PhysCond-WMA는 두 단계의 최적화를 통해 작동합니다. 첫 번째 단계는 품질 보존 가이던스 단계로, 회귀 확산 손실을 허용된 임계값 이하로 억제합니다. 두 번째 단계는 모멘텀 가이던스 배경 최적화로, 안정적이고 일관된 의미적 변화를 위해 목표 정렬 그래디언트를 축적합니다. 이를 통해 물리적 조건 레이어에 작은 교란을 자연스럽게 흡수시키며, 이미지 충실도를 최소한으로 감소시키며 공격의 효과를 극대화합니다.

- **Performance Highlights**: 실험 결과에 따르면, PhysCond-WMA의 공격 성공률은 0.55에 이르며, FID는 평균 9%, FVD는 평균 3.9% 증가합니다. 또한, 공격받은 비디오를 사용하여 3D 감지 성능이 약 4% 감소하고, 오픈 루프 계획 성능이 약 20% 감소함을 보여줍니다. 이러한 성과는 생성적 세계 모델에서의 보안 취약점을 정량적으로 평가하고, 이후 작업에 실질적인 위험을 나타냅니다.
...
- **What's New**: 이번 논문은 Large Language Models (LLMs)에서의 훈련 데이터 유출 문제를 다루고 있습니다. 저자들은 기존의 memorization 측정 방법들이 통계적으로 일반적인 시퀀스와 진정한 암기를 혼동하는 문제를 지적합니다. 이를 해결하기 위해 Prior-Aware Memorization이라는 새로운 기준을 제안하여 훈련 시퀀스와의 강한 연관성을 평가하는 방식을 채택하고 있습니다.

- **Technical Details**: Prior-Aware Memorization(PA memorization)은 추가적인 모델 훈련 없이도 작동하며, 통계적 일반성과 실제 암기를 명확히 구분합니다. 저자들은 두 개의 사전 훈련된 모델인 LLaMA와 OPT를 사용하여 실험을 진행하고, 긴 시퀀스 및 명명된 개체를 통해 개인 식별 정보(PII) 유출 위험을 모의합니다. 이를 통해 최대 90%의 훈련 데이터로부터 이전에 암기된 것으로 분류된 시퀀스가 실제로는 통계적으로 일반적임을 밝혀냈습니다.

- **Performance Highlights**: 연구 결과, SATML 훈련 데이터 추출 도전 세트에서도 약 40%의 시퀀스가 일반적인 패턴 행동을 보인 것으로 나타났습니다. 저자들은 이를 통해 저빈도 시퀀스가 암기의 증거로 부족하다는 점을 강조하며, LLM의 암기 평가 시 모델의 사전 지식을 충분히 고려해야 함을 주장합니다.
...
Comments:
          Project Page and Code at: this https URL

- **What's New**: 본 논문에서는 비연속적인 모델에서 생성 순서를 조정하기 위한 새로운 방법인 LFlexMDM(FlexMDM with Learnable Order Dynamics)를 제안합니다. 이는 삽입 기반의 변동 길이 생성 모델로, 시뮬레이션 없이도 생성기와 목표 순서 동적을 동시에 학습할 수 있도록 설계되었습니다. FlexMDM의 장점을 유지하면서도 학습의 효율성을 높이고 유효한 샘플의 수를 증가시킵니다.

- **Technical Details**: LFlexMDM은 생성 과정에서 데이터 샘플에 의존하는 삽입 및 비마스킹 비율을 학습 가능한 매개변수로 통합하여 생성 순서를 제어할 수 있습니다. 이 과정에서 일반적인 수치 해석을 요구하지 않으며, 단일 자동 미분 패스를 통해 효과적으로 학습합니다. 또한, 비율 측정에 사용되는 목표 함수는 Bregman divergence를 최소화하는 원리에 기반합니다.

- **Performance Highlights**: 실험 결과, LFlexMDM 모델을 사용한 경우 깔끔하고 유효한 분자의 수가 증가하고 생성 품질이 향상되었음을 확인했습니다. 이는 기존의 FlexMDM과 비교할 때 많은 이점을 제공하며, 특히 화합물 생성 작업에서 두드러진 성과를 보였습니다. 이러한 결과는 설계된 동적 순서 학습이 실제 응용 가능성을 높임을 시사합니다.
...
### Transformers for dynamical systems learn transfer operators in-contex (https://arxiv.org/abs/2602.18679)
Comments:
          6 pages, 3 figures

- **What's New**: 이번 연구에서는 물리적 설정에서 관찰되지 않은 상태에 적응하는 대규모 기초 모델을 다루었으며, 특히 동적 시스템의 컨텍스트 내 학습(in-context learning)을 소규모로 연구했습니다. 저자들은 두 개의 층을 가진 트랜스포머 모델이 다른 동적 시스템을 예측할 수 있는 능력을 평가하여, 학습과정에서의 성능 간의 상반된 트레이드오프를 발견했습니다. 이로 인해 대규모 사전 훈련 모델이 재훈련 없이도 테스트 단계에서 물리적 시스템을 예측할 수 있는 메커니즘을 명확히 했습니다.

- **Technical Details**: 연구에서는 V=100인 분산된 어휘를 사용하는 간결한 인과 언어 모델을 구현하였습니다. 모델 구조는 GPT 계열과 유사한 디코더 전용 트랜스포머로, 각 입력 토큰은 더 높은 차원의 임베딩으로 매핑되어, 총 dmodel=256 차원입니다. 두 개의 동일한 트랜스포머 블록을 사용하며, 각 블록은 단일 헤드 인과적 자기 주의(attention)와 LayerNorm을 적용하여, 모델의 안정성과 성능을 유지합니다.

- **Performance Highlights**: 결과적으로 주목(attention) 기반 모델은 낮은 차원의 시계열 데이터를 바로 강화할 수 있는 전이 연산자 예측 전략을 적용합니다. 이들은 시스템의 높은 차원의 동적 다양체를 탐지하고 식별하기 위해 지연 삽입(delay embedding)을 사용하며, 전세계 흐름을 특징 짓는 장기 불변 집합을 예측합니다. 저자들은 이러한 방식이 단기 예측을 위해 전지구 매력 정보(global attractor information)를 활용하는 독창적인 능력을 부각시켰습니다.
...
- **What's New**: 이번 논문에서는 매개변수화된 네트워크(parameterized networks)의 무작위 입력 섭동(random input perturbations)에 대한 복원력을 이론적으로 연구하였습니다. 특히, 절편 추가적(random additive) 섭동이 특정 입력에서 오분류(misclassification)를 유도할 확률을 계량화하여 지역적 안정성(local robustness)을 분석합니다. 레쿠티파이드 선형 유닛(deep networks with rectified linear units)을 갖는 네트워크를 중심으로 하였으며, 입력 차원(input dimensionality)과 네트워크 유닛의 총 개수(total number of units)에 따른 하한(lower bounds)을 도출했습니다.

- **Technical Details**: 이 연구에서는 입력 xx에 대한 지역적 안정성(local robustness)을 확립하기 위해, 입력에 일정한 크기 r의 추가적 섭동을 가했을 때 네트워크가 오분류할 확률을 정의합니다. 저자들은 이러한 영역을 다루기 위해 깊은 ReLU 네트워크(deep ReLU networks)의 입력 차원에 따른 확률적 하한을 도출하고, 입력 벡터의 차원이 증가함에 따라 지역적 안정성이 급격히 증가하는 성질을 보였습니다. 또한, 이론적 분석은 깊은 ReLU 네트워크의 입력-출력 함수의 기하학적 복잡성(geometric complexity) 특성을 바탕으로 하여 이루어졌습니다.

- **Performance Highlights**: 결과적으로, 저자들은 네트워크의 깊이와 무작위 촘촘하게 채워진 패턴의 존재 가능성을 연구하면서, 높은 차원에서 가벼운 섭동 만으로도 명확한 오분류가 발생할 가능성이 낮음을 발견하였습니다. 이는 무작위 테스트(random testing) 하에서도 적대적 예제(adversarial examples)를 찾기 어려움을 시사합니다. 또한, 이를 통해 적대적 패턴이 존재할 가능성이 있는 입력의 지역적 이웃(local neighborhoods)을 식별할 수 있는 가능성을 제시하였습니다.



### Large Causal Models for Temporal Causal Discovery (https://arxiv.org/abs/2602.18662)
Comments:
          32 pages (16 main text, 16 Appendix), 11 Figures

- **What's New**: 이번 연구에서는 대규모 인과 모델(LCM)을 제안합니다. LCM은 다양한 다중 데이터 세트에 대한 사전 훈련이 가능하며, 효율적인 시계열 인과 추론을 할 수 있도록 설계된 신경망 모델입니다. 기존의 작은 변수에서의 한계를 극복하고, 큰 변수를 다룰 수 있는 가능성을 제시합니다.

- **Technical Details**: LCM의 구조적 인과 모델(SCM)은 내적 변수를 인과 부모와 외적 노이즈 항의 함수로 표현합니다. 이 연구에서는 최대 인과 지연 ℓmax를 가지는 다차원 시계열 데이터에서 중요한 변수를 정의하고, 이들 간의 인과 관계를 지연 인과 그래프로 나타냅니다. 또한, 데이터 다양성을 강조하며 이를 바탕으로 LCM의 훈련을 진행했습니다.

- **Performance Highlights**: 대규모 실험 결과, LCM은 고차원 및 깊은 구조에서도 강력한 성능을 유지하며, 기존의 고전적인 방법 및 신경망 기법보다 경쟁력 있는 정확도를 달성했습니다. 특히, zero-shot 설정에서도 강력한 일반화를 보여주어 LCM이 시계열 인과 발견의 기초 모델 패러다임으로서의 가능성을 보여줍니다.



### Communication-Efficient Personalized Adaptation via Federated-Local Model Merging (https://arxiv.org/abs/2602.18658)
- **What's New**: 이 논문에서는 Potara라는 새로운 프레임워크를 제안하여, 클라이언트 각자의 개인화된 모델을 생성하기 위해 일반 지식을 포착하는 연합 모델과 개인화된 지식을 포착하는 로컬 모델을 병합합니다. 이 방식은 클라이언트 간의 지식 균형을 이룰 수 있도록 하며, 기존의 접근 방식들이 이론적 근거가 부족한 점을 보완하였습니다.

- **Technical Details**: Potara는 선형 모드 연결(linear mode connectivity) 이론에 기초하여 클라이언트 모델을 구성합니다. 이를 통해 예상되는 작업 손실의 분산 추적 상한을 설정하고, 이를 최소화하여 최적의 혼합 가중치를 도출합니다. 이 방법은 연합 모델과 로컬 모델 각각보다 병합된 모델에서 더 엄격한 경계를 보장합니다.

- **Performance Highlights**: 실험 결과, Potara는 기존의 방법들과 비교하여 개인화 성능이 일관되게 향상되었으며, 통신 비용을 크게 줄였습니다. 이는 성능과 통신 효율 간의 만족스러운 균형을 제공하여 연합 학습(federated learning) 상황에서의 실용성을 높입니다.
...
- **What's New**: 이번 연구에서는 기존의 시간 시계열(時間時系列) 추론(task) 접근법의 한계를 극복하기 위해, ARTIST라는 새로운 모델을 소개합니다. ARTIST는 시간 시계열 추론을 순차적 의사결정 문제로 구조화했으며, 적응형(Adaptive) 시계열 세그먼트 선택을 통해 분석을 수행합니다. 이 모델은 단순히 전체 시퀀스를 고정된 표현으로 인코딩하는 대신, 특정 세그먼트를 선택해 작업과 연관된 정보를 수집하여 결론을 도출합니다.

- **Technical Details**: ARTIST는 컨트롤러-리존너(controller-reasoner) 아키텍처를 채택하고, 강화 학습(reinforcement learning)을 통해 두 가지 역할을 최적화합니다. 컨트롤러는 유용한 세그먼트를 선택하고, 리존너는 선택된 세그먼트에 기초하여 추론(traces) 및 최종 답변을 생성합니다. 이러한 방식으로 모델은 전체 시퀀스의 정적(summary) 요약이 아닌, 작업 관련 정보를 능동적으로 수집합니다.

- **Performance Highlights**: ARTIST는 여섯 가지 시간 시계열 추론 벤치마크에서 평가되었으며, 대형 언어 모델(large language models) 및 이전의 시계열 추론 시스템과 비교하였습니다. 강화된 성능은 평균 정확도를 6.46%포인트 높였으며, 드문 사건(localization) 탐지 및 다중 세그먼트 추론(multi-segment reasoning) 작업에서 가장 두드러진 증가를 보였습니다. 감독된 세밀 조정(supervised fine-tuning) 또한 성능을 향상시켰고, 강화 학습은 적응형 세그먼트 선택을 최적화하여 추가적인 성과를 제공했습니다.
...
- **What's New**: 이번 연구에서는 비시뮬레이션 인코더(bisimulation encoder)를 도입하여 시각적 세계 모델의 성능을 향상시켰습니다. 기존의 DINO 세계 모델(DINO-WM)이 수행하는 느리게 변화하는 시각적 특징에 대한 민감성을 해결하기 위해, 이 새로운 접근 방식은 제어와 관련된 상태 동등성을 교수하여 이러한 문제점을 완화합니다. 새로운 비시뮬레이션 기반 모델은 기존의 느린 특징에 대한 견고성을 향상시키면서도 DINO-WM보다 최대 10배 작은 잠재 공간(latent space)에서 운영됩니다.

- **Technical Details**: 비시뮬레이션 메트릭을 활용하여 사전 훈련된 시각적 특징 추출기(예: DINOv2, SimDINOv2, iBOT) 위에 불변 표현을 학습하는 방법론을 개발했습니다. 이 메트릭은 즉각적인 보상과 전환 동역학을 기반으로 상태 동등성을 정의하며, 이를 통해 제어와 관련 없는 시각적 정보를 제거하도록 설계되었습니다. 본 연구는 비시뮬레이션 정규화기를 통해 관련 없는 특징을 배제하고, 제어에 직접 영향을 미치는 상태 정보만을 효과적으로 학습할 수 있습니다.

- **Performance Highlights**: 제안한 모델은 다양한 배경 변화 및 시각적 방해물 아래에서 내비게이션 작업에 대한 강인한 계획 성능을 입증했습니다. 세 가지 서로 다른 사전 훈련된 시각적 인코더를 사용하여 밀접한 비교를 통해 일관된 성능 향상을 확인하였으며, DINO-WM과 비교하여 느린 특징에 대한 강인함을 유지하는 것을 보여주었습니다. 또한, 명시적인 보상 예측 없이도 불변 세계 모델 표현을 학습할 수 있었으며, 이는 훈련 과정을 단순화합니다.
...
- **What's New**: 본 연구는 비침습적인 방법으로 쥐의 전두엽에서 아사이닉 EEG(EEG)를 통해 자율적으로 설정된 보행 속도를 연속적으로 디코딩하는 것을 목표로 합니다. 기존 연구들은 외부에서 주어진 속도에 대해서만 디코딩이 이루어졌으나, 본 연구는 이러한 한계를 극복하려고 합니다. 저자들은 133시간 이상의 EEG 데이터를 활용하여 깨달은 바, 자율적으로 행해지는 보행 속도는 정확하게 디코딩 가능하다는 것입니다.

- **Technical Details**: 제안된 방법은 32개의 전극으로 구성된 EEG를 사용하여 비모터 구동의 트레드밀에서 쥐의 운동 속도를 디코딩하는 비동기식 뇌-컴퓨터 인터페이스(BCI)를 개발하는 것입니다. 재발 신경망을 활용하여 EEG 데이터를 현재의 속도에 매핑하는 디코더를 훈련시켰고, 실험적 조사는 14마리의 쥐를 대상으로 하였습니다. 각 쥐에서 세션마다 일관된 신경 서명을 발견했으며, 이를 통해 세션 간 일반화가 가능함을 보여주었습니다.

- **Performance Highlights**: 디코딩 성능은 0.88의 상관계수를 기록하며, 이는 주로 시각 피질 전극과 저주파(8Hz 이하) 진동에 의해 주도되었습니다. 또한, 향후 및 과거의 동역학에 대한 정보도 전달하는 경향을 보였으며, 이는 약 1000ms까지 연장됩니다. 이러한 발견은 자율적으로 선택된 보행 속도를 정확하면서도 지속적으로 비침습적으로 디코딩할 수 있음을 입증하고, 향후 고성능 BCI 시스템 개발에 기여할 것입니다.
...
- **What's New**: 이 연구에서는 인공 일반 지능(AGI)을 향상시키기 위해 다중 모드 대형 언어 모델(MLLMs)의 체계적인 평가의 필요성을 강조합니다. 이를 위해, MLLMs의 제한된 추론 능력을 평가하기 위한 새로운 벤치마크인 MapTab을 소개합니다. MapTab은 경로 계획 작업을 통해 MLLMs의 다중 모드 추론 능력을 검증하는 데 중점을 둡니다.

- **Technical Details**: MapTab 벤치마크는 MLLMs가 지도 이미지에서 시각적 단서(visual cues)를 인식하고, 구조화된 테이블 데이터(table data)에서 경로 속성(예: 시간, 가격)과 결합하여 판단하도록 요구합니다. 이 벤치마크는 두 가지 시나리오를 포함하므로, 메트로 네트워크를 다루는 Metromap과 대표적인 관광지를 나타내는 Travelmap으로 구성됩니다. 총 328개의 이미지와 196,800개의 경로 계획 질의, 3,936개의 QA 질의가 포함되어 있으며, 4개의 주요 제약 조건(Time, Price, Comfort, Reliability)을 갖추고 있습니다.

- **Performance Highlights**: 15개의 대표 MLLMs에 대한 광범위한 평가에서 현재 모델들이 제한된 다중 모드 추론에서 상당한 어려움을 직면하고 있음이 밝혀졌습니다. 특히, 제한된 시각적 인식 환경에서 다중 모드 협력이 단일 모드 접근법(unimodal approaches)보다 성능이 저조하다는 결과가 나왔습니다. 이는 MLLMs의 체계적인 평가를 위한 도전적이고 현실적인 테스트베드(testbed)를 제공한다는 점에서 중요합니다.



### Ensemble Prediction of Task Affinity for Efficient Multi-Task Learning (https://arxiv.org/abs/2602.18591)
- **What's New**: 이번 논문에서는 다중 작업 학습(Multi-task Learning, MTL)에서 작업 그룹을 효과적으로 식별하는 문제를 해결하는 ETAP (Ensemble Task Affinity Predictor)라는 새로운 프레임워크를 제안합니다. 이 모델은 작업 간의 상호작용을 예측하여 함께 학습했을 때의 성능 향상을 측정합니다. ETAP는 다양한 작업 세트를 다루기 위해 데이터 기반(estimator) 방법과 원칙 기반(principled) 방법을 통합합니다.

- **Technical Details**: ETAP는 두 가지 기본 요소로 구성됩니다. 첫째, MTL 모델의 공통 파라미터에 대한 기울기 기반 업데이트를 고려하여 작업 간의 친화도(affinity)를 평가하고, 이를 통해 affinity score를 도출합니다. 둘째, 비선형 변환(non-linear transformations)을 적용하여 잔차 오류(residual errors)를 수정하고, 복잡한 작업 관계를 더 잘 포착할 수 있도록 예측기를 학습시킵니다.

- **Performance Highlights**: ETAP는 여러 벤치마크 데이터셋에서 MTL 성능 예측을 개선하고, 더 효과적인 작업 그룹핑(task grouping)을 가능하게 함을 보여주었습니다. 다양한 응용 분야에서 기존의 최첨단 방법들보다 뛰어난 성능을 발휘하여, 실용적인 다중 작업 학습 기술의 발전에 기여하고 있습니다.
...
- **What's New**: 이번 논문에서는 명시적인 목표 없이 학습하는 동적 프레임워크를 제안합니다. 기존의 머신 러닝이 외부 오류 신호를 최소화하는 방식에서 벗어나, 시스템이 스스로의 내부 역학을 평가하여 구조 변화(변경)를 조절하는 방법에 초점을 맞추고 있습니다. 이는 진정한 자율성을 향한 중요한 발걸음으로, 바람직한 자기 평가 및 내부 조정을 가능하게 합니다.

- **Technical Details**: 이 연구는 빠른 상태 변화와 느린 구조 적응을 구분하는 최소한의 두 가지 시간 척도(dynamical framework)를 개발합니다. 시스템은 이론적으로 사고 과정에 대한 자체 평가를 진행할 수 있으며, 이를 통해 탐색 부족 또는 비효율적인 구조의 두 가지 실패 모드를 구분할 수 있습니다. 또한, 내재된 인지 스트레스 필드(Cognitive Stress Field)를 도입하여 사고 품질을 시간적으로 통합된 정보를 바탕으로 평가하고, 구조적 수정(구성 변화)을 조절할 수 있습니다.

- **Performance Highlights**: 연구 결과, 이 스트레스 조절 메커니즘을 통해 외부 목표에 의존하지 않고도 자가 조직화된 학습 에피소드를 생성할 수 있음을 보여주었습니다. 또한, 추상적 목표 없이도 사고의 품질을 평가할 수 있는 기준을 제시하며, 성공적인 사고는 동적 특성을 통해 정의된다는 점에서 의미가 있습니다. 이는 자율 학습 시스템 개발에 중요한 기반을 마련할 것으로 보입니다.



### Sub-City Real Estate Price Index Forecasting at Weekly Horizons Using Satellite Radar and News Sentimen (https://arxiv.org/abs/2602.18572)
- **What's New**: 이 논문은 위성 레이더 데이터와 뉴스 텍스트의 시장 내러티브를 결합하여 주간 빈도로 하위 도시 가격 지수를 예측할 수 있는 가능성을 탐구합니다. 두바이 토지 당국의 35만 건이 넘는 거래 데이터를 활용하여 19개의 하위 도시 지역에 대한 주간 가격 지수를 구축했습니다. 결과적으로, 정서(sentiment)와 SAR(합성 개구 레이더) 데이터가 가격 예측의 결정적인 요소임을 확립하였습니다.

- **Technical Details**: 이 연구는 지역 거래 이력, GDELT로부터 얻은 뉴스 정서, Sentinel-1 SAR 후방산란 데이터 및 금리와 같은 거시경제 지표를 통합하는 4가지 신호 유형을 활용합니다. 텍스트 모달리티에서는 단순한 긍정-부정 극성을 넘어 주제 구조를 포착하기 위해 렉시컬 톤 점수와 의미적 임베딩을 결합합니다. 이 방법론은 신뢰성 있는 예측을 위한 다중 모달 기여도를 평가할 수 있는 기회를 제공합니다.

- **Performance Highlights**: 평가 결과, 10주까지는 거래 이력만으로도 만족스러운 성과를 보였으나, 14주 이후에는 정서와 SAR 데이터가 예측의 정확성을 향상시키고, 전체 다중 모달 모델이 평균 절대 오차를 35% 감소시켰습니다. 비모수 학습기인 K-최근접 이웃과 랜덤 포레스트가 리니어 모델 및 순환 신경망보다 성능이 우수함을 보여주었습니다. 이 발견은 하위 도시 가격 지수 예측의 벤치마크를 확립하고 원거리 탐지 및 뉴스 정서의 유용성을 입증합니다.
...
- **What's New**: 본 연구에서는 Test-Time Adaptation (TTA) 분야에서 Audio-Visual Continual Test-Time Adaptation (AV-CTTA)이라는 새로운 방법론을 소개합니다. 이전 방법들은 테스트 데이터에 대해 지속적으로 매개변수를 업데이트할 때 성능 저하를 경험하는데, 특히 Catastrophic Forgetting 문제로 인해 소스 모델의 성능이 크게 감소합니다. 본 연구는 모달리티 퓨전 레이어를 타겟 도메인에만 적응시키는 방법이 성능을 향상시킬 수 있음을 보여줍니다.

- **Technical Details**: 우리는 AV-CTTA 방법이 특정 버퍼에서 최적의 퓨전 레이어 매개변수를 동적으로 검색하고 통합하여 테스트 분포에 적응할 수 있도록 설계되어 있음을 강조합니다. 이는 제한된 테스트 데이터 배치만을 사용하여 작업을 수행합니다. 이 과정에서 샘플 간의 통계 정보를 활용하여 최적의 매개변수를 선택하고, 이러한 방식을 통해 Catastrophic Forgetting을 최소화합니다.

- **Performance Highlights**: 대규모 실험을 통해 AV-CTTA 방법이 기존의 AV-TTA 방법들에 비해 성능이 유의미하게 개선되었음을 입증하였습니다. 예를 들어, VGGSound-2C 같은 어려운 테스트 세트에서는 READ의 경우 27.9%의 성능 저하가 발생한 반면, 우리의 방법은 단지 2.9%의 성능 저하만을 기록하였습니다. 일반적으로 AV-CTTA는 음성-시각 CTTA 수행에 있어 모든 기준선 방법들을 뛰어넘는 성능을 보여주었습니다.
...
- **What's New**: 이 논문은 사용자 생성 콘텐츠 플랫폼의 안전성을 향상시키기 위해 사용자 경험을 반영하는 새로운 메트릭, 즉 '프리밸런스(prevalence)' 측정을 도입합니다. 기존의 보고서 기반의 접근 방식은 많은 해로운 콘텐츠가 보고되지 않거나 드물게 발생하는 문제를 해결하기 위해, 매일의 사용자 노출을 분석하여 정책을 위반하는 콘텐츠의 비율을 추정합니다. 머신러닝(Machine Learning)과 대규모 언어 모델(LLM)을 활용한 이 시스템은 데이터 수집과 라벨링의 효율성을 크게 향상시킵니다.

- **Technical Details**: 제안된 시스템은 매일 사용자 인상 데이터를 기반으로 샘플링을 수행하며, 고위험 콘텐츠를 집중적으로 라벨링하는 방식으로 운영됩니다. 프리밸런스 추정치는 통계적 추정기(statistical estimator)와 신뢰 구간(confidence intervals)을 통해 정확도를 보장하며, 설계 기반의 샘플링을 통해 다양한 세그먼트에서 결과를 조정할 수 있습니다. 특히, 이 시스템은 정책 정의와 SME(주제 전문가) 검토 프롬프트를 활용하여 일관된 측정 파이프라인을 구축할 수 있도록 설계되었습니다.

- **Performance Highlights**: 시스템은 다양한 정책 영역에서 매일 95% 신뢰 구간(CI)을 포함한 프리밸런스 추정치를 제공하며, 이를 통해 사용자 경험의 변화를 실시간으로 모니터링할 수 있습니다. 생산 환경에서 1년 이상 운영된 이 시스템은 안전성 관련 목표 추적과 자동 경고 기능을 지원하며, 이로 인해 보고서 기반 메트릭에 비해 더욱 유용한 인사이트를 제공합니다. 프리밸런스 측정치는 사용자 보고와의 상관관계가 낮아 추가적인 가시성을 제공하며, 정책에 대한 즉각적인 피드백을 가능하게 합니다.
...
- **What's New**: 이번 논문에서는 기존의 레이더 탐지 기법의 한계를 극복하기 위해 Support Vector Data Description (SVDD)와 그 심화 버전인 Deep SVDD를 활용한 새로운 탐지 알고리즘을 제안합니다. 기존의 방법들이 Gaussian 환경에서는 효과적이나 clutter가 존재할 경우 성능이 저하되는 문제를 다루고 있습니다. SVDD는 직접적인 노이즈 공분산 추정을 필요로 하지 않으며, CFAR 탐지기로서 조정되었습니다.

- **Technical Details**: 저자는 이 논문에서 SVDD와 Deep SVDD 알고리즘을 포함하여 두 가지 새로운 탐지 알고리즘을 제안합니다. SVDD는 고차원 데이터의 최소 볼륨 초구를 찾아 감지 문제를 해결하는 데 초점을 맞춥니다. 이러한 방법들은 전통적인 Gaussian 기반 방법들과는 다른 접근 방식을 제공하며, 더 복잡한 비-Gaussian 환경에서도 신뢰할 수 있는 성능을 발휘할 수 있습니다.

- **Performance Highlights**: 제안된 알고리즘의 효과를 검증하기 위해 시뮬레이션된 레이더 데이터에서 성능을 비교하였고, 기존의 최첨단 탐지 방식과의 비교 또한 수행되었습니다. SVDD 및 Deep SVDD는 다양한 환경에서 우수한 탐지 성능을 나타내었다고 보고되었으며, 이는 복합적인 쿨터와 열 노이즈가 동시에 존재하는 상황에서도 효과적인 결과를 보여줍니다.
...
- **What's New**: 본 논문은 다변량 시계열 예측에서 오류를 최소화하기 위해 시계열의 트렌드(trend) 및 계절성(seasonal) 요소 각각을 분리하여 다룬다는 점에서 새로운 접근 방식을 제안합니다. 기존의 SOTA(state-of-the-art) 모델보다 더 높은 효율성을 달성하기 위해 신경망 아키텍처를 간소화하고, 두 개의 새로운 dual-MLP 모델을 도입하여 계산 효율성을 높였습니다. 이 방법은 4개의 주요 기준 모델에 대해 평균 10% MSE(mean squared error) 감소를 달성하며, 실제 환경 데이터에서도 효과적으로 성능을 입증합니다.

- **Technical Details**: 제안된 방법은 입력 신호를 트렌드와 시즌으로 분리하여 각각의 신호를 특수화된 네트워크를 통해 예측한 후 최종 예측값을 재결합합니다. Moving Average, Mixture of Experts(MoE) 및 Frequency-Based Decomposition과 같은 세 가지 분해 방법을 탐구하며, 이동 평균 기법이 가장 간단하고 빠른 O(L) 복잡성을 가진다고 강조합니다. 예측된 트렌드와 시즌 요소는 RevIN(Reversible Instance Normalization) 및 MLP(Multilayer Perceptron)를 이용해 각각 처리되며, 계절성 요소는 기존의 모델에서 정규화 과정을 제거하고 경량화된 대안 모델로 처리하여(predict) 예측 성능을 개선합니다.

- **Performance Highlights**: 제안한 방법은 USGS의 수문학적 데이터셋을 포함한 여러 벤치마크 데이터셋에서 평균적으로 10%의 MSE 감소를 달성했습니다. 또한, 계산 효율성을 높이기 위한 경량 백본(backbone) 네트워크를 통해 기존의 복잡한 아키텍처들보다 우수한 성능을 보였으며, 실제 환경에서의 유용성을 입증하였습니다. 이를 통해 필자는 다변량 시계열 예측 분야에서 비용 효율적인 솔루션을 제공하고, 지속적으로 다양한 도메인에서 성능이 개선될 수 있음을 보여줍니다.
...
- **What's New**: LLM 에이전트들이 코드 실행, 도구(tool), 그리고 최근 도입된 에이전트 스킬(agent skills) 기능으로 인해 빠르게 발전하고 있습니다. 스킬을 통해 사용자들은 LLM 애플리케이션을 전문화된 서드파티 코드, 지식, 및 지침으로 확장할 수 있으며, 이는 새로운 도메인에서의 에이전트의 가능성을 넓혀줍니다. 하지만 이러한 확장은 점점 더 복잡한 에이전트 공급망을 만들어내며, 프롬프트 주입 공격(prompt injection attacks)에 대한 새로운 취약성을 노출합니다.

- **Technical Details**: 우리는 스킬 기반 프롬프트 주입이 중요한 위협으로 인식하고, SkillInject라는 벤치마크를 도입하여 널리 사용되는 LLM 에이전트의 스킬 파일을 통한 주입에 대한 취약성을 평가합니다. SkillInject는 명백히 악의적인 주입에서부터 잘 보이지 않는, 문맥에 의존적인 공격에 이르는 202개의 주입-작업 쌍(injection-task pairs)을 포함하고 있습니다. 우리는 최전선 LLM들을 SkillInject로 평가하여 유해한 지침 회피와 합법적인 지침 준수의 측면에서 보안을 측정했습니다.

- **Performance Highlights**: 우리의 결과는 오늘날 에이전트들이 매우 취약하다는 것을 보여주며, 최전선 모델의 경우 공격 성공률이 80%에 달합니다. 이들은 종종 데이터 유출(data exfiltration), 파괴적 행동(destructive action), 그리고 랜섬웨어와 유사한 행동을 포함한 극도로 해로운 지시를 실행합니다. 이러한 문제는 모델 확장(model scaling)이나 간단한 입력 필터링으로 해결되지 않을 것이며, 강력한 에이전트 보안을 위해서는 문맥 인식 권한 부여 프레임워크(context-aware authorization frameworks)가 필요하다는 것을 시사합니다.



### JUCAL: Jointly Calibrating Aleatoric and Epistemic Uncertainty in Classification Tasks (https://arxiv.org/abs/2602.20153)
Comments:
          11 pages + appendix. Preliminary version of an ongoing project that will be expanded with furhter evaluations

- **What's New**: 이 논문에서는 훈련된 분류기의 앙상블에 대한 포스트-캘리브레이션 불확실성을 연구합니다. 특히, 데이터 라벨 노이즈가 포함된 aleatoric(알레아토릭) 불확실성과 모델에 의한 epistemic(에피스탬릭) 불확실성을 모두 고려합니다. 기존의 캘리브레이션 기법들이 이 두 불확실성을 균형 있게 조정하지 못하는 단점을 해결하기 위해 제안된 Joint Uncertainty Calibration (JUCAL) 알고리즘은 적대적인 환경에서도 효과적으로 작동합니다.

- **Technical Details**: JUCAL은 aleatoric과 epistemic 불확실성을 각각 가중치와 규모를 조정하는 두 상수를 최적화하여 동시에 캘리브레이션할 수 있도록 설계되었습니다. 이 방법은 validation/calibration 데이터셋에서의 negative log-likelihood(NLL)를 최적화하여 수행됩니다. JUCAL은 다양한 classifier(분류기)에 적용 가능하며, 모델의 내부 파라미터에 대한 접근 없이도 최소한의 계산 자원으로 실행할 수 있습니다.

- **Performance Highlights**: 실험 결과, JUCAL은 다양한 텍스트 분류 과제에서 SOTA 캘리브레이션 방법보다 상당히 우수한 성능을 보였습니다. NLL과 예측 세트 크기를 각각 최대 15% 및 20% 줄이는 결과를 보여 최적의 앙상블 모델의 추론 비용을 약 10배 줄일 수 있었습니다. JUCAL은 다양한 분류 작업에서 신뢰할 수 있는 캘리브레이션을 제공하는 강력한 도구로 자리 잡을 것으로 기대됩니다.



### Conformal Risk Control for Non-Monotonic Losses (https://arxiv.org/abs/2602.20151)
- **What's New**: 이번 논문에서는 비모노토닉 손실(non-monotonic loss)에 대한 리스크 제어 보장(risk control guarantee)을 제시합니다. 기존의 conformal prediction 알고리즘은 1차원 매개변수(monodimensional parameter)에서 손실이 단조적(monotonic)일 때를 다루었으나, 본 연구는 다차원 매개변수를 가진 다양한 알고리즘에 적용할 수 있는 안정성(stability) 보장을 제공합니다. 본 논문은 이미지 분류(selective image classification), 종양 분할(tumor segmentation)의 FDR 및 IOU 제어, 및 인종과 성별 그룹의 중복된 재범 예측 다중 그룹 디바이싱(multigroup debiasing)에 대한 응용 사례를 포함합니다.

- **Technical Details**: 리스크 제어 보장은 불안정한 알고리즘(unstable algorithm)일수록 느슨한 보장을 제공합니다. 본 연구에서는 데이터 교환 가능성(exchangeability) 및 대칭성(symmetry)을 가정하며, 알고리즘이 참조 알고리즘(reference algorithm)과 β-안정적(β-stable)하다는 것을 바탕으로 매개변수 θ의 안정성과 관련하여 주요 정리를 증명합니다. 또한, 본 논문에서는 리스크를 제어할 수 있는 전략을 제시하며, 전체 데이터셋을 사용할 때의 참조 알고리즘을 찾아서 이를 근사화(approximate)하는 방법을 설명합니다.

- **Performance Highlights**: 본 연구의 주요 성과는 비모노토닉 손실에 대해 효과적인 리스크 제어를 보장하는 알고리즘을 제시한 점입니다. θ의 안정성을 기반으로 하여, 알고리즘이 다른 대칭적인 알고리즘에 비해 우수한 성능을 보임을 확인하였습니다. 또한, 전통적인 conformal risk control 보장을 복구하는 데 성공하였으며, 이로 인해 새로운 비모노토닉 상황에서도 리스크 제어가 가능함을 입증하였습니다.
...
### Multivariate time-series forecasting of ASTRI-Horn monitoring data: A Normal Behavior Mod (https://arxiv.org/abs/2602.19984)
Comments:
          15 pages, 12 figures

- **What's New**: 이 연구는 ASTRI-Horn Cherenkov 망원경의 정상 운영 조건에서 모니터링 시계열 데이터를 예측하기 위해 개발된 Normal Behavior Model (NBM)을 소개합니다. 연구는 2022년 9월부터 2024년 7월까지 망원경 제어 장치에서 수집된 15가지 물리적 변수에 중점을 두었습니다. 이 모델은 센서 측정값을 기반으로 하여 자료를 정제하고 다시 샘플링하여 피처 선택 및 상관 분석을 진행했습니다.

- **Technical Details**: 데이터셋은 고정된 길이 구간으로 나누어졌으며, 첫 번째 I 샘플은 모델에 제공된 입력 시퀀스를 나타냅니다. 예측 길이 T는 예측해야 할 미래 시간 단계를 나타냅니다. 슬라이딩 윈도우(슬라이딩 윈도우) 기법이 적용되어 간격 수를 증가시켰고, 여러 피처에 대해 동시에 다변량 예측을 수행하기 위해 Multi-Layer Perceptron (MLP) 모델이 훈련되었습니다.

- **Performance Highlights**: 모델 성능은 Mean Squared Error (MSE)와 Normalized Median Absolute Deviation (NMAD)을 사용하여 평가되었으며, Long Short-Term Memory (LSTM) 네트워크에 대한 벤치마크도 진행했습니다. MLP 모델은 다양한 피처와 I-T 구성에서 일관된 결과를 보여주었으며, LSTM과 유사한 성능을 보이면서 더 빨리 수렴했습니다. 최적 구성에서 MSE는 0.019+/-0.003, NMAD는 0.032+/-0.009을 기록하였고, 예측 기간을 6.5시간까지 연장해도 성능 저하가 없었습니다.
...
### RobPI: Robust Private Inference against Malicious Clien (https://arxiv.org/abs/2602.19918)
Comments:
          Accepted by SaTML 2026

- **What's New**: 본 논문은 private inference (PI) 프로토콜의 보안을 강화하기 위해 새로운 공격 기법인 PI-ATTACK을 소개하고, 이를 바탕으로 RobPI라는 견고한 private inference 프로토콜을 제안합니다. 기존의 semi-honest 모델 가정을 벗어나 고객이 악의적인 행동을 할 수 있는 상황을 고려했습니다. RobPI는 보안성을 증대시키기 위해 암호화 가능한 노이즈를 모델의 logits과 feature에 통합하여 공격을 저지합니다.

- **Technical Details**: PI-ATTACK은 기존의 공격 기법보다 3배에서 8배 더 적은 쿼리로 모델 출력을 조작할 수 있게 설계되었습니다. RobPI는 이를 방어하기 위해 복잡한 암호화 프로토콜을 사용하며, 동적 노이즈 훈련 기법(DNT)을 도입하여 악의적인 고객에게 대한 저항력을 더욱 개선했습니다. FHE(완전 동형 암호화)를 바탕으로 하여 비대면 모델 추론의 안전성을 높였습니다.

- **Performance Highlights**: RobPI는 여러 신경망과 데이터 세트에 대한 실험에서 약 91.9%의 공격 성공률 감소와 함께 악의적인 공격에 필요로 하는 쿼리 수를 10배 이상 증가시켰습니다. 이는 RobPI가 현재의 방어 방법들과 비교할 때 공격의 난이도를 크게 높인다는 것을 보여줍니다. 이를 통해 private inference 분야의 보안을 한층 더 강화할 수 있는 가능성을 제시합니다.
...
Comments:
          5 pages, 5 figures, Final version accepted to the 59th Asilomar Conference on Signals, Systems, and Computers (2025)

- **What's New**: 이 논문은 인과 발견(causal discovery) 방법이 신호 처리(signal processing)의 관점에서 시간 간격(time interval)과 윈도우 길이(window length) 변화에 따라 어떻게 민감한지 연구합니다. 기존의 인과 발견 기법들이 이러한 하이퍼파라미터에 익숙하지 않은 경우가 많음을 지적하며, 이들 방법의 성능에 미치는 영향을 분석합니다. 또한, 신호 처리 커뮤니티가 이러한 문제 해결에 어떻게 기여할 수 있을지에 대한 통찰을 제공합니다.

- **Technical Details**: 연구에서는 다변량 신호(multivariate signal)와 관련된 모델을 설정하고, 이러한 모델이 둔화된(reduced) 샘플링 비율(sampling rate) 및 메모리 창(window length) 변화에 대해 어떻게 반응하는지를 설명합니다. 고전적인 그랜자 원리(granger causality)와 같은 기법들이 이러한 시간적 재현성과 예측 성능에 미치는 영향을 다루며, 다양한 인과 발견 방법론을 설명합니다. 또한, 적시된 데이터에서 인과성이 어떻게 식별되고 추론될 수 있는지를 분석합니다.

- **Performance Highlights**: 이 논문에서는 인과 발견 방법이 하이퍼파라미터 변화에 민감하다는 사실을 확인했습니다. 특히, 전통적인 기법과 현대적인 기법 모두에서 이러한 요소들이 성능에 미치는 영향을 실험과 이론적 증거를 통해 입증하였습니다. 마지막으로, 이 연구는 신호 처리 완전도(signal processing integrity)와 인과성을 잇는 새로운 통찰력을 제시하며 실험적 접근방식으로 모든 인과 발견 기술의 통합 가능성을 제안합니다.
...
- **What's New**: 이번 연구에서는 베이지안 신경망(Bayesian Neural Networks, BNNs)의 한계를 극복하기 위한 새로운 사전 분포 클래스인 Dirichlet scale mixture (DSM) prior를 제안합니다. 기존의 사전 분포 지정 과정에서 발생하는 어려움을 해결하기 위해 구조화된 희소성 촉진 축소(sparsity-inducing shrinkage)를 활용합니다. DSM prior는 신경망의 기하학적 특성에 맞게 설계되어 있으며, 네트워크의 효과적인 파라미터 수를 대폭 줄일 수 있습니다.

- **Technical Details**: DSM prior는 Dirichlet 분포를 사용하여 고정된 분산 예산을 그룹 수준에서 파라미터를 정규화하는 방식으로 설계되었습니다. 이를 통해 네트워크의 가중치가 같은 노드에 맵핑될 경우, 동일한 그룹 특정 분산을 할당하여 희소한 솔루션을 촉진하는 효과를 기대할 수 있습니다. 이 방식은 기존의 글로벌-로컬 스케일 혼합 사전(priors)보다 효과적인 의존성 구조와 축소 특징을 제공합니다.

- **Performance Highlights**: 실험에서는 DSM priors가 희소 네트워크를 더욱 촉진하며, 적은 수의 효과적 파라미터로도 경쟁력 있는 예측 성능을 제공한다는 결과를 보였습니다. 특히, 이 방법은 상관관계가 있고 적당히 작은 데이터 상황에서 가장 두드러지며, 가중치 다듬기(weight pruning)에 유리한 특성을 나타냅니다. 또한, 무거운 꼬리를 가진 축소 메커니즘을 채택함으로써, 기존의 가우시안 사전이 가지는 한계를 극복할 수 있는 공정한 대안을 제시합니다.
...
- **What's New**: 이 연구는 조합적(combinatorial) 처치에 대한 비용 효용 추정(uplift estimation) 방법론을 제안합니다. 기존의 약식적인 처리 방법과는 달리, 본 연구에서는 정책의 의미를 명확하게 반영하는 처리 표현을 통해 더욱 정확하고 견고한 결과를 도출할 수 있는 프레임워크를 제공합니다. 이 방법론은 특히 긴 꼬리(long-tailed) 처리 체계에서의 정확도 및 안정성을 향상시키는 데 기여합니다.

- **Technical Details**: 본 연구에서는 정책을 컨텍스트-액션(context-action) 구성 요소들에 대한 혼합으로 나타내고, 이를 순열 불변(permutation-invariant) 집합을 통해 통합하는 방법을 사용합니다. 이러한 표현 방식은 기존의 처리 모델링에서 발생하는 통계적 강도의 공유 문제를 해결하고, 최적화 문제에서 발생할 수 있는 편향을 줄이는데 효과적입니다. 또한, 제안된 모델은 처치의 복잡한 조합에서도 오류를 최소화하며, 매끄러운 정책 변동에 대한 이해도를 높입니다.

- **Performance Highlights**: 대규모 무작위 플랫폼 데이터에 대한 실험을 통해 본 방법론은 유의미한 향상을 보였고, 특히 긴 꼬리 처리 체계에서의 안정성을 강조했습니다. 결과적으로, 제안된 접근법은 드물거나 이전에 관찰되지 않은 정책 변형에 대한 일반화를 지원하여 실제 사용할 수 있는 모델의 활용도를 높입니다.
...
- **What's New**: 이 논문은 ReLU 신경망의 매개변수에서 발생하는 재스케일링(Rescaling) 대칭성을 활용하는 새로운 방법을 제안합니다. 기존의 재스케일링 대칭성은 신경망의 함수에 영향을 주지 않지만, 훈련 동력학에는 상당한 변화를 초래할 수 있습니다. 이를 바탕으로 신경망 매개변수를 정렬하는 새로운 기하학적 기준을 도입하고, 이를 통해 더 나은 훈련 특성을 가진 매개변수로의 재스케일링을 가능하게 합니다. 또한, 이 과정에서 경량의 알고리즘 경로 정렬(PathCond)을 개발하여 훈련 속도를 개선하는 방법을 탐구합니다.

- **Technical Details**: 논문에서는 ReLU 네트워크의 매개변수 공간과 네트워크 구현에 따른 무한 차원 표현 사이의 중간 단계인 경로 정 lifting을 사용하여 기하학적으로 동기화된 새로운 재스케일링 기준을 개발합니다. 이 기준은 매개변수 θ를 재스케일링하여 훈련 동력학을 개선할 수 있는 가능성을 보여줍니다. 저자들은 체계적인 초기화 스케일과 아키텍처 선택이 훈련 동력학에 미치는 영향을 분석하고, 다양한 데이터셋에서 실험을 통해 초기화 시 이 기준을 사용하면 훈련 속도가 크게 향상된다는 것을 입증합니다.

- **Performance Highlights**: CIFAR-10 데이터셋에서 연구진은 제안한 재스케일링 기준을 사용하여 기준 모델에 비해 1.5배 적은 에포크로 동일한 정확도를 달성할 수 있음을 보여줍니다. 이를 통해 이 방법이 훈련 과정을 보다 효율적으로 만들 수 있는 잠재력을 지니고 있음을 확인했습니다. 이 논문은 실질적으로 신경망의 훈련 동력을 개선하기 위한 새로운 방향을 제시하고 있으며, 여러 최신 모델에 적용 가능성을 가지고 있습니다.
...
- **What's New**: 이번 연구에서는 연속 시간 마르크프 체인(CTMC)의 정확한 확률적 시뮬레이션을 제안합니다. 기존의 Gillespie 유형 알고리즘이 직면한 하드 카테고리 이벤트 선택의 제약을 해소하여, 마지막 단계에서 발생하는 경량의 노이즈에 보다 적합한 해결책을 제시합니다. 본 방법론은 확률적 시뮬레이션과 역방향 미분을 분리함으로써 시뮬레이션의 유연성을 극대화합니다.

- **Technical Details**: 하드 카테고리 샘플링을 통해 정확한 경로를 생성하고, Gumbel-Softmax를 통해 연속적으로 기울기를 전파합니다. 이를 통해 기존 시뮬레이터보다 네 배 더 큰 매개변수 스케일에서 최적화를 가능하게 했습니다. 논문에서는 역전거(가역적인) 이합체 모델과 유전자 진동기, 203,796개의 매개변수를 갖는 유전자 조절망에서도 검증하였습니다.

- **Performance Highlights**: 본 GPU 구현은 초당 19억 스텝을 달성하여 비미분 가능한 시뮬레이터와 동등한 정확성을 보입니다. 결과적으로 시스템 생물학, 화학 동역학, 물리학 및 CTMC에 의해 지배되는 관련 분야 전반에 걸쳐 고차원 매개변수 추론과 역설계를 가능하게 합니다. 이러한 점에서 본 연구는 HPC(고성능 컴퓨팅) 영역에서 현저한 기여를 합니다.
...
- **What's New**: 이번 연구는 동적 예측(dynamic predictions) 모델을 통해 주어진 데이터를 바탕으로 1차 담즙성 담관염(primary biliary cholangitis, PBC) 환자들의 생존 확률을 개선하는 방법을 제안합니다. 연구진은 Super Learner라는 앙상블 학습(ensemble learning) 프레임워크를 통해 서로 다른 예측 알고리즘을 결합하여 예측 정확도를 높이는 방식을 탐구했습니다. 이 방법은 다양한 모델의 출력 결과를 유연하게 결합할 수 있어, 생존 예측의 정확성을 크게 증가시킬 수 있습니다.

- **Technical Details**: 연구진은 Super Learner를 기반으로 한 동적 예측 기법에서 여러 모델의 예측 성능을 조합하는 방법을 제시하였습니다. Super Learner는 cross-validation과 최적의 성능 기능을 사용하여 다양한 예측 모델의 조합을 가중치로 최적화합니다. 이 기술은 기존의 joint modelling과 landmarking 방법의 한계를 극복하기 위한 것으로, 특히 데이터에서 직접적으로 선택된 변수들에 의해 더 유연한 예측을 가능하게 합니다.

- **Performance Highlights**: Super Learner를 이용한 연구 결과, PBC 환자 데이터셋에서 예측 성능이 기존 단일 모델에 비해 유의미하게 향상되었습니다. 다양한 가정으로부터 출력 결과를 결합할 수 있기 때문에, 예측 정확도가 높아지고 임상 의사결정에서 더 효과적으로 활용될 수 있습니다. 이러한 접근법은 다중 결과를 다룰 때에도 훌륭한 성능을 발휘하여 실제 진료현장에서의 실용성을 제고하였습니다.
...
- **What's New**: 이 논문에서는 현대 딥러닝에서 부드러운 활성화 함수(smooth activation functions)의 이론적 장점을 비부드러운(non-smooth) 함수와 비교하여 설명하고자 합니다. 특히, Sobolev 공간 $W^{s,	ext{infty}}([0,1]^d)$ 내 부드러운 활성화 함수를 가진 신경망의 근사화 및 통계적 특성을 조사했습니다.

- **Technical Details**: 저자들은 부드러운 활성화를 가진 고정 깊이(constant-depth) 신경망이 목표 함수의 높은 차수의 부드러움을 자택적으로 활용할 수 있음을 증명하였습니다. 이 연구는 명시적인 신경망 근사기(neural network approximators)를 구축하는 방법론을 제시하며, 매개변수의 노름(norm) 및 모델 크기를 세밀하게 제어하여 통계적 학습 가능성을 확보하였습니다.

- **Performance Highlights**: 부드러운 활성화를 가진 신경망은 최소최적(minimax-optimal) 근사 및 추정 오차율을 달성할 수 있으나, 비부드러운 활성화인 ReLU를 가진 신경망은 깊이에 의해 제한되며 덜 효과적입니다. 이 연구 결과는 활성화의 부드러움이 깊이(depth) 대신 통계적 최적성을 달성하기 위한 근본적인 메커니즘임을 밝힙니다.
...
- **What's New**: 이 논문은 안전-critical (safety-critical) 시스템 공학에서 대규모 언어 모델의 채택을 제한하는 문제를 다루고 있습니다. 저자들은 신뢰성, 추적 가능성 및 기존 검증 관행과의 정렬을 바탕으로 한 신뢰할 수 있는 GenAI 통합을 위한 워크플로우 수준의 설계 원칙을 제안합니다. 이 원칙은 자동차 파이프라인에서 요구 사항 차이점 식별부터 SysML v2 아키텍처 업데이트 및 재테스트까지 적용됩니다.

- **Technical Details**: 모놀리식 프롬프트가 대규모 사양에서 중요한 변경 사항을 놓치는 경우, 섹션별 분해 및 다양성 샘플링, 경량 NLP 체크를 통해 완전성과 정확성을 향상시킬 수 있음을 보여줍니다. 요구 사항 델타를 SysML v2 모델로 전파하고 컴파일 및 정적 분석을 통해 업데이트를 검증하는 과정을 포함합니다. 또한 스펙 변수와 아키텍처 포트 및 상태 간의 명시적 매핑을 통해 테스트 케이스를 생성하여 추적 가능한 회귀 테스트를 보장합니다.

- **Performance Highlights**: 이 approach는 단순히 더 큰 모델을 사용하는 것과는 다른 방향으로, 모듈화, 체크 및 툴 기반 검증을 통해 신뢰성과 추적 가능성을 향상시키는데 초점을 맞추고 있습니다. 실제 산업 현장에서 이미 일부 툴 체인이 배포되어 사용되고 있으며, 요구 사항 변경 검토 시 상당한 효율성 향상이 입증되었습니다. 이 연구는 GenAI를 안전-critical 자동차 시스템 공학에 적용하는 데 필요한 실질적 보호 장치를 제공하는 중요한 기초를 형성합니다.



### Manifold-Aligned Generative Transpor (https://arxiv.org/abs/2602.19600)
Comments:
          64 pages, 5 figures

- **What's New**: 본 논문에서는 MAGT(Manifold-Aligned Generative Transport)라는 새로운 생성 모델링 프레임워크를 제안합니다. 이 방법은 고차원 데이터의 생성에서 매우 중요한 manifold(다양체) 구조를 고려하여, 적은 수의 부드러운 샘플로부터 데이터 공간으로의 전송(map)을 학습합니다. MAGT는 기존의 diffusion models(확산 모델)이나 normalizing flows(정규화 흐름)와 비교했을 때, 더 나은 샘플링 효율성과 높은 충실도를 동시에 달성하는 것을 목표로 하고 있습니다.

- **Technical Details**: MAGT는 고정된 Gaussian smoothing level에서 데이터를 학습하고 안정적인 score(점수)를 정의하여, 학습된 맵 h를 통해 낮은 차원의 latent variable로부터 고차원 데이터로의 직접적인 전송을 생성합니다. 이 과정에서도 self-normalized importance sampling(자기 정규화 중요 샘플링)을 활용하여 적절한 수치적 목적을 설정하고, 이를 통해 점수를 추정합니다. 이론적으로는 새로운 single-level pull-back inequality(단일 레벨 풀백 부등식)를 증명하여, 두 부드러운 분포 간의 점수 차이를 통해 Wasserstein error bound(워스터인 오류 경계)를 도출합니다.

- **Performance Highlights**: MAGT는 합성 manifold와 실 데이터 세트를 대상으로 수행한 실험에서, 기존의 확산 기반 모델들에 비해 전반적으로 더 높은 충실도를 보였으며, GANs보다도 일관되게 우수한 성능을 발휘했습니다. 특히, MAGT는 MNIST, Superconduct, Genomes 데이터셋에서 다른 모델들보다 월등한 결과를 보여줬으며, CIFAR10-0의 경우에는 흐름 일치(mode matching) 방법이 약간 더 좋은 성능을 보였습니다. 또한, MAGT는 inference time(추론 시간) 내에서의 계산량을 크게 줄이며, 데이터의 지지(support) 근처의 집중도를 개선하는 성과를 이뤘습니다.



### Goal-Oriented Influence-Maximizing Data Acquisition for Learning and Optimization (https://arxiv.org/abs/2602.19578)
- **What's New**: 이번 논문에서는 GOIMDA(Goal-Oriented Influence-Maximizing Data Acquisition)라는 새로운 능동 데이터 획득 알고리즘을 제안합니다. 기존의 데이터 획득 방법들은 신뢰성 있는 예측 불확실성 추정에 의존하여 어려움을 겪고 있지만, GOIMDA는 일반화된 선형 모델에 대해 예측 엔트로피 최소화를 근사하면서도 명시적인 후향 확률 추정 없이도 불확실성을 인식합니다. 이 알고리즘은 사용자가 지정한 목표 함수에 대한 기대 영향을 극대화하여 데이터를 선택합니다.

- **Technical Details**: GOIMDA는 목표 경량화 목표 함수를 최적화하는 과정에서 훈련 손실의 곡률을 반영합니다. 구체적으로, 이 알고리즘은 목표의 기울기, 훈련 손실의 역 헤시안, 그리고 후보 데이터의 민감도를 결합하여 획득 규칙을 도출합니다. 이 과정을 통해 GOIMDA는 기존의 베이지안 추정을 사용하지 않고도 탐색과 활용의 균형을 달성합니다.

- **Performance Highlights**: 실험적으로, GOIMDA는 이미지 및 텍스트 분류, 잡음이 섞인 글로벌 최적화 및 신경망 하이퍼파라미터 튜닝을 포함한 여러 학습 및 최적화 작업에서 성능 목표를 효과적으로 달성했습니다. 기존의 불확실성 기반 활성 학습 알고리즘이나 Gaussian-process 베이지안 최적화 방법들보다도 현저히 적은 레이블링 샘플 또는 함수 평가로 원하는 성과를 이루는 결과를 보여주었습니다.
...
- **What's New**: 이번 논문에서는 전하 이동(charge transfer) 및 전기적 상호 작용(electrostatic interactions)의 정확한 모델링을 위한 새로운 전기적 기초 모델(electrostatic foundation model)을 제시합니다. 이 모델은 MACE 아키텍처를 기반으로 하여 장거리 상호 작용(long-range interactions)과 전기적 유도(electrostatic induction)를 명시적으로 처리하여 분자 화학(molecular chemistry)의 정확성을 향상시킵니다. 새로운 접근 방식은 지역적 다체 기하학적 특징(local many-body geometric features)과 비자기적 장(non-self-consistent field) 포멀리즘을 결합하여 유도 모델링을 진행합니다.

- **Technical Details**: 모델은 학습 가능한 전하(charge)와 스핀(spin) 밀도를 업데이트하는 폴라리자블 반복(polarsable iterations)을 통해 기초적으로 설계되었습니다. 학습 가능한 후쿠이 함수(Fukui functions)를 통해 총 전하와 총 스핀을 조절하며, 이는 다양한 전하 상태와 스핀 상태를 가진 시스템을 정확하게 설명하는 데 기여합니다. 이 모델은 OMol25 데이터셋을 기반으로 훈련되었으며, 다양한 벤치마크에서 화학적 정확성을 달성합니다.

- **Performance Highlights**: 장거리 전기적 상호작용이 비결합(non-covalent) 상호작용 및 초분자(complex) 구조의 설명에서 큰 개선을 가져오며, X23-DMC 데이터셋에서 분자 결정 형성 에너지를 단위 당 0.1kcal/mol 이하로 예측하는 성과를 보였습니다. 단백질-리간드 상호작용에서는 단거리 모델보다 4배와 같은 개선을 달성했습니다. 이 모델은 소분자부터 단백질-리간드 복합체까지의 전하 및 스핀 상태를 처리하며, 외부 전기장에 반응하고 해석 가능한 스핀 해상 전하 밀도(spin-resolved charge densities)를 제공합니다.
...
- **What's New**: 현대 조직들은 기술적인 결함보다는 인간의 행동에 의해 유발되는 사이버 보안 사건에 직면하고 있습니다. 이에 대한 해결책으로 제안된 개념적 보안 프레임워크는 하이브리드 CNN-LSTM 모델을 통해 생체 인식 및 환경 데이터를 분석하여 상황 인식(Security decision) 보안을 강화합니다.

- **Technical Details**: 이 프레임워크에서 CNN(Convolutional Neural Network)은 센서 데이터에서 공간 패턴을 추출하고, LSTM(Long Short-Term Memory)은 인간의 실수 민감성과 관련된 시간적 동역학을 포착합니다. 이 모델은 84%의 정확도를 기록하며, 인간 중심의 사이버 위험을 증가시키는 조건을 신뢰성 있게 감지할 수 있는 능력을 보여줍니다.

- **Performance Highlights**: 이 프레임워크는 지속적인 모니터링과 적응형 안전장치를 가능하게 하여 인간 주도의 사이버 사건의 가능성을 줄이는 선제적 조치를 지원합니다. 이를 통해 보안 사건을 예방하고 조직의 전반적인 사이버 보안 태세를 강화할 수 있습니다.
...
- **What's New**: 이 연구는 $	ext{R}^d$의 두 번째 차수 타원형 편미분 방정식(PDEs)에 대한 정규성 정리를 제정합니다. 약한 타원성과 작은 조건하에, 해결책은 두 개의 추가적인 Barron 정규성을 얻습니다. 또한, 코사인 활성화 함수를 사용하는 2층 신경망으로 근사될 수 있는 PDE 클래스가 식별됩니다.

- **Technical Details**: 우리는 Barron 공간에서 고차원 PDE 해의 정규성 문제를 해결하기 위해 Barron 함수의 정의를 수정하고 스펙트럼 Barron 공간을 도입합니다. 여기에서 특정 PDE의 해가 이러한 스펙트럼 Barron 공간에 속하는 조건을 정립하고 추가적인 Barron 정규성의 획득을 보여줍니다. 이 과정에서 변동 계수를 가진 2차 타원형 PDE에서의 고전적 정규성 결과를 스펙트럼 Barron 공간으로 확장합니다.

- **Performance Highlights**: 주요 결과는 모든 원천 항 f가 $	ext{B}^s$에 속할 경우, 유일한 해 $u^{*}$가 Barron 정규성을 두 배로 증가시키며 이를 통해 2층 신경망이 코사인 활성화를 통해 근사될 수 있음을 증명합니다. 이 네트워크의 폭은 공간 차원에 독립적이며, 이론적으로 산출된 수는 단위 부피의 모든 영역에서 단독으로 검증됩니다.
...
- **What's New**: 이 논문에서는 Vid2Sid라는 새로운 동영상 기반 시스템 식별 파이프라인을 제안합니다. 이 시스템은 기본 모델(perception)과 비디오 언어 모델(VLM)을 결합하여 시뮬레이션과 실제 비디오를 비교 분석하고, 물리적 매개변수를 업데이트하는 게 특징입니다. Vid2Sid는 각 반복에서 설명 가능한 진단을 제공하여 파라미터 조정 과정에서의 투명성을 높입니다.

- **Technical Details**: Vid2Sid는 두 개의 주요 레이어로 구성되어 있습니다. 첫 번째 레이어는 비디오에서 궤적(trajectory)을 추출하고, 두 번째 레이어는 VLM을 사용하여 시뮬레이션과 실제 비디오의 차이를 분석하여 물리적 매개변수 업데이트를 제안합니다. 이 과정은 반복적으로 이루어져 실제 행동과 시뮬레이션 행동 간의 정렬 오류(minimum alignment error)를 최소화하는 파라미터를 찾아냅니다.

- **Performance Highlights**: Vid2Sid는 훈련 중 보지 못한 시나리오에서 최상의 평균 순위를 기록하며, 시뮬레이션-시뮬레이션 테스트에서도 가장 정확한 정답 파라미터를 복원합니다. 상대적 오류는 평균 13%로, 기존 기법들에 비해 훨씬 우수한 성능을 나타냅니다. 또한, 다양한 형태의 로봇에 대해 적용 가능하다는 점에서 큰 장점을 지니고 있습니다.
...
- **What's New**: 이번 논문에서는 미국의 하위 국가 행정 단위에서의 산림 손실과 관련된 탄소 배출 간의 관계를 정량화하기 위한 포괄적인 패널 데이터 세트를 구축했습니다. 이 데이터는 2001년부터 2023년까지의 연간 산림 손실과 탄소 배출 추정치를 포함하며, Hansen 글로벌 산림 변화 데이터셋을 기반으로 합니다. 연구 결과, 산림 손실이 단기적으로 탄소 배출에 상당한 긍정적인 탄력성을 나타내며, 탄소 배출이 시간에 따라 강한 지속성을 보임을 확인했습니다.

- **Technical Details**: 산림 손실과 탄소 배출 간의 관계는 본질적으로 동적이며, 탄소 방출 과정은 생물량 분해, 화재 및 지연된 토지 전환 등의 여러 해에 걸쳐 진행될 수 있습니다. 본 연구에서는 두 방향 고정 효과 모델을 이용하여 지역 내 변수를 분리하고 정적 회귀 분석에서 나타나는 누적 또는 장기적 영향을 과소 평가하지 않도록 했습니다. 또한, 동적 패널 회귀모델을 통해 변동성을 고려하고, 자기회귀 구조를 통합하여 장기 탄력성을 평가했습니다.

- **Performance Highlights**: 우리의 연구는 단기적뿐만 아니라 장기적인 탄력성을 정량화하여 산림 손실의 직후 영향과 누적 동적 영향을 구별했습니다. 또한, 자기 회귀 구조를 무시할 경우 반복적인 산림 교란의 장기적인 환경적 결과를 상당히 과소평가할 수 있음을 보여주었습니다. 이 연구는 높은 해상도의 원거리 감지 데이터와 경제계량학적 패널 모델링을 통합하여 환경 변화 과정을 분석하기 위한 강력하고 해석 가능한 방법론을 제공합니다.
...
### Metasurfaces-Integrated Wireless Neural Networks for Lightweight Over-The-Air Edge Inferenc (https://arxiv.org/abs/2602.19312)
Comments:
          9 pages, 6 figures, submitted for magazine publication

- **What's New**: 이번 논문에서는 다가오는 6세대(6G) 무선 네트워크의 초저지연성과 에너지 효율적인 Edge Inference (EI)를 위한 새로운 접근 방식을 제안합니다. 이 연구는 Metasurfaces-Integrated Neural Networks (MINNs)라는 물리적 레이어 기반 심층 학습 프레임워크를 도입하여, 무선 채널을 활용해 계산 작업을 수행하는 혁신적인 과제를 보여줍니다. 또한, 무선 전송 및 수신 모듈에서 작동하는 다양한 아키텍처를 통해 전통적인 디지털 하드웨어의 전력 소모를 대폭 줄일 수 있는 가능성을 제시합니다.

- **Technical Details**: MINN 시스템은 Encoder, Channel 및 Decoder의 세 가지 모듈로 구성됩니다. Channel 모듈에는 메타서피스와 함께 다중 입력-다중 출력(MIMO) 채널이 포함되어 있으며, 모든 모듈의 가중치는 훈련 가능한 형태로 최적화됩니다. 이 아키텍처는 물리적 레이어에서의 연산을 오프로드하여 에너지 효율성을 극대화하며, 기존의 딥 뉴럴 네트워크(DNN)와 유사한 성능을 자랑합니다.

- **Performance Highlights**: 연구 결과 MINNs는 상대적으로 간단한 하드웨어 구조에도 불구하고 뛰어난 성능을 보여 주었으며, 특히 Edge Inference 같은 응용 프로그램에서 가능성을 입증했습니다. 이 프레임워크의 훈련 및 배치 관련 세부 사항을 포함하고 있으며, 기존의 ML프레임워크보다 저전력 소모를 통해 IoT 분야에서 더 효율적으로 작동할 수 있는 잠재력을 지니고 있습니다. 마지막으로, MINNs에 대한 열린 도전 과제와 향후 연구 방향도 논의됩니다.
...
- **What's New**: 본 연구에서는 제조 시스템의 예측 유지보수를 위한 새로운 베이지안 비모수 프레임워크를 제안합니다. 이는 비지도 학습 방식으로 고장 모드를 파악할 수 있는 디리클레 프로세스 혼합 모델(Dirichlet Process Mixture Model, DPMM)과 인공 신경망 기반의 예측 모듈을 결합하여 고장 모드 탐색과 RUL(남은 유효 수명) 예측을 통합합니다. 이 프레임워크는 실제 데이터가 도착할 때마다 고장 모드를 동적으로 추론, 확장 또는 병합할 수 있는 iterative feedback mechanism을 특징으로 합니다.

- **Technical Details**: 제안된 프레임워크는 DPMM을 통해 비지도 방식으로 고장 모드를 발견하고, 인공 신경망 기반의 예측 모듈이 여러 센서 데이터를 활용하여 남은 유효 수명을 예측하도록 구성되어 있습니다. 이 두 모듈은 상호 연관성을 가지며, 서로를 지속적으로 향상시키기 위해 반복적으로 업데이트됩니다. 이를 통해 시스템의 비정상적인 동적 환경에서도 새로운 고장 모드를 효과적으로 처리할 수 있는 유연성을 갖추었습니다.

- **Performance Highlights**: 시뮬레이션과 항공기 엔진 데이터셋에서 수행된 실험 결과, 제안된 방법은 기존의 접근 방식들과 비교하여 경쟁력 있는 성능을 보였습니다. 특히 이 방법은 복잡한 제조 환경에서 디지털 트윈 기반 시스템 건강 관리에 적합한 강력한 실시간 적응 능력을 보여주었습니다. 이러한 성과는 본 연구의 효율성과 유연성을 입증하며, 향후 적용 가능성을 더욱 높입니다.
...
- **What's New**: 이 논문에서는 대형 언어 모델이 복잡한 절차를 따를 수 있지만, 자신이 계산한 값을 잘못 보고하는 현상인 procedural hallucination을 연구한다. 이는 모델이 명확하게 정해진 사양을 따르지 못하는 것으로, 기존 지식의 결여가 아닌 정보는 존재하지만 사용하지 못하는 문제를 나타낸다. 특히, 기존 문맥 내에서 정확한 정보가 이미 존재할 때 발생하는 경향이 있다.

- **Technical Details**: 제안된 방식은 정보 이론과 인과적 추론에 기반한 procedural hallucination 이론을 개발하여 이를 여러 모델 가족에 대해 실증적으로 검증한다. 연구에서 등장하는 두 가지 주요 단계는 Stage 2A (gating)와 Stage 2B (binding)로, 주로 Stage 2B에서 오류가 발생한다. 이 단계에서 모델은 답변 모드에 들어가지만 잘못된 후보를 선택하는 것으로 나타난다.

- **Performance Highlights**: Stage 2B의 오류를 교정하기 위한 다양한 방법이 제시된다. 특히, 확인 지점(oracle checkpointing)_intervention을 통해, 쿼리 근처에서 올바른 결합을 재진술함으로써 긴 거리에서의 D2B 실패를 거의 제거할 수 있음을 입증하였다. 결과적으로, 실험에서는 Qwen2.5-3B 모델이 0/400에서 399/400으로 정확도가 크게 향상되었다.
...
- **What's New**: 이 논문에서는 조류 비행의 역학을 설명하는 생성 물리 모델이 부족하다는 점을 강조하고, 동적 모드 분해(Dynamic Mode Decomposition, DMD) 기법을 통해 매의 비행 동작을 분석하여 다수의 행동 상태들을 모델링할 수 있음을 보여준다. DMD는 단순하고 해석 가능한 모달 구조를 통해 비행 관찰을 재현하고, 실제 비행 데이터에 대한 직접 적합성을 제공하여 전통적인 물리학적 접근법의 한계를 극복하였다. 논문은 또한 서로 다른 비행 행동에서 매의 공통 동적 모드를 찾았음을 보고한다.

- **Technical Details**: 연구에서는 5마리의 해리스 매(Parabuteo unicinctus)를 대상으로 모션 캡처 기법을 활용하여 비행 데이터를 수집하였다. DMD 분석을 통해 비행 데이터는 선형 동적 시스템을 가정하고, 매의 날개와 꼬리 형태를 결정짓는 지배적인 모드로 분해되었다. DMD의 접근은 다른 파라미터와 함께 비행 동작을 매개변수화 할 수 있으며, 이는 비행 기술 이해에 중요한 역할을 한다.

- **Performance Highlights**: DMD 모델은 매의 비행 역학을 고도의 정확도로 재구성하는데, 비행을 정량화하기 위해 필요한 파라미터가 단 3개로 제한된다. 연구 결과는 비행 동작이 복잡해 보일지라도, 세 가지 주요 날개-꼬리 형태로 인해 비행 역학이 매개변수화될 수 있음을 시사한다. 이처럼 DMD는 다목적 비행 행동을 직관적으로 제시하고, 생물학적 해석이 용이한 결과를 도출하는 데 기여하고 있다.
...
- **What's New**: 이 논문은 분산 학습을 위한 새로운 이벤트 기반 gossip 프레임워크를 제안합니다. 이 프레임워크는 모델 정보 교환 시기를 각 노드가 자율적으로 결정하도록 하여 통신 비용을 효과적으로 절감합니다. 또한, 이 시스템은 비공식적인 목적(function)의 경우에서도 수렴 보장을 제공합니다.

- **Technical Details**: 제안된 프레임워크는 노드 간의 모델 상태 교환을 조정하는커뮤니케이션 제어 메커니즘을 도입하고, 이를 통해 불필요한 정보 교환을 줄이면서도 높은 모델 정확도를 유지하는 것을 목표로 합니다. 연구는 이벤트 트리거링 정책을 통해 다양한 조건 하에서의 수렴 속도를 분석하였으며, 이는 학습 정확성과 통신 비용 간의 균형에 대한 통찰을 제공합니다.

- **Performance Highlights**: 실험 결과, 제안된 프레임워크는 기존의 분산 학습 방법들과 비교했을 때 최대 71.61%의 통신 비용 절감을 달성하였으며, 정확도의 경미한 감소로 유사한 학습 성능을 나타냈습니다. MNIST 및 Fashion-MNIST 데이터셋에서 효과적인 성능 저하 없이 통신량을 현저히 줄이는 결과를 보여 주었습니다.
...
### Implicit Bias and Convergence of Matrix Stochastic Mirror Descen (https://arxiv.org/abs/2602.18997)
- **What's New**: 이번 연구에서는 다중 클래스 분류 및 행렬 완성 문제와 관련된 행렬 파라미터 및 벡터 값 예측을 사용하는 Stochastic Mirror Descent (SMD)를 조사하였습니다. 특히 과적합 (overparameterized) 영역에 초점을 맞추어, SMD가 전역 보간기 (global interpolator)로 지수적으로 수렴한다는 것을 증명하였습니다. 또한, 행렬 SMD 알고리즘이 Bregman 발산을 최소화하는 고유 솔루션으로 수렴함을 일반화하여, 고차원 다출력 문제에서의 귀납적 편향 (inductive bias)을 어떻게 결정하는지를 드러내었습니다.

- **Technical Details**: 논문은 SMD가 강하게 볼록한 잠재 함수 (potential function)인 ψ에 의해 유도된 이중 공간에서 업데이트를 수행함으로써 표준 경량 하강법 (gradient descent)의 일반화를 이루는 방법을 설명합니다. 주어진 훈련 손실 함수 ℒt에 대해 SMD는 행렬 가중치와 벡터 출력을 고려하는 업데이트 규칙을 통해 과적합 문제를 해결하는 새로운 방법론으로 제시됩니다. 또한, SMD는 Bregman 발산을 최소화하는 솔루션을 찾기 위해 사용될 수 있으며, 이는 고유한 해결책을 도출하는 성질을 보여줍니다.

- **Performance Highlights**: 행렬 SMD는 낮은 랭크 (low-rank) 솔루션을 생성하는데 있어 기존의 특이값 임계값 방법 (singular value thresholding methods)보다 더 낮은 오류를 보이는 것으로 나타났습니다. 특히, p≈1로 설정하여 핵 노름 (nuclear norm)을 근사할 때 더 효과적입니다. 이러한 결과는 다양한 신호 처리 및 데이터 과학 문제에서 행렬 복구 문제를 해결하기 위한 새로운 가능성을 열어줍니다.
...
- **What's New**: 이 논문은 금융 위험 모델 관리를 위한 사후 설명 가능성이 중요한 상황에서 대형 언어 모델(LLMs)이 신용 위험 예측에 어떻게 활용될 수 있는지를 탐구합니다. 연구는 LLMs가 번역자(translators)와 자율 설명자(autonomous explainers)라는 두 가지 역할을 수행할 수 있는지를 중심으로 진행되며, 결론적으로 LLMs는 복잡한 모델 출력을 비기술적 이해관계자에게 전달하는 내러티브 인터페이스로서 효과적임을 보여줍니다.

- **Technical Details**: 논문에서는 LendingClub의 개인 대출 데이터셋을 사용하여 GPT-4-turbo, Claude Sonnet 4 및 Gemini-2.0-Flash와 같은 상용 LLM 3종을 평가합니다. LLM이 제공하는 설명은 주로 기계 학습 모델의 출력을 기반으로 하며, 사후 설명 방법(SHAP, LIME 등)으로 생성된 기여도와 비교됩니다. LLM은 주어진 특정 피처 중요도 랭킹을 보존할 수 있지만, 자율적으로 추론하는 경우에는 낮은 일관성을 보입니다.

- **Performance Highlights**: 연구 결과에 따르면, LLMs는 참조 랭킹(reference rankings)이 제공될 경우 대부분 올바른 Top-K 피처 세트를 보존합니다. 그러나 참조 랭킹이 없을 경우 LLM의 일치도는 일반적으로 낮았습니다. 이는 LLM이 비선형적 상호작용 추론을 재구성하는 데에 한계가 있음을 시사하며, LLM을 사용자가 요구하는 내러티브 생성 도구로 활용하는 것이 중요하다는 것을 강조합니다.
...
- **What's New**: 이 논문에서는 프라이버시 규제 때문에 공유할 수 없는 단편화된 데이터 수집과 불일치를 다루고 있습니다. 수직적으로 연결된 기관들(병원, 보험사, 법원, 은행) 간의 협업 모델링을 위한 수평적 연합 학습(horizonal federated learning, FL)을 제안하고 있습니다. 인구 통계학적 공정성(demographic parity)을 평가하기 위해 각 시로(silo)에서 최소한의 요약정보를 사용하여 전체 인구 수준의 불공정성을 추정하는 방법론을 제시하고 있습니다.

- **Technical Details**: 저자들은 감지성과 통신 제약 하에서 인구 수준의 불공정성을 감사(audit)하는 것을 연구하고 있습니다. Wasserstein–Fréchet 변이를 활용하여 민감 그룹(score laws) 간의 불일치를 측정하고, 이를 통해 로컬과 글로벌 불일치 간의 세부 메커니즘을 진단합니다. 논문에서는 각 시로가 소수의 그룹 수와 정량화된 요약만 공유하여 서버가 글로벌 불일치와 그 기원을 추정하는 효율적인 프로토콜을 제안합니다.

- **Performance Highlights**: 실험에서는 몇몇 정량화된 수치만으로도 글로벌 불공정성을 추정하고 그 출처를 진단하는 데 충분함을 보여줍니다. SYNTHETIC 데이터와 COMPAS 데이터셋에서 로컬 감사를 사용한 경우 잘못된 결론을 도출할 수 있음을 발견했습니다. 이는 각 시로에서의 지방 공정성(local fairness) 관찰이 전체적(global) 결과와 일치하지 않는 문제를 강조합니다.
...
- **What's New**: 이 논문은 기초 모델의 개방형 가중치가 해로운 용도로 세밀화될 수 있는 가능성을 다루고 있으며, 기존의 훈련 저항 방법이 이론적인 보장을 제공하지 못한다는 점에 주목합니다. 저자들은 최적화 속도와 모델 가중치의 스펙트럼 구조를 연결하여 새로운 수렴 속도 제어 방법을 개발하였습니다. 이 방법은 첫 번째 및 두 번째 차수 최적화를 비적대적 설정에서 입증 가능하게 느리게 만드는 알고리즘인 SpecDef를 포함합니다.

- **Technical Details**: 이 연구는 모델의 수렴 속도를 제어하기 위해 헤시안(Hessian) 스펙트럼을 조작하는 방법을 제시합니다. 모집단의 발산 방향과 손실 함수의 구부러짐 정도가 최적화 속도에 미치는 영향을 확인하였고, 이러한 측정값으로부터 최적의 업데이트 속도를 도출 할 수 있음을 보였습니다. SpecDef 알고리즘은 이러한 수치적 제어를 구현하여 최적화의 효율성을 높입니다.

- **Performance Highlights**: 제안된 방법을 통해 실험적으로 최적화 성능이 향상되었음을 입증하였으며, 특히 비적대적 안전 검증을 통한 평가에서 두드러진 결과를 확인할 수 있었습니다. 또한, 시각 및 언어 도메인에서의 안전성 평가에 대한 방법론적 향상도 포함되어 있어 더욱 폭넓은 적용 가능성을 보여줍니다. 이러한 성과는 기존의 훈련과 안전성 개선을 위한 새로운 이론적 기초를 제공하며 향후 발전 가능성을 시사합니다.
...
### Habilis-$β$: A Fast-Motion and Long-Lasting On-Device Vision-Language-Action Mod (https://arxiv.org/abs/2602.18813)
- **What's New**: Habilis-$\beta$ 모델은 실제 환경에서의 비전-언어-행동(VLA) 시스템의 성능 저하 문제를 해결하기 위해 개발되었다. 기존의 VLA 모델들이 단일 시도 성공률에만 초점을 맞추는 반면, Habilis-$\beta$는 연속 실행 중의 효율성과 내구성을 평가하는 Productivity-Reliability Plane (PRP)라는 새로운 평가 기준을 도입했다. 이 모델은 빠른 속도와 지속적인 작동을 목표로 하며, 효과적인 반복 작업을 지원하기 위한 특수 데이터를 활용한다.

- **Technical Details**: Habilis-$\beta$는 방대한 플레이 데이터에 기반하여 비언어적 사전 훈련을 수행하고, 주기적 과제 시연 데이터로 포스트 훈련을 통해 상태 변화에 대응하는 능력을 향상시킨다. ESPADA 기법을 이용하여 빠른 속도의 자유 공간 이동을 지원하며, rectified-flow distillation을 통해 가장자치기기에서 신속한 제어를 가능하게 한다. 또, Classifier-Free Guidance (CFG)를 통해 실행 중에 명령 준수와 학습된 상호작용 우선 순위를 동적으로 조정할 수 있다.

- **Performance Highlights**: 1시간의 연속 실행 평가에서 Habilis-$\beta$는 시뮬레이션 환경에서 572.6 TPH와 39.2초 MTBI를 기록하며, 실제 환경에서는 124 TPH와 137.4초 MTBI를 달성했다. Habilis-$\beta$는 기존의 $\pi_{0.5}$ 모델에 비해 5배 이상의 성과를 나타내며, RoboTwin 2.0 리더보드에서도 최고의 성능을 기록하여 복잡한 조작 시나리오에서의 효과성을 입증하였다.
...
- **What's New**: 이 논문에서는 머신 보조 정리 증명(Machine-Assisted Theorem Proving, MATP)의 강점을 살리기 위해 여러 새로운 개념과 기능을 도입합니다. 
먼저, 모든 증명 가능한 명제를 증명할 수 있는 소수의 원자적 전술(atomic tactics)을 소개하며, 이를 통해 정리 증명 작업의 선택 가능성을 줄입니다. 이어서, 임의의 증명 표현을 원자적 전술의 시퀀스로 변환하는 원자화 변환(transposing atomization) 알고리즘을 제안하고, Lean 표현을 간결하게 나타내는 ExprGraph 데이터 구조를 소개합니다.

- **Technical Details**: 논문에서는 총 다섯 가지의 주요 기여를 통해 MATP 접근 방식을 혁신합니다. 
원자적 전술은 작은 집합의 간단한 전술로 구성되어 있으며, 이는 증명을 점진적으로 구성할 수 있게 합니다. 또한 원자화 변환 알고리즘은 기존의 형식화된 수학에서 대량의 훈련 데이터를 생성하여 시스템이 증명을 찾는 방법을 학습하도록 합니다. 그리고 ExprGraph는 Lean 표현의 최소 그래프 기반 표현으로, 이를 통해 목표에 따라 적절한 전술을 선택할 수 있습니다. 마지막으로, Nazrin Prover라는 저자화된 데이터로 훈련된 그래프 신경망 기반의 정리 증명 에이전트를 소개합니다.

- **Performance Highlights**: Nazrin은 기존 정리 증명 에이전트가 직면한 많은 문제를 회피할 수 있으며, 소비자용 하드웨어에서 훈련과 평가가 가능할 정도로 강력합니다. 
이 논문에서는 Lean의 표준 라이브러리와 Mathlib에서의 정리를 통해 Nazrin의 독특한 기능을 보여줍니다. 또한, Nazrin은 정리 증명 자동화의 새로운 가능성을 제시하며, 수학 증명의 작업을 효율적으로 수행할 수 있는 기회를 제공합니다.
...
- **What's New**: 이 논문은 잠재적 결과(Potential Outcomes, POs)와 관찰 변수(Observed Variables, OVs)의 공동 확률을 평가하는 문제를 다룬다. 논문에서는 이들 확률의 경계(bounding)와 식별(identification) 문제를 다루며, 이산 처리(discrete treatment)와 이산 순서형 결과(discrete ordinal outcome)에서의 적용을 제안한다. 새로운 단조성 가정(monotonicity assumptions) 패밀리를 도입하며, 경계 문제를 선형 프로그래밍(Linear Programming) 접근법으로 정식화한다.

- **Technical Details**: 이 논문에서는 공동 POs/OVs 확률을 경계짓고 이들의 선형 조합(linear combinations)을 다루는 문제를 해결하기 위해 새로운 단조성 가정(MAs)을 도입하였다. 특히, 이산 치료와 순서형 결과의 설정에서 기존의 MAs를 통합하여 여러 확률적 및 증가 제한 관계를 포함하는 일반 MA를 제시하였다. 또한, 새로운 MA를 통해 관찰 데이터로부터 이러한 확률의 식별을 위한 방법론을 제안하였다.

- **Performance Highlights**: 실험에서는 제안한 방법의 유효성을 검증하기 위해 실제 데이터 세트를 활용한 수치 실험을 수행하였다. 연구에서는 이산 치료 및 순서형 결과를 가진 설정에서 새로운 MA의 필요성을 강조하고, 선형 프로그래밍을 통한 경계 추정 과정을 시연하였다. 결과적으로, 제안된 방법은 실세계 데이터에 대한 적용 아이디어도 제공하며, causal inference에 대한 유용한 통찰력을 제공한다.
...
### Stochastic Gradient Variational Inference with Price's Gradient Estimator from Bures-Wasserstein to Parameter Spac (https://arxiv.org/abs/2602.18718)
- **What's New**: 이번 연구에서는 Wasserstein Variational Inference (WVI)와 Black-Box Variational Inference (BBVI) 알고리즘을 비교하여, 두 방법 모두 동등한 최첨단 반복 복잡도(iteration complexity) 보장을 제공함을 밝혔습니다. 기존에는 WVI가 Gaussian 변분 가족(Gaussian variational family)에 대해 BBVI보다 우수한 수렴 보장을 제공한다고 알려져 있었습니다. 그러나 이번 연구에서 두 방법의 차이를 줄일 수 있음을 보였습니다.

- **Technical Details**: WVI는 Bures-Wasserstein 공간에서의 경량화 방법을 사용하며, 특정 그라디언트 추정기(gradient estimator)인 Price의 평면(Price's theorem)을 사용하여 타겟 로그 밀도(target log-density)의 두 번째 정보(Hessians)를 활용합니다. 반면, BBVI는 재매개화 그라디언트(reparametrization gradient)를 사용하여 로그 밀도의 그라디언트만 필요로 합니다. 이번 연구를 통해 WVI와 BBVI의 접근 방식에서의 장점을 평행적으로 평가하였습니다.

- **Performance Highlights**: 실험적으로 Price의 그라디언트를 사용하는 것이 성능 향상의 주요 원인임을 증명했습니다. WVI의 적용성을 재매개화 그라디언트를 통해 넓힐 수 있음을 보여주었으며, 이는 실용적인 측면에서 두 접근 방식의 응용 가능성을 높이는 데 기여합니다.
...
- **What's New**: 이번 연구에서는 백질(white matter)의 경로가 대규모 신경 동기화(neural synchronisation)를 지원하는 방식을 이해하기 위한 데이터 기반 프레임워크를 개발하였습니다. 이 프레임워크는 확산 MRI(dMRI)와 기능적 MRI(fMRI)를 통합하여 백질 경로가 지원하는 동적 결합(dynamic coupling)을 모델링합니다. 특히, 개별 백질 경로로 연결된 원거리 회색질(grey matter)의 기능적 결합을 특성화하기 위해 Track-DFC(track dynamic functional connectivity)를 사용했습니다.

- **Technical Details**: 연구에서는 독립 성분 분석(independent component analysis)과 k-medoids 클러스터링을 이용하여, Human Connectome Project의 젊은 성인 집단에서 기능적으로 일관된 백질 경로 클러스터를 도출하였습니다. 노화하는 HCP 집단에 이 클러스터들을 적용하였더니, 기능적 결합 강도와 시간적 변동성의 광범위한 노화 관련 감소가 관찰되었습니다. 이는 백질 경로의 기능적 조직을 반영하며, 뇌 노화(brain ageing)와 인지 감소(cognitive decline)를 연구하는 강력한 도구가 될 것입니다.

- **Performance Highlights**: 특히, 조절(control), 기본 모드(default mode), 주의(attention), 시각 시스템(visual systems) 등을 연결하는 경로를 포함한 특정 클러스터가 나이와 인지 성능(cognitive performance) 사이의 관계를 유의미하게 매개했습니다. 이 연구 결과는 백질 경로의 기능적 조직을 그려내며, 노화에 따른 뇌 변화와 인지 성능 저하를 더 잘 이해하는 데 기여할 수 있습니다.
...
- **What's New**: 이 연구에서는 자율 기구를 통해 기계적 혈전 제거(MT) 내비게이션을 위한 신규 프레임워크인 계층적 모듈 다중 에이전트 강화 학습(HM-MARL)을 제안합니다. HM-MARL은 대퇴동맥에서 내경동맥(ICA)까지의 두 개의 장비를 자동으로 내비게이션할 수 있도록 설계되었으며, 이를 통해 복잡한 내비게이션 작업을 세분화하여 효율적이고 일반화된 방법으로 수행할 수 있습니다. 이 연구의 성과로는 최초의 in vitro MT 내비게이션 자율 실험이 포함되어 있으며, 향후 MT의 임상 변환에 기여할 것으로 기대됩니다.

- **Technical Details**: MT 내비게이션 작업은 다섯 가지 하위 작업으로 나뉘어 있으며, 각각은 단순화된 방식으로 강화 학습(RL)을 통해 학습됩니다. 연구에서는 stEVE 프레임워크 및 SOFA를 활용하여 두 개의 기구(주입 카테터 및 가이드 와이어)의 움직임을 모델링하였으며, 다양한 환자 해부학적 구조에서 일반화 가능성을 평가하기 위해 in silico 및 in vitro 환경에서 검증 작업을 수행하였습니다. 학습된 에이전트들은 목표 위치를 향해 순차적으로 이동하면서 자연스럽게 내비게이션을 수행할 수 있도록 설계되었습니다.

- **Performance Highlights**: 이 연구에서 제안된 HM-MARL 모델은 분석 대상인 다중 환자 해부학에 대해 56-80%의 성공률을 기록했으며, 특히 대퇴동맥에서 우측 공통 경동맥까지의 100% 성공적인 내비게이션을 달성했습니다. 하지만 좌측 혈관의 도전 과제에서 실패하였고 이는 사용된 해부학적 제약 때문이었습니다. 또한, 연구자들은 다양한 환자 환경에서의 일반화 가능성을 확보하기 위해 향후 모델 개선이 필요함을 강조했습니다.
...
- **What's New**: 이 논문에서는 디지털 트윈(Digital Twin, DT) 기술을 활용하여 개인 정보를 노출하지 않으면서 에너지 제약이 있는 전송 스케줄링 문제를 해결하는 새로운 프레임워크를 제안합니다. 기존의 연구들에서는 DT의 데이터를 중앙 집중식으로 접근하여 스케줄링 결정을 내렸지만, 이 연구는 DT가 비공식 요약만을 중앙 조정자와 교환하는 분산 최적화 방법을 사용합니다. 이를 통해 DT들은 개인 정보를 보호하면서도 전송 효율성을 극대화할 수 있습니다.

- **Technical Details**: 저자들은 고정 전력 전송과 고정률 타임 슬롯팅을 포함한 세 가지 스케줄링 문제를 고려하여, 전송 기간의 최소화 및 고정된 시간 블록 내에서의 데이터 전송 최대화를 다룹니다. 각 DT는 개인 데이터를 활용하여 지역 하위 문제를 해결하고, 중앙 조정자가 이들과 상호작용하여 글로벌 최적화를 향해 수렴합니다. 주목할 점은, DT 간의 통신이 고속 유선 네트워크를 통해 이루어져, 통신 오버헤드가 최소화된다는 것입니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘은 일관된 시간 단축을 보여주며, 대역폭 및 에너지 위반이 거의 없고, 일반적인 엣지 서버 하드웨어에서 밀리초 단위의 엔드 투 엔드 실행 시간을 기록합니다. 이러한 결과는 DT가 제공하는 정보의 개인 정보를 보호하면서도 동시에 성능을 크게 향상시킬 수 있는 가능성을 보여줍니다.



### Enhancing Goal Inference via Correction Timing (https://arxiv.org/abs/2602.18603)
Comments:
          25th International Conference on Autonomous Agents and Multiagent Systems (AAMAS)

- **What's New**: 이 연구는 로봇의 행동을 수정하기 위한 인간의 개입 결정 타이밍이 작업 목표에 대한 유용한 신호가 될 수 있는지를 조사합니다. 기존 연구들은 주로 수정된 행동을 새 시연으로 해석하거나 선호로 간주했으나, 인간이 로봇의 행동에 개입하기로 결정한 시점이 중요한 요소라는 점을 간과했습니다. 이 연구는 개입의 타이밍이 로봇 학습에 기여할 수 있는 세 가지 잠재적 응용 분야를 탐구합니다.

- **Technical Details**: 연구의 주요 목표는 로봇의 작업 진척도, 인간의 기대치와 정렬, 동역학, 운동 가독성, 최적성 등 다양한 요인들이 인간의 개입 결정에 미치는 영향을 이해하는 것입니다. 연구자는 개입의 타이밍을 통해 로봇의 운동 특징을 식별하고, 개입의 방향과 시작 시점에서 인간의 목표를 신속하게 추론하며, 작업 목표에 대한 보다 정확한 제약 조건을 학습할 수 있는 가능성을 탐구합니다.

- **Performance Highlights**: 연구 결과, 개입 타이밍 정보는 두 가지 응용 분야에서 학습 향상에 기여함을 보여줍니다. 첫째, 운동 특성이 개입 결정에 미치는 영향을 평가하여 인간의 개입을 유도하는 로봇 경로를 설계하는 데 유용할 수 있습니다. 둘째, 개입 시작 정보와 타이밍을 포함함으로써 인간이 의도한 목표의 초기 추론이 개선됩니다.
...
- **What's New**: 이번 연구에서 제안된 Multicategory Linear Log Odds (MCLLO) 보정 방법은 모델에 대한 내부 접근 없이도 멀티카테고리 확률 예측의 보정을 수행할 수 있습니다. 이는 기준 테스트와 쉬운 해석 가능성을 포함하고 있어 다양한 분류 문제에 적용할 수 있는 장점을 가지고 있습니다. 본 연구는 이미지 분류, 비만 분석, 생태학 모델링 등 세 가지 실제 사례를 통해 MCLLO의 효용성을 입증하였습니다.

- **Technical Details**: MCLLO는 우선 국가(Null) 가설과 대립 가설을 기반으로 다중 카테고리의 모델 보정을 평가하는 리히드( likelihood ratio) 가설 검정을 포함합니다. 기존의 보정 방법들이 로그 확률 예측에만 의존하는 것과 달리, MCLLO는 확률 스케일 예측 전반에 대해 작동하여 접근성을 개선하였습니다. 또한 MCLLO는 임의의 분류 기법에서 접근 가능한 확률 출력에 직접적으로 적용할 수 있는 장점을 제공합니다.

- **Performance Highlights**: 시뮬레이션 및 실제 사례 연구를 통해 MCLLO는 다른 보정 기술들과 비교하여 효과적인 성능을 드러냈습니다. 특히 모델의 유효성 측정 시 H0 가설 테스트를 포함하여 단일 모델의 보정 상태를 명확히 파악할 수 있게 해주는 점이 편리합니다. 기존의 기대 보정 오류(Expected Calibration Error, ECE)와 경쟁하여 MCLLO의 성능을 평가할 수 있으며, 다양한 분류 문제에 효과적으로 적용됨을 보였습니다.
...
Comments:
          Accepted to ICASSP 2026. 6 pages, 2 figures, 2 tables

- **What's New**: 이번 연구에서는 개인화된 페더레이티드 학습(PFL)이 새로운 주파수 도메인 접근 방식을 통해 백도어 공격(backdoor attack)에도 여전히 취약하다는 것을 밝혔습니다. 제안된 DCInject는 주파수 도메인에서 적응형 백도어 공격을 수행하며, 낮은 주파수 성분을 제거하고 가우시안 분포의 샘플로 교체하여 작동합니다. 본 백도어 공격은 기존의 스페이셜 도메인 공격에 비해 우수한 공격 성공률을 기록하며, 여러 데이터셋에서 깨끗한 정확도를 유지합니다.

- **Technical Details**: DCInject는 공격 프레임워크를 통해 DC 성분을 조작하여 모든 클라이언트 간에 공유되는 글로벌 피처 표현을 변형합니다. 주파수 도메인 변환은 입력 이미지의 2D 주파수 변환을 사용하며, 적응형 노이즈 생성을 통해 자연 통계량을 보존합니다. 이 공격 방식은 PFL의 개인화 메커니즘을 우회하는 특성을 가지고 있으며, 백도어가 클라이언트 특정 적응을 통해 생존할 수 있도록 설계되었습니다.

- **Performance Highlights**: DCInject는 CIFAR-10, SVHN, GTSRB와 같은 네 가지 데이터셋에서 공격 성공률(ASR) 96.83%(CIFAR-10), 99.38%(SVHN), 100%(GTSRB)를 기록했습니다. 평소의 성능 유지에도 불구하고 I-BAU 방어 기법 하에서도 공격의 지속성이 확인되었으며, 이는 PFL 보안 가정의 중요한 취약점을 드러냅니다.
...
- **What's New**: 본 논문에서는 응집상태 시스템에서의 평형 분포 샘플링을 위한 새로운 접근법을 제시합니다. 기존의 방식보다 훨씬 더 큰 시스템에서 매우 정확한 자유 에너지 추정치를 얻을 수 있는 연속 정규화 흐름(Continuous Normalizing Flows, CNFs)과 리만 유량 정합(Riemannian flow matching)을 결합하여 새로운 프레임워크를 구현합니다. 이 방법은 기존 모델보다 더 높은 샘플 품질과 확장성을 보장합니다.

- **Technical Details**: 리만 유량 정합(RFM)은 주기적 경계 조건이 있는 시스템을 처리하기 위한 CNFs의 향상된 훈련 방식으로, 임의의 분포에 대한 조건부 확률 경로를 통해 벡터 필드를 훈련시킵니다. 이 작업은 이전의 방법보다 더 큰 크기의 단원자 얼음(monatomic ice) 시스템에서 성공적으로 검증되었습니다. 또한, 허치슨의 추정기를 사용하여 정확도 높은 밀도 추정을 가능하게 하고, 스토캐스틱 추정치를 열역학적으로 재가중치할 수 있는 바이어스 수정 단계를 추가했습니다.

- **Performance Highlights**: 제안된 접근법은 훈련된 CNFs가 이전 방식보다 네 배 더 큰 시스템에 대해서도 우수한 샘플 품질을 제공할 수 있음을 보여주었습니다. 자유 에너지 추정의 정확성은 전통적인 다단계 추정기 없이도 수행된다는 점에서 큰 의미를 가집니다. 이를 통해 연구자들은 더 큰 규모의 시스템에서 유의미한 열역학적 관측치를 보다 효율적으로 얻을 수 있게 됩니다.
...
- **What's New**: 본 연구에서는 세계적으로 주요 사망 원인인 호흡기 질환에 대한 조기 및 접근 가능한 검진 방법의 필요성을 강조합니다. 기존의 청진기 분석과 머신러닝 모델을 활용해 호흡기 질환을 예측하는 것에서 한 발 더 나아가, 실시간 상담 시 자연어로 상호작용할 수 있는 지능형 시스템의 필요성을 제기합니다. 이 연구는 11개의 호흡기 오디오 데이터셋에서 수집된 데이터를 기반으로 최초의 Respiratory Audio Question Answering (RA-QA) 데이터셋을 제작하여 임상 오디오와 자연어의 다리를 잇기 위한 새로운 자원을 제공합니다.

- **Technical Details**: RA-QA 데이터셋은 약 750만 개의 QA 쌍을 포함하며 60개 이상의 임상 및 인구 통계적 특성을 아우르고, 질문 유형은 단일 검증, 다중 선택, 개방형 질문으로 나뉩니다. 이 데이터셋은 임상 정보와 관련된 청진 수음 소리를 자연어로 설명하는 구조적 메타데이터와 원시 호흡기 음악 기록을 결합하여, 현실 세계 환경에서 수집된 자료로 구성되어 있습니다. 연구진은 RA-QA 데이터셋을 사용하여 오디오-텍스트 생성 모델과 기존의 오디오 분류 모델을 비교하여 평가할 수 있는 벤치마크를 소개합니다.

- **Performance Highlights**: 실험 결과, 질문 유형 및 특성에 따른 성능 차이가 관찰되었으며, 이는 향후 더 발전된 아키텍처의 개발로 이어질 수 있는 기초를 제공합니다. RA-QA 데이터셋을 기반으로 한 연구는 머신러닝과 실제 임상 대화를 결합하여 호흡기 건강 관리에서의 더 인터랙티브하고 지능적이며 접근 가능한 진단 도구 개발을 위한 새로운 길을 열었습니다. 최종적으로, 본 논문은 다양한 의료 도메인에 적용 가능한 포괄적인 데이터 수집과 벤치마크 개발의 중요성을 강조합니다.

...
### (How) Can Transformers Predict Pseudo-Random Numbers? (https://arxiv.org/abs/2502.10390)
Comments:
          10+16 pages, 12+20 figures

- **What's New**: 이 논문은 Transformer 모델이 선형 합동 생성기(Linear Congruential Generators, LCG)에서 생성된 의사 난수 수열을 학습하는 능력을 연구합니다. 충분한 신경망 아키텍처 용량과 다양한 훈련 데이터를 통해 unseen modulus와 parameters에 대한 예측이 가능하다는 놀라운 결과를 보였습니다. 이 연구는 Transformer가 LCG 수열을 학습하는 알고리즘 구조를 개발하는 방법을 분석함으로써 Transformer의 패턴 학습 능력과 구조적 한계를 조명합니다.

- **Technical Details**: Transformer는 두 가지 복잡도 시나리오에서 LCG 수열을 학습하는 방식을 조사합니다. 첫 번째로, 고정된 modulus에서의 unseen parameters를 가진 LCG 수열을 학습하는 방식은, 모델이 modulus를 분해하고 자리별 숫자 표현을 이용하여 예측을 수행하는 것으로 나타났습니다. 두 번째로, unseen modulus에 대한 예측은 복잡한 두 단계 전략을 사용하여, 먼저 컨텍스트에서 알려지지 않은 modulus를 추정한 후, 소수 분해를 활용하여 예측을 생성하는 방식으로 진행됩니다.

- **Performance Highlights**: 이 연구에서는 Transformer 모델이 깊이가 3일 때 예측 정확도가 급격히 전환되는 현상을 발견했습니다. 특히 높은 정확도를 달성하기 위해 필요한 in-context sequence 요소의 수가 modulus와 비례하여 증가하는 것이 아니라, sublinear 하게 스케일링 되는 현상을 확인했습니다. 이는 Transformer의 효율적인 학습 능력을 나타내며, 다양한 training methodologies와 context length가 모델의 예측 성능에 미치는 영향을 밝히는 데 기여합니다.



### Balancing the Scales: A Theoretical and Algorithmic Framework for Learning from Imbalanced Data (https://arxiv.org/abs/2502.10381)
- **What's New**: 본 연구는 클래스 불균형(class imbalance) 문제를 해결하기 위한 새로운 이론적 프레임워크를 제시합니다. 클래스 불균형을 고려한 새로운 경계 손실(margin loss) 함수가 제안되며, 강력한 H-consistency를 보장함을 입증합니다. 또한, 이론적 성과를 바탕으로 IMMAX(Imbalanced Margin Maximization) 알고리즘을 개발하여 다양한 가설 집합에 적용 가능하도록 했습니다.

- **Technical Details**: 연구의 핵심은 클래스의 불균형을 분석하는 새로운 방법론을 개발하는 것입니다. 이론적 분석을 통해 제안된 손실 함수는 이진 및 다중 클래스 설정 모두에서 강력한 성능을 보이며, 클래스에 민감한 Rademacher 복잡성을 바탕으로 학습 보장을 제공합니다. IMMAX 알고리즘은 신뢰 한계를 포함하여 데이터 샘플링 과정을 보다 정교하게 수행합니다.

- **Performance Highlights**: 실험 결과, 제안된 IMMAX 알고리즘은 기존의 안착 기준(baselines)에 비해 더욱 효과적임을 입증하였습니다. 이 알고리즘은 클래스 불균형으로 인한 문제를 해결하는 데 있어 긍정적 결과를 도출했으며, 이전 방법들과 비교해 확연한 성과 차이를 보여 주었습니다. 이론적 기초가 강력한 만큼 다양한 실제 문제에도 적용 가능성을 보여줍니다.



### AffinityFlow: Guided Flows for Antibody Affinity Maturation (https://arxiv.org/abs/2502.10365)
Comments:
          14 pages, 5 figures

- **What's New**: 본 연구에서는 AffinityFlow라는 새로운 방법론을 제안하여 항체의 affinity maturation 과정을 개선합니다. 기존의 방법들이 시간 소모적이고 고비용인 반면, AffinityFlow는 항체 및 항원 서열만을 사용하여 효율적으로 affinity를 향상시키는 접근법을 사용합니다. 이 방법은 AlphaFlow의 흐름 일치(flow matching) 메커니즘을 활용하여 단백질 구조 생성을 최적화하고, 반복적인 변화를 통해 우수한 성능을 달성합니다.

- **Technical Details**: 이 연구에서는 항체와 항원의 서열 정보만을 이용해 affine maturation을 진행합니다. AlphaFlow를 기반으로 하는 간섭 최적화 프레임워크를 도입하여 구조 생성을 고도화하고, 그에 따른 변이를 통해 다음 반복을 위한 유망한 서열을 선택합니다. 또한, 혼합된 물리적 에너지를 활용하여 두 가지 예측기를 개선하는 공동 교사 모듈(co-teaching module)을 개발하였습니다.

- **Performance Highlights**: AffinityFlow는 affinity maturation 실험에서 최첨단 성능을 보이며, 기존의 방법들보다 더 효율적으로 항체의 결합 친화도를 향상시킬 수 있습니다. 연구진은 이 연구의 피드백을 바탕으로 코드를 오픈소스로 제공할 계획이며, 이는 향후 더욱 많은 연구의 기초가 될 것입니다.



### Proper Learnability and the Role of Unlabeled Data (https://arxiv.org/abs/2502.10359)
Comments:
          ALT 2025, 22 pages

- **What's New**: 이번 논문에서는 proper learning의 개념을 심층적으로 연구하며, 특정 가설 클래스 또는 학습자에게 제공되는 정보에 따라 문제의 proper learnability를 결정할 수 있는 조건을 제시합니다. 특히, unlabeled data distribution이 주어질 경우, 분포 기반 정규화를 통해 최적의 proper learner가 항상 존재함을 보여줍니다. 또한, distribution-fixed PAC 모델을 도입하여, 여러 분포에 대한 최악의 성능을 평가할 수 있는 방법론을 제안합니다.

- **Technical Details**: 본 논문은 metric loss function과 그 크기에 상관없이 모든 유한 학습 문제에 대해 결과가 적용됨을 증명합니다. 또한, distribution-fixed PAC 모델에서의 sample complexity가 고전적인 PAC 모델에서 로그 스케일 요인으로만 감소할 수 있음을 입증합니다. 적절한 학습 가능성을 규명하기 위한 여러 불가능성 결과를 추가로 제시하며, proper learnability가 논리적으로 결정 불가능할 수 있다는 주장을 포함합니다.

- **Performance Highlights**: binary classification과 multiclass classification의 차이를 명확히 하고, multiclass 학습에서 proper learning의 복잡성을 깊이 탐구합니다. 본 연구에서는 proper learnability가 단조성이 아니며, 클래스의 서브셋이나 슈퍼셋에 대해 영속성을 가지지 않음을 보여주는 여러 결과를 통해 proper multiclass learnability characterization의 복잡함을 강조합니다. 이러한 발견들은 기존의 VC, DS 차원 등과는 fundamentally 차별화된 사고방식을 필요로 할 수 있음을 암시합니다.



### Dimension-free Score Matching and Time Bootstrapping for Diffusion Models (https://arxiv.org/abs/2502.10354)
- **What's New**: 본 논문은 확산 모델(difussion models)의 학습 및 샘플 복잡성을 다루며, 기존의 다차원 의존성을 해결하는 새로운 이론적 경계를 제시합니다. 이 연구는 다양한 노이즈 수준에서 점수 함수(score function)를 동시에 추정할 수 있는 단일 함수 근사기(single function approximator)의 사용을 중점적으로 다루고 있습니다. 이러한 접근 방식은 diffussion 모델들이 timesteps를 횡단하여 일반화할 수 있게 합니다.

- **Technical Details**: 연구에서는 단일 함수 근사기를 사용하여 모든 timesteps에서의 점수 함수의 추정 가능성을 논의합니다. 기존의 이론적 경계는 다차원성의 저주(curse of dimensionality)로 인해 polynomial dependency를 보였으나, 본 연구에서는 적절한 매끄러움(smoothness) 조건 하에 로그-로그 성장을 가지는 샘플 복잡성을 달성하였습니다. 이를 통해, 마르코프 과정에서 생성된 의존 데이터로부터 효율적인 학습이 가능하게 하며, 새로운 martingale 기반의 오류 분해(martingale-based error decomposition)와 정확한 분산 경계(sharp variance bounds)를 제시합니다.

- **Performance Highlights**: 본 연구는 Bootstrapped Score Matching (BSM)이라는 새로운 변동성 축소 기법을 제안하여, 이전에 학습된 점수를 이용하여 높은 노이즈 수준에서의 정확도를 향상시킵니다. 실험 결과, 제안된 방법이 기존의 Denoising Score Matching (DSM)보다 개선된 성능을 나타냄을 보여줍니다. 이러한 연구 결과는 생성 모델링(generative modeling)에서 확산 모델의 효율성과 효과에 대한 귀중한 통찰력을 제공합니다.



### InfoPos: A ML-Assisted Solution Design Support Framework for Industrial Cyber-Physical Systems (https://arxiv.org/abs/2502.10331)
- **What's New**: 이번 논문은 데이터 중심의 기계 학습(ML) 기반 솔루션 설계에서 가장 효과적인 구성 요소를 선택할 수 있도록 도와주는 InfoPos 프레임워크의 첫 번째 버전을 소개합니다. 이 프레임워크는 지식 및 데이터 차원의 가용성에 따라 사용 사례의 배치를 가능하게 하며, 이는 솔루션 설계 프로세스를 간소화하는 데 주안점을 두고 있습니다. 연구는 산업 Cyber-Physical Systems(CPS)의 이상 탐지(use-case)에서 다양한 데이터 처리 블록을 통해 달성된 성과를 보여줍니다. 이를 통해 ML 모델의 성능을 지표로 삼고 있으며, 공개된 데이터 처리 코드와 구성된 데이터 세트를 제공합니다.

- **Technical Details**: 이 논문에서 다루는 데이터는 주로 센서에서 수집된 측정 추적(metric traces)을 포함합니다. 특히 현대의 CPS는 제품 품질 제어를 위한 다양한 센서로 장착되어 있으며, 데이터 수집과 모니터링은 신중히 계획되어야 합니다. 정보는 시스템의 작동 순서, 특성, 적용된 구성 등을 포함하는 다양한 인공물에서 출처를 가집니다. 이러한 맥락에서 저자들은 지식 위치(knowledge position)와 데이터 위치(data position)가 ML 기반 솔루션의 설계 및 효과성을 형성하는 두 가지 주요 차원으로 등장한다고 설명합니다.

- **Performance Highlights**: 연구의 본질적인 목적은 ML 알고리즘과 데이터 처리 단계의 조합을 선택하는 데 있어서 각각의 요소가 어떻게 가장 효과적으로 작용하는지를 이해하는 것입니다. 저자들은 정보 기반 세분화(segmentation) 기법을 적용하여 데이터 수집 과정에서의 다양한 시나리오를 분석하였으며, 이를 통해 실전에서 수집된 균형 잡힌 데이터를 활용하였습니다. 이 프레임워크를 활용한 결과, 다양한 조합을 통해 도출된 초기 결과들은 풍부한 예시를 제공하며, 공개된 데이터와 코드들은 연구자들이 이 결과를 재현하고 향후 연구의 기초 자료로 활용할 수 있도록 돕습니다.



### DiOpt: Self-supervised Diffusion for Constrained Optimization (https://arxiv.org/abs/2502.10330)
- **What's New**: 최근 확산 모델(difussion model)의 발전은 학습 기반의 최적화에서 매우 유망한 가능성을 보여줍니다. 그러나 기존의 확산 기반 최적화 접근 방식은 감독 학습(supervised training)에 의존하여 실제 어플리케이션에서 요구되는 엄격한 제약 조건 만족을 보장하는 메커니즘이 부족합니다. 본 논문에서는 DiOpt라는 새로운 확산 패러다임을 제안하며, 이를 통해 최적화 문제의 제약 조건을 충족하는 근사 최적 해(solution distribution)를 체계적으로 학습합니다.

- **Technical Details**: DiOpt는 여러 주요 혁신을 포함하고 있습니다. 첫째, 목표 분포(target distribution)를 설계하여 제약 조건 해표면과의 중첩(overlap)을 극대화합니다. 둘째, 부트스트래핑(self-training) 메커니즘을 도입하여 제약 위반의 심각성과 최적성 간의 차이에 따라 후보 해에 가중치를 할당합니다. 셋째, 훈련 반복 과정에서 고품질 솔루션을 보존하여 수렴(convergence)을 가속화하는 동적 메모리 버퍼를 소개합니다.

- **Performance Highlights**: DiOpt는 전력망 제어, 동작 재타겟팅(motion retargeting), 무선 자원 할당 등 다양한 평가에서 최적성 및 제약 조건 만족 측면에서 우수성을 입증하였습니다. 또한, 이 방법은 합성 문제, 전력망 제어, 무선 전력 할당과 같은 여러 최적화 문제에 대한 광범위한 평가에서 일반화 가능성을 보여주었습니다. 이는 복잡한 제약 조건을 갖춘 최적화 시나리오에서의 능력을 입증합니다.
...
- **What's New**: 이 논문에서는 Fenchel-Young (FY) 손실 함수에 기반한 새로운 변분 메서드의 일반적인 클래스를 소개합니다. 이는 고전적인 변분 학습의 핵심인 Kullback-Leibler divergence를 일반화하고 포괄하는 방식으로, FY 변분 학습(FY variational learning)을 제시합니다. 새로운 FY 자유 에너지, FY 증거, FY 증거 하한 및 FY 사후 분포와 같은 특성을 포함하여 다양한 모델을 학습할 수 있는 새로운 통찰력을 제공합니다.

- **Technical Details**: 논문은 Fenchel-Young 손실을 통한 변분적 후방 추론의 새로운 구조를 제시하며, 이를 바탕으로 FYEM (FY expectation-maximization) 알고리즘과 FY 변분 오토인코더 (FYVAE)와 같은 고전적인 알고리즘의 일반화를 포함합니다. 다채로운 모델에 적합한 유도 방법론이 포함되어 있어, 사후 분포가 희소한 모델을 포함한 다양한 상호작용이 가능합니다. 특히 FYEM 알고리즘은 데이터의 분포에 따라 적응적으로 희소한 E-단계를 생성할 수 있습니다.

- **Performance Highlights**: FYEM 알고리즘과 FYVAE는 실험적으로 경쟁력이 뛰어나며, 고전적인 경쟁 모델들을 종종 초월하는 성능을 보여줍니다. FYVAE는 희소한 관측 모델과 결합함으로써 더욱 향상된 성능을 발휘합니다. 이러한 모델은 스팟 관찰이 있을 때에도 새로운 형태의 기능성을 제공하여, 업계에 새로운 통찰력을 제공합니다.



### Small Loss Bounds for Online Learning Separated Function Classes: A Gaussian Process Perspectiv (https://arxiv.org/abs/2502.10292)
- **What's New**: 이 논문은 oracle-efficient 알고리즘을 통해 온라인 학습(online learning)과 차등 개인 정보 보호(differential privacy)에서 동일한 안정성(stability)을 요구하는 상황을 탐구합니다. 새로운 개념인 ρ-separation을 도입하여 기존 방법에 대한 일반화를 이루며, 알고리즘의 효율성과 성능을 향상시키는 데 기여합니다. 이를 통해 연구자들은 더 넓은 범위의 함수 클래스를 처리하는 안정적인 알고리즘의 개발 가능성을 확인했습니다.

- **Technical Details**: ρ-separation은 함수 클래스 ℱ의 함수들이 L₂(μ) 공간에서 충분히 구별 가능하다는 조건을 제공합니다. 이 식별 능력은 차등 개인 정보 보호의 안정성을 높이는 데 중요한 역할을 합니다. 이를 바탕으로 온라인 학습을 위한 oracle-efficient 알고리즘(제1정리)을 제시하며, 이는 L⋆(L-star) 기준에서 작은 손실을 달성할 수 있게 합니다.

- **Performance Highlights**: 제안된 알고리즘은 기존의 oracle-efficient 알고리즘과 비교했을 때 더 나은 성능을 보여주며, 다양한 설정에서 향상된 결과를 제공합니다. 또한, 해당 알고리즘의 변형은 ρ-separation 조건 아래에서 최적의 속도로 차등 개인 정보 보호를 만족하는 PAC 학습이 가능함을 보여줍니다. 이 외에도 가우시안 프로세스의 최소화기(minimizers)에 대한 새로운 안정성 경계를 증명하여 이전 결과들을 확장하였습니다.
...
- **What's New**: 이 연구에서는 머신 언러닝(machile unlearning) 분야에서 데이터 프라이버시(data privacy)를 위해 중요한 문제인 재해적 언러닝(catastrophic unlearning)을 해결하는 새로운 접근 방식을 제안합니다. 이 방식은 MixUnlearn이라는 생성기-언러너(generator-unlearner) 프레임워크를 기반으로 하여, 합성된 믹스업 샘플(synthesized mixup samples)을 사용하여 언러닝 과정을 정규화함으로써 재해적 효과를 완화합니다. 이를 통해 모델이 중요한 지식을 잃지 않고 목표 정보를 정확히 삭제할 수 있도록 지원합니다.

- **Technical Details**: 제안된 방법의 핵심은 생성기와 언러너가 서로 협력하여 작동하는 것입니다. 생성기는 Forgetting 데이터와 Remaining 데이터를 혼합하여 도전적인 믹스업 샘플을 생성하고, 언러너는 이러한 합성 데이터와 실제 데이터를 함께 처리하여 언러닝을 진행합니다. 특히, 이 과정에서 새로운 대조 손실(contrastive loss) 개념을 도입하여 언러너가 잊어야 할 정보를 효과적으로 처리하도록 유도합니다.

- **Performance Highlights**: 실험 결과, 제안한 방법은 여러 기준 데이터셋에서 기존의 최신 언러닝 기법보다 훨씬 우수한 성능을 보였습니다. 특히, 레이블이 없는 상황에서도 효과적인 언러닝이 가능하여, 약하게 레이블된 데이터셋에서 머신 언러닝 모델을 학습하는 데 있어 큰 이점을 제공합니다. 이 연구는 믹스업 샘플을 활용한 머신 언러닝의 가능성을 강조하며, 관련 코드도 공개되었습니다.



### Probabilistic Super-Resolution for High-Fidelity Physical System Simulations with Uncertainty Quantification (https://arxiv.org/abs/2502.10280)
- **What's New**: 이번 논문에서는 ProbSR이라고 불리는 새로운 베이지안 확률적 프레임워크를 제안하여, 기존의 초해상도(super-resolution, SR) 기법이 갖는 대규모 레이블 데이터 세트 필요성과 신뢰할 수 있는 불확실성 정량화(uncertainty quantification, UQ) 부족을 해결하고자 하였습니다. 이 방법은 통계적 유한요소법(statFEM)과 에너지 기반 생성 모델링(energy-based generative modeling)을 결합하여, 고해상도 예측을 제공하면서도 레이블 데이터 수요를 줄입니다.

- **Technical Details**: 제안된 ProbSR 방법은 고해상도(High-Resolution, HR) 필드의 사전 분포를 제공하기 위해 통계적 유한요소법(statFEM)을 사용합니다. 이는 일반적으로 사용되는 두 번째 차수 타원 편미분 방정식(PDE)의 확률적 설명을 모델링하는데, 가우시안 프로세스를 활용하여 모델의 미스테이시피케이션을 표현합니다. 또한, Langevin dynamics를 통해 복잡한 확률 분포를 효과적으로 샘플링할 수 있는 윤곽을 제공합니다.

- **Performance Highlights**: 2차원 포아송 방정식 예제를 통해 제안된 방법의 효율성과 성능을 검증하였고, 기존의 bicubic interpolation 업스케일과 비교하여 계산 속도를 대폭 향상시켰습니다. 실험 결과, 제안한 방법은 고해상도 수치 해석기보다 더 빠르면서도 신뢰할 수 있는 불확실성 추정을 가능하게 합니다. 이는 엔지니어링 어플리케이션에서 요구되는 고해상도 예측의 신뢰성을 크게 향상시킵니다.
...
### Comparison of Deep Recurrent Neural Networks and Bayesian Neural Networks for Detecting Electric Motor Damage Through Sound Signal Analysis (https://arxiv.org/abs/2502.10224)
Comments:
          Draft articles. arXiv admin note: substantial text overlap with arXiv:2409.08309

- **What's New**: 본 연구는 전기 모터의 고장을 진단하기 위해 Recurrent Neural Networks (RNNs) 및 Bayesian Neural Networks (BNNs)를 활용한 새로운 접근 방식을 제안합니다. 특히, 소리 신호의 주파수 영역 표현(frequency domain representation)을 통해 진단 정확도를 향상시키는 방법을 모색했습니다. 이를 통해 전통적인 방법과 차별화된 고급 진단 기술을 제시합니다.

- **Technical Details**: RNN과 BNN의 아키텍처는 스마트폰을 통해 수집된 가정용 기기에서의 실제 음향 데이터에 대해 설계되고 평가되었습니다. 특히, BNN은 불균형 데이터셋에서 고장 탐지(fault detection) 성능이 우수하며, 전통적인 방법보다 견고하고 해석 가능한 예측을 제공합니다. 이는 산업 진단 애플리케이션에서 BNN의 우수성을 입증하는 중요한 요소입니다.

- **Performance Highlights**: 실험 결과는 BNN이 특히 불균형 데이터셋에 대해 우수한 성능을 보임을 보여줍니다. 자료 효율성(resource efficiency)과 분류 능력(classification capabilities)을 탐구하기 위한 추가 분석과 벤치마크가 필요합니다. 이는 향후 연구를 통해 산업에 최적화된 진단 방안을 제시할 기초가 될 것입니다.
...
### Control-flow anomaly detection by process mining-based feature extraction and dimensionality reduction (https://arxiv.org/abs/2502.10211)
Comments:
          16 pages, 9 figures, 7 tables, 56 references

- **What's New**: 이번 논문에서는 비즈니스 프로세스에서 정상적인 제어 흐름에서 벗어날 수 있는 이상 현상을 효과적으로 탐지하기 위한 새로운 프로세스 마이닝 기반 특징 추출 방법을 제안합니다. 제안된 접근 방식은 기존의 conformance checking(정합성 검사) 기법의 한계를 극복하며, 비즈니스 프로세스 분석의 설명 가능성을 유지하면서 경쟁력 있는 기술을 지원하도록 설계되었습니다.

- **Technical Details**: 제안된 방법은 변형된 alignment-based conformance checking(정렬 기반 정합성 검사) 접근 방식을 사용하여 이상 제어 흐름을 참조 Petri net에 맞춥니다. 이 정렬 결과를 통해 불일치의 원인이 되는 활동 횟수와 같은 추가 통계를 추출할 수 있으며, 이는 high-dimensional feature sets(고차원 특징 집합)을 효과적으로 처리하고 설명 가능성을 지원하는 유연하고 설명 가능한 프레임워크에 통합되어 있습니다.

- **Performance Highlights**: 프레임워크 내의 기술들은 기존의 정합성 검사 기반 기법과 비교했을 때 탐지 효과성이 우수한 결과를 보이며, 정합성 검사 기법의 설명 가능한 특성을 유지합니다. 다양한 데이터 세트에 적용한 결과, 제안한 접근 방식이 기존 기술들보다 성능이 뛰어나며 비즈니스 프로세스의 복잡성에 효과적으로 대응할 수 있음을 입증했습니다.



### SGS-GNN: A Supervised Graph Sparsification method for Graph Neural Networks (https://arxiv.org/abs/2502.10208)
- **What's New**: SGS-GNN은 새로운 종류의 지도(graph) 희소화 기법으로, 사용자 지정 크기의 희소 부분 그래프를 샘플링하여 GNN의 추론 작업에서 발생하는 계산 비용을 줄입니다. 이 방법은 이웃 노드가 상이한 레이블을 갖는 이질성 그래프(heterophilic graphs)에서 GNN의 정확도를 높이기 위해 손실 함수에 정규화(regularizers)를 사용합니다. 특히, SGS-GNN은 고정 분포를 사용하는 기존 방법보다 효과적으로 부분 그래프의 탐색 공간을 학습하여 더 적은 에폭(epoch)으로 높은 정확도를 달성합니다.

- **Technical Details**: 이 논문에서는 SGS-GNN의 다양한 구성 요소가 전체 성능에 기여하는 방식을 조사가 이루어졌습니다. 조건부 업데이트(conditional updates) 메커니즘을 통해 학습한 확률 분포의 업데이트를 최적화하였고, 일반적인 GNN들과의 비교도 진행되었습니다. 매개변수 설정, 정규화 및 샘플링 방법의 차이를 조사하였으며, 이를 기반으로 정규화 계수와 다른 요소가 성능에 미치는 영향을 분석했습니다.

- **Performance Highlights**: 33개의 이질성 및 동질성 그래프를 사용한 광범위한 실험 결과에 따르면, SGS-GNN은 20%의 엣지를 보존하면서도 F1 점수를 기하 평균 4% 개선하였고, 이질성 그래프에서는 예측 정확도가 최대 30% 향상되었습니다. 기존의 최신 기법들에 비해 성능이 4~7% 개선된 점수가 나타났으며, 고정 분포를 사용하는 희소화 기법에 비해 수렴에 필요한 에폭이 약 절반으로 줄어드는 결과가 있었습니다.



### Looking around you: external information enhances representations for event sequences (https://arxiv.org/abs/2502.10205)
- **What's New**: 본 연구에서는 다수의 사용자 표현을 집계하여 특정 사용자 표현을 증강하는 새로운 방법을 제안합니다. 이는 특히 금융과 같이 외부 환경이 급변하는 분야에서 다수의 공존하는 이벤트 시퀀스를 고려하는 시나리오에 적합합니다. 제안하는 방법은 기존의 인코더 위에서 작동하며, 추가적으로 효율적인 파인 튜닝을 지원합니다. 여러 데이터셋에서 Kernel attention이 ROC AUC 점수를 개선하는 결과를 보였습니다.

- **Technical Details**: 연구는 다양한 집계 접근 방식, 단순 풀링 기법부터 학습 가능한 어텐션 기반 접근 방식까지 다룹니다. 특히, Kernel attention 집계는 다른 사용자로부터의 복잡한 정보 흐름을 강조할 수 있습니다. 방법론적으로, 'self-supervised representation learning'을 적용하여 레이블 없는 이벤트 시퀀스에서 강력하고 보편적인 표현을 생성하는데 기여합니다. 전체적으로 모델의 품질과 성능을 향상시키는 외부 정보 집계 방법을 제시합니다.

- **Performance Highlights**: 실험을 통해 외부 맥락 벡터의 추가가 하위 작업의 성능을 크게 향상시킨다는 점을 보여주었습니다. Kernel attention 방법이 다른 방법들에 비해 성능 이점을 가지며, 풀링 기법보다도 더 큰 성과를 거두었습니다. 이러한 내용은 Figure 1에서 확인할 수 있으며, 제안하는 방법의 우수성을 입증합니다.



### AI-in-the-Loop Sensing and Communication Joint Design for Edge Intelligenc (https://arxiv.org/abs/2502.10203)
- **What's New**: 본 논문에서는 인공지능(AI), 무선 통신 및 감지 기술의 최근 발전을 바탕으로 엣지 인텔리전스의 진화에 기여하는 혁신적인 프레임워크를 제안합니다. 제안된 프레임워크는 AI가 포함된 순환 제어 아키텍처를 통해 시스템 자원을 공동 최적화하며, 시스템 차원에서의 성능을 획기적으로 향상시킵니다. 또한 검증 손실(validation loss)과 시스템의 조정 가능한 매개변수 간의 명확한 관계를 설정하여 모델 일반화 오류를 동적으로 줄일 수 있는 기회를 제공합니다.

- **Technical Details**: 본 연구에서는 JSAC(수행 채널 및 통신) 프레임워크에서 AI를 활용한 적응형 데이터 수집 전략을 도입하여 엣지 장치가 실시간 모델 피드백에 따라 데이터 수집 종료 결정과 샘플 가중치 할당을 자율적으로 수행할 수 있도록 합니다. 또한 확률적 경량 그라디언트 동역학(SGLD)을 기반으로 하여 전송 전력과 배치 크기의 공동 최적화를 진행하며, 이를 통해 채널 및 데이터 노이즈를 경량화된 그라디언트 왜곡으로 변환하여 과적합을 완화합니다.

- **Performance Highlights**: 실험 결과에 따르면 제안된 프레임워크는 통신 에너지 소모를 77%까지 줄이고, 수집된 샘플 수로 측정된 감지 비용을 52%까지 감소시켰으며 모델 일반화를 획기적으로 개선하여 최종 검증 손실을 최대 58% 감소시켰습니다. 이러한 결과는 AI와 JSAC 시스템의 상호 이익을 받아들여 모델 자체를 제어 루프에 통합하는 것이 얼마나 효과적인지를 뒷받침합니다.
...
### A Powerful Random Forest Featuring Linear Extensions (RaFFLE) (https://arxiv.org/abs/2502.10185)
- **What's New**: RaFFLE (Random Forest Featuring Linear Extensions)는 기존 랜덤 포레스트의 한계를 극복하기 위해 개발된 새로운 프레임워크입니다. 이 프레임워크는 최근 개발된 PILOT 트리(Piecewise Linear Organic Tree)를 기본 학습기로 통합하여 더 나은 예측 정확성을 제공할 수 있습니다. RaFFLE을 통해 고정밀 회귀 분석이 가능하며, 복잡한 데이터 구조를 효과적으로 처리할 수 있습니다. 이를 통해 다양한 회귀 문제에 대한 유연한 도구로 자리 잡을 수 있습니다.

- **Technical Details**: RaFFLE은 PILOT 트리를 기본 학습기로 사용하는 랜덤 포레스트로, 이는 전통적인 결정 트리의 계산 효율성에 선형 모델 트리의 유연성을 결합한 것입니다. 모델 간 다양성을 확보하기 위해 조정 가능한 정규화 매개변수를 도입하고 노드 수준의 특징 샘플링을 사용합니다. 이러한 방식은 트리의 정확성을 개선할 뿐만 아니라 약한 조건에서도 일관성을 보장하는 이론적 근거를 제공합니다. 또한, 라플(Forests)의 수렴 속도가 더 빠른 것도 입증되었습니다.

- **Performance Highlights**: RaFFLE은 136개의 회귀 데이터셋에서 비교 평가된 결과, 기존의 CART 및 RAND(랜덤) 포레스트 방법, Lasso 및 Ridge와 같은 정규화된 선형 방법, 그리고 최신 XGBoost 알고리즘까지 초월하는 성능을 보여주었습니다. 각 데이터셋의 특성에 따라 RaFFLE의 뛰어난 정확도가 돋보이며, 다양한 회귀 문제 해결에 있어 탁월한 효율성을 제공합니다. 이로 인해 RaFFLE은 예측 정확성과 계산 효율성을 모두 겸비한 다재다능한 도구로 자리매김하게 되었습니다.



### Realistic Evaluation of Deep Partial-Label Learning Algorithms (https://arxiv.org/abs/2502.10184)
Comments:
          ICLR 2025 Spotlight

- **What's New**: 이 논문에서는 기존의 Partial-label learning (PLL) 알고리즘들이 간과된 중요한 문제들을 다룹니다. 특히, 모델 선택(model selection)과 실험 설정(experimental settings)의 불일치, 그리고 실세계 이미지 데이터셋의 부족을 지적합니다. 이러한 문제들을 체계적으로 분석하고 최초로 PLENCH라는 PLL 벤치마크를 제안하여, 현대의 deep PLL 알고리즘들을 비교할 수 있는 기준을 제시합니다.

- **Technical Details**: PLL에서 각 훈련 예제는 여러 후보 레이블(candidate labels)과 연관되며, 진정한 레이블은 이 후보 집합에 숨겨져 있습니다. 이 문제를 다루기 위한 깊은 신경망(deep neural networks)에 기반한 알고리즘들이 문헌에 많이 나타나지만, 제안된 PLENCH는 모델 선택 기준과 실용적인 평가를 위한 새로운 데이터셋인 Partial-Label CIFAR-10 (PLCIFAR10)을 포함합니다. PLCIFAR10은 Amazon Mechanical Turk에서 수집된 인간 주석(partial labels) 데이터를 기반으로 합니다.

- **Performance Highlights**: PLENCH를 통해 연구자들은 PLL 알고리즘을 체계적이고 공정하게 평가할 수 있는 기회를 가집니다. 실험을 통해 복잡한 알고리즘보다 간결한 알고리즘이 더 나은 성능을 발휘할 수 있다는 결과도 도출됩니다. 또한, 현재 모든 상황에서 최상의 성능을 보이는 알고리즘은 존재하지 않으며, 조건에 따라 알고리즘 설계가 조정되어야 한다는 점도 강조됩니다.
...
### Provably Efficient RL under Episode-Wise Safety in Linear CMDPs (https://arxiv.org/abs/2502.10138)
- **What's New**: 본 논문에서는 제약이 있는 마르코프 결정 과정(constrained Markov decision process, CMDP)에서 강화 학습(reinforcement learning, RL) 문제를 탐구합니다. 에이전트는 예상 누적 보상을 극대화하면서 각 에피소드에서 예상 총 유틸리티 값에 대한 단일 제약을 만족해야 합니다. 이 문제는 테이블 설정(tabular setting)에서는 잘 이해되고 있지만, 함수 근사를 위한 이론적 결과는 부족했습니다.

- **Technical Details**: 이 논문에서는 선형 CMDPs에 대한 강화 학습 알고리즘을 제안합니다. 이 알고리즘은 에피소드별로 제로 위반(zero-violation) 보장을 제공하며, $	ilde{	ext{O}}(	ext{sqrt}(K))$의 후회(regret)를 달성합니다. 또한, 우리의 방법은 문제 의존적 매개변수에 대해 다항적으로 확장 가능하며, 상태 공간 크기와는 무관한 계산 효율성을 자랑합니다.

- **Performance Highlights**: 이번 연구의 결과는 최근의 선형 CMDP 알고리즘들에 비해 큰 향상을 보여주며, 과거의 방법들은 종종 제약을 위반하거나 계산 비용이 기하급수적(exponential)으로 증가하는 경향이 있습니다. 이는 제약 조건을 만족하면서도 더 나은 성능을 제공하는 알고리즘 개발의 새로운 퍼즐을 제시합니다.
...
- **What's New**: 본 논문에서는 현대 Hopfield 네트워크(HNs)와 Transformer 주의 메커니즘 간의 관계를 새롭게 정립하고, 이를 기반으로 대규모 메모리를 효과적으로 압축하는 새로운 접근 방법을 제안합니다. 심리학 이론인 작업 기억에서의 지속적인 신경 자원 할당 개념을 활용하였으며, 전통적인 메모리 저장 방식을 재정의하여 메모리 효율성을 개선할 수 있는 기회를 모색하였습니다.

- **Technical Details**: 제안하는 방식은 기존의 Hopfield 네트워크의 업데이트 규칙을 개조해 연속적인 주의 메커니즘을 도입한 것입니다. 이로 인해 메모리 업데이트가 전통적인 softmax 기반 확률 질량 함수 대신 연속적인 확률 밀도 함수(probability density function, PDF)를 사용하여 수행됩니다. 이 접근은 기억력과 주의력 간의 심리학적 연결을 보다 명확하게 잘 설명해 주며 새로운 메모리 처리 프레임워크를 제시합니다.

- **Performance Highlights**: 실험 결과, 본 연구에서 제안한 메커니즘은 기존 HN과 동일한 수준의 기억 검색 성능을 보여주면서도 메모리를 압축하여 사용할 수 있음을 입증하였습니다. 따라서 메모리 집합체 모델에 있어 계산 비용을 줄이고, 보다 효율적인 메모리 사용 방향성을 제공하는 중요한 발전이 될 것입니다.



### SeWA: Selective Weight Average via Probabilistic Masking (https://arxiv.org/abs/2502.10119)
- **What's New**: 이 논문은 Selective Weight Averaging (SeWA)라는 새로운 알고리즘을 제안하여 모델 성능을 개선하는 방법을 다룹니다. 기존의 Stochastic Weight Averaging (SWA)나 Latest Weight Averaging (LAWA) 같은 방법들은 수동으로 설계된 절차로 인해 많은 시간과 노력이 필요했습니다. SeWA는 최종 훈련 단계에서 체크포인트를 적응적으로 선택하여 평균을 내는데, 이를 통해 더 나은 일반화와 빠른 수렴을 달성할 수 있습니다.

- **Technical Details**: SeWA는 훈련 과정의 마지막 단계에서 k크기의 구간을 선택하고, 해당 구간 내에서 몇 개의 체크포인트를 평균 내는 방법으로 설계되어 있습니다. 이 과정은 binary variables의 비미분 가능성을 해결하기 위해 Gumbel-Softmax estimator를 사용하여 확률적 최적화 프레임워크로 변환됩니다. 이 알고리즘은 안정성 기반의 일반화 경계를 이론적으로 도출하며, 이는 다른 알고리즘들보다 더 날카로운 성능을 보입니다.

- **Performance Highlights**: SeWA는 이미지 분류, 텍스트 분류, 행동 클로닝을 포함한 다양한 도메인에서 실험을 통해 그 효과성이 입증되었습니다. 이 방법은 단 몇 개의 선택된 포인트만을 사용해서 유사한 성능을 내고, 수십 배 더 많은 포인트가 필요한 기존 방법들과 비교했을 때 동등하거나 더 나은 성능을 보여줍니다. 또한 이 알고리즘은 수동 설계 방법에 대한 의존도를 최소화하여 모델의 강력한 일반화와 빠른 수렴을 보장합니다.



### Accelerometry-based Energy Expenditure Estimation During Activities of Daily Living: A Comparison Among Different Accelerometer Compositions (https://arxiv.org/abs/2502.10112)
Comments:
          This work has been submitted to the IEEE for possible publication

- **What's New**: 이번 연구는 PAEE(Physical Activity Energy Expenditure)를 측정하기 위해 COM(center of mass) 기반과 손목 기반의 설정을 비교하였습니다. COSMED K5로 측정한 호흡 데이터를 기준으로 하고, 여러 위치에 착용된 가속도계의 성능을 평가하여 결과를 도출하였습니다. 주요 궁금증은 단일 센서와 멀티 센서를 통해 COM 가속도를 얼마나 잘 나타낼 수 있는지에 대한 것이었습니다.

- **Technical Details**: 연구에서는 30Hz로 5개의 신체 위치(왼쪽 손목, 오른쪽 손목, 왼쪽 허벅지, 오른쪽 허벅지, 골반)에서 3축 가속도 데이터를 수집하였습니다. 기본 대사율(RMR)을 추정하기 위해, 조용한 휴식 상태에서 얻은 호흡 데이터를 평균하여 RMR을 계산하고, 이후 ADL(Activities of Daily Living)을 수행하여 TEE(Total Daily Energy Expenditure)의 변수를 분석했습니다. 두 가지 모델(Linear Regression, CNN-LSTM)을 사용하여 PAEE를 추정하였습니다.

- **Performance Highlights**: 연구의 결과, COM 기반의 3-acc 설정에서 최고의 성능을 내었으며, LR 모델은 결정계수 $R^2$ = 0.41, CNN-LSTM 모델은 $R^2$ = 0.53을 기록했습니다. 손목 기반 설정(l-wrist-acc, r-wrist-acc)은 PAEE 예측에서 유의미한 성과를 내지 못했으며, 두 COM 기반 설정은 통계적으로 유의미한 차이를 보였습니다(p-values $<$ 0.05). 이는 팔찌 단일 센서보다 COM에 근접한 위치에서 다수의 센서를 사용하는 것이 더 효과적임을 시사합니다.



### COMBINEX: A Unified Counterfactual Explainer for Graph Neural Networks via Node Feature and Structural Perturbations (https://arxiv.org/abs/2502.10111)
- **What's New**: 이 논문에서는 새로운 GNN 설명 기법인 COMBINEX를 제안합니다. COMBINEX는 노드 특성과 그래프 구조를 동시에 조정하여 카운터팩추얼 설명을 생성할 수 있습니다. 이전의 접근법과는 달리 구조적 변화와 특성 기반 변화를 독립적으로 처리하지 않고, 이 두 가지 perturbation을 공동 최적화하는 방식을 채택했습니다.

- **Technical Details**: COMBINEX는 GNN 모델에 대한 카운터팩추얼 설명을 생성하기 위해 stochastic optimization 프레임워크를 도입합니다. 이는 모델의 예측을 변경하기 위해 필요한 최소한의 perturbation을 효율적으로 찾고, 설명의 해석 가능성을 유지합니다. 이 방법은 연속형 및 이산형 노드 특성을 모두 처리할 수 있어 다양한 데이터세트와 GNN 아키텍처에 대한 적합성을 높입니다.

- **Performance Highlights**: 실제 데이터세트와 다양한 GNN 아키텍처에 대한 광범위한 실험 결과, COMBINEX는 기존의 카운터팩추얼 설명 기법에 비해 유효성(validity), 충실성(fidelity), 희소성(sparsity) 모든 면에서 우수한 성능을 나타냈습니다. 이러한 결과는 COMBINEX가 노드 특성을 수정함으로써 설명의 품질을 향상시킬 수 있음을 보여줍니다.



### NeuroXVocal: Detection and Explanation of Alzheimer's Disease through Non-invasive Analysis of Picture-prompted Speech (https://arxiv.org/abs/2502.10108)
- **What's New**: 본 연구에서는 알츠하이머병(Alzheimer's Disease, AD)의 조기 진단을 위한 전이학습 기법과 기계학습 방법론을 결합한 NeuroXVocal 시스템을 제안합니다. NeuroXVocal은 음성 분석을 통해 AD 사례를 분류하고 설명할 수 있는 새로운 이중 성분 시스템으로, 기존 방법에 비해 95.77%의 높은 정확도를 기록하여 최신 기술을 선도합니다. 이 시스템은 설명 가능성(Explainability)을 제공하는 XVocal 컴포넌트를 포함하여, 의학적 결정 지원 도구로서의 잠재력을 보여줍니다.

- **Technical Details**: NeuroXVocal 시스템은 AD 탐지를 위한 멀티모달 음성 데이터 분석을 통해 기능을 발휘하는 Neuro 클래시파이어와 임상적으로 해석 가능한 설명을 생성하는 XVocal 설명기 두 가지 주요 구성 요소로 이루어져 있습니다. 이 시스템은 입력된 오디오 샘플에서 3가지 고유한 데이터 스트림을 추출하며, Acoustic features, Speech embeddings, Textual features를 포함합니다. 이러한 데이터는 커스터마이즈된 트랜스포머 기반의 아키텍처에서 융합되어 강력한 크로스모달 상호작용을 가능하게 합니다.

- **Performance Highlights**: NeuroXVocal 시스템은 IS2021 ADReSSo Challenge 벤치마크 데이터셋에서 95.77%의 정확도로 AD 분류에서 최신 성능을 달성했습니다. 시각화된 결과는 의료 전문가들이 완료한 구조화된 질문지를 통해 질적으로 평가되었으며, XVocal의 임상적 적합성을 검증하는 데 기여했습니다. 이 연구는 높은 정확도의 분류와 문헌 기반의 해석 가능한 설명을 결합하여, 임상 AD 진단을 지원하는 실용적인 도구로서의 잠재력을 제시합니다.



### Data-Adaptive Low-Rank Sparse Subspace Clustering (https://arxiv.org/abs/2502.10106)
Comments:
          5 pages, 1 figure, 1 table

- **What's New**: 이번 논문에서는 데이터 적응형 (data-adaptive) 대체 시스템을 통합한 저순위 희소 서브스페이스 클러스터링 (Low-Rank Sparse Subspace Clustering, LRSSC) 알고리즘을 제안합니다. 기존의 접근방식은 Sp/Lp 노르 (norm)에 기반한 근접 연산자 (proximal operators)에 의존하여 데이터 구조에 적응하지 못했습니다.

- **Technical Details**: 제안된 LRSSC 알고리즘은 근접 매핑 프레임워크 (proximal mapping framework) 내에서 공식화되며, 분석적 표현이 없는 경우에 대한 수치적 해결책을 제공합니다. 우리는 해당 알고리즘의 전역 수렴 (global convergence)에 대한 이론적 증명을 제시합니다.

- **Performance Highlights**: 세 가지 잘 알려진 데이터셋에 대한 성능 평가를 통해, Sp/Lp 제약에 의해 제한된 기존의 LRSSC 알고리즘과 비교하였습니다. 결과적으로 제안된 방법이 기존 방법들에 비해 데이터 구조를 더 잘 캡처 (capture)함을 보여주었습니다.



### Representation Learning on Out of Distribution in Tabular Data (https://arxiv.org/abs/2502.10095)
- **What's New**: TCL(Tabular Contrastive Learning)은 표 형식 데이터에 최적화된 경량 솔루션으로, 다양한 분류 작업에서 기존의 모델보다 우수한 성능을 보여줍니다. 또한, 표준 CPU 하드웨어에서도 효과적으로 작동하여 이를 사용하는 모든 사용자에게 접근성을 높입니다. 이 연구는 OOD(out-of-distribution) 데이터 감지와 평가를 위한 실용적인 가이드를 제공하며, 간단한 실험과 시각화를 통해 활용할 수 있습니다.

- **Technical Details**: TCL은 대칭 손실(contrastive loss) 원칙을 채택하여 전체 행렬 증강(full matrix augmentation)과 단순화된 손실 계산(simplified loss calculation)을 통합합니다. 이 방법론은 기존의 고급 머신러닝 알고리즘들이 요구하는 복잡한 하드웨어 요건을 극복할 수 있도록 설계되었습니다. 본 연구에서 10개의 다양한 데이터셋에 걸쳐 TCL의 성능을 평가하였으며, 그 결과 FT-Transformer와 ResNet을 포함한 여러 모델보다 우수함을 입증하였습니다.

- **Performance Highlights**: TCL은 회귀 문제에서 경쟁력 있는 성능을 유지하면서도 계산 요구 사항이 크게 감소하여, 제한된 하드웨어를 가진 사용자에게 특히 유용합니다. 실험 결과, TCL은 기존 모델과 비교할 때 속도/정확도 트레이드오프 순위가 더 높았으며, 이는 많은 머신러닝 연구자들이 OOD 데이터를 다룰 때 신뢰할 수 있는 성능을 제공함을 의미합니다. 이러한 결과는 특히 계산적 제약이 있는 일반 머신러닝 실무자들에게 큰 도움이 됩니다.
...
### Classification of Temporal Graphs using Persistent Homology (https://arxiv.org/abs/2502.10076)
- **What's New**: 이 논문은 Persistent Homology(PH)를 이용하여 Temporal Graph를 분석하는 새로운 방법을 제안합니다. 기존의 정적 그래프 분석 도구와 달리, 동적 시스템을 효과적으로 모델링할 수 있게 해줍니다. 이 방법은 δ-temporal motifs(재발하는 부분 그래프)를 사용하여 시간의 변화를 캡처하며, 노드 클래스에 의존하지 않으므로 보다 유연한 분석이 가능합니다.

- **Technical Details**: 본 방법은 평균 필터링(average filtration)을 정의하고, 이를 통해 관련된 clique complex 위에 PH를 적용합니다. 이는 전역(topological) 구조와 함께 시간적(dynamic) 구조를 동시에 포착할 수 있는 안정적인 방법을 제공합니다. 필터링은 그래프의 상호작용 빈도와 속도에 따라 변하는 구조를 추적하여, 일반적인 지속성 다이어그램(persistence diagram)을 생성합니다.

- **Performance Highlights**: 실험 결과, 이 접근법은 92% 이상의 정확도를 달성하였으며, 특정 경우에서는 100%에 도달했습니다. 기존의 방법과 달리 노드 클래스가 필요 없으며, 이에 따라 다양한 크기의 Temporal Graph를 분류할 수 있는 일반화된 솔루션을 제공합니다. 이로 인해 복잡한 동적 시스템의 분석에 더 적합한 프레임워크를 갖추게 되었습니다.



### Heterogeneous Resource Allocation with Multi-task Learning for Wireless Networks (https://arxiv.org/abs/2502.10027)
- **What's New**: 본 논문에서는 다양한 최적화 문제를 동시에 해결하기 위한 멀티태스크 학습(multi-task learning, MTL) 프레임워크를 제안합니다. 기존의 DNN(Deep Neural Networks)은 문제의 크기, 목표 또는 제약 사항이 변경될 때 조정이 필요하거나 새로 재훈련되어야 할 수 있습니다. 제안하는 방식은 서로 다른 목표와 제약을 가진 최적화 문제를 독립적인 작업으로 간주하여 공동으로 해결할 수 있도록 합니다.

- **Technical Details**: 이 MTL 프레임워크는 조건부 계산 기반의 접근법을 사용하며, 두 가지 주요 구성 요소로 이루어져 있습니다: 기본 DNN(base DNN, bDNN)과 라우팅 DNN(routing DNN, rDNN)입니다. bDNN은 모든 최적화 문제의 솔루션을 추출하기 위해 사용하는 단일 DNN이며, rDNN은 각 작업의 전파 과정에서 어떤 노드와 레이어를 사용할지를 관리합니다. rDNN의 출력은 이진 벡터로, 이는 bDNN의 모든 가중치와 곱해져 각 작업에 대한 고유한 계산 경로를 생성합니다.

- **Performance Highlights**: 제안하는 MTL 접근법은 다양한 최적화 문제를 해결하는 데 효율성을 입증하였습니다. rDNN 메커니즘이 없는 기존 기준 DNN들은 비슷한 성능 수준을 달성하지 못해, 제안된 아키텍처의 효과성을 강조합니다. 이 프레임워크는 감독(supervised) 및 비감독(unsupervised) 학습 시나리오 모두를 지원하며, 사용 효율성과 성능 측면에서 우수한 결과를 보였습니다.



### Exploring Neural Granger Causality with xLSTMs: Unveiling Temporal Dependencies in Complex Data (https://arxiv.org/abs/2502.09981)
- **What's New**: 새롭게 제안된 GC-xLSTM 모델은 복잡한 시계열 데이터 내 Granger 인과 관계를 발견하는 데 중점을 두고 있습니다. 이 모델은 기존의 Granger 인과 분석 방법의 한계를 극복하기 위해 Extended Long Short-Term Memory (xLSTM) 아키텍처를 활용하며, 독창적인 dynamic lasso 패널티를 도입하여 모델의 희소성을 강화합니다. 이러한 접근은 비선형 종속성을 갖는 시계열의 장기 관계를 효과적으로 포착할 수 있는 잠재력을 제공합니다.

- **Technical Details**: GC-xLSTM 모델은 xLSTM의 구조를 바탕으로 여러 시계열 요소 간의 희소성을 강제로 설정하며, 각 시계열 구성 요소는 별도의 xLSTM 모델로 모델링됩니다. 이 방식은 Granger 인과 관계를 보다 해석 가능하게 발견하도록 도와줍니다. 모델은 예측 결과에 대해 중요 특징을 부각시키고 덜 중요한 특징은 축소하는 연합 최적화 기법을 사용하여, 시계열 간의 복잡한 관계를 포착하는 데 초점을 맞추고 있습니다.

- **Performance Highlights**: 세 개의 데이터셋에 대한 실험 평가 결과, GC-xLSTM 모델은 강력하고 안정적인 Granger 인과 관계를 추구하는데 효과적임을 보여주었습니다. 이 모델은 소음에 민감하지 않은 Robust한 성능을 유지하며, 예측 모델을 공동으로 개선하고 엄격한 희소성을 자율적으로 적용하는 기능을 갖추고 있습니다. 이러한 혁신적인 접근 방식 덕분에 GC-xLSTM은 기존의 비선형 방법들보다 뛰어난 성능을 발휘할 것으로 기대됩니다.
...
### On Space Folds of ReLU Neural Networks (https://arxiv.org/abs/2502.09954)
Comments:
          Accepted at Transactions on Machine Learning Research (TMLR), 2025

- **What's New**: 최근 연구는 ReLU 신경망의 연속적 층이 입력 공간의 기하학적 변형으로 이해될 수 있으며, 이는 자기 유사성의 패턴을 드러낸다고 제안했습니다. 본 논문에서는 ReLU 신경망의 이러한 공간 접기(space folding) 현상을 처음으로 정량적으로 분석합니다. 우리는 유클리드 입력 공간의 직선 경로가 해밍 활성화 공간에서 어떻게 매핑되는지 살펴보며, 이 과정에서 직선의 볼록성이 일반적으로 상실된다는 것을 발견합니다.

- **Technical Details**: 직선 경로의 볼록성 상실을 정량화하기 위해 우리는 임의의 걷기(random walks) 연구에서 사용되는 범위 메트릭(range metrics)을 기반으로 한 새로운 측정을 도입합니다. 우리는 입력 공간(유클리드)과 활성화 공간(해밍) 간의 볼록성 개념의 동등성을 증명하고, ReLU 신경망의 활성화 공간에서 지역적인 볼록성의 편차를 정량화하는 방법을 제시합니다. 이러한 분석은 활성화 패턴의 위상적 조사를 기반으로 하며, 경로를 따라 걸을 때 볼록성이 보존되지 않는 현상을 보여줍니다.

- **Performance Highlights**: 우리는 CantorNet과 MNIST 데이터셋을 사용하여 제안한 측정 방법의 행동을 실험적으로 조사했습니다. 결과적으로, 접힘 값이 크게 변화하지는 않지만, 접힘 효과를 나타내는 서로 다른 숫자 간의 경로 비율이 유의미하게 증가하는 것을 발견했습니다. 이러한 발견은 ReLU 신경망이 입력 정보를 처리하는 방식을 이해하는 데 중요한 통찰력을 제공합니다.
...
### Fused Partial Gromov-Wasserstein for Structured Objects (https://arxiv.org/abs/2502.09934)
Comments:
          arXiv admin note: text overlap with arXiv:2402.03664

- **What's New**: 이번 연구에서는 Fused Gromov-Wasserstein (FGW) 거리의 한계를 극복하기 위해 Fused Partial Gromov-Wasserstein (FPGW) 프레임워크를 제안합니다. FPGW는 데이터의 불균형을 수용할 수 있도록 기존의 질량 제약을 완화합니다. 실험에 따르면, FPGW는 아웃라이어 노이즈가 있는 이상의 데이터에서 강력한 성능을 보입니다.

- **Technical Details**: FPGW는 기본적인 Gromov-Wasserstein 문제의 확장으로, 서로 다른 총 질량을 가진 구조화된 객체를 비교할 수 있도록 설계되었습니다. 이론적으로 FPGW와 FGW의 관계를 확립하고, FPGW의 메트릭 속성을 증명했습니다. 더불어, FPGW 문제를 해결하기 위한 Frank-Wolfe 알고리즘을 제안하고, FPGW barycenter 및 관련 계산 솔버들도 소개합니다.

- **Performance Highlights**: FPGW를 그래프 분류 및 클러스터링 실험에 적용한 결과, 아웃라이어가 포함된 데이터에서 더 나은 성능을 나타냈습니다. 특히, FGW의 비슷한 기존 연구와 비교할 때 FPGW는 데이터의 불균형적 특성을 잘 처리할 수 있음을 보여주었습니다. 이 연구는 구조적 데이터 분석에 있어 새로운 방향을 제시합니다.



### Robust Anomaly Detection via Tensor Chidori Pseudoskeleton Decomposition (https://arxiv.org/abs/2502.09926)
- **What's New**: 본 논문은 Tensor Chidori pseudoskeleton decomposition을 활용하여 고차원 텐서 데이터에서의 이상 탐지(anomaly detection)를 개선하기 위한 새로운 접근법을 제안합니다. 기존 방법들이 고차원 데이터에 적용될 때 발생하는 문제들을 해결하기 위해, 저자들은 sparse anomaly를 분리하여 안정성을 높이는 방법론을 도입하였습니다. 이를 통해 고차원 데이터에서 특징 추출의 효율성을 개선한 결과를 보여줍니다.

- **Technical Details**: Tensor Chidori pseudoskeleton decomposition은 고차원 및 복잡한 의존성을 가진 텐서 데이터에서 정보를 선택적으로 추출합니다. 본 논문에서는 Tucker decomposition을 기반으로 한 Robust Principal Component Analysis 알고리즘을 제안하며, 이는 sparsity 및 regularization constraints를 통합하여 이상값에 대한 민감성을 감소시킵니다. 이러한 접근법은 고차원 데이터의 구조적 통찰을 제공하며, 텐서 데이터 처리에 있어 컴퓨팅 효율성을 유지합니다.

- **Performance Highlights**: 뉴욕시 taxi trip records의 실제 시계열 데이터를 활용한 수치 실험 결과, 제안된 방법이 기존 벤치마크 방법들에 비해 도시 이벤트 이상 탐지에서 뛰어난 성능을 보임을 확인하였습니다. 논문의 결과는 Tensor Chidori pseudoskeleton decomposition이 대규모 고차원 데이터에서 이상 탐지의 정확성과 안정성을 높이는 잠재력을 지닌다는 점을 강조합니다.
...
### Thompson Sampling for Repeated Newsvendor (https://arxiv.org/abs/2502.09900)
- **What's New**: 이번 연구에서는 온라인 학습에서의 센서링된 피드백(censored feedback) 문제를 탐색하며, 전통적인 뉴스벤더 모델을 중심으로 밝혀진 최적 알고리즘인 톰슨 샘플링(Thompson Sampling, TS)의 적용 가능성을 입증하고 있습니다. TS는 과거 주문의 양이 충분할 경우 수요 파라미터를 정확히 추정하는 등 탐색(exploration)과 활용(exploitation)의 균형을 잘 맞추는 알고리즘으로 제시됩니다.

- **Technical Details**: 연구에서 수요는 Weibull 분포를 기반으로 모델링하며, Gamma prior를 사용하여 TS를 초기화합니다. 이는 재고 수량을 동적으로 조정하는 효과적인 방법으로, TS의 성능을 향상시킵니다. 우리의 분석을 통해 TS의 빈번한(regret) 분석이 이루어지며, 소규모 과거 주문이 있을 경우 TS가 미래 주문량을 자동으로 증가시켜 추가적인 수요 정보를 수집할 수 있도록 한다는 사실이 드러났습니다.

- **Performance Highlights**: 광범위한 수치 테스트를 통해 TS는 보수적인 기존 접근 방식들인 온라인 볼록 최적화(online convex optimization), 상한 신뢰 구간(upper confidence bounds) 및 단기 Bayesian 동적 프로그래밍(myopic Bayesian dynamic programming)보다 우수한 성능을 보여주었습니다. 이 연구는 TS의 효과성을 입증할 뿐만 아니라, 센서링된 피드백을 포함한 여러 가지 온라인 학습 문제를 탐구할 수 있는 기초를 마련합니다.



### Optimal lower Lipschitz bounds for ReLU layers, saturation, and phase retrieva (https://arxiv.org/abs/2502.09898)
Comments:
          22 pages

- **What's New**: 이 논문은 ReLU 레이어의 injectivity, 클립 또는 포화 측정에서 벡터 복구, 그리고 실수 위상 회수 문제를 통합된 관점에서 재조명하고 있습니다. 저자들은 이 세 가지 문제의 유사한 문제 정의와 성격을 제시하고, ReLU 레이어와 클리핑에 대해 낮은 Lipschitz 경계를 도출하여 기존의 위상 회수 결과와 유사하며 상수 계수까지 최적화된 결과를 제공합니다.

- **Technical Details**: 논문에서 다루는 측정 및 활성화의 계산 단계는 선형 연산자와 비선형 함수로 구성됩니다. 이 경우의 비선형 측정 연산자는 ReLU 레이어와 λ-saturation(크리핑) 등을 포함하며, 특정 입력 영역에 대해 일대일 성격을 가질 수 있는 조건을 탐구합니다. 또한, 비선형 함수 ρ(ρ)와 입력 도메인 X에 대한 구체적인 정의를 포함하여 이론적 배경을 제공합니다.

- **Performance Highlights**: ReLU 레이어와 클리핑에 대한 새로운 경계 결과는 기계학습과 신호처리 분야에서 중요한 활용 가능성을 나타내며, 정보 복원 및 모델 해석 가능성을 개선하는데 기여합니다. 이 논문은 라이브러리와 프레임 이론에 기반한 방법론을 통해 이러한 조건들을 명확히 제시함으로써 기존 연구와의 연계성을 강화합니다.



### Symmetry-Preserving Diffusion Models via Target Symmetrization (https://arxiv.org/abs/2502.09890)
- **What's New**: 본 연구에서는 대칭성을 가진 데이터를 모델링하는 과제를 해결하기 위해 전통적인 eqivariant denoisers의 복잡성을 줄이는 새로운 방법을 제안합니다. 우리는 대칭화된 손실 함수(symmetrized loss function)를 통해 대칭성을 강제하며, 이는 모델의 예측 목표에 대해 시간-의존적 가중 평균(time-dependent weighted averaging) 작업을 적용합니다. 이 접근 방식은 명시적 구조적 제약 없이 대칭성을 보장하고, 기울기 분산을 줄여 더 안정적이고 효율적인 최적화를 가능합니다.

- **Technical Details**: 우리의 방법은 Monte Carlo sampling을 사용하여 평균을 추정하고 최소한의 계산 오버헤드를 발생시킵니다. 또한, 손실 함수의 최소화기에 대해 대칭성의 이론적 보장을 제공합니다. 이는 대칭성 강제를 구조적 복잡성 없이 해결할 수 있는 새로운 경로를 제시합니다.

- **Performance Highlights**: 실험 결과, GEOM-QM9 데이터셋을 이용한 분자 형태 생성을 포함하여 기존 방법들에 비해 샘플 품질이 향상되었음을 보여줍니다. 이 연구는 생성 작업에서 대칭적인 확산 모델의 확장 가능성과 실용성을 향상시킬 수 있는 잠재력을 강조하며, 복잡한 분포를 포착하는 중요한 도구로 자리잡을 것으로 기대됩니다.
...
### Elastic Representation: Mitigating Spurious Correlations for Group Robustness (https://arxiv.org/abs/2502.09850)
Comments:
          Accepted at AISTATS 2025

- **What's New**: 본 논문은 Elastic Representation (ElRep)이라는 새로운 방법론을 제안합니다. ElRep는 핵 심각도(nuclear norm)와 프로베니우스 정규화(Frobenius norm) 페널티를 적용하여 신경망의 마지막 층에서 특징을 학습합니다. 이 방법은 불균형한 그룹에서도져 성능 저하를 최소화하면서 강건성을 향상시키는 데 초점을 맞춥니다.

- **Technical Details**: ElRep는 신경망의 특징 표현을 정규화함으로써 스퓨리어스(correlations) 관계를 완화하고 불변 특징을 강조합니다. 스퓨리어스 특징에 의한 학습을 방지함으로써 그룹 강건성을 향상시키는 간단하면서도 효과적인 접근 방식을 제공합니다. 또한, ElRep는 전통적인 방법들처럼 그룹 정보를 요구하지 않아 현실적인 적용 가능성을 높입니다.

- **Performance Highlights**: ElRep는 기존의 방법들에 비해 전체 성능을 크게 희생하지 않으면서도 최악의 그룹의 정확도를 개선합니다. 본 논문의 실험 결과는 ElRep가 신경망에 통합되어 다양한 최첨단 접근 방식을 개선할 수 있다는 것을 시사합니다. 이 제안된 방법은 불균형한 데이터나 제한된 데이터셋에서도 높은 성능을 유지할 수 있는 잠재력을 가지고 있습니다.



### A Survey on Human-Centered Evaluation of Explainable AI Methods in Clinical Decision Support Systems (https://arxiv.org/abs/2502.09849)
Comments:
          10 pages, 1 table

- **What's New**: 본 논문은 Explainable AI (XAI) 방법들이 Clinical Decision Support Systems (CDSS)에서 어떻게 인간 중심의 평가를 통해 성과를 내는지를 체계적으로 조사한 것입니다. 기존의 XAI 연구들이 주로 새로운 기술 개발에 집중한 반면, 이 논문은 XAI 설명이 실제 임상에서의 채택과 의사결정에 미치는 영향을 분석합니다. 또한, 여러 이해 관계자들의 요구에 부합하도록 XAI 평가 방법을 정렬하는 구조적 프레임워크를 제안하고 있습니다.

- **Technical Details**: CDSS는 환자 데이터를 활용하여 특정 치료 권장 사항을 생성하는 시스템으로, 머신러닝 알고리즘을 통해 전자 건강 기록 (Electronic Health Records, EHRs)에서 대규모 데이터를 처리합니다. XAI는 CDSS의 투명성을 높이는 데 기여하며, 이는 의료 제공자들이 AI 시스템의 예측 이유를 이해하는 데 도움을 줍니다. 본 연구는 XAI 방법론과 평가 프레임워크, 임상 채택 문제를 바탕으로 기존 연구를 분류하여 체계적으로 정리하고 있습니다.

- **Performance Highlights**: 연구 결과, XAI 기반 CDSS는 의사들로부터 긍정적인 반응을 얻고 있지만, 높은 인지 부담, 임상 지식과의 불일치, 이해 관계자와의 조기 참여 부족과 같은 여러 가지 도전 과제가 여전히 존재함을 나타냈습니다. 제안된 개념적 프레임워크는 AI 기반 CDSS의 사회 기술적 격차를 메우기 위해 여려 이해 관계자의 요구와 평가 방법을 정렬하도록 구성되었습니다. 이는 CDSS 솔루션이 임상적으로 관련 있고 해석 가능하며, 의료 워크플로에 효과적으로 통합될 수 있도록 보장하는 데 초점을 맞추고 있습니다.



### Solving Empirical Bayes via Transformers (https://arxiv.org/abs/2502.09844)
Comments:
          27 pages, 14 figures, 11 tables

- **What's New**: 이 논문에서는 transform model (트랜스포머 모델)을 활용하여 고전적인 통계 문제인 Poisson-EB 문제를 해결하는 방법을 제시합니다. Poisson-EB 설정에서는 고차원 평균 벡터인 θ를 $X=	ext{Poisson}(	heta)$를 기반으로 추정합니다. 연구는 기존의 비모수 최대 우도 추정(NPMLE) 방법보다 더 작은 모델이 더 유효하다는 것을 발견했습니다.

- **Technical Details**: 본 연구에서는 트랜스포머 모델을 사전 훈련하여, $X와 θ$의 쌍을 통한 고차원 매개변수 추정을 수행합니다. Poisson-EB 문제는 관측한 샘플을 기반으로 평균 추정값을 최소화하는 것이며, 이는 MSE (Mean Squared Error)를 통해 평가됩니다. 이를 통해 추정(estimation) 과정에서 트랜스포머 내부 구조의 작동 원리에 대한 통찰을 제공합니다.

- **Performance Highlights**: 결과적으로 작은 모델들도 기존의 NPMLE 방법과 비교하여 몇 배 빠르고 낮은 검증 손실을 보임을 확인했습니다. 실제 데이터셋(NHL, MLB, BookCorpusOpen 등)에서 트랜스포머가 NPMLE보다 더 나은 성능을 발휘하는 것을 확인했습니다. 이를 통해 트랜스포머의 EB 추정기는 NPMLE 또는 Robbins의 추정기와는 다르게 작동하는 것으로 나타났습니다.



### Learning Fair Policies for Infectious Diseases Mitigation using Path Integral Contro (https://arxiv.org/abs/2502.09831)
- **What's New**: 이 논문은 불확실성 하에서 순차적인 의사결정을 위한 새로운 프레임워크를 제안합니다. 이 프레임워크는 다양한 불공정성 측정을 통합하여 공정한 질병 완화 정책을 설계할 수 있도록 합니다. 특히, 저소득 지역의 백신 접종과 봉쇄 전략을 학습하는 스토캐스틱 다중 그룹 SIR 모델을 기반으로 합니다.

- **Technical Details**: 본 연구에서는 기존의 방법에 비해 계산적으로 도전적인 순차 의사결정 문제를 해결하기 위해 경로 적분 제어(path integral control) 알고리즘을 채택하고 있습니다. 이 알고리즘은 몬테카를로 샘플링을 통해 비최적화적인 방법으로 문제를 효과적으로 해결합니다. 연구는 COVID-19 사례를 통해 응용되고 있으며, 다양한 사회경제적 지역에서 공정성을 향상시키는 정책을 제안합니다.

- **Performance Highlights**: 실증 사례 연구를 통해 제안한 정책이 기존의 동질적 정책에 비해 사회경제적 지역 간의 공정을 극대화하는 데 효과적임을 보여줍니다. 특히, 저소득 지역을 우선적으로 접종하도록 제안하는 정책이 골자로 포함되어, 정책 입안자들에게 중요한 통찰을 제공합니다.



### ATM-Net: Adaptive Termination and Multi-Precision Neural Networks for Energy-Harvested Edge Intelligenc (https://arxiv.org/abs/2502.09822)
- **What's New**: ATM-Net은 에너지 수확(IoT) 장치에 맞춤화된 새로운 신경망 구조로, 적응형 종료 포인트와 다중 정밀도 연산을 통합합니다. 이 시스템은 에너지 가용성에 따라 컴퓨팅 정밀도(32/8/4비트)와 네트워크 깊이를 동적으로 조정할 수 있어, 에너지를 절약하면서도 높은 정확도를 유지할 수 있습니다. 또한, 에너지-정확도 트레이드오프를 최적화하는 에너지 인식 작업 스케줄러를 도입했습니다.

- **Technical Details**: ATM-Net은 에너지 수확 시스템을 위해 설계된 적응형 신경망 아키텍처로, 정량화(Quantization)와 다중 종료 포인트(Multi-Termination) 두 가지 주요 측면을 통합하여 성능과 에너지 효율성을 최적화합니다. 8비트(Q8) 및 4비트(Q4) 정량화 기법을 활용하여 자원 제약이 있는 다양한 환경에서 신경망 성능을 최적화합니다. 이 과정에서 양자화 인식 학습(QAT)을 통해 정확도를 유지하면서도 에너지 소비를 줄입니다.

- **Performance Highlights**: CIFAR-10, PlantVillage, TissueMNIST 데이터셋에서 ATM-Net은 최대 96.93%의 정확도를 달성하면서, 32비트 연산 대비 87.5%의 전력 소모 감소를 보였습니다. DenseNet-121의 파워-지연(product) 수치는 13.6J에서 0.141J로 감소했고, ResNet-18은 10.3J에서 0.106J로 줄어들어, 에너지 수확 시스템에 적합함을 입증했습니다.
...
### Medical Applications of Graph Convolutional Networks Using Electronic Health Records: A Survey (https://arxiv.org/abs/2502.09781)
Comments:
          5 pages, 4 figures

- **What's New**: 이 논문은 Graph Convolutional Networks (GCNs)가 전자 건강 기록(EHR) 데이터에 적용되는 최신 연구를 종합적으로 소개하고 있습니다. GCNs는 환자 데이터의 그래프 표현을 구성하고 노드의 이웃에서 합성곱 작업을 수행함으로써 복잡한 관계를 포착하고 의료 의사결정을 지원하는 데 유용한 통찰력을 추출할 수 있습니다. 이 연구는 GCN의 주요 의료 분야와 예측 작업, 일반적인 벤치마크 데이터셋, 그리고 아키텍처 패턴을 식별하며, 이 분야의 포괄적인 조사를 제공합니다.

- **Technical Details**: 논문에서는 GCN의 기본 개념과 작동 방식을 설명합니다. GCN은 그래프 구조의 데이터에 대한 합성곱 작업을 수행하며, 노드의 이웃에서 정보를 집계하여 새롭게 표현을 생성하는 함수 학습을 목표로 합니다. 공간 기반 GCN은 메시지 전파 방식(message passing scheme)을 통해 각 노드가 이웃의 표현을 집계하여 자신의 표현을 업데이트합니다. GCN은 노드 분류, 링크 예측, 그래프 분류 등 다양한 응용 분야에서 구조적 정보와 노드의 특성 정보를 모두 포착할 수 있는 점에서 혁신적인 접근 방식을 보여줍니다.

- **Performance Highlights**: 최근 발표된 GCN과 EHR 데이터의 결합을 통한 의료 작업 수행 경향이 뚜렷해지고 있습니다. 본 논문에서 검토한 결과, 혼합 모델이 일반적으로 이 도메인에서 문제 해결에 사용되고 있으며, MIMIC-III 및 MIMIC-IV와 같은 공개 벤치마크 데이터셋이 널리 활용되고 있습니다. 또한, 비특정 의료 분야에 대한 연구를 통해 진단 예측 및 코드 분류와 같은 다양한 작업이 수행되고 있음이 확인되었습니다.
...
### Fine-Tuning Foundation Models with Federated Learning for Privacy Preserving Medical Time Series Forecasting (https://arxiv.org/abs/2502.09744)
Comments:
          submitted to IEEE EMBC 2025; 7 pages, 4 figures

- **What's New**: 이 논문은 연속적인 데이터 예측을 위한 Time Series Foundation Models (FMs)의 파인 튜닝을 위해 Federated Learning (FL)을 활용하는 새로운 접근 방식을 제안합니다. 기존의 FL 연구에서 주로 다루어졌던 분야인 의료 데이터에 대한 적용이 드문 점에서 주목할 만합니다. 우리는 Electrocardiogram (ECG)와 Impedance Cardiography (ICG) 데이터를 사용하여 여러 FL 기법으로 모델 성능을 향상시킬 수 있는 가능성을 탐구하였습니다.

- **Technical Details**: 이 연구에서는 FedAvg, FedProx 및 로컬 적응을 포함한 세 가지 FL 기술을 평가하였습니다. 각 기술은 반복 과정을 통해 FM을 파인 튜닝하며, 특히 IID(Independent and Identically Distributed)와 non-IID(Non-Independent and Identically Distributed) 환경에서의 도전과제를 분석하였습니다. 실험을 통해 신뢰할 수 있는 FL 모델 집계 알고리즘이 어떻게 성능에 영향을 미치는지를 비교하였습니다.

- **Performance Highlights**: 결과적으로, FL은 특정 경우에서 로컬 파인 튜닝보다 우수한 성능을 보일 수 있지만, 반대로 일부 상황에서는 성능이 저하될 수도 있음을 발견했습니다. 데이터 분포와 클라이언트의 자원 제약이 FL의 효과에 매우 중요한 영향을 미친다는 것을 강조하였습니다. 이러한 연구는 의료 분야에서 FL을 통한 FM의 적용 가능성을 확인하는 중요한 기초 자료가 될 것입니다.



### Navigating the Social Welfare Frontier: Portfolios for Multi-objective Reinforcement Learning (https://arxiv.org/abs/2502.09724)
- **What's New**: 본 논문에서는 강화 학습(Reinforcement Learning, RL)에서 여러 이해관계자에게 가장 적합한 정책을 효과적으로 집계하는 방법을 제안합니다. 특히, 다양한 $p$에 대한 일반화된 $p$-평균을 활용하여 여러 사회적 복지 함수에 따른 정책 결정의 복잡성을 줄이는 접근법을 제시하고 있습니다. 이러한 방법은 정책 포트폴리오를 구성하여 의사결정자가 다양한 상황에서 정책 선택을 용이하게 하는 데 도움을 줍니다.

- **Technical Details**: 연구진은 $eta$-근사 적합 포트폴리오를 도입하였으며, 이는 모든 $p$에 대한 일반화된 $p$-평균 목표를 근사적으로 최적화하는 정책 세트를 형성합니다. p-MeanPortfolio 알고리즘을 통해 $p$가 $-ullet 	ext{∞}에서 1$까지의 값에 대해 최적 정책을 차례로 선택하여 포트폴리오를 생성합니다. 이 과정에서 $eta$의 근사도, 포트폴리오 크기 및 계산 효율성 간의 무역 제약에 대한 이론적 보장을 제공합니다.

- **Performance Highlights**: 실험 결과, 제안한 포트폴리오 방법은 합성 및 실제 데이터셋에서 효과적으로 작동하며, 제한된 수의 정책으로도 거의 최적의 성능을 달성할 수 있음을 보여줍니다. 또한, Budget-ConstrainedPortfolio 알고리즘은 계산 비용을 현저히 줄이면서도 높은 품질의 포트폴리오 생성을 가능하게 합니다. 이 연구는 RL에서 다중 이해관계자와 정책 선택의 역학을 더 잘 이해하도록 돕는 데 기여합니다.
...
- **What's New**: 이번 논문은 Post-training quantization (PTQ)의 효율적인 배포를 위해 NestQuant라는 새로운 양자화 방식을 제안합니다. NestQuant는 자기 유사한 중첩 격자를 기반으로 하여 저정밀도 행렬 곱셈에 대해 정보 이론적으로 최적임을 수학적으로 입증했습니다. 이 방식은 Llama-3-8B 모델에서 가중치, KV-캐시 및 활성화를 4비트로 양자화하여 개선된 perplexity를 기록했습니다.

- **Technical Details**: PTQ의 주된 목표는 매개변수당 비트 수를 줄이고, 추론 속도를 가속화하며, 대형 LLM의 성능 저하를 줄이는 것입니다. 이 논문에서는 양자화된 가중치와 KV 캐시, 활성화를 위한 NestQuant 알고리즘을 제안하며, 이 알고리즘은 Gosset 격자를 기반으로 낮은 복잡도로 실용적인 버전으로 구현됩니다. 또한 양자화의 성능을 극대화하기 위해 정보 이론적 한계에 가까운 성능을 발휘하는 것으로 설명됩니다.

- **Performance Highlights**: NestQuant는 SpinQuant와 비교하여 55% 이상의 perplexity 감소를 달성하며, 전반적인 성능 감소를 최소화함을 보여줍니다. 다양한 LLM 평가 벤치마크에서도 우수한 성능을 인정받고 있으며, Llama-3-8B에서 End-to-End 성능 개선이 크게 나타났습니다. NestQuant는 다른 알고리즘에 대한 우수한 대안임을 입증하며, 실제 데이터 및 합성 데이터 모두에서 눈에 띄는 개선 효과를 보입니다.
...
### Leveraging Machine Learning and Deep Learning Techniques for Improved Pathological Staging of Prostate Cancer (https://arxiv.org/abs/2502.09686)
- **What's New**: 본 연구는 전통적인 진단방법의 제약을 극복하기 위해 머신 러닝(machine learning)과 딥 러닝(deep learning) 접근 방식의 통합을 통해 전립선암(pathological staging)의 예측을 향상시키는 방법을 제시합니다. RNA 시퀀싱 데이터(RNA sequencing data)를 활용하여 486개의 종양에서 생성된 유전자 발현 프로파일을 분석하였습니다. 이 과정에서 Random Forest, Logistic Regression, Extreme Gradient Boosting, 그리고 Support Vector Machine과 같은 고급 알고리즘이 사용되었습니다.

- **Technical Details**: 연구에서 사용된 기술적 요소는 높은 차원의 데이터 분석과 정보 추출을 위한 다양한 방법론을 포함합니다. 이에 따라, 모델의 정확도 향상을 위해 Synthetic Minority Over-sampling Technique(SMOTE)와 같은 입력 데이터 확장 기법을 적용하였습니다. 각 알고리즘의 성능은 F1-score, precision, recall을 기준으로 평가되었으며, Random Forest 알고리즘이 약 83%의 최고 F1-score를 기록했습니다.

- **Performance Highlights**: 본 연구의 결과는 전통적인 진단 방법에 비해 AI 기반 접근 방식이 진단 정확도 향상에 기여할 수 있는 가능성을 보여줍니다. Random Forest 알고리즘 외에도 Logistic Regression은 80%의 F1-score를 기록했으며, 딥 러닝 모델은 데이터 증강 기법을 통해 71.23%의 정확도를 달성했습니다. 이러한 성과는 전립선암 관리에서 환자 결과를 개선할 수 있는 진단 도구의 개발에 기초를 마련합니다.



### A Novel Hybrid Approach to Contraceptive Demand Forecasting: Integrating Point Predictions with Probabilistic Distributions (https://arxiv.org/abs/2502.09685)
- **What's New**: 이 연구는 개발도상국에서의 피임수단 수요 예측의 정확성을 높이기 위한 새로운 접근 방식을 제시합니다. 기존의 간단한 예측 방법들이 데이터의 불완전성 및 품질 문제로 인해 피임수단 수요의 불확실성을 충분히 반영하지 못하는 점을 해결하려고 합니다. 저자들은 도메인 특화 모델의 포인트 예측(Dense Forecasts)과 확률적 방법(Probabilistic Methods)을 결합한 하이브리드 모델을 개발하여, 전문가의 전문성을 통합함으로써 예측의 정확도를 향상시키고자 합니다.

- **Technical Details**: 연구에서는 여러 가지 예측 방법을 평가합니다. 시간 시계열(Time Series), 베이지안(Bayesian), 머신러닝(Machine Learning) 및 기초 시간 시계열(Baseline Time Series) 방법을 포함하여, 각 방법의 성과를 비교하고 그 강점과 약점을 분석합니다. 특히, 제안된 제약된 분위수 회귀 평균(Constrained Quantile Regression Averaging, CQRA) 방법은 전문가의 포인트 예측과 시스템 생성 확률적 예측을 결합하여 새로운 예측을 생성하는 데 중점을 둡니다.

- **Performance Highlights**: 본 연구는 피임수단 재고 관리 및 공급망에서의 불확실성을 줄이는 데 중요한 기여를 하고 있습니다. 또한, 시간 시계열, 머신러닝, 베이지안 방법과 같은 기타 예측 방법들과 비교하여 하이브리드 방법의 성능을 면밀히 분석합니다. 저자들은 제안된 방법에 대한 코드와 데이터를 공개하여 연구의 재현성을 보장하며, 다른 인도적 상황에서도 일반화 가능성을 강조합니다.
...
- **What's New**: 이 논문에서는 Time-series forecasting (TSF) 연구의 현재 평가 시나리오가 실제 연구 상태를 왜곡하고 있음을 보여줍니다. 기존의 간단한 데이터셋 덕분에 모델의 채널 간 상관관계를 과소평가해 왔으며, 채널 의존성을 잘 모델링하는 것이 필수적이지 않다는 주장을 하고 있습니다. 이에 따라, 개선된 평가 시나리오에서 Crossformer가 SOTA(State Of The Art) 모델로 자리잡게 된 배경을 설명하고, 이를 기반으로 Fast Channel-dependent Transformer (FaCT)라는 새로운 경량 모델을 제안합니다.

- **Technical Details**: 이 연구는 멀티변량 타임 시리즈 데이터셋을 분석하고, 현재의 평가 시나리오가 어떻게 모델의 성능을 왜곡하는지를 다룹니다. 특히, 기존의 TSF 방법론에서 모델링된 채널 간 상호작용이 실제적으로 중요한 경우가 드물다는 점을 강조합니다. FaCT 모델은 Crossformer의 수정판으로, 처리 시간과 메모리 비용을 줄이면서도 뛰어난 성능을 유지합니다.

- **Performance Highlights**: Fast Channel-dependent Transformer (FaCT)는 Crossformer에 비해 약 51.17%의 처리 시간 복잡성 감소를 달성하면서 ODE 벤치마크에서 경쟁력 있는 성능을 보입니다. 이 모델은 Crossformer에 이어 두 번째로 뛰어난 성능을 자랑하며, 성능과 처리 시간의 균형을 잘 잡고 있는 모델로 평가받습니다. 일반적으로 간단한 데이터셋에서는 채널 의존성이 그다지 중요한 요소가 아니지만, 복잡한 데이터셋에서는 Crossformer가 여전히 최고의 성능을 낼 수 있음을 입증합니다.
...
- **What's New**: 이번 연구에서는 엘립틱 커브(elliptic curves)의 Frobenius trace $a_p$를 예측하기 위해 transformer 모델과 feedforward neural networks를 적용했습니다. 또한, 다른 traces $a_q$를 이용하여 $a_p mod 2$를 예측하는 모델을 훈련시켜 높은 정확도를 기록했습니다. 이 연구는 기존의 수론적 도구 없이도 모델이 높은 예측 성능을 발휘할 수 있음을 보여주며, 머신러닝과 수론의 융합 가능성을 제시합니다.

- **Technical Details**: 연구의 목적은 비동형 엘립틱 커브들 사이의 Euler 요인(Euler factors) 및 Frobenius trace가 서로 다르게 나타나는 경우를 탐구하는 것입니다. 이 과정에서 Riemann 가설을 전제로 하고, ML 기법이 Frobenius traces 예측에 어떻게 기여할 수 있는지를 분석합니다. 실험에서는 transformer 모델과 feedforward neural networks를 사용하여 특정 $p$에 대한 $a_p$ 값을 여러 $a_q$값에서 예측하도록 하였습니다.

- **Performance Highlights**: 모델들은 특히 $q<97$인 경우에 대해 Frobenius trace의 정확한 예측을 수행하여 놀라운 높은 정확도를 보였습니다. PCA 분석 결과, 모델이 $a_p mod 2$의 모듈러 행동을 배워가고 있음을 제안하며, 학습 과정에서 몇 가지 중요한 패턴을 발견했습니다. 또한, $a_p$를 예측하기 위한 추가 실험에서도 비교적 비슷한 성능을 나타내며, 입력 인코딩의 의존성을 강조하는 중요 특성을 발견하였습니다.



### Assortment Optimization for Patient-Provider Matching (https://arxiv.org/abs/2502.10353)
Comments:
          36 pages, 11 Figures

- **What's New**: 이 연구는 의료 제공자의 높은 이직률로 인한 환자-제공자 재배치의 어려움을 해결하기 위해 환자-제공자 매칭 알고리즘을 개발했습니다. 특히 패턴 최적화(assortment optimization)를 통해 환자에게 제공자를 선택할 수 있는 메뉴를 제공하여 제공자의 물리적 한계와 행정적 부담을 완화하고자 합니다. 연구 결과, 적절한 정책을 적용함으로써 환자와 제공자 간의 매칭 품질을 13%까지 개선할 수 있음을 보여줍니다.

- **Technical Details**: 환자-제공자 매칭 모델을 통해 조사한 결과, 제안된 메뉴를 통해 환자에게 제공자를 선택하는 프로세스를 기획했습니다. 연구에서는 균형적 접근을 위해 다양한 종류의 선택 모델을 사용하였고, 선택 모델로는 균일 선택 모델과 다항 로짓 선택 모델이 포함되었습니다. 추가로 매칭 품질을 정의하기 위해 지역적 근접성, 제공자의 전문성, 그리고 언어 및 성별의 일치와 같은 요소를 고려한 매칭 품질 행렬을 사용했습니다.

- **Performance Highlights**: 정량적 분석을 통해 서로 다른 환자와 제공자 비율에 따라 최적의 정책을 제안했습니다. 예를 들어, 환자가 제공자보다 많을 경우, 기울기 하강 정책(gradient descent policy)이 최고의 성과를 보였으며, 환자가 제공자와 균형을 이룰 때는 탐식적(greedy) 정책이 높은 매칭 품질을 달성했습니다. 또한, 실제 Medicare 데이터를 이용하여 제안된 정책의 성능을 검증하고, 공정성과 인지 과부하와 같은 실제 고려사항을 확장하여 분석했습니다.
...
### Studying number theory with deep learning: a case study with the Möbius and squarefree indicator functions (https://arxiv.org/abs/2502.10335)
Comments:
          10 pages

- **What's New**: 본 논문은 Charton의 연구를 기반으로 작은 transformer 모델을 훈련시켜 Möbius 함수 μ(n)과 squarefree 지표 함수 μ²(n)를 계산하는 방법을 탐구합니다. 이러한 모델들은 비헙박적인 예측력을 달성하였으며, 각 모델의 기능을 이해하기 위해 반복적으로 추가 모델을 훈련시켰습니다. 마지막으로 이론적 설명을 찾아내었습니다.

- **Technical Details**: 논문에서는 작은 transformer 아키텍처를 활용하여 고전적인 수학 함수인 Möbius 함수와 squarefree 지표 함수를 연구하고 있습니다. 이 함수들은 주로 n의 소인수 분해에 기반하여 값이 결정되며, 알고리즘의 선택과 모델의 학습 방식이 중요합니다. Adleman과 McCurley의 연구 결과를 언급하며, 기존의 알고리즘이 더 나은 성능을 보이지 못한다고 설명합니다.

- **Performance Highlights**: 모델은 입력-출력 쌍(n, μ²(n))을 학습하여 n이 특정 수로 나누어지는지를 식별하는 것을 점차적으로 배우며, 이를 통해 엄청난 정확도로 μ²(n)을 예측할 수 있습니다. 원래의 단순한 예측을 넘어, 제곱 나눗셈 패턴을 포함하지 않는 n의 표현 방식을 선택하므로써 모델의 성능과 예측 행동이 어떻게 설명될 수 있는지를 살펴봅니다. 추가 실험을 통해 수정된 표현에 대한 결과를 분석하여 이러한 설명을 확인합니다.



### Generalised Parallel Tempering: Flexible Replica Exchange via Flows and Diffusions (https://arxiv.org/abs/2502.10328)
- **What's New**: 이번 논문에서는 Parallel Tempering (PT) 알고리즘의 한계를 극복하고자 Generalised Parallel Tempering (GePT) 프레임워크를 제안합니다. GePT는 일반적인 MCMC 기반 방법론과 동일한 이론적 보장을 제공하면서 현대의 생성 모델링 기술인 normalising flows와 diffusion models의 장점을 PT에 통합할 수 있게 합니다. 이를 통해 샘플 품질을 향상시키고 복잡한 분포를 다루기 위한 계산 자원의 증가를 줄일 수 있습니다.

- **Technical Details**: GePT는 PT의 수학적 구조를 변경하여 더 유연한 형태의 복제 교환 메커니즘을 형성합니다. Flow-GePT와 Diff-GePT 두 가지 인스턴스가 제시되며, 각각 normalising flows와 diffusion models를 활용하여 이웃 분포 간의 변환 맵을 효과적으로 학습할 수 있도록 설계되었습니다. 이러한 접근 방식은 다양한 샘플을 생성하는 데 필요한 계산 비용을 줄이며 MCMC 방법의 비대칭 일관성을 유지합니다.

- **Performance Highlights**: 실험적으로 Flow-GePT와 Diff-GePT가 PT보다 뛰어난 성능을 보임을 입증하였습니다. 이 새로운 알고리즘들은 기본적으로 참조 상태와 목표 상태 간의 통신을 가속화하여 더욱 효과적인 샘플링을 제공합니다. 따라서 GePT는 복잡한 분포에서 샘플링 효율성을 높이는 데 기여할 것으로 기대됩니다.
...
- **What's New**: 이 논문은 AdaPTS라는 새로운 프레임워크를 소개하여, 기존의 사전 훈련된 단일 변수(time series) 모델을 다변수(multivariate) 예측에 효과적으로 적용하는 방법을 제안합니다. 이 연구는 예측 불확실성(uncertainty quantification) 및 특징 간 의존성(dependencies) 문제를 해결하는 데 중점을 두고, 기존 모델들을 확장할 수 있는 다양한 어댑터(adapters)를 제공합니다. 특히, 어댑터는 시간 순서에 따라 특징 공간(feature space)을 변환하여 다변수 입력을 적합한 잠재 공간(latent space)으로 매핑합니다.

- **Technical Details**: AdaPTS는 단일 변수(time series) 사전 훈련 모델을 사용하여 다변수 예측을 위한 프로바빌리스틱 어댑터를 통합합니다. 이 어댑터는 입력 시계열을 변환(tranformation)하여 잠재 공간에서 예측을 수행할 수 있도록 해 주며, 예측 결과는 원래 특징 공간으로 변환됩니다. 이 과정에서 형태학적 원치(Essentially Invertibility) 제약을 적용하여, 예측의 신뢰성(calibration)과 견고성(robustness)을 증대시키는 특징이 있습니다.

- **Performance Highlights**: 실험 결과, AdaPTS는 다양한 다변수 시계열 예측 벤치마크에서 기존 방법들에 비해 예측 정확도(forecasting accuracy)에서 유의미한 개선을 보여주었습니다. 또한 어댑터는 특징 공간의 차원을 줄임으로써 비용 효율적인 적응(adaptation)을 가능하게 하고, 학습된 잠재 표현(latent representation)의 해석가능성(interpretability)을 분석하며, 보다 높은 예측 신뢰성을 확보할 수 있음을 확인했습니다.



### Learning to Solve the Min-Max Mixed-Shelves Picker-Routing Problem via Hierarchical and Parallel Decoding (https://arxiv.org/abs/2502.10233)
- **What's New**: 본 연구는 Mixed-Shelves Picker Routing Problem (MSPRP)을 해결하기 위한 새로운 계층 및 병렬 디코딩 방식의 다중 에이전트 강화 학습 접근법을 제안합니다. 이러한 접근 방식은 picker의 조정 및 신속한 디코딩을 가능하게 하여 다차원의 액션 공간에서의 충돌을 피하는 순차적 액션 선택을 통합합니다. 이를 통해 기존의 전통적인 방법들이 가지는 한계를 극복할 수 있는 가능성을 보여줍니다.

- **Technical Details**: 본 연구에서 제안하는 방법은 협력적 다중 에이전트 마르코프 결정 프로세스(MMDP)로 MSPRP의 min-max 변형을 공식화합니다. 각 에이전트는 주어진 상태에서 액션을 선택하며 이들 간의 공동 보상을 기반으로 합니다. 또한 계층적 및 병렬 디코딩 프레임워크를 통해 picker 간의 조정 효율성을 극대화하며, 순차적 액션 선택 전략을 통해 다양한 문제 상황에서의 강력한 일반화 성능을 보여주고 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 솔루션 품질과 추론 속도 모두에서 최첨단 성능을 나타냈습니다. 특히 대규모 및 분포 외(In-distribution) 인스턴스에서 주요 성과를 달성하였습니다. 이는 효율적인 창고 운영을 위한 기존 접근법의 한계를 넘는 중요한 발전으로 평가됩니다.
...
### Agentic End-to-End De Novo Protein Design for Tailored Dynamics Using a Language Diffusion Mod (https://arxiv.org/abs/2502.10173)
- **What's New**: 본 논문에서는 VibeGen이라는 생성적 AI 프레임워크를 소개합니다. 이 프레임워크는 정상 모드 진동(normal mode vibrations)을 기반으로 하는 새로운 단백질 설계를 가능하게 합니다. VibeGen은 지정된 진동 모드에 따라 시퀀스 후보를 생성하는 단백질 디자이너와 동적 정확성을 평가하는 단백질 예측기로 구성된 이중 모델 아키텍처를 채택하고 있습니다.

- **Technical Details**: VibeGen은 구조(sequence), 형태(structure), 분자 운동(molecular motion) 간의 복잡한 관계를 다루며 단백질 디자인의 정확성과 다양성을 동시에 고려합니다. 모델은 전체 원자(full-atom) 분자 시뮬레이션을 통해 면밀히 검증되었으며, 설계된 단백질들이 지정된 정상 모드의 진폭을 정확히 재현하는 것을 보여줍니다. 생성된 시퀀스들은 자연 단백질과의 유의미한 유사성이 없으며, 이는 진화적 제약을 초월하는 새로운 단백질 공간을 확장합니다.

- **Performance Highlights**: VibeGen이 설계한 단백질은 다양한 안정적이고 기능적으로 관련된 구조를 채택하며, 동적이며 기능적인 특성을 갖춘 생체 분자의 설계에 새로운 경로를 열어줍니다. 이 연구는 유연한 효소(flexible enzymes), 역동적 지지체(dynamic scaffolds), 생체 재료(biomaterials)의 합리적 설계를 위한 폭넓은 응용 가능성을 가지고 있습니다. 나아가, 이 프레임워크는 단백질 디자인에 있어 다이내믹스 인포메드(dynamics-informed) 접근 방식을 활용할 수 있는 가능성을 제시하고 있습니다.



### Enhancing anomaly detection with topology-aware autoencoders (https://arxiv.org/abs/2502.10163)
Comments:
          12 pages, 5 figures, 2 tables

- **What's New**: 본 연구에서는 고에너지 물리학에서의 비정상 탐지를 향상시키기 위해 토폴로지 인식 오토인코더(topology-aware autoencoders)를 제안합니다. 기존의 오토인코더는 잠재 공간의 토폴로지에 제약을 받으나, 본 연구는 에너지-운동량 보존을 반영한 컴팩트 매니폴드(compact manifolds)에 위상 공간 분포를 맵핑하여 성능을 개선합니다. 이러한 새로운 접근 방식은 데이터의 전역 구조(global structure)를 보존하면서 비정상 이벤트를 더 효과적으로 구분하도록 합니다.

- **Technical Details**: 본 논문에서는 구형($S^n$), 곱($S^2 	imes S^2$), 그리고 사영($	ext{RP}^2$) 상태를 갖는 여러 유형의 오토인코더를 구축하며, 이들의 비정상 탐지 성능을 분석합니다. 연구는 잠재 공간이 자신의 토폴로지를 알고 있는 경우, 데이터의 신뢰성 있는 표현을 보장한다고 강조합니다. 또한, 다양한 잠재 공간 제약 조건을 가진 오토인코더 구조를 실험하며, 각 토폴로지가 비정상 탐지 성능에 미치는 영향을 평가합니다.

- **Performance Highlights**: 모의 하드로닉(top-quark) 붕괴에 대한 적용을 통해, 적절한 토폴로지 제약을 가진 잠재 공간이 비정상 이벤트를 감지하는 데 있어 민감도와 강인성을 향상시키는 효과를 보여줍니다. 실험 결과, 선택된 잠재 토폴로지를 가진 오토인코더가 배경 및 비정상 이벤트 간의 분리를 우수하게 수행함을 확인했습니다. 본 연구는 비지도 학습 환경에서 새로운 물리를 탐색하는 데 강력한 도구로서의 토폴로지 인식 오토인코더의 가능성을 제시합니다.



### Combinatorial Reinforcement Learning with Preference Feedback (https://arxiv.org/abs/2502.10158)
Comments:
          Preprint. Under review

- **What's New**: 이번 논문에서는 선호 피드백(preference feedback)을 사용하는 조합적 강화 학습(combinatorial reinforcement learning)에 대해 논의합니다. 학습 에이전트는 사용자가 선호하는 조합을 제공하고, 이 선호 피드백은 다항 로지스틱 모델(multinomial logistic model)을 따른다고 설명합니다. 이 연구는 추천 시스템(recommender systems) 및 온라인 광고와 같은 장기 사용자 참여(long-term user engagement)에 관련된 실제 시나리오를 모델링할 수 있는 기회를 제공합니다.

- **Technical Details**: 논문에서는 맥락적 MNL 선호 모델(contextual MNL preference model)을 가정하고, 각 항목의 평균 효용(mean utility)은 선형(linear)으로, 각 항목의 가치는 일반 함수(general function)로 근사화합니다. 또한, MNL-VQL이라는 알고리즘을 제안하여 각 항목의 미지의 값(unknown value)으로 인한 문제와 조합적 행동 공간(combinatorial action space)에서의 최적 선택(optimal assortment selection) 어려움을 해결합니다.

- **Performance Highlights**: MNL-VQL 알고리즘은 계산적(computational) 및 통계적(statistical) 효율성을 제공하면서, 특수한 경우로 선형 MDP(linear MDP)와 MNL 선호 피드백에서 최소 회귀(lower bound on regret)를 제시하고, MNL-VQL이 거의 minimax-optimal regret에 도달함을 보여줍니다. 이 연구는 조합적 RL(combinatorial RL)에서 선호 피드백을 제공하는 첫 번째 작업으로, 통계적 보장(statistical guarantees)을 제공합니다.
...
### Topological Neural Networks over the Air (https://arxiv.org/abs/2502.10070)
- **What's New**: 이번 논문에서는 Topological Neural Networks (TNNs)의 새로운 설계인 AirTNN을 제안합니다. AirTNN는 정규 셀 복합체 상에서 작동하며, 무선 통신 모델을 통합하여 오버-더-에어(computation) 방식으로 계산을 수행합니다. 이 모델은 훈련 및 추론 과정에서 페이딩(fading)과 잡음(noise)과 같은 채널 손상을 고려하여 토폴로지적 컨볼루션 필터링 작업을 진행합니다.

- **Technical Details**: AirTNN 아키텍처는 다층 구조로, 무선 통신 채널을 통해 신호를 이동시키는 셀 복합체 필터를 사용하여 설계되었습니다. 이는 이웃으로부터 정보를 수집하고 복잡한 신호의 다중 홉(multihop) 네트워크를 활용하여 작동합니다. 또한, AirTNN은 토폴로지적 데이터를 학습하는 데 적합하게 설계되어 있어, 센서로부터 수집된 데이터를 통해 분산 처리(distributed processing)를 가능하게 합니다.

- **Performance Highlights**: 실험 결과는 AirTNN이 기존의 AirGNN 및 기초 GNN, TNN의 성능을 능가함을 보여줍니다. 특히 소스 로컬라이제이션(source localization) 작업에서 뛰어난 효과를 발휘하며, 무선 채널에서 발생할 수 있는 손상에 대한 내성을 지니고 있습니다. 이는 다양한 센서 네트워크 환경에서 중요한 신뢰성과 성능을 제공함을 의미합니다.
...
### Improved Online Confidence Bounds for Multinomial Logistic Bandits (https://arxiv.org/abs/2502.10020)
Comments:
          Preprint. Under review

- **What's New**: 이번 논문에서 연구자들은 다항 로지스틱 모델(multinomial logistic, MNL)에 대한 향상된 온라인 신뢰 구간(online confidence bound)을 제안하고, 이를 MNL 밴딧(MNL bandits)에 적용하여 분산 의존적인 최적 후회(regret)를 달성했습니다. 이전 연구인 Lee & Oh (2024)가 제시한 결과는 여전히 알려지지 않은 매개변수 $B$의 노름 경계(norm-boundedness)와 가능한 결과의 최대 크기 $K$에 의존하고 있었으나, 새로운 신뢰 구간의 도출로 이런 제약이 크게 줄어들었습니다.

- **Technical Details**: 이 논문에서는 $Oig(	ext{sqrt}(d 	ext{log} t) + Big)$라는 새로운 온라인 신뢰 구간을 도출하였으며, 이는 이전의 $O(B 	ext{sqrt}(d) 	ext{log} t 	ext{log} K)$ 보다 현저한 개선입니다. 이 향상은 MNL 손실(MNL loss)의 더紧实한(self-concordant) 속성 확립과 추정 오차를 제한하기 위한 새로운 중간 용어의 도입으로 가능했습니다. 새로운 신뢰 구간을 기반으로 한 상수 시간 알고리즘인 OFU-MNL++는 충분히 큰 $T$에 대해 $Oig(d 	ext{log} T 	ext{sqrt}ig(	ext{sum}_{t=1}^T 	ext{sigma}_t^2ig)ig)$의 분산 의존 후회 구간을 달성합니다.

- **Performance Highlights**: 또한, 최대 우도 추정(Maximum Likelihood Estimation, MLE) 기반의 알고리즘 OFU-MN$^2$L을 소개하여 항시 적용 가능한(poly( B)-free) 후회 구간을 $Oig(d 	ext{log}(BT) 	ext{sqrt}ig(	ext{sum}_{t=1}^T 	ext{sigma}_t^2ig)ig)$로 구현했습니다. 이 새로운 접근 방식은 MNL 밴딧 상황에서의 효율성을 크게 향상시키며, 다양한 분야에 응용될 수 있습니다. 궁극적으로, 제안된 방법들은 온라인 학습(online learning) 문제의 실질적인 개선을 위한 기초를 제공합니다.



### InterGridNet: An Electric Network Frequency Approach for Audio Source Location Classification Using Convolutional Neural Networks (https://arxiv.org/abs/2502.10011)
Comments:
          The 10th International Conference on Advances in Signal, Image and Video Processing (SIGNAL 2025)

- **What's New**: 새로운 프레임워크인 InterGridNet이 도입되었으며, 이는 얕은 RawNet 모델을 활용하여 Electric Network Frequency (ENF) 서명을 기반으로 geolocation(위치 추정) 분류를 수행한다. 데이터 준비 과정에서는 오디오 및 전력 그룹으로 분류되고, 이후 spectrogram 분석을 통해 50 Hz와 60 Hz 그룹으로 나뉜다. InterGridNet의 전반적인 정확도는 테스트 기록에서 92%에 달하며, 이는 SP Cup 2016에서 검토한 최신 방법들과 비교하여 높은 성능을 보인다.

- **Technical Details**: InterGridNet는 preprocessing(전처리) 기술과 classification(분류) 방법을 통합하여 설계된 포괄적인 접근 방식이다. 이 프레임워크는 residual blocks를 사용하여 frame-level embeddings를 추출하고, softmax activation(소프트맥스 활성화) 함수를 사용하여 결정 과정에서 이를 활용한다. Neural Architecture Search(NAS)를 통해 모델의 topology(구조) 및 hyperparameters(하이퍼파라미터)를 최적화하여 효율적인 inter-grid localization(그리드 간 위치 추정)을 구현한다.

- **Performance Highlights**: InterGridNet은 SP Cup 2016 데이터셋에서 9개의 서로 다른 전력 그리드 간 geolocation(위치 추정) 분류의 성능을 입증하며, 최신 방법들과 비교되는 실험 평가를 통해 효과성을 강조한다. 각 전력 그리드는 50Hz 또는 60Hz의 기본 주파수에서 ENF 신호를 기반으로 하여 분류되며, 높아진 신호 대 잡음 비율(Signal-to-Noise Ratio, SNR)로 인해 오디오 및 전력 기록의 구분이 명확해진다.
...
### Estimation of the Learning Coefficient Using Empirical Loss (https://arxiv.org/abs/2502.09998)
Comments:
          15 pages, 6 figures, 4 tables

- **What's New**: 본 논문은 정보 기준의 성능 분석에 중요한 학습 계수(learning coefficient)를 추정하는 새로운 수치 방법을 제안합니다. 기존의 방법들과 근본적으로 다른 접근을 통해 Watanabe가 도입한 "Empirical Loss"라는 새로운 개념을 활용합니다. 실험 결과, 제안된 방법은 Watanabe와 Imai의 방법에 비해 낮은 편향(bias)과 분산(variance)을 보입니다. 

- **Technical Details**: 학습 계수는 Kullback-Leibler 발산(Kullback-Leibler divergence) 및 사전 분포(prior distribution)에서 유도된 제타 함수의 극(pole) 차수의 절대값으로 정의됩니다. 기존 방법들은 Markov Chain Monte Carlo (MCMC) 기법을 사용하여 후향 분포를 추정하지만, 이는 추정 정확도에 큰 영향을 미치는 문제들이 존재합니다. 본 논문에서는 이러한 문제를 해결하기 위해 'Empirical Loss'를 통한 새로운 수치 추정 방법을 제안하며, 이 방법은 수치 실험을 통하여 검증되었습니다.

- **Performance Highlights**: 제안된 방법은 기존 방법보다 측정된 평균 제곱 오차(MSE)가 낮고 더 안정된 추정을 제공합니다. 본 연구는 일반 및 특이 모델에 대한 WAIC 및 WBIC의 이론적 근거를 강화하며, 이러한 개선을 통해 학습 계수의 계산적 효율성을 높이는 데 기여합니다. 또한, MCMC 샘플링 오류에 대한 분석을 통해 특이 모델에서 학습 계수 추정의 실제 적용 가능성을 향상시키고 있습니다.
...
### On Volume Minimization in Conformal Regression (https://arxiv.org/abs/2502.09985)
- **What's New**: 이번 연구에서는 split conformal regression에서의 부피 최적화(volume optimality) 문제를 다루었습니다. 이는 coverage control과 비교할 때 아직 명확히 이해되지 않은 주제입니다. 고전적 split 방법으로 반환된 구간(interval)의 과잉 부피 손실(excess volume loss)의 유한 샘플 상한을 도출하였고, 이를 통해 split 방법과 최단 오라클 예측 구간의 길이 차이를 측정합니다.

- **Technical Details**: 이 연구에서 제안된 EffOrt 방법론은 학습 단계(modification of the learning step)를 수정하여 반환된 구간의 길이를 최소화하는 방식으로 기본 예측 함수(base prediction function)를 선택합니다. 이론 분석을 통해 EffOrt에 의해 생성된 예측 집합(prediction sets)의 과잉 부피 손실(excess volume loss)과 학습 및 보정(calibration) 단계 간의 관계를 밝히고, 기본 예측기의 함수 클래스 선택이 미치는 영향을 강조합니다. 또한 Ad-EffOrt라는 기존 방법의 확장을 소개하며, 이는 공변량(covariate)의 값에 따라 구간의 크기를 조정합니다.

- **Performance Highlights**: 마지막으로, 제안된 방법론들의 실험적 성능(empirical performance)과 강건성(robustness)을 평가하였습니다. 이 평가 결과는 EffOrt와 Ad-EffOrt가 기존 방법들보다 더 우수한 성능을 보임을 시사합니다. 특히, 이 연구는 예측 구간의 길이를 최적화하는 새로운 접근 방식을 제시하며, split conformal regression의 적용 가능성을 확대합니다.



### Universal Machine Learning Interatomic Potentials are Ready for Solid Ion Conductors (https://arxiv.org/abs/2502.09970)
- **What's New**: 본 연구에서는 차세대 리튬이온 배터리를 위한 고성능 고체 전해질(Solid-State Electrolyte, SSE)로서의 유니버설 머신 러닝 상호 원자적 포텐셜(universal machine learning interatomic potentials, uMLIPs)의 가능성을 평가하였습니다. 기존의 밀도 함수 이론(Density Functional Theory, DFT) 및 경험적 힘 필드의 한계를 극복하기 위한 다양한 uMLIP 모델을 체계적으로 비교 분석한 결과가 특징적입니다.

- **Technical Details**: 연구에서는 MatterSim, MACE, SevenNet, CHGNet, M3GNet, ORBFF 등 6개의 고급 uMLIP 모델을 에너지, 힘, 열역학적 속성, 탄성 계수, 리튬 이온 확산 거동 등 여러 측면에서 평가하였습니다. 그 결과, MatterSim이 복잡한 물질 시스템에서 특히 우수한 정확도와 물리적 일관성을 나타내며, 리튬 이온 확산 계산에서 참조 값과의 훌륭한 일치를 이루었습니다.

- **Performance Highlights**: MatterSim은 여러 지표에서 가장 높은 성능을 보여주며, 특히 리튬 이온의 확산 계산에서 뛰어난 결과를 도출했습니다. 또한, Li3YCl6 및 Li6PS5Cl에서 크리스탈 구조와 이온 배열의 무질서 수준이 이온 전도도에 미치는 영향을 분석하였으며, 적절한 S/Cl 무질서 수준과 최적화된 Na/Li 배열이 전반적인 이온 전송 성능을 개선하는 데 기여합니다.
...
### Tradeoffs in Processing Queries and Supporting Updates over an ML-Enhanced R-tr (https://arxiv.org/abs/2502.09937)
Comments:
          arXiv admin note: text overlap with arXiv:2207.00550

- **What's New**: 본 논문은 전통적인 R-tree 다차원 인덱스 구조에 머신 러닝 모델을 결합하여 성능을 향상시키는 AI+R-tree를 제안합니다. AI+R-tree는 쿼리 처리 중 불필요한 브랜치 탐색을 줄여줌으로써 중첩된 R-tree 노드 간의 비효율적인 탐색을 방지합니다. 이 연구는 다양한 머신 러닝 모델이 AI+R-tree의 쿼리 처리 성능에 미치는 영향을 조사하였습니다.

- **Technical Details**: AI+R-tree는 전통적 디스크 기반 R-tree를 기반으로 하며, 머신 러닝 모델을 통해 쿼리 성능을 개선합니다. 본 논문에서는 동적 쿼리 로드 처리 및 업데이트 지원을 위한 에mpirical tradeoffs를 다루었으며, 맞춤형 손실 함수(custom loss function)를 설계한 케이스 스터디도 포함되었습니다. 이를 통해 AI+R-tree의 유연한 데이터 삽입, 업데이트 및 삭제를 위한 설계 교환(trade-offs)을 논의합니다.

- **Performance Highlights**: 실제 데이터셋을 바탕으로 실험한 결과, AI+R-tree는 전통적인 R-tree에 비해 높은 겹침 범위 쿼리에서 쿼리 처리 성능을 최대 5.4배 향상시키는 것으로 나타났습니다. 또한, 평균 쿼리 리콜을 99%까지 달성할 수 있음을 보여주었습니다. 이러한 성과는 AI+R-tree가 다차원 데이터 쿼리에 대해 매우 유용한 선택임을 입증합니다.
...
### Interpretable Early Warnings using Machine Learning in an Online Game-experimen (https://arxiv.org/abs/2502.09880)
- **What's New**: 이 논문은 기후 및 생태학적 시스템에서의 전환을 예측하는 데 있어 기계 학습 기반 경고 시스템의 사용을 소개합니다. Reddit의 r/place 실험을 통해, 수천 개의 하위 시스템에서 통계적인 조기 경고 신호를 탐지하는 새로운 방법을 개발하였습니다. 경고 시스템은 시스템 특화 시계열 데이터의 예측 기능을 결합하여 만들어졌으며, 기존의 조기 경고 지표보다 성능이 우수합니다.

- **Technical Details**: 연구팀은 XGBoost 모델을 사용하여 여러 전환 시나리오에 대한 예측을 수행하였습니다. 특히, 예측 정확도가 시간에 따라 어떻게 변하는지를 분석하였고, 20분 내의 전환을 3.7%의 거짓 긍정률로 탐지할 수 있음을 확인하였습니다. SHAP(Shapley Additive exPlanations)를 활용하여 예측의 기초가 되는 원인을 조사하였고, 결과적으로 패턴 변화의 주요 동인을 파악할 수 있었습니다.

- **Performance Highlights**: 모델은 2022년 r/place 데이터에서 훈련되어 2023년 데이터에 대해서도 높은 예측 정확도를 유지하고 있습니다. 다양한 경고 범위에서의 성능을 평가한 결과, 특정 전환에서는 AUC(Area Under Curve) 값이 0.8을 초과하는 경우가 41%에 달했습니다. 전체 변수를 고려한 경우, 단일 변수 경고 지표보다 월등한 예측 능력을 보여주며, 조기 경고 시스템이 복잡한 사회 생태계 시스템에 미치는 잠재력을 강조합니다.
...
- **What's New**: 이 논문은 화학 산업에서 분자의 발견이 가져온 주요 혁신을 다루고 있습니다. 기존의 유전자 알고리즘은 무작정 탐색 방식에 의존하고 있어 솔루션의 품질과 수렴 속도에 제한이 있었습니다. 반면, 본 연구에서는 Gradient Genetic Algorithm (Gradient GA)을 제안하여 목표 함수의 기울기 정보를 통합해 최적 솔루션으로의 반복적 진행을 가능하게 합니다.

- **Technical Details**: Gradient Genetic Algorithm은 분자 설계를 위해 그래프 신경망(GNN)을 사용하여 미분 가능한 목표 함수를 학습하며, Discrete Langevin Proposal (DLP)을 통해 기울기 정보를 이용한 탐색을 가능하게 합니다. 이는 혼합적인 화학 공간을 효과적으로 탐색할 수 있게 하며, 변이, 선택, 교차 등의 전통적인 GA 연산자를 개선합니다. 이 접근법을 통해 솔루션 품질과 수렴 속도가显著 향상되었습니다.

- **Performance Highlights**: 실험 결과에 따르면, Gradient GA는 기존의 유전자 알고리즘보다 최고 25% 더 향상된 성능을 보여주며, 여러 최첨단 방법들과 비교했을 때 일관된 개선을 입증했습니다. 이러한 성능 향상은 특히 mestranol 유사성 속성을 최적화할 때 더욱 두드러지며, 본 연구는 기울기 정보를 활용하여 분자를 탐색하는 첫 번째 방법임을 강조합니다.
...
### Algorithmic contiguity from low-degree conjecture and applications in correlated random graphs (https://arxiv.org/abs/2502.09832)
Comments:
          40 pages. arXiv admin note: text overlap with arXiv:2311.00289 by other authors

- **What's New**: 이 논문에서는 낮은 차수 가설( low-degree conjecture)의 자연스러운 강화에 따라 스파스 상관 에르되시-레니 그래프의 (부분) 매칭 복구 문제와 상관된 희소 확률 모델 간의 탐지 문제에 대한 계산적 난이도를 입증합니다.

- **Technical Details**: 제시된 알고리즘적 근접성(algorithmic contiguity)의 개념을 사용하여 두 확률 분포 간의 관계를 파악하고, low-degree advantage에 대한 경계를 기반으로 특정 형태의 근접성을 도출합니다. 이를 사용하여 빠른 알고리즘이 존재하지 않음을 보여주며, 이 프레임워크는 다양한 추론 작업 간의 축소를 수행하는 데 유용한 도구가 됩니다.

- **Performance Highlights**: 연구 결과는 특정 조건에서 계산적 난이도를 분석하는 중요한 통찰을 제공합니다. 이는 앞으로의 이론적 및 실용적 응용 프로그램에서 그래프 매칭 및 관련 문제의 해법을 찾는 데 기여할 수 있음을 의미합니다.
...
### $Λ$CDM and early dark energy in latent space: a data-driven parametrization of the CMB temperature power spectrum (https://arxiv.org/abs/2502.09810)
Comments:
          17 pages, 12 figures, comments welcome

- **What's New**: 최근 우주론 데이터의 개선으로 ΛCDM 모델의 여섯 가지 매개변수를 점점 더 정밀하게 결정할 수 있게 되었습니다. 그러나 이러한 매개변수들 간의 긴장이 발생하여 새로운 대안 모델들에 대한 연구가 촉진되었습니다. 이 연구에서는 변별 가능한 latent 표현을 이용한 데이터 기반의 매개변수를 제안하여 우주 마이크로파 배경(CMB) 온도 전력 스펙트럼을 압축하는 방법을 제안합니다.

- **Technical Details**: 변이자동인코더(Variational Autoencoder, VAE)는 CMB 온도 전력 스펙트럼을 주어진 latent 변수 개수로 압축할 수 있습니다. 이 방법은 ΛCDM과 조기 어두운 에너지(Early Dark Energy, EDE) 모델을 가정한 두 가지 CMB 스펙트럼 세트를 사용하여 훈련됩니다. 얻어진 latent 변수들은 데이터 최적화된 매개변수를 나타내며, 이는 모델의 물리적 해석을 도울 수 있습니다.

- **Performance Highlights**: 제안된 방법론을 통해 압축된 스펙트럼은 Planck 오차 범위 내에서 높은 정확도로 복원됩니다. 또, VAE는 EDE 효과 및 ΛCDM 매개변수와 관련된 효과를 완전히 분리하는 하나의 latent 매개변수를 발견하여 새로운 자유도를 드러냅니다. 마지막으로 Planck 데이터를 사용하여 얻은 결과는 이전의 ΛCDM 및 EDE 우주론적 제약들과 일치하는 값들을 보여주었습니다.
...
### Reconstruction of frequency-localized functions from pointwise samples via least squares and deep learning (https://arxiv.org/abs/2502.09794)
- **What's New**: 본 논문은 신호 처리에서 주파수 지역화된 함수(frequency-localized functions)를 점(data) 기반의 샘플로부터 복원하는 새로운 접근법을 제시합니다. 특히, 최적화된 방법들로 최소제곱법(least squares)과 딥러닝(deep learning)을 채택하여 정확한 함수 근사를 위한 이론적 근거를 제공합니다. 이를 통해 주파수 제한 함수(bandlimited functions)의 복원이 이론적으로 가능함을 보이며, 현대적 신경망 구조에 대한 새로운 기여도 포함되어 있습니다.

- **Technical Details**: 저자들은 새로운 복원 정리를 세우며 Slepian 기저(Slepian basis)를 사용한 최소제곱 근사 방법에 대해 논의합니다. 이 기저는 주파수 지역화된 함수들에 대해 우수한 수학적 성질을 보여주며, 특히 주파수 및 공간(또는 시간)에서의 최적 집중을 지원합니다. 연구는 이러한 기법을 바탕으로 깊은 신경망이 어떤 조건 하에 정확한 근사를 할 수 있는지를 다룹니다.

- **Performance Highlights**: 실험 결과, 최소제곱법과 딥러닝 기반 방법 모두 대차밋한 추정 오차를 보이며 데이터 수가 증가할 때 수렴성을 나타냈습니다. 또한 함수의 도메인(domain)이 성능에 미치는 영향을 강조합니다. 이러한 연구는 신호 처리 분야의 다양한 응용 분야에 중요한 이론적 및 실용적 기여를 할 것으로 기대됩니다.



### ExoMiner++ on TESS with Transfer Learning from Kepler: Transit Classification and Vetting Catalog for 2-min Data (https://arxiv.org/abs/2502.09790)
- **What's New**: 새로운 ExoMiner++ 모델은 TESS 데이터의 2분 주기로 측정된 transit 신호 분류를 개선하기 위해 ExoMiner의 성공을 기반으로 한 심화된 딥 러닝 모델입니다. 본 모델은 periodogram, flux trend, difference image, unfolded flux 및 우주선 태세 제어 데이터 같은 추가 진단 입력을 통합하여 transit 신호를 더 효과적으로 식별합니다. 또한, Kepler 우주 망원경에서의 고품질 레이블이 있는 데이터로부터 전이 학습을 활용하여 TESS의 더 많은 노이즈와 모호한 레이블의 영향을 줄이고자 했습니다.

- **Technical Details**: ExoMiner++는 여러 분류 및 순위 메트릭에서 높은 정확성을 달성하며, 후속 조사가 필요한 새로운 행성을 확인하기 위한 검색 공간을 크게 축소합니다. 총 147,568개의 비식별 TCE 중 7,330개를 행성 후보로 식별하였으며, 이러한 후보들은 1,868개의 기존 TESS 입회 객체(TOIs), 69개의 커뮤니티 TESS 객체(CTOIs), 그리고 50개의 새로운 CTOIs와 일치합니다. 또한, 이전에 행성 후보로 분류되었던 2,506개의 TOI 중 1,797개는 ExoMiner++에 의해 행성 후보로 재분류되었습니다.

- **Performance Highlights**: ExoMiner++의 뛰어난 순위 품질 덕분에, 후속 연구는 가장 가능성이 높은 후보에게 집중될 수 있게 되어 전반적인 행성 발견 가능성을 증가시킵니다. 또한, TESS 데이터의 독특한 관측 방식으로 인해 발생할 수 있는 다양한 과제를 조명하고, 자동화된 TCE 분류를 통해 분석 파이프라인을 간소화하며, 수동 검토의 필요성을 줄이고 있습니다. 이로 인해 TESS 임무의 결과물인 신규 행성 발견의 유효성이 높아지기 기대됩니다.
...
- **What's New**: 본 논문에서는 Compliance Refusal Initialization (CRI)이라는 새로운 공격 비분리 초기화 프레임워크를 제안합니다. 이 프레임워크는 해로운 프롬프트의 준수 서브스페이스에 근접하게 최적화를 효율적으로 초기화하여, 기존의 자동 jailbreak 공격이 가진 높은 계산 자원 소모 문제를 해결합니다. CRI를 사용하면 성공적인 공격 비율이 크게 향상되며, 종종 단일 최적화 단계만으로도 결과를 얻을 수 있습니다.

- **Technical Details**: CRI는 공격에 독립적인 프레임워크로 설계되어 있으며, 사전 훈련된 jailbreak 프롬프트를 활용하여 보이지 않는 프롬프트에 대한 공격을 초기화합니다. 이를 통해 CRI는 해로운 프롬프트와 준수 프롬프트 간의 평균적인 차이를 반영한 임베딩 공간의 거부 방향을 고려하여 공격을 수행합니다. 이 방향은 최적화 과정에서 프롬프트의 변화를 나타내며, 초기화 단계에서의 거리 단축으로 공격 성공률을 높이고 계산 오버헤드를 줄입니다.

- **Performance Highlights**: 실험 결과, CRI는 AdvBench 데이터셋에서 GCG 및 AutoDAN에 대한 기존의 표준 jailbreak 공격보다 최대 60배까지 중앙 성공 단계수를 감소시켰습니다. 이는 공격 수행에 필요한 단계 수를 획기적으로 줄여, 공격의 효율성을 크게 향상시킵니다. CRI는 여러 모델에 대해 평가되었으며, 공격 효율성을 유지하면서 성능을 극대화하는 데 기여했습니다.
...
### Iterative quantum optimisation with a warm-started quantum sta (https://arxiv.org/abs/2502.09704)
Comments:
          feedback welcome, 13 pages, 12 figures

- **What's New**: 이 논문은 측정을 통한 따뜻하게 시작한 양자 상태(warm-started quantum state)를 준비하는 방법을 제안하며, 양자 근사 최적화 알고리즘(QAOA)의 성능을 향상시키기 위한 반복적인 프레임워크를 소개합니다. 수치 시뮬레이션 결과, 이 방법이 표준 QAOA의 '막힘 문제(stuck issue)'에 효과적으로 대응할 수 있음을 보여주었습니다. 특히, 3-정규(MaxCut) 문제에 적용했을 때 보다 향상된 근사 비율을 달성하였으며, 포트폴리오 최적화에서도 유리한 결과를 나타냅니다.

- **Technical Details**: 초기 상태 준비(initial state preparation)는 변량 양자 알고리즘(VQA)의 수렴 시간을 줄이고 최적화 품질을 향상시키는데 중요한 역할을 합니다. Hartree-Fock 상태, Hardamard 변환, Dicke 상태와 같은 여러 초기 상태가 널리 사용되며, 이러한 따뜻한 시작한 방법은 여러 응용 분야에서 장점을 보여주었습니다. 이 논문에서는 이전 방법들의 한계를 극복하기 위해 최상의 측정된 고전 문자열(superposing the best-measured classical strings)을 활용한 새로운 양자 상태 준비 방법을 제안하며, 이를 통해  QAOA의 근사 비율을 Iteratively 개선하고자 합니다.

- **Performance Highlights**: MaxCut와 DGMVP 모델에 대한 시뮬레이션을 통해 제안된 방법이 단일 문자열 초기 상태 방법의 막힘 문제를 피하며, 포트폴리오 최적화에서 더 나은 최적 솔루션을 찾는 데 도움이 된다는 긍정적인 결과를 얻었습니다. 이러한 결과는 양자 알고리즘 설계에 대한 새로운 관점을 제시하며, 다양한 고전적 모델에서 QAOA의 효과를 확장할 수 있는 가능성을 보여줍니다. 제안된 방법은 특히 조합 최적화(combinatorial optimisation)의 여러 적용 사례에 효과적일 것으로 기대됩니다.
...
### Lifespan tree of brain anatomy: diagnostic values for motor and cognitive neurodegenerative diseases (https://arxiv.org/abs/2502.09682)
- **What's New**: 이 연구에서는 여러 신경퇴행성 질환을 동시 진단할 수 있는 새로운 머신러닝 프레임워크인 'lifespan tree of brain anatomy'를 개발하였습니다. 이 프레임워크는 뇌 구조의 부피 변화 모델링을 통합하여 질병 진행 과정에 따른 뇌 해부학의 해석 가능한 표현을 생성합니다. 이를 통해 기존의 방법들과는 달리, 과거에 학습된 개별 질환을 진단하기 위한 방식에서 탈피하여 여러 질환을 동시에 분별하는 것이 가능해졌습니다.

- **Technical Details**: 이 프레임워크는 124개의 뇌 구조에 대한 부피 변화와 비선형 차원 축소(non-linear dimensionality reduction), 합성 샘플링(synthetic sampling) 기법을 결합하여 개발되었습니다. 연구는 37594개의 MRI 데이터를 training dataset으로 사용하여 신경퇴행성 치매의 6가지 원인 및 파킨슨증의 4가지 원인에 대한 인지 및 운동 lifespan tree를 구축했습니다. 이 모델은 신경퇴행성 질환의 서로 다른 원인을 효과적으로 구별하는 데 중점을 두고 설계되었습니다.

- **Performance Highlights**: 이 연구에서 제안된 lifespan tree는 1754명의 외부 검증 코호트에서 기존의 최신 머신러닝 기술들을 능가하는 성과를 보였습니다. 특히, 효과적인 생물학적 바이오마커가 부족한 질병에 대한 진단 도구로서의 가능성을 제시합니다. 이 방법은 신경퇴행성 질환의 차별적 진단에 있어 상당한 효율성을 높여줄 것으로 기대됩니다.
...
### On the Bias, Fairness, and Bias Mitigation for a Wearable-based Freezing of Gait Detection in Parkinson's Diseas (https://arxiv.org/abs/2502.09626)
Comments:
          Submitted to IMWUT 2025

- **What's New**: 이 연구는 Parkinson's disease (PD)의 중요한 증상 중 하나인 Freezing of gait (FOG)의 자동 검출을 위한 기계학습(model) 시스템의 공정성과 편향을 다루고 있습니다. 특히, 다양한 인구 통계학적 특성을 고려하여 여러 데이터셋에서 HAR(human activity recognition) 모델의 편향을 분석하였습니다. FOG의 발생이 환자 간에 이질적이기 때문에, 다양한 FOG 유형에 대해 일관되게 평가할 수 있는 모델 개발이 중요합니다.

- **Technical Details**: 연구는 demographic parity ratio (DPR) 및 equalized odds ratio (EOR)와 같은 공정성 지표를 사용하여 HAR 모델의 편향을 평가했습니다. FOG 데이터셋의 미세한 차이를 고려하기 위해 transfer learning을 활용하여 멀티 사이트 데이터 및 일반적인 인간 활동 표현을 훈련 데이터로 사용했습니다. 이를 통해 다양한 인구 집단의 공정성과 성능을 개선할 수 있음을 증명하였습니다.

- **Performance Highlights**: 연구 결과, transfer learning을 적용한 HAR 모델은 DPR 및 EOR 값에서 평균적으로 각각 +0.027 및 +0.039의 개선을 보여주었습니다. F1-score에서도 +0.026 및 +0.018의 성능 향상을 가져왔습니다. 이 연구는 다양한 인구 통계적 특성을 만족할 수 있는 건강 분석을 위한 공정한 모델 훈련의 중요성을 강조합니다.



### Transformer Based Time-Series Forecasting for Stock (https://arxiv.org/abs/2502.09625)
- **What's New**: 이 연구는 주식 가격 예측을 단순한 자기 회귀 문제 대신, 다변량 예측(multivariate forecasting) 문제로 모델링하였습니다. 'Stockformer'라는 변형된 트랜스포머 아키텍처를 활용하여, 주식 가격 예측에 필요한 시계열 데이터를 통합적으로 분석할 수 있는 방법을 제시합니다. 이를 통해, 기계 학습 및 인공지능의 접근 방식이 주식 시장의 비효율성에 기반하여 보다 효과적인 예측을 가능하게 함을 보여줍니다.

- **Technical Details**: 연구에서는 다중 상관 주식 가격 데이터와 금융 정보를 사용하여, 한 시간 후의 주식 가격을 예측하고자 했습니다. 주식 가격을 예측하기 위해서, 연구팀은 변형된 Transformer 아키텍처인 Stockformer를 구현했고, 이 구조는 특정 산업 내에서 주식 가격 간의 관계를 이용할 수 있도록 설계되었습니다. 특히, 주식 시장의 비효율성을 고려한 다변량(time series) 접근 방식이 적용되었습니다.

- **Performance Highlights**: 실험 결과, Stockformer는 정확한 주식 가격 예측에 있어 기존의 전통적인 접근 방식보다 더 좋은 성과를 보여주었습니다. 연구진은 다양한 데이터 출처를 활용하여 시간당 이력 데이터를 기반으로 학습하여, 트레이더가 더 나은 결정을 내릴 수 있도록 지원할 수 있는 가능성을 크게 향상시켰습니다. 특히, 변형된 트랜스포머 구조는 장기 의존성을 효율적으로 학습할 수 있는 이점을 제공하였습니다.
...
### LucidAtlas$: Learning Uncertainty-Aware, Covariate-Disentangled, Individualized Atlas Representations (https://arxiv.org/abs/2502.08445)
Comments:
          28 pages

- **What's New**: 이번 연구에서는 고차원 데이터 세트로부터 정보를 추출하기 위한 원칙적인 기술을 개발하고 있으며, 이는 개인 및 인구 수준의 변동성을 이해할 수 있도록 돕습니다. 새로운 접근법인 $	exttt{LucidAtlas}$를 개발하여 공간적으로 변화하는 정보를 효과적으로 표현하고, 공변량(covariates)과 인구 불확실성을 포착할 수 있는 강력한 기능을 제공합니다. LucidAtlas는 질병 분석을 위한 포괄적인 도구를 제공하면서 기존의 모델보다 한층 진일보한 해석 성능과 유연성을 자랑합니다.

- **Technical Details**: LucidAtlas 모델은 공변량 분리(covariate disentanglement)와 불확실성 추정(uncertainty estimation)을 통합하여 관찰된 데이터를 기반으로 환자 개별 예측(individualized prediction)을 가능하게 합니다. 또한, Marginalization 접근 방식을 통해 개별 예측 변수가 모델의 응답(atlas)과 어떻게 의존하는지를 설명할 수 있습니다. 이 모델은 기존의 Neural Additive Model(NAM)을 확장하여 불확실성 정량화(quantification) 및 선험적 지식(prior knowledge)을 통합할 수 있는 가능성을 제공합니다.

- **Performance Highlights**: 연구는 OASIS Brain Volume 데이터 세트와 Pediatric Airway Shape 데이터 세트를 통해 LucidAtlas의 일반화 가능성을 검증했습니다. 실험 결과, LucidAtlas는 기존의 방법들에 비해 우수한 성능을 보였으며, 해석 가능성을 강조하면서 과학적 발견을 진전시키는 데 중요한 역할을 한다고 요약할 수 있습니다. 또한, 이 연구에서는 NAM을 사용할 때 발생할 수 있는 잠재적 위험성과 공변량 해석의 문제도 논의하였습니다.

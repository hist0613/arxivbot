...
- **What's New**: 이 논문에서는 Sparse Autoencoders (SAEs)의 한계를 극복하기 위해 Low-Rank Adapters (LoRA)를 활용하여 언어 모델을 미세 조정하는 방법을 제안합니다. 기존 연구들은 SAEs의 훈련 시 많은 리소스를 요구했지만, 새로운 접근법을 통해 모델과 SAEs의 결합을 최적화하여 성능을 향상시켰습니다. 특히, SAEs가 모델에 삽입될 때 cross entropy loss가 30%에서 55%까지 감소하는 것을 확인했습니다.

- **Technical Details**: SAE는 언어 모델의 숨겨진 상태를 분해하여 선형 특징으로 변환하는 방법입니다. 이 과정에서 모델의 logits와 SAEs의 logits 간의 KL divergence를 감소시키기 위해 LoRA가 사용되었습니다. 연구에서는 Gemma Scope family의 SAEs를 활용하여 SAE의 폭(SAE width), sparsity, 언어 모델 크기, LoRA 차원(LoRA rank)과 층(layer)의 조합을 분석했습니다.

- **Performance Highlights**: 연구 결과, 새로운 접근법은 기존 end-to-end SAEs에 비해 3배에서 20배 빠른 속도로 동일한 downstream cross entropy loss를 달성할 수 있음을 보여주었습니다. LoRA 기법을 활용하여 다양한 SAEs의 결합 시 성능 향상을 확인하였으며, 훈련 시간과 리소스 소비를 획기적으로 줄일 수 있었습니다. 전반적으로, 모델 이해 가능성(interpretability)과 성능 간의 균형을 개선하는 간단하면서도 효율적인 방법으로 확인되었습니다.
...
### Detection Is All You Need: A Feasible Optimal Prior-Free Black-Box Approach For Piecewise Stationary Bandits (https://arxiv.org/abs/2501.19401)
Comments:
          13 pages, 5 figures

- **What's New**: 이번 연구에서는 기존의 Non-Stationarity(비정상성)에 대한 사전 지식 없이도 사용할 수 있는 piecewise stationary bandits(부분적으로 정적인 밴딧) 문제를 다룹니다. 제안하는 Detection Augmented Bandit(DAB) 알고리즘은 모듈 방식으로 작동하며, 기존의 stationary bandit 알고리즘에 변화 감지기를 추가하여 최적의 regret을 달성합니다. DAB는 다양한 파라메트릭 밴딧 환경에서 현재의 연구 결과를 재현하며, 특히 self-concordant bandits(자기 일치 밴딧)에서는 기존 연구보다 우수한 성과를 나타냅니다.

- **Technical Details**: DAB 알고리즘은 주어진 stationary bandit 알고리즘을 입력으로 받아들여 이를 변화 감지기로 향상시키는 구조를 가지고 있습니다. 이 알고리즘은 piecewise stationary 환경에서의 regret을 \(\tilde{\mathcal{O}}(\sqrt{N_T T})\)로 최적화합니다. 또한, 알고리즘은 각 변화 지점 사이의 파라미터가 상수임을 가정하며, 이러한 점에서 이전 연구들과 차별화됩니다.

- **Performance Highlights**: DAB는 다양한 변화의 수에 따라 시뮬레이션에서도 기존 방법들보다 뛰어난 성능을 보였습니다. 비록 DAB가 이론적으로 부분적으로 정적인 환경을 위해 설계되었음에도 불구하고, 실제로는 drifting environments(변동 환경)에서도 기존의 특화된 방법들보다 뛰어난 성능을 자랑합니다. 이 연구는 다양한 응용 분야에서의 이론적 기반을 확립하고, 향후 연구에 중요한 기여를 하고 있습니다.
...
- **What's New**: 이번 연구에서는 Key-Value (KV) 캐시의 압축 방식을 개선하기 위한 AQUA-KV라는 새로운 접근방식을 제안합니다. 기존의 압축 기술들이 품질을 저하시키는 경우가 많았지만, AQUA-KV는 계층 간의 의존성을 활용하여 정보 손실을 최소화하면서 압축을 최적화합니다. 특히, 2비트의 놀라운 압축 성과를 보이며, 1-6시간 이내에 단일 GPU에서 캘리브레이션이 가능합니다.

- **Technical Details**: AQUA-KV는 Key와 Value 간의 내재된 의존성을 파악하여 효율적인 압축을 달성합니다. 이를 통해 다양한 양자화(quantization) 기법과 호환될 뿐만 아니라, 트리밍(pruning) 기법과 결합할 수 있습니다. 작동 과정은 간단하며, 경량화된 캘리브레이션 과정으로 최적화된 양자화 정확도를 제공합니다.

- **Performance Highlights**: 최신 LLM 패밀리에 대해 AQUA-KV는 perplexity와 zero-shot 정확도에서 기존 기술보다 현저한 성능 향상을 보입니다. Llama 3.2 모델에서, 2-2.5비트의 높은 압축율을 유지하면서도 상대 오류율이 1% 미만인 거의 무손실 추론을 달성하였습니다. AQUA-KV는 다양한 양자화 및 트리밍 조합에서도 효과적으로 작동하여, 다채로운 압축 기법의 이점을 제공합니다.
...
- **What's New**: 이번 논문은 federated sketching LoRA (FSLoRA)라는 새로운 방식의 기법을 제안합니다. FSLoRA는 sketching 메커니즘을 활용하여 서버가 유지하고 있는 LoRA 모듈의 부분 행렬을 선택적으로 업데이트할 수 있도록 합니다. 이를 통해 디바이스의 통신 및 계산 제약에 맞춰 동적으로 LoRA의 랭크를 조정할 수 있습니다. 이 기법은 LLM(대형 언어 모델)의 퍼포먼스를 획기적으로 개선할 것으로 기대됩니다.

- **Technical Details**: FSLoRA는 디바이스에서 LoRA 모듈의 서브매트릭스를 조정하여 계산 및 통신 소비를 줄입니다. 특히, sketching 비율을 조정하여 기기별 장치의 통신 능력과 계산 능력에 맞게 조절할 수 있습니다. 또한, 연구팀은 FSLoRA의 수렴 속도에 미치는 영향을 분석하여, sketching 비율을 변화시키는 것이 최적화 과정에 미치는 영향에 대한 이론적 분석을 진행합니다. 이를 통해, FSLoRA는 비균질적인 서브매트릭스 업데이트 시나리오에서도 효과적으로 작동함을 보여줍니다.

- **Performance Highlights**: 다양한 데이터셋과 LLM 모델을 통한 실험 결과, FSLoRA는 기존의 여러 기법들에 비해 뛰어난 성능을 나타냈습니다. 특히, 테스트 정확도, 훈련 시간 및 리소스 활용 측면에서 개선된 결과를 보여주었습니다. 이러한 결과는 FSLoRA의 sketching 메커니즘이 모델의 적응성과 효율성을 극대화할 수 있는 가능성을 뒷받침합니다.
...
### Fixing the Double Penalty in Data-Driven Weather Forecasting Through a Modified Spherical Harmonic Loss Function (https://arxiv.org/abs/2501.19374)
- **What's New**: 이번 연구에서는 기존의 데이터 기반 날씨 예측 모델들이 겪고 있는 '이중 패널티(double penalty)' 문제를 해결하기 위해 손실 함수(loss function)를 수정했습니다. 이 새로운 손실 함수는 분산(amplitude) 보존과 전통적인 평균 제곱 오차(mean squared error, MSE)를 부드럽게 보간(interpolate)함으로써 예측의 정확성을 높였습니다. 이러한 방식으로 GraphCast 모델을 재조정(fine-tune)하여 날씨 예보에서의 정확성을 크게 향상시켰습니다.

- **Technical Details**: 기존의 데이터 기반 모델들이 MSE 손실 함수를 사용하는 것은 예측의 미세한 스케일을 감소시켜 부정확한 평활화(smoothing)를 유도하는 경향이 있습니다. 이 연구에서는 MSE 손실 함수를 변경하여 예측의 날카로움(sharpness)과 효과적인 해상도(effective resolution)를 높이는 방법을 제안합니다. 손실 함수는 스펙트럼 진폭(spectral amplitude) 오류와 비상관(decorrelation) 오류를 명확히 분리하여 최적화됩니다.

- **Performance Highlights**: 새로운 손실 함수를 통한 fine-tuning 결과, GraphCast 모델의 예측 해상도가 기존 1,250km에서 160km로 증가했으며, 열대 사이클론(tropical cyclone) 세기와 표면 바람 강도 예측의 정확성이 현저히 개선되었습니다. 또한, 모델의 앙상블 스프레드(ensemble spread)도 향상되어, 더욱 신뢰할 수 있는 날씨 예보가 가능해졌습니다.
...
- **What's New**: 본 논문은 Reinforcement Learning from Human Feedback (RLHF)에서 나타나는 Energy Loss Phenomenon을 식별하고, 이 현상이 보상 해킹(reward hacking)과 어떻게 연결되는지를 분석합니다. 특히, Large Language Model (LLM)의 마지막 레이어에서 에너지 손실이 증가하는 과정을 보여주며, 보상 해킹은 이 에너지 손실의 과도한 증가로 특징지어진다고 다룹니다. 뿐만 아니라, 이 문제를 해결하기 위해 에너지 손실을 인식하는 PPO 알고리즘(EPPO)을 제안하여 보상 계산 시 에너지 손실 증가에 대해 패널티를 부과합니다.

- **Technical Details**: 제안된 EPPO 알고리즘은 LLM의 마지막 레이어에서의 에너지 손실을 집중적으로 조정하여 보상 해킹을 완화하려는 접근 방식을 사용합니다. 기존의 RLHF 알고리즘들이 출력 공간에 직접적인 제약을 가하는 것과는 달리, EPPO는 에너지 손실에 초점을 맞추어 정책 탐색과 최적화를 위한 더 넓은 범위를 제공합니다. 이 과정을 통해 LP 알고리즘을 이론적으로 엔트로피 정규화된 RL 알고리즘으로 해석할 수 있음을 보여줍니다.

- **Performance Highlights**: 다양한 LLM 및 작업에 대한 광범위한 실험을 통해 에너지 손실 현상이 일반적으로 존재함을 입증하고, EPPO의 보상 해킹 완화 및 RLHF 성능 향상에 대한 효과를 실증합니다. 연구에서는 Llama3-8B, Llama2-7B, Mistral-7B, Deepspeek-7B 등 네 가지 인기 LLM과 일반적인 대화 및 요약 작업을 대상으로 하여 EPPO의 우수성을 강조합니다. 이러한 결과들은 RLHF의 성능을 더욱 개선할 수 있는 새로운 접근법으로 기여할 것으로 예상됩니다.



### Neural Implicit Solution Formula for Efficiently Solving Hamilton-Jacobi Equations (https://arxiv.org/abs/2501.19351)
- **What's New**: 이 논문은 Hamilton-Jacobi 편미분 방정식(HJ PDE)의 암시적 해법을 제시합니다. 이 해법은 특징의 방법(method of characteristics)을 통해 유도되며, 해밀토니안(Hamiltonian)이나 초기 함수가 convex일 경우 Hopf와 Lax의 공식을 만족함을 보입니다. 이 방법은 HJ PDE의 비오정확도(vicinity solution)를 계산하기 위해 간단하고 효율적인 수치적 접근 방식을 제공하며, 해밀토니안이나 초기 조건의 Legendre 변환을 요구하지 않습니다.

- **Technical Details**: 논문은 HJ PDE의 암시적 해법 공식을 도입하며, 이 공식은 HJ PDE의 특성을 통해 유도됩니다. 초기 상태에서 현재 시점까지의 특징 ODE 시스템을 해결할 필요 없는 방식으로 설계되어, 효과적인 최적 제어의 관점에서도 탐구됩니다. 제안된 공식을 이용하여 deep learning 기반의 방법을 개발하며, 이는 Lipschitz 연속 함수로서 솔루션을 근사합니다.

- **Performance Highlights**: 제안된 방법은 40차원 문제에서도 높은 정확도의 솔루션을 제공함을 보여주며, 비공식적인 HJ PDE에서도 우수한 성능을 기록합니다. 상태 의존 해밀토니안을 취급하는 확장된 접근 방법을 통해, 단편 선형 근사법을 적용하면서 복잡한 최적 제어 문제와 동적 시스템을 해결하는 데 유용함을 입증합니다.



### An All-digital 65-nm Tsetlin Machine Image Classification Accelerator with 8.6 nJ per MNIST Frame at 60.3k Frames per Second (https://arxiv.org/abs/2501.19347)
Comments:
          10 pages, 6 figures. This work has been submitted to the IEEE for possible publication

- **What's New**: 본 논문은 Tsetlin 머신(Tsetlin Machine, TM) 원리에 기반한 완전 디지털 프로그래머블 기계 학습 가속기 칩을 제시합니다. 이 가속기는 합친 TM 버전을 활용하여 28x28 픽셀의 불리언화된 이미지를 10개 카테고리로 분류하며, 128개의 클로즈를 사용하는 고도로 병렬 아키텍처를 구현합니다. 가속기의 동작은 65nm 저소비 전력 CMOS 기술로 이루어졌으며, 제조된 최초의 TM ASIC입니다.

- **Technical Details**: Tsetlin 머신은 명제 논리에 기반한 기계 학습 알고리즘으로, 독립적인 학습 자동자에 의해 정의된 병렬 접합 클로즈를 사용합니다. 이 연구에서 제안하는 컨볼루션 CoTM은 컨볼루션을 통해 작동하며, 클로즈 풀을 단일로 사용하여 리소스가 제한된 환경에서도 높은 정확도를 달성할 수 있도록 돕습니다. ASIC의 구조는 표준 디지털 워크플로우와 호환되는 완전 디지털 동기 설계를 채택하고 있습니다.

- **Performance Highlights**: 본 가속기는 초당 60.3k개의 분류 성능을 자랑하며, 각 분류당 8.6nJ의 에너지를 소비합니다. MNIST, Fashion-MNIST, Kuzushiji-MNIST 데이터셋에 대해 각각 97.42%, 84.54%, 82.55%의 테스트 정확도를 달성하였으며, 이는 TM 소프트웨어 모델과 일치하는 성능입니다. 낮은 지연시간(25.4μs) 또한 저전력 시스템에서의 실시간 운영에 필수적입니다.



### PUATE: Semiparametric Efficient Average Treatment Effect Estimation from Treated (Positive) and Unlabeled Units (https://arxiv.org/abs/2501.19345)
- **What's New**: 이 연구는 평균 치료 효과(ATE) 추정의 새로운 접근 방식을 제안하는데, 특히 치료 그룹과 치료 여부가 불확실한 그룹만을 고려하여 ATE를 추정하는 방법을 개발하였다. 기존의 ATE 추정은 치료 및 대조 그룹 모두에 대한 정보를 요구하는 반면, 이 연구에서는 부분적인 정보만으로도 ATE를 추정할 수 있는 새로운 방법론을 제시한다. 이 경우는 Positive and Unlabeled Data (PU learning)와 관련이 있으며, 결측 데이터를 가진 ATE 추정의 특정 케이스로 볼 수 있다.

- **Technical Details**: 이 연구에서는 먼저 세미파라메트릭(semi-parametric) 효율 경계를 도출하여 일반 추정량의 점근적 분산(asymptotic variance)에 대한 하한을 제공한다. 이어서 효율적인 영향 함수(efficient influence function)를 활용하여 세미파라메트릭 ATE 추정량을 개발하고, 그 추정량은 점근적 분산이 효율 경계에 도달하도록 설계된다. 연구에서는 PU 설정과 관련된 두 가지 데이터 생성 프로세스(data generating processes, DGPs)를 다루며, 각 DGP에 대한 효율적인 추정량을 제안한다.

- **Performance Highlights**: 연구에서 제안된 새로운 ATE 추정량은 시뮬레이션 연구를 통해 그 효율성과 성능이 입증될 것이다. 이 새로운 접근법은 결측 처리(missing data)와 약한 감독 학습(weakly supervised learning) 환경에서도 유용하게 적용될 수 있다. 연구 결과는 금융, 의료, 사회 과학 등 다양한 분야에 걸쳐 효과적으로 활용될 수 있는 가능성을 제시한다.



### Covering Multiple Objectives with a Small Set of Solutions Using Bayesian Optimization (https://arxiv.org/abs/2501.19342)
- **What's New**: 이번 논문에서는 기존의 multi-objective optimization 접근방식과는 다르게 K개(K < T)의 솔루션을 찾아 T개의 객관적 목표를 모두 '커버'하는 새로운 문제 설정을 제안합니다. 이는 약물 설계처럼 각 병원체에 하나 이상의 효과적인 항생제를 찾는 실제 문제에 적용될 수 있으며, 각 솔루션이 모든 객관적 목표를 동시에 최적화할 필요가 없음을 보여줍니다. 저자들은 Multi-Objective Coverage Bayesian Optimization (MOCOBO) 알고리즘을 통해 이 문제를 해결하는 방법을 제시했습니다.

- **Technical Details**: MOCOBO는 고차원(Higher-dimensional) 객체와 구조화된 입력 영역에 대한 실험을 통해 검증된 알고리즘으로, 효율적으로 T개의 목적을 커버하는 K개의 솔루션을 찾도록 설계되었습니다. 기존의 방법들과 달리, MOCOBO는 모든 목적에 대한 트레이드오프를 조정하는 대신, 특정 요건을 충족하는 전문화된 솔루션 세트를 식별하는 데 집중합니다. Bayesian optimization의 틀 안에서 문제 해결을 위한 원리적인 알고리즘으로, 각 요건에 대해 최소한 하나의 성능이 뛰어난 솔루션이 존재하도록 하는 '커버링 세트'를 찾습니다.

- **Performance Highlights**: MOCOBO는 복잡한 고차원 문제에서의 엄격한 실험을 통해, K < T 솔루션을 통해 개별적으로 최적화된 T 솔루션과 거의 일치하는 성능을 낼 수 있음을 보여주었습니다. 또한, 약물 발견 실험에서 효과적인 항균 펩타이드가 11개의 병원체 중 9개를 대상으로 뛰어난 효과를 보여주는 성과를 거두었습니다. 이는 다양한 객관적 목표를 충족시키는 동시에 실질적인 약물 설계에 있어 MOCOBO의 잠재력을 강조합니다.
...
### Offline Learning for Combinatorial Multi-armed Bandits (https://arxiv.org/abs/2501.19300)
- **What's New**: 본 논문에서는 조합적 다중팔뚝밴딧(combinatorial multi-armed bandit, CMAB)에 대한 최초의 오프라인 학습 프레임워크인 Off-CMAB를 제안합니다. 기존의 연구들은 주로 온라인 설정에 집중하여 온라인 상호작용의 큰 비용과 쉽게 얻을 수 있는 오프라인 데이터셋을 간과했습니다. Off-CMAB의 주된 알고리즘은 비관적 보상 추정값과 조합적 솔버를 결합한 조합적 낮은 신뢰구간(combinatorial lower confidence bound, CLCB)입니다.

- **Technical Details**: 이 프레임워크는 비선형 보상 함수와 일반적인 확률적 피드백 모델을 지원하며, 최적 움직임과 확인된 움직임 간의 보상 차이를 최소화하는 조합적 행동을 식별하는 것을 목표로 합니다. 두 가지 새로운 데이터 커버리지 조건을 제안하고, 이러한 조건 하에 CLCB가 거의 최적(suboptimality gap)을 달성함을 이론적으로 증명합니다. CLCB는 고확률의 낮은 신뢰구간을 구성하며, 불충분한 관측수의 팔(arms)에 페널티를 부여하여 최적 솔루션을 찾는 과정을 돕습니다.

- **Performance Highlights**: Off-CMAB의 성능은 학습 순서, 대규모 언어 모델(large language model, LLM) 캐싱 및 사회적 영향 극대화를 포함한 실제 응용 프로그램에서 검증되었습니다. 실험 결과, CLCB는 기초 알고리즘에 비해 우수한 성능을 보여주며 비선형 보상 함수 및 기타 복잡한 피드백 모델을 처리하는 능력을 입증했습니다. 이는 오프라인 데이터셋을 활용하여 온라인 탐색에서의 큰 비용과 리스크를 피하면서도 효과적인 의사결정을 가능하게 합니다.



### Differentially Private In-context Learning via Sampling Few-shot Mixed with Zero-shot Outputs (https://arxiv.org/abs/2501.19287)
- **What's New**: 본 논문에서는 Differentially Private Sampling by Mixing One-shot with Zero-shot Outputs (DPS-MOZO)라는 새로운 프레임워크를 제안합니다. 이 프레임워크는 다양한 예시로부터 생성된 데이터의 민감한 정보를 최소화하면서, 질 높은 텍스트 생성을 가능하게 합니다. DPS-MOZO는 샘플링 기법을 통해 생성된 결과물을 조합해 통첩을 통해 사생활을 보호하며, 소음(noise)을 추가하지 않고도 안전성이 확보됩니다.

- **Technical Details**: DPS-MOZO는 다수의 one-shot 출력과 zero-shot 출력을 혼합하여 민감한 정보를 소거한 상태에서 텍스트를 생성하는 차별적 프라이버시(decipher privacy) 디코딩 프레임워크입니다. 이 방법론은 데이터의 차원 수가 상당히 다양한 텍스트 생성 작업에 적합하며, 예전의 연구들에 비해 더 적은 연산 비용으로 DP 쿼리 출력 생성을 가능하게 합니다. 결과적으로 이 방식은 민감한 정보 누출을 방지하면서 시연(demonstration)에서 얻어진 정보를 최대한 활용할 수 있도록 합니다.

- **Performance Highlights**: DPS-MOZO는 SAMSum 데이터셋에서 0.3%의 ROUGE-L F1 점수 감소로 ϵ=2의 강력한 프라이버시 보장을 달성했습니다. 실험 결과, 기존의 비프라이빗 몇 샷 학습(non-private few-shot learning)에 비해 최소한의 유틸리티 저하로 높은 프라이버시 보호 수준을 유지했습니다. 이 연구는 다양한 하이퍼파라미터 값에 대한 견고성을 입증하며, Membershio Inference Attack을 통해 실험적인 프라이버시를 평가합니다.



### OneBatchPAM: A Fast and Frugal K-Medoids Algorithm (https://arxiv.org/abs/2501.19285)
Comments:
          Paper accepted by AAAI 2025

- **What's New**: 이 논문은 큰 규모의 데이터셋을 효과적으로 처리할 수 있는 새로운 k-medoids 근사 알고리즘을 제안합니다. 기존의 메소드를 대체하여, 반복적인 로컬 검색 알고리즘을 통해 medoid 선택을 개선하고, 메모리 사용량을 낮추며 계산 복잡도를 줄입니다. 특히, m << n 조건을 이용하여 필요한 메모리 크기와 쌍별 유사성 계산을 O(mn)으로 줄이는 혁신적인 방법론을 소개합니다.

- **Technical Details**: k-medoids 문제는 n개의 포인트 집합에서 k개의 medoid를 선택하여, 이들 포인트와 가장 가까운 medoid 간의 쌍별 유사성의 합을 최소화하는 것입니다. 기존의 PAM 알고리즘은 메모리와 계산 비용이 O(n^2)에 달하지만, 제안된 OneBatchPAM 알고리즘은 O(log(n)) 크기의 하나의 배치를 사용하여 이 문제를 극복하고 O(T) 속도 향상을 제공합니다. 이러한 접근법은 BanditPAM 방법의 단점을 해결하며, 효과적인 시간 및 목표 값의 균형을 제공합니다.

- **Performance Highlights**: 다양한 크기와 차원의 실제 데이터셋에서 수행한 여러 실험 결과, OneBatchPAM은 FasterPAM 및 BanditPAM++와 유사한 성능을 보이면서도 훨씬 더 단축된 실행 시간을 기록했습니다. 이는 k-medoids 알고리즘 간의 효율적인 교환을 가능하게 하여, 계산 자원을 절약하면서도 최상의 성능을 제공합니다. 이 알고리즘은 특히 대규모 데이터 처리를 요구하는 현대의 기계 학습 응용 분야에서 유용성이 입증되었습니다.



### S-VOTE: Similarity-based Voting for Client Selection in Decentralized Federated Learning (https://arxiv.org/abs/2501.19279)
Comments:
          Submitted to IJCNN

- **What's New**: 이번 논문은 분산형 연합 학습(Decentralized Federated Learning, DFL)의 성능을 향상시키기 위한 S-VOTE라는 투표 기반의 클라이언트 선택 메커니즘을 제안합니다. S-VOTE는 비독립 및 동일 분포(non-IID) 데이터 환경에서 자원 사용을 최적화하고 모델 성능을 개선하는 것을 목표로 합니다. 이 메커니즘은 자원 비용을 크게 증가시키지 않으면서도 참여가 저조한 클라이언트가 기여할 수 있도록 하는 적응형 전략을 포함하고 있습니다.

- **Technical Details**: 제안된 S-VOTE 메커니즘은 모델 유사성을 활용하여 유의미한 클라이언트 간의 집합적 협업을 촉진합니다. 이 과정에서 불필요한 통신 및 에너지 소비를 줄이는 동시에 스폰테니어스(local) 훈련을 지원하여 리소스 효율성을 높입니다. 실험을 통해 S-VOTE는 통신 효율성을 21%까지 개선하고, 수렴 속도를 4-6% 증가시키며, 특정 구성에서 9-17%의 더 높은 정확도를 달성하는 것으로 나타났습니다.

- **Performance Highlights**: S-VOTE는 실험을 통해 기존 방법보다 향상된 성능을 보여주었습니다. 특히, 에너지 소비를 14-24%까지 줄였으며, 이는 DFL의 다양한 환경에서의 적용 가능성을 시사합니다. 이 결과들은 S-VOTE가 비동질적인 환경에서 DFL의 문제를 해결하는데 큰 잠재력을 가지고 있음을 강조합니다.
...
### Clustering in hyperbolic balls (https://arxiv.org/abs/2501.19247)
- **What's New**: 최근에 음수 곡률을 가진 다양체에서의 데이터 표현에 대한 연구가 활성화되었으며, 이를 통해 하이퍼볼릭 머신러닝(hyperbolic machine learning)이라는 새로운 연구 방향이 나타났습니다. 이 논문에서는 하이퍼볼릭 공간에서의 클러스터링을 위한 엄격한 수학적 프레임워크를 제공하며, 새로운 바리센터(barycenter) 정의를 기반으로 한 k-평균 클러스터링(k-means clustering)과 하이퍼볼릭 구역에서의 새로운 확률 분포 혼합 학습을 위한 기대 최적화 알고리즘(expectation-maximization algorithm)을 소개합니다.

- **Technical Details**: 하이퍼볼릭 공간에서의 데이터 분석 및 통계 모델링을 위해 필요한 수학적 도구들을 체계화합니다. 특히, Poincaré 볼(Poincaré ball)과 그 대칭 그룹(isometry groups) 등을 다루며, 이를 통해 비유클리드 데이터에 대한 클러스터링 기법을 확립합니다. 각 분포 샘플링 기법과 최대우도(maximum likelihood) 알고리즘은 하이퍼볼릭 공간에서의 매개변수 추정에 활용됩니다.

- **Performance Highlights**: 이 논문에서는 하이퍼볼릭 볼에서의 클러스터링 알고리즘을 제안하고, Poincaré 원판(Poincaré disc)에서의 세 가지 실험을 통해 이 방법론의 효과를 입증합니다. 이어서, 다양한 차원에서의 Poincaré 볼에 대한 알고리즘 확장을 제안하며, 3차원 하이퍼볼릭 볼에 대한 실험 결과를 보고합니다. 이 연구는 하이퍼볼릭 머신러닝의 잠재력과 중요성을 강조하며, 미래 연구를 위한 기초를 다지는 데 기여합니다.



### Multi-agent Multi-armed Bandit with Fully Heavy-tailed Dynamics (https://arxiv.org/abs/2501.19239)
Comments:
          40 pages

- **What's New**: 이 논문에서는 완전한 heavy-tailed 환경에서의 분산 다중 에이전트 다중 무장 강도(Multi-agent Multi-armed Bandit) 문제를 다룹니다. 클라이언트들이 sparse random graphs에서 통신하며, 이들 그래프는 heavy-tailed degree distributions와 reward distributions를 가지고 있습니다. 기존 연구와의 차별점은 시간 불변 연결 그래프 또는 조밀한 그래프에서의 light-tailed dynamics를 다루던 접근 방식에서 벗어나, 불균형하고 sparse한 그래프에서의 새로운 문제를 탐구하는 것입니다.

- **Technical Details**: 이 연구는 클라이언트들이 hub-like 구조를 활용하여 보상을 집계하고 노이즈를 줄이는 알고리즘 프레임워크를 제시합니다. 특히, M개의 클라이언트와 power-law 지수를 가진 degree distribution 하에서, 새로운 접근법으로 O(M^{1 - \frac{1}{\alpha}} \log{T})의 regret bound를 얻습니다. 또한 heterogeneous reward 설정에서는 클라이언트들이 이웃과 소통하여 UCB 지수에서 교환된 추정치를 집계함으로써 O(M \log{T})의 regret bound를 증명했습니다.

- **Performance Highlights**: 제안된 알고리즘은 기존 연구보다 더 나은 성능을 보이며, sparse random graphs와 heavy-tailed rewards 하에서도 잘 작동합니다. homogeneous settings에서는 O(M^{1-\frac{1}{\alpha}} \log{T})의 regret 상한을 제공하며, 이를 통해 sample complexity가 감소했음을 보였습니다. heterogeneous 보상 하에서도 O(M \log{T})의 upper bound를 얻어, sparse 및 비대칭 그래프에서의 효과성을 입증하였습니다.



### Hourly Short Term Load Forecasting for Residential Buildings and Energy Communities (https://arxiv.org/abs/2501.19234)
- **What's New**: 본 논문은 단기 전력 부하 예측(short-term load forecasting, STLF) 모델의 정확도를 높이기 위해 다양한 예측 모델을 비교 및 분석하는 연구를 진행했습니다. 특히 기존의 블랙박스 모델과는 달리, 간단한 구조의 대안 모델인 지속성 기반 자회귀 모델(persistence-based auto-regressive model, PAR)과 계절적 지속성 기반 회귀 모델(seasonal persistence-based regressive model, SPR)을 도입했습니다. 새로운 모델은 시간별 예측을 위한 맞춤형 설계를 기반으로 하여 15-30%의 예측 정확도를 증가시켰습니다.

- **Technical Details**: 이 연구에서는 주거 건물 및 소규모 에너지 커뮤니티를 위한 단기 전력 부하 예측 모델의 계산 효율성을 개선하는 것을 목표로 하였습니다. 논문은 PAR 및 SPR 모델에 대한 적응을 통해 사용자의 행동에 일반적으로 존재하는 인과적 의존성을 포착하였습니다. 다양한 블랙박스 모델링 기법과 비교 분석하여 전력 소비에서 인과적 의존성을 활용하여 정확한 예측을 생성할 수 있는 가능성을 제시하였습니다.

- **Performance Highlights**: 결과적으로 새로운 시간별 모델은 기존의 기술들에 비해 예측 정확도가 15-30% 향상되었습니다. 협력적으로 작업되는 주거 건물들은 에너지 재난 대응(Demand Response)에 효과적으로 참여할 수 있으며, 에너지 관리에서 높은 자율성을 확보할 수 있습니다. 연구 결과는 에너지 커뮤니티의 에너지 유연성을 극대화하는 박차를 가할 것입니다.



### \underline{E2}Former: A Linear-time \underline{E}fficient and \underline{E}quivariant Trans\underline{former} for Scalable Molecular Modeling (https://arxiv.org/abs/2501.19216)
- **What's New**: 이번 연구에서는 E2Former라는 새로운 equivariant (에퀴바리언트) 및 효율적인 transformer 아키텍처를 도입하여 Wigner 6j convolution (Wigner 6j 컨볼루션)을 통합하였습니다. 기존의 Equivariant Graph Neural Networks (EGNNs)의 한계를 극복하기 위해 edge에서 node로 계산 부담을 이동하여 복잡도를 O(|ℱ|)에서 O(|𝒱|)로 줄이는 혁신적인 방법입니다. 이로 인해 분자 모델링에서의 효율성과 확장성을 향상시키면서도 모델의 표현력과 회전 대칭성을 유지합니다.

- **Technical Details**: Wigner 6j convolution을 사용하여 spherical-equivariant (구형 에퀴바리언트) 방법론을 도입합니다. 이는 복잡도를 감소시킴으로써 많은 분자 시스템을 효율적으로 모델링할 수 있습니다. E2Former는 이러한 Wigner 6j convolution을 활용하여 회전 대칭성을 유지하면서도 계산 효율을 유의미하게 강화합니다. 실험 결과, E2Former는 전통적인 SO(3) 컨볼루션에 비해 7배에서 30배의 속도 향상을 보여주었습니다.

- **Performance Highlights**: E2Former는 OC20, OC22, SPICE와 같은 벤치마크 데이터셋에서 분자 에너지 및 힘 예측에서 최첨단의 정확도를 달성했습니다. 또한 기존의 spherical-equivariant 방법에 비해 더 나은 효율성과 확장성을 입증했습니다. 분자 동역학 평가에서도 DFT 수준의 정확성을 기록하며 최신 경험적 잠재적 방법을 능가하는 성능을 보였습니다. 이는 대규모 분자 시뮬레이션 및 관련 응용 프로그램에서의 미래 가능성을 시사합니다.
...
- **What's New**: 이번 연구에서는 RIGNO(Region Interaction Graph Neural Operator)라는 새로운 GNN(그래프 신경망) 기반의 신경 연산자를 제안합니다. 이 모델은 복잡한 도메인에서의 PDE(부분 미분 방정식) 해법을 학습하기 위해 설계되었습니다. RIGNO는 Cartesian 도메인으로의 변환 없이도 작동하며, 스케일 비불변성과 시간 연속성을 보장하는 여러 혁신적인 요소를 포함하고 있습니다. 이를 통해 RIGNO는 다양한 도메인 형태에서 고차원의 문제를 효율적으로 지원할 수 있습니다.

- **Technical Details**: RIGNO는 GNN을 활용하여 다중 임계치를 처리하며, 입력과 출력 포인트 클라우드 간의 매핑을 통해 PDE의 해법 연산자를 학습하는 다중 스케일 모델입니다. 이 모델은 상대적으로 불규칙한 데이터 구조를 다룰 수 있으며, 템포럴 연속성을 유지하면서도 다양한 공간 해상도의 PDE를 처리할 수 있는 능력을 가지고 있습니다. RIGNO는 기존의 여러 신경 연산자들이 갖고 있는 기하학적 제약을 극복하고, 더욱 복잡한 비정형 도메인에서도 정확한 근사를 제공합니다. 이러한 구조적 특징은 RIGNO가 다양한 실제 문제를 해결하는 데 기여할 수 있음을 시사합니다.

- **Performance Highlights**: 실험 결과, RIGNO는 기존의 신경 연산자 기초 모델들에 비해 높은 정확도를 자랑하며, 보지 못한 공간 해상도와 시간 인스턴스에 대해서도 잘 일반화되는 능력을 나타내었습니다. 다양한 시간 의존적 및 정적 PDE benchmarks에 대해 테스트하여 그 성능을 입증하였습니다. RIGNO는 각기 다른 도메인 기하학을 가진 문제들에 대해 일관된 성능과 안정성을 유지하며, 이는 복잡한 엔지니어링 문제 해결에 중요한 역할을 할 수 있음을 보여줍니다.



### A Variational Perspective on Generative Protein Fitness Optimization (https://arxiv.org/abs/2501.19200)
- **What's New**: 이 논문에서는 Variational Latent Generative Protein Optimization (VLGPO)라는 새로운 단백질 최적화 방법을 제안합니다. VLGPO는 단백질 서열을 연속적인 잠재 공간에 임베딩하여, 이 공간에서 더 나은 피트니스(fitness)를 가진 서열을 효율적으로 샘플링할 수 있도록 합니다. 기존의 방법들과는 달리, 단백질 서열의 변화가 많은 그라디언트 계산이 가능한 특정 구조를 활용하여 최적화를 수행합니다.

- **Technical Details**: VLGPO는 먼저 반복적으로 학습한 비선형 전이 모델을 통해 단백질 서열의 잠재 분포를 구축한 후, 피트니스 예측기와 결합하여 높은 피트니스 값을 가진 서열을 탐색합니다. 이 방법은 제한된 데이터 환경에서도 두 가지 단백질 벤치마크에 대해 효과적으로 성능을 발휘함을 보여주었습니다. 또한, VLGPO는 단백질 최적화를 위한 유연한 프레임워크를 제공, 다양한 디자인 작업에 맞춰 쉽게 조정할 수 있습니다.

- **Performance Highlights**: VLGPO는 Adeno-Associated Virus (AAV)와 Green Fluorescent Protein (GFP)라는 두 가지 공개 벤치마크에서 최첨단 성능을 달성했습니다. 특히, 해당 방법은 중간 난이도와 높은 난이도의 문제를 다루며, 제한된 데이터 환경에서도 효과적으로 작동함을 입증하였습니다. 이러한 결과는 VLGPO가 단백질 최적화 분야에서 향후 연구와 개발에 큰 기여를 할 수 있음을 시사합니다.
...
### Position: Curvature Matrices Should Be Democratized via Linear Operators (https://arxiv.org/abs/2501.19183)
Comments:
          8 pages, 2 figures

- **What's New**: 본 논문에서는 머신 러닝에서 중요한 곡률 행렬(curtvature matrices) 처리를 위한 선형 연산자(linear operators)의 사용 가능성을 주장하고 있습니다. 기존의 자동 미분(automatic differentiation)과 복잡한 곡률 프록시(curvature proxies)의 문제점을 피하면서, 사용자 친화적이고 확장 가능한 접근 방식을 제안합니다. 이러한 주장을 지원하기 위해, 연구자들은 $	extit{curvlinops}$라는 라이브러리를 개발하였습니다.

- **Technical Details**: $	extit{curvlinops}$ 라이브러리는 통합된 선형 연산자 인터페이스를 통해 곡률 행렬을 제공합니다. 이 인터페이스는 곡률 행렬의 복잡성을 숨기고, 애플리케이션을 단순화하며, 여러 라이브러리와 호환성(interoperability) 및 확장성을 제공합니다. 논문에서 설명하는 선형 연산자는 행렬-벡터 곱(matrix-vector products)을 수행할 수 있도록 하여, 다양한 머신 러닝 작업에서 메모리 요구 사항을 줄이고 효율성을 높입니다.

- **Performance Highlights**: 저자들은 $	extit{curvlinops}$의 유용성을 다양한 머신 러닝(ML) 응용 프로그램에서 보여줍니다. 이 라이브러리는 2차 최적화(second-order optimization), 데이터 기여도(model attribution), 모델 프루닝(model pruning), 손실 경관 분석(loss landscape analysis), 불확실성 정량화(uncertainty quantification)와 같은 여러 작업에서 실험적으로 성능이 향상되었음을 입증합니다. 특히, 대규모 신경망(neural networks)에서도 효율적으로 확장 가능하다는 점이 강조됩니다.



### A Comunication Framework for Compositional Generation (https://arxiv.org/abs/2501.19182)
- **What's New**: 이 논문은 self-supervised generative communication game 기반의 프레임워크를 제시하여 사전 훈련된 encoder-decoder 모델에서 학습된 표현으로부터 compositional encodings를 생성하는 첫 번째 단계로 나아갑니다. 특히, Iterated Learning (IL) 프로토콜을 적용하여, 압축(compression)과 다양성(diversity)을 위한 교대 압박을 통해 효율적이면서도 모호하지 않은 인코딩으로 수렴하게 합니다. 이러한 접근법은 인코딩의 효율성, 명확성 및 비전체성(non-holisticity)을 정의하고 균형을 잡는 엄격한 정당화와 증명을 바탕으로 합니다.

- **Technical Details**: 제안된 프레임워크는 압축 및 다양성을 위한 교대 압박을 적용하여, sender와 receiver가 조화롭게 상호작용하는 통신 게임을 통해 compositional encodings를 생성하는 방식입니다. 이 과정에서 Approximate message entropy regularization을 이용해 compositional encodings를 선호하도록 하고, 이론적 근거를 가지고 인코딩의 효율, 명료성, 비전체성의 개념을 균형 있게 정의합니다. 이를 통해 Shapes3D 데이터셋에서 강력한 성능을 보여주며, 기존의 다른 일명 discrete message 프레임워크보다 우수한 결과를 도출합니다.

- **Performance Highlights**: 논문에서는 compositional image dataset인 Shapes3D를 사용하여 제안한 방법론이 재구성(reconstruction) 및 조합 성능(compositionality metrics)에서 견고한 성능을 보여주었다고 보고합니다. 또한, 이 방법론은 기존의 방법들에 비해 월등히 높은 성능을 자랑하며, 유의미한 개선을 보여 객체의 조합 가능성을 효과적으로 설명할 수 있음을 입증했습니다. 향후 연구는 더 복잡한 목표를 위한 확장으로 이어질 것입니다.



### No Foundations without Foundations -- Why semi-mechanistic models are essential for regulatory biology (https://arxiv.org/abs/2501.19178)
Comments:
          19 pages, 8 figures

- **What's New**: 이 논문은 깊이 있는 기계 학습(machine learning) 기술이 규제 생물학(regulatory biology)에서 유전자 발현 예측 모델링을 혁신하지 못하는 이유를 탐구합니다. 저자들은 반기계적(semi-mechanistic) 프레임워크를 제안하여 CRISPR 스크린의 실험 설계를 통합하고 새로운 데이터 세트 생성을 통한 엄밀한 테스트를 촉진합니다. 이 접근 방식은 변형된 손실 함수(modified loss function)를 제시하여 예측 성능을 향상할 수 있음을 증명합니다.

- **Technical Details**: 제안된 수학적 모델은 CRISPR 스크린과 단일 세포(single-cell) 판독 결과에 대한 개입을 캡처하는데 중점을 둡니다. 저자들은 다양한 유전자 조작 및 실험 설계를 아우르는 통합 모델을 구축하며, 이는 우리가 기존의 기계 학습 모델에서 간과한 기본적인 가정들을 드러냅니다. 특히, 저자들이 제안한 손실 함수는 생물학적 직관을 통합하여 예측 성능을 더욱 향상시킬 수 있도록 설계되었습니다.

- **Performance Highlights**: 저자들은 기존의 통계적 방법보다 개선된 성능을 나타내는 새로운 모델을 제시하며, 이를 통해 데이터 기반 아키텍처의 한계를 극복할 수 있음을 보여줍니다. 이러한 방식은 궁극적으로 예측 가능하고 해석 가능한 모델을 통해 규제 생물학 분야에 임상적으로 유용한 가치 있는 기여를 할 수 있는 가능성을 제시합니다. 모델의 검증 및 적용 시, 크고 다양한 샘플의 해석력 향상이라는 손쉬운 실험적 이점을 제공합니다.



### PSyDUCK: Training-Free Steganography for Latent Diffusion (https://arxiv.org/abs/2501.19172)
- **What's New**: 이번 논문에서는 잠재적 확산 모델(latent diffusion models)을 활용한 새로운 스테가노그래피 방법인 PSyDUCK을 소개합니다. PSyDUCK은 비훈련(training-free) 모델 무관 스테가노그래피 방법으로, 이미지와 비디오 데이터에 안전하게 메시지를 인코딩할 수 있게 해줍니다. 이 연구는 스테가노그래피의 기존 한계를 극복하는 동시에 최신 생성 모델의 강점을 반영합니다.

- **Technical Details**: PSyDUCK은 사용자가 비밀 메시지를 교환하기 위해 공유된 키를 이용하여, 모델의 디노이징 경로에서 제어된 변화를 도입하도록 설계되었습니다. 이 방법은 기존의 픽셀 공간에서 동작하는 모델들에 비해 보다 효율적인 잠재적 표현을 사용할 수 있으며, 메시지를 인코딩할 때 정확성과 보안성을 높이는 유연성을 제공합니다. 스테가노그래피의 스케일과 보안 측면에서 중요한 발전을 이루었습니다.

- **Performance Highlights**: 실험 결과, PSyDUCK은 기존의 방법들과 비교해 우수한 확장성과 보안성을 보여주었습니다. 스테가노그래피 능력을 평가하며 흔히 취약점으로 여겨지는 왜곡 및 디코딩 오류에 대한 내성을 입증했습니다. 이 연구는 생성적 확산 모델을 활용한 일반적인 목적의 스테가노그래피 시스템 연구의 기초를 제공합니다.



### Locality-aware Surrogates for Gradient-based Black-box Optimization (https://arxiv.org/abs/2501.19161)
- **What's New**: 본 논문에서는 비가역적인 블랙박스 최적화(Black-box optimization) 문제를 해결하기 위해 지역 인식 서라겟 모델(locality-aware surrogate models)을 제안하고 있습니다. 이는 주어진 데이터에 대한 그래디언트(gradient) 일치를 보장함으로써 최적화 효율성을 높이기 위한 새로운 접근법입니다. 특히, GradPIE 손실(GradPIE loss)을 최소화하는 방법이 제안되어 이론적으로 검증되었습니다.

- **Technical Details**: 작성된 알고리즘은 블랙박스 함수가 존재하는 다양한 입력 공간에서 최소 질의 예산(query budget)으로 최적의 입력을 찾기 위한 훈련 방안을 제공합니다. 이 방법은 오프라인 학습과 온라인 학습 모두 지원하며, 고속 데이터 처리 속도를 유지하면서 대규모 디자인 공간에서도 잘 작동합니다. 특히, Coupled Nonlinear Oscillator Network와 같은 예시 실험에서 이론 결과가 유효함을 입증했습니다.

- **Performance Highlights**: 세 가지 실제 작업에 대한 평가 결과, 제안한 방법이 전통적인 방법들에 비해 최적화 성능이 일관되게 향상되었음을 보여줍니다. 스마트 센서, 로보틱스, 물리적 시스템과 같은 다양한 분야에서 자동 질의가 가능한 상황에서 이 기술이 적용 가능함을 입증했습니다. 이러한 결과는 보다 신뢰할 수 있는 그래디언트 추정을 필요로 하는 오프라인 및 온라인 최적화 작업에 유용한 솔루션을 제공할 수 있음을 시사합니다.



### A theoretical framework for overfitting in energy-based modeling (https://arxiv.org/abs/2501.19158)
Comments:
          23 pages, 13 figures (including appendix)

- **What's New**: 이 연구는 한정된 데이터가 쌍 관 에너지를 기반으로 한 모델의 학습에 미치는 영향을 조사합니다. 가우시안 모델을 테스트베드로 사용하여, 결합 행렬의 고유 기저에 걸친 학습 궤적을 분석하고, 경험적 공분산 행렬의 스펙트럼 분해와 연관된 학습 시간 척도를 밝혀냅니다. 또한, 유한 데이터 보정이 비대칭 난수 행렬 이론을 통해 정확하게 모델링할 수 있음을 보여줍니다.

- **Technical Details**: 에너지 기반 모델(EBM)은 볼츠만의 법칙에 기반하여 다양한 데이터의 경험적 분포를 확률적 틀로 인코딩합니다. 이 연구에서는 단순 가우시안 모델을 사용하여 오버피팅(overfitting) 문제를 정량적으로 분석하며, 인위적 공분산 행렬을 통해 고유값 동역학을 조사합니다. 연구의 주요 기법은 난수 행렬 이론을 기반으로 하여 학습된 고유값의 정확성을 교정하며, 수축 보정(shrinkage correction) 기술을 사용하여 모델의 신뢰성과 예측 정확성을 향상하는 방법론을 제시합니다.

- **Performance Highlights**: 연구 결과는 학습 초기 조건과 시간 척도 간의 상호작용을 통해 조기 중단의 최적 지점이 발생함을 보여줍니다. 또한 분석 프레임워크는 이산 변수 모델에서의 오버피팅 조절 전략을 제공하여 에너지 기반 생성 모델의 오버피팅 관리 개선에 기여합니다. 전반적으로 이 연구는 에너지 기반 모델의 신뢰도를 높이고 예측 정확성을 향상시키는 데 중요한 기여를 하고 있습니다.



### Test-Time Training Scaling for Chemical Exploration in Drug Design (https://arxiv.org/abs/2501.19153)
- **What's New**: 이 논문에서는 생물학적 활성이 유사한 비슷하지 않은 분자를 발견하기 위한 새로운 벤치마크인 Molecule Exploration (MolExp)을 제안합니다. 이는 약물 발견에서 흔한 상황이지만 최적화하기 어려운 문제입니다. 연구결과, 여러 독립적인 RL 에이전트가 단일 에이전트보다 화학 공간 탐색을 더 잘 수행하며, 에이전트 수에 따라 성능이 로그-선형적으로 증가하는 경향이 있음을 보여주었습니다.

- **Technical Details**: 재강화 학습(RL)은 에이전트가 보상을 극대화하기 위해 환경을 탐색하는 최적화 알고리즘입니다. 이 연구에서는 RL을 이용하여 분자 생성을 최적화하는 화학 언어 모델(Chemical Language Models, CLMs)의 성능 향상을 위해 여러 독립적 에이전트를 활용합니다. 또한, 에이전트 간의 협력 전략은 성능 향상에 큰 영향을 미치지 않는 것으로 나타났습니다.

- **Performance Highlights**: Molecule Exploration 벤치마크에서 여러 에이전트를 활용했을 경우 성능이 향상되며, 에이전트 수가 많아질수록 성능 증가가 뚜렷하게 나타났습니다. 또한, 협동 전략 적용 시 탐색 효율성이 전반적으로 크게 개선되지 않는다는 점도 발견되었습니다. 이 연구는 약물 설계 최적화에서 화학 공간의 다양성을 고려하는 새로운 관점을 제공합니다.
...
Comments:
          7 pages, 29 pages total

- **What's New**: 이번 연구에서는 부분적으로 관측 가능한 환경에서 비대칭 학습(paradigm) 하의 강화 학습(reinforcement learning) 알고리즘의 이론적 정당성을 제시합니다. 특히, 비대칭 actor-critic 알고리즘에 대한 새로운 학습 목표를 통해 더 빠른 학습 속도를 얻을 수 있음을 보입니다.

- **Technical Details**: 연구자들은 선형 함수 근사기(linear function approximators) 및 유한 시간 수렴 분석(finite-time convergence analysis)을 활용하여 비대칭 critic의 이점을 설명합니다. 이 분석을 통해 제안된 방법의 성능은 에이전트 상태에서 발생할 수 있는 aliasing 에러 항을 없애는 것으로 나타났습니다.

- **Performance Highlights**: 이 연구의 결과는 비대칭 critic이 에이전트 상태에서의 aliasing 문제를 해결함으로써 학습 성능을 향상시킬 수 있음을 보여줍니다. 이러한 결과는 실질적인 학습 환경에서의 에이전트 성능 향상에 기여할 것으로 기대됩니다.
...
### Brain-inspired sparse training enables Transformers and LLMs to perform as fully connected (https://arxiv.org/abs/2501.19107)
- **What's New**: 이 연구는 희소 연결성을 가진 인공 신경망(ANNs) 훈련을 위한 뇌 영감을 받은 네트워크 과학 원리의 적용에 대한 현재의 지식을 확장하려고 합니다. 본 연구는 동적 희소 훈련(Dynamic Sparse Training, DST)을 사용하여 계산 요구 사항을 줄일 수 있지만, 높은 희소성 수준에서 최고 성능을 유지하는 데 어려움을 겪는다는 문제를 다룹니다. 이를 해결하기 위해 Cannistraci-Hebb Training (CHT) 방법을 제안하며, 이는 연결 복구에서 경량화된 접근법으로 GPU 친화적인 방법으로 계산 복잡성을 O(N^3)로 줄입니다.

- **Technical Details**: CHTs는 링크 제거와 재성장에서의 연결 샘플링 전략을 도입하여 네트워크 토폴로지의 탐색과 활용 균형을 이루어냅니다. 새로운 노드 기반 링크 예측 메커니즘을 통해 CHTs는 시간 복잡성을 O(N^3)로 줄이며 대규모 모델의 확장을 가능하게 하였습니다. 또한, CHTs는 작은 세계 속성을 갖는 이분 네트워크를 위한 초기 희소 토폴로지를 설정하는 데 있어 네트워크 과학 원리를 활용합니다.

- **Performance Highlights**: CHTs는 MLP에서 1%의 연결을 사용하였을 때 완전 연결 네트워크보다 뛰어난 성능을 보여주고, 5%의 연결을 사용할 때 Transformer 기반 기계 번역 작업에서도 우수한 결과를 달성했습니다. 특히, 30%의 연결을 사용하여 언어 모델링 작업에서 다른 동적 희소 훈련 방법보다 우수한 성능을 기록했으며, zero-shot 평가에서도 완전 연결 모델을 능가하는 결과를 보였습니다.



### Relating Misfit to Gain in Weak-to-Strong Generalization Beyond the Squared Loss (https://arxiv.org/abs/2501.19105)
Comments:
          22 pages, 4 figures

- **What's New**: 본 논문은 약한 인공지능(weak AI) 모델이 레이블링한 데이터를 기반으로 강한 인공지능(strong AI) 모델을 훈련시키는 약-강 일반화(weak-to-strong generalization) 패러다임을 다루고 있습니다. 기존 연구에서는 주로 실수 회귀(regression) 설정에서 성과 증가를 정량적으로 특성화했으나, 이번 연구는 일반적인 Bregman divergence를 사용하는 손실 함수에 적용할 수 있는 특성을 확장하였습니다. 또한, 강한 모델의 클래스가 볼록(convex)하지 않을 경우를 고려하여 여러 강한 모델의 볼록 조합을 통해 분류(classification) 작업에 대한 새로운 통찰력을 제공합니다.

- **Technical Details**: 약-강 일반화의 이론적 기초는 정보 기하학(information geometry)에서 영감을 받아 발전하였습니다. 특히, Pythagorean inequality는 Bregman divergence로 여겨지는 손실 함수에 대해 더 일반적으로 성립함을 보여줍니다. 본 연구에서는 강한 모델의 출력과 약한 레이블 사이의 Kullback-Leibler (KL) divergence를 최소화하기 위해, k개의 로지스틱 회귀(logistic regression) 층의 볼록 조합을 최적화하는 방식을 제안합니다. 이 접근 방식은 강한 모델의 성능 향상을 효과적으로 측정할 수 있는 새로운 방법론을 제공합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법론에 따라 훈련된 강한 모델은 약한 모델과의 KL misfit에 의해 성능 향상이 신뢰성 있게 정의됨을 확인했습니다. 특히, k의 값이 증가할수록 이 성능 향상은 더욱 뚜렷하게 나타났습니다. 이러한 결과는 약-강 일반화의 이론적 기반을 실제 데이터에 성공적으로 적용할 수 있음을 나타냅니다.



### Neural Collapse Beyond the Unconstrainted Features Model: Landscape, Dynamics, and Generalization in the Mean-Field Regim (https://arxiv.org/abs/2501.19104)
- **What's New**: 이번 논문은 Neural Collapse(NC) 현상에 대한 연구로, 마지막 층의 표현이 고도로 구조화된 기하학으로 수렴하는 것을 다룬다. 특히 NC의 첫 번째 속성인 NC1에 집중하여, 클래스 내 변동성이 사라지는 것을 주요하게 설명한다. 데이터에 대한 특정 관점을 채택하여, 세 개의 층으로 구성된 신경망에서 NC1의 발생을 분석하고, 손실 경관(landscape)과의 연결을 입증한다.

- **Technical Details**: NC1의 발생은 손실 경관의 특성과 관련이 있으며, 우리는 경험적 손실과 기울기 노름(gradient norm)이 작은 모든 근접 정지점들이 대략 NC1 솔루션을 만족한다고 보인다. 또한, 평균 제곱 오차(mean squared error, MSE)에 대한 기울기 흐름이 경험적 손실이 작은 NC1 솔루션으로 수렴함을 보여준다. 이 연구는 NC 현상을 보다 명확히 이해하기 위해 데이터 분포와의 관계도 설명한다.

- **Performance Highlights**: 특히, 잘 분리된 데이터 분포의 경우 NC1과 시험 오차(test error)가 동시에 사라짐을 증명하여, NC1이 학습 과정에서 발생하고 있는 경향을 나타낸다. 모델이 최적화된 경량화된 특징을 학습하는 과정에서 NC1이 발생하는 것으로 확인되며, 이는 실제 훈련 절차가 소실된 기울기와 손실에 수렴하기 때문임을 강조한다.



### Reinforcement Learning on Reconfigurable Hardware: Overcoming Material Variability in Laser Material Processing (https://arxiv.org/abs/2501.19102)
Comments:
          Accepted for the 2025 IEEE International Conference on Robotics and Automation (ICRA), May 19-23, 2025, Atlanta, USA

- **What's New**: 이번 연구는 레이저 프로세스 제어를 위한 새로운 실시간 강화 학습 접근 방식을 제안합니다. 이는 Field Programmable Gate Array (FPGA)에서 구현되어 실시간 실행을 가능하게 하며, 기존의 방법들이 가지는 여러 한계를 극복하고자 합니다. 실험 결과는 이 방법이 표면 거칠기가 다양한 스테인리스강 샘플을 대상으로 자율적으로 적응할 수 있음을 보여주며, 보상 엔지니어링이나 사전 설정 정보에 의존하지 않고 진행됩니다.

- **Technical Details**: 제안된 시스템은 레이저 매개변수를 실제 환경에서 광학 신호에 기반하여 조정할 수 있는 강화 학습 알고리즘을 포함합니다. 이를 통해 시스템은 다양한 표면 조건과 물질 특성에 자율적으로 적응할 수 있으며, 제어는 마이크로초 단위로 이루어집니다. 실험은 316L 스테인리스강 샘플을 사용하여 진행되었으며, 시스템의 실시간 데이터 처리 및 제어 능력이 강조되었습니다.

- **Performance Highlights**: 실제 레이저 용접 실험을 통해 이 방법은 손으로 공학적으로 최적화된 일정한 출력 전략보다 최대 23% 더 나은 성능을 달성했습니다. 또한, 복합적인 표면에서의 성능 개선률도 7%를 기록하여 강화 학습 알고리즘이 레이저 프로세스 자동화와 최적화에 기여할 수 있는 가능성을 보여줍니다. 이 연구는 다양한 산업 분야에 걸쳐 레이저 프로세스의 전자동화와 최적화를 위한 중요한 진전을 나타냅니다.



### Unraveling Zeroth-Order Optimization through the Lens of Low-Dimensional Structured Perturbations (https://arxiv.org/abs/2501.19099)
Comments:
          35 pages, 5 figures

- **What's New**: 이번 연구에서는 zeroth-order (ZO) 최적화의 수렴(convergence)과 일반화(generalization) 속성을 구조적 변동(structured perturbations) 아래에서 분석하는 통합 이론적 프레임워크를 개발하였습니다. 특히, 높은 차원에서의 차원성(dimensionality)이 주요한 병목 현상으로 작용함을 보였으며, 
‘stable rank’와 ‘effective overlap’ 개념을 도입하여 이러한 구조적 변동이 경량 경량화(great convergence)의 시작점임을 설명하였습니다. 

또한, 저차원 제약이 gradient noise를 줄이고 수렴을 가속화하는 데 효과적이라는 점을 보여주었습니다.

- **Technical Details**: 이 연구는 ZO 최적화의 수렴 및 일반화 속성을 이해하기 위해, gradient 추정의 노이즈를 세 가지 기본 요인으로 분해하는 통합 이론적 프레임워크를 제시합니다. 
연구자는 높은 차원에서의 차원성이 변동성을 증대시키고, 이로 인해 수렴 속도가 늦춰진다는 사실을 강조했습니다.
또한, 구조적 변동으로 제안된 희소(sparse) 및 저랭크(low-rank) 구조가 ZO 최적화의 효율성을 높이는 방식을 설명하는 이론적 기초를 제공하였습니다.

- **Performance Highlights**: MeZO-BCD는 다른 기존 방법들과 비교하여 메모리 효율을 극대화하며 최대 2.09배 빠른 수렴 속도를 보여주는 실험 결과를 도출했습니다. 
특히 ZO 최적화의 대규모 적용을 위한 유망한 방법으로 자리잡게 되었습니다. 
MeZO-BCD 방식의 성공적인 수행은 차세대 대형 언어 모델(LLMs) 조정에서도 그 효능을 입증하였습니다.
...
- **What's New**: 본 논문에서는 Pivoting Factorization (PIFA)라는 새로운 손실 없는 메타 저차원 표현을 제안하여, 불필요한 정보를 제거하고 낮은 랭크 표현을 효율적으로 압축합니다. PIFA는 선형 독립적인 피벗 행(pivot rows)을 파악하여 비피벗 행(non-pivot rows)을 선형 조합으로 표현함으로써 시간 성능과 메모리 절약을 동시에 달성합니다. 이를 통해 기존의 저차원 프루닝 방법보다 뛰어난 성능을 보여주며, 준구조적 프루닝(semi-structured pruning)과 대등한 성능을 제공합니다.

- **Technical Details**: 이 연구는 저차원 프루닝 방법에서 발생하는 정보 중복 문제를 해결하기 위해 PIFA를 도입하고, 온라인 오류 누적 최소화 재구성(Online Error-Accumulation-Minimization Reconstruction, M)을 제안합니다. M은 여러 데이터 흐름을 활용하여 오류 누적 문제를 해결하고, 이를 통해 재구성된 데이터의 품질을 향상시킵니다. PIFA와 M을 결합한 MPIFA는 훈련 없이 최적의 저차원 프루닝 프레임워크를 구축하여, LLM에서의 처리 성능을 크게 개선합니다.

- **Performance Highlights**: MPIFA는 LLaMA2 모델(7B, 13B, 70B)에 대해 40%-70%까지 심리적 난이도(perplexity) 격차를 줄이며, 레이어별 및 종단 간(end-to-end)에서 준구조적 프루닝보다 우수한 속도와 메모리 절약을 일관되게 달성합니다. 특히, 55% 밀도에서 PIFA는 2.1배의 속도 향상을 보여주며, 다양한 준희소(semi-sparse) 방식은 밀집(linear) 레이어보다 느리거나 실행되지 않는 결과를 나타냅니다. 이 연구는 LLM의 실용적이고 효율적인 모델 압축을 위한 새로운 방향을 제시합니다.



### Understanding Oversmoothing in GNNs as Consensus in Opinion Dynamics (https://arxiv.org/abs/2501.19089)
Comments:
          23 pages, 3 figures

- **What's New**: 이 논문에서는 그래프 신경망(Graph Neural Networks, GNN)에서 나타나는 오버스무딩(oversmoothing) 현상과 의견 역학(opinion dynamics)에서의 합의(consensus) 현상 사이의 유사성을 제시합니다. 연구자들은 최근의 연속 깊이 GNN 모델이 선형 합의 모델에 해당함을 보여줍니다. 이를 바탕으로, 비선형 의견 동역학(nonlinear opinion dynamics)에 기반한 새로운 GNN 모델인 BIMP(Behavior-Inspired Message Passing Neural Network)를 설계하여 오버스무딩을 방지할 수 있음을 입증합니다.

- **Technical Details**: 오버스무딩은 신경망의 깊이가 증가함에 따라 학습된 표현이 점점 유사해져 결국 예측 성능이 저하되는 현상입니다. 본 연구에서는 옵서전스에 대한 비유를 통해 최근의 연속 깊이 GNN 구조의 메시지 전달 타입이 선형 의견 동역학과 동등하다는 것을 증명합니다. 비선형 의견 동역학을 활용한 BIMP 모델은 다양한 초기 조건에서 더 높은 표현력을 보여주며, 심도 있는 실험을 통해 이 모델이 강력한 성능을 발휘함을 입증하였습니다.

- **Performance Highlights**: 제안된 BIMP 모델은 오버스무딩과 적대적 공격(adversarial attack)에 대해 강력한 저항력을 보여줍니다. 여러 벤치마크에서 경쟁 모델과 비교하여 일관되게 우수한 성능을 발휘하며, 이는 그래프 신경망 분야에서의 중요한 기여로 평가됩니다. 연구 결과는 GNN의 한계 극복을 위한 새로운 접근 방식을 제시하며, 다양한 응용 분야에서의 가능성을 열어줍니다.



### A Bias-Correction Decentralized Stochastic Gradient Algorithm with Momentum Acceleration (https://arxiv.org/abs/2501.19082)
- **What's New**: 본 논문에서는 Exact-Diffusion with Momentum (EDM)이라는 새로운 모멘텀 가속 분산 확률적 경량 알고리즘을 제안합니다. 이 알고리즘은 데이터 이질성으로 인한 편향을 수정하고, 깊은 학습에서 일반적으로 사용되는 모멘텀 방법을 도입하여 수렴 속도를 향상시킵니다. EDM은 비볼록(Non-convex) 목적 함수에 적용될 때 최적값의 근처로 수렴함을 이론적으로 증명합니다.

- **Technical Details**: 이 알고리즘은 대규모 데이터 처리를 위한 분산 확률적 최적화(Distributed Stochastic Optimization) 문제를 다루며, 정보 교환을 통해 전반적인 글로벌 목표 함수를 최적화할 수 있습니다. EDM은 기존의 분산 최적화 알고리즘과 달리 데이터의 이질성을 제거하고 네트워크 희소성(hyper-sparsity) 문제를 해결하여, 더욱 견고하게 알고리즘의 수렴을 보장합니다. 또한, 논문에서는 Polyak-Łojasiewicz 조건 하에서의 수렴 특성도 다룹니다.

- **Performance Highlights**: 시뮬레이션을 통해 제안된 알고리즘의 성능을 검증하며, 기존의 다양한 탈중앙화 최적화 알고리즘과 비교하여 데이터 이질성과 네트워크 희소성을 해결하는 데에 효과적임을 보여줍니다. 이 실험 결과는 EDM 알고리즘이 비볼록 최적화 문제에서도 견고하게 수렴할 수 있는 가능성을 나타냅니다. 또한, 기존의 알고리즘과 비교하여 수렴 속도가 향상된다는 결과를 얻게 됩니다.



### Differentially Private Policy Gradien (https://arxiv.org/abs/2501.19080)
- **What's New**: 이 논문은 실제 세계에서의 강화 학습(Reinforcement Learning, RL) 보급 증가에 따라 개인 데이터 소비가 증가하고 있는 문제를 다루고 있습니다. 논문에서는 차별 개인 정보 보호(Differential Privacy, DP) 기반의 정책 경량화(policy gradient) 알고리즘을 제안하며, 이를 통해 DP가 적절한 신뢰 영역(trust region)의 계산으로 줄어들 수 있음을 보여주고 있습니다. 실험적으로 다양한 벤치마크에서 이 알고리즘의 성능을 검증한 결과, 기존의 DP 알고리즘보다 개선된 결과를 보였습니다.

- **Technical Details**: 이 연구는 정책 경량화 방법 중 신뢰 영역 방법(trust-region methods)에 중점을 두어, TRPO(Trust Region Policy Optimization) 및 PPO(Proximal Policy Optimization)와 같은 알고리즘을 분석했습니다. 이들 방법은 교육 중 성능 저하를 방지하는 데 초점을 맞추며, DP 잡음의 존재 하에서도 정책 업데이트를 신뢰 영역 내에 유지하는 이론적 통찰을 제시합니다. 또한, 제안된 방법은 실제적이고 형식적인 경로 수준의 DP 보장을 제공하며, 기존의 복잡한 환경에서 정책 경량화 성능에 제한적인 영향을 미치는 것을 입증합니다.

- **Performance Highlights**: 제안된 알고리즘은 과거 연구보다 성능이 개선되어, RLHF(인간의 피드백을 통한 강화 학습)에서의 LLM(대형 언어 모델) 정렬에도 매우 유용하게 적용될 수 있습니다. 연구 결과는 한정된 개인 정보 예산 내에서 DP 정책 경량화 알고리즘이 복잡한 환경에서 효과적으로 배포될 수 있음을 보여줍니다. 이 점에서, 실제 세계의 문제를 해결하기 위한 DP RL 연구에는 더 많은 실용적인 접근이 필요함을 강조하고 있습니다.



### Temperature-Annealed Boltzmann Generators (https://arxiv.org/abs/2501.19077)
- **What's New**: 이번 논문에서는 온도 소산 찰시계 수단(Temperature-Annealed Boltzmann Generators, TA-BG)을 제안하여 비정상 확률 밀도, 특히 분자 시스템의 Boltzmann 분포 샘플링 문제에 대한 새로운 접근법을 제공합니다. 역 Kullback-Leibler 발산을 활용하여 고온에서 정상화 흐름을 훈련시키면서 모드 붕괴 없이 이 문제를 해결할 수 있음을 보여줍니다. 또한, 배급 기준에 따라 초기 분포를 낮은 목표 온도로 소산하는 새로운 재가중 기반 훈련 목표를 소개합니다.

- **Technical Details**: TA-BG 방법론은 고온에서 훈련된 정상화 흐름(normalizing flow)을 활용하며, 이를 통해 다양한 분자 시스템에 대해 점진적으로 목표 온도로 소산시키는 방식입니다. 기존의 샘플링 방법들의 비효율성을 극복하기 위해 흥미로운 방식으로 구현된 이 접근법은 특히 복잡성이 증가하는 세 가지 분자 시스템에서 검증되었습니다. 이 방법론을 통해 더 적은 에너지 평가를 요구하면서도, 기준 성능 대비 거의 모든 지표에서 높은 결과를 달성했습니다.

- **Performance Highlights**: 연구 결과, 제공된 TA-BG 방법은 특히 가장 큰 시스템에 대해 메타스테이블 상태를 정확히 식별할 수 있는 유일한 방법임을 입증했습니다. 이 방법은 기존 기법에 비해 샘플링 효율성과 정확성을 크게 향상시킵니다. 결과적으로, 실험 데이터를 기반으로 하지 않고도 복합 분자 시스템의 샘플링을 효과적으로 수행할 수 있는 가능성을 제시합니다.



### Pareto-frontier Entropy Search with Variational Lower Bound Maximization (https://arxiv.org/abs/2501.19073)
- **What's New**: 이번 연구에서는 Pareto-frontier의 정보 이득(information gain)을 통해 다목표 베이지안 최적화(multi-objective Bayesian optimization, MOBO)를 다룹니다. 이 연구는 기존의 연구에서 접근할 수 없던 Pareto-frontier를 트렁케이트(truncated) 분포로 변환하여 정보 이득을 최소화하는 방법을 제안합니다. 이를 통해 과도한 트렁케이션(over-truncation)과 보수적인 트렁케이션(under-truncation)을 혼합하는 새로운 방식을 도입했습니다.

- **Technical Details**: 제안하는 접근방식은 변분 하한 최적화(variational lower bound optimization)를 활용하여, 다목표 MOBO 식에서 파레토 전선의 상호정보(mutual information) 추정 시 발생하는 오차를 최소화합니다. 연구는 두 가지 상반된 트렁케이션 방식인 과도 트렁케이션과 보수적 트렁케이션을 결합하여 혼합 분포를 정의합니다. 이를 통해 최적의 혼합 계수를 도출하고, 다목표 문제에서의 성과를 향상시키는 효과를 보여줍니다.

- **Performance Highlights**: 실험적 평가 결과, 제안된 PFEV(Parametric Frontier Entropy Search with Variational lower bound maximization) 방식이 특히 목적 함수의 차원이 3 이상인 경우에 기존 방식과 현저한 차이를 보이는 것으로 확인되었습니다. Gaussian process 모델을 기반으로 한 다양한 벤치마크 함수들에 대한 성능 분석을 통해, PFEV 방식의 우수성을 입증했습니다. 이러한 결과는 트렁케이션 방식을 통해 다목표 최적화 분야에서의 새로운 가능성을 제시합니다.



### Deep Multi-Task Learning Has Low Amortized Intrinsic Dimensionality (https://arxiv.org/abs/2501.19067)
- **What's New**: 이 논문에서는 딥 멀티 태스크 학습(Deep Multi-Task Learning) 분야에서 심층 신경망이 어떻게 강력한 일반화(generalization) 능력을 가지는지를 분석하고 설명합니다. 저자들은 고차원 모델을 낮은 차원(low-dimensional)으로 매개화하는 새로운 방법을 제시하며, 이 과정에서 무작위 확장(random expansions) 기법을 활용합니다. 또한, 멀티 태스크 네트워크가 단일 작업 학습(single-task learning)에 비해 훨씬 적은 자유 매개변수로 고정밀도(high-accuracy) 솔루션을 찾을 수 있음을 보여줍니다.

- **Technical Details**: 논문에서는 학습 작업을 확률 분포와 데이터 집합으로 구성하는 방법을 통해 형식화합니다. 또한, 멀티 태스크 설정에서 여러 관련 작업을 해결하기 위한 딥 러닝 모델의 높은 정확성을 달성하는 방법을 제시합니다. 새로운 일반화 경계(generalization bounds)는 네트워크 압축(weight compression) 및 PAC-Bayesian 추론을 기반으로 하며, 학습 중에 사용할 수 있는 양에 의존합니다.

- **Performance Highlights**: 실험 결과는 딥 멀티 태스크 모델이 단일 작업 설정보다 더 작은 유효 차원(effective dimension)을 가질 수 있음을 확인했습니다. 이러한 작은 차원과 무게 압축 기술의 결합은 이러한 모델의 강력한 일반화 능력을 뒷받침합니다. 따라서, 제안된 일반화 경계는 기존 연구보다 더 명확한 수치적 보장을 제공합니다.
...
### Optimizing Job Allocation using Reinforcement Learning with Graph Neural Networks (https://arxiv.org/abs/2501.19063)
Comments:
          12 pages, 5 figures

- **What's New**: 이번 보고서에서는 복잡한 스케줄링 문제에서의 작업 할당 효율성을 높이기 위해 강화 학습(Reinforcement Learning, RL)과 그래프 신경망(Graph Neural Networks, GNNs)을 결합한 새로운 접근 방식을 제안합니다. 이를 통해 주어진 자원에 최대한 많은 작업을 할당하면서 여러 제약 조건을 고려할 수 있는 방법을 모색했습니다. 기존의 수동 주석 필요성을 제거하여 더욱 실용적이고 확장 가능한 방식으로 작업 할당을 최적화할 수 있습니다.

- **Technical Details**: 우리는 작업 할당 문제(Job Allocation Problem, JAP)를 RL 프레임워크 내에서 형식화하고 딥 Q-러닝(Deep Q-learning) 방법론을 활용하여 그래프 신경망을 결합했습니다. GNN은 문제의 그래프 구조를 이용하여 효과적으로 학습하기 위해 사용됩니다. 이를 통해 시뮬레이션을 통해 직접 학습하고 성능 신호를 관찰하여 수동 라벨링 없이도 유용한 정책을 배울 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 접근 방식이 기존의 기준 알고리즘보다 뛰어난 성능을 보여 주었으며, 실제 및 합성 데이터셋 모두에서 그 효과성과 일반화 가능성을 입증했습니다. 특히, GNN은 기존 알고리즘을 지속적으로 초월하며, 이러한 성과는 복잡한 스케줄링 문제에서 작업 할당을 최적화할 수 있는 잠재력을 보여줍니다.



### TeZO: Empowering the Low-Rankness on the Temporal Dimension in the Zeroth-Order Optimization for Fine-tuning LLMs (https://arxiv.org/abs/2501.19057)
- **What's New**: 이 논문에서는 새로운 저순위( low-rank) 제로 오더 최적화( Zero-Order Optimization) 추정기인 TeZO를 제안합니다. TeZO는 모델과 시간 차원 모두에서 그라디언트의 저순위를 포착하여 GPU 메모리 소비를 줄이면서 효과적으로 학습을 진행할 수 있도록 설계되었습니다. 기존의 메소드를 개선하여 메모리 사용량을 감소시키면서 학습 비용도 저렴하게 유지할 수 있는 혁신적인 접근법입니다.

- **Technical Details**: TeZO는 그라디언트의 저순위를 확보하기 위해 Canonical Polyadic Decomposition (CPD)를 활용합니다. ZO 변동을 3D 텐서로 나타내어 각 반복(iteration)에서 저순위 2D 행렬을 추출함으로써 계산 비용을 기존의 𝒪⁢(d⁢T)에서 𝒪⁢(d+T)로 줄이는 것을 목표로 합니다. 또한 TeZO는 레이어마다 동적으로 순위를 선택할 수 있는 보조 기술을 도입하여 계층적 효율성을 높입니다.

- **Performance Highlights**: TeZO는 MeZO-Adam 및 MeZO-SGD와 비교할 때 메모리 소비 측면에서 약 35%만 사용하는 등 매우 효율적인 성능을 보여줍니다. 또, TeZO-Adam은 기존 ZO 최적화 방법과 비교해도 성능이 우수하며, 실험 결과에서 fine-tuning LLMs에서 최첨단(SOTA) 수준의 결과를 달성하는데 필요한 시간과 메모리 오버헤드를 줄입니다.
...
- **What's New**: 이번 연구에서는 저자들이 파라미터 효율적인 파인튜닝을 위한 새로운 기법인 norm-bounded low-rank adaptation (NB-LoRA)을 제안하였습니다. 이 방법은 가중치 적응 행렬의 각 singular value에 대한 명시적인 경계를 설정할 수 있도록 두 가지 파라미터화 방식이 도입되었습니다. 이로 인해 Schatten norm을 포함한 다양한 유니타리 불변 노름 경계를 만족할 수 있습니다. 또한 NB-LoRA는 모델의 재학습 중 발생할 수 있는 catastrophic forgetting을 방지하면서도 높은 적응 성능을 보장합니다.

- **Technical Details**: NB-LoRA는 기존의 LoRA 방법을 발전시켜, 행렬의 랭크(rank)와 노름(norm)을 모두 제어할 수 있는 유연한 파라미터화를 제공합니다. 이렇게 설정된 두 가지 방법은 각각 파라미터의 높은 효율성을 유지하면서 미리 정해진 랭크와 노름 조건을 자동으로 만족할 수 있게 설계되었습니다. 또한, NB-LoRA는 학습 속도(learning rate)와 에폭(epoch)에 대한 민감도를 줄이며, 다양한 하이퍼파라미터 설정에서도 견고한 성능을 보이는 것으로 확인되었습니다.

- **Performance Highlights**: 실험 결과, NB-LoRA는 기존 LoRA 및 기타 파인튜닝 기법들과 동등한 적응 성능을 달성하면서도 원본 모델의 '잊음(forgetting)'을 최소화하는 경향이 있습니다. 노름 경계를 설정하여 하이퍼파라미터 변화에 대한 민감도를 현저하게 줄이는 효과도 관찰되었습니다. 마지막으로, 이 연구는 프라이버시 보호 모델 병합 및 저랭크 행렬 완성(low-rank matrix completion)과 같은 실제 응용 사례에서도 NB-LoRA의 유용성을 보여주고 있습니다.



### Towards the Worst-case Robustness of Large Language Models (https://arxiv.org/abs/2501.19040)
- **What's New**: 이 논문은 최근의 연구 결과를 바탕으로 대형 언어 모델(Large Language Models, LLMs)의 적대적 공격에 대한 취약성을 탐구합니다. 특히, 새로운 공격 기법 I2-GCG를 통해 기존 방어 기법의 효과를 검증하고, DiffTextPure라는 새로운 방어 메커니즘을 제안합니다. 이 메커니즘은 정해진 smoothing distribution을 사용하여 입력 텍스트를 분산시켜 주고, 이를 프리트레인된 언어 모델로 정화합니다. 이론적으로는 모든 smoothing distribution에 대한 내구성 하한을 도출할 수 있습니다.

- **Technical Details**: 논문에서는 먼저, 적대적 공격에 대한 방어 메커니즘인 DiffTextPure를 제안하며, 이는 입력 프로프트를 perturb하고 다시 정화하는 두 가지 프로세스로 이루어집니다. 이 과정에서 smoothing distribution을 사용하여 입력을 통계적으로 처리하며, 확률적으로 각각의 단어를 마스킹 토큰 또는 다른 단어로 변환합니다. 이 과정에서 얻은 데이터는 사전 훈련된 LLM을 통해 정화되어, 최종적으로 적대적 예제에 대해 평균 80% 이상 내구성을 향상시킵니다.

- **Performance Highlights**: DiffTextPure는 입력의 확산 및 정화 과정을 통해 LLM의 안전성을 크게 향상시킵니다. 기존의 LLM 방어 방법들이 0%의 안전성을 보이는 것에 반해, DiffTextPure는 평균 $	ext{l}_0$ 변동 2.02 또는 평균 접미사 길이 6.41로 어떤 공격에 대해서도 저항력을 인증받았습니다. 이 연구는 LLM의 안정성을 높이는 새로운 접근법을 제시하며, 향후 연구에 대한 기초 자료로 활용될 것입니다.



### Error Slice Discovery via Manifold Compactness (https://arxiv.org/abs/2501.19032)
- **What's New**: 이 논문에서는 'manifold compactness'라는 새로운 일관성 메트릭을 제안합니다. 이 메트릭은 추가 정보 없이 데이터 기하학적 특성을 활용하여 error slice를 발견하는 데 도움을 줍니다. 이를 통해, 오류가 발생하는 구간(underperforming slices)에 대한 해석 가능한 세멘틱 패턴을 찾는 것이 가능해집니다. MCSD(Manifold Compactness based error Slice Discovery)라는 새로운 알고리즘은 이러한 일관성과 위험을 최적화 목표로 삼고 있습니다.

- **Technical Details**: 이 논문은 고차원 데이터가 저차원 매니폴드에 놓이는 경향을 기반으로 'manifold compactness'를 제안하며, 이를 통해 error slice의 일관성을 측정합니다. 기존 메트릭들과 달리, 이 새로운 메트릭은 미리 정의된 슬라이스 레이블에 의존하지 않고, 오류 있는 구간을 더 잘 포착합니다. MCSD 알고리즘은 평균 위험과 manifold compactness를 동시에 최적화하며, 다양한 작업의 훈련된 모델에도 유연하게 적용 가능합니다.

- **Performance Highlights**: MCSD 알고리즘은 기존의 방법들과 비교하여 dcbench 데이터셋에서 우수한 성능을 보임을 실험적으로 입증하였습니다. 더불어, 다양한 데이터셋과 작업에 대한 사례 연구를 통해 알고리즘의 유용성과 유연성을 극찬하고 있습니다. 이를 통해, 해당 알고리즘이 여러 실세계 응용 분야에서 효과적으로 활용될 수 있음을 시사합니다.



### True Online TD-Replan(lambda) Achieving Planning through Replaying (https://arxiv.org/abs/2501.19027)
- **What's New**: 본 논문에서는 True Online TD의 능력을 확장한 새로운 계획 방법을 개발하였습니다. 이 방법은 에이전트가 과거 경험을 효율적으로 재플레이(replay)할 수 있도록 하며, 매 시간 단계에서 발생한 경험을 활용합니다. 추가적으로, {\\lambda} 매개변수는 타깃 업데이트의 깊이와 재플레이 과정의 밀도를 지정하는 두 가지 역할을 수행합니다.

- **Technical Details**: 이 논문에서는 TD(\lambda)-Replan 알고리즘을 통해 과거 경험의 재플레이 과정이 어떻게 구성되는지를 보여줍니다. 특히, 여러 단계의 타깃을 사용하여 에이전트가 각 재플레이 업데이트에 대해 더 깊이 있는 타깃을 선택할 수 있습니다. 이 방법은 경험 재플레이를 최대한 활용하면서 상대적으로 저렴한 복잡도로 여러 단계를 효과적으로 다룰 수 있도록 설계되었습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 True Online TD(\lambda)에 비해 성능이 우수한 것으로 나타났습니다. 또한, Dyna Planning 및 TD(\lambda)-Replan 알고리즘과 같은 유사한 복잡도를 가진 다른 방법들보다도 뛰어난 성능을 보였습니다. 이러한 성능 향상은 특히 랜덤 워크 문제와 myoelectric 제어 도메인에서 입증되었습니다.
...
### Permutation-Based Rank Test in the Presence of Discretization and Application in Causal Discovery with Mixed Data (https://arxiv.org/abs/2501.18990)
- **What's New**: 이번 논문에서는 혼합 데이터를 처리할 수 있는 새로운 통계적 순위 테스트인 MPRT(Mixed data Permutation-based Rank Test)를 제안합니다. 기존의 통계적 순위 테스트가 모든 변수가 완벽하게 측정될 수 있다고 가정한 반면, 현실에서는 많은 변수가 이산화(discretization)된 형태로만 측정됩니다. MPRT는 이산화된 변수에 대해 Type I 오차를 효과적으로 제어할 수 있는 방법으로, 통계적 오류를 적절히 관리하면서도 활용 가능성을 높여줍니다.

- **Technical Details**: MPRT는 Permutation을 기반으로 하여, 다양한 형태의 데이터를 수용할 수 있는 통계적 테스트입니다. 특히, 이 테스트는 연속, 부분적으로 이산화되거나 완전히 이산화된 관측값을 아우르는 통계적 방법으로 설정됩니다. 또한, 이 논문에서는 최대 우도(maximum likelihood) 추정법을 사용하여 이산화의 영향을 분석하고, null 분포(null distribution)를 추정하는 과정을 기술하고 있습니다.

- **Performance Highlights**: MPRT의 효과는 여러 합성 데이터(synthetic data) 및 실제 데이터(real-world data)를 통해 실험적으로 입증되었습니다. 테스트 결과, 기존 방법들과 비교할 때 Type I 오차와 Type II 오차를 적절히 제어할 수 있다는 점에서 우수성을 보였습니다. 이러한 특성 덕분에 MPRT는 심리학, 생체 인식(biometrics), 경제학 등 여러 과학 분야에서 널리 적용될 수 있는 가능성을 제시합니다.
...
### Meta-learning of shared linear representations beyond well-specified linear regression (https://arxiv.org/abs/2501.18975)
- **What's New**: 본 연구에서는 다중 작업(multi-task) 및 메타 학습(meta-learning) 접근 방식에 영감을 받아, 작업 또는 사용자가 공유하는 구조(예: shared low-rank representations, clustered structures)를 학습하는 문제를 다룹니다. 이전 연구들이 정상적인 선형 회귀(linear regression)에 초점을 맞춘 반면, 본 연구는 일반적인 convex objectives를 고려하고, 각 함수의 극값에서 구조적 low-rank 및 군집(cluster) 가정을 표현합니다.

- **Technical Details**: 연구는 Hessian concentration과 noise concentration at the optimum 같은 약한 가정 하에, rank와 clustered regularized estimators가 이러한 구조를 어떻게 회복할 수 있는지를 보여줍니다. 특히, 각 작업에 대한 샘플 수와 작업 수가 충분히 많을 때 가능한 결과를 제시합니다. 또한, 단일 샘플만 있는 경우에 모든 해(solution)가 위치하는 서브스페이스(subspace)를 회복하는 문제를 연구합니다.

- **Performance Highlights**: 결론적으로, 본 연구는 convex learning objectives의 맥락에서 shared linear representation을 학습하기 위한 핵심적인 알고리즘을 제시하며, 여기서 핵심은 nuclear norm constraints를 활용하여 다항식 시간(polynomial-time) 내에 해결된다는 점입니다. 이러한 접근은 각 에이전트의 로컬 데이터 세트를 기반으로 n개의 모델을 공동으로 학습하는 데 큰 기여를 제공할 수 있습니다.
...
- **What's New**: BCAT 모델은 2차원 유체 역학 문제의 솔루션을 예측하기 위해 설계된 PDE 기반의 기초 모델로, 블록 인과(transformer) 구조를 적용합니다. 이 모델은 이전 프레임을 맥락적 기초로 활용하여 다음 프레임을 예측하며, 토큰 예측을 넘어선 새로운 접근을 선보입니다. 실험 결과, BCAT는 기존 모델에 비해 2.9배 높은 정확성을 기록하며, 6가지 예측 작업에서 평가되었습니다.

- **Technical Details**: BCAT 모델은 spatio-temporal dynamics를 모델링하기 위해 설계된 블록 인과(transformer) 아키텍처 기반으로 동작합니다. 입력된 유체 상태 데이터는 패치로 나누어 시퀀스 시각적 토큰으로 변환되어 처리됩니다. 이 과정에서 BCAT는 다음 상태를 자가 회귀적으로 예측하며, 이는 복잡한 의존성을 포착하는 데 도움을 줍니다.

- **Performance Highlights**: BCAT는 널리 사용되는 벤치마크 데이터셋에서 최고 수준의 정확도를 기록하였으며, 다른 대형 모델에 비해 효율적인 매개변수 조정을 통해 11%의 오류 개선을 보였습니다. 전체 평가 작업에서 평균 상대 오류는 1.92%로, 기존의 PDE 기초 모델보다 45% 더 나은 정확성을 보여주었습니다.
...
- **What's New**: 이 논문은 대규모 모델 훈련에서 학습률 스케줄(learning-rate schedules)이 비결정적 볼록 최적화(non-smooth convex optimization) 이론의 성능 경계(performance bound)와 유사하게 작용한다는 점을 밝혔습니다. 특히, 상수 스케줄과 선형 쿨다운(linear cooldown)에서의 경계를 제공하며, 쿨다운의 실용적인 이점이 로그 항(logarithmic terms)의 부재로 반영된다는 점을 강조했습니다. 또한, 이론을 실용화하여 Llama 스타일 모델의 훈련에서 뚜렷한 성능 개선을 달성하는 방법을 제시합니다.

- **Technical Details**: 논문은 주로 와이드 스케줄(warmup-stable-decay, wsd)과 코사인 스케줄(cosine schedule)을 비교하고, wsd의 쿨다운 기간이 손실(loss)의 급격한 감소를 유도한다는 이론적 결과를 도출합니다. 이 연구에서는 쿨다운 기간이 성능에 미치는 영향을 설명하며, 특히 비결정적 스토캐스틱 볼록 환경에서 wsd의 수렴 결과를 도출합니다. 이 이론적 결과는 SGD에서의 최적화 문제에서 생성된 준최적 토대(suboptimality bound)를 활용하여 여러 경험적 결과를 재현할 수 있음을 보여줍니다.

- **Performance Highlights**: 본 연구 결과에 따르면, 훈련 중 쿨다운을 최적화한 wsd 스케줄을 사용하는 것이 124M 및 210M Llama 스타일 모델 훈련 시 성능을 눈에 띄게 향상시켰습니다. 실험을 통해, 이론적으로 최적의 스케줄을 적용하여 훈련을 계속하면 동일한 학습률로 훈련하는 것보다 성능이 개선되며, 또한 스케줄을 넘나드는 최적 학습률의 전이 가능성을 보여줍니다. 이러한 결과는 비결정적 딥러닝에서 학습률 스케줄의 경험적 행동이 비결정적 볼록 최적화 이론의 경계로 정밀하게 설명될 수 있음을 나타냅니다.



### Spend Wisely: Maximizing Post-Training Gains in Iterative Synthetic Data Boostrapping (https://arxiv.org/abs/2501.18962)
- **What's New**: 본 연구에서는 iterative bootstrapping 과정에서의 최적 예산 할당 전략을 이론적으로 분석합니다. 특히, 생성된 데이터의 양을 매 iteration에서 어떻게 결정해야 최종 성능을 극대화할 수 있는지에 대한 질문을 다룹니다. 기존의 고정 예산 할당 전략인 constant policies가 높은 확률로 수렴하지 못하는 반면, exponential growth policies는 이론적으로 유의미한 강점을 보입니다.

- **Technical Details**: 논문에서는 Gaussian 데이터와 exponential reward functions에 대한 이론적 분석을 통해, 데이터의 양을 매 iteration에서 지수적으로 증가시킬 필요가 있음을 증명합니다. 적절한 예산 할당 및 성과 극대화를 위한 정책을 수립하는 데 필요한 기본 표기법을 다루며, 가장 일반적인 환경에서도 고정 전략이 수렴하지 못하는 반면 증가하는 정책들이 최적 성과로 수렴할 수 있음을 보여줍니다. 이론적 결과는 다양한 실험을 통해 검증됩니다.

- **Performance Highlights**: 이미지 노이즈 제거 및 다수의 언어 모델에 대한 수치 실험을 통해 지수적 및 다항적 증가 정책이 고정 정책을 일관되게 초과 성능을 보임을 확인했습니다. 특히, 지수적 정책은 선형 정책과 비교해도 안정적인 성과를 제공하며, 모든 실험에서 우수한 성능을 발휘했습니다. 본 연구는 synthetic data 생성을 최적화하기 위한 전략을 마련하면서 실무자에게도 유용한 가이드를 제공합니다.
...
### O-MAPL: Offline Multi-agent Preference Learning (https://arxiv.org/abs/2501.18944)
- **What's New**: 이번 연구에서는 다중 에이전트 강화 학습(MARL)에서의 보상 함수 추정과 정책 학습을 통합한 새로운 접근 방식을 소개합니다. 이는 기존의 방법들이 겪는 불안정한 훈련 문제를 해결하기 위해, 제안된 O-MAPL(Offline Multi-Agent Preference Learning) 알고리즘을 통해 선호 기반 학습 프레임워크를 도입합니다. 이러한 end-to-end 접근법은 보상 모델 없이 선호 데이터로부터 직접적으로 정책을 학습할 수 있게 돕습니다.

- **Technical Details**: O-MAPL 알고리즘은 강화 학습의 MaxEnt(RL)에서 보상과 soft Q-function 간의 관계를 활용하여 선호 데이터로부터 직접 soft Q-function을 학습합니다. 이 과정은 중앙 집중식 훈련(CTDE, Centralized Training with Decentralized Execution) 패러다임 안에서 이루어지며, 지역 정책의 효율적인 훈련을 가능하게 합니다. 연구팀은 관련 이론적 분석을 통해 선호 기반 학습 목표의 볼록성(convexity) 및 지역-글로벌 일관성(local-global consistency)의 필요성을 설명합니다.

- **Performance Highlights**: O-MAPL 알고리즘은 SMAC 및 MAMuJoCo 벤치마크에서 다양한 작업에 대해 기존 방법들과 비교하여 우수한 성능을 보였습니다. 특히, 제안된 방법은 역할에 따른 다양성을 유지하면서도 효과적으로 훈련 효율성을 높였습니다. 실험 결과, O-MAPL이 다중 에이전트 환경에서 선호 기반 학습 방법으로써 매우 강력한 성능을 입증했다는 점에서 이 연구의 중요성을 강조할 수 있습니다.
...
### TabFSBench: Tabular Benchmark for Feature Shifts in Open Environmen (https://arxiv.org/abs/2501.18935)
- **What's New**: 이 논문에서는 탭 형식 데이터(tabular data)에서의 피처 시프트(feature shift)에 대한 최초의 종합 연구를 수행합니다. 이전 연구들은 주로 배포 변화(distribution shift)에 초점을 맞췄던 반면, 이 연구는 피처 시프트라는 독특하고 탐색되지 않은 문제를 다룹니다. 논문은 TabFSBench라는 최초의 피처 시프트 벤치마크를 도입하여 피처 시프트의 영향을 평가합니다.

- **Technical Details**: TabFSBench는 네 가지 다양한 피처 시프트 시나리오가 네 가지 탭 모델 카테고리에서 어떻게 작용하는지를 분석합니다. 또한 이 벤치마크는 대형 언어 모델(LLMs) 및 탭형 LLM의 성능을 처음으로 평가합니다. 연구 결과, 피처의 중요도와 모델의 성능 저하 간의 선형 관계가 존재함을 확인하였습니다.

- **Performance Highlights**: 연구는 탭 형식 모델의 대다수가 피처 시프트 시나리오에서의 적용성이 제한적이라는 세 가지 주요 관찰 결과를 보여줍니다. 또한, 닫힌 환경(closed environment)에서의 모델 성능은 피처 시프트 성능과 상관관계가 있음을 나타냅니다. 마지막으로, 각 관찰 결과에 대한 향후 연구 방향도 논의하였습니다.



### LLM Program Optimization via Retrieval Augmented Search (https://arxiv.org/abs/2501.18916)
- **What's New**: 이번 논문에서는 대형 언어 모델(LLMs)을 활용하여 프로그래밍 최적화 문제를 해결하는 혁신적인 방법인 Retrieval Augmented Search (RAS)를 제안합니다. 기존 방법들과 달리 RAS는 코드 대신 자연어 설명을 기반으로 예제를 검색하여, 프로그램 최적화에 있어 더 효과적인 결과를 보여줍니다. 또한, AEGIS라는 방법을 통해 더 작은 단위의 수정인 "atomic edits"로 나누어 해석 가능성을 높입니다. 실험 결과, RAS는 다른 최신 기법보다 1.8배 뛰어난 성능을 기록했습니다.

- **Technical Details**: RAS 알고리즘은 두 가지 주요 기법을 통해 동적 검색을 개선합니다. 첫째, 코드 자체가 아닌 LLM이 생성한 자연어 설명을 기반으로 예제를 검색합니다. 둘째, 고정된 프로그램 집합을 검색하는 대신, 반복적으로 검색-최적화-평가 루프를 수행하여 브림 검색(beam search)을 통해 프로세스를 진행합니다. 이러한 방법을 통해 LLM을 보다 효율적으로 최적화하는 블랙박스 기법을 개발했습니다.

- **Performance Highlights**: 제안된 RAS 방법은 C++ 프로그램 최적화에 대한 PIE 벤치마크를 통해 검증되었으며, 평균 8.01배의 속도 개선을 달성했습니다. AEGIS는 평균 6.08배 속도 개선을 보이며, 수정 크기를 RAS 대비 17% 감소시켰습니다. 이러한 결과는 RAS와 AEGIS가 코드 최적화를 위한 블랙박스 조정의 유망한 전략이 될 수 있음을 보여줍니다.



### An Invitation to Neuroalgebraic Geometry (https://arxiv.org/abs/2501.18915)
- **What's New**: 이번 논문에서는 기계 학습 모델에 의해 매개변수화된 함수 공간에 대한 연구를 대수기하학(algebraic geometry)의 관점에서 제안합니다. 특히, 다항 활성화(polynomial activations)를 갖는 신경망(neural networks)과 같은 대수 모델(algebraic models)에 집중하며, 이와 관련된 함수 공간은 반대수적 다양체(semi-algebraic varieties)로 설명할 수 있습니다. 이 연구는 기계 학습과 대수기하학 간의 다리 역할을 하는 새로운 연구 방향인 신경대수기하학(neuroalgebraic geometry)의 기초를 마련합니다.

- **Technical Details**: 기계 학습 모델은 그 매개변수가 변화함에 따라 함수 공간을 정의하며, 이러한 공간은 신경 다양체(neuromanifolds)라고 불립니다. 신경 다양체의 기하학은 모델의 통계적 및 계산적 측면을 제어하는 데 중요한 역할을 하며, 여기에는 샘플 복잡도(sample complexity)와 표현력(expressivity) 등이 포함됩니다. 더욱이, 신경망은 목표 함수(objective function)의 기울기 흐름(gradient flow)을 통해 학습하며, 이는 신경 다양체에서 기능적 거리를 최소화하는 것으로 해석될 수 있습니다.

- **Performance Highlights**: 대수적 모델들은 일반 함수(continuous functions)를 임의의 정밀도로 근사할 수 있기 때문에, 이들은 매우 일반적이며 임의의 신경 다양체(neuromanifolds)를 근사할 수 있는 능력이 있습니다. 이 연구는 고차원 공간에서의 거리가 기계 학습 모델 학습 과정에서 중요한 역할을 한다고 제안하며, 메트릭 대수기하학(metric algebraic geometry)에서 발생하는 거리 기반 문제들을 탐구합니다. 이러한 점에서, 신경대수기하학은 대수적 통계(algebraic statistics)와 관계가 깊으며, 기계 학습에서 중요한 통찰을 제공할 수 있는 잠재력을 가지고 있습니다.



### Scaling Laws for Differentially Private Language Models (https://arxiv.org/abs/2501.18914)
- **What's New**: 이 연구에서는 Differential Privacy (DP) 훈련의 복잡한 특성을 모델링하는 새로운 스케일링 법칙(scaling laws)을 제안합니다. 이러한 법칙은 DP 훈련에서의 계산(compute), 개인정보 보호(privilege), 유용성(utility) 간의 트레이드오프(trade-off)의 전체적인 그림을 제공합니다. 연구는 많은 설정에서 최적의 훈련 구성(configuration)을 찾는 데 필요한 가이드를 제공합니다.

- **Technical Details**: 연구에서는 마스크 언어 모델링(masked language modeling) 작업에 중점을 두고, 데이터셋은 각 개인이 기여하는 텍스트 시퀀스로 구성됩니다. DP-SGD(종단간 차별 개인정보 보호의 감리 운동) 알고리즘을 사용하여 훈련하는 방식으로, 각 예제가 기여하는 민감성을 제한하고 평균 클립(클립된) 기울기에 가우시안 노이즈를 추가하여 DP 보장을 도출합니다. 모델링의 기초로써, 연구진은 전통적인 스케일링 법칙을 DP 훈련에 맞게 확장하였습니다.

- **Performance Highlights**: 경험적으로 분석한 결과에 따르면, DP 훈련에서는 비Private 스케일링 법칙이 최적의 계산 예산 분배를 제공하지 않음을 발견했습니다. 그러나 DP 훈련에서는 어떤 개인정보 보호 예산과도 적합한 모델 크기, 배치 크기, 반복 사이클을 예측할 수 있는 모델을 제공합니다. DP 훈련 하에서 계산 예산을 약 5배에서 100배 절약할 수 있으며 비슷한 수준의 개인정보 보호와 유용성을 유지할 수 있습니다.
...
### A machine learning approach for Premature Coronary Artery Disease Diagnosis according to Different Ethnicities in Iran (https://arxiv.org/abs/2501.18893)
- **What's New**: 이번 연구는 조기 관상동맥 질환(PCAD)에 있어 인종(Ethnicity)이 주요 위험 요소 중 하나로 작용하는지를 밝히고자 하였습니다. 기존 연구들은 인종이 PCAD에 미치는 영향에 대한 분석이 부족했으며, 본 연구에서는 다양한 인종의 PCAD 환자들을 대상으로 조사를 진행하였습니다. 결과적으로, 인종이 PCAD 진단 및 예방에서 중요한 요소임을 시사합니다.

- **Technical Details**: 연구에서는 PCAD 환자들의 나이, 성별, 체질량지수(BMI), 복부 비만(허리 둘레), 당뇨병(DM), 고혈압(HBP), 고밀도 지단백 콜레스테롤(LDL-C), 흡연과 같은 아홉 가지 주요 위험 요소를 비교하였습니다. 환자들은 관상 혈관 조영술을 통해 CAD 진단을 확인했으며, 특성 가중 알고리즘을 통해 인종의 중요성을 분석했습니다. 또한, 인종을 포함한 예측 모델과 구분한 모델의 성능을 비교하여 인종이 포함될 때 예측력 향상을 보여주었습니다.

- **Performance Highlights**: 연구 결과, 성별과 나이가 PCAD 예측에 가장 큰 영향을 미치는 요소로 확인되었으며, 그 다음으로 인종이 중요하게 작용하는 것으로 나타났습니다. 예측 모델에서 인종을 위험 요소로 포함했을 때, 모델의 효율성이 개선된 것으로 나타났습니다. 이러한 결과는 PCAD 진단 및 예방조치에서 인종의 중요성을 강조합니다.



### CAAT-EHR: Cross-Attentional Autoregressive Transformer for Multimodal Electronic Health Record Embeddings (https://arxiv.org/abs/2501.18891)
- **What's New**: 본 연구에서는 전자 건강 기록(EHR, Electronic Health Records)로부터 강력하고 작업 비의존적인 임베딩(embeddings)을 생성하는 새로운 아키텍처인 CAAT-EHR을 소개합니다. CAAT-EHR은 원시 EHR 데이터를 통해 얻은 정보를 활용하여 데이터의 시간적(temporal) 및 맥락적(contextual) 관계를 통합하여 복잡한 의존성을 포착하는 임베딩을 생성합니다. 이 모델은 수작업 특성 공학(manual feature engineering)의 필요성을 제거하며 다양한 하위 작업으로의 이식성을 향상시킵니다.

- **Technical Details**: CAAT-EHR는 인코더(encoder)에서 자기 및 교차 주목(self- and cross-attention) 메커니즘을 사용하여 다중 모드(multi-modal) 데이터를 통합합니다. 이 과정에서 데이터는 풍부한 임베딩으로 전환되어, EHR 데이터의 시간적 및 다중 모드 의존성을 완전히 활용할 수 있는 기반을 마련합니다. 또한, 오토회귀 디코더(autoregressive decoder)는 사전 학습(pre-training) 동안 향후 시간 점의 데이터를 예측하여 생성된 임베딩이 시간적 일관성(temporal consistency)을 유지하도록 합니다.

- **Performance Highlights**: 광범위한 벤치마크 데이터셋에서의 평가 결과, CAAT-EHR로 생성된 임베딩은 전처리된 원시 EHR 데이터와 기존 방법들보다 뛰어난 성능을 보였습니다. 이를 통해 CAAT-EHR가 EHR 데이터의 활용 가능성을 크게 증가시킬 수 있다는 것을 입증하였습니다. 이 연구는 전자 건강 기록의 분석 및 예측 모델링에서의 새로운 가능성을 제시합니다.
...
### Understanding Generalization in Physics Informed Models through Affine Variety Dimensions (https://arxiv.org/abs/2501.18879)
- **What's New**: 최근 물리 정보를 반영한 머신러닝(Physics-Informed Machine Learning, PIML)이 물리적 구조를 머신러닝 모델에 통합함으로써 통계적 성능과 샘플 효율성을 향상시킬 수 있는 능력 덕분에 주목받고 있습니다. 이 연구는 선형 회귀 분석기(linear regressors)가 외부 파라미터 수가 아닌, 관련된 affine variety의 차원(dimension)에 의해 일반화 능력이 결정된다는 사실을 보여줍니다. 이 발견은 비선형(nonlinear) 방정식을 포함하여 다양한 방정식에 대한 통합 분석을 가능하게 합니다.

- **Technical Details**: PIML은 머신러닝 모델에 물리적 지식을 통합하는 하이브리드 접근법으로, 주로 미분 방정식(differential equations), 보존 법칙(conservation laws), 대칭(symmetries) 등의 물리적 구조를 통해 일반화 능력을 높입니다. 본 연구는 실험적으로 affine variety의 차원 측정을 위한 방법과 함께 이론적 통찰력을 validates합니다. 이론적으로 살펴본 결과, 물리적 구조는 데이터 양에 비해 충분히 많은 파라미터가 있는 상황에서도 내재적 차원(intrinsic dimension)을 줄여 overfitting을 예방하는 역할을 합니다.

- **Performance Highlights**: 본 연구는 물리적 구조가 포함된 모델이 일반화 성능을 향상시키는 메커니즘을 이해하는 데 기여합니다. 연구 결과는 PIML 모델이 여러 복잡한 물리 현상에 적용될 수 있는 가능성을 열어줍니다. 실험적 증거를 통해 기존의 모델들과 비교했을 때 물리 정보의 통합이 실제적 이점을 제공함을 확인하였습니다.
...
- **What's New**: 이 논문은 preference-based reinforcement learning (PbRL)에서 최적 정책 식별 문제를 다룹니다. 특히, 노이즈가 있는 이진 선호로부터 학습하여 일반적인 리워드 모델을 대체할 수 있는 방법을 제안합니다. 또한, 본 연구에서는 Top-Two Thompson Sampling에 영감을 받은 Posterior Sampling for Preference Learning (PSPL)이라는 새로운 알고리즘을 소개합니다.

- **Technical Details**: PSPL은 불확실한 보상 모델과 전이 동역학에 대한 독립적인 사후 확률을 유지하여, 데이터 활용을 극대화합니다. 이 알고리즘은 오프라인 학습 데이터와 온라인 탐색 데이터를 조합하여 최적의 정책을 학습합니다. 본 논문은 PbRL 환경에서 PSPL의 이론적 보장을 제공하며, 간단한 Bayesian regret에 대한 상한선을 설정합니다.

- **Performance Highlights**: PSPL 알고리즘은 계산적으로 복잡하지만, 근사버전을 제공하여 기존의 기준선에 비해 우수한 성능을 보입니다. 본 연구에서 제안된 알고리즘은 간단한 벤치마크 환경에서 실행되었으며, 그 결과는 기존 방법들보다 뛰어난 성능을 보여주었습니다. 이로 인해, 인간 피드백을 활용한 생성 모델의 조정에 매우 유용한 접근 방식을 제시하고 있습니다.



### Neural SDEs as a Unified Approach to Continuous-Domain Sequence Modeling (https://arxiv.org/abs/2501.18871)
- **What's New**: 이 연구에서는 Neural SDE(Neural Stochastic Differential Equation)를 이용하여 연속 시퀀스 모델링을 위한 새로운 접근 방식을 제안합니다. 우리의 방법은 시간에 따른 변화를 모델링하며, 지속적인 동적 시스템의 샘플로 시간 시퀀스 데이터를 해석합니다. 특히, 이 연구는 고차원 및 시간적으로 복잡한 데이터에서 SDE 기반 모델링의 가능성을 보여주는 첫 번째 사례로, 새로운 연구 방향을 여는 데 기여할 것으로 기대됩니다.

- **Technical Details**: 우리의 방법은 Neural SDE를 통해서 시간에 따른 연속 시스템의 진화를 직접 모델링합니다. 이 모델은 흐름(drift)과 확산(diffusion) 항을 신경망(neural networks)으로 매개화하여 구현되며, 최대 우도(maximum likelihood) 목표를 유도하고 시뮬레이션 없이 효율적으로 훈련할 수 있는 방법을 제안합니다. 이를 통해 연속적인 동적 특성을 보존하면서도 더 자연스러운 시퀀스 표현을 제공합니다.

- **Performance Highlights**: 우리는 제안한 모델이 다양한 시퀀스 모델링 작업에서 뛰어난 성능을 발휘함을 실험적으로 입증하였습니다. 실험에서는 궤적 생성(trajctory generation), 모방 학습(imitation learning), 비디오 예측(video prediction) 등의 과제를 수행하였으며, 모델의 다재다능성과 유연성을 강조하였습니다. 이러한 결과들은 Neural SDE가 복잡한 연속 데이터 처리에 적합하다는 것을 시사합니다.



### Continuous-Time Analysis of Federated Averaging (https://arxiv.org/abs/2501.18870)
Comments:
          Under review

- **What's New**: 이번 논문은 수평 연합 학습(horizontal federated learning)에서 중요한 알고리즘인 페더레이티드 애버징(FedAvg)에 대한 연구를 지속 시간(continuous-time) 관점에서 처음으로 확장하였습니다. 기존의 이산 반복 설정(discrete iteration setting)에서 FedAvg의 수렴(convergence) 분석이 광범위하게 이루어졌다면, 이번 연구는 멀티변량 확률 미분 방정식(multivariate stochastic differential equation)에 따른 글로벌 가중치의 진화를 다룹니다.

- **Technical Details**: 우리는 확률 과정(stochastic processes)에서 얻은 기법을 사용하여 다양한 손실 함수(loss functions) 아래의 수렴 보장(convergence guarantees)을 연립 방정식 형태로 수립합니다. 이 연구에서는 이산 설정(discrete setting)에서의 기존 연구보다 더 일반적인 경우를 다루며, 추가적으로 FedAvg의 업데이트가 정규 랜덤 변수(normal random variables)로 근사될 수 있는 조건도 제공합니다.

- **Performance Highlights**: 연속 시간 설정을 사용함으로써 FedAvg의 일반화 속성(generalization properties)을 드러내는 새로운 통찰을 제시합니다. 이는 연합 학습의 수렴 특성을 이해하는 데 기여하며, 실제로 다양한 손실 함수에 대해 성능을 더욱 향상시킬 수 있는 가능성을 제시합니다.



### A Deep Spatio-Temporal Architecture for Dynamic Effective Connectivity Network Analysis Based on Dynamic Causal Discovery (https://arxiv.org/abs/2501.18859)
- **What's New**: 본 논문에서는 동적 효과적 연결망(dECNs)을 분석하기 위한 심층 시공간 융합 아키텍처를 제안합니다. 기존 방법들이 동적 인과성을 간과하고 있을 뿐만 아니라, 뇌 활동 데이터의 시공간 정보를 제대로 활용하지 못하고 있는 문제를 해결하고자 합니다. 제안된 모델은 시뮬레이션 데이터와 필라델피아 신경발달 집단(PNC) 데이터를 통해 저자의 방법이 데이터 분석에서 더 우수한 결과를 나타낸다는 것을 보여줍니다.

- **Technical Details**: 제안된 방법은 심층 자기 인코더(deep auto-encoder) 구조를 활용하여 동적 인과성을 발견하는 과정을 거칩니다. 모델은 Gated Recurrent Unit (GRU) 인코더를 사용하여 동적 인과 마스크 행렬을 생성하고, 디코더를 통해 발견된 인과 정보의 정확성을 검증합니다. 또한, 그래프 합성곱 신경망(Graph Convolutional Network, GCN) 모듈을 결합하여 시공간 정보를 융합하고 인과성을 파악하는 데 기여합니다.

- **Performance Highlights**: 실험 결과, 제안된 STDCDAE 모델은 선형 및 비선형 시뮬레이션 데이터에서 기존 인과 발견 알고리즘보다 더 정확하게 인과성을 발견할 수 있음을 입증하였습니다. 또한, 동적 효과적 연결망(dECNs)을 추론하는 데 있어 신경 발달 과정 중 뇌의 기능적 네트워크 변화에 대한 통찰을 제공합니다. 연구 결과, 뇌 기능 네트워크는 어릴 때 무작위 상태에서 성숙하면서 더욱 안정화된 전연결망으로 발전함을 보여줍니다.
...
- **What's New**: 이번 연구에서는 reasoning models(추론 모델)에서 inference-time compute(추론 시간 계산량)를 증가시켰을 때의 강인성(robustness)에 미치는 영향을 실험합니다. OpenAI o1-preview와 o1-mini 모델을 대상으로 하며, 다양한 공격에 대해서 compute를 증가시키면 강인성이 향상된다는 사실을 발견했습니다. 이는 일반적으로 model samples(모델 샘플)에서 공격이 성공하는 비율이 증가된 compute와 함께 0에 가까워진다는 것을 의미합니다.

- **Technical Details**: 우리는 혼합적 공격(adversarial attack)에 대한 훈련(adversarial training)을 수행하지 않고, 단순히 모델이 추론에 더 많은 계산 자원을 사용할 수 있도록 허용하여 inference-time compute를 증가시킵니다. 연구의 주요 목표는 대형 언어 모델(Large Language Models)의 강인성 향상을 위한 inference-time compute의 잠재력을 평가하는 것이었습니다. 또한 새로운 공격 방법을 제안하며, 추론 모델에서 compute가 신뢰성을 높이지 않는 경우에 대해서도 논의합니다.

- **Performance Highlights**: 연구 결과, 많은 경우 inference-time compute의 증가가 모델의 성능 향상과 밀접한 관련이 있음을 보여줍니다. 하지만 몇 가지 중요한 예외적인 경우도 발견되어, 모든 상황에서 compute의 증대가 항상 유리하지는 않음을 시사합니다. 이러한 경과를 통해 향후 더 효과적인 공격 방법과 이를 해결할 수 있는 방안에 대한 논의가 필요함을 제안합니다.
...
### Transfer Learning for Nonparametric Contextual Dynamic Pricing (https://arxiv.org/abs/2501.18836)
- **What's New**: 이 논문은 dynamic pricing의 새로운 연구 영역으로 transfer learning을 통한 비모수적 맥락 동적 가격 책정에 대해 분석합니다. Transfer Learning for Dynamic Pricing (TLDP) 알고리즘을 제안하여, 출처 영역(source domain)에서 수집된 데이터를 활용하여 목표 영역(target domain)에서의 가격 책정을 향상시킬 수 있습니다. 특히, 이 연구는 가격 조정의 최적화를 위한 전통적인 접근 방식의 한계를 극복할 수 있는 가능성을 보여줍니다.

- **Technical Details**: TLDP 알고리즘은 covariate shift 모델 하에서 작동하며, reward function은 동일하게 유지되지만, covariate의 분포는 출처와 목표 영역간에 다릅니다. 이 논문에서는 TLDP의 후회 상한(regret upper bound)을 Lipschitz 조건 설정하에 수학적으로 증명하고, Minimax 하한도 도출하여 연구의 최적성을 입증합니다. 이를 통해 TLDP의 성능이 다른 기존 방법들보다 우수함을 명확히 하고 있습니다.

- **Performance Highlights**: 실험 결과 TLDP 알고리즘은 기존의 방법들에 비해 뛰어난 효과를 보였습니다. 특히, 실세계 응용에서의 실제 유용성을 강조하며, 이 알고리즘이 dynamic pricing 문제를 해결하는 혁신적인 접근임을 보여주고 있습니다. 따라서 새로운 시장 진입이나 신제품 출시 시, 데이터 부족 문제를 해결하기 위한 유망한 전략으로 자리잡을 가능성이 큽니다.



### Transcoders Beat Sparse Autoencoders for Interpretability (https://arxiv.org/abs/2501.18823)
- **What's New**: 이번 연구는 스킵 트랜스코더(Skip Transcoder)를 도입하여, 원래의 MLP(다층 퍼셉트론) 기능을 보존하면서 재구성 오류를 줄이는 동시에 해석 가능성을 손상시키지 않음을 보여줍니다. 연구팀은 동일한 모델과 데이터에 대해 SAEs(희소 자동 인코더)와 트랜스코더를 비교하며, 트랜스코더의 기능이 해석 가능성이 현저히 높은 것으로 나타났습니다. 또한, 재구성 손실(reconstruction loss)을 효과적으로 줄이는 새로운 아키텍처 개선 방안을 제시했습니다.

- **Technical Details**: 스킵 트랜스코더는 트랜스코더 아키텍처에 선형 스킵 연결(linear skip connection)을 추가하여, 해석 가능성 점수에 영향을 주지 않으면서 원래 MLP를 잘 근사할 수 있도록 합니다. 이 연구에서는 Pythia, Llama, Gemma와 같은 다양한 모델의 MLP 계층에서 SAEs, 스파스 트랜스코더(ST), 스파스 스킵 트랜스코더(SST)를 훈련했습니다. 훈련 과정에서는 손실함수를 최소화하기 위해 평균 제곱 오차(mean squared error)를 사용하였으며, 최신 TopK 활성화 함수가 적용되었습니다.

- **Performance Highlights**: 실험 결과, 스킵 트랜스코더는 재구성과 해석 가능성 사이의 트레이드오프에서 SAEs를 초과하는 성능을 보였습니다. 또한, SAEBench에서 전반적인 성능을 평가하며 모델 간의 품질 개선을 입증하였습니다. 연구를 통해 해석 가능성을 중요시하는 연구자들이 SAEs의 활용에서 스킵 트랜스코더로의 전환을 고려해야 할 필요성이 강조되었습니다.
...
### Estimating the Probability of Sampling a Trained Neural Network at Random (https://arxiv.org/abs/2501.18812)
- **What's New**: 이 연구에서는 신경망(parameter space) 파라미터 공간에서 특정 행동에 해당하는 영역의 확률 질량을 추정하는 알고리즘을 제안합니다. 이는 Gaussian 또는 uniform prior를 바탕으로 하며, uniform prior일 경우 영역의 볼륨(volume)을 측정하는 것과 동등합니다. 이 연구는 기존 볼륨 추정 알고리즘들이 실제 볼륨을 수백만 배 과소 추정하고 있다는 점을 이론적으로나 경험적으로 보여줍니다. 또한 중요 샘플링(importance sampling) 방법을 통해 이 오류를 극적으로 줄일 수 있음을 발견했습니다.

- **Technical Details**: 제안된 알고리즘은 비용 함수(cost function)와 특정 threshold에 대해 weight vector의 지역적 볼륨(local volume)을 정의합니다. 이는 로컬 볼륨이 𝒮내에서 네트워크를 샘플링할 확률로 해석될 수 있으며, 이 때 사용한 measure(m μ)는 초기화 전 네트워크의 확률 분포와 밀접하게 관련되어 있습니다. 연구자들은 특히 예측 확률 분포를 출력하는 신경망에 대해 KL 비용 함수(KL cost function)를 고려하며, 이는 실제 레이블과는 무관하게 두 파라미터가 행동적으로 얼마나 다른지를 측정하는데 유용합니다. 이를 통해 반응 네트워크의 복잡성도 분석합니다.

- **Performance Highlights**: 이 연구에서는 훈련된 Pythia 31M 언어 모델이 초기화 분포에서 랜덤으로 샘플링될 확률이 약 1/1 뒤에 3억 6천만 개의 0이 오는 확률임을 추정합니다. 이는 관측 가능한 우주에 있는 원자의 수(약 10의 80제곱)와 비슷한 수치로, 특정 원자를 연속으로 맞추는 것이 매우 극단적인 사건임을 시사합니다. 또한, 일반화가 잘 되지 않는 행동 영역은 상대적으로 작아서 무작위로 샘플링될 가능성이 적다는 점도 발견하여, 잘 일반화되는 함수에 대한 유도 편향(inductive bias)을 잘 보여줍니다.



### Learning Hamiltonian Dynamics with Bayesian Data Assimilation (https://arxiv.org/abs/2501.18808)
Comments:
          8 pages, 12 figures

- **What's New**: 본 논문에서는 알려지지 않은 해밀토니안 동적 시스템을 위한 시계열 예측을 위한 신경망 기반 접근 방식을 개발했습니다. 이를 위해 서프 로겟 모델을 활용하여 일반화된 좌표(positions)와 그에 대응하는 운동량(conjugate momenta)을 사용하여 시스템 동역학을 학습하며, 일정한 해밀토니안을 유지합니다. 또한, 장기 예측 정확도를 높이기 위해 자가회귀 예측 오류를 학습 목표에 통합한 Autoregressive Hamiltonian Neural Network를 도입했습니다. 이 외에도 베이지안 데이터 동화(Bayesian data assimilation)를 통해 온라인 측정 데이터를 이용해 실시간으로 예측을 정제하는 방법을 포함하고 있습니다.

- **Technical Details**: 이 연구의 주요 기여 중 하나는 HNN 훈련을 위한 손실 함수(loss function)를 제안하여, 위치와 속도 데이터를 기반으로 알려지지 않은 동역학을 효과적으로 학습할 수 있게 한다는 점입니다. HNN은 모델의 해밀토니안 구조를 보존하면서도 자가회귀 형태의 손실 함수를 활용하여 예측 정확도를 더욱 개선하는 방식으로 설계되었습니다. 또한, 우리는 HNN과 비선형 시스템 상태 추정을 위한 Unscented Kalman Filter(UKF)를 통합하여 실시간 측정값을 동화함으로써 예측의 정확도를 향상시키고 있습니다.

- **Performance Highlights**: 숫자 실험은 스프링-질량 시스템 및 중력이 섭동을 받는 고도로 타원형 궤도에서 수행되었으며, 제안된 방법의 효과성을 입증했습니다. 특히, HNN 기반의 UKF 접근 방식은 우주선 궤도 결정 문제에서 높은 타원 궤도에 대해 효과적인 성능을 보였으며, 이는 장기 예측을 위한 정확하고 견고한 방법의 잠재력을 강조합니다. 이러한 결과는 과학적 연구 및 우주 탐사 분야에서의 향후 활용 가능성을 보여줍니다.
...
- **What's New**: 자율 시스템(autonomous systems)은 날로 증가하는 악의적인 공격자(adversaries)와의 상호작용에서 작동해야 합니다. 본 연구에서는 민감한 정보(sensitive information)를 숨기는 동시에 공격자를 적극적으로 오도하는 기만적 의사결정 프레임워크를 제시합니다. 이는 Markov decision processes(MDP)로 모델링된 자율 시스템에서 상벌 구조(reward functions)를 오도로 복잡한 의사결정을 하도록 합니다.

- **Technical Details**: 우리는 두 가지 기만 전략을 소개합니다: 'diversionary' 기만은 공격자가 시스템의 보상이 무엇인지에 대해 잘못된 결론을 내리도록 이끕니다. 반면 'targeted' 기만은 특정한 잘못된 결론을 유도합니다. 이를 통해 디시전 정책(policy) 최적화 문제에서 각 기만을 효과적으로 구현할 수 있는 방법을 제시합니다.

- **Performance Highlights**: 우리는 시뮬레이션을 통해 두 가지 기만 전략의 효율성을 평가하였으며, 'diversionary' 기만을 사용하여 중요한 에이전트가 가장 덜 중요한 에이전트로 인식되도록 만들면서도 총 누적 보상이 최적의 98.83%에 해당함을 확인했습니다. 또한, 'targeted' 기만은 어떤 미끼(agent)가 가장 중요한 에이전트로 보이게 하면서도 99.25%의 보상을 유지할 수 있음을 시연했습니다.
...
### Bayesian Optimization with Preference Exploration by Monotonic Neural Network Ensemb (https://arxiv.org/abs/2501.18792)
- **What's New**: 이 논문에서는 다중 충돌하는 목표를 가진 실제 블랙박스 최적화 문제를 해결하기 위해 Bayesian Optimization with Preference Exploration (BOPE) 문제에 접근합니다. 기존 연구들이 보지 않았던 유틸리티 함수의 단조성을 활용하여, 신경망 앙상블을 유틸리티 대리 모델로 사용하는 새로운 방법을 제안합니다. 이 접근 방식은 단조성을 자연스럽게 통합하고 두 가지 항목 간 비교 데이터를 지원합니다.

- **Technical Details**: 제안된 방법은 기존 최첨단 기법들과 비교하여 노이즈에 강한 강인성을 보여줍니다. 실험 결과는 단조성이 성능 향상에 중요한 역할을 한다는 사실을 부각시키며, 이를 통해 유틸리티 평가의 노이즈에 강한 성능을 보여줍니다. 또한, 이 방법은 실용적인 환경에서 잘 작동할 수 있도록 설계되었습니다.

- **Performance Highlights**: 실험에서는 제안된 방법이 최신 기술에 비해 뛰어난 성능을 발휘함을 보여줍니다. 특히, 유틸리티 평가의 불확실성을 관리하면서도 효율적으로 최적화를 수행할 수 있는 강점을 지니고 있습니다. 더 나아가, 성능 향상에 있어 단조성이 핵심적인 요인임을 입증했습니다.



### Achieving $\widetilde{\mathcal{O}}(\sqrt{T})$ Regret in Average-Reward POMDPs with Known Observation Models (https://arxiv.org/abs/2501.18790)
- **What's New**: 이 논문에서는 알려진 관찰 모델과 알려지지 않은 전이 모델을 가지는 평균 보상 무한 수평 POMDP 문제를 다루고 있습니다. 기존의 연구들은 서브옵티멀 확률 정책 또는 강한 가정에 의존했으나, 우리는 전이 모델에 대한 유용한 추정 보장을 제공하는 알고리즘을 도입했습니다. 이 알고리즘은 최적의 결정론적 신념 기반 정책 클래스의 이점을 활용하여, 더 나은 성능을 달성할 수 있습니다.

- **Technical Details**: Action-wise OAS라는 절차를 통해 관찰 모델이 알려져 있을 때 전이 모델을 추정하는 기법을 제시합니다. 이 방법은 서로 다른 정책에서 수집된 샘플도 처리할 수 있으며, 각 행위에 대한 전이 행렬의 추정 보장을 제공합니다. 최적의 정책과 비교했을 때, 우리의 알고리즘은 새로운 이론적 결과를 통해 보장된 후회값이 𝒪(√T log T)로서, 기존의 기술을 개선합니다.

- **Performance Highlights**: 제안하는 Action-wise OAS-UCRL 알고리즘은 실험을 통해 기존의 기법들과 비교하여 효율성을 입증했습니다. 우리는 수치 시뮬레이션을 통해 제안한 방법의 효과성을 보여주었고, 이는 평균 보상 POMDP 설정에서 관찰 모델에 대한 기존 지식을 활용할 수 있는 새로운 길을 제시합니다. 이 작업은 실제 시나리오에서의 전략적 의사 결정 문제 해결에 기여할 수 있습니다.



### Navigating the Fragrance space Via Graph Generative Models And Predicting Odors (https://arxiv.org/abs/2501.18777)
- **What's New**: 이 연구는 복잡한 화학 공간과 향기 공간의 탐색에서 효과적인 생성 모델 기법을 제시합니다. 기존의 접근 방식과 달리, 우리는 생성된 분자의 향기 가능성을 예측하고 ROC AUC 점수 0.97을 기록했습니다. 또한, SHAP (SHapley Additive exPlanations) 기법을 활용하여 분자와 향기 간의 관계에 대한 해석 가능성을 보여줍니다. 전체 과정은 분자 생성, 검증, 향기 가능성 선별 및 향기 예측의 네 가지 주요 단계로 구성됩니다.

- **Technical Details**: 모델은 다양한 그래프 생성 모델에 대해 교육되었으며, 여기에는 Graph AutoEncoder (GAE), Variational Graph AutoEncoder (VGAE), Transformer 모델, Diffusion 모델, Adversarially Regularized Graph Autoencoder (ARGA), Adversarially Regularized Graph Variational Autoencoder (ARGVA) 등이 포함됩니다. 모델은 오도르 분자의 커리된 데이터 세트를 기반으로 훈련되었습니다. 각 모델은 분자의 물리화학적 특징을 효과적으로 분석하고 향기 가능성을 예측하는 데 중점을 두고 설계되었습니다. 특히 VGAE는 변량 분포를 이용해 입력을 다루며, KL divergence를 통해 Latent 공간을 정규화합니다.

- **Performance Highlights**: 연구 결과는 MOSES 벤치마크를 통해 생성 모델의 성능이 검증되었습니다. ROC AUC 점수 0.97은 향기 가능성 예측의 높은 정확성을 나타내며, 향기 라벨을 예측하는 정확도가 향상되었습니다. 연구진은 코드와 훈련된 모델을 공개하여 향기 발견 및 후각 연구에서의 보다 넓은 적용 가능성을 기대하고 있습니다. 이러한 접근 방식은 비용 절감과 실험 접근 방식의 시간을 줄이는 데 기여할 것입니다.
...
### Probabilistic Joint Recovery Method for CO$_2$ Plume Monitoring (https://arxiv.org/abs/2501.18761)
- **What's New**: 본 논문에서는 CO₂ 포집 및 저장(Carbon Capture and Storage, CCS) 기술의 유망한 접근 방식인 확률론적 공동 복구 방법(Probabilistic Joint Recovery Method, pJRM)을 제안합니다. 기존의 공동 복구 방법(Joint Recovery Method, JRM)이 가진 불확실성 정량화의 한계를 극복하기 위해, pJRM은 공통의 생성 모델(shared generative model)을 통해 조사 결과에 대한 후행 분포(posterior distributions)를 추정합니다. 이 방법은 CCS 프로젝트에서의 위험 평가를 개선하기 위해 불확실성 정보를 제공합니다.

- **Technical Details**: pJRM은 여러 번 진행된 지진 측정(NN 개)을 입력으로 받아, 재구성된 CO₂ 플룸의 후행 분포를 출력합니다. 이를 위해 각 모니터링 조사에 대해 다양한 잠재 분포를 제공하는 공유 생성 모델(Shared Generative Model, SGM)을 활용합니다. pJRM은 단순히 두 개의 시간 변화를 다루는 예시를 통해 설명하지만, N이 2 이상인 일반화된 프레임워크로 확장될 수 있습니다.

- **Performance Highlights**: pJRM을 사용하면 기존의 독립적으로 풀이된 문제에 비해 성능이 향상됨을 평가할 수 있습니다. 성능을 비교하기 위해 확률적 독립 복구 모델(Probabilistic Independent Recovery Model, pIRM)을 정의하여, 각 시간 변화에 대해 서로 다른 생성 모델을 훈련하는 방식과 대조합니다. pJRM을 통한 계산 비용은 상당하며, 이를 해결하기 위해 외부 루프와 내부 루프를 분리하는 접근 방식을 채택하여 전체적으로 필요한 전방 연산자 값 평가 수를 줄였습니다.
...
### chebgreen: Learning and Interpolating Continuous Empirical Green's Functions from Data (https://arxiv.org/abs/2501.18715)
Comments:
          Code is available at this https URL

- **What's New**: 이 논문은 'chebgreen'이라는 메쉬 독립적이고 데이터 기반의 라이브러리를 소개하며, 알려지지 않은 제어 매개변수를 가진 1차원 시스템을 수학적으로 모델링하는 방법을 제시합니다. 이 방법은 Rational Neural Network 형태로 숨겨진 경계 값 문제에 대한 경험적 그린 함수를 학습합니다.

- **Technical Details**: 이 연구에서 제안된 방법은 관측 데이터를 사용하여 Green’s 함수(Green's Function)를 학습하는 것에 중점을 두고, Chebyshev 급수(chebyshev basis)를 활용한 이차 표현 방식으로 발전합니다. 연구진은 ‘greenlearning’이라는 딥러닝 라이브러리를 사용하여 강제 함수와 시스템 응답 쌍으로 구성된 합성 데이터 세트에서 Green’s 함수를 학습합니다.

- **Performance Highlights**: 논문에서는 학습된 Green’s 함수를 기반으로 기존의 재현 기법을 확장하여, 연속적인 Green’s 함수 표현을 보간(interpolation)하는 방법을 소개합니다. 이를 통해 메쉬에 의존하지 않는 Green's 함수 형식을 확보하며, 고정밀 계산에 적합한 형태로 저장할 수 있게 되었습니다.



### Invisible Traces: Using Hybrid Fingerprinting to identify underlying LLMs in GenAI Apps (https://arxiv.org/abs/2501.18712)
- **What's New**: 이 논문은 대형 언어 모델(LLM)의 핑거프린팅 기술 개발에 중점을 두고 있습니다. 기존의 핑거프린팅 방법은 주로 애플리케이션과의 직접 상호작용에 의존해 모델의 정체성을 추론하는데, 이는 다중 에이전트 시스템, 잦은 모델 업데이트, 모델 내부 정보 접근 제한 등이 있는 실제 환경에서는 한계가 있습니다. 저자들은 정적(static) 및 동적(dynamic) 핑거프린팅 통합 기술을 제안하여 이러한 문제를 해결하고자 합니다.

- **Technical Details**: 제안하는 새로운 핑거프린팅 프레임워크는 아키텍처 특징(architectural features)과 행동 특성(behavioral traits)을 식별하여 동적 환경에서도 LLM의 정확하고 견고한 핑거프린팅을 가능하게 합니다. 논문은 핑거프린팅을 정적 및 동적 두 가지 패러다임으로 나누어, 각 방법론의 강점과 한계를 분석합니다. 저자들은 이 두 가지 패러다임의 장점을 통합한 하이브리드 파이프라인(hybrid pipeline)을 제안하며, 이를 통해 복잡한 실제 시나리오에서 모델 식별의 견고성과 정확성을 향상시킬 수 있음을 보여줍니다.

- **Performance Highlights**: 저자들은 LLM의 다양한 활용 사례에서 제안한 프레임워크의 효과를 검증하기 위해 광범위한 평가 세트를 구성하고 실험 결과를 공유합니다. 실험에 따르면, 이 새로운 접근 방식은 기존의 정적 및 동적 방법들을 형합로 뛰어넘는 성능을 보여주며, LLM의 식별 및 모니터링에서 그 유용성을 입증합니다. 결과적으로 이 프레임워크는 진화하는 배치 환경에 대한 높은 적응성을 지니고 있음을 강조합니다.



### STAN: Smooth Transition Autoregressive Networks (https://arxiv.org/abs/2501.18699)
- **What's New**: 이 논문에서는 전통적인 Smooth Transition Autoregressive (STAR) 모델과 다층 신경망 아키텍처 간의 유사성을 기반으로 한 새로운 접근 방식을 제안합니다. 제안된 신경망 구조는 STAR 프레임워크를 모방하여 여러 층을 활용하여 레짐 전환을 시뮬레이션하고 복잡하고 비선형적인 관계를 캡처합니다. 이 연구는 신경망이 STAR 모델에 대한 강력한 대안을 제공할 수 있으며, 경제 및 재정 예측의 예측 정확성을 향상시킬 수 있는 잠재력을 지니고 있음을 시사합니다.

- **Technical Details**: 이 논문에서는 STAR 모델을 연상시키는 새로운 신경망 아키텍처를 소개합니다. 제안된 모델은 STAR 모델의 해석 가능성과 레짐 스위칭 구조를 깊은 학습의 유연성과 확장성과 결합하였습니다. 모델은 신경망에 부드러운 전환 메커니즘을 직접 임베드하여 시간 시계열 데이터에서 비선형 레짐 종속 동역학을 학습하게 합니다.

- **Performance Highlights**: 제안된 모델은 다양한 데이터 세트에 대해 평가되어 기존의 전통적인 경제학 모형 및 최신 딥러닝 접근 방식과 성능을 비교합니다. 이러한 평가를 통해 아키텍처의 다재다능성과 경제 및 금융 영역에서의 잠재적 응용을 입증하고 있습니다. 또한, 모델의 해석 가능성을 분석하여 부드러운 전환 메커니즘이 데이터의 근본 구조에 대한 통찰력을 제공하는 방법을 보여줍니다.



### Regularized second-order optimization of tensor-network Born machines (https://arxiv.org/abs/2501.18691)
Comments:
          11 pages, 5 figures

- **What's New**: 이 논문에서는 텐서 네트워크 본 머신(TNBM)의 최적화에서 발생하는 여러 문제를 해결하기 위한 개선된 2차 최적화 기법을 제시합니다. 기존의 단일 텐서 로그 최적화 문제는 분석적으로 해결할 수 없기 때문에 반복적인 접근 방식이 필요하고, 이는 수렴 속도를 늦추고 비최적 로컬 미니마에 갇힐 위험을 증가시킵니다. 제안된 방법은 정규화된 상태의 매니폴드에서 수정된 뉴턴 방법을 사용하여 손실 경관을 정규화하여 로컬 미니마 문제를 완화합니다.

- **Technical Details**: TNBM은 비트 문자열에 대한 분포를 설명하는 생성 모델로, 텐서 네트워크의 수학적 프레임워크를 활용합니다. TNBM에서 사용되는 손실 함수는 네거티브 로그 우도(NLL)로, 모델 확률 분포와 목표 분포 샘플 간의 차이를 측정합니다. 우리는 1차원 행렬 곱 상태(MPS)에서 TNBM을 최적화하는 과정에서, 각각의 텐서 코어가 서로 연결되어 있는 구조를 활용하여 이들의 상관 관계를 반영합니다.

- **Performance Highlights**: 제안된 2차 최적화 기법은 MNIST와 같은 이산 데이터셋과 IRIS와 같은 연속 변수 데이터셋에서 빠른 수렴과 낮은 손실 값을 달성함을 보여주었습니다. 기존의 경량 경량 경량 방법보다 더 나은 안정성, 효율성, 일반화 성능을 입증했습니다. 이 연구는 TNBM의 훈련을 개선할 수 있는 새로운 방향을 제시하며, 향후 다양한 데이터 분포에 적용 가능성을 열어줍니다.



### Machine Learning Strategies for Parkinson Tremor Classification Using Wearable Sensor Data (https://arxiv.org/abs/2501.18671)
Comments:
          28 pages, 9 figures, 3 tables, Journal Artificial Intelligence In Medicine

- **What's New**: 이 논문은 파킨슨병의 진단 향상을 위해 머신러닝(Machine Learning, ML) 기법을 리뷰하고 있습니다. 특히 착용 가능 센서 데이터(wearable sensor data)를 활용한 새로운 분류 방법론을 제안합니다. 기존 연구에서 사용된 다양한 ML 모델을 체계적으로 비교 및 분석하며, 효과적인 tremor (진전) 분류를 위한 접근법을 강조합니다.

- **Technical Details**: 본 논문에서는 Support Vector Machines (SVM), Random Forests와 같은 전통적인 ML 기법과 Convolutional Neural Networks (CNN), Long Short-Term Memory networks (LSTM)와 같은 심층 학습(deep learning) 구조까지 다양한 모델을 평가합니다. 각각의 모델이 파킨슨병과 관련된 진전 패턴을 분류하는 데 있어 가지는 강점과 한계도 함께 논의합니다. 다양한 데이터 수집 방법(data acquisition methodologies)과 신호 전처리(signal preprocessing) 기법, 특성 선택(feature selection) 기법을 시간 및 주파수 영역에서 분석합니다.

- **Performance Highlights**: 연구에서는 다양한 ML 기술들의 성능을 비교하여 최선의 tremor 분류 접근법을 제시합니다. 그러나 여전히 해결해야 할 연구의 한계와 도전 과제들이 존재하며, 이는 착용 가능 센서 데이터를 통한 паркинсон병 진단에 적용될 때 발생하는 문제들입니다. 마지막으로, 앞으로의 연구 방향을 제시하며 ML의 파킨슨병 진단 응용을 위한 발전 방향을 제안합니다.
...
### SAFL: Structure-Aware Personalized Federated Learning via Client-Specific Clustering and SCSI-Guided Model Pruning (https://arxiv.org/abs/2501.18659)
- **What's New**: 이 논문은 기존의 Federated Learning (FL) 접근 방식의 한계를 극복하기 위해 SAFL (Structure-Aware Federated Learning)이라는 새로운 프레임워크를 제안합니다. SAFL은 클라이언트의 특정 데이터를 고려하여 클러스터링하고, Similar Client Structure Information (SCSI)를 활용한 모델 프루닝을 통해 개인화된 연합 학습을 향상시킵니다. 이를 통해 데이터 부족으로 인한 부적절한 서브 모델 생성을 방지하고, 더 효과적인 모델 훈련을 지원합니다.

- **Technical Details**: SAFL은 두 단계로 구성된 프로세스를 사용합니다. 첫째, 클라이언트를 데이터 유사성에 따라 그룹화하고, 집계된 프루닝 기준을 통해 최적의 서브 모델을 식별합니다. 둘째, 클라이언트는 이러한 프루닝된 모델을 훈련시키고 서버 기반 집계에 참여하여 각 클라이언트에 맞춤화된 효율적인 모델을 보장합니다.

- **Performance Highlights**: SAFL 방법론을 통해 계산 오버헤드가 크게 줄어들고, 추론 정확도가 향상되었습니다. 실험 결과, SAFL은 모델 크기를 현저히 줄이면서 성능을 개선하여 이질적인 데이터 환경에서 연합 학습에 매우 효과적임을 입증하였습니다.
...
### Beyond Fixed Horizons: A Theoretical Framework for Adaptive Denoising Diffusions (https://arxiv.org/abs/2501.19373)
- **What's New**: 본 논문에서는 새로운 유형의 생성적 확산 모델(generative diffusion models)을 소개합니다. 기존의 denoising diffusion models와는 달리, 우리가 제안하는 모델은 노이즈(noise) 수준에 따라 단계 수가 적응하도록 설계되어 있습니다. 이것은 Doob의 h-transform을 사용하여 진행 과정을 적절한 샘플링 분포에서 종료시킴으로써 가능해집니다.

- **Technical Details**: 우리가 제안한 모델은 데이터의 내재적 차원이 낮을 때 특히 효과적이며, 이 모델은 데이터의 상태에 따라 동적으로 denoising 단계 수를 조정합니다. 이 과정은 랜덤하게 정의된 시간이 종료 기준이 되어, 이는 기존의 고정된 시간 수평선(time horizon)에서 탈피한 것입니다. 또한, 이 방법은 모델이 다양한 다운스트림 작업에 적응할 수 있게 하여, 자연적인 초음파(natural conditioning)가 가능하게 합니다.

- **Performance Highlights**: 이 모델은 다양한 데이터 분포에서 작업을 수행하는 데 필요한 유연성을 제공합니다. 특히 노이즈가 부분적으로 포함된 데이터에서 시작할 수 있게 되며, 이는 분류(classification)와 이상 탐지(anomaly detection) 같은 작업에서 효과적입니다. 또한, 이 모델의 구조는 다양한 응용 분야에서 자연스럽게 적응할 수 있도록 지원하므로 전이 학습(transfer learning)에 강력한 기반을 제공합니다.
...
- **What's New**: 이 논문은 정부 프로그램에서 기계 학습(Machine Learning)의 사용을 통해 가장 취약한 사람들을 식별하고 지원하는 방법에 대해 논의합니다. 기존 정책들과 비교하여 예측의 복지(Welfare) 영향을 분석하고, 독일 주민들의 장기 실업 사례 연구를 통해 이러한 시도가 어떻게 효과적인지를 보여줍니다. 연구 결과는 정책 입안자들이 이러한 시스템을 설계할 때 원칙에 입각한 결정을 내릴 수 있도록 데이터 기반의 도구 및 명확한 분석 프레임워크를 제공합니다.

- **Technical Details**: 연구는 통계적 예측의 설계 결정이 각기 갖는 상대적 가치와 관련된 이론적 및 실증적 기준을 제시합니다. 특히, 개선된 예측이 조직이 가장 불리한 사람들을 식별하는 데 있어 더 높은 한계 이익을 제공하는 경우를 식별합니다. 예측 정확도를 고정하고 행정 역량 확대와 같은 대안적 전략과 비교하여 논의됩니다.

- **Performance Highlights**: 결과에 따르면, 예측 개선의 영향은 일반적으로 선별 능력 확장으로 인해 초과됩니다. 그러나 예측이 결과의 변동성을 거의 모두 또는 전혀 설명하지 않는 경우에는 이러한 관계가 달라질 수 있습니다. 이 연구는 정책 입안자들이 예측 시스템 설계 시 다른 개입과의 상대적 투자 여부를 판단할 수 있는 실용적인 프레임워크를 제공합니다.
...
### Statistical Physics of Deep Neural Networks: Generalization Capability, Beyond the Infinite Width, and Feature Learning (https://arxiv.org/abs/2501.19281)
Comments:
...
- **What's New**: 이 논문은 Deep Neural Networks (DNNs)의 내부 작동 방식을 이해하기 위해 통계역학(Statistical Mechanics)의 통찰을 적용합니다. 연구는 세 가지 상호 보완적인 접근 방식을 통해 DNN의 일반화 능력을 분석하며, 특히 마지막 레이어의 크기만으로 일반화에 대한 경계를 도출합니다.

- **Technical Details**: 첫 번째 접근 방식은 데이터를 평균화하여 마지막 레이어의 크기에만 의존하는 일반화의 비대칭 경계를 제공합니다. 두 번째로, 유한 너비 열역학 한계를 탐구하여, (i) 유한 너비의 단일 은닉층 네트워크의 일반화 오류를 위한 닫힌 형태 표현, (ii) 더 깊은 네트워크의 근사 분할함수, (iii) 열역학 한계에서의 DNN과 Student's t-processes 간의 연결을 제시합니다.

- **Performance Highlights**: 마지막으로, DNN이 제어된 데이터셋과 상호작용하는 방식을 분석하여, 네트워크가 단순히 기억하는 것이 아니라 실제로 데이터 구조를 내재화하는지를 연구합니다. 이 연구는 효과적인 내부 표현을 촉진하는 방법을 이해하는 데 기여하며, 통계 물리학과 기계 학습 간의 시너지 효과를 강조합니다.



### On Pareto Optimality for the Multinomial Logistic Band (https://arxiv.org/abs/2501.19277)
- **What's New**: 본 논문에서는 Multinomial Logit Bandit (MNL-Bandit) 문제를 해결하기 위한 새로운 온라인 학습 알고리즘을 제안합니다. 제안된 방식은 Upper Confidence Bound (UCB) 기반으로, 소비자 선호를 학습하면서 수익을 최대화하는 적응형 결정 정책을 특징으로 합니다. 이 알고리즘은 파레토 최적성(Pareto optimality)을 달성하며, 탐색과 이용 간의 균형을 통해 후회를 최소화하고 MNL 파라미터의 추정 오차를 줄이는 데 초점을 맞추고 있습니다.

- **Technical Details**: MNL-Bandit 문제는 소비자 선호를 동적으로 학습하고 조합적 선택을 관리하는 동적 프레임워크입니다. 본 논문에서는 정보 이론적 경계를 통해 후회와 추정 오차 간의 트레이드오프를 특성화하였고, 강제 탐색(forced exploration) 메커니즘을 도입한 수정 UCB 알고리즘을 제안합니다. 이 알고리즘은 파레토 최적성을 유지하면서 MNL 파라미터의 추정 정확도를 향상시키는 동적 조정을 수행합니다.

- **Performance Highlights**: 제안된 알고리즘은 서브선형 후회(sublinear regret)와 추정 오차 성능을 달성하며, 파레토 프론티어에 점점 가까워지는 성능을 보이는 것으로 입증되었습니다. 이 연구는 적응형 어소르트 최적화에서 최신 기술 수준을 높이며, 복잡한 동적 환경에서 효율적이고 통계적으로 견고한 결정 정책 설계를 위한 엄격한 프레임워크를 제공합니다. 이로 인해 온라인 추천 시스템 및 다른 응용 프로그램에서 사용자 선호에 대한 정확한 학습을 위한 중요한 통찰을 제공합니다.
...
### DINAMO: Dynamic and INterpretable Anomaly MOnitoring for Large-Scale Particle Physics Experiments (https://arxiv.org/abs/2501.19237)
- **What's New**: LHC(대형 강입자 충돌기)의 데이터 수집과 관련하여, DINAMO(Dynamic and INterpretable Anomaly MOnitoring)라는 새로운 프레임워크를 소개합니다. 이 프레임워크는 통계적 표현을 기반으로 데이터의 변화를 모니터링하고, 실제 감지된 이상 징후를 판별해내는 기능을 갖추고 있습니다. 특히 DINAMO는 통계적 및 머신 러닝(Machine Learning) 변형을 동시에 제공하여, 진화하는 히스토그램 템플릿을 통한 자동화된 이상 탐지를 가능하게 합니다.

- **Technical Details**: DINAMO는 두 가지 주요 알고리즘인 DINAMO-S와 DINAMO-ML을 기반으로 합니다. DINAMO-S는 Exponentially Weighted Moving Average(EWMA)의 새로운 일반화를 활용하여 철저하면서도 간편한 통계 기반 접근을 제공합니다. 반면 DINAMO-ML은 Transformer 인코더를 사용하여 과거의 양호한 히스토그램을 결합하고, 더 미세한 시계열 또는 히스토그램 레벨 종속성을 추적하여 더 높은 정확성과 적응성을 보장합니다.

- **Performance Highlights**: 실험 검증 결과, 두 알고리즘 모두 높은 정확도와 적응성을 가지고 있으며, 실제 데이터와의 관련성 또한 입증되었습니다. 특히 DINAMO-S는 LHCb 실험에서 오프라인 DQM(Data Quality Monitoring)에 사용되고 있어, 실용적인 효과를 보여줍니다. 이처럼 DINAMO 프레임워크는 대규모 입자 물리 실험에서 데이터 모니터링의 필수적인 문제를 해결하는 데 크게 기여할 것으로 기대됩니다.



### Fast exact recovery of noisy matrix from few entries: the infinity norm approach (https://arxiv.org/abs/2501.19224)
Comments:
          56 pages, 1 figure

- **What's New**: 이 연구에서는 데이터 복원(matrix recovery) 문제를 다루며, 소음(noise)이 있는 경우에도 정확한 복원을 달성할 수 있는 알고리즘을 제안하고 있습니다. 이전 연구에서는 정확한 복원에 대해 추가적인 스펙트럼 조건을 요구했으나, 본 연구에서는 이러한 조건을 제거하고 단순한 알고리즘을 소개합니다. 이는 노이즈가 있는 상황에서도 행렬의 모든 항목을 정확하게 복원할 수 있다는 것을 보여줍니다.

- **Technical Details**: 주요 가정은 (1) 행렬 A의 랭크(rank)가 매우 작고, (2) A의 특이 벡터(singular vectors)가 비국소화(delocalized)되어 있으며, (3) 샘플 크기가 충분히 큰 경우입니다. 이 연구는 다양하고 복잡한 기존 알고리즘과 달리, 새로운 테크닉인 contour integration을 도입하여 알고리즘을 분석합니다. 이 방식은 이전 방법들과는 완전히 다른 접근법을 사용합니다.

- **Performance Highlights**: 새롭게 개발된 알고리즘은 노이즈 환경에서도 정확한 복원이 가능하다는 강력한 성능을 보여주며, 이는 다양한 실제 응용에 큰 영향을 미칠 수 있습니다. 연구 결과는 추천 시스템, 이미지 복원, 센서 네트워크 기반의 기하학적 재구성 등 여러 분야에서 활용될 수 있는 가능성을 제시합니다. 이 알고리즘은 기존의 기존 모델보다 뛰어난 정확도를 제공하며 향후 연구에도 중요한 기반이 될 것입니다.



### Through the Looking Glass: LLM-Based Analysis of AR/VR Android Applications Privacy Policies (https://arxiv.org/abs/2501.19223)
Comments:
          7 pages; appeared in ICMLA 2024

- **What's New**: 이 논문은 AR/VR 응용 프로그램의 개인 정보 보호 정책을 분석하고 BERT라는 최첨단 텍스트 분류 모델을 활용하여 이러한 정책의 명확성과 포괄성을 평가합니다. 연구는 AR/VR 응용 프로그램과 무료 및 프리미엄 웹사이트의 개인 정보 보호 정책을 비교하여 현재 AR/VR 산업의 개인 정보 보호 관행에 대한 전반적인 관점을 제공합니다. 결과적으로, AR/VR 앱은 무료 콘텐츠보다 긍정적인 측면이 더 많지만 프리미엄 웹사이트에는 미치지 못하는 경향을 보였습니다.

- **Technical Details**: 이 연구는 BERT를 사용하여 AR/VR 응용 프로그램의 개인 정보 보호 정책을 평가하는 방법론을 수립하였습니다. 수집 및 분석된 개인 정보 보호 정책의 데이터셋은 철저하게 큐레이션되어 BERT가 세분화된 정책 요소를 명확히 매핑할 수 있도록 fine-tuning 되었습니다. 이를 통해 AR/VR 애플리케이션 내에서 긍정적인 정책 세그먼트를 평가하고, 다양한 응용 프로그램의 개인 정보 보호 관행을 비교하여 더욱 깊이 있는 통찰을 제공합니다.

- **Performance Highlights**: 연구 결과 우수한 데이터 품질을 유지하면서 302개의 유용한 개인 정보 보호 정책이 수집되었습니다. BERT는 문서의 구조와 장황함을 평균 지표를 통해 분석하며, 서로 다른 응용 프로그램 유형 간의 개인 정보 보호 정책 변화 및 경향을 비교하는 데 기여했습니다. 이 분석은 기술 발전에 따른 개인 정보 보호 관행의 명확성과 효과를 높이는 데 중요한 통찰을 제공합니다.



### A single-loop SPIDER-type stochastic subgradient method for expectation-constrained nonconvex nonsmooth optimization (https://arxiv.org/abs/2501.19214)
Comments:
          Key word: stochastic, subgradient, expectation constraints, weakly convex, fairness constrained classification

- **What's New**: 이 논문에서는 공정성 제약이 있는 복잡한 기대값 제약과 대규모 데이터세트를 다루기 위해 효율적인 확률적 방법을 설계합니다. 기존 연구들은 제약이 없거나 쉽게 예측할 수 있는 제약 또는 결정론적 제약에 초점을 맞추었고, 본 논문에서는 비볼록 비부드러운 확률적 최적화 문제에 대해 새로운 정확한 패널티 모델을 제시합니다. 이를 통해 기대값 제약을 포함한 비볼록 최적화 문제를 해결하고, 미분 가능한 함수와 제약 함수의 서브그래디언트를 활용하는 새로운 방법을 개발합니다.

- **Technical Details**: 제안된 방법은 단일 루프 SPIDER 타입의 확률적 서브그래디언트 방법으로, 각 반복마다 목적 함수와 제약 함수의 서브그래디언트를 함께 사용합니다. 우리는 특정 정규성 조건 하에 패널티 문제의 근사해를 찾기 위한 반복 복잡도가 O(ε^{-4})임을 증명하고, 이는 기존 연구에서의 하한에 부합합니다. 정확한 패널리제이션 기법을 바탕으로 원래 문제의 (ε, ε)-KKT 포인트를 획득합니다.

- **Performance Highlights**: 제안된 방법은 두 가지 공정성 제약 문제가 있을 때, 최신 알고리즘에 비해 최대 466배 더 빠른 성능을 보였습니다. 우리의 접근법은 목적 샘플 서브그래디언트나 제약 샘플 함수 값의 복잡도를 기존 방법보다 ε^{-2}만큼 더 낮출 수 있습니다. 이러한 성능 향상은 대규모 데이터 세트를 다루는 응용 프로그램에 있어 중요한 기여를 제공합니다.



### Learning While Repositioning in On-Demand Vehicle Sharing Networks (https://arxiv.org/abs/2501.19208)
- **What's New**: 본 논문은 일방향 요금제 차량 공유 서비스의 네트워크 인벤토리 문제를 다룹니다. 차량 반환과 수요의 불확실성, 그리고 고정된 렌탈 유닛 수로 인해 공급과 수요를 매칭하기 위해 차량 재배치의 최적 정책을 찾는 것이 어렵습니다. 저자들은 최적 기초 재고 재배치 정책(best base-stock repositioning policy)을 제안하고, 이를 일반적인 네트워크 구조에서 비대칭 최적성을 증명합니다.

- **Technical Details**: 재배치 문제는 Markov Decision Process (MDP) 모델을 사용하여 n차원 공간 인벤토리를 포착하는 간단한 모델로 수립되었습니다. 점검 완료된 정보를 바탕으로 다양한 알고리즘적 접근 방식을 제안하며, 비선형 구조에서의 제약 조건을 상쇄하기 위해 Mild cost-structure 가정하에 오프라인 문제를 선형 프로그램으로 재구성할 수 있음을 보여줍니다.

- **Performance Highlights**: 온라인 환경에서 저자들은 Lipshitz-bandits repositioning 알고리즘을 제안하며, 이는 거래가 진행되는 동안 수익성 보장을 제공합니다. 최적 재배치 알고리즘인 Online Gradient Repositioning (OGR)은 취합된 수요 데이터만 필요로 하며, O(n^2.5√T)의 최적 회귀를 달성하였습니다. 실험 결과 제시된 방법의 효율성이 입증되었습니다.



### Learning Sheaf Laplacian Optimizing Restriction Maps (https://arxiv.org/abs/2501.19207)
Comments:
          Proc. 58th Annual Asilomar Conference on Signals, Systems, and Computers (Asilomar), Pacific Grove, CA, Oct. 27 - Oct. 30, 2024

- **What's New**: 본 논문은 그래프의 노드에서 관찰된 데이터 세트를 바탕으로 그래프의 토폴로지와 제한 맵(restriction maps)을 포함하는 sheaf Laplacian을 유추하는 새로운 프레임워크를 제안합니다. 제안된 방법은 그래프 신호 처리(graph signal processing, GSP)의 중요한 일반화로서의 sheaf 이론에 기초하고 있습니다. 관찰된 데이터의 총 변동성을 최소화하고 각 엣지에서 국소적으로 변동성을 최적화하는 것을 목표로 하는 학습 문제에 관해 다루고 있습니다.

- **Technical Details**: 이 연구에서 우리는 cell complexes에 대한 cellular sheaves 이론의 기본 개념을 검토하고, 각 셀에 벡터 공간을 할당하는 방법과 셀 간의 관계를 선형 변환으로 정의하는 방법을 다룹니다. 특히, sheaf Laplacian 연산자는 불연속적인 Hodge Laplacian 개념을 확장하며, 지역적 관계에서 전역적 속성을 추출하는 데 핵심적인 연산자입니다. 이를 통해 실험 데이터에서 sheaf Laplacian을 추론하는 과정은 실용적인 문제에 대한 sheaf 이론 도구의 적용에 있어 매우 중요한 단계라고 할 수 있습니다.

- **Performance Highlights**: 본 논문에서 제안된 알고리즘은 그래프의 구조를 추론하고 제한 맵을 최적화하는 과정에서 우수한 수치 효율성을 보여줍니다. 모든 기본 단계가 닫힌 형태로 해결되기 때문에 매우 효율적입니다. 실험을 통해 제안된 방법은 제한 맵을 최적화하지 않는 전통적인 그래프 기반 방법에 비해 높은 잠재력을 가지고 있음을 입증합니다.
...
### Learning Non-Local Molecular Interactions via Equivariant Local Representations and Charge Equilibration (https://arxiv.org/abs/2501.19179)
- **What's New**: 이번 연구에서는 Charge Equilibration Layer for Long-range Interactions (CELLI)를 제안하여 기존의 Message-passing Neural Networks (MPNNs)가 갖는 지역적 한계를 극복하고 비지역적 상호작용을 정확하게 모델링하려 합니다. CELLI는 네 번째 세대 고차원 신경망(4GHDNN) 개념을 일반화하여, 전하 평형화(charge equilibration, Qeq) 방법을 통합함으로써 MLP의 정확성과 효율성을 동시에 증진시킵니다. 이러한 새로운 아키텍처는 지역적 상호작용뿐만 아니라 비지역적 상호작용과 전하 이전 현상을 모델링할 수 있는 가능성을 보여줍니다.

- **Technical Details**: CELLI 아키텍처는 MPNNs의 구조를 개선하여 불필요한 계산 비용을 줄이면서 더욱 정확한 에너지 및 힘 예측을 가능하게 합니다. 이 연구에서 도입된 Qeq 방법은 시스템의 전자적 환경에 따라 전하를 재분배하는 과정을 포함하여, 긴 범위 상호작용의 모델링으로 MLP의 적용 범위를 확장하는 데 기여합니다. 특히, CELLI는 Allegro와 같이 E(3)-등가성이 있는 접근 방식을 제공하며, 다양한 화학적 환경에서의 일반화를 통해 GNN 잠재력을 극대화합니다.

- **Performance Highlights**: CELLI는 기존 Allegro 아키텍처를 약 2배의 계산 효율성으로 확장하여 높은 비지역적 상호작용과 전하 이전을 모델링합니다. 여러 기준 사례에서 실험 결과, CELLI는 MPNNs와 비슷한 정확도를 달성하면서도 연산 효율성을 크게 개선하였습니다. 이러한 발전은 복잡한 분자 시스템에 대한 시뮬레이션의 가능성을 확대하고, 실용적인 화학 응용 분야로의 전이를 돕는데 기여할 것으로 기대됩니다.
...
### FL-APU: A Software Architecture to Ease Practical Implementation of Cross-Silo Federated Learning (https://arxiv.org/abs/2501.19091)
- **What's New**: 이번 논문에서는 Federated Learning (FL)의 실제 사용을 촉진하기 위한 시나리오 기반 아키텍처를 제안합니다. 다양한 기업들이 협력하여 머신러닝 모델의 품질을 향상시키는 크로스-사일로(cross-silo) 응용 프로그램에 초점을 맞추었으며, 신뢰할 수 있는 참가자 만이 참여할 수 있는 환경을 조성합니다. 이를 통해 거버넌스(governance)와 인증(authentication)의 결합, 학습 과정 추적 등을 통해 실질적인 FL의 응용을 이끌어내고자 합니다.

- **Technical Details**: 제안된 아키텍처는 FL 서버와 FL 클라이언트로 구성되며, 운영 시스템의 다양한 역할을 정의합니다. FL 서버 관리자는 전체 FL 프로세스를 모니터링하고, FL 참가자는 학습 및 프로세스 파라미터에 대한 의사결정에 참여하며, FL 클라이언트 관리자는 클라이언트의 운영을 감독합니다. 이러한 역할 정의는 FL 시스템의 효율성을 높이고, 다양한 산업 환경에 맞춘 맞춤형 아키텍처 설계를 가능하게 합니다.

- **Performance Highlights**: 제안된 아키텍처는 크로스-사일로 환경에서 여러 기업 간의 협업과 의사결정 과정을 투명하게 관리할 수 있는 기능을 강조합니다. 학습 모델의 저장 및 추적, 보안과 프라이버시 관련 요구 사항을 지속적으로 충족하면서 모든 참가자가 공정한 이익을 공유할 수 있도록 합니다. 시나리오 기반의 분석 방법을 통해 실제 FL 사용에 대한 요구사항을 평가하고, 이 아키텍처의 유용성을 검증하였습니다.



### SpikingSoft: A Spiking Neuron Controller for Bio-inspired Locomotion with Soft Snake Robots (https://arxiv.org/abs/2501.19072)
Comments:
          8th IEEE-RAS International Conference on Soft Robotics

- **What's New**: 이번 연구는 생물의 운동 신경망 및 물리적 탄성에서 영감을 받아 부드러운 뱀 로봇이 저수준 스파이킹 신경 메커니즘을 통해 이동 동작을 생성할 수 있는 가능성을 탐구합니다. 새로운 Double Threshold Spiking neuron 모델을 도입하여 출력 패턴을 조절함으로써 부드러운 뱀의 자연 동역학을 자극하고, 신경 임계값을 변경하는 것으로 단순하게 특별한 움직임을 생성할 수 있습니다. 이로 인해 강화 학습(Reinforcement Learning)과 자연스럽게 결합되는 SpikingSoft 접근 방식이 구현되었습니다.

- **Technical Details**: 이 연구는 Leaky Integrate-and-Fire (LIF) 신경 모델을 활용하고, 이에 대한 한계를 극복하기 위해 Double Threshold Spiking (DTS) 모델을 제안합니다. DTS 모델에서는 신경 세포의 막 전위(u)가 시간에 따라 축적되고, 임계값에 도달했을 때 스파이크가 발생합니다. 조정 가능한 두 개의 임계값을 통해 다양한 출력 스파이크 패턴을 생성하여 로봇의 복잡한 움직임 제어를 가능하게 합니다.

- **Performance Highlights**: 제안된 SpikingSoft 컨트롤러는 부드러운 뱀 로봇의 목표 달성 성공률을 21.6% 증가시키고, 목표 도달 시간을 29% 단축하며, 전통적인 강화 학습 제어기들보다 더 매끄럽고 효율적인 움직임을 구현했습니다. 이는 뱀 로봇이 생물 모사형 보행을 수행하고, 복잡한 운동 패턴을 생성하는 데 있어 중요한 진전을 나타냅니다.



### Machine Learning in Gamma Astronomy (https://arxiv.org/abs/2501.19064)
Comments:
          20 pages, 5 figures, Proceedings of The 8th International Conference on Deep Learning in Computational Physics, June 19-21, 2024, Moscow, Russia

- **What's New**: 본 논문은 Imaging Atmospheric Cherenkov Telescopes (IACT) 데이터를 분석하기 위해 사용되는 최신 딥러닝(methods) 방법들에 대한 리뷰를 제공합니다. 특히, IACT 데이터에서 감마선(gamma-ray)과 우주선(cosmic-ray) 이벤트를 분리하고 이들의 특성(reconstruction) 분석에 관한 연구들로 구성되어 있습니다. 기존 문헌과 함께 최근 출판된 논문들이 포함되어 있어 이 분야의 최신 동향을 이해하는 데 도움이 됩니다.

- **Technical Details**: 이 논문에서는 딥러닝 기술을 사용하여 감마선과 우주선 이벤트를 구별하고 재구성하는 방법에 대해 논의합니다. Monte Carlo(MC) 시뮬레이션을 통해 CORSIKA 패키지를 사용하여 EAS(Extensive Air Showers) 시뮬레이션을 수행하고, ResNet50과 Inception V3과 같은 두 가지 유명한 신경망(neural network) 아키텍처를 적용하여 성능 비교를 합니다. 딥러닝을 활용한 이미지 분석 기술들은 CNN(convolutional neural networks)과 RNN(recurrent neural networks)의 통합 사용을 강조합니다.

- **Performance Highlights**: Inception V3 아키텍처는 0.1에서 100 TeV 범위의 에너지에서 81.4%에서 91.6%까지 정확도를 증가시켰습니다. 합성모드(stereoscopic mode)에서 여러 망원경의 데이터를 동시에 사용하는 방법이 EAS의 주입자(primary particle) 특성을 더욱 잘 재구성할 수 있도록 합니다. 또한, CRNN(convolutional recurrent neural networks) 방법을 통해 배경을 20-25% 더 효과적으로 제거할 수 있음을 보여줍니다.
...
### Conformal Prediction in Hierarchical Classification (https://arxiv.org/abs/2501.19038)
- **What's New**: 이번 연구에서는 split conformal prediction 프레임워크를 계층적 분류(hierarchical classification) 문제로 확장하였습니다. 특히, 예측 세트를 사전 정의된 계층의 내부 노드에 제한할 수 있는 알고리즘을 제안하였으며, 두 개의 고효율 추론 알고리즘을 개발했습니다. 첫 번째 알고리즘은 내부 노드를 예측 세트로 반환하고, 두 번째 알고리즘은 표현 복잡성(representation complexity)의 개념을 사용하여 이 한계를 완화하여 보다 일반적이고 조합적인 추론 문제를 만듭니다.

- **Technical Details**: 계층적 다중 클래스 분류(multi-class classification)에서는 예측한 클래스 레이블에 대한 불확실성이 존재할 수 있습니다. 이러한 경우, 특정 내부 노드에 국한되지 않고 더 많은 클래스의 집합을 예측할 수 있는 set-valued 예측이 유용할 수 있습니다. 본 연구에서는 계층적 분류를 위한 새로운 추론 알고리즘을 제안하며, 각 알고리즘은 표본의 분포에 관계없이 유한 샘플 보증을 갖추고 있습니다. 특히, 내부 노드 몇 개만을 후보 세트로 반환하는 방법과 더 많은 내부 노드 집합을 예측할 수 있는 방법이 각각 제시됩니다.

- **Performance Highlights**: 제안된 알고리즘들은 여러 기준 데이터셋(benchmark datasets)에서 평가되었으며, 명목적 커버리지(nominal coverage) 달성에 있어 효과적인 것으로 입증되었습니다. 첫 번째 알고리즘은 r=1의 제한된 경우에 최적화되어 있으며, 두 번째 알고리즘은 r>1에 대한 확장을 제공합니다. 연구 결과, 두 알고리즘 모두 예측 세트의 유효성을 보장하며, 효율성 또한 높임을 보여주었습니다.



### Quantum SMOTE with Angular Outliers: Redefining Minority Class Handling (https://arxiv.org/abs/2501.19001)
- **What's New**: 새로운 Quantum-SMOTEV2 방법은 업그레이드된 Quantum-SMOTE 변형으로, K-Means 클러스터링 없이 클래스 불균형 문제를 해결합니다. 이 방법은 단일 데이터 중심을 중심으로 데이터 샘플을 생성하며, 소수 클래스 데이터 포인트의 각도 분포와 Angular Outliers (AOL) 개념에 중점을 둡니다. 실험 결과, 중간 수준의 SMOTE(30-36%)에서 모델 성능이 현저히 개선되었으며, 이는 기존 방법보다 효과적입니다.

- **Technical Details**: Quantum-SMOTEV2는 기존 Quantum-SMOTE에서는 필요했던 클러스터링 단계를 제거하고, 데이터 집합의 단일 중심으로부터 소수 데이터 포인트의 각도를 분석합니다. 이 방법은 Quantum-SWAP 테스트와 양자 회전 회로를 사용하여 합성 데이터 포인트를 생성하며, 각도 범위를 벗어난 outlier 데이터를 식별합니다. 이를 통해 각도 outlier의 특성을 활용하여 모델의 강건성을 높입니다.

- **Performance Highlights**: 공공 Cell-to-Cell Telecom 데이터셋을 통해 Random Forest (RF), K-Nearest Neighbours (KNN) Classifier, Neural Network (NN)와 같은 다양한 분류 알고리즘에서 성능을 평가했습니다. Angular Outliers를 통합함으로써 정확도, F1 Score, AUC-ROC, AUC-PR 등의 분류 지표가 모드 높게 개선되었습니다. 이 연구는 Quantum-SMOTEV2가 극단적인 경우에 모델 성능을 향상시킬 수 있음을 보여줍니다.
...
### Optimal Transport-based Conformal Prediction (https://arxiv.org/abs/2501.18991)
- **What's New**: 이 논문에서는 Conformal Prediction (CP)의 새로운 절차를 소개합니다. 기존 방법들이 다변량 출력의 기하학적 구조를 완전히 활용하지 못하는 문제를 해결하기 위해, 최적 수송(optimal transport)의 관점을 적용하여 다변량 점수 함수(multivariate score functions)를 처리합니다. 이를 통해 유연하고 비볼록(non-convex) 형태의 예측 영역을 구축할 수 있습니다.

- **Technical Details**: 특히, 우리는 Monge-Kantorovich 벡터 순위(vector ranks)와 분위수(quantiles)를 사용하여 예측 지역을 생성합니다. 이 방법은 전통적인 CP 방법과 유사한 유한 샘플(finite-sample) 분포 자유(distribution-free) 커버리지 보장(properties)을 제공합니다. 또한, 다출력 회귀(multi-output regression)와 다중 클래스 분류(multiclass classification)를 위해 우리의 방법을 조정하고, 점근적 조건부 커버리지 보장(asymptotic conditional coverage guarantees)을 갖는 적응형 예측 지역을 생성하는 간단한 조정을 제안합니다.

- **Performance Highlights**: 논문에서는 실제 회귀(regression)와 분류(classification) 문제에 대해 제안된 방법을 평가하고, (조건부) 커버리지 및 효율성 측면에서 장점을 보여줍니다. 결과적으로, 이는 다변량 학습 과제에서 복잡한 불확실성 패턴을 보다 나은 방식으로 포착할 수 있음을 나타냅니다.



### Optimizing Through Change: Bounds and Recommendations for Time-Varying Bayesian Optimization Algorithms (https://arxiv.org/abs/2501.18963)
- **What's New**: 이 논문은 시간 변동이 있는 Bayesian Optimization (TVBO) 알고리즘의 누적 후회(cumulative regret)에 대한 이론적 경계를 제시합니다. 특히, 이 논문은 이러한 알고리즘이 가졌던 비현실적인 가정 없이, 시간에 따라 변하는 목표 함수의 성질을 반영한 후회 경계를 제공하는 최초의 연구로 자리잡고 있습니다. 또한, 실험을 통해 제안된 알고리즘 BOLT가 기존의 TVBO 알고리즘들보다 우수한 성능을 보임을 나타냅니다.

- **Technical Details**: TVBO 알고리즘은 Gaussian Process (GP)를 사용하여 목표 함수 f에 대한 대리 모델을 구축합니다. 이러한 대리 모델은 관측 노이즈를 포함하고 있으며, n개의 관측 데이터에 기반한 사전 GP 모델로부터 후험 모델을 형성합니다. 이 논문은 TVBO의 성능과 response time R(n) 간의 관계를 분석하여, 이러한 관계성을 접목한 알고리즘 BOLT를 제안합니다.

- **Performance Highlights**: 실험 결과, BOLT는 기존의 TVBO 최첨단 알고리즘들에 비해 우수한 성능을 보여주었습니다. 특히, 다양한 합성 및 실제 문제에서 BOLT의 효율성을 입증하였습니다. 본 연구는 TVBO 알고리즘의 성능 개선을 위해 후회 분석을 활용하는 새로운 방향성을 제시하고 있습니다.



### Solving Inverse Problem for Multi-armed Bandits via Convex Optimization (https://arxiv.org/abs/2501.18945)
- **What's New**: 이번 연구에서는 다중 무장 강도(Multi-Armed Bandits, MAB) 문제의 역문제(Inverse Problem of Multi-Armed Bandits, IMAB)에 대해 다룹니다. IMAB는 생물학적 행동 모델링에 널리 사용되지만, 일반적으로 비볼록(non-convex) 문제임을 보여주고, 변수를 변환함으로써 볼록(convex) 문제로 완화할 수 있다는 점이 흥미롭습니다. 또한, IMAB 문제를 해결하기 위한 두 단계의 순차적 휴리스틱(Sequential Heuristic) 방법을 제안합니다.

- **Technical Details**: 제안된 방법은 첫 번째 단계에서는 비볼록 문제를 볼록 문제로 변환하고, 두 번째 단계에서는 효율적인 계산을 위한 조건을 규명합니다. 이 과정에서 제공되는 인증서(certificates)를 통해 IMAB 문제에 대한 글로벌 솔루션(global solution)을 얻을 수 있습니다. 구현은 CVXPY를 기반으로 하여, 볼록 최적화(convex optimization)에 대한 경험이 없는 사용자도 쉽게 사용할 수 있도록 설계되었습니다.

- **Performance Highlights**: 수치 실험 결과에 따르면, 우리의 휴리스틱 방법은 반복적인 지역 최적화(local optimization) 방법에 비해 훨씬 더 견고하며, 몬테카를로 방법(Monte Carlo methods)에 가까운 성능을 훨씬 단축된 실행 시간 내에 달성할 수 있음을 보여줍니다. 이러한 결과는 IMAB 문제 해결을 위한 보다 효율적인 접근 방식을 제공합니다.



### Trustworthy Evaluation of Generative AI Models (https://arxiv.org/abs/2501.18897)
Comments:
          5 figures, 1 table, 15 pages

- **What's New**: 본 논문은 생성 모델(Generative Model)의 평가에서 불확실성 정량화(Uncertainty Quantification)를 고려한 새로운 방법론을 제안합니다. 이를 통해 두 개의 생성 모델 간의 성능 차이를 비편향 추정기를 통해 비교할 수 있습니다. 본 연구는 기존의 평가 방식들이 가지고 있는 한계를 극복하고, 통계적 신뢰성을 바탕으로 보다 정확한 결과를 도출할 수 있는 프레임워크로 자리매김하고 있습니다.

- **Technical Details**: 제안된 방법에서는 상대 성능(relative performance)을 평가하기 위해 첫 번째 순서 U-통계량(first-order U-statistic)을 사용합니다. 이 추정기는 파라메트릭 수렴 속도(parametric convergence rate)와 비대칭 정상성(asymptotic normality)을 충족하여 유효한 추론을 가능하게 합니다. 또한, 이 방법은 병렬 컴퓨팅(parallel computing)과 중간 결과를 사전 저장(pre-storing)함으로써 컴퓨팅 효율성(computational efficiency)을 더욱 높일 수 있습니다.

- **Performance Highlights**: 모의 데이터셋(simulated datasets)에서는 우리의 접근 방식을 통해 신뢰 가능한 신뢰구간(confidence intervals)을 구성할 수 있음을 드러냈으며, 상기된 기존 추정기들이 올바른 커버리지 비율(coverage rate)을 달성하지 못한 것을 보였습니다. 실제 이미지 데이터셋(CIFAR10)에서도 우리의 방법이 확산 모델(diffusion models)을 평가하는 데 효과적이라는 것을 보여주었으며, 본 방법은 기존 메트릭보다 우수한 통계적 정확성을 제공합니다.



### Can We Predict the Effect of Prompts? (https://arxiv.org/abs/2501.18883)
Comments:
          5 pages, 3 figures

- **What's New**: 본 논문에서 제안된 Syntactic Prevalence Analyzer (SPA)는 Large Language Models (LLMs)의 프롬프트를 분석하고 그 효과를 예측하는 도구로, 예측 프롬프트 분석(predictive prompt analysis)이라는 새로운 개념을 제시합니다. SPA는 성능 손실 없이 프롬프트를 신속하게 분석하여 LLM이 해당 프롬프트에 어떻게 반응할지를 예측할 수 있습니다. 또한 SPA는 코드 생성을 위한 타겟 신택틱 구조를 생성하는 데 유용하며, 이는 코드 합성 작업에서의 LLM 사용을 크게 쉽게 만듭니다.

- **Technical Details**: 이 연구에서는 Sparse Autoencoders (SAEs)를 기반으로 한 SPA를 설계하여, 사용자가 요청한 신택틱 구조가 LLM에 의해 얼마나 잘 생성될지를 예측합니다. SPA는 입력으로 타겟 신택틱 구조의 자연어 설명과 여러 가지 프롬프트를 받아들여, 이 두 가지를 통해 프롬프트의 효과를 측정합니다. 실험 결과 SPA는 최대 0.994의 피어슨 상관관계를 보여주며, LLM을 실행하는 데 소요되는 시간의 단 0.4%만으로 예측 분석을 수행할 수 있음을 입증합니다.

- **Performance Highlights**: SPA는 신택틱 구조 생성에 대한 프롬프트의 효과를 예측하는 데 있어 매우 높은 정확도를 자랑합니다. 실제로 SPA의 예측 결과는 각 프롬프트에 대한 신택틱 구조의 발생 빈도와 밀접한 상관관계를 보였으며, 이는 LLM 사용 중 발생하는 계산 비용을 크게 줄일 수 있음을 의미합니다. 이러한 기술은 LLM을 활용한 소프트웨어 개발의 효율성을 높이고, 연구자와 실무자 모두에게 유용하게 적용될 수 있는 가능성을 지니고 있습니다.
...
### QMe14S, A Comprehensive and Efficient Spectral Dataset for Small Organic Molecules (https://arxiv.org/abs/2501.18876)
Comments:
          11 pages, 4figures

- **What's New**: 이 연구에서는 QMe14S 데이터셋을 소개합니다. 이 데이터셋은 186,102개의 소형 유기 분자로, 14가지 원소(H, B, C, N, O, F, Al, Si, P, S, Cl, As, Se, Br)와 47개의 기능 그룹이 포함되어 있습니다. QMe14S는 분자 시뮬레이션을 위한 포괄적이고 효율적인 데이터셋으로 자리매김합니다.

- **Technical Details**: QMe14S 데이터셋의 생성에는 B3LYP/TZVP 수준의 밀도 함수 이론(Density Functional Theory)이 사용되었습니다. 여기서는 원자 구조 최적화와 에너지, 원자 전하, 원자력, 쌍극 모멘트 등 여러 물리적 성질이 계산되었습니다. 또한, 고유한 IR, 라만 및 NMR 스펙트럼을 생성하기 위해 파동 함수 기반 시뮬레이션도 수행되었습니다.

- **Performance Highlights**: DetaNet이라는 E(3)-불변 메시지 전파 신경망을 이용하여 QMe14S 데이터셋으로 훈련된 모델은 기존의 QM9S 데이터셋을 사용한 모델보다 분자 스펙트럼 시뮬레이션에서 더 뛰어난 성능을 보였습니다. 이러한 결과는 QMe14S 데이터셋이 분자 구조와 성질 간의 관계에 대한 귀중한 통찰력을 제공함을 보여줍니다.
...
### Adaptivity and Convergence of Probability Flow ODEs in Diffusion Generative Models (https://arxiv.org/abs/2501.18863)
- **What's New**: 이 논문은 확률 흐름 ODE(Ordinary Differential Equation) 샘플러의 수렴 이론에 대한 새로운 이론적 보장을 제공합니다. 기존의 연구들은 일반적인 수렴 이론에 초점을 맞추었으나, 자연 이미지 데이터에서 적게 자주 나타나는 저차원 구조에 적응할 수 있는지에 대한 의문은 여전히 남아 있었습니다. 본 연구에서는 정확한 점수 함수 추정을 통해, 확률 흐름 ODE 샘플러가 총 변동 거리(Total Variation Distance)에서 O(k/T)의 수렴 속도를 성취할 수 있음을 보여줍니다.

- **Technical Details**: 확률 흐름 ODE 샘플러는 데이터의 목표 분포에서 생성된 데이터를 점진적으로 잃어버리게 하여 노이즈로 변환한 후, 이 노이즈로부터 샘플을 재구성하는 방식으로 작동합니다. 샘플러의 수렴 속도는 k는 목표 분포의 본질적 차원, T는 반복 횟수를 나타내며, 기존 연구 결과에 비해 저차원 구조를 최대한 활용한 점이 특징입니다. 이러한 수렴 속도는 일반적으로 더 큰 환경 차원에 따라 스케일링되는 기존 결과보다 개선된 것입니다.

- **Performance Highlights**: 본 연구는 확률 흐름 ODE 샘플러가 자연 이미지와 같은 저차원 구조를 포함하는 데이터 분포에서 더욱 빠른 샘플링을 수행할 수 있는 능력을 강조합니다. 또한, 기존 DDPM(Denoising Diffusion Probabilistic Models) 및 DDIM(Denoising Diffusion Implicit Models) 샘플러들과의 비교를 통해, 이 방법론이 실질적인 성능 개선에 기여할 수 있음을 입증합니다. 이 논문은 딥러닝과 생성 모델링 분야에서 확률 기반 접근법의 중요한 발전을 제시합니다.
...
### Beyond Short Steps in Frank-Wolfe Algorithms (https://arxiv.org/abs/2501.18773)
- **What's New**: 본 연구에서는 전통적인 짧은 스텝을 넘어서 함수 부드러움을 활용하여 Frank-Wolfe 알고리즘을 향상시키는 새로운 기법을 소개합니다. 특히, 프라이멀-듀얼 보증을 포함한 스텝 사이즈로 최적화 문제를 해결하는 Frank-Wolfe 알고리즘을 제안하고, 이를 위한 실용적인 종료 기준도 제공합니다.

- **Technical Details**: 이 알고리즘은 최적화 기법에서의 경량성을 유지하면서도, 고급 프라이멀-듀얼 분석을 통해 수렴 속도를 강화하는 방법론을 채택합니다. 프라이멀-듀얼 짧은 스텝 전략을 도입하여 카운터 프로그래시브 기법을 통해 프라이멀-듀얼 갭을 최적화할 뿐만 아니라, 그 방법은 그래디언트 하강 알고리즘에도 적용 가능하다는 점이 흥미롭습니다.

- **Performance Highlights**: 실험 결과에서 제안된 낙관적 Frank-Wolfe 알고리즘이 기존 방법들보다 뛰어난 성능을 보였으며, 이는 실용적인 장점을 강조합니다. 특히, 여러 실험에서 기존 방법보다 프라이멀-듀얼 갭을 더 효율적으로 최소화하는 경향을 보여주었습니다.



### A Unified Framework for Entropy Search and Expected Improvement in Bayesian Optimization (https://arxiv.org/abs/2501.18756)
- **What's New**: 이번 연구에서는 Variational Entropy Search (VES)라는 새로운 이론적 프레임워크를 소개하며, Expected Improvement (EI)와 정보 이론적 취득 함수 간의 관계를 새롭게 조명합니다. EI가 Max-value Entropy Search (MES)라는 정보 이론적 취득 함수의 변형으로 해석될 수 있음을 보이며, 두 가지 방법론이 본질적으로 더 밀접하게 연결되어 있음을 증명합니다. 이를 통해 VES-Gamma라는 새로운 취득 함수가 EI와 MES의 장점을 효과적으로 결합할 수 있음을 제안합니다.

- **Technical Details**: 연구의 배경에서 Gaussian Process (GP)는 데이터의 확률적 모델링을 위한 중요한 도구로 언급됩니다. Gaussian Process는 특정한 커널 함수에 의해 정의되며, 주어진 관측값을 바탕으로 새로운 포인트에서의 예측값과 공분산을 계산할 수 있습니다. VES-Gamma는 이러한 이론을 바탕으로 EI의 성능을 유지하면서 정보 이론적 고려사항을 통합하여 설계되었습니다.

- **Performance Highlights**: VES-Gamma는 저차원 및 고차원 합성 데이터와 실제 데이터 세트를 포함한 광범위한 평가에서 종합적으로 실험되었습니다. 그 결과, VES-Gamma는 최신의 취득 함수들과 경쟁할 수 있는 성능을 보였으며, 많은 경우 EI와 MES를 초월하는 결과를 보여주었습니다. 이러한 성능 개선은 다양한 최적화 문제에 효과적으로 적응할 수 있는 가능성을 제시합니다.



### STaleX: A Spatiotemporal-Aware Adaptive Auto-scaling Framework for Microservices (https://arxiv.org/abs/2501.18734)
- **What's New**: 이 논문에서는 마이크로서비스 기반 아키텍처의 자율 스케일링(auto-scaling) 문제를 해결하기 위해 STaleX라는 적응형 프레임워크를 소개합니다. 기존의 집중형(auto-scaling) 방법이 각 서비스의 고유한 성능 요구사항을 다루는데 부족하다는 점을 지적하고, 각 마이크로서비스에 개별 스케일러가 필요하다는 것을 강조합니다. STaleX는 제어 이론(Control Theory), 머신러닝(Machine Learning), 그리고 휴리스틱(Heuristics)을 통합하여 자원의 실시간 조정을 가능하게 합니다.

- **Technical Details**: 제안된 STaleX 프레임워크는 각 서비스에 대해 가중치가 조정되는 비례-적분-미분(PID) 제어기를 사용합니다. 이때 가중치는 시공간(spatiotemporal) 특성을 통합하여 조정되는 감독 유닛(supervisory unit)에 의해 지속적으로 모니터링됩니다. STaleX는 서비스 사양과 서로 간의 의존성 뿐만 아니라, 작업부하의 시간변화에 따른 자원 할당을 최적화합니다.

- **Performance Highlights**: Kubernetes 클러스터에 배포된 마이크로서비스 기반 시연 애플리케이션을 통해, STaleX 프레임워크는 기존의 Kubernetes Horizontal Pod Autoscaler (HPA)와 비교하여 성능 개선과 비용 절감을 입증하였습니다. 연구 결과, STaleX는 자원 사용량을 26.9% 줄이면서 SLO 준수를 효과적으로 향상시키는 것으로 나타났습니다.
...
### Combining physics-based and data-driven models: advancing the frontiers of research with Scientific Machine Learning (https://arxiv.org/abs/2501.18708)
Comments:
          127pages. Accepted for publication in Mathematical Models and Methods in Applied Sciences (M3AS)

- **What's New**: 지난 몇 년간 과학적 기계 학습(Scientific Machine Learning, SciML)은 물리학 기반 모델과 데이터 기반 모델을 통합하여 미분 문제의 수치적 근사화를 위한 새로운 연구 분야로 떠올랐습니다. SciML은 기존의 물리적 이해를 바탕으로 하는 모델의 장점과 데이터 기반 알고리즘의 효율성을 동시에 활용하여 복잡하고 비선형적인 패턴을 발견할 수 있습니다.

- **Technical Details**: SciML에서는 물리학적 지식과 수학적 지식을 기계 학습 알고리즘에 주입할 수 있으며, 데이터 기반 모델은 복잡한 패턴을 탐색하여 물리학 기반 모델의 설명 능력을 향상시킬 수 있습니다. 이 논문에서는 디지털 모델링과 기계 학습 알고리즘의 수학적 기초를 기억하고, 가장 널리 사용되는 기계 학습 아키텍처를 소개합니다.

- **Performance Highlights**: SciML의 다양한 전략을 통해 부분 미분 방정식으로 제어되는 복잡한 문제를 해결할 수 있는 큰 잠재력을 논의하였습니다. 특히 심장 기능 시뮬레이션에 SciML을 성공적으로 적용하였으며, 이는 사회 경제적으로 중요한 분야로 많은 수학적 및 계산적 도전 과제를 가지고 있습니다. 데이터 기반 모델을 활용하여 심장 세포에 대한 구성 법칙과 심근 물질의 특성을 밝혀내고, 계산 복잡성을 극복하기 위한 효율적인 축소 차원 모델을 개발하는 데 성공하였습니다.
...
### Constructing Cell-type Taxonomy by Optimal Transport with Relaxed Marginal Constraints (https://arxiv.org/abs/2501.18650)
- **What's New**: 최근에 등장한 single-cell 데이터는 세포 수준에서 다양한 생물학적 조건을 연구하는 데 기여하고 있습니다. 그러나 모든 샘플에서 기초적인 상관 관계를 구축하기 위한 새로운 접근법이 필요합니다. 이 논문은 Optimal Transport와 Relaxed Marginal Constraints(OT-RMC) 기법을 조합하여 셀 클러스터의 분류 체계를 구축하는 방법을 제안합니다. 이를 통해 다수의 샘플에 걸쳐 클러스터를 동시에 정렬할 수 있습니다.

- **Technical Details**: 이 연구의 핵심 기법은 OT-RMC를 활용하여 서로 다른 샘플 간 클러스터 비율의 차이나 일부 클러스터가 전체 샘플에 존재하지 않을 때 발생하는 문제를 해결하는 것입니다. 연구자는 20개 이상의 데이터셋을 사용한 실험에서, 새로운 시스템으로 구축된 세포 유형 분류 체계가 높은 정확도로 클러스터를 주석을 붙일 수 있음을 보여주었습니다. 또한, 이 체계를 기반으로 샘플 수준의 특징을 추출하여 샘플 분류를 더욱 효과적으로 수행할 수 있습니다.

- **Performance Highlights**: 논문에서 제안한 방법은 기존의 방법들보다 더 높은 성능을 보입니다. 특히, 여러 데이터셋에서 합성된 정보를 기반으로 하여 더 많은 정보를 포함하고 있으며, 이는 클러스터의 일관된 주석을 통해 하위 분석에서 유용성을 제공합니다. 최종적으로 이 방법은 다양한 생물학적 조건에 대한 샘플 분류의 정확성을 향상시키는 데 기여할 것으로 기대됩니다.
...
### The Relationship Between Network Similarity and Transferability of Adversarial Attacks (https://arxiv.org/abs/2501.18629)
- **What's New**: 본 연구는 신경망의 유사성이 적대적 공격의 성공률에 미치는 영향을 분석하여 새로운 방어 메커니즘을 제안하는 것을 목표로 하고 있습니다. 특히, 다양한 CNN 아키텍처 간의 유사성을 측정하는 Centered Kernel Alignment (CKA) 지표를 활용하여 기존 네트워크와 비교할 수 있는 방법을 제공합니다. 이러한 방법론을 통해 네트워크 설계자들은 자신들의 네트워크의 취약성을 더 잘 이해할 수 있을 것으로 기대됩니다.

- **Technical Details**: 연구에서는 Convolutional Neural Networks (CNN) 간의 유사성과 적대적 공격의 성공률 간의 복잡한 관계를 조사했습니다. DenseNet과 같은 복잡한 모델은 구조적 복잡성으로 인해 lower similarity scores를 보였고, 기본적인 레이어(예: DataParallel, Dropout, Conv2d)에 대한 유사성이 가장 높았습니다. 본 연구에서는 DecisionTreeRegressor를 사용하여 black-box 및 Carlini & Wagner 공격에 대한 성공률을 90% 이상으로 예측할 수 있음을 보여주었습니다.

- **Performance Highlights**: 적대적 공격의 성공률은 전이 공격에 대해 다소 일관된 결과를 보였지만, 특정 전이 공격의 경우 큰 변동성을 나타냈습니다. 복잡한 네트워크는 더 취약한 경향이 있으며, 이러한 점은 다양한 데이터 하위 집합에서의 결과 변동성을 통해 강조되었습니다. 이러한 발견은 다양한 공격 시나리오와 네트워크 아키텍처에서의 일반화를 위한 추가 연구가 필요함을 시사합니다.
...
- **What's New**: 본 논문에서는 새로운 백도어 공격 방식인 DarkMind를 소개합니다. 이 공격은 맞춤형 LLM의 추론 능력을 이용하며, 기존 공격과 달리 사용자 질의에 트리거를 주입하지 않고 작동하여 숨겨진 위협이 됩니다. 이러한 특성 때문에 DarkMind는 맞춤형 AI 솔루션의 급증과 함께 등장하는 새로운 취약점을 지적하고 있습니다.

- **Technical Details**: DarkMind는 추론 체인(Chain-of-Thought) 내에서 활성화되어 최종 결과를 비밀리에 변경할 수 있는 구조로 설계되었습니다. 우리는 이 공격 방식을 수학적, 상식, 기호 추론 영역을 포함하는 8개의 데이터셋을 활용하여 평가하였으며, 다섯 가지 최첨단 LLM과 다섯 가지 별도의 트리거 구현을 사용하였습니다. 이를 통해 DarkMind의 효과성을 입증하였습니다.

- **Performance Highlights**: DarkMind 공격은 모든 평가 시나리오에서 효과적으로 작용했으며, 맞춤형 LLM의 추론 프로세스에 미치는 영향을 강조했습니다. 이러한 결과는 공격 방어 필요성을 제기하며, 향후 보다 강력한 보안 조치를 취할 필요성을 시사합니다. 마지막으로, 우리는 DarkMind의 위험을 완화하기 위한 잠재적인 방어 메커니즘을 탐구하였습니다.

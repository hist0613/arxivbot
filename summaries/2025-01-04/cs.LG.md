...
- **What's New**: 이 논문에서는 label noise(레이블 노이즈)에 대한 robust(강건한) deep learning classifiers(딥러닝 분류기)의 개발 방법을 제안합니다. 레이블 노이즈는 모델의 정확성을 저하시킬 수 있는 실제 적용에서 흔히 발생하는 현상으로, 이로 인한 영향 조정이 중요합니다. 연구에서는 transition matrices(전환 행렬)를 이용한 forward correction(정방향 보정) 및 importance reweighting(중요도 재가중치)과 같은 세 가지 접근 방법을 소개합니다.

- **Technical Details**: 제안하는 방법은 두 가지 주요 전략을 기반으로 합니다. 첫 번째는 이미 알려진 transition matrices를 활용하여 noise(노이즈)를 정정하고, 두 번째는 anchor point(앵커 포인트) 가정을 사용하여 noise matrix를 추정하는 것입니다. 또한, T-Revision 방법의 성능을 개선하여 T-Revision-Alpha 및 T-Revision-Softmax를 제안해 안정성과 강건성을 높였습니다. 이를 통해 multi-layer perceptron (MLP)과 ResNet-18 모델을 구현하였습니다.

- **Performance Highlights**: FashionMINIST 데이터셋에서는 알려진 전환 행렬을 기반으로 clean labels(깨끗한 레이블)를 예측하고 transition matrices을 추정하는 성능을 비교하였습니다. CIFAR-10 데이터셋에서는 noise matrix를 추정하고 제안한 방법의 clean label 예측 능력을 평가했습니다. 실험 결과 모든 방법이 레이블 노이즈에 대해 높은 강건성을 나타내었고, T-Revision 방법이 특히 우수한 성능을 보였습니다.
...
### Machine Learning for Modeling Wireless Radio Metrics with Crowdsourced Data and Local Environment Features (https://arxiv.org/abs/2501.01344)
Comments:
          6 pages, 12 figures

- **What's New**: 이번 논문에서는 4G 환경에서 RSRP, RSRQ, RSSI와 같은 무선 라디오 메트릭을 모델링하기 위해 설계된 CRC-ML-Radio Metrics라는 머신러닝 모델 제품군을 소개합니다. 이 모델은 크라우드소싱된 데이터와 지역 환경 특성을 통합하여 예측 정확도를 개선합니다.

- **Technical Details**: 모델은 실내 및 고도, 야외 도시 환경에서의 성능을 최적화하기 위해 설계되었습니다. 300,000개 이상의 데이터 포인트를 기반으로 평가된 결과들에서, RSRP는 9.76에서 11.69 dB, RSRQ는 2.90에서 3.23 dB, RSSI는 9.50에서 10.36 dB의 RMSE 성능을 보여주었습니다.

- **Performance Highlights**: 이 연구 결과는 모델의 견고성과 적응성을 입증하며, 복잡한 캐나다 도시 환경에서의 네트워크 계획 및 서비스 품질 최적화에 기여할 수 있음을 보여줍니다. 다양한 환경에서의 효과적인 예측을 통해, 더 나은 통신 품질을 제공할 수 있는 가능성을 제시합니다.



### Simultaneous Latent State Estimation and Latent Linear Dynamics Discovery from Image Observations (https://arxiv.org/abs/2501.01339)
- **What's New**: 이 보고서는 이미지 기반 관측을 고려하여 잠재 상태 추정 문제를 해결하기 위한 새로운 솔루션을 제안하고 있습니다. 기존 알고리즘을 정리하여 다양한 필터링 기술을 통해 노이즈가 포함된 관찰 결과로부터 후방 필터링 분포를 도출하는 방법에 관한 연구를 진행하였습니다. 특히, 고차원 관측 정확도를 높이기 위한 필터링 알고리즘과 관측 모델의 이해를 심화시키고자 합니다.

- **Technical Details**: 보고서는 필터링 알고리즘의 요소와 위에서 소개된 시스템에 적용할 수 있는 방법을 다룹니다. 동적 시스템은 통계적 비선형 함수로 구성되며, 관측 모델은 고차원 데이터인 이미지에서 정보를 추출하는 역할을 합니다. 여기에 사용되는 수학적 프레임워크는 베이지안 필터와 확장 칼만 필터(Extended Kalman Filter, EKF)와 같은 다양한 필터링 테크닉을 포함하고 있습니다.

- **Performance Highlights**: 주목할 만한 성능은 고차원 관측을 기반으로 한 상태 추정의 정확성을 대폭 향상시킨 것입니다. 필터링 알고리즘의 적용을 통해, 고차원 데이터에서 실제 시스템의 내부 상태를 효과적으로 회복할 수 있는 방법을 제시하고 있습니다. 최근의 로봇 공학 응용 사례에서 이러한 접근 방식이 특히 유용한 결과를 보여주고 있습니다.
...
### Optimized Relay Lens Design For High-Resolution Image Transmission In Military Target Detection Systems (https://arxiv.org/abs/2501.01287)
- **What's New**: 이 논문에서는 군사 광학 시스템에서 표적 탐지와 추적을 위한 고성능 이미지 전송을 제공합니다. 연구의 주요 초점은 ZEMAX 소프트웨어를 사용하여 설계하고 최적화한 릴레이 렌즈 시스템의 광학 성능에 대한 상세한 분석입니다. 이 연구는 특히 군사 응용 프로그램에서 높은 해상도와 낮은 왜곡을 가지고 있음을 보여줍니다.

- **Technical Details**: 이 논문에서는 모듈레이션 전송 함수(Modulation Transfer Function, MTF), 스폿 다이어그램(spot diagrams), 사이델 다이어그램(Seidel diagram), 필드 곡률(field curvature) 및 왜곡(distortion)과 같은 중요한 광학 특성을 평가하고 분석합니다. ZEMAX 소프트웨어를 통해 최적화된 릴레이 렌즈 시스템은 선명하고 손실 없는 이미지 전송을 가능하게 합니다.

- **Performance Highlights**: 연구 결과는 해당 릴레이 렌즈가 고해상도의 표적 탐지와 추적을 위한 기능을 갖추고 있음을 나타냅니다. 특히, 긴 거리에서도 높은 품질의 이미지를 전송할 수 있어 군사 작전에서의 활용도가 높습니다. 따라서, 이 렌즈 시스템은 군사 응용 프로그램에서 매우 중요한 역할을 할 것으로 예상됩니다.



### Bayesian Active Learning By Distribution Disagreemen (https://arxiv.org/abs/2501.01248)
- **What's New**: 이 연구에서는 Normalizing Flows를 이용한 회귀(Regression) 문제에서 Active Learning(AL) 방법론을 제안합니다. 제안된 BALSA(Bayesian Active Learning by Distribution Disagreement) 알고리즘은 기존 BALD 알고리즘을 활용하여, 예측 분포를 직접 활용함으로써 불확실성 추정(Uncertainty Quantification)에 대한 새로운 접근을 제공합니다. 또한, 기존의 AL에서 일반적으로 사용되는 휴리스틱(Heuristics) 방법들이 Normalizing Flows에 잘 작동하지 않는다는 점을 강조합니다.

- **Technical Details**: 연구에서는 Gaussian Neural Networks(GNN)와 Normalizing Flows를 회귀 문제에 적용하며, 이들 모델이 제공하는 예측 분포를 기반으로 하는 여러 Acquisition Function을 설계합니다. AL의 효과적인 구현을 위해, 모델이 가진 두 종류의 불확실성(aleatoric과 epistemic)을 구분하는 것이 핵심입니다. 최근 여러 데이터셋과 쿼리 크기를 갖춘 실험을 통해, BALSA 알고리즘의 성능을 평가하고 기존 SOTA(State of the Art) 알고리즘과의 비교를 진행했습니다.

- **Performance Highlights**: BALSA 알고리즘은 4개의 데이터셋과 2개의 아키텍처에서 SOTA 결과를 달성했습니다. 연구를 통해 제안된 알고리즘이 불확실성 추정과 AL의 응용에 있어 실질적으로 기여할 수 있음을 입증했습니다. 이를 통해 데이터 수집 및 학습 효율을 높여주는 새로운 알고리즘의 가능성을 제시합니다.



### High-Order Tensor Regression in Sparse Convolutional Neural Networks (https://arxiv.org/abs/2501.01239)
Comments:
          14 pages, 1 algorithm

- **What's New**: 본 논문은 전통적인 기계 학습 방법론과는 근본적으로 다른 일반적인 합성곱(convolution) 접근 방식을 제시합니다. 수학적 측면에서 이 접근 방식은 고차 텐서(high-order tensors)와 관련하여 매우 간단하고 유리한 것으로 입증되었습니다. 이 연구의 주된 초점은 희소 합성곱 신경망(sparse convolutional neural networks)을 위한 합리적인 회귀 이론을 개발하는 것입니다.

- **Technical Details**: 연구는 실수 힐베르트 공간(real Hilbert space) 내에서만 작업하며, 이러한 공간들은 실수 스칼라에 의해 정의되는 완전한 내적 벡터 공간입니다. 이 논문은 규칙적인 텐서(tensor)의 생성 및 활용을 다루며, 각 텐서는 주어진 벡터 공간의 구조에 따라 자기 자신의 좌표로 구성됩니다. 사용되는 텐서의 차원과 관련하여, 일반적으로 텐서의 차원(order)이 1인 경우 벡터(vector)로 간주됩니다.

- **Performance Highlights**: 이 접근 방식은 기존의 역전파 알고리즘(Backpropagation Algorithm)에 대한 새로운 정의를 제시하여, 텐서 기반(tensor-based) 접근 방식 및 희소성(sparsity) 개념에 조화를 이룹니다. 이 결과는 신경망의 성능을 개선하고 특정 형태의 데이터에 대한 더 나은 일반화를 가능하게 합니다. 고차 텐서를 통한 이론적 틀의 개발로 인해 다양한 기계 학습 응용에 확장될 수 있는 가능성도 열려 있습니다.



### Modeling Multi-Task Model Merging as Adaptive Projective Gradient Descen (https://arxiv.org/abs/2501.01230)
- **What's New**: 이 논문은 여러 전문가 모델을 병합하는 새로운 접근 방식을 제안하며, 기존 방법들이 모델 병합의 근본 요건을 간과하고 있다는 점을 강조합니다. 특히, 기존 방법들은 개별 모델의 성능과 비교하여 병합된 모델의 성능을 확보하는 데 실패하고 있음을 지적합니다. 저자들은 모델 병합을 제약 최적화 문제로 재구성하고, 적응형 사영 경량 하강법(adaptive projective gradient descent)을 통해 이를 해결하는 방법을 제안합니다.

- **Technical Details**: 저자들은 병합된 모델의 손실 함수를 분해하고 재구성하여 개별 모델과 정렬하도록 함으로써, 데이터가 없는(task-free) 최적화를 통해 충돌을 완화합니다. 이 과정에서, 모든 작업을 포함하는 공유 하위공간(shared subspace)을 기반으로 최적화 문제를 설정하고, 병합 계수를 작업에 민감한 학습률로 간주하여 정의하는 방식을 사용합니다. 이러한 작업 벡터의 최적화 과정은 각 작업의 손실을 최소화하면서도 공유된 지식을 유지할 수 있도록 설계되었습니다.

- **Performance Highlights**: 저자들이 제안한 방법은 다양한 비전 및 자연어 처리(NLP) 작업에서 기존 방법들을 상회하는 성과를 보여주었습니다. 실험 결과, Task Arithmetic(TA) 대비 최대 11.6%, AdaMerging 대비 5.8% 향상을 달성했으며, 단순 작업 인식 λ라는 방법만으로도 2.8%의 성능 개선을 기록했습니다. 이러한 성과는 비탄력적인(task-unseen) 설정에서도 일관되게 나타났으며, 각 구성 요소의 메커니즘을 명확히 하는 광범위한 실험이 실시되었습니다.



### Comparative Analysis of Topic Modeling Techniques on ATSB Text Narratives Using Natural Language Processing (https://arxiv.org/abs/2501.01227)
Comments:
          conference paper

- **What's New**: 이 논문은 항공 안전 분석에서 사고 보고서에 포함된 방대한 텍스트 데이터를 활용하기 위한 혁신적인 기법들을 연구합니다. 특히, 네 가지 주요 주제 모델링 기법인 Probabilistic Latent Semantic Analysis (pLSA), Latent Semantic Analysis (LSA), Latent Dirichlet Allocation (LDA), Non-negative Matrix Factorization (NMF)를 적용하여 호주 교통안전국(ATSB) 데이터셋의 항공 사건 서사를 분석합니다. 이 연구는 안전 전문가들이 실행 가능한 통찰력을 얻기 위한 체계적인 접근 방식을 제공하는 데 중점을 두고 있습니다.

- **Technical Details**: 논문에서는 각 기법이 데이터내의 잠재적인 주제 구조를 드러내는 능력을 평가합니다. pLSA, LSA, LDA, NMF와 같은 기법들은 서로 다른 방법론을 적용하여 데이터에서 얻는 통찰력을 비교합니다. 이러한 기법들은 텍스트에서 유의미한 패턴을 추출하고, 이를 통해 더욱 효과적인 안전 분석을 가능하게 합니다.

- **Performance Highlights**: 비교 분석을 통해 이 연구는 다양한 주제 모델링 기법들이 항공 안전 분야에서 잠재력을 가지고 있음을 보여줍니다. 또한 각 기법의 고유한 장점과 한계를 설명함으로써, 향후 연구 설계와 실무 적용에 있어 중요한 방향을 제시합니다. 이러한 통찰력은 항공 안전을 개선하는 데 있어 의미 있는 기여를 할 것으로 기대됩니다.



### Classification of Operational Records in Aviation Using Deep Learning Approaches (https://arxiv.org/abs/2501.01222)
Comments:
          conference paper; aviation safety, NLP, DL, operational record classification, Socrata

- **What's New**: 이 연구는 항공 산업의 안전성을 보장하기 위해 4종의 딥러닝 모델(DP)의 성능을 평가하였습니다. 사용된 모델은 Bidirectional Long Short-Term Memory (BLSTM), Convolutional Neural Networks (CNN), Long Short-Term Memory (LSTM), Simple Recurrent Neural Networks (sRNN)입니다. 이들은 Socrata 항공 데이터셋을 활용하여 상업, 군사, 개인 범주에서 다중 분류 작업을 수행하였습니다.

- **Technical Details**: 모델의 성능은 분류 보고서(classification report), 혼동 행렬(confusion matrix) 분석, 정확도 메트릭(accuracy metrics), 검증 손실(validation loss), 정확도 곡선(accuracy curves)을 통해 평가되었습니다. 결과적으로 BLSTM은 72%의 정확도로 가장 우수한 성능을 보였으며, LSTM은 71%로 상업 클래스에서의 재현율(recall)에서 두각을 나타냈습니다. CNN과 sRNN은 각각 67%와 69%의 정확도로 낮은 성능을 보였으며, 특히 개인 클래스에서의 오분류가 두드러졌습니다.

- **Performance Highlights**: 이 연구는 BLSTM과 LSTM 모델이 순차적 의존성(sequential dependencies) 및 복잡한 분류 작업에서 특히 우수한 성능을 보였다는 점을 강조합니다. 그러나 모든 모델은 군사 및 개인 범주 예측에서 클래스 불균형(class imbalance) 문제에 직면하였습니다. 향후 데이터 증강(data augmentation), 고급 특성 엔지니어링(feature engineering), 앙상블 학습(ensemble learning) 기법을 통해 분류 정확도 및 강인성을 개선할 수 있는 가능성을 제시합니다.



### TabTreeFormer: Tree Augmented Tabular Data Generation using Transformers (https://arxiv.org/abs/2501.01216)
- **What's New**: 본 논문은 TabTreeFormer라는 하이브리드 트랜스포머 아키텍처를 제안합니다. 이는 트리 기반 모델을 통합하여 표의 특성을 반영하는 inductive biases를 보존하고 데이터 신뢰도와 유용성을 높입니다. 또한, 이 모델은 이중 양자화(tokenization)를 적용하여 숫자 값의 분포를 모델링하고, 어휘 크기와 시퀀스 길이를 감소시킵니다.

- **Technical Details**: TabTreeFormer는 표의 비선형성과 잠재적으로 낮은 상관관계를 포착하기 위한 트리 기반 모델을 사용합니다. 또한, K-Means 양자화를 이용한 첫 번째 단계와 정량화를 적용하여 숫자 값을 정밀하게 표현하는 두 번째 양자화 단계를 도입합니다. 이러한 설계를 통해 모델 크기를 크게 줄이고, 효율성을 보장합니다.

- **Performance Highlights**: TabTreeFormer는 10개의 데이터셋에서 다양한 생성 모델과 비교 평가하였으며, 신뢰도, 유용성, 개인 정보 보호 및 효율성 측면에서 우수한 성능을 보였습니다. 특히, 최상의 모델은 기존 모델 크기보다 1/16의 크기로 40%의 유용성 향상을 달성하였습니다.



### Empirical Analysis of Nature-Inspired Algorithms for Autism Spectrum Disorder Detection Using 3D Video Datas (https://arxiv.org/abs/2501.01202)
- **What's New**: 이번 논문에서는 3차원 보행 비디오 데이터를 활용하여 자폐 스펙트럼 장애(ASD)를 탐지하기 위한 새로운 방법론을 제안합니다. 이 방법론은 감독 학습(supervised learning) 기법과 자연에서 영감을 받은 최적화 알고리즘을 결합하여 데이터셋에서 중요한 특징(feature)을 추출합니다. 특이하게도, 초기 주요 파티클을 선정하여 계산 시간을 줄이고 총 효율성을 높이는 방안도 포함했습니다.

- **Technical Details**: 제안된 방법론은 감독 학습 알고리즘을 사용하여 ASD 환자를 분류하며, 자연에서 영감을 받은 최적화 알고리즘을 통해 특징 추출을 수행합니다. 다양한 조합의 학습 알고리즘과 최적화 알고리즘을 통해 분류 정확도 100%를 달성한 사례를 보여줍니다. 3D 보행 비디오 데이터는 인간의 기본 기술을 나타내며, ASD 탐지에 중요한 역할을 할 수 있습니다.

- **Performance Highlights**: 연구 결과, 제안된 방법론은 ASD 탐지에서 뛰어난 성능을 보이며, 의료 및 학문 분야에 중요한 기여를 할 것으로 기대됩니다. 기존 연구들과의 비교를 통해 높은 정확도와 짧은 계산 시간을 달성하였는데, 이는 조기 진단 및 치료에 있어 중요한 요소입니다. 이런 성과는 향후 ASD 진단 관련 연구에서 더욱 발전할 수 있는 기반을 제공합니다.



### Machine Learning-Based Prediction of ICU Readmissions in Intracerebral Hemorrhage Patients: Insights from the MIMIC Databases (https://arxiv.org/abs/2501.01183)
- **What's New**: 이 연구는 중증 뇌출혈(Intracerebral hemorrhage, ICH) 환자의 중환자실(ICU) 재입원 위험을 예측하기 위해 기계 학습(machine learning) 모델을 개발하였습니다. MIMIC-III 및 MIMIC-IV 데이터베이스에서 포괄적인 임상 및 인구학적 데이터를 활용하여 모델 정확성을 높이고자 했습니다. 이 연구의 결과는 임상 결정 지원과 자원 최적화에 기여할 수 있는 예측 프레임워크를 제공합니다.

- **Technical Details**: 연구에서 사용된 데이터는 MIMIC-III와 MIMIC-IV 데이터베이스에서 추출되었습니다. ICH 환자는 ICD-9 및 ICD-10 분류를 기반으로 식별되었으며, 최종 데이터 세트는 871명과 1,445명이 포함되었습니다. 특징 선택은 문헌 검토와 전문가의 의견을 기반으로 하여 RFE(Recursive Feature Elimination) 방법으로 진행되어, 최종적으로 12개의 주요 특징이 도출되었습니다.

- **Performance Highlights**: 개발된 기계 학습 모델은 AUROC(Area Under Receiver Operating Characteristic), 정확도, 민감도 및 특이성과 같은 여러 메트릭을 사용하여 평가되었습니다. 모델은 ICH 환자의 ICU 재입원 예측에서 우수한 정확성을 보여주었으며, 주요 예측 인자로는 인구 통계 정보, 임상 매개 변수 및 실험실 측정량이 포함되었습니다.
...
### An Inclusive Theoretical Framework of Robust Supervised Contrastive Loss against Label Nois (https://arxiv.org/abs/2501.01130)
- **What's New**: 본 논문은 레이블 노이즈에 대한 강력한 감독 대조 손실을 설계하기 위한 통합 이론적 프레임워크를 제안합니다. 특히, 무작위 대조 손실에 대한 일반적인 강력 조건을 도출하여 감독 대조 손실이 레이블 노이즈에 대해 이론적으로 강건한지 검증하는 기준을 제시했습니다. 기존의 InfoNCE 손실이 실제로 강건하지 않음을 보여주고, 이를 기반으로 Symmetric InfoNCE (SymNCE)라는 강력한 버전을 개발하였습니다.

- **Technical Details**: 제안된 이론적 프레임워크는 기계 학습의 레이블 노이즈 문제를 다루기 위한 기존의 방법론과 강력한 대조 학습 간의 관계를 명확히 합니다. 문헌에서의 연구들은 샘플 선택 전략에 의존하고 있으나, 본 연구는 일반적인 강력 조건을 제시하여 모든 대조 손실의 강건성을 검증할 수 있는 통일된 이해를 가능하게 합니다. 이러한 이론적 발견은 과거의 강력한 기법들에 대한 설명을 제공하고, 새로운 강력 손실 함수를 고안하는 데 영감을 줍니다.

- **Performance Highlights**: 실험 결과, SymNCE는 레이블 노이즈가 있는 상황에서도 기존의 감독 대조 학습 방법에 비해 뛰어난 성능을 보였습니다. CIFAR-10과 CIFAR-100 벤치마크 데이터셋에서, SymNCE는 대조 학습의 기존 방법들에 비해 직접적으로 성능 개선을 보여주었습니다. 일반적인 상황에서도 SymNCE는 다른 최첨단 강력 손실 함수들과 비교하여 우수한 성능을 나타내며, 레이블 노이즈에 대한 저항력을 증명하였습니다.



### Graph2text or Graph2token: A Perspective of Large Language Models for Graph Learning (https://arxiv.org/abs/2501.01124)
- **What's New**: 이번 논문에서는 그래프를 텍스트로 변환하여 대규모 언어 모델(Large Language Models, LLMs)을 활용하는 새로운 접근 방법인 LLM4graph에 대해 자세히 살펴봅니다. 기존 그래프 모델링의 한계를 극복하기 위해, 이 연구는 Graph2text 및 Graph2token 두 가지 패러다임으로 기존 방법론을 분류하였습니다. 이 방법론은 그래프 구조를 텍스트로 변환하여 LLM이 이해할 수 있도록 하며, 그래프 데이터의 자연어 처리의 가능성을 제시합니다.

- **Technical Details**: LLM4graph의 핵심은 그래프를 LLM이 이해하고 분석할 수 있는 텍스트로 변환하는 것입니다. 그래프는 본질적으로 비정형 데이터인데, 이는 노드 간의 이질적 관계때문입니다. 이 논문에서는 특히 그래프에서 발생하는 네 가지 주요 문제점인 정렬(alignment), 위치(position), 다중 수준의 의미(multi-level semantics), 맥락(contexts)에 대해 논의합니다. 이러한 문제들을 해결하기 위해 연구자들에게 적절한 모델 및 LLM 선택에 대한 지침을 제시합니다.

- **Performance Highlights**: LLMs는 그래프 표현 학습 성능을 크게 향상시킬 수 있는 능력을 가지고 있습니다. LLM을 통해 그래프 구조를 텍스트로 변환하면, 구조적 및 의미적 정보를 동시에 담아낼 수 있어 후속 작업에서 성능을 극대화할 수 있습니다. 이 연구는 LLM4graph의 최신 연구 방향을 제시하며, 연구자들이 LLM 기반 그래프 학습 기술을 더 효과적으로 활용할 수 있도록 돕기 위한 실용적인 지침과 미래 연구 방향을 제공합니다.
...
### Long-range Brain Graph Transformer (https://arxiv.org/abs/2501.01100)
- **What's New**: 이번 논문에서는 브레인 네트워크(Brain networks)의 분석을 개선하기 위해 Adaptive Long-range aware TransformER(ALTER)라는 새로운 방법론을 제안합니다. ALTER는 브레인 ROIs(Regions of Interest) 간의 장거리 종속성을 포착하기 위해 편향된 랜덤워크(biased random walk)를 사용하여 개발되었습니다. 이 방법은 브레인 네트워크 내의 통신 연결성을 명확하게 구조화하여 보다 통합된 이해를 제공합니다.

- **Technical Details**: ALTER는 구체적으로 Adaptive Long-ranGe Aware(ALGA) 전략을 기반으로 하여 브레인 ROIs 간의 장거리 종속성을 포착합니다. 이 전략은 BRIs 간의 상관관계를 계산하여 다음 스텝을 결정하는 데 더 높은 상관값을 가진 연결을 선택하는 방식으로 동작합니다. 이를 통해 ALTER는 변동하는 통신 강도에 따라 랜덤워크 샘플링을 수행할 수 있으며, transformer 프레임워크에 장거리 임베딩을 통합하여 짧은 거리 및 긴 거리의 종속성을 모두 포착합니다.

- **Performance Highlights**: ALTER는 ABIDE 및 ADNI 데이터 세트에서 다른 최신 그래프 학습 방법들보다 일관되게 우수한 성능을 보였습니다. 특히, 브레인 네트워크 분석 방법들, 특히 FBNETGEN, BrainNetGNN, BrainGNN 등과 비교할 때 높은 정확도와 신뢰성을 보여주었습니다. 논문은 장거리 종속성의 사례를 제시하여 ALTER의 효용성을 명확하게 입증합니다.
...
### Noise-Resilient Symbolic Regression with Dynamic Gating Reinforcement Learning (https://arxiv.org/abs/2501.01085)
Comments:
          15 pages, 2 figures, accepted by AAAI 2025

- **What's New**: 이 논문에서는 Noise-Resilient Symbolic Regression (NRSR)라는 혁신적인 방법을 제안합니다. 이 새로운 접근법은 강화학습(RL)을 활용하여 고잡음(high-noise) 데이터에서 심볼릭 표현(symbolic expression)을 복구하는 능력을 가지고 있습니다. NRSR은 노이즈 저항형 게이팅 모듈(Noise-Resilient Gating Module, NGM)을 설계하여 의미 없는 정보를 동적으로 필터링할 수 있는 기능을 제공합니다.

- **Technical Details**: NRSR은 RL과 NGM을 결합하여 고잡음 데이터 속의 유용한 정보를 추출합니다. NGM은 높은 노이즈 데이터에서 비효율적인 변수들을 제거하며 탐색 능력을 향상시키기 위한 혼합 경로 엔트로피(Mixed Path Entropy, MPE) 보너스 항을 디자인했습니다. 이러한 구조는 기호 선택 정책을 학습하고, RL 과정에서 모델이 과적합(overfitting)되는 것을 방지합니다.

- **Performance Highlights**: 실험 결과, NRSR은 고잡음 데이터에 대한 여러 기준선(baselines)보다 현저하게 우수한 성능을 나타내었습니다. 또한, NRSR은 깨끗한 데이터에서도 기존의 최신 기술(state-of-the-art, sota) 성능을 달성하여, 심볼릭 회귀(symbolic regression) 작업에서의 안정성과 효율성을 잘 보여주었습니다.
...
### Enhancing Precision of Automated Teller Machines Network Quality Assessment: Machine Learning and Multi Classifier Fusion Approaches (https://arxiv.org/abs/2501.01067)
- **What's New**: 이 연구는 ATM(Automated Teller Machine) 네트워크의 신뢰성을 향상시키기 위해 다중 분류기 융합 기술, 특히 Stacking Classifier를 중심으로 한 데이터 융합 접근법을 소개합니다.Synthetic Minority Over-sampling Technique(SMOTE)를 적용하여 클래스 불균형을 해결하며, 다양한 분류 모델인 Random Forest, LightGBM, CatBoost를 통합하여 거짓 경고를 3.56%에서 0.71%로 감소시키고, 전체 정확도를 99.29%에 달하게 했습니다.

- **Technical Details**: ATM 상태를 검출하는 문제는 분류 문제로 설정될 수 있으며, 상태 파일과 ATM 저널 파일 등 두 가지 주요 데이터 소스를 통해 ATM의 현재 상태를 결정합니다. ATM 저널 파일은 사용자가 카드 삽입부터 키 입력 및 ATM의 모든 결정까지의 자세한 로그를 제공합니다. 이런 다양한 데이터 소스를 효율적으로 처리하는 것이 신뢰할 수 있는 상태 탐지에 필수적이며, 머신러닝 모델들은 이러한 데이터 융합 정확도를 높이는 데 중요한 역할을 합니다.

- **Performance Highlights**: 연구 결과, 다중 분류기 융합 방법이 거짓 경고를 크게 줄이고, 운영 결정 프로세스를 개선하여 금융 기관들이 ATM 네트워크의 성능을 최적화하는 데 기여할 수 있음을 증명했습니다. 이 접근 방식은 실질적으로 비용 절감과 고객 만족을 동시에 추구할 수 있는 효율적인 해결책을 제공합니다.



### State-of-the-art AI-based Learning Approaches for Deepfake Generation and Detection, Analyzing Opportunities, Threading through Pros, Cons, and Future Prospects (https://arxiv.org/abs/2501.01029)
- **What's New**: 이번 논문에서는 딥페이크(deepfake) 기술의 급속한 발전과 이를 통해 생성된 몰입감 있는 얼굴 이미지 및 비디오 콘텐츠에 대한 관심이 증가하고 있음을 강조합니다. 특히, 법의학 분석(forensic analysis), 사이버 보안(cybersecurity), 그리고 디지털 캐릭터(digital characters) 제작과 같은 다양한 분야에 걸쳐 이루어지고 있는 혁신적인 발전을 다룹니다. 논문은 최신 딥러닝(deep learning) 방법론인 Generative Adversarial Networks(GANs), Variational Autoencoders(VAEs), Few-Shot Learning 전략과 Transformers를 통해 이루어진 주요 성과들을 조망합니다.

- **Technical Details**: 이 리뷰 논문은 약 400개의 최근 연구를 종합적으로 분석하여 딥페이크 생성 및 탐지 기술의 최첨단 혁신을 조사합니다. 문헌 리뷰(methodologies) 방법론을 체계적으로 검토하며, 딥페이크 생성을 위한 다양한 기술적 복잡성과 직면한 어려움, 가능한 해결책, 조작 공식(manipulation formulations)과 관련된 세부사항을 면밀히 다룹니다. 또한, 주목할 만한 데이터 세트(data sets)를 기준으로 주요 접근 방식들을 정확하게 벤치마킹(benchmarking)하며, 이 중요한 분야에 미친 기여들에 대한 철저한 평가를 제공합니다.

- **Performance Highlights**: 논문 마지막 부분에서는 딥페이크 기술의 사용과 관련된 잠재적 남용 문제를 해결하기 위해 발전 중인 탐지 기술(detection technologies)에 대한 논의가 진행됩니다. 여기서는 정치적 조작(political manipulation), 가짜 뉴스(fake news) 전파, 사이버 괴롭힘(cyberbullying) 문제와 같은 중요한 우려사항들에 대해 다루며, 연구 분야의 지속적인 발전을 위해 해결해야 할 도전과제(challenges)를 상세히 논의합니다. 연구 결과는 넓은 적용 가능성을 보여 주며, 딥페이크 기술의 발전을 이해하는 데 중요한 기초가 될 것입니다.
...
### Prediction of Geoeffective CMEs Using SOHO Images and Deep Learning (https://arxiv.org/abs/2501.01011)
Comments:
          21 pages, 13 figures

- **What's New**: 최근 기계 학습(ML)의 적용이 코로나 질량 방출(CME) 및 지구에 대한 그 영향을 연구하는 데 있어 크게 증가하고 있습니다. 특히 GeoCME라는 새로운 딥러닝 프레임워크가 개발되어 CME가 지구에 도달했을 때 geomagnetic storm(지자기 폭풍)을 일으킬 가능성을 예측할 수 있습니다. 이 연구에서는 CME의 영향력을 예측하기 위해 SOHO의 관측 데이터를 활용하며, 이 프레임워크의 효과성을 입증하는 실험 결과를 제시합니다.

- **Technical Details**: GeoCME는 LASCO C2, EIT, MDI를 포함한 SOHO의 다양한 장치로부터 수집된 데이터를 기반으로 학습됩니다. 이 프레임워크는 ensemble 학습 및 transfer 학습 기법을 사용하여 SOHO 관측 데이터에서 숨겨진 특징을 추출하고, 이를 기반으로 예측을 수행합니다. geomagnetic storm의 정의는 지구의 자기권이 방해를 받을 때 Dst 지수가 -50 nT 이하로 감소하는 것입니다.

- **Performance Highlights**: GeoCME는 결정론적(predeterministic) 예측 모델로 사용할 때 Matthew's correlation coefficient가 0.807, true skill statistics score가 0.714에 도달하여 우수한 성능을 보였습니다. 또한, 확률적(probabilistic) 예측 모델로는 Brier score가 0.094, Brier skill score가 0.493을 기록했습니다. 이러한 결과는 GeoCME가 CME가 유발하는 태양-지구 상호작용에 대한 이해를 심화할 수 있는 가능성을 보여줍니다.
...
### Multi-Objective Optimization-Based Anonymization of Structured Data for Machine Learning (https://arxiv.org/abs/2501.01002)
- **What's New**: 이 연구는 데이터 공유 및 출판 과정에서의 개인 정보 보호를 위한 다목적 최적화 모델을 제안합니다. 기존의 정보 손실을 최소화하고 공격으로부터 보호를 극대화하는 모델로, 다변량 데이터의 효과적인 처리와 ML 모델 성능 유지에 중점을 두고 있습니다. 이러한 접근은 데이터 유틸리티와 개인 정보 보호 간의 균형을 맞추는 데 기여하고 있습니다.

- **Technical Details**: 제안된 모델은 엔트로피를 활용한 목표 함수와 k-anonymity 제약을 통합하여 동질성 공격에서 보호하도록 설계되었습니다. 이는 두 가지 기존 알고리즘과 비교하면서 정보 손실, 공격에 대한 저항력, 그리고 익명화된 데이터의 ML 성능을 평가합니다. 데이터 유틸리티에 대한 효과 평가 또한 중요한 연구 요소로 다루어집니다.

- **Performance Highlights**: 제안된 모델은 정보 손실이 적고 여러 정도의 데이터 세트에서 공격에 대한 저항력이 강화되었음을 보여주었습니다. 비교 결과, 기존 알고리즘보다 몇 가지 경우에서 더 나은 성능을 보였으며, ML 모델 성능도 원본 데이터 또는 다른 방법으로 익명화된 데이터에 비례해 유지되었습니다. 이는 데이터 공유에서 개인 정보 보호의 중요성을 강화하는 연구 결과입니다.



### Physics-informed Gaussian Processes for Safe Envelope Expansion (https://arxiv.org/abs/2501.01000)
- **What's New**: 이 논문에서는 기존의 비효율적인 비행 시험 데이터 수집을 개선하기 위해, 물리 정보가 포함된 Gaussian 프로세스(GP) 모델을 제안합니다. 이 방법은 T-38 전투기의 실제 비행 데이터를 사용하여 검증되었으며, 고정된 시험 포인트 없이도 비행 시험 데이터를 바탕으로 공기역학적 양을 추정할 수 있게 해줍니다. 특히, pitching moment coefficient (C_{m})를 측정하는 데 있어 기존 방법의 필요성을 줄이며, 실험 시간과 비용을 크게 절감할 수 있는 가능성을 제시합니다.

- **Technical Details**: 제안된 방법은 GP 프레임워크 내에 공기역학적 모델을 사전 정보로 통합하여, 다양한 비행 조건에서의 예측 정확도를 높입니다. 이를 통해 반복적인 비행 테스트 없이도 공기역학적 양을 추정할 수 있으며, 테스트 조종사에게 높은 데이터 정확성을 요구하지 않아도 됩니다. 논문에서는 데이터를 기반으로 한 방법만으로는 예측의 신뢰성이 떨어질 수 있음을 강조하며, 물리적 지식을 모델에 통합하는 방식을 제안합니다.

- **Performance Highlights**: 제안한 GP 기반 접근 방식을 통해, 다양한 Mach 수 및 동적 압력 프로필에서 T-38의 단기 주파수와 감쇠를 정확하게 예측할 수 있음을 보여주었습니다. 이 연구의 결과는 비행 시험 과정을 간소화하고, 비용을 절감하며, 데이터 기반 모델의 적용 가능성을 확대할 수 있는 강력한 도구로 작용할 것이라 기대됩니다.



### Is It Still Fair? Investigating Gender Fairness in Cross-Corpus Speech Emotion Recognition (https://arxiv.org/abs/2501.00995)
- **What's New**: 이 연구는 음성 감정 인식(SER)에서 성별 공정성을 다루는 새로운 관점을 제시합니다. 기존의 연구들은 주로 데이터셋 특정의 공정성에 초점을 맞췄으나, 본 연구는 교차 데이터셋에서의 성별 공정성 일반화를 조사했습니다. 또한, 우리는 교차 데이터셋 SER 과제에서 성별 공정성을 향상시키기 위한 결합된 공정성 적응 메커니즘을 제안합니다.

- **Technical Details**: MSP-Podcast와 BIIC-Podcast라는 두 개의 대규모 음성 코퍼스를 활용하여 실험을 진행합니다. 실험에는 전이 학습 기법과 공정성 접근 방식을 결합하여 성별 공정성을 평가합니다. 성별 불공정성을 평가하기 위해 Statistics Parity (ΔSP)와 Equal Odds (ΔEO) 메트릭을 적용하며, 모델의 성능은 Unweighted Average Recall (UAR)로 측정합니다.

- **Performance Highlights**: 실험 결과, 기존의 전이 학습 방식에도 불구하고 교차 데이터셋 SER 일반화가 성별 편향을 도입한다는 것을 보여줍니다. 제안된 결합된 공정성 적응 방식은 성별 공정성에서 기존 기법에 비해 놀라운 성과를 나타내었으며, 연구의 초기 결과는 이 방향에서의 필요성을 강조합니다.
...
### Optimizing Noise Schedules of Generative Models in High Dimensionss (https://arxiv.org/abs/2501.00988)
- **What's New**: 본 논문은 확산 모델(difffusion models)의 파동 전이(phase transition)에 대한 새로운 접근 방식을 제시합니다. 특히, 편향-보존(Variance Preserving, VP) 및 편향-폭발(Variance Exploding, VE)과 같은 다양한 노이지 스케줄(noise schedules)을 사용하여 고급 및 저급 피처를 모두 복구할 수 있는 방법을 제안합니다. 또한, 생성 모델의 성질을 계속 분석하면서 특이화 시간(speciation time)의 개념을 다룹니다.

- **Technical Details**: 연구에서는 주어진 도메인을 위한 확률 흐름 ODE(probability flow ODE)의 개념과 이를 통해 생성 과정을 이해하고, 연속 상태 공간에서 변동 특성을 극복하기 위한 적절한 노이지 스케줄의 선택을 강조합니다. 특히 두 모드 가우시안 혼합(Two-mode Gaussian mixture) 예제를 통해 VP와 VE가 어떻게 서로 다른 특성을 캡처하고 상호 보완적으로 작용하는지를 보여줍니다. 초기 편향 및 시간 스케줄의 적절한 선택이 중요함을 강조하며, 이를 통해 차원 증가에 따른 세부 조정이 가능함을 증명합니다.

- **Performance Highlights**: 제안된 노이지 스케줄은 데이터의 특성을 더 잘 포착할 수 있도록 하며, 생성 프로세스의 주요 피처를 정확하게 추정하기 위해 필요한 격자의 단계를 줄일 수 있습니다. 논문에서 확인한 바에 따르면, 고차원 데이터에서도 Θ_d(1) 단계로 변동성을 추정하는 것이 가능하며, 이는 기존의 Θ_d(√d) 단계보다 크게 개선된 결과입니다. 결론적으로 이러한 접근 방식은 이미지 생성 및 다차원 데이터 처리를 위한 모델 설계에 있어 중요한 기초 자료를 제공합니다.
...
### On the Low-Complexity of Fair Learning for Combinatorial Multi-Armed Band (https://arxiv.org/abs/2501.00924)
- **What's New**: 본 논문은 공정성 제약이 있는 조합 멀티 암드 배딧(Combinatorial Multi-Armed Bandit, CMAB) 프레임워크를 제안합니다. 새로운 알고리즘은 기존의 비관적-낙관적(pessimistic-optimistic) 알고리즘의 높은 계산 복잡성을 해결하기 위해 'pick-and-compare' 접근 방식을 사용하여 저복잡도 공정 학습 알고리즘을 개발했습니다. 이 알고리즘은 매 라운드의 수행을 통해 각 팔의 가중치를 동적으로 조정하며, 각 팔에 대해 최소 평균 보상을 보장합니다.

- **Technical Details**: 제안된 알고리즘은 M개의 팔 집합을 랜덤하게 선택하여 이들 집합의 가중치를 평가하고, 이전 라운드에서 선택된 집합과 비교하여 최대 가중치를 가진 집합을 선택합니다. 이 방식은 계산 복잡성을 상수로 줄이며, 따라서 알고리즘의 전반적인 성능을 향상시킵니다. 또한, 이론적으로 비관적-낙관적 알고리즘의 성능을 어느 정도 유지하면서도 공정성과 후회(regret)의 성능에서 경미한 희생을 감수한다고 주장합니다.

- **Performance Highlights**: 제안된 저복잡도 알고리즘은 시뮬레이션을 통해 검증했으며, 고차원 데이터에서 매우 좋은 성능을 보였습니다. 특정 라운드 이후에는 공정성 제약 위반이 없음을 보여주어, 이론적 성과를 실제 문제에 효과적으로 적용할 수 있음을 나타냅니다. 이는 특히 무선 네트워크와 같은 고복잡도 환경에서 중요한 결과로 평가됩니다.



### Exploring Geometric Representational Alignment through Ollivier-Ricci Curvature and Ricci Flow (https://arxiv.org/abs/2501.00919)
Comments:
          Presented at NeuReps workshop, NeurIPS 2024

- **What's New**: 이 논문에서는 인공 신경망과 인간의 인지 시스템 간의 표현을 비교하는 새로운 방법론으로 Ollivier-Ricci curvature와 Ricci flow를 활용합니다. 고차원 표현 공간 내에서 지역적인 기하학적 특성을 고려하지 못했던 기존 방법의 한계를 극복하고자 합니다. 특히, VGG-Face 모델과 인간의 유사성 판단을 비교하여, 두 시스템 간의 표현 정렬을 기하학적 수준에서 분석합니다.

- **Technical Details**: 연구에서는 ORC 방법을 사용하여 데이터의 그래프 표현을 통해 인간의 유사성 판단과 VGG-Face 모델의 활성화 패턴을 비교합니다. 실험에서는 1,397명의 참가자가 100개의 생성된 얼굴 간 유사성을 판단하는 작업에 참여하였고, Aligned VGG-Face 모델이 인간의 판단에 맞춰 조정되었습니다. 이 과정에서 각 노드의 국소 연결성과 그라디언트를 기반으로 지역적 구조 유사성 및 차이를 발견하였습니다.

- **Performance Highlights**: 분석 결과, Aligned VGG-Face는 인간 판단 그래프와의 곡률 구조가 유사하여, 인간의 유사성 판단과 일치하는 경향을 보였습니다. 반면, 원래의 VGG-Face는 군집 구조가 명확하지 않았으며, 알리인드 모델보다 더 비대칭적인 패턴을 나타냈습니다. 이러한 차이는 주변 연결성을 통해 더욱 두드러지며, 이로 인해 고차원 데이터의 내재적 기하학적 속성을 잘 반영했습니다.
...
### Aligning LLMs with Domain Invariant Reward Models (https://arxiv.org/abs/2501.00911)
- **What's New**: 이번 연구에서는 인간의 선호도가 수집되지 않은 대상 도메인에서도 보상 모델(reward model)을 학습할 수 있는 새로운 접근 방법을 제안합니다. 우리는 보다 간단한 출처 도메인(source domain)에서 수집한 피드백을 활용하여 도메인 불변 보상 모델(domain-invariant reward model)을 학습하는 프레임워크를 개발했습니다. 우리 방법론에서는 도메인 간의 차이를 줄이는 손실(doman loss)과 출처 도메인에서의 선호도를 최적화하는 손실(source loss)의 두 가지 목표를 동시에 최적화합니다.

- **Technical Details**: 우리가 제안하는 DIAL(Domain Invariant Alignment for Language) 프레임워크는 레이블이 있는 출처 데이터와 레이블이 없는 대상 데이터에 기반하여 도메인 불변 보상 모델을 학습하는 방식입니다. 이 모델은 크리틱(head)과 보상(head) 두 개의 스칼라 출력 헤드를 갖추고 있습니다. 크리틱 헤드는 출처와 대상 분포 간의 Wasserstein 거리(Wasserstein Distance)를 최소화하고, 보상 헤드는 출처 데이터에서 선호도를 최적화하여 도메인 불변한 방식으로 학습합니다.

- **Performance Highlights**: DIAL의 강력함은 네 가지 다양한 설정에서 검증되었습니다. 첫째, 고자원 언어에서 저자원 언어로의 교차 언어 전이(cross-lingual transfer)에서 정확도가 0.621에서 0.661로 향상되었습니다. 둘째, 깨끗한 데이터에서 노이즈가 있는 데이터로의 전이(clean-to-noisy transfer)에서 정확도가 0.671에서 0.703으로 증가했습니다. 셋째, 몇몇 예제로부터 전체 데이터로의 전이(few-shot-to-full transfer)에서는 정확도가 0.845에서 0.920으로 상승하는 성과를 보였습니다. 마지막으로, 단순 작업에서 복잡 작업으로의 전이(simple-to-complex transfer)에서 상관관계가 0.508에서 0.556으로 개선되었습니다.
...
### Evaluating Time Series Foundation Models on Noisy Periodic Time Series (https://arxiv.org/abs/2501.00889)
- **What's New**: 최근의 다수의 기초 모델(Foundation Models)들은 기계 학습 분야에 획기적인 발전을 가져왔으나, 특히 시간 시계열 기초 모델(Time Series Foundation Models, TSFMs)의 성능에 대한 철저한 테스트는 부족한 상황입니다. 본 논문에서는 노이즈가 있는 주기적 시간 시계열 데이터에 대한 여러 TSFMs의 zero-shot 장기 예측 능력을 평가하는 경험적 연구를 제시합니다. 모델의 유효성은 다양한 노이즈 수준, 기반 주파수 및 샘플링 속도에 걸쳐 평가되며, 비교 기준으로 두 가지 통계적 기법을 선택하였습니다.

- **Technical Details**: 본 논문에서는 TSFM의 zero-shot 예측 능력을 실증적으로 탐구하며, 시간 시계열 데이터의 복잡성, 노이즈, 기반 주파수 및 샘플링 속도가 모델 성능에 미치는 영향을 평가합니다. 평가된 모델은 TimesFM, Lag-Llama, CHRONOS(‘small’ 및 ‘base’), MOIRAI(‘small’ 및 ‘base’)입니다. TSFMs는 이론적 보장 및 가정이 결여되어 있어 새로운 시간 시계열의 예측 성능이 어떻게 나타날지에 대한 불확실성이 존재합니다.

- **Performance Highlights**: 결과적으로 TSFMs는 짧은 주기와 매끄러운 형태의 시간 시계열에서는 우수한 성능을 보이며, 낮은 노이즈 및 높은 샘플링 속도에서 통계적 기법보다 더 나은 성능을 나타내기도 합니다. 반면, 기간이 길어지거나 노이즈 수준이 높고 샘플링 속도가 낮아지며, 시계열의 형태가 복잡해질수록 TSFMs의 예측 능력은 급격히 감소합니다. 이러한 결과는 기초 모델이 시간 시계열 데이터를 처리하는 데 있어 그 한계를 드러내고 있습니다.
...
### Hybridising Reinforcement Learning and Heuristics for Hierarchical Directed Arc Routing Problems (https://arxiv.org/abs/2501.00852)
- **What's New**: 이 논문에서는 Hierarchical Directed Capacitated Arc Routing Problem (HDCARP)에 대한 새로운 접근법을 제안합니다. 기존에 발표된 matheuristic 방법이 느린 성능을 보였지만, 우리는 고속 히uristic와 Reinforcement Learning (RL)을 통합하여 HRDA라는 새로운 하이브리드 알고리즘을 개발했습니다. 이 알고리즘은 실시간 피드백을 통해 경로 최적화를 달성합니다.

- **Technical Details**: HDCARP는 우선순위에 따라 분류된 그래프의 아크를 효율적으로 탐색하는 문제입니다. D에 있는 모든 필수 아크는 하나의 차량에 의해 반드시 서비스되어야 하며, 차량의 부하 용량인 Q를 초과해서는 안 됩니다. 이는 우선순위에 따라 아크 서비스 순서를 결정하는 계층적 목표를 설정하여 해결됩니다.

- **Performance Highlights**: 우리는 인위적으로 생성된 사례를 통한 광범위한 실험을 통해 하이브리드 접근법이 기존 방법과 비교해 속도를 크게 향상시키면서도 해답의 질이 떨어지지 않음을 입증했습니다. 따라서 HRDA는 계산 시간 단축과 더불어 안정성을 제공하며, 동적인 문제 구조에 적합하게 조정될 수 있습니다.
...
### Hardness of Learning Fixed Parities with Neural Networks (https://arxiv.org/abs/2501.00817)
- **What's New**: 본 논문에서는 학습 이론에서의 패리티 함수(Parity Function) 학습의 어려움을 다루고 있습니다. 고정된 패리티(예: 모든 좌표에 대한 전체 패리티 함수)를 다룰 때, 기존의 표준 예측기와 경량화된 방법(gradient-based methods)으로는 효과적인 학습이 불가능하다는 점을 강조합니다. 이 연구는 고차원에서 패리티 함수의 학습이 특히 어려운 이유를 명확히 밝히고 있습니다.

- **Technical Details**: 패리티 함수는 특정 집합 S의 요소인 xᵢ의 곱을 통해 표현됩니다. 연구자는 비선형 형태인 ReLU 네트워크에 대해 응용된 경량화된 방법을 통해 고정된 패리티 함수를 목표로 할 때 유의미한 결과를 도출할 수 없음을 입증합니다. 이를 위해 선형 임계값 함수의 푸리에 계수(Fourier coefficients)의 감소에 대한 새로운 결과를 제시하여 그 의미를 강조합니다.

- **Performance Highlights**: 이 논문에서는 경량화된 방법이 패리티 함수 학습에 있어 막대한 차이를 보이기 때문에, 기존 방법들이 실질적으로 더 효과적인 학습을 미치지 못한다는 강력한 실증적 증거를 제시합니다. 또한 고차원에서는 표준 학습 방법의 적용이 더욱 어렵다는 점을 부각시키며, 이와 관련된 새로운 성과를 도출하는 중요한 이정표가 될 것으로 기대됩니다.



### Follow The Sparse Approximate Leader for No-Regret Online Sparse Linear Approximation (https://arxiv.org/abs/2501.00799)
Comments:
          12 pages, 5 figures

- **What's New**: 이번 논문에서는 온라인 희소 선형 근사(online sparse linear approximation) 문제를 다룹니다. 이 문제는 데이터 측정에 대한 최적의 희소 근사를 예측하는 것으로, 활용 분야는 의료 실험, 웹 캐싱, 자원 할당 등 다양합니다. 특히, 제안된 방법론인 Follow-The-Approximate-Sparse-Leader(FTASL)는 데이터 의존적인 하한을 제공하는 효율적인 온라인 메타 정책을 제시합니다.

- **Technical Details**: 희소 선형 근사는 주어진 측정 벡터를 최적으로 근사하는 측정 행렬의 특정 열을 찾는 문제입니다. 본 연구에서는 FTASL 메타 정책을 활용해 K-희소 신호를 복구하는 Online Sparse Linear Approximation(OSLA) 문제를 해결합니다. 이때 FTASL 정책은 아예 정적인 후회(static regret)의 변수적인 보장을 제공하고, 이는 문제 구조에 따라 로그(logarithmic)에서 제곱근(square-root)까지 다양합니다.

- **Performance Highlights**: 이 논문을 통해 제안된 정책은 이론적인 분석과 수치적 시뮬레이션을 통해 성능이 입증되었습니다. 특히, 데이터 수집 순서에 대한 특정 가정 하에 FTASL 메타 정책이 제공하는 정적인 후회는 기존 방법보다 우수한 성능을 나타냅니다. 수치 실험은 제안된 접근 방식의 유효성을 확인하는 데 중요한 역할을 합니다.
...
### Avoiding Oversmoothing in Deep Graph Neural Networks: A Multiplicative Ergodic Analysis (https://arxiv.org/abs/2501.00762)
- **What's New**: 이 연구는 그래프 신경망(GNN)에서 발생하는 '오버스무딩(oversmoothing)' 문제를 해결하기 위한 새로운 이론적 분석을 제시합니다. 특히, 잔여 연결(residual connection)의 추가가 오버스무딩 현상을 효과적으로 완화할 수 있음을 보여줍니다. 연구자들은 정상화된 정점 유사성 측정에 대한 수렴 속도를 파악하여 이 문제에 대한 깊이 있는 통찰을 제공합니다.

- **Technical Details**: 논문에서는 깊은 GNN이 잔여 연결을 포함할 때와 포함하지 않을 때의 비대칭 오버스무딩 비율을 분석합니다. 이 분석은 곱셈적 에르고딕 정리(multiplicative ergodic theorem)에 뿌리를 두고 있으며, 다양한 매개변수 분포에 대한 성능을 검증합니다. 구체적으로, 그래프 합성곱 네트워크(GCN)와 그래프 주의 네트워크(GAT) 등의 두 가지 널리 사용되는 아키텍처를 다룹니다.

- **Performance Highlights**: 연구 결과는 시뮬레이션 실험을 통해 이론적 발견을 강력히 뒷받침합니다. 잔여 연결을 통해 오버스무딩을 효과적으로 방지할 수 있는 가능성을 제시하며, 이는 그래프 학습 작업에서 GNN의 표현력을 극대화하는 데 기여할 수 있습니다. 이러한 통찰은 실용적 응용을 위한 중요한 전환점을 제공합니다.



### FasterSTS: A Faster Spatio-Temporal Synchronous Graph Convolutional Networks for Traffic flow Forecasting (https://arxiv.org/abs/2501.00756)
Comments:
          13pages,3 figures

- **What's New**: 본 연구는 효과적인 spatio-temporal 동기화 모델을 제안하여 교통 흐름 예측의 정확도를 높이고 복잡성을 줄이는 데 초점을 맞추고 있다. 많은 기존 연구가 spatial과 temporal 차원에서 별도의 상관 관계를 포착하여 복잡한 spatio-temporal 이질성을 포착하기 어려운 점을 반영하였다. 새로운 FasterSTS 모델은 기존의 spatio-temporal 특성 추출 구성 요소에 의존하지 않고 그래프 컨볼루션 네트워크(Graph Convolutional Networks)를 활용하여 동적 spatio-temporal 상관 관계를 모델링한다.

- **Technical Details**: FasterSTS 모델은 전통적인 그래프 계산 방식을 두 단계인 노드 표현 프로젝션과 노드 정보 집합으로 분리하여 시간 복잡성을 획기적으로 줄인다. 노드 정보 집합 단계에서 n x N 행렬을 사용하여 원래의 N x N 적응 행렬 대신 채택하고, 노드 표현 프로젝션 단계에서는 1x1 컨볼루션 레이어를 사용하여 N차원 노드 표현을 생성한다. 또한, static adaptive embeddings와 embedding projection을 통해 temporal correlation의 정적 모델링 및 feature 매핑을 수행하며, dynamic adaptive embeddings를 통해 dynamic spatial correlation의 동적 모델링 및 feature 매핑도 가능하게 한다.

- **Performance Highlights**: 이 모델은 기존의 복잡한 STGNNs와 비교했을 때 예측 성능을 크게 향상시킬 수 있으며, 시간 복잡성을 줄이는 데 투입되는 효과적인 방법을 제시한다. FasterSTS는 graph convolution kernel을 spatio-temporal synchronous graph convolution kernel로 변환하여 동적 및 정적 특성을 모두 모델링할 수 있도록 하여 글로벌 및 로컬 adaptive embeddings을 활용하여 spatial correlations를 완전하고 효과적으로 포착한다. 결과적으로, 이 모델은 복잡한 모델링을 피하면서도 높은 정확도로 교통 흐름을 예측하는 데 성공한다.
...
### Experimental Demonstration of an Optical Neural PDE Solver via On-Chip PINN Training (https://arxiv.org/abs/2501.00742)
- **What's New**: 이 논문에서는 물리적 지식을 활용한 신경망(Physics-informed Neural Networks, PINN)의 광학 칩에서의 훈련을 통해 부분 미분 방정식(Partial Differential Equation, PDE)의 해결 방법을 실험적으로 보여줍니다. 기존의 백 프로파게이션(Back Propagation, BP) 방법을 사용하지 않고도 학습이 가능하며, 이러한 방법은 온칩에서 효과적인 훈련을 가능하게 합니다. 이를 통해 강력한 GPU의 필요 없이도 실시간으로 PDE 솔버를 구현할 수 있습니다.

- **Technical Details**: PINN은 미분 방정식의 잔여물(Residuals)과 경계/초기 조건을 사용하여 손실 함수(loss function)를 설정하는 방식으로 훈련됩니다. 이 연구에서는 제로 차수 최적화(Zeroth-Order Optimization, ZO)를 통해 훈련을 진행하며, 이는 미분 가능한 신경망 모델이 필요하지 않아 광학 장치의 가변 매개변수를 직접 최적화할 수 있게 합니다. 이러한 방법은 제작 오류와 환경 잡음(noises)을 실시간으로 완화할 수 있어 별도의 보정(calibration) 과정이 필요하지 않습니다.

- **Performance Highlights**: 실험적으로, 이 연구는 1차원 열 방정식에 대한 PINN 솔버를 성공적으로 훈련시켰으며, 1000회의 업데이트 후 5E-3의 잔차 오류로 PDE 솔루션을 근사하였습니다. 제안된 광학 신경망이 높은 처리량과 낮은 에너지 소비를 가능하게 하며, 다양한 공학 분야에 응용될 수 있는 가능성을 보여줍니다. 이 연구는 광학 신경망 기술의 상용화를 위한 중요한 초석이 될 것으로 기대됩니다.
...
- **What's New**: 이 논문은 Kolmogorov-Arnold Neural Network (KAN)을 Signed Graph Convolutional Networks (SGCN)에 통합하여, 긍정 및 부정 신호가 있는 signed 그래프에서 KAN의 성능을 평가하고 있습니다. 기존 KAN의 최적화 추구에 이어, 본 연구에서는 KAN이 signed GNN에 미치는 영향을 최초로 탐구합니다. 이로 인해 KAN의 잠재력을 활용하여 signed 네트워크의 임베딩 품질을 높이는 방법을 제시합니다.

- **Technical Details**: 본 연구에서는 KAN을 SGCN에 통합하기 위해 Kolmogorov-Arnold 표현 정리를 기반으로 하고, KAN은 학습 가능한 활성화 함수를 활용하여 이전의 고정된 활성화 함수 대신 사용됩니다. KAN의 주요 하이퍼파라미터는 스플라인 차수와 그리드 크기로 설정되며, 이를 통해 각 입력 차원의 비선형 스플라인 함수가 적용되어 최적의 임베딩을 생성합니다. 또한, KAN은 MLP에 비해 파라미터 수가 적으면서도 높은 표현력을 보입니다.

- **Performance Highlights**: KAN이 통합된 SGCN(KASGCN)은 signed 커뮤니티 탐지 및 링크 신호 예측과 같은 다양한 다운스트림 작업에서 평가되었습니다. KASGCN은 일관성 있는 성과를 보였고, 상대적으로 큰 표준 편차가 나타나기는 했지만, 평가된 작업에서 기존 SGCN과 경쟁력 있는 성능을 나타냅니다. KASGCN의 효율성은 연결된 그래프의 맥락에 따라 달라지는 것으로 나타났습니다.



### Kolmogorov GAM Networks are all you need! (https://arxiv.org/abs/2501.00704)
- **What's New**: 본 논문에서는 Kolmogorov 일반화 가법 모델(K-GAM)을 소개합니다. 이는 전통적인 접근법에 비해 매개변수가 크게 줄어들어 훈련과 추론을 간소화합니다. K-GAM은 Kolmogorov의 Superposition Theorem을 활용하여 다변량 함수의 효율적인 표현을 제공합니다. 이러한 구조는 병렬 처리 가능하여 계산적으로 매력적인 특성을 가집니다.

- **Technical Details**: K-GAM의 아키텍처는 독립적인 토폴로지적 임베딩과 일반화 가법 모델(GAM)을 사용하는 덧셈 계층으로 구성됩니다. 이는 α-변환(blend) 형식으로, 단일 변수가 아닌 두 개의 함수의 합으로 다변량 연속 함수를 표현할 수 있습니다. 모델의 학습 과정에서 ReLU 신경망을 사용하여 효율적으로 외부 함수를 훈련시킬 수 있습니다. 이를 통해 높은 차원의 입력-출력 관계를 효과적으로 처리하는 방법을 제시합니다.

- **Performance Highlights**: K-GAM 모델은 전통적인 딥러닝 알고리즘보다 훨씬 적은 매개변수를 요구합니다. 또한 기존의 transformer 아키텍처와 비교할 때, 보다 간단하고 유연한 대안을 제공합니다. 이는 Iris 데이터셋을 활용한 실험에서도 확인되며, K-GAM은 다양한 머신러닝 문제에서도 사용될 수 있습니다. 마지막으로, 우리는 K-GAM이 머신러닝에 있어 transformer의 자연스러운 대안이 될 수 있음을 강조합니다.



### NN-ResDMD: Learning Koopman Representations for Complex Dynamics with Spectral Residuals (https://arxiv.org/abs/2501.00701)
- **What's New**: 이 논문에서는 Neural Network-Residual Dynamic Mode Decomposition (NN-ResDMD)를 제안하여, 기존의 Residual Dynamic Mode Decomposition (ResDMD)의 한계를 극복했습니다. NN-ResDMD는 스펙트럼 잔차를 최적화하여 Koopman 스펙트럼을 직접적으로 추정하며, 신경망을 활용하여 자동으로 최적의 기저 함수(basis functions)를 식별합니다. 이러한 방법은 복잡한 동적 시스템 분석의 정확성과 확장성을 크게 향상시킵니다.

- **Technical Details**: NN-ResDMD는 Koopman 고유쌍(eigenpairs)을 수치적으로 최소화함으로써 스펙트럼 잔차를 직접적으로 계산하는 방법을 제공합니다. 또한, 이러한 신경망 기반 접근법은 자동으로 Koopman 불변 부분 공간(Koopman invariant subspace)의 최적 기저 함수를 선택하여, 수동 선택의 번거로움을 제거합니다. 실험 결과는 NN-ResDMD가 다른 기존 방법들보다 더 나은 성능을 보임을 입증합니다.

- **Performance Highlights**: 물리적 및 생물학적 시스템에서 실시한 실험들은 NN-ResDMD가 정확성과 확장성 모두에서 현저한 개선을 이룬 것을 보여줍니다. 이는 복잡한 동적 시스템을 이해하고 분석하는 데 있어 실용적이고 효과적인 도구로 자리잡게 됩니다. 특히, NN-ResDMD는 복잡한 시스템의 고차원 동적 데이터를 처리하는 데 있어 강력한 성능을 발휘합니다.



### Cost and Reward Infused Metric Elicitation (https://arxiv.org/abs/2501.00696)
Comments:
          Accompanying code at this https URL

- **What's New**: 이 논문에서는 머신러닝에서의 metric elicitation(측정 지표 유도)의 필요성과 기존의 방법에서 간과된 다양한 요인들을 고려한 새로운 알고리즘을 제안합니다. 특히, 기존의 confusion matrix(혼동 행렬) 중심의 메트릭 유도 방법과는 달리, 추가적인 비용과 보상 요소를 포함하는 방향으로 다중 클래스 메트릭 유도 프레임워크를 발전시킵니다. 새로운 알고리즘은 더불어 실제 애플리케이션의 요구에 맞춘 성능 지표를 유도할 수 있는 가능성을 보여줍니다.

- **Technical Details**: 이 연구에서는 Hiranandani et al.(2019a)에서 제안한 다중 클래스 metric elicitation 프레임워크를 기반으로, 추가적인 경계 비용(bound costs) 및 보상을 통합할 수 있는 Diagonal Linear Performance Metric Elicitation (DLPME) 알고리즘을 도입합니다. 본 알고리즘은 다양한 모델의 혼동 행렬을 기반으로 하여, 각각의 클래스에 대한 비용과 보상을 정의하고, 이를 통해 최적의 분류기를 찾기 위한 최적화를 수행합니다. 결과적으로 이 방식은 훨씬 더 다양한 요소를 고려하여 성능 지표를 빠르게 수렴시킬 수 있는 것으로 보입니다.

- **Performance Highlights**: 실험적으로 생성된 데이터에 대한 결과는 저자들이 제안한 알고리즘이 실제 성능 지표에 신속하게 수렴하는 능력을 나타냅니다. 기존의 단순한 혼동 행렬 접근법을 넘어, 추가적인 경제적 비용 및 시간 제약을 AR 기반의 메트릭 유도에 포함시킴으로써, 실제 환경에서의 모델 선택에 유용한 정보를 제공할 수 있음을 입증합니다. 그러므로 기업이 다양한 속성을 갖춘 모델 선택을 할 수 있도록 지원할 수 있는 가능성과 이익을 기대할 수 있습니다.
...
### Controlled Causal Hallucinations Can Estimate Phantom Nodes in Multiexpert Mixtures of Fuzzy Cognitive Maps (https://arxiv.org/abs/2501.00673)
Comments:
          17 pages, 9 figures, The Ninth International Conference on Data Mining and Big Data 2024 (DMBD 2024), 13 December 2024

- **What's New**: 이 논문은 대규모 인과 모델에서 누락된 또는 유령(phantom) 노드를 근사화할 수 있는 적응형 다중 전문가 혼합 피드백 인과 모델을 제안합니다. 이를 통해 "big knowledge"의 확장된 형태를 제공하며, 샘플링된 동적 시스템의 주요 리미트 사이클 평형을 근사화하는 혼합 모델을 개발했습니다. 각 전문가가 최소한 하나의 누락된 인과 노드를 포함한 퍼지 인지 맵(Fuzzy Cognitive Map, FCM)을 작성하면서 모델링을 진행합니다.

- **Technical Details**: 논문은 전문가들이 균형 행동에 영향을 미치는 관련 노드를 추정하고, 한 개 이상의 누락된 노드를 포함해야 한다고 강조합니다. 여러 전문가의 피드백을 통해 모델을 혼합하고 조정하는 과정을 통해 더 나은 인과 모델을 구축합니다. 각 FCM은 부분적 인과성을 모델링하며, 가중치 부여된 방향성 그래프의 구조를 통해 복잡한 피드백 동적 시스템에서의 인과 관계를 나타냅니다.

- **Performance Highlights**: 실험 결과, 유령 노드가 추가된 혼합 FCM이 개별 FCM보다 성능이 우수하다는 것을 증명했습니다. 또한, 작은 양의 인과적인 환각이 누락된 유의미한 인과 노드를 추정하는 데 도움이 되며, 이러한 환각을 통제함으로써 동적 시스템을 더 잘 근사화할 수 있음을 확인했습니다. 논문에서는 FCM의 혼합이 풍부한 인과 및 동적 구조를 만들어 낸다고 제시하며, 전문가 샘플 크기가 증가함에 따라 혼합 FCM의 성능이 향상된다고 설명합니다.
...
- **What's New**: 이 논문에서는 Structured State Space Models (SSMs)의 한계와 재구성을 논의합니다. SSMs는 transformers의 대안으로 부상했지만, 강한 최신성 바이어스(recent bias)에 의해 제한된다고 주장합니다. 저자들은 이 바이어스가 먼 정보 회상(distinct retrieval) 능력을 저해하고 모델의 강건성(robustness)도 저하한다고 설명합니다.

- **Technical Details**: SSMs은 두 가지 작동 모드를 가지며, 이를 통해 전체 시퀀스에 대한 정보를 병렬적으로 처리할 수 있습니다. 특히 Mamba 알고리즘은 효율성을 유지하면서 더 표현력이 풍부한 시퀀스 혼합을 제공합니다. 그러나 SSMs은 층이 깊어질수록 정보 표현이 과도하게 평활화(over-smoothing)되어 토큰의 구별 가능성이 떨어지는 패턴을 보입니다.

- **Performance Highlights**: 실험 결과, 저자들은 기존 SSMs 모형의 심각한 한계를 발견했습니다. 특히 모델이 깊어질수록 성능은 포화(pareto optimality)에 이르게 되고, 심지어 먼 컨텍스트를 잃어버리는 현상이 발생합니다. 이를 해결하기 위해 polarization 기법을 도입하여 바이어스와 과도한 평활화를 동시에 완화하며, 이로 인해 긴 범위의 연상 기억(associative recall 정확도가 개선된다고 보고합니다.
...
### Matrix factorization and prediction for high dimensional co-occurrence count data via shared parameter alternating zero inflated Gamma mod (https://arxiv.org/abs/2501.00628)
Comments:
          39 pages, 5 figures

- **What's New**: 이 논문에서는 고차원 희소 행렬 데이터를 다루고 있으며, 특히 많은 제로를 포함하는 비대칭 분포에서의 데이터 모형에 관해 다룹니다. 최근에는 사용자-항목 또는 항목-항목 동시 발생 데이터와 같은 다양한 응용 분야에서 이러한 데이터 모형의 필요성이 증가하고 있습니다. 제로가 많고 긍정적인 값들이 극도로 치우친 분포를 가진 데이터를 위해 공유 매개변수 모델링(shared parameter modeling) 기법을 활용한 새로운 접근법인 SA-ZIG 모델을 제안합니다.

- **Technical Details**: SA-ZIG 모델은 제로가 과도하게 포함된 감마 분포를 기반으로 하며, 비어있는 데이터를 베르누이 분포로 간주하여 많은 제로를 설명합니다. 이 모델은 비슷한 매개변수 추정을 간소화하기 위해 두 부분에서 공유 매개변수를 사용합니다. 또한, 두 가지 링크 함수(정규 링크와 로그 링크)를 고려하여 감마 회귀 부분의 매개변수 추정을 진행합니다.

- **Performance Highlights**: SA-ZIG 모델은 Fisher scoring을 활용한 시뮬레이션에서 학습률 조정이 없을 경우 최대 우도 추정(maximum likelihood estimate)을 찾지 못하는 상황에 직면할 수 있습니다. 반면, 학습률 조정을 통해 성능이 만족스러운 결과를 도출하는 것으로 보여집니다. numerical studies에서 제안된 모델의 유용성을 입증하며, 실제 데이터에 대한 적용 가능성을 보여주고 있습니다.



### Global dense vector representations for words or items using shared parameter alternating Tweedie mod (https://arxiv.org/abs/2501.00623)
Comments:
          43 pages 12 figures

- **What's New**: 이 논문에서는 사용자-아이템, 아이템-아이템 데이터와 같은 실제 데이터로부터 파생된 공출현 카운트 데이터를 분석하기 위한 Shared parameter Alternating Tweedie (SA-Tweedie) 모델을 제안합니다. 이 모델은 높은 차원의 공출현 행렬을 메모리에 적합하게 만들기 위해 새로운 접근 방식을 사용하고, Fisher scoring 방법과 함께 학습률 조정 기법을 도입하여 파라미터 최적화를 돕습니다. 시뮬레이션 연구 결과, 제안된 알고리즘이 다른 방법들에 비해 우수한 성능을 보인 것으로 나타났습니다.

- **Technical Details**: SA-Tweedie 모델은 기본적으로 Compound Poisson Gamma 분포를 활용하여 공출현 카운트를 처리하고, 이 분포는 독립적인 Gamma 랜덤 변수의 합을 모델링합니다. Tweedie 분포는 파라미터 공유를 가능하게 하여 동일한 아이템-아이템 쌍에서 다양한 출처로부터의 정보를 조합할 수 있게 해주며, 이는 회귀 분석 프레임워크 내에서 추정됩니다. 또한 이 모델에서 파라미터 추정을 위한 교차 엔트로피 기반의 방법을 탐구합니다.

- **Performance Highlights**: 모델의 성능은 시뮬레이션 연구를 통해 검증되었으며, Fisher scoring 방법과 학습률 조정 기법을 포함한 SA-Tweedie 알고리즘이 기존의 다른 두 방법보다 우수한 성능을 보여주었습니다. 논문에서는 이 모델이 대규모 데이터셋, 예를 들어 전체 Wikipedia 코퍼스에서의 학습도 가능하다는 점을 강조하고 있으며, 이는 Named Entity Recognition과 같은 실제 작업에 적용될 수 있음을 나타냅니다.



### Predicting Barge Presence and Quantity on Inland Waterways using Vessel Tracking Data: A Machine Learning Approach (https://arxiv.org/abs/2501.00615)
- **What's New**: 이 연구는 자동 식별 시스템(Automatic Identification System, AIS) 데이터를 활용하여 내륙 수로에서 선박이 운반하는 바지선 수를 예측하는 기계 학습 방법론을 제시합니다. 기존 AIS는 선박의 위치를 추적하지만, 바지선의 존재나 수량은 모니터링하지 않기 때문에 이는 중요한 정보입니다. 바지선의 수와 종류를 이해하는 것은 국가의 물류 수송량을 추정하는 데 필수적이며, 수로 관리 및 인프라 운영에서도 유용합니다.

- **Technical Details**: 모델 개발에는 최대 42개의 바지선 대열을 대표하는 164개의 선박 샘플이 사용되었습니다. AIS 데이터에서 유도된 피쳐에는 속도 측정, 선박 특성, 회전 측정, 그리고 상호 작용 항들이 포함되었습니다. 바지선의 존재 예측에는 AdaBoost 모델이 F1 점수 0.932를 달성했으며, 바지선 수량 예측 시에는 Random Forest와 AdaBoost 앙상블 모델이 F1 점수 0.886을 기록하였습니다. 하이퍼파라미터 조정을 위해 베이esian 최적화(Bayesian optimization)가 사용되었습니다.

- **Performance Highlights**: 이 연구에서는 내륙 수로에 대한 예측 모델링을 발전시켜 교통량에 대한 세부 지식을 요구하는 교통 계획자 및 기관에 귀중한 통찰력을 제공합니다. 연구 결과는 화물의 흐름, 목적지 및 항만으로 이동하는 톤수를 포함하여 교통량 파악에 필수적입니다. 이는 목표 정수 작업(targeted dredging operations)과 데이터 기반 자원 할당(data-driven resource allocation)에도 영향을 미치는 중요한 데이터로 작용할 수 있습니다.



### Time-Varying Graph Learning for Data with Heavy-Tailed Distribution (https://arxiv.org/abs/2501.00606)
- **What's New**: 이번 논문에서는 동적 데이터의 시간 변동 그래프 모델을 학습하는 새로운 방법론을 제안합니다. 기존의 접근 방식들이 데이터의 이상치에 민감하며, 금융 데이터와 같은 비정상적인 분포를 잘 처리하지 못하는 문제를 해결하고자 합니다. 본 연구는 스펙트럼 및 구조적 제약을 활용하여 효과적으로 이러한 문제를 해결할 수 있는 알고리즘을 발전시켰습니다.

- **Technical Details**: 제안된 방법론은 비부정적인 벡터 자가 회귀(Non-negative Vector Auto-regressive, VAR) 모델을 기반으로 하여 그래프의 변동을 포착합니다. 또한, Student-t 분포를 사용해 시간 가변 그래프에서 발생하는 신호를 모델링합니다. 이는 데이터 클러스터링을 개선하고 소음 및 결측값을 처리하는 데 도움이 됩니다. 우리가 제안한 방법은 미니 배치(mini-batch)로 그래프를 업데이트하는 세미 온라인 프레임워크를 포함합니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 모델이 금융 시장의 특수한 환경에서 중간 거대 분포를 효과적으로 분석할 수 있음을 확인할 수 있었습니다. 생성된 그래프는 동적 상관관계를 잘 캡처하며, 실험에서 세간의 관심을 받는 데이터 세트를 통해 그 성능이 입증되었습니다. 이는 재정적 데이터 분석에서 강력한 도구로 자리 잡을 가능성을 보여줍니다.
...
### Adaptive Tabu Dropout for Regularization of Deep Neural Network (https://arxiv.org/abs/2501.00538)
- **What's New**: 이번 논문에서는 Tabu Dropout 메커니즘을 개선하기 위해 두 가지 방법을 제안합니다. 첫 번째는 'tabu tenure'라고 불리는 기법으로, 특정 유닛이 몇 개의 에폭 동안 dropout(드롭아웃)되지 않도록 설정합니다. 두 번째는 훈련 성능에 따라 자동으로 'tabu tenure'를 선택하는 적응형 알고리즘인 Adaptive Tabu Dropout입니다.

- **Technical Details**: Tabu Dropout은 신경망의 각 에폭 동안 일부 뉴런을 드롭하여 모델의 일반화 성능을 향상시키는 Regularization(정규화) 기법입니다. 논문에서는 Tabu Tenure Dropout을 통해 특정 유닛이 이전 T epochs 동안 드롭되지 않도록 하여 다양성을 증가시키는 방법을 제시합니다. Adaptive Tabu Dropout은 다중 조작 문제로 모델링되어, 각 에폭의 손실을 기반으로 보상을 계산하여 최적의 tabu tenure을 선택합니다.

- **Performance Highlights**: 여러 표준 벤치마크 데이터셋에서 실험한 결과, Adaptive Tabu Dropout과 Tabu Tenure Dropout이 기존의 dropout 기법보다 우수한 성능을 보였습니다. Tabu Tenure 6이 실험 전반에서 가장 좋은 결과를 내었으며, 표준 Tabu Dropout과 비교하여 최대 62.06%의 오류율 감소를 달성했습니다. 이러한 개선 덕분에 신경망의 일반화 성능이 크게 향상되었습니다.



### Rapid Learning in Constrained Minimax Games with Negative Momentum (https://arxiv.org/abs/2501.00533)
- **What's New**: 본 논문은 제약이 있는 minimax 게임에서의 negative momentum 기법 활용에 대해 탐구합니다. 기존의 기법들을 넘어서는 모멘텀 버퍼 갱신 프레임워크를 제안하고, 이론적 수렴 보장을 제공합니다. 새로운 알고리즘은 기존의 게임 해결 방식과 비교하여 성능이 현저하게 향상됩니다.

- **Technical Details**: 논문에서 제안하는 기법은 entropic regularizer가 적용된 momentum-augmented 알고리즘을 통해, 제약이 있는 텍스트 모델에 대해서도 적용될 수 있도록 설계되었습니다. 이 알고리즘은 dilated distance generator 함수를 사용하여 특정 게임들에 대한 모멘텀 증강 버전을 제안합니다. 이를 통해, Nash 균형 세트에 수렴하거나 높은 버퍼를 통해 지수적 수렴 속도를 달성할 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 모멘텀 증강 알고리즘이 원래 알고리즘 및 기존의 강력한 last-iterate 수렴 기준을 초과하는 성능을 보였습니다. 특히 MoRM+와 MoCFR+ 알고리즘은 기존의 RM+와 CFR+ 알고리즘에 비해 10억 배 낮은 exploitability를 달성하여 다양한 게임 유형에서 최적 성능을 나타냈습니다.
...
### Stochastic Extragradient with Flip-Flop Shuffling & Anchoring: Provable Improvements (https://arxiv.org/abs/2501.00511)
Comments:
          73+7 pages, 4 figures. Published in NeurIPS 2024

- **What's New**: 이번 논문에서는 minimax 최적화(minimax optimization)에서 extragradient (EG) 방법을 활용하여 gradient descent-ascent 방법보다 성능이 우수한 점에 주목했습니다. 특히, stochastic EG (SEG) 방법이 unconstrained 문제에서 제한적인 성공만을 거둔 이유를 살펴보았습니다. 새로운 shuffling 기반(stochastic methods) 접근을 통해 unconstrained finite-sum minimax 문제에서 SEG의 수렴(convergence)을 조사하고, 새로운 방법론을 제안했습니다.

- **Technical Details**: 연구에 따르면, random reshuffling과 최근 제안된 flip-flop shuffling만으로는 C-C 문제에서 diverge 현상이 발생할 수 있습니다. 하지만 anchoring이라는 간단한 기법을 추가하여 SEG와 flip-flop anchoring을 결합한 SEG-FFA 방법을 개발하였고, 이를 통해 C-C 문제에서 성공적으로 수렴하게 하였습니다. 또한, strong-convex-strongly-concave 환경에서 상한 및 하한 경계를 설정하여, SEG-FFA의 수렴 속도가 다른 shuffling 기반 방법보다 빠르다는 것을 입증하였습니다.

- **Performance Highlights**: SEG-FFA 방법은 기존 shuffling 방식에 비해 더 빠른 수렴 속도를 가지며, 특히 강한 볼록-강한 오목 설정(strongly-convex-strongly-concave)에서 그 효과가 잘 드러납니다. 이러한 결과는 minimax 최적화 문제를 해결함에 있어 매우 유용한 통찰을 제공하며, 기존 방법보다 향상된 성능을 제시하고 있습니다. 본 연구는 최적화 분야에서의 새로운 방향성을 제시하고 있으며, 다양한 응용 분야에서 활용될 가능성을 보여줍니다.



### Active Learning of General Halfspaces: Label Queries vs Membership Queries (https://arxiv.org/abs/2501.00508)
Comments:
          Accepted by NeurIPS 2024

- **What's New**: 이 논문은 R^d에서 Gaussian 분포 하의 일반적인 halfspaces를 학습하는 문제를 다룹니다. 특히, passive 학습 환경에 비해 active 학습 모델이 레이블 복잡성을 줄일 수 있는 한계를 정량적으로 설정합니다. 여기서 제시된 정보 이론적 하한은 알고리즘이 상당한 개선을 이루기 위해서는 적어도 $2^{poly(d)}$의 비레이블 샘플을 요구함을 보여줍니다. 이러한 결과는 레이블 쿼리 접근 방법과 membership 쿼리 접근 방법 사이의 강한 구분을 이룹니다.

- **Technical Details**: 연구에서는 pool-based active learning 모델을 활용하여 learner가 unlabeled 데이터에서 레이블을 요청할 수 있도록 합니다. 이 접근 방식의 장점은 확률적인 태도에서 효과적인 학습을 가능하게 하며, membership query 접근 방식을 통해 더 나은 정보 획득이 가능함을 보여줍니다. 구체적으로, `membership query`를 통해 데이터의 각 지점에서 유타겟 함수를 평가할 수 있는 방법이 소개됩니다. 이로 인해 다른 분포 환경에서도 보다 높은 효율성을 달성할 수 있습니다.

- **Performance Highlights**: 연구의 주요 성과로는 일반적인 halfspaces의 학습이 Gaussian 분포 하에서 보다 효율적으로 이루어질 수 있다는 것을 제시하였습니다. 특히 멤버십 쿼리 접근을 사용할 경우에서 기존 `passive learning` 대비 오랜 모델 성과를 극복할 수 있는 가능성을 보여줍니다. 이를 통해 균형있는 dataset의 퀄리티와 유용성을 확보하게 되며, 실제 적용 가능한 다양한 이론적 결과들을 도출했습니다. 이는 향후 다양한 머신러닝 응용 분야에 긍정적인 영향을 미칠 것으로 기대됩니다.
...
### Score-Based Metropolis-Hastings Algorithms (https://arxiv.org/abs/2501.00467)
- **What's New**: 이번 논문에서는 점수 기반(score-based) 모델과 메트로폴리스-헤이스팅스(Metropolis-Hastings) 알고리즘을 통합하는 새로운 접근 방식을 제안합니다. 기존의 점수 기반 확산 모델은 데이터 포인트로부터 점수 함수(score function)를 정확하게 학습하는 데 강점을 가지고 있으나, 에너지 함수가 없기 때문에 메트로폴리스-헤이스팅스 조정 단계에 접근할 수 없는 한계가 있습니다. 이를 해결하기 위해, 상세 균형 조건(detailed balance condition)에 기반한 새로운 손실 함수를 도입하여 학습된 점수 함수를 바탕으로 메트로폴리스-헤이스팅스 수용 확률을 추정할 수 있는 방법을 제시합니다.

- **Technical Details**: 이 연구에서는 메트로폴리스-헤이스팅스(MH) 알고리즘의 수용 함수(acceptance function)를 활용하여 샘플링할 수 있는 기반을 마련합니다. MH 알고리즘은 제안 분포(proposal distribution)와 수용 기준을 기반으로 마르코프 체인(Markov Chain)을 구축하고, 이를 통해 주어진 데이터로부터 새로운 샘플을 수신해오는 방식을 사용합니다. 점수 기반 모델은 데이터로부터 로그 밀도의 그래디언트를 직접 학습하여 새로운 샘플을 생성할 수 있는 가능성을 높입니다. 이 방법은 비정상성(distributional irregularity)에 대한 강건성을 지니고 있으며, 특히 헤비테일 분포(heavy-tail distributions)와 같은 복잡한 분포에서 효과적입니다.

- **Performance Highlights**: 제안된 방법은 다양한 실험을 통해 그 효율성을 입증하였습니다. 특히, 기존의 일반화된 Langevin 알고리즘(ULA)과 비교할 때, 메트로폴리스-헤이스팅스 알고리즘을 통한 샘플링의 품질과 정밀도가 크게 향상되었습니다. 수용 함수의 추정 방법은 다양한 샘플링 시나리오에 적용 가능하며, 이를 통해 모델의 성능을 크게 개선할 수 있는 가능성을 보여줍니다. 또한, 이 연구는 점수 기반 생성 모델링(score-based generative modeling)에서 MH 기준을 사용하는 새로운 방향성을 제시합니다.



### Dementia Detection using Multi-modal Methods on Audio Data (https://arxiv.org/abs/2501.00465)
Comments:
          4 pages

- **What's New**: 이번 연구에서는 오디오 기록을 기반으로 한 모델을 개발하여 치매의 발병을 예측하는 방법을 제안합니다. 특히, Whisper를 이용한 Automatic Speech Recognition (ASR) 모델과 RoBERTa 회귀 모델을 결합하여 환자의 MMSE 점수를 생성합니다. 연구진은 PROCESS Grand Challenge 2025에서 제공된 PROCESS_V1 데이터셋을 사용하여 모델의 성능을 향상시켰습니다.

- **Technical Details**: 제안된 방법은 Whisper(ASR) 모델로 오디오를 텍스트로 변환한 후 RoBERTa를 통해 컨텍스트 임베딩을 생성하는 방식으로 진행됩니다. 사전 처리 단계에서 16 kHz로 오디오를 샘플링하고, MMSE 점수를 정규화하여 일관된 훈련 패턴을 형성합니다. 모델은 세 가지 음성 과제를 통해 각각의 MMSE 점수를 예측하며, 최종 예측은 Root Mean Square Error (RMSE)를 사용하여 평가합니다.

- **Performance Highlights**: 모델은 PROCESS Grand Challenge의 회귀 작업에서 상위 모델들과 견줄 만큼의 성능을 보였습니다. 평가 결과, 모델의 RMSE 점수는 2.6911로, 기준보다 약 10% 낮은 성과를 기록하였습니다. 이러한 결과는 치매의 조기 발견과 예측에 도움을 줄 수 있는 가능성을 보여줍니다.



### Addressing Challenges in Data Quality and Model Generalization for Malaria Detection (https://arxiv.org/abs/2501.00464)
Comments:
          22 pages, 17 figures, 17 tables, In: Journal of Sensor Networks and Data Communications (JSNDC). ISSN: 2994-6433. DOI: https://doi.org/10.33140/JSNDC.04.03.09

- **What's New**: 이번 연구는 말라리아 진단을 위한 딥러닝 모델의 효과성을 저해하는 데이터 품질 문제에 대한 포괄적인 분석을 제공합니다. 데이터 불균형, 제한된 다양성 및 주석 변동성과 같은 문제들이 진단의 신뢰성을 감소시키고 있음이 강조됩니다. GAN 기반 데이터 증강 기법을 통해 15-20%의 정확성 향상이 이루어졌으며, 전이 학습 및 도메인 적응 기법이 모델의 강건성을 높이는 데 기여했습니다.

- **Technical Details**: 말라리아 진단을 위한 딥러닝 모델의 훈련 데이터 세트 품질은 진단 정확도에 중요한 역할을 합니다. 불균형 클래스, 제한된 다양성 및 주석 품질의 변동 등 여러 데이터 품질 문제는 모델 성능을 저하시킵니다. 데이터 전처리 파이프라인을 통해 노이즈 제거 및 대표성이 부족한 클래스의 증강, 데이터 세트 균형이 이루어져 모델 훈련의 강건성을 보장합니다.

- **Performance Highlights**: 불균형 데이터 세트가 말라리아 진단 시스템에서 발생하며, 이는 모델이 다수 클래스에 편향된 예측을 하도록 초래합니다. 데이터 증강 기법, 전이 학습 및 새로운 손실 함수 적용을 통해 모델 성능을 향상시킬 수 있습니다. 확률적 손실(Focal Loss)과 같은 고급 손실 함수를 적용하여 불균형 데이터 처리 성능이 개선되는 것이 관찰되었습니다.
...
### Intuitive Analysis of the Quantization-based Optimization: From Stochastic and Quantum Mechanical Perspectiv (https://arxiv.org/abs/2501.00436)
Comments:
          published in NeurIPS 2024 workshop OPT2024

- **What's New**: 이 논문에서는 목적 함수의 양자화(quantization)에 기반한 최적화 기법을 직관적으로 분석합니다. 양자화는 여러 안장점(saddle points) 및 지역 최소값(local minima)을 포함하는 수준 집합(level set)의 크기를 줄이고, 최적점을 찾는 효과적인 최적화 방법론으로 소개됩니다. 또한, 이 연구는 온도역학(thermodynamics) 및 양자역학(quantum mechanics) 최적화의 방법론을 통합한 양자화 기반 최적화의 이점을 강조합니다.

- **Technical Details**: 연구는 재생산적 양자화(product quantization)의 통계적 분석을 통해 발전된 확률적 최적화 알고리즘을 제안합니다. 이 알고리즘은 목적 함수에 대한 양자화를 적용하고 이를 통해 비선형(non-linear) 최적화 문제를 해결하는 데 초점을 맞추고 있습니다. 스토캐스틱 미분 방정식(stochastic differential equation, SDE)을 도입하여 양자화 기반 최적화의 역학을 설명하며, 보어-라플라시안(Witten-Laplacian)을 통해 지역 최소값을 탈출하는 특성을 보여줍니다.

- **Performance Highlights**: 이 논문에서 제안된 양자화 기반 알고리즘의 성능은 다른 전통적 글로벌 최적화 알고리즘들과 비교하여 검증됩니다. 시뮬레이티드 어닐링(simulated annealing) 및 양자 어닐링(quantum annealing)과 같은 기존 알고리즘과의 성능 비교를 통해 양자화 기반 최적화의 타당성을 입증했습니다. 시뮬레이션 결과는 기준 함수(benchmark functions)를 사용하여 비선형 최적화의 성능 향상을 입증합니다.



### KAE: Kolmogorov-Arnold Auto-Encoder for Representation Learning (https://arxiv.org/abs/2501.00420)
- **What's New**: 본 논문에서는 기존의 다층 퍼셉트론(MLP)과 비교하여 정확성과 해석력을 개선한 콜모고로프-아놀드 네트워크(KAN)를 소개합니다. KAN의 활성화 함수가 학습 가능하다는 점을 활용하여, 이 논문은 KAN과 오토인코더(AE)를 통합한 콜모고로프-아놀드 오토인코더(KAE)를 제안합니다. 이 통합을 통해 데이터의 복잡한 패턴과 비선형 관계를 더 잘 포착할 수 있습니다.

- **Technical Details**: KAE는 KAN 구조를 활용하여 입력 데이터를 저차원 잠재 공간으로 압축하고 다시 복원하는 기능을 개선합니다. KAN 레이어에서 사용되는 학습 가능한 다항식 함수는 기존의 고정된 활성화 함수와 선형 가중치를 넘어서 더 복잡한 표현을 학습할 수 있게 합니다. 이를 통해 KAE는 복원 오류를 줄이고, 정보 표현의 품질을 개선하며, 검색 및 분류와 같은 다운스트림 작업에서 우수한 성능을 발휘합니다.

- **Performance Highlights**: 실험 결과 KAE는 기존의 오토인코더 및 다른 KAN 변형에 비해 뛰어난 성능을 보여줍니다. KAE는 잠재 표현의 질을 향상시키고, 재구성 오류를 감소시키며, 여러 벤치마크 데이터셋에서 검색, 분류, 노이즈 제거 작업에서의 성과를 개선합니다. 이러한 결과는 KAE가 표현 학습에 있어 유용한 도구로 자리 잡을 가능성을 시사합니다.
...
### Toward Information Theoretic Active Inverse Reinforcement Learning (https://arxiv.org/abs/2501.00381)
Comments:
          NeurIPS 2024 Workshop on Bayesian Decision-making and Uncertainty

- **What's New**: 이 논문은 인공지능(AI) 시스템의 의사결정을 인간의 선호에 맞추는 방법에 대한 새로운 접근법을 제안합니다. 인공지능의 자율성이 증가함에 따라, 보상 함수를 수동으로 작성하는 것이 어려워지고 있습니다. 역강화학습(Inverse Reinforcement Learning, IRL) 기술을 통해 이러한 보상을 추론할 수 있지만, 인간의 시연을 확보하는 데 많은 비용이 소모됩니다.

- **Technical Details**: 활성 역강화학습(Active IRL)은 가장 정보가 가득한 시나리오를 전략적으로 선택하여 인간의 시연을 줄이는 방법을 다루고 있습니다. 이 논문에서는 전문가의 전체 경로를 수집하고 이 정보를 효율적으로 활용하기 위한 정보 이론적 접근 방식을 제안합니다. 기대 정보 이득(Expectation Information Gain, EIG) 획득 함수를 적응시켜 문제를 공식화하며, 이를 근거로 한 알고리즘을 발표합니다.

- **Performance Highlights**: 검증 실험을 통해 제안된 방법의 성능이 무작위 샘플링 및 다른 두 가지 기준선보다 우수함을 보여주었습니다. 그리드 월드(Gridworlds) 실험을 통해, 이 접근법은 보다 일반적인 환경으로 확장 가능한 초기 연구로 평가됩니다. 이 연구는 인공지능이 더욱 신뢰성을 가지고 작동할 수 있도록 하는 방향으로 나아가는 계기가 될 것입니다.



### Federated Dropout: Convergence Analysis and Resource Allocation (https://arxiv.org/abs/2501.00379)
- **What's New**: 본 논문에서는 Federated Dropout(FedDrop)이라는 새로운 기법을 제안합니다. 이는 federated learning을 네트워크 엣지에서 효율적으로 배포하기 위한 방법으로, 각 훈련 라운드에서 엣지 장치가 서브 모델만 업데이트하고 전송하여 통신 및 계산 병목 현상을 극복합니다. 그러나 Federated Dropout에 대한 이론적 수렴 분석은 아직까지 부족하여, dropout 비율이 수렴에 미치는 정량적 영향을 규명하는 것이 이 논문의 주요 목표입니다.

- **Technical Details**: Taylor 전개 방식을 사용하여 dropout 비율이 수렴 속도에 미치는 영향을 수학적으로 분석합니다. 특히, dropout 비율이 높은 장치일수록 수렴 속도가 느려짐을 보여주며, 이는 각 훈련 주기별 지연 시간과 전체 수렴까지 소요되는 라운드 간의 균형을 맞추기 위한 이론적 토대를 제공합니다. 또한, 통신 대역폭 배분과 dropout 비율 최적화를 위한 저복잡도 알고리즘을 제안합니다.

- **Performance Highlights**: 제안된 알고리즘의 효과성을 나타내는 수치적 결과를 제공합니다. FedDrop은 각 장치에서 서로 다른 서브넷을 업데이트하면서도 전체 모델 구조를 보존하여 성능을 유지합니다. 이로 인해, 다양한 애플리케이션에서 성능을 최적화할 수 있는 유연성과 조정 가능성을 갖게 됩니다.
...
### A New Dataset and Methodology for Malicious URL Classification (https://arxiv.org/abs/2501.00356)
- **What's New**: 이 논문은 Cybersecurity의 중요한 측면인 악의적인 URL (Uniform Resource Locator) 분류를 다루고 있습니다. 기존의 불완전한 데이터셋으로 인해 어려움을 겪고 있는 이 분야에 대해, 저자들은 'DeepURLBench'라는 새로운 다중 클래스 데이터셋을 제안하여 정상, 피싱 및 악성 URL을 구분합니다. 이 데이터셋은 철저하게 정제되고 구조화되었으며, 기존 데이터셋보다 우수한 대안으로 여겨집니다.

- **Technical Details**: DeepURLBench는 다중 클래스 접근 방식을 채택함으로써 기존의 이진 분류 방식보다 딥러닝 모델의 성능을 향상시킵니다. 또한, URLNet에 개선 사항을 적용하여 DNS와 관련된 특징을 통합하였으며, 이를 통해 모델의 능력을 향상시킵니다. 이러한 접근 방식은 실시간 실행 효율성을 유지하면서도 사이버 보안 애플리케이션에 적합한 성과를 달성하는 균형을 이룹니다.

- **Performance Highlights**: 새롭게 제안된 데이터셋과 개선된 URL 분류기가 실제 공격에 대한 대응을 더욱 효과적으로 만듭니다. 연구에서는, 악의적인 URL을 분류하는 데 있어 이전 모델에 비해 현저히 개선된 성능을 보여줍니다. 이는 사이버 보안 분야에서 딥러닝 기술의 잠재력을 더욱 확장시키며, 실시간 처리의 효율성을 강조합니다.
...
### diffIRM: A Diffusion-Augmented Invariant Risk Minimization Framework for Spatiotemporal Prediction over Graphs (https://arxiv.org/abs/2501.00305)
- **What's New**: 이번 연구에서는 스페이셜-템포럴 예측(over graphs) 문제에 대해 invariant risk minimization (IRM) 프레임워크를 제안합니다. 기존의 IRM은 유클리드 데이터에 최적화되어 있어 그래프 구조 데이터에서 효과적으로 일반화되지 못하는 한계가 있었습니다. 연구의 주된 혁신점은 환경 다양성과 불변 존재 원칙을 결합하여 STPG 문제를 해결하는 것입니다.

- **Technical Details**: 제안된 diffIRM 프레임워크는 두 가지 주요 프로세스, 즉 데이터 증가(data augmentation)와 불변 학습(invariant learning)으로 구성됩니다. 데이터 증가 과정에서는 인과 마스크 생성기(causal mask generator)가 인과 특성을 식별하고, 그래프 기반 확산 모델(graph-based diffusion model)이 환경을 증강하기 위한 스페이셜-템포럴 그래프 데이터를 생성합니다. 불변 학습 과정에서는 증가된 데이터를 활용한 불변 패널티(invariance penalty)가 설계되고, 이는 스페이셜-템포럴 예측 모델 훈련의 정규화기(regularizer)로 작용합니다.

- **Performance Highlights**: 실제 실험은 SafeGraph, PeMS04, PeMS08 세 가지 인간 이동성 데이터 세트를 사용하여 진행되었습니다. 제안된 diffIRM은 기반 모델(baselines)보다 우수한 성능을 보이며, 스페이셜-템포럴 그래프 데이터의 효과적인 예측을 가능하게 합니다. 이 연구는 기존 연구의 한계를 극복하고, OOD 일반화 문제에 대한 새로운 접근을 제공합니다.



### ReFormer: Generating Radio Fakes for Data Augmentation (https://arxiv.org/abs/2501.00282)
- **What's New**: 최근 발표된 ReFormer 모델은 기존의 데이터 수집 방식으로는 얻기 힘든 차세대 무선 통신 시스템의 신호 데이터를 생성하는 데 중점을 둡니다. 이 모델은 기존의 RF 신호를 기반으로 하여 synthetical한 RF 데이터인 RF fakes를 생성하는데, 이는 훈련 데이터 집합을 보강하는 데 매우 유용합니다. 그렇게 함으로써, RF 머신러닝을 위한 데이터 생성에서의 효율성을 크게 향상시킬 수 있습니다.

- **Technical Details**: ReFormer는 transformer 기반의 autoregressive generation을 활용하여 RF 신호의 discrete representations에서 학습된 모델로, 조건부 분포 P(Xg|C)를 통해 맞춤형 데이터를 생성할 수 있습니다. 여기서 C는 생성할 데이터의 특성을 나타내는 프롬프트 역할을 하며, 이는 RF 데이터 세트를 대량으로 생성하여 훈련 데이터의 다양성을 높이는 데 필수적입니다. 또한, 서로 다른 transformer 아키텍처와 설계 선택이 생성된 RF fakes의 품질에 미치는 영향을 평가하는 실험도 포함되어 있습니다.

- **Performance Highlights**: ReFormer는 정밀도(Precision) 및 재현율(Recall), 분류 정확도(Classification Accuracy)와 같은 다양한 메트릭을 사용하여 성능을 평가하였고, 기존의 모델들을 활용한 결과보다 향상된 품질의 RF fakes를 생성하는 것으로 나타났습니다. 이러한 모델은 실제로 RF 데이터 수집이 어려운 환경에서도 유용하게 사용될 수 있어, 무선 통신 분야 및 머신러닝 알고리즘의 효율성을 높일 것으로 기대됩니다.
...
### Scalable Neural Network Verification with Branch-and-bound Inferred Cutting Planes (https://arxiv.org/abs/2501.00200)
Comments:
          Accepted by NeurIPS 2024. BICCOS is part of the alpha-beta-CROWN verifier, the VNN-COMP 2024 winner

- **What's New**: 최근 GCP-CROWN와 같은 절단 평면(cutting-plane) 방법이 신경망(verifier) 검증을 향상시키기 위해 탐구되었고, 이로 인해 많은 발전이 있었다. 하지만 GCP-CROWN은 여전히 외부 혼합 정수 프로그래밍(MIP) 솔버에서 생성된 일반적인 절단 평면에 의존하고 있으며, 이는 대규모 신경망에는 효과적이지 않다는 문제점이 있다. 본 논문에서는 신경망 검증 문제의 구조를 활용하여, 효율적이고 확장 가능한 문제 특정 절단 평면을 생성하는 새로운 접근 방식을 제안한다.

- **Technical Details**: 본 논문에서 제안하는 Branch-and-bound Inferred Cuts with COnstraint Strengthening(BICCOS) 방법은 가지치기(branch-and-bound) 탐색 트리 내에서 검증된 부분 문제의 뉴런 간 논리적 관계를 활용하고, 이러한 관계가 다른 부분 문제에서 금지되도록 절단을 도입한다. 또한, 각 경로의 뉴런에 영향 점수를 부여하여 절단을 강화하는 메커니즘을 개발하고, 다수의 트리에서 추가 절단을 탐지하여 탐색 공간을 축소시키는 멀티트리 검색 기술도 설계하였다.

- **Performance Highlights**: BICCOS는 가지치기 과정에서 수백 개의 유용한 절단을 생성할 수 있으며, 광범위한 벤치마크에서 다른 최신 신경망 검증기들보다 검증 가능한 인스턴스 수를 일관성 있게 증가시킨다. 특히, 이전의 절단 평면 방법들이 처리할 수 없었던 대규모 신경망에서도 우수한 성능을 보인다. 이는 BICCOS가 VNN-COMP 2024에서 우승한 $
alpha,eta$-CROWN 검증기의 일부로 사용되며, 실제 코드 또한 제공된다.
...
### Urban Water Consumption Forecasting Using Deep Learning and Correlated District Metered Areas (https://arxiv.org/abs/2501.00158)
Comments:
          Keywords: urban water management, water consumption, time series forecasting

- **What's New**: 이 논문에서는 Urban Water Distribution Networks (WDNs) 내의 District Metered Areas (DMAs)의 단기 수요 예측을 위한 새로운 접근법을 제안합니다. 제안된 방법은 상관관계가 있는 DMAs의 소비 패턴을 식별한 후 이를 사용하여 심층 학습 모델에 입력으로 제공함으로써 예측을 수행하는 것입니다. 이를 통해 기존의 통계 모델을 초월하는 성과를 거두었으며, 심지어 다수의 DMA의 정보를 활용하는 것이 예측 정확성을 높이는 데 도움이 됨을 보여주었습니다.

- **Technical Details**: 이 연구에서는 심층 학습(Deep Learning) 모델을 사용하여 DMAs의 소비 패턴을 분석하고 예측합니다. DMA 소비를 예측하기 위해 Sliding Window 기법을 도입하여 시간적 요소를 고려한 데이터 분석을 진행하며, 각 DMA의 소비자 데이터와 상관관계가 있는 다른 DMA의 데이터를 결합하여 모델에 입력합니다. 이를 통해 시간에 따른 비선형적인 소비 흐름을 정확히 예측할 수 있는 방법을 제시합니다.

- **Performance Highlights**: 실제 연구를 통해 5개의 DMA 데이터를 분석한 결과, 제안된 심층 학습 모델이 전통적인 통계 모델보다 현저히 높은 예측 성능을 보였음을 확인하였습니다. 또한, 단독으로 신뢰할 수 있는 데이터가 없는 DMA의 경우에도 상관관계가 있는 DMAs의 데이터를 활용함으로써 예측의 정확성을 더욱 개선할 수 있음을 보여주었습니다. 이러한 결과는 물 자원 관리의 효율성과 지속 가능성을 높이는 데 기여할 것으로 기대됩니다.
...
### Efficient and Scalable Deep Reinforcement Learning for Mean Field Control Games (https://arxiv.org/abs/2501.00052)
- **What's New**: 이 논문은 Mean Field Control Games (MFCGs) 문제를 해결하기 위해 확장 가능한 딥 강화 학습(Deep Reinforcement Learning) 접근 방식을 제안합니다. 이전 연구에 기반하여 무한 에이전트 확률적 제어 문제를 마코프 의사결정 과정(Markov Decision Process, MDP)으로 재구성하며, 각 대표 에이전트가 평균 필드 분포와 상호작용합니다. 이를 통해 MFCG의 균형 해결 방안을 근사화하며, 다양한 알고리즘 버전을 제안하여 샘플 효율성을 크게 향상시킵니다.

- **Technical Details**: MFCG 문제는 Hamilton-Jacobi-Bellman (HJB) 및 Fokker-Planck 방정식의 결합된 시스템으로 설명되며, 저자는 이를 해결하기 위해 여러 가지 딥 RL 기술을 도입합니다. 이 알고리즘은 Actor-Critic 프레임워크를 기반으로 하며, 배치 샘플 수집(batch sample collection), 미니 배치(mini-batching), 타겟 네트워크(target network), Proximal Policy Optimization (PPO), 그리고 일반화된 이점 추정(Generalized Advantage Estimation, GAE)과 같은 기술을 활용하여 효율성과 훈련 안정성을 개선합니다. 이러한 방법을 통해 고차원 문제에서도 근사화가 가능해집니다.

- **Performance Highlights**: 제안한 방법은 선형-제곱(LQ) 기준 문제에서 평가되었으며, MFCG 균형에 대한 해석적 솔루션과의 비교를 통해 우수한 성능을 보였습니다. 개선된 알고리즘은 기존 방법보다 1차원 속도에서 훨씬 빠른 수렴 속도를 보여주며, 샘플 효율성 측면에서도 뛰어난 성과를 나타냈습니다. 이러한 결과는 향후 더욱 복잡한 MFCG 문제를 해결하기 위한 가능성을 열어줍니다.
...
### Numerical solutions of fixed points in two-dimensional Kuramoto-Sivashinsky equation expedited by reinforcement learning (https://arxiv.org/abs/2501.00046)
- **What's New**: 이 논문은 Jacobian-Free Newton-Krylov (JFNK) 방법의 효율성을 증대시키기 위해 딥 강화 학습 (DRL) 기법을 결합하여 2D Kuramoto-Sivashinsky Equation (KSE) 내에서 고정점을 식별하는 새로운 접근법을 제시합니다. 또한, 문헌에 보고되지 않은 새로운 고정점 결과를 보고하며, 평행 강화 학습 기법을 기반으로 2D KSE의 제어 최적화를 탐구합니다. 이를 통해 복잡한 동적 시스템에서도 새로운 고정점 솔루션을 찾는 데 있어 JFNK 방법의 개선을 강조합니다.

- **Technical Details**: Kuramoto-Sivashinsky 방정식 (KSE)은 비선형 시스템의 하나로, 복잡한 시공간 동역학을 보입니다. 이 연구에서는 JFNK 방법이 고정점을 찾는 과정에서 좋은 초기 추정치에 의존한다는 점을 강조하며, DRL을 통한 초기 추정치 생성의 역할을 강조합니다. DRL은 시스템 제어를 위한 강화 학습 기술로, KSE의 복잡한 비선형 문제를 해결하기 위한 접근 방식으로 적용됩니다.

- **Performance Highlights**: 연구 결과, JFNK와 DRL의 결합 방법을 통해 2D KSE 내에서 300개 이상의 고정점을 성공적으로 찾아 식별하였습니다. 이 과정에서 DRL을 이용해 초기 추정치를 생성하고, 이를 바탕으로 JFNK 방법을 통해 고정점을 효과적으로 식별했습니다. 이로 인해 2D KSE의 동적 성질에 대한 이해가 한층 강화되었습니다.
...
### Deep Discrete Encoders: Identifiable Deep Generative Models for Rich Data with Discrete Latent Layers (https://arxiv.org/abs/2501.01414)
- **What's New**: 이번 논문에서는 Deep Discrete Encoders (DDEs)라는 새로운 프레임워크를 제안합니다. DDE는 여러 개의 이진 잠재 레이어를 가진 방향 그래픽 모델로, 다양한 데이터 유형에 대한 해석 가능한 딥 생성 모델을 제공합니다. 이 모델은 점점 더 작은 크기의 잠재 레이어로 구성되어 있어, 데이터의 심층적인 해석을 가능하게 합니다.

- **Technical Details**: DDE 모델은 통계적 식별 가능성을 확보하기 위해 이웃 레이어 간의 의존 그래프 구조를 기반으로 한 일반적인 식별 가능성 조건을 제안합니다. 본 연구에서는 이러한 조건이 딥 아키텍처의 해석 가능한 설계를 어떻게 독려하는지를 설명하고 있습니다. 또한, 비선형 스펙트럴 초기화와 패널티된 확률적 근사 EM 알고리즘을 이용한 확장 가능한 추정 파이프라인을 제안하여, 잠재 변수가 다수인 경우에도 효율적인 모델 추정이 가능하도록 합니다.

- **Performance Highlights**: 모든 시뮬레이션 연구는 제안된 알고리즘의 우수한 성능을 입증하며, 다양한 실제 데이터 수집에서 DDE를 성공적으로 적용했습니다. 이들은 계층 토픽 모델링, 이미지 표현 학습, 교육 테스트의 응답 시간 모델링을 포함하여, 매우 해석 가능한 잠재 구조를 추출하여 유용한 데이터 표현을 학습합니다. 논문의 결과는 높은 표현력을 가지면서도 해석 가능한 딥 생성 모델인 DDE의 잠재력을 부각시킵니다.
...
### Learning Spectral Methods by Transformers (https://arxiv.org/abs/2501.01312)
Comments:
          77 pages, 12 figures

- **What's New**: 이 논문에서는 최신 LLM(대규모 언어 모델)의 기본 구성 요소로서 Transformers의 잠재력을 조사합니다. 연구 결과, 다층 Transformers가 충분히 큰 사전 학습 인스턴스 집합을 제공받을 경우, 스스로 알고리즘을 학습하고 새로운 인스턴스에 대해 통계적 추정 작업을 수행할 수 있음을 보여줍니다. 이러한 학습 패러다임은 맥락 내 학습(in-context learning)과는 다르며, 과거 경험을 통해 기술을 배우는 인간 두뇌의 학습 프로세스와 유사합니다.

- **Technical Details**: 이론적으로, 우리는 사전 학습된 Transformers가 스펙트럼 방법(spectral methods)을 학습할 수 있음을 증명하며, 이 과정에서 이분 가우시안 혼합 모델(bi-class Gaussian mixture model)의 분류가 예시로 사용됩니다. 해당 증명은 알고리즘 설계 기법을 사용하여 구성적 방식으로 이루어졌습니다. 우리의 결과는 다층 Transformer 아키텍처의 유사성을 기반으로 실제에서 사용되는 반복 회복 알고리즘(iterative recovery algorithms)과 연결되어 있습니다.

- **Performance Highlights**: 실증적으로, 다층 사전 학습된 Transformer가 무감독 학습(unsupervised learning)에서 강력한 능력을 발휘할 수 있음을 PCA(주성분 분석)와 클러스터링(clustering) 작업을 통해 합성 데이터셋(synthetic datasets) 및 실제 데이터셋(real-world datasets)에서 검증하였습니다. 이러한 결과는 다층 Transformers의 무감독 학습 능력에 대한 신뢰성을 높이고 있습니다.
...
### Marketing Mix Modeling in Lemonad (https://arxiv.org/abs/2501.01276)
- **What's New**: 이번 논문은 마케팅 믹스 모델링(Marketing Mix Modeling, MMM)의 선진적 접근 방식인 베이지안 MMM을 통한 온라인 보험 회사 레모네이드(Lemonade)의 사례를 다룹니다. 베이지안 MMM은 사전 정보(prior information), 불확실성 정량화(uncertainty quantification), 확률적 예측(probabilistic predictions)을 통합할 수 있는 기능을 제공합니다. 논문에서는 레모네이드의 마케팅 활동과 성과 데이터를 수집하고, 각 마케팅 채널의 총 성과에 대한 기여도를 추정하는 과정을 설명합니다.

- **Technical Details**: 논문은 레모네이드의 온라인 광고, 소셜 미디어 및 브랜드 마케팅과 같은 데이터 수집 과정을 강조합니다. 베이지안 프레임워크를 통해 계절성(seasonality), 시장 동향(market trends), 거시경제 지표(macroeconomic indicators) 등 다양한 요소를 고려하여 각 채널의 기여도를 평가하였습니다. 모델의 유효성을 검증하기 위해 A/B 테스트와 슬라이딩 윈도우 홀드아웃 데이터를 사용하여 실제 성과 데이터와 예측 데이터를 비교하였습니다.

- **Performance Highlights**: 모델의 예측 결과는 A/B 테스트 성과와 일치하며 실제적인 조정 가능성을 보여주었습니다. 또한, 모델의 민감도를 평가하기 위해 수많은 시나리오 분석(scenario analyses)을 시행하였고, 마케팅 믹스의 변화가 판매에 미치는 영향을 평가할 수 있었습니다. 이 모델을 통해 레모네이드는 마케팅 전략을 조정하고 예산을 보다 효율적으로 할당하는 인사이트를 얻을 수 있었습니다.
...
- **What's New**: 이번 논문에서는 RingFormer라는 새로운 신경 보코더(neural vocoder)를 제안합니다. 이 모델은 경량(transformer) 변형인 Conformer에 링 어텐션(ring attention) 메커니즘을 도입하여 오디오 생성의 효율성을 높였습니다. RingFormer는 지역(local) 세부사항을 효과적으로 포착하면서도 글로벌(global) 정보를 통합할 수 있도록 설계되어 있습니다.

- **Technical Details**: RingFormer는 샘플 수준에서 긴 오디오 신호를 생성해야 하는 신경 보코더의 도전 과제를 해결하기 위해 개발되었습니다. 이 모델은 두 개의 판별기(discriminator)를 사용하는 적대적 훈련(adversarial training) 방법으로 훈련되며, 텍스트-투-스피치(text-to-speech) 모델 VITS의 디코더에 적용됩니다. 링 어텐션은 전체 시퀀스에 대한 처리 능력을 극대화하며 실시간(real-time) 오디오 생성이 가능합니다.

- **Performance Highlights**: 실험 결과, RingFormer는 HiFi-GAN, iSTFT-Net 및 BigVGAN과 같은 최신 보코더들과 비교했을 때 동등하거나 우수한 성능을 보였습니다. 특히 실시간 오디오 생성에서 두드러진 성과를 이루어냈으며, 다양한 객관적(objective) 및 주관적(subjective) 메트릭을 통해 성능을 비교했습니다. 관련 코드와 오디오 샘플은 GitHub에서 확인할 수 있습니다.



### Ultrasound Lung Aeration Map via Physics-Aware Neural Operators (https://arxiv.org/abs/2501.01157)
- **What's New**: 이번 연구에서는 폐 초음파 검사에 대한 새로운 AI 모델인 LUNA를 제안합니다. LUNA는 기존의 B-모드 이미지 해석을 넘어서 RF(data)를 직접 처리하여 폐 공기 분포 맵을 재구성합니다. 이를 통해 폐 질환의 진단에 있어 독립적이고 정량적인 접근 방식을 제공하여 전문가의 경험에 의존할 필요를 줄입니다.

- **Technical Details**: LUNA는 Fourier 신경 연산자(Fourier Neural Operator)를 기반으로 하여, RF 데이터를 효율적으로 처리하고 폐 공기 분포 맵을 정확하게 복원합니다. 이 모델은 시뮬레이션된 많은 데이터와 적은 양의 실제 데이터를 가진 렌즈를 활용하여 훈련되었습니다. 최종적으로 LUNA는 ex-vivo 돼지 폐 스캔에서 9%의 공기 추정 오차를 달성하여 기존 평가 방법보다 우수한 성능을 보였습니다.

- **Performance Highlights**: LUNA의 성능은 기존의 비정량적 폐 초음파 점수 시스템보다 뛰어난 결과를 보여주며, 예상한 대로 0.1의 오류 범위 내에서 공기량을 정확하게 추정할 수 있습니다. 이러한 접근법은 기존의 융통성 있는 방법보다 진단의 재현성과 유용성을 향상시키는데 기여할 것으로 기대됩니다.
...
### Regularized Proportional Fairness Mechanism for Resource Allocation Without Money (https://arxiv.org/abs/2501.01111)
- **What's New**: 이번 연구에서는 자원이 제한된 환경에서 자기 이익을 추구하는 에이전트 간의 자원 할당을 위한 새로운 메커니즘 설계를 제안합니다. 제안하는 Regularized Proportional Fairness Network (RPF-Net)는 기존의 비금전적 할당 메커니즘의 한계를 극복하고, 사회적 복지를 최대화하면서도 효율적인 유인 호환성(incentive compatibility) 보장을 목표로 합니다. 또한, 이 구조는 에이전트가 유틸리티를 달리 보고하는 경우 발생하는 기회를 줄이도록 설계되었습니다.

- **Technical Details**: RPF-Net은 자원 할당 문제에 적합하도록 고안된 혁신적인 신경망 아키텍처로, 선형 정규화(regularization)를 추가하여 비정확한 보고를 견딜 수 있는 강인성을 제공합니다. 이 연구는 PF 메커니즘의 출력에 대한 (sub) gradient를 도출하여, 소셜 복지 최대화를 위한 최적화를 계산하고, 실험적으로 그 성능을 검증합니다. 얻어진 일반화 경계(generalization bounds)는 유한 샘플에서 훈련된 RPF-Net의 성능을 보장합니다.

- **Performance Highlights**: RPF-Net은 다양한 문제 차원과 분포 변화(distribution shift)에서 실험적으로 검증되었으며, 전통적인 메커니즘에 비해 재정적 인센티브 없이도 높은 효율성을 유지합니다. 연구 결과는 RPF-Net이 분포의 변화에 대해 강력한 견고성을 나타내며, 적대적 오염이 발생한 훈련 데이터에서도 우수한 성능을 유지함을 보여줍니다. 이는 비금전적 자원 할당의 연구에 중요한 기여를 합니다.
...
### An Efficient Outlier Detection Algorithm for Data Streaming (https://arxiv.org/abs/2501.01061)
Comments:
          12 pages, 10 figures

- **What's New**: 이 논문은 온라인 이상 탐지를 위한 새로운 접근 방법인 Efficient Incremental LOF (EILOF) 알고리즘을 제안합니다. 이 알고리즘은 새로운 데이터 포인트의 LOF 점수만을 계산하고 기존 데이터 포인트의 LOF 점수를 변경하지 않음으로써 효율성을 높입니다. 기존의 Incremental LOF (ILOF) 알고리즘보다 계산 비용을 크게 줄이면서도 탐지 정확도를 높이는 것을 목표로 합니다.

- **Technical Details**: EILOF 알고리즘은 기존 데이터에 대한 정확한 LOF 점수를 계산하지 않고 새로운 데이터 포인트에 대한 LOF 점수만을 업데이트합니다. 이 방법은 데이터 스트리밍 상황에서 LOF 점수를 재계산함으로써 발생하는 높은 계산 부담을 줄이는 데 유리하며, 특히 대량의 데이터가 들어올 때 효율성을 극대화합니다. 알고리즘은 최적화된 데이터 구조와 근사 방법을 사용하여 동적 환경에서도 견고한 성능을 보장합니다.

- **Performance Highlights**: EILOF 알고리즘은 다양한 시나리오에서 ILOF 알고리즘보다 우수한 성능을 보여주고, 특히 데이터 스트리밍의 양이 증가함에 따라 더욱 두드러진 성과를 보입니다. 실험 결과, EILOF는 세이프티(즉, 계산 비용의 절감), 탐지 정확도의 체계적 개선을 이루어내며, 이는 금융 사기 탐지, 센서 모니터링, 네트워크 침입 탐지와 같은 실제 어플리케이션에서도 효과적입니다.
...
### HPC Application Parameter Autotuning on Edge Devices: A Bandit Learning Approach (https://arxiv.org/abs/2501.01057)
- **What's New**: 이 연구에서는 자원 제약이 있는 엣지 장치에서 HPC(고성능 컴퓨팅) 응용 프로그램의 성능을 최적화하기 위한 LASP(Lightweight Autotuning of Scientific Application Parameters)라는 새로운 전략을 제안합니다. LASP는 온라인 탐색 및 활용에 중점을 두는 다중 무장 강도(MAB) 기술을 활용하여 엣지 장치의 동적인 환경에 적응합니다. 이를 통해 LASP는 복잡한 매개변수 구성 문제를 해결하고, 자원이 제한된 환경에서도 효율적으로 HPC 응용 프로그램을 실행할 수 있게 합니다.

- **Technical Details**: LASP는 엣지 장치에서 실행되는 HPC 응용 프로그램을 위한 자동 튜닝(search space) 기법으로 설계되었습니다. 이 방법은 저자원 환경을 고려하여 고성능 시행을 가능하게 하며, 기본적으로 응용 프로그램을 저정도(Low Fidelity)로 실행하여 최적의 매개변수를 식별한 후, 이러한 매개변수를 전통적인 HPC 플랫폼으로 전달합니다. 이러한 과정은 시간이 많이 소요되는 수동 매개변수 조정의 필요성을 줄여줍니다.

- **Performance Highlights**: 실험 결과에 따르면, LASP는 네 가지 HPC 응용 프로그램(Lulesh, Kripke, Clomp, Hypre)에 대해 적용되었으며, 매개변수 탐색의 효율성을 크게 향상시켰습니다. LASP는 동적 환경에서 사용자 요구에 적응하여 최적 매개변수를 찾아내어, HPC 응용 프로그램의 성능을 현저히 개선하는 것으로 나타났습니다. 이 연구는 엣지 장치에서의 HPC 응용 프로그램 최적화를 위한 혁신적인 접근 방식을 제시하여, 미래 연구 방향을 제안합니다.
...
### On the Implementation of a Bayesian Optimization Framework for Interconnected Systems (https://arxiv.org/abs/2501.00967)
Comments:
          32 pages, 12 figures

- **What's New**: 이번 연구에서는 새로운 그레이 박스(Bayesian optimization, BO) 최적화 방식인 BOIS를 제시합니다. BOIS는 비용이 많이 드는 시스템의 성능을 모델링하는 데 있어 기존 방식의 한계를 극복할 수 있는 방법론으로, 구조적 지식을 효과적으로 활용할 수 있게 합니다.

- **Technical Details**: BOIS는 시스템 성능 함수 $f(x)$에 대한 적응형 선형화를 활용하여 결합 함수의 통계적 모멘트의 해석적 표현을 도출합니다. 이는 기존의 그레이 박스 모델링 접근 방식에서 발생하는 복잡한 문제를 수치적으로 해결하는 것보다 효율적인 방법입니다.

- **Performance Highlights**: 실험에서는 BOIS가 표준 BO 및 기존의 그레이 박스 BO 알고리즘과 비교하여 화학 공정 최적화 및 설계 케이스 스터디를 통해 그 성능을 입증했습니다. 결과적으로 BOIS는 기존의 그레이 박스 방법보다 동등하거나 더 나은 성과를 보였으며, 계산 집약도가 낮다는 장점을 확인했습니다.
...
### Tight Constraint Prediction of Six-Degree-of-Freedom Transformer-based Powered Descent Guidanc (https://arxiv.org/abs/2501.00930)
Comments:
          AIAA SCITECH 2025 Forum

- **What's New**: 이 논문에서는 Transformer 기반의 Successive Convexification (T-SCvx)를 소개하며, 이는 Transformer 기반의 Powered Descent Guidance (T-PDG)의 확장판입니다. T-SCvx는 6개 자유도(DoF)의 연료 최적 powered descent 경로 생성을 위한 효율성을 significantly 향상시키는 것을 목표로 합니다. 데이터셋의 회전 불변 변환을 통해 비선형 및 비볼록(powered descent guidance) 문제에서의 샘플 효율성과 솔루션 품질을 크게 개선합니다.

- **Technical Details**: 이 논문의 T-SCvx는 최적 제어 문제에서의 강한 제약 조건을 학습하여 최소한의 크기로 축소된 문제를 초기화합니다. 그런 후, 이 축소된 문제의 솔루션을 사용하여 직접 최적화 솔버를 웜 스타트(warm-start) 합니다. SCvx는 비볼록 전체 문제를 일련의 볼록 서브 문제로 변환하여 성공적으로 해결합니다.

- **Performance Highlights**: T-SCvx는 6-DoF 화성 착륙 응용 문제에서 온보드 실시간 유도 경로 계산을 가능하게 합니다. 이 방법은 과거 3-DoF LCvx 문제의 경험을 바탕으로 학습하며, 2,000개 미만의 샘플로 최적화 문제의 샘플 수를 줄여줍니다. T-SCvx는 효율적인 서브문제 생성을 지원하고, 최적화 구현을 더 간소화하여 연료 최적 경로 생성을 현저하게 개선합니다.



### Diffusion Policies for Generative Modeling of Spacecraft Trajectories (https://arxiv.org/abs/2501.00915)
Comments:
          AIAA SCITECH 2025 Forum

- **What's New**: 이 논문에서는 기계 학습 기반의 궤도 생성 방법의 단점을 극복하고, 적은 양의 예제(프레임워크)로부터 다수의 다양한 궤도 설계 사양을 지원하기 위해 조합적 확산 모델(compositional diffusion modeling)을 활용하였습니다. 기존의 딥러닝 기법은 특정 궤도 최적화 문제에 대한 구조만 학습할 수 있었으나, 확산 모델은 확률 밀도 함수(probability density function, PDF)로 해결책을 표현함으로써 다중 궤도 설계 제약 조건을 포함한 PDF의 조합을 가능하게 합니다.

- **Technical Details**: 궤도 최적화 문제는 다차원 시스템의 동적 실행 가능성을 위해서는 조작이 필요한데, 기존의 방사선 내구성(sensitive) 비행 컴퓨터는 계산 자원이 제한적입니다. 이들 문제를 해결하기 위해 이전 연구들은 문제 매핑(problem-to-solution mapping) 학습을 통해 기계 학습 기반 접근법을 이용하였으나, 이러한 방식은 단일 궤도 최적화 공식에만 국한되며 종종 새로운 데이터셋을 필요로 합니다. 조합적 확산 모델을 통한 새로운 방법은 이 문제를 해결해 줄 수 있으며, 궤도 샘플을 동시에 예측할 수 있습니다.

- **Performance Highlights**: 이 논문에서 제안된 방법은 다중 착륙지 선택을 위한 유효하고 일반화 가능한 궤도를 생성하는 데 성공적으로 적용되었습니다. 이 방식은 저위험 착륙 지역의 분포를 자동차하고 제약 조건을 적용하여 보다 효율적으로 문제를 해결할 수 있도록 돕습니다. 수행 결과, 확산 학습 모델은 기존의 딥러닝 모델보다 충돌 회피 제한을 위반하는 비율을 크게 줄이며 더 높은 효율성을 보여주었습니다.



### Spatial Temporal Attention based Target Vehicle Trajectory Prediction for Internet of Vehicles (https://arxiv.org/abs/2501.00890)
- **What's New**: 이 연구에서는 Spatio Temporal Attention 기반의 Target Vehicle Trajectory Prediction (STATVTPred) 방법론을 소개합니다. 이 방법은 GPS 위치 확인 기술을 통합하여 대상 차량의 미래 경로를 동적으로 예측하고 복합적인 시공간 궤적 데이터를 활용합니다. 기존의 Trajectory 예측 기술의 한계를 극복하기 위해 그래프 주의 네트워크(Graph Attention Networks, GATs)와 Transformer 기술을 사용하여 공간적 및 시계열 특성을 추출합니다.

- **Technical Details**: STATVTPred는 도로 네트워크를 지향 그래프로 표현하여 차량 궤적을 훨씬 더 정확하게 예측하기 위해 공간적 특성을 추출합니다. 그래프 주의 네트워크를 활용하여 도로 네트워크에서 공간적 피처를 얻고, Transformer를 사용하여 입력 시퀀스의 시계열 특성을 추출합니다. 이를 기반으로 로컬 도로 네트워크 구조 맵과 결합하여 연속적인 궤적 시퀀스를 필터링하여 차량 궤적 예측을 수행합니다.

- **Performance Highlights**: 실험 결과, STATVTPred는 베이징 및 충다우 데이터셋에서 Transformer 모델보다 각각 6.38% 및 10.55% 높은 평균 일치율(Average Match Rate, AMR)을 달성했습니다. 또한 LSTM Encoder-Decoder 모델과 비교했을 때 같은 데이터셋에서 AMR을 각각 37.45% 및 36.06% 향상시켰습니다. 이러한 결과는 STATVTPred가 물류 및 교통 시나리오에서 궤적 예측을 처리하는 새로운 접근 방식임을 입증합니다.
...
### A Graphical Approach to State Variable Selection in Off-policy Learning (https://arxiv.org/abs/2501.00854)
Comments:
          25 pages (not including appendix and references), 10 figures, 2 tables

- **What's New**: 이번 연구는 Sequential decision problems에 대한 오프 폴리시(Off-policy) 학습의 새로운 접근 방안을 제시합니다. 특히, 동적 치료 체계(Dynamic Treatment Regimes, DTRs)와 오프라인 강화 학습(Offline Reinforcement Learning, RL)이라는 두 가지 기존 설정의 간격을 메우고자 합니다. 이 논문에서는 인과 추론(Causal Inference) 이론을 활용하여 DTR과 MDPs를 모두 포함하는 그래픽스 기반 식별 기준을 제공합니다.

- **Technical Details**: 연구진은 비순환 유향 혼합 그래프(Acyclic Directed Mixed Graph, ADMG)를 기반으로 인과 추론 이론을 적용하여 일반적인 의사결정 프로세스에서의 그래픽 식별 기준을 제시하였습니다. 이러한 기준은 DTRs와 MDPs 사이의 복잡한 관계를 설명하는데 도움을 주며, 기존 문헌에서 자주 간과되는 인과 가정(Causal Assumptions)의 명확한 설명도 포함되어 있습니다.

- **Performance Highlights**: 시뮬레이션 연구를 통해 컨테이너 물류에서의 동적 가격 책정 문제를 실증적으로 분석하였습니다. 그래픽 기준이 위반될 경우 어떻게 서브옵티멀(Suboptimal) 정책으로 이어지는지를 보여주며, 이러한 결과는 실제 문제 해결에서 오프 폴리시 학습의 중요성을 강조합니다.
...
### Beyond Static Datasets: A Behavior-Driven Entity-Specific Simulation to Overcome Data Scarcity and Train Effective Crypto Anti-Money Laundering Models (https://arxiv.org/abs/2501.00757)
- **What's New**: 이 연구에서는 다양한 거래 유형을 생성하고, 자금세탁에 관련된 실제 주소를 감지하는 데 도움을 주기 위해 행위 기반의 엔티티 특정 자금세탁 비슷한 거래 시뮬레이션을 제안합니다. 최근 머신러닝 모델의 효과적인 훈련을 위해 필요한 데이터셋이 매우 제한적이라는 문제를 지적하고 있습니다. 이를 해결하기 위해 시뮬레이터를 통해 생성된 사용자 맞춤형 데이터셋을 다루고 있으며, 이는 기존 데이터셋의 동적, 불균형 문제를 극복할 수 있습니다.

- **Technical Details**: 논문은 시뮬레이터의 설계 및 아키텍처를 설명하고 있으며, 이를 바탕으로 생성된 맞춤형 데이터셋을 통해 자금세탁 탐지 모델을 훈련했습니다. 연구에서는 다양한 거래 모델을 embedding하여 여러 엔티티의 행동을 반영하도록 설계되어 있습니다. 각 모델은 자금세탁에 연루된 주소를 정확히 감지할 수 있도록 훈련되었으며, 이는 매우 복잡한 블록체인 환경에서의 거래 패턴을 분석하는 데 기여합니다.

- **Performance Highlights**: 시뮬레이터를 통해 생성된 데이터셋으로 훈련된 모델들은 자금세탁에 연루된 실제 주소를 정확하게 감지하는 성능을 보였습니다. 이는 기존의 정적이고 불균형한 데이터셋에 비해 훨씬 나은 결과를 보여주며, 데이터 양상에 따라 모델의 유연성을 향상시켰습니다. 이러한 접근 방식은 자금세탁 탐지 분야의 연구와 실무에 큰 기여를 할 것으로 기대됩니다.
...
### A Distributional Evaluation of Generative Image Models (https://arxiv.org/abs/2501.00744)
- **What's New**: 최근 생성 모델(generative models)의 발전은 현실적인 샘플을 합성할 수 있는 다양한 접근 방법을 탄생시켰습니다. 그러나 이러한 발전에도 불구하고, 합성 샘플과 목표 분포(target distribution) 간의 분포 일치를 통계적으로 평가하는 것은 여전히 큰 도전입니다. 본 연구에서는 기존 평가 지표들이 뒷받침할 수 없는 분포 간 차이를 효과적으로 측정하기 위한 새로운 지표인 Embedded Characteristic Score (ECS)를 제안합니다.

- **Technical Details**: ECS는 학습된 분포와 목표 샘플 분포 간의 분포 일치를 평가하는 종합적인 지표로, 특징 임베딩(feature embeddings)과 특징 함수 변환(characteristic function transforms)이라는 두 가지 주요 구성 요소로 이루어져 있습니다. 특성 임베딩을 사용하여 물체를 유클리드 공간(Euclidean space)으로 표현하고, 샘플 이미지를 비교할 특징 함수의 값을 원점 근처에서 추정합니다. 이러한 방법을 통해 ECS는 고차 모멘트(higher moments)와 꼬리(tail) 정보를 효과적으로 포착할 수 있습니다.

- **Performance Highlights**: ECS는 기존의 Fréchet Inception Distance (FID)와 같은 평가 지표를 보완하여, 학습된 분포와 목표 분포 간의 차이를 더 잘 측정할 수 있는 가능성을 보여줍니다. 본 연구에서는 ECS의 성능을 검증하기 위해 시뮬레이션과 실제 이미지 데이터셋을 활용한 경험적 연구(empirical study)를 진행하였습니다. 이 연구결과 ECS는 실제 이미지와 합성 이미지 간의 차이를 정확하게 포착하는 데 효과적임을 시사합니다.



### Learning Weather Models from Data with WSINDy (https://arxiv.org/abs/2501.00738)
- **What's New**: 이 논문은 기후 변화 예측에 대한 데이터 기반 접근법에 대한 최근의 관심을 반영하고 있습니다. 이전의 수치적 기상 예측(NWP) 방법론과 달리, 이 연구에서는 기상 현상을 지배하는 부분 미분 방정식을 explicit하게 발견하여 해석 가능한 수학적 모델을 제시하고 있습니다. 특히, Weak form Sparse Identification of Nonlinear Dynamics (WSINDy) 알고리즘을 통해 시뮬레이션된 데이터와 동화된 데이터를 사용하여 효과적인 기상 모델을 학습하는 과정을 보여줍니다.

- **Technical Details**: WSINDy 알고리즘은 고차원 유체 데이터에 대한 처리에 적합하게 조정되어 있습니다. 이 연구에서는 통합 불가능한 항을 처리하기 위한 새로운 접근법도 개발되었습니다. 접근 방식은 관측 노이즈에 대한 저항력이 강화된 WSINDy로, 데이터가 국소화된 테스트 함수에 통합됨으로써 신호 우세 모드와 특정 시간 및 길이 스케일을 적절하게 추출할 수 있습니다.

- **Performance Highlights**: 저자들은 WSINDy 알고리즘을 사용하여 다양한 기상 데이터와 관련된 효과적인 부분 미분 방정식 모델을 발견할 수 있었음을 보여줍니다. 특히, 논문에서는 시뮬레이션된 데이터 및 동화된 데이터를 통해 기존의 기상 예측 모델에 비해 해석 가능성을 높인 결과를 제시하고 있습니다. 이러한 결과는 대부분의 기존 알고리즘들이 해석 불가능한 다수의 매개변수로 인해 제한된 성과를 보이는 것과 대조적으로, 기후 모델링의 새로운 가능성을 열어줍니다.
...
### Enhancing Unsupervised Feature Selection via Double Sparsity Constrained Optimization (https://arxiv.org/abs/2501.00726)
- **What's New**: 이 논문에서는 새로운 방식의 비지도 특성 선택(unsupervised feature selection, UFS) 방법인 DSCOFS를 제안합니다. DSCOFS는 전통적인 주성분 분석(principal component analysis, PCA) 프레임워크에 이중 희소성(double sparsity) 제약 최적화 기법을 포함시켜, 보다 유용하고 차별화된 특성 집합을 선택하는 데 도움을 줍니다. 이러한 접근법은 특히 고차원 데이터의 구조적 특성이 다양할 때, 기존의 방법들과 비교해 더욱 뛰어난 성능을 발휘할 수 있음을 보여줍니다.

- **Technical Details**: 이 연구에서는 이중 희소성 제약 조건으로 구성된 새로운 PCA 모델을 개발하고, 💡ℓ_{2,0} -norm과 ℓ_0-norm을 통해 데이터를 보다 효과적으로 선택할 수 있는 방법을 제안합니다. 또한, 이 모델을 해결하기 위한 근접 교대 최소화(proximal alternating minimization) 알고리즘을 도입하였으며, 이 과정에서 발생하는 하위 문제들은 폐쇄형 해를 가질 수 있습니다. 따라서 이 방식은 계산 효율성을 크게 향상시킬 수 있습니다.

- **Performance Highlights**: 성능 실험에서는 제안된 DSCOFS 방법이 평균 클러스터링 정확도(average clustering accuracy, ACC) 및 정규화된 상호정보(normalized mutual information, NMI)에서 기존 최첨단 방법들에 비해 각각 최소 3.34% 및 3.02%의 향상을 보였습니다. 결과적으로 DSCOFS는 비지도 특성 선택 분야에 새로운 시각을 제공하며, 통계적 검증을 통해 그 효과성을 입증한 것으로 평가됩니다.
...
### Beyond Model Scale Limits: End-Edge-Cloud Federated Learning with Self-Rectified Knowledge Agglomeration (https://arxiv.org/abs/2501.00693)
Comments:
          16 pages, 7 tables, 6 figures. arXiv admin note: text overlap with arXiv:2312.11489

- **What's New**: 이 논문은 End-Edge-Cloud Collaboration (EECC) 환경에서의 Hierarchical Federated Learning (HFL) 방식의 한계점을 극복하기 위해 새로운 방법론인 FedEEC를 제안합니다. FedEEC은 다양한 컴퓨팅 노드 간의 효과적인 협업을 가능하게 하며, 모델 크기와 일반화 능력이 증가하는 구조를 구현합니다. 또한, 모델 간의 지식 전이와 최적화 과정에서 발생할 수 있는 문제를 해결하기 위한 두 가지 주요 혁신도 소개됩니다.

- **Technical Details**: FedEEC는 Bridge Sample Based Online Distillation Protocol (BSBODP)과 Self-Knowledge Rectification (SKR)라는 두 가지 핵심 기술을 포함하고 있습니다. BSBODP는 인접한 노드 간에 생성된 브리지 샘플을 통해 지식을 전이할 수 있도록 하여, 다양한 모델 간 협력을 지원합니다. SKR은 전달된 지식을 개선하여 오차를 줄이고, 정확한 모델 업데이트를 보장하기 위해 여러 단계를 통해 지식을 정제합니다.

- **Performance Highlights**: 실험 결과, FedEEC는 SVHN, CIFAR-10, CINIC-10 데이터셋에서 기존의 최첨단 방법론에 비해 클라우드 모델 정확도를 향상시킴을 입증했습니다. FedEEC는 네트워크의 동적인 노드 이동성에도 원활하게 대응할 수 있어, EECC 환경에서의 다양한 요구 조건을 충족하는 강력한 성능을 보였습니다. 이를 통해 FedEEC는 보다 효과적인 AI 모델의 훈련과 지식 강화가 가능한 유망한 프레임워크임을 확인하게 됩니다.
...
### Finding Missed Code Size Optimizations in Compilers using LLMs (https://arxiv.org/abs/2501.00655)
Comments:
          Accepted to appear in The International Conference on Compiler Construction (CC) 2025

- **What's New**: 이번 연구에서는 컴파일러의 최적화 기회를 식별하기 위해 차별적 테스트(differential testing)를 적용했습니다. 이전에는 주로 입력에 대해 올바른 코드를 생성하는지 확인하는 데 초점을 맞추었으나, 성능에 대한 검증은 소홀히 여겨졌습니다. 우리는 대형 언어 모델(LLMs)과 여러 차별적 테스트 전략을 결합하여 C/C++ 컴파일러에서 누락된 코드 크기 최적화 기회를 탐지하는 새로운 테스트 접근 방식을 개발했습니다.

- **Technical Details**: 우리는 무작위 코드를 생성하는 복잡한 작업을 상용 LLM에 외주 주어 쉽게 처리할 수 있습니다. 이를 통해 비정상적인 컴파일러 동작을 식별하기 위한 휴리스틱(heuristics)과 분석을 사용합니다. 이 접근법은 150줄도 안 되는 코드로 구현할 수 있으며, 이는 새롭게 추가하거나 변경하기 대단히 쉽습니다.

- **Performance Highlights**: 이 접근 방식을 C/C++에서 Rust 및 Swift로 간단히 전환하여 각각에서 버그를 찾아냈습니다. 현재까지 우리는 생산 컴파일러에서 24개의 확인된 버그를 보고했습니다. LLM 지원 테스트는 실제 컴파일러 내에서 최적화 버그를 검출하는 유망한 방법으로 분석됩니다.
...
### Different thresholding methods on Nearest Shrunken Centroid algorithm (https://arxiv.org/abs/2501.00632)
Comments:
          30 pages 9 figures

- **What's New**: 본 논문에서는 Nearest Shrunken Centroid (PAM) 알고리즘에 다양한 thresholding 기법이 적용되는 효과를 연구합니다. PAM 알고리즘은 일반적으로 soft thresholding을 사용하여 높은 연산 효율성과 분류 정확도를 달성하나, 과도한 feature 선택 때문에 후속 연구가 어려워질 수 있습니다. 이를 개선하기 위해, 하드(thresholding)와 순서(thresholding) 기법을 적용하여 feature 수를 줄이는 방법을 제안합니다.

- **Technical Details**: 하드(thresholding)와 순서(thresholding) 기법을 사용하여 PAM 알고리즘을 확대하고, 더 나은 thresholding 파라미터 추정을 위해 깊은 검색(deep search) 알고리즘을 도입합니다. 각 thresholding 기법을 적용한 후, 원래 PAM 알고리즘과 실데이터 및 몬테카를로 연구를 통해 비교 평가합니다. 이를 통해 파라미터 추정 정확도를 우선시하고 모델의 간결성을 높입니다.

- **Performance Highlights**: 수정된 알고리즘은 기존의 PAM 알고리즘에 비해 더 높은 암 예측 정확도를 달성하고, 선택된 feature 수를 현저히 줄이는 결과를 보였습니다. 이러한 결과는 후속 연구를 위한 더 간결한 모델을 제공하며, 높은 차원의 데이터에서 좋은 성능을 발휘하는 PAM의 활용도를 더욱 높이는 데 기여합니다.
...
### Per Subject Complexity in Eye Movement Prediction (https://arxiv.org/abs/2501.00597)
Comments:
          14 pages

- **What's New**: 본 연구는 가상 현실(VR) 장치에서의 시선 추적 시스템으로 인한 지연(latency)을 보완하기 위한 시선 이동 예측(eye movement prediction)의 복잡성을 분석합니다. 연구팀은 Long Short-Term Memory network(LSTM), Transformer-based network for multivariate time series representation learning(TST), Kalman Filter 프레임워크를 활용한 Oculomotor Plant Mathematical Model(OPKF)의 세 가지 모델을 사용하여 평가하였습니다. 모델의 성능을 주제별로 평가하고, 각 주제 및 모델의 복잡성을 개별적으로 분석하여 모델의 일반화 가능성을 높이기 위한 방법을 논의합니다.

- **Technical Details**: 올바른 시선 이동 예측을 위해 세 가지 모델(LSTM, TST, OPKF)을 사용했습니다. 연구는 데이터 수집 과정에서 노이즈(noise)와 불확실성으로 인해 발생할 수 있는 다양한 요소를 고려하여 각 모델의 예측 성능을 평가합니다. 특히, 각 주제의 시선 이동 패턴이 가지는 복잡성을(Per-subject complexity) 분석하여 예측 오류의 원인을 깊이 있게 탐구합니다.

- **Performance Highlights**: 각 모델은 예측 성능의 유사한 경향을 보였지만, 특정 주제에 따라 예측이 더 어려운 경향이 나타났습니다. 기존 연구들과의 비교를 통해, 시선 이동 예측의 성능 차이에 대한 새로운 인사이트를 제공합니다. 또한, 각 주제의 데이터를 기반으로 한 분석은 모델의 성능을 향상시킬 수 있는 구체적인 문제와 해결 방법을 제시합니다.
...
### Polynomial time sampling from log-smooth distributions in fixed dimension under semi-log-concavity of the forward diffusion with application to strongly dissipative distributions (https://arxiv.org/abs/2501.00565)
- **What's New**: 이번 논문에서는 고정 차원에서 다항 복잡도를 가진 확률적 샘플링 알고리즘을 소개합니다. 이 알고리즘은 최근의 확산 모델(diffusion models)에서의 발전을 활용하여, 일반 오르슈타인-울렌벡 과정의 중간 점수에 대한 정확한 추정을 통해 샘플링을 달성하는 방법을 제시합니다. 특히, 로그 매끄러운(data) 분포에서 샘플링 문제를 정규성 추정 문제로 변환하는 방법을 보여줍니다.

- **Technical Details**: 제안하는 알고리즘은 저렴한 계산 비용으로 점수를 접근하는 것에 중점을 두고 있으며, 추정기의 분산을 전방 과정(forward process)의 부드러움 속성과 관련짓습니다. 논문에서는 샘플링하려는 밀도가 L-log-smooth하고 전방 과정이 반 로그-오목(semi-log-concave)일 때의 복잡도를 수학적으로 분석합니다. 특히, 알고리즘의 성능은 복잡성을 O(d^7L^{d+2}epsilon^{-2(d+3)}(L+beta)^2d^{2(d+1)})로 제시됩니다.

- **Performance Highlights**: 이 알고리즘은 비정상 상태에서 강한 로그 오목성을 가진 분포에 대해 샘플링을 할 때 지수적으로 향상된 복잡성을 도출합니다. 이전의 복잡도 O(poly(e^{RL^2}, L, alpha^{-1}, log(epsilon^{-1})))에 비해 매우 개선된 성능을 보여줍니다. 또한, 이 연구는 분포가 비대칭적일 때의 샘플링 문제를 해결하는 데에도 기여할 수 있는 중요한 결과를 보여줍니다.
...
### Finding the Underlying Viscoelastic Constitutive Equation via Universal Differential Equations and Differentiable Physics (https://arxiv.org/abs/2501.00556)
- **What's New**: 이 연구는 Universal Differential Equations (UDE)와 differentiable physics를 결합하여 점성탄성 유체를 모델링합니다. 전통적인 differential equations, neural networks 및 numerical methods를 통합하여 constitutive models의 누락된 항을 복원하는 방법을 제시합니다. 이 연구의 주요 초점은 Upper Convected Maxwell (UCM), Johnson-Segalman, Giesekus, 및 Exponential Phan-Thien-Tanner (ePTT)과 같은 네 가지 점성탄성 모델을 분석하는 것입니다.

- **Technical Details**: 연구 방법론은 합성 데이터셋을 사용하여 다양한 실험 조건에서 테스트되었습니다. 특히 oscillatory 및 startup 흐름을 포함한 실험이 진행되었습니다. UDE 프레임워크는 대부분의 모델에 대해 shear 및 normal stresses를 효과적으로 예측하지만, ePTT 모델에 적용할 때 일부 한계를 보였습니다. 이를 통해 UDE가 유체역학에서의 잠재력을 강조하며, 방법론 개선의 중요한 영역을 식별할 수 있습니다.

- **Performance Highlights**: This research emphasizes a model distillation 접근 방식을 사용하여 복잡한 모델에서 단순화된 모델을 추출하며, UDE의 다양성과 견고함을 강조합니다. UDE를 통해, 물리적 원리를 따르면서도 데이터 드리븐 접근 방식을 통해 유체의 복잡한 행동을 더 잘 이해할 수 있습니다. 이러한 연구 결과는 유체역학 분야에서 앞으로의 혁신적 접근을 제시함으로써, 기존 모델링 기법의 한계를 극복할 수 있는 가능성을 보여줍니다.
...
### Unrolled Creative Adversarial Network For Generating Novel Musical Pieces (https://arxiv.org/abs/2501.00452)
- **What's New**: 이 논문에서는 기존의 RNN 기반 음악 생성 방법보다는 GAN(Generative Adversarial Networks)과 그 변형인 CAN(Creative Adversarial Networks)을 활용한 새로운 음악 생성 시스템을 제안합니다. 두 가지 시스템을 통해 클래식 음악과 작곡가의 스타일을 학습하여 창의적인 음악을 생성하는 것을 목표로 했습니다. 특히, unrolled CAN을 도입하여 모드 붕괴(mode collapse)를 방지하는 방법을 소개합니다.

- **Technical Details**: 이 연구는 다양한 데이터 세트를 사용하여 GAN과 CAN 아키텍처를 설계하였으며, 생성기(generator)와 구분기(discriminator)로 구성된 네트워크를 설명합니다. 입력으로는 무작위 벡터(z vector)를 사용하고, MIDI 파일을 이미지로 변환하여 GAN 또는 CAN에 제공했습니다. CAN에서는 생성기가 클래스를 구분하기 어려운 음악을 생성하도록 설계되어 있습니다.

- **Performance Highlights**: 실험 결과, GAN, CAN 및 unrolled CAN의 세 가지 아키텍처의 음악 생성 성능을 비교했습니다. GAN은 데이터 세트에 유사한 음악 생성에 성공했으며, CAN과 unrolled CAN은 더 다양한 스타일의 음악을 생성하는 데 성과를 보였습니다. 결과로 얻은 MSE(Mean Squared Error) 점수는 생성된 음악의 창의성 및 학습된 데이터 집합과의 편차를 측정하는 데 사용되었습니다.
...
### Outlier-Robust Linear System Identification Under Heavy-tailed Nois (https://arxiv.org/abs/2501.00421)
- **What's New**: 이 논문에서는 시간 변이 없이 선형 시스템의 상태 전이 행렬(transition matrix)을 추정하는 방법을 새롭게 제시합니다. 기존 연구들은 Gaussian 또는 sub-Gaussian noise 모델에 의존한 반면, 본 연구는 noise 분포의 4차 모멘트 존재만을 가정하여 더 약한 noise 모델을 사용하는 특징이 있습니다. 이러한 설정 아래에서 처음으로 sample-complexity bounds를 제시하며, 이는 sub-Gaussian noise 하의 결과와 유사한 수준에 이릅니다.

- **Technical Details**: 제안된 알고리즘은 여러 개의 약한 집계(estimator)를 구성하고, 이 성능을 고차원 내구성 통계(high-dimensional robust statistics) 기술을 통해 향상시키는 방식을 가집니다. 특히, 노이즈 분포의 히스토그램(kurtosis)은 필요한 trajectory 수에 미치는 영향을 분석하여 보여주는 점이 흥미롭습니다. 또한, 적대적(adversary) 데이터 손상이 있는 시나리오에서도 쉽게 확장 가능하다는 점에서 유연성을 지닙니다.

- **Performance Highlights**: 본 연구 결과는 프로바빌리티 약정(PAC) 프레임워크에서 noise의 4차 모멘트에 대한 가정을 바탕으로 처음으로 PAC bounds를 제공함으로써, 머신러닝 및 피드백 제어 시스템에서의 데이터 기반 접근에 기여합니다. 잘 설계된 강인한 추정기를 통해, sub-Gaussian noise 하에서 알려져 있는 경계를 거의 회복할 수 있음을 보여주었습니다. 이러한 연구 내용은 실세계에서 자주 발생하는 비이상적인 데이터 설정에 대한 견고한 통계적 학습 이론의 기초를 다지는 데 기여합니다.
...
- **What's New**: 최근 기계 학습 기반의 부분 미분 방정식 (PDE) 해결법이 주목받고 있습니다. 본 논문에서는 랜덤 피쳐 (random feature) 기반의 프레임워크를 소개하여 PDE를 효과적으로 해결할 수 있는 방법을 제시합니다. 이 방법은 기존의 물리 기반 신경망 (PINNs)과 커널 방법의 한계를 극복할 수 있는 가능성을 보여줍니다.

- **Technical Details**: 제안된 랜덤 피쳐 프레임워크는 PDE의 잔여 최소화를 통해 커널 행렬을 구성할 필요가 없습니다. 또한, 우리는 이 방법의 수렴성을 분석하며, 두 가지 단계로 구성된 수렴 분석을 제공합니다. 첫 단계는 커널 방법의 표준 수렴 분석을 따르고, 두 번째는 랜덤 피쳐 방법을 이용한 커널 방법의 근사화와 관련된 문제입니다.

- **Performance Highlights**: 논문에서 제안한 방법은 비선형 타원 PDE, 고차원 비선형 포아송 PDE 등 여러 벤치마크에서 성능을 테스트하였습니다. 실험 결과, 기존 방법에 비해 유사하거나 더 나은 성능을 보여주면서도 필요한 컴퓨팅 자원을 대폭 줄일 수 있음을 증명합니다.
...
### LASSE: Learning Active Sampling for Storm Tide Extremes in Non-Stationary Climate Regimes (https://arxiv.org/abs/2501.00149)
- **What's New**: 이번 연구에서는 열대 저기압(typhoon)으로 인해 발생하는 파괴적인 폭풍 해일을 신속하게 식별하기 위한 새로운 방법론을 제시합니다. 기존의 비싼 Monte Carlo 수치 시뮬레이션을 최소화하면서, 대체 모델(surrogate model)을 활용하여 매우 정확한 결과를 도출하는 방법을 보여주었습니다. 이 연구는 예측 정확도 100%를 달성하면서도 훈련 데이터의 20% 미만을 사용하여 파괴적인 폭풍을 성공적으로 찾아냈습니다.

- **Technical Details**: 연구에서는 고해상도 수치 수치 시뮬레이션 대신, 효율적인 대체 모델과 유익한 학습(informative learning) 방법론을 결합하여 열대 저기압을 분석합니다. 이 방법론은 TCs에서 풍속과 해일 모델을 결합하여 홍수 피해를 예측하며, 초기 적은 데이터에서 출발하여 점진적으로 예측 결과를 향상시킵니다. 데이터를 정확하게 지도하는 단계적 재훈련 과정을 통해 성능을 지속적으로 개선합니다.

- **Performance Highlights**: 실험에 따르면, 대체 모델은 고온의 과거 기후 시나리오와 향후 기후 시나리오를 아우르며 20%의 데이터로도 100%의 정확도를 기록합니다. 이러한 접근은 방글라데시와 같은 자원이 제한된 지역에서 산업 인프라와 인구의 안정성을 평가하는 데 혁신적인 가능성을 제공합니다. 또한 이 연구의 결과는 다양한 기후 시나리오와 지역에 일반화할 수 있는 잠재력을 가지고 있습니다.
...
### Post Launch Evaluation of Policies in a High-Dimensional Setting (https://arxiv.org/abs/2501.00119)
Comments:
          15 pages, 2 figures, 6 tables

- **What's New**: 이 논문에서는 기존의 A/B 테스트 방법 대신 대체로 사용할 수 있는 synthetic control 방법론을 제안합니다. 특히, 수억 개의 unit이 있는 대규모 환경에서의 적용 가능성을 고려하여 treatment이 일부 unit에만 영향을 미치는 경우에 유용하다고 설명합니다. 이러한 방법은 영향을 받지 않은 unit의 데이터를 활용하여 treated unit의 counterfactual outcomes를 추정할 수 있게 합니다.

- **Technical Details**: 제안된 두 단계 접근법에서는 첫 단계로 unit의 covariates를 바탕으로 유사한 control unit을 선택하기 위해 nearest neighbor matching을 수행합니다. 두 번째 단계에서는 고차원 데이터를 위한 supervised learning 방법을 사용하여 보정된 counterfactual outcomes를 추정합니다. 이러한 방법론은 interpolation bias와 machine learning bias를 고려하여 정확도를 높이고자 합니다.

- **Performance Highlights**: 여섯 개의 대규모 실험을 통해 제안된 방법이 counterfactual outcome의 정확도를 성공적으로 개선했다고 보고합니다. 그러나, machine learning bias가 치료 효과의 추정 결과에 영향을 미칠 수 있음을 확인하였고, 이에 대한 효과적인 de-biasing 기법을 제시합니다. 이 분석을 통해 대규모 실험 환경에서도 성공적으로 작동할 수 있는 방법론의 타당성을 제시하였습니다.
...
### Machine Learning Gravity Compactifications on Negatively Curved Manifolds (https://arxiv.org/abs/2501.00093)
Comments:
          42 pages, code available at this http URL

- **What's New**: 이번 연구는 고차원 중력 이론의 진공(vacuum) 구성을 기계 학습(Machine Learning) 기법을 통해 해결 가능성을 탐구합니다. 특히, 일반적인 휨(compactification) 중력의 운동 방정식을 풀기 위해 신경망(Neural Networks)을 사용하여 하이퍼볼릭 다양체(hyperbolic manifolds) 위에서 아인슈타인 PDEs를 해결합니다. 이 접근법은 비대칭적인 구성을 포함할 수 있는 유연성을 제공하여 다양한 환경에서의 적용 가능성을 높입니다.

- **Technical Details**: 저자들은 비대칭 간섭과 물질 필드를 포함하는 운동 방정식을 직접 해결하는 기계 학습 기반의 새롭고도 유용한 접근 방법을 제안하고 있습니다. 이 방법은 고차원 공간에서 아인슈타인 유사 PDEs를 해결하는 운동 방정식 최적화 문제로 변환합니다. 최신 기계 학습 최적화 알고리즘, 예를 들어 Adam 최적화 기법과 같은 발전을 활용하면서 이러한 문제를 해결하기 위한 기술적 장점을 강조합니다.

- **Performance Highlights**: 연구에서 제공하는 기계 학습 방법은 고차원에서의 전형적인 문제 해결에 대한 진전을 이루었고, 기존의 수치적 접근 방법이 어려움을 겪는 영역에서도 잘 작동하는 것으로 나타났습니다. 또한, 이 방법은 이전에 발견된 솔루션에 대한 경향성을 활용하여 기계 학습 모델의 성능을 더욱 향상시킬 수 있는 방법을 제공합니다.



### Insights on Galaxy Evolution from Interpretable Sparse Feature Networks (https://arxiv.org/abs/2501.00089)
Comments:
          Submitted to AAS Journals. 10 pages, 4 figures, 2 tables

- **What's New**: 이 논문에서는 Sparse Feature Network (SFNet)이라는 새로운 신경망 아키텍처를 제안합니다. SFNet은 이미지의 특성을 선형적으로 결합하여 은하의 물리적 특성을 예측할 수 있는 해석 가능한 특징을 생성합니다. 이를 통해, 기존의 딥 뉴럴 네트워크의 해석 가능성 부족 문제를 해결하려고 합니다.

- **Technical Details**: SFNet은 CNN (Convolutional Neural Network) 아키텍처를 기반으로 하며, 훈련 중 k-sparse 제약 조건을 부여하여 각 이미지 예제에 대해 k개의 특징만 활성화되도록 합니다. 이 구조는 특징들을 가능한 가장 적은 뉴런에 통합하여 해석 가능한 결과를 이끌어냅니다. SFNet은 SDSS 데이터에서 수집한 250,207개의 은하 이미지를 사용하여 훈련되었습니다.

- **Performance Highlights**: SFNet은 강력한 성능을 발휘하면서도 해석 가능한 특징을 학습할 수 있음을 보여줍니다. 기존의 최신 모델들과 비슷한 성능을 기록하며, 대규모 데이터셋에서 물리적 패턴을 발견하는 데 유용한 접근법이 될 것입니다. 이러한 연구 결과는 천문학자들이 머신러닝의 결과를 해석하는 데 도움을 줄 것으로 기대됩니다.



### High-Dimensional Markov-switching Ordinary Differential Processes (https://arxiv.org/abs/2501.00087)
- **What's New**: 본 연구는 이산 관측치에서 마르코프 스위칭 일반 미분방정식의 매개 변수 복원을 조사합니다. 특히 비선형 가법 모델이 적용되며, 이는 생물학적 시스템 및 제어 시스템 등 여러 분야에 널리 활용됩니다. 그러나 관측치로부터 생성 프로세스를 재구성하는 연구는 적습니다. 이 연구는 관측치에서 생성 프로세스를 재구성하는 데 중점을 두고 있습니다.

- **Technical Details**: 연구에서는 먼저 이산 샘플에서 연속 샘플 경로를 복원하고, 이후 프로세스의 매개변수를 추정하는 두 단계 알고리즘을 개발했습니다. 이 알고리즘은 통계적 오류와 선형 수렴 보장에 대한 새로운 이론적 통찰을 제공합니다. 특히 프로세스가 $eta$-mixing일 때 이러한 오류가 어떻게 발생하고 감소하는지를 설명합니다.

- **Performance Highlights**: 이 모델은 ADHD 그룹과 정상 대조군 간의 안정 상태 뇌 네트워크의 차이를 조사하는 데 적용되었습니다. 연구 결과, 두 그룹 간의 전이 속도 행렬의 차이가 발견되었습니다. 이런 차이를 통해 뇌의 기능적 연결망에 대한 새로운 통찰을 제공합니다.
...
- **What's New**: 최근 데이터 집약적 애플리케이션의 기하급수적인 성장으로 인해 현대 스토리지 시스템에 대한 수요가 급증하고 있습니다. 이러한 문제를 해결하기 위해 본 논문은 RL-Storage라는 강화 학습 기반의 새로운 프레임워크를 소개합니다. RL-Storage는 딥 Q-러닝 알고리즘을 활용하여 실시간 I/O 패턴으로부터 학습하고, 캐시 크기, 큐 깊이 및 리드어헤드 설정과 같은 최적의 스토리지 파라미터를 예측합니다.

- **Technical Details**: RL-Storage는 스토리지 커널 내에서 작동하며, 최소한의 지연과 낮은 계산 오버헤드를 보장합니다. 이를 통해 RL-Storage는 적응형 피드백 메커니즘을 통해 중요한 파라미터를 동적으로 조정하여 다양한 워크로드에서 효율적인 자원 활용을 달성합니다. 시스템 성능을 모니터링하며 블록 크기, 큐 깊이, 리드어헤드 값과 같은 주요 파라미터를 조정합니다.

- **Performance Highlights**: 실험 결과, RL-Storage는 RocksDB 및 PostgreSQL과 같은 여러 벤치마크에서 최대 2.6배의 처리량 향상과 43%의 지연 시간 감소를 보여주었습니다. CPU 오버헤드는 단 0.11%에 불과하며 메모리 사용량도 5KB에 지나지 않아 실제 환경에서의 원활한 배치에 적합합니다. 이러한 성과는 RL 기법이 현대 스토리지 시스템의 동적 특성을 해결하는 데 있어 기 transformative potential을 지니고 있음을 강조합니다.
...
- **What's New**: 본 연구에서는 폐 질환 진단을 위한 호흡음 분류 작업에서 데이터 증강 기법인 Lungmix를 소개합니다. Lungmix는 Mixup에서 영감을 받아 웨이브폼을 혼합하고 라벨을 의미 기반으로 보간하여 더 일반화된 표현을 학습하도록 돕습니다. 이 혁신적인 방법은 여러 데이터셋에 걸쳐 모델의 일반화 성능을 크게 향상시키는 것으로 나타났습니다.

- **Technical Details**: Lungmix는 두 개의 웨이브폼을 결합하여 새로운 데이터 샘플을 생성하는 방법으로, 이 과정에서 랜덤 마스크와 음량 마스크를 사용합니다. 라벨 보간에는 비트 연산을 통해 의미를 고려한 방식이 적용되며, 이러한 접근은 전통적인 Mixup 접근이 가지는 한계를 극복하기 위해 고안되었습니다. 특히, Lungmix는 멜 스펙트로그램이 아닌 원시 웨이브폼에서 직접 적용됩니다.

- **Performance Highlights**: Lungmix는 ICBHI, SPR, HF의 세 가지 데이터셋에 걸쳐 실험을 진행하여 보지 못한 데이터에 대한 일반화 능력을 높이는 데 성공했습니다. 특히, Lungmix를 적용함으로써 4개 클래스 분류 점수가 최대 3.55% 증가하여 기존 모델과 비슷한 성능을 달성했습니다. 이러한 결과는 폐 질환 진단에 있어 Lungmix의 중요성을 입증합니다.
...
- **What's New**: 본 논문은 제한된 데이터 샘플로 인해 직면하는 사이버 공격 패턴 탐지의 어려움을 해결하기 위해 새로운 Multi-Space Prototypical Learning (MSPL) 프레임워크를 제안합니다. 이 프레임워크는 여러 유사성 공간(Euclidean, Cosine, Chebyshev, Wasserstein distance)에서 작동하며, 이를 통해 패턴 인식을 향상시키고 임베딩의 강인성을 높입니다. Polyak-averaged prototype generation을 활용하여 학습 과정을 안정화하고 드문 또는 제로 데이 공격에 적응하는 데 효과적입니다.

- **Technical Details**: MSPL 프레임워크는 다양한 공격 패턴의 특성을 파악하기 위해 여러 메트릭 스페이스를 활용합니다. 특히, Polyak-averaged prototype generation과 균형 잡힌 에피소드 학습 방법을 통합하여 다양한 공격 유형에 대한 검출 안정성을 높이고, 이를 통해 대표성을 보장합니다. 이 접근 방식은 Martinez et al.의 이중 공간 프로토타입 네트워크 작업을 기반으로 하며, 벤치마크 데이터 세트에서 검증되었습니다.

- **Performance Highlights**: 실험 결과, MSPL은 기존의 방법들에 비해 낮은 프로파일 및 새로운 공격 유형을 탐지하는 데 우수한 성능을 보였습니다. 제안된 프레임워크는 현대 사이버 보안 응용 프로그램에서 스케일과 효과성을 입증하며, 부족한 데이터 상황에서도 잘 작동합니다. 이 연구는 드문 공격 탐지의 최전선에서 진전을 이루는 것을 목표로 하며, 향후 연구 방향을 제시합니다.
...
### Magnetic Field Data Calibration with Transformer Model Using Physical Constraints: A Scalable Method for Satellite Missions, Illustrated by Tianwen-1 (https://arxiv.org/abs/2501.00020)
- **What's New**: 이 연구는 Tianwen-1 화성 탐사 임무에서 수집한 자기장 데이터를 물리학 원리에 기반한 신경망 구조와 통합하는 새로운 접근 방식을 소개합니다. 이 방법은 Transformer 기반 모델을 활용하여 위성 동역학, 도구 간섭 및 환경 노이즈로 인해 발생하는 측정 이상을 보정합니다. 이를 통해 보정된 데이터의 정확성과 물리적 일관성이 크게 향상되었습니다.

- **Technical Details**: 기존의 자기계 보정 방법은 주로 물리 모델과 선험적 수학적 접근 방식에 의존하며, 이는 기기 드리프트, 동적 필드 및 환경적 간섭을 보정하는 데 초점을 맞추고 있습니다. 연구에서는 Maxwell의 방정식을 신경망 아키텍처에 통합하여 물리적 정보로써 사용함으로써 데이터 기반 모델과 물리학 기반 이해 간의 차이를 메꾸고자 합니다.

- **Performance Highlights**: 이 접근법은 기존 전통적인 방법에 비해 보정 과정을 며칠 또는 수개월에서 단 몇 분 또는 몇 시간으로 단축시키며, 예측은 몇 초 만에 수행할 수 있게 해줍니다. 이는 우주 날씨 모델링 및 행성 자기권 연구의 과정을 가속화할 뿐만 아니라, 미래의 행성 탐사 및 태양풍 상호작용 연구를 위한 강력한 프레임워크를 제공합니다.



### Energy-Efficient Sampling Using Stochastic Magnetic Tunnel Junctions (https://arxiv.org/abs/2501.00015)
Comments:
          10 pages, 7 figures, preprint

- **What's New**: 이번 논문에서 제안한 에너지 효율적인 알고리즘은 기존 Mersenne-Twister 알고리즘보다 최소 9721배 더 높은 에너지 효율성을 달성하며, PCG 알고리즘과 비교했을 때도 5649배 더 개선된 성능을 보입니다. 이 알고리즘은 일반적인 Float16 샘플링을 위한 하드웨어 프레임워크를 기반으로 하며, 진정한 무작위성을 생성할 수 있는 스토캐스틱 매그네틱 터널 접합 장치(s-MTJ)를 활용하고 있습니다. 이렇게 함으로써 기존의 기호 계산을 피하고, 물리적 현상을 통계적 특성에 직접 매핑하여 샘플링 효율성을 높였습니다.

- **Technical Details**: 우리는 Bernoulli 분포로부터보다 더 넓은 범위의 비임의 분포 샘플링을 가능하게 하는 새로운 스토캐스틱 매그네틱 터널 접합 장치 구조를 제안합니다. 이 장치는 부하 전류를 기준으로끔 변화하는 p 매개변수를 통해 Bernoulli 분포에서 샘플을 생성할 수 있으며, 이는 기호 계산 없이 부동소수점 형식의 비트 위치에 매핑된 다양한 Bernoulli 분포에 대한 매개변수를 정의할 수 있습니다. 이러한 방식을 통해 우리는 임의의 1D 분포를 근사적 균일 분포로 분해하고, 이를 통해 높은 효율의 샘플링이 가능합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 기존의 랜덤 넘버 생성기 (RNG)보다 본 연구의 하드웨어 구성으로 Float16을 사용할 경우 에너지 효율성이 5649배 더 뛰어난 것으로 나타났습니다. 특히, KL 발산 지표를 기반으로 한 실험에서는 폐쇄형 해법과 비교할 때 0.0343±0.1473의 대략적인 오차를 보였습니다. 이러한 성과는 각기 다른 비임의 분포에 대해 저전력으로 안정적인 샘플링이 가능하다는 것을 시사합니다.
...
### Machine learning models for Si nanoparticle growth in nonthermal plasma (https://arxiv.org/abs/2501.00003)
Comments:
          9 pages, 8 figures, 10 SI figures, 8 SI tables, 2 SI algorithms

- **What's New**: 본 연구에서는 비열플라즈마(nonthermal plasmas, NTPs)에서 생성된 나노입자(nanoparticles)의 성장을 모사하는 데 필요한 파라미터 추정을 가속화하는 방법을 제시합니다. 기존의 모델링 방식의 한계에도 불구하고 머신러닝(machine learning) 모델을 사용하여 예측 결과를 향상시키는 방법에 대해 설명합니다. 이 방법은 NTP 환경에서 실리콘(silane) 조각의 충돌과 관련된 데이터에 적용되어 그 효용성을 보여줍니다.

- **Technical Details**: 본 연구는 전통적인 반응적 분자역학(classical reactive molecular dynamics) 시뮬레이션을 수행하여 실리콘 클러스터와 다른 분자 간의 충돌을 분석하였습니다. 충돌 벡터(impact vector)를 과대학교의 평균 중앙선을 통한 다양한 각도로 설정하고, LAMMPS를 활용하여 시뮬레이션을 진행하였습니다. 충돌 결과를 분석하기 위해 최대 최소 거리(minimum distance)를 모니터링했으며, 이를 통해 실리콘 분자의 스티킹 확률(sticking probability)을 예측하기 위한 데이터 세트를 구성했습니다.

- **Performance Highlights**: 모델의 예측 성능은 ElasticNet, Support Vector Regression(SVR), K-nearest neighbors(KNN) 등 다양한 머신러닝 모델을 통해 비교되었습니다. 연구 결과, 적절한 손실 함수(loss function)를 구현하고 정확한 불변량(invariance)을 적용할 때 우수한 예측 성능을 달성할 수 있음이 증명되었습니다. 특히, 에너지 및 온도 샘플링의 15-25%만으로도 높은 정확도를 이루며, 이는 향후 계산 노력을 대폭 줄일 수 있는 가능성을 시사합니다.

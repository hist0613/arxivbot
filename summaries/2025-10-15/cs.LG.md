...
- **What's New**: 이번 연구에서는 여러 의사결정자가 있는 상황에서 다중 손실(multiple losses)을 동시에 최소화하는 단일 예측기(single predictor)를 설계하는 문제를 다룹니다. 이는 기존의 omniprediction 문제의 변형으로, 정확한 결정을 내릴 수 있는 확률적 예측(probabilistic predictions)을 구축하는 것을 목표로 합니다.

- **Technical Details**: 기존의 omniprediction 알고리즘은 크게 두 가지 범주로 나뉩니다. 첫 번째는 다중 보정(multicalibration)을 최적화하는 보스팅 방법(boasting methods)이고, 두 번째는 온라인 방식으로 '최악의 경우(worst-case)' 손실을 추정하는 적대적(two-player game) 접근법입니다. 이번 연구에서는 다중 보정이 omniprediction보다 어려운 문제임을 보여주는 하한값(lower bounds)을 제시합니다.

- **Performance Highlights**: 마지막으로, 이러한 아이디어를 활용하여 샘플 효율(sample-efficient) 알고리즘을 얻는 온라인-배치 변환(online-to-batch conversion) 방법에 대해 논의합니다. 이 과정은 복잡하고 무작위화된 예측기를 생성하는 단점이 있지만, 우리는 구조적 요소(structural elements)를 활용하여 보다 직접적이고 비무작위화된(unrandomized) 알고리즘을 설계하여 이 방법을 개선했습니다.



### KoALA: KL-L0 Adversarial Detector via Label Agreemen (https://arxiv.org/abs/2510.12752)
- **What's New**: 저자들은 KoALA(KL-L0 Adversarial detection via Label Agreement)라는 새로운 적대적 공격 탐지기를 소개합니다. 이 탐지기는 아키텍처 변경이나 적대적 재훈련이 필요하지 않아 기존 시스템에 쉽게 적용할 수 있습니다. KoALA는 두 개의 상호 보완적인 유사성 지표(KL divergence 및 L0 기반 유사성)의 분쟁을 기반으로 적대적 공격을 감지합니다.

- **Technical Details**: KoALA는 두 가지 측정값인 KL divergence와 L0기반 유사성을 사용하여 입장의 유사성을 비교합니다. KL divergence는 작은 크기의 변화에 민감하고, L0 기반 유사성은 드문 대규모 변화에 초점을 맞춥니다. 이 탐지기는 청정 이미지를 사용하여 사전 훈련된 이미지 인코더를 간단히 미세 조정하면 사용할 수 있습니다.

- **Performance Highlights**: 실험 결과, KoALA는 ResNet/CIFAR-10에서 0.94의 정밀도와 0.81의 재현율, CLIP/Tiny-ImageNet에서 0.66의 정밀도와 0.85의 재현율을 달성했습니다. 이는 KoALA가 이론적으로 주장한 내용을 강력히 뒷받침하며, 안전-critical한 상황에서도 효과적으로 적대적 샘플을 탐지하는데 유용한 솔루션임을 보여줍니다.



### Doctor Rashomon and the UNIVERSE of Madness: Variable Importance with Unobserved Confounding and the Rashomon Effec (https://arxiv.org/abs/2510.12734)
- **What's New**: 본 논문에서는 UNobservables와 Inference for Variable importancE using Rashomon SEts(UNIVERSE)라는 새로운 방법론을 소개합니다. 기존의 Variable importance (VI) 방법론이 갖고 있는 한계를 극복하고, 관측되지 않은 변수들(unobserved variables)이 포함되더라도 실제 변수의 중요성을 추정할 수 있게 해줍니다. Rashomon sets를 활용해 변수 중요도의 경계를 정량화하여, 누락된 변수가 있어도 편향되지 않은 결정을 내릴 수 있도록 합니다.

- **Technical Details**: UNIVERSE 접근법은 데이터셋 내의 근사 최적 모델이 포함된 Rashomon 집합을 활용하여 누락된 변수가 있을 때에도 진정한 변수 중요도를 추정할 수 있는 경계를 제시합니다. 이 방법은 정량적 분석을 통해 변수의 중요도를 추정하며, 특히 unobserved confounding과 finite sample uncertainty를 고려하여 이를 극복하는 이론적 보장을 포함합니다. 실험 결과는 이 접근법이 실제로 유용한 결론을 도출할 수 있음을 보여줍니다.

- **Performance Highlights**: UNIVERSE 방법론은 반-합성 반정형(sSemi-synthetic) 실험에서 강력한 성능을 보이며, 중요 변수가 누락된 상황에서도 유용한 경계를 제공합니다. 신용 리스크 평가에서 이 접근법을 적용함으로써 변수의 중요도를 보다 명확하게 이해할 수 있게 됩니다. 이러한 성과는 과학적인 검증과 가설 생성을 위한 중요한 기여로 작용할 것입니다.
...
### Improving Decision Trees through the Lens of Parameterized Local Search (https://arxiv.org/abs/2510.12726)
Comments:
          Accepted at NeurIPS 2025

- **What's New**: 이 논문에서는 결정 트리(decision tree)를 학습하는 알고리즘에서 발생하는 일반적인 히uristic(local-search) local-search 작업인 임계(threshold) 조정이나 특성(feature) 교환을 통한 분류 오류(classification error) 최소화 문제를 연구합니다. 기존에는 이러한 작업들이 복잡도로 인해 해결하기 어려운 문제로 여겨졌으나, 특정 조건 하에 해결 가능성을 제시합니다.

- **Technical Details**: 이 연구는 NP-완전(NP-complete) 문제임을 확인하였지만, 문제의 복잡성을 파라미터화(parameterized-complexity)하여 분석합니다. 특히, d개의 작은 특성이나 작은 영역(domain) 크기 D에서는 문제의 난해함이 여전히 유지되지만, 두 조건의 조합은 고정-파라미터 추적 가능(fixed-parameter tractability)을 나타낸다는 것을 보여줍니다. 구체적으로, 입력 크기(|I|)와 특성 수를 바탕으로 $(D + 1)^{2d} 	imes |I|^{O(1)}$ 시간 내에 문제를 해결할 수 있음을 증명하였습니다.

- **Performance Highlights**: 제안한 알고리즘의 개념 증명(proof-of-concept) 구현을 제공하며, 이에 대한 실험 결과를 보고합니다. 이 실험은 알고리즘의 효율성 및 실제 응용 가능성을 보여주어, 결정 트리 학습 문제에 대한 더 깊은 통찰을 제공합니다.



### CARVQ: Corrective Adaptor with Group Residual Vector Quantization for LLM Embedding Compression (https://arxiv.org/abs/2510.12721)
Comments:
          Accepted at EMNLP Findings 2025

- **What's New**: 이번 연구에서는 메모리에 제약이 있는 엣지 디바이스에서 LLM의 임베딩 레이어 압축을 위한 새로운 기술인 CARVQ를 제안합니다. CARVQ는 그룹 잔차 벡터 양자화(group Residual Vector Quantization)와 결합된 사후 훈련 수정 어댑터(Corrective Adaptor)를 기반으로 하여, 기존 임베딩을 약 1.6비트로 압축할 수 있습니다. 이는 특수 하드웨어 없이도 구현 가능하다는 장점을 지니고 있습니다.

- **Technical Details**: CARVQ는 선형 및 비선형 매핑을 조합하여 구성된 수정 어댑터를 사용하여 원래 모델의 임베딩을 효과적으로 모방합니다. 이를 통해 그룹 RVQ 작동으로 인한 손실을 보상하며, 기존의 변환기 층 양자화 기법과 호환됩니다. CARVQ는 다양한 NLP 작업에서 효과적으로 검증되었으며, 모델의 비트당 평균 비트폭(bitwidth-per-parameter)을 낮추는 동시에 합리적인 Perplexity를 유지했습니다.

- **Performance Highlights**: 실험에 따르면 CARVQ는 여러 NLP 작업에서 스칼라 양자화(scalar quantization)보다 높은 성능을 보여주었습니다. 특히, 2.4비트에서 거의 무손실의 성능을 보였고, 평균적으로 1.6비트에서 합리적인 perplexity(퍼플렉시티) 점수를 기록했습니다. 이는 메모리 제약이 있는 디바이스에서 LLM의 효율적인 배치를 가능하게 하는 중요한 단계임을 나타냅니다.
...
- **What's New**: 이번 연구에서는 약물 탐색에 대한 화학 프리트레인 모델(chemical pretrained models)의 활용 가능성을 강조하고 있습니다. Kinetic GROVER Multi-Task (KERMT) 모델을 통해 여러 작업에서의 개선된 성능을 확인하였으며, 이전에는 파악되지 않았던 대량 데이터의 중요성을 제시하고 있습니다. 또한, 두 가지 멀티태스크 ADMET 데이터 분할을 발표하여 멀티태스크 딥러닝 방법에 대한 보다 정확한 벤치마킹을 가능하게 합니다.

- **Technical Details**: KERMT는 화학 프리트레인 GROVER 모델의 개선된 버전으로, 화학 그래프를 기반으로 한 모델입니다. KERMT는 PyTorch Distributed Data Parallel (DDP)로 구현되어 있으며, 자동 하이퍼파라미터 최적화를 통해 더 나은 피니트 튜닝을 지원합니다. 이 연구는 ADMET와 같은 약물 속성 예측 분야에서 다중 작업 학습(multi-task learning)의 이점을 활용하여 성능을 향상시킴을 보여줍니다.

- **Performance Highlights**: KERMT 모델은 머크(Merck & Co., Inc.)의 내부 및 공개 데이터 셋에서 단일 작업 모델 대비 더 나은 성능을 보여줍니다. 특정 대량 데이터 설정에서 KERMT가 다른 벤치마크 모델보다 훨씬 더 높은 성능 개선을 나타낸 점도 주목할 만합니다. 이 연구는 실제 약물 발견 맥락에서의 일반화 능력을 평가하기 위해 시간적 분할을 사용하여 모델 성능을 측정하였고, 이로 인하여 모델의 강력한 판별 성능을 확인할 수 있었습니다.
...
### Few Shot Semi-Supervised Learning for Abnormal Stop Detection from Sparse GPS Trajectories (https://arxiv.org/abs/2510.12686)
- **What's New**: 이 논문은 도시 간 버스 운송에서 비정상 정차(ASD) 감지를 위한 접근 방식을 제안합니다. 기존 방법들이 GPS 데이터의 희소성 문제를 효과적으로 해결하지 못하는 반면, 본 연구에서는 Sparsity-Aware Segmentation (SAS) 방법을 통해 지리적 밀도를 기반으로 세그먼트 경계를 정의하고, Locally Temporal-Indicator Guided Adjustment (LTIGA)를 통해 데이터 희소성을 완화합니다. 이러한 방법들은 적은 수의 레이블로도 비정상 정차를 효과적으로 감지할 수 있도록 해줍니다.

- **Technical Details**: Sparsity-Aware Segmentation (SAS) 방식은 GPS 경로를 로컬 밀도에 따라 점진적으로 세분화하여 비정상 정차 행동을 포착하기 위해 Temporal Influence Score (TIS), Maximum Speed Deviation (MSD), Top-kk Aggregated Temporal Score (TTA@k)와 같은 세 가지 도메인 특화 지표를 정의합니다. 이 후, LTIGA를 통해 유사성 그래프를 활용하여 지표의 품질을 부드럽게 조정하고 아울러 레이블 희소성 문제를 해결하기 위해 공간-시간 그래프를 구성하여 레이블 전파(label propagation)를 적용합니다. 이를 통해 그래프 상에서의 약한 감독을 확장하고, GCN을 통해 관계 패턴을 학습합니다.

- **Performance Highlights**: 실제 데이터에 대한 실험 결과, 단 10개의 레이블링된 인스턴스만 사용하여 AUC는 0.854, AP는 0.866을 기록했습니다. 이는 기존 방법들을 능가하는 성능을 보여주며, 이 논문의 접근 방식이 희소한 GPS 데이터 환경에서도 효과적으로 작동함을 입증합니다. 코드는 공개적으로 제공되며, 연구에 활용된 데이터셋 또한 접근 가능합니다.



### CoRA: Covariate-Aware Adaptation of Time Series Foundation Models (https://arxiv.org/abs/2510.12681)
- **What's New**: 이번 연구에서는 Time Series Foundation Models (TSFMs)의 성능을 향상시키기 위한 새로운 구조인 CoRA(covariate-aware adaptation)를 제안합니다. CoRA는 다양한 외생 변수(covariates)와 함께 TSFM의 사전 훈련된 백본(backbone)을 효율적으로 활용하여 예측의 질을 개선합니다. 더불어, CoRA는 Granger Causality Embedding (GCE)을 채택하여 외생 변수의 인과관계를 자동으로 평가할 수 있는 방법론을 채택하였습니다.

- **Technical Details**: CoRA 프레임워크는 사전 훈련된 모델을 고정된 피쳐 추출기로 취급하고, Granger Causality Embedding을 통해 외생 변수의 인과적 중요도를 평가하여 데이터 적응 시 가중치를 반영합니다. 이 때, CoRA는 제로 초기화(zero-initialization) 조건 주입 메커니즘을 통해 사전 훈련된 모델의 정보 손실(catastrophic forgetting)을 막고 서서히 외부 정보를 통합합니다. 이러한 방법론을 통해 CoRA는 고차원 멀티모달 데이터에 대한 TSFM의 일반화 가능성을 높입니다.

- **Performance Highlights**: CoRA는 다양한 벤치마크에서 이전 최첨단(covariate-aware deep forecasters) 방식들보다 뛰어난 성능을 보이며, 특히 데이터 샘플 수가 적을 때도 31.1% MSE 감소를 달성합니다. 또한, CoRA는 상관 관계가 복잡한 다양한 외생 변수를 다룰 수 있는 강력한 적응성을 지니고 있으며, 각 외생 변수의 인과관계를 해석할 수 있는 통찰을 제공합니다. 이는 TSFM을 활용한 예측 문제에 실용적인 패러다임을 제시합니다.
...
- **What's New**: 본 논문에서는 LLMs(Large Language Models)에 대한 jailbreak 공격에 대응하기 위해 CALM(Concept Alignment and Concept Manipulation)이라는 새로운 방법을 제안합니다. CALM은 모델의 마지막 층의 잠재 표현(latent representation)을 수정함으로써 유해한 개념을 억제하는 추론 시간(inference-time) 방법입니다. 이 방법은 추가적인 데이터나 모델 재학습 없이도 작동하며, 대다수의 메트릭에서 기존 방법보다 유해한 출력의 양을 줄이는 데 성공하였습니다.

- **Technical Details**: CALM은 기본적으로 Concept Whitening(CW) 기법과 Subspace Projection 기술을 활용하여 LLM의 내부 표현을 조작합니다. 이 방식은 유해한 개념과 관련된 잠재 방향을 제거하여 모델의 출력을 안전하게 조정합니다. CALM은 기존 CW 기법을 확장하여 LLM에 적합하게 적용하여 해로운 표현을 동적으로 억제하는 메커니즘을 제공합니다.

- **Performance Highlights**: 실험 결과에 따르면 CALM은 유해한 응답을 효과적으로 줄이고, 많은 기준 지표에서 기존의 방안을 초월하는 성능을 보였습니다. 이 방법은 추가적인 훈련 데이터나 모델 세부 조정 없이도 가벼운 계산 오버헤드로 AI의 안전성을 높이는 새로운 접근을 제시합니다. CALM은 특히 개방형 모델에 대한 jailbreak 공격을 방어하는 데 중요한 기여를 할 수 있는 기술입니다.



### Structure-Aware Spectral Sparsification via Uniform Edge Sampling (https://arxiv.org/abs/2510.12669)
Comments:
          19 pages, 4 figures, NeurIPS 2025

- **What's New**: 이 논문에서는 스펙트럴 클러스터링(Spectral Clustering) 기법에서 균일 엣지 샘플링(uniform edge sampling)의 유용성을 탐구합니다. 전통적인 방법은 고비용의 전처리 과정을 요구하는 반면, 균일 샘플링은 이러한 복잡한 절차 없이 스펙트럼 특성을 잘 유지할 수 있음을 보여줍니다. 특히, 두 클러스터 간의 분리가 잘 이루어진 경우에 균일 샘플링이 특히 효과적임을 입증하였습니다.

- **Technical Details**: 스펙트럴 클러스터링은 그래프의 라플라시안 고유벡터(eigenvector)를 사용하여 노드를 저차원 공간으로 임베드합니다. 하지만 큰 그래프의 경우 고유벡터를 계산하는 것이 어려운 문제로, 본 연구에서는 고유값의 일부 표준화를 활용하여클러스터 정보를 유지하는 조건을 도출했습니다. 특히, 충분히 분리된 클러스터 구조가 있을 경우 균일 샘플링이라는 단순한 방법이 스펙트럼 하위 공간을 유지할 수 있다는 것을 증명합니다.

- **Performance Highlights**: 이 연구는 스펙트럴 클러스터링을 위한 새로운 저비용 전처리 방법을 제안합니다. 균일 엣지 샘플링을 통해 그래프를 간소화하는 것이 가능해져 큰 그래프에서도 클러스터링을 수행할 수 있게 됩니다. 이로 인해 스펙트럼 군집 데이터의 처리 속도와 효율성이 크게 향상되며, 기존 방법에 비해 계산적 오버헤드가 줄어드는 것이 특징입니다.



### Structured Sparsity and Weight-adaptive Pruning for Memory and Compute efficient Whisper models (https://arxiv.org/abs/2510.12666)
- **What's New**: 이번 논문에서는 Whisper 모델의 대규모 특성이 자원 제약이 있는 엣지 장치에서의 배치를 어렵게 한다는 점에 주목하고, 이를 해결하기 위한 프레임워크를 제안합니다. Structured sparsity를 구현하기 위해 Sparse Group LASSO(SGL) 패널티를 사용하여 FLOPs를 줄이는 동시에, 모델 파라미터와 메모리 소비를 줄일 수 있는 방법을 모색합니다. 이 연구는 Whisper-small 및 Whisper-medium 모델에 대해 기존의 방법보다 월등한 성능을 보여줍니다.

- **Technical Details**: 모델의 파라미터 수를 줄이기 위해 두 가지 주요 방법인 TSPAR(fineTuning for Structured sParsity and Adapti-ve pRuning)를 적용했습니다. 이 방법은 비구조적 감소를 초래하는 전통적인 L1 패널티를 대신하여, SGL 패널티를 통해 구조적 희소성을 달성합니다. 이러한 방식으로, 서로 다른 모델 레이어에서 무작위로 분포된 파라미터가 아닌, 특정 열의 파라미터를 함께 제거하여 FLOPs를 실제로 줄이는 효과를 얻습니다.

- **Performance Highlights**: 실험 결과, Common Voice 11.0 Hindi 데이터셋에서 Whisper-small 모델에 대해 35.4%의 모델 파라미터 감소, 14.25%의 메모리 소비 감소, 18.5%의 FLOPs 감소를 기록했습니다. 또한 Whisper-medium 모델에서는 31%의 모델 파라미터 감소와 15.29%의 메모리 소비 감소 및 16.95%의 FLOPs 감소 결과를 보여줍니다. 이 연구는 가장 최신의 Iterative Magnitude Pruning 기반 방법보다 18.7% 더 많은 파라미터를 제거하며, WER에서도 12.31 감소하는 성과를 보여줍니다.
...
### Towards Foundation Inference Models that Learn ODEs In-Contex (https://arxiv.org/abs/2510.12650)
- **What's New**: 이번 논문은 FIM-ODE라는 사전학습된 신경망 모델을 소개합니다. 이 모델은 노이즈와 희소한 관측값으로부터 ODE를 즉각적으로 추정할 수 있도록 설계되었습니다. 기존 모델들에 비해 FIM-ODE는 더 정확한 벡터 필드 예측을 제공합니다.

- **Technical Details**: FIM-ODE는 ODE 추정 문제를 다루며, DeepONet 신경 연산자를 사용하여 벡터 필드를 모델링합니다. 데이터는 최대 3차원 ODE 시스템에서 샘플링되며, 각 벡터 필드는 다항식의 배급에서 생성한 값을 바탕으로 구성됩니다. 훈련 과정에서는 관찰값에 근접한 지점에서 목표 벡터 필드와의 차이를 최소화하는 방향으로 학습이 이루어집니다.

- **Performance Highlights**: FIM-ODE는 복수의 궤적을 동시에 처리할 수 있도록 설계되어, 다수의 궤적이 있을 때 더 정확한 벡터 필드 추정을 가능하게 합니다. 실험 결과, FIM-ODE는 ODEFormer보다 우수한 재구성 및 일반화 성능을 보여주었고, 많은 궤적이 제공될수록 정확도가 향상되었습니다.



### On Foundation Models for Temporal Point Processes to Accelerate Scientific Discovery (https://arxiv.org/abs/2510.12640)
- **What's New**: 이 논문에서는 이벤트 데이터의 기본 패턴을 학습하는 단일 강력한 모델, 즉 'Foundation Model'을 소개했습니다. 이 모델은 수백만 개의 시뮬레이션된 이벤트 시퀀스에서 훈련되어 새로운 과학 데이터를 즉시 분석할 수 있도록 하며, 데이터셋의 몇 가지 예를 통해 재훈련 없이 빠르게 적응할 수 있습니다. 이러한 접근 방식은 정교한 이벤트 분석을 더 접근 가능하게 만들고 과학적 발견의 속도를 가속화합니다.

- **Technical Details**: 이 연구에서 제안한 MTPP(Marked Temporal Point Processes) 집중 추론을 위한 새로운 인-컨텍스트 학습 방법은 많은 마크된 이벤트 시퀀스를 생성하여 신경망 모델을 훈련시키는 것입니다. 이 모델은 관찰된 이벤트 시퀀스로부터 조건부 강도 함수(conditional intensity function)를 직접 추정하도록 설계되었습니다. FIM-PP는 이벤트 이력을 쿼리로 사용하여 컨텍스트를 고려한 역사적 임베딩을 기반으로 강도 함수를 파라미터화합니다.

- **Performance Highlights**: FIM-PP는 다양한 합성 시간적 포인트 프로세스에 대한 제로샷(zero-shot) 성능을 시각화하며, 이 모델의 핵심 기능인 조건부 강도를 정확하게 추론하는 능력을 보여줍니다. 실험 결과 FIM-PP는 다양한 이벤트 유형 간의 영향력을 분석하거나 다음 이벤트의 시간을 예측하는 데 매우 정확한 결과를 제공합니다. FIM-PP는 이후 평가에서 대부분의 과제에서 경쟁하는 방법들을 초월하는 성능을 나타냈습니다.



### Expert or not? assessing data quality in offline reinforcement learning (https://arxiv.org/abs/2510.12638)
- **What's New**: 이 논문은 오프라인 강화 학습(Offline Reinforcement Learning, RL)에서 데이터셋 품질을 평가하는 혁신적인 접근법을 제시합니다. 제안된 벨만-워서스타인 거리(Bellman-Wasserstein Distance, BWD)는 의사결정 정책과 무작위 정책 간의 차이를 측정하여, 데이터셋의 행위 품질을 평가합니다. 이를 통해 데이터 품질을 사전 데이터 수집 없이도 효율적으로 예측할 수 있습니다.

- **Technical Details**: 논문에서는 저자들이 제안한 BWD를 통해, Q-함수를 활용하여 행동 정책과 무작위 정책 간의 비유사성을 정량화합니다. 이 과정은 환경과의 상호작용이나 전체 정책 최적화를 필요로 하지 않으며, 오프라인 RL의 특성을 활용하여 행동 비판기(behavioral critic)로부터 직접적으로 계산됩니다. 이 방법은 데이터셋의 다양한 경로와 여러분의 품질을 효과적으로 고려할 수 있도록 설계되었습니다.

- **Performance Highlights**: BWD는 D4RL MuJoCo 작업에서 높은 데이터 품질과 강한 상관관계를 보이며, 여러 오프라인 RL 알고리즘을 집계한 오라클 성능 점수와도 결합됩니다. 또한, 정책 최적화 과정에서 BWD를 정규화 구성 요소로 추가하면, 정책 성능이 향상됩니다. 이를 통해 BWD는 오프라인 RL 데이터셋의 효율적인 분류 및 정책 최적화 도구로 기능할 수 있음을 보여줍니다.
...
### Towards Fast Coarse-graining and Equation Discovery with Foundation Inference Models (https://arxiv.org/abs/2510.12618)
- **What's New**: 이 논문에서는 높은 차원의 동적 프로세스를 설명하는 저차원의 유효 변수를 찾아내기 위한 새로운 접근 방식을 제안합니다. 최근 도입된 Foundation Inference Models (FIMs)를 활용하여 두 가지 문제, 즉 적절한 coarse-grained (조잡화된) 변수를 발견하고 지배 방정식을 동시에 맞추는 작업을 분리하는 방법을 소개합니다. 이 방식은 FIM을 정지 상태로 유지하고 인코더-디코더 구조만 훈련하여 안정적인 표현 학습을 위한 손실 함수를 정의합니다.

- **Technical Details**: 제안된 FIM-SDE 모델은 고정된 FIM 주위를 둘러싸고 있는 오토인코더 모델을 통해 구성됩니다. 데이터는 𝐲1∗,…,𝐲N∗ 형식의 고차원 관측값으로 표현되며, 인코더와 디코더 네트워크가 각각 조잡화된 좌표 𝐱i와 관측 공간의 데이터를 재구성합니다. FIM-SDE는 𝐟와 𝐠의 최대 추정량을 제공하여, 관측 데이터에 대한 동적 모델을 정규화합니다.

- **Performance Highlights**: 증명 개념으로서, 반원 확산을 가진 이중 웰 시스템을 사용한 결과는 제안된 접근 방식의 빠름과 재사용 가능성을 강조합니다. 300회의 훈련 단계 후에 조잡화된 잠재 변수가 동적 특성을 캡처하는 데 성공하며, 이전 지식 없이도 지배 동역학을 신속하게 회복할 수 있음을 보여줍니다. 이 모델은 향후 데이터 밀도가 낮은 경우의 시뮬레이션 방향성을 제안하며, 여러 내부 단계의 시뮬레이션을 사용할 수 있는 가능성을 남깁니다.
...
### Research in Collaborative Learning Does Not Serve Cross-Silo Federated Learning in Practic (https://arxiv.org/abs/2510.12595)
Comments:
          Main text: 23 pages, 2 tables, 2 figures

- **What's New**: 본 논문은 cross-silo federated learning (FL)의 실제 도입에서의 도전 과제를 이해하기 위한 인터뷰 연구를 수행하였습니다. 다양한 이해관계자들, 즉 사용자 조직, 소프트웨어 제공업체 및 학계 연구자들과의 인터뷰를 통해 모델 성능, 인센티브, 신뢰 문제와 같은 여러 장벽을 밝혀내었습니다. 연구 결과, 현재의 연구가 실체와 잘 맞지 않아 cross-silo FL의 연구 우선 사항을 재조정할 필요성을 강조합니다.

- **Technical Details**: Federated Learning (FL)은 여러 참여자가 데이터 공유 없이 협력하여 모델을 학습할 수 있는 분산 학습 패러다임입니다. 본 논문은 cross-silo FL에 초점을 맞추어, 각 조직이 별도의 데이터 저장소로서 참여하는 구조를 제공합니다. 연구자와 각 분야의 전문가로 구성된 21명의 참여자를 인터뷰하여 코어 문제 및 우선 사항을 분석하였습니다. 결과적으로, 모델 최적화 알고리즘에 대한 연구에 비해 커뮤니케이션 효율성, 법적 요구 사항 평가 등은 상대적으로 덜 다루어졌습니다.

- **Performance Highlights**: 연구는 cross-silo FL이 직면한 도전 과제가 기존 연구에서 잘 포착되지 않았음을 강조합니다. 특히, FL 문헌의 상당 부분이 통신 오버헤드를 줄이기 위한 복잡한 최적화 알고리즘 개발에 집중하는 반면, 실제로는 투명성과 인센티브 관련 문제에 대한 언급이 빈약한 것으로 나타났습니다. 마지막으로, 향후 연구 방향에 대한 논의를 통해 실질적인 운영 장벽을 극복할 수 있는 잠재적인 솔루션을 제시하고 있습니다.
...
### Multi-Armed Bandits with Minimum Aggregated Revenue Constraints (https://arxiv.org/abs/2510.12523)
- **What's New**: 이번 논문에서는 컨텍스트 정보가 포함된 멀티-암 밴딧(Multi-Armed Bandit, MAB) 문제를 다루며, 각 암(arm)이 컨텍스트마다 최소 집계 보상을 수령하도록 보장하는 동시에 총 누적 보상을 극대화하는 것을 목표로 하고 있습니다. 이 프레임워크는 현실 세계에서 공정한 수익 분배가 중요한 다양한 응용 사례를 포착합니다. 특히, 제한 사항들이 성능 최적화에 미치는 영향을 분석하며, 전통적인 MAB 환경에서는 구할 수 없는 최적 할당이란 기법의 부재를 강조합니다.

- **Technical Details**: 연구에서는 각 암이 평균적으로 최소 보상 λk를 달성하도록 보장하면서 누적 보상을 극대화하는 알고리즘을 설계하고 분석합니다. 이러한 알고리즘은 성능을 낙관적으로 우선시하거나 제약 조건 만족을 비관적으로 강제하는 방식으로 진행됩니다. 각 알고리즘에 대해, 후회(regret)와 제약 위반(constraint violation)에 대한 문제 의존적 상한을 도출하였으며, 시간 범위에 대한 의존성이 일반적으로 최적임을 나타내는 하한도 설정했습니다.

- **Performance Highlights**: 제안된 알고리즘은 다양한 파라미터에 따라 서로 다른 레짐(regime)에서 작동하며, 성능 갭이 작고 보상과 제약 조건 만족 사이의 균형을 쉽게 유지할 수 있는 경우를 탐구합니다. 특히, 알고리즘은 각 암이 사용자 참여를 유지하도록 보장하며 현실적인 문제를 해결하는 데 효과적입니다. 본 연구는 현실 세계의 문제를 해결하기 위한 멀티-암 밴딧 확장의 필요성을 강조하며, 기존 알고리즘의 성능을 개선하는 다양한 접근법을 논의합니다.
...
### Mitigating the Noise Shift for Denoising Generative Models via Noise Awareness Guidanc (https://arxiv.org/abs/2510.12497)
- **What's New**: 이번 논문은 기존의 denoising generative models의 문제점, 특히 noise shift 현상을 다루고 있습니다. 이 문제는 샘플링 중의 사전 정의된 노이즈 수준과 실제 중간 상태의 노이즈 수준 간의 불일치를 가리키며, 이는 모델의 생성 품질에 부정적인 영향을 미칩니다. 저자들은 Noise Awareness Guidance (NAG)라는 새로운 보정 방안을 제안하여, 샘플링 경로를 사전 정의된 노이즈 스케줄과 일관되게 유지하려고 합니다.

- **Technical Details**: NAG는 중간 상태의 고유한 노이즈 수준을 인식하고 이를 향상시키기 위해 특정 가이던스 신호를 생성하는 방법입니다. 또한, 저자들은 외부 분류기에 의존하지 않는 classifier-free NAG 변형을 제안하여, 노이즈 조건 dropout 방식을 통해 통합 모델을 함께 훈련할 수 있게 합니다. 이를 통해, noisy inputs에 대한 추가적인 복잡한 교육을 요구하지 않도록 했습니다.

- **Performance Highlights**: NAG는 다양한 실험을 통해 noise shift 문제를 크게 완화시키고, 주류 denoising generative models의 생성 품질을 현저히 향상시키는 것으로 나타났습니다. 저자들은 ImageNet 생성 및 다양한 감독 세부 조정 과제에서 NAG의 효과를 증명하였으며, 이 방법은 기존의 모델에 직접 통합되어 활용될 수 있습니다. 따라서 NAG는 현재 사용 중인 여러 generative 모델의 성능을 개선하는 데 기여할 전망입니다.
...
### CrossAD: Time Series Anomaly Detection with Cross-scale Associations and Cross-window Modeling (https://arxiv.org/abs/2510.12489)
Comments:
          Accepted by the thirty-ninth annual conference on Neural Information Processing Systems

- **What's New**: 이번 연구는 CrossAD라는 새로운 프레임워크를 제안하여 시간 시계열 이상 탐지에서 cross-scale associations와 cross-window modeling을 고려하고 있습니다. 기존 방법들은 보통 각 스케일의 정보를 독립적으로 모델링하거나 간단한 feature fusion 전략에 의존했지만, 우리는 cross-scale reconstruction과 query library를 통해 이러한 한계를 극복합니다. 이로 인해 전례 없는 정확도로 이상을 탐지할 수 있는 능력을 발휘하고 있습니다.

- **Technical Details**: CrossAD는 세 가지 단계로 이루어진 cross-scale reconstruction을 통해 각 스케일 간의 연관성을 명시적으로 모델링합니다: (1) 다중 스케일 생성 및 임베딩, (2) 스케일 독립적 인코딩, (3) 다음 스케일 생성. 또한, query library를 구축하여 각 슬라이딩 윈도우에서 공유되는 다양한 서브시퀀스 패턴을 모델링함으로써, 고정 윈도우 크기의 한계를 극복하고 글로벌한 맥락을 활용한 cross-window modeling을 실현합니다.

- **Performance Highlights**: CrossAD는 다양한 데이터셋에서 9개의 평가 기준을 사용하여 철저한 실험을 수행했으며, 기존의 state-of-the-art 모델들과 비교할 때 consistently superior한 성능을 보였습니다. 특히, 각 스케일에서 이상 징후를 감지하는 능력은 사전 학습된 정상 연관성을 기준으로 이루어지므로, 기존 방법들에 비해 더욱 부정확한 데이터 패턴을 포착할 수 있습니다.



### Time-Correlated Video Bridge Matching (https://arxiv.org/abs/2510.12453)
- **What's New**: 이번 연구에서는 시간 상관 데이터 시퀀스를 다룰 수 있는 Time-Correlated Video Bridge Matching (TCVBM)이라는 새로운 프레임워크를 제안합니다. 기존의 Bridge Matching 기법이 데이터 간 변환을 지원하지만, 시간적으로 연관된 데이터에는 적용되지 못했던 한계를 극복하고자 하였습니다. TCVBM은 비디오 도메인에서의 시퀀스 간 의존성을 명시적으로 모델링하여 시각적 일관성과 품질 향상을 도모합니다.

- **Technical Details**: TCVBM은 전통적인 Bridge Matching 방법을 확장하여, 시간적으로 상관관계 있는 비디오 데이터 시퀀스를 처리할 수 있도록 설계되었습니다. 이 프레임워크는 확산 과정에서 시간적 상관성을 통합하여 샘플링 프로세스를 개선하며, 이는 비디오 생성 및 조작의 질을 높이는 데 기여합니다. 연구에서는 TCVBM을 세 가지 비디오 관련 작업에 적용하여 비교 분석을 수행하였습니다: 프레임 보간(frame interpolation), 이미지-비디오 생성(image-to-video generation), 비디오 초해상도(video super-resolution).

- **Performance Highlights**: TCVBM은 기존의 확산 모델과 Bridge Matching 기반 메서드와 비교할 때, 정량적인 지표에서 뛰어난 성능을 보여주었습니다. 특히, TCVBM은 시간적 일관성을 유지하며 품질이 높은 비디오 생성 및 복원을 성취하였습니다. 이러한 결과는 TCVBM이 다중 프레임 간 상관관계를 효과적으로 모델링 할 수 있음을 입증하며, 비디오 생성 작업에 필수적인 요소라고 할 수 있습니다.
...
### Bayesian Optimization for Dynamic Pricing and Learning (https://arxiv.org/abs/2510.12447)
- **What's New**: 이 논문은 다이나믹 프라이싱(dynamic pricing)에 대한 비모수적(nonparametric) 접근 방식을 제안합니다. 기존 방법들이 가정하는 수요 함수의 특정 매개변수(parametric) 형태를 피하고, Gaussian Process (GP) 기반의 알고리즘을 도입하여 더 많은 유연성을 제공합니다. 특히, Bayesian Optimization (BO)을 활용하여 수익을 최적화하는 방법을 제시하며, 이는 복잡한 환경에서도 효과적임을 보여줍니다.

- **Technical Details**: 질문에 대한 답변으로, 논문은 무한 재고(infinite inventory) 및 유한 재고(finite inventory) 설정에서의 다이나믹 프라이싱 문제를 다룹니다. B​OBO-I​n​fInf 알고리즘은 수요 함수의 형태에 대한 가정을 하지 않고 성능 보장이 있는 이론적 레그렛(regret) 경계를 제공합니다. G​PGP-F​i​nFin-Model-Based는 Gaussian Processes를 사용하여 전이 모델을 학습하고 최적 가격 정책을 계산하는 방법을 제안하며, B​OBO-F​i​nFin-Heuristic는 모델이 없는 접근방식으로 직접 가격을 선택하는 알고리즘입니다.

- **Performance Highlights**: 실험 결과, 제안된 BO 기반 방법들이 기존의 최첨단 RL 알고리즘보다 수익 측면에서 우수한 성능을 보임을 입증합니다. 또한, 제안된 알고리즘은 가격 조정 횟수가 적더라도 빠른 수렴을 달성하며, 여러 환경에서도 높은 수익을 기록합니다. 이러한 결과들은 다이나믹 프라이싱 문제를 해결하기 위한 Bayesian Optimization의 힘과 실용성을 강조합니다.



### Continuous Uniqueness and Novelty Metrics for Generative Modeling of Inorganic Crystals (https://arxiv.org/abs/2510.12405)
Comments:
          13 pages (5 pages of main text), accepted to the AI4Mat workshop at NeurIPS 2025. See this https URL for the code

- **What's New**: 이번 연구에서는 기존의 결정 구조(distance function)가 가진 네 가지 주요 한계를 극복하기 위해 두 가지 연속 거리 함수를 제안합니다. 이들 연속 거리 함수는 생성 모델의 유일성과 참신성을 평가하는 데 있어 보다 정확하고 신뢰할 수 있는 분석을 제공합니다. 기존의 d_{smat} 함수는 화합물 간의 유사성을 정량화하는 데 실패하고 있으며, 구조적 차이와 성분 차이를 구별할 수 없습니다. 따라서, 새로운 정의된 거리 함수를 통해 이러한 단점을 해결하고자 합니다.

- **Technical Details**: 제안된 두 가지 거리 함수는 결정 구조에 대한 화학적 및 구조적 비교를 더 잘 정량화합니다. 화학적 비교를 위해 제안된 d_{magpie}는 145개의 속성을 포함하는 Magpie 핑거프린트 간의 유클리드 거리입니다. 구조적 비교를 위한 d_{amd}는 결정의 평균 최소 거리(AMD) 벡터 간의 L∞ 거리입니다. 이들 함수는 대칭성 불변 및 Lipschitz 연속성을 확보하여 보다 강인한 평가를 가능하게 합니다.

- **Performance Highlights**: 실험 결과, 새로운 연속 거리 함수들이 기존의 d_{smat} 및 다른 전통적인 거리 함수들이 놓친 통찰력을 드러내는 것으로 나타났습니다. 또한, 두 함수 모두 변수 변화에 강한 안정성을 가지며, 이러한 특성 덕분에 실질적인 평가에 매우 유용합니다. 본 연구는 이러한 새로운 거리 함수를 통해 결정 생성 모델의 성능 평가 문제를 개선할 수 있는 가능성을 제시합니다.



### Cautious Weight Decay (https://arxiv.org/abs/2510.12402)
- **What's New**: 이번 논문에서는 Cautious Weight Decay (CWD)라는 새로운 최적화 기법을 소개합니다. CWD는 매개변수 좌표의 부호가 최적화 기법의 업데이트 방향과 일치할 때만 가중치 감소(Weight Decay)를 적용하는 방법입니다. 기존의 decoupled decay와 달리, CWD는 원래 손실을 유지하며, 이를 통해 더 나은 성능을 보여줄 수 있습니다.

- **Technical Details**: CWD는 최적화 알고리즘의 업데이트 규칙을 간단히 수정하여 구현할 수 있는 특징이 있습니다. 주의 깊은 가중치 감소는 정보 손실을 피하고 매개변수와 업데이트의 일치하는 방향에서만 작용하므로, 인해 안정성과 정확성을 높여줍니다. 또한, Lyapunov 분석을 통해 표준 최적화 기법에서도 안정성을 보장하며, 원래의 손실을 최적화하는데 효율적인 연결을 제공합니다.

- **Performance Highlights**: CWD는 다양한 대규모 언어 모델 학습 및 ImageNet 분류에서 유의미한 개선을 보여줍니다. AdamW, Lion 및 Muon과 같은 최적화 알고리즘에 CWD를 적용하면 안정적인 수렴과 낮은 최종 검증 손실이 관찰되었습니다. 이러한 결과는 수 백만에서 수십억 파라미터에 이르는 범위에서 나타났으며, 재조정 없이도 높은 정확도를 기록했습니다.



### Enhanced Pre-training of Graph Neural Networks for Million-Scale Heterogeneous Graphs (https://arxiv.org/abs/2510.12401)
Comments:
          26 pages

- **What's New**: 최근 그래프 신경망 (Graph Neural Networks, GNNs)의 발전에서, 레이블이 없는 데이터에 대한 의존성을 줄이기 위해 자가 지도 방식(self-supervised)으로 GNN을 미리 훈련하는 접근법이 제안되었습니다. 본 논문에서는 대규모 이질적 그래프(heterogeneous graph)에서 GNN을 효과적으로 미리 훈련하는 프레임워크를 소개합니다. 특히 구조 인식(pre-training)과 의미 인식(semantic-aware) 두 가지 훈련 작업을 설계하여 이질적 그래프의 특성을 효과적으로 포착하는 것을 목표로 하고 있습니다.

- **Technical Details**: 연구진은 이질적 그래프의 걸맞은 구조를 캡처하기 위해 네트워크 스키마(network schema) 서브스페이스를 구성하고, 주의 메커니즘(attention mechanism)을 통해 미세한 이질적 정보를 추출합니다. 또한, 의미 불일치(semantic mismatch) 문제를 해결하기 위해 Perturbation subspace를 설계하여 모델이 의미 이웃에 집중할 수 있도록 합니다. 이러한 접근 방식은 대규모 이질적 그래프에 적합하게 미니 배치(mini-batch) 방식으로 조정되어 적용됩니다.

- **Performance Highlights**: 본 논문에서 제안한 방법은 실제 대규모 이질적 그래프에서 성능을 평가받으며, 기존의 최첨단 기술(state-of-the-art)보다 우수한 결과를 보여줍니다. 실험 결과에 따르면, 이 방법은 수백만 개의 노드와 수천만 개의 엣지를 포함하는 실제 세계의 이질적 그래프에서도 효과적으로 작동하며 성과를 개선합니다. 이 같은 성능 개선은 GNN의 효율적인 배치와 자원 효율성을 고려한 접근법 덕분입니다.



### Towards Cross-Modal Error Detection with Tables and Images (https://arxiv.org/abs/2510.12383)
- **What's New**: 이 논문은 대규모 조직에서 데이터 품질을 보장하는 데 중점을 두고 있으며, 특히 여러 데이터 양식을 다루는 복잡성에 대해 논의합니다. 전통적인 오류 탐지 및 수정 방법은 일반적으로 단일 모달리티에 집중되어 있어 크로스 모달 오류를 간과할 수 있습니다. 저자들은 크로스 모달 오류 탐지 문제를 제기하고 이 문제를 해결하기 위한 초기 단계로 다양한 방법들을 벤치마킹하여 평가합니다.

- **Technical Details**: 이 연구는 여러 데이터셋을 활용하여 크로스 모달 오류 탐지의 기초 벤치마크를 설계하고, 다양한 최첨단 오류 탐지 방법을 평가합니다. 주요 결과로 Cleanlab 및 DataScope와 같은 라벨 오류 탐지 프레임워크가 AutoML 프레임워크와 결합되어 가장 높은 성능을 보임을 확인하였습니다. 논문에서는 크로스 모달 오류 탐지를 위해 필요한 테이블 및 이미지 데이터 사용의 중요성을 강조하고, 현재 방법들의 한계를 지적합니다.

- **Performance Highlights**: 저자들은 평가 결과, Cleanlab과 DataScope가 다중 모달 상황에서 최상의 성능 (F1 점수)을 보여주었음을 밝혔습니다. 그러나, 이들 접근법은 현실 세계의 다양한 데이터에 적용할 때 한계가 있음을 시사합니다. 향후 연구의 필요성을 강조하며, 연구 커뮤니티에 크로스 모달 오류 탐지에 대한 포괄적인 접근이 필요하다고 주장합니다.



### Traveling Salesman-Based Token Ordering Improves Stability in Homomorphically Encrypted Language Models (https://arxiv.org/abs/2510.12343)
Comments:
          34 pages

- **What's New**: 이 논문에서는 기밀한 정보로 작동하는 대형 언어 모델(LLMs)의 다음 토큰 예측 문제를 해결하기 위한 TSP 기반의 토큰 재정렬 전략을 제안합니다. 이전 연구들은 동형 암호(homomorphic encryption, HE) 아래 LLM을 실행하는 데 감춰진 한계를 가지고 있었으나, 본 연구는 텍스트 생성의 어려움을 해결하고 모델의 안정성을 보장하는 방법을 제공합니다. 제안된 방법은 모델이 안정적으로 작동할 수 있도록 하며, 보안성을 유지하면서도 데이터 개인 정보를 보호합니다.

- **Technical Details**: 본 연구는 CKKS(이름을 가진 연구자들의 약칭화된 이름, Cheon et al., 2017, 2019) 스킴을 사용하여 동형 암호 환경에서의 효과적인 다음 토큰 예측 알고리즘을 제시합니다. 제안된 알고리즘은 최대값(maximum) 연산과 정렬(sorting) 연산을 피하고 SIMD(단일 명령어 다중 데이터) 친화적인 연산만 사용하므로, 실제적인 동형 구현이 가능하게 합니다. 또한, TSP(여행하는 세일즈맨 문제)를 적용하여 의미적으로 유사한 벡터를 인접하게 배치하여 모델 안정성을 확보하는 방법도 설명합니다.

- **Performance Highlights**: 이론적 분석과 실험 결과는 제안된 방법이 모델의 붕괴를 방지하고 생성된 텍스트의 일관성을 개선하며 데이터 프라이버시를 유지함을 보여줍니다. 특히, 오류를 작게 유지하면서도 토큰 예측의 정확도를 높이는 동시에, embedding 결합에 대한 안정성을 개선하였습니다. 따라서 이 연구는 실용적이고 프라이버시를 보호하는 LLM 추론의 가능성을 발전시키는 데 기여합니다.
...
- **What's New**: 이 논문은 극한 기상 현상에 대한 정확한 강우 예측을 위해 신규 물리 기반 그래프 신경망(Physics-informed Graph Neural Networks, GNNs)과 극값 분석 기법을 결합한 방법을 제시합니다. 태국 전역의 강우 예측 성능을 향상시키기 위해, 그래프 구조의 게이지 스테이션을 활용하여 복잡한 시공간 패턴을 포착할 수 있습니다. 또한, 이 모델은 원거리 연결성(teleconnections)을 통해 해석 가능성을 제공합니다.

- **Technical Details**: 제안된 Graph Attention Network with Long Short-Term Memory (Attention-LSTM)는 단순한 지형-강수(ologic-precipitation) 물리 모델에서 유래된 초기 엣지 특성을 사용하여 주의 메커니즘을 적용합니다. 또한, 극한 현상을 처리하기 위해 새로운 Spatial Season-aware Generalized Pareto Distribution (GPD) 방법을 사용하여 Peak-Over-Threshold (POT) 매핑을 수행합니다. 이를 통해 전통적인 기계 학습 모델의 한계를 극복합니다.

- **Performance Highlights**: 실험 결과, 본 방법은 극한 상황에 취약한 지역을 포함하여 대부분의 지역에서 잘 확립된 기준선 모델을 초월하는 성능을 보여주었습니다. 운영 예측 시스템 SEAS5와 비교했을 때, 실제 적용에서 극한 이벤트 예측을 개선하고 장기 물 관리에 도움이 되는 세밀한 해상도 맵을 생성하는 데 있어 실질적인 향상을 제공합니다.
...
### General Fourier Feature Physics-Informed Extreme Learning Machine (GFF-PIELM) for High-Frequency PDEs (https://arxiv.org/abs/2510.12293)
- **What's New**: 본 논문에서는 고주파(high-frequency) 및 변동주파수(variable-frequency) 성질을 다루는 편미분방정식(partial differential equations, PDEs) 해결에 있어 기존의 물리 정보 기반 극한 학습 기계(PIELM)가 직면하는 문제를 해결하기 위해 일반 푸리에 특징 물리 정보 기반 극한 학습 기계(GFF-PIELM)를 제안합니다. GFF-PIELM은 푸리에 특징 매핑(Fourier feature mappings, FFM)과 극한 학습 기계(Extreme Learning Machine, ELM) 네트워크를 효과적으로 결합하여 고주파 문제를 효율적으로 처리할 수 있습니다.

- **Technical Details**: GFF-PIELM은 세 가지 주요 단계로 구축됩니다. 첫 번째로, FFM의 변형을 ELM에 통합하여 푸리에 기반 활성화 함수로 사용하여 GFF-PIELM 프레임워크 내에서 하나의 은닉층을 유지합니다. 두 번째로, 은닉 뉴런(hidden neurons)에게 주파수 계수(frequency coefficients) 세트를 지정하여 ELM 네트워크가 목표 해결책의 다양한 주파수 구성 요소를 캡처할 수 있게 합니다. 마지막으로, ELM 출력 가중치의 분포를 모니터링하여 이러한 하이퍼파라미터(hyperparameters)의 혁신적이고 간단한 초기화 방법을 개발합니다.

- **Performance Highlights**: GFF-PIELM은 PIELM 프레임워크의 높은 정확도, 효율성, 간단함을 유지하며, 고주파 문제를 효과적으로 처리하기 위한 FFM의 능력을 계승합니다. 연구에서는 총 10개의 수치 예제를 포함한 5개의 사례 연구를 수행하여 GFF-PIELM의 실행 가능성과 유효성을 강조합니다. 전통적인 PIELM과 비교할 때, GFF-PIELM 접근 방식은 훈련 시간 및 아키텍처 복잡성에 대한 추가 비용 없이 예측 정확도를 상당히 향상시킵니다.



### Multi-Action Self-Improvement for Neural Combinatorial Optimization (https://arxiv.org/abs/2510.12273)
- **What's New**: 본 논문은 MACSIM(Multi-ACtion Self-Improvement Method)을 제안하여 기존의 자가 개선(self-improvement) 프레임워크를 확장합니다. MACSIM은 여러 에이전트의 동시 작업을 예측하여 문제에 대한 다수의 동등한 전문가 할당을 학습하는 방식을 채택했으며, 이를 통해 에이전트 간의 협조를 향상시킵니다. 이로 인해 샘플 효율성이 증가하고 솔루션 생성 속도가 크게 향상되었습니다.

- **Technical Details**: MACSIM은 모든 에이전트를 공동으로 할당하는 다중-작업 정책(multi-agent policy)을 사용할 뿐만 아니라, 세트 예측 손실(set-prediction loss)을 통해 문제의 대칭성을 활용합니다. 이로 인해 각 결정 단계에서 정책이 동시 에이전트 할당을 예측하고, 각 상태별로 여러 전문가 할당에서 학습할 수 있습니다. 결과적으로, 이 모델은 다중 에이전트 환경에서의 협조적 행동을 효과적으로 학습할 수 있습니다.

- **Performance Highlights**: 실험 결과, MACSIM은 여러 조합 최적화(combinatorial optimization) 문제에 대해 성능 향상을 일관되게 보여주었으며, 기존의 자가 개선 방법들보다 솔루션 생성 지연(latency)이 감소했습니다. 특히, 차량 경로 설정(vehicle routing)과 일정 수립(scheduling) 분야에서 개선된 솔루션 품질을 확인할 수 있었습니다. MACSIM은 이론적 논의뿐만 아니라 강력한 실험적 증거를 통해 그 효과성을 입증하였습니다.
...
### FedMMKT:Co-Enhancing a Server Text-to-Image Model and Client Task Models in Multi-Modal Federated Learning (https://arxiv.org/abs/2510.12254)
- **What's New**: 이 논문은 Federated Multi-modal Knowledge Transfer (FedMMKT)라는 새로운 프레임워크를 소개하여, 데이터 프라이버시를 유지하면서 클라이언트의 T2I 모델과 서버 모델의 성능을 상호 개선할 수 있도록 한다. 기존의 방법들이 데이터의 통합이나 조정에 한계를 보였던 반면, FedMMKT는 다양한 다중 모달 클라이언트의 전문 지식을 안전하게 통합할 수 있는 솔루션을 제공한다. 또한, 일반적인 공공 데이터에 대한 의존도를 줄이고, 각각의 클라이언트의 지식을 활용하여 합성 데이터의 품질을 개선한다.

- **Technical Details**: FedMMKT는 중앙 서버와 다수의 클라이언트가 있는 이기종 Federated Learning (FL) 환경을 고려하여 설계되었다. 각 클라이언트는 자신의 로컬 데이터셋을 바탕으로 개인 모델을 훈련한 뒤, 서버 모델에 교차 모달 데이터 합성을 통해 지식을 전이한다. 데이터 프라이버시를 보장하기 위해 클라이언트의 데이터와 로컬 모델은 서버를 떠나지 않으며, 모든 과정은 서버 내에서 안전하게 이루어진다.

- **Performance Highlights**: 실험 결과, FedMMKT는 클라이언트와 서버 모델 모두에서 성능이 크게 향상되는 것을 보여준다. FedMMKT는 클라이언트에서 생성된 다중 모달 데이터를 효과적으로 이용하여 T2I 모델의 생성 능력을 증대시키고, 각 클라이언트의 특정 도메인 작업에 대한 모델 성능을 향상시키는 데 기여한다. 이러한 성능 개선은 기존의 단일 모달 접근 방식에 비해 훨씬 더 효율적인 지식 통합을 가능하게 한다.
...
### Optimal Regularization for Performative Learning (https://arxiv.org/abs/2510.12249)
- **What's New**: 이번 연구의 핵심은 performative learning(퍼포머티브 학습)에서 정규화(regularization)가 작용하는 방식을 탐구하는 것입니다. 기존의 머신러닝 모델이 사용자 행동에 의해 데이터 분포가 변화하는 현상에 대해, 저자들은 높은 차원의 ridge regression(리지 회귀)에서 정규화가 이러한 performative 효과를 완화할 수 있음을 제시합니다. 연구 결과, 일반적인 데이터 설정에서는 performative 효과가 테스트 위험을 악화하지만, 오버파라미터화된(pover-parameterized) 환경에서 정규화가 오히려 긍정적인 영향을 미친다는 것을 보여주었습니다.

- **Technical Details**: 저자들은 정규화가 모델 성능에 미치는 영향을 두 가지 환경에서 고찰합니다: 하나는 충분한 데이터가 있는 모집단(population) 환경과 다른 하나는 데이터 샘플의 수가 파라미터 수의 일정 비율인 오버파라미터화된 환경입니다. 연구는 ridge regularization이 performative 효과에 강하게 민감하며, 이를 통해 성능 손실을 완화할 수 있다는 것을 이론적으로 입증했습니다. 특히, 정규화의 최적 스케일이 performative 효과의 전체 강도에 비례하여 조정될 수 있음을 발견했습니다.

- **Performance Highlights**: 실험을 통해 저자들은 최적 정규화 파라미터가 synthetic dataset(합성 데이터셋) 및 real-world dataset(실제 데이터셋)에서 어떻게 작용하는지 입증하였습니다. 연구 결과, 작은 잡음 분산 환경에서 최적 정규화는 예측 특성에 대한 performative 효과와 같은 방향으로 움직이며, 이는 여전히 존재하는 경향을 강화하는 performative 효과가 있을 때 성능이 향상됨을 보여주었습니다. 이러한 발견은 머신러닝이 실제 배포 과정에서 사용자와 상호작용할 때 발생하는 복잡한 동적 시스템을 이해하는 데 중요한 기여를 합니다.
...
- **What's New**: 이번 연구에서는 Interpretable Multi-Dimensional Graph Attack (IMDGA)라는 새로운 인공지능 공격 프레임워크를 제안합니다. IMDGA는 그래프의 구조와 텍스트 특성에 대해 다층적인 perturbation을 orchestrate (조정)하여 Graph-LLM의 취약성을 보다 깊이 이해할 수 있도록 돕습니다. 이 기법은 기존의 공격 방법론을 통합하여, 해석 가능성과 공격 효과성을 동시에 향상시킬 수 있도록 설계되었습니다.

- **Technical Details**: IMDGA는 세 가지 핵심 모듈로 구성됩니다. 첫 번째 모듈인 Topological SHAP은 노드 분류 예측에 영향을 미치는 중요한 단어의 기여도를 정량화합니다. 두 번째로, Semantic Perturbation Module은 논리적으로 대체 가능한 단어들을 생성하고, 마지막으로 Influence-based Edge Pruning Module을 통해 가장 취약한 에지를 식별하여 공격의 정밀도를 높입니다.

- **Performance Highlights**: IMDGA는 다양한 데이터셋과 아키텍처에서 기존 방법들보다 우수한 해석 가능성, 공격의 효과성, 스텔스성 및 강건성을 demonstrated (증명)합니다. 이 연구는 TAG 표현 학습에서 중요한 약점을 드러내며, Graph-LLM의 회복력을 개선하기 위한 귀중한 통찰을 제공합니다.



### Hierarchical Koopman Diffusion: Fast Generation with Interpretable Diffusion Trajectory (https://arxiv.org/abs/2510.12220)
Comments:
          NeurIPS 2025

- **What's New**: 이 논문에서는 **Hierarchy Koopman Diffusion**라는 새로운 프레임워크를 소개합니다. 이 방법은 샘플을 생성하는 동안 해석 가능성과 세밀한 제어를 모두 성취하면서 단일 단계의 샘플링을 가능하게 합니다. 이를 통해 이미지 생성의 해석 가능성을 높이고, 생성 과정에서의 수동 개입을 허용하는 혁신적인 접근 방식을 제시합니다.

- **Technical Details**: Hierarchical Koopman Diffusion은 Koopman 연산자 이론에 기반하여 비선형 확산 동역학을 잠재 공간으로 가져와 진화가 전 세계적으로 선형 연산자에 의해 지배되도록 합니다. 이 방법은 반복적인 샘플링을 없애고, 생성 과정의 중간 상태에 대한 완전한 접근을 제공합니다. 또한, 스페어 분석을 통해 발산 메커니즘을 해석하고 조작할 수 있는 원리를 제공합니다.

- **Performance Highlights**: CIFAR-10과 FFHQ 데이터셋을 통해 제안된 프레임워크의 경쟁력 있는 단일 단계 생성 성능을 보여줍니다. 생성 품질을 넘어서, 인지 동역학을 해석하고 주파수 특화 개입을 통해 생성 단계 중 단계 개입의 유용성을 입증합니다. 이러한 연구는 설명 가능한 이미지 합성을 위한 길을 열어줍니다.
...
### Self-Verifying Reflection Helps Transformers with CoT Reasoning (https://arxiv.org/abs/2510.12157)
Comments:
          Accepted by NeurIPS2025

- **What's New**: 이 논문에서는 자기 검증(self-verifying) 반영을 통해 작은 트랜스포머 모델이 문제 해결 성능을 개선할 수 있는 최소한의 추론 체계를 제시합니다. 기존의 대형 언어 모델(LLMs)에 비해 이 시스템은 간단히 효과적인 분석을 가능하게 하며, 보다 저렴한 비용으로 포괄적인 실험을 진행할 수 있게 합니다. 이는 작은 트랜스포머를 통해 기계학습의 효율성을 높일 수 있음을 보여줍니다.

- **Technical Details**: 최소한의 추론 체계는 자연어 없이도 작동하며 기본적인 추론 능력을 지원합니다. 이 체계 아래에서 모델은 각 사고 단계의 정확성을 스스로 검증할 수 있으며, 잘못된 단계를 재샘플링하거나 이전 단계로 돌아갈 수 있는 기능을 갖추고 있습니다. 이론적으로, 자기 검증 반영이 적절히 오류가 한정될 경우 추론 정확도를 개선할 수 있음을 증명했습니다.

- **Performance Highlights**: 실험 결과, 1M, 4M, 16M 파라미터를 가진 작은 트랜스포머가 정수 곱셈 및 스도쿠 퍼즐 해결에서 LLM 수준의 성능을 달성하였습니다. 학습과 반영 실행 모두에서 자기 검증이 긍정적인 영향을 미쳤으며, 특히 RL(강화학습)을 통해 반영 빈도가 늘어나지만, 주로 수치적 패턴을 최적화하여 일반화 문제 해결 능력이 제한적임을 발견했습니다.
...
Comments:
          To appear in IEEE TrustCom 2025

- **What's New**: 이 논문은 연합 학습(FL)에서의 의도적인 공정성 공격을 제안합니다. 이 공격은 공격력이 있는 참가자가 공정성 손실을 증가시켜 편향된 모델을 생성하는 방식입니다. FL은 데이터 이동을 제한하면서 모델 공유를 가능하게 하여 참가자들에게 독립성을 제공하지만, 이로 인해 악의적인 공격에 대해 취약해집니다.

- **Technical Details**: FL은 여러 장치에서 데이터를 분산하여 훈련함으로써 프라이버시를 보호하는 머신러닝 기법입니다. 기존의 독립성 있는 데이터 훈련은 다양한 데이터 분포와 역사적 편향으로 인해 공정성 문제를 발생시킵니다. 논문에서 제안하는 공격은 공정성 메트릭을 최적화하는 문제를 해결하여 모델의 정확성을 유지하면서 편향을 강화합니다.

- **Performance Highlights**: 실험 결과, 논문의 공격은 FL 시스템 내 단일 악의적 클라이언트가 존재하는 경우에도 편향을 최대 90%까지 증가시킬 수 있음을 보여주었습니다. 이는 기존의 바이잔틴 방어 및 공정성 인식 집계 방식에 대해 강력한 공격 효과를 입증한 것입니다. 이러한 접근법은 FL의 공정성 이슈를 해결하기 위한만큼, 새로운 위협 모델을 제시합니다.



### Graph Few-Shot Learning via Adaptive Spectrum Experts and Cross-Set Distribution Calibration (https://arxiv.org/abs/2510.12140)
Comments:
          NeurIPS25

- **What's New**: 본 연구에서는 GRACE라는 새로운 그래프 few-shot learning 프레임워크를 제안합니다. GRACE는 Adaptive spectrum experts와 Cross-set distribution calibration 기법을 통합하여 그래프 FSL의 문제를 해결합니다. 이를 통해 모델의 일반화 성능을 향상시키고, 다양한 지역적 구조에 적응할 수 있도록 설계되었습니다.

- **Technical Details**: GRACE는 노드의 국소적인 연결 패턴을 모델링하기 위해 여러 개의 전문가를 이용하는 노드 전용 필터링 메커니즘을 개발합니다. 각 전문가는 특정 그래프 필터링 행동을 캡처하며, 게이팅 메커니즘을 사용하여 노드의 구조적 특성에 기반해 전문가 가중치를 적응적으로 할당합니다. 또한, 지원 세트에서 클래스 프로토타입을 초기 도출한 후, 쿼리 세트에 의한 명시적 보정 과정을 통해 개선됩니다.

- **Performance Highlights**: GRACE는 여러 벤치마크 데이터 세트에서 기존의 최첨단 방법들보다 일관되게 뛰어난 성능을 보여주었습니다. 특히, GRACE는 복잡한 실세계 그래프에서의 지역적 구조 변화와 분포 이동을 조정하여 모델의 일반화 성능을 향상시킵니다. 이러한 결과는 그래프 FSL의 변별력을 높이는 데 중요한 기여를 합니다.



### nuGPR: GPU-Accelerated Gaussian Process Regression with Iterative Algorithms and Low-Rank Approximations (https://arxiv.org/abs/2510.12128)
Comments:
          22 pages, 6 figures, published in SIAM Journal on Scientific Computing, E-print available at: this https URL

- **What's New**: 최근 가우시안 프로세스 회귀(Gaussian Process Regression, GPR)는 감독형 확률 모델로서 많은 주목을 받고 있습니다. 본 논문에서 제안된 nuGPR 프레임워크는 GPR의 높은 계산 비용 문제를 해결하기 위해 설계되었습니다. 이 프레임워크는 수치 선형 대수(numerical linear algebra)의 여러 개념을 활용하여 GPR의 주요 계산 단계를 효율적으로 줄이는 접근 방식을 통합합니다.

- **Technical Details**: nuGPR는 클러스터링된 입력 데이터를 활용하여 공분산 행렬(covariance matrix)의 대각선 블록에서만 비싼 연산을 제한하고, 각 클러스터의 대표 점을 통해 비대각 블록을 근사하는 방법을 사용합니다. 또한, 전처리된 공액 경량 컨설던트(preconditioned conjugate gradient, PCG) 기법을 통해 공분산 행렬의 역수와 로그 행렬식(log determinant)을 계산하여 성능을 극대화합니다. 이러한 접근은 GPU의 아키텍처에 유리하며, CUDA Toolkit을 효율적으로 활용하여 최적화된 계산을 수행합니다.

- **Performance Highlights**: nuGPR는 기존의 GPR 구현과 비교했을 때, 총 훈련 시간을 최대 2배 줄이고 메모리 소비를 최대 12배 줄이는 성과를 보입니다. 다양한 합성 및 실제 데이터셋에서 실험한 결과, nuGPR은 정확도를 유지하면서 계산 속도와 메모리 사용량에서 상당한 이점을 제공합니다. 이는 GPyTorch와 같은 기존의 최고급 구현과 비교했을 때 두드러진 성능 향상을 의미합니다.
...
### Rethinking the Role of Dynamic Sparse Training for Scalable Deep Reinforcement Learning (https://arxiv.org/abs/2510.12096)
- **What's New**: 이번 연구에서는 심층 강화 학습(Deep Reinforcement Learning, DRL)에서의 확장성 문제를 해결하기 위해 Module-Specific Training (MST)이라는 실용적인 프레임워크를 제안합니다. MST는 다양한 RL 알고리즘에서 알고리즘 수정 없이도 상당한 확장성 향상을 보여줍니다. 기존의 동적 훈련 방법들이 가진 한계를 극복하기 위해, 모듈별 특화 훈련 전략을 통해 네트워크 아키텍처 개선의 이점을 극대화합니다.

- **Technical Details**: 본 연구에서는 정적 및 동적 훈련 방법의 구현 세부사항을 구체적으로 기술하며, 희소성 메커니즘과 프루닝 주기, 구조 적응 절차를 포함합니다. 정적 희소 훈련은 무작위 프루닝을 통해 각 레이어에 이진 마스크를 생성하고, 이 마스크는 훈련 기간 동안 변하지 않습니다. 동적 희소 훈련(Dynamic Sparse Training, DST)은 초기에는 무작위 프루닝 네트워크를 사용하고, 훈련 중에 네트워크 구조를 동적으로 발전시킵니다.

- **Performance Highlights**: 연구 결과, 동적 희소 훈련 전략이 모듈 특화 이점을 제공하며, DRL의 기본 확장성 기초를 보완하는 것으로 나타났습니다. 다양한 RL 알고리즘에서 성능이 크게 향상되었으며, 이러한 결과는 알고리즘 수정 없이도 이루어졌습니다. 기존의 동적 훈련 방법들과 비교하여, MST는 DRL 모델의 확장성을 획기적으로 개선할 수 있는 가능성을 제시합니다.



### H4G: Unlocking Faithful Inference for Zero-Shot Graph Learning in Hyperbolic Spac (https://arxiv.org/abs/2510.12094)
- **What's New**: 본 논문에서는 그래프-텍스트 정렬(Graph-Text Alignment) 분야에서 발견된 과도한 추상화(over-abstraction) 문제를 지적하고, 이를 해결하기 위한 새로운 프레임워크 H4G를 제안합니다. 기존의 방법들이 과도한 하이퍼볼릭 반경(hyperbolic radius)에서 작동하여 구조적 세부 정보를 손실하는 문제를 해결하고, 정교한 패턴을 복원하기 위한 방안을 제시합니다. H4G는 배치 대각선(block-diagonal) 스케일링 행렬과 Möbius 행렬 곱셈을 활용하여 효율적으로 반경을 조절해 나갑니다.

- **Technical Details**: H4G 프레임워크는 하이퍼볼릭 공간의 임베딩 반경을 체계적으로 축소하여, 기존 방법에서 손실된 세부적인 구조 정보를 복원합니다. 이 프레임워크에서 도입된 학습 가능한 대각선 스케일링 행렬은 Fuß와 같은 고급 수학적 기법을 통해 구현되며, 반경 조절을 통해 정보의 다체적 성질을 유지합니다. 이러한 방법론은 다양한 그래프 작업에 내재된 구조적 세부사항을 반영하기 위해 representational granularity를 조정하는데 중점을 둡니다.

- **Performance Highlights**: H4G는 이종 그래프(heterophilic graph)에서 12.8%, 동종 그래프(homophilic graph)에서 8.4%의 성능 향상을 달성하며, 최신 방법들과 비교하여 우수한 제로샷(zero-shot) 정확도를 보여줍니다. 이 결과는 하이퍼볼릭 반경의 감소가 다체적 구조 정보의 신뢰할 수 있는 표현을 가능하게 하여 제로샷 그래프 학습을 더욱 발전시킬 수 있음을 시사합니다. H4G는 최소한의 계산 비용으로 중요한 지역 패턴을 유지하고, 다양한 그래프 학습 상황에 걸쳐 효과적인 방법을 제시합니다.



### GraphShaper: Geometry-aware Alignment for Improving Transfer Learning in Text-Attributed Graphs (https://arxiv.org/abs/2510.12085)
- **What's New**: 그래프 기반 모델(Graph foundation models)은 다양한 그래프 도메인에서 전이 가능한 표현(representations)을 학습하기 위한 혁신적인 패러다임을 나타냅니다. 기존의 방법들은 대형 언어 모델(large language models)을 활용하여 그래프와 텍스트 모달리티(text modalities)를 공통 표현 공간으로 통합하는 대조 학습(contrastive learning)을 사용하지만, 구조적 경계(structural boundaries)에서 성능 저하를 보입니다. 본 논문에서는 이러한 문제를 해결하기 위해 'GraphShaper'라는 기하학적으로 인식하는 프레임워크를 소개하여 그래프 인코딩을 향상시키는 방안을 제시합니다.

- **Technical Details**: GraphShaper은 여러 기하학적 특성을 통합하는 전문가 네트워크(expert networks)를 도입하여 다양한 그래프 구조가 가지는 고유한 기하학적 다양성을 존중하는 정렬 프레임워크를 설계합니다. 이 방법은 노드의 지역 구조적 특성에 기반하여 동적으로 융합 가중치(fusion weights)를 계산하며, 각 그래프의 기하학적 속성을 효과적으로 통합할 수 있도록 합니다. 특히, 트리 구조(tree structures)와 순환 구조(cyclic patterns) 등의 слож h적 패턴을 인코딩할 때 발생하는 기하학적 충돌(conflicts)을 해결하는 데 중점을 두었습니다.

- **Performance Highlights**: 광범위한 실험을 통해 GraphShaper는 인용 네트워크(citation networks)에서 9.47%의 정확도 개선과 사회 네트워크(social networks)에서 7.63%의 개선을 달성하였습니다. 이 성능 향상은 제로샷(zero-shot) 설정에서도 검증되었으며, 기존의 그래프 모델들이 직면했던 기하학적 한계를 극복한 것으로 평가됩니다. GraphShaper은 라벨이 없는 대상 데이터 없이도 강력한 교차 도메인 전이(cross-domain transfer)를 가능하게 합니다.
...
- **What's New**: 이번 연구에서는 Neural Networks의 학습 과정에서 발생하는 동적 영향을 고려한 새로운 데이터 기여도 추정 방법을 제시합니다. 기존의 Training Data Attribution(TDA) 방법은 샘플 간의 영향을 정적인 것으로 간주했으나, 우리는 학습 단계에 따라 영향이 비선형적으로 변화할 수 있음을 예측합니다. 이를 통해 모델의 의미적 계층 구조에 대한 학습 과정을 분석하였습니다.

- **Technical Details**: 우리는 단일 학습 이론(singular learning theory)에 기초한 단계별 데이터 기여도 추정 프레임워크를 도입하여, 변화하는 패턴을 가진 영향을 연구합니다. 모델-진실-사전 분포의 삼중 구조를 통해 posterior 분포를 구하고, 이를 Bayesian Influence Function(BIF)으로 확장했습니다. 또한, 이론적인 기반으로 Phase Transitions를 설명하며, 샘플들의 영향을 보다 정확하게 추정할 수 있는 방법론을 제안하였습니다.

- **Performance Highlights**: 모델의 영향은 언어 모델을 통해 실험적으로 검증되었으며, 토큰 수준의 영향 변화를 통해 학습 과정에서 나타나는 알려진 발전 단계와 잘 일치함을 보여주었습니다. 이러한 발견은 Neural Networks의 학습 과정에서의 동적 상호작용을 더욱 깊이 이해하는 발판이 될 것입니다. 특히, 각 단계에서 발생하는 기여도 변화를 정량적으로 분석하는 데 기여하여, 더 나은 모델 학습 전략 및 데이터 효율성을 제시할 수 있습니다.
...
### Mamaba Can Learn Low-Dimensional Targets In-Context via Test-Time Feature Learning (https://arxiv.org/abs/2510.12026)
Comments:
          34 pages

- **What's New**: Mamba는 최근 발표된 선형 시간 시퀀스 모델로, 계산 효율성과 강력한 경험적 성능으로 주목받고 있습니다. 그러나 그 메커니즘에 대한 이론적 이해는 아직 제약이 있습니다. 본 논문에서는 Mamba의 인컨텍스트 학습(in-context learning, ICL) 능력에 대한 이론적 분석을 제공하며, 낮은 차원의 비선형 목표 함수로 정의된 작업에 초점을 맞춥니다.

- **Technical Details**: Mamba는 시간에 따라 은닉 상태를 유지하고 변환하는 순환 상태 공간 모델 구조를 가지고 있으며, 테스트 시간에서 직접 맥락 예제에서 적절한 방향을 추출할 수 있는 능력을 갖추고 있습니다. 본 연구에서는 단일 인덱스 모델(single-index model)을 통해 Mamba의 ICL 능력을 분석하고, 줄어든 샘플 복잡도(sample complexity)를 보입니다. 이러한 분석은 Mamba가 테스트 시간에서 기능 학습을 수행할 수 있음을 확인시킵니다.

- **Performance Highlights**: Mamba는 테스트 시간 기능 학습을 통해 커널 회귀 기초선을 초월할 수 있으며, 비선형 게이팅(nonlinear gating) 메커니즘을 통해 성능을 향상시킵니다. Mamba의 인컨텍스트 학습 기전은 순환 상태 공간 업데이트를 통해 이루어지며, 이는 기존의 attention 기반 모델과는 질적으로 다른 메커니즘으로써, ICL의 이론적 지형을 확장합니다.



### Nonlinear discretizations and Newton's method: characterizing stationary points of regression objectives (https://arxiv.org/abs/2510.11987)
- **What's New**: 이 논문은 고전적인 1차 최적화 기법 대신 2차 최적화 기법에 대한 새로움을 제시합니다. 특히, 기존의 quasi-Newton 방법 대신에 exact Newton 방법을 고려하고, 그 사용이 신경망 훈련에서 실패를 유발할 수 있음을 보여줍니다. 이 글에서는 이러한 실패가 신경망의 비선형 이산화(Nonlinear Discretizations) 및 손실 경관의 정적 점(Stationary Points) 구조에 대한 통찰을 제공합니다.

- **Technical Details**: 논문에서는 regression 문제에 대해 exact Newton 방법과 quasi-Newton 방법의 차이를 설명합니다. Exact Newton 방법은 목표 함수의 2차 미분 행렬인 Hessian 행렬을 사용해서 로컬 2차 근사를 빌드합니다. 반면, quasi-Newton 방법은 optimizer의 최적화 이력을 통해 Hessian의 근사치를 지속적으로 갱신합니다.

- **Performance Highlights**: 연구 결과, exact Newton 방법은 단순한 회귀 문제에서도 외면적인 해(trivial solution)를 찾는 경향이 있어, 고차원 손실 경관에서의 효과적인 수행이 어렵다는 것을 드러냅니다. 이는 quasi-Newton 방법이 실제로 더 나은 성능을 보여주는 이유와 관련이 있으며, 특히, 신경망 훈련에서 이 방법들이 어떻게 저지점을 피할 수 있는지를 설명합니다.



### Learning by Steering the Neural Dynamics: A Statistical Mechanics Perspectiv (https://arxiv.org/abs/2510.11984)
- **What's New**: 이번 연구는 생물학적 신경망과 현대 인공지능(AI) 간의 간극을 메우기 위한 새로운 접근 방식을 제안합니다. 특히, 자가 결합(self-coupling) 강도를 조절하여 이루어지는 고차원 동적 고정점의 구조 변화와 이로 인해 발생하는 위상 전이(phase transition)에 중점을 두고 있습니다. 연구 결과는 저전력에서 견고한 학습을 위한 유기적인 신경망의 동역학 뒷받침을 제시하며, 표준 백프로파게이션(backpropagation) 없이도 신뢰할 수 있는 학습 방법을 제시합니다.

- **Technical Details**: 연구는 비대칭 반복 신경망에서 자가 결합 강도에 따른 고정점 수의 닫힌 형태 표현을 도출합니다. 고정점의 구조는 자가 결합 강도가 임계값을 넘어서는 순간 변경되는 것으로, 이때 밀집하고 다양한 클러스터가 형성됩니다. 이러한 고정점은 비선형 분류(task) 문제를 다루기 위해 간단한 비동기적 동적 규칙에 접근할 수 있습니다. 저자는 이러한 이론적 분석을 바탕으로 자가 결합을 사용하는 생물학적으로 타당한 학습 알고리즘을 제안합니다.

- **Performance Highlights**: 제안된 알고리즘은 MNIST의 얽힌(entangled) 버전을 성공적으로 학습할 수 있으며, 심층 구조를 활용하여 계층적 표현을 개발하고 이질 연결(hetero-association) 용량을 증가시킬 수 있습니다. 여러 아키텍처(architecture)에서 사용 가능하며, 자가 결합 강도의 선택이 알고리즘 성능에 중요한 영향을 미치는 것을 보여주고, 이는 신경망 내 고정점의 접근성과 관련이 있습니다. 향후 연구는 더욱 복잡한 기계 학습 벤치마크에 대한 접근성을 높이는 방향으로 나아갈 것입니다.
...
- **What's New**: 이 논문은 자연어 처리에서 Transformers의 이해를 위한 새로운 수학적 틀을 제안합니다. 기존의 방법들이 모델의 추론 과정에서 중간 예측을 식별하는 데 성공했지만 각 레이어가 예측 경로에서 어떻게 전환을 촉진하는지를 기계적으로 모델링하는 데 한계가 있었습니다. 이에 따라 저자들은 물리학에서 영감을 받아 Transformers의 생성 과정에 대한 물리 기반의 관점을 개발하고자 합니다.

- **Technical Details**: 저자들은 새롭게 제안된 QLENS를 통해 Transformer의 잠재 활성화를 Hilbert 공간의 상태 벡터로 변환하고, 이 상태가 숨겨진 레이어를 통해 진화하는 과정을 연구합니다. 각 레이어는 유니터리 연산자로 재구성되고, 최종 확률 분포는 특정 측정 연산자를 적용하여 얻어집니다. QLENS는 Transformer의 레이어가 예측 경로에서 각각 어떻게 기여하는지를 기계적으로 특성화할 수 있는 수학적 틀을 제공합니다.

- **Performance Highlights**: QLENS를 검증하기 위해 저자들은 장난감 Transformer를 이용하여 개별 레이어가 모델의 예측 경로에 미치는 영향을 조사하는 개념 증명(proof-of-concept) 실험을 수행하였습니다. 이를 통해 QLENS는 나름의 수학적 방식으로 각 레이어의 효과를 설명할 수 있음을 보여주었습니다. 이 연구는 Transformers에 대한 더 깊은 이해를 향한 길을 열어주는 중요한 기초 작업으로 평가됩니다.
...
### On efficiently computable functions, deep networks and sparse compositionality (https://arxiv.org/abs/2510.11942)
- **What's New**: 본 연구에서는 Turing computability의 효율성이 특정 입력/출력 정밀도에서 compositionally sparse (bounded-fan-in, polynomial-size) DAG 표현의 존재를 의미한다는 것을 보여줍니다. 특정 함수가 efficiently computable할 때, 각 정밀도에 대해 해당 함수를 나타내는 효율적인 구조적 표현을 구축할 수 있습니다. 이러한 구조적 희소성은 최적화 문제에서의 계층적 탐색 장점도 나타냅니다.

- **Technical Details**: 글에서 다루는 정밀도 및 이산화 모델을 통해, 이산적인 계산 모델과 연속 함수를 연결할 수 있는 방법을 제시합니다. 입력 및 출력을 유한 정밀도로 표현하며, 특정 함수가 Turing 컴퓨터에 의해 효율적으로 계산할 수 있을 때, 이에 해당하는 이산 함수의 집합을 구성합니다. 또한, 이 함수 집합을 통해 회로와 신경망을 구축하여 정밀도를 높일 수 있음을 설명합니다.

- **Performance Highlights**: 이 연구에서 제안하는 방법론은 Neural Networks의 성능을 강화하는 것을 목표로 합니다. 각 Boolean 게이트를 적절한 크기의 Neural Network로 교체하고 이를 통해 나타나는 정확도는 정밀도에 따라 조정됩니다. 최종적으로, 본 연구는 주어진 목표 정밀도에 도달하기 위해 효율적으로 구성된 Neural Network를 생성할 수 있음을 보여줍니다.



### Efficient Restarts in Non-Stationary Model-Free Reinforcement Learning (https://arxiv.org/abs/2510.11933)
Comments:
...
- **What's New**: 이번 연구에서는 모델-프리 비정상 강화 학습을 위한 세 가지 효율적인 리스타트 패러다임을 제안합니다. 기존의 RestartQ-UCB 알고리즘에서 발견된 두 가지 주요 문제인 완전 망각(complete forgetting)과 정해진 시간에만 리스타트가 발생하는 점을 보완하기 위해, 부분(partial), 적응형(adaptive), 그리고 선택적(selective) 리스타트를 도입합니다. 이러한 접근 방식들은 환경 동적에 따라 더 적합하게 알고리즘을 수정할 수 있도록 돕습니다.

- **Technical Details**: 비정상 환경에서 강화 학습은 탐색(exploration)과 활용(exploitation), 기억(remembering)과 잊기(forgetting) 사이의 두 가지 주요 trade-off를 관리해야 하며, 본 연구는 이러한 관계를 해결하기 위한 다양한 리스타트 방법론을 소개합니다. 각 리스타트 방법론은 QQ-테이블의 업데이트 방식을 다르게 하여 학습의 효율성을 높이고, 동적 보상(dynamic regret)을 줄이는 데 중점을 둡니다. 새로운 RandomMDP 및 Bidirectional Diabolical Combination Locks 환경에서의 실험을 통해 이러한 접근 방식의 유효성을 검증하였습니다.

- **Performance Highlights**: 실험 결과, RandomMDP 환경에서 7474%, BDCL 환경에서는 9191%의 동적 보상 감소를 기록했습니다. 특히 BDCL 환경은 탐색이 어려운 설계로 되어 있어, 이 결과는 더욱 주목할 만합니다. 이러한 성과는 우리가 제안한 리스타트 수정이 강화 학습의 비효율성을 극복할 수 있는 가능성을 보여줍니다.
...
### Variational Mixture of Graph Neural Experts for Alzheimer's Disease Biomarker Recognition in EEG Brain Networks (https://arxiv.org/abs/2510.11917)
- **What's New**: 이번 연구는 알츠하이머병(AD) 및 전두측두형 치매(FTD) 진단을 위한 새로운 접근 방식인 변이 혼합 그래프 신경 전문가(Variational Mixture of Graph Neural Experts, VMoGE)를 제안합니다. VMoGE는 EEG의 주파수 대역별 바이오마커를 식별하고 구조화된 변이 추론을 통해 치매 진단 및 단계화를 향상시킵니다. 기존의 EEG 기반 방법의 한계를 극복하고, 전반적인 치매 진단 및 모니터링에 기여하려는 목표를 가지고 있습니다.

- **Technical Details**: VMoGE는 다중 모드 변환기(multi-granularity transformer)를 사용하여 네 가지 주파수 대역에서의 다중 스케일 시간 패턴을 추출합니다. 이후 가우시안 마르코프 랜덤 필드(Gaussian Markov Random Field) 기반의 변이 그래프 컨볼루션 인코더를 이용하여 신경 특화(neural specialization)를 생리학적으로 의미 있는 EEG 주파수 대역에 연결합니다. 이 과정에서는 구조화된 변이 추론과 적응형 게이팅(adaptive gating) 기법을 활용합니다.

- **Performance Highlights**: VMoGE는 두 개의 다양한 데이터 세트에서 하위 유형 분류 및 심각도 단계화 평가를 통해 최신 기술 대비 AUC를 4%에서 10%까지 향상시키며 우수한 성능을 입증했습니다. 또한, 전문가 가중치(expert weights)를 통해 임상 지표와 관련된 해석 가능한 통찰력을 제공하여 EEG 바이오마커 발견에 기여하고 있습니다. 이러한 결과는 포괄적인 치매 진단 및 모니터링에 매우 유용합니다.
...
### ADARL: Adaptive Low-Rank Structures for Robust Policy Learning under Uncertainty (https://arxiv.org/abs/2510.11899)
- **What's New**: 이 논문에서는 강화 학습(RL)에서 환경 동역학의 인지적 불확실성을 처리하기 위한 새로운 접근인 Adaptive Rank Representation (AdaRL)을 제안합니다. 기존의 중첩된 최소-최대(min--max) 최적화 방식은 계산적으로 비효율적이고 지나치게 보수적인 정책을 생성하는 경향이 있습니다. AdaRL은 정책 복잡도와 작업의 본질적인 차원을 일치시키는 두 단계 최적화 프레임워크로, 비정상적인 동역학에서도 강인성을 보장합니다.

- **Technical Details**: AdaRL은 하위 수준에서 고정 순위 제약 하에 정책 최적화를 수행하고, 상위 수준에서 순위를 조정하여 편향(bias)과 분산(variance) 간의 균형을 맞춥니다. 이 과정에서 Wasserstein 볼 주변에서 샘플링된 동역학을 활용합니다. 이러한 접근 방식은 강인성을 보장하면서도 과도한 매개변수화(over-parameterization)를 피하고, 모델 불확실성 하에서도 효율적인 정책 표현을 제공합니다.

- **Performance Highlights**: Empirical results show that AdaRL은 MuJoCo 연속 제어 벤치마크에서 고정 순위 기초선과 최첨단 robust RL 방법을 일관되게 초월하며, 기본 작업의 본질적인 순위에 수렴하는 경향을 보입니다. 이는 환경이 학습과 다를 때도 신뢰할 수 있는 정책을 생성하는 데 효과적임을 의미합니다. 해당 결과는 불확실성 하의 robust RL에 대한 효율적이고 정당한 대안을 제시합니다.



### Robust Adversarial Reinforcement Learning in Stochastic Games via Sequence Modeling (https://arxiv.org/abs/2510.11877)
Comments:
          Accepted by Reliable ML Workshop @ NeurIPS 2025

- **What's New**: 이번 논문에서는 Decision Transformer(DT)의 적대적 강건성을 향상시키기 위한 Conservative Adversarially Robust Decision Transformer(CART)를 소개합니다. CART는 최초의 적대적 확률 게임에서 DT의 강건성을 높이기 위해 설계된 프레임워크로, 각 단계의 상호작용을 스테이지 게임으로 표현합니다. 이를 통해 stochastic state transitions을 명시적으로 포함시키고, NashQ 값을 기반으로 Transformer 정책을 개선합니다.

- **Technical Details**: CART는 강화 학습에서 수치적인 안정성을 보장하기 위해 Expectile Regression과 temporal-difference(TD) 학습을 활용합니다. 모델은 각 단계에서 주인공과 적의 상호작용을 정의하였으며, 주어진 상태에 대한 스마트한 행동을 최대화하기 위한 동시에 적절한 보상을 고려합니다. 이 방법은 DT와 비교할 때, 더 적은 착취 가능성을 가지고 있는 정책을 생성함으로써 적대적 강건성을 높입니다.

- **Performance Highlights**: 실험 결과, CART는 다양한 적대적 확률 게임에서 더 정확한 minimax 가치 추정을 달성하며, 최악의 경우에서도 우수한 결과를 보입니다. CART는 강화 학습 알고리즘의 변별력을 증가시키고, 안정된 성능을 제공하여 기존 DT 프레임워크의 한계를 극복합니다. 따라서 CART는 적대적 환경에서 의사결정을 최적화하는 데 기여할 것으로 기대됩니다.



### Improving Knowledge Graph Embeddings through Contrastive Learning with Negative Statements (https://arxiv.org/abs/2510.11868)
Comments:
          Accepted at the Thirteenth International Conference on Knowledge Capture (K-CAP 2025)

- **What's New**: 본 연구는 명시적으로 선언된 부정적 진술(negative statements)을 지식 임베딩 학습 과정에 통합하는 새로운 접근 방식을 제안합니다. 두 개의 임베딩 모델을 병렬로 훈련하여 긍정적인 진술과 부정적인 진술을 각각 다루며, 긍정적인 샘플을 손상시키고 다른 모델에 의해 점수가 높은 후보를 선택하여 부정적인 샘플을 생성합니다. 이 방법은 다양한 목적의 KG 및 도메인 특정 KG에서 평가되어 링크 예측(link prediction)과 삼중 분류(triple classification) 작업에 초점을 맞추고 있습니다.

- **Technical Details**: 지식 그래프(KG)는 정보를 구조화 된 트리플(h, r, t) 형식으로 표현합니다. 기존의 KG 임베딩 방법들은 대부분 Closed World Assumption(CWA) 또는 Local Closed World Assumption(LCWA)에 의존하여, 누락된 트리플을 잘못된 것으로 간주합니다. 그러나 본 연구에서는 대조 학습(constrastive learning)을 기반으로 한 새로운 부정적 샘플 생성 전략을 통해 긍정적 진술과 부정적 진술을 따로 학습하고, 더 어려운 부정 샘플을 생성하여 학습의 효과성을 높입니다.

- **Performance Highlights**: 제안된 방법은 기존의 최첨단 임베딩 모델들과 비교할 때 예측 성능이 개선되었음을 보여주었습니다. 광범위한 실험을 통해 다양한 KG와 작업에서 방법의 효과성, 견고함 및 광범위한 적합성을 입증했습니다. 특히, 단백질-단백질 상호작용(PPI) 예측과 관련된 작업에서 긍정적 및 부정적 진술의 통합이 성능을 개선하는 데 기여함을 확인했습니다.



### Actor-Enriched Time Series Forecasting of Process Performanc (https://arxiv.org/abs/2510.11856)
Comments:
          Accepted at ICPM 2025

- **What's New**: 본 연구에서는 Predictive Process Monitoring (PPM) 분야에서 actor behavior를 multivariate time series로 모델링하여 throughput time (TT) 예측 성능을 개선할 수 있는지를 조사합니다. 기존 연구는 actor behavior를 시간적으로 변하는 신호로 다루지 않았으므로, 이 측면을 채울 필요가 있습니다. 새롭게 제안된 actor-enriched forecasting framework는 actor behavior의 변화를 TT와 정렬하여 모델링합니다.

- **Technical Details**: 연구는 실제 event log로부터 actor 중심의 feature를 추출하여 분석하며, 이는 actor의 수행 행동인 continuation, interruption, handover의 지속 시간과 빈도를 포함합니다. 이러한 feature를 이용해 다양한 예측 모델을 훈련시키고, autoregressive 모델에서 기계 학습(machine learning) 및 심층 학습(deep learning) 모델까지 포괄합니다. 각 모델의 성능은 RMSE, MAE, R2로 평가됩니다.

- **Performance Highlights**: 연구 결과, actor behavior로 보강된 모델들이 기존의 baseline 모델들보다 consistently RMSE, MAE, R2 측면에서 우수한 성능을 보이는 것을 확인했습니다. 특히 tree-based learners 및 recurrent nets에서 actor behavior feature가 TT 예측 성능을 더욱 향상시킨다는 것을 실증적으로 보여주었습니다. 이는 actor behavior를 모델링하는 것이 성능 지표 예측에 긍정적으로 기여한다는 중요한 통찰을 제공합니다.



### Evaluating Open-Source Vision-Language Models for Multimodal Sarcasm Detection (https://arxiv.org/abs/2510.11852)
Comments:
          Accepted to ICDMW 2025 Workshop on Multimodal AI (MMAI). Full workshop info: this https URL

- **What's New**: 최근 오픈소스 비전-언어 모델(VLM)에서의 발전으로 비꼼(sarcasm)과 같은 복잡하고 주관적인 다중 모달(multimodal) 현상을 이해할 수 있는 새로운 기회가 열리고 있습니다. 본 연구에서는 BLIP2, InstructBLIP, OpenFlamingo, LLaVA, PaliGemma, Gemma3, Qwen-VL 등 일곱 개의 최신 모델이 제시된 다중 모달 비꼼 감지 능력을 평가합니다. 특히, 모델들이 비꼼 사례에 대한 설명 생성 능력도 평가하며, 세 가지 벤치마크 데이터셋(Muse, MMSD2.0, SarcNet)을 활용했습니다.

- **Technical Details**: 이 연구에서 자동 비꼼 감지의 복잡성을 고려하여 비전-언어 모델들은 큰 이미지-텍스트 코퍼스에서 사전 학습(pre-trained)된 모델들입니다. VLMs의 제로샷(zero-shot), 원샷(one-shot), 몇 샷(few-shot) 능력을 통해 다중 모달 비꼼 감지의 성능을 평가하고, 이 과정에서 각 샷 설정에 대한 통일된 프롬프트 템플릿을 활용합니다. 또한, 모델들이 비꼼을 식별하는 것 외에도 이들의 결정에 대한 인간 수준의 설명을 생성할 수 있는 능력을 분석합니다.

- **Performance Highlights**: 모델들은 이진 비꼼 감지에서 중간 정도의 성공을 나타냈지만, 특정 작업에 대한 세부 튜닝 없이 높은 품질의 설명을 생성하는 데는 여전히 한계가 있었습니다. 이 연구는 다중 모달 비꼼 감지와 설명 생성을 위한 오픈소스 VLM의 성능을 체계적으로 평가했다는 점에서 의미가 있으며, 향후 연구가 이 분야에서 더 확장될 필요가 있음을 시사합니다.



### Balancing Synthetic Data and Replay for Enhancing Task-Specific Capabilities (https://arxiv.org/abs/2510.11842)
Comments:
...
- **What's New**: 이 연구는 언어 모델을 새로운 작업에 적응시키는 과정에서 발생하는 지속적인 학습의 문제를 다루고 있습니다. 특히, 기존 지식을 잃지 않으면서 새로운 능력을 학습하는 방법에 대한 효과적인 전략을 제안합니다. 실험을 통해, 작업 성능과 일반 지식 유지 간의 최적의 균형을 이룰 수 있는 구성 요소를 조사하였습니다.

- **Technical Details**: 연구에서는 bAbI 추론 작업을 목표로 설정하고, 합성 데이터 생성 및 다양한 총 토큰 예산과 리플레이 비율 구성에 대한 체계적인 평가를 실시했습니다. 합성 데이터 확장을 통한 훈련은 더 다양한 고유 샘플을 생성하고, 이에 따라 모델의 성능을 개선합니다. 본 연구에서는 특정 작업 적응을 위한 최적의 리플레이 비율을 식별하기 위한 실험을 수행하며, 컴퓨팅 비용을 최소화하고 작업 특정 데이터를 최대화하는 방안을 모색하였습니다.

- **Performance Highlights**: 실험 결과, 5-10%의 리플레이 비율이 일반 지식을 유지하는 데 충분하며, 그 이상의 비율은 추가적인 혜택 없이 비용만 증가시킵니다. bAbI-Synthetic 데이터 세트가 bAbI-Original 데이터 세트보다 뛰어난 성과를 보이며, 데이터의 다양성이 작업 숙련에 있어서 중요하다는 점을 강조했습니다. 최적의 구성은 작업 성능과 일반 지능 유지 간의 효율적인 자원 활용을 가능하게 하였습니다.



### WaveletDiff: Multilevel Wavelet Diffusion For Time Series Generation (https://arxiv.org/abs/2510.11839)
- **What's New**: WaveletDiff는 시간 및 주파수 영역에서의 기존 모델들이 다루기 힘든 다중 규모의 구조를 활용하기 위해 개발된 새로운 확산(diffusion) 모델입니다. 이 모델은 각각의 분해 수준에 특화된 트랜스포머(transformers)와 크로스 레벨 주의(attention) 메커니즘을 결합하여 시계열 데이터의 다중 해상도 구조를 직접적으로 다룹니다. 이를 통해 모델은 시간적 및 주파수적 정보의 선택적 교환을 가능하게 하여 실제 시계열의 복잡한 패턴을 더 잘 재현합니다.

- **Technical Details**: WaveletDiff는 파세발 정리에 기반한 에너지 보존 제약을 도입하여 확산 과정 전반에 걸쳐 스펙트럼 충실도를 유지합니다. 개별적으로 각 웨이브렛(decomposition level)에서 확산 과정을 실행하며, 다양한 수준에 대해 적합한 경량화된 손실 함수(loss function)와 변환기로 구성되어 있습니다. 또한, 크로스 스케일(attention) 메커니즘을 통해 서로 다른 시간 스케일 간 정보 흐름을 가능하게 하며, 전체 모델의 훈련 목표를 방해하지 않도록 레벨 별 균형 전략을 포함합니다.

- **Performance Highlights**: WaveletDiff는 여섯 개의 실제 데이터셋에 대한 테스트에서 기존의 시계열 생성 모델인 Fourier 및 시간 기반 접근 방식에 비해 약 세 배 성능 향상을 달성했습니다. 예를 들어, WaveletDiff는 모든 데이터셋에서 두 번째로 우수한 기준 모델보다 평균적으로 세 배 작은 차별화(discriminative) 점수를 보였습니다. 이러한 성능 향상은 짧은 및 긴 시계열 모두에서 발생하며, 다섯 가지 다양한 성능 지표에서 검증되었습니다.
...
- **What's New**: 이번 논문에서는 안전 분류기(safety classifiers)와 함께 작동하는 생성 모델(generative models)의 개선점을 제안합니다. 특히, 기존의 파인 튜닝(fine-tuning) 방법이 생성된 결과물이 안전 분류기의 결정 경계(decision boundary) 근처에 있음을 간과하며, 이로 인해 잘못된 긍정 및 부정 오류가 발생할 수 있음을 지적합니다. 새로운 방법인 Boundary Guidance를 통해 이러한 결정 경계에서의 생성을 피하는 강화 학습 기반의 파인 튜닝 방법론을 제안하고 있습니다.

- **Technical Details**: Boundary Guidance는 생성 모델의 출력을 명시적으로 안전 분류기의 경계에서 멀어지도록 조정하며, 이는 단순히 거부 확률을 줄이는 것과는 차별화됩니다. 이 방법은 안전성과 유용성을 동시에 최적화하기 위한 보상 모델 신호(reward model signals)와 안전 분류기 신뢰도(safety classifier confidence)를 활용합니다. 실험적으로 0.5에서 14억 개의 파라미터를 가진 여러 모델에 대해 이 방법의 유효성을 검증하였고, 작은 모델에서 특히 뛰어난 성능 향상을 보였습니다.

- **Performance Highlights**: Boundary Guidance는 생성 모델의 유용성(utility)과 안전성(safety) 모두에서 개선된 성능을 보였습니다. 전반적으로, 이 방법은 판단 결과에서 유용성이 최소화되는 결정 경계 근처에서 시스템의 최적화를 제공합니다. 다양한 모델 아키텍처와 스케일에 걸쳐 실험적으로 입증된 향상된 결과는 결합 시스템 최적화가 개별 구성 요소가 독립적으로 달성할 수 있는 결과보다 우수하다는 것을 보여줍니다.



### Z0-Inf: Zeroth Order Approximation for Data Influenc (https://arxiv.org/abs/2510.11832)
- **What's New**: 본 논문에서는 대규모 머신러닝 모델에서 훈련 데이터의 영향력을 추정하는 효율적인 제로(th-order) 근사 방법을 제안합니다. 기존의 방법들은 주로 정확도 부족이나 계산 비용이 높아 큰 모델에서는 비현실적이었습니다. 새로 제안된 방법은 중간 체크포인트에서의 손실 값만을 사용하여 계산하기 때문에, 비미분 가능 손실 함수에서도 폭넓게 적용될 수 있습니다.

- **Technical Details**: 이 방법은 손실 차이의 첫 번째 근사로 그래디언트를 사용하지 않고 유한 차이 방향 그래디언트를 통해 손실 값을 집계합니다. 이러한 접근은 대규모 모델에서도 상당히 빠르며, 계산 비용과 메모리 사용량을 크게 줄입니다. 이론적으로, 제안된 방법은 TracIn에 대한 제로(th-order) 근사 형태로 보여지며, 자기 영향을 추정하는 데 있어 작업 속도 또한 개선되었습니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 고급의 언어 모델에서 TracIn 및 최신 데이터 영향력 근사 기술보다도 더 높은 정확도를 보여주었습니다. 특히 자기 영향을 추정하거나 훈련-테스트 영향력을 분석할 때 우수한 성능을 발휘합니다. 계산 비용과 메모리 측면에서 기존의 방법들보다 월등히 효율적이기 때문에, 모델 디버깅 및 데이터 평가와 같은 실제 어플리케이션에 매우 유용할 것입니다.



### Schrödinger bridge for generative AI: Soft-constrained formulation and convergence analysis (https://arxiv.org/abs/2510.11829)
Comments:
          31 pages

- **What's New**: 이번 연구에서는 전통적인 Schrödinger bridge 문제의 경직된 종단 제약(terminal constraints)을 완화한 소프트 제약(SCSBP) Schrödinger bridge 문제를 탐구합니다. 이 접근법은 Penalty function을 사용하여 더 유연한 확률적 제어(formulation)를 가능하게 하며, McKean-Vlasov 유형의 구조를 도입합니다. 또한, 최적의 솔루션의 존재성을 보장하고, 페널티가 증가할수록 솔루션이 전통적인 SBP와 선형 속도로 수렴함을 증명합니다.

- **Technical Details**: SCSBP를 다룸에 있어, 우리는 두 개의 분포 μ_{ini}와 μ_{tar} 정도의 초기 및 목표 분포를 설정합니다. G라는 매끄러운 페널티 함수는 2-Wasserstein 공간에서 정의됩니다. 이 연구는 해석적 접근법을 사용해 최적 제어 정책을 조명하고, δ-측도(delta measure)와 같은 특별한 경우에서 최적의 제어 정책을 명시적으로 도출합니다.

- **Performance Highlights**: 소프트 제약의 적용은 기존 SBP의 제한을 뛰어넘어, 특히 높은 차원 문제나 데이터가 부족한 경우의 안정성을 높입니다. 이 연구는 SCSBP가 데이터 생성, 세부 조정(fine-tuning), 그리고 전이 학습(transfer learning) 등 다양한 생성 AI 분야에 응용될 수 있는 가능성을 제시합니다. 최적 제어 정책의 선형 수렴 속도는 이 연구의 주요 기여 중 하나로, 문헌상 새롭고 중요한 성과입니다.
...
### Think as a Doctor: An Interpretable AI Approach for ICU Mortality Prediction (https://arxiv.org/abs/2510.11745)
Comments:
          42 pages

- **What's New**: 이번 연구에서는 ICU 사망 예측을 위해 ProtoDoctor라는 새로운 프레임워크를 제안합니다. ProtoDoctor는 기존의 모델들이 간과했던 임상 경과 식별(clinical course identification)과 예후 의식(prognostication awareness) 요소들을 통합하여, 내재적 해석 가능성(intrinsic interpretability)을 유지합니다. 이는 환자의 인구 통계학적 이질성(demographic heterogeneity) 또한 반영하여 보다 신뢰성 높은 결과를 도출합니다.

- **Technical Details**: ProtoDoctor의 혁신적인 구성 요소는 두 가지 주요 모듈로 이루어져 있습니다. 첫째, Prognostic Clinical Course Identification 모듈은 프로토타입 학습(prototype learning)을 통해 임상 경과를 식별하고, 새로운 정규화 메커니즘을 활용하여 예후 의식을 달성합니다. 둘째, Demographic Heterogeneity Recognition 모듈은 집단 별 프로토타입(cohort-specific prototypes)과 위험 조정을 통해 인구통계학적 이질성을 모델링합니다.

- **Performance Highlights**: 실험적 평가 결과, ProtoDoctor는 기존의 최첨단 모델들보다 더 높은 예측 정확도를 보였습니다. 뿐만 아니라, 인간 평가에서도 그 해석이 임상적으로 더 의미 있고 신뢰할 수 있으며 ICU 실무에 적용 가능하다는 것을 확인했습니다. 이로써 ProtoDoctor는 ICU 환경에서의 임상 신뢰를 강화하고, 효과적인 자원 할당을 지원할 수 있는 가능성을 제시합니다.
...
### Wavefront Coding for Accommodation-Invariant Near-Eye Displays (https://arxiv.org/abs/2510.12778)
- **What's New**: 이번 논문에서는 스테레오스코픽 (stereoscopic) 디스플레이에서 수용과 조절 간의 갈등 (vergence-accommodation conflict) 문제를 해결하기 위한 새로운 근시안 (near-eye) 디스플레이 방법을 제시합니다. 이 시스템은 굴절 렌즈 (refractive lens) 아이피스와 독창적인 파면 코딩 (wavefront coding) 회절 광학 소자를 통합하며, 전처리 합성곱 신경망 (convolutional neural network)과 함께 작동합니다.

- **Technical Details**: 저자는 최적화된 파면 코딩 광학과 이미지 전처리 모듈을 공동으로 최적화하기 위해 엔드투엔드 (end-to-end) 학습을 활용합니다. 이를 구현하기 위해, 제한된 조리개 (limiting aperture)와 눈의 광학에 의해 발생하는 색수차 (chromatic aberrations)를 고려한 미분 가능한 망막 이미지 형성 모델을 개발하였습니다. 또한 지각 효과를 반영하기 위해 신경 전달 함수 (neural transfer function)와 대비 민감도 함수 (contrast sensitivity function)를 손실 모델에 통합하였습니다.

- **Performance Highlights**: 모델은 시뮬레이션 기반의 엄격한 분석을 통해 검증되었으며, 설계된 회절 광학 소자를 제작하고 벤치톱 설정을 구축하였습니다. 이 시스템은 최대 4 디옵터의 깊이 범위에 대해 수용 불변성 (accommodation-invariance)을 입증하였습니다.
...
### Dendrograms of Mixing Measures for Softmax-Gated Gaussian Mixture of Experts: Consistency without Model Sweeps (https://arxiv.org/abs/2510.12744)
Comments:
          Do Tien Hai, Trung Nguyen Mai, and TrungTin Nguyen are co-first authors

- **What's New**: 본 논문에서는 Softmax-gated Gaussian mixture of experts (SGMoE)의 통합 통계적 프레임워크를 개발하고 있습니다. 이 접근법은 매개변수 추정과 모델 선택에서 오랜 문제인 비식별성, 내부 게이트-전문가 상호작용, 소프트맥스에 의한 조건부 밀도 간의 밀접한 결합 등을 해결하고자 합니다. 구체적으로, Voronoi 스타일의 손실 함수를 도입하여 정밀한 수렴 속도를 보장하는 최대 우도 추정기(MLE)를 수립합니다.

- **Technical Details**: SGMoE는 게이트 매개변수의 비식별성 문제와 소프트맥스 게이트에서 발생하는 고유한 게이트-전문가 결합으로 인해 매개변수 추정이 어려운 모델입니다. 이를 위해, 본 연구는 다중 저장된 Voronoi 셀 내에서 병합된 모멘트 결합을 통해 느린 방향을 드러내고, 이로 인해 계층적 병합 연산자를 유도하며, 혼합 조치에 대한 집합 경로(덴드로그램)를 제공합니다. 이 과정에서 통계적 효율성을 유지하면서도 다중 크기 훈련을 피하는 일관된 모델 선택 기준을 수립했습니다.

- **Performance Highlights**: 실험 결과, 제안된 덴드로그램 선택 기준(DSC)은 AIC/BIC/ICL보다 과적합에 덜 민감하며, 조합 후에도 빠른 매개변수 속도를 회복하는 것으로 나타났습니다. 감염상태에서의 모델 선택에 강건성을 보이면서도 두 명의 전문가를 선택하고, 명확한 혼합 측정 계층 구조를 노출하였으며, 전통적 기준보다 성능이 우수함을 보여주었습니다. 마지막으로, 이 방법은 통계적 효율성을 유지하면서도 다중 크기 훈련을 피하는 첫 번째 방법으로 평가됩니다.
...
### Data-Model Co-Evolution: Growing Test Sets to Refine LLM Behavior (https://arxiv.org/abs/2510.12728)
- **What's New**: 본 논문은 데이터와 모델 간의 경계를 허물고, 상호 발전(data-model co-evolution)하는 새로운 패러다임을 제시합니다. Large Language Models(LLMs)의 발전으로, 사용자들은 자연어 명령어(prompt instructions)를 통해 모델 동작을 즉각적으로 수정하는 것이 가능해졌습니다. 그러나 기존의 데이터 테스트 시스템은 이러한 신속한 모델 개선을 지원하지 않아, 보다 유연한 개발 프로세스가 필요하다는 점을 강조합니다. 이러한 문제를 해결하기 위해, 연구진은 사용자가 정확한 정책을 정의하고 모델의 행동을 지속적으로 개선할 수 있는 인터랙티브 시스템을 제안합니다.

- **Technical Details**: 이 연구에서는 모델과 테스트 예제(test examples)가 함께 성장하는 상호 발전의 과정을 도모하는 인터랙티브 시스템을 구현하였습니다. 사용자는 초기 정책과 프롬프트로 시작하여, 시스템은 모호한 입력을 제안하고 모델의 출력을 통해 동작을 검증할 수 있도록 돕습니다. 잘못된 출력을 생성할 경우, 사용자는 예제를 표시하고 원인을 설명하며, 이 정보는 지속적인 테스트 세트의 발전에 반영됩니다. 이러한 접근은 모델의 맹점을 발견하고, 각 수정( revision)을 실제 예제와 함께 테스트하여 피드백 루프를 강화하는 것을 목표로 합니다.

- **Performance Highlights**: 제안된 시스템을 활용한 사용자 연구에서는 참가자들이 보다 구체적이고 명확한 프롬프트를 작성하며, 보다 큰 테스트 세트를 체계적으로 구성하는 데 성공했습니다. 참여자들은 모델 행동의 일관된 변화를 경험하고, 기존 방법보다 높은 만족도를 보고했습니다. 우리는 이러한 결과가 LLM 응용 프로그램의 발전에 기여하며, 보다 책임감 있는 AI 사용을 가능하게 한다고 결론짓습니다.
...
### Contraction and entropy production in continuous-time Sinkhorn dynamics (https://arxiv.org/abs/2510.12639)
Comments:
          10 pages excluding references

- **What's New**: 최근 Sinkhorn 알고리즘의 소실하는 스텝 사이즈 한계가 확률 측정 공간에서의 미러 디센트(mirror descent)로 나타났습니다. 이는 두 가지 시간 의존적 메트릭에서 L^2 수축 기준을 제시하며, 이는 특정 조건부 기대 연산자의 강제성을 줄이는 결과를 보여줍니다. 또한, Sinkhorn 흐름의 엔트로피 생성율(entropy production rate)에 대한 정확한 정체성을 제공하여, 비양수성만 알려져 있었던 것을 초과한 결과를 도출합니다.

- **Technical Details**: 엔트로피 정규화된 최적 수송(OT_ε)과 슈뢰딩거 다리 문제(Schrödinger bridge problem)는 생성 모델과 최적 수송 맵의 빠른 추정에 널리 사용됩니다. 이 알고리즘은 상대 엔트로피에서 하위 선형 속도로 수렴하며, Sinkhorn 흐름에 대해 지수적 수렴 조건을 제공하는 연구 결과를 제시합니다. 여기에는 시간 의존적 리만 메트릭에서의 섭동의 지수적 감쇠와 확률 측정 공간에서의 그레이디언트 흐름을 위한 수축 분석이 포함됩니다.

- **Performance Highlights**: Sinkhorn 흐름은 타깃 주변에서 가역적 마르코프 동역학을 유도하며, 이는 온사거 경량 흐름(Onsager gradient flow)로 나타납니다. 이에 따라 이 논문은 Dirichlet 형태를 정의하고 Poincaré 불평등(Poincaré inequality)을 증명하며, $\varepsilon > 0$일 때 스펙트럼 갭(spectral gap)이 항상 양수임을 보여줍니다. 마지막으로, 로그 소벨 불평등(logarithmic Sobolev inequality, LSI)이 성립할 때만 엔트로피 감소가 지수적인 것을 확인하였으며, 이는 생성 모델 훈련의 잠재 공간 설계 원칙 및 이산 시간 알고리즘의 중지 휴리스틱으로서 두 가지 즉각적인 적용 사례를 제공합니다.



### Adapting Noise to Data: Generative Flows from 1D Processes (https://arxiv.org/abs/2510.12636)
- **What's New**: 이 논문에서는 1차원 소음 과정(1D noising processes)을 활용하여 생성 모델(generative models)을 구축하기 위한 일반적인 프레임워크를 소개합니다. 확산 프로세스(diffusion processes) 외에도 이 접근법의 유연성을 보여주는 여러 예제를 설명하고, 데이터에 적응하는 분포를 학습할 수 있도록 소음 분포(noise distribution)를 분위수 함수(quantile functions)로 매개변수화하는 새로운 프레임워크를 제안합니다.

- **Technical Details**: 우리의 접근법은 Flow Matching 및 일관성 모델(consistency models)과 같은 표준 목표와 원활하게 통합되며, 학습된 분위수 기반 소음이 무거운 꼬리(heavy tails)와 압축된 지원(compact supports)을 자연스럽게 캡처할 수 있도록 합니다. 1D 소음 프로세스를 구성하기 위한 일반적인 방법론을 제시하며, 그 과정에서 Kac 프로세스 및 Wasserstein 기울기 흐름(Wasserstein gradient flow) 등 다양한 사례를 들어 유연성을 강조합니다.

- **Performance Highlights**: 수치 실험(numerical experiments)을 통해 제안된 방법의 유연성과 효율성이 입증되었습니다. 우리의 방법을 통해 단순 온도에 기반하여 소음을 데이터에 맞게 조정할 수 있으며, 전송 경로(transport paths)를 대폭 단축할 수 있음을 보여줍니다. 또한, 데이터 적응형 잠재 소음(latent noise)의 높은 유연성으로 인해 기존의 생성 모델에 비해 더 나은 성능을 달성할 수 있음을 밝혔습니다.



### Same model, better performance: the impact of shuffling on DNA Language Models benchmarking (https://arxiv.org/abs/2510.12617)
- **What's New**: 최근 대형 언어 모델(large Language Models)이 유전학(genomics) 분야에서 DNA 언어 모델(DNA LMs)의 평가를 위한 표준화된 벤치마크(benchmark)의 필요성을 보여주고 있습니다. BEND(Benchmarking DNA Language Models) 프레임워크는 이러한 평가에 있어 하드웨어 의존적인 하이퍼파라미터가 벤치마크 결과의 왜곡을 초래할 수 있음을 입증하였습니다. 개선된 데이터 셔플링(pre-shuffling) 방식을 통해 벤치마크의 유효성을 높일 수 있음을 제안합니다.

- **Technical Details**: BEND는 7개의 감독(supervised) 및 비감독(unsupervised) 작업에서 DNA 기능 요소에 대한 언어 모델의 이해를 평가합니다. 각 작업에 대한 주석 데이터는 .bed 파일로 제공되며, DNA 서열이 평가를 위해 임베딩됩니다. 특히, 비감독 작업에서는 단일 뉴클레오타이드 변이를 효과 또는 비효과로 분류하고, 감독 작업은 CpG 메틸화, 히스톤 주석, 크로마틴 접근성 등을 포함합니다.

- **Performance Highlights**: 연구 결과, 현재 BEND의 구현 방식은 데이터의 할당 및 셔플링 과정을 적절히 수행하지 못하여 극단적인 왜곡을 초래합니다. 특히, 하드웨어 자원에 따라 성능이 좌우되며, 이는 자원이 풍부한 연구자들에게 유리하게 작용하는 문제가 있습니다. 프리 셔플링(pre-shuffling)의 채택은 이러한 문제를 해결할 수 있으며, 이는 전문 분야 벤치마크 디자인에 대한 중요성을 강조하는 사례로 작용합니다.
...
### Universal Adaptive Environment Discovery (https://arxiv.org/abs/2510.12547)
Comments:
          8 papes in the main body, 4 pages in the appendix, 4 figures and 9 tables overall, conference

- **What's New**: 이번 논문에서는 Machine Learning에서 데이터를 통한 spurious correlations(거짓 상관관계)를 피하는 방법에 대한 문제를 다룹니다. Universal Adaptive Environment Discovery (UAED)라는 새로운 프레임워크를 제안하여 데이터를 변환할 수 있는 분포를 학습하고 이를 통해 robust objective를 최적화합니다. UAED는 사전 정의된 그룹이나 수동 환경 설계 없이도 IRM, REx, GroupDRO, CORAL의 적응형 변형을 제공합니다.

- **Technical Details**: 논문에서는 X∈𝒳, Y∈𝒴, e∈ℰ로 주어지는 입력, 레이블, 환경을 고려합니다. 각 환경은 joint distribution Pe를 유도하며, 이를 통해 환경 간 일반화 가능한 예측기를 학습하는 목표를 가지고 있습니다. 기존의 방법들이 사전 정의된 환경을 요구하는 반면, UAED는 환경 지정 방식을 학습 가능하게 만들어 보다 유연한 접근을 제공합니다.

- **Performance Highlights**: UAED는 이론적으로 PAC-Bayes 경계를 통해 배포에 강건한 보장을 제공하며, 다양한 벤치마크에서 최악의 경우 정확도를 향상시키는 데 성공했습니다. 또한 평균 정확도에서도 경쟁력을 유지하면서 해석 가능한 환경 분포를 발견하였습니다. 결과적으로 UAED는 out-of-distribution generalization에 대한 실용적인 접근법을 제공하는 것으로 나타났습니다.



### Why the noise model matters: A performance gap in learned regularization (https://arxiv.org/abs/2510.12521)
- **What's New**: 이 논문은 선형 역문제(linear inverse problem)를 해결하기 위한 효율적인 정규화기(regularizer) 학습의 도전에 대해 다룹니다. 특히, 노이즈 통계(noise statistics)가 알려지지 않은 상황에서 Tikhonov, Lavrentiev 및 이차 정규화(quadratic regularization) 방법의 성능을 분석합니다. 다양한 정규화 방법들이 서로 다른 결과를 산출할 수 있음을 보여줍니다.

- **Technical Details**: 이 연구는 대칭 목적 함수(objective function)와 정규화 항(regularizer)을 포함하는 변량 정규화(variational regularization)의 최초 이론 분석을 수행합니다. 이를 위해, 주어진 데이터 쌍에 대한 기대값을 통해 두 레벨 bilevel 최적화 문제를 고려하며, 이는 비형식화된 방법의 한계를 이해하기 위한 것입니다. 연구의 초점은 노이즈 분포가 알려지지 않은 경우의 Tikhonov 정규화를 포함한 다양한 정규화 방법의 비교입니다.

- **Performance Highlights**: 이론적 분석과 수치 실험 결과에 따르면, 비백색 노이즈(non-white noise) 환경에서 다양한 정규화 방법들 간의 성능 차이가 나타나며, 이는 최적의 아핀 재구성(optimal affine reconstruction)과의 성능 격차로 드러납니다. 이 연구는 데이터 기반 정규화에서 노이즈 통계의 정확한 모델링 또는 공동 학습(co-learning)의 중요성을 강조합니다.



### Diff-XYZ: A Benchmark for Evaluating Diff Understanding (https://arxiv.org/abs/2510.12487)
- **What's New**: Diff-XYZ는 대규모 저장소에서 코드 diffs를 이해하고 처리하는 데 필요한 컴팩트한 벤치마크로, 세 가지 감독 작업(apply, anti-apply, diff generation)을 포함합니다. 이 벤치마크는 실제 커밋에서 파생된 사례를 사용하여 코드 변경 사항을 평가할 수 있는 명확한 평가 프로토콜을 제공합니다. Diff-XYZ의 목적은 다양한 diff 형식에서 모델이 어떻게 작동하는지를 집중적으로 연구하고 평가할 수 있는 기반을 제공하는 것입니다.

- **Technical Details**: Diff-XYZ는 세 가지 작업으로 구성되어 있으며, 각 작업은 코드의 특정 변형을 요구합니다. Apply Task는 새로운 코드 생성, Anti-Apply Task는 이전 코드 복원, Diff Generation Task는 diff 생성입니다. 이 작업들은 코드 diffs와 관련된 문제를 다루며, 모델의 능력을 다양한 형식으로 сравcompare할 수 있도록 설계되었습니다. 코드 diffs 처리에서의 포맷의 영향을 효과적으로 측정하고 분석하기 위해 레퍼런스 결과를 확립하는 데 중점을 두었습니다.

- **Performance Highlights**: Diff-XYZ 벤치마크는 상용 및 오픈 소스 LLM의 성능을 비교한 결과, 상용 모델이 오픈 소스 모델보다 일관되게 높은 점수를 기록했습니다. 특히, GPT-4.1이 다양한 작업에서 가장 높은 성적을 보였으며, 모델 크기가 커질수록 성능이 향상되는 경향을 보였습니다. 이 연구는 diff 형식과 프롬프트가 LLM의 성능에 미치는 영향을 명확히 하고, 향후 모델 개발과 포맷 개선에 기여할 수 있는 방향을 제시합니다.



### SMEC: Rethinking Matryoshka Representation Learning for Retrieval Embedding Compression (https://arxiv.org/abs/2510.12474)
Comments:
          Accepted by EMNLP2025

- **What's New**: 본 논문에서는 Sequential Matryoshka Embedding Compression (SMEC)이라는 혁신적인 교육 프레임워크를 제안합니다. SMEC은 훈련 중 기울기 변동을 완화하기 위한 Sequential Matryoshka Representation Learning(SMRL) 방법과 정보 손실을 줄이기 위한 Adaptive Dimension Selection (ADS) 모듈을 도입합니다. 또한, Selectable Cross-batch Memory (S-XBM) 모듈을 통해 고차원 및 저차원 임베딩 간의 비지도 학습을 향상시키는 것을 목표로 합니다.

- **Technical Details**: SMEC의 주된 특징은 고차원 구성을 간소화하면서도 성능을 유지할 수 있도록 설계되었다는 점입니다. 이를 위해, SMEC는 Multi-scale representation을 통해 고차원의 정보 손실을 최소화하며, 실험을 통해 이미지, 텍스트 및 다중 모드 데이터셋에서 괄목할 만한 차원 축소를 달성했습니다. 예를 들어, BEIR 데이터셋에서 SMEC를 이용한 LLM2Vec 임베딩의 성능 개선을 관찰했습니다.

- **Performance Highlights**: SMEC는 낮은 차원의 임베딩을 제시하면서도 기존의 Matryoshka-Adaptor 및 Search-Adaptor 모델에 비해 성능을 각각 1.1 및 2.7 포인트 개선하는 결과를 보였습니다. 이는 비즈니스 및 정보 검색 시스템에서 LLM을 보다 실용적으로 사용할 수 있는 가능성을 높입니다. 따라서 SMEC는 실제 환경에서 LLM을 더욱 효과적으로 적용할 수 있는 기초가 될 것입니다.



### Formal Models and Convergence Analysis for Context-Aware Security Verification (https://arxiv.org/abs/2510.12440)
Comments:
          11 pages, 4 figures, 4 tables. Presents formal framework for context-aware security verification with ML-enhanced adaptive systems. Includes theoretical bounds (sample complexity, information-theoretic limits, convergence guarantees, soundness preservation) and empirical validation on 97,224 exploit samples

- **What's New**: 새롭게 도입된 개념인 context-completeness(컨텍스트 완전성)는 ML(머신러닝) 기반의 적응형 시스템을 위한 보안 검증의 이론적 기초가 됩니다. 본 연구에서는 적응형 검증이 정적 방법보다 높은 completeness(완전성)를 달성하는 것을 증명하였으며, 이는 context(맥락) 정보를 통해 검증 능력이 비례함을 정립했습니다. 또한, 대규모 언어 모델(LLM)을 활용한 맥락 인식을 통해 검증 파이프라인에 대한 수렴성 및 타당성 보장을 제시했습니다.

- **Technical Details**: 이 논문에서는 보안 검증 시스템을 수학적으로 정립하였으며, 예를 들어 SQL injection(SQL 주입) 공격을 사용하여 맥락 인식 검증 모형을 설명합니다. 보안 검증 시스템은 시스템 상태, 검증 페이로드, 검증 함수 등을 포함하는 일련의 수학적 정의를 포함하고 있습니다. 또한, 컨텍스트를 활용한 동적 검증 시스템과 정적 컨텍스트 블라인드 검증 시스템 간의 명확한 구분을 제공하고, 서로 다른 맥락의 검증 능력을 수량화할 수 있는 정보를 제시합니다.

- **Performance Highlights**: 지금까지의 실험 결과, 우리의 알고리즘은 97,224개의 공격 샘플에 대한 검증을 통해 감지 정확도가 58%에서 69.93%로 향상되었음을 보여줍니다. 정보가 풍부할 경우 성공 확률이 51%에서 82%로 증가하며, 훈련 손실은 O(1/sqrt(T))의 수렴 속도를 유지합니다. 이 연구는 이론적으로 근거 있는 적응형 검증 시스템이 명시된 가정 하에 정적 접근보다 확실한 개선을 달성할 수 있음을 입증합니다.



### Neural Guided Sampling for Quantum Circuit Optimization (https://arxiv.org/abs/2510.12430)
Comments:
          12 pages, 9 Figures

- **What's New**: 이 논문에서는 2D neural guided sampling을 제안하여 효율적인 양자 회로(transpilation) 최적화를 달성하는 방법을 다룹니다. 기존의 sampling 방법은 샘플링 효율성의 문제로 인해 긴 시간과 에너지를 소모하는 단점을 가지고 있었으나, 제안된 방법은 2D 양자 회로의 표현을 기반으로 하여 가변성을 줄이고 컴퓨팅 시간을 단축합니다. 여러 실험을 통해 이 방법이 기존 qiskit와 BQSKit 최적화 방법보다 우수함을 입증하였습니다.

- **Technical Details**: 양자 회로의 목표는 제공된 하드웨어 아키텍처에서 구현될 수 있는 게이트 시퀀스로 변환하는 과정입니다. 여기서 양자 회로의 길이를 줄이기 위해 term replacement 방식이 사용되며, 2D 그래픽 모델을 통해 저항 키(shortest gate count)를 가지는 블록을 식별하고 샘플링을 수행합니다. 신경 네트워크는 관심 있는 블록을 식별하기 위한 attention map을 예측하며, 이를 통해 신경 유도 샘플링(neural guided sampling)을 진행합니다.

- **Performance Highlights**: 제안된 방법은 여러 차례의 실험을 통해, 기존 반복 최적화 기법에 비해 더 효율적인 양자 회로를 생성하는 동시에 생성 속도도 향상됨을 보여주었습니다. 사용된 두 가지 게이트 세트에서 모두 실험이 이루어졌으며, 각각의 세트에서 R​X, R​Y, R​Z 및 C​Z 게이트가 포함되어 있습니다. 최종적으로 본 연구는 양자 회로의 성능을 최적화하는 데 중요한 기여를 하였습니다.



### Geopolitics, Geoeconomics and Risk:A Machine Learning Approach (https://arxiv.org/abs/2510.12416)
- **What's New**: 이 논문에서는 42개국에 대한 새로운 고빈도 일별 패널 데이터셋을 소개합니다. 이 데이터셋은 지정학적 위험(Geopolitical Risk), 경제 정책 불확실성(Economic Policy Uncertainty), 무역 정책 불확실성(Trade Policy Uncertainty), 그리고 정치적 정서(Political Sentiment)와 같은 요소를 포함하고 있습니다. 감정 역학이 국가 채무 불이행 위험(sovereign risk)에 미치는 영향을 분석하며, 전통적인 요인과 비교하여 새로운 지표의 예측 가치를 평가합니다.

- **Technical Details**: 우리는 신뢰할 수 있는 국가 신용디폴트스왑(CDS) 스프레드를 측정하는 방법을 통해 이러한 감정 역학의 영향을 실증적으로 검토합니다. 감정 지표는 기존의 금융 변수와 결합하여 사용되며, 기계 학습(Machine Learning) 모델을 통해 이들의 역할을 식별합니다. 특히, 랜덤 포레스트(Random Forests)와 같은 비선형(non-linear) 기법이 최고의 예측 성능을 나타냄을 보여주었습니다.

- **Performance Highlights**: 그 결과, 지정학적 위험과 경제 정책 불확실성이 국가 신용 위험에 중요한 역할을 한다는 사실을 확인할 수 있었습니다. 또한, 이들 요소는 글로벌 금융 조건과의 비선형 상호작용을 통해 그 영향력이 증폭됩니다. 지역 간 이질성도 두드러지게 나타나고, 자산 클래스 및 신흥 시장에 따라 정책 금리의 변화와 글로벌 금융 변동성, 지정학적 위험에 대한 민감도가 상이함을 보여줍니다.



### Robot Learning: A Tutoria (https://arxiv.org/abs/2510.12403)
Comments:
          Tutorial on Robot Learning using LeRobot, the end-to-end robot learning library developed by Hugging Face

- **What's New**: 이 논문에서는 로봇 학습(Robot Learning)의 발전을 탐구하고, 고전적인 모델 기반 방법에서 데이터 기반 학습 패러다임으로의 전환 과정을 다룬다. 이 과정은 자율 시스템의 획기적인 능력을 열어주고 있으며, 강화 학습(Reinforcement Learning)과 행동 복제(Behavioral Cloning) 등 다양한 기법들을 통합하여 앞으로 나아가고 있다. 또한, 이 튜토리얼은 연구자와 실무자들이 로봇 학습 발전에 기여할 수 있도록 개념적인 이해와 실용적인 도구를 제공하는 것을 목표로 한다.

- **Technical Details**: 자율 로봇은 반복적이고 위험한 수동 작업을 인간으로부터 해소할 잠재력을 가지고 있다. 최근 기계 학습(Machine Learning) 기술의 발전은 데이터와 컴퓨테이션을 활용하여 자율 시스템을 개발하기 위한 새로운 방법들을 촉진하고 있다. 여기서 lerobot 라이브러리는 다양한 로봇 플랫폼을 지원하고, 로봇 상태와 행동을 기록할 수 있는 통합된 데이터 형식인 LeRobotDataset을 제공하여 로봇 데이터의 활용성을 높인다.

- **Performance Highlights**: 이 튜토리얼은 로봇 학습의 현대적 접근 방식을 소개하며, 다양한 배경 지식이 필요한 로봇 공학의 멀티디iscipline적 특성을 강조한다. 링크된 lerobot 라이브러리는 실제 로봇 사례를 중심으로 구현된 효율적인 코드 예시와 함께 로봇 학습 기법의 실용적인 적용을 설명한다. 고급 알고리즘을 지원하는 lerobot 덕분에 연구자들은 실험과 결과 추적을 용이하게 할 수 있다.



### Improving Generative Behavior Cloning via Self-Guidance and Adaptive Chunking (https://arxiv.org/abs/2510.12392)
Comments:
          Accepted at NeurIPS25

- **What's New**: 이 논문은 로봇 학습을 위한 Generative Behavior Cloning (GBC)이라는 새로운 접근 방식을 소개합니다. GBC는 다중 작업 환경에서 로봇이 행동을 배우는 데 있어 효과적인 프레임워크로, 특히 확산 과정을 사용하는 방식이 주목받고 있습니다. 그러나 기존의 open-loop (OL) 제어 방식에서 발생한 오류와 지연 문제를 해결하기 위해 두 가지 새로운 기술인 self-guidance와 adaptive chunking을 제안합니다.

- **Technical Details**: Self-guidance는 과거 관찰을 활용하여 행동의 신뢰성을 높이고, future-aware 행동을 촉진하는 방법입니다. Adaptive chunking은 반응성이 향상된 행동 시퀀스를 선택적으로 업데이트하여, OL 제어의 안정성과 CL 제어의 반응성을 조화롭게 결합하는 기술입니다. 이러한 두 가지 기술은 표준 확산 정책의 성능을 크게 향상시킵니다.

- **Performance Highlights**: 이 연구에서 제안한 방법은 여러 실험을 통해 기존의 Vanilla Diffusion Policy에 비해 23.25% 향상된 성능을 보였고, 최신 BID보다 12.27% 더 나은 결과를 나타냈습니다. 또한, 계산 비용을 16배 줄이는 데 성공하여 실제 로봇 조작 작업에서 효율성을 높였습니다.
...
### Improved Central Limit Theorem and Bootstrap Approximations for Linear Stochastic Approximation (https://arxiv.org/abs/2510.12375)
- **What's New**: 본 논문에서는 선형 확률적 근사(Linear Stochastic Approximation, LSA) 알고리즘의 Polyak-Ruppert 평균 반복값에 대한 다변량 정규 근사의 Berry-Esseen 경계를 세부적으로 다룹니다. Gaussian 분포의 공분산 행렬을 기반으로 한 정상 근사를 고려하고, 표본 수 $n$에 대한 오차의 비율을 $n^{-1/3}$까지 증명합니다. 또한, 평균 LSA 추정기의 재조정된 오차 분포를 근사하는 멀티플라이어 부트스트랩 절차의 비비대칭(validity)도 증명합니다.

- **Technical Details**: LSA 알고리즘은 관측값의 수열 {(𝐀(Zk), 𝐛(Zk))}를 기반으로 고유한 해 θ⋆를 근사하는 기법입니다. 이 논문에서는 평균 반복값 θ¯n을 사용하여 추정의 안정성 및 수렴 속도를 가속화하고, 새로운 결과로서 $Σ∞$의 경계들을 제시합니다. Berry-Esseen 경계를 통해 평균 분포의 근사 비율을 $n^{-1/3}$으로 개선하였으며, 이는 기존의 연구 결과에 비해 향상된 결과입니다.

- **Performance Highlights**: 본 논문에서 제안한 방법은 이전 연구들에 비해 구간 확률(coverage probabilities)의 비율을 $1/	ext{sqrt}{n}$로 달성하는 결과를 보였습니다. 또한, Polyak-Ruppert 추정기의 분포를 멀티플라이어 부트스트랩 절차를 통해 근사하여, 그 신뢰 구간의 확률을 효과적으로 추정할 수 있음을 보여주었습니다. 이 연구 결과는 머신러닝과 통계 분야에서 LSA 알고리즘의 성능을 크게 향상시킬 것으로 기대됩니다.



### Constrained Sensing and Reliable State Estimation with Shallow Recurrent Decoders on a TRIGA Mark II Reactor (https://arxiv.org/abs/2510.12368)
- **What's New**: 이번 연구에서 Shallow Recurrent Decoder (SHRED) 네트워크는 핵발전소와 같은 복잡한 엔지니어링 시스템의 상태 추정에 유망한 데이터 기반 접근법으로 제안되었습니다. 이 아키텍처는 제한된 측정 데이터로부터 전체 상태 공간을 매핑할 수 있는 능력을 가지고 있으며, 노이즈가 있는 데이터와 센서 위치에 대한 제약을 극복하는 데 초점을 맞추고 있습니다. 이를 통해 짧은 훈련 시간과 하이퍼파라미터 조정 없이도 정확한 상태 추정이 가능합니다.

- **Technical Details**: SHRED 네트워크는 재귀적 유닛과 얕은 디코더를 결합하여 제한된 관측 데이터를 활용하여 시스템의 상태 공간을 매핑합니다. 이 아키텍처는 Takens 삽입 정리에 기반하여 시간에 따른 센서 측정치의 경로를 활용하고, 열역학적 시스템을 설명하는 유체역학 모델과 결합하여 사용됩니다. 연구의 주요 목표는 TRIGA Mark II 원자로의 온도 측정을 통해 시스템의 전체 상태(온도, 속도, 압력 등)를 복원하고, 모델과 데이터 간의 불일치를 수정하는 능력을 평가하는 것입니다.

- **Performance Highlights**: SHRED 아키텍처는 다양한 핵발전소 시스템에서 효과적인 성능을 보였으며, 실시간으로 모든 특성 필드를 정확하게 복원할 수 있는 가능성을 보여주었습니다. 본 연구에서 SHRED는 TRIGA 원자로의 열 과도 상태 중에 작동되었으며, 특정 저동역학 채널에 제한된 센서를 통해 성능을 검증하였습니다. 연구 결과, 비 최적 센서 위치에서도 유용한 정보를 제공할 수 있는 강인한 하드웨어 성능이 입증되었습니다.



### Pretraining in Actor-Critic Reinforcement Learning for Robot Motion Contro (https://arxiv.org/abs/2510.12363)
Comments:
          Submitted to ICLR 2026

- **What's New**: 이번 논문은 로봇 운동 제어 분야에서 강화를 활용한 학습 방식의 새로운 Paradigm을 제안한다. 기존에 개별 기술이 개별적으로 학습되는 한계점을 해결하기 위해, 다수의 과제에서 공통적으로 사용될 수 있는 일반화 가능한 지식을 포착하는 신경망 모델을 사전 학습하는 방법을 정의하였다. 이를 통해 Proximal Policy Optimization(PPO)와 같은 로깅(learning) 알고리즘을 보다 효율적으로 사용할 수 있게 되었다.

- **Technical Details**: 이 연구에서는 데이터 수집 방법으로 탐색 기반 알고리즘을 활용하여 다양한 전이 데이터를 수집하고, 이를 통해 Proprioceptive Inverse Dynamics Model(PIDM)을 감독 학습을 통해 트레이닝하였다. 초기 단계의 학습과정을 더욱 부드럽게 하기 위해 PIDM의 가중치를 actor와 critic 네트워크에 불러와 실제 과제의 policy optimization을 시작할 수 있도록 했다. 이 방식을 통해 초기 단계에서 로봇이 자신의 특성과 지식을 습득할 수 있도록 했다.

- **Performance Highlights**: 제안한 방법은 7가지 로봇 운동 제어 과제를 통해 검증되었으며, 초기화 전략이 실질적인 이점이 있음을 입증하였다. 이 방법은 평균적으로 샘플 효율성을 40.1% 향상시키고, 과제 수행 능력은 7.5% 개선되었다. 또한, 다양한 로봇 기술 세트에 대한 실험적인 분석을 통해 이 방법의 효과성을 추가적으로 입증하였다.



### Learning Latent Energy-Based Models via Interacting Particle Langevin Dynamics (https://arxiv.org/abs/2510.12311)
- **What's New**: 이번 연구에서는 에너지 기반 선형 모델(Energy-Based Models, EBMs)의 파라미터 추정을 위한 연속적 시간의 상호 작용 입자 시스템 기반 새로운 알고리즘을 제안합니다. 제안된 방법은 Langevin dynamics와 최적화(optimisation)의 관계를 활용하여, 잠재 변수 모델(latent variable model)에서 최대 주변 우도 추정(Maximum Marginal Likelihood Estimation, MMLE) 문제를 해결하고자 합니다. 또한, 이 구조를 통해 이전의 MCMC 방법들보다 더 효율적이고 빠른 학습이 가능하다고 주장합니다.

- **Technical Details**: 연구에서는 에너지 기반 상호 작용 입자 랜주빈 알고리즘(Energy-Based Interacting Particle Langevin Algorithm, EBIPLA)을 소개하며, 이는 MMLE 문제를 해결하기 위한 SDE(확률적 미분 방정식) 시스템을 이산화(discretisation)한 알고리즘입니다. 이를 통해 파라미터 추정이 이루어지고, 라그랑지안 조건(log-concavity)과 매끄러움(smoothness) 가정 하에서 수렴 보장을 제공하고 있습니다. 이론적 분석 결과, EBIPLA는 데이터 포인트 수와 입자 수가 증가할 때 최적 파라미터에 수렴하는 것으로 나타났습니다.

- **Performance Highlights**: 이 방법은 합성 데이터셋(synthetic datasets)과 이미지 생성(image generation) 작업에서의 효과성을 실증적으로 입증하였습니다. 경쟁력 있는 성능을 보여주며, LEBM(latent energy-based model) 훈련의 최초의 수렴 한계를 제시하였습니다. 이를 통해 EBIPLA가 기존 방법들과 비교했을 때 유의미한 이점을 제공함을 확인하였습니다.
...
- **What's New**: 이번 연구에서는 DeepTrust라는 새로운 메타휴리스틱 메소드를 제시하며, 이를 통해 다양한 유연한 분류기들을 계층적으로 배치하여 최종 결정을 단일 내부 모델이 내리도록 합니다. 이 모델은 조건에 따라 활성화되며, 기존 방법들보다 266% 높은 성능을 보여주었습니다.

- **Technical Details**: DeepTrust는 깊은 신경망과 같은 분류기를 정렬된 순서로 배열하여 입력을 순차적으로 평가하는 다단계 방어 메타휴리스틱입니다. 이를 통해 학습된 표현의 다양성을 극대화하고 공격자가 예측할 수 없는 결정 공간을 제공합니다.

- **Performance Highlights**: DeepTrust는 ELSA-RAMD 벤치마크 대회에서 금메달을 수상하였으며, 적대적 공격을 포함한 여러 테스트에서 이전의 최첨단 검출기보다 상당히 높은 성능을 보였습니다. 또한, 비적대적 샘플에 대한 높은 검출율과 1% 이하의 허위 긍정률을 유지하는 성능을 여전히 달성하였습니다.
...
- **What's New**: 이번 연구에서는 MARL-BC라는 새로운 프레임워크를 소개합니다. 이 프레임워크는 딥 멀티 에이전트 강화 학습(deep Multi-Agent Reinforcement Learning)과 실제 경제 주기 모델(Real Business Cycle models)을 통합하여 에이전트 간의 이질성을 보다 효과적으로 모사할 수 있도록 돕습니다. MARL-BC는 여러 면에서 기존 모델의 제약을 극복하며, 전통적인 일반 균형 모형이 가지는 한계를 뛰어넘는 가능성을 보여줍니다.

- **Technical Details**: MARL-BC 프레임워크는 다수의 이질적인 에이전트를 포함하여 고전적인 RBC 모델을 확장합니다. 또한, 최첨단 강화 학습 알고리즘(예: Proximal Policy Optimization(PPO), Soft Actor Critic(SAC), Deep Deterministic Policy Gradient(DDPG))을 사용하여 MARL-BC의 훈련이 computationally feasible함을 입증합니다. 이러한 접근 방식은 전통적인 GE 모형의 한계를 극복하며, 보장된 합리적 의사결정을 요구하지 않는 대안 모델로서 주목받고 있습니다.

- **Performance Highlights**: MARL-BC는 단일 에이전트를 사용할 때 고전적인 RBC 결과를 재현할 수 있으며, 대규모 동일 에이전트를 통해 평균 필드 Krusell-Smith 모델의 결과를 복원할 수 있습니다. 또한, 전통적인 GE 접근 방식으로는 어려운 에이전트 간의 풍부한 이질성을 효과적으로 시뮬레이션할 수도 있습니다. 이는 경제학 및 컴퓨터 과학 분야의 연구자들에게 큰 가치를 제공하며, 향후 다양한 응용 및 연구에 기여할 가능성을 높입니다.



### The Living Forecast: Evolving Day-Ahead Predictions into Intraday Reality (https://arxiv.org/abs/2510.12271)
- **What's New**: 본 논문은 완전 확률적(day-ahead probabilistic) 예측을 실시간 intraday 예측으로 변환하는 베이지안 업데이트 메커니즘을 소개합니다. 기존의 예측 모델을 재훈련(retrain)하거나 재추론(re-inference)할 필요 없이, 관측된 측정값을 기반으로 Gaussian mixture output의 조건부 분포를 이용하여 업데이트합니다. 이 방법은 포인트(point), 분위수(quantile), 앙상블(ensemble) 예측을 가능하게 하여 효율적이고 실시간 애플리케이션에 적합합니다.

- **Technical Details**: 이 연구에서 제안하는 방법은 새로운 intraday 관측 데이터를 바탕으로 기존의 day-ahead 예측 분포를 업데이트하는 베이지안 방법을 사용합니다. 이 접근 방식은 조건부 변형 오토인코더(Conditional Variational Autoencoder, CVAE)를 통해 모델링된 예측 분포로부터 직접 확률 분포를 업데이트합니다. 이를 통해 구성된 확률적 분포는 포인트 및 분위수 예측을 직접적으로 추출할 수 있는 구조를 유지하며, 계산 효율성을 고려하여 실용적인 적용을 염두에 두고 설계되었습니다.

- **Performance Highlights**: 주택 전력 소비와 태양광 발전 데이터를 대상으로 한 실험에서, 제안된 방법은 예측 정확도를 최대 25% 향상시키는 것으로 나타났습니다. 특히, 관측 데이터와 강한 시간적 상관관계를 가진 타임 스텝에서 가장 큰 성과가 있었습니다. 최적의 성능을 위해 패턴 사전 기반의 공분산 구조를 채택함으로써 더욱 향상된 결과를 보여주었습니다.
...
### A Gradient Guided Diffusion Framework for Chance Constrained Programming (https://arxiv.org/abs/2510.12238)
- **What's New**: 이번 논문에서는 불확실성이 포함된 최적화 문제를 다루기 위한 새로운 Gradient-Guided Diffusion 기반 최적화 프레임워크인 GGDOpt를 소개합니다. GGDOpt는 세 가지 주요 혁신을 통해 Chance Constrained Programming (CCP) 문제를 해결하며, 특히 정확한 분포 정보를 요구하지 않고 표본 집합만으로 폭넓은 CCP 문제를 처리할 수 있습니다. 이는 다양한 분야에서 CCP의 적용 가능성을 확대하는 데 기여합니다.

- **Technical Details**: GGDOpt는 비선형 제약 조건의 비볼록성 문제를 두 개의 분포, 즉 비록 알려지지 않은 데이터 분포와 목적 함수에 의해 정의된 볼츠만 분포의 곱으로 재구성하여 해결합니다. 이 과정에서 GGDOpt는 첫 번째 및 두 번째 차수의 기울기 정보를 활용합니다. 이론적 수렴 보장이 있으며, 수렴과 관련하여 두 가지 실용적인 오류 한계도 제시합니다.

- **Performance Highlights**: 실험 결과에 따르면 GGDOpt는 합성 데이터셋 및 무선 통신의 파형 설계 작업에서 기존 방법들보다 응답의 질과 안정성 측면에서 우수한 성능을 보입니다. 실제로는 약 80%의 부하 감소를 이루어내며, 이는 GGDOpt의 전반적인 효율성을 잘 보여줍니다.
...
- **What's New**: DISTAR는 전적으로 이산 잔여 벡터 양자화(RVQ) 코드 공간에서 작동하는 제로샷 텍스트-투-스피치(TTS) 프레임워크입니다. 이 시스템은 언어 모델(AR)과 마스크된 확산 모델을 결합하여 강력한 발음 및 스타일의 제어를 제공하면서 긴 처리 시간에도 높은 자연스러움을 유지합니다. DISTAR는 블록 수준 RVQ 토큰을 초안 작성하고, 마스크된 확산을 통해 블록을 보완하여 긴 형식의 합성을 가능하게 합니다.

- **Technical Details**: DISTAR는 RVQ 코드 공간 내에서 작동하는데, 이는 이전에 비해 높은 해상도의 오디오 복원을 지원하고 훈련/추론의 일관성을 보장합니다. 확산 모델은 마스크된 위치에서 이산적인 형태로 반복적으로 복원을 수행하여 질을 높이며, 패치 수준의 병렬 처리를 통해 긴 거리의 일관성을 개선합니다. RVQ 레이어는 아키텍처 상에서 전통적인 AR 시스템보다 연속적이지 않으면서도 질적 이점을 제공합니다.

- **Performance Highlights**: DISTAR는 기존 제로샷 TTS 시스템보다 효과적으로 높은 강건성과 자연스러움을 유지하며, 인퍼런스 비용을 줄여줍니다. 다양한 디코딩 전략을 지원하고, RVQ 레이어를 동적으로 조절하여 최적의 비트 전송률과 계산 제어를 가능하게 합니다. 또한, 다양한 오디오 샘플이 데모 페이지에 제공되고 있어 사용자가 직접 성능을 확인할 수 있습니다.



### Controllable Collision Scenario Generation via Collision Pattern Prediction (https://arxiv.org/abs/2510.12206)
Comments:
          8 pages, 3 figures. Submitted to IEEE International Conference on Robotics and Automation (ICRA) 2026

- **What's New**: 이 논문에서는 자율주행차(AV)의 안전성을 평가하기 위해 특정 충돌 유형과 사고 시간(TTA)을 지정해 이를 실현하는 경로를 생성하는 새로운 작업인 controllable collision scenario generation 을 제안합니다. 연구팀은 COLLIDE라는 대규모의 충돌 시나리오 데이터셋을 구축하여 다양한 충돌 유형과 시간을 균형있게 포함하도록 하였습니다. 여기에 기반하여, 충돌 패턴을 예측하는 프레임워크를 통해 효과적이고 해석 가능한 충돌 시나리오 생성을 가능하게 합니다.

- **Technical Details**: 논문에서 제안된 방법은 충돌 패턴을 활용하여 자율주행차의 예측 경로를 생성하는 구조를 채택하고 있습니다. COLLIDE 데이터셋은 현실 세계의 운전 로그를 변환하여 충돌 시나리오를 생성하며, 이를 통해 시스템적인 평가가 가능하도록 설계되었습니다. 실험에 사용된 프레임워크는 규칙 기반 모션 플래너에 대해 기존 방법들보다 더 높은 실패율을 유도하여, 현재 플래너의 한계를 드러냅니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 사용자 지정 충돌 유형과 TTA에 부합하는 시나리오를 지속적으로 생성하며, 높은 충돌률을 기록하여 더 그럴듯한 시나리오를 만들어냈습니다. 또한, 생성된 시나리오는 현재의 규칙 기반 플래너가 자주 실패하는 경향을 보여주었으며, 이는 AV의 안전성을 향상시키기 위한 훈련 데이터로 유용하게 사용될 수 있습니다. 최종적으로 본 연구는 자율주행차의 안전성에 대한 종합적이고 체계적인 평가를 위한 새로운 작업을 제안합니다.



### Learning Mean-Field Games through Mean-Field Actor-Critic Flow (https://arxiv.org/abs/2510.12180)
- **What's New**: 이 연구에서는 Mean-Field Actor-Critic (MFAC) 흐름을 제안하며, 이는 강화학습과 최적 수송(optimal transport) 기법을 결합한 연속 시간 학습 동역학(learning dynamics)입니다. MFAC 프레임워크는 제어(액터), 가치 함수(비평가), 분포 구성 요소를 부분 미분 방정식(partial differential equations; PDEs)에 의해 지배되는 커플링된 그래디언트 기반 업데이트를 통해 공동으로 진화시킵니다. 이 논문의 주요 혁신은 Wasserstein-2 측도를 따라 균형으로 분포를 운반하는 Optimal Transport Geodesic Picard (OTGP) 흐름입니다.

- **Technical Details**: MFAC 흐름은 액터가 비평가의 정보를 바탕으로 정책을 업데이트하고, 비평가는 현재 정책에 해당하는 가치 함수를 평가하며, OTGP 흐름에 의해 지배되는 분포 업데이트기로 구성된 세 가지 상호 의존적인 구성 요소로 이루어져 있습니다. 이 흐름은 단일 시간 척도에서 정책 업데이트, 정책 평가 및 인구 진화를 연계하는 연속 시간 학습 동역학을 소개합니다. 이 연구의 이론적 보장으로는 Lyapunov 기반 분석을 통한 MFAC 흐름의 글로벌 기하급수적 수렴이 있습니다.

- **Performance Highlights**: MFAC 흐름의 실용적 성능을 여러 벤치마크 예제에서 시연하여 그 안정성, 확장성 및 알려진 MFG 솔루션을 회복하는 능력을 확인하였습니다. 기계 학습 알고리즘은 액터와 비평가 모두를 매개변수화하는 신경망을 활용하며, 고차원 분포를 효율적으로 표현하기 위해 스코어 네트워크(score network)를 도입하였습니다. 이 방법은 보상과 동역학에서의 일반 분포 의존성을 처리할 수 있어, 실험적인 효율성을 개선하였습니다.



### Follow-the-Perturbed-Leader for Decoupled Bandits: Best-of-Both-Worlds and Practicality (https://arxiv.org/abs/2510.12152)
Comments:
          Preprint, 29 pages

- **What's New**: 본 논문에서는 독립적으로 탐색과 활용을 수행 할 수 있는 변별된 다중 무장 잡기(MAB) 문제를 다루고 있습니다. 이를 통해 탐색 손실을 관찰할 수 있지만, 활용 손실은 관찰할 수 없는 설정을 제안합니다. 제안된 정책은 Follow-the-Perturbed-Leader (FTPL) 프레임워크 내에서 Pareto perturbations을 사용하여 시행됩니다.

- **Technical Details**: 정책은 모든 환경에서 (근사적으로) 최적의 후회(regret)를 달성하는 것을 목표로 하며, 이는 Best-of-Both-Worlds (BOBW) 보장을 통해 이루어집니다. 랜덤하게 선택된 두 개의 팔을 통해 각 라운드에서 손실을 관찰하고 발생시키며, 과거의 최적해를 바탕으로 후회를 최소화하도록 합니다. 기존의 정책과 달리, 복잡한 최적화 단계를 피할 수 있어 계산적으로 더 효율적입니다.

- **Performance Highlights**: 본 연구의 정책은 기존의 Decoupled-Tsallis-INF와 비교했을 때 약 20배 더 빠르며, 두 가지 모든 환경에서 우수한 성능을 보여줍니다. 또한 순수 탐색 정책보다 나은 성능을 발휘하며, 순수 탐색과 일반 활용 정책을 단순 결합한 접근 방식보다 더 나은 성능을 나타냅니다. 이러한 성과는 실질적으로 비슷한 문제를 해결하는 데 있어 중요한 진전을 의미합니다.



### Probabilistic Super-Resolution for Urban Micrometeorology via a Schrödinger Bridg (https://arxiv.org/abs/2510.12148)
- **What's New**: 이번 연구는 Schrödinger bridge 문제를 해결하는 신경망(neural network)을 활용하여 도시 지역에서의 2m 온도의 초고해상도(supre-resolution)를 수행했습니다. 일반적으로 Schrödinger bridge는 확산 과정(diffusion process)에 기반한 두 데이터 분포(data distributions) 간의 변환을 설명합니다. 본 연구에서는 저해상도(low-resolution) 데이터를 직접적으로 고해상도(high-resolution) 데이터로 변환하는 특별한 Schrödinger-bridge 모델을 사용하였습니다.

- **Technical Details**: 우리는 20m 해상도에서 5m 해상도로 2m 온도를 초고해상도로 변환하는 과정을 설명합니다. Schrödinger bridge 모델은 특정 SDE(확률적 미분 방정식)를 사용하여 저해상도 데이터로부터 고해상도 데이터를 생성합니다. 이 모델은 기존의 Denoising diffusion probabilistic models(DMs)보다 훨씬 적은 계산 비용을 요구하며, 각각의 데이터에 대해 50번의 신경망 평가가 필요했던 DMs와 비교하여, SM은 단 10번의 평가로 고해상도 샘플을 생성합니다.

- **Performance Highlights**: 연구 결과, SM이 DM과 비교하여 비슷한 정확도를 얻으면서도 계산 비용을 5분의 1로 줄일 수 있음을 보여주었습니다. 또한, SM으로 생성된 고해상도 샘플은 더 큰 분산을 보이며, 이는 DM 대비 더 나은 불확실성 정량화(uncertainty quantification)를 의미합니다. SM의 계산 비용 절감 덕분에, 우리 연구 결과는 실제 시간에 따른 집합 변화 기상 예측(real-time ensemble micrometeorological prediction)의 가능성을 시사합니다.
...
- **What's New**: 본 논문에서는 기존의 기능 잠금 기술(FLoTE)에서의 한계를 극복한 새로운 시스템인 Locket을 소개하고 있습니다. 이 시스템은 기능 요청에 대해 유효한 응답을 제공하면서도, 비인가된 기능 요청에 대해서는 거부하는 기능을 제공합니다. Locket은 고유한 병합 접근 방식을 사용하여, 대형 언어 모델(LLM)에 어댑터를 추가함으로써 비인가된 기능 사용을 차단합니다.

- **Technical Details**: Locket은 어댑터를 통해 각 프리미엄 기능에 대한 접근 제어를 가능하게 하며, 사용자 확인 기능을 해결하기 위해 액세스 제어 모듈을 사용할 수 있습니다. 일반 비밀번호 기반 방법과 달리 Locket은 비밀번호와 같은 비밀 자격 증명을 요구하지 않아 비인가 공유를 방지하며, 새로운 기능에 대해 어댑터를 하나씩 학습시키기만 하면 되므로 효율적으로 확장됩니다.

- **Performance Highlights**: Locket에 대한 포괄적인 평가 결과, 잠금된 기능에 대해서는 100%의 거부율을 기록했으며, 해제된 기능의 경우 7% 이하의 유틸리티 저하를 보였습니다. 또한, 공격 성공률은 5% 이하로 나타나 강력한 견고성을 입증하였습니다. 이 시스템은 여러 기능과 사용자에 대해 확장 가능한 기능을 가지고 있어, 향후 다양한 상업적 매출원 도입에 기여할 것으로 기대됩니다.
...
### FedLoDrop: Federated LoRA with Dropout for Generalized LLM Fine-tuning (https://arxiv.org/abs/2510.12078)
- **What's New**: 이번 연구는 Federated LoRA with Dropout (FedLoDrop)이라는 새로운 프레임워크를 제안합니다. 이 프레임워크는 파라미터 효율적인 미세 조정(GPT) 방법을 통해 훈련 가능한 매트릭스의 행과 열에 드롭아웃(dropout) 기법을 적용하여 일반화 능력을 향상시킵니다. 연구에서는 점추측 가설 안정성(pointwise hypothesis stability)과 수렴(convergence) 분석을 수행하여 과적합(overfitting)과 미적합(underfitting) 간의 트레이드오프를 명확히 설명합니다.

- **Technical Details**: FedLoDrop은 네트워크 자원 제약 하에 미세 조정의 일반화 오류를 최소화하기 위한 최적화 문제를 공식화하며, 드롭아웃 비율(dropout rate)과 자원 할당(resource allocation)을 동시에 최적화합니다. 특히, 더 높은 드롭아웃 비율은 낮은 일반화 오류를 초래할 수 있으며, 이는 각 장치에서의 네트워크 요구 사항에 따라 조정됩니다. 또한, 전역 최적 해(global optimal solution)를 찾기 위해 가지치기(pruning) 방식의 최적화 방법이 제안되었습니다.

- **Performance Highlights**: 제안된 방법은 RoBERTa-large(355M) 및 LLaMA(7B) 모델을 GLUE 및 MMLU 벤치마크에서 훈련시켜 성능을 평가했습니다. 드롭아웃을 적용한 모델의 일반화 능력이 크게 향상되었으며, 이는 미세 조정 데이터셋의 지식을 자연어 응답 작업에 효과적으로 적용할 수 있게 합니다. 더 많은 네트워크 자원과 긴 지연(latency)을 통해, 모든 장치에서 낮은 드롭아웃률을 허용하여 테스트 성능이 개선되었습니다.
...
- **What's New**: 이 논문은 신경망의 압축 가능성을 연구하며, 최소 묘사 길이(MDL) 원칙을 신경망과 같은 특이 모델에 확장하는 방법을 제시합니다. 저자들은 Pythia 스위트에서 양자화, 인수 분해 및 기타 압축 기술을 사용한 광범위한 실험을 통해 지역 학습 계수(LLC) 기반의 복잡성 추정이 압축 가능성과 밀접하게 연결되어 있음을 발견했습니다. 이러한 결과는 모델 압축의 한계를 엄밀하게 평가할 수 있는 경로를 제공합니다.

- **Technical Details**: 저자들은 신경망에 대한 특이 MDL 원칙을 도출하였습니다. 이들은 특이 학습 이론(SLT)에서 아이디어를 바탕으로, MDL 원칙을 신경망에 확장하고 지역 학습 계수(LLC)와 관련된 코드 길이의 비대칭적 여분을 증명했습니다. 이 과정에서 일반적인 MDL 처리와는 달리, 이러한 신경망에서는 기하학적 특성이 퇴화성(degeneracy)이라는 점이 중요한 요소로 나타났습니다.

- **Performance Highlights**: 저자들은 Pythia 모델에서 압축 성능과 LLC를 서로 비교하여 실험적으로 검토했습니다. LLC가 큰 모델은 일반적으로 덜 압축 가능함을 발견했으며, 양자화 과정의 경우 LLC와 압축 가능성 간에 특히 밀접한 선형 관계를 관찰했습니다. 이러한 결과는 LLC 추정치가 큰 변환기 모델에도 유효하게 사용할 수 있는 독립적인 검증 방법을 제공할 수 있음을 시사합니다.
...
### Statistical Guarantees for High-Dimensional Stochastic Gradient Descen (https://arxiv.org/abs/2510.12013)
Comments:
          Accepted to NeurIPS 2025

- **What's New**: 이 논문에서는 상수 학습률을 사용하는 Stochastic Gradient Descent (SGD)와 Ruppert-Polyak 평균 변형인 ASGD에 대해 고차원 환경에서의 통계적 보장을 제공합니다. 기존의 이론적 연구에서 고차원 설정에서 SGD의 일반화 경계가 제한적이었으나, 특히 비선형 자기회귀 과정으로 SGD를 재구성하여 이 문제를 해결했습니다. 이를 통해 고차원 기계 학습 알고리즘을 분석할 수 있는 새로운 도구를 제시했습니다.

- **Technical Details**: 우리는 고차원 설정에서 SGD의 기하학적 모멘트 수축을 증명하고 반복(iterate)의 비대칭 평형성을 확립했습니다. 여기서 SGD는 고차원 역설계 문제에서 자주 사용되며, 특히 큰 상수 학습률을 사용하여 수렴 속도를 가속화할 수 있습니다. 또한, 일반적인 ℓ^s-노름에 대한 SGD와 ASGD의 q차 모멘트 수렴을 유도하고, 고차원 희소 혹은 구조화 모델에서 자주 사용되는 ℓ^∞-노름에 대한 분석도 포함하였습니다.

- **Performance Highlights**: 이 논문에서는 고차원 ASGD의 확률적 경계를 제공하며, SGD의 평균 성능을 수치적으로 보증할 수 있는 방법을 제시합니다. 또한, 연구 결과는 다양한 학습 문제에 유효한 높은 확률의 경계와 응답 시간이 필요할 때의 반복 수를 제공합니다. 발전된 이론적 결과는 비대칭 환경에서도 학습의 안정성을 보장하는 데 기여하며, SGD 이론에서의 중요한 간극을 메우는 데 중요한 기여를 하고 있습니다.



### UALM: Unified Audio Language Model for Understanding, Generation and Reasoning (https://arxiv.org/abs/2510.12000)
- **What's New**: 최근 오디오 언어 모델링(Audio Language Modeling, ALM) 분야의 발전은 오디오 이해와 텍스트-오디오 생성(Text-to-Audio Generation)을 별개의 작업으로 다루고 있습니다. 그러나 몇몇 연구는 이 작업들을 통합하여 멀티모달 추론(Multimodal Reasoning)의 발전을 위해 필수적인 작업을 시도하고자 합니다. 본 논문은 Unified Audio Language Model (UALM)을 소개하며, 이는 오디오 이해, 텍스트-오디오 생성, 그리고 멀티모달 추론을 단일 모델에서 통합하는 것을 목표로 합니다.

- **Technical Details**: UALM은 텍스트-오디오 언어 모델인 UALM-Gen을 통해 오디오 토큰을 직접 예측하며, 최첨단 확산(diffusion) 모델과 비교 가능한 성능을 보여줍니다. 이 모델은 데이터 블렌딩, 훈련 레시피, 그리고 추론 기술을 효과적으로 사용하여, 오디오 이해, 텍스트-오디오 생성 및 텍스트 추론 분야에서 최첨단 전문 모델과 유사한 품질을 달성합니다. 특히 UALM-Reason은 텍스트와 오디오를 활용하여 복잡한 생성 작업을 지원하는 멀티모달 추론 모델입니다.

- **Performance Highlights**: 본 연구는 UALM-Gen이 텍스트-오디오 생성에서 최첨단 품질을 달성하며, UALM이 오디오 이해, 텍스트-오디오 생성, 텍스트 문제 해결 세 가지 분야에서 경쟁력 있는 결과를 도출하는 것을 보여줍니다. 또한 UALM-Reason은 멀티모달 추론 능력을 통해 이해 및 생성 작업 간의 통합을 실현하고, 더욱 정교한 프롬프트에 대한 제어성을 향상시킴을 입증하였습니다. 이 모든 기여는 일반 오디오 지능(General Audio Intelligence) 달성을 향한 중요한 발전으로 여겨집니다.



### Scaling Long-Horizon LLM Agent via Context-Folding (https://arxiv.org/abs/2510.11967)
- **What's New**: 이번 논문에서는 대규모 언어 모델(LLM) 에이전트가 긴 작업에서 제약을 받는 컨텍스트 길이를 관리하는 새로운 방법인 Context-Folding을 소개합니다. 이 프레임워크는 에이전트가 하위 작업을 수행하기 위해 절차적으로 서브 경로로 분기한 후, 완료 후 중간 단계를 간추린 요약으로 접을 수 있게 해줍니다.

- **Technical Details**: Context-Folding을 학습 가능하게 하기 위해, 우리는 FoldGRPO라는 엔드 투 엔드 강화 학습(framework) 프레임워크를 개발했습니다. 이 프레임워크는 효과적인 작업 분해(task decomposition)와 컨텍스트 관리를 장려하기 위한 특정 프로세스 보상(process rewards)을 포함합니다. 이를 통해 에이전트가 하위 작업을 효율적으로 수행할 수 있도록 지원합니다.

- **Performance Highlights**: 실험 결과, 복잡한 장기 작업인 Deep Research 및 SWE에서 우리의 접기 에이전트는 ReAct 기준선을 초과하거나 동등한 성과를 보였습니다. 매우 작은 활성 컨텍스트(active context)를 사용하면서도, 요약 기반 컨텍스트 관리에 의존하는 모델보다 상당히 뛰어난 성과를 나타냈습니다.



### Enhancing Diffusion-Based Sampling with Molecular Collective Variables (https://arxiv.org/abs/2510.11923)
- **What's New**: 이번 논문에서는 집합 변수(collective variables, CVs)를 활용하여 확산 기반 샘플러의 탐색 능력을 향상시키는 방법을 소개합니다. 주목할 점은 최근 샘플에 대한 CV 중심의 반발(a repulsive potential) 잠재력을 도입하여, 샘플들이 새로운 CV 영역으로 추출될 수 있도록 유도한다는 것입니다. 이 접근법은 샘플링 효율성을 향상시키고, 자유 에너지 차이를 추정하는 데 있어 중요한 역할을 합니다.

- **Technical Details**: 제안된 방법인 Well-Tempered Adjoint Schrödinger Bridge Sampler (WT-ASBS)는 최신 확산 기반 샘플러인 ASBS에 기반하여 만들어졌습니다. 이 방법은 잘 조절된 메타다이나믹스(well-tempered metadynamics)에서 영감을 받아, 샘플링 과정 중에 CV 공간에서의 편향(bias)을 적용합니다. 이로 인해, 잦은 샘플링을 통해 탐색되지 않는 드문 모드에 대해 보다 넓은 탐색을 가능하게 하며, 통계적으로 정확한 추정을 유지합니다.

- **Performance Highlights**: WT-ASBS는 표준 펩타이드 구조 샘플링 벤치마크에서 다양한 구조 상태와 정확한 자유 에너지 프로필을 복원하는 데 성공하였습니다. 또한, 이는 확산 기반 샘플러로서의 반응 샘플링을 처음으로 시연하였으며, 결합의 형성과 파괴를 포착함으로써 일반적인 원자 간 포텐셜(universal interatomic potentials)에서 근사적인 1차 원리의 정확도를 달성하였습니다. 본 접근법은 표준 샘플링 방법에 비해 시간 효율성을 크게 향상시키며, 분자 과학에서의 활용 가능성을 높이고 있습니다.



### Simplifying Optimal Transport through Schatten-$p$ Regularization (https://arxiv.org/abs/2510.11910)
Comments:
          26 pages, 4 figures

- **What's New**: 이 논문은 Schatten-$p$ 노름(Schatten-$p$ norm) 정규화를 사용하여 최적 수송(optimal transport)에서 저차원 구조를 복원하는 새로운 일반 프레임워크를 제안합니다. 이 접근법은 간결하고 해석 가능한 수송 맵(transport maps)을 촉진하는 기존 방법을 확장하며, 저차원 구조를 장려하는 통합된 볼록 프로그램(convex programs)을 제공합니다. 이를 통해 저차원 결합(low-rank coupling) 및 중심 맵(barycentric maps)에 대한 최적 조건(optimality conditions) 및 회복 보장을 증명하였습니다.

- **Technical Details**: 제안된 Schatten OT 프로그램은 이전의 저차원 및 희소(sparse) 방법에 대한 많은 연구를 동시에 일반화하며 새로운 정규화된 형식을 제공합니다. 또한, 우리는 Schatten OT 문제를 효율적으로 해결하는 일반적인 미러 하강(mirror descent) 프레임워크를 제안하였습니다. p≥1인 경우, 최적화 문제는 볼록(convex)하게 되어 미러 하강을 통한 수렴 보장 및 저차원 결합, 수송 변위의 저차원 구조에 대한 분석이 가능합니다.

- **Performance Highlights**: 합성 데이터(synthetic data)와 실제 데이터(real data)를 사용한 실험은 Schatten OT의 유연성(flexibility)과 효과성(effectiveness)을 입증했습니다. 이 연구는 정규화된 OT가 현재의 응용 및 이론적 연구에서 더욱 중요해지고 있다는 점을 강조하며, 구체적인 세부 구조(sparse structures) 및 효율성을 동시에 달성할 수 있는 기회를 제공합니다.



### LLM Knowledge is Brittle: Truthfulness Representations Rely on Superficial Resemblanc (https://arxiv.org/abs/2510.11905)
- **What's New**: 이 논문은 대규모 언어 모델(LLM)의 신뢰성을 높이기 위해 일반화된 지식을 학습해야 한다는 점을 강조합니다. 연구 결과, LLM은 훈련 과정과 유사하지 않은 입력 변화에 대해 지나치게 민감하며, 이는 내부 지식 표현의 불안정성과 관련이 있다고 제안합니다. 저자들은 기초 지식의 강건성을 평가하기 위해 진실성 프로브를 활용하여, 각 데이터의 외부 분포(OOD)가 증가함에 따라 LLM의 진실성 분리 가능성이 어떻게 감소하는지를 연구했습니다.

- **Technical Details**: 연구 방법론은 세 가지 구성 요소로 이루어집니다. 첫째, 주어진 문장이 훈련 데이터와 얼마나 OOD인지에 대한 측정으로 평균 perplexity를 사용합니다. 둘째, 문장의 의미를 변경하지 않으면서 OOD로 전환시키는 의미 보존 변형을 적용합니다. 마지막으로, 세 가지 probing 기술을 사용하여 변형된 샘플에서 학습된 지식 표현의 분리 가능성을 테스트합니다. 이 분석은 네 가지 모델 계열과 네 개의 데이터셋을 포함하여 2000개 이상의 프로브를 사용하여 수행되었습니다.

- **Performance Highlights**: 연구 결과, LLM의 내부 지식 표현의 강건성은 훈련 데이터와의 표면 유사성에 밀접하게 연결되어 있음을 확인했습니다. OOD가 증가함에 따라 진실성 분리 가능성이 점점 감소하며, 이는 모든 테스트된 진실성 프로브 기술에서 나타났습니다. 특정 주제(예: 마케팅)는 분포 변화에 더 강한 경향이 있는 반면, 역사와 같은 다른 주제는 덜 강건한 것으로 나타났습니다. 종합적으로, LLM은 얕고 비강건한 지식 표현을 학습하여 훈련 중에 보지 못한 시나리오에 대한 일반화에 실패하는 경향이 있음을 시사합니다.



### High-Probability Bounds For Heterogeneous Local Differential Privacy (https://arxiv.org/abs/2510.11895)
- **What's New**: 이번 연구에서는 사용자들이 이질적인 개인 정보 보호 수준(heterogeneous privacy levels)을 가질 때, 지역적 차등 개인 정보 보호(local differential privacy, LDP) 하에서 통계적 추정을 연구합니다. 특히 모든 사용자가 동일한 개인 정보 보호 보장을 공유하는 기존의 모델을 넘어, 각 사용자의 개인 정보 요구 수준에 맞는 알고리즘을 개발했습니다. 이를 통해, 통계적으로 정확한 결과를 높은 확률로 보장할 수 있는 새로운 접근 방식을 제안합니다.

- **Technical Details**: 본 논문에서는 한 차원 및 다차원 평균 추정 문제를 다루며, $	ext{l}_2$-norm 내에서 유한 샘플 상한을 개발하였습니다. 각 사용자에 대해 개별적인 개인 정보 보호 매개변수(εi)를 고려하고, 이러한 이질적인 설정에서의 오류 경계를 적절히 정의합니다. 연구 결과는 각 사용자에 대한 개인 정보 요구가 다를 경우에도, απο지에서의 제약 조건을 충족할 수 있는 효율적 알고리즘을 제안합니다.

- **Performance Highlights**: 고신뢰도로 달성된 오류 경계는 전통적인 방법에 비해 더 긴밀하며, 최적의 의존성을 보여줍니다. 특히, 우리의 기법은 높은 차원의 LDP 평균 추정 문제에 대해서도 최적의 성능을 보이며, 모든 매개변수에 대한 의존성을 정립한 최초의 사례입니다. 또한, 이 연구에서 제안한 방법론은 고확률 확률 보장 하에 분포 학습(distribution learning) 문제에 적용되어 새로운 상한을 제시합니다.



### Active Subspaces in Infinite Dimension (https://arxiv.org/abs/2510.11871)
- **What's New**: 이 논문에서는 Hilbert 공간의 실수 값 함수들에 대해 Active Subspace 분석 방법론을 확장하였습니다. 이 방법론은 기존의 Euclidean 공간에 대한 Active Subspace 행렬과 일치하는 연산자를 정의합니다. 또한, 언급된 방법론은 무한 차원 설정에 대해서도 많은 유용한 특성을 확장할 수 있음을 입증합니다.

- **Technical Details**: 본 논문에서는 Active Subspace 분석을 Hilbert 공간의 실수 값 함수에 적용하는 방법을 제안하였습니다. Monte Carlo 절차를 제안하여 그 수렴 특성에 대해서도 논의하고, 이 방법론을 사용하여 복잡한 테스트 문제에서 시각화 및 모델링, 최적화를 개선하는 방법을 보여줍니다. 특히, 표준 Active Subspace 분석이 아닌 기능적 Active Subspace 분석을 통해 주요 차원을 발견하는 과정을 설명합니다.

- **Performance Highlights**: 이 방법론은 실제 분석에서의 적용을 통해 뚜렷한 이점을 보여주며, FEniCS를 기반으로 한 오픈 소스 코드를 배포하여 실용적인 사례 연구를 제공하고 있습니다. 이러한 연구 결과는 과학 및 공학 분야에서도 기존의 분석 방법보다 높은 성능을 보여줄 것으로 기대됩니다.
...
### On Thompson Sampling and Bilateral Uncertainty in Additive Bayesian Optimization (https://arxiv.org/abs/2510.11792)
- **What's New**: 이번 논문에서는 Additive Bayesian Optimization (BO)에서 쌍방 불확실성(bilateral uncertainty, BU)을 고려하여 효율적인 Thompson Sampling 방식을 제안합니다. 기존의 접근 방식들은 이 BU를 무시함으로 인해 발생하는 두 번째 수준의 근사화 문제를 다루지 못했습니다. 연구 결과에 따르면, BU를 무시한 근사화가 실제 응용에서는 큰 문제가 되지 않는다는 점이 입증되었습니다.

- **Technical Details**: 최적화 문제는 고차원에서 함수를 정확히 모델링하기 어려움으로 고통받고 있습니다. 이 연구에서는 conditional independence의 개념을 활용해, BU를 고려한 효율적인 Thompson Sampling을 제안합니다. 또한, BU를 무시했을 때의 손실을 실증적으로 조사하였고, 그 결과 BU를 무시한 추가 근사가 BO에서 실용적으로 충분하다는 것을 보여줍니다.

- **Performance Highlights**: 실험 결과, BU를 포함한 Thompson Sampling이 기존의 대안보다 일관되게 우수한 성과를 보이지 않았습니다. 논문에서는 BU를 무시하는 것이 관찰된 특정 함수에 대해 실질적으로 큰 이점을 제공하지 않는다는 결론을 도출했습니다. 이는 BO 방법론 커뮤니티에 공헌할 수 있는 중요한 시사점을 제공하며, BU를 고려하기 위한 연구 노력의 우선순위를 재고할 필요성을 제기합니다.



### Dimension-Free Minimax Rates for Learning Pairwise Interactions in Attention-Style Models (https://arxiv.org/abs/2510.11789)
- **What's New**: 본 논문에서는 단일 레이어 주의(attention) 스타일 모델에서 쌍(pairwise) 상호작용의 수렴 속도를 규명합니다. 우리는 주의 메커니즘이 입력 토큰 간의 비국소(nonlocal) 의존성을 모델링하는 방식을 분석하여, 몬테 카를로 샘플의 크기 M에 따라 $M^{-rac{2eta}{2eta+1}}$ 의 최적 수렴 속도를 입증했습니다. 중요한 것은 이 속도가 토큰 수나 차원에 독립적이라는 점입니다.

- **Technical Details**: 여기서 주의 메커니즘을 통해 두 개의 토큰 간 상호작용을 모델링하며, 이 상호작용은 비선형 활성화 함수와 무관하게 학습됩니다. 또한, 우리는 이 문제를 IPS(Interacting Particle System) 모델을 통해 설명하며, 이는 상호작용에서 발생하는 복잡함을 이해하는 데 유용한 프레임워크를 제공합니다. 본 연구에서는 비독립적이고 비등방성(anisotropic) 데이터에 대해서도 다룰 수 있습니다.

- **Performance Highlights**: 우리가 입증한 수렴 속도는 $M^{-rac{2eta}{2eta+1}}$로, 이는 2차원 쌍 상호작용 함수를 추정하는 최적의 최소 최대(minimax) 수렴 속도입니다. 이 속도는 주의 스타일 모델이 차원의 저주(curse of dimensionality)에서 벗어날 수 있음을 의미하며, 이는 다양한 실제 상황에서 더욱 효율적인 모델 학습과 설명 가능성을 향상시킬 수 있습니다.
...
- **What's New**: PRISM은 단백질 역접기(inverse folding) 문제를 해결하기 위해 설계된 다중 모달 검색 기반 생성(framework) 프레임워크로, 알려진 단백질에서 잠재적인 모티프에 대한 세부 표현을 검색하고 이를 하이브리드 셀프-크로스 어텐션 디코더(hybrid self-cross attention decoder)와 통합한다. 이 프레임워크는 단백질 설계의 새로운 접근법으로, 진화적으로 보존된 세부 구조-서열 패턴(reusable fine-grained structure-sequence patterns)을 명시적으로 재사용할 수 있는 메커니즘을 제공하여 성능을 향상시킨다.

- **Technical Details**: PRISM은 잠재 변수(probabilistic model) 모델로서 형성되어 있으며, 이론적인 기반과 실용적 확장성을 결합하여 효율적인 근사(approximation) 방법으로 구현된다. 이 모델은 거대 단백질 벡터 데이터베이스(large vector database)에서 잠재적인 모티프의 임베딩을 검색하고, 하이브리드 변환 디코더(hybrid transformer decoder)를 사용하여 시퀀스 출력을 개선하는 방식을 따른다. 이를 통해 각 리지듀(residue) 예측은 검색된 로컬 프래그먼트(local fragments)에 의해 안내된다.

- **Performance Highlights**: PRISM은 CATH-4.2, TS50, TS500, CAMEO 2022, PDB 날짜 분할의 다섯 가지 벤치마크를 기준으로 하여 마침내 perplexity와 아미노산 회복(amino acid recovery) 영역에서 새로운 최고 성능을 달성하였다. 또한, foldability 메트릭(RMSD, TM-score, pLDDT)의 개선을 통해, 세부 다중 모달 검색(fine-grained multimodal retrieval)이 단백질 서열 설계에 강력하고 효과적인 패러다임임을 입증하였다.



### Quantum Kernel Methods: Convergence Theory, Separation Bounds and Applications to Marketing Analytics (https://arxiv.org/abs/2510.11744)
Comments:
          15 pages, 3 figures

- **What's New**: 이번 연구는 NISQ(Noise Intermediate-Scale Quantum) 환경에서 진짜 소비자 데이터셋에 대한 양자 커널 방법의 적용 가능성을 조사했습니다. 이를 위해 양자-커널 지지 벡터 머신(Q-SVM)과 양자 특징 추출 모듈(QFE)을 결합한 하이브리드 파이프라인을 제안했습니다. 이 방법은 클래식 및 양자 기준선과 비교하여 성능을 평가하며, 초기 결과로 0.7790의 정확도와 0.83 AUC를 기록했습니다.

- **Technical Details**: 양자 특징 맵(quantum feature map)은 고전적 입력 공간에서 양자 힐베르트 공간으로 매핑하는 기능을 수행하며, 이 과정은 파라미터화된 양자 회로를 통해 구현됩니다. Q-SVM의 최적화 문제는 주어진 데이터셋에 대해 특정 조건을 만족해야 하며, 하이퍼파라미터를 고정한 결과가 정확도 및 재현율을 높입니다. 이 연구에서는 NISQ 제한 아래에서 얕은 임베딩의 적합성을 무엇보다 강조하고 있으며, 배경에 있는 이론적 기초를 강화하고 있습니다.

- **Performance Highlights**: Q-SVM은 0.8609의 재현율과 0.7647의 정밀도를 기록하며, 실제 소비자 분류 문제에 대한 해결책을 제시했습니다. 0.83 AUC는 재훈련 없이도 정책 선택을 지원하는 유연한 임계값 조정이 가능함을 나타냅니다. 결과적으로 이 연구는 마케팅 분석에 직접적인 영향을 미치며, 정밀도와 재현율에 따른 다양한 접근 방식을 가능하게 합니다.
...
### Multi-objective Bayesian Optimization with Human-in-the-Loop for Flexible Neuromorphic Electronics Fabrication (https://arxiv.org/abs/2510.11727)
- **What's New**: 이번 연구에서는 메탈 산화물 소재를 사용한 유연한 신경모방 전자소자의 제작을 위한 새로운 방법론을 제시합니다. 특히, 폴리머 기판과의 호환성 문제를 해결하기 위해 포토닉 커링(photonic curing) 기술을 도입하였습니다. 이는 기존의 전통적인 프로세스를 넘어서는 혁신적인 접근법으로, 유연한 금속-절연체-금속 커패시터(flexible metal-insulator-metal capacitors)의 제작 가능성을 보여줍니다.

- **Technical Details**: 연구진은 다목적 베이지안 최적화(multi-objective Bayesian optimization, MOBO)를 통해 전기적 속성을 최적화하는 파라미터를 탐색하였습니다. 이 방법은 높은 정전용량과 낮은 누설 전류를 가진 최적의 포토닉 커링 조건을 식별하는데 도움을 줍니다. 또한, 인체-in-the-loop(human-in-the-loop, HITL) 프레임워크를 개발하여 실험 실패를 MOBO ML(머신러닝) 워크플로우에 통합하였습니다.

- **Performance Highlights**: 최적화가 완료된 후, 다양한 파레토 최적(Pareto-optimal) 조건을 분석하여 절연체 속성을 조정하고 Shapley Additive exPlanations 분석을 통해 다양한 입력의 중요성을 강조하였습니다. MOBO와 HITL 피드백을 결합한 이 프레임워크는 상호 연결된 입력과 높은 실험 실패율을 가진 다양한 다목적 실험 문제에 적응할 수 있으며, 머신러닝 모델에 유용한 결과를 생성할 수 있습니다.



### scPPDM: A Diffusion Model for Single-Cell Drug-Response Prediction (https://arxiv.org/abs/2510.11726)
- **What's New**: 이번 논문은 scRNA-seq 데이터에서 약물 반응 예측을 위한 첫 번째 확산 기반 프레임워크인 Single-Cell Perturbation Prediction Diffusion Model (scPPDM)을 소개합니다. scPPDM은 두 가지 조건 채널, 즉 변동 전 상태(pre-perturbation state)와 약물-용량(drug with dose)을 통합된 잠재 공간(latent space)에서 결합하여 사용합니다. 이는 상태 보존(state preservation) 및 약물 반응 강도(drug-response strength)에 대한 해석 가능한 제어를 제공합니다.

- **Technical Details**: 이 모델은 GD-Attn이라는 비-연결(concatenative) 방법을 통해 조건을 결합하여, 주어진 용량에 맞춰 저항적인 표현을 예측합니다. scPPDM은 노이즈 제거(diffusion) 및 시간 임베딩을 활용하여 정확한 포스트-변동(post-perturbation) 표현을 모델링하며, 여러 상태에서의 예측을 위해 인퍼런스(inference) 시 두 개의 계수를 가진 가이던스 규칙을 형성합니다. 이는 약물 채널의 가이드 힘에 용량을 매핑하여 투명한 제어를 가능하게 합니다.

- **Performance Highlights**: Tahoe-100M 벤치마크에서 scPPDM은 이전 모델에 비해 log fold-change recovery, delta correlations, explained variance, DE-overlap 등에서 새로운 최첨단 결과를 달성했습니다. 특히, unseen drugs(UD)에서 DEG logFC-Spearman/Pearson 스코어에서 각각 +36.11%와 +34.21%의 성과를 보였습니다. 이 모델은 실험적인 부담을 줄이면서 생물학적 특성을 보존하는 투명한 분석 및 용량 조정을 가능하게 합니다.

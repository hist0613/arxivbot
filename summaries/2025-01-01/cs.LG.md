...
### Unified dimensionality reduction techniques in chronic liver disease detection (https://arxiv.org/abs/2412.21156)
- **What's New**: 이 연구는 인도 간 환자 데이터셋(ILPD)을 사용하여 간 질환 예측을 위한 다양한 머신 러닝 알고리즘의 성능을 평가하는 혁신적인 방법론을 제안합니다. 새로운 기법으로 IQR 기반의 이상치 대체(outlier replacement), 오버샘플링(oversampling), LDA, FA, t-SNE, UMAP과 같은 차원 축소(dimensionality reduction) 기법의 조합을 포함합니다. 연구 결과, Random Forest 알고리즘은 98.31%의 높은 정확도를 기록하며, 이는 간 질환 조기 진단 개선에 중요한 통찰을 제공합니다.

- **Technical Details**: 본 연구에서는 583명의 환자 기록을 포함하는 ILPD 데이터셋을 활용하며, 여러 차원 축소 기법과 피쳐 추출(feature extraction) 방법을 적용하여 데이터 전처리를 진행하였습니다. 또한 Multi-layer Perceptron, Random Forest, K-nearest neighbours, Logistic Regression 등의 다양한 분류(classification) 알고리즘을 사용하여 모델의 예측 성능을 평가하였습니다. 이러한 기술적 접근 방법은 고차원 데이터를 효과적으로 변환하고 예측 정확도를 향상시키는 데 중점을 두고 있습니다.

- **Performance Highlights**: Random Forest 알고리즘의 성능이 특히 두드러졌으며, 10-fold cross-validation에서는 98.31%, train-test split에서는 95.79%의 정확도를 기록하였습니다. 이는 간 질환 환자의 진단 정확도를 향상시키는 데 기여할 것으로 기대됩니다. 본 연구는 간 질환 진단에 있어 맞춤형 피쳐 추출 및 차원 축소 기법의 채택과 활용에 대한 새로운 관점을 제공하며, 이는 임상 의사 결정에 실질적인 영향을 미칠 것입니다.
...
### Functional Risk Minimization (https://arxiv.org/abs/2412.21149)
- **What's New**: 기계 학습(Machine Learning) 분야는 1970년대 이후 크게 변화했지만, 가장 기본적인 원칙인 경험적 위험 최소화(Empirical Risk Minimization, ERM)는 여전히 유지되고 있습니다. 본 논문에서는 출력(output)이 아니라 함수(function)를 비교하는 손실(loss) 기반으로 하는 일반 프레임워크인 함수적 위험 최소화(Functional Risk Minimization, FRM)을 제안합니다. FRM은 감독 학습(supervised learning), 비감독 학습(unsupervised learning), 그리고 강화 학습(Reinforcement Learning) 실험에서 더 나은 성능을 보이는 것을 목표로 하고 있습니다.

- **Technical Details**: ERM은 모델이 간단하고 작았던 시기에는 좋은 교육 성능이 테스트 성능을 보장할 수 있었지만, 현재의 거대한 신경망(neural networks)에서는 이러한 보장이 더 이상 유효하지 않습니다. FRM은 각 데이터 포인트 $(x_i,y_i)$에 대해 적합한 함수 $f_{	heta_i}$를 설정하여, 기능적 노이즈(function noise)로 데이터를 설명합니다. 이는 FRM이 ERM을 대체하며, 데이터 셋을 단일 함수로 묶기보다는 여러 개의 함수로 구성해 데이터의 다양한 변동성을 포착할 수 있도록 합니다.

- **Performance Highlights**: 본 논문에서는 기능적 생성 모델(Functional Generative Models, FGMs)을 소개하고, FRM 프레임워크를 도출하며, 이를 통해 과도 파라미터화(Over-parameterization)된 신경망의 일반화를 연결지었습니다. FRM의 여러 실험 결과들을 통해 감독 학습, 비감독 학습, 강화 학습 등 다양한 분야에서 성능 향상을 입증하고 있습니다. 이러한 접근은 현대 기계 학습 모델의 효율성을 개선하고, 데이터를 보다 현실적으로 모델링 할 수 있는 방법을 제공할 것으로 기대됩니다.



### Adaptive Batch Size Schedules for Distributed Training of Language Models with Data and Model Parallelism (https://arxiv.org/abs/2412.21124)
- **What's New**: 이번 논문에서는 대규모 언어 모델의 사전 훈련을 위해 제안된 적응형 배치 크기 스케줄의 중요성을 강조합니다. 기존의 상수 배치 크기 및 휴리스틱한 배치 크기 웜업 스케줄과는 달리, 이 연구는 더 동적인 방법으로 훈련 효율성과 모델 일반화 성능을 동시에 향상시키는 것을 목표로 합니다. 제안된 방법은 PyTorch의 Fully Sharded Data Parallel(FSDP)와의 호환성이 크며, 다양한 모델 크기에 대해 활용할 수 있습니다.

- **Technical Details**: 제안된 적응형 배치 크기 스케줄은 데이터 병렬성과 모델 병렬성을 모두 지원하며 이론적으로 원리를 기반으로 설계되었습니다. 특히 Adam 최적화기를 사용하는 일반적인 부드러운 비볼록(Nonconvex) 목표를 위한 수렴 보장을 자립적으로 설정했습니다. 연구에서는 FSDP의 실용적인 구현을 통해 다양한 크기의 언어 모델을 사전 훈련할 수 있는 환경을 제공합니다.

- **Performance Highlights**: 실험 결과, 제안된 적응형 배치 크기 스케줄이 기존의 상수 배치 크기 및 휴리스틱 배치 크기 웜업 스케줄을 초월하여 더 나은 성능을 보여주었습니다. 특히, 최대 30억 개의 매개변수를 가진 Llama 계열 모델의 사전 훈련에 있어 두드러진 효과를 나타냈습니다. 적응형 배치 크기 스케줄의 이점은 훈련 안정성 및 일반화 성능의 개선에 기여한다고 강조됩니다.



### BridgePure: Revealing the Fragility of Black-box Data Protection (https://arxiv.org/abs/2412.21061)
Comments:
          26 pages,13 figures

- **What's New**: 이 논문에서는 데이터 소유자가 머신러닝 모델의 학습을 방해하는 방어 기술인 Availability attacks와 unlearnable examples의 새로운 연구 결과를 제시합니다. 이들은 사용자가 개인 데이터를 업로드하고 보호된 데이터로 변환할 수 있는 인기 있는 블랙박스 도구의 개발로 이어졌습니다. 하지만 이 연구에서 블랙박스 보호가 얼마나 쉽게 우회될 수 있는지를 보여주고 있습니다.

- **Technical Details**: 특히 연구자들은 공격자가 비보호 (unprotected) 인-디스트리뷰션 (in-distribution) 데이터의 소규모 집합만 있으면 블랙박스 보호를 우회할 수 있다고 주장합니다. 이 과정에서 (1) 비보호 데이터를 이용해 보호된 (protected) 데이터 쌍을 쉽게 획득하고, (2) diffusion bridge 모델을 훈련하여 매핑을 구축할 수 있다고 설명합니다. 이 매핑은 BridgePure라고 불리며, 동일 분포 내의 이전에 보지 못한 데이터에서 보호를 효과적으로 제거할 수 있습니다.

- **Performance Highlights**: 위협 모델 하에서, 이 방법은 분류 (classification) 및 스타일 모방 (style mimicry) 작업에서 우수한 정화 성능을 나타냅니다. 이는 블랙박스 데이터 보호의 중요한 취약점을 드러내어, 데이터 보안의 새로운 도전 과제를 제시합니다. 따라서 향후 연구는 이러한 발견을 바탕으로 추가적인 보호 기법을 개발해야 할 필요성을 가집니다.
...
### Learning Epidemiological Dynamics via the Finite Expression Method (https://arxiv.org/abs/2412.21049)
Comments:
          13 pages, 5 figures

- **What's New**: 이 논문에서 소개된 Finite Expression Method (FEX)는 감염병의 역학을 모델링하고 예측하는 데 있어 강력한 도구로, 기존의 평면적 모델링 방법과 비교해 해석 가능성을 높이고 예측 성능을 유지할 수 있는 혁신적인 접근 방식입니다. FEX는 강화 학습(reinforcement learning)을 기반으로 하여 수학적 표현을 명시적으로 도출할 수 있도록 구성되어, 전통적인 구획 모델에서 발생하는 복잡한 문제를 해결할 수 있는 잠재력을 가지고 있습니다.

- **Technical Details**: FEX는 고차원 문제를 다루기 위한 상징적 학습(symbolic learning) 프레임워크로, 미리 정의된 연산자 집합을 바탕으로 구성된 유한 수학적 표현을 탐색합니다. 이 프레임워크에서 각 연산자는 이진 트리(binary tree)로 표현되며, 훈련 가능한 계수와 편향 매개변수들을 포함하여 최적화 문제로 구성됩니다. FEX는 적합성을 평가하기 위한 점수(score) 계산을 통해 지배 방정식을 도출하는 과정에서 RL을 활용하여 전반적인 최적화 프로세스를 간소화합니다.

- **Performance Highlights**: 노이즈가 있는 가상 데이터 및 실제 COVID-19 데이터를 사용한 수치 실험을 통해 FEX는 전통적인 SIR, SEIR 및 SEIRD 모델을 포함한 다양한 전염병 모델링 방법과 경쟁하는 예측 성능을 보여주었습니다. FEX는 또한 의사 결정 과정에서 의도한 해석 가능성을 유지함으로써, 대중 건강 분야에서의 실제 응용 가능성을 넓히는 데 기여할 수 있는 잠재력을 입증했습니다.



### Mind the truncation gap: challenges of learning on dynamic graphs with recurrent architectures (https://arxiv.org/abs/2412.21046)
Comments:
          Published in Transactions on Machine Learning Research (TMLR)

- **What's New**: 이 논문은 동적 그래프(dynamic graphs)에서 발생하는 트렁케이션 갭(truncation gap)에 대해 새로운 통찰력을 제공합니다. 기존의 그래프 순환 신경망(graph recurrent neural networks, GRNNs)은 정보 전달을 단일 홉으로 제한하는 트렁케이션에 대한 영향을 충분히 검토하지 않았습니다. 저자들은 이러한 트렁케이션이 GRNN 모델의 학습 및 성능을 저하시킨다는 사실을 발견했습니다.

- **Technical Details**: 논문에서는 CTDG(continuous-time dynamic graphs)와 GRNNs에 대한 심층적인 분석을 수행합니다. GRNN은 RNN을 기반으로 하여 동적 그래프의 상태를 진화시키는 방법론으로, 기존 방법에 비해 사건 발생 시점을 고려하여 학습할 수 있는 장점이 있습니다. 그러나 T-BPTT(truncated backpropagation-through-time) 기법 사용 시 학습의 배치 간 정보 의존성을 간과하게 되어 성능이 저하됩니다.

- **Performance Highlights**: 저자들은 F-BPTT(full backpropagation-through-time)와 T-BPTT의 성능 차이를 여러 실제 동적 그래프 데이터세트를 통해 검증하였습니다. 특정한 합성 작업(synthetic task)에서 T-BPTT는 다중 홉 의존성을 학습하지 못하고, 이는 트렁케이션 갭으로 불립니다. 이 연구는 향후 GRNN 모델에 대한 더 나은 학습 접근 방식 개발의 필요성을 강조하고 있습니다.



### Machine Learning Optimal Ordering in Global Routing Problems in Semiconductors (https://arxiv.org/abs/2412.21035)
Comments:
          18 pages, 13 figures, 6 tables; published in Scientific Reports

- **What's New**: 본 연구에서는 다층 반도체 패키지 설계에서 발생하는 글로벌 라우팅 문제에 대해 새로운 망(ordering nets) 정렬 방법을 제안합니다. 제안된 방법은 기계 학습(machine learning) 기술에 기반하며, 기존의 휴리스틱(score functions) 기반 망 정렬 기법을 능가하는 성능을 보여줍니다. 다층 반도체 패키지 환경에서의 글로벌 라우팅 실험을 통해 우리의 방법이 휴리스틱 기반 방법보다 우수함을 입증합니다.

- **Technical Details**: 반도체 칩 및 패키지 설계에서의 라우팅은 다양한 컴포넌트 간 전기적 연결을 생성하는 과정입니다. 본 연구에서는 특정한 다층 라우팅 문제의 레이어 배정 문제에 집중하며, 이는 2차원 그리드로 단순화된 라우팅 문제에서 발생합니다. 제안된 방법은 2차원 라우팅 솔루션의 특징을 기계 학습하여 최적의 망 순서를 예측하고, 이를 통해 3차원 라우팅 솔루션의 품질을 개선합니다.

- **Performance Highlights**: 실험 결과에 따르면, 제안된 방법이 기존의 휴리스틱 기반 방법보다 더 나은 성과를 거두었음을 확인하였습니다. 글로벌 라우팅에서 망 정렬 단계의 개선을 통해 효과적으로 라우팅의 품질과 성능을 향상시킬 수 있음을 보여줍니다. 이는 다층 반도체 패키지 설계의 복잡성을 고려할 때 중요한 성과로 평가됩니다.



### Improving Location-based Thermal Emission Side-Channel Analysis Using Iterative Transfer Learning (https://arxiv.org/abs/2412.21030)
- **What's New**: 이 논문에서는 사이드 채널 공격(SCA)을 위한 딥러닝 모델에 반복적 전이 학습(iterative transfer learning)을 적용하는 방법을 제안합니다. 기존의 사이드 채널 공격 기법은 각 바이트(byte) 별로 모델을 훈련하는 데 반해, 이 방법은 서로 다른 바이트 간의 상관관계를 고려하여 효과적인 모델 훈련을 수행합니다. 이를 통해 데이터가 부족할 경우 성능을 향상시킬 수 있습니다.

- **Technical Details**: 본 연구에서는 AES-128 암호화의 계산 과정을 기반으로 한 데이터셋을 구성하였으며, 입력으로는 전력 소비 또는 열 맵을 사용합니다. 모델 학습에는 다층 퍼셉트론(MultiLayer Perceptron, MLP) 또는 합성곱 신경망(Convolutional Neural Network, CNN)을 활용하며, 데이터 전처리 및 특징 선택(feature selection) 과정이 포함됩니다. 특히 라플라시안 필터(Laplacian filter)를 사용하여 주요 특징들을 추출하고, 전이 학습을 통해 모델 성능을 개선하는 전략을 채택하였습니다.

- **Performance Highlights**: 실험 결과, 전이 학습을 통해 반복적으로 학습한 모델은 AES-128 암호화를 효과적으로 해독할 수 있었으며, 필요한 입력의 수를 최소화할 수 있었습니다. 또한, 데이터의 최소 조건에서 모델의 성능이 빠르게 수렴하는 것을 보여주어, 바이트 순서의 영향이 크지 않음을 입증하였습니다. 이 연구는 사이드 채널 공격 분야에서 머신러닝의 응용 가능성을 더욱 확장하는 결과를 제공합니다.



### EdgeRAG: Online-Indexed RAG for Edge Devices (https://arxiv.org/abs/2412.21023)
- **What's New**: EdgeRAG는 자원 제한이 있는 엣지 장치에서 Retrieval Augmented Generation (RAG)을 구현하는 데 있어 기념비적인 접근법입니다. 이 시스템은 클러스터 내에서 임베딩(embedding)을 가지치기(pruning)하고 검색 중에 필요시 임베딩을 생성하여 메모리 제약을 해결합니다. 특히 대량의 데이터를 검색하는 데 필요한 임베딩의 사전 생성 및 저장을 통해 지연시간(latency)을 줄이는 혁신적인 방법을 제공하고 있습니다.

- **Technical Details**: EdgeRAG는 모바일 플랫폼에서 메모리 용량을 효율적으로 활용하기 위해 두 가지 핵심 기법을 사용합니다. 첫째, 이 시스템은 두 번째 검색에만 사용되는 클러스터의 벡터 임베딩을 줄여 메모리 용량을 절약합니다. 둘째, 잔여 벡터 임베딩을 적응적으로 캐싱(caching)하여 중복 계산을 줄이고 전반적인 지연 시간을 개선합니다. 이 과정을 통해 EdgeRAG는 모바일 엣지 플랫폼의 기억 용량에 맞게 벡터 데이터베이스를 유지하면서도 높은 품질의 응답을 제공합니다.

- **Performance Highlights**: 실험 결과에 따르면 EdgeRAG는 평균적으로 IVF 인덱스(baseline)보다 1.8배 더 빠른 시간-첫-토큰(time-to-first-token, TTFT) 지연시간을 기록했습니다. 대형 데이터셋에 대해서는 3.82배의 성능 향상을 보여줍니다. 또한 EdgeRAG는 전체 평가 데이터셋이 메모리에 적합하도록 하여 메모리 쓰레싱(memory thrashing)을 방지하면서 기록 소환(recall) 및 생성 품질(generation quality)을 유지합니다.



### Text Classification: Neural Networks VS Machine Learning Models VS Pre-trained Models (https://arxiv.org/abs/2412.21022)
- **What's New**: 이 논문은 텍스트 분류(task) 수행을 위한 다양한 기법을 비교합니다. 특히, 사전 훈련된(pre-trained) 모델(BERT, DistilBERT 등)과 표준 신경망(MLP, RNN 등) 및 머신러닝 모델(Support Vector Machines, Random Forest 등)의 성능을 분석합니다. TF-IDF와 GloVe의 임베딩 기법도 함께 비교하여 GloVe가 성능에서 일관되게 우수함을 보여줍니다.

- **Technical Details**: 자연어 처리(NLP)에서 텍스트를 이해하고 생성하기 위해 임베딩(embedding) 기법이 필수적입니다. 이 논문에서는 TF-IDF와 GloVe 두 가지 임베딩 기법을 다룹니다. TF-IDF는 문서 내 단어의 중요성을 평가하는 통계적 방법이며, GloVe는 단어의 동시 출현 통계를 분석하여 의미론적 관계를 포착하는 비지도 학습 알고리즘입니다.

- **Performance Highlights**: 실험 결과, 사전 훈련된 모델들은 전통적인 모델보다 항상 우수한 성능을 발휘했습니다. GloVe 기반의 임베딩을 사용할 때 더욱 높은 정확도(accuracy)를 기록하는 경향이 있었습니다. 논문은 다양한 수준(level)의 카테고리에 대한 균형 잡힌 데이터셋에서 각 모델의 성능을 비교함으로써 결과의 신뢰성을 높였습니다.



### Weber-Fechner Law in Temporal Difference learning derived from Control as Inferenc (https://arxiv.org/abs/2412.21004)
Comments:
          36 pages 9 figures

- **What's New**: 이 논문에서는 강화 학습(Reinforcement Learning, RL)에서 비선형 업데이트 규칙을 탐구하고 있습니다. 기존의 TD 오류(Temporal Difference, TD error)는 업데이트의 정도에 선형적으로 비례하는 것으로 여겨졌지만, 최근 생물학적 연구에 따르면 TD 오류와 업데이트의 정도는 비선형적임을 보여주고 있습니다. 이는 학습 중 정책에 편향을 주며, 생물학적 학습에서 의도적으로 남겨두어진 유용한 특성으로 여겨집니다.

- **Technical Details**: 논문은 'Control as Inference' 프레임워크를 중심으로 비선형성을 수학적으로 탐구합니다. 비선형 업데이트 규칙이 필요하며, 이를 통해 만나는 'Weber-Fechner 법칙(Weber-Fechner Law, WFL)'에 대한 해석을 제공합니다. WFL은 자극 변화에 대한 반응으로 받아들여지는 업데이트의 정도가 자극 강도에 로그적으로 영향을 받는다는 것을 의미합니다.

- **Performance Highlights**: 제안된 RL 알고리즘은 실험과 로봇 실험을 통해 학습과정에서 보상을 극대화하고 처벌을 억제하는 기대 효과를 보입니다. 특히, 작은 보상에서의 학습 속도를 가속화하는 능력 덕분에 로봇이 효율적으로 작업을 수행할 수 있게 됩니다. 결론적으로, WFL은 RL에서 매우 유용하며, 생물체에서도 유사한 효용이 존재할 것으로 기대됩니다.
...
- **What's New**: 본 연구는 딥러닝(Deep Learning) 연산자의 검증된 리프팅(verified lifting)을 위한 새로운 프레임워크를 도입합니다. 이 프레임워크는 로우레벨(low-level) 구현에서 하이레벨(high-level) 수학적 수식을 합성(synthesize)하여 기능을 요약할 수 있는 도구의 필요성에 초점을 맞추고 있습니다. 이를 통해 개발자들은 자신이 정의한 연산자의 동작을 명확하게 이해하고 검증할 수 있는 기반을 제공합니다.

- **Technical Details**: 프레임워크는 세 가지 주요 단계로 구성되어 있습니다: 합성, 검증 및 단순화. 합성 단계에서는 프로그램을 심볼릭하게 실행하여 사양을 추출하고, 상향식 및 하향식 합성 기법을 사용하여 연산자의 동작을 요약하는 수학적 수식을 생성합니다. 검증 단계에서는 생성된 수식이 모든 유효 입력에 대해 기능이 올바른지확인하며, 단순화 단계에서는 산술 동등성(equality saturation)에 기반하여 수식을 자연스러운 수학적 형태로 복원합니다.

- **Performance Highlights**: 실험 결과, 본 프레임워크를 통해 33개의 깊이 있는 딥러닝 연산자 구현을 수집하였고, 32개의 연산자인증에 성공하였습니다. 또한, 우리의 접근 방식은 Tenspiler와 비교할 때 더 짧은 시간 비용으로 동작했으며, 합성과 검증의 성공률을 각각 88%와 75% 증가시켰음을 보여줍니다. 이러한 결과는 제안된 방법이 효과적임을 입증합니다.



### RobustBlack: Challenging Black-Box Adversarial Attacks on State-of-the-Art Defenses (https://arxiv.org/abs/2412.20987)
- **What's New**: 이 논문에서는 블랙박스 공격이 강력한 화이트박스 공격 방어 모델에 대해 얼마나 효과적인지를 평가하는 새로운 프레임워크를 제시합니다. 기존 연구들은 약한 방어 모델에서만 블랙박스 공격을 평가했으나, 우리는 최근의 강력한 방어 모델에 대한 공격의 효율성을 분석합니다. 그 결과, 간단한 적대적 훈련 기법조차도 블랙박스 공격의 효과성을 감소시키는 것으로 나타났습니다.

- **Technical Details**: 블랙박스 공격에서는 두 가지 주된 방식이 사용됩니다: (1) 전이 가능성 (transferability) 및 (2) 반복적인 쿼리 (iterative query)입니다. 이러한 방식은 대리 모델에서 생성된 적대적 예시를 사용하여 목표 모델을 공격하거나, 목표 모델로부터 쿼리 피드백을 통해 최적화를 진행합니다. 더불어, 대리 모델과 목표 모델 간의 일치성은 전이 기반 공격의 성공률에 중요한 역할을 하는 요소입니다.

- **Performance Highlights**: 실험 결과, 최근의 블랙박스 공격이 간단한 적대적 훈련 모델에 대해서도 성공하지 못하는 경향이 있음을 보여주었습니다. AutoAttack과 같은 강력한 화이트박스 공격 방어를 위해 최적화된 모델은 블랙박스 공격에 대해서도 뛰어난 강인성을 나타냅니다. 또한, 견고한 모델을 사용한 블랙박스 공격의 성공률이 평균 6.42% 증가하였으며, 이는 주요 방어 메커니즘과의 상호작용이 중요한 요소임을 뒷받침합니다.



### AlignAb: Pareto-Optimal Energy Alignment for Designing Nature-Like Antibodies (https://arxiv.org/abs/2412.20984)
Comments:
          30 pages

- **What's New**: 이 논문은 항체(sequence-structure) 디자인에 특화된 딥 러닝 모델을 훈련시키기 위한 3단계 프레임워크를 제안합니다. 먼저, 수백만 개의 항체 서열 데이터를 사용하여 언어 모델을 사전 훈련한 후, Diffusion Model을 사용하여 서열과 구조의 공동 최적화를 수행합니다. 마지막으로, Antigen binding site에 대해 낮은 반발력과 높은 끌림을 갖는 항체를 더욱 선호하도록 모델을 최적화하여 디자인의 합리성과 기능성을 향상시킵니다.

- **Technical Details**: 모델의 에너지 기반 정렬 목표를 통해 Pareto optimality로 나아가기 위해 AbDPO (Antibody Direct Preference Optimization)를 확장합니다. 또한, 온도 스케일링을 포함한 반복 학습 패러다임을 채택하여 추가 데이터 없이 다양한 온라인 데이터세트로부터 이익을 얻을 수 있도록 합니다. 이러한 방법들을 통해 낮은 에너지 격차를 가진 항체 디자인을 생성하는 데 성공합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법들이 기존 샘플들 및 이전 정렬 기술들보다 높은 안정성과 효율성을 보이며, 혁신적인 자연과 유사한 항체를 높은 결합 친화력으로 지속적으로 생성하는 데 우수한 성능을 나타냅니다. 특히, AlignAb 모델이 모든 에너지 측정에서 더 나은 성과를 내며, 타겟 항원에 대한 강한 결합 친화력을 보여주는 항체들을 생성하였다. 향후 성능 향상에 대한 가능성을 제시하며, AVE 및 RMSD와 같은 일반적인 메트릭 외에도 물리 기반의 측정이 필요함을 강조합니다.
...
### Generalizing in Net-Zero Microgrids: A Study with Federated PPO and TRPO (https://arxiv.org/abs/2412.20946)
Comments:
          Submitted to Environmental Data Science Journal from Cambridge University Press

- **What's New**: 본 연구는 마이크로그리드에서의 최적 에너지 관리 문제를 해결하기 위한 협력적이고 개인 정보 보호가 가능한 프레임워크를 제안합니다. 여기서 제안하는 FedTRPO 방법론은 Federated Learning (FL)과 Trust Region Policy Optimization (TRPO)를 통합하여 분산 에너지 자원(DERs)을 효율적으로 관리합니다. CityLearn 환경을 맞춤화하여 여러 건물로 구성된 마이크로그리드의 net-zero 에너지 시나리오를 시뮬레이션하였으며, 에너지 비용과 탄소 배출 감소를 강조합니다.

- **Technical Details**: 이 연구는 에너지 시스템에서 최적 제어 정책을 달성하기 위한 협력적 학습의 실현 가능성을 강조합니다. 특정 유형의 RL 알고리즘인 Trust Region Policy Optimization (TRPO)를 사용하여 기계 학습의 두 가지 일반적인 방법론인 RL과 federated learning (FL)을 기반으로 하는 새로운 FedTRPO 학습 프레임워크를 제안합니다. 우리의 실험적 결과는 FL의 일반화 능력과 개인화, 분할 학습을 통해 학습한 정책 최적성의 차이를 줄일 수 있는 방법을 보여줍니다.

- **Performance Highlights**: FedTRPO는 하이퍼파라미터 조정 없이 최첨단 federated RL 방법론과 비교 가능한 성능을 보여줍니다. 제시한 프레임워크는 기존의 RL 방법론에 대한 비교 실험을 통해 스마트 그리드 문제에서의 효과성을 증명하였으며, 에너지 효율 분야에서의 응용 가능성과 개인 정보 보호 요구 사항을 충족합니다. 연구의 결과는 협력적 에너지 관리에서의 RL의 유용성을 강화하고, 지속 가능한 스마트 그리드를 향한 진전을 보여줍니다.



### Rethinking Aleatoric and Epistemic Uncertainty (https://arxiv.org/abs/2412.20892)
Comments:
          Presented at the Workshop on Bayesian Decision-Making and Uncertainty (NeurIPS 2024)

- **What's New**: 이번 논문에서는 aleatoric(우연적) 및 epistemic(지식적) 불확실성에 대한 기존 논의에서의 일관성 부족을 지적하고, 이러한 개념들이 머신러닝 모델의 확률적 예측을 충분히 표현하지 못함을 설명합니다. 저자들은 다양한 모델 기반 불확실성과 데이터 생성 과정의 명확한 구분을 제안하여, aleatoric-epistemic 관점을 대체할 수 있는 방법을 제시합니다. 이러한 방법은 해당 분야의 담론을 보다 명확하게 할 것으로 기대됩니다.

- **Technical Details**: 논문은 aleatoric 및 epistemic 불확실성과 관련된 다양한 개념을 분석합니다. 특히, 입력 x와 출력 y의 조건부 예측 작업을 시작으로, 학습 데이터가 직접적으로 예측과 연관되지 않을 수 있음을 강조합니다. 저자들은 경험적 데이터 공간과 완전히 다른 공간에서 데이터가 생성될 수 있음을 설명하며, 파라메트릭 모델을 활용하여 학습 데이터를 기반으로 예측을 수행하는 방법을 논의합니다.

- **Performance Highlights**: 이 논문은 aleatoric-epistemic 관점의 한계를 극복할 새로운 불확실성 분해를 제안하여, 머신러닝의 보다 명확한 이해를 촉진할 수 있습니다. 기존 문헌에서 사용된 여러 개념들이 혼합되어 있음을 밝히며, 이는 향후 연구에서 문제를 해결하는 데 기여할 수 있는 기반이 될 것입니다. 저자들은 Bayesian 통계의 기초 개념으로 돌아가, 예측 불확실성을 보다 명확히 연관 지을 수 있는 길을 제시합니다.



### Isoperimetry is All We Need: Langevin Posterior Sampling for RL with Sublinear Regr (https://arxiv.org/abs/2412.20824)
- **What's New**: 이 논문은 강화 학습(Reinforcement Learning, RL) 분야에서 저자들이 보편적으로 사용할 수 있는 모델과 분포를 고려하여 하위 선형 후회(sublinear regret)를 달성할 수 있는 알고리즘을 설계하는 방법을 제안합니다. 특히 Log-Sobolev 불평등(Log-Sobolev Inequality, LSI)을 만족하는 지오메트리적 분포(isoperimetric distributions)에 중점을 두고 있습니다. 이는 기존의 알고리즘들이 설명하지 못했던 여러 비로그-오목(non-log-concave) 및 섭동된 분포에 대한 연구의 기초가 됩니다.

- **Technical Details**: 본 연구에서는 Posterior Sampling 기반의 강화 학습 알고리즘인 PSRL이 LSI를 만족하는 데이터 분포에서 하위 선형 후회를 달성할 수 있음을 보여줍니다. 데이터 분포가 정확한 사후 분포를 계산하거나 샘플링할 수 없을 때는 Langevin 샘플링 방법을 기반으로 한 LaPSRL 알고리즘을 제안하며, 이 알고리즘은 각 에피소드에 대해 최적의 후회(order optimal regret)를 달성하고, 서브제곱 복잡성(subquadratic complexity)을 가집니다. 실험에서 LaPSRL은 Langevin 샘플러인 SARAH-LD와 함께 배포되어 다양한 밴딧(bandit) 및 MDP 환경에서 테스트되었습니다.

- **Performance Highlights**: 실험 결과 LaPSRL 알고리즘은 여러 환경에서 일반성을 나타내며, 기준선들과 비교했을 때 경쟁력 있는 성능을 보이는 것으로 확인되었습니다. 이는 알고리즘이 다양한 분포와 모델에 대해 강력한 성능을 발휘할 수 있음을 시사합니다. 이러한 결과들은 RL의 응용 가능성을 넓히고, 실제 환경에서의 적용을 위한 새로운 가능성을 보여줍니다.



### TimeRAF: Retrieval-Augmented Foundation model for Zero-shot Time Series Forecasting (https://arxiv.org/abs/2412.20810)
- **What's New**: 이번 논문에서는 시간 시계열 예측의 효율성을 높이기 위한 새로운 모델인 TimeRAF를 소개합니다. TimeRAF는 Retrieval-Augmented Generation (RAG) 기법을 통해 제로샷(time series forecasting에서 특정 도메인 지식이 부족한 상황에서도 모델의 예측 능력을 크게 향상시킵니다. 모델은 맞춤형 시간 시계열 지식 기반을 사용하여 데이터 수집을 더욱 효율적으로 수행하며, Channel Prompting 기법을 통해 정보 통합이 효과적으로 이루어집니다.

- **Technical Details**: TimeRAF는 지식 기반으로부터 관련된 시간 시계열 데이터를 검색하는 retriever를 기반으로 하고 있습니다. 이 모델은 기존의 TSFM (Time Series Foundation Models) 구조를 유지하면서도 외부 지식을 통합하는 새로운 접근 방식을 적용하여 제로샷 예측 성능을 강화합니다. 모델은 end-to-end learnable retriever를 도입하여 검색된 데이터를 효과적으로 활용하고, Channel Prompting을 통해 지식을 결합한 후 보다 정밀한 예측을 수행할 수 있습니다.

- **Performance Highlights**: 실험 결과, TimeRAF는 다양한 도메인과 데이터 세트에서 기존의 TSFM 및 여러 제로샷 시간 시계열 예측 방법들과 비교할 때 현저한 성능 향상을 보였습니다. 논문에서는 또한 시간 시계열 예측에서의 참고 문헌 분석과 함께 광범위한 실험을 통해 हमारे 접근 방식의 유효성을 입증합니다. TimeRAF는 다양한 도메인에 걸쳐 예측의 정확성을 크게 개선하였음을 강조합니다.
...
### Accelerating Energy-Efficient Federated Learning in Cell-Free Networks with Adaptive Quantization (https://arxiv.org/abs/2412.20785)
- **What's New**: 이 논문에서는 Federated Learning (FL)의 통신 지연 문제를 해결하기 위해 Cell-Free Massive MIMO (CFmMIMO) 기술을 적용한 새로운 FL 프레임워크를 제안합니다. 특히, 클라이언트-서버 협업을 지원하기 위해 최적화된 업링크 전력 할당을 통해 에너지 효율성과 낮은 지연을 달성하는 방법을 다룹니다. 또한, 적응형 양자화 기법을 통해 로컬 그래디언트 업데이트를 위한 비트 할당을 동적으로 조정하여 통신 비용을 감소시키는 방식도 포함되어 있습니다.

- **Technical Details**: 제안된 프레임워크는 FL 모델 업데이트, 로컬 반복, 전력 할당을 포함하는 결합 최적화 문제를 수립하며, 이를 해결하기 위해 순차적으로 이차 프로그래밍(Sequential Quadratic Programming, SQP) 접근 방식을 사용합니다. 클라이언트는 로컬 FL 모델 업데이트를 위해 AdaDelta 방법을 이용하며, 이는 표준 SGD(Standard Gradient Descent)보다 더 나은 로컬 모델 수렴성을 제공합니다. 이와 함께, AdaDelta 로컬 업데이트와 관련된 FL 수렴에 대한 포괄적인 분석을 제공합니다.

- **Performance Highlights**: 실험 결과에 따르면, 동일한 에너지 및 지연 예산 내에서 제안된 전력 할당 방식은 Dinkelbach 및 max-sum rate 방법보다 테스트 정확도를 각각 최대 7% 및 19% 향상시킵니다. 또한, 세 가지 전력 할당 방법 중에서 제안된 양자화 기법은 AQUILA 및 LAQ보다 테스트 정확도를 각각 최대 36% 및 35% 증가시키며 그 유효성을 입증합니다.
...
### Joint Scoring Rules: Zero-Sum Competition Avoids Performative Prediction (https://arxiv.org/abs/2412.20732)
- **What's New**: 이 논문에서는 전문가 에이전트의 조건부 예측을 활용한 의사결정 과정에서의 이해 상충 문제를 해결하는 방법을 제시합니다. 에이전트가 예측 정확도를 최적화하면서 의사결정자의 선호도를 조작하려는 유인을 제거하기 위해 두 개 이상의 예측자를 공동으로 평가하는 접근 방식을 도입합니다. 이러한 경쟁적 접근을 통해 의사결정자는 자신의 진정한 선호도에 따라 최적의 행동을 선택할 수 있게 됩니다.

- **Technical Details**: 주요 기술적 결과는 예측자들이 제로섬 경쟁(zero-sum competition)에 참여할 때 올바른 조건부 예측을 이끌어낼 수 있다는 정리입니다. 이 접근법은 대칭 점수 규칙(symmetric scoring rule)을 이용하여 에이전트가 선택한 행동에 대한 영향을 제어합니다. 여러 조건부 예측을 통해 수행적 예측(performativity)의 문제를 피할 수 있으며, 이는 서로간에 신뢰할 수 있는 예측을 확보하는 데 도움이 됩니다.

- **Performance Highlights**: 실험에서는 제로섬 목표(zero-sum objective)로 훈련된 모델이 수행적 행동을 보이지 않음을 보여줍니다. 또한, 제로섬 훈련이 성과를 불러일으키는 환경에서도 이전에 학습된 조작적 행동을 효과적으로 제거함을 확인했습니다. 이러한 결과들은 조건부 예측을 안전하게 활용할 수 있는 가능성을 제시하며, 실제 의사결정에서의 유용성을 강화합니다.



### AverageLinear: Enhance Long-Term Time series forcasting with simple averaging (https://arxiv.org/abs/2412.20727)
- **What's New**: 본 연구에서는 AverageLinear이라는 새로운 모델을 소개합니다. 이 모델은 단순한 평균화(channel averaging) 및 파라미터 독립적인 접근을 사용하여 시계열 데이터의 채널 간 상관관계를 효과적으로 모델링할 수 있습니다. 실험 결과에 따르면, AverageLinear은 기존의 Transformer 기반 모델과 같은 성능을 내거나 그 이상을 나타내는 것으로 확인되었습니다.

- **Technical Details**: AverageLinear 모델은 입력 데이터의 채널을 임베딩하여 원본 데이터의 차원에 맞는 데이터를 생성합니다. 이 과정을 반복할 수 있으며, 이어서 모든 데이터는 완전히 파라미터 독립적인 선형 계층을 통해 예측됩니다. 채널 벡터에서 예측한 결과는 평균화(mean)되어 원본 데이터의 예측과 직접 더해집니다.

- **Performance Highlights**: 최신 Transformer 아키텍처와 비교했을 때, AverageLinear는 멀티채널 장기 시계열 예측에서 강력한 예측 능력을 보여줍니다. 실험에서는 단순한 선형 모델임에도 불구하고, 채널 상관관계를 효과적으로 추출하여 예측 성능을 향상시키는 것을 증명하였습니다. 이로 인해, 단순한 선형 구조 또한 모델에 강력한 예측력을 부여할 수 있음을 나타냅니다.



### Differentiable Convex Optimization Layers in Neural Architectures: Foundations and Perspectives (https://arxiv.org/abs/2412.20679)
- **What's New**: 이번 논문은 신경망 아키텍처 내에서 최적화 문제를 통합하는 새로운 접근 방식을 제시합니다. 이는 기존의 제약 조건 처리 방식을 기초로 하여, 최적화 계층(optimization layers)을 직접적으로 신경망 내에 포함시킴으로써 딥러닝의 제약 조건을 더 효과적으로 다룰 수 있도록 합니다. 특히, 이는 초기의 제한된 이차 최적화에서부터 최근의 일반적인 볼록 최적화 프레임워크에 이르기까지의 진화를 포괄적으로 조망합니다.

- **Technical Details**: 이 논문에서는 신경망 아키텍처 내에서 볼록 최적화 계층을 통해 하드 제약 조건(hard constraints)을 구현하는 방법에 대해 다룹니다. Amos와 Kolter(2021)의 초기 작업을 시작으로, 이 개념이 어떻게 발전해왔는지를 소개하며, 유효하고 효율적인 구현을 위한 수학적 원리와 계산 방법을 자세히 설명합니다. 이 계층은 네트워크의 파라미터를 사용하여 학습 가능한 최적화 문제를 구성하고, 그 해결책을 후속 계층에 전달합니다.

- **Performance Highlights**: 볼록 최적화 계층을 통한 신경망의 성능 향상은 여러 실제 적용 사례에서 입증되고 있습니다. 논문에서는 이러한 혼합 접근 방식의 잠재력을 보여주는 다양한 사용 사례를 다루며, 이들이 복잡한 제약 조건을 준수하면서도 구조적 문제를 해결하는 데 기여할 수 있음을 강조합니다. 궁극적으로, 최적화 이론과 딥러닝의 교차점에서 이러한 기술이 가져올 수 있는 현재의 능력과 미래 연구 방향을 제시합니다.



### Attention-Driven Metapath Encoding in Heterogeneous Graphs (https://arxiv.org/abs/2412.20678)
- **What's New**: 본 연구는 동질적 그래프에서 메타패스(metapath)라는 구조를 활용한 노드 분류(node classification)에서 주목(attention) 메커니즘을 도입하는 최초의 시도로, 중간 노드를 생략하지 않고 전체 메타패스를 인코딩하는 방법을 제안합니다. 저자들은 두 개의 새로운 인코더를 설계하여, 첫 번째 인코더는 순차적 주목을 활용하여 기존의 다중 홉 메시지 전달 알고리즘을 메타패스 환경으로 확장하고, 두 번째 인코더는 메타패스에서 의미 관계를 추출하기 위해 직접 주목을 사용합니다.

- **Technical Details**: 연구에서 제안된 인코더는 메타패스 인스턴스를 의미 있게 구현할 수 있도록 설계되었습니다. 특히, 모든 노드 기능을 메타패스에서 사용하여 다중 홉 환경에서 정보를 캡처하는 역할을 하며, 중간 노드를 포함시키는 새로운 방법을 통해 전체 구조를 개선했습니다. 메타패스 간 및 메타패스 내 집계 메커니즘을 활용하여 세밀한 의미 표현을 해석합니다.

- **Performance Highlights**: 제안된 모델은 IMDB 데이터셋에서 노드 분류를 수행하는 데 있어 최신 모델(State-of-the-Art)과 경쟁할 수 있는 성능을 입증하였습니다. 그 결과로, 연구는 동질적 그래프에서의 의미 구조를 캡처하기 위한 강력하고 일반적인 프레임워크를 제공하며, 모델이 복잡한 노드를 분류하도록 학습하는 데 최적화된 훈련 스케줄러를 사용합니다.
...
### Uncertainty Herding: One Active Learning Method for All Label Budgets (https://arxiv.org/abs/2412.20644)
- **What's New**: 이번 논문에서는 기존의 Active Learning 연구가 레이블 예산이 낮을 때 성능이 극단적으로 저하되는 문제를 다룹니다. 저자들은 Uncertainty Coverage라는 새로운 목표를 제안하여 저예산과 고예산 문제 모두에서 뛰어난 성능을 보이는 방법을 개발하였습니다. 이 방법은 Uncertainty Herding이라고 불리는 간단한 최적화 기법을 통해 구현되며, 계산적으로 빠르고 효율적입니다.

- **Technical Details**: Uncertainty Herding(UHerding)은 기존의 Low와 High Budget 간의 경계를 원활하게 연결할 수 있도록 설계된 기법입니다. UHerding은 불확실성(uncertainty) 측정을 포함하여 대표성 기반(representation-based) 및 불확실성 기반(uncertainty-based) 접근 방식 간의 자연스러운 혼합을 제공합니다. 실험 데이터를 통해 UHerding이 MaxHerding 및 기존의 다른 방법들을 초과하는 성능을 보여주었음을 입증하였습니다.

- **Performance Highlights**: 실험 결과, UHerding은 CIFAR-10, CIFAR-100, TinyImageNet, DomainNet, ImageNet와 같은 여러 벤치마크 데이터셋에서 저예산과 고예산 모두에서 기존의 최첨단(active learning state-of-the-art) 방법들을 초과하는 성능을 발휘했습니다. 이 방법은 특히 낮은 예산에서 효과적으로 작동하며, 모든 실험에서 일관된 성능 향상을 보여주었습니다. 뿐만 아니라, UHerding은 기존의 하이브리드(active learning hybrid) 방법들과도 밀접한 관련이 있음을 확인하였습니다.



### SafeSynthDP: Leveraging Large Language Models for Privacy-Preserving Synthetic Data Generation Using Differential Privacy (https://arxiv.org/abs/2412.20641)
Comments:
          15 pages, 1 figure, 5 tables

- **What's New**: 이 논문에서는 Differential Privacy (DP) 메커니즘을 통합한 Large Language Models (LLMs)을 활용하여 민감한 정보를 통한 직접적인 노출없이 데이터 기반 연구와 모델 훈련을 가능하게 하는 합성 데이터 생성을 연구하였습니다. 기존의 데이터 보호 법규를 준수하면서도 데이터 유용성을 유지하는 전략을 개발하는 것이 핵심 목표입니다. 실험 결과, DP를 통합한 LLM 기반의 합성 데이터 생성이 개인 정보 보호와 데이터 유용성 간의 적절한 균형을 제공한다는 점을 입증했습니다.

- **Technical Details**: 본 연구에서 제안하는 방법은 Laplace와 Gaussian 분포를 사용한 DP 기반의 노이즈 주입 방법을 데이터 생성 프로세스에 결합하여 합성 데이터를 생성합니다. 연구는 ML 모델이 합성 데이터로 훈련될 때의 성능과 원본 데이터로 훈련된 모델과 비교를 통해 DP가 강화된 합성 데이터의 유용성을 평가합니다. 또한, 생성된 합성 데이터의 개인 정보 보호 보장을 입증하기 위해 멤버십 추론 공격과 관련된 위협에 대한 저항성을 평가합니다.

- **Performance Highlights**: 실험 결과, LLM이 생성한 DP 강화 합성 데이터가 모델의 예측 정확도와 강건성을 얼마나 잘 유지하는지를 보여주고 있습니다. DP 노이즈 매개변수와 분포가 개인 정보 보호 보장과 데이터 유용성 간의 균형에 미치는 영향을 연구하며, 다양한 ML 아키텍처에서 DP 강화 합성 데이터가 지원되는 가능성을 제시합니다. 이 연구는 따라서 개인 정보 보호를 보장하면서도 ML의 실질적인 발전에 기여할 수 있는 방법론을 제공합니다.
...
### Audiopedia: Audio QA with Knowledg (https://arxiv.org/abs/2412.20619)
Comments:
          Accepted to ICASSP 2025

- **What's New**: 이 논문에서는 Audiopedia라는 새로운 작업을 소개합니다. Audiopedia는 Audio Question Answering with Knowledge로, 오디오 이해와 외부 지식 추론이 모두 필요한 과제입니다. 기존의 파라미터로 간단한 질문에 대해서만 답변하는 Audio Question Answering (AQA) 벤치마크와는 달리, Audiopedia는 지식 집약적인 질문에 초점을 맞추고 있습니다. 이 연구는 고급 오디오 이해를 다루는 첫 번째 작업으로, 새로운 연구 방향을 제시합니다.

- **Technical Details**: Audiopedia는 세 가지 하위 과제로 구성되어 있습니다: (i) Single Audio Question Answering (s-AQA), (ii) Multi-Audio Question Answering (m-AQA), (iii) Retrieval-Augmented Audio Question Answering (r-AQA)입니다. 이들 하위 과제는 각각 단일 오디오 샘플, 여러 오디오 샘플 및 관련 오디오를 검색하여 질문에 답변하는 것을 요구합니다. 저자들은 또한 Audio Entity Linking (AEL)과 Knowledge-Augmented Audio Large Multimodal Model (KA2LM)이라는 두 가지 구성 요소를 포함하는 일반 프레임워크를 제안합니다.

- **Performance Highlights**: 세 가지 Audiopedia 하위 과제를 통해 대규모 오디오 언어 모델 (LALMs)의 성능을 평가한 결과, 본 연구에서 제안한 프레임워크를 통해 성능이 향상된 것을 확인하였습니다. 이 연구는 GAMA, Audio-flamingo 및 LTU-AS와 같은 최근 LALMs에서 우수한 성능을 보여주었습니다. 이러한 성과는 고급 오디오 질문 답변 기술의 발전에 기여할 것으로 기대됩니다.



### Converting Time Series Data to Numeric Representations Using Alphabetic Mapping and k-mer strategy (https://arxiv.org/abs/2412.20617)
- **What's New**: 이 논문에서는 시간 시계열 데이터를 생물학적 서열에 유사한 방식으로 표현하는 새로운 접근 방법을 제안합니다. 이러한 변환을 통해 생물정보학에서 개발된 k-mers 기반의 분석 기법을 활용하여 비선형 패턴과 관계를 더 잘 파악할 수 있습니다. 특히, 이러한 방법은 산업 모니터링 및 예방적 유지보수에 유용할 수 있습니다.

- **Technical Details**: 제안된 방법은 시간 시계열 신호를 26개의 범위로 나누어 각 범위에 해당하는 영어 알파벳으로 매핑하는 독특한 알파벳 매핑 기법을 사용합니다. 이 변환은 시간 시계열의 시계열적 순서와 상대 크기를 보존하며, 이후 k-mers 기반의 분석 알고리즘을 사용해 분석됩니다. 알고리즘은 원래 신호의 각 값을 특정 범위에 매핑하여 문자 시퀀스를 생성합니다.

- **Performance Highlights**: 실험을 통해 실제 데이터 세트를 문자 시퀀스로 변환하고 시퀀스 분류를 수행한 결과, 제안된 접근 방식이 시계열 데이터의 표현을 단순화하고 분류 정확도가 향상됨을 보여주었습니다. 특히, 생물정보학에서 효과적으로 사용되는 시퀀스 분석 기법을 활용하여 숨겨진 패턴을 발견할 수 있습니다.



### Hilbert Curve Based Molecular Sequence Analysis (https://arxiv.org/abs/2412.20616)
- **What's New**: 본 논문에서는 생물학적 분자 시퀀스를 이미지 표현으로 변환하는 새로운 방법인 Universal Hilbert curve 기반의 Chaos Game Representation (CGR)을 제안합니다. 이 방법은 분자 시퀀스에서 Hilbert curve 기반의 이미지 표현을 구축하기 위한 독창적인 Alphabetic index mapping 기술을 포함합니다. 제안된 방법은 다양한 생물학적 데이터에 적용할 수 있으며, 깊은 학습 모델의 성능을 극대화합니다.

- **Technical Details**: Hilbert curve는 1차원 생물학적 시퀀스를 2차원 평면으로 매핑하는 연속적인 프랙탈 공간 채우기 곡선입니다. 이 방식은 시퀀스 요소 간의 근접성을 유지하는 데 유리합니다. 수학적으로, p차의 Hilbert curve는 재귀적으로 정의되며, p가 증가할수록 생성되는 이미지 크기가 기하급수적으로 증가하여 더욱 복잡한 데이터를 효과적으로 표현할 수 있습니다.

- **Performance Highlights**: 제안된 CGR 방법은 폐암 데이터셋을 사용한 CNN 모델에서 현재 최첨단 방법들을 초과하여 94.5%의 높은 정확도와 93.9%의 F1 점수를 달성했습니다. 이는 이미지 분류 방법을 이용한 분자 시퀀스 분석의 새로운 가능성을 열어줍니다. 또한, 제안된 방법론은 NLP 분야의 다른 시퀀스 데이터셋에도 적용 가능하다는 점에서 폭넓은 영향을 미칩니다.
...
### Edge of Stochastic Stability: Revisiting the Edge of Stability for SGD (https://arxiv.org/abs/2412.20553)
Comments:
          28 pages, 24 figures

- **What's New**: 본 논문은 미니배치 확률적 경량 하강법(Gradient Descent, SGD)의 학습 메커니즘을 새로운 관점에서 분석한 것으로, Edge of Stochastic Stability(EoSS)라는 새로운 개념을 도입합니다. 이는 기존의 전체 배치(Hessians, MiniBS)와는 다른 영역에서 최적화되며, 미니배치 손실의 최고 고유값의 평균에 기반하였습니다. 이 연구를 통해 SGD가 훈련하는 동안의 '날카로움(sharpness)'의 변화를 정량화하고, 훈련 파라미터(learning rate, batch size)가 이 과정에 미치는 영향을 분석하였습니다.

- **Technical Details**: SGD의 학습 과정에서 미니배치 손실에 대한 미니배치 날카로움(MiniBS)을 정의합니다. MiniBS는 미니배치 손실의 헤시안(Hessian) 최대 고유값들의 평균으로, 이는 전체 배치 날카로움(FullBS)과 비교하여 항상 큰 값을 가집니다. 연구에서는 MiniBS가 2/η 근처에서 안정화되는 EoSS라는 새로운 구간을 발견하였고, 이는 미니배치의 배치 크기(batch size)와 학습률(learning rate)과 밀접한 관련이 있습니다. 이로 인해 SGD의 훈련 현상에 대한 통찰을 제공하고, MiniBS와 FullBS 간의 간극을 정량화하여 각기 다른 배치 크기에서의 Plateau 현상을 명확히 하였습니다.

- **Performance Highlights**: 경험적으로 미니배치에서의 날카로움과 관련된 특성을 재확인하였으며, 전체 배치 날카로움(FullBS)이 낮은 고유값에서 Plateau를 이루는 현상을 관찰하였습니다. MiniBS와 FullBS 간의 간극은 배치 크기에 따라 대략적으로 (batch size)^{-0.7} 비율로 감소하는 경향이 있음을 확인하였고, 이는 실험적으로 통계적 의미를 가지는 수치입니다. 이러한 결과는 SGD를 사용하는 다양한 딥러닝 모델에 적용될 수 있으며, 학습 과정의 안정성과 일반화 능력(GGA)을 높이는 데 기여할 수 있습니다.



### Diminishing Return of Value Expansion Methods (https://arxiv.org/abs/2412.20537)
Comments:
          arXiv admin note: substantial text overlap with arXiv:2303.03955

- **What's New**: 이 논문은 모델 기반 강화 학습에서 동력학 모델(dynamics model)의 향상이 샘플 효율(sample efficiency)에 미치는 영향을 실증적으로 조사합니다. 연구 결과, 오라클 동력학 모델을 사용하여 누적 오류(compounding errors)를 제거했을 때, 샘플 효율성이 향상되었으나 이러한 개선이 매우 제한적이라는 점을 발견했습니다. 특히, 모델 정확도(model accuracy)의 증가가 샘플 효율성을 크게 개선하지 않는다는 주장을 통해 모델 기반 가치 확장(value expansion) 방법의 한계를 재조명합니다.

- **Technical Details**: 이 연구는 모델 기반 가치 확장 방법에서 동력학 모델의 품질이 샘플 효율성에 미치는 한계를 알아봅니다. 실험적 접근법을 통해, 오라클 동력학 모델을 사용해도 완벽한 모델이 사용될 경우에도 샘플 효율성의 실질적인 향상이 없음을 보여주었습니다. 또한, 롤아웃 호라이즌(rollout horizon)을 늘릴 때 샘플 효율성을 개선하기 위한 이점이 점점 감소하는 현상을 발견하였습니다.

- **Performance Highlights**: 모델 없는 방법인 retrace와 비교했을 때, 모델 기반 가치 확장 방법은 비슷한 성능을 보이면서도 계산 오버헤드(computational overhead)를 도입하지 않아 강력한 기준선이 됨을 제시합니다. 또한, 비판적 확장(critic expansion)에 있어, 학습된 모델을 사용할 만큼의 이점이 계산 복잡도를 정당화하지 않음을 보여주었습니다. 이러한 결과는 다양한 환경에 걸쳐 통계적 유의성을 강화하며, 보여주는 성과에 대한 통찰을 제공합니다.
...
### Treatment Effect Estimation for Graph-Structured Targets (https://arxiv.org/abs/2412.20436)
- **What's New**: 이 연구는 그래프 구조의 대상에서 치료 효과 추정을 위한 새로운 프레임워크인 GraphTEE(그래프 대상 치료 효과 추정)를 제안합니다. 기존의 연구들이 개별 타겟에서 치료 효과를 추정하는 데 집중한 반면, 이 논문은 그래프 구조의 관계를 갖는 집단 타겟에 대한 치료 효과를 이해하는 필요성에 주목합니다. GraphTEE는 관찰적 편향(observational bias)을 완화하는 방안으로, 새로운 정규화 프레임워크를 고려하여 치료 효과를 추정합니다.

- **Technical Details**: GraphTEE는 치료 할당에 있어 그래프 내 특정 노드의 영향으로 발생할 수 있는 관찰적 편향을 해결하기 위해 두 가지 주요 단계로 구성됩니다. 첫째, 입력 그래프를 편향 완화를 위한 필수 노드와 비필수 노드로 분해하는 함수를 구축합니다. 둘째, 이론적 분석을 기반으로 효율적인 편향 완화 규제를 적용하여 보다 효과적인 치료 효과 추정을 목표로 합니다.

- **Performance Highlights**: 합성 및 반합성 데이터 세트를 사용한 실험 결과, GraphTEE는 기존의 전 그래프 노드를 사용하는 방법보다 효율적으로 편향을 완화할 수 있음을 입증했습니다. 이 연구는 그래프 구조의 대상에 대한 치료 효과 추정을 다룬 최초의 연구로, 기존 방법들의 비효율적인 편향 완화 문제를 해결하기 위한 새로운 접근 방식을 제시합니다. 이로 인해 GraphTEE의 효과성이 명확하게 드러났습니다.



### Impact of Data Distribution on Fairness Guarantees in Equitable Deep Learning (https://arxiv.org/abs/2412.20377)
- **What's New**: 이 논문은 데이터 분포와 공정성 보장(fairness guarantees)의 관계를 분석하는 포괄적인 이론적 프레임워크를 제시합니다. 데이터 분포의 이질성을 고려하여, 다양한 인구 집단 간의 예상 손실 차이를 최소화하는 형식적 분석 프레임워크를 도입했습니다. 이론적 경계(theoretical bounds)와 수렴 속도(convergence rates)를 도출하여, 공정성과 정확성 간의 기본적인 균형(trade-off)에 대한 이해를 심화시켰습니다.

- **Technical Details**: 본 연구는 다양한 인구 집단 간의 데이터 분포 이질성을 명문화하여 새로운 분석 경계를 도출하는 것을 목표로 합니다. 공정성 오류 경계(fairness error bounds) 및 알고리즘 복잡성(algorithmic complexity)과 같은 여러 정리(theorem)를 통해 다양한 데이터 분포와 손실 함수에 대한 가정 하에 학습 모델의 공정성 특성을 조명합니다. 이러한 결과는 깊은 학습 모델의 공정성 속성에 영향을 미치는 주요 요소를 이해하는 데 기여합니다.

- **Performance Highlights**: 실험 결과, 데이터 분포 차이가 인구 집단 간의 모델 공정성에 상당한 영향을 미친다는 것이 입증되었습니다. 특히 인종 카테고리에서 성능의 차이가 두드러지며, 이론적 경계가 이러한 경험적 관찰을 뒷받침하고 있습니다. 본 연구는 AI 기반 진단 시스템의 공정성에 대한 이해를 발전시키고, 보다 공정한 알고리즘을 개발하기 위한 이론적 기초를 제공합니다.



### Scalable Bayesian Optimization via Focalized Sparse Gaussian Processes (https://arxiv.org/abs/2412.20375)
Comments:
          Accepted by NeurIPS 2024

- **What's New**: 이 연구는 희소(Gaussian Process) GP를 활용한 베이esian 최적화(Bayesian Optimization)의 효율성을 강화하는 방법을 제안합니다. 새로운 변별적 손실 함수를 이용한 희소 GP(focalized GP)와 점진적으로 작은 탐색 공간에서의 최적화를 위한 FocalBO를 소개합니다. FocalBO는 robotic morphology design와 585차원 근골격계 제어 문제에서 뛰어난 성능을 보여줍니다.

- **Technical Details**: 제안된 focalized GP는 변별적 손실 함수를 통해 지역적 예측 능력을 향상시킵니다. 이 모델은 제한된 표현 능력으로도 지역 기능 구조에 집중하여 높은 차원의 문제를 최적화할 수 있도록 설계되었습니다. FocalBO는 탐색 공간의 다양한 규모에서 획득 함수(acquisition function)를 최적화하는 계층적 알고리즘으로, 대규모 오프라인 데이터와 온라인 데이터를 효과적으로 활용합니다.

- **Performance Highlights**: FocalBO는 heterogeneous 함수의 최적화 및 대규모 오프라인 데이터 활용에서 기존의 획득 함수보다 뛰어난 성능을 보였습니다. 실험 결과, FocalBO는 500개 이상의 변수를 포함한 근골격계 시스템 제어 문제를 효과적으로 최적화하며, 대규모 오프라인 데이터와 온라인 데이터를 활용해 베이esian 최적화의 성능을 큰 폭으로 향상시킵니다.
...
### Safe Bayesian Optimization for the Control of High-Dimensional Embodied Systems (https://arxiv.org/abs/2412.20350)
Comments:
          Accepted by CoRL 2024

- **What's New**: 이 논문에서는 고차원 안전 베이지안 최적화(HdSafeBO)라는 새로운 접근 방식을 제안합니다. 이는 확률적 안전 제약하에서 고차원 샘플링 문제를 처리하기 위한 방법론으로, 지역 낙관적 탐색(local optimistic exploration)을 도입하여 목표 함수를 효율적이고 안전하게 최적화합니다. 또한, HdSafeBO는 근본적으로 인간 움직임 제어에 사용되는 신경 자극을 안전하게 최적화하는 데 성공적으로 적용되었습니다.

- **Technical Details**: HdSafeBO는 고차원 공간에서의 제어 최적화 문제를 다루며, 특히 몇 백에서 수 천 차원에 이르는 파라미터 공간을 처리할 수 있는 알고리즘입니다. 이 방법은 단순한 최적화 전략을 넘어, 전반적인 안전성 보장(probabilistic safety guarantee)과 누적 안전 위반 한계(cumulative safety violation bound) 개념을 포함합니다. 이ometric embedding 기법을 이용하여 고차원 문제의 차원 축소를 통해 안전 보장을 유지하면서도 많은 차원의 문제를 해결할 수 있도록 합니다.

- **Performance Highlights**: HdSafeBO는 그러나 기존의 다른 방법들과는 달리 고차원 근육골격계 시스템을 성공적으로 제어하는 능력을 보였습니다. 실제 실험에서도 HdSafeBO는 신경 자극 유도 움직임 제어를 안전하게 최적화하는 데 효과적임을 입증하였습니다. 이 알고리즘은 고차원 안전 최적화의 새로운 가능성을 열어주며, 브레인 로봇 시스템 등 다양한 응용 분야에서 활용될 수 있을 것으로 기대됩니다.



### Asynchronous Federated Clustering with Unknown Number of Clusters (https://arxiv.org/abs/2412.20341)
Comments:
          Accepted by AAAI 2025

- **What's New**: 이 논문은 비동기적으로 통신하는 클라이언트에서 최적의 클러스터 수를 학습할 수 있는 비동기 연합 클러스터 학습(Asynchronous Federated Cluster Learning, AFCL) 방법을 제안합니다. 기존의 방법들은 클라이언트가 학습해야 할 클러스터의 수를 미리 알고 있다고 가정합니다. 그러나 이 연구에서는 클라이언트 간의 비동기적 소통과 올바른 클러스터 수에 대한 정보가 부족한 상황을 함께 해결하고자 합니다. 이러한 혁신은 연합학습(FL)의 강건성과 범용성을 높이는 데 기여합니다.

- **Technical Details**: AFCL 방법은 클라이언트마다 일정한 수의 seed points를 생성하고, 각 클라이언트의 클러스터링 정보를 수집하여 이를 서버와 공유합니다. 클라이언트는 주변 객체의 분포 정보를 기반으로 seed points를 업데이트하며, 이 과정에서 커뮤니케이션 모드가 다양한 클라이언트의 기여도를 균형 있게 조정하는 메커니즘도 포함됩니다. 이후 클라이언트 간의 정보 교환을 통해 더 나은 클러스터링 결과를 도출합니다.

- **Performance Highlights**: 다양한 실험 결과를 통해 AFCL의 유효성을 입증하였으며, 제공된 지원이 서로 다른 클라이언트의 비동기 참여로 인한 편향성을 줄이는데 효과적임을 보여주었습니다. AFCL은 클러스터 수를 자동으로 수렴시킬 수 있는 능력을 보이며, 비동기 연합 시나리오에서 적절한 수의 클러스터를 학습할 수 있습니다. 이러한 성과는 실제 애플리케이션에서 FC의 적용 가능성을 더욱 넓힐 것입니다.
...
### An experimental study on fairness-aware machine learning for credit scoring problem (https://arxiv.org/abs/2412.20298)
Comments:
          The manuscript is submitted to Springer Nature's journal

- **What's New**: 본 논문에서는 신용 평가에서 공정성( fairness)을 고려한 머신러닝( machine learning) 모델에 대한 실험적 연구를 제공합니다. 기존의 연구들은 공정성을 다루었으나, 신용 평가와 관련하여 충분한 조사가 이루어지지 않았습니다. 따라서 이 연구는 다양한 재무 데이터셋을 분석하고, 공정성을 측정하는 방안을 제시하며, 공정성을 고려한 예측 모델들을 평가하는 데 중점을 두고 있습니다.

- **Technical Details**: 신용 평가 문제는 이진 분류 문제로 모델링되며, 각 데이터셋은 클래스 속성을 기반으로 구성됩니다. 공정성 관련 모델은 크게 사전 처리( pre-processing), 처리 중( in-processing), 사후 처리( post-processing)로 분류되며, 각 접근법은 무작위 샘플링과 데이터 분석 기법을 활용하여 이루어집니다. 논문은 Bayesian networks를 포함한 다양한 기법을 사용하여 재무 데이터셋을 분석하고, 다른 공정성 인식 분류 모델의 성능을 평가합니다.

- **Performance Highlights**: 기존 연구들과 달리, 본 논문은 공정성을 고려한 머신러닝 기술의 포괄적인 평가를 통해 신용 평가에서의 성능과 공정성 간의 균형을 탐구합니다. 실험 결과는 공정한 결정에 대한 필요성이 강조되며, 이는 대출 결정에 적용 가능한 새로운 평가 척도를 제공합니다. 이 연구는 또한 공정성 메트릭 선택의 복잡성을 설명하고, 여러 공정성 메트릭을 사용하여 신용 평가 문제를 명확하게 분석하는 기초 자료를 제공합니다.



### An analytic theory of creativity in convolutional diffusion models (https://arxiv.org/abs/2412.20292)
- **What's New**: 이 논문은 합성곱 확산 모델(convolutional diffusion models)에서 창의성의 첫 번째 분석 가능하고 해석 가능한 예측 이론을 제시합니다. 특히, score-based diffusion 모델이 학습 데이터로부터 멀리 떨어진 매우 창의적인 이미지들을 생성할 수 있다는 점을 강조합니다. 기존 최적 점수 맞춤(score-matching) 이론과의 괴리를 해소하기 위해, 저자들은 지역성(locality)과 동치성(equivariance)이라는 두 가지 간단한 귀납적 편향을 식별했습니다.

- **Technical Details**: 저자들은 ELS(Equivariant Local Score) 기계 모델을 통해 창의적인 이미지를 생성하는 메커니즘을 제시하며, 이는 특정 학습 과정 없이도 학습된 합성곱 확산 모델의 출력을 높은 정확도로 예측할 수 있습니다. ELS 모델은 CIFAR10, FashionMNIST, MNIST 데이터셋에서 각각의 결과에 대해 중간 $r^2$값이 0.90, 0.91, 0.94에 달하는 예측 정확도를 보입니다. 이러한 성과는 지역적으로 일관된 패치 모자이크를 통해 이루어지며, 이를 통해 새로운 이미지를 생성하는 방법을 설명합니다.

- **Performance Highlights**: 모델은 ELS 기계를 통해 생성된 샘플이 어떻게 지역적으로 일관된 패치 조합을 통해 기하급수적인 창의성을 달성하는지를 보여줍니다. 또한, 훈련된 UNets와 ResNets의 출력을 개별적으로 예측할 수 있는 능력을 갖추고 있으며, 이론과 실험 간의 비교 결과는 훈련된 확산 모델이 시간에 따라 공간적 구조를 조정하는 과정을 잘 설명합니다. 논문은 ELS 이론이 비국소적인 자기 주의(self-attention) 레이어가 포함된 더 강력한 모델과 어떻게 비교되는지 또한 다룹니다.



### Causal Discovery on Dependent Binary Data (https://arxiv.org/abs/2412.20289)
- **What's New**: 이번 연구는 종속적인 이진 데이터에서 인과 그래프 학습을 위한 탈상관 기반 접근 방식을 제안합니다. 기존의 인과 그래프 세우기 방법이 가정하는 독립성 가정이 현실 데이터와 충돌하는 문제를 해결하기 위해, 의존성이 있는 단위를 고려한 새로운 모델을 개발했습니다. 이 모델은 잠재적인 효용 모델(latent utility model)을 활용하여 단위 간의 오류를 설명합니다.

- **Technical Details**: 연구에서는 쌍(pairwise) 최대 우도(maximum likelihood) 방법을 이용해 단위 간의 공분산 행렬(covariance matrix)을 추정합니다. 추정된 공분산 행렬을 바탕으로 EM 알고리즘을 통하여 독립적인 대체 데이터(surrogate data)를 생성하고, 이를 활용하여 인과 그래프를 학습하는 방법을 제시합니다. 이로 인해 다양한 표준 인과 발견(causal discovery) 방법이 적용 가능합니다.

- **Performance Highlights**: 실험 결과, 제안된 탈상관 접근 방식이 기존 데이터보다 인과 그래프 학습의 정확성을 크게 향상시킨다고 밝혔습니다. 합성 데이터(synthetic data)와 실제 데이터(real-world data)를 모두 사용하여 실험이 이루어졌고, 최신 기술을 활용하여 얻은 결과들이 더 높은 정확도를 나타냈습니다.



### TeLU Activation Function for Fast and Stable Deep Learning (https://arxiv.org/abs/2412.20269)
Comments:
          Updated version of "Stable and Robust Deep Learning By Hyperbolic Tangent Exponential Linear Unit (TeLU)"

- **What's New**: 이번 논문은 하이퍼볼릭 탄젠트 지수 선형 유닛(Hyperbolic Tangent Exponential Linear Unit, TeLU)이라는 새로운 신경망 활성화 함수를 제안합니다. TeLU는 비선형성을 도입하면서도 수렴 속도를 높이고, 학습 안정성을 개선하며, 기울기 소실 문제를 효과적으로 완화하는 방식으로 설계되었습니다. 이 함수는 ReLU의 장점을 보존하면서 Smoothness와 Curvature의 이점을 도입하여 기존의 활성화 함수들을 대체할 수 있는 조건을 갖추고 있습니다.

- **Technical Details**: TeLU(x) = x * tanh(exp(x))라는 간단한 수식으로 정의되며, 이는 신경망의 학습 과정에서 발생하는 다양한 문제들을 해결하는 데 중점을 둡니다. 이 함수는 활성화 영역에서 항등 함수에 밀접하게 근사하여 강력한 수렴을 달성하며, 포화 영역에서 기울기 소실 문제를 무시할 수 있도록 돕습니다. 실험적으로 TeLU는 ResNet18, Dynamic-Pooling Transformers, 순환 신경망(Recurrent Neural Networks) 등 다양한 구조에서 테스트되었으며, 이론적 분석을 통해 그 잠재력을 입증하였습니다.

- **Performance Highlights**: TeLU는 기존의 활성화 함수들 대비 학습 효율성을 높이고, 모델의 최종 정확도를 향상시킵니다. 이 함수의 도입은 신경망 학습 과정에서 더 강력하고 안정적인 성능을 제공할 수 있으며, 특히 깊고 복잡한 네트워크에서 두드러진 효과를 거두었습니다. TeLU는 다양한 아키텍처와 초기화 방법 및 옵티마이저에서 학습 안정성을 극대화하여 깊은 학습 분야의 기초 연구 및 실용적 응용에서 큰 도움이 될 것으로 기대됩니다.



### Towards Ideal Temporal Graph Neural Networks: Evaluations and Conclusions after 10,000 GPU Hours (https://arxiv.org/abs/2412.20256)
- **What's New**: 이 논문에서는 Temporal Graph Neural Networks (TGNNs)의 복잡한 설계 공간을 탐구하고, 기존 연구들이 이 공간을 충분히 탐구하지 못해 최적의 설계로 이어지지 않았음을 지적합니다. 연구진은 TGNN 모델을 성능 중심으로 보는 접근 방식이 각 기술의 장단점을 깊이 이해하는 데 방해가 된다고 주장합니다.

- **Technical Details**: 연구진은 TGNN 모듈을 기준으로 한 통합된 최적화 코드 구현을 바탕으로 설계 공간 검색을 수행하는 실용적인 비교 평가 프레임워크를 제안하였습니다. 이 프레임워크를 통해 TGNN 설계에서 10,000 GPU 시간 이상을 투자하여 효율성, 데이터셋 패턴과 모듈 효과성의 상관관계, 여러 모듈 간의 상호작용을 분석하였습니다.

- **Performance Highlights**: 주요 연구 결과로는 최근 이웃 샘플링(Neighbor Sampling) 및 어텐션 집계기(Attention Aggregator)가 균일 이웃 샘플링과 MLP-Mixer 집계기보다 우수하다는 것을 보여주었습니다. 또한, 정적 노드 메모리가 효과적인 대안이 될 수 있으며, 데이터셋의 반복 패턴에 따라 정적 또는 동적 노드 메모리 선택이 달라져야 함을 보여주었습니다.
...
### IMSSA: Deploying modern state-space models on memristive in-memory compute hardwar (https://arxiv.org/abs/2412.20215)
Comments:
          5 pages, 4 figures, submitted to IEEE ISCAS 2025

- **What's New**: 최근 S4(D) 모델은 긴 시퀀스 처리에서 메모리 요구 사항을 크게 줄이기 위해 정량화 인식을 통한 훈련을 도입했습니다. 이 모델은 메모리 제한이 있는 엣지 컴퓨팅 하드웨어에서도 효율적으로 작동할 수 있도록 최적화되었습니다. 이를 통해, S4D 커널이 메모리 내 컴퓨팅 하드웨어에서 처음으로 구현되었습니다.

- **Technical Details**: S4(D) 모델 내에서는 HiPPO 커널이 핵심 역할을 하며, 이 커널은 입력 신호를 높은 차원 상태로 프로젝션합니다. 이 상태는 프로젝션 매트릭스를 통해 주기적으로 업데이트되며, 출력은 상태와 벡터의 곱셈으로 산출됩니다. 추가적으로, 메모리 제약 하에서 VMM(vector-matrix multiplication)을 수행하기 위한 메모리 채널 배열이 활용됩니다.

- **Performance Highlights**: S4D 모델은 실세계의 간단한 작업에서 이진 분류 성능을 확인하였으며, 특히 음성 인식 작업에서 'zero'와 'one'을 구분하는 데 성공했습니다. 이러한 성능은 소음이 있는 계산 기판 상에서도 효과적으로 실행될 수 있도록 정량화 기술이 중요한 역할을 함을 시사합니다.



### Generative Regression Based Watch Time Prediction for Video Recommendation: Model and Performanc (https://arxiv.org/abs/2412.20211)
Comments:
          10 pages, 5 figures, conference or other essential info

- **What's New**: 본 논문에서는 단기 비디오 추천 시스템에서 사용자 관심을 포착하기 위한 Watch Time Prediction (WTP) 작업의 새로운 Generative Regression (GR) 패러다임을 제안합니다. 기존의 회귀 방법들은 스칼라 값으로부터 직접적인 예측을 하기 때문에 데이터의 비대칭성이나 오류에 취약했으나, GR은 시퀀스 생성을 통해 이를 해결할 수 있는 잠재력을 지니고 있습니다. GR은 예측을 토큰 시퀀스로 변환하여 원래 값을 손실 없이 재구성하고 예측의 정확성을 유지합니다.

- **Technical Details**: GR 모델은 각 시계열 점을 예측하는 과정에서 발생하는 조건부 종속성을 활용합니다. 기존의 방법들이 고정된 시간 간격을 기반으로 한 이진 분류 문제로 회귀 문제를 나누었다면, GR은 각 예측 단계에서 특정 시간 슬롯을 선택할 수 있는 유연성을 제공합니다. 이를 위해 데이터 기반의 통합 어휘 구성 방법을 제안하여 토큰 불균형 문제를 완화하고, 손실 없는 레이블 인코딩 전략을 통해 watch time 값을 복원할 수 있도록 합니다.

- **Performance Highlights**: GR 모델은 네 개의 공개 데이터셋과 하나의 산업 데이터셋에서 기존의 최첨단 모델들을 상회하는 성능을 보였습니다. 또한, 4억 명의 일일 활성 사용자(DAU)를 가진 Kuaishou에서 온라인 A/B 테스트를 수행하여 실제 효용성을 입증하였습니다. GR은 추천 시스템의 또 다른 회귀 작업인 Lifetime Value (LTV) 예측에도 성공적으로 적용되어, 일반적인 회귀 문제에 대한 효과적인 솔루션으로서의 가능성을 부각시켰습니다.
...
### Accurate Coresets for Latent Variable Models and Regularized Regression (https://arxiv.org/abs/2412.20189)
- **What's New**: 새로운 연구에서는 기계 학습에서의 정확한 coreset을 구축하기 위해 통합된 프레임워크를 제안합니다. 이 프레임워크는 다양한 문제에 대해 정확한 coreset을 구성하는 알고리즘을 포함하고 있으며, 특히 잠재 변수가 있는 모델 및 ℓ_p 정규화 회귀에 대해 상세히 다룹니다. 연구 결과, 우리의 알고리즘은 코어셋 크기가 모델 복잡도 감소에 따라 작아지며, 이는 전반적으로 더 작은 데이터 셋으로 훈련하면서도 비슷한 정확도를 유지할 수 있음을 보여줍니다.

- **Technical Details**: 제안하는 알고리즘들에서는 Kernelization(커널화)이라는 개념을 도입하여 입력 데이터 포인트를 높은 차원으로 매핑합니다. 이를 통해 각 쿼리의 전체 데이터에 대한 손실을 유지하면서 이는 정확한 coreset의 크기를 줄이는 데 기여합니다. 특히, Ridge 회귀를 위한 알고리즘에서는 정규화 매개변수에 따라 코어셋 크기가 통계적 차원에 의존하게 되어, 특정 조건에서 d²보다 작아지는 결과를 도출합니다.

- **Performance Highlights**: 실제 데이터셋에 대한 광범위한 실험을 통해, 제안한 접근 방식이 이론적으로 예측한 결과를 지지함을 입증했습니다. 특히, 잠재 변수 모델에 대한 coreset 크기가 전체 데이터의 1% 미만으로 줄어들어 훈련 시간을 상당히 단축할 수 있음을 보여주었습니다. 이러한 성과는 여러 기계 학습 문제에 효과적인 해결책을 제공할 것으로 기대됩니다.



### Pushing the Envelope of Low-Bit LLM via Dynamic Error Compensation (https://arxiv.org/abs/2412.20185)
- **What's New**: 최근 대형 언어 모델(LLM)의 양자화와 관련된 기술들이 발전하고 있으며, 특히 저사양 하드웨어에서의 활용이 주목받고 있습니다. 그러나 낮은 정밀도의 양자화(quantization) 설정은 모델의 품질 저하를 초래할 수 있습니다. 본 논문에서 제안하는 QDEC는 이러한 문제를 해결하기 위해 실시간으로 두드러진 채널을 식별하고, 이들에 대한 오차 보정을 통해 품질을 개선하는 방법을 제시합니다.

- **Technical Details**: QDEC는 잔차 행렬(residual matrix)을 CPU 메모리에 저장하고, 동적으로 식별된 두드러진 채널에 대해서만 해당 잔차를 가져오는 방식으로 작동합니다. 이를 통해 양자화에 따른 오차를 보상하며, GPU 메모리 사용량을 최소화하고 추론 지연(latency)을 줄입니다. QDEC는 각 디코딩 단계에서 입력 활성화를 분석하여 두드러진 채널을 동적으로 파악합니다.

- **Performance Highlights**: QDEC는 기존의 최첨단 양자화 방법들을 보강하며, 3비트 Llama-3-8B-Instruct 모델의 perplexity를 10.15에서 9.12로 감소시키는 성과를 보였습니다. 추가적인 GPU 메모리 사용량은 0.0003% 미만으로 유지되었고, NVIDIA RTX 4050 Mobile GPU에서 추론 속도는 오직 1.7%만 감소했습니다. 논문에서 제안된 기법은 여러 소비자 등급 GPU에서 중요한 품질 개선을 보이며 평가되었습니다.
...
### A Greedy Strategy for Graph Cu (https://arxiv.org/abs/2412.20035)
- **What's New**: 본 논문에서는 그래프 컷(Graph Cut) 문제를 해결하기 위한 Greedy 전략인 GGC(Greedy Graph Cut)을 제안합니다. GGC는 각 데이터 샘플이 클러스터로 간주되는 상태에서 시작하여 글로벌 목표 함수(global objective function)의 값을 가장 많이 줄이는 두 클러스터를 동적으로 병합합니다. 이 과정은 원하는 클러스터 수에 도달할 때까지 반복되며, 목표 함수 값의 단조성(monotonicity)이 입증됩니다.

- **Technical Details**: GGC는 클러스터와 그 이웃 간의 병합만을 고려하여 계산 복잡성을 줄입니다. 이로 인해 GGC는 샘플 수에 비례하여 거의 선형(linear) 계산 복잡성을 가진 알고리즘으로, 다른 알고리즘과 달리 Greedy 전략 때문에 제안된 알고리즘의 해는 유일성을 가집니다. 또한, 본 논문에서는 GGC를 널리 사용되는 그래프 컷 문제인 정규화 컷(normalized cut) 문제에 적용하였습니다.

- **Performance Highlights**: 실험 결과에 따르면, GGC는 전통적인 두 단계 최적화 알고리즘(고유 분해(eigendecomposition) + k-means)보다 정규화 컷 문제에서 더 나은 성능을 보이는 경우가 많습니다. GGC는 최신 상태의 클러스터링 알고리즘에 비해서도 성능상 장점을 가지고 있습니다.
...
### A Nearly Optimal Single Loop Algorithm for Stochastic Bilevel Optimization under Unbounded Smoothness (https://arxiv.org/abs/2412.20017)
Comments:
          ICML 2024

- **What's New**: 이 논문은 비선형이며 잠재적으로 경계가 없는 smoothness를 가진 상위 수준 함수와 강하게 볼록한 하위 수준 함수의 확률적 이중 최적화 문제를 다룹니다. 새로운 알고리즘인 SLIP(Single Loop bIlevel oPtimizer)를 제안하여, 기존의 복잡한 중첩 루프 디자인에 비해 단일 루프 구조로 효율성을 개선했습니다. 이 알고리즘은 상위 수준 변수와 하위 수준 변수를 동시에 업데이트하여 수렴을 보장하는 새로운 접근 방식을 보여줍니다.

- **Technical Details**: SLIP 알고리즘은 몇 단계의 확률적 경량 하강법(SGD)을 통해 하위 수준 변수를 업데이트한 후, Normalized stochastic gradient descent와 모멘텀을 사용하여 상위 수준 변수를 업데이트합니다. 이 과정은 하위 변수의 정확한 추정 없이 안정적인 수렴을 보장하도록 설계되었습니다. 저자들은 하위 수준 변수의 정교한 특성과 제어 방식을 개발하여 기존 연구에서 요구하는 복잡함을 피할 수 있었습니다.

- **Performance Highlights**: 다양한 실험을 통해 SLIP 알고리즘이 이중 최적화 문제에서 강력한 기준선과 비교하여 상당한 성과를 보이는 것을 확인했습니다. SLIP 알고리즘은 하이퍼 표현 학습 및 데이터 하이퍼 클리닝과 같은 작업에서 효율적인 성능을 발휘하였습니다. 최종적으로 이 알고리즘은 $	ilde{O}(1/	ext{ϵ}^4)$의 복잡도로 기대 값과 높은 확률 하에 수렴점을 찾을 수 있음을 보여주었습니다.
...
### Discrete Curvature Graph Information Bottleneck (https://arxiv.org/abs/2412.19993)
Comments:
          Accepted by the Main Technical Track of the 39th Annual AAAI Conference on Artificial Intelligence (AAAI-2025)

- **What's New**: 본 논문에서는 Graph Neural Networks(GNNs)에서의 정보 전송 구조를 최적화하기 위해 Discrete Curvature Graph Information Bottleneck(CurvGIB) 프레임워크를 제안합니다. 기존 연구들이 그래프 구조의 배열이나 휴리스틱한 가정에 의존했다면, CurvGIB는 기하학적 시각에서 최적의 정보 전송 구조를 추구합니다. 이 프레임워크는 정보 이론과 그래프 기하학의 통합된 관점을 제공하여 GNN의 효율적 메시지 전송 구조를 탐색합니다.

- **Technical Details**: CurvGIB는 Ricci curvature 최적화를 통해 정보 전송 패턴을 학습하며, 전반적인 GNN 성능을 개선하는 최적 구조를 학습합니다. 이를 위해 VIB(Variational Information Bottleneck) 원리를 확장하고, Ricci flow와 결합하여 Ricci curvature의 계산 복잡성을 줄이는 접근법을 사용합니다. 이 과정에서 그래프의 로컬 연결 패턴을 더 잘 반영할 수 있는 그래프 커버처를 활용하여, 효율적이고 안정적인 학습을 위한 구조를 제공합니다.

- **Performance Highlights**: 다양한 데이터셋에서 수행된 실험 결과, CurvGIB는 기존의 방법들에 비해 뛰어난 성능과 해석 가능성을 보여주었습니다. 특히, 노드 분류(nod classification) 작업에서 최적의 효과를 발휘하며, GNN의 학습 및 성능 향상을 동시에 지원하는 새로운 구조를 정의합니다. CurvGIB는 정보 전송과 관련된 최적의 그래프 구조를 효과적으로 마이닝할 수 있는 기반을 마련하였습니다.



### A Robust Federated Learning Framework for Undependable Devices at Sca (https://arxiv.org/abs/2412.19991)
- **What's New**: 이 논문에서는 FLUDE라는 새로운 연합 학습(FL) 프레임워크를 제안합니다. FLUDE는 비신뢰 환경에서 장치의 신뢰성을 평가하고, 적절한 장치만을 선택하여 모델 훈련의 효율성과 성능을 향상시키고자 합니다. 또한, 각 장치에 모델 캐시를 유지하여 비신뢰 장치에서의 훈련 중단 문제를 해결합니다.

- **Technical Details**: FLUDE는 장치의 역사적 행동의 확률 분포를 기반으로 신뢰성을 평가하고, 높은 신뢰성을 가진 장치들을 적응적으로 선택합니다. 이를 통해 장치의 참여 빈도가 비대칭적으로 변할 때 발생하는 모델의 편향을 완화합니다. 추가적으로 FLUDE는 모델의 신뢰도와 최신성 간의 균형을 고려하여 전 세계 모델을 배포하는 새로운 전략을 제안합니다.

- **Performance Highlights**: FLUDE가 구현된 실험에서는 모델 정확도가 기존 방법에 비해 2.28%에서 7.43% 향상되었고, 시간 대비 정확도는 1.2배에서 3.2배 개선되었습니다. 또한, 통신 비용은 23.71%에서 40.71% 절감되었습니다. 이 결과들은 FLUDE가 비신뢰 환경에서도 모델 훈련 성능과 자원 효율성을 극대화 할 수 있음을 보여줍니다.



### Caesar: A Low-deviation Compression Approach for Efficient Federated Learning (https://arxiv.org/abs/2412.19989)
Comments:
          12 pages, 27 figures

- **What's New**: 이번 연구에서는 Caesar라는 새로운 연합 학습(FL) 프레임워크를 제안하고, 이를 통해 모델 정확성과 통신 비용 간의 균형을 맞추는 저편차 압축 접근 방식을 구현하였습니다. 기존의 압축 기반 FL 시스템에서는 데이터의 이질성과 모델 구식 문제로 인해 성능 저하가 심각했으나, Caesar는 이를 효과적으로 해결할 수 있는 방법을 제시합니다. Caesar는 디바이스의 로컬 모델의 노후화를 고려하여 압축 비율을 최적화하여 초기 모델의 정확성을 보장합니다.

- **Technical Details**: Caesar 프레임워크는 서버 측에서 각 디바이스의 로컬 모델 노후화에 기반하여 글로벌 모델의 압축 비율을 선택합니다. 또한, 로컬 그래디언트 압축은 각 디바이스의 데이터 특성에 의해 가이드되며, 고품질 데이터를 가진 디바이스는 작은 압축 비율을 사용하여 중요한 그래디언트의 편차를 줄입니다. 이 과정에서 압축 비율 결정에 있어 기계적 과제가 남아 있으며, 다양한 디바이스의 주파수에 따라 로컬 모델과 데이터 특성이 상이함을 감안해야 합니다.

- **Performance Highlights**: Caesar는 40대의 OPPO 스마트폰과 80대의 NVIDIA Jetson 장치에서 구현되었습니다. 실험 결과에 따르면, Caesar는 동일한 목표 정확도로 기존 압축 기반 기준 모델에 비해 약 25.54%에서 37.88%의 통신 비용을 줄일 수 있었으며, 최종 테스트 정확도는 0.68%만 저하됩니다. 이러한 성과는 FL 시스템에서 압축 전략의 효과적인 구현이 가능함을 보여주며, 대규모 통신 오버헤드를 줄이는 데 기여할 것으로 기대됩니다.
...
### Explainable Semantic Federated Learning Enabled Industrial Edge Network for Fire Surveillanc (https://arxiv.org/abs/2412.19979)
Comments:
          9 pages

- **What's New**: 이 논문에서는 산업용 사물인터넷(IIoT) 장치들이 대량의 모니터링 데이터를 자주 전송함으로써 발생하는 스펙트럼 자원의 소모를 해결하기 위해 산업 엣지 의미 네트워크(IESN)를 제안합니다. 제안된 시스템은 의미 통신(SC)을 통해 IIoT 장치들이 경고를 발송할 수 있도록 하며, 데이터 프라이버시 및 보안, 이종 장치에 대한 SC 모델 적응, 의미의 설명 가능성을 중점적으로 다룹니다. 이를 통해 전송된 정보량을 줄이고 통신 효율성을 개선할 수 있습니다.

- **Technical Details**: IESN 아키텍처는 SC를 통해 IIoT 장치와 엣지 서버(ES) 간의 통신을 수행합니다. SC 모델은 eXplainable Semantic Federated Learning (XSFL) 방법론을 통해 분산 학습을 가능하게 하여 산업 데이터의 프라이버시와 보안을 보호합니다. Adaptive Client Training (ACT) 전략을 통해 IIoT 장치의 지역 자원에 따라 개인화된 모델이 제공되어 이종 장치의 적응성이 향상됩니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 XSFL이 효과적으로 작동하며 IIoT 환경에서의 fire surveillance (화재 감시)에 있어 통신 오버헤드를 크게 줄이고, 데이터 전송의 정확성을 높임을 입증하였습니다. ESC(Explainable SC) 메커니즘은 중요한 의미 기능을 로컬라이즈하여 모니터링 데이터와의 상관관계를 명확히 설명할 수 있도록 도와줍니다. 이러한 개선은 화재 감시에 있어서 IIoT 시스템의 신뢰성 및 효율성을 크게 향상시킵니다.
...
- **What's New**: 이 연구는 공구 마모 예측을 위한 데이터 기반 접근 방식을 채택하여, 단일 가속도 센서를 사용하는 저비용 데이터 생성 방법을 개발했습니다. 이를 통해 다수의 센서를 사용하는 전통적인 방법의 한계를 극복하고자 하며, 다양한 공정에 대한 모델 범용성을 검증합니다. 특히, ConvNeXt 모델이 고성능을 보이며 4개의 밀링 공구 데이터를 활용해 99.1%의 정확도를 달성했습니다.

- **Technical Details**: 연구에서는 데이터 수집을 위해 단일 가속도 센서를 사용하여 다양한 기계 및 공정에 쉽게 통합할 수 있는 접근 방식을 활용합니다. 여러 머신러닝 모델, 특히 CNN, LSTM, SVM 및 결정 트리를 평가하며, 각 모델은 훈련 데이터 양이나 형식에 따라 성능이 다르게 나타날 수 있습니다. 예측 모델의 일반화 능력을 검증하기 위해 두 가지 공정에서 테스트를 수행하였습니다.

- **Performance Highlights**: 모델 성능 평가에서는 제한된 데이터 조건 하에서도 효과적인 공구 마모 예측이 가능함을 보여줍니다. 검토된 여러 머신러닝 아키텍처 중에서 ConvNeXt 모델이 가장 높은 정확도를 기록하였으며, 다른 모델들보다 두 배 이상의 성능 향상을 보였습니다. 이 연구는 공구 마모 예측의 발전에 기여하고, 예측 유지 관리 전략의 개선 가능성을 제시합니다.
...
### On the Convergence of DP-SGD with Adaptive Clipping (https://arxiv.org/abs/2412.19916)
- **What's New**: 이 논문은 quantile clipping (QC-SGD)와 함께 사용하는 Stochastic Gradient Descent (SGD)의 수렴 분석을 처음으로 제공한다. 기존의 상수 임계값 클리핑이 가진 문제점들을 극복하고자 적응형 클리핑 접근 방법을 제안하며, 이의 이론적 기반을 다지는 데 중점을 두었다. 연구 결과는 여러 파라미터 선택이 수렴 정황에 끼치는 영향을 규명하고 있다.

- **Technical Details**: QC-SGD는 일반적인 상수 임계값 기반 클리핑이 가지는 편향 문제를 보여준다. 이 논문은 quantile과 스텝 사이즈의 조절을 통해 이러한 편향을 완화할 수 있는 방법을 제시하며, 수렴 행동에 대한 중요한 관계를ศึกษ한다. 또한, differential privacy (DP) 환경에서의 최적화 문제에 대한 이론적 보장을 설정하는 첫 번째 연구로 자리잡았다.

- **Performance Highlights**: 본 연구는 adaptive clipping의 유용성과 그 이론적 기반을 보여줌으로써 향후 연구 방향에 대한 중요한 통찰을 제공한다. 이는 differential privacy를 보장하면서도 성능 최적화에 기여할 수 있는 방법론으로 기대된다. 이 논문의 발견은 클리핑 기법에 대한 기존의 이해를 확장하고 이를 통해 다양한 문제 해결에 기여할 수 있음을 시사한다.



### Minimax-Optimal Multi-Agent Robust Reinforcement Learning (https://arxiv.org/abs/2412.19873)
- **What's New**: 이번 논문에서는 multi-agent robust reinforcement learning (MARL) 및 multi-player robust Markov games (RMGs)의 샘플 복잡도(sample complexity)를 개선하기 위한 새로운 알고리즘을 제안합니다. 이 알고리즘은 확률적 모델을 사용하는 제한된 환경에서 에이전트들이 최악의 경우 누적 보상을 최대화하는 방법을 탐구합니다. 기존 연구에서 발견된 세 가지 주요 장애물을 해결하여, 정보 이론적 하한을 초과하지 않는 성능을 제공합니다.

- **Technical Details**: 제안된 Q-FTRL 알고리즘은 유한한 수평 설정에서 작동하며, 생성 모델(generative model)에 대한 접근이 있는 경우에 적용됩니다. 논문에서는 이 알고리즘이 특정한 조건 하에 상관 균형(코어 코릴라티드 균형, CCE)을 달성하며, 샘플 복잡도가 $	ilde{O}(H^3S 	imes 	ext{sum}_{i=1}^{m}A_i	ext{min}egin{Bmatrix}H \ 1/R	ext{end{Bmatrix}}/	ext{ε}^2)$로 알려졌습니다. 해당 결과는 정보 이론적 하한과 결합하여 minimax 최적(minimax optimal)임을 증명합니다.

- **Performance Highlights**: 제안된 알고리즘은 두 에이전트가 있는 제로섬 게임에서 Robust Nash Equilibrium (NE)을 달성할 수 있으며, 동일한 샘플 복잡도를 유지합니다. 연구 결과는 MARL의 복잡한 환경에서의 안정성과 효율성을 향상시킬 수 있는 가능성을 제시하며, 실제 애플리케이션에서의 데이터 제한 문제를 해결할 수 있습니다. 더 나아가, 이 접근 방식은 환경 불확실성이 존재하는 다중 에이전트 시스템의 다양한 실용적 사용 사례에 적합합니다.



### Reduced Order Models and Conditional Expectation (https://arxiv.org/abs/2412.19836)
Comments:
          24 pages, 2 appendices

- **What's New**: 이 논문은 파라미터 의존 시스템에 대해 논의하며, 이러한 시스템이 수치 시뮬레이션에서 자주 발생한다는 점을 강조합니다. 저자들은 파라미터 감소 모델(reduced order model)을 공급하는 새로운 방법론을 제시하며, 이는 특정 파라미터의 값을 기반으로 하여 더 낮은 차원으로 프로젝션을 수행합니다. 특히, 기계 학습에서의 최적화 기법과의 관련성에 초점을 두어, 조건부 기대값(conditional expectation)과 베이지안 업데이트(Bayesian updating) 간의 연결 또한 다룹니다.

- **Technical Details**: 이 연구에서는 파라미터 의존 문제를 해결하기 위해, 선형 매핑을 통해 모델링하는 접근 방식을 사용합니다. 일반적으로 파라미터에 대한 의존성을 가진 수학적 모델을 구성하고, 여기서 고충실도 모델(high fidelity model)이란 실제 시스템의 수학적 표현을 나타냅니다. 저자들은 이러한 모델을 낮은 차원의 매니폴드(manifold)로 투영(projection)하는 과정을 통해 가장 적합한 근사치를 추구하며, 이를 통해 필터링과 같은 과정을 소개합니다.

- **Performance Highlights**: 결과적으로, 저자들은 ROM과 기계 학습의 통합을 통해 조건부 기대값을 이해하고, 베이지안 업데이트와의 관계를 강화하는 새로운 관점을 제시합니다. 연구에서 제안된 방법론은 다양한 파라미터에 대해 고속으로 작동하며, 불확실성을 고려할 수 있는 가능성을 제공합니다. 이러한 접근법은 데이터 동화(data assimilation)와 저차원 모델 간의 상호 작용을 통해 빠르고 정확한 예측을 가능하게 합니다.
...
### SoS Certificates for Sparse Singular Values and Their Applications: Robust Statistics, Subspace Distortion, and Mor (https://arxiv.org/abs/2412.21203)
- **What's New**: 이 논문에서는 무작위 직사각형 행렬에 대한 새로운 	extit{sparse singular value certificates} (희소 특이값 인증서)를 연구합니다. 저자들은 독립적인 Gaussian 항목을 가진 $n \times d$ 행렬 $M$에 대해, 주어진 $\eta \in (0,1)$를 사용하여 최대값 $\,\|Mu\|$의 상한을 증명할 수 있는 다항 시간 알고리즘의 새로운 가족을 제시합니다. 이 알고리즘은 알고리즘 통계 및 이론적 컴퓨터 과학의 광범위한 문제에 기초가 됩니다.

- **Technical Details**: 연구의 중심은 $\,\|Mu\|$의 상한을 증명하는 것입니다. 특히 저자들은 기존의 최악의 경우의 어려운 결과에도 불구하고, 희소 주성분 분석(sparse PCA), 무작위 아랫공간의 왜곡 계산, 이상치에 강한 평균 및 공분산 추정을 위한 새로운 알고리즘을 포함하여, 다양한 알고리즘적 작업에 대한 효율적인 알고리즘을 도출합니다. 저자들은 또한 Efron-Stein 분해와 의존 함수의 그래프 매트릭스 접근 방식을 결합하여 알고리즘의 정확성을 증명하는 새로운 조합적 연결을 개발했습니다.

- **Performance Highlights**: 제안된 알고리즘은 알고리즘의 다양한 설정에서 기존의 알고리즘적 보증보다 더 나은 성능을 제공하며, 특히 희소 주성분 분석 및 $\,\ell_1/\ell_2$ 왜곡 인증에 대한 새로운 다항식 시간 보증을 제공합니다. 이러한 알고리즘들은 기존의 경우보다 더 넓은 $n, d, $ 및 $\eta$의 범위를 다루며, 저자들이 정립한 SQ 및 저차 다항식 하한과 근접한 성과를 보여줍니다.
...
### Sparse chaos in cortical circuits (https://arxiv.org/abs/2412.21188)
- **What's New**: 본 연구는 신경 세포의 행동과 집단 혼돈(collective chaos)간의 관계를 탐구하며, 신경 충동 생성 메커니즘이 회로의 제어 가능성과 정보 흐름을 강화하는 방식으로 조정된다는 새로운 통찰을 제공한다. 이를 위해 신경 충동 발생의 기본 특성이 신경 회로의 집단 혼돈에 미치는 영향을 분석하는 컴퓨터 기반의 비가역 이론을 사용하였다. 연구 결과, 신경세포 내의 신경 충동 생성의 변화가 정보 인코딩률에 미치는 중간 정도의 영향 외에, 위상 공간 구조를 정성적으로 변화시킨다는 것을 입증했다.

- **Technical Details**: 연구진은 행동 잠재력의 시작 속도(onset rapidness)가 대뇌 회로의 집단 동역학에 미치는 영향을 분석하기 위해 변형 가능한 행동 잠재력 시작 속도를 갖는 새로운 신경 세포 모델인 rapid theta neuron model을 구축하였다. 이 모델은 전통적인 theta neuron model의 장점을 결합하여, 행동 잠재력 발생 속도의 변화에 따라 신경 세포의 불안정한 고정점에서의 동역학을 효율적으로 계산할 수 있도록 설계되었다. 이를 통해 기억 및 학습을 위한 뇌의 집단 동역학을 과학적으로 분석하고, 이들이 어떻게 신경 충동의 생성 메커니즘에 최적화되어 있는지를 조명하였다.

- **Performance Highlights**: 신경 회로에서의 집단 혼돈은 특정 임계점을 넘어서면서 희박한 혼돈(sparse chaos)으로 나타나며, 이는 지속적인 안정적인 동역학과 짧은 강한 혼돈의 폭발이 교차하는 현상을 보여준다. 대규모의 실제 구조화된 네트워크에서의 분석을 통해 이러한 결과의 일반성을 지지하며, 이는 대뇌 회로에 내재된 생물물리학적 특성이 이 희박한 혼돈에 맞춰 조정되어 있음을 나타낸다. 연구 결과는 단일 신경 세포의 기본 생물물리학과 대뇌 회로의 집단 동역학 간의 밀접한 연결고리를 드러내며, 두 구성 요소 간의 상관관계를 강조한다.
...
### DeepF-fNet: a physics-informed neural network for vibration isolation optimization (https://arxiv.org/abs/2412.21132)
- **What's New**: 이번 연구에서는 DeepF-fNet이라는 새로운 신경망 프레임워크를 제안하여, 진동 기반 구조 최적화에서 전통적인 활성 시스템을 대체할 수 있도록 설계되었습니다. DeepONets의 개념을 통합한 DeepF-fNet은 데이터와 물리 법칙을 결합하여 고주파수에서의 주요 진동을 효과적으로 억제하는 최적의 매개변수를 신속하게 식별할 수 있는 기능을 제공합니다. 이를 통해 DeepF-fNet은 자동차 소음, 진동 및 하중과 같은 고성능 환경에서 요구되는 실시간 진동 억제의 필요성을 충족할 수 있는 가능성을 보이고 있습니다.

- **Technical Details**: DeepF-fNet 프레임워크는 SICE4 배치 알고리즘과 이중 네트워크 물리 지식 신경망(PINN)을 통합하여 특정 불필요한 주파수를 목표로 최적화된 구조 매개변수를 추정하도록 설계되었습니다. 이 알고리즘은 사용자 정의 주파수를 필터링하기 위한 메타재료의 최적 구조 매개변수를 도출하는 네 가지 주요 단계로 구성됩니다. 특히, 진동 대역갭과 목표 주파수의 복잡한 관계를 학습하기 위해 여러 dispersion curves를 초기화하고 반복적으로 수정하는 과정을 포함합니다.

- **Performance Highlights**: DeepF-fNet은 전통적인 유전 알고리즘(GA)보다 계산 속도 면에서 뛰어난 성능을 발휘하며, 비교 가능한 결과를 제공함으로써 진동 민감한 응용 프로그램에 대한 유망한 도구로 자리잡고 있습니다. 연구 결과는 DeepF-fNet이 기존 방법에 비해 구조 최적화를 보다 효율적이고 비용 효과적인 방식으로 수행할 수 있는 가능성을 제시하고 있음을 보여줍니다. 이 새로운 접근 방식은 진동 억제 장치의 빠른 반응이 중요한 분야에서 활용 가능성이 높습니다.



### On the Generalizability of Machine Learning-based Ransomware Detection in Block Storag (https://arxiv.org/abs/2412.21084)
- **What's New**: 이번 연구에서는 랜섬웨어 탐지를 위해 저장 시스템에서 효율적으로 IO 작업을 분석할 수 있는 커널 기반 프레임워크를 제안합니다. 이는 기존의 OS, 파일 시스템, 네트워크 수준의 방어 차원의 한계를 인식한 결과로, 경량의 특성을 사용하여 머신러닝 모델을 최적화하여 탐지 성능을 향상시킵니다. 또한, 다양한 환경에서의 일반화 가능성을 검토하여 실질적인 적용 가능성을 강조하고 있습니다.

- **Technical Details**: 연구는 IO 작업에서 추출된 정보를 바탕으로 랜섬웨어 탐지의 어려움을 논의하며, 실험 설정 및 악성 및 정상 workload를 소개합니다. 이전 연구에서처럼, 다양한 파일 시스템의 블록 할당 전략과 장기 데이터 배치 효과를 고려하여 IO에서의 특성 공학을 수행합니다. 이를 통해, 머신러닝 접근 방식의 효과와 전반적인 성능을 개선하는 것이 목표입니다.

- **Performance Highlights**: 실험 결과, 제안된 결정 트리 기반 모델은 기존의 접근 방식보다 높은 중위 F1 점수인 12.8% 향상, 낮은 거짓 음성 비율 10.9% 감소, 그리고 특히 거짓 양성 비율이 17.1% 감소하는 성과를 보여주었습니다. 이는 저장 기반 탐지 접근 방식에 대한 강한 주장을 뒷받침하며, 랜섬웨어 탐지 기술의 발전에 중요한의미가 있습니다.



### Quantum Diffusion Model for Quark and Gluon Jet Generation (https://arxiv.org/abs/2412.21082)
Comments:
          Accepted for the NeurIPS 2024 MLNCP workshop

- **What's New**: 이번 논문에서는 전통적인 Denoising Diffusion Models (DDMs)의 계산적 한계를 해결하기 위해 양자 컴퓨팅 기술을 활용한 새로운 diffusion model을 소개합니다. 이 fully quantum diffusion model은 전통적으로 Gaussian noise를 사용하던 부분을 random unitary matrices로 바꾸며, denoising architecture 내 U-Net에 variational quantum circuit를 통합합니다. 이러한 접근 방식은 고에너지 물리학 데이터의 생성 성능을 높이는 데 도움을 줍니다.

- **Technical Details**: 양자 비트(qubit)는 서로 다른 상태의 중첩(Superposition)으로 존재할 수 있어 정보를 더 효율적으로 처리할 수 있는 특징이 있습니다. 본 연구에서는 denoising diffusion 과정을 통해 Gaussian noise를 아래로 추가하고, 그 과정에서 얻은 latent variables의 분포를 접근합니다. 훈련 과정에서는 Kullback-Leibler divergence 및 mean squared error를 사용하여 모델을 최적화합니다.

- **Performance Highlights**: 실험 결과, fully quantum 및 hybrid models는 유사한 classical model과 경쟁할 수 있는 성능을 나타냈습니다. 특히, Large Hadron Collider의 복잡한 quark 및 gluon jets 데이터셋에서 시행된 평가에서는 이러한 양자 기술들이 머신 러닝 문제를 해결하는 데 있어 큰 잠재력을 가지고 있음을 강조하고 있습니다.



### Enhanced coarsening of charge density waves induced by electron correlation: Machine-learning enabled large-scale dynamical simulations (https://arxiv.org/abs/2412.21072)
Comments:
          11 pages, 4 figures

- **What's New**: 본 논문에서는 상관 전자 시스템에서 전하 밀도 파동(Charge Density Waves, CDW)의 조화성(coarsening) 동력을 기계 학습(Machine Learning, ML) 방법을 통해 선형 확장(linear-scaling) 알고리즘으로 시뮬레이션하는 새로운 접근법을 제안합니다. 주요 목표는 전자 상관(electron correlations)이 비평형 동역학(non-equilibrium dynamics)에 미치는 영향을 탐구하고, ML 포스 필드(force-field) 접근법의 가능성을 강조하는 것입니다. 특히, 이 연구는 전자-전자 상호작용의 스크리닝(screening) 효과와 연관된 CDW의 조화성 증진(On-site potential enhancement)을 밝혀냅니다.

- **Technical Details**: 상관 전자 시스템의 동적 시뮬레이션은 전자 자유도(electron degrees of freedom)와 다수 스케일(multiscale)의 복잡성을 고려해야 합니다. 본 연구는 스퀘어 격자 하버드-홀슈타인 모델을 사용하여 CDW의 동적 진화 과정을 분석합니다. 이 과정에서 ML 방법을 활용하여 국소 힘(local forces)이 인접한 지역의 오더 파라미터(order-parameter) 구성 상태에 의존하도록 하는 원리인 근접성 원칙(nearsightedness principle)을 모델에 통합합니다.

- **Performance Highlights**: 연구 결과, 전자 상관이 CDW 도메인 성장 과정에 미치는 영향을 상세히 분석하면서, CDW의 조화성 증가를 보였습니다. 평형 상태에서 광범위하게 연구된 CDW 시스템과는 달리, 비평형 동역학(non-equilibrium dynamics)에 대한 연구는 아직 초기 단계에 있습니다. 본 연구는 강력한 데이터 기반 접근법을 통해 CDW 도메인의 조화적 성장이 전자 상관에 의해 크게 증진될 수 있음을 보여주며, 이는 기능성 전자 재료의 다양한 응용 가능성을 시사합니다.



### Investigating layer-selective transfer learning of QAOA parameters for Max-Cut problem (https://arxiv.org/abs/2412.21071)
Comments:
          8 pages, 6 figures. Comments are welcome

- **What's New**: 이 논문에서는 Quantum Approximate Optimization Algorithm (QAOA) 관련 새로운 접근 방식을 제안합니다. 특히, 조합 최적화 문제(combinatorial optimization problems, COPs)의 파라미터 전이(parameter transfer) 후, 전체 레이어를 최적화하는 대신 일부 레이어만 선택적으로 최적화하는 방법을 탐구합니다. 이러한 방식을 통해, 최적화 시간을 크게 단축할 수 있으며, 최적화 대상 문제의 성격에 따라 성능 개선이 가능합니다.

- **Technical Details**: QAOA는 Trotterized quantum annealing 알고리즘으로 해석되며, 최적화 과정에서 classical optimization routine을 활용합니다. QAOA는 cost Hamiltonian Hc와 mixer Hamiltonian Hm 두 가지 해밀토니안으로 생성된 시간 진화를 사용합니다. 이를 통해, 문제를 해결하는 고유 상태는 Hc의 바닥 상태이며, QAOA 회로는 교대적으로 이 두 해밀토니안을 적용하여 구성됩니다.

- **Performance Highlights**: 제안된 레이어 선택적 전이 학습은 전체 레이어를 최적화할 때보다 더 낮은 근사 비율을 가져오지만, 짧은 최적화 시간 덕분에 성능이 개선되는 것을 볼 수 있습니다. 수치 실험을 통해, 이 방법이 최적화 시간을 줄이면서도 양호한 성능을 발휘할 수 있음을 확인했습니다. 정확한 성능 측정치를 통해, 근사 비율과 최적화 시간 사이의 균형을 잘 잡았음을 보여줍니다.



### Privacy-Aware Multi-Device Cooperative Edge Inference with Distributed Resource Bidding (https://arxiv.org/abs/2412.21069)
Comments:
          This article was submitted to IEEE for possible publication

- **What's New**: 이번 연구에서는 데이터 프라이버시를 고려한 다중 장치 협력 엣지 추론 시스템을 개발했습니다. 제안하는 시스템은 MEC(모바일 엣지 컴퓨팅) 서버의 계산 자원에 대한 분산 입찰 메커니즘을 통합하여, 여러 장치가 함께 협력하여 분류 작업을 수행할 수 있도록 설계되었습니다. 또한, 중간 특성 압축(intermediate feature compression)을 통해 데이터 프라이버시 누수(minimize data privacy leakage)를 최소화하는 방법을 제시하였습니다.

- **Technical Details**: 본 시스템은 DEC-POMDP(분산 부분 관찰 마르코프 결정 과정)을 기반으로 하며, MADDPG(다중 에이전트 심층 결정 비율 경량화) 알고리즘을 통해 각 MD(모바일 장치)가 자원의 입찰 가치와 중간 특성 압축 비율을 독립적으로 결정하도록 합니다. 이 알고리즘에서는 각 MD가 가지는 업링크 상향률, 입찰 예산, 데이터 분류 난이도를 고려하여 최적의 값을 찾습니다. 이러한 과정을 통해, 전체 시스템 상태에 대한 접근이 어려운 경우에도 효과적인 자원 할당이 가능해집니다.

- **Performance Highlights**: 시뮬레이션 결과는 제안된 알고리즘이 중앙 집중식 상한선과 비교할 때 경쟁력 있는 성과를 달성함을 보여줍니다. 특히, 무선 채널 조건에 무관한 방법 대비 0.31-0.95%의 분류 정확도 향상을 이룬 것으로 나타났습니다. 또한, 추론 데이터의 난이도를 추가 고려했을 때 1.54-1.67%의 추가 향상을 달성하여, 데이터 프라이버시 보호를 위한 효과적인 협력 엣지 추론이 가능하다는 것을 입증하였습니다.



### Active Learning with Variational Quantum Circuits for Quantum Process Tomography (https://arxiv.org/abs/2412.20925)
- **What's New**: 이 논문에서는 양자 프로세스 토모그래피(Quantum Process Tomography, QPT)에서 액티브 러닝(Active Learning, AL) 프레임워크를 활용하여 정보량이 높은 양자 상태를 적절히 선택하는 방법을 제안합니다. 기존 연구들은 정보량 차이를 무시하고 무작위로 양자 상태를 선택하는 단순한 접근 방식을 사용했으며, 이로 인해 비용이 많이 드는 여러 실험이 필요했습니다. 본 연구는 QPT 작업을 수행하기 위해 광범위하게 사용되는 변분 양자 회로(Variational Quantum Circuits, VQCs)를 통합하여 이 과정을 최적화합니다.

- **Technical Details**: 이 논문은 다양한 유형의 AL 알고리즘(위원회 기반(committee-based), 불확실성 기반(uncertainty-based), 다양성 기반(diversity-based))을 설계하고 평가하며, 각 알고리즘은 성능 및 계산 비용 측면에서 뚜렷한 장점을 보입니다. 이러한 알고리즘들은 정보량이 많은 양자 상태를 선택하여 재구성을 더욱 효율적으로 향상시키고, 대규모 양자 시스템에서도 적용 가능성을 보여줍니다. QPT는 정보적으로 완전한 양자 상태 집합을 입력으로 요구하며, 이로 인해 알고리즘의 복잡도가 시스템 크기에 대해 기하급수적으로 증가하는 문제를 가지고 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘이 무작위로 선택된 양자 상태와 비교하여 현저하게 개선된 재구성을 달성하는 것으로 나타났습니다. 특히, 대규모 양자 시스템에서 우리의 알고리즘의 중요성이 더욱 강조되었습니다. 본 논문은 다양한 시나리오에 맞는 알고리즘 선정에 대한 가이드를 제공하여, 향후 연구에 기여할 수 있는 방향성을 제시합니다.



### Uncertainty-Aware Out-of-Distribution Detection with Gaussian Processes (https://arxiv.org/abs/2412.20918)
- **What's New**: 최근 심층 신경망(DNN) 기술은 다양하게 사용되지만, 훈련 데이터와 다른 분포를 가진 out-of-distribution (OOD) 데이터에 대해 성능이 저하되는 문제가 있습니다. 본 논문에서는 OOD 데이터가 훈련 동안 사용되지 않더라도 DNN의 성능을 개선하기 위한 Gaussian 프로세스 기반의 OOD 탐지 방법을 제안합니다. 이 방법은 InD 데이터만을 기반으로 결정 경계를 설정하여 OOD 탐지를 수행할 수 있도록 합니다.

- **Technical Details**: 제안된 OOD 탐지 방법은 고유의 Softmax 점수의 불확실성을 정량화하기 위해 multi-class Gaussian process (GP)를 사용합니다. 이를 통해 InD와 OOD 데이터의 예측 분포의 근본적인 차이를 포착하는 점수 함수를 정의합니다. 기존 기술들과의 주요 차별점은 OOD 데이터를 필요로 하지 않으며, 대다수의 기존 DNN 구조와 호환됩니다.

- **Performance Highlights**: 제안된 방법은 기존의 OOD 탐지 방법과 비교하여 OOD 샘플이 훈련 단계에서 관찰되지 않는 경우에도 우수한 성능을 나타냅니다. 연구에는 전통적인 이미지 분류 데이터셋과 실제 이미지 데이터셋을 활용한 두 가지 사례 연구가 포함되어 있으며, 각 사례에서 기존의 최첨단 방법보다 더 나은 결과를 보였습니다.
...
### CF-CGN: Channel Fingerprints Extrapolation for Multi-band Massive MIMO Transmission based on Cycle-Consistent Generative Networks (https://arxiv.org/abs/2412.20885)
Comments:
          13 pages, 12 figures

- **What's New**: 이 논문에서는 채널 지식 맵(Channel Knowledge Map) 또는 무선 환경 맵(Radio Environment Map)으로 알려진 채널 핑거프린트(Channel Fingerprint, CF)를 다중 대역 대규모 다중입력다중출력(MIMO) 통신을 위한 새로운 방법으로 제안합니다. CF-CGN(채널 핑거프린트와 주기 일관성을 가진 생성 네트워크)을 통해 라이센스와 비라이센스 스펙트럼이 협력하여 보편적 연결성을 제공하기 위해 CF를 외삽하는 접근 방식을 소개합니다. 이전 연구에서 다룬 단일 대역 시스템의 한계를 극복하기 위해 다중 대역 시스템에서의 상관관계를 활용하여 CF를 상호 변환할 수 있는 새로운 모델을 제안합니다.

- **Technical Details**: 제안된 방법은 CF를 다채널 이미지로 모델링하고 외삽 문제를 이미지 변환 작업으로 전환하여, 하나의 주파수에서 다른 주파수로 CF를 변환합니다. 이를 위해 페어 생성 네트워크(Paired Generative Networks)를 설계하고, 서로 다른 대역에서의 상호 관계를 반영하기 위해 가변 가중치 주기 일관성 손실(variable-weight cycle consistency loss)을 결합했습니다. 또한, 모든 훈련 가능한 매개변수의 동기화 최적화를 지원하는 공동 훈련 전략(joint training strategy)을 개발하여 효율성을 높였습니다.

- **Performance Highlights**: 수치적 결과에 따르면, CF-CGN은 다양한 통신 시나리오에서 벤치마크보다 5-17 dB 낮은 오류로 양방향 외삽을 달성할 수 있습니다. 이는 CF-CGN이 일반화 능력이 뛰어난 것을 보여줍니다. 또한, CF-CGN 기반 CF를 사용했을 때의 총 전송률(sum rate performance)은 완벽한 채널 상태 정보(CSI)를 사용할 때와 유사한 성과를 거두어, 다중 대역 대규모 MIMO 전송에 대한 CF의 효율성을 잘 입증합니다.



### Machine Learning of Slow Collective Variables and Enhanced Sampling via Spatial Techniques (https://arxiv.org/abs/2412.20868)
- **What's New**: 이 논문은 화학 물리학에서의 복잡한 물리적 과정의 동적 패턴 인식 능력이 중요하다는 점에 주목하며, 이 과정들을 단순화하기 위해 반응 좌표(collective variables, CVs)를 도입합니다. CV의 품질은 동적 이해에 중대한 영향을 미치며, 최근 비지도 학습 기술을 활용하여 CV를 식별하는 데 중요한 진전을 이루었습니다. **Temporal information** 없이도 상태 간의 느린 전환을 나타내는 CV를 탐색하는 기술에 집중하고 있습니다.

- **Technical Details**: 복잡한 시스템은 일반적으로 여러 시간 척도를 가지며, 이는 메타 안정 상태 간의 드문 전이로 인한 것입니다. 이를 이해하기 위해 우리는 CV의 정의와 함께 효율적인 데이터 샘플링이 필요하다고 제안합니다. 논문에서는 **diffusion maps**와 같은 스펙트럴 방법과 심층 신경망을 활용한 최근 개발된 알고리즘을 통해 CV를 학습하는 다양한 접근 방식을 다루고 있습니다.

- **Performance Highlights**: CV의 품질은 샘플링 문제로 인해 종종 저하되며, 이는 ML 기술의 학습 데이터 세트 구성에 어려움을 초래합니다. CV는 메타 안정성 및 긴 시간 척도에서의 전이를 잘 인식해야 하며, 학습의 효율성을 높이기 위한 여러 방향이 논의됩니다. 향후 발전 방향으로 공간적 특성을 활용한 느린 CV 학습의 가능성을 제시하고 있습니다.
...
### Acquisition-Independent Deep Learning for Quantitative MRI Parameter Estimation using Neural Controlled Differential Equations (https://arxiv.org/abs/2412.20844)
Comments:
          29 pages, 10 figures, 7 supplementary figures, pre-print

- **What's New**: 이번 연구에서는 Neural Controlled Differential Equations (NCDEs)를 사용하여 정량적 MRI (QMRI) 파라미터 추정에서 발생하는 문제를 해결하였습니다. NCDE는 불완전하고 불규칙하게 샘플링된 데이터에서 변동 길이로 샘플링할 수 있는 능력 덕분에 다양한 QMRI 값 예측에 적합한 도구로 자리잡고 있습니다.

- **Technical Details**: NCDEs는 QMRI 시퀀스의 길이나 독립 변인의 구성에 관계없이 정량적 MRI 파라미터를 정확하게 예측할 수 있는 기능을 가지고 있습니다. 본 연구에서는 NCDEs가 낮은 신호 대 잡음 비(SNR)의 환경에서도 획기적으로 낮은 평균 제곱 오차(mean squared error)를 달성했음을 보여주었습니다.

- **Performance Highlights**: NCDEs는 특히 불확실성이 높은 상황이나 이미지 품질이 낮은 조건에서 QMRI 파라미터 추정을 위한 신뢰성 있는 접근 방식을 제공하는 것으로 나타났습니다. 이 연구는 NCDEs가 정량적 MRI 파라미터 추정에 있어 딥러닝의 주요 도전 과제를 해결했다는 점에서 매우 중요합니다.
...
### Robust Matrix Completion for Discrete Rating-Scale Data (https://arxiv.org/abs/2412.20802)
- **What's New**: 최근 행렬 완성(matrix completion) 분야에서, 기존 연구들은 주로 연속 데이터(continuous data)에 초점을 맞췄으나, 본 논문에서는 불완전한 등급 척도 데이터(rating-scale data)의 특수성을 고려한 강인(discrete matrix completion) 방법인 Robust Discrete Matrix Completion(RDMC)을 제안합니다. 이는 손상된 관측값(corrupted observations) 존재 시에도 안정성을 유지하도록 설계되어, 추천 시스템(recommender systems) 및 설문 조사(surveys)에서의 실용적 적용 가능성을 높입니다.

- **Technical Details**: 제안된 방법론은 여러분의 관측된 등급 행렬에 대한 정규화된 최적화 문제를 설정하고, 이를 해결하기 위해 특정 강인 손실 함수(robust loss function)를 적용합니다. 이러한 알고리즘은 R 환경에서 사용할 수 있으며, 주 코드의 대부분은 C++로 작성되어 계산 효율성을 개선한 것입니다. 또한, MNAR(missing not at random) 메커니즘의 영향을 연구하여 실질적인 데이터 손실 상황에서 성능을 평가했습니다.

- **Performance Highlights**: 시뮬레이션 연구에 따르면, 제안된 RDMC 방법은 추천 시스템 및 등급 척도 설문 조사에서 기존 방법들과 비교했을 때 뛰어난 성능을 보였습니다. 특히, 데이터 손상이 발생하거나 결측치가 있는 경우에도 강력한 대처 능력을 보이며, 실험적인 응용 사례에서도 유의미한 결과를 도출해냈습니다. 이로 인해, RDMC는 다양한 영역에 통합 가능성이 높습니다.



### FastCHGNet: Training one Universal Interatomic Potential to 1.5 Hours with 32 GPUs (https://arxiv.org/abs/2412.20796)
- **What's New**: 이번 논문에서는 FastCHGNet이라는 고속 CHGNet 모델을 도입합니다. 이 모델은 혁신적인 Force/Stress Readout 모듈을 설계하여 힘과 응력 예측을 분해합니다. 또한 GPU의 계산 능력을 충분히 활용하기 위한 대규모 최적화를 연계하여 소프트웨어 아키텍처의 능동적인 실현 가능성을 제고하였습니다. 마지막으로, 다중 GPU 지원을 포함하여 로드 밸런싱 기술을 통해 GPU 사용률을 향상시키고 있습니다.

- **Technical Details**: FastCHGNet은 기존 CHGNet의 복잡한 모델 아키텍처를 개선하여 더 효율적인 훈련 메커니즘을 제공합니다. 이는 GNN의 전파 메커니즘을 기반으로 하며, 네트워크의 다양한 블록을 통해 원자 정보를 전파합니다. 훈련 과정에서는 GPU 계산 리소스를 최대한 활용하기 위한 커널 융합 및 중복 탐지를 포함한 최적화 전략이 적용됩니다.

- **Performance Highlights**: FastCHGNet은 기존 CHGNet의 훈련 시간을 8.3일에서 1.53시간으로 줄이면서도 모델의 정확도를 유지합니다. 이를 통해 32개의 NVIDIA A100 GPU를 사용하여 약 130배의 속도 향상을 달성하였습니다. 또한, 메모리 사용량이 3.59배 감소하여 더 효율적인 메모리 관리가 가능해졌습니다.



### Enhancing Privacy in Federated Learning through Quantum Teleportation Integration (https://arxiv.org/abs/2412.20762)
- **What's New**: 이번 논문은 연합 학습(federated learning)에 양자 텔레포테이션(quantum teleportation)을 통합할 가능성을 탐구합니다. 양자 텔레포테이션은 원거리에서 양자 상태를 물리적 입자의 전송 없이 전송하는 과정을 포함하며, 이는 데이터의 보안을 강화하는 데 기여할 수 있습니다. 이 연구는 데이터 침해 위험을 줄이고 클라이언트와 서버 간의 모델 파라미터 및 기울기의 안전한 교환을 구현하기 위한 새로운 아키텍처를 제안합니다.

- **Technical Details**: 정확한 데이터 전송을 유지하기 위해 양자 얽힘(quantum entanglement)과 복제 불가능 정리(no-cloning theorem)를 활용합니다. 이를 통해 데이터가 전송 중에 더 안전하게 유지되며, 도청 시도가 감지될 수 있습니다. 또한 논문은 기존의 양자 네트워크 인프라의 한계와 혼합 양자-고전 프로토콜(hybrid quantum-classical protocols)의 필요성 같은 실용적인 도전 과제도 논의합니다.

- **Performance Highlights**: 양자 통신 기술과 연합 학습의 융합은 분산 머신 러닝에서 전례 없는 수준의 개인 정보 보호를 달성할 수 있는 유망한 경로를 제시합니다. 이 시스템 구현은 데이터 유출 및 적대적 공격에 대한 기존의 리스크를 완화함으로써, 연합 학습의 효과성을 더욱 높여줄 것입니다.
...
### Training Deep Neural Classifiers with Soft Diamond Regularizers (https://arxiv.org/abs/2412.20724)
Comments:
          8 pages, 10 figures

- **What's New**: 새롭게 제안된 소프트 다이아몬드(soft diamond) 정규화 기법은 시냅스 희소성(synaptic sparsity)을 향상시키면서 깊은 신경망(deep neural network)의 분류 정확도를 유지합니다. 이 정규화 기법은 Lasso 회귀의 상태를 초월하며, 비 가우시안 분포의 두꺼운 꼬리를 가진 시냅틱 가중치(prior) 사전이 사용됩니다. CIFAR-10, CIFAR-100, Caltech-256 데이터셋에서 테스트한 결과, 이 정규화 기법이 기존 방법들보다 우수한 정확도를 보여주었습니다.

- **Technical Details**: 소프트 다이아몬드 정규화 기법은 대칭 알파 안정 분포($
	ext{	extbackslash mathcal{S	extbackslash alpha S}}
$)의 벨 커브-bel curve 가중치 사전을 사용하여 깊은 신경 분류기를 훈련합니다. 기존 리지 및 라쏘 정규화 기법보다 효율적인 성능을 보이며, 이 정규화는 짧은 크기의 매체에 따른 차별성과 두께의 변화에 따라 다양한 형태를 가집니다. Bayesian Backpropagation(BP) 알고리즘은 이러한 새로운 접근 방식을 통합하여 신경망의 파라미터를 최적화합니다.

- **Performance Highlights**: 소프트 다이아몬드 정규화 기법은 CIFAR-10에서 4.57%, CIFAR-100에서 4.27%, Caltech-256에서 6.69%의 정확도 향상을 기록했습니다. 이 방법은 또한 모든 테스트 케이스에 대해 기존 L2 정규화법을 초월하였으며, L1 라쏘 혹은 라플라스 정규화법보다도 더 우수한 희소성 확보와 정확도 개선을 이뤘습니다. 드롭아웃(dropout), 배치(batch), 데이터 증강(data-augmentation) 정규화와 결합되었을 때, CIFAR-10에서 눈에 띄는 정확도 향상을 보여주었습니다.
...
### Blockchain-Empowered Cyber-Secure Federated Learning for Trustworthy Edge Computing (https://arxiv.org/abs/2412.20674)
- **What's New**: 이 연구는 페더레이티드 러닝(Federated Learning)과 블록체인 기술을 결합한 새로운 프레임워크를 제안합니다. 각 참가edge device는 고유한 토큰을 부여받아 안전하게 등록되고, 악성 공격을 방지하기 위한 신뢰 기반의 접근 방식을 구현합니다. 또한, 블록체인을 통해 모든 참가자의 신뢰 점수를 기록하고, 이를 통한 평판 관리를 통해 공정성을 유지하는 시스템을 도입합니다.

- **Technical Details**: 제안된 프레임워크는 에지 디바이스의 등록, 고유 토큰 생성, 리소스 인식 분산 감지 메커니즘을 포함합니다. 이 시스템은 참가 장치의 과거 활동 및 리소스 요건을 분석하여 최소 요구사항을 충족하는 장치를 선택하고, 모델 수렴에 기여하는 정도를 평가합니다. 블록체인의 스마트 계약을 활용하여 디바이스의 자원 현황 및 훈련 요구 사항을 안전하게 관리합니다.

- **Performance Highlights**: 프레임워크는 NASA의 터보팬 데이터셋을 활용하여 FL 학습 과정의 효율성을 입증합니다. 악성 업데이트와 신뢰할 수 없는 참가자로부터 시스템을 보호하기 위한 여러 안전 프로토콜을 통해, 참가자의 평판 점수를 활용해 적대적 공격을 완화하는 방법의 효과를 검증합니다. 이를 통해 에지 디바이스 간의 신뢰 관계가 향상되고 FL 학습의 공정성을 보장합니다.



### Two Birds with One Stone: Improving Rumor Detection by Addressing the Unfairness Issu (https://arxiv.org/abs/2412.20671)
- **What's New**: 이 연구는 소문 탐지에서 민감한 속성이 가져오는 성능 저하와 그룹 간 불공평함 문제를 다룬 최초의 시도 중 하나입니다. 제안된 두 단계의 프레임워크는 소문 탐지의 성능 저하를 초래하는 민감한 속성을 식별하고, 이를 통해 불공평함을 해결하고자 합니다. 기존의 소문 탐지기와 통합이 용이한 방법론을 제시하며, 민감한 속성에 대한 주석이 필요 없습니다.

- **Technical Details**: 연구 방법은 두 가지 주요 단계로 구성되어 있습니다: 1) Unfair Partition 단계에서 민감한 속성에 따른 불공정 분할을 수행하고, 2) Invariant Learning 단계에서 각 그룹별로 성능을 균형잡기 위한 정보를 학습합니다. 이 방식은 데이터 주석 없이 다양한 그룹 세트를 고려하는 다이나믹 파티셔닝(dynamic partitioning)을 통해 성능 향상 및 공정성을 동시에 추구합니다.

- **Performance Highlights**: 실험 결과, 이 방법론은 기존의 소문 탐지기와의 통합 시 탐지 성능과 공정성을 모두 유의미하게 개선하는 것으로 나타났습니다. 특히, 다양한 그룹에 걸쳐 중립적인 성과를 나타내며, 소문 탐지에서의 학습 효과를 극대화하는 데 기여합니다. 이러한 방식은 다양한 사회적 기준을 충족시키는 공정한 소문 탐지기 개발에 중요한 기여를 할 수 있을 것입니다.
...
### Matrix Concentration for Random Signed Graphs and Community Recovery in the Signed Stochastic Block Mod (https://arxiv.org/abs/2412.20620)
Comments:
          29 pages, 10 figures

- **What's New**: 이 연구에서는 노드 쌍에서 독립적으로 랜덤으로 추가된 간선과 그 부호를 고려한 그래프를 분석합니다. 우리는 이 랜덤 그래프 모델에서 얻은 인접 행렬(adjacency matrix)과 라플라시안 행렬(Laplacian matrix)에 대한 강력한 집중 불평등(concentration inequality)을 수립하였습니다.

- **Technical Details**: 연구에서는 서명된 스토캐스틱 블록 모델(signed stochastic block model)에서 샘플링된 그래프를 조사합니다. 두 커뮤니티 설정(two-community setting)에서, 각 커뮤니티 내의 간선은 긍정적인 부호를 가지며, 커뮤니티 간의 간선은 부정적인 부호를 갖는 상황을 다룹니다. 또한, 확률 $0 < s < 1/2$의 랜덤 부호 섭동(random sign perturbation)을 적용했습니다.

- **Performance Highlights**: 구체적으로, 대응하는 서명된 라플라시안 행렬의 스펙트럼 간격(spectral gap)은 높은 확률로 $2s$에 가까워지는 결과를 보여줍니다. 또한, 라플라시안 행렬의 첫 번째 고유 벡터(eigenvector) 부호가 균형 잡힌 커뮤니티 감지 문제(balanced community detection problem)에 대한 약한 일관성 추정기(weakly consistent estimator)를 정의함을 밝혔습니다. 이론적 기여를 보완하기 위해 실험 데이터도 포함하였습니다.



### Testing and Improving the Robustness of Amortized Bayesian Inference for Cognitive Models (https://arxiv.org/abs/2412.20586)
- **What's New**: 이번 연구에서는 인지 모델의 매개변수 추정을 향상시키기 위한 강건성(robustness) 향상 기법으로 Amortized Bayesian Inference (ABI)와 신경망(neural networks)을 활용합니다. 연구팀은 오염 데이터가 포함된 상황에서 ABI의 민감도를 분석하고, 데이터 생성 과정에서 오염 분포를 통합한 데이터 증대(data augmentation) 및 노이즈 주입(noise injection) 접근 방식을 제안합니다. 이 방법은 선택적 추정과 전통적인 유사성과 같은 정확성 및 효율성 손실을 평가하며, Cauchy 분포로부터의 오염물을 도입함으로써 신경 밀도 추정기(neural density estimator)의 강건성을 크게 개선하는 결과를 보였습니다.

- **Technical Details**: 연구에서는 강건統계학(robust statistics)의 도구를 활용하여 ABI의 강건성을 개선하기 위한 필요성을 설명합니다. ABI는 시뮬레이션 기반(simulation-based) 추정 접근 방식을 이용하며, 다양한 데이터 세트에 대해 사전 학습된 신경망을 통해 파라미터 추정을 효율적으로 수행합니다. 훈련 과정에서는 넓은 사전 분포(prior distribution) 및 생성 모델(generative model)을 정의하고, 이에 따라 시뮬레이션된 매개변수와 데이터 세트를 생성하는 방식으로 진행됩니다.

- **Performance Highlights**: 본 연구의 방법론은 Drift Diffusion Models (DDM)를 포함한 다양한 확률적 모델에 적용 가능하며, 전통적인 추정 방법에 비해 더 강건한 결과를 제공합니다. 실험결과, 제안된 오염물 도입 접근 방식은 검증된 방법에 항상 우수한 성능을 보였으며, 특히 오염 유무의 미세한 구분이 어려운 데이터를 다루는 데 유용합니다. 이러한 접근 방식은 오염물 감지 및 제거에 어려움을 겪는 유사한 분야에서도 널리 적용될 수 있을 것입니다.
...
### Distributionally Robust Optimization via Iterative Algorithms in Continuous Probability Spaces (https://arxiv.org/abs/2412.20556)
- **What's New**: 이번 논문은 연속적인 worst-case distribution을 고려하여, distributionally robust optimization (DRO)에서의 minimax 문제를 해결하기 위한 반복 알고리즘을 제시합니다. 기존 연구에서는 신경망 기반 생성 모델을 활용하여 worst-case distribution을 학습하려고 했으나, 알고리즘 수렴 보장이 부족했습니다. 본 연구에서는 벡터 공간의 minimax optimization과 연속 확률 밀도의 볼록 분석을 활용하여 이론적 공백을 메우고, 부드러운 가정하에 전역 수렴(global convergence)을 달성합니다.

- **Technical Details**: Brenier의 정리를 이용하여 최악의 분포를 연속적인 기준 측정에 대해 적용된 transport map으로 표현하고, 정규화된 discrepancy 기반 DRO를 Wasserstein 공간에서의 minimax 문제로 재구성합니다. 또한, 일반적으로 사용되는 여러 discrepancy 함수의 ambiguity set 반경에 따라서, Jordan-Kinderlehrer-Otto (JKO) 방식의 변형을 통해 worst-case distribution을 효율적으로 계산할 수 있음을 보여줍니다. 반복적으로 무한 차원의 최적화 문제를 해결하기 위한 수치적 접근법을 제안하며, 이는 여러 현대 기계 학습 애플리케이션에 널리 적용될 수 있습니다.

- **Performance Highlights**: 본 연구에서 제안된 알고리즘은 약한 볼록성(weak convexity)이 요구되는 다양한 현대 기계 학습 과제에 적용 가능하며, 쉽게 구현할 수 있는 우수한 성능을 보입니다. 각 반복(iteration)에서 LFD를 점진적으로 변화시키며, 적절한 조건 하에 지수 수렴 속도(exponential convergence rate)를 증명했습니다. 이로 인해, 주어진 문제를 실제 문제에 적용할 수 있는 가능성이 확장되었으며, 수렴 속도와 필요로 하는 서브그래디언트 및 수정된 JKO 반복 횟수를 수량화할 수 있는 결과를 도출했습니다.
...
### Random Matrix Theory for Stochastic Gradient Descen (https://arxiv.org/abs/2412.20496)
Comments:
...
- **What's New**: 이 논문은 머신러닝(ML) 알고리즘의 학습 동역학을 이해하기 위해 확률 행렬 이론(random matrix theory)에서 개념을 적용하여 확률적 가중치 행렬의 동태를 설명하고 있습니다. 다이슨 브라운 운동(Dyson Brownian motion)이라는 틀을 이용하여, 학습 속도(learning rate)와 배치 크기(batch size) 간의 선형 스케일링 법칙을 유도하였습니다. 이 연구는 가우시안 제한 볼츠만 기계(Gaussian Restricted Boltzmann Machine)와 선형 은닉층이 있는 신경망(neural network)에서 실험적으로 확인되었습니다.

- **Technical Details**: 논문은 확률 경량 경량 기울기 하강법(stochastic gradient descent, SGD)의 동역학을 다루며, 가중치 행렬을 업데이트하는 과정을 다이슨 브라운 운동을 통해 설명합니다. SGD는 가중치 행렬 W의 최적 상태를 찾기 위해 첫 번째 순서 방정식의 정적 점을 탐색하는 방법입니다. 여기에서는 각 데이터 포인트에서 계산된 경량의 평균 및 변동을 기반으로 한 Langevin 방정식을 도출합니다.

- **Performance Highlights**: 이 연구의 핵심 성과는 경량 속도와 배치 크기 간의 비율이 플럭튜에이션(fluctuation)의 세기를 결정하는 스케일링 계수로 간주된다는 점입니다. 이 비율은 실용적인 ML 훈련에서 이미 관찰된 것으로, 선형 스케일링 법칙으로 명명되었습니다. 실험 결과는 머신러닝 모델의 성능 향상에 기여할 가능성을 보여주고 있습니다.
...
### Convergence of the Min-Max Langevin Dynamics and Algorithm for Zero-Sum Games (https://arxiv.org/abs/2412.20471)
- **What's New**: 이 논문은 확률 분포(probability distributions) 공간에서의 제로섬 게임(zero-sum games)을 다루며, 엔트로피 정규화(entropy regularization)를 통해 플레이어 간의 상호작용 함수가 부드럽고 강한 볼록-오목(convex-concave)일 때의 상황에서 연구를 진행합니다. 또한, 평균장(min-field) min-max Langevin 역학(dynamics)의 수렴 보장을 증명하여 제로섬 게임의 평형 분포(equilibrium distribution)를 계산하는 새로운 접근법을 제시합니다.

- **Technical Details**: 이 연구에서는 연속 시간과 이산 시간 모두에서 평균장 min-max Langevin 역학의 유한 입자(finite-particle) 근사화에 대해 논의합니다. 특히, 연속 시간의 유한 입자 min-max Langevin 역학이 정적 평균장 평형 분포(stationary mean-field equilibrium distribution)로의 편향된 수렴 보장을 갖는다는 것을 증명하며, 이 경우 편향 예측(bias estimate)은 입자 수와 무관하게 음이 아닌 값을 갖습니다.

- **Performance Highlights**: 이 연구는 정적 평균장 평형 분포에 대한 이산 시간 유한 입자 min-max Langevin 알고리즘의 편향된 수렴 보장도 제시합니다. 추가적인 편향 항(bias term)은 단계 크기(step size) 및 입자 수와 함께 스케일링되며, 이는 유한 입자 알고리즘을 통해 제로섬 게임의 평형 분포를 근사적으로 계산하기 위한 명시적인 반복 복잡성(iteration complexity)을 제공합니다.
...
### A Particle Algorithm for Mean-Field Variational Inferenc (https://arxiv.org/abs/2412.20385)
Comments:
          22 pages

- **What's New**: 이 논문에서는 PArticle VI (PAVI)라고 명명된 새로운 입자 기반(mean-field variational inference, MFVI) 알고리즘을 소개합니다. 이 알고리즘은 완전 조건부에 대한 모수적 가정에 의존하지 않으며, 비모수적(nonparametric) 설정에서 적용 가능합니다. 또한, 입자 수와 총 시간 지속시간에 상관없이 수렴 보장을 제공합니다.

- **Technical Details**: PAVI 알고리즘은 MFVI의 SDE(확률 미분 방정식) 설명과 확률 근사(stochastic approximation)에 기반하여 구축됩니다. 본 논문에서는 각 반복에서 입자의 경험적 측정(empirical measure)과 변칙적 문제의 해결책 사이의 2-Wasserstein 거리의 상한을 제공합니다. 이는 MFVI의 입자 기반 접근 방식에 대한 첫 번째 종단 간(end-to-end) 수렴 보장입니다.

- **Performance Highlights**: PAVI 알고리즘은 강력한 수렴 보장을 제공하며 비모수적(nonparametric)이고 유연합니다. 기존의 coordinate ascent variational inference (CAVI) 알고리즘은 매개변수 분포의 조건부 한 쌍을 유지해야 하지만, PAVI는 이를 요구하지 않고 다양한 상황에서 응용할 수 있는 장점을 가집니다. 이를 통해 관계없이 입자 수에 관계없는 효율적인 추론이 가능하다는 것이 큰 성과입니다.
...
### Accelerated regularized learning in finite N-person games (https://arxiv.org/abs/2412.20365)
Comments:
          30 pages, 4 figures

- **What's New**: 이번 연구는 Nesterov의 가속 경량화 알고리즘이 게임의 온라인 학습에서 유사한 성능 향상을 달성할 수 있는지에 대한 탐구를 다룹니다. 새로운 방법론으로 소개된 'follow the accelerated leader' (FTXL) 알고리즘은 정규화된 학습의 일반적인 프레임워크 내에서 모멘텀을 활용합니다. FTXL은 국소적으로 엄격한 Nash 균형에 수렴하는 성능을 보이며, 기존의 정규화된 학습 방법에 비해 지수적인 속도 향상을 보여줍니다.

- **Technical Details**: FTXL은 연속 및 이산 시간 모두에서 적용 가능한 가속화된 정규화 학습 방식으로, Nesterov의 알고리즘에서 영감을 얻었습니다. 이 연구에서는 FTXL이 다양한 피드백 구조에서 유지되는 초선형 수렴 속도와 관련된 메커니즘을 탐구합니다. 특히 FTXL은 완전 정보, 실현 기반 피드백 및 밴딧(bandit) 정보를 사용할 때 모두 엄격한 Nash 균형에 수렴하며, 이는 대단히 흥미로운 결과입니다.

- **Performance Highlights**: FTXL은 기존의 정규화된 학습 방법과 비교할 때 엄청난 속도 이점을 제공하면서도 다양한 상황에서 그 성능을 유지합니다. 특히, 밴딧 피드백을 통해도 초선형 수렴 속도를 달성한 점은 주목할 만한 성과입니다. 이 연구는 향후 게임 이론 및 머신 러닝에서 가속화된 알고리즘의 활용 가능성을 열어줍니다.



### Confidence Interval Construction and Conditional Variance Estimation with Dense ReLU Networks (https://arxiv.org/abs/2412.20355)
- **What's New**: 이 논문은 비모수 회귀(nonparametric regression)에서 Conditional Variance(조건부 분산) 추정 및 Confidence Interval(신뢰 구간) 구축 문제를 다룹니다. 특히, ReLU(비선형 활성화 함수) 네트워크를 사용한 새로운 접근 방식을 제안하고, 이론적 보장을 통해 신뢰 구간의 신뢰성을 크게 향상시킵니다. 기존 연구의 가정을 완화하여 다차원 데이터에도 적용할 수 있습니다.

- **Technical Details**: 논문에서는 Residual-based framework(잔차 기반 프레임워크)를 제시하고, 이론적으로 유한한 경계(non-asymptotic bounds)를 도출합니다. 또한, ReLU 신경망 추정기가 조건부 평균(mean)과 조건부 분산(variance) 추정을 위한 최초의 경계를 제공함을 강조합니다. 이 방법은 Efron의 부트스트랩(bootstrap) 절차를 기반으로 하여 신뢰 구간을 구성합니다.

- **Performance Highlights**: 연구 결과, 제안된 방법은 고차원 환경에서도 효과적으로 Conditional Variance 및 Confidence Interval을 추정할 수 있는 능력을 보입니다. 이론적 지원을 바탕으로 신뢰 구간의 덮개(coverage)를 보장하며, 이는 심층 학습(deep learning) 환경에서의 불확실성 정량화에 있어 중요한 발전을 의미합니다. 이로 인해 통계적 예측의 신뢰성을 높이고 의사 결정에 있어 중요한 정보를 제공할 수 있습니다.
...
### Exploiting Hybrid Policy in Reinforcement Learning for Interpretable Temporal Logic Manipulation (https://arxiv.org/abs/2412.20338)
Comments:
          Accepted by IROS 2024. Code:this https URL

- **What's New**: 이번 연구에서는 로봇 학습을 위한 Temporal-Logic-guided Hybrid policy framework (HyTL)을 제안합니다. 이러한 프레임워크는 의사결정의 세 가지 수준(three-level decision layers)을 활용하여, 롱 호리즌(long-horizon) 작업에서 샘플링 효율성과 동작 해석 가능성을 개선합니다. 또한, linear temporal logic (LTL)을 사용하여 작업의 의미를 모델링하고, 얻은 피드백을 토대로 하는 새로운 waypoints 계획 모듈을 설계했습니다.

- **Technical Details**: HyTL은 세 가지 정책 계층으로 구성되어 있습니다: 높은 수준의 정책은 LTL-인코딩된 작업을 기반으로 waypoints를 계획하고, 중간 수준의 정책은 행동 원시(behavior primitives)를 선택, 최하위 정책은 환경과 상호작용할 파라미터를 지정합니다. Co-safe LTL(sc-LTL)은 유한 상태 경로에 대해 만족하는 LTL의 하위 클래스이며, 이를 기반으로 하는 parameterized action Markov decision process (PAMDP)는 롱 호리즌 작업을 해결하는 데 사용됩니다.

- **Performance Highlights**: 이 프레임워크는 네 가지 도전적인 조작 작업에서 평가되었으며, 다섯 가지 기준선(baselines)과 비교하여 학습 효율성, 성능 및 해석 가능성이 뛰어난 결과를 보여줍니다. 특히, 실험 결과는 HyTL이 기존 방법들, 특히 [17]과 비교하여 개선된 샘플링 효율을 보였음을 강조합니다. 향후 연구는 이 모델을 기반으로 더 복잡한 환경 및 작업에 대한 적용 가능성을 탐구할 예정입니다.
...
### Zeroth-Order Methods for Nonconvex Stochastic Problems with Decision-Dependent Distributions (https://arxiv.org/abs/2412.20330)
Comments:
          This paper has been accepted by The Thirty-Ninth AAAI Conference on Artificial Intelligence (AAAI-25)

- **What's New**: 이번 연구에서는 의사결정 변수에 의존하는 불확실성을 가진 최적화 문제를 다룹니다. 이 문제는 특히 기계 학습 및 가격 최적화 어플리케이션에서 중요성이 부각되고 있습니다. 기존의 영차 함수(zeroth-order methods)는 파라미터의 분포가 알려지지 않아 기울기를 명시적으로 얻을 수 없던 상황에서 유용하게 사용됩니다.

- **Technical Details**: 이 연구는 약한 가정 하에 두 가지 영차 함수 방법을 제안합니다. 첫 번째 방법은 분산 감소 파라미터를 포함한 새로운 한 점 기울기 추정기를 개발하였고, 두 번째 방법은 두 점 기울기 추정기를 사용하는 방식을 제안합니다. 이러한 두 방법 모두 정적 점으로의 수렴과 최악의 경우 반복 및 샘플 복잡도 분석을 통해 이론적인 결과를 보입니다.

- **Performance Highlights**: 실제 데이터를 기반으로 한 소매 서비스 어플리케이션에서 수행한 시뮬레이션 실험 결과, 본 연구의 방법들이 기존의 영차 함수 방법보다 낮은 목적 함수 값을 출력하는 것을 보여주었습니다. 이는 제안된 방법들이 기존 방법들보다 성능이 뛰어남을 입증하는 중요한 결과입니다.



### Predicting Customer Lifetime Value Using Recurrent Neural N (https://arxiv.org/abs/2412.20295)
Comments:
          4 pages, 4 figures, first presented in Customer Journey Optimization Workshop in KDD 2022

- **What's New**: 이 논문은 SaaS (Software as a Service) 애플리케이션에서 사용자 생애 가치를 예측하기 위한 순환 신경망(Recurrent Neural Network) 접근법을 소개합니다. 이 방법은 사용자 코호트, 시스템 내 사용자 연령, 현재 날짜의 세 가지 연결된 시간 차원을 고려하여 모델링합니다. 다중 셀 아키텍처(multi-cell architecture)를 사용하는 RNN 구조는 각각이 LSTM (Long Short-Term Memory)과 유사하여 사용자 가치를 예측하는데 효과적임이 입증되었습니다.

- **Technical Details**: 연구에서는 기존의 경량 그래디언트 부스팅 모델(light gradient boost models) 및 BTYD(Buy Till You Die) 모델에 비해 중간 절대 백분율 오차(median absolute percent error)를 크게 개선하는 결과를 보입니다. 사용자의 연령, 사용자 코호트, 예측 날짜와 같은 시간 차원을 함께 고려하여 과거의 값으로부터 미래의 값을 예측하는 전략이 적용됩니다. 이러한 시각적 데이터 분석을 통해 사용자의 연령과 날짜에 따른 가치를 효과적으로 추정할 수 있습니다.

- **Performance Highlights**: 이 모델은 사용자 취득 및 기존 사용자 생애 가치를 다양한 시간 지평선(time horizons)에 걸쳐 예측하는데 성공하였으며, 특히 사용자 데이터를 기반으로 시뮬레이션 데이터를 통해 예측 정확도를 높였습니다. 이를 통해 코호트 효과(cohort effects)와 동시 효과(contemporaneous effects)를 잘 반영하여 사용자 행동을 보다 정확하게 예측할 수 있음을 확인하였습니다.
...
### Deep Generalized Schr\"odinger Bridges: From Image Generation to Solving Mean-Field Games (https://arxiv.org/abs/2412.20279)
- **What's New**: 이번 논문에서 제안하는 일반화된 슈뢰딩거 브리지(Generalized Schrödinger Bridges; GSB)는 최소 작용의 원리에 따라 입자의 최적 진화를 분석하기 위한 새로운 알고리즘적 접근법을 제공합니다. 기존의 이론적 결과를 바탕으로 이러한 GSB는 생성 모델링, 평균장 게임(mean-field games) 등 다양한 과학적 문제에 깊이 연관되어 사용될 수 있음을 보여줍니다. GSB를 확률적 모델로 재해석함으로써, 이 논문은 딥러닝 및 신경망 기반의 새로운 계산 프레임워크를 소개합니다.

- **Technical Details**: GSB는 퍼텐셜 에너지를 포함하여 기존의 슈뢰딩거 브리지(Schrödinger Bridge; SB)보다 복잡성을 더한 개념으로, 주어진 경계 분포 간에 입자를 운반하는 확률적 과정으로 모델링됩니다. 이 연구에서는 비선형 파인만-카츠 보조정리(nonlinear Feynman-Kac lemma)를 사용하여 GSB의 최적 구조로부터 자연스럽게 발생하는 다양한 알고리즘적 개념들을 정리합니다. 이러한 구조적 특징들은 완전 연속 상태 공간에서 동작하며, 기존의 그리드 기반 수치 해법과는 달리 메쉬(mesh) 없이도 연산이 가능합니다.

- **Performance Highlights**: 제안된 GSB 방법은 생성 모델링 및 평균장 게임에서 효용성을 입증하였습니다. 특히, GSB는 수학적 모델링, 확률 과정, 제어 이론 및 머신러닝의 교차점에서 혁신적인 응용을 보여주며, 퍼포먼스 측면에서도 기존의 방법들에 비해 우수한 성능을 발휘합니다. 이 연구는 GSB를 활용한 다양한 문제 해결의 가능성을 제시하며, 향후 더 많은 과학적 응용 분야로의 확장이 기대됩니다.



### An Anomaly Detection System Based on Generative Classifiers for Controller Area Network (https://arxiv.org/abs/2412.20255)
- **What's New**: 이 논문에서는 자동차 네트워크에서 이상 감지를 위해 설계된 새로운 생성 분류기 기반의 침입 탐지 시스템(IDS)을 소개합니다. 이 시스템은 주로 자동차 내의 Controller Area Network(CAN)에서 적용되며, 변분 베이즈(variational Bayes)를 활용하여 잠재 변수의 관계를 모델링합니다. 이를 통해 가용한 훈련 데이터를 적게 사용하면서도 높은 성능을 유지하도록 설계되었습니다.

- **Technical Details**: 제안된 IDS는 딥 촉발 변수 모델을 기반으로 하여 입력, 출력, 잠재 변수 간의 인과 관계를 모델링합니다. 이를 위해 자가 인코더(autocoder) 아키텍처를 사용하여 조건부 확률을 추정하고, 베이esian 추론을 통해 최종 예측 확률을 도출합니다. 이 방식은 다양한 공격 메시지를 포함한 공개 데이터셋에 대한 실험적 평가를 통해 기존의 IDS보다 우수한 성능을 입증하였습니다.

- **Performance Highlights**: 상당수의 기존 IDS와 비교한 결과, 논문에서 제안한 모델은 탐지 정확도 및 F1 점수에서 더욱 향상된 성능을 보여줍니다. 또한, 학습 데이터가 제한된 상황에서도 기존 모델들을 초월하는 보안 보장을 제공하는 것으로 나타났습니다. 이 연구는 차량 시스템의 보안을 개선하기 위한 혁신적 접근 방식을 제시하고 있습니다.



### Machine and Deep Learning for Credit Scoring: A compliant approach (https://arxiv.org/abs/2412.20225)
- **What's New**: 이 논문은 기존의 신용 평가 모델의 한계를 극복하기 위해 Gradient Boosting Machines, 특히 XGBoost를 활용하여 은행 A의 자동차 대출 신청자의 신용 점수를 평가하는 새로운 접근 방식을 제시합니다. 이를 통해 BASEL 2 및 3 준수 기술을 도입하여 규제에 맞춘 신용 평가 모델의 필요성을 강조합니다. 또한 Shapley Values를 활용하여 복잡한 모델의 출력 결과를 해석하고, 현재 규제 시스템이 믿고 있는 모델의 투명성을 높이려는 노력을 보여줍니다.

- **Technical Details**: 모델 프레임워크는 트리 기반 모델, 부스팅, 교차 검증 및 클래스 재조정 이론을 포함하여 저자들이 내린 모델링 결정에 대한 증명을 제공합니다. 수학적 설정을 정의한 후, Bayesian 분류기 및 손실 최소화 문제 등을 다룹니다. 최종적으로 은행 데이터를 바탕으로 XGBoost를 신용 평가에 적용하고, 다양한 신용 평가 모델의 성능을 비교합니다.

- **Performance Highlights**: 실험 결과, XGBoost 모델은 BANK A에서 사용하고 있는 기존 모델에 비해 성능과 디폴트 포착률을 크게 개선하는 것으로 나타났습니다. 이 연구는 XGBoost를 사용하여 신용 평가의 정확성을 높이는 방법으로, 자동차 대출 및 기업 대출 시장에서의 활용 가능성을 제시합니다. 또한, 이러한 모델의 설명 가능성을 통해 신용 점수 산출 과정에 대한 더 깊은 이해를 제공하고 있습니다.
...
### No-regret learning in harmonic games: Extrapolation in the face of conflicting interests (https://arxiv.org/abs/2412.20203)
Comments:
          36 pages, 5 figures

- **What's New**: 본 논문은 조화 게임(harmonic games)에서의 무희망 학습(no-regret learning)의 수렴 성질을 조사하려고 합니다. 특히, 가장 널리 연구된 무희망 학습 방식인 규칙적 리더 따르기(follow-the-regularized-leader, FTRL)를 집중적으로 분석합니다. 연구 결과, FTRL의 연속시간 동역학이 Poincaré 재귀성을 보이고 있으며, 이로 인해 수렴하지 않는다는 것을 보여줍니다. 하지만 적절한 외삽 단계가 추가된 경우에는 모든 초기 상태에서 Nash 균형으로 수렴할 수 있음을 증명하였습니다.

- **Technical Details**: 연구는 조화 게임의 연속 및 이산 시간 모델에서 무희망 학습의 동역학을 다룹니다. 연속 시간 모델에서는 모든 FTRL 동역학이 Poincaré 재귀성을 가지고, 이산 시간 모델에서는 기본 FTRL 구현이 게임의 균형으로 수렴하지 않음을 나타냈습니다. 이에 따라, FTRL을 외삽 단계로 보완하면, 플레이어들은 상수 수준의 개인적 후회를 갖고 Nash 균형에 수렴할 수 있게됩니다. 이 결과는 조화 게임에서의 무희망 학습의 포괄적인 이해를 제공합니다.

- **Performance Highlights**: 결과적으로, FTRL+ 알고리즘은 조화 게임에서 플레이어의 후회를 O(1) 수준으로 제한하면서 Nash 균형으로 수렴함을 보입니다. 이는 플레이어의 행위 빈도가 조잡한 상관 균형(coarse correlated equilibrium)으로 수렴함을 의미합니다. 또한, 정상적인 학습 동역학의 수렴성에 대한 기존의 이해를 확장하며, 조화 게임이 잠재적 게임(potential games)의 정통한 보완임을 기술적으로 증명하고 있습니다.



### Learning physical unknowns from hydrodynamic shock and material interface features in ICF capsule implosions (https://arxiv.org/abs/2412.20192)
- **What's New**: 이번 논문은 고에너지 밀도 물리학(HEDP)과 관성 구속 융합(ICF)에서 모델링 예측에서의 불확실성을 해결하기 위한 머신러닝 기법을 제시합니다. 특히, X-ray를 이용한 방사선 측정을 통해 수집된 데이터에서 간결한 유체역학적 특징을 정의하여, 이러한 파라미터를 직접 추론하는 방법론을 소개합니다. 연구팀은 방사선 이미지를 통해 산출된 정보를 기반으로 파라미터 posterior distribution을 근사할 수 있는 두 개의 네트워크 아키텍처(R2FNet과 F2PNet)를 개발하였습니다.

- **Technical Details**: 본 연구에서는 방사선 이미지를 통한 기초 물리 파라미터 및 초기 섭동을 추정하기 위해 머신러닝 기반의 역방향 매핑 프레임워크를 구축했습니다. 이는 가압된 Euler 방정식을 따르며, 초기 조건 및 물질 특성이 알려지지 않은 상태에서 수행됩니다. 연구진은 모르고 있는 물질 특성과 초기 조건으로부터 파라미터 추정을 할 수 있는 기법을 개발하여 ICF 시스템의 유동 역학적 예측 성능을 향상시키고자 했습니다.

- **Performance Highlights**: 결과적으로, 제안된 기법은 실험적 방사선 데이터에서 일관된 밀도장 및 유체역학적 충격 및 외부 모서리 특징을 재구성하는데 성공했습니다. 또한 기존의 EOS 모델을 기반으로 예측된 특성의 매핑이 가능하여 네트워크가 물리학을 학습하고 있음을 의미합니다. 이를 통해 유체역학 모델을 보정하고 초기 조건을 개선하여 불확실성을 줄일 수 있는 잠재력을 보여주었습니다.



### Debiased Nonparametric Regression for Statistical Inference and Distributionally Robustness (https://arxiv.org/abs/2412.20173)
- **What's New**: 이 연구에서는 부드러운 비모수 추정기(smooth nonparametric estimators)를 위한 디바이징(debiasing) 방법을 제안합니다. 머신 러닝 기법이 강력한 예측 성능을 입증했지만, 이들의 이론적 특성은 상대적으로 탐구되지 않았습니다. 본 연구는 이러한 문제를 해결하기 위해 오히려 전통적인 방법인 나다라야-왓슨 회귀(Nadaraya-Watson regression)에서 확립된 점근적 정규성(pointwise asymptotic normality)과 균일 수렴(uniform convergence)을 보장하는 모델 없는 디바이징 방법을 소개합니다.

- **Technical Details**: 이 논문은 비모수 회귀(nonparametric regression)의 문제를 다룹니다. 공변량 X와 타겟 변수 Y의 조인트 분포를 정의하고, 추정기의 조건부 기대 잔차를 추정하는 교정 항을 추가하여 디바이즈된 추정기를 얻습니다. 제안된 추정기는 세 가지 단계로 구성되어 있으며, 부드러움의 의미에서 대상 비모수 추정기, 진정한 회귀 함수, 혹은 이들의 차이는 Hölder 클래스에 속함을 가정합니다.

- **Performance Highlights**: 부가적으로, 제안한 디바이즈 비모수 회귀 추정기는 완만한 조건 하에 점근적 정규성(asymptotic normality)과 균일 수렴(uniform convergence)을 나타냅니다. 두 번 robust한 특성을 가지며, 첫 번째 단계 비모수 회귀 추정기나 두 번째 단계 조건부 기대 잔차 추정기 중 하나가 일관되면, 결과적인 디바이즈된 추정기도 일관성을 가집니다. 이러한 특성 덕분에 이 방법은 다양한 비모수 회귀 문제에 광범위하게 적용될 수 있습니다.
...
### Gradient Descent Methods for Regularized Optimization (https://arxiv.org/abs/2412.20115)
Comments:
          20 pages, 8 figures, 1 table; To be published in journal: "MANU Contributions, Section of Natural, Mathematical and Biotechnical Sciences"

- **What's New**: 이 논문에서는 $	ext{proximal gradient descent}$ 방법의 새로운 알고리즘을 제안합니다. 기존의 방법과 달리, 제안된 알고리즘은 각 반복에서 $	ext{Lipschitz constant}$를 지역적으로 추정하고 그 역수를 스텝 크기로 사용하는 방식입니다. 이를 통해 전역 $	ext{Lipschitz constant}$의 필요성을 없앴으며, 이는 계산상 비실용적일 수 있습니다.

- **Technical Details**: 정상적인 $	ext{gradient descent (GD)}$ 방법은 $	ext{differentiable}$한 목적 함수를 최적화하는 데 효과적입니다. 그러나 $	ext{L1 regularized}$ 최적화 문제는 비분별 지점에서 비연속적이기 때문에 GD는 수렴하지 않거나 진동하게 됩니다. 이에 반해, $	ext{proximal GD}$는 $	ext{soft-thresholding}$ 기법을 이용하여 반복 업데이트를 0으로 수축시켜 해결합니다.

- **Performance Highlights**: 날짜 세트에 대한 수치 실험 결과, 제안된 방법은 기존의 일정한 스텝 사이즈를 가진 $	ext{proximal GD}$ 방법보다 반복 횟수와 시간 모두에서 개선된 성능을 보였습니다. 결과적으로, 새롭게 제안한 방식은 더 빠른 수렴과 향상된 최적화 성능을 제공합니다.
...
### Global Search of Optimal Spacecraft Trajectories using Amortization and Deep Generative Models (https://arxiv.org/abs/2412.20023)
Comments:
...
- **What's New**: 본 논문은 우주선 궤적 최적화(trajectory optimization) 문제를 파라미터 의존적인 글로벌 탐색(global search) 문제로 설정하여, 고품질의 다양성 있는 솔루션 세트를 제공하는 방법을 제안합니다. 특히, 고유한 최적 제어 문제(optimal control problem)와 제어 변환(control transcription)의 선택, 그리고 기울기 기반 수치 해결 수(Numerical solver)의 동작에 따라 해결 방법이 달라지는 것을 강조합니다. 이러한 접근은 파라미터 변화에 따라 지역적인 매력 구역(local basins of attraction)이 어떻게 변화하는지를 예측할 수 있는 심층 생성 모델(deep generative models)을 통해 구현됩니다.

- **Technical Details**: 해당 연구에서는 조건부 확률 분포(conditional probability distribution)를 샘플링하는 작업으로 문제를 정형화합니다. 이 조건부 분포는 고품질 솔루션을 향한 지역 기울기 곡선의 이웃(neighborhoods)에서 지원되며, 이는 파라미터에 의해 변화하는 저항적 궤적 최적화 문제(low thrust spacecraft trajectory optimization problem)를 중심으로 실험됩니다. 논문은 저항적 궤적 최적화 문제의 다중 모드 퍼널 구조(multi-modal funnel structure)를 깊이 분석합니다.

- **Performance Highlights**: 제안된 접근법은 단순한 멀티 스타트(multi-start) 방법과 기존의 일반적인 머신러닝 접근법에 비해 상당한 성능 향상을 보여줍니다. 특히, 원형 제한 삼체 문제(circular restricted three-body problem)에서 저항적 궤적 최적화 문제를 평가하여 그 효과성을 입증하였습니다. 결과적으로, 이 연구는 미래의 우주 임무에 있어 효율적인 궤적 설계(trajectory design)에 기여할 수 있는 가능성을 제시합니다.
...
### Mouth Articulation-Based Anchoring for Improved Cross-Corpus Speech Emotion Recognition (https://arxiv.org/abs/2412.19909)
- **What's New**: 이 연구에서는 기존의 음향 특징에 중점을 두는 방식에서 벗어나 감정 특정 구강 움직임(articulatory gestures)에 기반한 새로운 접근법을 도입하고 있습니다. 이를 통해 감정 인식을 위한 전이 학습(emotion transfer learning)에서 더 나은 성과를 기대하고 있습니다. 구술 감정 인식을 위한 두 개의 데이터세트(CREMA-D와 MSP-IMPROV)를 활용하여 구현된 이 방법은 기존 모델에 비해 향상된 성과를 보입니다.

- **Technical Details**: 본 연구는 구강 움직임(articulatory gestures)의 안정적인 성질을 이용하여 교차 코퍼스(cross-corpus) 정렬을 개선하려고 합니다. 구강 부분의 특징은 전자기 구강도(EMA) 및 실시간 자기공명영상(MRI) 방법들에 의해 수집될 수 있지만, 이 방법들은 실제로 기록하기 어렵고 제한된 코퍼스를 가집니다. 대신, 본 연구는 비주얼 모달리티에서 추출된 구강 랜드마크를 사용하여 구강 굴절 제약(AG constraints)을 통합한 전이 학습을 탐색하고 있습니다.

- **Performance Highlights**: 새로운 구강 움직임 중심의 접근 방식인 AG-CC는 기존의 기준선 모델에 비해 더욱 향상된 성능을 나타냅니다. AG 클러스터 모델을 사용하여 각 감정 카테고리 간의 겹침을 분석하였고, 다양한 코퍼스 간의 구강 움직임 패턴을 비교하여 유사성을 평가했습니다. 최적의 클러스터 수는 엘보우 방법(elbow method)을 통해 10으로 설정되었고, AG 패턴이 다양한 클러스터 간에 겹치는 정도를 보여주었습니다.



### Surrogate Modeling for Explainable Predictive Time Series Corrections (https://arxiv.org/abs/2412.19897)
- **What's New**: 이번 연구는 설명 가능한 시계열 예측을 위한 지역적인 대리 모델(local surrogate model) 접근 방식을 소개합니다. 기존의 이해할 수 없는 예측 모델을 사용하여 기본 모델의 예측 정확도를 향상시키는데, 이 과정에서 데이터의 오류 예측을 제거하고 다시 모델을 적합하여 설명 가능한 차이를 도출합니다. 이를 통해 데이터 내 깊은 패턴을 발견하고 설명할 수 있는 잠재력을 제시합니다.

- **Technical Details**: 혜택을 디자인하는 지역적 대리 모델에서의 선택은 인스턴스 주변의 적절한 크기와 위치를 결정하는 것입니다. BAPC(이전 및 이후 예측 파라미터 비교) 프레임워크에 기반하여, 기본 모델과 고성능의 머신러닝 모델을 결합한 하이브리드 형태의 시계열 모델이 소개됩니다. 이 방법은 예측 오차를 보정하고, 이러한 보정이 적용된 후의 파라미터 변화를 통해 모델의 동작을 설명합니다.

- **Performance Highlights**: 제안된 모델은 해석 가능한 기본 모델이 기반으로, 고성능 머신러닝 모델의 작용을 정량화할 수 있습니다. 이를 통해 시계열 데이터의 이해도를 높이고, 예측의 신뢰도를 강화하는 데 기여할 수 있습니다. 실험 결과에서는 인위적 데이터 및 실제 데이터를 활용하여 다양한 사례를 통해 방법론의 효과를 입증하였습니다.



### A Neural Network-Based Search for Unmodeled Transients in LIGO-Virgo-KAGRA's Third Observing Run (https://arxiv.org/abs/2412.19883)
- **What's New**: 이 논문은 LIGO, Virgo, KAGRA의 세 번째 관측 주기에서 단기 중력파 과도 현상에 대한 신경망(Neural Network, NN) 기반 검색 결과를 제시합니다. GWAK(Gravitational Wave Anomalous Knowledge) 방법을 활용하여 기존 파이프라인에 의해 식별된 세 가지Compact Binary Coalescences (CBC)와 다양한 탐지기 글리치를 발견했습니다. 이 연구는 중력파 탐색의 한계를 극복할 수 있는 GWAK의 가능성을 보여주며, 향후 탐지 전략의 기초를 마련합니다.

- **Technical Details**: 이 연구는 LIGO-Virgo-KAGRA 관측소의 데이터를 수집하여, 고유한 중력파 신호의 물리적 특징을 포착하는 저차원 임베딩 공간을 구성합니다. GWAK 방법은 다양한 기준으로 훈련된 오토인코더(autoencoder)를 사용하여, 각각 배경 잡음, 글리치 및 신호 유형(예: 이진 블랙홀의 파동형)을 분류하고 특성을 구분합니다. 이러한 semi-supervised(반지도 학습) 접근 방식은 신호 사전 정보를 통합하면서도 이전에 관찰되지 않은 현상을 감지할 수 있는 일반성을 유지합니다.

- **Performance Highlights**: 이 논문에서 제안한 검색 방법은 CBC 및 가능한 모델화되지 않은 원천, 예를 들어 핵붕괴 초신성(core-collapse supernovae) 및 백색잡음 폭발(white-noise bursts)을 식별하는 데 있어 강력한 성능을 보입니다. 결과적으로 O3 동안 단기 중력파 폭발에 대한 기회를 넓혔으며, 향후 연구의 방향성을 제시합니다. 이 연구는 고유 신호를 탐지하는데 있어 딥러닝 방법의 중요성을 강조하며, 중력파 천문학의 미래에 기여할 수 있는 잠재력을 내포하고 있습니다.
...
### Multi-Agent Q-Learning for Real-Time Load Balancing User Association and Handover in Mobile Networks (https://arxiv.org/abs/2412.19835)
- **What's New**: 이번 연구에서는 밀집한 전송 네트워크 내에서 사용자와 기지국(BS)을 최적으로 연관짓고, 기지국이 과부하에 걸리지 않도록 보장하는 온라인 다중 에이전트 Q-러닝(QL) 알고리즘을 제안합니다. 이를 통해 실시간 로드 밸런싱(user association) 및 핸드오버를 수행할 수 있는 접근 방식을 제공합니다. 제안된 알고리즘은 네트워크 다이나믹스, 사용자 이동성, 그리고 핸드오버 비용을 고려하여 실시간으로 적응할 수 있는 특징을 가지고 있습니다.

- **Technical Details**: 본 연구는 중앙 집중형 및 분산형 두 가지 다중 에이전트 액션 선택 정책을 구현합니다. 중앙 집중형 정책은 중앙 로드 밸런서(CLB)에 의해 수행되며, 최적의 학습 보상을 최대화하기 위해 최악의 연결을 교환하는 알고리즘을 사용합니다. 분산형 정책은 각 사용자 장치(UE)가 로컬 정보를 바탕으로 실행되며, 기지국과의 분산 매칭 게임에 참여하여 로컬 보상을 극대화합니다. 이 두 정책은 모든 학습 단계에서 로드 밸런싱을 보장하며, 에이전트 간의 정보 교환이 필요하지 않고 신호 오버헤드가 적습니다.

- **Performance Highlights**: 제안된 QL 알고리즘은 3GPP max-SINR 연관 방식을 초월하여 양호한 성능과 빠른 수렴성을 보여주었습니다. 성능 평가 결과, 이 알고리즘은 다양한 사용자 속도 프로필(걷기, 달리기, 자전거 타기, 교외 운전)에서 mmWave 채널의 변동성과 사용자 이동성에 잘 적응하며 높은 스루풋과 낮은 핸드오버 빈도를 달성합니다. 또한, 이 알고리즘은 다양한 네트워크 다이나믹스에 대해 충분히 강력하고 실시간으로 적합함을 입증하였습니다.
...
### GFormer: Accelerating Large Language Models with Optimized Transformers on Gaudi Processors (https://arxiv.org/abs/2412.19829)
- **What's New**: 본 논문에서는 Gaudi 프로세서에서 Transformer 기반의 대형 언어 모델(LLM) 연산을 최적화하기 위한 새로운 접근법인 GFormer를 소개합니다. GFormer는 희소(sparse) 및 선형(linear) 주의(attention) 메커니즘을 통합하여 Gaudi 프로세서의 행렬 곱셈 엔진(Matrix Multiplication Engine, MME)과 텐서 프로세싱 코어(Tensor Processing Cores, TPC)의 계산 능력을 극대화하는 것을 목표로 합니다. 이러한 통합 접근방식은 모델 품질을 유지하면서 LLM 추론을 효율적으로 수행할 수 있도록 돕습니다.

- **Technical Details**: GFormer는 TPC의 벡터 유닛을 활용한 윈도우 기반의 지역적(self-windowed) 맥락 주의(kernel) 및 인과선형(causal linear) 주의에 대한 효율적인 외적(outer product) 커널을 포함하여 다양한 계산 효율성과 모델 충실도를 동시에 최적화합니다. 이러한 시스템 구조는 각 하드웨어 리소스의 성능을 보다 균형 있게 활용하기 위한 최적의 작업 부하 분배 알고리즘에도 기초하고 있습니다.

- **Performance Highlights**: GFormer를 평가한 결과, GPT 및 ViT 모델에서 각각 2배와 2.2배의 처리 속도 향상을 달성했습니다. 또한 GFormer는 Gaudi 프로세서에서 다양한 작업에 대해 효율성과 모델 성능을 크게 향상시켰으며, 최신 GPU 대비 우수한 성능을 보였습니다. 이는 기업과 연구자들이 Gaudi와 같은 이종(hétérogène) 하드웨어에서 LLM을 활용하는 데 중요한 기여를 할 것으로 예상됩니다.
...
### Enhancing Drug-Target Interaction Prediction through Transfer Learning from Activity Cliff Prediction Tasks (https://arxiv.org/abs/2412.19815)
- **What's New**: 이 연구는 활동 절벽(activity cliff, AC) 예측에서 전이 학습(transfer learning)을 적용하여 약물-타겟 상호작용(drug-target interaction, DTI) 예측의 성능을 향상시키는 혁신적인 방법을 제시합니다. 이 접근법을 통해 기존의 유사성 기반 모델의 한계를 극복하고, 구조적으로 복잡한 영역에서 DTI 모델 성능을 개선할 수 있음을 보여줍니다. AC를 인식하는 통합된 프레임워크를 통해 데이터 부족과 예측 문제에 대한 새로운 해결책을 제공하는 것이 이 연구의 핵심입니다.

- **Technical Details**: 본 연구는 KIBA와 BindingDB 데이터셋을 활용하여 AC 쌍을 정의합니다. AC 쌍은 구조적으로 유사하고 동일한 타겟에 대해 서로 다른 친화도를 가지는 화합물로 결정됩니다. 또한 Extended Connectivity Fingerprints (ECFPs)를 생성하여 입력 특성으로 사용하고, 단백질은 단어 임베딩(word embeddings)으로 인코딩되며, 이는 계산 복잡성을 줄이고 다양한 데이터셋을 쉽게 통합할 수 있도록 돕습니다. 데이터 분할 방법으로는 무작위 분할과 화합물 기반 분할이 사용되며, 후자는 데이터 누출을 방지하여 모델 성능을 더욱 신뢰할 수 있도록 합니다.

- **Performance Highlights**: 이 연구는 AC 예측에서의 통찰을 바탕으로 DTI 예측을 개선할 수 있음을 나타냅니다. 실험 결과, AC 인식이 모델의 복잡한 화학 상호작용을 처리하는 데 효과적임을 보여주었으며, 유사한 전반적 성능을 유지하면서도 AC가 포함된 어려운 사례에서도 더 나은 성능을 발휘했습니다. 이는 약물 발견 프로세스에서 AC에 대한 인식을 통합함으로써 DTI 예측의 신뢰성과 효율성을 크게 향상시킬 수 있음을 시사합니다.
...
### Pharmacophore-constrained de novo drug design with diffusion bridg (https://arxiv.org/abs/2412.19812)
Comments:
          13 pages, 6 figures, 4 tables

- **What's New**: 본 연구에서는 PP2Drug이라는 약물 후보를 유도하는 새로운 접근 방식을 제안합니다. 이는 pharmacophore(약리적 구조)의 제약을 사용하여 생물학적 활성이 기대되는 약물 분자를 설계합니다. 전통적인 CADD 시스템과의 차별점은 생성된 분자의 생화학적 특성을 정교하게 제어할 수 있는 구조입니다.

- **Technical Details**: PP2Drug은 diffusion bridge(확산 다리) 알고리즘을 활용하여 공간 공간 속의 약리적 구조를 효과적으로 분자 구조로 변환합니다. 이는 대칭 변환(equivariant transformation)의 방식으로 구현되어 생성된 분자에서 최적의 생화학적 특성을 정렬합니다. 특히, 이 연구는 약물 후보가 타겟 단백질에 대해 높은 결합 친화력을 나타낼 수 있음을 입증하였습니다.

- **Performance Highlights**: PP2Drug은 약물 설계 분야에서 고성능을 보여줍니다. 기존의 SMILES 표기법이나 구조 기반 메커니즘에 비해 향상된 화학적 유효성 및 더 많은 후보 물질의 생성을 가능하게 합니다. 이 방법론은 약리적 특징을 명확히 반영하여 실행되므로 약물 발견 과정에 실질적인 기여를 할 수 있습니다.
...
### High-Rank Irreducible Cartesian Tensor Decomposition and Bases of Equivariant Spaces (https://arxiv.org/abs/2412.18263)
Comments:
          43 pages

- **What's New**: 본 연구는 Irreducible Cartesian Tensors (ICTs)의 분해 행렬을 n=6부터 n=9까지 효율적으로 얻는 방법을 제시합니다. 이 방식은 기존의 복잡한 접근법을 대체하여 경량화된 경로 행렬(path matrices)을 통해 이루어집니다. 연구자들은 이 방법이 고차 테너 고유 분해 문제를 해결하는 데 크게 기여할 것으로 기대하고 있습니다.

- **Technical Details**: 연구진은 기존의 RREF(Reduced Row Echelon Form) 알고리즘과는 달리, Cartesian tensor product space가 spherical space에 따라 직접곱으로 변환될 수 있음을 이용합니다. 이를 통해 연결된 외적의 결과를 이용한 변환 매트릭스를 생성하는 방법을 밝혔습니다. 최종적으로 이 방법은 Cartesian tensor를 spherical tensor로 변환하는 기법으로 기능하게 됩니다.

- **Performance Highlights**: 연구는 n=6, 7, 8, 9에 대해 각각 1초, 3초, 11초, 4분 32초로 ICT 분해 행렬을 도출해내어, 이전의 비효율적인 접근법과 비교할 때 월등한 시간적 성과를 보여줍니다. 이러한 성과는 고차 ICT를 다루는 데 있어 실용적인 접근을 가능하게 만들어 줍니다. 또한 오르소노말한 기저의 완전성을 제시하여 EGNN의 설계 공간 확장에도 기여합니다.

...
- **What's New**: 이 논문은 화재 발생 시 건물 구조의 안정성에 미치는 영향을 예측하기 위한 새로운 프레임워크를 제안합니다. Most Fire-Sensitive Point (MFSP) 예측에 중점을 두며, 신경망 기반 접근 방식을 통해 화재 역학(fire dynamics)과 유한 요소 분석(Finite Element Analysis, FEA)을 통합합니다. 새로운 방법을 통해 화재 시 최대 층간 변위 비율(Maximum Interstory Drift Ratio, MIDR)을 예측함으로써 효율적인 구조 평가 및 디자인 프로세스를 지원합니다.

- **Technical Details**: 제안된 프레임워크는 MIDR 예측기와 MFSP 예측기의 두 가지 주요 구성 요소로 이루어집니다. 이 시스템은 GNN(Graph Neural Networks)을 활용하여 구조 물체의 특성을 모델링하고, 전이가 가능한 학습(Transfer Learning) 방법론을 통해 학습 과정의 효율성을 높입니다. 또한 에지 속성의 변화를 동적으로 반영하기 위한 에지 업데이트 메커니즘(Edge Update mechanism)이 도입되어, 구조의 여러 속성 변화에 적응합니다.

- **Performance Highlights**: 평가 결과, 제안된 모델은 MFSP 및 MIDR 예측의 정확성이 뛰어난 것으로 나타났습니다. 이로 인해 건물 구조의 화재 안전 분석이 획기적으로 발전할 수 있는 기반이 마련되었습니다. 연구의 결과는 실시간 화재 위험 평가 및 건물 구조 설계의 신뢰성을 높이는 데 기여할 것입니다.



### From Features to Transformers: Redefining Ranking for Scalable Impac (https://arxiv.org/abs/2502.03417)
- **What's New**: 이번 논문에서는 LinkedIn에서 개발한 방대한 규모의 순위화 프레임워크인 LiGR을 소개합니다. LiGR은 최첨단 transformer 기반 모델링 아키텍처를 생산 환경에 도입하며, 기존에 수작업으로 설계된 다수의 feature engineering을 필요로 하지 않게 만드는 혁신적인 모습이 특징입니다. 이 아키텍처는 사용자의 역사와 순위가 매겨진 항목에 대한 동시 세트 단위 어텐션을 포함하여, 다양성을 자동으로 향상시키는 방법도 제시합니다.

- **Technical Details**: LiGR에서는 학습된 게이티드 정규화(learned gated normalization)와 세트 방식의 어텐션을 통합한 수정된 transformer 아키텍처를 사용하고 있습니다. 이 아키텍처는 대규모 모델, 데이터 및 긴 문맥 시퀀스를 활용하여 성능을 향상시키며, 7개의 feature만으로도 이전의 최고 성능 시스템보다 뛰어난 결과를 보입니다. 또한, 사용자의 히스토리 및 집합적 어텐션을 활용한 효율적인 추론 기술을 통해 대규모 모델의 생산 환경에서의 배포를 지원합니다.

- **Performance Highlights**: 논문에서는 다양한 ablation 연구와 A/B 테스트의 주요 통찰력을 요약하며, 이 기술이 Daily Active Users (DAU)의 0.27% 증가와 같은 막대한 성과를 가져왔음을 강조합니다. 특히, LiGR 아키텍처는 기존보다 더 적은 feature를 사용하면서도 품질 높은 추천 결과를 제공하며, 여러 성공적인 모델과의 비교를 통해 그 우수성을 입증했습니다. 이 논문은 대규모 순위화 및 검색 시스템의 개발을 위한 실용적인 솔루션과 통찰력을 제공한다고 보여집니다.



### Detecting Strategic Deception Using Linear Probes (https://arxiv.org/abs/2502.03407)
Comments:
          Website: this http URL Code: this http URL

- **What's New**: 이번 연구에서는 AI 모델이 사용 가능한 기만적인 전략들을 탐지하기 위한 새로운 방법을 제시합니다. 기존의 모니터링 방식이 단순히 출력(output)을 감시하는 데 그쳤다면, 우리는 모델의 활성화(activation)를 모니터링함으로써 진짜 내부 reasoning의 비일치(disalignment)를 파악할 수 있는 가능성을 평가합니다. 이는 단순한 출력 확인을 넘어서는 중요한 진전을 나타냅니다.

- **Technical Details**: 우리는 두 가지 프로브 훈련 데이터셋을 이용하여 실험을 수행하였습니다. 하나는 정직한(integrity) 답변과 기만적인(deceptive) 답변을 구분하기 위한 대비되는 지시(instructions)로 구성되었고, 다른 하나는 간단한 롤플레잉(roleplaying) 시나리오에 대한 반응(responses)으로 구성됩니다. Llama-3.3-70B-Instruct 모델이 기만적으로 행동하는 상황을 포함하여 여러 현실적인 환경에서 이 프로브가 일반화(generalize)되는지를 테스트했습니다.

- **Performance Highlights**: 우리의 프로브는 평가 데이터셋에서 기만적인 답변과 정직한 답변을 0.96에서 0.999 사이의 AUROC(Area Under Receiver Operating Characteristic) 성능으로 구분할 수 있었습니다. 결정 임계값(decision threshold)을 1%의 허위 긍정률(false positive rate)로 설정했을 때, 우리는 95-99%의 기만적인 답변을 효과적으로 포착(catch)했습니다. 그러나 현재의 성능은 기만에 대한 robust defense로는 불충분하다는 점에서 개선이 필요합니다.
...
### CAPE: Covariate-Adjusted Pre-Training for Epidemic Time Series Forecasting (https://arxiv.org/abs/2502.03393)
- **What's New**: CAPE (Covariate-Adjusted Pretraining for Epidemic forecasting)는 다양한 지역의 방대한 질병 데이터셋을 활용하여 환경적 요인을 모델링하는 새로운 전염병 예측 프레임워크입니다. 기존의 모델들이 특정 질병과 지역에 국한되어 있는 반면, CAPE는 여러 질병에서 일반화된 패턴을 발견하여 보다 정확한 예측을 가능하게 합니다. 이 연구는 환경 요인의 복잡한 상호작용을 직접적으로 반영하여 역학 예측의 질을 향상시키는 데 초점을 맞추고 있습니다.

- **Technical Details**: CAPE는 환경적 요인과 데이터의 복잡한 관계를 해결하기 위해 자기 감독 학습(self-supervised learning) 및 계층적 환경 대조(hierarchical environment contrasting)를 통합한 프레임워크입니다. 또한 숨겨진 환경의 영향을 추정하기 위해 Expectation-Maximization 알고리즘을 활용하여 환경 모델링을 수행합니다. 이러한 기능은 제한된 관찰값으로도 정확한 예측이 가능하도록 돕습니다.

- **Performance Highlights**: CAPE는 다양한 평가 시나리오에서 평균 9.9%의 성능 향상(풀샷 시나리오)과 14.3%의 성능 향상(제로샷 시나리오)을 기록하며 기존의 모델들을 뛰어넘는 결과를 보여주었습니다. 연구진은 긴급 상황에서도 더 나은 의사 결정 지원과 질병 예측의 정확성을 향상시킬 수 있는 가능성을 제시하고 있습니다.



### Explain Yourself, Briefly! Self-Explaining Neural Networks with Concise Sufficient Reasons (https://arxiv.org/abs/2502.03391)
Comments:
          To appear in ICLR 2025

- **What's New**: 이번 연구에서는 모델의 예측을 위한 간결한 충분 이유(sufficient reason)를 생성하는 자가 지도 학습 방법인 *sufficient subset training* (SST)을 제안합니다. SST를 사용하면, 복잡한 사후(post-hoc) 방법에 비해 더 효율적이고 정확한 충분 subset을 생성할 수 있습니다. 이 논문은 그동안의 접근 방식이 가지던 주요 문제점들을 극복하려는 노력을 강조하고 있습니다.

- **Technical Details**: SST는 두 가지 출력을 갖는 모델을 훈련시키며, 하나는 예측 출력이고 다른 하나는 해당 예측의 충분 이유를 뽑아내는 것입니다. 이 훈련 과정에서는 두 개의 추가 최적화 목표를 포함하여, 선택된 subset이 예측출력에 대해 충분하도록 하고, subset의 크기도 최소화하도록 합니다. 다양한 마스킹 전략을 사용하며, 세 가지 종류의 충분 이유를 다룹니다.

- **Performance Highlights**: SST 방법은 이미지 및 언어 분류 작업을 위한 여러 모델 아키텍처에서 테스트되었으며, 생성된 충분 이유는 간결하고 신뢰성이 있음을 보여주었습니다. 비교 결과, SST는 유명한 사후 방법들과 함께 사용될 때 효율적인 성능을 유지하며, 예측 성능 또한 유사한 수준을 보여주었습니다.



### A Structured Reasoning Framework for Unbalanced Data Classification Using Probabilistic Models (https://arxiv.org/abs/2502.03386)
- **What's New**: 이번 연구는 비대칭 데이터(unbalanced data)에 대한 마르코프 네트워크 모델을 제안하며, 전통적인 기계 학습 모델의 분류 편향(classification bias)과 소수 클래스(minority class) 인식 부족 문제를 해결하고자 합니다. 이를 위해 공동 확률 분포(joint probability distribution)와 조건부 의존성(conditional dependency)을 구축하여 샘플 카테고리의 글로벌 모델링(global modeling)과 추론 최적화(reasoning optimization)를 달성합니다.

- **Technical Details**: 이 모델은 주변 확률 추정(marginal probability estimation)과 가중 손실 최적화(weighted loss optimization) 전략을 도입하여 정규화 제약(regularization constraints) 및 구조적 추론(structured reasoning) 방법과 결합했습니다. 이러한 접근 방식을 통해 모델의 일반화 능력(generalization ability)과 강건성(robustness)을 효과적으로 향상시킵니다.

- **Performance Highlights**: 실험 단계에서는 신용 카드 사기 탐지 데이터셋을 사용하여 로지스틱 회귀(logistic regression), 서포트 벡터 머신(support vector machine), 랜덤 포레스트(random forest), XGBoost와 같은 기존 모델들과 비교했습니다. 실험 결과 마르코프 네트워크는 가중 정확도(weighted accuracy), F1 점수(F1 score), AUC-ROC 등의 지표에서 우수한 성능을 보이며, 전통적인 분류 모델들보다 현저히 뛰어난 결정 능력과 비대칭 데이터 시나리오에서의 적용 가능성을 입증했습니다.
...
### SyMANTIC: An Efficient Symbolic Regression Method for Interpretable and Parsimonious Model Discovery in Science and Beyond (https://arxiv.org/abs/2502.03367)
Comments:
          Main and SI compiled into the pdf Main:48 pages, 7 figures SI: 29 pages, 2 figures

- **What's New**: 본 논문은 SyMANTIC이라는 새로운 기호 회귀(symbolic regression, SR) 알고리즘을 소개합니다. SyMANTIC은 상호 정보 기반 기능 선택(mutual information-based feature selection), 적응형 기능 확장(adaptive feature expansion), 재귀적으로 적용된 ℓ0 기반 희소 회귀(ℓ0-based sparse regression)를 통해 고차원 데이터에서 저차원 기술자를 효율적으로 식별합니다. 또한, 정보 이론적 측정을 사용하여 주어진 복잡도에 대해 최적의 정확도를 제공하는 Pareto-optimal 방정식 집합을 작성합니다.

- **Technical Details**: 이 연구는 기호 회귀가 특정 입력 특징에 대해 가장 적합한 수학적 표현을 자동으로 발견하는 과정임을 강조합니다. SyMANTIC은 약 10^5에서 10^{10} 이상의 후보 집합에서 저차원 기술자를 효과적으로 추출하며, 이는 처리 비용을 크게 줄입니다. 이 알고리즘은 PyTorch 생태계에서 구현되어 쉽게 설치하고 GPU 가속이 가능합니다.

- **Performance Highlights**: 실험을 통해 SyMANTIC이 표준 SR 방법들에 비해 비슷하거나 더 높은 정확도의 모델을 훨씬 낮은 비용으로 발견함을 보여주었습니다. 이 효율성은 합성 예제(synthetic examples), 과학적 벤치마크(scientific benchmarks), 현실 세계의 물질 특성 예측 등 다양한 문제에서 입증되었습니다. 결론적으로, SyMANTIC은 기호 회귀 연구에 있어 중요한 발전으로 평가됩니다.



### Rethinking Approximate Gaussian Inference in Classification (https://arxiv.org/abs/2502.03366)
Comments:
          29 pages, 15 figures

- **What's New**: 이번 연구에서는 softmax 함수를 통한 예측 확률 생성의 한계를 극복하기 위해, 정확한 예측 확률을 계산할 수 있는 새로운 학습 목표를 제안합니다. 이 방법은 Monte Carlo (MC) 근사 없이도 훈련 동역학을 개선하며, 소프트맥스와 요소별 정규 누적 분포 함수(normCDF), 시그모이드와 같은 여러 출력 활성화 함수와 호환됩니다. 또한, Dirichlet 분포를 통해 가우시안 푸시포워드를 근사화 할 수 있는 특징을 포함하고 있습니다.

- **Technical Details**: 모델은 입력 x에 대해 확률 측정값을 출력함으로써, 단일 점 추정이 아닌 분포적 불확실성(epistemic uncertainty)을 포착할 수 있도록 설계되었습니다. 이를 위해 가우시안 확률 측정값을 logit 공간을 통해 softmax를 사용하여 밀어 내는 구조를 따릅니다. 연구에서는 Laplace, HET, SNGP와 같은 여러 근사 가우시안 추론 방법과 결합된 우리의 접근 방식을 평가하였습니다.

- **Performance Highlights**: 제안된 방법은 ImageNet과 CIFAR-10을 포함한 대규모 및 소규모 데이터셋에서 softmax MC 샘플링과 비교할 때 불확실성 정량화에서 향상된 성능을 보여주었습니다. 이 방법은 복잡한 샘플링을 피하면서도 시간과 메모리의 오버헤드 없이 뛰어난 동작을 유지합니다. 관련된 코드도 공개되어 있어, 연구자들이 쉽게 접근하여 활용할 수 있도록 하고 있습니다.



### Scaling laws in wearable human activity recognition (https://arxiv.org/abs/2502.03364)
- **What's New**: 이번 연구는 wearable multimodal sensors에서의 사람 행동 인식(Human Activity Recognition, HAR)을 위한 첫 번째 스케일링 법칙(scaling laws)을 제시합니다. 기존 언어 및 비전 영역에서의 스케일링 법칙들은 잘 연구되었지만, HAR 분야에서는 이와 같은 연구가 부족했습니다. 본 논문에서는 대규모 공공 데이터셋을 활용하여 모델 용량(parameter count)과 데이터 양(data volume) 간의 관계를 구체적으로 규명하였습니다.

- **Technical Details**: HAR 시스템에서 인식 과정은 일반적으로 가속도계(accelerometers), 자이로스코프(gyroscopes) 및 그 조합으로 이루어진 여러 센서를 통한 시간적 데이터(time series data)를 바탕으로 동작합니다. 본 연구에서는 Vision Transformer (ViT) 아키텍처를 HAR에 적합하게 변형하고, Masked Autoencoder 프리트레이닝(pre-training) 기법을 적용하였습니다. 모델의 매개변수 수(parameter count)와 데이터 양(data volume) 간의 상관관계를 더욱 명확히 하기 위해, 훈련이 완료될 때까지 트레이닝을 진행하며 데이터 다양성과 유용성을 평가하였습니다.

- **Performance Highlights**: 테스트 결과, HAR의 다운스트림 성능은 UCI HAR, WISDM Phone, WISDM Watch의 세 가지 벤치마크 데이터셋에서 향상된 성능을 보였습니다. 이 연구는 사전 훈련(pre-training) 데이터의 질(quality)에 강조점을 두며, 동일한 사용자로부터 추가 데이터를 더하는 것보다 더 많은 사용자로부터 데이터를 확보하는 것이 성능 향상에 더 효과적임을 보여주었습니다. 이러한 결과는 향후 연구에서 모델 용량(model capacity)과 데이터 획득 전략(data acquisition strategy)의 최적화를 위한 중요한 참고 자료로 활용될 수 있습니다.
...
### Interaction-Aware Gaussian Weighting for Clustered Federated Learning (https://arxiv.org/abs/2502.03340)
- **What's New**: 이번 연구에서는 클러스터링된 연합 학습 (Clustered Federated Learning) 방법인 FedGWC를 제안합니다. FedGWC는 클라이언트의 데이터 분포에 기반하여 클러스터를 형성하고, 이를 통해 더욱 강력하고 개인화된 모델을 훈련합니다. 이 접근 방식은 데이터 이질성에 따른 부정적인 영향을 줄이면서도 개인 정보 보호를 통한 모델 훈련을 가능하게 합니다.

- **Technical Details**: FedGWC는 비슷한 데이터 분포를 가진 클라이언트들을 그룹화하는 방법을 적용합니다. 개별적인 경험적 손실을 변환하여 Gaussian 보상 메커니즘을 통해 클라이언트 상호작용을 모델링하여 동질적인 클러스터를 식별합니다. 또한, 연합 학습을 위한 새로운 클러스터링 지표인 Wasserstein Adjusted Score를 도입하여 클러스터 응집도를 평가합니다.

- **Performance Highlights**: 벤치마크 데이터 세트를 활용한 실험 결과, FedGWC는 기존의 연합 학습 알고리즘에 비해 클러스터 품질 및 분류 정확도에서 우수한 성능을 보였습니다. 이러한 결과는 제안된 방법의 효율성을 입증합니다.
...
### General Time-series Model for Universal Knowledge Representation of Multivariate Time-Series data (https://arxiv.org/abs/2502.03264)
- **What's New**: 이 논문은 다변량 시계열(MTS) 데이터의 보편적 지식 표현(universal knowledge representation)에 대한 문제를 다룹니다. 특별히 시계열의 다양한 시간 세분성(time granularity)이 주파수 도메인에서 독특한 결합 분포(joint distributions)를 나타낸다는 것을 발견하였습니다. 이는 이전 연구에서 간과한 중요한 요소로, 보편적 지식 학습에 필요한 차원인 것을 강조합니다.

- **Technical Details**: 본 연구에서는 Fourier knowledge attention 메커니즘을 도입하여 시계열 분석에서 시간 세분성을 인식하는 표현을 학습할 수 있도록 하고, 새로운 자기 회귀(blank infilling) 사전 훈련 구조를 통합하였습니다. 이를 통해 다양한 하위 작업에서의 적응성을 높이는 General Time-series Model(GTM)을 개발했습니다. GTM 모델은 시계열의 다양성을 다룰 수 있는 통합된 MTS 재단 모델로, 기존 방법의 한계를 극복합니다.

- **Performance Highlights**: GTM은 긴 기간 예측(long-term forecasting), 이상 탐지(anomaly detection), 수치 보간(imputation)과 같은 여러 생성 작업에서 최첨단(SOTA) 방법들을 초과하는 성능을 보였습니다. 연구 결과는 GTM이 거의 모든 분야에서 성능 향상을 입증하며, 보편적 지식 표현과 적응성을 향상시키는 데에 기여함을 나타냅니다.



### RiemannGFM: Learning a Graph Foundation Model from Riemannian Geometry (https://arxiv.org/abs/2502.03251)
Comments:
          Accepted by WWW25

- **What's New**: 이 논문은 그래프 구조를 위한 기초 모델(Graph Foundation Model, GFM)에 대한 새로운 접근 방식을 제안합니다. 전통적인 언어 또는 비전 도메인에서의 공유 어휘(shared vocabulary)에 착안하여, 그래프 도메인에서도 구조적 어휘를 도입했습니다. 이를 통해, 특정 텍스트 속성이 없는 그래프에서도 유용한 정보 전이를 가능하게 하려는 목표를 가지고 있습니다.

- **Technical Details**: 제안된 모델인 RiemannGFM은 리만 기하학(Riemannian geometry)의 원리를 활용하여 그래프 구조를 학습합니다. 이 모델은 다양한 기하학적 구조를 포함하는 새로운 제품 번들을 구축하여, 그래프의 구조적 어휘를 리만 다양체(Riemannian manifold)에서 학습합니다. RiemannGFM은 보편적인 리만 층(재난 아 카키 언층)에 기반하여 구성되어 있으며, 다양한 그래프에 대해 높은 일반화 능력을 보여줍니다.

- **Performance Highlights**: 광범위한 실험을 통해 RiemannGFM이 다양한 실제 그래프에 대한 교차 도메인 전이 학습(cross-domain transfer learning) 및 적은 샷 학습(few-shot learning)에서 뛰어난 성능을 보임을 입증했습니다. 이 연구는 구조적 기하학에 기반한 GFM의 가능성을 확장하여, 향후 그래프 데이터에 대한 보다 효과적인 학습 전략을 제시할 수 있을 것입니다.



### Calibrated Unsupervised Anomaly Detection in Multivariate Time-series using Reinforcement Learning (https://arxiv.org/abs/2502.03245)
Comments:
          This paper has been accepted for publication and presentation at the 2025 IEEE International systems Conference (SysCon)

- **What's New**: 본 논문에서는 오토인코더의 잠재 공간에서 강화 학습(reinforcement learning, RL)을 이용한 다변량 시계열 데이터의 비지도(anomaly detection) 이상 탐지를 조사합니다. 이상 데이터를 제한적으로 사용할 수 있는 문제를 극복하기 위해 RL을 활용하여 탐색(exploration)과 활용(exploitation)을 균형 있게 조절하여 오버피팅을 방지합니다. 또한, 웨이브렛 분석(wavelet analysis)을 통해 시계열 데이터를 시간 및 주파수 영역으로 분해하여 이상을 다양한 해상도에서 포착할 수 있습니다.

- **Technical Details**: 다변량 시계열 데이터는 여러 센서에서 측정한 데이터를 포함하며, 우리는 슬라이딩 윈도우(sliding window) 접근 방식을 통해 시간 순서에 따른 데이터를 겹쳐지는 하위 시퀀스로 나누어 처리합니다. 각 하위 시퀀스에 대해 웨이브렛 분석을 적용하여 이상 탐지를 개선하고, DWT(Discrete Wavelet Transform)를 통해 시계열 신호를 다중 해상도로 근사(coefficient)와 세부(coefficient)로 분해합니다. 이렇게 얻은 웨이브렛 계수를 바탕으로 일반 데이터 및 이상을 분류하는 데 필요한 정보를 추출합니다.

- **Performance Highlights**: 강화 학습 에이전트는 탐지 성능에 대한 피드백을 기반으로 결정 경계를 동적으로 조정하는 학습을 진행합니다. 합성(anomalies) 이상을 생성하여 감독(supervised) 프레임워크와 함께 융합함으로써 RL 에이전트가 개선된 결정을 내리도록 돕습니다. 이 접근법은 비지도 학습 과정에서 경계 조정을 정교하게 하여 정상 패턴과 비정상 패턴을 효과적으로 구별하는 모델의 능력을 강화합니다.



### Analysis of Value Iteration Through Absolute Probability Sequences (https://arxiv.org/abs/2502.03244)
Comments:
          8 pages

- **What's New**: 이번 연구에서는 Markov Decision Processes (MDPs)을 해결하기 위해 널리 사용되는 Value Iteration 알고리즘의 수렴성을 새로운 관점에서 분석합니다. 이전 연구들은 주로 infinity norm에 대한 수렴성에 집중했으나, 본 논문에서는 절대적 확률 수열을 활용하여 L² norm에 따른 수렴성을 분석합니다. 이를 통해 알고리즘의 행동과 성능에 대한 새로운 시각을 제공합니다.

- **Technical Details**: 연구는 유한 상태 집합과 유한 행동 집합을 포함하는 MDP의 일반적인 할인 무한 수명 설정을 따릅니다. 각 상태에서의 동작에 대한 보상과 전이 확률 분포를 설정한 뒤, 결정론적 정책을 고려하여 최적 정책을 찾기 위한 수식을 제시합니다. Value Iteration 알고리즘은 특정 초기 값을 가진 값의 시퀀스를 생성하고, 일정한 업데이트 룰을 통해 수렴성을 보장합니다.

- **Performance Highlights**: 이 논문에서 제안하는 수렴성 증명 기법을 통해 Value Iteration의 시퀀스가 L² norm 측면에서 고유한 최적 정책으로 수렴함을 보여줍니다. 또한, 고유 최적 정책이 불가분성과 비주기적 Markov 프로세스를 유도한다는 가정을 통해 최악의 상황을 피할 수 있음을 강조합니다. 이러한 새로운 분석 접근 방식은 Value Iteration 알고리즘의 효과성을 높일 수 있는 가능성을 제시합니다.



### Pioneer: Physics-informed Riemannian Graph ODE for Entropy-increasing Dynamics (https://arxiv.org/abs/2502.03236)
Comments:
          Accepted by AAAI25

- **What's New**: 이번 논문은 Riemannian geometry를 활용하여 물리 법칙을 반영한 동적 시스템 모델링을 제안합니다. 기존의 Euclidean 공간 중심의 모델링과 달리, 시스템의 본질적인 기하학을 고려하며, 엔트로피 증가 원칙을 포함한 더 현실적인 동적 모델링 문제를 제기합니다. 특히 이는 기존의 그래프 Ordinary Differential Equations (ODEs)을 확장한 것입니다.

- **Technical Details**: 제안된 모델은 Riemannian manifold 위에서 정의된 차별 방정식을 기반으로 하며, 물리 법칙에 부합하는 constrained Ricci flow와 시스템 기하학을 반영하는 manifold-preserving Gyro-transform을 포함합니다. Gyro-transform은 기존의 비싼 지수/로그 맵을 사용하지 않고 간단한 행렬-벡터 곱셈으로 이루어져 있습니다. 이는 모델이 엔트로피가 감소하지 않도록 보장합니다.

- **Performance Highlights**: Pioneer는 다양한 실제 데이터셋에 대한 실험에서 기존의 최첨단 방법들보다 뛰어난 성능을 보였습니다. 연구 결과는 제안된 방법이 시스템의 엔트로피와 기하학을 고려함으로써 더 정확한 예측 결과를 생성한다는 것을 보여주므로, 동적 시스템 모델링 분야에서의 변화를 예고합니다.
...
### Adversarial Dependence Minimization (https://arxiv.org/abs/2502.03227)
- **What's New**: 이 논문은 기계 학습에서 출력 특성 차원 간의 공분산(Covariance)을 최소화하여 중복이 최소화된 표현을 추출하는 방법에 대한 새로운 접근을 제시합니다. 기존의 방법들은 비선형 관계를 제거하지 않으며, 이를 해결하기 위한 미분 가능하고 확장 가능한 새로운 알고리즘을 제공합니다. 본 알고리즘은 작은 네트워크들이 특성 차원 간의 의존성을 식별하고, 인코더가 이를 활용하여 의존성을 축소하는 적대적 게임(Adversarial Game)을 사용합니다.

- **Technical Details**: 이 연구에서 제안하는 알고리즘은 신경망을 활용하여 학습된 임베딩 차원 간의 의존성을 줄이도록 설계되었습니다. 이 과정은 두 가지 플레이어 간의 적대적 게임으로 이루어지며, 첫 번째 플레이어는 하나의 차원을 예측하고, 두 번째 플레이어는 복원(counter reconstruction)을 방지하기 위해 표현을 업데이트합니다. 이 적대적 목표는 태스크 특화 최적화 문제 내에 소프트 독립 제약(Soft Independence Constraint)으로 통합할 수 있어, 다양한 학습 알고리즘에 활용될 수 있습니다.

- **Performance Highlights**: 이 알고리즘은 비선형 차원 축소(nonlinear decorrelation)와 같은 여러 응용 프로그램에 효과적임을 실증적으로 입증하였습니다. 또한 기존 PCA(Principal Component Analysis) 알고리즘의 확장을 통해 비선형 분산 최대화 및 이미지 분류 방법의 일반화 개선, 자가 감독 표현 학습(Self-Supervised Representation Learning)에서의 차원 붕괴 방지에 대한 유용성을 보여주었습니다. 이 논문은 차원 간의 의존성이 최소화된 균형 상태로 시스템적으로 수렴하는 알고리즘의 수학적 정당성을 증명합니다.



### SpaceGNN: Multi-Space Graph Neural Network for Node Anomaly Detection with Extremely Limited Labels (https://arxiv.org/abs/2502.03201)
- **What's New**: 이번 연구에서는 Node Anomaly Detection (NAD)에서 비유클리드적 공간의 잠재력을 탐구하고, 제한된 레이블을 통한 효율적인 데이터 처리를 위한 SpaceGNN 모델을 제안했습니다. 기존의 NAD 방법론이 대부분 단일 유클리드 공간 내에서 그래프를 임베딩하는 것과는 달리, SpaceGNN은 다양한 공간에서 노드를 효과적으로 표현할 수 있는 새로운 접근 방식을 제공합니다. 이러한 방식은 최신 실험을 통해 데이터 증강 기술보다 유의미한 이점을 드러냅니다.

- **Technical Details**: SpaceGNN은 세 가지 주요 구성 요소, 즉 Learnable Space Projection (LSP), Distance Aware Propagation (DAP), Multiple Space Ensemble (MulSE)을 포함합니다. LSP는 각 노드를 가장 적합한 공간에 임베딩하기 위한 학습 가능한 곡률을 사용하고, DAP는 다양한 공간 내에서의 거리를 반영하여 정보를 전파합니다. MulSE는 여러 공간에서의 모델 통합을 통해 NAD 작업에서 종합적인 정보를 효과적으로 추출합니다.

- **Performance Highlights**: 9개의 실제 데이터셋에 대한 광범위한 실험을 통해 SpaceGNN의 성능이 이전 최고 경쟁 모델보다 평균적으로 AUC 8.55% 및 F1 점수에서 4.31% 높은 성과를 기록했음을 확인했습니다. 이러한 결과는 SpaceGNN이 기존 방법론에 비해 더 뛰어난 성능을 발휘함을 입증하며, 다양한 공간을 활용한 NAD 작업의 효과를 강조합니다.



### PICBench: Benchmarking LLMs for Photonic Integrated Circuits Design (https://arxiv.org/abs/2502.03159)
- **What's New**: 이 논문에서는 Photonic Integrated Circuits (PICs) 디자인 생성을 자동화하기 위한 새로운 벤치마크 프레임워크인 PICBench를 소개합니다. PICBench는 대규모 언어 모델(LLM)을 활용하여 PIC 디자인 문제를 해결하며, 생성된 출력은 시뮬레이션 가능한 넷리스트(netlist) 형태로 제공됩니다. 또한, 이 벤치마크는 다양한 설계 난이도를 포함하는 여러 문제를 다루어, PIC 디자인의 자동화를 위한 기초 작업으로 자리 잡고 있습니다.

- **Technical Details**: PICBench는 24개의 정교하게 설계된 PIC 문제를 포함하며, 각 문제에 대한 정확한 디자인과 기능성을 검증하기 위한 기준 데이터(ground-truth)가 제공됩니다. SAX라는 오픈소스 시뮬레이터를 활용하여, LLM이 생성한 결과물의 조건을 자동으로 평가하여, 문법(syntax)과 기능(functionality)을 모두 측정합니다. 이 과정에서 모든 생성된 넷리스트는 SAX의 요구 사항을 준수하도록 설계되어 있습니다.

- **Performance Highlights**: PICBench는 기존 LLM들의 성능을 블렌치 마크하며, 자동화된 PIC 디자인 생성에 있어의 가능성과 도전 과제를 드러냅니다. 연구 결과, 이 프레임워크는 문법과 기능 검사에서의 통과 여부를 기준으로 평가를 수행하며, LLM 기반 솔루션의 성능 확장성을 평가하는 데 중요한 역할을 합니다. 나아가, 이 연구는 PIC 디자인 자동화의 최적화를 위한 추가 연구 필요성을 강조합니다.



### Symmetry-Aware Bayesian Flow Networks for Crystal Generation (https://arxiv.org/abs/2502.03146)
- **What's New**: 이번 연구에서는 새로운 결정 물질 생성을 위한 대칭 인식 베이지안 흐름 네트워크(SymmBFN)을 도입합니다. 이 모델은 실험적으로 관찰된 결정에서 발견되는 공간군의 분포를 정확하게 재현할 수 있습니다. SymmBFN은 기존 방법에 비해 안정적인 구조를 최소 50배 더 빠르게 생성할 수 있는 효율성을 개선했습니다. 또한, 물질의 맞춤형 속성을 설계할 수 있는 능력을 증명하여 결정 물질 발견을 가속화할 수 있는 도구로서 BFNs의 가능성을 확인했습니다.

- **Technical Details**: SymmBFN은 결정 구조에 대한 대칭 인식을 활용하여, 분수 좌표(fractional coordinates), 원자 유형(atom types), 단위 셀 매개변수(unit cell parameters), 및 자리 대칭(site symmetries) 등의 분포를 통합하여 모델링할 수 있습니다. 이 방법은 실제 재료에서 관찰된 공간군의 분포를 정확하게 재현하며, 생성 프로세스를 심각하게 향상시킵니다. 또한, SymmBFN은 기존의 확산 기반 모델에 비해 속도 개선을 두 배 이상 달성합니다.

- **Performance Highlights**: 여기서 제안된 SymmBFN은 기존 모델에 비해 결정 구조 생성을 위한 고품질 샘플을 생성할 때 필요한 통합 단계의 수를 현저히 줄이며, 계산 비용을 감소시킵니다. 특히, 분포 모델링의 통합된 프레임워크 덕분에 다중 변수를 균일하게 처리할 수 있는 장점이 있습니다. 이로 인해, 결정 물질 설계의 효율성이 크게 향상되었습니다.



### Machine Learning-Driven Student Performance Prediction for Enhancing Tiered Instruction (https://arxiv.org/abs/2502.03143)
- **What's New**: 이 연구는 기계 학습을 이용한 학생 성과 예측 결과와 단계적 교육(tiered instruction)을 통합함으로써 교육 데이터 마이닝의 최신 응용을 제시합니다. 연구의 목표는 최적의 기계 학습 방법을 통해 학생들에게 맞춤형 교육 전략을 제공하고, 학습 성과를 향상시키는 것입니다. 이를 통해 교육의 실제 상황에서 기계 학습의 효과적인 활용을 탐구할 수 있는 새로운 연구 방향을 신설했습니다.

- **Technical Details**: 연구에서는 원본 교육 데이터를 수집하고, 정보 중복을 줄이기 위해 피처 선택(feature selection) 과정을 거칩니다. 여러 기계 학습 알고리즘의 성능을 비교한 결과, Random Forest가 가장 우수한 성능을 보였으며, 이 모델을 사용해 학생들을 분류하고 단계적 교육법을 적용합니다. 이후 교육 성과를 제어 집단과 비교하여 단계적 교육의 효과를 검증합니다.

- **Performance Highlights**: 실험 결과, 단계적 교육을 적용한 실험 집단의 학습 성과가 제어 집단보다 유의미하게 향상되었습니다. 이 연구는 기계 학습 기반 예측 결과와 교육 방법론을 연계하여 효과적으로 향상된 교육 전략을 제시하며, 이는 컴퓨터 과목에 국한되지 않고 다양한 과목에 적용할 수 있는 포괄적인 프레임워크로서 가치가 있습니다.
...
### Multi-objective methods in Federated Learning: A survey and taxonomy (https://arxiv.org/abs/2502.03108)
- **What's New**: 본 연구는 Federated Learning (FL)과 다중 목표 최적화(multi-objective optimization, MOO) 분야 간의 연계를 다룬 최초의 체계적 개요를 제공합니다. 이 두 분야의 통합 방법을 제안하고 실태에 대한 포괄적인 검토를 통해 공헌을 체계적으로 분류하는 새로운 분류 체계를 제시합니다. 또한, 현재의 연구 방향과 향후 추가 연구의 가능성을 탐색하며, 다양한 주제를 포함하는 여러 연구 과제를 제시합니다.

- **Technical Details**: FL은 데이터를 중앙 집중화하지 않고 다양한 클라이언트에서 기계 학습 모델을 훈련하는 분산 머신러닝 패러다임입니다. 이는 각 클라이언트가 개별적으로 학습한 후 모델을 주기적으로 집계하여 글로벌 모델을 생성하는 방식으로 작동합니다. 반면, MOO는 여러 목표에 대한 문제를 동시 해결하며, 이는 클라이언트 각각의 특성이 상충할 수 있는 문제를 다룰 수 있게 해줍니다. 본 논문은 FL과 MOO의 통합을 위한 최초의 분류 체계를 제공합니다.

- **Performance Highlights**: 이 연구는 FL과 MOO의 결합을 통해 접근하는 데 있어 명확한 기준을 설정합니다. 여러 클라이언트 간의 불균형 문제와 계산 자원 소비 문제를 해결할 수 있는 전략을 제시하여, 향후 연구에 기초를 제공합니다. 또한, 모델 효율성과 공정성을 동시 충족시키기 위한 다양한 접근 방식을 기존 연구를 바탕으로 논의하여 향후 발전 방향을 제안합니다.
...
- **What's New**: 이 논문에서는 최신의 Large Language Models (LLMs)와 관련하여 Reinforcement Learning from Human Feedback (RLHF) 알고리즘의 발전을 다룹니다. 특히, actor-critic 방식의 Proximal Policy Optimization (PPO)와 Direct Preference Optimization (DPO) 간의 차이를 분석합니다. 이러한 분석을 통해 DPO를 RL 알고리즘으로 분류할 수 있는지에 대한 혼란을 해소하려고 합니다.

- **Technical Details**: 논문에서는 DPO와 RL 알고리즘의 관계를 이해하기 위해 세 가지 주요 요소에 집중합니다: (1) 손실 함수(loss function)의 구성, (2) 알고리즘이 수렴하는 목표 분포(target distribution), (3) 손실 함수 내 주요 구성 요소의 영향. 이를 위해 'Unified DPO-RL Relationship Analysis (UDRRA)'라는 통합 프레임워크를 설정하고, 이 프레임워크 내에서 DPO의 목표 정책 분포(target policy distributions)를 밝혀냅니다.

- **Performance Highlights**: 마지막으로, DPO의 주요 구성 요소들이 수렴 속도(convergence rate)에 미치는 영향을 조사하여 RLHF 알고리즘의 개선 방안에 대한 새로운 통찰을 제공합니다. 이 연구는 DPO, RL 및 기타 RLHF 알고리즘 간의 관계를 더 깊이 이해할 수 있는 기회를 제공합니다.
...
### Optimal Best Arm Identification with Post-Action Contex (https://arxiv.org/abs/2502.03061)
Comments:
          37 pages, 7 figures

- **What's New**: 이 논문은 행동 후 맥락(post-action context)을 고려한 최적 팔 식별(best arm identification, BAI)의 새로운 문제를 제시한다. 이 문제는 학습자가 각 행동을 수행한 후 보상과 함께 추가 정보를 수신하는 경우를 다룬다. 이러한 행동 후 맥락은 의사 결정 과정에 큰 도움을 줄 수 있다.

- **Technical Details**: 행동 후 맥락은 비분리(non-separator) 및 분리(separator) 두 가지 유형으로 나누어 분석된다. 비분리 상황에서는 보상이 행동과 맥락 모두에 의존하며, 분리 상황에서는 보상이 오직 맥락에만 의존한다. 저자들은 두 경우에 대해 샘플 복잡성(sample complexity)에 대한 하한을 도출하고 최적 샘플 복잡성을 달성하는 알고리즘을 제안한다.

- **Performance Highlights**: 실험 결과는 제안된 방법들이 기존 기법들에 비해 우수한 성능을 보임을 보여준다. 특히, 비분리 설정에서는 Track-and-Stop 알고리즘을 확장하는 방식으로 접근하며, 분리 설정에서는 G-tracking이라는 새로운 샘플링 규칙을 제안한다. 이러한 결과는 행동 후 맥락이 최적 팔 선택 전략에 미치는 영향을 강조한다.
...
- **What's New**: 이번 연구에서는 Jailbreaking 공격이 오픈 소스 대형 언어 모델(LLM)에 효과적으로 작용하지만, 이러한 공격이 상용 LLM에는 일관된 영향을 미치지 않는다는 점을 강조합니다. 연구진은 공격의 이식 가능성을 분석하고, 악의적인 의도(ill-intent tokens)를 인식하는 모델의 인식을 방해하는 공격 방법을 제시합니다. 이를 통해, 기존의 공격 방법이 상용 LLM에서의 효과가 제한적이라는 사실을 밝히고, 새로운 접근법인 Perceived-importance Flatten (PiF) 방법을 개발 하였습니다.

- **Technical Details**: PiF 방법은 모델의 초점을 악의적인 의도 토큰에서 중립적인 의도 토큰으로 고르게 분산시킵니다. 이 접근 방식은 오버피팅된 적대적 시퀀스에 의존하지 않으며, 대안적인 방법으로 동의어 대체를 사용할 수 있습니다. 연구팀은 이러한 기법을 통해 모형의 인지 변화에 기반한 동적 최적화를 시행하며, 적대적 공격 생성 시 고정된 목표 대신 변화를 반영하는 목표를 사용하는 방식을 도입하였습니다.

- **Performance Highlights**: PiF는 다양한 상용 LLM, 데이터셋에서 효과적으로 평가되었으며, 공격이 시도된 LLM을 성공적으로 평가하는 데 도움이 됩니다. 연구를 통해, PiF가 구성하는 공격이 직면하는 모델의 공격 인식(safety guardrails)을 우회할 수 있는 가능성을 보여주었습니다. 따라서, PiF는 상용 LLM의 취약점을 효과적이고 효율적으로 평가할 수 있는 방법으로 자리잡고 있습니다.



### The Ensemble Kalman Update is an Empirical Matheron Upda (https://arxiv.org/abs/2502.03048)
- **What's New**: 본 논문에서는 Ensemble Kalman Filter (EnKF)의 앙상블 업데이트 단계가 Gaussian process regression에서 인기가 있는 Matheron 업데이트의 경험적 버전과 동등하다는 것을 보여줍니다. 이 두 방법의 연결이 잘 알려지지 않았으며, 서로 다른 문헌에서도 이들 간의 관계가 활용되지 않았음을 강조합니다. 저자들은 이 연결성을 통해 EnKF에 대한 새로운 확률적 기초를 마련하고, Matheron 업데이트의 속성을 활용하여 앙상블 데이터 동화 기술을 향상시킬 수 있는 가능성을 제시합니다.

- **Technical Details**: EnKF는 대규모 동적 시스템에서 데이터 동화(data assimilation) 방법으로 널리 사용되며, 모델 동역학에 따라 상태 벡터의 앙상블을 진화시키고 관측 데이터를 사용하여 이를 업데이트합니다. 반면, Matheron 업데이트는 관측치에 조건화된 Gaussian 랜덤 변수의 샘플링을 위한 기법으로, 후방 공분산을 명시적으로 계산하지 않고도 가능하다고 합니다. 저자들은 이러한 두 방법을 통합하여 경험적 Matheron 업데이트와 앙상블 업데이트 간의 동등성을 입증합니다.

- **Performance Highlights**: EnKF의 앙상블 업데이트 과정과 Matheron 업데이트의 연계를 통해, 데이터 동화 커뮤니티에서 발생한 많은 계산 최적화를 활용할 수 있는 잠재력이 있습니다. 이 연구는 EnKF와 Matheron 방법론의 교차점에서 발생할 수 있는 다양한 기회를 제안하며, 이는 통계학과 머신러닝 분야에서 더욱 효율적인 샘플링 및 추정 기술을 개발하는 기반이 될 것입니다.



### RepLoRA: Reparameterizing Low-Rank Adaptation via the Perspective of Mixture of Experts (https://arxiv.org/abs/2502.03044)
- **What's New**: 본 논문은 Low-rank adaptation (LoRA)의 이론적 분석을 통해 Mixture of Experts 모델과의 연결성을 탐구합니다. 간단한 reparameterization 기법을 통해 LoRA 행렬의 성능을 향상시키고, 필요한 데이터 양을 제곱 규모로 줄이는 방법을 제시합니다. 이를 통해 제안된 Reparameterized Low-rank Adaptation (RepLoRA)은 LoRA보다 더 효율적으로 작동함을 보여줍니다.

- **Technical Details**: LoRA는 매우 큰 규모의 모델을 효율적으로 조정할 수 있는 기법으로, 최신 연구에서는 주목적이 되는 기법으로 평가받고 있습니다. 본 연구에서는 LoRA의 성능을 높이기 위한 새로운 reparameterization 방안을 제안을 하는데, 저중량 미니멀 프레임워크를 사용하여 low-rank 행렬을 재정의합니다. 이로 인해 LoRA의 샘플 효율성을 이론적으로 높일 수 있음을 입증합니다.

- **Performance Highlights**: RepLoRA는 여러 분야에서 실험한 결과, 일반 LoRA보다 최대 40% 성능 향상을 이끌어내며, 30%의 훈련 데이터만으로도 LoRA와 동등한 성능을 달성했습니다. 이는 이론적 기반과 실험적 결과 모두에서 RepLoRA의 효과성을 입증합니다. 또한, 훈련 후 reparameterization에 사용된 MLP는 제거하여 추론 시 효율성을 유지합니다.



### Aggregate to Adapt: Node-Centric Aggregation for Multi-Source-Free Graph Domain Adaptation (https://arxiv.org/abs/2502.03033)
Comments:
          Accepted by WWW-2025

- **What's New**: 본 논문은 멀티 소스 프리 비지도 그래프 도메인 적응(Multi-Source-Free Unsupervised Graph Domain Adaptation) 문제를 처음으로 다룹니다. 이는 레이블이 없는 타겟 도메인에 여러 소스 도메인으로부터 지식을 이전하는 실용적이지만 탐색되지 않은 설정입니다. 기존 방법들과는 달리, 본 연구는 레이블이 있는 소스 그래프 없이도 작동할 수 있는 방법론을 제안합니다.

- **Technical Details**: 제안된 GraphATA는 각 노드의 로컬 컨텍스트 정보를 기반으로 개별 그래프 컨볼루션 매트릭스를 동적으로 파라미터화하는 노드 중심 적응 프레임워크입니다. 이 방법은 그래프 구조 데이터에 따라 유연하게 적응할 수 있는 최적의 가중치 매트릭스를 각 노드에 할당합니다. Sparse constraints를 적용하여 관련 없는 정보를 필터링함으로써 불필요한 노이즈를 줄입니다.

- **Performance Highlights**: 광범위한 공개 데이터셋을 사용한 실험 결과, GraphATA는 최신 최첨단 기준을 지속적으로 초월하는 성능을 보여주었습니다. 다양한 데이터셋에 대한 초세부 실험(ablation studies)을 통해 노드 중심 적응의 효과성을 추가적으로 검증하였습니다.
...
- **What's New**: LLaMA-Adapter는 최근 효율적인 fine-tuning 기법으로 주목받고 있으며, zero-initialized attention을 활용해 훈련을 안정화하고 성능을 향상시킵니다. 이 기법은 경험적으로 성공을 거두었지만, zero-initialized attention의 이론적 기초는 아직 많이 탐구되지 않았습니다. 이번 연구에서는 zero-initialized attention과 mixture-of-expert 모델 간의 연결성을 규명하며, 비선형 프롬프트가 주는 유연성도 강조합니다.

- **Technical Details**: 연구팀은 zero-initialized attention을 mixture-of-expert 모델의 특수한 형태로 해석하며, 선형 프롬프트 설정 하에 프롬프트와 게이팅 파라미터를 최적 추정할 수 있는 가능성을 보여주었습니다. 또한, 비선형 프롬프트의 경우에도 최적 추정이 가능함을 입증하여 향후 응용 가능성을 높였습니다. 이러한 이론적 분석은 다양한 데이터셋에서 zero-initialized attention이 날무 랜덤으로 초기화된 attention보다 항상 우수하다는 empirical findings로 뒷받침됩니다.

- **Performance Highlights**: 연구 결과, 비선형 프롬프트는 선형 프롬프트보다 더 뛰어난 성능을 보였으며, 제한된 훈련 데이터에서도 프롬프트 유형이 vanilla attention을 일관되게 초과했습니다. 이러한 결과는 zero-initialized attention의 견고함과 적응력을 강조하며, LLaMA-Adapter의 데모에서 주목할 만한 성과를 거두었습니다. 궁극적으로, 다양한 질문-응답 데이터셋에 대한 실험을 통해 연구의 이론적 통찰력이 검증되었습니다.



### Parametric Scaling Law of Tuning Bias in Conformal Prediction (https://arxiv.org/abs/2502.03023)
- **What's New**: 이 연구는 많은 적합한 예측 방법에서 파라미터 조정에 대한 새로운 현상인 튜닝 편향(tuning bias)을 규명하였습니다. 튜닝 편향은 동일한 데이터 세트를 튜닝과 검증에 사용하는 데서 나타나는 커버리지 격차로, 특정 상황에서 미미한 영향을 미친다는 것을 발견했습니다. 특히, 파라미터 공간의 복잡성에 따라 이 편향이 증가하거나 감소함을 보여주는 스케일링 법칙을 증명하였습니다.

- **Technical Details**: 제안된 이론적 프레임워크는 조정된 경험적 위험 최소화(constrained ERM)를 통해 튜닝 편향을 정량화합니다. 튜닝 편향의 upper bound를 도출하여 유한 및 무한 파라미터 공간에서의 튜닝 편향을 rigorously 증명하였습니다. 이러한 이론은 또한 RAPS, 점수 집계(score aggregation), 신뢰도 조정(confidence calibration)과 같은 여러 적합한 예측 방법의 튜닝 편향을 평가하는 데 사용됩니다.

- **Performance Highlights**: 연구 결과에 따르면 대부분의 적합한 예측 방법에서 느린 편향은 적은 영향을 미치며, 벡터 스케일링과 ConfTr의 미세 조정 버전만 예외로 나타났습니다. 특히, 얼마나 많이 파라미터 공간의 복잡성이 높아질수록 튜닝 편향이 증가하고, 검증 세트의 크기가 증가하면 편향이 감소함을 확인했습니다. 이러한 발견은 문제 해결을 위한 신뢰도 조정 방식들을 적용하는 데 유용할 수 있습니다.
...
### Conformal Uncertainty Indicator for Continual Test-Time Adaptation (https://arxiv.org/abs/2502.02998)
- **What's New**: 이번 논문에서 제안된 Conformal Uncertainty Indicator (CUI)는 테스트 중 문제 도메인의 비틀림을 극복하며 지속적인 테스트 시 적응 (CTTA)의 성능을 향상시키기 위한 새로운 방법론입니다. CUI는 Conformal Prediction (CP)을 활용하여 예측 집합을 생성함으로써 잘못된 의사결정으로 인한 성능 저하 문제를 해결합니다. CUI는 도메인 및 데이터 차이를 측정하여 신뢰할 수 있는 퍼짐도(coverage)를 보장하며, 비효율적인 의사결정의 악순환을 개선하고 있습니다.

- **Technical Details**: CUI는 기존의 CP 방법과 달리, 훈련이 완료된 사전 모델에 대한 훈련된 라벨을 가진 정적 출처 보정 세트를 활용합니다. 테스트 중에는 도메인 변화로부터 불확실성을 평가하고, 보정 세트에 따라 샘플의 분위수를 계산하여 불확실성을 보강합니다. 최종적으로, 조정된 분위수로부터 비일치(non-conformity) 임계값을 유도하여 예측 집합을 생성하고 그 크기는 불확실성 수준을 반영하도록 설계되었습니다.

- **Performance Highlights**: 실험 결과, CUI는 다양한 benchmark 데이터셋에서 기존의 CTTA 방법들에 비해 테스트 불확실성을 더 잘 평가할 수 있음을 보여주었습니다. CP 기반의 적응 전략을 통합함으로써 동적이고 불확실한 테스트 환경에서 모델의 예측 신뢰성 및 강인성을 증대시키는 성과를 확인했습니다. 따라서 CUI는 기존의 여러 CTTA 방법들을 보완하며 전반적인 성능 향상에 기여할 수 있습니다.
...
- **What's New**: 이 논문에서는 기존의 membership inference attack (MIA) 전략이 distilled generative models에 대해 실패한다는 점을 강조하고 있습니다. 학생 모델이 교사 모델로부터 학습할 때, 특정 훈련 인스턴스를 식별할 수 없는 이유는 학생이 원본 데이터가 아니라 교사의 출력 데이터로부터 학습하기 때문입니다. 결과적으로 MIA의 접근 방식이 인스턴스 수준에서 분포 수준으로 변화해야 한다고 주장합니다.

- **Technical Details**: 학생 모델에서 훈련 인스턴스의 멤버십 정보가 부족함을 발견했습니다. 이는 각 인스턴스를 개별적으로 기억하는 것이 아닌, 데이터 분포 집합에 대한 상대적인 통계로서 분석할 수 있음을 시사합니다. 이를 위해 D-MIA라는 새로운 MIA 프레임워크를 도입하고, 특정 후보 데이터셋과 학생 생성 데이터 간의 분포적 거리를 측정합니다.

- **Performance Highlights**: D-MIA는 실험을 통해 교사의 멤버 데이터와 비회원 데이터 간의 분포적 불일치를 성공적으로 식별할 수 있음을 입증했습니다. 이 방법은 특히 교사 모델의 데이터가 포함된지 여부를 확인하는 데 효과적이며, 개인 데이터의 비밀 보호 측면에서도 안전성을 강화합니다. 논문은 D-MIA의 한계와 향후 연구 방향도 논의합니다.



### Direct Distributional Optimization for Provable Alignment of Diffusion Models (https://arxiv.org/abs/2502.02954)
- **What's New**: 본 논문에서는 확산 모델의 정렬 방법을 제안하며, 배급 최적화의 관점에서 새로운 접근방식을 소개합니다. 이 방법은 샘플링 오류에 대한 종합적인 경계와 엄격한 수렴 보장을 제공합니다. 기존의 방법들과 달리 우리의 방법은 다중 모드 분포에서 직접 샘플링을 가능하게 하며, 이는 Diffusion 모델의 정렬 문제를 해결하는 데 중요한 기여를 합니다.

- **Technical Details**: 제안된 방법은 기초 모델의 정규화된 손실 최소화 문제를 해결하고, Dual Averaging 기법을 통해 확률 분포를 직접 최적화합니다. Doob의 h-변환 기법을 사용하여 학습된 분포에서 샘플링을 가능하게 합니다. 이 프레임워크는 RLHF, DPO, KTO와 같은 일반 배급 최적화 문제에 적용 가능하며, 복잡하고 다중 모드인 밀도 문제를 해결할 수 있도록 합니다.

- **Performance Highlights**: 우리는 합성 데이터 세트와 이미지 데이터 세트에서 DPO 목표를 사용하여 성능을 실험적으로 검증하였습니다. 제안된 방법은 수렴 보장 제시 뿐 아니라, 다양한 정렬 작업에서의 뛰어난 적용 가능성을 보여줍니다. 이 연구는 일반적인 배급 최적화 문제에 대한 향후 연구 방향에 대한 중요한 통찰을 제공합니다.



### Fast T2T: Optimization Consistency Speeds Up Diffusion-Based Training-to-Testing Solving for Combinatorial Optimization (https://arxiv.org/abs/2502.02941)
Comments:
          Published at NeurIPS 2024, the implementation code is available at this https URL

- **What's New**: 이 논문에서는 최적화 문제 해결을 위한 새로운 접근 방법인 Fast T2T를 제안합니다. 기존의 Diffusion 모델의 느린 샘플링 과정을 개선하여, 다양한 노이즈 수준에서 최적의 솔루션으로의 직접 매핑을 학습함으로써 빠른 솔루션 생성을 가능하게 합니다. 이를 통해 최소한의 샷(shot)으로도 고품질 결과를 생성할 수 있습니다.

- **Technical Details**: Fast T2T는 노이즈 수준에 따라 최적 솔루션으로 이동하는 과정을 최적화 일관성(training consistency) 모델을 통해 학습합니다. 또한, 테스트 단계에서는 새로운 일관성 기반의 경량 경량 그라디언트 탐색(gradient search) 방안이 도입되어, 실제 문제 해결 시의 탐색 효율을 높입니다. 이 모델은 기존의 Diffusion 모델 대비 반복 단계 수를 크게 줄여, 효율적인 최적화 문제 해결을 가능케 합니다.

- **Performance Highlights**: 실험 결과, Fast T2T는 Traveling Salesman Problem(TSP)와 Maximum Independent Set(MIS) 과제에서 기존의 최첨단 SOTA 모델보다 우수한 성능을 보였습니다. 특히, Fast T2T는 단일 단계 생성 및 단일 단계 그라디언트 탐색으로도 여러 단계가 요구되는 기존 모델들보다 수십 배 빠른 속도를 달성했습니다. 이로 인해 Fast T2T는 최적의 솔루션을 더 빠르고 효율적으로 도출할 수 있습니다.
...
### Robust Reward Alignment in Hypothesis Spac (https://arxiv.org/abs/2502.02921)
Comments:
          17 pages, including appendix

- **What's New**: 이번 연구에서는 보상 설계(reward design)를 위한 새로운 방법론을 제안합니다. 특히, 인간의 선호(preference)를 기반으로 한 보상 정렬(reward alignment) 방법이 기존 방법보다 더 견고하고 효율적이라는 점에 주목했습니다. 이는 '가설 공간 배치 절단(hypothesis space batched cutting)'이라는 새로운 관점을 통해 이루어집니다.

- **Technical Details**: 제안된 방법은 인간의 선호를 기반으로 한 "컷(cuts)"을 통해 보상 가설 공간을 반복적으로 정제합니다. 각 배치(batch)에서, 불일치(disagreement)를 고려하여 그룹화된 인간의 선호는 투표(voting) 기능을 통해 적절한 컷을 결정합니다. 이를 통해 잘못된 인간 선호가 보상 가설 공간에 과도한 영향을 미치지 않도록 보수적인 절단(conservative cutting) 방법이 도입되었습니다.

- **Performance Highlights**: 모델 예측 제어(model predictive control) 환경에서 diverse한 과제를 다루며 제안된 방법의 성과를 평가하였습니다. 결과적으로, 제안된 프레임워크는 오류가 없는 환경에서 최첨단(state-of-the-art) 방법과 비교 가능한 또는 우수한 성능을 보였으며, 높은 비율의 잘못된 인간 선호를 처리할 때 기존 방법에 비해 훨씬 향상된 성능을 보여주었습니다.
...
- **What's New**: 본 논문에서는 사전 훈련 단계에서 개인 정보 유출을 사전 평가하기 위한 새로운 접근 방식을 제안합니다. 특히, 'privacy tokens'(프라이버시 토큰)이라 불리는 개념을 도입하여 훈련 과정 중 기밀성을 측정할 수 있는 방법을 제공합니다. 이는 기울기(gradient) 정보를 직접 활용하여 개인 정보 유출을 실시간으로 모니터링할 수 있습니다.

- **Technical Details**: 제안된 프라이버시 토큰은 훈련 중 기울기 정보로부터 유도되어, 훈련 데이터와 기울기 간의 관계를 정량적으로 평가하는 데 유용합니다. 이를 위해 'Mutual Information'(상호 정보량)이라는 지표를 이용하고, MINE(상호 정보량 신경 추정기)를 통해 훈련 과정에서의 개인 정보 유출을 지속적으로 모니터링하는 프레임워크를 구현합니다.

- **Performance Highlights**: 실험 결과, 프라이버시 토큰과 상호 정보량이 개인 정보 유출을 효과적으로 식별하고 정량화할 수 있음을 보여줍니다. 이 연구는 민감한 데이터가 포함된 딥러닝 모델의 안전한 배포를 촉진하며, 훈련 과정에서 개인 정보 보호를 위한 혁신적인 방법론을 제시합니다.
...
### PH-VAE: A Polynomial Hierarchical Variational Autoencoder Towards Disentangled Representation Learning (https://arxiv.org/abs/2502.02856)
Comments:
          15 pages,14 figures

- **What's New**: 이 논문에서는 다항 계층 변분 자동 인코더(Polynomial Hierarchical Variational Autoencoder, PH-VAE)를 제안한다. 기존의 변분 자동 인코더(VAE)가 갖고 있는 여러 단점을 해결하기 위해 다항 계층 데이터 형식을 사용하여 데이터 분포를 생성하거나 재구성한다. 또한, 제안된 PH-VAE는 Kullback-Leibler(KL) 발산 대신 새로운 다항 발산을 도입함으로써 재구성의 정확성과 이미지 품질을 크게 향상시킬 수 있음을 보인다.

- **Technical Details**: PH-VAE는 여러 인코더 레이어를 동시에 처리하면서 통합된 디코더로 출력을 통합하는 구조를 갖는다. 이 구조는 재구성 정확도를 높이고 정보가 풍부한 잠재 공간을 생성하여 후방 붕괴(posterior collapse)를 완화하는 데 초점을 맞추고 있다. 데이터 샘플은 실험 측정 또는 실제 응용에서 나오는 데이터로 구성되며, 이를 정규화하여 [0,1] 범위로 변환한 후 다항 특징으로 변환한다.

- **Performance Highlights**: 제안된 PH-VAE는 강화된 데이터 재현성과 데이터 생성 능력을 가진 단독 비지도 학습(unsupervised disentangled learning)을 실현한다. 그 결과 더 복잡한 관계를 포착할 수 있으며, 원본 데이터 세트 크기는 그대로 유지하면서도 데이터의 세밀한 해상도를 유지하는 성능을 보여준다. 이 연구는 PH-VAE가 데이터 분포가 복잡한 데이터 세트에 대해 우수한 재구성과 생성 효과를 제공할 수 있음을 입증한다.
...
### Slowing Learning by Erasing Simple Features (https://arxiv.org/abs/2502.02820)
- **What's New**: 이번 연구에서는 QLEACE라는 새로운 개념 삭제 방법을 도입하였습니다. 이 방법은 개념에 대한 모든 2차 정보를 representation에서 제거하는 방식으로 작동합니다. 연구팀은 LEACE와 QLEACE를 비교하여, 데이터셋에서 낮은 차수 통계정보가 제거되었을 때 네트워크가 여전히 학습할 수 있는지를 조사하였습니다.

- **Technical Details**: 우리는 QLEACE를 통해 높은 차수의 Shannon 정보를 사용하여 클래스 레이블에 대한 정보를 주입하는 것이 가능한 반면, ALF-QLEACE는 클래스 레이블에 관계없이 모든 데이터 포인트에 동일한 변환을 적용하여 정보를 유지합니다. Prequential minimum description length (MDL) 프레임워크를 사용하여 삭제된 데이터에서 학습의 "어려움"을 측정하였고, LEACE 방법이 학습을 일관되게 어렵게 만드는 결과를 보였습니다.

- **Performance Highlights**: 결과적으로 QLEACE는 피드포워드 아키텍처에서 학습 속도를 일관되게 저하시켰으나, 복잡한 아키텍처는 클래스 레이블에 대한 추가 정보를 활용하여 더 나은 성능을 발휘했습니다. 또한, ALF-QLEACE의 경우 일부 데이터셋에서 데이터 증강 기술로 작용하여 LEACE와 비교하여 학습 속도를 증가시키는 흥미로운 결과를 얻었습니다.
...
### When Machine Learning Gets Personal: Understanding Fairness of Personalized Models (https://arxiv.org/abs/2502.02786)
Comments:
          35 pages, 9 figures, submitted to ICML 2025

- **What's New**: 이번 연구는 개인화 (Personalization)가 예측 정확도와 설명 품질에 미치는 영향을 평가하기 위한 통합 프레임워크를 제안합니다. 이 프레임워크는 분류와 회귀 작업 모두에서 개인 속성의 사용의 최대 한계를 도출합니다. 특히, 회귀 모델이 분류 모델보다 더 많은 개인 속성을 사용할 수 있음을 보여주며, 이는 개인화의 혜택을 검증하는 데 중요한 통찰력을 제공합니다.

- **Technical Details**: 개인화는 사용자의 고유한 특성을 ML 모델의 입력으로 맞춤 설정하는 과정입니다. 그러나 개인화가 예측 정확도를 향상시킬 수 있는 것처럼, 설명 품질 저하라는 위험도 내포되어 있습니다. 본 연구는 ML 모델의 개인화가 두 가지 근본적인 목표, 즉 정확한 예측 달성과 명확한 설명 제공이라는 측면에서 어떻게 영향을 미치는지를 중점적으로 분석합니다.

- **Performance Highlights**: 실제 데이터셋을 통해 검증된 본 프레임워크는 개인화된 모델에서 정확성, 공정성, 해석 가능성 사이의 균형을 맞추기 위한 실제적인 지침을 제공합니다. 연구 결과, 개인화로 인해 예측 정확도가 향상되었다고 해도 반드시 설명 가능성이 향상되는 것은 아니므로, 둘 모두를 함께 고려해야 합니다.



### OpenSTARLab: Open Approach for Spatio-Temporal Agent Data Analysis in Soccer (https://arxiv.org/abs/2502.02785)
- **What's New**: OpenSTARLab는 스포츠 분석에서 시공간적 에이전트 데이터(spatio-temporal agent data)의 접근성을 높이는 오픈소스 프레임워크입니다. 이 시스템은 이벤트 데이터와 추적 데이터를 통합하여 효율적인 분석을 지원하며, 특히 아마추어 경쟁에 대한 데이터의 부족 문제를 해결하고자 합니다. 특히, 데이터 전처리, 이벤트 모델링, 강화 학습 작업을 위한 패키지들이 포함되어 있어 다양한 분석 작업을 가능하게 합니다.

- **Technical Details**: OpenSTARLab는 이벤트 예측 모델을 위한 Event Modeling Package와 데이터 표준화를 위한 Preprocessing Package를 제공합니다. 데이터 소스를 통합할 수 있는 Unified and Integrated Event Data (UIED) 포맷과 강화 학습 작업을 위한 State-Action-Reward (SAR) 포맷이 포함되어 있습니다. 이를 통해 다양한 데이터 형식을 처리할 수 있으며, 강화 학습 모델의 교육과 결과 시각화를 지원하는 RLearn Package도 제공합니다.

- **Performance Highlights**: 실험 평가에서 OpenSTARLab의 이벤트 예측 모델이 정확한 행동(action) 및 시간(time) 예측 정확도를 보였으며, 강력한 이벤트 시뮬레이션 성능도 유지했습니다. 강화 학습 실험은 행동 정확도와 시간적 차이 손실 간의 트레이드오프를 보여주며, 종합적인 시각화도 구현했습니다. 이로 인해 연구자와 실무자에게 혁신과 협업을 증진할 수 있는 강력한 플랫폼으로 자리 잡고 있습니다.
...
### Theoretical Guarantees for Low-Rank Compression of Deep Neural Networks (https://arxiv.org/abs/2502.02766)
- **What's New**: 이 논문에서는 데이터 기반의 포스트 트레이닝 저랭크 압축(post-training low-rank compression)을 위한 분석 프레임워크를 개발합니다. 저랭크 구조의 활성화에 대한 점점 약해지는 가정 하에 세 가지 복구 정리를 증명하였으며, 이는 기존 방법들에 대한 이론적 기반을 제공합니다. 이러한 접근은 대규모 언어 모델의 압축 필요성이 커지는 현상과 관련하여, 성능을 유지하면서 추론 비용을 줄일 수 있는 잠재력을 제시합니다.

- **Technical Details**: 저자들은 저랭크 압축이 어떻게 작동하는지 설명하기 위해 L-layer multi-layer perceptron를 사용합니다. 각 층은 비선형 활성화 함수와 아핀 맵의 연속적인 결합으로 구성되며, 각 층의 인풋은 이전 층의 출력을 기반으로 합니다. 데이터 세트와 훈련된 신경망이 주어지면, 이 구조를 통해 활성화 및 가중치 매트릭스를 효과적으로 분석하고 최적화할 수 있습니다.

- **Performance Highlights**: 이 연구의 결과는 데이터 기반 저랭크 압축 방법들이 데이터 무관적 접근 방식을 초월하는 이유를 설명하는 중요한 단계입니다. 저자들은 이러한 이론적 기반 위에서 더 효율적인 압축 알고리즘 개발 가능성을 제시하며, 이는 특히 자원 제약이 있는 환경에서 중요한 실질적 이점을 가질 수 있습니다.
...
### ReGNet: Reciprocal Space-Aware Long-Range Modeling and Multi-Property Prediction for Crystals (https://arxiv.org/abs/2502.02748)
- **What's New**: 본 논문에서는 결정 구조에서 물질 특성을 예측하는 새로운 접근 방식인 Reciprocal Geometry Network (ReGNet)를 제안합니다. 기존의 GNN 기반 방법들이 긴 거리 상호작용을 효과적으로 캡처하지 못하는 한계를 극복하기 위해, 우리는 Fourier 변환 내에서 학습 가능한 필터를 사용하여 긴 거리 상호작용을 효율적으로 인코딩 하는 방법을 도입했습니다. 또한, 여러 특성을 동시에 예측할 수 있는 다중 작업 확장 모델(ReGNet-MT)을 소개합니다.

- **Technical Details**: ReGNet은 기하학적 GNN과 reciprocal block을 결합하여 짧은 거리 및 긴 거리 상호작용을 모델링 합니다. 이 모델은 응집수(Density Functional Theory)와 같은 높은 계산 비용 없이도 현미경 레벨의 구조적 세부 사항을 다루며, reciprocal space는 전자적 성질을 포함한 결정 특성을 효과적으로 계산하는 데 필수적입니다. 기존 방법들과의 차별점으로, ReGNet은 미리 정의된 초격자 그리드에 의존하지 않고, reciprocal 격자 벡터와 원자동적 분수를 사용하여 대칭성을 유지합니다.

- **Performance Highlights**: JARVIS 및 Material Project 벤치마크 데이터셋에서 ReGNet은 다양한 기존 방법들과 비교하여 뛰어난 성능을 보여주었습니다. 특히, ReGNet-MT는 OPT 및 MBJ 밴드갭 예측에서 새로운 기준을 세우며 모든 특성을 동시에 예측하면서도 낮은 메모리 및 연산 비용을 유지하는 것으로 평가받았습니다. 이러한 결과들은 결정 특성 예측을 위한 확장 가능하고 정확한 솔루션으로서의 모델의 잠재력을 강조합니다.



### LLM Bandit: Cost-Efficient LLM Generation via Preference-Conditioned Dynamic Routing (https://arxiv.org/abs/2502.02743)
- **What's New**: 이 논문에서는 사용자의 쿼리에 가장 적합한 대형 언어 모델을 선택하기 위한 새로운 프레임워크를 제안합니다. 이 프레임워크는 LLM(large language models) 선택 과정을 다중 무장 강도 문제(multi-armed bandit problem)로 구성하여 동적으로 쿼리를 라우팅할 수 있습니다. 특히, 사용자가 추론 시 성능과 비용 사이의 균형을 사용자 정의할 수 있도록 하는 선호 조건의 동적 라우팅 메커니즘을 포함하고 있습니다.

- **Technical Details**: 제안된 시스템은 모델의 능력을 네 가지 자각 벡터를 통해 표현하여 효율적으로 비교 및 선택을 수행합니다. 사용자는 간단한 선호 파라미터를 통해 성능과 비용 사이의 원하는 트레이드오프를 지정할 수 있으며, 새로운 모델은 기존 벤치마크 프롬프트의 서브셋에 대한 성능 평가를 통해 신속하게 통합될 수 있습니다. 또한, 라우팅 문제를 다목적 최적화(task) 과제로 정의하여 성능과 비용 간의 트레이드오프를 최적화하는 것을 목표로 합니다.

- **Performance Highlights**: 다양한 LLM 플랫폼에서 진행된 실험 결과, 제안한 방법은 기존 라우팅 접근 방식에 비해 비용 효율성에서 최대 27%의 개선 효과를 보였습니다. 이 프레임워크는 특히 성능 요건과 비용 제약이 다양한 실제 시나리오에서 효과적임을 입증하며, 각 쿼리에 대해 가장 비용 효율적인 모델을 자동으로 선택하여 LLM 배포를 최적화할 수 있도록 도와줍니다.
...
### Dobi-SVD: Differentiable SVD for LLM Compression and Some New Perspectives (https://arxiv.org/abs/2502.02723)
- **What's New**: 본 논문에서는 기존의 quantization과 pruning 방법을 넘어서는 새로운 LLM-압축 솔루션인 Dobi-SVD를 제안합니다. 이 연구는 SVD의 최적 사용법이 활성화(activation)를 절단하는 것에 있으며, 이를 통해 LLM 압축에 있어 주요 문제점들을 해결하고자 합니다. Dobi-SVD는 LLM 압축을 위한 원칙적인 접근 방식을 수립하며, 성능 저하 없이 메모리와 계산 자원을 줄이는 방법을 제시합니다.

- **Technical Details**: Dobi-SVD는 세 가지 핵심 과제에 초점을 맞추어 개발되었습니다. 첫째, LLM의 각 가중치 행렬에 대해 최적의 절단 위치를 결정하는 방법을 제안합니다. 둘째, 절단된 활성화를 기반으로 가중치 행렬을 얼마나 효율적으로 재구성할 수 있는지를 분석합니다. 마지막으로, SVD에서 발생하는 정보 손실 문제를 해결하기 위한 방법을 구상하여, 안정적인 SVD 역전파를 가능하게 했습니다.

- **Performance Highlights**: Dobi-SVD는 LLaMA-7B 모델을 0.4의 압축 비율로 줄일 때 WikiText2에서 9.07의 PPL(perplexity)을 달성하며, 이는 기존의 SVD 압축 방법보다 78.5% 개선된 성능을 나타냅니다. 또한, Dobi-SVD는 저비용 GPU에서의 추론 시간을 상당히 줄이고, NVIDIA TITAN Xp에서 모델의 생성 속도를 25.97 토큰/초로 향상시켜 12.4배의 속도 개선을 보였습니다. 마지막으로, Dobi-SVD는 다른 압축 방법과 효과적으로 결합되어 3.4GB로 모델을 압축하는 동시에 9.97의 perplexity를 유지하는 등 유연한 성능을 발휘합니다.



### Beyond Topological Self-Explainable GNNs: A Formal Explainability Perspectiv (https://arxiv.org/abs/2502.02719)
- **What's New**: 본 논문에서는 Self-Explainable Graph Neural Networks (SE-GNNs)의 설명 속성과 한계를 명확히 분석합니다. SE-GNNs는 고성능과 해석 가능성을 결합하도록 설계된 그래프 신경망으로, Trivial Explanations (TEs)와 Prime Implicant (PI), faithful explanations 사이의 차이를 비교합니다. 이 연구는 TEs가 특정 임무 기반 예측 작업에서 PIs와 유사하다는 것을 보여주지만, 일반적으로는 정보성이 떨어짐을 밝힙니다.

- **Technical Details**: SE-GNNs는 설명 추출기와 분류기를 통합하여 작동하는 구조로, 입력의 클래스 차별적 서브그래프를 찾아내고 이를 기반으로 예측을 수행합니다. 이 논문에서는 설명 추출기의 역할을 명확히 하여 TEs를 형식적으로 정의하며, PIs와 faithful explanations과의 차별성을 설명합니다. 또한 새로운 Dual-Channel GNNs (DC-GNNs)를 제안하며, 이는 SE-GNN과 비관계형 white-box 예측기를 결합하여 최적화를 시도합니다.

- **Performance Highlights**: DC-GNNs는 세 가지 합성 및 다섯 가지 실제 그래프 분류 데이터셋에서 SE-GNNs와 비교하여 동등하거나 더 나은 성능을 보였습니다. 이 새로운 아키텍처는 TEs의 크기를 줄이는 동시에 효율성을 유지하는데 유리하며, TEs의 추출 비용이 일반적으로 지수적이지 않도록 설계되었습니다. 전체적으로 새로운 접근 방식은 현재의 SE-GNNs가 가진 한계를 극복할 가능성을 보여줍니다.
...
### Scalable Higher Resolution Polar Sea Ice Classification and Freeboard Calculation from ICESat-2 ATL03 Data (https://arxiv.org/abs/2502.02700)
- **What's New**: 이번 연구는 NASA의 ICESat-2 위성을 활용하여 해양 빙산 연구를 위한 기계 학습 도구를 개발하는 것을 목표로 하고 있습니다. 특히, 야간 데이터(
ATL03)를 2m 윈도우로 리샘플링하여 더 높은 해상도의 해수면 높이 및 자유보드(freeboard) 정보를 생성했습니다. 이를 통해 ATL07 및 ATL10 데이터보다 더 나은 정확도(96.56%)를 달성했습니다.

- **Technical Details**: 연구자는 ATL03 데이터를 기반으로 심층 학습 방법(MLP와 LSTM)을 사용하여 두꺼운 얼음, 얇은 얼음, 개수면(open water)으로 분류했습니다. 데이터 레이블링을 위해서 Sentinel-2 이미지를 활용해 Auto-labeling을 통해 해양 빙산 특징을 매핑했습니다. 분산 병렬 컴퓨팅을 통해 ATL03 데이터의 레이블링 및 계산 속도를 약 16.25배, 자유보드 계산 속도를 15.7배 향상시켰습니다.

- **Performance Highlights**: LSTM 기반의 깊이 학습 모델은 96.56%의 분류 정확도를 달성하며, MLP 모델보다 7.25배 더 빠른 성능을 기록했습니다. 연구 결과는 ICESat-2의 ATL03 데이터가 제공하는 해양 빙산 연구의 효율성과 정확성을 크게 향상시킬 수 있다는 것을 시사합니다. 이 연구는 기후 변화와 관련된 다양한 과학적 연구에 기여할 수 있는 가능성을 가지고 있습니다.



### Pseudo-Physics-Informed Neural Operators: Enhancing Operator Learning from Limited Data (https://arxiv.org/abs/2502.02682)
- **What's New**: 이 논문은 Pseudo Physics-Informed Neural Operator (PPI-NO) 프레임워크를 제안합니다. PPI-NO는 간단한 물리 법칙에서 유도한 partial differential equations (PDEs)을 사용하여 목표 시스템에 대한 대체 물리 시스템을 구성합니다. 이 접근법은 심층적인 물리 이해 없이도 효과적인 결과를 제공합니다.

- **Technical Details**: PPI-NO는 고전적인 operator learning을 개선하는 데 기여합니다. 이 프레임워크는 직접 데이터에서 유도된 neural PDE와 neural operator를 결합하여 순차적 업데이트 및 학습 프로세스를 통해 이루어집니다. 각 반복에서 물리 지식을 추출하고 통합하는 과정을 통해 모델의 예측 능력을 강화합니다.

- **Performance Highlights**: PPI-NO는 다섯 가지 벤치마크 작업과 피로 모델링 응용 프로그램에서의 평가를 통해 그 효과가 입증되었습니다. 이 방식은 기존 operator learning 모델의 정확성을 데이터가 부족한 경우에도 크게 향상시키는 결과를 보여주었습니다. 이러한 성능 개선은 복잡한 문제에서도 유용하게 작용할 것으로 기대됩니다.
...
### Recovering Imbalanced Clusters via Gradient-Based Projection Pursu (https://arxiv.org/abs/2502.02668)
- **What's New**: 이번 논문에서는 Imbalanced Clusters(불균형 클러스터)나 Bernoulli-Rademacher 분포를 포함하는 프로젝션을 회복하기 위한 새로운 메서드를 제안합니다. 이 방법은 gradient-based(경량 경량화) 기술을 활용하여 프로젝션 인덱스를 최적화합니다. 특히, sample complexity(샘플 복잡성) 측면에서 기존의 방법들과 비교하며, 불균형 클러스터가 균형 클러스터보다 쉽게 회복됨을 분석합니다.

- **Technical Details**: 프로젝션 퍼슛(Projection Pursuit)의 기본 개념은 데이터셋의 흥미로운 프로젝션을 찾는 것입니다. 본 연구에서는 Planted Vector 설정 하에서 알고리즘의 샘플 복잡성을 분석하며, 다양한 데이터 분포 및 프로젝션 인덱스에 적용 가능한 일반화된 결과를 제공합니다. 특별히, gradient descent(경량 경량화) 기법을 통해 수렴 속도를 높이는 방법과 초기화 기법에 대한 다양한 접근 방식을 논의합니다.

- **Performance Highlights**: 모델의 성능을 FashionMNIST와 Human Activity Recognition Dataset을 통해 평가한 결과, 제안한 알고리즘이 샘플 수가 적은 상황에서도 경쟁 알고리즘보다 우수한 성능을 보였습니다. 샘플 수가 줄어들수록 알고리즘의 성능이 더욱 두드러짐을 보여, 실제 데이터에 적용했을 때 그 유용성을 입증하였습니다. 이 연구는 프로젝션 퍼슛의 새로운 가능성을 제시하며, 적은 샘플에서의 효과적인 데이터 분석에 기여할 것을 기대합니다.
...
### Bayesian Parameter Shift Rule in Variational Quantum Eigensolvers (https://arxiv.org/abs/2502.02625)
Comments:
          8 pages, 5 figures. arXiv admin note: text overlap with arXiv:2502.01704

- **What's New**: 이 논문에서는 변형 양자 고유 상태 해석기에 대한 효과적인 기울기 추정 기법으로서 파라미터 이동 규칙(PSR)을 베이즈 변형으로 제안하고 있습니다. Gaussian process(가우시안 프로세스)를 사용하였으며, 이를 통해 불확실성 정보를 포함한 유연한 기울기 추정을 가능하게 합니다. 또한 이 연구는 기존 PSR에 비해 관측 데이터를 재사용할 수 있는 장점을 내세웁니다.

- **Technical Details**: 베이즈 PSR은 어떤 위치에서도 관측을 통해 기울기를 추정할 수 있으며, 이는 SGD(확률적 경량 하강법)의 각 단계에서 관측 비용을 최소화하는데 크게 기여합니다. GradCoRe(기울기 신뢰 지역)라는 새로운 개념을 도입하여 기울기 추정의 불확실성이 주어진 임계값 이하인 영역을 정의하고, 이를 통해 관측 비용을 조정하는 방식을 구현합니다. 이와 같은 접근은 기울기 추정의 정확성을 향상시키면서 최적화 프로세스를 가속화합니다.

- **Performance Highlights**: 실험 결과, 베이즈 PSR과 GradCoRe를 활용한 SGD는 기존의 최첨단 방법들을 초월하며, Nakanishi-Fuji-Todo(NFT) 알고리즘 및 그 변형들보다 유의미한 성능 개선을 보여주었습니다. 논문에 제시된 이론 기반의 접근 방식은 통계적 특성을 고려한 새로운 해결책을 제시하며, 효과적인 최적화를 위한 데이터 활용 방식을 혁신적으로 변화시킵니다. 이러한 결과는 변형 양자 고유값 해석기의 기여도를 높일 수 있습니다.
...
### Linearized Optimal Transport pyLOT Library: A Toolkit for Machine Learning on Point Clouds (https://arxiv.org/abs/2502.03439)
- **What's New**: 이번 논문에서는 pyLOT 라이브러리를 소개하여, 선형화된 최적 수송(linearized optimal transport, LOT) 기법을 파이썬으로 구현한 내용을 설명합니다. 이 라이브러리는 고정된 기준 분포(reference distribution)에서 최적 수송 맵(optimal transport maps)을 통해 확률 분포를 힐베르트 공간(Hilbert space)에 매핑하여, 일반적인 머신러닝 알고리즘을 사용하여 하위 작업을 간편하게 수행할 수 있도록 합니다. 또한, 연산의 효율성을 높이며, 복잡한 데이터 구조도 다룰 수 있는 기회를 제공합니다.

- **Technical Details**: LOT는 확률 분포 집합({μi})을 고정된 기준 분포(σ)로부터 최적 수송 맵(Tσμi)으로 변환하는 방식을 채택합니다. 이 과정에서 제곱 유클리드 거리(squared Euclidean distance)를 비용 함수로 사용하며, 각 확률 분포는 L2 공간(L2(σ))으로 매핑되어 기계 학습(number of classical machine learning algorithms)에 활용될 수 있습니다. 또한 LOT 임베딩은 원래의 데이터 구조를 유지하며, permutation 불변 메트릭을 학습하는데 도움을 줍니다.

- **Performance Highlights**: 본 연구는 3D 스캔을 활용한 여우원숭이의 치아 분류, 클러스터링 및 차원 축소 문제를 LOT를 통해 효과적으로 해결하는 사례를 제시합니다. LOT 임베딩으로 변환된 데이터는 기본적인 선형 연산을 통해 손쉽게 처리가 가능하여, 복잡한 데이터 처리 과정의 단순화 및 효율성을 입증하였습니다. 따라서, LOT는 여러 머신러닝 작업을 위한 강력한 도구로 자리잡을 것으로 기대됩니다.



### Taking a Big Step: Large Learning Rates in Denoising Score Matching Prevent Memorization (https://arxiv.org/abs/2502.03435)
- **What's New**: 이 논문에서는 Denoising Score Matching이 확산 기반 생성 모델에서 어떤 역할을 하는지를 심도 있게 분석합니다. 특히, 경험적인 최적 스코어가 메모리화(memorization)를 초래하는 것과 관련하여, 새로운 관점을 제공합니다. 저자들은 대규모 학습률(learning rate)로 인한 암시적 규제(implicit regularization) 메커니즘을 밝혀냈습니다.

- **Technical Details**: 논문은 소음이 작은 영역(small-noise regime)에서 경험적 최적 스코어가 불규칙성을 나타낸다고 주장합니다. 또한, 충분히 큰 학습률을 사용하는 확률적 경량 하강(stochastic gradient descent) 훈련 시, 신경망(neural networks)은 로컬 최소(local minimum)로 안정적으로 수렴할 수 없다는 것을 증명합니다. 이러한 현상은 메모리화를 줄이는 데 기여합니다.

- **Performance Highlights**: 저자들은 1차원 데이터와 2층 신경망을 사용하여 분석의 유효성을 검증했습니다. 실험을 통해 학습률이 메모리화를 방지하는 데 중요한 역할을 한다는 것을 확인했습니다. 또한, 하나의 차원 설정을 넘어서도 이 결과를 관찰할 수 있었습니다.



### Deep Reinforcement Learning-Based Optimization of Second-Life Battery Utilization in Electric Vehicles Charging Stations (https://arxiv.org/abs/2502.03412)
Comments:
          5 pages, 6 figures, Accepted, 2025 IEEE Power and Energy Society General Meeting (PESGM 2025), Austin, TX, USA

- **What's New**: 전기차(EV)의 급격한 보급으로 폐기된 EV 배터리 관리가 중요한 과제로 부각되고 있습니다. 연구에 따르면, 폐기된 EV 배터리, 즉 2차 생명 배터리(Second-Life Batteries, SLBs)는 여전히 상당한 잔여 용량을 지니고 있어 EV 충전소(EVCS)에 효율적으로 재활용될 수 있습니다. 이 논문은 SLB를 활용한 배터리 에너지 저장 시스템(Battery Energy Storage Systems, BESS)와 결합하여 EVCS의 운영 최적화 방안을 제시합니다.

- **Technical Details**: 딥 강화 학습(Deep Reinforcement Learning, DRL) 기반의 계획 프레임워크가 도입되어 EVCS의 효율적인 운영을 위한 기초를 마련합니다. 특히, 소프트 액터-크리틱(Soft Actor-Critic, SAC) 접근법이 적용되어 계절적 변동을 고려한 연간 데이터로 모델을 훈련합니다. 맞춤형 보상 함수는 오프라인 훈련을 효과적으로 가능하게 하여 불확실한 환경 속에서도 EVCS 운영을 실시간으로 최적화할 수 있습니다.

- **Performance Highlights**: 이 연구는 SLB를 BESS로 활용하는 EVCS의 운영 및 계획 최적화 프레임워크를 제안하며, 비용 효율성과 환경 영향을 최소화합니다. 또한, BESS의 열화 요인을 최적화 문제에 통합하여 실제 운영과 계획의 중요한 간극을 메우고 있습니다. 최종적으로, SLBs의 재사용은 EVCS의 설치 비용 절감 및 전체 전력망의 부담 완화에 기여할 것으로 기대됩니다.



### Energy-Efficient Flying LoRa Gateways: A Multi-Agent Reinforcement Learning Approach (https://arxiv.org/abs/2502.03377)
Comments:
          6 pages, 6 figures

- **What's New**: 이 논문에서는 다음 세대 사물인터넷(NG-IoT) 네트워크의 에너지 효율성을 향상시키기 위해 UAV(무인 항공기)에 장착된 비행 LoRa 게이트웨이( GW)를 활용한 새로운 접근 방식을 소개합니다. 전송 전력( TP), 확산 계수( SF), 대역폭( W) 및 ED(최종 장치) 연관성의 통합 최적화를 통해 무선 LoRa 네트워크의 글로벌 시스템 에너지 효율성( EE)을 극대화하는 것을 목표로 합니다. 특히, 부분 관찰 마르코프 의사결정 과정(POMDP) 모델을 사용하여, 각 비행 GW가 협력형 다중 대리자 강화학습(MARL) 접근을 통해 데이터를 수집하도록 설계되었습니다.

- **Technical Details**: 모델은 LoRa 네트워크의 자원 할당 및 사용자 연결 문제를 단일 최적화 문제로 구성합니다. 비행 LoRa GW의 각 에이전트는 개인화된 액션 공간 및 보상 함수와 함께 POMDP로 구성되어 안정적인 MAPPO(다중 대리자 근접 정책 최적화) 학습을 지원합니다. 또한, A2G(공중-지상) 링크 특성을 고려하여 디바이스와 게이트웨이 간의 링크 품질에 미치는 영향을 통합하고 있습니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 MAPPO 기반 방법이 기존의 MARL 기법보다 글로벌 시스템의 에너지 효율성을 크게 향상시키며 더 빠르고 안정적인 수렴성을 보임을 입증했습니다. 이러한 접근 방식은 실질적으로 LoRa 네트워크의 에너지 효율성을 극대화하고, 동적 트래픽 요구를 충족하는 자원 관리 알고리즘 개발에 기여할 것입니다.
...
### A Match Made in Heaven? Matching Test Cases and Vulnerabilities With the VUTECO Approach (https://arxiv.org/abs/2502.03365)
Comments:
          This work was partially supported by EU-funded project Sec4AI4Sec (grant no. 101120393)

- **What's New**: VUTECO는 Java 리포지토리에서 취약성 관찰 테스트(vulnerability-witnessing tests)를 수집하는 깊은 학습 기반 접근법입니다. 이 시스템은 보안 관련 테스트 케이스 찾기(Finding)와 취약성과 관련된 테스트 케이스 매칭(Matching)의 두 가지 작업을 수행합니다. 특히 보안 관련 테스트 케이스를 스스로 식별할 수 있는 능력이 두드러집니다.

- **Technical Details**: VUTECO는 사전 훈련된 CodeBERT를 사용하는 심층학습 모델과 로지스틱 분류기를 통해 테스트 케이스의 보안 관련성을 판단합니다. Matching 작업은 Finder 모델과 추가적인 CodeBERT 기반의 Linker 모델을 결합하여 특정 취약성과 관련된 테스트 케이스를 매칭하는 방식을 사용합니다. VUTECO는 62,635개의 테스트 케이스에 대한 데이터 세트를 바탕으로 훈련되었습니다.

- **Performance Highlights**: VUTECO는 Finding 작업에서 완벽한 정밀도(precision)와 0.83의 F0.5 스코어를 달성하였으며, 244개의 오픈 소스 Java 프로젝트에서 보안 관련 테스트 케이스 102개를 성공적으로 찾아냈습니다. 그러나 취약성과의 매칭 결과는 기대 이하였고, 실제 취약성과 일치하는 테스트 케이스를 찾지 못했습니다. 그럼에도 불구하고 VUTECO는 보안 관련성을 가진 모든 테스트 케이스와 관련이 있었음을 확인했습니다.
...
### Optimal Task Order for Continual Learning of Multiple Tasks (https://arxiv.org/abs/2502.03350)
- **What's New**: 이번 연구에서는 다양한 작업을 계속 학습하는 데 있어 작업 순서가 어떻게 지속적인 학습에 영향을 주는지를 탐구했습니다. 특히, 선형 teacher-student 모델을 기반으로 작업의 유사성과 순서가 학습 성과에 어떻게 관계있는지를 분석하여 유용한 최적화 전략을 제안합니다. 연구 결과, 작업은 가장 비대표적인 작업부터 가장 대표적인 작업으로 순차적으로 배치해야 하며, 인접 작업 간에는 유사성이 적어야 한다는 두 가지 원칙을 도출했습니다.

- **Technical Details**: 작업 순서 최적화는 모든 작업에 대한 데이터 샘플을 미리 수집해야 하므로 실제 온라인 학습 환경에서는 실행 가능성이 제한적입니다. 그러나 이 연구에서는 그럼에도 불구하고 작업 순서가 학습에 미치는 영향을 평가하기 위한 분석을 통해 일반적인 프레임워크를 제안합니다. 첫째로, 작업 유사도에 따라 평균 오류를 분석하였더니, 오류는 작업 순서에 의존한다는 것을 발견했습니다. 또한, 작업 위치에 따라 유사성이 오류에 미치는 상대적 및 절대적 영향을 조사하였습니다.

- **Performance Highlights**: 연구에서 제안한 순서 최적화 원칙은 Fashion-MNIST, CIFAR-10, CIFAR-100 데이터셋을 포함한 다양한 이미지 분류 작업에 적용하여 검증되었습니다. 이 결과는 다층 퍼셉트론과 합성곱 신경망 모두에서 일관된 성능 개선을 보여줍니다. 따라서 이번 연구는 작업 증가 지속 학습에서 작업 순서 최적화에 대한 간단하고 일반화 가능한 이론을 제공합니다.
...
### A Mixture-Based Framework for Guiding Diffusion Models (https://arxiv.org/abs/2502.03332)
- **What's New**: 이번 논문은 Bayesian inverse problem 분야에서의 새로운 접근법을 제안합니다. 기존의 Denoising diffusion 모델을 사용하여 다양한 문제를 해결하면서, 매번 특정 작업을 위해 재훈련할 필요 없이 효율적인 방법으로 진행합니다. 이 방법은 중간 posterior 분포의 혼합 근사를 통해 posterior를 근사하는 혁신적인 접근을 포함하고 있습니다.

- **Technical Details**: 이 연구에서는 Gibbs sampling을 기반으로 한 실용적인 방법을 제시하며, 기존의 직접적인 gradient 기반 샘플링이 불가능한 문제를 해결합니다. 구체적으로, 우리는 복수의 intermediate posterior distributions에서 샘플링을 수행하며, 복잡한 likelihood 함수가 포함된 문제를 효과적으로 처리할 수 있는 방법을 개발하였습니다. 이를 통해 사용자들이 더 많은 넓은 범위의 문제를 다룰 수 있습니다.

- **Performance Highlights**: 우리는 이미지 역 문제(image inverse problems)와 오디오 확산 모델(audio diffusion model)을 활용한 소스 분리(audio source separation)에서 실험을 통해 제안된 방법의 효과를 검증하였습니다. 실험 결과, pixel 및 latent-space diffusion priors를 활용하여 상당한 성능 개선을 확인하였습니다. 해당 연구 코드 또한 공개되어 연구자들이 접근할 수 있습니다.



### Is In-Context Universality Enough? MLPs are Also Universal In-Contex (https://arxiv.org/abs/2502.03327)
- **What's New**: 이번 연구에서는 Transformer 모델의 성공이 in-context universality와 관련이 있다는 기존의가설을 반박하며, 학습 가능한 활성화 함수를 갖는 다층 퍼셉트론(MLP)도 in-context에서 보편적임을 증명합니다. 이 결과는 Transformer의 성능이 in-context universality보다는 통계적 또는 최적화 현상과 같은 다른 요인에 기인할 가능성이 높다는 것을 시사합니다.

- **Technical Details**: Transformer 모델은 입력 시퀀스의 각 토큰을 처리하고 다음 토큰을 예측할 수 있는 능력을 가지고 있으며, 이는 in-context learning(ICL)으로 알려져 있습니다. 본 연구는 MLP와 Transformer의 in-context approximation 능력을 비교하고, ReLU MLP가 Wasserstein 거리(Wasserstein distance)를 정확하게 구현할 수 있음도 보여줍니다. 우리는 permutation-invariant contexts(PICs) 하에서 MLP 모델의 in-context 보편성을 도출했습니다.

- **Performance Highlights**: 연구 결과에 따르면, 다양한 주의 헤드를 가진 Transformer는 MLP와 동일한 in-context 근사 성능을 보입니다. 그러나 기존의 Transformer 모델이 MLP에 비해 우수한 성능을 갖는 이유는 in-context universality가 아닌 다른 통계적 또는 최적화적 현상이라고 결론짓습니다. 이러한 연구 결과는 Transformer의 poise와 MLP의 성능을 비교하는 데 있어 중요한 기초를 제공합니다.
...
- **What's New**: 이 논문은 다양한 모의 환경과 실제 환경에서 로봇 데이터 수집 및 상호 작용을 위한 XR(Extended Reality)을 활용하는 몰입형 로봇 상호 작용 시스템(IRIS)을 소개합니다. 기존의 XR 기반 데이터 수집 시스템은 특정 시뮬레이터에 맞춰져 있어 재사용 및 재현이 어려운 문제가 있었으나, IRIS는 여러 시뮬레이터와 실제 센서 정보를 통합할 수 있는 유연한 프레임워크를 제공합니다. 또한, IRIS는 다수의 XR 헤드셋과의 상호 작용을 가능하게 하여 협업적 데이터 수집을 지원합니다.

- **Technical Details**: IRIS는 통합 장면 명세(unified scene specification)를 통해 모든 객체를 데이터 구조로 표현하며, 이를 통해 다양한 로봇과 자산을 손쉽게 사용할 수 있습니다. 이 시스템은 Mujoco, IsaacSim, CoppeliaSim 및 Genesis와 같은 여러 인기 로봇 시뮬레이터와 호환되며, Unity 프레임워크를 통해 다양한 XR 장치에서 쉽게 배포할 수 있습니다. 특히, IRIS는 여러 XR 헤드셋 간의 통신을 위한 견고한 프로토콜을 도입하여 동일한 장면을 공유할 수 있게 합니다.

- **Performance Highlights**: 사용자 연구를 통해 IRIS가 LIBERO 기준선에 비해 데이터 수집 작업에서 목표 지표와 주관적 지표 모두에서 상당한 성능 향상을 보였음을 입증하였습니다. IRIS는 협업 데이터 수집을 통해 뚜렷한 장점으로 작용하며, 사용자의 몰입도를 높이고 효율성을 극대화합니다. 다양한 테스트를 통해 IRIS는 실제 및 시뮬레이션 환경 모두에서 높은 유연성을 보여주었습니다.
...
- **What's New**: 최근 대형 언어 모델(Large Language Models, LLMs)의 급속한 성장과 함께, LLM 라우팅(LLM routing) 즉, 적절한 응답을 제공할 수 있는 저렴한 LLM으로 쿼리(query)를 유도하는 것에 대한 관심이 증가하고 있습니다. 이 논문에서는 성능과 비용 사이의 원하는 트레이드오프(trade-off)에 따라 모델을 선택하는 CARROT(Cost AwaRe Rate Optimal rouTer)를 소개합니다.

- **Technical Details**: CARROT는 주어진 쿼리를 기반으로 모델의 비용(cost)과 성능(performance) 추정치를 활용해 최적의 모델을 선택합니다. 그 단순성 덕분에 CARROT는 계산(computational) 효율성을 제공하며, 이론적 분석을 통해 라우팅 성능에서 minimax rate-optimality를 입증합니다. 또한, 우리는 최신의 state-of-the-art LLM을 기준으로 쿼리에 대한 라우팅을 지원하기 위해 Smart Price-aware Routing (SPROUT) 데이터셋을 소개합니다.

- **Performance Highlights**: SPROUT 및 기존 벤치마크인 Routerbench와 open-LLM-leaderboard-v2를 활용하여 CARROT의 성능을 여러 대안 라우터와 비교하여 실험적으로 검증하였습니다. 이 과정에서 CARROT의 효율성과 효과성이 확인되었습니다.
...
### From Kernels to Features: A Multi-Scale Adaptive Theory of Feature Learning (https://arxiv.org/abs/2502.03210)
Comments:
          24 pages, 6 figures

- **What's New**: 이번 연구는 신경망(Neural Network, NN)의 특징 학습(Feature Learning, FL)에 대한 새로운 이론적 프레임워크를 제시합니다. 다양한 FL 이론을 체계적으로 연결하여 서로의 관계를 밝혀내고, 특정 스케일링 체제에서 유효한 다중 스케일 적응 이론을 개발했습니다. 이 연구는 신경망의 출력 통계를 다루기 위한 분석적 표현을 도출하여, 다양한 스케일에서 유효함을 입증합니다.

- **Technical Details**: 연구진은 통계 역학(Statistical Mechanics) 기법을 이용해 네트워크 출력의 사후 확률 분포(Posterior Distribution)에 대한 이론적 설명을 최소화 문제(Minimization Problem)로 변환하였습니다. 서로 다른 FL 이론은 다양한 '오더 파라미터(Order Parameter)' 선택에서 파생된다는 것을 발견하였고, 이는 차원적 요소(dimensionality)와 관련이 깊습니다. 다중 스케일 적응 이론은 유한 폭 수정(Finite-width Corrections)을 체계적으로 포함할 수 있는 효용성을 가집니다.

- **Performance Highlights**: 다중 스케일 적응 이론은 평균 네트워크 출력 예측을 위한 커널의 효과적인 재조정(effective kernel rescaling)으로 가까워질 수 있음을 보여주며, 이는 기존의 재조정 이론에서는 나타나지 않는 FL 현상을 설명하는 데 도움을 줍니다. 본 연구는 평균 필드 스케일링에서 출력 공분산(Output Covariance)이 단순 재조정 이론에 의해 예측된 것과는 다르게 특정 방향(Task-relevant direction)에 맞추어 적응된다는 사실을 정확히 예측했습니다. 이러한 결과는 FL에 대한 포괄적 이해가 커널 재조정에서 벗어나 고차원 커널 적응으로 나아가야 함을 시사합니다.
...
### SimSort: A Powerful Framework for Spike Sorting by Large-Scale Electrophysiology Simulation (https://arxiv.org/abs/2502.03198)
- **What's New**: 이 논문에서는 신경 녹음(neural recording) 프로세스의 핵심인 스파이크 소팅(spike sorting) 문제를 다룹니다. 특히, 데이터 기반(data-driven) 접근방식으로 심층학습(deep learning)을 통한 스파이크 소팅 방법인 SimSort를 제안합니다. SimSort는 대규모 시뮬레이션 데이터셋에 대한 사전 훈련(pretraining)을 통해, 실제 스파이크 소팅 작업에서도 강력한 제로샷 일반화를 보여줍니다.

- **Technical Details**: 스파이크 소팅은 외부 기록에서 단일 신경 세포의 활동을 추출하는 프로세스이며, 이는 수학적으로 블라인드 소스 분리(blind source separation) 문제로 정의됩니다. 논문에서는 206개의 신경 모델을 포함한 시뮬레이션 데이터셋을 생성하고, SimSort를 통해 이러한 데이터셋의 이점을 활용하여 SNR(signal-to-noise ratio)이 낮고 가변성이 큰 상황에서도 우수한 성능을 발휘하도록 설계되었습니다. 또한, SimSort는 대조 학습(contrastive learning)을 통해 웨이브폼 특징 표현을 향상시켜 잡음에 대한 강인함을 확보하였습니다.

- **Performance Highlights**: SimSort는 공개적으로 사용 가능한 데이터셋에서 제로샷 실험을 통해 기존의 방법들과 비교하여 최첨단 성능을 입증했습니다. 이 프레임워크는 실험적 신경과학에서 스파이크 소팅의 신뢰성과 확장성을 높이는 데이터 기반 기술의 잠재력을 강조합니다. 또한, 수동으로 정의된 매개변수들에 의존하지 않고 전자동 스파이크 소팅 작업을 위한 실용적이고 강력한 도구를 제공합니다.
...
### Signature Reconstruction from Randomized Signatures (https://arxiv.org/abs/2502.03163)
Comments:
          37 pages, 7 figures

- **What's New**: 이 논문에서는 제어된 상미분 방정식(Controlled Ordinary Differential Equations, CDEs)의 관점에서 연속적으로 유계 변화를 하는 경로로부터 추출된 시그니처 특성(signature features)의 재구성을 다룹니다. 특히 무작위 벡터 필드(random vector fields)로부터 CDE가 얼마나 많은 시그니처 특성을 복원할 수 있는지를 분석합니다. 이를 통해 알고리즘적으로 중요한 발견들이 이루어졌으며, 기계 학습(machine learning) 맥락에서도 기존의 Lie 대수 이론이 어떻게 연관될 수 있는지를 탐구합니다.

- **Technical Details**: CDE는 주어진 입력 경로를 기반으로 한 네트워크의 셀을 정의하며, 특히 N차원적으로 숨겨진 차원을 갖습니다. 저자들은 임의로 선택된 벡터 필드를 이용해 제어된 상미분 방정식의 비선형 흐름(non-linear flow)에서 얼마나 많은 시그니처 특성을 복원할 수 있는지를 알게 되었습니다. 수학적으로 이는 선형 독립성(linear independence) 조건 하에서 이루어지며, 특정 차원 및 깊이에서만 유효함을 보였습니다.

- **Performance Highlights**: 이 연구의 결과는 깊이 2의 신경망(neural networks) 유형의 벡터 필드를 사용할 때, 재구축 가능한 시그니처 특성의 수가 숨겨진 차원 수에 대해 지수적이라는 점에서 매우 주목할 만합니다. 또한, 깊이 m에 대한 선형 독립성 문제와 관련하여, 입력 공간 차원의 크기가 m에 비해 충분히 클 때 가능하다는 것을 보여줍니다. 이로써 제어된 상미분 방정식이 입력 특성의 복원을 위해 얼마나 강력할 수 있는지를 강조합니다.



### Fast Sampling of Cosmological Initial Conditions with Gaussian Neural Posterior Estimation (https://arxiv.org/abs/2502.03139)
Comments:
          9 + 2 pages, 7 figures, 1 table. Comments welcome!

- **What's New**: 본 논문은 고차원 매개변수 공간에서의 우주론적 초기 조건을 복원하기 위한 새로운 접근 방식을 제시합니다. 현재의 우주 데이터 분석어는 전통적인 통계적 추론 기법의 한계를 극복하기 위해 simulation-based inference (SBI)를 활용하여 더욱 효율적으로 대규모 구조 (LSS)를 연구할 수 있도록 합니다. 이 방법은 비미분 가능 시뮬레이터를 사용하여 원시 암흑 물질 밀도 필드를 효율적으로 재현합니다.

- **Technical Details**: 제안된 방법은 초기 조건의 posterior 분포를 모델링하는데 Gaussian 분포와 대각 공분산 행렬을 사용하는 것에 기반합니다. 이로 인해 단일 GPU에서 수천 개의 posterior 샘플을 몇 초 내에 생성할 수 있으며, 이는 기존 방법보다 수 배나 빠른 속도입니다. 또 다른 중요한 특징은 공분산이 파동수에 의존하는 것을 분석적으로 적합시켜 빠른 샘플러로 전환하는 점입니다.

- **Performance Highlights**: 실험 결과, 제안된 방법은 요약 통계량과 베이지안 일관성 테스트를 통해 확인된 실제 값들과 비교했을 때 우수한 샘플 품질을 보여주었습니다. 우리의 접근 방식은 고해상도 암흑 물질 N-body 시뮬레이션에 적용 가능하며, 앞으로의 우주론적 분야에 대한 순차적 SBI의 가능성을 제기합니다. 또한, 이 방법은 향후 예정된 대규모 조사에서 제공하는 새로운 정보의 잠재적 활용을 극대화할 것입니다.



### Underwater Soft Fin Flapping Motion with Deep Neural Network Based Surrogate Mod (https://arxiv.org/abs/2502.03135)
Comments:
          Accepted in IEEE International Conference on Soft Robotics 2025 (Robosoft)

- **What's New**: 이 연구는 Deep Neural Network (DNN) 기반의 대체 모델(surrogate model)과 Reinforcement Learning (RL)를 통합하여 제어 정확도를 높인 수조 로봇의 새로운 힘 제어 프레임워크를 제시합니다. 기존 연구에서 다루기 어려웠던 복잡한 수중 환경과 고비용 실험 문제를 해결하기 위해, DNN 대체 모델이 RL 에이전트의 효율적인 훈련을 도와주는 시뮬레이터 역할을 수행합니다. 이를 통해 고급 MDynamics (동역학) 모델이 필요한 기존 접근법을 대신하며, 수조 로봇의 제어 솔루션에 효율성을 가져옵니다.

- **Technical Details**: 연구 방법론에서 제안된 Real2Sim2Real 구성은 실제 데이터를 기반으로 한 시뮬레이터를 개발하고, 이 시뮬레이터에서 훈련된 모션 컨트롤러를 실제 액추에이터에 재적용하는 과정입니다. 실험은 조용한 물이 있는 수조에서 진행되었으며, 액추에이터는 부드러운 실리콘 지느러미로 제작되어 서보 모터에 부착되었습니다. 데이터 수집을 위해 모터에는 각도와 속도를 기록하는 인코더가 장착되어 있으며, 힘은 방수형 힘-토크 센서를 통해 측정됩니다.

- **Performance Highlights**: 실험 결과, RL 에이전트가 대체 시뮬레이션에서 훈련된 후 복잡한 추력(thrust) 동작을 생성하며 실제 부드러운 지느러미 액추에이터에서도 정확한 힘 제어를 달성하는 것으로 나타났습니다. 이 방법은 수조 환경에서 지느러미 구동 로봇을 위한 효율적인 제어 솔루션을 제공하며, 다양한 환경 조건에서도 강인한 성능을 보여 주었습니다.
...
### Double Distillation Network for Multi-Agent Reinforcement Learning (https://arxiv.org/abs/2502.03125)
- **What's New**: 본 논문은 Multi-agent Reinforcement Learning (MARL) 분야에서의 비효율적인 협업 정책 훈련 문제를 해결하기 위해 Double Distillation Network (DDN)라는 새로운 프레임워크를 제안합니다. DDN은 두 개의 증류 모듈을 통합하여 에이전트들이 부분 관측 상태에서 협업할 수 있도록 지원합니다. 이 네트워크는 비공식적인 정보의 통합에서 발생하는 오류를 제거하는 데 중점을 둡니다.

- **Technical Details**: DDN의 외부 증류 모듈은 Global Guiding Network (GGN)와 Local Policy Network (LPN)으로 구성됩니다. GGN은 상태 정보를 통합하여 에이전트의 관측 특징을 개인화된 방식으로 융합하고, LPN은 각 에이전트의 로컬 관측 정보만을 사용하여 다단계 지식 증류를 통해 두 네트워크 간의 격차를 줄입니다. 내부 증류 모듈은 글로벌 상태 정보와 관련된 내재적 보상을 도입하여 에이전트의 탐색 능력을 향상시킵니다.

- **Performance Highlights**: SMAC 환경에서 실시된 실험 결과, DDN은 에이전트의 협업 작업에서 통계적으로 유의미한 성과 개선을 보여줍니다. DDN은 기존의 중앙집중식 가치 함수 회로에서 발생할 수 있는 오류를 효과적으로 제거하여 협업 정책 훈련의 효율성을 크게 향상시키며, 기존 방법들에 비해 높은 정밀도와 유연성을 달성합니다.



### Comparison of the Cox proportional hazards model and Random Survival Forest algorithm for predicting patient-specific survival probabilities in clinical trial data (https://arxiv.org/abs/2502.03119)
- **What's New**: 이번 연구에서는 Cox 비례 위험 모델(Cox proportional hazards model)과 랜덤 생존 숲(Random Survival Forests, RSF) 알고리즘의 예측 성능을 비교했습니다. RCT(무작위 대조 시험) 데이터를 포함한 실제 데이터와 시뮬레이션 데이터를 기반으로 한 포괄적인 비교 연구가 진행되었습니다. 다양한 성능 지표(performance measures)를 활용하여 두 모델의 예측력을 분석했습니다.

- **Technical Details**: Cox 모델과 RSF의 예측 성능은 C 지수($C$ index)를 통해 비교되었으며, RSF가 일반적으로 더 높은 성능을 보였습니다. 그러나 Cox 모델은 예측의 보정(calibration) 측면에서 더 나은 모습을 보이는 경우가 많았습니다. 특히, 비례 위험 설정(nonproportional hazards settings)에서는 Cox 모델이 RSF보다 더 나은 성능을 보였고, 작은 샘플 크기에서는 RSF가 우수한 성능을 발휘했습니다.

- **Performance Highlights**: RSF의 전체 성능은 높은 검열률(censoring rates)에 더욱 큰 영향을 받는 반면, Cox 모델은 작은 샘플 크기에서 성능 저하가 두드러지는 것으로 나타났습니다. 이러한 결과는 두 모델의 활용에 있어 특정 상황에서의 장단점을 명확히 보여줍니다. 종합적으로 볼 때, 연구에서는 Cox 모델과 RSF의 특성을 이해하는 것이 데이터 분석에서 중요한 요소임을 강조하고 있습니다.
...
### A Bayesian perspective on single-shot laser characterization (https://arxiv.org/abs/2502.03100)
- **What's New**: 이 논문에서는 초강력 레이저의 시공간 연결(spatio-temporal couplings, STCs)을 측정하기 위한 베이지안 프레임워크(Bayesian framework)를 소개합니다. 기존의 단일 촬영(single-shot) 측정의 개념을 재정의하고, 실제로 각각의 레이저 샷을 해결할 수 있는 기준을 제공하기 위한 체계를 제시합니다. 이 연구는 단일 샷의 가능성이 기기 고유의 속성이 아닌 측정 정밀도와 내재된 파라미터 변동성의 관계에서 어떻게 발생하는지를 보여줍니다.

- **Technical Details**: 기술적인 세부사항으로는, 베이지안 추론(Bayesian inference)의 틀 안에서 측정 과정의 진화를 결정론적(predetermined) 및 확률적(stochastic) 요소의 조합으로 설명합니다. 각 파라미터는 예측 가능한 변화와 샷 간 변동성을 나타내는 독립적으로 분포된 정상 랜덤 변수로 모델링됩니다. 이러한 통계적 모델링은 측정의 정확성을 높이고, 레이저 시스템의 동적 변화를 이해하는 데 필요한 기반을 제공합니다.

- **Performance Highlights**: 성능 강조점을 보면, ATLAS-3000 페타와트 레이저를 이용한 구현 결과 단일 샷 측정의 불확실성 한계를 최초로 정량적으로 제한했습니다. 특히, 베이지안 방법을 적용하여 전통적 접근 방식과 비교했을 때 불확실성을 최대 60%까지 감소시킬 수 있다는 점이 주목할 만합니다. 이러한 결과는 레이저-물질 상호작용의 정밀한 제어가 중요한 응용에 직접적인 영향을 미치는 통찰을 제공합니다.
...
### Automatic Prompt Optimization Techniques: Exploring the Potential for Synthetic Data Generation (https://arxiv.org/abs/2502.03078)
Comments:
          Accepted for publication in the Proceedings of the 2025 HCI International Conference

- **What's New**: 이 연구는 의료 데이터의 비공식적인 접근이 제한되는 상황에서 합성 데이터 생성을 위한 효과적인 프롬프트 최적화 기술을 탐구합니다. 기존의 수동 프롬프트 엔지니어링 방식의 한계를 극복하기 위해 자동화된 반복 프롬프트 엔지니어링 파이프라인을 개발하여 사용자의 개입을 최소화합니다. 이 연구는 의료 도메인에서 AI의 발전을 촉진하는 데 기여할 수 있는 새로운 접근 방식을 제안합니다.

- **Technical Details**: 제안된 시스템은 고품질 합성 의료 데이터를 생성하기 위해 문헌 리뷰를 기반으로 한 고급 프롬프트 최적화 기술을 통합한다. PRISMA 지침을 준수하여 2020년부터 현재까지의 자동화된 반복 프롬프트 엔지니어링 기술에 관한 최근 연구를 분석하였습니다. 분석된 방법에는 피드백 중심, 오류 기반, 제어 이론적 접근 방식이 포함됩니다.

- **Performance Highlights**: 연구 결과는 다양한 각도에서 진행된 프롬프트 최적화 접근 방식의 유망한 가능성을 보여줍니다. 특히, PACE(Actor-Critic Editing)와 STRAGO(Strategic-Guided Optimization)는 각기 다른 피드백 메커니즘을 통해 프롬프트 성능을 향상시키며, 최적화를 위한 다양한 시나리오에서 효과를 발휘하는 것으로 나타났습니다. 이러한 결과는 향후 의료 AI 시스템의 질적 개선과 윤리적 데이터 사용을 위한 기반을 마련할 것입니다.



### Optimizing Electric Vehicles Charging using Large Language Models and Graph Neural Networks (https://arxiv.org/abs/2502.03067)
- **What's New**: 이 연구에서는 전기차(EV) 충전 인프라의 효율적 활용이 중요하다고 강조하며, 기존의 최적화 방법과 강화학습(RL) 접근법이 고차원성과 동적 특성 때문에 실시간 충전 요구사항에 만족하지 못함을 지적했습니다. 이에 따라, 대형 언어 모델(LLM)과 그래프 신경망(GNN)의 결합이 전통적인 EV 스마트 충전 방법보다 뛰어난 성능을 발휘함을 보였습니다. 이 연구는 EV 충전 문제에 LLM을 적용한 최초의 사례로, 새로운 연구 방향과 혁신적인 솔루션을 제시합니다.

- **Technical Details**: 이 논문에서는 충전 지점 운영자(CPO)의 관점에서 전기차의 충전 및 방전을 전략적으로 관리하여 수익을 극대화하는 V2G 스마트 충전 문제를 다루고 있습니다. 또한, LLM과 GNN의 결합을 통해 EV 충전 문제를 그래픽 표현으로 재해석하고, 각 노드를 EV, 충전기, 변압기, CPO 등으로 설정하여 관계형 정보를 효과적으로 추출합니다. 최적의 정책을 개발하기 위해 오프라인 RL 방식을 사용하고, 고정된 데이터셋을 기반으로 정책을 학습하여 비용을 최소화하고 그리드 안정성을 유지합니다.

- **Performance Highlights**: 결과적으로, 제안된 모델은 기존 방법 대비 에너지 충전 및 방전, 사용자 만족도, 전력 위반 최소화 등에서 월등한 성과를 기록했습니다. CPO의 각 EV가 출발 시간에 필요한 에너지를 충분히 공급받는 것을 보장함으로써 보다 정밀한 일정 관리를 가능하게 합니다. 이러한 성과는 GNN과 LLM의 조합을 통해 이룬 것으로, 이는 EV 충전 전략의 혁신을 예고합니다.



### Time Series Anomaly Detection in the Frequency Domain with Statistical Reliability (https://arxiv.org/abs/2502.03062)
- **What's New**: 이 논문은 복잡한 시스템에서 변칙 탐지를 위한 새로운 방법을 제안합니다. 특히, 주파수 영역에서의 변화점( Change Points, CP) 탐지를 위해 Selective Inference (SI) 기법을 도입하여 통계적 유의성을 측정하는 방법을 설명합니다. 기존의 데이터로 CP를 감지하고 다시 평가하는 과정에서 발생하는 이중 사용 문제를 해결하기 위한 방향으로 발전시켰습니다.

- **Technical Details**: 변환된 이산 푸리에 변환(Discrete Fourier Transform, DFT)을 활용하여 SI 프레임워크를 주파수 영역에 확장하는 방법을 제안하고, 다수의 주파수에서 변화가 일어나는 CP의 통계적 유의성을 정량화하는 새로운 SI 기법을 개발했습니다. 이 과정에서 조합 최적화 문제를 해결하기 위해 근사적 솔루션을 제공하는 시뮬레이티드 어닐링(Simulated Annealing) 기법을 활용하여 통계적 검증을 수행합니다.

- **Performance Highlights**: 실험 결과, 제안된 방법이 강력한 통계적 보장을 통해 진정한 CP를 안정적으로 식별함을 입증하였습니다. 이는 복잡한 시스템에서 주파수 영역의 문제를 해결할 때 보다 정확한 근본 원인 분석을 가능하게 합니다. 또한, 이 연구는 Synthetic 및 실제 데이터에 대한 종합적인 수치 실험을 통해 효과성을 입증했습니다.
...
### An analysis of optimization problems involving ReLU neural networks (https://arxiv.org/abs/2502.03016)
- **What's New**: 이번 논문에서는 ReLU(정류선형단위) 활성화 함수가 적용된 신경망을 포함한 혼합 정수 최적화 문제를 해결하는 데 있어서의 도전과제를 다룹니다. 특히, 이러한 함수와 관련된 이진 결정의 완화에서 발생하는 Big-M 계수의 기하급수적 증가 문제를 분석하고 해결 방안을 제시하였습니다. 이러한 접근법으로는 훈련 중 적용되는 클리핑 변형(clipped variants) 및 정규화 기법, 그리고 최적화 기반의 경계 강화(bound tightening) 및 새로운 스케일링 방법이 포함됩니다.

- **Technical Details**: 논문에서는 신경망의 각 층을 통해 입력의 선형 변환 후 비선형 활성화 함수를 적용하는 feed-forward ANN을 모델링합니다. ReLU 활성화 함수를 사용하여 ANN은 조각별 아핀 선형 함수(piecewise affine-linear function)를 정의하고, 이는 다양한 최적화 문제의 근사화에 활용됩니다. 혼합 정수 비선형 최적화 문제(MINLP)의 맥락에서 ANN을 임베드하여 다양한 입력 및 출력을 처리하는 과정을 설명합니다.

- **Performance Highlights**: 세 가지 벤치마크 문제에 대한 수치적 비교를 통해, 선형 영역의 개수, 안정적인 뉴런의 비율, 전체 계산 노력을 지표로 사용하여 성능을 평가하였습니다. 이러한 연구를 통해 신경망 모델의 다루기 쉬운 다중성(redundancy)과 최적화 문제 해결을 위한 계산 비용 간의 트레이드오프를 정량적으로 관찰하였습니다. 이는 향후 신경망 모델 최적화 연구에 중요한 기여를 할 것입니다.



### An Augmented Backward-Corrected Projector Splitting Integrator for Dynamical Low-Rank Training (https://arxiv.org/abs/2502.03006)
- **What's New**: 본 논문에서는 기존의 QR 분해를 줄이는 새로운 저랭크( low-rank ) 학습 방법을 제안합니다. 이 방법은 projection-splitting 기법에 증강(augmentation) 단계를 통합하여 로컬 최적 솔루션에 수렴할 수 있게 합니다. 저자들은 제안한 방법에 대한 철저한 이론적 분석을 제공하고 여러 벤치마크에서의 효과성을 입증합니다.

- **Technical Details**: 저자들은 신경망의 가중치를 저랭크 행렬의 다양체로 제한하여 학습하는 다이나믹 저랭크 훈련 방식을 분석합니다. 이 과정에서 대규모 가중치 행렬을 저장하고 업데이트하는 대신, factorized 형태의 작은 행렬만을 요구합니다. 저자들은 backward-corrected PSI 방식을 도입하여 로컬 최적점으로의 수렴을 보장하며, 각 학습 단계에서 필요한 QR 분해의 수를 줄입니다.

- **Performance Highlights**: 제안된 방법은 다양한 통합 기법과 비교하여 MNIST 데이터셋에서 저랭크 신경망을 학습하는 데 더 나은 성능을 보여줍니다. 특히, 새로운 PSI 기법은 이전 방법들에 비해 효율성을 높이며 안정성을 강화했습니다. 결과적으로, 이 연구는 저랭크 신경망의 학습 및 최적화를 위한 보다 나은 접근법을 제공합니다.
...
### Building Bridges between Regression, Clustering, and Classification (https://arxiv.org/abs/2502.02996)
- **What's New**: 이 논문에서는 회귀(Regression) 문제를 해결하기 위한 새로운 방법을 제안합니다. 전통적인 접근 방식인 평균 제곱 오차(minimum squared error) 최적화가 신경망 모델을 훈련하는 데 비효율적인 결과를 초래할 수 있다는 것을 이론적으로 분석하였습니다. 제안된 방법은 타겟 인코더(target encoder)와 예측 디코더(prediction decoder)를 사용하여 문제를 새로운 방식으로 접근하는 것입니다.

- **Technical Details**: 새로운 방법은 분류(classification)와 군집화(clustering)에서 사용되는 방법론에서 영감을 받았습니다. 이 과정에서 연속적인 스칼라 목표(continuous scalar target)를 향상시키는 데 초점을 맞추고 있습니다. 그러므로 기존의 데이터 처리 방법보다 더 효과적으로 데이터 간의 관계를 모델링할 수 있습니다.

- **Performance Highlights**: 제안된 방법은 다양한 실제 데이터셋에서 효과적인 성능을 보였음을 입증합니다. 이러한 성과는 회귀 문제를 해결하는 데 있어 신경망의 훈련 과정을 개선할 수 있는 가능성을 보여줍니다. 이 연구는 기계 학습 및 통계 분야에서 회귀 분석의 접근 방식을 혁신할 수 있는 기회를 제공합니다.
...
### Learning Efficient Flocking Control based on Gibbs Random Fields (https://arxiv.org/abs/2502.02984)
Comments:
          9 pages, 10 figures

- **What's New**: 이번 논문은 다수의 로봇들이 조화를 이루며 함께 이동하는 '플록킹(flocking)' 제어를 위한 새로운 다중 에이전트 강화 학습(MARL) 프레임워크를 제안합니다. 이 프레임워크는 기브스 랜덤 필드(GRFs) 기반으로 설계되며, 전통적인 중앙 집중식 방법의 한계를 넘어 분산 학습과 실행을 통해 효율성을 개선합니다. 또한, 이 연구는 다양한 로봇 수에 맞춰 스케일러블한 학습 프로세스를 제공함으로써 복잡한 환경에서도 뛰어난 성과를 보여줍니다.

- **Technical Details**: 제안된 프레임워크는 로봇 간의 상호작용을 비방향 그래프의 간선으로 모델링하며, 각각의 로봇의 행동을 에너지 함수로 표현합니다. 효율적인 학습을 위해 비대칭적 보상 시스템을 적용해 각 로봇의 기여를 반영한 개별 보상 함수를 기반으로 합니다. 또한, 동료 로봇의 행동을 예측하기 위한 액션 어텐션 모듈을 도입하여, 로봇이 이웃의 의도하는 움직임을 사전에 감지할 수 있게 지원합니다.

- **Performance Highlights**: 이 연구에서 제안된 방법은 시뮬레이션 및 다양한 실험을 통해 약 99%의 성공률을 기록하며 기존의 최첨단 솔루션과 비교하여 우수한 성능을 입증하였습니다. 다양한 환경 조건 하에서도 안정적이고 안전한 플록킹 제어가 가능함을 보여주며, 이를 통해 실제 다중 로봇 시스템에 효과적으로 적용할 수 있는 가능성을 제시하고 있습니다.



### Label Anything: An Interpretable, High-Fidelity and Prompt-Free Annotator (https://arxiv.org/abs/2502.02972)
Comments:
          Accepted by ICRA 2025

- **What's New**: 이번 연구에서는 Label Anything Model (LAM)이라는 주목할 만한 모델을 제안하며, 이는 해석 가능하고 고충실도이며 프롬프트가 필요 없는 데이터 주석 도구로서 작동합니다. LAM은 사전 학습된 Vision Transformer (ViT)를 활용하여 이미지에서 특성을 추출하고, Semantic Class Adapter (SCA)와 Optimization-Oriented Unrolling(OptOU) 알고리즘을 통해 입력 데이터에 대한 높은 품질의 주석을 생성합니다. 중요한 점은 LAM이 단 한 장의 사전 주석 RGB 이미지만으로도 훈련이 가능하다는 것입니다.

- **Technical Details**: LAM은 사전 학습된 ViT와 함께 작은 수의 학습 가능한 매개변수를 가진 SCA와 OptOU로 구성되어 있습니다. SCA는 ViT에서 추출된 특성을 결합하여 주석 생성을 위한 기초를 통합하고, OptOU는 다수의 계층을 통해 출력을 실제 값과 최대한 일치하도록 최적화합니다. 이 과정에서 LAM은 복잡한 프롬프트 없이도 다양한 주석을 생성할 수 있는 구조를 가져, 데이터 변화에 적절하게 대응합니다.

- **Performance Highlights**: 실험 결과, LAM은 Camvid, Cityscapes, Apolloscapes 등 여러 실제 데이터셋과 CARLA 시뮬레이션 데이터셋에서 거의 100%의 mIoU를 달성하는 높은 충실도의 주석을 생성할 수 있음을 보여주고 있습니다. LAM은 이미지 하나당 0.12초의 빠른 주석 처리 시간을 기록, SAM의 5.34초에 비해 월등한 성능을 입증하였습니다. 이러한 점에서 LAM은 보다 실용적이고 확장 가능한 주석 솔루션으로 자리 잡을 것으로 예상됩니다.
...
### Behavioral Homophily in Social Media via Inverse Reinforcement Learning: A Reddit Case Study (https://arxiv.org/abs/2502.02943)
- **What's New**: 본 연구는 전통적인 접근 방식인 콘텐츠 기반 또는 인접 기반 방법 대신 사용자 행동의 동질성을 측정하기 위한 새로운 방법을 제안합니다. Inverse Reinforcement Learning (IRL) 프레임워크를 사용하여 사용자의 정책을 추론하고, 이러한 정책을 행동 동질성의 척도로 삼습니다. 이를 통해 Reddit에서 590만 회의 상호작용을 분석하며, 서로 다른 커뮤니티 간에 사용자 행동 패턴과 역할이 어떻게 다르게 나타나는지를 탐구합니다.

- **Technical Details**: 전통적인 동질성 측정 방법은 주로 팔로워 네트워크(포괄적인 정보)나 해시태그를 기반으로 하지만, Reddit과 같은 플랫폼에서는 상호작용이 팔로워 기반으로 구성되지 않기 때문에 이러한 방법이 적절하지 않습니다. 우리는 사용자의 관찰된 행동을 기반으로 동질성을 이해하기 위해 IRL을 활용한 새롭고 구체적인 모델을 개발하였으며, 이를 통해 소셜 미디어의 복잡한 상호작용을 효과적으로 분석할 수 있도록 하였습니다.

- **Performance Highlights**: 이 연구는 Reddit의 다양한 서브레딧을 통해 동질성과 행동 간의 관계를 조명하며, 예를 들어, 축구와 e스포츠와 같은 전혀 다른 주제에 대해 사용자가 매우 유사하게 행동할 수 있음을 보여줍니다. 기존 콘텐츠 기반 동질성과 비교하여 우리가 제안한 행동 동질성 척도가 어떻게 사용자 행동의 중요한 차이를 포착하는지를 입증하며, 이는 소셜 미디어 동태 분석 및 그에 따른 사회적 영향력을 이해하는 데에 중요한 통찰을 제공합니다.



### Data denoising with self consistency, variance maximization, and the Kantorovich dominanc (https://arxiv.org/abs/2502.02925)
- **What's New**: 본 논문에서는 martingale optimal transport에 부분적으로 영감을 받은 새로운 데이터 노이즈 제거(data denoising) 프레임워크를 소개합니다. 이 방법은 주어진 노이즈를 포함한 분포에서 특정한 구조를 가진 가장 가까운 분포를 찾는 것을 목표로 하고 있습니다. 이 접근 방식은 노이즈가 있는 데이터에 대한 self-consistency(자기 일관성)를 요구하며, 다양한 고전적 노이즈 제거 방법과 밀접하게 관련되어 있습니다.

- **Technical Details**: 제안된 문제는 주어진 노이즈 분포에 대해 convex order에 의해 지배되는 측정값들의 분산을 극대화하는 것으로, 특정한 조건 하에서 해가 존재하고 강인성을 갖는 것으로 증명되었습니다. Kantorovich dominance라는 새로운 분포 간 관계를 도입하여, 이는 convex order의 특정 측면을 보존하면서도 더 강력하고 검증하기 쉬운 조건입니다. 이를 통해 최적화 조건을 충족하는 새로운 노이즈 제거 문제를 제기하고 분석합니다.

- **Performance Highlights**: 제안된 문제는 특정한 도메인에서 더 의미 있는 해를 제공하며, 계산 효율성도 크게 향상되었습니다. 간단한 수치 예제를 통해 전체 convex order 문제와 Kantorovich dominance 문제에 대한 해를 보여줍니다. 이러한 결과는 다양한 노이즈 제거 기법의 발전에 기여할 것으로 기대됩니다.
...
### DANDI: Diffusion as Normative Distribution for Deep Neural Network Inpu (https://arxiv.org/abs/2502.02910)
Comments:
          DeepTest 2025 Workshop

- **What's New**: 이 논문에서는 Deep Neural Networks (DNN)의 테스트 효율성을 향상시키기 위해 DANDI라는 새로운 기술을 제안합니다. DANDI는 원본 학습 데이터에 접근할 수 없으면서도 Surprise Adequacy (SA) 값을 계산할 수 있도록 도와줍니다. 이를 위해 Stable Diffusion을 활용하여 대체 입력 분포를 생성하며, 이는 실제 훈련 데이터에서 계산한 SA 값과 높은 상관관계를 보입니다.

- **Technical Details**: Surprise Adequacy (SA)는 현재 입력과 학습 데이터의 차이를 측정하는 out-of-distribution metric입니다. 이 연구에서는 데이터의 Activation Trace (AT)를 기반으로 SA 값을 Likelihood-based Surprise Adequacy (LSA)로 수치화하고, Gaussian kernel function을 사용하여 데이터의 밀도를 계산합니다. DANDI는 이러한 SA 값을 원본 학습 데이터 없이 생성하는 방법론으로, 생성 모델을 활용해 입력을 부여할 수 있습니다.

- **Performance Highlights**: CIFAR-10 및 ImageNet-1K 데이터셋에 대한 실험 결과, DANDI에서 계산한 SA 값이 훈련 데이터에서 계산한 값과 높은 Spearman Rank correlation 값인 0.852 (ImageNet-1K) 및 0.881 (CIFAR-10)을 기록했습니다. 또한, DANDI를 사용하여 생성된 입력은 DeepMutation으로 변형된 DNN 모델을 테스트하는 데 있어 효과적으로 우선순위를 매길 수 있음을 보였습니다. 이는 DANDI의 실제 DNN 테스트 적용 가능성을 크게 개선할 수 있음을 나타냅니다.



### COSMosFL: Ensemble of Small Language Models for Fault Localisation (https://arxiv.org/abs/2502.02908)
Comments:
          LLM4Code 2025 Workshop

- **What's New**: COSMos는 여러 개의 오픈소스 Small Language Models (SLMs)를 기반으로 한 앙상블 기법으로, 소프트웨어 공학에서 LLM의 대안적인 해결책을 제시합니다. 이 기법은 다양한 비용-효용(trade-off) 옵션을 제공하여, 사용자들이 성능과 비용 사이에서 선택할 수 있도록 합니다. 특히, COSMos는 Fault Localisation 작업에 특화된 AutoFL 기법을 통해 SLMs의 집합을 구성하고, 이를 통해 에너지 소비 및 시간 효율성을 높입니다.

- **Technical Details**: COSMos의 주요 구성 요소는 self-consistency로, 이는 여러 샘플을 통해 더 정확한 답을 도출하는 방식을 채택합니다. 두 가지 앙상블 방식, 즉 기본적인 투표 기반 앙상블과 최적화된 가중치를 사용하는 앙상블이 비교됩니다. 이 연구는 속도, 에너지 소비 등 다양한 비용 측정과 FL 정확도 간의 상관관계를 평가하여 SLMs의 효과를 실험적으로 검증합니다.

- **Performance Highlights**: COSMosFL를 deploy한 결과, 에너지 소비나 토큰 수에 제약이 있는 경우 개별 모델보다 우수한 성능을 보여주었습니다. 특히, Defects4J 벤치마크를 통한 평가로 COSMos의 성능이 각종 비용 측면에서 효과적이라는 것을 입증했습니다. 이러한 연구 성과는 소프트웨어 공학 분야에서 오픈소스 LLMs의 활용 가능성을 높이는 중요한 기초 자료가 될 것입니다.
...
- **What's New**: 이 논문은 최근 몇십 년간의 재료 디자인에서의 컴퓨터 기술 발달을 요약합니다. 특히, 전통적 탐색 방식에서 인버스 디자인(inverse design)으로의 패러다임 전환이 강조됩니다. 전통적인 시도와 다르게 AI 기술, 특히 딥러닝(deep learning) 기반의 모델들이 새로운 재료의 설계에서 어떻게 적용되는지를 다룹니다. 이러한 인버스 디자인 방식은 특정 응용 프로그램을 위한 우수한 재료 발견의 가능성을 크게 높였습니다.

- **Technical Details**: 재료 설계의 초기 접근법으로는 진화 알고리즘(evolutionary algorithms, EAs)과 몬테 카를로 트리 탐색(Monte Carlo tree search, MCTS) 등이 있습니다. 이들 알고리즘은 재료의 구조와 속성을 정의하는 매개변수 집합을 평가하여 최적 조합을 찾는 데 어려움이 따릅니다. 그러나 딥 강화 학습(deep reinforcement learning) 등의 신기술이 도입되면서 효율성이 획기적으로 개선되고 있습니다. 새로운 딥 생성 모델들은 목표 속성에 따라 재료 생성을 수행할 수 있는 뛰어난 능력을 자랑합니다.

- **Performance Highlights**: 고속 스크리닝(forward screening) 방법은 다양한 재료의 특성을 평가하는 데 널리 사용되고 있으나, 비효율적인 탐색 공간을 가진다는 한계가 있습니다. 이 논문에서는 인버스 디자인이 이러한 한계를 극복할 수 있는 가능성을 제시하고 있습니다. 실제로, 논문에서 다루는 다양한 사례들은 인버스 디자인 접근법을 통해 효율적으로 우수한 특성을 가진 재료를 설계할 수 있는 방법을 보여줍니다. 이 연구는 향후 기술 혁신을 위한 재료 발견의 로드맵을 제시하며, AI 기반 재료 설계의 중요성을 강조합니다.
...
### Variations on the Expectation Due to Changes in the Probability Measur (https://arxiv.org/abs/2502.02887)
Comments:
          Submitted to the IEEE International Symposium on Information Theory (ISIT2025)

- **What's New**: 이 논문에서는 기대값의 변화에 대한 명시적인 표현이 새롭게 제시되었습니다. 특히, Gibbs 확률 분포(Gibbs probability measures)와의 흥미로운 연관성이 밝혀졌으며, 상호 정보(mutual information) 및 Lautum 정보(lautum information)와의 연결고리도 제시됩니다. 이러한 접근은 확률 측정이 기대값에 미치는 영향을 보다 명확히 이해하는 데 도움이 됩니다.

- **Technical Details**: 기대값의 변화를 정량화하는 함수인 𝖦h는 주어진 보렐 측정 함수에 대해 두 확률 측정 P1과 P2의 변화에 따른 기대값 변동을 계산합니다. 조건부 확률 측정이 형성된 방식에 따라, 𝖦h의 기대값은 특정 조건 아래에서 정의됩니다. 이러한 모델링 방식은 확률적 구조와 관련된 다양한 수학적 개념들을 통합하여 설명합니다.

- **Performance Highlights**: 저자들은 새로운 기능의 중요성을 강조하며, 기대값의 변화를 더 잘 이해할 수 있는 도구를 제공합니다. 특히, PY|X 및 PX와 같은 조건부 확률 측정의 조합을 통해 실용적인 결과를 도출할 수 있음을 시사합니다. 이러한 연구 결과는 정보 이론을 기반으로 한 다양한 응용 분야에서도 전문가들에게 유용한 통찰을 제공합니다.
...
### Uncertainty Quantification with the Empirical Neural Tangent Kern (https://arxiv.org/abs/2502.02870)
Comments:
          24 pages, 5 figures, 9 tables

- **What's New**: 이 논문에서는 오버파라미터화된 신경망에 대해 사후(post-hoc) 샘플링 기반의 불확실성 정량화(Uncertainty Quantification, UQ) 방법을 제안합니다. 제안한 방법은 적절히 선형화된 네트워크에 대해 확률론적(stochastic) 경량 경사 하강법을 이용하여 효율적이고 의미 있는 딥 앙상블(deep ensemble)을 구축합니다. 이 연구는 특히 현대 신경망의 불확실성을 정량화하고 향상시키기 위한 새로운 접근 방식을 보여주고 있습니다.

- **Technical Details**: 제안된 '신경망 불확실성 정량화(Neural Uncertainty Quantification by Linearized Sampling, NUQLS)' 방법은 다수의 선형화된 모델 앙상블을 통해 예측 분포를 근사합니다. NUQLS는 경량이며 계산적으로 안정적이고, 쉽게 병렬화 할 수 있는 특성을 가지고 있습니다. 이는 경량의 특징 덕분에 다양한 머신 러닝 문제에 대한 예측 수행 시 기존 방법보다 우수한 성능을 보입니다.

- **Performance Highlights**: Numerical 실험을 통해 NUQLS는 경쟁 방식보다 연산 효율성에서 뛰어난 성능을 발휘하며, 종종 여러 배의 비용을 줄일 수 있음을 보여줍니다. 또한, 이 방법은 회귀와 분류 작업 모두에서 다양한 UQ 메트릭에서 최첨단 성능을 유지하고 있습니다. 특히 대규모 이미지 분류 작업에도 효과적으로 스케일링할 수 있는 가능성을 제시합니다.
...
- **What's New**: 최근 머신러닝(ML) 모델의 발전은 연구자들이 전통적인 온라인 알고리즘의 설계를 재고하는 계기가 되었습니다. 본 논문에서는 예측을 활용한 알고리즘 설계의 일환으로, 각 입력에 대한 신뢰도를 특정 방식으로 측정하는 칼리브레이션(calibration)을 제안합니다. 특히, 이를 통해 스키 대여 문제와 온라인 작업 스케줄링 문제에서 효과성을 입증하였습니다.

- **Technical Details**: 칼리브레이션은 예측자가 예측한 확률이 실제 관찰된 빈도와 일치하도록 하는 방법론입니다. 본 연구에서는 예측의 정확도에 따라 성능이 향상되는 알고리즘들을 개발하였으며, 높은 분산을 가지는 예측에서 칼리브레이션이 제공하는 장점을 강조합니다. 또한, 스키 대여 및 작업 스케줄링 사례 연구를 통해 이론적 성과를 실증적으로 검증하였습니다.

- **Performance Highlights**: 스키 대여 문제에서는 칼리브레이션된 예측자를 사용하여 최적의 예측 성능을 달성하였으며, 높은 분산이 있는 상황에서도 효과적인 성과를 입증하였습니다. 온라인 작업 스케줄링 문제에서는 전통적인 방식보다 더 나은 성과를 보였으며, 이는 칼리브레이션된 예측자가 주는 세부적인 불확실성을 바탕으로 한 것입니다. 실세계 데이터에서의 평가 결과는 이론적 발견의 실용적 영향을 잘 보여주고 있습니다.



### Gap-Dependent Bounds for Federated $Q$-learning (https://arxiv.org/abs/2502.02859)
- **What's New**: 본 논문에서는 tabular episodic finite-horizon Markov decision processes (MDPs)에서의 on-policy federated $Q$-Learning의 후회(regret)와 통신 비용(communication cost)을 처음으로 gap-dependent 분석을 제시합니다. 기존의 Federated Reinforcement Learning (FRL) 방법은 최악의 상황을 중심으로 연구되어 $	ext{surd{T}}$ 유형의 후회 경계를 나타냅니다. 그러나 저자들은 MDP의 긍정적인 구조를 활용하여 $	ext{log} T$ 유형의 후회 경계를 달성하였습니다.

- **Technical Details**: FRL은 여러 에이전트가 중앙 서버의 조정 하에 최적 정책(optimal policy)을 학습하는 분산 학습 방식이며, 이 논문에서는 비동질적인 전이 커널(inhomogeneous transition kernels)을 가진 episodic tabular MDPs에 대해 다룹니다. 각 에이전트는 S 상태와 A 행동을 갖는 episodic MDP와 H 단계로 상호 작용하며, 저자들은 탐색(exploration)과 활용(exploitation)을 분리하여 통신 비용 경계를 정제(refined communication cost bound)하였습니다.

- **Performance Highlights**: 저자들이 제안한 gap-dependent 후회 경계는 다중 에이전트 속도 향상 패턴을 나타내며, 통신 비용 경계는 $	ext{log} T$ 항에서 $MSA$에 대한 의존성이 제거되었습니다. 특히, $M=1$인 경우에는 글로벌 전환 비용(global switching cost)이 개선되며, 이를 통해 FRL의 학습과 통신 과정에서의 효율성을 높일 수 있음을 보였습니다.
...
- **What's New**: 본 연구는 Behavior Cloning (BC) 기법을 정보 이론적 관점에서 재조명하고, 잠재적 표현에서 중복 정보를 정량화하고 완화하기 위해 상호 정보(mutual information)를 도입합니다. 연구진은 정보 차단(Information Bottleneck, IB) 원리를 BC에 통합하여 관련 없는 정보를 압축하며 작업 관련 기능을 유지하는 구조화된 프레임워크를 제공합니다. 이는 다양한 방법론과 실험 설정에 걸쳐 잠재적 표현의 중복성을 종합적으로 연구한 최초의 시도입니다.

- **Technical Details**: BC는 전문가의 시연 데이터를 통해 상태-행동 쌍을 기반으로 학습하여 상태에서 행동으로의 매핑(mapping)을 학습하는 대표적인 모방 학습(Imitation Learning) 방법입니다. 본 연구에서는 BC 아키텍처를 공간적 융합(spatial fusion)과 시간적 융합(temporal fusion)으로 분류하여 각 융합 방법에 적용 가능한 시나리오를 제시합니다. IB 원리를 통해 잠재적 표현의 정보를 정리하고 중복성을 감소시켜 보다 강력한 기능을 학습하고 성능을 향상시킬 수 있음을 보여줍니다.

- **Performance Highlights**: CortexBench와 LIBERO 벤치마크에서 IB를 적용한 결과, 기존 BC 알고리즘의 일반화 성능이 크게 향상됨을 경험적으로 입증했습니다. 상세한 이론 분석도 제공하여 IB가 BC 방법의 전이 가능성(transferability)을 향상시키는 이유를 설명하고, 현재 로봇 궤적 데이터셋에서 상당한 중복성이 존재함을 간접적으로 강조합니다.



### Multimodal Brain-Computer Interfaces: AI-powered Decoding Methodologies (https://arxiv.org/abs/2502.02830)
- **What's New**: 본 리뷰 논문은 브레인-컴퓨터 인터페이스(BCI)에 대한 최근의 핵심 디코딩 알고리즘을 강조합니다. 특히, 교차 모달 매핑(cross-modality mapping), 순차 모델링(sequential modeling), 그리고 고전적인 다중 모달 융합(multi-modality fusion)에서의 알고리즘 발전을 다룹니다. AI의 발전이 뇌 데이터를 디코딩하는 방식을 어떻게 개선하고 있는지를 설명하며, 감정 인식, 시각 및 음성 디코딩 등 다양한 응용 분야에 대한 현재 문헌을 종합적으로 검토합니다.

- **Technical Details**: BCI 시스템은 입력 유도(input elicitation), 뇌 데이터 수집(brain data acquisition), 신경 기제 해석(neuromechanism interpretation), 출력 상호작용(output interaction)의 네 가지 주요 요소로 구성됩니다. 각 구성 요소는 사용자의 신경 반응을 유도하고, 이를 캡처하여 기능적인 출력으로 변환하는 중요한 역할을 합니다. 더욱이, AI 기반 디코딩 방법론은 고전 설계와의 차별성을 부여하여 다중 모달 BCI의 정교한 통합을 가능하게 합니다.

- **Performance Highlights**: 최근 AI 디코딩 알고리즘의 발전은 BCIs가 인간의 인지 상태를 해석하는 매우 효과적인 도구가 됨을 보여줍니다. 이 기술들은 헬스케어와 재활 분야 등에서 사람의 능력을 향상시키는 새로운 가능성을 제공하며, 특히 사회적 영향력이 큽니다. 미래의 연구 방향으로는 다중 모달 Transformers와 같은 새로운 아키텍처가 소개되며, 뇌 데이터의 이질성과 일반적인 오류와 같은 문제와 그 해결 방안도 논의됩니다.



### Accessible and Portable LLM Inference by Compiling Computational Graphs into SQL (https://arxiv.org/abs/2502.02818)
- **What's New**: 이번 논문에서는 대형 언어 모델(LLM)의 추론 그래프를 SQL 쿼리로 변환하는 새로운 컴파일러를 제안합니다. 이 접근 방식은 가장 널리 사용되는 관계형 데이터베이스(relational databases)를 실행 런타임으로 활용하여 LLM의 접근성을 높입니다. 특히, 기술 자원이 제한된 엣지 디바이스(edge devices)와 조직에 적합합니다.

- **Technical Details**: 우리는 행렬 곱셈(matrix multiplication) 및 어텐션(attention)과 같은 신경 연산자(neural operators)를 조인(join) 및 집계(aggregation)와 같은 관계형 기본 원칙(relational primitives)으로 매핑합니다. 이를 통해 데이터베이스의 디스크 기반 데이터 관리 및 네이티브 캐싱(native caching) 기능을 활용하며, 주요 트랜스포머 구성 요소인 어텐션 메커니즘과 키-값 캐싱(key-value caching)을 지원합니다. 최종적으로 SQL 파이프라인(SQL pipelines)을 생성하여 LLM 추론을 수행합니다.

- **Performance Highlights**: Llama3 가족을 사례 연구로 활용하여, 메모리 제약이 있는 시나리오에서 CPU 기반 프레임워크와 비교해 최대 30배의 속도 향상을 보여주었습니다. 이 연구 결과는 다양한 배포 환경에서 LLM 서비스를 위한 접근 가능하고 효율적인 솔루션을 제공합니다.
...
### LLM-USO: Large Language Model-based Universal Sizing Optimizer (https://arxiv.org/abs/2502.02764)
- **What's New**: 이번 논문에서는 대형 언어 모델 기반의 범용 사이징 최적화기(LLM-USO)를 제안합니다. LLM-USO는 아날로그 회로 설계 지식을 구조화된 텍스트 형식으로 표현하여 회로 디자인 최적화의 효율성을 높입니다. 이 방식은 유사한 서브 구조를 가진 회로에서 이전 최적화의 지식을 체계적으로 재사용할 수 있도록 도와줍니다.

- **Technical Details**: LLM-USO는 베이지안 최적화(Bayesian Optimization, BO)와 대형 언어 모델(Large Language Models, LLMs)을 통합한 하이브리드 구조를 가지고 있습니다. 이 프레임워크는 회로 간의 지식 전이를 촉진하고, 도메인 특정 지식을 BO 과정에 주입하여 설계를 최적화합니다. 이러한 통합은 자동화된 설계 과정에서 전문가의 인지 전략을 반영합니다.

- **Performance Highlights**: LLM-USO 프레임워크는 여러 아날로그 회로에서 뛰어난 성능 개선을 입증하였습니다. 특히, 정보 재사용 및 BO의 불확실성 추정치를 바탕으로 LLM이 생성한 제안의 우선 순위를 매김으로써 최적화 효율을 크게 증대시켰습니다. 이러한 접근 방식은 회로 설계를 가속화하며 디자인 품질을 향상시켰습니다.
...
### Achievable distributional robustness when the robust risk is only partially identified (https://arxiv.org/abs/2502.02710)
- **What's New**: 이 논문에서는 머신 러닝 모델이 최악의 경우의 분포 변화에 대해 잘 일반화되도록 하는 중요성을 강조합니다. 특히, 강건성(robustness)을 측정하기 위한 새로운 척도로 최악의 강건 리스크(worst-case robust risk)를 도입합니다. 이 리스크는 부분 식별 가능성(partial identifiability) 상황에서도 항상 명확하게 정의됩니다.

- **Technical Details**: 저자들은 부분적으로 식별 가능한 설정을 고려하여, 주어진 구조적 관계에 따라 최적의 최악 성능을 달성할 수 있는 모델을 찾으려고 합니다. 논문에서는 선형 모델을 대상으로 최악의 강건 리스크를 도출하고, 이를 통해 기존 강건성 방법들이 부분 식별 가능한 경우에 돌이킬 수 없는 손실을 초래할 수 있음을 증명합니다.

- **Performance Highlights**: 실험 결과, 기존의 강건성 방법들이 보지 못한 환경의 비율이 증가함에 따라 성능이 저하되는 경향을 보였습니다. 반면, 부분 식별 가능성을 고려하는 경우 더 나은 일반화 성능을 나타냈습니다. 이러한 발견은 부분적으로 식별 가능한 설정에서의 평가 및 벤치마크가 강건성 방법의 효과성을 판단하는 데 중요하다는 것을 보여줍니다.



### Rapidly Adapting Policies to the Real World via Simulation-Guided Fine-Tuning (https://arxiv.org/abs/2502.02705)
- **What's New**: 이 논문에서는 Simulation-Guided Fine-tuning (SGFT) 프레임워크를 제안합니다. 이 프레임워크는 물리적 시뮬레이터에서 추출한 구조적 우선순위를 활용하여 실제 환경에서의 적응을 가속화합니다. SGFT 방법은 시뮬레이션에서 학습된 가치 함수를 이용하여 실제 환경 탐색을 안내하며, 이를 통해 작은 양의 실제 데이터로도 성능 향상이 가능합니다.

- **Technical Details**: SGFT는 시뮬레이터에서 학습한 가치 함수 V_{sim}을 사용하여 dense reward를 생성해 목표 지향적인 실제 탐색을 유도합니다. 여기서 핵심은 시뮬레이션과 현실에서 성공적인 행동을 캡처하는 V_{sim}의 순서를 활용하는 것입니다. 기존의 불규칙한 탐색 전략 대신, 구조적 정보를 유지하며 학습할 수 있는 새로운 방법론을 제공합니다.

- **Performance Highlights**: SGFT는 기존의 기법들보다 상당히 더 적은 수의 실제 샘플을 요구하며, 높은 성능의 정책을 신속하게 학습할 수 있음을 보여줍니다. 특히, 이 접근 방식은 5개의 복잡한 조작 작업에서 기존의 기준 방법들보다 우수한 성과를 거두었습니다. 이론적으로도 SGFT는 불리한 오차를 제거하고, 샘플 복잡성을 효과적으로 개선한 것으로 나타났습니다.
...
### Three-dimensional signal processing: a new approach in dynamical sampling via tensor products (https://arxiv.org/abs/2502.02684)
- **What's New**: 이번 논문에서는 3차원 신호 복원과 관련된 동적 샘플링 문제를 다룹니다. 구체적으로, 텐서 곱(tensor product)에 의해 정의되는 진화 시스템의 맥락에서 연구를 수행하였습니다. 샘플링 집합에 대한 필수 조건을 제공하며, 이를 통해 3차원 신호의 성공적인 복원을 보장할 수 있습니다.

- **Technical Details**: 이 연구에서는 초기 신호가 구성된 형태를 고려하며, 시간에 따라 변화하는 신호를 3차원 텐서의 형태로 다룹니다. 또한, 최적화를 통해 복원 문제를 해결할 수 있도록 재구성하였습니다. 동적 샘플링 데이터는 특정 수학적 기호와 조합으로 표현되어 있으며, 이에 따라 효율적인 데이터 복원을 위한 조건들이 규명됩니다.

- **Performance Highlights**: 이 방법의 효과성을 보여주기 위해 여러 간단한 수치 시뮬레이션을 포함하였습니다. 실험을 통해 최적의 총 샘플링 시간을 결정하고, 주어진 샘플로부터 초기 신호를 성공적으로 복원할 수 있음을 입증합니다. 각종 산업 응용에서의 다차원 데이터의 복원이 중요한 평가 요소로 작용할 것으로 기대됩니다.



### Networks with Finite VC Dimension: Pro and Contra (https://arxiv.org/abs/2502.02679)
- **What's New**: 이 논문에서는 신경망의 대규모 데이터 세트를 통한 분류기 학습 및 근사화에 대한 고차원 기하학과 통계적 학습 이론의 역할을 조사합니다. 특히, VC(VC Dimension) 차원이 네트워크의 근사화 능력과 학습의 일관성에 어떤 영향을 미치는지를 비교합니다. 논문은 유한한 VC 차원이 경험적 오류의 균일 수렴을 가능하게 하지만, 특정 응용에서 함수의 근사화에는 적절하지 않을 수 있음을 보여줍니다.

- **Technical Details**: 고차원 기하학의 측정 집중(concentration of measure) 속성을 기반으로, 유한한 VC 차원을 가진 네트워크에서 발생하는 근사화 오류와 경험적 오류가 거의 결정론적으로 행동함을 증명합니다. 이 연구는 또한 ReLU 유닛을 가진 깊은 네트워크의 정확성과 일관성에 대한 영향을 논의합니다. 네트워크의 입력-출력 함수 집합의 성장을 고려하여 확률적 경계를 도출합니다.

- **Performance Highlights**: 네트워크의 유니버설 근사화 속성에도 불구하고 유한한 VC 차원을 가진 네트워크는 매우 큰 오류를 보일 수 있으며, 이는 함수의 발생 확률이 균일할 때 특히 두드러집니다. 또한, 이 논문은 근사화의 정확성과 데이터 학습의 일관성 사이의 균형을 논의하는데, VC 차원의 유한성은 두 오류의 집중을 악화시킬 수 있음을 보여줍니다.
...
### Learning to Double Guess: An Active Perception Approach for Estimating the Center of Mass of Arbitrary Objects (https://arxiv.org/abs/2502.02663)
Comments:
          Accepted to ICRA 25; 7 pages, 5 figures

- **What's New**: 본 논문에서는 U-GRAPH: Uncertainty-Guided Rotational Active Perception with Haptics라는 새로운 프레임워크를 소개합니다. 이 방법은 로봇 시스템이 여러 정보가 풍부한 상호작용을 통해 물체의 질량 중심(center of mass, CoM)을 보다 정확하게 추정할 수 있도록 하는 데 중점을 둡니다. Bayesian Neural Network (BNN)를 통합하여 불확실성을 정량화하고, Force-Torque (F/T) 센서의 한계를 극복함으로써 보다 유용한 데이터 기반 솔루션을 제공합니다.

- **Technical Details**: U-GRAPH 시스템은 6차원 힘-토크(F/T) 판독값과 2차원 방향 데이터를 처리하여 3차원 CoM 추정을 제공합니다. 이 시스템은 먼저 로봇이 고정된 방향으로 물체를 잡고 CoM의 사전 추정을 제공합니다. 이후 ActiveNet을 활용하여 정보 이득을 극대화하는 방향을 결정하고, 그에 따라 F/T 판독값에서 새롭게 CoM을 추정합니다. 각 액션을 평가하기 위해 그리드 탐색(grid search)을 사용하며, 2차원적인 그립 포즈의 방향을 조정합니다.

- **Performance Highlights**: U-GRAPH 시스템은 204개의 그립 실험을 통해 학습하였으며, 평균 1.47cm의 오차로 복잡한 실세계 물체에서도 우수한 성능을 보입니다. 이 시스템은 다양한 접촉 기하학, 표면 마찰 및 전체 형태를 가진 임의의 경질 물체에 대해 일반화 가능성을 보여줍니다. 결과적으로, 사전 훈련된 데이터 세트의 다양성이 제한적임에도 불구하고, 다양한 실제 물체에 대해 효과적으로 CoM을 추정하는 능력을 입증하였습니다.
...
### Regret-Optimized Portfolio Enhancement through Deep Reinforcement Learning and Future Looking Rewards (https://arxiv.org/abs/2502.02619)
Comments:
          11 pages, 7 figures

- **What's New**: 이 논문은 Proximal Policy Optimization (PPO)을 사용하여 기존의 포트폴리오 전략을 향상시키기 위한 새로운 에이전트 기반 접근법을 소개합니다. 전통적인 포트폴리오 구축보다는 이미 높은 성과를 내고 있는 전략을 동적인 리밸런싱을 통해 개선하는 데 중점을 두고 있습니다. Regret-based Sharpe 보상 함수를 통해 기존의 60/40 벤치마크(60% 주식, 40% 채권)를 능가하는 것을 목표로 합니다.

- **Technical Details**: 기존 연구에서 간과되기 쉬운 거래 비용 마찰을 다루기 위해 거래 비용 일정을 개발하였습니다. 또한, 원형 블록 부트스트랩 방법으로 합성 데이터 학습을 통해 일반화할 수 있는 할당 전략의 학습을 용이하게 합니다. 에이전트의 성과는 기간마다 훈련된 20개의 독립적인 에이전트의 평균 성과를 통하여 벤치마크에 대해 평가됩니다.

- **Performance Highlights**: 전략적 리밸런싱을 통해 기존 포트폴리오 전략의 성과를 향상시킬 뿐만 아니라, 다른 기초선에 비해 강력한 결과를 보여주고 있습니다. 두 가지 주요 평가 기준인 수익률(return)과 최대 낙폭(maximum drawdown)에서 개선된 성과를 달성하였습니다. 또한, 우리는 에이전트의 여러 구성 대비 벤치마크 방법 및 다른 두 가지 접근법과 비교하여 결과를 논의하였습니다.
...
### Physically Interpretable Representation and Controlled Generation for Turbulence Data (https://arxiv.org/abs/2502.02605)
- **What's New**: 이번 논문은 전통적인 Computational Fluid Dynamics (CFD) 방법론이 가진 자원 소모 문제를 해결하기 위해 데이터 기반 접근법을 제안합니다. Gaussian Mixture Variational Autoencoder (GMVAE)를 통해 고차원 과학 데이터를 저차원으로 압축하면서도 물리적으로 의미 있는 표현을 유지할 수 있는 방법을 다룹니다. 이를 통해 모델의 해석 가능성을 높이고, 복잡한 흐름의 클러스터링 및 생성 모델링을 촉진합니다.

- **Technical Details**: 제안된 GMVAE는 표준 Variational Autoencoder (VAE)와 Gaussian Mixture Model (GMM)을 결합하여 데이터의 잠재 공간에서 클러스터링을 수행합니다. 모델의 훈련 과정에서 Evidence Lower Bound (ELBO)를 최적화하고, Expectation-Maximization (EM) 알고리즘을 사용하여 클러스터의 평균과 분산을 업데이트합니다. 또한, 그래프 스펙트럼 이론을 활용하여 물리적 값의 부드러움(smoothness)을 평가하는 새로운 메트릭을 도입하여 잠재 표현의 해석 가능성을 높였습니다.

- **Performance Highlights**: GMVAE는 2D Navier-Stokes 시뮬레이션을 통해 연속적인 유체 흐름의 특성을 효과적으로 인코딩하며, 다른 차원 축소 방법들보다 더 개선된 클러스터링과 의미 있는 잠재 구조를 보여주었습니다. 연구 결과 GMVAE는 클러스터 수에 대해 강건한 특성을 보였으며, 유동 상태에 따라 압력 및 속도 필드를 생성할 수 있는 조건부 생성(generative) 모델링이 가능해졌습니다. 이제 이 프레임워크는 데이터 기반의 난류 모델링과 CFD 및 공학 시스템에 적용될 가능성을 제시합니다.



### OverThink: Slowdown Attacks on Reasoning LLMs (https://arxiv.org/abs/2502.02542)
- **What's New**: 이 논문에서는 reasoning LLMs(대형 언어 모델)의 응답 성능을 저하시키기 위해 OverThink 공격을 제안합니다. 이 공격은 사용자가 제공한 질문에 대한 문맥에 맞는 답변을 제공하면서도, 대량의 reasoning tokens를 소비하게 만드는 방식으로 작동합니다. OverThink 공격은 text의 안전성을 해치지 않으면서도 reasoning LLM의 연산 비용을 증가시키는 특징이 있습니다.

- **Technical Details**: OverThink 공격은 사용자가 입력한 데이터에는 변화가 없지만, 공격자가 제어하는 공공 소스에서 계산 복잡성이 큰 허구 문제를 주입함으로써 작동합니다. 이 공격은 context-aware 및 context-agnostic 방식으로 decoy task를 통합하여 reasoning LLM의 inference 속도를 저하시키고, 그 피해를 최소화하는 다양한 전략을 포함하고 있습니다. 연구 결과, 공격 후 FreshQA 데이터 세트에서 18배, SQuAD 데이터 세트에서 46배의 속도 저하가 관찰되었습니다.

- **Performance Highlights**: OverThink 공격은 서로 다른 모델과 데이터 세트에서 reasoning LLM의 레이블링 효율성을 현저하게 방해할 수 있는 것으로 확인되었습니다. o1 모델의 경우 SQuAD 데이터 세트에서 46배 증가의 reasoning tokens 증가가 관측되었고, DeepSeek-R1 모델에서는 최대 20배 증가의 결과를 보였습니다. 이 연구 결과는 reasoning LLM의 관리 및 보안 접근 방법 개발의 필요성을 강조합니다.



### Brief analysis of DeepSeek R1 and its implications for Generative AI (https://arxiv.org/abs/2502.02523)
- **What's New**: 2025년 1월 말, DeepSeek이 OpenAI의 모델과 경쟁할 수 있는 새로운 추론 모델 DeepSeek R1을 출시했습니다. 이 모델은 GPU 수출 금지에도 불구하고 개발 비용이 매우 저렴했으며, 특히 저렴한 비용으로도 경쟁력을 보여줍니다. 이 발표는 Generative AI 분야에 더 넓은 의미를 갖고 있으며, 최근 중국에서 발표된 다른 모델들과의 유사성도 분석하고 있습니다.

- **Technical Details**: DeepSeek R1 모델은 Mixture of Experts (MoE) 아키텍처와 Reinforcement Learning (RL)을 활용하여 개발되었습니다. DeepSeek V3 모델은 671억 개의 파라미터를 가지며, 다양한 특정 작업에 맞춘 소형 모델들을 조합해 훈련 부담을 줄이고 있습니다. R1 모델은 OpenAI의 o1 모델과 유사한 성능을 달성했으며, 자기 반영과 다른 접근 방식을 탐구하는 능력이 주목할 만한 발견으로 언급되었습니다.

- **Performance Highlights**: R1-Zero 모델은 AIME 2024에서 성능이 15.6%에서 71.0%로 향상되었으며, 이후 RL의 조정으로 86.7%를 달성했습니다. 최종 R1 모델은 AIME 2024에서 79.8%의 성능을 보였고, 이는 사용성과 가독성 간의 트레이드 오프가 있었음을 나타냅니다. 최근 연구에서는 이러한 모델의 추론 패턴을 소형 모델로 증류하는 방법이 다루어졌으며, 이는 더 작은 모델도 높은 성능을 달성할 수 있게 해줍니다.



### A User's Guide to Sampling Strategies for Sliced Optimal Transpor (https://arxiv.org/abs/2502.02275)
- **What's New**: 본 논문은 sliced optimal transport를 위한 샘플링 전략에 대한 사용자 가이드를 제공합니다. Sliced Wasserstein 거리(Sliced Wasserstein distance)에 대한 주의사항과 추가 정규성 결과를 제시하며, 각 전략의 구축 방법 및 이론적 보장을 설명합니다.

- **Technical Details**: 논문에서는 각 샘플링 전략의 생성 시간 복잡도(genaration time complexity)와 조건에 대해 상세히 설명합니다. 또한 sliced optimal transport에 대한 이론적 적합성(insight into their suitability)도 다루어, 이론적으로 각 전략이 어떻게 활용될 수 있는지를 깊이 있게 분석합니다.

- **Performance Highlights**: 시뮬레이션된 데이터와 실제 데이터 모두에 대한 광범위한 실험을 통해 전략들의 대표적인 비교를 제공합니다. 결과적으로, 각 샘플링 전략의 최적 사용을 위한 실제적인 권고 사항(practical recommendations)을 제시하며, 연구자들에게 유용한 정보를 제공합니다.

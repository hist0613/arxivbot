...
- **What's New**: 이 논문에서는 Language Models (LMs)의 과학적 발견을 가속화할 수 있는 잠재력에 대한 흥미로운 이론을 제시합니다. 특히, 연구자들이 가설을 반증(falsifying)하는 과정을 강조하며, 현재의 평가 기준이 해결책을 제시하는 능력에 초점을 맞추고 있음을 비판합니다. 새로운 평가 기준을 개발하여 LM이 미세하게 잘못된 해결책에 대한 반례(counterexamples)를 생성할 수 있는 능력을 측정해야 한다고 주장합니다.

- **Technical Details**: 이 연구에서는 알고리즘 문제 해결의 도메인에서 자동적으로 반례를 평가할 수 있는 REFUTE라는 동적으로 업데이트되는 벤치마크를 소개합니다. REFUTE는 프로그래밍 대회에서 잘못 제출된 문제와 최근 문제들을 포함하고 있으며, 체계적으로 잘못된 해결책에 대한 반례를 생성하는 과정을 분석합니다. 이를 통해, LMs의 반증 능력을 향상시키기 위한 방법론적 접근이 제안됩니다.

- **Performance Highlights**: 분석 결과, OpenAI o3-mini (high)는 코드 실행 피드백을 활용하더라도 REFUTE에서 잘못된 해결책의 <9%에 대해서만 반례를 생성할 수 있음을 보여주었습니다. 반면, 이 모델은 처음부터 48%의 문제를 해결할 수 있는 능력을 지니고 있음에도 불구하고 반례 생성에는 한계를 보였습니다. 이 연구는 LMs의 반증 능력을 평가하고 개선하는 데 기여하여, 연구 가속화와 자가 개선을 위한 중요한 도약을 마련할 수 있기를 희망합니다.
...
### Verde: Verification via Refereed Delegation for Machine Learning Programs (https://arxiv.org/abs/2502.19405)
- **What's New**: 이번 논문은 머신러닝(ML) 프로그램의 정확성을 보장하기 위한 새로운 방법으로, 비신뢰할 수 있는 컴퓨팅 제공자에게 작업을 위임하는 방식에 중점을 두고 있습니다. 제안된 방법인 '레퍼리 위임(refereed delegation)'은 적어도 하나의 서버가 신뢰할 수 있을 경우 올바른 결과를 보장합니다. 이 모델은 머신러닝 프로그램에 대한 분쟁 해결 프로토콜인 'Verde'와 비결정론을 제거하는 'RepOps' 라이브러리를 도입하여 효율성을 높입니다.

- **Technical Details**: 레퍼리 위임 모델은 두 가지 주요 기술적 도전을 해결해야 합니다. 첫째, ML 프로그램에 적합한 분쟁 해결 방법을 확립하는 것이고, 둘째, 다른 하드웨어 환경에서 ML 프로그램을 일관되게 재현할 수 있는 방법입니다. Verde는 신경망 매개변수와 높은 병렬성을 고려하여 설계되었으며, RepOps는 부동 소수점 연산의 순서를 고정하여 하드웨어 간 비결정론을 제거합니다.

- **Performance Highlights**: 제안된 방법은 기존의 암호학적 증명 시스템에 비해 4배 이상 효율적입니다. RepOps 사용 시 오버헤드는 60%에 불과하며, 전체적으로 레퍼리 위임은 프로그램의 최소 2배 오버헤드를 수용하더라도 1배수에서 10배수의 오버헤드 내에서 운영될 수 있습니다. 이는 분산 훈련 애플리케이션이나 위임된 계산의 감사(work)에서 매우 유용하게 활용될 수 있습니다.
...
- **What's New**: 이 논문에서는 대형 언어 모델(LLMs)이 일반 지능(AGI)으로 전환하기 위해 지식과 추론을 분리하는 새로운 접근 방식을 제안하고 있습니다. 기존 LLMs는 독창성이 부족하고 훈련 데이터에 과적합(overfitting)되는 경향이 있어 새로운 상황에 대한 일반화에 어려움을 겪고 있습니다. 연구자들은 강화 학습(RL)을 통한 새로운 추론 학습 방식을 도입하여 이 문제를 해결하려고 합니다.

- **Technical Details**: LLMs의 제한된 일반화 능력은 지식과 추론의 결합에 있다고 가설을 세우고, 세 가지 주요 방향을 제시합니다. 첫째, 전통적인 다음 토큰 예측(next-token prediction) 훈련 대신 RL을 이용한 추론 사전 훈련을 제안합니다. 둘째, 준synthetic 작업으로 구성된 커리큘럼을 사용하여 RL에서 추론 우선순위를 학습할 수 있도록 지원합니다. 셋째, 더 일반화 가능한 추론 기능을 학습하기 위해 작은 맥락 윈도우(context window)를 사용하여 허위 상관관계를 피합니다.

- **Performance Highlights**: 제안된 접근 방식은 LLMs가 새로운 프로그래밍 언어와 같은 생소한 맥락에서도 참조를 잘하고 적응할 수 있는 능력을 강화할 것으로 기대됩니다. 선택적 훈련 데이터의 영향을 통해, 모델이 더 강력하고 일반화 가능한 추론 능력을 발달시킬 수 있음을 보여줍니다. 실험 결과, 이 새로운 접근 방식이 기존 모델보다 더 나은 성능을 보여줄 가능성이 높습니다.
...
### HDEE: Heterogeneous Domain Expert Ensemb (https://arxiv.org/abs/2502.19385)
- **What's New**: 본 논문에서는 이종 모델 집합(ensemble)의 도메인 전문가 모델에서 이질성이 미치는 영향을 탐구합니다. 특히, 모델 크기와 훈련 단계 수를 도메인 난이도에 따라 조절하여 훈련 데이터 세트에 포함된 도메인과 제외된 도메인에 대한 성능을 비교합니다. 기존 연구와는 달리, 이기종 모델을 사용하는 방식은 통합 중앙 집중형 훈련 방식보다 더 효율적임을 보여줍니다.

- **Technical Details**: 우리는 ELMForests라는 독립적으로 훈련된 전문가 언어 모델의 집합을 정의합니다. 이 모델들은 각각 특정 소도메인에서 훈련되며, Branch-Train-Merge (BTM) 알고리즘을 통해 이질성이 도입됩니다. 이질성을 적용하여 다양한 모델 크기와 훈련 단계를 실험하였으며, 이를 통해 모델 간의 성능 비교를 진행했습니다.

- **Performance Highlights**: 이 논문에서 제안한 이기종 모델 집합은 허용된 $21$개의 데이터 도메인 중 $20$개에서 가장 낮은 perplexity 점수를 달성했습니다. 특히, 모델 크기를 다르게 하거나 훈련 단계를 조정한 경우가 동질적인 기준 모델보다 우수한 성과를 보였습니다. 이러한 연구 결과는 이기종 모델 집합의 이점이 단순히 데이터 도메인 간의 편차를 활용하는 방식에 기인함을 보여주고 있습니다.
...
### dCMF: Learning interpretable evolving patterns from temporal multiway data (https://arxiv.org/abs/2502.19367)
- **What's New**: 본 연구에서는 시간에 민감한 텐서 분해(tensor factorization) 방법인 d(ynamical)CMF를 제안합니다. 이 모델은 선형 동적 시스템(Linear Dynamical Systems, LDS) 구조에 따라 잠재 요인(latent factors)의 시간적 진화를 제한하여 시간 정보를 고려합니다. dCMF는 PARAFAC2 모델과의 관계를 탐구하며, 기존 텐서 분해 방식과의 차별점을 명확히 하였습니다.

- **Technical Details**: dCMF 모델은 텐서를 구성하는 각 요소가 시간(multiway) 차원에서 어떻게 변화하는지를 효과적으로 추적할 수 있도록 설계되었습니다. 이를 통해, 시간 점을 인덱스로 갖는 다차원 배열 구조를 잘 고려합니다. 라벨이 없는 데이터(unlabeled data)에 대한 고유한 혼합을 보장하면서, 기존 방법보다 더 유연하고 정확한 분석이 가능합니다.

- **Performance Highlights**: 실험 결과, dCMF는 PARAFAC2 및 t(PARAFAC2) 모델 대비 시간적 변화에 더 잘 대응할 수 있었으며, 특히 복잡한 패턴을 학습하는 데 있어 더욱 뛰어난 성능을 보였습니다. 노이즈가 있는 환경에서도 높은 정확도를 유지할 수 있는 경향을 보여, dCMF의 유용성이 입증되었습니다.



### Deep Learning For Time Series Analysis With Application On Human Motion (https://arxiv.org/abs/2502.19364)
- **What's New**: 이번 논문은 정기적으로 나열된 시간 지점으로 정의되는 시간 시계열 데이터 분석에 딥러닝(deep learning)을 활용하여 새로운 기법을 제안합니다. 주된 초점은 skeleton 기반의 운동 분석(motion analysis)과 데이터 부족 문제를 해결하기 위한 self-supervised learning(자기 감독 학습)입니다. 여기에는 인간의 운동 데이터를 위한 생성 모델(generative model)의 도입이 포함되어, 영상 제작 및 게임 개발에 적용할 수 있는 가치가 큽니다.

- **Technical Details**: 제안된 방법론은 기능 공학(feature engineering)을 통해 분류(classification)의 정확도를 향상시키고, compact yet state-of-the-art architecture를 개발하는 것을 목표로 합니다. 또한, 제한된 레이블이 있는 데이터를 처리하기 위해 self-supervised learning을 채택하였으며, 시간 시계열 데이터에서의 회귀(regression)와 행동 인식(action recognition)을 지원하기 위해 shape-based synthetic sample generation 방식을 제안합니다. 이러한 기술적 접근은 모형 평가에서의 한계를 극복하려는 노력을 포함합니다.

- **Performance Highlights**: 논문은 공개 데이터셋을 사용하여 실험적으로 입증된 새로운 통찰과 방법론을 제시하며, 실제 적용에 중점을 두고 있습니다. 이 연구는 시간 시계열 분석을 위한 강력한 평가 프레임워크의 필요성을 강조합니다. 이를 통해 운동 인식 및 재활 분야에서의 혁신적인 응용 프로그램을 위한 토대를 마련합니다.
...
### Recurrent Auto-Encoders for Enhanced Deep Reinforcement Learning in Wilderness Search and Rescue Planning (https://arxiv.org/abs/2502.19356)
Comments:
          Submitted to Machine Learning with Applications

- **What's New**: 이번 연구에서는 드론을 이용한 야생에서의 수색 및 구조 작업에서 정보를 최대한 수집할 수 있도록 돕는 새로운 알고리즘을 제안합니다. 반복 오토인코더(recurrent autoencoder)와 심층 강화 학습(deep reinforcement learning) 방법의 융합이 기존의 방법들보다 더 효율적이라는 점을 강조하고 있습니다. 특히, 이 모델은 학습에 필요한 매개변수 수가 적고 학습 시간도 크게 단축된다는 장점이 있습니다.

- **Technical Details**: 제안된 알고리즘은 특징 추출을 분리하는 방식으로 반복 오토인코더를 사용하여 드론의 비행 제어를 간소화합니다. 이를 통해 심층 강화 학습의 효율성을 높여 전체 전략의 성능을 극대화합니다. 연구에서는 또한 세 가지 추가 아키텍처를 구현하여 제안된 방법과의 종합적인 비교를 진행하며, 소프트 액터-비판기(soft actor-critic)와 근접 정책 최적화(proximal policy optimization) 기법이 적용됩니다.

- **Performance Highlights**: 제안된 아키텍처는 기존의 벤치마크와 비교하여 성능이 현저히 우수하다는 결과를 보였습니다. 특히, 소프트 액터-비판기를 활용한 모델은 전체 학습 가능 매개변수 수의 5분의 1 이하로 성능을 발휘하며, 학습 시간도 4분의 1에 불과한 결과를 나타냈습니다. 이러한 성과는 야생 환경에서의 구조 작업을 위한 최적의 솔루션을 제시합니다.



### CryptoPulse: Short-Term Cryptocurrency Forecasting with Dual-Prediction and Cross-Correlated Market Indicators (https://arxiv.org/abs/2502.19349)
Comments:
          10

- **What's New**: 이 논문은 암호화폐 시장의 변동성을 예측하기 위한 새로운 프레임워크인 ‘CryptoPulse’를 소개합니다. 이는 매크로 경제 변동, 기술 지표, 개별 암호화폐 가격 변화 등 세 가지 주요 요소를 통합하여 다음 날의 종가를 예측합니다. 이전의 예측 방법들은 종종 시장의 복잡한 동태를 간과했으나, 본 연구의 접근법은 독창적인 정제 메커니즘을 통해 시장 심리에 기반한 예측 개선을 가능하게 합니다.

- **Technical Details**: 논문에서 제안하는 모델은 이중 예측 메커니즘을 통해 구현됩니다. 하나는 매크로 조건 기반으로 가격을 예측하고, 다른 하나는 개별 암호화폐 동태에 따라 예측이 이루어지며, 이후 시장 심리에 따른 융합 과정을 통해 예측 정확도를 향상시킵니다. 이를 위해 연구진은 Yahoo Finance와 Cointelegraph 등의 출처에서 수집한 대규모 실제 데이터셋을 활용하여 모델 검증을 수행했습니다.

- **Performance Highlights**: 제안된 모델은 10개의 비교 방법들을 지속적으로 초월하는 성능을 보여주며, 최신 기술적 성과를 달성하였습니다. 특히, 다양한 암호화폐의 시대에 걸쳐 불안정한 성능을 극복할 수 있는 능력을 갖춘 것으로 평가됩니다. 연구의 결과는 실제 적용 가능성을 높이고, 더 나은 수익 예측을 위한 기반을 마련하는 데 기여할 것입니다.
...
### I Know What I Don't Know: Improving Model Cascades Through Confidence Tuning (https://arxiv.org/abs/2502.19335)
- **What's New**: 본 연구에서는 Gatekeeper라는 새로운 손실 함수(loss function)를 도입하여 소규모 모델들을 계단식(cascade) 설정에서 조정하는 방법을 제시합니다. 이 방법은 소규모 모델이 정확하게 다룰 수 있는 작업을 자신 있게 처리하도록 세밀하게 조정하고, 복잡한 작업은 대규모 모델에 위임하는 메커니즘이 포함되어 있습니다. 또한, 모델 성능과 위임 정확도 간의 균형을 관리하는 메커니즘을 포함하여 추가적인 구조적인 변화를 필요로 하지 않습니다.

- **Technical Details**: 소규모 모델이 정확한 예측에는 높은 신뢰도를, 잘못된 예측에는 낮은 신뢰도를 출력하도록 조정하여 불확실성 추정의 신뢰성을 향상시킵니다. Gatekeeper 손실 함수는 Vision-Language Models (VLMs) 등 다양한 작업에서 적용할 수 있는 내재된 메커니즘을 포함하고 있습니다. 이 연구에서는 이미지 분류, 언어 모델링 및 비전-언어 작업을 포함한 여러 아키텍처에서 Gatekeeper의 효용성을 검증하였습니다.

- **Performance Highlights**: Gatekeeper로 훈련된 모델은 CIFAR-100/TinyImagenet에서 각각 0.72배/2배, ARC-e/c에서 각각 7배/10배의 성능 향상을 보여주었습니다. 이러한 성과는 대규모 모델의 강점을 활용하면서 실시간 애플리케이션에서 고품질 결과를 제공하기 위한 보다 확장 가능하고 효율적인 배포 전략으로 나아가는 길을 열어줍니다.
...
- **What's New**: 최근 연구에서는 대형 언어 모델(LLM)의 학습 과정을 점검하고, 저작권 및 개인 정보 침해와 같은 위험을 식별하기 위한 감사(mechanism)를 강조합니다. 이 논문에서는 LLM의 유학습(unlearning) 연구를 중심으로, 특정 지식을 제거하면서도 모델의 나머지 응답의 무결성을 유지하는 접근법이 필요함을 논의합니다. 연구 결과, 제안된 G-effect 툴킷이 다양한 유학습 목표에 대한 모델 성능의 영향을 정량적으로 평가할 수 있는 새로운 방법론으로 주목받고 있으며, 이는 기존 방법론의 한계를 이해하는 데 중요한 통찰을 제공합니다.

- **Technical Details**: 논문에서는 고차원 언어 모델의 일반적인 구조와 훈련 방식에 대해 설명하며, 이러한 LLM들이 어떻게 렌덤 데이터로부터 자극받는 경향이 있는지를 강조합니다. LLM 유학습의 목표는 두 가지로, (a) 특정 지식의 제거와 (b) 비대상 데이터에 대한 응답의 무결성 보장입니다. G-effect 개념은 선택된 유학습 목표와 관련된 성능 변화를 기울기를 통해 근사할 수 있도록, 심층적 분석을 가능하게 합니다.

- **Performance Highlights**: 유학습 과정이 피라미드 형태의 신경망을 통해 실행될 때 각 층의 반응을 분석한 결과, 얕은 층에서 더 뚜렷한 영향을 관찰했습니다. 그러나 과도한 유학습이 모델의 전반적인 성능 저하를 초래할 수 있음을 경고하며, 정규화 기법이 성능 유지에 필수적이라는 점을 강조합니다. 또한, G-effect 분석을 통해 도출된 여러 새로운 최첨단 방법이 추가 연구 대상으로서의 가치가 있음을 제시하고 있습니다.



### Global Graph Propagation with Hierarchical Information Transfer for Incomplete Contrastive Multi-view Clustering (https://arxiv.org/abs/2502.19291)
- **What's New**: 이번 연구는 불완전 다중 시각 클러스터링(incomplete multi-view clustering) 문제를 해결하기 위해 계층적 정보 전달(hierarchical information transfer) 기술을 적용한 새로운 방법인 GHICMC(Globally graph propagation with Hierarchical information transfer for Incomplete Contrastive Multi-view Clustering)를 제안합니다. 기존 방법들이 놓치는 데이터 속 숨겨진 정보를 효과적으로 활용하지 못한다는 문제를 해결하고, 표현 학습과 클러스터링 과정을 통합한 최적화 프레임워크입니다. 이러한 접근을 통해 데이터의 그래프 구조를 더 잘 활용할 수 있게 되었습니다.

- **Technical Details**: GHICMC는 그래프 컨볼루셔널 네트워크(Graph Convolutional Networks, GCN)를 사용하여 각 뷰에 대한 특화된 표현(view-specific representation)을 학습합니다. 이는 결합된 합의 표현(consensus representation)으로 융합되며, 글로벌 그래프 전파(global graph propagation) 기술을 도입하여 누락된 데이터를 처리합니다. 또한, 가중치 공유 가상 분류기(weight-sharing pseudo-classifier)를 통해 대조 학습(contrastive learning)을 사용하여 최종 클러스터링 결과를 획득하는 방법론을 제안합니다.

- **Performance Highlights**: 다섯 개의 다중 시각 데이터셋에서 수행된 실험을 통해 GHICMC는 기존의 최첨단 방법들에 비해 우수한 성능을 보여주고 있음을 확인했습니다. 특히, 결합된 접근 방식을 통해 누락된 데이터의 정보를 효과적으로 활용하며, 최적의 클러스터링 결과를 달성할 수 있음을 입증하였습니다. 이 연구 결과는 불완전 다중 시각 데이터 처리에 있어 새로운 통찰을 제공합니다.
...
### Set and functional prediction: randomness, exchangeability, and conforma (https://arxiv.org/abs/2502.19254)
Comments:
          15 pages

- **What's New**: 본 논문에서는 conformal prediction(적합 예측)의 효율성을 더 일반적인 randomness prediction(무작위 예측) 및 exchangeability prediction(교환 가능 예측)과 비교하여 연구합니다. 이 연구는 분류(classification) 문제에 국한되지 않으며 회귀(regression) 문제에도 적용될 수 있는 결과를 제시합니다. 이 연구의 핵심 메시지는 conformal prediction과 randomness prediction 사이의 차이가 크지 않지만, 이를 효과적으로 활용할 수 있는지는 여전히 열려있는 질문이라는 점입니다.

- **Technical Details**: conformal predictor(적합 예측기)는 예측 집합(prediction set)을 생성하는 방법으로, 각 테스트 객체에 대한 p-value 함수를 생성합니다. p-value 대신 e-value를 사용하는 conformal e-predictors(적합 e-예측기)는 예측 기능을 생성하는 데 있어 유용한 기술 도구가 됩니다. 이 두 예측기의 연결성을 증명하기 위해, conformal p-predictors와 conformal e-predictors 간의 관계를 Establish 하며, 이는 교환 가능성(e)과 무작위성(e) 예측기 간에도 얼마든지 확장 가능합니다.

- **Performance Highlights**: 논문에서 제시된 결과에 따르면, conformal predictors의 예측 기능은 평균적으로 모든 randomness predictors와 경쟁력이 있다는 것을 확인했습니다. 이 평균적인 효율성은 테스트 예제에 따라 달라질 수 있는 임의의 확률 측정값에 관한 것입니다. 특히, 이 논문에서는 분류 및 회귀 문제에서의 적용 사례를 통해 표현된 성과를 강조하고, 결론에서 연구의 주요 결과를 요약합니다.
...
### INFO-SEDD: Continuous Time Markov Chains as Scalable Information Metrics Estimators (https://arxiv.org/abs/2502.19183)
- **What's New**: INFO-SEDD는 고차원 이산 분포의 정보 이론적 양을 추정하기 위한 새로운 방법을 제시합니다. 기존의 방법은 이산 데이터를 연속 공간에 임베딩하여 추정하지만, INFO-SEDD는 Continuous-Time Markov Chains (CTMCs)를 활용하여 이러한 문제를 해결합니다. 이 방법은 단일 매개변수 모델을 훈련시키는 것으로 높은 계산 및 메모리 장점을 제공합니다.

- **Technical Details**: 이 논문에서는 CTMC의 시간 역전 성질과 Kullback-Leibler (KL) 발산 개념을 연결하여, 이산 상태 공간에서의 정보 이론적 양을 추정하는 방법을 설명합니다. INFO-SEDD는 Dynkin의 정리를 통해 확장된 점을 활용하여 여러 변수 간의 상호정보량 및 엔트로피 계산을 가능하게 합니다. 이 접근법은 고차원 문제에서도 효율적으로 작동하며 사전 훈련된 네트워크와의 통합이 용이합니다.

- **Performance Highlights**: INFO-SEDD는 synthetic benchmark에서 신뢰성과 성능이 입증되었으며, 기존의 neural estimation 방법을 능가하는 결과를 보여줍니다. 또한 실제 이산 모델인 Ising 모델을 기반으로 한 엔트로피 추정에서도 뛰어난 정확성을 발휘하여, 복잡한 이산 데이터 분포에 대한 신뢰할 수 있는 추정기로 자리매김하고 있습니다. 이러한 결과는 INFO-SEDD가 고차원 시나리오에서도 확장 가능성을 지닌 강력한 새로운 추정기임을 강조합니다.
...
### A Model-Centric Review of Deep Learning for Protein Design (https://arxiv.org/abs/2502.19173)
- **What's New**: 딥 러닝은 단백질 디자인에 혁신을 가져와 정확한 구조 예측, 서열 최적화, 그리고 새로운 단백질 생성(de novo protein generation)을 가능하게 했습니다. AlphaFold2, RoseTTAFold, ESMFold 등에서의 단일 체인(single-chain) 단백질 구조 예측의 발전은 거의 실험적 정확도에 이르렀으며, 이후 연구는 바이오 분자 복합체(biomolecular complexes)로까지 확장되었습니다. 더불어 새롭게 등장한 공동 서열-구조(co-design) 모델들, 예를 들어 ESM3는 두 가지 모드를 통합한 통합 프레임워크로 향상된 디자인 가능성을 보여주고 있습니다.

- **Technical Details**: 최근 고도화된 생성 모델(generative models)인 ProtGPT2, ProteinMPNN, RFdiffusion는 자연 선택 기반의 한계를 넘어 서열(sequence)과 뼈대(backbone) 디자인을 가능하게 하였습니다. 그럼에도 불구하고 서열-구조-기능(sequence-structure-function) 관계 모델링과 훈련 데이터의 범위를 넘어서는 일반화(generalization)의 보장과 같은 도전 과제가 여전히 존재합니다. 향후 발전은 이러한 모드가 독립적으로 처리되는 모델보다 피트니스(fitness) 풍경을 더 효과적으로 모델링할 수 있는 공동 서열-구조-기능 co-design 프레임워크에 초점을 맞출 가능성이 높습니다.

- **Performance Highlights**: 현재의 기술 능력은 매우 놀라운 발전 속도에 결합되어 있으며, 이는 곧 자연 선택이 부과한 한계를 초월하는 맞춤형 구조와 기능을 가진 단백질을 신속하고 합리적으로 설계할 수 있게 할 것입니다. 이 리뷰에서는 단백질 디자인을 위한 딥 러닝 방법의 현재 능력을 논의하며, 가장 혁신적이고 능력 있는 모델들의 기능과 활용 가능성을 집중적으로 살펴봅니다. 또한, 현재 이 분야의 도전 과제와 앞으로 나아가야 할 최적의 경로에 대해서도 논의합니다.



### On the Byzantine Fault Tolerance of signSGD with Majority Vo (https://arxiv.org/abs/2502.19170)
- **What's New**: 이번 논문에서는 signSGD 알고리즘의 내성을 다루고 있으며, 기존의 약한 적대자에 대한 내성뿐만 아니라 모든 정보를 공유하는 전지전능한 적대자에 대해서도 내성을 증명합니다. 이를 통해 signSGD 알고리즘이 Byzantine Fault Tolerance (BFT)를 갖추고 있음을 입증하고 있으며, 이는 대규모 AI 모델의 분산 학습에서 중요한 이슈입니다. 이 연구는 signSGD가 복잡한 공격에도 불구하고 여전히 효율적으로 작동할 수 있음을 보여줍니다.

- **Technical Details**: 연구에서는 signSGD 알고리즘의 전개를 위해 BFT 프레임워크를 기반으로 하여, 전지전능한 적대자에게서 견뎌낼 수 있는 방법을 제안합니다. 이를 위해 특히 공격자의 협동 가능성을 고려하여 전략적 공격을 분석하고 최적 공격에 대한 우상을 제시합니다. signSGD는 미니 배치 확률적 경량 기법으로, 각 구성 요소의 부호만을 전송하여 통신 성능을 최적화하면서도 악의적인 공격으로부터의 내성을 강화합니다.

- **Performance Highlights**: 실험은 다양한 강도의 적대자와 함께 MNIST 데이터세트에서 수행되었으며, signSGD 알고리즘이 극복할 수 있는 공격 유형을 다루었습니다. 특히, 우리의 접근법은 기존 방법과 비교하여 알려지지 않은 상수를 사용하지 않고도 느리지만 안정적인 수렴 속도를 유지할 수 있음을 입증합니다. 결과적으로, signSGD는 강한 적대자 환경에서도 효과적인 성능을 보여줍니다.



### Generalizable deep learning for photoplethysmography-based blood pressure estimation -- A Benchmarking Study (https://arxiv.org/abs/2502.19167)
Comments:
          20 pages, 5 figures, code available at this https URL

- **What's New**: 이 논문은 Photoplethysmography (PPG) 기반으로 혈압(BP) 예측을 위한 최신 딥 러닝 모델을 소개하며, 기존의 커프 기반 혈압 측정 방법에 대한 유망한 대안을 제공합니다. PulseDB 데이터셋을 사용하여 모델을 훈련하고, 내부 및 외부 데이터셋에서의 성능을 평가하여 모델의 일반화 가능성을 검증했습니다. 가장 우수한 모델인 XResNet1d101은 내부 데이터셋에서 수축기 및 이완기 혈압에 대해 각각 9.4 mmHg와 6.0 mmHg의 MAE(Mean Absolute Error)를 기록했습니다.

- **Technical Details**: 이 연구에서는 PulseDB 데이터셋을 활용하여 훈련된 여러 딥 러닝 모델의 내부(Internal) 및 외부(Out-of-Distribution, OOD) 일반화 성능을 평가합니다. 전통적인 PPG 데이터 처리 방식이 가진 한계를 극복하기 위해 머신 러닝과 딥 러닝 기술을 사용하여 PPG 신호에서 자동으로 특징을 추출합니다. 또한, 성능 개선을 위한 표본 기반의 도메인 적응(domain adaptation) 접근법을 조명하며, PPG 신호와 혈압 분포 간의 차이로 인해 성능이 크게 영향을 받을 수 있음을 강조합니다.

- **Performance Highlights**: 최고 성능의 모델인 XResNet1d101은 내부 데이터셋 PulseDB에서 수축기 및 이완기 혈압의 MAE가 각각 14.0 mmHg와 8.5 mmHg로, 외부 데이터셋에서는 수축기 혈압이 15.0-25.1 mmHg, 이완기 혈압이 7.0-10.4 mmHg로 나타났습니다. 이는 모델 성능이 데이터셋 간의 혈압 분포 차이에 의해 크게 영향을 받음을 시사합니다. 연구팀은 또한 효과적인 일반화 속성을 지닌 모델 훈련을 위한 권장 사항을 제시하며, OOD 일반화의 중요성과 도전 과제를 강조하고 있습니다.



### Design of Cavity Backed Slotted Antenna using Machine Learning Regression Mod (https://arxiv.org/abs/2502.19164)
- **What's New**: 이번 논문에서는 회귀 기반의 머신러닝 모델을 활용하여 캐비티 백드 슬롯 안테나(cavity backed slotted antenna)의 설계를 수행하였습니다. 이 안테나는 군사 및 항공 통신 시스템에서 일반적으로 사용됩니다. 새로운 접근법은 전자기 솔버(electromagnetic solver)를 통해 생성된 반사 계수(reflection coefficient) 데이터를 활용하여 안테나의 최적 구성을 빠르게 예측할 수 있게 합니다.

- **Technical Details**: 캐비티 백드 슬롯 안테나의 초기 반사 계수 데이터는 1GHz에서 8GHz까지의 넓은 주파수 대역을 아우르는 입력 반사 계수에 기반하여 생성됩니다. 이 데이터를 바탕으로 회귀 모델이 훈련되어 다양한 주파수에서의 안테나 크기를 예측합니다. 이 과정은 반복적인 물리적 테스트와 수동 조정의 필요성을 줄여 설계 및 개발 비용의 절감으로 이어집니다.

- **Performance Highlights**: 제안된 모델은 1GHz에서 8GHz까지의 다중 주파수 공진에 대한 예측에서도 그 범용성을 입증하였습니다. 이는 레이더(radar), 군사 식별 시스템(military identification systems), 안전한 통신 네트워크에 이르기까지 다양한 실제 응용 프로그램에서 머신러닝을 활용하여 안테나 설계를 개선할 수 있는 잠재력을 보여줍니다.



### Design of Resistive Frequency Selective Surface based Radar Absorbing Structure-A Deep Learning Approach (https://arxiv.org/abs/2502.19151)
- **What's New**: 이 논문에서는 저항성 주파수 선택 표면(resistive frequency selective surface)을 이용한 레이더 흡수 구조(radar absorbing structure) 설계를 위한 딥 러닝 기반 접근법이 제안되었습니다. 이 설계에서는 반사 계수(reflection coefficient)를 딥 러닝 모델의 입력으로 사용하며, 예루살렘 십자가(Jerusalem cross) 기반의 유닛 셀(unit cell) 치수를 예측 결과로 도출합니다. 이렇게 설계된 레이더 흡수기는 L에서 Ka 대역에서 작동하도록 멀티 주파수 대역(absorber) 설계를 가능하게 합니다.

- **Technical Details**: 제안된 모델에서 순차 신경망(sequential neural network) 기반의 딥 러닝 방식과 적응적 모멘트 추정(adaptive moment estimation) 최적화를 사용하여 다중 주파수 대역 흡수기를 설계합니다. 이 모델은 유닛 셀 파라미터와 두께에 따라 설계된 레이더 흡수기를 제공합니다. 딥 러닝 모델의 결과는 풀 웨이브 시뮬레이션 소프트웨어(full-wave simulation software)와 비교되며, 우수한 일치를 보입니다.

- **Performance Highlights**: 제안된 모델은 단일 유닛 셀과 주파수 범위에 걸친 두께를 사용하여 다양한 레이더 흡수 구조를 저비용으로 설계하는 데 활용할 수 있습니다. 이 접근 방식은 레이더 흡수기 설계의 효율성과 정확성을 높여줍니다. 결과적으로, 기존 방법에 비해 경제성 및 성능 모두에서 이점을 제공합니다.



### Random Similarity Isolation Forests (https://arxiv.org/abs/2502.19122)
- **What's New**: 본 논문에서는 다양한 데이터 유형을 처리할 수 있는 새로운 이상치 탐지 알고리즘인 Random Similarity Isolation Forest (RSIF)를 제안합니다. RSIF는 기존의 방법들이 특정 데이터 유형에 맞춰져 있는 한계를 극복하며, 혼합된 데이터셋을 다룰 수 있는 능력을 갖추고 있습니다. 특히 이 알고리즘은 다양한 데이터 표현을 변환하거나 여러 감지기를 합치는 과정을 피할 수 있어 실제적입니다.

- **Technical Details**: RSIF는 데이터 타입의 유연성을 위해 유사성 기반의 프로젝션과 고립 개념을 결합하여 작동합니다. 실험에 사용된 데이터셋은 수치 데이터, 복잡한 객체 혹은 혼합된 다중 모달 설명을 포함하며, 이를 통해 알고리즘의 범용성을 입증합니다. 우리는 다중 데이터 모드에 대해 다양한 유사 함수들을 테스트하였고, 올바른 데이터 프로젝션을 선택하는 것이 비지도 학습 방식에서 얼마나 중요한지를 강조합니다.

- **Performance Highlights**: RSIF는 47개의 기준 데이터셋에서 다섯 개의 최신 알고리즘들을 능가하는 성능을 보여줍니다. 이 연구는 다중 모달리티를 사용함으로써 이상 탐지 성능을 향상시킬 수 있음을 보여주고, 다중 모달 환경에 적합한 새로운 이상 탐지 벤치마크의 필요성을 강조합니다. 따라서 RSIF는 다양한 센서 정보와 데이터 유형을 결합한 복잡한 환경에서 유용하게 사용될 수 있습니다.
...
### Software demodulation of weak radio signals using convolutional neural network (https://arxiv.org/abs/2502.19097)
Comments:
          4 pages, 8 figures. This is the preprint version of the paper published in 2020 IEEE 7th International Conference on Energy Smart Systems (ESS). The final version is available at IEEE Xplore: this https URL. arXiv admin note: text overlap with arXiv:2502.16371

- **What's New**: 본 연구에서는 JT65A 라디오 통신 프로토콜을 전력 시스템의 광역 감시 시스템에서 데이터 교환을 위해 제안했습니다. 특히, 깊은 합성곱 신경망(deep convolutional neural network)을 활용하여 JT65A 통신 프로토콜로 전송된 다중 주파수 변위 키잉(multiple frequency shift keying) 약한 신호의 소프트웨어 복조(demodulation)를 조사했습니다.

- **Technical Details**: 본 연구는 -30 dB에서 0 dB 사이의 평균 신호 대 잡음 비율(signal-to-noise ratios)에서 덧셈성 백색 가우시안 잡음(additive white Gaussian noise) 하에서 프로토콜의 간섭 면역성(interference immunity)에 중점을 두었습니다. 또한, 비동기 복조(non-coherent demodulation)의 이론적 한계보다 약 1.5 dB 낮은 간섭 면역성을 입증했습니다.

- **Performance Highlights**: 심볼(symbol) 및 비트(bit) 오류율(error rates) 형태로 복조 성능을 제시하였으며, 연구 결과는 JT65A 통신 프로토콜의 효과성을 보여줍니다. 첫 번째로 얻어진 이 결과는 전력 시스템 모니터링에 대한 새로운 통신 기술 발전을 의미합니다.



### MCLRL: A Multi-Domain Contrastive Learning with Reinforcement Learning Framework for Few-Shot Modulation Recognition (https://arxiv.org/abs/2502.19071)
- **What's New**: 이 논문은 자동 변조 인식(AMR) 분야에서 제한된 수의 라벨이 있는 샘플로도 만족스러운 성능을 달성할 수 있도록 하는 새로운 프레임워크인 MCLRL을 소개합니다. MCLRL은 다중 도메인 대조 학습(multi-domain contrastive learning)과 강화 학습(reinforcement learning)을 결합하여 기능의 풍부함을 높이고, 분류를 위해 심층 기능을 추출합니다. 특히, 이 연구에서는 AMR 작업에서 최소한의 샘플과 훈련 주기를 사용하여 우수한 성능을 달성하는 방법을 제시합니다.

- **Technical Details**: MCLRL 프레임워크는 세 가지 주요 구성 요소—인코더 사전 훈련, 특징 융합(feature fusion) 및 분류기 미세 조정(classifier fine-tuning), 하류 작업 평가(downstream task evaluation)—로 이루어져 있습니다. 인코더 사전 훈련 단계에서, 대조 학습은 세 개의 표현 도메인 간의 상호 감독을 활용하며, 강화 학습이 최적의 데이터 증강(intensity) 강도를 선택합니다. 이렇게 추출된 특징은 주성분 분석(projection head)을 통해 정규화되고, 내부 도메인(intra-domain) 및 상호 도메인(inter-domain) 조인트 손실 함수로 통합됩니다.

- **Performance Highlights**: MCLRL은 RML2016.10a 및 Sig2019-12 데이터셋에서 전통적인 이미지 대조 학습(Image Contrastive Learning) 방법 및 시간 시계열 대조 학습(Time-Series Contrastive Learning) 방법과 비교했을 때 3%에서 15%까지 더 높은 정확도를 기록했습니다. 이 결과들은 MCLRL의 접근법이 기존 방법에 비해 우수하다는 것을 강조합니다.



### A Sample-Level Evaluation and Generative Framework for Model Inversion Attacks (https://arxiv.org/abs/2502.19070)
Comments:
          Accepted to be appeared in 39th Annual AAAI Conference on Artificial Intelligence (AAAI-25)

- **What's New**: 이 논문은 Neural Networks의 모델 역전(Model Inversion, MI) 공격에 초점을 맞추고 있으며, 이러한 공격이 훈련 데이터셋을 재구성할 수 있음을 보여줍니다. 특히, 논문에서는 기존 연구에서 간과된 개별 샘플 수준의 개인정보 보호의 중요성을 강조합니다. 새로운 평가 척도인 Diversity and Distance Composite Score (DDCS)를 도입하여 샘플 수준의 사생활 보호를 개선할 수 있는 방법을 제시합니다.

- **Technical Details**: DDCS는 MI 공격의 특성을 포괄적으로 평가하며, 각 훈련 샘플의 재구성 신뢰도를 측정합니다. 이 메트릭은 다양한 속성(예: distance, coverage, distributional similarity)을 통합하여 MI 공격의 성공적 평가를 가능하게 합니다. 또한, 이 연구에서는 싱글 타겟 샘플의 개인정보를 보호하기 위한 GAN 대체 프레임워크를 제안합니다.

- **Performance Highlights**: 폭넓은 실험을 통해 DDCS의 효과성을 입증하였으며, 이는 현재의 MI 공격에서 샘플 복구율을 입증합니다. DDCS는 MI 방어에도 활용될 수 있으며, 공격에 취약한 샘플을 비지도 학습 방식으로 식별하는 데 유용하다는 점을 보여줍니다. 이 연구는 MI 공격의 성능을 향상시키기 위한 새로운 접근 방식을 제시합니다.



### Foundation Inference Models for Stochastic Differential Equations: A Transformer-based Approach for Zero-shot Function Estimation (https://arxiv.org/abs/2502.19049)
- **What's New**: 이번 연구에서는 FIM-SDE(Foundation Inference Model for SDEs)라는 새로운 변환기 기반의 인식 모델을 도입합니다. 이 모델은 다양한 차원의 경험적 과정에서 존재하는 잡음과 불규칙한 관측 데이터를 바탕으로 SDE의 drift와 diffusion 함수를 정확하게 비지도 추정할 수 있습니다. 기존 기법들이 소음이나 희소성 있는 데이터에 대해 민감한 것과 달리, FIM-SDE는 매우 다양한 합성 및 실제 데이터셋에서 견고한 기능 추정을 가능하게 합니다.

- **Technical Details**: FIM-SDE는 신경망의 개념을 활용하여 SDE 경로에서 잡음이 있는 관측 데이터를 목표 drift 및 diffusion 함수로 매핑합니다. 모델은 전통적인 기호 회귀(symbolic regression) 방식이 아닌, 신경 연산자(neural operators)를 통한 학습을 통해 SDE의 다양한 동적 시스템을 포괄하는 방식을 채택하였습니다. 훈련 과정에서 대규모의 잡음 있는 데이터셋을 사용하여 모델이 SDE의 동작을 효과적으로 파악할 수 있도록 합니다.

- **Performance Highlights**: 이 연구에서 제안된 FIM-SDE는 전통적인 기계 학습 방법과 달리, 매번 새로운 시스템에 대한 개별 최적화 없이 0-shot(fine-tuning 없이) 모드에서 기능을 정확하게 추정할 수 있는 능력을 갖추고 있습니다. 실제 데이터(예: 유가, 풍속 변동, 인체 동작 기록)에서도 뛰어난 성능을 보여주며, 다양한 합성 SDE 시스템에서도 일관되게 잘 작동합니다.
...
- **What's New**: 이번 연구에서는 llull-environment라는 공기 오염 예측 시스템을 소개합니다. 이 시스템은 스페인 마드리드와 바야돌리드에서 운영 중인 모델에 영감을 받아 개발되었으며, 주요 오염물질(NOx, O3, PM10, PM2.5)의 평균 오염 수준을 예측할 수 있는 인코더-디코더 CNN(Convolutional Neural Network)을 포함하고 있습니다. 이 논문에서는 주목(attention) 메커니즘을 추가하여 예측 정확성을 향상시키는 방안을 탐구합니다.

- **Technical Details**: llull-environment는 역사적 데이터와 외부 예측을 이용하여 오염 수준을 예측하는 복합적인 구조를 가지고 있습니다. 이 시스템은 스페이셜-템포럴(Spatial-Temporal) 컨볼루션 레이어를 활용하여 모든 측정소의 데이터를 동시에 처리합니다. 본 논문에 도입된 하이브리드 강화를 통해, 모델은 "하이브리드 강화 자기회귀 변환기(Hybrid Enhanced Autoregressive Transformer, HEART)"로 명명되었습니다.

- **Performance Highlights**: 모델의 성능 개선은 평균 제곱 오차(Mean Square Error, MSE)를 기준으로 평가되었으며, 평균 7.5%의 감소가 관찰되었습니다. 특정 조정 구조에서는 최대 22%의 MSE 감소가 확인되었으며, 이는 오염 물질에 따라 성능 차이가 존재함을 보여줍니다. 이 접근법은 공기 품질 예측 모델을 최적화하는 것에 국한되지 않고 일반적인 시간 시계열 예측에도 적용 가능합니다.
...
### Gaussian Process Upper Confidence Bound Achieves Nearly-Optimal Regret in Noise-Free Gaussian Process Bandits (https://arxiv.org/abs/2502.19006)
Comments:
          14 pages

- **What's New**: 이 논문은 노이즈가 없는 Gaussian Process (GP) bandits 문제를 연구하며, 학습자는 알려진 재생 커널 Hilbert 공간(RKHS)에서 존재하는 블랙박스 목적 함수의 노이즈 없는 관측을 통해 레그렛(regret)을 최소화하고자 합니다. GP-UCB(Gaussian Process Upper Confidence Bound)는 이 문제에 대한 잘 알려진 알고리즘으로, 이 논문에서는 GP-UCB의 이론적 한계와 실험적 성능 사이의 간극을 해소하고자 합니다. 기존의 알고리즘들은 이론적으로 더 뛰어난 결과를 보였지만, 실제 성능은 GP-UCB에 못 미치는 경우가 많았기 때문에 본 연구는 GP-UCB의 우수한 성능을 강조합니다.

- **Technical Details**: 이 논문에서 다루는 GP-UCB의 레그렛 분석은 식별된 커널에 대해 아래의 두 가지 주요 결과를 포함합니다: 첫 번째로, 스퀘어드 익스포넨셜(Squared Exponential) 및 Matérn 커널에 대한 첫 상수 누적 레그렛(cumulative regret) O(1)을 도출했습니다. 두 번째로, 알고리즘과 무관한 posterior 표준 편차 관측에 대한 새로운 상한을 제시하였는데, 이는 노이즈 없는 환경에서의 GP-UCB 성능을 보장합니다.

- **Performance Highlights**: 논문의 결과는 정리된 표 1과 2에 요약되어 있으며, Matérn 커널에 대해서는 conjectured 레그렛 하한과 일치하는 점을 강조합니다. 구체적으로, GP-UCB는 d>ν인 경우 상수 O(1)의 레그렛을 달성하며, 이는 기존의 이론적 결과를 향상시키는 것입니다. 이러한 결과는 노이즈 없는 GP-bandits 알고리즘에 있어서 새로운 기초를 제공합니다.
...
### Long-term Causal Inference via Modeling Sequential Latent Confounding (https://arxiv.org/abs/2502.18994)
- **What's New**: 본 연구는 기존의 CAECB(Conditional Additive Equi-Confounding Bias) 가정을 확장하여 시간 종속적 단기 결과를 다루는 새로운 FCAECB(Functional CAECB) 가정을 제안합니다. FCAECB 가정은 순차적인 잠재 혼란 편향 간의 기능적 관계를 posits(제시)하고 이를 통해 장기 인과 효과의 식별을 확립합니다. 또한, 제안된 방법론이 기존의 연구에서 제한적으로 적용되었던 경우를 넘어 실제 데이터에 적합하게 작용할 수 있도록 발전시켰습니다.

- **Technical Details**: 이 연구에서는 장기 인과 효과 식별을 위한 새로운 FCAECB 가정을 제시하며, 이 가정은 순차적인 단기 결과 간의 잠재 혼란 편향의 관계를 수학적으로 정의합니다. 이를 통해 개발된 추정량은 비모수 회귀(nonparametric regression) 프레임워크 하에서 이론적 수렴 속성을 분석하며, 머신러닝 회귀 방법을 통해 구현할 수 있습니다. 이러한 기법은 장기 효과 추정에 대해 포괄적으로 접근할 수 있도록 제공합니다.

- **Performance Highlights**: 실험 결과는 제안된 방법의 이론적 발견을 검증하며, 장기 인과 관계를 추정하는 데 있어 FCAECB 가정이 효과적임을 보여줍니다. 이는 기존의 단기 결과에만 기반한 방법론에 비해 우수한 성능을 발휘하고, 다양한 실제 시나리오에 적용이 가능하다는 점에서 중요합니다. 이러한 시스템적 기여는 장기 causal inference 분야에서 중요한 발전을 나타냅니다.



### Invariance Pair-Guided Learning: Enhancing Robustness in Neural Networks (https://arxiv.org/abs/2502.18975)
- **What's New**: 이번 연구에서는 기계 학습 모델의 Out-of-distribution generalization 문제를 해결하기 위한 새로운 접근법을 제시합니다. 기존 방법들은 데이터 조작이나 특별한 증강 기법을 요구하는 반면, 본 연구에서는 신경망을 훈련 단계에서 유도하기 위한 기술을 개발했습니다. 제안된 Invariance Pair-Guided learning (IPG) 기법은 잘못된 속성과 일관된 속성을 분리하여 보다 견고한 모델을 학습할 수 있음을 보여줍니다.

- **Technical Details**: 제안된 IPG 방법은 입력 쌍(일관성과 불일치 성질로 구성된)으로부터 전통적인 기울기 하강법에 보정 기울기를 추가하는 형태로 구성됩니다. 이 방법은 미리 정의된 일관성 조건에 따라 보정 기울기를 적응적으로 변화시키며, 훈련 데이터에 대한 일반화 성능을 향상시킵니다. 세 가지 데이터셋(ColoredMNIST, Waterbird-100, CelebA)에서의 실험을 통해 제안된 접근법이 효과적임을 입증합니다.

- **Performance Highlights**: IPG 방법은 다양한 현실적 데이터셋에서 효과적으로 적용 가능한 것으로 나타났습니다. 특히, spurious correlation이 있는 상황에서도 모델의 출력이 일관성을 유지하는 것을 확인했습니다. 기존의 기법들과 비교했을 때, IPG는 추가적인 주석 없이도 더 높은 데이터 효율성을 제공하며, 다른 기법에 비해 높은 일반화 성능을 보여줍니다.
...
### One Set to Rule Them All: How to Obtain General Chemical Conditions via Bayesian Optimization over Curried Functions (https://arxiv.org/abs/2502.18966)
- **What's New**: 이 연구에서는 화학 반응 조건 최적화를 위한 일반성과 관련된 Bayesian 최적화(Bayesian Optimization, BO) 접근법을 제안하여 일반적인 최적 조건을 발견하는 데 도움이 되는지 평가합니다. 일반 매개변수(general parameters)에 집중하여 실험 계획에서의 고찰을 통해 시간을 절약하고 효율성을 높이는 방안을 모색합니다. 특히, 실험 데이터와 연계하여 매개변수와 작업 선택을 분리하는 간단한 최적화 전략이 복잡한 전략에 필적하는 성능을 보임을 밝혀냅니다.

- **Technical Details**: 일반성 지향 최적화 문제를 curried 함수 최적화 문제로 공식화하며, 기존의 글로벌 최적화(Global Optimization) 접근법에 대한 확장으로 간주합니다. 연구에서 Bayesian 최적화의 원리를 설명하고, 이를 통해 알려지지 않은 목적 함수에 대해 샘플 효율적으로 최소값을 찾기 위한 프로세스를 기술합니다. 또한, Gaussian Process(GP)와 Bayesian Neural Networks(BNN)를 활용하여 예측하는 방법을 설명하여 이들 기법이 어떻게 최적화의 성공을 이끄는지를 보여줍니다.

- **Performance Highlights**: 연구 결과, 여러 기초(substrate)에 걸쳐 최적화를 수행하면 더 일반적인 최적 해결책을 찾을 수 있으며, 매개변수와 작업 선택을 분리하는 것이 효율적인 탐색으로 이어진다는 것을 입증합니다. 본 연구는 기존의 반응 최적화 벤치마크들을 확장 및 적응하여 일반성 지향 BO의 유용성을 개선하였습니다. 최적화를 위한 다양한 알고리즘의 평가와 함께 'CurryBO'라는 오픈소스 솔루션을 BoTorch에 추가하여 실험실 환경에서도 활용 가능임을 강조합니다.



### Nonparametric Heterogeneous Long-term Causal Effect Estimation via Data Combination (https://arxiv.org/abs/2502.18960)
- **What's New**: 본 논문은 이질적인 장기 인과 효과(Heterogeneous Long-term Causal Effects) 추정 방법의 선진화를 다루고 있습니다. 기존 방법들은 주로 평균 장기 효과(Average Long-term Effects)를 추정하는데 중점을 둔 반면, 본 연구는 개별적 필요에 맞춘 개인화된 전략을 설계하기 위한 이질적인 효과를 고려합니다. 저자들은 CAECB(Conditional Additive Equi-Confounding Bias) 가정을 기반으로 하는 여러 개의 비모수(nonparametric) 추정기를 제안하며, 이들은 두 단계 회귀(two-stage regression) 프레임워크에서 작동합니다.

- **Technical Details**: 이 연구에서는 CAECB 가정 하에 비모수 추정기를 설계하고, 이러한 추정기의 점근적 수렴성 및 성질을 이론적으로 분석합니다. 특히, 저자들은 네 가지 다른 모델 사양의 합집합에서 일관성을 보여주는 Multiple Robust (MR) 추정기를 제안하며, 이는 기존의 더블/멀티 로버스트 방법들과 차별화됩니다. 이론 분석을 통해 다양한 추정기가 성능을 다르게 보이는 조건을 규명하며, 신경망 기반의 HLCE(beta) 추정기를 구축하여 실험적으로 성능을 검증합니다.

- **Performance Highlights**: 실제 여러 개의 합성 및 반합성 데이터셋을 통한 실험 결과는 제안된 추정기의 이론적 결과의 유효성을 입증했습니다. 또한, ML 기반의 방법이 특히 이질적인 장기 효과 추정에서 효과적임을 보여주며, 개인화를 위한 치료 계획 수립에 중요한 잠재력을 지니고 있음을 강조합니다. 이러한 결과는 장기 인과 추정 분야에서의 새로운 연구 방향성을 제시하며 보다 세밀한 정책 설계에 기여할 수 있습니다.



### Fourier Multi-Component and Multi-Layer Neural Networks: Unlocking High-Frequency Potentia (https://arxiv.org/abs/2502.18959)
Comments:
          Our code and implementation details are available at this https URL

- **What's New**: 이 논문에서는 소위 'Fourier Multi-Component and Multi-Layer Neural Network (FMMNN)'라는 새로운 신경망 구조를 제안합니다. FMMNN은 고주파 성분을 효과적으로 포착할 수 있는 놀라운 적응력과 효율성을 입증하며, 이론적으로는 근사 용량 측면에서 기하급수적인 표현 능력을 갖고 있음을 보여줍니다. 이러한 구조는 기존의 완전 연산 신경망(Fully Connected Neural Networks)과 비교할 때 최적화 경관이 상당히 유리하며, 다양한 작업에서 더 높은 정확도와 효율성을 달성함을 실험적으로 입증했습니다.

- **Technical Details**: FMMNN의 구조는 다수의 구성 요소와 다층적 구조로 구성되어 있으며, 각 구성 요소는 매개변수화된 sine 활성화 함수의 선형 조합으로 이루어집니다. 이를 통해 FMMNN은 매끄러운 함수의 효율적이고 정확한 근사를 지원하며, 다층 조합을 통해 높은 주파수 성분을 생성할 수 있습니다. 또한, 중요한 실용적 응용을 위해 FMMNN은 함수와 그 미분을 모두 근사할 수 있는 능력을 지니고 있습니다.

- **Performance Highlights**: FMMNN은 복잡한 함수 구조를 효과적으로 캡처할 수 있는 구조적 장점 덕분에 다양한 데이터 세트에서 우수한 성능을 보여주었습니다. 특히 고주파 성분이 포함된 작업에서는 더욱 인상적인 성과를 기록하였으며, 기존의 신경망 구조들과 비교했을 때 상대적으로 높은 정확도를 달성했습니다. 이러한 성능 차이는 고주파 성분과 관련된 최적화 경관의 유리함과 관련이 있으며, 향후 신경망 설계 및 구현에 있어 새로운 가능성을 열어줍니다.



### Fewer May Be Better: Enhancing Offline Reinforcement Learning with Reduced Datas (https://arxiv.org/abs/2502.18955)
- **What's New**: 이번 논문에서는 오프라인 강화 학습(offline reinforcement learning, RL)의 데이터 선택 문제에 대해 새로운 해결책인 ReDOR(Reduced Datasets for Offline RL) 방법을 제안합니다. ReDOR는 데이터세트 선택을 그래디언트 근사 최적화 문제로 설정하여 알고리즘 성능과 훈련 효율성을 동시에 향상시킵니다. 이 방법은 기존의 Actor-Critic 프레임워크를 하위 모듈 최적화 문제로 변형하여, 데이터의 하위 집합 선택을 효율적으로 수행할 수 있도록 합니다.

- **Technical Details**: ReDOR는 오프라인 RL의 데이터 선택 문제를 해결하기 위해 정교하게 조정된 직교 매칭 추구(Orthogonal Matching Pursuit, OMP)를 적용합니다. 기존의 감독 학습(supervised learning)와의 차이점으로, 오프라인 RL의 타겟 값들은 정책 업데이트에 따라 변동하며 이로 인해 불안정한 그래디언트가 발생합니다. 이를 해결하기 위해 저자는 OMP에 여러 가지 수정을 추가하여 학습 과정을 안정화하고, 이론적으로 수렴 특성(convergence properties)을 분석하여 적합한 데이터 선택 기준을 제시합니다.

- **Performance Highlights**: ReDOR는 D4RL 벤치마크 데이터셋에서 평가되었으며, 다양한 기준선 및 변별 실험과의 비교를 통해 알고리즘 성능을 크게 향상시키는 것을 확인했습니다. ReDOR로 선정된 데이터 하위 집합은 계산 비용이 적게 드는 동시에 높은 성능을 보여주며, 오프라인 강화 학습에서 데이터 축소에 관한 최초의 연구라는 점에서 의의가 있습니다.
...
- **What's New**: 이 논문에서는 LLMs(대형 언어 모델)의 미세 조정(fine-tuning)을 위한 새로운 접근법인 CLLoRA를 제안합니다. 특히 데이터 소유자 간의 데이터 프라이버시를 보장하면서 헤테로지니티 문제를 해결하는 데 중점을 둡니다. CLLoRA는 LLM의 문맥 길이(context length)가 모델 성능에 미치는 영향을 효율적으로 측정할 수 있는 방법론을 제공합니다.

- **Technical Details**: CLLoRA는 파라미터 효율적인 미세 조정 방식인 LoRA를 활용하여 다양한 크기의 LLMs에 대해 실행됩니다. 본 연구는 데이터를 문맥 길이에 따라 분류하고 Dirichlet 분포를 사용하여 서로 다른 클라이언트에 맞춤형 데이터 세트를 생성하는 방식을 채택합니다. 이 논문은 비독립적이고 동일하게 분포되지 않는(non-IID) 데이터의 영향을 분석하여 클라이언트와 글로벌 모델 모두에 대한 성능을 평가합니다.

- **Performance Highlights**: 연구 결과, 문맥의 질과 양의 불균형은 클라이언트에서의 로컬 학습과 글로벌 모델의 성능 모두에 영향을 미친다는 것을 발견했습니다. 반면에 문맥 길이는 로컬 학습에는 미미한 영향을 미치지만 글로벌 모델에는 더 큰 영향을 미치는 것으로 나타났습니다. 이는 LLM 미세 조정의 성능에 있어 문맥의 질과 길이의 중요성을 부각시킵니다.



### A Pipeline of Augmentation and Sequence Embedding for Classification of Imbalanced Network Traffic (https://arxiv.org/abs/2502.18909)
Comments:
          10 pages, 4 figures. arXiv admin note: text overlap with arXiv:1901.00204

- **What's New**: 이번 연구에서는 네트워크 트래픽 분류(Network Traffic Classification, NTC)의 데이터 불균형 문제를 해결하기 위한 새로운 파이프라인을 제안했습니다. 특히, 인공 데이터 생성을 위해 LSTM(Long Short-Term Memory) 네트워크와 커널 밀도 추정(Kernel Density Estimation, KDE)을 이용하며, 카테고리형 특성에 대한 새로운 임베딩 프레임워크인 FS-Embedding을 도입합니다. 이 프레임워크는 패킷의 소스 및 목적지 포트와 방향을 하나의 단어처럼 취급하여 새로운 특성 공간을 학습합니다.

- **Technical Details**: 제안된 방법은 LSTM을 활용하여 시스템의 데이터 불균형 문제를 해결하며, 특히 패킷 흐름의 의미론적 관계를 포착하는 데 권장되는 FS-Embedding을 사용합니다. FS-Embedding은 패킷의 소스 및 목적지 포트와 데이터 흐름의 방향을 기반으로 하여 임베딩 벡터 공간을 형성합니다. 이번 연구에서는 Convolutional Recurrent Neural Network(CRNN) 및 Transformer와 비교하는 실험을 수행하여 제안된 파이프라인의 효과를 평가합니다.

- **Performance Highlights**: 우리는 제안된 증강 파이프라인과 FS-Embedding의 결합이 모델 수렴 속도를 증가시키고, 모델 파라미터의 수를 획기적으로 줄이면서도 정확도 측면에서 일관된 성능을 유지함을 보여줍니다. 이는 기존의 방법들보다 향상된 성능을 제공하며, 데이터 불균형 문제 해결에 중요한 기여를 합니다.



### VEM: Environment-Free Exploration for Training GUI Agent with Value Environment Mod (https://arxiv.org/abs/2502.18906)
Comments:
          20pages,5 figures

- **What's New**: 이번 연구는 비디오 및 언어 모델(Visualization-Language Models, VLMs)을 강화 학습(Reinforcement Learning, RL)을 통해 그래픽 사용자 인터페이스(Graphical User Interfaces, GUI) 에이전트로 훈련하는 과정에서 발생하는 주요 문제들을 해결하고자 합니다. 환경 기반 RL은 비용이 많이 드는 상호작용을 요구하는 반면, 환경 없는 방법들은 분포 변화(distribution shift)와 보상 일반화(reward generalization)에서 어려움을 겪습니다. 이 연구는 사전 훈련된 가치 환경 모델(Value Environment Model, VEM)을 활용하여 가치 추정(value estimation)과 정책 최적화(policy optimization)를 분리하는 환경 없는 RL 프레임워크를 제안합니다.

- **Technical Details**: VEM은 오프라인 데이터에서 직접 상태-행동 값(state-action values)을 예측하여, 다음 상태 예측이나 환경 피드백 없이 GUI 상호작용 결과에 대한 사람과 유사한 선험적(prior) 지식을 증류합니다. 이를 통해 오류 누적을 피하고 UI 변화에 대한 강인성을 증대시킵니다. 제안된 프레임워크는 두 단계로 운영됩니다: (1) VEM을 사전 훈련하여 장기 행동 유용성을 추정하고, (2) 동결된 VEM 신호를 이용해 정책 탐색(policy exploration)을 안내합니다.

- **Performance Highlights**: Android-in-the-Wild 벤치마크에서 평가된 결과, VEM은 오프라인 및 온라인 설정 모두에서 최첨단(state-of-the-art) 성능을 달성하였습니다. 특히, 환경 없는 기초 방법에 비해 상당한 성능 향상을 보였으며, 상호작용 비용 없이 환경 기반 접근 방식과 유사한 성능을 기록했습니다. 중요한 점은 VEM이 의미 인식(value estimation) 방법을 통해 온라인 훈련된 방법들과 비교 가능한 성능을 달성할 수 있음을 보여주었습니다.
...
- **What's New**: TabGLM(탭글램)은 기계 학습이 전통적으로 어려움을 겪던 이질적인(tabular) 데이터셋을 효과적으로 처리하기 위해 설계된 새로운 다중 모드 구조입니다. 각 테이블의 행(row)을 완전 연결 그래프(fully connected graph) 및 직렬화된 텍스트(serialized text)로 변환하여, 그래프 신경망(Graph Neural Network)와 텍스트 인코더를 통해 인코딩합니다. 이를 통해 TabGLM은 구조적(structural) 및 의미적(semantic) 정보를 동시에 캡처할 수 있습니다.

- **Technical Details**: TabGLM은 다중 모달 학습 프레임워크를 사용하여 이질적인 데이터의 양쪽 모두에서 상호 보완적인 정보를 활용하고, MuCosa라 불리는 공동 손실 함수(joint loss)를 통해 그래프 및 텍스트 인코더로부터 학습한 표현을 정렬합니다. 이 접근법은 과적합(overfitting)을 방지하는 정규화(regularization) 기능도 하며, 이는 성능 향상에 기여합니다.

- **Performance Highlights**: TabGLM의 효율적인 그래프-텍스트 파이프라인은 기존 딥러닝 접근 방식보다 더 적은 매개변수(parameter)로 이질적인 데이터셋을 처리할 수 있습니다. 25개의 벤치마크 데이터셋에서 AUC-ROC 점수의 평균 5.56% 개선을 기록하여 최신 기술(State-of-the-Art) 모델들과 비교하여 현저한 성과를 나타냈습니다.



### Adversarial Combinatorial Semi-bandits with Graph Feedback (https://arxiv.org/abs/2502.18826)
- **What's New**: 이 논문은 조합 반밴드(combinatorial semi-bandits) 프레임워크를 확장하여 그래프 피드백(graph feedback)을 포함합니다. 학습자는 선택된 팔(arms)의 이웃 팔다 이 보상(rewards)을 관찰하게 됩니다. 최적의 후회(regret) 모델이 시간 지평선 T에 대해 $	ilde{	heta}(S	ext{sqrt}(T) + 	ext{sqrt}(	ext{α}ST))$로 확장되어, 이는 완전 정보(full information) 및 반밴드 피드백(semi-bandit feedback) 상황에서의 잘 알려진 후회 값을 연결합니다.

- **Technical Details**: 조합 반밴드 문제는 K 개의 개별 팔(arms)로부터 선택된 조합을 기반으로 작동하며, 각 시간 (time) t에서 학습자는 제한된 예산 S 내에서 조합을 선택합니다. 학습자는 결정 후 각 팔의 보상을 관찰하며, 적대적인(adversarial) 환경의 경우 보상 벡터는 사전에 통계적 가정 없이 생성됩니다. 이 논문에서는 방향 그래프 G를 통해 추가적인 정보를 활용하는 구조를 제시하며, 이 구조는 학습자가 인접 팔의 보상을 관찰할 수 있게 합니다.

- **Performance Highlights**: 이 연구에서는 조합 반밴드 문제에서의 훈련 알고리즘의 성능을 알기 위한 목적을 가지고 있으며, 특히 새로운 그래프 피드백을 효과적으로 활용하는 것을 목표로 합니다. 그래프 G가 완전하면 완전 정보의 피드백과 동일한 상황이 되며, 반대로 자가 루프만 있을 경우 반밴드 피드백 상황으로 축소됩니다. 이러한 복잡한 탐색-활용 탐지(exploration-exploitation trade-off)를 이해하고 최적의 후회를 최소화하는 방향으로 나아가고자 합니다.



### CAMEx: Curvature-aware Merging of Experts (https://arxiv.org/abs/2502.18821)
Comments:
          ICLR 2025 Poster

- **What's New**: 이번 논문은 CAMEx(Curvature-Aware Merging of Experts)라는 새로운 전문가 병합 프로토콜을 소개합니다. 기존의 방법들은 주로 Euclidean geometry에 의존하여 복잡한 매개변수 공간의 곡률을 고려하지 못했습니다. CAMEx는 자연 기울기를 사용하여 비유클리드 곡률을 반영함으로써 모델 업데이트와 매개변수 공간 간의 정렬을 개선합니다. 이를 통해 일반화 능력이 향상되어 성능을 높이고, 메모리 오버헤드를 줄이는 데 기여합니다.

- **Technical Details**: 문제 해결을 위해 CAMEx는 전문가 매개변수의 곡률 정보를 포함하는 새로운 병합 기법을 사용합니다. 이 방법은 SMoE(Sparse Mixture of Experts) 아키텍처 내에서 적용되며, 자연 기울기를 통해 전문가들 간의 병합 프로세스를 최적화합니다. 또한, 도메인 특화 병합 기법을 통해 각 모델이 특정 작업에 대해 적합하게 학습하도록 지원합니다. 이론적으로, CAMEx는 전문가와 작업 도메인 간의 더 나은 정렬을 달성한다고 주장합니다.

- **Performance Highlights**: CAMEx는 전통적인 Euclidean 기반 병합 기술에 비해 다양한 자연어 처리 작업에서 뛰어난 성능을 보여줍니다. 실험 결과, 본 방법은 사전 훈련(pre-training) 및 미세 조정(fine-tuning)에서 빠른 수렴 속도를 보이며, 언어 모델링, 텍스트 분류, 질문 응답 및 이미지 분류 등 다양한 실제 작업에서 성과를 향상시킵니다. 이러한 접근 방식은 대규모 언어 모델의 효율적인 확장을 가능하게 합니다.



### Optimal Stochastic Trace Estimation in Generative Modeling (https://arxiv.org/abs/2502.18808)
Comments:
          Accepted by AISTATS 2025

- **What's New**: 본 논문에서는 Hutch++라는 새로운 최적 확률 전이 추정기를 제안합니다. 이 방식은 확산 모델의 훈련 변동성을 줄이면서도 최적 운송(optimal transport)의 특성을 유지하기 위해 고안되었습니다. 높은 차원의 데이터가 낮은 차원의 구조를 보일 때 발생하는 문제를 효과적으로 처리할 수 있습니다. 특히 비용이 많이 드는 QR 분해의 빈도를 줄이기 위한 실제적인 스킴을 제안합니다.

- **Technical Details**: Hutch++는 생성 모델을 위한 최적 확률 전이 추정기로, 기존의 Hutchinson estimator 대비 훈련 변동성을 최소화합니다. 본 연구에서는 큰 고유값을 결정론적으로 계산하고, 나머지는 Hutchinson 추정기를 사용하여 근사함으로써 변동성을 줄이는 방법론을 소개합니다. 또한, 우리의 방식은 신경 ODE(neural ODEs), 슬라이스 된 스코어 매칭(sliced score matching) 등 다양한 생성 모델에 적용될 수 있는 가능성을 보여줍니다.

- **Performance Highlights**: Hutch++ 방법을 통해 생성된 데이터는 높은 품질을 자랑하며, 다양한 응용에서 효과적인 변동성 감소를 수반합니다. 시뮬레이션, 조건부 시계열 예측 및 이미지 생성과 같은 다양한 분야에서 그 효과가 입증되었습니다. 이 연구는 확산 모델 및 생성 모델의 성능을 개선하는 데 중요한 기여를 하고 있습니다.
...
### Exploring Graph Tasks with Pure LLMs: A Comprehensive Benchmark and Investigation (https://arxiv.org/abs/2502.18771)
- **What's New**: 본 논문에서는 그래프 태스크에 적용할 수 있는 대규모 언어 모델(LLM)의 가능성을 자세히 탐구하였습니다. 기존의 그래프 신경망(GNN) 모델들이 가진 한계를 극복하기 위해, LLM이 제공하는 데이터 부족 상황이나 다양한 작업 간 전이 가능성을 평가했습니다. 특히, LLM 중 지침 조정(instruction tuning)이 포함된 모델들이 몇몇 샷(few-shot) 환경에서도 흔히 사용되는 전통적인 모델들보다 뛰어난 성능을 보였습니다.

- **Technical Details**: 이 연구에서는 다양한 그래프 학습 모델들과 LLM을 비교하고, 전통적인 GNNs, 자기 감독 그래프 학습, 그래프 변환기(GT) 모델을 포함하여 16개의 다양한 모델을 분석하였습니다. 실험에서는 Cora, PubMed, ArXiv 및 Products와 같은 데이터셋을 사용하여 각 모델의 성능을 시험하였고, LLM의 지침 조정이 성능에 미치는 영향을 특별히 조사하였습니다. 또한, 그래프 구조를 이해하기 위해 LLM의 입력 형식인 프롬프트(prompt) 설계를 통해 노드 분류 및 링크 예측 작업을 수행했습니다.

- **Performance Highlights**: 결과적으로, 지침 조정된 LLM들은 기존의 그래프 학습 기술들보다 뛰어난 일반화 능력을 보여주었으며, 특히 작은 모델조차 전통적인 모델의 성능을 초과하는 경우도 있었습니다. LLM은 몇몇 샷 환경에서 강력한 예측 능력을 유지하며, 그래프의 구조적 정보를 효과적으로 활용하는지 평가되었습니다. 이러한 결과는 LLM의 그래프 학습 적용 가능성을 보여주며, 실질적인 데이터 부족 환경에서도 그 효과성을 입증하였습니다.
...
### Bandit and Delayed Feedback in Online Structured Prediction (https://arxiv.org/abs/2502.18709)
Comments:
          33 pages

- **What's New**: 이번 연구에서는 온라인 구조 예측(online structured prediction)에서 정보 피드백이 제한된 상황에서도 효과적으로 작동하는 알고리즘을 제안합니다. 특히, bandit와 지연된 피드백(이전 관측을 기반으로 한 예측) 환경에서 지속적으로 낮은 surrogate regret을 달성할 수 있는 방법을 모색하고 있습니다. 이는 종종 복잡한 구조의 출력물을 다룰 때 발생하는 도전과제에서 비롯된 것입니다.

- **Technical Details**: 제안된 알고리즘은 bandit 환경을 위한 일반적인 역가중 경량 추정기(inverse-weighted gradient estimator)를 사용하여, T 시간 지평선과 K 출력 집합 크기에 대해 O(√KT) 의 surrogate regret 경계를 보여줍니다. 하지만, 출력이 복잡한 경우 K가 극도로 커질 수 있어 이 결과는 바람직하지 않을 수 있습니다. 따라서 K에 독립적인 O(T^{2/3})의 surrogate regret 경계를 달성하는 알고리즘도 제안되었습니다.

- **Performance Highlights**: 제안된 알고리즘은 온라인 분류에서 bandit 피드백의 성능을 수치적으로 평가하였습니다. 지연된 완전 정보 피드백 상황에서는 O(D^{2/3} T^{1/3})의 surrogate regret 경계를 제공하며, 지연된 bandit 피드백 상황에 대한 알고리즘도 논의됩니다. 이러한 평가 결과는 향후 연구와 실제 적용 가능성에 긍정적인 시사점을 제공합니다.



### Differentially Private Federated Learning With Time-Adaptive Privacy Spending (https://arxiv.org/abs/2502.18706)
Comments:
          International Conference on Learning Representations (ICLR), April 2025, Singapore

- **What's New**: 이번 연구에서는 개인 정보 보호(DP)가 포함된 연합 학습(FL) 프레임워크를 제안합니다. 이 프레임워크는 클라이언트가 엄격한 개인 정보 보호 기준을 준수하면서도 공유 모델을 훈련할 수 있게 합니다. 특히, 각 클라이언트가 초기 라운드에서 다양한 양의 노이즈를 추가해 개인 정보 보호 예산을 비균일하게 사용할 수 있음을 보입니다.

- **Technical Details**: 클라이언트들은 초기 라운드에서 개인 정보 보호 예산을 절약하고 나중에 더 미세한 특징(특징)을 학습할 때 더 많은 예산을 쓸 수 있도록 설계되었습니다. 이를 통해 고객이 개인 정보 보호 예산을 고르게 사용하기보다는 비균일하게 사용하는 경우 유용성이 향상됨을 이론적으로 증명합니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘이 기존의 기준 방식에 비해 개인 정보 보호-유용성(utility) 균형 개선을 입증했습니다. 표준 벤치마크 데이터 세트에서 진행된 이 실험들은 제안된 방법의 효과성을 증명하며, 클라이언트마다 다른 개인 정보 보호 예산을 사용할 때 유용성의 향상을 보여줍니다.



### Tukey Depth Mechanisms for Practical Private Mean Estimation (https://arxiv.org/abs/2502.18698)
Comments:
          17 pages, 10 figures

- **What's New**: 이 연구에서는 다변량 가우시안 분포에 대한 이론적으로 최적의 평균 추정기를 구현한 (Restricted) Tukey Depth Mechanism을 소개합니다. 이를 통해 작은 샘플 크기나 저차원 데이터에서도 사적(mean estimation) 평균 추정을 위한 효율적인 방법을 제공합니다. 새로운 알고리즘의 개발로 이론적 발전과 실제 적용 간의 격차를 줄이는 데 기여하게 됩니다.

- **Technical Details**: Tukey 깊이 메커니즘은 다변량 데이터에 대해 강력한 이론적 보장을 제공하며, 이는 다변량 데이터의 평균 추정에 유리합니다. 이 논문에서는 Tukey 깊이의 근사 버전을 사용하는 다양한 메커니즘을 구현하여 정확도를 포기하고 계산 속도를 높이는 방법을 제안합니다. 이러한 방법은 다차원 데이터의 평균 추정에서 높은 정확도와 견고성(robustness) 보장을 제공합니다.

- **Performance Highlights**: 연구 결과, 저차원 데이터에서 이러한 알고리즘의 효율성이 입증되었으며, 중간 차원에서도 실행 가능성이 높은 옵션으로 평가됩니다. 앞으로 이 알고리즘의 계산 효율성을 개선하기 위한 방향으로 빠른 다면체 볼륨 근사기법(polytope volume approximation techniques)을 활용할 방안을 모색하고 있습니다.
...
- **What's New**: 이번 논문은 카일리 그래프(Cayley graph)의 경로 탐색을 위한 인공지능 기반 접근 방식을 개발하기 위한 연구 시리즈의 두 번째 논문으로, 대규모 그래프(예: $10^{70}$ 노드)의 효과적인 경로 탐색에 중점을 두고 있습니다. 본 논문은 강화 학습(reinforcement learning) 접근 방식과 이전 논문에서의 확산 거리(diffusion distance) 접근 방식을 결합한 새로운 방법론을 제안하며, CayleyPy 프로젝트의 핵심 구성 요소로 기능합니다. 또한, 이 연구는 고전 컴퓨터 대수 시스템인 GAP와의 비교를 통해 "GAP를 뛰어넘는" 성과를 보여줍니다.

- **Technical Details**: 논문에서는 다양한 신경망 아키텍처(architectures)와 무작위 탐색(random walk) 생성기 및 빔 탐색(beam search) 경로 탐색 알고리즘에 대한 벤치마킹을 포함한 상세한 분석을 수행합니다. 우리는 특히 LRX 생성기를 통해 $n$이 약 30인 경우 경로를 찾을 수 있으며, GAP는 $n$이 20에 가까운 경우까지만 작동하는 것을 보여줍니다. 최적화된 PyTorch 구현을 통해 대량의 순열(permutations) 처리를 위한 기술적 솔루션을 제공하며, 단일 코드 수정으로 경로 탐색 성능을 극적으로 개선했습니다.

- **Performance Highlights**: 우리는 특정 LRX 카일리 그래프에서 최장 요소(longest element)를 확인하고, 해당 요소의 분해(decomposition)가 n(n-1)/2보다 짧지 않다는 것을 확인했습니다. 또한, 다양한 [Kaggle](https://www.kaggle.com) 플랫폼에서 도전 과제를 조직하여 커뮤니티의 참여를 유도하고, 최적의 해를 찾기 위한 협업을 촉진했습니다. 이 과정에서 우리는 규명된 하한과 상한을 증명하고, 경로 탐색 문제에 대한 깊은 이해를 제공하며, 산출된 결과는 AI와 수학적 결과로서 중요한 기여를 합니다.



### Provably Efficient RL for Linear MDPs under Instantaneous Safety Constraints in Non-Convex Feature Spaces (https://arxiv.org/abs/2502.18655)
- **What's New**: 이 논문에서는 강화 학습 (Reinforcement Learning, RL)에서 비선형 제약 문제를 해결하기 위한 새로운 방법론을 제안합니다. 특히, 비별 주사형(star-convex)과 비선형(non-star-convex) 상황에서의 비중손실(regret bound)을 제시합니다. 필수로 요구되는 안전 제약 조건을 준수하면서 에이전트가 안정적으로 탐색하고 활용할 수 있는 전략을 개발하였습니다.

- **Technical Details**: 이 논문에서 제안한 핵심 기술은 Objective Constraint-Decomposition (OCD)입니다. OCD는 주어진 상태에서의 안전한 행동 집합을 안정적으로 갱신하며, Q-function을 통해 가치 함수(value function)의 커버링 번호(covering number)를 성공적으로 제한합니다. 또한, 비선형 환경을 위해 Non-Convex Safe Least Squares Value Iteration (NCS-LSVI)이라는 두 단계 알고리즘을 개발하여 안전 집합을 먼저 추정한 후 탐색과 활용을 균형 있게 수행합니다.

- **Performance Highlights**: 수치 시뮬레이션을 통해 제안한 NCS-LSVI 알고리즘의 효과를 확인하였으며, 자율주행 시나리오에서 비선형 안전 집합이 존재하는 상황에서도 이론적 상한선에 부합하는 초선형 비중손실(sublinear regret)을 달성했습니다. 이는 비별 주사형 및 비선형 설정에서 함께 적용 가능한 최초의 결과로, 안전한 RL의 실용적인 가능성을 높여줍니다.
...
### On the Privacy-Preserving Properties of Spiking Neural Networks with Unique Surrogate Gradients and Quantization Levels (https://arxiv.org/abs/2502.18623)
- **What's New**: 이 연구는 머신 러닝 모델이 민감한 데이터를 처리할 때 프라이버시 공격에 대한 취약성을 이해하는 것이 얼마나 중요한지를 강조합니다. 특히, Membership Inference Attacks (MIA)는 특정 데이터 포인트가 모델 훈련에 사용되었는지를 유추하는 데 사용됩니다. Spiking Neural Networks (SNNs)는 인공 신경망(ANNs)보다 MIA에 대한 저항력이 더 강하고, 이는 비확률적인 활성화와 본래의 확률론적 특성 덕분입니다. 이 연구에서는 SNNs의 프라이버시를 향상시키기 위한 두 가지 기술, 즉 양자화(quantization)와 대체 그래디언트(surrogate gradients)를 탐구합니다.

- **Technical Details**: 양자화 과정은 정보 누출을 제한하기 위해 모델의 정밀도를 줄이는 방법으로, ANNs에서 프라이버시를 향상시키는 데 성공적으로 사용되었습니다. SNNs는 이벤트 중심의 계산을 수행하기 때문에, 양자화를 통해 스파이크 패턴을 더 혼잡하게 만들어 MIA 공격을 방해할 수 있습니다. 대체 그래디언트는 비미분적 스파이크 사건의 그래디언트를 근사하여 역전파 기반 학습을 가능하게 하며, 다양한 대체 그래디언트 함수가 SNN의 프라이버시와 정확도에 미치는 영향을 평가합니다.

- **Performance Highlights**: 연구 결과, 양자화는 두 아키텍처 모두에서 프라이버시를 향상시키지만, SNNs는 여전히 더 높은 프라이버시를 유지합니다. 특히, 스파이크 비율 탈출(Spike Rate Escape) 방식은 높은 정확도를 유지하면서도 우수한 프라이버시 보호 성능을 보여주었습니다. 반면, 아크탄젠트(arctangent) 및 일률 진행 추정기(Straight Through Estimator)는 MIA에 대한 취약성을 증가시키는 경향이 있었습니다. 이러한 결과들은 SNNs의 본질적인 프라이버시 이점을 강화하며, 양자화 및 대체 그래디언트 선택이 SNNs에서 프라이버시-정확도 간의 균형에 미치는 영향을 보여줍니다.



### Tighten The Lasso: A Convex Hull Volume-based Anomaly Detection Method (https://arxiv.org/abs/2502.18601)
- **What's New**: 본 연구에서는 데이터 품질을 보장하기 위한 필수 작업인 OOD(out-of-distribution) 데이터 감지에 대해 다룹니다. 데이터셋의 볼륨이 증가하는 특성을 이용하여 새로운 이상치 탐지 알고리즘을 제안했습니다. 이 알고리즘은 데이터 포인트가 제거됨에 따라 CH(convex hull)의 볼륨을 계산하여 OOD 데이터와 인디스리뷰션(in-distribution) 데이터 간의 결정 경계를 정의합니다.

- **Technical Details**: 제안된 알고리즘은 데이터셋에서 데이터 포인트를 제거하면서 CH의 볼륨이 증가하는 성질을 이용합니다. 이를 통해 OOD 데이터와 인디스리뷰션 데이터 간의 결정 경계를 정의하는 데 필요한 결정을 내릴 수 있습니다. 연구에서는 일곱 가지 널리 사용되는 이상치 탐지 알고리즘과 비교하여 성능을 평가했습니다.

- **Performance Highlights**: 제안된 알고리즘은 10개의 데이터셋에서 성능을 비교한 결과, SOTA(state-of-the-art) 알고리즘과 유사한 결과를 나타냈습니다. 또한, 계산 비용이 낮고 간단한 방법으로 제안된 알고리즘에 적합한 데이터셋을 검출할 수 있는 가능성을 제시했습니다. 이는 SOTA 이상치 탐지 알고리즘보다 뛰어난 성능을 보여줍니다.



### Error-related Potential driven Reinforcement Learning for adaptive Brain-Computer Interfaces (https://arxiv.org/abs/2502.18594)
- **What's New**: 이 연구는 강화 학습(reinforcement learning; RL)을 활용한 적응형 오류 관련 잠재력(error-related potentials; ErrPs) 기반 BCI 접근 방식을 새롭게 제안합니다. 기존 비침습적인 뇌-컴퓨터 인터페이스가 EEG 데이터의 비정상성(non-stationarities)으로 인해 성능 저하를 겪는 문제에 대한 해결책을 모색하고 있습니다. ErrPs는 사용자의 실수에 대한 신경 반응으로, BCI의 성능을 향상시키기 위해 통합되고 활용됩니다. 이 연구의 결과는 RL 기반 프레임워크가 사용자 상호작용에서 학습하여 강력한 성능을 달성할 수 있음을 보여 줍니다.

- **Technical Details**: 제안된 시스템은 사용자의 뇌신호에서 직접적으로 의도를 학습하고 사용자의 상호작용 중에 발생하는 ErrPs의 신경 신호를 활용합니다. 이 연구에서는 공개된 운동 상상(motor imagery) 데이터셋과 사용자 참여를 증진시키기 위해 설계된 고속 게임을 사용하여 프레임워크의 타당성을 검증합니다. 또한, 이 시스템은 사용자 정신 상태의 변화, 환경적 요인 및 입력 데이터 품질에 따라 동적으로 조정되도록 설계되었습니다. 연구가 강조하는 것은 기존 BCI 기술에서의 비정상성을 다루기 위한 적응형 접근이 필요하다는 것입니다.

- **Performance Highlights**: 결과는 RL 에이전트가 사용자와의 상호작용에서 제어 정책을 성공적으로 학습하여 데이터셋 전반에 걸쳐 강력한 성능을 달성했음을 보여 줍니다. 그러나 고속 상호작용 패러다임에서의 운동 상상 방식은 대부분의 참가자에게 비효과적이라는 중요한 발견이 있었습니다. 따라서 실시간 BCI 애플리케이션에서의 작업 설계 한계를 지적하는 결과를 제공합니다. 이러한 발견은 RL의 적응형 BCI에 대한 잠재력을 강조하며, 작업의 복잡성과 사용자 반응성에 관련된 실용적인 제약을 지적합니다.



### Transported Memory Networks accelerating Computational Fluid Dynamics (https://arxiv.org/abs/2502.18591)
- **What's New**: 최근 연구에서는 신경망을 사용한 미분 가능한 PDE 솔버의 증강이 유망한 결과를 보여주었습니다. 특히, 전통적인 전산 유체 역학(CFD)에서는 CNN(Convolutional Neural Networks)에 의존하는 경향이 있지만, 이것이 산업 수준의 비구조 메쉬 문제를 해결하는 데 한계를 지니고 있음을 지적합니다. 본 연구에서는 이러한 한계를 해결하기 위해 "Transported Memory Networks"라는 새로운 아키텍처를 제안합니다.

- **Technical Details**: 이 아키텍처는 전통적인 난류 모델과 순환 신경망(Recurrent Neural Networks)에서 영감을 받아, 기본적인 격자 구조에 구애받지 않습니다. 예를 들어, 이 아키텍처는 직접 이웃(cell neighbors) 정보만을 사용하므로, 대규모 산업 CFD 코드와의 호환성을 높입니다. 또한, 롱쇼트텀메모리(Long Short Term Memory) 개념을 활용하여 흐름에 따라 효과적으로 전송되는 장기 기억(long-term memory)을 구현합니다.

- **Performance Highlights**: 결과적으로, 본 연구에서 제안한 Transported Memory Network는 기존 방법들과 비교하여 정확도와 계산 효율성 모두에서 동등하거나 개선된 성능을 보입니다. 이는 복잡한 기하학적 형태에서 CNN보다 더 나은 일반화 성능을 기대할 수 있음을 의미합니다. 본 연구에서는 새로운 접근 방식을 통해 산업적 응용이 가능한 수준의 남은 문제를 해결하고자 합니다.
...
### Target Defense with Multiple Defenders and an Agile Attacker via Residual Policy Learning (https://arxiv.org/abs/2502.18549)
- **What's New**: 이번 논문에서는 목표 방어 문제(target defense problem)에서 더 민첩한 공격자를 방어하기 위한 새로운 정책을 제안합니다. 제안된 방법은 Residual Policy Framework를 통해 Deep Reinforcement Learning (DRL)과 Boids 모델을 통합하여 방어자들의 행동을 최적화합니다. 이 연구는 시뮬레이션을 통해 전통적인 방어 전략보다 더 나은 성능을 보여주며, 강력한 확장성과 적응성을 입증합니다.

- **Technical Details**: 논문에서는 각 로봇의 운동학을 유니사이클(unicycle) 모델로 설정하고, 공격자와 방어자 각각의 속도 및 각속도를 정량화합니다. 문제는 여러 균질한 방어자들이 더 민첩한 공격자를 상대로 목표 지역을 방어하는 시나리오로 정의됩니다. 공격자가 방어자를 효과적으로 피할 수 있는 능력을 요구하므로, 팀워크와 협력이 필수적입니다.

- **Performance Highlights**: 제안된 방법은 기존의 전통적인 DRL 및 Force-based 방법보다 우수한 성능을 보였습니다. 다양한 공격자의 민첩성 수준과 방어자의 수를 다룰 수 있는 일반화된 성능을 보여주었습니다. 또한, 훈련 과정 중 약간 더 민첩한 공격자를 시작으로 점진적으로 난이도를 증가시킴으로써 학습의 효과성을 극대화했습니다.
...
### Modelling Chemical Reaction Networks using Neural Ordinary Differential Equations (https://arxiv.org/abs/2502.19397)
- **What's New**: 이 논문에서는 화학 반응 네트워크 이론에서 전통적인 미분 방정식(ODE) 대신 신경망과 결합된 신경 미분 방정식(Neural ODEs, nODEs)을 사용하여 화학 물질 농도의 동적 변화를 모델링합니다. 이를 통해 기존의 경험적 모델이 갖고 있는 한계를 극복하고, 미래의 반응 네트워크 설계에 도움을 줄 수 있는 통찰력을 제공합니다. 이러한 접근법은 실험적 데이터의 노이즈나 불완전성을 고려하여 예측의 품질을 향상시키는 것이 목표입니다.

- **Technical Details**: 전통적으로 ODE는 화학 반응 네트워크(Chemical Reaction Network, CRN)와 질량 작용 법칙을 결합하여 정의됩니다. 그러나 nODE는 데이터 기반의 신경망이 통합되어 이론적인 모델과 실험 데이터를 비교하고, 숨겨진 상호작용을 포착함으로써 동적 시스템의 예측 성능을 향상시킵니다. nODE는 훈련 과정에서 형태를 갖추어 데이터를 더 잘 설명하기 위한 파라미터화된 미분 방정식을 사용합니다.

- **Performance Highlights**: 실험적으로 nODE를 사용하여 화학 진동 시스템의 상태 공간을 예측해보았습니다. 그 결과, nODE는 실험적 설정에서의 진동 농도를 예측하는 데 있어 기존 ODE 모델보다 더 높은 정확도를 보였습니다. 이러한 성과는 nODE의 네트워크 기여를 통해 이론적 모델이 설명하지 못한 동적 행동을 발견하고 검증할 수 있음을 나타냅니다.



### Fast and Accurate Antibody Sequence Design via Structure Retrieva (https://arxiv.org/abs/2502.19395)
- **What's New**: 최근 단백질 설계 분야에서 확산 모델(difussion models)을 활용하여 구조적 스캐폴드를 생성한 후, 이 구조를 기반으로 서열 역폴딩(protein inverse folding) 과정을 진행하는 방법론이 선보였습니다. 하지만 이러한 방법들은 항체의 상보성 결정 영역(CDRs)와 같은 고변동 구조에 적용할 때 상당한 문제에 직면하게 됩니다. 이에 따라 새롭게 제안된 Igseek는 자연 항체 데이터베이스로부터 유사한 구조를 검색하여 CDR 서열을 추론하는 혁신적인 구조 검색 프레임워크입니다.

- **Technical Details**: Igseek는 다채널 동형 그래프 신경망(multi-channel equivariant graph neural network)을 활용하여 CDR 백본(structure)의 고품질 기하학적 표현을 생성합니다. 이 프레임워크는 구조적으로 유사한 CDR의 서열을 정렬하고, 구조적으로 보존된 서열 모티프를 활용하여 추론 정확도를 향상시키는 방식으로 작동합니다. 본 연구는 Igseek이 구조 검색에서의 효율성 뿐만 아니라 항체와 T-세포 수용체의 서열 복구를 위한 최첨단 접근 방식을 초과하는 성과를 올렸음을 실험적으로 입증하였습니다.

- **Performance Highlights**: 실험 결과, IgSeek은 구조에서 서열로의 생성 과정에서 매우 효과적인 성능을 보였습니다. 구조 검색 모델인 FoldSeek와 비교했을 때, IgSeek의 유사 구조 검색 품질이 더 뛰어난 것으로 나타났습니다. 이 연구는 단백질 치료 설계 분야에서 새로운 검색 기반의 관점을 제시하며, 비록 시퀀스 회복률이 증가해도 기존의 AI 기반 단백질 설계 방식이 가진 약점을 극복할 길을 제시합니다.



### Towards More Accurate Full-Atom Antibody Co-Design (https://arxiv.org/abs/2502.19391)
- **What's New**: 이번 연구에서는 Igformer라는 새로운 end-to-end 프레임워크를 소개합니다. 이 프레임워크는 항체-항원 결합 인터페이스의 혁신적인 모델링을 통해 기존의 방법들이 가지고 있는 한계를 극복하고 있습니다. Igformer는 개인화된 전파(Personalized Propagation)와 글로벌 주의 메커니즘(Global Attention Mechanisms)을 통합하여 복잡한 상호작용을 정교하게 캡처할 수 있도록 합니다.

- **Technical Details**: Igformer는 상호 그래프 표현(Inter-Graph Representation)을 개선하여 여러 스케일의 잔기 상호작용(Multi-scale Residue Interactions)을 명시적으로 모델링합니다. 이러한 처리를 통해 로컬 화학 상호작용(Local Chemical Interactions)과 글로벌 형태 의존성(Global Conformational Dependencies) 간의 상호작용을 종합적으로 이해할 수 있습니다. 이는 특히 항체-항원 인식(Antibody-Antigen Recognition)과 결합 특이성(Binding Specificity)에 중요한 역할을 합니다.

- **Performance Highlights**: Igformer는 에피톱-결합 CDR 디자인(Epitope-Binding CDR Design)과 구조 예측(Structure Prediction) 작업에서 기존 방법들보다 상당한 개선을 보입니다. 이 프레임워크는 치료 응용을 위한 계산적 항체 디자인을 크게 발전시킬 가능성을 제시하고 있습니다. 광범위한 검증을 통해, Igformer는 효율적인 항체-항원 결합을 위한 원동력이 되는 기능을 입증했습니다.
...
### Enhancing Gradient-based Discrete Sampling via Parallel Tempering (https://arxiv.org/abs/2502.19240)
Comments:
          24 pages, 5 figures. arXiv admin note: text overlap with arXiv:2402.17699 by other authors

- **What's New**: 본 논문에서는 복잡한 이산 분포에서의 샘플링 효율성을 향상시키기 위해 평행 온도 조절(Parallel Tempering)과 이산 Langevin 제안을 결합한 새로운 알고리즘인 PTDLP (Parallel Tempering enhanced Discrete Langevin Proposal)를 소개합니다. 연구진은 구간 온도에서 반복적인 샘플 스왑을 통해 지역 최소값에 빠지는 문제를 해결하고, 자동적으로 최적의 온도 스케줄과 체인 수를 결정하는 시스템을 개발하였습니다. 이 접근법은 다양한 작업에 대해 최소한의 조정으로 적응할 수 있습니다.

- **Technical Details**: PTDLP 알고리즘은 높은 온도의 체인이 다양한 모드 간의 연결을 촉진하는 역할을 하도록 설계되었습니다. 개별 체인 간의 스왑은 이산 샘플링을 위해 설계된 Metropolis 기준을 통해 결정되며, 근본적으로 상세한 균형이 유지됩니다. 본 연구에서는 이론적으로 알고리즘이 목표 에너지로 비 비대칭 수렴(non-asymptotic convergence)한다는 점과 여러 체인의 혼합 속도를 향상시키는 특성을 입증하였습니다.

- **Performance Highlights**: 실험 결과, PTDLP는 합성 문제가 포함된 복잡하고 다중 모드인 이산 분포에서 샘플링 및 학습 작업 모두에 있어 탁월한 성능을 보였습니다. 제안된 방법은 제한된 볼츠만 기계(restricted Boltzmann machines) 및 딥 에너지 기반 모델(deep energy-based models)에서도 기존 방법들에 비해 뛰어난 샘플링 능력을 입증하였습니다. 이러한 결과는 복잡한 이산 분포를 샘플링하기 위한 PTDLP의 우수성을 강조합니다.
...
### Langevin Multiplicative Weights Update with Applications in Polynomial Portfolio Managemen (https://arxiv.org/abs/2502.19210)
Comments:
          Accepted for AAAI-2025

- **What's New**: 이번 연구는 간단히 말해, 비볼록 최적화(nonconvex optimization) 문제와 그 제약이 단순체의 곱으로 이루어지는 경우를 다룹니다. Langevin 곱 가중치 업데이트(LM-weighted update, LMWU)라는 새로운 알고리즘을 제시하여, 복잡한 스케일의 노이즈를 추가해 비유클리드 기하학(non-Euclidean geometry)에서의 글로벌 최적화 문제를 해결하고자 합니다. 이러한 연구 방향은 신경망(neural networks) 근사화 및 내시 균형(Nash equilibrium) 탐색과 같은 다양한 비볼록 최적화 문제에서의 응용을 염두에 두고 있습니다.

- **Technical Details**: LMWU 알고리즘은 고전적인 MWU(Multiplicative Weights Update) 알고리즘의 변형으로, 이 알고리즘은 일반적으로 게임 이론과 다중 에이전트 시스템의 최적화에 사용됩니다. 특히, 단순체의 자연 기하학과 관련하여 확장 가능한 노이즈를 도입하여 알고리즘이 국소 최소값(local minima)을 탈출할 수 있는 기회를 제공합니다. LM-weighted update는 기하학적 브라운 운동(geometric Brownian motion) 개념을 바탕으로 하여 리만 다양체(Riemannian manifold)에서 작동하도록 설계되었습니다.

- **Performance Highlights**: LMWU 알고리즘은 제약 조건의 내부에서 글로벌 최적점에 수렴한다는 것을 보여주었으며, 이러한 수렴은 비대칭적(convergence analysis)으로 분석되었습니다. 제안된 알고리즘의 효율성은 실제 데이터인 다항식 포트폴리오 관리(polynomial portfolio management)에서 검증되었으며, 이 경우 최적화된 비선형 목표함수의 중요성이 강조되었습니다. 이로 인해, 알고리즘은 비볼록 최적화의 도전과제를 해결하기 위한 유망한 수단으로 자리잡고 있습니다.



### Quantum Annealing Feature Selection on Light-weight Medical Image Datasets (https://arxiv.org/abs/2502.19201)
- **What's New**: 본 연구에서는 quantum computing 알고리즘을 실제 quantum 하드웨어에서 적용하여 경량 의료 이미지 데이터셋에 대한 feature selection의 계산 집약적 문제를 해결하는 방법을 조사합니다. feature selection은 종종 k of n 선택 문제로 공식화되며, k와 n이 증가함에 따라 복잡성이 이항적으로 증가합니다. 전통적인 접근법들은 문제의 크기가 커짐에 따라 효율적으로 확장하기 어렵습니다. 이에 따라, 연구진은 quantum annealers를 이용해 기존보다 더 큰 feature selection 문제를 해결하는 방법을 제시합니다.

- **Technical Details**: 본 연구에서는 'linear Ising penalty mechanism'과 'subsampling', 'thresholding' 기술을 결합하여 문제의 확장성을 향상시키는 방법을 제안합니다. 실험은 의료 이미지를 복원하기 위해 사용되는 pixel masks를 학습하는 toy 문제를 통해 이루어졌습니다. 연구진은 이러한 방법들이 경량의 의료 이미지 데이터셋에서 효과적임을 보였고, 더 나아가 높은 차원의 최적화 작업에서도 그 가능성을 제시합니다. 그러나 현재 quantum 하드웨어의 한계로 인해 넓은 실세계 문제에 대한 적용 가능성은 여전히 불확실합니다.

- **Performance Highlights**: 연구 결과, quantum annealing 기반의 feature selection 방법이 간소화된 케이스에 효과적이라는 사실이 밝혀졌습니다. 연구진은 총 6개의 경량 의료 이미지 데이터셋에 대해 QA 기반 feature selection을 구현하고, 이를 통해 얻은 특징들이 이미지 압축을 위한 컨볼루션 인코더에서 유용하다는 것을 증명합니다. 비록 현재의 quantum 하드웨어의 제약으로 인해 규모와 성능의 한계가 존재하지만, 이 연구는 quantum 기술이 점점 더 복잡한 문제를 해결하는 데 어떻게 기여할 수 있는지를 보여주는 개념 증명 역할을 합니다.
...
### CodeIF: Benchmarking the Instruction-Following Capabilities of Large Language Models for Code Generation (https://arxiv.org/abs/2502.19166)
- **What's New**: 본 연구는 코드 생성에서의 LLMs(대형 언어 모델)의 명령 준수 능력을 평가하기 위해 최초로 설계된 베이스라인인 CodeIF를 소개합니다. CodeIF는 함수 합성, 오류 디버깅과 같은 다양한 작업을 포함하며, 이를 통해 모델의 성능을 다각도로 평가할 수 있습니다. 저자들은 35개의 최신 LLM을 대상으로 다양한 프로그래밍 언어와 난이도에서 체계적인 평가를 시행하여 현재 모델의 강점과 한계를 밝힙니다.

- **Technical Details**: CodeIF는 8개의 주요 범주와 50개의 세분화된 하위 지침으로 작업을 분류하여 LLM의 코드 생성 성능을 광범위하게 평가하는 데 초점을 맞추고 있습니다. 이 연구는 Completely Satisfaction Rate (CSR), Soft Satisfaction Rate (SSR), Rigorous Satisfaction Rate (RSR), Consistent Continuity Satisfaction Rate (CCSR)라는 새로운 평가 지표를 제안하여 다중 제약 문제를 다양한 관점에서 다룰 수 있도록 합니다.

- **Performance Highlights**: 실험 결과는 현재 LLM 모델들이 인간의 명령에 얼마나 잘 부합하는지를 밝히며, 일관되고 유지 관리하기 쉬운 코드 생성의 가능성을 강조합니다. CodeIF를 활용한 평가는 LLM 기반 코드 생성을 현대 소프트웨어 개발의 중요한 요소로 조명하며, 향후 연구의 방향과 개선 가능성을 제시합니다.
...
- **What's New**: 본 논문에서는 간헐적 시간 시계열의 확률적 예측을 위해 Gaussian Processes (GPs)를 도입했습니다. 새로운 모델은 숨겨진 함수에 대한 불확실성을 고려하는 Bayesian framework 내에서 훈련되며, 예측 시 이를 제거합니다. 특히, 처음으로 전체 매개변수가 지정된 Tweedie 분포를 간헐적 시간 시계열 예측에 활용하고 있습니다.

- **Technical Details**: 모델은 두 가지 유형의 예측 분포인 음수 이항 분포(negative binomial, NegBinGP)와 Tweedie 분포(TweedieGP)와 결합되어 있습니다. Tweedie 분포는 제로-인플레이티드(zero-inflated) 특성을 가지며 heavy-tailed 형태를 통해 기존 모델에서의 단순화된 가정을 회피합니다. 수천 개의 간헐적 카운트(time series) 데이터 세트에서 모델을 평가하고, 예측 성능을 비교했습니다.

- **Performance Highlights**: 실험 결과, 제안한 모델들이 경쟁 모델들에 비해 일관되게 더 나은 확률적 예측을 제공하는 것으로 나타났습니다. 특히, TweedieGP는 가장 높은 분위수(quantiles)의 추정에서 최상의 성과를 기록하여 NegBinGP보다 더 유연한 특성을 가진 것으로 밝혀졌습니다.



### Efficient and Accurate Spatial Mixing of Machine Learned Interatomic Potentials for Materials Scienc (https://arxiv.org/abs/2502.19081)
Comments:
          21 pages, 13 figures. To access the ML-MIX GitHub, click this https URL

- **What's New**: ML-MIX는 다양한 복잡성을 가진 상호 원자 포텐셜(interatomic potentials)을 혼합하여 대규모 분자 동역학 시뮬레이션의 속도를 크게 향상시키는 LAMMPS 패키지입니다. 이 방법은 양자역학/분자역학 모델링(QM/MM)에서 영감을 받았으며, '비싼' MLIP에서 '저렴한' 근사 모델을 생성하여 시뮬레이션 비용을 절감합니다. ML-MIX는 다수의 원자를 포함한 시스템에서 최대 11배의 속도 향상을 달성하며, 정적 및 동적 양질에서도 정확도를 유지합니다.

- **Technical Details**: ML-MIX는 다양한 원자 간 포텐셜을 선형 피팅(constrained linear fitting)을 통해 생성할 수 있도록 합니다. 이 방법은 두 가지 제약조건('soft' 및 'hard')을 적용하여 비용 효율적인 포텐셜이 비싼 포텐셜에 근접하도록 합니다. MLIP는 기존의 'empirical' 포텐셜보다 크게 느리지만, ML-MIX를 통해 시뮬레이션 효율성을 높일 수 있습니다.

- **Performance Highlights**: ML-MIX의 성능은 Si, Fe, W-He 시스템에서 유의미하게 검증되었습니다. 각 시스템에서 정적 및 동적 양들의 계산을 포함하여 '비싼' 시뮬레이션 결과와 일치하는 결과를 얻었으며, 이는 기본적으로 8,000개의 원자를 포함한 시뮬레이션에서 확인되었습니다. 공개된 데이터와 함께한 결과는 실제 결과와의 높은 일치도를 보여주어 ML-MIX의 유용성을 입증합니다.



### Fatigue-PINN: Physics-Informed Fatigue-Driven Motion Modulation and Synthesis (https://arxiv.org/abs/2502.19056)
Comments:
          13 pages, 9 pages. This work has been submitted to the IEEE for possible publication

- **What's New**: 본 연구에서는 Fatigue-PINN이라는 Physics-Informed Neural Networks (PINNs)에 기반한 딥러닝 프레임워크를 소개하여 피로 상태에서의 인간 움직임을 모델링합니다. 이 모델은 관절별 피로 구성을 제공하여 관절 수준에서 모션 아티팩트를 완화하고적으로 결합하여 더욱 사실적인 애니메이션을 생성합니다. 또한, 피로로 인한 최대 관절 토크의 변동을 시뮬레이션하여 실제 실험적 피로 연구의 결과와 일치하는 애니메이션을 제공합니다.

- **Technical Details**: Fatigue-PINN은 Three-Compartment Controller (3CC) 모델의 PINN 변형을 사용하여 피로를 효과적으로 모델링합니다. 이 모델은 피로 상태에 따라 운동 토크를 조정하고, 관절 각도와 토크 간의 변환을 보장하는 인코더-디코더 구조를 제공합니다. 이 구조는 특정 관절에 대한 피로를 추정할 수 있는 반면 동적 물리 모델링의 복잡성을 피할 수 있습니다.

- **Performance Highlights**: Fatigue-PINN은 고성능 운동과 비운동적 활동 모두에서 피로의 영향을 시뮬레이션할 수 있어, 다양한 분야에서의 응용 가능성을 지니고 있습니다. 이 모델은 실제 피로 연구와의 일관성을 유지하면서도, 고품질의 현실적인 피로 기반 애니메이션을 생성하는 데 강력한 성능을 발휘합니다. 또한, 깊은 학습 방법을 사용하여 시간 소모를 줄이고 더욱 정교한 동작 합성을 가능하게 합니다.



### Blending Optimal Control and Biologically Plausible Learning for Noise-Robust Physical Neural Networks (https://arxiv.org/abs/2502.19053)
Comments:
          28 pages, 10 figures

- **What's New**: 이 논문에서는 인공지능(AI)의 증가하는 계산 요구에 대응하기 위해 물리적 신경망(Physical Neural Networks, PNNs)을 활용한 새로운 훈련 접근 방식을 제안합니다. 이 접근법은 연속 시간 동적 시스템에 대한 최적 제어 방법과 생물학적으로 타당한 훈련 방식인 직접 피드백 정렬(Direct Feedback Alignment)을 통합하여 훈련 비용을 대폭 줄이는 데 초점을 맞추고 있습니다.

- **Technical Details**: 훈련 접근 방식은 물리적 프로세스의 고유한 계산 능력을 활용하여 효율적인 정보 처리를 가능하게 하며, 훈련 시간 단축과 함께 측정 오차(noise)와 잡음(noise) 하에서도 강건한 처리(robust processing)를 달성합니다. 이 방법은 상세한 시스템 정보 없이도 구현 가능하여 다양한 물리적 시스템을 PNN으로 활용할 수 있게 만듭니다.

- **Performance Highlights**: 제안된 접근법은 광전기 지연 시스템(optoelectronic delay system)에서 수치적(numerical) 및 실험적(experimental)으로 검증되었으며, 훈련 비용 감소와 강건한 처리 능력을 입증하였습니다. 이러한 결과는 PNNs를 실제로 사용할 수 있는 물리적 시스템의 범위를 상당히 확장하는 것으로 평가됩니다.



### Stationary distribution of node2vec random walks on household models (https://arxiv.org/abs/2502.19039)
Comments:
          19 pages, 6 figures

- **What's New**: 이 논문에서는 node2vec의 랜덤 워크(random walk)가 네트워크 임베딩(neural embedding) 알고리즘에서 중요한 도구임을 확인했습니다. 이러한 랜덤 워크는 조정 가능(tunable)하며, 전이 확률(transition probabilities)은 이전에 방문한 노드와 현재 및 이전에 방문한 노드를 포함하는 삼각형(triangle)에 따라 달라집니다. 이들 랜덤 워크의 수학적 속성은 충분히 연구되지 않았습니다.

- **Technical Details**: 우리는 커뮤니티 구조(community-structured)를 가진 가정 모델 그래프(household model graphs)에서 node2vec 랜덤 워크를 연구하였습니다. 논문에서는 워크 매개변수(parameters)에 따른 stationary distribution의 명시적인 설명을 증명하였으며, 이 매개변수를 조정하여 균일(uniform), 크기 편향(size-biased), 또는 단순 랜덤 워크(simple random walk) stationary distributions 간을 보간(interpolate)할 수 있음을 보여주었습니다.

- **Performance Highlights**: 또한, 특정 그래프 설정에서 이러한 효과를 탐색하였습니다. 이 연구는 node2vec 랜덤 워크의 다양한 가능성을 시연하고, 특정 상황에서의 적용성을 높이는 방법론을 제시합니다.



### Graph Neural Networks embedded into Margules model for vapor-liquid equilibria prediction (https://arxiv.org/abs/2502.18998)
- **What's New**: 이 논문에서는 Graph Neural Networks (GNNs)를 활용하여 extended Margules 모델에 통합함으로써 vapor-liquid equilibrium (VLE) 예측이 이루어졌다. 기존 UNIFAC-Dortmund 모델과 비교했을 때, Margules 모델에 내장된 GNN은 전반적으로 낮은 정확도를 보였지만, 다양한 이원 혼합물에서는 더 높은 정확도가 관찰되었다. 이는 GNN 기반의 Margules 모델이 분자 조각화의 실행 가능성 한계나 파라미터의 가용함에 따른 group contribution 방법의 대안으로 제시될 수 있음을 의미한다.

- **Technical Details**: VLE는 액체상 L에서의 각 성분의 fugacity와 기체상 V에서의 fugacity가 같을 때 달성된다. 이 연구에서는 두 가지 접근 방식으로 VLE를 계산하는데, 하나는 fugacity coefficients를 사용하고 다른 하나는 액상에 대한 activity coefficients와 standard fugacity를 사용한다. 본 연구는 GH-GNN 모델을 활용하여 infinite dilution 영역에서의 activity coefficients를 예측하고, 이를 extended Margules 모델로 확장하여 결국 유한 농도에서의 예측을 도출하였다.

- **Performance Highlights**: 실험 결과, GNN이 통합된 Margules 모델은 현재 사전에 훈련된 데이터에 기반하여 VLE를 예측할 수 있는 가능성을 보여준다. 특히, 이원 시스템에서의 VLE 예측 성능이 향상된 것으로 나타났다. 이러한 발견은 간단한 excess Gibbs energy 모델과 GNN의 조합이 예측 정확도를 어떻게 높일 수 있는지를 보여주는 기초 자료를 제공한다.



### Evaluating Membership Inference Attacks in heterogeneous-data setups (https://arxiv.org/abs/2502.18986)
Comments:
          Accepted in SiMLA workshop 2025 (co-located with ACNS)

- **What's New**: 이 논문은 머신 러닝(ML) 분야에서의 개인정보 보호 공격 중 회원 추론 공격(Membership Inference Attack, MIA)에 대한 새로운 접근을 제시합니다. 기존의 연구들은 주로 공격자의 보조 데이터셋과 목표 데이터셋이 동일한 분포에서 샘플링 된다는 전제를 밝혔지만, 이는 실세계에서는 흔치 않은 경우입니다. 이 연구는 이질적인 데이터 분포의 경우 MIA의 유효성을 평가하기 위한 메트릭(metric)을 설계하고, 두 가지의 서로 다른 데이터 이질성 시뮬레이션 방법론을 비교합니다.

- **Technical Details**: 이 연구에서 제안하는 데이터 이질성 메트릭은 두 개의 테이블 데이터 분포 간의 이질성을 측정하는 데 초점을 맞추고 있습니다. 특히, 이 메트릭은 데이터의 특성과 레이블 비대칭을 구분하며, MIA에서 데이터의 특성이 집중적으로 연구되어야 함을 강조합니다. 이질성의 평가를 위해 기존의 통계적 거리 개념을 적절히 확장하여 클래스를 고려하여 계산할 수 있는 방법을 제시합니다.

- **Performance Highlights**: 실험 결과는 이질적인 데이터 설정에 따른 MIA의 정확도가 실험 방법론에 따라 크게 달라질 수 있음을 보여줍니다. 한 시뮬레이션에서는 90%의 공격 정확도를 달성했지만, 다른 시뮬레이션에서는 50%의 정확도에 그쳤습니다. 이러한 결과는 MIA에 대한 연구가 이질적인 데이터 설정을 고려하면서도 표준화된 기준 없이 이루어지고 있다는 점을 시사합니다.
...
- **What's New**: 본 논문에서는 최근의 zero-shot text-to-speech (TTS) 모델의 한계를 극복하기 위해 S-DiT라는 새로운 TTS 시스템을 소개합니다. S-DiT는 혁신적인 sparse alignment 알고리즘을 사용하여 음성 텍스트 정렬의 어려움을 줄이며 자연스러운 발음 생성을 가능하게 합니다. 또한, 이 시스템은 액센트 강도를 조정하기 위한 다조건 classifier-free guidance 전략을 채택하고, 더 빠른 생성 속도를 위한 piecewise rectified flow 기술을 적용했습니다.

- **Technical Details**: S-DiT는 latent diffusion transformer (DiT) 모델을 기반으로 하며, phoneme tokens을 sparse하게 배치하여 발음 정보를 제공합니다. 이 모델은 강제 정렬 모델보다 더 높은 자연성과 안정성을 보여주며, 오류에 대한 내구성이 뛰어납니다. 또한, S-DiT의 구성은 음성 생성 프로세스를 가속화하며, 8개의 샘플링 단계만으로도 고품질의 음성을 생성할 수 있는 기능을 제공합니다.

- **Performance Highlights**: S-DiT는 실험 결과, state-of-the-art zero-shot TTS 음질을 달성했으며, 음성의 발음을 더욱 유연하게 조정할 수 있는 기능을 지원합니다. 실험에서 S-DiT는 LibriSpeech test-clean 세트에서 뛰어난 음성 인지율과 화자 유사성을 보여주었습니다. 이 연구는 음성 합성 시스템에서 사용자 맞춤형 TTS 서비스에 대한 새로운 가능성을 제시합니다.
...
### Automated Code Generation and Validation for Software Components of Microcontrollers (https://arxiv.org/abs/2502.18905)
Comments:
...
- **What's New**: 이 논문은 개발자 개입 없이 기존 임플리멘테이션에 원활하게 통합될 수 있는 임베디드 시스템의 소프트웨어 컴포넌트를 생성하는 방법을 제안합니다. STM32F407 마이크로컨트롤러의 GPIO 작업을 위한 하드웨어 추상화 계층(Hardware Abstraction Layer, HAL) 코드를 자동 생성함으로써, 자동화된 코드 완성을 통해 임베디드 애플리케이션을 지원합니다. 전통적인 코드 생성기와 비교할 때, 이 접근 방식은 RAG(Retrieval-Augmented Generation)과 AST(Abstract Syntax Trees)를 활용해 좀 더 유연하고 효율적으로 컴포넌트를 생성할 수 있는 경로를 제공합니다.

- **Technical Details**: AST는 소스 코드의 구조를 트리 구조로 표현한 것으로, 코드를 분석하고 컴포넌트를 생성하는 데 중요한 역할을 합니다. HAL은 하드웨어와의 상호작용을 위한 표준화된 인터페이스를 제공하여 코드의 이식성과 재사용성을 높입니다. RAG 기반 접근법에서는 코드 생성을 위한 문맥 정보를 제공하기 위해 임베딩과 벡터 스토어를 활용하여, 기존 코드베이스와 정합하게 생성된 HAL 코드를 통합하는 과정을 진행합니다.

- **Performance Highlights**: 제안된 방법의 유효성을 검증하기 위해 STM32F407 마이크로컨트롤러를 대상으로 프로토타입 구현을 진행했습니다. 각종 GPIO 작업을 위한 최적화된 하드웨어 특정 함수를 자동 생성하여 코드 생성의 일관성을 확보했습니다. 현재 프로토타입은 코드 분석과 수정 과정에서 수동 개입을 최소화함으로써 HAL 생성 과정을 효율적으로 수행할 수 있음을 입증하고 있습니다.
...
### Data-Driven and Theory-Guided Pseudo-Spectral Seismic Imaging Using Deep Neural Network Architectures (https://arxiv.org/abs/2502.18852)
Comments:
          163 pages, 91 figures, 16 tables, PhD Thesis, this https URL

- **What's New**: 본 논문에서는 Deep Learning (DL) 기술을 활용하여 Full Waveform Inversion (FWI)의 성능을 향상시키는 새로운 접근법을 제안합니다. 특히, 기존 FWI에서 잘 사용되지 않았던 pseudo-spectral 접근법을 DL과 통합하여 데이터 기반(data-driven) 및 이론 중심(theory-guided) 방법을 수립하였습니다. 이를 통해 DL 기반 FWI의 가능성을 탐구하고, 향후 연구 방향을 제시합니다.

- **Technical Details**: 논문의 핵심 기술로는 Deep Neural Networks (DNNs)와 Recurrent Neural Networks (RNNs)를 사용하여 pseudo-spectral FWI를 모델링하는 것입니다. 이 방법들은 이론적으로 유도되어, synthetic 및 Marmousi 데이터셋에서 테스트되었습니다. DNN은 속도 대비 회복에서 뛰어난 성능을 보이고, RNN은 정확도와 결함 식별에서 우수한 결과를 나타냅니다.

- **Performance Highlights**: 결과적으로, 데이터 기반 pseudo-spectral DNN은 깊고, 압축된 지역에서 전통적인 FWI를 초월하는 성능을 보여주었습니다. 이론 중심의 RNN은 오류가 적고, 결함 식별력이 뛰어나며, 표면 및 깊은 영역에서 안정성을 제공합니다. 이 연구는 FWI 성능 향상을 넘어 DL 기반 역전파의 폭넓은 응용 가능성을 식별하고 있습니다.
...
### FinTSB: A Comprehensive and Practical Benchmark for Financial Time Series Forecasting (https://arxiv.org/abs/2502.18834)
- **What's New**: 본 논문에서는 재무 시간 시계열 예측(FinTSF)을 위한 포괄적이고 실용적인 벤치마크인 FinTSB를 제안합니다. 이는 기존 연구에서 나타난 세 가지 시스템적 한계를 해결하고자 하며, 이동 패턴을 네 가지 세부 카테고리로 나누고 데이터의 품질을 평가하는 과정을 포함합니다. 이를 통해 다양한 평가 환경에서의 편향을 제거하고 표준화된 메트릭스를 통해 효과적인 비교를 가능하게 합니다.

- **Technical Details**: FinTSB는 다양한 규제 조건을 모델링하여 현실적인 거래 시나리오를 정확히 시뮬레이션하며, 거래 수수료와 같은 요소들을 포함합니다. 게다가 데이터를 처리하는 사용자 친화적이고 경량화된 파이프라인을 구축하여 다양한 백본 방법론을 통합하는 것을 목표로 합니다. 데이터 이동 패턴의 특성에 따라 질을 평가하고, 세 가지 차원에서 메트릭스를 표준화하여 평가의 일관성을 보장합니다.

- **Performance Highlights**: 광범위한 실험을 통해 FinTSB의 효과성을 검증하고 다양한 시장 조건에서 모델 선택을 위한 주요 통찰력을 제공합니다. 이는 연구자들이 새로운 FinTSF 방법론을 개선하고 평가하는 데 도움을 주는 혁신적인 플랫폼 역할을 할 것입니다. 또한, 본 연구는 응용 및 실용성 측면에서 의미 있는 성과를 달성할 수 있도록 설계되었습니다.



### Optimal Approximate Matrix Multiplication over Sliding Windows (https://arxiv.org/abs/2502.18830)
- **What's New**: 본 논문은 sliding window 모델 내에서 근사 행렬 곱셈(Approximate Matrix Multiplication, AMM) 문제를 탐구합니다. 여기서는 제한된 공간을 이용하여 대규모 행렬 곱셈을 스트리밍 방식으로 수행하는 알고리즘을 제안합니다. 새로운 DS-COD 알고리즘은 최적의 공간-오류(space-error) 트레이드오프를 달성하며 이론적 오류 경계와 복잡도 분석을 제공합니다.

- **Technical Details**: 이 논문에서 제안된 DS-COD 알고리즘은 두 개의 입력 행렬이 열 단위로 들어오는 상황에서 작업합니다. 알고리즘은 두 개의 저차원 근사 행렬을 유지하며, 이를 통해 입력 데이터를 효과적으로 처리하고 공간 및 시간 비용을 최소화합니다. 제안된 adaptive 버전인 aDS-COD는 계산 효율성을 향상시키고 실제 성능을 개선합니다.

- **Performance Highlights**: 대규모 실험을 통해 제안된 알고리즘은 이론적 결과를 검증하며 실제 데이터셋에서도 우수한 성능을 나타냅니다. 기존 COD 알고리즘보다 효율적이며, AMM 문제의 공간 하한도 설정하였습니다. 이 연구는 데이터 분석 및 기계 학습의 다양한 응용 분야에서 AMM의 활용 가능성을 제공합니다.
...
- **What's New**: 이 논문에서는 다중 뷰(multi-view) 고차원 데이터에서 변수 선택을 위한 세 가지 비선형(nonlinear), 희소(sparse), 일반화된 정준 상관 분석(generalized canonical correlation analysis, GCCA) 방법인 HSIC-SGCCA, SA-KGCCA, TS-KGCCA를 제안합니다. 이러한 방법은 기존의 SCCA-HSIC, SA-KCCA, TS-KCCA를 두 개 이상의 뷰로 확장하며, 분석에 있어 필수적인 세 가지 측면인 비선형 의존성(nonlinear dependence), 변수 선택을 위한 희소성(sparsity), 다중 데이터 뷰에 대한 일반화를 통합합니다.

- **Technical Details**: 제안된 HSIC-SGCCA 방법은 단위 분산 제약 조건을 포함하여 SCCA-HSIC에서 종전까지 간과되었던 문제를 해결합니다. HSIC-SGCCA는 비凸(nonconvex), 비 다중凸(non-multiconvex) 최적화 문제이며, 이를 해결하기 위해 블록 근접 선형(block prox-linear) 방법과 선형화된 대체 방향 방법(linearized alternating direction method of multipliers, ADMM)을 통합한 효율적인 알고리즘을 제안합니다. SA-KGCCA 및 TS-KGCCA의 경우, 다중凸(multi-convex) 최적화 문제를 블록 좌표 하강(block coordinate descent) 전략을 통해 해결합니다.

- **Performance Highlights**: 시뮬레이션 및 TCGA-BRCA 데이터 분석 결과, 제안된 HSIC-SGCCA가 변수 선택에서 경쟁 방법들보다 우수한 성능을 나타내었습니다. HSIC-SGCCA를 통해 선택된 변수들은 유방암 아형 분류 및 생존 시간 예측에서 훌륭한 결과를 보여주었습니다. 논문에서 제안한 방법들은 다중 뷰 고차원 데이터 분석에 있어 새로운 기준을 마련하며, 향후 생명과학 분야의 연구에 기여할 것으로 기대됩니다.
...
### Adaptive conditional latent diffusion maps beam loss to 2D phase space projections (https://arxiv.org/abs/2502.18684)
- **What's New**: 이 논문에서는 입자 가속기에서 흔히 사용되는 빔 손실 모니터(Beam Loss Monitor, BLM)와 빔 전류 모니터(Beam Current Monitor, BCM)가 기존의 제한된 정보를 넘어, 6차원(6D) 빔 위상 공간(phase space) 밀도를 상세히 분석할 수 있는 새로운 방법론을 제시합니다. 일반적인 비침습 장치들이 복잡한 패턴을 학습하여 고차원의 데이터로 변환하는 가능한 방식을 보여주고 있습니다.

- **Technical Details**: 논문은 generative conditional latent diffusion model을 사용하여 수십 개의 BLM 또는 BCM에서 수집된 파형(waveform)을 2D 프로젝션으로 변환하는 방법을 설명합니다. 이러한 방법은 가속기에서의 다차원 빔 동역학(dynamics)을 이해하는 데 중요한 역할을 합니다. 고강도 빔을 가진 LANSCE 직선 프로톤 가속기의 다중 입자 시뮬레이션을 통해 이 접근 방식의 효용성을 증명합니다.

- **Performance Highlights**: 이 혁신적인 방법론은 모든 입자 가속기에서 적용 가능하며, 간단한 비침습 장치들을 정교한 빔 위상 공간 진단(diagnostics) 도구로 변환할 수 있습니다. 이는 가속기 성능 향상과 실험의 깊이를 더하는 데 기여할 것으로 기대됩니다.
...
### A Distributional Treatment of Real2Sim2Real for Vision-Driven Deformable Linear Object Manipulation (https://arxiv.org/abs/2502.18615)
- **What's New**: 이번 연구에서는 시각 인지에 기반한 변형 가능한 선형 물체(Deformable Linear Objects, DLO) 조작을 위한 통합(end-to-end) 프레임워크를 제안합니다. 본 프레임워크는 변형 물체의 물리적 매개변수에 대한 사후 확률 분포를 추정하는데, 이는 likelihood-free inference (LFI)를 통해 이루어집니다. 이를 통해 로봇의 시뮬레이션에서 훈련된 정책을 실제 환경에서는 추가적인 조정 없이 적용할 수 있게 됩니다.

- **Technical Details**: 연구에서는 Bayesian inference의 표현력과 모델 프리 강화 학습(model-free reinforcement learning)의 유연성을 결합하여 Real2Sim2Real 문제를 해결하고자 합니다. Bayesian Sim (BayesSim)은 시각적으로 관찰된 DLO 집합에서 물리적 매개변수의 사후 확률을 추정하는 데 사용됩니다. 이 과정에서 Domain Randomisation (DR)을 통해 다양한 시뮬레이션 환경을 생성하고, 각 환경에서 작동하는 정책을 훈련합니다.

- **Performance Highlights**: 실험 결과, 본 통합 프레임워크를 통해 물리적 특성의 미세한 차이를 추론하고, RL 에이전트 행동을 이에 맞게 조정할 수 있음을 보여주었습니다. 특히 DLO에 대해 시뮬레이션 훈련 된 정책은 실제 환경에서도 효과적으로 전이되어 성과를 거두는 것으로 평가되었습니다. 이러한 연구 결과는 변형 가능한 물체 조작의 신뢰성을 높이는 데 기여할 것으로 기대됩니다.



### Tight Bounds on the Binomial CDF, and the Minimum of i.i.d Binomials, in terms of KL-Divergenc (https://arxiv.org/abs/2502.18611)
- **What's New**: 이 연구에서는 Binomial tail probability에 대한 유한 샘플의 상한 및 하한을 제시합니다. 이는 Sanov의 정리에 직접적으로 적용됩니다. 이러한 결과를 바탕으로 i.i.d. Binomial 확률 변수의 최소값에 대한 높은 확률의 상한 및 하한을 도출합니다.

- **Technical Details**: 제시된 상한과 하한은 유한 샘플(finite sample) 상황에서, KL-divergence를 기반으로 하여 표현됩니다. 이 연구는 대칭적으로 타이트(asymptotically tight)한 경계를 제공합니다. 이러한 기술적 접근은 통계적 동등성을 강화하는 데 기여합니다.

- **Performance Highlights**: 제공된 상한과 하한은 실제 데이터에서의 효과적인 추정을 가능하게 해줍니다. 따라서 연구 결과는 이론적 통계학뿐만 아니라 실무에서의 응용에도 유용합니다. 이러한 방법론은 binomial 변수에 대한 보다 정확한 해석을 가능하게 합니다.



### Toward Breaking Watermarks in Distortion-free Large Language Models (https://arxiv.org/abs/2502.18608)
Comments:
          5 pages, AAAI'25 Workshop on Preventing and Detecting LLM Generated Misinformation

- **What's New**: 이번 연구에서는 역공학(reverse-engineering) 기술을 통해 LLM의 왜곡 없는 워터마크 기법(distortion-free watermarking)인 Kuditipudi et al. (2024)의 핵심 비밀 키(secret key)를 예측할 수 있는 새로운 방법론을 제시합니다. 이 기법은 로그 벡터에 대한 변화를 주지 않고도 워터마크를 생성할 수 있는 접근법으로, 기존의 알고리즘보다 더 발전된 공격 모델을 다룹니다. 연구팀은 적은 표본만으로도 효과적인 스푸핑(spoofing) 공격을 수행할 수 있음을 보여 주었습니다.

- **Technical Details**: 연구에서 제안된 혼합 정수 선형 프로그래밍(mixed integer linear programming) 모델은 LLM에서 생성된 워터마크 데이터셋으로부터 비밀 워터마크 키를 정확히 추정합니다. 이 과정을 통해 특정한 워터마크 알고리즘을 사용한 입력 데이터에서, 변형되거나 손상된 상태에서도 Key를 강인하게 추정할 수 있는 기능을 갖추었습니다. 이러한 기법은 이론적으로 만들어진 기존의 워터마크 기술의 신뢰성을 도전하게 합니다.

- **Performance Highlights**: 제안된 공격 방법은 기존의 이해와 대비해 상대적으로 높은 성공률을 보입니다. 특히, 이 연구는 binary vocabulary와 실제 LLM 환경 모두에서 성공적인 스푸핑 결과를 보여주며, 이는 기업이나 개인에게 LLM의 신뢰성을 저하시킬 수 있는 우려를 더합니다. 이로 인해, 향후 연구자들은 워터마킹 기법의 안전성을 더욱 강화할 필요성이 대두됩니다.



### Expected Variational Inequalities (https://arxiv.org/abs/2502.18605)
- **What's New**: 이번 논문에서는 기존의 Variational Inequalities (VIs)을 바탕으로 Expected Variational Inequalities (EVIs)라는 새로운 개념을 소개하고 분석합니다. EVIs는 VI의 제약 조건을 기대값으로 만족하는 분포를 찾는 문제로, 게임 이론의 최근 기술을 활용하여 다루어집니다. 이 접근법 덕분에 EVIs는 일반(비단조) 연산자 하에서도 다항 시간(polynomial time) 내에 해결될 수 있어 기존 VIs의 복잡함을 완화합니다.

- **Technical Details**: EVIs는 비율적 균형(correlated equilibria)의 개념을 포착하면서 게임 이론의 영역을 넘어 더 넓은 적용 가능성을 가지게 됩니다. 구체적으로, EVIs는 ϵ-근사 Φ-expected variational inequality 문제로 정의되며, 이 문제는 μ∈Δ(𝒳)인 분포를 찾는 것입니다. 제약 조건을 정의하는 Φ의 크기에 따라 솔루션 집합의 타이트함이 달라지며, 이 결과는 게임 이론에서의 상관된 균형(correlated equilibrium) 개념과 밀접하게 연결됩니다.

- **Performance Highlights**: 이번 연구는 EVIs가 동적 게임 및 다양한 경제적 설정에서 기존의 불연속적인 결과를 일반화하는 데 사용될 수 있음을 보여줍니다. 특히, 매끄러운 게임, 결합 제약이 있는 게임 또는 비 볼록성을 가진 유틸리티를 다루는 문제에서도 효과적으로 적용될 수 있습니다. 이는 경제학과 게임 이론의 교차점에서 다루어지는 새로운 연구의 경향을 반영하며, VI 문제의 해를 구하는 데 있어 비효율적 경로를 피할 수 있는 가능성을 열어줍니다.
...
### Learning and Computation of $Φ$-Equilibria at the Frontier of Tractability (https://arxiv.org/abs/2502.18582)
- **What's New**: 이 논문에서는 $
icePhi$-균형을 계산하기 위한 새로운 알고리즘을 제시합니다. 특히, $
icePhi$가 다항식으로 이루어진 경우에서의 구성 가능성을 보여주며, 이는 Daskalakis et al.의 연구를 확장한 결과입니다. 주요 성과는 $n$명의 다변량 게임에서 $
icePhi$-균형을 근사하기 위한 다항시간 알고리즘을 제공한다는 것입니다.

- **Technical Details**: 제안된 알고리즘은 다차원 convex 제약 집합 $
iceX$에 대한 oracle 접근을 기반으로 하며, $O(n, d, k, 	ext{log}(1/
iceepsilon))$ 시간 복잡도로 작동합니다. 또한, 평균적인 $
icePhi$-후회가 $
iceepsilon$ 이하가 되도록 하는 효율적인 온라인 알고리즘을 제시하여, $
iceepsilon^2$ 라운드를 요구합니다. 기술적으로는, Papadimitriou와 Roughgarden의 방법을 바탕으로 하는 기대 고정점 계산 알고리즘이 핵심입니다.

- **Performance Highlights**: 이 연구는 온라인 학습 환경에서 $
icePhi$-후회의 학습 가능성을 규명하여, 첫 번째로 이 분야에 대한 명확한 하한을 제시합니다. 이에 따라 제안된 방법은 일반적인 $
icePhi$-후회 최소화에 효율적으로 기여할 수 있습니다. 성과로는 $
icePhi$-균형 계산에 대한 시간 복잡도를 대폭 개선하며, 온라인 및 오프라인 환경에 모두 적용 가능한 기술적 기반을 마련하였습니다.
...
### Colored Jones Polynomials and the Volume Conjectur (https://arxiv.org/abs/2502.18575)
Comments:
          27 pages, 16 figures

- **What's New**: 이번 연구에서는 3색 조안스 다항식(adjoint Jones polynomial)을 활용하여 15개 교차점까지의 하이퍼볼릭 노드(hyperbolic knots)에 대해 스핀-1 값의 다항식을 계산했습니다. 완전 연결 피드포워드 신경망을 사용하여 노드 보완(knot complement)의 볼륨(volume)을 99.34% 정확도로 예측할 수 있음을 보여주었습니다. 조안스 다항식의 특정 위상에서의 평가값을 기반으로 볼륨을 예측하는 데에도 유사한 정확도를 달성했습니다.

- **Technical Details**: Chern-Simons 이론은 가장 단순한 비자명 양자장 이론 중 하나로, 자연 연산자(natural operators)는 윌슨 루프(Wilson loops)입니다. 3차원 위상적 장 이론에서는 노드 K를 매개로 윌슨 루프 연산자의 기댓값을 통해 색조안스 다항식(Jn(K;q))를 정의합니다. 본 연구에서는 특히 하이퍼볼릭 노드의 노드 보완 볼륨과의 관계를 분석하고, 조안스 다항식을 기반으로 한 볼륨 추정의 개선된 진술을 제시합니다.

- **Performance Highlights**: 신경망은 조안스 다항식의 계수를 활용하여 볼륨을 예측하며, 이 예측의 정확도는 98%를 초과합니다. 또한, 2색 및 3색 조안스 다항식에 대한 분석을 통해 n색 조안스 다항식의 최적 위상을 추측하고 이를 통해 볼륨 추정의 개선 가능성을 제시합니다. 연구 결과는 하이퍼볼릭 노드에 대한 볼륨 추정에서 눈에 띄는 수렴성을 보여주었습니다.



### Transfer Learning for Transient Classification: From Simulations to Real Data and ZTF to LSS (https://arxiv.org/abs/2502.18558)
Comments:
          6 pages, 3 figures, 1 table

- **What's New**: 본 논문은 천문학적 전이체를 자동으로 분류하기 위한 기계 학습의 필요성을 강조합니다. 특히, 기존 분류 모델의 한계를 극복하기 위해 전이 학습(transfer learning)의 가능성을 제시했습니다. 이를 통해 Zwicky Transient Facility (ZTF)에서 훈련된 모델을 활용하여 LSST(진화의 공간과 시간에 대한 레거시 조사) 데이터에 적응할 수 있음을 보여주고 있습니다.

- **Technical Details**: 저자들은 RNN(Recurrent Neural Network)을 기반으로 하여 Gated Recurrent Units (GRU)를 사용한 분류기를 구축했습니다. 각 전이체(transient)마다 입력 벡터는 시간대별 데이터로 이루어진 매트릭스를 형성하며, 여기서 메타데이터(Metadata)의 처리도 함께 이루어집니다. 전이 학습 방법론은 다양한 신경망(Neural Network) 설계에서도 적용될 수 있습니다.

- **Performance Highlights**: 연구 결과에 따르면, 전이 학습을 통해 레이블이 있는 실질적인 데이터의 필요량을 75%까지 줄일 수 있으며, LSST 데이터에 적합하도록 ZTF 모델을 조정할 경우 30%의 훈련 데이터로 95%의 성능을 달성할 수 있음을 보여줍니다. 이러한 성과는 LSST의 초기 운영에서 신뢰할 수 있는 자동 분류가 가능함을 나타내며, 충분한 훈련 데이터를 수집하기 위해 몇 개월 혹은 몇 년을 기다릴 필요가 없음을 시사합니다.
...
- **What's New**: 이번 논문은 기존의 도구 기반 접근 방식을 넘어서는 새로운 환경인 'Programming with Pixels' (PwP)를 소개합니다. PwP는 에이전트가 미리 정의된 도구 API에 의존하지 않고 IDE에서 시각적 지각, 입력, 클릭을 통해 직접 작업할 수 있도록 합니다. 이를 통해 다양한 프로그래밍 언어와 도메인에 걸쳐 소프트웨어 엔지니어링(SWE) 작업을 통합하고 일반화 가능성을 높이는 것을 목표로 합니다. 또한 'PwP-Bench'라는 벤치마크를 통해 기존 SWE 벤치마크를 통합하여 에이전트를 체계적으로 평가할 수 있도록 합니다.

- **Technical Details**: PwP 환경은 VSCode 기반의 IDE로, 에이전트는 화면을 인식하고 입력, 포인팅 및 클릭과 같은 기본 동작을 통해 작업을 수행합니다. 이 환경은 언어나 도메인에 구애받지 않고 에이전트가 IDE 내에서 수행할 수 있는 모든 소프트웨어 엔지니어링 작업을 지원하며, 디버깅, 코드 제안 등 IDE의 다양한 도구를 사용할 수 있도록 합니다. PwP-Bench는 코드 생성, 풀 리퀘스트 해결, UI 개발 및 DevOps 워크플로우와 같은 15개 소프트웨어 엔지니어링 활동을 아우르는 통합된 벤치마크를 제공합니다.

- **Performance Highlights**: 실험 결과, 일반 목적의 컴퓨터 사용 에이전트가 다양한 SWE 작업에서 도구 기반의 전문 에이전트와 비슷하거나 더 나은 성능을 발휘할 수 있음을 보여줍니다. 그러나 현재 모델은 시각적 기초가 제한적이며, IDE의 도구를 활용하지 못하는 문제를 안고 있습니다. IDE 도구에 직접 접근할 수 있을 때 에이전트의 성능이 크게 향상됨을 보여주며, 이는 IDE의 내장 기능을 활용하는 잠재력을 강조합니다. 현재까지의 연구 결과, PwP 환경은 향후 소프트웨어 엔지니어링 에이전트를 개발하고 평가하는 데 있어 의미 있는 플랫폼이 될 것입니다.
...
### Disproving Program Equivalence with LLMs (https://arxiv.org/abs/2502.18473)
- **What's New**: 이번 연구에서는 코드의 대규모 언어 모델(LLM)을 평가하기 위해 기존의 수작업으로 생성된 유닛 테스트 기반 벤치마크가 다루지 못한 한계점을 해결하는 새로운 방법인 ProbeGen을 소개합니다. ProbeGen은 두 개 이상의 실행 가능한 코드 조각을 사용하여 그들의 동등성에 대한 반례를 찾는 화이트박스 방법론입니다. 이 방법은 코드 의미론을 비교하기 위해 코드에 대한 깊은 이해를 필요로 합니다.

- **Technical Details**: ProbeGen은 LLM이 실행 피드백을 받을 때 이 작업을 잘 수행함을 보여주었습니다. 일반적인 코드 합성 벤치마크에서, ProbeGen은 벤치마크에서 제공된 유닛 테스트가 동등하다고 간주했던 샘플의 18%를 반박하는 결과를 보였습니다. 이를 통해 유사한 의미를 가지면서도 구문적으로 구별되는 샘플들을 통합하여 의미적 자기 일관성을 개선하는데 도움을 제공합니다.

- **Performance Highlights**: ProbeGen을 사용함으로써 LLM 샘플의 의미적 클러스터링이 가능해져, pass@1 성능이 10% 향상되었습니다. 이는 코드 샘플의 의미적 유사성을 통해 더 나은 성능을 거둘 수 있음을 나타냅니다.
...
Comments:
          20 LaTeX pages, 11 Figures

- **What's New**: 이번 연구에서는 Physics-Informed Neural Networks (PINNs)를 활용하여 대체 우주론에서의 freeze-in 다크 매터(DM)을 지배하는 Boltzmann 방정식을 매개변수적으로 해결합니다. 이를 통해 단일 실험 데이터를 기반으로 이론의 물리적 속성을 도출하며, 기초가 되는 우주론 모델과 입자 간의 상호작용 단면적을 결정합니다. 또한, 우주의 확장을 모사하는 함수와 함께 FIMP(Feebly Interacting Massive Particles) 모델의 진화를 구체적으로 모형화합니다.

- **Technical Details**: 논문에서는 PINNs를 활용하여 Boltzmann 방정식(BEs)을 접목한 새로운 방법론을 제안합니다. 대수(positive/negative exponent)에 따라 DM의 실험적 관찰과 일치하는 입자 상호작용의 크기를 예측하고, 고유한 힘 법칙과 초기 조건을 통해 우주론의 진화에 대한 정보를 결합합니다. 이러한 접근 방식은 전통적인 수치 해법보다 더 적은 데이터를 통해 효율적으로 이론적 문제를 해결할 수 있습니다.

- **Performance Highlights**: Bayesian 방법론을 통해 이론적 매개변수의 불확실성을 정량화하며, 이 연구가 다크 매터 모델과 우주론적 현상에 대한 이해를 어떻게 개선할 수 있는지 보여줍니다. PINNs의 사용은 기존 방식보다 더 나은 예측 결과를 제공하며, 대체 우주론에서 DM의 생성 메커니즘을 탐구하는 효과적인 도구가 될 수 있습니다.
...
- **What's New**: 이 논문에서는 양자 기계 학습(quantum machine learning)을 이용한 단일 화소 카메라(single-pixel camera)의 이미징(imaging) 실험을 시뮬레이션했습니다. 이는 머신 러닝(machine learning) 기법으로 단일 화소 이미지를 신속하게 분석할 수 있게 해주며, 고차원(single-pixel) 시각화 문제를 해결할 가능성을 제시합니다. MNIST 데이터셋을 활용하여 단일 화소 이미지를 복원하고 분류하는 알고리즘을 개발했습니다.

- **Technical Details**: 단일 화소 이미징은 구조화된 빛(structured light)을 사용하여 이미지를 재구성하는 방식으로, 종래의 카메라에 비해 특정 파장(wavelength) 영역에서 높은 해상도를 제공합니다. 연구에서는 Hadamard 기저 패턴을 사용하여 구한 64개의 측정값을 바탕으로 고전적인 신경망(classical neural networks)과 양자 회로(parameterized quantum circuits)를 통해 이미지를 분류하고 재구성했습니다. 고전적 분류기와 양자 분류기는 각각 96%와 95%의 정확도를 기록하며 이러한 접근 방식의 유용성을 증명했습니다.

- **Performance Highlights**: 고전 신경망은 10회의 학습(epoch) 후 재구성에서 0.76의 구조적 유사성 지수(structural similarity index measure) 값을 보였고, 양자 신경망의 경우 0.25라는 상대적으로 낮은 값을 기록했습니다. 이는 현재 양자 신경망이 이런 구성에서 과제 해결에 있어 어려움을 겪고 있음을 나타냅니다. 작업의 결과들은 양자 기계 학습이 단일 화소 이미징 문제에 대한 유망한 결과를 만들어낼 수 있음을 시사하고 있습니다.

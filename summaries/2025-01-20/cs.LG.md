...
### Credit Risk Identification in Supply Chains Using Generative Adversarial Networks (https://arxiv.org/abs/2501.10348)
Comments:
...
- **What's New**: 본 연구는 신용 위험 관리(credit risk management)가 공급망(supply chain) 내에서 어떻게 중요한지에 대해 소개합니다. 특히, Generative Adversarial Networks (GANs)의 활용을 통해 신용 위험 식별을 강화하는 방안을 탐구합니다. GANs는 데이터 부족(data scarcity) 및 불균형 데이터셋(imbalanced datasets)과 같은 문제를 해결할 수 있는 합성 신용 위험 시나리오(synthetic credit risk scenarios)를 생성하는 데 도움을 줍니다.

- **Technical Details**: 연구는 세 가지 산업(제조업, 제약유통, 전자상거래)을 대상으로 신용 위험 전파(credit risk contagion)를 평가합니다. GAN이 생성한 데이터는 공급망 데이터의 동적(dynamic) 및 시간적(temporal) 의존성을 효과적으로 포착하며, 예측 정확도(predictive accuracy)를 향상시킵니다. 연구 결과, GAN 기반 모델은 전통적인 방법들(로지스틱 회귀, 결정 트리, 신경망)보다 더 높은 정확도, 재현율(recall), 그리고 F1 점수를 기록했습니다.

- **Performance Highlights**: 실험 결과, GAN 기반 모델이 전통적인 모델들과 비교했을 때 우수한 성과를 보여주며, 이는 공급망 내 재정적 붕괴(financial disruptions)를 완화하는 데 강력한 도구가 될 수 있음을 입증합니다. 이러한 결과는 GANs가 적극적인 위험 관리에 있어 중요한 잠재력을 지닌다는 것을 강조합니다. 향후 연구는 외부 시장 요인과 공급업체 관계를 포함하여 예측 능력을 더욱 향상시킬 수 있는 방향으로 확장될 것입니다.



### ColNet: Collaborative Optimization in Decentralized Federated Multi-task Learning Systems (https://arxiv.org/abs/2501.10347)
- **What's New**: 이번 연구는 Decentralized Federated Multi-Task Learning (DFMTL)에 대한 새로운 해결책으로 ColNet을 제안합니다. ColNet은 서로 다른 작업을 수행하는 클라이언트들을 그룹화하고 각 그룹의 리더가 서로의 모델을 교환하여 훈련하는 방식을 채택합니다. 이를 통해 레이블 및 작업 이질성이 존재하는 경우에도 성능이 향상되는 것을 보여줍니다.

- **Technical Details**: ColNet은 모델을 backbone과 task-specific layer로 나누어, 비슷한 작업을 수행하는 클라이언트들로 그룹을 형성합니다. 각 그룹의 리더는 hyper-conflict-averse (HCA) 집합 기술을 사용하여 서로 다른 그룹 간의 모델 평균을 수행합니다. 이러한 방식을 통해 conflicting gradients를 줄이고 다양한 작업 간 협업을 촉진합니다.

- **Performance Highlights**: CIFAR-10과 CelebA 데이터셋에서 ColNet의 성능을 실험하여 모델 성능이 향상되는 것을 확인했습니다. Aggregation round의 빈도가 늘어남에 따라 성능이 개선되었으며, 레이어 공유 수준에 따른 결과도 유의미했습니다. 기존의 다른 집합 기법과 비교했을 때, ColNet은 레이블 및 작업 이질성 모두에서 지속적으로 더 나은 성능을 보여주었습니다.



### Hybrid Deep Learning Model for epileptic seizure classification by using 1D-CNN with multi-head attention mechanism (https://arxiv.org/abs/2501.10342)
- **What's New**: 이 연구는 새로운 하이브리드 딥러닝 모델을 제안하며, 웨이브렛 변환(wavelet transform)과 1D 컨볼루션 레이어(Conv1D)를 결합하여 발작 감지(accidental detection) 정확도를 향상시키려는 목표를 갖고 있습니다. 이 모델은 기존 하이브리드 모델 중에서 가장 높은 정확도를 기록하였고, 특히 주의 메커니즘(attention mechanism)의 중요성을 강조합니다. 이러한 접근 방식은 자동 발작 감지 시스템의 개선에 기여할 것으로 기대됩니다.

- **Technical Details**: 모델은 웨이브렛 변환과 다중 헤드 주의 메커니즘(multi-head attention mechanism)을 결합하여 신경망 성능을 극대화합니다. EEG 신호의 복잡한 특성을 학습하기 위해 CNN과 LSTM 네트워크의 장점을 최대로 활용하여 시계열 데이터를 분석하고 분류하는 익숙한 방법론을 따릅니다. 이와 같은 하이브리드 구조는 EEG 신호에 특화된 정교한 특성 및 패턴 인식을 가능하게 합니다.

- **Performance Highlights**: 본 연구에서 제안한 모델은 기존의 하이브리드 모델들과 비교하여 우수한 발작 인식 정확도를 달성하면서, 중요한 시간적 특성을 캡처하는데 주의 메커니즘이 차지하는 중요한 역할을 입증합니다. 또한, 이 연구는 EEG 신호 분석과 관련된 기계 학습 성능 향상에 기여하며, 임상적 의사 결정을 개선하는 데 있어 실제적인 가능성을 보여줍니다.
...
- **What's New**: 본 논문에서는 CliMB-DC라는 새로운 프레임워크를 소개합니다. 이 프레임워크는 LLM(large language model) 기반의 코파일럿을 위한 데이터 중심(data-centric) 접근 방식을 제공하여, 도메인 전문가의 요구를 충족시키는데 중점을 둡니다. 데이터의 복잡한 문제를 해결하기 위한 적절한 도구와 인간의 개입을 결합하여, 실질적이고 효과적인 ML 도구를 구축할 수 있도록 돕습니다.

- **Technical Details**: CliMB-DC는 다중 에이전트 추론 시스템을 도입하여 동적인 계획 및 적응을 위한 전략적 조정자와 정밀 실행을 위한 전문 작업자를 결합합니다. 이를 통해 데이터 중심의 도전을 더욱 효과적으로 해결할 수 있게 됩니다. 또한, 인간이 참여하는(loop) 접근 방식을 통해 도메인 전문가의 전문지식을 체계적으로 통합하여, 추론 과정을 안내합니다.

- **Performance Highlights**: 실제 의료 데이터셋을 사용한 실험을 통해, CliMB-DC는 기존 코파일럿 기준보다 데이터 중심의 문제를 처리하는 능력에서 유의미한 성과를 보여주었습니다. 이 프레임워크는 다양한 도메인 전문가들, 즉 의료, 금융, 사회과학 등에서 ML을 통한 실제 효과를 높이는 데 기여할 수 있습니다.



### Pairwise Elimination with Instance-Dependent Guarantees for Bandits with Cost Subsidy (https://arxiv.org/abs/2501.10290)
- **What's New**: 이번 논문에서는 Multi-Armed Bandits with Cost Subsidy (MAB-CS)라는 새로운 프레임워크를 소개합니다. 일반적으로 결정의 보상(reward)이 미지수인 경우, 결정의 총 비용을 최소화하는 것을 더 중요하게 여기는 것을 다룹니다. 특히, 결정의 보상이 특정 기준(decision) 이상의 조건을 충족하면서 비용을 최소화하는 문제를 정의하였습니다.

- **Technical Details**: 논문에서는 MAB-CS의 여러 변형을 다루며, 특히 알려진 기준 팔(reference arm)로 제한되는 보상 문제와 보조금이 지원된 최상의 보상을 적용하는 경우를 포함합니다. Pairwise-Elimination (PE) 알고리즘을 기준 팔 변형에 적용하고, PE-CS라는 알고리즘을 보조금이 지원된 최상의 보상으로 일반화합니다. 이러한 알고리즘은 비용과 품질 회귀(regret)에 대한 로그(logarithmic) 상한을 가지며, 이는 다른 정책들과는 차별화된 보장을 제공합니다.

- **Performance Highlights**: MovieLens 25M과 Goodreads 데이터셋을 사용한 실험 결과, PE와 PE-CS 모두 높은 성능을 보였습니다. 특히 PE-CS는 문헌의 다양한 기준선 모델들과 비교하여 성능과 신뢰성을 균형 있게 제공하는 것으로 나타났습니다. 이러한 결과는 PE와 PE-CS 두 알고리즘의 효용성을 충분히 증명합니다.
...
### Logarithmic Regret for Nonlinear Contro (https://arxiv.org/abs/2501.10261)
- **What's New**: 이 논문에서는 알려지지 않은 비선형 동적 시스템을 학습하여 제어하는 문제를 다룹니다. 특히, 로보틱스와 헬스케어와 같은 높은 위험을 동반하는 분야에서의 오류가 치명적일 수 있는 응용 사례를 고려합니다.

- **Technical Details**: 빠른 연속 학습(fast sequential learning)을 통해 학습 에이전트가 완전한 정보 기준에 대해 로그 형태의 후회(logarithmic regret)를 가져올 수 있음을 보여주었습니다. 이 연구는 시스템의 동적 특성이 알려지지 않은 파라미터에 부드럽게 의존할 때, 지속적으로 흥미로움을 불러일으키는 최적 제어 정책이 있을 경우에 적용됩니다.

- **Performance Highlights**: 최적 정책이 지속적으로 흥미를 유발하지 않는 경우, 상호작용 수에 대해 제곱근 비례로 증가하는 후회 경계가 도출되었습니다. 시뮬레이션을 통해 제안된 이론의 트렌드가 확인되었으며, 이는 알려지지 않은 파라미터에 비선형적으로 의존하는 비선형 동적 시스템 제어에 대한 최초의 후회 경계를 제공합니다.
...
- **What's New**: 이번 연구는 메모리스터(memsristor)에 기반한 아날로그 컴퓨팅으로 구성된 다중 센서 무선 추론 시스템을 제안합니다. 이는 에너지 효율적인 컴퓨팅을 통해 전송 비용을 줄이고, 단일 센서 구성과 유사한 수준으로 유지합니다. 특히, Lp-norm을 기반으로 한 센서 융합 기술을 도입하여 네트워크와 센서의 특성에 맞춘 사용자 지정 통합을 가능하게 합니다.

- **Technical Details**: 연구에서는 M개의 아날로그 센서가 공통의 대상을 관찰하고, 각 센서는 메모리스터를 활용한 전면 네트워크로 원시 데이터를 처리하여 중간 피쳐 벡터를 생성합니다. Lp-norm 기법을 통해 변환 불변 피쳐를 추출하고, 여러 센서에서 주고받는 각 피쳐는 보안성과 개인 정보 보호를 강화하는 데 기여합니다. 이를 통해 이 시스템은 복잡한 작업에 대한 효율적인 솔루션을 제공합니다.

- **Performance Highlights**: 제안된 시스템은 CARLA, ModelNet, MNIST 데이터 세트를 사용하여 다양한 설정과 채널 조건 하에서 광범위한 시뮬레이션을 통해 검증되었습니다. 결과적으로, 에너지 소비를 줄인 동시에 성능 손실을 최소화하는 데 성공했습니다. 이 연구는 저전력 IoT 장치와의 호환성과 함께, 실시간 추론의 실행 가능성을 크게 향상시킵니다.
...
### SpaceTime: Causal Discovery from Non-Stationary Time Series (https://arxiv.org/abs/2501.10235)
- **What's New**: 이번 논문에서는 시간과 환경에 따라 변화하는 인과관계를 이해하기 위한 새로운 접근법을 제시합니다. 기존의 방법들은 정적( stationary) 가정을 전제하거나 시간과 공간의 변화 분포를 포함하지 않아 한계가 있었던 반면, 저자들은 이러한 세 가지 작업을 통합하여 해결했습니다. 저자들은 최소 설명 길이(Minimum Description Length) 원칙을 활용해 일관된 스코어를 생성하고, 이를 기반으로 인과 그래프를 탐색하는 새로운 방법을 개발했습니다.

- **Technical Details**: 제안된 알고리즘 SPACETIME은 비정상 다맥락(non-stationary multi-context) 환경에서의 인과 그래프 발견, 시간 체계 복원, 데이터셋 및 시간 구간 분할을 동시에 처리합니다. SPACETIME은 여러 개의 시간 시퀀스를 분석하여 변화점(regime changepoints)과 시간 인과 그래프를 비모수적(functional modeling) 접근법과 커널화된 불일치 테스트(kernelized discrepancy testing)를 사용하여 발견합니다. 이를 통해 인과관계가 불변하는 구간도 파악할 수 있습니다.

- **Performance Highlights**: 이 방법은 실제 사례에서도 큰 유용성을 보였으며, 다양한 유역에서 측정된 강 유출(water runoff) 데이터와 생물권-대기(biosphere-atmosphere) 상호작용에 대한 통찰을 제공합니다. 제안된 기법을 통해 연구자들은 복잡한 환경에서도 인과관계를 효과적으로 탐지하고 분석할 수 있습니다. 결과적으로 이 알고리즘은 시간과 공간의 복잡성을 동시에 고려하여 보다 정교한 인과 분석을 가능하게 합니다.



### Counterfactual Explanations for k-means and Gaussian Clustering (https://arxiv.org/abs/2501.10234)
- **What's New**: 이 논문은 클러스터링 결과를 설명하기 위해 counterfactuals의 사용을 제안하고 있습니다. 기존의 연구는 classification(분류) 문제에 초점을 맞췄으나, 제안된 방법은 clustering(클러스터링) 결과에 대한 새로운 정의와 방법론을 제공합니다. 본 연구는 k-means와 Gaussian 클러스터링에서의 counterfactual 생성 문제를 다루고 있습니다. 이러한 접근법을 통해 샘플을 클러스터에 어떻게 할당하는지에 대한 명확한 설명을 제공합니다.

- **Technical Details**: 제안된 counterfactual의 일반적인 정의는 모델 기반 클러스터링을 지원하며, plausibility(타당성)와 feasibility(가능성) 제약 조건을 포함합니다. k-means 클러스터링과 Gaussian 클러스터링 문제를 다룰 때, 유클리드 거리와 같은 조건 하에 counterfactual을 생성하는 비반복적(고정된) 방법이 도입되었습니다. 이 방법은 factual 인스턴스, 목표 클러스터, 바이너리 마스크 및 plausibility factor를 사용하여 최적의 해결책을 도출합니다.

- **Performance Highlights**: 이 연구는 합성 및 실제 데이터셋을 사용하여 제안된 방법을 평가하였고, tạo된 counterfactual이 기존의 간접 접근 방식을 기반으로 한 클러스터링 결과에 비해 유용함을 보여주었습니다. 실험 결과는 제안된 접근 방식이 더 나은 설명력을 제공함을 강조하였으며, 다양한 클러스터 모델에 대한 적합성을 테스트했습니다. 최종적으로, 이 방법이 클러스터링에서의 주목할 만한 성과를 보여주며, 향후 연구 방향을 제시하고 있습니다.



### Modelling Activity Scheduling Behaviour with Deep Generative Machine Learning (https://arxiv.org/abs/2501.10221)
- **What's New**: 이 연구는 딥 생성 기계 학습(deep generative machine learning)을 활용하여 인간의 활동 일정(activities schedule) 모델링을 제안합니다. 이 새로운 접근 방식은 사람들의 선호도(preferences)와 일정 논리(scheduling logic)를 복잡한 상호작용 하의 서브 모델(sub-model)이나 사용자 정의 규칙(custom-rules) 없이 학습합니다. 우리는 활동 일정 데이터를 연속적인 이미지 데이터(continuous image data)와 이산적인 텍스트 데이터(discrete text data)의 융합으로 파악하여 새로운 접근 방식을 요구합니다.

- **Technical Details**: 활동 일정은 개인의 활동 시작 시간과 지속 시간을 나타내는 최대 24시간의 활동 시퀀스를 포함합니다. 기존 연구에서는 일정 모델링을 분리된 선택들로 분해하는 방법을 선호하며, 이는 결정을 내리는 비현실적인 순서를 전제합니다. 연구는 활동 일정 생성에서의 복잡한 분포(complex distribution)를 다루고 있으며, 새로운 일정 표현과 생성된 일정의 종합적인 평가 프레임워크를 설정하여 다룬다는 점에서 의미가 있습니다.

- **Performance Highlights**: 이 방법론은 빠르고 단순하며 다양한 출력 시퀀스를 생성할 수 있는 강력한 특징을 보여줍니다. 실제 활동 일정을 신속하게 생성할 수 있으며 보안이나 데이터 익명화(data anonymisation)와 같이 다양한 응용 분야에 적합합니다. 실험 결과는 해당 접근 방식이 대량의 현실적이고 다양한 합성 샘플(synthetic samples)을 효과적으로 생성한다는 것을 확인하였습니다.



### The Relevance of AWS Chronos: An Evaluation of Standard Methods for Time Series Forecasting with Limited Tuning (https://arxiv.org/abs/2501.10216)
- **What's New**: 이번 연구는 AWS Chronos라는 트랜스포머 기반의 시간 시계열 예측 프레임워크를 전통적인 접근 방식인 ARIMA와 Prophet과 체계적으로 비교하였습니다. Chronos는 긴 예측에서 뛰어난 성능을 보여주었고, 과거 정보의 길이가 증가함에 따라 정확도를 유지합니다. 반면에 전통적인 모델은 문맥 길이가 증가함에 따라 성능이 급격히 저하되는 경향을 보였습니다. 이 연구는 제한적인 모델 조정이 가능한 실제 응용 프로그램에서 Chronos의 배포 가능성을 제기합니다.

- **Technical Details**: 연구에서는 2011년부터 2012년까지의 Washington D.C.의 자전거 대여 기록을 포함한 단일 데이터셋을 사용하여 Chronos의 성능을 분석했습니다. 데이터셋을 분석하기 위해 자전거 대여의 타임스탬프와 고객 유형의 이진 지표만을 추출하였으며, 이를 통해 두 개의 사용자 카테고리에 기반한 24개의 평가 시나리오를 생성했습니다. Chronos는 기존 대형 언어 모델의 구조를 통합하여 시간 시계열 데이터를 이산 토큰으로 변환하는 새로운 방법론을 도입했습니다.

- **Performance Highlights**: Chronos는 42개의 데이터셋을 포함한 벤치마크에서 기존의 로컬 모델 및 특정 작업의 딥러닝 방법보다 우수한 성능을 보여주었습니다. 특히, 장기 예측에서 뛰어난 결과를 나타냈으며, 새로운 데이터셋에 대해서도 경쟁력 있는 결과를 기록했습니다. 연구에 따르면 고객 유형에 따라 예측 품질이 체계적으로 달라지며, 이는 각 모델의 성능에 항상 영향을 미친다고 확인되었습니다.



### Temporal Graph MLP Mixer for Spatio-Temporal Forecasting (https://arxiv.org/abs/2501.10214)
- **What's New**: 이 논문은 Temporal Graph MLP-Mixer (T-GMM)라는 새로운 아키텍처를 소개하는데, 이는 스페이셜(Spatial) 및 템포럴(Temporal) 의존성을 효과적으로 처리하는 데 도움을 줍니다. 특히, 대규모의 연속된 결측 데이터가 존재하는 경우에도 예측 성능을 유지할 수 있도록 설계되었습니다. T-GMM은 노드 수준 처리와 패치 수준 서브그래프 인코딩을 결합하여 지역화된 공간적 의존성을 포착합니다.

- **Technical Details**: T-GMM은 3차원 MLP-Mixer 구조를 활용하여 템포럴, 스페이셜, 그리고 피처 기반 의존성을 동시에 처리합니다. 모델은 시계열 데이터와 불리언 유효성 마스크를 잠재 표상으로 변환하는 노드 인코더를 포함하고, 그래프는 METIS 알고리즘을 사용해 서브그래프로 분할됩니다. MLP-Mixer 핵심은 공간, 특징 및 시간 차원 전반에서 복잡한 의존성을 포착하여 예측 정확도를 향상시킵니다.

- **Performance Highlights**: AQI, ENGRAD, PV-US 및 METR-LA 데이터셋에서 실험된 결과, T-GMM은 심각한 결측 데이터가 존재하는 경우에도 효과적인 예측을 보여줍니다. 비록 모든 시나리오에서 최신 모델을 초월하지는 못했지만, T-GMM은 특히 장거리 의존성을 포착하는 데 강력한 능력을 갖추고 있습니다. 이 결과는 안정적이고 확장 가능한 스페이셜-템포럴 예측을 위한 잠재력을 증명합니다.
...
- **What's New**: 이 논문은 머신러닝에서 분포 파라미터 추정의 복잡성을 다룹니다. 특히 번역 파라미터(translation parameters)인 \boldsymbol{\mu} 및 축소(shrinkage) 파라미터 \sigma를 추정하는 문제에 초점을 맞추고 있습니다. 본 연구는 Maximum Likelihood Estimation(MLE)에서 NP-hard 문제임에도 불구하고, Wasserstein 거리(Wasserstein distance)를 통해 임의의 \varepsilon > 0에 대한 \varepsilon-근사치를 다항시간(poly-time) 내에 얻는 것이 가능함을 보여줍니다.

- **Technical Details**: 논문에서는 우선적으로 표본의 집합을 기반으로 하는 하이퍼 직사각형(hyperrectangle)의 유한합으로 구성된 분포를 고려합니다. 이러한 분포에 대해 \boldsymbol{\mu}와 \sigma 파라미터를 추정하고, Wasserstein 거리가 최소화되는 방안을 연구합니다. 세미-디스크리트 전송 문제(semi-discrete transport problem)의 프레임워크를 활용하여, \boldsymbol{\mu}와 \sigma를 다항시간 내에 추정할 수 있는 알고리즘을 제안합니다.

- **Performance Highlights**: Wasserstein 거리 기반 접근법은 MLE가 NP-완전(NP-complete) 문제를 야기하는 특정 분포 클래스에서 파라미터 추정을 수행하는 데 있어 효과적이라는 장점을 지니고 있습니다. 이 알고리즘은 대규모 최적화에는 비효율적일 수 있으나, 다항시간 복잡성을 달성하는 주요 결과를 분명히 제시합니다. 또한, Gaussian 혼합 모델의 Sliced Wasserstein 거리 연구와 같은 관련 연구들은 Wasserstein 메트릭스의 파라미터 추정에서의 유용성을 더욱 뒷받침합니다.
...
### Gene Regulatory Network Inference in the Presence of Selection Bias and Latent Confounders (https://arxiv.org/abs/2501.10124)
- **What's New**: 이 논문에서는 Gene Regulatory Network Inference (GRNI)의 중요한 도전 과제인 selection bias 및 latent confounders를 해결하기 위해 GISL(Gene Regulatory Network Inference in the presence of Selection bias and Latent confounders)라는 새로운 알고리즘을 제안합니다. 기존의 방법을 향상시키기 위해 다양한 유전자 교란 실험에서 얻은 데이터를 활용하며, 강력한 모수적 모델 없이도 진정한 규제 관계를 식별할 수 있음을 보여줍니다. 이를 통해 GRNI 연구에 새로운 접근 방식을 제공합니다.

- **Technical Details**: GISL 알고리즘은 selection bias와 latent confounders가 존재하는 상황에서 유전자 간의 인과 관계를 추론하는 데 중점을 둡니다. 주어진 데이터에서 두 변수 X와 Y의 독립성을 평가하는 수학적 모델을 포함하며, d-separated과 d-connected 개념을 통해 복잡한 인과 구조를 분석합니다. 알고리즘의 구조는 GISB와 유사하게 단계별로 설명되며, 관찰 데이터 및 교란 데이터를 활용하는 방법에 대한 자세한 내용이 포함되어 있습니다.

- **Performance Highlights**: 여러 합성 및 실제 단일 세포 유전자 발현 데이터셋에 대한 실험적 결과는 GISL이 기존 방법들보다 우수한 성능을 보임을 입증합니다. 특히, GISL은 조건부 독립성 패턴을 기반으로 하여 인과 과정, 선택 과정 및 잠재적 혼란 요소를 구별하는 데 효과적입니다. 이러한 성과는 Gene Regulatory Network를 더욱 정확하게 모델링하고 이해하는 데 기여할 것입니다.



### A recursive Bayesian neural network for constitutive modeling of sands under monotonic loading (https://arxiv.org/abs/2501.10088)
- **What's New**: 이번 연구에서는 불확실성 정량화를 위한 일반화된 Bayesian 추론을 도입한 재귀적 Bayesian 신경망(frBNN) 프레임워크를 제안합니다. 이 프레임워크는 기존의 재귀 피드 포워드 신경망(rFFNN)에서 시간 의존성을 효과적으로 포착할 수 있는 슬라이딩 윈도우 접근 방식을 통합하여 모델링의 정확성을 높입니다. 또한, rBNN은 모델 파라미터를 랜덤 변수로 간주하여 최종 예측에서 불확실성을 제시합니다.

- **Technical Details**: 이 연구는 두 가지 데이터셋에서 제안된 rBNN 프레임워크를 검증하고, LSTM, Bi-LSTM, GRU와 비교합니다. 연구는 rFFNN이 결정론적 모델로서 우수한 예측 정확성을 보이는 반면, rBNN은 측정 오차와 데이터의 희소성을 다루는 강력한 신뢰구간을 제공함을 입증합니다. 이러한 Bayesian 접근 방식은 모델의 복잡성을 줄이고 과적합을 방지하는 데 도움을 줍니다.

- **Performance Highlights**: 결과 분석에서 결정론적 rFFNN이 하드닝 토양 모델을 사용하는 수치적으로 시뮬레이션된 데이터셋 및 Baskarp 모래에 대한 실험 데이터셋에서 더 나은 예측 정확성을 보여주지만, rBNN은 반복적인 예측을 통해 신뢰구간을 제공함으로써 불확실성을 관리하는 데 강점을 보입니다. 이 연구는 결정론적 접근과 확률적 접근 간의 트레이드오프를 강조하며, 불확실성을 고려한 구성 모델링에 대한 rBNN의 가능성을 부각시킵니다.



### Two-level Solar Irradiance Clustering with Season Identification: A Comparative Analysis (https://arxiv.org/abs/2501.10084)
Comments:
          30 pages, 9 figures, 6 tables

- **What's New**: 이 연구에서는 두 단계의 클러스터링 기법을 통해 태양 복사 (solar irradiance) 데이터를 효과적으로 군집화합니다. 첫 번째 단계에서는 맑은 날의 복사량을 기준으로 계절을 식별하고, 두 번째 단계에서는 각각의 계절 내에서 맑음, 흐림, 부분 흐림에 따라 일별 클라우드 수준을 분류합니다. 본 연구는 또한 Daily Irradiance Index (DII), Euclidean Distance (ED), Dynamic Time Warping (DTW) 거리의 성능을 비교하여 DII 기반 클러스터링의 우수성을 입증하였습니다.

- **Technical Details**: DII는 측정된 복사량의 시간 적분과 맑은 하늘에서의 복사량의 시간 적분의 비율로 계산됩니다. 클러스터링은 각 단계에서 정확한 클러스터 식별을 위해 평가 척도를 사용하여 정량적 및 정성적으로 비교되었습니다. 특히 DII 기반 클러스터링은 연간 데이터에서도 효과적이며, ED 기반 클러스터링이 DTW 기반 클러스터링보다 더 높은 성능을 보였습니다.

- **Performance Highlights**: 연구 결과는 두 개의 서로 다른 미국 위치에서 데이터를 사용하여 철저히 검증되었습니다. 이 방법은 대규모 데이터 세트에 대해 뛰어난 확장성을 보여주며, 다른 위치에서도 적용 가능성을 지니고 있습니다. DII 기반 클러스터링은 태양 복사 클러스터링 연구의 새로운 벤치마크를 설정하며, 재생 가능한 에너지 예측 모델의 개선에도 기여할 것으로 기대됩니다.
...
- **What's New**: PandaSkill은 e스포츠에서 선수의 성과와 스킬 등급을 평가하기 위한 새로운 프레임워크입니다. 전통적인 등급 시스템은 개인 기여를 간과하고, 비효율적인 게임 데이터 및 단편화된 경쟁 장면으로 인해 어려움을 겪습니다. PandaSkill은 머신러닝을 활용하여 개별 선수 통계로부터 게임 내 성과를 추정하고, 이를 바탕으로 Bayesian 프레임워크인 OpenSkill을 통해 선수의 스킬 등급을 업데이트합니다.

- **Technical Details**: PandaSkill은 두 단계의 프레임워크로 구성되어 있으며, 첫 단계에서는 머신러닝 모델을 통해 선수의 게임 성과를 예측합니다. 두 번째 단계에서는 선수의 스킬 등급을 과거 성과 점수를 바탕으로 계산하며, 참여한 개별 플레이어들이 상대적으로 평가받는 '자유형 (free-for-all)' 접근 방식을 채택합니다. 또한, 지역 간 비교를 개선하기 위해 지역 등급과 메타 등급의 조합으로 이중 등급 시스템을 도입했습니다.

- **Performance Highlights**: PandaSkill은 5년 간의 전문 League of Legends 경기 데이터를 적용하여, 기존 방법론보다 더욱 정확한 게임 결과 예측과 전문가 의견과의 일치성을 보여주었습니다. 이 방법론은 플레이어의 개인 성과를 강조하며, 국제 대회 외에도 다양한 지역의 선수들의 성과를 비교할 수 있는 기회를 제공합니다. 모든 소스코드와 데이터는 PandaSkill의 GitHub 리포지토리에 공개되어 있어, 추가 연구와 실무에서의 활용을 촉진하고자 합니다.
...
### Sparse Binary Representation Learning for Knowledge Tracing (https://arxiv.org/abs/2501.09893)
- **What's New**: 이 논문에서는 Sparse Binary Representation KT (SBRKT)라는 새로운 지식 추적 모델을 제안합니다. 기존의 KT 모델은 주로 인간이 정의한 지식 개념(KCs)에 의존하고 있는데, 이로 인해 모델의 성능이 제한되는 문제를 해결하고자 합니다. SBRKT는 새로운 보조 KCs를 생성하여 기존 KCs를 보완하는 방식으로 이를 해결합니다. 이 모델은 이진 벡터 표현을 통해 학습하여 각 보조 KC의 존재 여부를 나타내며, 이러한 표현은 여러 가지 KT 모델에서 활용될 수 있습니다.

- **Technical Details**: SBRKT는 먼저 학생의 상호작용 데이터를 기반으로 각 연습 문제에 대해 희소 이진 벡터를 학습하는 신경망 모델을 개발합니다. 이 모델은 보조 KCs를 기반으로 한 희소 표현을 생성하며, 이진 벡터에서 1은 보조 KC의 존재를, 0은 부재를 나타냅니다. SBRKT는 순환 신경망(RNN)을 통합하여 시간적 동역학을 포착하고, 미래 학생의 반응을 예측합니다. 이 과정에서 학습된 보조 KCs는 전통적인 BKT 및 현대의 심층 학습 방법과 호환 가능하여, 다양한 KT 모델에서의 응용성이 높습니다.

- **Performance Highlights**: 실험 결과, SBRKT는 다양한 데이터셋에서 기존의 기준선 모델들보다 더 뛰어난 성능을 보였으며, 특히 BKT의 성능이 전반적으로 향상되는 것을 보여주었습니다. 보조 KCs를 통합한 후 모델의 예측 정확성이 상당히 개선되었고, 이는 기존의 KT 모델들과 비교했을 때 중요한 결과입니다. 이러한 결과는 SBRKT가 KT 작업에서의 효과적인 접근법임을 나타냅니다.



### An LLM-Guided Tutoring System for Social Skills Training (https://arxiv.org/abs/2501.09870)
- **What's New**: 이 논문은 대화형 트레이닝을 통한 사회적 기술 훈련을 위해 대규모 언어 모델(LLMs)을 활용하는 새로운 프레임워크를 제안합니다. 기존의 전통적인 교실 교육 방식들이 효과적인 의사소통을 가르치는 데 불충분함을 지적하며, 현실적인 시나리오 기반의 실습을 통해 학습 효과를 극대화하는 방법을 제시합니다. 이 프레임워크는 강사가 기술적 전문 지식 없이도 실시간으로 시나리오를 동적으로 생성하고 수정할 수 있도록 돕습니다.

- **Technical Details**: 제안된 'Guided Learning for Optimizing Soft Skills (GLOSS)' 프레임워크는 강사와 LLM이 협력하여 내러티브 그래프를 구축하는 프론트 엔드 빌더를 포함합니다. 이 그래프는 학생의 반응에 따른 다양한 상호작용을 시각적으로 표현하며, 학생들이 특정한 시나리오에 따라 실습을 진행할 수 있도록 돕습니다. LLM은 학생의 입력에 대한 피드백을 제공하며, 필요 시 새로운 시나리오 경로를 즉시 생성합니다.

- **Performance Highlights**: GLOSS 시스템은 학생들이 상호작용을 통해 얻은 경험을 기반으로 피드백을 제공함으로써 사회적 기술 향상을 지원합니다. 이 시스템은 특히 감정적으로 민감한 상황이나 어려운 시나리오에서의 실습을 가능하게 하여 안전한 학습 환경을 제공합니다. 향후 연구에서는 아바타의 반응 개선, 즉각적 및 지연 피드백 메커니즘 최적화 및 사용자 연구를 통한 효과성을 평가하는 작업이 예정되어 있습니다.
...
### Learning Noisy Halfspaces with a Margin: Massart is No Harder than Random (https://arxiv.org/abs/2501.09851)
Comments:
          Appeared in NeurIPS 2024

- **What's New**: 이번 논문에서는 Massart noise가 있는 𝛾-margin halfspaces의 PAC 학습 문제를 다루고, Perspectron이라는 간단한 알고리즘을 제안합니다. 이 알고리즘은 샘플 복잡도가 𝑜((𝜖𝛾)^{-2})이며, 분류 오류는 최대 𝜂 + 𝜖에 달하는 것을 보장합니다. 이는 과거의 연구들보다 개선된 샘플 복잡도 보장을 제공하며, 특히 일반화된 선형 모델에서도 유사한 성능을 발휘합니다.

- **Technical Details**: Massart noise 모델은 학습 문제가 난이도와 실용성을 모두 고려할 수 있게 해주며, 여러 머신러닝 커뮤니티에서 주목받고 있습니다. 정의된 1번의 Massart noise 모델은 세 가지 주요 특징을 가지고 있으며, 이는 점으로 제약된 다양한 노이즈 유형을 통합할 수 있게 해줍니다. 이를 통해 기존의 RCN 모델보다 학습이 용이할 수 있는 가능성을 제시합니다.

- **Performance Highlights**: Perspectron 알고리즘은 Massart noise 환경에서도 이전 연구보다 나은 샘플 복잡도를 유지하며, 심지어 일반화된 선형 모델 학습에서도 호환되는 성능을 보입니다. 이는 다양한 과거 연구와 비교하였을 때, 현재 성능의 선두주자로 자리매김할 수 있는 가능성을 보여줍니다. 이러한 결과는 실제 애플리케이션에서도 유용할 것으로 기대됩니다.



### Coded Deep Learning: Framework and Algorithm (https://arxiv.org/abs/2501.09849)
- **What's New**: 이 논문에서는 'coded deep learning'(CDL)이라는 새로운 프레임워크를 소개합니다. CDL은 정보 이론적 코딩 개념을 딥 러닝의 내부 작동에 통합하여, 모델 가중치와 활성화를 크게 압축하고, 훈련 및 후처리 추론 단계 모두에서 계산 복잡성을 줄입니다. 또한, 모델 및 데이터 병렬성을 효율적으로 가능하게 함으로써 자원 제한 환경에서도 딥 뉴럴 네트워크(DNN)의 효율적 배포를 지원합니다.

- **Technical Details**: CDL에는 다양한 혁신적인 요소가 포함되어 있습니다. 첫째, 모델 가중치와 활성화를 양자화하기 위한 새로운 확률적 방법을 제안합니다. 둘째, 훈련 중에 양자화된 가중치와 활성화를 기반으로 전방 및 후방 패스를 실행하여 대부분의 부동소수점 연산(Floating Point Operations, FLOPs)을 제거합니다. 셋째, 훈련 과정에서 가중치와 활성화가 엔트로피 제약을 받아 정보 이론적 압축이 가능하도록 설계되었습니다.

- **Performance Highlights**: 포괄적인 실험 결과, CDL과 그 변형인 relaxed CDL(R-CDL)은 기존의 최첨단 양자화 방법들과 비교해 우수한 성능을 보여줍니다. 특히, CDL은 양자화를 통한 높은 정확도와 압축 간의 균형을 유지하면서도 훈련 복잡성을 감소시킬 수 있는 강력한 접근법으로 자리 잡았습니다. R-CDL은 풀 프리시전으로 훈련의 정확도를 높이며, 이전 방법들보다 역시 뛰어난 성능을 보였습니다.



### pFedWN: A Personalized Federated Learning Framework for D2D Wireless Networks with Heterogeneous Data (https://arxiv.org/abs/2501.09822)
Comments:
          16 pages, 9 figures, 3 tables, submitted to Transactions on Networking

- **What's New**: 이 논문에서는 전통적인 Federated Learning (FL) 접근법의 한계를 극복하기 위해 Personalized Federated Learning (PFL) 방법론을 제안합니다. 클라이언트 간의 데이터 이질성과 비독립적으로 분포된(non-IID) 데이터 문제를 해결하며, 자원 할당과 간섭 관리와 같은 무선 링크에서의 새로운 도전과제를 다룹니다. 또한, server-free PFL 접근법에서 D2D 무선 채널 조건을 통합한 최적화 문제를 공식화합니다.

- **Technical Details**: 제안된 방법, pFedWN은 각 클라이언트의 학습 성능을 최적화하기 위해 D2D 무선 채널의 변동성을 고려합니다. 이 문제를 해결하기 위해 먼저 PFL 이웃 선정과 PFL 가중치 할당으로 나누어 접근하며, 비면허 스펙트럼 대역에서 채널 인식 이웃 선정(channel-aware neighbor selection)을 통해 이웃을 선택합니다. 또한, Expectation-Maximization (EM) 방법을 사용하여 클라이언트 데이터 간의 유사성을 평가하고 최적의 가중치 분포를 통해 PFL 가중치를 할당합니다.

- **Performance Highlights**: pFedWN은 비독립적이며 불균형한 데이터셋에서 효율적이고 개인화된 학습 성능을 제공합니다. 실험 결과, pFedWN은 기존의 FL 및 PFL 방법보다 학습 효율성과 강건성이 뛰어나며, 특히 동적이고 예측 불가능한 무선 채널 조건에서도 우수한 성능을 보입니다.



### BN-Pool: a Bayesian Nonparametric Approach to Graph Pooling (https://arxiv.org/abs/2501.09821)
- **What's New**: 이 논문에서는 그래프 신경망(GNN)을 위한 최초의 클러스터링 기반 풀링 방법인 BN-Pool을 소개합니다. BN-Pool은 베이지안 비모수적 프레임워크를 활용하여, 그래프에서 슈퍼노드(supernode)의 수를 적응적으로 결정합니다. 이 방법은 그래프 노드를 무한한 수의 클러스터로 분할할 수 있는 생성 모델을 사용하며, 기존 그래프 토폴로지를 재구성하는 것을 장려하고 불필요한 클러스터의 증가를 처벌하는 방식으로 학습합니다.

- **Technical Details**: BN-Pool은 입력 그래프의 인접 행렬에 대한 생성 과정을 정의하며, 두 노드 간의 연결 확률은 클러스터 소속에 따라 달라집니다. 이를 통해 클러스터는 그래프의 토폴로지를 반영하며, 클러스터 수는 사전 고정되지 않고 입력 그래프에 맞춰 조정됩니다. 본 연구에서는 이러한 비모수적 클러스터링 기능이 GNN의 파라미터와 함께 두 가지 상보적 목표를 최적화하여 학습됩니다.

- **Performance Highlights**: BN-Pool은 다양한 벤치마크에서 우수한 성능을 보여주며, 그래프의 구조적 변동에 유연하게 적응할 수 있는 특성을 갖추고 있습니다. 이는 클러스터 수를 사전에 정의할 필요 없이 그래프의 특성에 기반하여 자동으로 최적의 수준을 발견할 수 있도록 해줍니다. 따라서 BN-Pool은 GNN의 성능 향상을 위한 혁신적인 접근법으로 자리잡을 것으로 기대됩니다.
...
### Graph Neural Networks for Travel Distance Estimation and Route Recommendation Under Probabilistic Hazards (https://arxiv.org/abs/2501.09803)
Comments:
          17 pages, 11 figures

- **What's New**: 이 논문은 극한 상황 발생 시 도시 내 다양한 위치 간의 최단 이동 시간을 추정하고 경로 추천을 제공하기 위한 새로운 프레임워크를 제안합니다. 기존의 Dijkstra 알고리즘은 대규모 네트워크에 적용할 경우 계산 비용이 매우 높아지는데, 이를 해결하기 위해 그래프 신경망(GNN, Graph Neural Networks)을 기반으로 한 빠른 방법을 활용합니다. 해당 연구는 충수 상황에서 공공 대피소까지의 대기 시간 계산에 GNN 모델을 적용한 사례 연구를 포함하고 있습니다.

- **Technical Details**: 제안된 모델은 GNN을 기반으로 하여 두 위치 간의 최단 거리를 추정하고, 네트워크의 토폴로지 변화에 효과적으로 대응할 수 있도록 설계되었습니다. GNN은 노드와 엣지의 표현을 학습하기 위해 이웃 특징을 추출하면서 정보 교환을 수행합니다. 이 과정에서 생성된 임베딩은 이후 다운스트림 작업에서 활용될 수 있습니다.

- **Performance Highlights**: 실험 결과는 GNN 모델이 정확성과 계산 효율성 모두를 확보하고 있음을 보여줍니다. 특히, 극한 상황에서 공공 안전을 보장하기 위해 대피소 위치를 정하고 구출 시간을 추정하는 데 있어 최단 거리 계산이 필수적입니다. 이 연구는 긴급 계획 및 관리에서 GNN의 효과적인 구현 가능성을 제시합니다.



### Multi-Head Self-Attending Neural Tucker Factorization (https://arxiv.org/abs/2501.09776)
- **What's New**: 이 연구는 Multi-head Self-attending Neural Tucker Factorization (MSNTucF)라는 새로운 신경망 기반의 텐서 분해 접근 방식을 제안합니다. 이 모델은 복잡한 비선형 공간적-시간적(spatiotemporal) 특징 상호작용 패턴을 학습할 수 있도록 설계되었으며, 예측 성능을 향상시키는 데 중점을 둡니다. 기존의 QoS 예측 모델들이 고차원 데이터의 동적 패턴을 잘 캡처하지 못하는 문제를 해결하는 데 기여하고자 합니다.

- **Technical Details**: MSNTucF 모델은 전통적인 Tucker 텐서 분해 프레임워크를 일반화한 신경망 구조를 도입하며, 이를 바탕으로 다중 헤드 자기 주의 모듈을 활용해 비선형 잠재 상호작용을 강화합니다. 이 연구는 시계열적 요소를 고려함으로써 QoS 데이터 내에서 시간이 지남에 따라 변화하는 패턴을 캡처하는 데 초점을 맞췄습니다. 텐서 데이터는 3D 텐서 형태로 표현되며, 각 차원은 다양한 서비스를 나타내는 사용자-서비스 QoS 매트릭스의 행(row)과 열(column)로 구성됩니다.

- **Performance Highlights**: 제안된 MSNTucF 모델은 두 개의 실제 QoS 데이터 세트에서 기존의 최신 기술을 초월하는 성능을 보여주었습니다. 이 연구는 QoS 데이터의 빈 값(missing values)의 예측에서 놀라운 정확성을 자랑하며, 기존 모델들이 간과했던 비선형 공간적-시간적 표현을 잘 학습하여 정보를 효과적으로 전달합니다. 이는 결국 서비스 선택과 의사 결정 과정에서 보다 신뢰할 수 있는 예측을 가능하게 합니다.
...
### DADA: Dual Averaging with Distance Adaptation (https://arxiv.org/abs/2501.10258)
- **What's New**: 이번 논문에서는 볼록 최적화 문제를 해결하기 위한 새로운 보편적 그래디언트 방법인 Dual Averaging with Distance Adaptation (DADA)를 제안합니다. DADA는 전통적인 이중 평균(Dual Averaging) 기법에 기반하고 있으며, 관찰된 그래디언트와 시작점 사이의 거리의 변화에 따라 계수를 동적으로 조정합니다. 이를 통해 문제 특정 파라미터의 필요성을 없애고, 다양한 문제 클래스에 대해 효과적으로 작동할 수 있습니다.

- **Technical Details**: DADA는 비매끄러운 Lipschitz 함수, Lipschitz-부드러운 함수, Hölder-부드러운 함수 등 여러 종류의 함수에 적용됩니다. 이 알고리즘은 최적화의 최소값 근처에서 목표 함수의 지역적 증가량이 제한될 수 있는 경우에 유효합니다. 또한, DADA는 제약이 없는 문제뿐만 아니라 제약이 있는 문제에도 적용 가능하며, 도메인이 무한대일 때도 효율적인 성능을 보여줍니다.

- **Performance Highlights**: DADA는 사전에 반복 횟수나 원하는 정확도에 대한 지식이 없어도 적용 가능합니다. 이러한 특성 덕분에 DADA는 다양한 최적화 문제가 주어졌을 때, 특별한 조정 없이도 일관된 성능을 보장하여 실용성이 높습니다. 이는 다양한 최적화 상황에서 DADA의 사용을 유도할 수 있는 중요한 장점이 됩니다.
...
### Amortized Bayesian Mixture Models (https://arxiv.org/abs/2501.10229)
Comments:
          34 pages, 17 figures

- **What's New**: 이 논문에서는 Amortized Bayesian Inference (ABI)의 새로운 확장을 소개합니다. 이 확장은 mixture 모델에 특화되어 있으며, posterior를 매개변수 분포와 mixture 지표 (indicator) 분포로 분해합니다. 이는 매개변수 추정에는 생성적 신경망 (generative neural networks)을 사용하고, mixture 구성원 식별에는 분류 네트워크 (classification networks)를 사용할 수 있도록 합니다.

- **Technical Details**: 제안된 프레임워크는 독립적 (independent) 및 종속적 (dependent) mixture 모델 모두를 수용할 수 있으며, 필터링 (filtering) 및 스무딩 (smoothing) 기능을 포함합니다. 이러한 접근 방식은 명시적인 likelihood 없이 모델 적합을 가능하게 하여 빠른 추론 (inference)을 제공합니다. 또한, 이 프레임워크는 고차원 posterior 추론과 레이블 스위칭 (label switching) 문제를 해결하기 위한 혁신적인 방법론을 제공합니다.

- **Performance Highlights**: 합성 (synthetic) 및 실제 (real-world) 데이터 세트를 통해 우리의 접근 방식을 검증하고 시연하였습니다. ABI의 확장은 mixture 모델을 추정하는 데 있어 매우 매력적인 프레임워크로 입증되었습니다. 본 연구의 결과는 Bayesian mixture 모델링의 새로운 가능성을 열어줄 것으로 기대됩니다.
...
- **What's New**: 이 논문에서는 Sample-efficient Probabilistic Detection using Extreme Value Theory (SPADE)라는 새로운 방법을 제안합니다. 이 방법은 분류기를 기권한(classifier into an abstaining classifier) 분류기로 변환하여 OOD(out-of-distribution) 및 적대적 샘플에 대해 확실한 보호를 제공합니다. GEV(Generalized Extreme Value) 모델을 기반으로 하여 학습된 분포를 형식적으로 특성화할 수 있게 도와줍니다.

- **Technical Details**: SPADE 방법은 Extreme Value Theory (EVT)를 사용하여 학습된 모델의 잠재 공간에서 샘플 간 거리의 분포를 모델링합니다. 논문에서는 GEV 모델이 OOD 및 적대적 샘플을 형식적으로 식별할 수 있는 방법을 제시하며, 적당한 가정 하에 높은 확률로 적대적 샘플을 거부할 수 있음을 입증합니다. 이 방식은 다양한 신경망 아키텍처(ResNet, VGG, Vision Transformer)와 중대형 데이터셋(CIFAR-10, CIFAR-100, ImageNet)에서 실험적으로 검증되었습니다.

- **Performance Highlights**: 실험 결과 SPAD는 기존의 방법에 비해 경제성(fragility), 안정성(stability), 효율성(efficiency) 측면에서 우수한 성능을 보여주었습니다. 각기 다른 아키텍처의 학습 모델과 강력한 기준선 대비 SPADE의 효과성을 성공적으로 증명하였습니다. 이 접근법은 OOD 감지의 정확성을 높이는 중요한 기여를 할 수 있을 것으로 기대됩니다.



### Contributions to the Decision Theoretic Foundations of Machine Learning and Robust Statistics under Weakly Structured Information (https://arxiv.org/abs/2501.10195)
Comments:
          Habilitation Thesis

- **What's New**: 이번 연구는 여러 공동 저자와 함께 진행된 연구 결과를 종합적으로 정리한 것입니다. 특히, 하빌리테이션(habilitation) 과정의 일환으로 발표된 10편의 논문이 핵심적으로 포함되어 있으며, 이들 논문은 서로의 연구 프로젝트를 연결하는 구조를 가지고 있습니다. 연구자는 독자들이 각 프로젝트의 내용을 쉽게 이해하고 접근할 수 있도록 간단히 설명합니다.

- **Technical Details**: 연구의 첫 번째 부분인 A에서는 결정 이론(decision theory)의 기초와 관련된 여러 허가 연구를 다루고 있습니다. B 부분에서는 약하게 구조화된 정보(weakly structured information) 하에서의 머신 러닝(machine learning) 알고리즘 비교와 관련된 논의가 포함되어 있습니다. C 부분에서는 비표준 측정 척도(non-standard scales of measurement) 하에서의 강건 통계(robut statistics)에 대한 연구가 진행됩니다.

- **Performance Highlights**: 각 논문에서 제시된 연구 결과들은 통계적 비교와 다양한 기법들의 효율성을 강조하고 있습니다. 머신 러닝 알고리즘의 성능을 정량적으로 평가하는 방법론이 제시되며, 향후 연구 방향에 대한 통찰을 제공합니다. 연구자들은 이 연구가 보다 넓은 청중에게 유용할 것이며, 결정 이론 및 머신 러닝 분야에서의 기여를 기대하고 있습니다.



### Surrogate-based multiscale analysis of experiments on thermoplastic composites under off-axis loading (https://arxiv.org/abs/2501.10193)
Comments:
          21 pages. 31 figures

- **What's New**: 이번 논문에서는 유도 방향 열가소성 복합재료에서 비축 방향 하중 하의 일정 변형률 및 크리프(Creep) 실험을 모델링하기 위한 대리 모델 기반(multiscale) 접근 방식을 제안합니다. 이전 연구에서는 단일 스케일 미세역학적 시뮬레이션을 통해 실험을 모델링했지만, 낮은 비축 각도에서 실험 결과와의 유의미한 차이를 보였습니다. 본 연구에서는 이러한 불일치가 거시적 비균일성(macroscopic inhomogeneity)으로 발생한다고 가정하고 다중 스케일 접근이 필요하다고 주장합니다.

- **Technical Details**: 논문에서 제안된 대리 모델은 Physically Recurrent Neural Network (PRNN)으로, 데이터 기반의 구성 요소와 내재적 법칙적 모델을 결합하여 이력 의존적 행동(history-dependent behavior)을 자연스럽게 포착합니다. 이러한 모델은 컴퓨터 계산 비용을 줄이고 효율성을 높이는 방법으로, 전체 필드(multiscale) 시뮬레이션을 대체할 수 있는 가능성을 지니고 있습니다. 또한 이 네트워크의 잠재적 공간(latent space)에 대한 설명 가능성(explainability) 또한 탐구되어, 재훈련 없이도 전이 학습(transfer learning) 전략을 사용할 수 있음을 보여줍니다.

- **Performance Highlights**: 대리 모델 기반 다중 스케일 접근을 통해 거시적 변형 필드의 비균일성에 대한 가설을 확인하였으며, 경사지 끝판(oblique end-tabs)의 실험 설정 조정이 미치는 영향에 대한 통찰을 얻었습니다. 다양한 설정에서 대리 모델 기반의 시뮬레이션 결과는 단일 스케일 미세역학적 접근 방식보다 실험과 더 나은 일치를 보였으며, 크리프 실험에 대해서는 제한된 정확도를 보였습니다. 그러나 물질 특성 보정(material properties calibration) 단계에서 거시적 테스트 영향을 암시적으로 고려함으로써 향상된 이해를 제공합니다.



### Improved learning rates in multi-unit uniform price auctions (https://arxiv.org/abs/2501.10181)
Comments:
          NeurIPS 2024

- **What's New**: 본 논문은 전력 생산자가 전력 당일 시장에서 전략적으로 참여하는 문제를 연구하며, 다수의 유사 가격 경매에서의 온라인 학습 문제를 다룹니다. 새로운 입찰 공간 모델을 제시하며, 특정 기법을 통해 개선된 후회율(regret rate) $	ilde{O}(K^{4/3}T^{2/3})$를 달성함을 입증했습니다. 또한, 모든 당첨 입찰이 공개되는 다양한 피드백 모델을 소개하여, 그에 따른 새로운 후회율 $	ilde{O}(K^{5/2}	ext{sqrt}(T))$을 도출했습니다.

- **Technical Details**: 이 연구는 다수의 유사 가격 경매에서의 반복 입찰 문제에 관련된 정보를 활용해 의사결정자가 최적의 입찰 전략을 학습할 수 있는 환경을 조성합니다. 논문에서는 다수의 입찰자가 참여하는 상황을 구성하고, 각 참가자가 제출하는 입찰 프로필과 해당 가격 결정 규칙(예: Last Accepted Bid, First Rejected Bid 등)을 명시했습니다. Uniform pricing 설정에서의 알고리즘 성능도 엄밀히 분석하여 그 경과를 수치적으로 밝히고 있습니다.

- **Performance Highlights**: 제안된 알고리즘은 다양한 피드백 상황에 따라 크게 향상된 후회율을 달성합니다. 특히, Uniform pricing 하에서 $	ilde{O}(K^{2/3}T^{2/3})$의 후회율을 논의하며, 이는 기존의 문헌에서 제시된 후회율보다 개선된 결과입니다. 이 연구는 전력 시장에서 의사결정자들이 나가야 할 효율적인 방향을 제시하고, 반복 입찰에서의 전략 실행 가능성을 보여줍니다.
...
### Convex Physics Informed Neural Networks for the Monge-Amp\`ere Optimal Transport Problem (https://arxiv.org/abs/2501.10162)
Comments:
          17 pages, 14 figures. Submitted to Engineering Computations on 26 September 2024

- **What's New**: 이 연구는 최적 운송 문제(optimal transport problem)를 해결하기 위해 Physics Informed Neural Networks (PINNs)와 Input Convex Neural Networks (ICNNs)를 활용하는 새로운 접근법을 제안합니다. 이 모델은 Monge-Ampère 방정식을 해결하기 위해 연속적인 방법을 사용하며, 테포지션 경계 조건(transport boundary conditions)을 손실 함수(loss function)에 포함시켜 적용합니다. 논문에서 제시된 방법은 여러 구성에서 최적 운송 문제를 해결하는데 중요한 진전을 보여줍니다.

- **Technical Details**: 제안된 모델은 두 개의 유계(open)이고 매끄러운(smooth) 집합에 대한 확률측도(probability measures)에 기반하고 있습니다. 이 연구에서는 Monge-Ampère 방정식의 변형을 통해 최적 측량 맵(optimal transport map)을 근사하는 것이 중요하다는 것을 강조합니다. 논문에서는 ICNN을 사용하여 수치 방법(numerical method)의 근사가 항상 볼록(convex)하도록 보장하는 방법 또한 설명합니다.

- **Performance Highlights**: 수치 실험(numerical experiments)을 통해 제안된 방법의 유효성이 입증되었으며, 입력 볼록 PINNs의 중요성이 강조되었습니다. 결과적으로, 이 연구는 고전적인 Monge-Ampère 방정식을 해결하는 데 있어 PINNs의 활용 가능성을 보여줍니다. 최적 운송 문제의 대한 감도 분석(sensitivity analyses) 또한 수행되어, 제안된 접근 방법의 강점을 부각합니다.



### Visual Exploration of Stopword Probabilities in Topic Models (https://arxiv.org/abs/2501.10137)
- **What's New**: 이 논문은 corpus-specific한 stopword 추출을 위한 새로운 방법을 제안합니다. 이 방법은 각 단어가 stopword일 확률을 추정하고, 사용자가 이를 분석할 수 있도록 지원하는 대화형 시각화 시스템을 포함합니다. 이를 통해 사용자는 stopword 목록을 보다 신뢰성 있게 분석하는 데 도움을 받을 수 있습니다.

- **Technical Details**: 제안된 시스템은 Gaussian processes classification (GPC) 모델을 기반으로 한 동적 확률 추정 방법을 사용합니다. 이 방법을 통해 사용자는 모델에서 단어가 stopword일 가능성을 예측할 수 있으며, 각기 다른 확률 수준에서 stopword 제거를 목표로 하고 있습니다. 또, 사용자 인터페이스는 stopword 분석을 위한 시각적 툴을 제공하여, 사용자가 선택한 확률 임계값의 영향을 실시간으로 확인할 수 있도록 설계되었습니다.

- **Performance Highlights**: 사용자 연구 결과, 제안된 시스템은 사용자가 topic 모델의 신뢰성을 높이는 데 기여하는 것으로 나타났습니다. 특히, 합리적인 확률 반환, 일반적인 stopword 목록 확장 및 stopword 시각적 분석을 위한 조정 가능한 임계값을 제공합니다. 이 연구는 향후 기계 학습 및 주제 모델링 과정에서 실무자에게 유용한 인사이트와 권장 사항을 제공하는 데 목표를 두고 있습니다.
...
### Double descent in quantum machine learning (https://arxiv.org/abs/2501.10077)
- **What's New**: 이 연구는 양자 기계 학습(Quantum Machine Learning, QML)에서 발생하는 이중 하강 현상(double descent phenomenon)을 분석하고 증명합니다. 전통적인 통계 학습 이론은 모델의 복잡성이 증가할수록 일반화 성능이 저하된다고 주장하는데, 이 연구는 이러한 현상이 양자 모델에서도 나타날 수 있음을 보여줍니다. 특히, 양자 커널 방법(Quantum Kernel Methods)을 사용하여 다양한 실제 데이터셋에 대한 숫자 실험을 통해 테스트 오류 최댓값을 확인했습니다. 이러한 결과는 양자 모델이 과적합(overfitting) 없이 현대의 과매개변수화(overparameterized) 영역에서 작동할 수 있음을 시사합니다.

- **Technical Details**: 이 연구는 양자 모형이 Hilbert 공간에서 선형적으로 작용함을 활용하여 테스트 오류에 대한 식을 도출했습니다. 랜덤 행렬 이론(random matrix theory)을 통해 이론적으로 증명했으며, 양자 모델이 데이터 분포에 대한 합리적인 가정을 만족할 때, 과매개변수화로의 전환 지점에서 테스트 오류가 최고점을 찍는다고 합니다. 이 연구는 특히 양자 특성 맵(quantum feature maps)과 관련된 다양한 데이터셋에서 수행된 수치 실험을 통해 이론적 결과를 확인하고 있습니다.

- **Performance Highlights**: 양자 모델에서 이중 하강 현상이 존재한다는 최초의 이론적 및 경험적 관찰로, 이는 모델 성능과 일반화에 대한 추가적인 탐구의 기반을 제공합니다. 특히 대규모의 고차원 데이터에서 양자 기계 학습이 전통적인 기계 학습에 비해 경쟁력을 가질 수 있는 가능성을 보여줍니다. 이 연구는 향후 QML이 실제 응용 프로그램에서 성공적인 도약을 이루기 위한 중요한 첫걸음입니다.
...
### Tracking student skills real-time through a continuous-variable dynamic Bayesian network (https://arxiv.org/abs/2501.10050)
- **What's New**: 이 논문은 Performance Distribution Tracing (PDT)라는 새로운 방법을 제안합니다. PDT는 성공률(success rate)의 분포를 실시간으로 추적하는 방식으로, Dynamic Bayesian Network를 사용하여 연속적인 확률 변수를 노드로 둡니다. 이를 통해 성공률 추정의 정확도에 대한 정보와, 유사한 기술들 간의 데이터를 결합하여 더 정확한 추정치를 도출할 수 있습니다.

- **Technical Details**: PDT는 각 성공률의 분포를 추적하여, 실시간으로 데이터를 업데이트하는 기능을 갖추고 있습니다. 베타 분포(beta distribution)를 공약수(prior)로 활용하여 모든 분포를 분석적인 형태로 유지할 수 있으며, 새로운 관측치가 발생할 때 신속하게 온라인 업데이트를 가능하게 합니다. 수업이나 연습 문제에서 여러 가지 기술을 필요로 하는 경우에도, PDT는 관련 기술의 데이터 활용을 통해 유용한 추정치를 제공합니다.

- **Performance Highlights**: PDT를 활용한 실험 결과, 사용자는 제공된 추천에 만족감을 느끼며, 이 추천 사항을 수용할 수 있었습니다. PDT는 학교나 교육 현장에서의 기술 수준 추적을 자동화하여, 방대한 양의 데이터에서 신뢰할 수 있는 추정을 제공하며, 교육에서의 개인화된 접근법과 더불어 시간과 자원에 대한 효율성을 동시에 증가시킵니다.
...
### Statistical Inference for Sequential Feature Selection after Domain Adaptation (https://arxiv.org/abs/2501.09933)
- **What's New**: 본 연구는 SeqFS 후 도메인 적응(Domain Adaptation, DA)의 결과를 평가하기 위한 새로운 통계 방법, SI-SeqFS-DA(statistical inference for SeqFS-DA)를 제안합니다. 이 방법은 비교적 높은 차원의 회귀 분석에서 피처 선택(feature selection)에서 신뢰성을 확보할 수 있으며, 거짓 긍정률(false positive rate, FPR)을 유의 수준 아래에서 적절히 제어할 수 있는 장점이 있습니다. 또한, 이를 통해 특정 모델 선택 기준을 포함한 다양한 확장을 제공합니다.

- **Technical Details**: 연구에서는 SeqFS의 두 가지 주요 유형인 Forward Selection과 Backward Elimination을 기반으로 하는 방법론을 제시합니다. 특히, FPR을 제어하기 위한 전략적 접근법을 도입하고, AIC(Akaike Information Criterion), BIC(Bayesian Information Criterion), 조정된 R² 등 모델 선택 기준에 대한 연장도 함께 논의합니다. 이러한 방법들은 데이터 부족 문제를 해결하고, SeqFS가 실제 문제에서 높은 성능을 발휘할 수 있도록 합니다.

- **Performance Highlights**: 다양한 합성 및 실제 데이터 세트에 대한 광범위한 실험을 통해 제안된 SI-SeqFS 방법의 이론적 결과를 검증하였습니다. 이 실험에서 제시된 방법은 기존의 피처 선택 방법들과 비교하여 우수한 성능을 보여주며, 특히 의료 또는 감염병 진단과 같은 고위험 영역에서도 신뢰성 있는 결정 지원을 가능하게 합니다. 구현 코드는 GitHub에서 제공되며, 연구의 활용 가능성을 더욱 높여줍니다.
...
### SBAMDT: Bayesian Additive Decision Trees with Adaptive Soft Semi-multivariate Split Rules (https://arxiv.org/abs/2501.09900)
- **What's New**: 이 논문은 새로운 확률적 가법 결정 트리 모델을 소개합니다. 기존의 BART(Bayesian Additive Regression Trees)와 같은 결정 트리 모델은 단일 변수(feature) 기반의 결정 규칙에 의존하여 복잡한 결정 경계를 포착하는 데 한계가 있었습니다. 제안된 모델은 소프트 분할 규칙(soft split rule)을 통해 높은 유연성을 가지며, 다변량(multi-variate) 특성을 잘 활용할 수 있습니다.

- **Technical Details**: 이 연구에서 다룹니다는 비모수(non-parametric) 회귀 문제에서 구조화된(multivariate structured) 및 비구조적(unstructured) 특성을 갖는 입력(feature) 데이터에 대한 것입니다. 제안된 모델은 결정 노드에서 동적으로 변화하는 확률적 분할 규칙(probabilistic split rule)을 적용하여 회귀 함수의 매끄러움 수준에 따라 적응합니다. 예를 들어, 구조화된 특성은 도로 네트워크나 도시의 지리적 복잡성을 포함할 수 있습니다.

- **Performance Highlights**: 논문에서는 제안된 모델의 유용성을 합성 데이터셋(synthetic datasets)과 뉴욕시 교육 데이터셋(New York City education dataset)과의 비교를 통해 입증합니다. 기존의 트리 기반 모델들과 비교할 때, 새로운 모델은 예측 성능이 뛰어나고 불확실성을 정량화하는 능력이 뛰어납니다. 이 연구는 부동산, 공공 보건, 사회 과학 및 환경 연구와 같은 분야에서의 광범위한 응용 프로그램을 염두에 두고 있습니다.
...
- **What's New**: 이번 연구에서는 Contrastive Language-Audio Pretraining (CLAP) 모델을 섬유광학(acoustic recognition domain)에서 활용하기 위한 새로운 방법론인 CLAP-S를 제안합니다. CLAP-S는 Support Set을 활용해 implicit(암묵적) 지식과 explicit(명시적) 지식을 결합하여 cross-domain generalization(크로스 도메인 일반화)을 효과적으로 구현합니다. 실험 결과, 제안된 방법이 실험실에서 수집된 ESC-50 데이터셋 및 실제 환경에서 수집된 총격-불꽃놀이 데이터셋 모두에서 경쟁력 있는 성능을 보인다고 밝혔습니다.

- **Technical Details**: 본 연구는 섬유광학 기반 음향 인식 분야에서 CLAP 모델을 적응시키기 위해 여러 모델 적응 방법을 평가합니다. CLAP 모델을 학습하기 위한 N-class K-shot 샘플을 사용하여 fine-tuning을 진행하며, 지원세트는 새로운 지식을 저장하는 역할을 수행합니다. 모델의 추론 과정에서는 normalized audio embedding을 쿼리로 사용하고, 이와 관련된 audio embedding들을 키로 사용하여 cross-attention 메커니즘을 적용합니다.

- **Performance Highlights**: 제안된 CLAP-S 방법은 기존의 다른 방법들과 비교할 때 Labeled data의 활용을 극대화하면서 성능 향상을 보여줍니다. 모든 실험에서 제안된 방법이 기존의 prompt tuning, Tip-Adapter, Treff와 같은 메소드에 비해 현저히 개선된 성능을 기록했습니다. 이 연구는 섬유광학 음향 인식뿐만 아니라 다른 음향 인식 작업에도 유용한 통찰력을 제공합니다.



### Geometry-Preserving Encoder/Decoder in Latent Generative Models (https://arxiv.org/abs/2501.09876)
Comments:
          41 pages

- **What's New**: 이 논문은 새로운 encoder/decoder 구조를 도입하여 데이터 분포의 기하학적 구조를 보존함으로써 기존 변분 오토인코더(VAE)와는 다른 이론적 특성을 갖도록 설계된 내용을 다룹니다. 이 구조의 혁신은 모델 학습 과정에서 중요한 이점을 제공합니다. 특히, 이 geometry-preserving encoder는 인코더와 디코더의 훈련 과정에서 효율성을 높이는 데 기여합니다.

- **Technical Details**: 기술적으로, 이 연구는 고차원 입력 공간 대신 잠재 공간에서 확산 모델(difussion models)을 해결하는 접근 방식을 다룹니다. 모델이 정해진 경로를 따라 수렴하도록 보장하는 이론적 결과를 제시하며, 인코더 훈련의 수렴 보장 및 기하학적 구조를 보존하는 인코더를 사용할 때 디코더 훈련의 빠른 수렴 결과를 보여줍니다.

- **Performance Highlights**: 성능 측면에서, 제안된 기하학적 구조를 보존하는 인코더는 기존의 VAE 기반 모델에 비해 훈련 속도를 향상시키고, 더 나은 생성 성능을 제공합니다. 이 연구는 데이터 분포와의 관계를 명확히 하여 기하학적으로 의미 있는 샘플 생성을 가능하게 합니다. 이를 통해 향후 생성 모델 연구에서 중요한 기초 자료로 활용될 수 있을 것입니다.
...
### Boosting the Accuracy of Stock Market Prediction via Multi-Layer Hybrid MTL Structur (https://arxiv.org/abs/2501.09760)
- **What's New**: 이 논문에서는 주식 시장 예측의 비선형적이고 고차원적인 특성을 보다 효율적으로 해결하기 위해 새로운 다층 하이브리드 다중 태스크 학습(MTL) 프레임워크를 제안합니다. 이 모델은 다양한 입력 특징 간의 복잡한 상관관계를 추출하기 위해 Transformer 인코더를 사용하며, 장기적인 시계열 관계를 포착하기 위해 양방향 게이트 순환 유닛(BiGRU)을 활용합니다. 또한, Kolmogorov-Arnold 네트워크(KAN)를 통해 학습 과정을 향상시킵니다.

- **Technical Details**: 제안된 프레임워크는 다층 구조를 통해 서로 다른 채널 데이터 간의 결합 관계를 동시에 추출하고, 시계열에서 강한 상관관계를 포착하여 모든 예측 작업의 공동 최적화를 달성할 수 있습니다. Transformer 인코더는 다양한 입력 특징 간의 복잡한 상관관계를 추출하고, BiGRU는 장기적인 시계열 관계를 인식하며, KAN은 비선형 함수 근사화를 개선합니다. 이로 인해 MAE, MAPE, R^2 평가 지표에서 우수한 성능을 보여줍니다.

- **Performance Highlights**: 실험 결과, 제안한 학습 구조는 MAE가 1.078, MAPE가 0.012, R^2가 0.98로 매우 낮은 오차를 보이며, 다른 경쟁 네트워크와 비교했을 때 높은 예측 성능을 달성했습니다. 이러한 성능은 전통적인 방법들과 비교할 때 머신 러닝 기술의 우수성을 강조하며, 주식 시장 예측에서의 실질적인 적용 가능성을 제시합니다.

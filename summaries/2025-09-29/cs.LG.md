...
### Transport Based Mean Flows for Generative Modeling (https://arxiv.org/abs/2509.22592)
- **What's New**: 본 논문에서는 Optimal Transport MeanFlow (OT-MF) 방법을 제안하여, 기존의 Mean Flow 접근 방식을 개선합니다. OT-MF는 최적 수송 기반 샘플링 전략을 포함하여, 다단계 흐름 프로세스의 충실도를 유지하면서 더 효율적인 1단계 생성기를 가능하게 합니다. 이 방법은 점 군 생성과 이미지 변환 실험에서 해당 정확성을 유의미하게 향상시킴을 보였습니다.

- **Technical Details**: OT-MF는 최적 수송의 원칙을 MeanFlow의 시간 평균화 공식과 통합하여, 직선적인 생성 경로를 형성합니다. 이 새로운 기법은 OT 기반의 결합과 평균 흐름 감독을 통해 기하학적으로 인식하고 효율적인 1단계 생성 모델링을 가능하게 합니다. 또한, 점군 생성 및 이미지 변환 실험에서 MeanFlow의 1단계 생성 능력을 유지하면서도 정확성을 크게 향상시킵니다.

- **Performance Highlights**: OT-MF는 기존의 1단계 생성 방식을 유지하면서도 정확도를 높여줍니다. 실험 결과, OT-MF를 적용한 모델이 점군 생성 및 이미지 변환 태스크에서 더 우수한 성능을 보였으며, 훈련 효율성을 높이기 위해 가속화된 OT 솔버를 사용하여 계산 비용을 감소시킵니다. 이러한 결과는 OT-MF가 향후 고차원 데이터 생성에 매우 효과적일 수 있음을 시사합니다.



### The Lie of the Average: How Class Incremental Learning Evaluation Deceives You? (https://arxiv.org/abs/2509.22580)
- **What's New**: 이 논문에서는 Class Incremental Learning (CIL)의 평가 방법론에서 발생하는 문제를 다루고 있습니다. CIL은 새로운 클래스를 지속적으로 학습하면서 이전에 학습한 클래스를 기억해야 하는 과제입니다. 그러나 기존의 평가 프로토콜은 랜덤으로 선택된 몇 개의 클래스 순서에서 평균 및 분산을 계산하는 방식으로, 전체 성능 분포를 포착하지 못하고 있다는 점을 지적합니다. 연구 결과 이 방법은 성능의 편향된 추정을 초래하고 신뢰할 수 있는 평가의 필요성을 강조하고 있습니다.

- **Technical Details**: 제안된 EDGE(Extreme case-based Distribution & Generalization Evaluation) 평가 프로토콜은 inter-task similarity에 기반하여 극단적인 클래스 시퀀스를 식별하고 샘플링하는 방법을 사용합니다. 이 과정에서 클래스 수준의 텍스트 설명을 이용해 유사도 행렬을 작성하고, 서로 다르지만 대표적인 세 개의 클래스 시퀀스를 생성하여 모델의 성능을 평가하게 됩니다. 이를 통해 더욱 공정하고 신뢰할 수 있는 성능 분포 추정을 목표로 합니다. 각 클래스 간의 유사성이 모델 성능에 미치는 영향을 분석하여, CIL의 평가를 보다 정밀하게 수행할 수 있습니다.

- **Performance Highlights**: 논문에서 제안한 EDGE 프로토콜은 기존의 랜덤 샘플링 접근 방식보다 훨씬 정밀한 성능 분포 추정을 가능하게 합니다. 실험 결과, EDGE는 성능의 극단적인 경우를 효과적으로 포착해내며, 정확한 분포 경계를 추정할 수 있음을 보여주고 있습니다. 이러한 성과는 모델 선택 및 강건성 검증에 유의미한 인사이트를 제공하며, CIL의 미래 설계에 중요한 기초 자료를 구성합니다.
...
### Machine learning approaches to seismic event classification in the Ostrava region (https://arxiv.org/abs/2509.22574)
Comments:
          10 pages, 5 figures

- **What's New**: 이번 연구는 체코 공화국의 북동부 지역을 연구 대상으로 하여, 지진 유발 원인인 광산 활동과 자연 지진(tectonic events)을 머신러닝(machine learning) 기법을 통해 구분하는 방법을 제안합니다. 특히 1992년부터 2002년까지의 기록된 지진 사건들을 포함하는 Frenštát Seismic Polygon (SPF) 데이터셋을 이용해, 이 데이터에 대한 분류 기법의 효과를 검증합니다. 최신 머신러닝 기법으로 Long Short-Term Memory (LSTM) 및 XGBoost 모델을 적용해 높은 정확도를 기록했습니다.

- **Technical Details**: 연구에서는 SPI 데이터셋을 기반으로 LSTM과 XGBoost 모델을 활용하여 지진 사건을 ‘광산 유발 지진(mining-induced seismic events)’과 ‘자연 지진(natural tectonic seismic events)’으로 구분하는 이진 분류(binary classification)의 성능을 평가했습니다. 1992-2002년 사이에 기록된 17,384개의 지진 사건이 분석되었으며, 이는 대부분 광산으로 인해 발생한 사건입니다. 머신러닝 모델은 F1-score 0.94~0.95를 기록하며 높은 분류 정확도를 보여주었습니다.

- **Performance Highlights**: LSTM과 XGBoost 모델은 지진 사건의 신속한 분류를 가능하게 했습니다. 특히, 자연 지진과 광산 유발 지진 간의 신속한 차별화는 향후 지진 위험 평가 및 모니터링에 중요한 기여를 할 것입니다. 연구는 체코 지역의 지진 활동에 대한 기초적인 데이터를 제공하며, 지진 관측의 효율성과 정확성을 향상시키는 데 도움이 될 것으로 기대됩니다.
...
### Learning to Price Bundles: A GCN Approach for Mixed Bundling (https://arxiv.org/abs/2509.22557)
- **What's New**: 이번 연구에서는 번들 가격 설정 문제에 대해 Graph Convolutional Networks (GCNs)을 활용하는 새로운 접근법을 제안합니다. 일반적으로 번들 가격 설정 문제는 후보 번들의 수가 기하급수적으로 증가하여 계산적으로 비효율적이지만, 본 연구는 GCN을 통해 최적의 번들을 학습하여 이 문제를 해결합니다. 또한 FCP(고정 컷오프 프루닝)와 PCP(진행 컷오프 프루닝) 전략을 제안하여 고품질의 해결책을 제공합니다.

- **Technical Details**: 논문은 두 층의 GCN을 설계하여 고객 세그먼트에 대한 번들 매칭을 학습하고 최적 번들에 포함될 확률을 예측합니다. 이 예측을 바탕으로, 특정 확률 기준을 만족하는 번들만을 선택하는 두 가지 전략을 개발해 후보 솔루션을 생성합니다. 또한, Hanson & Martin(1990)의 혼합 정수 선형 프로그래밍(MILP) 공식을 적용하여 최적 가격을 산출하며, 로컬 서치 방법을 통해 대안적인 유망한 번들을 탐색하여 솔루션 품질을 개선합니다.

- **Performance Highlights**: 실험 결과, 본 접근법은 5개 제품의 경우 최적의 수익의 97% 이상을 달성하며, 소요 시간이 기존 방법의 일부에 불과함을 보여줍니다. 중규모 문제(15-25개 제품)에서도 기존 방법을 초과하는 성과를 보이며, 대규모 문제(30개 이상의 제품)에서도 효율적인 솔루션을 제공합니다. 이는 비가역적인 유틸리티를 가진 경우에서도 내구성이 뛰어난 최초의 확장 가능 접근법임을 입증합니다.



### ECHO: Toward Contextual Seq2Seq Paradigms in Large EEG Models (https://arxiv.org/abs/2509.22556)
- **What's New**: 이번 연구에서는 ECHO라는 새로운 디코더 중심의 LEM(framework) 패러다임을 도입하여 EEG 모델링을 시퀀스-투-시퀀스 학습(sequence-to-sequence learning)으로 재구성했습니다. 이 접근 방식은 시퀀스 공간에서 신호, 레이블 및 작업 간의 중첩 관계를 포착하며, 이산적 지원 샘플을 포함하여 맥락 단서를 구성합니다. ECHO는 기존의 LEM들이 가지고 있던 디코더 용량의 제약 문제를 해결하여, 다양한 작업에 동적으로 적응할 수 있도록 설계되었습니다.

- **Technical Details**: ECHO는 다양한 EEG 데이터셋을 활용하여 N개의 샘플을 가진 inputs와 각각의 데이터셋에 따라 달라지는 레이블을 포함하는 구조화된 시퀀스를 구성합니다. 이 paradigm은 고급 모델 구성 요소를 사용하지 않고도 기존 LEM의 성능을 크게 향상시킬 수 있는 방법론을 제시합니다. 표적 EEG 샘플과 지원 EEG 인스턴스의 시퀀스에서 다음 토큰 예측(next-token prediction)을 통해 작동하여, 레이블 및 작업 토큰을 통합한 출력을 생성합니다.

- **Performance Highlights**: ECHO는 여러 데이터셋에 걸쳐 광범위한 실험을 통해 기존의 최신 싱글 작업 LEM(모델)들을 일관되게 초월하는 성능을 보여주었습니다. ECHO는 구체적인 프롬프트 없이도 특정 패러다임을 식별할 수 있으며, 혁신적인 지원 샘플(컨텍스트)을 통해 새로운 작업 및 환경에 적응하는 능력도 입증되었습니다. 이러한 결과는 ECHO가 복잡한 시나리오 모델링에 대한 중요한 역할을 하며, 기존 LEM의 잠재력을 최대한 끌어낼 수 있는 통찰을 제공함을 나타냅니다.
...
### Dual Optimistic Ascent (PI Control) is the Augmented Lagrangian Method in Disguis (https://arxiv.org/abs/2509.22500)
- **What's New**: 이 논문은 제약 최적화(constrained optimization)가 신경망에 요구 사항을 적용하는 강력한 프레임워크임을 강조합니다. 저자들은 기존의 여러 접근 방식 간의 새로운 동등성을 제시하여, dual optimistic ascent가 Augmented Lagrangian에서의 gradient descent-ascent와 동일하다는 것을 밝혔습니다. 이러한 발견은 dual optimistic 방법이 모든 로컬 솔루션에 선형적으로 수렴하는 것을 보장할 수 있는 이론적 근거를 제공합니다.

- **Technical Details**: 주요 기술 세부 사항으로는, Augmented Lagrangian 방법의 gradient와 Hessian에 대한 계산을 포함합니다. 이들은 stationary point가 특정 조건을 만족해야 함을 명시하며, strict complementary slackness를 따르는 KKT 포인트에 대해 논의합니다. 이러한 수학적 장치는 최적화 문제의 성질을 심층적으로 이해하는 데 도움을 줍니다.

- **Performance Highlights**: 성능 하이라이트로는 dual optimistic ascent 메서드가 이론적인 보장을 갖도록 만들어 임의의 제약 조건 하에 잘 작동한다는 점을 들 수 있습니다. 이 방법은 실제로 높은 성능을 발휘하면서도 이론적으로 안정성을 확보했습니다. 이러한 결과는 제약 최적화의 두 가지 접근 방식이 교환 가능하다는 사실을 명확히 함으로써, 향후 연구에 많은 가능성을 제시합니다.
...
### Bayesian Transfer Operators in Reproducing Kernel Hilbert Spaces (https://arxiv.org/abs/2509.22482)
- **What's New**: 이 논문에서는 비선형 동적 시스템을 선형적으로 표현하는 Koopman 연산자와 데이터 과학에서 인기 있는 개념인 재생 커널 힐베르트 공간(reproducing kernel Hilbert spaces, RKHS)을 결합하는 새로운 접근 방식을 제시합니다. 특히 Gaussian process(GP) 방법을 통해 기존의 커널 기반 Koopman 알고리즘의 두 가지 문제를 해결하고자 합니다. 이를 통해 계산 요구량을 줄이고 센서 노이즈에 대한 저항력을 향상시킬 수 있습니다.

- **Technical Details**: 이 연구는 Gaussian process 회귀와 동적 모드 분해(dynamic mode decomposition, DMD)의 통합을 포함합니다. DMD는 주성분 분석(principal component analysis, PCA) 및 이산 푸리에 변환(discrete-time Fourier transform)과 결합된 갤레르킨 방법으로 설명되며, 비선형 시스템을 효과적으로 모델링하는 데 사용됩니다. 저자들은 GP 회귀의 이점을 DMD에 적용하여 데이터로부터 모델을 구축하는 방법을 제안하며, 불확실성 추정도 제공합니다.

- **Performance Highlights**: 이 알고리즘은 비선형 동적 시스템의 노이즈가 있는 측정 데이터로부터 모델을 얻는 데 효과적이며, 다단계 예측에 대한 불확실성 추정치를 제공합니다. 기존의 EDMD(Extended Dynamic Mode Decomposition)와 비교할 때, 더욱 유연하고 저렴한 계산 비용으로 모델을 학습할 수 있습니다. 실험 결과는 이 방법이 향상된 성능 보장과 우수한 통계적 일관성을 제공함을 보여줍니다.
...
- **What's New**: 본 논문에서는 트레이닝된 신경망을 최적화하기 위한 축소 공간(formulation) 접근법을 제시합니다. 본 접근법은 신경망의 출력과 도함수를 GPU에서 평가하며, 최적화 문제를 해결하는 과정에서 중간 변수와 제약 조건을 감춥니다. 기존의 전체 공간 접근법에 비해 이 방법은 더 빠른 해결 시간과 적은 반복횟수를 제공합니다.

- **Technical Details**: 비선형 최적화에서 코스트 함수는 두 번 연속 미분 가능해야 하며, 이는 IPOPT와 같은 내부 점 방법을 통해 해결됩니다. 신경망 예측기는 반복적인 아핀 변환과 비선형 활성화 기능을 포함하여 정의되며, 최적화 문제에 통합될 수 있습니다. 이 논문에서 제안된 축소 공간 접근법은 대규모 신경망을 고려하면서도 GPU 가속을 통해 계산 효율을 높이는 방법론입니다.

- **Performance Highlights**: 두 가지 테스트 문제인 MNIST 이미지에 대한 적대적 생성과 신경망 제약 조건이 있는 안전 제약 최적 전력 흐름(SCOPF) 문제에서 이 방법의 장점을 보여주었습니다. 이러한 문제들은 복잡한 비선형 제약 조건을 포함하고 있으며, 제안된 방법을 통해 더 효율적으로 해결될 수 있음을 확인했습니다. 이 연구는 대규모 신경망을 포함한 복잡한 최적화 문제에서 GPU 가속의 유용성을 입증했습니다.
...
### Overclocking Electrostatic Generative Models (https://arxiv.org/abs/2509.22454)
- **What's New**: 최근 전기적 생성 모델인 PFGM++는 이미지 합성에서 최첨단 성능을 달성하는 강력한 프레임워크로 자리 잡았습니다. PFGM++는 보조 차원 $D$를 가진 확장된 데이터 공간에서 작동하며, 특정한 한계에서 확산 모델 프레임워크를 복원합니다. 그러나 이 모델은 샘플 생성을 위해 비싼 ODE 시뮬레이션에 의존하여 계산 비용이 높습니다. 이에 우리는 모든 값의 $D$에 대한 전기적 생성 모델을 가속화하는 새로운 증류 프레임워크인 Inverse Poisson Flow Matching (IPFM)을 제안합니다.

- **Technical Details**: IPFM은 전기적 필드가 선생 모델의 필드와 일치하는 생성기를 학습하는 역문제( inverse problem)로서 증류를 재정립합니다. 우리는 이 문제에 대해 적용 가능한 트랙터블(training objective)을 도출하고, $D 	o \\infty$의 한계에서 IPFM이 최근의 확산 모델 증류 방법인 Score Identity Distillation (SiD)와 밀접한 연관이 있음을 보입니다. IPFM은 손수 기능 평가를 통해 선생 모델에 필적하거나 이를 초월하는 샘플 품질을 달성하는 증류 생성기를 생산합니다.

- **Performance Highlights**: IPFM을 사용하여 얻은 증류 생성기는 임계 차원(critical dimension) $D$에서 이전보다 높은 수렴 속도를 보여줍니다. 이는 유한-$D$ 모델들이 더 나은 최적화 및 샘플링 특성을 나타낸다는 이전의 연구 결과와 일치합니다. 이러한 발견은 전기적 생성 모델이 뛰어난 생성 성능과 함께 경쟁력 있는 연구 방향이 될 가능성을 보여줍니다.
...
### The Flood Complex: Large-Scale Persistent Homology on Millions of Points (https://arxiv.org/abs/2509.22432)
- **What's New**: 이 논문에서는 대규모 유클리드 포인트 클라우드 데이터에 대한 persistent homology (PH) 계산 문제를 다룹니다. 기존의 Vietoris-Rips 복합체는 계산 제한으로 인해 효과적인 사용이 어려운 반면, Alpha 복합체나 희소 Rips 근사치는 여전히 많은 수의 단순체(simplices)를 생성하는 문제를 가지고 있습니다. 이러한 문제를 해결하기 위해 Flood 복합체를 도입하였습니다. 이 복합체는 작은 포인트 클라우드 서브셋으로부터 생성된 Delaunay 삼각형화에서 일정 반경 내의 단순체를 포함하게 됩니다.

- **Technical Details**: Flood 복합체는 $r \geq 0$의 필터링 값에서 X의 일정 subset을 통해 Delaunay 삼각형화된 단순체들을 포함합니다. 이 과정은 "flooding"이라고 불리우며, radius $r$의 구가 X로부터 발산하여 단순체를 완전히 커버할 수 있도록 합니다. 이러한 구조는 PH 계산을 효율적으로 만들며, 여러 가지 이론적 특성을 포함하고 있습니다. 또한, GPU 병렬화에 적합해, 대규모 데이터 처리에서 강력한 성능을 발휘합니다.

- **Performance Highlights**: 3D 포인트 클라우드 데이터에서 수백만 개의 점에 대해 최대 차원 2의 PH를 계산할 수 있음을 보여줍니다. 실제 및 합성 데이터에 대한 객체 분류 성능 평가에서, Flood 복합체의 스케일링 기능이 필요하다는 것을 입증하였습니다. 특히, 기하학적 또는 위상적으로 복잡한 객체일 경우 다른 PH 기반 방법이나 신경망보다 뛰어난 성능을 발휘하는 것으로 나타났습니다.



### Learning from Delayed Feedback in Games via Extra Prediction (https://arxiv.org/abs/2509.22426)
Comments:
          11 pages, 3 figures (main); 9 pages (appendix)

- **What's New**: 이번 연구는 게임에서 학습의 시간 지연 피드백 문제를 다룹니다. 여러 에이전트가 독립적으로 전략을 학습하는 과정에서 발생하는 최적화 불일치 문제를 해결하기 위해 미래 보상을 예측하는 알고리즘인 Optimistic Follow-the-Regularized-Leader (OFTRL)를 제안합니다. 연구는 OFTRL의 성능이 단일 단계 지연에 의해 악화됨을 입증했습니다.

- **Technical Details**: 논문에서는 시간 지연 문제를 해결하기 위해 weighted OFTRL (WOFTRL) 알고리즘을 제안합니다. 이 알고리즘은 OFTRL의 미래 보상 예측 벡터를 n배로 가중치하여 지연을 상쇄합니다. 이 결과, WOFTRL은 일반 합 게임에서 O(1) 회귀를 유지하고, 폴리 매트릭스 제로섬 게임에서 Nash 균형으로 수렴하는 것을 입증했습니다.

- **Performance Highlights**: WOFTRL 알고리즘은 실험을 통해 이론적 결과를 뒷받침하며 강력한 성능을 보였습니다. 특히, 낙관적 가중치가 시간 지연을 초과할 경우, O(1) 낙인을 달성하고 전략이 Nash 균형으로 수렴하였습니다. 따라서 이 연구는 다중 에이전트 학습에서 시간 지연 피드백의 중요성을 강조합니다.
...
### One Prompt Fits All: Universal Graph Adaptation for Pretrained Models (https://arxiv.org/abs/2509.22416)
Comments:
          accepted by NeurIPS 2025 main conference

- **What's New**: 본 논문에서는 Graph Prompt Learning (GPL)의 효과와 원리를 탐구하고, 기존의 GPL 방법론의 두 가지 주요 한계를 지적하고 있습니다. 첫 번째는 프롬프트가 사전 훈련 모델과 상호작용하는 메커니즘에 대한 합의 부족이며, 두 번째는 다양한 하류 시나리오에 대한 적응성의 제한입니다. 이를 해결하기 위해 저자들은 UniPrompt라는 새로운 방법론을 제안하여 사전 훈련 모델의 능력을 끌어내고자 합니다.

- **Technical Details**: UniPrompt는 입력 그래프의 구조를 보존하면서 학습 가능한 프롬프트 그래프를 활용하여 사전 훈련 모델에 적응하도록 설계되었습니다. 이 접근 방식은 프롬프트 그래프와 원본 그래프 토폴로지를 통합하여 모델의 붕괴 및 과적합을 방지하는 부트스트랩 전략을 도입합니다. 이러한 기술적인 혁신을 통해 저자들은 프롬프트 메커니즘을 통해 모델이 더 나은 성능을 발휘할 수 있도록 하는 방법을 제시합니다.

- **Performance Highlights**: 실험 결과는 UniPrompt가 여러 사전 훈련 모델에 대해 효과적으로 통합될 수 있으며, in-domain 및 cross-domain 시나리오에서 강력한 성과를 이루는 것을 보여줍니다. 저자들은 동질적 및 이질적 데이터셋에서의 실험을 통해 기존의 GPL 기준선들을 일관되게 초월하는 성과를 기록했습니다. 이는 UniPrompt가 다른 GPL 방법론과 비교했을 때 현저한 성능 개선을 이룬다는 것을 의미합니다.



### Fast-Forward Lattice Boltzmann: Learning Kinetic Behaviour with Physics-Informed Neural Operators (https://arxiv.org/abs/2509.22411)
- **What's New**: 이 논문에서는 격자 볼츠만 방정식(LBE)을 위한 물리 기반 신경 연산자(framework)의 새로운 접근법을 제안합니다. 이 접근법은 단계별 통합 없이 긴 시간 범위에서 예측할 수 있게 해 주며, 충돌 커널(collison kernel)을 명시적으로 해결할 필요성을 효과적으로 우회합니다. 이를 통해 복잡한 유체 역학적 현상을 더 쉽게 모델링할 수 있는 가능성을 제시합니다.

- **Technical Details**: 제안된 물리 기반 신경 연산자(PINO) 프레임워크는 격자 볼츠만 방정식에 특화되어 있으며, 각 입자의 분포 함수(single-particle distribution functions)를 빠르게 예측합니다. 이 모델은 지역 모멘트 일치(local moment-matching constraints)와 전체 분포 함수 필드의 글로벌 대칭(global equivariance)을 활용하여 물리적 일관성을 유지합니다. 주로 U자형 신경 연산자(U-shaped Neural Operator, UNO) 아키텍처를 사용하여 메모리 효율성을 높였습니다.

- **Performance Highlights**: 실험 결과, 제안된 모델은 복잡한 유체 흐름 시나리오에서 강력한 성능을 보여 주었습니다. 연구에서는 von Karman 와류 생성을 비롯하여 리가먼트 분해, 기포 부착 등의 다양한 현상을 모방하였습니다. 이 데이터 기반 접근 방식은 다양한 동역학에 적용 가능한 잠재력을 보여 주며, 기존의 LBE 솔루션에서 발생하는 시간적 제약을 극복할 수 있는 새로운 길을 제시합니다.



### MoveFM-R: Advancing Mobility Foundation Models via Language-driven Semantic Reasoning (https://arxiv.org/abs/2509.22403)
- **What's New**: Mobility Foundation Models (MFMs)는 인간의 이동 패턴을 모델링하는 데 큰 발전을 이루었지만 데이터 규모와 의미적 이해의 한계로 인해 제약을 받고 있습니다. 새로운 프레임워크인 MoveFM-R은 언어 기반의 의미적 추론 기능을 활용하여 MFMs의 잠재력을 완전히 발휘할 수 있도록 설계되었습니다. 이 프레임워크는 지리적 좌표와 언어 토큰 간의 어휘 불일치를 해소하고, MFMs의 잠재 벡터와 LLM의 의미적 세계 간의 표현 공백을 줄이고자 합니다.

- **Technical Details**: MoveFM-R은 세 가지 핵심 혁신을 바탕으로 구축됩니다. 첫째, 지리-언어 간의 격차를 좁히기 위해 의미적으로 향상된 위치 인코딩을 도입합니다. 둘째, LLM의 추론을 이동 패턴과 조정하기 위한 점진적 커리큘럼을 사용합니다. 마지막으로, 조건부 경로 생성을 위한 상호작용적 자기 반성 메커니즘을 도입하여 다양한 시나리오에서의 플로백을 보장합니다.

- **Performance Highlights**: MoveFM-R은 기존의 MFM 및 LLM 기반 모델에 비해 현저한 성능 향상을 보여주며, 제로샷(Zero-shot) 설정에서도 강력한 일반화를 보여줍니다. 이 프레임워크는 자연어 지침으로부터 현실적인 경로를 생성하는 데 탁월한 능력을 발휘합니다. MoveFM-R은 MFMs의 통계적 힘과 LLM의 깊은 의미적 이해를 결합하여 인간 이동 모델링의 새로운 패러다임을 선도하고 있습니다.



### ReLAM: Learning Anticipation Model for Rewarding Visual Robotic Manipulation (https://arxiv.org/abs/2509.22402)
- **What's New**: 이번 연구에서는 로봇 조작을 위한 시각적 강화 학습에서 보상 설계의 문제를 해결하기 위한 새로운 방법을 제안합니다. 기존의 방식에서 벗어나, 이미지를 통해 추출한 키포인트를 바탕으로 공간적 거리를 암시적으로 유추합니다. 새로운 프레임워크인 Reward Learning with Anticipation Model (ReLAM)을 도입하여, 행동이 없는 비디오 시연으로부터 밀집하고 구조화된 보상을 자동으로 생성합니다.

- **Technical Details**: ReLAM은 최적 경로에 대한 중간 키포인트 기반의 하위 목표를 제안하는 계획 모델로서 작동합니다. 이 모델은 목표 지향 정책을 훈련시키기 위한 연속 보상 신호를 제공합니다. 또한, 주요 입력으로는 현재 RGB 관찰, 키포인트의 현재 좌표, 그리고 예측된 키포인트의 목표 좌표가 포함되며, 세 층의 CNN과 MLP를 결합한 구조를 사용하여 포인트 표현을 생성합니다.

- **Performance Highlights**: 복잡하고 긴 작업을 수행하는 실험에서 ReLAM의 성능은 기존의 최첨단 방법들에 비해 현저하게 향상되었습니다. 실험 결과, ReLAM이 학습 속도를 크게 향상시킬 수 있음을 보여줍니다. HRL(계층 강화 학습) 프레임워크 내에서 provable sub-optimality bound를 유지하면서도 효율적인 목표 도달이 가능하다는 점에서 중요한 의미를 가집니다.



### Improving accuracy in short mortality rate series: Exploring Multi-step Forecasting Approaches in Hybrid Systems (https://arxiv.org/abs/2509.22395)
- **What's New**: 이 연구는 보험 및 연금 시장에서의 정확한 사망률 예측의 중요성을 강조하며, 적절한 다단계 예측 방법론과 머신러닝(Machine Learning) 모델 선택이 성능 향상에 미치는 영향을 평가합니다. 12개의 데이터셋과 21개 모델을 사용하여, 재귀적 접근법을 사용하는 ARIMA-LSTM 하이브리드 모델이 다른 모델에 비해 통상적으로 더 높은 예측 정확도를 보여주었습니다.

- **Technical Details**: 하이브리드 시스템은 통계 모델과 머신러닝 모델을 결합하여 선형 및 비선형 패턴을 처리할 수 있도록 설계되었습니다. 연구에서는 재귀적, 직접적, 다중 입력 다중 출력(Multi-Input Multi-Output, MIMO) 방식의 세 가지 다단계 예측 접근법을 평가하였고, 잔차 예측을 위해 다층 퍼셉트론(Multilayer Perceptron, MLP), 장기 단기 메모리(Long Short-Term Memory, LSTM), N-BEATS 모델을 사용했습니다.

- **Performance Highlights**: 결과적으로, 다단계 재귀적 접근법이 사망률 예측을 위한 잔차 모델링에 있어 더 나은 정확도를 보여주었으며, LSTM 모델이 가장 적합한 것으로 평가되었습니다. 하이브리드 시스템인 ARIMA+LSTM은 단일 머신러닝, 통계 및 다변량 모델에 비해 평균 절대 백분율 오차(Mean Absolute Percentage Error, MAPE) 측면에서 우수한 예측 성능을 나타내었습니다.
...
### (Sometimes) Less is More: Mitigating the Complexity of Rule-based Representation for Interpretable Classification (https://arxiv.org/abs/2509.22384)
Comments:
          Presented at IJCNN 2025

- **What's New**: 이 논문에서는 Multi-layer Logical Perceptron (MLLP) 네트워크에 L0 정규화를 적용하여 해석 가능성을 개선하고 모델 복잡성을 줄이는 방법을 제안합니다. 고전적 모델인 Concept Rule Set (CRS)와 비교하여 성능 유지 여부를 평가합니다. 노이즈가 적은 방식으로 네트워크를 희소화하여 복잡성과 성능 간의 균형을 찾고자 합니다.

- **Technical Details**: L0 정규화는 일반적인 정규화 방법 이상으로 희소성을 높이는 데 도움을 주며, 이 논문에서는 그 미분 근사 방법을 적용하여 MLLP 프레임워크와 결합합니다. 네트워크의 손실 함수에 정규화 항을 포함시켜 네트워크의 희소성을 촉진하고, 훈련된 연속 모델을 해석 가능한 이산 모델과 정렬하는 효과를 분석합니다. 실험을 통해 MLLP와 L0 정규화가 통합된 모델의 성능과 복잡성을 비교합니다.

- **Performance Highlights**: 기존 연구 결과를 재현하여 우리의 접근법의 기본 성능 기준을 설정하였으며, L0 정규화를 사용하여 달성한 희소성을 분석했습니다. 비교 분석 후, L0 정규화가 MLLP의 복잡성과 성능에 긍정적인 영향을 미친다는 것을 보여줍니다. 이와 같은 성과는 설계되어 있는 접근법이 모델 해석 가능성 강화와 성능 유지에 효과적임을 입증합니다.



### Enhancing Credit Risk Prediction: A Meta-Learning Framework Integrating Baseline Models, LASSO, and ECOC for Superior Accuracy (https://arxiv.org/abs/2509.22381)
Comments:
          36 pages

- **What's New**: 본 연구에서는 금융 의사결정에 중요한 신용 위험 관리(credit risk management)를 위한 포괄적 메타 학습(meta-learning) 프레임워크를 제안합니다. 이 프레임워크는 XGBoost, Random Forest, Support Vector Machine 및 Decision Tree와 같은 감독 학습(supervised learning) 알고리즘과 K-Nearest Neighbors와 같은 비감독 방법(unsupervised methods), 다층 퍼셉트론(Multilayer Perceptron)과 같은 딥러닝(deep learning) 아키텍처를 통합합니다.

- **Technical Details**: LASSO 정규화(LASSO regularization)를 통한 특징 선택(feature selection)과 차원 축소(dimensionality reduction), 그리고 불균형 다중 클래스 문제(multi-class imbalance problems)를 처리하기 위한 오류 수정 출력 코드(Error-Correcting Output Codes)를 메타 분류기로 사용합니다. 또한 모델 투명성을 높이기 위해 모든 모델에서 예측 클래스별로 Permutation Feature Importance 분석을 구현하였습니다.

- **Performance Highlights**: 미국 상장기업 2,029개의 신용 등급이 포함된 Corporate Credit Ratings 데이터셋을 통해 접근 방식을 실증적으로 검증하였으며, 신용 등급 이행(upgrades and downgrades) 및 디폴트 확률(default probability) 추정과 관련된 금융 엔티티(classification)에 대한 정확도를 크게 향상시킨 것으로 나타났습니다.



### Role-Aware Multi-modal federated learning system for detecting phishing webpages (https://arxiv.org/abs/2509.22369)
Comments:
          22 pages, 9 figures

- **What's New**: 본 논문에서는 URL, HTML, 이미지 입력을 지원하는 연합 다중 모달 피싱 웹사이트 탐지기(federated, multi-modal phishing website detector)를 제안합니다. 클라이언트는 고정된 모달리티에 얽매이지 않고 다른 곳에서 훈련된 모든 모달리티 헤드를 호출할 수 있습니다. 이는 사용자의 유연성을 높이며 다양한 데이터 형식을 활용할 수 있는 새로운 접근 방식을 제공합니다.

- **Technical Details**: 방법론적으로, 우리는 Mixture-of-Experts와 FedMM에서 영감을 받아 FedProx 위에 역할 인식(bucket aggregation) 집계를 제안합니다. 여기서 우리는 학습 가능한 라우팅을 버리고 샘플 모달리티에 따라 IMAGE/HTML/URL 전문가를 선택하는 하드 게이팅(hard gating)을 사용합니다. 이는 모달리티별 파라미터의 분리된 집계를 가능하게 하여 교차 임베딩(conflicts) 문제를 격리하고 수렴 안정성을 높입니다.

- **Performance Highlights**: TR-OP 데이터세트에서 Fusion head는 97.5%의 정확도(Acc)와 2.4%의 잘못된 긍정 비율(FPR)을 달성했습니다. 이미지 세트의 경우, 95.5%의 정확도와 5.9%의 잘못된 긍정 비율을 기록했습니다. WebPhish(HTML)에서 정확도 96.5% / 1.8% FPR, TR-OP(원시 HTML)에서 95.1% / 4.6% FPR을 얻어, 하드 게이팅 전문의 bucket aggregation이 엄격한 프라이버시 하에서도 안정적인 연합 훈련을 가능하게 함을 보여줍니다.



### Investigating Faithfulness in Large Audio Language Models (https://arxiv.org/abs/2509.22363)
- **What's New**: 본 논문은 대형 오디오-언어 모델(large audio-language models, LALMs)의 체인-오브-생각(chain-of-thought, CoT) 설명의 신뢰성을 조사합니다. 이전 연구에서는 텍스트 기반의 LLM들이 CoT를 생성할 때 종종 신뢰성이 결여된다고 보고했습니다. 본 연구에서는 LALMs에서 CoT가 얼마나 신뢰성을 가지는지를 평가하기 위해 다양한 개입(interventions) 방법을 적용했습니다.

- **Technical Details**: 연구에 사용된 개입 방법에는 패러프레이징(paraphrasing), 필러 토큰 인젝션(filler token injection), 조기 답변(early answering), 그리고 실수 추가(addition of mistakes)가 포함됩니다. 이러한 방법을 통해 모델의 CoT가 최종 예측에 미치는 영향을 분석하였습니다. 실험 데이터셋으로는 SAKURA와 MMAR을 사용하였으며, 각 개입이 CoT에 미치는 효과를 검토했습니다.

- **Performance Highlights**: 실험 결과, LALMs는 각 작업과 데이터셋에 대해 일반적으로 신뢰할 수 있는 CoT를 생성하는 것으로 나타났습니다. CoT의 변경율에 따른 모델의 일관성(consistency)을 측정하였고, 변경된 CoT에 대해서도 결과의 일관성이 유지되었습니다. 이는 LALMs가 생성한 CoT가 모델의 실제 결정 과정을 잘 반영하고 있음을 시사합니다.



### Neural Feature Geometry Evolves as Discrete Ricci Flow (https://arxiv.org/abs/2509.22362)
Comments:
          38 pages, 14 figures

- **What's New**: 이번 연구에서는 딥 뉴럴 네트워크의 피처 표현이 어떻게 진화하는지를 기하학적인 관점에서 조사합니다. 특히, 데이터의 입력 매니폴드(potential manifold)가 관찰되지 않을 때 기하학적 그래프(geometric graphs)를 사용하여 지역 유사성(local similarity) 구조를 근사화하는 방법을 제안합니다. 이론적으로, 깊은 선형 네트워크는 피처 기하학을 보존하는 반면, ReLU와 같은 비선형 활성화는 기하학적 변형을 가능하게 한다는 사실을 증명했습니다.

- **Technical Details**: 연구에서는 페달링(data propagation) 동안 기하학적 그래프의 진화(evolution)에 대한 이론적 결과를 제공합니다. Ricci 곡률(Ricci curvature) 및 연관된 Ricci 흐름(Ricci flow) 개념을 사용하여, 이러한 기하학적 변형이 어떻게 딥 뉴럴 네트워크의 데이터 매니폴드 Geometry를 단순화하고 부드럽게 하는지를 설명합니다. 네트워크의 여러 층에서 곡률 기반 변형(curvature-driven transformations)을 분석함으로써, 깊이 선택에 관한 새로운 기하학적 기준을 제안합니다.

- **Performance Highlights**: 20,000개 이상의 피드포워드 neural networks를 사용한 실험을 통해, 클래스 분리가 이루어지는 과정이 그래프 표현에서의 커뮤니티 구조의 출현과 밀접하게 연관되어 있음을 관찰했습니다. 이 연구 결과는 효율적인 훈련 및 하이퍼파라미터(hyperparameter) 선택에 대한 실용적인 도구를 제공하며, 특히 기하학적으로 정보된 조기 중단(early-stopping) 휴리스틱을 도입했습니다.
...
### Distributed Associative Memory via Online Convex Optimization (https://arxiv.org/abs/2509.22321)
- **What's New**: 본 논문에서는 분산 연상 기억(distributed associative memory, DAM)에 대해 연구하고 있습니다. 여러 에이전트가 로컬 연상 기억을 유지하며 자체적인 연상과 다른 에이전트의 선택적 정보를 회상할 수 있는 방법을 제시합니다. 이를 위해 트리 기반의 온라인 경량 하강법(online gradient descent, OGD)을 활용한 DAM-TOGD 알고리즘을 도입하였습니다.

- **Technical Details**: 팀의 연구는 각각의 에이전트가 연상 기억 메커니즘을 온라인으로 최적화하는 방법을 수립합니다. 구체적으로, 각 에이전트는 자신이 이전에 처리한 데이터와 다른 에이전트의 데이터를 회상할 수 있도록 요구됩니다. 연구자는 에이전트 간 통신을 위한 무향 그래프(undirected graph)를 설정하고, 이를 통해 에이전트들이 상호작용하며 정보를 교환할 수 있게 합니다.

- **Performance Highlights**: 실험 결과, 제안된 DAM-TOGD 프로토콜은 기존의 온라인 최적화 기반 대비 일관되게 우수한 성능을 보였습니다. 이 알고리즘은 이론적으로도 서브선형 회귀(sublinear regret) 보장을 제공합니다. 이러한 성과는 분산 환경에서의 연상 기억 최적화가 가능함을 시사합니다.
...
### SoDaDE: Solvent Data-Driven Embeddings with Small Transformer Models (https://arxiv.org/abs/2509.22302)
Comments:
...
- **What's New**: 이번 연구에서는 새로운 용매 표현 방식인 SoDaDE(Solvent Data Driven Embeddings)를 제안하여 수소화 카테콜(Catechol) 벤치마크에서 더 나은 성능을 발휘했습니다. 기존의 용매 표현 방식이 직면한 문제를 해결하기 위해 작은 데이터셋에서 학습된 의미 있는 ‘fingerprint’를 개발했습니다. 특히, SoDaDE는 데이터가 제한된 상황에서도 좋은 성과를 낼 수 있음을 시사합니다.

- **Technical Details**: SoDaDE는 소형 transformer 모델을 사용하여 191가지 용매의 물리적 속성 데이터를 기반으로 ‘fingerprint’를 생성합니다. 본 연구에서는 자가 지도 학습(self-supervised learning) 기법을 통해 모델을 학습시켰으며, 사전 훈련(pre-training) 과정에서 다양한 속성 값 조합을 섞어 데이터 증대(data augmentation)를 수행했습니다. 모델의 구조는 64의 차원 크기와 16개의 주의(attention) 헤드를 가지고 있으며, 5개의 레이어로 구성되었습니다.

- **Performance Highlights**: SoDaDE의 성능은 기존의 Gaussian process(GP) 모델 및 Random Forest(RF) 모델과 비교하여 우수함을 입증했습니다. 연구 결과, SoDaDE는 카테콜 벤치마크의 다양한 반응 조건 하에서 예측 성능이 개선되었으며, 이를 통해 화학 산업에서 친환경 용매를 찾기 위한 기초 데이터를 제공할 수 있을 것으로 기대됩니다.
...
- **What's New**: 이번 논문에서는 다중모드 입력을 지원하고 제로샷 추론(zero-shot inference)을 수행할 수 있는 Aurora라는 다중모드 시계열 기초 모델을 소개합니다. Aurora는 Cross-domain Multimodal Time Series Corpus에서 사전 훈련(pretrained)되어, 텍스트 또는 이미지 모드에서 도메인별 지식을 효과적으로 추출하고 집중할 수 있는 능력을 갖추었습니다. 이러한 방식으로, Aurora는 강력한 크로스 도메인 일반화 기능을 제공합니다.

- **Technical Details**: Aurora는 토큰화(tokenization), 인코딩(encoding), 증류(distillation)를 통해 다중모드 도메인 지식을 추출하고, 이를 모델링 과정에서 시간적 표현에 주입하기 위해 Modality-Guided Multi-head Self-Attention 메커니즘을 사용합니다. 디코딩(decoding) 단계에서는 다중모드 표현을 활용하여 미래 토큰의 조건과 프로토타입을 생성하는 새로운 Prototype-Guided Flow Matching 방식을 적용합니다. 이로 인해 Aurora는 생성적 확률 추정(generative probabilistic forecasting)에 기여합니다.

- **Performance Highlights**: 종합적인 실험 결과, TimeMMD, TSFM-Bench 및 ProbTS를 포함한 여러 검증된 벤치마크에서 Aurora는 시계열 예측의 단일 모드 및 다중 모드 시나리오 모두에서 일관되게 최첨단 성능을 달성했습니다. Aurora는 기존 모델들과 비교하여 우수한 제로샷 추론(zero-shot inference) 능력을 보여주었으며, 복잡한 상황에서 하나의 출구 없는 강력한 예측 도구 역할을 할 수 있습니다.



### A Multi-Level Framework for Multi-Objective Hypergraph Partitioning: Combining Minimum Spanning Tree and Proximal Gradien (https://arxiv.org/abs/2509.22294)
- **What's New**: 이번 논문은 새로운 다목적 비볼록 제약 완화 모델에 기반한 효율적인 하이퍼그래프 분할 프레임워크를 제안합니다. 이를 통해 지역 최적에 빠지지 않고 분할 품질을 향상시키기 위해 수정된 가속형 전진 경량 알고리즘을 사용합니다. 또한, 다양한 데이터 규모에 맞춘 MST 기반의 두 가지 전략이 설계되었습니다.

- **Technical Details**: 하이퍼그래프는 두 개 이상의 정점을 연결할 수 있는 하이퍼엣지를 지닌 그래프의 일반화된 형태로, 이 논문에서는 정점 집합을 k개의 서로 다를 분할로 나누는 문제를 다룹니다. 복잡한 시나리오에서도 높은 성능을 유지할 수 있도록, 기존의 어떻게 것을 집중하는 이론을 기반으로 수정된 전략을 통해 개선된 분할 결과를 도출합니다. 특히, greedy migration, swapping, 재귀적 MST 기반 클러스터링과 같은 정제 전략을 통해 분할 결과를 보다 향상시킵니다.

- **Performance Highlights**: 실험 결과, 제안된 알고리즘은 KaHyPar에 비해 2%, 3%, 4-way 분할에서 평균적으로 2%-5%의 절단 크기 감소 효과를 보였으며, 특정 인스턴스에서는 35%까지 개선되었습니다. 또한 가중치 정점 집합에 대한 성능이 탁월하여 최첨단 분할기들을 능가하는 결과를 나타냈습니다. 더불어, 제안된 정제 전략은 hMetis의 분할 품질을 최대 16%까지 향상시켰습니다.



### Conditional Denoising Diffusion Autoencoders for Wireless Semantic Communications (https://arxiv.org/abs/2509.22282)
- **What's New**: 이 논문에서는 무선 의미 통신(SemCom) 시스템을 위한 확산 오토인코더 모델을 제안하고 있습니다. 이 모델은 의미 공간과 실제 데이터의 확률 분포 간의 매핑을 학습하는 데 중점을 두고 있으며, 기존의 오토인코더 기반 아키텍처의 한계를 극복하고자 합니다. 사용자는 채널 적응형 인코딩 및 디코딩에 중점을 두었던 기존 프레임워크의 제약을 벗어나 더 나은 확장성을 제공받게 됩니다.

- **Technical Details**: 제안된 모델은 의미 송신기에서 고수준의 의미를 추출하는 신경 인코더와, 신호 공간 디노이징(signal-space denoising)을 위해 원천 데이터 분포를 활용하는 조건부 확산 모델(Conditional Diffusion Model, CDiff)로 구성됩니다. CDiff는 수신된 의미 잠재 값(semaphore latent)을 조건 입력으로 사용하여 디코딩 과정을 송신자가 의도한 의미로 유도합니다. 이러한 기술은 다양한 채널 대역폭 비율(Channel Bandwidth Ratios, CBR)에서도 작동하며, 엔드 투 엔드(E2E) 연합 훈련 없이도 다양한 신경 인코더에 적응할 수 있습니다.

- **Performance Highlights**: CIFAR-10 및 MNIST 데이터셋을 포함한 광범위한 시뮬레이션을 통해, 기존의 오토인코더 및 변형 오토인코더(Variational Autoencoders, VAE)와 비교했을 때, 55%의 개선된 인식 이미지 패치 유사도(Learned Perceptual Image Patch Similarity, LPIPS) 및 30%의 향상된 구조적 유사도 지수(Structural Similarity Index Measure, SSIM)를 보여줍니다. 또한, VAE 벤치마크와 비교하여 LPIPS에서 50% 개선을 보였습니다. 이러한 성능 향상은 다음 세대의 무선 시스템에 중요한 기여를 할 것입니다.



### Unlocking the Power of Mixture-of-Experts for Task-Aware Time Series Analytics (https://arxiv.org/abs/2509.22279)
- **What's New**: 이번 연구에서는 PatchMoE라는 새로운 MoE 기반의 시계열 분석 프레임워크를 제안합니다. 이 프레임워크는 다양한 작업에 대한 '지식' 활용을 지원하여, 작업 인식(task-aware) 능력을 갖추고 있습니다. 기존 Mixture-of-Experts (MoE) 구조의 한계를 극복하고자, 계층적 정보를 라우팅(routing) 과정에서 효과적으로 활용할 수 있는 방법을 모색하였습니다.

- **Technical Details**: PatchMoE의 핵심 구성 요소로는 Recurrent Noisy Gating(RNG-Router)이 있으며, 이는 계층별 표현(representation)의 차이를 인식하여 계층 조건부 확률 분포를 모델링합니다. 이러한 라우팅 전략은 시간적 및 채널 차원에서 시간 시퀀스 토큰을 동시에 라우팅하고, 정교하게 설계된 Temporal & Channel Load Balancing Loss를 통해 복잡한 시간적 및 채널 상관관계를 모델링합니다. 이는 시계열 분석에서 MoE의 활용을 최적화하는 데 기여합니다.

- **Performance Highlights**: 다섯 가지 다운스트림 작업에 대한 포괄적인 실험을 통해 PatchMoE는 최신 성능을 자랑하고 있습니다. 특히 예측, 이상 탐지, 결측 데이터 보정, 분류 작업에서 뛰어난 성능을 입증했습니다. 이 연구는 다양한 시계열 분석 작업을 지원하는 새로운 벤치마크를 제시하며, MoE 아키텍처의 잠재력을 극대화하는데 기여하고 있습니다.



### Fine-Grained Uncertainty Decomposition in Large Language Models: A Spectral Approach (https://arxiv.org/abs/2509.22272)
- **What's New**: 이번 논문에서는 Large Language Models (LLMs)에서의 예측 불확실성을 정량화하고 분해하기 위한 새로운 방법, Spectral Uncertainty를 제안합니다. 이 접근법은 Von Neumann entropy(폰 네이만 엔트로피)를 활용하여 전체 불확실성을 aleatoric(우연적) 및 epistemic(인지적) 구성 요소로 분리하는 데 필요한 이론적 기초를 제공합니다. 기존 방법론의 한계를 개선하고 다양한 의미적 해석을 미세하게 구별할 수 있는 레이어를 포함하여 LLM의 예측 불확실성을 더 잘 이해할 수 있게 합니다.

- **Technical Details**: Spectral Uncertainty 프레임워크는 functional Bregman information을 기반으로 하여 aleatoric 및 epistemic 구성 요소로의 커널 기반 분해를 가능하게 합니다. 이 방법론은 지속적인 의미적 공간에서 불확실성을 측정하기 위해 두 단계 샘플링 및 임베딩 프로세스를 거쳐 커널 함수를 통한 스펙트럴 분석을 수행합니다. 이를 통해 모델 출력에 대한 임베딩 기반 표현을 실질적으로 활용하여 다양한 LLM 시나리오에서 적용할 수 있는 불확실성 추정기를 제안합니다.

- **Performance Highlights**: 실험 결과, Spectral Uncertainty는 다양한 모델 및 벤치마크 데이터셋에서 aleatoric 및 총 불확실성 측정에서 최첨단 성능을 달성했습니다. 이는 모호성 탐지 및 정확성 예측 작업에서도 강력한 의미 기반 및 분해 기반 기법을 능가하는 결과를 보여주었습니다. 이러한 성능은 LLM 예측의 신뢰성과 해석 가능성을 향상시킬 잠재력을 강조합니다.
...
- **What's New**: 이 논문은 베어링 결함 진단을 위한 데이터 누수(data leakage)의 문제를 조명하고, 이로 인해 발생하는 모델 평가의 편향을 해결하는 방법을 제안합니다. 특히, 전통적인 데이터 분할 방식이 성능 지표를 과장하는 경향이 있음을 밝혀냅니다. 저자들은 조건별 및 세그먼트별로 데이터 세트를 분할하는 기존 방식을 비판하고, 베어링 단위로 데이터를 파티셔닝하는 새로운 평가 방법론을 제시하여 실제 상황에서의 신뢰성을 높이기 위해 노력하였습니다.

- **Technical Details**: 제안된 새로운 방법론은 여러 종류의 결함을 동시 검출할 수 있는 다중 레이블(multi-label) 분류 문제로 재설계되었으며, 물리적 구성 요소 간의 겹침을 방지하는 방식으로 데이터 세트를 분할합니다. 이를 통해 전통적인 방법이 가질 수 있는 유사성 편향(sSimilarity Bias)을 감소시키고, 각 베어링의 다양성이 모델 일반화에 미치는 영향을 분석하여 실제 산업 환경에서의 성능을 더 신뢰할 수 있도록 만듭니다. 저자들은 CWRU, Paderborn, Ottawa 대학의 세 가지 데이터 세트를 통해 방법론의 효과를 입증하고 있습니다.

- **Performance Highlights**: 이 연구는 모델의 성능을 과장하는 데이터 누수의 영향을 명확히 보여주며, 특히 베어링 데이터 세트를 사용할 때 신뢰할 수 있는 평가 프로토콜의 필요성을 강조합니다. 또한, 다중 레이블 문제로 접근함으로써 더 정밀하고 실용적인 평가 방법을 제시하고, 다양한 베어링에서 훈련한 모델의 일반화 성능의 차이를 검증하였습니다. 이러한 연구는 더욱 견고하고 신뢰할 수 있는 기계 학습 시스템의 발전에 기여할 것으로 기대됩니다.
...
- **What's New**: 이 논문에서는 기존의 unlearning 방식들이 효과적으로 타겟 지식을 삭제하는 데 실패하고, 대신 이를 숨기기 위해 새로운 spurious unlearning neurons를 생성한다는 문제를 제기합니다. 이에 대한 해결책으로 새로운 unlearning 방법인 Ssiuu를 제안하여, spurious negative influence를 방지하고 타겟 지식을 보다 정확하게 제거하는 접근 방식을 사용합니다. 실험 결과, Ssiuu는 두 가지 상황에서 강력한 기준선보다 우수한 성능을 보이며, 타겟 지식을 신뢰성 있게 삭제할 수 있음을 입증하였습니다.

- **Technical Details**: 일반적으로, unlearning 작업은 특정 지식을 언어 모델의 매개변수에서 제거하면서 다른 지식은 유지하는 것을 목표로 하며, 이는 주어진 입력에 대해 출력을 생성하지 않도록 모델을 훈련시키고자 합니다. 그러나 기존의 연구들은 unlearning 과정에서 shallow alignment 문제가 발생하여, 타겟 지식이 완전히 제거되지 않고 외부로부터의 재훈련에 취약한 모습을 보입니다. 특히 spurious unlearning neurons의 출현이 이 문제의 주요 원인으로 지적되며, 이는 새로운 지식을 습득하여 타겟 지식을 억제하게 됩니다.

- **Performance Highlights**: Ssiuu 방법은 정밀한 평가와 실험을 통해 타겟 지식을 효과적으로 제거하며, 두 개의 실제 공격 시나리오에서 우수한 성능을 발휘했습니다. 기존의 unlearning 기법들이 상대적으로 저조한 결과를 보였던 반면, Ssiuu는 재훈련 중 목표 지식의 회복을 방지하면서 안정성을 유지하는 데 성공했습니다. 이러한 결과는 언어 모델의 안전한 배포를 위해 신뢰할 수 있는 unlearning 방법의 필요성을 강조합니다.
...
- **What's New**: 이번 연구에서는 대규모 딥러닝 모델에서 memorization(기억)과 데이터 복원(data reconstruction)의 관계를 새로운 관점에서 분석합니다. 특히 모델의 파라미터 수 $p$가 훈련 샘플 수 $n$에 비례하는 게 아니라, 데이터 차원 $d$에 기반하여 $p > dn$일 때 전체 훈련 데이터셋을 복원할 수 있음을 밝혔습니다. 이 연구는 다양한 아키텍처에서 잘 작동하는 최적화 방법을 제안하며, 대규모 모델이 개인정보 보호와 저작권 침해와 관련된 문제를 일으킬 수 있음을 강조합니다.

- **Technical Details**: 우리는 랜덤 피처 모델(random features model)을 기준으로, 훈련 샘플이 피처 공간(feature space) 내에서 어떤 서브스페이스(subspace)를 형성하는지 보여줍니다. 연구 결과에 따르면 $p 	ext{ (파라미터 수)} >> dn 	ext{ (샘플 수)}$인 경우 주어진 서브스페이스에서 모든 훈련 데이터를 복원할 수 있게 됩니다. 이를 통해, 모델 파라미터를 통해 데이터셋을 복원할 수 있는 최적화 알고리즘을 제안하고, CIFAR-10 데이터셋을 이용해 이를 입증합니다.

- **Performance Highlights**: 제안한 방법은 다양한 아키텍처에 대해 안정적으로 성능을 보이며, $p = 10dn$의 경우 전체 데이터셋을 성공적으로 복원함을 보여줍니다. 이 실험에 따르면, 훈련 손실(training loss)은 $p=n$에서 0으로 수렴하지만, 복원 손실(reconstruction error)은 $p$가 $dn$의 차원에 도달해야 감소하기 시작합니다. 결과적으로, 연구는 adversarial robustness와 데이터 복원 능력 간의 연관성을 밝혀내어 향후 연구에 중요한 기반을 제공합니다.
...
### Kernel Regression of Multi-Way Data via Tensor Trains with Hadamard Overparametrization: The Dynamic Graph Flow Cas (https://arxiv.org/abs/2509.22197)
- **What's New**: 논문에서는 KReTTaH(케널 회귀를 통한 텐서 기차와 하다마르 과파라미터화)라는 새로운 회귀 기반 프레임워크를 도입합니다. 이 방법은 비모수(nonparametric) 방식으로 데이터 보간을 회귀 문제로 변환하여 해석 가능성을 높입니다. KReTTaH는 고정된 텐서 트레인(rank) 구조를 활용하고, 하다마르 과파라미터화를 통해 파라미터 공간 내에서 희소성을 촉진합니다.

- **Technical Details**: KReTTaH는 다차원 데이터에 대한 회귀를 재생산 커널 힐베르트 공간(RKHSs)에서 정의하고, 비모수 및 비선형 함수 근사를 가능하게 합니다. 또한 고정된 TT-랭크의 리만 다양체(Riemannian manifold)에서 부드러운 역문제를 해결하여 학습을 진행합니다. 이 과정은 Riemannian 최적화 방법을 통해 해결할 수 있으며, KReTTaH는 기존 방법들과 차별화되는 장점을 가지고 있습니다.

- **Performance Highlights**: KReTTaH는 동적 그래프 흐름의 누락된 엣지 흐름을 예측하는 문제에 적용되며, 전통적인 신경망(neural network) 방법에 비해 해석 가능한 해결책을 제공합니다. 실제 그래프 데이터셋에 대한 수치 실험에서는 KReTTaH가 최신 비모수 텐서 및 신경망 기반 방법에 비해 뛰어난 성능을 보였습니다. 향후 연구에서는 다른 기존 방법들과의 성능 비교를 포함한 추가적인 수치 실험 결과가 제공될 예정입니다.



### Mechanistic Independence: A Principle for Identifiable Disentangled Representations (https://arxiv.org/abs/2509.22196)
- **What's New**: 이번 논문에서는 disentangled representations의 성격을 이해하는 새로운 통합 프레임워크를 소개합니다. 이 프레임워크는 latent factors가 관찰된 변수에 어떻게 작용하는지를 중심으로 정의되며, 이러한 관점은 latent density의 변화에 불변성을 갖습니다. 또한 여러 독립성 기준을 제안해, 비선형 혼합의 경우에도 latent subspaces의 식별 가능성을 보여줍니다.

- **Technical Details**: 논문에서는 여러 가지 독립성 기준을 제시하며, 이 기준들이 어떻게 서로 연결되어 있는지 및 각 기준이 식별 가능성을 어떻게 보장하는지 논의합니다. 특정 구조적 조건을 갖춘 manifold에서 데이터 생성 과정을 모델링하고, latent factors는 각자의 공간에서 독립적으로 변동할 수 있음을 전제로 합니다. 이를 통해 작성된 프레임워크는 multidimensional latent factors 및 부분적인 disentanglement을 지원합니다.

- **Performance Highlights**: 연구 결과, 제안된 메커니즘 독립성 기준을 적용하여 각 숨겨진 요인들이 연결된 컴포넌트로 정의될 수 있음을 입증했습니다. 이러한 연구는 통계적 가정 없이도 disentangled representations를 식별할 수 있는 조건을 명확히 하고, 비선형 생성기와의 결합으로 보다 일반화된 해석을 제공합니다. 또한, 다양한 최근 결과들과의 관계를 설명하며, 연구의 결과들이 넓은 범위의 latent densities에서도 유효함을 강조합니다.
...
### Slicing Wasserstein Over Wasserstein Via Functional Optimal Transpor (https://arxiv.org/abs/2509.22138)
- **What's New**: 본 논문에서는 Wasserstein 거리(Wasserstein distance)의 새로운 접근 방식을 제안합니다. 특히, 기존의 높은 차원에서 발생하는 불안정성 문제를 피하고, 일반적인 Banach 공간(Banach spaces)에서 계산 가능한 sliced Wasserstein 메트릭을 개발하였습니다. 이를 통해 Double-sliced Wasserstein (DSW) 메트릭을 도입하고, 이 메트릭이 분산 계산을 위한 WoW(Wasserstein over Wasserstein) 거리의 대안이 될 수 있음을 보여줍니다.

- **Technical Details**: Optimal Transport (OT)는 확률 측도들 사이의 Wasserstein 거리를 효율적으로 정의합니다. 본 논문에서는 1차원 Wasserstein 공간과 L2([0,1]) 함수 공간의 quantile 함수 사이의 등거리(isometry)를 활용하여 1차원 메타-측도에서의 sliced Wasserstein 메트릭을 정의합니다. 또한, Gaussian 프로세스를 매개변수화하여 무한 차원 함수 공간의 단위 공에 대한 균일 분포의 부족을 해결하고, 이 방식을 다차원 메타-측도로 확장하여 DSW 메트릭을 생성합니다.

- **Performance Highlights**: 실험 결과, DSW 메트릭은 다양한 데이터셋, 형태, 이미지를 비교함에 있어 높은 성능을 보여줍니다. DSW의 최소화가 디스크리타이즈된 메타-측도의 WoW 최소화와 동등하다는 것을 입증하였으며, 수치적 실험을 통해 DSW가 WoW 거리의 효율적인 대안이 됨을 확인했습니다. 이러한 비교 결과는 DSW가 비용 효율적이며 안정적인 방법으로 데이터를 비교하는 데 기여할 수 있음을 보여줍니다.



### Mind the Missing: Variable-Aware Representation Learning for Irregular EHR Time Series using Large Language Models (https://arxiv.org/abs/2509.22121)
- **What's New**: 본 논문은 비정기적으로 샘플링된 생리학적 시계열 데이터를 학습하기 위해 설계된 VITAL이라는 새로운 프레임워크를 제안합니다. VITAL은 두 가지 유형의 임상 변수를 구분하여 vital signs(생체 신호)와 laboratory tests(실험실 검사)의 차별적 처리를 통해 시간적 컨텍스트를 캡처하고 결측치를 다루는 방식을 제공합니다. 이 모델은 고정 길이의 임베딩 벡터로 불완전한 시계열을 효과적으로 변환할 수 있습니다.

- **Technical Details**: VITAL은 LLM(대형 언어 모델)을 기반으로 하며, 비정기적으로 샘플링된 데이터의 특성을 반영하여 설계되어 있습니다. 생체 신호는 시간적 구조가 있는 밀집 변수로 다루어지며, 결측 관찰을 처리하는 방법이 적용됩니다. 실험실 검사 변수는 관측된 여부에 따라 대표 값 임베딩 또는 학습 가능한 토큰을 사용하여 보완됩니다.

- **Performance Highlights**: VITAL은 PhysioNet에서 제공하는 벤치마크 데이터세트에서 기존의 최첨단 방법과 비교하여 우수한 성능을 보였습니다. 실제 임상 시나리오를 시뮬레이션한 고정 센서 제외 설정에서 VITAL은 결측값에 대한 강력한 저항성을 보여주며, 기존 방법들보다도 뛰어난 성능을 입증했습니다.
...
- **What's New**: 이 논문에서는 회귀 시나리오에 대한 비관적 이급 최적화 프로그램(pessimistic bilevel optimization program)을 제안합니다. 이 설정은 적대적 회귀 분석(adversarial regression analysis)을 위한 기존의 알고리즘에서 수반하는 볼록성(convexity)이나 유일성(uniqueness)에 대한 가정을 하지 않습니다. 이 논문은 적대적 공격(adversarial attacks)으로부터 더 회복력이 있는 예측 모델을 훈련하는 새로운 접근 방식을 제공합니다.

- **Technical Details**: 저자들은 적대적 훈련(adversarial training) 과정을 통해 회귀 문제에서의 예측자(predictors)의 회복력을 높이는 방법을 보입니다. 비관적 이급 모델은 적대자가 데이터를 어떻게 변경할 수 있을지를 고려하여 상위 수준(upper-level)에서 예측 함수를 훈련합니다. 이 모델은 적대자의 데이터가 의미를 잃지 않도록 대표적인 제약 조건(constrained conditions)을 적용하여 현실적으로 시뮬레이션할 수 있도록 설계되었습니다.

- **Performance Highlights**: 논문에서는 제안한 모델이 기존 방법들을 초월하는 성능을 보여줄 수 있는 두 가지 데이터 세트에서의 실험 결과를 평가합니다. 이 모델은 기존 방법들이 커널 변환(kernel transformations)을 요구하는 반면, 원래의 특성 공간(original feature space)에서 적대자의 데이터를 유지함으로써 더 큰 변형을 필요로 하는 피쳐를 효과적으로 식별할 수 있는 능력을 갖추고 있습니다. 구조적으로 볼록성을 필요로 하지 않는 이 모델은 강력한 성능을 입증하고 있습니다.



### Modeling Psychological Profiles in Volleyball via Mixed-Type Bayesian Networks (https://arxiv.org/abs/2509.22111)
- **What's New**: 이번 연구에서는 이탈리아의 C 및 D 리그에서 활동 중인 164명의 여성 배구 선수 데이터를 활용하여 심리적 특성을 분석했습니다. 새로운 알고리즘인 latent MMHC 는 혼합 형식 변수를 다루는 하이브리드 구조 학습기를 도입하여 변수 간의 관계를 식별합니다. 이 모델을 통해 목표 설정과 자기 신뢰와 같은 정신적 기술들이 어떻게 연결되는지를 파악할 수 있습니다.

- **Technical Details**: latent MMHC 알고리즘은 잠재적인 가우시안 코퓰라와 제약 기반의 스켈레톤, 제약 기반의 점수 개선을 결합하여 단일 방향 비순환 그래프(DAG)를 제공합니다. 이 알고리즘은 다양한 샘플 크기와 희소성, 차원에서의 시뮬레이션을 통해 구조적 회복과 엣지 회상에서 경쟁 방법보다 우수한 성능을 보였습니다. 연구진은 또한 부트스트랩 집계 방법을 통해 보다 안정적인 변형을 검토하였습니다.

- **Performance Highlights**: 최종적으로 학습된 네트워크는 심리적 특성들의 조건부 관계를 명확하게 보여주며, 특정 기술 향상이 네트워크를 통해 어떻게 영향을 미치는지를 시나리오 분석으로 정량화했습니다. 이 접근 방식은 스포츠에서 심리적 특성 프로파일링을 위한 해석 가능한 데이터 기반 틀을 제공하여, 선수 개발에 대한 의사결정을 지원할 수 있는 기초를 마련합니다.
...
### SHAKE-GNN: Scalable Hierarchical Kirchhoff-Forest Graph Neural Network (https://arxiv.org/abs/2509.22100)
- **What's New**: 이번 논문에서는 SHAKE-GNN이라는 새로운 스케일 가능한 그래프 수준의 GNN 프레임워크를 소개합니다. 이 프레임워크는 Kirchhoff Forest의 계층적 구조를 기반으로 하여 그래프의 확률적 다중 해상도 분해를 구축하는 데 사용됩니다. SHAKE-GNN은 다중 규모의 표현을 생성하여 효율성과 성능 간의 유연한 균형을 가능하게 합니다.

- **Technical Details**: SHAKE-GNN은 Kirchhoff Forest를 기반으로 한 새로운 그래프 신경망 아키텍처로, 계층적 그래프 분해 파이프라인을 정의합니다. 각 계층에서는 의미 있는 성분을 집계하기 위해 평균 또는 합계 풀링을 사용하며, 모델의 성능을 높이기 위한 데이터 기반의 해상도 매개변수 선택 전략을 도입합니다. 또한 이 알고리즘은 시간 복잡도를 분석하여 기존 GNN 프레임워크에 비해 처리 속도를 향상시키는 이점을 제공합니다.

- **Performance Highlights**: 여러 대규모 그래프 분류 벤치마크에서 SHAKE-GNN은 경쟁력 있는 성능을 보이며 스케일 가능성을 향상시킵니다. 실제 실험을 통해 기존의 최첨단 GNN 모델에 비해 밝은 스케일의 이점을 입증하며, 다양한 그래프 도메인에 적용 가능한 효과적인 솔루션을 제공합니다.
...
- **What's New**: 이번 논문은 연합 학습(Federated Learning)에서의 gradient inversion attack (GIA)의 문제를 해결하기 위해 비선형 대리 모델(NL-SME)을 소개합니다. 기존의 선형 대리 모델(SME)에 비해 NL-SME는 SGD(확률적 경사 하강法)의 비선형 특성을 모델링하는 최초의 방법으로, 학습 가능한 2차 Bézier 곡선을 활용하여 성능을 크게 향상시켰습니다. 이 새로운 접근법은 연합 학습의 다단계 업데이트 패러다임에서의 프라이버시 취약성을 강조하며, 강력한 방어 전략 개발을 위한 새로운 관점을 제공합니다.

- **Technical Details**: NL-SME는 비선형 파라미터 경로 모델링을 통해 GIA의 효과를 극대화합니다. 학습 가능한 제어점을 통해 SGD의 곡선 형태를 포착하였으며, 제어점 정규화 및 매개변수 규모 조정 메커니즘을 결합하여 표현력을 강화합니다. 이 방법은 CIFAR-100 및 FEMNIST 데이터셋을 사용한 실험에서 뛰어난 성능을 입증하며, 기존의 방법들과 비교하여 모든 지표에서 차별화된 성과를 보였습니다.

- **Performance Highlights**: NL-SME는 코사인 유사도 손실에서의 수십 배의 개선을 달성하며, PSNR과 SSIM 메트릭에서도 기존 기준에 비해 상당한 성능 향상을 기록했습니다. 이 연구는 연합 학습 환경에서 클라이언트의 프라이버시를 더욱 보호할 수 있는 방향성을 제시하며, 향후 GIA에 대한 방어 전략 마련에 중요한 기초 자료로 활용될 것으로 기대됩니다.
...
### Towards Understanding Feature Learning in Parameter Transfer (https://arxiv.org/abs/2509.22056)
- **What's New**: 이번 연구는 파라미터 전이(parameter transfer)에 대한 이론적 분석을 제공하며, 훈련 동역학(training dynamics)을 통해 전이가 유익한 이유와 부정적 전이(negative transfer)가 발생하는 정확한 조건을 밝혀냅니다. 또한, 다양한 요인들이 전이 효과에 미치는 영향을 분석하여, 이를 통해 깊이 있는 이론적 지원을 제공합니다. 특히, 이 연구에서는 전이 학습의 이론적 경계가 아닌 실제 훈련 과정에서의 변화에 집중하고 있습니다.

- **Technical Details**: 이 연구는 비선형 동역학 시스템을 기반으로 하여 유니버설 지식(universal knowledge)과 소스(task) 및 타겟(task) 간의 관계를 모델링합니다. 연구의 프레임워크에서는 업스트림 모델에서 다운스트림 모델로전이되는 'α' 비율을 설정하고, 그 결과로 전이 효과에 기여하는 세 가지 주요 요인(소스와 타겟 간의 지식, 업스트림 모델의 샘플 크기, 노이즈 수준)을 집중적으로 분석합니다. 이를 통해 이론적으로 파라미터 전이의 성공적 적용의 조건을 규명하였습니다.

- **Performance Highlights**: 실험 결과, 더 많은 전이된 파라미터, 큰 샘플 크기, 낮은 노이즈 수준이 다운스트림 모델의 성능을 향상시키는 것으로 나타났습니다. 통계적 결과들은 파라미터 전이의 실험적 성과를 지지하며, 전이 학습과 관련된 이론적 기여를 강조합니다. 특히, 전이 학습이 직접 훈련하는 것보다 우수한 성능을 낸다는 이론적 설명도 제공되어, 파라미터 전이의 방식이 어떻게 적용될 수 있는지를 명확히 밝혀냅니다.
...
### BrainPro: Towards Large-scale Brain State-aware EEG Representation Learning (https://arxiv.org/abs/2509.22050)
Comments:
          26 pages, 9 figures

- **What's New**: 이번 논문에서 제안하는 BrainPro는 다양한 EEG 데이터셋을 빠르게 적응할 수 있는 대규모 EEG 모델입니다. 이 모델은 channel(채널) 및 region(지역) 간의 상호작용을 포착하는 retrieval-based spatial learning block을 도입하여 유연성을 높입니다. 또한, 뇌 상태를 분리하여 표현을 학습하는 brain state-decoupling block을 추가하여 다양한 작업에 적응할 수 있도록 설계되었습니다.

- **Technical Details**: BrainPro 모델의 구조는 공통 계층적 설계를 따른 여러 개의 인코더로 구성되어 있습니다. 첫 번째로, 다양한 EEG 모드에 대한 채널 및 지역 차원의 의존성을 포착하도록 설계된 retrieval-based spatial learning을 사용합니다. 두 번째로, 인코더는 공유 표현과 뇌 과정 관련 특정 표현을 분리하여 얻는 decoupling 및 region-aware reconstruction 목적을 통해 고유한 작업 특성에 대한 정보도 학습할 수 있습니다. 이러한 기법은 EEG 모양의 다양한 변화에 강건성을 부여합니다.

- **Performance Highlights**: BrainPro는 방대한 EEG 데이터셋에서 사전 훈련을 마치고, 9개 공개 BCI 데이터셋을 평가하여 최첨단 성과를 달성했습니다. 성능 평가에서 BrainPro는 여러 작업과 하드웨어 구성에 대해 뛰어난 일반화 능력을 보여주며, 뇌 상태 인식 기반의 EEG 표현 학습에 새로운 기준을 설정하였습니다.



### MO-GRPO: Mitigating Reward Hacking of Group Relative Policy Optimization on Multi-Objective Problems (https://arxiv.org/abs/2509.22047)
- **What's New**: 이 논문에서는 Multi-Objective Group Relative Policy Optimization (MO-GRPO)라는 새로운 알고리즘을 제안합니다. 기존의 Group Relative Policy Optimization (GRPO)에서는 고차원의 보상 모델이 필요하지만, 현실에서는 그러한 모델을 얻기가 어렵습니다. MO-GRPO는 보상 기능의 변동성을 자동으로 조정하여 여러 목표를 동시에 최적화할 수 있는 방법을 제공합니다.

- **Technical Details**: MO-GRPO는 여러 보상 모델에 대해 각각의 이점(advantage) 기능을 별도로 계산하여 안정적 학습을 가능하게 합니다. 이 알고리즘은 특정 보상 기능의 변동성이 상대적으로 높은 경우에 대해 최적의 정책을 학습하는 데 유리합니다. 보상 함수의 규모를 개별적으로 조정함으로써, 각 보상이 정책 업데이트에 균형 있게 기여하도록 합니다.

- **Performance Highlights**: MO-GRPO는 여러 실험 분야에서 GRPO보다 뛰어난 성능을 보여 줍니다. 특히, 다중 목표 강화 학습 문제에 대한 평가 결과, MO-GRPO는 보상 해킹(reward hacking) 현상을 성공적으로 완화하여 원하는 정책을 결과적으로 생성했습니다. 이는 MO-GRPO가 여러 목표 간의 상관관계를 균등하게 분배하는 데 기여함을 나타냅니다.



### Convexity-Driven Projection for Point Cloud Dimensionality Reduction (https://arxiv.org/abs/2509.22043)
- **What's New**: 이 논문에서는 Convexity-Driven Projection (CDP)라는 점 구름(point cloud)의 차원 축소(dimensionality reduction)를 위한 새로운 경계 없는 선형 방법을 제안합니다. CDP는 k-NN 그래프를 구축하고, 유클리드 거리와 최단 경로 거리 비율이 특정 임계값 이하인 유효 쌍을 찾아내며, 이들의 방향을 모아 양의 준정칙(non-convexity structure matrix)을 형성합니다. 이 방법은 기본적으로 지역 비볼록성(local non-convexity)을 보존하는 데 초점을 맞추며, 기존의 차원 축소 방법들이 간과하는 부분을 해결합니다.

- **Technical Details**: CDP는 유효 쌍을 찾기 위해 비볼록 비율(convexity ratio) r_{ij}를 주목하면서, 이 값이 임계값 τ보다 작거나 같은 경우를 정의합니다. 각 유효 쌍에 대해 정규화된 외적의 평균으로 비볼록성 구조 행렬(S_{nc})을 형성하고, 이 행렬의 상위 k개의 고유 벡터를 사용하여 점을 투사합니다. 알고리즘은 복잡성과 그래프 생성 고려사항을 포함하여 단계별로 정리되어 있습니다.

- **Performance Highlights**: 실험 평가를 통해 CDP의 성능을 점검하고, 새로운 증명 가능한 보증을 제시합니다. 알고리즘은 고차원 데이터에서 비볼록성(non-convexity)을 유지하는 능력이 뛰어나며, 실험 결과는 제안된 방법의 유용성을 확인합니다. 또한, 고정 및 재선택 쌍에 대한 지표와 인증(distribution) 정보를 제공합니다.
...
- **What's New**: 본 연구에서는 Sparse Autoencoders (SAEs)의 한계를 극복하기 위해 Orthogonal SAE (OrtSAE)를 제안합니다. 기존 SAE가 겪는 feature absorption과 feature composition 문제를 해결하기 위해, 학습된 특성 간의 orthogonality를 강제하는 방법을 도입했습니다. OrtSAE는 새로운 훈련 절차를 통해 서로 유사한 특성을 억제하여 독립적인 특성이 생겨나도록 합니다.

- **Technical Details**: OrtSAE는 SAE latents 사이의 높은 pairwise cosine similarity를 패널티로 부여하여 불필요한 상관관계를 줄입니다. 훈련시에도 복잡성을 줄이기 위해 chunk-wise 전략을 적용해 비용을 선형적으로 조정하며, 기존 SAE 구조에 큰 변화를 주지 않고 효율적으로 확장할 수 있습니다. 이 접근 방식은 효율적인 계산을 가능하게 하면서 각 latents 간의 독립성을 강화합니다.

- **Performance Highlights**: OrtSAE는 기존 SAE와 비교했을 때 9% 더 많은 독립된 특성을 발견하고, feature absorption을 65%, feature composition을 15% 줄였습니다. SAEBench에서의 실험 결과, OrtSAE는 spurious correlation 제거에서 6%의 성능 향상을 보이며, 다른 다운스트림 작업에서도 경쟁력을 유지하는 성과를 달성했습니다.



### MCGM: Multi-stage Clustered Global Modeling for Long-range Interactions in Molecules (https://arxiv.org/abs/2509.22028)
Comments:
          27 pages, 1 figures

- **What's New**: 본 연구에서는 Multi-stage Clustered Global Modeling (MCGM)이라는 혁신적인 방법론을 소개합니다. MCGM은 기하학적 그래프 신경망(GNN)에서 계층적 글로벌 컨텍스트를 효율적으로 통합할 수 있도록 하는 경량 모듈입니다. 이 방법은 동적 계층형 클러스터링을 통해 원자 집합을 다중 해상도로 구축하고 글로벌 정보를 증류하여 전통적인 GNN의 한계를 극복합니다. 이러한 접근 방식은 분자 물성을 더욱 정교하게 모델링할 수 있게 합니다.

- **Technical Details**: MCGM은 동적 클러스터링을 통해 계층적 분자 조직을 발견하는 자율적인 프레임워크로, 고정된 구조적 가정을 두지 않습니다. 학습 동안 원자들이 계층 클러스터로 재구성되며, 이를 통해 고유한 다중 규모의 구조 패턴을 자동으로 식별합니다. MCGM은 기존의 방법들과 비교했을 때 계산 효율성을 높이면서 복잡한 파라미터 튜닝 없이 강력한 성능을 보입니다. 덕분에 이 모듈은 다양한 기하학적 GNN 아키텍쳐와 원활하게 통합될 수 있습니다.

- **Performance Highlights**: MCGM을 사용하여 OE62 데이터셋에서 평균 26.2%의 에너지 예측 오차 감소 효과를 얻었습니다. AQM 데이터셋에서도 17.0 meV의 에너지 및 4.9 meV/Å의 힘 예측 정확도를 달성하며, Neural P3M보다 20% 적은 파라미터로 상태-of-아트 성능을 보여줍니다. 이 결과들은 MCGM의 효율성과 유연성을 입증하며, 다양한 아키텍쳐에서의 일관된 성능 개선을 확인할 수 있었습니다.



### Teaching Transformers to Solve Combinatorial Problems through Efficient Trial & Error (https://arxiv.org/abs/2509.22023)
- **What's New**: 이번 연구는 대형 언어 모델(LLM)이 조합 최적화 문제를 해결하는 방법에서의 한계를 극복하기 위한 새로운 접근법을 제안합니다. 특히, Sudoku 퍼즐을 해결하기 위해 Vanilla Decoder-only Transformer (GPT-2)를 사용하여 99%의 높은 정확도를 기록했습니다. 기존의 신경-상징적 접근법과 달리, 외부 도구나 함수 호출 없이도 문제를 해결할 수 있는 방법을 개발했습니다.

- **Technical Details**: 이 방법은 단순한 Sudoku 규칙의 모방 학습과 Depth-First Search (DFS) 탐색 전략을 통합하여 informed guessing 및 backtracking을 포함합니다. 모델은 유효한 해답과 무효한 해답을 구별하며, 올바른 해답에 도달하기 위한 여러 가지 대안을 탐색하는 훈련을 받습니다. 이 방식은 NP-클래스의 모든 조합 문제 해결에 적용할 수 있으며, 문제의 해답을 쉽게 검증할 수 있습니다.

- **Performance Highlights**: 우리는 여러 모델을 평가한 결과, 아무것도 해결할 수 없었던 기존 LLM과는 달리, 우리의 방법은 Sudoku 퍼즐에서 99%의 정확도를 달성하며, 이는 NN 기반 Sudoku 해결 방법 중에서 최첨단 성능을 보여줍니다. 제안된 메서드는 조합 문제에서도 우수한 성능을 발휘하며, 이를 통해 조합 최적화 문제 해결의 유망한 가능성을 확인했습니다.



### Task-Adaptive Parameter-Efficient Fine-Tuning for Weather Foundation Models (https://arxiv.org/abs/2509.22020)
- **What's New**: 이 논문에서는 Weather Foundation Models (WFMs)에서의 파라미터 효율적인 파인튜닝을 위한 새로운 프레임워크인 WeatherPEFT를 소개합니다. WeatherPEFT는 Task-Adaptive Dynamic Prompting (TADP)와 Stochastic Fisher-Guided Adaptive Selection (SFAS) 두 가지 혁신적인 방법을 통합하여, 다양한 기상 하위 작업에 최적화된 접근 방식을 제공합니다. 이러한 접근은 기후 변화가 심화되는 시대의 기상 예측 정확도를 향상시키는 데 기여할 것입니다.

- **Technical Details**: WeatherPEFT는 두 가지 주요 기법을 통해 WFMs에 파라미터 효율적인 파인튜닝을 가능하게 합니다. TADP는 특정 하위 작업에 맞춰 입력 토큰에 임베딩 가중치를 동적으로 주입하여 맥락에 맞는 특징 재조정을 지원합니다. SFAS는 Fisher 정보를 활용하여 가장 중요한 작업 관련 매개변수를 식별하고 업데이트하여 기존의 지식을 유지하고, 랜덤성을 도입하여 선택의 안정성을 높입니다.

- **Performance Highlights**: WeatherPEFT는 세 가지 하위 작업에서 실험을 통해 기존의 PEFT 방법들이 Full-Tuning에 비해 성능 차이를 보이는 반면, WeatherPEFT는 훨씬 적은 수의 학습 가능한 매개변수로 Full-Tuning과 동등한 성능을 달성했습니다. 특히 지역 강우 예측 작업에서 WeatherPEFT는 Full-Tuning을 초과하는 성능을 발휘하며, 전체 모델의 효율적인 조정을 통해 기상 응용 분야의 다양한 요구를 효과적으로 해결할 수 있음을 보여줍니다.
...
- **What's New**: 이 논문에서는 AEGIS(Authentic Edge Growth In Sparsity)라는 엣지 전용 증강 프레임워크를 소개합니다. 이 프레임워크는 기존의 훈련 엣지를 재샘플링하여 원래의 노드 집합을 보존하고 조작된 끝점을 회피합니다. AEGIS는 자연적으로 희소한 그래프에서 진정성을 조사하며, 이 방법이 데이터가 부족한 이분 그래프(link prediction)에서 효과적이라는 것을 보여줍니다.

- **Technical Details**: AEGIS는 엣지를 단순하게 또는 역도수 편향을 고려하여 보존하는 방식으로 무작위 엣지와 합성 엣지보다 더 나은 성능을 발휘합니다. 이 연구에서는 두 가지 보조 지표인 AUC-ROC(높을수록 좋음)와 Brier score(낮을수록 좋음)를 사용하여 증강 효과를 평가합니다. 또한, 게임 디자인 패턴의 그래프와 같이 희소성이 강조되는 그래프에서의 결과를 보여줍니다.

- **Performance Highlights**: Amazon과 MovieLens 데이터셋에서 AEGIS 변형이 기존 베이스라인과 동등한 성능을 나타내며, semantic KNN 증강을 통해 AUC 및 보정(calibration)이 회복되는 유일한 방법으로 나타났습니다. GDP 그래프에서 semantic KNN은 가장 큰 AUC 개선과 Brier score 감소를 보여주며, 간단한 방법 또한 Brier score를 낮추는 효과를 보였습니다. 이러한 결과는 데이터 효율적인 전략으로서의 진정성 기반 재샘플링의 가능성을 강조합니다.



### Concept-SAE: Active Causal Probing of Visual Model Behavior (https://arxiv.org/abs/2509.22015)
- **What's New**: 이번 논문에서는 Concept-SAE라는 새로운 프레임워크를 소개합니다. 이 프레임워크는 기존의 Sparse Autoencoder(SAE) 기술을 업그레이드하여, 개념 토큰을 생성하는 하이브리드 분리 전략을 통해 보다 신뢰할 수 있는 도구로 변모시킵니다. 연구팀은 이중 감독(dual-supervision) 접근법을 통해 명확하고 공간적으로 로컬라이즈된 개념 토큰을 생성하여, 모델의 동작을 인과적으로 탐색할 수 있는 가능성을 제시합니다.

- **Technical Details**: Concept-SAE는 기존 SAE에서 제한된 개념을 활용하여, 정의된 개념이 존재하는지 및 공간적 위치를 감지하는 robust한 이중 감독 메커니즘을 사용합니다. 이러한 과정을 통해 데이터의 복잡성을 보존하면서도 모델 내부의 인과 구조를 탐색할 수 있는 정량적 메커니즘을 제공합니다. 연구진은 세 가지 핵심 기여를 발표하며, 개념 표현의 충실도와 공간적 위치화가 어떻게 인과적 링크를 세우고 모델의 취약점을 체계적으로 로컬라이징하는 데 기여하는지를 설명합니다.

- **Performance Highlights**: 실험 결과, Concept-SAE는 기존의 SAEs보다 훨씬 높은 충실도와 명쾌한 개념 표현을 제공하여 여러 개념 기반 방법들과 비교해 우수한 성능을 보였습니다. 이 시스템은 예측에 직접적인 개입을 통해 인과적인 연결을 탐색하고, 모델의 실패 모드를 특定 레이어에 국한하여 체계적으로 분석할 수 있는 기능을 제공합니다. 결과적으로 Concept-SAE는 상관관계 해석을 넘어 기계적이고 인과적으로 모델 동작을 조사할 수 있는 유망한 Blueprint를 제공합니다.



### Goal-Guided Efficient Exploration via Large Language Model in Reinforcement Learning (https://arxiv.org/abs/2509.22008)
- **What's New**: 이 논문에서는 Structured Goal-guided Reinforcement Learning (SGRL) 방법을 제안하며, 이는 구조화된 목표 계획자와 목표 조건 액션 프루너를 통합합니다. 이를 통해 RL(강화학습) 에이전트가 효율적으로 탐색할 수 있도록 유도합니다. LLM(대형 언어 모델)의 강력한 계획 능력을 활용하여 목표를 생성하고 우선순위를 설정하는 방식으로, RL 에이전트의 탐색 효율성과 장기 계획 능력을 개선합니다.

- **Technical Details**: SGRL은 구조화된 목표 생성 함수와 목표 조건을 반영하여 RL 에이전트의 정책을 조정하는 기능을 포함합니다. 이러한 구조는 목표의 우선 순위를 동적으로 조정하고, 비효율적인 행동을 필터링하여 목표와 일치하는 정책 선택을 강제합니다. 본 연구는 Crafter 및 Craftax-Classic과 같은 복잡한 오픈 월드 환경에서 수행된 실험을 통해 기존의 LLM 기반 RL 방법보다 우수한 성능을 보여줍니다.

- **Performance Highlights**: SGRL은 성공률, 총 점수, 누적 보상 및 성과 깊이와 같은 여러 지표에서 기존 최첨단 방법과 비교하여 우수한 성능을 기록하였습니다. 실험 결과는 SGRL이 RL 에이전트의 탐색 효율성을 크게 향상시키면서 낮은 LLM 호출 빈도를 유지하는 것을 보여줍니다. 또한 목표 조건 모델을 통해 RL 에이전트가 더 적응력 있게 오픈 월드 환경에서 탐색할 수 있도록 지원합니다.
...
- **What's New**: 이 논문에서는 Classifier-Free Guidance (CFG)가 확산 모델에서 조건부 충실도를 향상시키는 데 널리 사용되지만 샘플링 동역학에 미치는 영향을 잘 이해하지 못하고 있다는 점을 강조합니다. 특히 이 연구는 다중 모드 조건부 분포에서 CFG를 분석하여 샘플링 과정이 세 가지 단계로 진행된다는 점을 보여줍니다. 이는 기존 연구들이 주로 단일 모드 조건부 분포에 국한되어 있었던 것을 넘어서, 보다 현실적인 모델링을 가능하게 합니다.

- **Technical Details**: 샘플링 과정은 Direction Shift, Mode Separation, Concentration의 세 가지 단계로 나뉩니다. Direction Shift 단계에서는 클래스 가중 평균으로의 수렴이 ускор되고 초기화 편향이 도입됩니다. Mode Separation 단계에서는 로컬 동역학이 무난하지만 상속된 편향이 약한 모드를 억제하여 전반적인 다양성을 감소시킵니다. 마지막으로 Concentration 단계에서 강한 가이던스가 모드 내 수축을 증폭시키며 미세한 변동성을 줄입니다.

- **Performance Highlights**: 이 연구의 실험 결과는 초기 강한 가이던스가 전 세계적인 다양성을 억제하며, 후기에 강한 가이던스가 미세한 변동성을 억제함을 보여줍니다. 또한 시간에 따라 가이던스 강도를 조정하는 것이 품질-다양성의 균형을 개선할 수 있음을 제안하고, 이론적 기여를 보완해 보다 나은 샘플링 성능을 달성할 수 있음을 입증합니다. 이러한 결과들은 이론적 및 경험적 관점을 통합하여 guided diffusion sampler의 본질적 설계를 위한 기초를 마련합니다.



### GRAM-TDI: adaptive multimodal representation learning for drug target interaction prediction (https://arxiv.org/abs/2509.21971)
- **What's New**: 이번 논문에서는 GRAMDTI라는 새로운 사전 훈련(pretraining) 프레임워크를 소개합니다. 이 프레임워크는 단백질과 소분자의 멀티모달(multi-modal) 정보를 통합하여 통합된 표현을 생성합니다. GRAMDTI는 기존의 쌍(pair) 기반 접근 방식을 넘어서는 고차원 의미 정렬(high-order semantic alignment)을 포착하고, 각 모달리티의 정보 기여도를 동적으로 조절합니다. 또한, IC50 활성 측정을 약한 지도 신호로 활용하여 생물학적으로 의미 있는 상호작용 강도를 기준으로 표현을 강화합니다.

- **Technical Details**: GRAMDTI는 여러 모달리티를 통합한 표현을 학습하기 위해 볼륨 기반 대조 학습(volume-based contrastive learning)을 확장합니다. 이 방법은 SMILES 서열, 단백질 서열, 텍스트 설명 및 계층적 분류 주석을 포함한 다양한 입력을 사용하여 모달리티 간의 원활한 정렬을 실현합니다. 뿐만 아니라, GRAMDTI는 모든 모달리티의 상대적인 정보 가치를 인식하며, 이를 기반으로 각 모달리티의 기여도를 조절하는 적응형(dropout) 기술을 제공합니다.

- **Performance Highlights**: GRAMDTI는 공개된 네 가지 데이터셋에서 상태-of-the-art 성능을 입증하였습니다. 실험을 통해, 높은 차원의 멀티모달 정렬과 적응형 모달리티 활용, 보조 감독(auxiliary supervision)을 통해 DTI 예측의 강력함과 일반화 가능성을 강조하고 있습니다. 이러한 결과들은 GRAMDTI가 실질적인 약물 발견 애플리케이션에서 유용하다는 것을 입증합니다.



### Think Smart, Not Hard: Difficulty Adaptive Reasoning for Large Audio Language Models (https://arxiv.org/abs/2509.21960)
- **What's New**: 최근 대규모 오디오 언어 모델(LALMs)의 추론 능력이 크게 향상되었습니다. 이 논문에서는 문제의 복잡성에 따라 추론 깊이를 조절하는 데 중점을 두고, 효율성과 효과성을 동시에 향상시키기 위해 새로운 방법을 제안합니다. 특히, 우리는 문제의 난이도에 따라 추론 길이를 동적으로 연결하는 보상 함수(reward function)를 도입하여, 간단한 작업에는 간결한 추론, 복잡한 작업에는 심화된 추론을 유도합니다.

- **Technical Details**: 우리는 LALMs의 성능을 높이기 위해 두 가지 난이도 적응형(length-based) 보상 기준을 제안합니다. 이 기준은 각 질문의 난이도에 따라 적절한 추론 길이를 맞추기 위해 설계되었습니다. 실험을 통해, 제안된 새로운 보상 함수를 사용하는 모델이 MMAU 벤치마크에서 더욱 효과적인 성능을 발휘하며, 동시에 추론 길이를 유의미하게 줄임을 보여주었습니다.

- **Performance Highlights**: 제안된 LALMs의 접근 방식은 특히 어려운 질문에서 우수한 성능을 보였습니다. 질적 연구를 통해 모델의 출력 구조를 분석한 결과, 효율적인 추론과 성능 개선을 동시에 이루어냈음을 확인했습니다. 모든 실험 모델, 데이터셋 및 스크립트는 향후 공개될 예정입니다.
...
### Structural Information-based Hierarchical Diffusion for Offline Reinforcement Learning (https://arxiv.org/abs/2509.21942)
Comments:
          Accepted by NeurIPS 2025

- **What's New**: 본 논문에서는 Sparse Rewards가 있는 장기 환경에서 효과적이고 안정적인 오프라인 정책 학습을 위한 새로운 구조 기반의 계층적 확산 프레임워크 SIHD를 제안합니다. SIHD는 오프라인 경로에서 내재된 구조적 정보를 분석하여 적응형 확산 계층을 구성하므로, 여러 시간 규모에 걸친 유연한 경로 모델링이 가능합니다. 이러한 접근방식은 기존의 고정된 두 계층 확산 계층의 기초 위에 세워져 있으며, 다양한 하부 작업을 수행할 수 있는 분산 모델을 요구합니다.

- **Technical Details**: SIHD는 각 상태 커뮤니티의 구조적 정보 획득을 정량화하며 이를 조건부 신호로 사용하여 인접한 확산 계층에서 적용합니다. 또한, 구조적 엔트로피 정규화를 도입하여 역사적 경로에서 저조하게 표현된 상태를 탐색하도록 유도하며, 데이터 세트에 대한 과도한 의존을 줄이는 역할을 합니다. 이 모델은 트리 구조의 상태 커뮤니티를 파생하여 경로를 적응형으로 세분화하며, 비상 또는 오프라인 데이터의 불일치로 인한 외삽 오류를 줄이도록 설계되었습니다.

- **Performance Highlights**: SIHD는 D4RL 벤치마크에서 비계층적 및 계층적 최신 기법들과 비교 시, 의사결정 성능에서 최대 12.6% 우수한 성과를 보였습니다. 고급 일반화는 다양한 장기 보상 희소 작업에서 나타났으며, 다양한 하위 작업들을 처리하는데 효과적임을 입증하였습니다. SIHD의 주요 기여는 적응형 모델링, 조건부 신호 활용, 상태 탐색 및 전반적인 성능 개선이 결합된 것입니다.



### Statistical Advantage of Softmax Attention: Insights from Single-Location Regression (https://arxiv.org/abs/2509.21936)
- **What's New**: 본 논문에서는 대형 언어 모델(Large Language Models, LLMs)의 소프트맥스(softmax) 활성화 함수와 대안들 간의 성능 차이를 보다 체계적으로 연구합니다. 단일 위치 회귀(single-location regression) 작업을 통해 소프트맥스가 Bayes 위험(Bayes risk)을 달성하는 반면, 선형 주의(linear attention)는 본질적으로 부족하다는 점을 분석합니다. 이 연구는 여러 가지 활성화 함수의 효율적인 성능을 결정짓는 필수 속성을 규명하는 데 중점을 두었습니다.

- **Technical Details**: 연구에서 제안하는 단일 위치 회귀 모델은 입력 토큰의 선형 변환에 의존하며, 이는 이전 이론적 연구들을 포함합니다. 연구는 고차원 한계(high-dimensional limit)에서 주의 메커니즘을 분석하며, 결과적으로 소프트맥스와 선형 주의 간의 성능 격차를 증명하고, 두 주의 메커니즘을 비교합니다. 이 연구는 경량 최적화 경량화 알고리즘을 통해 소프트맥스의 이점을 제시하며, 이 과정에서 샘플 복잡성이 차원에 비례함을 보여줍니다.

- **Performance Highlights**: 결과적으로 소프트맥스는 대체 주의 메커니즘들과 비교했을 때 실제 언어 설정에서 일관되게 우수한 성능을 보입니다. 우리는 다양한 실험을 통해 소프트맥스가 선형 주의 및 커널화 주의(kernelized attention)에 비해 높은 정확도를 기록함을 입증했습니다. 마지막으로 경량 기반의 최적화 알고리즘이 예측된 경험적 위험 최소화(ERM)를 달성하는 것을 보여줍니다.



### Extracting Actionable Insights from Building Energy Data using Vision LLMs on Wavelet and 3D Recurrence Representations (https://arxiv.org/abs/2509.21934)
Comments:
          IEEE International Conference on Data Mining 2025

- **What's New**: 이 논문에서는 에너지 데이터의 복잡한 비선형 및 다중 규모 특성을 다루기 위해 3D 그래픽 표현을 사용하는 새로운 프레임워크를 제안합니다. 1D 시간 시계열 데이터를 연속 웨이브렛 변환(Continuous Wavelet Transforms, CWTs)과 재발 플롯(Recurrence Plots, RPs)을 통해 3D로 변환하여 이를 시각적으로 해석할 수 있도록 합니다. 또한, 이는 에너지 소비 패턴을 시각적으로 감지하고 최적화를 위한 권장 사항을 생성할 수 있는 기회를 제공합니다.

- **Technical Details**: 제안된 프레임워크는 1D 건물 에너지 시간 시계열을 3D 시각적 표현으로 변환합니다. CWT는 원래의 에너지 소비 신호에서 시간-주파수 표현을 생성하며, 이는 고주파사건과 저주파 경향을 동시에 포착할 수 있는 다해상도 분석을 가능하게 합니다. 이 프레임워크는 VLLMs를 기반으로 하여 이러한 3D 시각 표현을 해석하도록 설계되었습니다.

- **Performance Highlights**: 실험 결과, Idefics-7B VLLM은 샤르자 대학 에너지 데이터 셋에서 CWT를 사용할 경우 0.0952의 유효성 손실을 달성하여, 기본 시간 시계열 데이터에서의 직접적인 조정(0.1176)보다 우수한 결과를 보였습니다. 또한, VLLMs를 통해 에너지 소비 패턴을 모니터링하고 반복적인 이상을 감지하며 최적화 권장 사항을 생성할 수 있는 능력을 검증하였습니다.
...
### Multiplicative-Additive Constrained Models:Toward Joint Visualization of Interactive and Independent Effects (https://arxiv.org/abs/2509.21923)
- **What's New**: 이번 연구에서는 선언적인 모델을 활용하여 해석 가능성과 예측 성능을 동시에 개선하는 새로운 접근 방식인 Multiplicative-Additive Constrained Models (MACMs)을 제안합니다. 기존의 Generalized Additive Models (GAMs)와 Curve Ergodic Set Regression (CESR)을 기반으로 하며, GAMs은 고차 상호작용 효과를 생략하여 예측 정확성에 한계를 두고 있었으나 MACMs은 이러한 한계를 해결합니다. 이 모델은 상호작용 및 개별 특성 효과를 동시에 시각화할 수 있어 사용자의 해석을 지원합니다.

- **Technical Details**: MACM은 곱셈 부분(multiplicative part)과 더하기 부분(additive part)으로 구성되어 있으며, 이러한 구조는 모델의 가설 공간을 확장합니다. 이로 인해 독립항 및 상호작용 항의 계수가 분리되어 안정적인 훈련을 가능하게 합니다. MACM은 다양한 형태의 함수 맵핑(functional mappings)을 사용할 수 있어, 신경망(neural network)을 통해 표현력을 더욱 키울 수 있습니다.

- **Performance Highlights**: 실험 결과, 신경망 기반의 MACMs는 기존의 CESR 및 최신 GAMs보다 예측 성능이 현저히 향상된 것으로 나타났습니다. 특히, MACMs은 모든 특성 간의 상호작용을 고려하며, 모델의 출력에 대한 집합적 기여를 보여주는 기능을 가지고 있습니다. 이러한 성과는 해석 가능성을 유지하면서도 높은 예측 정확성을 달성하는 데 기여하고 있습니다.



### Discrete Guidance Matching: Exact Guidance for Discrete Flow Matching (https://arxiv.org/abs/2509.21912)
- **What's New**: 이 논문에서는 이산 데이터에 대한 새로운 가이던스(framework for guidance) 프레임워크를 제안하여 샘플링 과정을 원하는 분포로 유도합니다. 기존의 접근 방식은 첫 번째 차수 Taylor 근사(first-order Taylor approximation)에 의존했으나, 이산 상태 공간에서는 정확하지 않아 큰 근사 오차(approximation error)를 초래할 수 있습니다. 제안하는 방법은 학습된 이산 흐름 일치 모델을 기반으로 정확한 전이율(transition rate)을 도출하여 각 샘플링 단계에서 단일 순방향 패스(single forward pass)만을 요구하여 효율성을 크게 향상시킵니다.

- **Technical Details**: 제안된 가이던스 프레임워크는 연속 시간 마르코프 체인(Continuous-Time Markov Chain, CTMC) 설정 하에 동작하며, 소스 분포(source distribution)로부터 샘플링하기 위한 사전 학습된 모델과 목표 분포(target distribution) 간의 밀도 비율(density ratio)을 이용하여 정확한 전이율을 식별합니다. 이 가이던스는 기존의 메소드들을 일반적인 형태로 포괄하며, 에너지 기반 샘플링(energy-guided sampling) 및 개인화된 생성 모델(class-conditional generation) 등 다양한 기존 접근 방식에도 적용 가능합니다. Bregman 다이버전스를 이용하여 조건부 기대값을 추정하기 위한 네트워크를 학습하는 방법을 도입하였고, 목표 분포에서 샘플을 활용하기 위한 정규화 기법도 제안하였습니다.

- **Performance Highlights**: 제안한 가이던스 프레임워크는 에너지 기반 샘플링 시뮬레이션 및 텍스트-이미지 생성, 다중 모드 이해(multimodal understanding) 작업에서 효과성을 입증하였습니다. 각 샘플링 단계마다 단 하나의 순방향 패스를 통해 필요한 전이율을 계산할 수 있어, 기존 방법보다 샘플링 효율성이 크게 향상되었습니다. 이를 통해 여러 분야에서 이산 흐름 모델(Discrete Flow Models)과 이산 확산 모델(Discrete Diffusion Models)의 발전에 기여할 수 있습니다.
...
- **What's New**: 이 논문에서는 Koopman 연산자, 그룹 표현 및 재생 커널 힐버트 공간(RKHS)을 활용하여 딥 뉴럴 네트워크에 대한 새로운 Rademacher 복잡성 경계를 유도합니다. 제안된 경계는 고랭크(weight matrix의 장수)가 있는 모델이 잘 일반화되는 이유를 설명합니다. 기존의 경계들은 제한된 유형의 모델에만 적용될 수 있었지만, 이 연구는 현실적인 모델을 위한 더 넓은 범위의 경계를 도출합니다.

- **Technical Details**: 이 논문은 딥 뉴럴 네트워크의 대수적 표현을 소개하고, RKHS를 구성하기 위해 매개변수 공간에서 정의된 커널 함수를 도입합니다. 이러한 커널 함수는 비부드러운 활성화 함수와 제한된 데이터 공간을 가진 현실적인 딥 모델을 설명할 수 있습니다. 이 논문에서 다루는 Koopman 기반 경계는 활성화 함수의 규범에 의존하며, 이는 많은 경우 평가하기 어려운 요소입니다.

- **Performance Highlights**: 제안된 Rademacher 복잡성 경계는 고랭크 가중치 행렬을 가진 모델이 일반화 잘 되는 원인을 설명하며, 기존 Koopman 기반 경계보다 더 넓은 범위의 모델에 적용될 수 있도록 설계되었습니다. 연구는 선형 연산자의 작용을 개념적으로 묘사하며, 그룹 표현과 Koopman 연산자를 결합합니다. 도출된 경계는 기존 한계를 넘어 더 일반적인 뉴럴 네트워크를 포함하는 중요성을 가지며, 실질적인 적용 가능성을 보여줍니다.
...
- **What's New**: 이번 논문에서는 Neural ODEs의 로버스트니스(robustness)를 개선하기 위한 새로운 프레임워크 Zubov-Net을 제안합니다. 이 프레임워크는 Zubov의 방정식을 혁신적으로 재구성하여 안정성 지역(Region of Attraction, RoA)과 설정된 안정성 지역(Prescribed RoA, PRoA) 간의 일관성을 밝힙니다. 이를 통해 정확도와 로버스트니스 간의 긴장을 해결할 수 있는 새로운 패러다임을 도입합니다.

- **Technical Details**: Zubov-Net은 일관성 손실(consistency loss), 분류 손실(classification loss), 그리고 분리 손실(separation loss)의 삼중 손실 체계를 통해 Neural ODE와 Lyapunov 함수가 공동 최적화되도록 구성됩니다. 또한, 입력 주의(input attention) 기반의 볼록 신경망(convex neural network)을 설계하여 Lyapunov 함수의 분별성을 향상시키고, 준거 효과를 증대시키며 훈련 안정성을 유지합니다.

- **Performance Highlights**: 실험 결과, Zubov-Net은 높은 분류 정확도를 유지하면서 다양한 확률적 잡음과 적대적 공격(adversarial attack)에 대한 로버스트니스를 크게 향상시킵니다. 논문에서 제시된 이론적 증명은 삼중 손실을 최소화하면 PRoA와 RoA의 일관성 정렬, 궤적 안정성(trajectory stability) 및 비중복 PRoA(non-overlapping PRoA)가 보장됨을 나타내고 있습니다.



### Abductive Logical Rule Induction by Bridging Inductive Logic Programming and Multimodal Large Language Models (https://arxiv.org/abs/2509.21874)
- **What's New**: 이 논문에서는 ILP-CoT라는 새로운 방법을 제안하여 유도 논리 프로그래밍(ILP)과 다중 모달 대형 언어 모델(MLLMs)을 통합하여 논리 규칙 유도를 수행합니다. 이 방법은 비구조적인 텍스트나 시각적 입력으로부터 논리적 사실을 발견하고 이로부터 규칙을 유도하는 것을 목표로 합니다. ILP-CoT는 기존의 ILP 시스템을 통해 구조를 기반으로 한 규칙 제안을 사용하여 ILP 작업을 자동으로 구축하고 고정된 논리적 사실과 형식적 유도 추론에 따라 규칙을 생성합니다.

- **Technical Details**: ILP-CoT는 CoT(Chain-of-Thought) 추론 과정 중에 ILP 시스템을 '플러그 앤 플레이' 방식으로 통합합니다. 이 과정은 MLLMs의 강력한 다중 모달 인식과 기호 추출 능력을 활용하여 입력 인스턴스로부터 초기 논리적 사실을 생성합니다. 이후 MLLMs가 제안한 규칙 구조를 정형화된 메타 규칙으로 변환하고 ILP를 통해 공식적인 규칙 유도를 수행합니다. 이 접근법은 복잡한 기호 기초 과정을 분리하여 의사 결정 과정에서의 환각 위험을 줄이고, ILP의 형식적 검증을 통해 규칙의 정확성과 일관성을 보장합니다.

- **Performance Highlights**: 저자들은 CLEVR-Hans와 ARC라는 논리 유도 벤치마크를 사용하여 ILP-CoT의 효율성을 검증하고 있습니다. 또한, 사용자가 제공한 소수의 이미지를 기반으로 텍스트-이미지 맞춤 생성을 위한 잠재적 응용 프로그램을 제안합니다. 이 접근법은 훈련 이미지에서 잠재적 논리 규칙을 유도하여, 이를 활용하여 사용자 선호에 부합하는 이미지를 생성할 수 있도록 합니다.
...
- **What's New**: 이번 논문에서는 Retrieval-Augmented Generation (RAG)를 대체하는 LDAR (Learning Distraction-Aware Retrieval)라는 새로운 방법을 제안합니다. LDAR는 대량의 텍스트를 처리할 때 발생하는 불필요한 정보(정보의 방해 요소)로부터 학습하며, 이러한 방해를 줄여주는 적응형(retriever) 모델입니다.

- **Technical Details**: 종래의 RAG는 외부의 최신 정보를 활용해 Large Language Models (LLMs)를 강화하는 프레임워크입니다. 하지만, LDAR는 기술적 한계를 극복하고, 128K 토큰 이상의 긴 입력 문서의 무분별한 처리로 인한 토큰 효율성을 향상시킵니다. 또한, 이 방법은 제한된 모델 용량에서 시각적 주의산만(distraction)을 줄여 LLM의 출력 품질을 높입니다.

- **Performance Highlights**: 다양한 LLM 아키텍처와 여섯 가지 지식 기반 기준을 통해 LDAR의 우수성을 입증하였습니다. 실험 결과, LDAR는 기존의 긴 컨텍스트(long-context) 방식에 비해 현저히 적은 토큰 사용으로 높은 성능을 달성했습니다. 이는 정보의 범위와 방해 요소 간의 균형을 유지하는 것이 중요하다는 점을 강조합니다.



### MolSpectLLM: A Molecular Foundation Model Bridging Spectroscopy, Molecule Elucidation, and 3D Structure Generation (https://arxiv.org/abs/2509.21861)
- **What's New**: 최근 분자 기초 모델(molecular foundation models)의 발전은 분자 특성 예측 및 de novo 분자 설계에서 인상적인 성과를 보여주고 있으며, 약물 발견과 반응 예측과 같은 분야에 유망한 응용을 가지고 있습니다. 기존 접근 방식들은 주로 SMILES표현(SMILES representations)에 의존해왔고, 3D 구조와 실험 스펙트럼을 간과했습니다. 이러한 한계를 극복하기 위해, 우리는 실험 스펙트로스코피와 분자 3D 구조를 통합한 MolSpectLLM을 제안합니다.

- **Technical Details**: MolSpectLLM은 3D 구조 정보와 실험 스펙트럼을 모두 포함하는 분자 기초 모델로서, NMR, IR, MS와 같은 스펙트럼을 명시적으로 모델링합니다. 이를 통해 각 스펙트럼 모드에 맞춰 설계된 표준 텍스트 설명을 생성하여 대규모 언어 모델이 실험 데이터를 효과적으로 해석하고 활용할 수 있도록 합니다. 실험적 스펙트럼 정보로부터 구조적 정보를 획득함으로써, 우리는 분자 명세서와 생성적인 분자 설계 간의 간극을 메울 수 있습니다.

- **Performance Highlights**: MolSpectLLM은 스펙트럼 관련 작업에서 최첨단 성능을 보이며, NMR, IR, MS 벤치마크에서 평균 정확도 0.53을 기록했습니다. 또한, Spectra-to-SMILES 작업에서 15.5%의 시퀀스 정확도와 41.7%의 토큰 정확도를 달성하여 대형 범용 LLM보다 현저히 뛰어난 성능을 발휘하고 있습니다. 이러한 결과는 MolSpectLLM이 분자 구조 해석 작업에서 강력한 성능을 제공하며, SMILES 또는 스펙트럼 입력에서 직접 3D 분자 구조를 생성할 수 있음을 보여줍니다.
...
### On the Complexity Theory of Masked Discrete Diffusion: From $\mathrm{poly}(1/ε)$ to Nearly $ε$-Fr (https://arxiv.org/abs/2509.21835)
Comments:
          44 pages

- **What's New**: 마스크된 이산 확산(Masked Discrete Diffusion)이라는 유연한 패러다임을 연구하였습니다. 이 방식은 텍스트 생성을 위해 점진적으로 토큰을 손상시킨 후 노이즈를 제거하는 과정을 포함합니다. 본 연구는 기존의 uniform discrete diffusion에 비해 마스크된 이산 확산이 갖는 실질적인 장점을 이론적으로 입증하고, 이를 위한 새로운 방법론인 MATU(Mask-Aware Truncated Uniformization)를 제안합니다.

- **Technical Details**: 연구팀은 마스크된 이산 확산에서의 전형적인 오일러 샘플러(Euler sampler)를 분석하였으며, 이를 통해 ε-TV 수렴을 달성하기 위해 필요한 이산 점수 평가의 수를 O~(d²ε⁻³/₂)로 보여주었습니다. MATU 방법을 통해 한정 점수 가정 없이도 이산 평가에서의 오차를 줄이면서, 연속 시간 마르코프 체인의 구조를 유지할 수 있음을 증명하였습니다.

- **Performance Highlights**: MATU는 ε-TV 수렴을 거의 ε-자유의 복잡도로 달성하여 기존 uniformization 방법의 O(ln(1/ε)) 복잡성을 초월하였고, 수렴 속도를 크게 향상시켰습니다. 본 연구는 마스크된 이산 확산이 실질적으로 uniform diffusion 보다 뛰어난 성능을 보임을 이론적 기초를 제공함으로써, 향후 확산 기반 언어 모델에 대한 분석에 기여할 것으로 기대됩니다.



### Preference-Guided Learning for Sparse-Reward Multi-Agent Reinforcement Learning (https://arxiv.org/abs/2509.21828)
- **What's New**: 이 논문에서는 리워드가 희소한 환경에서 온라인 다중 에이전트 강화 학습(online Multi-Agent Reinforcement Learning, MARL) 문제를 연구하고 있습니다. 기본적으로 리워드 피드백이 에피소드의 끝에서만 제공되는 형태의 환경을 다루며, 이러한 희소 리워드는 정책 학습을 비효율적으로 만듭니다. 이를 해결하기 위해, 저자들은 온라인 역선호 학습(online Inverse Preference Learning)과 다중 에이전트 온-정책 최적화(multi-agent on-policy optimization)를 통합한 새로운 프레임워크를 제안합니다.

- **Technical Details**: 본 연구는 선호 기반 가치 분해 네트워크(preference-based value-decomposition network)를 기반으로 하는 암묵적인 다중 에이전트 리워드 학습 모델을 도입하여 글로벌 및 로컬 리워드 신호를 생성합니다. 이러한 신호는 중앙 집중식 비평가(centralized critic)와 분산된 액터(decentralized actors)를 위한 차별화된 학습 목표를 구성하는 데 사용됩니다. 또한, 대형 언어 모델(LLMs)을 활용하여 학습된 리워드 모델의 품질을 향상시키는 방법도 보여줍니다.

- **Performance Highlights**: 저자들은 SMACv2 및 MAMuJoCo와 같은 최첨단 벤치마크에서 제안한 방법의 효과성을 empirically 평가했습니다. 결과적으로, 이 방법은 기존의 기준선 대비 뛰어난 성능과 샘플 효율성을 달성했으며, 희소 리워드 환경에서도 더 나은 최종 성과를 보였습니다. 추가적인 분석을 통해 지역적 및 글로벌 어드밴티지 스트림을 결합하는 것이 성능 향상에 중요한 역할을 함을 입증하였습니다.



### Sharpness-Aware Minimization Can Hallucinate Minimizers (https://arxiv.org/abs/2509.21818)
- **What's New**: 이 논문에서는 Sharpness-Aware Minimization (SAM)이 기존의 목적 함수의 최소값이 아닌 환각(minimized hallucinated) 지점으로 수렴할 수 있음을 보여주고 있습니다. 이러한 환각 최소값은 SAM이 깊은 학습 환경에서 발생할 수 있는 실패 모드를 나타내며, 기존의 국소 최소값(local minima) 또는 안장점(saddle points)과는 기본적으로 구별됩니다. 마지막으로, 이 문제를 피하기 위한 간단하면서도 효과적인 전략을 제안합니다.

- **Technical Details**: SAM은 데이터에 적합하도록 학습을 유도하며 동시에 sharpness를 저항하는 방법으로 설계되었습니다. 특정한 미분 가능한 함수 f에 대해 SAM은 objective를 최소화하는데, 여기서 perturbation radius ρ는 regularization의 강도를 조절합니다. 이 논문에서는 SAM의 환각 최소값의 존재를 이론적으로 증명하고, 이를 통해 어떻게 이러한 지점으로의 국소 수렴이 발생할 수 있는지 설명합니다.

- **Performance Highlights**: SAM은 다양한 깊은 학습 과제에서 뛰어난 성능을 보여주며 여러 실용적인 변형을 유도했습니다. 그러나 이 논문을 통해 SAM이 원래의 목적 함수의 최소값이 아닌 지점으로 수렴할 수 있다는 점이 강조되며, 이는 실제 환경에서 SAM의 수렴 결과에 대한 보장을 위협할 수 있습니다. 환각 최소값을 피하기 위한 실질적인 수치 실험이 제공되어, SAM의 일반화 성능을 저해하는 위험에 대한 경각심을 높이고 있습니다.



### Scaling Laws for Neural Material Models (https://arxiv.org/abs/2509.21811)
Comments:
          12 pages, 11 figures, preprint

- **What's New**: 이 논문에서는 배터리, 반도체 및 의료 기기 설계를 향상시키기 위해 재료 속성을 예측하는 데 딥 러닝을 활용하는 새로운 방법을 제시합니다. 연구팀은 모델 훈련 데이터, 모델 크기 및 컴퓨트 리소스(자원)가 재료 속성 예측에 미치는 영향을 분석했습니다. 특히, 변환기(transformer)와 EquiformerV2 신경망을 훈련시키고 경험적으로 스케일링 법칙을 발견하였습니다.

- **Technical Details**: OMat24 데이터셋은 1억 1천 8백만 개의 구조-속성 쌍으로 구성되어 있으며, 이는 재료 과학 분야에서 딥 러닝 모델 훈련에 사용되는 가장 큰 공개 데이터셋 중 하나입니다. 연구진은 신경 네트워크 모델의 훈련 데이터 크기, 모델 크기 및 자동 계산 리소스(FLOPs)에 따른 예측 성능을 분석하였습니다. 또한, 모델 설계와 훈련 전략을 최적화하기 위해 경험적으로 스케일링 법칙을 적용하였습니다.

- **Performance Highlights**: 이 연구의 결과는 물질 속성 예측 분야에서 신경망 기반 모델의 성능을 향상시키기 위한 방향성을 제시합니다. 예측 성능과 훈련 커다란 조합에 대한 파워 법칙 관계를 Establish 함으로써, 연구자들은 효율적인 자원 배분과 모델 설계를 위한 기준을 마련할 수 있습니다. 이 연구는 향후 약물 발견, 촉매 설계 등의 분야에 기여할 수 있는 중요한 기초 자료를 제공합니다.
...
### Exploring the Relationships Between Physiological Signals During Automated Fatigue Detection (https://arxiv.org/abs/2509.21794)
Comments:
          14 Pages, 12 Figures, 3 Tables

- **What's New**: 이번 연구는 생리 신호 쌍 간의 통계적 관계를 분석하여 피로 감지의 정확성과 강건성을 개선하는 데 중점을 두고 있습니다. 기존의 연구들은 주로 단일 모달리티에 초점을 맞췄으나, 본 연구에서는 여러 생리 신호의 조합을 통해 더 효과적인 피로 감지 시스템을 개발하고자 합니다. 특히, EMG와 EEG의 조합이 XGBoost 모델에서 최고 성능을 나타냈고, SHAP 분석을 통해 ECG와 EOG의 상관 관계가 중요한 특징으로 강조되었습니다.

- **Technical Details**: 연구는 DROZY 데이터셋을 활용하여 EEG, ECG, EMG, EOG 등의 생리 신호를 기반으로 15가지 신호 조합을 분석했습니다. 이들은 Decision Tree, Random Forest, Logistic Regression, XGBoost와 같은 다양한 머신러닝 분류기를 사용하여 피로 상태의 분류 능력을 평가했습니다. 또한, 신호 쌍 간의 상관관계와 공분산을 측정하기 위해 통계적 특징을 추출하여 모델의 성능을 강화했습니다.

- **Performance Highlights**: 모델의 성능을 평가하기 위해 정확성(accuracy), 정밀도(precision), 재현율(recall), ROC AUC 등의 지표가 사용되었습니다. 다중 신호 모델이 단일 신호 모델보다 지속적으로 높은 성능을 보였으며, 이로 인해 피로 모니터링 시스템의 신뢰성이 높아질 수 있다는 것을 확인했습니다. 연구의 결과는 피로 감지 시스템의 개발에 있어 응용 가능성이 높은 정보들을 제공합니다.
...
### Beyond Formula Complexity: Effective Information Criterion Improves Performance and Interpretability for Symbolic Regression (https://arxiv.org/abs/2509.21780)
- **What's New**: 이번 연구에서는 Effective Information Criterion (EIC)을 제안하여 기호 회귀(symbolic regression)에서 비합리적인 구조를 식별하는 접근 방식을 제공합니다. 기존의 기호 회귀 방법들은 주로 공식의 크기를 기준으로 해석 가능성을 판단하지만, EIC는 공식의 내적 구조를 고려하여 평가합니다. 특히, 물리학 공식의 수치적 안정성을 기반으로 하여, 데이터가 흐를 때 발생할 수 있는 소수점 자리 손실(loss of significant digits)을 분석하여 비합리적 구조를 식별합니다.

- **Technical Details**: EIC는 정보를 처리하는 시스템으로 공식을 바라보아, 유한한 처리 정밀도 하에서 최대 출력 정보를 기준으로 효율적인 정보 손실을 평가합니다. 이는 기존의 기호 회귀 알고리즘으로 발견된 모델의 구조적 합리성과 실제 물리적 공식 간의 간극을 드러냅니다. EIC는 기존의 검색 기반 기호 회귀 알고리즘에 적용되어 성능 향상과 구조적 비이성 감소에 기여하며, 생성 기반 알고리즘의 경우에는 샘플 효율성을 2~4배 향상시킬 수 있습니다.

- **Performance Highlights**: EIC를 활용한 연구 결과, 전통적인 복잡성 측정 방법과 달리 공식 내부 구조의 합리성을 평가하여 해석 가능성을 높였습니다. 또한, EIC를 통합한 기법은 더 나은 성과를 내었으며, 전통적인 방법에 비해 비합리적 구조를 가진 공식이 감소했습니다. 마지막으로, 비슷한 정확도와 복잡성을 가진 172개 공식 쌍을 대상으로 한 평가에서 EIC는 70.2%의 일치를 보이며, 해석 가능성 평가에서도 효과적임을 입증하였습니다.



### Machine Learning and AI Applied to fNIRS Data Reveals Novel Brain Activity Biomarkers in Stable Subclinical Multiple Sclerosis (https://arxiv.org/abs/2509.21770)
- **What's New**: 이번 연구에서는 다발성 경화증(Multiple Sclerosis, MS) 환자의 손 조작 능력과 인지적 피로에 대한 주관적 보고를 설명할 수 있는 뇌 활동 바이오마커를 탐색했습니다. 비침습적 신경영상 기법인 기능적 근적외선 분광법(fNIRS)을 사용하여 인지 및 운동 작업 중 뇌의 혈역학적 반응을 측정하였습니다. 총 15명의 MS 환자와 12명의 건강한 대조군을 대상으로 진행되었습니다.

- **Technical Details**: 연구 참가자들은 주 손으로 단일 작업과 이중 작업(Nine Hole Peg Test 진행 중 동일 손의 다섯 번째 손가락과 가는 손바닥 사이에 공을 잡고 있는 작업)을 수행하였습니다. fNIRS 데이터를 분석하기 위해 머신 러닝 프레임워크를 사용하여 양측 전두엽(prefrontal) 및 감각운동 피질(sensorimotor cortex)에서의 뇌 활성 패턴을 기반으로 MS 환자와 대조군을 분류했습니다. K-최근접 이웃 분류기(K-Nearest Neighbor classifier)는 단일 작업에서 75.0%, 복잡한 이중 작업에서 66.7%의 정확도를 달성했습니다.

- **Performance Highlights**: 해석 가능한 인공지능(XAI)을 사용하여 머신 러닝 모델에 기여하는 주요 뇌 영역이 우측 측두엽(supramarginal/angular gyri) 및 전중앙구(precentral gyrus)로 나타났습니다. MS 그룹은 이러한 영역의 활동이 억제되고 신경혈관 반응이 느리다는 결과가 발견되었습니다. 또한, 비산소화 헤모글로빈의 수준이 전통적인 산소화 헤모글로빈보다 더 나은 예측 인자로 나타났으며, 이는 개인 맞춤형 뇌 자극 목표 개발에 기여할 수 있는 새로운 뇌 활동 바이오마커를 제공하였습니다.
...
- **What's New**: 이번 연구에서는 전통적인 사차원 변동 자료 동화(4DVAR)의 비용 함수 최적화를 단순화하기 위해 신경망을 활용한 새로운 재매개변수화(neural reparameterization) 접근법을 제안합니다. 이 방법은 모든 시공간(state) 정보를 연속 함수로 표현하여 기존 4DVAR의 시간 종속성을 제거하고, 매개변수 공간에서 병렬 최적화를 가능하게 만듭니다. 또한 물리적 제약을 통합한 물리 정보 손실(physics-informed loss)을 통해 구현이 간소화되고 계산 비용이 감소합니다.

- **Technical Details**: 신경장(neural fields) 기반 접근법에서는 두 가지 전략을 사용하는데, 첫 번째는 초기 조건만 신경장으로 매개변수화하여 최적화 과정을 안정화시키는 것입니다. 두 번째는 전체 시공간 상태를 신경장으로 매개변수화하면서, 물리적 제약을 통해 물리 정보 손실을 포함시키는 방법입니다. 이러한 접근은 비선형성이 강한 4DVAR 문제를 해결하는 데 효과적이며, 계산 시간이 33% 감소하는 등의 장점을 나타냅니다.

- **Performance Highlights**: 제안된 방법은 전통적인 4DVAR 구현과 비교할 때 더 안정적인 초기 조건 추정치를 제공합니다. 특히 신경 매개변수화 방식은 참조 상태(ground-truth states)나 재분석 데이터에 대한 접근이 필요 없어, 관측 데이터가 제한적이거나 불완전한 상황에서도 적용 가능성이 높습니다. 이는 고차원의 시스템에 대한 확장 가능한 데이터 동화(data assimilation) 방법을 제시합니다.
...
- **What's New**: 이번 연구에서는 기존의 바이레벨 최적화(bilevel optimization) 문제에 대한 새로운 정보 이론적 접근법을 제안합니다. 특히, 흑백 상자(black-box) 함수가 포함된 상위 및 하위 레벨의 최적화 문제를 동시에 고려하여 정보를 최대화할 수 있는 기준을 수립하였습니다. 제안된 기법은 기존의 연구에서 다루어지지 않았던 동시 정보 이득(bilevel information gain)을 사용하여 두 레벨의 문제에 대한 이점을 측정합니다.

- **Technical Details**: 연구는 상위 레벨과 하위 레벨의 목표 함수들을 f와 g로 나타내며, 이들은 비용이 많이 드는 블랙박스 함수로 가정합니다. 상위 레벨 변수는 x, 하위 레벨 변수는 θ를 나타내며, 이들 각각은 정의된 입력 공간 내의 값으로 취급됩니다. 새로운 기준인 BLJES(Bilevel optimization via Lower-bound based Joint Entropy Search)는 정보를 효과적으로 평가할 수 있는 방법으로 자리 잡고 있으며, 특히 각 레벨에 대한 제약 문제를 다룰 수 있는 확장성을 제공합니다.

- **Performance Highlights**: 본 논문에서는 제안된 기법을 사용하여 Gaussian 프로세스와 여러 벤치마크 문제를 통해 실험적으로 효과를 입증하였습니다. 실험 결과, 정보 이론적 접근이 바이레벨 최적화 문제에 있어 기존 연구 대비 향상된 성능을 나타냄을 보여주었습니다. 또한, 제안된 방법은 coupled 및 decoupled 설정 모두에서 응용 가능성을 드러내며, 각기 다른 시뮬레이터로부터의 정보 관리를 효과적으로 수행할 수 있습니다.



### A Unifying Framework for Parallelizing Sequential Models with Linear Dynamical Systems (https://arxiv.org/abs/2509.21716)
Comments:
          Repo: this https URL

- **What's New**: 이번 연구는 비선형 재귀를 평가하는 고정점(iteration) 방법들인 Jacobi, Picard 및 Newton 이터레이션이 선형 동적 시스템(Linear Dynamical Systems, LDSs)이라는 공통 프레임워크 내에서 어떻게 이해될 수 있는지를 보여줍니다. 이러한 통합적 관점은 다양한 알고리즘 간의 연결을 통해, 순차 프로세스의 병렬화를 위한 이론적 기반을 명확히 해줍니다. 또한, 이러한 방법들이 각기 다른 기계 학습 문제에 어떻게 적용될 수 있는지에 대한 통찰도 제공합니다.

- **Technical Details**: 논문에서는 비선형 재귀를 평가하는 Newton, quasi-Newton, Picard와 Jacobi 이터레이션이 반복적으로 LDS를 평가하는 모델로 통합될 수 있음을 제시합니다. 재귀 관계는 기본적으로 선형 함수와 비선형 함수의 조합으로 구성될 수 있으며, 설정된 고정점 연산자는 수렴성을 보장받기 위해 독특한 고정점을 가져야 합니다. 이러한 구조는 현대의 하드웨어 가속기(GPUs, TPUs 등)를 최대한 활용할 수 있는 가능성을 제공합니다.

- **Performance Highlights**: 고정점 방법을 이용한 평행 처리 방식은 기존의 순차적 접근 방식과 비교하여 효율성을 극대화할 수 있음을 보여줍니다. 여러 유망한 기계 학습 문제들을 비선형 재귀로 재구성함으로써, 태스크에 가장 적합한 고정점 방법을 결정하는 데 중요한 정보를 제공합니다. 특히, 병렬 스캔(parallel scan) 알고리즘을 통해 많은 수의 프로세서를 활용한 계산 시간이 𝒪(log T)로 단축될 수 있습니다.



### PQFed: A Privacy-Preserving Quality-Controlled Federated Learning Framework (https://arxiv.org/abs/2509.21704)
- **What's New**: PQFed는 데이터의 이질성 문제를 해결하기 위해 설계된 개인화된 연합 학습 프레임워크입니다. 이 프레임워크는 연합 학습 프로세스 전에 각 클라이언트에 맞춘 훈련 전략을 설계하여 개인화 모델 성능을 향상시키는 데 중점을 둡니다. PQFed는 클라이언트의 데이터 유사성을 기반으로 클라이언트 선택 전략을 구현하여, 데이터 배포가 유사한 클라이언트와 협력할 수 있도록 지원합니다.

- **Technical Details**: PQFed는 초기 품질 관리를 수행하여 클라이언트 간의 호환성을 평가합니다. 이를 통해, 프라이버시를 유지하면서 클라이언트가 각자의 데이터 분포에 가장 적합한 소수의 다른 클라이언트와 협력할 수 있게 합니다. PQFed는 서버가 훈련한 특징 추출기를 사용하여 클라이언트의 로컬 데이터를 통일된 의미 공간에 투영하고, 지역적으로 차별화된 사양 증명을 통해 개인정보 보호를 강화합니다.

- **Performance Highlights**: PQFed는 CIFAR-10과 MNIST의 두 가지 벤치마크 데이터 세트에서 기존의 연합 학습 알고리즘과 함께 평가되었습니다. 실험 결과 PQFed는 제한된 참여자 수에서도 목표 클라이언트의 모델 성능을 지속적으로 개선함을 보여줍니다. PQFed는 낮은 참여 시나리오에서도 기존의 군집 기반 알고리즘인 IFCA보다 더 나은 성능을 발휘하며, 개인화된 연합 학습 환경에서의 확장성과 효율성을 입증했습니다.



### Downscaling human mobility data based on demographic socioeconomic and commuting characteristics using interpretable machine learning methods (https://arxiv.org/abs/2509.21703)
- **What's New**: 이 연구는 다양한 공간 수준에서 도시 인간 이동 패턴을 이해하는 데 초점을 맞추고 있습니다. 특히 뉴욕시의 택시 이동 데이터를 대형 공간 단위에서 소형 공간 단위로 다운스케일링(downscale)하기 위해 새로운 머신 러닝(framework) 프레임워크를 제시합니다.

- **Technical Details**: 연구에서는 OD(Origin-Destination) 여행과 인구 통계학적(demographic), 사회경제적(socioeconomic), 출퇴근(commuting) 특성 간의 상관관계를 네 가지 모델인 선형 회귀(Linear Regression), 랜덤 포레스트(Random Forest), 서포트 벡터 머신(Support Vector Machine), 신경망(Neural Networks)을 통해 개발했습니다. 비선형 모델의 변수 중요도를 해석하기 위해 돌발 기반 민감도 분석(perturbation-based sensitivity analysis)을 적용했습니다.

- **Performance Highlights**: 결과적으로 선형 회귀 모델은 복잡한 변수 상호작용을 정확히 포착하지 못했습니다. 반면, 신경망 모델이 훈련(training) 및 테스트(testing) 데이터셋에서 가장 우수한 성능을 보였고, 서포트 벡터 머신이 다운스케일링 성능에서 최고의 일반화(generalization) 능력을 보여주었습니다. 이 연구에서 제시한 방법론은 운송 서비스 개선 및 도시 개발을 위한 분석적 발전과 실용적 응용을 제공합니다.



### Exact Subgraph Isomorphism Network for Predictive Graph Mining (https://arxiv.org/abs/2509.21699)
- **What's New**: 이 논문에서는 Exact subgraph Isomorphism Network (EIN)을 제안하며, 이는 정확한 서브그래프 열거 (subgraph enumeration), 신경망 (neural network), 그리고 희소 정규화 (sparse regularization)를 결합합니다. 그래프 레벨 예측 모델을 구축하는 데 있어 높은 구분능력 (discriminative ability)과 해석 가능성 (interpretability)을 동시에 달성하는 것이 여전히 도전 과제로 남아있습니다. 이 연구는 입력 그래프의 서브그래프 구조에 대한 높은 구분능력을 가진 새로운 네트워크 구조를 소개합니다.

- **Technical Details**: EIN은 그래프 G∈𝒢의 분류 문제를 고려하고, 각각의 노드가 범주형 레이블을 가질 수 있는 구조를 갖습니다. 이 모델은 일반적으로 서브그래프 동형성 특성 (subgraph isomorphism feature, SIF)을 사용하여 입력 그래프가 특정 서브그래프를 포함하는지를 나타냅니다. 그룹 희소 정규화 (group sparse regularization)를 통해 포괄적인 서브그래프 선택 메커니즘을 구현하여 연산의 효율성을 높입니다.

- **Performance Highlights**: EIN은 기존의 그래프 신경망 모델에 비해 상당히 높은 예측 성능을 보여주며, 동시에 중요한 서브그래프를 식별할 수 있는 능력을 지속적으로 유지합니다. 본 논문에서는 합성 및 벤치마크 데이터셋을 기반으로 EIN이 표준 그래프 예측 모델과 비교하여 우수하거나 유사한 성능을 나타냄을 입증합니다. 이러한 결과는 EIN이 높은 해석 가능성을 갖춘 상태로 예측 품질을 유지하며, 효율적인 서브그래프 선택을 통해 성공적으로 모델 훈련이 가능하다는 것을 보여줍니다.



### Wav2Arrest 2.0: Long-Horizon Cardiac Arrest Prediction with Time-to-Event Modeling, Identity-Invariance, and Pseudo-Lab Alignmen (https://arxiv.org/abs/2509.21695)
Comments:
          Submitted to BPSC

- **What's New**: 최근 발표된 Wav2Arrest 2.0 (W2A2) 모델은 Photoplethysmography (PPG)을 기반으로 한 심정지 예측을 개선하기 위해 세 가지 독립적인 아이디어를 제안합니다. 첫 번째로, 이벤트 발생 시점을 예측하는 Time-to-Event (TTE) 모델링을 도입하여 예측 정확성을 높입니다. 두 번째로, 환자 개별 정보의 과메모리화를 방지하여 심정지와 관련된 일반화된 특성을 학습하도록 하고, 마지막으로, 미리 훈련된 네트워크를 통해 생성된 의사 실험실 값을 활용하는 회귀 분석을 통해 예측할 수 있습니다.

- **Technical Details**: W2A2 모델은 Serially 인코딩된 PPG 신호와 같은 단일 모드 데이터를 사용하여 심정지 예측을 개선하는 방법을 다룹니다. 이 모델은 시간에 대한 회귀 분석과 생존 모델을 포함한 TTE 모델링 기술을 사용하여 심정지 예측에 대한장기적인 심혈관 이벤트를 모델링합니다. 또한, 대규모 비식별 환자 생체 모델인 p-vector를 통해 환자 개별 정보에서 벗어난 특성을 학습하며, 이로 인해 모델의 과적합 문제가 감소합니다.

- **Performance Highlights**: 이 연구에 따르면 Wav2Arrest 2.0 모델은 24시간 시간 평균 AUC를 0.74에서 0.78-0.80로 개선하는 성과를 기록했습니다. 특히 긴 시간 동안의 예측 성능을 개선하면서 사건 발생 시점 근처에서의 성능 저하는 최소화하여 조기 경고 시스템 연구를 한 단계 끌어올렸습니다. 모델은 경쟁 손실 간의 높은 그래디언트 충돌 비율을 인식하고, PCGrad 최적화 기법을 사용하여 이를 완화하는 방식으로 다중 목표 학습을 구현했습니다.



### SpecMER: Fast Protein Generation with K-mer Guided Speculative Decoding (https://arxiv.org/abs/2509.21689)
Comments:
          Accepted as spotlight at NeurIPS 2025

- **What's New**: 이 논문은 SpecMER(Speculative Decoding via k-mer Guidance)라는 새로운 프레임워크를 제안하여 단백질 서열 생성에서 생리학적, 구조적, 기능적 우선 순위를 통합합니다. 기존의 모델이 구조 및 기능 제한을 무시하고 생물학적으로 타당하지 않은 결과를 초래하는 문제를 해결하고자 합니다. SpecMER는 다중 서열 정렬에서 추출한 k-mer 모티프를 활용하여 후보 서열을 병렬로 점수화하고, 알려진 생물학적 패턴에 가장 일치하는 서열을 선택합니다.

- **Technical Details**: 이 논문에서는 생물학적, 구조적, 기능적 선행 지식을 결합하여 SpecMER을 개발합니다. k-mer는 짧은 연속 아미노산 서브시퀀스로, 이 구조적 모티프는 단백질의 접힘 및 기능과 상관관계가 있습니다. SpecMER 프레임워크는 ProGen2-S를 드래프트 모델로, ProGen2-M을 타겟 모델로 활용하여, 시퀀스 생성을 가속화합니다.

- **Performance Highlights**: SpecMER은 표준 자기 회귀 디코딩에 비해 24-32%의 속도 향상을 달성하며, 수용률(acceptance rates)과 시퀀스 가능성(likelihood)을 개선합니다. 이는 고처리량 단백질 스크리닝에서 수천 개의 단백질 서열을 생성하고 스크리닝하는 데 필요한 효율성을 제공합니다. 최종적으로  인간이 자연적으로 디자인한 단백질과 더 유사한 서열을 생성할 수 있도록 돕습니다.



### Prophecy: Inferring Formal Properties from Neuron Activations (https://arxiv.org/abs/2509.21677)
- **What's New**: Prophecy는 feed-forward 신경망의 공식적인 속성을 자동으로 추론하는 도구로, 뉴런의 활성 상태가 중요한 로직을 형성한다는 점에서 출발합니다. 이 도구는 뉴런의 활성화 값이나 on/off 상태를 기반으로 규칙을 추출하여 원하는 출력 속성을 암시하는 조건을 설정합니다. 다양한 모델 및 출력 속성에 대한 사용을 시연하며, 심층적인 분석을 통해 신경망의 동작을 뒷받침하는 명확한 설명을 제공합니다.

- **Technical Details**: Prophecy 프레임워크는 사전 훈련된 모델(FF), 출력 속성(PP), 그리고 모델에 적합한 데이터 세트(DD)를 입력으로 받습니다. 뉴런의 활성화 값을 수집하여 사용하고, 이 활성화 값에 기반한 조건을 통해 딥러닝 모델의 출력 속성을 결정하는 데 사용됩니다. Decision-tree learning 기법을 활용하여 뉴런 패턴을 규명하고 이를 통해 모델의 행동을 예측하는 규칙을 생성합니다.

- **Performance Highlights**: Prophecy는 DNN 모델의 복잡성을 간소화하여 분석이 용이한 규칙 세트로 분해합니다. 이를 통해 동작 보장, 테스트 및 디버깅, 인간이 이해할 수 있는 개념으로의 분석이 가능해집니다. 여러 산업(예: NASA, Boeing)과 학계에서의 활용 사례가 많아, 신경망의 투명성과 신뢰성을 높이는 데 기여합니다.



### Scalable Second-order Riemannian Optimization for $K$-means Clustering (https://arxiv.org/abs/2509.21675)
- **What's New**: 이번 논문에서는 K-평균 클러스터링 문제에 대한 새로운 수학적 공식을 제안하고, 이를 Riemannian 구조를 통해 두 번째 차수의 cubic-regularized Riemannian Newton 알고리즘을 사용해 해결할 수 있도록 만듭니다. 이 접근 방식은 기존의 많은 알고리즘들이 가진 제약과 최적 목표 간의 균형 문제를 해결하는 데 도움을 줍니다.

- **Technical Details**: K-평균 문제는 smooth unconstrained optimization으로 재구성되며, 새로운 Riemannian manifold의 구조가 도입됩니다. 작은 차원의 제품 다양체로 나누어져 각 Newtown 하위 문제를 선형 시간 내에 처리할 수 있어, 전체 실행 시간을 크게 줄일 수 있습니다.

- **Performance Highlights**: 제안된 방법은 기존의 비부정합 low-rank 분해 방법보다 훨씬 더 빠르게 수렴하며, 비슷한 통계적 정확도를 달성합니다. 특히, 평균 경우에서 제안된 알고리즘은 전통적인 방법에 비해 두 번째 차수의 임계점을 효율적으로 계산할 수 있는 새로운 성능을 보여줍니다.
...
### Neuroprobe: Evaluating Intracranial Brain Responses to Naturalistic Stimu (https://arxiv.org/abs/2509.21671)
Comments:
          31 pages, 7 main figures

- **What's New**: 이번 논문에서는 Intracranial EEG (iEEG) 데이터셋을 활용하여 Neuroprobe라는 디코딩 프레임워크를 소개합니다. Neuroprobe는 뇌의 다중 모달 언어 처리 연구를 위한 다양한 디코딩 태스크를 포함하며, 이를 통해 기존의 비침습적 EEG 기록 및 평가 방식의 한계를 극복하고자 합니다. 특히, 실험을 통해 높은 시간 및 공간 해상도를 유지하며 신경 활동을 시각화하고, 복잡한 언어 처리와 관련된 뇌의 정보를 분석합니다.

- **Technical Details**: Neuroprobe는 BrainTreebank 데이터셋을 기반으로 하여 15개의 디코딩 태스크를 정의합니다. 이 태스크는 오디오, 비전, 언어 도메인을 포괄하며, iEEG 기술을 활용하여 높은 주파수 신호를 효과적으로 기록합니다. Neuroprobe는 신경과학 분야의 다양한 문제에 대해 생명과학자들이 간편하게 접근할 수 있도록 설계되어 있으며, 커뮤니티의 참여를 유도하고 있습니다.

- **Performance Highlights**: Neuroprobe는 iEEG 모델들의 성능을 평가하는 엄격한 기준을 마련합니다. 초기 실험 결과, 기존의 현대적인 기초 모델들이 Linear baseline보다 부족한 성능을 보였으며, Neuroprobe를 통해 모델 간의 성능 비교를 쉽게 수행할 수 있습니다. 궁극적으로, Neuroprobe는 다중 모달 감각 처리를 이해하는 데 중요한 기여를 할 것으로 기대됩니다.
...
### MMPlanner: Zero-Shot Multimodal Procedural Planning with Chain-of-Thought Object State Reasoning (https://arxiv.org/abs/2509.21662)
Comments:
          17 pages, 9 figures, 14 tables, Findings of the Association for Computational Linguistics: EMNLP 2025

- **What's New**: 이번 연구에서는 Multimodal Procedural Planning (MPP) 분야에서 새로운 접근방식인 MMPlanner를 제안합니다. MMPlanner는 제로샷(zero-shot) 프레임워크를 사용하여 이미지와 텍스트를 결합한 정확한 멀티모달 계획을 생성하며, 객체 상태 변화에 대한 체계적인 모델링을 통해 이 문제를 해결합니다. 또한, 기존의 접근 방식들이 시각적 객체 상태 정렬과 체계적 평가를 충분히 고려하지 못한 점을 강조하며, 새로운 평가 프로토콜을 제안합니다.

- **Technical Details**: MMPlanner는 Object State Reasoning Chain-of-Thought (OSR-CoT) 프로그래밍을 통해 시각적 단계 간의 객체 상태 전환을 명시적으로 모델링합니다. 이를 통해 표현된 시각적 정보는 텍스트와 함께 적절한 시각적 지침을 제공하여 사용자에게 더 나은 이해를 도와줍니다. 연구에서 MMPlanner는 RECIPEPLAN과 WIKIPLAN 데이터셋을 이용하여 실험을 수행했으며, 이 결과는 기존의 상태-of-the-art (SoTA)보다 상당한 성과를 보였습니다.

- **Performance Highlights**: MMPlanner는 텍스트 기반 계획에서 6.8%, 교차 모달 정렬에서 11.9%, 시각적 단계 정렬 정확도에서 26.7% 향상을 기록했습니다. 이 프레임워크는 이전 단계의 문맥을 활용하여 작업 단계 간의 명확한 객체 상태 모델링을 통해 추론 시간을 약 46.25% 줄였습니다. 또한, 새로운 평가 프레임워크는 인간의 평가와 강한 상관관계를 나타내어 자동화된 평가의 효율성을 입증합니다.



### A Systematic Review of Conformal Inference Procedures for Treatment Effect Estimation: Methods and Challenges (https://arxiv.org/abs/2509.21660)
Comments:
          13 pages, 3 figures

- **What's New**: 이 연구에서는 치료 효과 추정을 위한 conformal prediction(컨포멀 예측) 방법론에 대한 체계적인 검토를 실행하며, 이는 데이터 기반 의사 결정을 위한 중요한 진전을 나타냅니다. 기존의 머신러닝 모델들이 제공하지 못하는 예측의 불확실성을 정량화할 수 있는 가능성을 제시합니다. 이 방법론은 특히 헬스케어, 경제학, 공공 정책과 같은 높은 이해당사리 환경에서 의사 결정 개선에 기여할 수 있는 잠재력을 가지고 있습니다.

- **Technical Details**: Conformal prediction은 다양한 점 추정 모델의 출력을 안정화하여 유효한 예측 구역을 컴퓨팅할 수 있게 하는 방법입니다. 이 논문에서 다루는 split conformal prediction은 데이터를 훈련, 보정 및 테스트 세트로 나누어 최소한의 가정 하에 추정 모델의 예측에 대한 통계적으로 유효한 진술을 제공하도록 설계되었습니다. 모델이 예측한 결과는 이미 설정된 한계를 기반으로 하여 불확실성을 정량화하고, 결과적으로 의사 결정에 도움이 되는 정보 제공이 가능합니다.

- **Performance Highlights**: 이 연구에서 소개된 11개 핵심 논문들은 현대 confromal prediction의 최첨단 방법들을 규명하고 있습니다. 이 방법들이 다양한 문제 유형에 얼마나 적합한지에 대한 분석이 이루어졌으며, 그 결과들은 향후 연구 방향을 제안하기 위한 기초 자료를 제공할 것입니다. 전체적으로, 이 체계적인 검토는 치료 효과 추정 방법에 대한 새로운 접근법을 탐구하는 연구자들에게 중요한 정보를 제시합니다.



### RED-DiffEq: Regularization by denoising diffusion models for solving inverse PDE problems with application to full waveform inversion (https://arxiv.org/abs/2509.21659)
- **What's New**: 본 논문에서는 PDE(Partial Differential Equation) 기반의 역문제를 해결하기 위한 새로운 계산 프레임워크인 RED-DiffEq를 소개합니다. RED-DiffEq는 물리 기반의 역전 방식과 데이터 기반의 학습을 통합하여, 기존의 제어 방식보다 더 높은 정확도와 강인성을 보여주는 방법입니다. 이 방법은 geophysics의 full waveform inversion 문제에 적용되어, 지하 속도 모델을 재구성할 수 있습니다.

- **Technical Details**: RED-DiffEq는 사전 훈련된 diffusion 모델을 사용하여 PDE 기반의 역문제에 정규화 메커니즘으로 활용합니다. 이 프레임워크는 노이즈가 있는 데이터에서 합리적인 해를 갖는 속도 모델을 위한 prior distribution을 학습하며, 이를 통해 보다 견고한 해를 제공합니다. 특히, 이는 현재의 데이터와 예측 데이터 간의 잔차를 계산하여 정규화 효과를 달성합니다.

- **Performance Highlights**: 실험 결과, RED-DiffEq는 기존의 정규화 방법과 비교하여 정확도와 강인성이 뛰어난 성능을 보였습니다. OpenFWI 데이터셋에서 훈련된 후, Marmousi 및 Overthrust와 같은 벤치마크 모델에 대한 강한 일반화 능력을 보여주었습니다. 이는 실제 필드 데이터에 적용할 수 있는 잠재력을 강조합니다.



### Differentiable Structure Learning for General Binary Data (https://arxiv.org/abs/2509.21658)
Comments:
          33 pages, 6 figures, to appear at NeurIPS 2024

- **What's New**: 기존의 이산 데이터에 대한 미분 가능한 구조 학습 방법은 특정 구조적 방정식 모델(Structural Equation Models, SEMs)에서 생성된 데이터라는 가정을 전제로 하고 있습니다. 하지만 이러한 가정은 실제 데이터 생성 과정과 일치하지 않아 일반적인 적용 가능성이 제한됩니다. 본 연구에서는 이산 변수 간의 임의의 종속성을 포착할 수 있는 차별화 가능한 구조 학습 프레임워크를 제안합니다.

- **Technical Details**: 이 프레임워크는 관찰적 데이터만으로 DAG(Directed Acyclic Graph)를 비식별성(non-identifiability)에 따라 완전히 특성화할 수 있음을 보여주며, 관찰된 분포와 일치하는 충분히 희소한 DAG를 선택하는 방법을 제공합니다. 문제를 단일 미분 최적화 작업으로 공식화하여, 이전 방법들이 채택한 비현실적인 단순화를 피했습니다. 우리의 방법은 선형성(linearity)이나 가법성(additivity) 같은 가정을 피하면서, 복잡한 인과 관계를 모델링할 수 있도록 설계되었습니다.

- **Performance Highlights**: 실험 결과, 우리 방법은 이산 데이터의 복잡한 관계를 효과적으로 포착할 수 있음을 입증하였습니다. 그리고 두 단계 접근법인 Notears-MLP-Reg를 도입하여 계산 복잡성을 줄이고 기존의 기법들을 초월하는 성능을 보여주었습니다. 이로써, 높은 차원의 이산 데이터에 대한 확장 가능하고 유연한 접근법의 필요성을 강조하였습니다.



### DriftLite: Lightweight Drift Control for Inference-Time Scaling of Diffusion Models (https://arxiv.org/abs/2509.21655)
- **What's New**: Diffusion 모델의 추론(추정) 시간 확장 기술을 다룬 이번 연구에서는, 사전 훈련된 모델을 재훈련 없이 새로운 목표 분포(target distribution)로 적응시키는 방법을 제안합니다. 기존 메서드는 단순하지만 바이어스를 발생시키고, 입자 기반 보정 방법은 계산 비용이 높고 가중치 감쇠(weight degeneracy) 문제를 가지고 있습니다. 새로 도입된 DriftLite는 경량의 훈련 없는 입자 기반 접근법으로, 메모리와 계산 비용을 최소화하면서도 최적의 안정성(Numerical Stability) 제어를 통해 추론 동적을 유도합니다.

- **Technical Details**: 본 연구에서는 DriftLite를 소개하며, 이 방법은 Fokker-Planck 방정식의 드리프트(drift)와 입자 포텐셜(particle potential) 간의 새로운 자유도를 활용합니다. DriftLite는 Variance-Controlling Guidance (VCG)와 Energy-Controlling Guidance (ECG)라는 두 가지 실용적인 구현을 제공합니다. 이 메서드는 각 시간 단계에서 작고 추가적인 선형 시스템(linear system)을 푸는 것만으로도 가능합니다.

- **Performance Highlights**: 드리프트와 무작위 샘플링에서 DriftLite는 고차원 가우시안 혼합 모델, 분자 입자 시스템, 그리고 대규모 단백질-리간드 공동 접힘 문제에서 일관되게 변동성을 줄이며 샘플 품질을 향상시킵니다. 이 연구의 결과는 Diffusion 모델의 확장 가능한 추론적 적응을 위한 원칙적이고 효율적인 경로를 보여줍니다. DriftLite는 현재의 기법들에 비해 체계적으로 샘플 품질을 높였습니다.
...
### Understanding and Enhancing Mask-Based Pretraining towards Universal Representations (https://arxiv.org/abs/2509.21650)
Comments:
          NeurIPS 2025

- **What's New**: 이 논문에서는 마스크 기반 프리트레이닝(mask-based pretraining)의 작용 메커니즘을 명확히 하고 그 한계를 고차원 최소 노름 선형 회귀(minimum-norm linear regression)와 연관 지어 설명합니다. 새로운 이론적 프레임워크를 구축하여 마스크 예측을 위한 테스트 리스크(test risk)를 정의하고, 이를 다양한 신경망 아키텍처에 적용하여 실제 성능 개선을 입증했습니다. 특히, 새로운 프리트레이닝 스킴인 Randomly Random Mask AutoEncoding (R²MAE)을 제안하여 고정된 마스크 비율을 대신함으로써 성능을 향상시키는 데 성공했습니다.

- **Technical Details**: 논문은 마스크 기반 프리트레이닝의 성능을 고차원 선형 회귀 설정에서 설명하고, 그 결과를 통해 다양한 신경망의 마스크 비율과 관련된 질적 및 정량적 행동을 정립합니다. 우리의 연구에서는 랜덤 행렬 이론(random matrix theory)을 활용하여 테스트 리스크에 대한 명시적 표현식을 도출했습니다. 결과적으로 마스크 기반 프리트레이닝이 오버파라미터화된 레짐(overparametrized regime)에서만 유용하며, 최적의 마스크 비율은 작업(task) 및 모델 크기(model size)에 의존한다는 점을 확인했습니다.

- **Performance Highlights**: 제안하는 R²MAE는 비전, 언어, DNA 시퀀스 및 단일 세포 모델에서 일관되게 기준 마스크 방식 및 다른 향상 전략들을 초월하는 성능 향상을 보여줍니다. 특히, 다양한 작업에서 모델이 다양한 피처 스케일(feature scale)을 캡처하도록 강제하여 고정된 최적 마스크 비율을 초과하는 성능을 발휘했습니다. 이를 통해 최신 상태의 선진 모델들이 향상된 성능을 나타내게 되었습니다.



### Blockwise Hadamard high-Rank Adaptation for Parameter-Efficient LLM Fine-Tuning (https://arxiv.org/abs/2509.21637)
- **What's New**: 이번 연구에서는 Block Hadamard High-Rank Adaptation (BHRA)이라는 새로운 기법을 제안합니다. BHRA는 모든 가중치 행렬을 블록으로 나누어 그 안에서 독립적으로 작용하는 HiRA 스타일의 곱셈 조정을 적용함으로써, PEFT(파라미터 효율적인 파인튜닝)의 한계를 극복합니다. 이 방식은 parameter footprint를 유지하면서도 로컬 집합에 대한 rank 증대를 가능하게 합니다.

- **Technical Details**: BHRA는 각 가중치 행렬을 b×b 그리드로 나누고, 각 블록 내에서 독립적으로 HiRA 스타일의 조정을 적용합니다. 블록 (i,j)의 경우, 업데이트는 ΔWi​j=W0,i​j⊙(Bi​j​Ai​j)로 계산되며, 이를 통해 자원 사용을 최적화하면서도 더 높은 rank를 구현할 수 있습니다. 이러한 설계는 Hadamard 어댑터의 계산 비용을 유지하되, 하위 작업에서의 요구에 맞춰 가용 용량을 더욱 정확하게 배분할 수 있도록 합니다.

- **Performance Highlights**: BHRA는 Llama-3.2 1B, Mistral-7B, Gemma-2 9B를 포함하여 8개의 일반 상식 추론 작업과 2개의 산술 벤치마크에서 강력한 PEFT 기준선을 지속적으로 초과하는 성과를 보였습니다. 특히, 높은 rank에서 가용 용량을 재분배하는 능력이 가장 큰 이점을 제공하며, 이는 전체적인 성능 향상으로 이어집니다.



### Shoot from the HIP: Hessian Interatomic Potentials without derivatives (https://arxiv.org/abs/2509.21624)
Comments:
this https URL

- **What's New**: 본 논문에서는 Hessian을 자동 미분이나 유한 차분에 의존하지 않고 심층 학습 모델을 통해 직접 예측할 수 있음을 보여준다. 이는 그래프 신경망에서의 메시지 전송 중 계산된 불가약 표현(irrep) 특징을 통해 가능하다. 그 결과, HIP Hessians는 대폭 빠르고 정확하며 메모리 효율성을 높이며, 시스템 크기에 따라 더 유리한 확장을 가능하게 한다.

- **Technical Details**: HIP(Hessian Interatomic Potentials)은 전통적인 방법보다 훨씬 낮은 계산 비용으로 Hessian을 직접 예측할 수 있도록 설계된 SE(3)-등가 대칭 Hessians를 생성하는 방법이다. 이 방법은 원자 간의 메시지 전송을 통해 할당된 구면 조화(bispherical harmonic) 특징을 이용하여 Hessian을 구축한다. 이에 따라, 예측 속도는 기존 자동 미분 방식에 비해 평균 10배에서 70배까지 증가하는 동시에 메모리 사용량은 2-3배 감소한다.

- **Performance Highlights**: Hessian의 예측 정확성은 기준 데이터세트에서 기존 방식들보다 우수함을 입증했으며, 특히 Hessian MAE는 2배 낮고, 고유값 고유 벡터의 코사인 유사도는 1.5배 향상되었다. 또한, 제로포인트 에너지 수정, 2차 기하학 최적화, 전이 상태 검색, 진동 분석 등 다양한 다운스트림 작업에서 일관되게 우수한 성능을 보였다. HIP의 코드베이스와 모델 가중치도 오픈소스로 제공되어 추가 개발이 가능하도록 했다.



### PreLoRA: Hybrid Pre-training of Vision Transformers with Full Training and Low-Rank Adapters (https://arxiv.org/abs/2509.21619)
Comments:
          7 pages, 7 figures, 2 algorithms, 1 table, conference paper

- **What's New**: 이번 연구에서는 대규모 모델의 학습 효율성을 높이기 위해 Low-Rank Adaptation (LoRA) 기법을 사전 학습에 통합하는 PreLoRA 프레임워크를 제안합니다. 기존의 방법들과는 달리 PreLoRA는 사용자 정의 하이퍼파라미터를 통해 전이 시점을 조정할 수 있으며, 각 모듈 레이어의 수렴 정도에 따라 특정한 랭크를 할당합니다. 이로 인해 모델의 정확성을 유지하면서도 10%로 줄어든 학습 가능한 파라미터 수로 학습 효율성을 3배 개선하고 평균 학습 시간을 1.5배 단축할 수 있음을 보여줍니다.

- **Technical Details**: PreLoRA는 두 가지 주요 전략에 기반합니다. 첫째, 전체 파라미터에서 LoRA 기반 학습으로 안전하게 전환할 수 있는 적절한 시점을 결정하는 것입니다. 이를 위해 모듈 레벨에서의 부분 수렴을 평가하는 기준을 도입하고, 이 기준의 엄격성을 사용자가 조정할 수 있도록 하여 유연성을 부여합니다. 둘째, LoRA로 전환하는 경우, 가중치 노름 수렴 정도에 따라 타겟 모듈의 개별 레이어에 역동적으로 랭크를 할당하는 알고리즘을 통해 모델을 계속 학습합니다.

- **Performance Highlights**: 실험 결과, PreLoRA는 Vision Transformer 모델에 대해 10%의 원래 파라미터로 학습 가능한 수를 줄이면서도 정확도를 유지했습니다. 학습 효율성이 3배 개선되고, 평균 학습 시간은 1.5배 단축되며, GPU 메모리 사용량 또한 20% 감소하였습니다. 이러한 성능 개선은 대규모 모델의 학습에서 자원의 효율적인 활용을 가능하게 하며, 여러 도메인에서의 응용 가능성을 높입니다.
...
### Causal Abstraction Inference under Lossy Representations (https://arxiv.org/abs/2509.21607)
Comments:
          35 pages, 8 figures, published at ICML 2025

- **What's New**: 이번 논문에서는 새로운 유형의 추상화인 projected abstractions를 도입하여 기존의 정의를 일반화합니다. 이러한 projected abstractions는 정보 손실을 수반하는 경우에도 일관된 수학적 접근을 가능하게 하여, 고차원 데이터에서의 인과 추론을 용이하게 합니다. 주요한 기여 중 하나는, 보다 안정적인 추상화를 통해 수집된 저차원 데이터로부터 고차원 인과 질문을 식별하고 추정할 수 있는 그래픽 기준을 제시하는 것입니다.

- **Technical Details**: 인과 추론 및 추상화는 구조적 인과 모델(SCM)과 관련이 있으며, 이 논문에서는 AIC(Abstract Invariance Condition)를 초월하는 새로운 접근 방식을 설명합니다. 기존의 인과 모델에서 세 가지 구분층(관찰, 개입, 반사)을 설명한 후, AIC의 위반을 고려한 새로운 형태의 추상화기법을 고안합니다. 이를 통해 한번의 개입도 다르게 수행된 다수의 저차원 개입이 동일한 고차원 개입으로 매핑될 수 있음을 보여줍니다.

- **Performance Highlights**: 실험 결과, 고차원 이미지 설정에서도 projected abstraction 모델이 효과적으로 작동함을 확인했습니다. 특히 제한된 저차원 데이터로부터 고차원 인과 추론을 수행할 수 있는 가능성을 시연하였고, 이는 고차원 데이터의 복잡성 속에서도 유용하게 적용될 수 있는 방법론을 제시합니다. 따라서 이 연구는 보다 발전된 인공지능 시스템 구축에 필요한 인과적 사고와 추상적 개념화의 융합이 가능하다는 것을 입증합니다.



### Task-Agnostic Federated Continual Learning via Replay-Free Gradient Projection (https://arxiv.org/abs/2509.21606)
- **What's New**: 이번 논문에서는 연합 지속 학습(Federated Continual Learning, FCL) 문제를 해결하기 위해 연합 그래디언트 투영 기반 지속 학습(FedProTIP) 프레임워크를 제안합니다. FedProTIP는 작업 식별 예측(Task Identity Prediction)을 결합하여, 이전에 학습한 표현의 직교 보조 공간에 클라이언트 업데이트를 투영함으로써 치명적인 망각(catastrophic forgetting)을 완화합니다. 이 방식은 작업 간의 간섭을 줄이고 작업 시퀀스 전반에 걸쳐 성능을 유지합니다.

- **Technical Details**: FedProTIP는 각 클라이언트가 로컬 훈련 후 레이어별 활성화를 수집하고, 이를 기반으로 랜덤화된 특이값 분해(Singular Value Decomposition, SVD)를 수행하여 작업의 특성 공간의 핵심 기저(core bases)를 추출합니다. 이러한 기저는 서버에 전송되어 투영 행렬을 구축하고, 클라이언트는 이를 통해 로컬 모델을 미세 조정하고 이전 작업의 특징을 보존하는 방식으로 업데이트를 수행합니다. 또한, FedProTIP는 인퍼런스 시에 가장 가능성 있는 작업 식별(taks identity)을 추론하여 출력 결과를 동적으로 조정합니다.

- **Performance Highlights**: 다수의 지속 이미지 분류 벤치마크에서 진행된 실험 결과, FedProTIP는 기존의 최첨단 FCL 방법들을 일관되게 능가하며, 정확도는 최소 4.3%에서 최대 47%까지 개선되었습니다. 다양한 벤치마크 데이터셋에서 FedProTIP가 이전 상태보다 높은 평균 정확도를 달성했음을 보여주며, 이는 작업 식별이 사전에 알려지지 않은 환경에서도 큰 성능 향상을 가져온다는 점에서 주목할 만합니다.



### GenUQ: Predictive Uncertainty Estimates via Generative Hyper-Networks (https://arxiv.org/abs/2509.21605)
Comments:
          9 pages, 6 figures

- **What's New**: 이 논문에서는 GenUQ라는 새로운 방법을 소개합니다. GenUQ는 생성적 하이퍼 네트워크 모델을 도입하여, 관측된 데이터와 일치하는 파라미터 분포를 생성합니다. 이 방법은 기존의 likelihood 기반 접근 방식을 피하면서도 불확실성을 정량화합니다.

- **Technical Details**: GenUQ는 확률론적 모델을 통해 데이터의 변동성으로 인한 예측 불확실성을 추정합니다. 이 과정에서 생성적 하이퍼 네트워크는 전통적인 오퍼레이터 학습 모델과 함께 학습되며, 복잡한 likelihood 함수에 대한 가정이 필요 없습니다. 이로 인해 높은 차원의 매개 변수를 소수의 샘플로 변환하여 예측 분포를 적절히 생성합니다.

- **Performance Highlights**: 실험 결과, GenUQ는 제작된 오퍼레이터 재구성, 확률적 엘립틱 PDE 솔루션 학습 및 긴장 하의 다공성 강철 고장 위치 모델링 등 세 가지 문제를 해결하며 다른 불확실성 정량화 방법들보다 뛰어난 성능을 보였습니다.
...
### Interpretable time series analysis with Gumbel dynamics (https://arxiv.org/abs/2509.21578)
Comments:
          15 pages, 5 figures

- **What's New**: 이 논문에서는 Gumbel Dynamical Model (GDM)을 제안합니다. GDM은 이산 상태를 연속적으로 완화하고, Gumbel 분포를 이용한 새로운 노이즈 모델을 도입하여 부드러운 전이와 비정상적인 동적 시스템을 효과적으로 모델링할 수 있게 합니다. GDM은 또한 완전 미분 가능하여 표준 경량화 방법으로 빠르고 확장 가능한 훈련이 가능합니다.

- **Technical Details**: GDM은 Gumbel-Softmax 트릭을 활용하여 이산 확률 변수의 연속적 완화를 가능하게 합니다. 이 과정에서, Gumbel 노이즈를 샘플링하고 이를 통해 지표(logits)를 변형하여 Gumbel-Max 트릭을 사용해 상태를 모델링합니다. 이 모델에서는 파라미터를 조정하여 상태 전이에 대한 불확실성을 명시적으로 고려할 수 있습니다.

- **Performance Highlights**: GDM은 복잡한 시뮬레이션 데이터셋과 실제 데이터셋에서 성능을 평가하였으며, 기존의 경쟁 모델들과 비교했을 때 더 빠른 학습 속도와 해석 가능한 상태 추정을 제공합니다. 특히, GDM은 연속적이고 비선형적인 동적 시계열 데이터를 효과적으로 모델링할 수 있는 능력을 입증하였습니다.



### Machine Learning. The Science of Selection under Uncertainty (https://arxiv.org/abs/2509.21547)
- **What's New**: 이 논문은 자연적 또는 인공적 학습이 후보 옵션의 선택 과정을 통해 이루어진다는 점을 강조합니다. 기계 학습에서 선택은 데이터에 기반한 예측 규칙의 정확성에 대한 실증적 추정을 바탕으로 이루어지며, 데이터 샘플링의 랜덤성으로 인해 이러한 추정이 본질적으로 노이즈를 포함하게 됩니다. 따라서 불확실성 하에서 선택의 결과에 대한 이론적 보장을 제공하기 위해 통계적 도구를 소개합니다.

- **Technical Details**: 통계적 도구로 시작하여, 기대값의 실증적 추정이 실제 기대값에서 얼마나 벗어나는지를 제어하는 주요 통계적 수단인 측정 중심 불평등(concentration of measure inequalities)을 설명합니다. Markov, Chebyshev, Hoeffding, Bernstein, Empirical Bernstein, Unexpected Bernstein, KL, split-KL 등 다양한 불평등을 다루며, 각각의 불평등을 통해 일반화 경계를 도출하는 도구를 제공합니다. 오프라인 감독 학습에서 발생하는 문제를 분석하고, 그 다음 온라인 학습 환경으로 전환하여, 알고리즘의 성능을 비교하고 잃어버린 값인 'regret'을 도출하는 방법을 제시합니다.

- **Performance Highlights**: 논문은 오프라인 및 온라인 학습 설정에서의 다양한 성과를 강조합니다. 특히, 오프라인 감독 학습에서는 예시를 통해 예측 규칙의 기본 개념을 설명하고, KK 최근접 이웃 알고리즘(KK Nearest Neighbors)과 같은 그에 따른 문제를 다룹니다. 온라인 학습에서는 환경 피드백, 내성 및 구조적 복잡성에 의해 특징 지어지는 문제 공간을 설명하고, 인사이트를 제공하기 위한 도구들을 제시합니다.
...
- **What's New**: 최근 대형 언어 모델(LLM)에서 자의식을 측정하는 새로운 방법론이 제안되었습니다. 이 연구는 LLM의 메타인지 능력을 정량적으로 평가하며, 비언어적 동물에 대한 연구에서 영감을 받았습니다. LLM은 내부 상태에 대한 지식을 전략적으로 활용할 수 있는지 테스트하기 위해 새로운 실험 패러다임을 사용합니다.

- **Technical Details**: 연구는 두 가지 실험 패러다임을 통해 메타인지 능력을 측정합니다. 첫 번째 실험은 'Delegate Game'으로, LLM이 도전적인 질문에 답하거나 팀원에게 위임하는 방식으로 게임이 진행됩니다. 두 번째 실험은 'Second Chance Game'으로, LLM이 이전 답안의 맞고 틀림을 평가하고 이에 따라 새로운 답변을 생성하는 능력을 테스트합니다.

- **Performance Highlights**: 최근 모델은 'Delegate Game'에서 일정한 성공을 보여 주며, 메타인지 능력의 일부를 갖추고 있을 가능성이 있습니다. 그러나 메타인지의 효과는 제한적이고 여러 질문 세트에 따라 일관성이 떨어지는 경향이 있습니다. 지나치게 고도화된 모델일수록 이러한 능력이 더욱 뚜렷해지는 경향을 보이며, 이는 모델 훈련 후 메타인지 능력이 발전할 수 있음을 시사합니다.



### A circuit for predicting hierarchical structure in-context in Large Language Models (https://arxiv.org/abs/2509.21534)
- **What's New**: 이번 연구에서는 대형 언어 모델(LLMs)의 문맥 내 학습(in-context learning)이 더 복잡한 반복적인 패턴과 계층 구조를 지원할 수 있는지에 대한 질문을 다룹니다. 기존의 induction heads는 단순한 복사 기능을 넘어 문맥에 적응하여 하위 및 상위 수준의 토큰 전이 관계를 파악할 수 있는지를 탐구합니다. 이러한 발견은 LLM들이 추가적인 문맥 정보를 통해 예측 적합성을 향상시키는 방법을 이해하는 데 중요한 기초를 마련합니다.

- **Technical Details**: 연구팀은 계층 의존성을 가진 반복적인 텍스트 시퀀스를 포함하는 문맥 내 학습 작업을 위해 새로운 인공 시퀀스를 설계했습니다. 여기서 induction heads는 특정 후속 토큰을 주의 깊게 살펴보도록 학습해야 하며, 이 과정에서 문맥에 따라 조정되도록 설계된 주의 메커니즘을 사용합니다. 연구에서는 여러 LLM들, 특히 Qwen 및 Llama 모델을 평가하여 문맥 내 학습 능력을 지원하는 induction heads의 존재를 확인했습니다.

- **Performance Highlights**: 연구 결과, 여러 모델에서 induction heads가 문맥 내 학습을 기반으로 후속 토큰을 주의하여 보다 정확한 예측을 가능하게 함을 보여주었습니다. 이는 LLM들이 맥락을 이해하고 응답할 수 있는 능력을 크게 향상시키며, 특히 다른 상위 수준의 구조와 통합하여 문맥과의 관계를 파악하는 데 유용한 기제를 제공한다고 강조합니다. 이러한 능력은 있는 그대로의 통계 패턴 복사 이상의 기능을 함의할 수 있습니다.



### Expert-guided Clinical Text Augmentation via Query-Based Model Collaboration (https://arxiv.org/abs/2509.21530)
Comments:
          18 pages, 5 figures

- **What's New**: 이 논문에서는 LLM(대형 언어 모델)을 활용한 안전한 임상 텍스트 증강(data augmentation)을 위한 새로운 모델 협업 프레임워크를 제안합니다. 경량의 도메인 전문가 모델을 통해 LLM의 생성 과정을 지도하여 중요한 의료 정보를 보존할 수 있도록 설계되었습니다. 이러한 접근 방식은 기존 LLM 기반 증강 방법을 능가하며, 안전성을 높이는 요소로 작용합니다.

- **Technical Details**: 우리는 LLM의 생성 과정에서 전문가 수준의 지식을 주입하기 위해, 경량의 '약한 전문가(weak expert)' 모델(BERT 기반)을 사용합니다. 이 모델은 LLM의 생성을 감독하며 품질이 높은 증강 데이터를 생성하는 데 도움을 줍니다. 이러한 방법은 기존의 데이터 증강 연구에 비해 증강의 안전성과 효율성을 동시에 달성하는 것을 목적으로 합니다.

- **Performance Highlights**: 실험 결과, 본 논문에서 제안한 협업 기반 증강 방법은 의료 용어 삭제를 줄이고 불필요한 용어의 도입을 최소화하여 여러 임상 작업에서 기존 방법들보다 우수한 성능을 보여줍니다. 또한, 우리의 모델은 추가적인 훈련 없이도 단일 모델로 응축할 수 있는 가능성을 지니고 있어 다양한 배포 환경에서도 활용할 수 있는 장점을 제공합니다.
...
### Contrastive Mutual Information Learning: Toward Robust Representations without Positive-Pair Augmentations (https://arxiv.org/abs/2509.21511)
Comments:
          Preprint. 9 pages main manuscript, 23 pages with appendix

- **What's New**: 이번 연구에서는 Mutual Information Machine (MIM)의 대조적 확장을 소개하며, cMIM(contrastive Mutual Information Machine)이라는 새로운 프레임워크를 제안합니다. cMIM은 긍정 데이터 증강의 필요성을 제거하고, InfoNCE보다 배치 크기에 덜 민감한 특성을 보여줍니다. 또한, 유용한 임베딩(informative embeddings) 기술을 도입하여 분리적(discriminative) 성능을 향상시키면서 추가 학습 없이도 폭넓은 적용이 가능합니다.

- **Technical Details**: cMIM은 MIM의 구조를 확장하여 전역적인 분리 구조(global discriminative structure)를 추가하며, 생성의 정확성을 유지합니다. 이 접근법은 인코더-디코더 아키텍처와 간단한 임베딩 추출(post-hoc embedding extraction)을 지원하여 다운스트림 작업에 쉽게 적용될 수 있습니다. 대조 학습(contrastive learning)을 활용하여 긍정 쌍의 유사성을 극대화하고 부정 쌍의 유사성을 최소화하여, 각 샘플의 정보량을 수준 높은 방향으로 조절합니다.

- **Performance Highlights**: cMIM은 MNIST와 같은 이미지 분류 및 분자 특성 예측 벤치마크에서 MIM과 동등한 재구성 품질을 유지하면서, 평균적으로 더 높은 다운스트림 정확도를 기록합니다. 이 방법은 적은 배치 크기에서도 강력한 성능을 발휘하며, 전반적으로 분류(classification)와 회귀(regression) 작업에서 더 나은 결과를 제공합니다. 이러한 성과는 cMIM이 분리적 및 생성적 애플리케이션 모두에 적합한 통합 프레임워크라는 점을 부각시킵니다.
...
### GraphPFN: A Prior-Data Fitted Graph Foundation Mod (https://arxiv.org/abs/2509.21489)
- **What's New**: 이번 연구에서는 GraphPFN이라는 새로운 모델을 소개하며, 이는 합성 (synthetic) 그래프 데이터에 대한 사전-데이터 적합 네트워크 (prior-data fitted network)로, 노드 레벨 예측에 특화되어 있습니다. GraphPFN은 기존의 그래프 기반 모델들과는 달리 핸드크래프트된 그래프 특성에 의존하지 않고, 학습 가능한 메시지 전달 메커니즘 (message-passing mechanism)을 통해 복잡한 그래프 의존성을 모델링할 수 있습니다. 이를 통해 다양한 현실 세계의 그래프 데이터셋에서 뛰어난 성능을 보입니다.

- **Technical Details**: GraphPFN은 LimiX라는 테이블 기반 모델을 초기화하여 시작하며, 주의 기반 메시지 전달 층 (attention-based message-passing layer)을 각 블록에 추가합니다. 이 모델은 합성 그래프 데이터에서 학습하며, 그 생성 과정에서 복수의 확률적 블록 모델 (stochastic block models)과 선호 연결 과정 (preferential attachment process)을 결합하여 그래프 구조를 생성합니다. 이러한 방법을 통해 수백만 개의 현실적이고 다양한 합성 그래프 데이터셋을 효율적으로 생성할 수 있습니다.

- **Performance Highlights**: 50,000개 노드까지 다양한 실제 그래프 데이터셋에서 GraphPFN은 뛰어난 인-컨텍스트 학습 성능을 보여주었으며, 후속 미세 조정(finetuning) 이후에는 기존의 G2T-FM 및 그래프 뉴럴 네트워크(GNN) 기준선보다 더 나은 성과를 기록했습니다. GraphPFN의 이러한 성과는 그래프 기반 모델의 구축에 있어 합성 그래프 데이터에서의 사전 학습이 유효한 전략임을 방증합니다.
...
### High-Probability Analysis of Online and Federated Zero-Order Optimisation (https://arxiv.org/abs/2509.21484)
- **What's New**: 이 논문에서는 gradient-free zero-order 최적화 환경에서 분산 학습을 연구하고, FedZero라는 연합 기반의 zero-order 알고리즘을 제안합니다. FedZero는 연합 convex 설정에서 최적화 오차 경계에 대한 높은 확률의 보장을 제공하며, 단일 작업자 환경에서는 최초로 convex zero-order 최적화에 대한 높은 확률의 수렴 보장을 제공합니다. 이 알고리즘은 ℓ1-구를 기반으로 하는 gradient estimator를 사용합니다.

- **Technical Details**: FedZero 알고리즘은 중심 서버와 여러 작업자 머신으로 구성되며, 각 라운드마다 서버는 최적화 포인트를 모든 작업자에게 방송합니다. 각 작업자는 샘플링을 통해 convex Lipschitz 함수를 평가하고, 이를 통해 gradient estimator를 구축하여 서버에 보고합니다. 서버는 작업자들의 업데이트를 통합하여 다음 단계의 최적화 포인트를 생성합니다.

- **Performance Highlights**: FedZero는 연합 zero-order 최적화 문제에서 첫 번째 높은 확률 수렴 보장을 제공하며, 단일 작업자의 경우 표준 zero-order 프레임워크로 간단히 줄어듭니다. 이 알고리즘은 새로운 concentration inequality를 통해 Lipschitz 함수의 성능을 향상시키며, 이는 고확률 보장에 중심적인 역할을 합니다. 이러한 방법론은 여러 실질적인 응용에 있어 중요한 의미를 가질 수 있습니다.



### Filtering with Confidence: When Data Augmentation Meets Conformal Prediction (https://arxiv.org/abs/2509.21479)
- **What's New**: 본 논문에서는 데이터 스카시티(data scarcity) 문제를 해결하기 위해 새로운 방법인 conformal data augmentation을 제안합니다. 이 방법은 conformal prediction의 원리를 활용하여, 품질이 높은 다양한 합성 데이터를 생성하면서 품질이 낮은 생성을 걸러내는 데이터 필터링 프레임워크입니다. 이 접근법은 구현이 간단하고, 모델의 내부 로짓(logits)에 접근할 필요가 없으며, 대규모 모델 재훈련 없이도 사용 가능합니다.

- **Technical Details**: 제안된 방법은 생성된 데이터의 품질을 평가하기 위한 원칙적 프레임워크를 기반으로 합니다. 이 프레임워크는 두 가지 주요 구성 요소로 이루어져 있습니다: 생성 샘플 품질을 정량화하는 스코어링 함수와, conformal risk prediction을 이용하여 최소 수용 가능 품질을 정의하는 거부 임계값입니다. 이러한 방식은 고온(높은 변동성) 설정에서도 질 높은 출력을 선택적으로 보존할 수 있는 가능성을 제공합니다.

- **Performance Highlights**: 논문에서는 텍스트 분류, 감정 분석, 사기 탐지 및 이미지 분류 등 여러 작업에서 제안된 방법의 효과를 입증했습니다. 이 방법은 비증강(uncaught) 기준보다 최대 40%의 F1 점수 향상을, 다른 필터링된 증강 기준보다 4% 향상을 보여주었습니다. 이러한 일관된 성과 개선은 데이터 증강의 실용성을 뒷받침하는 중요한 evidence를 제공합니다.
...
- **What's New**: 이번 논문은 기존의 확산 언어 모델(Diffusion Language Models, DLMs)의 추론 능력을 개선하기 위해 새로운 정책 기울기 알고리즘을 도입합니다. 이 알고리즘은 마스킹(masking)의 특성을 활용하여 샘플링 경로의 가능성을 정확하게 추정합니다. 제안하는 d2라는 추론 프레임워크는 DLMs가 효율적인 확산 기반 추론을 수행할 수 있도록 지원하며, RL(강화 학습)을 바탕으로 이전 방안에 비해 성능을 크게 향상시킵니다.

- **Technical Details**: d2 프레임워크는 두 가지 가능도 추정기, 즉 d2-StepMerge와 d2-AnyOrder를 사용하여 마스킹된 확산을 위한 새로운 정책 기울기 알고리즘을 구현합니다. d2-StepMerge는 특정 단계에서만 경로를 평가하여 전방 패스를 줄이면서 근사 오차를 제어하는 반면, d2-AnyOrder는 비편향적인 단일 패스 가능도 추정이 가능한 특성을 갖습니다. 이러한 기술적 접근법은 특히 LLaDA와 같은 대중적인 DLMs에서 유용하게 적용되며 효율적인 확산 기반 추론의 핵심이 됩니다.

- **Performance Highlights**: d2는 Countdown 및 Sudoku와 같은 논리 추론 작업 및 GSM8K와 MATH500과 같은 수학 기반 벤치마크에서 기존의 RL 기반 DLMs 방식을 초과하는 성능을 보여줍니다. 특히, LLaDA-8B-Instruct에 적용했을 때 기존 방법들보다 월등한 결과를 기록하며 새로운 최첨단 성과를 달성하였습니다. 따라서, d2는 DLMs의 추론 능력을 한층 더 발전시키는 데 기여할 것으로 기대됩니다.
...
- **What's New**: 본 논문에서는 Tabular 데이터에 대한 기존의 블랙박스 모델을 대신할 수 있는 해석 가능한 의사결정 트리(decision tree) 생성 방법을 제안합니다. 이 방법은 Reasoning-capable LLM을 활용하여, 데이터를 통해 학습한 지식과 사전 지식을 결합하여 소형 데이터 세트에서 높은 성능을 보여주는 경량 의사결정 트리를 생성합니다. 이로써 기존 모델들이 가진 해석 가능성과 고비용 문제를 해결하고자 합니다.

- **Technical Details**: 제안된 방법에서는 주의 깊게 설계된 다양한 도구 세트를 LLM에 제공하여 데이터 전처리(data preprocessing), 분할 탐색(split search), 검증(validation), 가지치기(pruning), 중간 트리 개선(refinement)을 수행합니다. LLM은 이러한 도구들을 사용하여 결정 트리를 점진적으로 개선하며, 체계적인 사고(chain-of-thought reasoning)를 통해 데이터와 도메인 지식을 활용해 나갑니다. 예를 들어, 특정 노드를 잔여 노드로 변경하는 등의 작업이 가능합니다.

- **Performance Highlights**: 이 연구에서 제안한 LLM 기반의 의사결정 트리는 기존 CART 모델을 초월하는 성능을 보이는 경량 대안으로 자리잡게 됩니다. 하지만 단일 의사결정 트리는 여전히 최신 블랙박스 모델에 미치지 못합니다. 그럼에도 불구하고 결과적으로 사용자에게는 인간이 이해할 수 있는 해석 가능성을 제공하여 편향(bias)이나 데이터 누출(data leaks)을 확인할 수 있는 이점이 있으며, 추가적인 도메인 지식을 반영할 수 있는 가능성도 제공합니다.



### Forecasting Seismic Waveforms: A Deep Learning Approach for Einstein Telescop (https://arxiv.org/abs/2509.21446)
Comments:
          8 pages, 3 figures, ICRC 2025 Proceedings

- **What's New**: 본 논문은 세 가지 구성 요소의 지진파형 예측을 위한 트랜스포머 기반 모델인 SeismoGPT를 제안합니다. 이 모델은 단일 스테이션 및 배열 기반 입력 모두에서 작동할 수 있도록 훈련되었으며, 웨이브폼 데이터에서 시간적 및 공간적 의존성을 학습하여 현실적인 지면 진동 패턴을 포착합니다. SeismoGPT는 단기 예측에서 우수한 성능을 보이며, 자가 회귀 시스템의 기대에 따라 시간이 지남에 따라 점차 성능이 저하되는 경향이 있습니다.

- **Technical Details**: SeismoGPT는 시퀀스 모델링을 통해 웨이브폼의 미래 진화를 예측하는 방법으로 훈련됩니다. 이 모델은 단일 스테이션 입력과 배열 기반 입력의 두 가지 버전이 있으며, 인코더 기반 트랜스포머 아키텍처를 사용합니다. 입력 데이터는 고차원적 시퀀스로 변환된 후 1D 또는 2D 합성곱 네트워크를 통과하여 포지셔널 인코딩이 추가된다. 특히, 자가 회귀 설정에서 각 토큰은 오직 현재 및 과거의 정보만을 참조하여 예측합니다.

- **Performance Highlights**: SeismoGPT의 결과는 단기 예측에서 탁월한 성능을 보여줍니다. 특히, 자가 회귀적으로 워크 플로우를 진행하면서도 지진파형 예측에서 기대 이상의 정확성을 기록했습니다. 이 접근 방식은 경량의 AI 기반 예측 모델로서 새로운 세대의 중력파 탐지기에서 지진 노이즈 완화와 실시간 관측소 제어를 지원할 수 있는 가능성을 제시합니다.



### Null-Space Filtering for Data-Free Continual Model Merging: Preserving Transparency, Promoting Fidelity (https://arxiv.org/abs/2509.21413)
- **What's New**: 이 논문은 데이터 없이 연속적으로 모델을 병합하는 새로운 프레임워크인 NUFILT(NUll-space FILTering)를 소개합니다. 이 프레임워크는 데이터 수준의 요구사항인 투명성(transparency)과 충실도(fidelity)를 직접적으로 매개변수 공간(parameter-space) 최적화와 연결시킵니다. NUFILT는 새롭게 조정된 작업 벡터의 일부를 필터링하여 이전 작업과의 간섭을 줄이고, 보완적인 신호를 주입하는 경량 LoRA 어댑터를 사용하여 새로운 작업에 대한 적응성을 높입니다.

- **Technical Details**: NUFILT는 태스크 벡터가 표현 서브공간(representation subspaces)과 근사적으로 정렬되는 구조적 특성을 활용합니다. 투명성을 보장하기 위해 널-공간 프로젝터(null-space projector)를 설계하여 특정 작업 벡터와 이전 서브공간의 겹치는 부분을 필터링합니다. 또한, LoRA 어댑터는 데이터가 없는 대리 손실(data-free surrogate loss)을 통해 작업 벡터를 조정하여 충실도를 확보합니다. 이 프로세스는 레이어별 선형 방식으로 업데이트를 통합하여 추가 파라미터나 추론 비용 없이 기존의 행동을 유지합니다.

- **Performance Highlights**: NUFILT는 비전 및 자연어 처리(NLP) 벤치마크에서 최신 성능을 자랑하며, 최소한의 잊기를 보이며 평균 정확도가 4-7% 향상되었습니다. 기존의 방법인 OPCM 및 WUDI-Merging과 비교하여 NUFILT는 정확도가 향상되었으며, 개별적인 미세 조정(fine-tuning)과의 간격도 줄였습니다. 이러한 결과는 NUFILT가 이전 지식을 유지하면서도 새로운 지식을 효과적으로 흡수할 수 있는 능력을 가지고 있음을 나타냅니다.



### Object Identification Under Known Dynamics: A PIRNN Approach for UAV Classification (https://arxiv.org/abs/2509.21405)
Comments:
          2025 International Conference on Machine Learning and Applications (ICMLA)

- **What's New**: 이번 연구는 무인 항공기(无人航空器) 응용에서 알려진 동역학(known dynamics) 하의 객체 식별(object identification)에 중점을 두고 있습니다. 학습(learning)과 분류(classification)를 물리 정보 기반 잔여 신경망(physics-informed residual neural network)을 통해 결합하는 새로운 프레임워크를 제안합니다. 이 연구는 쿼드콥터(quadcopter), 고정익 비행기(fixed-wing), 헬리콥터(helicopter)를 사례로 사용하였습니다.

- **Technical Details**: 제안된 프레임워크는 물리 정보 기반 학습(physics-informed learning)을 활용하여 상태 매핑(state mapping)과 상태 미분(prediction of state derivatives)을 수행합니다. 또한 소프트맥스 레이어(softmax layer)를 통해 다중 클래스 신뢰도 추정(multi-class confidence estimation)을 가능하게 합니다. 이러한 기술적인 접근 방식은 기존의 동역학을 잘 이해하고 있는 상황에서 사용될 수 있습니다.

- **Performance Highlights**: 결과적으로, 제안된 방법은 높은 분류 정확도(high classification accuracy)를 달성하였으며, 훈련 시간(training time)을 단축했습니다. 이는 기본 동역학이 잘 이해된 영역에서 시스템 식별 문제(system identification problems)에 유망한 해결책을 제공합니다.
...
- **What's New**: 이 연구에서는 Physics Informed Neural Networks (PINNs)의 손실 가중치 선택에 대한 민감도를 개선하기 위해 두 가지 새로운 가중치 방식이 제안됩니다. 구체적인 항을 기반으로 한 방법과 비구체적인 항도 포함하여 더 균형 잡힌 훈련을 위한 두 번째 방법이 도입됩니다. 실험 결과, 열 전도 및 대류 확산 흐름과 같은 다양한 문제에서 이 방법이 안정성과 정확성을 높이는 데 기여함을 보여줍니다.

- **Technical Details**: PINNs는 시스템의 암묵적 governing equations를 설명하는 명시적 함수로 작용하는 신경망입니다. 이들은 독립 변수를 입력으로 받고, 종속 변수를 출력으로 생성합니다. 손실 함수는 서로 다른 손실 구성 요소의 가중 합으로 정의되며, 이는 미분 방정식, 경계 조건 및 초기 조건을 시행합니다.

- **Performance Highlights**: 실험을 통해, 고 Peclet 수 대류 확산과 같은 전통적인 해법이 실패하는 상황에서도 제안된 방법이 안정적이고 정확한 예측을 제공함을 발견했습니다. 특히, 이 접근법은 CFD 문제에서의 강건성 및 일반성을 강조하며, 기존의 균일 가중치 방법보다 월등한 성능을 발휘합니다.



### Discovering and Analyzing Stochastic Processes to Reduce Waste in Food Reta (https://arxiv.org/abs/2509.21322)
- **What's New**: 이 논문은 식품 소매 프로세스 분석을 위한 새로운 방법을 제안하며, 주로 식품 폐기물(식품 조기 폐기) 감소에 중점을 두고 있습니다. 이 접근법은 객체 중심 프로세스 마이닝(Object-Centric Process Mining, OCPM)과 확률적(process stochastic) 프로세스 발견 및 분석 방법을 통합합니다. 마트 판매 데이터에서 연속 시간 마르코프 체인(continuous-time Markov chain) 형태의 확률적 프로세스가 발견되며, 이는 공급 활동과 통합되어 분석됩니다.

- **Technical Details**: 먼저, 고객 구매 행동과 공급 전략 간의 최적 균형을 식별하기 위해, 제품의 양이 시간에 따라 어떻게 변하는지를 평가하는 'what-if(가정적)' 분석이 수행됩니다. 데이터는 판매 이벤트 집합으로 제공되며, 프로세스 마이닝 기술을 통해 식품 공급 최적화(조정)에 대한 인사이트를 얻습니다. 이 논문은 특히 매장 용량의 동적 변화를 파악하는 확률적 프로세스 발견 방법을 제시하고 있습니다.

- **Performance Highlights**: 식품 소매 프로세스에서 발생하는 다양한 이벤트를 시간적으로 모델링함으로써, 과잉 공급 또는 제품 부족으로 인한 식품 폐기물 문제를 방지하는 전략을 지원할 수 있습니다. 본 연구의 목적은 데이터에 기반한 인사이트를 통해 지속 가능성을 높이는 것입니다. 이러한 분석 도구는 실제 데이터에 적용되어 그 결과가 실질적인 식품 폐기물 감소 및 프로세스 개선으로 이어질 수 있습니다.
...
### Effective Policy Learning for Multi-Agent Online Coordination Beyond Submodular Objectives (https://arxiv.org/abs/2509.22596)
Comments:
          Accepted to NeurIPS 2025

- **What's New**: 이번 논문에서는 다중 에이전트 온라인 협업(multi-agent online coordination, MA-OC) 문제를 해결하기 위한 두 가지 효과적인 정책 학습 알고리즘을 제안합니다. 첫 번째 알고리즘인 	exttt{MA-SPL}은 서브모듈러(submodular) 목표를 가진 MA-OC 문제에 대해 최적 보장 $(1 - \frac{c}{e})$를 달성할 수 있으며, 미처 탐색되지 않은 $\\alpha$ 약한 감소 수익(DR) 서브모듈러 및 $(\gamma, \beta)$ 약한 서브모듈러 상황을 처리할 수 있습니다. 두 번째 알고리즘인 	exttt{MA-MPL}은 파라미터에 의존하지 않고 첫 번째 알고리즘과 동일한 근사 비율을 유지합니다.

- **Technical Details**: 이 논문의 핵심은 정책 기반 연속 확장(policy-based continuous extension)이라는 새로운 연속 완화 기법입니다. 이 기법은 잘 개발된 다중 선형 확장(multi-linear extension)과 비교할 때, 임의 집합 함수에 대한 손실 없는 반올림 방안을 제공하는 장점이 있습니다. 이를 통해 약한 서브모듈러 목표를 효과적으로 다룰 수 있습니다. 또한, 이 알고리즘은 $\\alpha$, $\\gamma$, $\\beta$와 같은 알려지지 않은 파라미터의 의존성을 줄이는 데 기여합니다.

- **Performance Highlights**: 제안된 두 알고리즘은 광범위한 시뮬레이션을 통해 그 효과성이 입증되었습니다. 특히, 	exttt{MA-SPL}은 서브모듈러 목적을 가진 MA-OC 문제에 대해 높은 근사 비율을 제공합니다. 이러한 알고리즘들은 실제 세계의 다양한 응용 프로그램에 적합한 성능을 보이며, 특히 복잡한 매트로이드 제약(matroid constraint)을 포함하는 경우에도 유용합니다.
...
### Nearly Tight Regret Bounds for Profit Maximization in Bilateral Trad (https://arxiv.org/abs/2509.22563)
Comments:
          Accept at FOCS '25

- **What's New**: 이번 연구는 판매자와 구매자가 서로 거래하고자 할 때, 브로커 관점에서 시작됩니다. 중개자는 매출 증대를 목표로 하며, 판매자 및 구매자 각각의 진정한 선호에 따라 행동하도록 유도하는 메커니즘을 제안해야 합니다. 이전 연구는 경제적 효율성을 극대화하는 데 초점을 맞추었으나, 이러한 접근은 수익 극대화라는 목표에서는 한계가 있음을 보여줍니다. 본 연구는 비대칭 모델에서 수익성을 높이는 새로운 알고리즘을 제안합니다.

- **Technical Details**: 연구는 두 가지 데이터 생성 모델인 적대적인 입력 모델과 확률적 i.i.d. 모델을 다룹니다. 확률적 i.i.d. 모델에서는 판매자와 구매자의 평가가 고정된 확률적 분포에서 독립적으로 나온다고 가정합니다. 연구의 주요 결과로는 이 모델에서 최적의 메커니즘에 대한 O~(T)O(√T)의 거의 타이트한 후회 경계(regret bound)를 보장하는 학습 알고리즘을 제안한 점이 있습니다. 반면 적대적 환경에서는 어떤 학습 알고리즘도 하한선인 O(2/3+ε)의 후회를 달성할 수 없음을 보여줍니다.

- **Performance Highlights**: 확률적 설정에서 제안된 알고리즘은 최적 메커니즘에 대한 매우 우수한 성능을 보여주며, O~(T)O(√T) 후회를 달성함으로써 최적의 기준에 근접한 성과를 보입니다. 반대편인 적대적 설정에서는 어떤 학습 알고리즘도 수익성에 대한 하한을 초과할 수 없다는 사실을 확립하며, 이는 경제적 모델에서 적대적 및 확률적 모델 간의 흥미로운 차이를 드러냅니다. 마지막으로, 본 연구는 두 광고주 간의 광고 문제를 해결하는 데 기여하면서, 효율적인 메커니즘 설계의 새로운 경계를 탐색합니다.



### Linear Causal Representation Learning by Topological Ordering, Pruning, and Disentanglemen (https://arxiv.org/abs/2509.22553)
- **What's New**: 본 논문은 Causal Representation Learning (CRL)의 발전에 기여하며, 잠재적 인과적 요인을 확인하기 위해 구조적 인과 모델을 이용하는 새로운 알고리즘을 제안합니다. 기존의 선형 CRL 방법들보다 더 약한 가정 아래에서 작동하며, 데이터 생성 분포의 이질성을 잘 반영하여 잠재적 인과적 요인들을 복원합니다. 특히, 이는 단일 노드 중재적 데이터(건너뛰기)나 잠재적 요인 및 외부 측정 노이즈에 대한 엄격한 제약 없이도 가능하다는 점에서 차별화됩니다.

- **Technical Details**: 이 연구는 선형 구조적 인과 모델 내에서의 비가우시안(exogenous noise) 변수를 가정하고, 이를 바탕으로 새로운 CRL 알고리즘을 설계하였습니다. 제안된 알고리즘은 잠재적 인과적 요인과 그 메커니즘을 동등 클래스(eqivalence class)까지 식별할 수 있습니다. 알고리즘은 세 개의 주요 서브루틴으로 구성되어 있으며, 각 서브루틴은 잠재적 요인의 위상적 순서를 추론하고, 가지치기(pruning)를 수행하며, 마지막으로 잠재적 요인들을 분리합니다.

- **Performance Highlights**: 본 연구에서 제안하는 알고리즘은 LiNGCReL과 비교하여 샘플 수가 한정된 환경에서도 우월한 성능을 보입니다. 또한, 대형 언어 모델(LLMs)의 출력을 분석하는 작업에 적용하였으며, LLMs의 후속 분석에 있어서도 실질적인 유용성을 입증하였습니다. 다양한 합성 실험(synthetic experiments)을 통해 알고리즘의 효과성을 상세히 검증하였습니다.
...
### Metrics for Parametric Families of Networks (https://arxiv.org/abs/2509.22549)
- **What's New**: 이 논문에서는 매개변수화된 네트워크 집합으로 모델링된 데이터를 분석하기 위한 일반적인 프레임워크를 소개합니다. 여러 가지 기존 메트릭을 포함하여 Gromov-Wasserstein 거리의 매개변수화된 가족을 정의하고, 이러한 거리가 실제로 유용하게 활용될 수 있음을 실험을 통해 입증하고 있습니다.

- **Technical Details**: Gromov-Wasserstein 거리 이론에 기반하여, 이 연구는 시간에 따라 변하는 메트릭 공간 및 가중 그래프와 같은 매개변수화된 측정 네트워크를 비교하기 위한 새로운 메트릭을 확장했습니다. 매개변수화된 측정 네트워크는 고유한 확률 공간과 측정된 매개변수 공간을 포함하는 구조로 정의되며, 일반적인 GW 거리와의 관련성을 입증하였습니다.

- **Performance Highlights**: 저자들은 제안한 프레임워크의 유용성을 다수의 수치 실험을 통해 보여주고, 매개변수화된 구조의 비교에 대한 이론적 보장과 계산 가능성이 높은 하한을 제공했습니다. 특히, 랜덤 그래프 이론에서 일반적으로 사용되는 그래프 통계량과의 연관성을 밝혀냈습니다.



### Debiased Front-Door Learners for Heterogeneous Effects (https://arxiv.org/abs/2509.22531)
Comments:
          27 pages, 3 figures. Preprint. Code available at this https URL

- **What's New**: 이 연구에서는 관찰적 데이터에서 치료 효과를 추정하는 데 도움을 주기 위해 'front-door (전면 도어) 조정' 방법을 활용하여 heterogeneous treatment effect (HTE, 이질적 치료 효과)를 다룹니다. 특히, FD-DR-Learner와 FD-R-Learner라는 두 가지 새로운 debiased learners(디버이즈 학습자)를 제안하여, 이들이 빠른 quasi-oracle rates(준 oracle 비율)를 달성하는 것을 보여줍니다. 이 방법은 사례 연구와 함께 실증 분석을 통해 강력한 성능을 입증하였습니다.

- **Technical Details**: 연구에서는 이질적 치료 효과 추정 기법을 front-door 설정에 맞추어 조정하였습니다. FD-DR-Learner는 conditional mean(조건부 평균)을 기준으로 한 pseudo-outcome(가상 결과)를 기반으로 합니다. FD-R-Learner는 세 단계 절차를 통해 XX가 ZZ에 미치는 영향과 ZZ가 YY에 미치는 영향을 추정하여 HTE를 도출합니다. 이러한 과정은 느린 nuisance 함수의 수렴 하에서도 오류 분석을 통해 신뢰성을 보장합니다.

- **Performance Highlights**: 제안된 학습자들은 synthetic studies(합성 연구)와 실제 사례 연구에서 뛰어난 성능을 보였습니다. 특히, FD-DR-Learner와 FD-R-Learner는 불균형한 데이터에서도 신뢰할 수 있고 샘플 효율성이 높은 HTE 추정을 제공합니다. 이 연구 결과는 의사결정 플랫폼에서 개인 맞춤형 효과를 지원하는 데 매우 유용하다고 강조됩니다.



### Smoothing-Based Conformal Prediction for Balancing Efficiency and Interpretability (https://arxiv.org/abs/2509.22529)
- **What's New**: 이 논문은 SCD-split을 제안하여 기존의 CD-split이 생성하는 작은 비연결 구간 문제를 해결하고자 합니다. SCD-split은 예측 세트를 구성하기 전에 조건부 밀도 함수에 스무딩(smoothing) 기술을 적용하여 여러 개의 비연결 하위 구간을 합치는 데 도움을 주는 새로운 접근 방식을 제공합니다. 이 방법은 예측 세트의 해석 가능성을 높이고, 사용자가 원하는 개수의 비연결 구간을 생성할 수 있도록 지원합니다.

- **Technical Details**: SCD-split에서는 사용자가 원하는 비연결 구간의 수를 사전에 지정할 수 있으며, 이를 충족하기 위해 스무딩 절차를 사용하여 조건부 밀도 함수의 과도한 방정식을 줄습니다. 이 스무딩 과정은 예측 세트의 길이와 비연결 구간의 수를 조정하며, 이론적으로도 마진 커버리지 보장을 유지하는 것은 물론, 특정 조건 하에 비연결 구간의 수를 줄이는 것을 증명합니다. Theorems 4.1과 4.4에서는 SCD-split이 커버리지 보장을 유지하면서 예측 세트의 길이를 조정할 수 있음을 보여줍니다.

- **Performance Highlights**: 실험 결과, SCD-split은 복잡하고 다봉 분포에서 효과적으로 예측 세트의 해석 가능성을 개선하면서, 예측 세트의 길이와 비연결 구간의 수 간의 균형을 이룬다는 것을 입증했습니다. 이 연구는 의료 예측이나 금융 모델링 등 다양한 실제 문제에서 해석 가능하고 실행 가능한 예측을 가능하게 하여 관련 산업에 실질적인 기여를 할 것으로 기대됩니다.
...
- **What's New**: 이번 논문에서는 의료, 경제 및 공공 정책과 같은 민감한 분야에서의 신뢰성과 채택을 높이기 위해, 상태-of-the-art 성능을 보이는 심층 신경망을 사용하여 이질적인 치료 효과를 추정하는 방법에 대한 새로운 접근 방식인 causalKANs를 제안합니다. 이 방법은 조건부 평균 치료 효과(Conditional Average Treatment Effects, CATE)의 신경 추정기를 Kolmogorov--Arnold Networks (KANs)로 변형하여 구현됩니다.

- **Technical Details**: causalKANs는 잘 확립된 인과 신경 구조를 기반으로 하며, 가지치기(pruning) 및 기호 단순화(symbolic simplification)를 통합하여 해석 가능한 폐쇄형 공식(closed-form formulas)을 생성합니다. 이 과정에서 예측 정확성(predictive accuracy)을 유지하며, 이런 방식으로 얻어진 공식은 신뢰성을 높이는 데 기여합니다.

- **Performance Highlights**: 실험 결과, causalKANs는 CATE 오차 지표에서 기존의 신경망 기초 모델과 동등한 성능을 보이고, 간단한 KAN 변형조차도 경쟁력 있는 성능을 달성함을 보여주었습니다. 이는 높은 정확성과 해석 가능성(interpretability)의 균형을 잘 이루는 방법으로, 고위험(threatening) 환경에서의 개인 맞춤형 의사 결정에 신뢰성을 부여하는 데 기여합니다.



### Universal Inverse Distillation for Matching Models with Real-Data Supervision (No GANs) (https://arxiv.org/abs/2509.22459)
- **What's New**: 본 논문에서는 RealUID라는 보편적인 증류 프레임워크를 소개합니다. 이 프레임워크는 GAN 없이 실 데이터를 증류 절차에 원활하게 통합할 수 있는 기능을 갖추고 있습니다. 특히, Diffusion Models과 Flow Matching Models뿐만 아니라 Bridge Matching과 Stochastic Interpolants에 대한 지원도 포함되어 있습니다.

- **Technical Details**: RealUID는 효율적인 1단계 생성기를 훈련하는 이전의 증류 방법들을 통합하여 정리합니다. 이 방법은 선형화 기법에 기반한 간단하면서도 강력한 이론적 설명을 제공하며, 다양한 증류 기술 간의 관계를 밝혀냅니다. 증류 손실에 실 데이터를 직접 통합하는 새로운 방법을 도입하여, 추가적인 GAN 기반 손실을 요구하지 않도록 설계되었습니다.

- **Performance Highlights**: RealUID는 다양한 매칭 모델에 적용 가능하여 생성 속도를 크게 개선할 수 있습니다. 기존 증류 방법들은 특정 프레임워크에 제한되는 반면, RealUID는 다중 단계 동작을 효율적인 일 단계 또는 소수 단계 생성기로 압축하는 데 주력합니다. 이를 통해 실제 데이터를 활용하면서도 훈련 과정에서의 복잡성을 줄이는 효과를 기대할 수 있습니다.
...
### NeuroScalar: A Deep Learning Framework for Fast, Accurate, and In-the-Wild Cycle-Level Performance Prediction (https://arxiv.org/abs/2509.22410)
- **What's New**: 이 논문은 새로운 마이크로프로세서 설계 평가를 위해 고정밀의 ‘in-the-wild’ 시뮬레이션을 지원하는 혁신적인 딥러닝 프레임워크를 도입합니다. 기존의 사이클 정확도 시뮬레이터는 느리고 비현실적인 벤치마크에 의존하는 한계가 있었는데, 이 모델은 마이크로아키텍처와 독립적인 특성을 학습하여 실제 하드웨어에서 새로운 프로세서 설계를 평가할 수 있도록 합니다. 이 시스템은 경량 하드웨어 트레이스 수집기와 샘플링 전략을 포함하여 사용자에 대한 영향을 최소화하면서 5 MIPS의 시뮬레이션 속도를 달성합니다.

- **Technical Details**: NeuroScalar 시스템은 고속 예측을 위해 딥러닝 모델을 사용하여 기존 하드웨어에서 새로운 가상의 마이크로아키텍처의 성능을 예측합니다. 마이크로아키텍처에 독립적인 특성만을 사용하여 훈련된 이 모델은 사용자의 실제 프로세서에서 생성된 명령어 트레이스를 기반으로 작동합니다. 또한, 28mW의 저전력 하드웨어 가속기를 설계하여 GPU 대비 85배의 에너지 절약과 391배의 면적 감소를 달성했습니다.

- **Performance Highlights**: 이 프레임워크는 A/B 테스트에서 95%의 정확도로 다섯 개의 프로세서 디자인을 비교하며 디자인 공간 탐색을 효과적으로 지원합니다. NeuroScalar는 고속 샘플링 덕분에 사용자가 실제 애플리케이션을 실행하는 동안 투명하게 성능 예측을 하고, 이를 통해 대규모 디자인 탐색을 가능하게 합니다. 더 나아가, 이 시스템은 실세계 사용 사례에서의 다양한 애플리케이션에 대한 깊은 통찰을 제공합니다.
...
### Multidimensional Uncertainty Quantification via Optimal Transpor (https://arxiv.org/abs/2509.22380)
- **What's New**: 이번 연구는 예측 불확실성을 단일 스칼라로 표현하는 대신, 여러 보완적인 불확실성 측정을 벡터로 쌓아 표현하는 방식을 제안합니다. 제안된 VecUQ-OT 알고리즘은 Monge-Kantorovich 순위를 활용하여 벡터들 간의 정당한 순서를 매기는 방법론을 제공합니다. 이 연구는 불확실성 정량화(UQ) 분야에서 첫 번째로 불확실성을 벡터로 명시적으로 다루는 프레임워크를 제시합니다.

- **Technical Details**: VecUQ-OT 알고리즘은 다양한 특성의 불확실성 점수를 벡터 형태로 수집하고, 이를 Monge-Kantorovich 순위에 따라 정렬하여 비교합니다. 이러한 벡터는 최적 수송(optimal transport) 방법을 통해 불확실성 기준과의 거리를 정의하여, 각 예측의 불확실성을 평가합니다. 실험에서는 aleatoric과 epistemic을 포함한 비가법적 불확실성 융합(fusion)을 통해 다양한 경우에 유연하게 적용 가능합니다.

- **Performance Highlights**: VecUQ-OT는 합성 데이터, 이미지, 텍스트 데이터에서 각각의 개별 측정이 실패하더라도 높은 효율성을 보입니다. 이 방법은 선택적 예측(selective prediction), 오분류 감지(misclassification detection), OOD 탐지(out-of-distribution detection), 선택적 생성(selective generation) 등 다양한 다운스트림 작업에서 강건한 성능을 입증하였습니다. 실험 결과, 제안된 벡터 기반 불확실성 측정 방식은 전반적으로 일관된 성과를 나타내고 있습니다.



### Multi-channel convolutional neural quantum embedding (https://arxiv.org/abs/2509.22355)
Comments:
          20 pages, 7 figures

- **What's New**: 이번 연구는 전통적인 양자 회로 모델의 한계를 넘어 다채널 데이터를 위한 양자 임베딩을 최적화하는 고전-양자 하이브리드 접근 방식을 제안합니다. 이 방법론은 네트워크와 양자 회로 간의 상호 작용 구조를 다르게 설정하여 분류 성능을 향상시키는 데 중점을 두고 있습니다. 특히, CNN(Convolutional Neural Networks) 기반의 양자 임베딩 구조를 도입하여 기존의 방법보다 나은 성능을 달성할 수 있음을 보여주었습니다.

- **Technical Details**: 고전-양자 하이브리드 임베딩(NQE) 프레임워크를 통해 양자 개념을 활용하여 고전적 데이터를 양자 상태로 변환하고 이를 기반으로 학습이 이루어집니다. 이 연구에서는 세 가지 독특한 인터페이스 구조와 두 가지 훈련 손실 함수를 제안하여 양자 상태의 표현력과 분리성을 분석했습니다. 양자 맵을 통해 데이터를 임베드하고, VQA(Variational Quantum Algorithms)에 의해 파라미터화된 유니타리 회로가 훈련되어 분류 성능을 극대화합니다.

- **Performance Highlights**: CIFAR-10 및 Tiny ImageNet과 같은 일반적으로 사용되는 이미지 분류 데이터셋에서의 실험 결과, 제안하는 CNQE 프레임워크가 높은 분류 성능을 기록하며 파라미터 복잡성을 줄이는 데 성공했습니다. 이러한 결과는 실제 문제 해결을 위한 자원 효율적인 양자 기계 학습(QML)의 잠재성을 강조합니다. 추후 연구에서 이 구조가 다채널 데이터에 적합한 다양한 방식으로 적용될 수 있을 것으로 기대됩니다.
...
- **What's New**: 이번 논문에서는 생성 모델의 퇴화 현상, 즉 모델 콜랩스(model collapse)를 다룹니다. 과적합된 문제에서 실제 데이터와 합성 데이터를 혼합할 때 적절한 혼합 비율을 찾는 것이 논문의 주요 목표입니다. 연구자들은 minimum-ℓ2-norm interpolation과 ridge regression의 두 가지 추정기 클래스를 통해 이러한 효과를 분석하였으며, 최적의 혼합 비율을 규명했습니다.

- **Technical Details**: 연구에서는 각 반복(iteration)마다 실제 레이블(real labels)과 이전 반복에서 학습된 모델에서 생성된 합성 레이블(synthetic labels)을 혼합하는 방법을 사용합니다. 특히, 최적 혼합 비율을 적분하는 과정에서 황금비(golden ratio)의 역수에 수렴한다는 흥미로운 결과를 발견하였습니다. 또한, ridge regression의 경우, 최적의 혼합 비율은 최소 1/2 이상으로 나타나며, 이는 실 데이터의 중요성을 강조합니다.

- **Performance Highlights**: 논문에서는 이론적 결과를 다양한 시뮬레이션을 통해 검증하였습니다. 실제 데이터에 대한 비율이 높을수록 예측 오류가 최소화된다는 점을 입증하였고, 이러한 분석이 과적합 문제에서 모델 콜랩스를 방지하는 데 중요한 지침을 제공함을 보여주었습니다. 이 연구는 이전의 낮은 차원에서의 연구들을 넘어 복잡한 문제에 적용될 수 있는 견고한 결과를 제시합니다.
...
### Incorporating priors in learning: a random matrix study under a teacher-student framework (https://arxiv.org/abs/2509.22124)
Comments:
          5 pages, 4 figures

- **What's New**: 이번 연구는 높은 차원에서의 regularized linear regression을 더욱 심층적으로 이해하며, 정보 prior를 포함한 최대 사후 확률(MAP) 회귀의 교육 및 테스트 리스크에 대한 첫 번째 정확한 비대칭 특성을 제시합니다. 연구의 주요 초점은 Gaussian prior가 포함된 MAP 회귀에 있으며, 이는 ridge regression, least squares, 그리고 prior-informed regression을 통합하는 틀을 제공합니다. 본 연구는 Bayesian priors와 전통적 regularization, 현대 비대칭을 연결하여 도메인 지식을 활용하는 실용적인 지침을 제공합니다.

- **Technical Details**: 고전적인 최소 제곱법이나 ridge regression 등의 기법은 데이터 차원 수(d)와 샘플 수(n)가 동시에 증가할 때(overfitting, double descent 현상 발생) 여러 버전의 해석이 필요하게 됩니다. 본 연구는 Gaussian prior를 사용하여 MAP 회귀의 asymptotic를 분석하며, noise variance와 regularization 간의 상호작용을 설명하는 명확한 공식을 유도합니다. 또한, 우리는 고차원 MAP 회귀에 대한 고유한 추정기와 최적 regularization 파라미터를 도출하여 실제로 활용 가능한 방법을 제시합니다.

- **Performance Highlights**: 시뮬레이션 실험을 통해 이론적 예측값과 높은 일치를 확인하였으며, 설명된 편향-분산 tradeoff 조정의 명확한 이해를 제공합니다. MAP 회귀에 대한 현재까지의 포괄적인 분석을 통해 일반화 이론의 최전선을 확장하고 높은 차원에서의 데이터 분석에 대해 도메인 지식을 활용하는 실용적 가이드를 제공합니다. 연구 결과는 Bayesian 접근 방식을 기반으로 한 회귀 분석에서 학습 및 성능 향상에 기여할 수 있습니다.



### Direct Bias-Correction Term Estimation for Propensity Scores and Average Treatment Effect Estimation (https://arxiv.org/abs/2509.22122)
- **What's New**: 이번 연구에서는 평균 치료 효과(Average Treatment Effect, ATE) 추정에 대한 새로운 접근 방식을 제안합니다. 우리는 기울기 보정(term) 추정을 통해 직접 편향(bias) 보정 항을 최소화하여 평균 치료 효과를 추산하는 방법을 개발했습니다. 기존 연구들은 최대 우도(maximum likelihood)나 변수 균형(covariate balancing) 기법을 사용했지만, 본 연구에서 제시하는 방법은 이러한 접근법보다 더 나은 성능을 기대할 수 있습니다.

- **Technical Details**: ATE 추정에서 중요한 요소인 경향 점수(propensity score)를 직접 추정하는 방법론을 고려합니다. 경향 점수는 관측된 변수를 기반으로 치료군에 할당될 확률을 나타내며, 편향 보정 항 $h_0(X_i, D_i)$를 통해 ATE 추정을 개선할 수 있습니다. 본 연구에서는 직접적인 편향 보정 항 추정 방법을 Bregman divergence 최소화 관점에서 일반화하고, 이를 통해 기존 기법과의 연결점을 보여주었습니다.

- **Performance Highlights**: 제안된 방법의 효과성을 평가하기 위한 시뮬레이션 연구를 실시하였으며, 본 연구의 기법은 기존 ATE 추정 기법들에 비해 우수한 성과를 보여주었습니다. 특히, 편향 보정 항의 정확한 추정은 ATE 추정 정확도를 크게 향상시킬 수 있는 중요한 요소입니다. 이러한 접근은 향후 인과 추론(causal inference) 분야에 기여할 것으로 기대됩니다.
...
### Exploring the Early Universe with Deep Learning (https://arxiv.org/abs/2509.22018)
Comments:
          EPIA 2025 preprint version, 12 pages, 3 figures

- **What's New**: 이 논문에서는 수소 신호의 2D 파워 스펙트럼에서 정보를 추출하기 위해 최신 딥 러닝 기술을 개발하였습니다. 이를 통해 Square Kilometre Array Observatory (SKAO)에서 예상되는 우주 초창기 수소 재이온화 역사에 대한 데이터에서 정보를 예측할 수 있는 능력을 정량화합니다. 딥 러닝 기술은 초기 우주 연구에 현대적인 기법을 적용할 수 있게 해주며, 특히 지정된 머신 러닝 알고리즘이 재이온화 역사 회수에 0.95 이상의 R^2 점수를 달성할 수 있음을 보여줍니다.

- **Technical Details**: 우리는 SKA-Low에서 예상되는 데이터를 생성하기 위해 훈련 세트를 생산하였고, 각기 다른 우주론적 초기 조건과 여섯 가지 천체물리학적 파라미터를 변화시켜 21-cm 신호 측정을 시뮬레이션합니다. 이 데이터를 기반으로 2D 파워 스펙트럼을 계산하고, 다양한 주파수 범위에서의 신호 변동을 분석하여 EoR의 역사에 대한 인퍼런스를 수행합니다. 머신 러닝 방법을 적용하여 데이터 분석 파이프라인을 구축하는 것이 이 연구의 핵심입니다.

- **Performance Highlights**: 연구 결과, 딥 러닝 기법이 EoR의 역사 회복에서 0.95 이상의 R^2 점수를 달성하며, 이는 초기 우주의 구조 형성에 대한 신뢰할 수 있는 우주론적 및 천체물리학적 추론을 가능하게 합니다. SKAO에서 생성되는 대규모 데이터셋을 효과적으로 분석하기 위해 자동화되고 확장 가능한 접근 방식이 필수적입니다. 본 연구는 이러한 요구에 부응하는 머신 러닝 기반의 접근법이 중요함을 강조합니다.



### A Random Matrix Perspective of Echo State Networks: From Precise Bias--Variance Characterization to Optimal Regularization (https://arxiv.org/abs/2509.22011)
Comments:
          2026 IEEE International Conference on Acoustics, Speech, and Signal Processing

- **What's New**: 이번 연구에서는 Linear Teacher와 oracle weights를 이용한 Echo State Networks (ESNs)의 비대칭 이론을 제시합니다. 랜덤 매트릭스 이론을 활용하여 입력 통계, oracle 벡터 및 ridge 정규화 매개변수에 대한 평균 제곱 오차(MSE), 비대칭 편향, 분산을 함수 형태로 유도했습니다. 이 분석 덕분에 ESNs가 double descent을 나타내지 않으며, 훈련 샘플 수와 teacher 메모리 길이가 제한적일 때 MSE가 낮아진다는 두 가지 주요 발견이 있었습니다.

- **Technical Details**: 연구는 감독 학습 작업을 다루며, 입력 신호는 T 길이의 temporally signal 𝐮∈ℝT로 주어집니다. 시스템의 출력은 노이즈가 포함된 선형 teacher 모델에 의해 생성됩니다. 학생 모델은 이 입력을 fixed transformation F를 통해 feature 𝐳로 변환하고, 이를 통해 teacher를 근사화하는 방식을 채택합니다. 다양한 feature map을 사용하여 모델의 성능을 비교하고 있습니다.

- **Performance Highlights**: ESNs는 제한된 데이터와 짧은 메모리 상태에서 ridge regression을 능가하는 성능을 발휘함을 보이고 있습니다. 또한 isotropic inputs의 경우 최적의 정규화 방식에 대한 명확한 공식을 도출했습니다. 최종적으로, 연구 결과는 ESNs의 조정에 관한 해석 가능한 이론과 실용적인 가이드를 제공하여 최근의 경험적 관찰과 증명 가능한 성능 보장을 조화롭게 하고 있습니다.
...
- **What's New**: 이 논문에서는 Gaussian Process Discrete Hawkes Process (GP-DHP)라는 비모수 모델을 제안합니다. GP-DHP는 사건 발생의 바탕이 되는 baseline과 사건을 유도하는 excitation을 모두 Gaussian process priors를 사용하여 모델링합니다. 이는 데이터에 적응할 수 있는 부드러운 구조를 제공하며, 기존의 팩터 모델에서 요구되는 제한적인 형태를 벗어날 수 있게 합니다.

- **Technical Details**: GP-DHP는 사건 발생의 강도를 이산 시간 단계에서 정의하며, 이는 자극적인 사건과 비자극적인 사건의 가중합으로 나타냅니다. 모델은 MAP 추정을 통해 파라메트릭한 제약 없이 baseline과 excitation을 추정할 수 있는 능력을 가지고 있습니다. 이 과정에서 시간 복잡도는 O(T log T)로, 빠르면서도 해석 가능한 구조를 보장합니다.

- **Performance Highlights**: 시뮬레이션과 두 가지 사례 연구(미국 테러 사건 및 주간 Cryptosporidiosis 발생 수)를 통해 GP-DHP는 테스트 예측 로그 우도를 개선하는 성과를 보였습니다. 이 모델은 폭발적인 사건 발생, 시간 지연, 계절적 배경 변화를 포착하며, 기존의 모델들보다 더 유연한 비모수적 접근이 가능함을 보여주었습니다.
...
### Learnable Conformal Prediction with Context-Aware Nonconformity Functions for Robotic Planning and Perception (https://arxiv.org/abs/2509.21955)
- **What's New**: 이번 연구에서는 Learnable Conformal Prediction (LCP)이라는 새로운 접근 방식을 제안합니다. LCP는 고정된 비순응도 점수 대신 경량 신경망 기능을 사용하여 맥락 인식 불확실성 집합을 생성합니다. 이 방법은 기존의 Conformal Prediction (CP)이 제공하는 이론적 보장을 유지하면서 클래스 분류에서 예측 집합 크기를 18% 줄이고, 감지 간격을 52% 단축시키며, 경로 계획의 성공률을 72%에서 91%로 개선합니다.

- **Technical Details**: LCP는 기능 지향 모델 sθ​(x)=fθ​(ϕ​(x))를 도입하여 예측 오류의 구조에 적응합니다. 입력값의 기하학적, 의미적, 그리고 작업 특정 기능을 인코딩하는 ϕ​(x)와 하이퍼파라미터 θ로 학습된 기능 fθ가 결합되어 예측의 불확실성을 다루는 새로운 방식입니다. 이 방법은 학습된 점수들에 대한 보정을 통해 CP의 유한 샘플 적용 보장을 유지하면서 간단한 경우에는 인터벌을 축소하고 어려운 경우에는 확대합니다.

- **Performance Highlights**: LCP는 세 가지 로봇 작업(경로 계획, 물체 감지, 이미지 분류)에서 테스트되었으며, 모든 벤치마크에서 Standard CP 및 앙상블 기준보다 일관되게 성능을 개선하였습니다. MRPB 벤치마크에서 경로 계획 성공률을 91.5%로 증가시키고 4.5%의 경로 팽창을 유지했습니다. 또한 COCO, BDD100K, Cityscapes에서 물체 감지 시 평균 인터벌 폭을 46-54% 단축하며, CIFAR-100, HAM10000, ImageNet의 분류에서 예측 집합 크기를 4.7-9.9% 줄였습니다.
...
### Sequential 1-bit Mean Estimation with Near-Optimal Sample Complexity (https://arxiv.org/abs/2509.21940)
- **What's New**: 이 논문에서는 1비트 통신 제약을 가진 분산 평균 추정 문제를 연구합니다. 특히, 제안하는 1비트 평균 추정기는 무작위화된 구간 쿼리를 기반으로 하며, 이는 주어진 샘플이 특정 구간에 포함되는지를 나타내는 1비트 결과를 제공합니다. 기존의 결과와 비교했을 때, 이는 평균 추정에 대한 미니맥스 하한에 거의 일치하는 샘플 복잡도를 제공합니다.

- **Technical Details**: 우리는 평균 추정기의 샘플 복잡도 경계를 도출했으며, 이는 bound된 평균(-λ ≤ E(X) ≤ λ)과 분산(Var(X) ≤ σ²) 조건을 갖는 모든 분포에 대해 적용됩니다. 제안된 알고리즘은 샘플과 상호작용 역사에 의존하여 쿼리 선택을 조정하는 적응형(Adaptive) 구조를 가지고 있습니다. 또한, 더욱이 강한 꼬리 감소를 가진 분포에 대해 더 엄밀한 샘플 복잡도 경계를 제공합니다.

- **Performance Highlights**: 제안된 1비트 평균 추정기는 양자화되지 않은 설정의 미니맥스 하한과 거의 일치하는 샘플 복잡도를 달성합니다. 이는 기존의 다른 비어르간 제약을 가진 평균 추정기의 하한과는 대조적으로 λ/σ의 로그 스케일을 가지며, 이는 기존의 하한과 다릅니다. 또한, 구간 쿼리 기반 추정기의 변별력이 뛰어나며, 특히 더 큰 λ/σ 비율에서 비적응형 평균 추정기보다 월등한 성능을 나타냅니다.



### Error Analysis of Discrete Flow with Generator Matching (https://arxiv.org/abs/2509.21906)
- **What's New**: 이번 연구에서는 이산 흐름 모델(discrete flow models)의 수렴 특성과 오류 분석(error analysis)에 대한 기존 연구가 부족하다는 점을 지적하며, 이를 보완하기 위해 확률 미적분학 이론(stochastic calculus theory)에 기초한 통합 프레임워크를 개발했습니다. 우리는 새로운 Girsanov 유형 정리를 통해 서로 다른 전이율(transition rates)을 가진 두 개의 연속 시간 마르코프 체인(CTMC) 간의 경로 측정(path measures)의 KL 발산(KL divergence)을 도출했습니다.

- **Technical Details**: 본 논문에서는 전이율 추정에서 발생하는 오류와 조기 중지(early stopping)로 인한 포괄적인 분석을 제공하며, 특히 기존 연구에서 잘 다뤄지지 않은 전이율 추정 오류를 집중적으로 분석합니다. 이산 흐름 모델은 노이징(noising) 과정에서 시간 지평을 절단할 때 발생하는 절단 오류(truncation error)가 없기 때문에, 기존의 이산 확산 모델(discrete diffusion models)보다 더 유리한 특징을 가지고 있습니다.

- **Performance Highlights**: 우리는 생성기 매칭(generator matching) 및 균일화(uniformization)에 기초하여 분포 추정(distribution estimation)에 대한 비대칭 오차 경계(non-asymptotic error bounds)를 확립했습니다. 이 결과는 이산 흐름 모델에 대한 최초의 오류 분석을 제공하며, 이 모델들이 이산 확산 모델보다 우수한 성능을 발휘할 수 있는 이유를 명확히 보여줍니다.
...
### Causal-EPIG: A Prediction-Oriented Active Learning Framework for CATE Estimation (https://arxiv.org/abs/2509.21866)
- **What's New**: 이 논문에서는 Conditional Average Treatment Effect (CATE)를 추정하기 위한 능동 학습(active learning)의 중요성을 강조하고 있습니다. 기존의 능동 학습 전략들은 모델 파라미터의 불확실성을 줄이는 데 초점을 맞추었으나, 이는 실제로 관심 대상인 관찰할 수 없는 인과량(causal quantities)을 직접적으로 다루지 못한다는 문제를 지적합니다. 이 문제를 해결하기 위해, 인과 목표 정렬(causal objective alignment) 원리를 도입하였으며, 이를 통해 진정한 인과량에 관한 획득 함수를 개발합니다.

- **Technical Details**: 제안된 Causal-EPIG 프레임워크는 Expected Predictive Information Gain (EPIG)이라는 정보 이론적 기준을 활용하여, 관찰할 수 없는 인과량에 대한 불확실성을 줄이는 쪽으로 쿼리(query)의 가치를 정량화하는 방법을 제시합니다. 이 프레임워크를 통해 직관적인 접근법과 CATE 추정값에 직접 초점을 맞춘 접근법을 포함하는 두 가지 전략을 도출합니다. 각 전략은 기초 추정기(base estimator)와 데이터 복잡성(data complexity)에 따라 최적의 성능을 발휘합니다.

- **Performance Highlights**: 광범위한 실험을 통해 제안된 전략들이 기존의 표준 기준을 일관되게 초월하는 것을 입증하였습니다. 특히, 최적의 전략은 문맥에 따라 달라지며, 기초 추정기와 데이터의 복잡성에 의존하는 경향이 있음을 밝혔습니다. 이 연구의 결과는 실제에서 샘플 효율적인 CATE 추정(sample-efficient CATE estimation)을 위한 원칙적 가이드를 제공합니다.
...
### Reinforcement Learning Based Traffic Signal Design to Minimize Queue Lengths (https://arxiv.org/abs/2509.21745)
- **What's New**: 이번 연구에서는 Reinforcement Learning (RL)에 기반한 새로운 적응형 교차로 신호 제어 프레임워크가 제안되었습니다. Proximal Policy Optimization (PPO) 알고리즘을 활용하여 모든 신호 단계에서 총 대기 열을 최소화하는 것이 목표입니다. 특히, 기존의 RL 접근 방식들이 최적화 목표에 집중한 반면, 이 방법은 교통 패턴을 반영한 상태 표현을 여러 가지 방식으로 확장했습니다.

- **Technical Details**: 이 연구에서는 다양한 상태 표현 방식을 통해 복잡한 교통 조건을 효율적으로 표현하는 문제를 다루었습니다. AUTOENCODER 및 K-Planes inspired 표현이 포함된 다중 상태 표현 방식이 적용되었습니다. SUMO (Simulation of Urban Mobility) 시뮬레이터를 통해 제안된 알고리즘이 구현되었고, 교통 신호 및 감지기 데이터를 이용해 대기 열을 추정하는 다양한 데이터 기반 모델이 사용되었습니다.

- **Performance Highlights**: 제안된 PPO-RL 접근 방식은 전통적인 Webster 방법 및 다른 RL 기반 접근 방식에 비해 평균 대기 열에서 약 29%의 감소를 기록하며 우수한 성능을 보였습니다. 대기 길이를 기반으로 한 보상 기획 또한 효과적임을 입증하여, 도시의 교통 관리를 위한 스케일러블하고 적응 가능한 잠재력을 보여주었습니다. 추가적으로 다양한 보상 공식들과의 비교 평가를 통해 제안된 접근 방식의 효과성이 입증되었습니다.
...
### Noise-to-Notes: Diffusion-based Generation and Refinement for Automatic Drum Transcription (https://arxiv.org/abs/2509.21739)
- **What's New**: 이 논문에서는 전통적인 자동 드럼 전사(ADT)를 조건부 생성 과제로 재정의하고, 오디오 조건화된 가우시안 노이즈를 드럼 이벤트로 변환하는 Noise-to-Notes(N2N) 프레임워크를 소개합니다. 이 생성적 확산 접근 방식은 속도와 정확도 간의 유연한 트레이드 오프 및 강력한 완료 기능을 제공합니다. 특히 새로운 Annealed Pseudo-Huber 손실을 도입하여 효과적인 공동 최적화를 가능하게 하고, 음악 기반 모델로부터의 특징을 통합하여 저수준 스펙트로그램 기능을 보강합니다.

- **Technical Details**: N2N은 동시에 드럼 온셋과 속도 전사를 수행하는 오디오 조건부 확산 모델로, 전사 목표는 x∈ℝN×D×2로 표현됩니다. 각각의 프레임에 대해, 기본 진실 주석(x0)은 드럼 히트의 존재를 나타내는 온셋과 강도를 나타내는 속도로 구성됩니다. ADT를 확산 프레임워크로 재구성하고, 네트워크는 노이즈 전사를 깨끗한 전사로 복원하도록 훈련됩니다. 기존의 손실 함수에 비해 Annealed Pseudo-Huber 손실을 사용하여 최적화를 보다 원활하게 이끌어냅니다.

- **Performance Highlights**: 실험 결과, N2N 모델은 여러 ADT 벤치마크에서 새롭고 최첨단의 성능을 나타내며, MFM 피처를 포함시키는 것이 모델의 견고성을 크게 향상시킴을 보여줍니다. 훈련은 4대의 NVIDIA A100 GPU를 사용하여 약 하루 정도 소요되었으며, 모델 설정은 기존의 연구를 따랐습니다. 최종적으로 N2N은 기존의 판별 모델보다 우수한 성과를 올리며, 드럼 전사 작업에서 새로운 가능성을 제시합니다.
...
### Multi-modal Bayesian Neural Network Surrogates with Conjugate Last-Layer Estimation (https://arxiv.org/abs/2509.21711)
Comments:
          35 pages including references and appendix, 5 figures

- **What's New**: 이 논문에서는 멀티모달 데이터(multi-modal data)를 활용하는 두 가지 새로운 베이지안 신경망(Bayesian Neural Network, BNN) 서리게이트 모델(surrogate model)을 소개합니다. 멀티모달 학습(multi-modal learning)의 이점을 활용하여, 이 모델은 실제 관측치가 부분적으로 결여된 경우에도 추정 과정을 수행할 수 있는 방법을 제안합니다. 이 연구는 기존 유니모달 서리게이트 모델(unimodal surrogate model) 대비 예측 정확성(prediction accuracy)과 불확실성 정량화(uncertainty quantification)를 개선한 결과를 보입니다.

- **Technical Details**: 제안된 모델은 조건부 공액분포(conditional conjugate distributions)를 활용하여 최종 레이어에서 매개변수(parameter)를 추정하는데 있어 확률적 변분 추론(stochastic variational inference, SVI) 기법을 사용합니다. 이 방법론은 복합적인 비모수적 모델에 대한 추정 방법을 제공하며, 관측치의 일부가 결여된 상황에서도 견고한 성능을 발휘할 수 있도록 설계되었습니다. 이러한 접근 방식은 목표 함수(objective function)의 예측을 보다 정확하게 수행하며, Bayesian 최적화(Bayesian optimization)에서 유용하게 활용될 수 있습니다.

- **Performance Highlights**: 실험 결과, 제안된 멀티모달 베이지안 신경망 모델은 스칼라(scalar) 및 시계열(time series) 데이터 모두에서 유니모달 서리게이트 모델보다 우수한 성능을 보여주었습니다. 특히, 예측의 정확성과 더불어 예측에 대한 불확실성을 효과적으로 정량화하는 능력을 입증하였습니다. 이러한 결과는 멀티모달 데이터의 복잡한 관계를 통해 의미 있는 예측을 가능하게 하며, 향후 다양한 응용 분야에 적용될 수 있는 잠재력을 지니고 있습니다.



### SADA: Safe and Adaptive Inference with Multiple Black-Box Predictions (https://arxiv.org/abs/2509.21707)
- **What's New**: 이 논문에서는 레이블이 있는 데이터의 부족 문제를 해결하기 위해 여러 블랙박스 모델에서 예측을 안전하고 적응적으로 집계하는 새로운 접근 방식을 제안합니다. 제안된 방법은 예측의 품질에 관계없이 레이블이 있는 데이터만 사용할 때보다 나쁘지 않도록 보장하며, 만약 예측 중 하나가 진짜 값을 완벽히 반영한다면 이를 효과적으로 활용할 수 있습니다. 이는 머신러닝 모델의 다양한 예측을 최대한 활용하는 동시에 신뢰할 수 있는 추정과 추론을 가능하게 합니다.

- **Technical Details**: 제안된 방법은 데이터 기반의 가중치를 예측값에 부여하여 유용한 정보를 활용하고, 품질이 낮은 예측의 영향을 줄여 분산을 완화할 수 있도록 설계되었습니다. 특징적으로, 하나의 예측이 완벽하게 정확할 경우 이 방법은 진짜 값을 아는 것처럼 작동하게 됩니다. 본 연구는 특정 고유 조건을 만족하는 예측이 포함된 경우에 최적의 효율성을 달성할 수 있음을 이론적으로 입증합니다.

- **Performance Highlights**: 실험을 통해 제안된 알고리즘의 효과를 입증하였으며, 그 결과는 합성 데이터셋과 벤치마크 데이터셋 모두에서 나타났습니다. 또한, 제안된 방법은 레이블이 있는 데이터만 사용하는 것보다 항상 나쁘지 않은 성능을 보장하며, 예측의 품질에 따라 더욱 효과적으로 추정을 수행할 수 있음을 보여줍니다.
...
- **What's New**: 이 논문에서는 HuLA(인간 유사 청취기)를 제안하여 반사기(anti-spoofing) 시스템에서 감정 표현이 풍부한 합성 음성을 효과적으로 탐지하는 방법을 소개합니다. 기존 시스템의 한계를 극복하기 위해, F0 패턴과 나타난/나타나지 않은(Voiced/Unvoiced) 구조와 같은 운율 정보(prosody)를 사용하여 자연 음성과 합성 음성을 구별합니다. 이는 반사기 기술이 생체 인증 또는 정보 전파를 악용하는 데 큰 위험이 있음을 강조하며, 더욱 강력하고 일반화된 시스템의 필요성을 제기합니다.

- **Technical Details**: HuLA는 두 단계로 구성된 다중 작업 학습 프레임워크로, 1단계에서는 실제 음성 데이터에서 F0 예측 및 V-UV 분류를 수행하는 자기지도 학습(SSL) 기초 모델을 훈련합니다. 2단계에서는 실제 및 합성 데이터에서 스푸프 탐지를 위한 모델 최적화를 시행하여, 자연과 감정이 풍부한 합성 음성 간의 불일치를 탐지하는 데 운율 정보를 활용합니다. 이는 모델이 자연스러운 운율 변화를 포착하도록 증가시키는 데 초점을 맞추고 있습니다.

- **Performance Highlights**: HuLA는 도메인 외 데이터셋에서도 일관되게 우수한 성능을 보이며, 특히 감정적 음성, 교차 언어 공격 및 복잡한 공격 유형에서 강력한 성능을 발휘합니다. 실험 결과는 명시적 운율 감독이 SSL 임베딩과 결합될 경우, 고급 합성 음성 공격에 대한 내구성을 상당히 향상시킨다는 것을 입증합니다. 이를 통해, 이 연구는 반사기 연구 분야에서 운율의 중요성을 부각시키며 향후 연구 방향을 제시합니다.
...
### Generating Stable Placements via Physics-guided Diffusion Models (https://arxiv.org/abs/2509.21664)
Comments:
          Submitted to the IEEE International Conference on Robotics and Automation 2026, Vienna, Austria, June 1-5, 2026

- **What's New**: 이 논문에서는 다중 객체 장면에서 물체를 안정적으로 배치하는 문제를 해결하기 위한 새로운 접근 방식을 제안합니다. 기존의 방법들이 시뮬레이션 엔진이나 외관 기반 추정에 의존하는 것과 달리, 이 연구는 물리 기반 피쳐를 고려하여 안정성 평가를 샘플링 과정에 직접 통합합니다. 이를 통해 수치적 재훈련 없이도 오프-the-shelf 모델에 적용할 수 있는 안정을 추구하는 새로운 방식으로 자리잡고 있습니다.

- **Technical Details**: 제안된 접근 방식은 물체의 일반적인 형태와 장면의 객체 포인트 클라우드를 조건으로 한 확산 모델(diffusion model)을 활용하여 안정적인 배치를 생성합니다. 모델의 훈련 과정에서는 물리 기반 안정성 척도깡(stability-aware loss)을 활용해 높은 안정성 지역에서 샘플링의 확률을 증가시키며, 이를 통해 배치의 유효 비율을 40% 향상시킬 수 있음을 보여줍니다. 이 과정은 추가적인 재훈련이 필요하지 않아, 기존 모델에 쉽게 통합할 수 있습니다.

- **Performance Highlights**: 제안된 물리 안내 모델은 다양한 벤치마크 장면에서 평가되어, 가장 최근의 기하학적 방법에 비해 56% 더 강한 외력 내구성을 보이며, 실행 시간을 47% 단축하는 성과를 달성하였습니다. 연구 결과는 특히 복잡한 장면에서의 안정적인 물체 배치의 효율성을 높일 수 있음을 보여 주며, 이는 로봇 조작 및 관련 애플리케이션에서 큰 의미를 가집니다.



### Automating Sensor Characterization with Bayesian Optimization (https://arxiv.org/abs/2509.21661)
- **What's New**: 이 논문은 검출기 특성화의 효율성을 높이기 위해 자동화된 센서 보정(automated sensor calibration) 기술을 제안합니다. 이 기술은 실시간 측정을 활용하여 최적의 운영 상태를 식별하고 파라미터 선택을 안내하는 폐쇄 루프 베이지안 최적화(closed-loop Bayesian optimization)를 기반으로 합니다. 이를 통해 전문가의 감독 없이 몇 일 만에 센서를 효율적으로 특성화하고 최적화할 수 있습니다.

- **Technical Details**: SiSeRO 센서는 새로운 읽기 아키텍처를 통해 향상된 전하 감도를 제공하는 이미징 센서입니다. 이 센서는 MOSFET을 CCD 감지 노드에 통합하여 전하 측정의 정밀도를 개선하며, 광학 및 근적외선 대역에서 신뢰할 수 있는 광자 계수를 가능하게 합니다. 또한 센서는 배경 노이즈와 상당히 빠른 읽기 속도를 제공하여 Skipper-CCDs보다 6배 이상 빠르게 작동합니다.

- **Performance Highlights**: SiSeRO는 단일 전자의 감도를 기록적으로 1.54 nA/e-로 달성하며, 단일 샘플 모드에서 0.74 e-rms/pixel로 서브 전자 노이즈(sub-electron noise)를 보입니다. 반복 샘플링을 통해 0.021 e-rms/pixel의 노이즈를 실현하며, 이러한 성능은 넓은 동적 범위에 걸쳐 단일 전자 해상도를 확인합니다. 결과적으로 이 센서는 다음 세대 실험을 위한 요구 사항을 충족하기 위해 더욱 정밀한 모델링 및 특성화 필요성을 충족시키는 데 기여할 수 있습니다.



### A regret minimization approach to fixed-point iterations (https://arxiv.org/abs/2509.21653)
- **What's New**: 본 논문에서는 후회 최소화 알고리즘을 고정점(iterations) 반복으로 변환하는 새로운 방안을 제안합니다. 이 방법은 후회 경계(regret bounds)에서 유도되는 수렴 보장을 따릅니다. 고전적인 Krasnoselskii--Mann 반복을 일반화한 형태로, Online Gradient Descent 알고리즘을 변환함으로써 재현할 수 있습니다.

- **Technical Details**: 제안된 방법은 AdaGrad와 같은 후회 최소화 알고리즘을 고정점 반복으로 변환하는 데 중점을 두고 있습니다. 이러한 접근법은 비자기 연산자(non-self operators)의 고정점을 찾기 위한 새로운 간단한 반복(iterations)을 제공합니다. 새로운 형태의 적응 보장(adaptive guarantees)을 가진 고정점 반복을 생성할 수 있습니다.

- **Performance Highlights**: 다양한 문제에 대한 수치 실험(numerical experiments) 결과, AdaGrad 기반의 고정점 반복이 Krasnoselskii--Mann 반복보다 더 빠른 수렴 경향을 보임을 보여주었습니다. 즉, 제안된 접근법은 기존의 방법들에 비해 성능 향상이 있음을 나타냅니다.



### Automated Machine Learning Pipeline for Training and Analysis Using Large Language Models (https://arxiv.org/abs/2509.21647)
- **What's New**: 이번 연구는 Automated Machine Learning Pipeline (AMLP)을 소개합니다. AMLP는 데이터셋 생성에서 모델 검증까지의 전체 워크플로우를 통합하여, 머신러닝 상호 원자 포텐셜(MLIPs)을 보다 효과적으로 개발할 수 있도록 합니다. 이 파이프라인은 대형 언어 모델(Large Language Models)을 활용하여 전자 구조 코드 선택 및 입력 파일 준비를 지원하며, 다양한 분자 시뮬레이션을 가능한 하는 분석 도구인 AMLP-Analysis를 포함합니다.

- **Technical Details**: AMLP는 MACE 아키텍처를 기반으로 하며, 크리딘 다형체(acridine polymorphs)에 대한 검증을 통해 평균 절대 오차가 에너지에 대해 약 1.7 meV/atom, 힘에 대해 ~7.0 meV/Å에 도달하는 것을 보여줍니다. 이 모델은 DFT(밀도 범함수 이론) 지오메트리를 서브-Å 정확도로 재현하며, 분자 동역학 시뮬레이션에서도 안정성을 입증합니다. 이를 통해 각기 다른 시뮬레이션 조건에서의 일반화 및 적응력을 효과적으로 향상시킬 수 있습니다.

- **Performance Highlights**: AMLP는 사용자가 최소한의 코드 전문 지식으로도 편리하게 활용할 수 있도록 설계되어 있습니다. 또한 사용자가 제공한 간단한 프롬프트에 따라 적합한 QM(basis set) 방법론을 추천하여 신속하게 양질의 QM 계산을 설정할 수 있습니다. 모델 활용을 위해 AMLP-Analysis와 통합된 Python 기반의 Atomic Simulation Environment(ASE)에서 시뮬레이션 과제가 유연히 수행될 수 있도록 지원합니다.
...
### Effective continuous equations for adaptive SGD: a stochastic analysis view (https://arxiv.org/abs/2509.21614)
- **What's New**: 이 논문은 소규모 학습 비율에서 기인한 유명한 적응형 Stochastic Gradient Descent(SGD) 방법들의 이론적 분석을 제시합니다. Li et al.이 제안한 stochastic modified equations framework를 활용하여 이러한 방법들의 효과적인 연속 확률 동역학을 도출하였습니다. 연구의 주요 기여는, SGD에서의 샘플링 유도 잡음이 매개변수와 경량 이차 모멘텀 진화에 대한 독립적인 브라운 운동으로 나타난다는 것입니다.

- **Technical Details**: 우리는 RMSprop와 Adam과 같은 적응형 SGD 알고리즘의 연속시간 효과적인 SDE를 도출하기 위해 Stochastic Modified Equations(SMEs) 이론을 활용했습니다. 결과적으로, 적응형 방법들이 유지하는 내부 상태 변수들(예: 지수 가중 이동 평균)과 매개변수들이 함께 진화하여 복잡한 연속 시간 근사를 가져온다는 점이 확인되었습니다. 이러한 연구를 통해 샘플링으로부터 유도된 고유한 잡음으로 인해 발생하는 추가적인 잡음 층을 식별하고 특성을 분석했습니다.

- **Performance Highlights**: 적응형 방법이 학습 비율과 주요 하이퍼파라미터 간의 관계를 규명하는 스케일링 규칙 분석을 통해 비후보 SDE 경향성을 모두 특성화함을 보여주었습니다. 우리의 분석은 SGD의 샘플링 메커니즘의 확률성을 반영하여 추가적인 독립 랜덤 성분을 생성하는 것을 발견했습니다. 이 연구는 기계 학습 모델 훈련의 이론적 기초를 이해하는 데 중요한 기여를 할 것으로 기대됩니다.
...
### IndiSeek learns information-guided disentangled representations (https://arxiv.org/abs/2509.21584)
- **What's New**: 이 논문에서는 IndiSeek라는 새로운 불교차 표현 학습 접근법을 소개합니다. 이 방법은 독립성을 강제하는 목표와 조건부 상호정보(mutual information)를 제한하는 계산 효율적인 재구성 손실을 결합하여 이 문제를 해결합니다. 이로 인해 독립성과 완전성을 명확하게 균형잡고 모달리티 별 특징(modality-specific features)을 효과적으로 추출할 수 있게 됩니다.

- **Technical Details**: IndiSeek는 불교차 표현 학습에 있어 상호정보의 최소화 및 데이터와 이들의 조합 간의 상호정보 최대화라는 두 가지 정보론적 목표를 동시에 해결하려고 합니다. 최신 기법들이 이러한 상호정보를 정확히 추정하기 어려워, 독립성과 완전성 간의 트레이드오프가 발생하는 한계를 보이고 있습니다. 이러한 문제를 해결하기 위해, IndiSeek에서는 모달리티 별 특징을 독립적으로 유지하면서도 모든 보완 정보를 캡처할 수 있는 방식을 채택했습니다.

- **Performance Highlights**: IndiSeek는 합성 시뮬레이션, CITE-seq 데이터셋, 다양한 실제 다중 양식(multi-modal) 벤치마크에서 그 효과성을 입증했습니다. 기존의 SOTA 기법인 FactorizedCL과 InfoDisen이 이상적인 표현을 학습하지 못하는 점을 보여준 반면, IndiSeek는 모든 조건에서 성공적으로 모달리티 별 특징을 학습했습니다. 논문 내의 실험 결과, 세 가지 다른 학습 알고리즘을 비교하였고, IndiSeek는 모든 설정에서 제대로 된 모달리티 별 표현을 도출하였습니다.



### General Pruning Criteria for Fast SBL (https://arxiv.org/abs/2509.21572)
Comments:
          5 pages, 2 figures, submitted to IEEE Signal Processing Letters

- **What's New**: 본 연구는 Sparse Bayesian Learning (SBL)의 새로운 분석을 제안하여, 잡음 샘플과 가중치 분포에 대한 가우시안 가정이 약화된 경우의 주변 우도(marginal likelihood)를 조사합니다. 이 논문에서는 하이퍼파라미터의 추정 조건을 새로운 관점에서 다루며, 기존 F-SBL(빠른 SBL) 알고리즘의 가지치기(pruning) 조건과의 관련성을 강조합니다. 이를 통해 SBL의 이론적 기초를 더욱 견고하게 다지고 있으며, 그래픽적 해석을 통해 내부 작동 메커니즘을 직관적으로 설명합니다.

- **Technical Details**: SBL에서는 각 가중치가 평균이 0인 가우시안 분포를 따른다고 가정하며, 이는 하이퍼파라미터와 관련이 있습니다. 하이퍼파라미터는 가중치를 제외한 데이터를 마지널라이징(marginalizing)함으로써 추정됩니다. 본 연구에서는 하이퍼파라미터가 유한 혹은 무한으로 수렴하는 조건을 도출하고, 이러한 조건들이 기존의 가우시안 가정 하의 가지치기 조건과 상호 보완적인 관계임을 보여줍니다. 또한, F-SBL에서 마지널 우도를 최적화하는 방법을 소개합니다.

- **Performance Highlights**: 기존의 SBL 방법은 느린 수렴 속도로 인해 실용성이 떨어지는 문제를 겪고 있습니다. 그러나 본 논문에서는 F-SBL을 통해 마지널 우도를 극대화하는 새로운 접근을 제시하며, 이로 인해 많은 하이퍼파라미터 추정이 무한값으로 수렴되는 현상을 해결합니다. 연구진은 이러한 방식으로 가중치를 모델에서 효과적으로 제거하였고, 이는 희소한 가중치 벡터 추정을 가능하게 하였습니다.
...
Comments:
          Accepted at RAID 2025. (c) IEEE

- **What's New**: 이 논문에서는 기능적 암호화(Functional Encryption, FE)를 활용하여 암호화된 데이터에 대한 안전한 신경망 훈련을 시도하는 기존의 연구에서 존재하는 취약점을 밝혀냈습니다. 연구원들은 이전에 제안된 방법들이 정보 유출을 방지하지 못한다고 지적하며, 이를 통해 훈련 데이터의 원래 입력을 복원할 수 있는 공격을 제안합니다. 더욱이, 클라이언트 참여를 통해 계산 단계에서 보안을 강화하는 두 가지 솔루션도 제안하고 있습니다.

- **Technical Details**: 논문에서는 기능적 암호화(FE)의 구조를 설명하며, 이를 통해 암호화된 데이터에 대한 부분적인 복호화 결과를 얻는 과정을 다룹니다. FE는 내부 공학적 특수 기능을 가진 암호화 방법으로, 신경망의 훈련 과정에서 이 기능을 활용하여 연산을 수행합니다. 또한, 선형 프로그래밍(Linear Programming)을 이용한 공격 방법을 설명하며, 기존의 방법들과 비교하여 새로운 공격 방식을 제안합니다.

- **Performance Highlights**: 제안된 공격 방법은 CIFAR-10 데이터셋을 사용하여 확인되었으며, 두 가지 솔루션(MITIG1 및 MITIG2)의 효과가 분석되었습니다. MITIG1은 기능 숨기기(inner product) 암호화 기술을 이용하며, MITIG2는 완전히 암호학적 방법을 사용하지 않고 클라이언트 참여를 통해 보안을 유지합니다. 이 두 솔루션은 기존의 안전한 훈련 방법들과 비교하여 우수한 성능을 보였습니다.



### Context-Aware Hybrid Routing in Bluetooth Mesh Networks Using Multi-Model Machine Learning and AODV Fallback (https://arxiv.org/abs/2509.21490)
Comments:
          15 pages, 2 figures

- **What's New**: 이 논문에서는 비상 상황이나 자원이 제한된 환경에서 오프라인 통신을 위한 Bluetooth 기반의 메쉬 네트워크를 다룹니다. 기존의 라우팅 전략인 Ad hoc On-Demand Distance Vector (AODV)가 혼잡이나 동적 토폴로지 변화 아래에서 성능 저하를 보인다는 문제를 해결하고자 합니다. 이를 위해, AODV를 감독 기계 학습(Supervised Machine Learning)과 결합한 하이브리드 지능형 라우팅 프레임워크를 제안합니다.

- **Technical Details**: 제안된 프레임워크는 배달 성공 분류기(delivery success classifier), TTL 회귀모델(TTL regressor), 지연 회귀모델(delay regressor), 및 전달자 적합성 분류기(forwarder suitability classifier) 등 네 가지 예측 모델을 통합합니다. 이 모델들은 다중 홉 메시지 전송 중 이웃 노드를 동적으로 순위 매기는 통합 스코어링 메커니즘을 통해 상호작용하여 라우팅을 최적화합니다. 또한, 버퍼 제약(buffer constraints)과 장치 이질성(device heterogeneity)을 고려한 시뮬레이션 환경을 구축하여 평가를 진행했습니다.

- **Performance Highlights**: 하이브리드 ABCD 모델은 제한된 조건 하에서 약 99.97%의 패킷 전달 성공률을 기록하며, 기본 AODV 및 중간 하이브리드 모델인 ABC를 크게 초월하는 성능을 보여줍니다. 이 결과는 경량화되고 설명 가능한 기계 학습 모델이 Bluetooth 메쉬 네트워크에서 라우팅 신뢰성과 적응력을 향상시킬 수 있음을 입증합니다. 특히 인프라가 없는 환경에서 전달 성공률이 지연 제약보다 우선시된다는 점에서도 중요한 시사점을 제공합니다.
...
### Data-driven approach to the design of complexing agents for trivalent transuranium elements (https://arxiv.org/abs/2509.21362)
Comments:
          9 pages, 7 figures,

- **What's New**: 이 연구에서는 트랜스우라늄(Transuranium) 원소의 복합체를 조사하기 위해 최신 인공지능(AI) 기술을 활용하여 새로운 신경망 아키텍처를 개발하였습니다. 이 신경망은 기존의 실험 데이터를 사용하여 연구의 품질을 크게 향상시키 추세입니다. 특히, 복합체의 안정성에 영향을 주는 분자 파편을 규명하고 모델의 적용 범위를 밝히는 데 중점을 두었습니다.

- **Technical Details**: 연구의 주요 기술적 요소는 인공신경망(ANN)과 그래프 컨볼루셔널 네트워크(GNN)입니다. GNN은 몰분 구조와 금속 특성 벡터 간의 관계를 정의하고, 다중 입력 구조를 통해 데이터 부족 문제를 보완하는 데 효과적입니다. 데이터셋은 NIST46 데이터베이스와 문헌 자료를 기반으로 구축되었으며, 1:1 스토이코메트릭 복합체에 대해 실험 안정성 상수만을 포함하였습니다.

- **Performance Highlights**: 개발된 모델은 복합체의 안정성 상수를 예측하는 데 매우 높은 정확성을 보였습니다. 또한, 각 금속에 대한 특성 벡터와 리간드 그래프를 입력으로 사용하여 안정성 예측을 수행하며, 특정 금속을 훈련에 사용하지 않고도 예측이 가능하다는 장점을 가지고 있습니다. 이를 통해 모델의 적용 가능성을 높이고 안정성 상수 예측의 질을 제고할 수 있었습니다.
...
### Accurate typhoon intensity forecasts using a non-iterative spatiotemporal transformer mod (https://arxiv.org/abs/2509.21349)
Comments:
          41 pages, 5 figures in the text and 6 figures in the appendix. Submitted to npj Climate and Atmospheric Science

- **What's New**: TIFNet은 트랜스포머(Transformer) 기반의 예측 모델로, 5일간의 강도 변화 예측을 제공합니다. 고해상도(global) 예측과 역사적 진화 융합 메커니즘(historical-evolution fusion mechanism)을 통합하여 비반복적(non-iterative)인 예측을 생성합니다. 이 모델은 기존의 모델들이 겪는 급격한 강도 변화 예측 문제를 해결하는 데 중점을 두고 있습니다.

- **Technical Details**: TIFNet은 재분석 데이터(reanalysis data)를 기반으로 훈련되고, 운영 데이터(operational data)로 세밀하게 조정됩니다. 이 모델은 강력한 슈퍼Typhoon(super typhoon) 범주와 약한 강도 범주를 포함한 모든 예측 영역에서 운영 수치 모델을 일관되게 초월하고 있습니다. 특히, 강도가 급격하게 변화하는 환경에서 전통적인 모델들보다 예측 오류를 29-43% 줄이는 성과를 보여주고 있습니다.

- **Performance Highlights**: TIFNet의 성능은 극한 조건에서도 전통적인 모델들이 일관되게 저조한 예측을 하는 반면, 인공지능을 통한 TC 강도 예측에서 상당한 개선을 나타냅니다. 이 결과는 자연재해 대비와 인프라 회복력 측면에서 중요한 의미를 갖습니다. 또한, TIFNet은 모든 예측 기간에 걸쳐 신뢰할 수 있는 개선을 제공합니다.
...
- **What's New**: 이번 연구는 뇌파 신호(Electroencephalography, EEG) 기반으로 하는 머신러닝(Machine Learning, ML) 모델의 제약을 극복하기 위해 스파이킹 뉴럴 네트워크(Spiking Neural Networks, SNN)를 채택하여 인지 부하(Mental Workload)를 실시간으로 감지하는 가능성을 탐구합니다. 전통적인 기계 학습 모델의 높은 계산 비용을 고려할 때, SNN은 저전력과 빠른 이벤트 기반 처리의 장점을 제공합니다. 본 연구는 다중 모달(open-source multimodal) 데이터셋을 활용하여 SNN의 성능을 전통적인 ML 모델과 비교하면서 이의 가능성을 입증하고 있습니다.

- **Technical Details**: 연구 방법론은 24명의 참가자로부터 수집된 315시간의 다중 모달 생리 신호를 포함합니다. EEG는 Muse S 헤드밴드를 사용하여 256 Hz에서 기록되었으며, 심박수 변동성(Heart Rate Variability, HRV)과 전기 피부 반응(Electrodermal Activity, EDA) 등 다양한 생리적 지표가 사용되었습니다. 신호는 잡음 제거 후 시간 및 주파수 도메인에서 특징이 추출되었으며, 각 세그먼트는 '쉬운' 또는 '어려운' 태스크로 레이블링되었습니다.

- **Performance Highlights**: SNN 모델은 다중 모달 생리 신호 통합을 통해 기존 ML 모델들과 비교할 때 유사한 정확도를 얻는 것으로 나타났습니다. 제안된 SNN 접근 방식은 특히 저지연, 에너지 효율적인 시스템에서 실시간 인지 부하 모니터링을 위한 유망한 대안으로 자리 잡을 수 있습니다. 본 연구는 SNN이 인지 부하 모니터링에 있어 전통적인 ML 접근법보다 생물학적으로 영감을 받은 효과적인 대안임을 보여주고 있습니다.
...
### Interpretable Spectral Features Predict Conductivity in Self-Driving Doped Conjugated Polymer Labs (https://arxiv.org/abs/2509.21330)
Comments:
          31 Pages, 19 Figures

- **What's New**: 이 논문에서는 자체주행 실험실(Self-driving labs, SDLs)을 통한 빠른 물질 발견의 가능성을 탐구합니다. 특히, 도핑된 공액 고분자(Conjugated Polymers, CPs)의 전기 전도도를 예측하는 방법을 제시하며, 저비용으로 신속하게 측정할 수 있는 광학 스펙트럼을 활용하여 해석 가능한 스펙트럼 지문을 학습합니다. 이 방법은 전통적인 실험과 비교하여 33%의 실험 노력을 줄이면서도 높은 예측 정확도를 유지합니다.

- **Technical Details**: 연구에서는 유전자 알고리즘(Genetic Algorithm, GA)과 곱셈법(Area Under the Curve, AUC)을 결합하여 스펙트럼에서 특징을 추출합니다. 광학 스펙트럼은 비파괴적이고 빠르며 집합과 전하 생성에 민감하게 반응할 수 있는 장점을 보입니다. 도메인 지식을 바탕으로 한 특징 확장을 통해 소량 데이터 상황에서도 예측 모델의 정확도와 해석 가능성을 향상시키며, SHAP 선택 기법을 적용하여 중요한 피처 집합을 보존합니다.

- **Performance Highlights**: 이 모델은 전문가가 정의한 피처를 활용한 기준 모델과 유사한 성능을 보이며, 데이터 기반 모델과 전문가 피처의 하이브리드 모델은 두 모델보다 우수한 예측 성능을 보여 줍니다. 이 연구는 광학 스펙트럼이 전도성을 예측하는 데 효과적임을 입증하며, 도핑에 따른 폴리머의 알려진 서술자를 회복하고, 성공적인 도핑 과정에서의 고분자 탈색과 관련된 tail-state 영역을 밝힙니다.

...
### Water and Electricity Consumption Forecasting at an Educational Institution using Machine Learning models with Metaheuristic Optimization (https://arxiv.org/abs/2410.19709)
Comments:
...
- **What's New**: 본 연구는 기후 변수의 영향을 평가하기 위해 Random Forest (RF)와 Support Vector Regression (SVR) 두 가지 머신러닝 모델을 비교하여 파라나 연방 대학교 캠퍼스에서의 물과 전기 소비 예측을 실시한 것입니다.

- **Technical Details**: 데이터는 지난 5년간 수집되었으며, 청구서 정보와 외생(exogenous) 및 내생(endogenous) 변수를 결합하여 사용했습니다. 두 모델의 하이퍼파라미터는 Genetic Algorithm (GA)을 이용하여 최적화 하였고, 물과 전기 소비 예측에서 기후 변수가 포함된 경우와 없는 경우의 성능을 분석했습니다.

- **Performance Highlights**: 12개월 예측 기간 동안 Random Forest 모델이 가장 우수한 성능을 보였으나, 기후 변수를 포함할 경우 예측 정확도가 감소하여 높은 오류율을 나타냈습니다. 또한, 두 모델 모두 물 소비 예측에 어려움이 있어, 향후 다른 모델이나 변수를 활용한 추가 연구가 필요함을 시사합니다.



### Super Gradient Descent: Global Optimization requires Global Gradien (https://arxiv.org/abs/2410.19706)
- **What's New**: 이 논문에서는 Super Gradient Descent (SuGD)라는 새로운 최적화 방법을 소개하여 1차원 함수의 전역 최소값(global minimum)을 보장합니다. k-Lipschitz 함수에 대해 전역 수렴(convergence)을 보장하며, 형식적인 최적화 알고리즘의 한계를 극복합니다.

- **Technical Details**: Super Gradient Descent는 전통적인 gradient descent 알고리즘과의 차별점으로, 지역 정보만을 수집하는 것이 아니라 전역 그래디언트(global gradient)에 기반하여 결정을 하여 전역 최소값에 일관되게 접근합니다. 이 방법은 종료 구간 [a, b]에서 정의된 k-Lipschitz 연속 함수에 대해 수렴을 보장합니다.

- **Performance Highlights**: SuGD를 다양한 1차원 함수에 적용한 결과, 전통적인 알고리즘보다 지역 최소값(local minima)을 피하고, 전역 최적값에 도달하는 속도가 뛰어난 것으로 나타났습니다. 이는 비볼록(non-convex) 최적화 문제에 대한 더 신뢰할 수 있는 해결책을 제공합니다.



### Robust Thompson Sampling Algorithms Against Reward Poisoning Attacks (https://arxiv.org/abs/2410.19705)
- **What's New**: Thompson 샘플링(Thompson sampling) 알고리즘이 적대적 보상 오염(adversarial reward poisoning)에 강건하게 만들어질 수 있는 가능성을 처음으로 제시한 연구입니다. 이를 통해 적대적 상황에서도 신뢰성 있는 의사결정을 지원하려고 합니다.

- **Technical Details**: 이 연구에서는 스토캐스틱(stochastic) 및 선형 컨텍스트(linear contextual) 다중무장밴딧(multi-armed bandit) 환경에서 강건한 알고리즘을 제안합니다. 두 가지 아이디어를 도입하였는데, 첫 번째 아이디어는 '오염에 맞서는 최적성(optimality in the face of corruption)'으로, 두 번째는 공격에 덜 취약한 가중 추정량(weighted estimator)입니다.

- **Performance Highlights**: 우리의 알고리즘은 공격 하에서도 근사 최적의 회귀(regret)를 보장하며, 실험적으로도 기존의 다중무장밴딧 알고리즘보다 더 강건한 성능을 발휘합니다. state-of-the-art인 CW-OFUL 알고리즘에 비해 효율성을 유지하면서 Thompson 샘플링의 탐색 전략의 장점도 함께 갖추고 있습니다.



### Learning the Regularization Strength for Deep Fine-Tuning via a Data-Emphasized Variational Objectiv (https://arxiv.org/abs/2410.19675)
- **What's New**: 이 논문은 그리드 서치(grid search) 대신 전체 훈련 세트에서 정규화 하이퍼파라미터를 직접 학습하는 새로운 방법을 제안합니다. Bayesian 모델 선택을 위한 증거 하한 증명(ELBo, Evidence Lower Bound) 기반의 기법을 활용하여 보다 효율적으로 하이퍼파라미터를 선택할 수 있습니다.

- **Technical Details**: 정규화 하이퍼파라미터를 조정하기 위해 제안된 접근 방식은 고차원 매개변수 θ(θ)와 사전 분포 η(η)를 모델링하고, 데이터 D(𝒟)의 증거(evidence) 최대화를 통해 η(η)를 직접 학습합니다. 이를 위해 변분 방법(variational methods)에서의 유용한 하한을 이용하여 복잡한 계산을 피하면서 하이퍼파라미터를 선택합니다.

- **Performance Highlights**: 여러 데이터셋에서 이미지 분류 작업을 통해 제안된 방법이 기존의 접근법들에 비해 비슷한 정확도를 유지하면서도 훈련 시간이 크게 줄어드는 효과를 보여줍니다. 16시간에서 150시간 이상 걸리던 훈련 시간이 3시간 미만으로 단축되었습니다.



### Spatial Shortcuts in Graph Neural Controlled Differential Equations (https://arxiv.org/abs/2410.19673)
Comments:
          Accepted as a workshop paper at the NeurIPS 2024 workshop on Data-driven and Differentiable Simulations, Surrogates, and Solvers (D3S3)

- **What's New**: 본 연구에서는 Neural Controlled Differential Equation (NCDE)에 사전 그래프 토폴로지 정보를 포함시켜 그래프에 정의된 동적 시스템의 미래 상태를 예측합니다. 이 접근 방식은 이전 기법들에 비해 적은 매개변수로 더 낮은 Mean Absolute Error (MAE)를 달성합니다.

- **Technical Details**: 본 연구는 그래프의 정점에서의 미래 동역학을 추론하기 위해 이전에 알려진 그래프 토폴로지 정보를 NCDE에 통합합니다. 모델 아키텍처 내의 여러 위치에서 그래프 정보를 NCDE에 전달하는 방식을 조사하며, 이론적 및 경험적으로 숨겨진 상태와 제어 사이의 외부 위치가 유리함을 확인합니다. 또한, 불규칙한 시계열 데이터를 처리할 수 있는 능력을 갖춘 NCDE의 장점을 설명합니다.

- **Performance Highlights**: 이 연구에서 제안한 NCDE는 사전 그래프 구조의 정보를 통해 적은 관측치로 예측할 수 있는 잠재력을 가지고 있으며, 이는 교통 예측, 강수위 예측, 기후 및 날씨 예측, 질병 전파 등의 분야에서 특히 유망합니다.



### MetaTrading: An Immersion-Aware Model Trading Framework for Vehicular Metaverse Services (https://arxiv.org/abs/2410.19665)
- **What's New**: 이 논문은 차량 메타버스에서 증강 현실(AR) 서비스의 데이터 품질과 사용자 개인정보 보호를 동시에 해결하는 혁신적인 몰입 인식 모델 거래 프레임워크를 제안합니다. 이 프레임워크는 메타버스 사용자(MUs)가 자율적으로 자신의 최신 로컬 데이터를 기반으로 학습 모델을 기여하도록 유도합니다.

- **Technical Details**: 논문에서는 메타버스 서비스 제공자(MSPs)와 MUs간의 거래 상호작용을 균형 문제(Equilibrium Problem with Equilibrium Constraints, EPEC)로 모델링하고, 멀티 에이전트 마코프 결정 프로세스(Multi-Agent Markov Decision Process)로 보상 결정을 형성합니다. 또한, 딥 강화 학습(Deep Reinforcement Learning) 기반의 MDDR 방식을 통해 분산형 동적 보상 방법을 제시합니다.

- **Performance Highlights**: 실험 결과, 제안된 프레임워크가 AR 관련 차량 데이터셋에서 객체 탐지 및 분류에 있어 benchmark schemes에 비해 더 높은 가치의 모델을 효과적으로 제공할 수 있음을 보여주었습니다.



### Conformal Prediction for Multimodal Regression (https://arxiv.org/abs/2410.19653)
Comments:
          20 pages, 34 figures

- **What's New**: 이 논문은 다중 모드(멀티모달) 데이터 컨포멀 회귀(conformal regression) 방법론을 새롭게 소개합니다. 전통적으로 숫자 입력 피처에 국한되어 있던 컨포멀 예측이 이제 이미징(image) 및 비구조적 텍스트(unstructured text)와 같은 복합 네트워크 아키텍처에서 추출한 정보를 활용하여 멀티모달 환경으로 확장되었습니다.

- **Technical Details**: 연구에서 제안하는 방법론은 신경망의 내부 피처를 활용하여 컨포멀 예측의 신뢰구간(prediction intervals, PIs)을 생성하는데, 이는 다양한 입력 데이터, 특히 이미지, 비구조적 텍스트와 같은 복합적인 데이터 입력을 처리하는 회귀 문제에서 유용합니다. 데이터 교환 가능성(exchangeability)에 기초한 컨포멀 예측을 사용하여, 통계적으로 엄정한 불확실성 구간을 생성할 수 있으며, 이는 특정 데이터 분포나 모델 가정에 의존하지 않습니다.

- **Performance Highlights**: 실험 결과, 본 연구에서 제안한 다중 모드 콘포멀 회귀 방법론은 RSRP(Reference Signal Received Power) 예측 및 멜버른 에어비앤비 오픈 데이터셋 가격 예측에서 효과적으로 작동하였으며, 예측 간격은 모델의 신뢰 수준에서 일관되게 true value를 포함함을 보여주었습니다. 또한, 다중 모드 입력을 포함한 복잡한 문제에서의 불확실성 정량화(quantification)에 강점을 보였습니다.
...
### Efficient Biological Data Acquisition through Inference Set Design (https://arxiv.org/abs/2410.19631)
- **What's New**: 이 논문은 약물 발견(Drug Discovery)에서 고속 스크리닝(high-throughput screening) 실험의 비용을 줄이기 위해, 어려운 예제를 선택적으로 레이블링하여 나머지 실험 결과를 예측하는 시퀀스 하위 집합 선택 문제를 모델링합니다.

- **Technical Details**: 제안된 접근법은 'inference set design'이라는 메커니즘을 사용합니다. 이 방법은 활성 학습(active learning)을 통해 어려운 예제의 레이블을 선택적으로 얻어, 상대적으로 쉬운 예제만 남아 전체 시스템 성능을 향상시키는 방식입니다. 알고리즘은 target accuracy에 도달했다고 판단되면 실험을 중단하는 명시적 중단 기준도 포함하고 있습니다.

- **Performance Highlights**: 이미지 및 분자 데이터셋과 실제 생물학적 평가 사례에 대한 실험 결과, 활성 학습을 활용한 inference set design이 실험 비용을 크게 줄이면서도 시스템 성능을 높이는 것을 보여주었습니다. 이 접근법은 일반적으로 활성 학습의 장점이 미미한 경우에도 효과적입니다.



### Analyzing Neural Network Robustness Using Graph Curvatur (https://arxiv.org/abs/2410.19607)
- **What's New**: 본 논문은 그래프 이론(Graph Theory) 및 그래프 곡률(Graph Curvature) 관점에서 신경망(Neural Network, NN) 강건성 문제를 재조명합니다. 특히, 신경 Ricci 곡률(Neural Ricci Curvature) 정의를 통해 NN의 병목 에지(Bottleneck Edges)를 식별하는 방법을 제안합니다.

- **Technical Details**: 신경 Ricci 곡률(NRC)은 NN 아키텍처의 형태로 구성된 신경 데이터 그래프(Neural Data Graph)에서 계산됩니다. 에지는 NN 가중치(Weights)와 특정 샘플이 입력될 때 해당 에지를 통과하는 데이터의 크기를 바탕으로 가중치화되며, 음의 NRC를 갖는 에지는 NN의 강건성 취약성(Vulnerabilities)과 관련이 있습니다.

- **Performance Highlights**: MNIST 데이터셋에서 평가한 결과, 강건한 예제에 해당하는 신경 데이터 그래프는 음의 NRC 에지가 적은 것으로 나타났으며, 이러한 결과는 다양한 아키텍처에서도 일관되고 있습니다. 이는 적은 수의 음의 NRC 에지를 최소화함으로써 강건 훈련(Robust Training)을 위한 새로운 방법의 기초가 될 것입니다.



### Neuromorphic IoT Architecture for Efficient Water Management: A Smart Village Case Study (https://arxiv.org/abs/2410.19562)
Comments:
          Submitted to Edge AI conference, Cagliari, Italy

- **What's New**: 본 연구에서는 IoT(Internet of Things) 네트워크의 성장에 따른 새로운 아키텍처를 제안합니다. 이는 생물학적 시스템에서 영감을 받아 높은 유연성과 학습 능력을 제공하면서도 낮은 에너지 소비와 통신 오버헤드를 유지합니다.

- **Technical Details**: 제안된 아키텍처는 생물학적 원칙을 통합한 신경형(neuromorphic) IoT 구조로, 에지 컴퓨팅(edge computing) 시나리오에 최적화되어 있습니다. 비동기적 처리(asynchronous processing)와 이벤트 기반 통신(event-driven communication)의 장점을 활용하여, 에너지 효율적이며 반응성이 뛰어난 IoT 프레임워크를 생성합니다.

- **Performance Highlights**: 사례 연구로 Carinthian의 Neuhaus 지역에서 물 관리에 대한 예측 및 이상 탐지 결과를 제시합니다. 초기 결과는 에너지 절약, 통신 오버헤드 감소, 시스템 반응성 향상 등 신경형 IoT 아키텍처의 이점을 강조합니다.



### FLiP: Privacy-Preserving Federated Learning based on the Principle of Least Privileg (https://arxiv.org/abs/2410.19548)
- **What's New**: FLiP는 Federated Learning (FL) 훈련에 Least Privilege 원칙(Principle of Least Privilege, PoLP)을 적용하여 훈련 세션 중에 공유된 지식을 최소화하는 새로운 접근 방식을 제안합니다. 고객은 FL 모델 정확성을 향상시키기 위해 훈련 과제와 관련된 정보만 공유함으로써 데이터 프라이버시를 보호할 수 있습니다.

- **Technical Details**: FLiP의 핵심 설계는 지역-전역 데이터셋 증류(local-global dataset distillation)입니다. 이 방법을 통해 각 클라이언트는 원시 데이터에서 훈련 과제와 가장 관련성이 높은 정보를 추출하고, 중앙 서버는 이 정보를 수집하여 클라이언트 간의 정보 추출을 지원합니다. FLiP는 모델 정확성과 데이터 프라이버시 보호 간의 균형을 유지하는 것을 목표로 합니다.

- **Performance Highlights**: 실험 결과, FLiP는 기존의 Vanilla FL과 유사한 정확성을 유지하면서도 훈련 과제와 관련 없는 정보를 풍부하게 보호할 수 있음을 보여주었습니다. FLiP는 모델 훈련과 데이터 프라이버시 간의 균형을 잘 유지하였고, 훈련 중 발생할 수 있는 여러 공격에 대해 강력한 방어 성능을 나타냈습니다.
...
### AgentForge: A Flexible Low-Code Platform for Reinforcement Learning Agent Design (https://arxiv.org/abs/2410.19528)
Comments:
...
- **What's New**: AgentForge는 강화 학습(RL) 시스템의 다양한 매개변수를 쉽게 최적화할 수 있는 유연한 저 코드(low-code) 플랫폼입니다. 이 플랫폼은 사용자들이 복잡한 매개변수 집합을 손쉽게 최적화할 수 있도록 돕고 있으며, 비전문가도 쉽게 접근할 수 있게 설계되었습니다.

- **Technical Details**: AgentForge는 사용자가 저 코드로 최적화 문제를 정의하고 여러 가지 최적화 기법(예: Random Search, Bayesian Optimization, Particle Swarm Optimization)을 사용할 수 있게 합니다. 사용자는 단순한 코드 몇 줄로 매개변수 집합의 개별 또는 공동 최적화를 수행할 수 있으며, 환경, 목표 함수 및 최적화할 매개변수를 정의하여 최적화 문제로 변환할 수 있습니다.

- **Performance Highlights**: AgentForge는 Gymnasium 라이브러리의 전통적인 Lunar Lander 에이전트를 기반으로 한 픽셀 기반 문제에서 그 성능을 평가하였으며, 다양한 매개변수 집합을 효과적으로 처리할 수 있는 능력을 보여주었습니다. 이 플랫폼을 사용하면 사용자가 강화 학습 에이전트를 개발할 때 복잡한 최적화 기술을 마스터하지 않고도 빠르게 프로토타입을 만들고 결과를 확인할 수 있습니다.



### Marked Temporal Bayesian Flow Point Processes (https://arxiv.org/abs/2410.19512)
- **What's New**: 본 논문에서는 새로운 생성적 마크된 시점 과정 모델인 BMTPP(Marked Temporal Bayesian Flow Point Process)를 제안합니다. BMTPP는 기존의 생성적 MTPP 모델과 달리, 이벤트의 타임스탬프와 타입 간의 상관관계를 효과적으로 모델링할 수 있도록 유연하게 설계되었습니다.

- **Technical Details**: BMTPP는 Bayesian Flow Network(BFN)를 기반으로 하여 연속 타임스탬프와 이산 이벤트 타입의 결합 분포를 직접 모델링합니다. 이는 마크된 시간 데이터 공간 내에서 결합 노이즈를 추가하여 타임스탬프와 이벤트 타입 간의 상호 작용을 명확히 드러냅니다. 이 접근법은 복잡한 상관관계를 포착하는 데 도움을 줍니다.

- **Performance Highlights**: BMTPP는 네 가지 실제 데이터셋에서 평가되었으며, 기존의 최첨단 모델들에 비해 월등한 성능을 보여주었습니다. 특히, 결합 노이즈 전략이 마크된 시간 의존성을 효과적으로 캡처하는데 기여하는 것을 확인했습니다.
...
### A neural network approach for solving the Monge-Amp\`ere equation with transport boundary condition (https://arxiv.org/abs/2410.19496)
- **What's New**: 이번 논문은 optical design(광학 설계) 분야에 적용 가능한 transport boundary condition(이동 경계 조건)을 가지고 Monge-Ampère 방정식을 해결하기 위한 새로운 신경망 기반 접근 방식을 제시합니다.

- **Technical Details**: 딥러닝 기반의 multilayer perceptron(다층 퍼셉트론)을 활용하여 방정식의 잔여(residual), 경계 조건(boundary conditions), 그리고 convexity constraints(볼록성 제약조건)을 포함한 손실 함수(loss function)를 최소화하여 근사 해를 학습하는 방법을 설명합니다. 이 방법은 L-BFGS를 통해 최적화되었습니다.

- **Performance Highlights**: 전통적인 least-squares finite-difference solver(최소제곱 유한차분 해법)와 비교하여, 본 신경망 접근법이 검토된 테스트 케이스에서 경쟁력 있고 종종 우수한 성능을 보여줌을 밝혔다. 또한, 샘플 밀도(sampling density), 네트워크 구조(network architecture), 최적화 알고리즘(optimization algorithm) 등의 다양한 하이퍼파라미터(hyperparameter)가 성능에 미치는 영향도 면밀하게 분석하였습니다.



### TRADE: Transfer of Distributions between External Conditions with Normalizing Flows (https://arxiv.org/abs/2410.19492)
Comments:
          Preprint, under review

- **What's New**: 본 논문에서는 TRADE라는 새로운 기법을 소개합니다. TRADE는 외부 조건에 대한 임의의 연속 의존성을 가지고 있는 생성 모델을 학습하는 방법으로, 경계값 문제(boundary value problem)로 학습 프로세스를 공식화하여 학습하는 방식입니다.

- **Technical Details**: TRADE는 함수의 비정규화 밀도(unnormalized density)의 형태를 알고 있는 경우에 조건부 확률 밀도(conditional probability density)를 학습하는 방법으로, 특정 조건에서 모델을 초기 훈련합니다. 이후 외부 매개변수에 대한 비정규화 밀도의 그래디언트를 통해 정보를 전파합니다. 이를 통해 제한 없는 구조(unconstrained architectures)로 복잡한 목표 분포(target distribution)에 적응할 수 있으며, 데이터가 불완전한 상황에서도 효과적으로 훈련할 수 있습니다.

- **Performance Highlights**: TRADE는 베이지안 추론(Bayesian inference), 분자 시뮬레이션(molecular simulations), 물리 격자 모델(physical lattice models) 등 다양한 응용 분야에서 뛰어난 성과를 보여줍니다.



### Measuring memorization through probabilistic discoverable extraction (https://arxiv.org/abs/2410.19482)
- **What's New**: 본 논문은 대형 언어 모델(Large Language Models, LLMs)의 메모리(memory) 문제를 다룬다. 특히, 모델이 훈련 데이터에서 민감한 정보를 얼마나 잘 기억하는지를 측정하는 새로운 확률적 기법인 (n,p)-discoverable extraction을 도입한다.

- **Technical Details**: 이 새로운 정의는 사용자가 선택한 샘플링 방식에 따라 특정 확률 p로 목표 시퀀스를 추출하는데 필요한 시도 횟수 n을 정량화한다. 연구진은 이 방법이 기존의 discoverable extraction 방식의 한계를 해결하며, 더 많은 메모리 비율을 발굴할 수 있음을 입증했다.

- **Performance Highlights**: 실험 결과, (n,p)-discoverable extraction이 훈련 데이터의 메모리 비율을 보다 잘 대변하며, 더 큰 모델이나 대상 데이터 반복 수가 클수록 메모리 비율의 차이가 증가함을 보여주었다. 이 기법은 다양한 샘플링 방식 아래에서 메모리 비율을 비교하는 데 유용함을 잘 입증했다.
...
### Computational Bottlenecks of Training Small-scale Large Language Models (https://arxiv.org/abs/2410.19456)
Comments:
          8 pages, 4 figures

- **What's New**: 이 연구는 Small-scale large Language Models (SLMs)의 훈련 동태성과 계산 요구 사항을 분석하며, 다양한 하이퍼파라미터와 설정이 SLM 훈련에 미치는 영향을 평가합니다. 특히, GPU 유형, 배치 크기, 모델 크기, 통신 프로토콜, 어텐션 유형 및 GPU 수에 대한 실험을 통해 비용 효율성을 강조합니다.

- **Technical Details**: SLMs(최대 2B 파라미터)의 훈련 시 여러 하이퍼파라미터가 메모리 및 성능에 미치는 영향을 연구합니다. Flash Attention(FA) 및 Fully Sharded Data Parallel(FSDP)와 같은 고급 설정이 더 큰 모델에서 필요하며, A100-40GB GPU 및 Distributed Data Parallel(DDP)이 더 작은 모델에 가장 적합하다는 것을 발견했습니다. 또한, 훈련 비용을 줄이기 위해 소프트웨어 및 하드웨어 구성의 최적화를 연구합니다.

- **Performance Highlights**: 연구 결과, 1k 토큰 비율에 대해 $1의 훈련 비용을 기준으로, SLMs는 LLMs보다 성능이 뛰어나고, DDP가 SLM 훈련에 가장 효과적인 분산 훈련 방식임을 나타냈습니다. SLM의 훈련 효율성을 최대화 하기 위해 여러 하드웨어 설정을 분석하였으며, FlashAttention이 SLM 훈련에서 더욱 중요한 역할을 한다는 점을 강조했습니다.
...
### Generative Diffusion Models for Sequential Recommendations (https://arxiv.org/abs/2410.19429)
Comments:
          ROEGEN@RecSys'24: The 1st Workshop on Risks, Opportunities, and Evaluation of Generative Models in Recommender Systems, Co-located with ACM RecSys in Bari, Italy, October 2024

- **What's New**: 이 연구에서는 Diffusion 모델을 활용하여 Sequential Recommendation (SR)에서 발생하는 여러 문제를 해결하는 새로운 접근법을 제시합니다.

- **Technical Details**: DiffuRecSys는 기존 Diffusion Recommender Model의 성능을 향상시키기 위해, Approximator에서 cross-attention 메커니즘을 도입하고, diffusion 과정에서 offset noise를 추가하였습니다. 이는 사용자 상호작용의 변동성을 효과적으로 처리하고, 사용자-아이템 상호작용을 더 잘 찍어낼 수 있도록 도와줍니다.

- **Performance Highlights**: DiffuRecSys는 세 개의 공개 벤치마크 데이터셋에서 실험을 수행하였으며, 기존 방법들과 비교해 아이템 표현을 개선하고 다양한 사용자 선호를 효과적으로 포착하여 성능이 향상된 것을 확인했습니다.



### Analyzing Generative Models by Manifold Entropic Metrics (https://arxiv.org/abs/2410.19426)
- **What's New**: 이번 연구에서는 고급 생성 모델의 성능 향상을 위해 해석 가능한 표현을 활용하고, 분리된 표현(disentangled representations)의 속성을 객관적으로 측정할 수 있는 정보 이론 기반의 평가 지표를 제안합니다.

- **Technical Details**: 연구에서 제안된 방법은 생성 모델의 디코더(decoder)를 기반으로 하여 분리(disentanglement)와 정렬(alignment)이라는 두 가지 중요한 조건을 고려합니다. 이는 기존 연구들이 주로 인코더(encoder)에 초점을 맞춘 것과는 다르게, 데이터 생성 과정의 영향을 더 체계적으로 평가할 수 있게 합니다. 구체적으로, 제안된 정보 이론적 지표는 생성 데이터의 특정 특성이 모델의 숨겨진(latent) 변수에 의해 어떻게 영향을 받는지를 정량적으로 측정합니다.

- **Performance Highlights**: EMNIST 데이터셋에서 다양한 normalizing flow 아키텍처 및 β-VAEs를 비교한 결과, 제안된 정보 이론적 메트릭을 통해 각 모델의 학습 과정에서 얻은 표현의 품질과 중요성을 효율적으로 평가할 수 있음을 보여주었습니다. 실험 결과, 다양한 모델 아키텍처가 정렬되고 분리된 표현에 수렴하는 경향성과 그에 대한 응답성을 평가할 수 있는 유용한 방법을 제공합니다.
...
- **What's New**: 이 논문은 언어 모델( LM)에서의 행동 변화를 감지하는 지속적인 행동 변화 감사(BSA) 방법을 제시합니다. 기존의 가설 검정을 기반으로 한 이 감사 테스트는 모델 생성만으로 행동 변화를 탐지할 수 있도록 설계되었습니다.

- **Technical Details**: 이 방법은 모델 생성을 baseline 모델과 비교하며, 변화 감지에 대한 이론적 보장을 제공하면서 거짓 양성(false positive)을 제어합니다. 감사 테스트는 다양한 사용 사례에 맞추어 행동 변화에 대한 민감도를 조정할 수 있는 구성 가능한 허용 오차(tolerance parameter)를 특징으로 합니다. 실험적으로 우리는 두 가지 사례 연구(독성 및 번역 성능 변동 모니터링)를 통해 접근 방식을 평가했습니다.

- **Performance Highlights**: 우리의 테스트는 수백 개의 예제만으로도 행동 분포의 의미 있는 변화를 감지할 수 있음을 발견했습니다. 이 연구는 LM의 행동이 사회적 가치와 일치하도록 보장하는 데 중요한 기여를 할 것으로 기대됩니다.



### Offline Reinforcement Learning with OOD State Correction and OOD Action Suppression (https://arxiv.org/abs/2410.19400)
Comments:
          Accepted to NeurIPS 2024

- **What's New**: 이 논문은 오프라인 강화 학습(offline reinforcement learning)에서 간과되어온 OOD(출처가 불명확한, out-of-distribution) 상태 문제를 다루며, OOD 동작(issue) 문제뿐만 아니라 성능 저하를 초래하는 OOD 상태 문제가 있음을 강조한다. 이에 대한 해결책으로 SCAS(Simultaneous Correction and Action Suppression)라는 간단하면서도 효과적인 접근법을 제안한다.

- **Technical Details**: SCAS는 OOD 상태 교정과 OOD 동작 억제를 통합한 기법이다. 이 방법은 값에 민감한 OOD 상태 교정을 통해 에이전트를 OOD 상태에서 고가치 ID(in-distribution) 상태로 교정하는 능력을 가지며, KL 발산(KL divergence)을 사용하여 훈련된 정책에 의해 유도된 동역학과 정렬시킨다. 이론적 및 경험적 결과에 따르면 SCAS는 OOD 동작 억제 효과도 동시에 나타낸다.

- **Performance Highlights**: SCAS는 D4RL 및 NeoRL과 같은 표준 오프라인 RL 벤치마크에서 뛰어난 성능을 보여준다. 또한, 추가적인 하이퍼파라미터 조정 없이도 일관된 하이퍼파라미터로 높은 성능을 달성하며, 환경의 변동에 대한 내구성이 향상됨을 보여준다.
...
### Toward Finding Strong Pareto Optimal Policies in Multi-Agent Reinforcement Learning (https://arxiv.org/abs/2410.19372)
Comments:
          Submitted to ACML 2024 Special Issue Journal track

- **What's New**: 이 연구에서는 협력 보상 구조를 가진 다중 요인 강화 학습(MARL) 문제에서 Pareto 최적 정책을 찾는 문제를 다룹니다. 각 에이전트가 자신의 보상만 최적화하는 알고리즘은 최적이 아닌 수렴(convergence)을 초래할 수 있음을 보여줍니다. 따라서 Pareto 최적성을 달성하려면 에이전트들이 타인의 보상을 고려해야 합니다.

- **Technical Details**: 이 논문에서 제안된 Multiple Gradient Descent Algorithm(MGDA)을 다중 에이전트 환경에 적용하는 프레임워크를 제시합니다. MGDA는 약한 Pareto 수렴에 문제를 겪고 있는데, 이는 MARL에서 흔히 발생하는 문제입니다. 이를 해결하기 위해 MGDA++를 제안하며, 이 알고리즘은 강한 Pareto 최적 해(solution)에 수렴함을 이론적으로 증명합니다. 또한, Cooperative 환경에서 Gridworld 벤치마크를 통해 성능을 입증합니다.

- **Performance Highlights**: MGDA++는 Gridworld 벤치마크에서 여러 협력적 시나리오를 통해 다른 방법들과 비교하여 더 나은 수렴 솔루션을 달성할 수 있음을 보여주며, 최적 정책의 우수성을 강조합니다.



### Notes on the Mathematical Structure of GPT LLM Architectures (https://arxiv.org/abs/2410.19370)
Comments:
          10 pages

- **What's New**: 이 논문은 GPT-3 스타일의 대형 언어 모델(LLM)의 수학적 구조를 설명합니다. 이러한 모델의 건설과 훈련 과정을 순수한 수학적 관점에서 접근합니다.

- **Technical Details**: 이 모델은 특정 맵을 생성하는 함수의 집합으로 설명되며, 매개변수 공간(Θ=𝐑^N)에 의해 인덱싱됩니다. 논문은 변환기(transformer) 언어 모델이 실제 텍스트를 처리하는 메커니즘의 구성 요소인 토큰화(tokenization), 인코딩(encodings), 및 임베딩(embeddings)에 대해 언급합니다.

- **Performance Highlights**: 이러한 수학적 접근 방식은 LLM 아키텍처에 대한 이해를 높이고, 이를 더욱 발전시키는 다양한 훈련 알고리즘을 개발하는 데 기여할 수 있을 것으로 기대됩니다.
...
### FeBiM: Efficient and Compact Bayesian Inference Engine Empowered with Ferroelectric In-Memory Computing (https://arxiv.org/abs/2410.19356)
Comments:
          6 pages, 8 figures, to be published in the 61st DAC (Design Automation Conference) proceedings

- **What's New**: 본 논문에서는 다중 비트 강유전체 필드 효과 트랜지스터(FeFET)에 기반한 효율적이고 컴팩트한 베이지안 추론 엔진인 FeBiM을 제안합니다. FeBiM은 훈련된 확률을 컴팩트한 FeFET 크로스바에 인코딩하며, 추론에 필요한 추가 회로 없이 메모리 내에서 효율적인 베이지안 추론을 가능하게 합니다.

- **Technical Details**: FeBiM은 베이지안 모델의 훈련된 로그 확률을 양자화하여 이산 FeFET 상태에 매핑합니다. 이 접근 방식은 크로스바의 누적 출력을 자연스럽게 posterior probabilities로 나타내며, Bayesian inference를 위한 주요 연산이 다중 곱셈-누적(MAC) 작업과 다르다는 점에서 차별화됩니다. FeBiM은 26.32 Mb/mm²의 높은 저장 밀도와 581.40 TOPS/W의 컴퓨팅 효율성을 달성했습니다.

- **Performance Highlights**: 제안된 FeBiM은 대표적인 베이지안 분류 작업에서 기존의 베이지안 하드웨어 구현에 비해 10.7배의 콤팩트함과 43.4배의 효율성 향상을 보여주었습니다.
...
### Golden Ratio-Based Sufficient Dimension Reduction (https://arxiv.org/abs/2410.19300)
- **What's New**: 이 논문에서는 고차원 데이터에 대한 충분한 차원 축소(Sufficient Dimension Reduction, SDR) 기술로, 신경망 기반 방법인 Golden Ratio-based Neural Network for SDR (GRNN-SDR)를 제안합니다. 이 방법은 구조적 차원을 효과적으로 찾고, 중심 공간을 잘 추정합니다. 신경망의 함수 근사 능력을 활용하여 계산 비용을 줄이고, 실용적 차원 축소를 위한 확장이 가능하다는 장점이 있습니다.

- **Technical Details**: GRNN-SDR은 전통적인 비모수 회귀 도구로는 접근할 수 없었던 복잡한 함수 형태를 캡처하기 위해 신경망을 활용합니다. 이 알고리즘은 Golden Ratio를 이용하여 동적으로 구조적 차원을 탐색하며, 이로 인해 계산 시간과 복잡성을 현저히 줄이는 결과를 얻습니다. 안정성 문제와 관련하여, O(N) 복잡성을 가지며 여기서 N은 샘플 크기입니다.

- **Performance Highlights**: 수치 결과에 따르면 GRNN-SDR은 사전 지식 없이도 실제 구조적 차원을 효과적으로 추정할 수 있으며, 많은 경우에서 중심 공간의 추정 정확도가 높고, 진정한 차원이 작지 않을 때 비교적 높은 안정성을 발휘합니다.
...
### Coordinated Reply Attacks in Influence Operations: Characterization and Detection (https://arxiv.org/abs/2410.19272)
- **What's New**: 이번 연구에서는 트위터에서의 영향력 작전(context of influence operations)과 관련하여 협조적(reply) 공격(coordinated reply attacks)을 분석하고 이를 탐지하기 위한 방법론을 제시합니다. 기존 연구에서는 이 전술에 대한 분석이 부족했던 반면, 본 연구는 이러한 공격의 주요 타겟을 저널리스트(journalists), 뉴스 매체(news media), 정부 관계자(state officials), 정치인(politians) 등으로 확인하였습니다.

- **Technical Details**: 우리는 두 가지 감독 머신러닝(supervised machine-learning) 모델을 제안합니다. 첫 번째 모델은 트윗이 공격(targeted) 받았는지를 분류하고, 두 번째 모델은 해당 트윗에 응답하는 계정이 협조적으로 참여(coordinated) 했는지를 분류합니다.

- **Performance Highlights**: 분류기의 AUC 점수는 각각 0.88과 0.97로, 이는 응답 공격에 참여한 계정을 효과적으로 탐지할 수 있음을 나타냅니다. 타겟 계정(targeted accounts) 자체가 영향력 작전 탐지를 위한 센서(sensors) 역할을 할 수 있다는 중요한 발견을 하였습니다.



### A Survey of Deep Graph Learning under Distribution Shifts: from Graph Out-of-Distribution Generalization to Adaptation (https://arxiv.org/abs/2410.19265)
Comments:
          18 pages, 2 figures. arXiv admin note: text overlap with arXiv:2402.11153

- **What's New**: 이번 논문은 그래프 머신 러닝에서 발생하는 데이터 분포의 변화(Distribution Shift)에 대해 체계적인 리뷰를 제공하고 있습니다. 특히, 그래프 OOD(Out-Of-Distribution) 일반화, 학습 시간 그래프 OOD 적응, 테스트 시간 그래프 OOD 적응의 세 가지 주요 시나리오를 다룹니다.

- **Technical Details**: 그래프 머신 러닝의 다양한 분포 변화 유형, 예를 들어 공동 변동(Covariate Shift) 및 개념 변동(Concept Shift) 등을 공식적으로 정의하고, 기존 모델을 새로운 분류 체계로 분류합니다. 또한, 모델 중심(Model-Centric)과 데이터 중심(Data-Centric) 접근법으로 나누어 기술을 구체적으로 설명합니다.

- **Performance Highlights**: 이 논문은 그래프 머신 러닝의 신뢰성을 높이기 위해 강화된 모델 일반화 및 적응성을 통해 향상된 성과를 제시하고 있습니다. 다양한 연구 방향과 이에 따른 도전 과제를 강조하여 향후 연구의 가능성을 넓히고자 합니다.



### Spatioformer: A Geo-encoded Transformer for Large-Scale Plant Species Richness Prediction (https://arxiv.org/abs/2410.19256)
Comments:
          Submitted to IEEE Transactions on Geoscience and Remote Sensing

- **What's New**: 이 연구에서는 지리적 문맥(geolocation context)을 반영하여 식물 종의 풍부성(richness) 예측에 있어 Transformer 모델을 발전시킨 Spatioformer 모델을 제안합니다. 이는 대규모 공간에서 식물 종의 다양성을 예측하는 데 있어 중요한 접근방식을 제공합니다.

- **Technical Details**: Spatioformer는 혁신적인 지리적 인코더(geolocation encoder)를 사용하여 위성 이미지를 인코딩하며, 원거리 지역의 식물 종 조성을 반영한 예측 모델입니다. 이 연구에서는 HAVPlot이라는 68,170개의 현장 샘플을 포함한 대규모 데이터셋을 활용하여 Spatioformer의 성능을 기존 모델들과 비교했습니다.

- **Performance Highlights**: Spatioformer는 호주의 식물 종 풍부성 예측에서 최신 모델보다 월등한 성과를 보였으며, 예측 불확실성이 큰 지역을 추가적으로 식별하여 향후 현장 조사 필요성을 강조했습니다. 이 연구는 2015년부터 2023년까지 Landsat 아카이브로부터 제작된 식물 종 풍부성 지도를 통해 결과를 시각화했습니다.



### CHESTNUT: A QoS Dataset for Mobile Edge Environments (https://arxiv.org/abs/2410.19248)
- **What's New**: 이 논문에서는 모바일 엣지 환경에서 Quality of Service (QoS) 예측을 위한 새로운 데이터셋을 제안합니다. 기존의 정적 QoS 메트릭스에 집중하는 데이터셋과는 달리, 본 연구는 시간과 지리적 위치를 포함한 동적 속성을 기록하여 QoS를 더 정확하게 평가합니다.

- **Technical Details**: 제안된 데이터셋은 상하이에 있는 두 개의 실세계 데이터셋을 활용하여 만들어졌습니다. 첫 번째 데이터셋은 상하이 존슨 택시에서 수집된 정보로, 위도, 경도, 이동 방향 및 속도 관련 데이터를 포함합니다. 두 번째 데이터셋은 상하이 텔레콤으로부터 제공된 9,481대 모바일폰이 3,233개 기지국을 통해 인터넷에 접근한 720만 개 이상의 기록을 포함합니다. 이 데이터는 엣지 서버 배치, 서비스 이전 및 서비스 추천과 같은 모바일 엣지 컴퓨팅 문제를 평가하는 데 사용됩니다.

- **Performance Highlights**: 제안된 데이터셋은 QoS 성능이 시간과 지리적 위치에 따라 변동한다는 점을 고려하여 더 정확하고 신뢰할 수 있는 데이터 제공을 목표로 합니다. 이를 통해 모바일 엣지 환경에서의 QoS 예측 개선을 기대할 수 있습니다.



### Enhancing Exchange Rate Forecasting with Explainable Deep Learning Models (https://arxiv.org/abs/2410.19241)
Comments:
          Accepted by 2024 5th International Conference on Machine Learning and Computer Application

- **What's New**: 본 연구는 LSTM, CNN, Transformer 등의 고급 딥러닝 모델을 활용하여 RMB/USD 환율 예측의 정확성을 향상시키는 방법을 탐구하였습니다. 특히 TSMixer 모델이 가장 효과적인 것으로 나타났습니다.

- **Technical Details**: 연구는 40개의 주요 특성을 6개의 범주로 나누어 분석했으며, 정책 변화와 글로벌 경제의 실시간 영향을 분석하는 데 집중하고 있습니다. 또한 grad-CAM 시각화 기법을 통해 모델의 해석 가능성을 높였습니다.

- **Performance Highlights**: 연구 결과, 머신러닝 모델이 기존의 전통적 환율 예측 모델보다 더 정확하고 신뢰할 수 있는 예측을 가능하게 하며, 금융 분석과 의사 결정을 위한 유용한 도구로 작용할 수 있음을 입증했습니다.



### SHAP zero Explains All-order Feature Interactions in Black-box Genomic Models with Near-zero Query Cos (https://arxiv.org/abs/2410.19236)
- **What's New**: SHAP zero 알고리즘 개발: 모델 스케치 비용을 한 번 지불한 후, 요청된 각 시퀀스에 대해 거의 제로에 가까운 비용으로 모든 차수의 Shapley feature interactions 추정 가능.

- **Technical Details**: SHAP zero는 Shapley interactions와 모델의 Fourier transform 간의 연결 고리를 통해 기능합니다. 이 알고리즘은 데이터 차원이 증가하는 경우에도 비선형 feature interactions를 효율적으로 설명할 수 있도록 설계되었습니다.

- **Performance Highlights**: SHAP zero는 두 개의 유전체 모델을 설명하면서 기존의 최첨단 알고리즘에 비해 계산 비용을 수십 배 줄이는 것을 보여주었습니다. 또한 DNA 수선 결과를 예측할 수 있는 all microhomologous motifs를 발견하였습니다.
...
### Peptide-GPT: Generative Design of Peptides using Generative Pre-trained Transformers and Bio-informatic Supervision (https://arxiv.org/abs/2410.19222)
- **What's New**: 최근 자연어 처리(NLP) 모델이 전통적인 텍스트 생성 분야를 넘어서 다양한 분야에서 놀라운 능력을 보여주고 있습니다. 이번 연구에서는 PeptideGPT라는 단백질 언어 모델을 소개하며, 이는 특정한 특성(hemolytic activity, solubility, non-fouling characteristics)을 갖는 단백질 서열을 생성하도록 설계되었습니다.

- **Technical Details**: PeptideGPT는 hemolytic, non-hemolytic, non-fouling 및 soluble 단백질의 데이터셋을 기반으로 ProtGPT2를 미세 조정하여 개발되었습니다. 생성된 서열은 생물 정보학 원칙에 따라 단백질 유효성 검사 및 구조 평가를 포함한 여러 필터링 단계를 거칩니다. ESMFold를 사용하여 단백질 구조를 예측하고 pLDDT 값이 70 이상인 단백질을 선택하여 주문형 구조를 보장합니다.

- **Performance Highlights**: PeptideGPT는 hemolytic 단백질 생성에서 76.26%, non-hemolytic에서 72.46%, non-fouling에서 78.84%, solubility에서 68.06%의 정확도를 기록했습니다. 이는 PeptideGPT가 단백질 서열과 그 특성 간의 관계를 잘 포착하여 이를 기반으로 적절한 서열을 생성할 수 있음을 보여줍니다.
...
### Predicting Liquidity Coverage Ratio with Gated Recurrent Units: A Deep Learning Model for Risk Managemen (https://arxiv.org/abs/2410.19211)
- **What's New**: 본 논문은 글로벌 경제 통합과 금융 시장 간의 높은 상호 연계성 속에서 금융 기관들이 직면한 유동성 리스크를 효과적으로 관리할 수 있도록 돕기 위한 유동성 커버리지 비율(LCR) 예측 모델을 제안합니다.

- **Technical Details**: 이 연구에서는 gated recurrent unit (GRU) 네트워크를 기반으로 한 심층 학습 기술을 활용하여 역사적 데이터에서 복잡한 패턴을 자동으로 학습하고 이후 특정 시간 기간에 대한 LCR을 정확하게 예측하는 모델을 개발하였습니다.

- **Performance Highlights**: 실험 결과, 제안된 GRU 모델은 전통적인 방법들에 비해 평균 절대 오차(mean absolute error, MAE)에서 현저히 우수하여 높은 정확성과 강건성을 입증했습니다. 이는 금융 기관에 신뢰할 수 있는 유동성 리스크 관리 도구를 제공할 뿐 아니라 규제 기관이 보다 과학적이고 합리적인 정책을 수립하는 데 기여하여 전체 금융 시스템의 안정성을 향상시킵니다.
...
### Binary Classification: Is Boosting stronger than Bagging? (https://arxiv.org/abs/2410.19200)
- **What's New**: 본 논문에서는 Enhanced Random Forests라는 새로운 모델을 소개하며, 이는 전통적인 Random Forests의 성능을 개선하고 해석 가능성을 높이기 위한 추가 기능을 통합하고 있습니다. 이 모델은 샘플 및 모델 가중치를 적응식으로 배정하여, 훈련 샘플의 중요성을 강조합니다.

- **Technical Details**: Enhanced Random Forests는 반복 알고리즘을 통해 훈련 샘플 가중치를 조정하고, 새로운 샘플에 대한 개인화된 나무(weighted tree) 가중치 체계를 찾기 위한 접근 방식을 개발하였습니다. 이 방법은 15개의 이진 분류 데이터셋에서 일반 Random Forests에 비해 성능을 크게 향상시키고, 기본 하이퍼파라미터로 실행될 때 XGBoost를 포함한 다른 나무 방법들보다 유의미하게 뛰어난 성능을 보여줍니다.

- **Performance Highlights**: Enhanced Random Forests는 15개의 데이터셋에서 13개에서 AUC 성능이 평균 2% 향상되었으며(XGBoost와 유사한 성능), 해석 가능성 측면에서도 중요한 발전을 이뤘습니다. 이는 각 샘플에 대해 분류에 기여하는 나무의 중요 점수를 정의할 수 있게 하여, 사용자가 새로운 샘플을 분류하는 데 영향을 미치는 몇 가지 나무에만 집중할 수 있도록 하여 해석 가능성을 부분적으로 회복합니다.



### TEAM: Topological Evolution-aware Framework for Traffic Forecasting--Extended Version (https://arxiv.org/abs/2410.19192)
Comments:
...
- **What's New**: 이 논문에서는 도시화(global trend towards urbanization)로 인한 교통 예측의 중요성을 다루고 있으며, 기존의 고정적(topology) 도로망과 정적(static) 교통 시계열(time series) 가정이 도시의 변화와 맞지 않음을 지적합니다. 이 문제를 해결하기 위해 제안된 Topological Evolution-aware Framework (TEAM)은 교통 예측을 위한 새로운 접근법입니다.

- **Technical Details**: TEAM은 convolution과 attention 기법을 결합하여 새로운 시계열 데이터에 적응하면서도 기존의 지식을 유지할 수 있도록 설계되었습니다. 이 프레임워크는 Wasserstein metric을 이용한 지속적인 학습 모듈을 특징으로 하여, 안정적인(stable) 노드와 변화하는(changing) 노드를 구분합니다. 재훈련 시에는 안정적인 노드와 관련된 데이터만 사용되고, 새 노드와 인접 노드, 변화하는 노드의 데이터만으로 모델이 재훈련됩니다.

- **Performance Highlights**: 실제 교통 데이터셋을 활용한 경험적 연구에 따르면, TEAM은 기존 방법들보다 훨씬 낮은 재훈련 비용(re-training costs)으로도 예측 정확도(forecasting accuracy)를 유지할 수 있는 능력을 보여줍니다.



### Perturbation-based Graph Active Learning for Weakly-Supervised Belief Representation Learning (https://arxiv.org/abs/2410.19176)
- **What's New**: 이 논문은 소셜 네트워크에서 반지도 학습을 위한 라벨링 자원 할당 최적화 문제를 다룹니다. 제한된 예산 내에서 라벨링할 가치가 있는 메시지를 전략적으로 식별하는 방법을 제안합니다.

- **Technical Details**: 제안하는 방법은 그래프 데이터 증강에 기초한 perturbation-based active learning 전략인 PerbALGraph입니다. 이 접근법은 관찰 데이터의 구조적 특징에 민감한 메시지를 자동 추정기를 통해 선택하며, 이 추정기는 모델에 구애받지 않고 적용할 수 있습니다.

- **Performance Highlights**: 실험 결과, PerbALGraph 전략이 기존 방법들보다 분류 정확도와 라벨링 효율성 측면에서 우수한 성능을 보이며, 반지도 그래프 학습에서 라벨링 예산 할당 문제를 최적화하는 데 유망한 방향임을 보여줍니다.
...
### Learning Coupled Subspaces for Multi-Condition Spike Data (https://arxiv.org/abs/2410.19153)
Comments:
          27 pages, 3 figures

- **What's New**: 이 연구에서는 여러 실험 조건에서 신경 데이터 세트를 공동 분석할 수 있는 새로운 GPFA 모델인 공동 서브스페이스 GPFA (CS-GPFA)를 제안합니다.

- **Technical Details**: CS-GPFA는 조건의 매개변수적 특성을 활용하여 데이터를 분석하며, 데이터 증강을 통한 효율적인 Bayesian 추론 방법을 개발하여 모델 후행 확률에 대한 간단한 폐쇄형 업데이트를 제공합니다. 이 모델은 적은 수의 반복 실험 및 적은 수의 실험 조건 하에서도 유용성을 보여줍니다.

- **Performance Highlights**: CS-GPFA는 이전에 보지 못한 실험 조건에서 신경 활동 예측을 제공할 수 있으며, 신뢰할 수 있는 불확실성 추정치를 제공합니다. 이는 적응형 신경 과학 실험의 새로운 가능성을 열어줍니다.



### Structured Diffusion Models with Mixture of Gaussians as Prior Distribution (https://arxiv.org/abs/2410.19149)
- **What's New**: 본 논문에서는 Gaussian 혼합 모델(Gaussian Mixture Model, GMM)을 사전 분포(prior distribution)로 활용한 구조화된 확산 모델(structured diffusion models) 클래스를 제안합니다. 이는 기존의 Gaussian 분포를 대체합니다.

- **Technical Details**: 기존의 확산 모델은 데이터 구조에 따라 적절한 정보를 통합하지 못하는 경향이 있습니다. 본 연구에서는 구조적 정보의 통합을 위해 혼합 Gaussian을 사전 분포로 사용하며, 이와 관련된 간단한 학습 과정을 개발했습니다. 또한, 'Reverse Effort'라는 새로운 정량적 지표를 도입하여 혼합 확산 모델의 효율성을 정량적으로 설명합니다.

- **Performance Highlights**: 수치 실험을 통해 작은 데이터 세트, 운영 데이터 및 이미지 데이터에서 혼합 확산 모델이 기존 확산 모델에 비해 뛰어난 성능을 보임을 입증했습니다. 특히, 자원이 제한된 상황에서도 우수한 성능을 발휘할 수 있는 기반을 마련했습니다.



### Initialization Matters: On the Benign Overfitting of Two-Layer ReLU CNN with Fully Trainable Layers (https://arxiv.org/abs/2410.19139)
Comments:
          80 pages, 3 figures, 1 table

- **What's New**: 이번 연구에서는 benign overfitting(유익한 과적합)의 개념을 두 층 ReLU CNN(합성곱 신경망)으로 확장하여, 전면적으로 훈련 가능한 층을 갖는 신경망의 훈련 동역학을 분석하였습니다.

- **Technical Details**: 연구에서는 출력 층의 초기화 스케일링(initialization scaling)이 훈련 동역학에 미치는 영향을 다루었습니다. 큰 스케일은 고정된 출력을 가진 경우와 유사한 훈련 행동을 유도하며, 숨겨진 층(hidden layer)은 빠르게 성장하는 반면 출력 층은 거의 변하지 않았습니다. 반면에 작은 스케일은 복잡한 층 상호작용을 초래하며, 숨겨진 층은 초기에는 출력 층에 비해 특정 비율로 성장하다가 훈련 전반에 걸쳐 두 층이 함께 성장하여 그 비율을 유지합니다.

- **Performance Highlights**: 이론적 결과를 바탕으로 우리는 테스트 오류(test errors)에 대한 상한과 하한을 제시하였으며, benign overfitting이 달성될 수 있는 초기화 스케일링과 신호 대 잡음 비율(signal-to-noise ratio, SNR)에 대한 명확한 조건을 규명하였습니다. 또한, 수치 실험(numerical experiments)을 통해 이론적 결과를 뒷받침하였습니다.



### Context-Aware Trajectory Anomaly Detection (https://arxiv.org/abs/2410.19136)
- **What's New**: 이 논문은 도시 및 인간 이동 관리에서 효과적인 의사 결정을 위해 중요한 궤적(trajectory) 이상 탐지의 새로운 접근 방식을 제안합니다. 기존의 궤적 이상 탐지 방법이 다루지 않았던 문맥(contextual) 정보를 포함하여 이상 행동을 정확히 포착하는 데 도움을 주기 위해, 궤적 재구성 모델과 문맥 정보를 결합한 방법을 소개합니다.

- **Technical Details**: 제안된 방법은 궤적 재구성 프레임워크(various trajectory reconstruction framework)에 기반하며, 각 궤적의 문맥적 요소인 에이전트 ID와 장소(Contextual POI)를 포함하여 이상 탐지의 성능을 향상시키고자 합니다. 실험은 두 도시에서 수행되었으며, 문맥 정보를 효과적으로 모델링하는 제안된 접근 방식이 기존 방법보다 월등한 성능을 보여주었습니다.

- **Performance Highlights**: 제안된 방법은 두 개의 대도시에서 수행된 실험을 통해 기존의 문맥 정보를 포함하지 않은 궤적 이상 탐지 방법에 비해 유의미한 개선을 나타냈습니다. 이는 궤적 데이터의 문맥 정보를 효과적으로 활용하여 정확한 이상 행위를 탐지하는 데 기여합니다.
...
### LanFL: Differentially Private Federated Learning with Large Language Models using Synthetic Samples (https://arxiv.org/abs/2410.19114)
- **What's New**: 본 논문에서는 여러 참가자가 단일 글로벌 모델을 훈련할 수 있는 연합 학습 (Federated Learning, FL) 프레임워크에서 대형 언어 모델 (Large Language Models, LLM) 적용의 비효율성을 극복하기 위해 새로운 모델인 LanFL을 제안합니다. 기존 방식이 아닌, 프롬프트 기반의 접근 방식을 채택하여 LLM을 블랙 박스로 취급하고, 모델의 구조나 가중치에 접근하지 않고도 학습이 가능하도록 설계되었습니다.

- **Technical Details**: LanFL은 참가자들이 자신의 로컬 LLM을 사용하여 합성 샘플을 생성하고, 이 샘플을 기반으로 프롬프트를 생성 후 이를 공유하는 과정을 포함합니다. 이 과정에서 합성 샘플을 생성할 때 차등 프라이버시 (differential privacy)를 보장하기 위해 특별한 메커니즘을 도입하여 참가자 간의 지식 공유를 촉진합니다. 또한, LanFL은 모델의 성능을 향상시키는 프롬프트 최적화 기법을 활용합니다.

- **Performance Highlights**: 실험 결과, LanFL은 다양한 데이터셋에서 뛰어난 성과를 보여 주었으며, 데이터 이질성 (heterogeneity)이 존재하는 환경에서도 robust한 학습을 지원합니다. 특히, 참여자들은 각자의 데이터에 접근하지 않고도 모델 학습을 이어갈 수 있어, 프라이버시 보호와 협업 학습을 동시에 달성할 수 있음을 입증했습니다.
...
- **What's New**: 이 논문에서는 대형 언어 모델(Large Language Models, LLMs)의 포스트 훈련 양자화(Post-Training Quantization, PTQ) 성능을 최적화하기 위해 'TesseraQ'라는 새로운 상태-of-the-art 기법을 제안합니다. TesseraQ는 블록 재구성(Block Reconstruction) 기술을 사용하여 LLM의 가중치를 초저비트(ultra-low bits)로 양자화합니다.

- **Technical Details**: TesseraQ는 Progressive Adaptive Rounding(점진적 적응형 양자화) 방법을 도입하여 LLM의 양자화 과정 중 소프트 변수를 하드 변수로 점진적으로 변환하는 과정으로 가중치 재구성을 안정화합니다. 이 기술은 기존의 AWQ와 OmniQuant 같은 PTQ 알고리즘과 통합이 가능하여 성능을 크게 향상시킬 수 있습니다.

- **Performance Highlights**: TesseraQ는 LLaMA-2-7B 모델에서 2비트 가중치 양자화를 사용했을 때 wikitext2의 perplexity를 14.65에서 6.82로 개선하였고, 평균 다운스트림 정확도는 50.52에서 59.27로 증가시켰습니다. 다양한 양자화 스키마에서 TesseraQ는 지속적으로 우수한 성능을 보여주었습니다.



### Provable Tempered Overfitting of Minimal Nets and Typical Nets (https://arxiv.org/abs/2410.19092)
Comments:
          60 pages, 4 figures

- **What's New**: 본 연구는 노이즈가 있는 학습 세트에 대해 완벽하게 분류할 수 있는 이진 가중치를 가진 완전 연결 심층 신경망(Deep Neural Networks, NNs)의 오버피팅(overfitting) 행동을 분석합니다. 특히 본 연구에서는 최소 가중치 수를 갖는 '최소 크기(min-size)' 신경망 보간기(interpolator)를 사용하여 오버피팅을 조절하는 방법을 제시합니다.

- **Technical Details**: 우리는 두 가지 학습 규칙, 즉 최소 크기 신경망 보간기(min-size NN interpolator)와 임의의 보간기(random interpolating NN)를 통해 오버피팅 현상을 분석합니다. 이 과정에서, 특정 조건 하에 일정 크기의 교사 모델에 의해 생성된 데이터셋을 적절하게 보간할 수 있음을 증명합니다. 사용된 모델은 이진 가중치(binary weights)와 활성화(activations)를 갖는 신경망으로, 이는 자원 제약이 있는 환경에서 유용합니다.

- **Performance Highlights**: 연구 결과, 최적(min-size) 신경망 보간기가 따르는 학습 규칙은 오버피팅을 일정 부분 억제한다는 것을 보여줍니다. 우리는 또한 무작위 보간기가 지나치게 많은 매개변수를 가지더라도 일반적으로 조절된 오버피팅을 보이며, 이는 최소 크기 신경망 보간기와 유사한 일반화 성능(generalization performance)을 나타냄을 확인했습니다.



### FastSurvival: Hidden Computational Blessings in Training Cox Proportional Hazards Models (https://arxiv.org/abs/2410.19081)
Comments:
          Accepted into NeurIPS 2024

- **What's New**: 이번 논문에서는 생존 분석(Survival Analysis)의 일환으로 Cox 비례 위험 모델(Cox Proportional Hazards Model, CPH)의 훈련에 대한 새로운 최적화 방법을 제안합니다. 이는 기존 알고리즘(주로 Newton 방법 기반)이 고차원 데이터와 높은 특성 상관관계에서 수렴에 어려움을 겪는 문제를 해결하고자 합니다.

- **Technical Details**: 제안된 방법은 CPH 모델의 숨겨진 수학적 구조를 활용하여 대리 함수(Surrogate Functions)를 구성하고 최소화(minimization)함으로써, 단조로운 손실 감소(Monotonic Loss Decrease)와 전역 수렴(Global Convergence)을 보장합니다. 이는 높은 차원의 데이터(n, p)가 존재할 때도 효율적으로 작동하며, cardinality-constrained CPH 문제를 해결할 수 있는 가능성을 보여줍니다.

- **Performance Highlights**: 제안된 최적화 방법은 이전에 비해 매우 희소하고 고품질의 모델을 생산할 수 있으며, 여러 확장 가능성을 통해 CPH 모델의 수학적 구조에 대한 이론적 질문 및 다른 CPH 관련 응용 가능성을 제공합니다.



### Target Strangeness: A Novel Conformal Prediction Difficulty Estimator (https://arxiv.org/abs/2410.19077)
Comments:
          8 pages, 2 figures

- **What's New**: 이 논문에서는 Conformal Prediction (CP)에서 새로운 난이도 추정기인 Target Strangeness를 제안합니다. 이 방법은 예측 구간(Prediction Intervals, PIs)을 정규화하기 위한 대안적 접근 방식을 제공하며, 가장 가까운 이웃의 목표 분포(context of its nearest neighbours' target distribution) 내에서 예측의 비정상성을 평가하여 현 상태의 성능을 초월할 수 있습니다.

- **Technical Details**: Target Strangeness는 K-최근접 이웃(K-nearest neighbours)의 목표 값의 표준 편차를 사용하여 예측이 이웃의 목표 분포에 속할 가능성을 평가하는 새로운 난이도 추정기입니다. Kernel Density Estimation (KDE)을 사용해 예측의 가능성을 평가하고, 그 결과로 'Strangeness measure'를 산출합니다. 이 방법은 CIFAR-10 및 유사한 데이터 세트를 사용한 다중 회귀 실험에서 평가는 진행됩니다.

- **Performance Highlights**: Target Strangeness는 주택 가격 예측 및 무선 경로 손실 예측 문제에서 기존의 난이도 추정기들과 비교하여 경쟁력 있는 결과를 보여주었습니다. 특히, 확신이 증가할수록 Target Strangeness의 예측 구간이 작아지는 경향을 보였으며, 이는 신뢰할 수 있는 예측을 수행하는 데 있어 중요한 요소입니다.



### An Investigation on Machine Learning Predictive Accuracy Improvement and Uncertainty Reduction using VAE-based Data Augmentation (https://arxiv.org/abs/2410.19063)
- **What's New**: 이 연구는 원자력 공학에서 데이터 부족 문제를 해결하기 위한 방법으로 변동 오토인코더(Variational Autoencoder, VAE) 기반의 깊은 생성 모델을 활용하여 데이터 증강(data augmentation)의 효과를 평가합니다. 이를 통해 딥 신경망(Deep Neural Network, DNN)의 예측 정확성과 불확실성을 줄이기 위한 새로운 접근 방법을 선보이고 있습니다.

- **Technical Details**: 이 연구는 DNN을 훈련시키는데 있어 VAE를 사용하여 새로운 합성 샘플을 생성하고, 이를 통해 데이터 세트를 확장함으로써 DNN의 예측 정확성을 향상시킵니다. 또한, Bayesian Neural Networks (BNN)와 conformal prediction (CP) 방법을 활용하여 DNN 예측의 불확실성을 정량화합니다. 연구에서는 NUPEC Boiling Water Reactor Full-size Fine-mesh Bundle Test (BFBT) 벤치마크를 기반으로 한 TRACE 시뮬레이션 데이터를 사용하여 성과를 테스트하였습니다.

- **Performance Highlights**: 연구 결과, VAE를 통한 데이터 증강이 DNN 모델의 예측 정확성을 향상시키고, 예측 신뢰 구간(confidence intervals)을 개선하며, 예측 불확실성을 감소시키는 데 효과적임을 입증하였습니다.



### Newton Losses: Using Curvature Information for Learning with Differentiable Algorithms (https://arxiv.org/abs/2410.19055)
Comments:
          Published at NeurIPS 2024

- **What's New**: 본 논문에서는 Newton Losses라는 방법을 제시하여 훈련하기 어려운 손실 함수의 성능을 향상시키는 새로운 접근법을 소개합니다. 이 방법은 손실 함수의 두 번째 차수 정보를 활용하여 경량화된 방식으로 신경망을 훈련합니다.

- **Technical Details**: Newton Losses는 Fisher와 Hessian 행렬을 통해 손실 함수의 두 번째 차수 정보를 활용합니다. 신경망을 2차 기술로 훈련하는 대신, 우리는 손실 함수의 두 번째 차수 정보를 사용하여 Newton Loss로 대체하고, 이를 이용해 경량화된 gradient descent 방식으로 신경망을 훈련합니다.

- **Performance Highlights**: 8개의 차분 가능한 알고리즘에 Newton Losses를 적용하여, 최적화가 덜 된 차분 가능한 알고리즘들에 대해 상당한 개선을 달성하였고, 이미 잘 최적화된 알고리즘에 대해서도 일관된 성능 향상을 보였습니다.



### Mixture of Parrots: Experts improve memorization more than reasoning (https://arxiv.org/abs/2410.19034)
- **What's New**: 본 논문에서는 Mixture-of-Experts (MoE) 아키텍처가 모델 파라미터 수를 크게 증가시키면서도 계산량은 최소화할 수 있음을 보여줍니다. 최신 MoE와 표준 밀집 변환기(Transformer) 간의 성능 상충이 무엇인지 명확히 분석한 것은 새로운 시도로 평가됩니다.

- **Technical Details**: MoE의 이론적 한계를 분석하며, 특정 너비를 가진 전문가(expert)가 있을 때 해결할 수 없는 그래프 문제를 증명합니다. 반면, 같은 작업은 약간 더 큰 너비를 가진 밀집 모델이 쉽게 해결할 수 있음을 보여줍니다. 또한, 메모리 집약적인 작업에서는 MoE가 적은 수의 활성 파라미터(active parameters)와 다수의 전문가를 활용하여 데이터를 효과적으로 암기할 수 있다는 것을 발견했습니다.

- **Performance Highlights**: 통합된 평가를 통해 MoE와 밀집 변환기를 사전 훈련(pre-train)하고, 수학 및 자연어 처리의 일반적인 벤치마크에서 평가한 결과, 전문가 수 증가가 지식 집약적(task) 작업 해결에는 도움이 되었지만, 추론(task reasoning) 작업에서는 동일한 혜택을 제공하지 못함을 발견했습니다.
...
### Heterogeneous Random Fores (https://arxiv.org/abs/2410.19022)
Comments:
          18 pages, 6 figures

- **What's New**: 이 연구에서는 Heterogeneous Random Forest (HRF)라는 새로운 방법을 소개하여 개별 결정 트리의 다양성을 증가시키는 것을 목표로 합니다. 이는 이전 트리의 루트 노드에 가까운 분할에 사용되는 특징에 낮은 가중치를 부여함으로써 이루어집니다.

- **Technical Details**: HRF는 트리를 생성할 때 트리 내부의 변수 선택을 조정하여 다양성을 높이는 기법을 사용합니다. 이는 가중 샘플링을 통해 이루어지며, 이는 이전 트리의 성과를 반영하여 다음 트리를 위한 특징 서브 공간을 구성할 때 적용됩니다.

- **Performance Highlights**: HRF는 52개의 데이터세트에서 다른 앙상블 방법들과 비교했을 때 대부분의 데이터세트에서 정확도 측면에서 개선된 성능을 보였습니다. 특히 노이즈 특징이 적은 데이터세트에서 HRF의 성능이 더욱 두드러졌습니다.
...
- **What's New**: 이번 연구는 Large Language Models (LLMs)의 zero-shot reasoning 방법의 한계를 극복하기 위해 새로운 구조 지향 분석 방법을 도입합니다. 이 방법은 LLM이 문제를 이해하고 문제 해결 프로세스를 안내하는 데 도움을 줍니다.

- **Technical Details**: 구조 지향 분석 방법은 통사론 및 문법 구조를 기반으로 하여, LLM의 언어 패턴 분석 능력을 활용하여 문제의 주요 구성 요소와 그 관계를 정확하게 식별할 수 있게 합니다. 또한, multi-agent reasoning system인 Structure-oriented Autonomous Reasoning Agents (SARA)를 제안하여 복잡한 질문-응답 작업에서 신뢰성을 높입니다.

- **Performance Highlights**: 제안하는 시스템은 다수의 실험을 통해 몇몇 경우에서 few-shot 방법을 초월하는 성능을 보였습니다. 이 시스템은 복잡한 태스크에서의 reasoning 정확성 향상뿐만 아니라, reasoning 프로세스의 공격에 대한 강도도 높입니다.



### cymyc -- Calabi-Yau Metrics, Yukawas, and Curvatur (https://arxiv.org/abs/2410.19728)
Comments:
          35 pages, 12 figures

- **What's New**: 새롭게 소개된 	exttt{cymyc}는 고차원 Calabi-Yau 다양체의 기하학과 관련된 모듈리 공간을 수치적으로 조사하기 위한 고성능 파이썬 라이브러리입니다. 이 라이브러리는 텐서 필드를 수치 모델링하는 명확한 기하학적 가정을 개발합니다.

- **Technical Details**: 	exttt{cymyc}는 다양한 Calabi-Yau 다양체에서 관심 있는 텐서 필드를 모델링하기 위해 특정 부분 미분 방정식 시스템의 근사 해를 찾는 기계 학습 요소를 포함하고 있습니다. 이 툴킷을 통해 사용자는 Ricci-평면 Calabi-Yau 메트릭을 포함하여 텐서 필드를 모델링하고, 몬테카를로 근사를 통해 복소 구조 모듈리 공간의 메트릭을 평가하며, 이러한 공간의 기하학과 토폴로지를 탐색할 수 있습니다.

- **Performance Highlights**: 기계 학습을 포함한 두 가지 독립된 방법을 사용하여 프로젝트 무작위 공간에서 수치적으로 정상화된 Yukawa 결합을 계산했습니다. 두 방법 모두 탁월한 일치를 보였습니다. 또한, Calabi-Yau 다양체의 Ricci 만곡이 0인 메트릭과 관련된 곡률 불변량의 분포를 조사했습니다.
...
- **What's New**: 이번 연구에서는 선형 연산자(Linear operator)를 대상으로 한 능동적 데이터 수집 전략(Active data collection strategies)의 효과를 분석하였습니다. 특히 연속 공분산 커널(Continuous covariance kernels)에 의해 정의되는 평균 0의 확률 과정에서 입력 함수가 추출되는 상황을 살펴보았습니다.

- **Technical Details**: 능동적 데이터 수집 전략을 통해 공분산 커널의 고유값(Eigenvalue) 감소 속도에 따라 오차 수렴률(Error convergence rate)을 설정할 수 있습니다. 고유값이 빠르게 감소하는 공분산 커널을 사용할 경우, 오차 수렴 속도를 임의로 빠르게 할 수 있는 반면, 수동적 데이터 수집 방법인 독립적으로 동일하게 분포된(i.i.d.) 샘플을 사용할 경우 수렴 속도는 $	ext{sim } n^{-1}$로 제한된다는 점에서 차별화됩니다.

- **Performance Highlights**: 본 연구의 결과는 전통적인 수동적 학습 대신 능동적 데이터 수집 전략을 활용했을 때 연산자 학습에서 얻는 이점을 강조합니다. 수동적 데이터 수집 전략에 대해서는 	extit{non-vanishing} 하한이 설정되어 있으며, 이는 고유값 감소 속도와는 관계없이 모든 수동적 전략에 적용됩니다.



### Temporal Convolution-based Hybrid Model Approach with Representation Learning for Real-Time Acoustic Anomaly Detection (https://arxiv.org/abs/2410.19722)
Comments:
          10 pages, 10 figures, ICMLC2024

- **What's New**: 본 연구는 산업 기계 구성 요소의 초기 결함 탐지를 위한 혁신적인 접근 방식을 제시하여 Machine Condition Monitoring (MCM)을 향상시킵니다. 제안된 방법은 반지도학적(semisupervised) 시간 합성곱(temporal convolution)과 표현 학습(representation learning)을 결합한 하이브리드 모델 전략을 사용하여, 다양한 복잡한 이상 패턴을 효과적으로 처리합니다.

- **Technical Details**: 제안된 모델은 Temporal Convolutional Networks (TCN)을 기반으로 하며, acoustic data에서의 실시간 이상 탐지를 위한 복잡한 시계열 특징을 포착합니다. 이 모델은 표준 컨볼루션에 비해 과거 값에만 의존하여 계산 비용을 줄이는 이점을 제공합니다. 또한, 반지도학습과 표현 학습을 통해 정상과 이상 경계를 구별하는 데 필요한 시각적 표현을 제공합니다.

- **Performance Highlights**: 제안된 하이브리드 모델은 기존 연구들과 비교하여 우수한 성능을 나타냅니다. 양적 증거와 시각적 표현(t-SNE 플롯 등)을 통해 모델의 유효성을 입증하였습니다. 이 연구는 산업 환경에서의 MCM을 지원하기 위한 효과적인 솔루션을 제공함으로써, 사전 탐지 및 운영 효율성을 최적화하는 데 기여합니다.



### Multi-view biomedical foundation models for molecule-target and property prediction (https://arxiv.org/abs/2410.19704)
Comments:
          34 pages including supplement. 9 figures, 4 tables

- **What's New**: 이번 연구에서는 다중 뷰(multi-view) 기초 모델 접근 방식을 개발하여 분자의 그래프(graph), 이미지(image), 텍스트(text) 뷰를 통합하여 약물 발견(accelerate drug discovery)을 가속화할 수 있는 가능성을 제시합니다.

- **Technical Details**: 연구진은 다중 뷰 분자 임베딩 모델인 MMELON을 통해 여러 형태의 분자 표현을 조합하여 기초 다중 모달 임베딩을 학습합니다. 이 모델은 이미지, 2D 화학 결합 그래프 및 텍스트 뷰를 포함합니다. 각각의 뷰는 최대 2억 개의 분자 데이터셋에서 사전 훈련(pre-trained)되었으며, 이후 결합된 표현으로 최종 모델이 생성되었습니다.

- **Performance Highlights**: 모델은 18개의 다양한 임무(tasks)에서 강력한 성능을 보였으며, 100개 이상의 G 단백질 결합 수용체(GPCRs)에 대해 화합물을 스크리닝하여 노인성 치매(Alzheimer's disease) 관련 33개 타겟을 식별했습니다. 이 부분집합에 대해서는 강력한 결합제를 식별하고, 구조 기반 모델링(structure-based modeling)을 통해 검증하였습니다.
...
### Electromechanical Dynamics of the Heart: A Study of Cardiac Hysteresis During Physical Stress Tes (https://arxiv.org/abs/2410.19667)
- **What's New**: 이번 연구는 저비용 기술인 전기심장도(Electrocardiography, ECG)와 청심도(Phonocardiography, PCG)를 이용하여 심장 전기 및 기계 기능 간의 상호작용을 분석하는 방법론을 제시합니다.

- **Technical Details**: EPHNOGRAM ECG-PCG 데이터셋을 활용하여 23명의 건강한 남성을 대상으로 심박동(RR), QT, 수축기(systolic), 이완기(diastolic) 간의 전기기계 간격 및 상관관계를 분석하였습니다. 시간 지연 분석을 통해 QT, 수축기, 이완기 간의 명확한 시간적 관계를 밝혀내었고, QT 간격은 RR 간격 변화에 10.5초 평균 지연으로 반응하며, 수축기 간격은 28.3초 평균 지연으로 반응함을 확인했습니다.

- **Performance Highlights**: QT, 수축기 및 이완기 간의 히스테리시스(loop) 패턴을 분석하여 이완기 RR의 좁은 루프와 수축기 RR의 넓은 루프를 발견하였으며, 심박수 변화와 히스테리시스 루프 면적 간의 유의미한 상관관계(평균: 0.75)를 찾아냈습니다. 이는 운동 스트레스 하에서 심장 기능을 평가할 수 있는 새로운 바이오마커로의 가능성을 제시합니다.



### Improving Stochastic Cubic Newton with Momentum (https://arxiv.org/abs/2410.19644)
- **What's New**: 이 논문에서는 일반 비볼록(non-convex) 최적화 문제를 해결하기 위해 확률적(stochastic) 2차 방법을 연구하고 있습니다. 특별한 형태의 모멘텀(momentum)을 활용하여 뉴턴(Newton) 방법에서 확률적 경량(mean) 및 헤시안(Hessian) 추정값의 변동성을 개선하고, 이러한 방식이 어떤 노이즈 수준에서도 수렴할 수 있음을 보여줍니다.

- **Technical Details**: 기존의 확률적 2차 방법들은 일반적으로 대량의 배치(batch)를 요구하는 반면, 본 연구에서는 임의의 크기의 배치로 전 세계적 수렴(global convergence)을 증명한 첫 번째 사례로, 단 하나의 확률적 데이터 샘플만을 사용해도 수렴할 수 있음을 입증합니다. 또한, 모멘텀을 사용하여 정규화된 뉴턴 방법이 볼록(convex) 확률적 문제에 대해 향상된 속도를 낸다는 점도 강조합니다.

- **Performance Highlights**: 이 연구는 비볼록 문제들을 다루기 위한 효과적인 확률적 2차 최적화 알고리즘을 설계, 구현함으로써, 기존 방법들에 비해 더 작은 배치를 사용하는 순간에도 안정적으로 수렴함을 보였습니다. 이를 통해 기계 학습 및 다른 대규모 응용 프로그램에서의 효과적인 문제 해결을 위한 새로운 가능성을 제시합니다.



### Mask-Weighted Spatial Likelihood Coding for Speaker-Independent Joint Localization and Mask Estimation (https://arxiv.org/abs/2410.19595)
Comments:
          ©2024 IEEE. Personal use of this material is permitted. Permission from IEEE must be obtained for all other uses, in any current or future media, including reprinting/republishing this material for advertising or promotional purposes, creating new collective works, for resale or redistribution to servers or lists, or reuse of any copyrighted component of this work in other works

- **What's New**: 이 논문은 음성 분리를 위한 새로운 접근 방식을 제안합니다. 특히, 시간-주파수 마스크와 고정된 공간 맵에서의 음성의 상대 방향을 동시에 추정하는 방법에 중점을 두고 있습니다. Mask-weighted spatial likelihood coding을 도입하여, 기존의 문제를 해결하고 성능을 향상시킵니다.

- **Technical Details**: 저자들은 음성 소스의 위치와 마스크 정보를 효과적으로 통합하기 위해, mask-weighted spatial likelihood coding (MW-SLC)을 제안합니다. MW-SLC는 기존의 방법인 mask-weighted spatial binary coding (MW-SBC)과 비교하여, 고해상도 공간에서의 최적화 문제를 해결하고 label imbalance의 영향을 줄일 수 있습니다.

- **Performance Highlights**: 이 접근법은 joint localization과 mask estimation에서 모두 뛰어난 성능을 보이며, 각 작업을 개별적으로 수행할 때도 상당한 성능을 유지합니다. 기존의 sound source localization 시스템을 대체할 수 있는 보편적인 접근 방식을 제공하여, 성능이 중요한 시나리오에 매우 적합합니다.



### Considerations for Distribution Shift Robustness of Diagnostic Models in Healthcar (https://arxiv.org/abs/2410.19575)
- **What's New**: 이번 연구에서는 헬스케어의 진단 모델에서 분포 변화에 대한 강건성을 위하여 인과적 기계 학습 이론의 최근 결과를 적용하는 방법을 제안합니다. 일반적으로 적용되는 예측 모델에서 환자에 대한 추가 정보(V) 없이 바이오마커(X)만으로 질병(Y)의 존재를 예측하는 경우가 많은데, 이러한 접근법이 가진 단점을 분석하고 보완합니다.

- **Technical Details**: 연구에서는 인과적 관계의 역할을 강조하며, 예측 모델이 X와 Y 간의 불안정한 혼동 의존성(confounding dependencies)을 학습하는 경우 성능이 저하될 수 있음을 보여줍니다. 예측 모델은 V와 Y 간의 인과적 관계를 활용해야 하며, 특정 공변량을 포함한 모델이 더 강건한 예측기를 제공할 수 있음을 이론적으로 증명합니다.

- **Performance Highlights**: 시뮬레이션 연구에서는 다양한 데이터 생성 프로세스 하에서 서로 다른 예측기의 강건성과 성능 저하를 비교하여, 특정 공변량을 포함한 모델이 최적의 강건성을 달성하는 데 기여함을 보여주었습니다. 특히 PTB-XL 데이터셋을 사용하여 실제 예측 성능 분석도 수행하였습니다.



### How Critical is Site-Specific RAN Optimization? 5G Open-RAN Uplink Air Interface Performance Test and Optimization from Macro-Cell CIR Data (https://arxiv.org/abs/2410.19565)
Comments:
...
- **What's New**: 이 논문에서는 특정 위치에서의 채널 측정 데이터의 중요성과 그것이 공중 인터페이스 최적화 및 테스트에 미치는 영향을 다룹니다. 기존의 통계적 채널 모델과 실제 측정 기반 채널을 비교하여, 3GPP TDL 모델과 측정된 매크로 셀 채널 임펄스 응답(CIR) 데이터를 활용하여 뉴럴 리시버의 성능을 향상시키는 방법을 제안합니다.

- **Technical Details**: OmniPHY-5G 뉴럴 리시버를 사용하여 NR PUSCH 업링크 시뮬레이션을 수행하며, 통계적 TDL 채널 모델로 사전 훈련한 뒤 측정된 사이트 특화 MIMO CIR 데이터로 미세 조정하는 과정을 거칩니다. 이 미세 조정 방법은 단순히 TDL 채널에 대한 사전 훈련을 한 경우보다 1.85 dB 낮은 신호 대 잡음 비율(SNR)에서 10% 블록 오류율(BLER)을 달성합니다.

- **Performance Highlights**: 제안된 방법은 사이트 특정 채널 진단의 효과를 정량적으로 제시하며, 향후 RAN(무선 접속망) 최적화를 위한 학습 개선 방향을 제시합니다. 이를 통해 물리적 레이어의 성능을 크게 향상시킬 수 있음을 입증합니다.
...
### Parametric Nonlinear Volterra Series via Machine Learning: Transonic Aerodynamics (https://arxiv.org/abs/2410.19514)
- **What's New**: 본 연구에서는 비정상(unstable) 트랜소닉(aerodynamics) 유동 해석을 모형화하는 새로운 접근 방식을 소개합니다. Volterra 시리즈를 사용하여 공기역학적 응답을 캡처하고 머신러닝(ML)을 통해 보간(interpolation)을 가능하게 합니다.

- **Technical Details**: 본 연구는 컴퓨터 유체역학(computational fluid dynamics)으로 얻은 인디셜(Indicial) 공기역학적 응답을 통해 1차 및 2차 Volterra 커널(kernels)을 도출합니다. 2차 커널은 우세한 선형 응답에 대한 수정으로 계산됩니다. 머신러닝 알고리즘, 특히 인공신경망(Artificial Neural Network)과 가우시안 프로세스 회귀(Gaussian Process Regression)를 사용하여 마하 수(Mach number)와 공격각(angle of attack)으로 정의된 매개변수 공간에서 커널 계수를 보간합니다.

- **Performance Highlights**: 방법론은 트랜소닉 조건에서의 합성 공기역학 모델과 2D 에어포일, 3D 날개의 세 가지 테스트 케이스에 적용되었습니다. 두 번째 차수 커널을 포함함으로써 강한 비선형성을 해결할 수 있는 장점이 강조되며 인공신경망의 효과적인 성능이 입증되었습니다. 연구 결과는 개념 설계에서의 사용에 충분한 정확도를 보여주었습니다.
...
- **What's New**: 본 논문에서는 기존의 샘플링 방법들이 가진 하이퍼파라미터 조정 문제를 해결하기 위해 다중 모드(multimodal) 분포를 위한 Learned Reference-based Diffusion Sampler (LRDS)를 제안합니다. LRDS는 목표 모드의 위치에 대한 사전 지식을 활용하여 샘플링을 단순화합니다.

- **Technical Details**: 제안된 LRDS는 두 단계로 진행됩니다: (i) 고밀도 공간에서 샘플링한 데이터로 참고 확산 모델(reference diffusion model)을 학습하고, (ii) 이 모델을 사용하여 샘플러를 훈련합니다. LRDS는 기존의_score-based diffusion_ 방법론에 기초하고 있으며, 다양한 도전적인 분포에서 상대적으로 우수한 성능을 보입니다.

- **Performance Highlights**: 실험적으로, LRDS는 다양한 분포에서 기존 알고리즘 대비 목표 분포에 대한 사전 지식을 가장 효과적으로 활용하는 것으로 입증되었습니다. 특히, LRDS는 다중 모드 분포 샘플링에서의 성능이 특히 뛰어난 것으로 나타났습니다.
...
### On the Application of Deep Learning for Precise Indoor Positioning in 6G (https://arxiv.org/abs/2410.19436)
Comments:
          6 Pages, 6 Figures

- **What's New**: 본 논문은 실내 공장 환경에서의 위치 추적 정확도를 향상시키기 위해 AI/ML 기법을 활용하는 새로운 신경망 모델 LocNet을 제안합니다. LocNet은 여러 변환 수신 지점(Transmit Receive Points, TRP)에서 수집된 측정치를 기반으로 훈련됩니다.

- **Technical Details**: LocNet은 채널 임펄스 응답(Channel Impulse Response, CIR)과 기준 신호 수신 전력(Reference Signal Received Power, RSRP)과 같은 측정값을 입력으로 받아 유저의 (x,y) 좌표를 예측하는 구조입니다. 이 모델은 residual blocks와 attention mechanisms을 사용하여 설계되었습니다.

- **Performance Highlights**: LocNet은 18개의 TRP로부터 측정값을 활용할 때 90번째 백분위수 기준으로 9 센티미터의 위치 정확도를 달성하였으며, 일부 TRP에서 측정이 불가능할 경우에도 효과적으로 일반화된 성능을 보였습니다.
...
### COMSPLIT: A Communication-Aware Split Learning Design for Heterogeneous IoT Platforms (https://arxiv.org/abs/2410.19375)
Comments:
          Accepted for publication in IEEE Internet of Things Journal

- **What's New**: 새로운 통신 인식 설계인 COMSPLIT이 IoT 네트워크의 시계열 데이터 처리에 최적화된 분할 학습(SL) 및 추론 패러다임으로 소개되었습니다.

- **Technical Details**: COMSPLIT은 다양한 통신 채널 조건에 영향을 받는 IoT 네트워크에 적응 가능한 SL 배포를 위한 다재다능한 프레임워크를 제공합니다. 또한 이 프레임워크는 이른 종료 전략(early-exit strategy)을 통합하고, 이종 컴퓨팅 능력을 가지는 장치들을 고려하여 설계되었습니다.

- **Performance Highlights**: COMSPLIT은 기존의 빈약한 SL 접근 방식에 비해 뛰어난 성능을 보이며, 다양한 채널 조건에 대한 적응성과 설계의 단순성을 제공하는 능력을 입증했습니다.



### Noise-Aware Differentially Private Variational Inferenc (https://arxiv.org/abs/2410.19371)
- **What's New**: 본 연구에서는 고차원 및 비-공액 모델에 적용 가능한 새로운 소음 인식 근사 베이지안 추론 방법을 제안합니다. 이는 확률적 경량 변동 추론(stochastic gradient variational inference) 기반으로, 여러 기존 방법의 한계를 극복하고자 하였습니다.

- **Technical Details**: Noise-Aware DP VI (NA-DPVI) 접근 방법을 제안하며, 이는 DP 변동 추론(DP Variational Inference)에서 얻은 노이즈 그래디언트를 사용하여 불확실성을 포착하고 원래 베이지안 추론 문제에 대해 노이즈 인식 사후확률(noise-aware posterior)을 생성합니다. 이 방법은 다양한 모델에 적용 가능하며, 하이퍼파라미터가 노이즈 인식 사후확률 근사에 미치는 영향을 분석합니다.

- **Performance Highlights**: 고차원 베이지안 선형 회귀 및 UCI Adult 데이터셋을 이용한 베이지안 로지스틱 회귀에서 기존 방법과 유사한 성능을 보이며, 더 넓은 범위에서 정확한 커버리지(coverage)와 잘 보정된 예측 확률(predicted probabilities)을 제공합니다.
...
### Double Difference Earthquake Location with Graph Neural Networks (https://arxiv.org/abs/2410.19323)
- **What's New**: 이번 논문에서는 Graph Neural Network (GNN)을 활용한 새로운 지진 이중 차이 재배치 프레임워크인 Graph Double Difference (GraphDD)를 제안합니다. 이 방법은 이중 차이 잔차를 최소화하여 지진을 위치시키는 모델을 학습합니다.

- **Technical Details**: GraphDD는 두 개의 그래프를 사용하여 스테이션과 소스를 표현하며, 이 두 그래프 간의 데카르트 곱 그래프를 생성하여 스테이션과 소스 간의 관계를 포착합니다. 이 아키텍처는 이중 차이 잔차를 최소화하는 데 적합한 구조를 제공합니다.

- **Performance Highlights**: 다양한 테스트 케이스를 통해 높은 해상도의 재배치를 달성하였으며, 다양한 손실 함수 및 위치 목표에 대한 적응성을 보여줍니다. 특히, 여러 지진 데이터의 품질 및 분포가 변동성이 심한 경우에도 좋은 성능을 발휘하였습니다.



### Free-Rider and Conflict Aware Collaboration Formation for Cross-Silo Federated Learning (https://arxiv.org/abs/2410.19321)
- **What's New**: 이번 논문에서는 자율 이익(self-interest)과 경쟁(competition)이 존재하는 비즈니스 분야의 Federated Learning (FL)을 다루고 있습니다. 제안된 FedEgoists 접근 방식은 FL-PT (Federated Learning Participants) 간의 최적 협력(cooperation) 구성을 보장하여, FL 생태계 내에서 불공평한 혜택을 방지할 수 있습니다.

- **Technical Details**: FedEgoists는 FL-PT를 동일한 이해관계를 가진 코얼리션(coalition)으로 그룹화하는 효율적인 클러스터링 솔루션입니다. 이 접근법은 각 코얼리션 내에서 FL-PT들이 상호 이익을 공유하도록 하며, 이론적으로는 다른 코얼리션과 협력하더라도 구성원들의 유틸리티를 증가시킬 수 없음을 증명합니다. 또한 그래프 이론(graph theory)을 활용하여 FL-PT 간의 경쟁과 이익 관계를 모델링합니다.

- **Performance Highlights**: 실험 결과, FedEgoists는 9개의 최신 방법들에 비해 FL-PT 간 효율적인 협력 네트워크를 구축하는데 뛰어난 성능을 보였습니다. 특히 비즈니스 활동에 참여하는 FL-PT를 대상으로 하는 크로스-사일로(cross-silo) FL 환경에서의 효과성이 입증되었습니다.



### Fully First-Order Methods for Decentralized Bilevel Optimization (https://arxiv.org/abs/2410.19319)
Comments:
          46 pages

- **What's New**: 본 논문은 에이전트들이 오직 이웃과만 소통하는 분산 확률적 이단 최적화(DSBO)를 다룹니다. 저희는 경량화된 알고리즘인 DSGDA-GT를 제안하며, 이는 기존에 많이 사용되는 2차 오라클에 비해 1차 오라클만 필요합니다.

- **Technical Details**: DSGDA-GT 알고리즘은 경량화된 1차 오라클(First-order oracle)만 요구하여 계산 비용을 줄입니다. 마찬가지로, n개의 에이전트가 협력하여 DSBO 문제를 해결할 경우, 샘플 복잡도(Sample complexity)는 $	ext{O}(n^{-1}	ext{ε}^{-7})$로, 현재 알려진 단일 에이전트에 대한 최적 결과와 일치합니다.

- **Performance Highlights**: 수치 실험(Numerical experiments)에서는 저희 알고리즘이 통신(Communication) 및 훈련 효율성(Training efficiency) 모두에서 우수하다는 것을 보여주었습니다.
...
### Reinforcement Learning the Chromatic Symmetric Function (https://arxiv.org/abs/2410.19189)
- **What's New**: 본 논문에서는 강화학습(reinforcement learning)을 활용하여 단위 구간 그래프(unit interval graphs)의 색상 대칭 함수(chromatic symmetric function)의 계수에 대한 추측적(counting) 공식(conjectural counting formula)을 제안합니다. 이 공식은 특정 조건을 만족하는 서로 다른 사이클 집합(cycle-tuples)을 세어줍니다.

- **Technical Details**: 제안된 공식은 딥 강화학습(deep reinforcement learning) 모델을 사용하여 특정 연결(connected) 조건을 식별합니다. 이러한 조건은 특정 단위 구간 그래프와 관계 없이 일반적인(counting) 표현을 제공합니다. 이 연구는 저자들이 그래프 색칠(graph coloring)의 수학적 성질을 탐구하고 이를 외부 기법(기계 학습)과 결합하여 원하는 결과를 이끌어내는 데 목적을 두고 있습니다.

- **Performance Highlights**: 제안된 공식의 성능은 특정 단위 구간 그래프에 대한 그래프 색칠의 성격을 분석함으로써 증명될 예정이며, 이는 그래프 이론과 조합론(algebraic combinatorics) 분야의 연구에 중요한 기여를 할 것입니다.
...
### Cascading Failure Prediction via Causal Inferenc (https://arxiv.org/abs/2410.19179)
- **What's New**: 본 논문은 전력 전송 네트워크에서의 연쇄 실패(cascading failures)를 분석하기 위한 새로운 인과 추론(causal inference) 프레임워크를 제안합니다. 이 프레임워크는 전송선의 노드(node)와 인과 관계를 나타내는 방향성(latent) 그래프(directed latent graph)를 생성합니다.

- **Technical Details**: 제안된 프레임워크는 두 가지 알고리즘을 포함하여 전송선 고장으로 인해 발생하는 연쇄적 효과를 예측합니다. 이 프레임워크는 기존 시뮬레이션 방법에 비해 데이터 기반(data-driven)이며, 전력 시스템의 복잡한 상호작용을 캡처합니다. 인과 그래프(causal graphs)는 각 노드와 엣지(edge)를 통해 네트워크 내의 복잡한 인과 관계를 표현합니다.

- **Performance Highlights**: IEEE 14-bus, 39-bus 및 118-bus 시스템을 대상으로 한 성능 평가에서, 제안된 인과 프레임워크는 영향 그래프(influence graphs) 및 그래프 신경망(graph neural networks) 기반 접근 방식과 비교 하여 우수한 예측 성능을 보였습니다.



### Cross Spline Net and a Unified World (https://arxiv.org/abs/2410.19154)
- **What's New**: 이 논문에서는 cross spline net (CSN)이라는 새로운 모델링 프레임워크를 제안합니다. CSN은 스플라인 변환(spline transformation)과 크로스 네트워크(cross-network)의 조합에 기반하여 개발되었습니다. 이 방법은 기존의 복잡하고 해석이 어려운 XGBoost와 완전 연결 신경망(FCNN)보다 간단하고 해석이 용이하며 강건한 특성을 가지고 있습니다.

- **Technical Details**: CSN 프레임워크는 스플라인 레이어(spline layer)를 유연하게 구성할 수 있어 다양한 모델을 생성할 수 있습니다. 이 방법을 통해 선형 및 스플라인 기반 통계 모델, 트리, 규칙 맞춤, 트리 앙상블(예: gradient boosting trees, random forest), 경사진 나무(oblique tree), 다변량 적응 회귀 스플라인(MARS), 다항 커널을 가진 SVM 등과 같은 비신경망(non-neural network) 모델을 재현하거나 근사할 수 있습니다.

- **Performance Highlights**: CSN의 하위 유형인 TreeNet을 사용하여 XGBoost 및 FCNN과 성능을 비교합니다. 이 연구는 TreeNet이 더 높은 성능과 편리함을 제공함을 보여주고, CSN이 실무자가 고성능, 강건하며 더 해석이 용이한 모델을 구축할 수 있는 유연하고 편리한 프레임워크가 될 것이라고 믿고 있습니다.



### Functional Brain Network Identification in Opioid Use Disorder Using Machine Learning Analysis of Resting-State fMRI BOLD Signals (https://arxiv.org/abs/2410.19147)
Comments:
          20 pages, 7 figures, 8 tables

- **What's New**: 이번 연구는 opioid use disorder (OUD)의 neurobiology를 이해하기 위해 resting-state functional magnetic resonance imaging (rs-fMRI)를 활용하였습니다. 특히 데이터 기반 기계 학습 (machine learning, ML) 모델링을 통해 OUD와 건강한 대조군 (healthy controls, HC)을 구분하는 지역을 처음으로 식별했습니다.

- **Technical Details**: 연구는 triple network 모델을 기반으로 하여 31명의 OUD 환자와 45명의 HC 피험자로부터 DMN (default mode network), SN (salience network), ECN (executive control network)에서 rs-fMRI BOLD 신호를 추출하였습니다. Boruta ML 알고리즘을 사용하여 OUD와 HC를 구분하는 통계적으로 유의미한 BOLD 특징을 식별하였으며, DMN이 OUD에 가장 중요한 기능적 네트워크로 나타났습니다.

- **Performance Highlights**: 5-fold cross-validation 설정에서 DMN의 기능적 네트워크 특징을 사용해 OUD와 HC를 구분했으며, BOLD와 인구학적 특징을 결합할 때 평균 AUC 80.91% 및 F1 점수 73.97%를 달성했습니다. DMN 내에서 중앙 전두엽 피질(medial prefrontal cortex), 후방 싱귤레이트 피질(posterior cingulate cortex), 왼쪽 및 오른쪽 두정엽 접합부(temporoparietal junction)에서 추출한 BOLD 특징에 대한 후속 Boruta 분석 역시 중요한 특징들이 확인되었습니다.
...
### Maximum a Posteriori Inference for Factor Graphs via Benders' Decomposition (https://arxiv.org/abs/2410.19131)
- **What's New**: 본 논문에서는 Benders' decomposition을 사용하여 일반적인 Bayesian factor 모델에서 최대 사후 확률 추정(maximum a-posteriori inference) 방법을 새롭게 제안합니다. 이 방법은 완전히 이완된 이중 문제에 제약 조건을 순차적으로 추가하는 방식으로, 클러스터링 문제에서 표현력이 뛰어난 정수 및 논리 제약을 통합할 수 있습니다.

- **Technical Details**: 제안된 알고리즘은 Bayesian Gaussian mixture model 및 Latent Dirichlet allocation 모델에 적용되어 실험 결과를 도출합니다. 이 알로리즘은 비선형 혼합 정수 프로그래밍 문제를 해결하기 위해 Benders' decomposition 기법을 채택하며, 제약 조건을 추가할 때 순차적인 접근 방식을 사용합니다. 알고리즘은 최적의 사후 가치(optimal posterior value)를 도출하는 동시에 수렴(certificates of convergence)을 보장합니다.

- **Performance Highlights**: 제안된 방법은 Gibbs 샘플링 및 변분 베이즈 방법에 비해 표준 데이터 세트에서 더 높은 최적 사후 값을 생성하며, 수렴 인증을 제공합니다. 실험 결과는 제안된 방법이 클러스터링 성능 및 계산 효율성을 높인다는 것을 보여줍니다.



### A spectral method for multi-view subspace learning using the product of projections (https://arxiv.org/abs/2410.19125)
Comments:
          23 pages, 7 figures

- **What's New**: 본 논문은 다중 뷰(multi-view) 데이터에서 공유 신호(joint signal)와 개별 신호(individual signal) 아우라를 명확하게 식별할 수 있는 조건을 제시합니다. 저자들은 신호 순위(signal rank), 잡음 수준(noise level), 개별 아우라 간의 주각(principal angles) 등의 요소들이 신호 식별에 미치는 영향을 분석합니다.

- **Technical Details**: 제안된 방법은 회전 부트스트랩(rotational bootstrap)과 확률 행렬 이론(random matrix theory)을 사용하여 관찰된 스펙트럼을 공유, 개별, 잡음 아우라로 나누는 알고리즘을 제공합니다. 이 알고리즘은 고차원 데이터에서 신호를 저차원으로 표현하는데 중점을 두며, 잡음을 포함한 다양한 아우라 간의 구별을 용이하게 합니다.

- **Performance Highlights**: 시뮬레이션 결과, 제안된 방법은 기존의 방식들보다 공유 및 개별 아우라를 더욱 정확하게 추정하며, 대장암 환자의 다중 오믹스(multi-omics) 데이터와 영양유전학(nutrigenomics) 연구에서 개선된 예측 성능을 나타냈습니다.
...
### Distributed Blind Source Separation based on FastICA (https://arxiv.org/abs/2410.19112)
Comments:
          5 pages

- **What's New**: 이 논문에서는 무선 센서 네트워크(WSNs) 환경에서 원신호(원래 신호) 소스를 식별하기 위한 분산 독립 성분 분석(Independent Component Analysis, ICA) 알고리즘인 DistrICA를 제안합니다. 이 알고리즘은 중앙 집중 처리 없이 각 센서가 서로 전송하는 선형 융합 센서 신호만을 전송하여 통신 부담을 줄이고도 중앙 집중 ICA 문제의 일부 소스를 추출합니다.

- **Technical Details**: DistrICA 알고리즘은 네트워크 전체의 ICA 문제를 해결하는 분산 알고리즘으로, 복잡한 사전 백색화(pre-whitening) 과정을 생략합니다. 이 알고리즘은 Distributed Adaptive Signal Fusion(DASF) 프레임워크의 속성을 활용하여 노드 간의 신호를 선형적으로 융합합니다. 이를 통해 전체 네트워크에서의 사전 백색화 과정을 피하면서도, 통신 부하에 비례하여 필요한 비가우시안 독립 성분들을 얻을 수 있습니다.

- **Performance Highlights**: DistrICA 알고리즘은 기존의 FastICA와 같은 중앙 집중 방식의 ICA에 비해 우수한 성능을 발휘하며, 커뮤니케이션 비용을 대폭 줄이면서도 원신호 소스의 효과적인 추출을 가능하게 합니다. 이 알고리즘은 특히 로우 SNR (Signal to Noise Ratio) 환경에서도 안정적인 결과를 제공합니다.
...
- **What's New**: 본 논문은 얕은 결정 트리를 기초 학습자(base learners)로 사용할 때 트리 앙상블 모델이 본질적으로 해석 가능해지며 더 나은 일반화 성능을 발휘할 수 있음을 보여줍니다. 또한 기능적 ANOVA 기반의 해석 알고리즘을 개발하여 트리 앙상블 모델을 해석할 수 있는 방법을 제시합니다.

- **Technical Details**: 제안된 기법은 트리 앙상블 모델을 기능적 ANOVA(fANOVA) 표현으로 변환하는 해석 알고리즘을 포함합니다. 이 알고리즘은 본질적으로 해석 가능하고, 나중에 도입되는 제약 조건과 후처리 효과 가지치기(pruning)를 통해 모델 해석 가능성을 강화합니다. 다양한 방법으로 수치 실험을 통해 그 효과를 입증하고 있습니다.

- **Performance Highlights**: 얕은 결정 트리를 사용한 트리 앙상블 모델은 기존의 EBM(explainable boosting machine)보다 더 나은 예측 성능과 해석 가능성을 보여줍니다. 시뮬레이션 및 실제 데이터세트에 대한 실험을 통해 제안된 방법이 기존의 기준 모델들과 비교했을 때 모델 해석과 예측 성능 간의 더 나은 균형을 제공함을 확인하였습니다.
...
### A Generalized Framework for Multiscale State-Space Modeling with Nested Nonlinear Dynamics: An Application to Bayesian Learning under Switching Regimes (https://arxiv.org/abs/2410.19074)
- **What's New**: 이 연구는 전환(revolving) 체제 하에서 Bayesian 학습을 통합한 다중 시간 척도(state-space) 모델링을 위한 일반화된 프레임워크를 제안합니다. 이 프레임워크는 시스템 내에서 빠른 프로세스와 느린 프로세스 간의 복잡한 상호작용을 포착하며, 이러한 다이내믹스가 서로에게 영향을 미치는 방식에 대한 분석을 가능하게 합니다.

- **Technical Details**: 제안된 모델은 더 세밀한 시간 척도의 동적이 더 거친 시간 척도 내에 중첩되어 있는 계층적 구조를 통해 상호작용을 모델링합니다. Bayesian 학습 접근 방식을 통해 잠재(state) 상태 및 전환 동태를 추정하여 시스템이 체제 변경에 효과적으로 적응할 수 있도록 합니다. 또한 Sequential Monte Carlo, 또는 particle filtering 기법을 사용하여 추론을 수행합니다.

- **Performance Highlights**: 시뮬레이션 결과는 제안된 프레임워크가 상태 전이를 효과적으로 추적하고 다중 시간 척도 시스템에서 전환 동태의 정확한 식별을 달성함을 보여줍니다. 특히, Bayesian 학습 접근 방식은 복잡한 시스템에서 발생하는 다이내믹스의 변화를 잘 포착하고 대응할 수 있음을 증명합니다.



### Less Discriminatory Alternative and Interpretable XGBoost Framework for Binary Classification (https://arxiv.org/abs/2410.19067)
- **What's New**: 이 논문에서는 공정하고 설명 가능한 이진 분류를 위한 새로운 기계 학습 모델인 LDA-XGB1을 소개합니다. 이 모델은 정확성과 공정성을 균형 있게 맞추기 위해 바이오브젝티브 최적화 (biobjective optimization)를 통해 개발되었습니다.

- **Technical Details**: LDA-XGB1은 XGBoost의 예측 능력과 계산 효율성을 활용하면서, 깊이 1의 얕은 트리를 통해 모델 해석 가능성을 보장합니다. 이 모델은 또한 주요 피쳐에 대해 단조 제약 조건(monotonic constraints)을 강제하여 예측의 논리적이고 투명한 행동을 보장합니다.

- **Performance Highlights**: LDA-XGB1은 SimuCredit 및 COMPAS 두 가지 데이터 세트에서 평가되었으며, 예측 정확성, 공정성, 해석 가능성 간의 효과적인 균형을 달성하고 기존의 공정 대출 모델들보다 우수한 성과를 지닌 것을 보여주었습니다.



### Privacy-Computation trade-offs in Private Repetition and Metaselection (https://arxiv.org/abs/2410.19012)
- **What's New**: 이번 연구에서는 차별적 개인 정보 보호 (differential privacy)를 유지하면서 알고리즘의 성공 확률을 높일 수 있는 Private Repetition 알고리즘을 제안하였습니다. 이 알고리즘은 기존 알고리즘에 비해 개인 정보 보호 비용을 최소화하면서도 높은 성공 확률을 보장할 수 있는 방법을 제시합니다.

- **Technical Details**: Private Repetition 알고리즘은 기존의 메타 선택 (metaselection) 및 하이퍼파라미터 조정 (hyperparameter tuning) 알고리즘과 관련이 깊습니다. 이를 통해 개인 정보 보호 비용이 상수 요인만큼 유지되면, 실패 확률은 계산 비용에 대해 다항적으로만 감소할 수 있다는 강한 하한을 보여줍니다. 추가적으로, 기존 알고리즘을 조합함으로써 컴퓨테이션과 개인 정보 보호 간의 트레이드오프를 달성할 수 있습니다.

- **Performance Highlights**: 하이퍼파라미터 선택 과정에서 기존의 알고리즘과 조합하여 성능과 개인 정보 보호 비용 사이의 연속적인 트레이드오프를 가능하게 하는 실험 결과를 제시하였습니다. 이는 계산 비용에 비례하여 개인 정보 보호 비용을 유지하는 방법에 대한 중요한 통찰력을 제공합니다.



### DamFormer: Generalizing Morphologies in Dam Break Simulations Using Transformer Mod (https://arxiv.org/abs/2410.18998)
- **What's New**: 이번 연구에서는 DamFormer라는 새로운 transformer 기반 모델을 소개하여, 댐 붕괴와 같은 구조적 경계와 파도의 상호 작용을 학습하고 시뮬레이션하는 방법을 제시합니다. 이 모델은 원형, 삼각형 및 사각형과 같은 다양한 구조 형태에서 동적 파면 변화를 탐구합니다.

- **Technical Details**: DamFormer 모델은 transformer의 인코더를 이용하여 긴 데이터 시퀀스의 역학을 학습합니다. 이 모델은 초기 조건을 입력으로 받고, 물리적 모델을 기반으로 하여 수심, 유속 및 중력 가속도를 고려한 천연수역 방정식(shallow water equations)을 사용합니다.

- **Performance Highlights**: DamFormer는 삼각형, 원형, 사각형의 형상에 대해 학습(learning)을 진행하며, 특히 삼각형 조건에서도 우수한 성능을 유지하는 것을 목표로 합니다. 이 모델은 댐 붕괴 발생 시 물리적 경계 및 상호 작용을 예측하는 데 있어 높은 정확성과 일반화 능력을 제공합니다.



### Deterministic Fokker-Planck Transport -- With Applications to Sampling, Variational Inference, Kernel Mean Embeddings & Sequential Monte Carlo (https://arxiv.org/abs/2410.18993)
Comments:
          28 pages, 6 figures, 1 table

- **What's New**: 이번 논문에서는 Fokker-Planck 방정식(Fokker-Planck equation)을 연속 방정식(continuity equation)으로 재구성하는 방법을 제안하고, 이를 통해 입자 흐름 방법(particle flow methods)에서 관련된 속도 필드를 활용하는 방안을 모색합니다.

- **Technical Details**: 입자 흐름 방법에서는 다수의 입자를 통해 목표 확률 밀도인 𝜌ₜ를 근사하며, 이러한 확률 밀도를 다루기 위해 확률 흐름 ODE(probability flow ODE)라는 결정론적 입자 동역학을 소개합니다. 이 ODE는 𝜌ₜ를 목표 밀도 𝜌ₜₐₗ로 기하급수적으로 안내하는 방식으로 작동하며, 속도 필드(vᵗᶻ) 및 Kullback-Leibler 발산(Kullback-Leibler divergence)을 최소화하는 데 집중합니다.

- **Performance Highlights**: 핵심 결과로는 속도 필드가 목표 밀도에 대한 로그의 기울기만을 요구하여, 실제의 밀도 𝜌ₜ를 계산하는 것이 불가능한 높은 차원에서도 유용하다는 점이 강조됩니다. 이는 베이지안 추론(Bayesian inference) 등에 적합하며, 다양한 사전 분포(prior distribution)와의 연결을 통해 더욱 효율적인 샘플링을 가능하게 합니다.

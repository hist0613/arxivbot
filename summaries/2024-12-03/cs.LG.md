...
### Scalable Out-of-distribution Robustness in the Presence of Unobserved Confounders (https://arxiv.org/abs/2411.19923)
Comments:
          24 pages, 3 figures

- **What's New**: 이 논문은 관찰되지 않은 혼란변수(confounder) Z로 인해 발생하는 분포적 이동(out-of-distribution, OOD) 일반화 문제를 다룹니다. 기존의 접근과는 달리 본 연구에서는 단 하나의 추가 변수만을 요구하며, 혼란변수가 훈련 중 관측되지 않기 때문에 OOD 일반화 문제의 해결 방안을 모색합니다. 접근 방식으로는 간단한 정규화를 도입하여 예측기의 효율성을 크게 향상시킵니다.

- **Technical Details**: 이 연구는 인코더-디코더 네트워크를 활용하여 공변량(covariates) X 주어진 혼란변수 Z의 후속 분포를 학습합니다. 이후 혼란변수 및 공변량에 대한 mixture-of-experts 모델을 훈련하여 예측을 수행하며, 이는 복잡한 모델 없이도 효율적으로 동작합니다. 본 연구의 새로운 정규화 방법은 모형의 수렴을 보장하기 위한 핵심 요소로 작용합니다.

- **Performance Highlights**: 제안된 접근 방식은 단순성과 확장성에도 불구하고 기존의 방법들에 비해 더 뛰어난 성능을 보입니다. 기준선 방법들에 비해, 이 모델은 혼란변수의 동적 변화나 환경적인 변화에 대해 높은 일반화 성능을 나타냅니다. 또한, 논문에서 언급된 실험 결과는 제안된 방법의 효과를 명확히 보여줍니다.
...
### Open source Differentiable ODE Solving Infrastructur (https://arxiv.org/abs/2411.19882)
- **What's New**: 이 논문은 GPU 가속 Ordinary Differential Equations (ODE) 솔버를 오픈소스 DeepChem 프레임워크에 통합하여 복잡한 시스템 모델링을 보다 쉽게 접근할 수 있도록 하였습니다. 이 솔버는 여러 수치적 방법을 지원하며, 전체적으로 미분 가능하여 복잡한 프로그램에 쉽게 통합할 수 있습니다. 연구자들은 Lotka-Volterra 포식-피식자 동역학, 약리학적 compartment 모델, 신경망 ODE 및 반응-확산 방정식을 통해 구현의 능력을 입증하였습니다.

- **Technical Details**: ODE는 물리학, 화학, 생물학 등 다양한 분야에서 동적 시스템을 모델링하는 데 사용됩니다. 본 작업에서는 DeepChem의 기능을 확장하여 약리학 모델링에 초점을 맞춘 GPU 가속 ODE 해결 인프라를 통합하였습니다. 우리의 기여는 파라미터 추정 및 시뮬레이션을 위한 새로운 최적화 원리를 포함하며, 이를 통해 연구자들은 약물 수송 및 동역학을 효율적으로 모델링할 수 있습니다.

- **Performance Highlights**: 우리가 개발한 솔버는 평균 제곱 오차(mean squared errors)가 $10^{-4}$에서 $10^{-6}$ 사이의 높은 정밀도를 달성했습니다. 또한 최대 100개의 compartment가 포함된 큰 시스템을 해결하는 데 있어 확장성을 보여주었습니다. 이 솔버는 약리학적 모델링 및 데이터 집약적인 연구를 위한 효율적인 도구로 자리잡을 것입니다.
...
### GradAlign for Training-free Model Performance Inferenc (https://arxiv.org/abs/2411.19819)
- **What's New**: 최근에 소개된 GradAlign은 모델 성능을 훈련 없이 추론할 수 있도록 설계된 간단하면서도 효과적인 방법입니다. 이 방법은 초기화 과정에서 각 샘플의 gradient에 존재하는 갈등 정도를 정량화하며, 이러한 갈등이 모델 수렴을 방해하고 성능 저하를 초래한다는 사실에 기반합니다. GradAlign은 기존의 훈련 없는 NAS 기법과 비교하여 더 나은 성과를 보여주며, 널리 사용되는 지표인 선형 영역 수가 초기화 시 네트워크 아키텍처 선택의 신뢰할 수 있는 기준이 아니라고 주장합니다.

- **Technical Details**: 이 논문에서는 GradAlign을 통해 각 샘플별 gradient의 갈등을 정량화합니다. 두 가지 전략을 통해 이 갈등을 측정하는데, 첫 번째는 각 샘플의 gradient를 평균 gradient와 정렬하는 것이며, 두 번째는 per-sample gradient에서 도출된 gram 행렬의 행렬식을 계산하는 것입니다. 이 연구는 GradAlign의 성능을 NAS201, NAS101, NDS와 같은 표준 NAS 벤치마크와 비교하여 일반적으로 기존의 훈련 없는 NAS 방법보다 우수한 성능을 보인다는 것을 입증했습니다.

- **Performance Highlights**: GradAlign은 모델 초기화 시의 성능 추정에서 효율적이며, 기존의 훈련 없는 NAS 기술에 비해 전반적으로 더 나은 성과를 나타냅니다. 연구 결과는 per-sample gradient의 갈등이 모델 수렴에 미치는 영향이 크고, 네트워크 아키텍처 초기화 시 단순히 선형 영역 수를 세는 것이 신뢰할 수 있는 평가 방법이 아님을 보여줍니다. 이러한 발견은 훈련 없는 NAS 방법의 성능 평가 기준에 새로운 통찰력을 제공합니다.



### Rethinking the initialization of Momentum in Federated Learning with Heterogeneous Data (https://arxiv.org/abs/2411.19798)
- **What's New**: 본 연구에서는 데이터 이질성(data heterogeneity) 문제를 해결하기 위해 새로운 방법인 Reversed Momentum Federated Learning (RMFL)을 제안합니다. 기존의 모멘텀(cumulation) 기법이 비효율적이라는 점을 지적하며, 모멘텀을 축적할 때 최근 그라디언트(gradients)에 지나치게 편향되는 문제를 해결하고자 합니다. RMFL은 시간이 지남에 따라 그라디언트에 지수적으로 감소된 가중치를 부여하여 이질적 데이터를 고려한 보다 최적화된 방법을 제시합니다.

- **Technical Details**: Federated Learning (FL)의 새로운 두 단계 훈련 프로세스에서 RMFL 기법은 각 클라이언트의 로컬 트레이닝을 글로벌 모멘텀(global momentum)을 기반으로 초기화합니다. RMFL은 데이터 이질성의 영향을 줄이기 위해, 기존의 모멘텀 누적(cumulation) 방식을 거부하고 기울기의 평균을 이용해 경량화한 지수 가중치를 할당합니다. 이러한 접근 방식은 각 로컬 트레이닝 단계에서 클라이언트 간의 이질성을 효과적으로 반영하며, 보다 일관된 모멘텀 값을 유지합니다.

- **Performance Highlights**: 다양한 이질성 수준을 가진 세 가지 벤치마크 데이터셋에서 RMFL의 효과를 평가한 결과, RMFL 기법이 기존 모멘텀 기반 최적화 기법들보다 더 나은 성능을 보이는 것으로 나타났습니다. 이 연구를 통해 RMFL이 글로벌 모델의 정확도 및 수렴 속도를 더욱 향상시킬 수 있음을 시사합니다. 최종적으로, RMFL은 클라이언트 간의 이질성 문제를 해결하는 데 중요한 역할을 할 것으로 기대됩니다.



### Tractable Agreement Protocols (https://arxiv.org/abs/2411.19791)
- **What's New**: 본 논문에서는 어떤 기계 학습 알고리즘도 상호작용 프로토콜로 변환할 수 있는 효율적인 감소 방식을 제시합니다. 이를 통해 인간과 같은 다른 당사자와 협업하여 예측에 대한 합의를 도출하고 정확성을 향상시킬 수 있습니다. 이 접근법은 각 당사자에 대해 Bayesian rationality의 계산적으로 통계적으로 다루기 쉬운 완화 조건을 부과하며, 이는 선험적 정보가 없는 환경에서도 합리적입니다.

- **Technical Details**: 프로토콜에서 모델은 먼저 예측을 제시하며, 인간은 동의하거나 피드백을 제공함으로써 응답합니다. 모델은 자신의 상태를 업데이트하고 예측을 수정하며, 인간은 자신의 신념을 조정할 수 있습니다. 이 반복 과정은 두 당사자가 합의에 도달할 때까지 계속됩니다. 특히, 한 차원 기대값에 대한 합의를 목표로 하는 설정에서 Aumann의 합의 정리를 확장하여 분석하고, 약한 가정 하에서 Aaronson의 수렴 정리를 회복합니다.

- **Performance Highlights**: 두 개 이상의 당사자 간의 시나리오로 일반화할 수 있으며, 이 경우 계산 복잡성은 참가자 수에 비례하여 선형적으로 증가합니다. 제안된 프로토콜은 간단하고 효율적인 조건에 의존하여, 각 개별 당사자가 가진 예측보다 더 높은 정확성을 달성하는 예측을 생성합니다. 이 방법은 특히 피드백 메커니즘을 통해 통계적 신뢰도를 개선하는 데 기여합니다.
...
### Riemannian Denoising Score Matching for Molecular Structure Optimization with Accurate Energy (https://arxiv.org/abs/2411.19769)
- **What's New**: 이 연구는 높은 에너지 정확도를 가진 분자 구조 생성을 목표로 한 수정된 score matching 방법을 제안합니다. 전통적인 방법들과는 달리 물리학적으로 유도된 내부 좌표를 기반으로 하는 Riemannian score matching 접근 방식을 도입하여 에너지 지형을 모방합니다. 이러한 새로운 접근법은 특히 계산 화학 분야에서 다양한 응용 가능성을 열어줍니다.

- **Technical Details**: 제안된 Riemannian score matching 방법은 분자 구조를 정의된 매니폴드 위에서 표현합니다. 이를 통해 에너지 경관을 효율적으로 모사하게 되며, 구조가 노이즈 오염 및 정화 과정을 통해 생성됩니다. Gaussian perturbation을 이용한 훈련 과정은 실제 potential energy surface의 기울기에 가까운 score function을 학습해 분자 구조를 정확하게 예측하도록 합니다.

- **Performance Highlights**: 제안된 접근 방식은 QM9 및 GEOM 데이터셋에서 여러 초기 구조를 정제하는 과정에서 높은 정확도를 달성하였습니다. Riemannian 모델은 화학적 정확도 범위 내에서 평균 에너지 오차 0.177 kcal/mol과 RMSD 0.031 Å을 기록하며 우수한 성능을 입증하였습니다. 이러한 결과는 분자 모델링에서 실제 응용에 필수적인 높은 에너지 정확도를 달성할 수 있는 가능성을 보여줍니다.
...
### A Note on Small Percolating Sets on Hypercubes via Generative AI (https://arxiv.org/abs/2411.19734)
- **What's New**: 이번 연구에서는 PatternBoost라는 생성적 AI 패턴 인식 기법을 사용하여 하이퍼 큐브에서 부트스트랩 퍼콜레이션(bootstrap percolation)을 연구하여 기존의 최고의 상한식을 소폭 개선하였습니다. 부트스트랩 퍼콜레이션은 그래프 G에서 "감염된" 정점의 초기 집합으로 시작하며, 각 단계마다 감염된 이웃이 r개 이상인 정점도 감염됩니다.

- **Technical Details**: 부트스트랩 퍼콜레이션의 핵심 문제는 전체 그래프가 감염되도록 만드는 초기 집합의 최소 크기를 결정하는 것입니다. 본 논문에서는 d차원 하이퍼큐브에서 r-이웃 부트스트랩 퍼콜레이션 과정을 분석하며, r이 양의 정수일 때 이 과정을 다룹니다. 연구 결과는 M(G, r)로 표시된 초기 집합의 크기에 대한 새로운 상한식을 제공합니다.

- **Performance Highlights**: 최신 연구에서 제시된 주요 결과는 M(G, r)에 대한 계산을 통해 생성된 부트스트랩 감염 집합의 크기를 개선하였음을 보여줍니다. 이러한 새로운 구조는 기존의 격차를 줄여 상한식을 향상시켰으며, 이는 추가적인 연구와 발전 가능성을 제시합니다. 또한, 강력한 예시와 함께 개선된 알고리즘을 통해 더 나은 성능이 확인되었습니다.



### Risk-Averse Certification of Bayesian Neural Networks (https://arxiv.org/abs/2411.19729)
- **What's New**: 이번 연구에서는 실세계 환경의 복잡하고 역동적인 특성을 반영하여, 딥 러닝 모델의 강건성 평가에 필수적인 위험 측정을 통합하는 방법을 제안합니다. 특히 Bayesian neural networks (BNN)을 위한 새로운 Risk-Averse Certification 프레임워크인 RAC-BNN을 제시하여, 강건성 평가의 새로운 기준을 설정했습니다.

- **Technical Details**: RAC-BNN은 샘플링(sampling)과 최적화(optimisation)를 활용하여 BNN의 출력 집합(output set)을 근사적으로 계산합니다. 이를 위해 템플릿 폴리토프(template polytopes)로 표현되는 출력 집합을 사용하고, Conditional Value at Risk (CVaR)라는 일관된 왜곡 위험 측정을 통합해 실제 분포(empirical distributions)에 기반한 확률적 보장을 제공합니다.

- **Performance Highlights**: RAC-BNN은 다양한 회귀(regression) 및 분류(classification) 벤치마크에서 검증되었으며, 최신 방법과의 성능 비교를 통해 그 효과를 입증했습니다. 연구 결과, RAC-BNN은 최악의 위험 시나리오에서 강건성을 효과적으로 정량화하며, 복잡한 작업에서 더 조밀한 인증 한계와 높은 효율성을 달성했습니다.
...
### Relative Representations of Latent Spaces enable Efficient Semantic Channel Equalization (https://arxiv.org/abs/2411.19719)
- **What's New**: 이 논문은 서로 다른 언어를 사용하는 에이전트 간의 통신을 가능하게 하는 새로운 semantic equalization 알고리즘을 제안합니다. 기존의 방식은 추가적인 재훈련을 요구했으나, 본 알고리즘은 이를 필요로 하지 않습니다. 제안된 방법은 다양한 neural network 모델을 사용하는 에이전트 간의 통신을 통합된 표현 공간으로 변환하여 의미를 더 효율적으로 전달합니다.

- **Technical Details**: 제안된 알고리즘은 상대적 표현(relative representations)에 기반하여 서로 다른 neural network의 잠재 벡터(latent vectors)를 공동 공간으로 사영(projection)합니다. 이 과정에서 데이터 샘플 집합을 기반으로 한 앵커(bool anchors)들을 사용하며, 각각의 에이전트는 이 상대적 공간 내에서 샘플링된 의미 기호(semantic symbols)를 통해 소통합니다. 기존의 기술과 달리, 제안된 접근 방식은 재훈련의 필요 없이 에이전트 간의 의미 표현을 통합하는 데 중점을 둡니다.

- **Performance Highlights**: 실험 결과, 본 논문에서 제안한 방법은 서로 다르게 훈련된 모델 간에도 원활한 통신을 가능하게 하였습니다. 특히 neural network 아키텍처와 초기 훈련에 사용된 데이터셋에서 차이가 있는 경우에도 성능을 유지했습니다. 새로운 앵커 선택 알고리즘인 prototypical anchors를 적용하여 기존 방법들보다 더 높은 성능을 입증하였습니다.



### Fast Mutual Information Computation for Large Binary Datasets (https://arxiv.org/abs/2411.19702)
- **What's New**: 이 연구에서는 대규모 데이터 세트에서 상호 정보(Mutual Information, MI) 계산을 가속화하는 새로운 행렬 기반 알고리즘을 제안합니다. 이 알고리즘은 측정된 두 확률 변수 사이의 정보를 효율적으로 계산할 수 있도록 설계되어, 전통적인 쌍wise 계산 방식을 대체합니다. 제안된 방법은 벡터화된 연산을 활용하여 복수의 변수 쌍에 대해 동시에 MI를 계산할 수 있게 합니다.

- **Technical Details**: 제안된 알고리즘은 두 가지 주요 부분으로 나뉘어 있으며, 첫 번째 부분은 행렬 연산을 이용하여 MI 계산을 벡터화합니다. 두 번째 부분은 계산 중 발생하는 불필요한 중복을 줄이기 위한 최적화를 탐구합니다. 이로 인해 모든 쌍의 이진 변수에 대한 MI 계산이 단 하나의 행렬 곱셈으로 수행될 수 있습니다.

- **Performance Highlights**: 실험 결과에 따르면, 최적화된 구현을 사용할 경우 최대 50,000배의 계산 시간 단축이 가능하였으며, 이는 특히 하드웨어 최적화 프레임워크를 사용할 때 두드러진 성과입니다. 이 접근법은 MI의 계산 가능한 범위를 확대하여 데이터 기반 연구에 대한 적용 가능성을 높이는데 기여할 것입니다.



### Privacy-Preserving Orthogonal Aggregation for Guaranteeing Gender Fairness in Federated Recommendation (https://arxiv.org/abs/2411.19678)
Comments:
          accepted by WSDM 2025

- **What's New**: 이번 연구에서는 연합 추천 시스템(Federated Recommendation Systems, FRS)에서 그룹 공정성을 달성하는 데 있어서의 한계를 탐구합니다. 기존의 연구들은 성별 공정성과 같은 문제를 다루지 못했으며, 특히 데이터 불균형과 같은 현상이 성별에 따른 불공정성을 초래하는 원인으로 작용할 수 있음을 강조합니다. 저자들은 개인 정보를 보호하는 동시에 소수 집단의 선호도를 유지하기 위해 Privacy-Preserving Orthogonal Aggregation (PPOA)라는 새로운 방법론을 제안합니다.

- **Technical Details**: 연구는 PPOA를 통해 사용자들이 각자의 그룹에 대한 모델 집합 결과를 받을 수 있도록 하여 서로 다른 선호도를 유지하게끔 합니다. 이 방법은 Secure Aggregation과 Quantization 기법을 활용하여 다수의 정보가 소수 집단의 정보를 억압하지 않도록 설계되었습니다. 또한, PPOA는 모델 훈련 과정에서 소수 집단의 선호도를 보호하고, 학습 결과의 편향을 최소화하기 위한 새로운 접근 방식을 제공합니다.

- **Performance Highlights**: 실험 결과, PPOA는 실제 데이터셋에서 남성과 여성 각각에 대해 추천 효과성을 최대 8.25%와 6.36% 향상시키고, 전체적으로는 최대 7.30%의 개선을 달성하였습니다. 저자들은 PPOA가 대부분의 경우 최적의 공정성을 달성하였음을 밝혔으며, 다양한 성별 그룹의 선호도를 잘 유지함을 보여줍니다. 이와 같은 결과는 더 나은 그룹 공정성을 달성하는 데 중요한 기여를 하고 있음을 나타냅니다.



### On the Performance Analysis of Momentum Method: A Frequency Domain Perspectiv (https://arxiv.org/abs/2411.19671)
- **What's New**: 이 논문에서는 Momentum 기반 옵티마이저의 최적 모멘텀 계수 선택의 어려움을 극복하기 위해 Frequency Stochastic Gradient Descent with Momentum (FSGDM)이라는 새로운 휴리스틱 옵티마이저를 제안합니다. 이 방법은 Gradient의 시간 변화를 고려하여 모멘텀 필터링 특성을 동적으로 조정합니다. 기존의 모멘텀 옵티마이저보다 성능이 우수하다는 실험 결과도 포함되어 있습니다.

- **Technical Details**: 이 논문은 Z-transform을 기반으로 한 Frequency Domain Analysis Framework를 통해 모멘텀 메소드의 작동 원리를 분석합니다. 이를 통해 학습 프로세스 동안 일정한 주파수 대역의 Gradient 성분을 증폭하거나 감쇠시키는 방식을 설명하였습니다. 또한, 다양한 모멘텀 메서드 간의 관계를 밝혀내어, 이에 기반한 실용적인 최적화 설계를 제안합니다.

- **Performance Highlights**: FSGDM 옵티마이저는 실험을 통해 기존의 SGD 기반 모멘텀 옵티마이저들보다 뛰어난 성능을 보였습니다. 특히, 학습 초반에는 원본 Gradient를 보존하고, 후반에는 저주파 성분을 점진적으로 강화하는 과정이 일반화 성능을 향상시키는 데 긍정적인 역할을 한다는 것을 확인하였습니다. 이러한 결과는 모멘텀 계수를 적절히 조정함으로써 얻어진 것입니다.
...
### Learned Random Label Predictions as a Neural Network Complexity Metric (https://arxiv.org/abs/2411.19640)
- **What's New**: 본 논문에서는 클래스 레이블과 무작위로 생성된 레이블을 병렬 학습하는 것이 딥 뉴럴 네트워크의 암기(memorization), 모델 복잡도(model complexity), 일반화(generalization)에 미치는 영향을 실증적으로 조사하였습니다. 새로운 멀티 헤드 네트워크 아키텍처를 제안하여 기존 CNN 아키텍처를 확장하고, 불필요한 샘플의 암기를 방지하는 방법을 모색합니다. 이 연구는 Rademacher 복잡도를 기반으로 하여 일반적인 정규화 기법의 효과를 분석하고, CNN의 특성 추출(feature extraction)과 분류(classification) 과정에 대한 기존 이해에 도전합니다.

- **Technical Details**: 새로운 네트워크 아키텍처는 각 클래스마다 무작위 레이블을 예측하는 추가적인 헤드를 도입하여, 클래스 레이블과 무작위 레이블을 동시에 학습합니다. 이 구조는 무작위 레이블의 예측을 위한 손실(loss)을 클래스 예측 손실과 충돌하지 않게 조절할 수 있도록 설계되었습니다. 이 모델은 Kronecker 델타와 여러 종류의 손실 함수를 사용하여 훈련되며, 무작위 레이블의 예측 정확도를 높이는 것이 아니라 일반화를 향상시키는 것을 목표로 합니다.

- **Performance Highlights**: WideResNet-16-4 아키텍처를 사용하여 CIFAR100 데이터셋에서 실험을 진행했으며, 무작위 예측 헤드의 정확성을 바탕으로 복잡도 메트릭을 정의했습니다. 정규화가 적용되지 않은 경우, 즉 무작위 예측 헤드는 기본 네트워크에 영향을 주지 않지만 메트릭으로서의 작용을 하였습니다. 주요 결과로, 제안된 정규화 기법이 암기를 효과적으로 줄였음에도 불구하고 일반화 성능이 개선되지 않았다는 점에서 기존의 통계 학습 이론이 제시하는 기억과 일반화 간의 인과 관계에 의문을 제기합니다.



### Enhancing Sentiment Analysis in Bengali Texts: A Hybrid Approach Using Lexicon-Based Algorithm and Pretrained Language Model Bangla-BER (https://arxiv.org/abs/2411.19584)
Comments:
          13 pages, 12 figures

- **What's New**: 본 연구는 감정 분석(sentiment analysis) 분야에서 벵갈리(Bengali) 언어에 대한 연구가 제한적이라는 점을 지적하며, 자체적으로 구축한 15,000개의 수작업으로 레이블링된 리뷰 데이터셋을 바탕으로 벵갈리 텍스트의 감정을 분석하기 위한 새로운 알고리즘인 Bangla Sentiment Polarity Score (BSPS)를 소개합니다. 이 알고리즘은 감정 점수를 생성하고, 아홉 가지 심층 감정 카테고리로 분류하여 기존의 이진 또는 삼진 분류 방식을 넘어서는 것이 특징입니다. 또한, 기존의 BanglaBERT 모델과의 결합을 통해 벵갈리 언어에서의 감정 분석의 효과성을 입증했습니다.

- **Technical Details**: 이 연구는 규칙 기반(rule-based) 알고리즘과 사전 훈련된(pre-trained) 언어 모델을 통합한 하이브리드(hybrid) 접근 방식을 활용하였습니다. BSPS 알고리즘은 벵갈리 리뷰에 대한 감정 점수를 생성하며, 이를 통해 사용자 감정의 복잡성을 더 잘 이해할 수 있게 합니다. 연구팀은 또한 BanglaBERT라는 사전 훈련된 트랜스포머 기반 언어 모델을 사용하여 이 알고리즘의 능력을 검증하였고, 감정 분류를 아홉 개의 카테고리로 수행했습니다.

- **Performance Highlights**: BSPS와 BanglaBERT를 결합한 하이브리드 접근 방식은 단독으로 사용된 BanglaBERT 모델보다 더 높은 정확도와 정밀도를 달성했습니다. 본 연구에서 제안된 방법론은 벵갈리 감정 분석의 정확성과 디테일을 크게 향상시킬 뿐만 아니라, 유사한 언어 복잡성을 가진 다른 언어를 위한 미래 연구의 방향성도 제시합니다. 전반적으로, 이 연구는 벵갈리 언어의 감정 분석에 있어 규칙 기반 접근 방식과 사전 훈련된 언어 모델의 통합이 크게 기여할 수 있음을 강조하고 있습니다.
...
### Differentiable Causal Discovery For Latent Hierarchical Causal Models (https://arxiv.org/abs/2411.19556)
Comments:
          25 pages with references, 7 figures

- **What's New**: 이 논문에서는 관찰 데이터로부터 비선형 잠재 계층 인과 모델(non-linear latent hierarchical causal models)의 인식 가능성(identifiability)에 대한 새로운 이론적 결과를 제시합니다. 기존 연구의 결정적(latent) 변수와 외생(또는 부가적) 노이즈에 대한 가정에서 벗어나, 비선형 잠재 계층 모델에 대한 인식 가능성을 확립합니다. 또한 이 연구는 차별화 가능한(differentiable) 인과 발견(causal discovery) 알고리즘을 제안하여, 이러한 모델의 구조를 효율적으로 추정할 수 있습니다.

- **Technical Details**: 이 연구는 관찰 데이터로부터 비선형 잠재 계층 모델의 인식 가능성을 증명하는 것뿐만 아니라, 새로운 차별적 인과 발견 방법을 도입합니다. 이 방법은 기존의 반복적인 구조 학습 접근법이 아니라, 연속 최적화 문제로 인과 발견 문제를 정식화하여 인과 구조와 같은 요구 사항을 동적으로 포함합니다. 이를 통해 오류 전파 및 테스트 순서에 대한 민감성을 줄이고, 더 나아가 모델의 성능을 향상시킬 수 있습니다.

- **Performance Highlights**: 제안된 차별적 인과 발견 방법은 기존 방법에 비해 정확도와 확장성에서 우수한 결과를 도출합니다. 실험을 통해 고차원 이미지 데이터로부터 해석 가능한 계층적(latent hierarchical) 구조를 학습하며, 이는 실제 적용 가능성을 입증하는 사례입니다. 본 방법은 데이터의 복잡한 의존성을 이해하고 처리하는 데 도움을 줄 수 있습니다.



### Analysis of High-dimensional Gaussian Labeled-unlabeled Mixture Model via Message-passing Algorithm (https://arxiv.org/abs/2411.19553)
- **What's New**: 이 논문에서는 반지도 학습(SSL)에서 고차원 가우시안 혼합 모형(Gaussian Mixture Model, GMM)의 특성을 심층 분석하며, 특히 이론적 및 실험적 결과를 통한 효율성을 검증합니다. 다양한 추정 기법 중에서도 Bayesian 방식과 ℓ2 정규화된 최대 우도 추정(maximum likelihood estimation, RMLE) 방식의 성능을 비교하여 SSL의 이해를 넓히고자 합니다. RMLE는 불라벨 데이터의 양이 많을 때에도 거의 최적의 성능을 달성할 수 있음을 밝혀내어, 이에 대한 정량적 근거를 제공합니다.

- **Technical Details**: 연구에서는 두 가지 추정 접근법인 Bayesian 접근법과 ℓ2-정규화 RMLE 접근법을 다루며, 각 접근법의 전 세계적인 단계 다이어그램(global phase diagram)과 매개변수의 추정 오류, 라벨의 예측 오류 등을 비교합니다. 특히 RMLE는 Bayesian 방법과 유사하지만, 높은 차원에서 처리할 수 있는 실용적인 방법으로서의 가치를 입증합니다. 또한, 이러한 접근법은 근본적으로 통계 역학에서 유래한 근사 메시지 전송(approximate message passing, AMP) 및 상태 진화(state evolution, SE) 방법론을 포함하여 연구됩니다.

- **Performance Highlights**: 실험 결과, 정규화를 적절히 적용한 RMLE는 추정 오류 및 예측 오류 측면에서 거의 최적의 성능을 보였으며, 이는 불라벨 데이터가 많을 때 더욱 두드러졌습니다. 연구 결과는 비구조적 데이터에서도 GMM이 반지도 학습에서 우수한 성능을 제공함을 입증하며, 가우시안 혼합 모델을 기반으로 한 이러한 반지도 학습 접근법이 미래의 다양한 응용에 기여할 가능성을 제시합니다.



### Development of Low-Cost IoT Units for Thermal Comfort Measurement and AC Energy Consumption Prediction System (https://arxiv.org/abs/2411.19536)
Comments:
          RoomVent2024 conference

- **What's New**: 일본 정부는 2019년에 큰 에너지 소비 문제를 해결하기 위해 BI-Tech (Behavioral Insights X Technology) 프로젝트를 시작했습니다. 이 연구는 중소형 사무실 건물에 초점을 맞추어, AI와 IoT 기술을 활용한 저비용의 IoT 기반 BI-Tech 시스템을 도입했습니다. 이 시스템은 실시간으로 실내 열 조건과 에어컨의 설정 온도를 모니터링합니다.

- **Technical Details**: Raspberry Pi 4B+ 플랫폼을 사용하여 데이터를 수집하고, 머신 러닝(machine learning)과 이미지 인식(image recognition) 기술로 PMV index를 계산하며 온도 조정에 따른 에너지 소비 변화를 예측합니다. 이 시스템은 사용자가 쉽게 접근할 수 있도록 모바일 및 데스크탑 애플리케이션을 통해 정보를 제공합니다. 이와 같은 기술적 접근으로 에너지 절약 행동 수정이 장려됩니다.

- **Performance Highlights**: 머신 러닝 모델은 R² 값이 97%에 달하여 사용자의 에너지 절약 습관을 촉진하는 시스템의 효율성을 입증했습니다. 이는 건물의 에너지 효율성을 높이고, 사용자에게 더욱 동기부여를 주는 긍정적인 결과를 가져옵니다.
...
### Graph-Enhanced EEG Foundation Mod (https://arxiv.org/abs/2411.19507)
- **What's New**: 이번 연구에서는 EEG 분석을 위한 새로운 기초 모델을 제안합니다. 기존 모델이 시간적 동향(temporal dynamics)에만 초점을 맞추는 반면, 본 연구는 시간적 정보와 채널 간 관계(inter-channel relationships)를 모두 통합하여 EEG 신호를 보다 효과적으로 이해할 수 있도록 합니다. 이 모델은 Graph Neural Networks (GNNs)를 사용하여 채널 간 복잡한 상호작용을 포착할 수 있는 능력을 갖추고 있으며, 이를 통해 EEG 분석의 진전을 꾀하고자 합니다.

- **Technical Details**: 제안된 모델은 GNN과 Transformer 기반의 아키텍처를 통합하여 EEG 데이터를 그래프 구조로 표현합니다. 각 EEG 채널은 노드로, 해당 채널의 신호는 노드 피쳐로, 채널 간의 연결은 엣지로 모델링됩니다. 이 접근 방식은 다중 데이터 세트에 적합하게 적용될 수 있도록 하기 위해 EEG 신호의 길이를 표준화하는 메커니즘도 도입하였습니다.

- **Performance Highlights**: 세 가지 하류(task)에서 제안한 모델의 성능을 평가한 결과, 최적화된 구성을 가진 GCN 아키텍처를 사용할 때 모든 작업에서 기존의 방법들을 일관되게 초월하는 성과를 보였습니다. 이러한 결과는 제안된 모델이 EEG 분석을 위해 안정적이고 유용한 기초 모델로 확립될 수 있음을 시사합니다.
...
### Random Feature Models with Learnable Activation Functions (https://arxiv.org/abs/2411.19468)
- **What's New**: 이번 연구에서는 Learnable Activation Functions (RFLAF)를 사용하는 랜덤 특징 모델을 제안합니다. 이는 기존의 랜덤 특징 모델의 표현력(expresivity)을 크게 향상시키며, 전통적인 모델보다 더 나은 해석 가능성(interpretability)을 제공합니다. 저자들은 RFLAF가 고정된 활성화 함수를 사용하는 기존 모델의 한계를 극복할 수 있음을 강조합니다.

- **Technical Details**: RFLAF 제안 모델은 가중 합(weighted sums)으로 구성된 Radial Basis Functions (RBFs)에 의해 학습 가능한 활성화 함수를 매개변수화합니다. 이는 모델이 비선형 학습 문제를 해결하는 데 효과적일 뿐만 아니라, 새로운 해석 가능한 커널(kernel)의 분석 가능성을 제공합니다. RFLAF는 이론적으로도 전통적인 랜덤 특징 모델에 비해 약 두 배의 파라미터 수로도 강력한 표현 능력을 발휘할 수 있음을 증명합니다.

- **Performance Highlights**: 실험 결과, RFLAF는 동일한 파라미터 수를 가진 전통적인 랜덤 특징 모델보다 다양한 작업에서 더 나은 성능을 보입니다. 최적화된 가중치는 학습된 활성화 함수를 직접 추론 가능하게 하여 모델의 해석 가능성을 증가시킵니다. 궁극적으로, RFLAF는 랜덤 특징 모델 내에서 더 많은 표현성과 해석 가능성을 갖춘 프레임워크를 개발하는 데 기여하는 바가 큽니다.



### Multi-task CNN Behavioral Embedding Model For Transaction Fraud Detection (https://arxiv.org/abs/2411.19457)
Comments:
          7 pages, 2 figures, ICDMW 2024

- **What's New**: 이번 논문에서는 거래 사기 탐지를 위한 다중 작업 CNN 행동 임베딩 모델(MTCNN)을 제안합니다. LSTM 및 Transformer 모델보다 성능이 뛰어난 단일 레이어 CNN 설계와 다중 범위 커널을 특징으로 하여 확장성과 도메인 중심의 유도 편향을 향상시키고 있습니다. 추가적으로, CNN에 위치 인코딩을 통합하여 시간이 중요한 시퀀스 신호를 도입하고, 무작위 레이블 가중치를 사용하여 수동 조정의 필요성을 제거하는 멀티태스킹 학습을 구현하는 데 초점을 맞추고 있습니다.

- **Technical Details**: MTCNN 모델은 사용자 페이지 뷰 시퀀스를 포함한 다변량 시계열(MTS) 데이터를 기반으로 설계되었습니다. 각 시퀀스는 페이지 ID, 아이템 카테고리 및 페이지 뷰 시간을 나타내는 세 가지 변수를 포함합니다. 다양한 변수 유형을 통합하기 위해, 우리는 카테고리 변수에 대한 전통적인 임베딩 방법을 적용하고, 연속 변수에 대해서는 로그 정규화를 사용하여 임베딩을 수행합니다.

- **Performance Highlights**: 실제 산업 데이터 테스트 결과, MTCNN 모델은 TST 모델과 유사한 경쟁력을 가지고 있으며, 다른 Transformer 모델 대비 성능이 우수함을 입증하고 있습니다. 이는 사기 행위를 탐지하는 데 있어 모델의 일반화 능력을 증대시키고, 과적합 문제를 완화시키는 데 기여합니다. 또한, 본 연구는 복잡한 거래 데이터와 관련된 다양한 과제를 동시에 모델링 하는 효과적인 방법론을 제시하고 있습니다.
...
- **What's New**: 이 논문은 기존의 HiPPO 프레임워크를 발전시켜 입력 시퀀스의 자기상관(autocorrelation)이 SSM 파라미터 초기화에 미치는 영향을 분석합니다. 저자들은 SSM의 시계열(timescale)과 상태 행렬(state matrix)의 고유값(eigenvalues) 설정이 모델의 성능에 어떠한 영향을 미치는지를 연구합니다. 이를 통해, 적절한 시계열 설정이 초기화에서 안정성을 유지하면서 메모리의 저주(curse of memory)를 완화할 수 있음을 보여줍니다.

- **Technical Details**: SSM 레이어는 연속 시간의 상미분 방정식(ordinary differential equation)을 기반으로 하며, 입력 시퀀스와 출력 시퀀스를 처리합니다. 이 논문에서는 이상적인 상태 행렬 W가 복소수 값으로 초기화될 때, 그 실수부를 제로로 설정하는 방법을 제안합니다. 이를 통해 SSM의 안정성을 유지하면서도 학습 과정에서의 수렴 속도를 향상시킬 수 있음을 분석하였습니다.

- **Performance Highlights**: 저자들은 다양한 훈련 절차 및 초기화 기술로 SSM 모델이 주목받고 있는 이유는, 계산 비용 및 샘플 효율(sample efficiency) 측면에서 주의(attention)-기반 아키텍처와 유사한 성능을 달성할 수 있기 때문이라고 강조합니다. 또한, 복소수 초기화 방법이 실수 초기화에 비해 수렴 속도를 빠르게 하는 효과가 있음을 설명하고, 특정 대상 함수에서 근사-추정(tradeoff) 관계를 제시합니다.
...
### A Simple Sparse Matrix Vector Multiplication Approach to Padded Convolution (https://arxiv.org/abs/2411.19419)
Comments:
          10 pages, 2 figures, 2 tables

- **What's New**: 이번 논문에서는 제로 패딩(zero-padding)과 스트라이드(stride)를 포함한 컨볼루션(convolution)을 효율적으로 표현하기 위한 알고리즘을 소개합니다. 이 알고리즘은 희박 변환 행렬(sparse transformation matrix)을 활용해 벡터화된 입력(input)을 처리하며, 희박 행렬-벡터 곱셈(sparse matrix-vector multiplication, SpMV)을 통해 수행됩니다. 새로운 이론적 기여로는 스트라이드와 패딩을 가진 컨볼루션에서 비제로 곱셈(non-zero multiplications)의 수를 구하는 명시적 표현이 포함됩니다. 또한, Python을 통한 개념 증명 구현(proof-of-concept implementation)이 CPU와 GPU 아키텍처에서의 성능을 보여줍니다.

- **Technical Details**: 컨볼루션과 패딩은 신호 처리(signal processing) 및 머신 러닝(machine learning) 분야에서 널리 사용되는 두 가지 기본적인 선형 변환입니다. 본 논문에서는 희박 행렬-벡터 곱셈(SpMV) 알고리즘을 제안하며, 이는 희소성을 활용하여 컨볼루션의 시간 복잡도를 줄이는 데 기여합니다. 논문에는 SciPy와 NumPy를 활용한 CPU 구현, 그리고 PyTorch 및 CuPy를 이용한 GPU 구현이 포함되어 있습니다. 희박 변환 행렬의 구축은 일회성 비용으로 클고, 이후의 컨볼루션은 단순히 SpMV 연산으로 처리됩니다.

- **Performance Highlights**: 본 연구에서는 기존 인기 있는 방법과 비교했을 때 더 나은 성능을 제공하는 것을 목표로 합니다. 특히, im2col 메소드를 사용하는 대신 매 회 컨볼루션에 대해 입력을 변환하지 않고 희박성을 이용하는 방법을 제시합니다. 실험 결과는 PyTorch의 Conv2D 메소드와 비교하여 CPU 및 GPU에서의 우수한 성능을 나타냅니다. 궁극적으로, 이러한 방법론은 기계 학습과 신호 처리 분야에서 효율성을 개선하는 데 기여할 것으로 기대됩니다.
...
### On the effectiveness of discrete representations in sparse mixture of experts (https://arxiv.org/abs/2411.19402)
Comments:
          17 pages

- **What's New**: 이 논문은 Sparse Mixture of Experts (SMoE)의 문제를 해결하기 위해 새로운 구조인 Vector-Quantized Mixture of Experts (VQMoE)를 제안합니다. VQMoE는 디스리트(discrete) 표현을 통해 전문가를 선택하여 라우터(router)의 필요성을 없앴습니다. 결과적으로, VQMoE는 전통적인 라우터에서 발생하는 경로 일관성 문제와 표현 붕괴(representation collapse) 문제를 효과적으로 해결하며 이론적 지지와 실험적 증거를 제공합니다.

- **Technical Details**: VQMoE는 벡터 양자화(vector quantization)를 활용하여 디스리트 표현을 학습합니다. 이 구조는 입력에 대한 전문가 선택을 직접적으로 수행하며, 이를 통해 기존의 훈련 라우터가 지닌 정보 손실 문제를 최소화합니다. hybrid representation 방식을 도입하여 디스리트와 연속 표현을 통합함으로써 모델 성능을 개선하고, 라우터 의존성을 제거함으로써 SMoE의 훈련 안정성을 높였습니다.

- **Performance Highlights**: VQMoE는 대규모 언어 모델과 비전 태스크에서 광범위한 평가에서 전통적인 SMoE 라우팅 방법에 비해 28%의 향상된 강건성을 보였습니다. 이러한 성능 개선にも 불구하고, VQMoE는 미세 조정 태스크에서도 강력한 성능을 유지합니다. 논문에서는 추가적인 실험 결과를 통해 VQMoE의 효과성과 안정성을 심층 분석하여 제시합니다.



### Scale Invariance of Graph Neural Networks (https://arxiv.org/abs/2411.19392)
Comments:
          13 pages,. arXiv admin note: substantial text overlap with arXiv:2411.08758

- **What's New**: 이 논문에서는 그래프 신경망(Graph Neural Networks, GNNs)에서 두 가지 주요 문제인 불변성 학습(invariance learning)의 이론적 지원 부족과 동질적(homophilic) 및 이질적(heterophilic) 그래프 데이터셋에서 우수한 성능을 보이는 통합 모델의 부재를 다루고 있습니다. 이를 해결하기 위해, 우리는 그래프에서 크기 불변성(scale invariance)을 설정하고 이를 증명하였으며, ScaleNet이라는 통합 네트워크 아키텍처를 제안하여 여러 데이터셋에서 최첨단 성능을 달성했습니다.

- **Technical Details**: ScaleNet은 지향 다중 스케일 그래프(directed multi-scaled graphs)와 적응형 자기 루프 전략(adaptive self-loop strategy)을 활용하여 구현되었습니다. 이 방법은 계산적으로 비싼 엣지 가중치(edge weights)를 균일 가중치로 대체할 수 있으며, 성능을 유지하거나 개선할 수 있음을 보여주었습니다. 또한, Hermitian Laplacian 방법과 GraphSAGE의 동등성을 입증하여 GNN 구조의 이론적 기반을 강화했습니다.

- **Performance Highlights**: 제안된 방법은 네 가지 동질적 그래프와 두 가지 이질적 그래프에서 최첨단 결과를 달성하였으며, 기존 접근 방식과 비교하여 동질적 데이터셋에서는 Dir-GNN보다 우수하고, 이질적 데이터 처리에 있어 MagNet보다 더 나은 성능을 보였습니다. 다중 스케일 그래프 접근 방식은 매우 불균형한 데이터셋에 대한 장점을 제공하며, 이를 통해 성능을 극대화할 수 있습니다.
...
### Controlling Participation in Federated Learning with Feedback (https://arxiv.org/abs/2411.19242)
- **What's New**: 이번 논문에서는 연합 학습(federated learning)에서 클라이언트 참여(client participation)의 문제를 다룹니다. 기존의 전통적인 방법들이 훈련 라운드마다 소수의 클라이언트를 무작위로 선택하는 방식인 반면, 본 연구에서는 FedBack이라는 결정론적(deterministic) 접근법을 제안합니다. 이 방법은 제어 이론(control theory) 원리를 활용하여 클라이언트 참여를 효율적으로 관리합니다.

- **Technical Details**: FedBack은 클라이언트 참여를 이산시간 동적 시스템(discrete-time dynamical system)으로 모델링하며, 적분 피드백 제어기(integral feedback controller)를 사용하여 각 클라이언트의 최적화 동적(optimization dynamics)에 기반하여 참여율을 개별적으로 조정합니다. 본 연구는 최근의 연합 학습 연구를 기반으로 글로벌 수렴 보장(global convergence guarantee)를 제공합니다.

- **Performance Highlights**: 연합 이미지 분류(federated image classification)에서의 수치 실험(numerical experiments) 결과, FedBack은 클라이언트를 무작위로 선택하는 알고리즘에 비해 통신 및 계산 효율성(communication and computational efficiency)에서 최대 50% 향상을 달성한 것으로 나타났습니다.
...
### Large width penalization for neural network-based prediction interval estimation (https://arxiv.org/abs/2411.19181)
Comments:
          28 pages, 12 figures

- **What's New**: 본 연구에서는 예측 구간(Prediction Interval, PI)의 폭을 줄이기 위해 새로운 PI 손실 함수를 제안합니다. 이 함수는 평균적으로 큰 PI 폭에 더 높은 패널티를 부여하여 PI 추정 방법의 개선을 목표로 합니다. 이 손실 함수는 기울기 기반 알고리즘(Gradient-based algorithms)과의 호환성을 가지며, 최첨단 신경망(Neural Networks) 및 기존 딥러닝 기술과 통합될 수 있습니다.

- **Technical Details**: 연구에서는 PI의 품질을 향상시키기 위해 PI 커버리지 확률(PICP)과 PI 폭의 손실 항목을 포함하는 질 기반(Quality-based) 손실 함수를 사용합니다. 제안된 방법은 확률적 모델을 통해 의사 결정에 필요한 불확실성을 효과적으로 수량화하며, 합성 데이터 세트를 이용한 실험에서 PICP는 효과적으로 유지하면서 PI 폭이 크게 줄어드는 것으로 나타났습니다. 이 연구는 솔라 강우 예측에서 제안된 손실 함수의 실제 구현을 통해 높은 불확실성을 가진 데이터에서 PI 폭 최소화의 효과를 강조합니다.

- **Performance Highlights**: 제안된 손실 함수를 사용하면 PI 폭이 유의미하게 줄어들어 예측 신뢰성을 높일 수 있습니다. 연구 결과는 PI 폭 감소가 백업 자원의 과잉 배분으로 이어지는 substantial cost savings를 가져올 수 있음을 보여줍니다. 실험 결과는 제안된 접근법이 더 복잡한 신경망 모델과의 호환성을 가지며, 전력 시스템과 같은 실제 응용 프로그램에서 효과적인 위험 관리 및 자원 할당의 기초가 될 수 있음을 시사합니다.
...
- **What's New**: 이번 연구에서는 LLM(대형 언어 모델)의 추론 속도를 개선할 수 있는 Puzzle이라는 프레임워크를 소개합니다. 이 프레임워크는 수십억 개의 매개변수를 가진 모델을 특정 하드웨어에 최적화하여 추론 시의 효율성을 극대화합니다. 특히, 이 모델은 Knowledge Distillation(지식 증류) 및 Mixed Integer Programming(MIP)을 효과적으로 결합하여 하드웨어 제약을 준수하면서 높은 성능을 유지합니다.

- **Technical Details**: Puzzle 프레임워크는 블록 단위의 로컬 지식 증류(Blockwise Local Knowledge Distillation, BLD)를 활용하여 파라미터 수가 많은 LLM의 아키텍처를 최적화합니다. 이는 대규모의 가능한 모델 아키텍처를 탐색할 수 있게 해주며, 각 대체 조각을 효율적으로 평가하여 우수한 성능을 발휘하는 모델을 찾는 방식입니다. 그 결과, 최적화된 아키텍처는 성능 저하 없이 비교적 적은 수의 토큰(45B)으로 훈련됩니다.

- **Performance Highlights**: Nemotron-51B라는 모델은 NVIDIA H100 GPU에서 동작하며, 2.17배의 추론 성능 향상을 보여줍니다. 이 모델은 기존 Llama-3.1-70B-Instruct 모델에서 98.4%의 성능을 유지하면서도 메모리 사용 효율성을 높이고, 상업적 응용에 있어 새로운 기준을 설정합니다. Puzzle 프레임워크의 도입으로 LLM의 효율적 배포가 가능해지며, 다양한 사용 사례에 맞춰 최적화된 모델을 생산할 수 있습니다.



### TEA: Trajectory Encoding Augmentation for Robust and Transferable Policies in Offline Reinforcement Learning (https://arxiv.org/abs/2411.19133)
- **What's New**: 이번 논문에서는 보이지 않는 동적 환경에서 단일 정책의 강인한 일반화를 목표로 하는 오프라인 강화 학습(offline reinforcement learning, RL)에 대한 새로운 접근 방식을 제안합니다. 우리는 Trajectory Encoding Augmentation (TEA)라는 방법을 소개하며, 이는 시퀀스 인코더(예: AutoEncoders)를 통해 얻은 환경 동적에 대한 잠재 표현을 통합하여 상태 공간을 확장합니다. TEA를 사용하면 단일 정책이 새로운 동적 환경에서 뛰어난 이전 성능을 유지하며, 기존의 상태에만 의존하는 방법을 초월합니다.

- **Technical Details**: 본 연구에서는 CartPole 환경을 고전적으로 사용하여 다섯 개의 소스 환경을 설계하고, 각 환경의 동적 변수를 조정하여 실험을 진행했습니다. 상태-행동 쌍의 시퀀스를 압축할 목적의 시퀀스 인코더로 AutoEncoders를 사용하고, 이 인코더로부터 얻은 잠재 인코딩을 상태 공간에 추가하여 에이전트의 적응성을 향상시키는 방법을 제시합니다. 연구에서 제안된 방법은 BCQ(Batch-Constrained Q-learning) 알고리즘을 사용하여 기존의 데이터만으로 정책을 학습하며, 새로운 환경으로의 정책 전이 가능성을 평가합니다.

- **Performance Highlights**: TEA 방법은 다양한 새로운 환경에서 실험한 결과, 기준선 방법과 비교하여 성능이 통계적으로 유의미하게 개선되었습니다. 이러한 결과는 잠재 인코딩을 통한 상태 공간의 증가가 새로운 동적을 가진 환경에서 정책의 일반화를 효과적으로 높이는데 기여함을 보여줍니다. 우리는 TEA를 통해 정책의 이전 가능성이 향상됨을 확인했으며, 이는 강화 학습 에이전트가 변화하는 동적 환경에서도 더 잘 적응할 수 있게 해줍니다.
...
Comments:
          On going work. Codes are released at this https URL

- **What's New**: 본 논문은 FedAMoLE이라는 경량화된 개인화된 연합 파인튜닝(Federated Fine-Tuning) 프레임워크를 제안합니다. 이 프레임워크는 서로 다른 클라이언트에서의 이질적인 데이터 분포를 고려하여 LoRA 기반의 도메인 전문가를 효율적으로 할당하여 모델 이질성을 촉진합니다. 이를 통해 데이터 전송 시의 통신 비용을 최소화함으로써 연합 학습 환경에서의 처리 성능을 향상시킬 수 있습니다.

- **Technical Details**: FedAMoLE는 Adaptive Mixture of LoRA Experts(AMoLE) 모듈을 도입하여, 각 클라이언트에 다양한 수의 도메인 전문가를 할당합니다. 또한 신뢰 기반의 전문가 할당(reverse selection-based expert assignment, RSEA) 전략을 개발하여 모델 아키텍처 조정 시 데이터 기반의 접근 방식을 적용합니다. 이는 클라이언트의 데이터 특성과 도메인 전문가의 지식 도메인을 고려하여, 최적의 모델 아키텍처 결정을 지원합니다.

- **Performance Highlights**: 여섯 가지 다양한 데이터 이질성 시나리오에서, FedAMoLE는 기존 연합 LLM 파인튜닝 방법들보다 유의미하게 뛰어난 성능을 보였습니다. 데이터 이질성이 가장 높은 시나리오에서는, FedAMoLE가 최고 성능 기준선에 비해 Rouge-L 점수를 5.25% 향상시켰습니다. 또한, 이는 합리적인 지연 시간, 메모리 및 통신 오버헤드를 유지하는 가운데 달성된 결과입니다.



### Advancing Generalization in PINNs through Latent-Space Representations (https://arxiv.org/abs/2411.19125)
- **What's New**: 이번 논문에서는 다양한 PDE 구성에 대해 효과적으로 일반화할 수 있는 새로운 물리 정보 신경망 솔버인 PIDO(Physics-Informed Dynamics representatiOn learner)를 제안합니다. PIDO는 초기 조건, PDE 계수 및 훈련 시간의 변화를 포함한 다양한 설정에 맞춰 동적 시스템의 공유된 구조를 활용하여 PDE 솔루션을 잠재 공간(latent space)으로 프로젝션합니다. 이를 통해 훈련 안정성과 시간 외삽 성능을 향상시킬 수 있습니다.

- **Technical Details**: PIDO는 두 가지 핵심 구성 요소로 이루어져 있으며, 첫 번째는 격자 독립(grid-independent) 공간 표현 학습기인 DEC로, 동적 시스템들이 공유하는 내재적 구조를 활용하여 솔루션 공간과 저차원 잠재 공간 간의 매핑을 학습합니다. 두 번째는 DYN으로 명명된 시간 동역학 모델로서, Neural ODEs를 사용하여 잠재 임베딩의 계수 인지적(equation-aware) 진화를 학습합니다. 이러한 접근 방식은 물리 정보 손실이 직면한 최적화 어려움에도 효과적으로 대응할 수 있도록 돕습니다.

- **Performance Highlights**: PIDO는 1D 조합 방정식 및 2D Navier-Stokes 방정식을 포함한 여러 벤치마크에서 검증되었습니다. 그 결과, PIDO는 데이터 기반 접근 방식에 비해 일반화 성능이 향상됨을 보여주었습니다. 또한, PIDO의 학습된 표현들은 장기 통합(long-term integration) 및 역문제(inverse problems)와 같은 다운스트림 태스크에 대해 전이 가능성을 입증하였습니다.



### Deep Learning for GWP Prediction: A Framework Using PCA, Quantile Transformation, and Ensemble Modeling (https://arxiv.org/abs/2411.19124)
Comments:
          10 pages, 5 figures, 2 tables

- **What's New**: 환경 친화적인 냉매 개발이 인류의 온실가스 영향을 완화하는 데 중요하다는 점을 강조한 연구입니다. 이 연구는 Multi-Sigma 플랫폼에서 구현된 완전 연결 신경망을 사용하여 단일 성분 냉매의 100년 글로벌 온난화 잠재력(GWP 100)을 추정할 수 있는 예측 모델링 프레임워크를 제시합니다. RDKit 기반 모델이 가장 우수한 성능을 나타내며, 이는 냉매 설계 시 유용한 가이드를 제공합니다.

- **Technical Details**: 연구에 사용된 GWP 100 데이터셋은 IPCC AR6 데이터셋에서 수집한 207개의 냉매와 온실가스를 포함합니다. RDKit, Mordred, alvaDesc의 세 가지 분자 설명자를 사용해 각각의 다양한 화학적 특징을 포착하였으며, PCA와 양자 변환과 같은 차원 축소 기법을 적용하여 모델의 안정성과 성능을 높였습니다. 관련 있는 분자 특성을 식별하기 위해 요인 분석이 수행되었습니다.

- **Performance Highlights**: RDKit 기반 모델이 RMSE 481.9와 R2 점수 0.918을 기록하여 예측 정확도가 뛰어남을 입증하였습니다. 또한, 본 연구의 모델은 저-GWP 냉매의 빠른 가상 선별을 위한 확장 가능한 솔루션을 제공하여 기후 완화에 기여할 수 있는 잠재력을 가지고 있습니다. 이는 국제적인 기후 목표에 부응하는 차세대 냉매 설계를 가능하게 합니다.



### Neural Window Decoder for SC-LDPC Codes (https://arxiv.org/abs/2411.19092)
Comments:
          12 pages, 16 figures

- **What's New**: 이번 논문에서는 공간적으로 결합된 저밀도 패리티 검사(SC-LDPC) 코드에 대한 신경 윈도 디코더(NWD)를 제안합니다. 이 NWD는 기존의 윈도 디코더(WD) 프로세스를 유지하면서 훈련 가능한 신경 가중치를 통합합니다. 두 가지 새로운 훈련 전략을 통해 NWD의 훈련 효율성을 향상시키며, 이를 통해 SC-LDPC 코드의 성능을 대폭 개선할 수 있는 가능성을 제시합니다.

- **Technical Details**: NWD는 먼저 타겟 변수 노드(VN)만을 대상으로 하는 손실 함수를 사용하여 신경망을 다듬고, 훈련 과정에서 특정 영역에 편향되지 않도록 Active Learning 기법을 활용합니다. 또한 현재와 이전 업데이트 간의 상대적 중요도를 반영하는 훈련 가능한 감쇠 인자를 도입하여, 덜 중요한 CN 업데이트를 생략할 수 있습니다. 이로 인해 기존 WD와 비교해 성능 저하 없이 CN 업데이트 수를 41% 줄일 수 있음을 보여줍니다.

- **Performance Highlights**: 제안된 NWD는 오류 전파 문제를 완화하기 위해 보조 가중치 세트를 배치하여, 이전 윈도에서 오류가 감지되는 경우 활성화합니다. 이 적응형 디코딩 전략은 코드와 디코더 구조를 수정할 필요 없이 효과적으로 오류 전파를 저하시킵니다. 이를 통해 SC-LDPC 코드에서의 디코딩 성능이 크게 향상되고, 여러 개의 다른 코드 클래스에도 적용될 수 있음을 입증합니다.



### Improving sub-seasonal wind-speed forecasts in Europe with a non-linear mod (https://arxiv.org/abs/2411.19077)
- **What's New**: 본 연구는 유럽에서 서브시즌(부정기적 하부 주기) 풍속 예측 기술을 향상시키기 위해 '500 hPa 기압의 지오포텐셜 높이'(Z500)와 표면 풍속 간의 비선형 관계를 활용하는 가능성을 탐구합니다. 제안된 프레임워크는 다중 선형 회귀(Multiple Linear Regression; MLR)와 컨볼루션 신경망(Convolutional Neural Network; CNN)을 사용하여 Z500에서 표면 풍속을 회귀합니다. 연구 결과, CNN이 비선형성 덕분에 MLR보다 더 나은 성능을 보이는 것으로 나타났습니다.

- **Technical Details**: 연구에서는 ERA5 재분석을 통해 CNN 모델이 MLR 모델보다 우수한 성능을 보였으며, 이는 비선형성 때문이라는 점을 강조합니다. 서브시즌 예측에서 통계 모델의 한계로 인해 편차가 작게 나타나는 문제를 해결하기 위해, 랜덤 요인을 도입해 설명되지 않은 부분의 불확실성을 표현합니다. 그 결과, 망가진 CNN은 처음 몇 주 동안 망가진 MLR보다 성능이 뛰어난 반면, 두 주 후에는 달라지게 됩니다.

- **Performance Highlights**: 비선형성을 통해 향상된 예측 결과는 예측 시간에 따라 다른 효과를 보였습니다. 연구에 따르면, 비선형성을 통한 향상된 성능은 특정 리드 타임에 따라 달라지며, 서브시즌 예측 스킬을 높이는 것이 가능하다는 것을 보여줍니다. 이러한 비선형 접근법은 재생 가능 에너지 자원의 생산과 운영 효율성을 높이는 데 중요한 기여를 할 것으로 기대됩니다.
...
- **What's New**: 이번 논문에서는 Multiple Instance Regression (MIR)와 Learning from Label Proportions (LLP)이라는 두 가지 학습 프레임워크에서 데이터셋을 어떻게 최적의 방법으로 분할하여 하위 작업의 유용성을 극대화할 수 있는지를 다룹니다. 각각의 상황에 맞는 손실 함수에서 최적의 bagging 전략을 찾는 것이 중요하며, 특히 클러스터링을 통해 최적화를 이루는 방법을 설명합니다. 또한, label-differential privacy를 보장하는 메커니즘도 제안하여 개인 정보 보호를 강화하는 방안을 제시합니다.

- **Technical Details**: 논문에서는 MIR과 LLP 각각의 설정에서 최적의 bagging 전략이 무엇인지를 분석합니다. MIR에서는 각 bag-label이 bag의 특정 instance-label 중 하나로 정의되며, LLP에서는 bag-label이 bag 내의 label의 평균으로 정의됩니다. 이러한 최적화 과정에서 k-means 클러스터링 기법이 핵심적이며, 각 bag의 제공이 모델의 유용성에 미치는 영향을 고려하여 설정할 수 있습니다. 또한, 다양한 손실 함수에 따라 모델의 유틸리티를 최대화하기 위한 최적의 분할 전략을 모색하였습니다.

- **Performance Highlights**: 실험을 통해 k-means 클러스터링 기반의 bagging 전략이 효과적임을 입증하였고, 다양한 매개변수 조정에 따른 결과를 분석하였습니다. 본 연구는 특히 광고 데이터에서의 유용성을 고려하여 MIR 및 LLP 설정에서의 최적화 방법론을 제공하고 있으며, label-dependent bagging을 통해 개인 정보 보호를 강화하는 방법과 배제된 손실 또한 정량화하였습니다. 결과적으로 본 연구는 데이터의 비공식적인 분석으로부터 정보 손실을 줄이며 신뢰성을 높이는 데 기여합니다.
...
### Pilot Contamination Aware Transformer for Downlink Power Control in Cell-Free Massive MIMO Networks (https://arxiv.org/abs/2411.19020)
Comments:
          13 paged (double-column), 10 figures, 3 tables

- **What's New**: 본 논문은 Pilot contamination-aware power control (PAPC) transformer neural network를 소개하여, 기존의 학습 기반 방법이 채널 데이터의 본질적인 구조와 파일럿 할당 정보(pilot allocation information)를 제대로 활용하지 못하는 문제를 해결합니다. PAPC는 사용자 간의 관계를 학습할 수 있는 attention mechanism을 도입해 파일럿 오염(pilot contamination) 문제를 효과적으로 다룹니다. 또한, 비지도 학습 프레임워크를 통해 훈련되어 기존의 가속 미 proximal gradient (APG) 알고리즘에 비해 더 나은 계산 효율성을 보여줍니다.

- **Technical Details**: PAPC는 특별히 설계된 전처리(preprocessing) 및 후처리(post-processing) 단계를 포함하여 입력에서 특징(feature)을 효율적으로 추출하고 전력 제약 조건(power constraints)을 준수합니다. 모델은 사용자 수가 동적으로 변할 때 유연하게 대응할 수 있도록 패딩(padding) 기술을 사용하여 설계되었습니다. 이와 함께, PAPC는 대규모 CFmMIMO 네트워크에서 높은 확장성을 제공하며 이는 비중격 optmization 문제를 해결하기 위한 새로운 접근법을 제시합니다.

- **Performance Highlights**: 시뮬레이션 결과, PAPC는 파일럿 정보가 없는 전통적인 Fully Connected Networks (FCNs)보다 우수한 성능을 보여주며, CFmMIMO 시스템에서 대규모 사용자 수를 처리하는 데 성공적으로 적응하는 모습을 나타냈습니다. PAPC는 성능 대 공정성(spectral efficiency fairness) 측면에서 APG와 유사한 결과를 보이며, 계산상의 효율성을 크게 향상시킵니다. 또한, 대규모 네트워크에서 요구되는 계산 복잡성을 현저히 낮추어 주목받고 있습니다.
...
### FedRGL: Robust Federated Graph Learning for Label Nois (https://arxiv.org/abs/2411.18905)
- **What's New**: 이번 논문에서는 Federated Graph Learning(FGL)에서 라벨 노이즈 문제를 해결하기 위해 FedRGL이라는 새로운 방법을 제안합니다. FedRGL은 dual-perspective consistency noise node filtering을 도입하여 글로벌 모델과 서브그래프 구조를 활용하여 노이즈 필터링을 수행합니다. 이를 통해 클라이언트 측 훈련을 강화하고, 가짜 라벨이 부여된 노이드에도 고신뢰성을 부여할 수 있도록 합니다.

- **Technical Details**: FedRGL은 글로벌 모델 정보와 서브그래프 구조 정보를 결합하여 라벨 노이즈가 글로벌 모델에 미치는 영향을 완화합니다. 클라이언트 측에서는 클래스 분포의 이질성을 고려하여 샘플 손실을 개별적으로 계산하고, 클래스 인식 동적 임계값 제약 하에 노이즈 노드의 정확한 선택이 이루어집니다. 서버 측에서는 예측 엔트로피를 통해 클라이언트 업로드 모델의 품질을 측정하고, 글로벌 모델 집합체를 견고하게 만드는 방식을 채택합니다.

- **Performance Highlights**: FedRGL은 여러 실제 그래프 데이터셋에 대한 비교 실험에서 12개의 기준 방법을 초과하는 우수한 성능을 보여주었습니다. 다양한 노이즈 비율, 유형 및 클라이언트 수에서 FedRGL은 일관되게 높은 테스트 정확도를 기록하여 라벨 노이즈가 있는 상황에서도 강력한 성능을 유지합니다. 이러한 결과는 라벨 노이즈가 존재하는 Federated Graph Learning의 적용에 대한 새로운 통찰력을 제공합니다.
...
- **What's New**: 이번 연구는 Swarm Intelligence Optimization (SI) 알고리즘을 활용한 Federated Learning (FL)에서의 클라이언트 선택에 관한 새로운 관점을 제시합니다. 특히 사이버 보안(cybersecurity) 응용 프로그램에 중점을 두고, 기존 연구에서는 중앙 집중식 머신 러닝을 위한 최적화 기술만을 다루었고, 분산 FL의 독특한 도전 과제를 거의 다루지 않았습니다.

- **Technical Details**: 연구는 Grey Wolf Optimization (GWO), Particle Swarm Optimization (PSO), Cuckoo Search, Bat Algorithm, Bee Colony, Ant Colony Optimization, Fish Swarm, Glow Worm, Intelligent Water Droplet를 포함한 아홉 가지 SI 알고리즘을 네 가지 실험 시나리오에서 평가합니다. 실험 시나리오는 고정 클라이언트 참여, 동적 참여 패턴, 이질적 비IID(non-IID) 데이터 분포, 적대적 소음 조건을 포함합니다.

- **Performance Highlights**: GWO는 모든 설정에서 가장 높은 정확도, 리콜(recall), F1 점수를 달성하며 뛰어난 적응성(adaptability)과 강력성을 보였습니다. PSO와 Cuckoo Search도 강력한 성능을 보여주었습니다. 이러한 결과는 SI 알고리즘이 분산 및 적대적 FL 문제를 해결하는 데 있어 가능성을 제시하며, IoT와 대규모 네트워크에서의 침입 탐지(intrusion detection)와 같은 사이버 보안 응용에 있어 확장 가능하고 회복력 있는 솔루션을 제공합니다.



### A Lean Dataset for International Math Olympiad: Small Steps towards Writing Math Proofs for Hard Problems (https://arxiv.org/abs/2411.18872)
- **What's New**: 이 연구는 인공지능이 수학 문제에 대한 공식 증명을 자동으로 작성할 수 있는 가능성을 탐구합니다. miniF2F 데이터셋에는 20개의 IMO 문제가 포함되어 있으나, 정확한 공식 증명은 7개 문제에만 존재합니다. 본 논문은 나머지 13개의 문제에 대한 완전한 공식 증명을 작성하고, 추가로 IMO 2022 및 2023의 문제 3개를 포함하여 총 5,150줄의 Lean 코드로 작성된 증명을 추가합니다.

- **Technical Details**: 논문에서는 900개의 레머(lemma) 데이터셋을 구축하였으며 각 레머는 약 25,500줄의 Lean 코드로 구성됩니다. 이러한 레머들은 비트리비얼하지 않지만 접근 가능하여 AI 모델의 실패와 성공을 평가하고 진단할 수 있는 기회를 제공합니다. 연구에서는 GPT-4의 공식 증명 작성 능력을 평가하기 위해 제로 샷 프롬프팅(zero shot prompting), 연쇄적 사고(CoT reasoning) 및 레머 검색(lemma retrieval)을 활용합니다.

- **Performance Highlights**: GPT-4는 많은 레머에 대한 올바른 공식 증명을 거의 작성하지 못했습니다. 공식 증명의 작성을 계획하고 실행하는 과정에서 각 레머들을 적절히 조합하는 것이 어렵다는 점이 강조됩니다. 마지막으로, 연구는 GPT-4가 작성한 증명들을 웹의 내용을 기준으로 비교하며 이 모델의 성능을 진단합니다.
...
### One-Step Early Stopping Strategy using Neural Tangent Kernel Theory and Rademacher Complexity (https://arxiv.org/abs/2411.18806)
Comments:
          7 pages, 2 figures

- **What's New**: 이번 연구에서는 neural network (NN)에서 초기 멈춤(early stopping) 전략을 최적의 멈춤 시간으로 분석합니다. 초기 훈련 오류 벡터와 neural tangent kernel (NTK)의 고유값을 바탕으로, 일반화 특성을 유지하며 훈련 오류가 최소화되기 전에 멈출 수 있는 시점에 대한 수학적 추정을 제공합니다. 이러한 방법은 Van der Pol 진동기의 MPC 제어 예시를 통해 입증되었습니다.

- **Technical Details**: 연구에서는 SGD(gradient descent) 방식으로 NN을 훈련시키며, 입력 데이터 집합 S로부터 무작위로 선택된 샘플을 사용합니다. 초기 단계에서 특정 조건을 만족하는 값 γ를 계산하여, population loss L_{D}의 상한 값 Ω(t_{1})가 감소하도록 합니다. NTK 이론을 활용하여 훈련 동작에 대한 지침을 제공하며, 특히 underparameterized 환경에서 초기 멈춤 전략에 대한 설명을 제시합니다.

- **Performance Highlights**: 이 연구는 NN을 통한 MPC 모사에서 초기 멈춤 전략이 population loss를 최소화하는 데 기여함을 강조합니다. 기존의 overparameterized 환경과 비교할 때, NN의 노드 수에 대한 가정을 두지 않고도 유용한 결과를 도출하였으며, NTK의 특정 고유값을 이용하여 효율적인 멈춤 시점을 제시하고 있습니다. 이를 통해 NN이 달성할 수 있는 좋은 일반화 능력을 더욱 강화할 수 있는 가능성을 보여줍니다.



### Stratified Non-Negative Tensor Factorization (https://arxiv.org/abs/2411.18805)
Comments:
          5 pages. Will appear in IEEE Asilomar Conference on Signals, Systems, and Computers 2024

- **What's New**: 본 논문에서는 Stratisfied-NMF를 비선형성이 포함된 텐서 데이터에 확장한 Stratified-NTF를 제안합니다. 기존의 NMF와는 달리, 이 방법은 서로 다른 소스(스트라타)에서 발생하는 다중 모달 데이터의 특성을 함께 고려하도록 설계되었습니다. 여러 소스에서 여러 주제를 효과적으로 식별하는 능력을 보여줍니다.

- **Technical Details**: Stratified-NTF는 비음수 텐서 데이터를 비음수 행렬 요인화(NMF) 기법을 활용하여 분해합니다. 이 과정은 기하학적 구조를 보존하면서 동시에 다양성 있게 모델링할 수 있는 주제와 특징을 추출하도록 합니다. 제안된 방법은 텐서 데이터의 레벨에 따라 다양한 랭크(r)의 특징을 학습할 수 있도록 설계되었습니다.

- **Performance Highlights**: 실험 결과, Stratified-NTF는 기존 Stratified-NMF보다 더 낮은 메모리 요구 사항으로 해석 가능한 주제를 식별할 수 있음을 확인했습니다. 또한, 정규화된 변형을 도입했으며, 이 방법이 노이즈가 있는 이미지 데이터에서도 효과적임을 증명하였습니다. 연구 결과는 공개된 코드와 함께 제공되어, 향후 연구에 활용될 수 있는 가능성을 제시합니다.
...
### Graph-Based Biomarker Discovery and Interpretation for Alzheimer's Diseas (https://arxiv.org/abs/2411.18796)
Comments:
          9 pages, 7 figures

- **What's New**: 이 논문은 알츠하이머병(AD)에 대한 조기 진단 및 치료 약물 표적 발견을 위한 새로운 기계 학습 프레임워크인 BRAIN(Biomarker Representation, Analysis, and Interpretation Network)을 소개합니다. BRAIN은 서로 다른 ML 모델을 통해 AD 진단에 기여하는 중요한 바이오마커를 동시 최적화하여 식별하도록 설계되었습니다. 이는 기존의 고비용 방사선 영상 기반 방법과는 달리, 접근성이 용이하고 최소 침습적인 혈액 검사 데이터를 활용합니다.

- **Technical Details**: BRAIN은 다양한 기계 학습 모델을 활용하여 바이오마커의 중요도를 평가하고 이를 시각적으로 표현하는 그래프 기반의 새로운 방법을 제공합니다. 모델 훈련 후, SHAP 분석을 통해 기능 점수를 생성하고, 이러한 점수를 통합하여 중요한 바이오마커의 포괄적인 집합을 도출합니다. 특히 AD와 대조군 간의 세 가지 새로운 바이오마커 하위 네트워크를 발견하여 이들 간의 관계를 규명하는데 중점을 둡니다.

- **Performance Highlights**: 연구는 텍사스 알츠하이머 연구 및 치료 컨소시엄의 공개 접근 가능한 데이터 셋을 분석하여 AD와 대조군 간의 뚜렷한 바이오마커 상호작용을 보여줍니다. 결과적으로 기존의 방법보다 더 나은 바이오마커 분석을 제공하며, 단독으로는 복잡한 AD 병리학을 포착하기 어려운 바이오마커들이 서로 어떻게 연관되어 있는지를 시각적으로 전달합니다. BRAIN은 다양한 데이터셋에서도 적용 가능하여 AD 진단 및 약물 발견의 새로운 패러다임을 제안합니다.



### Investigating Plausibility of Biologically Inspired Bayesian Learning in ANNs (https://arxiv.org/abs/2411.18788)
- **What's New**: 이번 논문은 재앙적 망각(catastrophic forgetting) 문제를 해결하기 위해 생물학적 시스템에서 영감을 받은 베이지안 학습(Bayesian Learning) 방법을 인공 시스템에 적용하고자 합니다. 기존의 인공 신경망(ANNs)은 새로운 데이터에 직면했을 때 성능 저하를 경험하거나, 이전에 학습한 내용을 잊어버리는 경향이 있습니다. 이 연구는 베이지안 추론(Bayesian inference)에 임계값(thresholding) 메커니즘을 적용하여 생물학적으로 영감을 받은 모델을 모사하려고 하며, MNIST 데이터셋을 통해 이러한 접근 방식의 효과를 검증합니다.

- **Technical Details**: 저자들은 미생물학적 모델의 효율성을 강조하며, 생물학적 시스템이 불확실성을 계산하는 능력이 뛰어나다고 설명합니다. 이 논문에서는 입력 네트워크에서 발생하는 잠재적인 불확실성을 측정하기 위해 베이지안 신경망(Bayesian Neural Networks, BNNs)의 원리를 활용하고, 생물학적 뉴런의 전달 방식에 따라 가중치와 편향을 다루는 방식을 제안합니다. 특히, 시간 및 공간 임계값을 설정하여 활성화 결정 과정에서의 기능적 관계를 개선하고자 합니다.

- **Performance Highlights**: 결과적으로, 모델이 새 데이터에 대해 예측할 때와 예측하지 않을 때의 상대적인 성능 개선을 보여주었습니다. 생물학적 영감을 받은 베이지안 학습을 통해 모델의 지속적인 적응 가능성을 높일 수 있음을 발견하였습니다. 이러한 연구는 종합적으로 볼 때 인공 신경망의 구조적 한계를 극복하고, 더 신뢰적인 인공지능 시스템을 개발하는데 기여할 수 있을 것으로 기대됩니다.



### Classification of Deceased Patients from Non-Deceased Patients using Random Forest and Support Vector Machine Classifiers (https://arxiv.org/abs/2411.18759)
- **What's New**: 이 연구는 COVID-19 환자의 데이터를 분석하여 생존 여부를 예측하기 위한 새로운 접근법을 제시합니다. 특히, 환자가 사망할 위험이 높다는 신호를 발견할 수 있는 패턴을 데이터 마이닝을 통해 탐색하는 과정을 중점적으로 다루고 있습니다. 기존 연구들과의 차별점은 세포막 감염을 포함한 다기관 시스템에 대한 또 다른 통찰을 제공한다는 점입니다.

- **Technical Details**: 연구자들은 Support Vector Machine (SVM)과 Random Forest (RF) 분류 기법을 사용하여 COVID-19 환자를 분류하였습니다. 환자의 인구 통계학적 정보, 실험실 검사 결과 및 기존 건강 상태를 바탕으로 생존 환자와 사망 환자를 구분하기 위해 10-fold validation 절차를 수행하였습니다. 또한, Receiver Operating Characteristic (ROC) 곡선과 Confusion Matrix를 활용하여 분류기의 성능을 평가했습니다.

- **Performance Highlights**: 이 연구는 COVID-19 환자의 생존 예측을 위한 효과적인 예측 모델을 제공함으로써, 향후 의료 분야에서 데이터 기반 의사결정에 기여할 수 있을 것으로 기대됩니다. 클러스터 분석을 통해 환자의 기존 질환 유무와 패혈증 여부와 같은 이진 요인 및 인구 통계학적 변수를 예측 변수로 사용하였으며, 정확한 분류 성능을 달성했습니다.
...
### Locally Differentially Private Online Federated Learning With Correlated Nois (https://arxiv.org/abs/2411.18752)
Comments:
          arXiv admin note: text overlap with arXiv:2403.16542

- **What's New**: 이 논문에서는 온라인 연합 학습(online federated learning, OFL)을 위한 새로운 로컬 차별적 개인정보 보호(Locally Differential Privacy, LDP) 알고리즘을 소개합니다. 이 알고리즘은 시간적으로 상관된 노이즈(temporally correlated noise)를 활용하여 유용성을 개선하면서도 개인정보를 보호할 수 있도록 설계되었습니다. 기존의 방법들과 달리, 이 연구는 실제로 복잡한 환경에서의 개인 정보 보호와 유틸리티 간의 균형을 맞추는 데 이론적인 보장을 제시합니다.

- **Technical Details**: 제안된 알고리즘은 Perturbed Iterate 분석 기법을 사용하여 노이즈가 유틸리티에 미치는 영향을 제어합니다. 또한, 로컬 업데이트(local updates)와 비독립적 비동기 데이터(streaming non-IID data)에 대한 드리프트 오류(drift errors)를 효과적으로 관리하는 방법을 보여줍니다. 논문에서는 (ε,δ)-LDP 예산에 따라 주요 파라미터와 변동 환경의 강도가 학습 성능에 미치는 영향을 정량화하는 동적 후회(bound)를 수립하고 있습니다.

- **Performance Highlights**: Numerical 실험을 통해 제안된 알고리즘의 효율성을 검증하였습니다. 이 알고리즘은 비볼록 손실 함수(nonconvex loss functions) 클래스에 대해 강력한 성능을 발휘하며, 여러 통신 라운드와 로컬 업데이트가 있는 상황에서도 유익한 결과를 제공합니다. 기존 연구와 비교할 때, 온라인 환경에서 로컬 업데이트와 상관된 노이즈를 사용하는 방안에 대한 이론적 보장이 제시된 점이 주목할 만합니다.



### Foundation Models in Radiology: What, How, When, Why and Why No (https://arxiv.org/abs/2411.18730)
Comments:
          This pre-print has been accepted for publication in Radiology

- **What's New**: 이 논문에서는 인공지능의 최근 발전을 통해 등장한 대규모 딥러닝 모델인 foundation models에 대해 다룹니다. 특히 이들 모델이 비지도(unlabeled) 데이터로 학습되는 방식을 설명하며, 방사선학 분야에서의 중요성을 강조합니다. 이 리뷰는 방사선학에 특화된 foundation models의 교육 방식과 요구 사항을 명확히 하고자 합니다.

- **Technical Details**: 방사선학 분야에서 foundation models의 훈련 데이터 요구 사항, 모델 훈련 패러다임(training paradigms), 모델의 능력(capabilities), 평가 전략(evaluation strategies)을 표준화된 용어로 정리합니다. 또한 이 모델들의 교육을 촉진하기 위한 잠재적 경로를 탐구하며, 이러한 모델들이 가져올 수 있는 이점과 도전 과제를 강조합니다.

- **Performance Highlights**: 이 리뷰 논문은 방사선학 분야에서 foundation models의 교육이 환자, 의료 제공자, 방사선학자에게 이익을 줄 수 있도록 안전하고 책임 있는 방식으로 진행되도록 하는 데 초점을 맞추고 있습니다. 또한, 기술 발전과 임상 요구를 통합하는 방법을 제안하여, 궁극적으로 방사선학의 발전에 기여할 수 있는 방향을 제시합니다.
...
### Addressing bias in Recommender Systems: A Case Study on Data Debiasing Techniques in Mobile Games (https://arxiv.org/abs/2411.18716)
Comments:
          RobustRecSys workshop @ RecSys 2024

- **What's New**: 모바일 게임 산업에서의 추천 시스템(Recommender Systems, RS)에서 데이터 편견(bias) 문제를 다룬 새로운 연구가 주목받고 있습니다. 특히, 이 연구는 모바일 게임에서 주어진 암묵적 피드백(implicit feedback) 데이터에 대한 편견을 식별하고 분류하려고 합니다. 또한 이러한 편견을 줄이기 위한 기존 기술들을 검토하고, 이를 King의 실제 데이터에 적용한 효과성을 평가합니다.

- **Technical Details**: 연구에서는 COAT, yahooR3와 같은 공공 데이터셋과 King의 세 가지 고유 데이터셋(Set A, Set B, Set C)을 사용하여 이론적 결과를 검증하였습니다. 데이터셋의 편견을 파악하기 위해, 노출 편향(exposure bias), 선택 편향(selection bias), 위치 편향(position bias) 등의 여러 유형의 편향이 존재함을 이해하였습니다. 이를 기반으로 Matrix Factorization (MF), Inverse Propensity Scoring (IPS), Doubly Robust learning, AutoDebias와 같은 다양한 디바이싱(debiasing) 기술을 적용하여 성능을 평가합니다.

- **Performance Highlights**: 연구진은 추천 시스템의 예측 정확도 및 품질을 평가하기 위한 여러 메트릭스(RMSE, AUC, NDCG@5, Gini index, Entropy)를 사용했습니다. 이를 통해 각 모델의 디바이싱 효과성을 측정하고, 모델의 예측력 및 추천 항목의 다양성을 비교 분석합니다. 이 연구는 모바일 게임에서 데이터 편향 문제를 해결하기 위한 새로운 접근 방식을 제공하여 게임 개발자들에게 큰 기여를 할 것으로 기대됩니다.
...
- **What's New**: 이번 연구는 Stochastic Gradient Descent(SGD)의 중량 평균(weight averaging)을 체계적으로 분석한 것으로, 특히 Exponential Moving Average(EMA)에 중점을 둡니다. EMA는 학습 동적을 탐구하면서 하이퍼파라미터 튜닝 가이드를 제공하고, 초기 성능이 뛰어난 이유를 설명합니다. 연구팀은 EMA 모델이 SGD에 비해 낮은 학습률 감소를 요구하며, 이는 잡음을 자연스럽게 줄임으로써 암묵적 정규화를 도입한다고 밝혔습니다.

- **Technical Details**: EMA는 SGD의 마지막 이터레이트(iterate) 솔루션과 다른 결과를 생성하며, 학습 루프 외부에서의 특성을 강조합니다. 연구에서는 EMA 모델의 훈련 동태를 분석하여, 학습 속도가 높게 유지되면서도 좋은 솔루션을 찾을 수 있음을 발견했습니다. 특히, Batch Normalization(BN)과 함께 사용할 경우 EMA 평균화의 윈도우 크기를 조정하면 일반화 성능을 더욱 향상시킬 수 있음을 보여줍니다.

- **Performance Highlights**: EMA 모델은 노이즈 레이블에 대한 견고성, 예측 일관성, 보정(calibration), 전이 학습(transfer learning) 등에서 개선된 성능을 보였습니다. 실험 결과, EMA 모델은 마지막 이터레이트 SGD 솔루션에 비해 일반화 성능이 일관되게 향상되었으며, 단순한 EMA 모델이 레이블 노이즈에 대한 강력한 훈련 방법들과 경쟁할 수 있는 것으로 나타났습니다.
...
### Another look at inference after prediction (https://arxiv.org/abs/2411.19908)
- **What's New**: 이 연구는 머신러닝(ML) 모델의 예측을 활용하여 통계적 추론(statistical inference)을 수행하는 예측 기반(PB) 추론의 효율성을Study합니다. 최근 Motwani와 Witten(2023)이 검토한 두 가지 혁신적인 PB 추론 방법을 소개하며, Wang et al.(2020)의 방법은 ML 모델이 정확하게 회귀 함수(regression function)를 포착할 때 일관된 추정량을 제공함을 보여줍니다. 반면, Angelopoulos et al.(2023)의 예측 기반 추론(PPI) 방법은 모델의 정확도와 관계없이 유효한 추정을 제공합니다.

- **Technical Details**: 연구에서는 Y(결과)와 Z(예측 변수) 간의 관계 설정을 다룹니다. Y는 부분적으로 관측되며, ML 모델을 통해 예측할 수 있습니다. 연구진은 PPI 추정량의 비율적 효율성을 강조하며, Chen과 Chen(2000)이 제안한 가중치를 추가하여 더 효율적인 추정량을 얻을 수 있음을 보여줍니다.

- **Performance Highlights**: 실험 결과, Chen과 Chen(CC) 추정량이 ML 모델의 규격에 대한 로버스트성(robustness)과 통계적 효율성(statistical efficiency) 사이의 균형을 제공하여 실제 사용에 있어 최적의 선택임을 나타냅니다. 이 연구는 1960년대 통계 및 경제학 문헌의 기존 방법들과 PB 추론 문제의 맥락을 연결지으며, PPI 방법의 조건부 통계적 효율성을 평가합니다.
...
### Noncommutative Model Selection for Data Clustering and Dimension Reduction Using Relative von Neumann Entropy (https://arxiv.org/abs/2411.19902)
Comments:
          20 pages

- **What's New**: 이 논문에서는 완전 데이터 기반의 비지도 분류(unsupervised classification)와 차원 축소(dimension reduction)를 위한 알고리즘 쌍을 제안합니다. 기존의 k-means와 같은 알고리즘과 달리, 새로 제안한 알고리즘은 이웃의 크기나 원하는 클러스터 수 같은 정보를 필요로 하지 않으므로 사용자 입력 없이도 작동할 수 있습니다. 또한, 알고리즘은 데이터로부터 자연스러운 그래프를 구성하여 특정 열전달 연산자(heat operator)의 상대적인 von Neumann 엔트로피를 극대화하는 그래프를 선택하는 방식으로 작동합니다.

- **Technical Details**: 제안된 알고리즘은 데이터 점들을 가중 그래프로 연결하고, 그래프의 Laplacian 고유벡터(eigenvector)를 사용하여 데이터의 차원을 축소하고 클러스터를 식별합니다. 이 과정에서 diffusion process를 고려하여 그래프를 선택하는 방식은 클러스터의 연결성과 차원 축소에 대한 통찰을 제공합니다. 특히, 알고리즘은 Euclidean 공간의 거리를 명시적으로 반영하여 가중치를 설정하고, 비슷한 점들을 더욱 명확히 연결합니다.

- **Performance Highlights**: 계산 실험 결과, 제안된 클러스터링 알고리즘은 비선형 기하학(non-trivial geometry)과 위상(topology)을 가진 데이터셋에서 기존의 k-means 알고리즘보다 뛰어난 성능을 보였습니다. 특히, COIL-20 이미지 데이터셋과 같은 실제 이미지에서도 우수한 성과를 나타냈으며, 단순한 사례에서 잘 작동하는 것이 입증되었습니다. 이로 인해, 새로운 알고리즘이 다양한 데이터 구조에서 효과적으로 적용될 수 있음을 보여줍니다.



### Efficient quantum-enhanced classical simulation for patches of quantum landscapes (https://arxiv.org/abs/2411.19896)
Comments:
          10 + 47 pages, 4 figures

- **What's New**: 이번 연구에서는 매개변수화된 양자 회로에 의해 생성된 기대값 풍경의 부분 영역(패치)을 클래식하게 근사하는 서브루틴을 제시합니다. 이를 통해 양자 컴퓨터의 효율성을 극대화하며, 필요하지 않은 경우 클래식 장치로 옮길 수 있는 작업을 식별할 수 있습니다. 백그라운드로는 양자 보강된 클래식 알고리즘이 있으며, 간단한 양자 측정을 통해 가능한 시뮬레이션을 수행할 수 있습니다.

- **Technical Details**: 제안된 알고리즘은 특정 양자 회로 패밀리에 대해 시간 복잡도(time complexity)와 샘플 복잡도(sample complexity)를 보장하며, Hamiltonian 변동 추정 및 127 큐비트의 긴 시간 동적 시뮬레이션에서 수치적으로 검증됩니다. 이는 양자 회로의 하나의 부분 영역을 클래식 환경에서 근사적으로 시뮬레이션할 수 있음을 보여주는 결과입니다.

- **Performance Highlights**: 연구에서 제안한 시뮬레이션 알고리즘은 향후 큐비트 수가 증가할수록 더욱 향상될 가능성을 지니며, 양자 컴퓨터의 활용도를 높일 수 있는 중요한 기초를 마련합니다. 특히, Hamiltonian variational ansatz를 이용한 시뮬레이션은 정확한 검증이 가능한 방식으로, 결과의 신뢰성을 더욱 높입니다.



### Noncommutative Model Selection and the Data-Driven Estimation of Real Cohomology Groups (https://arxiv.org/abs/2411.19894)
Comments:
          15 pages, sequel to "Noncommutative Model Selection for Data Clustering and Dimension Reduction Using Relative von Neumann Entropy"

- **What's New**: 이번 논문은 compact metric-measure space인 (X, d_X, μ_X)가 metric-measure space인 (Y, d_Y, μ_Y) 내에 임베디드 되었을 때, 주어진 샘플 포인트들로부터 실 코호몰로지 그룹 H^k (X ; ℝ)를 추정하는 완전히 데이터 기반의 세 가지 방법을 제안합니다. 샘플 포인트는 균일 분포(μ_X)에서 중첩되어 있으며, Y로부터의 노이즈에 영향을 받을 수 있습니다. 이 연구는 X가 ℝ^n에 임베디드 되어 있을 때의 여러 계산 실험 결과를 제시합니다.

- **Technical Details**: 제안된 방법은 샘플 포인트를 사용하여 후보 반군(semigroup)을 구성하고, Hodge-Laplacian에서 생성된 연산자 반군의 구조를 이용합니다. 이 과정은 특정 위상 불변량을 추정할 수 있는 모델을 선택하기 위해 통계 모델 선택 기법에서 영감을 받은 여러 아이디어를 포함합니다. 각 알고리즘은 특정 차원의 가장 많은 기하학(geometry)을 포착할 수 있도록 설계된 선택 기준을 따릅니다.

- **Performance Highlights**: 논문에서 제안하는 알고리즘들은 실제로 계산해보았을 때, 그중 두 가지 방법이 상대적으로 뛰어난 성능을 보였습니다. 이러한 알고리즘들은 순수한 위상적 방법이 아니라 관찰된 데이터로부터 특성에 따라 선택된 반군의 스펙트럴 속성을 활용하여 실제 코호몰로지 그룹을 효과적으로 추정할 수 있음을 보여줍니다. 이러한 접근 방식은 위상 데이터 분석과 결합하여 새로운 연구 방향을 제시할 수 있는 가능성을 제시합니다.
...
### Machine learning force-field model for kinetic Monte Carlo simulations of itinerant Ising magnets (https://arxiv.org/abs/2411.19780)
Comments:
          11 pages, 7 figures

- **What's New**: 이 논문에서는 대규모 케결론식 몬테카를로(kMC) 시뮬레이션을 위한 확장 가능한 머신러닝(ML) 프레임워크를 제안합니다. 저자들은 국소성 원리를 가정하고, 국소적인 전자 구조 문제를 해결하여 Ising 구성이 어떻게 에너지 변화를 일으키는지를 예측하는 CNN 모델을 개발했습니다. 이를 통해 큰 래티스에 대한 kMC 시뮬레이션을 쉽게 확장할 수 있는 가능성을 제시하였습니다. ML 방법을 활용하여 전형적인 페로자성 도메인의 조화시간 변화를 분석할 수 있게 되었습니다.

- **Technical Details**: 이 연구는 입자의 동적 특성을 모델링하는 머신러닝 접근 방식의 발전을 다룹니다. 저자들은 Ising 변수와 같은 이산 동적 고전 변수가 있는 시스템에서의 kMC 방법의 한계를 극복하기 위해 ML 프레임워크를 채택하였습니다. CNN 모델은 주어진 스핀 업데이트에 따른 효과적인 국소 장과 에너지 변화를 직접 예측하도록 훈련되며, 이를 통해 계산 속도를 크게 향상시킬 수 있습니다.

- **Performance Highlights**: 대규모 ML 기반 kMC 시뮬레이션을 수행한 결과, 저자들은 Ising-DE 모델에서 흥미로운 온도 의존적인 페이즈 정렬 동역학을 발견하였습니다. 높은 온도에서는 도메인의 조화 시간이 특정 성장 법칙을 따르는 반면, 낮은 온도에서는 비정상적인 힘 법칙이 발견되었습니다. 이러한 결과는 이론적으로 중요하며, 전자상과 관련한 특수한 거동을 이해하는 데 기여할 것으로 보입니다.
...
### Nonparametric Instrumental Regression via Kernel Methods is Minimax Optima (https://arxiv.org/abs/2411.19653)
- **What's New**: 본 논문에서는 Singh et al. (2019)의 커널 도구 변수를 사용하는 비모수적 2단계 최소제곱법(2SLS)의 알고리즘을 연구합니다. 기존 알고리즘의 성능을 개선하고, 커널 NPIV 추정기가 최적 해답으로 수렴하는 방법을 분석하여, 알려지지 않은 구조적 함수에 대한 경우도 다룹니다. 이는 커널 NPIV의 수렴 분석을 통해 새로운 통찰을 제공합니다.

- **Technical Details**: 우리는 강한 $L_2$-노름에 대한 수렴 분석을 수행하며, 이 분석은 pseudo-norm 대신에 진행됩니다. 또한, 새로운 형태의 투영된 부분공간 크기(description of the projected subspace size)를 사용하여, 도구 변수에 의존하지 않고 목표 함수의 부드럼(smoothness)을 특징지웁니다. 이러한 결과는 역학습(inverse learning) 문헌에서의 link condition과 밀접한 관련이 있습니다.

- **Performance Highlights**: 표준 커널 학습 가정을 바탕으로, 우리는 강한 $L_2$-노름에서 커널 NPIV의 minimax 최적 학습률을 유도했습니다. 이 결과는 도구 변수의 강도가 효율적인 학습을 달성하는 데 필수적임을 매우 잘 보여줍니다. 또한, 원래의 커널 NPIV 알고리즘을 향상시켜, 1단계 회귀에서 일반적인 스펙트럼 정규화를 도입하였습니다. 이에 따라 티호노프 정규화의 포화 현상을 극복할 수 있습니다.
...
- **What's New**: 이 논문은 Physics-Informed Neural Networks (PINNs)를 이용한 부분 미분 방정식(Partial Differential Equations, PDEs) 문제의 해결 방법을 제안합니다. 특히, Point Adaptive Collocation Method for Artificial Neural Networks (PACMANN)라는 새로운 방법을 통해 collocation 포인트를 효율적으로 조정하여 더 나은 예측 결과를 도출하고자 합니다. 기존의 방법들은 차원 수가 증가할수록 성능이 저하되는 한계가 있었으나, PACMANN은 이러한 문제를 극복할 수 있는 잠재력을 가지고 있습니다.

- **Technical Details**: PACMANN의 핵심 기술은 제곱 잔여(residual) 함수의 기울기를 이용하여 collocation 포인트를 높은 잔여 지역으로 이동시키는 것입니다. 이 방법은 표준 최적화 알고리즘을 사용하여 포인트를 조합-최적화(maximization) 문제로 변환함으로써 구현됩니다. 또한 PACMANN은 두 가지 주요 하이퍼파라미터인 단계 사이즈(stepsize)와 최적화 알고리즘의 반복 횟수(step 수)를 갖추고 있으며, 이를 통해 신경망의 학습 과정을 조정합니다.

- **Performance Highlights**: 실험 결과에 따르면, PACMANN은 낮은 차원 문제에서 기존의 최첨단(adaptive) 방법들과 동등한 성능을 보이며, 고차원 문제에서는 더욱 뛰어난 성능을 나타냅니다. 특히, Adam 최적화 알고리즘을 사용할 경우, 가장 우수한 결과를 기록했습니다. PACMANN은 저렴한 계산 비용과 기존의 PINN 파이프라인에 대한 통합의 용이성을 제공하여 향후 다양한 응용 분야에서 활용될 것으로 기대됩니다.



### Non-linear Equalization in 112 Gb/s PONs Using Kolmogorov-Arnold Networks (https://arxiv.org/abs/2411.19631)
Comments:
          Submitted for possible publication at Optical Fiber Communication Conference (OFC) 2025

- **What's New**: 이번 논문은 112 Gb/s PAM4 수동 광 네트워크(PON)에서 비선형 동등화(non-linear equalization)를 위한 Kolmogorov-Arnold Networks (KANs)의 사용을 조사합니다. 기존의 선형 동등기와 컨볼루션 신경망(CNN)보다 더 낮은 계산 복잡도로 성능을 초과하는 것을 목표로 합니다. KAN은 학습된 비선형 활성화 함수(non-linear activation function)를 통해 PON의 비선형 채널을 쉽게 보정할 수 있는 잠재력이 있습니다.

- **Technical Details**: KAN은 기존의 다층 퍼셉트론(MLP) 신경망과 달리 입력의 가중치 곱셈을 학습 가능한 1D 함수로 대체하는 구조를 가지고 있습니다. 이러한 구조는 각 레이어에서 데이터 간의 조합적 관계를 학습할 수 있게 하여, 비선형 채널의 보정에서 더 효과적으로 작용할 수 있도록 설계되었습니다. 논문에서는 KAN의 동작 방식과 다양한 아키텍처에 대한 설명을 포함하여, KAN의 특성과 구현 가능성을 상세히 다룹니다.

- **Performance Highlights**: KAN은 낮은 복잡성을 유지하면서도 기존의 FIR 선형 필터 및 CNN 기반 동등기들과의 비교에서 우수한 성능을 보여줍니다. 저자들은 다양한 실험을 통해 KAN이 비선형 동등화에 적합한 후보임을 강조하며, 특히 비용 효율적인 접근 네트워크 구현에 유리한 특성을 통해 그 가능성을 펼치고 있습니다.



### OpenQDC: Open Quantum Data Commons (https://arxiv.org/abs/2411.19629)
- **What's New**: 이 논문에서는 MLIP(머신러닝 상호 원자 포텐셜) 모델링에 필요한 37개의 양자역학(QM) 데이터셋을 통합한 OpenQDC 패키지를 소개하며, 이는 250개 이상의 양자 방법과 4억 개의 기하학적 구조로 구성된 단일 데이터 리포지토리입니다. OpenQDC는 MLIP 훈련을 위해 세심하게 전처리 및 표준화된 데이터셋을 제공하여 유기 화학에서 다루는 다양한 화학 원소와 상호작용을 포함하고 있습니다. 이 패키지는 Python을 통해 쉽게 접근 가능하며, 데이터의 정규화 및 통합을 위한 도구도 포함됩니다.

- **Technical Details**: MD(분자 동역학) 시뮬레이션은 약물 및 소재 발견에서 중요한 역할을 하여 생물 유기 및 무기 분자의 동적 행동을 이해하는 데 도움을 줍니다. 그러나 기존의 QM 데이터는 여러 저작권에 분산되어 있고, 사용에 있어 높은 장벽이 존재했던 실정입니다. 다수의 잘 알려진 아키텍처인 SchNet, TorchMD-Net 및 DimeNet을 활용한 실험이 이루어졌으며, 이는 MLIP 학습을 위한 도전 과제를 제시하고 있습니다.

- **Performance Highlights**: OpenQDC는 MLIP 모델링의 진전을 위한 중요한 전환점을 표방하며, HuggingFace, Torchvision 및 TDC와 유사한 생태계를 구축할 것입니다. 이 패키지는 사용자에게 새로운 데이터셋을 쉽게 추가할 수 있도록 하여, MLIP 연구의 효과성 및 진행을 가속화하려고 합니다. OpenQDC를 통해 양자역학 데이터셋의 접근성을 민주화하고, 협력과 혁신을 증진할 수 있는 가능성을 열고 있습니다.
...
### Materials Learning Algorithms (MALA): Scalable Machine Learning for Electronic Structure Calculations in Large-Scale Atomistic Simulations (https://arxiv.org/abs/2411.19617)
- **What's New**: 이번 논문에서는 대규모 원자 시뮬레이션에 적합한 밀도 함수 이론(density functional theory, DFT) 계산을 가속화하기 위해 설계된 Materials Learning Algorithms (MALA) 패키지를 소개합니다. MALA는 원자 환경의 지역적(descriptor) 정보를 이용하여 전자 밀도(electronic density), 상태 밀도(density of states), 총 에너지(total energy) 등 주요 전자 관측값을 효율적으로 예측합니다.

- **Technical Details**: MALA 패키지는 데이터 샘플링(data sampling), 모델 훈련(model training) 및 대규모 추론(scalable inference)을 통합한 라이브러리로, 표준 DFT 및 분자 역학(molecular dynamics) 코드와의 호환성을 보장합니다. 이 패키지는 보론 클러스터, 고체-액체 경계의 알루미늄, 대형 베릴륨 판의 스태킹 결함 전자 구조 예측을 포함한 다양한 사례를 통해 그 기능이 입증되었습니다.

- **Performance Highlights**: MALA의 성능 분석에서는 계산 효율성이 밝혀졌으며, 향후 최적화를 위한 병목 현상(bottlenecks)도 파악되었습니다. MALA는 표준 DFT 범위를 넘어선 전자 구조 모델링이 가능하여 복잡한 재료 시스템 모델링에 적합하여 고급 재료 연구의 다목적 도구로 자리잡고 있습니다.
...
### Topology-Preserving Scaling in Data Augmentation (https://arxiv.org/abs/2411.19512)
Comments:
          20 pages

- **What's New**: 이번 논문에서는 비균일 스케일링 변환(non-uniform scaling transformations)에 대한 데이터셋 정규화의 알고리즘적 프레임워크를 제안합니다. 이 프레임워크는 데이터 증강 파이프라인(data augmentation pipelines)에서 기본적인 위상적 특징(topological features)을 유지하도록 합니다. 주요 결과는 스케일 변환 전후의 지속적 다이어그램(persistence diagrams) 간의 병목 거리(bottleneck distance)가 특정 수식으로 제한된다는 것을 보여줍니다.

- **Technical Details**: 주어진 유한 메트릭 공간 X와 scaling factor를 기반으로 하는 스케일링 함수 S를 정의했습니다. 특히, 본 연구에서는 병목 거리와 스케일링 변동성(scaling variability)간의 관계를 분석하고, 특정 제약 하에 변동성을 최소화하는 최적화 문제를 정립했습니다. 이 문제 해결을 통해 데이터 증강 과정에서 위상적 특성이 유지될 수 있도록 합니다.

- **Performance Highlights**: 제안된 방법론은 스케일 변환을 통한 데이터 증강이 데이터의 중요한 위상적 특성을 보존하는 것을 보장합니다. 또한, Wasserstein distance와 같은 대안적 거리 측정 방법 및 고차 동형(homological) 특징을 포함한 확장된 분석을 제공합니다. 이 연구는 위상적 데이터 분석(Topological Data Analysis) 분야에서 의의 있는 기여를 하며, 데이터셋 정규화에 대한 엄격한 수학적 기초를 제공합니다.
...
### Real-time Anomaly Detection at the L1 Trigger of CMS Experimen (https://arxiv.org/abs/2411.19506)
Comments:
          Contribution to 42nd International Conference on High Energy Physics (ICHEP 2024)

- **What's New**: 이 논문은 CMS 실험의 Global Trigger(GT) 테스트 크레이트 FPGA에서 새로운 물리적 신호를 비편향적으로 탐지하기 위해 훈련된 오토인코더(autoencoder)의 준비, 배치 및 테스트 과정을 제시합니다. GT는 40MHz 속도로 발생하는 LHC 충돌 데이터의 수집 여부를 결정하며, 이를 위해 인공 신경망(Neural Network)을 이용하여 각 이벤트를 예측합니다. 이 접근 방식은 기존의 선택 기준에 의존하지 않으면서 잠재적으로 흥미로운 비정상 이벤트를 분리할 수 있도록 하기 위한 것입니다.

- **Technical Details**: 연구진은 두 가지 비정상 탐지 접근법인 AXOL1TL(Anomaly eXtraction Online Level-1 Trigger aLgorithm)과 CICADA(Calorimeter Image Convolutional Anomaly Detection Algorithm)를 개발하였습니다. 이 두 방법은 고급 머신 러닝 기술을 활용하여 비정상적인 이벤트를 실시간으로 식별합니다. AXOL1TL은 변형 오토인코더(Variational Autoencoder)를 기반으로 하여 Gaussian 분포의 잠재 공간 벡터를 학습하며, CICADA는 이미지 형태의 입력을 처리하기 위해 합성곱층(convolutional layers)을 사용하여 전송 에너지(Transverse energy deposit)를 분석합니다.

- **Performance Highlights**: AXOL1TL은 CMS 트리거 프레임워크에서 실시간 테스트 및 데이터 수집에 성공적으로 배치되었습니다. 이 트리거는 다섯 가지 서로 다른 임계값에서 비정상 이벤트를 유발하며, 정규 L1 트리거 메뉴와는 orthogonal한 데이터셋을 생성하여 안정적인 성능을 보였습니다. AXOL1TL은 고유한 배경 프로세스에 대해 높은 거부율을 유지하면서도 높은 효율성을 나타내며, Higgs boson의 이국적인 붕괴를 포착할 때는 46%의 효율 향상으로 측정되었습니다.
...
- **What's New**: 이 논문에서는 SANGO(Socially Aware Navigation through Grouped Obstacles)라는 새로운 방법을 소개합니다. SANGO는 장애물을 동적으로 그룹화하여 안전을 유지하면서 사회적 규범을 준수하는 경로 계획 방법입니다. 이를 통해 로봇이 복잡한 환경에서도 사회적으로 적절한 행동을 보일 수 있도록 훈련됩니다.

- **Technical Details**: SANGO는 동적이고 복잡한 환경에서 에이전트를 훈련시키기 위해 DBSCAN(Density-Based Spatial Clustering of Applications with Noise) 알고리즘을 사용합니다. 이 알고리즘은 장애물을 클러스터링하며, Proximal Policy Optimization(PPO)을 이용해 최적의 경로 계획을 수행합니다. 이러한 접근 방식은 사회적 규범을 준수하면서 충돌을 회피하고 적절한 거리를 유지하는 것으로 나타납니다.

- **Performance Highlights**: 상세한 실험 결과, SANGO는 불편함을 최대 83.5% 줄이고 충돌율을 최대 29.4% 감소시켰으며, 복잡하고 혼잡한 상황에서의 성공적인 경로 탐색률을 높이는 성과를 보였습니다. 이러한 결과는 실제 응용 가능성을 강조하며, 사회적 능력을 갖춘 로봇 내비게이션 시스템을 위한 기초를 마련합니다.



### Diffusion Models Meet Network Management: Improving Traffic Matrix Analysis with Diffusion-based Approach (https://arxiv.org/abs/2411.19493)
- **What's New**: 이 논문에서는 Diffusion-TM이라는 새로운 확산 기반( diffusion-based ) 트래픽 매트릭스 분석 프레임워크를 제안합니다. 이 프레임워크는 문제에 구애받지 않고 확산(diffusion)을 이용하여 트래픽 분포와 정확도에서 추정 성능을 크게 향상시키는 것을 목표로 합니다. 기존 기술들과는 달리, Diffusion-TM은 다양한 네트워크 환경에서도 효과적으로 작동할 수 있도록 설계되었습니다.

- **Technical Details**: Diffusion-TM은 강력한 생성 능력을 가진 확산 모델을 사용하여 현실적인 네트워크 트래픽을 생성합니다. 이 프레임워크는 이론적인 보장을 바탕으로 모든 엔드 투 엔드 트래픽을 비편향적으로 추정할 수 있도록 하는 디노이징 과정(denoising process)을 활용합니다. 또한, 완전한 트래픽 데이터셋을 구성하기 어려운 상황을 고려하여, 데이터셋의 결측값에 둔감하게 작동할 수 있는 두 단계(training scheme) 훈련 방식을 제안합니다.

- **Performance Highlights**: Diffusion-TM은 실제 데이터셋과 함께 광범위한 실험을 통해 여러 작업(Task)에서 효과성을 입증하였습니다. 특히, 데이터셋에서 알려진 값이 $5\%$만 남아있는 경우에도 유망한 결과를 얻을 수 있음을 보여주었습니다. 이러한 결과는 Diffusion-TM이 현대의 복잡하고 동적인 네트워크 통신 환경에서도 신뢰할 수 있는 성능을 제공할 수 있다는 것을 의미합니다.
...
### Unsupervised Learning Approach to Anomaly Detection in Gravitational Wave Data (https://arxiv.org/abs/2411.19450)
- **What's New**: 이 연구는 중력파(Gravitational Waves, GW) 시간 시리즈 데이터에서 이상 탐지를 위한 비지도 학습 방법을 제안합니다. Variational Autoencoders (VAEs)를 사용하여 노이즈 데이터에 대한 훈련을 통해, VAE는 정상적인 노이즈 입력을 정확하게 재구성할 수 있지만 이상 신호, 예를 들어 GW 신호는 재구성 실패를 보여줍니다. 이러한 재구성 오류는 이상 현상의 유무를 평가하는 데 사용될 수 있습니다.

- **Technical Details**: 이 연구는 General Relativity의 원리를 바탕으로 중력파의 공간-시간 기하학을 설명하며, Einstein Tensor와 스트레스-에너지 텐서의 개념을 다룹니다. LIGO 탐지기는 레이저 간섭계를 이용하여 중력파의 통과로 인해 발생하는 미세한 변화를 측정합니다. 모델에서는 Deep Autoencoder (AE)를 통해 데이터 간의 관계를 측정하며, AE는 입력 데이터를 저차원 잠재 변수로 변환하고 다시 복원하는 구조를 갖습니다.

- **Performance Highlights**: 제안된 방법은 LIGO H1 및 L1 탐지기의 데이터를 분석하여 신뢰할 수 있는 이상 탐지를 보여주었습니다. 테스트 데이터셋에서 GW 이벤트와 노이즈가 혼합된 결과, ROC 곡선 아래 면적(AUC)은 0.89를 기록하여 높은 검출 능력을 입증했습니다. 이 연구는 GW 데이터에서 이상 탐지를 위한 강력하고 비지도 학습 접근법으로서 VAEs의 가능성을 제시하고 신뢰할 수 있는 이상 현상 탐지 프레임워크를 제공합니다.
...
### Parameter-Efficient Transfer Learning for Music Foundation Models (https://arxiv.org/abs/2411.19371)
Comments:
          6+2 pages

- **What's New**: 최근 음악 기초 모델(music foundation models)이 개발되고 있으며, 이들은 음악 정보의 일반적이고 주로 작업 독립적인 인코딩을 약속하고 있습니다. 본 연구에서는 parameter-efficient transfer learning (PETL) 방식을 도입하여 프로빙(probing)과 파인튜닝(fine-tuning)의 장점을 통합하려고 합니다. 우리는 어댑터 기반(Adapter-based) 방법, 프롬프트 기반(Prompt-based) 방법, 재파라미터화 기반(Reparameterization-based) 방법 등 세 가지 PETL 방법을 소개합니다.

- **Technical Details**: 우리가 사용하는 파라미터 효율 전이 학습(parameter-efficient transfer learning) 접근 방식은 세 가지 유형으로 분류됩니다. 어댑터 기반 방법은 원래 모델에 추가된 모듈로, 입력된 히든 상태를 새로운 히든 상태로 변환하는 역할을 합니다. 프롬프트 기반 방법은 입력 시퀀스의 시작에 학습 가능한 추가 매개변수를 추가하여, 모델이 특정 작업에 대한 지식을 학습하는 데 도움을 줍니다.

- **Performance Highlights**: PETL 방법은 음악 자동 태깅(music auto-tagging)에서 프로빙과 파인튜닝을 초과하는 성능을 보였습니다. 또한 키(key) 감지 및 템포(tempo) 추정에서는 파인튜닝과 유사한 결과를 달성하면서도 훨씬 적은 훈련 비용을 소모했습니다. 그러나 현재 기초 모델의 유용성에 대한 의문이 제기되며, 소규모 모델을 처음부터 훈련한 결과와 유사한 성능이 관측되었습니다.



### Machine learning the Ising transition: A comparison between discriminative and generative approaches (https://arxiv.org/abs/2411.19370)
Comments:
          11+5 pages, 4+4 figures

- **What's New**: 이번 연구는 다체 물리학에서의 상 전이 탐지 문제를 자동화하려는 새로운 접근 방식을 제안합니다. 상 다이어그램을 매핑하는 과정을 분류 문제로 정의하고, 이 문제를 해결하기 위한 두 가지 방법인 변별적(discriminative) 접근법과 생성적(generative) 접근법을 다룹니다. 이 연구는 특히 고전적인 2차원 정사각형 격자 강자성(Ising) 모델에서의 열적 상 전이를 사용할 다양한 설정을 통해 이에 대한 해결책을 모색합니다.

- **Technical Details**: 연구에서는 각 상 전이에 대한 스칼라 인디케이터(I(γ))를 기반으로 한 세 가지 접근 방식을 제시합니다. 이 인디케이터의 지역 극대값은 상 경계를 나타내며, 각각의 접근 방식은 분류 작업의 특정 방식을 적용하여 인디케이터를 계산합니다. 두 가지 설정, 즉 데이터 기반(data-driven)과 지식 기반(knowledge-driven)의 경우를 고려하여 상 전이를 자동으로 탐지하는 방법을 탐구합니다.

- **Performance Highlights**: 이 연구에서 제안된 방식은 특히 상 경계를 정확하게 탐지하는 데 효과적입니다. 각각의 접근 방식은 이론적으로 Fisher 정보의 하한(underapproximator)을 따르며, 이는 상 전이를 정확하게 식별하는 데 기여합니다. 결과적으로, 제안된 알고리즘은 실험적으로 접근할 수 있는 복잡한 물리적 시스템을 다루는 데 있어 높은 잠재력을 보여줍니다.
...
### LD-EnSF: Synergizing Latent Dynamics with Ensemble Score Filters for Fast Data Assimilation with Sparse Observations (https://arxiv.org/abs/2411.19305)
- **What's New**: 이 논문에서는 Latent Dynamics Ensemble Score Filter (LD-EnSF)라는 새로운 데이터 동화(data assimilation) 방법론을 소개합니다. LD-EnSF는 기존의 Latent Ensemble Score Filter (Latent-EnSF)의 한계를 극복하며 고차원 비선형 데이터 동화 문제에서 빠르고 효율적인 데이터 동화 프로세스를 가능하게 합니다. 이 접근법은 모든 데이터 동화 단계에서 잠재 상태와 관측값을 효과적으로 캡처하고 유지하기 위해 새로운 Latent Dynamics Networks (LDNets) 변형을 도입합니다.

- **Technical Details**: LD-EnSF는 고차원 동적 문제에서 전체 동역학을 진화시키는 대신 잠재 공간(latent space)에서 데이터 동화를 수행하도록 설계되었습니다. 이는 비용이 많이 드는 전체 동역학 과정을 완전히 회피하여 데이터 동화 과정을 크게 가속화할 수 있습니다. 또한, LSTM(Long Short-Term Memory) 네트워크를 활용하여 희소한 관측값을 잠재 공간으로 인코딩하는 새로운 방법을 제안합니다. 이를 통해 현재 단계의 관측값뿐만 아니라 이전 단계의 모든 정보를 활용하여 관측 인코딩의 정확성과 강건성을 향상시킵니다.

- **Performance Highlights**: 제안된 LD-EnSF 방법은 두 가지 도전적인 동적 시스템에 대해 높은 희소성과 잡noise가 포함된 관측값에도 불구하고 강력한 정확성과 효율성을 입증했습니다. LD-EnSF는 EnSF 및 Latent-EnSF와 비교하여 고차원 데이터 동화 문제에서 현저한 가속화 효과를 보여, 실제 응용에서의 활용 가능성을 높였습니다. 실험 결과, LD-EnSF 방법은 특히 복잡한 동적 문제에 대한 데이터 동화의 신뢰성을 크게 향상시켰습니다.



### Perspective of Software Engineering Researchers on Machine Learning Practices Regarding Research, Review, and Education (https://arxiv.org/abs/2411.19304)
Comments:
          under review

- **What's New**: 이 논문은 소프트웨어 엔지니어링(SE) 분야의 연구자들이 기계 학습(ML)의 응용을 연구하고 가르치는 과정에서 겪는 관행과 도전 과제를 조명합니다. 주요 기여는 연구자, 교육자 및 리뷰어의 관점을 포함하여 ML과 SE의 시너지에 대한 통찰력을 제공하는 것입니다. 그 결과, SE에서 ML을 적용하는 데 있어 여전히 존재하는 공백과 개선의 필요성이 강조됩니다.

- **Technical Details**: 연구는 SE 연구자들이 ML을 어떻게 연구하고 교육하며 검토하는지를 분석하기 위해 질적 분석(qualitative analysis)과 근거 이론 코딩(grounded theory coding)을 활용했습니다. 주요 초점은 데이터 수집, 모델 훈련 및 평가와 같은 다양한 관행, 그리고 그 과정에서 직면하는 도전 과제를 조사하는 것입니다. 특정 ML 기술 및 비기능적 속성을 포함한 모델 평가와 같은 요소들이 평가에 큰 영향을 미치고 있음을 발견했습니다.

- **Performance Highlights**: 연구 결과, SE 연구자들은 다양한 데이터 처리 및 모델 평가 방법을 활용하지만, 최적 매개변수 튜닝(hyperparameter tuning)과 같은 권장 관행은 20% 미만의 문헌에서만 등장하는 것으로 나타났습니다. 교육 현장에서는 실습 활동이 일반적이나, 전통적인 방법들도 여전히 사용되고 있습니다. 이러한 결과는 SE 커뮤니티가 ML 응용에 있어 최적의 관행을 수립하고, 다양한 교육 방법을 채택하여 공백을 해소해야 함을 시사합니다.
...
### The role of data-induced randomness in quantum machine learning classification tasks (https://arxiv.org/abs/2411.19281)
Comments:
          23 pages, 6 figures

- **What's New**: 논문에서는 양자 기계 학습(Quantum Machine Learning, QML)의 새로운 접근 방식을 제안합니다. 특히 이들은 바이너리 분류 작업에 초점을 맞추고 있으며, 클라스 마진(class margin)이라는 새로운 메트릭을 소개합니다. 이는 데이터 임베딩(data embedding) 과정을 평가하는 데 효과적인 방법을 제공해주며, 기존의 분석 도구에서 발생하는 격차를 해결할 것으로 기대됩니다.

- **Technical Details**: 연구 프레임워크 내에서 QML 알고리즘은 두 가지 구성 요소로 나뉩니다: (i) 데이터를 양자 상태로 변환하는 임베딩 맵(embedding map)과 (ii) 데이터에 의해 유도된 양자 상태의 기대값을 측정하기 위한 오브저버블(observable)입니다. 이를 통해 다양한 데이터 임베딩 전략을 비교하고, 클라스 마진 메트릭을 통해 이러한 전략의 분류 성능을 평가할 수 있습니다.

- **Performance Highlights**: 이 연구는 데이터 유도 랜덤성(data-induced randomness)이 분류 성능에 어떤 제한을 미치는지 분석했습니다. 이 메트릭을 활용하여 다양한 임베딩 전략을 벤치마킹하고, 양자 알고리즘을 평가하는 새로운 지표를 제시하였습니다. 이러한 접근은 QML 모델의 데이터 임베딩 프로세스를 평가하는 데 있어 중요한 기초가 될 것으로 보입니다.
...
### Quantum feedback control with a transformer neural network architectur (https://arxiv.org/abs/2411.19253)
Comments:
          9 pages, 4 figures

- **What's New**: 최근의 연구에서는 quantum feedback control을 위해 transformers를 활용하는 방법을 제시했습니다. 이 방식은 recurrent neural networks와 reinforcement learning을 사용하는 기존 방법의 한계를 극복할 수 있는 잠재력을 보여줍니다. 특히, transformers는 긴 시계열 상관관계를 잘 포착하고 훈련 효율성이 높아 빠른 시간 내에 목표 상태에 대한 unit fidelity를 달성할 수 있습니다.

- **Technical Details**: 본 연구에서는 수학적으로 PaQS 접근법을 사용하여 시스템 최적 제어를 다룹니다. 이 방법은 계측 효율성을 고려하여 state evolution과 feedback 없이 진화를 분리합니다. 이를 통해 전체 시스템과 반응 좌표의 결합 상태를 밀도 행렬로 취급하여 표준 Markovian master equation 기법을 적용할 수 있습니다.

- **Performance Highlights**: 제안된 transformer 아키텍처는 두 수준 시스템의 상태 안정화 예제를 통해 테스트되었으며, 비효율적인 측정과 Hamiltonian 동란이 포함된 경우에도 빠르게 목표 상태에 도달하는 능력을 입증했습니다. 이 방법은 non-Markovian 시스템의 제어에도 잘 일반화되며, quantum error correction 및 현실 시간의 양자 장치 조정에 유용하게 활용될 수 있습니다.
...
### ABROCA Distributions For Algorithmic Bias Assessment: Considerations Around Interpretation (https://arxiv.org/abs/2411.19090)
Comments:
          Accepted to Learning Analytics and Knowledge (LAK 2025)

- **What's New**: 이 연구에서는 학습 분석에서 알고리즘 편향(algorithmic bias)의 문제를 해결하기 위해 Absolute Between-ROC Area (ABROCA) 지표의 통계적 특성을 연구하고 있다. ABROCA는 분류 모델의 성능에서 발생하는 그룹 간의 차이를 정량화하며, 비슷한 AUC 값을 가진 경우에도 미세한 성능 차이를 탐지할 수 있는 유용한 도구이다. 연구에서는 샘플 크기와 클래스 불균형(class imbalance)과 같은 다양한 조건에서 ABROCA의 분포를 샘플링하여, 데이터의 분포 특성을 이해하는 방법을 제시한다.

- **Technical Details**: ABROCA는 ROC 곡선(ROC curve) 간의 절대적 차이를 기반으로 하여 분류기의 성능을 평가한다. 연구에서는 AUC ROC의 차이가 없거나 적을 때 ABROCA의 분포를 연구하고, 샘플 크기와 AUC의 진짜 차이에 따라 ABROCA가 어떻게 변하는지를 체계적으로 조사하였다. 특히, 클래스 불균형 상황에서 ABROCA의 상관성과 그 불균형이 성능 추정에 미치는 영향을 분석하며, 이로 인해 불균형 데이터에서 알 수 없는 편향의 가능성을 탐구한다.

- **Performance Highlights**: 연구 결과, ABROCA는 샘플 크기가 작고 결과 클래스 또는 소수 클래스가 불균형할 때 높은 편향을 보이는 경향이 있음을 발견했다. 이러한 경향은 알고리즘 편향을 구별하는 데 있어 신뢰성을 떨어뜨린다. 반면에, 샘플 크기가 커질 경우 ABROCA는 AUC의 차이에 수렴하지만, 여전히 ABROCA 값의 해석에 주의가 필요하다는 점을 강조한다.
...
### Intrinsic Wrapped Gaussian Process Regression Modeling for Manifold-valued Response Variab (https://arxiv.org/abs/2411.18989)
- **What's New**: 이 논문에서는 Riemannian manifold에서 측정된 반응 변수를 위한 새로운 본질적인 wrapped Gaussian process regression 모델을 제안합니다. 이 모델은 parallel transport operator를 적용하여 본질적인 공분산 구조를 정의하며, 이는 Gaussian process regression 모델을 구성할 때 중요한 요소를 다룹니다. 제안된 방법은 Euclidean submanifold뿐만 아니라 자연스러운 주변 공간이 없는 manifold에도 적용될 수 있습니다.

- **Technical Details**: 제안된 모델은 D차원의 Riemannian manifold에 대해 정의되며, 각 점 p에서 내적을 정의하는 Riemannian metric을 갖추고 있습니다. 이 모델은 공분산 함수의 표현에서 공통의 tangent space와 좌표 프레임의 선택에 불변성을 유지하며, 제안된 모형의 비대칭성과 posterior consistency와 같은 비대칭 성질을 수립합니다. parallel transport operator의 혁신적 사용은 covariance 구조가 manifold의 본질적인 기하 정보를 보존할 수 있게 합니다.

- **Performance Highlights**: 시뮬레이션 연구와 실제 데이터 분석 결과는 제안된 방법이 강력한 성능을 제공함을 강조합니다. 특히, 이 모델은 비선형 및 비모수 관계를 효율적으로 모델링할 수 있는 능력을 보여 주며, 다양한 중요한 Riemannian manifold에서도 적용 가능함을 입증합니다. 이 연구는 통계 분석 및 머신 러닝 분야에서 일반적으로 접할 수 있는 여러 Riemannian manifolds에서의 활용도를 높일 것으로 기대됩니다.
...
### Neural Operators for Predictor Feedback Control of Nonlinear Delay Systems (https://arxiv.org/abs/2411.18964)
Comments:
          22 pages, 2 figures

- **What's New**: 이번 논문에서는 비선형 시스템의 지연 보상 제어기에서 중요한 예측 피드백 설계를 다룬다. 기존의 아날로그 예측기보다 더 효율적으로 수치 근사 방식을 사용하여 예측기 매핑을 신경 연산자(neural operator)로 근사하는 새로운 접근 방식을 제안한다. 이 연구는 예측기 설계를 연산자 학습 문제로 재구성하여 고차원에서의 안정성을 높일 수 있는 가능성을 탐구한다.

- **Technical Details**: 제안된 방법론은 신경 연산자가 예측 연산자의 근사가 가능하다는 것을 증명하고, 이를 통해 닫힌 루프 비선형 시스템의 반글로벌 실용 안정성(semi-global practical stability)을 달성한다. 반글로벌이라는 표현은 초기 상태의 집합을 원하는 만큼 크게 증가시키는 것이 가능하나, 이는 신경 연산자 근사 훈련의 어려움을 증가시킨다는 것을 의미한다.

- **Performance Highlights**: 본 연구에서는 5링크 로봇 매니퓰레이터 제어 실험을 수행하였으며, 최신 신경 연산자 아키텍처를 통해 전통적인 예측기 근사 방식보다 $10^2$ 배의 속도 향상을 Demonstrating하였다. 이러한 결과는 신경 연산자 접근 방식이 훈련이 한번만 필요하고, 피드백 제어기에서 매우 적은 계산 비용으로 배포될 수 있는 장점을 가지고 있음을 강조한다.
...
### MSEMG: Surface Electromyography Denoising with a Mamba-based Efficient Network (https://arxiv.org/abs/2411.18902)
Comments:
          This paper is under review of 2025 ICASSP

- **What's New**: 이번 연구에서는 sEMG(noise 제거) 시스템인 MSEMG를 소개합니다. MSEMG는 Mamba State Space Model과 CNN(convolutional neural network)을 통합하여 경량의 신경망 아키텍처를 제공합니다. 기존의 방법들과 비교했을 때, MSEMG는 더 적은 파라미터로 더 높은 품질의 sEMG 신호를 생성하는 성능을 보여주었습니다.

- **Technical Details**: MSEMG는 Mamba SSM(state space model)의 선택적 스캔 메커니즘을 사용하여 입력 신호를 출력 신호로 적응적으로 매핑합니다. 이 모델은 학습 가능한 파라미터와 상태 전이 행렬, 입력 및 출력 프로젝션 행렬을 포함하며, 숨겨진 상태의 차원은 문제의 복잡성에 따라 결정됩니다. 이 연구는 연속 신호의 상태 전환 과정을 수식으로 설명하며, 실험의 실제 신호는 주로 이산 신호로 존재함을 강조합니다.

- **Performance Highlights**: 실험 결과에 따르면, MSEMG는 여러 평가 지표에서 기존의 sEMG noise 제거 기법들보다 우수한 성능을 나타냈습니다. 또한, MSEMG는 최신 sEMG noise 제거 방법보다 파라미터 수가 적어 효율성을 높였습니다. 이 연구는 Mamba의 sEMG 처리력이 처음으로 탐구된 사례로, 향후 다양한 sEMG 응용 분야에서 유망한 솔루션이 될 것으로 기대됩니다.
...
### Automating Energy-Efficient GPU Kernel Generation: A Fast Search-Based Compilation Approach (https://arxiv.org/abs/2411.18873)
- **What's New**: 이 논문에서는 Deep Neural Networks (DNNs)의 GPU 에너지 소비 문제를 해결하기 위해 새로운 검색 기반 컴파일 방법을 제안합니다. 에너지 효율성을 검색 과정에 통합하여 에너지 효율이 높은 GPU 커널을 생성하는 이 방안을 통해, 최대 21.69%의 에너지 소모를 줄일 수 있습니다. 특히, 높은 수준의 커널 특징을 기반으로 하는 정확한 에너지 비용 모델을 개발하여 에너지 평가 과정을 가속화합니다.

- **Technical Details**: 이를 통해 제안된 방법은 GPU 커널 생성을 위한 첫 번째 검색 기반 에너지 인식 프레임워크를 개발하였습니다. 또한 커널의 에너지 소비를 예측하는 효율적인 에너지 비용 모델을 도입하고, 온라인 업데이트 전략을 통해 검색 과정을 가속화하면서 검색 품질을 유지합니다. 논문은 GPU의 Streaming Processor (SP)와 Streaming MultiProcessor (SM)에 대한 배경도 제공합니다.

- **Performance Highlights**: 제안된 방법을 통해 생성된 GPU 커널은 낮은 대기 시간과 함께 에너지 소비를 효과적으로 줄일 수 있습니다. 예를 들어, 메타는 24,576개의 NVIDIA H100 GPU가 포함된 클러스터에서 LLaMA-3를 훈련했으며, 이로 인해 보고된 에너지 소비 문제의 심각성을 부각시키고 있습니다. 실험 결과, 제안한 방법이 기존의 탐색 기반 커널 생성 방법보다 에너지 효율성을 높이면서 성능도 향상시킬 수 있음을 보여주었습니다.
...
### ELEMENTAL: Interactive Learning from Demonstrations and Vision-Language Models for Reward Design in Robotics (https://arxiv.org/abs/2411.18825)
- **What's New**: ELEMENTAL (intEractive LEarning froM dEmoNstraTion And Language)는 자연어 지침과 시각적 사용자 시연을 결합하여 로봇의 행동을 사용자 의도에 더 잘 맞추도록 설계된 새로운 프레임워크입니다. 이 프레임워크는 텍스트 기반 설명만으로는 부족한 한계를 극복하고, 역강화학습(inverse reinforcement learning, IRL)을 사용하여 특성 가중치를 균형 있게 조정하고 시연된 행동과 최적으로 일치시킵니다. 또한 자가 반영(self-reflection) 피드백 루프를 도입하여 특성, 보상 및 정책 학습을 개선합니다.

- **Technical Details**: ELEMENTAL은 시각적 입력과 언어 모델을 통합하여 로봇이 사용자 시연에서 주요 작업 특성을 식별하고, 이러한 특성에 맞는 보상과 정책을 학습하며, 성과를 반복적으로 반영하여 지속적으로 개선할 수 있도록 합니다. 이 방법론은 로봇이 시연을 관찰하고, 시연과 행동 간의 불일치를 반영하며, 성과를 향상시키기 위한 프로세스를 통해 인간 학습과 유사한 방식을 따릅니다. 이를 통해 로봇은 리워드 컴포넌트와 시연된 행동의 매칭을 통해 보상의 상관관계를 더욱 효과적으로 계산할 수 있습니다.

- **Performance Highlights**: 실험 결과, ELEMENTAL은 작업 성공률에서 이전 방법들보다 42.3% 더 뛰어난 성과를 보였고, 분포 외(out-of-distribution) 작업에서 41.3% 더 나은 일반화 능력을 달성했습니다. 이러한 결과는 ELEMENTAL이 LfD(학습으로부터의 시연)에서의 강력함과 효과성을 잘 보여줍니다. 특히, ELEMENTAL은 IsaacGym에서의 어려운 로봇 벤치마크에서 탁월한 성능을 입증했습니다.
...
### Graph Max Shift: A Hill-Climbing Method for Graph Clustering (https://arxiv.org/abs/2411.18794)
- **What's New**: 이 논문은 그래프 클러스터링을 위한 새로운 방법을 제시하며, 공간에서의 포인트 클러스터링을 위해 이전에 제안된 경량 상승 방법과 유사한 접근 방식을 사용합니다. 이 방법은 무작위 기하 그래프(random geometric graph)에 적용할 경우 점들이 어떤 밀도(density)에서 독립적으로 생성되는 경우 점진적 일관성을 보여줍니다. 특히, 이 연구에서는 밀도의 메인 모드의 매력 분지에 의해 유도된 밀도가 주어진 그래프의 클러스터링을 일관되게 만드는 방법을 기술합니다.

- **Technical Details**: 해당 연구에서는 무향 그래프의 노드와 인접 행렬을 정의하고, 노드 집합의 이분화를 통해 자른 값을 최소화하는 과정을 설명합니다. 알고리즘은 그래프를 초기화한 후, 현재 이분화에서 두 노드를 교환하여 절단 값을 최소화하는 방식으로 작동합니다. 논문에서는 이 접근이 더욱 효과적이라는 점을 강조하며, 경량 상승 흐름(grandient ascent flow)을 통한 클러스터링과의 연결도 추가적으로 설명합니다.

- **Performance Highlights**: 제안된 알고리즘(그래프 최대 이동(Graph Max Shift)은 기하 그래프에서 수렴적으로 매력 분지에 따라 클러스터링을 수행할 수 있음을 입증합니다. 실험을 통해 알고리즘이 실제 데이터에 잘 적용됨을 보여주며, 다양한 시뮬레이션을 통해 결과를 뒷받침합니다. 코드가 공개되어 있어 연구자들이 실험을 재현할 수 있는 기회를 제공합니다.
...
### Inference Privacy: Properties and Mechanisms (https://arxiv.org/abs/2411.18746)
- **What's New**: 이 논문은 추론 단계에서 사용자의 개인정보를 보호하기 위한 새로운 개념인 Inference Privacy (IP)를 도입합니다. 기존의 Differential Privacy (DP) 프레임워크는 주로 훈련 데이터의 보호에 중점을 두었지만, 본 연구에서는 추론 단계에서의 개인정보 보호 메커니즘을 체계적으로 제안합니다. IP는 사용자가 모델과 상호 작용할 때, 예를 들어 분류기 또는 AI 지원 챗봇을 사용할 때 사용자의 데이터에 대한 엄격한 개인 정보 보호 보장을 제공합니다.

- **Technical Details**: IP의 기본 속성으로는 사후 처리(post-processing), 합성(composition), 연결 속성(chaining property) 등이 포함됩니다. 이 메커니즘은 입력 변조(input perturbation)와 출력 변조(output perturbation) 두 가지로 나뉘며, 사용자가 유틸리티와 개인 정보 보호 간의 균형을 맞출 수 있도록 맞춤화가 가능하도록 설계되었습니다. IP는 주로 모델의 민감도에 따라 제어된 노이즈를 출력에 추가하여 기밀 정보를 유출하지 않도록 방지합니다.

- **Performance Highlights**: 실험을 통해 IP 메커니즘의 유용성을 입증 하였으며, 유틸리티와 개인 정보 보호 간의 트레이드오프를 강조합니다. 연구 결과, 높은 추론 개인 정보 보호 요건은 종종 유틸리티 감소와 연결된다는 점을 발견하였습니다. 다양한 데이터 세트에서 IP 요구 사항과 방법에 따른 사전 훈련된 모델의 유틸리티를 비교하여 전반적으로 유용성을 평가하였습니다.
...
### A quantum inspired predictor of Parkinsons disease built on a diverse, multimodal datas (https://arxiv.org/abs/2411.18640)
Comments:
          20 pages, 3 figures, 1 table

- **What's New**: 이번 연구에서는 전통적인 단일 특성 (single feature) 기반 분류의 한계를 극복하기 위해 다양한 생체 신호를 사용한 파킨슨병 조기 진단 모델을 제안하였습니다. mPower 데이터베이스를 활용하여 목소리 (voice), 보행 (gait), 두드림 (tapping), 그리고 인구통계 데이터 (demographic data)를 포함한 150,000개의 샘플에서 64개의 특징을 추출했습니다. 이를 통해 자원 집약적 양자 컴퓨터 (quantum computers)가 아닌 일반 하드웨어에서 실행할 수 있는 시뮬레이터 양자 서포트 벡터 머신 (qSVM)을 설계하였습니다.

- **Technical Details**: 우리는 80번째 백분위수 (80th percentile) 이상의 특징을 선택하기 위해 기본 랜덤 포레스트 모델 (Random Forest model)을 훈련시키고, 고차원 패턴 (high-dimensional patterns)을 탐지할 수 있는 qSVM을 구현했습니다. 이 모델은 최근의 양자 기계 학습 (quantum machine learning) 발전을 활용하여 설계되었습니다. 연구에서 측정된 64개의 특성의 조합을 통해 다양한 표본의 변동성을 효과적으로 포착할 수 있는 특징 기반 분류 프레임워크 (classification framework)를 구축하였습니다.

- **Performance Highlights**: 새로운 시뮬레이터 아키텍처는 90퍼센트의 정확도 (accuracy)와 0.98의 AUC (Area Under the Curve) 성능으로 기존 벤치마크 모델들을 초월했습니다. 이러한 성과는 파킨슨병의 조기 선별 (screening)을 위한 접근 방안을 제공하며, 전 세계적으로 서비스 가능함을 강조합니다. 다양한 특성 세트를 활용한 혁신적인 분류 모델은 향후 연구 및 실제 응용 가능성을 높일 것으로 기대됩니다.



### Topological Approach for Data Assimilation (https://arxiv.org/abs/2411.18627)
Comments:
          16 pages, 13 figures

- **What's New**: 이 논문에서는 고급 물리 기반 모델을 사용하기 어려운 동적 시스템을 위해 새로운 데이터 동화(data assimilation) 알고리즘을 제안합니다. 이 알고리즘은 위상 데이터 분석(topological data analysis)에 기반하여 개발되었으며, 기존의 잡음 통계 정보를 이용하지 않고도 모델 계수를 조정하는 방식으로 예측의 정확도를 높입니다. 새로운 접근 방식은 측정값과 예측값 간의 위상적 차이를 최소화하기 위해 경량 최적화 기법을 사용합니다.

- **Technical Details**: 제안된 방법은 persistence 함수의 미분 가능성을 활용하여 Gradient Descent 최적화를 통해 새로운 측정치를 바탕으로 데이터 기반 모델 계수를 조정합니다. 즉, 모델 예측과 측정 간의 위상적 차이를 최소화하면서 잡음 정보 없이 최적의 예측을 가능하게 합니다. 예를 들어, 혼돈 시스템인 Lorenz 시스템을 사용하여 이 방법의 성능을 집중적으로 분석합니다.

- **Performance Highlights**: 이 연구에서는 고전적인 데이터 동화 방법과의 비교를 통해 제안된 TADA(Topological Approach for Data Assimilation) 방법이 뛰어난 예측 성능을 보임을 강조합니다. 전통적인 회귀 분석 및 Kalman 필터와의 통합을 통해 이 알고리즘은 다양한 시스템의 상태를 더욱 효율적으로 추정하고 업데이트할 수 있습니다. 특히, 입력 잡음에 대한 견고함이 향상되어 변동성이 큰 데이터를 다루는 응용 분야에서 혁신적인 가능성을 보여줍니다.



### Diffeomorphic Latent Neural Operators for Data-Efficient Learning of Solutions to Partial Differential Equations (https://arxiv.org/abs/2411.18014)
- **What's New**: 이 논문에서는 다양한 기하학적 도메인에서의 고충실도 데이터에 대해 훈련된 후, 잠재 신경 연산자(latent neural operator)를 이용하여 편미분 방정식(PDE) 솔루션 연산자를 일반화하는 방법을 제안합니다. 기존의 방법들이 기하학적 변화를 포함한 충분한 데이터를 요구하는 것과 달리, 이 접근法은 특정한 참조 구성(reference configuration)으로 사상(mapping)된 몇 가지 솔루션 필드에만 의존합니다. 이를 통해 데이터 요구량을 크게 줄이면서 정확한 모델을 달성할 수 있습니다.

- **Technical Details**: 신경 연산자(neural operators)는 비선형 연속 연산자를 근사하기 위해 설계된 신경망의 일종입니다. 이 논문에서는 2D 라플라스 방정식에 대한 다양한 사상의 효과를 분석하며, 특히 수학적 성질을 보존하는 사상(mapping)이 학습의 정확성과 효율성에 중요한 영향을 미친다는 점을 강조합니다. 신경 연산자는 입력 함수 공간과 출력 함수 공간 사이의 매핑을 학습하며, 이 과정에서 기하적 변화와 매핑 선택이 결과에 미치는 영향을 실험을 통해 보여줍니다.

- **Performance Highlights**: 간단한 실험을 통해 다항식 흐름 솔루션을 정형화된 도메인에 매핑함으로써 매핑 선택과 PDE 솔루션의 변환 표현(transformed representation) 간의 관계를 분석합니다. 변환된 표현에서의 학습 효율성은 사상의 성질에 크게 의존함을 입증하였으며, 이는 고충실도의 수치 시뮬레이션 및 신경 연산자 훈련에서 도움을 줄 수 있는 새로운 방향을 제시합니다. 이 연구는 데이터 효율성을 높이는 데 기여할 수 있는 매핑 설계의 중요성을 강조합니다.

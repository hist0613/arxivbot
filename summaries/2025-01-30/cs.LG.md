...
### rEGGression: an Interactive and Agnostic Tool for the Exploration of Symbolic Regression Models (https://arxiv.org/abs/2501.17859)
Comments:
          9 pages, 4 figures, 2 tables

- **What's New**: 본 논문은 rEGGression이라는 상징 회귀(Symbolic Regression) 도구를 소개합니다. 이 도구는 e-graphs 구조를 사용하여 다양한 상징 표현을 탐색하고, 쿼리, 필터링, 그리고 패턴 매칭 기능을 제공합니다. 이를 통해 사용자는 SR 모델에 대한 통찰을 얻는데 도움을 받을 수 있습니다.

- **Technical Details**: Regression 분석은 독립 변수가 종속 변수에 미치는 영향을 이해하고 예측하기 위해 사용됩니다. 상징 회귀는 비선형 회귀 모델을 자동으로 검색하는 방법으로, e-graph는 중복된 부분을 효율적으로 처리하여 대량의 표현을 압축하여 저장합니다. rEGGression은 다양한 SR 솔루션 후보를 탐색할 수 있게 해주며, 패턴 매칭 알고리즘을 통해 사용자가 원하는 특정 패턴을 포함하거나 배제하는 모델을 쉽게 찾을 수 있습니다.

- **Performance Highlights**: rEGGression은 과거에 visited 된 표현들을 쉽게 탐색할 수 있게 하여, 주어진 데이터의 다른 표현을 분석할 수 있는 풍부한 라이브러리를 제공합니다. 이 도구는 모델 크기, 복잡성, 그리고 숫자 매개변수 수에 따라 필터링할 수 있는 쿼리 기능을 지원하며, 일반적인 패턴의 빈도를 계산하는 고급 쿼리 또한 가능합니다. 최종적으로, 이 도구는 e-graph를 통하여 불필요한 과매개변수화를 방지하고 적절한 표현을 탐색하는 데 기여합니다.



### Improving Genetic Programming for Symbolic Regression with Equality Graphs (https://arxiv.org/abs/2501.17848)
Comments:
          10 pages, 5 figures, 5 tables

- **What's New**: 이번 연구는 genetic programming (GP)을 이용한 symbolic regression (SR) 모델 탐색에서, 동일한 수학적 표현을 반복적으로 평가하는 비효율성을 제거하기 위한 방법을 제시합니다. 연구진은 e-graph라는 데이터 구조를 활용하여 모든 방문한 표현을 저장하고, 이미 방문한 표현을 생성할 가능성이 있는 서브트리의 조합을 필터링하여 새로운 표현을 생성할 가능성을 높입니다. 이를 통해 simple GP 알고리즘의 성능을 PySR 및 Operon과 경쟁할 수 있는 수준으로 향상시켰습니다.

- **Technical Details**: 이 연구에서 제안하는 기법 eggp는 방문한 모든 표현을 e-graph에 저장하여, 선택 가능한 서브트리에서 이미 방문한 표현을 생성할 조합을 차단합니다. 이를 통해 교차와 변이 연산 시 새로운 표현을 생성할 가능성을 높입니다. 이 방법은 tinyGP 기반의 GP 구현에서 시험되었으며, Operon 및 PySR와 비교하여 성능이 현저히 향상되었습니다.

- **Performance Highlights**: egg-p는 실험에서 SRBench와 여러 실제 데이터셋을 사용하는 벤치마크에 대해 짧고도 정확한 모델을 안정적으로 생성할 수 있었습니다. 특히, small expressions의 경우 simple GP 알고리즘의 성능이 향상되어 경쟁력 있는 결과를 달성했습니다. 이러한 결과들은 e-graph가 symbolic regression 및 GP 연구에 새로운 가능성을 제공함을 보여줍니다.
...
### Hierarchical Fallback Architecture for High Risk Online Machine Learning Inferenc (https://arxiv.org/abs/2501.17834)
- **What's New**: 이 논문은 Open Banking 환경에서 머신러닝 응용 프로그램의 견고성을 개선하기 위한 계층적 대체 아키텍처를 제안합니다. 이는 금융 도메인에서 발생할 수 있는 다양한 실패 시나리오에 대응하기 위한 것입니다. 특히, 외부 데이터 제공자에 의존하는 온라인 추론의 일반적인 실패 시나리오를 정의하고 이에 대한 해결책을 제시합니다.

- **Technical Details**: 논문은 데이터 제공자의 불가용성이나 부정확성으로 인한 데이터 결여, 모델 추론의 불가능성, 응답 지연 증가 등의 네 가지 실패 시나리오에 초점을 맞춥니다. 계층적 대체 아키텍처는 하나의 주요 모델과 여러 개의 대체 모델로 구성되어 있으며, 각 모델은 특정 실패 상황에서 작동합니다. 또한, 이러한 대체 모델을 개발하고 온라인 추론 환경에서 조직하는 방법을 설명합니다.

- **Performance Highlights**: 실제 Open Banking 데이터를 활용한 거래 사기 리스크 평가의 사례 연구를 통해 이 아키텍처의 효과성을 입증합니다. 대체 모델의 구조적 접근법은 높은 스트레스 상황에서도 응답 시간 요구를 충족하도록 설계되어, 금융 기관의 위험 평가 및 의사 결정 과정에서 신뢰성을 높이는 데 기여합니다.



### Langevin Soft Actor-Critic: Efficient Exploration through Uncertainty-Driven Critic Learning (https://arxiv.org/abs/2501.17827)
Comments:
...
- **What's New**: 본 논문에서는 기존의 actor-critic 알고리즘의 샘플 효율성 문제를 해결하기 위해 Langevin Soft Actor Critic (LSAC)이라는 새로운 모델-프리(모델이 없는) 강화 학습(RL) 알고리즘을 제안합니다. LSAC는 정책 최적화보다 불확실성 추정을 통한 비평가(critic) 학습의 향상에 중점을 둡니다. 본 연구는 Thompson sampling의 원리를 효과적으로 활용하여 탐색 성능을 개선하는 방법을 모색하고 있습니다.

- **Technical Details**: LSAC는 세 가지 주요 혁신을 포함하고 있습니다. 첫째로, distributional Langevin Monte Carlo (LMC) 기반의 $Q$ 업데이트를 통해 근사적인 Thompson sampling을 구현합니다. 둘째, parallel tempering 방식을 사용하여 $Q$ 함수의 posterior 여러 모드를 탐색하며, 셋째, $Q$ action gradients로 정규화된 확산 합성(state-action) 샘플을 사용합니다.

- **Performance Highlights**: LSAC는 연속 제어 과제에서 기존의 모델-프리 강화 학습 알고리즘의 성능을 초월하거나 동등한 성능을 발휘하는 것으로 나타났습니다. 특히, LSAC는 연속 동작 공간을 가지는 연속 제어 작업에서 LMC 기반의 Thompson sampling을 성공적으로 적용한 최초의 사례로 주목받고 있습니다.



### LEKA:LLM-Enhanced Knowledge Augmentation (https://arxiv.org/abs/2501.17802)
- **What's New**: 이 논문에서는 LLM-Enhanced Knowledge Augmentation (LEKA)라는 새로운 지식 증강 방법론을 제안합니다. LEKA는 타겟 도메인과 관련된 외부 지식 소스를 자동으로 검색하고, 이를 통합하여 데이터 학습 성능을 향상시키는 방식으로 설계되었습니다. 이 방법은 트레이닝 데이터를 수동으로 선택하는 기존의 비효율적인 방법에 비해 자동화되어 있어, 데이터 품질을 높이고, 학습 결과를 최적화하는 데 도움을 줍니다.

- **Technical Details**: LEKA 방법은 먼저 LLM을 활용하여 타겟 도메인에서 핵심 텍스트 정보를 추출합니다. 이후, 데이터베이스에서 관련 지식을 효율적으로 검색하는 데이터셋 RAG(Retrieval-Augmented Generation)를 사용하여 외부 데이터 소스를 확보합니다. 마지막으로, LLM은 추출된 데이터 소스를 타겟 도메인과 특징 공간(feature space) 및 마지널 확률 측정(marginal probability measures)에서 자동으로 조화롭게 조합합니다.

- **Performance Highlights**: 실험 결과, LEKA 방법은 전통적인 방법에 비해 컴퓨팅 비용을 줄이고, 데이터 정렬(data alignment)을 자동화하며, 전반적인 전이 학습 성과를 최적화하는 데 있어 상당한 개선을 보여주었습니다. 다양한 도메인에서 수행된 광범위한 실험을 통해, 논문은 LEKA가 기존 방법 대비 학습 성능을 향상시키고 있다는 것을 입증하였습니다.



### Detecting Anomalies Using Rotated Isolation Fores (https://arxiv.org/abs/2501.17787)
- **What's New**: 본 논문은 Rotated Isolation Forest (RIF) 알고리즘을 소개하여 기존의 Isolation Forest (iForest)와 Extended Isolation Forest (EIF)가 가지고 있는 단점을 해결하고 있음을 보여줍니다. 특히 기존 알고리즘이 정상 클러스터 사이에서 발생하는 새로운 형태의 고스트 클러스터 문제를 밝혀냈습니다. RIF는 데이터셋을 무작위로 회전시켜 이러한 문제를 효과적으로 처리하며, 향상된 예측 성능을 제공합니다.

- **Technical Details**: RIF는 데이터셋을 iForest 구성을 하기 전에 무작위 회전 행렬(random rotation matrix)과 QR 분해(QR decomposition)를 사용하여 회전시킵니다. 이로 인해 데이터 세트의 변동성이 증가하고 기존 iForest에서 나타나는 고스트 클러스터를 제거하게 됩니다. RIF의 구조는 EIF보다 단순하며, 고차원 공간에서도 보다 일관된 이상 점수(anomaly score)를 제공합니다.

- **Performance Highlights**: 실험 결과, RIF는 인공 데이터셋(synthetic datasets)과 실제 데이터셋(real-world datasets) 모두에서 iForest와 EIF를 능가하는 성능을 보여주었습니다. RIF를 통해 예측의 정확도가 향상되었으며 머신러닝 및 데이터 분석 기법에서 뛰어난 효율성을 자랑합니다. 기존의 방법들이 가지는 단점을 보완함으로써 RIF는 더욱 신뢰성 있는 결과를 도출할 수 있게 되었습니다.



### AdditiveLLM: Large Language Models Predict Defects in Additive Manufacturing (https://arxiv.org/abs/2501.17784)
- **What's New**: 이번 연구에서는 대형 언어 모델이 공정 매개변수 입력을 바탕으로 적층 제조( additive manufacturing ) 결함 영역을 예측하는 능력을 조사하였습니다. 이를 위해 프로세스 매개변수와 결함 데이터 세트를 활용하여 'AdditiveLLM'이라는 모델을 세부 조정하여 Keyholing, Lack of Fusion( 용융 부족 ), Balling과 같은 결함 영역을 예측합니다. 이러한 접근법은 모델의 정밀도를 높이고 사용자에게 최적의 매개변수 설정을 찾는 데 도움을 줍니다.

- **Technical Details**: 레이저 분말 침대 융합(Laser Powder Bed Fusion, L-PBF) 과정에서 최적의 공정 매개변수 선택이 결함 없는 부품 제작에 중요한 요소로 작용합니다. 연구에서는 다양한 결함 영역을 정의하고 실험, 컴퓨터 시뮬레이션 및 대체 모델링을 통해 예측할 수 있는 크리터리온을 설정합니다. 모델은 DistilBERT, SciBERT, Llama 및 T5와 같은 다양한 대형 언어 모델을 이용해 세부 조정되어, 입출력 데이터 패턴을 기반으로 결함 예측 정확도를 높입니다.

- **Performance Highlights**: 모델은 공정 매개변수가 주어졌을 때 결함 영역을 예측하는 데 93%의 정확도를 달성하였습니다. Baseline 데이터 세트와 자연어 Prompt 데이터 세트의 다양한 입력 형식을 비교하여 모델 성능을 평가하며, 프로세스를 간소화하여 사용자가 필요한 매개변수를 더 쉽게 선택할 수 있게 합니다. 이로 인해 실험적 구축이나 추가 시뮬레이션을 수행하지 않고도 결함 예측을 신속하게 수행할 수 있는 장점이 존재합니다.



### Picard-KKT-hPINN: Enforcing Nonlinear Enthalpy Balances for Physically Consistent Neural Networks (https://arxiv.org/abs/2501.17782)
- **What's New**: 이번 연구에서는 Neural Networks (NNs)를 물리적 법칙에 맞춰 예측하도록 강제하는 새로운 방법인 Picard-KKT-hPINN을 소개합니다. 이 방법은 비선형 대수적 제약 조건을 enforce하여 화학 공정의 대체 모델을 개선합니다. 특히, 비선형 엔탈피와 선형 원자 밸런스를 머신 수준의 정확도로 효율적으로 처리하는 능력을 보여줍니다. 기존의 ML(머신러닝) 모델이 직면한 물리적 일관성 문제를 해결하는 것을 목표로 합니다.

- **Technical Details**: Picard-KKT-hPINN은 피카르의 성공적 근사 방법을 기반으로 하여, 변수 동결(variable-freezing) 및 지역 투영(local projection) 등의 수치 기법을 통합하여 비선형 제약 조건을 NN 예측에 적용합니다. 데이터셋 (𝑥𝑖,𝑦𝑖)에서 이해 관계에 대한 요구 조건을 충족하도록 NN을 훈련시키며, 이 과정에서 엔탈피와 같은 비선형 관계를 만족하도록 설계되었습니다. 이는 NN의 출력 차원이 제약 조건의 수보다 크도록 하여, 데이터로부터 학습하면서도 제약 조건을 준수할 수 있게 합니다.

- **Performance Highlights**: 의 실험 결과에 따르면, Picard-KKT-hPINN은 데이터가 부족한 상황에서도 정확도를 향상시킬 수 있음을 나타냅니다. vanilla 다층 퍼셉트론 대비 보다 높은 품질의 예측을 가능하게 하여, 널리 사용되는 ML 도구의 신뢰성과 실용성을 높이고 있습니다. 이 방법은 대체 데이터 모델링에서 기계적 법칙을 효과적으로 통합하여 연구 결과의 유용성을 극대화합니다.



### Generative Unordered Flow for Set-Structured Data Generation (https://arxiv.org/abs/2501.17770)
Comments:
          Paper under review

- **What's New**: 이번 논문에서는 순서가 없는 데이터에 대한 새로운 생성 모델인 'unordered flow'를 제안하였습니다. 기존의 flow-based generative models는 대부분 순서가 있는 벡터 데이터에 최적화되어 있었기 때문에, 순서가 없는 데이터 생성에 대한 연구가 부족했습니다. 이는 다양한 실제 응용 분야에서 매우 중요하며, 본 연구는 이 격차를 메우기 위한 첫 걸음으로 볼 수 있습니다.

- **Technical Details**: 'unordered flow' 모델은 순서가 없는 데이터를 연속 함수 형태로 표현하고, 이러한 표현의 확률 측정을 함수 값 기반의 flow matching을 통해 학습합니다. 또한, 함수 표현에서 순서가 없는 데이터로의 역 변환을 위해 파티클 필터링과 유사한 방법을 제안하며, Langevin dynamics를 사용해 초기 파티클을 준비하고, 그래디언트 기반 탐색을 통해 업데이트합니다. 이 접근 방식은 유사한 데이터 모델링의 가능성을 열어줍니다.

- **Performance Highlights**: 여러 실제 데이터 세트에 대한 실험을 통해 'unordered flow' 모델이 세트형 데이터 생성에서 우수한 성능을 보이며, 기존의 기준을 크게 초과하는 성과를 나타냈습니다. 이 연구는 순서가 없는 데이터에 대한 생성 모델링의 잠재력을 보여주는 중요한 기여가 될 것으로 기대됩니다.



### Dynamics of Transient Structure in In-Context Linear Regression Transformers (https://arxiv.org/abs/2501.17745)
- **What's New**: 이 논문에서는 변환기(transformers)가 맥락 내 선형 회귀(linear regression) 작업에서 일시적인 리지 생성(transient ridge phenomenon)을 보이는 것을 분석합니다. 이 과정에서 변환기는 훈련 데이터의 다양한 작업에 대한 초기 단계에서 리지 회귀(ridge regression)처럼 동작하며, 그 후 점차적으로 특화된 전문 솔루션으로 변화합니다. 이 전환 과정은 결합 궤적 주성분 분석(joint trajectory principal component analysis)을 통해 드러납니다.

- **Technical Details**: 사용된 변환기 아키텍처는 Hoogland et al.(2024)와 유사한 두 층(transformer architecture)으로 구성되어 있습니다. 이 아키텍처는 예측 가능한 잠재적 위치 임베딩(learnable positional embedding)을 포함하며, 2개의 층(L=2)과 각 층마다 4개의 어텐션 헤드(H=4)를 갖습니다. 이 모델은 훈련 가능한 2.65백만 개의 매개변수를 갖고 있으며, 모델 복잡도(model complexity)는 국소 학습 계수(local learning coefficient)를 사용하여 측정됩니다.

- **Performance Highlights**: 이 연구에서는 변환기의 훈련 시 손실(loss)과 복잡도(complexity) 간의 진화하는 균형을 바탕으로 일시적인 구조(transient structure) 현상에 대한 일반적인 설명을 제공합니다. 이러한 이론적 기반 위에, 변환기가 다양한 작업의 맥락을 고려하여 보다 복잡한 기능적 출력을 생성할 수 있음을 강조합니다. 마지막으로, 훈련 과정에서 맥락 예측을 평가하는 것이 중요하다는 점을 언급하며 결과의 신뢰도를 높입니다.
...
Comments:
          29 pages, 5 figures, 8 tables, 2 listings

- **What's New**: 이 논문은 Automatic Sparse Differentiation (ASD)의 최신 발전을 다루고 있습니다. 이들은 Jacobian과 Hessian이 sparsity를 가진 경우, Automatic Differentiation (AD)의 속도를 크게 향상시킬 수 있는 가능성을 설명합니다. 또한, 작성된 파이프라인은 기존 ML 코드베이스와의 호환성을 제공하면서도 코드 수정이 필요 없는 자동화된 시스템이라는 점에서 주목할 만합니다.

- **Technical Details**: 저자들은 sparsity 검출을 위한 새로운 관점을 제공하며, operator overloading 기법을 사용하여 전통적인 평균 그래프 접근법에서 발생할 수 있는 문제를 자연스럽게 피할 수 있다고 설명합니다. 논문의 ASD 파이프라인은 Julia 언어로 구현되었으며, Matrix coloring과 differentiation을 포함한 독립 소프트웨어 패키지를 통해 기능을 제공합니다. 이를 통해 사용자는 복잡한 sparsity 패턴을 자동으로 활용할 수 있습니다.

- **Performance Highlights**: 실험 결과, ASD 파이프라인은 Jacobian 및 Hessian 매트릭스를 계산하는 데 있어 최대 3배의 속도 향상을 보여주었습니다. 이는 기존의 AD 방법론에 비해 한 번의 연산에서도 더 효율적임을 나타냅니다. 특히, 과거에 비현실적으로 여겨졌던 대규모의 매트릭스 계산에서의 성능 우위를 입증하였습니다.



### Sparse Autoencoders Can Interpret Randomly Initialized Transformers (https://arxiv.org/abs/2501.17727)
- **What's New**: 이 연구에서는 랜덤 파라미터로 초기화된 트랜스포머(Transformer)에서 희소 오토인코더(Sparse Autoencoders, SAE)를 примен하여 내부 표현을 해석하는 새로운 방법을 선보였습니다. 기존의 훈련된 트랜스포머와 랜덤 트랜스포머의 결과를 비교한 결과, 두 경우의 SAE 잠재(latent)는 유사한 해석 가능성을 제공하는 것으로 나타났습니다. 이로 인해 SAE가 추출한 특성이 과연 학습된 계산(computation)을 반영하는지에 대한 의문이 제기되었습니다.

- **Technical Details**: 논문에서 제시된 SAE는 네트워크 활성화에서 희소하고 해석 가능한 특징을 학습하여 내부 표현을 분리하는 데 초점을 맞추고 있습니다. 랜덤 트랜스포머와 훈련된 모델 모두에서 유사한 오토 해석 평가(auto-interpretability score)가 나타났으며, 이는 두 모델 간의 기본적인 계산 구조에 대한 통찰력을 제한할 수 있음을 시사합니다. 저자들은 이러한 결과가 모델 크기와 층(layer)에 구애받지 않고 일관된 패턴을 보인다고 언급합니다.

- **Performance Highlights**: 이 연구는 SAE의 훈련 결과가 임의로 초기화된 네트워크에서도 유사한 성능을 보여주며, 이는 훈련된 모델의 기능을 이해하는 데 있어서 중요한 의미를 가집니다. 또한, SAE의 품질 메트릭이 훈련된 모델과 랜덤 모델 간에 유사한 경향을 보였고, 이는 기계적 해석(mechanistic interpretability) 연구에 중요한 질문을 던지게 됩니다. 저자들은 이러한 발견들이 SAEs 사용의 한계를 지적하며, 모델의 학습된 계산을 명확히 구분하는 데 더욱 철저한 방법론이 필요함을 강조합니다.



### STGCN-LSTM for Olympic Medal Prediction: Dynamic Power Modeling and Causal Policy Optimization (https://arxiv.org/abs/2501.17711)
Comments:
          18pages, 7figures

- **What's New**: 이 논문은 STGCN-LSTM이라는 새로운 하이브리드 모델을 제안하여 올림픽 메달 배분 예측을 개선하고자 하였습니다. 이 모델은 Spatial-Temporal Graph Convolution Network (STGCN)를 통해 국가 간의 공간-시간적 관계를 모델링하고, Long Short-Term Memory (LSTM) 모듈을 사용하여 국가 성과의 장기적 의존성을 분석합니다. 또한, Zero-Inflated Compound Poisson (ZICP) 프레임워크를 도입하여 메달 획득 국가 간의 편차를 분석하여 보다 명확한 성과 예측을 제공합니다.

- **Technical Details**: 하이브리드 STGCN-LSTM 모델은 STGCN과 LSTM을 결합하여 특성 추출 및 국가 간 관계 모델링의 정확성을 높였습니다. 추가로, SMAPE 기반의 백테스팅과 정책 충격 시뮬레이션, 그리고 반사실적 인과 분석을 포함한 삼중 검증 프레임워크를 구축하여 모델의 신뢰성과 해석 가능성을 확보하였습니다. 정책 및 자원 최적화 시스템에서는 Markowitz 모델과 동적 국가 강점 가중 행렬을 활용하여 스포츠 자원 할당을 최적화하는 방법론을 제안합니다.

- **Performance Highlights**: 연구 결과는 코칭 이동, 이벤트 전문화, 전략적 투자 등이 메달 예측에 미치는 영향을 분석하였습니다. 예를 들어, 프랑스 올림픽 위원회의 몬테카를로 시뮬레이션을 통해 수영에서 펜싱으로의 자금 재배치로 2024년 메달 수가 22% 증가할 것으로 예상하였습니다. 이러한 데이터 기반의 접근 방식은 올림픽 국가의 스포츠 정책 최적화와 자원 배분에 중요한 통찰을 제공합니다.



### Temperature-Free Loss Function for Contrastive Learning (https://arxiv.org/abs/2501.17683)
Comments:
          10 pages, 5 figures

- **What's New**: 이 논문은 self-supervised learning의 중요한 방법 중 하나인 contrastive learning을 개선하기 위해 InfoNCE loss의 온도(temperature) 조정 없이 사용할 수 있는 새로운 방법을 제안합니다. 기존의 InfoNCE loss는 유사성 점수를 조정하기 위해 온도를 사용하는데, 이 과정은 실험상 많은 시도와 오류를 필요로 합니다. 저자들은 온도 대신 역쌍곡선 탄젠트 함수(inverse hyperbolic tangent function)를 사용하는 수식을 제안하여, 온도 조정의 필요성을 제거했습니다.

- **Technical Details**: 제안된 방법은 기존의 온도 분할(scaling)을 삭제하고, 이 대신에 인과 관계를 파악하고 권장되는 그래디언트(gradient) 속성을 선사하는 새로운 수식으로 대체합니다. 이는 미니 배치(mini-batch) 데이터의 여러 쌍 쌍(pair)을 통해 특정한 유사성 점수를 계산하고, 이는 크로스 엔트로피(cross-entropy)를 통해 점수를 최적화하도록 설계되었습니다. 저자들은 제안된 메서드가 gradient descent의 최적화를 향상시킨다고 주장합니다.

- **Performance Highlights**: 제안된 방법은 온도 조정 없이도 높은 성능을 발휘하며, 기존의 온도 분할 방식에 비해 더 나은 성능을 보여준다고 보고합니다. 실험 결과는 다섯 가지 벤치마크에서 진행되었으며, 제시된 방법이 다른 대비 학습 기술들과 비교했을 때 비슷하거나 더 나은 결과를 보였음을 나타냅니다. 이를 통해 저자들은 온도에 의존하지 않는 효과적인 대비 학습 방법을 확립했습니다.



### Explainable Artificial Intelligence for identifying profitability predictors in Financial Statements (https://arxiv.org/abs/2501.17676)
Comments:
          Short paper presented at Workshop on "Ai in Finance" at European Conference on Artificial Intelligence

- **What's New**: 이 논문은 이탈리아 상장 기업의 재무제표 데이터를 활용하여 머신러닝 기법을 적용함으로써 기업의 수익 예측에 대한 새로운 접근법을 제시합니다. 기존의 간단한 재무 비율 모델 대신 복잡한 상호 작용을 포착할 수 있는 비선형 분류 모델을 비교 분석하며, 유럽 AI 규정을 준수한 해석 가능성 기법도 통합합니다. 이를 통해 예측 결과의 투명성을 높이고 기업 성과 평가에 기여하고자 합니다.

- **Technical Details**: 연구에 사용된 데이터셋은 AIDA Database에서 제공되며, 2013년부터 2022년까지의 이탈리아 기업 재무제표, 인구 통계 및 산업 데이터를 포함합니다. 327개 상장 기업에 대해 321개의 특징을 포함하여 모델을 학습하고, XgBoost, Random Forest, SVM 및 Logistic Regression과 같은 다양한 머신러닝 모델을 평가합니다. SHAP(Shapley Additive Explanations) 기법을 통해 모델의 설명 가능성을 강화하고, 금융 성과 예측에 가장 중요한 특징들을 분석합니다.

- **Performance Highlights**: 모델 간의 성능 비교에서 XgBoost가 다른 모델들보다 우수한 성능을 보여주었고, 원시 데이터와 재무 비율을 활용한 결과를 분석했습니다. 또한 ROC-AUC 점수를 사용하여 모델의 분류 성능을 평가하였고, 가장 중요한 특징의 선택 및 노이즈를 제거함으로써 설명 가능성을 높였습니다. 이러한 연구는 향후 이탈리아 기업의 수익성 예측을 위한 기반을 마련하고 현재의 방법론 개선을 제안합니다.



### CAMP in the Odyssey: Provably Robust Reinforcement Learning with Certified Radius Maximization (https://arxiv.org/abs/2501.17667)
Comments:
          Accepted to USENIX Security Symposium 2025, Seattle, WA, USA. Source code is available at Github (this https URL) and Zenodo (this https URL)

- **What's New**: 이번 논문에서는 새로운 패러다임인 Certified-rAdius-Maximizing Policy (CAMP) 훈련을 제안합니다. CAMP는 강화학습 에이전트의 인증된 강인성을 유지하면서도 더 나은 유틸리티를 달성하도록 설계되었습니다. 이는 현재의 Gaussian 보강 방법에서 발생하는 인증된 강인성과 기대 수익 간의 최적화 문제를 해결하는 데 초점을 맞춥니다. 실험 결과, CAMP는 다양한 과제에서 강인성 및 수익 트레이드오프를 크게 개선한 것으로 나타났습니다.

- **Technical Details**: CAMP는 교육 중의 지역인증 반경(local certified radius) 정보를 활용하여 대리 손실(surrogate loss)을 설정하고 이를 통해 정책을 최적화합니다. 기본적으로 드롭아웃(dropout) 같은 개념을 통해 더 높은 인증된 기대 수익을 유지하려는 목표를 가지고 있습니다. 정책 모방(policy imitation) 기법을 도입함으로써 훈련 안정성을 높이고 Bellman 오차 문제를 해결하였습니다. 최종적으로, CAMP는 심층 Q-러닝(deep Q-learning)에 사용할 수 있도록 설계되었습니다.

- **Performance Highlights**: CAMP는 기존의 방법들보다 최대 두 배 더 높은 인증된 기대 수익을 달성하면서도 인증된 강인성을 유지하는 성능을 보여주었습니다. 실험을 통해 다양한 과제에서 뛰어난 성능을 입증하였으며, 특히 게임 에피소드에서의 강인성 측면에서도 눈에 띄는 성과를 보였습니다. 이러한 결과는 CAMP가 실제 환경에서 다양한 문제 해결에 효과적이라는 것을 시사합니다.



### Landscape Features in Single-Objective Continuous Optimization: Have We Hit a Wall in Algorithm Selection Generalization? (https://arxiv.org/abs/2501.17663)
- **What's New**: 이 연구에서는 다양한 문제 표현을 기초로 한 알고리즘 선택 (Algorithm Selection, AS) 모델의 일반화 가능성을 평가합니다. 특히, 최근 제안된 Topological Landscape Analysis 특징과 딥러닝 기반 특징들(예: DeepELA, TransOptAS, Doe2Vec)과 널리 사용되는 Exploratory Landscape Analysis 특징을 비교합니다. 연구 결과는 AS 모델이 새로운 문제에 대해 일반화할 수 있는 능력에 한계가 있음을 보여줍니다.

- **Technical Details**: AS 모델은 문제의 러닝 랜드스케이프(features)를 기반으로 하여 알고리즘 성능을 예측하는 역할을 합니다. ELA (Exploratory Landscape Analysis) 특징은 최적화 문제를 나타내는 데 널리 사용되지만, 샘플 크기 및 샘플링 방법에 민감하고 일반화하는 데 한계가 있습니다. 이 논문에서는 최근 2년 이내에 개발된 다양한 문제 표현 기술을 분석하여 이들의 일반화 가능성을 평가합니다.

- **Performance Highlights**: 연구에서 제안된 여러 AS 모델은 단순한 기준 모델인 Single Best Solver를 초과하는 성능을 보이지 않았습니다. 이는 기존의 문제 표현 방식이 새로운 문제에 대한 일반화에 한계가 있음을 가운데 뚜렷이 나타냅니다. 향후 연구에서는 문제가 조합된 새로운 BBOB 벤치마크를 활용하여 더욱 심층적인 분석을 진행할 필요가 있습니다.



### Drivetrain simulation using variational autoencoders (https://arxiv.org/abs/2501.17653)
Comments:
          27 pages

- **What's New**: 이번 연구는 Variational Autoencoders (VAEs)를 활용하여 주어진 토크 수요에 따른 차량의 jerk를 예측하는 방법을 제안합니다. 이는 실제 세계 데이터셋의 한계를 해결하기 위한 것으로, 무조건적 및 조건부 VAE를 구현하여 다양한 드라이브 트레인 시나리오에서의 특징을 통합한 jerk 신호를 생성합니다. 실험 데이터는 최대 토크 전달과 드라이브 트레인 구성에서 차이가 있는 두 가지 전기 SUV 모델에서 수집되었습니다.

- **Technical Details**: VAE는 데이터의 잠재 공간(latent space)을 해석하여 새로운 의미 있는 jerk 신호를 생성할 수 있는 능력을 갖추고 있습니다. 이 방법은 수치적 시스템의 매개변수를 수동으로 조정할 필요 없이 특정 입력에 기초하여 실제적인 데이터 생성을 가능하게 합니다. 조건부 VAE(CVAE)는 추가 입력을 반영하여 토크에 특화된 신호를 생성하며, 이로 인해 비싼 실험 요구 사항을 줄일 수 있습니다.

- **Performance Highlights**: VAEs의 성능은 물리 기반 모델 및 하이브리드 모델과 비교하여 그 효과성이 확인되었으며, 비현실적인 데이터 세트에 의존하지 않고도 물리적 타당성을 유지하면서 새로운 신호를 생성할 수 있음을 보여줍니다. 연구는 VAEs가 드라이브 트레인 모델 설계 및 검증 과정을 가속화하는 데 기여할 수 있음을 강조하고 있습니다. 이는 복잡한 운전 시나리오를 시뮬레이션하는 보완적 방법론으로서 generative AI의 가능성을 제시합니다.
...
### nabqr: Python package for improving probabilistic forecasts (https://arxiv.org/abs/2501.17604)
- **What's New**: NABQR(Neural Adaptive Basis for (time-adaptive) Quantile Regression)라는 오픈 소스 Python 패키지가 소개되었습니다. 이 패키지는 LSTM 네트워크를 사용해 시나리오를 수정한 후, 수정된 시나리오에 대해 time-adaptive quantile regression을 적용하여 신뢰성 높은 확률 예측을 제공합니다. 덴마크에서 육상의 풍력 및 해상의 풍력 발전 예측의 경우, 최대 40%의 정확성 향상을 기록하고 있습니다.

- **Technical Details**: NABQR는 주로 Python 3.10 이상의 버전에서 작동하며, TensorFlow, NumPy, pandas와 호환되는 현대적인 소프트웨어 개발 관행을 준수합니다. 이 패키지는 시계열 데이터를 수용하는 다양한 입력 형식을 지원하며, 결과는 CSV 및 NumPy 배열과 같은 접근 가능한 형식으로 제공합니다. 사용자는 LSTM 네트워크를 학습시키고 quantile regression 방법을 적용하기 위한 통합된 파이프라인을 제공받습니다.

- **Performance Highlights**: NABQR는 덴마크의 풍력 발전 예측을 위한 특정 영역에서 평균 절대 오차 측면에서 40% 이상의 향상을 제공합니다. 이 패키지는 신뢰성 있는 quantile 예측을 생성하며, LSTM과 TAQR 알고리즘을 활용하여 다차원 quantile 손실 함수를 최적화합니다. 이를 통해 예측의 신뢰성을 크게 향상시키며, 다양한 데이터와 시나리오를 쉽게 비교하고 테스트할 수 있습니다.



### RegionGCN: Spatial-Heterogeneity-Aware Graph Convolutional Networks (https://arxiv.org/abs/2501.17599)
Comments:
          28 pages, 6 figures

- **What's New**: 이 논문에서는 공간 다형성이 데이터 생성 과정에서 중요한 역할을 한다고 강조하며, 특히 그래프 신경망에서의 다형성 모델링의 필요성을 제시합니다. 기존의 지리적 가중치 접근법이 효과적이지 않음을 인식하고, 공간적으로 변화하는 파라미터의 수를 줄이기 위해 지역 수준에서 모델링을 제안합니다. 이를 통해 과적합(over-fitting) 위험을 감소시키고, 모델 훈련 과정에서 지역 분할을 적응적으로 학습하는 방법을 개발합니다.

- **Technical Details**: 제안된 RegionGCN은 그래프 신경망에 기반한 공간 다형성 인식(convolutional network) 구조를 가지고 있으며, 미국 대선에서 카운티별 투표 비율 예측에 적용됩니다. 이 모델은 지역 수준에서의 파라미터 변화를 허용하여, 이전 모델들보다 예측 정확도를 크게 향상시킵니다. 지역 분할을 학습하기 위한 휴리스틱 최적화 절차도 함께 제시되어, 모델의 유연성을 증가시킵니다.

- **Performance Highlights**: RegionGCN은 기본 GCN 및 지리적으로 가중치가 부여된 GCN에 비해 현저한 성능 향상을 기록하였습니다. 또한 이 모델은 비선형 관계의 공간적 변화를 탐색하는 도구로서, 지역 분할의 앙상블 학습을 통해 통찰력을 제공합니다. 이 연구는 Geospatial Artificial Intelligence (GeoAI) 분야에 기여하며, 공간적 이질성을 다루는 데 중요한 역할을 하고 있습니다.



### Histogram approaches for imbalanced data streams regression (https://arxiv.org/abs/2501.17568)
- **What's New**: 본 논문에서는 임계 값을 기준으로 하는 기존 방식 대신, 데이터 전반에서 비율에 상관없이 희소인스턴스를 효과적으로 처리할 수 있는 새로운 샘플링 전략인 HistUS와 HistOS를 제안합니다. 이 방법들은 히스토그램 기반 접근 방식을 활용하여 데이터스트림을 동적으로 균형을 맞추며, 실제 데이터와 합성 데이터를 대상으로 한 실험에서 기존 방법들보다 우수한 성능을 보였습니다.

- **Technical Details**: HistUS와 HistOS는 분포의 극단값에 국한된 희소 케이스의 전통적인 가정에서 벗어나, 목표 변수가 가지는 범위 내에서 어느 위치에도 발생할 수 있는 희소 상황을 인식하고 처리할 수 있습니다. 이를 통해, 이러한 새로운 기법들은 예측 모델의 정확성과 강건성을 높이는 데 기여합니다. 이 전략들은 데이터 스트림 학습과 불균형 학습의 결합된 문제를 해결하기 위해 설계되었습니다.

- **Performance Highlights**: 우리는 실험을 통해 제안된 방법들이 전통적인 Chebyshev 기반 방법들보다 우수한 성능을 나타냄을 확인하였으며, 이는 샘플링 전략에 의해 모델의 편향을 줄이고 비율이 낮은 사례들을 효과적으로 처리할 수 있도록 돕습니다. 이 연구 결과는 향후 데이터 스트림 환경에서 보다 정교한 회귀 모델 개발에 기여할 것으로 기대됩니다.



### Heuristic-Informed Mixture of Experts for Link Prediction in Multilayer Networks (https://arxiv.org/abs/2501.17557)
Comments:
          Under Review

- **What's New**: 본 논문에서는 다층 네트워크(link prediction algorithms for multilayer networks)를 위한 최초의 Mixture-of-Experts (MoE) 프레임워크인 MoE-ML-LP를 소개합니다. 기존의 방법들은 특정 층에서의 링크 예측에 탁월하지만 다른 층에서는 효과적이지 않은 문제를 가지고 있었습니다. MoE-ML-LP는 다양한 전문가의 결정을 통합하여 개선된 예측 능력을 제공합니다.

- **Technical Details**: MoE-ML-LP는 다층 링크 예측을 위해 설계된 프레임워크로, 다층 휴리스틱(multilayer heuristics) 위에 구축되었습니다. 이 프레임워크는 다양한 네트워크 층에 인코딩된 정보를 능동적으로 활용하여 각 층이 제공하는 고유한 맥락을 포착합니다. 또한, 모듈화된 아키텍처를 갖추고 있어 새로운 전문가(experts)를 매끄럽게 통합할 수 있는 장점을 가지고 있습니다.

- **Performance Highlights**: 실제 및 합성 네트워크에 대한 광범위한 실험 평가를 통해 MoE-ML-LP는 여러 기준선(baselines)과 경쟁 방법들보다 일관되게 뛰어난 성능을 보여주었습니다. 평균 역순위(Mean Reciprocal Rank)에서 +60%, Hits@1에서 +82%, Hits@5에서 +55%, Hits@10에서 +41%의 놀라운 개선을 달성했습니다. 이는 링크 예측 분야의 미래 발전 가능성을 열어주는 결과로 해석될 수 있습니다.



### Closing the Gap Between Synthetic and Ground Truth Time Series Distributions via Neural Mapping (https://arxiv.org/abs/2501.17553)
- **What's New**: 이 논문에서는 NM-VQTSG(Neural Mapper for Vector Quantized Time Series Generator)라는 신기술을 소개합니다. 이 방법은 벡터 양자화(VQ) 기반의 시계열 생성에서 충실도(fidelity) 문제를 해결하는 데 중점을 두고 있습니다. 기존 방법들이 시계열 생성을 성공적으로 수행했지만, 정보 손실 및 배경 분포의 차이로 인해 신뢰성에 문제가 있었던 점을 보완하고자 합니다.

- **Technical Details**: NM-VQTSG는 U-Net 기반의 신경 매핑 모델을 활용하여 합성 데이터와 실제 데이터 간의 분포 차이를 줄이는 역할을 합니다. 이 모델은合成 시계열 생성 과정에서 발생하는 아티팩트를 수정하여 양질의 데이터를 생성하는 데 기여합니다. 다양한 데이터 세트를 사용하여 NM-VQTSG의 성능을 평가하였으며, FID, IS, cFID와 같은 지표들을 통해 품질 개선을 입증하였습니다.

- **Performance Highlights**: NM-VQTSG는 UCR 시계열 분류 아카이브의 데이터 세트를 통해 실험적으로 검증되었으며, FID에서 평균 151%, IS에서 7%, cFID에서 37%의 개선을 보였습니다. 또한, 데이터와 잠재 공간 모두에서 시각적인 검토를 통해 합성 데이터와 실제 데이터 간의 정렬 향상을 확인했습니다. 이 결과는 NM-VQTSG가 합성 시계열의 품질을 향상시키는 효과적인 방법임을 시사합니다.
...
### NF-MKV Net: A Constraint-Preserving Neural Network Approach to Solving Mean-Field Games Equilibrium (https://arxiv.org/abs/2501.17450)
Comments:
          7 pages

- **What's New**: 본 논문은 Neural Network 기반의 방법이 Mean-Field Games(MFGs) 평형을 해결하는 데 주목받고 있음을 강조합니다. 기존의 여러 알고리즘이 밀도 배포의 수학적 제약을 보장하는 데 어려움을 겪는다고 언급하며, 안정성 있는 분포 흐름을 달성하기 위해 stochastic process 관점에서 접근합니다. NF-MKV Net이라는 새로운 방법론이 제안되어, 이를 통해 MFGs 평형을 해결하는 고효율적인 방식이 마련되었습니다.

- **Technical Details**: NF-MKV Net은 Process-regularized Normalizing Flow(NF) 프레임워크와 state-policy-connected time-series neural networks를 결합하여 MKV FBSDE 문제를 해결합니다. 이 방법은 밀도 분포 흐름과 관련된 수학적 제약 조건을 준수하며, 각 시간 단계마다 부피 불변성을 보장하는 밀도 전송 함수를 모델링한다고 설명합니다. 논문에서는 MKV FBSDE의 관계를 끌어내며, 이를 통해 MFGs 내적 문제를 해결하는 과정의 복잡성을 감소시킵니다.

- **Performance Highlights**: 제안된 방법은 교통 흐름, 다양한 차원의 군중 운동, 장애물 회피 문제에 적용 가능함을 보여줍니다. 기존의 Neural Network 기반 접근 방식보다 수학적 제약 조건을 더 잘 충족하며, 고차원 MKV FBSDEs에 대한 NFT 프레임워크의 활용성을 한층 높이 평가합니다. 이 논문은 MFGs 문제 해결의 새로운 패러다임을 제시하며, 실용적인 결과들을 통해 효과성을 입증합니다.



### Gradual Domain Adaptation for Graph Learning (https://arxiv.org/abs/2501.17443)
- **What's New**: 기존 문헌에서는 큰 배포 변화에 대한 그래프 도메인 적응(DA) 기술이 부족했습니다. 본 논문에서는 정보 손실을 최소화하는 응집된 도메인 시퀀스를 구성하는 그래프 점진적 도메인 적응(GGDA) 프레임워크를 제안합니다. GGDA는 Fused Gromov-Wasserstein(FGW) 메트릭을 기반으로 지식을 보존하는 중간 그래프를 효율적으로 생성합니다.

- **Technical Details**: GGDA 프레임워크는 '근접한' 정점 선택과 적응형 도메인 발전을 통해 도메인 간 정보 전이 가능성을 강화합니다. 이는 Wp(μt,μt+1)의 하한과 상한을 통해 설명 가능한 이론적 프레임워크로 뒷받침됩니다. 해당 프레임워크는 각 도메인의 안정적인 지식 전이를 위해 중간 도메인 시퀀스를 설계하는 방식을 포함합니다.

- **Performance Highlights**: 15개 데이터셋에 대한 광범위한 실험을 통해 GGDA 프레임워크의 효과성이 검증되었습니다. 특히 본 프레임워크는 최상의 기준선보다 평균 5.02% 향상된 성능을 보였습니다. 이는 GGDA가 큰 도메인 간 차이를 처리하는 데 있어 효율적임을 보여줍니다.



### Human-Aligned Skill Discovery: Balancing Behaviour Exploration and Alignmen (https://arxiv.org/abs/2501.17431)
Comments:
          Accepted at the 24th International Conference on Autonomous Agents and Multiagent Systems (AAMAS 2025)

- **What's New**: 이 논문은 비지도 학습 기법을 이용하여 강화 학습에서 안전하고 유용한 기술(skill)을 발견하기 위한 새로운 프레임워크인 Human-aligned Skill Discovery (HaSD)를 제안합니다. 기존의 방법들이 자율적으로 다양한 행동을 발견할 때 유용한 기술을 찾기 어려웠던 문제를 해결하고자 하며, 인간의 가치와 일치하는 기술을 발견하는 데 초점을 맞추고 있습니다. 이는 다양한 기술의 탐색이 보다 효율적이고 안전하게 이루어지도록 합니다.

- **Technical Details**: HaSD는 기술의 다양성과 인간 가치에 대한 정렬을 동시에 최적화하는 구조를 가지고 있습니다. 이 프레임워크는 복잡한 환경에서 안전하고 실용적인 기술을 발견하기 위해 인간의 피드백을 통합하여, 발견된 기술들이 일관되고 효율적으로 정렬되도록 합니다. 실험에서는 2D 내비게이션 및 SafetyGymnasium 환경에서 HaSD의 효과를 입증하였습니다.

- **Performance Highlights**: HaSD는 다양한 인간 정렬 기술을 발견할 수 있으며, 이는 후속 작업에 안전하고 유용하게 활용될 수 있습니다. 간단한 구성 가능한 기술들을 학습하여, 실용적인 시나리오에서 다양한 정도의 다양성 및 정렬 트레이드오프를 제공할 수 있음을 보여주었습니다. 이는 향후 실제 애플리케이션에서의 활용 가능성을 확대하는 데 중요한 역할을 할 것입니다.



### WCDT: Systematic WCET Optimization for Decision Tree Implementations (https://arxiv.org/abs/2501.17428)
- **What's New**: 이 논문에서는 기계 학습 모델의 수행 시간 최적화를 통해 자원이 제한된 내장 시스템에서의 안전한 운영을 보장하는 데 필요한 worst-case execution time (WCET) 최적화 접근 방식을 개발하였습니다. 특히, 결정 트리(decision tree)의 내부 구조의 WCET 최적화에 중점을 두고 있으며, 기존의 평균 사례 실행 시간(ACET) 대신 WCET에 대한 접근 방법을 제시합니다. 이를 위해 경량의 근사 모델(approximation model)을 소개하며, 결정 트리 경로의 실행 시간을 효율적으로 추정합니다.

- **Technical Details**: 결정 트리는 대체로 데이터 구조로 저장된 노드를 루프를 통해 방문하는 네이티브 트리 구현이나, 'if-else' 문으로 각 노드를 인코딩한 if-else 트리 구현으로 진행될 수 있습니다. 본 논문에서는 WCET 최적화를 위하여 경로 접근 확률 분포에서 벗어나 가장 최악의 실행 경로(worst-case execution path, WCEP)를 식별하고 최적화하는 방법을 제안합니다. 또한, 실행 시간을 추정하기 위한 선형 근사 모델을 정의하고, 다양한 결정 트리 모델에 대해 최적의 실행 경로를 신속하게 추정하는 알고리즘을 개발하였습니다.

- **Performance Highlights**: 제안된 최적화 알고리즘은 전통적인 구현에 비해 결정 트리의 WCET를 최대 17%까지 감소시키는 효과를 보여줍니다. 이 알고리즘은 실제로 여러 데이터셋에서 훈련된 결정 트리 모델에 대해 평가되었으며, 근사 모델을 사용한 결과적인 WCET가 정확한 WCET 분석과 비교하여 높은 정확성을 보였습니다. 이러한 성과는 실시간 시스템에서의 효율적인 기계 학습 구현을 가능하게 할 것으로 기대됩니다.



### si4onnx: A Python package for Selective Inference in Deep Learning Models (https://arxiv.org/abs/2501.17415)
Comments:
          35pages, 3figures

- **What's New**: 이번 논문에서는 si4onnx라는 패키지를 소개합니다. 이 패키지는 딥러닝 모델에서 선택적 추론(selective inference)을 수행하는 기능을 제공합니다. 특히, XAI(Explainable AI)의 CAM 기술과 VAE(Variational Autoencoder)를 사용한 재구성 기반 이상 탐지 방법을 통해 이미지 내의 중요한 영역을 식별할 수 있습니다.

- **Technical Details**: si4onnx는 Python 패키지로, 선택적 추론을 통해 제어된 제1유형 오류(type I error) 비율로 가설 검정을 손쉽게 수행할 수 있도록 설계되었습니다. 이 패키지는 PyTorch 및 TensorFlow와 같은 일반적인 프레임워크를 사용하여 구축된 딥러닝 모델과 호환됩니다.

- **Performance Highlights**: 디자인된 si4onnx의 주요 장점은 AI 시스템의 신뢰성을 확립하기 위해 중요 영역의 통계적 유의성을 평가할 수 있는 점입니다. 이는 선택적으로 중요한 데이터를 집중적으로 검토할 수 있게 하여, AI 모델의 해석 가능성을 높이는 데 기여할 수 있습니다.
...
### Data-Informed Model Complexity Metric for Optimizing Symbolic Regression Models (https://arxiv.org/abs/2501.17372)
Comments:
          Submitted to GECCO 2025

- **What's New**: 이 논문에서는 훈련 데이터를 초월하여 일반화되는 모델을 선택하기 위한 새로운 방법을 제안합니다. 특히, 계산 효율성이 높은 Hessian rank를 이용하여 모델 복잡성을 추정하고, 이를 데이터의 내재 차원(intrinsic dimensionality, ID)과 연결시키는 방식을 사용합니다. 이러한 접근은 데이터 복잡성에 따르면 최적의 모델 선택을 돕고, 모델의 표현력을 데이터 ID와 정렬시키는 데 기여합니다.

- **Technical Details**: 유전 프로그래밍(Genetic Programming, GP) 프레임워크에 ID 추정 범위를 사후 처리(post-processing) 단계로 포함시키는 것을 탐구합니다. 이를 통해 모델 복잡성과 데이터의 내재 차원이 일치하는 최적의 Pareto 모델 집합을 식별할 수 있습니다. 본 연구는 scikit-dimension 라이브러리에서 제공하는 다양한 ID 추정 알고리즘을 활용하여 Penn Machine Learning Benchmark에서 상징 회귀(symbolic regression) 문제에 대한 모델 발견과 성능을 평가합니다.

- **Performance Highlights**: 제안된 방법은 모델이 일반화에 매우 효과적인 경향을 보임을 보여줍니다. 특히, 사용자 정의 매개변수에 의존하는 방법에서 흔히 발생하는 편향을 줄이고, 모델의 표현력과 정확성을 동시에 높이는 복잡성 창(window)을 찾는 데 기여합니다. 최종적으로, 이 연구는 데이터의 내재 차원에 맞춘 모델 선택의 중요성을 강조하며, 향후 연구에 대한 새로운 방향성을 제시합니다.



### Breaking the $\log(1/\Delta_2)$ Barrier: Better Batched Best Arm Identification with Adaptive Grids (https://arxiv.org/abs/2501.17370)
Comments:
          21 pages, published at ICLR 2025

- **What's New**: 이번 연구에서는 다중 무장 밴디트(multi-armed bandits) 문제에서 배치된 최적 팔(best arm) 식별 문제를 다룹니다. 논문에서는 샘플(sample)과 배치(batch)의 수를 최소화하며, 주어진 $n$개의 팔 중에서 최적의 팔을 식별하는 알고리즘을 소개합니다. 제안된 알고리즘은 거의 최적의 샘플 복잡도(sample complexity)를 달성하며, 인스턴스별(batch complexity) 복잡도에서 새로운 접근 방식을 제공합니다.

- **Technical Details**: 제안된 알고리즘은 탐색(exploration)과 착취(exploitation)를 효과적으로 균형 잡는 샘플 할당(sampling allocation) 방식을 포함하고 있습니다. 특히, 우리의 알고리즘은 $	ext{log}(1/	ext{Δ}_2)$ 한계를 깨뜨리며 인스턴스에 민감한 배치 복잡도를 구현합니다. 이 알고리즘은 선형 밴디트(linear bandits) 문제에 대해서도 배치된 최적 팔 식별 문제로 확장되어 유사한 개선 효과를 보입니다.

- **Performance Highlights**: 실험 결과는 제안된 접근 방식이 다양한 설정에서 배치 효율성(batch efficiency) 측면에서 더 우수함을 보여줍니다. 알고리즘의 성능은 여러 벤치마크 데이터 세트에서 확인되었으며, 샘플 수를 줄이면서도 최적의 팔을 효과적으로 식별할 수 있음을 입증합니다. 이를 통해 실제 적용 가능성이 높아짐을 알 수 있습니다.
...
### CardiCat: a Variational Autoencoder for High-Cardinality Tabular Data (https://arxiv.org/abs/2501.17324)
- **What's New**: 이 논문에서는 고차원 범주적 특성을 효과적으로 모형화하기 위해 CardCat이라는 새로운 변분 오토인코더(Variational Autoencoder, VAE) 모델을 제안합니다. 기존의 생성 모델은 범주적 특성을 매개변수화하는 데 어려움을 겪으며, 특히 고차원 범주적 데이터에서 저조한 성능을 나타내었습니다. CardCat은 원-핫 인코딩(One-hot encoding) 대신 규제된 이중 인코더-디코더 임베딩 레이어를 활용하여 데이터의 다차원성을 더 잘 포착하고 있습니다.

- **Technical Details**: CardiCat 모델은 범주적 특성을 효과적으로 표현하기 위해 낮은 차원의 숫자 벡터 표현을 사용하는 임베딩 레이어를 채택합니다. 이 임베딩 레이어는 고차원 범주적 특성과 다른 특성 간의 공동 분포에 따라 최적화되며, 이를 통해 복잡한 관계를 보다 잘 반영합니다. 기존의 원-핫 인코딩 방식 없이 임베딩 공간에서 손실을 계산하여 파라미터 공간을 현저히 축소하고, 훈련 가능한 파라미터 수를 줄였습니다.

- **Performance Highlights**: CardiCat은 다양한 실험에서 고차원 및 불균형적인 특성을 잘 표현하는 고품질 합성 데이터를 생성하는 데 성공하였습니다. 다른 VAE 모델들과 비교했을 때, 학습된 임베딩 방식이 데이터의 주변 및 공동 분포를 더 잘 복구하는 성능을 보여주었습니다. 특히, 고차원 범주적 특성을 효율적으로 모형화하는 공개 벤치마크 프레임워크를 개발하여 연구자들이 새로운 접근 방식을 비교하고 평가할 수 있도록 하였습니다.



### Exploring Non-Convex Discrete Energy Landscapes: A Langevin-Like Sampler with Replica Exchang (https://arxiv.org/abs/2501.17323)
Comments:
          7 figures, 23 pages

- **What's New**: 새로운 연구에서는 Gradient-based Discrete Samplers (GDSs)의 한계를 극복하고, 복잡한 비볼록(non-convex) 환경에서의 탐색을 개선하기 위해 두 가지 새로운 샘플러를 제안합니다: Discrete Replica EXchangE Langevin (DREXEL)과 Adjusted Metropolis가 추가된 DREAM입니다. 이 샘플러들은 서로 다른 온도와 단계 크기를 가진 두 개의 GDS를 사용하여, 한쪽은 로컬 탐색(local exploitation)에 집중하고 다른쪽은 더 넓은 에너지 경관을 탐색합니다.

- **Technical Details**: DREXEL과 DREAM은 비볼록 이산 공간의 효과적인 탐색을 위해 GDS와 replica exchange Markov Chain Monte Carlo (reMCMC)를 결합합니다. 낮은 온도의 샘플러는 로컬 탐색에 집중하고, 높은 온도의 샘플러는 로컬 함정(local traps)을 탈출하여 넓은 탐색을 가능하게 합니다. 샘플 스왑(sample swaps)은 에너지 차이가 클 때 발생하며, 이는 이산 샘플링에 적합한 메커니즘에 의해 결정되어 상세한 균형(detailed balance)을 보장합니다.

- **Performance Highlights**: 이론적으로, DREXEL과 DREAM은 목표 에너지(target energy)로 점근적으로 수렴하며 단일 GDS보다 더 빠른 혼합(mixing)을 나타냅니다. 실험 결과는 DREXEL과 DREAM이 비볼록 이산 에너지 경관을 탐색하는 데 있어 훨씬 더 효율적임을 입증하고 있습니다. 이러한 결과는 현대 응용 프로그램에서 복잡한 이산 구조로부터 샘플링할 때 이 방법들의 효과성을 강조합니다.



### MDDM: A Molecular Dynamics Diffusion Model to Predict Particle Self-Assembly (https://arxiv.org/abs/2501.17319)
- **What's New**: 본 논문에서는 새로운 물질 시스템 발견과 연구를 위한 비용을 절감할 수 있는 'Molecular Dynamics Diffusion Model (MDDM)'을 제안했습니다. MDDM은 입력된 쌍 잠재 함수에 따라 유효한 입자 구조를 예측할 수 있으며, 기존의 diffusion 모델 보다 월등한 성능을 보입니다. 이 모델을 통해 분자 시스템의 시뮬레이션 시간을 크게 단축시키면서도 불확실성을 줄일 수 있습니다.

- **Technical Details**: MDDM은 입력된 쌍 잠재 함수의 노이즈를 제거하는 프로세스를 통해 Particle Locations를 생성합니다. 이 모델은 특정 주기적 경계 조건을 만족하고, 이동 불변성을 유지하는 등 도메인 특성을 내장하고 있습니다. MDDM의 트레이닝은 대량의 MD 시뮬레이션 결과 데이터셋을 통해 이루어지며, 결과적으로 효과적인 구조 생성을 위해 T번의 반복을 진행합니다.

- **Performance Highlights**: MDDM은 조건부 및 비조건부 생성 작업에서 기존 모델보다 높은 성과를 보이며, 입자 자가 조립 구조를 정량적, 정성적으로 더 잘 평가합니다. 구성된 구조는 수백 번의 diffusion 반복만으로 얻어지는 반면, 기존 MD 시뮬레이션은 수백만 번의 시간 단계가 필요합니다. 이를 통해 엔지니어들은 물질 시스템을 보다 신속하게 연구할 수 있는 기반을 제공합니다.
...
### Nonlinear dynamics of localization in neural receptive fields (https://arxiv.org/abs/2501.17284)
Comments:
          Appeared at the 38th Conference on Neural Information Processing Systems (NeurIPS 2024); spotlight presentation

- **What's New**: 이 논문은 전통적인 학습 방법과는 다른 방식으로 국소 수용 필드(localized receptive fields)가 어떻게 발생하는지를 설명합니다. 특히, 비평면 효율성(top-down efficiency) 제약 없이 피드포워드 신경망(feedforward neural network)이 자연 이미지의 구조에서 영감을 받은 데이터 모델에 대해서 훈련된 결과를 제시합니다.

- **Technical Details**: 작업의 핵심은 비가우시안 통계(non-Gaussian statistics)가 국소화에 미치는 영향을 파악하고, 단일 비선형 뉴런(single nonlinear neuron)의 효과적인 학습 동역학(effective learning dynamics)을 도출하는 것입니다. 이를 통해 입력 데이터의 고차 통계적 특성이 어떻게 국소화를 유도하는지를 명확히 설명합니다.

- **Performance Highlights**: 모델의 예측은 다수 뉴런 시스템에서도 확장 가능하다는 것을 입증했습니다. 이러한 결과는 신경 회로(neural circuits)에서 학습의 비선형 동역학이 국소화의 보편성을 설명할 수 있는 대안적 관점을 제공함을 의미합니다.



### Stiff Transfer Learning for Physics-Informed Neural Networks (https://arxiv.org/abs/2501.17281)
- **What's New**: 논문에서는 스티프(stiff) 시스템을 다룰 수 있는 새로운 방법, physics-informed neural networks (PINNs)에 대한 스티프 전이 학습(stiff transfer learning) 방법인 STL-PINNs를 제안합니다. STL-PINNs는 low-stiff regime에서 Multi-Head-PINN을 훈련하고, 이 정보를 활용해 high stiff regime에서 최종 솔루션을 계산하는 방법으로, 기존 PINNs가 겪는 'failure mode' 문제를 해결합니다. 이 방법은 'one-shot' 솔루션을 제공함으로써 계산 효율성도 유지할 수 있습니다.

- **Technical Details**: STL-PINNs는 스티프 ODE(Ordinary Differential Equation)와 PDE(Partial Differential Equation) 문제의 해를 다루기 위해 설계되었습니다. 이 방법은 특정 방식으로 훈련된 MH-PINN이 다양한 파라미터에 대해 즉각적으로 해를 제공할 수 있도록 해주며, 이는 시간 소모를 줄이는데 기여합니다. 특히 'latent space'를 학습する 능력을 통해께에야 하는 연산량이 줄어들고, 고차원 PDE 문제에서도 유리한 성능을 보입니다.

- **Performance Highlights**: 제안된 STL-PINNs 방법은 스티프 파라미터가 있는 선형 및 비선형 ODE와 PDE를 해결하는 데 있어 기존의 PINNs 기반 방법보다 뛰어난 정확도와 속도를 자랑합니다. 계산 시간은 스티프 정도와 무관하게 최소화되며, 기존의 암시적 수치 방법들보다 더욱 빠른 성능을 보여주었습니다. 추가적으로 초기 조건이나 강제 함수의 재파라미터화가 포함된 시뮬레이션에서 뛰어난 속도를 제공하는 것으로 나타났습니다.



### A 1-D CNN inference engine for constrained platforms (https://arxiv.org/abs/2501.17269)
- **What's New**: 이 논문에서는 1D-CNN을 활용한 시계열 분류 및 추론 방식을 개선하기 위한 새로운 방법을 제시합니다. 전통적인 접근방식은 데이터를 버퍼에 모은 후 추론을 수행하는 반면, 본 연구는 샘플 간의 짧은 간격에 convolution 연산을 혼합하여 latency를 줄입니다. 이로 인해 메모리 사용량을 약 절반으로 줄이면서 추론 지연을 10% 감소시키는 성과를 극대화했습니다.

- **Technical Details**: 제안된 방법은 링 버퍼 (ring buffers)를 사용하여 convolution 연산에 필요한 신호의 일부만 저장합니다. 이는 메모리 발자국 (memory footprint)을 줄이고, 자원이 제한된 엣지 디바이스에서 효율적으로 작동할 수 있게 합니다. 실험에서는 ARM Cortex MCU를 대상으로 수행된 시간 측정 결과를 통해 제안된 방법의 효과성을 입증했습니다.

- **Performance Highlights**: 본 연구는 AVR 기반 및 ARM 기반의 아두이노 보드에서 구현 검증을 통해 일반 소비자 기기에서의 실행 가능성을 보여줍니다. 결과적으로, 제안된 방법은 기존 TFLite의 추론 방식에 비해 성능이 크게 향상되었으며, 이는 시간 민감한 태스크를 수행하는 엣지 디바이스에서 유용하게 활용될 수 있습니다.



### Increasing Information for Model Predictive Control with Semi-Markov Decision Processes (https://arxiv.org/abs/2501.17256)
- **What's New**: 최근의 학습 기반 모델 예측 제어( Learning-Based Model Predictive Control, LB-MPC ) 연구는 정보 이론의 기준을 사용하여 샘플 복잡성을 향상시키는 인상적인 성과를 보이고 있습니다. 그러나 현재의 탐색 경로에서 얻는 관측 정보가 제한적이며, 이 문제를 해결하기 위해 반 마르코프 결정 프로세스(Semi-Markov Decision Processes, SMDP) 프레임워크를 도입하였습니다. 이 프레임워크는 고정된 샘플링 예산으로 수집하는 데이터의 총 정보를 증가시켜 샘플 복잡성을 줄이는 데 기여합니다.

- **Technical Details**: 기계학습 제어(Machine Learning Control, MLC)는 통계 학습과 제어 이론의 융합 영역으로, 모델 없는 최적 제어 문제를 해결합니다. LB-MPC는 시스템 동역학에 대한 충분한 설명 모델을 기반으로 한 최적화 방법인 모델 예측 제어(Model Predictive Control, MPC)와 학습 방법의 결합을 가리킵니다. 본 연구에서는 SMDP 모델링을 통한 시간 추상화를 소개하여 상태-제어 공간의 탐색 정보를 개선하고, 이전 연구에서 얻은 샘플 복잡성을 더욱 개선하고자 합니다.

- **Performance Highlights**: 기존의 학습 기반 모델들은 데이터 수집 과정에서 정보의 중복성이 발생할 수 있는 문제를 가지고 있었습니다. 개선된 방법론을 통해, 다양한 시간 수준에서의 행동과 상태를 표현할 수 있는 능력이 강화되었습니다. 이로 인해 동적 시스템의 복잡한 동작 모델을 학습할 때 필요한 샘플 수를 줄일 수 있는 효과가 입증되었습니다.
...
- **What's New**: 이번 논문에서는 기존의 시계열 예측 모델이 저에너지 구성요소를 간과하는 문제를 해결하기 위해 에너지 증폭 기술을 제안합니다. 이 기술은 저에너지 구성요소의 에너지를 향상시키는 에너지 증폭 블록과 원래 수준의 에너지로 복원하는 에너지 복원 블록으로 구성됩니다. 특히, 두 개의 에너지 피크를 독립적으로 모델링할 수 있도록 계절-추세 예측기와 통합되어 제안된 모델 'Amplifier'의 기초가 됩니다.

- **Technical Details**: 저자들은 시계열 데이터를 처리하는 과정에서 저에너지 구성요소를 효과적으로 늘리고 복원할 수 있도록 설계된 Amplifier 모델을 제안합니다. 에너지 증폭과 계절-추세 분해를 결합하여 데이터를 변환하며, 'semi-channel interaction temporal relationship enhancement block'을 통해 각 채널의 공통성과 특수성에 따라 시간적 관계를 개선합니다. 이로 인해 모델이 시간적 관계를 더욱 효과적으로 포착할 수 있게 됩니다.

- **Performance Highlights**: 8개의 시계열 예측 벤치마크에서 실시된 실험 결과, Amplifier 모델은 최신 방법들에 비해 뛰어난 효과성과 효율성을 보여주었습니다. 특히 모델은 저에너지 구성요소를 간과하지 않고 학습할 수 있어 예측 정확도를 향상시키는 데 기여하였습니다. 이러한 성능은 대규모 데이터셋에서 일관되게 나타나며, 향후 연구와 응용 분야에서의 가능성을 제시합니다.
...
- **What's New**: 최근 생물 다양성 모니터링에 있어 인공지능(AI)과 수동 음향 모니터링(PAM)의 결합이 점점 더 중요해지고 있습니다. 기존 PAM 시스템은 수동 데이터 오프로드가 필요하고 저장 및 컴퓨팅 인프라에 큰 부담을 줍니다. 새롭게 개발된 'acoupi'는 사용자들이 특화된 모니터링 목표에 맞게 장치 동작을 조정하고, 맞춤형 하드웨어에 AI 모델을 통합할 수 있는 접근성을 제공합니다.

- **Technical Details**: 소프트웨어 'acoupi'는 오픈 소스 Python 프레임워크로, 생물 음향 장치의 구축과 배포를 단순화합니다. 이 프레임워크는 오디오 기록, AI 기반 데이터 처리, 데이터 관리 및 실시간 무선 메시징을 통합하여 사용자가 특정 요구에 따라 쉽게 구성할 수 있도록 합니다. 주요 기능으로는 사용자 정의 AI 기반 모델 통합, 장치 동작 조정, Linux 기반 SBC 장치에서의 안정적인 배포가 포함됩니다.

- **Performance Highlights**: acoupi의 유연성은 BirdNET과 BatDetect2 같은 두 가지 생물음향 분류기를 통합하여 검증되었습니다. 영국 도시 공원에서 한 달간 두 대의 acoupi 장치를 배포하여 성능을 평가하였으며, 저비용 하드웨어인 Raspberry Pi에서도 손쉽게 배포 및 커스터마이즈 할 수 있음을 보여주었습니다. 이러한 접근 방식은 연구자와 보존 활동가들이 AI 기반 PAM 시스템을 채택하는 데 기여할 것입니다.
...
### Matrix Product Sketching via Coordinated Sampling (https://arxiv.org/abs/2501.17836)
Comments:
          18 pages

- **What's New**: 이번 연구에서는 스파스(sparse) 행렬 𝐀와 𝐁의 근사 행렬 곱 𝐀^T𝐁를 계산하기 위한 새로운 접근법을 제시합니다. 기존의 선형 스케칭(linear sketching) 방법에 비해 'coordinated random sampling' 방식을 활용하여 메모리 압박을 줄이는 동시에 성능을 개선할 수 있음을 보여줍니다. 이는 데이터베이스의 분산 선형 회귀 및 Transformer 기반 언어 모델의 어텐션 행렬 근사와 같은 응용 프로그램에서 실질적으로 더 나은 결과를 창출합니다.

- **Technical Details**: 연구에서 제안된 방법은 각 행렬 스케치 𝒮(𝐀)와 𝒮(𝐁)를 독립적으로 계산하는 것을 강조합니다. 스케치의 크기는 특정 조건 하에 조정 가능하며, 이는 𝓞(s/ε^2)와 같은 크기를 필요로 하여 더 많은 비효율성을 줄입니다. 반면, 기존의 선형 스케칭 방법은 𝓞(d/ε^2)와 𝓞(m/ε^2) 크기를 요구하게 됩니다.

- **Performance Highlights**: 두 가지 실험적 평가를 통해 제안된 샘플링 알고리즘이 선형 스케칭에 비해 한 차원 개선된 성능을 보임을 확인했습니다. 분산 선형 회귀 문제와 Transformer 기반 언어 모델의 어텐션 근사 모두에서 상당한 성능 향상을 이끌어냈습니다. 결과적으로, 이 접근법은 대규모 데이터 처리에서 스케치와 샘플링 기술의 사용 가능성을 높이고 있습니다.
...
### Long-term prediction of El Ni\~no-Southern Oscillation using reservoir computing with data-driven realtime filter (https://arxiv.org/abs/2501.17781)
Comments:
          21 pages, 7 figures

- **What's New**: 이번 연구에서 제안된 새로운 유형의 band-pass filter는 과거의 시계열 데이터만을 활용하여 머신러닝 모델의 장기 예측 가능성을 향상시키기 위해 설계되었습니다. 기존의 방식이 미래 정보를 포함하는 것과 달리, 본 연구의 접근법은 실시간 예측에 적합하도록 고안되었습니다. 이를 통해 El Niño-Southern Oscillation(ENSO)와 같은 기후 동적 현상의 예측을 더 정교하게 수행할 수 있을 것으로 기대됩니다.

- **Technical Details**: 제안된 방법은 reservoir computing(저장소 컴퓨팅)과 결합하여 사용되며, 복잡한 대기 시스템의 예측에 효과적입니다. Bayesian optimization을 통해 적절한 filter를 찾고, moving average 기법을 사용하여 이전 데이터만으로 데이터를 필터링하여 처리합니다. 새로운 필터링 방법은 3년에서 8년 주기의 신호를 효과적으로 추출할 수 있도록 설계되었습니다.

- **Performance Highlights**: 연구 결과는 ENSO의 다년간 역학을 24개월 동안 예측하는 데 있어 뛰어난 성능을 보여주었습니다. 머신러닝 기술이 기후 예측 정확성을 높이는 데 기여할 수 있음을 입증하였으며, 1년 이상의 예측을 가능하게 하였습니다. 이로 인해 복잡한 기후 시스템의 이해와 예측 가능성을 증대시킬 수 있는 잠재력이 확인되었습니다.



### Self-Supervised Frameworks for Speaker Verification via Bootstrapped Positive Sampling (https://arxiv.org/abs/2501.17772)
Comments:
          submitted to IEEE/ACM TASLP in January 2025

- **What's New**: 이 논문은 Self-Supervised Positive Sampling (SSPS)이라는 새로운 샘플링 기법을 소개합니다. SSPS는 기존의 음성 인식 시스템이 서로 다른 녹음 조건의 양성 샘플을 효과적으로 샘플링할 수 있도록 도와줍니다. 이는 speaker representation의 intra-class variance를 줄이고 채널 정보를 감소시켜 더 강력한 표현을 학습할 수 있게 합니다.

- **Technical Details**: SSPS는 현재의 SSL 프레임워크에서 더 다양한 양성 샘플을 선택하는 부트스트랩 방식으로 작동합니다. 이 방법은 다른 음성 발화에서 유사한 speaker identity를 가진 샘플을 찾아내어 학습의 정확성을 높입니다. 또한, SSPS는 nearest neighbors와 clustering 기반의 다양한 샘플링 알고리즘을 통해 구현됩니다.

- **Performance Highlights**: 실험 결과, SSPS를 적용한 SimCLR 및 DINO는 VoxCeleb1-O에서 각각 2.57% 및 2.53%의 EER을 달성했습니다. SSPS는 또한 intra-class variance를 줄여 speaker representation의 안정성을 향상시키고 데이터 증강의 의존성을 낮추는 효과가 있었습니다. 이러한 개선은 모든 주요 SSL 프레임워크에서 SV 성능을 일관되게 향상시켰습니다.
...
### Decision-Theoretic Approaches in Learning-Augmented Algorithms (https://arxiv.org/abs/2501.17701)
- **What's New**: 이번 논문에서는 머신 러닝 예측을 통한 알고리즘 설계 및 분석에서의 의사 결정 이론 지표(decision-theoretic metrics)의 체계적인 연구를 시작했습니다. 기존 연구들이 극단적인 오류 값에 집중하는 반면, 본 연구는 거리 기반 평가와 같은 결정론적(deterministic) 측정법과 불확실성을 고려한 확률적(stochastic) 측정법을 도입하여 알고리즘의 성능을 균형 있게 평가하는 방법을 제시합니다.

- **Technical Details**: 제안된 접근 방식은 이상적인 해답에 가까운 정도를 정량화할 수 있는 거리 기반 평가와, 불완전한 오라클(oracle)과 관련된 위험을 고려해 알고리즘의 성능과 리스크 간의 균형을 맞출 수 있는 확률적 측정법을 포함합니다. 이를 통해 알고리즘 성능을 예측 오류의 전 범위에 걸쳐 정량화할 수 있게 됩니다.

- **Performance Highlights**: 이러한 기법은 자원 할당과 온라인 의사결정과 같은 두 가지 잘 알려진 문제에 적용되었습니다. 특히 계약 일정(contract scheduling) 및 1-max 검색(1-max search) 문제를 분석하여 알고리즘 성능의 향상된 평가를 보여주었습니다.
...
### Machine-Learning-Enhanced Optimization of Noise-Resilient Variational Quantum Eigensolvers (https://arxiv.org/abs/2501.17689)
Comments:
          14 pages, 3 figures, contribution to the 41st International Symposium on Lattice Field Theory (Lattice 2024), July 28th - August 3rd, 2024, Liverpool, UK

- **What's New**: 최근 연구에서 Gaussian Processes (GPs)와 Bayesian Optimization (BO)을 활용하여 Variational Quantum Eigensolver (VQE)의 고전적 최적화를 개선할 것을 제안하였습니다. 이러한 머신러닝 기법들은 잡음이 있는 데이터를 처리하는 데 적합하여, VQE의 성능을 향상시킬 수 있습니다. 본 연구에서는 새로운 알고리즘의 추가적인 통찰과 수치 실험 결과를 제시하며, 하드웨어 잡음과 오류 완화가 성능에 미치는 영향을 분석합니다.

- **Technical Details**: VQE는 매개변수화된 양자 회로의 고전적 변분 최적화를 통해 양자 해밀토니안의 바닥 상태를 근사하는 데 사용됩니다. 특정 조건에서 VQE의 목표 함수의 기능적 형태를 명시적으로 도출하고 이를 통해 효율적인 최적화 절차 개발이 가능하다는 점을 강조합니다. NFT 알고리즘은 1차원 부분 공간에서 한 번에 하나의 파라미터를 최적화하기 위해 두 개의 측정을 사용하여 최적화 효율성을 높이는 방법입니다.

- **Performance Highlights**: GP 및 BO를 통합한 EMICoRe 알고리즘은 NFT 알고리즘의 성능을 초과하여, 하드웨어 잡음이 존재하는 환경에서도 우수한 결과를 보여주었습니다. 교란이 있는 양자 하드웨어의 고전적 시뮬레이션을 통해 EMICoRe의 성능을 검증하였으며, 이는 향후 실제 양자 하드웨어와 격자 장 이론 설정에 기법을 적용하는 데에 기반이 될 것입니다.
...
### Extracting Inter-Protein Interactions Via Multitasking Graph Structure Learning (https://arxiv.org/abs/2501.17589)
Comments:
          Submit

- **What's New**: 본 논문은 단백질-단백질 상호작용(PPI) 예측을 위한 새로운 방법인 MgslaPPI를 제안합니다. 이 방법은 그래프 주의 메커니즘을 활용해 단백질의 내부 구조 정보를 효과적으로 추출하고, 다중 작업 학습(multi-task learning) 전략을 적용하여 단백질 인코더의 표현력을 강화합니다. 기존의 PPI 예측 방법들이 단백질 서열 연구에 편향되었던 것에 비해, MgslaPPI는 단백질 구조를 고려하여 예측 성능을 향상시킵니다.

- **Technical Details**: MgslaPPI는 최종적인 PPI 예측 과정을 두 단계로 나눕니다: 아미노산 잔기 재구성(A2RR)과 단백질 상호작용 예측(PIP). A2RR 단계에서는 그래프 주의 네트워크(Graph Attention Network, GAT)를 사용하여 아미노산 잔기 간의 내부 관계를 탐색합니다. PIP 단계에서는 그래프 합성 신경망(Graph Convolutional Network, GCN)을 통해 단백질 간의 상호작용 정보를 추출하고, 추가적으로 단백질 특징 재구성(Protein Feature Reconstruction, PFR) 및 마스킹된 상호작용 예측(Masked Interaction Prediction, MIP) 작업을 도입합니다.

- **Performance Highlights**: 실험 결과, MgslaPPI는 다양한 데이터 분할 방식에서 기존의 최첨단 방법들을 크게 능가한다는 것을 보여주었습니다. 특히, MgslaPPI는 저비용으로 메모리 소비를 줄이고, 모델의 학습 부담을 경감시키는 동시에 정확한 구조적 기능을 획득하는 데 효과적입니다. 따라서 MgslaPPI는 기존의 PPI 예측 방법들을 보완하고, 구조 기반 예측 연구를 더욱 풍부하게 만듭니다.
...
### Sequential Learning of the Pareto Front for Multi-objective Bandits (https://arxiv.org/abs/2501.17513)
- **What's New**: 이번 연구는 다목적 다중 팔대의 Pareto front를 순차적으로 학습하는 문제를 다룹니다. 에이전트는 K개의 가능한 팔 중 하나를 선택하고 벡터 형태의 보상을 받습니다. 연구의 목표는 $	ext{1 - } oldsymbol{	ext{δ}}$의 확률로 정확한 답변을 제공하는 알고리즘을 설계하는 것입니다.

- **Technical Details**: 우리는 위험(위험도) $oldsymbol{	ext{δ}}$가 작을 때 최적의 샘플 복잡성을 달성하는 알고리즘의 효율적인 구현을 제안합니다. K개의 팔이 있고 d차원에서 p개가 Pareto 집합에 속하는 경우, 이 알고리즘은 매 라운드마다 O(Kp^d)의 시간복잡도로 실행됩니다.

- **Performance Highlights**: 제안된 알고리즘은 에이전트가 충분한 정보를 갖춘 후 빠르게 Pareto front를 식별할 수 있도록 돕습니다. 이 알고리즘은 복잡도 측면에서 효율적이며, 실제 응용 프로그램에서 실용적입니다.



### A Survey on Cluster-based Federated Learning (https://arxiv.org/abs/2501.17512)
- **What's New**: 이번 논문에서는 Federated Learning (FL)의 최신 연구에서 클러스터 기반 해결책(cluster-based solutions, CFL)을 중점적으로 분석합니다. 특히, 고객 데이터를 도메인 지식을 통해 분리할 수 있는 경우 없으면 PFL (개인화된 FL)의 필요성을 강조합니다. 또한, CFL의 개인화를 위한 분류 체계와 문헌 리뷰를 제안합니다.

- **Technical Details**: CFL은 FL에서 클라이언트의 데이터를 비독립적(non-independent)이고 동일하게 분포되지 않은(non-IID) 상황에서 효율적으로 활용하기 위해 개발되었습니다. 이 접근법은 여러 개의 가설을 학습할 수 있도록 하여 모델이 다양한 사용자 요구에 맞게 적응할 수 있게 합니다. 논문은 CFL의 다양한 솔루션을 정리하고, 이들에 대한 문헌 검토를 통해 기존의 연구들을 체계적으로 정리합니다.

- **Performance Highlights**: CFL 솔루션은 FL의 단일 모델 접근 방식의 한계를 극복하고, 다양한 클라이언트 요구에 적절한 맞춤형 모델을 제공할 가능성을 보여줍니다. 기존 연구와의 비교 및 대안적 사용 사례를 검토함으로써, CFL의 유용성을 강조하고 향후 연구 방향성을 제시합니다.
...
### Certificated Actor-Critic: Hierarchical Reinforcement Learning with Control Barrier Functions for Safe Navigation (https://arxiv.org/abs/2501.17424)
Comments:
          Accepted to ICRA 2025

- **What's New**: 이 논문에서는 Control Barrier Functions (CBFs)의 한계점을 극복하기 위한 새로운 모델 프리 강화 학습 알고리즘인 Certificated Actor-Critic (CAC)을 제안합니다. CAC는 CBFs로부터 유도된 명확한 보상 함수를 도입하여 내비게이션 안전성을 정량적으로 평가합니다. 또한, 안전한 정책을 학습한 후 이를 개선하는 계층적 강화 학습 프레임워크를 개발하였습니다.

- **Technical Details**: 논문에서는 무한 지평선 마르코프 결정 과정 (MDP)을 기본으로 하여, 해당 시스템의 동역학을 설명하는 수학적 정의를 제공합니다. CAC 알고리즘은 안전성 및 목표 도달 능력을 통합하는 계층적 프레임워크를 설계하여 기존의 모델 기반 제어 설계 방법에서 발생하는 한계를 해결하려고 합니다. 특히, CBFs로부터 파생된 보상 함수가 안전성을 수량적으로 평가할 수 있게 해줍니다.

- **Performance Highlights**: 실험 결과를 통해 CAC 알고리즘의 효과성이 입증되었습니다. 연구자는 두 개의 시뮬레이션 실험을 수행하여 제안된 알고리즘의 성능을 비교 분석하였으며, 목표 달성 능력 향상과 안전성 유지를 동시에 달성할 수 있음을 보여주었습니다. 이를 통해 CAC는 기존 방법들에 비해 더 나은 안전성과 성능을 제공함을 입증하였습니다.
...
### Poisoning Attacks and Defenses to Federated Unlearning (https://arxiv.org/abs/2501.17396)
Comments:
          To appear in The Web Conference 2025

- **What's New**: 본 논문에서는 BadUnlearn이라는 새로운 공격 유형을 소개합니다. 이 공격은 federated unlearning(FU) 프로세스를 겨냥하여, 악의적인 클라이언트가 설계된 로컬 모델 업데이트를 서버에 전송하고, 이를 통해 최종적으로 발생하는 unlearned 모델이 여전히 감염된 상태로 남아있도록 합니다. 기존의 FU 방법들은 이러한 공격의 보안 문제를 고려하지 않고 있었습니다.

- **Technical Details**: BadUnlearn 공격은 최적화 문제로 구성되어 있으며, 서버의 집계 규칙에 의존하여 악성 업데이트를 조작합니다. 이를 해결하기 위해 각 악성 업데이트를 학습된 모델의 잡음이 섞인 버전으로 모델링합니다. 이러한 공격을 차단하기 위한 프레임워크 UnlearnGuard를 제안하며, 이 프레임워크는 클라이언트의 로컬 모델 업데이트를 추정하고, 제공된 업데이트가 정확한지를 검증하기 위한 필터링 전략을 포함하고 있습니다.

- **Performance Highlights**: 광범위한 실험 결과를 통해 BadUnlearn 공격이 기존 FU 방법들을 효과적으로 손상시키는 것을 보였습니다. 반면, 저희가 제안한 UnlearnGuard는 이러한 공격으로부터 확실히 안전한 성능을 유지함을 입증하였습니다. 이로 인해 UnlearnGuard가 악성 공격에 저항력이 있는 견고한 FU 프레임워크라는 것을 이론적으로 증명하였습니다.



### Byzantine-Robust Federated Learning over Ring-All-Reduce Distributed Computing (https://arxiv.org/abs/2501.17392)
Comments:
          To appear in The Web Conference 2025

- **What's New**: 본 논문은 Byzantine 공격에 저항하는 RAR 기반의 연합 학습(FL) 알고리즘인 BRACE를 소개합니다. 기존의 SC 아키텍처에서 발생하는 단일 실패 지점과 통신 병목 현상을 해결하며, RAR의 장점을 유지하면서도 효과적인 보안을 제공합니다. BRACE는 클라이언트들이 서로 직접 통신하는 링 구조를 이용하며, 이는 이상적인 데이터 교환을 가능하게 합니다.

- **Technical Details**: BRACE 알고리즘은 클라이언트가 1비트 양자화를 적용하여 악의적인 데이터에 대한 민감성을 줄이며, 이웃 클라이언트와 서브 벡터를 교환하고 합의 메커니즘을 도입합니다. 이러한 방법으로 BRACE는 모델의 업데이트가 다수의 합의에 기반하여 이루어지도록 하여, Byzantine 공격의 영향을 최소화합니다. 더불어, 알고리즘은 통신 효율성을 유지하며 원활한 스케일링을 보장합니다.

- **Performance Highlights**: BRACE는 이론적으로 𝒪(1/T) 수렴 속도를 가지며, 실험을 통해 두 개의 실제 데이터 세트에서 Byzantine 공격에 효과적으로 저항하는 동시에 통신 비용을 줄이는 것으로 검증되었습니다. 이는 RAR 기반 FL 시스템에서의 통신 효율성을 높이며, 알고리즘의 실용성을 입증합니다.



### Do We Really Need to Design New Byzantine-robust Aggregation Rules? (https://arxiv.org/abs/2501.17381)
Comments:
          To appear in NDSS 2025

- **What's New**: 이 논문에서는 Federated Learning(FL) 환경에서의 poisoning attacks에 대한 새로운 방어 메커니즘인 FoundationFL을 제안합니다. 기존의 Byzantine-robust aggregation 방법을 보강하여 FL 시스템의 내구성을 향상시키는 것을 목표로 합니다. FoundationFL은 클라이언트로부터 받은 로컬 모델 업데이트 후 서버가 합성 업데이트를 생성하는 방식을 사용하여, 안전성을 높이는 혁신적인 접근법입니다.

- **Technical Details**: FoundationFL은 서버가 각 클라이언트의 로컬 업데이트와 결합하여 합성 업데이트를 생성하는 방법을 사용합니다. 이를 통해 Trimmed-mean이나 Median과 같은 기존의 Byzantine-robust foundational aggregation 규칙을 적용하여 모델을 통합합니다. 서버는 각 클라이언트의 업데이트가 극단적인 업데이트와 얼마나 밀접한지를 평가하여, 가장 크게 벗어난 클라이언트의 업데이트를 선택해 합성 업데이트의 기초로 사용합니다.

- **Performance Highlights**: FoundationFL은 6개의 데이터셋에서 12개의 다양한 poisoning attack 시나리오에 대해 평가되었으며, 기존의 Byzantine-robust FL 방법들을 초월하는 성능을 보였습니다. 이 연구는 FoundationFL이 FL 환경 내에서 저항력을 유지할 수 있음을 이론적으로 입증하였으며, 실험 결과는 FoundationFL의 유효성을 강력히 지지합니다.



### Fundamental Computational Limits in Pursuing Invariant Causal Prediction and Invariance-Guided Regularization (https://arxiv.org/abs/2501.17354)
Comments:
          70 pages, 3 figures

- **What's New**: 이 논문은 이질적인 환경에서 불변 예측(invariant prediction)을 추구하는 것이 인과성(causality)을 데이터 기반으로 학습하는 데 중요한 기회를 제공한다는 점을 강조합니다. 기존의 방법들은 계산적으로 어려운 문제에 직면했으며, 특히 NP-하드(NP-hard) 문제로서 가장 간단한 선형 인과관계(linear causal relationship)마저 해결하는 데 한계를 지니고 있음을 보여줍니다.

- **Technical Details**: 저자들은 새로운 방법론을 제안하여 추가 조건 하에 계산적(computationally) 및 통계적(statistically) 효율성을 동시에 달성할 수 있는 추정기(estimator)를 제공합니다. 이 추정기는 불확실한 세트에서 타원형(ellipse-shaped) 구조를 가지며, 불변 방향(invariant directions)에 비해 비불변 방향(spurious directions)에 더 많은 불확실성을 할당합니다. 이를 통해 가장 예측성이 높은 솔루션과 인과성 솔루션 간의 원활한 보간(smooth interpolation)이 가능합니다.

- **Performance Highlights**: 이론적 결과와 함께 비비대칭성(non-asymptotic) 결과 및 실험적(empirical) 응용 사례가 제시되어, 제안된 방법의 유효성을 뒷받침합니다. 특히, 논문에서 제안한 추정기는 주어진 조건 하에서 계산적 효율성을 확보하고 있으며, 이는 인과성 탐지(causal discovery)와 강건 전이 학습(robust transfer learning) 분야에서 중요한 의미를 가집니다.



### Testing Conditional Mean Independence Using Generative Neural Networks (https://arxiv.org/abs/2501.17345)
Comments:
          18 pages. 4 figures

- **What's New**: 이번 논문에서는 조건 평균 독립성(Conditional Mean Independence, CMI) 테스트를 위한 새로운 모집단 CMI 측정값과 부트스트랩 기반의 테스트 절차를 소개합니다. 이 절차는 심층 생성 신경망(deep generative neural networks)을 활용하여 모집단 측정과 관련된 조건 평균 함수를 추정하고, 비록 느리게 감소하는 비모수적 추정 오류가 테스트의 점근적 정확성에 영향을 미치지 않도록 설계되었습니다. 고차원 공변량 및 반응 변수가 포함된 시나리오에서 강력한 실증적 성능을 보여주며, 다변량 응답을 처리할 수 있는 가능성도 갖추고 있습니다.

- **Technical Details**: CMI 테스트는 공변량과 반응 변수가 다변량인 경우에 대한 접근법을 제시합니다. 우리는 랜덤 벡터 X∈ℝdX, Y∈ℝdY, Z∈ℝdZ의 조건부 평균 독립성을 테스트하며, 특정한 실험 조건을 정의하여 공변량이 반응 변수를 의미 있게 예측하는지를 평가합니다. 이러한 접근 방법은 깊은 신경망을 통해 높은 차원의 데이터를 처리하고 관련 없는 특징들을 제거하는 데 도움을 준다는 장점을 갖고 있습니다.

- **Performance Highlights**: 조건 평균 독립성 테스트는 머신러닝 응용에서 공변량의 중요성을 평가하는 유용한 도구로 작용합니다. 우리의 방법은 실제 이미지 데이터 응용 사례에서도 뛰어난 효율성과 적응성을 보여줍니다. 또한, 비모수적 회귀를 위한 새로운 방법론이 고차원적 환경에서도 유효한 성능을 발휘할 수 있도록 만들어 주었습니다.
...
### A Guaranteed-Stable Neural Network Approach for Optimal Control of Nonlinear Systems (https://arxiv.org/abs/2501.17333)
- **What's New**: 이번 논문에서는 최적 제어 문제에 Neural Network (NN)을 통합한 Neural Optimization Machine (NOM)을 개발하여 비선형 시스템의 최적 제어 입력을 생성하는 방법을 제안하고 있습니다. 기존의 온라인 최적화 방안이 제한된 계산 자원에서 비현실적일 수 있는 문제를 해결하고자, NOM은 NN 훈련 문제로 변환하여 비선형, 비볼록 최적화 문제를 해결합니다.

- **Technical Details**: NOM은 NN의 내장 역전파 알고리즘을 활용하여 비선형 제어 시스템을 위한 최적화 문제를 풀도록 설계되었습니다. 이 방식은 NN이 NOM에서 생성된 데이터를 기반으로 훈련되어 닫힌 루프 시스템에서 안정성과 추적 성능을 보장하도록 합니다. 또한, 기존 최적화 문제 해결 방식에 비해 기존 솔버의 한계를 극복할 수 있는 가능성을 보여줍니다.

- **Performance Highlights**: 제안된 방법론의 효과는 시뮬레이션과 실험 조사를 통해 입증되었습니다. 연구에서 보여준 바와 같이, NOM을 통해 훈련된 NN은 안정성 및 상태 수렴 성질을 보장하면서 비선형 시스템 제어의 적용 가능성을 높였습니다. 이러한 접근은 다양한 비선형 제어 시스템의 통제를 위한 실질적인 수단으로 특히 유용할 것입니다.
...
- **What's New**: 이 논문은 접근성 애플리케이션을 위한 고품질의 콤팩트한 신경망 기반 text-to-speech (TTS) 시스템을 개발하여, 15ms의 낮은 지연 시간과 저용량의 디스크 공간을 제공한다고 소개합니다. 기존의 TTS 솔루션들은 자연스러움과 음질을 잃으면서 낮은 지연 시간을 제공하는 반면, 최근의 클라우드 기반 신경 TTS 시스템들은 높은 자연스러운 음성을 제공하지만 지연 시간 문제가 발생합니다. 본 연구의 TTS 시스템은 이러한 문제를 해결하고 저전력 장치에서도 실행 가능하다는 장점을 가지고 있습니다.

- **Technical Details**: 제안된 시스템은 전통적인 TTS 시스템이 사용하는 통계적 접근법 대신, Transformer 기반의 인코더-디코더 아키텍처와 FastSpeech2 (FS2) 모델을 채택하여 음성을 생성합니다. 프론트엔드 및 VOCODER 구성 요소의 최적화를 통해 디스크 공간 사용을 줄이고, 모델의 속도를 향상시키며, 모델 성능이 크게 감소하지 않도록 하였습니다. WaveRNN을 사용하여 고품질 음성을 생성하였으며, 이를 효율적으로 리소스 제약이 있는 환경에서도 운영할 수 있도록 조정하였습니다.

- **Performance Highlights**: 제안된 시스템은 5백만 개의 예제와 36시간의 음성 녹음을 기반으로 훈련되었으며, Mean Opinion Score (MOS) 테스트를 통해 평가한 결과, 신뢰할 수 있는 자연스러운 음성을 생성하며 디스크 공간을 4배 줄이는 성과를 달성했습니다. 또한, 연령, 대화 등을 포함한 다양한 도메인에서 모델의 품질이 일관되게 유지되는 것을 보여주었습니다. 추가적으로, 각 구성 요소의 최적화가 디스크 풋프린트와 지연 시간 개선에 미친 영향을 분석한 결과도 포함되어 있습니다.
...
### RLPP: A Residual Method for Zero-Shot Real-World Autonomous Racing on Scaled Platforms (https://arxiv.org/abs/2501.17311)
Comments:
          This paper has been accepted for publication at the IEEE International Conference on Robotics and Automation (ICRA), Atlanta 2025. The code is available at: this http URL

- **What's New**: 본 논문에서는 RLPP라는 잔여 강화 학습(residual Reinforcement Learning, RL) 프레임워크를 제안하여, 기존의 PP 컨트롤러의 성능을 향상시킵니다. RLPP는 RL 기법을 통해 주어진 상황에서 더욱 정밀하게 컨트롤러의 성능을 조정할 수 있도록 돕습니다. 이는 기존의 시뮬레이션 기반 RL 기법이 실제 환경에서 성능 저하를 겪는 문제를 해결하기 위한 중요한 접근 방식입니다.

- **Technical Details**: 이 연구는 F1TENTH 플랫폼을 활용하여, 잔여 RL 방식의 컨트롤러를 물리적 플랫폼에 최초로 효과적으로 구현하였습니다. RLPP는 전통적인 PP 컨트롤러에 ML 기반의 정책을 추가하여, 시스템 식별의 필요성을 제거하고 단순한 조정만으로 성능을 극대화할 수 있습니다. RLPP는 타이어 특성을 고려하지 않고도 효과적으로 작동할 수 있으며, 이는 제어의 복잡성을 줄이는 데 기여합니다.

- **Performance Highlights**: RLPP 프레임워크는 F1TENTH 플랫폼에서 최대 6.37%의 랩 타임 향상을 달성하였으며, 기존 방법에 비해 52% 이상의 성능 향상을 보여줍니다. 실제 환경에서도 실시간 배치에서 신뢰할 수 있는 성능을 발휘하여, Sim-to-Real 환경에서의 변별력을 크게 개선했습니다. 논문에서는 RLPP 구현 코드를 오픈 소스로 제공하여, 자율 주행 연구 분야의 추가적인 탐색과 발전을 장려합니다.



### Summary of the NOTSOFAR-1 Challenge: Highlights and Learnings (https://arxiv.org/abs/2501.17304)
- **What's New**: NOTSOFAR-1 챌린지는 실제 비즈니스 환경의 요구를 반영한 새로운 데이터셋을 제공하여 DASR(원거리 자동 음성 인식) 연구에 중요한 이정표를 세웠습니다. 이 챌린지는 30개 다양한 환경에서 녹음된 280회의 미팅과 1000시간의 시뮬레이션 데이터셋을 포함하여, 실제 사운드 조건과 대화의 역동성을 포착했습니다. 이는 참가자들이 새로운 접근 방식을 탐색하고 혁신을 촉진하는 기회를 제공합니다.

- **Technical Details**: NOTSOFAR-1 챌린지는 단일 채널(SC) 트랙과 고정 기하학적 다채널(MC) 트랙의 두 가지 주요 트랙으로 구성되어 있습니다. 참가자들은 각 트랙에 제출할 수 있으며, 이는 그들의 시스템을 종합적으로 평가할 수 있는 기회를 제공합니다. 챌린지는 280개의 고품질 영어 미팅과 1000시간의 시뮬레이션 데이터를 새로운 데이터셋으로 소개하여, 모두 DASR 시스템의 벤치마킹에 기여합니다.

- **Performance Highlights**: 제출된 시스템들의 성능은 시간 제한 최소 순열 단어 오류율(tcPER)과 최적 참조 조합 단어 오류율(tcorcWER)을 기반으로 평가되었습니다. 많은 참가팀들이 기본 성능을 개선했으며, 특히 단일 채널 시스템과 다채널 시스템 간의 성능 차이를 확인하였습니다. 이 챌린지는 DASR 분야에서 이전의 데이터셋과 알고리즘 개선을 위한 중요하고 새로운 통찰력을 제공합니다.
...
### MR imaging in the low-field: Leveraging the power of machine learning (https://arxiv.org/abs/2501.17211)
Comments:
          To appear as a book chapter in T. Küstner et al, "Machine Learning in MRI: From Methods to Clinical Translation"

- **What's New**: 최근 저자기장(Magnetic Resonance Imaging, MRI) 하드웨어 및 소프트웨어의 혁신이 저자기장(<1 T) 및 초저자기장(<0.1 T) MRI에 대한 관심을 다시 불러일으켰습니다. 이러한 기술들은 낮은 전력 소모, 감소된 특정 흡수율(specific absorption rate, SAR), 향상된 균일성, 비용 효율성 등의 이점을 제공하여 자원이 제한된 환경에서도 유망한 대안이 될 수 있습니다. 하지만 저자기장 MRI에는 신호 대 잡음비(signal-to-noise ratio, SNR)가 낮아져 공간 해상도(spatial resolution)나 스캔 시간이 길어질 수 있는 내재된 도전이 있습니다.

- **Technical Details**: 저자기장 및 초저자기장 MRI는 기존의 높은 자기장 MRI에서 겪는 몇 가지 한계를 해결할 가능성을 보이고 있으며, 특히 특정 머신러닝(ML) 기법을 통해 이러한 한계에 대응할 수 있습니다. 이 논문에서는 딥 뉴럴 네트워크(deep neural networks)를 활용해 저자기장 MRI의 성능을 향상시키는 다양한 ML 기반 솔루션에 대해 설명하고 있습니다. 특히 고급 이미지 재구성, 노이즈 제거(denoising), 초해상도(super-resolution) 알고리즘이 논의되어 ML이 저자기장 MRI에 어떻게 적용될 수 있는지를 설명합니다.

- **Performance Highlights**: 저자기장 MRI의 SNR 감소는 이미지 품질을 저해하는 중요한 요소이며, 이를 극복하기 위한 여러 ML 기법이 제안되고 있습니다. 머신러닝 기법을 활용한 이미지 재구성 및 분석 자동화는 저자기장 MRI의 진단 및 효율성을 향상시키는 데 기여할 수 있습니다. 또한, 저자기장 MRI의 접근성을 높이고 임상 적용을 넓힐 수 있는 가능성을 제시하며, 이는 다양한 의료 환경에서 변화의 혁신을 가져올 수 있습니다.
...
### Near-Optimal Algorithms for Omniprediction (https://arxiv.org/abs/2501.17205)
- **What's New**: 이번 연구에서는 오믹프레딕션(Omniprediction)을 위한 거의 최적의 학습 알고리즘을 제시합니다. 오믹프레딕터는 가설 클래스 $	ext{H}$에 대해 손실 함수 클래스 $Ł$ 내에서 손실을 최소화하는 예측 기능으로, 온라인 및 오프라인 설정에서의 적용 가능성을 보여줍니다. 특히, Lipschitz 손실 함수의 클래스를 위한 온라인 학습 알고리즘이 주목할 만한 성과를 나타냈습니다.

- **Technical Details**: 제시된 알고리즘은 $Ł$와 $	ext{H}$의 조합에 대해 $	ilde{O}(	ext{sqrt}(T 	ext{log} |	ext{H}|))$의 후회(regret)를 달성합니다. 이 후회 경계는 단일 손실 함수의 최소화에 대한 최적 후회와 유사하여, $	ext{sqrt}(	ext{log}(T))$ 요인이 제외됩니다. 또한, 오프라인 학습 알고리즘으로의 전환 방법이 개발되어, Bounded Variation 손실 내에서 효과적인 오믹프레딕터를 생성하는 데 기여하고 있습니다.

- **Performance Highlights**: 제안된 알고리즘은 모든 유계 손실 함수뿐만 아니라 다양한 손실 함수 클래스에 대해 거의 최적의 복잡성을 보여줍니다. (오프라인) 경험적 리스크 최소화(ERM) 오라클과 $m$개의 샘플을 활용하여, Rademacher 복잡도에 비례하여 효율적인 오믹프레딕터를 반환하는 성과를 기대할 수 있습니다. 이 연구는 오믹프레딕션을 위한 학습 알고리즘의 가능성을 넓히는 중요한 기초를 제공합니다.
...
### Deep Learning in Wireless Communication Receiver: A Survey (https://arxiv.org/abs/2501.17184)
Comments:
          16 Pages, 9 Figures

- **What's New**: 이번 논문은 복잡하고 동적 환경에서의 신호 처리를 향상시키기 위해 심층 신경망(Deep Neural Networks, DNNs)을 활용한 무선 수신기 설계의 변화를 다룹니다. 기존의 전통적인 무선 수신기는 데이터로부터 학습하거나 적응하지 못하는 수학적 모델과 알고리즘에 의존합니다. 반면, 딥러닝 기반 수신기는 데이터를 학습하고 그에 따라 적응할 수 있어 현대 무선 통신 시스템에 더 적합합니다. 이 연구는 무선 수신기 설계에서 다양한 딥러닝 아키텍처의 적용과 그 기능 강화를 탐구합니다.

- **Technical Details**: 무선 통신에서 수신기는 전송된 신호를 처리하여 원본 데이터를 복구합니다. 전통적인 수신기는 각기 다른 신호 처리 작업을 수행하는 모듈들로 구성되어 있으며, 이 모듈에는 동기화, 채널 추정, 보정, 공간-시간 디코딩, 복조 및 디코딩 등이 포함됩니다. 또한, 주목해야 할 점은 MIMO 시스템에서의 공간-시간 코딩의 중요성으로, 이는 통신의 신뢰성을 높이고 데이터 전송률을 개선할 수 있는 가능성을 제공합니다. 다양한 최신 알고리즘과 hybrid methods의 통합 등을 통해 이러한 모듈의 성능을 향상시킬 수 있습니다.

- **Performance Highlights**: 논문은 딥러닝 기반 수신기가 현대 무선 통신의 다양한 시스템에서 어떻게 성능을 향상시킬 수 있는지를 보여줍니다. 특히 OFDM, MIMO 및 차세대 네트워크에서의 적용 사례를 논의하며, 딥러닝이 전통적인 방법에 비해 뛰어난 성능을 발휘함을 강조합니다. 다양한 딥러닝 아키텍처, 즉 MLPs, CNNs, RNNs, GANs 및 autoencoders의 특별한 기능은 통신의 복잡성 문제를 효과적으로 해결하는 데 기여하고 있습니다. 이러한 발전은 결국 향후 무선 통신 기술의 혁신을 위한 새로운 가능성을 열어줍니다.
...
### Deep Learning in Early Alzheimer's disease's Detection: A Comprehensive Survey of Classification, Segmentation, and Feature Extraction Methods (https://arxiv.org/abs/2501.15293)
Comments:
          22 pages

- **What's New**: 최근 연구에서는 알츠하이머병(Alzheimer's disease)의 조기 탐지 및 치료가 중요하다는 점을 강조했습니다. 이 논문은 딥러닝(Deep Learning) 알고리즘을 활용하여 알츠하이머 병을 조기에 식별하는 방법을 검토하고, 공개 데이터셋을 기반으로 하는 접근 방식을 제시했습니다. 또한, 현재의 문헌에서는 알츠하이머병 예측을 위한 기계 학습(Machine Learning) 적용에 대한 부족한 부분을 지적했습니다.

- **Technical Details**: 이 연구에서는 CNN(Convolutional Neural Network) 및 RNN(Recurrent Neural Network)과 같은 딥러닝 기술을 사용하여 알츠하이머병의 분류 정확도가 최대 96%에 달한다고 보고했습니다. 또한, 가벼운 인지 장애(mild cognitive impairment, MCI) 전환 예측의 경우 84.2%의 정확성을 달성했습니다. 다양한 데이터 채널을 통해 알츠하이머병 탐지의 효과를 분석하고, 기능 세분화(feature segmentation) 및 분류 방법(classification methods)을 적용하여 자가 진단 방법을 제안했습니다.

- **Performance Highlights**: 딥러닝 기술이 기존의 기계 학습 기법보다 높은 성과를 내고 있으며, 이는 고차원 데이터에서 복잡한 구조를 식별하는 능력에 기인합니다. 또한, 알츠하이머병 진단을 위한 연구에서 2015년 세계 알츠하이머병 보고서에 따르면, 전 세계적으로 4,680만 명이 치매로 고통 받고 있으며, 2030년까지 7,470만 명으로 증가할 것으로 예상됩니다. 이 논문은 알츠하이머병 탐지에서의 연구 격차와 한계도 식별하여 향후 연구에 대한 방향성을 제시했습니다.

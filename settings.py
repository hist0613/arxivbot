...
_PROMPT_SUMMARIZATION_INSTRUCTION = """Please summarize the paper abstract into 3 **Korean** sentences. Please provide the summary sentences with a line break ("1. ", "2. ", and "3. ") Feel free to include some technical keywords in English itself. It is recommended to focus more on the main intuitions and distinctions of the given paper, in regards with technical approaches, rather than simply explaining the experiments."""

PROMPT_SUMMARIZATION = f"""{_PROMPT_SUMMARIZATION_INSTRUCTION}
...
{_PROMPT_SUMMARIZATION_INSTRUCTION}
...
1. 최근 deep model들이 NLP 태스크에서 사람보다 나은 정확성을 보였으나, spurious pattern에 의존하는 문제 때문에 강건성이 제한된다고 보고되고 있다. 
2. 기존 방법들은 사람이 counterfactual을 만들거나 모델이 데이터셋에서 counterfactual 비슷한 것들을 찾아야 했으나, 여전히 spurious correlation에 영향을 받는다는 문제가 있다.
3. 이 논문에서는 "여러 개의" counterfactual을 생성하고, 집합적 의사 결정을 통해 더 robust하게 단어들의 인과관계를 파악하는 방법을 제안한다. 
